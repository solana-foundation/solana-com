...
import { createCrossAppRewrites } from "@workspace/app-topology";
...
describe("Cross-app rewrites", () => {
  it("matches every route and asset proxy generated by the topology manifest", () => {
    const sortRewrites = <T extends { source: string; destination: string }>(
      rewrites: T[],
    ) =>
    ...
    expect(sortRewrites(beforeFiles)).toEqual(
      sortRewrites(createCrossAppRewrites(APP_URLS)),
    );
  });
