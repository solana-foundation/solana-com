      ...
      >
        <div className="p-4 md:p-12 ">
          <h2 className="nd-heading-l flex items-center">
            <SolanaMono className="size-7 md:size-14 mr-2.5 md:mr-6 mt-[-4px]" />
            {title}
          </h2>
          <div className="grid grid-cols-3 gap-1 mt-6 md:mt-10 relative">
            {tabs?.map((item) => (
              <button
                key={item.id}
                className={cn(
                  "flex flex-col items-start gap-3 md:gap-10 p-3 md:p-6 rounded-xl bg-nd-border-light hover:bg-nd-border-prominent text-nd-mid-em-text-alpha hover:text-nd-cta backdrop-blur-[8px]",
          ...
          <div className="overflow-hidden mt-3 md:mt-8">
            {activeLinks?.map((item, index) => (
              <a
                key={index}
                className="group flex flex-row gap-3 md:gap-4 py-3 md:p-5 w-full text-nd-mid-em-text-alpha hover:text-nd-cta hover:border-nd-border-hovered border-b last:border-b-0 border-nd-border-light"
                href={item.href}
                target="_blank"
