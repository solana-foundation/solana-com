...
type WalletFilterState = WalletFilter & { checked: boolean };
...
export interface WalletFiltersProps {
  ...
  updateWallets: (_filters: Record<string, boolean>) => void;
  walletData: WalletEntry[];
  category: (typeof CATEGORIES)[number]["key"];
  updateCategory: (_category: (typeof CATEGORIES)[number]["key"]) => void;
}
...
const WalletFilters = ({
   ...
   * @returns Object
   */
  const setFilterInitialState = (): WalletFilterState[] => {
  ...
  const toggleFilterActiveState = (index: number) => {
