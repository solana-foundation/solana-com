...
/* ── the globe ──
   Ported from the CTA reference (github.com/saugatxyz/solana-cta). The wireframe
   is one path of meridians; at mount it is split on every `M` into subpaths,
   each is sampled at a fixed gap, and a dot is set travelling along it. Dots
   near the pointer speed up, swell and glow. */
...
export default function Globe({
  ...
  gap = 490,
  speed = 1,
  accelerateOnHover = true,
  dotColor = "#ffffff",
}: {
  dotSize?: number;
  gap?: number;
  speed?: number;
  accelerateOnHover?: boolean;
  dotColor?: string;
}) {
  ...
  useEffect(() => {
    ...
    const glowSVG = glowRef.current;
    const dotLayer = dotLayerRef.current;
    const stack = stackRef.current;
    if (!basePath || !glowSVG || !dotLayer || !stack) return;
    ...
    subpaths.forEach((sp, subIdx) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        c.setAttribute("r", String(dotSize));
    ...
    const cursor = { x: Infinity, y: Infinity, inside: false };
    const toSvg = (cx: number, cy: number) => {
      const pt = glowSVG.createSVGPoint();
      pt.x = cx;
      pt.y = cy;
      const m = glowSVG.getScreenCTM();
      return m ? pt.matrixTransform(m.inverse()) : { x: Infinity, y: Infinity };
    };
    ...
    const tick = (now: number) => {
      ...
      lastT = now;
      for (const d of dots) {
        const sp = subpaths[d.subIdx];
        if (!sp) continue;
        let stride = sp.length / (LAP_SECONDS / speed);
        let grow = 1;
        let glow = 1;
        if (accelerateOnHover && cursor.inside) {
          ...
          if (dist < INFLUENCE_RADIUS) {
            ...
            const influence = t * t;
            stride *= 1 + SPEED_BOOST * influence;
            grow *= 1 + (SIZE_BOOST - 1) * influence;
            glow *= 1 + (GLOW_BOOST - 1) * influence;
          }
        }
    ...
    return () => {
      ...
      subpaths.forEach((sp) => sp.path.remove());
    };
  }, [dotSize, gap, speed, accelerateOnHover, dotColor]);
