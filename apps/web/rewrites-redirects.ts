...
export default {
  rewrites: {
    beforeFiles: [
      ...
      // Ask Solana (docs AI agent) proxy. The docs app owns the route handler
      // that injects X-Ask-Proxy-Secret before forwarding to the bot service.
      {
        source: "/api/ask/:path*",
        destination: `${DOCS_APP_URL}/api/ask/:path*`,
        locale: false,
      },
