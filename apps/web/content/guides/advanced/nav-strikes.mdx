...
### Token Metadata Best Practices for Fund Shares

When creating fund share tokens, proper metadata is essential for regulatory
compliance, NAV tracking, and integration with institutional systems.

#### Required Metadata Fields

The Token-2022 Metadata extension requires these core fields:

| Field    | Description                      | Example                                 |
| -------- | -------------------------------- | --------------------------------------- |
| `name`   | Full name of the fund            | "ACME Prime Money Market Fund"          |
| `symbol` | Short ticker symbol (2-10 chars) | "ACME-MMF"                              |
| `uri`    | Link to off-chain metadata JSON  | "https://fund.com/shares/acme-mmf.json" |

#### Recommended Additional Fields for Fund Shares

For money market funds and similar products, include these custom fields using
`additionalMetadata`:

```typescript
const initialMetadata: [string, string][] = [
  ["currentNAV", "1.000000"], // Current Net Asset Value
  ["lastStrikeTime", new Date().toISOString()], // Last NAV calculation
  ["strikeSchedule", JSON.stringify(["09:30", "12:00", "15:00", "16:00"])],
  ["totalAUM", "0.00"], // Assets Under Management
  ["fundType", "Money Market Fund"], // Fund classification
  ["baseCurrency", "USD"], // Settlement currency
  ["minInvestment", "1000"], // Minimum subscription
  ["managementFee", "0.25"], // Annual fee percentage
  ["cusip", "123456789"], // CUSIP identifier
  ["lei", "549300EXAMPLE0000000"] // Legal Entity Identifier
];
```

#### Off-Chain Metadata JSON Structure for Funds

The `uri` field should point to a JSON file with extended fund metadata:

```json
{
  "name": "ACME Prime Money Market Fund",
  "symbol": "ACME-MMF",
  "description": "A stable value money market fund investing in high-quality short-term instruments",
  "image": "https://fund.com/assets/acme-mmf-logo.png",
  "external_url": "https://fund.com/funds/acme-prime-mmf",
  "attributes": [
    { "trait_type": "Fund Type", "value": "Money Market" },
    {
      "trait_type": "Investment Objective",
      "value": "Current Income + Capital Preservation"
    },
    { "trait_type": "Base Currency", "value": "USD" },
    { "trait_type": "NAV Frequency", "value": "Multiple Daily Strikes" },
    { "trait_type": "Minimum Investment", "value": "$1,000" },
    { "trait_type": "Management Fee", "value": "0.25%" }
  ],
  "properties": {
    "fund_documents": {
      "prospectus": "https://fund.com/docs/prospectus.pdf",
      "factsheet": "https://fund.com/docs/factsheet.pdf",
      "kiid": "https://fund.com/docs/kiid.pdf"
    },
    "regulatory": {
      "sec_registration": "333-XXXXXX",
      "fund_administrator": "State Street",
      "auditor": "Deloitte"
    }
  }
}
