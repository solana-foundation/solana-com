...
- **민트 권한**: 새 토큰 생성을 제어합니다. 모든 토큰 계정에 토큰을 발행할 수
  있습니다. 초기 공급량이 생성된 후 고정 공급량 토큰을 만들기 위해 종종
  취소됩니다.
...
- **계정 소유자**: 토큰 계정에 대한 완전한 제어 권한을 가집니다. 토큰 전송,
  소각, 위임자 승인, 잔액이 0일 때 계정 폐쇄가 가능합니다.
- **폐쇄 권한**: 토큰 계정의 잔액이 0일 때 계정을 폐쇄할 수 있습니다. 기본적으로
  계정 소유자가 이 권한을 가지지만 다른 계정에 위임할 수 있습니다.
...
명령어는 민트와 토큰 계정의 권한을 변경하거나 취소합니다. 현재 권한을 가진
주체만 자신의 권한을 새 주소로 이전하거나 권한을 null로 설정하여 영구적으로
취소할 수 있습니다. 한번 취소된 권한은 복원할 수 없습니다.

<Callout type="info">
  [Token
  Program](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L153)과
  [Token Extensions
  Program](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/instruction.rs#L195)은
