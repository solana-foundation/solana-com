---
title: 네이티브 동기화
description: 네이티브 SOL을 래핑된 SOL로 변환하는 방법을 알아보세요.
---

## 네이티브 SOL 동기화

래핑된 SOL(WSOL)은 계정의 lamport 수량을 토큰 잔액으로 추적하는 토큰 계정입니다.
WSOL은 SOL을 SPL 토큰으로 전송하기 위해 DeFi 프로토콜과의 통합을 가능하게
합니다.
...
명령어는 래핑된 SOL(WSOL) 토큰 계정 잔액을 실제로 저장된 SOL(lamport)과
동기화합니다. 네이티브 SOL을 WSOL 토큰 계정으로 전송할 때, 토큰 잔액은 자동으로
