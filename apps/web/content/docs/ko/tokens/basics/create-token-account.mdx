...
## 연결된 토큰 계정(Associated Token Account)이란 무엇인가요?

연결된 토큰 계정(ATA)은 지갑이 특정 토큰을 보유하기 위한 기본 토큰 계정입니다.
이는
...
ATA는 결정론적 주소를 사용하여 특정 민트에 대한 지갑의 토큰 계정을 쉽게 찾을 수
있게 합니다.
...
계정 주소를 수동으로 추적할 필요가 없어지며, 항상 올바른 주소를 도출할 수
있습니다.

<Callout type="info">
  **Associated Token Program의 작동 방식**

Associated Token Program은 다음과 같은 도움을 제공합니다:
...
이 단일 명령어는 Cross Program Invocation(CPI)을 통해 token account를 생성하고
초기화하는 과정을 자동으로 처리합니다.
