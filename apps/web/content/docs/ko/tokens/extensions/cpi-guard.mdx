...
Cross Program Invocation(CPI)을 통한 예상치 못한 토큰 전송으로부터 토큰 계정을
보호합니다. 이 확장 기능이 활성화되면, 토큰은 오직 Token Extensions Program의
전송 명령어를 직접 호출해서만 전송될 수 있으며, 다른 모든 프로그램이 CPI를 통해
토큰을 전송하는 것을 차단합니다.
