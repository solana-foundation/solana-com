...
- **maximum_pending_balance_credit_counter**: 보류 중인 잔액을 사용 가능한
  잔액으로 변환하기 위해 _rs`ApplyPendingBalance`_ 명령어가 필요하기 전까지
  허용되는 보류 중인 크레딧의 개수 제한입니다.

- **expected_pending_balance_credit_counter**: 마지막으로
  _rs`ApplyPendingBalance`_ 명령어가 처리되었을 때 명령어 데이터를 통해
  클라이언트가 제공한 `pending_balance_credit_counter` 값입니다.
...
모든 입금 및 전송 금액은 처음에 보류 중인 잔액에 추가됩니다. 토큰 계정 소유자는
보류 중인 잔액을 사용 가능한 잔액으로 변환하기 위해 _rs`ApplyPendingBalance`_
...
기밀 보류 중인 잔액은 `pending_balance_lo`와 `pending_balance_hi`로 분할됩니다.
...
1. 클라이언트는 현재 보류 중인 잔액과 사용 가능한 잔액을 조회하고, 합계를
   암호화한 다음, 토큰 계정 소유자의 AES 키를 사용하여 암호화된
   `decryptable_available_balance`를 제공합니다.

2. 예상 및 실제 보류 중인 크레딧 카운터는 _rs`ApplyPendingBalance`_ 명령어가
   ...
   - `actual_pending_balance_credit_counter`: _rs`ApplyPendingBalance`_ 명령어가
     처리되는 시점에 토큰 계정에 있는 `pending_balance_credit_counter` 값
...
예상 카운터와 실제 대기 중인 잔액 카운터가 다를 경우, 다음 단계에 따라
`decryptable_available_balance`를 조정하세요:

1. 토큰 계정의 `decryptable_available_balance`부터 시작합니다
2. 카운터 차이(실제 - 예상)만큼 입금 및 전송 명령을 포함하는 최근 트랜잭션을
   가져옵니다:
   - 입금 명령의 공개 금액을 추가합니다
   - 전송 명령의 대상 암호문 금액을 복호화하여 추가합니다
