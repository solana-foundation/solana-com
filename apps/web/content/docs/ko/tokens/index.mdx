...
각 token account의 데이터에는 token account에 대한 권한을 가진 주체를 식별하는
`owner` 필드가 포함되어 있습니다. 이는 모든 token account에 대해 Token
Program으로 지정된 기본 [Account](/docs/core/accounts#account-type) 타입의
프로그램 소유자와는 다릅니다.
...
_sh`spl-token`_ CLI는 SPL 토큰을 실험하는 데 도움을 줍니다. 아래 예제는
...
Playground 지갑을 mint 권한으로 하는 mint account 주소를 출력합니다.
...
Token Program의 _rs`MintTo`_ 명령어는 새로운 토큰을 생성합니다. 민트 권한자가
트랜잭션에 서명해야 합니다. 이 명령어는 token account에 토큰을 민트하고 mint
