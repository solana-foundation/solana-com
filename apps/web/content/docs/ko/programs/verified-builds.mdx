...
> 무료 RPC의 속도 제한에 걸릴 수 있으므로 유료 RPC URL을 사용하는 것이
> 권장됩니다. 따라서 `-um` 대신 더 안정적인 검증을 위해 `--url yourRpcUrl`를
> 사용해야 합니다.

`--remote` 플래그는 OtterSec API에 빌드 요청을 보내고, 이는 프로그램의 원격
빌드를 트리거합니다. 빌드가 완료되면, 시스템은 체인상의 프로그램 해시가
저장소에서 생성된 빌드 결과물의 해시와 일치하는지 확인합니다.
...
이는 `Cargo.lock` 파일에 지정된 솔라나 버전을 사용하여 도커 컨테이너를 통해 검증
가능한 빌드를 생성합니다.
...
이 멀티시그 가이드의 나머지 부분에서는 예시 프로그램 ID인
...
프로그램 권한을 로컬에 가지고 있는 경우, `solana-verify verify-from-repo` 명령을
사용할 때 빌드 데이터를 체인상에 업로드하라는 메시지가 표시됩니다.
...
`commit-hash` 매개변수를 사용하여 저장소 작업을 계속하려는 경우 특정 커밋을
정의할 수도 있습니다: `--commit-hash 5b82b86f02afbde330dff3e1847bed2d42069f4e`
...
`--remote` 명령은 OtterSec API에 빌드 요청을 보내 프로그램의 원격 빌드를
...
실행하거나 프로그램의 배포 권한과
[verify 프로그램](https://explorer.solana.com/address/verifycLy8mB96wd9wqq3WDXQwM4oU6r42Th37Db9fC)에서
파생된
[PDA](https://explorer.solana.com/address/63XDCHrwZu3mXsw2RUFb4tbNpChuUHx4eA5aJMnHkpQQ/anchor-account)에
저장된 온체인 검증 데이터를 사용하여 `verify-from-repo` 명령으로 로컬에서 직접
프로그램 검증을 실행할 수 있습니다.
...
구현되면 `security.txt`는 검증 PDA에 저장된 검증 데이터에 쉽게 액세스할 수
있도록 검증자 공개 키를 보유하게 됩니다. 프로그램을 빌드하고 검증하는 데 필요한
모든 정보를 포함하는 PDA는 프로그램 주소와 검증자 pubkey에서 파생됩니다.
기본적으로 이는 프로그램을 빌드하고 배포한 것과 동일한 pubkey입니다. 하지만
`security.txt`에 지정할 수 있는 다른 pubkey일 수도 있습니다.

`security.txt` 기능을 통해 개발자는 Solana 스마트 컨트랙트 내에 연락처 및 보안
정보를 직접 포함할 수 있습니다. [securitytxt.org](https://securitytxt.org)에서
영감을 받은 이 접근 방식은 보안 연구원이 컨트랙트 주소만 알고 있어도 프로젝트
...
`security.txt` 파일을 포함하면 보안 연구원이 올바른 담당자에게 쉽게 연락할 수
있어 잠재적인 취약점을 방지하고 적시에 버그 리포트를 받을 수 있습니다.
...
쉽게 조회할 수 있어, 잠재적인 문제를 보고하려는 모든 사람이 연락처 및 보안 세부
정보를 확인할 수 있습니다.
...
- 검증: 배포하기 전에 `query-security-txt` 도구를 사용하여 형식과 내용을
  검증하세요. 이 도구는 온체인 프로그램과 로컬 바이너리 모두를 검증할 수
  있습니다:
