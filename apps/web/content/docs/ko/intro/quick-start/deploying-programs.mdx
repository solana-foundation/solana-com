...
블록체인에 빌드, 배포 및 테스트해보세요. 이 가이드는 Solana Playground에 더
익숙해지고 Program Derived Address(PDA) 및 Cross Program Invocation(CPI)을
사용할 준비를 하는 데 도움이 될 것입니다.
...
`new_account` 계정의 `data` 필드에 접근하여 `data: u64`의 값으로 설정합니다
...
**참고:** 원하는 경우, 아래 스크린샷과 같이 왼쪽 패널에 있는 **빌드**와 **배포**
버튼을 사용할 수 있습니다.
...
4. `solana confirm -v [TxHash]`를 실행하여 트랜잭션 로그 확인
...
<Callout type="warn">
  프로그램은 업그레이드 권한이 있는 계정에서만 종료하거나 업데이트할 수
  있습니다. 프로그램이 배포될 때 Playground 지갑이 자동으로 각 프로그램의
  업그레이드 권한을 갖게 됩니다. 업그레이드 권한이 취소되면 프로그램은 변경
  불가능한 상태가 됩니다.
</Callout>

축하합니다! Anchor를 사용하여 첫 번째 Solana 프로그램을 구축하고 배포했습니다.
...
이제 첫 번째 프로그램을 배포했으므로
[프로그램 파생 주소(PDA)](/docs/intro/quick-start/program-derived-address)와
[크로스 프로그램 호출(CPI)](/docs/intro/quick-start/cross-program-invocation)을
탐색할 준비가 되었습니다.
