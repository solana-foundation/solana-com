...
- **상태**: 읽고 유지하기 위한 데이터입니다. 예를 들어, 토큰에 관한 정보, 사용자
  데이터 또는 프로그램 내에 정의된 기타 데이터가 있습니다.
- **실행 가능한 프로그램**: Solana 프로그램의 실제 코드를 포함하는 계정입니다.
  이러한 계정은 사용자가 호출할 수 있는 명령어를 저장합니다.
...
실행 가능한 프로그램 계정에는 계정의 `owner`로 지정된 프로그램도 있습니다.
