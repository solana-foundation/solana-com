...
Également appelées `transaction cost`, c'est une estimation complète de toutes
les ressources nécessaires pour traiter une transaction, mesurée en
...
- L'entrée a été générée après une durée de temps
...
Le compte de frais dans la transaction est le compte qui paie pour le coût
...
Un [hash](#hash) de [l'état du validator](#bank-state) à une certaine
[hauteur de tick](#tick-height). Il comprend l'affirmation d'un
...
[client mince](#thin-client) et moins qu'un [validator](#validator).
...
Voir [Preuve d'histoire](#proof-of-history-poh).
...
Voir aussi [exempté de rent](#rent-exempt) ci-dessous. En savoir plus sur le
rent ici : [Qu'est-ce que le rent ?](/docs/intro/rent).
...
Un [block](#block) ou [slot](#slot) qui a atteint le [lockout](#lockout) maximum
sur un [validator](#validator). La root est le bloc le plus élevé qui est un
ancêtre de toutes les branches actives sur un validator. Tous les blocs ancêtres
d'une root sont aussi transitivement une root. Les blocs qui ne sont ni un
ancêtre ni un descendant de la root sont exclus de la considération pour le
consensus et peuvent être écartés.
...
Une fraction d'un [block](#block) ; la plus petite unité envoyée entre les
...
petit et que S soit un scalaire dans la plage de [0, l-1]. Cette exigence
...
slots de leader dans l'epoch actuel. Cette métrique peut être trompeuse car elle
présente une forte variance après la limite d'epoch lorsque la taille de
...
slot ignoré n'apparaîtra pas comme un ancêtre pour les blocs des slots suivants,
n'incrémentera pas la [hauteur de bloc](#block-height), et n'expirera pas le
plus ancien `recent_blockhash`.

On ne peut déterminer si un slot a été ignoré que lorsqu'il devient plus ancien
que le dernier slot [enraciné](#root) (donc non-ignoré).
...
Un compte qui peut produire (ou 'émettre') des tokens. Les différents tokens
sont distingués par leurs adresses d'émission de token uniques.
...
pour transférer, geler et émettre des tokens.
...
utilisant une ou plusieurs [keypair](#keypair) et exécutées de manière atomique
...
de son exécution, pouvant ensuite être vérifiée en moins de temps qu'il n'en a
...
pendant lesquels il devient progressivement effectif. Pendant cette période,
l'enjeu est considéré comme "en activation". Plus d'informations sur :
