...
[Quantifier la décentralisation](https://news.earn.com/quantifying-decentralization-e39db233c28e).
...
leur exécution pour minimiser les frais.
...
est un ancêtre de toutes les branches actives sur un validateur. Tous les blocs
...
séquentiellement et non-chevauchants, comprenant approximativement le même temps
réel selon [PoH](#proof-of-history-poh).
...
sur l'état du cluster comme la hauteur actuelle du tick, les valeurs de
...
Le [Token Program](https://spl.solana.com/token) a l'ID de programme
...
qu'elle a été exécutée, qui peut ensuite être vérifiée en moins de temps qu'il
...
Un certain nombre d'[epochs](#epoch) après qu'un [stake](#stake) a été délégué
pendant lesquels il devient progressivement effectif. Durant cette période, le
stake est considéré comme "en cours d'activation". Plus d'informations sur :
