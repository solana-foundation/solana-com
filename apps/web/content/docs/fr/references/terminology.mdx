...
0,000001 lamport par unité de calcul) arrondi au lamport supérieur.

Les transactions devraient demander le minimum d'unités de calcul nécessaires à
...
Un [bloc](#block) ou un [slot](#slot) qui a atteint le [verrouillage](#lockout)
...
ancêtres d'une racine sont aussi transitivement une racine. Les blocs qui ne
...
première signature dans une transaction peut être considérée comme
...
de slots de leader, mais peut également être utile pour identifier les mauvaises
...
slot omis n'apparaîtra pas comme un ancêtre pour les blocs des slots suivants,
...
séquentiellement et non-chevauchants, comprenant approximativement des durées
égales en temps réel selon le [PoH](#proof-of-history-poh).
...
Jetons confisqués au [cluster](#cluster) si un comportement malveillant d'un
...
via un compte Sysvar (clé publique) ou en interrogeant via un appel système.
...
Le [Token Extensions Program](https://spl.solana.com/token-2022) a l'ID de
programme `TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb` et inclut toutes les
