...
vous permet de définir un taux d'intérêt directement sur un compte mint. Les
intérêts sont composés en continu en fonction de l'horodatage du réseau, mais le
solde réel de tokens stocké dans chaque compte de tokens reste inchangé. La
valeur totale (solde de tokens plus intérêts accumulés) n'est qu'un calcul.
Aucun nouveau token n'est créé avec cette extension.
