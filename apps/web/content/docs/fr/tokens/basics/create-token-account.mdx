...
<Callout type="info">
  Notez que dans le code source, les développeurs appellent un compte de jeton
  ...
  partagent la même implémentation de base pour le compte de jeton.
</Callout>
...
- Circle contrôle le jeton via l'autorité propriétaire du token account :
...
Les token accounts résultants sont détenus par le Token Program et utilisent la
...
La création d'un token account nécessite l'instruction
