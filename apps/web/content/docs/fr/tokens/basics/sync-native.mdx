---
title: Sync Native
description: Apprenez comment convertir des SOL natifs en SOL encapsulés.
---

## Synchroniser le SOL natif

Le SOL encapsulé (WSOL) est un compte de token qui suit le montant de lamports
dans le compte comme solde de token. Le WSOL permet l'intégration avec les
protocoles DeFi pour transférer le SOL comme un token SPL.
...
synchronise le solde d'un compte de token SOL encapsulé (WSOL) avec les SOL
(lamports) réellement stockés. Lorsque vous transférez des SOL natifs vers un
