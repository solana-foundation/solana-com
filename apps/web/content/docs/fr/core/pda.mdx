...
avec une valeur différente peut toujours dériver une PDA valide.
...
compte avec une adresse dérivée de programme. Le programme inclut une seule
...
[bump seed](mention:bump) utilisés pour dériver la PDA.
...
utilisera `find_program_address` pour dériver la PDA et trouver le bump.
...
<Callout type="warn">
  Si vous invoquez l'instruction `initialize` à nouveau avec la même adresse
  `user` comme seed, la transaction échouera. Cela se produit parce qu'un compte
