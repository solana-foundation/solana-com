...
Les programmes (contrats intelligents) définissent la logique métier pour chaque
instruction. Lorsque vous soumettez une transaction, le runtime Solana exécute
chaque instruction en séquence et de manière atomique (ce qui signifie que soit
toutes les instructions réussissent, soit la transaction entière échoue).
...
Demandez un airdrop de SOL pour financer le `sender`.
...
Créer un `Connection` pour gérer l'envoi des transactions et la récupération des
données de compte.
...
Générer de nouvelles paires de clés à utiliser comme comptes émetteur et
récepteur.
...
de mint account et détenu par le Token Extensions Program.

### !initializeMintInstruction

Construire l'instruction pour initialiser les données du nouveau compte en tant
que type de mint account.
