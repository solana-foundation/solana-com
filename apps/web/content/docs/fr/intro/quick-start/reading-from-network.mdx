...
L'exemple ci-dessous génère une [nouvelle paire de clés](mention:keypair),
...
Pour les comptes de portefeuille, ce champ est vide (0 octets).
...
Le champ `lamports` contient le solde en SOL du compte, exprimé en
...
entre les comptes de portefeuille et les comptes de programme. Le compte de
programme stocke le bytecode compilé pour le
...
Le Token Program est un compte de programme exécutable. Les programmes ont les
mêmes champs sous-jacents que tous les
...
d'un jeton spécifique. Cela inclut l'offre totale, le nombre de décimales et les
comptes autorisés à créer ou geler des jetons. L'adresse du mint account
...
Le champ `data` du mint account stocke l'état, pas de code exécutable, donc le
champ `executable` est `false`.
...
dans le type de données `Mint`, ce qui est montré dans
...
accounts. Un token account gelé ne peut pas transférer ou brûler le jeton qu'il
contient.
...
Le champ `tlvData` contient des données supplémentaires pour Token Extensions et
nécessite une désérialisation supplémentaire. Ce champ n'est pertinent que pour
les comptes créés par le [Token Extension Program](/docs/tokens/extensions)
(Token2022).
