...
Program Derived Addresses (PDA) et les Cross Program Invocations (CPI).
...
Invoque le System Program pour créer un nouveau compte et désigne ce programme
...
1. Compilez le code du programme
...
2. Ajoutez du SOL à votre portefeuille _(Facultatif)_
...
3. Déployez le programme sur le réseau
...
<Callout type="info">
  Lors du déploiement, le bytecode du programme est stocké dans un compte de
...
1. Ouvrez le fichier de test nommé `tests/anchor.test.ts`. Ce fichier démontre
...
3. Stockez le hash de transaction (`TxHash`) inclus dans la sortie
...
4. Exécutez `solana confirm -v [TxHash]` pour voir les journaux de transaction
...
Fermez le programme pour récupérer le SOL alloué au programme sur la chaîne.
Pour ce faire, exécutez `solana program close [ProgramID]`, en utilisant
l'adresse du programme stockée dans la macro _rs`declare_id!()`_.
...
<Callout type="warn">
  ...
  ayant l'autorité de mise à niveau. Votre portefeuille Playground devient
  automatiquement l'autorité de mise à niveau pour chaque programme lors de son
  déploiement. Si l'autorité de mise à niveau est révoquée, le programme devient
  immuable.
</Callout>

Félicitations ! Vous avez construit et déployé votre premier programme Solana en
utilisant Anchor.
