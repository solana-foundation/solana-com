...
léger pour tester les programmes Solana. Il fournit une interface simple pour
tester les instructions des programmes Solana dans un environnement minifié de
la Machine Virtuelle Solana (SVM). Tous les comptes de test doivent être
...
Solana basique en utilisant Mollusk.

<ScrollyCoding>

## !!steps Programme Hello World

Cet exemple démontre comment tester un programme Solana basique en utilisant
...
| [`process_and_validate_instruction`](#single-instruction-with-checks)          | Traite une instruction et effectue une série de vérifications sur le résultat, déclenchant une panique si une vérification échoue.            |
| [`process_instruction_chain`](#multiple-instructions)                          | Traite plusieurs instructions et retourne le résultat.                                                                                        |
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | Traite plusieurs instructions et effectue une série de vérifications sur chaque résultat, déclenchant une panique si une vérification échoue. |
...
personnalisée pour modifier ses valeurs lors des tests.
...
modifier les autres valeurs sysvar de la même manière.
