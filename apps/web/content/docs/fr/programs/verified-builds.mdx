...
> Important : il s'agit du `lib name`, pas du nom du package !
...
> ceux-ci ne produiront très probablement pas le même hash et votre vérification
> échouera donc.
...
> programme, en utilisant `verify-from-repo` vous devez ajouter le flag
> `--mount-path`. Ce sera le chemin vers le dossier contenant le `Cargo.toml`
> qui contient le nom de la bibliothèque de votre programme.
...
> rencontrer des limites de taux des RPC gratuits. Donc au lieu de `-um` vous
...
correspond au hash de l'artefact de compilation généré à partir de votre dépôt.
...
télécharger les données de compilation sur la chaîne lors de l'utilisation de la
...
chemin exécutable dans le conteneur est identique au hash du programme en chaîne
fourni à la commande. Étant donné que la compilation a déjà été téléchargée vers
une image, il n'est pas nécessaire de reconstruire entièrement l'exécutable, ce
...
branche principale. Vous pouvez également définir un commit spécifique au cas où
vous souhaiteriez continuer à travailler sur le dépôt en utilisant le paramètre
