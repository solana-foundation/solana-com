...
<Callout type="info">
  Хотя этот баланс называется rent, он больше похож на депозит, так как весь
  баланс может быть возвращен, когда аккаунт закрывается. (Название "rent"
  происходит от теперь уже устаревшего поля [rent epoch](#rent-epoch).)
</Callout>
