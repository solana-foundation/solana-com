...
Считывайте данные из сети Solana, запрашивая различные аккаунты. Этот раздел
поможет вам понять структуру Solana [аккаунтов](/docs/core/accounts). Каждый
аккаунт Solana имеет уникальный [адрес](/docs/core/accounts#account-address),
который используется для поиска соответствующих данных в блокчейне. Аккаунты
содержат либо
...
Кошелёк — это аккаунт, принадлежащий
...
впервые отправляется на новый адрес, системный аккаунт создаётся автоматически.
...
[извлекает данные аккаунта](mention:info) для вновь профинансированного
кошелька.
...
является полем в [типе аккаунта](/docs/core/accounts#account-structure) самом по
...
кошельковыми и программными аккаунтами. Программный аккаунт хранит
скомпилированный байт-код исходного кода Token Program
...
[программе загрузчику](/docs/core/programs#loader-programs). В этом примере
...
Program, который включает инструкции, определяющие, как создавать и обновлять
аккаунты mint.
...
созданных [Token Extension Program](/docs/tokens/extensions) (Token2022).
