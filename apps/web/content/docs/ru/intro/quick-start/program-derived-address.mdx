...
значения экономит вычисления, так как не требуется пересчитывать его в
...
пользователями, и содержит три поля:
...
- `bump` — _rs`u8`_, который хранит ["bump" seed](/docs/core/pda#canonical-bump)
  ...
  вычисления, так как не требуется пересчитывать его в последующих инструкциях.
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - `space` выделяет необходимое количество байт для поля данных аккаунта
...
В этом случае аккаунт _rs`user`_ получает lamport'ы с закрытого
_rs`message_account`_.
...
_rs`close`_ в структуре _rs`Delete`_ отвечает за закрытие аккаунта.
...
[`pg.wallet`](mention:two) — к вашему кошельку Playground.
...
по его адресу (`messagePda`) с помощью `fetchNullable`. Этот метод возвращает
null, если аккаунт больше не существует после закрытия.
