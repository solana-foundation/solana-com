...
фреймворк для тестирования программ Solana. Он предоставляет простой интерфейс
для тестирования инструкций программ Solana в минимизированной среде Solana
...
программы Solana с использованием Mollusk.

<ScrollyCoding>

## !!steps Программа Hello World

Этот пример демонстрирует, как протестировать базовую программу Solana с
...
Для тестирования программы Solana с использованием Mollusk:

1. [Создайте экземпляр `Mollusk`](mention:mollusk) — инициализируйте Mollusk с
   программным ID и путем к скомпилированной программе (файл `.so`)
...
пользователи могут также напрямую обращаться к ним и изменять их для большего
контроля.
...
| [`process_and_validate_instruction`](#single-instruction-with-checks)          | Обрабатывает инструкцию и выполняет серию проверок результата, вызывая панику при ошибке проверки.                       |
| [`process_instruction_chain`](#multiple-instructions)                          | Обрабатывает несколько инструкций и возвращает результат.                                                                |
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | Обрабатывает несколько инструкций и выполняет серию проверок для каждого результата, вызывая панику при ошибке проверки. |
...
Следующий пример обрабатывает инструкцию перевода SOL без проверок результата.

<Callout type="info">

Примеры ниже запускают Mollusk в функции `main` для демонстрации. На практике
обычно Mollusk используется в тестовом модуле с аннотацией `#[test]`.
...
Следующий пример обрабатывает инструкцию перевода SOL с проверками валидации.
...
нескольких инструкций без проверок.
...
Следующий пример обрабатывает две инструкции перевода SOL без проверок
валидации.
...
Следующий пример обрабатывает цепочку из двух инструкций перевода SOL с
...
— это обёртка вокруг `Mollusk`, которая поддерживает состояние аккаунта между
вызовами инструкций с помощью `account_store`. Методы обработки инструкций
идентичны методам `Mollusk`.
...
внутренне через свой `account_store`. Это устраняет необходимость в параметре
...
Следующий пример обрабатывает две отдельные инструкции перевода SOL с
сохранением состояния аккаунта между инструкциями через `account_store`.
...
Используйте метод `warp_to_slot`, чтобы обновить sysvar clock и смоделировать
перемещение вперёд или назад во времени до определённого slot.
...
Следующий пример показывает, как напрямую изменить sysvar Mollusk, обратившись к
полю `sysvars` для изменения параметров rent. Другие значения sysvar можно
...
отслеживает использование вычислительных единиц инструкциями программы.
...
чтобы добавить token program, token2022 program (расширения токенов) и
