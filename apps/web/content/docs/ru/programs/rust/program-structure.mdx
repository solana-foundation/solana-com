...
практике для крупных программ рекомендуется разделять код на несколько файлов.
...
<Callout type="warn">
  Обновите поле `edition` в файле `Cargo.toml` до 2021 года. В противном случае
  при сборке программы может возникнуть ошибка.
</Callout>
...
раздел `[lib]`, чтобы настроить сборку программы в Cargo.
...
Solana-программа `entrypoint` имеет следующий сигнатур функции. Разработчики
могут создавать собственную реализацию функции `entrypoint`.
...
вариантов нашего перечисления `CounterInstruction`. Метод Borsh `try_from_slice`
выполняет это преобразование автоматически.
...
Теперь давайте обновим основную функцию `process_instruction`, чтобы направлять
инструкции к соответствующим функциям-обработчикам.

Этот паттерн маршрутизации часто используется в Solana-программах.
...
<Callout type="warn">
  Необходимо запустить `cargo build-sbf` перед запуском тестов, чтобы
...
Вы можете проверить деплой с помощью команды `solana program show` и вашего
program ID:
...
Когда локальный validator запущен, вы можете просматривать транзакции в
...
`http://localhost:8899`, на котором запущен `solana-test-validator`.
