...
защищает токен-аккаунты от неожиданных переводов через Cross Program Invocations
(CPI). При включении это расширение гарантирует, что токены могут быть
переведены только с помощью прямых вызовов инструкции transfer из Token
Extensions Program, блокируя все остальные программы от перевода токенов через
CPI.
