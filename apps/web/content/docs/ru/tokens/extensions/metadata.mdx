...
Программа Token Extensions реализует
[Token Metadata Interface](https://github.com/solana-program/token-metadata/blob/main/interface/src/instruction.rs)
напрямую, позволяя хранить метаданные непосредственно в mint аккаунте. Это
включает имя токена, символ, URI и дополнительные описательные данные. Когда
указатели ссылаются на сам mint, расширения
