...
позволяет установить процентную ставку непосредственно на mint аккаунте.
Проценты непрерывно начисляются на основе сетевого временного штампа, но
фактический баланс токенов, хранящийся в каждом токен аккаунте, остается
неизменным. Общая стоимость (баланс токенов + начисленные проценты) является
только расчетом. С этой расширенной функцией новые токены не создаются.
