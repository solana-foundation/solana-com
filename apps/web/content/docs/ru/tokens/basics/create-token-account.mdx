...
- Этот аккаунт содержит только токены USDC
- Circle управляет токеном через owner authority token account:
...
В результате созданные token accounts принадлежат Token Program и используют
стандартную структуру `Account`.

</Callout>

## Как создать токен-аккаунт

Создание token account требует инструкции
