---
title: Создание токенового mint
description: Узнайте, как создать SPL Token mint.
---

## Что такое аккаунт mint?

Аккаунт mint уникально представляет токен в Solana и хранит его глобальные
метаданные.
...
Каждый токен в Solana имеет аккаунт mint, а адрес этого аккаунта служит
уникальным идентификатором токена.
...
## Как создать аккаунт mint

Для создания аккаунта mint требуется инструкция
...
Подробности реализации можно найти
[здесь](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/processor.rs#L27).

Для создания аккаунта mint необходимо выполнить две инструкции:

1. System Program: Создать аккаунт с выделенным пространством для аккаунта mint
   и передать права собственности Token Program.
2. Token Program: Инициализировать данные аккаунта mint.
