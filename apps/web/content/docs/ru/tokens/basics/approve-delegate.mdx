...
количество токенов с вашего token account. Делегат может переводить токены
только в пределах утверждённой суммы и не может её превышать. Только владелец
token account может одобрить делегата, и у каждого token account может быть
только один делегат одновременно. Любое новое одобрение заменяет предыдущее.
