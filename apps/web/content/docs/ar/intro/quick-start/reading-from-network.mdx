...
فهم بنية [حسابات](/docs/core/accounts) سولانا. يمتلك كل حساب سولانا
...
يقوم المثال أدناه بإنشاء [زوج مفاتيح جديد](mention:keypair)،
...
عندما تقوم بجلب حساب محفظة، تتضمن الاستجابة الحقول الموضحة في مثال الإخراج على
اليمين.
...
يشير حقل `executable` إلى ما إذا كان حقل `data` الخاص بالحساب يحتوي على رمز
...
يحتوي حقل `lamports` على رصيد الحساب من SOL، بوحدة
...
يوضح حقل `owner` البرنامج الذي يمتلك الحساب. بالنسبة للمحافظ، المالك دائمًا هو
...
يوضح حقل `space` عدد البايتات الموجودة في حقل `data`. هذا ليس حقلاً في
...
يقوم المثال أدناه بجلب Token Program لتوضيح الفرق بين حسابات المحفظة وحسابات
البرامج. يخزن حساب البرنامج الشفرة الثنائية المجمعة لـ
...
Token Program هو حساب برنامج قابل للتنفيذ. تمتلك البرامج نفس الحقول الأساسية مثل
جميع [الحسابات](/docs/core/accounts#account-structure)، ولكن مع اختلافات رئيسية.
...
يتضمن ذلك إجمالي العرض، وعدد المنازل العشرية، والحسابات المصرح لها بإنشاء أو
تجميد الرموز. يحدد عنوان حساب mint بشكل فريد رمزًا على شبكة سولانا.

يسترجع المثال أدناه حساب mint الخاص بـ USD Coin لتوضيح كيفية تخزين حالة البرنامج
في حساب منفصل.
...
وعدد المنازل العشرية.

لقراءة بيانات من حساب Mint، يجب عليك فك تسلسل حقل `data` إلى نوع البيانات
`Mint`، كما هو موضح في [المثال التالي](#deserialize-mint-account).
...
حساب mint مملوك من قبل
[Token Program](/docs/references/terminology#token-program). هذا يعني أن حقل
...
الدوال بايتات الحساب الخام إلى أنواع بيانات منظمة، مما يسهل العمل مع بيانات
الحساب.
...
تقوم دالة _ts`getMint()`_ بفك تسلسل حقل `data` الخاص بحساب mint إلى نوع حساب
Mint.
...
يمكنك عرض البيانات المفككة بالكامل لـ
...
يُظهر حقل `supply` العدد الإجمالي للرموز التي تم إنشاؤها. يتم قياس هذه القيمة
بأصغر وحدة للرمز. للحصول على إجمالي المعروض بالوحدات القياسية، قم بتعديل قيمة
حقل `supply` حسب `decimals`.
...
يشير حقل `isInitialized` إلى ما إذا كان حساب mint قد تمت تهيئته. يُستخدم هذا
الحقل كفحص أمان في Token Program.
...
يُظهر حقل `freezeAuthority` الحساب الذي لديه صلاحية تجميد حسابات الرموز. لا يمكن
لحساب الرمز المجمد تحويل أو حرق الرمز الذي يحتويه.
...
يحتوي حقل `tlvData` على بيانات إضافية لـ Token Extensions ويتطلب مزيدًا من فك
التشفير. هذا الحقل مهم فقط للحسابات التي تم إنشاؤها بواسطة
[Token Extension Program](/docs/tokens/extensions) (Token2022).

<CodePlaceholder title="بيانات mint بعد فك التشفير" />
