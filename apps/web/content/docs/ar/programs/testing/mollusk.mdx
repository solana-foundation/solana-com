...
لاستخدام Token Program وToken2022 Program (Token Extensions) و Associated Token
...
| [`process_and_validate_instruction`](#single-instruction-with-checks)          | معالجة تعليمة وإجراء سلسلة من الفحوصات على النتيجة، مع إيقاف التنفيذ في حالة فشل أي فحص.          |
| [`process_instruction_chain`](#multiple-instructions)                          | معالجة تعليمات متعددة وإرجاع النتيجة.                                                             |
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | معالجة تعليمات متعددة وإجراء سلسلة من الفحوصات على كل نتيجة، مع إيقاف التنفيذ في حالة فشل أي فحص. |
...
استخدم طريقة `process_instruction` لمعالجة تعليمة واحدة دون إجراء فحوصات على
النتيجة. يمكنك التحقق من النتائج يدوياً بعد المعالجة.
...
تقوم الأمثلة أدناه بتشغيل Mollusk في دالة `main` لأغراض التوضيح. في الممارسة
العملية، ستستخدم عادةً Mollusk في وحدة اختبار مشروحة بسمة `#[test]`.
...
التحقق. ستوقف هذه الطريقة التنفيذ في حالة فشل أي فحص.
...
فحوصات التحقق بعد كل تعليمة. كل تعليمة لها مجموعة خاصة من الفحوصات التي يجب أن
تنجح.
...
غلافاً حول `Mollusk` يحافظ على حالة الحساب عبر استدعاءات تعليمات متعددة من خلال
`account_store` الخاص به. طرق معالجة التعليمات مطابقة لـ `Mollusk`.

على عكس `Mollusk`، الذي يتطلب تمرير `accounts` إلى كل دالة (مثل
...
استخدم دالة `warp_to_slot` لتحديث ساعة sysvar لمحاكاة التحرك للأمام أو للخلف في
الوقت إلى slot محدد.
...
تُكتب نتائج المعايير القياسية إلى `out_dir` المحدد كملف markdown باسم
`compute_units.md`.
...
لإضافة Token Program وToken2022 Program (Token Extensions) وAssociated Token
...
<Callout type="info">
  يحدد المثال أدناه حسابات الاختبار يدوياً لأغراض التوضيح. يتضمن
  `mollusk-svm-programs-token` أيضاً دوال مساعدة لإنشاء حسابات mint وtoken.
</Callout>
