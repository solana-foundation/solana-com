...
[Account](/docs/core/accounts#account-type) الأساسي، وهو Token Program لجميع
Token Accounts.
...
يساعدك _sh`spl-token`_ CLI على تجربة رموز SPL. تستخدم الأمثلة أدناه طرفية
[Solana Playground](https://beta.solpg.io/) لتشغيل أوامر CLI مباشرة في المتصفح
...
_sh`solana airdrop`_ في طرفية Playground. يمكنك أيضًا الحصول على SOL من devnet
من [صنبور الويب](https://faucet.solana.com/) العام.
...
يُخرج أمر _sh`create-token`_ عنوان mint account مع محفظة Playground الخاصة بك
كصلاحية mint.
...
يُرجع أمر _sh`supply`_ لرمز جديد `0`:
...
تعليمة _rs`MintTo`_ في Token Program تنشئ رموزاً مميزة جديدة. يجب على سلطة mint
...
تعليمة _rs`Transfer`_ في Token Program تتعامل مع تحويلات الرموز المميزة. يجب على
