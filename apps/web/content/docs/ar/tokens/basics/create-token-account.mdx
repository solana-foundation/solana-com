...
يخزن token account رصيدك من رمز معين. يرتبط كل token account بعملة واحدة بالضبط
ويتتبع رصيد الرمز الخاص بك والتفاصيل الإضافية.
...
<Callout type="info">
  لاحظ أنه في الكود المصدري، يطلق المطورون على Token account نوع `Account`.
  يشترك كل من [Token
  ...
  في نفس التنفيذ الأساسي لحساب Token.
</Callout>

للاحتفاظ بالرموز، تحتاج إلى token account لتلك العملة المحددة. يتتبع كل token
account:

1. **العملة (Mint)**: نوع الرمز المحدد الذي يحتفظ به
2. **المالك (Owner)**: السلطة التي يمكنها تحويل الرموز من هذا الحساب
...
- تتحكم Circle في الرمز من خلال سلطة مالك token account:
...
1. **مالك token account**: السلطة التي يمكنها تحويل أو حرق أو تفويض الرموز. يتم
   تخزين هذا في بيانات token account.

2. **مالك البرنامج**: البرنامج الذي يملك الحساب (دائمًا Token Program أو Token
   Extensions Program لحسابات الرموز).

عند العمل مع token accounts، يشير "المالك" عادةً إلى السلطة التي يمكنها تحويل
الرموز، وليس البرنامج الذي يملك الحساب.

</Callout>

## ما هو associated token account؟

associated token account (ATA) هو token account الافتراضي للمحفظة للاحتفاظ برمز
معين. يستخدم عنوانًا محددًا (PDA) تم إنشاؤه بواسطة
...
- إجراء استدعاءات عبر البرامج (CPIs) إلى Token Program
...
يتطلب إنشاء حساب توكن مرتبط (ATA) تعليمة
...
استدعاءات البرامج المتقاطعة (Cross Program Invocation).
