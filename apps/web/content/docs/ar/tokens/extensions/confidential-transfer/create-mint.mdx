...
يحتوي _rs`ConfidentialTransferMint`_ على ثلاثة حقول تكوين:
...
2. **تهيئة ملحق التحويل السري**: استدعاء تعليمة
   ...
   الخاصة ببرنامج Token Extension لتكوين حالة _rs`ConfidentialTransferMint`_
   لحساب mint.

3. **تهيئة mint**: استدعاء تعليمة _rs`Instruction::InitializeMint`_ الخاصة
   ببرنامج Token Extension لتهيئة حالة mint القياسية.
