...
كل برنامج مملوك من قبل برنامج آخر - وهو المحمّل الخاص به. تُستخدم المحمّلات لنشر
البرامج أو إعادة نشرها أو ترقيتها أو إغلاقها. كما تُستخدم لإنهاء البرنامج ونقل
سلطة البرنامج.

هناك حالياً خمسة برامج تحميل، كما هو موضح في الجدول أدناه.

| المحمّل | معرّف البرنامج                                | ملاحظات                                                                                     | رابط التعليمات                                                                                                                               |
| ------- | --------------------------------------------- | ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
...
| v4      | `LoaderV411111111111111111111111111111111111` | قيد التطوير (غير مُصدر)                                                                     | [التعليمات](https://docs.rs/solana-loader-v4-interface/latest/solana_loader_v4_interface/instruction/enum.LoaderV4Instruction.html)          |

قد تكون البرامج المنشورة باستخدام loader-v3 أو loader-v4 قابلة للتعديل بعد
النشر، كما تحدده سلطة الترقية الخاصة بها.

### البرامج المُسبقة التجميع

بالإضافة إلى برامج التحميل، توفر سولانا البرامج المُسبقة التجميع التالية.
...
يعالج برنامج ed25519 تعليمة. يحتوي البايت الأول من التعليمة `u8` على عدد
التوقيعات المراد التحقق منها، متبوعًا ببايت واحد للحشو. بعد ذلك، يتم تسلسل
البنية التالية، واحدة لكل توقيع يراد التحقق منه.
...
يسمح هذا للمستخدم بتحديد أي بيانات تعليمات في المعاملة لبيانات التوقيع والرسالة.
من خلال تحديد متغير نظام تعليمات خاص، يمكن للمستخدم أيضًا استلام بيانات من
المعاملة نفسها.

#### التحقق من استرداد secp256k1

يُستخدم برنامج secp256k1 للتحقق من عمليات استرداد المفتاح العام لـ secp256k1.

| البرنامج         | معرف البرنامج                                 | الوصف                                                           | التعليمات                                                                                        |
| ---------------- | --------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| برنامج Secp256k1 | `KeccakSecp256k11111111111111111111111111111` | يتحقق من عمليات استرداد المفتاح العام لـ secp256k1 (ecrecover). | [التعليمات](https://docs.rs/solana-secp256k1-program/latest/solana_secp256k1_program/index.html) |

يعالج برنامج secp256k1 تعليمة واحدة. يحتوي البايت الأول من التعليمة على عدد
المفاتيح العامة التي سيتم التحقق منها. بعد ذلك، يتم إنشاء البنية التالية مرة
واحدة لكل مفتاح عام، ثم يتم تسلسلها وإضافتها إلى بيانات التعليمة.

```rust
pub struct Secp256k1RecoverOffsets {
    secp_recovery_id_offset: u16,          // offset to recovery id of 1 byte
    secp_recovery_id_instruction_index: u16,// instruction index to find recovery id
    ...
    message_data_size: u16,                // size of message data
    message_instruction_index: u16,        // index of instruction data to get message data
    signature_offset: u16,                 // offset to signature data of 64 bytes
    signature_instruction_index: u16,      // index of instruction data to get signature
}
```
...
يعالج برنامج secp256r1 تعليمة واحدة. البايت الأول من `u8` التعليمة هو عدد
التوقيعات التي سيتم التحقق منها، متبوعًا ببايت واحد للحشو. بعد ذلك، يتم إنشاء
...
<Callout>
  يتم فرض قيم S المنخفضة لجميع التوقيعات لتجنب قابلية التلاعب العرضي بالتوقيع.
</Callout>
