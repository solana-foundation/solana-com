...
يبلغ الحد الأقصى لحجم المعاملات
...
إن `message` الخاص بالمعاملة هو بنية
...
<Callout>
  لتوفير المساحة، لا تخزن المعاملة الأذونات لكل حساب بشكل فردي. بدلاً من ذلك،
  يتم تحديد أذونات الحساب باستخدام `header` و `account_keys`.
</Callout>
...
إن `header` الخاص بالرسالة هو بنية
[`MessageHeader`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/message/mod.rs#L97).
تحتوي على المعلومات التالية:

- [`num_required_signatures`](mention:num_required_signatures): إجمالي عدد
  التوقيعات المطلوبة للمعاملة
- [`num_readonly_signed_accounts`](mention:num_readonly_signed_accounts): إجمالي
  عدد الحسابات للقراءة فقط التي تتطلب توقيعات
- [`num_readonly_unsigned_accounts`](mention:num_readonly_unsigned_accounts):
  إجمالي عدد الحسابات للقراءة فقط التي لا تتطلب توقيعات
...
الخاص بالرسالة من مصفوفة لعناوين الحسابات، يتم إرسالها بتنسيق
[compact array format](/docs/references/terminology#compact-array-format). يشير
بادئة المصفوفة إلى طولها. كل عنصر في المصفوفة هو مفتاح عام، يشير إلى حساب
تستخدمه التعليمات. يجب أن تكون مصفوفة `accounts_keys` كاملة ومرتبة بشكل صارم،
كما يلي:
...
يعتبر `recent_blockhash` الخاص بالرسالة قيمة تجزئة تعمل كطابع زمني للمعاملة
...
<Callout>
  ...
  الحصول على blockhash الحالي وآخر ارتفاع للكتلة الذي سيكون فيه blockhash
  صالحًا.
</Callout>
...
الخاص بالرسالة من مصفوفة لجميع التعليمات المراد معالجتها، يتم إرسالها بتنسيق
[compact array format](/docs/references/terminology#compact-array-format). يشير
بادئة المصفوفة إلى طولها. كل عنصر في المصفوفة هو بنية
[`CompiledInstruction`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/instruction.rs#L22)
ويتضمن المعلومات التالية:
