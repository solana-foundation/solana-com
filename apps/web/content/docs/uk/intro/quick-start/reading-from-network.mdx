...
Читайте дані з мережі Solana, отримуючи різні облікові записи. Цей розділ
допоможе вам зрозуміти структуру [облікових записів](/docs/core/accounts)
Solana. Кожен обліковий запис Solana має унікальну
[адресу](/docs/core/accounts#account-address), яка використовується для пошуку
відповідних даних у блокчейні. Облікові записи містять або
[дані стану, або виконувану програму](/docs/core/accounts#types-of-accounts).

## Отримання облікового запису гаманця

<WithMentions>

Гаманець — це обліковий запис, яким володіє
[System Program](/docs/core/programs#the-system-program). Гаманці в основному
використовуються для зберігання SOL та підписання транзакцій. Коли SOL вперше
надсилається на нову адресу, системний обліковий запис створюється автоматично.
...
[отримує дані облікового запису](mention:info) для щойно поповненого гаманця.
...
Коли ви отримуєте обліковий запис гаманця, відповідь включає поля, показані у
прикладі виводу праворуч.
...
Поле `lamports` містить баланс SOL облікового запису у
...
Поле `owner` показує програму, яка володіє рахунком. Для гаманців власником
...
Поле `rentEpoch` є застарілим полем з неактуального механізму rent. (Це поле
...
Приклад нижче отримує Token Program, щоб продемонструвати різницю між рахунками
гаманців та програмними рахунками. Програмний рахунок зберігає скомпільований
байткод
[вихідного коду](https://github.com/solana-program/token/tree/main/program)
Token Program. Ви можете переглянути цей програмний рахунок у
...
Token Program є виконуваним програмним рахунком. Програми мають ті самі базові
поля, що й усі [рахунки](/docs/core/accounts#account-structure), але з ключовими
відмінностями.
...
Поле `executable` встановлено на `true`, що вказує на те, що поле `data` рахунку
...
## Отримання облікового запису mint

[Обліковий запис mint](https://github.com/solana-program/token/blob/program%40v8.0.0/program/src/state.rs#L16-L30)
є обліковим записом, що належить Token Program і зберігає глобальні метадані для
конкретного токена. Це включає загальну пропозицію, кількість десяткових знаків
та облікові записи, які мають право створювати або заморожувати токени. Адреса
облікового запису mint унікально ідентифікує токен у мережі Solana.

Приклад нижче отримує обліковий запис mint USD Coin, щоб продемонструвати, як
стан програми зберігається в окремому обліковому записі.
...
Облікові записи mint зберігають стан, а не виконуваний код. Вони належать Token
Program, який включає інструкції, що визначають, як створювати та оновлювати
облікові записи mint.
...
емісії (mint authority), загальна пропозиція, кількість десяткових знаків.
...
Перш ніж необроблені байти в полі `data` облікового запису можуть бути осмислено
...
token account. Заморожений token account не може переказувати або спалювати
токени, які він містить.
