...
[`update`](mention:two) та [`delete`](mention:three), які потрібно додати в
наступних кроках.
...
Перед початком виконайте _shell`build`_ у терміналі Playground, щоб перевірити,
що стартова програма успішно збирається.
...
Атрибут _rs`#[account]`_ у програмі Anchor анотує структури, які представляють
дані облікового запису (тип даних для зберігання в полі data акаунта).
...
Поле _rs`message`_ зберігає _rs`String`_, що містить повідомлення користувача.

### !bump

Поле _rs`bump`_ зберігає _rs`u8`_ ["bump" seed](/docs/core/pda#canonical-bump),
який використовується для виведення Program Derived Address (PDA). Зберігання
...
- `bump` - _rs`u8`_, який зберігає ["bump" seed](/docs/core/pda#canonical-bump)
  для виведення Program Derived Address (PDA). Зберігання цього значення
  економить обчислювальні ресурси, усуваючи необхідність перераховувати його в
  наступних інструкціях.

При створенні облікового запису програма серіалізує дані _rs`MessageAccount`_ та
зберігає їх у полі даних нового облікового запису.

Пізніше, при читанні з облікового запису, програма десеріалізує ці дані назад у
тип даних _rs`MessageAccount`_. Розділ тестування демонструє процес створення та
читання даних облікового запису.
...
Обмеження _rs`mut`_ оголошує обліковий запис як змінюваний.
...
Обмеження _rs`seeds`_ визначає необов'язкові вхідні дані, які використовуються
для отримання PDA.
...
Обмеження _rs`space`_ визначає кількість байтів для виділення під поле даних
...
У цьому випадку обліковий запис повинен відповідати користувацькому типу
_rs`MessageAccount`_.

### !program

Тип _rs`Program<'info, T>`_ вимагає, щоб обліковий запис відповідав програмі.

У цьому випадку обліковий запис повинен відповідати типу _rs`System`_, який
посилається на System Program.
...
Атрибут _rs`#[derive(Accounts)]`_ у програмі Anchor позначає структури, які
визначають облікові записи, необхідні для інструкції.

Кожне поле в структурі представляє акаунт, який перевіряється двома способами:

1. Тип облікового запису (наприклад, _rs`Signer<'info>`_ або
   _rs`Account<'info, T>`_), який вказує, який тип облікового запису очікує
   програма
2. Необов'язкові обмеження (наприклад, _rs`#[account(mut)]`_ або
   _rs`#[account(init)]`_), які визначають додаткові вимоги
...
У цьому прикладі структура _rs`Create`_ визначає облікові записи, необхідні для
інструкції _rs`create`_.

1. _rs`user: Signer<'info>`_
   - Представляє користувача, який створює обліковий запис повідомлення
   - Потребує статусу змінного (_rs`#[account(mut)]`_), оскільки оплачує новий
     обліковий запис
   - Повинен підписати транзакцію для схвалення вирахування lamport з цього
     облікового запису

2. _rs`message_account: Account<'info, MessageAccount>`_
   - Новий обліковий запис, який зберігає повідомлення користувача
   - Обмеження `init` створює обліковий запис під час виконання інструкції
   - Обмеження `seeds` та `bump` виводять адресу облікового запису як Program
     Derived Address (PDA)
   - `payer = user` визначає, хто оплачує створення нового облікового запису
   - `space` виділяє необхідні байти для поля даних облікового запису

3. _rs`system_program: Program<'info, System>`_
   - Необхідний для створення облікового запису
   - За лаштунками обмеження `init` викликає System Program для створення нового
     облікового запису з вказаним `space` і змінює власника на поточну програму.
...
Обмеження `seeds` визначає необов'язкові вхідні дані, що використовуються для
отримання PDA.
...
Обмеження `bump` вказує Anchor автоматично знайти та використати правильний bump
seed. Anchor використовує `seeds` та `bump` для отримання PDA.

---

Обчислення `space` _rs`(8 + 32 + 4 + message.len() + 1)`_ виділяє простір для
...
Функція `create` реалізує логіку для ініціалізації даних нового облікового
запису повідомлення. Вона приймає два параметри:

1. _rs`ctx: Context<Create>`_ - Надає доступ до облікових записів, визначених у
...
2. Ініціалізація даних облікового запису:
   ...
   - Встановлює поле `user` на публічний ключ облікового запису `user`.
   ...
   - Встановлює значення `bump`, яке використовується для виведення PDA,
     отримане з `ctx.bumps.message_account`.
...
Далі додайте інструкцію `update` для зміни `MessageAccount` з новим
повідомленням.
...
1. _rs`user: Signer<'info>`_
   ...
   - Позначений як змінюваний (_rs`#[account(mut)]`_), оскільки може оплачувати
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   - Існуючий обліковий запис, що зберігає повідомлення користувача для
     оновлення
   - Обмеження `mut` вказує на модифікацію даних цього облікового запису
...
Функція `update` реалізує логіку модифікації існуючого облікового запису
повідомлення. Вона приймає два параметри:

1. _rs`ctx: Context<Update>`_ - Надає доступ до облікових записів, вказаних у
...
2. Оновлює дані облікового запису:
   - Отримує доступ до `message_account` з контексту.
   - Встановлює поле `message` на нове `message` з аргументу функції.
...
1. _rs`user: Signer<'info>`_
   ...
   - Позначений як змінюваний (_rs`#[account(mut)]`_) для отримання лампортів із
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - Обмеження `close = user` позначає цей обліковий запис для закриття та
     переказує його лампорти на обліковий запис `user`
...
1. _rs`_ctx: Context<Delete>`_ - Надає доступ до облікових записів, визначених у
   структурі _rs`Delete`_. Синтаксис _rs`_ctx`_ показує, що функція не
   використовує Context у своєму тілі.

Тіло функції просто виводить повідомлення в логи програми за допомогою макросу
_rs`msg!()`_. Функції не потрібна додаткова логіка, оскільки обмеження
...
Ви завершили базову CRUD-програму. Розгорніть програму, виконавши `deploy` у
...
Стартовий код також містить тестовий файл у `anchor.test.ts`.
...
Додайте код нижче всередині _ts`describe()`_, але перед секціями _ts`it()`_.
...
Solana Playground усуває частину шаблонного коду, де [`pg.program`](mention:one)
надає доступ до методів для взаємодії з програмою, тоді як
[`pg.wallet`](mention:two) надає доступ до вашого гаманця playground.
...
переконатися, що він працює як очікується. Наступні кроки додають фактичні
тести.
...
Спочатку тестовий файл надсилає транзакцію, яка викликає інструкцію `update`,
...
Метод _ts`fetchNullable()`_ отримує дані облікового запису з мережі, який може
не існувати.
...
Спочатку цей код надсилає транзакцію, яка викликає інструкцію `delete` для
...
отримати обліковий запис за його адресою (`messagePda`) за допомогою
...
терміналі Playground. Ця команда запускає тести для програми, розгорнутої на
devnet, і виводить посилання на SolanaFM для перегляду деталей транзакції.
...
<Callout type="info">
  Зверніть увагу, що в цьому прикладі, якщо ви запустите тест знову, інструкція
  `create` завершиться помилкою, оскільки `messageAccount` вже існує як
  обліковий запис. Для заданого PDA може існувати лише один обліковий запис.
</Callout>
