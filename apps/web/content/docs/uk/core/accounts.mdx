...
виводиться з використанням ідентифікатора програми та одного чи кількох
необов'язкових вхідних даних (seeds). Приклад нижче демонструє, як
використовувати SDK Solana для створення program derived address.
...
- [`lamports`](mention:lamports): Кількість [lamports](#lamports) в акаунті
- [`data`](mention:data): [Дані](#data) акаунта
- [`owner`](mention:owner): Ідентифікатор програми, яка [володіє](#owner)
  акаунтом
- [`executable`](mention:executable): Вказує, чи містить акаунт
  [виконуваний](#executable) бінарний код
- [`rent_epoch`](mention:rent_epoch): Застаріле поле [epoch оренди](#rent-epoch)
...
Баланс облікового запису в [lamports](/docs/references/terminology#lamport).

Кожен обліковий запис повинен мати мінімальний баланс lamport, який називається
[rent](/docs/references/terminology#rent), що дозволяє зберігати його дані в
блокчейні. Rent пропорційний розміру облікового запису.

<Callout type="info">
  Хоча цей баланс називається rent, він працює більше як депозит, оскільки
  повний баланс можна повернути при закритті облікового запису. (Назва "rent"
  походить від тепер застарілого поля [rent epoch](#rent-epoch).)
</Callout>
...
- Program accounts: Це поле містить або виконуваний код програми, або адресу
  [program data account](#program-data-accounts), який зберігає виконуваний код
  програми.
...
1. Отримання облікового запису за його [адресою](#account-address)
2. Десеріалізація поля `data` облікового запису з необроблених байтів у
   відповідну структуру даних, як визначено програмою, яка володіє обліковим
   записом.
...
Кожен обліковий запис Solana має [програму](/docs/core/programs), призначену як
...
Раніше це поле відстежувало, коли акаунт повинен був сплачувати rent. Однак цей
...
- [Program accounts](#program-accounts): Акаунти, що містять виконуваний код
- [Акаунти даних](#data-accounts): Акаунти, що не містять виконуваного коду

Це розділення означає, що виконуваний код програми та її стан зберігаються в
окремих акаунтах. (Подібно до операційних систем, які зазвичай мають окремі
файли для програм та їхніх даних.)

### Program accounts

Кожна програма належить
[програмі-завантажувачу](/docs/core/programs#loader-programs), яка
використовується для розгортання та управління акаунтом. Коли нова
[програма](/docs/core/programs) розгортається, створюється акаунт для зберігання
її [виконуваного](#executable) коду. Це називається program account. (Для
простоти можна вважати program account самою програмою.)
...
![Діаграма program account, його 4 компонентів та програми-завантажувача.](/assets/docs/core/accounts/program-account-simple.svg)
...
![Акаунт програми з даними. Дані вказують на окремий акаунт даних програми](/assets/docs/core/accounts/program-account-expanded.svg)

<Callout>
  Під час розгортання або оновлення програми буферні акаунти використовуються
  для тимчасового розміщення завантаження.
</Callout>
...
Приклад нижче створює та отримує акаунт Token Mint, яким володіє програма
Token 2022.
...
гаманці є системними акаунтами, що дозволяє їм оплачувати
...
Облікові записи Sysvar існують за попередньо визначеними адресами і надають
доступ до даних про стан кластера. Вони динамічно оновлюються даними про
мережевий кластер. Перегляньте повний список
[облікових записів Sysvar](https://docs.anza.xyz/runtime/sysvars).

У прикладі нижче отримуються та десеріалізуються дані з облікового запису Sysvar
Clock.
