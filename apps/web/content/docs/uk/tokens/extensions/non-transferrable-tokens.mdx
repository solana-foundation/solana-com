...
запобігає передачі токенів між token account після їх створення. Після створення
токени залишаються постійно в token account. Проте власник token account все ще
може спалювати токени та закрити token account, коли баланс досягне нуля.
