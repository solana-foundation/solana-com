...
description: Дізнайтеся, як конвертувати нативний SOL у wrapped SOL.
---

## Синхронізація нативного SOL

Wrapped SOL (WSOL) — це токен-акаунт, який відстежує кількість лампортів в
акаунті як баланс токена. WSOL забезпечує інтеграцію з DeFi-протоколами для
передачі SOL як SPL-токена.
...
синхронізує баланс токен-акаунта wrapped SOL (WSOL) з фактичним SOL
(лампортами), що зберігаються в ньому. Коли ви переказуєте нативний SOL на
токен-акаунт WSOL, баланс токена не оновлюється автоматично. Ви повинні
викликати SyncNative, щоб відобразити правильний баланс WSOL.
