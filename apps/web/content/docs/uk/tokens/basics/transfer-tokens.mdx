...
description: Дізнайтеся, як переказувати токени між token account.
---

## Як переказувати токени

Переказ токенів передбачає переміщення токенів з одного token account до іншого
token account, які мають однаковий mint. Переказ токенів відбувається, коли ви
викликаєте інструкцію
...
у token program. Лише адреса, вказана як власник (authority) вихідного token
account, може переказувати токени з цього рахунку.
