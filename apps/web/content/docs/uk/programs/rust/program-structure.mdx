...
забезпечує гнучкість в організації коду. Єдина вимога полягає в тому, що
програма повинна мати `entrypoint`, яка визначає, де починається виконання
програми.
...
<Callout type="warn">
  Оновіть поле `edition` у `Cargo.toml` до 2021. Інакше ви можете зіткнутися з
...
Тепер додамо необхідні залежності для збірки програми Solana. Нам потрібні
...
Програми Solana повинні бути скомпільовані як динамічні бібліотеки. Додайте
секцію `[lib]`, щоб налаштувати, як Cargo збирає програму.
...
Функція `entrypoint` програми Solana має наступну сигнатуру. Розробники можуть
вільно створювати власну реалізацію функції `entrypoint`.
...
Тепер визначимо структуру даних, яка зберігатиметься в наших акаунтах
лічильника. Це дані, які зберігатимуться в полі `data` акаунта.
...
з варіантів нашого enum `CounterInstruction`. Метод Borsh `try_from_slice`
...
Цей патерн маршрутизації є поширеним у програмах Solana. `instruction_data`
...
Атрибут `#[cfg(test)]` гарантує, що цей код компілюється лише під час виконання
тестів.
...
Виконайте команду `cargo build-sbf`, щоб зібрати програму. Це згенерує файл
...
Додайте клієнтський код до `examples/client.rs` та замініть `program_id` на
вивід попередньої команди:
...
Коли локальний валідатор запущено, ви можете переглянути транзакції в
