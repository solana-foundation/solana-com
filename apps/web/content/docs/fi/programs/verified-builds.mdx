...
Jos ohjelmasi on alikansiossa ja sinulla on Rust-työtila, sinun tulee luoda
...
ohjelma on alikansiossa, sinun tulee myöhemmin lisätä tämä kansio
...
`cargo generate-lockfile`-komennon luodaksesi lukitustiedoston ja jatkaa
ohjelmasi rakentamiseen.
...
> Kun ajat varmennetun buildin ohjelmahakemistossasi, `verify-from-repo`
> komennon yhteydessä sinun tulee lisätä `--mount-path` lippu. Tämä on polku
> kansioon, joka sisältää `Cargo.toml` tiedoston ohjelmasi kirjaston nimellä.
...
Jos haluat lukita varmentamisen tiettyyn julkaisuun, voit lisätä komentoon
`--commit-hash` lipun.
...
> On suositeltavaa käyttää maksullista RPC URL:ää, koska muuten saatat kohdata
> ilmaisten RPC:iden nopeusrajoituksia. Joten `-um` sijaan sinun tulisi käyttää
> `--url yourRpcUrl` luotettavampaa varmentamista varten.

`--remote` lippu lähettää build-pyynnön OtterSec API:lle, joka käynnistää
...
Tämä rakentaa varmennettavan buildin käyttäen docker-konttia ja Solana-versiota,
joka on määritelty `Cargo.lock` tiedostossa.
...
Tämän multisig-oppaan loppuosan ajan käytämme esimerkkiohjelman ID:tä
...
build-data ketjuun käyttäessäsi komentoa `solana-verify verify-from-repo`.
...
Tämä palauttaa base58-koodatun transaktion. Jos haluat base64-koodatun
transaktion käytettäväksi transaktiotarkastimessa, voit käyttää komentoa
`--encoding base64`.
...
vahvistaa, että kontainerin suoritettavan tiedoston tiiviste on sama kuin
komennolle annetun ketjussa olevan ohjelman tiiviste. Koska käännös on jo
ladattu imageen, ei ole tarvetta suoritettavan tiedoston täydelle
uudelleenkäännökselle, joka voi kestää pitkään.

Dockerfile, joka luo imagen `ellipsislabs/hello_world_verifiable_build:latest`,
löytyy Ellipsis Labs -repositoriosta
...
Tässä on esimerkki esimerkkiohjelman vahvistamisesta ID:llä
...
Oletuksena `verify-from-repo`-komento käyttää viimeisintä committia
main-haarassa. Voit myös määrittää tietyn commitin, jos haluat jatkaa
repositorion parissa työskentelyä käyttämällä `commit-hash`-parametria:
...
Komento `--remote` lähettää rakennuspyynnön OtterSec API:lle, joka käynnistää
...
tai suorittaa ohjelman vahvistuksen paikallisesti itse käyttämällä
`verify-from-repo` -komentoa käyttäen lohkoketjussa olevaa vahvistustietoa, joka
on tallennettu
...
Vahvistettujen buildien lisäksi voit myös lisätä `security.txt` -tiedoston
ohjelmaasi. Tulevaisuudessa, kun se on toteutettu, `security.txt` sisältää
verifier-julkisen avaimen helppoa pääsyä varten vahvistus-PDA:han tallennettuun
vahvistustietoon. PDA, joka sisältää kaikki tarvittavat tiedot ohjelman
rakentamiseen ja vahvistamiseen, johdetaan ohjelman osoitteesta ja
verifier-pubkeystä. Oletuksena tämä on sama pubkey, joka rakensi ja otti
ohjelman käyttöön. Mutta se voi olla myös toinen pubkey, joka voidaan määrittää
`security.txt` -tiedostossa.

`security.txt` -ominaisuus mahdollistaa kehittäjille yhteystietojen ja
turvallisuustietojen upottamisen suoraan heidän Solana-älysopimusten sisään.
[Securitytxt.org](https://securitytxt.org)-sivustosta inspiroituneena tämä
lähestymistapa tarjoaa standardoidun tavan turvallisuustutkijoille ottaa
yhteyttä projektin ylläpitäjiin, vaikka he tietäisivät vain sopimuksen
osoitteen.
...
mahdollisesti estää hyväksikäytön ja varmistaa oikea-aikaiset virheraportoinnit.
...
Kun `security.txt`-tiedot on upotettu ohjelmaasi, niitä voidaan helposti kysellä
työkaluilla kuten Solana Explorer, mikä varmistaa, että yhteystietosi ja
tietoturvatietosi ovat saatavilla kaikille, jotka haluavat raportoida
...
- Vahvistus: Ennen käyttöönottoa vahvista muoto ja sisältö käyttämällä
  `query-security-txt`-työkalua, joka voi validoida sekä onchain-ohjelmat että
  paikalliset binäärit:
