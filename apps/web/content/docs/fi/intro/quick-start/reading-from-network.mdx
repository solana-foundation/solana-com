...
Alla oleva esimerkki generoi [uuden avainparin](mention:keypair),
[pyytää SOL-tokeneita](mention:airdrop) rahoittaakseen uuden julkisen avaimen
osoitteen ja [hakee tilidatan](mention:info) juuri rahoitetusta lompakosta.
...
`owner` -kenttä näyttää ohjelman, joka omistaa tilin. Lompakkotileillä omistaja
on aina System Program, jonka osoite on `11111111111111111111111111111111`.
...
mukana taaksepäin yhteensopivuuden vuoksi.)
...
itse [Account-tyypin](/docs/core/accounts#account-structure) kenttä, vaan se on
sisällytetty vastaukseen.
...
Alla oleva esimerkki hakee Token Program -ohjelman havainnollistaakseen eroa
lompakko- ja ohjelmatilien välillä. Ohjelmatili tallentaa käännetyn tavukoodin
Token Program -ohjelman
...
Token Program on suoritettava ohjelmatili. Ohjelmilla on samat peruskentät kuin
kaikilla [tileillä](/docs/core/accounts#account-structure), mutta merkittävillä
eroilla.
...
`data` -kenttä sisältää serialisoidun `Mint` -tilin tilan, kuten mint-oikeuden
haltijan, kokonaistarjonnan ja desimaalien määrän.

Lukeaksesi Mint account -tilistä, sinun täytyy deserialisoida `data` -kenttä
`Mint` -datatyyppiin, mikä näytetään
...
että sen `data` -kenttää voi muokata vain Token Program -ohjelman ohjeiden
avulla.
...
Ennen kuin tilin `data` -kentän raakatavuja voidaan tulkita merkityksellisesti,
ne täytyy deserialisoida. Sopivan datatyypin määrittelee ohjelma, joka omistaa
tilin. Useimmat Solana-ohjelmat tarjoavat asiakaskirjastoja, joissa on
apufunktioita, jotka abstrahoivat deserialisointiprosessin. Nämä funktiot
...
account -tilin `data` -kentän Token Program -ohjelman määrittelemään
...
-tilin dataa Solana Explorerissa.
...
`mintAuthority` -kenttä näyttää ainoan tilin, joka voi luoda uusia yksiköitä
tokenista.
...
mukauta `supply` -kentän arvoa `decimals` -arvolla.
...
vaatii lisädeserialisointia. Tämä kenttä on merkityksellinen vain tileille,
jotka on luotu [Token Extension Program](/docs/tokens/extensions) -ohjelmalla
(Token2022).
