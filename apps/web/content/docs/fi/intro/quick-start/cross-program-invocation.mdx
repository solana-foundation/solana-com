...
`update`- ja `delete`-ohjeita on muokattava käsittelemään SOL-siirtoja tilien
välillä kutsumalla System Programia.
...
Ensin ohjelmaan tarvitaan yksinkertainen "maksa päivittääksesi" -mekanismi
muuttamalla _rs`Update`_ -rakennetta ja `update`-funktiota.

Aloita päivittämällä `lib.rs`-tiedosto tuomaan näkyviin kohteet
...
`vault_account`. Tämä ohjelman hallitsema tili vastaanottaa SOL:ia käyttäjältä,
kun tämä päivittää viestinsä tilin.
...
Tämä tili toimii ohjelman hallitsemana "holvina", joka vastaanottaa SOL:ia
käyttäjiltä, kun he päivittävät viestejään.
...
`vault_account`-tilin keskeiset ominaisuudet:

- Tilin osoite johdetaan PDA:sta käyttäen siemeniä
  _rs`[b"vault", user.key().as_ref()]`_
- PDA:na sillä ei ole yksityistä avainta, joten vain ohjelma voi
  "allekirjoittaa" osoitteen puolesta suorittaessaan CPI-kutsuja
...
`delete`-ohjeessa näet, kuinka ohjelma voi "allekirjoittaa" tämän PDA:n puolesta
CPI-kutsussa.

</Accordion>
</Accordions>

Seuraavaksi lisää CPI-logiikka `update`-ohjeeseen siirtääksesi 0,001 SOL:ia
...
joka kutsuu System Program -ohjelman `transfer` -ohjetta. Tämä havainnollistaa,
miten CPI-kutsu suoritetaan ohjelman sisältä, mahdollistaen Solana-ohjelmien
koostettavuuden.
...
- CPI-kutsussa vaadittavat tilit (määritelty _rs`Transfer`_ -rakenteessa)
...
CPI-kutsun asetukset vastaavat sitä, miten asiakaspuolen ohjeet rakennetaan,
jossa määrität ohjelman, tilit ja instruction data -tiedot tietylle kutsulle.
Kun ohjelman `update` -ohje vastaanottaa kutsun, se kutsuu sisäisesti System
Program -ohjelman siirto-ohjetta.
...
Lisää nyt "palautus poistettaessa" -mekanismi muuttamalla _rs`Delete`_
-rakennetta ja `delete` -funktiota.

Päivitä ensin _rs`Delete`_ -rakenne sisältämään `vault_account`. Tämä
mahdollistaa holvitilillä olevien SOL-tokenien siirtämisen takaisin käyttäjälle,
kun he sulkevat viestinsä tilin.
...
Lisää myös `system_program`, koska CPI siirtoa varten vaatii System Program
-ohjelman kutsumisen.
...
`vault_account` käyttää samaa PDA-johdannaisuutta kuin Update-rakenteessa.

`vault_account` -kohteen lisääminen Delete-rakenteeseen mahdollistaa ohjelman
pääsyn käyttäjän holvitilille poisto-ohjeen aikana, jotta kertynyt SOL voidaan
...
Seuraavaksi lisää CPI-logiikka `delete` -ohjeeseen siirtääksesi SOL:n
...
_rs`signer_seeds`_ määrittelevät valinnaiset siemenet ja bump-siemenet, joita
käytetään PDA:n johdannaiseen.
...
Huomaa, että _rs`_ctx: Context<Delete>`_ muuttuu _rs`ctx: Context<Delete>`_
-muotoon, jotta kontekstia voidaan käyttää funktion rungossa.
...
- Siirrettävä summa (holvitiliin koko saldo)
...
Seuraavaksi päivitä `anchor.test.ts` -tiedosto sisällyttämään uusi holvitili
ohjeisiin. Tämä vaatii holvitiliin PDA:n johdannaisuuden ja sen sisällyttämisen
update- ja delete-ohjekutsuihin.
...
Napsauta `Import` -painiketta ja syötä projektin nimi lisätäksesi sen
projektiluetteloosi Solana Playgroundissa. Kun projekti on tuotu, kaikki
muutokset tallennetaan ja säilyvät automaattisesti.
