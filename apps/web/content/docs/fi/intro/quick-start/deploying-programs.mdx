...
addresseja (PDA) ja cross-program invocationia (CPI).
...
3. Syötä **Project name**
4. Valitse **Anchor (Rust)** kehykseksi
5. Napsauta **Create**
...
- Vastaanottaa pakollisen parametrin nimeltä _rs`data: u64`_
- Luo uuden tilin
- Tallentaa _rs`data: u64`_-arvon uuteen tiliin
- Lisää viestin tapahtuman ohjelmalokehin
...
Ohjelmasi osoite lohkoketjussa. Se päivitetään, kun rakennat projektin
...
Tilirajoitukset. Esimerkiksi yleiset tarkistukset tai ominaisuudet, jotka
...
Käyttää `data`kenttää `new_account`tilissä ja asettaa sen arvoksi `data: u64`
...
Kutsuu System Programia luomaan uuden tilin ja asettaa tämän ohjelman
omistajaksi. Ohjelman omistajana tämä ohjelma voi sitten kirjoittaa tilin
tietoihin
...
- `ctx: Context<Initialize>`: Välittää tämän ohjeen vaatimat tilit funktiolle,
  kuten `Initialize` -rakenteessa on määritetty.
- `data: u64`: Mukautettu parametri, joka sinun on annettava kutsuessasi
  ohjetta.
...
- [`#[derive(Accounts)]`](mention:one) -attribuutti määrittää rakenteen, joka
  määrittelee tietyn ohjeen vaatimat tilit. Jokainen kenttä edustaa erillistä
  tiliä.
...
3. Tallenna transaktiotunniste (`TxHash`) joka sisältyy tulosteeseen
...
**Huomaa:** Halutessasi voit käyttää `Test` -painiketta vasemman puolen
paneelissa.
...
Sulje ohjelma saadaksesi takaisin lohkoketjuohjelmalle allokoidun SOL:n. Tee
tämä suorittamalla `solana program close [ProgramID]`, käyttäen ohjelman
...
<Callout type="warn">
  Ohjelmia voi **vain** sulkea tai päivittää tili, jolla on päivitysoikeudet.
  Playground-lompakostasi tulee automaattisesti päivitysoikeuksien haltija
  jokaiselle ohjelmalle, kun se otetaan käyttöön. Jos päivitysoikeudet
  peruutetaan, ohjelmasta tulee muuttumaton.
</Callout>

Onnittelut! Rakensit ja otit käyttöön ensimmäisen Solana-ohjelmasi käyttäen
Anchoria.
...
Nyt kun olet ottanut käyttöön ensimmäisen ohjelmasi, olet valmis tutkimaan
[ohjelmasta johdettuja osoitteita](/docs/intro/quick-start/program-derived-address)
ja
[ohjelmien välistä kutsumista](/docs/intro/quick-start/cross-program-invocation).
