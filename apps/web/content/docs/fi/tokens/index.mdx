...
määrittää, kenellä on valtuudet token accountiin. Tämä eroaa
perus-[Account](/docs/core/accounts#account-type)-tyypin määrittämästä ohjelman
...
käyttävät [Solana Playground](https://beta.solpg.io/) -terminaalia
CLI-komentojen suorittamiseen suoraan selaimessa ilman paikallista asennusta.

Tokenien ja tilien luominen vaatii SOL:ia tilien rent-talletuksiin ja
...
Terminaalin tulosteessa näkyvät tiliosoitteet eroavat alla olevista
...
_sh`supply`_ -komento uudelle tokenille palauttaa `0`:
...
_sh`create-account`_ -komento luo associated token accountin, jossa lompakon
osoitteesi on omistajana.
...
Huomautus: `<TOKEN_ADDRESS>` määrittää token accountin mint accountin.
...
Token Programin _rs`MintTo`_ -käsky luo uusia tokeneita. Mint authorityn on
allekirjoitettava transaktio. Käsky luo tokeneita token accountille ja kasvattaa
mint accountin kokonaistarjontaa. Tässä on Javascript-esimerkki
...
Token Programin _rs`Transfer`_ -käsky käsittelee token-siirtoja. Lähettäjän
