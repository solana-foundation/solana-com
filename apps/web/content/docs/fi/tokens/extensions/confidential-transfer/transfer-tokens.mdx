...
määritetty `ConfidentialTransferAccount` -tilaan ja hyväksytty luottamuksellisia
siirtoja varten. Lähettäjän token accountilla on myös oltava käytettävissä oleva
...
1. Luo
   ...
   **Equality Proof
   ...
   Varmistaa, että uusi käytettävissä olevan saldon ciphertext siirron jälkeen
...
- `confidential_transfer_create_context_state_account` -metodi, joka luo
  proof-tilin.
- `confidential_transfer_transfer` -metodi, joka kutsuu `Transfer`
  -instruktiota.
