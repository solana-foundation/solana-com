---
title: Sync Native
description: Opi kuinka muuntaa natiivi SOL käärityksi SOL:ksi.
---

## Synkronoi natiivi SOL

Kääritty SOL (WSOL) on token account, joka seuraa tilillä olevien
lamport-yksiköiden määrää token-saldona. WSOL mahdollistaa integraation
DeFi-protokollien kanssa SOL:n siirtämiseksi SPL-tokenina.

[`SyncNative`](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/instruction.rs#L387)
-käsky synkronoi käärityn SOL:n (WSOL) token account -saldon siihen
tallennettujen todellisten SOL-yksiköiden (lamport) kanssa. Kun siirrät natiivia
SOL:ia WSOL token account -tilille, token-saldo ei päivity automaattisesti.
Sinun täytyy kutsua SyncNative-toimintoa, jotta oikea WSOL-saldo näkyy.
