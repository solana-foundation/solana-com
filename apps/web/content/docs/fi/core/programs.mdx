...
sulkemiseen. Niitä käytetään myös ohjelman viimeistelyyn ja ohjelman hallinnan
siirtämiseen.

Tällä hetkellä on viisi latausohjelmaa, kuten alla olevassa taulukossa on
esitetty.
...
Latausohjelmien lisäksi Solana tarjoaa seuraavat esikäännetyt ohjelmat.
...
tarkistettavien allekirjoitusten määrän, jota seuraa yhden tavun täyte. Sen
...
#### Secp256k1-palautuksen varmennus
...
| Ohjelma           | Ohjelmatunnus                                 | Kuvaus                                                              | Ohjeet                                                                                        |
| ----------------- | --------------------------------------------- | ------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| Secp256k1-ohjelma | `KeccakSecp256k11111111111111111111111111111` | Varmistaa secp256k1-julkisen avaimen palautustoiminnot (ecrecover). | [Ohjeet](https://docs.rs/solana-secp256k1-program/latest/solana_secp256k1_program/index.html) |

Secp256k1-ohjelma käsittelee ohjetta. Ohjeen ensimmäinen tavu sisältää
tarkistettavien julkisten avainten määrän. Tämän jälkeen seuraava rakenne
luodaan kerran jokaiselle julkiselle avaimelle, sitten serialisoidaan ja
lisätään instruction data -tietoihin.
...
pub struct Secp256k1SignatureOffsets {
    ...
    eth_address_instruction_index: u8, // instruction index to find eth_address
    message_data_offset: u16,          // offset to start of message data
    message_data_size: u16,            // size of message data
...
```
let eth_pubkey = ecrecover(message, signature)
let eth_address = keccak256(eth_pubkey)[12:32]
if eth_address != expected_eth_address then fail
```
...
Secp256r1-ohjelma käsittelee ohjetta. Ohjeen ensimmäinen instruction data -tavu
sisältää tarkistettavien allekirjoitusten määrän, jota seuraa yhden tavun täyte.
Tämän jälkeen seuraava rakenne luodaan jokaiselle allekirjoitukselle, sitten
serialisoidaan ja lisätään instruction data -tietoihin.

Allekirjoituksen varmennuksen pseudokoodi:

<Callout>
  Matalat S-arvot ovat pakollisia kaikissa allekirjoituksissa tahattoman
...
| ------------------------ | --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| **System**               | `11111111111111111111111111111111`            | Luo uusia tilejä, allokoi tilitietoja, määritä tilit omistaville ohjelmille, siirrä lamporteja System Program -omisteisilta tileiltä ja maksa transaktiomaksuja                                                                                                                                                                                      | [SystemInstruction](https://docs.rs/solana-program/latest/solana_program/system_instruction/enum.SystemInstruction.html)                              |
| **Vote**                 | `Vote111111111111111111111111111111111111111` | Luo ja hallinnoi tilejä, jotka seuraavat validatorin äänestystilaa ja palkkioita                                                                                                                                                                                                                                                                     | [VoteInstruction](https://docs.rs/solana-vote-program/latest/solana_vote_program/vote_instruction/enum.VoteInstruction.html)                          |
