...
laskentabudjetti-[ohjeessa](#instruction) [transaktioiden](#transaction)
priorisoimiseksi.
...
Prosenttiosuus [ohitetuista sloteista](#skipped-slot) suhteessa johtajan
slottien kokonaismäärään nykyisessä epochissa. Tämä mittari voi olla
harhaanjohtava, koska sillä on suuri vaihtelu epochin rajan jälkeen, kun
otoskoko on pieni, sekä validaattoreilla, joilla on vähän johtajaslotteja. Se
voi kuitenkin olla hyödyllinen tunnistettaessa solmujen virheellisiä
konfiguraatioita.
...
slottien lohkojen esivanhempana, ei kasvata [lohkon korkeutta](#block-height)
...
ne eivät mene päällekkäin, käsittäen suunnilleen yhtä pitkiä ajanjaksoja
...
(julkisen avaimen) kautta tai kyselemällä järjestelmäkutsun avulla.

## thin client

[Asiakasohjelma](#client), joka luottaa kommunikoivansa kelvollisen
[klusterin](#cluster) kanssa.

## tick

[Kirjaus](#entry) kirjanpitoon, joka arvioi seinäkellon kestoa.
...
kaikki samat ominaisuudet kuin [Token Program](#token-program), mutta siinä on
lisäksi laajennuksia kuten luottamukselliset siirrot, mukautettu siirtologiikka,
