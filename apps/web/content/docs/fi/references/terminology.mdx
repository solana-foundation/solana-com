...
[panokselle](#stake) lunastuksen yhteydessä maksettavien pisteiden määrä on
...
exemption -kynnysarvon.

## root

[Lohko](#block) tai [slot](#slot), joka on saavuttanut maksimi
[lockoutin](#lockout) [validatorilla](#validator). Root on korkein lohko, joka
on kaikkien validatorilla aktiivisten haarojen esi-isä. Kaikki rootin
esi-isälohkot ovat myös transitiivisesti rooteja. Lohkot, jotka eivät ole
esi-isiä eivätkä rootin jälkeläisiä, jätetään konsensuksen ulkopuolelle ja
voidaan hylätä.

## runtime

[Validatorin](#validator) komponentti, joka vastaa [ohjelmien](#program)
suorittamisesta.
...
[Lohkon](#block) osa; pienin yksikkö, jota lähetetään [validatorien](#validator)
välillä.
...
järjestyksessä, ja S on skalaari välillä [0, l-1]. Tämä vaatimus varmistaa,
...
validatoreilla, joilla on vähän johtajan sloteja. Se voi kuitenkin olla
...
offline-tilassa tai klusterin konsensus hylkäsi [haaran](#fork), joka sisälsi
slotin, paremman vaihtoehdon vuoksi. Ohitettu slot ei näy esivanhempana
myöhempien slotien lohkoille, ei kasvata [lohkon korkeutta](#block-height), eikä
vanhenna vanhinta `recent_blockhash`.

Voidaan määrittää, onko slot ohitettu vasta kun siitä tulee vanhempi kuin
...
Yhdessä slotit muodostavat loogisen kellon. Slotit ovat järjestetty peräkkäin ja
eivät mene päällekkäin, käsittäen suunnilleen saman verran todellista aikaa
...
(pubkey) kautta tai kyselemällä syscallin avulla.

## thin client

[Asiakasohjelma](#client), joka luottaa kommunikoivansa validin
...
[Ledgerin](#ledger) N:s [tikki](#tick).
...
[Token Extensions Program](https://spl.solana.com/token-2022) -ohjelman tunniste
on `TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb` ja se sisältää kaikki samat
ominaisuudet kuin [Token Program](#token-program), mutta sisältää laajennuksia
kuten luottamukselliset siirrot, mukautetun siirtologiikan, laajennetut
metatiedot ja paljon muuta.
...
[Token Program](https://spl.solana.com/token) -ohjelman tunniste on
...
allekirjoittanut yhdellä tai useammalla [keypairilla](#keypair) ja joka
...
hyväksyttiin [ledgeriin](#ledger). Transaktio viimeistellään, kun sen lohkosta
tulee [juuri](#root).
...
uusia [lohkoja](#block). Validator validoi [ledgeriin](#ledger) lisätyt
transaktiot.
