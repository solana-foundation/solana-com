...
modifizieren, über die es Kontrolle hat. Diese Programme werden oft auf anderen
Blockchains als "[_Smart Contracts_](/docs/core/programs)" bezeichnet.
...
geschuldet werden, das Produkt aus den verdienten [Stimmguthaben](#vote-credit)
und der Anzahl der gestakten Lamports.

## privater Schlüssel

Der private Schlüssel eines [Schlüsselpaars](#keypair).
...
Eine Reihe von Nachweisen, von denen jeder beweist, dass bestimmte Daten
existierten, bevor der Nachweis erstellt wurde, und dass eine präzise Zeitspanne
vor dem vorherigen Nachweis vergangen ist. Ähnlich wie eine
...
Transaktionen sollten die minimale Anzahl an Compute-Einheiten anfordern, die
für die Ausführung erforderlich sind, um die Fee zu minimieren.

## public key (pubkey)

Der öffentliche Schlüssel eines [Schlüsselpaars](#keypair).
...
Konten, die ein Mindestguthaben an lamports aufrechterhalten, das proportional
...
Block, der ein Vorgänger aller aktiven Forks auf einem Validator ist. Alle
...
muss mindestens eine Signatur für das [Gebührenkonto](#fee-account) haben. Daher
...
irreführend sein, da sie nach der epoch-Grenze bei kleiner Stichprobengröße
sowie bei Validatoren mit einer geringen Anzahl von Leader-Slots eine hohe
Varianz aufweist, kann aber auch nützlich sein, um Fehlkonfigurationen von
...
Token, die an den [Cluster](#cluster) verfallen, wenn bösartiges Verhalten eines
...
stellen Cluster-Statusinformationen wie aktuelle Tick-Höhe, Rewards
[Points](#point)-Werte usw. bereit. Programme können auf Sysvars über ein
...
Erweiterungen wie vertraulichen Transfers, benutzerdefinierter Transferlogik,
...
[Transaktionsverarbeitungseinheit](https://docs.anza.xyz/validator/tpu).

## transaction

Eine oder mehrere [Anweisungen](#instruction), die von einem [Client](#client)
mit einem oder mehreren [Schlüsselpaaren](#keypair) signiert und atomar
ausgeführt werden, mit nur zwei möglichen Ergebnissen: Erfolg oder Misserfolg.
...
den [Ledger](#ledger) aufgenommen wurde. Eine Transaktion wird abgeschlossen,
...
[Transaktionsvalidierungseinheit](https://docs.anza.xyz/validator/tvu).
...
kann, als es dauerte, ihn zu erzeugen.
...
wurde, während er schrittweise wirksam wird. Während dieser Zeit wird der Stake
als "aktivierend" betrachtet. Weitere Informationen zu:
