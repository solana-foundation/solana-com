...
modifizieren, über die es Kontrolle hat. Diese Programme werden oft als
"[_Smart Contracts_](/docs/core/programs)" auf anderen Blockchains bezeichnet.
...
mindestens eine Signatur für das [Gebührenkonto](#fee-account) haben. Daher kann
die erste Signatur in der Transaktion als [Transaktions-ID](#transaction-id)
behandelt werden
...
stellen Cluster-Statusinformationen bereit, wie aktuelle Tick-Höhe, Rewards
[Points](#point)-Werte usw. Programme können auf Sysvars über ein Sysvar-Konto
(Pubkey) oder durch Abfrage über einen Syscall zugreifen.
...
erweiterten Metadaten und vielem mehr.
...
Eine Funktion, die eine feste Zeit zur Ausführung benötigt und einen Nachweis
