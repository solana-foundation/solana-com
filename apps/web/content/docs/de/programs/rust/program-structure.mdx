...
Der Programm-Status wird mit Rust-Strukturen definiert, die das Datenlayout
deiner Programm-Konten repräsentieren. Du kannst mehrere Strukturen definieren,
um verschiedene Kontentypen für dein Programm darzustellen.
...
Wenn ein Client dein Programm aufruft, muss er instruction data (als Byte-Array)
bereitstellen, wobei:
...
Anweisungs-Handler beziehen sich auf die Funktionen, die die Geschäftslogik für
...
hinzu. Dieser Anweisungs-Handler:

1. Erstellt ein neues Konto und weist Speicherplatz für die Counter-Daten zu
2. Initialisiert die Kontodaten mit dem an die Anweisung übergebenen
   `initial_value`
...
3. Das System Program, das wir aufrufen, um das neue Konto zu erstellen
...
2. Den erforderlichen Mindestguthaben für "rent" berechnen. Auf Solana müssen
...
Konto durch Aufrufen der `create_account` Anweisung des System Programs.

Auf Solana können neue Konten nur vom System Program erstellt werden. Bei der
Erstellung eines Kontos geben wir die Anzahl der zu allozierenden Bytes und den
Programmeigentümer des neuen Kontos an. Das System Program:
...
Um das System Program aus der Anweisung unseres Programms aufzurufen, machen wir
eine Cross Program Invocation (CPI) über die `invoke` Funktion. Eine CPI
