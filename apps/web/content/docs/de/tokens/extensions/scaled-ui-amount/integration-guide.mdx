...
- Für den Live-Saldo eines Benutzers können Sie aktualisierte Informationen zu
  den oben genannten Beträgen erhalten, indem Sie entweder
...
- Wenn Sie den UI-Betrag für einen beliebigen Betrag benötigen, können Sie diese
  Berechnung durch Aufrufen der
  [`amountToUiAmountForMintWithoutSimulation`](https://github.com/solana-program/token-2022/blob/main/clients/js-legacy/src/actions/amountToUiAmount.ts#L164)-Funktion
...
- `getTokenAccountBalance`
  - Gibt das Token-Konten-Guthaben und die Mint-Informationen zurück
...
- `getAccountInfo`
  - Gibt die Konten-Informationen und die Mint-Informationen zurück
...
- Benutzer sollten Beträge eingeben, die als skalierter „UIAmount" interpretiert
  ...
  - Bei Rundungsproblemen sollte abgerundet werden und es sollte bevorzugt
    werden, einen winzigen Restbetrag zu belassen, anstatt zu riskieren, dass
    die Transaktion fehlschlägt
  - Um diese Umrechnung durchzuführen, können Sie die
    [`uiAmountToAmountForMintWithoutSimulation`](https://github.com/solana-program/token-2022/blob/main/clients/js-legacy/src/actions/amountToUiAmount.ts#L312C23-L312C63)-Funktion
...
- Der aktuelle Multiplikator kann jederzeit vom Token-Mint abgelesen werden,
  indem `getAccountInfo` aufgerufen wird. Wenn zusätzlich ein zukünftiger
  Multiplikator festgelegt ist, sind diese Informationen ebenfalls vom
  Token-Mint verfügbar. Wir empfehlen, diesen Multiplikator nicht anzuzeigen, da
  er die UX verwirren kann.
