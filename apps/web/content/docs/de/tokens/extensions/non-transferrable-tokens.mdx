...
verhindert, dass Token nach dem Minting zwischen token accounts übertragen
werden können. Einmal geminted, verbleiben die Token dauerhaft im token account.
Der Besitzer des token accounts kann jedoch weiterhin Token verbrennen und den
token account schließen, wenn der Kontostand null erreicht.
