...
1. **Mint-Konto erstellen**: Rufen Sie die _rs`CreateAccount`_ Anweisung des
   System Program auf, um das Mint-Konto zu erstellen.
...
2. **Confidential Transfer-Erweiterung initialisieren**: Rufen Sie die
   ...
   Anweisung des Token Extension Program auf, um den
...
3. **Mint initialisieren**: Rufen Sie die _rs`Instruction::InitializeMint`_
   Anweisung des Token Extension Program auf, um den Standard-Mint-Zustand zu
   initialisieren.

Sie könnten diese Anweisungen zwar manuell schreiben, aber die
`spl_token_client` Crate bietet eine `create_mint` Methode, die eine Transaktion
mit allen drei Anweisungen in einem einzigen Funktionsaufruf erstellt und
sendet, wie im folgenden Beispiel gezeigt.
