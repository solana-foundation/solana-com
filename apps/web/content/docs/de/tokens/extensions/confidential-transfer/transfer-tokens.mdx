...
Um vertraulich Tokens von einem Token-Konto auf ein anderes zu übertragen,
müssen sowohl der Absender als auch der Empfänger Token-Konten haben, die mit
dem `ConfidentialTransferAccount`-Status konfiguriert und für vertrauliche
Überweisungen freigegeben sind. Das Token-Konto des Absenders muss außerdem über
ein verfügbares vertrauliches Guthaben verfügen, von dem übertragen werden kann.
...
1. Erstellen Sie
   ...
   **Gleichheitsbeweis
   ...
   Verifiziert, dass der neue Chiffretext des verfügbaren Guthabens nach der
   Übertragung mit seinem entsprechenden
...
- Die `confidential_transfer_create_context_state_account`-Methode, die ein
  Beweis-Konto erstellt.
- Die `confidential_transfer_transfer`-Methode, die die `Transfer`-Anweisung
  aufruft.
- Die `confidential_transfer_close_context_state_account`-Methode, die ein
