...
setzt einen Standardzustand für alle neuen token account, die für eine Prägung
erstellt werden. Wenn aktiviert, initialisiert diese Erweiterung automatisch
jedes neue token account mit einem festgelegten Zustand, wie zum Beispiel
eingefroren. Dies ermöglicht Token-Erstellern, bestimmte Verhaltensweisen
durchzusetzen, wie zum Beispiel die Anforderung, dass alle neuen Konten
standardmäßig eingefroren sind, bis sie explizit entsperrt werden.
