...
Beachten Sie, dass die Daten jedes Token-Kontos ein `owner`-Feld enthalten, das
angibt, wer die Berechtigung über das Token-Konto hat. Dies unterscheidet sich
vom Programmeigentümer, der im
Basis-[Konten](/docs/core/accounts#account-type)-Typ angegeben ist, welcher für
alle Token-Konten das Token Program ist.
...
Die _sh`spl-token`_ CLI hilft Ihnen beim Experimentieren mit SPL-Token. Die
...
Das Erstellen von Token und Konten erfordert SOL für Konto-Rent-Einlagen und
Transaktionsgebühren. Für Solana Playground-Erstnutzer erstellen Sie ein
...
Die in Ihrer Terminal-Ausgabe angezeigten Kontoadressen unterscheiden sich von
den folgenden Beispielen. Verwenden Sie die Adressen aus Ihrem
Playground-Terminal, wenn Sie mitmachen. Zum Beispiel gibt der Befehl
_sh`create-token`_ eine Mint-Kontoadresse mit Ihrem Playground-Wallet als
Mint-Berechtigung aus.
...
Hinweis: `<TOKEN_ADDRESS>` gibt das Mint-Konto für das Token-Konto an.
...
Die _rs`MintTo`_-Anweisung im Token Program erstellt neue Token. Die
Mint-Autorität muss die Transaktion signieren. Die Anweisung prägt Token zu
einem Token-Konto und erhöht den Gesamtvorrat auf dem Mint-Konto. Hier ist ein
...
Anweisung verschiebt Token zwischen Token-Konten. Hier ist ein
Javascript-Beispiel auf
...
{/_ CODE_PLACEHOLDER_4303cba408a66f3e81313fca1e2f6ab1 _/
