---
title: Sync Native
description: Erfahren Sie, wie Sie natives SOL in Wrapped SOL umwandeln können.
---

## Native SOL synchronisieren

Wrapped SOL (WSOL) ist ein Token-Konto, das die Anzahl der Lamports im Konto als
Token-Guthaben verfolgt. WSOL ermöglicht die Integration mit DeFi-Protokollen,
um SOL als SPL-Token zu übertragen.
...
Anweisung synchronisiert den Kontostand eines Wrapped SOL (WSOL) Token-Kontos
mit den tatsächlich darin gespeicherten SOL (Lamports). Wenn Sie natives SOL auf
ein WSOL-Token-Konto übertragen, wird der Token-Kontostand nicht automatisch
