...
- Für program account (Smart Contracts) enthält dieses Feld entweder den
  ausführbaren Programmcode selbst oder die Adresse eines anderen Konten, das
  den ausführbaren Programmcode speichert.
- Für nicht ausführbare Konten speichert dies in der Regel Zustände, die gelesen
  werden sollen.
...
Loader-v3 verfügt über einen speziellen Kontotyp für die temporäre
Bereitstellung des Uploads eines Programms während des Deployments oder der
Aktualisierung.
