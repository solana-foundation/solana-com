...
Anchor-Framework. Dieser Leitfaden hilft Ihnen, sich mit dem Solana Playground
...
Eine Struktur, die die Kontendatentypen darstellt, die dieses Programm erstellen
und besitzen kann

### !constraints

Konteneinschränkungen. Zum Beispiel gängige Prüfungen oder Funktionen, die die
...
Das _rs`msg!`_ Makro protokolliert eine Nachricht in den Programmlogs der
Transaktion
...
- Das [`declare_id!`](mention:one) Makro gibt die On-Chain-Adresse deines
...
- `ctx: Context<Initialize>`: Übergibt die für diese Anweisung erforderlichen
  Konten an die Funktion, wie in der `Initialize` Struktur angegeben.
...
den bereitgestellten `data` Parameter und gibt dann eine Nachricht in den
Programmlogs aus.
...
- Das Attribut [`#[account]`](mention:one) definiert eine Struktur, die den
...
1. Kompilieren Sie den Programmcode
...
<Callout>
  Ihre [Programm-ID](/docs/intro/quick-start/deploying-programs), definiert in
  _rs`declare_id!()`_, wird jetzt aktualisiert.
</Callout>

2. SOL zu Ihrer Wallet hinzufügen _(Optional)_

Die Bereitstellung eines Programms erfordert SOL im On-Chain-Konto des
Programms. Falls notwendig,
[fügen Sie SOL zu Ihrer Wallet hinzu](/docs/intro/quick-start#add-sol-to-your-wallet),
bevor Sie mit Schritt 3 fortfahren.

3. Stellen Sie das Programm im Netzwerk bereit
...
**Hinweis:** Bei Bedarf können Sie auch die Schaltflächen **Build** und
...
1. Öffnen Sie die Testdatei mit dem Namen `tests/anchor.test.ts`. Diese Datei
   zeigt, wie man die [`initialize`](mention:one) Struktur des Programms vom
   Client aus aufruft.
...
2. Führen Sie die Testdatei aus
...
3. Speichern Sie den Transaktions-Hash (`TxHash`), der in der Ausgabe enthalten
   ist
...
**Hinweis:** Bei Bedarf können Sie auch die Schaltfläche `Test` im linken
Seitenbereich verwenden.

![Test ausführen](/assets/docs/intro/quickstart/pg-test.png)

4. Führen Sie `solana confirm -v [TxHash]` aus, um die Transaktionslogs
   anzuzeigen
...
Öffnen Sie entweder [SolanaFM](https://solana.fm/) oder
[Solana Explorer](https://explorer.solana.com/?cluster=devnet) und suchen Sie
nach dem Transaktions-Hash.

<Callout type="warn">
  **Solana Explorer:** Stellen Sie den Netzwerk-Cluster auf `devnet` ein, damit
  er mit dem des Solana Playground übereinstimmt.
</Callout>
...
Schließen Sie das Programm, um die SOL zurückzugewinnen, die dem
On-Chain-Programm zugewiesen wurden. Führen Sie dazu
`solana program close [ProgramID]` aus und verwenden Sie die Programmadresse,
die im _rs`declare_id!()`_ Makro gespeichert ist.
...
<Callout type="warn">
  ...
  oder aktualisiert werden. Ihre Playground-Wallet wird automatisch zur
...
Herzlichen Glückwunsch! Sie haben Ihr erstes Solana-Programm mit Anchor erstellt
und bereitgestellt.
...
Nachdem Sie Ihr erstes Programm bereitgestellt haben, sind Sie bereit, um
