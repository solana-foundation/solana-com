...
Das _rs`user`_ Feld enthält einen _rs`Pubkey`_, der den Benutzer identifiziert,
der das Nachrichtenkonto erstellt hat.

### !message

Das _rs`message`_ Feld enthält einen _rs`String`_ mit der Nachricht des
Benutzers.
...
du die _rs`Create`_ Struktur mit folgendem Code aktualisierst:
...
1. _rs`user: Signer<'info>`_
   ...
   - Benötigt den Status veränderbar (_rs`#[account(mut)]`_), da es für das neue
     Konto bezahlt
   - Muss die Transaktion signieren, um den Abzug von Lamports von diesem Konto
     zu genehmigen
...
Die Einschränkung `seeds` definiert die optionalen Eingaben, die zur Ableitung
...
Die Einschränkung `bump` weist Anchor an, automatisch den korrekten bump seed zu
finden und zu verwenden. Anchor verwendet `seeds` und `bump`, um die PDA
abzuleiten.

---

Die Berechnung von `space` _rs`(8 + 32 + 4 + message.len() + 1)`_ reserviert
Speicherplatz für den Datentyp `MessageAccount`:
...
Füge als Nächstes die Geschäftslogik für die _rs`create`_ Anweisung hinzu, indem
du die Funktion `create` mit folgendem aktualisierst:
...
1. _rs`ctx: Context<Create>`_ - Bietet Zugriff auf die Konten, die in der
   Struktur _rs`Create`_ angegeben sind.
...
Als Nächstes füge die `update` Anweisung hinzu, um das `MessageAccount` mit
einer neuen Nachricht zu ändern.

Wie im vorherigen Schritt, gib zuerst die für die `update` Anweisung
erforderlichen Konten an.
...
Die _rs`Update`_ Struktur definiert die für die _rs`update`_ Anweisung
erforderlichen Konten.

1. _rs`user: Signer<'info>`_
   - Repräsentiert den Benutzer, der das Nachrichten-Konto aktualisiert
   - Als veränderbar markiert (_rs`#[account(mut)]`_), da es möglicherweise für
     mehr Speicherplatz für das `message_account` bezahlt, wenn nötig
...
3. _rs`system_program: Program<'info, System>`_
   - Erforderlich für potenzielle Neuzuweisung des Kontospeicherplatzes
   - Die `realloc` Einschränkung ruft das System Program auf, um die Datengröße
     des Kontos anzupassen

Beachte, dass die _rs`bump = message_account.bump`_ Einschränkung den auf dem
_rs`message_account`_ gespeicherten Bump-Seed verwendet, anstatt ihn von Anchor
...
ausführen, um zu prüfen, ob sie wie erwartet läuft. Die nächsten Schritte fügen
die eigentlichen Tests hinzu.
...
Die Methode _ts`create()`_ ruft die `create`-Anweisung auf.
...
Die Methode _ts`fetch()`_ ruft die Kontendaten aus dem Netzwerk ab.
...
Die _ts`fetchNullable()`_ Methode ruft Kontodaten aus dem Netzwerk ab, die
