...
zainstalowane Solana CLI, aby użyć lokalnego validatora.
...
| Instrukcja                          | Opis                                                                                                                                                         |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`InitializeMint`_                | Konfiguruje mint account do poufnych transferów. Ta instrukcja musi być zawarta w tej samej transakcji co instrukcja _rs`TokenInstruction::InitializeMint`_. |
| _rs`UpdateMint`_                    | Aktualizuje ustawienia poufnych transferów dla mint.                                                                                                         |
| _rs`ConfigureAccount`_              | Konfiguruje token account do poufnych transferów.                                                                                                            |
| _rs`ApproveAccount`_                | Zatwierdza token account do poufnych transferów, jeśli mint wymaga zatwierdzenia nowych token accounts.                                                      |
...
| _rs`Withdraw`_                      | Konwertuje dostępne poufne saldo z powrotem na saldo publiczne.                                                                                              |
| _rs`Transfer`_                      | Przesyła tokeny pomiędzy token accounts w sposób poufny.                                                                                                     |
| _rs`ApplyPendingBalance`_           | Konwertuje oczekujące saldo na dostępne saldo po wpłatach lub transferach.                                                                                   |
| _rs`EnableConfidentialCredits`_     | Pozwala token account na odbieranie poufnych transferów tokenów.                                                                                             |
| _rs`DisableConfidentialCredits`_    | Blokuje przychodzące poufne transfery, jednocześnie umożliwiając transfery publiczne.                                                                        |
| _rs`EnableNonConfidentialCredits`_  | Pozwala token account na odbieranie publicznych transferów tokenów.                                                                                          |
| _rs`DisableNonConfidentialCredits`_ | Blokuje zwykłe transfery, aby konto mogło otrzymywać tylko poufne transfery.                                                                                 |
| _rs`TransferWithFee`_               | Przesyła tokeny pomiędzy token accounts w sposób poufny z opłatą.                                                                                            |
| _rs`ConfigureAccountWithRegistry`_  | Alternatywny sposób konfiguracji token accounts do poufnych transferów z użyciem konta _rs`ElGamalRegistry`_ zamiast dowodu _rs`VerifyPubkeyValidity`_.      |
