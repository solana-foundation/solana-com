...
chroni konta tokenów przed nieoczekiwanymi transferami za pośrednictwem wywołań
Cross Program Invocations (CPI). Po włączeniu ta rozszerzenie zapewnia, że
tokeny mogą być przesyłane wyłącznie poprzez bezpośrednie wywołania instrukcji
transferu Token Extensions Program, blokując wszystkie inne programy przed
transferem tokenów przez CPI.
