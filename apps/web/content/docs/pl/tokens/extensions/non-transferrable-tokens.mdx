...
uniemożliwia przenoszenie tokenów między kontami tokenów po ich wyemitowaniu. Po
emisji tokeny pozostają na stałe na koncie tokenów. Jednak właściciel konta
tokenów nadal może spalić tokeny i zamknąć konto tokenów, gdy saldo osiągnie
zero.
