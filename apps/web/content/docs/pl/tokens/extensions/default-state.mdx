...
ustawia domyślny stan dla wszystkich nowych token accounts utworzonych dla
danego mint. Po włączeniu, to rozszerzenie automatycznie inicjalizuje każde nowe
token account z określonym stanem, na przykład zamrożonym. Pozwala to twórcom
tokenów wymusić określone zachowania, takie jak wymaganie, aby wszystkie nowe
konta były domyślnie zamrożone, dopóki nie zostaną wyraźnie odmrożone.
