---
title: Tokeny oprocentowane
description: Dowiedz się, jak włączyć rozszerzenie InterestBearingMintExtension.
---

## Jak włączyć rozszerzenie InterestBearingExtension
...
pozwala ustawić stopę procentową bezpośrednio na mint account. Odsetki są
nieustannie kapitalizowane na podstawie znacznika czasu sieci, ale rzeczywisty
stan tokenów przechowywany w każdym token account pozostaje niezmieniony.
Całkowita wartość (stan tokenów + naliczone odsetki) jest jedynie obliczeniem. W
ramach tego rozszerzenia nie są emitowane nowe tokeny.
