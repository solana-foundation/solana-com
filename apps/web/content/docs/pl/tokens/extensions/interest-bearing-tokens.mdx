...
nieustannie kapitalizowane na podstawie znacznika czasu sieci, ale faktyczny
stan tokenów przechowywany w każdym token account pozostaje niezmieniony.
Całkowita wartość (saldo tokenów + naliczone odsetki) jest jedynie obliczeniem.
W ramach tego rozszerzenia nie są emitowane nowe tokeny.
