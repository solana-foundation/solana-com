...
pozwala na ustawienie stopy procentowej bezpośrednio na mint account. Odsetki są
stale kapitalizowane na podstawie znacznika czasu sieci, ale faktyczny stan
tokenów przechowywany w każdym token account pozostaje niezmieniony. Całkowita
wartość (stan tokenów plus naliczone odsetki) jest jedynie obliczeniem. Żadne
nowe tokeny nie są emitowane w ramach tego rozszerzenia.
