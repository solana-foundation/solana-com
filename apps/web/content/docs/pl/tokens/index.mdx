...
Ta sekcja obejmuje podstawowe koncepcje dotyczące reprezentacji tokenów w
Solanie. Przykłady kodu znajdziesz w sekcji
...
Zwróć uwagę, że dane każdego token account zawierają pole `owner`, które
identyfikuje osobę mającą uprawnienia do tego token account. Różni się to od
właściciela programu określonego w bazowym typie
[Account](/docs/core/accounts#account-type), którym dla wszystkich token account
jest Token Program.
...
CLI _sh`spl-token`_ pomaga eksperymentować z tokenami SPL. Przykłady poniżej
...
Tworzenie tokenów i kont wymaga SOL na depozyty rent oraz opłaty transakcyjne.
Jeśli korzystasz z Solana Playground po raz pierwszy, utwórz portfel Playground
i uruchom polecenie _sh`solana airdrop`_ w terminalu Playground. Możesz też
otrzymać SOL z devnetu z publicznego [web faucet](https://faucet.solana.com/).
...
Adresy kont wyświetlane w twoim terminalu mogą się różnić od poniższych
przykładów. Podczas wykonywania instrukcji używaj adresów z terminala
Playground. Na przykład polecenie _sh`create-token`_ zwraca adres mint account z
...
Polecenie _sh`create-account`_ tworzy associated token account z twoim adresem
...
Instrukcja _rs`MintTo`_ w Token Program tworzy nowe tokeny. Mint authority musi
...
Właściciel Token Account nadawcy musi podpisać transakcję. Instrukcja przenosi
tokeny między Token Accounts. Oto przykład w Javascript na
