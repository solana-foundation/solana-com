...
<Callout type="info">
  Zwróć uwagę, że w kodzie źródłowym deweloperzy nazywają token account typem
  ...
  korzystają z tej samej bazowej implementacji dla token account.
</Callout>
...
- Circle zarządza tokenem poprzez uprawnienia właściciela token account:
...
Utworzone token accounts są własnością Token Program i korzystają ze
...
Utworzenie token account wymaga instrukcji
