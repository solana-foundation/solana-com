...
Mint account unikalnie reprezentuje token w sieci Solana i przechowuje jego
globalne metadane.
...
Każdy token w sieci Solana posiada mint account, a adres tego mintu służy jako
unikalny identyfikator tokena.

Na przykład USD Coin (USDC) ma adres mintu
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`. Ten adres unikalnie identyfikuje
USDC w całym ekosystemie Solana. Możesz zobaczyć ten mint na stronie
...
Musisz wywołać dwie instrukcje, aby utworzyć mint account:

1. System Program: Utwórz konto z przydzieloną przestrzenią dla mint account i
   przenieś własność na Token Program.
2. Token Program: Zainicjalizuj dane mint account.
