...
description: Dowiedz się, jak ustawić autorytet dla mintów lub token account.
---
...
- **Autorytet mint**: Kontroluje tworzenie nowych tokenów. Może mintować tokeny
  na dowolny token account. Często jest odwoływany po utworzeniu początkowej
  podaży, aby stworzyć token o stałej podaży.
- **Autorytet zamrażania**: Kontroluje możliwość zamrażania i rozmrażania token
  account. Może uniemożliwić dowolnemu token account transfer tokenów. Często
  jest odwoływany, aby zagwarantować użytkownikom, że ich tokeny nie mogą zostać
  zamrożone.

### Autorytety token account

- **Właściciel konta**: Ma pełną kontrolę nad token account. Może transferować
  tokeny, spalać tokeny, zatwierdzać delegatów i zamykać konto, gdy saldo wynosi
  zero.
- **Autorytet zamknięcia**: Może zamknąć token account, gdy jego saldo wynosi
...
zapewnia możliwość zmiany lub odwołania autorytetów na mintach i token account.
Tylko obecny autorytet może przenieść swoje uprawnienia na nowy adres lub trwale
je odwołać, ustawiając autorytet na null. Po odwołaniu autorytet nie może zostać
przywrócony.
