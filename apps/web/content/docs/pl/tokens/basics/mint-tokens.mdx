...
Tworzenie tokenów odnosi się do procesu generowania nowych jednostek tokena
poprzez wywołanie instrukcji
...
w programie tokenów. Tylko adres określony jako autorytet tworzenia tokenów na
koncie mint może tworzyć nowe tokeny. Instrukcja wymaga również istnienia konta
tokenów jako miejsca docelowego dla utworzonych tokenów.
