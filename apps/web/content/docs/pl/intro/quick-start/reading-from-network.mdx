...
Pole `rentEpoch` to pole historyczne pochodzące z przestarzałego mechanizmu
...
Zanim surowe bajty w polu `data` konta mogą być interpretowane w sensowny
...
[_ts`getMint()`_](mention:one), która pomaga zdeserializować pole mint accountu
`data` do typu danych
...
Funkcja _ts`getMint()`_ deserializuje pole mint accountu `data` do typu konta
Mint.
...
Pole `freezeAuthority` pokazuje konto z uprawnieniami do zamrażania kont token
account. Zamrożony token account nie może transferować ani spalać tokena, który
posiada.
