...
インストラクションを呼び出すことで、トークンの新しい単位を作成するプロセスを指します。mint
accountでミント権限を持つと指定されたアドレスのみが新しいトークンをミントできます。このインストラクションでは、ミントされたトークンの送信先としてtoken
accountが存在する必要もあります。
