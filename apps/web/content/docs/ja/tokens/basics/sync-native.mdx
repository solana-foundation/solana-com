...
ラップドSOL（WSOL）は、アカウント内のlamportsの量をトークン残高として追跡するトークンアカウントです。WSOLはDeFiプロトコルとの統合を可能にし、SOLをSPLトークンとして転送できるようにします。

[`SyncNative`](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/interface/src/instruction.rs#L387)命令は、ラップドSOL（WSOL）トークンアカウントの残高を、そこに保存されている実際のSOL（lamports）と同期させます。ネイティブSOLをWSOLトークンアカウントに転送する場合、トークン残高は自動的に更新されません。正確なWSOL残高を反映させるには、SyncNativeを呼び出す必要があります。
