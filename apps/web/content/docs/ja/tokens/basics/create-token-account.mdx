...
token accountはSolanaのToken
Programsにおけるアカウントタイプの一つで、特定のトークン（ミント）に対する個人の所有権情報を保存します。各token
...
特定のミントのトークンを保有するには、ユーザーはまずtoken
accountを作成する必要があります。各token accountは以下を追跡します：

1. 特定のミント（token accountが保有するトークンの種類）
2. 所有者（アカウントからトークンを転送できる権限者）

Solana上のUSDコイン（USDC）を使用した例を示します：

- USDコインのミントアドレス：`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Circle（USDコインの発行者）は`3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`にtoken
  accountを持っています
- このtoken accountはUSDコイントークン（ミント）の単位のみを保持します
- Circleは`7VHUFJHWu2CuExkJcJrzhQPJ2oygupTWkL2A2For4BmE`に所有者権限を持ち、これらのトークンを転送できます
...
1. token accountの「所有者」 - これはtoken
   accountのデータに保存されているアドレスで、Token
   Programで定義されている[Account](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/state.rs#L106)タイプの「所有者」フィールドです。所有者はアカウントからトークンの転送、バーン、または委任を行うことができます。このアドレスは、プログラム所有者と区別するために、token
   accountの「権限者」と呼ばれることもあります。

2. プログラム「オーナー」 - これはSolanaでアカウントデータを所有するプログラムを指します。token
   accountの場合、これは常にToken ProgramまたはToken Extension
   Programのいずれかであり、基本的なSolana
...
token
accountを扱う場合、「owner」は通常、アカウントを所有するプログラムではなく、トークンを使用できる権限を持つエンティティを指します。
...
Derived Address (PDA)のアドレスを持つtoken accountです。associated token
accountは、ユーザーが特定のトークン（mint）のユニットを保持するためのデフォルトのtoken
accountと考えることができます。

<Callout type="info">
  「associated token accounts」という用語は、Associated Token
  Programが作成するtoken accountにのみ適用されます。
</Callout>

associated token accountは、任意のmintに対するユーザーのtoken
accountを決定論的に見つける方法を提供します。導出の実装は[こちら](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/interface/src/address.rs#L56)で確認できます。
...
この決定論的な導出により、ウォレットアドレスとトークンmintの任意の組み合わせに対して、正確に1つのassociated
token
accountアドレスが存在することが保証されます。このアプローチにより、任意のトークンmintに対するユーザーのtoken
accountを簡単に見つけることができ、token
accountアドレスを個別に追跡する必要がなくなります。

<Callout type="info">
  Associated Token Programは、決定論的なアドレス（PDA）を持つtoken
  accountを作成するヘルパープログラムとして機能します。associated token
  accountを作成する際、Associated Token ProgramはToken ProgramまたはToken
  Extension
  Programへのクロスプログラム呼び出し（CPI）を行います。トークンプログラムは作成されたアカウントを所有し、そのアカウントはトークンプログラムで定義されているのと同じ
  `Account` 型構造を持ちます。Associated Token Programは状態を維持しません -
  単に決定論的なアドレス（PDA）でtoken
  accountを作成するための標準化された方法を提供するだけです。
</Callout>
...
instructionsを呼び出します。このinstructionsの実装は
...
トークンアカウントを作成するトランザクションには2つのinstructionsが必要です：

1. System
   Programを呼び出してトークンアカウントのスペースを作成・割り当て、所有権をToken
   Programに移転します。
2. Token Programを呼び出してトークンアカウントデータを初期化します。
...
associated token accountを作成するinstructionsは、自動的にSystem
Programを呼び出してトークンアカウントを作成し、Token
Programを呼び出してトークンアカウントデータを初期化します。これはCross Program
