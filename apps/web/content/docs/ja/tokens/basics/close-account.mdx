...
instructionsは、トークンアカウントを永久に閉じ、残りのすべてのSOL（rent）を指定された宛先アカウントに転送します。トークンアカウントを閉じる前に、残高はゼロである必要があります。トークンアカウントの所有者または指定されたクローズ権限を持つ人だけがこのinstructionsを実行できます。
