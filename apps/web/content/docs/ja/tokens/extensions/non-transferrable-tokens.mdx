---
title: 譲渡不可トークン
description: InitializeNonTransferableMint拡張機能の有効化方法を学びます。
---

## InitializeNonTransferableMint拡張機能を有効にする方法

[`InitializeNonTransferableMint`](https://github.com/solana-program/token-2022/blob/6f2473344d70271f632c3e9b7e945be00186c536/interface/src/instruction.rs#L647)は、ミント後にトークンアカウント間でトークンが転送されるのを防ぎます。ミントされると、トークンはトークンアカウント内に永続的に残ります。ただし、トークンアカウントの所有者は、トークンを焼却（バーン）し、残高がゼロになったときにトークンアカウントを閉じることができます。
