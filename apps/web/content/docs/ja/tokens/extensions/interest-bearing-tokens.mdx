---
title: 利子付きトークン
description: InterestBearingMintExtensionの有効化方法を学びましょう。
---
...
accountに直接金利を設定できます。利息はネットワークのタイムスタンプに基づいて継続的に複利計算されますが、各token
accountに保存されている実際のトークン残高は変更されません。総価値（トークン残高＋発生利息）は計算上のものです。このエクステンションでは新しいトークンは発行されません。
