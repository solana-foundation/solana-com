...
を使用すると、mint
accountに直接金利を設定できます。金利はネットワークのタイムスタンプに基づいて継続的に複利計算されますが、各token
accountに保存されている実際のトークン残高は変更されません。総額(トークン残高と発生した利息の合計)は計算上のものに過ぎません。この拡張機能では新しいトークンは発行されません。
