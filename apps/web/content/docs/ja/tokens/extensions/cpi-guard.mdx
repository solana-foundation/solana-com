...
[`CpiGuardExtension`](https://github.com/solana-program/token-2022/blob/6f2473344d70271f632c3e9b7e945be00186c536/interface/src/instruction.rs#L661)は、Cross
Program
Invocation（CPI）を通じた予期しないトークン転送からトークンアカウントを保護します。この拡張機能を有効にすると、Token
Extensions
Programの転送instructionsへの直接呼び出しを通じてのみトークンを転送できるようになり、他のすべてのプログラムがCPIを通じてトークンを転送することをブロックします。
