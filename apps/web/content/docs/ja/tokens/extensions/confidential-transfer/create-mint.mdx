...
2. **機密転送拡張機能の初期化**：Token Extension
...
これらのinstructionsを手動で記述することもできますが、`spl_token_client`クレートは`create_mint`メソッドを提供しており、以下の例で示すように、3つすべてのinstructionsを含むトランザクションを単一の関数呼び出しで構築して送信します。
