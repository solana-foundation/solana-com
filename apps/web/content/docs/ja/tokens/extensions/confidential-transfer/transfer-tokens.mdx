...
あるトークンアカウントから別のトークンアカウントへ機密転送するには、送信者と受信者の両方が`ConfidentialTransferAccount`状態で構成され、機密転送が承認されたトークンアカウントを持っている必要があります。送信者のトークンアカウントには、転送可能な機密残高も必要です。
...
1. クライアント側で[3つの証明](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)を作成します：
   ...
   **等価性証明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**: 転送後の新しい利用可能残高の暗号文が対応する
   [Pedersen commitment](https://en.wikipedia.org/wiki/Commitment_scheme)と一致することを検証し、送信元アカウントの新しい利用可能残高が`new_balance = current_balance - transfer_amount`として正しく計算されていることを保証します。
...
- `confidential_transfer_create_context_state_account`メソッドは証明アカウントを作成します。
- `confidential_transfer_transfer`メソッドは`Transfer`命令を呼び出します。
- `confidential_transfer_close_context_state_account`メソッドは証明アカウントを閉じます。
