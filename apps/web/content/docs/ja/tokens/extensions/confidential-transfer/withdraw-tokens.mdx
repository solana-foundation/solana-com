...
1. クライアント側で
   ...
   **等価性証明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**: 出金後の残存利用可能残高暗号文が対応する
   [Pedersen commitment](https://en.wikipedia.org/wiki/Commitment_scheme)と一致することを検証し、アカウントの新しい利用可能残高が
   `remaining_balance = current_balance - withdraw_amount`として正しく計算されていることを保証します。
...
4. 証明アカウントを閉じて、それらを作成するために使用されたSOLを回収します。
...
- `confidential_transfer_withdraw`メソッドは`Withdraw` 命令を呼び出します。
...
Extension
Programを持つローカルvalidatorを起動します。ローカルvalidatorを起動するにはSolana
