...
CPIがPDA署名者を必要とする場合、[`invoke_signed`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/program.rs#L51-L73)関数が使用されます。これは、署名者[PDA](/docs/core/pda)を派生させるために使用される[署名者シード](mention:signer-seeds)を取ります。Solanaランタイムは、呼び出し元プログラムの`signers_seeds`と`program_id`を使用して、内部的に[`create_program_address`](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/bpf_loader/src/syscalls/cpi.rs#L552)を呼び出します。PDAが検証されると、[有効な署名者として追加](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/bpf_loader/src/syscalls/cpi.rs#L554)されます。
