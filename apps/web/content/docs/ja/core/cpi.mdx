...
CPIでPDA署名者が必要な場合、[`invoke_signed`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/program.rs#L51-L73)関数が使用されます。この関数は署名者[PDA](<(/docs/core/pda)>)を導出するための[署名者シード](mention:signer-seeds)を受け取ります。Solanaランタイムは内部的に呼び出し元プログラムの`signers_seeds`と`program_id`を使用して[`create_program_address`](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/bpf_loader/src/syscalls/cpi.rs#L552)を呼び出します。PDAが検証されると、[有効な署名者として追加されます](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/bpf_loader/src/syscalls/cpi.rs#L554)。
