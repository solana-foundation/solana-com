...
[`Instruction`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/instruction/src/lib.rs#L94)は、以下の情報で構成されています。
...
instructionの`accounts`配列は、[`AccountMeta`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/instruction/src/account_meta.rs#L25)構造体の配列です。instructionが相互作用する各アカウントに対してメタデータを提供する必要があります。(これにより、同じアカウントを変更しない限り、トランザクションはinstructionsを並列実行できます。)

以下の図は、単一のinstructionを含むトランザクションを示しています。instructionの`accounts`配列には、2つのアカウントのメタデータが含まれています。

![1つのinstructionを持つトランザクション。instructionの`accounts`配列には、2つの`AccountMeta`構造体が含まれています。](/assets/docs/core/transactions/accountmeta.svg)
...
instructionの`data`は、プログラムのどのinstructionを呼び出すかを指定するバイト配列です。また、instructionに必要な引数も含まれます。
...
<Callout type="info">
  ...
  ほとんどのプログラムは、instructionsを作成するヘルパー関数を備えたクライアントライブラリを提供しています。ライブラリが利用できない場合は、instructionを手動で構築できます。
</Callout>
