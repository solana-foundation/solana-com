...
[`Transaction`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/src/transaction/mod.rs#L207)は、以下の情報で構成されています:

<WithMentions>

- [`signatures`](mention:signatures): [署名](#signatures)の配列
- [`message`](mention:message): 処理されるinstructionのリストを含むトランザクション情報
...
トランザクションの合計サイズ制限は[1232](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/packet/src/lib.rs#L29)バイトです。この制限には、[`signatures`](#signatures)配列と[`message`](#message)構造体の両方が含まれます。
...
トランザクションの`signatures`配列には、`Signature`構造体が含まれています。各[`Signature`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/signature/src/lib.rs#L30)は64バイトで、トランザクションの`Message`をアカウントの秘密鍵で署名することによって作成されます。トランザクションのいずれかのinstructionに含まれる各[署名者アカウント](#account-addresses)に対して署名を提供する必要があります。
...
トランザクションの`message`は、[`Message`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/message/legacy.rs#L131)構造体であり、以下の情報を含んでいます:

<WithMentions>

- [`header`](mention:message-header): メッセージ[ヘッダー](#header)
- [`account_keys`](mention:account-addresses): トランザクションのinstructionに必要な[アカウントアドレス](#account-addresses)の配列
- [`recent_blockhash`](mention:recent-blockhash): トランザクションのタイムスタンプとして機能する[ブロックハッシュ](#recent-blockhash)
- [`instructions`](mention:instructions): [instruction](#instructions)の配列

<Callout>
  スペースを節約するため、トランザクションは各アカウントの権限を個別に保存しません。代わりに、アカウントの権限は`header`と`account_keys`を使用して決定されます。
</Callout>
...
メッセージの`header`は、[`MessageHeader`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/message/mod.rs#L97)構造体です。以下の情報を含んでいます:
...
はアカウントアドレスの配列で、[コンパクト配列形式](/docs/references/terminology#compact-array-format)で送信されます。配列の接頭辞はその長さを示します。配列内の各項目は公開鍵であり、instructions で使用されるアカウントを指します。`accounts_keys`
配列は完全で、以下のように厳密に順序付けされている必要があります:
...
<Callout>
  厳密な順序付けにより、`account_keys` 配列をメッセージの [`header`](#header)
  の情報と組み合わせて、各アカウントの権限を判断できます。
</Callout>
...
は、トランザクションのタイムスタンプとして機能し、重複トランザクションを防ぐハッシュ値です。ブロックハッシュは
[150ブロック](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/clock/src/lib.rs#L134)
後に期限切れになります。(各ブロックが400msと仮定すると、1分に相当します。)ブロックの期限が切れると、トランザクションは期限切れとなり、処理できなくなります。
...
は、処理されるすべての instructions の配列で、[コンパクト配列形式](/docs/references/terminology#compact-array-format)で送信されます。配列の接頭辞はその長さを示します。配列内の各項目は
...
1. [`program_id_index`](mention:program-id-index):
   ...
   配列内のアドレスを指すインデックス。この値は、instruction を処理するプログラムのアドレスを示します。
2. [`accounts`](mention:account-indexes): `account_keys`
   配列内のアドレスを指すインデックスの配列。各インデックスは、この instruction に必要なアカウントのアドレスを指します。
3. [`data`](mention:instruction-data): プログラム上で呼び出す instruction を指定するバイト配列。また、instruction に必要な追加データも含みます。(例: 関数の引数)
