...
- [メッセージヘッダー](mention:message-header): 署名者アカウントと読み取り専用アカウントの数を指定します。
- [アカウントアドレス](mention:account-addresses): トランザクション上の instructions に必要なアカウントアドレスの配列。
- [最新のブロックハッシュ](mention:recent-blockhash): トランザクションのタイムスタンプとして機能します。
- [Instructions](mention:instructions): 実行される instructions の配列。
