...
- Saves a message to the program logs
...
Closing the program allows full recovery of the SOL allocated to the
on-chain program.
To close the program, run `solana program close [ProgramID]`,
...
Your Playground wallet becomes the upgrade authority for each program when it is deployed.
If upgrade authority is revoked, the program becomes immutable.
</Callout>
