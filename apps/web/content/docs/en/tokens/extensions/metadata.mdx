...
description: Learn how to implement metadata and metadata pointer extensions.
---

## How to implement metadata and metadata pointer extensions

The metadata extension is a mint extension that allows token creators to associate on-chain metadata directly with a token mint. This metadata can include fields such as the token’s name, symbol, URI, and optional additional data that provides descriptive or identifying information about the token. By storing metadata on-chain, clients and dApps can reliably fetch token details without depending solely on off-chain registries.

The metadata pointer extension complements this by enabling token creators to store a reference (pointer) to an external account where the metadata is located. This makes it easier for clients to distinguish between the canonical metadata from other different metadata accounts claiming to describe the mint.

The token extensions program supports all instructions from [`token metadata interface`](https://github.com/solana-program/token-metadata/blob/main/interface/src/instruction.rs) and [`MetadataPointerExtension`](https://github.com/solana-program/token-2022/blob/6f2473344d70271f632c3e9b7e945be00186c536/interface/src/instruction.rs#L711).

<Callout type="info">
  Only the [Token Extension Program](https://github.com/solana-program/token-2022/blob/6f2473344d70271f632c3e9b7e945be00186c536/interface/src/instruction.rs#L711)
...
console.log("[DEBUG];", mint.address); // ! debug
...
console.log("Token Account (optional):", tokenAccount.publicKey.toBase58());
```
...
async fn main() -> Result<()> {
  ...
  println!("Token Name: {}", token_metadata.name);
  println!("Token Symbol: {}", token_metadata.symbol);
  println!("Token URI: {}", token_metadata.uri);
...
async fn main() -> Result<()> {
  ...
  let result = token
    .token_metadata_initialize_with_rent_transfer(
      ...
      token_metadata.symbol.to_string(), // symbol
      token_metadata.uri.to_string(),    // uri
      &[&payer],                         // mint keypair needed as signer
    )
    .await?;
