---
title: Memo transfer
description: Learn how to add memos to token transfers.
---

## How to Add Memo to token transfers

The memo transfer extension is a token account extension that when enabled requires incoming token transfer to include a memo. The extension can be enabled on token accounts by using the [`MemoTransferExtension`](https://github.com/solana-program/token-2022/blob/6f2473344d70271f632c3e9b7e945be00186c536/interface/src/instruction.rs#L623) desired token account. It is also possible to disabled the extension by the token account authority.


<Callout type="info">
  Only the [Token Extension
  Program](https://github.com/solana-program/token-2022/blob/6f2473344d70271f632c3e9b7e945be00186c536/interface/src/instruction.rs#L623)
...
// Get a fresh blockhash to try tranfer with memo enabled
const { value: closeMintBlockhash } = await rpc.getLatestBlockhash().send();
...
const transferMessage = pipe(
  createTransactionMessage({ version: 0 }),
  (tx) => setTransactionMessageFeePayerSigner(authority, tx),
  (tx) => setTransactionMessageLifetimeUsingBlockhash(closeMintBlockhash, tx),
  (tx) =>
    appendTransactionMessageInstructions(
...
const memoIx = new TransactionInstruction({
  ...
  data: Buffer.from("This transfer requires a memo to accompany it", "utf-8"), // Memo message
...
use spl_token_2022::{ID as TOKEN_2022_PROGRAM_ID, extension::ExtensionType};
