...
description: Learn how to implement token group and member extensions, commonly used for NFT collections.
---

## How to implement token groups and members

Token groups are commonly used to implement NFT collections and organize related tokens under a shared context. The Token-2022 program provides two types of extensions for this purpose: pointers and direct data storage.

**Group Pointer Extension:** Sets a group account on the token mint to hold token group information.

**Group Extension:** Allows group data to be directly saved within the mint itself.

**Member Pointer Extension:** Sets an individual member account on the token mint to hold data about the token’s membership in a group.

**Member Extension:** Allows member data to get saved directly within the mint itself.

These extensions enable flexible organization of tokens, making them especially useful for NFT collections, DAOs, and membership-based systems. Developers can choose between pointer-based storage (for dynamic off-chain or cross-program-managed data) or mint-embedded storage (for simpler, on-chain-managed data).

<Callout type="info"> Only the [Token Extension Program](https://github.com/solana-program/token-2022/blob/6f2473344d70271f632c3e9b7e945be00186c536/interface/src/instruction.rs#L712-L725) provides this functionality. </Callout>
