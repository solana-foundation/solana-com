---
title: Transfer Fees
description: Learn how to implement transfer fees on token transfers.
---
## How to implement transfer fees on token transfers

The transfer fees extension is a mint extension that allows token creators to configure automatic fees that are collected on every token transfer. For example, a token creator may wish to charge a small percentage fee (like 0.5%) on all transfers to fund project development or create a deflationary mechanism. The fees are automatically deducted from transfers and can be withdrawn by designated fee withdrawal authorities. This extension can be enabled using the [`TransferFeeExtension`](https://github.com/solana-program/token-2022/blob/6f2473344d70271f632c3e9b7e945be00186c536/interface/src/instruction.rs#L579)

<Callout type="info">
  Only the [Token Extension
  Program](https://github.com/solana-program/token-2022/blob/6f2473344d70271f632c3e9b7e945be00186c536/interface/src/instruction.rs#L579)
