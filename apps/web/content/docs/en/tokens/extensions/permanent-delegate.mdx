...
description: Learn how to implement a permanent delegate on tokens.
---

## How to implement permanent delegate tokens

The permanent delegate extension is a mint extension that allows token creators to configure a permanent delegate with authority over all token accounts for that mint. Unlike regular delegates which can be revoked by token account owners, a permanent delegate cannot be changed or revoked and maintains perpetual authority over token operations. This extension is ideal for use cases such as regulatory compliance, automated token management, taxation systems, or scenarios requiring guaranteed oversight authority. The permanent delegate can perform operations like transfers, burns, and freezes on any token account from this mint, regardless of the account owner's consent. This extension can be enabled using the [`PermanentDelegateExtension`](https://github.com/solana-program/token-2022/blob/6f2473344d70271f632c3e9b7e945be00186c536/interface/src/instruction.rs#L678)


<Callout type="info">
  Only the [Token Extension
  Program](https://github.com/solana-program/token-2022/blob/6f2473344d70271f632c3e9b7e945be00186c536/interface/src/instruction.rs#L678)
