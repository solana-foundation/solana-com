...
The transaction's `signatures` array can contain up to 19 `Signature` structs.
...
The first signature must belong to the account that will pay the [transaction fees](/fees).
This signature becomes the transaction signature
and can be used to look up the transaction's details on the network.
...
struct that can be up to 3 bytes.
...
is an array of account addresses, sent in [compact array format](/docs/core/transactions#compact-array-format).
The first item in the array indicates its length.
Each subsequent item is a public key,
pointing to an account used by its instructions.
...
is an array of all the instructions to be processed, sent in [compact array format](/docs/reference/terminology#compact-array-format).
The first item in the array indicates its length.
Each subsequent item is a [`CompiledInstruction`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/instruction.rs#L22)
