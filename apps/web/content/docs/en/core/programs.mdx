...
Program developers generally revoke the upgrade authority when a program reaches the production stage.
This gives users confidence that the program's business logic will not change.
...
You can also check whether an existing program is verified
by searching for its program ID on the [Solana Explorer](https://explorer.solana.com/address/PhoeNiXZ8ByJGLkxNfZRnkUfjvmuYqLR89jjFHGqdXY).
## Verifying programs
