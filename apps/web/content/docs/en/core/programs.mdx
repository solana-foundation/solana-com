...
Programs are [accounts](https://solana.com/docs/core/accounts) that contain executable binary code.
The code determines how data in other accounts will be read, written, or modified—
often determining the rules for transactions.

To interact with a program,
users send [instructions](/docs/core/transactions#instruction) by means of
a [transaction](/docs/core/transactions#transactions).
...
by an account designated as its "upgrade authority".
(Typically the account that originally deployed the program.)
If the
[upgrade authority](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/bpf_loader/src/lib.rs#L894)
is revoked and set to `None`, the program can no longer be updated.
...
You can create a verifiable build with [Anchor's built-in support](https://www.anchor-lang.com/docs/verifiable-builds).

To check whether an existing program is verified, you may:

- Search for its program ID on [Solana Explorer](https://explorer.solana.com/address/PhoeNiXZ8ByJGLkxNfZRnkUfjvmuYqLR89jjFHGqdXY)
...
The ELF file contains the program's binary in Solana Bytecode Format (sBPF)
and is saved on-chain in an executable account.
...
Historically these were referred to as "native" programs
and they used to be distributed together with the validator code.
