...
If you think of an [instruction](/docs/core/instructions) as an API endpoint
...
When a transaction is processed, account privileges extend from one program to another.
...
function is used.
The function takes `signer_seeds`, which are the seeds for deriving signer [PDAs]((/docs/core/pda)).
