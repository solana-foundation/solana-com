...
Program Derived Addresses (PDAs) are a feature of Solana development that serves
two main use cases:
...
PDAs are addresses that look like public keys, but have no private keys. This
means it is not possible to generate a valid signature for the address. However,
...
The program includes a single [`initialize`](mention:initialize) instruction to
create a new account using a PDA as the address of the account. The new account
stores the address of the [`user` and the `bump` seed](mention:seeds) used to
derive the PDA.
...
In this example, the [seeds](mention:seeds) for PDA derivation include the fixed
string `data` and the address of the `user` account provided in the instruction.
The Anchor framework automatically finds the canonical
...
The [`init`](mention:init) constraint instructs Anchor to invoke the System
Program to create a new account using the PDA as the address. Anchor does this
