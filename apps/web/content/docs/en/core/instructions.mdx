...
To interact with the Solana network, one or more instructions are added to a
...
To execute this instruction, the sender's wallet sends the transaction containing its [signature](/docs/references/terminology#signature)
...
- [`program_id`](mention:program-id):
  The [ID](#program-id) of the program being invoked.
...
address of the program that contains the instruction's business logic.
