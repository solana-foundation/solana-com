...
区块链计算资源消耗的最小计量单位。计算单元用于衡量处理交易时所使用的计算资源。
...
中本聪系数是衡量去中心化程度的指标，它表示能够集体关闭区块链的最小独立实体数量。该术语由 Balaji
S. Srinivasan 和 Leland
Lee 在[《量化去中心化》](https://news.earn.com/quantifying-decentralization-e39db233c28e)中提出。

## 原生代币

用于记录[节点](#node)在集群中完成工作的[代币](#token)。
...
Solana 区块链上的可执行代码，用于解释每笔[交易](#transaction)中发送的[指令](#instruction)，以读取和修改其控制的账户。这些程序在其他区块链上通常被称为“[智能合约](/docs/core/programs)”。
...
奖励机制中的加权[积分](#credit)。在[验证者](#validator)的[奖励机制](https://docs.anza.xyz/consensus/stake-delegation-and-rewards)中，赎回时某个[质押](#stake)应得的点数是[投票积分](#vote-credit)与质押的 lamports 数量的乘积。
...
一组证明，每个证明都表明某些数据在该证明创建之前已经存在，并且在前一个证明之前经过了精确的时间段。类似于
[可验证延迟函数 (VDF)](#verifiable-delay-function-vdf)，历史证明可以在比生成所需时间更短的时间内进行验证。
...
优先费的计算方法是将请求的最大计算单元数乘以计算单元价格（以每计算单元 0.000001
lamports 为增量指定），并向上取整到最接近的 lamport。

交易应请求执行所需的最小计算单元数量，以尽量减少费用。
...
账户保持与其存储数据量成比例的最低 lamport 余额。所有新创建的账户都会永久存储在链上，直到账户被关闭。无法创建低于免租金门槛的账户。

## 根

在 [验证者](#validator) 上达到最大 [锁定](#lockout) 的 [区块](#block) 或
[槽](#slot)。根是验证者上所有活动分叉的祖先中最高的区块。根的所有祖先区块也通过传递性成为根。不是根的祖先且不是根的后代的区块将被排除在共识考虑之外，并可以被丢弃。

## runtime

负责[程序](#program)执行的[验证者](#validator)组件。
...
当前 epoch 中[跳过的 slot](#skipped-slot)占总领导者 slot 的百分比。由于在 epoch 边界之后样本量较小，以及对于领导者 slot 数量较少的验证者，这一指标的方差较高，因此可能具有误导性。然而，有时也可以用来识别节点配置错误。

## skipped slot

一个过去的[slot](#slot)，由于领导者离线或集群共识选择了更好的替代[分叉](#fork)而未生成[区块](#block)。跳过的 slot 不会作为后续 slot 的区块祖先，也不会增加[区块高度](#block-height)，也不会使最旧的
`recent_blockhash` 过期。

只有当一个 slot 比最新的[已根](#root)（因此未跳过）slot 更旧时，才能确定它是否被跳过。

## slot

每个[领导者](#leader)接收交易并生成[区块](#block)的时间段。

插槽共同构成了一个逻辑时钟。插槽按顺序排列且不重叠，根据[历史证明 (PoH)](#proof-of-history-poh)，每个插槽大致对应相等的现实时间。
...
如果可以证明[验证者](#validator)存在恶意行为，其代币将被没收至[集群](#cluster)。
...
一个系统[账户](#account)。[系统变量](https://docs.anza.xyz/runtime/sysvars)
提供集群状态信息，例如当前的 tick 高度、奖励[点数](#point)值等。程序可以通过系统变量账户（公钥）或通过系统调用查询访问系统变量。
...
相同的功能，但增加了扩展功能，例如保密转账、自定义转账逻辑、扩展元数据等更多功能。

## 代币铸造

一个可以生成（或“铸造”）代币的账户。不同的代币通过其唯一的代币铸造地址来区分。
...
自交易被接受到[账本](#ledger)以来的[确认区块](#confirmed-block)数量。当交易所在的区块成为[根](#root)时，该交易即被最终确认。
...
Solana 网络[集群](#cluster)中的完全参与者，负责生成新的[区块](#block)。验证者验证添加到[账本](#ledger)中的交易。
...
一种需要固定时间执行的函数，它会生成一个证明，证明其已运行，该证明可以在比生成所需时间更短的时间内验证。

## 投票

请参见 [轻客户端投票](#ledger-vote)。

## 投票积分

[验证者](#validator) 的奖励统计。当验证者在其投票账户中达到一个 [根](#root)
时，会获得一个投票积分。

## 钱包

一个 [密钥对](#keypair) 的集合，允许用户管理其资金。

## 预热期

在 [质押](#stake) 被委托后的若干个 [epochs](#epoch)
期间，质押逐步生效。在此期间，质押被视为“激活中”。更多信息请参见：[预热和冷却](https://docs.anza.xyz/consensus/stake-delegation-and-rewards#stake-warmup-cooldown-withdrawal)。
