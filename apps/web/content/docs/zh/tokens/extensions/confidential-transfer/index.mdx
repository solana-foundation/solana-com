...
Program 的本地 validator。您必须安装 Solana CLI 才能使用本地 validator。
...
| 指令                                | 说明                                                                                                                                          |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | 为 confidential transfers 设置 mint account。此指令必须与 _rs`TokenInstruction::InitializeMint`_ 指令在同一笔交易中包含。                     |
| _rs`UpdateMint`_                    | 更新 mint 的 confidential transfer 设置。                                                                                                     |
| _rs`ConfigureAccount`_              | 为 confidential transfers 设置 token account。                                                                                                |
| _rs`ApproveAccount`_                | 如果 mint 要求新 token account 需审批，则批准该 token account 以进行 confidential transfers。                                                 |
| _rs`EmptyAccount`_                  | 清空待处理和可用的 confidential 余额，以便关闭 token account。                                                                                |
| _rs`Deposit`_                       | 将公开 token 余额转换为待处理的 confidential 余额。                                                                                           |
| _rs`Withdraw`_                      | 将可用的 confidential 余额转换回公开余额。                                                                                                    |
| _rs`Transfer`_                      | 在 token account 之间进行 confidential token 转账。                                                                                           |
| _rs`ApplyPendingBalance`_           | 存款或转账后，将待处理余额转换为可用余额。                                                                                                    |
| _rs`EnableConfidentialCredits`_     | 允许 token account 接收 confidential token 转账。                                                                                             |
| _rs`DisableConfidentialCredits`_    | 阻止接收 confidential 转账，但仍允许公开转账。                                                                                                |
| _rs`EnableNonConfidentialCredits`_  | 允许 token account 接收公开 token 转账。                                                                                                      |
| _rs`DisableNonConfidentialCredits`_ | 阻止常规转账，使账户仅能接收 confidential 转账。                                                                                              |
| _rs`TransferWithFee`_               | 在 token account 之间以 confidential 方式并收取手续费进行转账。                                                                               |
| _rs`ConfigureAccountWithRegistry`_  | 使用 _rs`ElGamalRegistry`_ account 替代 _rs`VerifyPubkeyValidity`_ proof，作为配置 token account 以进行 confidential transfers 的另一种方式。 |
