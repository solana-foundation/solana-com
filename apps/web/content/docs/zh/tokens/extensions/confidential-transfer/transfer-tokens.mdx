...
要在两个 token account 之间机密地转移 token，发送方和接收方都必须配置好带有
_rs`ConfidentialTransferAccount`_ 状态并已获批用于机密转账的 token
account。发送方的 token account 还必须有可用的机密余额可供转出。
...
1. 在客户端创建
   ...
   **等式证明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**：用于验证转账后新的可用余额密文与其对应的
   [Pedersen 承诺](https://en.wikipedia.org/wiki/Commitment_scheme)是否一致，以确保源账户的新可用余额被正确计算为
...
- `confidential_transfer_create_context_state_account` 方法用于创建 proof
  account。
- `confidential_transfer_transfer` 方法用于调用 `Transfer` 指令。
- `confidential_transfer_close_context_state_account` 方法用于关闭 proof
  account。
