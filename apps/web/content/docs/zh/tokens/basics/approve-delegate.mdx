...
指令允许另一个账户（delegate）获得权限，从你的 token
account 转移指定数量的代币。delegate 最多只能转移被批准的数量，不能超额。只有 token
account 的所有者才能批准 delegate，并且每个 token
account 同一时间只能有一个 delegate。每次新的批准都会覆盖之前的设置。
