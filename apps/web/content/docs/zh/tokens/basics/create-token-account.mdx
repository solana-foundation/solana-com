...
Token Account 用于存储您某种特定代币的余额。每个 Token
...
1. System Program：创建一个为 token
   account 分配空间的账户，并将所有权转移给 Token Program
...
指令。请参阅[此处的实现](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/program/src/processor.rs#L66)。
