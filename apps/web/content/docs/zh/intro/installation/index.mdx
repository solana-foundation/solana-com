...
如果你的 Ubuntu 终端看起来和下图一样，可能会遇到一个问题，就是
...
接下来，关闭 Windows
Terminal，然后再次搜索 Ubuntu 打开一个 Linux 终端。此时终端应该和下图一样，`ctrl + v`（粘贴快捷键）可以正常使用了。
...
<Callout>
如果你在安装 `protobuf-compiler` 时遇到如下错误，请确保你先运行了 _shell`sudo apt-get update`
