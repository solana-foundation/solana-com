...
属性用于注解表示账户数据（即存储在 Account 数据字段中的数据类型）的结构体。
...
["bump" seed](/docs/core/pda#canonical-bump)，用于派生 Program Derived Address
(PDA)。存储该值可以节省计算资源，无需在后续指令中重复计算。
...
- `bump` - _rs`u8`_，用于存储
  ["bump" seed](/docs/core/pda#canonical-bump)，以派生 Program Derived Address
  (PDA)。存储该值可以节省计算资源，无需在后续指令中重复计算。

创建账户时，程序会将 _rs`MessageAccount`_
数据序列化，并存储到新账户的数据字段中。
...
_rs`Program<'info, T>`_ 类型要求账户与某个程序匹配。
...
1. 账户类型（如 _rs`Signer<'info>`_ 或
   _rs`Account<'info, T>`_），用于指定程序期望的账户类型
2. 可选约束（如 _rs`#[account(mut)]`_ 或
   _rs`#[account(init)]`_），用于定义额外的要求
...
_rs`String`_ 类型需要 4 字节来存储字符串长度，剩余长度为实际数据。

</Accordion>
</Accordions>

接下来，通过如下方式更新 `create` 函数，为 _rs`create`_ 指令添加业务逻辑：
...
1. _rs`user: Signer<'info>`_
   - 表示更新消息账户的用户
   - 标记为可变 (_rs`#[account(mut)]`_)，因为在需要时可能为 `message_account`
     支付更多空间
   - 必须签署交易
...
在本例中，程序使用存储在 _rs`message_account`_ 上的现有 bump seed。
...
1. _rs`user: Signer<'info>`_
   - 表示关闭消息账户的用户
   - 标记为可变（_rs`#[account(mut)]`_），以接收已关闭账户中的 lamports
   - 必须签名，以确保只有正确的用户可以关闭其消息账户
...
_rs`close`_ 约束已在 _rs`Delete`_ 结构体中处理账户关闭。
...
来运行测试文件，以检查其是否按预期运行。接下来的步骤将添加实际的测试内容。
...
在发送交易并创建账户后，测试文件使用其地址 (`messagePda`) 获取该账户。
...
首先，测试文件发送一笔调用 `update` 指令的交易，并传递新消息 "Hello, Solana!"。
...
发送交易并更新账户后，测试文件会使用其地址 (`messagePda`) 获取该账户。
...
发送交易并关闭账户后，示例会尝试使用其地址 (`messagePda`) 和 `fetchNullable`
获取该账户。当账户关闭后，该方法会返回 null。
