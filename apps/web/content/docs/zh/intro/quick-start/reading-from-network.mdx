...
Token Program 是一个可执行程序账户。程序与所有
...
是由 Token
Program 拥有的账户，存储特定代币的全局元数据。这包括总供应量、小数位数，以及被授权铸造或冻结代币的账户。mint
...
Mint account 存储的是状态，而不是可执行代码。它们由 Token Program 拥有，Token
Program 包括定义如何创建和更新 mint account 的指令。
...
例如，_shell`@solana/spl-token`_ 库包含 [_ts`getMint()`_](mention:one)
