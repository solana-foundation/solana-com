...
这意味着在给定相同可选 seed 和 `programId` 的情况下，具有不同值的 bump
seed 仍然可以派生出有效的 PDA。
...
来使用 PDA 作为地址创建新账户。用于创建 PDA 的 [seeds](mention:seeds) 包括：
...
指令，以创建一个带有程序派生地址的新账户。文件中包含
[derive the PDA](mention:pda) 的代码。
...
<Callout type="warn">
  如果你再次使用相同的 `user` 地址 seed 调用 `initialize` 指令，
