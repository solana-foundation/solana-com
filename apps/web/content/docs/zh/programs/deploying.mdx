...
| 更新现有程序                     | `solana program deploy <path_to_program.so>`（与部署相同）                                    |
...
这会默认在 `~/.config/solana/id.json` 生成一个 keypair。
...
对于 devnet 或 localhost，可使用 `solana airdrop` 命令为你的钱包充值：
...
- **`--with-compute-unit-price`**：设置每个计算单元的优先费用，单位为微 lamports（0.000001
  SOL）。可通过
  [Helius Priority Fee API](https://docs.helius.dev/guides/priority-fee-api)
  查询当前费率。优先费用是支付给当前 leader 以优先处理你交易的额外费用。
- **`--max-sign-attempts`**：如果交易过期，使用新 blockhash 重试的次数。（默认值：5）
- **`--use-rpc`**：将交易发送到配置的 RPC。此标志需要来自
  [Triton](https://triton.one/) 或 [Helius](https://www.helius.dev/) 等服务商的
  [基于质押权重](/developers/guides/advanced/stake-weighted-qos) RPC 连接。
