...
要针对某个仓库进行验证，仓库根目录下需要有一个 `Cargo.lock`
...
请确保你的程序已包含在 `workspace/members` 数组中，并且你的程序的 `Cargo.toml`
已正确配置 `lib` 名称。
...
中，你可以看到一个包含子文件夹程序的 workspace 示例。还请注意，当程序位于子文件夹时，后续需要将该文件夹作为
...
要可验证地构建你的 Solana 程序，请导航到包含 workspace `Cargo.toml`
文件的目录并运行：
...
> `cargo build-sbf`
> 覆盖它，因为这些操作很可能不会生成相同的哈希值，从而导致验证失败。
...
如果你想将验证锁定到某个特定版本，可以在命令中追加 `--commit-hash` 标志。
...
这将返回一个 base58 交易。如果你需要用于交易检查器的 base64 编码交易，可以使用
`--encoding base64`。
...
的镜像，并验证容器中可执行文件路径的哈希值是否与命令中提供的链上程序哈希值一致。由于构建已经上传到镜像，无需重新完整构建可执行文件，从而节省大量时间。

创建 `ellipsislabs/hello_world_verifiable_build:latest`
镜像的 Dockerfile 可在 ellipsis labs 仓库中找到
...
命令会获取 main 分支上的最后一次提交。如果你希望继续在仓库上开发，也可以通过
`commit-hash` 参数指定某个特定提交：
...
API 发送构建请求，触发程序的远程构建。构建完成后，系统会验证你的程序链上哈希值是否与从代码库生成的构建工件哈希值一致。
...
如果您希望实现完全无需信任的设置，可以自行运行
[Verify API](https://github.com/otter-sec/solana-verified-programs-api)，或者使用
`verify-from-repo` 命令，利用保存在
[PDA](https://explorer.solana.com/address/63XDCHrwZu3mXsw2RUFb4tbNpChuUHx4eA5aJMnHkpQQ/anchor-account)（由程序部署权限和
[verify program](https://explorer.solana.com/address/verifycLy8mB96wd9wqq3WDXQwM4oU6r42Th37Db9fC)
派生）上的链上验证数据，在本地自行进行程序验证。
...
除了已验证的构建之外，您还可以为您的程序添加一个 `security.txt`
文件。未来该功能实现后，`security.txt`
文件将保存验证者公钥，方便访问存储在验证 PDA 中的验证数据。包含所有构建和验证程序所需信息的 PDA，是由程序地址和验证者 pubkey 派生的。默认情况下，这与构建和部署该程序的 pubkey 相同，但也可以在
`security.txt` 文件中指定其他 pubkey。

`security.txt`
功能允许开发者将联系方式和安全信息直接嵌入到他们的 Solana 智能合约中。该方法受
[securitytxt.org](https://securitytxt.org)
启发，为安全研究人员提供了一种标准化方式，即使只知道合约地址，也能联系到项目维护者。

## 为什么使用 security.txt？

对于许多项目，尤其是较小或私有的项目，仅通过合约地址来识别开发者可能既困难又耗时。在程序中嵌入一个
`security.txt`
文件，可以确保安全研究人员能够轻松联系到相关人员，从而有助于防止漏洞被利用，并确保及时收到漏洞报告。

## 如何实现 security.txt

要在你的 Solana 程序中添加 `security.txt`，请按照以下步骤操作：

将 `solana-security-txt` 依赖项添加到你的 `Cargo.toml`：
...
在你的合约中使用 `security_txt!`
宏来定义安全信息。你可以包含联系方式、项目网址，甚至安全策略。示例如下：
...
一旦你的程序中嵌入了 `security.txt` 信息，就可以通过 Solana
Explorer 等工具轻松查询，确保任何希望报告潜在问题的人都能获取到你的联系方式和安全信息。

## 最佳实践

- 使用链接：对于可能会更改的信息（例如联系方式），建议链接到网页，而不是将其硬编码到合约中。这可以避免频繁的程序升级。

- 验证：在部署前，使用 `query-security-txt`
  工具验证格式和内容，该工具可校验链上程序和本地二进制文件：
