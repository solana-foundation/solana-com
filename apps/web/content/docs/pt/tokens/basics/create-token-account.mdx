...
accounts—você sempre pode derivar o endereço correto.
...
Esta única instrução lida automaticamente com a criação da token account e a
inicializa através de Cross Program Invocations (CPI).
