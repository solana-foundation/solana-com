...
Solana instalada para usar o validator local.
...
| Instrução                           | Descrição                                                                                                                                                                |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`InitializeMint`_                | Configura a conta de mint para transferências confidenciais. Esta instrução deve ser incluída na mesma transação que a instrução _rs`TokenInstruction::InitializeMint`_. |
| _rs`UpdateMint`_                    | Atualiza as configurações de transferência confidencial para um mint.                                                                                                    |
| _rs`ConfigureAccount`_              | Configura uma conta de token para transferências confidenciais.                                                                                                          |
| _rs`ApproveAccount`_                | Aprova uma conta de token para transferências confidenciais se o mint exigir aprovação para novas contas de token.                                                       |
| _rs`EmptyAccount`_                  | Esvazia os saldos confidenciais pendentes e disponíveis para permitir o encerramento de uma conta de token.                                                              |
| _rs`Deposit`_                       | Converte o saldo público de tokens em saldo confidencial pendente.                                                                                                       |
| _rs`Withdraw`_                      | Converte o saldo confidencial disponível de volta para saldo público.                                                                                                    |
| _rs`Transfer`_                      | Transfere tokens entre contas de token de forma confidencial.                                                                                                            |
| _rs`ApplyPendingBalance`_           | Converte o saldo pendente em saldo disponível após depósitos ou transferências.                                                                                          |
| _rs`EnableConfidentialCredits`_     | Permite que uma conta de token receba transferências confidenciais de tokens.                                                                                            |
...
| _rs`EnableNonConfidentialCredits`_  | Permite que uma conta de token receba transferências públicas de tokens.                                                                                                 |
...
| _rs`TransferWithFee`_               | Transfere tokens entre contas de token de forma confidencial com uma taxa.                                                                                               |
| _rs`ConfigureAccountWithRegistry`_  | Forma alternativa de configurar contas de token para transferências confidenciais usando uma conta _rs`ElGamalRegistry`_ em vez de prova _rs`VerifyPubkeyValidity`_.     |
