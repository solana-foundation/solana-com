...
1. Crie
   ...
   **Prova de igualdade
   ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**:
   Verifica se o texto cifrado do saldo disponível restante após o levantamento
   corresponde ao seu
   [compromisso de Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme),
   garantindo que o novo saldo disponível da conta é calculado corretamente como
   `remaining_balance = current_balance - withdraw_amount`.
...
A crate `spl_token_client` fornece os seguintes métodos:
