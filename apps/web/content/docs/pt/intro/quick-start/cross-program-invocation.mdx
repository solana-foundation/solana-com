...
Esta secção adiciona uma nova conta chamada `vault_account` à estrutura
_rs`Update`_. Esta conta serve como um "cofre" controlado pelo programa que
recebe SOL dos utilizadores quando atualizam as suas mensagens.
...
Aspetos principais do `vault_account`:

- O endereço da conta provém de um PDA derivado usando as seeds
  _rs`[b"vault", user.key().as_ref()]`_
- Como PDA, não tem chave privada, portanto apenas o programa pode "assinar"
  pelo endereço ao realizar CPIs
- Como tipo _rs`SystemAccount`_, o System Program é o seu proprietário, tal como
  contas de carteira normais
...
Na instrução `delete`, verá como o programa pode "assinar" por este PDA numa
CPI.
...
SOL da conta do utilizador para a conta vault.
...
A estrutura _rs`CpiContext`_ especifica o programa e as contas para uma
Invocação de Programa Cruzado (CPI).
...
Na instrução `update`, a implementação inclui uma Invocação de Programa Cruzado
(CPI) para invocar a instrução `transfer` do System Program. Isso demonstra como
realizar uma CPI de dentro do programa, permitindo a componibilidade dos
programas Solana.
...
- O programa a invocar (System Program)
...
- O `amount` a transferir (1.000.000 lamports, ou 0,001 SOL)
...
A configuração para uma CPI corresponde a como as instruções do lado do cliente
são construídas, onde você especifica o programa, as contas e os dados de
instrução para uma instrução específica a invocar. Quando a instrução `update`
do programa recebe uma invocação, ela invoca internamente a instrução de
transferência do System Program.
...
O `vault_account` usa a mesma derivação de PDA que na estrutura Update.

Adicionar o `vault_account` à estrutura Delete permite que o programa aceda à
conta do cofre do usuário durante a instrução de exclusão para transferir
qualquer SOL acumulado de volta ao usuário.
...
conta do cofre de volta para a conta do usuário.
...
Os _rs`signer_seeds`_ especificam as seeds opcionais e bump seeds usadas para
derivar o PDA.

### !with_signer

O método _rs`with_signer()`_ passa as signer seeds com o CPI.
...
Note que _rs`_ctx: Context<Delete>`_ muda para _rs`ctx: Context<Delete>`_ para
...
- O `cpi_context` (programa, contas e PDA signer)
- O valor a transferir (o saldo total da conta do cofre)
...
Em seguida, atualize o ficheiro `anchor.test.ts` para incluir a nova conta do
cofre nas instruções. Isto requer derivar o PDA do cofre e incluí-lo nas
chamadas das instruções de atualização e exclusão.
...
as alterações são automaticamente guardadas e persistidas.
