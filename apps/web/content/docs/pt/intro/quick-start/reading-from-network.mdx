...
Uma carteira é uma conta de propriedade do
[System Program](/docs/core/programs#the-system-program). As carteiras são
principalmente usadas para guardar SOL e assinar transações. Quando SOL é
...
campo está incluído para compatibilidade com versões anteriores.)
...
de carteira e contas de programa. A conta do programa armazena o bytecode
compilado para o
...
O Token Program é uma conta de programa executável. Os programas têm os mesmos
...
Cada conta de programa é propriedade do seu
...
contas que estão autorizadas a criar ou congelar tokens. O endereço da mint
account identifica exclusivamente um token na rede Solana.

O exemplo abaixo busca a conta Mint do USD Coin para demonstrar como o estado de
um programa é armazenado em uma conta separada.
...
O campo `data` da mint account armazena estado, não código executável, então o
...
O campo `data` contém o estado serializado da conta `Mint`, como a autoridade do
mint, fornecimento total e número de casas decimais.
...
Você pode visualizar os dados completamente deserializados da
