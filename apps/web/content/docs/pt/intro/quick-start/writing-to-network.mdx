...
Crie um `Connection` para lidar com o envio de transações e a obtenção de dados
de conta.

Neste exemplo, estamos a conectar ao validador de teste local que é executado em
`localhost:8899`.
...
Adicione SOL à conta do remetente. Em redes que não sejam a mainnet, pode usar o
método `requestAirdrop` para obter SOL para testes.
...
conta [`fromPubkey`](mention:from) para a conta [`toPubkey`](mention:to) pelo
...
Calcule os lamports mínimos necessários para um mint account. A função
`getMinimumBalanceForRentExemptMint` calcula quantos lamports devem ser alocados
para os dados num mint account.
