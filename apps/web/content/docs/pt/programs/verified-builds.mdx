...
Para verificar em relação a um repositório, ele precisa ter um arquivo
`Cargo.lock` no diretório raiz do seu repositório. Se você tiver apenas um
programa no seu repositório e um arquivo `cargo.lock` na raiz, pode ir
diretamente para o próximo passo e compilar o seu programa.

Se o seu programa estiver numa subpasta e você tiver um workspace Rust, precisa
criar um arquivo `Cargo.toml` de workspace no diretório raiz do seu repositório.
...
Com este arquivo `Cargo.toml` no lugar, pode então executar
`cargo generate-lockfile` para criar um arquivo de bloqueio e continuar para
...
diretório que contém o arquivo `Cargo.toml` do seu workspace e execute:
...
> Certifique-se de que realmente implementa a compilação verificada e não a
...
> Enquanto você executa a build verificada no diretório do seu programa, ao
> executar `verify-from-repo` você precisa adicionar a flag `--mount-path`. Este
> será o caminho para a pasta que contém o `Cargo.toml` que contém o nome da
...
> É recomendado usar um URL de RPC pago porque caso contrário você pode
> encontrar limites de taxa dos RPCs gratuitos. Então, em vez de `-um` você deve
> usar `--url yourRpcUrl` para uma verificação mais confiável.

A flag `--remote` envia uma solicitação de build para a API da OtterSec, que
...
Isto irá criar uma build verificável usando um contêiner docker com a versão da
Solana especificada no ficheiro `Cargo.lock`.
...
Quando você tem a autoridade do programa localmente, é solicitado que carregue
os dados da build na blockchain ao usar o comando
...
fornecido ao comando. Como a compilação já foi carregada numa imagem, não há
necessidade de uma recompilação completa do executável, o que pode demorar
bastante tempo.
...
Por predefinição, o comando `verify-from-repo` utiliza o último commit no ramo
principal. Também pode definir um commit específico caso queira continuar a
trabalhar no repositório usando o parâmetro `commit-hash`:
...
O comando `--remote` envia uma solicitação de compilação para a API da OtterSec,
...
Se deseja ter uma configuração completamente confiável, pode executar a
[API Verify](https://github.com/otter-sec/solana-verified-programs-api) você
mesmo ou executar a verificação do programa localmente usando o comando
`verify-from-repo` utilizando os dados de verificação na blockchain que são
guardados numa
[PDA](https://explorer.solana.com/address/63XDCHrwZu3mXsw2RUFb4tbNpChuUHx4eA5aJMnHkpQQ/anchor-account)
que é derivada da autoridade de implementação do programa e do
[programa verify](https://explorer.solana.com/address/verifycLy8mB96wd9wqq3WDXQwM4oU6r42Th37Db9fC).
...
Além das compilações verificadas, também pode adicionar um ficheiro
`security.txt` ao seu programa. No futuro, uma vez implementado, o
`security.txt` conterá a chave pública do verificador para acesso fácil aos
dados de verificação armazenados na PDA de verificação. A PDA contendo todas as
informações necessárias para construir e verificar um programa é derivada do
endereço do programa e da pubkey do verificador. Por padrão, esta é a mesma
pubkey que construiu e implementou o programa. Mas também pode ser outra pubkey
que pode ser especificada no `security.txt`.

A funcionalidade `security.txt` permite aos programadores incorporar informações
de contacto e segurança diretamente nos seus contratos inteligentes Solana.
Inspirada em [securitytxt.org](https://securitytxt.org), esta abordagem fornece
uma forma padronizada para investigadores de segurança contactarem os
responsáveis do projeto, mesmo que apenas conheçam o endereço do contrato.
...
pesquisadores de segurança possam facilmente contactar as pessoas corretas,
potencialmente prevenindo explorações e garantindo relatórios de bugs atempados.
...
Use a macro `security_txt!` no seu contrato para definir as suas informações de
segurança. Pode incluir detalhes de contacto, URLs do projeto e até uma política
de segurança. Aqui está um exemplo:
...
Uma vez que a informação `security.txt` esteja incorporada no seu programa, pode
ser facilmente consultada através de ferramentas como o Solana Explorer,
garantindo que os seus detalhes de contacto e segurança estejam disponíveis para
qualquer pessoa que pretenda reportar potenciais problemas.
...
- Verificação: Antes de implementar, verifique o formato e o conteúdo usando a
  ferramenta `query-security-txt`, que pode validar tanto programas onchain como
  binários locais:
