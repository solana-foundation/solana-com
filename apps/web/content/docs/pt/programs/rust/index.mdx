...
<Callout type="warn">
  Atualize o campo `edition` em `Cargo.toml` para `2021`. Caso contrário, você
  poderá encontrar um erro ao compilar o programa.
</Callout>
...
<CodePlaceholder title="Cargo.toml">
  O seu arquivo `Cargo.toml` deverá ficar assim:
</CodePlaceholder>
...
1. Um arquivo `.so` (por exemplo, `hello_world.so`): Este é o programa Solana
   compilado que será implementado na rede como um "smart contract".
2. Um arquivo de keypair (por exemplo, `hello_world-keypair.json`): A chave
...
Enquanto o test validator estiver em execução, execute o comando
`solana program deploy` num terminal separado para implantar o programa no
validator local.
