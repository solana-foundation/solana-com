...
No Solana, um contrato inteligente é chamado de programa. Um programa é uma
...
Quando um programa é implantado, o Solana usa [LLVM](https://llvm.org/) para
compilá-lo em formato executável e vinculável
([ELF](https://en.wikipedia.org/wiki/Executable_and_Linkable_Format)). O arquivo
ELF contém o binário do programa no Formato de Bytecode Solana (sBPF) e é salvo
na cadeia em uma conta executável.

<Callout>
  sBPF é a versão personalizada do Solana do bytecode
...
- [Anchor](https://www.anchor-lang.com/docs): Anchor é um framework projetado
  para desenvolvimento rápido e fácil no Solana. Ele usa
...
O Solana suporta [builds verificáveis](/docs/programs/verified-builds), que
...
[CLI de Build Verificável do Solana](https://github.com/Ellipsis-Labs/solana-verifiable-build)
...
Além dos programas carregadores, a Solana fornece os seguintes programas
pré-compilados.
...
pub struct Secp256k1RecoverOffsets {
    hash_offset: u16,                  // offset to start of hash
    hash_instruction_index: u16,       // instruction index to find hash
    recovery_id_offset: u16,           // offset to recovery id
    recovery_id_instruction_index: u16,// instruction index to find recovery id
    signature_offset: u16,             // offset to start of signature
    signature_instruction_index: u16,  // instruction index to find signature
}
```
...
<Callout>
  Valores S baixos são aplicados para todas as assinaturas para evitar a
  maleabilidade acidental da assinatura.
</Callout>
...
| **Vote**                 | `Vote111111111111111111111111111111111111111` | Criar e gerenciar contas que rastreiam o estado de votação e recompensas do validator                                                                                                                                                                                                                                                                                        | [VoteInstruction](https://docs.rs/solana-vote-program/latest/solana_vote_program/vote_instruction/enum.VoteInstruction.html)                          |
| **Stake**                | `Stake11111111111111111111111111111111111111` | Criar e gerenciar contas que representam stake e recompensas para delegações aos validators                                                                                                                                                                                                                                                                                  | [StakeInstruction](https://docs.rs/solana-sdk/latest/solana_sdk/stake/instruction/enum.StakeInstruction.html)                                         |
