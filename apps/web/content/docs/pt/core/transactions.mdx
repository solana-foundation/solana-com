...
- [Endereços de Conta](mention:account-addresses): Uma matriz de endereços de
  conta necessários pelas instruções na transação.
- [Blockhash Recente](mention:recent-blockhash): Atua como um timestamp para a
  transação.
- [Instruções](mention:instructions): Uma matriz de instruções a serem
  executadas.
