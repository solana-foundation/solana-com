...
Her Token Account'un verisinin, Token Account üzerinde kimin yetkiye sahip
...
_sh`spl-token`_ CLI, SPL token'ları ile denemeler yapmanıza yardımcı olur.
Aşağıdaki örnekler, yerel kurulum gerektirmeden CLI komutlarını doğrudan
tarayıcıda çalıştırmak için [Solana Playground](https://beta.solpg.io/)
terminalini kullanır.
...
_sh`create-token`_ komutu, mint yetkisi olarak Playground cüzdanınızla birlikte
...
_sh`create-account`_ komutu, sahip olarak cüzdan adresinizle bir associated
token account oluşturur.
...
Not: `<TOKEN_ADDRESS>` token hesabı için mint hesabını belirtir.
...
yetkisinin işlemi imzalaması gerekir. Talimat, bir token hesabına token basar ve
mint hesabındaki toplam arzı artırır. İşte
...
[Solana Playground](https://beta.solpg.io/660ced84cffcf4b13384d013)'daki bir
Javascript örneği.
