...
basıldıktan sonra token'ların token hesapları arasında transfer edilmesini
engeller. Bir kez basıldıktan sonra, token'lar kalıcı olarak token hesabında
kalır. Ancak, token hesabı sahibi yine de token'ları yakabilir ve bakiye sıfıra
