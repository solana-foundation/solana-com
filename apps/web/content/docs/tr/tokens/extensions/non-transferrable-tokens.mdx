...
description:
  InitializeNonTransferableMint özelliğini nasıl etkinleştireceğinizi öğrenin.
---
...
tokenların basıldıktan sonra token hesapları arasında transfer edilmesini
engeller. Bir kez basıldıktan sonra, tokenlar kalıcı olarak token hesabında
kalır. Ancak, token hesabı sahibi yine de tokenları yakabilir ve bakiye sıfıra
ulaştığında token hesabını kapatabilir.
