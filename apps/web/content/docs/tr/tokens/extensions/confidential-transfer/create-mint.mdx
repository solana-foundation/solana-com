...
3. **Mint'i başlat**: Standart mint durumunu başlatmak için Token Extension
   Program'ın _rs`Instruction::InitializeMint`_ talimatını çağır.

Bu talimatları manuel olarak yazabilseniz de, `spl_token_client` crate'i üç
talimatın tümünü tek bir fonksiyon çağrısında içeren bir işlem oluşturup
gönderen `create_mint` metodu sağlar; aşağıdaki örnekte gösterildiği gibi.
