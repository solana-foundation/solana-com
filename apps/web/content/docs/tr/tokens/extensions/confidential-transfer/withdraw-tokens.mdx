...
1. İstemci tarafında
   ...
   **Eşitlik kanıtı
   ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**:
   Çekimden sonra kalan kullanılabilir bakiye şifreli metninin karşılık gelen
   [Pedersen taahhüdü](https://en.wikipedia.org/wiki/Commitment_scheme) ile
   eşleştiğini doğrular, hesabın yeni kullanılabilir bakiyesinin
...
- `confidential_transfer_create_context_state_account` yöntemi bir kanıt hesabı
  oluşturur.
- `confidential_transfer_withdraw` yöntemi `Withdraw` talimatını çağırır.
- `confidential_transfer_close_context_state_account` yöntemi bir kanıt hesabını
  kapatır.
