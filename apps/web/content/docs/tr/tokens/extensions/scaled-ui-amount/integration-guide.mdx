...
- Bir kullanıcının canlı bakiyesi için yukarıdaki tutarlarla ilgili güncel
  bilgileri getTokenAccountBalance veya getAccountInfo çağrısı yaparak
  alabilirsiniz
- Rastgele bir tutar için UI Amount'u bilmeniz gerekiyorsa, bu hesaplamayı
  [`amountToUiAmountForMintWithoutSimulation`](https://github.com/solana-program/token-2022/blob/main/clients/js-legacy/src/actions/amountToUiAmount.ts#L164)
  (web3.js v1) fonksiyonunu çağırarak veya amountToUiAmount kullanarak bir işlem
  simüle ederek alabilirsiniz.
  - Not: amountToUiAmount bir işlem simülasyonu gerektirir, bu da bakiyesi olan
    geçerli bir ücret ödeyiciye ihtiyaç duyduğu anlamına gelir. Bu nedenle,
...
- Kullanıcılar ölçeklendirilmiş "UIAmount" olarak yorumlanacak tutarları
  girmelidir. Bunu işlemesi gereken uygulama, işlem için ham token tutarına
  dönüştürmelidir.
...
- Mevcut çarpan, `getAccountInfo` çağrısı yapılarak herhangi bir zamanda token
  mint'ten okunabilir. Ayrıca, gelecekteki bir çarpan ayarlanmışsa, bu bilgi de
  token mint'ten edinilebilir. UX'i karıştırabileceği için bu çarpanı
  göstermemenizi öneririz.
