---
title: Sync Native
description: Native SOL'u wrapped SOL'a dönüştürmeyi öğrenin.
---

## Native SOL'u senkronize etme

Wrapped SOL (WSOL), hesaptaki lamport miktarını token bakiyesi olarak takip eden
bir token hesabıdır. WSOL, SOL'u bir SPL token olarak transfer etmek için DeFi
protokolleriyle entegrasyonu sağlar.

[`SyncNative`](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/instruction.rs#L387)
talimatı, wrapped SOL (WSOL) token hesap bakiyesini içinde depolanan gerçek SOL
(lamport) ile senkronize eder. Native SOL'u bir WSOL token hesabına
aktardığınızda, token bakiyesi otomatik olarak güncellenmez. Doğru WSOL
bakiyesini yansıtmak için SyncNative'i çağırmanız gerekir.
