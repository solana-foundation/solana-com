---
title: Token Mint Etme
description: Yeni token birimlerini nasıl mint edeceğinizi öğrenin.
---

## Token Nasıl Mint Edilir

Token mint etmek, bir token programında
...
talimatını çağırarak yeni token birimleri oluşturma sürecini ifade eder.
Yalnızca mint account üzerinde mint yetkilisi olarak belirtilen adres yeni token
mint edebilir. Bu talimat ayrıca mint edilen tokenlerin hedefi olarak bir token
account'ın varlığını gerektirir.
