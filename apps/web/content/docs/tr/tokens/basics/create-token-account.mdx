...
<Callout type="info">
  Yalnızca Associated Token Program tarafından oluşturulan token account'lar
  "associated token account" olarak adlandırılır.
</Callout>

ATA'lar belirleyici adresler kullanır, bu da herhangi bir cüzdanın belirli bir
mint için token account'unu bulmanızı kolaylaştırır.
...
Bu, token account adreslerini manuel olarak takip etme ihtiyacını ortadan
...
- Belirleyici adreslerde (PDA'lar) token account'lar oluşturur
- Token Program'a Cross-Program Invocation (CPI) yapar
- Kendisi herhangi bir durum bilgisi tutmaz

Oluşturulan token account'lar Token Program tarafından sahiplenilir ve standart
...
Token account oluşturmak için iki talimata ihtiyacınız vardır:

1. System Program: Token account için ayrılmış alana sahip bir hesap oluşturun
   ve sahipliği Token Program'a aktarın
2. Token Program: Token account verilerini başlatın
...
Bu tek talimat, Cross Program Invocation (CPI) aracılığıyla token hesabı
