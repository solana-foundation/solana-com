...
Rust ile yazılmış Solana programları minimal yapısal gereksinimlere sahiptir ve
kodun nasıl organize edileceği konusunda esneklik sağlar. Tek gereksinim,
programın `entrypoint` içermesi ve bu noktanın programın yürütülmesinin nerede
başladığını tanımlamasıdır.
...
Temel SDK için `solana-program` ve serileştirme için `borsh` gereklidir.
...
deserialize edilmesini gerçekleştirir.

Bir Solana programı `entrypoint` fonksiyonu aşağıdaki imzaya sahiptir.
Geliştiriciler `entrypoint` fonksiyonunun kendi implementasyonlarını
...
Şimdi ana `process_instruction` fonksiyonunu, instructionları uygun handler
...
instructionu temsil eden bir enum varyantına deserialize edilir, ardından uygun
handler fonksiyonu çağrılır. Her handler fonksiyonu, o instruction için
implementasyonu içerir.

Aşağıdaki kodu `lib.rs` dosyasına ekleyerek `process_instruction` fonksiyonunu
güncelleyin ve `InitializeCounter` ile `IncrementCounter` instructionları için
handlerları ekleyin:
...
yapar. Yeni hesap, sahibi olarak programımızla oluşturulur ve bu da programımıza
...
- `counter_account` için hesabın `data` alanını okur
- Bunu bir `CounterAccount` yapısına deserialize eder
- `count` alanını 1 artırır
- `CounterAccount` yapısını hesabın `data` alanına geri serialize eder
...
Aşağıdaki kodu `lib.rs` dosyasına, program kodunun hemen altına ekleyin:
...
<Callout type="warn">
  `.so` dosyasını oluşturmak için testleri çalıştırmadan önce `cargo build-sbf`
...
Dağıtımı, program kimliğinizle birlikte `solana program show` komutunu
...
Yerel doğrulayıcı çalışırken, çıktı işlem imzalarını kullanarak işlemleri
...
ayarlanması gerektiğini unutmayın; bu, `solana-test-validator` uygulamasının
çalıştığı `http://localhost:8899` adresine varsayılan olarak ayarlanır.
