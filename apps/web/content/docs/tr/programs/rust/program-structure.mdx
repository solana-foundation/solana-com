...
description:
  ...
  Rust'ta Solana programlarını nasıl yapılandıracağınızı öğrenin.
...
- `state.rs`: Programa özgü durumu (hesap verilerini) tanımlar.
...
Birden fazla talimatla yerel bir Rust programının nasıl oluşturulacağını
...
Solana program giriş noktası, bir program çağrıldığında çalıştırılan
...
(bayt olarak sağlanan) üç değere
[ayrıştırır](https://github.com/solana-labs/solana/blob/v2.0/sdk/program/src/entrypoint.rs#L277)
ve bunları
...
- `accounts`: Çağrılan talimat için gereken hesapların `AccountInfo` bilgisi
- `instruction_data`: Yürütülecek talimatı ve gerekli argümanlarını belirten,
  programa iletilen ek veriler
...
depolanan ham baytlara dönüştürmek ve geri dönüştürmek için bir yönteme
ihtiyacınız vardır:
...
Solana program geliştirmede herhangi bir serileştirme formatını
kullanabilirsiniz, ancak [Borsh](https://borsh.io/) yaygın olarak kullanılır.
Solana programınızda Borsh kullanmak için:

1. `borsh` paketini `Cargo.toml` dosyanıza bağımlılık olarak ekleyin:

```shell title="Terminal"
cargo add borsh
```

2. Borsh özelliklerini içe aktarın ve struct'larınız için özellikleri uygulamak
...
programınızla etkileşimde bulunurken hangi eylemleri gerçekleştirebileceklerini
tanımlarlar.
...
Bir istemci programınızı çağırdığında, şu instruction data'yı (bayt tamponu
olarak) sağlamalıdır:
...
2. Varyanta göre eşleştirir ve kalan baytlardan ek parametreleri ayrıştırır
...
Sayaç programı için talimatları tanımlamak üzere aşağıdaki kodu `lib.rs`
dosyasına ekleyin.
...
Hesapların sırası önemlidir - istemci tarafında talimatı oluştururken, talimatın
başarıyla yürütülmesi için hesaplar programda tanımlandığı sırayla
sağlanmalıdır.
...
1. Hesabın veri alanına ayrılacak alanı (bayt cinsinden) belirtmek. Bir u64
   değeri (`count`) depoladığımız için 8 bayta ihtiyacımız var.
...
hesap oluştururken, ayırmak istediğimiz bayt sayısını ve yeni hesabın program
sahibini belirtiriz. System Program:
...
- Mevcut hesabın veri alanını değiştirilebilir şekilde ödünç alırız
- Ham baytları `CounterAccount` yapımıza deserialize ederiz
- `count` değerini güncelleriz
- Değiştirilmiş yapıyı hesabın veri alanına geri serialize ederiz
...
çalıştırmak için `cargo test-sbf` komutunu kullanın. İsteğe bağlı olarak,
çıktıda yazdırma ifadelerini görmek için `--nocapture` bayrağını
kullanabilirsiniz.
...
hesabının adresi olarak kullanılacak yeni bir keypair oluşturun ve sayaç için
