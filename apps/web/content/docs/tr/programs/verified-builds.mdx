...
> Ücretli bir RPC URL'si kullanmanız önerilir, aksi takdirde ücretsiz RPC'lerin
> hız limitleriyle karşılaşabilirsiniz. Bu nedenle `-um` yerine daha güvenilir
> bir doğrulama için `--url yourRpcUrl` kullanmalısınız.
...
container ile doğrulanabilir bir yapı oluşturur.
...
İsteğe bağlı: Önce yerel olarak doğrulayıp doğrulayamayacağınızı görün (bu komut
örnek program ID'si `6XBGfP17P3KQAKoJb2s5M5fR4aFTXzPeuC1af2GYkvhD` kullanır):
...
komutunu kullanırken yapı verilerini zincir üstüne yüklemeniz istenir.
...
imajı yükler ve konteyner içindeki çalıştırılabilir dosya yolunun hash'inin,
komuta sağlanan zincir üzerindeki programın hash'i ile aynı olduğunu doğrular.
Yapı zaten bir imaja yüklendiği için, uzun sürebilecek çalıştırılabilir dosyanın
tam yeniden oluşturulmasına gerek yoktur.
