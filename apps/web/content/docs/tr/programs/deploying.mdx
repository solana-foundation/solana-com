...
| Program yetkisini aktar             | `solana program set-upgrade-authority <program-id> --new-upgrade-authority <path_to_keypair>` |
...
Hangi Solana cluster'ına dağıtım yapacağınızı seçin. Mevcut yapılandırmanızı
...
Devnet veya localhost için cüzdanınızı `solana airdrop` komutuyla fonlayın:
...
Bir programı derlemek için `cargo build-sbf` komutunu kullanın:
...
Programınızı `cargo build-sbf` komutunu kullanarak derleyin:
...
Programınızı aynı `solana program deploy` komutunu kullanarak güncelleyin:
...
Programınızı değiştirilemez hale getirmek için, program yetkisini kaldırmak
üzere `--final` bayrağıyla birlikte `solana program set-upgrade-authority`
komutunu kullanın:
...
- **`--with-compute-unit-price`**: Hesaplama birimi başına mikro-lamport
  (0.000001 SOL) cinsinden öncelik ücretini ayarlayın. Güncel oranlar için
  [Helius Öncelik Ücreti API'sine](https://docs.helius.dev/guides/priority-fee-api)
  bakın. Öncelik ücreti, işleminize öncelik vermesi için mevcut lidere ödenen ek
  bir ücrettir.
