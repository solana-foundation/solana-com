...
`update` ve `delete` talimatlarının, System Program'ı çağırarak hesaplar
arasında SOL transferlerini yönetmek için değiştirilmesi gerekir.
...
İlk olarak, programın _rs`Update`_ yapısını ve `update` fonksiyonunu
değiştirerek basit bir "güncelleme için ödeme" mekanizmasına ihtiyacı vardır.

`lib.rs` dosyasını güncelleyerek `system_program` modülündeki öğeleri kapsama
alarak başlayın.
...
Ardından, _rs`Update`_ yapısını `vault_account` adlı yeni bir hesap içerecek
şekilde güncelleyin. Program tarafından kontrol edilen bu hesap, kullanıcılar
...
hesap, kullanıcılar mesajlarını güncellediklerinde onlardan SOL alan, program
tarafından kontrol edilen bir "kasa" görevi görür.
...
- PDA olduğu için özel anahtarı yoktur, bu nedenle CPI'lar gerçekleştirilirken
  yalnızca program adres için "imzalayabilir"
- _rs`SystemAccount`_ türü olarak, normal cüzdan hesapları gibi System Program
...
Ardından, kullanıcının hesabından kasa hesabına 0.001 SOL transfer etmek için
`update` talimatına CPI mantığını ekleyin.
...
_rs`CpiContext`_ yapısı, Programlar Arası Çağrı (CPI) için programı ve hesapları
belirtir.
...
için bir Programlar Arası Çağrı (CPI) içerir. Bu, program içinden nasıl CPI
gerçekleştirileceğini gösterir ve Solana programlarının birleştirilebilirliğini
sağlar.

_rs`Transfer`_ yapısı, System Program'ın transfer talimatı için gereken
hesapları belirtir:

- `from` - Kullanıcının hesabı (fon kaynağı)
- `to` - Kasa hesabı (fon hedefi)
...
Bir CPI kurulumu, istemci tarafı talimatların nasıl oluşturulduğuyla eşleşir;
çağrılacak belirli bir talimat için programı, hesapları ve instruction data'yı
belirtirsiniz. Programın `update` talimatı bir çağrı aldığında, dahili olarak
System Program'ın transfer talimatını çağırır.
...
sırasında iade" mekanizması ekleyin.

İlk olarak, _rs`Delete`_ yapısını `vault_account` hesabını içerecek şekilde
güncelleyin. Bu, kullanıcılar mesaj hesaplarını kapattıklarında kasadaki
herhangi bir SOL'un kullanıcıya geri aktarılmasını sağlar.
...
Ayrıca `system_program` ekleyin çünkü transfer için CPI, System Program'ın
çağrılmasını gerektirir.
...
`vault_account`, Update yapısındakiyle aynı PDA türetimini kullanır.

`vault_account` öğesinin Delete yapısına eklenmesi, programın silme talimatı
sırasında kullanıcının kasa hesabına erişmesini sağlayarak birikmiş SOL'u
kullanıcıya geri aktarabilir.
...
bump seed'lerini belirtir.

### !with_signer

_rs`with_signer()`_ metodu, imzalayan seed'lerini CPI ile birlikte iletir.
...
_rs`_ctx: Context<Delete>`_ öğesinin, fonksiyon gövdesinde bağlamı kullanmak
için _rs`ctx: Context<Delete>`_ olarak değiştiğine dikkat edin.
...
- `cpi_context` (program, hesaplar ve PDA imzalayan)
- Aktarılacak miktar (kasa hesabının tüm bakiyesi)
...
Şimdi, talimatlara yeni kasa hesabını dahil etmek için `anchor.test.ts`
dosyasını güncelleyin. Bu, kasa PDA'sını türetmeyi ve güncelleme ve silme
...
otomatik olarak kaydedilir ve kalıcı hale gelir.
