...
64 bayttır ve işlemin `Message` değerinin hesabın özel anahtarıyla
imzalanmasıyla oluşturulur. İşlemin talimatlarında yer alan her
...
gönderilen hesap adreslerinden oluşan bir dizidir. Dizinin öneki uzunluğunu
belirtir. Dizideki her öğe, talimatları tarafından kullanılan bir hesaba işaret
eden bir public key'dir. `accounts_keys` dizisi eksiksiz ve aşağıdaki gibi kesin
bir sırayla düzenlenmiş olmalıdır:
...
<Callout>
  ...
  kısmındaki bilgilerle birleştirilerek her hesap için izinlerin belirlenmesini
  sağlar.
</Callout>
...
Mesajın `recent_blockhash` kısmı, işlem zaman damgası görevi gören ve yinelenen
...
Blok süresi dolduktan sonra işlem geçersiz hale gelir ve işlenemez.

<Callout>
  [`getLatestBlockhash`](/docs/rpc/http/getlatestblockhash) RPC metodu, mevcut
  blockhash'i ve blockhash'in geçerli olacağı son blok yüksekliğini almanızı
  sağlar.
</Callout>
...
kısmı,
[kompakt dizi formatında](/docs/references/terminology#compact-array-format)
gönderilen, işlenecek tüm talimatlardan oluşan bir dizidir. Dizinin öneki
uzunluğunu belirtir. Dizideki her öğe bir
...
2. [`accounts`](mention:account-indexes): `account_keys` dizisindeki adreslere
   işaret eden indekslerden oluşan bir dizi. Her indeks, bu talimat için gerekli
   olan bir hesabın adresine işaret eder.
3. [`data`](mention:instruction-data): Program üzerinde hangi talimatın
   çağrılacağını belirten bir byte dizisi. Ayrıca talimat için gerekli olan ek
   verileri de içerir. (Örneğin, fonksiyon argümanları)
