...
De kleinste eenheid voor het meten van verbruik van computationele bronnen van
de blockchain. Compute units worden gebruikt om de computationele bronnen te
meten die nodig zijn voor het verwerken van een transactie.
...
aantal onafhankelijke entiteiten dat gezamenlijk kan handelen om een blockchain
uit te schakelen. De term is bedacht door Balaji S. Srinivasan en Leland Lee in
[Quantifying Decentralization](https://news.earn.com/quantifying-decentralization-e39db233c28e).
...
is het aantal punten dat aan een [stake](#stake) verschuldigd is tijdens
...
het bewijs werd gecreëerd en dat er een precieze tijdsduur verstreek voor het
vorige bewijs. Net als een [VDF](#verifiable-delay-function-vdf), kan een Proof
of History geverifieerd worden in minder tijd dan het kostte om te produceren.
...
De prioriteringsvergoeding wordt berekend door het gevraagde maximum aantal
compute-eenheden te vermenigvuldigen met de compute-eenheidsprijs
(gespecificeerd in stappen van 0,000001 lamport per compute-eenheid), afgerond
...
vereiste dat R een gecomprimeerd Edwards-punt is dat niet van kleine orde is en
S een scalar in het bereik van `0 <= S < L`. Deze vereiste zorgt ervoor dat er
geen handtekeningveranderlijkheid is. Elke transactie moet ten minste één
handtekening hebben voor de [fee account](#fee-account). Daarom kan de eerste
handtekening in een transactie worden behandeld als
...
variantie heeft na de epochgrens wanneer de steekproefomvang klein is, evenals
voor validators met een laag aantal leader slots, maar kan ook nuttig zijn bij
het identificeren van verkeerde configuraties van nodes.
...
niet-overlappend, en beslaan ongeveer gelijke periodes in de echte wereld
...
[Sysvars](https://docs.anza.xyz/runtime/sysvars) bieden informatie over de
clusterstatus zoals huidige tick-hoogte, rewards [points](#point) waarden, enz.
Programma's kunnen Sysvars benaderen via een Sysvar-account (pubkey) of door een
query via een syscall.
...
Een [entry](#entry) in het grootboek dat de duur van de kloktijd schat.
...
program ID `TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb` en bevat alle dezelfde
functies als het [Token Program](#token-program), maar komt met uitbreidingen
zoals vertrouwelijke overdrachten, aangepaste overdrachtlogica, uitgebreide
metadata en veel meer.
...
Zie [verifieerbare vertragingsfunctie](#verifiable-delay-function-vdf).

## verifiable delay function (VDF)

Een functie die een vaste hoeveelheid tijd nodig heeft om uit te voeren en die
een bewijs produceert dat het is uitgevoerd, wat vervolgens in minder tijd kan
