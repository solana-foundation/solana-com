...
De kleinste eenheid voor het meten van het verbruik van computationele bronnen
van de blockchain. Compute units worden gebruikt om de computationele bronnen te
meten die nodig zijn om een transactie te verwerken.
...
worden vaak "[_smart contracts_](/docs/core/programs)" genoemd op andere
blockchains.
...
De publieke sleutel van een [keypair](#keypair).
...
Een [blok](#block) of [slot](#slot) dat de maximale [lockout](#lockout) op een
[validator](#validator) heeft bereikt. De root is het hoogste blok dat een
...
vereiste dat R een gepakt Edwards-punt is dat niet van kleine orde is en S een
scalar in het bereik van `0 <= S < L`. Deze vereiste zorgt ervoor dat er geen
handtekeningveranderlijkheid is. Elke transactie moet ten minste één
...
clusterstatus zoals huidige tick-hoogte, belonings[punten](#point)-waarden, enz.
...
Een [invoer](#entry) in het grootboek die de duur van de kloktijd schat.
...
Het [Token Program](https://spl.solana.com/token) heeft het program ID
`TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA`, en biedt de
basisfunctionaliteiten voor het overdragen, bevriezen en minten van tokens.
...
Een beloningsoverzicht voor [validators](#validator). Een vote credit wordt
