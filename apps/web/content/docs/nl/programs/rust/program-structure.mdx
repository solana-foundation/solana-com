...
een programma een `entrypoint` moet hebben, dat definieert waar de uitvoering
...
niet opneemt, zal de `target/deploy` map niet worden gegenereerd wanneer je het
programma bouwt.
...
macro uit de `solana_program` crate om de entrypoint in je programma te
definiëren.
...
de entrypoint de
[invoergegevens](https://github.com/solana-labs/solana/blob/v2.0/sdk/program/src/entrypoint.rs#L129-L131)
...
- `program_id`: De publieke sleutel van het programma dat wordt aangeroepen
  (huidige programma)
- `accounts`: De `AccountInfo` voor accounts die vereist zijn door de instructie
  die wordt aangeroepen
- `instruction_data`: Aanvullende gegevens die aan het programma worden
  doorgegeven en die de uit te voeren instructie en de vereiste argumenten
  specificeren

Deze drie parameters komen rechtstreeks overeen met de gegevens die clients
moeten verstrekken bij het bouwen van een instructie om een programma aan te
roepen.
...
gegevensindeling van de accounts van je programma weergeven. Je kunt meerdere
structs definiëren om verschillende soorten accounts voor je programma te
representeren.
...
Hoewel je elk serialisatieformaat kunt gebruiken voor
...
Voeg de `CounterAccount` struct toe aan `lib.rs` om de programma-status te
...
buffer van bytes) verstrekken waarbij:
...
1. Maakt een nieuw account aan en wijst ruimte toe om de countergegevens op te
   slaan
...
1. Het counter account dat zal worden aangemaakt en geïnitialiseerd
2. Het betaler account dat de aanmaak van het nieuwe account zal financieren
3. Het System Program dat we aanroepen om het nieuwe account aan te maken
...
accounts die vereist zijn voor de instructie.
...
gedefinieerd in het programma, zodat de instructie succesvol kan worden
uitgevoerd.
...
2. Het minimale "rent"-saldo berekenen dat vereist is. Op Solana moeten accounts
   ...
   gegevens die op het account is opgeslagen.
...
3. Draagt het eigendom over aan het gespecificeerde programma

Deze eigendomsoverdracht is belangrijk omdat alleen de programma-eigenaar van
een account de gegevens van een account kan wijzigen. In dit geval stellen we
ons programma in als de eigenaar, waardoor we de gegevens van het account kunnen
wijzigen om de counter-waarde op te slaan.
...
programma's - in dit geval, de System Program's `create_account` instructie.
...
Merk op dat in de praktijk een ontwikkelaar verschillende veiligheidscontroles
...
- Lenen we het gegevensveld van het bestaande account muteerbaar
...
Om de initialisatie-instructie te testen:
...
- Verstuur een transactie met de initialisatie-instructie
