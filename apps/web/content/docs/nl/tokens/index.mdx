...
aangeeft wie bevoegdheid heeft over het token account. Dit verschilt van de
...
De _sh`spl-token`_ CLI helpt je experimenteren met SPL-tokens. De onderstaande
voorbeelden gebruiken de [Solana Playground](https://beta.solpg.io/) terminal om
CLI-commando's direct in de browser uit te voeren zonder lokale installatie.

Het aanmaken van tokens en accounts vereist SOL voor account rent deposits en
transactiekosten. Voor nieuwe Solana Playground gebruikers, maak een Playground
wallet aan en voer het _sh`solana airdrop`_ commando uit in de Playground
terminal. Je kunt ook devnet SOL krijgen van de publieke
...
De accountadressen die in je terminal output worden getoond verschillen van de
onderstaande voorbeelden. Gebruik de adressen uit je Playground terminal wanneer
je meevolgt. Bijvoorbeeld, het _sh`create-token`_ commando geeft een mint
...
Het _sh`supply`_ commando voor een nieuwe token retourneert `0`:
...
Het _sh`create-account`_ commando maakt een associated token account aan met je
walletadres als de eigenaar.
...
De _rs`Transfer`_ instructie op het Token Program verwerkt token transfers. De
eigenaar van het token account van de verzender moet de transactie ondertekenen.
De instructie verplaatst tokens tussen token accounts. Hier is een Javascript
voorbeeld op
