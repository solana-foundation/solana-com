...
voorkomt dat tokens worden overgedragen tussen token accounts na het minten.
Eenmaal gemint, blijven de tokens permanent in het token account. De eigenaar
van het token account kan echter nog steeds tokens verbranden en het token
account sluiten wanneer het saldo nul bereikt.
