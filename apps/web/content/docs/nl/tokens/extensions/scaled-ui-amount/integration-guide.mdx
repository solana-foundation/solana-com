...
- Voor het live saldo van een gebruiker kun je bijgewerkte informatie over
...
- Gebruikers moeten bedragen invoeren die geïnterpreteerd moeten worden als het
  geschaalde “UIAmount”. De app die dit moet verwerken, moet dit converteren
  naar het ruwe tokenbedrag voor de transactie.
...
- De huidige vermenigvuldiger kan op elk moment worden uitgelezen van de token
  mint door `getAccountInfo` aan te roepen. Bovendien, als er een toekomstige
  vermenigvuldiger is ingesteld, is deze informatie ook beschikbaar via de token
  mint. We raden aan deze vermenigvuldiger niet te tonen, omdat dit de UX kan
  verwarren.
