...
invocations (CPI's). Wanneer ingeschakeld, zorgt deze extensie ervoor dat tokens
alleen kunnen worden overgedragen via directe aanroepen naar de
transfer-instructie van het Token Extensions Program, waarbij alle andere
programma's worden geblokkeerd om tokens via CPI over te dragen.
