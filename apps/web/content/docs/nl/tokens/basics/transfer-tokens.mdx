...
Het overmaken van tokens houdt in dat je tokens verplaatst van het ene token
account naar een ander token account die dezelfde mint delen. Het overmaken van
tokens gebeurt wanneer je de
...
instructie aanroept op een token program. Alleen het adres dat is opgegeven als
eigenaar (authority) van het bron-token account kan tokens uit het account
overmaken.
