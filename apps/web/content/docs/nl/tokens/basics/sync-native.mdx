...
Wrapped SOL (WSOL) is een token account dat het aantal lamports in de account
...
instructie synchroniseert een wrapped SOL (WSOL) token account saldo met de
werkelijke SOL (lamports) die erin is opgeslagen. Wanneer je native SOL naar een
