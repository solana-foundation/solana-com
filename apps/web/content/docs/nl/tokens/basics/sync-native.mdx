---
title: Sync Native
description: Leer hoe je native SOL naar wrapped SOL kunt converteren.
---

## Sync Native SOL

Wrapped SOL (WSOL) is een token account dat het aantal lamports in het account
bijhoudt als het token saldo. WSOL maakt integratie met DeFi-protocollen
mogelijk om SOL als een SPL-token over te dragen.
...
instructie synchroniseert het saldo van een wrapped SOL (WSOL) token account met
de werkelijke SOL (lamports) die erin zijn opgeslagen. Wanneer je native SOL
overmaakt naar een WSOL token account, wordt het token saldo niet automatisch
bijgewerkt. Je moet SyncNative aanroepen om het juiste WSOL saldo weer te geven.
