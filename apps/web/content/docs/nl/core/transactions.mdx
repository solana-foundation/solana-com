...
[instructies](/docs/core/transactions#instruction) die te verwerken bewerkingen
specificeren. De uitvoeringslogica voor instructies is opgeslagen op
...
Je kunt een transactie zien als een envelop met formulieren. Elk formulier is
een instructie die het netwerk vertelt wat te doen. Het verzenden van de
transactie is als het versturen van de envelop om de formulieren te laten
verwerken.
...
- Elke instructie vereist drie stukken informatie:
  ...
  3. Eventuele extra gegevens die nodig zijn voor de instructie (bijv.
     functieargumenten)
...
Na het verzenden van de transactie verwerkt het System Program de transfer
instructie. Het System Program werkt vervolgens de lamport-saldi bij van zowel
het verzendende als het ontvangende account.

![SOL Transfer proces](/assets/docs/core/transactions/sol-transfer-process.svg)

De onderstaande voorbeelden laten zien hoe je een transactie verzendt die SOL
overmaakt van het ene account naar het andere. Bekijk de broncode van de
...
In de onderstaande secties behandelen we de details van transacties en
instructies.
...
op Solana [program](/docs/core/accounts#program-account) kan worden gezien als
een publieke functie die door iedereen via het Solana-netwerk kan worden
aangeroepen.
...
`POST` verzoek naar een API-eindpunt, waardoor gebruikers de bedrijfslogica van
...
- [Program ID](mention:program-id): Het adres van het programma met de
  bedrijfslogica voor de instructie die wordt aangeroepen.
- [Accounts](mention:accounts): De lijst van alle accounts waaruit de instructie
  leest of waarin deze schrijft.
- [Instruction Data](mention:instruction-data): Een byte-array dat specificeert
  welke instructie op het programma moet worden aangeroepen en eventuele
  argumenten die de instructie vereist.
...
Bij het maken van een `Instruction` moet je elk vereist account opgeven als een
[`AccountMeta`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/instruction/src/account_meta.rs#L25).
De `AccountMeta` specificeert het volgende:

- [pubkey](mention:pubkey): Het adres van het account
- [is_signer](mention:is-signer): Of het account de transactie moet ondertekenen
- [is_writable](mention:is-writable): Of de instructie de gegevens van het
  account wijzigt
...
In de praktijk hoef je meestal niet handmatig een `Instruction` te maken. De
...
De volgende voorbeelden tonen de uitvoer van de vorige codefragmenten. Het
exacte formaat verschilt afhankelijk van de SDK, maar elke Solana-instructie
vereist de volgende informatie:

<WithMentions>

- [Program ID](mention:program-id): Het adres van het programma dat de
  instructie zal uitvoeren.
- [Accounts](mention:accounts): Een lijst met accounts die vereist zijn voor de
  instructie. Voor elk account moet de instructie het adres specificeren, of het
  de transactie moet ondertekenen, en of er naar geschreven zal worden.
- [Data](mention:data): Een bytebuffer die het programma vertelt welke
  instructie uitgevoerd moet worden en eventuele argumenten bevat die de
...
het creëren van een `Transaction` en het toevoegen van de instructies aan de
...
1. [Handtekeningen](mention:signatures): Een array van
   [handtekeningen](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/signature/src/lib.rs#L30)
   van alle accounts die vereist zijn als ondertekenaars voor de instructies in
   de transactie. Een handtekening wordt gemaakt door de transactie `Message` te
   ondertekenen met de privésleutel van het account.
2. [Bericht](mention:message): Het transactie
   [bericht](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/message/legacy.rs#L131)
   bevat de lijst met instructies die atomisch moeten worden verwerkt.
...
![Transactieformaat](/assets/docs/core/transactions/tx_format.png)

De structuur van een transactiebericht bestaat uit:

<WithMentions>

- [Berichtkop](mention:message-header): Specificeert het aantal ondertekenaars
  en alleen-lezen accounts.
- [Accountadressen](mention:account-addresses): Een reeks accountadressen die
  vereist zijn voor de instructies in de transactie.
- [Recente Blockhash](mention:recent-blockhash): Fungeert als een tijdstempel
  voor de transactie.
- [Instructies](mention:instructions): Een reeks instructies die uitgevoerd
  moeten worden.
...
- Bericht: Kop (3 bytes), accountsleutels (32 bytes elk), recente blockhash (32
  bytes), en instructies

![Transactieformaat](/assets/docs/core/transactions/issues_with_legacy_txs.png)

### Berichtkop

De
[berichtkop](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/message/mod.rs#L97)
specificeert de rechten voor het account in de transactie. Het werkt in
combinatie met de strikt geordende
[accountadressen](/docs/core/transactions#array-of-account-addresses) om te
bepalen welke accounts ondertekenaars zijn en welke beschrijfbaar zijn.

<WithMentions>
1. [Het aantal handtekeningen vereist voor alle instructies in de transactie.](mention:num_required_signatures)
2. [Het aantal ondertekende accounts dat alleen-lezen is.](mention:num_readonly_signed_accounts)
3. [Het aantal niet-ondertekende accounts dat alleen-lezen is.](mention:num_readonly_unsigned_accounts)
...
Een compact array in een transactiebericht is een array die geserialiseerd is in
...
Dit formaat wordt gebruikt om de lengtes van de
...
Een transactiebericht bevat een enkele lijst van alle
...
`MessageHeader` en een strikte ordening van de accountadressen om permissies te
bepalen.
...
De `MessageHeader` levert de waarden die worden gebruikt om het aantal accounts
...
als verlopen wordt beschouwd en niet meer kan worden verwerkt.
...
Net als de array met accountadressen in het bericht, begint het met een
[compact-u16](/docs/core/transactions#compact-array-format) lengte gevolgd door
de instructiegegevens. Elke instructie bevat:

<WithMentions>

1. [Program ID Index](mention:program-id-index): Een index die verwijst naar het
   programma-adres in de array met accountadressen. Dit specificeert het
   programma dat de instructie verwerkt.
2. [Account Indexes](mention:account-indexes): Een array van indexen die
   verwijzen naar de accountadressen die vereist zijn voor deze instructie.
3. [Instruction Data](mention:instruction-data): Een byte-array die specificeert
   welke instructie moet worden aangeroepen op het programma en eventuele
   aanvullende gegevens die nodig zijn voor de instructie (bijv.
   functieargumenten).
...
enkele SOL overdrachtsinstuctie te zien.
...
De volgende voorbeelden tonen de uitvoer van het transactiebericht uit de vorige
...
structuur hebben die vergelijkbaar is met het volgende fragment. Als alternatief
kun je de transactie inspecteren met
