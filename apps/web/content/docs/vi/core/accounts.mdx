...
Ví dụ dưới đây minh họa cách sử dụng SDK Solana để tạo một keypair.
...
[Program Derived Address](/docs/core/pda) (PDA) là một địa chỉ được tạo ra một
cách xác định bằng cách sử dụng ID của một chương trình và một hoặc nhiều đầu
vào tùy chọn (seeds). Ví dụ dưới đây minh họa cách sử dụng SDK Solana để tạo một
Program Derived Address.
...
- [`lamports`](mention:lamports): Số lượng [lamports](#lamports) trong tài khoản
- [`data`](mention:data): [Dữ liệu](#data) của tài khoản
- [`owner`](mention:owner): ID của chương trình [sở hữu](#owner) tài khoản
- [`executable`](mention:executable): Chỉ ra liệu tài khoản có chứa mã nhị phân
  [có thể thực thi](#executable) hay không
- [`rent_epoch`](mention:rent_epoch): Trường [epoch rent](#rent-epoch) đã không
  còn được sử dụng
...
Mỗi tài khoản phải có số dư lamport tối thiểu, được gọi là
...
<Callout type="info">
  Mặc dù số dư này được gọi là rent, nhưng nó hoạt động giống như một khoản đặt
  cọc hơn, vì toàn bộ số dư có thể được thu hồi khi tài khoản bị đóng. (Tên
  "rent" xuất phát từ trường [rent epoch](#rent-epoch) hiện đã không còn được sử
  dụng.)
</Callout>
...
coi là tùy ý vì nó có thể chứa bất kỳ chuỗi byte nào. Mỗi chương trình định
nghĩa cấu trúc của dữ liệu được lưu trữ trong trường này.

- Program accounts: Trường này chứa mã chương trình thực thi hoặc địa chỉ của
  [program data account](#program-data-accounts) lưu trữ mã chương trình thực
  thi.
- Data accounts: Trường này thường lưu trữ dữ liệu trạng thái, dùng để đọc.
...
liệu của tài khoản hoặc trừ lamports, theo chỉ dẫn của chương trình.

(Trong trường hợp program account, chủ sở hữu là
[loader program](https://solana.com/docs/core/programs#loader-programs) của nó.)
...
Trường này chỉ ra liệu một tài khoản có phải là
[tài khoản program](#program-accounts) hay [tài khoản dữ liệu](#data-accounts)

- Nếu `true`: Tài khoản là tài khoản program
...
Trong quá khứ, trường này theo dõi khi nào một tài khoản cần phải trả rent. Tuy
...
- [Tài khoản program](#program-accounts): Tài khoản chứa mã có thể thực thi
- [Tài khoản dữ liệu](#data-accounts): Tài khoản không chứa mã có thể thực thi

Sự phân tách này có nghĩa là mã thực thi của một chương trình và trạng thái của
nó được lưu trữ trong các tài khoản riêng biệt. (Tương tự như các hệ điều hành,
...
[có thể thực thi](#executable) của nó. Đây được gọi là tài khoản program. (Để
đơn giản, bạn có thể coi tài khoản program chính là bản thân chương trình.)

Trong sơ đồ dưới đây, bạn có thể thấy một chương trình tải được sử dụng để triển
khai một tài khoản program. Trường `data` của tài khoản program chứa mã chương
trình có thể thực thi.

![Sơ đồ của một tài khoản program, 4 thành phần của nó và chương trình tải của nó.](/assets/docs/core/accounts/program-account-simple.svg)

#### Tài khoản dữ liệu program
...
khoản dữ liệu program** riêng biệt, chứa mã chương trình. (Xem sơ đồ bên dưới.)

![Một program account với dữ liệu. Dữ liệu trỏ đến một program data account riêng biệt](/assets/docs/core/accounts/program-account-expanded.svg)

<Callout>
  Trong quá trình triển khai hoặc nâng cấp chương trình, các tài khoản đệm được
  sử dụng để tạm thời lưu trữ quá trình tải lên.
</Callout>

Ví dụ dưới đây lấy thông tin tài khoản Token Program. Lưu ý rằng trường
`executable` được đặt thành `true`, cho biết tài khoản này là một chương trình.
...
làm điều này, trước tiên họ phải tạo một tài khoản dữ liệu mới. Quá trình tạo
tài khoản trạng thái chương trình thường được trừu tượng hóa, nhưng việc hiểu
quy trình cơ bản là hữu ích.
...
2. Khởi tạo dữ liệu tài khoản, như được định nghĩa bởi các
   [hướng dẫn](/docs/core/instructions) của nó.

![Sơ đồ của một tài khoản dữ liệu thuộc sở hữu của một program account](/assets/docs/core/accounts/data-account.svg)

Ví dụ dưới đây tạo và lấy thông tin một tài khoản Token Mint thuộc sở hữu của
...
![Một ví thuộc sở hữu của System Program chứa 1.000.000 lamports](/assets/docs/core/accounts/system-account.svg)

Khi SOL được gửi đến một địa chỉ mới lần đầu tiên, một tài khoản được tạo tại
địa chỉ đó thuộc sở hữu của System Program.

Trong ví dụ dưới đây, một keypair mới được tạo và được cấp SOL. Sau khi chạy mã,
bạn có thể thấy địa chỉ của `owner` của tài khoản là
`11111111111111111111111111111111` (tức
