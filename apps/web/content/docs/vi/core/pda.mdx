...
Điều này có nghĩa là với cùng các seed tùy chọn và `programId`, một bump seed có
giá trị khác vẫn có thể tạo ra một PDA hợp lệ.
...
tạo một tài khoản mới với địa chỉ program-derived. Chương trình bao gồm một lệnh
[`initialize`](mention:initialize) để tạo tài khoản mới, tài khoản này sẽ lưu
trữ [địa chỉ người dùng](mention:user-address) và [bump seed](mention:bump) được
sử dụng để tạo ra PDA.
...
`find_program_address` để tạo ra PDA và tìm bump.
...
File test dưới đây chứa một giao dịch gọi lệnh
[`initialize`](mention:initialize) để tạo một tài khoản mới với địa chỉ
program-derived. File này chứa mã để [tạo ra PDA](mention:pda).
...
<Callout type="warn">
  Nếu bạn gọi lệnh `initialize` lần nữa với cùng địa chỉ seed `user`, giao dịch
  sẽ thất bại. Điều này xảy ra vì một tài khoản đã tồn tại tại địa chỉ đã tạo.
</Callout>
