...
Trường `owner` hiển thị chương trình sở hữu tài khoản. Đối với ví, chủ sở hữu là
System Program, với địa chỉ `11111111111111111111111111111111`.
...
Trường `rentEpoch` là trường cũ từ cơ chế rent đã ngừng sử dụng. (Trường này
được bao gồm để tương thích ngược.)
...
trong [kiểu Account](/docs/core/accounts#account-structure) chính nó, nhưng được
bao gồm trong phản hồi.
...
Mọi tài khoản chương trình đều thuộc sở hữu của
[chương trình loader](/docs/core/programs#loader-programs). Trong ví dụ này,
...
[_ts`getMint()`_](mention:one) để giúp giải tuần tự hóa trường `data` của mint
...
Hàm _ts`getMint()`_ giải tuần tự hóa trường `data` của mint account thành kiểu
tài khoản Mint.
...
đơn vị nhỏ nhất của token. Để có tổng cung ở đơn vị tiêu chuẩn, hãy điều chỉnh
giá trị của trường `supply` theo `decimals`.
...
Trường `isInitialized` cho biết mint account đã được khởi tạo hay chưa. Trường
này là một kiểm tra bảo mật được sử dụng trong Token Program.
