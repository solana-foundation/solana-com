...
description:
  Tìm hiểu cách xây dựng, triển khai và kiểm tra chương trình Solana đầu tiên
  của bạn bằng framework Anchor và Solana Playground. Hướng dẫn thân thiện với
  người mới này sẽ hướng dẫn tạo một chương trình đơn giản, triển khai lên
  devnet, chạy các bài kiểm tra và đóng chương trình.
h1: Triển khai chương trình Solana đầu tiên của bạn
---

Trong phần này, bạn sẽ xây dựng, triển khai và kiểm tra một chương trình Solana
(hợp đồng thông minh) đơn giản bằng framework Anchor. Đến cuối bài, bạn sẽ triển
khai được chương trình đầu tiên của mình lên blockchain Solana.
...
trang [Các chương trình trên Solana](/docs/core/programs).
...
Chương trình chứa một chỉ thị (`initialize`) thực hiện:

- Yêu cầu một tham số _rs`data: u64`_ làm đầu vào
- Tạo một tài khoản mới
- Lưu giá trị của tham số _rs`data: u64`_ vào dữ liệu của tài khoản
...
Macro _rs`declare_id!()`_ chỉ định địa chỉ trên chuỗi của chương trình của bạn.
Solana Playground cập nhật địa chỉ này khi bạn xây dựng dự án.
...
các tài khoản mà chương trình này có thể tạo ra và sở hữu.
...
cung cấp để nâng cao bảo mật cho chương trình.
...
_rs`ctx.accounts.new_account.data`_ đang truy cập vào trường `data` của tài
khoản `new_account` và đặt giá trị của nó bằng với tham số `data: u64`.

### !msg

Macro _rs`msg!`_ ghi một thông điệp vào nhật ký chương trình của giao dịch.
...
Ràng buộc `space` chỉ định số byte cần cấp phát cho tài khoản.
...
Hiện tại, đây là tổng quan cấp cao về mã chương trình:
...
2. `data: u64` - Một tham số tùy chỉnh mà bạn phải cung cấp khi gọi chỉ thị.

Phần thân hàm [thiết lập trường `data` của `new_account`](mention:three) thành
đối số `data` được cung cấp và sau đó in một thông báo vào nhật ký chương trình.
...
- Thuộc tính [`#[account]`](mention:one) định nghĩa một struct đại diện cho kiểu
  dữ liệu của các tài khoản mà chương trình này tạo và sở hữu.
...
![Xây dựng và Triển khai](/assets/docs/intro/quickstart/pg-build-deploy.png)
...
![Chạy kiểm tra](/assets/docs/intro/quickstart/pg-test.png)
...
Cuối cùng, việc đóng chương trình cho phép khôi phục hoàn toàn lượng SOL đã được
phân bổ cho chương trình trên chuỗi.
...
Chỉ có quyền nâng cấp của chương trình mới có thể đóng nó. Quá trình triển khai
thiết lập quyền nâng cấp khi bạn triển khai chương trình. Tài khoản này có quyền
độc quyền để cập nhật hoặc đóng chương trình. Nếu ai đó thu hồi quyền nâng cấp,
chương trình sẽ trở nên bất biến mà không có khả năng đóng hoặc cập nhật.

Khi triển khai chương trình trên Solana Playground, ví Playground của bạn tự
động trở thành quyền nâng cấp cho tất cả các chương trình của bạn.
