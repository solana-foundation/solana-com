...
Yêu cầu airdrop SOL để nạp tiền cho `sender`.
...
Trong ví dụ này, chúng ta đang kết nối với test validator cục bộ chạy trên
`localhost:8899`.
...
sử dụng phương thức `requestAirdrop` để nhận SOL cho mục đích thử nghiệm.
...
Phương thức `SystemProgram.transfer()` tạo một instruction chuyển SOL từ tài
khoản [`fromPubkey`](mention:from) sang tài khoản [`toPubkey`](mention:to) với
số lượng [`lamports`](mention:lamports) được chỉ định.
...
Thứ tự các instruction rất quan trọng ở đây. `createAccountInstruction` phải đến
...
Tạo token yêu cầu sử dụng cả hai thư viện `@solana/web3.js` và
`@solana/spl-token`. Đoạn code trong ví dụ dưới đây sẽ:
...
Tính toán số lượng lamport tối thiểu cần thiết cho một mint account. Hàm
`getMinimumBalanceForRentExemptMint` tính toán số lượng lamport phải được phân
bổ cho dữ liệu trên một mint account.
...
Lệnh đầu tiên gọi lệnh `createAccount` của System Program để:
...
Lệnh thứ hai gọi lệnh `createInitializeMint2Instruction` của
[Token Extensions Program](mention:programId) để khởi tạo mint account với dữ
liệu sau:
