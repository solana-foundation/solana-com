...
description: Tìm hiểu cách chuyển đổi SOL gốc sang wrapped SOL.
---

## Đồng bộ SOL gốc

Wrapped SOL (WSOL) là một tài khoản token theo dõi số lượng lamport trong tài
khoản dưới dạng số dư token. WSOL cho phép tích hợp với các giao thức DeFi để
chuyển SOL dưới dạng token SPL.
...
đồng bộ hóa số dư tài khoản token wrapped SOL (WSOL) với số lượng SOL thực tế
(lamport) được lưu trữ trong đó. Khi bạn chuyển SOL gốc vào tài khoản token
WSOL, số dư token không tự động cập nhật. Bạn phải gọi SyncNative để hiển thị
chính xác số dư WSOL.
