...
1. Tạo
   ...
   **Bằng chứng đẳng thức
   ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**:
   Xác minh rằng ciphertext số dư khả dụng còn lại sau khi rút tiền khớp với
   [cam kết Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme) tương
   ứng, đảm bảo số dư khả dụng mới của tài khoản được tính toán chính xác là
   `remaining_balance = current_balance - withdraw_amount`.
...
4. Đóng các tài khoản bằng chứng để lấy lại SOL đã sử dụng để tạo chúng.
...
- Phương thức `confidential_transfer_create_context_state_account` tạo một tài
  khoản bằng chứng.
- Phương thức `confidential_transfer_withdraw` gọi lệnh `Withdraw` instruction.
