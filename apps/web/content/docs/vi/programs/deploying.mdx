...
1. `hello_world-keypair.json`: Một tệp keypair có public key sẽ được sử dụng làm
   Program ID của bạn
...
- **`--with-compute-unit-price`**: Đặt phí ưu tiên tính theo micro-lamport
...
- **`--use-rpc`**: Gửi giao dịch đến RPC đã cấu hình. Cờ này yêu cầu kết nối RPC
  [có trọng số cổ phần](/developers/guides/advanced/stake-weighted-qos) từ các
  nhà cung cấp như [Triton](https://triton.one/) hoặc
