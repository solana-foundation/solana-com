...
Chạy `cargo build-sbf` sẽ tạo ra program đã được biên dịch tại
...
1. [Tạo instance `Mollusk`](mention:mollusk) - Khởi tạo Mollusk với program ID
   và đường dẫn đến program đã biên dịch (file `.so`)
2. [Xây dựng instruction](mention:instruction) - Tạo instruction để gọi program
3. [Xử lý và xác thực](mention:process) - Xử lý instruction bằng Mollusk và xác
   thực kết quả
...
Để thực thi test, chạy `cargo test`.
...
cung cấp giao diện đơn giản để kiểm thử các program trên Solana. Tất cả các
field có thể được thao tác thông qua một số phương thức helper, nhưng người dùng
cũng có thể trực tiếp truy cập và chỉnh sửa chúng nếu muốn kiểm soát nhiều hơn.
...
Để khởi tạo Mollusk với một program cụ thể, sử dụng phương thức `Mollusk::new`.
...
Để thêm program vào Mollusk, sử dụng phương thức `Mollusk::add_program`.
...
<Callout type="info">
  Khi cung cấp đường dẫn file, không bao gồm phần mở rộng `.so`. Ví dụ,
...
| [`process_and_validate_instruction`](#single-instruction-with-checks)          | Xử lý một lệnh và thực hiện một loạt kiểm tra trên kết quả, panic nếu bất kỳ kiểm tra nào thất bại.       |
| [`process_instruction_chain`](#multiple-instructions)                          | Xử lý nhiều lệnh và trả về kết quả.                                                                       |
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | Xử lý nhiều lệnh và thực hiện một loạt kiểm tra trên mỗi kết quả, panic nếu bất kỳ kiểm tra nào thất bại. |
...
Sử dụng phương thức `process_instruction` để xử lý một lệnh đơn mà không có kiểm
tra trên kết quả. Bạn có thể xác thực kết quả thủ công sau khi xử lý.
...
Các ví dụ dưới đây chạy Mollusk trong hàm `main` để minh họa. Trong thực tế, bạn
thường sẽ sử dụng Mollusk trong một module test được chú thích với thuộc tính
`#[test]`.
...
Sử dụng phương thức `process_and_validate_instruction` để xử lý một lệnh đơn với
các kiểm tra xác thực. Phương thức này sẽ panic nếu bất kỳ kiểm tra nào thất
bại.
...
Sử dụng phương thức `process_instruction_chain` để xử lý nhiều lệnh tuần tự mà
không có kiểm tra xác thực.
...
với các kiểm tra xác thực sau mỗi lệnh. Mỗi lệnh có tập hợp kiểm tra riêng phải
vượt qua.
...
là một wrapper xung quanh `Mollusk` duy trì trạng thái tài khoản qua nhiều lần
gọi lệnh thông qua `account_store` của nó. Các phương thức để xử lý lệnh giống
hệt với `Mollusk`.

Không giống như `Mollusk`, yêu cầu truyền `accounts` cho mỗi phương thức (ví dụ
`process_instruction`), `MolluskContext` quản lý các tài khoản nội bộ thông qua
...
Ví dụ sau đây xử lý hai chỉ thị chuyển SOL riêng biệt với trạng thái tài khoản
...
tùy chỉnh để sửa đổi các giá trị của nó cho việc kiểm thử.

Sử dụng phương thức `warp_to_slot` để cập nhật sysvar clock nhằm mô phỏng việc
di chuyển về phía trước hoặc lùi lại trong thời gian đến một slot cụ thể.
...
Ví dụ sau đây cho thấy cách sửa đổi sysvar Mollusk trực tiếp bằng cách truy cập
...
theo dõi việc sử dụng compute unit của các chỉ thị của chương trình. Kết quả
...
Kết quả benchmark được ghi vào `out_dir` được chỉ định dưới dạng file markdown
có tên `compute_units.md`.
...
<Callout type="info">
  ...
  họa. `mollusk-svm-programs-token` cũng bao gồm các hàm trợ giúp để tạo các tài
  khoản mint và token.
</Callout>
