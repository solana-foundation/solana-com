...
> Trong khi bạn chạy verified build trong thư mục chương trình của mình, khi
> chạy `verify-from-repo` bạn cần thêm flag `--mount-path`. Đây sẽ là đường dẫn
> đến thư mục chứa `Cargo.toml` có tên thư viện của chương trình của bạn.
...
Nếu bạn muốn khóa việc xác minh vào một phiên bản phát hành cụ thể, bạn có thể
thêm flag `--commit-hash` vào lệnh.
...
> Khuyến nghị sử dụng RPC Url trả phí vì nếu không bạn có thể gặp phải giới hạn
> tốc độ của các RPC miễn phí. Vì vậy thay vì `-um` bạn nên sử dụng
> `--url yourRpcUrl` để có quá trình xác minh đáng tin cậy hơn.

Flag `--remote` gửi yêu cầu build đến API của OtterSec, điều này kích hoạt quá
...
Điều này sẽ build một verifiable build bằng cách sử dụng docker container với
phiên bản solana được chỉ định trong file `Cargo.lock`.
...
Trong phần còn lại của hướng dẫn multisig này, chúng ta sẽ sử dụng program ID
mẫu là `6XBGfP17P3KQAKoJb2s5M5fR4aFTXzPeuC1af2GYkvhD`.
...
Tùy chọn: Xem liệu bạn có thể xác minh cục bộ trước không (lệnh này sử dụng
program ID mẫu `6XBGfP17P3KQAKoJb2s5M5fR4aFTXzPeuC1af2GYkvhD`):
...
Khi bạn có program authority ở cục bộ, bạn sẽ được nhắc tải dữ liệu build lên
onchain khi sử dụng lệnh `solana-verify verify-from-repo`.
...
`ellipsislabs/hello_world_verifiable_build:latest`, và xác minh rằng mã băm của
đường dẫn thực thi trong container giống với mã băm của chương trình trên chuỗi
được cung cấp cho lệnh. Vì bản build đã được tải lên image, không cần phải xây
dựng lại hoàn toàn tệp thực thi, điều này có thể mất nhiều thời gian.
...
cũng có thể xác định một commit cụ thể trong trường hợp bạn muốn tiếp tục làm
...
Lệnh `--remote` gửi yêu cầu xây dựng đến API OtterSec, kích hoạt việc xây dựng
từ xa cho chương trình của bạn. Khi quá trình xây dựng hoàn tất, hệ thống sẽ xác
...
Nếu bạn muốn có thiết lập hoàn toàn không cần tin tưởng, bạn có thể tự chạy
[API xác minh](https://github.com/otter-sec/solana-verified-programs-api) hoặc
tự chạy xác minh chương trình cục bộ bằng lệnh `verify-from-repo` sử dụng dữ
liệu xác minh trên chuỗi được lưu trong
[PDA](https://explorer.solana.com/address/63XDCHrwZu3mXsw2RUFb4tbNpChuUHx4eA5aJMnHkpQQ/anchor-account)
...
Ngoài các bản build đã xác minh, bạn cũng có thể thêm tệp `security.txt` vào
chương trình của mình. Trong tương lai, sau khi được triển khai, `security.txt`
sẽ chứa public key của người xác minh để dễ dàng truy cập dữ liệu xác minh được
lưu trong PDA xác minh. PDA chứa tất cả thông tin cần thiết để xây dựng và xác
minh chương trình được tạo ra từ địa chỉ chương trình và pubkey của người xác
minh. Theo mặc định, đây là cùng một pubkey đã xây dựng và triển khai chương
trình. Nhưng nó cũng có thể là pubkey khác có thể được chỉ định trong
`security.txt`.

Tính năng `security.txt` cho phép các nhà phát triển nhúng thông tin liên hệ và
bảo mật trực tiếp vào smart contract Solana của họ. Lấy cảm hứng từ
[securitytxt.org](https://securitytxt.org), cách tiếp cận này cung cấp phương
thức chuẩn hóa để các nhà nghiên cứu bảo mật liên hệ với người duy trì dự án,
ngay cả khi họ chỉ biết địa chỉ của hợp đồng.

## Tại sao nên sử dụng security.txt?

Đối với nhiều dự án, đặc biệt là các dự án nhỏ hơn hoặc riêng tư, việc xác định
các nhà phát triển chỉ từ địa chỉ hợp đồng có thể khó khăn và tốn thời gian.
Nhúng tệp `security.txt` vào trong chương trình đảm bảo rằng các nhà nghiên cứu
bảo mật có thể dễ dàng liên hệ với đúng người, có khả năng ngăn chặn các lỗ hổng
và đảm bảo báo cáo lỗi kịp thời.
...
- Xác minh: Trước khi triển khai, hãy xác minh định dạng và nội dung bằng công
  cụ `query-security-txt`, công cụ này có thể xác thực cả các chương trình
  onchain và các tệp nhị phân cục bộ:
