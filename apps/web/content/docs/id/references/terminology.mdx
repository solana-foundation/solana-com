...
Sebuah ukuran desentralisasi, Nakamoto Coefficient adalah jumlah terkecil dari
...
untuk membaca dan memodifikasi akun yang berada di bawah kontrolnya. Program ini
sering disebut sebagai "[_smart contracts_](/docs/core/programs)" pada
blockchain lainnya.
...
[validator](#validator), jumlah poin yang diberikan kepada [stake](#stake)
selama penebusan adalah hasil dari [vote credits](#vote-credit) yang diperoleh
...
blockchain sampai akun ditutup. Tidak mungkin membuat akun yang berada di bawah
ambang batas pembebasan rent.

## root

[Blok](#block) atau [slot](#slot) yang telah mencapai [lockout](#lockout)
...
Dengan persyaratan bahwa R adalah titik Edwards terpaket yang bukan dari urutan
kecil dan S adalah skalar dalam rentang `0 <= S < L`. Persyaratan ini memastikan
tidak ada kemungkinan pemalsuan tanda tangan. Setiap transaksi harus memiliki
...
Sebuah [akun](#account) sistem. [Sysvar](https://docs.anza.xyz/runtime/sysvars)
menyediakan informasi status cluster seperti tinggi tick saat ini, nilai
[poin](#point) rewards, dll. Program dapat mengakses Sysvar melalui akun Sysvar
...
[ledger](#ledger). Transaksi diselesaikan ketika bloknya menjadi [root](#root).
