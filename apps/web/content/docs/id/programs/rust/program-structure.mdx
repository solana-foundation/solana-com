...
Untuk kemudahan, gunakan makro
[`entrypoint!`](https://github.com/solana-labs/solana/blob/v2.0/sdk/program/src/entrypoint.rs#L124-L140)
dari crate `solana_program` untuk mendefinisikan entrypoint dalam program Anda.
...
- `program_id`: Kunci publik dari program yang dijalankan (program saat ini)
...
program. Struct ini akan digunakan dalam instruksi inisialisasi dan penambahan.
...
2. Mencocokkan varian dan mengurai parameter tambahan dari byte-byte yang
   tersisa
...
disarankan untuk menggunakan nama yang deskriptif.
...
2. Menghitung saldo "rent" minimum yang diperlukan. Di Solana, akun harus
...
Pemeriksaan validasi akun yang kurang merupakan sumber kerentanan program yang
umum.
...
- Bangun instruksi dengan ID program, instruction data, dan akun yang diperlukan
...
Untuk menguji instruksi penambahan:

- Bangun instruksi dengan ID program, instruction data, dan akun yang diperlukan
- Kirim transaksi dengan instruksi penambahan
- Periksa apakah akun telah ditambahkan ke nilai yang benar

Perhatikan bahwa instruction data untuk instruksi penambahan adalah `[1]`, yang
sesuai dengan varian 1 (`IncrementCounter`). Karena tidak ada parameter tambahan
untuk instruksi penambahan, data tersebut hanya berupa varian instruksi.
