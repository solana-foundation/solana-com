...
mengidentifikasi siapa yang memiliki otoritas atas token account tersebut. Ini
berbeda dari pemilik program yang ditentukan dalam tipe
[Account](/docs/core/accounts#account-type) dasar, yaitu Token Program untuk
semua token account.
...
CLI _sh`spl-token`_ membantu Anda bereksperimen dengan SPL token. Contoh di
...
transaksi. Untuk pengguna Solana Playground pertama kali, buat wallet Playground
...
Temukan lebih banyak contoh dalam
...
wallet Playground Anda sebagai mint authority.
...
harus menandatangani transaksi. Instruksi ini mencetak token ke Token Account
dan meningkatkan total supply pada Mint Account. Berikut adalah contoh
Javascript di
...
Instruksi _rs`Transfer`_ pada Token Program menangani transfer token. Owner dari
Token Account pengirim harus menandatangani transaksi. Instruksi ini memindahkan
token antar Token Account. Berikut adalah contoh Javascript di
