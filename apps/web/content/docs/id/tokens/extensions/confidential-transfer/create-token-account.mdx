...
- **approved**: Status persetujuan akun untuk transfer rahasia. Jika konfigurasi
  ...
  token account secara otomatis disetujui untuk transfer rahasia.
...
mengonversi saldo tertunda menjadi saldo tersedia. Transfer masuk atau deposit
...
2. Counter kredit tertunda yang diharapkan dan aktual melacak perubahan nilai
   counter antara saat instruksi _rs`ApplyPendingBalance`_ dibuat dan diproses:
   - `expected_pending_balance_credit_counter`: Nilai
...
Penghitung expected/actual yang cocok menunjukkan
`decryptable_available_balance` sesuai dengan `available_balance`.

Saat mengambil state token account untuk membaca
`decryptable_available_balance`, nilai penghitung expected/actual yang berbeda
mengharuskan klien untuk mencari instruksi deposit/transfer terbaru yang cocok
dengan selisih penghitung untuk menghitung saldo yang benar.

**Proses Rekonsiliasi Saldo**

Ketika penghitung saldo tertunda expected dan actual berbeda, ikuti
...
2. Ambil transaksi terbaru termasuk instruksi deposit dan transfer hingga
   selisih penghitung (actual - expected):
...
3. **Konfigurasi confidential transfer**: Panggil instruksi
