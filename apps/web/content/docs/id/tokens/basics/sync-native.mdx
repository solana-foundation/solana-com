---
title: Sync Native
description: Pelajari cara mengkonversi SOL native menjadi wrapped SOL.
---

## Sinkronisasi SOL Native

Wrapped SOL (WSOL) adalah token account yang melacak jumlah lamport dalam akun
sebagai saldo token. WSOL memungkinkan integrasi dengan protokol DeFi untuk
mentransfer SOL sebagai token SPL.
...
menyinkronkan saldo token wrapped SOL (WSOL) dengan SOL (lamport) aktual yang
disimpan di dalamnya. Ketika Anda mentransfer SOL native ke token account WSOL,
saldo token tidak otomatis diperbarui. Anda harus memanggil SyncNative untuk
menampilkan saldo WSOL yang benar.
