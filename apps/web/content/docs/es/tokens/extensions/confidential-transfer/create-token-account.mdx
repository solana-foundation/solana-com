...
- **maximum_pending_balance_credit_counter**: El límite de conteo de créditos
  ...
  convertir el saldo pendiente al saldo disponible.
...
_rs`ApplyPendingBalance`_ para convertir el saldo pendiente al saldo disponible.
...
`pending_balance_hi` porque el descifrado ElGamal requiere más computación para
...
1. El cliente consulta los saldos pendientes y disponibles actuales, cifra la
   ...
   clave AES del propietario de la cuenta de token.

2. Los contadores de crédito pendiente esperados y reales rastrean cambios en el
   valor del contador entre cuando se crea y se procesa la instrucción
   _rs`ApplyPendingBalance`_:
   - `expected_pending_balance_credit_counter`: El valor
     `pending_balance_credit_counter` cuando el cliente crea la instrucción
     _rs`ApplyPendingBalance`_
   - `actual_pending_balance_credit_counter`: El valor
     `pending_balance_credit_counter` en la cuenta de token en el momento en que
...
Al obtener el estado de una cuenta de token para leer el
`decryptable_available_balance`, valores diferentes de los contadores
...
1. Comienza con el `decryptable_available_balance` de la cuenta de token
2. Obtén las transacciones más recientes que incluyan instrucciones de depósito
   ...
   - Añade cantidades públicas de las instrucciones de depósito
   - Descifra y añade cantidades de texto cifrado de destino de las
...
1. **Crear la cuenta de token**: Invoca la instrucción
   _rs`AssociatedTokenAccountInstruction:Create`_ del Associated Token Program
   para crear la cuenta de token.
