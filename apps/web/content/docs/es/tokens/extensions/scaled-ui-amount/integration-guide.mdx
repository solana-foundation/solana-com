...
- Los usuarios deben ingresar cantidades que se interpreten como el “UIAmount”
  escalado. La aplicación que tenga que procesar esto debe convertir a la
  cantidad de tokens sin procesar para la transacción.
  - Si hay problemas de redondeo, redondea hacia abajo y prefiere dejar una
    cantidad mínima de residuo en lugar de arriesgar que la transacción falle
...
- El multiplicador actual se puede leer desde el token mint en cualquier momento
  llamando a `getAccountInfo`. Además, si se establece un multiplicador futuro,
  esta información también está disponible desde el token mint. Recomendamos no
  mostrar este multiplicador ya que puede confundir la experiencia de usuario.
...
{/_ CODE_PLACEHOLDER_62735ee96c277d398badd3d8c929366d _/
