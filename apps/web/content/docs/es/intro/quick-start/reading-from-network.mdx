...
## Obtener una cuenta de cartera

<WithMentions>

Una cartera es una cuenta que pertenece al
[System Program](/docs/core/programs#the-system-program). Las carteras se
...
pública, y [recupera los datos de la cuenta](mention:info) para la cartera
recién financiada.
...
Cuando obtienes una cuenta de cartera, la respuesta incluye los campos mostrados
en el ejemplo de salida a la derecha.
...
cuentas de cartera, este campo está vacío (0 bytes).
...
programa ejecutable. Para cuentas de cartera este campo es `false`.
...
no es un campo en el propio
[tipo Account](/docs/core/accounts#account-structure), pero se incluye en la
respuesta.
...
cuentas de cartera y cuentas de programa. La cuenta de programa almacena el
código de bytes compilado para el
...
Cada cuenta de programa es propiedad de su
...
El ejemplo a continuación obtiene la cuenta Mint de USD Coin para demostrar cómo
el estado de un programa se almacena en una cuenta separada.
...
El campo `data` de la mint account almacena estado, no código ejecutable, por lo
...
autoridad de emisión, el suministro total y el número de decimales.

Para leer desde una cuenta Mint, debes deserializar el campo `data` en el tipo
de datos `Mint`, lo cual se muestra en el
...
datos apropiado es definido por el programa que posee la cuenta. La mayoría de
...
El campo `isInitialized` indica si la mint account ha sido inicializada. Este
...
accounts. Una token account congelada no puede transferir ni quemar el token que
contiene.
