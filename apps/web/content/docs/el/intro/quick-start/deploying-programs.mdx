...
το Solana Playground και να προετοιμαστείτε για τη χρήση των Program Derived
Address (PDA) και Cross Program Invocation (CPI).
...
Η ενότητα που περιέχει τις οδηγίες του προγράμματος
...
Η μακροεντολή _rs`msg!`_ καταγράφει ένα μήνυμα στα αρχεία καταγραφής
...
- `data: u64`: Μια προσαρμοσμένη παράμετρο που πρέπει να παρέχετε όταν καλείτε
  την οδηγία.
...
1. Μεταγλώττιση του κώδικα του προγράμματος
...
<Callout>
  Το [αναγνωριστικό του προγράμματός
  σας](/docs/intro/quick-start/deploying-programs), που ορίζεται στο
  _rs`declare_id!()`_, θα ενημερωθεί τώρα.
</Callout>

2. Προσθήκη SOL στο πορτοφόλι σας _(Προαιρετικό)_

Η ανάπτυξη ενός προγράμματος απαιτεί SOL στον λογαριασμό του προγράμματος στην
αλυσίδα. Εάν είναι απαραίτητο,
...
<Callout type="info">
  Όταν αναπτύσσεται, ο δυαδικός κώδικας του προγράμματος αποθηκεύεται σε έναν
  εκτελέσιμο λογαριασμό προγράμματος.
</Callout>
...
![Build και Deploy](/assets/docs/intro/quickstart/pg-build-deploy.png)
...
2. Εκτέλεση του αρχείου δοκιμής
...
4. Εκτελέστε το `solana confirm -v [TxHash]` για να δείτε τα αρχεία καταγραφής
   της συναλλαγής
...
Ανοίξτε είτε το [SolanaFM](https://solana.fm/) είτε τον
[Solana Explorer](https://explorer.solana.com/?cluster=devnet) και αναζητήστε το
hash της συναλλαγής.

<Callout type="warn">
  **Solana Explorer:** Ορίστε το δίκτυο σε `devnet`, ώστε να ταιριάζει με αυτό
...
Κλείστε το πρόγραμμα για να ανακτήσετε το SOL που διατέθηκε στο πρόγραμμα στην
αλυσίδα. Για να το κάνετε αυτό, εκτελέστε το `solana program close [ProgramID]`,
χρησιμοποιώντας τη διεύθυνση του προγράμματος που είναι αποθηκευμένη στο
...
τις
[διευθύνσεις που προέρχονται από προγράμματα](/docs/intro/quick-start/program-derived-address)
και την
[διαπρογραμματική κλήση](/docs/intro/quick-start/cross-program-invocation).
