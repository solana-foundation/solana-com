...
Οι εντολές `update` και `delete` χρειάζονται τροποποίηση για να διαχειριστούν
...
Πρώτα, το πρόγραμμα χρειάζεται έναν απλό μηχανισμό "πλήρωσε-για-ενημέρωση"
αλλάζοντας τη δομή _rs`Update`_ και τη συνάρτηση `update`.

Ξεκινήστε ενημερώνοντας το αρχείο `lib.rs` για να φέρετε στο πεδίο εφαρμογής
...
Αυτή η ενότητα προσθέτει έναν νέο λογαριασμό που ονομάζεται `vault_account` στη
δομή _rs`Update`_. Αυτός ο λογαριασμός λειτουργεί ως "θησαυροφυλάκιο" που
ελέγχεται από το πρόγραμμα και λαμβάνει SOL από τους χρήστες όταν ενημερώνουν τα
μηνύματά τους.
...
Βασικές πτυχές του `vault_account`:

- Η διεύθυνση του λογαριασμού προέρχεται από ένα PDA που προκύπτει
  χρησιμοποιώντας seeds _rs`[b"vault", user.key().as_ref()]`_
- Ως PDA, δεν έχει ιδιωτικό κλειδί, οπότε μόνο το πρόγραμμα μπορεί να
  "υπογράψει" για τη διεύθυνση όταν εκτελεί CPIs
...
κλήση μεταξύ προγραμμάτων (Cross Program Invocation - CPI).
...
Στην εντολή `update`, η υλοποίηση περιλαμβάνει μια κλήση μεταξύ προγραμμάτων
(Cross Program Invocation - CPI) για να καλέσει την εντολή `transfer` του System
Program. Αυτό δείχνει πώς να εκτελέσετε ένα CPI από μέσα από το πρόγραμμα,
ενεργοποιώντας τη δυνατότητα σύνθεσης των προγραμμάτων Solana.
...
- Τους λογαριασμούς που απαιτούνται στο CPI (ορίζονται στη δομή _rs`Transfer`_)
...
- Το `amount` προς μεταφορά (1.000.000 lamports, ή 0,001 SOL)
...
την πλευρά του client, όπου καθορίζετε το πρόγραμμα, τους λογαριασμούς και τα
instruction data για μια συγκεκριμένη εντολή που θα κληθεί. Όταν η εντολή
`update` του προγράμματος λάβει μια κλήση, καλεί εσωτερικά την εντολή μεταφοράς
του System Program.
...
Τώρα προσθέστε έναν μηχανισμό "επιστροφής κατά τη διαγραφή" αλλάζοντας τη δομή
_rs`Delete`_ και τη συνάρτηση `delete`.
...
όταν κλείνει τον λογαριασμό μηνυμάτων του.
...
διαγραφής για να μεταφέρει οποιοδήποτε συσσωρευμένο SOL πίσω στον χρήστη.
...
Η μέθοδος _rs`with_signer()`_ περνά τα signer seeds με το CPI.
...
- Το `cpi_context` (πρόγραμμα, λογαριασμοί και PDA signer)
...
Στη συνέχεια, ενημερώστε το αρχείο `anchor.test.ts` ώστε να συμπεριλάβει τον νέο
λογαριασμό θησαυροφυλακίου στις εντολές. Αυτό απαιτεί την παραγωγή του vault PDA
και τη συμπερίληψή του στις κλήσεις εντολών update και delete.
...
Στη συνέχεια, ενημερώστε την εντολή ενημέρωσης ώστε να περιλαμβάνει το
...
Στη συνέχεια, ενημερώστε την εντολή διαγραφής ώστε να περιλαμβάνει το
[`vaultAccount`](mention:one)
...
προσθέσετε στη λίστα των έργων σας στο Solana Playground. Μόλις ένα έργο
εισαχθεί, όλες οι αλλαγές αποθηκεύονται και διατηρούνται αυτόματα.
