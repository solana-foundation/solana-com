...
στην πράξη ίσως θελήσετε να χωρίσετε μεγαλύτερα προγράμματα σε πολλαπλά αρχεία.
...
Προσθέστε την ενότητα `[lib]` για να ρυθμίσετε τον τρόπο με τον οποίο το Cargo
δημιουργεί το πρόγραμμα.
...
συνάρτησης {/_ INLINE_CODE_PLACEHOLDER_cd5d15d6c903f7a30ee6e9f48d3353aa _/.
...
του Borsh χειρίζεται αυτόματα αυτή τη μετατροπή.
...
τις εντολές στις κατάλληλες συναρτήσεις χειρισμού.
...
- Το αποσειριοποιεί σε ένα struct `CounterAccount`
- Αυξάνει το πεδίο `count` κατά 1
- Σειριοποιεί το struct `CounterAccount` πίσω στο πεδίο `data` του λογαριασμού
...
Πρώτα, ας προσθέσουμε τις εξαρτήσεις που χρειάζονται για τον έλεγχο. Θα
χρησιμοποιήσουμε το `litesvm` για τον έλεγχο και το `solana-sdk`.
...
μόνο κατά την εκτέλεση ελέγχων.
...
Τώρα πρέπει να κάνουμε build και να φορτώσουμε το πρόγραμμά μας στο περιβάλλον
ελέγχου. Εκτελέστε την εντολή `cargo build-sbf` για να κάνετε build το
...
Προσθέστε τον κώδικα του client στο `examples/client.rs` και αντικαταστήστε το
`program_id` με το αποτέλεσμα της προηγούμενης εντολής:
...
Με τον τοπικό validator σε λειτουργία, μπορείτε να δείτε τις συναλλαγές στο
[Solana Explorer](https://explorer.solana.com/?cluster=custom) χρησιμοποιώντας
τις υπογραφές συναλλαγών που εμφανίζονται. Σημειώστε ότι το cluster στο Solana
Explorer πρέπει να οριστεί σε "Custom RPC URL", το οποίο έχει προεπιλογή στο
