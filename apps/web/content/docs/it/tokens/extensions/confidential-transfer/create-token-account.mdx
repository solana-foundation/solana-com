...
al saldo in sospeso. I proprietari dei token account devono utilizzare
l'istruzione _rs`ApplyPendingBalance`_ per convertire il saldo in sospeso nel
saldo disponibile. I trasferimenti in entrata o i depositi non influenzano il
...
Il saldo confidenziale in sospeso è suddiviso in `pending_balance_lo` e
`pending_balance_hi` perché la decrittazione ElGamal richiede più calcoli per
numeri più grandi. Puoi trovare l'implementazione dell'aritmetica del ciphertext
...
1. Il client cerca i saldi in sospeso e disponibili correnti, cripta la somma, e
   fornisce un `decryptable_available_balance` crittografato utilizzando la
   chiave AES del proprietario del token account.
...
I contatori previsti/effettivi corrispondenti indicano che
...
2. Recupera le transazioni più recenti includendo le istruzioni di deposito e
   trasferimento fino alla differenza dei contatori (effettivo - previsto):
   - Aggiungi gli importi pubblici dalle istruzioni di deposito
   - Decifra e aggiungi gli importi ciphertext di destinazione dalle istruzioni
     di trasferimento
