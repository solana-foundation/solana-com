...
| _rs`UpdateMint`_                    | Aggiorna le impostazioni dei trasferimenti confidenziali per un mint.                                                                                                               |
...
| _rs`ConfigureAccountWithRegistry`_  | Modo alternativo per configurare i token account per i trasferimenti confidenziali utilizzando un account _rs`ElGamalRegistry`_ invece di una proof _rs`VerifyPubkeyValidity`_.     |
