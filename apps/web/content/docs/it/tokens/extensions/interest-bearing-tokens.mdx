...
L'interesse matura continuamente in base al timestamp della rete, ma il saldo
effettivo dei token memorizzato in ciascun token account rimane invariato. Il
valore totale (saldo token + interessi maturati) è solo un calcolo. Nessun nuovo
token viene creato con questa estensione.
