---
title: Sync Native
description: Impara come convertire SOL nativo in wrapped SOL.
---

## Sincronizza SOL nativo

Il Wrapped SOL (WSOL) è un token account che tiene traccia della quantità di
lamport nell'account come saldo del token. WSOL consente l'integrazione con i
protocolli DeFi per trasferire SOL come token SPL.
...
sincronizza il saldo di un token account wrapped SOL (WSOL) con il SOL (lamport)
effettivamente memorizzato in esso. Quando trasferisci SOL nativo a un token
account WSOL, il saldo del token non si aggiorna automaticamente. Devi chiamare
SyncNative per visualizzare il saldo WSOL corretto.
