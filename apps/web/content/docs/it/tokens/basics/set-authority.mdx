...
description:
  Scopri come impostare l'autorità per i mint account o i token account.
---
...
- **Autorità di emissione (Mint Authority)**: Controlla la creazione di nuovi
  token. Può emettere token verso qualsiasi token account. Spesso viene revocata
  dopo la creazione della fornitura iniziale per creare un token con fornitura
  fissa.
...
- **Autorità di chiusura (Close Authority)**: Può chiudere il token account
  quando il suo saldo è zero. Per impostazione predefinita, questa è il
  proprietario dell'account, ma può essere delegata a un altro account.
