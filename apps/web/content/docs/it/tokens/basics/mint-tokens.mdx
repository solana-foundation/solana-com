...
Coniare token si riferisce al processo di creazione di nuove unità di un token
invocando l'istruzione
...
su un token program. Solo l'indirizzo specificato come autorità di conio sul
mint account può coniare nuovi token. L'istruzione richiede anche l'esistenza di
un token account come destinazione dei token coniati.
