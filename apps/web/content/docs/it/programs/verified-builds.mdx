...
verifica che l'hash on-chain del tuo programma corrisponda all'hash
...
dati della build on-chain quando usi il comando
...
codificata in base64 da utilizzare in un transaction inspector, puoi usare
`--encoding base64`.
...
percorso eseguibile nel container corrisponda all'hash del programma onchain
...
commit sul branch main. Puoi anche definire un commit specifico nel caso tu
voglia continuare a lavorare sul repository utilizzando il parametro
...
sistema verifica che l'hash onchain del tuo programma corrisponda all'hash
dell'artefatto di build generato dal tuo repository.
...
eseguire localmente la verifica del programma utilizzando il comando
`verify-from-repo` usando i dati di verifica onchain che sono salvati in un
[PDA](https://explorer.solana.com/address/63XDCHrwZu3mXsw2RUFb4tbNpChuUHx4eA5aJMnHkpQQ/anchor-account)
che è derivato dall'autorità di deploy del programma e dal
...
programma e dalla pubkey del verificatore. Per impostazione predefinita questa è
la stessa pubkey che ha compilato e distribuito il programma. Ma può anche
...
manutentori del progetto, anche se conoscono solo l'indirizzo del contratto.
...
contattare le persone giuste, prevenendo potenzialmente exploit e assicurando
segnalazioni tempestive di bug.
...
- Verifica: prima del deployment, verifica il formato e il contenuto utilizzando
  lo strumento `query-security-txt`, che può validare sia programmi onchain che
  binari locali:
