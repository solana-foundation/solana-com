...
aggiungere nei passaggi seguenti.
...
Prima di iniziare, esegui _shell`build`_ nel terminale del Playground per
verificare che il programma di partenza venga compilato correttamente.
...
In `lib.rs`, aggiorna la struct _rs`MessageAccount`_ con quanto segue:
...
L'attributo _rs`#[account]`_ in un programma Anchor annota le struct che
...
Il campo _rs`message`_ contiene una _rs`String`_ con il messaggio dell'utente.

### !bump

Il campo _rs`bump`_ memorizza un _rs`u8`_
["bump" seed](/docs/core/pda#canonical-bump) utilizzato per derivare un Program
Derived Address (PDA). Memorizzare questo valore consente di risparmiare risorse
di calcolo eliminando la necessità di ricalcolarlo nelle istruzioni successive.
...
In questo esempio, la struct _rs`MessageAccount`_ memorizza un messaggio creato
dagli utenti che contiene tre campi:
...
- `bump` - _rs`u8`_ che memorizza il
  ...
  Address (PDA). Memorizzare questo valore consente di risparmiare risorse di
  calcolo eliminando la necessità di ricalcolarlo nelle istruzioni successive.

Quando si crea un account, il programma serializza i dati _rs`MessageAccount`_ e
li memorizza nel campo dati del nuovo account.

Successivamente, quando si legge dall'account, il programma deserializza questi
dati riportandoli al tipo di dato _rs`MessageAccount`_. La sezione di testing
...
L'attributo _rs`#[derive(Accounts)]`_ in un programma Anchor annota le strutture
...
1. Il tipo di account (come _rs`Signer<'info>`_ o _rs`Account<'info, T>`_) che
   specifica quale tipo di account il programma si aspetta
...
1. _rs`user: Signer<'info>`_
   ...
   - Necessita dello stato modificabile (_rs`#[account(mut)]`_) poiché paga per
     il nuovo account
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - `space` alloca i byte necessari per il campo data dell'account
...
I vincoli `seeds` e `bump` insieme definiscono l'indirizzo di un account come
...
bump seed corretto. Anchor utilizza `seeds` e `bump` per derivare il PDA.

---

Il calcolo `space` _rs`(8 + 32 + 4 + message.len() + 1)`_ alloca spazio per il
tipo di dato `MessageAccount`:

- Discriminatore account Anchor (identificatore): 8 byte
...
Un tipo _rs`String`_ necessita di 4 byte per memorizzare la lunghezza della
stringa, e la lunghezza rimanente contiene i dati effettivi.
...
account messaggio. Accetta due parametri:
...
2. Inizializzazione dei dati dell'account:
   ...
   - Imposta il campo `message` al `message` dall'argomento della funzione.
...
Aggiorna la struct `Update` con quanto segue:
...
1. _rs`user: Signer<'info>`_
   ...
   - Contrassegnato come mutabile (_rs`#[account(mut)]`_) poiché potrebbe pagare
     per più spazio per il `message_account` quando necessario
...
memorizzato sul _rs`message_account`_, invece di farlo ricalcolare ad Anchor.
...
La funzione `update` implementa la logica per modificare un account di messaggio
...
2. Aggiorna i dati dell'account:
   - Accede al `message_account` dal contesto.
   - Imposta il campo `message` al nuovo `message` dall'argomento della
     funzione.
...
Successivamente, aggiungi l'istruzione _rs`delete`_ per chiudere il
_rs`MessageAccount`_.
...
Il vincolo _rs`seeds`_ specifica i seed utilizzati per derivare il PDA.

### !bump

Il vincolo _rs`bump`_ specifica il bump seed per il PDA.

In questo caso, il programma utilizza il bump seed esistente memorizzato sul
...
In questo caso, l'account _rs`user`_ riceve i lamport dall'account
_rs`message_account`_ chiuso.
...
1. _rs`_ctx: Context<Delete>`_ - Fornisce accesso agli account specificati nella
   struct _rs`Delete`_. La sintassi _rs`_ctx`_ indica che la funzione non
   utilizza il Context nel suo corpo.

Il corpo della funzione stampa semplicemente un messaggio nei log del programma
utilizzando la macro _rs`msg!()`_. La funzione non necessita di logica
aggiuntiva perché il vincolo _rs`close`_ nella struct _rs`Delete`_ gestisce la
...
Hai completato il programma CRUD di base. Effettua il deploy del programma
...
Solana Playground rimuove parte del setup boilerplate dove
[`pg.program`](mention:one) consente l'accesso ai metodi per interagire con il
programma, mentre [`pg.wallet`](mention:two) fornisce l'accesso al tuo wallet di
playground.
...
Esegui il file di test eseguendo _shell`test`_ nel terminale di Playground per
verificare che funzioni come previsto. I passaggi successivi aggiungono i test
effettivi.
...
Prima, il file di test invia una transazione che invoca l'istruzione `create`,
...
Il metodo _ts`update()`_ invoca l'istruzione `update`.
...
Prima, il file di test invia una transazione che invoca l'istruzione `update`,
...
Il metodo _ts`delete()`_ richiama l'istruzione `delete`.
...
Prima di tutto, questo codice invia una transazione che richiama l'istruzione
...
Dopo aver preparato i tuoi test, esegui il file di test con _shell`test`_ nel
terminale del Playground. Questo comando esegue i test sul programma distribuito
...
<Callout type="info">
  ...
  account può esistere per un dato PDA.
</Callout>
