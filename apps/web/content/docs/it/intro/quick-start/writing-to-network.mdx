...
programmi di Solana. Per maggiori dettagli, consulta le pagine
...
Genera una nuova coppia di chiavi da utilizzare come `sender`.

### !receiver

Genera una nuova coppia di chiavi da utilizzare come `receiver`.
...
Su reti diverse dalla mainnet, puoi utilizzare il metodo `requestAirdrop` per
...
per l'importo specificato di [`lamports`](mention:lamports).
...
Il keypair del [mittente](mention:sender) è necessario nell'array dei firmatari
...
La prima istruzione richiama l'istruzione `createAccount` del System Program
per:
...
2. [Trasferire lamport](mention:lamports) dal wallet per finanziare il nuovo
   account
3. [Assegnare la proprietà](mention:programId) dell'account al programma Token
...
del [Token Extensions Program](mention:programId) per inizializzare il mint
...
creazione dell'account e l'inizializzazione avvengano in modo atomico. O
