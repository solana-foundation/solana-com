...
- La **tariffa base** per una transazione è di 5000 lamport per firma sulla
  transazione.
- La **tariffa di prioritizzazione** (opzionale) è una commissione extra che
  paghi al validator per aumentare la probabilità che il leader attuale elabori
  la tua transazione.
- La tariffa di prioritizzazione equivale a: **(limite unità di calcolo × prezzo
  unità di calcolo)**.
...
- Il pagatore della tariffa di transazione deve essere un account posseduto dal
  System Program.
