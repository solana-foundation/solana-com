...
blockchain. Le unità di calcolo sono utilizzate per misurare le risorse
...
Anche chiamate `transaction cost`, sono una stima completa di tutte le risorse
...
Una rete multicast utilizzata per convalidare in modo efficiente gli
[entry](#entry) e ottenere consenso.

## drone

Un servizio offchain che funge da custode per la chiave privata di un utente. In
genere serve per convalidare e firmare le transazioni.
...
- L'entry è stato generato dopo un certo periodo di tempo
...
Il tempo, cioè il numero di [slot](#slot), per cui uno
[schedule dei leader](#leader-schedule) è valido.
...
Il file di configurazione che prepara il [registro](#ledger) per il
...
Un aumento della quantità di token nel tempo utilizzato per finanziare le
...
[validator](#validator) mappate su [slot](#slot). Il cluster utilizza il
programma leader per determinare quale validator è il [leader](#leader) in
qualsiasi momento.

## registro

Un elenco di [voci](#entry) contenenti [transazioni](#transaction) firmate da
...
così come una promessa di non votare per un [blocco](#block) in conflitto \(cioè
un [fork](#fork)\) per un periodo di tempo specifico, il periodo di
[lockout](#lockout).
...
[validatore](#validator), il numero di punti dovuti a uno [stake](#stake)
durante il riscatto è il prodotto dei [crediti di voto](#vote-credit) guadagnati
e il numero di lamport in stake.

## chiave privata

La chiave privata di una [coppia di chiavi](#keypair).
...
Una commissione aggiuntiva che l'utente può specificare nel budget di calcolo
[instruction](#instruction) per dare priorità alle proprie
...
scalare nell'intervallo di `0 <= S < L`. Questo requisito garantisce che non ci
sia malleabilità della firma. Ogni transazione deve avere almeno una firma per
...
del leader nell'epoch corrente. Questa metrica può essere fuorviante poiché ha
un'alta varianza dopo il confine dell'epoch quando la dimensione del campione è
piccola, così come per i validator con un basso numero di slot leader, tuttavia
può anche essere utile per identificare configurazioni errate dei nodi.

## skipped slot

Uno [slot](#slot) passato che non ha prodotto un [blocco](#block), perché il
leader era offline o il [fork](#fork) contenente lo slot è stato abbandonato dal
consenso del cluster per un'alternativa migliore. Uno slot saltato non apparirà
...
sequenzialmente e non si sovrappongono, comprendendo approssimativamente lo
stesso tempo reale secondo [PoH](#proof-of-history-poh).
...
Un [entry](#entry) del ledger che stima la durata del tempo reale.
...
Il [Token Extensions Program](https://spl.solana.com/token-2022) ha l'ID del
...
per i loro indirizzi di conio token unici.

## Token Program

Il [Token Program](https://spl.solana.com/token) ha l'ID del programma
...
Una funzione che richiede un tempo fisso per essere eseguita e produce una prova
della sua esecuzione, che può essere verificata in meno tempo di quanto ne abbia
...
mentre diventa progressivamente efficace. Durante questo periodo, lo stake è
