...
Vedi [onchain program](#onchain-program).
...
La chiave pubblica dell'[account](#account) contenente un [program](#program).
...
Commissione pagata dagli [Account](#account) e dai [Program](#program) per
memorizzare dati sulla blockchain. Quando gli account non hanno un saldo
sufficiente per pagare il rent, potrebbero essere sottoposti a Garbage
Collection.
...
di un root sono anche transitivamente root. I blocchi che non sono né antenati
né discendenti del root vengono esclusi dalla considerazione per il consenso e
...
scalare nell'intervallo di [0, 2^252 - 27742317777372353535851937790883648493].
Questo requisito garantisce che non ci sia malleabilità della firma. Ogni
transazione deve avere almeno una firma per
...
Un [slot](#slot) passato che non ha prodotto un [blocco](#block), perché il
...
stata accettata nel [ledger](#ledger). Una transazione viene finalizzata quando
il suo blocco diventa una [root](#root).
