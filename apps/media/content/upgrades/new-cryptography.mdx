...
Rather than adding BLS12-381-only syscalls, the proposal extends Solana's
generic curve syscall interface with new BLS12-381 curve IDs, in both
little-endian and big-endian encodings. That includes group operations, point
validation, pairing, and decompression through `sol_curve_group_op`,
`sol_curve_validate_point`, `sol_curve_pairing_map`, and `sol_curve_decompress`.
The pairing syscall supports batches of up to 8 point pairs in a single call.
