...
[Solana](https://solana.com/?ref=solana.ghost.io) is the world’s first web-scale blockchain. On current iterations of the Solana Testnet, a network of 200 physically distinct nodes supports a sustained throughput of more than 50,000 transactions per second when running with GPUs. Achieving as such requires the implementation of several optimizations and new technologies, and the result is a breakthrough in network capacity that signals a new phase in blockchain development.

The core innovation that underlays the Solana Network is [Proof of History](https://solana.com/proof-of-history/?ref=solana.ghost.io), and is exactly what the name suggests — a proof of historical events. Utilizing Proof of History creates a historical record that proves that an event has occurred at a specific moment in time. Whereas other blockchains require validators to talk to one another in order to agree that time has passed, each Solana validator maintains its own clock by encoding the passage of time in a simple SHA-256, sequential-hashing verifiable delay function (VDF).

This differs from the current standard of blockchain infrastructure, which relies on a sequential production of blocks that are hindered by waiting for confirmation across the network before moving forward. [Proof of History](https://solana.com/proof-of-history/?ref=solana.ghost.io) presents a fundamental move forward in the structure of blockchain networks in regards to speed and capacity.

<video url="https://youtu.be/7UE5UFd-b2c " />

Here’s how [Proof of History](https://solana.com/proof-of-history/?ref=solana.ghost.io) works: The Solana data structure chains messages together. This provides a cryptographic proof of the relative order and time of each message in the historical record. This allows the network to ignore local clocks and gradually accommodate all potential network delays as the data structure is eventually delivered and re-assembled. This is why Solana is able to push the limits of confirmation times so that the network provides as effective of an experience as a centralized system without sacrificing security or decentralization.
...
Block time, more so than raw Transactions Per Second, is a parameter that clearly separates centralized systems from decentralized blockchains. Tendermint has a timeout window [of 3 seconds](https://tendermint.com/docs/tendermint-core/using-tendermint.html?ref=solana.ghost.io#initialize), Libra [around 10](https://developers.libra.org/docs/assets/papers/the-libra-blockchain.pdf?ref=solana.ghost.io), Aglorand [around 5](https://people.csail.mit.edu/nickolai/papers/gilad-algorand-eprint.pdf?ref=solana.ghost.io). Solana’s globally distributed testnet is deployed with a block time of [400ms blocks with a leader scheduled for 4 consecutive blocks](https://github.com/solana-labs/solana/pull/4806?ref=solana.ghost.io).
...
The result of this functionality is that the network is never delayed, and can continue producing at lightning fast speeds despite variations caused by block producers. Proof of History is a powerful mechanism that unlocks network speeds heretofore unseen in blockchain technology. Along with innovations like [Tower BFT](https://medium.com/solana-labs/tower-bft-solanas-high-performance-implementation-of-pbft-464725911e79?ref=solana.ghost.io), [Turbine](https://medium.com/solana-labs/turbine-solanas-block-propagation-protocol-solves-the-scalability-trilemma-2ddba46a51db?ref=solana.ghost.io), and [Replicator](https://medium.com/solana-labs/replicators-solanas-solution-to-petabytes-of-blockchain-data-storage-ef79db053fa1?ref=solana.ghost.io) nodes, Proof of History makes Solana the first web-scale blockchain with transactional capacity that is akin to the modern internet.

We invite you to take a deeper look at all of the technological innovations the [Solana](http://solana.com/?ref=solana.ghost.io) team is implementing and to get involved with the Solana community by joining our [Discord](https://discordapp.com/invite/pquxPsq?ref=solana.ghost.io) or checking out [‘No Sharding: The Solana Podcast.](https://podcast.solana.com/?ref=solana.ghost.io)’ For developers, it’s a great chance to get information about the project directly from Solana team members.
