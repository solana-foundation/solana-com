...
* $S\_t$ as the number of signatures
* $A\_t$ as the number of static accounts
* $I\_t$ as the number of top-level instructions
* $a\_\{t\_i}$ as the number of accounts index in the instruction $i$
* $d\_\{t\_i}$ as the number of instruction-data byte of instruction $i$
* $L\_t$ as the number of Address Lookup Table referenced
* $w\_\{t\_j}$ and $r\_\{t\_j}$ as the number of writable e read-only index loaded in the lookup table $j$
* $v\_t=1$ for a V0 transaction and $V\_t = 0$ for a legacy transaction
* $c(n)$ as the number of bytes used by the `compact-u16` representation of $n$.

The function $c(n)$ is defined as (see [here](https://solana.com/docs/core/transactions/transaction-structure#transaction-binary-format))
