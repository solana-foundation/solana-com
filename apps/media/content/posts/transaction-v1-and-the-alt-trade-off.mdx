...
<Figure src="/uploads/posts/transaction-v1-and-the-alt-trade-off/account_and_instruction_headroom.png" alt="Remaining v1 account slots, decomposed by top-level instruction headroom. ">
  Remaining v1 account slots, decomposed by top-level instruction headroom. Bar height reports the share of current v0 ALT transactions in each account-headroom bin; colors identify the corresponding range of remaining instruction slots. Source: [Dune](https://dune.com/queries/8135365/12126122).
</Figure>
...
<Figure src="/uploads/posts/transaction-v1-and-the-alt-trade-off/serialized_size_pdf_lookup_entries.png" alt="Empirical PDF of serialized lookup-entry bytes per transaction">
  **Fig. 3:** Empirical PDF of serialized lookup-entry bytes per transaction. The zero bin contains transactions with no serialized lookup entries, while the positive support measures the current on-wire cost of ALT references. Source: [Dune](https://dune.com/queries/8134032/12124239).
</Figure>
...
<Figure src="/uploads/posts/transaction-v1-and-the-alt-trade-off/serialized_size_pdf_lookup_entries_tail.png" alt="Zoom of Fig. 3 over the 60-1232 byte range.">
  **Fig. 4:** Zoom of Fig. 3 over the 60-1232 byte range. Recurring modes reflect the fixed cost of referencing additional lookup tables together with the one-byte cost of each selected account index. Source: [Dune](https://dune.com/queries/8134032/12124239).
</Figure>
...
<Figure src="/uploads/posts/transaction-v1-and-the-alt-trade-off/serialized_size_lookup_vs_writes_and_reads.png" alt="Writable and read-only addresses loaded through ALT versus serialized lookup-entry size.">
  **Fig. 5:** Writable and read-only addresses loaded through ALT versus serialized lookup-entry size. Bubble size is proportional to transaction count. The diagonal bands primarily reflect different numbers of referenced lookup tables.. Source: [Dune](https://dune.com/queries/8127373/12117653?sidebar=none).
</Figure>
...
<Figure src="/uploads/posts/transaction-v1-and-the-alt-trade-off/serialized_size_lookup_table.png" alt="Most frequently ALT-loaded accounts.">
  **Fig. 6:** Most frequently ALT-loaded accounts. For each account, the table reports the number of transactions in which it is loaded through ALT and the mean number of lookup tables referenced by those transactions. Source: [Dune](https://dune.com/queries/8134041/12125566).
</Figure>
...
<Figure src="/uploads/posts/transaction-v1-and-the-alt-trade-off/lookup_tables_reference.png" alt="ALT usage by account-frequency quantile.">
  **Fig. 7:** ALT usage by account-frequency quantile. Accounts are grouped into equal-population buckets according to the number of transactions in which they are loaded through ALT. Lines report the p05, p50, mean and p95 of the per-account mean lookup-table count. Source: [Dune](https://dune.com/queries/8134649).
</Figure>
...
<Figure src="/uploads/posts/transaction-v1-and-the-alt-trade-off/lookup_tables_reference_frequency.png" alt="ALT usage by absolute account frequency. ">
  Fig. 8: ALT usage by absolute account frequency. The same account-level statistics as Figure 7, with each bucket positioned by its median number of transactions per account on a logarithmic scale. Source: [Dune](https://dune.com/queries/8134649).
</Figure>
...
<Figure src="/uploads/posts/transaction-v1-and-the-alt-trade-off/lookup_tables_reference_vs_accounts.png" alt="ALT-loaded addresses by number of referenced lookup tables.">
  **Fig. 9:** ALT-loaded addresses by number of referenced lookup tables. The vertical axis reports the average number of addresses loaded through ALT; bubble size is proportional to transaction count. Source: [Dune](https://dune.com/queries/8135329/12125810).
</Figure>
...
<Figure src="/uploads/posts/transaction-v1-and-the-alt-trade-off/concentration_of_alt_usage.png" alt="Concentration of ALT usage. ">
  **Fig. 10:** Concentration of ALT usage. Lookup tables are ranked by the number of distinct transactions referencing them. The left axis reports transaction count on a logarithmic scale, while the right axis reports the cumulative share of all transaction-to-ALT references. Source: [Dune](https://dune.com/queries/8141531/?utm_source=share\&utm_medium=copy\&utm_campaign=query).
</Figure>
...
<Figure src="/uploads/posts/transaction-v1-and-the-alt-trade-off/avg_addresses_loaded _from_table_per_use.png" alt="Average addresses loaded per ALT use.">
  **Fig. 11:** Average addresses loaded per ALT use. Lookup tables are ordered by transaction-reference rank. The vertical axis reports the average number of writable and read-only addresses selected from each table whenever it is referenced. Source: [Dune](https://dune.com/queries/8141531/?utm_source=share\&utm_medium=copy\&utm_campaign=query).
</Figure>
...
<Figure src="/uploads/posts/transaction-v1-and-the-alt-trade-off/n_of_alt_by_event_type.png" alt="Daily ALT usage and lifecycle activity.">
  **Fig. 12:** Daily ALT usage and lifecycle activity. n\_alts\_used is the number of distinct ALTs referenced by successful v0 transactions on each date. The remaining series report the number of distinct tables successfully created, extended, or closed during the same date. Source: [Dune](https://dune.com/queries/8146517/12138154).
</Figure>
...
<Figure src="/uploads/posts/transaction-v1-and-the-alt-trade-off/lookup_tables_v1_conversion_cdf.png" alt="Empirical CDF of the ALT-to-v1 account-encoding delta.">
  **Fig. 12:** Empirical CDF of the ALT-to-v1 account-encoding delta. The delta compares the complete v0 ALT section with the cost of including the same loaded addresses inline in v1. The distribution is conditional on v0 transactions using at least one ALT. Source: [Dune](https://dune.com/queries/8147839/12139360).
</Figure>
...
Once the ALT mechanism has been isolated, we can reconstruct the complete transaction under the proposed v1 layout (Fig. 13).
...
<Figure src="/uploads/posts/transaction-v1-and-the-alt-trade-off/serialized_size_v1_format_conversion_pdf_all.png" alt="Counterfactual v1 serialized-size PDF for the sampled legacy and v0 workload.">
  **Fig. 13:** Counterfactual v1 serialized-size PDF for the sampled legacy and v0 workload. Static addresses and addresses originally loaded through ALT are shown separately. The distribution is computed from the same 1/16 transaction sample as Figs. 1 and 2. Source: [Dune](https://dune.com/queries/8122675/12111819?sidebar=none).
</Figure>
...
The headroom analysis therefore considers both remaining account slots and remaining top-level instruction slots (Fig. 14).

<Figure src="/uploads/posts/transaction-v1-and-the-alt-trade-off/account_and_instruction_headroom.png" alt="Remaining v1 account slots, decomposed by top-level instruction headroom. ">
  Fig. 14: Remaining v1 account slots, decomposed by top-level instruction headroom. Bar height reports the share of current v0 ALT transactions in each account-headroom bin; colors identify the corresponding range of remaining instruction slots. Source: [Dune](https://dune.com/queries/8135365/12126122).
</Figure>
...
* <InlineLatex formula="a_{t_i}" /> as the number of accounts index in the instruction <InlineLatex formula="i" />
* <InlineLatex formula="d_{t_i}" /> as the number of instruction-data byte of instruction <InlineLatex formula="i" />
* <InlineLatex formula="L_t" /> as the number of Address Lookup Table referenced
* <InlineLatex formula="w_{t_j}" /> and <InlineLatex formula="r_{t_j}" /> as the number of writable e read-only index loaded in the lookup table <InlineLatex formula="j" />
* <InlineLatex formula="v_t=1" /> for a v0 transaction and <InlineLatex formula="V_t = 0" /> for a legacy transaction
* <InlineLatex formula="c(n)" /> as the number of bytes used by the `compact-u16` representation of <InlineLatex formula="n" />.
...
* <InlineLatex formula="A_t" /> as the number of unique account addresses referenced by the transaction
* <InlineLatex formula="I_t" /> as the number of top-level instructions
* <InlineLatex formula="a_{t_i}" /> as the number of account indices passed to instruction <InlineLatex formula="i" />
* <InlineLatex formula="d_{t_i}" /> as the number of instruction-data bytes of instruction <InlineLatex formula="i" />
* <InlineLatex formula="M_t" /> as the 32-bit `TransactionConfigMask`
* <InlineLatex formula="K_t=\operatorname{popcount}(M_t)" /> as the number of four-byte configuration slots contained in `ConfigValues`
...
* <InlineLatex formula="w_{t_j}" /> and <InlineLatex formula="r_{t_j}" /> be the writable and read-only indices selected from table <InlineLatex formula="j" />.
