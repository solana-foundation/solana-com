...
slug: high-performance-memory-management-for-smart-contracts
date: "2018-08-14"
categories:
...
seo:
  ...
  noFollow: false
  openGraph:
    ogTitle: High performance memory management for smart contracts
    ogUrl: >-
      https://www.solana.com/blog/high-performance-memory-management-for-smart-contracts
    ogDescription: |

      How smart contracts work on Solana’s Proof of History based blockchain
    ogImage: >-
      https://solana.ghost.io/content/images/downloaded_images/High-performance-memory-management-for-smart-contracts/1-IJugcnD5j7CIkouYXfD0mg.png
    ogType:
      - article
  twitter:
    twitterTitle: High performance memory management for smart contracts
    twitterDescription: |
...
Most of the focus on performance of smart contracts has been focused on moving to WASM. The bytecode doesn’t matter! While our bytecode is based on [Berkley Packet Filter](https://en.wikipedia.org/wiki/Berkeley_Packet_Filter?ref=solana.ghost.io), we can use anything that is easy to JIT to x86 (or SPIRV!). The reason why we are basing our bytecode on BPF is because what the kernel does with untrusted code overlaps almost exactly with the requirements we have:
...
Check us out on [Github](https://github.com/solana-labs?ref=solana.ghost.io) and find ways to connect on our [community](https://solana.com/community/?utm_source=medium&utm_campaign=memory_smart_contracts) page.
