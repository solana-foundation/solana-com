...
<Figure src="/uploads/posts/increase_txs_size_and_v1_format/serialized_size_pdf_lookup_entries_component.png" alt="Lookup-entry contribution to transaction serialized-size density.">
  **Fig. 2:** Lookup-entry contribution to transaction serialized-size density. At each transaction-size bin, the series combines the aggregate transaction density with the average fraction of bytes attributable to serialized lookup entries. It is computed from the same 1/16 sample as Fig. 1. Source: [Dune](https://dune.com/queries/8121324/12110510).
</Figure>
