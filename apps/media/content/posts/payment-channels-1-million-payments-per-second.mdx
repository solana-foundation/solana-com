...
description: Payment channels support 1 million payments per second by
  authorizing agent spending once, metering off-chain, and settling once on
  Solana.
publishedAt: 2026-09-03T16:26:48.690Z
categories:
  - category: payments
status: published
...
1. Meter off-chain: the agent authorizes spending with signed messages, not transactions. Every call is a signature and incurs no transaction costs.

1. Settle: the actual amount consumed is recorded onchain in a single transaction.

1. Distribute: funds distributed to recipient(s), any unused deposit returns to the primary wallet.
