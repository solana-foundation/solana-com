...
Up to now, every endpoint through pay.sh worked one way: you paid for each individual request you made. Using Subscriptions & Allowances, pay.sh now supports subscriptions, so that a provider can offer recurring flat-fee access. A subscriber agrees once to a set fee every period, and their requests stay free at the point of use for as long as the subscription is active.

For the people and agents using an API, it removes the overhead of metering every single call while providing steady income to the provider and adding flexibility for agentic commerce on Solana.&#x20;

## **Built to be shared infrastructure**

Design partners [Helius](http://helius.dev), [Confirmo](https://confirmo.com/), [Dynamic](http://www.dynamic.xyz), [Majority](http://www.majority.com), [Mesh](http://www.meshpay.com), [Meow](http://www.meow.com) helped to shape Solana Subscriptions & Allowances and are live or actively integrating the program. Moonsong Labs built the underlying program, in partnership with Solana Foundation. 
