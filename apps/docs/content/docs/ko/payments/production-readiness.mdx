...
프로덕션 백엔드 서명의 경우
[solana-keychain](https://github.com/solana-foundation/solana-keychain) 사용을
고려하세요. 이는 단일 인터페이스를 통해 여러 키 관리 백엔드를 추상화하는 통합
서명 라이브러리입니다: Memory, Vault, Privy, Turnkey, AWS KMS, Fireblocks, GCP
KMS, CDP, Para, Dfns. 이를 통해 로컬 환경에서는 인메모리 키로 개발하고,
애플리케이션 코드 변경 없이 프로덕션급 백엔드로 전환할 수 있습니다.
