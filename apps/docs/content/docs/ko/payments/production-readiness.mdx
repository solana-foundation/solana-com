...
프로덕션 백엔드 서명에는 [Keychain](/docs/tools/keychain)을 사용하세요.
Keychain은 단일 인터페이스를 통해 여러 키 관리 백엔드를 추상화하는 통합 서명
라이브러리입니다: Memory, Vault, Privy, Turnkey, AWS KMS, Fireblocks, GCP KMS,
CDP, Para, Dfns, Crossmint, Openfort, Utila를 지원합니다. 이를 통해 로컬 개발
시에는 인메모리 키를 사용하고, 애플리케이션 코드를 변경하지 않고도 프로덕션
수준의 백엔드로 전환할 수 있습니다.

어떤 백엔드가 적합한지 확실하지 않으신가요?
[백엔드 선택 가이드](/docs/tools/keychain/choosing-a-backend)를 참고하세요.
Keychain은 [Rust](https://crates.io/crates/solana-keychain)와
[TypeScript](https://www.npmjs.com/package/@solana/keychain) 모두에서 사용할 수
있습니다.

### RPC 보안

RPC 엔드포인트는 API 키처럼 취급하세요. 프론트엔드 코드에 노출하면 추출되어
악용될 수 있습니다. 백엔드 프록시를 사용하거나, 클라이언트 코드에 번들되지 않는
환경 변수를 활용하세요.
...
- 트레저리에서 임계값 이상의 이체 발생
- 트랜잭션 실패율 급증
- 비정상적인 수신자 패턴
- RPC 오류율 증가

대규모 실시간 트랜잭션 모니터링에 대해서는
[인덱싱 가이드](/docs/payments/accept-payments/indexing)를 참고하세요.

### 주소 검증

모든 토큰과 프로그램은 메인넷에서 정확히 하나의 올바른 주소를 가집니다. USDC나
다른 스테이블코인을 모방한 스푸핑 토큰이 흔히 존재하며, 이름과 심볼은 동일하지만
민트 주소가 다릅니다. 애플리케이션은 민트 주소를 하드코딩하거나 허용 목록으로
관리해야 하며(요구 사항에 따라), 신뢰할 수 없는 출처로부터 동적으로 수신해서는
안 됩니다.

**환경 기반 구성:** Devnet과 Mainnet은 종종 완전히 다른 토큰 민트를 사용합니다.
환경별로 올바른 주소를 로드하도록 애플리케이션 구성을 설정하세요. Mainnet 주소를
하드코딩한 채 테스트 시 교체하는 것을 잊거나, 더 나쁜 경우 Devnet 주소를
프로덕션에 배포하는 실수를 해서는 안 됩니다.

자주 사용되는 스테이블코인 민트 주소는 다음과 같습니다:
...
프로그램 주소도 중요합니다. 잘못된 프로그램에 명령을 전송하면 실패하거나, 더
나쁜 경우 자금을 돌이킬 수 없이 잃게 됩니다. Token Program 주소는 다음과
같습니다:
...
올바른 주소를 허용 목록에 등록하면 위조된 토큰으로부터 보호할 수 있지만, 잘못된
*종류*의 계정으로 전송하는 것은 방지하지 못합니다. 수신자 주소는 지갑, token
account, 민트, 또는 프로그램일 수 있으며, 각각 다른 처리 방식이 필요합니다. 지갑
이외의 대상에 전송된 네이티브 SOL은 발신자의 제어를 벗어나며, 민트나
프로그램으로 전송된 경우 즉시 소실되고, token account로 전송된 경우에는 해당
계정 소유자만 복구할 수 있습니다. 이 과정에서 사용자에게 경고하는 실패
트랜잭션도 발생하지 않습니다.

<Callout type="warn" title="전송 전 수신자를 검증하세요">
  서명하기 전에 모든 수신자 주소를 분류하세요. 지갑, token account, 민트,
  프로그램을 구분하는 전체 결정 트리와 참조 코드는 [주소
  검증](/docs/payments/send-payments/verify-address)을 참조하세요.
</Callout>

## 출시 전 체크리스트

- [ ] 수수료 및 rent용 메인넷 SOL 확보
...
- [ ] 모든 일반 오류의 적절한 처리
- [ ] 가스리스 구성 완료 (해당되는 경우)
- [ ] 메인넷 토큰 주소 확인 (devnet 민트 사용 금지)
- [ ] 전송 전 수신자 주소 유효성 검사 (지갑 vs token account vs 민트 vs program
      account)
- [ ] 모든 키 안전하게 백업
- [ ] 키 관리 검토 완료 (프론트엔드에 키 저장 금지)
- [ ] 트랜잭션 모니터링 및 알림 활성화
- [ ] 예상 트래픽 수준으로 부하 테스트 완료

## 프로그램 배포

결제 인프라의 일부로 커스텀 Solana 프로그램을 배포하는 경우 추가적인 고려 사항이
있습니다.
...
- **Solana CLI 버전:** 최신 버전의
  ...
  확인하세요.
- **프로그램 keypair:** 프로그램은 메인넷에서 devnet과 다른 주소를 가집니다
  (동일한 keypair를 재사용하지 않는 경우). 애플리케이션 설정의 모든 참조를
...
- **계정 초기화:** 프로그램에 관리자 계정, PDA 또는 기타 상태 계정이 필요한
  ...
  생성되었는지 확인하세요. 프로그램에 필요한 Associated Token Accounts (ATAs)도
  마찬가지입니다.

### 배포 프로세스

- **버퍼 계정:** 대형 프로그램은 버퍼 계정을 통해 배포됩니다.
  `solana program deploy` 명령어가 이를 자동으로 처리하지만, 배포는 원자적이지
  않으므로 중단될 경우 버퍼 계정을 복구하거나 닫아야 할 수 있습니다.
  [프로그램 배포](https://solana.com/docs/programs/deploying)를 참고하세요.
- **업그레이드 권한:** 프로그램을 출시 후에도 업그레이드 가능하게 할지
  결정하세요. 불변성을 원한다면 배포 후 업그레이드 권한을 철회하세요. 유연성을
  원한다면 업그레이드 권한 키를 적절히 보호하세요.
- **Rent:** 배포 지갑에 모든 program account의 rent 면제 최소 요건을 충족할
  만큼의 SOL이 있는지 확인하세요.
- **검증:** 배포한 실행 가능한 프로그램이 리포지토리의 소스 코드와 일치하는지
  확인하기 위해 프로그램을
  [검증](https://solana.com/docs/programs/verified-builds)하세요.
