...
사용하는 것을 고려하십시오. 이는 단일 인터페이스를 통해 여러 키 관리 백엔드를
추상화하는 통합 서명 라이브러리입니다: Memory, Vault, AWS KMS, Privy, Turnkey,
Fireblocks, CDP, Dfns, Para. 이를 통해 메모리 내 키로 로컬에서 개발한 다음,
애플리케이션 코드를 변경하지 않고도 프로덕션급 백엔드로 전환할 수 있습니다.
...
RPC 엔드포인트를 API 키처럼 취급하십시오. 추출 및 악용될 수 있는 프론트엔드
코드에 노출하지 마십시오. 클라이언트 코드에 번들되지 않는 백엔드 프록시 또는
환경 변수를 사용하십시오.
...
[인덱싱 가이드](/docs/payments/accept-payments/indexing)를 참조하십시오.

### 주소 검증

모든 토큰과 프로그램은 메인넷에서 정확히 하나의 올바른 주소를 가집니다. USDC
또는 기타 스테이블코인을 모방한 위조 토큰은 흔합니다. 이들은 동일한 이름과
심볼을 가지지만 다른 민트 주소를 가집니다. 귀하의 애플리케이션은 민트 주소를
하드코딩하거나 허용 목록에 추가해야 하며(귀하의 요구 사항에 따라), 신뢰할 수
없는 출처에서 동적으로 받아들여서는 안 됩니다.

**환경 기반 구성:** Devnet과 Mainnet은 종종 완전히 다른 토큰 민트를 사용합니다.
환경별로 올바른 주소를 로드하도록 애플리케이션 구성을 설정하세요. 메인넷 주소를
하드코딩한 후 테스트 중에 교체하는 것을 잊거나, 더 심각하게는 프로덕션에 devnet
주소를 배포하지 마세요.
...
프로그램 주소도 중요합니다. 잘못된 프로그램으로 명령을 전송하면 실패하거나, 더
심각하게는 자금의 돌이킬 수 없는 손실을 초래할 수 있습니다. Token Program 주소는
다음과 같습니다:
...
- [ ] 대체 RPC 엔드포인트 구성 완료
- [ ] 동적 가격 책정을 통한 우선 수수료 구현
- [ ] 재시도 로직이 블록해시 만료 처리
- [ ] 사용 사례에 적합한 확인 수준 설정
- [ ] 모든 일반적인 오류를 우아하게 처리
- [ ] 가스리스 구성 완료 (해당되는 경우)
- [ ] 메인넷 토큰 주소 검증 완료 (devnet 민트가 아닌)
- [ ] 모든 키를 안전하게 백업
...
결제 인프라의 일부로 커스텀 Solana 프로그램을 배포하는 경우 추가 고려 사항이
있습니다.
...
- **Solana CLI 버전:** 최신 버전의
  ...
  확인하세요.
- **프로그램 keypair:** 프로그램은 메인넷과 개발넷에서 다른 주소를
  가집니다(동일한 keypair를 재사용하지 않는 한). 애플리케이션 구성의 모든 참조를
  업데이트하세요. 프로그램 keypair는 안전한 위치에 보관하세요(`cargo clean`를
  실행하면 프로그램 keypair가 삭제될 수 있습니다).
- **계정 초기화:** 프로그램에 관리자 계정, PDA 또는 기타 상태 계정이 필요한
  경우, 사용자가 애플리케이션과 상호 작용하기 전에 메인넷에서 이러한 계정을
  생성해야 합니다. 프로그램에 필요한 Associated Token Account(ATA)도
  마찬가지입니다.
...
- **렌트:** 배포 지갑에 모든 program account에 대한 렌트 면제 최소 금액을 충당할
  수 있는 충분한 SOL이 있는지 확인하세요.
- **검증:** 배포한 실행 가능한 프로그램이 저장소의 소스 코드와 일치하는지
  확인하기 위해 프로그램을
