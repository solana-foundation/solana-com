...
<Callout type="caution" title="프로덕션에서는 공개 RPC를 사용하지 마세요">
  공개 엔드포인트(`api.mainnet.solana.com`, `api.devnet.solana.com`)는
  속도 제한이 있고 SLA가 없으며 프로덕션 결제 흐름에는 적합하지 않습니다.
  프로덕션 배포를 위한 전용 RPC 엔드포인트를 확보하려면 [RPC 공급자](https://solana.com/rpc)를 사용하세요.
</Callout>
