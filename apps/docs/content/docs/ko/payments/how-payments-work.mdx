...
솔라나에서 프로그램은 계정 상태를 관리하는 실행 가능한 로직입니다. 토큰 계정은
**토큰 프로그램**에 의해 관리되며, 이는 전송을 검증하고 잔액을 원자적으로
업데이트하는 온체인 코드입니다.
...
Token-2022("Token Extensions"라고도 불림)는 전송 훅, 전송 수수료, 기밀 잔액과 같은 기능을 추가합니다. 두 프로그램 모두 기본 전송에서는 유사하게 작동하지만, ATA를 파생할 때는 올바른 프로그램을 사용해야 합니다.
...
<Callout type="info">
  Solana는 로컬 수수료 마켓을 사용합니다. 트랜잭션은 동일한 상태에 쓰는 다른 트랜잭션들과 경쟁합니다. 이는 네트워크의 다른 곳에서 활동이 급증하는 시기에도 결제 수수료가 낮고 예측 가능한 수준으로 유지됨을 의미합니다.
</Callout>
