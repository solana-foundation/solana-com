...
1. 논스 값을 트랜잭션의 "blockhash"로 설정합니다
2. `advanceNonceAccount` 인스트럭션을 자동으로 앞에 추가합니다 (모든 영구 논스
   트랜잭션에서 첫 번째 인스트럭션으로 필수)
...
직렬화된 문자열을 데이터베이스에 저장하면 논스가 진행될 때까지 유효합니다.
...
<Callout type="caution">
  ...
  결정한 경우 동일한 논스 계정으로 다른 트랜잭션을 준비하기 전에 논스를 진행해야
  합니다.
</Callout>
