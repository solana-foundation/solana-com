...
- 금액은 기본 단위로 표시됩니다. 소수점 6자리 토큰의 경우, `1_000_000`는 `1`
  토큰을 의미합니다.
- Subscription Authority 계정이 아직 존재하지 않을 때만
  `initSubscriptionAuthority`를 실행하세요.
- `expiryTs`는 강제 종료 시점입니다: 이 시점이 지나면 전송이 실패하고 스폰서는
  rent를 회수할 수 있습니다. 지출 유예 기간은 없습니다.
- 사용자는 설정 및 취소 트랜잭션에 서명하고, 위임자는 전송 트랜잭션에
  서명합니다.
