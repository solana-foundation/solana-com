...
<Callout type="caution">
  Commerce Kit은 현재 베타 버전입니다. 안정 버전 출시 전에 API가 변경될 수
  있습니다. Commerce Kit에 대한 자세한 내용은 [Commerce Kit
  문서](/docs/tools/commerce-kit)를 참조하세요.
</Callout>
...
| 콜백               | 매개변수              | 설명                         |
| ------------------ | --------------------- | ---------------------------- |
| `onPaymentSuccess` | `(signature: string)` | 온체인에서 트랜잭션이 확인됨 |
| `onPaymentError`   | `(error: Error)`      | 모든 단계에서 결제 실패      |
| `onPaymentStart`   | `()`                  | 결제 흐름 시작               |
| `onCancel`         | `()`                  | 사용자가 결제 취소           |
