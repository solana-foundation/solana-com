...
일괄 전송 후, 클라이언트의 RPC 연결을 통해 각 token account를 조회하여 모든
당사자의 토큰 잔액을 확인합니다.
...
크기 제한을 4096바이트로 늘려, 트랜잭션당 약 세 배 더 많은
전송을 처리할 수 있습니다. 그럼에도 그 이상의 경우에는 여전히 배칭이 필요한데,
런타임이 형식에 관계없이 트랜잭션당 최대 64개의 계정만 로드하기 때문입니다.
