...
<Callout type="info">
  헤드리스 환경에서 실행 중이신가요 — API 라우트, 워커, 또는 브라우저 지갑이
  없는 스크립트? raw keypair 대신 백엔드 키 관리 서비스로 서명하세요.
  [Keychain](/docs/tools/keychain) 및 [프로덕션에서
  서명하기](/docs/core/transactions/signing-in-production)를 참고하세요.
</Callout>

## Solana 개발자를 위한 일반적인 패턴

- **헤드리스 상태 머신**: API 라우트, 스크립트, 또는 워커 내부에서 클라이언트를
  실행하여 UI를 구동하는 동일한 지갑 + RPC 오케스트레이션 로직을 재사용하세요.
- **실시간 대시보드**: 워처(잔액, 계정, 서명)를 선호하는 UI 라이브러리와
  결합하세요. 클라이언트가 웹소켓 팬아웃 및 캐시 무효화를 처리합니다.
- **커스텀 스토어**: 자체 Zustand 스토어를 주입하여 IndexedDB/localStorage에서
  하이드레이션하거나, 서버 세션에 상태를 미러링하거나, 브라우저 탭 간에
  조율하세요.
- **React 훅 연결**: 동일한 런타임 위에서 편리한 훅을 사용하려면 구성된
  클라이언트 인스턴스를 `@solana/react-hooks`에 전달하세요.
- **테스트 용이성**: 단일 클라이언트 인터페이스는 유닛 테스트에서 모킹할 수
