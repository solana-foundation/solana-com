...
<Callout type="info">
  헤드리스로 실행 중인가요 — API 라우트, 워커 또는 브라우저 지갑이 없는
  스크립트에서? 원시 keypair 대신 백엔드 키 관리 서비스로 서명하세요.
  [Keychain](/docs/tools/keychain) 및 [프로덕션에서
  서명하기](/docs/core/transactions/signing-in-production)를 참조하세요.
</Callout>
...
- **실시간 대시보드**: 감시자(잔액, 계정, 서명)를 선호하는 UI 라이브러리와
  결합하세요. 클라이언트가 웹소켓 팬아웃과 캐시 무효화를 처리합니다.
- **커스텀 스토어**: 자체 Zustand 스토어를 주입하여 IndexedDB/localStorage에서
  하이드레이션하거나, 상태를 서버 세션에 미러링하거나, 브라우저 탭 간 조율을
  수행합니다.
- **React 훅 연동**: 동일한 런타임 위에서 편리한 훅을 사용하려면 설정된
