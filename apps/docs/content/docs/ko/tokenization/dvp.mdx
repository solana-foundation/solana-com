...
Solana 트랜잭션에는 크기 제한(~1232바이트)이 있습니다. 각 전송 명령어는
약 200바이트를 추가합니다. 규모가 매우 큰 다자간 정산의 경우,
[Address Lookup Tables](/developers/cookbook/transactions/lookup-tables)을 사용하여
계정 주소를 압축하고 트랜잭션당 더 많은 전송을 처리하는 것을 고려해 보세요.

곧 출시될
[v1 트랜잭션 형식](/docs/core/transactions/versioned-transactions#v1-message-format)은
제한을 4096바이트로 늘리지만 Address Lookup Table 지원이 제거되므로, v1
정산은 주소를 인라인으로 포함합니다. 어느 방식이든 런타임은 트랜잭션당 최대
64개의 계정을 로드합니다.
