...
ACL 프로그램의 코드는
[token-acl 저장소](https://github.com/solana-foundation/token-acl)에서 확인할 수 있으며,
ABL Gate Program은
[token-acl-gate 저장소](https://github.com/solana-foundation/token-acl-gate)에서 확인할 수 있습니다.

**중요**: 이 가이드에서 사용된 ABL(허용 차단 목록) Gate Program은
**참조 구현**입니다. 감사를 받았으며 프로덕션 환경에서 사용 가능하지만, 발행자는
특정 규정 준수 요구 사항에 더 적합한 맞춤형 Gate Program을 자유롭게 만들 수 있습니다. Token ACL 사양(sRFC37)만 준수하면 되며,
이 특정 Gate Program 설계에 구속되지 않습니다.

다음 사항 없이 **이 코드를 프로덕션 환경에서 직접 사용하지 마십시오**:
...
4. **명세서 읽기**: 전체 기술 명세를 위해 [sRFC37](https://github.com/solana-foundation/token-acl/blob/master/srfc37.md)을 검토하고 [sRFC37 토론](https://github.com/solana-foundation/SRFCs/discussions/2)에 참여하세요.
