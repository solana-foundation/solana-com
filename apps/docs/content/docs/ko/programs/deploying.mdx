...
| 프로그램을 불변으로 만들기             | `solana program set-upgrade-authority <program-id> --final`                                   |
| 프로그램 닫기                      | `solana program close <program-id> --bypass-warning`                                          |
| 지갑 잔액 확인               | `solana balance`                                                                              |
| 에어드랍 요청 (devnet/localhost) | `solana airdrop 2`                                                                            |
...
- **program account**: 프로그램의 메타데이터를 저장하는 온체인 계정입니다.
  ...
  account의 주소를 저장합니다.
- **programdata account**: 프로그램의 배포된 실행 코드를 저장하는 온체인
  계정입니다.
- **프로그램 권한**: 프로그램을 업데이트하거나 닫을 수 있는 권한을 가진
  계정입니다. 기본적으로 CLI 지갑이 해당됩니다.
...
이 명령은 기본적으로 `~/.config/solana/id.json`에 keypair를 생성합니다.
...
devnet 또는 localhost의 경우 `solana airdrop` 명령어를 사용하여 지갑에 자금을
충전하세요:
...
이 명령은 `target/deploy/`에 두 개의 중요한 파일을 생성합니다:
...
동일한 `solana program deploy` 명령어를 사용하여 프로그램을 업데이트하세요:

1. 프로그램 코드를 수정합니다
2. 프로그램을 다시 빌드합니다: `cargo build-sbf`
3. 업데이트를 배포합니다:
...
프로그램 권한을 다른 계정으로 이전하려면 `solana program set-upgrade-authority`
명령어를 사용하세요:
...
프로그램을 불변으로 만들려면 `solana program set-upgrade-authority` 명령어에
`--final` 플래그를 사용하여 프로그램 권한을 제거하세요:
...
프로그램을 닫고 프로그램 계정에 할당된 SOL을 회수하려면 `solana program close`
명령어를 사용하세요:
...
- **`--with-compute-unit-price`**: 컴퓨트 유닛당 마이크로 램포트(0.000001 SOL)
  ...
  [Helius Priority Fee API](https://docs.helius.dev/guides/priority-fee-api)를
  확인하세요. 우선순위 수수료는 트랜잭션의 우선순위를 높이기 위해 현재 리더에게
  지불하는 추가 수수료입니다.
- **`--max-sign-attempts`**: 트랜잭션이 만료되면 새로운 블록해시로 재시도할
  횟수입니다. (기본값: 5)
- **`--use-rpc`**: 설정된 RPC로 트랜잭션을 전송합니다. 이 플래그는 [스테이크 가중치](/docs/defi/stake-weighted-qos) RPC 연결이 필요하며, [Triton](https://triton.one/) 또는 [Helius](https://www.helius.dev/)와 같은 공급자가 제공합니다.
