...
`mollusk-svm`를 `Cargo.toml`에 의존성으로 추가합니다:
...
컴퓨트 유닛 사용량을 벤치마크하려면 `mollusk-svm-bencher`를 `Cargo.toml`에
의존성으로 추가합니다:
...
Extensions) 및 Associated Token Program을 사용하려면
`mollusk-svm-programs-token`를 `Cargo.toml`에 의존성으로 추가합니다:
...
1. [`Mollusk` 인스턴스 생성](mention:mollusk) - 프로그램 ID와 컴파일된 프로그램
   경로(`.so` 파일)로 Mollusk를 초기화합니다
2. [인스트럭션 빌드](mention:instruction) - 프로그램을 호출할 인스트럭션을
   생성합니다
3. [처리 및 검증](mention:process) - Mollusk를 사용하여 인스트럭션을 처리하고
   결과를 검증합니다
...
기본 인스턴스로 Mollusk를 초기화하려면 `Mollusk::default` 메서드를 사용합니다.
...
특정 프로그램으로 Mollusk를 초기화하려면 `Mollusk::new` 메서드를 사용합니다.
...
Mollusk에 프로그램을 추가하려면 `Mollusk::add_program` 메서드를 사용합니다.
...
일반적으로 `#[test]` 속성으로 주석이 달린 테스트 모듈에서 Mollusk를 사용합니다.
...
`process_and_validate_instruction` 메서드를 사용하여 검증 검사와 함께 단일
...
`process_instruction_chain` 메서드를 사용하여 검증 검사 없이 여러 명령어를
순차적으로 처리합니다.
...
상태를 유지합니다. 명령어 처리 메서드는 `Mollusk`와 동일합니다.
