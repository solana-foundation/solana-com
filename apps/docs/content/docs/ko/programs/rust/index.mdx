...
프로젝트 디렉토리로 이동합니다. 기본 `src/lib.rs` 및 `Cargo.toml` 파일이
표시됩니다.
...
다음으로 `solana-program` 의존성을 추가합니다. 이것은 Solana 프로그램을 빌드하는
데 필요한 최소 의존성입니다.
...
다음으로 `Cargo.toml`에 다음 스니펫을 추가합니다.
...
다음으로 `src/lib.rs`의 내용을 다음 코드로 교체합니다. 이것은 프로그램이 호출될
...
다음으로 `cargo build-sbf` 명령을 사용하여 프로그램을 빌드합니다.

```terminal
$ cargo build-sbf
```

이 명령은 두 개의 중요한 파일을 포함하는 `target/deploy` 디렉토리를 생성합니다:
...
다음으로, `litesvm` 크레이트를 사용하여 프로그램을 테스트하세요. `Cargo.toml`에
다음 의존성을 추가하세요.
...
프로그램 코드 아래의 `src/lib.rs`에 다음 테스트를 추가하세요. 이것은 hello world
...
`cargo test` 명령을 사용하여 테스트를 실행하세요. 프로그램 로그에 "Hello,
world!"가 표시됩니다.
...
다음으로, 프로그램을 배포하세요. 로컬에서 개발할 때는 `solana-test-validator`를
사용할 수 있습니다.
...
`solana-client` 및 `tokio` 의존성을 추가합니다.
