...
것이며, 이는 프로그램 실행이 시작되는 지점을 정의합니다.
...
- `entrypoint.rs`: 들어오는 명령어를 라우팅하는 진입점을 정의합니다.
...
이제 Solana 프로그램을 빌드하는 데 필요한 종속성을 추가해 보겠습니다. 핵심 SDK를
...
[엔트리포인트](https://github.com/anza-xyz/solana-sdk/blob/449d97c0ed164611dae538e2ee91ca0caaaec515/program-entrypoint/src/lib.rs#L126-L140)
매크로는 `input` 데이터를 `process_instruction` 함수의 매개변수로 역직렬화하는
작업을 처리합니다.

솔라나 프로그램 `entrypoint`는 다음과 같은 함수 시그니처를 가집니다. 개발자는
...
다음 코드를 `lib.rs`에 추가하여 `process_instruction` 함수를 업데이트하고
`InitializeCounter` 및 `IncrementCounter` 인스트럭션에 대한 핸들러를 추가하세요:
...
우리 프로그램은 CPI를 통해 시스템 프로그램의 `create_account` 명령어를
호출합니다. 새 계정은 우리 프로그램을 소유자로 하여 생성되며, 이를 통해 우리
프로그램이 계정에 쓰기 작업을 수행하고 데이터를 초기화할 수 있습니다.

`process_initialize_counter` 함수를 업데이트하는 다음 코드를 `lib.rs`에
추가하세요:
...
`process_increment_counter` 함수를 업데이트하는 다음 코드를 `lib.rs`에
추가하세요:
...
먼저 테스트에 필요한 의존성을 추가하겠습니다. 테스트를 위해 `litesvm`를 사용하고
`solana-sdk`를 사용합니다.
...
명령어를 실행하여 프로그램을 빌드하세요. 이렇게 하면 `target/deploy` 디렉터리에
...
`examples/client.rs`에 클라이언트 코드를 추가하고 `program_id`를 이전 명령의
출력으로 교체하세요:
...
이 명령은 프로그램을 컴파일하고 `target/deploy/`에 두 개의 중요한 파일을
생성합니다:
...
프로그램 ID와 함께 `solana program show` 명령을 사용하여 배포를 확인할 수
있습니다:
...
하며, 기본값은 `http://localhost:8899`로 `solana-test-validator`가 실행되는
주소입니다.
