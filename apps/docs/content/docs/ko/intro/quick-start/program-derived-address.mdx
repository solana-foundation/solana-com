...
title: 결정론적 계정 생성하기
description:
  Program Derived Address(PDA)와 Anchor 프레임워크를 사용하여 CRUD(생성, 읽기,
  업데이트, 삭제) Solana 프로그램을 구축하는 방법을 배워보세요. 이 단계별
  가이드는 PDA를 사용하여 온체인 메시지 계정을 생성, 업데이트 및 삭제하는 방법,
  계정 검증을 구현하는 방법, 테스트를 작성하는 방법을 설명합니다. Solana
...
`lib.rs` 파일에서 다음 단계에 추가할 [`create`](mention:one),
[`update`](mention:two), [`delete`](mention:three) 명령어가 포함된 프로그램을
찾을 수 있습니다.
...
시작하기 전에 Playground 터미널에서 _shell`build`_ 를 실행하여 스타터 프로그램이
성공적으로 빌드되는지 확인하세요.
...
Anchor 프로그램에서 _rs`#[account]`_ 속성은 계정 데이터를 나타내는 구조체에
주석을 답니다(계정의 데이터 필드에 저장할 데이터 타입).

### !user

_rs`user`_ 필드는 메시지 계정을 생성한 사용자를 식별하는 _rs`Pubkey`_ 를
포함합니다.

### !message

_rs`message`_ 필드는 사용자의 메시지를 포함하는 _rs`String`_ 를 보유합니다.

### !bump

_rs`bump`_ 필드는 Program Derived Address(PDA)를 도출하는 데 사용되는 _rs`u8`_
["bump" seed](/docs/core/pda#canonical-bump)를 저장합니다. 이 값을 저장하면 이후
명령어에서 재계산할 필요가 없어져 연산을 절약할 수 있습니다.
...
- `user` - 메시지 계정을 생성한 사용자를 식별하는 _rs`Pubkey`_.
- `message` - 사용자의 메시지를 포함하는 _rs`String`_.
- `bump` - Program Derived Address(PDA)를 도출하기 위한
  ["bump" seed](/docs/core/pda#canonical-bump)를 저장하는 _rs`u8`_. 이 값을
  저장하면 이후 명령어에서 재계산할 필요가 없어져 연산을 절약할 수 있습니다.

계정을 생성할 때 프로그램은 _rs`MessageAccount`_ 데이터를 직렬화하여 새 계정의
데이터 필드에 저장합니다.

나중에 계정에서 읽을 때 프로그램은 이 데이터를 _rs`MessageAccount`_ 데이터
타입으로 역직렬화합니다. 테스트 섹션에서는 계정 데이터를 생성하고 읽는 과정을
보여줍니다.

</Accordion>
</Accordions>

터미널에서 _shell`build`_ 를 실행하여 프로그램을 다시 빌드합니다.
...
이제 _rs`MessageAccount`_ 를 생성하고 초기화하는 _rs`create`_ 명령어를
추가합니다.

_rs`Create`_ 구조체를 다음과 같이 업데이트하여 명령어에 필요한 계정을 정의하는
것부터 시작합니다:
...
_rs`mut`_ 제약 조건은 계정을 변경 가능한 것으로 선언합니다.
...
_rs`Signer<'info>`_ 타입은 계정이 트랜잭션에 서명해야 함을 요구합니다.

### !account

_rs`Account<'info, T>`_ 타입은 계정이 지정된 타입과 일치해야 함을 요구합니다.

이 경우 계정은 커스텀 _rs`MessageAccount`_ 타입과 일치해야 합니다.

### !program

_rs`Program<'info, T>`_ 타입은 계정이 프로그램과 일치해야 함을 요구합니다.

이 경우 계정은 System Program을 참조하는 _rs`System`_ 타입과 일치해야 합니다.
...
1. 프로그램이 예상하는 계정의 종류를 지정하는 계정 타입(_rs`Signer<'info>`_ 또는
   _rs`Account<'info, T>`_ 과 같은)
2. 추가 요구 사항을 정의하는 선택적 제약 조건(_rs`#[account(mut)]`_ 또는
   _rs`#[account(init)]`_ 과 같은)
...
1. _rs`user: Signer<'info>`_
   ...
   - 새 계정 비용을 지불하므로 변경 가능 상태(_rs`#[account(mut)]`_)가
     필요합니다
   - 이 계정에서 lamport 차감을 승인하려면 트랜잭션에 서명해야 합니다

2. _rs`message_account: Account<'info, MessageAccount>`_
   - 사용자의 메시지를 저장하는 새 계정
   - `init` 제약 조건은 명령어 실행 중에 계정을 생성합니다
   - `seeds`와 `bump` 제약 조건은 계정 주소를 Program Derived Address(PDA)로
     파생합니다
   - `payer = user`는 새 계정 생성 비용을 지불하는 사람을 식별합니다
...
- _rs`b"message"`_ - 첫 번째 시드로 사용되는 고정 문자열입니다.
- _rs`user.key().as_ref()`_ - 두 번째 시드로 사용되는 _rs`user`_ 계정의 공개
  키입니다.

`bump` 제약 조건은 Anchor가 올바른 bump seed를 자동으로 찾아 사용하도록
지시합니다. Anchor는 `seeds`와 `bump`를 사용하여 PDA를 도출합니다.

---

`space` 계산 _rs`(8 + 32 + 4 + message.len() + 1)`_ 는 `MessageAccount` 데이터
타입을 위한 공간을 할당합니다:

- Anchor 계정 식별자(discriminator): 8바이트
- 사용자 주소(_rs`Pubkey`_): 32바이트
- 사용자 메시지(_rs`String`_): 길이를 위한 4바이트 + 가변 메시지 길이
...
다음으로, _rs`create`_ 명령어의 비즈니스 로직을 추가하기 위해 `create` 함수를
다음과 같이 업데이트합니다:
...
2. 계정 데이터 초기화:
   ...
   - `message` 필드를 함수 인수의 `message`로 설정합니다.

   ```rs
   account_data.message = message;
   ```

   - `ctx.bumps.message_account`에서 가져온 PDA를 파생하는 데 사용되는 `bump`
     값을 설정합니다.
...
다음으로, 새 메시지로 `MessageAccount`를 변경하는 `update` 명령어를 추가하세요.

이전 단계와 마찬가지로, 먼저 `update` 명령어에 필요한 계정을 지정하세요.
...
1. _rs`user: Signer<'info>`_
   ...
   - 필요할 때 `message_account`에 대한 추가 공간 비용을 지불할 수 있으므로
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - `realloc` 제약 조건은 System Program을 호출하여 계정의 데이터 크기를
     조정합니다

_rs`bump = message_account.bump`_ 제약 조건은 Anchor가 재계산하는 대신
_rs`message_account`_ 에 저장된 bump seed를 사용한다는 점에 유의하세요.
...
2. 계정 데이터 업데이트:
   - 컨텍스트에서 `message_account`에 접근합니다.
   - `message` 필드를 함수 인수의 새 `message`로 설정합니다.
...
다음으로, _rs`MessageAccount`_ 를 닫기 위한 _rs`delete`_ 명령어를 추가합니다.
...
_rs`seeds`_ 제약 조건은 PDA를 도출하는 데 사용되는 seed를 지정합니다.

### !bump

_rs`bump`_ 제약 조건은 PDA의 bump seed를 지정합니다.

이 경우 프로그램은 _rs`message_account`_ 에 저장된 기존 bump seed를 사용합니다.

### !close

_rs`close`_ 제약 조건은 계정을 닫습니다.

이 경우 _rs`user`_ 계정은 닫힌 _rs`message_account`_ 로부터 lamport를 받습니다.
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - `close = user` 제약 조건은 이 계정을 닫도록 표시하고 해당 lamport를 `user`
     계정으로 전송합니다
...
함수 본문은 _rs`msg!()`_ 매크로를 사용하여 프로그램 로그에 메시지를 출력하기만
합니다. _rs`Delete`_ 구조체의 _rs`close`_ 제약 조건이 계정 닫기를 처리하므로
함수에는 추가 로직이 필요하지 않습니다.
...
이제 기본 CRUD 프로그램을 완성했습니다. Playground 터미널에서 `deploy`를
실행하여 프로그램을 배포하세요.
...
스타터 코드에는 `anchor.test.ts`에 테스트 파일도 포함되어 있습니다.
...
_ts`describe()`_ 내부에, 그러나 _ts`it()`_ 섹션 앞에 아래 코드를 추가하세요.
...
Solana Playground는 일부 보일러플레이트 설정을 제거하여
[`pg.program`](mention:one)는 프로그램과 상호작용하는 메서드에 대한 접근을
허용하고, [`pg.wallet`](mention:two)는 플레이그라운드 지갑에 대한 접근을
제공합니다.
...
플레이그라운드 터미널에서 *shell`test`*를 실행하여 테스트 파일을 실행하고
예상대로 실행되는지 확인하세요. 다음 단계에서 실제 테스트를 추가합니다.
...
먼저, 테스트 파일은 `create` 명령을 호출하는 트랜잭션을 전송하며, 메시지로
"Hello, World!"를 전달합니다.
...
먼저, 테스트 파일은 `update` 명령을 호출하는 트랜잭션을 전송하며, 새 메시지로
"Hello, Solana!"를 전달합니다.
...
_ts`fetchNullable()`_ 메서드는 존재하지 않을 수 있는 계정 데이터를 네트워크에서
가져옵니다.
...
트랜잭션을 전송하고 계정을 닫은 후, 예제는 `fetchNullable`을 사용하여 계정
...
테스트를 준비한 후, Playground 터미널에서 *shell`test`*를 실행하여 테스트 파일을
실행합니다. 이 명령은 devnet에 배포된 프로그램에 대해 테스트를 실행하고 트랜잭션
세부 정보를 확인할 수 있는 SolanaFM 링크를 로그에 기록합니다.
...
<Callout type="info">
  이 예제에서 테스트를 다시 실행하면 `messageAccount`가 이미 계정으로 존재하기
  때문에 `create` 명령이 실패합니다. 주어진 PDA에 대해 하나의 계정만 존재할 수
  있습니다.
</Callout>
