...
다양한 계정을 가져와 Solana 네트워크에서 데이터를 읽어보세요. 이 가이드는
Solana [계정](/docs/core/accounts)의 구조를 설명합니다. 각 Solana
계정에는 고유한
[주소](/docs/core/accounts/account-structure#account-address)가 있으며, 이를 통해
계정의 온체인 데이터를 찾을 수 있습니다. Solana 계정에는
[상태 데이터 또는 실행 가능한 프로그램](/docs/core/accounts/account-types)이 포함되어 있습니다.
...
[Mint](mention:one)
데이터 타입으로 역직렬화하는 [_ts`fetchMint()`_](https://github.com/solana-program/token/blob/program%40v8.0.0/program/src/state.rs#L16-L30) 함수가 포함되어
있습니다.
