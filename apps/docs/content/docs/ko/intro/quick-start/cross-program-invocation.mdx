...
`update`와 `delete` 명령어는 System Program을 호출하여 계정 간 SOL 전송을
처리하도록 수정이 필요합니다.
...
먼저 _rs`Update`_ 구조체와 `update` 함수를 변경하여 간단한 "업데이트 시 지불"
메커니즘이 필요합니다.

`lib.rs` 파일을 업데이트하여 `system_program` 모듈의 항목을 스코프로 가져오는
것부터 시작하세요.
...
포함시키세요. 프로그램이 제어하는 이 계정은 사용자가 메시지 계정을 업데이트할 때
사용자로부터 SOL을 받습니다.
...
- 계정 주소는 _rs`[b"vault", user.key().as_ref()]`_ 시드를 사용하여 파생된
  PDA에서 가져옵니다
- PDA이므로 개인 키가 없어 CPI를 수행할 때 프로그램만 주소에 대해 "서명"할 수
  있습니다
- _rs`SystemAccount`_ 타입이므로 일반 지갑 계정처럼 System Program이 소유합니다
...
`delete` 명령어에서 프로그램이 CPI에서 이 PDA에 대해 "서명"하는 방법을 볼 수
있습니다.
...
_rs`CpiContext`_ 구조체는 크로스 프로그램 호출(CPI)을 위한 프로그램과 계정을
지정합니다.
...
`update` 명령어의 구현에는 System Program의 `transfer` 명령어를 호출하기 위한
크로스 프로그램 호출(CPI)이 포함되어 있습니다. 이는 프로그램 내에서 CPI를
수행하는 방법을 보여주며, Solana 프로그램의 조합 가능성을 가능하게 합니다.
...
`transfer` 함수는 System Program의 전송 명령어를 호출하며 다음을 전달합니다:

- `cpi_context`(프로그램 및 계정)
- 전송할 `amount`(1,000,000 램포트, 즉 0.001 SOL)
...
CPI 설정은 클라이언트 측 명령어가 구축되는 방식과 일치하며, 호출할 특정 명령어에
대한 프로그램, 계정 및 instruction data를 지정합니다. 프로그램의 `update`
...
먼저 _rs`Delete`_ 구조체를 업데이트하여 `vault_account`를 포함시킵니다. 이를
통해 사용자가 메시지 계정을 닫을 때 금고에 있는 모든 SOL을 사용자에게 다시
전송할 수 있습니다.
...
또한 전송을 위한 CPI가 System Program 호출을 필요로 하므로 `system_program`를
추가합니다.
...
`vault_account`는 Update 구조체와 동일한 PDA 유도를 사용합니다.

Delete 구조체에 `vault_account`를 추가하면 프로그램이 삭제 명령 중에 사용자의
금고 계정에 접근하여 누적된 SOL을 사용자에게 다시 전송할 수 있습니다.
...
다음으로, `delete` 명령어에 CPI 로직을 추가하여 금고 계정에서 사용자 계정으로
SOL을 전송합니다.
...
*rs`signer_seeds`*는 PDA를 유도하는 데 사용되는 선택적 시드와 범프 시드를
지정합니다.
...
*rs`_ctx: Context<Delete>`*가 함수 본문에서 컨텍스트를 사용하기 위해
*rs`ctx: Context<Delete>`*로 변경됩니다.
...
_rs`transfer()`_ 함수는 다음을 전달하여 System Program에서 전송 명령을
호출합니다:
...
다음으로, `anchor.test.ts` 파일을 업데이트하여 명령어에 새로운 금고 계정을
포함시킵니다. 이를 위해서는 금고 PDA를 유도하고 업데이트 및 삭제 명령 호출에
포함시켜야 합니다.
...
그런 다음 업데이트 명령어를 수정하여 [`vaultAccount`](mention:one)를 포함하도록
합니다
...
그런 다음 삭제 명령어를 수정하여 [`vaultAccount`](mention:one)를 포함하도록
합니다
...
목록에 추가합니다. 프로젝트를 가져오면 모든 변경 사항이 자동으로 저장되고
유지됩니다.
