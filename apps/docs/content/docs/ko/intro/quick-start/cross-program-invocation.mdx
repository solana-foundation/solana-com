...
포함시킵니다. 프로그램이 제어하는 이 계정은 사용자가 메시지 계정을 업데이트할 때
사용자로부터 SOL을 받습니다.
...
- 계정 주소는 _rs`[b"vault", user.key().as_ref()]`_ 시드를 사용하여 파생된
  PDA에서 가져옵니다
- PDA이므로 개인 키가 없어 CPI를 수행할 때 프로그램만 해당 주소에 대해 "서명"할
  수 있습니다
...
`delete` 명령어에서 프로그램이 CPI에서 이 PDA에 대해 "서명"하는 방법을 확인할 수
있습니다.
...
_rs`CpiContext`_ 구조체는 Cross Program Invocation (CPI)을 위한 프로그램과
계정을 지정합니다.
...
`update` 명령의 구현에는 System Program의 `transfer` 명령을 호출하는 Cross
Program Invocation (CPI)이 포함됩니다. 이는 프로그램 내에서 CPI를 수행하는
...
- 전송할 `amount` (1,000,000 lamports, 또는 0.001 SOL)
...
CPI 설정은 클라이언트 측 명령이 구축되는 방식과 일치하며, 호출할 특정 명령에
대한 프로그램, 계정 및 instruction data를 지정합니다. 프로그램의 `update` 명령이
...
또한 `system_program`를 추가합니다. 전송을 위한 CPI가 System Program을 호출해야
하기 때문입니다.
...
`vault_account`를 Delete 구조체에 추가하면 프로그램이 삭제 명령어 실행 중에
사용자의 금고 계정에 접근하여 누적된 SOL을 사용자에게 다시 전송할 수 있습니다.
...
_rs`signer_seeds`_ 는 PDA를 유도하는 데 사용되는 선택적 시드와 범프 시드를
지정합니다.
...
_rs`_ctx: Context<Delete>`_ 가 _rs`ctx: Context<Delete>`_ 로 변경되어 함수
본문에서 컨텍스트를 사용합니다.
...
_rs`transfer()`_ 함수는 System Program의 전송 명령어를 호출하며 다음을
전달합니다:

- `cpi_context` (프로그램, 계정, PDA 서명자)
- 전송할 금액 (금고 계정의 전체 잔액)
...
포함시킵니다. 이를 위해 금고 PDA를 유도하고 업데이트 및 삭제 명령어 호출에
포함시켜야 합니다.
