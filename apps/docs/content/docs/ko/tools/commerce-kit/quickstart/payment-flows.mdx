...
이 함수들은 Solana와 상호작용하여 트랜잭션을 검증하고 확인을 기다립니다. `@solana/kit` 라이브러리의 Solana RPC 클라이언트가 필요합니다.
...
- **`rpc`** (`Rpc<SolanaRpcApi>`, 필수) - `@solana/kit`의 RPC 클라이언트.
  `createSolanaRpc(rpcUrl)`로 생성합니다.
