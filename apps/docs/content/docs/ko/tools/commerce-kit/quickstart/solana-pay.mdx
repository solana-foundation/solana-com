...
인코딩/파싱, 커스텀 스타일링이 적용된 QR 코드 생성, 그리고 SOL 및 SPL 토큰 전송을 위한 트랜잭션
구성을 처리합니다.
...
- **`recipient`** (`Address`, 필수) - 결제를 수신할 판매자의 지갑 주소
  (base58로 인코딩된 공개 키)입니다. `@solana/kit`의 문자열 또는
  `Address` 타입을 사용할 수 있습니다.
...
결제 전송을 위한 완전한 Solana 트랜잭션 메시지를 빌드합니다. 이
함수는 `splToken` 파라미터를 기반으로 SOL 또는 SPL 토큰 전송 중 어느 것을 생성할지 자동으로 감지하며,
필요한 모든 명령어를 구성합니다.
이 함수는 RPC 클라이언트의 최신 블록해시를 사용하여 트랜잭션의 블록해시 수명을 설정하고,
서명 및 RPC 제출에 바로 사용 가능한 완전한 미서명 트랜잭션을 반환합니다 (타입:
`TransactionMessageWithBlockhashLifetime`, Solana Kit과 호환).

#### 매개변수

- **`rpc`** (`Rpc<SolanaRpcApi>`) - `@solana/kit`의 Solana RPC 클라이언트입니다. `createSolanaRpc(rpcUrl)`로
  생성하세요.
