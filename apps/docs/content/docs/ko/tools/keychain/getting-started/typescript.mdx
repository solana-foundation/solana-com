---
title: TypeScript
description: TypeScript 애플리케이션에서 @solana/keychain 설치 및 사용
---

## 설치

필요에 따라 통합 패키지 또는 개별 서명자를 설치하세요:
...
### 통합 팩토리 (여러 서명자를 지원하려는 앱에 권장)

`createKeychainSigner`를 구분된 설정과 함께 사용하여 모든 백엔드를 생성하세요:
...
또는 더 작은 의존성을 위해 개별 서명자 패키지를 설치하세요:
...
모든 서명자는 `SolanaSigner` 인터페이스를 구현하며, 이는 `@solana/kit` 및
`@solana/signers`와 호환됩니다:
...
### Kit 클라이언트와 함께 사용하기 (keychainSigner 플러그인)

`@solana/keychain-kit-plugin`는 키체인 서명자를
[Kit 클라이언트](https://github.com/anza-xyz/kit)에 직접 설치하여, `payer` 및
`identity`로 자동 연결하므로 위에 나온 수동 `pipe` 설정을 생략할 수 있습니다:
...
이 플러그인은 `createKeychainSigner`와 동일한 백엔드 태그 설정을 지원하며,
실제로 디스패치되는 백엔드만 번들에 포함됩니다(백엔드 패키지는 동적 `import()`를
통해 로드됩니다).

`keychainPayer` 또는 `keychainIdentity`를 사용하여 단일 역할만 설정하고, 하나의
클라이언트에서 여러 백엔드를 혼합할 수 있습니다:
...
이미 `SolanaSigner`가 있으신가요? 모든 키체인 서명자는 유효한 Kit
`TransactionSigner`이므로,
[`@solana/kit-plugin-signer`](https://github.com/anza-xyz/kit-plugins/tree/main/packages/kit-plugin-signer)를
사용하여 기존 서명자를 설치할 수 있습니다(`pnpm add @solana/kit-plugin-signer`):
...
Fordefi에는 두 가지 서명 모드가 있으며, `chain` 전달 여부에 따라 선택됩니다. API 요청은 ECDSA P-256 요청 서명으로 인증됩니다 — 서명 키를 `privateKeyPem`으로 제공하거나, `requestSigner`를 전달하여 KMS/HSM에 보관하세요.

**네이티브 Solana 모드** (온체인 사용에 권장): `chain`을 설정하면 Fordefi가 직접 트랜잭션에 서명하고 브로드캐스트합니다. Fordefi는 서명 전에 블록해시나 수수료를 교체할 수 있으므로, 서명자는 Kit의 `TransactionSendingSigner`입니다 — 위에 표시된 부분 서명자 흐름이 아닌 `signAndSendTransactionMessageWithSigners`와 함께 사용하세요.
...
**블랙 박스 모드**: `chain`을 생략합니다. Fordefi는 원시 바이트에 서명하며 브로드캐스트하지 않습니다. 트랜잭션을 직접 조합하고 제출해야 합니다. Fordefi 블랙 박스 볼트와 함께 사용하세요.
...
`SolanaSigner` 인터페이스는 완전한 호환성을 위해 `@solana/signers` 타입을
확장합니다:
...
`SolanaSigner` 인터페이스를 구현하여 커스텀 서명자를 생성합니다:
...
추가 키 관리 서비스를 통합하려면
[서명자 추가 가이드](/docs/tools/keychain/adding-signers)를 참조하세요.
