...
메인 README의 서명자 목록에 서비스를 추가하세요.

## 제출 체크리스트

- [ ] 서명자가 `solana-keychain` 크레이트에서 지원됨
- [ ] Cargo.toml의 `solana-keychain` 의존성을 최신 버전으로 업데이트함
- [ ] 서명자에 대한 구성 구조체 추가됨
- [ ] `SignerTypeConfig` 변형 추가됨
- [ ] `build_signer_from_config`에 빌드 로직 추가됨
- [ ] `validate_individual_signer_config`에 유효성 검사 로직 추가됨
- [ ] `mod.rs`에 구성 구조체 내보내기 완료
- [ ] (선택 사항) `config_mock.rs`에 테스트 모의 빌더 메서드 추가됨
- [ ] 경고 없이 코드 컴파일됨
- [ ] 모든 테스트 통과 (`make test` 및 `make test-integration`)
- [ ] 다음을 따르는 문서 추가됨
      [`ADDING_SIGNERS.md`](https://github.com/solana-foundation/solana-keychain/blob/main/docs/ADDING_SIGNERS.md)
- [ ] 예제 구성 파일 생성됨 (`.toml` 및 `.env.example`)
- [ ] 하드코딩된 값 또는 시크릿 없음
- [ ] 오류 메시지가 명확함
- [ ] Rust 네이밍 규칙 준수 (snake_case)
- [ ] 린팅 통과 (`make lint`)
- [ ] 통합 테스트를 위한 API 키를 Kora 팀에 전달
...
- **서명자 구현 관련**:
  [`solana-keychain`](https://github.com/solana-foundation/solana-keychain)
  저장소에 이슈를 등록하세요
- **Kora 통합 관련**: 설계 논의를 위해 Kora 저장소에 이슈를 등록하세요
- 커뮤니티 채널에 참여하세요
