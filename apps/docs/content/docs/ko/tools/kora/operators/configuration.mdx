...
> _참고: `fallback_if_unavailable`이 true로 설정된 경우, Redis를 일시적으로 사용할 수 없을 때
> 시스템이 트랜잭션을 계속 진행할 수 있도록 허용하여 서비스 중단을 방지합니다.
> `max_transactions`를 0으로 설정하면 트랜잭션 수에 제한이 없습니다._
...
| `get_payer_signer`          | 다음 수수료 지불자 및 결제 주소 반환        | ✅       | boolean |
...
[Solana 문서](https://solana.com/developers/cookbook/transactions/durable-nonces)에서
확인하세요.
...
| 확장 기능 이름               | 설명                                      |
| ---------------------------- | ------------------------------------------------ |
| `confidential_transfer_mint` | 민트에 대한 기밀 잔액 구성 |
...
| 확장 기능 이름                  | 설명                                 |
| ------------------------------- | ------------------------------------------- |
| `confidential_transfer_account` | 계정의 기밀 잔액 상태 |
