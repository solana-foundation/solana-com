...
title: JS API 레퍼런스
description:
  "@solana/surfpool 패키지의 전체 공개 API — Surfnet 클래스, 설정 타입, 배포 옵션 및 이벤트 형태."
---
...
<Callout type="info" title="Kit 진입점">
  아래 심볼들은 `@solana/surfpool`의 루트 내보내기입니다. `@solana/surfpool/kit` 서브패스는 `surfpool()`, `surfnetCheatcodes()`, `createSurfnetCheatcodesRpc()`, `DEFAULT_SURFNET_ENDPOINT`, 플러그인 설정 타입, 그리고 생성된 치트코드 와이어 타입을 추가하며 — 이 모든 것은 이 표가 아닌 [Kit 플러그인](/docs/tools/surfpool/sdk/kit-plugin)에 문서화되어 있습니다.
</Callout>
