...
계정 관리, 잔액 조회 및 계정 상태 설정을 위한 메서드입니다.
...
계정의 SOL 잔액을 가져옵니다. 계정이 존재하지 않으면 `null`을 반환합니다.
...
<Callout type="info">
  트랜잭션을 실행하지 않으므로 대량 계정 설정 시 `airdrop`보다 `setAccount`가 더 빠릅니다.
</Callout>
