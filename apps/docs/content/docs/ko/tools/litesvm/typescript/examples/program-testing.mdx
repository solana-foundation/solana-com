---
title: 프로그램 테스트
description: LiteSVM으로 커스텀 Solana 프로그램 로드 및 테스트
---
...
[Codama JS Renderer](https://github.com/codama-idl/renderers-js)를 사용하여 프로그램 클라이언트를 생성하는 것을 권장합니다.
...
1. **프로그램 로드**: `addProgramFromFile`을 사용하여 컴파일된 프로그램을 로드하고
   클라이언트에서 myProgramPlugin()을 임포트합니다
2. **설정**: 필요에 따라 sysvar, 빌트인, 프리컴파일을 활성화합니다
3. **계정 설정**: `setAccount`로 데이터 계정을 미리 채웁니다
4. **전송**: `client.myProgram.testInstruction({..}).sendTransaction()`을 사용합니다 —
   수동 트랜잭션 빌드가 필요 없습니다
5. **검증**: 실행 후 계정 상태를 확인합니다
