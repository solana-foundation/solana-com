---
title: 설정
description: TypeScript에서 LiteSVM을 설정하는 빌더 메서드
---

LiteSVM은 설정을 위해 빌더 패턴을 사용합니다. 모든 메서드는 체이닝을 위해 `LiteSVM`을 반환합니다.
...
<Callout type="info">
  서명 검증을 비활성화하면 테스트 속도가 빨라지지만 현실성이 떨어집니다.
</Callout>
...
Sysvar 계정(Clock, Rent, EpochSchedule 등)을 활성화합니다.
...
특정 Solana 버전 테스트를 위한 기능 세트를 설정합니다.
