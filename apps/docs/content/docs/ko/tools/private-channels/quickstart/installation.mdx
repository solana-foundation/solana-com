...
애플리케이션에서 이 로컬 경로를 통해 가져오세요. npm에는
`@private-channels/client` 패키지가 없습니다. 이 섹션의 코드 예시는
상대 경로(`../private-channel-escrow-program/...`)를 사용하며, 이는 코드가
복제된 저장소와 같은 위치에 있다고 가정합니다. 실제 프로젝트에서
