...
- [`executable`](mention:executable): 계정에 실행 가능한 바이너리가 포함되어
  있는지 여부
...
바이트 시퀀스를 포함할 수 있으므로 임의적인 것으로 간주됩니다. 각 프로그램은 이
...
2. 계정을 소유한 프로그램이 정의한 대로 계정의 `data` 필드를 원시 바이트에서
   적절한 데이터 구조로 역직렬화하기
...
계정의 소유자는 프로그램의 명령어에 따라 계정의 `data`를 변경하거나 lamport를
...
않습니다. 대신 `data`는 프로그램 코드를 포함하는 별도의 **program data
...
프로그램은 데이터 계정을 사용하여 상태를 유지합니다. 이를 위해 먼저 새로운
데이터 계정을 생성해야 합니다. 프로그램 상태 계정을 생성하는 과정은 종종
추상화되어 있지만, 기본 프로세스를 이해하는 것이 도움이 됩니다.
...
System Program에 의해 생성된 후 모든 계정에 새로운 소유자가 할당되는 것은
아닙니다. System Program이 소유한 계정을 시스템 계정이라고 합니다. 모든 지갑
계정은 시스템 계정이며, 이를 통해 [거래 수수료](/docs/core/fees)를 지불할 수
있습니다.
...
아래 예제에서는 새로운 keypair가 생성되고 SOL로 자금이 공급됩니다. 코드를 실행한
후 계정의 `owner` 주소가 `11111111111111111111111111111111`
([System Program](/docs/core/programs#the-system-program))인 것을 확인할 수
있습니다.
