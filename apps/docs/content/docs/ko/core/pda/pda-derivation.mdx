...
bump seed는 파생 과정에서 선택적 seed에 추가되는 단일 바이트(u8)입니다.
[`find_program_address`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L800)는
255부터 1까지 내림차순으로 검색하며, 각 값으로 *rs`create_program_address`*를
호출하여 결과가 Ed25519 곡선에서 벗어날 때까지 반복합니다. 성공하는 첫 번째 값이
**정규 bump**입니다.
...
1. 시드의 수가 _rs`MAX_SEEDS`_ (16)를 초과하지 않고 개별 시드가
   ...
   하나라도 실패하면 _rs`PubkeyError::MaxSeedLengthExceeded`_ 를 반환합니다.
2. 모든 시드, 프로그램 ID, 그리고 문자열 `"ProgramDerivedAddress"`를 함께
   SHA-256 해시하여 32바이트 결과를 생성합니다.
3. 결과가 Ed25519 곡선의 유효한 점인지 확인합니다.
4. 결과가 곡선 위에 있으면 _rs`PubkeyError::InvalidSeeds`_ 를 반환합니다(주소에
   해당하는 개인 키가 있어 PDA 보안 속성을 위반함).
5. 결과가 곡선 위에 없으면 이를 PDA로 반환합니다.

## 컴퓨트 유닛 비용

*rs`create_program_address`*에 대한
[온체인 시스템 호출](https://github.com/anza-xyz/agave/blob/v3.1.8/syscalls/src/lib.rs#L798-L834)은
호출당
...
PDA 도출은 주소만 계산합니다. 해당 주소에 온체인 계정을 생성하지는 않습니다.
계정은 별도의 명령어(일반적으로 CPI를 통한 _rs`create_account`_)를 통해
명시적으로 생성되어야 합니다.
...
<Callout type="warn">
  이는 `sol_try_find_program_address`에 대해 문서화된 현재 정규 검색 동작과
  다릅니다. 현재 동작은 255부터 1까지 내림차순으로 반복합니다.
</Callout>
...
<Callout type="info">
  ...
  함수가 내보내지지 않아 Kit 예제는 포함되지 않았습니다.
</Callout>
...
이 예제에서 bump 255는 곡선 상의 주소를 생성하여 실패합니다. 첫 번째 유효한
