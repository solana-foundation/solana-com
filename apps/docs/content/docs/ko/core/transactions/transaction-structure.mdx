...
배열의 첫 번째 서명은 **수수료 납부자**에 해당하며, 이 계정이 트랜잭션의
[기본 수수료 및 우선순위 수수료](/docs/core/fees/fee-structure#base-fee)를
지불합니다. 이 첫 번째 서명은 네트워크에서 트랜잭션을 조회하는 데 사용되는
**트랜잭션 ID** 역할도 합니다. 트랜잭션 ID는 일반적으로 **트랜잭션 서명**이라고
불립니다.

수수료 납부자 요건:

- 메시지에서 첫 번째 계정(인덱스 0)이어야 하며 서명자여야 합니다.
...
- `rent_exempt_minimum + total_fee`를 충당하기에 충분한 lamport를 보유해야
  합니다. 그렇지 않으면 트랜잭션이 *rs`InsufficientFundsForFee`*로 실패합니다.
...
필드는 공개 키의 압축 인코딩 배열입니다. 각 항목은 트랜잭션 명령 중 하나
이상에서 사용되는 계정을 식별합니다. 이 배열은 모든 계정을 포함해야 하며 다음의
엄격한 순서를 따라야 합니다:
...
<Callout>
  ...
  있는 세 가지 카운트와 결합하여, 계정별 메타데이터 플래그를 저장하지 않고도 각
...
`recent_blockhash` 필드는 두 가지 목적을 지닌 32바이트 해시입니다:
...
유효하지 않으면,
[지속형 논스 트랜잭션](/docs/core/transactions/durable-nonces)이 아닌 경우
_rs`BlockhashNotFound`_ 오류와 함께 거부됩니다.
...
계정 키, 명령어)에는 컴팩트-u16 길이 인코딩이 앞에 붙습니다. 이 형식은 0-127
값에는 1바이트를, 더 큰 값에는 2-3바이트를 사용합니다.
...
| 필드               | 크기                     | 설명                      |
| ------------------ | ------------------------ | ------------------------- |
| `program_id_index` | 1바이트                  | 계정 키의 인덱스          |
| `num_accounts`     | 1~3바이트(compact-u16)   | 계정 인덱스 수            |
| `account_indices`  | `num_accounts` x 1바이트 | 계정 키 인덱스            |
| `data_len`         | 1~3바이트(compact-u16)   | instruction data의 길이   |
| `data`             | `data_len`바이트         | 불투명한 instruction data |

### 크기 계산

_rs`PACKET_DATA_SIZE`_ = 1,232바이트라고 할 때, 사용 가능한 공간은 다음과 같이
계산할 수 있습니다:
...
아래 다이어그램은 사용자가 네트워크와 상호작용할 수 있도록 트랜잭션과 명령어가
어떻게 함께 작동하는지 보여줍니다. 이 예시에서는 SOL이 한 계정에서 다른 계정으로
전송됩니다.
...
변경되어야 하므로, 두 계정 모두 쓰기 가능 상태여야 합니다. 이 명령을 실행하려면
발신자의 지갑이 SOL 전송 명령이 포함된 메시지와 서명이 담긴 트랜잭션을
전송합니다.

![SOL 전송 다이어그램](/assets/docs/core/transactions/sol-transfer.svg)

트랜잭션이 전송된 후, System Program은 전송 명령을 처리하고 두 계정의 lamport
잔액을 업데이트합니다.

![SOL 전송 처리 다이어그램](/assets/docs/core/transactions/sol-transfer-process.svg)

<Callout type="warn" title="SOL을 보내기 전에 수신자를 확인하세요">

System Program 전송은 **모든** 계정에 lamport를 추가합니다. 수신자가 SOL을 다시
꺼낼 수 있는지에 대한 프로토콜 수준의 확인은 없습니다. lamport는 해당 계정을
소유한 프로그램만이 이동시킬 수 있으므로, 귀하가 제어하지 않는
[token mint](/docs/tokens/basics/create-mint), 프로그램 또는
[PDA](/docs/core/pda)로 SOL을 전송하면 **자금의 영구적인 손실 위험**이 있습니다
— 소유 프로그램이 지정한 권한만이 이를 반환할 수 있습니다.
[token account](/docs/tokens/basics/create-token-account)로 전송된 SOL은 해당
계정의 소유자만 회수할 수 있으며, 발신자는 회수할 수 없습니다.

SPL **토큰** 전송은 부분적으로 자체 보호 기능을 갖추고 있습니다. Token Program은
계정이 예상된 mint와 일치하지 않는 전송을 거부합니다. 네이티브 **SOL** 전송에는
이러한 보호 장치가 없으므로, 발신자는 서명하기 전에 수신자를 반드시 확인해야
합니다. 전체 분류 로직은
[주소 확인](/docs/payments/send-payments/verify-address)을 참고하세요.
...
SOL 전송은 어떤 계정으로도 성공할 수 있으므로, 서명하기 전에 수신자를
