...
title: 프로덕션 환경에서의 서명
description:
  프로덕션 Solana 애플리케이션에서 서명 키가 저장되는 위치, 그리고 로컬 키,
  브라우저 지갑, 클라우드 KMS/HSM, 관리형 서명 백엔드 중 선택하는 방법.
type: conceptual
prerequisites:
...
<Callout type="warn">
  클라이언트 측 코드에 개인 키를 절대 삽입하거나, 프론트엔드 번들에 포함하거나,
  소스 컨트롤에 커밋하지 마세요. 키를 획득한 누구든 해당 계정의 자산을 완전히
  제어할 수 있습니다.
</Callout>
...
방식입니다. 모든 프로덕션 방식은 키를 전용 인프라에 보관하고, 해당 인프라에
서명을 위임합니다.
...
Foundation은 [Keychain](/docs/tools/keychain)을 유지 관리하고 있으며, 이는 위
...
<Cards>
  <Card title="Keychain" href="/docs/tools/keychain">
    로컬 키, KMS, HSM, 매니지드 월렛 백엔드를 아우르는 통합 서명 솔루션.
  </Card>
  <Card title="백엔드 선택하기" href="/docs/tools/keychain/choosing-a-backend">
    커스터디 모델을 비교하고 애플리케이션에 맞는 서명 백엔드를 선택하세요.
  </Card>
</Cards>

키 관리, RPC 보안, 모니터링 등 전체 프로덕션 체크리스트는
