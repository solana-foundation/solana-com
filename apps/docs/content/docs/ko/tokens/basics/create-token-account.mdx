...
ATA를 생성하려면 계정의 온체인 스토리지에 필요한 환불 가능한 최소 잔액과 트랜잭션 수수료가 필요합니다. 생성 명령어에 전달되는 `payer`는 스토리지 잔액을 지원하며 서명이 필요합니다. 지불자는 트랜잭션 수수료 지불자 및 ATA를 소유할 지갑과 다를 수 있으므로, 애플리케이션이 수신자를 대신하여 ATA를 생성할 수 있습니다.

현재 클러스터 스토리지 요율 기준 기본 비용은 다음과 같습니다:

| Token program                             | ATA 크기  | 최소 스토리지 잔액             |
| ----------------------------------------- | --------- | ----------------------------------- |
| Original Token Program                    | 165바이트 | 2,039,280 lamports (0.00203928 SOL) |
| 자동 불변 소유자가 포함된 Token-2022 | 170바이트 | 2,074,080 lamports (0.00207408 SOL) |

Token-2022 민트는 자동 _rs`ImmutableOwner`_ 확장 외에 추가적인 계정 확장을 요구할 수 있습니다. 해당 ATA는 더 크며 더 높은 최소 잔액이 필요합니다. Associated Token Program은 선택된 Token Program에 필요한 계정 크기를 요청하고, 그에 맞게 ATA에 자금을 지원합니다.

이러한 잔액을 애플리케이션 로직에 하드코딩하지 마십시오. 필요한 데이터 크기에 대해
[`getMinimumBalanceForRentExemption`](/docs/rpc/http/getminimumbalanceforrentexemption)
을 조회하거나, Associated Token Program이 생성 시 자동으로 계산하고 자금을 지원하도록 하십시오.

<Callout type="info" title="스토리지 잔액은 환불 가능합니다">
  빈 token account를 닫으면 최소 스토리지 잔액을 포함한 모든 lamports가 닫기 명령어에 지정된 대상으로 전송됩니다. 최초 생성 지불자가 자동으로 환불 대상이 되지는 않습니다. [Token Account 닫기](/docs/tokens/basics/close-account)를 참조하십시오.
</Callout>
