...
토큰 계정은 솔라나에서 하나의 민트와 하나의 토큰 계정 소유자를 위한 토큰을
저장합니다.
...
여기서 *rs`owner`*는 token account에서 토큰을 전송, 소각 또는 위임할 수 있는
권한을 의미합니다. 계정의 프로그램 소유자는 여전히 Token Program 또는 Token
Extensions Program입니다.

각 token account는 정확히 하나의 민트에 연결되어 있으며, 이는 token account가
token account의 _rs`mint`_ 필드로 식별되는 하나의 토큰만 보유할 수 있음을
의미합니다.

## 연결된 토큰 계정이란

associated token account (ATA)는 지갑과 민트의 기본 토큰 계정입니다.
...
Associated Token Program에 의해 생성된 토큰 계정만 associated token account라고
합니다.

Associated Token Program은 표준적이고 결정론적인 주소에 token account를 생성하는
방법입니다. 생성된 계정은 여전히 Token Program 또는 Token Extensions Program이
소유하는 token account이며, Associated Token Program이 소유하지 않습니다.
...
Associated Token Program은 해당 주소에 표준 token account를 생성하며, 생성된
계정은 여전히 Token Program 또는 Token Extensions Program에서 정의한
_rs`Account`_ 타입을 사용합니다.

## Associated Token Account 생성 방법

Associated Token Account 생성은 Associated Token Program의 _rs`Create`_ 또는
_rs`CreateIdempotent`_ 명령어를 사용합니다. Associated Token Program은 ATA
주소를 파생하고, ATA 주소에 계정을 생성한 다음, Token Program 또는 Token
Extension Program이 소유한 token account로 계정을 초기화합니다.

<Callout type="info" title="권장사항">
  대부분의 애플리케이션에서는 System Program과 Token Program 명령어를 직접
  호출하여 token account를 생성하는 대신, Associated Token Program을 통해
  생성하는 것이 좋습니다. associated token account는 소유자, 토큰 프로그램,
  민트로부터 파생된 결정론적 주소를 사용하므로, 지갑과 애플리케이션이 특정
  민트의 기본 token account를 더 쉽게 찾을 수 있습니다.
</Callout>
...
| 항목                                          | 설명                                                                                                                                                                                                                     | 소스                                                                                                                          |
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- |
| _rs`Create`_                                  | 파생된 ATA 주소에 associated token account를 생성합니다.                                                                                                                                                                 | [소스](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L20-L29) |
| _rs`CreateIdempotent`_                        | associated token account를 생성하되, 동일한 소유자와 민트에 대해 해당 ATA가 이미 존재하는 경우에도 성공합니다.                                                                                                           | [소스](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | ATA 주소를 파생하고 *rs`create_pda_account`*와 선택한 토큰 프로그램으로의 CPI를 사용하여 token account를 초기화합니다. 선택한 프로그램이 Token Extension Program인 경우, ATA 프로세서는 불변 소유자 확장도 초기화합니다. | [소스](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | *rs`process_create_associated_token_account`*가 System Program으로의 CPI를 통해 PDA 계정을 생성하는 데 사용하는 헬퍼입니다.                                                                                              | [소스](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |
...
token account를 생성하려면 두 개의 명령어가 필요합니다:
...
2. Token Program의 _rs`InitializeAccount`_, _rs`InitializeAccount2`_, 또는
   _rs`InitializeAccount3`_ 명령어는 민트와 소유자를 위해 새 계정을
   초기화합니다.

_rs`CreateAccount`_ 명령어와 token account 초기화 명령어를 동일한 트랜잭션에
포함하세요.

token account 초기화 중에 Token Program은 계정이 아직 초기화되지 않았고 rent
면제 상태인지 확인합니다.

<Callout type="info">
  아래 섹션은 System Program과 Token Program 명령어를 직접 호출하여 token
  account를 생성하는 방법을 보여줍니다. 대부분의 애플리케이션에서는 Associated
  Token Program을 사용하세요. associated token account를 사용하지 않을 특정한
  이유가 있거나, 자체 Solana 프로그램에서 System Program과 Token Program
  명령어로 CPI를 수행하여 커스텀 PDA token account를 생성해야 하는 경우에만 직접
  System Program과 Token Program 호출을 사용하세요.
</Callout>
...
| --------------------------------- | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Account`_                     | 모든 token account에 저장되는 기본 token account 필드입니다.                                             | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`InitializeAccount`_           | 계정 목록에 소유자와 rent sysvar 계정을 필요로 하는 token account 초기화 명령어입니다.                   | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L45-L63)   | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L75-L93)   |
| _rs`InitializeAccount2`_          | 계정 목록 대신 instruction data로 소유자를 전달하는 token account 초기화 명령어입니다.                   | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L352-L365) | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L419-L433) |
| _rs`InitializeAccount3`_          | instruction data로 소유자를 전달하고 rent sysvar 계정이 필요하지 않은 token account 초기화 명령어입니다. | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L377-L387) | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L445-L456) |
| _rs`_process_initialize_account`_ | token account 초기화를 위한 공유 프로세서 로직입니다.                                                    | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L83-L141)      | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L153-L234)     |
| _rs`process_initialize_account`_  | *rs`InitializeAccount`*를 위한 공개 핸들러입니다.                                                        | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L143-L150)     | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L236-L240)     |
| _rs`process_initialize_account2`_ | *rs`InitializeAccount2`*를 위한 공개 핸들러입니다.                                                       | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L152-L160)     | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L242-L246)     |
| _rs`process_initialize_account3`_ | *rs`InitializeAccount3`*를 위한 공개 핸들러입니다.                                                       | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L162-L170)     | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L248-L252)     |

### Typescript

아래 예제는 `@solana/kit`를 사용하는 권장 접근 방식을 보여줍니다. 참고용으로
`@solana/web3.js`를 사용하는 레거시 예제도 포함되어 있습니다.
