...
<Callout type="info">
  소스 코드에서 개발자들은 token account를 `Account` 타입으로 호출합니다. [Token
...
- Circle은 token account 소유자 권한을 통해 토큰을 제어합니다:
...
생성된 token account는 Token Program이 소유하며 표준 `Account` 구조를
사용합니다.
...
{/_ CODE_PLACEHOLDER_b8c8fc7d327b4ea8b09da98b4a88ad01 _/
