...
description: SPL Token account를 생성하는 방법을 알아보세요.
---

## Token account란 무엇인가요?

Token account는 특정 토큰의 잔액을 저장합니다. 각 token account는 정확히 하나의
mint와 연결되어 있으며 토큰 잔액과 추가 세부 정보를 추적합니다.
...
<Callout type="info">
  소스 코드에서 개발자들은 token account를 `Account` 타입이라고 부릅니다. [Token
  ...
  모두 token account에 대해 동일한 기본 구현을 공유합니다.
</Callout>

토큰을 보유하려면 해당 특정 mint에 대한 token account가 필요합니다. 각 token
account는 다음을 추적합니다:

1. **Mint**: 보유하고 있는 특정 토큰 유형
2. **Owner**: 이 계정에서 토큰을 전송할 수 있는 권한

### 예시: USD Coin (USDC)

- Mint 주소: `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Circle의 token account: `3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
- 이 계정은 USDC 토큰만 보유합니다
- Circle은 token account owner 권한을 통해 토큰을 제어합니다:
  `7VHUFJHWu2CuExkJcJrzhQPJ2oygupTWkL2A2For4BmE`

이 token account의 세부 정보는
...
"Owner"라는 용어는 두 가지 다른 맥락에서 사용될 수 있습니다:

1. **Token account owner**: 토큰을 전송, 소각 또는 위임할 수 있는 권한입니다.
   이는 token account의 데이터에 저장됩니다.

2. **Program owner**: 계정을 소유한 프로그램입니다 (token account의 경우 항상
   Token Program 또는 Token Extension Program입니다).

Token account를 다룰 때 "owner"는 일반적으로 계정을 소유한 프로그램이 아니라
토큰을 전송할 수 있는 권한을 의미합니다.
...
<Callout type="info">
  Associated Token Program에 의해 생성된 토큰 계정만 "associated token
  account"라고 부릅니다.
</Callout>

ATA는 결정론적 주소를 사용하므로 주어진 민트에 대한 지갑의 토큰 계정을 쉽게 찾을
수 있습니다.
...
모든 지갑과 토큰 조합에 대해 정확히 하나의 ATA 주소가 존재합니다. 이를 통해 토큰
계정 주소를 수동으로 추적할 필요가 없어지며, 항상 올바른 주소를 파생할 수
있습니다.

<Callout type="info">
  **Associated Token Program의 작동 방식**

Associated Token Program은 다음과 같은 헬퍼입니다:

- 결정론적 주소(PDA)에 토큰 계정을 생성합니다
- Token Program에 대한 크로스 프로그램 호출(CPI)을 수행합니다
- 자체적으로 상태를 유지하지 않습니다

생성된 토큰 계정은 Token Program이 소유하며 표준 `Account` 구조를 사용합니다.
...
토큰 계정을 생성하려면 두 가지 명령어가 필요합니다:

1. System Program: 토큰 계정을 위한 공간이 할당된 계정을 생성하고 소유권을 Token
   Program으로 이전합니다
2. Token Program: 토큰 계정 데이터를 초기화합니다
...
이 단일 명령어는 크로스 프로그램 호출(CPI)을 통해 token account 생성과 초기화를
자동으로 처리합니다.
