...
title: 계정 동결
description:
  민트의 동결 권한을 사용하여 token account를 동결하여 전송, 소각 및 위임 변경을
  차단합니다.
...
## Token Account 동결은 무엇을 하나요?

token account를 동결하면 동일한 소유자, 민트 및 잔액이 유지되지만, token
account가 해제될 때까지 토큰 전송 또는 소각이 차단됩니다.

동결된 계정의 토큰 잔액이 0인 경우에도 계정을 닫을 수 있습니다.

동결 권한이 여전히 있는 민트만 token account를 동결할 수 있습니다. 민트의 동결
권한이 취소되면 해당 민트의 계정은 더 이상 동결될 수 없습니다.

## Token Account를 동결하는 방법

token account 동결은 Token Program의 _rs`FreezeAccount`_ 명령어를 사용합니다.

_rs`FreezeAccount`_ 명령어는 token account가 민트에 속하는지, 서명자가 민트의
동결 권한인지 확인한 후 token account 상태를 초기화됨에서 동결됨으로 변경합니다.

[네이티브 token account](/docs/tokens/basics/sync-native)는 동결을 지원하지
않습니다.
...
| ----------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Mint`_                          | 민트 상태는 token account를 동결하는 데 사용되는 선택적 동결 권한을 저장합니다. | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | _rs`Initialized`_ 및 *rs`Frozen`*을 포함한 token account 상태입니다.            | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | token account를 동결된 상태로 변경하는 명령어입니다.                            | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
| _rs`process_toggle_freeze_account`_ | token account를 동결하고 해제하기 위한 공유 프로세서 로직입니다.                | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |
