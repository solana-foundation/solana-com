...
명령은 특정 토큰 계정에서 모든 토큰 전송 또는 토큰 소각을 방지합니다. 동결되면
해당 계정은 해제될 때까지 토큰을 보내거나 받을 수 없습니다. 동결된 계정은 토큰
잔액이 0일 경우 여전히 닫을 수 있습니다. 토큰 민트의 동결 권한자만 계정을 동결할
수 있습니다. 민트 계정에서 동결 권한이 취소되면(null로 설정), 토큰은 절대 동결될
수 없습니다.
