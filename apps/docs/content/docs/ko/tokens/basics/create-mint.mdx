...
title: 토큰 민트 생성하기
description:
  토큰 민트를 생성하고 민트의 소수점 자릿수, 민트 권한, 동결 권한을 설정합니다.
...
mint account는 Solana에서 토큰을 정의하고 고유하게 식별하며, 해당 민트에 대한
모든 token account에 적용되는 공유 상태를 저장합니다.

Token Program은 _rs`Mint`_ 계정 유형을 다음과 같이 정의합니다:
...
모든 토큰은 하나의 mint account를 가지며, 민트 주소는 지갑, 애플리케이션,
익스플로러 전반에 걸쳐 토큰의 고유 식별자입니다.

예를 들어, USD Coin (USDC)은 `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`라는
민트 주소를 가집니다. 이 민트 주소는 Solana 생태계 전반에서 USDC를 고유하게
식별합니다. 이 민트는
...
민트를 생성하려면 두 가지 인스트럭션이 필요합니다:

1. System Program의 _rs`CreateAccount`_ 인스트럭션은 새로운 rent-exempt 계정을
   생성하고 Token Program을 새 계정의 프로그램 소유자로 할당합니다.
2. Token Program의 _rs`InitializeMint`_ 또는 _rs`InitializeMint2`_ 인스트럭션은
   새 계정을 민트로 초기화합니다.

_rs`CreateAccount`_ 인스트럭션과 민트 초기화 인스트럭션을 동일한 트랜잭션에
포함시키세요.

민트 초기화 과정에서 Token Program은 mint account가 아직 초기화되지 않았으며
rent-exempt 상태인지 확인합니다. 그런 다음 Token Program은 민트 권한, 동결 권한,
...
| ------------------------------ | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Mint`_                     | 모든 mint account에 저장되는 기본 민트 상태입니다.                         | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`InitializeMint`_           | 계정 목록에 rent sysvar 계정을 요구하는 민트 초기화 인스트럭션입니다.      | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L24-L44)   | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L50-L74)   |
| _rs`InitializeMint2`_          | 계정 목록에 rent sysvar 계정을 요구하지 않는 민트 초기화 인스트럭션입니다. | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L401-L414) | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L471-L486) |
| _rs`_process_initialize_mint`_ | 민트 초기화를 위한 공유 프로세서 로직입니다.                               | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
| _rs`process_initialize_mint`_  | *rs`InitializeMint`*에 대한 공개 핸들러입니다.                             | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L62-L70)       | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L132-L140)     |
| _rs`process_initialize_mint2`_ | *rs`InitializeMint2`*에 대한 공개 핸들러입니다.                            | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L72-L81)       | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L142-L151)     |

### Typescript

아래 `Kit` 예제는 `@solana/kit`을 사용하는 권장 접근 방식을 보여줍니다.
참고용으로 `@solana/web3.js`을 사용하는 레거시 예제도 포함되어 있습니다.
...
mint account는 기본적인 토큰 정보(공급량, 소수점 자릿수, 권한)만 저장합니다.
토큰에 이름, 심볼, 이미지와 같은 사람이 읽을 수 있는 메타데이터를 추가하려면 두
가지 옵션이 있습니다:

<Cards>
  <Card title="Metaplex 토큰 메타데이터" href="/docs/tokens/metaplex">
   Metaplex 토큰 메타데이터 프로그램을 사용하여 메타데이터를 추가합니다. 기존 Token Program과 Token-2022 모두에서 작동합니다.
  </Card>
  ...
  <Card title="토큰 확장 메타데이터" href="/docs/tokens/extensions/metadata">
   ...
   기존 Token Program으로 생성된 mint account에 메타데이터를 추가하려면 Metaplex 토큰 메타데이터 프로그램을 사용하세요.
  </Card>
</Cards>
