...
예를 들어, USD Coin(USDC)은 민트 주소
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`를 가지고 있습니다. 이 주소는
Solana 생태계 전체에서 USDC를 고유하게 식별합니다. 이 민트는
...
명령어가 필요합니다.
[구현 세부 사항은 여기에서](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/processor.rs#L27)
확인하세요.
...
민트 계정은 기본적인 토큰 정보(공급량, 소수점 자릿수, 권한)만 저장합니다. 토큰에
...
<Cards>
  ...
  <Card title="토큰 확장 메타데이터" href="/docs/tokens/extensions/metadata">
   Token-2022의 내장 메타데이터 확장 기능을 사용합니다. 
   메타데이터 확장은 Token-2022로 생성된 민트 계정에서만 사용할 수 있습니다.
   기존 Token Program으로 생성된 민트 계정에 메타데이터를 추가하려면 Metaplex 토큰 메타데이터 프로그램을 사용하세요.
  </Card>
</Cards>
