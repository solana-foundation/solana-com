...
title: 대리인 승인하기
description:
  토큰 계정에서 제한된 수량의 토큰을 전송하거나 소각할 수 있도록 다른 주소에
  권한을 부여합니다.
...
위임자 승인은 토큰 계정 소유자를 대신하여 다른 주소가 token account에서 제한된
수량의 토큰을 전송하거나 소각할 수 있도록 허용합니다.

token account는 한 번에 하나의 현재 위임자와 하나의 위임된 수량을 저장합니다.
새로운 위임자를 승인하면 계정의 이전 위임자와 허용량이 교체됩니다.

## 위임자 승인 방법

위임자 승인은 Token Program의 _rs`Approve`_ 또는 _rs`ApproveChecked`_ 명령어를
사용합니다.

아래 예제는 *rs`ApproveChecked`*를 사용하며, 이는 승인을 처리하기 전에 예상되는
민트와 토큰 정밀도를 확인할 수 있도록 호출자가 민트와 소수점 자릿수를 제공하도록
요구합니다.
...
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Account`_         | 토큰 계정 상태는 소유자, 위임자 및 위임된 수량을 저장합니다.                                                                      | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | 호출자가 민트나 소수점 자릿수를 제공하지 않고도 token account에 대한 위임자와 허용량을 기록하는 위임자 승인 명령어입니다.         | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | 호출자가 민트와 소수점 자릿수를 제공하도록 요구하고, 위임자와 허용량을 기록하기 전에 해당 값을 확인하는 위임자 승인 명령어입니다. | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
| _rs`process_approve`_ | 위임자 승인을 위한 공유 프로세서 로직입니다.                                                                                      | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L344-L392)     | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L582-L640)     |
