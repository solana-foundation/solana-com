...
명령어는 다른 계정(위임자)에게 귀하의 토큰 계정에서 특정 수량의 토큰을 전송할 수
있는 권한을 부여합니다. 위임자는 승인된 수량까지 토큰을 전송할 수 있지만, 이를
초과할 수는 없습니다. 토큰 계정 소유자만 위임자를 승인할 수 있으며, 각 토큰
계정은 한 번에 하나의 위임자만 가질 수 있습니다. 새로운 승인은 이전 승인을
덮어씁니다.
