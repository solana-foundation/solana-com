---
title: 토큰 발행
description: 토큰의 새로운 단위를 발행하는 방법을 알아보세요.
---
...
- 발행 권한자만 새로운 토큰을 발행할 수 있습니다
- 발행된 토큰을 받을 대상 토큰 계정이 존재해야 합니다
...
async fn main() -> Result<()> {
    ...
    // 수수료 지불자를 위한 새 keypair 생성
    let fee_payer = Keypair::new();

    // 수수료 지불자에게 1 SOL 에어드롭
    ...
    // mint 주소로 사용할 keypair 생성
    let mint = Keypair::new();

    // 기본 mint 계정 크기(바이트 단위) 가져오기, 확장 기능 비활성화
    let mint_space = Mint::LEN;
    let mint_rent = client
        .get_minimum_balance_for_rent_exemption(mint_space)
        .await?;

    // mint를 위한 새 계정 생성 명령어(token program)
    ...
    // fee_payer의 associated token account 주소 계산
    ...
    // associated token account 생성 명령어
