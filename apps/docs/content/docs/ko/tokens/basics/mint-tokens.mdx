...
- mint authority만 새로운 토큰을 발행할 수 있습니다
- 발행된 토큰을 받을 대상 token account가 존재해야 합니다
...
<Callout type="info">
  ...
  은 동일한 기능을 구현하기 위해 유사한 구현을 공유합니다.
</Callout>
...
async fn main() -> Result<()> {
    ...
    // fee payer를 위한 새로운 keypair 생성
    let fee_payer = Keypair::new();

    // fee payer에게 1 SOL 에어드롭
    ...
    // mint의 주소로 사용할 keypair 생성
    let mint = Keypair::new();

    // 기본 mint account 크기(바이트 단위) 가져오기, 확장 기능 비활성화
    ...
    // 발행할 토큰 수량(소수점 2자리로 100개 토큰)
    let amount = 100;

    // associated token account로 토큰을 발행하기 위한 mint_to 명령어 생성
    ...
    println!("associated token account로 1.00개의 토큰을 발행했습니다");
    println!("\nMint 주소: {}", mint.pubkey());
    println!("{:#?}", mint_data);

    println!(
        "\nAssociated Token Account 주소: {}",
        associated_token_address
    );
    println!("{:#?}", token_data);

    println!("트랜잭션 서명: {}", transaction_signature);
...
async def main():
    ...
    # 발행할 수량 (최소 단위)
    amount_to_mint = 1000000000  # 소수점 9자리를 가진 1 토큰
    ...
    async with rpc:
        ...
        print(f"발행: {mint_address}")
        print(f"대상: {destination_token_account}")
        print(f"수량: {amount_to_mint}")
        print(f"발행 권한: {mint_authority.pubkey()}")
        print(f"지불자: {payer.pubkey()}")
        print(f"발행 트랜잭션이 성공적으로 생성되었습니다")
