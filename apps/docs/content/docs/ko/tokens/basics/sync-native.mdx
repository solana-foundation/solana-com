...
래핑된 SOL(WSOL)은 계정의 lamports 양을 토큰 잔액으로 추적하는 토큰 계정입니다.
WSOL은 DeFi 프로토콜과의 통합을 통해 SOL을 SPL 토큰으로 전송할 수 있게 합니다.
...
명령어는 래핑된 SOL(WSOL) 토큰 계정 잔액을 실제 저장된 SOL(램포트)과
동기화합니다. 네이티브 SOL을 WSOL 토큰 계정으로 전송할 때 토큰 잔액이 자동으로
업데이트되지 않습니다. 올바른 WSOL 잔액을 반영하려면 SyncNative를 호출해야
합니다.
