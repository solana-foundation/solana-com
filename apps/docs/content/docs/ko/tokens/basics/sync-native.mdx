...
title: 네이티브 동기화
description:
  계정으로 SOL을 전송한 후 래핑된 SOL token account 금액을 업데이트합니다.
...
래핑된 SOL(WSOL)은 Token Program의 네이티브 민트를 위해 token account에 보유된
SOL입니다. 네이티브 민트는 Token Program이 SOL을 래핑하는 token account에
사용하는 민트 주소입니다. 네이티브 민트를 사용하면 Token Program의 명령어를 통해
SOL을 사용할 수 있습니다.
...
account의 _rs`amount`_ 필드를 통해 래핑된 SOL token account에 보유된 SOL을 토큰
잔액으로 추적합니다. 래핑된 SOL token account로 SOL을 전송하면 lamport 잔액은
즉시 증가하지만, Token Program의 _rs`SyncNative`_ 명령어를 호출할 때까지 토큰
...
필드를 렌트 면제 예비금을 초과하여 계정에 보유된 lamport와 일치하도록
업데이트합니다.
...
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Account`_             | token account 상태는 *rs`is_native`*를 사용하여 계정을 WSOL로 표시하고, 네이티브 예비금, 계정이 유지해야 하는 렌트 면제 최소 lamport 잔액 및 추적된 토큰 금액을 저장합니다. | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`SyncNative`_          | 래핑된 SOL token account의 _rs`amount`_ 필드를 렌트 면제 예비금을 초과하여 계정에 보유된 lamport와 일치하도록 업데이트하는 명령어입니다.                                    | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| _rs`process_sync_native`_ | 래핑된 SOL 잔액을 동기화하기 위한 공유 프로세서 로직입니다.                                                                                                                 | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |
