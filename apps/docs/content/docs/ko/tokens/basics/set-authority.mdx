...
title: 권한 설정
description:
  민트 또는 토큰 계정의 민트, 동결, 소유자 및 닫기 권한을 포함한 권한 역할을
  변경하거나 취소합니다.
...
권한 설정은 민트 또는 토큰 계정의 특정 권한 역할을 변경하거나 취소합니다.

민트 계정은 민트 권한과 동결 권한을 저장할 수 있습니다. 토큰 계정은 소유자를
저장하며 선택적으로 닫기 권한을 저장할 수 있습니다. 각 역할은 특정 권한 집합을
제어하며, 권한을 *rs`None`*로 설정하면 선택한 권한 역할이 영구적으로 제거됩니다.

Token Extensions Program은 특정 확장 기능에 대한 추가 권한 유형도 정의합니다.

## 권한을 설정하는 방법

권한 변경은 Token Program의 _rs`SetAuthority`_ 명령어를 사용합니다.

각 호출은 하나의 민트 또는 토큰 계정에 대한 하나의 권한 유형을 업데이트합니다.
현재 권한이 변경에 서명하며, 새 권한은 다른 주소이거나 선택한 권한을 영구적으로
취소하기 위한 *rs`None`*일 수 있습니다.

### 소스 참조

| 항목                        | 설명                                                                    | Token Program                                                                                                | Token Extensions Program                                                                                             |
| --------------------------- | ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                  | 민트 상태는 *rs`mint_authority`*와 *rs`freeze_authority`*를 저장합니다. | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)           |
| _rs`Account`_               | 토큰 계정 상태는 *rs`owner`*와 *rs`close_authority`*를 저장합니다.      | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)          |
...
| _rs`SetAuthority`_          | 민트 또는 토큰 계정의 하나의 권한을 변경하거나 취소하는 명령어입니다.   | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L141-L158) | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L183-L200)   |
