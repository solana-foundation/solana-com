---
title: 권한 설정
description: 민트 또는 토큰 계정의 권한을 설정하는 방법을 알아보세요.
---
...
- **민트 권한**: 새로운 토큰 생성을 제어합니다. 모든 토큰 계정에 토큰을 발행할
  수 있습니다. 초기 공급량 생성 후 고정 공급량 토큰을 만들기 위해 종종
  취소됩니다.
- **동결 권한**: 토큰 계정을 동결하고 해제하는 기능을 제어합니다. 모든 토큰
  계정의 토큰 전송을 방지할 수 있습니다. 사용자의 토큰이 동결될 수 없음을
  보장하기 위해 종종 취소됩니다.

### 토큰 계정 권한

- **계정 소유자**: 토큰 계정에 대한 완전한 제어권을 가집니다. 토큰을 전송하고,
  소각하고, 위임자를 승인하며, 잔액이 0일 때 계정을 닫을 수 있습니다.
- **닫기 권한**: 잔액이 0일 때 토큰 계정을 닫을 수 있습니다. 기본적으로 계정
  소유자이지만 다른 계정에 위임할 수 있습니다.

## 권한 설정 방법

`SetAuthority` 명령어는 민트 및 토큰 계정의 권한을 변경하거나 취소합니다. 현재
권한 소유자만 권한을 새 주소로 이전하거나 권한을 null로 설정하여 영구적으로
취소할 수 있습니다. 일단 취소되면 권한을 복원할 수 없습니다.
