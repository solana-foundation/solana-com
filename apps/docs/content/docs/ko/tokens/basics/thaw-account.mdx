...
해제는 토큰 계정을 동결된 상태에서 초기화된 상태로 되돌립니다.

해제가 성공한 후에는 계정이 다시 토큰을 전송, 소각 및 닫을 수 있습니다. 민트의
동결 권한만 토큰 계정을 해제할 수 있습니다.

## 토큰 계정을 해제하는 방법

토큰 계정 해제는 Token Program의 _rs`ThawAccount`_ 명령을 사용합니다.

_rs`ThawAccount`_ 명령은 토큰 계정이 민트에 속하는지, 서명자가 민트의 동결
권한인지 확인한 다음, 토큰 계정 상태를 동결된 상태에서 초기화된 상태로
변경합니다.
...
| ----------------------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Mint`_                          | 민트 상태는 토큰 계정 해제에 사용되는 선택적 동결 권한을 저장합니다. | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
...
| _rs`ThawAccount`_                   | 동결된 토큰 계정을 초기화된 상태로 되돌리는 명령입니다.              | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L229-L243) | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L292-L306) |
