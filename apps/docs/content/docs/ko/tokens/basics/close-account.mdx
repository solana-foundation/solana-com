...
계정을 닫으려면 토큰 잔액이 0이어야 합니다. 동결된 계정도 토큰 잔액이 0이면 닫을
수 있습니다. 래핑된 SOL 토큰 계정은 예외로, 기본 SOL을 회수하기 위해 토큰 잔액이
있어도 닫을 수 있습니다. 계정 소유자 또는 계정의 닫기 권한이 닫기 명령에
서명합니다.
...
계정을 초기화하며, 소스 계정을 삭제합니다. Token Extension Program은 특정 확장
기능에 대해 추가적인 닫기 검사를 적용하지만, 기본 닫기 흐름은 동일합니다.

### 소스 참조

| 항목                        | 설명                                                                                  | Token Program                                                                                                | Token Extension Program                                                                                            |
| --------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Account`_               | 토큰 계정 상태는 계정 닫기 시 사용되는 잔액, 네이티브 상태 및 닫기 권한을 저장합니다. | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
