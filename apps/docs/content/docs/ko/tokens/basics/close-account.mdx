...
title: 토큰 계정 닫기
description:
  토큰 계정을 닫고 계정의 lamport를 목적지 계정으로 반환합니다.
...
token account를 닫으면 해당 계정이 삭제되고, 환불 가능한 최소 저장 잔액을 포함한 모든 lamport가 대상 계정으로 전송됩니다.

계정을 닫으려면 토큰 잔액이 0이어야 합니다. 동결된 계정이라도 token balance가 0인 경우에는 닫을 수 있습니다. Wrapped SOL token account는 예외로, 기본 SOL을 회수하기 위해 토큰 잔액이 있는 상태에서도 닫을 수 있습니다. 계정 소유자 또는 계정의 close authority가 닫기 명령에 서명하고 대상을 선택합니다. ATA를 생성하기 위해 비용을 지불한 계정이 복구된 잔액을 자동으로 받지는 않습니다.

## 토큰 계정을 닫는 방법

토큰 계정 닫기는 Token Program의 _rs`CloseAccount`_ 명령을 사용합니다.

_rs`CloseAccount`_ 명령은 소스 계정의 lamport를 목적지 계정으로 전송하고, 소스
...
| _rs`CloseAccount`_          | 토큰 계정을 닫고 토큰 계정의 lamport를 목적지 계정으로 전송하는 명령입니다. | [소스](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
