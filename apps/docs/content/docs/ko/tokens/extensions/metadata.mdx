...
description:
  MetadataPointer 및 TokenMetadata 확장을 사용하여 민트 계정에 직접 메타데이터를 저장하는 민트를 생성한 다음, token-metadata 인터페이스를 통해 해당 메타데이터를 업데이트하고 제거하며 잠그는 방법을 다룹니다.
...
아래의 `Kit` 예제는 생성된 명령어를 직접 사용합니다. 참고용으로 `@solana/web3.js`, `@solana/spl-token`, `@solana/spl-token-metadata`를 사용하는 레거시 예제도 포함되어 있습니다. 레거시 예제는 web3.js v1이 [v1 트랜잭션](/docs/core/transactions/versioned-transactions#v1-message-format)을 역직렬화할 수 없기 때문에 `maxSupportedTransactionVersion`을 `0`으로 제한합니다.
