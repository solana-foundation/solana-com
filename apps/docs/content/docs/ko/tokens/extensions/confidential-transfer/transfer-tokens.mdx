...
발신자와 수신자 모두 `ConfidentialTransferAccount` 상태로 구성되고 기밀 전송이
승인된 token account를 보유해야 합니다. 발신자의 token account에는 전송할 수
있는 기밀 잔액도 있어야 합니다.
...
4. 증명 컨텍스트 상태 계정들을 닫아 생성에 사용된 SOL을 회수합니다.
...
- `confidential_transfer_create_context_state_account` 메서드는 증명 계정을
  생성합니다.
- `confidential_transfer_transfer` 메서드는 `Transfer` 명령을 호출합니다.
