---
title: 대기 잔액 적용
description: 자금을 사용 가능하게 만들기 위해 대기 잔액을 적용하는 방법을 알아보세요.
---
...
Confidential Balances는 메인넷과 devnet에서 활성화된 ZK ElGamal Proof 프로그램에 의존합니다. 기본 `solana-test-validator`는 이를 활성화하지 않지만, [Surfpool](https://surfpool.run)과 같은 메인넷 포킹 로컬 validator는 활성화합니다.
자금이 충전된 payer로 해당 환경 중 하나(코드는 devnet 사용)에서 예제를 실행하고, 플레이스홀더 민트 및 계정 주소를 본인의 것으로 교체하세요.
