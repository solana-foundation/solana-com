...
수 있습니다. 이는 프라이버시를 보호하는 거래에 유용합니다. 전송 금액과 토큰
잔액만 비공개로 유지됩니다. token account 주소는 공개 상태로 유지됩니다.
...
- [Confidential Balances Cookbook](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Confidential Transfer 확장 기능 사용 방법에 대한 코드 예제

### 작동 방식

Confidential Transfer 확장 기능은 Token Extension program에
...
1. Confidential Transfer 확장 기능이 포함된 mint account를 생성합니다.
2. 송신자와 수신자를 위한 Confidential Transfer 확장 기능이 포함된 token
   account를 생성합니다.
3. 송신자 계정에 토큰을 발행합니다.
...
<Cards>
  ...
  >
    Confidential Transfer 확장 기능이 포함된 mint account를 생성하는 방법
  </Card>
  <Card
...
| 명령어                              | 설명                                                                                                                                         |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | 기밀 전송을 위해 mint account를 설정합니다. 이 명령어는 _rs`TokenInstruction::InitializeMint`_ 명령어와 동일한 트랜잭션에 포함되어야 합니다. |
| _rs`UpdateMint`_                    | mint의 기밀 전송 설정을 업데이트합니다.                                                                                                      |
| _rs`ConfigureAccount`_              | 기밀 전송을 위해 token account를 설정합니다.                                                                                                 |
| _rs`ApproveAccount`_                | mint가 새 token account에 대한 승인을 요구하는 경우 기밀 전송을 위해 token account를 승인합니다.                                             |
