...
title: 기밀 잔액
description:
  기밀 잔액 익스텐션과 이를 사용하여 token mint 및 계정에 선택적 기능을 추가하는 방법에 대해 알아보세요.
---
...
기밀 잔액 권한은 익스텐션을 초기화할 때 감사자 키를 설정하거나 나중에 _rs`UpdateMint`_를 사용하여 변경할 수 있습니다. 권한은 키를 교체하거나 삭제할 수 있습니다. 키 교체는 이후의 전송에만 적용되므로, 과거 감사 접근이 필요한 경우 이전 비밀 키를 보관해야 합니다. 감사자 키는 읽기 접근 권한을 제공하며, 토큰 전송을 승인하거나 계정의 전체 기밀 잔액을 공개하지 않습니다.
...
| 명령어                         | 설명                                                                                                                                                       |
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | mint account에서 기밀 잔액을 활성화합니다. 이 명령어는 _rs`TokenInstruction::InitializeMint`_ 명령어와 동일한 트랜잭션에 포함되어야 합니다. |
| _rs`UpdateMint`_                    | mint의 계정 승인 정책 및 선택적 글로벌 감사자 ElGamal 공개 키를 업데이트합니다.                                                                        |
| _rs`ConfigureAccount`_              | token account에서 기밀 잔액을 활성화합니다.                                                                                                                 |
| _rs`ApproveAccount`_                | mint가 새 token account에 대한 승인을 요구하는 경우 기밀 잔액에 대해 token account를 승인합니다.                                                          |
...
| _rs`DisableNonConfidentialCredits`_ | 일반 전송을 차단하여 계정이 기밀 전송만 받을 수 있도록 합니다.                                                                                     |
| _rs`TransferWithFee`_               | token account 간에 수수료와 함께 기밀로 토큰을 전송합니다.                                                                                                |
| _rs`ConfigureAccountWithRegistry`_  | _rs`VerifyPubkeyValidity`_ 증명 대신 _rs`ElGamalRegistry`_ 계정을 사용하여 기밀 잔액을 위한 token account를 구성하는 대안적인 방법입니다.         |
