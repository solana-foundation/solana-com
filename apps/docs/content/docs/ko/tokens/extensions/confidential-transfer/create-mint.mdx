...
title: 토큰 민트 생성
description:
  Confidential Balances가 활성화된 토큰 mint를 생성하는 방법을 알아보세요.
---

## Confidential Balances로 mint를 생성하는 방법

Confidential Balances는 mint account에 추가 상태를 더해 프라이빗 토큰 전송을 가능하게 합니다. 이 섹션에서는 필요한 확장이 활성화된 토큰 mint를 생성하는 방법을 설명합니다.

다음 다이어그램은 Confidential Balances 확장으로 mint를 생성하는 데 관련된 단계를 보여줍니다:
...
- **authority**: mint에 대한 Confidential Balances 설정을 변경하고, 자동 승인이 비활성화된 경우 새로운 기밀 계정을 승인할 권한을 가진 계정입니다.

- **auto_approve_new_accounts**: true로 설정하면 사용자는 기본적으로 Confidential Balances가 활성화된 token account를 생성할 수 있습니다. false로 설정하면, 기밀 전송에 사용되기 전에 authority가 각각의 새로운 token account를 승인해야 합니다.
...
Confidential Balances가 활성화된 mint를 생성하려면 단일 트랜잭션에서 세 가지 명령어가 필요합니다:

1. **Mint Account 생성**: System Program의 _rs`CreateAccount`_ 명령어를 호출하여
   mint account를 생성합니다.

2. **Confidential Balances 초기화**: Token Extension Program의
...
다음 코드는 Confidential Balances가 활성화된 mint를 생성하는 방법을 보여줍니다.
...
자금이 충전된 payer와 함께 해당 환경 중 하나(코드는 데브넷을 사용)에서 예제를 실행하고, 플레이스홀더 mint 및 계정 주소를 본인의 것으로 교체하세요.
