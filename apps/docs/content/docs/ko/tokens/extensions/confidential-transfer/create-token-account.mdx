...
title: 토큰 계정 생성
description:
  Confidential Balances가 활성화된 token account를 생성하는 방법을 알아보세요.
---

## Confidential Balances로 token account를 생성하는 방법

Confidential Balances는 token account에 추가 상태를 더함으로써 프라이빗 토큰 전송을 가능하게 합니다. 이 섹션에서는 필요한 익스텐션이 활성화된 token account를 생성하는 방법을 설명합니다.

다음 다이어그램은 Confidential Balances가 활성화된 token account를 생성하는 데 관련된 단계를 보여줍니다:
...
### Confidential Balances Token Account 상태
...
_rs`ConfidentialTransferAccount`_에는 Confidential Balances를 관리하는 여러 필드가 포함되어 있습니다:

- **approved**: Confidential Balances에 대한 계정의 승인 상태입니다. mint account의 `auto_approve_new_accounts` 설정이 `true`로 지정된 경우, 모든 token account는 Confidential Balances에 대해 자동으로 승인됩니다.
...
Confidential Balances를 위한 token account 생성 및 구성에는 다음 명령어들이 사용되며, 모두 단일 트랜잭션에 포함됩니다:
...
4. **Confidential Balances 구성**: Token Extension Program의
   ...
   명령어를 호출하고, _rs`ProofLocation::ContextStateAccount`_를 통해 proof context state account를 참조하여
   _rs`ConfidentialTransferAccount`_ 상태를 초기화합니다.

<Callout type="info">

**token account에서 Confidential Balances를 활성화할 수 있는 것은 token account 소유자뿐입니다**.

_rs`ConfigureAccount`_ 명령어는 token account 소유자만 생성할 수 있는 암호화 키와 증명의 클라이언트 측 생성이 필요합니다.

pubkey 유효성 증명은 계정의 ElGamal 공개 키가 유효한지 검증합니다. 이는 _rs`build_pubkey_validity_proof_data`_로 생성되고, ZK ElGamal Proof 프로그램에 의해 체인 상에서 context state account로 검증된 후, _rs`ProofLocation::ContextStateAccount`_를 통해 _rs`ConfigureAccount`_에서 참조됩니다. 따라서 토큰 명령어 자체에는 증명 바이트가 포함되지 않습니다. 구현 세부 사항은 다음을 참조하세요:
...
다음 코드는 associated token account를 생성하고, 기존의 호환 가능한 mint와 함께 Confidential Balances를 사용하도록 구성합니다.

Confidential Balances는 ZK ElGamal Proof 프로그램에 의존하며, 이는 메인넷과 데브넷에서 활성화되어 있습니다. 기본 `solana-test-validator`는 이를 활성화하지 않지만, [Surfpool](https://surfpool.run)과 같은 메인넷 포킹 로컬 validator는 활성화합니다.
코드에서는 데브넷을 사용하며, 자금이 충분한 payer로 해당 환경 중 하나에서 예제를 실행하고, mint 플레이스홀더를 [Create a Mint](/docs/tokens/extensions/confidential-transfer/create-mint)에 따라 생성된 mint로 교체하세요.
