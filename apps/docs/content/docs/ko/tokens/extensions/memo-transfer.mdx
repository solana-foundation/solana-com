...
title: 메모 전송
description:
  Token Extensions Program 토큰 계정에서 MemoTransfer 계정 확장을 활성화하여
  수신 전송 시 메모를 포함하도록 요구합니다.
...
Token Extensions Program의 _rs`MemoTransfer`_ 계정 확장은 해당 토큰 계정으로의
수신 전송 직전에 메모 명령어를 요구합니다. 해당 토큰 계정에서의 송신 전송은
메모를 요구하지 않습니다.

이 확장은 민트가 아닌 목적지 토큰 계정에 구성됩니다. 활성화되면 토큰 계정
소유자가 요구 사항을 비활성화할 때까지 메모가 없는 전송은
*rs`TokenError::NoMemo`*로 실패합니다.

## MemoTransfer가 활성화된 토큰 계정 생성 방법

*rs`MemoTransfer`*가 활성화된 토큰 계정을 생성하려면:

1. 민트를 생성하고 초기화합니다.
2. *rs`MemoTransfer`*가 있는 토큰 계정에 필요한 토큰 계정 크기와 렌트를
   계산합니다.
3. 토큰 계정을 생성하고 해당 계정에서 *rs`MemoTransfer`*를 활성화합니다.
4. 해당 토큰 계정에 대한 _rs`Transfer`_ 및 _rs`TransferChecked`_ 명령어는 전송
   명령어 직전에 메모 명령어를 포함해야 합니다.

<ScrollyCoding>

## !!steps 민트 생성 및 초기화

토큰 계정이 사용할 민트를 생성하고 초기화합니다.
...
기본 토큰 계정에 _rs`MemoTransfer`_ 확장을 더한 토큰 계정 크기를 계산합니다.
...
_rs`MemoTransfer`_ 확장에 필요한 토큰 계정 크기를 사용하여 렌트를 계산합니다.
...
## !!steps 토큰 계정 생성 및 초기화

계산된 공간과 lamport로 토큰 계정을 생성한 다음, 민트에 대해 초기화합니다.
...
## !!steps MemoTransfer 활성화

토큰 계정에서 *rs`MemoTransfer`*를 활성화합니다.
...
| 항목                                            | 설명                                                                                         | 소스                                                                                                                                     |
| ----------------------------------------------- | -------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`MemoTransfer`_                              | 수신 전송에 메모가 필요한지 여부를 저장하는 계정 확장입니다.                                 | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/mod.rs#L17-L22)         |
| _rs`RequiredMemoTransfersInstruction::Enable`_  | 토큰 계정에서 메모 필수 전송을 활성화하는 명령입니다.                                        | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L50-L71) |
| _rs`RequiredMemoTransfersInstruction::Disable`_ | 토큰 계정에서 메모 필수 전송을 비활성화하는 명령입니다.                                      | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L74-L95) |
| _rs`process_toggle_required_memo_transfers`_    | 소유권을 확인한 후 _rs`MemoTransfer`_ 확장을 초기화하거나 다시 작성하는 프로세서 로직입니다. | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/memo_transfer/processor.rs#L20-L48)     |

### Typescript

아래의 `Kit` 예제는 생성된 명령을 직접 사용합니다. `@solana/web3.js`,
`@solana/spl-token`, `@solana/spl-memo`을 사용하는 레거시 예제는 참고용으로
포함되어 있습니다.
