...
title: 불변 소유자
description:
  Token Extensions Program 토큰 계정을 불변 소유권으로 초기화하여 소유자를
  영구히 재할당할 수 없도록 합니다.
...
Token Extensions Program의 _rs`ImmutableOwner`_ 계정 확장 기능은 토큰 계정의
소유자를 영구적으로 만듭니다.

토큰 계정이 *rs`ImmutableOwner`*로 초기화되면, 이후 _rs`SetAuthority`_ 명령으로
_rs`AccountOwner`_ 권한을 변경할 수 없습니다.

<Callout type="info" title="연결된 토큰 계정">
  Token Extensions Program을 위해 Associated Token Program을 통해 생성된
  associated token account는 토큰 계정에 _rs`ImmutableOwner`_를 자동으로
  초기화합니다.
</Callout>
...
1. 민트를 생성하고 초기화합니다.
2. *rs`ImmutableOwner`*를 위한 충분한 공간으로 토큰 계정을 생성합니다.
3. _rs`InitializeAccount`_ 전에 토큰 계정에 *rs`ImmutableOwner`*를 초기화합니다.
4. 나중에 *rs`SetAuthority`*로 계정 소유자를 변경하려고 하면
   _rs`TokenError::ImmutableOwner`_ 오류가 발생합니다.

<ScrollyCoding>

## !!steps 토큰 계정 크기 계산

기본 토큰 계정에 _rs`ImmutableOwner`_ 확장을 더한 토큰 계정 크기를 계산합니다.
...
_rs`ImmutableOwner`_ 확장에 필요한 토큰 계정 크기를 사용하여 rent를 계산합니다.
...
## !!steps ImmutableOwner 초기화

토큰 계정에서 _rs`ImmutableOwner`_ 확장을 초기화합니다.
...
동일한 트랜잭션에서 *rs`InitializeAccount`*를 사용하여 토큰 계정을 초기화합니다.
...
<Callout type="info" title="명령어 순서">
  _rs`InitializeImmutableOwner`_는 _rs`InitializeAccount`_보다 먼저 실행되어야
  합니다. _rs`CreateAccount`_, _rs`InitializeImmutableOwner`_, 그리고
  _rs`InitializeAccount`_는 동일한 트랜잭션에 포함되어야 합니다.
</Callout>
...
| 항목                                     | 설명                                                                                              | 소스                                                                                                                           |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| _rs`ImmutableOwner`_                     | 토큰 계정 소유자를 불변으로 표시하는 계정 확장입니다.                                             | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L13-L16) |
| _rs`InitializeImmutableOwner`_           | 초기화되지 않은 토큰 계정에서 불변 소유자 확장을 초기화하는 명령어입니다.                         | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1982-L1991)           |
| _rs`process_initialize_immutable_owner`_ | 계정에 *rs`ImmutableOwner`*를 할당하고 초기화하는 프로세서 로직입니다.                            | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1532-L1543)               |
| _rs`SetAuthority`_                       | 토큰 계정 소유자를 포함한 권한을 변경하는 데 사용되는 기본 토큰 명령어입니다.                     | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1611-L1633)           |
| _rs`process_set_authority`_              | 토큰 계정에 *rs`ImmutableOwner`*가 활성화되어 있을 때 소유자 변경을 거부하는 프로세서 로직입니다. | [소스](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L703-L900)                 |
