...
一个 token
account 一次只能存储一个当前委托和一个委托额度。批准新委托将替换该账户上先前的委托和额度。
...
以下示例使用
_rs`ApproveChecked`_,该指令要求调用者提供铸币地址和小数位数,以便指令在处理批准之前验证预期的铸币地址和代币精度。

token account 所有者签署批准。

### 源代码参考

| 项目                  | 描述                                                                                   | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------- | -------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_         | token account 状态存储所有者、委托和委托金额。                                         | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | 一个委托批准指令,无需调用者提供铸币地址或小数位数即可记录 token account 的委托和额度。 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
...
| _rs`process_approve`_ | 用于委托批准的共享处理器逻辑。                                                         | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L344-L392)     | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L582-L640)     |

### Typescript

以下 `Kit` 示例展示了使用 `@solana/kit` 的推荐方法。同时也包含了使用
