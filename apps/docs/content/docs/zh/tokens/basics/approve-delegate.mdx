---
title: 批准委托
description: 授权另一个地址从代币账户转移或销毁有限数量的代币。
...
批准委托允许另一个地址代表代币账户所有者从 token
account 转移或销毁有限数量的代币。

一个 token
account 一次只能存储一个当前委托和一个委托金额。批准新委托将替换账户上的先前委托和额度。
...
以下示例使用
_rs`ApproveChecked`_,它要求调用者提供铸币地址和小数位数,以便指令在处理批准之前验证预期的铸币地址和代币精度。
...
| --------------------- | -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_         | 代币账户状态存储所有者、委托和委托金额。                                         | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | 委托批准指令,记录代币账户的委托和额度,无需调用者提供铸币地址或小数位数。         | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | 委托批准指令,要求调用者提供铸币地址和小数位数,并在记录委托和额度之前检查这些值。 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
