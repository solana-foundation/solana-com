---
title: 解冻账户
description: 解冻已冻结的代币账户，以便恢复转账、销毁和委托变更操作。
...
解冻操作将 token account 从冻结状态恢复到已初始化状态。

解冻成功后，该账户可以再次进行代币转账、销毁和关闭操作。只有铸币厂的冻结权限持有者才能解冻 token
account。

## 如何解冻代币账户

解冻 token account 使用 Token Program 的 _rs`ThawAccount`_ 指令。

_rs`ThawAccount`_ 指令会验证该 token
account 属于该铸币厂，并且签名者是铸币厂的冻结权限持有者，然后将 token
account 状态从冻结恢复到已初始化状态。

[原生代币账户](/docs/tokens/basics/sync-native)不支持解冻操作。

### 源代码参考

| 项目                                | 描述                                                         | Token Program                                                                                                  | Token Extension Program                                                                                              |
| ----------------------------------- | ------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | 铸币厂状态存储用于解冻 token account 的可选冻结权限。        | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | token account 状态，包括 _rs`Frozen`_ 和 _rs`Initialized`_。 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`ThawAccount`_                   | 将冻结的 token account 恢复到已初始化状态的指令。            | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L229-L243) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L292-L306) |
| _rs`process_toggle_freeze_account`_ | 用于冻结和解冻 token account 的共享处理器逻辑。              | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |
