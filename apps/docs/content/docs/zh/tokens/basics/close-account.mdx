...
description: 关闭代币账户并将账户的 lamport 返还给目标账户。
...
关闭代币账户会删除该账户,并将代币账户的租金 lamport 返还给目标账户。

在关闭账户之前,代币余额必须为零。如果冻结账户的代币余额为零,该冻结账户仍然可以关闭。封装的 SOL 代币账户是个例外,即使有代币余额也可以关闭,以回收底层的 SOL。账户所有者或账户的关闭授权者需要签署关闭指令。
...
_rs`CloseAccount`_
指令将源账户的 lamport 转移到目标账户,清空源账户并删除源账户。Token Extensions
Program 对某些扩展应用了额外的关闭检查,但基本的关闭流程是相同的。

### 源代码参考

| 项目                        | 描述                                                             | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------------- | ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | 代币账户状态存储了在账户关闭期间使用的余额、原生状态和关闭授权。 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | 一个关闭代币账户并将代币账户的 lamport 转移到目标账户的指令。    | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | 代币账户关闭的共享处理逻辑。                                     | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |

### Typescript

下面的 `Kit` 示例展示了使用 `@solana/kit` 的推荐方法。为了参考,也包含了使用
