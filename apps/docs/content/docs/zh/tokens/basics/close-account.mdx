...
关闭 token account 将删除该账户，并将其所有 lamports（包括可退还的最低存储余额）转移到目标账户。

关闭账户前，代币余额必须为零。如果冻结账户的代币余额为零，该账户仍可关闭。Wrapped SOL token account 是例外，可以在代币余额不为零的情况下关闭，以取回底层的 SOL。账户所有者或账户的关闭权限方签署关闭指令并选择目标账户。支付创建 associated token account 费用的账户不会自动收回已回收的余额。
