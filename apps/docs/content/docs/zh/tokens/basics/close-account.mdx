...
description: 关闭代币账户并将账户的 lamport 返还至目标账户。
...
关闭代币账户会删除该账户,并将代币账户的 rent lamport 返还至目标账户。

账户关闭前,代币余额必须为零。如果冻结账户的代币余额为零,该冻结账户仍可关闭。封装的 SOL 代币账户是个例外,即使有代币余额也可以关闭以回收底层 SOL。账户所有者或账户的关闭权限需要签署关闭指令。
...
Program 对某些扩展应用额外的关闭检查,但基本关闭流程相同。

### 源代码参考

| 项目                        | 描述                                                         | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------------- | ------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | 代币账户状态存储账户关闭期间使用的余额、原生状态和关闭权限。 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
