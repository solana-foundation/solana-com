...
Token Account 用于存储你某一特定代币的余额。每个 Token
Account 都只关联一个 mint，并记录你的代币余额及其他详细信息。
...
<Callout type="info">
  ...
  还是 [Token Extension
  Program](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/state.rs#L102-L123)，Token
  Account 的底层实现都是一致的。
</Callout>

要持有代币，你需要为该 mint 创建一个 Token Account。每个 Token Account 会记录：

1. **Mint**：它所持有的具体代币类型
2. **Owner**：有权从该账户转出代币的权限方
...
- 此账户仅持有 USDC 代币
- Circle 通过 Token Account 的 owner 权限控制代币：
...
上查看该 Token Account 的详细信息。

<Callout type="info">
  **理解“Owner”术语**

“Owner” 这个词在两种不同的语境下使用：

1. **Token Account Owner**：有权转账、销毁或委托代币的权限方。该信息存储在 Token
   Account 的数据中。
