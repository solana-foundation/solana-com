...
这里， _rs`owner`_ 指的是可以从 token
account 中转移、销毁或委托代币的权限。该账户的程序所有者仍然是 Token
Program 或 Token Extensions Program。

每个 token account 都与一个铸币严格绑定，这意味着该 token
account 只能持有一种代币的单位，即由 token account 的 _rs`mint`_
字段标识的代币。

## 什么是关联代币账户

associated token account（ATA）是钱包和铸币的默认 token
account。[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program/src)
从钱包地址、代币程序地址和铸币地址派生 ATA 地址。

只有由 Associated Token Program 创建的 token account 才被称为 associated token
account。

Associated Token Program 是一种在标准的、确定性地址创建 token
account 的方法。生成的账户仍然是由 Token Program 或 Token Extensions
Program 拥有的 token account，而不是由 Associated Token Program 拥有。

Associated Token Program 如
...
Program 或 Token Extensions Program 定义的 _rs`Account`_ 类型。

## 费用与付款方

创建 ATA 需要为账户的链上存储支付可退还的最低余额，以及交易费用。传递给创建指令的 `payer` 负责存储余额的资金，并且必须签名。付款方可以与交易费用付款方以及将拥有 ATA 的钱包不同，因此应用程序可以代表接收方创建 ATA。

按当前集群存储费率，基础费用如下：

| Token program                             | ATA 大小  | 最低存储余额             |
| ----------------------------------------- | --------- | ----------------------------------- |
| Original Token Program                    | 165 字节 | 2,039,280 lamports（0.00203928 SOL） |
| Token-2022（自动不可变所有者） | 170 字节 | 2,074,080 lamports（0.00207408 SOL） |

Token-2022 铸币账户除自动启用的 _rs`ImmutableOwner`_ 扩展外，还可能要求额外的账户扩展。这类 ATA 体积更大，所需的最低余额也更高。Associated Token Program 会向所选 Token Program 查询所需的账户大小，并相应地为 ATA 提供资金。

请勿在应用逻辑中硬编码这些余额。请使用
[`getMinimumBalanceForRentExemption`](/docs/rpc/http/getminimumbalanceforrentexemption)
查询所需的数据大小，或让 Associated Token Program 在创建时自动计算并提供资金。

<Callout type="info" title="存储余额可退还">
  关闭一个空的 token account 后，其所有 lamports（包括最低存储余额）将转移到关闭指令指定的目标地址。原始创建付款方并不会自动成为退款目标。请参阅 [Close Token Account](/docs/tokens/basics/close-account)。
</Callout>
...
| 项目                                          | 描述                                                                                                                                                                                                                                                    | 源代码                                                                                                                          |
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| _rs`Create`_                                  | 在推导出的 ATA 地址创建关联代币账户。                                                                                                                                                                                                | [源代码](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L20-L29) |
| _rs`CreateIdempotent`_                        | 创建关联代币账户，但如果同一所有者和铸币厂的 ATA 已存在，仍然会成功执行。                                                                                                                                               | [源代码](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | 推导 ATA 地址并使用 _rs`create_pda_account`_ 加上对所选代币程序的 CPI 来初始化代币账户。当所选程序为 Token Extensions Program 时，ATA 处理器还会初始化不可变所有者扩展。 | [源代码](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | _rs`process_create_associated_token_account`_ 使用的辅助函数，通过对系统程序的 CPI 来创建 PDA 账户。                                                                                                                                         | [源代码](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |
...
1. System Program 的 _rs`CreateAccount`_ 指令创建一个新的免 rent 账户,并将 Token
   Program 指定为新账户的程序所有者。
2. Token Program 的 _rs`InitializeAccount`_、 _rs`InitializeAccount2`_ 或
   _rs`InitializeAccount3`_ 指令为铸币和所有者初始化新账户。

在同一个交易中包含 _rs`CreateAccount`_ 指令和 token account 初始化指令。

在 token account 初始化期间,Token
Program 会检查该账户是否尚未初始化且已免 rent。

<Callout type="info">
  下面的部分展示了如何通过直接调用 System Program 和 Token Program 指令来创建
  token account。对于大多数应用程序,请使用 Associated Token Program
  代替。仅在您有特定原因不使用 associated token account 或需要通过从您自己的
  Solana 程序向 System Program 和 Token Program 指令发起 CPI 来创建自定义 PDA
  token account 时,才使用直接的 System Program 和 Token Program 调用。
</Callout>
...
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_                     | 存储在每个 token account 中的基本 token account 字段。                                                                       | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`InitializeAccount`_           | 一个 token account 初始化指令,期望在其账户列表中包含所有者和 rent sysvar 账户。                    | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L45-L63)   | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L75-L93)   |
| _rs`InitializeAccount2`_          | 一个 token account 初始化指令,在 instruction data 中传递所有者而非在账户列表中传递。                 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L352-L365) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L419-L433) |
| _rs`InitializeAccount3`_          | 一个 token account 初始化指令,在 instruction data 中传递所有者且不需要 rent sysvar 账户。 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L377-L387) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L445-L456) |
| _rs`_process_initialize_account`_ | token account 初始化的共享处理器逻辑。                                                                           | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L83-L141)      | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L153-L234)     |
| _rs`process_initialize_account`_  | _rs`InitializeAccount`_ 的公共处理器。                                                                                        | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L143-L150)     | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L236-L240)     |
| _rs`process_initialize_account2`_ | _rs`InitializeAccount2`_ 的公共处理器。                                                                                       | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L152-L160)     | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L242-L246)     |
| _rs`process_initialize_account3`_ | _rs`InitializeAccount3`_ 的公共处理器。                                                                                       | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L162-L170)     | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L248-L252)     |
