...
<Callout type="info">
  ...
  还是 [Token Extension
  Program](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/state.rs#L102-L123)，它们对于
  Token account 都采用了相同的基础实现。
</Callout>
...
- 此账户仅持有 USDC 代币
- Circle 通过 token account 的 owner authority 控制代币：
...
生成的 token account 由 Token Program 拥有，并采用标准的 `Account` 结构。
