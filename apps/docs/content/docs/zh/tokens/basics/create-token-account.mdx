...
在 Solana 上,一个 token account 为一个铸币和一个代币账户所有者存储代币。
...
这里,_rs`owner`_ 是指能够从 token
account 转移、销毁或委托代币的权限。该账户的程序所有者仍然是 Token
Program 或 Token Extensions Program。

每个 token account 都与一个铸币绑定,这意味着该 token
account 只能持有一种代币的单位,即由该 token account 的 _rs`mint`_
字段标识的代币。
...
对于任何钱包、代币程序和铸币组合,都只有一个 ATA 地址。Associated Token
Program 在该地址创建一个标准 token account,生成的账户仍然使用 Token
Program 或 Token Extensions Program 定义的 _rs`Account`_ 类型。

2. **Program Owner**：拥有该账户的程序（对于 Token Account 来说，始终是 Token
   Program 或 Token Extension Program）。

在使用 Token
Account 时，“owner” 通常指的是有权转移代币的权限方，而不是拥有账户的程序。
...
ATA 使用确定性地址，使得查找任意钱包下某个 mint 的 token
account 变得简单。请参见
[此处的推导实现](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/interface/src/address.rs#L56)。
...
对于任意钱包和代币的组合，都只有一个 ATA 地址。这无需手动记录 token
account 地址——你总能推导出正确的地址。

<Callout type="info">
  **Associated Token Program 的工作原理**

Associated Token Program 是一个辅助程序，主要功能包括：

- 在确定性地址（PDA）上创建 token account
- 向 Token Program 发起跨程序调用（CPI）
- 自身不维护任何状态

最终生成的 token account 由 Token Program 拥有，并采用标准 `Account` 结构。

</Callout>

## 如何创建 Token Account

创建 token account 需要使用
...
你需要两个指令来创建 token account：

1. System Program：创建一个为 token
   account 分配空间的账户，并将其所有权转移给 Token Program
...
创建 Associated Token Account (ATA) 需要使用
...
此单一指令会自动处理 token account 的创建，并通过 Cross Program Invocations
(CPI) 进行初始化。
