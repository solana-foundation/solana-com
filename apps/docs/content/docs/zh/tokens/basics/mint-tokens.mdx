---
title: 铸造 Token
description: 使用铸币权限向代币账户铸造新的代币单位。
...
铸造会创建现有代币的新单位,并增加铸币的总供应量。

只有铸币权限可以铸造新代币。目标 token
account 必须已经存在,并且必须属于同一个铸币。
...
以下示例使用
_rs`MintToChecked`_,该指令要求调用者提供铸币的小数位数,以便指令可以在铸造前验证预期的代币精度。

[原生铸币](/docs/tokens/basics/sync-native) 不支持铸造。

### 源代码参考

| 项目                  | 描述                                                                                      | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------- | ----------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_            | 铸币状态存储铸币权限和代币总供应量。                                                      | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
...
| _rs`MintTo`_          | 一个铸造指令,向现有 token account 铸造代币,而无需调用者提供铸币的小数位数。               | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L159-L177) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L202-L220) |
| _rs`MintToChecked`_   | 一个铸造指令,要求调用者提供铸币的小数位数,并在向现有 token account 铸造代币之前检查该值。 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L301-L325) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L367-L391) |
| _rs`process_mint_to`_ | 用于铸造代币的共享处理器逻辑。                                                            | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L521-L583)     | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L981-L1067)    |

### Typescript

以下 `Kit` 示例展示了使用 `@solana/kit` 的推荐方法。包含使用 `@solana/web3.js`
的旧版示例供参考。
