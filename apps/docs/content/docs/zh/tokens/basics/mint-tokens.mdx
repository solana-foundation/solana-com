...
- 只有 mint authority 可以铸造新代币
- 必须存在目标 token account 用于接收铸造的代币
...
async fn main() -> Result<()> {
    ...
    // 计算 fee_payer 的 associated token account 地址
    ...
    // 要铸造的 token 数量（100 个，精度为 2 位小数）
    let amount = 100;

    // 创建 mint_to 指令，将 token 铸造到 associated token account
    ...
    println!("已向 associated token account 铸造 1.00 个 token");
    println!("\nMint 地址: {}", mint.pubkey());
    println!("{:#?}", mint_data);
