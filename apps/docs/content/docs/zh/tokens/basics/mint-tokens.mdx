---
title: 铸造 Token
description: 使用铸造权限向代币账户铸造新的代币单位。
...
铸造会创建现有代币的新单位,并增加铸造账户的总供应量。

只有铸造权限才能铸造新代币。目标 token
account 必须已经存在,并且必须属于同一个铸造账户。

## 如何铸造代币

铸造代币使用 Token Program 的 _rs`MintTo`_ 或 _rs`MintToChecked`_ 指令。

下面的示例使用
_rs`MintToChecked`_,该指令要求调用者提供铸造账户的小数位数,以便指令在铸造前可以验证预期的代币精度。

[原生铸造](/docs/tokens/basics/sync-native)不支持铸造。

### 源代码参考

| 项目                  | 描述                                                                                          | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------- | --------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_            | 铸造状态存储铸造权限和代币总供应量。                                                          | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_         | token account 状态存储通过铸造更新的目标余额。                                                | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`MintTo`_          | 一个铸造指令,向现有 token account 铸造代币,无需调用者提供铸造账户的小数位数。                 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L159-L177) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L202-L220) |
| _rs`MintToChecked`_   | 一个铸造指令,要求调用者提供铸造账户的小数位数,并在向现有 token account 铸造代币之前检查该值。 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L301-L325) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L367-L391) |
| _rs`process_mint_to`_ | 铸造代币的共享处理器逻辑。                                                                    | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L521-L583)     | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L981-L1067)    |
