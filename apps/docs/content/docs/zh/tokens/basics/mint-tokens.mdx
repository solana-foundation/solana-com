...
- 只有铸造权限（mint authority）才能铸造新的 Token
- 必须存在一个目标 token account 用于接收铸造的 Token
...
async fn main() -> Result<()> {
    ...
    // 生成一个 keypair 作为 mint 的地址
    let mint = Keypair::new();

    // 获取默认 mint account 的大小（字节），未启用扩展
    let mint_space = Mint::LEN;
    let mint_rent = client
        .get_minimum_balance_for_rent_exemption(mint_space)
        .await?;

    // 创建 mint（token program）新账户的指令
    let create_account_instruction = create_account(
        &fee_payer.pubkey(), // 支付者
        &mint.pubkey(),      // 新账户（mint）
        mint_rent,           // lamports
        mint_space as u64,   // 空间
        &token_program_id(), // program id
    );

    // 初始化 mint account 数据的指令
    ...
    // 计算手续费支付者的 associated token account 地址
    ...
    // 创建 associated token account 的指令
    ...
    // 要铸造的代币数量（2 位小数的 100 个代币）
    let amount = 100;

    // 创建 mint_to 指令，将代币铸造到 associated token account
    ...
    println!("已向 associated token account 铸造 1.00 个代币");
    println!("\nMint 地址: {}", mint.pubkey());
    println!("{:#?}", mint_data);

    println!(
        "\nAssociated Token Account 地址: {}",
...
async def main():
    ...
    # 要铸造的数量（最小单位）
    amount_to_mint = 1000000000  # 1 个代币，9 位小数
