---
title: 转移代币
description: 使用 Token Program 转账指令在相同铸币的代币账户之间移动代币。
...
转账代币是将代币从一个 token account 转移到相同铸币的另一个 token account。

转账不会改变铸币供应量。转账只会更新 token account 之间的余额。

## 如何转账代币

转账代币使用 Token Program 的 _rs`Transfer`_ 或 _rs`TransferChecked`_ 指令。

以下示例使用
_rs`TransferChecked`_,该指令要求调用者提供铸币和小数位数,以便指令在账户之间移动代币之前可以验证预期的铸币和代币精度。

在 Token Extensions Program 中,_rs`Transfer`_ 已被弃用,改为使用
_rs`TransferChecked`_ 或 _rs`TransferCheckedWithFee`_。

源账户所有者或经批准的委托人对转账进行签名。在 Token Extensions
Program 中,如果铸币启用了永久委托人扩展,铸币永久委托人也可以授权转账。

### 源代码参考

| 项目                   | 描述                                                                                                                                   | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_          | token account 状态存储由转账更新的余额。                                                                                               | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | 一个转账指令,在 token account 之间移动代币,无需调用者提供铸币或小数位数。在 Token Extensions Program 中,请改用 _rs`TransferChecked`_。 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | 一个转账指令,要求调用者提供铸币和小数位数,并在相同铸币的 token account 之间移动代币之前检查这些值。                                    | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | 代币转账的共享处理器逻辑。                                                                                                             | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |

### Typescript

以下 `Kit` 示例展示了使用 `@solana/kit` 的推荐方法。为了便于参考，还包含了使用
