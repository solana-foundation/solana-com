---
title: 转移代币
description: 使用 Token Program 转账指令在同一铸币的代币账户之间移动代币。
...
转账代币是将代币从一个 token account 移动到同一铸币的另一个 token account。

转账不会改变铸币供应量。转账仅更新 token account 之间的余额。
...
下面的示例使用
_rs`TransferChecked`_,它要求调用者提供铸币和小数位数,以便指令在账户之间移动代币之前验证预期的铸币和代币精度。

在 Token Extensions Program 中,_rs`Transfer`_ 已弃用,改为使用
_rs`TransferChecked`_ 或 _rs`TransferCheckedWithFee`_。

源账户所有者或经批准的委托人签署转账。在 Token Extensions
Program 中,如果铸币启用了永久委托扩展,铸币永久委托人也可以授权转账。
...
| _rs`TransferChecked`_  | 一个转账指令,要求调用者提供铸币和小数位数,并在同一铸币的 token account 之间移动代币之前检查这些值。                                    | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
