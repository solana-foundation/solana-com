---
title: 销毁代币
description: 从代币账户中销毁代币并减少铸币的总供应量。
...
以下示例使用
_rs`BurnChecked`_，该指令要求调用者提供铸币的小数位数，以便指令在销毁前验证预期的代币精度。

[原生铸币](/docs/tokens/basics/sync-native) 不支持销毁。请改用
_rs`CloseAccount`_。

代币账户所有者或经批准的委托人对交易进行签名。在 Token Extensions
Program 中，如果铸币启用了永久委托扩展，铸币永久委托人也可以授权销毁操作。

### 源代码参考

| 项目               | 描述                                                                                     | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ------------------ | ---------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_         | 铸币状态存储代币总供应量。                                                               | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
...
| _rs`Burn`_         | 一个销毁指令,减少代币账户余额和铸币供应量,无需调用者提供铸币的小数位数。                 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | 一个销毁指令,要求调用者提供铸币的小数位数,并在减少代币账户余额和铸币供应量之前检查该值。 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
...
下面的 `Kit` 示例展示了使用 `@solana/kit` 的推荐方法。为了方便参考,还包含了使用
`@solana/web3.js` 的旧版示例。
