---
title: 同步原生 SOL
description: 在将 SOL 转入账户后更新封装的 SOL token 账户金额。
...
封装的 SOL(WSOL)是存储在 Token
Program 原生铸币地址 token 账户中的 SOL。原生铸币是 Token
Program 用于封装 SOL 的 token 账户的铸币地址。原生铸币使 SOL 能够通过 Token
Program 的指令进行使用。
...
封装的 SOL token 账户将 SOL 存储在账户的 `lamports` 字段中,但通过 token 账户的
`amount` 字段将封装的 SOL
token 账户中持有的 SOL 作为代币余额进行追踪。如果您将 SOL 转入封装的 SOL
token 账户,lamport 余额会立即增加,但 `amount` 字段不会改变,直到您调用 Token
Program 的 `SyncNative` 指令。

## 如何同步原生 SOL

在将 SOL 转入封装的 SOL token 账户后,使用 Token Program 的 `SyncNative`
指令。`SyncNative` 指令会更新 token 账户的 `amount`
...
| --------------------- | -------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `Account`             | token 账户状态使用 `is_native` 来标记账户为 WSOL,并存储原生储备、账户必须保留的免租金最低 lamport 余额以及追踪的代币数量。 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| `SyncNative`          | 一个指令,用于更新封装的 SOL token 账户的 `amount` 字段,使其与账户中超出免租金储备的 lamport 数量相匹配。                   | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| `process_sync_native` | 用于同步封装的 SOL 余额的共享处理器逻辑。                                                                                  | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |
