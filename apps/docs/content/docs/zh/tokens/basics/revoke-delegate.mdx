...
撤销委托会清除代币账户的当前委托并将委托数量重置为零。

撤销后，之前的委托方将不再有权代表该代币账户转移或销毁代币。

## 如何撤销委托

撤销委托使用 Token Program 的 _rs`Revoke`_ 指令。

_rs`Revoke`_ 指令会清除源代币账户上的委托字段。在 Token
Program 中，代币账户所有者对撤销操作进行签名。在 Token Extensions
Program 中，当前委托方也可以撤销委托。

### 源代码参考

| 项目                 | 描述                               | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| -------------------- | ---------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_        | 代币账户状态存储委托方和委托数量。 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Revoke`_         | 从代币账户移除当前委托的指令。     | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L128-L140) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L170-L182) |
