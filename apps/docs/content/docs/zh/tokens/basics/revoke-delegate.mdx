---
title: 撤销委托
description: 移除代币账户的当前委托并清除其剩余的委托额度。
...
撤销委托会清除 token account 的当前委托并将委托金额重置为零。

撤销后，之前的委托人将不再有权代表该 token account 转移或销毁代币。

## 如何撤销委托

撤销委托使用 Token Program 的 _rs`Revoke`_ 指令。

_rs`Revoke`_ 指令会清除源 token account 上的委托字段。在 Token Program 中，token
account 所有者签署撤销操作。在 Token Extensions
Program 中，当前委托人也可以撤销委托。

### 源代码参考

| 项目                 | 描述                                      | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| -------------------- | ----------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_        | token account 状态存储委托人和委托金额。  | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Revoke`_         | 从 token account 中移除当前委托人的指令。 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L128-L140) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L170-L182) |
| _rs`process_revoke`_ | 委托撤销的共享处理器逻辑。                | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L395-L420)     | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L643-L674)     |
