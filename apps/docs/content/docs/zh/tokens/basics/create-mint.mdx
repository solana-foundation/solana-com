...
description: 创建代币铸造账户并配置铸造账户的小数位数、铸造权限和冻结权限。
...
铸造账户在 Solana 上定义并唯一标识一个代币,并存储适用于该铸造账户所有代币账户的共享状态。

Token Program 将 _rs`Mint`_ 账户类型定义为:
...
每个代币都有一个 mint
account,铸造地址是该代币在钱包、应用程序和区块链浏览器中的唯一标识符。

例如,USD Coin (USDC) 的铸造地址为
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`。该铸造地址在整个 Solana 生态系统中唯一标识 USDC。您可以在
...
2. Token Program 的 _rs`InitializeMint`_ 或 _rs`InitializeMint2`_
   指令将新账户初始化为铸造账户。

在同一交易中包含 _rs`CreateAccount`_ 指令和铸造初始化指令。

在铸造初始化期间,Token Program 会检查铸造账户是否尚未初始化且免 rent。然后 Token
Program 将铸造权限、冻结权限、小数位数和 _rs`is_initialized`_
标志写入铸造账户数据中。
...
| ------------------------------ | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                     | 存储在每个 mint account 中的基本铸造状态。                     | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`InitializeMint`_           | 一个铸造初始化指令,要求在其账户列表中包含 rent sysvar 账户。   | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L24-L44)   | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L50-L74)   |
| _rs`InitializeMint2`_          | 一个铸造初始化指令,不要求在其账户列表中包含 rent sysvar 账户。 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L401-L414) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L471-L486) |
| _rs`_process_initialize_mint`_ | 铸造初始化的共享处理器逻辑。                                   | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
| _rs`process_initialize_mint`_  | _rs`InitializeMint`_ 的公共处理程序。                          | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L62-L70)       | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L132-L140)     |
| _rs`process_initialize_mint2`_ | _rs`InitializeMint2`_ 的公共处理程序。                         | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L72-L81)       | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L142-L151)     |
...
mint
account 仅存储基本的代币信息(供应量、小数位数、权限)。要为代币添加人类可读的元数据,如名称、符号和图像,您有两个选择:

<Cards>
  <Card title="Metaplex 代币元数据" href="/docs/tokens/metaplex">
   使用 Metaplex 代币元数据程序添加元数据。兼容原始 Token Program 和 Token-2022。
  </Card>

  <Card title="代币扩展元数据" href="/docs/tokens/extensions/metadata">
   使用 Token-2022 内置的元数据扩展。
   元数据扩展仅适用于使用 Token-2022 创建的 mint account。
   要为使用原始 Token Program 创建的 mint account 添加元数据,请使用 Metaplex 代币元数据程序。
  </Card>
</Cards>
