...
例如，USD Coin ( USDC ) 的 mint 地址为
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`。该地址在整个 Solana 生态系统中唯一标识 USDC 。你可以在
[Solana Explorer](https://explorer.solana.com/address/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v)
查看此 mint。

## 如何创建一个 Mint 账户

创建 mint account 需要使用
...
指令。详细实现请参见
[此处](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/processor.rs#L27)。
...
mint
account 仅存储基本代币信息（供应量、小数位数、权限）。要为您的代币添加人类可读的元数据（如名称、符号和图片），您有两个选择：

<Cards>
  <Card title="Metaplex 代币元数据" href="/docs/tokens/metaplex">
   使用 Metaplex 代币元数据程序添加元数据。同时适用于原始 Token Program 和 Token-2022。
  </Card>
...
mint
account 仅存储基本的代币信息(供应量、小数位数、权限)。要为您的代币添加可读性强的元数据,如名称、符号和图像,您有两个选择:

<Cards>
  <Card title="Metaplex 代币元数据" href="/docs/tokens/metaplex">
   使用 Metaplex 代币元数据程序添加元数据。适用于原始 Token Program 和 Token-2022。
  </Card>

  <Card title="Token Extensions 元数据" href="/docs/tokens/extensions/metadata">
