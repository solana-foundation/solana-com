...
1. System Program 的 _rs`CreateAccount`_ 指令创建一个新的免租金账户,并将 Token
   Program 指定为新账户的程序所有者。
2. Token Program 的 _rs`InitializeMint`_ 或 _rs`InitializeMint2`_
   指令将新账户初始化为铸造账户。

在同一笔交易中包含 _rs`CreateAccount`_ 指令和铸造初始化指令。

在铸造初始化期间,Token
Program 检查铸造账户是否尚未初始化且已免除租金。然后 Token
...
| ------------------------------ | --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                     | 存储在每个铸造账户中的基础铸造状态。                      | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`InitializeMint`_           | 铸造初始化指令,需要在其账户列表中包含租金系统变量账户。   | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L24-L44)   | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L50-L74)   |
| _rs`InitializeMint2`_          | 铸造初始化指令,不需要在其账户列表中包含租金系统变量账户。 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L401-L414) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L471-L486) |
...
以下 `Kit` 示例展示了使用 `@solana/kit` 的推荐方法。同时包含了使用
`@solana/web3.js` 的旧版示例供参考。
...
mint
account 仅存储基本的代币信息(供应量、小数位数、权限)。要为您的代币添加名称、符号和图像等人类可读的元数据,您有两个选择:

<Cards>
  <Card title="Metaplex 代币元数据" href="/docs/tokens/metaplex">
   使用 Metaplex Token Metadata Program 添加元数据。同时支持原始 Token Program 和 Token-2022。
  </Card>

  <Card title="代币扩展元数据" href="/docs/tokens/extensions/metadata">
   使用 Token-2022 的内置元数据扩展。
   元数据扩展仅适用于使用 Token-2022 创建的铸币账户。
   要为使用原始 Token Program 创建的铸币账户添加元数据,请使用 Metaplex Token Metadata Program。
  </Card>
</Cards>
