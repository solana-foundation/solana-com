---
title: 冻结账户
description: 使用铸币的冻结权限冻结代币账户,以阻止转账、销毁和委托变更。
...
冻结 token account 会保持相同的所有者、铸币和余额，但会阻止该 token
account 接收、转移或销毁代币，直到该 token account 被解冻。

如果冻结的账户代币余额为零,仍然可以关闭该账户。

只有仍然拥有冻结权限的铸币才能冻结代币账户。如果铸币的冻结权限被撤销,则该铸币的账户将无法再被冻结。
...
_rs`FreezeAccount`_
指令会验证该代币账户属于该铸币,并且签名者是铸币的冻结权限,然后将代币账户状态从已初始化变更为已冻结。
...
| ----------------------------------- | ----------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | 铸币状态存储用于冻结代币账户的可选冻结权限。          | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
