...
指令会阻止某个特定 token 账户的所有 token 转账或销毁操作。一旦被冻结，该账户将无法发送、接收 token，也无法关闭，直到被解冻。只有该 token 铸造合约的冻结权限（freeze
authority）才能冻结账户。如果在铸造账户上撤销（设置为 null）冻结权限，则该 token 将无法再被冻结。
