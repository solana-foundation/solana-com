---
title: 冻结账户
description: 使用铸币厂的冻结权限冻结代币账户,以阻止转账、销毁和委托变更。
...
冻结代币账户会保持相同的所有者、铸币厂和余额,但会阻止该代币账户转账或销毁代币,直到代币账户被解冻为止。

如果冻结账户的代币余额为零,仍然可以关闭该账户。

只有仍然拥有冻结权限的铸币厂才能冻结代币账户。如果铸币厂的冻结权限被撤销,该铸币厂的账户将无法再被冻结。

## 如何冻结代币账户

冻结代币账户使用 Token Program 的 _rs`FreezeAccount`_ 指令。

_rs`FreezeAccount`_
指令会验证该代币账户是否属于该铸币厂,以及签名者是否为铸币厂的冻结权限,然后将代币账户状态从已初始化更改为已冻结。

[原生代币账户](/docs/tokens/basics/sync-native)不支持冻结。

### 源代码参考

| 项目                                | 说明                                                  | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ----------------------------------- | ----------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | 铸币厂状态存储用于冻结代币账户的可选冻结权限。        | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | 代币账户状态,包括 _rs`Initialized`_ 和 _rs`Frozen`_。 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | 将代币账户更改为冻结状态的指令。                      | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
...
以下 `Kit` 示例展示了使用 `@solana/kit` 的推荐方法。为参考起见，还包含了使用
`@solana/web3.js` 的传统示例。
