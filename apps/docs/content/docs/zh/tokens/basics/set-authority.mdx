---
title: 设置权限
description: 更改或撤销铸币账户或代币账户的权限角色,包括铸造权限、冻结权限、所有者权限和关闭权限。
...
铸币账户可以存储铸造权限和冻结权限。代币账户存储所有者权限,并可以选择性地存储关闭权限。每个角色控制特定的权限集,将权限设置为
...
每次调用会更新一个铸币账户或代币账户上的一个权限类型。当前权限持有者对更改进行签名,新权限可以是另一个地址或
...
| _rs`SetAuthority`_          | 用于更改或撤销铸币账户或代币账户上一个权限的指令。            | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L141-L158) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L183-L200)   |
| _rs`process_set_authority`_ | 权限更新的共享处理器逻辑。                                    | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L423-L518)     | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L677-L978)       |
