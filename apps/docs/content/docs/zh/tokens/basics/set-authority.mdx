---
title: 设置权限
description: 更改或撤销铸币账户或代币账户上的权限角色,包括铸币权限、冻结权限、所有者权限和关闭权限。
...
铸币账户可以存储铸币权限和冻结权限。代币账户存储所有者权限,并可选择性地存储关闭权限。每个角色控制特定的权限集,将权限设置为
_rs`None`_ 将永久移除所选的权限角色。

Token Extensions Program 还为特定扩展定义了额外的权限类型。

## 如何设置权限

更改权限使用 Token Program 的 _rs`SetAuthority`_ 指令。

每次调用都会更新一个铸币账户或代币账户上的一个权限类型。当前权限持有者对更改进行签名,新权限可以是另一个地址或
_rs`None`_ 以永久撤销所选权限。

### 源代码参考

| 项目                        | 说明                                                          | Token Program                                                                                                  | Token Extension Program                                                                                                |
| --------------------------- | ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                  | 铸币状态存储 _rs`mint_authority`_ 和 _rs`freeze_authority`_。 | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)           |
| _rs`Account`_               | 代币账户状态存储 _rs`owner`_ 和 _rs`close_authority`_。       | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)          |
| _rs`AuthorityType`_         | _rs`SetAuthority`_ 可以更新的权限角色。                       | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L739-L751) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1116-L1157) |
| _rs`SetAuthority`_          | 更改或撤销铸币账户或代币账户上的一个权限的指令。              | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L141-L158) | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L183-L200)   |
| _rs`process_set_authority`_ | 权限更新的共享处理逻辑。                                      | [源代码](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L423-L518)     | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L677-L978)       |
