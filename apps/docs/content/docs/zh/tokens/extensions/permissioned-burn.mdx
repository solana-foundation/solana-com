...
本地测试 validator 还内置了较旧版本的 Token-2022，因此若要运行本页面的示例，请将 devnet 上的 v11.0.0 程序加载到您的测试 validator 中：
...
1. 计算 mint account 所需的大小以及 _rs`PermissionedBurnConfig`_
   扩展所需的 rent。
2. 使用 _rs`CreateAccount`_ 创建 mint account，初始化
   _rs`PermissionedBurnConfig`_，并使用 _rs`InitializeMint`_ 初始化该 mint
   account。
3. 创建 token account 并铸造代币。
4. 使用 _rs`PermissionedBurnInstruction::BurnChecked`_ 进行销毁，需由 token
   account 所有者和销毁授权方共同签名。
5. 可选操作：使用 _rs`AuthorityType::PermissionedBurn`_ 和 `None` 授权方，通过
   _rs`SetAuthority`_
   禁用权限销毁，从而重新启用标准销毁指令。以下完整代码示例展示了此步骤。

<ScrollyCoding>

## !!steps 计算账户大小

计算基础 mint account 加上 _rs`PermissionedBurnConfig`_ 扩展所需的 mint
account 大小。该大小将用于 _rs`CreateAccount`_。
...
使用 mint account 加上 _rs`PermissionedBurnConfig`_ 扩展所需的大小来计算 rent。
...
使用计算得出的空间和 lamport 创建 mint account。
...
在 mint account 上初始化 _rs`PermissionedBurnConfig`_ 扩展，并设置销毁授权方。
...
## !!steps 初始化 mint account

在同一交易中使用 _rs`InitializeMint`_ 初始化 mint account。
...
为付款方创建一个 token account 并向其铸造代币。
...
使用由 token account 所有者和销毁权限方共同签名的
_rs`PermissionedBurnInstruction::BurnChecked`_ 来销毁代币。
...
| 条目                                           | 描述                                                                                     | 来源                                                                                                                                         |
| ---------------------------------------------- | ---------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | Mint 扩展，用于存储对该 mint 每次销毁操作进行联署所需的权限。                            | [来源](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
| _rs`PermissionedBurnInstruction::Initialize`_  | 在 _rs`InitializeMint`_ 之前初始化受权限控制的销毁配置的指令。                           | [来源](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L41-L49) |
| _rs`PermissionedBurnInstruction::Burn`_        | 需要销毁权限方以及 token account 所有者或委托方签名的销毁指令。                          | [来源](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | 带有小数位检查的销毁指令，使用与 _rs`PermissionedBurnInstruction::Burn`_ 相同的账户。    | [来源](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | 与 _rs`SetAuthority`_ 配合使用的权限鉴别器，用于轮换或禁用 mint 上的销毁权限。           | [来源](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | 处理器逻辑，用于在未初始化的 mint 上初始化 _rs`PermissionedBurnConfig`_ 并存储销毁权限。 | [来源](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
| _rs`process_burn`_                             | 处理器逻辑，在设置了销毁权限时拒绝标准销毁操作并验证权限方的签名。                       | [来源](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | 处理器逻辑，在使用 _rs`AuthorityType::PermissionedBurn`_ 时验证并轮换销毁权限。          | [来源](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### Typescript

以下 `Kit` 示例直接使用生成的指令。已发布的旧版 `@solana/spl-token`
包尚不支持权限销毁功能，因此不包含旧版示例。
