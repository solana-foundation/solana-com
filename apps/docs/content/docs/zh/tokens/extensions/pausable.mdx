...
title: 可暂停铸造
description:
  创建一个具有暂停权限的 Token-2022
  铸造账户,在铸造账户暂停时阻止转账,并稍后恢复活动。
...
Token Extensions Program 的 _rs`PausableConfig`_
铸造扩展使暂停权限能够暂停和恢复整个铸造账户的代币活动。

当铸造账户被暂停时,Token Extensions Program 会拒绝:

- 转账
- 铸造
- 销毁

当铸造账户恢复时,这些指令将再次正常工作。

## 如何创建、暂停和恢复铸造账户

要创建、暂停和恢复铸造账户:

1. 计算铸造账户大小以及铸造账户和 _rs`PausableConfig`_ 扩展所需的租金。
2. 使用 _rs`CreateAccount`_ 创建铸造账户,初始化 _rs`PausableConfig`_,并使用
   _rs`InitializeMint`_ 初始化铸造账户。
3. 为铸造账户铸造代币并创建代币账户。
4. 使用 _rs`Pause`_ 阻止铸造账户的代币活动。
5. 使用 _rs`Resume`_ 再次允许代币活动。

<ScrollyCoding>

## !!steps 计算账户大小

计算基础铸造账户加上 _rs`PausableConfig`_ 扩展的铸造账户大小。这是在
...
使用铸造账户加上 _rs`PausableConfig`_ 扩展所需的大小计算租金。
...
## !!steps 初始化 PausableConfig

在铸币账户上初始化 _rs`PausableConfig`_ 扩展。
...
<Callout type="info" title="指令顺序">
  _rs`PausableInstruction::Initialize`_ 必须在 _rs`InitializeMint`_ 之前执行。
  _rs`CreateAccount`_、_rs`PausableInstruction::Initialize`_ 和
  _rs`InitializeMint`_ 必须包含在同一交易中。
</Callout>
...
| 项目                                  | 描述                                                                           | 源代码                                                                                                                                |
| ------------------------------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PausableConfig`_                  | 存储暂停权限以及铸币账户当前是否处于暂停状态的铸币扩展。                       | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/mod.rs#L17-L22)         |
| _rs`PausableInstruction::Initialize`_ | 在 _rs`InitializeMint`_ 之前初始化可暂停配置的指令。                           | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L20-L32) |
| _rs`PausableInstruction::Pause`_      | 将铸币账户标记为已暂停的指令。                                                 | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L33-L44) |
| _rs`PausableInstruction::Resume`_     | 将铸币账户标记为未暂停的指令。                                                 | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L45-L56) |
| _rs`process_initialize`_              | 在未初始化的铸币账户上初始化 _rs`PausableConfig`_ 并存储暂停权限的处理器逻辑。 | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L22-L35)     |
| _rs`process_toggle_pause`_            | 在切换铸币账户的暂停标志之前验证暂停权限的处理器逻辑。                         | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L39-L64)     |

### Typescript

下面的 `Kit` 示例直接使用生成的指令。包含使用 `@solana/web3.js` 和
`@solana/spl-token` 的传统示例以供参考。
