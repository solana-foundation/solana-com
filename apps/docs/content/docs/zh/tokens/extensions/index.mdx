...
每个扩展功能都会添加特定的状态，通常在代币铸造账户或代币账户创建时初始化。在初始化任一账户时，您可以同时启用特定的扩展功能以实现不同的功能。大多数扩展功能在账户初始化后无法添加。这是设计代币时需要考虑的重要因素，因为您需要提前规划好希望代币支持哪些功能。每个扩展功能的集成指南可以在[Token Extensions 开发者文档](/docs/tokens/extensions)中找到。
