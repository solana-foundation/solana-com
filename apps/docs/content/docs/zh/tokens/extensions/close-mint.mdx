...
title: 关闭 Mint
description:
  创建一个带有 MintCloseAuthority 扩展的 Token Extensions Program
  铸币账户,然后销毁剩余供应量并关闭该铸币账户。
...
Token Extensions Program 的 _rs`MintCloseAuthority`_
铸币扩展允许指定的权限在铸币总供应量归零后关闭铸币账户并收回其租金。
...
1. 计算铸币账户大小以及铸币和 _rs`MintCloseAuthority`_ 扩展所需的租金。
2. 使用 _rs`CreateAccount`_ 创建铸币账户,初始化 _rs`MintCloseAuthority`_,并使用
   _rs`InitializeMint`_ 初始化铸币账户。
3. 如果总供应量为零,则可以使用 _rs`CloseAccount`_
   关闭铸币账户,需要由关闭权限签名。
...
计算基础铸币加上 _rs`MintCloseAuthority`_ 扩展的铸币账户大小。这是在
...
使用铸币加上 _rs`MintCloseAuthority`_ 扩展所需的大小来计算租金。
...
## !!steps 初始化 MintCloseAuthority

在铸币账户上初始化 _rs`MintCloseAuthority`_ 扩展。
...
<Callout type="info" title="指令顺序">
  _rs`InitializeMintCloseAuthority`_ 必须在 _rs`InitializeMint`_ 之前执行。
  _rs`CreateAccount`_、_rs`InitializeMintCloseAuthority`_ 和
...
| 项目                                          | 描述                                                                  | 源代码                                                                                                                                |
| --------------------------------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`MintCloseAuthority`_                      | 铸币扩展,用于存储允许在供应量归零后关闭铸币账户的授权。               | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mint_close_authority.rs#L14-L20) |
| _rs`InitializeMintCloseAuthority`_            | 在 _rs`InitializeMint`_ 之前初始化铸币关闭授权的指令。                | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1830-L1843)                |
| _rs`CloseAccount`_                            | 在总供应量归零后关闭铸币账户的基础代币指令。                          | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1582-L1608)                |
| _rs`process_initialize_mint_close_authority`_ | 将 _rs`MintCloseAuthority`_ 扩展写入未初始化铸币账户的处理器逻辑。    | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1376-L1389)                    |
| _rs`process_close_account`_                   | 共享的关闭账户处理程序,对具有关闭授权的铸币账户强制执行零供应量规则。 | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1288)                    |

### Typescript

下面的 `Kit` 示例直接使用生成的指令。包含使用 `@solana/web3.js` 和
`@solana/spl-token` 的旧版示例供参考。
