...
扩展允许 mint account 被关闭，并回收其 rent。此扩展必须在创建 mint 时初始化。

只有指定的 close authority 才能关闭 mint
account。类似于只有当 token 账户余额为零时才能关闭，带有此扩展的 mint
account 也只能在全部 token 供应量归零后才能关闭。
