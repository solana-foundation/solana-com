...
title: 默认状态
description:
  配置 Token Extensions Program
  铸币账户，使新创建的代币账户以选定的默认状态启动，例如冻结状态。
...
Token Extensions Program 的 _rs`DefaultAccountState`_
铸币扩展为该铸币创建的每个新 token account 设置初始 _rs`AccountState`_。

这最常用于使新 token account 以冻结状态启动。以冻结状态启动的 token
account 在铸币的冻结权限解冻它们之前无法使用。

## 如何创建具有默认账户状态的铸币

要创建具有默认账户状态的铸币：

1. 计算铸币账户大小以及铸币和 _rs`DefaultAccountState`_ 扩展所需的 rent。
2. 使用 _rs`CreateAccount`_ 创建铸币账户，初始化
   _rs`DefaultAccountState`_，并使用 _rs`InitializeMint`_ 初始化铸币。
3. 为铸币创建 token account，并观察每个新 token account 都以当前默认状态启动。
4. 使用 _rs`UpdateDefaultAccountState`_ 更改未来 token account 应启动的状态。

<ScrollyCoding>

## !!steps 计算账户大小

计算基础铸币加上 _rs`DefaultAccountState`_ 扩展的铸币账户大小。这是在
...
使用铸币加上 _rs`DefaultAccountState`_ 扩展所需的大小计算 rent。
...
## !!steps 初始化 DefaultAccountState

在铸币账户上初始化 _rs`DefaultAccountState`_ 扩展。
...
在同一笔交易中使用 _rs`InitializeMint`_ 初始化铸币账户。
...
为铸币账户创建一个代币账户。由于当前默认状态为
_rs`Frozen`_,新代币账户将以冻结状态启动。
...
使用 _rs`UpdateDefaultAccountState`_
更改默认状态,使未来的代币账户不再以冻结状态启动。
...
<Callout type="info" title="指令顺序">
  _rs`DefaultAccountStateInstruction::Initialize`_ 必须在 _rs`InitializeMint`_
  之前执行。_rs`CreateAccount`_、
  _rs`DefaultAccountStateInstruction::Initialize`_ 和 _rs`InitializeMint`_
  必须包含在同一笔交易中。
</Callout>
...
| 项目                                             | 描述                                                               | 源码                                                                                                                                             |
| ------------------------------------------------ | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`DefaultAccountState`_                        | 存储新代币账户应继承状态的铸币扩展。                               | [源码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/mod.rs#L16-L21)         |
| _rs`DefaultAccountStateInstruction::Initialize`_ | 在 _rs`InitializeMint`_ 之前初始化铸币账户默认代币账户状态的指令。 | [源码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L20-L36) |
| _rs`DefaultAccountStateInstruction::Update`_     | 允许冻结权限更改未来代币账户默认状态的指令。                       | [源码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L37-L53) |
| _rs`process_initialize_default_account_state`_   | 将初始默认状态写入未初始化铸币账户的处理器逻辑。                   | [源码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L29-L40)     |
| _rs`process_update_default_account_state`_       | 在更改铸币账户存储的默认状态之前验证冻结权限的处理器逻辑。         | [源码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L43-L74)     |
