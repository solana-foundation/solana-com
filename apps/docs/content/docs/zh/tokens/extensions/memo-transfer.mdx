...
title: 备忘录转账
description:
  通过在 Token Extensions Program 代币账户上启用 MemoTransfer
  账户扩展，要求接收的转账必须包含备忘录。
...
Token Extensions Program 的 _rs`MemoTransfer`_
账户扩展要求在向该代币账户进行任何接收转账之前，必须立即包含一个备忘录指令。从该代币账户发出的转账不需要备忘录。

该扩展配置在目标代币账户上,而不是在铸币账户上。一旦启用,没有备忘录的转账将失败并返回
_rs`TokenError::NoMemo`_,直到代币账户所有者禁用该要求。

## 如何创建启用了 MemoTransfer 的代币账户

要创建启用了 _rs`MemoTransfer`_ 的代币账户:

1. 创建并初始化铸币账户。
2. 计算带有 _rs`MemoTransfer`_ 的代币账户所需的账户大小和租金。
3. 创建代币账户并在该账户上启用 _rs`MemoTransfer`_。
4. 向该代币账户发送的 _rs`Transfer`_ 和 _rs`TransferChecked`_
   指令必须在转账指令之前立即包含备忘录指令。
...
计算基础代币账户加上 _rs`MemoTransfer`_ 扩展的代币账户大小。这是在
...
使用 _rs`MemoTransfer`_ 扩展所需的代币账户大小来计算租金。
...
## !!steps 启用 MemoTransfer

在代币账户上启用 _rs`MemoTransfer`_。
...
| 项目                                            | 说明                                                            | 源代码                                                                                                                                     |
| ----------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`MemoTransfer`_                              | 账户扩展,用于存储传入转账是否需要备注。                         | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/mod.rs#L17-L22)         |
| _rs`RequiredMemoTransfersInstruction::Enable`_  | 在代币账户上启用需要备注的转账指令。                            | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L50-L71) |
| _rs`RequiredMemoTransfersInstruction::Disable`_ | 在代币账户上禁用需要备注的转账指令。                            | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L74-L95) |
| _rs`process_toggle_required_memo_transfers`_    | 处理器逻辑,在验证所有权后初始化或重写 _rs`MemoTransfer`_ 扩展。 | [源代码](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/memo_transfer/processor.rs#L20-L48)     |
