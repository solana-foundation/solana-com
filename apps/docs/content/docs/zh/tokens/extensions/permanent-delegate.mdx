...
会为某个 mint 分配一个拥有所有 token account 管理权限的代理。与 token
account 拥有者可以撤销的普通代理不同，永久代理无法被 token
account 拥有者更改或移除。永久代理可以在该 mint 的任意 token
account 中转移或销毁代币。当前永久代理可以将永久代理角色更改为新的账户。
