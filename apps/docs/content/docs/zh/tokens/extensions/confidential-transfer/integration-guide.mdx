...
title: 机密余额集成指南
description:
  了解如何将使用机密余额的代币集成到钱包、浏览器、交易所及其他应用程序中。
---
...
机密余额扩展允许 Token-2022 铸币账户在链上对转账金额和账户余额进行加密。由于余额已加密，显示余额需要所有者的解密密钥，而发送转账则需要在客户端生成零知识证明。

本指南面向**集成**机密余额代币的团队（钱包、浏览器、交易所、托管方、索引器），而非配置铸币账户的发行方。如果您正在从头构建底层流程，请从下方链接的分步说明页面开始；本指南重点介绍您的产品需要做什么才能良好地支持这些代币。

每个账户的地址、铸币厂和所有者均保持**公开**。只有金额和余额是加密的，因此其他所有信息（包括交易双方的身份）仍然可见。这为公共观察者提供了保密性，而非匿名性。

### 资源

- [机密余额概述与分步指南](/docs/tokens/extensions/confidential-transfer)
...
当账户配置了机密余额后，`ConfidentialTransferAccount` 扩展会存储（在其他字段中）：
...
因此，请避免为每位用户静默开通机密账户：这会产生额外的 rent，并且仍需所有者参与。请在首次使用时进行配置，或在用户选择启用机密余额时进行配置。
...
两个可选扩展基于机密余额构建。两者主要由发行方负责，但各自会改变集成方需要处理的某些内容：
...
| 要求                   | 描述                                                                                                                                                     | 优先级 |
| :---------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------- |
| 检测扩展          | 识别铸币账户和普通账户上的机密余额，并明确处理这些代币，而非假设仅使用公开模式。                              | **P0**   |
