...
在代币能够以保密方式转账之前，公开代币余额必须转换为保密余额。此转换分两个阶段进行：

1. **保密待处理余额**：首先，代币从公开余额 "存入"到"待处理"保密余额。
2. **保密可用余额**：待处理余额随后被"应用"到可用余额，使代币可用于保密转账。

本节介绍第一阶段：将公开代币余额存入保密待处理余额。

下图展示了将代币从公开余额存入保密待处理余额所涉及的步骤：
...
`spl_token_client` crate 提供了 `confidential_transfer_deposit`
方法，该方法构建并发送包含 `Deposit` 指令的交易，如下面的示例所示。

## 示例代码

以下示例演示如何将公开代币余额存入保密待处理余额。

机密余额功能依赖于 ZK ElGamal 证明程序，该程序已在主网和开发网上启用。标准的 `solana-test-validator` 不启用该程序，但支持主网分叉的本地 validator（如 [Surfpool](https://surfpool.run)）则支持。
请使用已充值的付款账户，针对上述环境之一运行示例（代码默认使用开发网），并将占位符中的铸币地址和账户地址替换为您自己的地址。
