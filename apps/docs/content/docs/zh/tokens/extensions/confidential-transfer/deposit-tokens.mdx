...
保密转账依赖于 ZK ElGamal 证明程序，该程序已在主网和开发网上启用。标准的
`solana-test-validator` 不会启用它，但类似 [Surfpool](https://surfpool.run)
这样的主网分叉本地 validator 会启用。请使用已充值的付款账户针对其中一个环境运行示例（代码默认使用开发网），并将占位符的 mint 地址和账户地址替换为您自己的地址。
