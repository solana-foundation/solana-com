...
title: 创建 Token 账户
description:
  了解如何创建启用了机密余额的 token account。
---

## 如何创建启用机密余额的 token account

机密余额通过向 token account 添加额外状态来实现私密代币转账。本节介绍如何创建启用了所需扩展的 token account。

下图展示了创建启用机密余额的 token account 所涉及的步骤：
...
_rs`ConfidentialTransferAccount`_ 包含多个管理机密余额的字段：

- **approved**：账户的机密余额审批状态。如果 mint account 的 `auto_approve_new_accounts` 配置设为 `true`，所有 token account 将自动获得机密余额的使用批准。
...
为 token account 创建和配置机密余额需使用以下指令，这些指令均可在单笔交易中完成：
...
4. **配置机密余额**：调用 Token Extension Program 的
   ...
   指令，通过 _rs`ProofLocation::ContextStateAccount`_ 引用证明上下文状态账户，以初始化
   _rs`ConfidentialTransferAccount`_ 状态。

<Callout type="info">

**只有 token account 的所有者才能在 token account 上启用机密余额**。

_rs`ConfigureAccount`_ 指令需要在客户端生成加密密钥和证明，而这些只能由 token account 所有者生成。

pubkey 有效性证明用于验证账户的 ElGamal 公钥是否有效。它通过 _rs`build_pubkey_validity_proof_data`_ 生成，由链上 ZK ElGamal Proof 程序验证并存入上下文状态账户，然后通过 _rs`ProofLocation::ContextStateAccount`_ 从 _rs`ConfigureAccount`_ 中引用，因此代币指令本身不携带任何证明字节。如需了解实现细节，请参阅：
...
以下代码创建一个 associated token account，并将其配置为使用机密余额，所用的 mint 为已存在的兼容 mint。

机密余额依赖于 ZK ElGamal Proof 程序，该程序在主网和开发网上均已启用。标准的 `solana-test-validator` 不会启用该程序，但支持主网分叉的本地 validator（如 [Surfpool](https://surfpool.run)）会启用。请使用已充值的付款账户，在上述环境之一（代码默认使用开发网）中运行示例，并将 mint 占位符替换为按照 [Create a Mint](/docs/tokens/extensions/confidential-transfer/create-mint) 创建的 mint。
