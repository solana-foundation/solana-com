...
1. 在客户端创建
   ...
   **等式证明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**：用于验证提现后剩余可用余额的密文是否与其对应的
   [Pedersen 承诺](https://en.wikipedia.org/wiki/Commitment_scheme)一致，以确保账户新的可用余额被正确计算为
   `remaining_balance = current_balance - withdraw_amount`。
