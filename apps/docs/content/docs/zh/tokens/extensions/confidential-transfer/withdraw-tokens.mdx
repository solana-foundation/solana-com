...
1. 在客户端创建
   ...
   **等式证明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**：验证提取后剩余的可用余额密文与其对应的
   [Pedersen 承诺](https://en.wikipedia.org/wiki/Commitment_scheme)
   相匹配，确保账户的新可用余额被正确计算为
   `remaining_balance = current_balance - withdraw_amount`。

   **范围证明 ([BatchedRangeProofU64Data](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_range_proof/batched_range_proof_u64.rs#L37))**：验证提取后剩余的可用余额为非负数且在指定范围内。

2. 对于每个证明：
   - 调用 ZK ElGamal 证明程序以验证证明数据。
   - 将证明特定的元数据存储在证明"上下文状态"账户中，以供其他指令使用。
...
4. 关闭证明账户以回收创建它们所使用的 SOL。

下图展示了将代币从机密可用余额提取到公开余额的具体步骤：
...
crate 生成证明，通过 ZK
ElGamal 证明程序将每个证明验证至上下文状态账户，在提取指令中引用两个账户，并在之后关闭它们。TypeScript 示例使用来自
`@solana-program/token-2022/confidential` 的
`getConfidentialWithdrawInstructionPlan`
辅助函数，该函数将相同的证明账户创建、提取和关闭操作组合为一个多交易指令计划。

## 示例代码

以下示例将代币从机密可用余额提取回公开余额。该账户必须已配置为机密余额，并持有可用的机密余额。

机密余额依赖于 ZK ElGamal 证明程序，该程序已在主网和开发网上启用。标准的 `solana-test-validator` 不会启用它，但像 [Surfpool](https://surfpool.run) 这样的主网分叉本地 validator 则支持。请在其中一个环境中运行示例（代码使用开发网），并使用已充值的付款账户，同时将占位符替换为您的铸币地址和 token account。
