...
<Callout type="warn">
  ZK ElGamal Program
  目前在主网和开发网暂时禁用，正在进行安全审计。这意味着保密转账扩展当前不可用。虽然相关概念依然适用，但代码示例将无法运行。
</Callout>
...
1. 在客户端生成
   ...
   **等式证明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**：验证提取后剩余可用余额的密文与其对应的
   [Pedersen 承诺](https://en.wikipedia.org/wiki/Commitment_scheme)是否一致，确保账户新的可用余额正确计算为`remaining_balance = current_balance - withdraw_amount`。

   **区间证明 ([BatchedRangeProofU64Data](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_range_proof/batched_range_proof_u64.rs#L37))**：验证提取后剩余的可用余额为非负数且在指定区间内。

2. 对于每个证明：
   - 调用 ZK ElGamal 证明程序验证证明数据。
   - 将证明相关的元数据存储在证明“上下文状态”账户中，以便在其他指令中使用。
...
要将代币从保密可用余额提取到公开余额，你需要：

- 客户端生成等式证明和区间证明
- 调用 Zk ElGamal 证明程序验证证明并初始化“上下文状态”账户
- 调用
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  指令，并提供两个证明账户。
- 关闭两个证明账户以回收 rent。
...
- `confidential_transfer_create_context_state_account` 方法用于创建证明账户。
- `confidential_transfer_withdraw` 方法用于调用 `Withdraw` 指令。
- `confidential_transfer_close_context_state_account` 方法用于关闭证明账户。
...
<Callout type="info">
  截至撰写本文时，Confidential Transfers 尚未在默认本地 validator
