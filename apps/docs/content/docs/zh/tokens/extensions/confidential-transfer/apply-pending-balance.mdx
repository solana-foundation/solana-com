...
1. **机密待处理余额**：最初，代币从公开余额 "存入" 至 "待处理" 的机密余额。
2. **机密可用余额**：随后，待处理余额被 "应用" 至可用余额，使代币可用于机密转账。

本节介绍第二阶段：将待处理余额应用至可用余额。

当代币从公开余额 "存入"，或代币从一个 token account 机密转账至另一个 token
account 时，代币最初会被添加至机密待处理余额。在代币可用于机密转账之前，待处理余额必须被 "应用" 至可用余额。
...
Confidential Balances 依赖于 ZK ElGamal Proof 程序，该程序已在主网和开发网上启用。标准的 `solana-test-validator` 不会启用该程序，但主网分叉本地 validator（如 [Surfpool](https://surfpool.run)）则会启用。
请使用已充值的付款方，在上述环境之一（代码默认使用开发网）中运行示例，并将占位符铸币地址和账户地址替换为您自己的地址。
