...
<Callout type="warn">
  ZK ElGamal Program 目前在 mainnet 和 devnet
  上暂时禁用，正在进行安全审计。这意味着 confidential transfers
  扩展当前不可用。虽然相关概念依然有效，但代码示例将无法运行。
</Callout>

## 如何将待处理余额应用为可用余额

在代币能够进行机密转账前，公开代币余额必须转换为机密余额。该转换分为两个阶段：

1. **机密待处理余额**：最初，代币会从公开余额“存入”到“待处理”的机密余额中。
2. **机密可用余额**：然后将待处理余额“应用”到可用余额，使代币可用于机密转账。

本节将介绍第二阶段：如何将待处理余额应用为可用余额。

当代币从公开余额“存入”或从一个 token
account 机密转账到另一个时，代币最初会添加到机密待处理余额。在代币可用于机密转账前，必须先将待处理余额“应用”到可用余额。
...
方法，可用来构建并发送包含 `ApplyPendingBalance` 指令的交易，具体示例如下。

### 示例代码

以下示例演示如何将机密待处理余额应用到机密可用余额。
...
<Callout type="info">
  截至目前，默认本地 validator 尚未启用 Confidential Transfers。你必须克隆主网的
