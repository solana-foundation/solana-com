...
title: 创建 Token Mint
description:
  了解如何创建启用了保密余额的代币铸造账户。
---

## 如何创建带有保密余额的铸造账户

保密余额通过向 mint account 添加额外状态来实现私密代币转账。本节介绍如何创建启用了所需扩展的代币铸造账户。

下图展示了创建带有保密余额扩展的铸造账户所涉及的步骤：
...
- **authority**：有权更改铸造账户的保密余额设置，并在禁用自动审批时批准新的保密账户的账户。

- **auto_approve_new_accounts**：设置为 true 时，用户可以默认创建启用了保密余额的 token account。设置为 false 时，authority 必须在每个新 token account 可用于保密转账之前对其进行审批。

- **auditor_elgamal_pubkey**：一个可选的审计方，能够解密机密交易中的转账金额，在保护公众隐私的同时提供合规机制。

### 所需指令

创建启用了保密余额的铸造账户需要在单笔交易中执行三条指令：

1. **创建 Mint Account**：调用 System Program 的 _rs`CreateAccount`_
   指令以创建 mint account。

2. **初始化保密余额**：调用 Token Extensions Program 的
   ...
   指令，为铸造账户配置 _rs`ConfidentialTransferMint`_ 状态。
...
以下代码演示了如何创建一个启用了保密余额的铸造账户。

保密余额依赖于 ZK ElGamal Proof 程序，该程序已在主网和开发网上启用。标准的 `solana-test-validator` 不会启用它，但支持主网分叉的本地 validator（如 [Surfpool](https://surfpool.run)）会启用。请在上述环境之一中运行示例（代码使用开发网），并使用已充值的付款方，同时将占位符铸造地址和账户地址替换为您自己的地址。
