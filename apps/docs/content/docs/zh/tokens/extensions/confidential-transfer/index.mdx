...
<Callout type="warn">
  ZK ElGamal Program 目前在主网和 devnet
  上暂时禁用，正在进行安全审计。这意味着保密转账扩展当前不可用。虽然相关概念依然有效，但代码示例将无法运行。
</Callout>

## 什么是保密转账？

保密转账允许你在 token
account 之间转移代币时不公开转账金额。这对于保护隐私的交易非常有用。只有转账金额和 token 余额是私密的，token
account 地址仍然是公开的。

- [协议概览](https://www.solana-program.com/docs/confidential-balances/overview) - 了解底层密码学协议的细节
- [快速入门指南](https://www.solana-program.com/docs/confidential-balances#setup) - 设置和基础 CLI 命令
- [保密余额 Cookbook](https://github.com/solana-developers/Confidential-Balances-Sample) - 如何使用保密转账扩展的代码片段

### 它是如何工作的？

保密转账扩展为 Token
Extension 程序添加了[instructions](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)，允许你在账户之间转移代币时不公开转账金额。
...
1. 创建带有保密转账扩展的 mint account。
2. 为发送方和接收方创建带有保密转账扩展的 token account。
3. 向发送方账户铸造代币。
4. **存入**发送方的公共余额到**保密待处理余额**。
5. **应用**发送方的待处理余额到**保密可用余额**。
6. 从发送方 token account 向接收方 token account **保密转账**代币。
7. **应用**接收方的待处理余额到**保密可用余额**。
8. **提取**接收方的保密可用余额到**公共余额**。

有关保密转账流程各步骤的详细说明，请参阅以下相关页面：
...
<Cards>
  ...
  >
    如何在 token account 之间进行保密代币转账
  </Card>
  <Card
    title="发行方指南"
    href="/docs/tokens/extensions/confidential-transfer/issuer-guide"
  >
    如何发行和运营保密转账代币（审批策略、审计员、手续费、铸造与销毁）
  </Card>
</Cards>

下图展示了保密代币转账基本流程的详细顺序：
...
下面的 Rust 示例展示了与前述流程图对应的保密代币转账步骤。为便于理解，各步骤被拆分为辅助函数。

要运行该示例，请使用以下命令启动一个本地 validator，并从主网克隆 Token Extension
Program。你需要先安装 Solana CLI 才能使用本地 validator。
...
<Callout type="info">
  截至目前，默认本地 validator 尚未启用保密转账功能。你需要从主网克隆 Token
...
保密转账扩展的全部[instructions](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)如下：

| 指令                                | 说明                                                                                                                         |
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | 设置 mint account 以支持保密转账。此指令必须与 _rs`TokenInstruction::InitializeMint`_ 指令在同一笔交易中使用。               |
| _rs`UpdateMint`_                    | 更新 mint 的保密转账设置。                                                                                                   |
| _rs`ConfigureAccount`_              | 设置 token account 以支持保密转账。                                                                                          |
| _rs`ApproveAccount`_                | 如果 mint 需要新 token account 获得批准，则批准该 token account 进行保密转账。                                               |
| _rs`EmptyAccount`_                  | 清空待处理和可用的保密余额，以便关闭 token account。                                                                         |
| _rs`Deposit`_                       | 将公开代币余额转换为保密待处理余额。                                                                                         |
| _rs`Withdraw`_                      | 将可用的保密余额转换回公开余额。                                                                                             |
| _rs`Transfer`_                      | 在 token account 之间进行保密转账。                                                                                          |
| _rs`ApplyPendingBalance`_           | 存入或转账后，将待处理余额转换为可用余额。                                                                                   |
| _rs`EnableConfidentialCredits`_     | 允许 token account 接收保密代币转账。                                                                                        |
| _rs`DisableConfidentialCredits`_    | 阻止接收保密转账，但仍允许公开转账。                                                                                         |
...
| _rs`ConfigureAccountWithRegistry`_  | 通过使用 _rs`ElGamalRegistry`_ 账户而不是 _rs`VerifyPubkeyValidity`_ 证明，作为另一种方式配置 token account 以支持保密转账。 |
