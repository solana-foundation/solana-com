...
[指令](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)，使您能够在账户之间转移代币，而无需披露转账金额。
...
机密转账扩展的完整[指令](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)列表如下：

| 指令                                | 说明                                                                                                             |
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | 为机密转账配置 mint account。此指令必须与 _rs`TokenInstruction::InitializeMint`_ 指令包含在同一笔交易中。        |
| _rs`UpdateMint`_                    | 更新 mint 的机密转账设置。                                                                                       |
| _rs`ConfigureAccount`_              | 为机密转账配置 token account。                                                                                   |
| _rs`ApproveAccount`_                | 若 mint 要求对新 token account 进行审批，则批准该 token account 用于机密转账。                                   |
...
| _rs`ConfigureAccountWithRegistry`_  | 使用 _rs`ElGamalRegistry`_ 账户替代 _rs`VerifyPubkeyValidity`_ 证明，为机密转账配置 token account 的另一种方式。 |
