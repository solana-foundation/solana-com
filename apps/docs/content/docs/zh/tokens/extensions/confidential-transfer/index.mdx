...
title: 保密余额
description:
  了解保密余额扩展，以及如何使用它为 token mint 和账户添加可选功能。
---
...
- [协议概览](https://www.solana-program.com/docs/confidential-balances/overview) - 底层密码学协议的详细说明
- [快速入门指南](https://www.solana-program.com/docs/confidential-balances#setup) - 配置说明与基本 CLI 命令
- [保密余额使用手册](https://github.com/solana-developers/Confidential-Balances-Sample) -
  展示如何使用保密余额的代码片段

### 它是如何工作的？

保密余额扩展向 Token Extension 程序添加了
...
6. 以机密方式将代币从发送方 token account **转账**至接收方 token account。
...
保密余额授权方在初始化扩展时设置审计员密钥，或稍后通过 _rs`UpdateMint`_ 进行更改。授权方可以轮换或清除该密钥。轮换仅适用于未来的转账，因此在需要历史审计访问时请保留之前的私钥。审计员密钥提供读取权限，不授权代币转账，也不会泄露账户的完整保密余额。

有关设置和轮换示例，请参阅
[配置审计员](/docs/tokens/extensions/confidential-transfer/issuer-guide#configure-an-auditor)。
钱包、区块浏览器、交易所和合规服务提供商还应查阅
[审计员集成指南](/docs/tokens/extensions/confidential-transfer/integration-guide#auditors-and-compliance)。
...
| 指令                         | 说明                                                                                                                                                       |
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | 在 mint account 上启用保密余额。此指令必须与 _rs`TokenInstruction::InitializeMint`_ 指令包含在同一笔交易中。 |
| _rs`UpdateMint`_                    | 更新 mint 账户的审批策略及可选的全局审计员 ElGamal 公钥。                                                                        |
| _rs`ConfigureAccount`_              | 在 token account 上启用保密余额。                                                                                                                 |
| _rs`ApproveAccount`_                | 如果 mint 要求对新 token account 进行审批，则批准该 token account 使用保密余额。                                                          |
| _rs`EmptyAccount`_                  | 清空待处理和可用的机密余额，以便关闭 token account。                                                                         |
| _rs`Deposit`_                       | 将公开代币余额转换为待处理的机密余额。                                                                                                  |
| _rs`Withdraw`_                      | 将可用的机密余额转换回公开余额。                                                                                                   |
| _rs`Transfer`_                      | 在 token account 之间以机密方式转移代币。                                                                                                           |
...
| _rs`TransferWithFee`_               | 以机密方式在 token account 之间转移代币并收取手续费。                                                                                                |
| _rs`ConfigureAccountWithRegistry`_  | 使用 _rs`ElGamalRegistry`_ 账户代替 _rs`VerifyPubkeyValidity`_ 证明来配置 token account 保密余额的替代方式。         |
