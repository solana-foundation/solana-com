...
title: 转移 Token
description:
  了解如何将 token 从一个 token 账户私密地转移到另一个 token 账户。
---

## 如何将代币从一个token account机密转账至另一个token account

要将代币从一个 token account 机密转账至另一个 token account，发送方和接收方都必须拥有配置了 _rs`ConfidentialTransferAccount`_ 状态并获准使用机密余额的 token account。发送方的 token account 还必须具有可供转账的机密余额。
...
1. 在客户端创建
   ...
   **相等性证明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**：验证转账后新的可用余额密文与其对应的
   [Pedersen 承诺](https://en.wikipedia.org/wiki/Commitment_scheme)相匹配，确保来源账户的新可用余额被正确计算为
   `new_balance = current_balance - transfer_amount`。

   **密文有效性证明 ([BatchedGroupedCiphertext3HandlesValidityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_grouped_ciphertext_validity/handles_3.rs#L63))**：验证转账金额密文已为三方（来源方、目标方和审计方）正确生成，确保转账金额已在各方公钥下正确加密。

   **范围证明 ([BatchedRangeProofU128Data](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_range_proof/batched_range_proof_u128.rs#L37))**：验证新的可用余额和转账金额（拆分为低位/高位）均为非负数且在指定范围内。
...
以下示例将代币从一个账户机密转账至另一个账户。两个账户必须已配置好机密余额，且发送方必须具有可用的机密余额。

机密余额依赖于 ZK ElGamal Proof 程序，该程序已在主网和开发网上启用。标准的 `solana-test-validator` 不会启用该程序，但支持主网分叉的本地 validator（如 [Surfpool](https://surfpool.run)）则会启用。请使用已充值的付款账户，针对上述环境之一运行示例（代码默认使用开发网），并将占位符替换为您的铸造地址以及发送方和接收方账户。
