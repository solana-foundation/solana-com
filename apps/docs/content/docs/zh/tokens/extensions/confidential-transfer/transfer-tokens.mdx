---
title: 转移 Token
description: 了解如何将 token 从一个 token 账户私密地转移到另一个 token 账户。
---

<Callout type="warn">
  ZK ElGamal 程序目前在 mainnet 和 devnet
  上暂时禁用，正在进行安全审计。这意味着机密转账扩展当前不可用。虽然相关概念依然适用，但代码示例将无法运行。
</Callout>

## 如何在 token 账户之间机密地转移 token

要在 token 账户之间机密地转移 token，发送方和接收方都必须配置好带有
_rs`ConfidentialTransferAccount`_
状态并已获批进行机密转账的 token 账户。发送方的 token 账户还必须有可用的机密余额用于转账。

要机密地转移 token：

1. 在客户端生成[三种证明](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)：

   **等式证明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**：验证转账后新的可用余额密文与其对应的[Pedersen 承诺](https://en.wikipedia.org/wiki/Commitment_scheme)是否一致，确保源账户的新可用余额被正确计算为
   `new_balance = current_balance - transfer_amount`。

   **密文有效性证明 ([BatchedGroupedCiphertext3HandlesValidityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_grouped_ciphertext_validity/handles_3.rs#L63))**：验证转账金额的密文是否为三方（源、目标和审计方）正确生成，确保转账金额已根据每一方的公钥正确加密。

   **区间证明 ([BatchedRangeProofU128Data](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_range_proof/batched_range_proof_u128.rs#L37))**：验证新的可用余额和转账金额（按高低位拆分）均为非负且在指定区间内。

2. 对于每个证明：
   - 调用 ZK ElGamal 证明程序验证证明数据。
   - 将每个证明的元数据存储在证明“上下文状态”账户中，以便在其他指令中使用。
...
4. 关闭 proof context state 账户以回收用于创建它们的 SOL。

下图展示了将代币从发送方的 token account 转移到接收方的 token
account 所涉及的步骤。
...
要将代币从一个 token account 机密地转移到另一个 token account，你需要：

- 在客户端生成等式证明、密文有效性证明和区间证明
- 调用 Zk ElGamal proof 程序以验证这些证明并初始化 "context state" 账户
- 调用
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  指令，并提供三个 proof 账户。
- 关闭这三个 proof 账户以回收 rent。

`spl_token_client` crate 提供了以下方法：

- `confidential_transfer_create_context_state_account` 方法用于创建 proof 账户。
- `confidential_transfer_transfer` 方法用于调用 `Transfer` 指令。
- `confidential_transfer_close_context_state_account` 方法用于关闭 proof 账户。

### 示例代码

以下示例演示了如何在 token account 之间机密地转移代币。

要运行该示例，请使用以下命令启动一个本地 validator，并从主网克隆 Token Extension
...
<Callout type="info">
  截至目前，Confidential Transfers 尚未在默认的本地 validator
  上启用。你必须克隆主网的 Token Extension Program 才能运行示例代码。
</Callout>
