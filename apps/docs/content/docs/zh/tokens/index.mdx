...
_sh`spl-token`_ CLI 可帮助你体验 SPL token。下方示例使用
[Solana Playground](https://beta.solpg.io/)
终端，在浏览器中直接运行 CLI 命令，无需本地安装。

创建 token 和账户时需要 SOL 用于账户 rent 押金和交易手续费。首次使用 Solana
Playground 时，请创建一个 Playground 钱包，并在 Playground 终端运行
...
你在终端输出中看到的账户地址会与下方示例不同。跟随操作时，请使用你 Playground 终端中的地址。例如，_sh`create-token`_
命令会输出一个 mint account 地址，并将你的 Playground 钱包作为 mint authority。
...
用于新 token 的 _sh`supply`_ 命令会返回 `0`：
...
_sh`create-account`_ 命令会创建一个 associated token
account，并将你的钱包地址设为 owner。
...
authority 必须对交易进行签名。该指令会将代币铸造到 Token Account，并增加 Mint
Account 上的总供应量。这里有一个在
...
Token Program 上的 _rs`Transfer`_ 指令用于处理代币转账。发送方 Token
Account 的所有者必须对交易进行签名。该指令会在不同的 Token
Account 之间转移代币。这里有一个在
