...
Program 拥有的 [数据账户](/docs/core/accounts/account-types#data-accounts)。
...
account 的权限。这与基础
[Account](/docs/core/accounts/account-structure#account-fields)
...
在 Solana 上为代币添加元数据（名称、符号、图片）有两种方式：

1. **[Metaplex Token Metadata](/docs/tokens/metaplex)** - 创建一个与您的铸造账户关联的独立元数据账户。适用于原始 Token
   Program 和 Token-2022，并获得钱包和市场的广泛支持。

2. **[Token Extensions Metadata](/docs/tokens/extensions/metadata)** - 使用 Token-2022 将元数据直接存储在铸造账户上。这是一种更新的方法，具有内置支持。元数据扩展仅适用于使用 Token-2022 创建的 mint
   account。要为使用原始 Token Program 创建的 mint
   account 添加元数据，请使用 Metaplex Token Metadata Program。

#### SPL Token CLI 示例

Token Extensions Program 允许您将元数据直接存储在 Mint
Account 上。要创建带有元数据扩展的代币：
...
代币 URI 链接到链外元数据。请在[此处](https://raw.githubusercontent.com/solana-developers/opos-asset/main/assets/DeveloperPortal/metadata.json)查看 JSON 格式示例。
...
在
[Token Extensions 章节](/docs/tokens/extensions/metadata)中了解更多信息。有关 Token
Extensions 的详细信息，请参阅 [Token Extensions 章节](/docs/tokens/extensions)和
