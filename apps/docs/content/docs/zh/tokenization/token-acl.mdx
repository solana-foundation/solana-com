...
本指南包含一个完整的可运行实现，您可以在本地运行。
[源代码](https://github.com/Woody4618/access-control-list-acl-demo)
提供了可供探索和学习用途的参考实现。

ACL 程序的代码可在
[token-acl 仓库](https://github.com/solana-foundation/token-acl)中获取，
ABL Gate Program 可在
[abl-gate-program 仓库](https://github.com/solana-foundation/abl-gate-program)中获取。

**重要说明**：本指南中使用的 ABL（允许/屏蔽名单）Gate Program 是一个**参考实现**。虽然它已经过审计且可用于生产环境，但发行方可以自由创建更符合其特定合规需求的自定义 Gate Program。您仅受 Token ACL 规范（sRFC37）的约束，而非该特定 Gate Program 设计的约束。
