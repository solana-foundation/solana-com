...
Solana 交易有大小限制（约 1232 字节）。每条转账指令会增加约 200 字节。对于非常大型的多方结算，可以考虑使用
[地址查找表](/developers/cookbook/transactions/lookup-tables)
来压缩账户地址，从而在每笔交易中容纳更多转账。

即将推出的
[v1 交易格式](/docs/core/transactions/versioned-transactions#v1-message-format)
将限制提高至 4096 字节，但不再支持地址查找表，因此 v1 结算会直接内联其地址。无论哪种方式，运行时每笔交易最多加载 64 个账户。
