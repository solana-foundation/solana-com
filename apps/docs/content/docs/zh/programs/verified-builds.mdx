...
[Verify API](https://github.com/otter-sec/solana-verified-programs-api)，或者使用链上保存于
...
派生）中的验证数据，通过 `verify-from-repo` 命令在本地进行程序验证。
...
将保存验证者公钥，便于访问存储在验证 PDA 中的验证数据。包含所有构建和验证程序所需信息的 PDA，是由程序地址和验证者 pubkey 派生的。默认情况下，这与构建和部署该程序的 pubkey 相同，但也可以在
`security.txt` 中指定其他 pubkey。

`security.txt`
功能允许开发者将联系方式和安全信息直接嵌入到 Solana 智能合约中。该方法受
...
要将 `security.txt` 添加到你的 Solana 程序中，请按照以下步骤操作：
