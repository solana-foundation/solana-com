...
- **程序 ID（Program
  ID）**：您的程序在链上的地址。用户通过引用程序 ID 与您的程序进行交互。
- **程序账户（Program
  Account）**：存储程序元数据的链上账户。程序账户的地址就是程序 ID。程序账户还存储 ProgramData
  Account 的地址。
- **ProgramData Account**：存储程序已部署可执行代码的链上账户。
- **程序权限（Program
  Authority）**：有权更新或关闭程序的账户。默认情况下，这是您的 CLI 钱包。
...
程序 ID（Program
ID）是您的程序账户在链上的地址。用户和其他程序通过在交易中引用此 ID 来调用您的程序。
...
- **`--use-rpc`**：将交易发送到已配置的 RPC。此标志需要来自
  ...
  [stake-weighted](/docs/defi/stake-weighted-qos) RPC 连接。
