...
1. `.so` 文件（例如
   `hello_world.so`）：这是编译后的 Solana 程序，将作为“智能合约”部署到网络上。
2. keypair 文件（例如
...
在 `src/lib.rs` 中、程序代码下方添加如下测试。这是一个调用 hello
world 程序的测试模块。
...
使用 `cargo test` 命令运行测试。程序日志会显示 "Hello, world!"。
...
<Callout type="info">
  请注意，Solana Explorer 上的集群也必须为 localhost。Solana Explorer 的“Custom
