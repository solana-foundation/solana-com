...
用 Rust 编写的 Solana 程序在结构上几乎没有强制要求，因此代码组织方式非常灵活。唯一的要求是，程序必须有一个
`entrypoint`，用于定义程序执行的起始点。
...
1. `InitializeCounter`：创建并初始化一个带初始值的新账户。
2. `IncrementCounter`：递增已存在账户中存储的值。

为简化起见，程序将实现于单个 `lib.rs`
文件中，但实际开发中你可能会将较大的程序拆分为多个文件。
...
现在让我们添加构建 Solana 程序所需的依赖。我们需要 `solana-program`
作为核心 SDK，以及 `borsh` 用于序列化。
...
宏负责将 `input` 数据反序列化为 `process_instruction` 函数的参数。

一个 Solana 程序 `entrypoint` 具有如下函数签名。开发者可以自由实现自己的
`entrypoint` 函数。
...
`CounterInstruction` 枚举变体之一。Borsh `try_from_slice` 方法会自动完成此转换。
...
现在我们来更新主 `process_instruction` 函数，将指令路由到相应的处理函数。

这种路由模式在 Solana 程序中很常见。`instruction_data`
会被反序列化为表示指令的枚举变体，然后调用相应的处理函数。每个处理函数都包含该指令的实现。

将以下代码添加到 `lib.rs`，更新 `process_instruction` 函数并添加
`InitializeCounter` 和 `IncrementCounter` 指令的处理函数：
...
指令。新账户以我们的程序为所有者创建，这样我们的程序就可以写入该账户并初始化数据。
...
- 读取账户的 `data` 字段用于 `counter_account`
- 反序列化为 `CounterAccount` 结构体
- 将 `count` 字段加 1
...
首先，我们添加测试所需的依赖。我们将使用 `litesvm` 进行测试，还会用到
`solana-sdk`。
...
现在我们需要构建并加载程序到测试环境。运行 `cargo build-sbf`
...
<Callout type="warn">
  请确保 `edition` 在 `Cargo.toml` 中被设置为 `2021`。
</Callout>
...
将客户端代码添加到 `examples/client.rs`，并将 `program_id`
替换为上一个命令的输出：
...
你可以使用 `solana program show` 命令和你的程序 ID 来验证部署：
...
使用输出的交易签名查看交易。请注意，Solana Explorer 上的集群必须设置为“Custom
