...
为 64 字节，由账户私钥对交易的 `Message` 签名生成。每个包含在交易任一指令中的
...
<Callout>
  为了节省空间，交易不会为每个账户单独存储权限。账户权限是通过 `header` 和
  `account_keys` 来确定的。
</Callout>
...
后会过期。（假设每个区块为 400ms，相当于一分钟。）区块过期后，交易也会失效，无法被处理。

<Callout>
  [`getLatestBlockhash`](/docs/rpc/http/getlatestblockhash) RPC
  方法允许你获取当前的 blockhash 以及该 blockhash 有效的最后区块高度。
</Callout>
...
3. [`data`](mention:instruction-data)：一个字节数组，指定要在程序上调用的指令，同时包含指令所需的其他数据（如函数参数）。
