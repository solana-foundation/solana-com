...
- [`data`](mention:instruction-data)：包含指令所需额外 [数据] 的字节数组
...
结构体组成的数组。每个指令交互的账户都必须提供元数据。（这样只要不修改同一个账户，交易就可以并行执行指令。）

下图展示了包含单个指令的交易。该指令的 `accounts` 数组包含两个账户的元数据。

![一个包含一条指令的交易。该指令的 `AccountMeta` 结构体在其 `accounts` 数组中有两个。](/assets/docs/core/transactions/accountmeta.svg)
...
下方代码展示了前面代码片段的输出。不同 SDK 的格式可能不同，但请注意每条指令都包含以下三项必需信息：[`program_id`](mention:program-id)、[`accounts`](mention:accounts)、[`data`](mention:data)。
...
下面的示例展示了如何手动构建 transfer 指令。（ `Expanded Instruction`
选项卡在功能上等同于 `Instruction` 选项卡。）

<Callout type="info">
  实际上，通常无需手动构建 `Instruction`。
  大多数程序都提供带有辅助函数的客户端库，可以为你创建指令。如果没有可用的库，你也可以手动构建指令。
</Callout>
