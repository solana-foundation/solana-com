...
title: 交易结构
description:
  Solana 交易的结构，包括签名、消息格式、头部、账户地址、区块哈希、已编译指令、二进制编码和大小预算。
...
<Callout type="info" title="摘要">
  一笔交易包含签名和消息。消息由头部、账户地址、最近区块哈希以及已编译的指令组成。最大序列化大小：1,232 字节（即将推出的 v1 格式为 4,096 字节）。
</Callout>
...
<Callout type="info" title="v1 交易将上限提高至 4,096 字节">
  本页介绍的是旧版交易格式，v0 格式除版本前缀和地址查找表外也遵循相同结构。即将推出的
  [v1 格式](/docs/core/transactions/versioned-transactions#v1-message-format)
  将大小限制提高至 4,096 字节，并采用不同的线路布局。
</Callout>
...
<Callout type="info" title="旧版和版本化消息前缀">
  在旧版交易消息中，消息的第一个字节为 `num_required_signatures`，其后是另外两个 `MessageHeader` 字节。在版本化交易消息中，第一个字节为版本前缀；三字节的 `MessageHeader` 紧随该前缀之后。完整的 v0 和 v1 消息布局请参阅
