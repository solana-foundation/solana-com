...
bump seed 是在派生过程中附加到可选种子后的单字节(u8)。
[`find_program_address`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L800)
从 255 向下搜索到 1,用每个值调用
_rs`create_program_address`_,直到结果落在 Ed25519 曲线之外。第一个成功的值就是**规范 bump**。
...
4. 如果结果在曲线上，返回 _rs`PubkeyError::InvalidSeeds`_
   （此地址会有对应私钥，违背 PDA 的安全性）。
5. 如果结果不在曲线上，则将其作为 PDA 返回。

## 计算单元成本

用于 _rs`create_program_address`_ 的
[链上系统调用](https://github.com/anza-xyz/agave/blob/v3.1.8/syscalls/src/lib.rs#L798-L834)
每次调用收取
[1,500 个计算单元](https://github.com/anza-xyz/agave/blob/v3.1.8/program-runtime/src/execution_budget.rs#L200)。
...
派生 PDA 仅计算地址。它不会在该地址创建链上账户。账户必须通过单独的指令显式创建（通常通过 CPI 调用
...
<Callout type="warn">
  这与 `sol_try_find_program_address` 当前文档中记录的规范搜索行为不同,后者是从
  255 向下迭代到 1。
</Callout>
...
在此示例中,bump
255 会生成一个曲线上的地址并失败。第一个有效的 bump 是 254,因此它是规范 bump。
