...
如果你的 Ubuntu 终端看起来和下图一样，你可能会遇到一个问题：`ctrl + v`（粘贴快捷键）在终端中无法使用。
...
接下来，关闭 Windows
Terminal 并再次通过搜索 Ubuntu 打开 Linux 终端。此时终端应该如图所示，`ctrl + v`（粘贴快捷键）可以正常使用。
...
<Callout>
  你可以将 `stable` 替换为与你所需版本软件相匹配的发布标签（例如
  `v2.0.3`），或者使用以下三个符号通道名称之一：`stable`、`beta` 或 `edge`。
</Callout>

2. 添加 PATH 环境变量

首次安装 Solana CLI 时，可能会提示你添加 `PATH`
...
如果你在 Mac 上使用 `zsh`，运行安装过程中记录的默认 `export PATH`
命令，在关闭终端后不会生效。请通过运行以下命令，将 PATH 添加到你的 shell 配置文件中：
...
<Callout>
  上述步骤不适用于 `bash` 用户，因为对 PATH 变量的更改默认会被保留。
</Callout>
...
_shell`anchor init --test-template rust`_ 获取）
...
别忘了运行 _shell`avm use`_ 命令，声明你系统上要运行的 Anchor CLI 版本。
...
`solana-test-validator` 的升级替代品。你可以在
[Surfpool 文档](https://docs.surfpool.run/) 中了解 Surfpool 的更多功能。
