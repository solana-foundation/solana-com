...
只需一条命令，即可搭建本地 Solana 开发环境。该命令将在 Windows（WSL）、Linux 和 Mac 上安装 Rust、Solana CLI、Anchor Framework、Surfpool、Node.js 和 Yarn。
...
<Callout>
  如果安装命令失败，请[逐个安装各依赖项](/docs/intro/installation/dependencies)。
</Callout>
