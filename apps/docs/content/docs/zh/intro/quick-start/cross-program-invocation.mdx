...
首先，程序需要通过修改 _rs`Update`_ 结构体和 `update`
函数，实现一个简单的“付费更新”机制。

首先，更新 `lib.rs` 文件，引入 `system_program` 模块中的内容。
...
接下来，更新 _rs`Update`_ 结构体，新增一个名为 `vault_account`
的账户。该账户由程序控制，在用户更新消息账户时接收其转入的 SOL。
...
本节为 _rs`Update`_ 结构体新增了一个名为 `vault_account`
的账户。该账户作为程序控制的“金库”，在用户更新消息时接收其转入的 SOL。
...
- 该账户地址由使用种子 _rs`[b"vault", user.key().as_ref()]`_ 派生的 PDA 得到
- 作为 PDA，没有私钥，因此只有程序在执行 CPI 时才能为该地址“签名”
- 作为 _rs`SystemAccount`_ 类型，System Program 会像普通钱包账户一样拥有它
...
_rs`Transfer`_ 结构体指定了 System Program 转账指令所需的账户。

### !cpi_context

_rs`CpiContext`_ 结构体指定了用于跨程序调用（CPI）的程序和账户。
...
在 `update`_ 指令中，实现在内部通过跨程序调用（CPI）调用系统程序的 `transfer`_
...
`transfer`\_ 函数随后在系统程序上调用转账指令，传入：
...
CPI 的设置方式与客户端指令的构建方式一致，你需要为要调用的特定指令指定程序、账户和 instruction
data。当程序的 `update`\_ 指令被调用时，它会在内部调用系统程序的转账指令。
...
现在，通过更改 _rs`Delete`_ 结构体和 `delete`\_ 函数，添加“删除时退款”机制。

首先，更新 _rs`Delete`_ 结构体，包含
`vault_account`。这样，当用户关闭消息账户时，可以将金库中的所有 SOL 返还给用户。
...
`vault_account` 使用与 Update 结构体相同的 PDA 派生方式。

将 `vault_account`
添加到 Delete 结构体，可以让程序在删除指令时访问用户的金库账户，将所有累积的 SOL 返还给用户。
...
_rs`signer_seeds`_ 指定了用于派生 PDA 的可选种子和 bump seeds。

### !with_signer

_rs`with_signer()`_ 方法在 CPI 时传递签名者种子。
...
注意，_rs`_ctx: Context<Delete>`_ 会变为
_rs`ctx: Context<Delete>`_，以便在函数体内使用 context。
...
然后，_rs`transfer()`_ 函数会调用 System Program 上的转账指令，传入：
...
然后，更新 update 指令以包含 [`vaultAccount`](mention:one)
...
然后，更新 delete 指令以包含 [`vaultAccount`](mention:one)
...
点击 `Import` 按钮并输入项目名称，将其添加到你的 Solana
