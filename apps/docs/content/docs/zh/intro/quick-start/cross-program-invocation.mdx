...
的账户。该账户作为程序控制的“金库”，在用户更新消息时接收 SOL。
...
- 作为 PDA，没有私钥，因此只有程序在执行 CPI 时可以为该地址“签名”
- 作为 _rs`SystemAccount`_ 类型，System Program 像管理普通钱包账户一样管理它
...
_rs`transfer()`_ 函数会调用 System Program 的转账指令。
...
在 `update` 指令中，具体实现包含了一个跨程序调用（CPI），用于调用 System
Program 的 `transfer`
...
data。当程序的 `update` 指令被调用时，它会在内部调用 System Program 的转账指令。
...
现在，通过修改 _rs`Delete`_ 结构体和 `delete` 函数，添加“删除时退款”机制。

首先，更新 _rs`Delete`_ 结构体，加入
...
将 `vault_account`
添加到 Delete 结构体，使程序能够在删除指令时访问用户的金库账户，将所有累积的 SOL 返还给用户。
...
_rs`signer_seeds`_ 指定了用于派生 PDA 的可选 seeds 和 bump seeds。

### !with_signer

_rs`with_signer()`_ 方法在 CPI 时传递签名者 seeds。
...
注意，_rs`_ctx: Context<Delete>`_ 变为
...
_rs`transfer()`_ 函数随后在 System Program 上调用转账指令，传入：
...
然后，更新 update 指令，将 [`vaultAccount`](mention:one) 包含进去。
...
然后，更新 delete 指令，将 [`vaultAccount`](mention:one) 包含进去。
