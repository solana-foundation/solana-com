...
title: 创建确定性账户
description:
  学习如何使用 Program Derived Address (PDA) 和 Anchor 框架构建
  CRUD(创建、读取、更新、删除)Solana 程序。本分步指南演示如何使用 PDA
  创建、更新和删除链上消息账户,实现账户验证并编写测试。非常适合希望了解如何在
  Solana 程序中使用 PDA 的开发者。
...
在开始之前，请在 Playground 终端中运行
_shell`build`_，以检查起始程序是否能成功构建。
...
在 `lib.rs` 中，使用以下内容更新 _rs`MessageAccount`_ 结构体：
...
属性用于注解表示账户数据（存储在 Account 数据字段中的数据类型）的结构体。
...
_rs`message`_ 字段保存一个 _rs`String`_，其中包含用户的消息。

### !bump

_rs`bump`_ 字段存储一个 _rs`u8`_
["bump" seed](/docs/core/pda#canonical-bump)，用于派生 Program Derived
Address（PDA）。存储该值可以节省计算资源，无需在后续指令中重新计算。
...
在本例中， _rs`MessageAccount`_ 结构体存储了用户创建的消息，其中包含三个字段：

- `user` - _rs`Pubkey`_，用于标识创建消息账户的用户。
- `message` - _rs`String`_，包含用户的消息。
- `bump` - _rs`u8`_，用于存储用于派生 Program Derived Address（PDA）的
  ["bump" seed](/docs/core/pda#canonical-bump)。存储该值可以节省计算资源，无需在后续指令中重新计算。

创建账户时，程序会序列化 _rs`MessageAccount`_
数据，并将其存储在新账户的数据字段中。

之后，在读取账户时，程序会将这些数据反序列化回 _rs`MessageAccount`_
...
现在，添加用于创建和初始化 _rs`MessageAccount`_ 的 _rs`create`_ 指令。

首先，通过如下方式更新 _rs`Create`_ 结构体，定义该指令所需的账户：
...
_rs`mut`_ 约束声明该账户为可变。

### !init

_rs`init`_ 约束用于创建新账户。
...
_rs`bump`_ 约束声明 PDA 的 bump seed。

如果您未指定值，Anchor 会自动计算。

### !payer

_rs`payer`_ 约束指定由哪个账户支付新账户的创建费用。

### !space

_rs`space`_ 约束指定新账户数据字段分配的字节数。

### !signer

_rs`Signer<'info>`_ 类型要求账户对交易进行签名。

### !account

_rs`Account<'info, T>`_ 类型要求账户与指定类型匹配。

在本例中，账户必须与自定义的 _rs`MessageAccount`_ 类型匹配。

### !program

_rs`Program<'info, T>`_ 类型要求账户匹配某个程序。

在本例中，账户必须与 _rs`System`_ 类型匹配，该类型指的是 System Program。

### !instruction

_rs`#[instruction(message: String)]`_ 注解允许 _rs`Create`_ 结构体访问 `create`
指令中的 _rs`message`_ 参数。
...
在本例中， _rs`Create`_ 结构体定义了 _rs`create`_ 指令所需的账户。

1. _rs`user: Signer<'info>`_
   - 表示创建消息账户的用户
   - 需要可变状态（_rs`#[account(mut)]`_），因为该用户为新账户支付费用
...
3. _rs`system_program: Program<'info, System>`_
   - 账户创建所必需
   - 在底层，`init` 约束会调用 System Program 创建新账户，分配指定的
     `space`，并将所有者更改为当前程序。

---

_rs`#[instruction(message: String)]`_ 注解允许 _rs`Create`_ 结构体访问来自
`create` 指令的 _rs`message`_ 参数。

---

`seeds` 和 `bump` 约束共同将账户地址定义为 Program Derived Address (PDA)。
...
- _rs`user.key().as_ref()`_ - _rs`user`_ 账户的公钥，作为第二个 seed。

`bump` 约束会指示 Anchor 自动查找并使用正确的 bump seed。Anchor 会用 `seeds` 和
`bump` 来派生 PDA。

---

`space` 计算 _rs`(8 + 32 + 4 + message.len() + 1)`_ 会为 `MessageAccount`
数据类型分配空间：
...
_rs`String`_ 类型需要 4 字节来存储字符串长度，剩余部分为实际数据。

</Accordion>
</Accordions>

接下来，通过更新 `create` 函数，为 _rs`create`_ 指令添加业务逻辑：
...
1. _rs`ctx: Context<Create>`_ - 提供对 _rs`Create`_ 结构体中指定账户的访问。
2. _rs`message: String`_ - 用户要存储的消息。

函数主体执行以下逻辑：

1. 使用 _rs`msg!()`_ 宏向程序日志打印消息。
...
2. 初始化账户数据：
   ...
   - 设置用于派生 PDA 的 `bump` 值，该值来自 `ctx.bumps.message_account`。
...
接下来，添加 `update` 指令，以用新消息更改 `MessageAccount`。
...
_rs`realloc`_ 约束会重新分配账户的数据。

### !realloc::payer

_rs`realloc::payer`_ 约束指定为重新分配支付费用的账户。

### !realloc::zero

_rs`realloc::zero`_ 约束会将账户数据清零。
...
1. _rs`user: Signer<'info>`_
   - 表示更新消息账户的用户
   - 标记为可变（_rs`#[account(mut)]`_），因为在需要时可能需要为
     `message_account` 支付更多空间
   - 必须签署交易

2. _rs`message_account: Account<'info, MessageAccount>`_
   - 用于更新的现有用户消息账户
   - `mut` 约束表示该账户数据将被修改
   - `realloc` 约束允许调整账户数据大小
   - `seeds` 和 `bump` 约束验证该账户为正确的 PDA

3. _rs`system_program: Program<'info, System>`_
   - 账户空间可能重新分配时所需
   - `realloc` 约束会调用 System Program 以调整账户数据大小

请注意， _rs`bump = message_account.bump`_ 约束使用存储在 _rs`message_account`_
上的 bump seed，而不是让 Anchor 重新计算。

_rs`#[instruction(message: String)]`_ 属性使 _rs`Update`_ 结构体能够访问
_rs`update`_ 指令中的 _rs`message`_ 参数。
...
`update` 函数实现了修改已有消息账户的逻辑。它接受两个参数：

1. _rs`ctx: Context<Update>`_ - 提供对 _rs`Update`_ 结构体中指定账户的访问。
2. _rs`message: String`_ - 用于替换原有消息的新消息。
...
_rs`seeds`_ 约束指定了用于派生 PDA 的 seeds。

### !bump

_rs`bump`_ 约束指定了 PDA 的 bump seed。

在本例中，程序使用存储在 _rs`message_account`_ 上已有的 bump seed。

### !close

_rs`close`_ 约束用于关闭账户。

在本例中， _rs`user`_ 账户会收到已关闭 _rs`message_account`_ 的 lamports。
...
1. _rs`user: Signer<'info>`_
   - 表示关闭消息账户的用户
   - 标记为可变（_rs`#[account(mut)]`_），以便接收已关闭账户中的 lamports
   - 必须签名，以确保只有正确的用户可以关闭自己的消息账户

2. _rs`message_account: Account<'info, MessageAccount>`_
   - 要关闭的账户
   - `mut` 约束表示数据将被修改
   - `seeds` 和 `bump` 约束验证该账户为正确的 PDA
   - `close = user` 约束将该账户标记为待关闭，并将其 lamports 转移到 `user` 账户
...
1. _rs`_ctx: Context<Delete>`_ - 提供对 _rs`Delete`_ 结构体中指定账户的访问。
   _rs`_ctx`_ 语法表示该函数在主体中未使用 Context。

函数体仅使用 _rs`msg!()`_ 宏向程序日志打印消息。函数无需额外逻辑，因为
_rs`Delete`_ 结构体中的 _rs`close`_ 约束已处理账户关闭。
...
你现在已经完成了基础的 CRUD 程序。通过在 Playground 终端运行 `deploy`
来部署该程序。
...
起始代码还包含了一个测试文件，位于 `anchor.test.ts`。
...
请将以下代码添加到 _ts`describe()`_ 内，但要放在 _ts`it()`_ 部分之前。
...
在 Playground 终端中运行 _shell`test`_
来执行测试文件，以检查其是否按预期运行。接下来的步骤将添加实际的测试内容。
...
首先，测试文件发送一笔调用 `create` 指令的交易，并传递消息 "Hello, World!"。
...
发送交易并创建账户后，测试文件会使用其地址 (`messagePda`) 获取该账户。
...
首先，测试文件发送一笔调用 `update` 指令的交易，并传递消息 "Hello,
Solana!" 作为新消息。
...
在发送交易并更新账户后，测试文件会使用其地址 (`messagePda`) 获取该账户。
...
首先，此代码发送一笔交易，调用 `delete` 指令以关闭消息账户。
...
在发送交易并关闭账户后，示例会尝试使用其地址 (`messagePda`) 和 `fetchNullable`
获取该账户。当账户关闭后不再存在时，此方法会返回 null。
...
运行测试文件。该命令会针对部署在 devnet 上的程序运行测试，并记录 SolanaFM 的链接以查看交易详情。
...
<Callout type="info">
  请注意，在本例中，如果你再次运行测试，`create` 指令会失败，因为
  `messageAccount` 已经作为账户存在。对于给定的 PDA，只能存在一个账户。
</Callout>
