...
`src/lib.rs` 源文件将会打开。它包含一个名为 `Initialize` 的结构体，其功能如下：
...
函数体会[将 `data` 字段设置为 `new_account`](mention:three) 提供的 `data`
参数，然后向程序日志打印一条消息。
...
- [`#[derive(Accounts)]`](mention:one)
  属性定义了一个结构体，用于指定某个指令所需的账户。每个字段代表一个独立的账户。
...
<Callout>
  你的 [program ID](/docs/intro/quick-start/deploying-programs)，定义在
  _rs`declare_id!()`_，现在将被更新。
</Callout>
...
**注意：** 如果需要，你可以使用左侧面板上的 `Test` 按钮。

![运行测试](/assets/docs/intro/quickstart/pg-test.png)

4. 运行 `solana confirm -v [TxHash]` 以查看交易日志
...
{/* CODE_PLACEHOLDER_8bc4245b2bb4ee5931d2d6ac95f50814 */}，并使用存储在
_rs`declare_id!()`_ 宏中的程序地址。
