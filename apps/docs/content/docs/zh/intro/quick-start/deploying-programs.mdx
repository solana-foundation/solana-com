...
会打开 `src/lib.rs` 源文件。该文件包含一个名为 `Initialize`
的结构体，其功能如下：
...
访问 `data` 字段（属于 `new_account` 账户），并将其设置为 `data: u64` 的值

### !msg

_rs`msg!`_ 宏会将消息记录到交易的程序日志中
...
- [`#[program]`](mention:one) 属性用于标注包含代表程序指令函数的模块。
...
- `ctx: Context<Initialize>`：将本指令所需的账户传递给函数，具体账户在
  `Initialize` 结构体中指定。
- `data: u64`：调用指令时你必须提供的自定义参数。

函数体会[将 `data` 字段（属于 `new_account`）](mention:three) 设置为传入的
`data` 参数，然后向程序日志打印一条消息。
...
- [`#[derive(Accounts)]`](mention:one)
  属性定义了一个 struct，用于指定某个指令所需的账户。每个字段代表一个独立的账户。

  Anchor 会根据字段类型（例如 `Signer<'info>`）和约束条件（例如
...
- [`#[account]`](mention:one)
  属性定义了一个 struct，表示该程序创建和拥有的账户数据类型。
...
<Callout>
  你的 [program ID](/docs/intro/quick-start/deploying-programs)，在
  _rs`declare_id!()`_ 中定义，现在将会被更新。
</Callout>
