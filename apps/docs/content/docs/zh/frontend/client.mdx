...
<Callout type="info">
  在无头模式下运行——在没有浏览器钱包的 API 路由、Worker 或脚本中？
  请使用后端密钥管理服务而非原始 keypair 进行签名。请参阅
...
- **无头状态机**：在 API 路由、脚本或 Worker 中运行客户端，以复用驱动 UI 的相同钱包 +
  RPC 编排逻辑。
- **实时仪表盘**：将观察器（余额、账户、签名）与您偏好的 UI 库结合使用；客户端负责处理 WebSocket 分发和缓存失效。
- **自定义存储**：注入您自己的 Zustand 存储，以从 IndexedDB/localStorage 进行水合、将状态镜像到服务器会话，或在浏览器标签页之间进行协调。
- **桥接至 React
  Hooks**：当您希望在相同运行时之上使用符合人体工学的 Hooks 时，将已配置的客户端实例传递给
  `@solana/react-hooks`。
- **可测试性**：单一客户端接口可在单元测试中进行模拟，使其易于在无需浏览器钱包的情况下模拟 RPC 响应或钱包会话。
