...
<Callout type="info">
  以无头模式运行——在没有浏览器钱包的 API 路由、Worker 或脚本中？
...
- **无头状态机**：在 API 路由、脚本或 Worker 中运行客户端，复用驱动 UI 的同一套钱包 +
  RPC 编排逻辑。
- **实时仪表盘**：将监听器（余额、账户、签名）与您偏好的 UI 库结合使用；客户端负责处理 WebSocket 扇出与缓存失效。
- **自定义存储**：注入您自己的 Zustand
  store，以从 IndexedDB/localStorage 注水、将状态镜像到服务器会话，或在浏览器标签页之间协调。
- **桥接 React
  Hooks**：当您希望在同一运行时之上使用符合人体工程学的 Hooks 时，将已配置的客户端实例传递给
  `@solana/react-hooks`。
- **可测试性**：单一客户端接口可在单元测试中进行模拟，无需浏览器钱包即可轻松模拟 RPC 响应或钱包会话。
