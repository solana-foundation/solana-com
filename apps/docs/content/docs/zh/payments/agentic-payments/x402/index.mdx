...
下方列出了[可用的 402 SDK](#sdk-及其-solana-支持情况) 及其当前对 Solana 的支持情况。
...
- **高分辨率图片**：付款后解锁全分辨率下载（参见 [ACK 示例](#ack)）或 [x402 coinbase 示例](https://github.com/x402-foundation/x402/tree/main/typescript/packages/http/paywall)
...
| [原生示例](#原生示例)       | 是                      | 无依赖的最简示例                       | [示例](https://github.com/Woody4618/x402-solana-examples) |
...
<Callout type="info" title="web3.js (legacy) example">
  This third-party example uses `@solana/web3.js` (legacy). If you port it to
  web3.js v3, `Keypair.fromSecretKey` is async: use `await
