...
与其使用最近的区块哈希，不如使用 nonce 值作为交易的生命周期。为此，我们首先需要从 nonce 账户获取 nonce 值，然后用它来设置交易的生命周期。

### 获取 Nonce 值

首先，从 nonce 账户获取 nonce 值：
...
2. 自动在所有耐用 nonce 交易的第一条指令处添加 `advanceNonceAccount`
   指令（这是必需的）
...
将序列化后的字符串存储到数据库中——只要 nonce 未被推进，它就一直有效。
...
交易可以被序列化、存储，并在审批者之间传递。收集到所有必需签名后，即可提交到网络。
...
<Callout type="caution">
  每个 nonce 只能使用一次。如果交易失败或你决定不提交，必须在使用同一 nonce
  账户准备下一个交易前推进 nonce。
</Callout>

## 推进已用或废弃的 Nonce

如需使待处理交易失效或准备 nonce 重新使用，请手动推进：
...
- 创建一组 Nonce 账户以并行准备交易
- 跟踪哪些 Nonce 正在“使用中”（即有待签名的交易）
- 在交易提交或放弃后实现 Nonce 回收机制
