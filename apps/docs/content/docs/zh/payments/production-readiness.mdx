...
对于生产环境的后端签名，请考虑使用
[solana-keychain](https://github.com/solana-foundation/solana-keychain)——一个统一的签名库，它通过单一接口抽象了多个密钥管理后端：Memory、Vault、Privy、Turnkey、AWS
KMS、Fireblocks、GCP
KMS、CDP、Para、Dfns。这使您能够在本地使用内存密钥进行开发，然后无需更改应用程序代码即可切换到生产级后端。
