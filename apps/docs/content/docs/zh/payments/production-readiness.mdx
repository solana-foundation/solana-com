...
对于生产环境的后端签名,建议使用
[solana-keychain](https://github.com/solana-foundation/solana-keychain)——这是一个统一的签名库,通过单一接口抽象了多个密钥管理后端:
Memory、Vault、Privy、Turnkey、AWS KMS、Fireblocks、GCP
KMS、CDP、Para、Dfns、Crossmint。这使您可以在本地使用内存密钥进行开发,然后在不更改应用程序代码的情况下切换到生产级后端。
