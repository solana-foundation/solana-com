...
- `amountPerPeriod` 以基本单位表示。对于6位小数的代币，`1_000_000` 表示 `1`
  个代币。
- 程序会拒绝超出当前周期剩余限额的转账。
- 下一个周期开始后，已提取金额将重置。
- `expiryTs`
  是硬性截止时间：一旦超过该时间，转账将失败，赞助方可以收回 rent。不存在支出时间宽限期。
- 将 `startTs` 设置为
  `0`，可在交易上链时启动委托，而非在固定时间启动。这样可以扩大让用户签名和完成注册的时间窗口。执行此操作时，`expiryTs`
  必须为非零值——在上链时启动的委托不能同时设置为永不过期。
