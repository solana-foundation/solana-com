...
- 为每个用户和代币铸造对创建一个订阅授权。
- 对同一铸造的后续委托重用现有的订阅授权。
- 赞助方可以为账户提供资金：向 `initSubscriptionAuthority` 传入可选的
  `payer`（也支持 `createFixedDelegation`、`createRecurringDelegation` 和
  `subscribe`），或使用 `.use(payer(sponsorSigner))`
  进行配置。rent 随后会在您使用匹配的 `receiver`
  关闭时返还给该赞助方。若使用自筹资金的默认方式，则省略此项。
- 仅在依赖该订阅授权的所有委托均已撤销或关闭后，再关闭订阅授权。
