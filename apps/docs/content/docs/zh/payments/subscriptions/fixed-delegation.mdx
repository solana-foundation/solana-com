...
- 用户的token account必须在初始化之前存在。
- 金额以基本单位表示。对于6位小数的代币，`1_000_000`表示`1`个代币。
- 仅当Subscription Authority账户尚不存在时，才运行`initSubscriptionAuthority`。
- `expiryTs`是硬性截止时间：一旦超过，转账将失败，赞助商可以收回rent。没有消费时间宽限期。
- 用户签署设置和撤销交易。被委托方签署转账交易。
