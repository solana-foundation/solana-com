...
在 Solana 上,程序是管理账户状态的可执行逻辑。代币账户由**代币程序**管理——这是链上代码,用于原子性地验证转账并更新余额。
...
Token-2022（也称为"Token Extensions"）增加了转账钩子、转账费用和隐私余额等功能。两个程序在基本转账方面的工作方式类似，但在推导 ATA 时必须使用正确的程序。
...
<Callout type="info">
  Solana 采用本地费用市场——交易与其他写入相同状态的交易相互竞争。这意味着即使在网络其他部分活动频繁的时期，支付费用也能保持低廉且可预测。
</Callout>
