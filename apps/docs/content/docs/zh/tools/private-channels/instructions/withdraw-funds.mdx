...
title: 提取资金
description:
  销毁通道侧代币以发起提款，将资金返还至 Solana 主网托管合约
---
...
- 此指令属于 **Withdraw Program**
  (`J231K9UEpS4y4KAPwGc4gsMNCjKFRMYcQBcjVW7vBhVi` 开发网），而非 Escrow
  Program
- 在通道侧销毁代币**不会**自动释放
  主网上的资金；操作员随后必须携带有效的 SMT 排除证明调用 Escrow
  Program 上的 `ReleaseFunds`
- 若 `amount` 为 0，将抛出 `ZeroAmount` 错误
- 若 `destination` 为 `None`，操作员的 `ReleaseFunds` 调用将把
  资金释放至签名者的钱包
