...
title: 安装
description:
  配置您的环境并生成 Private Channels TypeScript 客户端
---
...
TypeScript 客户端**未**发布到 npm，必须使用 Codama 从 IDL 生成。
...
在您的应用中从这些本地路径导入；npm 上没有 `@private-channels/client` 包。本节的代码示例使用相对路径（`../private-channel-escrow-program/...`），假设您的代码与克隆的代码仓库位于同一目录；请根据您项目中实际的导入路径进行调整。
...
- 托管程序 ID：`9tgHa1DcnaSSUtmMsst8ovKTe1Gfxzezn27KnH9xXYeU`
- 提款程序 ID：`J231K9UEpS4y4KAPwGc4gsMNCjKFRMYcQBcjVW7vBhVi`

## 后续步骤

**[存入代币](/docs/tools/private-channels/quickstart/deposit)** - 完成您的首次存款
