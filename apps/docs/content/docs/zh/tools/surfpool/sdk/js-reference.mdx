...
title: JS API 参考文档
description:
  "@solana/surfpool 包的完整公共 API —— Surfnet 类、配置类型、部署选项及事件结构。"
---
...
<Callout type="info" title="Kit 入口点">
  以下符号是 `@solana/surfpool` 的根导出项。`@solana/surfpool/kit` 子路径额外提供了 `surfpool()`、`surfnetCheatcodes()`、`createSurfnetCheatcodesRpc()`、`DEFAULT_SURFNET_ENDPOINT`、插件配置类型以及生成的作弊码线路类型——这些均记录在 [Kit 插件](/docs/tools/surfpool/sdk/kit-plugin) 页面，而非本表格中。
</Callout>
