...
> _注意：当 `fallback_if_unavailable` 为 true 时，系统允许在 Redis 临时不可用的情况下继续处理交易，
> 从而防止服务中断。
> 将 `max_transactions` 设置为 0 将允许无限次交易。_
...
| `get_payer_signer`          | 返回下一个手续费支付方及付款地址        | ✅       | boolean |
...
| 扩展名称               | 描述                                      |
| ---------------------------- | ------------------------------------------------ |
| `confidential_transfer_mint` | 铸币的机密余额配置 |
...
| 扩展名称                  | 描述                                 |
| ------------------------------- | ------------------------------------------- |
| `confidential_transfer_account` | 账户的机密余额状态 |
