...
测试自定义 Solana 程序需要将编译好的 `.so` 文件加载到 SVM 中，配置程序所需的账户，并向其发送指令。LiteSVM 在进程内完成所有这些操作——无需 validator。我们推荐使用
[Codama JS Renderer](https://github.com/codama-idl/renderers-js) 生成程序客户端，以便访问兼容的 Kit 插件。
...
1. **程序加载**：使用 `addProgramFromFile` 加载已编译的程序，并从您的客户端导入 myProgramPlugin()
2. **配置**：根据需要启用 sysvars、内置程序和预编译程序
3. **账户设置**：使用 `setAccount` 预填充数据账户
4. **发送**：使用 `client.myProgram.testInstruction({..}).sendTransaction()` ——无需手动构建交易
5. **验证**：在执行后检查账户状态
   (`client.myProgram.myAccount.fetch`)

一旦这个基础框架运行正常，您便可以通过添加更多指令和断言来构建完整的测试套件。
