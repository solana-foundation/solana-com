---
title: 断言
description: 使用断言辅助工具验证账户状态、余额和所有权
---
...
<Callout type="info">
  `assert_account_closed` 在账户不存在、lamport
  余额为零或数据为空时通过。使用此方法验证关闭账户后的清理工作。
</Callout>
...
<Callout type="info">
  常见账户大小：- **铸币账户**：82 字节 - **token account**：165 字节 -
  **系统账户**：0 字节（仅含 lamport）
</Callout>
...
<Callout type="warn">
  断言旨在以清晰的错误消息快速失败。在测试中大量使用它们，以便尽早发现问题。
</Callout>
