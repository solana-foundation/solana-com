...
返回 `SimulatedTransactionInfo`，其中包含：

- `meta`：交易元数据（日志、计算单元、签名等）
- `post_accounts`：模拟后的账户状态
...
`send_transaction()` 直接返回 `TransactionMetadata`，而 `simulate_transaction()`
返回 `SimulatedTransactionInfo`，其中包含 `meta: TransactionMetadata`：
...
从历史记录中查询交易。需要设置 `with_transaction_history()`。
