...
`@solana-commerce/connector` 包提供了基于 [Wallet Standard](https://github.com/wallet-standard/wallet-standard) 构建的无头钱包连接功能。它负责管理钱包发现、连接状态、账户管理以及自动重连。该包设计为框架无关，并内置了 React 绑定。它专为在 Solana Commerce Kit 中无缝使用而定制，并与 [@solana/kit](https://github.com/anza-xyz/kit) 兼容。
