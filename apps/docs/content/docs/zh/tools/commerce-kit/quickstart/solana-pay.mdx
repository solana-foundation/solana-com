...
`@solana-commerce/solana-pay` 软件包为构建支付体验提供完整的
[Solana Pay](https://solanapay.com) 功能，与
[`@solana/kit`](https://github.com/anza-xyz/kit) 库兼容。它处理 URL
编码/解析、带有自定义样式的二维码生成，以及 SOL 和 SPL 代币转账的交易构建。
...
- **`recipient`**（`Address`，必填）- 将接收付款的商家钱包地址
（base58 编码的公钥）。可以是字符串或
`@solana/kit` 中的 `Address` 类型。
...
构建用于支付转账的完整 Solana 交易消息。此函数根据 `splToken` 参数自动检测应创建 SOL 还是 SPL 代币转账，并构建所有必要的指令。该函数使用来自 RPC 客户端的最新区块哈希为交易设置区块哈希生命周期，并返回完整的未签名交易，可用于签名并提交至 RPC（类型为 `TransactionMessageWithBlockhashLifetime`，与 Solana Kit 兼容）。

#### 参数

- **`rpc`**（`Rpc<SolanaRpcApi>`）- 来自 `@solana/kit` 的 Solana RPC 客户端。使用 `createSolanaRpc(rpcUrl)` 创建。
