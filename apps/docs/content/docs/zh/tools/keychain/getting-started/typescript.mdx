...
Fordefi 有两种签名模式，通过是否传入 `chain` 来选择。API 请求使用 ECDSA P-256 请求签名进行身份验证——可通过 `privateKeyPem` 提供签名密钥，或通过传入 `requestSigner` 将其保存在 KMS/HSM 中。

**原生 Solana 模式**（推荐用于链上操作）：设置 `chain`，Fordefi 将自行签名并广播交易。由于 Fordefi 在签名前可能会替换区块哈希或手续费，因此签名者为 Kit 的 `TransactionSendingSigner`——请与 `signAndSendTransactionMessageWithSigners` 配合使用，而非上述的部分签名者流程。
...
**黑盒模式**：省略 `chain`。Fordefi 对原始字节进行签名，不负责广播；您需自行组装并提交交易。请将此模式与 Fordefi 黑盒保险库配合使用。
