...
Het onderstaande voorbeeld demonstreert hoe je de Solana SDK gebruikt om een
...
Dit veld wordt vaak "account data" genoemd. De `data` in dit veld wordt als
willekeurig beschouwd omdat het elke reeks bytes kan bevatten. Elk programma
definiëert de structuur van de data die in dit veld is opgeslagen.
...
1. Het account ophalen met behulp van zijn [adres](#account-address)
2. Het `data` veld van het account deserialiseren van ruwe bytes naar de juiste
   datastructuur, zoals gedefinieerd door het programma dat eigenaar is van het
   account.
...
zijn eigenaar. De eigenaar van het account is het enige programma dat de `data`
van het account kan wijzigen of lamports kan aftrekken, zoals aangegeven door de
...
een programma-account te deployen. Het `data` van het programma-account bevat de
...
Programma's die zijn gedeployed met loader-v3 bevatten geen programmacode in hun
`data` veld. In plaats daarvan wijst hun `data` naar een apart **program data
account**, dat de programmacode bevat. (Zie het onderstaande diagram.)
...
gefinancierd met SOL. Na het uitvoeren van de code zie je dat het adres van de
`owner` van het account `11111111111111111111111111111111` is (het
