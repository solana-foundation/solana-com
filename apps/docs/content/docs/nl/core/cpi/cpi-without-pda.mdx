...
description:
  Hoe de invoke-functie te gebruiken voor cross-program invocations op Solana
  wanneer geen PDA-ondertekening vereist is, met Anchor CpiContext en Native
  Rust voorbeelden.
...
<Callout type="info" title="Samenvatting">
  Gebruik _rs`invoke`_ voor CPI's waarbij alle vereiste ondertekenaars de
...
functie gebruikt. De _rs`invoke`_ functie roept de _rs`invoke_signed`_ functie
aan met een lege _rs`signers_seeds`_ array. De lege ondertekenaars-array geeft
...
De onderstaande voorbeelden maken een CPI met Anchor en Native Rust. Het bevat
een enkele instructie die SOL overdraagt van het ene account naar het andere.
...
- **Voorbeeld 3**: Construeert de CPI-instructie handmatig. Deze benadering is
  nuttig wanneer er geen crate bestaat om de instructie te helpen bouwen die je
  wilt aanroepen.
...
Het volgende voorbeeld toont hoe een CPI te maken vanuit een programma
geschreven in Native Rust. Het bevat een enkele instructie die SOL overdraagt
van het ene account naar het andere. (Het testbestand gebruikt
