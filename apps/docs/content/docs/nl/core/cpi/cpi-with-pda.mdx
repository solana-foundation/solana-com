---
title: CPI's met PDA-ondertekenaars
description:
  Hoe de invoke_signed functie te gebruiken voor cross-program invocations op
  Solana wanneer het aanroepende programma moet ondertekenen als een PDA, met
  Anchor CpiContext en Native Rust voorbeelden.
...
<Callout type="info" title="Samenvatting">
  Gebruik _rs`invoke_signed`_ wanneer het aanroepende programma namens een PDA
...
met de [ondertekeningsseeds](mention:signer-seeds) die gebruikt worden om de
[PDA](/docs/core/pda) af te leiden. Voor details over hoe de runtime
...
maken van een PDA naar een ontvangeraccount, met behulp van een CPI ondertekend
door de PDA.

#### Anchor

De volgende voorbeelden tonen twee benaderingen voor het implementeren van CPI's
in een [Anchor](https://www.anchor-lang.com/docs)-programma. De voorbeelden zijn
functioneel equivalent, maar demonstreren verschillende abstractieniveaus.

- **Voorbeeld 1**: Gebruikt Anchor's _rs`CpiContext`_ en helperfunctie.
- **Voorbeeld 2**: Gebruikt de _rs`system_instruction::transfer`_-functie uit
  _rs`solana_program`_-crate. (Voorbeeld 1 is een abstractie van deze
  implementatie.)
- **Voorbeeld 3**: Construeert de CPI-instructie handmatig. Deze benadering is
...
overdraagt van een PDA-account naar een ander. De CPI wordt ondertekend door het
PDA-account. (Het testbestand gebruikt
