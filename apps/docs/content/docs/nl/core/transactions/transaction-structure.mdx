...
<Callout type="warn" title="Controleer de ontvanger voordat je SOL verstuurt">

Een System Program-overdracht voegt lamports toe aan **elk** account. Er is geen
protocol-niveau controle of de ontvanger de SOL er weer uit kan halen. Lamports
kunnen alleen worden verplaatst door het bezittende programma van het account,
dus het versturen van SOL naar een
[token mint](/docs/tokens/basics/create-mint), een programma, of een
[PDA](/docs/core/pda) die je niet beheert **brengt het risico van permanent
verlies van fondsen met zich mee** — alleen een autoriteit die is opgegeven door
het bezittende programma kan ze terugsturen. SOL gestuurd naar een
...
### Controleer de ontvanger voordat je overdraagt

Omdat een SOL-overdracht naar elk account slaagt, controleer de ontvanger
voordat je ondertekent. Haal het account op en stuur alleen naar een System
Program-portemonnee (of een onbegroot
...
<Callout>
  Dit fragment controleert ontvangers van native SOL. Voor de volledige
  classificatie die ook SPL token-verzendingen afhandelt (token accounts, ATA's,
  Token-2022), zie [Adres
  verifiëren](/docs/payments/send-payments/verify-address).
</Callout>

## Transactiedetails ophalen

Na het indienen kunt u transactiedetails ophalen met behulp van de
transactiehandtekening en de [getTransaction](/docs/rpc/http/gettransaction)
RPC-methode.

<Callout>
  U kunt de transactie ook vinden via [Solana
