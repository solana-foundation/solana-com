...
description:
  Maak een token mint aan en configureer de decimalen, mint authority en freeze
  authority van de mint.
...
Een mint account definieert en identificeert uniek een token op Solana, en slaat
de gedeelde staat op die van toepassing is op alle token accounts voor die mint.

Het Token Program definieert het _rs`Mint`_ accounttype als:
...
Elke token heeft één mint account, en het mint-adres is de unieke identificatie
van de token binnen wallets, applicaties en explorers.

USD Coin (USDC) heeft bijvoorbeeld het mint-adres
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`. Het mint-adres identificeert
USDC uniek binnen het gehele Solana-ecosysteem. Je kunt deze mint bekijken op
...
Het aanmaken van een mint vereist twee instructies:

1. De _rs`CreateAccount`_ instructie van het System Program maakt een nieuw
   rent-vrijgesteld account aan en wijst het Token Program toe als de program
   owner van het nieuwe account.
2. De _rs`InitializeMint`_ of _rs`InitializeMint2`_ instructie van het Token
   Program initialiseert het nieuwe account als een mint.

Neem de _rs`CreateAccount`_ instructie en de mint-initialisatie-instructie op in
dezelfde transactie.

Tijdens de mint-initialisatie controleert het Token Program of het mint account
nog niet is geïnitialiseerd en rent-vrijgesteld is. Het Token Program schrijft
vervolgens de mint authority, freeze authority, decimalen en de
_rs`is_initialized`_ vlag naar de mint account data.

### Bronverwijzing

| Item                           | Beschrijving                                                                                      | Token Program                                                                                                | Token Extensions Program                                                                                           |
| ------------------------------ | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Mint`_                     | De basale mint-staat die in elk mint account wordt opgeslagen.                                    | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`InitializeMint`_           | Een mint-initialisatie-instructie die het rent sysvar account in zijn accountslijst verwacht.     | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L24-L44)   | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L50-L74)   |
| _rs`InitializeMint2`_          | Een mint-initialisatie-instructie die het rent sysvar account niet vereist in zijn accountslijst. | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L401-L414) | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L471-L486) |
| _rs`_process_initialize_mint`_ | Gedeelde processorlogica voor mint-initialisatie.                                                 | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
| _rs`process_initialize_mint`_  | Publieke handler voor _rs`InitializeMint`_.                                                       | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L62-L70)       | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L132-L140)     |
| _rs`process_initialize_mint2`_ | Publieke handler voor _rs`InitializeMint2`_.                                                      | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L72-L81)       | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L142-L151)     |

### Typescript

De `Kit` voorbeelden hieronder tonen de aanbevolen aanpak met `@solana/kit`.
...
## Hoe Metadata Toevoegen

Een mint account slaat alleen basis tokeninformatie op (supply, decimalen,
authorities). Om leesbare metadata zoals een naam, symbool en afbeelding aan je
token toe te voegen, heb je twee opties:

<Cards>
  <Card title="Metaplex Token Metadata" href="/docs/tokens/metaplex">
   Voeg metadata toe met het Metaplex Token Metadata Program. Werkt met zowel het originele Token Program als Token-2022.
  </Card>

  <Card title="Token Extensions Metadata" href="/docs/tokens/extensions/metadata">
   Gebruik de ingebouwde metadata-extensie met Token-2022. 
   De metadata-extensie is alleen beschikbaar voor mint accounts die zijn aangemaakt met Token-2022.
   Om metadata toe te voegen aan mint accounts die zijn aangemaakt met het originele Token Program, gebruik je het Metaplex Token Metadata Program.
  </Card>
</Cards>
