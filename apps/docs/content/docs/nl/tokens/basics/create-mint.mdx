...
description:
  Maak een token mint en configureer de decimalen, mint authority en freeze
...
Een mint account definieert en identificeert een token op Solana uniek, en slaat
de gedeelde status op die van toepassing is op alle token accounts voor die
mint.
...
van de token in wallets, applicaties en explorers.

USD Coin (USDC) heeft bijvoorbeeld het mint-adres
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`. Het mint-adres identificeert
USDC uniek in het gehele Solana-ecosysteem. Je kunt deze mint bekijken op
...
1. De _rs`CreateAccount`_-instructie van het System Program creëert een nieuw
   rent-exempt account en wijst het Token Program toe als de programma-eigenaar
...
Voeg de _rs`CreateAccount`_-instructie en de mint-initialisatie-instructie toe
in dezelfde transactie.

Tijdens mint-initialisatie controleert het Token Program of het mint account nog
niet is geïnitialiseerd en rent-exempt is. Het Token Program schrijft vervolgens
de mint authority, freeze authority, decimalen en de _rs`is_initialized`_-vlag
naar de mint account data.

### Bronverwijzing

| Item                           | Beschrijving                                                                                   | Token Program                                                                                                | Token Extensions Program                                                                                           |
| ------------------------------ | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Mint`_                     | De basis mint-status die is opgeslagen in elk mint account.                                    | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`InitializeMint`_           | Een mint-initialisatie-instructie die de rent sysvar account verwacht in zijn accountenlijst.  | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L24-L44)   | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L50-L74)   |
| _rs`InitializeMint2`_          | Een mint-initialisatie-instructie die geen rent sysvar account vereist in zijn accountenlijst. | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L401-L414) | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L471-L486) |
...
Verouderde voorbeelden met `@solana/web3.js` zijn ter referentie toegevoegd.
...
Een mint account slaat alleen basis tokeninformatie op (aanbod, decimalen,
autoriteiten). Om leesbare metadata zoals een naam, symbool en afbeelding aan uw
token toe te voegen, heeft u twee opties:
...
<Cards>
  ...
  <Card title="Token Extensions Metadata" href="/docs/tokens/extensions/metadata">
   ...
   Om metadata toe te voegen aan mint accounts die zijn aangemaakt met het originele Token Program, gebruikt u het Metaplex Token Metadata Program.
  </Card>
</Cards>
