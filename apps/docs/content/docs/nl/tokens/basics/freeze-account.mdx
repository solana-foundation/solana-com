...
description:
  Bevries een token account met de bevriezingsautoriteit van de mint om
  overdrachten, verbrandingen en wijzigingen van gemachtigden te blokkeren.
...
Het bevriezen van een token account behoudt dezelfde eigenaar, mint en saldo,
maar voorkomt dat het token account tokens kan ontvangen, overdragen of
verbranden totdat het token account wordt ontdooid.

Een bevroren account kan nog steeds worden gesloten als het bevroren account een
tokensaldo van nul heeft.

Alleen een mint die nog steeds een bevriezingsautoriteit heeft, kan token
accounts bevriezen. Als de bevriezingsautoriteit van een mint wordt ingetrokken,
kunnen accounts voor die mint niet langer worden bevroren.

## Hoe een token account te bevriezen

Het bevriezen van een token account maakt gebruik van de
_rs`FreezeAccount`_-instructie van het Token Program.

De _rs`FreezeAccount`_-instructie verifieert dat het token account bij de mint
hoort en dat de ondertekenaar de bevriezingsautoriteit van de mint is, waarna de
status van het token account wordt gewijzigd van geïnitialiseerd naar bevroren.

[Natieve token accounts](/docs/tokens/basics/sync-native) ondersteunen geen
bevriezen.
...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Mint`_                          | De mint-status slaat de optionele bevriezingsautoriteit op die wordt gebruikt om token accounts te bevriezen. | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | De token account-statussen, waaronder _rs`Initialized`_ en _rs`Frozen`_.                                      | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | Een instructie die een token account in de bevroren status verandert.                                         | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
...
`@solana/kit`. Voorbeelden van de oudere aanpak met `@solana/web3.js` zijn ter
