...
title: Sync Native
description:
  Werk het bedrag van een wrapped SOL token account bij na het overmaken van SOL
  naar het account.
...
Token Program gebruikt voor token accounts die SOL verpakken. De native mint
maakt het mogelijk om SOL te gebruiken via de instructies van het Token Program.
...
Een wrapped SOL token account bewaart SOL in het `lamports`-veld van het
account, maar houdt de SOL in het wrapped SOL token account bij als een
tokensaldo via het `amount`-veld van het token account. Als u SOL overmaakt naar
een wrapped SOL token account, neemt het lamport-saldo onmiddellijk toe, maar
het `amount`-veld verandert pas wanneer u de `SyncNative`-instructie van het
Token Program aanroept.

## Hoe Native SOL te Synchroniseren

Gebruik de `SyncNative`-instructie van het Token Program na het overmaken van
SOL naar een wrapped SOL token account. De `SyncNative`-instructie werkt het
`amount`-veld van het token account bij zodat het overeenkomt met de lamports
die in het account worden aangehouden boven de huurvrijstelling.

### Bronverwijzing

| Item                  | Beschrijving                                                                                                                                                                                                                    | Token Program                                                                                                | Token Extensions Program                                                                                           |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| `Account`             | De token account-status gebruikt `is_native` om het account als WSOL te markeren en slaat de native reserve op, het minimale lamport-saldo dat het account moet behouden voor huurvrijstelling, en het bijgehouden tokenbedrag. | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| `SyncNative`          | Een instructie die het `amount`-veld van een wrapped SOL token account bijwerkt zodat het overeenkomt met de lamports die in het account worden aangehouden boven de huurvrijstelling.                                          | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
