...
title: Sync Native
description:
  Update het bedrag van een wrapped SOL token account na het overmaken van SOL
  naar het account.
...
Wrapped SOL (WSOL) is SOL die wordt aangehouden in een token account voor de
native mint van het Token Program. De native mint is het mint-adres dat het
Token Program gebruikt voor token accounts die SOL wrappen. De native mint maakt
het mogelijk om SOL te gebruiken via de instructies van het Token Program.

| Programma               | Native mint-adres                              |
| ----------------------- | ---------------------------------------------- |
...
Een wrapped SOL token account slaat SOL op in het veld `lamports` van het
account, maar houdt de SOL in het wrapped SOL token account bij als een
tokensaldo via het veld `amount` van het token account. Als je SOL overmaakt
naar een wrapped SOL token account, neemt het lamport-saldo onmiddellijk toe,
maar het veld `amount` verandert pas wanneer je de instructie `SyncNative` van
het Token Program aanroept.

## Hoe native SOL te synchroniseren

Gebruik de instructie `SyncNative` van het Token Program na het overmaken van
SOL naar een wrapped SOL token account. De instructie `SyncNative` werkt het
veld `amount` van het token account bij zodat het overeenkomt met de lamports
die in het account worden aangehouden boven de rent-exempt reserve.

### Bronverwijzing

| Item                  | Beschrijving                                                                                                                                                                                                          | Token Program                                                                                                | Token Extension Program                                                                                            |
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| `Account`             | De token account state gebruikt `is_native` om het account als WSOL te markeren en slaat de native reserve op, het minimale rent-exempt lamport-saldo dat het account moet aanhouden, en het bijgehouden tokenbedrag. | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| `SyncNative`          | Een instructie die het veld `amount` van een wrapped SOL token account bijwerkt zodat het overeenkomt met de lamports die in het account worden aangehouden boven de rent-exempt reserve.                             | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| `process_sync_native` | Gedeelde processorlogica voor het synchroniseren van wrapped SOL-saldi.                                                                                                                                               | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |

### Typescript

De onderstaande `Kit` voorbeelden tonen de aanbevolen aanpak met `@solana/kit`.
Legacy voorbeelden met `@solana/web3.js` zijn toegevoegd ter referentie.
