...
description:
  Verwijder de huidige gemachtigde van een token account en wis het resterende
  gedelegeerde bedrag.
...
## Wat doet het intrekken van een gemachtigde?

Het intrekken van een gemachtigde wist de huidige gemachtigde van het token
account en zet het gedelegeerde bedrag terug naar nul.

Na intrekking heeft de voormalige gemachtigde niet langer de bevoegdheid om
tokens namens het token account over te dragen of te verbranden.

## Hoe trek je een gemachtigde in

Het intrekken van een gemachtigde gebruikt de _rs`Revoke`_-instructie van het
Token Program.

De _rs`Revoke`_-instructie wist de gemachtigde-velden op het bron-token account.
In het Token Program ondertekent de eigenaar van het token account de
intrekking. In het Token Extensions Program kan de huidige gemachtigde de
delegatie ook intrekken.

### Bronverwijzing

| Item                 | Beschrijving                                                                        | Token Program                                                                                                | Token Extensions Program                                                                                           |
| -------------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Account`_        | De status van het token account slaat de gemachtigde en het gedelegeerde bedrag op. | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Revoke`_         | Een instructie die de huidige gemachtigde van een token account verwijdert.         | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L128-L140) | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L170-L182) |
| _rs`process_revoke`_ | Gedeelde processorlogica voor het intrekken van gemachtigden.                       | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L395-L420)     | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L643-L674)     |
