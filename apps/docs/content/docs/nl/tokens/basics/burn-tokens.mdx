...
description:
  Verbrand tokens van een token account en verlaag het totale aanbod van de
  mint.
...
Het verbranden van tokens verlaagt permanent het saldo van een token account en
vermindert het totale aanbod van de mint met hetzelfde bedrag.

## Hoe Tokens te Verbranden

Het verbranden van tokens maakt gebruik van de _rs`Burn`_ of _rs`BurnChecked`_
instructie van het Token Program.

De onderstaande voorbeelden gebruiken _rs`BurnChecked`_, waarbij de aanroeper de
decimalen van de mint moet opgeven zodat de instructie de verwachte
tokenprecisie kan verifiëren voordat deze verbrand wordt.
...
transactie. In het Token Extensions Program kan een permanente mint-gemachtigde
de verbranding ook autoriseren als de mint de permanente gemachtigde-extensie
heeft ingeschakeld.
...
| _rs`Account`_      | De token account-status slaat het tokensaldo op dat door een verbranding wordt verlaagd.                                                                                       | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Een verbrandingsinstructie die een token account-saldo en mint-aanbod verlaagt zonder dat de aanroeper de decimalen van de mint hoeft op te geven.                             | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Een verbrandingsinstructie waarbij de aanroeper de decimalen van de mint moet opgeven en die waarde controleert voordat een token account-saldo en mint-aanbod wordt verlaagd. | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
...
De onderstaande voorbeelden met `Kit` tonen de aanbevolen aanpak met
`@solana/kit`. Verouderde voorbeelden met `@solana/web3.js` zijn ter referentie
opgenomen.
