...
description:
  Verbrand tokens van een token account en verminder het totale aanbod van de
  mint.
...
Het verbranden van tokens vermindert permanent het saldo van een token account
en verlaagt het totale aanbod van de mint met hetzelfde bedrag.

## Hoe Verbrand Je Tokens

Het verbranden van tokens gebruikt de _rs`Burn`_ of _rs`BurnChecked`_ instructie
van het Token Program.

De onderstaande voorbeelden gebruiken _rs`BurnChecked`_, wat vereist dat de
aanroeper de decimalen van de mint verstrekt, zodat de instructie de verwachte
tokenprecisie kan verifiëren voordat er wordt verbrand.

De [native mint](/docs/tokens/basics/sync-native) ondersteunt geen verbranding.
Gebruik in plaats daarvan _rs`CloseAccount`_.

De eigenaar van het token account of een goedgekeurde gemachtigde ondertekent de
transactie. In het Token Extension Program kan een permanente gemachtigde van de
mint de verbranding ook autoriseren als de mint de permanente
gemachtigde-extensie heeft ingeschakeld.

### Bronverwijzing

| Item               | Beschrijving                                                                                                                                                                           | Token Program                                                                                                | Token Extension Program                                                                                            |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Mint`_         | De mint-status slaat het totale tokenaanbod op.                                                                                                                                        | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | De token account-status slaat het tokensaldo op dat wordt verminderd door een verbranding.                                                                                             | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Een verbrandingsinstructie die een token account-saldo en mint-aanbod vermindert zonder dat de aanroeper de decimalen van de mint hoeft te verstrekken.                                | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Een verbrandingsinstructie die vereist dat de aanroeper de decimalen van de mint verstrekt en die waarde controleert voordat een token account-saldo en mint-aanbod worden verminderd. | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | Gedeelde processorlogica voor tokenverbrandingen.                                                                                                                                      | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |

### Typescript

De onderstaande `Kit` voorbeelden tonen de aanbevolen aanpak met `@solana/kit`.
Voorbeelden met de oudere `@solana/web3.js` zijn ter referentie opgenomen.
