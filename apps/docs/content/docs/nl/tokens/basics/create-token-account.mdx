...
description:
  Maak tokenaccounts en associated token accounts aan voor een specifieke mint en eigenaar.
...
Het aanmaken van een ATA vereist een terugbetaalbaar minimumsaldo voor de onchain-opslag van het account, plus de transactiekosten. De `payer` die wordt meegegeven aan de aanmaakinstructie financiert het opslagsaldo en moet ondertekenen. De betaler kan verschillen van zowel de betaler van de transactiekosten als de wallet die eigenaar wordt van de ATA, zodat een applicatie een ATA kan aanmaken namens een ontvanger.

Tegen het huidige clusteropslagtarief zijn de basiskosten:

| Token-programma                             | ATA-grootte  | Minimumsaldo voor opslag             |
| ----------------------------------------- | --------- | ----------------------------------- |
| Original Token Program                    | 165 bytes | 2.039.280 lamports (0,00203928 SOL) |
| Token-2022 met automatische onveranderlijke eigenaar | 170 bytes | 2.074.080 lamports (0,00207408 SOL) |

Token-2022 mints kunnen accountextensies vereisen naast de automatische _rs`ImmutableOwner`_-extensie. Die ATA's zijn groter en vereisen een hoger minimumsaldo. Het Associated Token Program vraagt het geselecteerde Token Program naar de vereiste accountgrootte en financiert de ATA dienovereenkomstig.

Codeer deze saldi niet hardcoded in de applicatielogica. Gebruik
[`getMinimumBalanceForRentExemption`](/docs/rpc/http/getminimumbalanceforrentexemption)
voor de vereiste gegevensgrootte, of laat het Associated Token Program dit berekenen en
financieren tijdens het aanmaken.

<Callout type="info" title="Het opslagsaldo is terugbetaalbaar">
  Het sluiten van een leeg token account draagt alle lamports over, inclusief het
  minimumsaldo voor opslag, naar de bestemming die is opgegeven in de sluitinstructie. De oorspronkelijke aanmakende betaler is niet automatisch de terugbetalingsbestemming. Zie [Close Token Account](/docs/tokens/basics/close-account).
</Callout>
