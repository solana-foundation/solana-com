...
Een token account slaat je saldo van een specifiek token op. Elk token account
...
<Callout type="info">
  Merk op dat ontwikkelaars in de broncode een token account een `Account` type
...
Om tokens te kunnen houden, heb je een token account nodig voor die specifieke
mint. Elk token account houdt het volgende bij:

1. **Mint**: Het specifieke tokentype dat het bevat
2. **Owner**: De autoriteit die tokens van dit account kan overmaken
...
- Circle beheert tokens via de token account owner-autoriteit:
...
<Callout type="info">
  **Begrip van "owner"-terminologie**

De term "owner" kan in twee verschillende contexten worden gebruikt:

1. **Token account owner**: De autoriteit die tokens kan overmaken, verbranden
   of delegeren. Deze wordt opgeslagen in de data van het token account.

2. **Program owner**: Het programma dat eigenaar is van het account (altijd het
   Token Program of Token Extension Program voor token accounts).

Bij het werken met token accounts verwijst "owner" doorgaans naar de autoriteit
...
token account van elke wallet voor een bepaalde mint te vinden. Zie de
[afleidingsimplementatie hier](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/interface/src/address.rs#L56).
...
Voor elke combinatie van wallet en token bestaat er precies één ATA-adres. Dit
elimineert de noodzaak om token account-adressen handmatig bij te houden—je kunt
altijd het juiste adres afleiden.
...
- Cross-Program Invocations (CPI's) maakt naar het Token Program
- Zelf geen state bijhoudt
...
Het aanmaken van een token account vereist de
...
1. System Program: maak een account aan met toegewezen ruimte voor een token
...
Deze enkele instructie handelt automatisch het aanmaken van het token account en
het initialiseren ervan af via cross program invocations (CPI).
