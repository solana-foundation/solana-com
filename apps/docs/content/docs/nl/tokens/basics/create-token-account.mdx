...
Een token account bewaart tokens voor één mint en één tokenaccount-eigenaar op
Solana.
...
overmaken, verbranden of delegeren. De programma-eigenaar van het account blijft
...
account alleen eenheden van één token kan bevatten: het token dat wordt
...
Voor elke combinatie van wallet, tokenprogramma en mint bestaat precies één
...
## Een Associated Token Account aanmaken
...
<Callout type="info" title="Aanbevolen">
  Voor de meeste applicaties is het raadzaam om token accounts aan te maken via
  het Associated Token Program in plaats van ze direct aan te maken door System
...
| _rs`CreateIdempotent`_                        | Maakt het associated token account aan, maar slaagt ook als die ATA al bestaat voor dezelfde eigenaar en mint.                                                                                                                                                                         | [Bron](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
...
| _rs`create_pda_account`_                      | Helper die gebruikt wordt door _rs`process_create_associated_token_account`_ om het PDA-account aan te maken via CPI naar het System Program.                                                                                                                                          | [Bron](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |

### Typescript

De onderstaande `Kit` voorbeelden tonen de aanbevolen aanpak met `@solana/kit`.
Legacy voorbeelden met `@solana/web3.js` zijn ter referentie bijgevoegd.
...
1. De _rs`CreateAccount`_-instructie van het System Program creëert een nieuw
   rent-vrijgesteld account en wijst het Token Program toe als de
...
Tijdens de token account-initialisatie controleert het Token Program of het
account niet reeds geïnitialiseerd is en rent-vrijgesteld is.

<Callout type="info">
  De sectie hieronder toont hoe je een token account aanmaakt door rechtstreeks
  de instructies van het System Program en Token Program aan te roepen. Gebruik
  voor de meeste toepassingen in plaats daarvan het Associated Token Program.
  Gebruik directe aanroepen naar het System Program en Token Program wanneer je
  een specifieke reden hebt om geen associated token account te gebruiken of
  wanneer je aangepaste PDA token accounts moet aanmaken door CPI's naar het
  System Program en Token Program vanuit je eigen Solana-programma te maken.
</Callout>
...
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Account`_                     | De basis token account-velden opgeslagen in elk token account.                                                                    | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
...
| _rs`InitializeAccount3`_          | Een token account-initialisatie-instructie die de eigenaar in instruction data doorgeeft en het rent sysvar account niet vereist. | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L377-L387) | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L445-L456) |
...
De onderstaande `Kit` voorbeelden tonen de aanbevolen aanpak met behulp van
