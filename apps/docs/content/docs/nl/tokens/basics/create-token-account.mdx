---
title: Maak een token account aan
description:
  Maak token-accounts en bijbehorende token-accounts voor een specifieke mint en
  eigenaar.
...
Een token account slaat tokens op voor één mint en één token account-eigenaar op
Solana.
...
Hier betekent _rs`owner`_ de autoriteit die tokens kan overmaken, verbranden of
delegeren vanuit het token account. De programma-eigenaar van het account blijft
het Token Program of Token Extensions Program.

Elk token account is gekoppeld aan precies één mint, wat betekent dat het token
account alleen eenheden van één token kan bevatten, namelijk het token dat wordt
geïdentificeerd door het _rsINLLINE_CODE_PLACEHOLDER_a4fd5a3d8817d1fb_END_-veld
van het token account.
...
leidt het ATA-adres af van het wallet-adres, token program-adres en mint-adres.
...
Voor elke combinatie van wallet, token program en mint bestaat er precies één
...
_rs`Account`_-type dat is gedefinieerd door het Token Program of Token
Extensions Program.

## Hoe maak je een Associated Token Account aan

Het aanmaken van een associated token account gebruikt de _rs`Create`_ of
_rs`CreateIdempotent`_ instructie van het Associated Token Program. Het
...
<Callout type="info" title="Aanbevolen">
  Voor de meeste toepassingen kun je het beste token accounts aanmaken via het
  Associated Token Program in plaats van ze direct aan te maken door instructies
  van het System Program en Token Program aan te roepen. Een associated token
  account gebruikt een deterministisch adres afgeleid van de eigenaar, het token
  program en de mint, waardoor het standaard token account voor een bepaalde
  mint gemakkelijker te vinden is voor wallets en applicaties.
</Callout>
...
| _rs`CreateIdempotent`_                        | Maakt het associated token account aan, maar slaagt nog steeds als dat ATA al bestaat voor dezelfde eigenaar en mint.                                                                                                                                                                  | [Bron](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | Leidt het ATA-adres af en gebruikt _rs`create_pda_account`_ plus CPI's naar het geselecteerde token program om het token account te initialiseren. Wanneer het geselecteerde programma het Token Extension Program is, initialiseert de ATA-processor ook de immutable owner extensie. | [Bron](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | Hulpmiddel gebruikt door _rs`process_create_associated_token_account`_ om het PDA-account aan te maken via CPI naar het System Program.                                                                                                                                                | [Bron](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |

### Typescript

De `Kit` voorbeelden hieronder tonen de aanbevolen aanpak met behulp van
`@solana/kit`. Legacy voorbeelden met `@solana/web3.js` zijn ter referentie
opgenomen.
...
## Hoe een tokenaccount aanmaken

Het aanmaken van een tokenaccount vereist twee instructies:

1. De _rs`CreateAccount`_-instructie van het System Program maakt een nieuw
   rent-vrijgesteld account aan en wijst het Token Program toe als de
...
Neem de _rs`CreateAccount`_-instructie en de
tokenaccount-initialisatie-instructie op in dezelfde transactie.

Tijdens de initialisatie van het tokenaccount controleert het Token Program of
het account niet al is geïnitialiseerd en rent-vrijgesteld is.

<Callout type="info">
  Het onderstaande gedeelte laat zien hoe je een tokenaccount aanmaakt door
  direct de System Program- en Token Program-instructies aan te roepen. Voor de
  meeste applicaties is het beter om het Associated Token Program te gebruiken.
  Gebruik directe System Program- en Token Program-aanroepen wanneer je een
  specifieke reden hebt om geen associated token account te gebruiken of wanneer
  je aangepaste PDA-tokenaccounts moet aanmaken door CPI's te maken naar het
  System Program en Token Program vanuit je eigen Solana-programma.
</Callout>
...
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Account`_                     | De basistokenaccountvelden die in elk tokenaccount zijn opgeslagen.                                                          | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`InitializeAccount`_           | Een tokenaccount-initialisatie-instructie die de eigenaar en rent sysvar account in zijn accountlijst verwacht.              | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L45-L63)   | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L75-L93)   |
| _rs`InitializeAccount2`_          | Een tokenaccount-initialisatie-instructie die de eigenaar doorgeeft in instruction data in plaats van de accountlijst.       | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L352-L365) | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L419-L433) |
| _rs`InitializeAccount3`_          | Een tokenaccount-initialisatie-instructie die de eigenaar doorgeeft in instruction data en geen rent sysvar account vereist. | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L377-L387) | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L445-L456) |
| _rs`_process_initialize_account`_ | Gedeelde processorlogica voor tokenaccount-initialisatie.                                                                    | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L83-L141)      | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L153-L234)     |
