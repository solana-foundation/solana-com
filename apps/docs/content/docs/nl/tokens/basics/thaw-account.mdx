...
description:
  Ontdooi een bevroren token account zodat overdrachten, verbrandingen en
  wijzigingen van gedelegeerden kunnen worden hervat.
...
## Wat Doet Het Ontdooien van een Token Account?

Ontdooien verandert een token account van de bevroren status terug naar de
geïnitialiseerde status.

Nadat het ontdooien is geslaagd, kan het account opnieuw tokens overdragen,
verbranden en sluiten. Alleen de freeze authority van de mint kan token accounts
ontdooien.

## Hoe Een Token Account Te Ontdooien

Het ontdooien van een token account gebruikt de _rs`ThawAccount`_ instructie van
het Token Program.

De _rs`ThawAccount`_ instructie verifieert dat het token account bij de mint
hoort en dat de ondertekenaar de freeze authority van de mint is, waarna de
status van het token account wordt veranderd van bevroren terug naar
geïnitialiseerd.

[Native token accounts](/docs/tokens/basics/sync-native) ondersteunen geen
ontdooien.
...
| ----------------------------------- | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Mint`_                          | De mint status slaat de optionele freeze authority op die wordt gebruikt om token accounts te ontdooien. | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | De token account statussen, inclusief _rs`Frozen`_ en _rs`Initialized`_.                                 | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`ThawAccount`_                   | Een instructie die een bevroren token account terug verandert naar de geïnitialiseerde status.           | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L229-L243) | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L292-L306) |
| _rs`process_toggle_freeze_account`_ | Gedeelde processorlogica voor het bevriezen en ontdooien van token accounts.                             | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |

### Typescript

De `Kit` voorbeelden hieronder tonen de aanbevolen aanpak met `@solana/kit`.
Verouderde voorbeelden met `@solana/web3.js` zijn ter referentie opgenomen.
