---
title: Tokens minten
description: Leer hoe je nieuwe eenheden van een token mint.
---
...
- Alleen de mint authority kan nieuwe tokens minten
- Er moet een bestemmings-token account bestaan om de geminte tokens te
  ontvangen
...
async fn main() -> Result<()> {
    // Maak verbinding met lokale validator
    ...
    // Genereer een nieuwe keypair voor de fee payer
    let fee_payer = Keypair::new();

    // Airdrop 1 SOL naar fee payer
    ...
    // Genereer keypair om te gebruiken als adres van mint
    let mint = Keypair::new();

    // Verkrijg standaard mint account grootte (in bytes), geen extensies ingeschakeld
    let mint_space = Mint::LEN;
    let mint_rent = client
        .get_minimum_balance_for_rent_exemption(mint_space)
        .await?;

    // Instructie om nieuw account aan te maken voor mint (token program)
    let create_account_instruction = create_account(
        &fee_payer.pubkey(), // payer
        &mint.pubkey(),      // nieuw account (mint)
        mint_rent,           // lamports
        mint_space as u64,   // ruimte
        &token_program_id(), // program id
    );

    // Instructie om mint account data te initialiseren
    ...
    // Bereken het associated token account adres voor fee_payer
    let associated_token_address = get_associated_token_address_with_program_id(
        &fee_payer.pubkey(), // eigenaar
        &mint.pubkey(),      // mint
        &token_program_id(), // program_id
    );

    // Instructie om associated token account aan te maken
    let create_ata_instruction = create_associated_token_account(
        &fee_payer.pubkey(), // financieringsadres
        &fee_payer.pubkey(), // wallet-adres
        &mint.pubkey(),      // mint-adres
        &token_program_id(), // program id
    );

    // Maak transactie aan en voeg instructies toe
    ...
    // Verstuur en bevestig transactie
    client.send_and_confirm_transaction(&transaction).await?;

    // Aantal tokens om te minten (100 tokens met 2 decimalen)
    let amount = 100;

    // Maak mint_to instructie om tokens te minten naar het associated token account
    let mint_to_instruction = mint_to(
        &token_program_id(),
        &mint.pubkey(),            // mint
        &associated_token_address, // bestemming
        &fee_payer.pubkey(),       // authority
        &[&fee_payer.pubkey()],    // ondertekenaar
        amount,                    // hoeveelheid
    )?;

    // Maak transactie voor het minten van tokens
    let transaction = Transaction::new_signed_with_payer(
        &[mint_to_instruction],
        Some(&fee_payer.pubkey()),
        &[&fee_payer],
    ...
    println!("1,00 tokens gemint naar het associated token account");
    println!("\nMint-adres: {}", mint.pubkey());
    println!("{:#?}", mint_data);

    println!(
        "\nAssociated token account adres: {}",
        associated_token_address
    );
    println!("{:#?}", token_data);

    println!("Transactiehandtekening: {}", transaction_signature);
...
async def main():
    ...
    # Voorbeeld keypairs en adressen
    payer = Keypair()
    ...
    # Te minten hoeveelheid (in kleinste eenheid)
    amount_to_mint = 1000000000  # 1 token met 9 decimalen

    async with rpc:
        # Maak mint instructie
        ...
        print(f"Bestemming: {destination_token_account}")
        print(f"Hoeveelheid: {amount_to_mint}")
        print(f"Mint authority: {mint_authority.pubkey()}")
        print(f"Betaler: {payer.pubkey()}")
        print(f"Mint transactie succesvol aangemaakt")
