...
description:
  Verplaats tokens tussen token accounts voor dezelfde mint met Token Program
...
## Wat Doet het Overmaken van Tokens?

Het overmaken van tokens verplaatst tokens van het ene token account naar een
ander token account voor dezelfde mint.

Transfers veranderen de mint supply niet. Transfers werken alleen de saldi
tussen token accounts bij.

## Hoe Tokens Overmaken

Het overmaken van tokens gebruikt de _rs`Transfer`_ of _rs`TransferChecked`_
instructie van het Token Program.

De onderstaande voorbeelden gebruiken _rs`TransferChecked`_, wat vereist dat de
aanroeper de mint en decimalen verstrekt zodat de instructie de verwachte mint
en token-precisie kan verifiëren voordat tokens tussen accounts worden
verplaatst.

In het Token Extensions Program is _rs`Transfer`_ verouderd ten gunste van
_rs`TransferChecked`_ of _rs`TransferCheckedWithFee`_.

De eigenaar van het bronaccount of een goedgekeurde gemachtigde ondertekent de
transfer. In het Token Extensions Program kan een mint permanent delegate de
transfer ook autoriseren als de mint de permanent delegate-extensie heeft
ingeschakeld.
...
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Account`_          | De token account-status slaat de saldi op die door een transfer worden bijgewerkt.                                                                                                                                      | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | Een transfer-instructie die tokens tussen token accounts verplaatst zonder dat de aanroeper de mint of decimalen hoeft te verstrekken. Gebruik in het Token Extensions Program in plaats daarvan _rs`TransferChecked`_. | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Een transfer-instructie die vereist dat de aanroeper de mint en decimalen verstrekt en deze waarden controleert voordat tokens tussen token accounts van dezelfde mint worden verplaatst.                               | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | Gedeelde processorlogica voor token-transfers.                                                                                                                                                                          | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |

### Typescript

De onderstaande `Kit` voorbeelden tonen de aanbevolen aanpak met `@solana/kit`.
Verouderde voorbeelden met `@solana/web3.js` zijn ter referentie bijgevoegd.
