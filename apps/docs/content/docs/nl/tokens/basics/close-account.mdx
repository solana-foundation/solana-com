...
description:
  Sluit een tokenaccount en retourneer de lamports van het account naar een
  bestemmingsaccount.
...
## Wat doet het sluiten van een tokenaccount?

Het sluiten van een tokenaccount verwijdert het account en retourneert de rent
lamports van het tokenaccount naar een bestemmingsaccount.

Het tokensaldo moet nul zijn voordat het account kan worden gesloten. Een
bevroren account kan nog steeds worden gesloten als het tokensaldo van het
bevroren account nul is. Wrapped SOL tokenaccounts vormen hierop een
uitzondering en kunnen worden gesloten met een tokensaldo om de onderliggende
SOL terug te vorderen. De accounteigenaar of de close authority van het account
ondertekent de sluitingsinstructie.

## Hoe een tokenaccount te sluiten

Het sluiten van een tokenaccount maakt gebruik van de _rs`CloseAccount`_
instructie van het Token Program.

De _rs`CloseAccount`_ instructie draagt de lamports van het bronaccount over
naar een bestemmingsaccount, wist het bronaccount en verwijdert het bronaccount.
Het Token Extension Program past aanvullende sluitingscontroles toe voor
bepaalde extensies, maar de basissluitingsprocedure blijft hetzelfde.

### Bronverwijzing

| Item                        | Beschrijving                                                                                                                              | Token Program                                                                                                | Token Extension Program                                                                                            |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Account`_               | De tokenaccountstatus slaat het saldo, de native status en de close authority op die worden gebruikt tijdens het sluiten van het account. | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | Een instructie die een tokenaccount sluit en de lamports van het tokenaccount overdraagt naar een bestemmingsaccount.                     | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | Gedeelde processorlogica voor het sluiten van tokenaccounts.                                                                              | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |
