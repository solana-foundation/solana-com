...
description:
  Sluit een token-account en retourneer de lamports van het account naar een bestemmingsaccount.
...
Het sluiten van een token account verwijdert het account en draagt al zijn lamports over,
inclusief het restitueerbare minimale opslagsaldo, naar een bestemmingsaccount.

Het tokensaldo moet nul zijn voordat het account gesloten kan worden. Een bevroren account
kan nog steeds gesloten worden als het tokensaldo van het bevroren account nul is. Wrapped SOL
token accounts zijn de uitzondering en kunnen gesloten worden met een tokensaldo om de onderliggende SOL terug te vorderen. De accounteigenaar of de close authority van het account ondertekent de sluitingsinstructie en selecteert de bestemming. Het account dat betaald heeft voor het aanmaken van een associated token account ontvangt het teruggevorderde saldo niet automatisch.

## Hoe sluit je een token-account

Het sluiten van een token-account maakt gebruik van de
_rs`CloseAccount`_-instructie van het Token Program.
...
| --------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | De token-accountstatus slaat het saldo, native-status en sluitingsautoriteit op die tijdens het sluiten van het account worden gebruikt.     | [Bron](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
