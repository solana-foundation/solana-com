...
[data accounts](/docs/core/accounts/account-types#data-accounts) die eigendom
zijn van een Token Program.
...
[Account](/docs/core/accounts/account-structure#account-fields) type, wat het
Token Program is voor alle token accounts.
...
Er zijn twee manieren om metadata (naam, symbool, afbeelding) toe te voegen aan
tokens op Solana:

1. **[Metaplex Token Metadata](/docs/tokens/metaplex)** - Creëert een apart
   metadata-account gekoppeld aan je mint. Werkt met het oorspronkelijke Token
   Program en Token-2022 en wordt breed ondersteund door wallets en
   marktplaatsen.

2. **[Token Extensions Metadata](/docs/tokens/extensions/metadata)** - Slaat
   metadata rechtstreeks op in het mint account met behulp van Token-2022. Een
   nieuwere aanpak met ingebouwde ondersteuning. De metadata-extensie is alleen
   beschikbaar voor mint accounts die zijn aangemaakt met Token-2022. Om
   metadata toe te voegen aan mint accounts die zijn aangemaakt met het
   oorspronkelijke Token Program, gebruik je het Metaplex Token Metadata
   Program.

#### SPL Token CLI Voorbeeld

Het Token Extensions Program stelt je in staat om metadata rechtstreeks op te
slaan in het Mint Account. Om een token aan te maken met de metadata-extensie:
...
Lees meer in de [Token Extensions-sectie](/docs/tokens/extensions/metadata).
Voor details over Token Extensions, zie de
[Token Extensions-sectie](/docs/tokens/extensions) en de
