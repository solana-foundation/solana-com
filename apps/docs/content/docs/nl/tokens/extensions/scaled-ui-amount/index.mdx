...
De _rs`ScaledUiAmount`_ mint-extensie van het Token Extensions Program stelt een
mint in staat om een aanpasbare vermenigvuldiger toe te passen op het UI-bedrag
dat wordt weergegeven voor tokensaldi.

Dit is nuttig wanneer een token een real-world asset of instrument
vertegenwoordigt, zoals een aandelensplitsing of dividend. Bij een
aandelensplitsing verdubbelt bijvoorbeeld het aantal aandelen voor alle houders.
Zonder het saldo van elk token account te wijzigen, stelt _rs`ScaledUiAmount`_
wallets en applicaties in staat om een nieuw UI-bedrag weer te geven met behulp
van de vermenigvuldiger van de mint.

Deze functie kan ook worden gebruikt voor dividenden of het verdelen van
rendement.

Er worden geen nieuwe tokens aangemaakt. Het tokenbedrag dat is opgeslagen in
token accounts blijft hetzelfde. Alleen het weergegeven UI-bedrag verandert.

<Callout type="info" title="Incompatibele extensie">
  Schakel _rs`ScaledUiAmount`_ en _rs`InterestBearingConfig`_ niet in op
  dezelfde mint. Het Token Extensions Program weigert die extensiecombinatie.
</Callout>

## Hoe Scaled UI Amount werkt

- _rs`ScaledUiAmountConfig`_ slaat de update-authority, `multiplier`,
  `new_multiplier` en `new_multiplier_effective_timestamp` op.
- Vóór `new_multiplier_effective_timestamp` gebruiken conversies `multiplier`.
  Op of na dat tijdstempel gebruiken conversies `new_multiplier`.
- _rs`UpdateMultiplier`_ kan een nieuwe vermenigvuldiger onmiddellijk toepassen
  of plannen voor een toekomstig Unix-tijdstempel.
- _rs`AmountToUiAmount`_, _rs`UiAmountToAmount`_ en de extensieconversiehelpers
  gebruiken floating-point-rekenkunde voor scaled UI amount-mints, dus
  conversies zijn niet gegarandeerd exact round-trip.
...
1. Bereken de grootte van het mint account en de benodigde rent voor de mint en
   de _rs`ScaledUiAmount`_-extensie.
2. Maak het mint account aan met _rs`CreateAccount`_, initialiseer
   _rs`ScaledUiAmount`_ en initialiseer de mint met _rs`InitializeMint`_.
3. Gebruik _rs`AmountToUiAmount`_, _rs`UiAmountToAmount`_ of clienthelpers bij
   het weergeven van saldi of het converteren van UI-bedragen.
4. Gebruik _rs`UpdateMultiplier`_ om de weergegeven vermenigvuldiger
   onmiddellijk of op een toekomstig Unix-tijdstempel bij te werken.

<Callout type="info" title="Instructievolgorde">
  _rs`ScaledUiAmountMintInstruction::Initialize`_ moet vóór _rs`InitializeMint`_
  komen. _rs`CreateAccount`_, _rs`ScaledUiAmountMintInstruction::Initialize`_ en
...
<Cards>
  ...
  >
    Creëer een mint met Scaled UI Amount en werk de vermenigvuldiger in de loop
    van de tijd bij.
  </Card>
  <Card
  ...
  >
    Converteer tokenbedragen voor weergave en ondersteun Scaled UI Amount in
    wallets en applicaties.
  </Card>
</Cards>
...
| ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`ScaledUiAmountConfig`_                            | Mint-extensiestatus die de authority, huidige vermenigvuldiger, volgende vermenigvuldiger en de timestamp wanneer deze van kracht wordt, opslaat. | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L41-L55)         |
| _rs`ScaledUiAmountMintInstruction::Initialize`_       | Instructie die _rs`ScaledUiAmount`_ op een mint initialiseert vóór _rs`InitializeMint`_.                                                          | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/instruction.rs#L24-L42) |
| _rs`ScaledUiAmountMintInstruction::UpdateMultiplier`_ | Instructie die een nieuwe vermenigvuldiger instelt en de Unix-timestamp wanneer deze van kracht wordt.                                            | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/instruction.rs#L43-L66) |
| _rs`amount_to_ui_amount`_                             | Hulpfunctie die een tokenbedrag converteert naar een UI-bedragstring met gebruik van de huidige vermenigvuldiger, decimalen en timestamp.         | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L69-L84)         |
| _rs`try_ui_amount_into_amount`_                       | Hulpfunctie die een UI-bedragstring terugconverteert naar een tokenbedrag met gebruik van de huidige vermenigvuldiger, decimalen en timestamp.    | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L86-L107)        |
| _rs`process_initialize`_                              | Processor die _rs`ScaledUiAmountConfig`_ initialiseert en de initiële vermenigvuldiger opslaat op een niet-geïnitialiseerde mint.                 | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/scaled_ui_amount/processor.rs#L37-L57)     |
| _rs`process_update_multiplier`_                       | Processor die de authority valideert, de geplande vermenigvuldiger bijwerkt en deze onmiddellijk toepast wanneer van toepassing.                  | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/scaled_ui_amount/processor.rs#L59-L108)    |
| _rs`process_amount_to_ui_amount`_                     | Token Program-conversiepad dat _rs`ScaledUiAmountConfig`_ gebruikt wanneer een mint de extensie heeft ingeschakeld.                               | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1548-L1571)                            |
| _rs`process_ui_amount_to_amount`_                     | Token Program-conversiepad dat een UI-bedrag terugconverteert naar een tokenbedrag voor scaled UI amount mints.                                   | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1576-L1595)                            |

### Typescript

Het `Kit` voorbeeld hieronder gebruikt de gegenereerde instructies direct.
