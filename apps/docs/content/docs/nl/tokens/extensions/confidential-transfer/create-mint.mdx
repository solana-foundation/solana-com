...
description:
  Leer hoe je een token mint aanmaakt met Confidential Balances ingeschakeld.
---

## Hoe maak je een mint aan met Confidential Balances

Confidential Balances maakt privé-tokenoverdrachten mogelijk door extra toestand toe te voegen aan
de mint account. In dit gedeelte wordt uitgelegd hoe je een token mint aanmaakt met de
vereiste extensie ingeschakeld.

Het volgende diagram toont de stappen voor het aanmaken van een mint met de
Confidential Balances-extensie:
...
### Confidential Balances Mint-toestand
...
- **authority**: Het account dat toestemming heeft om de Confidential Balances-
  instellingen voor de mint te wijzigen en nieuwe vertrouwelijke accounts goed te keuren als automatische goedkeuring
  is uitgeschakeld.

- **auto_approve_new_accounts**: Wanneer ingesteld op true, kunnen gebruikers token
  accounts aanmaken met Confidential Balances standaard ingeschakeld. Wanneer false, moet de
  authority elk nieuw token account goedkeuren voordat het gebruikt kan worden voor
  vertrouwelijke overdrachten.

- **auditor_elgamal_pubkey**: Een optionele auditor die overdrachtsbedag in
  vertrouwelijke transacties kan ontsleutelen, wat een nalevingsmechanisme biedt
  terwijl de privacy ten opzichte van het grote publiek wordt gewaarborgd.

### Vereiste instructies

Het aanmaken van een mint met Confidential Balances ingeschakeld vereist drie instructies
in één enkele transactie:

1. **Maak het Mint Account aan**: Roep de instructie _rs`CreateAccount`_ van het
   System Program aan om de mint account aan te maken.

2. **Confidential Balances initialiseren**: Roep de instructie
   [ConfidentialTransferInstruction::InitializeMint](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L48) van het Token Extension Program aan
   om de _rs`ConfidentialTransferMint`_-toestand voor de
   mint te configureren.

3. **Initialiseer Mint**: Roep de instructie _rs`Instruction::InitializeMint`_
   van het Token Extensions Program aan om de standaard mintstatus te
   initialiseren.

Hoewel je deze instructies handmatig kunt schrijven, biedt de
`spl_token_client`-crate een `create_mint`-methode die een transactie met alle
drie de instructies opbouwt en verstuurt in één enkele functieaanroep, zoals
gedemonstreerd in het onderstaande voorbeeld.

## Voorbeeldcode

De volgende code laat zien hoe je een mint aanmaakt met Confidential Balances
ingeschakeld.
...
Voer het voorbeeld uit tegen een van deze omgevingen (de code gebruikt devnet) met een gefinancierde betaler,
en vervang de tijdelijke aanduidingen voor mint- en accountadressen door je eigen adressen.
