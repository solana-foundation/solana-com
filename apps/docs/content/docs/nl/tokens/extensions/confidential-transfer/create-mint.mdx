...
extra status toe te voegen aan de mint account. In dit gedeelte wordt uitgelegd
hoe je een token mint aanmaakt met deze extensie ingeschakeld.

Het volgende diagram toont de stappen voor het aanmaken van een mint met de
Confidential Transfer-extensie:
...
### Status van de Confidential Transfer Mint
...
- **authority**: Het account dat toestemming heeft om de instellingen voor
  vertrouwelijke overdrachten van de mint te wijzigen en nieuwe vertrouwelijke
  accounts goed te keuren als automatische goedkeuring is uitgeschakeld.

- **auto_approve_new_accounts**: Wanneer ingesteld op true, kunnen gebruikers
  token accounts aanmaken met vertrouwelijke overdrachten standaard
  ingeschakeld. Wanneer false, moet de authority elk nieuw token account
  goedkeuren voordat het gebruikt kan worden voor vertrouwelijke overdrachten.
...
2. **Initialiseer de Confidential Transfer-extensie**: Roep de instructie
   ...
   van het Token Extensions Program aan om de _rs`ConfidentialTransferMint`_
   status voor de mint te configureren.
...
De volgende code laat zien hoe je een mint aanmaakt met de Confidential
Transfer-extensie.
...
schakelt dit niet in, maar een mainnet-forking lokale validator zoals
...
de tijdelijke mint- en accountadressen door je eigen adressen.
