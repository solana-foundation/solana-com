...
Het volgende voorbeeld haalt tokens op uit het vertrouwelijke beschikbare saldo
terug naar het publieke saldo. Het account moet al geconfigureerd zijn voor
vertrouwelijke overdrachten en een beschikbaar vertrouwelijk saldo bevatten.
...
schakelt dit niet in, maar een mainnet-forkende lokale validator zoals
[Surfpool](https://surfpool.run) wel. Voer het voorbeeld uit tegen een van deze
(de code gebruikt devnet) met een gefinancierde betaler, en vervang de
tijdelijke aanduidingen door uw mint en token account.
