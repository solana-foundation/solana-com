...
## Hoe tokens op te nemen uit vertrouwelijk beschikbaar saldo

Om tokens op te nemen uit vertrouwelijk beschikbaar saldo naar openbaar saldo:
...
1. Maak
   ...
   **Gelijkheidsbewijs
   ...
   Verifieert dat de resterende beschikbare saldo ciphertext na de opname
   overeenkomt met de bijbehorende
   [Pedersen commitment](https://en.wikipedia.org/wiki/Commitment_scheme), wat
   ervoor zorgt dat het nieuwe beschikbare saldo van het account correct wordt
...
2. Voor elk bewijs:
   - Roep het ZK ElGamal proof program aan om de bewijsgegevens te verifiëren.
   - Sla de bewijs-specifieke metadata op in een "context state" account om te
...
4. Sluit de bewijsaccounts om de SOL die gebruikt is om ze te maken terug te
   krijgen.

Het volgende diagram toont de stappen die betrokken zijn bij het opnemen van
tokens uit vertrouwelijk beschikbaar saldo naar openbaar saldo:
...
- Een gelijkheidsbewijs en bereiksbewijis genereren aan de clientzijde
- Het Zk ElGamal proof program aanroepen om de bewijzen te verifiëren en de
  "context state" accounts te initialiseren
- De
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  instructie aanroepen met de twee bewijsaccounts.
...
Het volgende voorbeeld demonstreert hoe je tokens opneemt uit het vertrouwelijke
beschikbare saldo naar het openbare saldo.
