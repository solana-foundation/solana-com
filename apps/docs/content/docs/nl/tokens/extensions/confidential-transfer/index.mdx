...
over te dragen zonder het overdrachtsbedrag te onthullen. Dit is nuttig voor
...
- [Snelstartgids](https://www.solana-program.com/docs/confidential-balances#setup) -
  Installatie en basis CLI-commando's
- [Confidential Balances Cookbook](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Codefragmenten over hoe de Confidential Transfer-extensie te gebruiken
...
toe aan het Token Extension-programma waarmee je tokens tussen accounts kunt
overdragen zonder het overdrachtsbedrag te onthullen.
...
2. Maak token accounts aan met confidential transfer-extensie voor de verzender
   en ontvanger.
3. Mint tokens naar het verzenderaccount.
...
5. **Pas** het wachtende saldo van de verzender toe op het **vertrouwelijke
   beschikbare saldo**.
6. **Draag** vertrouwelijk tokens over van het verzender token account naar het
   ontvanger token account.
7. **Pas** het wachtende saldo van de ontvanger toe op het **vertrouwelijke
   beschikbare saldo**.
8. **Neem** het vertrouwelijke beschikbare saldo van de ontvanger op naar het
   **openbare saldo**.

Voor meer details over de stappen in de vertrouwelijke overdracht-flow, zie de
...
<Cards>
  ...
  >
    Hoe tokens storten naar het vertrouwelijke openstaande saldo
  </Card>
  <Card
    title="Apply Pending Balance"
    href="/docs/tokens/extensions/confidential-transfer/apply-pending-balance"
  >
    Hoe het openstaande saldo toepassen op het beschikbare vertrouwelijke saldo
  </Card>
  <Card
  ...
  >
    Hoe tokens vertrouwelijk overdragen tussen token accounts
  </Card>
  <Card
    title="Issuer Guide"
    href="/docs/tokens/extensions/confidential-transfer/issuer-guide"
  >
    Hoe een vertrouwelijk overdrachtstoken uitgeven en beheren
    (goedkeuringsbeleid, auditors, vergoedingen, aanmaken en vernietigen)
  </Card>
</Cards>

Het onderstaande diagram toont een gedetailleerde volgorde van de basisflow voor
vertrouwelijke token-overdrachten:
...
token-overdrachten die overeenkomen met het voorgaande diagram. De stappen zijn
voor de duidelijkheid opgesplitst in hulpfuncties.
...
Solana CLI geïnstalleerd hebben om de lokale validator te gebruiken.
...
<Callout type="info">
  Op het moment van schrijven is vertrouwelijke overdrachten niet ingeschakeld
  op de standaard lokale validator. Je moet het mainnet Token Extension Program
  klonen om de voorbeeldcode uit te voeren.
</Callout>
...
De volledige lijst met vertrouwelijke overdracht-extensie
[instructies](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
zijn als volgt:

| Instructie                          | Beschrijving                                                                                                                                                                            |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Stelt mint account in voor vertrouwelijke overdrachten. Deze instructie moet worden opgenomen in dezelfde transactie als _rs`TokenInstruction::InitializeMint`_ instructie.             |
| _rs`UpdateMint`_                    | Werkt vertrouwelijke overdracht-instellingen voor een mint bij.                                                                                                                         |
...
| _rs`EmptyAccount`_                  | Maakt het wachtende en beschikbare vertrouwelijke saldo leeg om het sluiten van een token account mogelijk te maken.                                                                    |
| _rs`Deposit`_                       | Converteert openbaar token-saldo naar wachtend vertrouwelijk saldo.                                                                                                                     |
| _rs`Withdraw`_                      | Converteert beschikbaar vertrouwelijk saldo terug naar openbaar saldo.                                                                                                                  |
| _rs`Transfer`_                      | Draagt tokens vertrouwelijk over tussen token accounts.                                                                                                                                 |
| _rs`ApplyPendingBalance`_           | Converteert wachtend saldo naar beschikbaar saldo na stortingen of overdrachten.                                                                                                        |
| _rs`EnableConfidentialCredits`_     | Stelt een token account in staat om vertrouwelijke token-overdrachten te ontvangen.                                                                                                     |
| _rs`DisableConfidentialCredits`_    | Blokkeert inkomende vertrouwelijke overdrachten terwijl openbare overdrachten nog steeds zijn toegestaan.                                                                               |
| _rs`EnableNonConfidentialCredits`_  | Stelt een token account in staat om openbare token-overdrachten te ontvangen.                                                                                                           |
| _rs`DisableNonConfidentialCredits`_ | Blokkeert reguliere overdrachten om account alleen vertrouwelijke overdrachten te laten ontvangen.                                                                                      |
| _rs`TransferWithFee`_               | Draagt tokens vertrouwelijk over tussen token accounts met een vergoeding.                                                                                                              |
| _rs`ConfigureAccountWithRegistry`_  | Alternatieve manier om token accounts te configureren voor vertrouwelijke overdrachten met behulp van een _rs`ElGamalRegistry`_ account in plaats van _rs`VerifyPubkeyValidity`_ proof. |
