...
1. **Vertrouwelijk uitstaand saldo**: Aanvankelijk worden tokens "gestort" van
   het publieke saldo naar een "uitstaand" vertrouwelijk saldo.
...
Dit gedeelte legt de eerste fase uit: het publieke tokensaldo storten naar het
vertrouwelijk uitstaand saldo.

Het volgende diagram toont de stappen voor het storten van tokens van het
publieke saldo naar het vertrouwelijk uitstaand saldo:
...
Om een publiek saldo om te zetten naar een vertrouwelijk uitstaand saldo, roept
u de
...
per storting is beperkt tot 2^48.
...
Het volgende voorbeeld laat zien hoe u een publiek tokensaldo stort naar het
vertrouwelijk uitstaand saldo.
...
Voer het voorbeeld uit tegen een van deze omgevingen (de code gebruikt devnet) met een gevulde betaler,
