...
## Hoe een uitstaand saldo op het beschikbare saldo toepassen

Voordat tokens vertrouwelijk kunnen worden overgedragen, moet het publieke
tokensaldo worden omgezet naar een vertrouwelijk saldo. Deze omzetting verloopt
in twee fasen:

1. **Vertrouwelijk uitstaand saldo**: Eerst worden tokens "gestort" van het
   publieke saldo naar een "uitstaand" vertrouwelijk saldo.
2. **Vertrouwelijk beschikbaar saldo**: Het uitstaande saldo wordt vervolgens
   "toegepast" op het beschikbare saldo, waardoor de tokens beschikbaar worden
   voor vertrouwelijke overdrachten.

Dit gedeelte beschrijft de tweede fase: het toepassen van het uitstaande saldo
op het beschikbare saldo.

Wanneer tokens worden "gestort" vanuit het publieke saldo of wanneer tokens
vertrouwelijk worden overgedragen van het ene token account naar het andere,
worden de tokens aanvankelijk toegevoegd aan het vertrouwelijke uitstaande
saldo. Voordat tokens kunnen worden gebruikt voor vertrouwelijke overdrachten,
moet het uitstaande saldo worden "toegepast" op het beschikbare saldo.
...
het uitstaande saldo op het beschikbare saldo:
...
Om een uitstaand saldo om te zetten naar een beschikbaar saldo, roep de
...
`confidential_transfer_apply_pending_balance` methode die een transactie opbouwt
en verzendt met de `ApplyPendingBalance` instructie, zoals gedemonstreerd in het
onderstaande voorbeeld.

## Voorbeeldcode

Het volgende voorbeeld laat zien hoe het vertrouwelijke uitstaande saldo op het
vertrouwelijke beschikbare saldo wordt toegepast.
...
en vervang de tijdelijke aanduidingen voor mint- en accountadressen door uw eigen adressen.
