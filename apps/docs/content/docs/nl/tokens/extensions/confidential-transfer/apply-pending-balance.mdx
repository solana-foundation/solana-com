---
title: Pending saldo toepassen
description: Leer hoe je pending saldo toepast om fondsen beschikbaar te maken.
---

<Callout type="warn">
  Het ZK ElGamal Program is tijdelijk uitgeschakeld op het mainnet en devnet
  omdat het een beveiligingsaudit ondergaat. Dit betekent dat de confidential
  transfers extension momenteel niet beschikbaar is. Hoewel de concepten nog
  steeds geldig zijn, zullen de code voorbeelden niet werken.
</Callout>

## Hoe je pending saldo toepast op beschikbaar saldo

Voordat tokens vertrouwelijk kunnen worden overgedragen, moet het publieke token
saldo worden omgezet naar een vertrouwelijk saldo. Deze conversie gebeurt in
twee fasen:

1. **Vertrouwelijk pending saldo**: Aanvankelijk worden tokens "gestort" van
   publiek saldo naar een "pending" vertrouwelijk saldo.
2. **Vertrouwelijk beschikbaar saldo**: Het pending saldo wordt vervolgens
   "toegepast" op het beschikbare saldo, waardoor de tokens beschikbaar komen
   voor vertrouwelijke overdrachten.

Deze sectie legt de tweede fase uit: het toepassen van het pending saldo op het
beschikbare saldo.

Wanneer tokens worden "gestort" van het publieke saldo of wanneer tokens
vertrouwelijk worden overgedragen van het ene token account naar het andere,
worden de tokens aanvankelijk toegevoegd aan het vertrouwelijke pending saldo.
Voordat tokens kunnen worden gebruikt voor vertrouwelijke overdrachten, moet het
pending saldo worden "toegepast" op het beschikbare saldo.
...
het pending saldo op het beschikbare saldo:
...
Om een pending saldo om te zetten naar een beschikbaar saldo, roep je de
...
en verstuurt met de `ApplyPendingBalance` instructie, zoals gedemonstreerd in
het voorbeeld hieronder.

### Voorbeeldcode

Het volgende voorbeeld demonstreert hoe je het vertrouwelijke openstaande saldo
toepast op het vertrouwelijke beschikbare saldo.
