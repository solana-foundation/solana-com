...
description:
  Leer hoe je een token account aanmaakt met Confidential Balances ingeschakeld.
---

## Een token account aanmaken met Confidential Balances

Confidential Balances maakt privétokenoverdrachten mogelijk door extra status toe te voegen aan
het token account. In dit gedeelte wordt uitgelegd hoe je een token account aanmaakt met de
vereiste extensie ingeschakeld.

Het volgende diagram toont de stappen voor het aanmaken van een token account met
Confidential Balances ingeschakeld:
...
### Token Account-status van Confidential Balances
...
De _rs`ConfidentialTransferAccount`_ bevat verschillende velden die
Confidential Balances beheren:

- **approved**: De goedkeuringsstatus van het account voor Confidential Balances. Als de
  configuratie `auto_approve_new_accounts` van het mint account is ingesteld op `true`, worden alle
  token accounts automatisch goedgekeurd voor Confidential Balances.
...
Het aanmaken en configureren van een token account voor Confidential Balances maakt gebruik van
de volgende instructies, die allemaal in één enkele transactie passen:
...
4. **Confidential Balances configureren**: Roep de instructie
   ...
   van het Token Extension Program aan, waarbij het bewijs-contextstatusaccount wordt gerefereerd via
   _rs`ProofLocation::ContextStateAccount`_, om de
   _rs`ConfidentialTransferAccount`_-status te initialiseren.

<Callout type="info">

**Alleen de eigenaar van het token account kan Confidential Balances inschakelen op een token
account**.

De instructie _rs`ConfigureAccount`_ vereist de clientzijdige generatie van
versleutelingssleutels en een bewijs dat alleen door de eigenaar van het token account kan worden gegenereerd.

Het pubkey-geldigsheidsbewijs verifieert dat de ElGamal-publieke sleutel van het account geldig is. Het wordt gegenereerd met _rs`build_pubkey_validity_proof_data`_, on-chain geverifieerd door het ZK ElGamal Proof-programma in een contextstatusaccount, en vervolgens gerefereerd vanuit _rs`ConfigureAccount`_ via
_rs`ProofLocation::ContextStateAccount`_, zodat er geen bewijsbytes worden doorgegeven in de token-instructie zelf. Zie voor implementatiedetails:
...
De volgende code maakt een associated token account aan en configureert het om
Confidential Balances te gebruiken met een bestaande compatibele mint.

Confidential Balances is afhankelijk van het ZK ElGamal Proof-programma, dat is ingeschakeld
op mainnet en devnet. Een standaard `solana-test-validator` schakelt dit niet in, maar een
lokale validator met mainnet-fork zoals [Surfpool](https://surfpool.run) wel.
Voer het voorbeeld uit op een van die omgevingen (de code gebruikt devnet) met een gefinancierde betaler,
en vervang de mint-placeholder door een mint die is aangemaakt via
