...
description:
  Leer hoe je privé tokens kunt overmaken van het ene token account naar het andere.
---

## Hoe tokens vertrouwelijk over te dragen van het ene token account naar het andere

Om tokens vertrouwelijk over te maken van het ene token account naar het andere, moeten zowel de
afzender als de ontvanger token accounts hebben die geconfigureerd zijn met de
_rs`ConfidentialTransferAccount`_ status en goedgekeurd zijn voor gebruik van Confidential
Balances. Het token account van de afzender moet ook een beschikbaar vertrouwelijk
saldo hebben om van over te maken.

Om tokens vertrouwelijk over te dragen:
...
1. Maak
   ...
   **Gelijkheidsbewijs
   ...
   Verifieert dat de nieuwe beschikbare saldo-ciphertext na de overdracht
   overeenkomt met de bijbehorende
   [Pedersen commitment](https://en.wikipedia.org/wiki/Commitment_scheme), en
   zorgt ervoor dat het nieuwe beschikbare saldo van de bronaccount correct
   wordt berekend als `new_balance = current_balance - transfer_amount`.

   **Ciphertext-geldigheidsbewijs
   ...
   Verifieert dat de ciphertexts van het overdrachtsbedrag correct zijn
   gegenereerd voor alle drie de partijen (bron, bestemming en controleur), en
   zorgt ervoor dat het overdrachtsbedrag correct is versleuteld onder de
   ...
   **Bereikbewijs
   ...
   Verifieert dat het nieuwe beschikbare saldo en het overdrachtsbedrag
   (opgesplitst in lage/hoge bits) allemaal niet-negatief zijn en binnen een
   opgegeven bereik vallen.

2. Voor elk bewijs:
   - Roep het ZK ElGamal-bewijsprogramma aan om de bewijsgegevens te verifiëren.
   - Sla de bewijsspecifieke metadata op in een bewijs-"contextstate"-account
     voor gebruik in andere instructies.
...
3. Roep de
   ...
   instructie aan en geef de context-state-accounts van de bewijzen op.

4. Sluit de context-state-accounts van de bewijzen om de gebruikte SOL voor het
   aanmaken ervan terug te vorderen.

Het volgende diagram toont de stappen voor het overdragen van tokens van het
token account van een afzender naar het token account van een ontvanger.
...
Om tokens vertrouwelijk over te dragen van het ene token account naar het
andere, moet u:

- Genereer een gelijkheidsbewijs, ciphertext-geldigheidsbewijs en bereikbewijs
  aan de clientzijde
- Roep het Zk ElGamal-bewijsprogramma aan om de bewijzen te verifiëren en de
  "contextstate"-accounts te initialiseren
- Roep de
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  instructie aan en geef de drie bewijsaccounts op.
- Sluit de drie bewijsaccounts om rent terug te vorderen.

Het onderstaande Rust-voorbeeld genereert de bewijzen met de
`spl-token-confidential-transfer-proof-generation` crate, verifieert elk bewijs
in een context-statusaccount via het ZK ElGamal Proof-programma, verwijst naar
de drie accounts in de overdrachtsinstructie en sluit ze daarna. Het
TypeScript-voorbeeld gebruikt de `getConfidentialTransferInstructionPlan` helper
van `@solana-program/token-2022/confidential`, die dezelfde bewijsaccounts,
overdracht en sluitingen samenstelt als een instructieplan met meerdere
transacties.

## Voorbeeldcode

Het volgende voorbeeld maakt tokens vertrouwelijk over van het ene account naar
het andere. Beide accounts moeten al geconfigureerd zijn voor Confidential Balances, en
de afzender moet een beschikbaar vertrouwelijk saldo hebben.

Confidential Balances is afhankelijk van het ZK ElGamal Proof-programma, dat ingeschakeld is
op mainnet en devnet. Een standaard `solana-test-validator` schakelt dit niet in, maar een
lokale validator die mainnet forkt, zoals [Surfpool](https://surfpool.run), wel.
Voer het voorbeeld uit op een van deze omgevingen (de code gebruikt devnet) met een gefinancierde betaler,
en vervang de tijdelijke aanduidingen door uw mint en de afzender- en ontvangeraccounts.
...
fn main() -> Result<()> {
    ...
    // accounts must already be configured for Confidential Balances, and the
