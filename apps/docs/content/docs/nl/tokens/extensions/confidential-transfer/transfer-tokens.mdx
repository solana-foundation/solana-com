...
description:
  Leer hoe je privé tokens kunt overmaken van het ene token account naar het
  andere.
---
...
andere, moeten zowel de afzender als de ontvanger token accounts hebben
geconfigureerd met de _rs`ConfidentialTransferAccount`_ status en goedgekeurd
voor vertrouwelijke overdrachten. Het token account van de afzender moet ook een
beschikbaar vertrouwelijk saldo hebben om van over te dragen.
...
Het volgende voorbeeld draagt tokens vertrouwelijk over van het ene account naar
het andere. Beide accounts moeten al geconfigureerd zijn voor vertrouwelijke
overdrachten, en de verzender moet een beschikbaar vertrouwelijk saldo hebben.
...
schakelt het niet in, maar een lokale validator die mainnet forkt, zoals
[Surfpool](https://surfpool.run), wel. Voer het voorbeeld uit tegen een van deze
omgevingen (de code gebruikt devnet) met een gefinancierde betaler, en vervang
de tijdelijke aanduidingen door uw mint en de verzender- en ontvangersaccounts.
