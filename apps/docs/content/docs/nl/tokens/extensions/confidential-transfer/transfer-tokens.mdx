...
<Callout type="warn">
  ...
  omdat het een beveiligingsaudit ondergaat. Dit betekent dat de extensie voor
  vertrouwelijke overboekingen momenteel niet beschikbaar is. Hoewel de
  concepten nog steeds geldig zijn, zullen de codevoorbeelden niet werken.
</Callout>

## Hoe je vertrouwelijk tokens kunt overmaken van het ene token account naar het andere

Om vertrouwelijk tokens over te maken van het ene token account naar het andere,
moeten zowel de verzender als de ontvanger token accounts hebben die zijn
geconfigureerd met de _rs`ConfidentialTransferAccount`_-status en goedgekeurd
zijn voor vertrouwelijke overboekingen. Het token account van de verzender moet
ook een beschikbaar vertrouwelijk saldo hebben om van over te maken.

Om tokens vertrouwelijk over te maken:

1. Maak aan de clientzijde
   ...
   **Gelijkheidsbewijs
   ...
   Verifieert dat de nieuwe beschikbare saldo-ciphertext na de overboeking
   overeenkomt met de bijbehorende
   [Pedersen-commitment](https://en.wikipedia.org/wiki/Commitment_scheme), zodat
   het nieuwe beschikbare saldo van het bronaccount correct wordt berekend als
   `new_balance = current_balance - transfer_amount`.

   **Ciphertext-validiteitsbewijs
   ...
   Verifieert dat de ciphertexts van het overboekingsbedrag correct zijn
   gegenereerd voor alle drie de partijen (bron, bestemming en auditor), zodat
   het overboekingsbedrag correct is versleuteld onder de publieke sleutel van
   elke partij.
   ...
   **Bereiksbewijis
   ...
   Verifieert dat het nieuwe beschikbare saldo en het overboekingsbedrag
   (opgesplitst in lage/hoge bits) allemaal niet-negatief zijn en binnen een
   gespecificeerd bereik vallen.

2. Voor elk bewijs:
   - Roep het ZK ElGamal-bewijsprogramma aan om de bewijsgegevens te verifiëren.
   - Sla de bewijsspecifieke metadata op in een "context state"-account voor het
     bewijs om te gebruiken in andere instructies.

3. Roep de
   [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)-instructie
   aan en geef de proof context state-accounts door.

4. Sluit de proof context state accounts om de SOL die gebruikt is om ze aan te
   maken terug te krijgen.

Het volgende diagram toont de stappen die betrokken zijn bij het overmaken van
tokens van het token account van een verzender naar het token account van een
ontvanger.
...
- Een equality proof, ciphertext validity proof en range proof client-side
  genereren
- Het Zk ElGamal proof programma aanroepen om de proofs te verifiëren en de
...
- De
  ...
  instructie aanroepen met de drie proof accounts.
- De drie proof accounts sluiten om rent terug te krijgen.
...
Het volgende voorbeeld demonstreert hoe je vertrouwelijk tokens kunt overmaken
