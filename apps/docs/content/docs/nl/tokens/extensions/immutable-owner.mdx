---
title: Onveranderbare eigenaar
description:
  Initialiseer een Token Extension Program token account met onveranderbaar
  eigenaarschap, zodat de eigenaar nooit opnieuw kan worden toegewezen.
...
## Wat Is Immutable Owner?

De _rs`ImmutableOwner`_ account-extensie van het Token Extension Program maakt
de eigenaar van een token account permanent.

Zodra een token account is geïnitialiseerd met _rs`ImmutableOwner`_, kunnen
latere _rs`SetAuthority`_-instructies de _rs`AccountOwner`_-autorisatie niet
meer wijzigen.

<Callout type="info" title="Associated Token Accounts">
  Associated token accounts die zijn aangemaakt via het Associated Token Program
  voor het Token Extension Program initialiseren automatisch
  _rs`ImmutableOwner`_ op het token account.
</Callout>

## Hoe Initialiseer Je een Immutable Owner Account

Om een immutable owner account te initialiseren:

1. Maak een mint aan en initialiseer deze.
2. Maak een token account aan met voldoende ruimte voor _rs`ImmutableOwner`_.
3. Initialiseer _rs`ImmutableOwner`_ op het token account vóór
   _rs`InitializeAccount`_.
4. Een poging om de account-eigenaar later te wijzigen met _rs`SetAuthority`_
   mislukt met _rs`TokenError::ImmutableOwner`_.

<ScrollyCoding>

## !!steps Bereken de grootte van het token account

Bereken de grootte van het token account voor het basis-token account plus de
_rs`ImmutableOwner`_-extensie. Dit is de grootte die wordt gebruikt in
_rs`CreateAccount`_.
...
de _rs`ImmutableOwner`_-extensie.
...
## !!steps Maak het token account aan

Maak het token account aan met de berekende ruimte en lamports.
...
## !!steps Initialiseer ImmutableOwner

Initialiseer de _rs`ImmutableOwner`_ extensie op het token account.
...
## !!steps Initialiseer het token account

Initialiseer het token account met _rs`InitializeAccount`_ in dezelfde
transactie.
...
<Callout type="info" title="Instructievolgorde">
  _rs`InitializeImmutableOwner`_ moet vóór _rs`InitializeAccount`_ komen.
  _rs`CreateAccount`_, _rs`InitializeImmutableOwner`_ en _rs`InitializeAccount`_
...
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| _rs`ImmutableOwner`_                     | Accountextensie die een token account eigenaar als onveranderlijk markeert.                                         | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L13-L16) |
| _rs`InitializeImmutableOwner`_           | Instructie die de onveranderlijke-eigenaar extensie initialiseert op een niet-geïnitialiseerd token account.        | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1982-L1991)           |
| _rs`process_initialize_immutable_owner`_ | Processorlogica die _rs`ImmutableOwner`_ op het account toewijst en initialiseert.                                  | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1532-L1543)               |
| _rs`SetAuthority`_                       | Basis token instructie die wordt gebruikt om autorisaties te wijzigen, inclusief een token account eigenaar.        | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1611-L1633)           |
| _rs`process_set_authority`_              | Processorlogica die eigendomswijzigingen afwijst wanneer het token account _rs`ImmutableOwner`_ heeft ingeschakeld. | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L703-L900)                 |
