...
description:
  Creëer een mint met overdrachtskosten, verplaats ingehouden kosten van token
  accounts naar de mint en trek ze op naar een ontvangende token account voor
  kosten.
...
De _rs`TransferFeeConfig`_ mint-extensie van het Token Extensions Program past
bij elke overdracht voor die mint een vergoeding toe.

Op het moment van overdracht:

- Het overgedragen bedrag wordt verminderd met de geconfigureerde vergoeding
- De ingehouden vergoeding wordt bijgehouden op de bestemmings-token account
- Ingehouden vergoedingen kunnen rechtstreeks worden opgenomen van token
  accounts of verzameld worden bij de mint
- De `withdraw_withheld_authority` kan verzamelde vergoedingen verplaatsen naar
  een ontvangende token account voor kosten

<Callout type="info" title="Token Account-extensies">
  Wanneer een token account wordt geïnitialiseerd voor een mint met
  _rs`TransferFeeConfig`_, wordt het token account geïnitialiseerd met
  _rs`TransferFeeAmount`_. Wanneer het token account wordt aangemaakt via het
  [Associated Token
  ...
  aangemaakt met de vereiste grootte en huurvrije lamports.
</Callout>

## Hoe maak je een mint met overdrachtskosten

Om een mint met overdrachtskosten te maken:

1. Bereken de grootte van het mint-account en de benodigde huur voor de mint en
   de _rs`TransferFeeConfig`_-extensie.
2. Creëer het mint-account met _rs`CreateAccount`_, initialiseer
   _rs`TransferFeeConfig`_ en initialiseer de mint met _rs`InitializeMint`_.
3. Creëer token accounts voor de mint. _rs`TransferFeeAmount`_ wordt automatisch
   ingeschakeld voor token accounts.
4. Gebruik _rs`TransferCheckedWithFee`_ om tokens over te dragen en verifieer de
   verwachte vergoeding. _rs`TransferChecked`_ werkt ook en houdt automatisch de
   geconfigureerde vergoeding in op het bestemmings-token account.
5. Gebruik _rs`WithdrawWithheldTokensFromAccounts`_ om ingehouden vergoedingen
   rechtstreeks van token accounts naar een ontvangende token account voor
   kosten te verplaatsen, ondertekend door de `withdraw_withheld_authority` van
   de mint.
6. Gebruik toestemmingsvrije _rs`HarvestWithheldTokensToMint`_ om ingehouden
   vergoedingen van token accounts naar het mint-account te verplaatsen, en
   gebruik vervolgens _rs`WithdrawWithheldTokensFromMint`_ om de ingehouden
   vergoedingen van het mint-account op te nemen naar een ontvangende token
   account voor kosten, ondertekend door de `withdraw_withheld_authority` van de
   mint.
7. Gebruik _rs`SetTransferFee`_ om de volgende configuratie van
   overdrachtskosten bij te werken, die twee epochs later van kracht wordt.
...
_rs`TransferFeeConfig`_-extensie. Dit is de grootte die wordt gebruikt in
_rs`CreateAccount`_.
...
Bereken rent op basis van de benodigde ruimte voor de mint plus de
_rs`TransferFeeConfig`_-extensie.
...
## !!steps Initialiseer TransferFeeConfig

Initialiseer de _rs`TransferFeeConfig`_-extensie op de mint.
...
Maak token accounts aan voor de bron, ontvangers en ontvanger van vergoedingen,
en mint vervolgens tokens naar de bron-token account.
...
## !!steps Overdracht met TransferCheckedWithFee

Draag tokens over met _rs`TransferCheckedWithFee`_ en verifieer de verwachte
kosten.
...
## !!steps Overdracht met TransferChecked

Draag tokens over met _rs`TransferChecked`_. De geconfigureerde kosten worden
nog steeds ingehouden op het doel-token account.
...
## !!steps Ingehouden kosten opnemen van token accounts

Gebruik _rs`WithdrawWithheldTokensFromAccounts`_ om ingehouden kosten
rechtstreeks van token accounts naar het token account van de kostenontvanger te
verplaatsen.
...
## !!steps Ingehouden kosten verzamelen naar de mint

Gebruik _rs`HarvestWithheldTokensToMint`_ om ingehouden kosten van token
accounts naar de ingehouden accumulator van het mint account te verplaatsen.
...
## !!steps Ingehouden kosten opnemen van de mint

Gebruik _rs`WithdrawWithheldTokensFromMint`_ om verzamelde kosten van het mint
account naar het token account van de kostenontvanger te verplaatsen.
...
## !!steps De volgende overdrachtskosten bijwerken

Gebruik _rs`SetTransferFee`_ om de volgende configuratie van overdrachtskosten
bij te werken.
...
<Callout type="info" title="Instructievolgorde">
  _rs`InitializeTransferFeeConfig`_ moet vóór _rs`InitializeMint`_ komen.
  _rs`CreateAccount`_, _rs`InitializeTransferFeeConfig`_ en _rs`InitializeMint`_
...
| ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`TransferFeeConfig`_                                          | Mint-extensie die de vergoedingsautoriteiten, het geaccumuleerde ingehouden bedrag en de oudere en nieuwere transfervergoedingsconfiguraties opslaat.                                                                                                | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L128-L145)         |
| _rs`TransferFee`_                                                | Vergoedingsconfiguratiestruct opgeslagen in _rs`TransferFeeConfig`_ die het epoch definieert wanneer de vergoeding van kracht wordt, de maximale vergoeding en de vergoeding in basispunten.                                                         | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L32-L40)           |
| _rs`TransferFeeAmount`_                                          | Token account-extensie die het bedrag opslaat dat tijdens transfers voor een token account is ingehouden.                                                                                                                                            | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L169-L177)         |
| _rs`TransferFeeInstruction::InitializeTransferFeeConfig`_        | Instructie die transfervergoedingsinstellingen initialiseert vóór _rs`InitializeMint`_.                                                                                                                                                              | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L24-L48)   |
| _rs`TransferFeeInstruction::TransferCheckedWithFee`_             | Transferinstructie die verifieert of de door de aanroeper opgegeven vergoeding overeenkomt met de huidige transfervergoedingsconfiguratie van de mint.                                                                                               | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L49-L80)   |
| _rs`TransferFeeInstruction::WithdrawWithheldTokensFromAccounts`_ | Instructie die ingehouden vergoedingen rechtstreeks van token accounts naar een vergoedingsontvanger token account verplaatst, ondertekend door de `withdraw_withheld_authority` van de mint.                                                        | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L99-L122)  |
| _rs`TransferFeeInstruction::HarvestWithheldTokensToMint`_        | Toestemmingsloze instructie die ingehouden vergoedingen van token accounts naar de ingehouden accumulator van het mint account verplaatst.                                                                                                           | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L123-L134) |
| _rs`TransferFeeInstruction::WithdrawWithheldTokensFromMint`_     | Instructie die de ingehouden vergoedingen van het mint account opneemt naar een vergoedingsontvanger token account, ondertekend door de `withdraw_withheld_authority` van de mint.                                                                   | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L81-L98)   |
| _rs`TransferFeeInstruction::SetTransferFee`_                     | Instructie die de nieuwere transfervergoedingsconfiguratie bijwerkt, die vanaf twee epochs later van kracht wordt.                                                                                                                                   | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L135-L154) |
| _rs`process_initialize_transfer_fee_config`_                     | Processorlogica die de _rs`TransferFeeConfig`_-extensie op een niet-geïnitialiseerde mint initialiseert en zowel de oudere als nieuwere transfervergoedingsconfiguraties instelt op de initiële vergoeding.                                          | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L26-L58)       |
| _rs`process_withdraw_withheld_tokens_from_accounts`_             | Processorlogica die `withdraw_withheld_authority` valideert en ingehouden vergoedingen van token accounts naar een bestemmings-token account verplaatst.                                                                                             | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L208-L278)     |
| _rs`process_harvest_withheld_tokens_to_mint`_                    | Processorlogica die ingehouden vergoedingen van token accounts naar de ingehouden accumulator van het mint account verzamelt.                                                                                                                        | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L181-L206)     |
| _rs`process_withdraw_withheld_tokens_from_mint`_                 | Processorlogica die `withdraw_withheld_authority` valideert en het ingehouden bedrag van het mint account naar een bestemmings-token account verplaatst.                                                                                             | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L114-L158)     |
| _rs`process_set_transfer_fee`_                                   | Processorlogica die de transfervergoedingsconfiguratie-autoriteit valideert en de nieuwere transfervergoedingsconfiguratie bijwerkt om vanaf twee epochs later van kracht te worden.                                                                 | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L61-L112)      |
| _rs`get_required_init_account_extensions`_                       | Wordt gebruikt om automatisch token account-extensies toe te voegen wanneer token accounts worden geïnitialiseerd op basis van extensies die op de mint zijn ingeschakeld. Voor _rs`TransferFeeConfig`_-mints voegt het _rs`TransferFeeAmount`_ toe. | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mod.rs#L1301-L1303)                    |

### Typescript

Het onderstaande `Kit` voorbeeld gebruikt expliciete Token-2022 instructies.
