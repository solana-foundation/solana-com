...
description:
  Vereist dat inkomende overdrachten een memo bevatten door de
  MemoTransfer-accountextensie in te schakelen op een token account van het
...
## Wat is memo-overdracht?

De _rs`MemoTransfer`_-accountextensie van het Token Extensions Program vereist
een memo-instructie direct voorafgaand aan elke inkomende overdracht naar dat
token account. Uitgaande overdrachten vanaf dat token account vereisen geen
memo.

De extensie wordt geconfigureerd op het bestemmings-token account, niet op de
mint. Eenmaal ingeschakeld, mislukken overdrachten zonder memo met
_rs`TokenError::NoMemo`_ totdat de eigenaar van het token account de vereiste
uitschakelt.

## Hoe maak je een token account aan met MemoTransfer ingeschakeld

Om een token account aan te maken met _rs`MemoTransfer`_ ingeschakeld:

1. Maak een mint aan en initialiseer deze.
2. Bereken de grootte van het token account en de benodigde rent voor een token
   account met _rs`MemoTransfer`_.
3. Maak het token account aan en schakel _rs`MemoTransfer`_ in op dat account.
4. _rs`Transfer`_- en _rs`TransferChecked`_-instructies naar dat token account
   moeten een memo-instructie bevatten direct voorafgaand aan de
   overdracht-instructie.

<ScrollyCoding>

## !!steps Maak de mint aan en initialiseer deze

Maak de mint aan en initialiseer deze die het token account zal gebruiken.
...
_rs`MemoTransfer`_-extensie. Dit is de grootte die wordt gebruikt in
_rs`CreateAccount`_.
...
Bereken rent met behulp van de benodigde grootte van het token account voor de
_rs`MemoTransfer`_- extensie.
...
## !!steps Schakel MemoTransfer in

Schakel _rs`MemoTransfer`_ in op het token account.
...
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`MemoTransfer`_                              | Accountextensie die opslaat of inkomende transfers een memo vereisen.                                             | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/mod.rs#L17-L22)         |
| _rs`RequiredMemoTransfersInstruction::Enable`_  | Instructie die memo-vereiste transfers inschakelt op een token account.                                           | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L50-L71) |
| _rs`RequiredMemoTransfersInstruction::Disable`_ | Instructie die memo-vereiste transfers uitschakelt op een token account.                                          | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L74-L95) |
| _rs`process_toggle_required_memo_transfers`_    | Processorlogica die de _rs`MemoTransfer`_ extensie initialiseert of herschrijft na verificatie van eigenaarschap. | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/memo_transfer/processor.rs#L20-L48)     |

### Typescript

Het onderstaande `Kit` voorbeeld maakt direct gebruik van de gegenereerde
instructies. Verouderde voorbeelden met `@solana/web3.js`, `@solana/spl-token`
en `@solana/spl-memo` zijn ter referentie opgenomen.
