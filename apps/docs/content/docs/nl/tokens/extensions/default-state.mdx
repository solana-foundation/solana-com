...
description:
  Configureer een Token Extensions Program mint zodat nieuw aangemaakte token
  accounts beginnen in een gekozen standaardstatus, zoals bevroren.
...
## Wat Is Standaard Accountstatus?

De _rs`DefaultAccountState`_ mint-extensie van het Token Extensions Program
stelt de initiële _rs`AccountState`_ in voor elk nieuw token account dat voor
die mint wordt aangemaakt.

Dit wordt meestal gebruikt om nieuwe token accounts bevroren te laten beginnen.
Token accounts die bevroren beginnen kunnen niet worden gebruikt totdat de
freeze authority van de mint ze ontdooit.

## Hoe Maak Je Een Mint Met Een Standaard Accountstatus

Om een mint te maken met een standaard accountstatus:

1. Bereken de mint accountgrootte en benodigde rent voor de mint en de
   _rs`DefaultAccountState`_-extensie.
2. Maak het mint account aan met _rs`CreateAccount`_, initialiseer
   _rs`DefaultAccountState`_, en initialiseer de mint met _rs`InitializeMint`_.
3. Maak token accounts voor de mint aan en constateer dat elk nieuw token
   account begint in de huidige standaardstatus.
4. Gebruik _rs`UpdateDefaultAccountState`_ om de status te wijzigen waarin
   toekomstige token accounts moeten beginnen.

<ScrollyCoding>

## !!steps Bereken accountgrootte

Bereken de mint accountgrootte voor de basismint plus de
_rs`DefaultAccountState`_-extensie. Dit is de grootte die wordt gebruikt in
_rs`CreateAccount`_.
...
## !!steps DefaultAccountState initialiseren

Initialiseer de _rs`DefaultAccountState`_ extensie op de mint.
...
## !!steps Een token account aanmaken

Maak een token account aan voor de mint. Omdat de huidige standaardstatus
_rs`Frozen`_ is, begint het nieuwe token account bevroren.
...
## !!steps De standaard accountstatus bijwerken

Gebruik _rs`UpdateDefaultAccountState`_ om de standaardstatus te wijzigen zodat
toekomstige token accounts niet langer bevroren beginnen.
...
<Callout type="info" title="Instructievolgorde">
  _rs`DefaultAccountStateInstruction::Initialize`_ moet vóór
  _rs`InitializeMint`_ komen. _rs`CreateAccount`_,
  _rs`DefaultAccountStateInstruction::Initialize`_, en _rs`InitializeMint`_
...
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`DefaultAccountState`_                        | Mint-extensie die de status opslaat die nieuwe token accounts moeten overnemen.                                       | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/mod.rs#L16-L21)         |
| _rs`DefaultAccountStateInstruction::Initialize`_ | Instructie die de standaard token accountstatus van de mint initialiseert vóór _rs`InitializeMint`_.                  | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L20-L36) |
| _rs`DefaultAccountStateInstruction::Update`_     | Instructie waarmee de freeze authority de standaardstatus voor toekomstige token accounts kan wijzigen.               | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L37-L53) |
| _rs`process_initialize_default_account_state`_   | Processorlogica die de initiële standaardstatus schrijft naar een niet-geïnitialiseerde mint.                         | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L29-L40)     |
| _rs`process_update_default_account_state`_       | Processorlogica die de freeze authority verifieert voordat de opgeslagen standaardstatus van de mint wordt gewijzigd. | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L43-L74)     |
