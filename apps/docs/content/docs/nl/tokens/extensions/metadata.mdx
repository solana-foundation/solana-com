...
description:
  Maak een mint die metadata direct op het mint account opslaat met behulp van de MetadataPointer en TokenMetadata extensies, en update, verwijder en vergrendel die metadata via de token-metadata interface.
...
Het onderstaande `Kit`-voorbeeld gebruikt de gegenereerde instructies direct. Verouderde voorbeelden met `@solana/web3.js`, `@solana/spl-token` en
`@solana/spl-token-metadata` zijn ter referentie opgenomen. De verouderde voorbeelden begrenzen
`maxSupportedTransactionVersion` op `0` omdat web3.js v1 geen
[v1-transacties](/docs/core/transactions/versioned-transactions#v1-message-format) kan deserialiseren.
