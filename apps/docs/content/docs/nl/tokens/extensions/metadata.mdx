...
stelt een mint account in staat om het adres van zijn metadata account te
specificeren. Deze pointer kan verwijzen naar elk account dat eigendom is van
een programma dat de
[Token Metadata Interface](https://github.com/solana-program/token-metadata/tree/main)
implementeert, waardoor flexibiliteit wordt geboden in waar metadata wordt
opgeslagen.
