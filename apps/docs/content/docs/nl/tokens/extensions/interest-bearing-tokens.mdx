...
stelt je in staat om een rentetarief direct op een mint account in te stellen.
Rente wordt continu samengesteld op basis van de netwerktijdstempel, maar het
werkelijke tokensaldo dat in elk token account is opgeslagen blijft ongewijzigd.
De totale waarde (tokensaldo plus opgebouwde rente) is slechts een berekening.
Er worden geen nieuwe tokens gemint met deze extensie.
