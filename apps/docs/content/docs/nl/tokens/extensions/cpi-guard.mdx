...
title: CPI Guard
description:
  Schakel de CpiGuard-accountextensie in om tokeninstructies te blokkeren die
  worden aangeroepen via cross-program invocations.
...
De _rs`CpiGuard`_-accountextensie van het Token Extensions Program voorkomt dat
een ander onchain-programma een token account gebruikt voor bepaalde
tokeninstructies via een Cross Program Invocation (CPI).

Wanneer CPI-beveiliging is ingeschakeld, wijst het Token Extensions Program deze
instructies af wanneer een ander onchain-programma ze aanroept via CPI:

- _rs`Transfer`_, _rs`TransferChecked`_ en _rs`TransferCheckedWithFee`_ wanneer
  de eigenaar van het token account wordt gebruikt als instructie-autoriteit
- _rs`Burn`_ en _rs`BurnChecked`_ wanneer de eigenaar van het token account
  wordt gebruikt als instructie-autoriteit
- _rs`Approve`_ en _rs`ApproveChecked`_
- _rs`CloseAccount`_ wanneer lamports ergens anders heen worden gestuurd dan
  naar de eigenaar van het token account
- _rs`SetAuthority`_ bij het toevoegen of vervangen van de sluitingsautoriteit

Wanneer CPI-beveiliging is ingeschakeld, zijn deze gevallen nog steeds
toegestaan:

- _rs`Transfer`_, _rs`TransferChecked`_, _rs`TransferCheckedWithFee`_,
  _rs`Burn`_ en _rs`BurnChecked`_ via CPI wanneer een gemachtigde of permanente
  gemachtigde ondertekent in plaats van de eigenaar van het token account
- _rs`CloseAccount`_ via CPI wanneer lamports naar de eigenaar van het token
  account worden gestuurd
- _rs`SetAuthority`_ om een bestaande sluitingsautoriteit te verwijderen
- _rs`Revoke`_

Wanneer de eigenaar van het token account wordt gebruikt als
instructie-autoriteit, kunnen Token Extension Program-instructies nog steeds
worden verwerkt als de instructie rechtstreeks aan de transactie wordt
toegevoegd in plaats van te worden aangeroepen via CPI. Het wijzigen van de
eigenaar van het token account met _rs`SetAuthority`_ blijft geblokkeerd zolang
CPI-beveiliging is ingeschakeld, zelfs buiten CPI. _rs`EnableCpiGuard`_ en
_rs`DisableCpiGuard`_ kunnen eveneens niet worden aangeroepen via CPI.
...
2. Bereken de grootte van het token account en de benodigde rent voor een token
   account met _rs`CpiGuard`_.
3. Creëer en initialiseer een token account voor de mint.
4. Roep _rs`EnableCpiGuard`_ aan op het token account.
5. Roep _rs`DisableCpiGuard`_ aan op het token account om die instructies
   opnieuw via CPI toe te staan.

<ScrollyCoding>

## !!steps Creëer en initialiseer de mint
...
## !!steps Bereken grootte token account

Bereken de grootte van het token account voor het basis token account plus de
_rs`CpiGuard`_ extensie. Dit is de grootte die wordt gebruikt in
_rs`CreateAccount`_.
...
Bereken rent met behulp van de grootte van het token account die nodig is voor
de _rs`CpiGuard`_ extensie.
...
## !!steps Creëer en initialiseer het token account

Creëer het token account met de berekende ruimte en lamports, en initialiseer
het vervolgens voor de mint.
...
## !!steps Schakel CPI guard in

Schakel _rs`CpiGuard`_ in op het token account.
...
## !!steps Schakel CPI guard uit

Schakel _rs`CpiGuard`_ uit op het token account.
...
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`CpiGuard`_                     | Accountextensie die opslaat of CPI guard momenteel is ingeschakeld voor het token account.                                                                                                                                        | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/mod.rs#L20-L25)         |
| _rs`CpiGuardInstruction::Enable`_  | Instructie die CPI guard inschakelt op een token account.                                                                                                                                                                         | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/instruction.rs#L20-L38) |
| _rs`CpiGuardInstruction::Disable`_ | Instructie die CPI guard uitschakelt op een token account.                                                                                                                                                                        | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/instruction.rs#L39-L53) |
| _rs`process_toggle_cpi_guard`_     | Processorlogica gebruikt door _rs`Enable`_ en _rs`Disable`_ die accounteigendom valideert, CPI-wijzigingen aan de guard zelf afwijst en _rs`CpiGuard`_ initialiseert indien nog niet aanwezig voordat `lock_cpi` wordt ingesteld. | [Bron](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/cpi_guard/processor.rs#L19-L52)     |

### Typescript

Het onderstaande `Kit`-voorbeeld maakt gebruik van de gegenereerde instructies
direct. Legacy-voorbeelden met `@solana/web3.js` en `@solana/spl-token` zijn ter
