...
description: Kernconcepten voor het verzenden en ontvangen van betalingen op Solana
---
...
Token accounts worden beheerd door een **Token Program**—de onchain code die
...
Token-2022 (ook wel "Token Extensions" genoemd) voegt functies toe zoals transfer hooks,
transferkosten en vertrouwelijke saldi. Beide programma's werken op dezelfde manier voor eenvoudige
overdrachten, maar je moet het juiste programma gebruiken bij het afleiden van ATA's.
...
<Callout type="info">
  Solana maakt gebruik van lokale feemarkten — transacties concurreren met andere transacties
  die naar dezelfde state schrijven. Dit betekent dat transactiekosten laag en voorspelbaar blijven,
  zelfs tijdens perioden van hoge netwerkactiviteit elders.
</Callout>
