...
gebruiken—een geünificeerde ondertekeningsbibliotheek die meerdere key
management backends abstraheert via één enkele interface: Memory, Vault, AWS
KMS, Privy, Turnkey, Fireblocks, CDP, Dfns, Para. Hiermee kun je lokaal
ontwikkelen met in-memory keys en vervolgens overschakelen naar
productie-waardige backends zonder applicatiecode te hoeven wijzigen.
...
om aanvullende key management services te integreren.

### RPC-beveiliging

Behandel RPC-endpoints als API-sleutels—stel ze niet bloot in frontend-code waar
ze geëxtraheerd en misbruikt kunnen worden. Gebruik een backend-proxy of
omgevingsvariabelen die niet in client-code gebundeld worden.
...
- Transfers boven drempelwaarde vanaf treasury
- Pieken in mislukte transacties
- Ongebruikelijke ontvangerspatronen
...
Elk token en programma heeft exact één correct adres op mainnet. Vervalste
tokens die USDC of andere stablecoins nabootsen komen vaak voor—ze hebben
dezelfde naam en symbool maar een andere mint. Je applicatie moet de
mint-adressen hardcoden of op een allowlist plaatsen (gebaseerd op je
vereisten), accepteer ze nooit dynamisch vanuit niet-vertrouwde bronnen.
...
deze niet om te wisselen tijdens het testen, of erger nog, lever geen
devnet-adressen aan productie.
...
verkeerde programma zal mislukken—of erger nog, resulteren in onomkeerbaar
...
- [ ] Mainnet SOL verworven voor kosten en rent
...
- [ ] Retry-logica handelt blockhash-expiratie af
...
- [ ] Load getest op verwacht volume
...
- **Programma-keypair:** Uw programma heeft een ander adres op mainnet dan op
  devnet (tenzij u hetzelfde keypair hergebruikt). Werk alle verwijzingen in uw
  applicatieconfiguratie bij. Bewaar uw programma-keypair op een veilige locatie
  (let op dat het uitvoeren van `cargo clean` waarschijnlijk uw
  programma-keypair zal verwijderen).
- **Accounts initialiseren:** Als uw programma beheeraccounts, PDA's of andere
  toestandsaccounts vereist, zorg er dan voor dat deze op mainnet worden
  aangemaakt voordat gebruikers met uw applicatie interacteren. Hetzelfde geldt
...
- **Bufferaccounts:** Grote programma's worden geïmplementeerd via
  bufferaccounts. Het `solana program deploy`-commando regelt dit automatisch,
  maar begrijp dat implementatie niet atomisch is—bij onderbreking moet u
  mogelijk bufferaccounts herstellen of sluiten. Zie
  [Programma's implementeren](https://solana.com/docs/programs/deploying).
- **Upgrade-autoriteit:** Bepaal of uw programma na lancering upgradebaar moet
  zijn. Voor onveranderlijkheid trekt u de upgrade-autoriteit in na
  implementatie. Voor flexibiliteit beveiligt u de upgrade-autoriteitsleutel op
  passende wijze.
- **Huur:** Zorg ervoor dat uw implementatiewallet voldoende SOL heeft om de
  huurvrijstellingsminima voor alle program accounts te dekken.
- **Verificatie:** [Verifieer](https://solana.com/docs/programs/verified-builds)
  uw programma om ervoor te zorgen dat het uitvoerbare programma dat u op het
