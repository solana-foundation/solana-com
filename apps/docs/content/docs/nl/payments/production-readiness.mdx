...
Voor productie backend signing, overweeg het gebruik van
[solana-keychain](https://github.com/solana-foundation/solana-keychain)—een
geïntegreerde signing bibliotheek die meerdere key management backends
abstraheert via een enkele interface: Memory, Vault, Privy, Turnkey, AWS KMS,
Fireblocks, GCP KMS, CDP, Para, Dfns, Crossmint. Hiermee kunt u lokaal
ontwikkelen met in-memory keys en vervolgens overschakelen naar
productie-waardige backends zonder uw applicatiecode te wijzigen.
