...
Voor productie backend-ondertekening gebruik je
[Keychain](/docs/tools/keychain)—een uniforme ondertekeningsbibliotheek die
meerdere sleutelbeheer-backends via één interface abstraheert: Memory, Vault,
Privy, Turnkey, AWS KMS, Fireblocks, GCP KMS, CDP, Para, Dfns, Crossmint,
Openfort en Utila. Hiermee kun je lokaal ontwikkelen met in-memory sleutels en
vervolgens overschakelen naar productie-waardige backends zonder de
applicatiecode te wijzigen.

Weet je niet welke backend bij je past? Zie
...
Behandel RPC-eindpunten zoals API-sleutels—stel ze niet bloot in frontend-code
waar ze kunnen worden uitgelezen en misbruikt. Gebruik een backend-proxy of
omgevingsvariabelen die niet worden meegebundeld in de clientcode.
...
| Statistiek                     | Waarom                           |
| ------------------------------ | -------------------------------- |
| Succespercentage transacties   | Problemen vroegtijdig detecteren |
| Bevestigingslatentie           | Netwerkgezondheid bewaken        |
| Uitgaven aan prioriteitskosten | Kostenbeheer                     |
| RPC-foutpercentage             | Gezondheid van de provider       |

Stel meldingen in voor:

- Overboekingen boven drempelwaarde vanuit de treasury
- Pieken in het percentage mislukte transacties
- Ongebruikelijke ontvangerpatronen
- Toename van het RPC-foutpercentage

Voor real-time transactiemonitoring op schaal, zie onze
[Indexeringsgids](/docs/payments/accept-payments/indexing).

### Adressen verifiëren

Elk token en programma heeft precies één correct adres op het mainnet. Vervalste
tokens die USDC of andere stablecoins nabootsen komen vaak voor—ze hebben
dezelfde naam en hetzelfde symbool, maar een ander mint-adres. Je applicatie
moet de mint-adressen hardcoderen of op een allowlist plaatsen (op basis van je
vereisten) en ze nooit dynamisch accepteren van niet-vertrouwde bronnen.

**Omgevingsgebaseerde configuratie:** Devnet en Mainnet gebruiken vaak volledig
verschillende token mints. Stel je applicatieconfiguratie in om de juiste
adressen per omgeving te laden—hardcode geen mainnet-adressen en vergeet niet
deze te wisselen tijdens het testen, of erger nog, stuur geen devnet-adressen
naar productie.

Enkele veelgebruikte stablecoin mints zijn:
...
Programma-adressen zijn ook belangrijk. Instructies naar het verkeerde programma
sturen zal mislukken—of erger, resulteren in onomkeerbaar verlies van fondsen.
De Token Program-adressen zijn:
...
Het op een allowlist zetten van de juiste adressen beschermt tegen vervalste
tokens, maar het beschrijft niet tegen het verzenden naar het verkeerde _soort_
account. Een ontvangersadres kan een wallet, een token account, een mint of een
programma zijn, en elk vereist een andere afhandeling. Native SOL verzonden naar
iets anders dan een wallet verlaat de controle van de verzender — direct
verloren bij een mint of programma, alleen herstelbaar door de eigenaar van het
account bij een token account — zonder een mislukte transactie om de gebruiker
te waarschuwen.

<Callout type="warn" title="Valideer ontvangers vóór het verzenden">
  Classificeer elk ontvangersadres vóór het ondertekenen van een overdracht. Zie
  ...
  programma's van elkaar onderscheidt.
</Callout>

## Checklist vóór lancering

- [ ] Mainnet SOL aangeschaft voor kosten en rent
- [ ] Productie-RPC geconfigureerd (geen publiek eindpunt)
- [ ] Fallback RPC-eindpunt geconfigureerd
- [ ] Prioriteitskosten geïmplementeerd met dynamische prijsstelling
- [ ] Retry-logica verwerkt blockhash-vervaldatum
- [ ] Bevestigingsniveau passend voor het gebruik
...
- [ ] Validatie van ontvangersadres vóór verzending (wallet vs token account vs
      mint vs program)
- [ ] Alle sleutels veilig opgeslagen
- [ ] Sleutelbeheer gecontroleerd (geen sleutels in de frontend)
- [ ] Transactiebewaking en meldingen actief
...
- **Solana CLI-versie:** Zorg ervoor dat u de nieuwste versie van de
  [Solana CLI](https://solana.com/docs/intro/installation) gebruikt.
- **Program keypair:** Uw programma heeft op mainnet een ander adres dan op
  devnet (tenzij u hetzelfde keypair hergebruikt). Werk alle verwijzingen in uw
  applicatieconfiguratie bij. Sla uw program keypair op een veilige locatie op
  (let op: het uitvoeren van `cargo clean` verwijdert waarschijnlijk uw program
  keypair).
- **Accounts initialiseren:** Als uw programma beheerdersaccounts, PDA's of
  andere statusaccounts vereist, zorg er dan voor dat deze op mainnet zijn
  aangemaakt voordat gebruikers met uw applicatie communiceren. Hetzelfde geldt
  voor eventuele Associated Token Accounts (ATA's) die uw programma nodig heeft.

### Implementatieproces

- **Buffer accounts:** Grote programma's worden via buffer accounts
  geïmplementeerd. Het `solana program deploy` commando verwerkt dit
  automatisch, maar houd er rekening mee dat implementatie niet atomisch is —
  bij onderbreking moet u mogelijk buffer accounts herstellen of sluiten. Zie
  [Programma's implementeren](https://solana.com/docs/programs/deploying).
- **Upgrade-autoriteit:** Bepaal of uw programma na de lancering upgradeable
  moet zijn. Voor onveranderlijkheid kunt u de upgrade-autoriteit na
  implementatie intrekken. Voor flexibiliteit beveiligt u de
  upgrade-autoriteitssleutel op de juiste wijze.
- **Rent:** Zorg ervoor dat uw implementatiewallet voldoende SOL heeft om de
  rent-exempt minimums te dekken voor alle program accounts.
- **Verificatie:** [Verifieer](https://solana.com/docs/programs/verified-builds)
  uw programma om te garanderen dat het uitvoerbare programma dat u op het
  Solana-netwerk implementeert overeenkomt met de broncode in uw repository

Voor volledige richtlijnen voor het implementeren van programma's, zie
[Programma's implementeren](https://solana.com/docs/programs/deploying).
