...
geünificeerde signing-bibliotheek die meerdere key management backends
abstraheert via een enkele interface: Memory, Vault, Privy, Turnkey, AWS KMS,
Fireblocks, GCP KMS, CDP, Para, Dfns. Hiermee kun je lokaal ontwikkelen met
in-memory keys en vervolgens overschakelen naar production-grade backends zonder
de applicatiecode te wijzigen.
