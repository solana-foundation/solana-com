...
- De token account van de gebruiker moet bestaan voor initialisatie.
- Bedragen zijn in basiseenheden. Voor een token met 6 decimalen betekent
  `1_000_000` dat `1` token.
- Voer `initSubscriptionAuthority` alleen uit wanneer het Subscription
  Authority-account nog niet bestaat.
- De gebruiker ondertekent setup- en intrekkingstransacties. De gedelegeerde
  ondertekent overboekingen.
