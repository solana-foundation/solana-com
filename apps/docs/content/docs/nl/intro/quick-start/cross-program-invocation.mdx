...
Eerst heeft het programma een eenvoudig "betaal-om-bij-te-werken" mechanisme
nodig door de _rs`Update`_ struct en `update` functie te wijzigen.

Begin met het bijwerken van het `lib.rs` bestand om items uit de
`system_program` module binnen scope te brengen.
...
De _rs`transfer()`_ functie roept de overdrachtsinstructie van het System
Program aan.
...
- `to` - Het kluisaccount (bestemming van fondsen)
...
- Het te transfereren `amount` (1.000.000 lamports, of 0,001 SOL)
...
De setup voor een CPI komt overeen met hoe instructies aan de clientzijde worden
opgebouwd, waarbij je het programma, de accounts en de instructiedata opgeeft
voor een specifieke aan te roepen instructie. Wanneer de `update` instructie van
het programma wordt aangeroepen, roept het intern de overdrachtsinstructie van
het System Program aan.
...
struct en de `delete` functie te wijzigen.

Update eerst de _rs`Delete`_ struct om de `vault_account` op te nemen. Hierdoor
kan eventuele SOL in de kluis worden teruggestort naar de gebruiker wanneer ze
...
Het toevoegen van de `vault_account` aan de Delete struct stelt het programma in
staat om tijdens de delete-instructie toegang te krijgen tot het kluis-account
van de gebruiker om eventuele opgebouwde SOL terug over te maken naar de
gebruiker.
