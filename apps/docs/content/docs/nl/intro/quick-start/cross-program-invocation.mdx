...
Eerst heeft het programma een eenvoudig "betaal-om-te-updaten" mechanisme nodig
door de _rs`Update`_ struct en `update` functie te wijzigen.

Begin met het updaten van het `lib.rs` bestand om items uit de `system_program`
module binnen scope te brengen.
...
Update vervolgens de _rs`Update`_ struct om een nieuw account genaamd
`vault_account` op te nemen. Dit account, beheerd door het programma, ontvangt
SOL van een gebruiker wanneer ze hun berichtaccount updaten.
...
Deze sectie voegt een nieuw account genaamd `vault_account` toe aan de
_rs`Update`_ struct. Dit account dient als een door het programma beheerde
"kluis" die SOL ontvangt van gebruikers wanneer ze hun berichten updaten.
...
- Als PDA heeft het geen private key, dus alleen het programma kan "tekenen"
  voor het adres bij het uitvoeren van CPI's
- Als _rs`SystemAccount`_ type is het eigendom van het System Program, net als
  gewone wallet accounts
...
In de `delete` instructie zie je hoe het programma kan "tekenen" voor deze PDA
in een CPI.
...
De _rs`CpiContext`_ struct specificeert het programma en accounts voor een Cross
...
demonstreert hoe je een CPI uitvoert vanuit het programma, waardoor de
componeerbaarheid van Solana-programma's mogelijk wordt.

De _rs`Transfer`_ struct specificeert de vereiste accounts voor de
transfer-instructie van het System Program:

- `from` - Het account van de gebruiker (bron van fondsen)
- `to` - Het kluis-account (bestemming van fondsen)
...
- Het aan te roepen programma (System Program)
- De vereiste accounts in de CPI (gedefinieerd in de _rs`Transfer`_ struct)
...
De `transfer` functie roept vervolgens de transfer-instructie aan op het System
Program, waarbij het doorgeeft:

- De `cpi_context` (programma en accounts)
- Het over te dragen `amount` (1.000.000 lamports, of 0,001 SOL)
...
De opzet voor een CPI komt overeen met hoe instructies aan de clientzijde worden
opgebouwd, waarbij je het programma, accounts en instruction data specificeert
voor een bepaalde instructie om aan te roepen. Wanneer de `update` instructie
van het programma een aanroep ontvangt, roept het intern de transfer-instructie
van het System Program aan.
...
Werk eerst de _rs`Delete`_ struct bij om de `vault_account` op te nemen. Dit
maakt het mogelijk om eventuele SOL in de kluis terug te sturen naar de
gebruiker wanneer ze hun berichtaccount sluiten.
...
Voeg ook het `system_program` toe, aangezien de CPI voor de overdracht het
...
Het `vault_account` gebruikt dezelfde PDA-afleiding als in de Update struct.

Door het `vault_account` toe te voegen aan de Delete struct, kan het programma
toegang krijgen tot het kluis-account van de gebruiker tijdens de
delete-instructie om eventuele opgebouwde SOL terug te sturen naar de gebruiker.
...
van het kluis-account terug naar het account van de gebruiker.
...
om de context te gebruiken in de body van de functie.
...
De _rs`transfer()`_ functie roept vervolgens de overdrachtsinstructie aan op het
System Program, waarbij het volgende wordt doorgegeven:

- De `cpi_context` (programma, accounts en PDA signer)
- Het over te maken bedrag (het volledige saldo van het kluis-account)
...
Werk vervolgens het `anchor.test.ts` bestand bij om het nieuwe kluis-account op
te nemen in de instructies. Dit vereist het afleiden van de kluis-PDA en het
opnemen ervan in de update- en delete-instructie-aanroepen.
