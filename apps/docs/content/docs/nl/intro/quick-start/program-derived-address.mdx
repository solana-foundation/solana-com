...
controleren of het startprogramma succesvol bouwt.
...
accountgegevens vertegenwoordigen (datatype om op te slaan in het dataveld van
het account).
...
["bump" seed](/docs/core/pda#canonical-bump) op die wordt gebruikt om een
Program Derived Address (PDA) af te leiden. Het opslaan van deze waarde bespaart
rekenkracht door de noodzaak om deze in latere instructies opnieuw te berekenen
te elimineren.
...
gebruikers is aangemaakt en drie velden bevat:
...
- `bump` - _rs`u8`_ die de ["bump" seed](/docs/core/pda#canonical-bump) opslaat
  ...
  waarde bespaart rekenkracht door de noodzaak om deze in latere instructies
  opnieuw te berekenen te elimineren.

Bij het aanmaken van een account serialiseert het programma de
_rs`MessageAccount`_ data en slaat deze op in het dataveld van het nieuwe
account.

Later, bij het lezen van het account, deserialiseert het programma deze data
terug naar het _rs`MessageAccount`_ datatype. De testsectie demonstreert het
proces van het aanmaken en lezen van accountdata.
...
De _rs`mut`_ constraint declareert het account als muteerbaar.
...
De _rs`bump`_ constraint declareert de bump seed voor de PDA.
...
toegewezen voor het dataveld van het nieuwe account.
...
1. Het accounttype (zoals _rs`Signer<'info>`_ of _rs`Account<'info, T>`_) dat
   specificeert wat voor soort account het programma verwacht
2. Optionele constraints (zoals _rs`#[account(mut)]`_ of _rs`#[account(init)]`_)
...
1. _rs`user: Signer<'info>`_
   ...
   - Moet de transactie ondertekenen om aftrekking van lamports van dit account
     goed te keuren

2. _rs`message_account: Account<'info, MessageAccount>`_
   - Het nieuwe account dat het bericht van de gebruiker opslaat
   - `init` constraint maakt het account aan tijdens de uitvoering van de
     instructie
   - `seeds` en `bump` constraints leiden het accountadres af als een Program
     Derived Address (PDA)
   - `payer = user` identificeert wie betaalt voor de aanmaak van het nieuwe
     account
   - `space` wijst de vereiste bytes toe voor het data-veld van het account
...
- _rs`b"message"`_ - Een vaste string als de eerste seed.
- _rs`user.key().as_ref()`_ - De publieke sleutel van het _rs`user`_ account als
  de tweede seed.
...
- Anchor Account discriminator (identificatie): 8 bytes
...
slaan, en de resterende lengte bevat de daadwerkelijke data.
...
De `create` functie implementeert de logica voor het initialiseren van de data
van een nieuw bericht account. Het neemt twee parameters:
...
2. Account data initialiseren:
   - Krijgt toegang tot het `message_account` vanuit de context.

   ```rs
   let account_data = &mut ctx.accounts.message_account;
   ```

   - Stelt het `user` veld in op de publieke sleutel van het `user` account.

   ```rs
   account_data.user = ctx.accounts.user.key();
   ```

   - Stelt het `message` veld in op de `message` uit het functieargument.
...
De _rs`realloc`_ beperking wijst de data van het account opnieuw toe.

### !realloc::payer

De _rs`realloc::payer`_ beperking specificeert het account dat betaalt voor de
herallocatie.

### !realloc::zero

De _rs`realloc::zero`_ beperking stelt de data van het account op nul.
...
De _rs`Update`_ struct definieert de accounts die vereist zijn voor de
...
1. _rs`user: Signer<'info>`_
   ...
   - Gemarkeerd als muteerbaar (_rs`#[account(mut)]`_) omdat het mogelijk
     betaalt voor meer ruimte voor het `message_account` wanneer nodig
   - Moet de transactie ondertekenen

2. _rs`message_account: Account<'info, MessageAccount>`_
   - Het bestaande account dat het bericht van de gebruiker opslaat voor
     bijwerken
   - `mut` beperking geeft datawijziging aan voor dit account
   - `realloc` beperking maakt het mogelijk om de grootte van de accountdata aan
     te passen
   - `seeds` en `bump` beperkingen verifiëren het account als de juiste PDA

3. _rs`system_program: Program<'info, System>`_
   - Vereist voor mogelijke herallocatie van accountruimte
   - De `realloc` beperking roept het System Program aan om de datagrootte van
     het account aan te passen

Merk op dat de _rs`bump = message_account.bump`_ constraint de bump seed
gebruikt die opgeslagen is op de _rs`message_account`_, in plaats van dat Anchor
deze opnieuw berekent.
...
De functie `update` implementeert de logica voor het wijzigen van een bestaand
berichtaccount. Het neemt twee parameters:

1. _rs`ctx: Context<Update>`_ - Biedt toegang tot de accounts die zijn
   gespecificeerd in de _rs`Update`_ struct.
2. _rs`message: String`_ - Het nieuwe bericht ter vervanging van het bestaande.
...
2. Werkt accountdata bij:
   - Krijgt toegang tot de `message_account` vanuit de context.
   - Stelt het `message` veld in op het nieuwe `message` uit het
     functieargument.
...
Update de _rs`Delete`_ struct met het volgende:
...
1. _rs`user: Signer<'info>`_
   ...
   - Moet ondertekenen om ervoor te zorgen dat alleen de juiste gebruiker hun
     berichtaccount kan sluiten

2. _rs`message_account: Account<'info, MessageAccount>`_
   - Het account dat wordt gesloten
   - `mut` constraint geeft aan dat data wordt gewijzigd
   - `seeds` en `bump` constraints verifiëren het account als de juiste PDA
   - `close = user` constraint markeert dit account voor sluiting en draagt de
...
De functiebody print alleen een bericht naar de programmalogs met behulp van de
_rs`msg!()`_ macro. De functie heeft geen extra logica nodig omdat de
_rs`close`_ constraint in de _rs`Delete`_ struct het sluiten van het account
afhandelt.
...
Je hebt nu het basis CRUD-programma voltooid. Deploy het programma door `deploy`
...
Solana Playground verwijdert wat standaard setup waarbij
...
Voer het testbestand uit door _shell`test`_ in de Playground terminal te draaien
om te controleren of het werkt zoals verwacht. De volgende stappen voegen de
daadwerkelijke tests toe.
...
aanroept, waarby "Hello, World!" als bericht wordt meegegeven.
...
aanroept, waarbij "Hello, Solana!" als nieuw bericht wordt meegegeven.
...
dat op het devnet is gedeployed en logt links naar SolanaFM om de
