...
description:
  ...
  behulp van Program Derived Addresses (PDA's) en het Anchor-framework. Deze
  stapsgewijze handleiding demonstreert hoe je onchain berichtaccounts kunt
  aanmaken, bijwerken en verwijderen met PDA's, accountvalidatie implementeert
  en tests schrijft. Ideaal voor ontwikkelaars die willen begrijpen hoe PDA's in
  Solana-programma's worden gebruikt.
...
rekenkracht door de noodzaak te elimineren om het in latere instructies opnieuw
te berekenen.
...
- `bump` - _rs`u8`_ die de ["bump" seed](/docs/core/pda#canonical-bump) opslaat
  ...
  waarde bespaart rekenkracht door de noodzaak te elimineren om het in latere
  instructies opnieuw te berekenen.
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - Achter de schermen roept de `init` beperking het System Program aan om een
...
De `seeds` en `bump` beperkingen definiëren samen het adres van een account als
...
van een nieuw berichtaccount. Het neemt twee parameters:
...
Net als bij de vorige stap, specificeer eerst de accounts die nodig zijn voor de
`update` instructie.
...
De _rs`realloc`_ constraint wijst de data van het account opnieuw toe.
...
De _rs`realloc::zero`_ constraint stelt de data van het account op nul.
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - `realloc` constraint maakt het mogelijk om de data van het account te
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - De `realloc` constraint roept het System Program aan om de datagrootte van
     het account aan te passen

Merk op dat de _rs`bump = message_account.bump`_ constraint de bump seed
gebruikt die is opgeslagen op de _rs`message_account`_, in plaats van dat Anchor
...
De _rs`Delete`_ struct definieert de accounts die vereist zijn voor de
...
De functiebody print alleen een bericht naar programmalogs met behulp van de
