...
title: Mollusk
description:
  Uitgebreide handleiding voor het testen van Solana-programma's met Mollusk
---

[Mollusk](https://github.com/anza-xyz/mollusk/tree/main) is een lichtgewicht
test-harness voor het testen van Solana-programma's. Het biedt een eenvoudige
...
- [Mollusk test-gebruiksvoorbeelden](https://github.com/anza-xyz/mollusk/tree/0.5.0/harness/tests)
...
Om het compute unit-gebruik te benchmarken, voeg `mollusk-svm-bencher` toe als
...
Het volgende voorbeeld toont een minimale setup voor het testen van een basaal
Solana-programma met Mollusk.

<ScrollyCoding>

## !!steps Hello World-programma

Dit voorbeeld demonstreert hoe je een basaal Solana-programma test met Mollusk.
Het programma print simpelweg "Hello, world!" naar de programmalogs wanneer het
wordt aangeroepen.
...
Om de test uit te voeren, voer je `cargo test` uit.

Wanneer de test succesvol wordt uitgevoerd, zie je output vergelijkbaar met het
volgende:
...
<Callout type="info">
  Voeg bij het opgeven van het bestandspad de `.so` extensie niet toe.
...
Het volgende voorbeeld verwerkt een SOL-overdrachtsinstructie zonder
validatiecontroles.

<Callout type="info">

De onderstaande voorbeelden draaien Mollusk in de `main` functie ter
demonstratie. In de praktijk gebruik je Mollusk doorgaans in een testmodule die
is geannoteerd met het `#[test]` attribuut.
...
te verwerken met validatiecontroles. Deze methode zal paniekeren als een
controle faalt.
...
Het volgende voorbeeld verwerkt een keten van twee SOL-overdrachtsinstructies
...
struct voor het aanpassen van zijn waarden voor testen.

Gebruik de `warp_to_slot` methode om de sysvar clock bij te werken om vooruit of
...
Het volgende voorbeeld laat zien hoe je de Mollusk sysvar direct kunt aanpassen
door toegang te krijgen tot het `sysvars` veld om de rent parameters te
wijzigen. Je kunt andere sysvar waarden op dezelfde manier aanpassen.
...
volgt het compute unit gebruik van de instructies van een programma. Resultaten
worden geschreven naar een markdown bestand.
...
De benchmark resultaten worden geschreven naar het opgegeven `out_dir` als een
...
Het volgende voorbeeld demonstreert het testen van een token transfer met
Mollusk.

<Callout type="info">
  Het onderstaande voorbeeld definieert handmatig de test accounts voor
