---
title: Mollusk
description: Uitgebreide gids voor het testen van Solana-programma's met Mollusk
---

[Mollusk](https://github.com/anza-xyz/mollusk/tree/main) is een lichtgewicht
test framework voor het testen van Solana-programma's. Het biedt een eenvoudige
interface voor het testen van Solana-programma-instructies in een
geminimaliseerde Solana Virtual Machine (SVM) omgeving. Alle testaccounts moeten
...
Om het gebruik van compute units te benchmarken, voeg `mollusk-svm-bencher` toe
...
Om het Token Program, token2022 programma (Token Extensions) en Associated Token
Program te gebruiken voor testen met Mollusk, voeg `mollusk-svm-programs-token`
toe als dependency in `Cargo.toml`:
...
eenvoudig Solana-programma met Mollusk.

<ScrollyCoding>

## !!steps Hello World-programma

Dit voorbeeld laat zien hoe je een eenvoudig Solana-programma kunt testen met
...
1. [Maak een `Mollusk` instantie](mention:mollusk) - Initialiseer Mollusk met
...
3. [Verwerk en valideer](mention:process) - Verwerk de instructie met behulp van
   Mollusk en valideer het resultaat
...
struct biedt een eenvoudige interface voor het testen van Solana-programma's.
Alle velden kunnen worden gemanipuleerd via een handvol hulpmethoden, maar
gebruikers kunnen ze ook direct openen en wijzigen als ze meer controle willen.

Om Mollusk te initialiseren met een standaard instantie gebruik je de
...
<Callout type="info">
  Wanneer je het bestandspad opgeeft, voeg dan niet de `.so` extensie toe.
...
| ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------- |
| [`process_instruction`](#single-instruction)                                   | Verwerk een instructie en retourneer het resultaat.                                                               |
| [`process_and_validate_instruction`](#single-instruction-with-checks)          | Verwerk een instructie en voer een reeks controles uit op het resultaat, met paniek als een controle faalt.       |
| [`process_instruction_chain`](#multiple-instructions)                          | Verwerk meerdere instructies en retourneer het resultaat.                                                         |
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | Verwerk meerdere instructies en voer een reeks controles uit op elk resultaat, met paniek als een controle faalt. |
...
Het volgende voorbeeld verwerkt een SOL overdrachts-instructie zonder validatie
controles.

<Callout type="info">

De onderstaande voorbeelden draaien Mollusk in de `main` functie ter
demonstratiedoeleinden. In de praktijk gebruik je Mollusk doorgaans in een
testmodule geannoteerd met het `#[test]` attribuut.
...
te verwerken met validatiecontroles. Deze methode zal in paniek raken als een
controle faalt.
...
Het volgende voorbeeld verwerkt een SOL-overdrachtsinstructie met
validatiecontroles.
...
sequentieel te verwerken zonder validatiecontroles.
...
Het volgende voorbeeld verwerkt twee SOL-overdrachtsinstructies zonder
validatiecontroles.
...
instructie heeft zijn eigen set controles die moeten slagen.
...
Het volgende voorbeeld verwerkt een reeks van twee SOL-overdrachtsinstructies
...
is een wrapper rond `Mollusk` die accountstatus behoudt over meerdere
instructie-aanroepen heen via zijn `account_store`. De methoden voor het
verwerken van instructies zijn identiek aan `Mollusk`.

In tegenstelling tot `Mollusk`, dat vereist dat `accounts` aan elke methode
wordt doorgegeven (bijv. `process_instruction`), beheert `MolluskContext`
accounts intern via zijn `account_store`. Dit elimineert de noodzaak voor de
`accounts`-parameter bij het verwerken van instructies.
...
Het volgende voorbeeld verwerkt twee afzonderlijke SOL-overdrachtsinstructies
met persistente accountstatus tussen de instructies via de `account_store`.
...
[`Sysvars`](https://docs.rs/mollusk-svm/0.5.0/mollusk_svm/sysvar/struct.Sysvars.html)-struct
voor het aanpassen van waarden voor testen.

Gebruik de `warp_to_slot`-methode om de sysvar-klok bij te werken om vooruit of
achteruit in de tijd te simuleren naar een specifiek slot.
...
Het volgende voorbeeld toont hoe je de Mollusk-sysvar direct kunt aanpassen door
toegang te krijgen tot het `sysvars`-veld om de rent-parameters te wijzigen. Je
kunt andere sysvar-waarden op dezelfde manier aanpassen.
...
volgt het gebruik van compute units van de instructies van een programma. De
resultaten worden naar een markdown-bestand geschreven.

Vereist
[`mollusk-svm-bencher`](https://docs.rs/mollusk-svm-bencher/0.5.0/mollusk_svm_bencher/index.html)
als afhankelijkheid.

Het volgende voorbeeld benchmarkt het compute unit gebruik van een SOL transfer
instructie.
...
De benchmarkresultaten worden naar de opgegeven `out_dir` geschreven als een
markdown-bestand met de naam `compute_units.md`.
...
Gebruik de
[`mollusk-svm-programs-token`](https://docs.rs/mollusk-svm-programs-token/0.5.0/mollusk_svm_programs_token/index.html)-crate
om het Token Program, Token2022 Program (Token Extensions) en Associated Token
Program toe te voegen aan Mollusk voor testen.
...
<Callout type="info">
  Het onderstaande voorbeeld definieert de testaccounts handmatig ter
  demonstratie. De `mollusk-svm-programs-token` bevat ook hulpfuncties om de
  mint- en tokenaccounts aan te maken.
</Callout>
