...
`lib.rs`-bestand, hoewel je in de praktijk grotere programma's wellicht over
meerdere bestanden wilt verdelen.
...
Laten we nu de benodigde dependencies toevoegen voor het bouwen van een
...
`CounterInstruction` enum varianten. De Borsh `try_from_slice` methode verzorgt
deze conversie automatisch.
...
hun juiste handler functies te routeren.
...
instructie representeert, waarna de juiste handler functie wordt aangeroepen.
Elke handler functie bevat de implementatie voor die instructie.
...
schrijven en de data te initialiseren.
...
Laten we eerst de dependencies toevoegen die nodig zijn voor testen. We
gebruiken `litesvm` voor testen en `solana-sdk`.
...
de output van het vorige commando:
...
Je kunt de deployment verifiëren met het `solana program show` commando met je
programma-ID:
...
output transactiehandtekeningen. Let op dat het cluster op Solana Explorer moet
worden ingesteld op "Custom RPC URL", wat standaard `http://localhost:8899` is
