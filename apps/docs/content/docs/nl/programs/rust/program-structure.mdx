...
description:
  ...
  state management, instruction handling en testing.
...
- `processor.rs`: Definieert de instruction handlers (functies) die de
...
`lib.rs`-bestand, hoewel je in de praktijk grotere programma's mogelijk in
...
## !!steps Voeg afhankelijkheden toe
...
<Callout type="info">
  Er is geen verplichting om Borsh te gebruiken. Het is echter een veelgebruikte
...
wanneer het programma wordt gestart. Laten we beginnen met het toevoegen van de
imports die we nodig hebben voor het programma en het instellen van het
entrypoint.
...
Laten we nu de hoofd `process_instruction` functie bijwerken om instructies naar
...
Voeg de volgende code toe aan `lib.rs` om de `process_instruction` functie bij
te werken en de handlers toe te voegen voor de `InitializeCounter` en
`IncrementCounter` instructies:
...
aanroepen van een ander programma vanuit ons programma is.
...
- **State management**: Definieert accountgegevensstructuren en beheert hun
  status in accounts die eigendom zijn van het programma
- **Cross program invocation (CPI)**: Roept het System Program aan om nieuwe
  accounts aan te maken die eigendom zijn van het programma
...
Laten we de initialisatie-instructie testen door een nieuw telleraccount aan te
maken met een startwaarde.
...
Na initialisatie controleren we of het telleraccount correct is aangemaakt met
de verwachte waarde.
...
Tot slot controleren we of de increment correct heeft gewerkt door de
bijgewerkte tellerwaarde te controleren.
...
Voer het volgende commando uit om je programma-ID op te halen uit het
keypair-bestand:
...
Voeg de clientcode toe aan `examples/client.rs` en vervang de `program_id` door
...
Terwijl de validator draait, implementeer je programma op de lokale cluster:
