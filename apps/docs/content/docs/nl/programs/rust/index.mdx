...
<Callout type="warn">
  Wijzig het `edition` veld in `Cargo.toml` naar `2021`. Anders kun je een
...
<Callout type="warn">
  Als je deze configuratie niet toevoegt, wordt de `target/deploy` map niet
...
minimaal Solana-programma dat "Hello, world!" naar de programmalog schrijft
wanneer het programma wordt aangeroepen.

De `msg!` macro wordt in Solana-programma's gebruikt om een bericht naar de
programmalog te schrijven.
...
1. Een `.so` bestand (bijv. `hello_world.so`): Dit is het gecompileerde
   Solana-programma dat als "smart contract" naar het netwerk wordt gedeployed.
2. Een keypair-bestand (bijv. `hello_world-keypair.json`): De publieke sleutel
   van dit keypair wordt gebruikt als programma-ID bij het deployen van het
   programma.
...
volgende dependencies toe aan `Cargo.toml`.
...
Voer de test uit met het `cargo test` commando. Het programmalogboek toont
"Hello, world!".
...
Terwijl de test validator draait, voer je het `solana program deploy` commando
...
<Callout type="info">
  Let op: het cluster op Solana Explorer moet ook localhost zijn. De optie
...
Maak eerst een `examples` directory en een `client.rs` bestand aan.
...
Voeg `solana-client` en `tokio` afhankelijkheden toe.
...
Deploy het programma opnieuw met hetzelfde `solana program deploy` commando.
...
<Callout type="info">
  ...
  uitvoeren, wat een nieuw keypair-bestand zal genereren.
</Callout>
