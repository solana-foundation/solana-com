...
1. `hello_world-keypair.json`: Een keypair-bestand waarvan de publieke sleutel
   wordt gebruikt als je program-ID
...
- **`--with-compute-unit-price`**: Stel de prioriteitsvergoeding in in
  micro-lamports (0,000001 SOL) per rekeneenheid. Controleer de
...
- **`--max-sign-attempts`**: Aantal keren om opnieuw te proberen met een nieuwe
