...
Het gebruik van [geverifieerde builds op Solana](/docs/programs/verified-builds)
waarborgt de integriteit en betrouwbaarheid van uw programma's op het netwerk en
stelt ontwikkelaars in staat uw SDK's rechtstreeks via een Solana Explorer te
vinden. Door gebruik te maken van tools zoals de Solana Verify CLI en Docker
kunt u verifieerbare en veilige builds onderhouden die overeenkomen met uw
broncode. Neem altijd de nodige voorzorgsmaatregelen om consistente omgevingen
te gebruiken en overweeg governance-oplossingen voor veilige upgrades en
implementaties.
