...
root directory van je repository hebben. Als je slechts één programma in je
repository hebt en een `cargo.lock` bestand in je root, kun je direct naar de
volgende stap gaan en je programma bouwen.

Als je programma in een submap staat en je een rust workspace hebt, moet je een
workspace `Cargo.toml` bestand aanmaken in de root directory van je repository.

Je kunt dit `Cargo.toml` voorbeeld als preset gebruiken:
...
ook op dat wanneer het programma in een submap staat, je later deze map moet
...
Om je Solana programma verifieerbaar te bouwen, navigeer naar de directory die
het `Cargo.toml` bestand van je workspace bevat en voer uit:
...
> uitvoeren van `verify-from-repo` de `--mount-path` vlag toevoegen. Dit is het
> pad naar de map die de `Cargo.toml` bevat met de bibliotheeknaam van je
> programma.
...
> tegen de rate limits van de gratis RPC's kunt aanlopen. Dus in plaats van
> `-um` zou je `--url yourRpcUrl` moeten gebruiken voor een betrouwbaardere
> verificatie.

De `--remote` vlag stuurt een buildverzoek naar de OtterSec API, die een externe
build van je programma start. Zodra de build is voltooid, verifieert het systeem
dat de onchain hash van je programma overeenkomt met de hash van het
...
solana-versie die is gespecificeerd in het `Cargo.lock` bestand.
...
Voor de rest van deze multisig-gids gebruiken we een voorbeeldprogramma-ID van
...
voorbeeldprogramma-ID `6XBGfP17P3KQAKoJb2s5M5fR4aFTXzPeuC1af2GYkvhD`):
...
Wanneer je de programma-authority lokaal hebt, wordt je gevraagd om de builddata
onchain te uploaden bij het gebruik van het commando
...
Het `--remote` commando stuurt een build-verzoek naar de OtterSec API, wat een
...
draaien of de programmaverificatie lokaal zelf uitvoeren met behulp van het
`verify-from-repo` commando met de onchain verificatiegegevens die zijn
opgeslagen in een
[PDA](https://explorer.solana.com/address/63XDCHrwZu3mXsw2RUFb4tbNpChuUHx4eA5aJMnHkpQQ/anchor-account)
...
verifier public key bevatten voor gemakkelijke toegang tot de
verificatiegegevens die zijn opgeslagen in de verificatie-PDA. De PDA die alle
informatie bevat die nodig is om een programma te bouwen en te verifiëren, is
afgeleid van het programma-adres en de verifier pubkey. Standaard is dit
dezelfde pubkey die het programma heeft gebouwd en geïmplementeerd. Maar het kan
ook een andere pubkey zijn die kan worden gespecificeerd in de `security.txt`.

De `security.txt` functie stelt ontwikkelaars in staat om contact- en
beveiligingsinformatie direct in hun Solana smart contracts in te sluiten.
...
Voor veel projecten, vooral kleinere of privéprojecten, kan het identificeren
van de ontwikkelaars aan de hand van alleen het contractadres moeilijk en
tijdrovend zijn. Het insluiten van een `security.txt`-bestand in het programma
zorgt ervoor dat beveiligingsonderzoekers gemakkelijk contact kunnen opnemen met
de juiste personen, waardoor exploits mogelijk worden voorkomen en tijdige
bugrapporten worden gegarandeerd.
...
- Verificatie: Verifieer vóór deployment het formaat en de inhoud met behulp van
  de `query-security-txt`-tool, die zowel onchain-programma's als lokale
