...
> Zorg ervoor dat je daadwerkelijk de geverifieerde build deployt en deze niet
> per ongeluk overschrijft met `anchor build` of `cargo build-sbf`, aangezien
> deze hoogstwaarschijnlijk niet in dezelfde hash zullen resulteren en je
> verificatie dus zal mislukken.
...
> Terwijl je de geverifieerde build uitvoert in je programmamap, moet je bij het
> uitvoeren van `verify-from-repo` de `--mount-path` flag toevoegen. Dit is het
> pad naar de map die de `Cargo.toml` bevat met de bibliotheknaam van je
> programma.
...
Nadat je je verificatie-PDA onchain hebt geüpload met `verify-from-repo`, dien
je een remote verificatietaak in bij de
...
De `--uploader` is het adres dat de verificatie-PDA heeft geüpload, meestal de
upgrade-autoriteit van je programma. Als je programma wordt beheerd door een
multisig, verder in het
...
> De verouderde `--remote` vlag op `verify-from-repo` is afgeschaft. Upload
> eerst je PDA en voer daarna `remote submit-job` uit.

Dit dient een taak in bij de OtterSec API. Je kunt de taakstatus controleren
met:
...
Zodra de verificatie succesvol is voltooid, wat even kan duren, kun je je
programma als geverifieerd zien in de
[OtterSec API voor afzonderlijke programma's](https://verify.osec.io/status/PhoeNiXZ8ByJGLkxNfZRnkUfjvmuYqLR89jjFHGqdXY)
en in de
...
wat bijdraagt aan een gezonder Solana-ecosysteem.

</Step>
</Steps>

## Hoe je je programma verifieert wanneer het wordt beheerd door een Multisig zoals Squads

Om de remote verificatie te laten werken, moet je de verificatiegegevens
schrijven naar een PDA ondertekend door de programma-autoriteit. Als je
programma wordt beheerd door een multisig, kun je deze schrijf-PDA-transactie
exporteren en indienen via [Squads Protocol](https://squads.so/protocol) of een
andere multisig-oplossing naar keuze.
...
Dit bouwt een verifieerbare build met behulp van een Docker-container met de
Solana- versie die is opgegeven in het `Cargo.lock` bestand.

</Step>
<Step>

### 2. Implementeer het programma
...
### 3. Commit en verifieer tegen de repository
...
Optioneel: Kijk of je eerst lokaal kunt verifiëren (dit commando gebruikt het
...
Alleen om er zeker van te zijn dat je parameters correct zijn.

</Step>
<Step>

### 4. Draag programmabeheer over aan multisig

Als je de programmarechten nog niet hebt overgedragen aan de multisig, kopieer
dan de multisig-autoriteit. Je hebt deze nodig in de volgende stap.
...
Wanneer je de programmarechten lokaal hebt, word je gevraagd om de buildgegevens
on-chain te uploaden bij gebruik van het commando
`solana-verify verify-from-repo`.

Omdat je dat niet kunt doen wanneer je een multisig gebruikt, moet je de
PDA-transactie handmatig exporteren en de transactie vervolgens via Squads
activeren.
...
transactie wilt voor gebruik in een transactie-inspecteur, kun je
...
Ga naar de Squads-transactiebouwer en importeer de base58-gecodeerde transactie.
Zorg ervoor dat de transactie in de simulatie alleen een aanroep heeft naar het
osec-verificatieprogramma en het computerbudgetprogramma, en niets anders!
...
Dat is het! Je hebt je programma geverifieerd aan de hand van een publieke
repository en een externe taak ingediend bij de OtterSec API. Je zou het nu
moeten kunnen zien in de Solana Explorer en op andere plaatsen.
...
en deze opnieuw via Squads indienen.

Een update uitvoeren op het programma:
...
Draag vervolgens die bufferbevoegdheid over aan de multisig of maak de buffer
direct aan met de bevoegdheid van de multisig.
...
### 9. Nieuwe PDA-transactie exporteren en indienen

Vergeet niet je wijzigingen naar GitHub te pushen. Exporteer de
PDA-upgradetransactie opnieuw:
...
Je kunt een voorbeeldtransactie bekijken
...
## Verifiëren vanuit een Docker-image

Je kunt je programma ook verifiëren aan de hand van een Docker-image door de
...
Deze opdracht laadt de image op die is opgeslagen op
`ellipsislabs/hello_world_verifiable_build:latest`, en verifieert dat de hash
van het uitvoerbare pad in de container overeenkomt met de hash van het onchain-
programma dat aan de opdracht is meegegeven. Omdat de build al naar een image is
geüpload, is er geen volledige herbouw van het uitvoerbare bestand nodig, wat
...
Standaard neemt het `verify-from-repo` commando de laatste commit op de
main-branch. Je kunt ook een specifieke commit opgeven als je verder wilt werken
aan de repository door de parameter `commit-hash` te gebruiken:
`--commit-hash 5b82b86f02afbde330dff3e1847bed2d42069f4e`

Wanneer daarom gevraagd wordt, antwoord je met ja om je verificatie-PDA onchain
te uploaden. Dien vervolgens een externe verificatietaak in via de OtterSec API:
...
> Let op: we moesten de `library-name` opgeven omdat de Squads-repository
> meerdere programma's bevat. We gebruiken de vlag `--bpf` omdat `squads_mpl`
> eerder geverifieerd was met Anchor.
...
- **Verkeerde ondertekenaar**: Bevestig dat uw ondertekenaar de
  ...
  `solana program show YourProgramId` uit te voeren
- **Geen onchain PDA**: Voer `solana-verify verify-from-repo -um` uit en
  **selecteer JA wanneer daarom wordt gevraagd**. Zonder het uploaden van de PDA
  kan de API uw verificatiemetadata niet ophalen.
- **PDA-gegevensmismatch**: Werk uw PDA bij als u uw programma opnieuw heeft
  gedeployed. Uw PDA-gegevens moeten overeenkomen met uw gedeployde programma.
- **Onjuiste commit-hash**: Maak uw PDA aan met de exacte commit-hash die u
  heeft gedeployed
- **Verschillen in buildomgeving**: Gebruik Docker met solana-verify bij het
  aanmaken van uw PDA

## Mijn lokale build-hash komt niet overeen met de onchain-hash. Waarom?

Dit betekent meestal:

- U gebruikt verschillende versies van de Rust/Solana-toolchain
- Uw afhankelijkheden zijn bijgewerkt tussen builds
- U heeft niet gebuild in een Docker-container
- U heeft de verkeerde commit uitgecheckt

Los dit op door te bouwen met `solana-verify build` in Docker met behulp van de
exacte commit die u heeft gedeployed.

## Hoe lang duurt mijn verificatie?

Verwacht de volgende tijdsduren op basis van uw programmagrootte:

- Eenvoudige programma's: 1-5 minuten
- Complexe programma's: 5-15 minuten
- Zeer grote programma's: Tot 30 minuten

Volg uw voortgang via het eindpunt voor de taakstatus.

## Mijn programma is onveranderlijk (geen upgrade-autoriteit). Hoe kan ik het verifiëren?

Als uw programma geen upgrade-autoriteit heeft of onveranderlijk werd gemaakt
voordat u een PDA kon aanmaken, hebben wij een gewhitelistet adres voor deze
situatie. Neem contact met ons op via contact@osec.io, en wij helpen u uw
programma geverifieerd te krijgen.

## Wat is de PDA en waarom is het belangrijk?

Uw PDA (Program Derived Account) maakt vertrouwensloze verificatie mogelijk:

- **On-Chain Opslag**: Sla uw verificatiemetadata (repo-URL, commit-hash,
  buildparameters) op in een PDA die eigendom is van het Otter Verify-programma
  (`verifycLy8mB96wd9wqq3WDXQwM4oU6r42Th37Db9fC`)
- **Cryptografische Koppeling**: Uw PDA wordt afgeleid van uw programmaadres,
  waardoor een onveranderlijke koppeling naar uw verificatiegegevens ontstaat
- **Gedecentraliseerd Vertrouwen**: Iedereen kan uw PDA lezen en uw programma
  onafhankelijk verifiëren

## Waarom moet ik de PDA aanmaken vóór het gebruik van de API?

De API werkt alleen met on-chain PDA's omdat:

- **Vertrouwensloos**: De API weigert willekeurige gegevens — het gebruikt
  alleen wat uw upgrade-autoriteit on-chain heeft opgeslagen
- **Eenvoudiger**: Geef alleen de ondertekenaar en program_id op; de API haalt
  alles overige op uit uw PDA
- **Manipulatiebestendig**: Uw PDA creëert een onveranderlijk record dat
  iedereen onafhankelijk kan verifiëren
- **Eigendomsbewijs**: Uw ondertekenaar moet de upgrade-autoriteit zijn, wat
  cryptografisch bewijst dat u het programma beheert
...
- Wanneer u uw bronrepository bijwerkt
- Maak u geen zorgen over herhaalde verificatie — de API verifieert automatisch
  alle programma's elke 24 uur opnieuw

## Wat gebeurt er wanneer ik mijn programma upgrade?

Volg deze stappen na een upgrade:

1. De API detecteert uw upgrade en markeert uw programma als niet-geverifieerd.
2. Werk uw PDA bij met nieuwe verificatiemetadata:
...
3. Dien een nieuw verificatieverzoek in met uw upgrade-autoriteit
4. De API verifieert uw nieuwe versie aan de hand van de bijgewerkte PDA

**Belangrijk**: Werk uw PDA altijd bij met uw upgrade-autoriteit en gebruik de
nieuwe commit-hash voor het geüpgradede programma.

## Kan ik de verificatieresultaten vertrouwen?

Ja - het systeem is ontworpen om trustless en onafhankelijk verifieerbaar te
zijn:

**Wat het betrouwbaar maakt:**

- **On-Chain PDA**: Je verificatiemetadata leeft onchain, niet beheerd door een
  centrale autoriteit
- **Bewijs van upgrade-autoriteit**: Alleen de upgrade-autoriteit van jouw
  programma kan de PDA aanmaken/bijwerken
- **Onafhankelijke verificatie**: Iedereen kan verifiëren door jouw PDA te lezen
  en `solana-verify` lokaal uit te voeren
- **Doorlopende herverificatie**: De API herverifieert automatisch alle
  programma's elke 24 uur

**Begrijp deze beperkingen:**

- Verificatie bevestigt dat de broncode overeenkomt met de implementatie -
  **NIET** dat jouw code veilig is
- Bekijk altijd de code voordat je interactie hebt met programma's
- Geverifieerd ≠ geauditeerd of veilig
- Controleer de repository en de commit in de PDA om te bevestigen dat deze
  afkomstig is van een vertrouwde bron

## Hoe kan ik een programma onafhankelijk verifiëren?

Verifieer elk programma zelf door de onchain PDA te lezen en verificatie lokaal
uit te voeren:
...
1. De PDA-metadata is authentiek (onchain opgeslagen)
2. De broncode in de repository van de PDA komt overeen met het geïmplementeerde
   programma
3. Je hoeft de API niet te vertrouwen - verifieer alles zelf onchain

## Kan iemand anders mijn programma verifiëren zonder toestemming?

Ja, dat is de reden waarom we vereisen dat de ondertekenaar de
upgrade-autoriteit is. We beschouwen de verificatie alleen als geldig als de
ondertekenaar de upgrade-autoriteit is.
...
- Cargo (Rust-pakketbeheerder)
- Solana Verify CLI: `cargo install solana-verify`
- Een openbare Git-repository met jouw broncode

## Kan ik privérepositories verifiëren?

Nee - verificatie vereist openbare broncode:

- Uw PDA slaat een openbare repository-URL op die iedereen kan openen
- Betrouwbare verificatie zonder vertrouwde partij vereist toegang tot openbare
  code
- Gebruikers moeten uw broncode kunnen lezen om te begrijpen wat uw programma
  doet
- Het hele doel is om gebruikers in staat te stellen onafhankelijk te verifiëren
  dat de broncode overeenkomt met de implementatie

Privérepositories doorbreken het kernvertrouwensmodel van het
verificatiesysteem.

## Hoe verifieer ik een programma dat wordt beheerd door Squads Multisig?

Volg deze stappen voor programma's die worden beheerd via multisig:
...
**Belangrijk**: Verifieer altijd lokaal (stap 2) om te bevestigen dat de
build-hash overeenkomt voordat u de PDA-transactie exporteert.

# Conclusie

Het gebruik van [geverifieerde builds op Solana](/docs/programs/verified-builds) garandeert de
integriteit en betrouwbaarheid van uw programma's op het netwerk en stelt
ontwikkelaars in staat uw SDK's rechtstreeks via een Solana Explorer te vinden. Door
gebruik te maken van tools zoals de Solana Verify CLI en Docker, kunt u verifieerbare en
veilige builds onderhouden die overeenkomen met uw broncode. Neem altijd de nodige
voorzorgsmaatregelen om consistente omgevingen te gebruiken, en overweeg governance-oplossingen
voor veilige upgrades en implementaties.

## Beveiliging + Disclaimer

Hoewel geverifieerde builds een krachtig hulpmiddel zijn voor het waarborgen van
de integriteit van uw Solana-programma's, is de standaardinstelling niet
volledig zonder vertrouwde partij. De Docker-images worden gebouwd en gehost
door de Solana Foundation.

Houd er rekening mee dat u uw project bouwt in een gedownload Docker-image en
dat uw volledige configuratie wordt gekopieerd naar dat Docker-image voor het
bouwen, inclusief mogelijk gevoelige informatie.

Als u een volledig vertrouwensloze opstelling wilt hebben, kunt u de
Docker-images zelf bouwen en hosten op uw eigen infrastructuur. Op deze manier
kunt u er zeker van zijn dat de Docker-images niet zijn gemanipuleerd. U kunt de
scripts voor het aanmaken van uw eigen Docker-images vinden in de
[Verified builds-repository](https://github.com/Ellipsis-Labs/solana-verifiable-build)
en u kunt deze forken en de GitHub Actions zelf uitvoeren, of valideren dat ze
correct zijn.

Bovendien vertrouwt u bij de externe verificatie in zekere mate op de OtterSec
API en de
...
Als u een volledig vertrouwensloze opzet wilt, kunt u de
[Verify API](https://github.com/otter-sec/solana-verified-programs-api) zelf
uitvoeren of de programmaverificatie lokaal uitvoeren met het `verify-from-repo`
commando, waarbij u gebruikmaakt van de on-chain verificatiegegevens die zijn
opgeslagen in een
[PDA](https://explorer.solana.com/address/63XDCHrwZu3mXsw2RUFb4tbNpChuUHx4eA5aJMnHkpQQ/anchor-account)
die is afgeleid van de deploy-autoriteit van het programma en het
[verificatieprogramma](https://explorer.solana.com/address/verifycLy8mB96wd9wqq3WDXQwM4oU6r42Th37Db9fC).

Het verificatieprogramma is ingezet door het [OtterSec-team](https://osec.io/)
en is nog niet bevroren, waardoor het op elk moment kan worden bijgewerkt.

De Solana Foundation, OtterSec en het Ellipsis Labs-team zijn niet
verantwoordelijk voor eventuele verliezen of schade die kunnen ontstaan door het
gebruik van de geverifieerde builds-pipeline.

# Security.txt voor Solana-programma's

Naast geverifieerde builds kunt u ook een `security.txt` bestand aan uw
programma toevoegen. In de toekomst, zodra geïmplementeerd, zal de
`security.txt` de verifier publieke sleutel bevatten voor eenvoudige toegang tot
de verificatiegegevens die zijn opgeslagen in de verificatie-PDA. De PDA met
alle informatie die nodig is om een programma te bouwen en te verifiëren, is
afgeleid van het adres van het programma en de verifier pubkey. Standaard is dit
dezelfde pubkey waarmee het programma is gebouwd en ingezet. Maar het kan ook
een andere pubkey zijn die kan worden opgegeven in de `security.txt`.

De `security.txt` functie stelt ontwikkelaars in staat om contact- en
beveiligingsinformatie direct in hun Solana smart contracts op te nemen.
...
Voor veel projecten, met name kleinere of privéprojecten, kan het lastig en
tijdrovend zijn om de ontwikkelaars te identificeren aan de hand van alleen het
contractadres. Door een `security.txt` bestand in het programma in te sluiten,
kunnen beveiligingsonderzoekers eenvoudig contact opnemen met de juiste
personen, wat exploits kan voorkomen en tijdige bugmeldingen garandeert.

## Hoe security.txt te implementeren

Om een `security.txt` aan uw Solana-programma toe te voegen, volgt u de volgende
stappen:

Voeg de `solana-security-txt` afhankelijkheid toe aan uw `Cargo.toml`:
...
Gebruik de `security_txt!` macro in uw contract om uw beveiligingsinformatie te
definiëren. U kunt contactgegevens, project-URL's en zelfs een
...
Zodra de `security.txt` informatie in uw programma is ingesloten, kan deze
eenvoudig worden opgevraagd via tools zoals de Solana Explorer, zodat uw
...
- Gebruik links: Voor informatie die waarschijnlijk zal veranderen (bijv.
  contactgegevens), wordt aanbevolen te verwijzen naar een webpagina in plaats
  van deze hard te coderen in het contract. Dit voorkomt de noodzaak van
  frequente programma-upgrades.

- Verificatie: Verifieer vóór de implementatie het formaat en de inhoud met
  behulp van de `query-security-txt` tool, die zowel onchain-programma's als
  lokale binaire bestanden kan valideren:

```bash
query-security-txt target/bpfel-unknown-unknown/release/my_contract.so
```

Door beveiligingscontactinformatie rechtstreeks in uw contract in te sluiten,
maakt u het voor onderzoekers gemakkelijker om u te bereiken, wat de beveiliging
en communicatie binnen het Solana-ecosysteem bevordert.
...
U kunt de verificatiestatus controleren en geverifieerde programma's bekijken op
