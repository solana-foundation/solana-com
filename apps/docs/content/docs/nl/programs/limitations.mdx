...
libcore en liballoc, evenals vele third-party crates.
...
- Beperkte toegang tot:
  - `std::hash`
  - `std::os`
- Bincode is extreem rekenintensief qua cycles en call depth en moet worden
  vermeden
- String formatting moet worden vermeden omdat het ook rekenintensief is.
- Geen ondersteuning voor `println!`, `print!`, gebruik in plaats daarvan de
  ...
  macro.
- De runtime handhaaft een limiet op het aantal instructies dat een programma
  kan uitvoeren tijdens de verwerking van één instructie. Zie
...
## Call stack depth - `CallDepthExceeded` foutmelding
...
Wanneer een programma de toegestane call stack depth limiet overschrijdt,
ontvangt het de `CallDepthExceeded` foutmelding.

## CPI call depth - `CallDepth` foutmelding

Cross-program invocations stellen programma's in staat om andere programma's
direct aan te roepen, maar de diepte is momenteel beperkt tot `4`.

Wanneer een programma de toegestane
[cross-program invocation call depth](/docs/core/cpi) overschrijdt, ontvangt het
een `CallDepth` foutmelding
