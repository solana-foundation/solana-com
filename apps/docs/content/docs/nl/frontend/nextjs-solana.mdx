...
description:
  Stel Solana wallet-integratie in Next.js in met @solana/kit en @solana/react
  om SOL-transfers te versturen.
---

Stel een minimale Solana wallet-integratie in Next.js (App Router) in met
`@solana/kit`, de kit-plugins en `@solana/react`. Je maakt een dropdown voor het
verbinden van een wallet en een SOL-transfercomponent.
...
Accepteer alle standaardinstellingen wanneer daarom wordt gevraagd (de starter
bevat Tailwind, dat in deze tutorial wordt gebruikt voor eenvoudige
utility-stijlen).

<Callout type="info" title="Twee aanpassingen aan het basisproject">
De standaard `create-next-app`-uitvoer vereist twee kleine wijzigingen voor deze tutorial:

- Stel in `tsconfig.json` de waarde `"target": "ES2020"` in (of later). De
  transferbedragen gebruiken `bigint`-literals zoals `1_000_000_000n`, waarvoor
  ES2020 vereist is.
- Verwijder in `app/globals.css` het gegenereerde
  `@media (prefers-color-scheme: dark)`-blok en de
  `body`-achtergrondregelinstelling. Deze UI is alleen licht; de donkere
  standaardwaarden overschrijven anders de `bg-white`- hulpprogrammaklasse en
  maken de pagina onleesbaar in de donkere modus.
...
Deze tutorial is gericht op Kit 7+, de `@solana/kit-plugin-*`-pakketten op 0.13+
en `@solana/react` 7+. Als je pakketbeheerder peer-waarschuwingen toont van een
`@solana-program/*`-pakket dat een oudere Kit-peer-range declareert, overschrijf
die dan — de pakketten werken met Kit 7.
...
`ClientProvider`. De `walletSigner`-plugin maakt de verbonden wallet de betaler
van de kosten en de identiteit; exporteer het clienttype zodat componenten
...
die te importeren en gebruiken:
...
Dit omsluit de volledige app met de Solana-context, zodat alle onderliggende
componenten toegang hebben tot de client- en wallet-hooks.

## !!steps 3. Wallet Connect Button

Dropdown om Wallet Standard-wallets te verbinden/verbreken. `useWallets` geeft
de in de browser gevonden wallets terug; `useConnect` / `useDisconnect` geven
een `ActionResult` terug — dit component gebruikt de velden `dispatch`,
`isRunning` en `error`.
...
![Wallet dropdown-opties](/assets/docs/frontend/02-wallet-dropdown-options.webp)

![Wallet dropdown verbonden](/assets/docs/frontend/03-wallet-dropdown-connected.webp)

## !!steps 4. SOL Transfer

Bouw een overdrachtsinstruction met `@solana-program/system` en verstuur deze
via `client.sendTransaction`. De verbonden wallet ondertekent en betaalt.
...
Wallets worden gevonden via Wallet Standard-detectie; zodra verbonden, gebruikt
de SOL-overdracht de verbonden ondertekenaar als betaler van de
transactiekosten.

## !!steps 5. Pagina

`app/page.tsx` rendert de wallet connect- en SOL-overdrachtscomponenten:
...
Open http://localhost:3000, verbind een Devnet-wallet en verstuur een
SOL-overdracht.
