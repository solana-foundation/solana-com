...
description:
  Testomgevingen instellen en instructies bouwen met AnchorContext en Program
---

## Clienttypen genereren met declare_program!

Anchor 1.0 introduceert `declare_program!`, dat de IDL van je programma inleest
tijdens het compileren en type-veilige `client::accounts::*`- en
`client::args::*`-modules genereerd. Dit is de aanbevolen manier om instructies
in tests te bouwen.
...
<Callout type="info">
  Voer `anchor build` eerst uit om `target/idl/my_program.json` te genereren.
  `declare_program!` leest dit bestand tijdens het compileren. De macronaam moet
  overeenkomen met de crate-naam van je programma (snake_case).
</Callout>

## De testomgeving instellen

De `AnchorLiteSVM`-builder biedt een fluent API voor het configureren van je
testomgeving.
...
### Verkorte notatie voor één programma
...
<Callout type="info">
  Als je geen betaler opgeeft, maakt `AnchorLiteSVM` automatisch één aan en
  financiert deze voor je.
</Callout>

## De AnchorContext

`AnchorContext` is de belangrijkste interface voor interactie met je
testomgeving.
...
### Gefinancierde accounts aanmaken
...
## Instructies Bouwen met Program

De `Program` struct biedt een fluent API voor het bouwen van instructies die
Anchor-client weerspiegelt. Gebruik de typen die gegenereerd zijn door
`declare_program!` voor type- veiligheid.

### Basisinstructies Bouwen
...
<Callout type="info">
  De `.accounts()` methode accepteert elk type dat `ToAccountMetas`
  implementeert, wat alle Anchor account structs automatisch doen. Op dezelfde
  manier accepteert `.args()` elk type dat `InstructionData` implementeert.
</Callout>
...
### Meerdere Instructies Uitvoeren
...
### send_and_confirm_transaction Gebruiken
...
Hier is een uitgebreid voorbeeld met het escrow-patroon met `declare_program!`:
