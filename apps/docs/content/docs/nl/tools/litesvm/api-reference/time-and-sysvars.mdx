---
title: Tijd & Sysvars
description: Methoden voor het manipuleren van tijd en systeemvariabelen
---
...
Spring naar een specifiek slot. Alleen voorwaarts reizen in de tijd wordt
ondersteund.
...
<Callout type="info">
  Gebruik `warp_to_slot()` voor de meeste tijdmanipulatie. Gebruik handmatige
  klokwijziging alleen wanneer u nauwkeurige controle over meerdere velden nodig
  heeft.
</Callout>
...
## Tijdgebaseerde testpatronen

### Test slot-gebaseerde functies
...
### Test tijdstempelgebaseerde functies
...
- Huurperiodes
- Epoch-gebaseerde beloningen

## Belangrijke opmerkingen

1. **Alleen voorwaarts** - Je kunt alleen vooruit in de tijd springen, niet
   achteruit
2. **Slot naar tijd** - Slots komen ruwweg overeen met ~0,4 seconden op het
   Solana-mainnet
3. **Clock sysvar** - Wordt altijd bijgewerkt bij gebruik van `warp_to_slot()`
4. **Geen automatische updates** - In tegenstelling tot echte validators gaat de
   tijd niet automatisch vooruit tussen transacties
