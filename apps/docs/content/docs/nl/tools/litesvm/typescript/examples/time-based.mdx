...
Demonstreert het gebruik van `warpToSlot` om tijdafhankelijke programmalogica te testen.
...
Je kunt het epoch-schema uitlezen om de slot/epoch-verhouding op het
cluster te begrijpen:
...
<Callout type="warn">
  ...
  `epoch` of `unixTimestamp` bij. Gebruik `setClock` voor volledige controle over de klok
  velden.
</Callout>

## Handmatige Klokmanipulatie

Wanneer je programma `epoch` of `unix_timestamp` uit de Clock sysvar uitleest, gebruik
`setClock` om die velden direct in te stellen:
...
Dit weerspiegelt het Rust `set_sysvar(&clock)`-patroon. Gebruik `warpToSlot` voor eenvoudige
slot-vooruitgang, `setClock` wanneer je epoch- of tijdstempelcontrole nodig hebt.

## Gebruiksscenario's

Tijdmanipulatie is handig voor het testen van:

| Scenario                | Aanpak                           |
| ----------------------- | ---------------------------------- |
| Token vesting           | Warp voorbij vesting cliff/mijlpalen |
| Veilingafsluitingen         | Warp voorbij eind-slot van veiling         |
| Stakingrewards         | Gebruik `setClock` om epoch in te stellen        |
| Tijdvergrendelde opnames | Warp voorbij ontgrendelingsslot              |
| Snelheidsbeperking           | Warp tussen toegestane intervallen     |
| Verlopende orders         | Warp voorbij ordervervaldatum         |

<Callout type="info">
  Houd er rekening mee dat `warpToSlot` alleen het slot wijzigt. De unix-tijdstempel wordt mogelijk niet
  proportioneel bijgewerkt, afhankelijk van de SVM-implementatie.
</Callout>

## Kernpunten

1. **Sysvars inschakelen**: Roep `withSysvars()` aan voordat je klokmethoden gebruikt
2. **Vooruitspoelen**: Gebruik `warpToSlot(slot)` om het slot vooruit te zetten
3. **Volledige controle**: Gebruik `setClock` wanneer je `epoch`,
   `unixTimestamp` of andere klokvelden wilt instellen
4. **Klok uitlezen**: Gebruik `getClock()` om het huidige slot, epoch en tijdstempel uit te lezen
