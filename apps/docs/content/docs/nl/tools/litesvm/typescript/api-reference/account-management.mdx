...
Methoden voor het beheren van accounts, saldi en het instellen van de
accountstatus.
...
Een account voorzien van lamports.
...
Accountstatus rechtstreeks instellen zonder een transactie.
...
| `data`           | `Uint8Array` | Databytes van account         |
| `executable`     | `boolean`    | Of het account uitvoerbaar is |
| `lamports`       | `Lamports`   | SOL-saldo                     |
| `programAddress` | `Address`    | Eigenaarprogramma             |
...
SOL-saldo van account opvragen. Geeft `null` terug als het account niet bestaat.
...
Het minimum aantal benodigde lamports voor rent-vrijstelling berekenen.
...
## Batchaccount instellen

Voor het testen kunt u meerdere accounts efficiënt instellen:
...
<Callout type="info">
  Het gebruik van `setAccount` is sneller dan `airdrop` voor het bulksgewijs
  instellen van accounts, omdat er geen transacties worden uitgevoerd.
</Callout>
