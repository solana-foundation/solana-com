...
description: Stel testaccounts en -status in met LiteSVM
---

# Voorbeeld van accountinstellingen

Demonstreert het rechtstreeks instellen van testaccountstatus zonder
transacties.
...
Stel een program-owned data account in:
...
<Callout type="info">
  Het gebruik van `setAccount` is sneller dan het uitvoeren van transacties voor
  het bulksgewijs instellen van accounts.
</Callout>

## Belangrijkste punten

1. **Directe status**: `setAccount` stelt de accountstatus in zonder transacties
2. **Vrijstelling van huur**: Gebruik `minimumBalanceForRentExemption` om de
   vereiste lamports te berekenen
3. **Accountstructuur**: Geef alle vereiste velden op: address, data,
   executable, lamports, programAddress, space
4. **Verificatie**: Gebruik `getAccount` of `client.rpc.getAccountInfo` ter
   verificatie
