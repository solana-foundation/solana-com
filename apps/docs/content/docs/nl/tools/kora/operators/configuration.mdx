...
> _Opmerking: Wanneer `fallback_if_unavailable` true is, staat het systeem transacties
> toe om door te gaan als Redis tijdelijk niet beschikbaar is, waardoor serviceonderbrekingen worden voorkomen.
> Het instellen van `max_transactions` op 0 staat onbeperkte transacties toe._
...
| `get_payer_signer`          | Geef de volgende betaler en het betalingsadres terug        | ✅       | boolean |
...
| ---------------------------- | ------------------------------------------------ |
| `confidential_transfer_mint` | Configuratie van Vertrouwelijke Saldi voor de mint |
...
| Extensienaam                  | Beschrijving                                 |
| ------------------------------- | ------------------------------------------- |
| `confidential_transfer_account` | Status van Vertrouwelijke Saldi voor het account |
