---
title: Transactieverzoeken
description:
  Bouw interactieve betalingsstromen met server-side transactiecreatie
---

Transactieverzoeken maken interactieve betalingsstromen mogelijk door wallets in
staat te stellen om te communiceren met uw server om elke Solana-transactie
samen te stellen. Dit ontsluit geavanceerde gebruikssituaties zoals NFT-minting,
dynamische prijsstelling, meerstaps DeFi- transacties en aangepaste
bedrijfslogica.

## Overzicht

Transactieverzoeken volgen dit URL-formaat:

```
solana:<server-endpoint-url>
```

De wallet voert twee HTTP-verzoeken uit naar uw endpoint:

1. **GET-verzoek** - Betalingsinformatie ophalen (label, pictogram)
2. **POST-verzoek** - De te ondertekenen transactie ophalen
...
### 2. API-endpoint aanmaken

Stel een API-endpoint in dat zowel GET- als POST-verzoeken verwerkt:
...
### 3. Betalings-URL aanmaken

Genereer de transactieverzoek-URL:
...
## Wallet-zijde: Transactieverzoeken ophalen

Gebruik aan de wallet-zijde `fetchTransaction` om een transactie op te halen en
te ondertekenen vanaf een transactieverzoek-URL:
...
Maak een transactie voor USDC-betaling:
...
### Dynamische prijsstelling met kortingen

Implementeer dynamische prijzen op basis van klantgegevens:
...
### Integratie van Orderbeheer

Integreer met uw bestaande ordersysteem:
...
Valideer voltooide transacties:
...
- Valideer alle invoerparameters
- Bereken bedragen uitsluitend server-side
- Gebruik HTTPS voor alle endpoints
- Implementeer rate limiting
- Sla gevoelige gegevens veilig op

### Prestaties

- Cache veelgebruikte gegevens
- Gebruik connection pooling
- Implementeer request timeouts
- Overweeg webhooks te gebruiken voor betalingsbevestiging

### Gebruikerservaring

- Geef duidelijke foutmeldingen
- Toon de voortgang van transacties
- Handel netwerkproblemen netjes af
- Ondersteun het opnieuw proberen van transacties
