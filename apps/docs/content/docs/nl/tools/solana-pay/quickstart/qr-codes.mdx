---
title: QR-code integratie
description: Genereer en pas QR-codes aan voor mobiele wallet-betalingen
---

QR-codes zijn de meest gebruikelijke manier om de kloof tussen webapplicaties en
mobiele wallets te overbruggen. Solana Pay bevat een ingebouwde `createQR`
functie aangedreven door `@solana/qr-code-styling` — er zijn geen aanvullende
QR-code pakketten nodig.

## Basis QR-code genereren

### Een QR-code genereren en weergeven
...
De `createQR` functie accepteert optionele parameters voor grootte,
achtergrondkleur en voorgrondkleur:
...
### QR-opties ophalen voor geavanceerd gebruik

Gebruik `createQROptions` om het ruwe configuratieobject te verkrijgen voor meer
controle:
...
## De Merchant Client gebruiken

De merchant client maakt ook QR-code methoden beschikbaar:
...
### Betalings-QR met statusupdates
...
### Weergave geoptimaliseerd voor mobiel

Op mobiele apparaten kunt u overwegen om naast de QR-code ook een directe link
aan te bieden:
