---
title: TypeScript
description: Installeer en gebruik @solana/keychain in TypeScript-applicaties
---

## Installatie

Installeer het overkoepelende pakket of individuele signers naar behoefte:
...
### Uniforme Factory (Aanbevolen voor apps die meerdere signers willen ondersteunen)

Gebruik `createKeychainSigner` met een gediscrimineerde configuratie om elke
backend te creëren:
...
Of installeer een individueel signer-pakket voor een kleinere
afhankelijkheidsfootprint:
...
### Gebruiken met Transaction Messages

Alle signers implementeren de `SolanaSigner` interface, die compatibel is met
`@solana/kit` en `@solana/signers`:
...
### Gebruiken met een Kit Client (keychainSigner plugin)

`@solana/keychain-kit-plugin` installeert een keychain signer rechtstreeks op
een [Kit client](https://github.com/anza-xyz/kit), en koppelt deze als de
`payer` en `identity`, zodat je de handmatige `pipe`-instelling zoals hierboven
beschreven overslaat:
...
De plugin accepteert dezelfde backend-tagged configuratie als
`createKeychainSigner`, en alleen de backend waarnaar het dispatcht wordt
meegeleverd (backend-pakketten worden geladen via dynamische `import()`).

Gebruik `keychainPayer` of `keychainIdentity` om slechts één rol in te stellen
en backends te combineren op een enkele client:
...
Heb je al een `SolanaSigner`? Elke keychain signer is een geldige Kit
`TransactionSigner`, dus je kunt een bestaande installeren met
[`@solana/kit-plugin-signer`](https://github.com/anza-xyz/kit-plugins/tree/main/packages/kit-plugin-signer)
in plaats daarvan (`pnpm add @solana/kit-plugin-signer`):
...
Fordefi heeft twee ondertekeningsmodi, bepaald door of je `chain` meegeeft. API-verzoeken worden geverifieerd met een ECDSA P-256-verzoekhandtekening — geef de ondertekeningssleutel mee als `privateKeyPem`, of bewaar deze in een KMS/HSM door een `requestSigner` mee te geven.

**Native Solana-modus** (aanbevolen voor on-chain gebruik): stel `chain` in, en Fordefi ondertekent en verstuurt de transactie zelf. Omdat Fordefi de blockhash of kosten vóór ondertekening kan vervangen, is de ondertekenaar een Kit `TransactionSendingSigner` — gebruik deze met `signAndSendTransactionMessageWithSigners`, niet de gedeeltelijke-ondertekenaarsstroom zoals hierboven weergegeven.
...
**Black box-modus**: laat `chain` weg. Fordefi ondertekent ruwe bytes en verstuurt niet; je assembleert en verzendt de transactie zelf. Gebruik dit met een Fordefi black box-vault.
...
De `SolanaSigner` interface breidt `@solana/signers` types uit voor volledige
compatibiliteit:
...
## Aangepaste Signers Bouwen

Implementeer de `SolanaSigner` interface om aangepaste signers te maken:
...
Zie de
[gids voor het toevoegen van signers](/docs/tools/keychain/adding-signers) om
aanvullende sleutelbeheerservices te integreren.
