...
| ------------ | ------------------------------- |
| `memory`     | Lokale keypair ondertekening (standaard) |
...
Voor Fordefi MPC-bewaring ondertekening. API-verzoeken worden geverifieerd met een ECDSA
P-256 verzoekhandtekening afgeleid uit `private_key_pem`; om die sleutel in een
KMS/HSM te bewaren, implementeer `FordefiRequestSigner` en gebruik
`Signer::from_fordefi_with_signer`.

Fordefi heeft twee ondertekeningsmodi. Stel `chain` in voor de **native Solana-modus**, waarbij
Fordefi de blockhash of kosten kan vervangen en de transactie zelf uitzendt.
Laat `chain` op `None` staan voor de **black box-modus**, waarbij Fordefi ruwe bytes ondertekent
en u de transactie zelf uitzendt.
