...
description:
  Hoe Private Channels een Sparse Merkle Tree gebruikt om dubbele uitbetalingen te voorkomen
---
...
<Callout type="info">
  ...
  huidige escrow-programma en wordt herzien zodra die wijziging is doorgevoerd.
</Callout>
...
- Boomhoogte: `16`
- Maximaal aantal bladeren: `65.536` (2^16)
- Hashfunctie: SHA-256
- Waarde leeg blad: `[0u8; 32]` (32 nulbytes)
...
- De uitbetalingsroot wordt teruggezet naar de lege boom
...
3. Alleen als beide controles slagen, slaat het programma `new_withdrawal_root` op als de
   nieuwe `Instance.withdrawal_transactions_root` en geeft het de tokens vrij
