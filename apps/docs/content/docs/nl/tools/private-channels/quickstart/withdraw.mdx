...
description:
  Tokens van een Private Channels-instantie terugtrekken naar uw Solana-wallet
---

## Overzicht

Opnemen vanuit Private Channels is een proces in twee stappen. Eerst verbrandt de gebruiker
...
fondsen vrij te geven. Het SMT-bewijs zorgt ervoor dat elke opname-nonce slechts één keer kan worden gebruikt,
waardoor dubbele uitgaven worden voorkomen.
...
Gebruik de `Async`-variant, die de `tokenAccount` PDA automatisch afleidt en de
...
- Dien deze transactie in bij de gateway (`http://localhost:8899`), niet bij een
  openbare Solana RPC
- Als `destination` is opgegeven, gaan vrijgegeven fondsen naar die wallet in plaats van naar die van de
  ondertekenaar

## Wat u kunt verwachten

Er zijn geen verdere acties vereist na het aanroepen van `WithdrawFunds`. De operator-
services verwerken de afwikkeling automatisch:

1. `indexer-private-channel` peilt het kanaal elke 1 seconde op verbrandingsgebeurtenissen
   en schrijft een record voor een openstaande opname wanneer die van u wordt gedetecteerd
2. `operator-private-channel` peilt de database elke 1 seconde op openstaande
   records en dient `ReleaseFunds` in bij het Escrow Program op Solana devnet
   met het vereiste SMT-uitsluitingsbewijs; het peilt voor devnet-bevestiging tot
   5 keer met intervallen van 400 ms alvorens opnieuw te proberen
...
vrijgeven zonder te bewijzen dat de opname-nonce nog niet eerder is gebruikt. Deze stap
wordt automatisch afgehandeld door `operator-private-channel`; u hoeft dit zelf niet aan te roepen.
...
- `sibling_proofs`: 512 bytes (16 x 32-byte broer/zus-hashes voor het boomsbewijs)

## Verifiëren

Controleer uw devnet-tokensaldo nadat de operatoraanroep is bevestigd:
...
de escrow, een off-chain overdracht verzonden via de gateway en fondsen teruggetrokken
...
<Cards>
  ...
  >
    Begrijp deelnemers en de volledige stroom van storting tot opname in detail.
  </Card>
  <Card
  ...
  <Card
    title="Referentie voor het vrijgeven van fondsen"
