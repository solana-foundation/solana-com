...
La fee di priorità è una fee opzionale che aumenta la priorità di schedulazione
di una transazione. Un'unità di calcolo (CU) è un'unità di computazione misurata dal runtime
durante l'esecuzione della transazione. Ogni operazione (aritmetica, accesso alla memoria,
syscall) ha un costo fisso in CU. È possibile impostare la fee di priorità includendo
istruzioni di budget di calcolo nella transazione. (Consultare la
[guida su come utilizzare le Priority Fee](/docs/core/fees) per i dettagli.)
...
<Callout type="info" title="Le transazioni v1 impostano la commissione direttamente">
  Questa formula prezzo-per-limite si applica alle transazioni legacy e v0. Nel
  prossimo [formato
  v1](/docs/core/transactions/versioned-transactions#v1-message-format), la
  commissione di priorità è un totale assoluto in lamport impostato direttamente nella
  configurazione del messaggio — nessun prezzo per CU, nessuna moltiplicazione, nessun arrotondamento.
</Callout>
