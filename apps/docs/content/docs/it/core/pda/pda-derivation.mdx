...
Il bump seed è un singolo byte (u8) aggiunto ai seed opzionali durante la
derivazione.
[`find_program_address`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L800)
ricerca da 255 fino a 1, chiamando _rs`create_program_address`_ con ogni valore
finché il risultato non cade fuori dalla curva Ed25519. Il primo valore che ha
successo è il **bump canonico**.
...
La derivazione di un PDA calcola solo un indirizzo. Non crea un account onchain
a quell'indirizzo. L'account deve essere creato esplicitamente tramite
un'istruzione separata (tipicamente _rs`create_account`_ via CPI).
...
### Iterazione di Tutti i Bump Possibili
...
<Callout type="warn">
  Questo è diverso dall'attuale comportamento di ricerca canonico documentato
  per `sol_try_find_program_address`, che itera da 255 fino a 1.
</Callout>
...
In questo esempio, il bump 255 produce un indirizzo sulla curva e fallisce. Il
