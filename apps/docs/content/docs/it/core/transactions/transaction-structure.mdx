...
description:
  Anatomia di una transazione Solana incluse firme, formato del messaggio, intestazione, indirizzi degli account, blockhash, istruzioni compilate, codifica binaria e limite di dimensione.
...
<Callout type="info" title="Riepilogo">
  Una transazione contiene firme e un messaggio. Il messaggio include un'intestazione,
  indirizzi di account, un blockhash recente e istruzioni compilate. Dimensione massima
  serializzata: 1.232 byte (4.096 byte per il prossimo formato v1).
</Callout>
...
<Callout type="info" title="Le transazioni v1 portano il limite a 4.096 byte">
  Questa pagina descrive il formato di transazione legacy, seguito anche dal formato v0
  ad eccezione del prefisso di versione e delle tabelle di ricerca degli indirizzi. Il prossimo
  [formato v1](/docs/core/transactions/versioned-transactions#v1-message-format)
  porta il limite di dimensione a 4.096 byte e utilizza un layout wire diverso.
</Callout>
...
<Callout type="info" title="Prefissi dei messaggi legacy e versionati">
  In un messaggio di transazione legacy, il primo byte del messaggio è
  `num_required_signatures`, seguito dagli altri due byte di `MessageHeader`. In
  un messaggio di transazione versionata, il primo byte è invece un prefisso di versione;
  il `MessageHeader` di tre byte inizia immediatamente dopo tale prefisso. Consulta
  [le transazioni versionate](/docs/core/transactions/versioned-transactions) per
  i layout completi dei messaggi v0 e v1.
</Callout>
...
<Callout>
  ...
  contatori nell'[`header`](#header) del messaggio per determinare i permessi di
...
sono preceduti da una codifica della lunghezza compact-u16. Questo formato
utilizza 1 byte per i valori da 0 a 127 e 2-3 byte per i valori più grandi.
...
dell'account mittente indicano che deve firmare la transazione. Ciò consente al
System Program di detrarre lamport. Sia l'account mittente che quello
...
![Diagramma di trasferimento SOL](/assets/docs/core/transactions/sol-transfer.svg)
...
Un trasferimento del System Program aggiunge lamport a **qualsiasi** account.
Non esiste alcun controllo a livello di protocollo che verifichi se il
destinatario può spostare nuovamente il SOL. I lamport possono essere spostati
...
[PDA](/docs/core/pda) che non controlli **rischia la perdita permanente dei
fondi** — solo un'autorità specificata dal programma proprietario può
restituirli. Il SOL inviato a un
...
[sulla curva](/docs/payments/send-payments/verify-address#on-curve-addresses)
non finanziato); rifiuta mint, token account, programmi e PDA che non controlli.
...
istruzioni interne (CPI) come blob codificati. Per risolverle in indirizzi e
navigare l'intero albero delle istruzioni, consulta
