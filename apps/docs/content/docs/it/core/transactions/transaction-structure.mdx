...
Ogni firma è prodotta da una chiave privata. La posizione in cui risiede tale
chiave — un keypair locale, un HSM o KMS cloud, oppure un servizio di wallet
gestito — è una decisione di progettazione per l'ambiente di produzione. Vedere
...
[commissione base e la commissione di prioritizzazione](/docs/core/fees/fee-structure#base-fee)
della transazione. Questa prima firma funge anche da **ID transazione**,
utilizzato per cercare la transazione sulla rete. L'ID transazione è comunemente
denominato **transaction signature**.
...
- Deve detenere abbastanza lamport per coprire
...
2. Firmatario + Solo lettura
3. Non firmatario + Scrivibile
4. Non firmatario + Solo lettura
...
Il campo `recent_blockhash` è un hash di 32 byte che svolge due funzioni:
...
transazione viene ricevuta, viene rifiutata con _rs`BlockhashNotFound`_, a meno
che non si tratti di una valida
...
utilizza 1 byte per valori da 0 a 127 e 2-3 byte per valori più grandi.
...
| `data`             | `data_len` byte         | instruction data opaco        |
...
dell'account del mittente indicano che deve firmare la transazione. Ciò consente
al System Program di detrarre lamport. Sia l'account del mittente che quello del
destinatario devono essere scrivibili, affinché il loro saldo in lamport possa
variare. Per eseguire questa istruzione, il portafoglio del mittente invia la
...
destinatario può spostare nuovamente i SOL. I lamport possono essere spostati
...
I trasferimenti di **token** SPL sono in parte auto-protetti: il Token Program
rifiuta un trasferimento i cui account non corrispondono al mint atteso. I
trasferimenti di **SOL** nativi non dispongono di tale protezione, quindi il
mittente deve verificare il destinatario prima di firmare. Consulta
...
ma si noti che ogni istruzione contiene le stesse informazioni obbligatorie.
...
### Verifica il destinatario prima di effettuare il trasferimento

Poiché un trasferimento di SOL va a buon fine su qualsiasi account, verifica il
