...
account. Non esiste alcun controllo a livello di protocollo che verifichi se il
destinatario possa trasferire nuovamente i SOL. I lamport possono essere
spostati solo dal programma proprietario dell'account: inviare SOL a un
[mint di token](/docs/tokens/basics/create-mint), a un programma o a un
...
può restituirli. I SOL inviati a un
[token account](/docs/tokens/basics/create-token-account) sono recuperabili solo
dal proprietario di quell'account, mai dal mittente.

I trasferimenti di **token** SPL offrono una parziale protezione automatica: il
Token Program rifiuta un trasferimento i cui account non corrispondono al mint
atteso. I trasferimenti di **SOL** nativi non dispongono di tale protezione,
...
L'esempio seguente mostra la struttura di una transazione contenente una singola
istruzione di trasferimento SOL.
...
destinatario prima di firmare. Recupera l'account e invia fondi solo a un wallet
...
<Callout>
  Questo snippet verifica i destinatari nativi di SOL. Per la classificazione
  completa che gestisce anche i trasferimenti di token SPL (token account, ATA,
  Token-2022), consulta [Verifica
  Indirizzo](/docs/payments/send-payments/verify-address).
</Callout>
