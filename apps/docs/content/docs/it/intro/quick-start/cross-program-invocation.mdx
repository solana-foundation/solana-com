...
trasferimenti di SOL tra account invocando il System Program.
...
Prima, il programma necessita di un semplice meccanismo "paga per aggiornare"
modificando la struct _rs`Update`_ e la funzione `update`.

Inizia aggiornando il file `lib.rs` per portare nello scope gli elementi dal
modulo `system_program`.
...
Successivamente, aggiorna la struct _rs`Update`_ per includere un nuovo account
chiamato `vault_account`. Questo account, controllato dal programma, riceve SOL
da un utente quando aggiorna il proprio account messaggio.
...
_rs`Update`_. Questo account funge da "vault" controllato dal programma che
...
- L'indirizzo dell'account proviene da un PDA derivato usando i seed
  _rs`[b"vault", user.key().as_ref()]`_
- Essendo un PDA, non ha chiave privata, quindi solo il programma può "firmare"
  per l'indirizzo quando esegue CPI
- Come tipo _rs`SystemAccount`_, il System Program lo possiede come i normali
  account wallet
...
- `from` - L'account dell'utente (sorgente dei fondi)
...
lato client, dove si specifica il programma, gli account e l'instruction data
per una particolare istruzione da invocare. Quando l'istruzione `update` del
programma riceve un'invocazione, invoca internamente l'istruzione di
trasferimento del System Program.
...
permette di trasferire qualsiasi SOL nel vault all'utente quando chiude il
proprio account di messaggi.
...
Aggiungi anche il `system_program` poiché il CPI per il trasferimento richiede
...
Il `vault_account` utilizza la stessa derivazione PDA della struct Update.

L'aggiunta del `vault_account` alla struct Delete consente al programma di
accedere all'account vault dell'utente durante l'istruzione di eliminazione per
trasferire eventuali SOL accumulati all'utente.
...
Il metodo _rs`with_signer()`_ passa i signer seed con il CPI.
...
Nota che _rs`_ctx: Context<Delete>`_ diventa _rs`ctx: Context<Delete>`_ per
...
- Il `cpi_context` (programma, account e signer PDA)
...
account vault nelle istruzioni. Questo richiede la derivazione del PDA vault e
la sua inclusione nelle chiamate alle istruzioni update e delete.
...
Quindi, aggiorna l'istruzione di aggiornamento per includere il
...
Quindi, aggiorna l'istruzione di eliminazione per includere il
[`vaultAccount`](mention:one)
...
Clicca sul pulsante `Import` e inserisci un nome progetto per aggiungerlo alla
tua lista di progetti in Solana Playground. Una volta importato un progetto,
tutte le modifiche vengono salvate automaticamente e mantenute in modo
persistente.
