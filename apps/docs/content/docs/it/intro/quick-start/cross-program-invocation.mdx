...
Le istruzioni `update` e `delete` richiedono modifiche per gestire i
...
da un utente quando aggiorna il proprio account di messaggi.
...
- L'indirizzo dell'account deriva da un PDA usando i seed
...
`update` del programma riceve un'invocazione, invoca internamente l'istruzione
di trasferimento del System Program.
...
permette il trasferimento di qualsiasi SOL nel vault all'utente quando chiude il
proprio account messaggio.
...
Il metodo _rs`with_signer()`_ passa i seed del firmatario con la CPI.
