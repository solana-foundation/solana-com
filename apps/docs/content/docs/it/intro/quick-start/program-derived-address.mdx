...
description:
  Scopri come costruire un programma Solana CRUD (Create, Read, Update, Delete)
  utilizzando Program Derived Address (PDA) e il framework Anchor. Questa guida
  passo dopo passo illustra come creare, aggiornare ed eliminare account di
  messaggi onchain usando PDA, implementare la validazione degli account e
  scrivere test. Perfetto per sviluppatori che desiderano comprendere come
  utilizzare i PDA nei programmi Solana.
...
dati riportandoli al tipo di dati _rs`MessageAccount`_. La sezione di testing
...
Successivamente, aggiungi l'istruzione `update` per modificare `MessageAccount`
...
1. _rs`user: Signer<'info>`_
   ...
   - Contrassegnato come modificabile (_rs`#[account(mut)]`_) poiché potrebbe
     pagare per più spazio per `message_account` quando necessario
...
memorizzato su _rs`message_account`_, invece di farlo ricalcolare ad Anchor.
...
Hai completato il programma CRUD di base. Distribuisci il programma eseguendo
`deploy` nel terminale di Playground.
...
Solana Playground rimuove parte della configurazione boilerplate dove
...
Il metodo _ts`create()`_ richiama l'istruzione `create`.
...
Prima di tutto, il file di test invia una transazione che richiama l'istruzione
`create`, passando "Hello, World!" come messaggio.
...
Prima di tutto, il file di test invia una transazione che richiama l'istruzione
`update`, passando "Hello, Solana!" come nuovo messaggio.
...
Prima di tutto, questo codice invia una transazione che invoca l'istruzione
...
Dopo aver inviato la transazione e chiuso l'account, l'esempio prova a
