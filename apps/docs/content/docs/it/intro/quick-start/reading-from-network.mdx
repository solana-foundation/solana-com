...
Il campo `rentEpoch` è un campo legacy di un meccanismo di rent deprecato.
...
è un campo nel [tipo Account](/docs/core/accounts#account-structure) vero e
proprio, ma è incluso nella risposta.
...
Per leggere da un mint account, è necessario deserializzare il campo `data` nel
tipo di dati `Mint`, come mostrato
nell'[esempio successivo](#deserialize-mint-account).
...
[Token Program](/docs/references/terminology#token-program). Ciò significa che
il suo campo `data` può essere modificato solo dalle istruzioni del Token
Program.
...
parte dei programmi Solana fornisce librerie client con funzioni helper che
...
Il campo `supply` mostra il numero totale di token che sono stati mintati.
Questo valore è misurato nell'unità più piccola del token. Per ottenere
l'offerta totale in unità standard, regola il valore del campo `supply` in base
al `decimals`.
...
Il campo `decimals` mostra il numero di cifre decimali per il token.
...
Il campo `tlvData` contiene dati extra per Token Extensions e richiede ulteriore
deserializzazione. Questo campo è rilevante solo per gli account creati dal
