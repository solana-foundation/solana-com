...
a familiarizzare con Solana Playground e ti preparerà all'utilizzo dei Program
...
`Initialize`, che esegue le seguenti operazioni:
...
L'indirizzo onchain del tuo programma. Verrà aggiornato quando compilerai il
progetto
...
- La macro [`declare_id!`](mention:one) specifica l'indirizzo onchain del tuo
  programma. Solana Playground aggiornerà automaticamente questo indirizzo
  quando compilerai il programma.
...
- `ctx: Context<Initialize>`: passa gli account richiesti per questa istruzione
  alla funzione, come specificato nella struct `Initialize`.
- `data: u64`: un parametro personalizzato che devi fornire quando chiami
  l'istruzione.

Il corpo della funzione
[imposta il campo `data` di `new_account`](mention:three) all'argomento `data`
...
- L'attributo [`#[account]`](mention:one) definisce una struct che rappresenta
  il tipo di dato per gli account che questo programma crea e possiede.
...
<Callout>
  Il tuo [ID programma](/docs/intro/quick-start/build-first-program), definito
  in _rs`declare_id!()`_, sarà ora aggiornato.
</Callout>

2. Aggiungi SOL al tuo wallet _(Opzionale)_

Il deploy di un programma richiede SOL nel program account onchain. Se
necessario,
[aggiungi SOL al tuo wallet](/docs/intro/quick-start#add-sol-to-your-wallet)
prima di procedere allo step 3.
...
**Nota:** Se preferisci, puoi usare il pulsante `Test` nel pannello laterale
sinistro.
...
Chiudi il programma per recuperare i SOL allocati al programma onchain. Per
...
[indirizzi derivati dal programma](/docs/core/pda) e
[invocazione tra programmi](/docs/core/cpi).
