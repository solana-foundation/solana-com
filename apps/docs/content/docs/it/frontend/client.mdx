...
<Callout type="info">
  Esecuzione headless — in una route API, un worker o uno script senza wallet
  browser? Firma con un servizio di gestione delle chiavi backend invece di un
  keypair grezzo. Consulta [Keychain](/docs/tools/keychain) e [Firma in
...
- **State machine headless**: Esegui il client all'interno di route API, script
  ...
  alimenta la tua UI.
- **Dashboard in tempo reale**: Combina i watcher (saldi, account, firme) con la
  tua libreria UI preferita; il client gestisce il fan-out websocket e
  l'invalidazione della cache.
- **Store personalizzati**: Inietta il tuo store Zustand per idratare da
  IndexedDB/localStorage, sincronizzare lo stato con le sessioni server o
  coordinare tra tab del browser.
- **Bridge con gli hook React**: Passa un'istanza client configurata a
  `@solana/react-hooks` quando desideri hook ergonomici sopra lo stesso runtime.
- **Testabilità**: L'interfaccia del client singolo può essere simulata nei test
  unitari, rendendo semplice la simulazione di risposte RPC o sessioni wallet
  senza un wallet browser presente.
