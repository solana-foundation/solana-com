...
description:
  Configura l'integrazione del wallet Solana in Next.js con @solana/kit e
  @solana/react per inviare trasferimenti SOL.
---

Configura un'integrazione minimale del wallet Solana in Next.js (App Router) con
`@solana/kit`, i plugin del kit e `@solana/react`. Creerai un dropdown per
connettere il wallet e un componente per i trasferimenti SOL.

![App Next.js Kit](/assets/docs/frontend/01-hero-wallet.webp)
...
Quando richiesto, accetta tutte le impostazioni predefinite (lo starter include
Tailwind, che questo tutorial utilizza per semplici stili di utilità).
...
- In `tsconfig.json`, imposta `"target": "ES2020"` (o versione successiva). Gli
  importi dei trasferimenti usano letterali `bigint` come `1_000_000_000n`, che
  richiedono ES2020.
- In `app/globals.css`, rimuovi il blocco `@media (prefers-color-scheme: dark)`
  generato e la regola di sfondo `body`. Questa UI è solo in modalità chiara; le
  impostazioni scure predefinite altrimenti sovrascrivono l'utility `bg-white` e
  rendono la pagina illeggibile in modalità scura.
...
Questo tutorial ha come target Kit 7+, i pacchetti `@solana/kit-plugin-*` dalla
versione 0.13+ e `@solana/react` 7+. Se il tuo package manager segnala avvisi di
peer da un pacchetto `@solana-program/*` che dichiara un intervallo di peer Kit
precedente, ignoralo — i pacchetti funzionano con Kit 7.

<ScrollyCoding>

## !!steps 1. Crea il Provider Solana

`app/providers.tsx` crea un client a livello di modulo e lo pubblica con
`ClientProvider`. Il plugin `walletSigner` imposta il wallet connesso come
pagatore delle commissioni e come identità; esporta il tipo di client in modo
che i componenti possano tipizzare `useClient`.
...
Questo avvolge l'intera app con il contesto Solana in modo che tutti i
componenti figlio possano accedere al client e agli hook del wallet.

## !!steps 3. Pulsante di Connessione Wallet

Menu a tendina per connettere/disconnettere i wallet Wallet Standard.
`useWallets` restituisce i wallet rilevati nel browser; `useConnect` /
`useDisconnect` restituiscono un `ActionResult` — questo componente utilizza i
suoi campi `dispatch`, `isRunning` e `error`.
...
Costruisce un'istruzione di trasferimento con `@solana-program/system` e la
invia tramite `client.sendTransaction`. Il wallet connesso firma e paga le
commissioni.
...
I wallet provengono dal rilevamento tramite Wallet Standard; una volta connesso,
il trasferimento SOL utilizza il firmatario connesso come pagante delle
commissioni.

## !!steps 5. Pagina

`app/page.tsx` renderizza i componenti per la connessione del wallet e il
trasferimento SOL:
...
Apri http://localhost:3000, connetti un wallet Devnet e invia un trasferimento
SOL.
