...
Ora aggiungiamo le dipendenze necessarie per compilare un programma Solana.
...
contatore. Questi sono i dati che verranno memorizzati nel campo `data`
dell'account.
...
Ora dobbiamo deserializzare `instruction_data` (byte grezzi) in una delle
varianti dell'enum `CounterInstruction`. Il metodo `try_from_slice` di Borsh
...
istruzioni alle rispettive funzioni handler.

Questo pattern di routing è comune nei programmi Solana. `instruction_data`
viene deserializzato in una variante di un enum che rappresenta l'istruzione,
quindi viene chiamata la funzione handler appropriata. Ogni funzione handler
include l'implementazione per quella istruzione.

Aggiungi il seguente codice a `lib.rs` aggiornando la funzione
`process_instruction` e aggiungendo gli handler per le istruzioni
...
proprietario, conferendo al nostro programma la possibilità di scrivere
...
Per prima cosa, aggiungiamo le dipendenze necessarie per il testing. Useremo
...
L'attributo `#[cfg(test)]` garantisce che questo codice venga compilato solo
...
Puoi verificare il deployment utilizzando il comando `solana program show` con
il tuo program ID:
...
firme delle transazioni restituite. Nota che il cluster su Solana Explorer deve
essere impostato su "Custom RPC URL", che per impostazione predefinita è
`http://localhost:8899` su cui `solana-test-validator` è in esecuzione.
