...
| Aggiornare un programma esistente            | `solana program deploy <path_to_program.so>` (come per la distribuzione)                                 |
| Mostrare le informazioni del programma                  | `solana program show <program-id>`                                                            |
| Trasferire l'autorità del programma         | `solana program set-upgrade-authority <program-id> --new-upgrade-authority <path_to_keypair>` |
...
- **ID del Programma**: L'indirizzo onchain del tuo programma. Gli utenti
  interagiscono con il tuo programma facendo riferimento all'ID del Programma.
- **program account**: L'account onchain che memorizza i metadati del tuo
  programma. L'indirizzo del program account è l'ID del Programma. Il program
  account memorizza anche l'indirizzo del programdata account.
- **programdata account**: L'account onchain che memorizza il codice eseguibile
  distribuito del tuo programma.
- **Autorità del Programma**: L'account che ha il permesso di aggiornare o
  chiudere il programma. Per impostazione predefinita, questo è il tuo wallet
  CLI.
...
Scegli su quale cluster Solana distribuire. Usa il comando `solana config get`
per controllare la tua configurazione attuale:
...
1. `hello_world-keypair.json`: un file keypair la cui chiave pubblica verrà
   utilizzata come ID del tuo programma
...
Aggiorna il tuo programma usando lo stesso comando `solana program deploy`:
...
Per verificare i metadati del tuo programma, usa il comando
`solana program show`:
...
L'ID del Programma è l'indirizzo onchain del tuo program account. Gli utenti e
altri programmi invocano il tuo programma facendo riferimento a questo ID nelle
loro transazioni.
...
Per chiudere il tuo programma e recuperare i SOL allocati al program account,
usa il comando `solana program close`:
...
- **`--with-compute-unit-price`**: Imposta la commissione prioritaria in
  micro-lamport (0,000001 SOL) per unità di calcolo. Controlla
  [Helius Priority Fee API](https://docs.helius.dev/guides/priority-fee-api) per
  le tariffe correnti. Una commissione prioritaria è una commissione aggiuntiva
  pagata al leader corrente per dare priorità alla tua transazione.
- **`--max-sign-attempts`**: Numero di tentativi da ripetere con un nuovo
  blockhash se le transazioni scadono. (Predefinito: 5)
- **`--use-rpc`**: Invia le transazioni all'RPC configurato. Questo flag richiede una connessione RPC [ponderata per stake](/docs/defi/stake-weighted-qos) da provider come [Triton](https://triton.one/) o [Helius](https://www.helius.dev/).
