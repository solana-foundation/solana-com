...
liballoc di Rust, oltre a numerose crate di terze parti.
...
- Bincode è estremamente costoso dal punto di vista computazionale sia in
  termini di cicli che di profondità delle chiamate e dovrebbe essere evitato
- La formattazione delle stringhe dovrebbe essere evitata poiché è anch'essa
  computazionalmente costosa
- Nessun supporto per `println!`, `print!`, utilizzare invece la macro
...
- Il runtime impone un limite al numero di istruzioni che un programma può
  eseguire durante l'elaborazione di una singola istruzione. Consulta
  [budget computazionale](/docs/core/fees#compute-budget) per maggiori
  informazioni
...
## Profondità delle chiamate CPI - errore `CallDepth`

Le invocazioni cross-program consentono ai programmi di invocare altri programmi
direttamente, ma la profondità è attualmente limitata a `4`.

Quando un programma supera la
[profondità di chiamata delle invocazioni cross-program](/docs/core/cpi)
consentita, riceverà un errore `CallDepth`
