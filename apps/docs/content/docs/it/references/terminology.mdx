...
Indicato anche come `transaction cost`, è una stima completa di tutte le risorse
necessarie per elaborare una transazione, misurata in
[compute units](#compute-units). Maggiori informazioni
...
una migliore alternativa dal consenso del cluster. Uno slot saltato non apparirà
...
trasferire, congelare e coniare token.
