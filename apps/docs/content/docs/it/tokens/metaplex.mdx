...
Il Token Metadata Program crea un Program Derived Address (PDA) per ogni conio
di token. Questo account di metadati memorizza informazioni onchain come il nome
e il simbolo del token, oltre a un URI che punta ai metadati JSON off-chain
(immagini, descrizioni, ecc.).
