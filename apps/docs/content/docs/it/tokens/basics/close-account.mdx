...
description:
  Chiudi un account token e restituisci i lamport dell'account a un account di
  destinazione.
...
La chiusura di un account token elimina l'account e restituisce i lamport di
rent dell'account token a un account di destinazione.

Il saldo dei token deve essere zero prima che l'account possa essere chiuso. Un
account congelato può comunque essere chiuso se il saldo dei token dell'account
congelato è zero. Gli account token di SOL wrappato costituiscono l'eccezione e
possono essere chiusi anche con un saldo di token per recuperare i SOL
sottostanti. Il proprietario dell'account o l'autorità di chiusura dell'account
firma l'istruzione di chiusura.
