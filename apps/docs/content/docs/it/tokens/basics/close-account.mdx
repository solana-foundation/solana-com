...
description:
  Chiudi un token account e restituisci i lamport dell'account a un account di
  destinazione.
...
## Cosa Fa la Chiusura di un Token Account?

La chiusura di un token account elimina l'account e restituisce i lamport di
rent del token account a un account di destinazione.
...
congelato è zero. I wrapped SOL token account sono l'eccezione e possono essere
chiusi con un saldo di token per recuperare i SOL sottostanti. Il proprietario
dell'account o l'autorità di chiusura dell'account firma l'istruzione di
chiusura.

## Come Chiudere un Token Account

La chiusura di un token account utilizza l'istruzione _rs`CloseAccount`_ del
Token Program.
