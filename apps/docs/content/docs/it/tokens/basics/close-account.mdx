---
title: Chiudere un Account Token
description:
  Chiudi un account token e restituisci i lamport dell'account a un account di destinazione.
...
## Cosa Comporta la Chiusura di un Account Token?

La chiusura di un token account elimina l'account e trasferisce tutti i suoi lamport,
compreso il saldo minimo rimborsabile per l'archiviazione, a un account di destinazione.

Il saldo del token deve essere zero prima che l'account possa essere chiuso. Un account bloccato
può comunque essere chiuso se il saldo del token dell'account bloccato è zero. I token account di SOL wrapped
costituiscono un'eccezione e possono essere chiusi con un saldo del token per
recuperare il SOL sottostante. Il proprietario dell'account o l'autorità di chiusura dell'account
firma l'istruzione di chiusura e seleziona la destinazione. L'account che ha pagato
per creare un associated token account non riceve automaticamente il saldo recuperato.

## Come Chiudere un Account Token

La chiusura di un account token utilizza l'istruzione _rs`CloseAccount`_ del
Token Program.

L'istruzione _rs`CloseAccount`_ trasferisce i lamport dell'account di origine a
un account di destinazione, svuota l'account di origine ed elimina l'account di
origine. Il Token Extension Program applica controlli di chiusura aggiuntivi per
determinate estensioni, ma il flusso di chiusura di base rimane lo stesso.

### Riferimento al Codice Sorgente

| Elemento                        | Descrizione                                                                                                     | Token Program                                                                                                  | Token Extension Program                                                                                              |
| --------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | Lo stato dell'account token memorizza il saldo, lo stato nativo e l'autorità di chiusura utilizzati durante la chiusura dell'account.     | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | Un'istruzione che chiude un account token e trasferisce i lamport dell'account token a un account di destinazione. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | Logica condivisa del processore per la chiusura dell'account token.                                                               | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |
