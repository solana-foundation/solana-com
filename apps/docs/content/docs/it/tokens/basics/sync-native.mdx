...
title: Sync Native
description:
  Aggiorna l'importo di un account token SOL incapsulato dopo aver trasferito
  SOL nell'account.
...
utilizza per i token account che incapsulano SOL. Il mint nativo consente di
utilizzare SOL attraverso le istruzioni del Token Program.
...
incapsulato come saldo token attraverso il campo `amount` del token account. Se
trasferisci SOL in un token account SOL incapsulato, il saldo in lamport aumenta
immediatamente, ma il campo `amount` del token non cambia finché non chiami
l'istruzione `SyncNative` del Token Program.

## Come Sincronizzare il SOL Nativo

Utilizza l'istruzione `SyncNative` del Token Program dopo aver trasferito SOL in
un token account SOL incapsulato. L'istruzione `SyncNative` aggiorna il campo
`amount` del token account per corrispondere ai lamport detenuti nell'account
oltre la sua riserva esente da affitto.

### Riferimento Sorgente

| Elemento              | Descrizione                                                                                                                                                                                                                   | Token Program                                                                                                    | Token Extensions Program                                                                                               |
| --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `Account`             | Lo stato del token account utilizza `is_native` per contrassegnare l'account come WSOL e memorizza la riserva nativa, il saldo minimo in lamport esente da affitto che l'account deve mantenere, e l'importo token tracciato. | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| `SyncNative`          | Un'istruzione che aggiorna il campo `amount` di un token account SOL incapsulato per corrispondere ai lamport detenuti nell'account oltre la sua riserva esente da affitto.                                                   | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| `process_sync_native` | Logica condivisa del processore per sincronizzare i saldi SOL incapsulati.                                                                                                                                                    | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |

### Typescript

Gli esempi seguenti mostrano l'approccio consigliato utilizzando `@solana/kit`.
Gli esempi legacy che utilizzano `@solana/web3.js` sono inclusi come
riferimento.
