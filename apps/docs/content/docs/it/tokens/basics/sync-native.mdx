...
title: Sync Native
description:
  Aggiorna l'importo di un token account SOL incapsulato dopo aver trasferito
  SOL nell'account.
...
## Cos'è il SOL Incapsulato?

Il SOL incapsulato (WSOL) è SOL detenuto in un token account per il mint nativo
del Token Program. Il mint nativo è l'indirizzo mint che il Token Program
utilizza per i token account che incapsulano SOL. Il mint nativo consente
l'utilizzo di SOL attraverso le istruzioni del Token Program.

| Programma                | Indirizzo mint nativo                          |
| ------------------------ | ---------------------------------------------- |
...
Un token account SOL incapsulato memorizza SOL nel campo `lamports`
dell'account, ma tiene traccia del SOL detenuto nel token account SOL
incapsulato come saldo token attraverso il campo _rs`amount`_ del token account.
Se trasferisci SOL in un token account SOL incapsulato, il saldo in lamport
aumenta immediatamente, ma il campo _rs`amount`_ del token non cambia finché non
richiami l'istruzione _rs`SyncNative`_ del Token Program.

## Come Sincronizzare il SOL Nativo

Utilizza l'istruzione _rs`SyncNative`_ del Token Program dopo aver trasferito
SOL in un token account SOL incapsulato. L'istruzione _rs`SyncNative`_ aggiorna
il campo _rs`amount`_ del token account per corrispondere ai lamport detenuti
nell'account al di sopra della sua riserva esente da affitto.

### Riferimenti al Codice Sorgente

| Elemento                  | Descrizione                                                                                                                                                                                                                       | Token Program                                                                                                    | Token Extensions Program                                                                                               |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_             | Lo stato del token account utilizza _rs`is_native`_ per contrassegnare l'account come WSOL e memorizza la riserva nativa, il saldo minimo in lamport esente da affitto che l'account deve mantenere, e l'importo token tracciato. | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`SyncNative`_          | Un'istruzione che aggiorna il campo _rs`amount`_ di un token account SOL incapsulato per corrispondere ai lamport detenuti nell'account al di sopra della sua riserva esente da affitto.                                          | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| _rs`process_sync_native`_ | Logica condivisa del processore per la sincronizzazione dei saldi SOL incapsulati.                                                                                                                                                | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |

### Typescript

Gli esempi seguenti mostrano l'approccio consigliato utilizzando `Kit`. Gli
