...
Wrapped SOL (WSOL) è un token account che tiene traccia della quantità di
...
sincronizza il saldo di un token account wrapped SOL (WSOL) con i SOL effettivi
(lamport) memorizzati al suo interno. Quando trasferisci SOL nativi a un token
account WSOL, il saldo del token non si aggiorna automaticamente. Devi chiamare
SyncNative per riflettere il saldo WSOL corretto.
