...
title: Brucia Token
description:
  Distruggi token da un token account e riduci l'offerta totale del mint.
...
Distruggere token riduce permanentemente il saldo di un token account e riduce
l'offerta totale del mint dello stesso importo.

## Come Distruggere Token

La distruzione di token utilizza l'istruzione _rs`Burn`_ o _rs`BurnChecked`_ del
Token Program.

Gli esempi seguenti utilizzano _rs`BurnChecked`_, che richiede al chiamante di
fornire i decimali del mint affinché l'istruzione possa verificare la precisione
dei token prevista prima della distruzione.

Il [native mint](/docs/tokens/basics/sync-native) non supporta la distruzione.
Utilizza invece _rs`CloseAccount`_.

Il proprietario del token account o un delegato approvato firma la transazione.
Nel Token Extension Program, un permanent delegate del mint può anche
autorizzare la distruzione se il mint ha l'estensione permanent delegate
abilitata.

### Riferimento Sorgente

| Elemento           | Descrizione                                                                                                                                                                      | Token Program                                                                                                  | Token Extension Program                                                                                              |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_         | Lo stato del mint memorizza l'offerta totale di token.                                                                                                                           | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | Lo stato del token account memorizza il saldo dei token ridotto da una distruzione.                                                                                              | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Un'istruzione di distruzione che riduce il saldo di un token account e l'offerta del mint senza richiedere al chiamante di fornire i decimali del mint.                          | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Un'istruzione di distruzione che richiede al chiamante di fornire i decimali del mint e verifica tale valore prima di ridurre il saldo di un token account e l'offerta del mint. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | Logica condivisa del processore per la distruzione di token.                                                                                                                     | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |
