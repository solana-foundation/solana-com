...
description:
  Congela un token account con l'autorità di congelamento del mint per bloccare
  trasferimenti, burn e modifiche ai delegati.
...
## Cosa Comporta il Congelamento di un Token Account?

Il congelamento di un token account mantiene lo stesso proprietario, mint e
saldo, ma impedisce al token account di ricevere, trasferire o bruciare token
finché il token account non viene scongelato.

Un account congelato può comunque essere chiuso se il saldo di token
dell'account congelato è pari a zero.

Solo un mint che dispone ancora di un'autorità di congelamento può congelare i
token account. Se l'autorità di congelamento di un mint viene revocata, gli
account per quel mint non possono più essere congelati.

## Come Congelare un Token Account

Il congelamento di un token account utilizza l'istruzione _rs`FreezeAccount`_
del Token Program.

L'istruzione _rs`FreezeAccount`_ verifica che il token account appartenga al
mint e che il firmatario sia l'autorità di congelamento del mint, quindi
modifica lo stato del token account da inizializzato a congelato.

I [token account nativi](/docs/tokens/basics/sync-native) non supportano il
congelamento.
...
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Lo stato del mint memorizza l'autorità di congelamento opzionale utilizzata per congelare i token account. | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Gli stati del token account, inclusi _rs`Initialized`_ e _rs`Frozen`_.                                     | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | Un'istruzione che modifica un token account nello stato congelato.                                         | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
| _rs`process_toggle_freeze_account`_ | Logica del processore condivisa per il congelamento e lo scongelamento dei token account.                  | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |

### Typescript

Gli esempi `Kit` seguenti mostrano l'approccio consigliato utilizzando
