...
description:
  ...
  trasferimenti, bruciature e modifiche ai delegati.
...
## Cosa Comporta Congelare un Token Account?

Congelare un token account mantiene lo stesso proprietario, mint e saldo, ma
impedisce al token account di trasferire o bruciare token finché il token
account non viene scongelato.

Un account congelato può comunque essere chiuso se l'account congelato ha un
saldo token pari a zero.

Solo un mint che ha ancora un'autorità di congelamento può congelare i token
account. Se l'autorità di congelamento di un mint viene revocata, gli account
per quel mint non possono più essere congelati.

## Come Congelare un Token Account

Congelare un token account utilizza l'istruzione _rs`FreezeAccount`_ del Token
Program.

L'istruzione _rs`FreezeAccount`_ verifica che il token account appartenga al
mint e che il firmatario sia l'autorità di congelamento del mint, quindi cambia
lo stato del token account da inizializzato a congelato.
...
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Lo stato del mint memorizza l'autorità di congelamento opzionale utilizzata per congelare i token account. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Gli stati del token account, inclusi _rs`Initialized`_ e _rs`Frozen`_.                                     | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | Un'istruzione che cambia un token account nello stato congelato.                                           | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
| _rs`process_toggle_freeze_account`_ | Logica del processore condivisa per congelare e scongelare i token account.                                | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |

### Typescript

Gli esempi `Kit` di seguito mostrano l'approccio consigliato utilizzando
`@solana/kit`. Gli esempi legacy che utilizzano `@solana/web3.js` sono inclusi a
