...
- Solo l'autorità del mint può coniare nuovi token
...
async fn main() -> Result<()> {
    ...
    // Genera keypair da utilizzare come indirizzo del mint
    ...
    let initialize_mint_instruction = initialize_mint(
        &token_program_id(),
        &mint.pubkey(),            // mint
        &fee_payer.pubkey(),       // autorità del mint
        Some(&fee_payer.pubkey()), // autorità di freeze
        2,                         // decimali
    )?
    ...
    // Crea la transazione e aggiungi le istruzioni
    ...
    // Quantità di token da mintare (100 token con 2 cifre decimali)
    let amount = 100;

    // Crea l'istruzione mint_to per mintare token nell'associated token account
    ...
    // Crea la transazione per mintare i token
    let transaction = Transaction::new_signed_with_payer(
        &[mint_to_instruction],
        Some(&fee_payer.pubkey()),
        &[&fee_payer],
    ...
    println!("Mintati 1.00 token nell'associated token account");
    println!("\nIndirizzo mint: {}", mint.pubkey());
    println!("{:#?}", mint_data);

    println!(
        "\nIndirizzo associated token account: {}",
        associated_token_address
    );
    println!("{:#?}", token_data);

    println!("Firma transazione: {}", transaction_signature);
...
async def main():
    ...
    # Keypair e indirizzi di esempio
    payer = Keypair()
    ...
    # Quantità da coniare (nell'unità più piccola)
    amount_to_mint = 1000000000  # 1 token con 9 decimali

    async with rpc:
        # Crea istruzione di conio
        ...
        print(f"Destinazione: {destination_token_account}")
        print(f"Quantità: {amount_to_mint}")
        print(f"Autorità di conio: {mint_authority.pubkey()}")
        print(f"Pagatore: {payer.pubkey()}")
        print(f"Transazione di conio creata con successo")
