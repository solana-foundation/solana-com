---
title: Impostare l'autorità
description:
  Modificare o revocare i ruoli di autorità su un mint account o token account,
  incluse le autorità di conio, congelamento, proprietà e chiusura.
...
## Cosa Fa l'Impostazione dell'Autorità?

L'impostazione dell'autorità modifica o revoca ruoli di autorità specifici su un
mint account o token account.

I mint account possono memorizzare un'autorità di conio e un'autorità di
congelamento. I token account memorizzano un proprietario e possono
facoltativamente memorizzare un'autorità di chiusura. Ogni ruolo controlla un
set di permessi specifico, e impostare un'autorità su _rs`None`_ rimuove
permanentemente il ruolo di autorità selezionato.

Il Token Extensions Program definisce anche tipi di autorità aggiuntivi per
estensioni specifiche.

## Come Impostare l'Autorità

La modifica di un'autorità utilizza l'istruzione _rs`SetAuthority`_ del Token
Program.

Ogni chiamata aggiorna un tipo di autorità su un mint account o token account.
L'autorità corrente firma la modifica, e la nuova autorità può essere un altro
indirizzo o _rs`None`_ per revocare permanentemente l'autorità selezionata.

### Riferimento al Codice Sorgente

| Elemento                    | Descrizione                                                                         | Token Program                                                                                                    | Token Extensions Program                                                                                                 |
| --------------------------- | ----------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| _rs`Mint`_                  | Lo stato del mint memorizza _rs`mint_authority`_ e _rs`freeze_authority`_.          | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)           |
| _rs`Account`_               | Lo stato del token account memorizza _rs`owner`_ e _rs`close_authority`_.           | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)          |
| _rs`AuthorityType`_         | I ruoli di autorità che _rs`SetAuthority`_ può aggiornare.                          | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L739-L751) | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1116-L1157) |
| _rs`SetAuthority`_          | Un'istruzione che modifica o revoca un'autorità su un mint account o token account. | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L141-L158) | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L183-L200)   |
| _rs`process_set_authority`_ | Logica condivisa del processore per gli aggiornamenti di autorità.                  | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L423-L518)     | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L677-L978)       |
