...
description: Scopri come impostare l'autorità per i mint o i token account.
---
...
- **Autorità di mint**: Controlla la creazione di nuovi token. Può coniare token
  verso qualsiasi token account. Spesso viene revocata dopo la creazione della
  fornitura iniziale per creare un token a fornitura fissa.
- **Autorità di freeze**: Controlla la capacità di congelare e scongelare i
  token account. Può impedire a qualsiasi token account di trasferire token.
  Spesso viene revocata per garantire agli utenti che i loro token non possano
  essere congelati.

### Autorità del token account

- **Proprietario dell'account**: Ha il controllo completo sul token account. Può
  trasferire token, bruciare token, approvare delegati e chiudere l'account
  quando il saldo è zero.
- **Autorità di chiusura**: Può chiudere il token account quando il suo saldo è
  zero. Per impostazione predefinita, questo è il proprietario dell'account ma
  può essere delegato a un altro account.
...
modifica o revoca le autorità sui mint e sui token account. Solo l'autorità
corrente può trasferire i propri permessi a un nuovo indirizzo o revocarli
permanentemente impostando l'autorità su null. Una volta revocata, l'autorità
