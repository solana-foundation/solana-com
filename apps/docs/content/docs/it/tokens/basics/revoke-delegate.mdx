...
description:
  Rimuovi il delegato corrente di un token account e azzera la quota delegata
  rimanente.
...
## Cosa Fa la Revoca di un Delegato?

Revocare un delegato cancella il delegato corrente del token account e reimposta
a zero l'importo delegato.

Dopo la revoca, il delegato precedente non ha più l'autorità di trasferire o
bruciare token per conto del token account.

## Come Revocare un Delegato

La revoca di un delegato utilizza l'istruzione _rs`Revoke`_ del Token Program.

L'istruzione _rs`Revoke`_ cancella i campi del delegato sul token account
sorgente. Nel Token Program, il proprietario del token account firma la revoca.
Nel Token Extensions Program, anche il delegato corrente può revocare la delega.

### Riferimento al Codice Sorgente

| Elemento             | Descrizione                                                            | Token Program                                                                                                    | Token Extensions Program                                                                                               |
| -------------------- | ---------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_        | Lo stato del token account memorizza il delegato e l'importo delegato. | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Revoke`_         | Un'istruzione che rimuove il delegato corrente da un token account.    | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L128-L140) | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L170-L182) |
| _rs`process_revoke`_ | Logica condivisa del processore per la revoca del delegato.            | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L395-L420)     | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L643-L674)     |

### Typescript

Gli esempi `Kit` qui sotto mostrano l'approccio consigliato utilizzando
`@solana/kit`. Esempi legacy che utilizzano `@solana/web3.js` sono inclusi come
riferimento.
