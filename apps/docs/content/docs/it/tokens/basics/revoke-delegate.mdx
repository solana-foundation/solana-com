...
description:
  Rimuovi il delegato corrente di un token account e azzera l'importo delegato
  residuo.
...
L'istruzione _rs`Revoke`_ cancella i campi del delegato nel token account di
origine. Nel Token Program, il proprietario del token account firma la revoca.
