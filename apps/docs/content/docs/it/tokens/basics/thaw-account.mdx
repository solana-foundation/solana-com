...
description:
  Sblocca un token account congelato in modo che trasferimenti, burn e modifiche
  ai delegati possano riprendere.
...
## Cosa Fa lo Sblocco di un Token Account?

Lo sblocco riporta un token account dallo stato congelato allo stato
inizializzato.

Dopo che lo sblocco ha avuto successo, l'account può nuovamente ricevere,
trasferire e bruciare token. Solo l'autorità di congelamento del mint può
sbloccare i token account.

## Come Sbloccare un Token Account

Lo sblocco di un token account utilizza l'istruzione _rs`ThawAccount`_ del Token
Program.
...
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Lo stato del mint memorizza l'autorità di congelamento opzionale utilizzata per sbloccare i token account. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
