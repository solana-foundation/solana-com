...
description:
  Scongela un token account congelato per consentire nuovamente trasferimenti,
  burn e modifiche ai delegati.
...
## Cosa fa lo scongelamento di un Token Account?

Lo scongelamento riporta un token account dallo stato congelato allo stato
inizializzato.

Una volta completato lo scongelamento, l'account può nuovamente trasferire,
bruciare e chiudere token. Solo l'autorità di congelamento del mint può
scongelare token account.

## Come scongelare un Token Account

Lo scongelamento di un token account utilizza l'istruzione _rs`ThawAccount`_ del
Token Program.

L'istruzione _rs`ThawAccount`_ verifica che il token account appartenga al mint
e che il firmatario sia l'autorità di congelamento del mint, quindi cambia lo
stato del token account da congelato a inizializzato.

I [token account nativi](/docs/tokens/basics/sync-native) non supportano lo
scongelamento.
...
| ----------------------------------- | --------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Lo stato del mint memorizza l'autorità di congelamento opzionale utilizzata per scongelare token account. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Gli stati del token account, inclusi _rs`Frozen`_ e _rs`Initialized`_.                                    | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`ThawAccount`_                   | Un'istruzione che riporta un token account congelato allo stato inizializzato.                            | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L229-L243) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L292-L306) |
| _rs`process_toggle_freeze_account`_ | Logica condivisa del processore per congelare e scongelare token account.                                 | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |
