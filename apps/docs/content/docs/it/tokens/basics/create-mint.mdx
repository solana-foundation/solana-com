...
description:
  Crea un mint di token e configura i decimali del mint, l'autorità di mint e
...
Un mint account definisce e identifica in modo univoco un token su Solana, e
memorizza lo stato condiviso che si applica a tutti i token account per quel
mint.

Il Token Program definisce il tipo di account _rs`Mint`_ come:
...
Ogni token ha un mint account, e l'indirizzo del mint è l'identificatore univoco
del token attraverso wallet, applicazioni ed explorer.
...
1. L'istruzione _rs`CreateAccount`_ del System Program crea un nuovo account
   ...
   nuovo account.
2. L'istruzione _rs`InitializeMint`_ o _rs`InitializeMint2`_ del Token Program
   inizializza il nuovo account come mint.
...
account non sia già inizializzato e sia esente da rent. Il Token Program quindi
scrive l'autorità di mint, l'autorità di congelamento, i decimali e il flag
_rs`is_initialized`_ nei dati del mint account.

### Riferimento al Codice Sorgente

| Elemento                       | Descrizione                                                                                                   | Token Program                                                                                                    | Token Extensions Program                                                                                               |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                     | Lo stato base del mint memorizzato in ogni mint account.                                                      | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`InitializeMint`_           | Un'istruzione di inizializzazione del mint che richiede l'account sysvar rent nella sua lista di account.     | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L24-L44)   | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L50-L74)   |
| _rs`InitializeMint2`_          | Un'istruzione di inizializzazione del mint che non richiede l'account sysvar rent nella sua lista di account. | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L401-L414) | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L471-L486) |
| _rs`_process_initialize_mint`_ | Logica condivisa del processore per l'inizializzazione del mint.                                              | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
| _rs`process_initialize_mint`_  | Gestore pubblico per _rs`InitializeMint`_.                                                                    | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L62-L70)       | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L132-L140)     |
| _rs`process_initialize_mint2`_ | Gestore pubblico per _rs`InitializeMint2`_.                                                                   | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L72-L81)       | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L142-L151)     |
...
Un mint account memorizza solo le informazioni di base del token (supply,
decimali, autorità). Per aggiungere metadati leggibili come nome, simbolo e
immagine al tuo token, hai due opzioni:
...
<Cards>
  ...
  <Card title="Token Extensions Metadata" href="/docs/tokens/extensions/metadata">
   Utilizza l'estensione metadata integrata con Token-2022. 
   L'estensione metadata è disponibile solo per i mint account creati con Token-2022.
   Per aggiungere metadati ai mint account creati con il Token Program originale, utilizza il Metaplex Token Metadata Program.
  </Card>
</Cards>
