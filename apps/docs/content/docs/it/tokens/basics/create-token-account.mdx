...
description:
  Crea account di token e account di token associati per un mint e proprietario
  specifici.
...
## Che Cos'è un Account di Token

Un account di token memorizza token per un mint e un proprietario di account di
token su Solana.
...
dall'account di token. Il proprietario del programma dell'account rimane
comunque il Token Program o il Token Extension Program.

Ogni account di token è legato a esattamente un mint, il che significa che
l'account di token può contenere unità di un solo token, il token identificato
dal campo _rs`mint`_ dell'account di token.

## Che Cos'è un Account di Token Associato

Un associated token account (ATA) è l'account di token predefinito per un wallet
e un mint.
...
Solo gli account di token creati dall'Associated Token Program sono chiamati
associated token account.

L'Associated Token Program è un modo per creare un account di token a un
indirizzo standard e deterministico. L'account risultante è comunque un account
di token posseduto dal Token Program o dal Token Extension Program, non
dall'Associated Token Program.
...
un indirizzo ATA. L'Associated Token Program crea un account di token standard a
quell'indirizzo, e l'account risultante utilizza comunque il tipo _rs`Account`_
definito dal Token Program o dal Token Extension Program.

## Cos'è un associated token account?

Un associated token account (ATA) è il token account predefinito per un wallet
che detiene un token specifico. Utilizza un indirizzo deterministico (PDA)
creato
dall'[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program).
...
ATA. Questo elimina la necessità di tracciare manualmente gli indirizzi dei
token account: puoi sempre derivare l'indirizzo corretto.
...
- Effettua Cross-Program Invocation (CPI) al Token Program
...
account e la sua inizializzazione tramite Cross Program Invocations (CPI).
