...
Un token account archivia token per un mint e un proprietario del token account
su Solana.
...
dal token account. Il proprietario del programma dell'account rimane il Token
Program o il Token Extensions Program.
...
standard e deterministico. L'account risultante è ancora un token account di
...
quell'indirizzo e l'account risultante utilizza ancora il tipo _rs`Account`_
...
_rs`CreateIdempotent`_ dell'Associated Token Program. L'Associated Token Program
deriva l'indirizzo ATA, crea l'account all'indirizzo ATA e inizializza l'account
come token account di proprietà del Token Program o del Token Extension Program.

<Callout type="info" title="Consigliato">
  Per la maggior parte delle applicazioni, crea i token account tramite
  l'Associated Token Program invece di crearli chiamando direttamente le
  ...
  program e dal mint, il che rende più facile per wallet e applicazioni trovare
  il token account predefinito per un determinato mint.
</Callout>
...
| _rs`CreateIdempotent`_                        | Crea l'associated token account, ma ha comunque successo se quell'ATA esiste già per lo stesso proprietario e mint.                                                                                                                                                             | [Sorgente](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | Deriva l'indirizzo ATA e utilizza _rs`create_pda_account`_ insieme a CPI verso il token program selezionato per inizializzare il token account. Quando il programma selezionato è il Token Extension Program, il processore ATA inizializza anche l'estensione immutable owner. | [Sorgente](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | Helper utilizzato da _rs`process_create_associated_token_account`_ per creare l'account PDA tramite CPI nel System Program.                                                                                                                                                     | [Sorgente](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |
...
1. L'istruzione _rs`CreateAccount`_ del System Program crea un nuovo account
   esente da rent e assegna il Token Program come proprietario del programma del
   nuovo account.
...
<Callout type="info">
  ...
  delle applicazioni, utilizza invece l' Associated Token Program. Utilizza le
  chiamate dirette al System Program e al Token Program quando hai una ragione
  specifica per non utilizzare un associated token account o quando devi creare
...
| _rs`InitializeAccount`_           | Un'istruzione di inizializzazione del token account che si aspetta il proprietario e l'account rent sysvar nella sua lista di account.       | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L45-L63)   | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L75-L93)   |
| _rs`InitializeAccount2`_          | Un'istruzione di inizializzazione del token account che passa il proprietario nei dati dell'istruzione anziché nella lista degli account.    | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L352-L365) | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L419-L433) |
| _rs`InitializeAccount3`_          | Un'istruzione di inizializzazione del token account che passa il proprietario nei dati dell'istruzione e non richiede l'account rent sysvar. | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L377-L387) | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L445-L456) |
...
| _rs`process_initialize_account`_  | Gestore pubblico per _rs`InitializeAccount`_.                                                                                                | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L143-L150)     | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L236-L240)     |
| _rs`process_initialize_account2`_ | Gestore pubblico per _rs`InitializeAccount2`_.                                                                                               | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L152-L160)     | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L242-L246)     |
| _rs`process_initialize_account3`_ | Gestore pubblico per _rs`InitializeAccount3`_.                                                                                               | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L162-L170)     | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L248-L252)     |
