...
description: Scopri come creare SPL token account.
---

## Cos'è un token account?

Un token account memorizza il tuo saldo di un token specifico. Ogni token
account è associato esattamente a un mint e tiene traccia del saldo del token e
...
Per detenere token, hai bisogno di un token account per quel mint specifico.
Ogni token account tiene traccia di:

1. **Mint**: Il tipo di token specifico che detiene
...
- Questo account detiene solo token USDC
- Circle controlla i token attraverso l'autorità owner del token account:
...
1. **Token account owner**: L'autorità che può trasferire, bruciare o delegare
   token. Questo è memorizzato nei dati del token account.
