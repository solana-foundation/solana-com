...
description:
  Trasferisci token tra token account della stessa mint con le istruzioni di
  trasferimento del Token Program.
...
## Cosa Fa il Trasferimento di Token?

Il trasferimento di token sposta i token da un token account a un altro token
account della stessa mint.

I trasferimenti non modificano l'offerta della mint. I trasferimenti aggiornano
solo i saldi tra i token account.

## Come Trasferire Token

Il trasferimento di token utilizza l'istruzione _rs`Transfer`_ o
_rs`TransferChecked`_ del Token Program.

Gli esempi seguenti utilizzano _rs`TransferChecked`_, che richiede al chiamante
di fornire la mint e i decimali in modo che l'istruzione possa verificare la
mint prevista e la precisione del token prima di spostare i token tra gli
account.

Nel Token Extensions Program, _rs`Transfer`_ è deprecato a favore di
_rs`TransferChecked`_ o _rs`TransferCheckedWithFee`_.

Il proprietario dell'account di origine o un delegato approvato firma il
trasferimento. Nel Token Extensions Program, un delegato permanente della mint
può anche autorizzare il trasferimento se la mint ha abilitato l'estensione del
delegato permanente.
...
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_          | Lo stato del token account memorizza i saldi aggiornati da un trasferimento.                                                                                                                                | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | Un'istruzione di trasferimento che sposta i token tra i token account senza richiedere al chiamante di fornire la mint o i decimali. Nel Token Extensions Program, utilizzare invece _rs`TransferChecked`_. | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Un'istruzione di trasferimento che richiede al chiamante di fornire la mint e i decimali e verifica tali valori prima di spostare i token tra i token account della stessa mint.                            | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | Logica condivisa del processore per i trasferimenti di token.                                                                                                                                               | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |

### Typescript

Gli esempi di seguito mostrano l'approccio consigliato utilizzando `Kit`. Gli
esempi legacy che utilizzano `@solana/web3.js` sono inclusi come riferimento.
