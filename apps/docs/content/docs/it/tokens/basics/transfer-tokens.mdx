...
description:
  Trasferisci token tra token account dello stesso mint con le istruzioni di
...
account dello stesso mint.

I trasferimenti non modificano la fornitura del mint. I trasferimenti aggiornano
solo i saldi tra token account.
...
di fornire il mint e i decimali affinché l'istruzione possa verificare il mint
previsto e la precisione del token prima di spostare i token tra account.

Nel Token Extensions Program, _rs`Transfer`_ è deprecata in favore di
_rs`TransferChecked`_ o _rs`TransferCheckedWithFee`_.

Il proprietario del token account di origine o un delegato approvato firma il
trasferimento. Nel Token Extensions Program, un delegato permanente del mint può
anche autorizzare il trasferimento se il mint ha abilitato l'estensione del
delegato permanente.
...
| _rs`Transfer`_         | Un'istruzione di trasferimento che sposta i token tra token account senza richiedere al chiamante di fornire il mint o i decimali. Nel Token Extensions Program, utilizza invece _rs`TransferChecked`_. | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Un'istruzione di trasferimento che richiede al chiamante di fornire il mint e i decimali e verifica tali valori prima di spostare i token tra token account dello stesso mint.                          | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | Logica di elaborazione condivisa per i trasferimenti di token.                                                                                                                                          | [Sorgente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |
