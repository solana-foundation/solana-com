...
chi ha l'autorità sul token account. Questo differisce dal program owner
specificato nel tipo base [Account](/docs/core/accounts#account-type), che è il
Token Program per tutti i token account.
...
per eseguire i comandi CLI direttamente nel browser senza installazione locale.
...
_sh`solana airdrop`_ nel terminale Playground. Puoi anche ottenere SOL devnet
...
dagli esempi seguenti. Usa gli indirizzi del tuo terminale Playground quando
segui le istruzioni. Ad esempio, il comando _sh`create-token`_ restituisce un
...
L'istruzione _rs`MintTo`_ sul Token Program crea nuovi token. L'autorità di mint
deve firmare la transazione. L'istruzione conia token verso un token account e
aumenta l'offerta totale sul mint account. Ecco un esempio Javascript su
...
Il proprietario del token account mittente deve firmare la transazione.
L'istruzione sposta token tra token account. Ecco un esempio Javascript su
