...
- I [Token Program](#token-programs) contengono tutta la logica delle istruzioni
...
effettivamente [data account](/docs/core/accounts/account-types#data-accounts)
di proprietà di un Token Program.
...
programma specificato nel tipo base
[Account](/docs/core/accounts/account-structure#account-fields), che è il Token
Program per tutti i token account.
...
Esistono due modi per aggiungere metadati (nome, simbolo, immagine) ai token su
Solana:

1. **[Metaplex Token Metadata](/docs/tokens/metaplex)** - Crea un account di
   metadati separato collegato al tuo mint. Funziona con il Token Program
   originale e Token-2022 ed è ampiamente supportato da wallet e marketplace.

2. **[Token Extensions Metadata](/docs/tokens/extensions/metadata)** - Memorizza
   i metadati direttamente sul mint account utilizzando Token-2022. Un approccio
   più recente con supporto integrato. L'estensione metadata è disponibile solo
   per i mint account creati con Token-2022. Per aggiungere metadati ai mint
   account creati con il Token Program originale, utilizza il Metaplex Token
   Metadata Program.

#### Esempio di SPL Token CLI

Il Token Extensions Program ti permette di memorizzare i metadati direttamente
sul Mint Account. Per creare un token con l'estensione metadata:
...
Scopri di più nella
[sezione Token Extensions](/docs/tokens/extensions/metadata). Per dettagli su
Token Extensions, consulta la
[sezione Token Extensions](/docs/tokens/extensions) e la
