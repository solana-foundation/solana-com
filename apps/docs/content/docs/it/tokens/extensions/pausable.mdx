...
title: Pausable mint
description:
  Crea un mint Token-2022 con un'autorità di pausa, blocca i trasferimenti
  mentre il mint è in pausa e riprendi l'attività in seguito.
...
## Cos'è un Mint Sospendibile?

L'estensione mint _rs`PausableConfig`_ del Token Extensions Program conferisce a
un'autorità di pausa la capacità di sospendere e riprendere l'attività dei token
per l'intero mint.

Quando un mint è in pausa, il Token Extensions Program rifiuta:

- Trasferimenti
- Coniazioni
- Bruciature

Quando il mint viene ripreso, queste istruzioni funzionano nuovamente in modo
normale.

## Come Creare, Sospendere e Riprendere un Mint

Per creare, sospendere e riprendere un mint:

1. Calcola le dimensioni del mint account e il rent necessario per il mint e
   l'estensione _rs`PausableConfig`_.
2. Crea il mint account con _rs`CreateAccount`_, inizializza
   _rs`PausableConfig`_ e inizializza il mint con _rs`InitializeMint`_.
3. Conia token e crea token account per il mint.
4. Usa _rs`Pause`_ per bloccare l'attività dei token per il mint.
5. Usa _rs`Resume`_ per consentire nuovamente l'attività dei token.

<ScrollyCoding>

## !!steps Calcola le dimensioni dell'account

Calcola le dimensioni del mint account per il mint base più l'estensione
_rs`PausableConfig`_. Questa è la dimensione utilizzata in _rs`CreateAccount`_.
...
## !!steps Inizializzare PausableConfig

Inizializza l'estensione _rs`PausableConfig`_ sul mint.
...
## !!steps Mettere in pausa il mint

Metti in pausa il mint con _rs`Pause`_.
...
## !!steps Riprendere il mint

Riprendi il mint con _rs`Resume`_.
...
<Callout type="info" title="Ordine delle istruzioni">
  _rs`PausableInstruction::Initialize`_ deve precedere _rs`InitializeMint`_.
  _rs`CreateAccount`_, _rs`PausableInstruction::Initialize`_ e
  _rs`InitializeMint`_ devono essere incluse nella stessa transazione.
</Callout>
...
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PausableConfig`_                  | Estensione del mint che memorizza l'autorità di pausa e se il mint è attualmente in pausa.                                | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/mod.rs#L17-L22)         |
| _rs`PausableInstruction::Initialize`_ | Istruzione che inizializza la configurazione pausabile prima di _rs`InitializeMint`_.                                     | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L20-L32) |
| _rs`PausableInstruction::Pause`_      | Istruzione che contrassegna il mint come in pausa.                                                                        | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L33-L44) |
| _rs`PausableInstruction::Resume`_     | Istruzione che contrassegna il mint come non in pausa.                                                                    | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L45-L56) |
| _rs`process_initialize`_              | Logica di elaborazione che inizializza _rs`PausableConfig`_ su un mint non inizializzato e memorizza l'autorità di pausa. | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L22-L35)     |
| _rs`process_toggle_pause`_            | Logica di elaborazione che verifica l'autorità di pausa prima di attivare/disattivare il flag di pausa del mint.          | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L39-L64)     |
