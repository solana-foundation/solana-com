...
description:
  Crea un mint che memorizza i metadati direttamente sull'account mint utilizzando le estensioni MetadataPointer e TokenMetadata, quindi aggiorna, rimuovi e blocca tali metadati tramite l'interfaccia token-metadata.
...
L'esempio `Kit` di seguito utilizza direttamente le istruzioni generate. Gli esempi legacy che utilizzano `@solana/web3.js`, `@solana/spl-token` e
`@solana/spl-token-metadata` sono inclusi come riferimento. Gli esempi legacy impostano
`maxSupportedTransactionVersion` a `0` perché web3.js v1 non è in grado di deserializzare
[le transazioni v1](/docs/core/transactions/versioned-transactions#v1-message-format).
