...
description:
  Crea un mint con commissioni sui trasferimenti, sposta le commissioni
  trattenute dai token account al mint e preleva le commissioni verso un token
  account ricevitore.
...
L'estensione mint _rs`TransferFeeConfig`_ del Token Extensions Program applica
...
- Le commissioni trattenute possono essere prelevate direttamente dai token
  account o raccolte nel mint
- Il `withdraw_withheld_authority` può spostare le commissioni raccolte verso un
  token account ricevitore delle commissioni

<Callout type="info" title="Estensioni del Token Account">
  Quando un token account viene inizializzato per un mint con
  _rs`TransferFeeConfig`_, il token account viene inizializzato con
  _rs`TransferFeeAmount`_. Quando il token account viene creato tramite
  l'[Associated Token
...
1. Calcola la dimensione del mint account e il rent necessari per il mint e
   l'estensione _rs`TransferFeeConfig`_.
2. Crea il mint account con _rs`CreateAccount`_, inizializza
   _rs`TransferFeeConfig`_ e inizializza il mint con _rs`InitializeMint`_.
3. Crea token account per il mint. _rs`TransferFeeAmount`_ viene abilitato
   automaticamente per i token account.
4. Usa _rs`TransferCheckedWithFee`_ per trasferire token e verificare la
   commissione prevista. _rs`TransferChecked`_ funziona anche e trattiene
   automaticamente la commissione configurata sul token account di destinazione.
5. Usa _rs`WithdrawWithheldTokensFromAccounts`_ per spostare le commissioni
   trattenute direttamente dai token account a un token account ricevitore delle
   commissioni, firmato dal `withdraw_withheld_authority` del mint.
6. Usa _rs`HarvestWithheldTokensToMint`_ senza permessi per spostare le
   commissioni trattenute dai token account al mint account, poi usa
   _rs`WithdrawWithheldTokensFromMint`_ per prelevare le commissioni trattenute
   del mint account verso un token account ricevitore delle commissioni, firmato
   dal `withdraw_withheld_authority` del mint.
7. Usa _rs`SetTransferFee`_ per aggiornare la prossima configurazione delle
   commissioni sui trasferimenti, che entrerà in vigore a partire da due epoch
   successivi.
...
_rs`TransferFeeConfig`_. Questa è la dimensione utilizzata in
_rs`CreateAccount`_.
...
## !!steps Inizializzare TransferFeeConfig

Inizializza l'estensione _rs`TransferFeeConfig`_ sul mint.
...
## !!steps Creare token account e coniare token

Crea token account per la sorgente, i destinatari e il ricevente delle
commissioni, quindi conia token sul token account sorgente.
...
## !!steps Trasferimento con TransferCheckedWithFee

Trasferisci token con _rs`TransferCheckedWithFee`_ e verifica la commissione
prevista.
...
## !!steps Trasferimento con TransferChecked

Trasferisci token con _rs`TransferChecked`_. La commissione configurata viene
...
## !!steps Prelievo delle commissioni trattenute dai token account

Usa _rs`WithdrawWithheldTokensFromAccounts`_ per spostare le commissioni
...
## !!steps Raccolta delle commissioni trattenute nel mint

Usa _rs`HarvestWithheldTokensToMint`_ per spostare le commissioni trattenute dai
...
## !!steps Prelievo delle commissioni trattenute dal mint

Usa _rs`WithdrawWithheldTokensFromMint`_ per spostare le commissioni raccolte
...
## !!steps Aggiornamento della prossima commissione di trasferimento

Usa _rs`SetTransferFee`_ per aggiornare la configurazione della prossima
...
<Callout type="info" title="Ordine delle Istruzioni">
  _rs`InitializeTransferFeeConfig`_ deve precedere _rs`InitializeMint`_.
...
| ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`TransferFeeConfig`_                                          | Estensione del mint che memorizza le autorità delle commissioni, l'importo trattenuto accumulato e le configurazioni delle commissioni di trasferimento precedenti e successive.                                                            | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L128-L145)         |
| _rs`TransferFee`_                                                | Struttura di configurazione delle commissioni memorizzata in _rs`TransferFeeConfig`_ che definisce l'epoca in cui la commissione diventa effettiva, la commissione massima e la commissione in punti base.                                  | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L32-L40)           |
| _rs`TransferFeeAmount`_                                          | Estensione del token account che memorizza l'importo trattenuto durante i trasferimenti per un token account.                                                                                                                               | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L169-L177)         |
| _rs`TransferFeeInstruction::InitializeTransferFeeConfig`_        | Istruzione che inizializza le impostazioni delle commissioni di trasferimento prima di _rs`InitializeMint`_.                                                                                                                                | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L24-L48)   |
| _rs`TransferFeeInstruction::TransferCheckedWithFee`_             | Istruzione di trasferimento che verifica che la commissione fornita dal chiamante corrisponda alla configurazione delle commissioni di trasferimento corrente del mint.                                                                     | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L49-L80)   |
| _rs`TransferFeeInstruction::WithdrawWithheldTokensFromAccounts`_ | Istruzione che sposta le commissioni trattenute direttamente dai token account a un token account destinatario delle commissioni, firmata dall'`withdraw_withheld_authority` del mint.                                                      | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L99-L122)  |
| _rs`TransferFeeInstruction::HarvestWithheldTokensToMint`_        | Istruzione permissionless che sposta le commissioni trattenute dai token account all'accumulatore di commissioni trattenute del mint account.                                                                                               | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L123-L134) |
| _rs`TransferFeeInstruction::WithdrawWithheldTokensFromMint`_     | Istruzione che preleva le commissioni trattenute del mint account verso un token account destinatario delle commissioni, firmata dall'`withdraw_withheld_authority` del mint.                                                               | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L81-L98)   |
| _rs`TransferFeeInstruction::SetTransferFee`_                     | Istruzione che aggiorna la configurazione delle commissioni di trasferimento più recente, che diventa effettiva a partire da due epoche successive.                                                                                         | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L135-L154) |
| _rs`process_initialize_transfer_fee_config`_                     | Logica del processore che inizializza l'estensione _rs`TransferFeeConfig`_ su un mint non inizializzato e imposta sia le configurazioni delle commissioni di trasferimento precedenti che successive alla commissione iniziale.             | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L26-L58)       |
| _rs`process_withdraw_withheld_tokens_from_accounts`_             | Logica del processore che convalida l'`withdraw_withheld_authority` e sposta le commissioni trattenute dai token account a un token account di destinazione.                                                                                | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L208-L278)     |
| _rs`process_harvest_withheld_tokens_to_mint`_                    | Logica del processore che raccoglie le commissioni trattenute dai token account nell'accumulatore di commissioni trattenute del mint account.                                                                                               | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L181-L206)     |
| _rs`process_withdraw_withheld_tokens_from_mint`_                 | Logica del processore che convalida l'`withdraw_withheld_authority` e sposta l'importo trattenuto del mint account a un token account di destinazione.                                                                                      | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L114-L158)     |
| _rs`process_set_transfer_fee`_                                   | Logica del processore che convalida l'autorità di configurazione delle commissioni di trasferimento e aggiorna la configurazione delle commissioni di trasferimento più recente per diventare effettiva a partire da due epoche successive. | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L61-L112)      |
| _rs`get_required_init_account_extensions`_                       | Utilizzato per aggiungere automaticamente le estensioni del token account quando i token account vengono inizializzati in base alle estensioni abilitate sul mint. Per i mint _rs`TransferFeeConfig`_, aggiunge _rs`TransferFeeAmount`_.    | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mod.rs#L1301-L1303)                    |
