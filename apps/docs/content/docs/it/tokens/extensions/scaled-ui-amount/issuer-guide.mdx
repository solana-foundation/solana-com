---
title: Guida per emittenti Scaled UI Amount
description:
...
Per utilizzare l'estensione Scaled UI Amount, devi abilitarla su un mint di
token o su un account token. Nota che una volta creato un token, non puoi
modificare quali estensioni sono abilitate.

### Abilitare l'estensione Scaled UI Amount su un mint di token

Per abilitare l'estensione Scaled UI Amount su un mint di token, devi impostare
il campo `scaled_ui_amount_extension` su `true` nell'account `Mint`. Ecco un
...
facoltativo e può essere utilizzato per impostare un orario di inizio
personalizzato per il nuovo moltiplicatore. Se non viene fornito alcun
timestamp, verrà utilizzato il timestamp corrente.

**Nota importante:** attualmente esiste una particolarità nel processo di
aggiornamento del moltiplicatore scaled ui. Al momento, se esegui le seguenti
operazioni:

1. Imposti un moltiplicatore scaled ui per il futuro
2. Il tempo passa oltre questo orario di aggiornamento
3. Imposti un altro moltiplicatore scaled ui per il futuro
4. Allora il precedente moltiplicatore scaled ui viene sovrascritto da quello
   nuovo, rimuovendolo di fatto.
...
attuale invece di essere sovrascritto, ma non è ancora attiva.

Fino a quel momento è necessario effettuare 2 aggiornamenti del moltiplicatore
durante il passaggio 3 sopra:

1. Imposta nuovamente il moltiplicatore precedente con il timestamp allo stesso
   timestamp originariamente impostato nel passaggio 1 (questo imposterà
   correttamente il moltiplicatore come moltiplicatore attuale)
2. Imposta il nuovo moltiplicatore come descritto nel passaggio 3 sopra
