...
- Gli utenti dovrebbero inserire importi da interpretare come “UIAmount”
  ...
  - Se ci sono problemi di arrotondamento, arrotonda per difetto e preferisci
    lasciare una piccola quantità di residuo piuttosto che rischiare il
...
- Il moltiplicatore corrente può essere letto dal token mint in qualsiasi
  momento chiamando `getAccountInfo`. Inoltre, se è impostato un moltiplicatore
  futuro, questa informazione è disponibile anche dal token mint. Consigliamo di
  non mostrare questo moltiplicatore poiché può confondere l'UX.
