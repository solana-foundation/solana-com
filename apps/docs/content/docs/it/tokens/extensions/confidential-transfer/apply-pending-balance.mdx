...
Per convertire un saldo in sospeso in un saldo disponibile, invoca l'istruzione
...
La crate `spl_token_client` fornisce un metodo
...
L'esempio seguente dimostra come applicare il saldo confidenziale in sospeso al
saldo confidenziale disponibile.

Per eseguire l'esempio, avviare un validator locale con il Token Extension
Program clonato da mainnet utilizzando il seguente comando. È necessario avere
installato la Solana CLI per avviare il validator locale.
...
<Callout type="info">
  ...
  da mainnet per eseguire il codice di esempio.
</Callout>
