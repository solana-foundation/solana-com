---
title: Applicare il saldo in sospeso
description:
  Scopri come applicare il saldo in sospeso per rendere i fondi disponibili.
---
...
2. **Saldo Confidenziale Disponibile**: Il saldo in sospeso viene quindi
   "applicato" al saldo disponibile, rendendo i token disponibili per i
   trasferimenti confidenziali.

Questa sezione spiega la seconda fase: l'applicazione del saldo in sospeso al
saldo disponibile.
...
initialmente aggiunti al saldo confidenziale in sospeso. Prima che i token
possano essere utilizzati per i trasferimenti confidenziali, il saldo in sospeso
...
Il seguente diagramma mostra i passaggi necessari per applicare il saldo in
...
transazione con l'istruzione `ApplyPendingBalance`, come mostrato nell'esempio
di seguito.

## Codice di Esempio

Il seguente esempio mostra come applicare il saldo confidenziale in sospeso al
...
abilita, ma un validator locale con fork della mainnet come
[Surfpool](https://surfpool.run) lo fa. Esegui l'esempio su uno di questi (il
codice utilizza devnet) con un payer finanziato, e sostituisci gli indirizzi
segnaposto di mint e account con i tuoi.
