...
1. **Saldo in Sospeso Confidenziale**: Inizialmente, i token vengono
   "depositati" dal saldo pubblico a un saldo confidenziale "in sospeso".
2. **Saldo Disponibile Confidenziale**: Il saldo in sospeso viene quindi
   "applicato" al saldo disponibile, rendendo i token disponibili per i
   trasferimenti confidenziali.

Questa sezione spiega la prima fase: il deposito del saldo pubblico dei token
nel saldo in sospeso confidenziale.

Il seguente diagramma mostra i passaggi necessari per depositare token dal saldo
pubblico al saldo in sospeso confidenziale:
...
Per convertire un saldo pubblico in un saldo in sospeso confidenziale, invocare
l' istruzione
...
Il seguente esempio mostra come depositare il saldo pubblico dei token nel saldo
in sospeso confidenziale.

Confidential Balances dipende dal programma ZK ElGamal Proof, che è abilitato
su mainnet e devnet. Un `solana-test-validator` standard non lo abilita, ma un
validator locale con fork dalla mainnet come [Surfpool](https://surfpool.run) sì.
Esegui l'esempio su uno di questi (il codice usa devnet) con un payer finanziato,
e sostituisci il mint e gli indirizzi di account segnaposto con i tuoi.
...
fn main() -> Result<()> {
    ...
    // account for Confidential Balances (see "Create a Token Account").
