...
I trasferimenti confidenziali dipendono dal programma ZK ElGamal Proof,
abilitato su mainnet e devnet. Un `solana-test-validator` standard non lo
abilita, ma un validator locale con fork di mainnet come
[Surfpool](https://surfpool.run) sì. Eseguire l'esempio su uno di questi
ambienti (il codice utilizza devnet) con un payer finanziato, e sostituire gli
indirizzi segnaposto di mint e account con i propri.
