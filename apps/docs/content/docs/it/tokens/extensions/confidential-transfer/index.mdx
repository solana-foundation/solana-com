...
<Callout type="warn">
  Il programma ZK ElGamal è temporaneamente disabilitato sulla mainnet e sulla
  devnet mentre è in corso un audit di sicurezza. Ciò significa che l'estensione
...
## Cosa sono i Confidential Transfer?

I confidential transfer ti consentono di trasferire token tra token account
senza rivelare l'importo del trasferimento. Questo è utile per transazioni che
preservano la privacy. Solo gli importi dei trasferimenti e i saldi dei token
sono privati. Gli indirizzi dei token account rimangono pubblici.
...
al programma Token Extension che ti consentono di trasferire token tra account
...
Il flusso base dei confidential token transfer è il seguente:

1. Creare un mint account con l'estensione confidential transfer.
2. Creare token account con estensione confidential transfer per il mittente e
   il destinatario.
3. Mintare token sul token account del mittente.
4. **Depositare** il saldo pubblico del mittente nel **saldo confidenziale in
   sospeso**.
5. **Applicare** il saldo in sospeso del mittente al **saldo confidenziale
   disponibile**.
6. **Trasferire** in modo confidenziale i token dal token account del mittente
   al token account del destinatario.
7. **Applicare** il saldo in sospeso del destinatario al **saldo confidenziale
   disponibile**.
8. **Prelevare** il saldo confidenziale disponibile del destinatario nel **saldo
   pubblico**.

Per maggiori dettagli sui passaggi del flusso di trasferimento confidenziale,
...
<Cards>
  ...
  >
    Come depositare token nel saldo in sospeso riservato
  </Card>
  <Card
    title="Applica Saldo in Sospeso"
    href="/docs/tokens/extensions/confidential-transfer/apply-pending-balance"
  >
    Come applicare il saldo in sospeso al saldo riservato disponibile
  </Card>
  <Card
    title="Preleva Token"
    href="/docs/tokens/extensions/confidential-transfer/withdraw-tokens"
  >
    Come prelevare token dal saldo riservato disponibile
  </Card>
  <Card
  ...
  >
    ...
    approvazione, revisori, commissioni, conio e combustione)
  </Card>
</Cards>
...
L'esempio Rust seguente mostra i passaggi per i trasferimenti confidenziali di
token corrispondenti al diagramma precedente. I passaggi sono separati in
funzioni helper per maggiore chiarezza.

Per eseguire l'esempio, avvia un validator locale con il Token Extension Program
clonato da mainnet usando il seguente comando. Devi avere la Solana CLI
installata per usare il validator locale.
...
<Callout type="info">
  Al momento della scrittura, i Confidential Transfer non sono abilitati sul
  validator locale predefinito. Devi clonare il Token Extension Program di
...
## Istruzioni Confidential Transfer
...
| _rs`EmptyAccount`_                  | Svuota i saldi confidenziali in sospeso e disponibili per consentire la chiusura di un token account.                                                                               |
| _rs`Deposit`_                       | Converte il saldo pubblico dei token in saldo confidenziale in sospeso.                                                                                                             |
...
| _rs`DisableConfidentialCredits`_    | Blocca i trasferimenti confidenziali in entrata consentendo comunque i trasferimenti pubblici.                                                                                      |
| _rs`EnableNonConfidentialCredits`_  | Consente a un token account di ricevere trasferimenti pubblici di token.                                                                                                            |
| _rs`DisableNonConfidentialCredits`_ | Blocca i trasferimenti regolari per fare in modo che l'account riceva solo trasferimenti confidenziali.                                                                             |
| _rs`TransferWithFee`_               | Trasferisce token tra token account in modo confidenziale con una commissione.                                                                                                      |
| _rs`ConfigureAccountWithRegistry`_  | Modo alternativo per configurare i token account per i trasferimenti confidenziali usando un account _rs`ElGamalRegistry`_ invece di una proof _rs`VerifyPubkeyValidity`_.          |
