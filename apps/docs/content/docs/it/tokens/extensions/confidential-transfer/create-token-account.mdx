...
<Callout type="warn">
  Il programma ZK ElGamal è temporaneamente disabilitato su mainnet e devnet in
  quanto è in corso un audit di sicurezza. Ciò significa che l'estensione
  confidential transfers non è attualmente disponibile. Sebbene i concetti siano
  ancora validi, gli esempi di codice non funzioneranno.
</Callout>

## Come creare un token account con l'estensione Confidential Transfer

L'estensione Confidential Transfer abilita trasferimenti di token privati
aggiungendo uno stato extra al token account. Questa sezione spiega come creare
un token account con questa estensione abilitata.

Il seguente diagramma mostra i passaggi coinvolti nella creazione di un token
account con l'estensione Confidential Transfer:
...
### Stato del Confidential Transfer Token Account
...
- **approved**: lo stato di approvazione dell'account per i confidential
  transfers. Se la configurazione `auto_approve_new_accounts` del mint account è
  impostata come `true`, tutti i token account sono automaticamente approvati
  per i confidential transfers.

- **elgamal_pubkey**: la chiave pubblica ElGamal utilizzata per crittografare i
  saldi e gli importi dei trasferimenti.

- **pending_balance_lo**: i 16 bit inferiori crittografati del saldo in sospeso.
  Il saldo è diviso in parti alte e basse per una decrittazione efficiente.

- **pending_balance_hi**: i 48 bit superiori crittografati del saldo in sospeso.
  Il saldo è diviso in parti alte e basse per una decrittazione efficiente.

- **available_balance**: il saldo crittografato disponibile per i trasferimenti.

- **decryptable_available_balance**: il saldo disponibile crittografato con una
  chiave Advanced Encryption Standard (AES) per una decrittazione efficiente da
  parte del proprietario dell'account.

- **allow_confidential_credits**: se true, consente i trasferimenti
  confidenziali in entrata.

- **allow_non_confidential_credits**: se true, consente i trasferimenti non
  confidenziali in entrata.

- **pending_balance_credit_counter**: conta i crediti del saldo in sospeso in
  entrata dalle istruzioni di deposito e trasferimento.
...
- **expected_pending_balance_credit_counter**: il valore
  `pending_balance_credit_counter` fornito dal client tramite i dati
...
tra il momento in cui la transazione viene inviata e quando viene elaborata,
risultando in una transazione fallita.
...
_rs`ApplyPendingBalance`_ per convertire il saldo in sospeso nel saldo
disponibile. I trasferimenti o depositi in entrata non influenzano il saldo
disponibile di un token account.

</Callout>

<Callout type="info">
  **Divisione alto/basso del saldo in sospeso**

Il saldo in sospeso confidenziale è diviso in `pending_balance_lo` e
`pending_balance_hi` perché la decrittazione ElGamal richiede più calcoli per
...
in sospeso nel saldo disponibile:

1. Il client cerca i saldi in sospeso e disponibili correnti, cripta la somma e
   fornisce un `decryptable_available_balance` criptato utilizzando la chiave
   AES del proprietario del token account.

2. I contatori di credito in sospeso previsti ed effettivi tracciano le
   modifiche al valore del contatore tra il momento in cui l'istruzione
   _rs`ApplyPendingBalance`_ viene creata e processata:
   - `expected_pending_balance_credit_counter`: Il valore
   ...
   - `actual_pending_balance_credit_counter`: Il valore
     ...
     l'istruzione _rs`ApplyPendingBalance`_ viene processata
...
richiedono al client di cercare le recenti istruzioni di deposito/trasferimento
corrispondenti alla differenza del contatore per calcolare il saldo corretto.

**Processo di riconciliazione del saldo**

Quando i contatori del saldo in sospeso previsti ed effettivi differiscono,
segui questi passaggi per riconciliare il `decryptable_available_balance`:

1. Inizia con il `decryptable_available_balance` dal token account
2. Recupera le transazioni più recenti includendo le istruzioni di deposito e
   ...
   - Decripta e aggiungi gli importi ciphertext di destinazione dalle istruzioni
     di trasferimento
...
1. **Creare il token account**: Invoca l'istruzione
...
3. **Configura i trasferimenti confidenziali**: invoca l'istruzione
   ...
   del Token Extension Program per inizializzare lo stato
