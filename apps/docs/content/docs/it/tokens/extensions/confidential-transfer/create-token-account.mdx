...
description:
  Scopri come creare un token account con i Saldi Riservati abilitati.
---

## Come creare un token account con i Saldi Riservati

I Saldi Riservati consentono trasferimenti di token privati aggiungendo uno stato extra al
token account. Questa sezione spiega come creare un token account con
l'estensione richiesta abilitata.

Il diagramma seguente mostra i passaggi necessari per creare un token account con
i Saldi Riservati abilitati:
...
### Stato del Token Account per i Saldi Riservati
...
Il _rs`ConfidentialTransferAccount`_ contiene diversi campi che gestiscono
i Saldi Riservati:

- **approvato**: Lo stato di approvazione dell'account per i Saldi Riservati. Se la
  configurazione `auto_approve_new_accounts` del mint account è impostata su `true`, tutti
  i token account vengono approvati automaticamente per i Saldi Riservati.
...
La creazione e la configurazione di un token account per i Saldi Riservati utilizza le
seguenti istruzioni, che rientrano tutte in una singola transazione:
...
4. **Configura i Saldi Riservati**: Richiama l'istruzione
   ...
   facendo riferimento all'account di stato del contesto della prova tramite
   _rs`ProofLocation::ContextStateAccount`_, per inizializzare lo
   stato _rs`ConfidentialTransferAccount`_.

<Callout type="info">

**Solo il proprietario del token account può abilitare i Saldi Riservati su un token
account**.

L'istruzione _rs`ConfigureAccount`_ richiede la generazione lato client di
chiavi di cifratura e una prova che può essere generata solo dal proprietario del token account.

La prova di validità del pubkey verifica che la chiave pubblica ElGamal dell'account sia
valida. Viene generata con _rs`build_pubkey_validity_proof_data`_, verificata on
chain dal programma ZK ElGamal Proof in un account di stato del contesto, e poi
referenziata da _rs`ConfigureAccount`_ tramite
_rs`ProofLocation::ContextStateAccount`_, in modo che nessun byte di prova transiti nell'istruzione
del token stessa. Per i dettagli di implementazione, consulta:
...
Il codice seguente crea un associated token account e lo configura per utilizzare
i Saldi Riservati con un mint compatibile già esistente.

I Saldi Riservati dipendono dal programma ZK ElGamal Proof, che è abilitato
su mainnet e devnet. Un `solana-test-validator` standard non lo abilita, ma un
validator locale con fork della mainnet come [Surfpool](https://surfpool.run) sì.
Esegui l'esempio su uno di questi (il codice utilizza devnet) con un payer finanziato,
e sostituisci il segnaposto del mint con un mint creato seguendo
