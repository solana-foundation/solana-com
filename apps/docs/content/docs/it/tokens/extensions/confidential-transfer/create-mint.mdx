...
<Callout type="warn">
  ...
  mentre è in corso un audit di sicurezza. Ciò significa che l'estensione
  confidential transfers è attualmente non disponibile. Sebbene i concetti siano
...
Il seguente diagramma mostra i passaggi coinvolti nella creazione di un mint con
...
- **authority**: L'account che ha il permesso di modificare le impostazioni di
  confidential transfer per il mint e approvare nuovi confidential account se
  l'approvazione automatica è disabilitata.

- **auto_approve_new_accounts**: Quando impostato su true, gli utenti possono
  creare token account con confidential transfer abilitati di default. Quando
  false, l'authority deve approvare ogni nuovo token account prima che possa
  essere utilizzato per confidential transfer.
...
- **auditor_elgamal_pubkey**: Un auditor opzionale che può decifrare gli importi
  ...
  conformità mantenendo la privacy dal pubblico generale.

### Istruzioni richieste

Creare un mint con Confidential Transfer abilitato richiede tre istruzioni in
una singola transazione:

1. **Crea il mint account**: Invoca l'istruzione _rs`CreateAccount`_ del System
   Program per creare il mint account.

2. **Inizializza l'estensione Confidential Transfer**: Invoca l'istruzione
   ...
   del Token Extension Program per configurare lo stato
   _rs`ConfidentialTransferMint`_ per il mint.

3. **Inizializza il mint**: Invoca l'istruzione
   _rs`Instruction::InitializeMint`_ del Token Extension Program per
   inizializzare lo stato standard del mint.

Sebbene tu possa scrivere queste istruzioni manualmente, il crate
...
clonato dalla mainnet utilizzando il seguente comando. Devi avere la Solana CLI
