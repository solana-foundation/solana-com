...
Per prelevare token dal saldo disponibile riservato al saldo pubblico:
...
1. Crea
   ...
   **Prova di uguaglianza
   ...
   prelievo corrisponda al relativo
   [impegno di Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme),
   garantendo che il nuovo saldo disponibile dell'account sia calcolato
   ...
   **Prova di intervallo
   ...
   e rientri in un intervallo specificato.

2. Per ogni prova:
   - Invoca il programma ZK ElGamal proof per verificare i dati della prova.
...
3. Invoca l'istruzione
   ...
   fornendo i due account delle prove.

4. Chiudi gli account delle prove per recuperare i SOL utilizzati per crearli.

Il seguente diagramma mostra i passaggi coinvolti nel prelievo di token dal
saldo disponibile riservato al saldo pubblico:
...
Per prelevare token dal saldo disponibile riservato al saldo pubblico, è
necessario:
...
- Invocare l'istruzione
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  fornendo i due account delle prove.
- Chiudere i due account delle prove per recuperare il rent.

L'esempio Rust di seguito genera le prove con il crate
`spl-token-confidential-transfer-proof-generation`, le verifica ciascuna in un
account context state tramite il programma ZK ElGamal Proof, fa riferimento a
entrambi gli account nell'istruzione di prelievo e li chiude successivamente.
L'esempio TypeScript utilizza l'helper `getConfidentialWithdrawInstructionPlan`
da `@solana-program/token-2022/confidential`, che assembla gli stessi account
delle prove, il prelievo e la chiusura come piano di istruzioni
multi-transazione.

## Codice di Esempio

Il seguente esempio preleva i token dal saldo riservato disponibile
tornando al saldo pubblico. L'account deve essere già configurato per
i Saldi Riservati e disporre di un saldo riservato disponibile.
...
Esegui l'esempio con uno di questi (il codice usa devnet) con un payer finanziato,
e sostituisci i segnaposto con il tuo mint e token account.
