...
<Callout type="warn">
  ...
  dei trasferimenti confidenziali è attualmente non disponibile. Sebbene i
  concetti siano ancora validi, gli esempi di codice non funzioneranno.
</Callout>

## Come prelevare token dal saldo disponibile confidenziale

Per prelevare token dal saldo disponibile confidenziale al saldo pubblico:
...
1. Crea
   ...
   **Prova di uguaglianza
   ...
   Verifica che il ciphertext del saldo disponibile rimanente dopo il prelievo
   corrisponda al suo
...
4. Chiudi gli account di prova per recuperare i SOL utilizzati per crearli.

Il seguente diagramma mostra i passaggi coinvolti nel prelievo di token dal
saldo disponibile confidenziale al saldo pubblico:
...
Per prelevare token dal saldo disponibile confidenziale al saldo pubblico, devi:
...
- Il metodo `confidential_transfer_create_context_state_account` che crea un
  account di prova.
- Il metodo `confidential_transfer_withdraw` che invoca l'istruzione `Withdraw`.
...
clonato da mainnet utilizzando il seguente comando. Devi avere la Solana CLI
...
<Callout type="info">
  Al momento della scrittura, i Confidential Transfers non sono abilitati sul
