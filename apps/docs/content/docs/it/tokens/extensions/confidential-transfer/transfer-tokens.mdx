...
stato _rs`ConfidentialTransferAccount`_ e approvati per i trasferimenti
confidenziali. Il token account del mittente deve inoltre disporre di un saldo
confidenziale disponibile da cui effettuare il trasferimento.
...
L'esempio seguente trasferisce token in modo riservato da un account a un altro.
Entrambi gli account devono essere già configurati per i trasferimenti riservati
e il mittente deve disporre di un saldo riservato disponibile.
...
sì. Esegui l'esempio su uno di questi (il codice usa devnet) con un payer
finanziato e sostituisci i segnaposto con il tuo mint e gli account del mittente
e del destinatario.
