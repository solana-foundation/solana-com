...
## Come trasferire token in modo confidenziale da un account token a un altro

Per trasferire token in modo confidenziale da un account token a un altro, sia
il mittente che il destinatario devono avere account token configurati con lo
stato _rs`ConfidentialTransferAccount`_ e approvati per i trasferimenti
confidenziali. L'account token del mittente deve inoltre avere un saldo
confidenziale disponibile da trasferire.
...
1. Crea
   ...
   **Prova di uguaglianza
   ...
   [commitment di Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme),
   garantendo che il nuovo saldo disponibile dell'account sorgente sia calcolato
   correttamente come `new_balance = current_balance - transfer_amount`.
   ...
   **Prova di validità del testo cifrato
   ...
   garantendo che l'importo del trasferimento sia crittografato correttamente
   con la chiave pubblica di ciascuna parte.
...
3. Invoca l'istruzione
   ...
   fornendo gli account context state della prova.

4. Chiudi gli account di stato del contesto di prova per recuperare i SOL
...
- Invocare il programma di prova Zk ElGamal per verificare le prove e
  inizializzare gli account di "stato del contesto"
...
- Il metodo `confidential_transfer_create_context_state_account` che crea un
  account di prova.
- Il metodo `confidential_transfer_transfer` che invoca l'istruzione `Transfer`.
- Il metodo `confidential_transfer_close_context_state_account` che chiude un
  account di prova.
...
Per eseguire l'esempio, avvia un validatore locale con il Token Extension
Program clonato da mainnet utilizzando il seguente comando. Devi avere la Solana
CLI installata per avviare il validatore locale.
...
<Callout type="info">
  Al momento della scrittura, i trasferimenti confidenziali non sono abilitati
  sul validatore locale predefinito. Devi clonare il Token Extension Program di
