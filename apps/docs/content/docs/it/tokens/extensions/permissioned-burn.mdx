...
Un validator di test locale include anche una build precedente di Token-2022,
quindi per eseguire gli esempi in questa pagina, carica il programma v11.0.0
dalla devnet nel tuo validator di test:
...
## Come Creare un Mint e Burn con Permesso

Per creare un mint con burn autorizzato e bruciare token:
...
5. Facoltativamente, disabilita il burning autorizzato con _rs`SetAuthority`_
   usando _rs`AuthorityType::PermissionedBurn`_ e un'autorità `None`, che
   riabilita le istruzioni di burn standard. Gli esempi di codice completi di
   seguito mostrano questo passaggio.
...
## !!steps Esegui il burn con l'autorità di burn

Esegui il burn dei token con _rs`PermissionedBurnInstruction::BurnChecked`_
firmato sia dal proprietario del token account che dall'autorità di burn.
...
<Callout type="info" title="Ordine delle Istruzioni">
  _rs`PermissionedBurnInstruction::Initialize`_ deve essere inserito prima di
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`PermissionedBurnConfig`_                   | Estensione del mint che memorizza l'autorità richiesta per co-firmare ogni burn per il mint.                                    | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
...
| _rs`process_set_authority`_                    | Logica del processore che convalida e ruota l'autorità di burn quando viene utilizzato _rs`AuthorityType::PermissionedBurn`_.   | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### Typescript

L'esempio `Kit` seguente utilizza direttamente le istruzioni generate. Il
pacchetto legacy pubblicato `@solana/spl-token` non include ancora il supporto
