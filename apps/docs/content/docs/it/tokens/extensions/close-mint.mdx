...
title: Chiusura Mint
description:
  Crea un mint del Token Extensions Program con l'estensione MintCloseAuthority,
  poi brucia la fornitura rimanente e chiudi il mint.
...
## Che cos'è la Mint Close Authority?

L'estensione _rs`MintCloseAuthority`_ del mint del Token Extensions Program
consente a un'autorità designata di chiudere un mint account e recuperare il suo
rent una volta che la fornitura totale del mint raggiunge lo zero.

Senza questa estensione, un mint account non può essere chiuso.

- La fornitura totale deve essere pari a zero, quindi tutti i token in tutti i
  token account devono essere bruciati prima che un mint possa essere chiuso
- Solo l'autorità di chiusura configurata può chiudere il mint

## Come creare e chiudere un mint

Per creare e chiudere un mint:

1. Calcola la dimensione del mint account e il rent necessario per il mint e
   l'estensione _rs`MintCloseAuthority`_.
2. Crea il mint account con _rs`CreateAccount`_, inizializza
   _rs`MintCloseAuthority`_, e inizializza il mint con _rs`InitializeMint`_.
3. Se la fornitura totale è zero, il mint account può essere chiuso con
   _rs`CloseAccount`_, firmato dall'autorità di chiusura.
...
_rs`MintCloseAuthority`_. Questa è la dimensione utilizzata in
_rs`CreateAccount`_.
...
## !!steps Inizializza MintCloseAuthority

Inizializza l'estensione _rs`MintCloseAuthority`_ sul mint.
...
<Callout type="info" title="Ordine delle istruzioni">
  _rs`InitializeMintCloseAuthority`_ deve precedere _rs`InitializeMint`_.
...
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`MintCloseAuthority`_                      | Estensione del mint che memorizza l'autorità autorizzata a chiudere un mint una volta che la sua offerta raggiunge zero. | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mint_close_authority.rs#L14-L20) |
| _rs`InitializeMintCloseAuthority`_            | Istruzione che inizializza l'autorità di chiusura del mint prima di _rs`InitializeMint`_.                                | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1830-L1843)                |
| _rs`CloseAccount`_                            | Istruzione token di base che chiude il mint account dopo che l'offerta totale raggiunge zero.                            | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1582-L1608)                |
| _rs`process_initialize_mint_close_authority`_ | Logica del processore che scrive l'estensione _rs`MintCloseAuthority`_ su un mint non inizializzato.                     | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1376-L1389)                    |
| _rs`process_close_account`_                   | Gestore condiviso di chiusura account che applica la regola dell'offerta zero per i mint con autorità di chiusura.       | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1288)                    |

### Typescript

L'esempio `Kit` di seguito utilizza direttamente le istruzioni generate. Esempi
