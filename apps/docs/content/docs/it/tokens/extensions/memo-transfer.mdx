...
description:
  Richiedi che i trasferimenti in entrata includano un memo abilitando
  l'estensione dell'account MemoTransfer su un account token del Token
  Extensions Program.
...
## Cos'è il Trasferimento con Memo?

L'estensione dell'account _rs`MemoTransfer`_ del Token Extensions Program
richiede un'istruzione memo immediatamente prima di qualsiasi trasferimento in
entrata verso quel token account. I trasferimenti in uscita da quel token
account non richiedono un memo.

L'estensione è configurata sul token account di destinazione, non sul mint. Una
volta abilitata, i trasferimenti senza memo falliscono con
_rs`TokenError::NoMemo`_ fino a quando il proprietario del token account non
disabilita il requisito.

## Come Creare un Token Account con MemoTransfer Abilitato

Per creare un token account con _rs`MemoTransfer`_ abilitato:

1. Crea e inizializza un mint.
2. Calcola la dimensione del token account e il rent necessario per un token
   account con _rs`MemoTransfer`_.
3. Crea il token account e abilita _rs`MemoTransfer`_ su quell'account.
4. Le istruzioni _rs`Transfer`_ e _rs`TransferChecked`_ verso quel token account
   devono includere un'istruzione memo immediatamente prima dell'istruzione di
   trasferimento.

<ScrollyCoding>

## !!steps Crea e inizializza il mint

Crea e inizializza il mint che il token account utilizzerà.
...
Calcola la dimensione del token account per il token account base più
l'estensione _rs`MemoTransfer`_. Questa è la dimensione utilizzata in
_rs`CreateAccount`_.
...
l'estensione _rs`MemoTransfer`_.
...
## !!steps Crea e inizializza il token account

Crea il token account con lo spazio e i lamport calcolati, quindi inizializzalo
per il mint.
...
## !!steps Abilita MemoTransfer

Abilita _rs`MemoTransfer`_ sul token account.
...
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`MemoTransfer`_                              | Estensione dell'account che memorizza se i trasferimenti in entrata richiedono un memo.                             | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/mod.rs#L17-L22)         |
| _rs`RequiredMemoTransfersInstruction::Enable`_  | Istruzione che abilita i trasferimenti con memo obbligatorio su un token account.                                   | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L50-L71) |
| _rs`RequiredMemoTransfersInstruction::Disable`_ | Istruzione che disabilita i trasferimenti con memo obbligatorio su un token account.                                | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L74-L95) |
| _rs`process_toggle_required_memo_transfers`_    | Logica del processore che inizializza o riscrive l'estensione _rs`MemoTransfer`_ dopo aver verificato la proprietà. | [Sorgente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/memo_transfer/processor.rs#L20-L48)     |

### Typescript

L'esempio `Kit` qui sotto utilizza le istruzioni generate direttamente. Esempi
legacy che utilizzano `@solana/web3.js`, `@solana/spl-token` e
`@solana/spl-memo` sono inclusi come riferimento.
