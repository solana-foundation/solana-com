...
Invece di un blockhash recente, usa il valore nonce come durata della
transazione. Per farlo, dobbiamo prima recuperare il valore nonce dall'account
nonce e poi usarlo per impostare la durata della transazione.

### Recuperare il valore nonce

Prima, recupera il valore nonce dall'account nonce:
...
Invece di usare un blockhash recente che scade, usa
`setTransactionMessageLifetimeUsingDurableNonce`. Questa funzione fa due cose:

1. Imposta il valore nonce come "blockhash" della transazione
2. Antepone automaticamente l'istruzione `advanceNonceAccount` (richiesta come
   prima istruzione in tutte le transazioni durable nonce)
...
Deserializza la transazione per aggiungere firme aggiuntive, poi serializza
...
il vecchio valore permanentemente invalida.
...
- L'autorità del nonce controlla se le transazioni possono essere invalidate.
  Considera di separare l'autorità del nonce dai firmatari delle transazioni per
  un controllo aggiuntivo e separazione dei compiti
