...
Su Solana, i programmi sono logica eseguibile che governa lo stato degli
account. Gli account dei token sono gestiti da un **Token Program**—il codice
onchain che verifica i trasferimenti e aggiorna i saldi in modo atomico.
...
Token-2022 (chiamato anche "Token Extensions") aggiunge funzionalità come transfer hook,
commissioni di trasferimento e Confidential Balances. Entrambi i programmi funzionano in modo simile per i trasferimenti di base,
ma è necessario utilizzare quello corretto per derivare gli ATA.
...
<Callout type="info">
  Solana utilizza mercati delle commissioni locali: le transazioni competono con altre transazioni
  che scrivono sullo stesso stato. Ciò significa che le commissioni di pagamento rimangono basse e prevedibili
  anche durante i periodi di elevata attività della rete altrove.
</Callout>
...
<Cards>
  ...
  <Card title="Invia il tuo primo pagamento" href="/docs/payments/send-payments">
