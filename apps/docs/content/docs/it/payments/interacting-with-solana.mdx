...
<Callout type="caution" title="Non utilizzare RPC pubblici per la produzione">
  Gli endpoint pubblici (`api.mainnet.solana.com`, `api.devnet.solana.com`) sono
  soggetti a limitazioni di frequenza, privi di SLA e non adatti ai flussi di pagamento in produzione.
  Utilizza un [provider RPC](https://solana.com/rpc) per ottenere un endpoint RPC privato
