...
<Callout type="caution">
  Il Commerce Kit è attualmente in versione beta. Le API potrebbero cambiare
  prima del rilascio stabile. Per ulteriori dettagli, consulta la
  [documentazione del Commerce Kit](/docs/tools/commerce-kit).
</Callout>
...
| ----------- | ----------- | --------------------------------------------------------- |
| `recipient` | `Address`   | Indirizzo del portafoglio del commerciante (obbligatorio) |
| `amount`    | `bigint`    | Importo in lamport o unità minori del token               |
| `splToken`  | `Address`   | Indirizzo mint del token SPL (omettere per SOL)           |
| `reference` | `Address[]` | Riferimento/i univoco/i per il monitoraggio dei pagamenti |
| `label`     | `string`    | Nome del commerciante visualizzato nel portafoglio        |
| `message`   | `string`    | Messaggio visualizzato prima del pagamento                |
| `memo`      | `string`    | Memo onchain allegato alla transazione                    |
