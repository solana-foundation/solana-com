...
backend di gestione delle chiavi attraverso un'unica interfaccia: Memory, Vault,
Privy, Turnkey, AWS KMS, Fireblocks, GCP KMS, CDP, Para, Dfns, Crossmint,
Openfort e Utila. Questo ti permette di sviluppare in locale con chiavi in
...
potrebbero essere estratti e utilizzati in modo improprio. Utilizza un proxy
backend o variabili d'ambiente che non vengano incluse nel bundle del codice
client.
...
| Metrica                       | Perché                          |
| ----------------------------- | ------------------------------- |
| Tasso di successo transazioni | Rilevare i problemi in anticipo |
| Latenza di conferma           | Monitorare la salute della rete |
| Spesa per fee prioritarie     | Gestione dei costi              |
| Tasso di errori RPC           | Salute del provider             |

Configura avvisi per:

- Trasferimenti sopra soglia dal treasury
- Picchi nel tasso di transazioni fallite
- Pattern insoliti nei destinatari
- Aumento del tasso di errori RPC
...
token falsificati che imitano USDC o altre stablecoin sono comuni—avranno lo
stesso nome e simbolo ma un mint diverso. La tua applicazione dovrebbe inserire
nel codice o nella allowlist gli indirizzi mint (in base ai tuoi requisiti),
...
indirizzi corretti in base all'ambiente—non hardcodare gli indirizzi mainnet e
dimenticare di sostituirli durante i test, o peggio, distribuire in produzione
gli indirizzi devnet.
...
programma sbagliato fallirà—o peggio, comporterà una perdita irreversibile di
...
Inserire nella allowlist gli indirizzi corretti protegge dai token contraffatti,
ma non protegge dall'invio al tipo _sbagliato_ di account. L'indirizzo di un
destinatario può essere un wallet, un token account, un mint o un programma, e
ognuno richiede una gestione diversa. SOL nativo inviato a qualcosa di diverso
da un wallet esce dal controllo del mittente — perso definitivamente nel caso di
un mint o di un programma, recuperabile solo dal proprietario dell'account nel
caso di un token account — senza alcuna transazione fallita che avvisi l'utente.
...
- [ ] SOL mainnet acquisito per le commissioni e il rent
...
- [ ] Test di carico effettuato al volume previsto
...
- **Program keypair:** Il tuo programma avrà un indirizzo diverso su mainnet
  rispetto a devnet (a meno che tu non stia riutilizzando lo stesso keypair).
  Aggiorna tutti i riferimenti nella configurazione della tua applicazione.
  Conserva il tuo program keypair in un luogo sicuro (nota che l'esecuzione di
  `cargo clean` eliminerà probabilmente il tuo program keypair).
- **Inizializzazione degli account:** Se il tuo programma richiede account
  amministrativi, PDA o altri account di stato, assicurati che questi siano
  creati su mainnet prima che gli utenti interagiscano con la tua applicazione.
  Lo stesso vale per tutti gli Associated Token Accounts (ATA) di cui il tuo
  programma ha bisogno.

### Processo di Distribuzione

- **Buffer accounts:** I programmi di grandi dimensioni vengono distribuiti
  tramite buffer accounts. Il comando `solana program deploy` lo gestisce
  automaticamente, ma tieni presente che la distribuzione non è atomica: se
  interrotta, potrebbe essere necessario recuperare o chiudere i buffer
  accounts. Consulta
  [Deploying Programs](https://solana.com/docs/programs/deploying).
- **Upgrade authority:** Decidi se il tuo programma deve essere aggiornabile
  dopo il lancio. Per garantire l'immutabilità, revoca l'upgrade authority dopo
  la distribuzione. Per maggiore flessibilità, proteggi adeguatamente la chiave
  dell'upgrade authority.
- **Rent:** Assicurati che il tuo wallet di distribuzione abbia abbastanza SOL
  per coprire i minimi rent-exempt per tutti i program account.
...
Per una guida completa al deployment dei programmi, consulta
