...
Per la firma del backend in produzione, considera l'utilizzo di
[solana-keychain](https://github.com/solana-foundation/solana-keychain)—una
libreria di firma unificata che astrae molteplici backend di gestione delle
chiavi attraverso un'unica interfaccia: Memory, Vault, Privy, Turnkey, AWS KMS,
Fireblocks, GCP KMS, CDP, Para, Dfns. Questo ti consente di sviluppare
localmente con chiavi in memoria, per poi passare a backend di livello
produzione senza modificare il codice dell'applicazione.
