...
Consulta la
[guida Adding Signers](https://github.com/solana-foundation/solana-keychain/blob/main/docs/ADDING_SIGNERS.md)
per integrare ulteriori servizi di gestione delle chiavi.

### Sicurezza RPC

Tratta gli endpoint RPC come chiavi API—non esporli nel codice frontend dove
possono essere estratti e utilizzati impropriamente. Utilizza un proxy backend o
variabili d'ambiente che non vengano incluse nel codice client.
...
| Tasso di errori RPC           | Stato del provider                |

Configura avvisi per:

- Trasferimenti sopra la soglia dal treasury
- Picchi nel tasso di transazioni fallite
- Pattern di destinatari insoliti
- Aumenti del tasso di errori RPC
...
di token completamente diversi. Configura l'applicazione per caricare gli
indirizzi corretti per ogni ambiente—non inserire indirizzi mainnet direttamente
nel codice dimenticandoti di sostituirli durante i test o, peggio ancora,
distribuendo indirizzi devnet in produzione.
...
programma sbagliato causerà un errore—o peggio, la perdita irreversibile di
...
- [ ] SOL mainnet acquisiti per commissioni e rent
- [ ] RPC di produzione configurato (non endpoint pubblico)
- [ ] Endpoint RPC di fallback configurato
- [ ] Commissioni di priorità implementate con prezzi dinamici
...
- [ ] Monitoraggio delle transazioni e avvisi attivi
- [ ] Test di carico eseguiti al volume previsto
...
- **Keypair del programma:** Il tuo programma avrà un indirizzo diverso su
  ...
  applicazione. Conserva il tuo keypair del programma in una posizione sicura
  (nota che l'esecuzione di `cargo clean` probabilmente eliminerà il tuo keypair
  del programma).
- **Inizializza account:** Se il tuo programma richiede account amministratore,
  PDA o altri account di stato, assicurati che questi siano creati su mainnet
  prima che gli utenti interagiscano con la tua applicazione. Lo stesso vale per
  qualsiasi Associated Token Account (ATA) di cui il tuo programma ha bisogno.
...
- **Account buffer:** I programmi di grandi dimensioni vengono distribuiti
  ...
  automaticamente, ma è importante capire che la distribuzione non è atomica—se
  interrotta, potresti dover recuperare o chiudere gli account buffer. Vedi
  [Distribuzione di Programmi](https://solana.com/docs/programs/deploying).
- **Autorità di aggiornamento:** Decidi se il tuo programma deve essere
  ...
  aggiornamento dopo la distribuzione. Per flessibilità, proteggi adeguatamente
  la chiave dell'autorità di aggiornamento.
- **Rent:** Assicurati che il tuo wallet di distribuzione abbia abbastanza SOL
  per coprire i minimi esenti da affitto per tutti gli account del programma.
- **Verifica:** [Verifica](https://solana.com/docs/programs/verified-builds) il
  ...
  sulla rete Solana corrisponda al codice sorgente nel tuo repository

Per una guida completa alla distribuzione di programmi, vedi
[Distribuzione di Programmi](https://solana.com/docs/programs/deploying).
