...
L'abbonato accetta i termini attuali del piano. Il PDA dell'abbonamento deriva
dal PDA del piano e dall'indirizzo dell'abbonato.
...
L'annullamento segna l'abbonamento come in fase di chiusura. La revoca chiude il
PDA dell'abbonamento dopo che è trascorso il periodo di scadenza
dell'annullamento. L'abbonato firma entrambe le transazioni.
...
- `amount` è espresso in unità base. Per un token a 6 decimali, `5_000_000`
  significa `5` token.
- L'SDK TypeScript recupera i termini del piano in tempo reale durante
  `subscribe` quando li ometti.
- Il `SubscribeBuilder` Rust richiede i termini del piano previsti. Prima
  recupera e decodifica l'account del piano, poi passa quei campi attraverso
  `SubscribeData`.
- Solo il merchant o un wallet elencato in `pullers` può riscuotere i pagamenti.
- Il sottoscrittore firma le transazioni di configurazione, cancellazione e
