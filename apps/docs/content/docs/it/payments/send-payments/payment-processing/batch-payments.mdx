...
aumento il limite di dimensione a 4096 byte, che consente circa tre volte più trasferimenti per transazione. Il batching è comunque necessario oltre tale soglia, poiché il runtime carica al massimo 64 account per transazione indipendentemente dal formato.
