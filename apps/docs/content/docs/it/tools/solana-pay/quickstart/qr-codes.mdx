---
title: Integrazione Codice QR
description: Genera e personalizza codici QR per pagamenti con wallet mobile
---

I codici QR sono il modo più comune per collegare applicazioni web e wallet
mobili. Solana Pay include una funzione `createQR` integrata basata su
`@solana/qr-code-styling` — non sono necessari pacchetti aggiuntivi per codici
QR.

## Generazione Base di Codici QR

### Generare e Visualizzare un Codice QR
...
### Personalizzare i Codici QR

La funzione `createQR` accetta parametri opzionali per dimensione, colore di
sfondo e colore di primo piano:
...
### Ottenere le Opzioni QR per Uso Avanzato

Utilizza `createQROptions` per ottenere l'oggetto di configurazione grezzo per
...
Il client merchant espone anche metodi per codici QR:
...
### QR di Pagamento con Aggiornamenti di Stato
...
## Integrazione Point of Sale

### Visualizzazione QR su Terminale POS
...
Su dispositivi mobili, considera di fornire un link diretto insieme al codice
QR:
