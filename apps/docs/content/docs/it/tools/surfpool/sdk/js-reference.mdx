...
description:
  API pubblica completa per il pacchetto @solana/surfpool — classe Surfnet, tipi di configurazione, opzioni di deploy e struttura degli eventi.
---
...
<Callout type="info" title="Punto di ingresso del kit">
  I simboli seguenti sono le esportazioni radice di `@solana/surfpool`. Il sottopercorso `@solana/surfpool/kit` aggiunge `surfpool()`, `surfnetCheatcodes()`, `createSurfnetCheatcodesRpc()`, `DEFAULT_SURFNET_ENDPOINT`, i tipi di configurazione del plugin e i tipi wire dei cheatcode generati — tutti documentati su [Kit Plugin](/docs/tools/surfpool/sdk/kit-plugin) anziché in questa tabella.
</Callout>
