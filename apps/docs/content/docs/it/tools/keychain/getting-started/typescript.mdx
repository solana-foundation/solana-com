---
title: TypeScript
description: Installa e utilizza @solana/keychain nelle applicazioni TypeScript
---

## Installazione

Installa il pacchetto principale o i singoli signer secondo necessità:
...
### Factory unificata (Consigliata per le app che desiderano supportare più signer)

Utilizza `createKeychainSigner` con una configurazione discriminata per creare
qualsiasi backend:
...
Oppure installa un singolo pacchetto signer per un footprint di dipendenze
ridotto:
...
### Utilizzo con i messaggi di transazione

Tutti i signer implementano l'interfaccia `SolanaSigner`, che è compatibile con
`@solana/kit` e `@solana/signers`:
...
### Utilizzo con un Kit Client (plugin keychainSigner)

`@solana/keychain-kit-plugin` installa un keychain signer direttamente su un
[Kit client](https://github.com/anza-xyz/kit), collegandolo come `payer` e
`identity` così da evitare la configurazione manuale di `pipe` mostrata in
precedenza:
...
Il plugin accetta la stessa configurazione con tag backend di
`createKeychainSigner`, e viene incluso nel bundle solo il backend a cui delega
(i pacchetti backend vengono caricati tramite `import()` dinamico).

Usa `keychainPayer` o `keychainIdentity` per impostare un solo ruolo e combinare
backend su un singolo client:
...
Hai già un `SolanaSigner`? Ogni keychain signer è un Kit `TransactionSigner`
valido, quindi puoi installarne uno esistente con
[`@solana/kit-plugin-signer`](https://github.com/anza-xyz/kit-plugins/tree/main/packages/kit-plugin-signer)
al suo posto (`pnpm add @solana/kit-plugin-signer`):
...
Fordefi dispone di due modalità di firma, selezionate in base al fatto che si passi o meno `chain`. Le richieste API vengono autenticate con una firma di richiesta ECDSA P-256 — fornisci la chiave di firma come `privateKeyPem`, oppure conservala in un KMS/HSM passando un `requestSigner`.

**Modalità Solana nativa** (consigliata per l'uso on-chain): imposta `chain`, e Fordefi firma e trasmette la transazione autonomamente. Poiché Fordefi potrebbe sostituire il blockhash o le commissioni prima della firma, il firmatario è un `TransactionSendingSigner` di Kit — utilizzalo con `signAndSendTransactionMessageWithSigners`, non con il flusso partial-signer mostrato in precedenza.
...
**Modalità black box**: ometti `chain`. Fordefi firma i byte grezzi e non trasmette la transazione; sei tu ad assemblare e inviare la transazione. Utilizzala con un vault black box di Fordefi.
...
## Interfaccia SolanaSigner

L'interfaccia `SolanaSigner` estende i tipi `@solana/signers` per la piena
compatibilità:
...
## Creazione di Signer Personalizzati

Implementa l'interfaccia `SolanaSigner` per creare signer personalizzati:
...
Consulta la [guida all'aggiunta di Signer](/docs/tools/keychain/adding-signers)
