...
| ------------ | ------------------------------- |
| `memory`     | Firma keypair locale (predefinita) |
| `vault`      | Motore Transit di HashiCorp Vault  |
...
Per la firma con custodia MPC Fordefi. Le richieste API sono autenticate con una firma di richiesta ECDSA P-256 analizzata da `private_key_pem`; per mantenere quella chiave in un KMS/HSM, implementa `FordefiRequestSigner` e usa `Signer::from_fordefi_with_signer`.

Fordefi dispone di due modalità di firma. Imposta `chain` per la **modalità Solana nativa**, in cui Fordefi può sostituire il blockhash o le commissioni e trasmette la transazione autonomamente. Lascia `chain` come `None` per la **modalità black box**, in cui Fordefi firma byte grezzi e sei tu a trasmettere la transazione.
