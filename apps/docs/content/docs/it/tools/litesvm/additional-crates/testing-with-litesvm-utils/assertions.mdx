...
title: Asserzioni
description:
  Verifica stati degli account, saldi e proprietà con gli helper di asserzione
---
...
<Callout type="info">
  `assert_account_closed` ha esito positivo quando un account non esiste, ha
  zero lamport, oppure ha dati vuoti. Usalo per verificare la pulizia dopo la
...
<Callout type="info">
  Dimensioni comuni degli account: - **Mint Account**: 82 byte - **token
  account**: 165 byte - **System Account**: 0 byte (solo lamport)
</Callout>
...
<Callout type="warn">
  Le asserzioni sono progettate per fallire rapidamente con messaggi di errore
  chiari. Usale liberamente nei tuoi test per individuare i problemi in
  anticipo.
</Callout>
