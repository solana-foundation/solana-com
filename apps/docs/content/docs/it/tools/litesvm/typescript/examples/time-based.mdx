...
description: Testa la logica dipendente dal tempo con la manipolazione del clock di LiteSVM
---

Dimostra l'utilizzo di `warpToSlot` per testare la logica di programma dipendente dal tempo.
...
Schema di esempio per testare funzionalità con blocco temporale:
...
Puoi leggere lo schedule dell'epoch per comprendere la relazione slot/epoch sul
cluster:
...
Quando il tuo programma legge `epoch` o `unix_timestamp` dalla sysvar Clock, usa
`setClock` per impostare direttamente quei campi:
...
Questo rispecchia il pattern Rust `set_sysvar(&clock)`. Usa `warpToSlot` per un semplice
avanzamento dello slot, `setClock` quando hai bisogno di controllare epoch o timestamp.
...
| Scenario                | Approccio                           |
| ----------------------- | ---------------------------------- |
| Vesting dei token           | Salta oltre il cliff/le milestone di vesting |
| Fine delle aste         | Salta oltre lo slot di fine asta         |
| Ricompense di staking         | Usa `setClock` per impostare l'epoch        |
| Prelievi con blocco temporale | Salta oltre lo slot di sblocco              |
| Limitazione della frequenza           | Salta tra gli intervalli consentiti     |
| Ordini in scadenza         | Salta oltre la scadenza dell'ordine         |
...
1. **Abilita le Sysvar**: Chiama `withSysvars()` prima di utilizzare i metodi del clock
2. **Avanza in Avanti**: Usa `warpToSlot(slot)` per avanzare lo slot
3. **Controllo Completo**: Usa `setClock` quando hai bisogno di impostare `epoch`,
   `unixTimestamp` o altri campi del clock
4. **Leggi il Clock**: Usa `getClock()` per leggere lo slot, l'epoch e il timestamp correnti
5. **Pattern di Test**: Testa prima e dopo i confini temporali
