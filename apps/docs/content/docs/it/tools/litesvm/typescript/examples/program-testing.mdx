...
Testare un programma Solana personalizzato significa caricare il suo file `.so` compilato nell'SVM, configurare gli account che si aspetta e inviargli delle istruzioni.
LiteSVM gestisce tutto questo in-process — nessun validator necessario. Consigliamo
di generare i client del programma con il
[Codama JS Renderer](https://github.com/codama-idl/renderers-js) per avere
accesso a un Kit Plugin compatibile.
...
1. **Caricamento del Programma**: Usa `addProgramFromFile` per caricare i programmi compilati e
   importa myProgramPlugin() dal tuo client
2. **Configurazione**: Abilita sysvar, builtin e precompile secondo necessità
3. **Configurazione degli Account**: Pre-popola gli account dati con `setAccount`
4. **Invio**: Usa `client.myProgram.testInstruction({..}).sendTransaction()` —
   nessuna costruzione manuale della transazione
5. **Verifica**: Controlla lo stato dell'account dopo l'esecuzione
   (`client.myProgram.myAccount.fetch`)

Una volta che questo scheletro funziona, puoi costruire una suite di test completa
aggiungendo altre istruzioni e asserzioni.
