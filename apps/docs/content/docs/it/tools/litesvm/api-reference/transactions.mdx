---
title: Transazioni
description: Metodi per la gestione delle transazioni
---
...
Esegui la transazione e modifica lo stato.
...
Simula la transazione senza modificare lo stato.

Restituisce `SimulatedTransactionInfo` che contiene:

- `meta`: Metadati della transazione (log, unità di calcolo, firma, ecc.)
- `post_accounts`: Stati degli account dopo la simulazione
...
Usa `simulate_transaction()` per:

- Testare la validità della transazione senza modificare lo stato
- Verificare il consumo delle unità di calcolo
- Controllare i log della transazione senza commit
- Testare i percorsi di gestione degli errori
...
`send_transaction()` restituisce `TransactionMetadata` direttamente, mentre
`simulate_transaction()` restituisce `SimulatedTransactionInfo` che contiene
...
### Accesso ai Dati della Transazione
...
Interroga una transazione dalla cronologia. Richiede che
`with_transaction_history()` sia impostato.
...
Ottieni il blockhash corrente per le transazioni.
...
Forza la scadenza del blockhash corrente.
...
### Controlla i Log delle Transazioni
