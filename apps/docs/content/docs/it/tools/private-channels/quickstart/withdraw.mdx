...
il proprio saldo di token lato canale chiamando `WithdrawFunds` sul Withdraw
...
## Fase 1: Avvia il Prelievo sul Canale

Chiama `WithdrawFunds` sul Withdraw Program per bruciare il tuo saldo lato canale.
...
- Invia questa transazione al gateway (`http://localhost:8899`), non a un
  RPC Solana pubblico
- Se `destination` è specificato, i fondi rilasciati vengono inviati a quel wallet invece che a quello
  del firmatario

## Cosa Aspettarsi

Non è richiesta alcuna azione aggiuntiva dopo aver chiamato `WithdrawFunds`. I servizi
dell'operatore gestiscono il settlement automaticamente:

1. `indexer-private-channel` interroga il canale ogni 1 secondo per gli eventi di burn
   e registra un record di prelievo in sospeso quando il tuo viene rilevato
2. `operator-private-channel` interroga il database ogni 1 secondo per i record
   in sospeso e invia `ReleaseFunds` all'Escrow Program su Solana devnet
   con la prova di esclusione SMT richiesta; verifica la conferma su devnet fino a
   5 volte a intervalli di 400 ms prima di riprovare

I fondi appaiono in genere nel tuo wallet devnet entro pochi secondi in condizioni normali.

## Come l'Operatore Esegue il Settlement (Riferimento)
...
i fondi senza dimostrare che il nonce di prelievo non sia già stato utilizzato in precedenza. Questo passaggio
...
- `sibling_proofs`: 512 byte (16 hash fratello da 32 byte per la prova dell'albero)

## Verifica

Dopo la conferma della chiamata dell'operatore, controlla il tuo saldo token su devnet:
...
escrow, inviato un trasferimento off-chain tramite il gateway e prelevato i fondi
...
<Cards>
  ...
  >
    Approfondisci i partecipanti e l'intero flusso dal deposito al prelievo.
  </Card>
  <Card
  ...
  >
    Riferimento completo dell'istruzione ReleaseFunds per gli strumenti dell'operatore.
  </Card>
  <Card
