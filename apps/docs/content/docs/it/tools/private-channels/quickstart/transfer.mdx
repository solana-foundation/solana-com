...
description:
  Invia un trasferimento di token SPL off-chain attraverso il gateway di Private Channels
---
...
la transazione al nodo di scrittura, che la esegue sul layer degli account off-chain del channel.
I trasferimenti vengono confermati quando il sequencer li elabora,
non quando un blocco Solana viene confermato, e non compaiono su Mainnet.

## Cambia l'RPC del Tuo Wallet

Prima di inviare un trasferimento, sostituisci l'endpoint RPC del tuo wallet con l'URL del gateway
...
Consulta **[Authentication & Roles](/docs/tools/private-channels/concepts/auth)** per
come registrarsi, accedere e ottenere un token.
...
Interroga il saldo del channel del destinatario attraverso il gateway per confermare che
...
prelevar i token e riportarli nel tuo wallet Solana
