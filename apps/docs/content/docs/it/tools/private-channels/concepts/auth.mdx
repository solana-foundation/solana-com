...
con autenticazione JWT e controllo degli accessi basato sui ruoli (RBAC). Quando
...
- **Sviluppatori** - registrazione, accesso, verifica del wallet e invio di
...
Quando `JWT_SECRET` non è impostato, il gateway opera in modalità aperta; non è richiesto alcun token.
...
> comando `docker compose`, includendo `--env-file .env` affinché i secret come
> `JWT_SECRET` e `POSTGRES_PASSWORD` vengano effettivamente risolti (Compose disabilita il
> caricamento automatico del file `.env` non appena viene passato un flag `--env-file`):
>
> ```bash
...
Autentica e riceve un JWT firmato valido per 24 ore.

```json
{ "username": "alice", "password": "hunter2" }
```

Restituisce `{ "token": "<jwt>" }`. Sia nome utente errato che password errata restituiscono
`401` per prevenire l'enumerazione dei nomi utente.

### `POST /auth/challenge-wallet`

Richiede una sfida di firma per dimostrare la proprietà di un wallet Solana. Richiede un
JWT valido.
...
Invia la sfida firmata per registrare un wallet come verificato. Richiede un JWT valido.
...
e memorizza il wallet. Ogni nonce può essere utilizzato una sola volta; i replay vengono
rifiutati.
...
Elenca tutti i wallet verificati dell'utente autenticato. Richiede un JWT valido.

### `DELETE /auth/wallets/{pubkey}`

Rimuove un wallet verificato dall'account dell'utente autenticato. Richiede un JWT valido.
...
> configurazione JWT del gateway, non deserializzati nella struct dei claim dell'applicazione.
> Il codice a livello applicativo ha accesso solo a `sub`, `role` ed `exp`.
...
Ruolo elevato. Deve essere assegnato direttamente; non esiste un percorso self-service per
eseguire l'escalation da `user` a `operator`.
...
<Callout type="warning">
  Questa è un'operazione privilegiata sul database. Limita l'accesso al database
...
Questo inserisce direttamente un wallet verificato nella tabella `verified_wallets`,
bypassing il flusso challenge/verify. Applica un vincolo di unicità su
`(user_id, pubkey)`. **Non** assegna di per sé il ruolo `operator`; usa
`set-role` o l'aggiornamento SQL precedente per farlo. Questo comando serve ad associare un
...
- Richiesto per: `ReleaseFunds`, `ResetSmtRoot`
...
## Invio di Richieste Autenticate
...
| Metodo                        | Route      | Nessun JWT | `user`           | `operator` |
| ----------------------------- | ---------- | ------ | ---------------- | ---------- |
...
¹ **Ownership-gated**: per un token account SPL (il campo owner è `TokenkegQ...`
o `TokenzQ...`, dati di almeno 165 byte), il gateway verifica che il campo
`owner` o `delegate` corrisponda a uno dei wallet verificati dell'utente autenticato. Per qualsiasi altro tipo di account (un wallet System Program, o un PDA sconosciuto), verifica invece se il pubkey interrogato è uno dei wallet verificati dell'utente, poiché tali account non hanno un campo owner/delegate da ispezionare.
Un controllo fallito restituisce 403.
