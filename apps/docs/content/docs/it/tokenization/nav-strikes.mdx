...
difficulty: advanced
title: "NAV Strikes per Fondi del Mercato Monetario su Solana"
description:
...
I NAV (Net Asset Value) strike sono orari fissi durante la giornata di negoziazione in cui le quote del fondo vengono prezzate e gli ordini vengono eseguiti. I fondi tradizionali hanno tipicamente un unico strike giornaliero alla chiusura del mercato, ma la velocità e i bassi costi di Solana consentono più strike infragiornalieri, offrendo agli investitori maggiore flessibilità.

## Cos'è il NAV?

Un fondo è un insieme di asset posseduti collettivamente da molti investitori. Il **NAV (Net Asset Value)** è semplicemente il prezzo di una quota del fondo. Pensatelo in questo modo:

```
NAV = (Total Value of Everything in the Fund - liabilities) ÷ Number of Shares
```

Ad esempio, se un fondo detiene $10 milioni di asset e ha 10 milioni di quote in circolazione, ogni quota vale $1,00.

**Perché il NAV è importante?**

- **Acquisto di quote (sottoscrizione)**: Si paga il NAV corrente. Se il NAV è $1,02 e
  si investe $102, si ottengono 100 quote.
- **Vendita di quote (rimborso)**: Si riceve il NAV corrente. Se si riscattano
  100 quote al NAV di $1,02, si ottengono $102.

I fondi del mercato monetario mantengono tipicamente un NAV stabile intorno a $1,00, con piccole fluttuazioni basate sugli interessi maturati.

## Il Problema

Quando si investe in un fondo del mercato monetario, tradizionalmente accadono due cose in momenti diversi:

1. Si invia un ordine → Si attende la chiusura del mercato
2. NAV calcolato → Quote emesse/rimborsate a T+1 o T+2

Questo crea diversi problemi:

- **Esecuzione ritardata**: Gli ordini inviati al mattino attendono fino alle 16:00
- **Ritardo nel regolamento**: Il denaro rimane in sospeso per 1-2 giorni
- **Flessibilità limitata**: Una sola opportunità al giorno per effettuare transazioni
- **Prezzi obsoleti**: Il NAV può essere obsoleto tra l'ordine e l'esecuzione

## La Soluzione

I **NAV Strike su Solana** consentono più finestre di regolamento giornaliere con esecuzione atomica. Gli ordini vengono accodati ed eseguiti istantaneamente ad ogni orario di strike con il NAV corrente.
...
Questa guida illustra come implementare i NAV strike su Solana utilizzando le estensioni SPL Token 2022 per l'emissione conforme di quote del fondo e USDC standard per il regolamento, il tutto senza scrivere programmi Rust personalizzati.

<Callout type="info" title="Implementazione di Riferimento Educativa">

Puoi utilizzare il
[codice sorgente](https://github.com/Woody4618/nav-strikes-reference) di questa
implementazione per provare i NAV strike in locale.

Questa guida fornisce un'implementazione di riferimento solo a scopo esplorativo ed educativo. **NON utilizzare questo codice direttamente in produzione** senza:
...
Il sistema NAV Strike è composto dai seguenti componenti principali:

1. **Token di Quote del Fondo**: SPL Token 2022 con metadati che memorizzano il NAV onchain
2. **Valuta di Regolamento**: USDC standard (token SPL esistente)
3. **Amministratore del Fondo**: Orchestra gli strike tramite autorità delegata
4. **Sistema di Whitelist**: Controlla quali indirizzi possono detenere quote del fondo

### Principi Chiave di Progettazione

- **Nessun Programma Personalizzato**: Utilizza solo Token Extensions Program e USDC standard
- **Regolamento Atomico**: Una singola transazione garantisce che la sottoscrizione/rimborso
  venga completata o fallisca insieme
- **Stato Predefinito Congelato**: Le quote richiedono una whitelist esplicita per la
  conformità normativa
- **Autorità Delegata**: L'amministratore del fondo opera senza prendere custodia
- **NAV Onchain**: Il prezzo corrente è memorizzato nei metadati del token, verificabile pubblicamente

### Fase 1: Configurazione del Fondo

1. Crea il token di Quote del Fondo con estensioni di metadati
2. Inizializza il NAV a $1,00
3. Imposta il calendario degli strike giornalieri
4. Inserisci in whitelist gli investitori istituzionali

### Fase 2: Periodo Pre-Strike

1. Gli investitori inviano richieste di sottoscrizione/rimborso
2. Gli ordini vengono accodati per il prossimo orario di strike
3. Il fondo calcola il NAV preliminare
4. Vengono eseguiti i controlli di rischio

### Fase 3: Esecuzione del NAV Strike
...
1. Genera le conferme di negoziazione
2. Aggiorna la composizione del fondo
3. Prepara il prossimo strike
4. Riconcilia con il depositario

### Schema di Autorità Delegata

Gli investitori pre-autorizzano l'amministratore del fondo a spostare i loro token, quindi l'amministratore esegue all'orario di strike senza necessità delle firme degli investitori:
...
### Flusso di Sottoscrizione (USDC → Quote del Fondo)
...
### Flusso di Rimborso (Quote del Fondo → USDC)
...
## Perché Solana per i NAV Strike?

L'architettura di Solana offre vantaggi significativi rispetto al regolamento tradizionale dei fondi:

| Aspetto            | Tradizionale (T+1) | NAV Strike su Solana          |
| ----------------- | ----------------- | --------------------------- |
| Logica di Regolamento  | Agenti di trasferimento   | Raggruppamento atomico delle transazioni |
| Tempo di Regolamento   | 1-2 giorni          | &lt;1 secondo                |
| Costo della Transazione  | $50-200           | &lt;$0,01                   |
| Strike al Giorno   | 1                 | 4+ (configurabile)           |
| Rischio prezzi obsoleti | Alto              | Inferiore                       |
| Trasparenza NAV  | Report a fine giornata | Onchain, in tempo reale          |

Le transazioni atomiche di Solana eliminano gli intermediari garantendo regolamenti istantanei, economici e sicuri con trasparenza NAV onchain.

## Token del Fondo con Token Extensions

SPL Token 2022 fornisce potenti estensioni che consentono l'emissione conforme di quote del fondo senza programmi personalizzati:

### Estensioni Essenziali per le Quote del Fondo

1. **Estensione Default Account State**: Imposta tutti i nuovi token account su congelato per
   impostazione predefinita, richiedendo una whitelist esplicita (conformità KYC/AML)
2. **Estensione Metadati**: Memorizza il NAV, il calendario degli strike e l'AUM onchain per
   la trasparenza

### Configurazione delle Autorità

- **Mint Authority**: Amministratore del Fondo (controlla l'emissione delle quote)
- **Freeze Authority**: Amministratore del Fondo (gestisce la whitelist)
- **Update Authority**: Amministratore del Fondo (aggiorna i metadati NAV)

<Callout type="info">

Lo stato congelato predefinito è fondamentale per la conformità normativa. Garantisce che solo gli indirizzi esplicitamente inseriti in whitelist (verificati tramite KYC) possano ricevere e detenere quote del fondo.

</Callout>

### Campi dei Metadati Onchain
...
## Implementazione Completa dei NAV Strike

### Configurazione del NAV Strike Engine

Si noti che in questa guida utilizziamo la libreria @solana/kit per creare il NAV Strike Engine. È possibile trovare anche un'implementazione con `web3.js` nel
[codice sorgente](https://github.com/Woody4618/nav-strikes-reference). Per prima cosa, creare
la classe del motore principale che gestisce tutte le operazioni del fondo:
...
<Callout type="tip" title="Aggiornamenti Dinamici dei Metadati">

Per le quote del fondo, i campi `currentNAV`, `lastStrikeTime` e `totalAUM` vengono
aggiornati onchain ad ogni NAV strike utilizzando
`getUpdateTokenMetadataFieldInstruction`. Questo fornisce:

- **Visibilità NAV in tempo reale** per tutti i partecipanti
- **Audit trail** di tutti gli aggiornamenti NAV onchain
- **Integrazione** con i protocolli DeFi in grado di leggere i metadati onchain
...
Ecco un esempio completo che illustra un'intera giornata di operazioni di NAV strike:
...
Prima del deployment in produzione, assicurarsi di affrontare i seguenti aspetti:

1. **Sicurezza**: Gestione professionale delle chiavi, controlli multi-firma e
   audit di sicurezza completi del codice sorgente
2. **Conformità Normativa**: Registrazione dei titoli, sistemi KYC/AML, restrizioni
   sui trasferimenti e reportistica obbligatoria
3. **Gestione delle Chiavi**: Soluzioni di custodia istituzionale con adeguate procedure
   di backup e ripristino
4. **Elaborazione delle Transazioni**: Commissioni prioritarie, logica di retry, gestione
   delle conferme e ridondanza RPC
5. **Operazioni del Fondo**: Calcolo del NAV da fonti autorevoli, integrazione con
   il depositario e riconciliazione
6. **Monitoraggio**: Tracciamento in tempo reale, sistema di alerting e reportistica
   normativa automatizzata

## Prossimi Passi

Dopo aver compreso i NAV strike su Solana:

1. **Esplora Token Extensions**: Scopri altre
   [Token 2022 extensions](/docs/tokens/extensions/transfer-hook) come ad esempio
   il delegato permanente per funzionalità di conformità aggiuntive

2. **Implementa Token ACL**: Per una conformità avanzata, considera l'utilizzo di
   [Token ACL](/docs/tokenization/token-acl) per gestire la whitelist degli investitori
   con sblocco senza permessi. Questo consente la verifica KYC self-service dove
   gli investitori possono attivare automaticamente i propri account dopo aver superato i controlli
   di conformità, riducendo il carico operativo pur mantenendo il controllo normativo.

3. **Studia il Regolamento DvP**: Consulta la nostra
   [guida Delivery vs Payment](/docs/tokenization/dvp) per i modelli di regolamento
   dei titoli

4. **Architettura di Produzione**: Progetta sistemi robusti di schedulazione, monitoraggio e
   disaster recovery

## Conclusione

Il modello di transazione atomica di Solana e le Token Extensions forniscono una base solida per implementare più NAV strike giornalieri per fondi del mercato monetario.
La combinazione di:

- Esecuzione atomica nativa (nessun rischio legato a smart contract)
- Finalità in meno di un secondo
- Costi di transazione quasi nulli
- Trasparenza NAV onchain
- Funzionalità di conformità integrate (default congelati, metadati)

Rende Solana una piattaforma ideale per modernizzare l'infrastruttura di regolamento dei fondi e offrire agli investitori opportunità di negoziazione più frequenti.

Tuttavia, il passaggio da questa implementazione di riferimento educativa alla produzione richiede un lavoro aggiuntivo significativo in termini di sicurezza, conformità, custodia e operatività. Collaborare sempre con esperti legali, normativi e tecnici qualificati quando si tratta di tokenizzazione di fondi.
