...
aggiunge ~200 byte. Per settlement multi-parte di grandi dimensioni, considera l'utilizzo di
...
Il prossimo
[formato di transazione v1](/docs/core/transactions/versioned-transactions#v1-message-format)
alza il limite a 4096 byte ma rimuove il supporto per le Address Lookup Table, quindi i settlement v1
inseriscono i loro indirizzi inline. In ogni caso, il runtime carica al massimo
64 account per transazione.
