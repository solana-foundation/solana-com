---
title: إنشاء حسابات محددة
description:
  تعلم كيفية بناء برنامج سولانا CRUD (إنشاء، قراءة، تحديث، حذف) باستخدام عناوين
  مشتقة من البرنامج (PDAs) وإطار عمل Anchor. يوضح هذا الدليل خطوة بخطوة كيفية
  إنشاء وتحديث وحذف حسابات الرسائل على السلسلة باستخدام PDAs، وتنفيذ التحقق من
  صحة الحساب، وكتابة الاختبارات. مثالي للمطورين الراغبين في فهم كيفية استخدام
  PDAs في برامج سولانا.
h1: عنوان مشتق من البرنامج
---

في هذا القسم، ستتعلم كيفية بناء برنامج أساسي للإنشاء والقراءة والتحديث والحذف
(CRUD).

يوضح هذا الدليل برنامجًا بسيطًا حيث يمكن للمستخدمين إنشاء وتحديث وحذف رسالة.
توجد كل رسالة في حساب بعنوان محدد مشتق من البرنامج نفسه (عنوان مشتق من البرنامج
أو PDA).

يرشدك هذا الدليل خلال بناء واختبار برنامج سولانا باستخدام إطار عمل Anchor مع
توضيح العناوين المشتقة من البرنامج (PDAs). لمزيد من التفاصيل، راجع صفحة
[العناوين المشتقة من البرنامج](/docs/core/pda).

للمرجع، يمكنك عرض
[الكود النهائي](https://beta.solpg.io/668304cfcffcf4b13384d20a) بعد إكمال قسمي
PDA واستدعاء البرامج المتقاطع (CPI).

<Steps>
<Step>

### الكود الأولي

ابدأ بفتح
[رابط Solana Playground](https://beta.solpg.io/66734b7bcffcf4b13384d1ad) مع
الكود الأولي. ثم انقر على زر "Import" لإضافة البرنامج إلى مشاريع Solana
Playground الخاصة بك.

![استيراد](/assets/docs/intro/quickstart/pg-import.png)

<WithMentions>

في ملف `lib.rs`، ستجد برنامجاً يحتوي على تعليمات [`create`](mention:one)،
[`update`](mention:two)، و [`delete`](mention:three) لإضافتها في الخطوات
التالية.

```rs title="lib.rs"
use anchor_lang::prelude::*;

declare_id!("8KPzbM2Cwn4Yjak7QYAEH9wyoQh86NcBicaLuzPaejdw");

#[program]
pub mod pda {
    use super::*;

    // !mention one
    pub fn create(_ctx: Context<Create>) -> Result<()> {
        Ok(())
    }

    // !mention two
    pub fn update(_ctx: Context<Update>) -> Result<()> {
        Ok(())
    }

    // !mention three
    pub fn delete(_ctx: Context<Delete>) -> Result<()> {
        Ok(())
    }
}

#[derive(Accounts)]
pub struct Create {}

#[derive(Accounts)]
pub struct Update {}

#[derive(Accounts)]
pub struct Delete {}

#[account]
pub struct MessageAccount {}
```

</WithMentions>

قبل البدء، قم بتشغيل _shell`build`_ في طرفية Playground للتحقق من أن البرنامج
الأساسي يبني بنجاح.

```terminal
$ build
Building...
Build successful. Completed in 3.50s.
```

</Step>
<Step>

### تحديد نوع حساب الرسالة

أولاً، حدد بنية حساب الرسالة الذي ينشئه البرنامج. تحدد هذه البنية البيانات
المراد تخزينها في الحساب الذي ينشئه البرنامج.

<WithNotes>

في `lib.rs`، قم بتحديث بنية _rs`MessageAccount`_ بما يلي:

```rs title="lib.rs"
// !tooltip[/account/] account
#[account]
pub struct MessageAccount {
    // !tooltip[/user/] user
    pub user: Pubkey,
    // !tooltip[/message/] message
    pub message: String,
    // !tooltip[/bump/] bump
    pub bump: u8,
}
```

### !account

تقوم سمة _rs`#[account]`_ في برنامج Anchor بتعليق البنيات التي تمثل بيانات
الحساب (نوع البيانات المراد تخزينها في حقل بيانات الحساب).

### !user

يحتوي حقل _rs`user`_ على _rs`Pubkey`_ يحدد المستخدم الذي أنشأ حساب الرسالة.

### !message

يحتفظ حقل _rs`message`_ بـ _rs`String`_ يحتوي على رسالة المستخدم.

### !bump

يخزن حقل _rs`bump`_ قيمة _rs`u8`_ ["bump" seed](/docs/core/pda#canonical-bump)
المستخدمة لاشتقاق عنوان مشتق من البرنامج (PDA). يوفر تخزين هذه القيمة الحوسبة من
خلال إلغاء الحاجة إلى إعادة حسابها في التعليمات اللاحقة.

</WithNotes>

<Accordions>
<Accordion title="Diff">

```diff
- #[account]
- pub struct MessageAccount {}

+ #[account]
+ pub struct MessageAccount {
+    pub user: Pubkey,
+    pub message: String,
+    pub bump: u8,
+ }
```

</Accordion>
<Accordion title="Explanation">

تقوم سمة _rs`#[account]`_ في برنامج Anchor بتعليق البنيات التي تمثل بيانات
الحساب (نوع البيانات المراد تخزينها في حقل بيانات الحساب).

في هذا المثال، تخزن بنية _rs`MessageAccount`_ رسالة أنشأها المستخدمون وتحتوي على
ثلاثة حقول:

- `user` - _rs`Pubkey`_ يحدد المستخدم الذي أنشأ حساب الرسالة.
- `message` - _rs`String`_ يحتوي على رسالة المستخدم.
- `bump` - _rs`u8`_ يخزن ["bump" seed](/docs/core/pda#canonical-bump) لاشتقاق
  العنوان المشتق من البرنامج (PDA). يوفر تخزين هذه القيمة الحوسبة من خلال إلغاء
  الحاجة إلى إعادة حسابها في التعليمات اللاحقة.

عند إنشاء حساب، يقوم البرنامج بتسلسل بيانات _rs`MessageAccount`_ ويخزنها في حقل
بيانات الحساب الجديد.

لاحقاً، عند القراءة من الحساب، يقوم البرنامج بإلغاء تسلسل هذه البيانات مرة أخرى
إلى نوع بيانات _rs`MessageAccount`_. يوضح قسم الاختبار عملية إنشاء وقراءة بيانات
الحساب.

</Accordion>
</Accordions>

قم ببناء البرنامج مرة أخرى عن طريق تشغيل _shell`build`_ في الطرفية.

```terminal
$ build
```

يحدد هذا الكود البيانات التي سيتم تخزينها في حساب الرسالة. بعد ذلك، ستقوم بإضافة
تعليمات البرنامج.

</Step>
<Step>

### إضافة تعليمة الإنشاء

الآن، أضف تعليمة _rs`create`_ التي تنشئ وتهيئ _rs`MessageAccount`_.

ابدأ بتحديد الحسابات المطلوبة للتعليمة عن طريق تحديث بنية _rs`Create`_ بما يلي:

<WithNotes>

```rs title="lib.rs"
#[derive(Accounts)]
// !tooltip[/instruction/] instruction
#[instruction(message: String)]
pub struct Create<'info> {
    // !tooltip[/mut/] mut
    #[account(mut)]
    // !tooltip[/Signer<'info>/] signer
    pub user: Signer<'info>,

    #[account(
        // !tooltip[/init/] init
        init,
        // !tooltip[/seeds/] seeds
        seeds = [b"message", user.key().as_ref()],
        // !tooltip[/bump/] bump
        bump,
        // !tooltip[/payer/] payer
        payer = user,
        // !tooltip[/space/] space
        space = 8 + 32 + 4 + message.len() + 1
    )]
    // !tooltip[/Account<'info, MessageAccount>/] account
    pub message_account: Account<'info, MessageAccount>,
    // !tooltip[/Program<'info, System>/] program
    pub system_program: Program<'info, System>,
}
```

### !mut

قيد _rs`mut`_ يعلن الحساب على أنه قابل للتعديل.

### !init

قيد _rs`init`_ ينشئ حساباً جديداً.

### !seeds

قيد _rs`seeds`_ يحدد المدخلات الاختيارية المستخدمة لاشتقاق عنوان البرنامج
المشتق.

### !bump

قيد _rs`bump`_ يعلن bump seed لعنوان البرنامج المشتق.

إذا لم تحدد قيمة، فسيقوم Anchor بحسابها تلقائيًا.

### !payer

قيد _rs`payer`_ يحدد الحساب الذي يدفع مقابل إنشاء الحساب الجديد.

### !space

قيد _rs`space`_ يحدد عدد البايتات المخصصة لحقل بيانات الحساب الجديد.

### !signer

نوع _rs`Signer<'info>`_ يتطلب أن يوقع الحساب على المعاملة.

### !account

نوع _rs`Account<'info, T>`_ يتطلب أن يطابق الحساب النوع المحدد.

في هذه الحالة، يجب أن يطابق الحساب نوع _rs`MessageAccount`_ المخصص.

### !program

نوع _rs`Program<'info, T>`_ يتطلب أن يطابق الحساب برنامجاً.

في هذه الحالة، يجب أن يطابق الحساب نوع _rs`System`_، والذي يشير إلى System
Program.

### !instruction

التعليق التوضيحي _rs`#[instruction(message: String)]`_ يتيح لبنية _rs`Create`_
الوصول إلى المعلمة _rs`message`_ من تعليمة `create`.

</WithNotes>

<Accordions>
<Accordion title="Diff">

```diff
- #[derive(Accounts)]
- pub struct Create {}

+ #[derive(Accounts)]
+ #[instruction(message: String)]
+ pub struct Create<'info> {
+     #[account(mut)]
+     pub user: Signer<'info>,
+
+     #[account(
+         init,
+         seeds = [b"message", user.key().as_ref()],
+         bump,
+         payer = user,
+         space = 8 + 32 + 4 + message.len() + 1
+     )]
+     pub message_account: Account<'info, MessageAccount>,
+     pub system_program: Program<'info, System>,
+ }
```

</Accordion>
<Accordion title="Explanation">

تُستخدم سمة _rs`#[derive(Accounts)]`_ في برنامج Anchor لتوصيف البنيات التي تحدد
الحسابات المطلوبة لتعليمة معينة.

يمثل كل حقل في البنية حسابًا يتم التحقق منه بطريقتين:

1. نوع الحساب (مثل _rs`Signer<'info>`_ أو _rs`Account<'info, T>`_) الذي يحدد نوع
   الحساب الذي يتوقعه البرنامج
2. قيود اختيارية (مثل _rs`#[account(mut)]`_ أو _rs`#[account(init)]`_) تحدد
   متطلبات إضافية

معًا، تمكن هذه الميزات Anchor من التحقق تلقائيًا من الحسابات المرسلة إلى
التعليمة وتأمين البرنامج.

توفر أسماء الحقول في البنية إمكانية الوصول إلى الحسابات في كود البرنامج الخاص
بك، لكنها لا تؤثر على التحقق. يجب استخدام أسماء وصفية للوضوح.

في هذا المثال، تحدد بنية _rs`Create`_ الحسابات المطلوبة لتعليمة _rs`create`_.

1. _rs`user: Signer<'info>`_
   - يمثل المستخدم الذي ينشئ حساب الرسالة
   - يحتاج إلى حالة قابلة للتعديل (_rs`#[account(mut)]`_) لأنه يدفع تكلفة الحساب
     الجديد
   - يجب أن يوقع على المعاملة للموافقة على خصم lamports من هذا الحساب

2. _rs`message_account: Account<'info, MessageAccount>`_
   - الحساب الجديد الذي يخزن رسالة المستخدم
   - يقوم قيد `init` بإنشاء الحساب أثناء تنفيذ التعليمة
   - يشتق قيدا `seeds` و `bump` عنوان الحساب كعنوان مشتق من البرنامج (Program
     Derived Address)
   - يحدد `payer = user` من يدفع تكلفة إنشاء الحساب الجديد
   - يخصص `space` البايتات المطلوبة لحقل بيانات الحساب

3. _rs`system_program: Program<'info, System>`_
   - ضروري لإنشاء الحساب
   - خلف الكواليس، يستدعي قيد `init` System Program لإنشاء حساب جديد بـ `space`
     المحدد ويغير المالك إلى البرنامج الحالي.

---

تتيح تعليقة _rs`#[instruction(message: String)]`_ لبنية _rs`Create`_ الوصول إلى
معلمة _rs`message`_ من تعليمة `create`.

---

يحدد قيدا `seeds` و `bump` معًا عنوان الحساب كعنوان مشتق من البرنامج (Program
Derived Address).

```rs title="lib.rs"
seeds = [b"message", user.key().as_ref()],
bump,
```

يحدد قيد `seeds` المدخلات الاختيارية المستخدمة لاشتقاق الـ PDA.

- _rs`b"message"`_ - نص ثابت كـ seed الأول.
- _rs`user.key().as_ref()`_ - المفتاح العام لحساب _rs`user`_ كـ seed الثاني.

يخبر قيد `bump` إطار Anchor بالعثور تلقائياً على bump seed الصحيح واستخدامه.
يستخدم Anchor `seeds` و `bump` لاشتقاق الـ PDA.

---

حساب `space` _rs`(8 + 32 + 4 + message.len() + 1)`_ يخصص مساحة لنوع بيانات
`MessageAccount`:

- معرّف حساب Anchor (discriminator): 8 بايتات
- عنوان المستخدم (_rs`Pubkey`_): 32 بايت
- رسالة المستخدم (_rs`String`_): 4 بايتات للطول + طول الرسالة المتغير
- PDA bump seed (_rs`u8`_): 1 بايت

```rs title="lib.rs"
#[account]
pub struct MessageAccount {
    pub user: Pubkey,
    pub message: String,
    pub bump: u8,
}
```

تحتاج جميع الحسابات التي تم إنشاؤها من خلال برنامج Anchor إلى 8 بايت لمميز
الحساب، والذي يعمل كمعرف لنوع الحساب الذي ينشئه Anchor تلقائيًا عند إنشاء
الحساب.

يحتاج نوع _rs`String`_ إلى 4 بايتات لتخزين طول النص، ويحتوي الطول المتبقي على
البيانات الفعلية.

</Accordion>
</Accordions>

بعد ذلك، أضف منطق العمل لتعليمة _rs`create`_ عن طريق تحديث دالة `create` بما
يلي:

```rs title="lib.rs"
pub fn create(ctx: Context<Create>, message: String) -> Result<()> {
    msg!("Create Message: {}", message);
    let account_data = &mut ctx.accounts.message_account;
    account_data.user = ctx.accounts.user.key();
    account_data.message = message;
    account_data.bump = ctx.bumps.message_account;
    Ok(())
}
```

<Accordions>
<Accordion title="Diff">

```diff
- pub fn create(_ctx: Context<Create>) -> Result<()> {
-     Ok(())
- }

+ pub fn create(ctx: Context<Create>, message: String) -> Result<()> {
+     msg!("Create Message: {}", message);
+     let account_data = &mut ctx.accounts.message_account;
+     account_data.user = ctx.accounts.user.key();
+     account_data.message = message;
+     account_data.bump = ctx.bumps.message_account;
+     Ok(())
+ }
```

</Accordion>
<Accordion title="Explanation">

تنفذ دالة `create` المنطق الخاص بتهيئة بيانات حساب رسالة جديد. وهي تأخذ معلمتين:

1. _rs`ctx: Context<Create>`_ - توفر الوصول إلى الحسابات المحددة في بنية
   _rs`Create`_.
2. _rs`message: String`_ - رسالة المستخدم للتخزين.

ثم ينفذ جسم الدالة المنطق التالي:

1. طباعة رسالة في سجلات البرنامج باستخدام وحدة الماكرو _rs`msg!()`_.

   ```rs
   msg!("Create Message: {}", message);
   ```

2. تهيئة بيانات الحساب:
   - الوصول إلى `message_account` من السياق.

   ```rs
   let account_data = &mut ctx.accounts.message_account;
   ```

   - يضبط حقل `user` على المفتاح العام لحساب `user`.

   ```rs
   account_data.user = ctx.accounts.user.key();
   ```

   - يضبط حقل `message` على `message` من معامل الوظيفة.

   ```rs
   account_data.message = message;
   ```

   - يضبط قيمة `bump` المستخدمة لاشتقاق PDA، والتي يتم استرجاعها من
     `ctx.bumps.message_account`.

   ```rs
   account_data.bump = ctx.bumps.message_account;
   ```

</Accordion>
</Accordions>

أعد بناء البرنامج.

```terminal
$ build
```

</Step>
<Step>

### إضافة تعليمة التحديث

بعد ذلك، أضف تعليمة `update` لتغيير `MessageAccount` برسالة جديدة.

مثل الخطوة السابقة، حدد أولاً الحسابات المطلوبة بواسطة تعليمة `update`.

قم بتحديث بنية `Update` بما يلي:

<WithNotes>

```rs title="lib.rs"
#[derive(Accounts)]
#[instruction(message: String)]
pub struct Update<'info> {
    #[account(mut)]
    pub user: Signer<'info>,

    #[account(
        mut,
        seeds = [b"message", user.key().as_ref()],
        bump = message_account.bump,
        // !tooltip[/realloc/] realloc
        realloc = 8 + 32 + 4 + message.len() + 1,
        // !tooltip[/realloc::payer/] realloc::payer
        realloc::payer = user,
        // !tooltip[/realloc::zero/] realloc::zero
        realloc::zero = true,
    )]
    pub message_account: Account<'info, MessageAccount>,
    pub system_program: Program<'info, System>,
}
```

### !realloc

قيد _rs`realloc`_ يعيد تخصيص بيانات الحساب.

### !realloc::payer

قيد _rs`realloc::payer`_ يحدد الحساب الذي يدفع مقابل إعادة التخصيص.

### !realloc::zero

قيد _rs`realloc::zero`_ يصفّر بيانات الحساب.

</WithNotes>

<Accordions>
<Accordion title="Diff">

```diff
- #[derive(Accounts)]
- pub struct Update {}

+ #[derive(Accounts)]
+ #[instruction(message: String)]
+ pub struct Update<'info> {
+     #[account(mut)]
+     pub user: Signer<'info>,
+
+     #[account(
+         mut,
+         seeds = [b"message", user.key().as_ref()],
+         bump = message_account.bump,
+         realloc = 8 + 32 + 4 + message.len() + 1,
+         realloc::payer = user,
+         realloc::zero = true,
+     )]
+     pub message_account: Account<'info, MessageAccount>,
+     pub system_program: Program<'info, System>,
+ }
```

</Accordion>
<Accordion title="Explanation">

بنية _rs`Update`_ تحدد الحسابات المطلوبة لتعليمة _rs`update`_.

1. _rs`user: Signer<'info>`_
   - يمثل المستخدم الذي يقوم بتحديث حساب الرسالة
   - مُعلّم كقابل للتعديل (_rs`#[account(mut)]`_) لأنه قد يدفع مقابل مساحة
     إضافية لـ `message_account` عند الحاجة
   - يجب أن يوقع على المعاملة

2. _rs`message_account: Account<'info, MessageAccount>`_
   - الحساب الموجود الذي يخزن رسالة المستخدم للتحديث
   - قيد `mut` يشير إلى تعديل البيانات لهذا الحساب
   - قيد `realloc` يسمح بتغيير حجم بيانات الحساب
   - قيود `seeds` و `bump` تتحقق من الحساب كـ PDA صحيح

3. _rs`system_program: Program<'info, System>`_
   - مطلوب لإعادة تخصيص مساحة الحساب المحتملة
   - قيد `realloc` يستدعي System Program لضبط حجم بيانات الحساب

لاحظ أن قيد _rs`bump = message_account.bump`_ يستخدم bump seed المخزن على
_rs`message_account`_، بدلاً من جعل Anchor يعيد حسابه.

تتيح سمة _rs`#[instruction(message: String)]`_ لبنية _rs`Update`_ الوصول إلى
معلمة _rs`message`_ من تعليمة _rs`update`_.

</Accordion>
</Accordions>

بعد ذلك، أضف المنطق لتعليمة `update`.

```rs title="lib.rs"
pub fn update(ctx: Context<Update>, message: String) -> Result<()> {
    msg!("Update Message: {}", message);
    let account_data = &mut ctx.accounts.message_account;
    account_data.message = message;
    Ok(())
}
```

<Accordions>
<Accordion title="Diff">

```diff
- pub fn update(_ctx: Context<Update>) -> Result<()> {
-     Ok(())
- }

+ pub fn update(ctx: Context<Update>, message: String) -> Result<()> {
+     msg!("Update Message: {}", message);
+     let account_data = &mut ctx.accounts.message_account;
+     account_data.message = message;
+     Ok(())
+ }
```

</Accordion>
<Accordion title="Explanation">

تقوم دالة `update` بتنفيذ المنطق الخاص بتعديل حساب رسالة موجود. وهي تأخذ
معلمتين:

1. _rs`ctx: Context<Update>`_ - توفر الوصول إلى الحسابات المحددة في بنية
   _rs`Update`_.
2. _rs`message: String`_ - الرسالة الجديدة لاستبدال الرسالة الموجودة.

ثم يقوم جسم الدالة بما يلي:

1. طباعة رسالة إلى سجلات البرنامج باستخدام وحدة الماكرو _rs`msg!()`_.

2. تحديث بيانات الحساب:
   - الوصول إلى `message_account` من السياق.
   - تعيين حقل `message` إلى `message` الجديد من معلمة الدالة.

</Accordion>
</Accordions>

أعد بناء البرنامج

```terminal
$ build
```

</Step>
<Step>

### إضافة تعليمة الحذف

بعد ذلك، أضف تعليمة _rs`delete`_ لإغلاق _rs`MessageAccount`_.

قم بتحديث بنية _rs`Delete`_ بما يلي:

<WithNotes>

```rs title="lib.rs"
#[derive(Accounts)]
pub struct Delete<'info> {
    #[account(mut)]
    pub user: Signer<'info>,

    #[account(
        mut,
        // !tooltip[/seeds/] seeds
        seeds = [b"message", user.key().as_ref()],
        // !tooltip[/bump/] bump
        bump = message_account.bump,
        // !tooltip[/close/] close
        close = user,
    )]
    pub message_account: Account<'info, MessageAccount>,
}
```

### !seeds

يحدد قيد _rs`seeds`_ البذور المستخدمة لاشتقاق الـ PDA.

### !bump

يحدد قيد _rs`bump`_ bump seed للـ PDA.

في هذه الحالة، يستخدم البرنامج bump seed الموجود المخزن على
_rs`message_account`_.

### !close

يقوم قيد _rs`close`_ بإغلاق الحساب.

في هذه الحالة، يتلقى حساب _rs`user`_ وحدات lamport من _rs`message_account`_
المُغلق.

</WithNotes>

<Accordions>
<Accordion title="Diff">

```diff
- #[derive(Accounts)]
- pub struct Delete {}

+ #[derive(Accounts)]
+ pub struct Delete<'info> {
+     #[account(mut)]
+     pub user: Signer<'info>,
+
+     #[account(
+         mut,
+         seeds = [b"message", user.key().as_ref()],
+         bump = message_account.bump,
+         close = user,
+     )]
+     pub message_account: Account<'info, MessageAccount>,
+ }
```

</Accordion>
<Accordion title="Explanation">

تحدد بنية _rs`Delete`_ الحسابات المطلوبة لتعليمة _rs`delete`_:

1. _rs`user: Signer<'info>`_
   - يمثل المستخدم الذي يغلق حساب الرسالة
   - محدد كقابل للتعديل (_rs`#[account(mut)]`_) لاستلام اللامبورتات من الحساب
     المغلق
   - يجب أن يوقع للتأكد من أن المستخدم الصحيح فقط يمكنه إغلاق حساب رسالته

2. _rs`message_account: Account<'info, MessageAccount>`_
   - الحساب المراد إغلاقه
   - يشير قيد `mut` إلى تعديل البيانات
   - يتحقق قيدا `seeds` و `bump` من أن الحساب هو العنوان المشتق من البرنامج
     (PDA) الصحيح
   - يحدد قيد `close = user` هذا الحساب للإغلاق وينقل لامبورتاته إلى حساب `user`

</Accordion>
</Accordions>

بعد ذلك، أضف المنطق لتعليمة `delete`.

```rs title="lib.rs"
pub fn delete(_ctx: Context<Delete>) -> Result<()> {
    msg!("Delete Message");
    Ok(())
}
```

<Accordions>
<Accordion title="Diff">

```diff
- pub fn delete(_ctx: Context<Delete>) -> Result<()> {
-     Ok(())
- }

+ pub fn delete(_ctx: Context<Delete>) -> Result<()> {
+     msg!("Delete Message");
+     Ok(())
+ }
```

</Accordion>
<Accordion title="Explanation">

تأخذ دالة `delete` معلمة واحدة:

1. _rs`_ctx: Context<Delete>`_ - توفر الوصول إلى الحسابات المحددة في بنية
   _rs`Delete`_. يوضح تركيب _rs`_ctx`_ أن الدالة لا تستخدم السياق (Context) في
   جسمها.

يقوم جسم الدالة فقط بطباعة رسالة إلى سجلات البرنامج باستخدام ماكرو _rs`msg!()`_.
لا تحتاج الدالة إلى منطق إضافي لأن قيد _rs`close`_ في بنية _rs`Delete`_ يتعامل
مع إغلاق الحساب.

</Accordion>
</Accordions>

أعد بناء البرنامج.

```terminal
$ build
```

</Step>
<Step>

### نشر البرنامج

لقد أكملت الآن برنامج CRUD الأساسي. انشر البرنامج عن طريق تشغيل `deploy` في
طرفية Playground.

<Callout type="info">
في هذا المثال، ستقوم بنشر البرنامج على devnet، وهي مجموعة سولانا
مخصصة لاختبار التطوير.

تتصل محفظة Playground بـ devnet افتراضيًا. تأكد من أن محفظة Playground الخاصة بك
تحتوي على SOL من devnet لدفع تكاليف نشر البرنامج. احصل على SOL من devnet من
[صنبور سولانا](https://faucet.solana.com/).

</Callout>

```terminal
$ deploy
Deploying... This could take a while depending on the program size and network conditions.
Deployment successful. Completed in 17s.
```

</Step>
<Step>

### إعداد ملف الاختبار

يتضمن الكود الأولي أيضًا ملف اختبار في `anchor.test.ts`.

```ts title="anchor.test.ts"
import { PublicKey } from "@solana/web3.js";

describe("pda", () => {
  it("Create Message Account", async () => {});

  it("Update Message Account", async () => {});

  it("Delete Message Account", async () => {});
});
```

أضف الكود أدناه داخل _ts`describe()`_، ولكن قبل أقسام _ts`it()`_.

```ts title="anchor.test.ts"
const program = pg.program;
const wallet = pg.wallet;

const [messagePda, messageBump] = PublicKey.findProgramAddressSync(
  [Buffer.from("message"), wallet.publicKey.toBuffer()],
  program.programId
);
```

<Accordions>
<Accordion title="Diff">

```diff
  import { PublicKey } from "@solana/web3.js";

  describe("pda", () => {
+    const program = pg.program;
+    const wallet = pg.wallet;
+
+    const [messagePda, messageBump] = PublicKey.findProgramAddressSync(
+      [Buffer.from("message"), wallet.publicKey.toBuffer()],
+      program.programId
+    );

    it("Create Message Account", async () => {});

    it("Update Message Account", async () => {});

    it("Delete Message Account", async () => {});
  });
```

</Accordion>
<Accordion title="Explanation">

في هذا القسم، يقوم هذا الكود ببساطة بإعداد ملف الاختبار.

<WithMentions>

تزيل Solana Playground بعض الإعدادات الأولية حيث يتيح
[`pg.program`](mention:one) الوصول إلى طرق التفاعل مع البرنامج، بينما يوفر
[`pg.wallet`](mention:two) الوصول إلى محفظة playground الخاصة بك.

```ts title="anchor.test.ts"
// !mention one
const program = pg.program;
// !mention two
const wallet = pg.wallet;
```

</WithMentions>

كجزء من الإعداد، يقوم ملف الاختبار باشتقاق حساب الرسالة PDA. هذا يوضح كيفية
اشتقاق PDA في جافاسكريبت باستخدام نفس البذور المحددة في البرنامج.

```ts title="anchor.test.ts"
const [messagePda, messageBump] = PublicKey.findProgramAddressSync(
  [Buffer.from("message"), wallet.publicKey.toBuffer()],
  program.programId
);
```

</Accordion>
</Accordions>

قم بتشغيل ملف الاختبار عن طريق تنفيذ _shell`test`_ في طرفية Playground للتحقق من
أنه يعمل كما هو متوقع. تضيف الخطوات التالية الاختبارات الفعلية.

```terminal
$ test
Running tests...
  anchor.test.ts:
  pda
    ✔ Create Message Account
    ✔ Update Message Account
    ✔ Delete Message Account
  3 passing (4ms)
```

</Step>
<Step>

### استدعاء تعليمة الإنشاء

<WithNotes>

قم بتحديث الاختبار الأول بما يلي:

```ts title="anchor.test.ts"
it("Create Message Account", async () => {
  const message = "Hello, World!";
  const transactionSignature = await program.methods
    // !tooltip[/create/] create
    .create(message)
    // !tooltip[/accounts/] accounts
    .accounts({
      messageAccount: messagePda
    })
    // !tooltip[/rpc/] rpc
    .rpc({ commitment: "confirmed" });

  // !tooltip[/fetch/] fetch
  const messageAccount = await program.account.messageAccount.fetch(
    messagePda,
    "confirmed"
  );

  console.log(JSON.stringify(messageAccount, null, 2));
  console.log(
    "Transaction Signature:",
    `https://solana.fm/tx/${transactionSignature}?cluster=devnet-solana`
  );
});
```

### !create

تستدعي طريقة _ts`create()`_ تعليمة `create`.

### !accounts

تحدد طريقة _ts`accounts()`_ الحسابات المطلوبة لتعليمة _ts`create()`_.

### !rpc

ترسل طريقة _ts`rpc()`_ المعاملة إلى الشبكة.

### !fetch

تسترجع طريقة _ts`fetch()`_ بيانات الحساب من الشبكة.

</WithNotes>

<Accordions>
<Accordion title="Diff">

```diff
- it("Create Message Account", async () => {});

+ it("Create Message Account", async () => {
+   const message = "Hello, World!";
+   const transactionSignature = await program.methods
+     .create(message)
+     .accounts({
+       messageAccount: messagePda,
+     })
+     .rpc({ commitment: "confirmed" });
+
+   const messageAccount = await program.account.messageAccount.fetch(
+     messagePda,
+     "confirmed"
+   );
+
+   console.log(JSON.stringify(messageAccount, null, 2));
+   console.log(
+     "Transaction Signature:",
+     `https://solana.fm/tx/${transactionSignature}?cluster=devnet-solana`
+   );
+ });
```

</Accordion>
<Accordion title="Explanation">

أولاً، يرسل ملف الاختبار معاملة تستدعي تعليمة `create`، ممرراً "Hello, World!"
كرسالة.

```ts title="anchor.test.ts"
const message = "Hello, World!";
const transactionSignature = await program.methods
  .create(message)
  .accounts({
    messageAccount: messagePda
  })
  .rpc({ commitment: "confirmed" });
```

بعد إرسال المعاملة وإنشاء الحساب، يسترجع ملف الاختبار الحساب باستخدام عنوانه
(`messagePda`).

```ts title="anchor.test.ts"
const messageAccount = await program.account.messageAccount.fetch(
  messagePda,
  "confirmed"
);
```

أخيرًا، يقوم ملف الاختبار بتسجيل بيانات الحساب ورابط لتفاصيل المعاملة.

```ts title="anchor.test.ts"
console.log(JSON.stringify(messageAccount, null, 2));
console.log(
  "Transaction Signature:",
  `https://solana.fm/tx/${transactionSignature}?cluster=devnet-solana`
);
```

</Accordion>
</Accordions>

</Step>
<Step>

### استدعاء تعليمة التحديث

<WithNotes>
قم بتحديث الاختبار الثاني بما يلي:

```ts title="anchor.test.ts"
it("Update Message Account", async () => {
  const message = "Hello, Solana!";
  const transactionSignature = await program.methods
    // !tooltip[/update/] update
    .update(message)
    // !tooltip[/accounts/] accounts
    .accounts({
      messageAccount: messagePda
    })
    // !tooltip[/rpc/] rpc
    .rpc({ commitment: "confirmed" });

  // !tooltip[/fetch/] fetch
  const messageAccount = await program.account.messageAccount.fetch(
    messagePda,
    "confirmed"
  );

  console.log(JSON.stringify(messageAccount, null, 2));
  console.log(
    "Transaction Signature:",
    `https://solana.fm/tx/${transactionSignature}?cluster=devnet-solana`
  );
});
```

### !update

تستدعي طريقة _ts`update()`_ تعليمة `update`.

### !accounts

تحدد طريقة _ts`accounts()`_ الحسابات المطلوبة لتعليمة _ts`update()`_.

### !rpc

ترسل طريقة _ts`rpc()`_ المعاملة إلى الشبكة.

### !fetch

تسترجع طريقة _ts`fetch()`_ بيانات الحساب من الشبكة.

</WithNotes>

<Accordions>
<Accordion title="Diff">

```diff
- it("Update Message Account", async () => {});

+ it("Update Message Account", async () => {
+   const message = "Hello, Solana!";
+   const transactionSignature = await program.methods
+     .update(message)
+     .accounts({
+       messageAccount: messagePda,
+     })
+     .rpc({ commitment: "confirmed" });
+
+   const messageAccount = await program.account.messageAccount.fetch(
+     messagePda,
+     "confirmed"
+   );
+
+   console.log(JSON.stringify(messageAccount, null, 2));
+   console.log(
+     "Transaction Signature:",
+     `https://solana.fm/tx/${transactionSignature}?cluster=devnet-solana`
+   );
+ });
```

</Accordion>
<Accordion title="Explanation">

أولاً، يرسل ملف الاختبار معاملة تستدعي تعليمة `update`، ممرراً "Hello, Solana!"
كرسالة جديدة.

```ts title="anchor.test.ts"
const message = "Hello, Solana!";
const transactionSignature = await program.methods
  .update(message)
  .accounts({
    messageAccount: messagePda
  })
  .rpc({ commitment: "confirmed" });
```

بعد إرسال المعاملة وتحديث الحساب، يقوم ملف الاختبار بجلب الحساب باستخدام عنوانه
(`messagePda`).

```ts title="anchor.test.ts"
const messageAccount = await program.account.messageAccount.fetch(
  messagePda,
  "confirmed"
);
```

أخيرًا، يقوم ملف الاختبار بتسجيل بيانات الحساب ورابط لتفاصيل المعاملة.

```ts title="anchor.test.ts"
console.log(JSON.stringify(messageAccount, null, 2));
console.log(
  "Transaction Signature:",
  `https://solana.fm/tx/${transactionSignature}?cluster=devnet-solana`
);
```

</Accordion>
</Accordions>

</Step>
<Step>

### استدعاء تعليمة الحذف

<WithNotes>

قم بتحديث الاختبار الثالث بما يلي:

```ts title="anchor.test.ts"
it("Delete Message Account", async () => {
  const transactionSignature = await program.methods
    // !tooltip[/delete/] delete
    .delete()
    // !tooltip[/accounts/] accounts
    .accounts({
      messageAccount: messagePda
    })
    // !tooltip[/rpc/] rpc
    .rpc({ commitment: "confirmed" });

  // !tooltip[/fetchNullable/] fetchNullable
  const messageAccount = await program.account.messageAccount.fetchNullable(
    messagePda,
    "confirmed"
  );

  console.log("Expect Null:", JSON.stringify(messageAccount, null, 2));
  console.log(
    "Transaction Signature:",
    `https://solana.fm/tx/${transactionSignature}?cluster=devnet-solana`
  );
});
```

### !delete

تقوم طريقة _ts`delete()`_ باستدعاء التعليمة `delete`.

### !accounts

تقوم طريقة _ts`accounts()`_ بتحديد الحسابات المطلوبة للتعليمة _ts`delete()`_.

### !rpc

تقوم طريقة _ts`rpc()`_ بإرسال المعاملة إلى الشبكة.

### !fetchNullable

تقوم طريقة _ts`fetchNullable()`_ باسترجاع بيانات الحساب من الشبكة التي قد لا
تكون موجودة.

يستخدم ملف الاختبار هذا لأن الحساب يُغلق وتصبح البيانات غير متاحة.

</WithNotes>

<Accordions>
<Accordion title="Diff">

```diff
- it("Delete Message Account", async () => {});

+ it("Delete Message Account", async () => {
+   const transactionSignature = await program.methods
+     .delete()
+     .accounts({
+       messageAccount: messagePda,
+     })
+     .rpc({ commitment: "confirmed" });
+
+   const messageAccount = await program.account.messageAccount.fetchNullable(
+     messagePda,
+     "confirmed"
+   );
+
+   console.log("Expect Null:", JSON.stringify(messageAccount, null, 2));
+   console.log(
+     "Transaction Signature:",
+     `https://solana.fm/tx/${transactionSignature}?cluster=devnet-solana`
+   );
+ });
```

</Accordion>
<Accordion title="Explanation">

أولاً، يرسل هذا الكود معاملة تستدعي التعليمة `delete` لإغلاق حساب الرسالة.

```ts title="anchor.test.ts"
const transactionSignature = await program.methods
  .delete()
  .accounts({
    messageAccount: messagePda
  })
  .rpc({ commitment: "confirmed" });
```

بعد إرسال المعاملة وإغلاق الحساب، يحاول المثال جلب الحساب باستخدام عنوانه
(`messagePda`) مع `fetchNullable`. تُرجع هذه الطريقة null عندما لم يعد الحساب
موجوداً بعد الإغلاق.

```ts title="anchor.test.ts"
const messageAccount = await program.account.messageAccount.fetchNullable(
  messagePda,
  "confirmed"
);
```

أخيرًا، يقوم ملف الاختبار بتسجيل بيانات الحساب ورابط لتفاصيل المعاملة. تظهر
بيانات الحساب كـ null لأن الحساب لم يعد موجودًا.

```ts title="anchor.test.ts"
console.log(JSON.stringify(messageAccount, null, 2));
console.log(
  "Transaction Signature:",
  `https://solana.fm/tx/${transactionSignature}?cluster=devnet-solana`
);
```

</Accordion>
</Accordions>

</Step>
<Step>

### تشغيل الاختبار

بعد إعداد اختباراتك، قم بتشغيل ملف الاختبار باستخدام _shell`test`_ في طرفية
Playground. يقوم هذا الأمر بتشغيل الاختبارات على البرنامج المنشور على devnet
ويسجل روابط إلى SolanaFM لعرض تفاصيل المعاملة.

```terminal
$ test
Running tests...
  anchor.test.ts:
  pda
    {
  "user": "3z9vL1zjN6qyAFHhHQdWYRTFAcy69pJydkZmSFBKHg1R",
  "message": "Hello, World!",
  "bump": 254
}
    Transaction Signature: https://solana.fm/tx/5oBT4jEdUR6CRYsFNGoqvyMBTRDvFqRWTAAmCGM9rEvYRBWy3B2bkb6GVFpVPKBnkr714UCFUurBSDKSa7nLHo8e?cluster=devnet-solana
    ✔ Create Message Account (1025ms)
    {
  "user": "3z9vL1zjN6qyAFHhHQdWYRTFAcy69pJydkZmSFBKHg1R",
  "message": "Hello, Solana!",
  "bump": 254
}
    Transaction Signature: https://solana.fm/tx/42veGAsQjHbJP1SxWBGcfYF7EdRN9X7bACNv23NSZNe4U7w2dmaYgSv8UUWXYzwgJPoNHejhtWdKZModHiMaTWYK?cluster=devnet-solana
    ✔ Update Message Account (713ms)
    Expect Null: null
    Transaction Signature: https://solana.fm/tx/Sseog2i2X7uDEn2DyDMMJKVHeZEzmuhnqUwicwGhnGhstZo8URNwUZgED8o6HANiojJkfQbhXVbGNLdhsFtWrd6?cluster=devnet-solana
    ✔ Delete Message Account (812ms)
  3 passing (3s)
```

افحص روابط SolanaFM لعرض تفاصيل المعاملة.

<Callout type="info">
  لاحظ أنه في هذا المثال، إذا قمت بتشغيل الاختبار مرة أخرى، فإن التعليمة
  `create` ستفشل لأن `messageAccount` موجود بالفعل كحساب. يمكن أن يوجد حساب واحد
  فقط لعنوان PDA معين.
</Callout>

</Step>
</Steps>
