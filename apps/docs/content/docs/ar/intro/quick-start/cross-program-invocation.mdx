...
أولاً، يحتاج البرنامج إلى آلية بسيطة "ادفع للتحديث" من خلال تغيير بنية
_rs`Update`_ ودالة `update`.

ابدأ بتحديث ملف `lib.rs` لإدراج العناصر من وحدة `system_program` في النطاق.
...
بعد ذلك، قم بتحديث بنية _rs`Update`_ لتشمل حساباً جديداً يسمى `vault_account`.
هذا الحساب، الذي يتحكم فيه البرنامج، يستقبل SOL من المستخدم عندما يقوم بتحديث
حساب رسالته.
...
يضيف هذا القسم حساباً جديداً يسمى `vault_account` إلى بنية _rs`Update`_. يعمل
هذا الحساب كـ"خزنة" يتحكم فيها البرنامج وتستقبل SOL من المستخدمين عندما يقومون
بتحديث رسائلهم.
...
- يأتي عنوان الحساب من PDA مشتق باستخدام البذور
  _rs`[b"vault", user.key().as_ref()]`_
- بصفته PDA، ليس لديه مفتاح خاص، لذا يمكن للبرنامج فقط "التوقيع" على العنوان عند
  تنفيذ CPIs
- بصفته من نوع _rs`SystemAccount`_، يمتلكه System Program مثل حسابات المحفظة
  العادية
...
في تعليمة `delete`، سترى كيف يمكن للبرنامج "التوقيع" على هذا PDA في CPI.
...
بنية _rs`Transfer`_ تحدد الحسابات المطلوبة لتعليمة التحويل الخاصة بـ System
Program.

### !cpi_context

تحدد بنية _rs`CpiContext`_ البرنامج والحسابات لاستدعاء برنامج متقاطع (CPI).
...
في تعليمة `update`، يتضمن التنفيذ استدعاء برنامج متقاطع (CPI) لاستدعاء تعليمة
`transfer` الخاصة بـ System Program. يوضح هذا كيفية تنفيذ CPI من داخل البرنامج،
مما يتيح قابلية التركيب لبرامج سولانا.
...
- الحسابات المطلوبة في CPI (المحددة في بنية _rs`Transfer`_)
...
ثم تستدعي وظيفة `transfer` تعليمة التحويل على System Program، مع تمرير:

- `cpi_context` (البرنامج والحسابات)
- `amount` المراد تحويله (1,000,000 لامبورت، أو 0.001 SOL)
...
يتطابق إعداد CPI مع كيفية بناء التعليمات من جانب العميل، حيث تحدد البرنامج
والحسابات وبيانات التعليمة لتعليمة معينة لاستدعائها. عندما تتلقى تعليمة `update`
الخاصة بالبرنامج استدعاءً، فإنها تستدعي داخلياً تعليمة التحويل الخاصة بـ System
Program.
...
الآن أضف آلية "استرداد عند الحذف" عن طريق تغيير بنية _rs`Delete`_ ووظيفة
`delete`.

أولاً، قم بتحديث بنية _rs`Delete`_ لتشمل `vault_account`. يتيح هذا تحويل أي SOL
في الخزنة إلى المستخدم عندما يغلق حساب الرسالة الخاص به.
...
أضف أيضاً `system_program` حيث أن CPI للتحويل يتطلب استدعاء System Program.
...
`vault_account` يستخدم نفس اشتقاق PDA كما في بنية Update.

إضافة `vault_account` إلى بنية Delete تمكّن البرنامج من الوصول إلى حساب الخزنة
الخاص بالمستخدم أثناء تعليمة الحذف لتحويل أي SOL متراكم إلى المستخدم.
...
بعد ذلك، أضف منطق CPI في تعليمة `delete` لتحويل SOL من حساب الخزنة إلى حساب
المستخدم.
...
_rs`signer_seeds`_ تحدد البذور الاختيارية وبذور bump المستخدمة لاشتقاق PDA.

### !with_signer

طريقة _rs`with_signer()`_ تمرر بذور الموقّع مع CPI.
...
لاستخدام السياق في جسم الوظيفة.
...
- `cpi_context` (البرنامج والحسابات وموقّع PDA)
...
بعد ذلك، قم بتحديث ملف `anchor.test.ts` لتضمين حساب الخزنة الجديد في التعليمات.
يتطلب هذا اشتقاق PDA الخاص بالخزنة وتضمينه في استدعاءات تعليمات التحديث والحذف.
...
ثم، قم بتحديث تعليمة التحديث لتتضمن [`vaultAccount`](mention:one)
...
ثم، قم بتحديث تعليمة الحذف لتتضمن [`vaultAccount`](mention:one)
...
Playground. بمجرد استيراد المشروع، يتم حفظ جميع التغييرات تلقائياً والاحتفاظ
بها.
