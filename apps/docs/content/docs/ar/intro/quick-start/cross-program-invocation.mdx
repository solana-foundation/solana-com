...
- بصفته PDA، ليس لديه مفتاح خاص، لذا يمكن للبرنامج فقط "التوقيع" للعنوان عند
  تنفيذ CPIs
...
في تعليمة `delete`، سترى كيف يمكن للبرنامج "التوقيع" لهذا PDA في CPI.
...
بنية _rs`CpiContext`_ تحدد البرنامج والحسابات لاستدعاء برنامج متقاطع (CPI).

### !transfer

وظيفة _rs`transfer()`_ تستدعي تعليمة التحويل الخاصة بـ System Program.
...
ثم تقوم وظيفة `transfer` باستدعاء تعليمة التحويل على System Program، وتمرير:
...
إعداد CPI يطابق كيفية بناء التعليمات من جانب العميل، حيث تحدد البرنامج والحسابات
وبيانات التعليمة لتعليمة معينة لاستدعائها. عندما تتلقى تعليمة `update` الخاصة
بالبرنامج استدعاءً، فإنها تستدعي داخليًا تعليمة التحويل الخاصة بـ System
Program.
...
في الخزنة إلى المستخدم عندما يغلقون حساب رسالتهم.
