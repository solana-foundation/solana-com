...
<Callout type="info">
  هل تعمل بوضع headless — في مسار API أو worker أو سكريبت بدون محفظة متصفح؟
  استخدم خدمة إدارة مفاتيح خلفية بدلاً من keypair خام. راجع
  [Keychain](/docs/tools/keychain) و[التوقيع في بيئة
  الإنتاج](/docs/core/transactions/signing-in-production).
</Callout>

## أنماط شائعة لمطوري سولانا

- **آلات الحالة Headless**: شغّل العميل داخل مسارات API أو سكريبتات أو workers
  لإعادة استخدام منطق تنسيق المحفظة + RPC نفسه الذي يُشغّل واجهة المستخدم.
- **لوحات المعلومات في الوقت الفعلي**: ادمج المراقبين (الأرصدة، الحسابات،
  التوقيعات) مع مكتبة واجهة المستخدم المفضلة لديك؛ يتولى العميل توزيع websocket
  وإبطال ذاكرة التخزين المؤقت.
- **المخازن المخصصة**: أدخل مخزن Zustand الخاص بك لترطيب البيانات من
  IndexedDB/localStorage، أو مزامنة الحالة مع جلسات الخادم، أو التنسيق بين
  تبويبات المتصفح.
- **الجسر إلى React hooks**: مرّر نسخة عميل مُهيَّأة إلى `@solana/react-hooks`
  عندما تريد hooks مريحة فوق نفس بيئة التشغيل.
- **قابلية الاختبار**: يمكن محاكاة واجهة العميل الموحدة في اختبارات الوحدة، مما
  يُسهّل محاكاة استجابات RPC أو جلسات المحفظة دون الحاجة إلى محفظة متصفح.
