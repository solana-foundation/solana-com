...
title: مرجع JS API
description:
  واجهة API العامة الكاملة لحزمة @solana/surfpool — فئة Surfnet، وأنواع الإعدادات، وخيارات النشر، وشكل الأحداث.
---
...
<Callout type="info" title="نقطة دخول الحزمة">
  الرموز أدناه هي الصادرات الجذرية لـ `@solana/surfpool`. يضيف المسار الفرعي `@solana/surfpool/kit` كلاً من `surfpool()` و`surfnetCheatcodes()` و`createSurfnetCheatcodesRpc()` و`DEFAULT_SURFNET_ENDPOINT` وأنواع إعداد الإضافة والأنواع التلقائية لبروتوكول الأوامر — وجميعها موثقة في [إضافة Kit](/docs/tools/surfpool/sdk/kit-plugin) وليس في هذا الجدول.
</Callout>
