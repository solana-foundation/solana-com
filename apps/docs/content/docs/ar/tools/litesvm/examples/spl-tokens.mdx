...
description: كيفية كتابة الاختبارات باستخدام رموز SPL مع litesvm-token
---

<Callout>يتطلب `cargo add --dev litesvm-token spl-token`</Callout>
...
1. **litesvm-token**: حزمة مساعدة تُبسّط اختبار رموز SPL
2. **إنشاء العملة**: إنشاء عملة بعدد منازل عشرية وصلاحيات محددة
3. **token account**: إنشاء token account لحفظ الرموز للمستخدمين
   - لفهم الفرق بين هذا وبين ATA، اقرأ
     [هذا القسم](/docs/tools/litesvm/additional-crates/testing-with-spl-tokens#types-of-token-accounts).
4. **الإصدار**: إصدار الرموز إلى token account باستخدام صلاحية الإصدار
5. **المنازل العشرية**: تذكّر مراعاة المنازل العشرية عند تحديد المبالغ

## انظر أيضًا

- [الاختبار باستخدام رموز SPL](/docs/tools/litesvm/additional-crates/testing-with-spl-tokens) -
  دليل شامل لاختبار رموز SPL
