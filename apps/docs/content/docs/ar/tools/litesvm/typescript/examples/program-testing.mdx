...
يعني اختبار برنامج سولانا المخصص تحميل ملف `.so` المُجمَّع الخاص به إلى
SVM، وإعداد الحسابات التي يتوقعها، وإرسال التعليمات إليه.
يتولى LiteSVM كل ذلك داخل العملية — دون الحاجة إلى validator. نوصي
بإنشاء عملاء البرنامج باستخدام
...
للاختبارات التي تتطلب برامج قياسية:
...
2. **الإعداد**: تفعيل sysvars وbuiltins وprecompiles حسب الحاجة
3. **إعداد الحسابات**: ملء حسابات البيانات مسبقًا باستخدام `setAccount`
4. **الإرسال**: استخدم `client.myProgram.testInstruction({..}).sendTransaction()` —
   دون الحاجة إلى بناء المعاملات يدويًا
5. **التحقق**: فحص حالة الحساب بعد التنفيذ
   (`client.myProgram.myAccount.fetch`)

بمجرد أن يعمل هذا الهيكل الأساسي، يمكنك بناء مجموعة اختبارات كاملة عن طريق
إضافة المزيد من التعليمات والتأكيدات.
