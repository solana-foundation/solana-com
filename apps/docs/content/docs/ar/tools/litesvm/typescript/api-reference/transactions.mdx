---
title: المعاملات
description: تنفيذ المعاملات ومحاكاتها باستخدام LiteSVM في TypeScript
---
...
<Callout type="warn">
  خصائص نتيجة المعاملة مثل `err()`، و`computeUnitsConsumed()`،
  و`logs()`، و`signature()` هي **دوال getter**، وليست خصائص. احرص دائمًا
...
استخدم مساعد RPC إذ يُعيد كلًا من `blockhash` و`lastValidBlockHeight`.
