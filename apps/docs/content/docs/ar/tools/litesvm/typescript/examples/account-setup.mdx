---
title: إعداد الحساب
description: إعداد حسابات الاختبار والحالة باستخدام LiteSVM
---

# مثال على إعداد الحساب

يوضح كيفية إعداد حالة حساب الاختبار مباشرةً دون الحاجة إلى معاملات.
...
إعداد program account مملوك للبرنامج:
...
<Callout type="info">
  يُعدّ استخدام `setAccount` أسرع من تنفيذ المعاملات لإعداد الحسابات بشكل مجمّع.
</Callout>

## النقاط الرئيسية

1. **الحالة المباشرة**: يضبط `setAccount` حالة الحساب مباشرةً دون معاملات
2. **الإعفاء من الإيجار**: استخدم `minimumBalanceForRentExemption` لحساب الـ
   lamports المطلوبة
3. **هيكل الحساب**: وفّر جميع الحقول المطلوبة: العنوان، والبيانات، وقابلية
   التنفيذ، والـ lamports، وعنوان البرنامج، والمساحة
4. **التحقق**: استخدم `getAccount` أو `client.rpc.getAccountInfo` للتحقق
