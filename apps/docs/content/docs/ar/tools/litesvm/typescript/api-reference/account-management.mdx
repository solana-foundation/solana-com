...
description: طرق لإدارة الحسابات والأرصدة و rent في TypeScript
---
...
تعيين حالة الحساب مباشرةً دون معاملة.
...
| الخاصية         | النوع         | الوصف                   |
| ---------------- | ------------ | ----------------------------- |
| `address`        | `Address`    | المفتاح العام pubkey للحساب            |
| `data`           | `Uint8Array` | بايتات بيانات الحساب            |
...
الحصول على رصيد SOL للحساب. يُعيد `null` إذا كان الحساب غير موجود.
...
احتساب الحد الأدنى من lamport المطلوبة للإعفاء من rent.
...
لأغراض الاختبار، يمكنك إعداد حسابات متعددة بكفاءة:
...
<Callout type="info">
  يُعدّ استخدام `setAccount` أسرع من `airdrop` لإعداد الحسابات بالجملة، إذ
  لا ينفّذ معاملات.
</Callout>
