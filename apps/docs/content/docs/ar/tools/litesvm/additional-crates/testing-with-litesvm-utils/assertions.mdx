...
title: التأكيدات
description:
  تحقق من حالات الحسابات والأرصدة والملكية باستخدام أدوات مساعدة للتأكيد
---

## وجود الحساب

تحقق مما إذا كانت الحسابات موجودة أو تم إغلاقها.
...
<Callout type="info">
  `assert_account_closed` ينجح عندما لا يكون الحساب موجوداً، أو يحتوي على صفر
  lamports، أو تكون بياناته فارغة. استخدم هذا للتحقق من عملية التنظيف بعد إغلاق
  الحسابات.
</Callout>

## تأكيدات الرصيد

تحقق من أرصدة SOL والرموز المميزة.
...
تحقق من ملكية الحساب وخصائص البيانات.

### التأكد من مالك الحساب
...
<Callout type="info">
  أحجام الحسابات الشائعة: - **حساب Mint**: 82 بايت - **token account**: 165 بايت
  - **حساب النظام**: 0 بايت (lamports فقط)
</Callout>

## مثال كامل

إليك اختباراً شاملاً يوضح تأكيدات متعددة:
...
## التعامل مع إخفاقات التأكيد

عندما يفشل تأكيد ما، سيتوقف البرنامج فجأةً برسالة وصفية:
...
<Callout type="warn">
  التأكيدات مصممة للفشل السريع مع رسائل خطأ واضحة. استخدمها بحرية في اختباراتك
  للكشف عن المشكلات مبكراً.
</Callout>
