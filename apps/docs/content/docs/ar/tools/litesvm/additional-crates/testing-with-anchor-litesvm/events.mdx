---
title: الأحداث
description: تحليل وفحص أحداث Anchor من سجلات المعاملات
---

## فهم أحداث Anchor

يمكن لبرامج Anchor إطلاق أحداث يتم تسجيلها أثناء تنفيذ المعاملات. يتم ترميز هذه
الأحداث بتنسيق Base64 في سجلات المعاملات مع البادئة `Program data:`. توفر حزمة
`anchor-litesvm` أدوات لتحليل هذه الأحداث والتحقق منها.
...
أولاً، قم بتعريف أحداثك في برنامج Anchor الخاص بك:
...
<Callout type="info">
  `assert_event_emitted` سيتوقف البرنامج بشكل مفاجئ إذا لم يتم العثور على أي
  أحداث من النوع المحدد في سجلات المعاملات.
</Callout>
...
## تحليل الأحداث يدوياً

في حالات الاستخدام المتقدمة، يمكنك تحليل بيانات الحدث مباشرةً:
...
يوفر المُعدَّد `EventError` معلومات تفصيلية عن الأخطاء:
...
| الخطأ                 | الوصف                                  |
| --------------------- | -------------------------------------- |
| `EventNotFound`       | لا يوجد حدث من النوع المحدد في السجلات |
| `ParseError(String)`  | فشل في تحليل بيانات الحدث              |
| `Base64Error`         | فشل في فك ترميز Base64                 |
| `InvalidFormat`       | بيانات الحدث تحتوي على هيكل غير صالح   |
| `AnchorError(String)` | فشل إلغاء تسلسل Anchor                 |

## مثال كامل

إليك مثالاً شاملاً يوضح معالجة الأحداث:
...
<Callout type="info">
  الأحداث طريقة رائعة للتحقق من أن برنامجك نُفِّذ بشكل صحيح دون الحاجة إلى جلب
  الحسابات وإلغاء تسلسلها. وهي مفيدة بشكل خاص لتتبع تغييرات الحالة بمرور الوقت.
</Callout>
