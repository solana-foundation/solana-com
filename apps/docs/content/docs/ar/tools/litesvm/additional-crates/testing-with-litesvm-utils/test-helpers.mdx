---
title: أدوات الاختبار المساعدة
description: أدوات لإنشاء الحسابات، وعمليات التوكن، واشتقاق PDA، وإدارة slot
---

## إنشاء حسابات ممولة

الاحتياج الأكثر شيوعاً في الاختبار هو إنشاء حسابات تحتوي على SOL. السمة
`TestHelpers` تجعل ذلك أمراً بسيطاً.
...
أنشئ مصكّات التوكن وحساباته دون الحاجة إلى الكود النمطي المعتاد.
...
<Callout type="info">
  **استخدم ATAs في معظم الحالات.** associated token account هي حسابات حتمية
  وتتبع اتفاقيات سولانا. استخدم regular token accounts فقط عندما تحتاج إلى
  حسابات متعددة لنفس زوج mint/owner.
</Callout>
...
اشتقّ Program Derived Addresses بسهولة لاختباراتك.
...
<Callout type="info">
  استخدم `derive_pda` عندما تحتاج إلى bump seed (مثلاً لاستدعاءات CPI). استخدم
  `get_pda` عندما تحتاج إلى العنوان فقط.
</Callout>

## إدارة Slot

تحكم في تقدم الوقت لاختبار المنطق المعتمد على الوقت.

### الحصول على الـ slot الحالي
...
إليك مثالاً شاملاً يستخدم عدة مساعدين للاختبار معاً:
