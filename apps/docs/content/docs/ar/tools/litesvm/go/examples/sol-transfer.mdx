---
title: تحويل SOL
description: بناء وإرسال معاملة تحويل SOL باستخدام litesvm-go
---

أبسط معاملة على سولانا: نقل SOL من حساب إلى آخر. يُعدّ هذا اختباراً أولياً جيداً للتحقق من أن إعداد `litesvm-go` يعمل بشكل صحيح قبل اختبار منطق برامج أكثر تعقيداً.
...
<Callout type="info">
  الرصيد النهائي للمُرسِل أقل بقليل من 9 SOL بسبب رسوم المعاملة
  (5,000 lamport لكل توقيع).
</Callout>

يغطي هذا النوع الأبسط من المعاملات. لاختبار البرامج المخصصة، راجع
[اختبار البرامج](/docs/tools/litesvm/go/examples/program-testing). للاطلاع على المنطق المعتمد على الوقت، راجع
[الاختبار المبني على الوقت](/docs/tools/litesvm/go/examples/time-based).
