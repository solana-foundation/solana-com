...
description: تحميل واختبار برامج سولانا المخصصة باستخدام litesvm-go
---

يعني اختبار برنامج سولانا مخصص من Go تحميل ملف `.so` المُجمَّع الخاص به
إلى SVM، وإعداد الحسابات التي يتوقعها، وإرسال التعليمات إليه.
`litesvm-go` يتولى كل هذا داخل العملية ذاتها - دون الحاجة إلى validator.
...
للاختبارات التي تحتاج إلى برامج قياسية (SPL Token، Memo، ...):
...
1. **الإعداد** - تفعيل sysvars وbuiltins وprecompiles حسب الحاجة.
2. **تحميل البرنامج** - `AddProgramFromFile` (أو `AddProgram` للبايتات الموجودة في الذاكرة).
3. **إعداد الحسابات** - ملء حسابات البيانات مسبقًا باستخدام `SetAccount`.
4. **الإرسال** - بناء التعليمة باستخدام `solana-go`، وتحويلها إلى تنسيق ثنائي، وإرسالها عبر `SendLegacyTransaction`.
5. **التحقق** - قراءة الحسابات مجددًا باستخدام `GetAccount` والتحقق من السجلات ووحدات الحوسبة والحالة النهائية.

بمجرد أن يعمل هذا الهيكل الأساسي، يمكنك التوسع بإضافة المزيد من التعليمات والحسابات ودالة مساعدة للتعامل مع الأجزاء المتكررة.
