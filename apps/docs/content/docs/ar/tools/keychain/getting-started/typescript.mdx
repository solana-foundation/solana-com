---
title: TypeScript
description: تثبيت واستخدام @solana/keychain في تطبيقات TypeScript
---
...
### الاستخدام مع عميل Kit (إضافة keychainSigner)

يقوم `@solana/keychain-kit-plugin` بتثبيت موقّع سلسلة المفاتيح مباشرةً على
[عميل Kit](https://github.com/anza-xyz/kit)، ويربطه بوصفه `payer` و `identity`،
مما يُغنيك عن الإعداد اليدوي لـ `pipe` الموضّح أعلاه:
...
تقبل الإضافة نفس الإعدادات المُعلَّمة بالخلفية كما هو الحال في
`createKeychainSigner`، ولا يتم تضمين سوى الخلفية التي يُوجَّه إليها الطلب
(تُحمَّل حزم الخلفية عبر `import()` الديناميكي).

استخدم `keychainPayer` أو `keychainIdentity` لتعيين دور واحد فقط، ومزج الخلفيات
على عميل واحد:
...
هل لديك `SolanaSigner` بالفعل؟ كل موقّع سلسلة مفاتيح هو `TransactionSigner` صالح
لـ Kit، لذا يمكنك تثبيت موقّع موجود باستخدام
[`@solana/kit-plugin-signer`](https://github.com/anza-xyz/kit-plugins/tree/main/packages/kit-plugin-signer)
عوضًا عن ذلك (`pnpm add @solana/kit-plugin-signer`):
...
يمتلك Fordefi وضعَين للتوقيع، يُحدَّد أيهما باستخدام المعامل `chain` من عدمه. تتم مصادقة طلبات API بتوقيع طلب ECDSA P-256 — قدِّم مفتاح التوقيع عبر `privateKeyPem`، أو احتفظ به في KMS/HSM بتمرير `requestSigner`.

**وضع سولانا الأصلي** (موصى به للاستخدام على السلسلة): عيِّن `chain`، وسيتولى Fordefi توقيع المعاملة وبثّها بنفسه. نظرًا لأن Fordefi قد يستبدل blockhash أو الرسوم قبل التوقيع، فإن الموقِّع هو `TransactionSendingSigner` من Kit — استخدمه مع `signAndSendTransactionMessageWithSigners`، لا مع تدفق الموقِّع الجزئي المبيَّن أعلاه.
...
**وضع الصندوق الأسود**: أغفِل `chain`. يوقِّع Fordefi البايتات الخام دون بثّها؛ وعليك أنت تجميع المعاملة وإرسالها. استخدم هذا الوضع مع خزنة الصندوق الأسود في Fordefi.
...
تمتد واجهة `SolanaSigner` لتشمل أنواع `@solana/signers` لضمان التوافق الكامل:
...
نفّذ واجهة `SolanaSigner` لإنشاء موقّعين مخصصين:
