...
| الميزة      | الوصف                     |
| ------------ | ------------------------------- |
| `memory`     | توقيع keypair محلي (افتراضي) |
...
لتوقيع حضانة Fordefi MPC. يتم التحقق من طلبات API بتوقيع طلب ECDSA
P-256 يُحلَّل من `private_key_pem`؛ للاحتفاظ بذلك المفتاح في
KMS/HSM، نفِّذ `FordefiRequestSigner` واستخدم
`Signer::from_fordefi_with_signer`.

يمتلك Fordefi وضعَي توقيع. اضبط `chain` لـ**وضع سولانا الأصلي**، حيث
قد يستبدل Fordefi blockhash أو الرسوم ويُذيع المعاملة بنفسه.
aترك `chain` كـ`None` لـ**وضع الصندوق الأسود**، حيث يوقّع Fordefi البايتات الخام
وتُذيع المعاملة بنفسك.
