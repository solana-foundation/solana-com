...
تتفاعل هذه الدوال مع سولانا للتحقق من المعاملات والانتظار حتى
اكتمال التأكيدات. وتتطلب عميل RPC من سولانا من مكتبة `@solana/kit`.
...
- **`rpc`** (`Rpc<SolanaRpcApi>`، مطلوب) - عميل RPC من `@solana/kit`.
  يُنشأ باستخدام `createSolanaRpc(rpcUrl)`.
