...
[Solana Pay](https://solanapay.com) الكاملة لبناء تجارب الدفع،
متوافقةً مع مكتبة
[`@solana/kit`](https://github.com/anza-xyz/kit). تتولى الحزمة ترميز عناوين URL
وتحليلها، وتوليد رموز QR بأنماط مخصصة، وإنشاء المعاملات
لكلٍّ من تحويلات SOL ورموز SPL.
...
- **`recipient`** (`Address`، مطلوب) - عنوان المحفظة الخاصة بالتاجر
  (مفتاح عام مُرمَّز بـ base58) الذي سيتلقى الدفعة. يمكن أن يكون نصًا أو
  من النوع `Address` المتوفر في `@solana/kit`.
...
يبني رسالة معاملة سولانا كاملة لتحويل مدفوعات. تكتشف هذه
الدالة تلقائيًا ما إذا كان ينبغي إنشاء تحويل SOL أو رمز SPL
بناءً على معامل `splToken`، وتُنشئ جميع التعليمات اللازمة.
تضبط الدالة مدة صلاحية المعاملة باستخدام أحدث blockhash من عميل RPC
وتُعيد معاملة كاملة وغير موقَّعة جاهزة
للتوقيع والإرسال إلى RPC (من النوع
`TransactionMessageWithBlockhashLifetime`، متوافق مع Solana Kit).

#### المعاملات

- **`rpc`** (`Rpc<SolanaRpcApi>`) - عميل RPC الخاص بسولانا من `@solana/kit`. يُنشأ
  باستخدام `createSolanaRpc(rpcUrl)`.
