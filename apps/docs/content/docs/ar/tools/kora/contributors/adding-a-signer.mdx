...
أضف خدمتك إلى قائمة الموقّعين في ملف README الرئيسي.

## قائمة مراجعة التقديم

- [ ] يتم دعم الموقّع الخاص بك في الحزمة `solana-keychain`
- [ ] تم تحديث اعتماد `solana-keychain` في Cargo.toml إلى أحدث إصدار
- [ ] تمت إضافة بنية الإعداد الخاصة بالموقّع
- [ ] تمت إضافة متغير `SignerTypeConfig`
- [ ] تمت إضافة منطق البناء في `build_signer_from_config`
- [ ] تمت إضافة منطق التحقق في `validate_individual_signer_config`
- [ ] تم تصدير بنية الإعداد في `mod.rs`
- [ ] (اختياري) تمت إضافة طريقة بناء النموذج الاختباري في `config_mock.rs`
- [ ] يتم تجميع الكود دون تحذيرات
- [ ] جميع الاختبارات ناجحة (`make test` و `make test-integration`)
- [ ] تمت إضافة التوثيق وفقاً لـ
      [`ADDING_SIGNERS.md`](https://github.com/solana-foundation/solana-keychain/blob/main/docs/ADDING_SIGNERS.md)
- [ ] تم إنشاء ملفات الإعداد النموذجية (`.toml` و `.env.example`)
- [ ] لا توجد قيم مضمّنة أو أسرار مشفّرة
- [ ] رسائل الخطأ مفيدة وواضحة
- [ ] يتبع اصطلاحات تسمية Rust (snake_case)
- [ ] يجتاز فحص الكود (`make lint`)
- [ ] تواصل مع فريق Kora للحصول على مفاتيح API لاختبار التكامل

## الحصول على المساعدة

- **لتنفيذ الموقّع**: افتح تذكرة في مستودع
  [`solana-keychain`](https://github.com/solana-foundation/solana-keychain)
- **لتكامل Kora**: افتح تذكرة في مستودع Kora لمناقشات التصميم
- انضم إلى قنوات مجتمعنا
- راجع إعدادات الموقّعين الموجودة في
...
مرحباً بك في نظام Kora البيئي! يسعدنا انضمام حل إدارة المفاتيح الخاص بك إلى
المنصة.
