...
title: تجميد الحساب
description:
  قم بتجميد حساب الرمز المميز باستخدام صلاحية التجميد الخاصة بالسك لمنع عمليات
  التحويل والحرق وتغييرات التفويض.
...
## ما الذي يفعله تجميد حساب الرمز المميز؟

يحافظ تجميد حساب الرمز المميز على نفس المالك والسك والرصيد، لكنه يمنع حساب الرمز
المميز من تحويل أو حرق الرموز المميزة حتى يتم إلغاء تجميد حساب الرمز المميز.

لا يزال بإمكان إغلاق الحساب المجمد إذا كان الحساب المجمد يحتوي على رصيد رمز مميز
صفري.

يمكن فقط لعملية سك لا تزال تمتلك صلاحية التجميد أن تقوم بتجميد حسابات الرموز
المميزة. إذا تم إلغاء صلاحية التجميد الخاصة بالسك، فلن يمكن بعد ذلك تجميد
الحسابات الخاصة بتلك السك.

## كيفية تجميد حساب الرمز المميز

يستخدم تجميد حساب الرمز المميز تعليمات _rs`FreezeAccount`_ الخاصة ببرنامج الرموز
المميزة.

تتحقق تعليمات _rs`FreezeAccount`_ من أن حساب الرمز المميز ينتمي إلى السك وأن
الموقّع هو صلاحية التجميد الخاصة بالسك، ثم تغير حالة حساب الرمز المميز من مهيأ
إلى مجمد.

[حسابات الرموز المميزة الأصلية](/docs/tokens/basics/sync-native) لا تدعم
التجميد.
...
| ----------------------------------- | -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | تخزن حالة السك صلاحية التجميد الاختيارية المستخدمة لتجميد حسابات الرموز المميزة. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | حالات حساب الرمز المميز، بما في ذلك _rs`Initialized`_ و _rs`Frozen`_.            | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | تعليمات تغير حساب الرمز المميز إلى الحالة المجمدة.                               | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
| _rs`process_toggle_freeze_account`_ | منطق المعالج المشترك لتجميد وإلغاء تجميد حسابات الرموز المميزة.                  | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |

### Typescript

توضح الأمثلة أدناه باستخدام `Kit` النهج الموصى به باستخدام `@solana/kit`. تم
تضمين الأمثلة القديمة التي تستخدم `@solana/web3.js` كمرجع.
