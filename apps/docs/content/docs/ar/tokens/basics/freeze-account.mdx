...
title: تجميد الحساب
description:
  تجميد حساب الرموز باستخدام صلاحية التجميد الخاصة بالإصدار لمنع عمليات النقل
  والحرق وتغييرات المفوضين.
...
## ماذا يفعل تجميد حساب الرموز؟

يحتفظ تجميد حساب الرموز بنفس المالك والإصدار والرصيد، لكنه يمنع حساب الرموز من
نقل أو حرق الرموز حتى يتم إلغاء تجميد حساب الرموز.

لا يزال بإمكانك إغلاق الحساب المجمد إذا كان رصيد الرموز في الحساب المجمد صفرًا.

يمكن فقط للإصدار الذي لا يزال لديه صلاحية تجميد أن يجمد حسابات الرموز. إذا تم
إلغاء صلاحية التجميد للإصدار، فلن يمكن بعد ذلك تجميد الحسابات الخاصة بهذا
الإصدار.

## كيفية تجميد حساب الرموز

يستخدم تجميد حساب الرموز تعليمة _rs`FreezeAccount`_ الخاصة ببرنامج الرموز.

تتحقق تعليمة _rs`FreezeAccount`_ من أن حساب الرموز ينتمي إلى الإصدار وأن الموقع
هو صلاحية التجميد الخاصة بالإصدار، ثم تغير حالة حساب الرموز من مهيأ إلى مجمد.

[حسابات الرموز الأصلية](/docs/tokens/basics/sync-native) لا تدعم التجميد.

### المرجع المصدري

| العنصر                              | الوصف                                                                       | برنامج الرموز                                                                                                  | برنامج Token Extensions                                                                                              |
| ----------------------------------- | --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | تخزن حالة الإصدار صلاحية التجميد الاختيارية المستخدمة لتجميد حسابات الرموز. | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | حالات حساب الرموز، بما في ذلك _rs`Initialized`_ و _rs`Frozen`_.             | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | تعليمة تغير حساب الرموز إلى الحالة المجمدة.                                 | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
| _rs`process_toggle_freeze_account`_ | منطق المعالج المشترك لتجميد وإلغاء تجميد حسابات الرموز.                     | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |

### Typescript

توضح الأمثلة أدناه باستخدام `Kit` النهج الموصى به باستخدام `@solana/kit`. تم
تضمين الأمثلة القديمة باستخدام `@solana/web3.js` للإشارة فقط.
