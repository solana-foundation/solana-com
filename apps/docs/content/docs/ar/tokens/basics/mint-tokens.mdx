---
title: سك الرموز
description: سك وحدات رمزية جديدة إلى حساب رمزي باستخدام صلاحية السك.
...
ينشئ السك وحدات جديدة من رمز موجود ويزيد من إجمالي المعروض من السك.

فقط صلاحية السك يمكنها سك رموز جديدة. يجب أن يكون حساب الرمز المستهدف موجودًا
بالفعل ويجب أن ينتمي إلى نفس السك.

## كيفية سك الرموز

يستخدم سك الرموز تعليمة _rs`MintTo`_ أو _rs`MintToChecked`_ من Token Program.

تستخدم الأمثلة أدناه _rs`MintToChecked`_، والتي تتطلب من المتصل توفير الكسور
العشرية للسك حتى تتمكن التعليمة من التحقق من دقة الرمز المتوقعة قبل السك.

لا يدعم [السك الأصلي](/docs/tokens/basics/sync-native) عملية السك.

### مرجع المصدر

| العنصر                | الوصف                                                                                                           | Token Program                                                                                                  | Token Extension Program                                                                                              |
| --------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_            | تخزن حالة السك صلاحية السك وإجمالي المعروض من الرموز.                                                           | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_         | تخزن حالة token account الرصيد المستهدف المحدث بواسطة السك.                                                     | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`MintTo`_          | تعليمة سك تسك الرموز إلى token account موجود دون مطالبة المتصل بتوفير الكسور العشرية للسك.                      | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L159-L177) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L202-L220) |
| _rs`MintToChecked`_   | تعليمة سك تتطلب من المتصل توفير الكسور العشرية للسك وتتحقق من تلك القيمة قبل سك الرموز إلى token account موجود. | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L301-L325) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L367-L391) |
| _rs`process_mint_to`_ | منطق المعالج المشترك لسك الرموز.                                                                                | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L521-L583)     | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L981-L1067)    |

### تايب سكريبت

توضح أمثلة `Kit` أدناه النهج الموصى به باستخدام `@solana/kit`. تم تضمين أمثلة
قديمة باستخدام `@solana/web3.js` للرجوع إليها.
