...
<Callout type="info">
  ...
  في تطبيقات مماثلة لتحقيق نفس الوظيفة.
</Callout>
...
async fn main() -> Result<()> {
    ...
    // توليد keypair جديد لدافع الرسوم
    let fee_payer = Keypair::new();

    // إسقاط جوي بقيمة 1 SOL لدافع الرسوم
    ...
    // توليد keypair لاستخدامه كعنوان للسك
    let mint = Keypair::new();

    // الحصول على حجم حساب السك الافتراضي (بالبايتات)، بدون تفعيل إضافات
    let mint_space = Mint::LEN;
    let mint_rent = client
        .get_minimum_balance_for_rent_exemption(mint_space)
        .await?;

    // تعليمة لإنشاء حساب جديد للسك (Token Program)
    ...
    // حساب عنوان حساب الرمز المرتبط للدافع
    ...
    // إرسال وتأكيد المعاملة
    client.send_and_confirm_transaction(&transaction).await?;

    // كمية الرموز المراد سكها (100 رمز بمنزلتين عشريتين)
    let amount = 100;

    // إنشاء تعليمة mint_to لسك الرموز إلى حساب الرمز المرتبط
    let mint_to_instruction = mint_to(
        &token_program_id(),
        &mint.pubkey(),            // السك
        &associated_token_address, // الوجهة
        &fee_payer.pubkey(),       // السلطة
        &[&fee_payer.pubkey()],    // الموقع
        amount,                    // الكمية
    )?;

    // إنشاء معاملة لسك الرموز
    let transaction = Transaction::new_signed_with_payer(
        &[mint_to_instruction],
        Some(&fee_payer.pubkey()),
        &[&fee_payer],
    ...
    println!("تم سك 1.00 رمز إلى حساب الرمز المرتبط");
    println!("\nعنوان السك: {}", mint.pubkey());
    println!("{:#?}", mint_data);

    println!(
        "\nعنوان حساب الرمز المرتبط: {}",
        associated_token_address
    );
    println!("{:#?}", token_data);

    println!("توقيع المعاملة: {}", transaction_signature);
...
async def main():
    ...
    # أمثلة على أزواج المفاتيح والعناوين
    payer = Keypair()
    ...
    # المبلغ المراد سكه (بأصغر وحدة)
    amount_to_mint = 1000000000  # رمز واحد مع 9 خانات عشرية
