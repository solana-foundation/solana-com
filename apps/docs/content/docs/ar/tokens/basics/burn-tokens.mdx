---
title: حرق الرموز
description: حرق الرموز من حساب رمزي وتقليل إجمالي المعروض من سك العملة.
...
حرق الرموز يقلل بشكل دائم من رصيد حساب رمزي ويقلل إجمالي المعروض من سك العملة
بنفس المقدار.

## كيفية حرق الرموز

حرق الرموز يستخدم تعليمة _rs`Burn`_ أو _rs`BurnChecked`_ من Token Program.

الأمثلة أدناه تستخدم _rs`BurnChecked`_، والتي تتطلب من المستدعي توفير الكسور
العشرية لسك العملة حتى تتمكن التعليمة من التحقق من دقة الرمز المتوقعة قبل الحرق.

[سك العملة الأصلي](/docs/tokens/basics/sync-native) لا يدعم الحرق. استخدم
_rs`CloseAccount`_ بدلاً من ذلك.

مالك حساب الرمز أو مفوض معتمد يوقع على المعاملة. في Token Extension Program،
يمكن لمفوض دائم لسك العملة أيضاً تفويض الحرق إذا كان سك العملة يحتوي على إضافة
المفوض الدائم مفعلة.
...
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_         | حالة سك العملة تخزن إجمالي المعروض من الرموز.                                                                                 | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | حالة حساب الرمز تخزن رصيد الرمز الذي يتم تقليله عن طريق الحرق.                                                                | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | تعليمة حرق تقلل رصيد حساب رمزي ومعروض سك العملة دون الحاجة إلى أن يوفر المستدعي الكسور العشرية لسك العملة.                    | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | تعليمة حرق تتطلب من المستدعي توفير الكسور العشرية لسك العملة وتتحقق من تلك القيمة قبل تقليل رصيد حساب الرمز ومعروض سك العملة. | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | منطق معالج مشترك لحرق الرموز.                                                                                                 | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |

### Typescript

توضح الأمثلة أدناه في `Kit` النهج الموصى به باستخدام `@solana/kit`. تم تضمين
الأمثلة القديمة باستخدام `@solana/web3.js` للرجوع إليها.
