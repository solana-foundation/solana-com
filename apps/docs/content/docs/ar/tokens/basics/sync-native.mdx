...
description: تحديث مقدار حساب رمز SOL المغلف بعد تحويل SOL إلى الحساب.
...
SOL المُغلّف (WSOL) هو SOL محتفظ به في حساب رمزي لعملة Token Program الأصلية.
العملة الأصلية هي عنوان العملة الذي يستخدمه Token Program للحسابات الرمزية التي
تغلف SOL. تتيح العملة الأصلية استخدام SOL من خلال تعليمات Token Program.

| البرنامج                 | عنوان العملة الأصلية                           |
| ------------------------ | ---------------------------------------------- |
...
يخزّن حساب رمز SOL المُغلّف مبلغ SOL في حقل `lamports` الخاص بالحساب، لكنه يتتبع
مبلغ SOL المحتفظ به في حساب رمز SOL المُغلّف كرصيد رمزي من خلال حقل `amount`
الخاص بالحساب الرمزي. إذا قمت بتحويل SOL إلى حساب رمز SOL مُغلّف، يزداد رصيد
lamport فورًا، لكن حقل `amount` لا يتغير حتى تستدعي تعليمة `SyncNative` الخاصة
بـ Token Program.

## كيفية مزامنة SOL الأصلي

استخدم تعليمة `SyncNative` الخاصة بـ Token Program بعد تحويل SOL إلى حساب رمز
SOL مُغلّف. تقوم تعليمة `SyncNative` بتحديث حقل `amount` الخاص بالحساب الرمزي
ليطابق lamports المحتفظ بها في الحساب فوق احتياطي الإعفاء من الإيجار.

### مرجع المصدر

| العنصر                | الوصف                                                                                                                                                                                                   | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `Account`             | تستخدم حالة الحساب الرمزي `is_native` لوضع علامة على الحساب كـ WSOL وتخزين الاحتياطي الأصلي، وهو الحد الأدنى من رصيد lamport المعفى من الإيجار الذي يجب على الحساب الاحتفاظ به، والمبلغ الرمزي المتتبع. | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| `SyncNative`          | تعليمة تقوم بتحديث حقل `amount` الخاص بحساب رمز SOL المُغلّف ليطابق lamports المحتفظ بها في الحساب فوق احتياطي الإعفاء من الإيجار.                                                                      | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| `process_sync_native` | منطق المعالج المشترك لمزامنة أرصدة SOL المُغلّفة.                                                                                                                                                       | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |

### Typescript

توضح الأمثلة أدناه في `Kit` النهج الموصى به باستخدام `@solana/kit`. تم تضمين
الأمثلة القديمة التي تستخدم `@solana/web3.js` للمرجعية.
