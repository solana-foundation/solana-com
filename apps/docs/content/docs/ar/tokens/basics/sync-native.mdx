---
title: مزامنة SOL الأصلي
description: تحديث مبلغ حساب رمز SOL المُغلف بعد تحويل SOL إلى الحساب.
...
SOL المُغلف (WSOL) هو SOL محفوظ في حساب رمزي لعملة السك الأصلية الخاصة بـ Token
Program. عملة السك الأصلية هي عنوان السك الذي يستخدمه Token Program للحسابات
الرمزية التي تُغلف SOL. تُمكن عملة السك الأصلية من استخدام SOL من خلال تعليمات
Token Program.

| البرنامج                 | عنوان عملة السك الأصلية                        |
| ------------------------ | ---------------------------------------------- |
...
يخزن حساب رمز SOL المُغلف عملة SOL في حقل `lamports` الخاص بالحساب، لكنه يتتبع
عملة SOL المحفوظة في حساب رمز SOL المُغلف كرصيد رمزي من خلال حقل _rs`amount`_
الخاص بالحساب الرمزي. إذا قمت بتحويل SOL إلى حساب رمز SOL مُغلف، يزداد رصيد
lamport فوراً، لكن حقل _rs`amount`_ لا يتغير حتى تستدعي تعليمة _rs`SyncNative`_
الخاصة بـ Token Program.

## كيفية مزامنة SOL الأصلي

استخدم تعليمة _rs`SyncNative`_ الخاصة بـ Token Program بعد تحويل SOL إلى حساب
رمز SOL مُغلف. تقوم تعليمة _rs`SyncNative`_ بتحديث حقل _rs`amount`_ الخاص
بالحساب الرمزي ليطابق عملات lamport المحفوظة في الحساب فوق احتياطيه المُعفى من
الإيجار.
...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_             | تستخدم حالة الحساب الرمزي _rs`is_native`_ لتمييز الحساب كـ WSOL وتخزن الاحتياطي الأصلي، وهو الحد الأدنى لرصيد lamport المُعفى من الإيجار الذي يجب على الحساب الاحتفاظ به، والمبلغ الرمزي المتتبع. | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`SyncNative`_          | تعليمة تقوم بتحديث حقل _rs`amount`_ الخاص بحساب رمز SOL المُغلف ليطابق عملات lamport المحفوظة في الحساب فوق احتياطيه المُعفى من الإيجار.                                                          | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| _rs`process_sync_native`_ | منطق المعالج المشترك لمزامنة أرصدة SOL المُغلفة.                                                                                                                                                  | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |
