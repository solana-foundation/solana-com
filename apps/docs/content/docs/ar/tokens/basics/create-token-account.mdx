...
description:
  إنشاء حسابات الرموز وحسابات الرموز المرتبطة لعملة معدنية ومالك محددين.
...
يخزن حساب الرموز الرموز لعملة معدنية واحدة ومالك حساب رموز واحد على سولانا.
...
هنا، _rs`owner`_ تعني الصلاحية التي يمكنها نقل أو حرق أو تفويض الرموز من حساب
الرموز. مالك البرنامج للحساب لا يزال Token Program أو Token Extensions Program.

كل حساب رموز مرتبط بعملة معدنية واحدة بالضبط، مما يعني أن حساب الرموز يمكنه
الاحتفاظ بوحدات من رمز واحد فقط، وهو الرمز المحدد بحقل _rs`mint`_ لحساب الرموز.

## ما هو حساب الرموز المرتبط

حساب الرموز المرتبط (ATA) هو حساب الرموز الافتراضي للمحفظة والعملة المعدنية.
يستخرج
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program/src)
عنوان ATA من عنوان المحفظة وعنوان برنامج الرموز وعنوان العملة المعدنية.

فقط حسابات الرموز التي تم إنشاؤها بواسطة Associated Token Program تسمى
associated token accounts.

Associated Token Program هو وسيلة لإنشاء حساب رموز بعنوان قياسي وحتمي. الحساب
الناتج لا يزال حساب رموز مملوكًا من قبل Token Program أو Token Extensions
Program، وليس من قبل Associated Token Program.

يستخرج Associated Token Program عنوان ATA كما هو موضح في
...
لأي مجموعة من المحفظة وبرنامج الرموز والعملة المعدنية، يوجد عنوان ATA واحد
بالضبط. ينشئ Associated Token Program حساب رموز قياسيًا عند ذلك العنوان، ولا
...
Associated Token Program. يقوم Associated Token Program باشتقاق عنوان الحساب
المرتبط، وإنشاء الحساب على ذلك العنوان، وتهيئة الحساب كحساب رمز مملوك لـ Token
Program أو Token Extensions Program.

<Callout type="info" title="موصى به">
  بالنسبة لمعظم التطبيقات، قم بإنشاء حسابات الرموز من خلال Associated Token
  Program بدلاً من إنشائها عن طريق استدعاء تعليمات System Program وToken Program
  مباشرةً. يستخدم حساب الرمز المرتبط عنواناً حتمياً مشتقاً من المالك وبرنامج
  الرمز والنعناع، مما يجعل حساب الرمز الافتراضي لنعناع معين أسهل للعثور عليه
  بواسطة المحافظ والتطبيقات.
</Callout>
...
| العنصر                                        | الوصف                                                                                                                                                                                                                                                 | المصدر                                                                                                                          |
| --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| _rs`Create`_                                  | ينشئ حساب رمز مرتبط على العنوان المشتق.                                                                                                                                                                                                               | [المصدر](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L20-L29) |
| _rs`CreateIdempotent`_                        | ينشئ حساب الرمز المرتبط، لكنه ينجح حتى لو كان هذا الحساب موجوداً بالفعل لنفس المالك والنعناع.                                                                                                                                                         | [المصدر](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | يشتق عنوان الحساب المرتبط ويستخدم _rs`create_pda_account`_ بالإضافة إلى CPIs لبرنامج الرمز المحدد لتهيئة حساب الرمز. عندما يكون البرنامج المحدد هو Token Extensions Program، يقوم معالج الحساب المرتبط أيضاً بتهيئة امتداد المالك غير القابل للتغيير. | [المصدر](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | مساعد يستخدمه _rs`process_create_associated_token_account`_ لإنشاء حساب PDA عن طريق CPI إلى System Program.                                                                                                                                           | [المصدر](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |

### تايب سكريبت

توضح أمثلة `Kit` أدناه النهج الموصى به باستخدام `@solana/kit`. تم تضمين الأمثلة
القديمة باستخدام `@solana/web3.js` للرجوع إليها.
...
## كيفية إنشاء حساب رمزي

يتطلب إنشاء حساب رمزي تعليمتين:

1. تعليمة _rs`CreateAccount`_ الخاصة بـ System Program تنشئ حسابًا جديدًا معفى
   من rent وتعيّن Token Program كمالك برنامج للحساب الجديد.
2. تعليمة _rs`InitializeAccount`_ أو _rs`InitializeAccount2`_ أو
   _rs`InitializeAccount3`_ الخاصة بـ Token Program تهيئ الحساب الجديد لعملة
   ومالك محددين.

قم بتضمين تعليمة _rs`CreateAccount`_ وتعليمة تهيئة token account في نفس
المعاملة.

أثناء تهيئة token account، يتحقق Token Program من أن الحساب لم يتم تهيئته بالفعل
وأنه معفى من rent.

<Callout type="info">
  يوضح القسم أدناه كيفية إنشاء token account عن طريق استدعاء تعليمات System
  Program و Token Program مباشرة. بالنسبة لمعظم التطبيقات، استخدم Associated
  Token Program بدلاً من ذلك. استخدم استدعاءات System Program و Token Program
  المباشرة عندما يكون لديك سبب محدد لعدم استخدام associated token account أو
  عندما تحتاج إلى إنشاء حسابات PDA رمزية مخصصة عن طريق إجراء استدعاءات CPI
  لتعليمات System Program و Token Program من برنامج سولانا الخاص بك.
</Callout>
...
| --------------------------------- | -------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_                     | حقول token account الأساسية المخزنة في كل token account.                               | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`InitializeAccount`_           | تعليمة تهيئة token account تتوقع حساب المالك وحساب rent sysvar في قائمة حساباتها.      | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L45-L63)   | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L75-L93)   |
| _rs`InitializeAccount2`_          | تعليمة تهيئة token account تمرر المالك في instruction data بدلاً من قائمة الحسابات.    | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L352-L365) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L419-L433) |
| _rs`InitializeAccount3`_          | تعليمة تهيئة token account تمرر المالك في instruction data ولا تتطلب حساب rent sysvar. | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L377-L387) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L445-L456) |
| _rs`_process_initialize_account`_ | منطق المعالج المشترك لتهيئة token account.                                             | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L83-L141)      | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L153-L234)     |
...
توضح الأمثلة أدناه في `Kit` النهج الموصى به باستخدام `@solana/kit`. تم تضمين
الأمثلة القديمة باستخدام `@solana/web3.js` كمرجع.
