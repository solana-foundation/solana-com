...
description:
  إنشاء حسابات رمزية وحسابات رمزية مرتبطة لعملة محددة ومالك محدد.
...
## التكلفة والجهة الدافعة

يتطلب إنشاء ATA رصيداً أدنى قابلاً للاسترداد لتغطية تكاليف التخزين على السلسلة، بالإضافة إلى رسوم المعاملة. يموّل `payer` المُمرَّر إلى تعليمات الإنشاء رصيد التخزين، ويجب أن يوقّع على المعاملة. يمكن أن يختلف الدافع عن كلٍّ من جهة دفع رسوم المعاملة والمحفظة التي ستمتلك ATA، وبذلك يمكن للتطبيق إنشاء ATA نيابةً عن المستلم.

بناءً على معدل تخزين الكلاستر الحالي، تكون التكاليف الأساسية كما يلي:

| Token program                             | حجم ATA  | الحد الأدنى لرصيد التخزين             |
| ----------------------------------------- | --------- | ----------------------------------- |
| Original Token Program                    | 165 bytes | 2,039,280 lamports (0.00203928 SOL) |
| Token-2022 مع مالك ثابت تلقائي | 170 bytes | 2,074,080 lamports (0.00207408 SOL) |

يمكن أن تتطلب عمليات سك Token-2022 امتدادات حسابية إضافية فوق امتداد _rs`ImmutableOwner`_ التلقائي. تكون ATAs هذه أكبر حجماً وتستلزم رصيداً أدنى أعلى. يستعلم Associated Token Program من Token Program المُحدَّد عن حجم الحساب المطلوب، ثم يموّل ATA وفقاً لذلك.

لا تُضمِّن هذه الأرصدة بشكل ثابت في منطق التطبيق. استعلم عن
[`getMinimumBalanceForRentExemption`](/docs/rpc/http/getminimumbalanceforrentexemption)
للحصول على حجم البيانات المطلوب، أو دع Associated Token Program يحسبه ويموّله أثناء الإنشاء.

<Callout type="info" title="رصيد التخزين قابل للاسترداد">
  يؤدي إغلاق token account فارغ إلى تحويل جميع lamports الموجودة فيه، بما فيها الحد الأدنى لرصيد التخزين، إلى الوجهة المحددة في تعليمة الإغلاق. لا يكون الدافع الأصلي للإنشاء تلقائياً هو وجهة الاسترداد. راجع [Close Token Account](/docs/tokens/basics/close-account).
</Callout>
