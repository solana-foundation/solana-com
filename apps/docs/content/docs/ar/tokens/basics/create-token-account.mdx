...
description: إنشاء حسابات الرموز وحسابات الرموز المرتبطة لسك ومالك محددين.
...
يقوم حساب الرموز بتخزين الرموز لسك واحد ومالك واحد لحساب الرموز على سولانا.
...
هنا، _rs`owner`_ تعني الصلاحية التي يمكنها نقل الرموز أو حرقها أو تفويضها من
حساب الرموز. مالك البرنامج للحساب لا يزال Token Program أو Token Extensions
Program.

كل حساب رموز مرتبط بسك واحد بالضبط، مما يعني أن حساب الرموز يمكنه الاحتفاظ
بوحدات رمز واحد فقط، وهو الرمز المحدد بواسطة حقل _rs`mint`_ لحساب الرموز.

## ما هو حساب الرموز المرتبط

حساب الرموز المرتبط (ATA) هو حساب الرموز الافتراضي للمحفظة والسك. يشتق
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program/src)
عنوان ATA من عنوان المحفظة وعنوان برنامج الرموز وعنوان السك.

فقط حسابات الرموز التي تم إنشاؤها بواسطة Associated Token Program تُسمى
associated token accounts.

Associated Token Program هو طريقة لإنشاء حساب رموز بعنوان قياسي وحتمي. الحساب
الناتج لا يزال حساباً للرموز مملوكاً بواسطة Token Program أو Token Extensions
Program، وليس بواسطة Associated Token Program.

يشتق Associated Token Program عنوان ATA كما هو موضح في
...
لأي مجموعة من المحفظة وبرنامج الرموز والسك، يوجد عنوان ATA واحد بالضبط. ينشئ
Associated Token Program حساب رموز قياسياً بهذا العنوان، ولا يزال الحساب الناتج
يستخدم نوع _rs`Account`_ المحدد بواسطة Token Program أو Token Extensions
Program.

## كيفية إنشاء حساب رمز مرتبط

يستخدم إنشاء حساب رمز مرتبط تعليمة _rs`Create`_ أو _rs`CreateIdempotent`_ من
برنامج الرموز المرتبطة. يقوم برنامج الرموز المرتبطة باشتقاق عنوان ATA، وإنشاء
الحساب على عنوان ATA، وتهيئة الحساب كحساب رمز مملوك من قبل برنامج الرموز أو
برنامج امتدادات الرموز.

<Callout type="info" title="موصى به">
  بالنسبة لمعظم التطبيقات، يُنصح بإنشاء حسابات الرموز من خلال برنامج الرموز
  المرتبطة بدلاً من إنشائها عبر استدعاء تعليمات برنامج النظام وبرنامج الرموز
  مباشرةً. يستخدم حساب الرمز المرتبط عنواناً حتمياً مشتقاً من المالك وبرنامج
  الرموز والسك، مما يجعل حساب الرموز الافتراضي لسك معين أسهل في الإيجاد بالنسبة
  للمحافظ والتطبيقات.
</Callout>
...
| العنصر                                        | الوصف                                                                                                                                                                                                                  | المصدر                                                                                                                          |
| --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| _rs`Create`_                                  | ينشئ حساب رمز مرتبط على عنوان ATA المشتق.                                                                                                                                                                              | [المصدر](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L20-L29) |
| _rs`CreateIdempotent`_                        | ينشئ حساب الرمز المرتبط، لكنه ينجح أيضاً في حال كان ATA موجوداً بالفعل لنفس المالك والسك.                                                                                                                              | [المصدر](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | يشتق عنوان ATA ويستخدم _rs`create_pda_account`_ بالإضافة إلى CPIs إلى برنامج الرموز المحدد لتهيئة حساب الرموز. عندما يكون البرنامج المحدد هو برنامج امتدادات الرموز، يقوم معالج ATA أيضاً بتهيئة امتداد المالك الثابت. | [المصدر](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | أداة مساعدة يستخدمها _rs`process_create_associated_token_account`_ لإنشاء حساب PDA عبر CPI إلى برنامج النظام.                                                                                                          | [المصدر](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |
...
1. تعليمة _rs`CreateAccount`_ الخاصة بـ System Program تنشئ حسابًا جديدًا معفيًا
   من rent وتعين Token Program كمالك برنامج للحساب الجديد.
...
قم بتضمين تعليمة _rs`CreateAccount`_ وتعليمة تهيئة الحساب الرمزي في نفس
المعاملة.

أثناء تهيئة الحساب الرمزي، يتحقق Token Program من أن الحساب غير مهيأ بالفعل وأنه
معفى من rent.

<Callout type="info">
  يوضح القسم أدناه كيفية إنشاء حساب رمزي عن طريق استدعاء تعليمات System Program
  وToken Program مباشرة. بالنسبة لمعظم التطبيقات، استخدم Associated Token
  Program بدلاً من ذلك. استخدم استدعاءات System Program وToken Program المباشرة
  عندما يكون لديك سبب محدد لعدم استخدام associated token account أو عندما تحتاج
  إلى إنشاء حسابات رمزية PDA مخصصة عن طريق إجراء استدعاءات CPI لتعليمات System
  Program وToken Program من برنامج سولانا الخاص بك.
</Callout>
...
| --------------------------------- | ---------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_                     | حقول الحساب الرمزي الأساسية المخزنة في كل حساب رمزي.                               | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`InitializeAccount`_           | تعليمة تهيئة حساب رمزي تتوقع حساب المالك وحساب rent sysvar في قائمة حساباتها.      | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L45-L63)   | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L75-L93)   |
| _rs`InitializeAccount2`_          | تعليمة تهيئة حساب رمزي تمرر المالك في instruction data بدلاً من قائمة الحسابات.    | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L352-L365) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L419-L433) |
| _rs`InitializeAccount3`_          | تعليمة تهيئة حساب رمزي تمرر المالك في instruction data ولا تتطلب حساب rent sysvar. | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L377-L387) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L445-L456) |
| _rs`_process_initialize_account`_ | منطق المعالج المشترك لتهيئة الحساب الرمزي.                                         | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L83-L141)      | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L153-L234)     |
| _rs`process_initialize_account`_  | معالج عام لـ _rs`InitializeAccount`_.                                              | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L143-L150)     | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L236-L240)     |
| _rs`process_initialize_account2`_ | معالج عام لـ _rs`InitializeAccount2`_.                                             | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L152-L160)     | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L242-L246)     |
| _rs`process_initialize_account3`_ | معالج عام لـ _rs`InitializeAccount3`_.                                             | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L162-L170)     | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L248-L252)     |

### Typescript

توضح الأمثلة أدناه باستخدام `Kit` النهج الموصى به باستخدام `@solana/kit`. تم
تضمين الأمثلة القديمة باستخدام `@solana/web3.js` للرجوع إليها.
