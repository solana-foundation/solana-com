...
title: تحويل الرموز
description:
  نقل الرموز بين حسابات الرموز لنفس السك باستخدام تعليمات النقل في Token
  Program.
...
## ما هو تأثير نقل الرموز؟

نقل الرموز يحرك الرموز من token account إلى token account آخر لنفس السك.

عمليات النقل لا تغير مخزون السك. تقوم عمليات النقل بتحديث الأرصدة بين حسابات
الرموز فقط.
...
الأمثلة أدناه تستخدم _rs`TransferChecked`_، والتي تتطلب من المستدعي توفير السك
والكسور العشرية بحيث يمكن للتعليمة التحقق من السك المتوقع ودقة الرمز قبل نقل
الرموز بين الحسابات.

في Token Extensions Program، _rs`Transfer`_ تم إهماله لصالح
_rs`TransferChecked`_ أو _rs`TransferCheckedWithFee`_.

مالك الحساب المصدر أو مفوض معتمد يوقع على عملية النقل. في Token Extensions
Program، يمكن لمفوض دائم للسك أيضًا تفويض النقل إذا كان السك يحتوي على ملحق
المفوض الدائم مفعلاً.

### المرجع المصدري

| العنصر                 | الوصف                                                                                                                                                                   | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_          | حالة token account تخزن الأرصدة التي يتم تحديثها بواسطة عملية النقل.                                                                                                    | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | تعليمة نقل تحرك الرموز بين حسابات الرموز دون أن تتطلب من المستدعي توفير السك أو الكسور العشرية. في Token Extensions Program، استخدم _rs`TransferChecked`_ بدلاً من ذلك. | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | تعليمة نقل تتطلب من المستدعي توفير السك والكسور العشرية وتتحقق من هذه القيم قبل نقل الرموز بين حسابات الرموز لنفس السك.                                                 | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
