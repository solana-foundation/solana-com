...
title: تحويل الرموز
description:
  انقل الرموز بين حسابات الرموز لنفس النعناع باستخدام تعليمات التحويل في Token
  Program.
...
## ما الذي يفعله نقل الرموز؟

نقل الرموز ينقل الرموز من حساب رموز إلى حساب رموز آخر لنفس النعناع.

عمليات النقل لا تغير من إمداد النعناع. عمليات النقل تحدّث فقط الأرصدة بين حسابات
الرموز.

## كيفية نقل الرموز

نقل الرموز يستخدم تعليمة _rs`Transfer`_ أو _rs`TransferChecked`_ من Token
Program.

الأمثلة أدناه تستخدم _rs`TransferChecked`_، التي تتطلب من المستدعي تقديم النعناع
والكسور العشرية حتى تتمكن التعليمة من التحقق من النعناع المتوقع ودقة الرمز قبل
نقل الرموز بين الحسابات.

في Token Extensions Program، تم إهمال _rs`Transfer`_ لصالح _rs`TransferChecked`_
أو _rs`TransferCheckedWithFee`_.

مالك الحساب المصدر أو مفوض معتمد يوقّع على النقل. في Token Extensions Program،
يمكن أيضًا لمفوض دائم للنعناع أن يصرّح بالنقل إذا كان النعناع قد فعّل ملحق
المفوض الدائم.
...
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_          | تخزّن حالة token account الأرصدة المحدثة بواسطة عملية النقل.                                                                                                               | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | تعليمة نقل تنقل الرموز بين حسابات الرموز دون أن تتطلب من المستدعي تقديم النعناع أو الكسور العشرية. في Token Extensions Program، استخدم _rs`TransferChecked`_ بدلاً من ذلك. | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | تعليمة نقل تتطلب من المستدعي تقديم النعناع والكسور العشرية وتفحص هذه القيم قبل نقل الرموز بين حسابات الرموز لنفس النعناع.                                                  | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | منطق معالج مشترك لعمليات نقل الرموز.                                                                                                                                       | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |

### Typescript

توضح الأمثلة أدناه باستخدام `Kit` الطريقة الموصى بها باستخدام `@solana/kit`. تم
