...
description: إغلاق حساب الرمز وإعادة لامبورتات الحساب إلى حساب وجهة.
...
يؤدي إغلاق حساب الرمز إلى حذف الحساب وإعادة لامبورتات الإيجار الخاصة بحساب الرمز
إلى حساب الوجهة.

يجب أن يكون رصيد الرمز صفرًا قبل إغلاق الحساب. يمكن إغلاق الحساب المجمد إذا كان
رصيد الرمز للحساب المجمد صفرًا. حسابات رموز SOL الملفوفة هي الاستثناء ويمكن
إغلاقها برصيد رمز لاستعادة SOL الأساسي. مالك الحساب أو سلطة إغلاق الحساب يوقع
على تعليمة الإغلاق.

## كيفية إغلاق حساب الرمز

يستخدم إغلاق حساب الرمز تعليمة _rs`CloseAccount`_ الخاصة ببرنامج الرمز.

تنقل تعليمة _rs`CloseAccount`_ لامبورتات الحساب المصدر إلى حساب الوجهة، وتمسح
الحساب المصدر، وتحذف الحساب المصدر. يطبق برنامج Token Extensions فحوصات إغلاق
إضافية لبعض الامتدادات، لكن تدفق الإغلاق الأساسي هو نفسه.

### المرجع المصدري

| العنصر                      | الوصف                                                                                   | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------------- | --------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | تخزن حالة حساب الرمز الرصيد والحالة الأصلية وسلطة الإغلاق المستخدمة أثناء إغلاق الحساب. | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | تعليمة تغلق حساب الرمز وتنقل لامبورتات حساب الرمز إلى حساب الوجهة.                      | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
...
توضح أمثلة `Kit` أدناه النهج الموصى به باستخدام `@solana/kit`. أمثلة قديمة
باستخدام `@solana/web3.js` مُضمّنة كمرجع.
