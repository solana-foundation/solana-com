...
description: إغلاق حساب رمز وإرجاع lamports الحساب إلى حساب وجهة.
...
## ماذا يفعل إغلاق حساب الرمز؟

إغلاق حساب الرمز يحذف الحساب ويعيد lamports الإيجار الخاصة بحساب الرمز إلى حساب
الوجهة.

يجب أن يكون رصيد الرمز صفرًا قبل أن يتم إغلاق الحساب. لا يزال من الممكن إغلاق
الحساب المجمد إذا كان رصيد الرمز للحساب المجمد صفرًا. تُعد حسابات رموز SOL
الملفوفة استثناءً ويمكن إغلاقها برصيد رمز لاستعادة SOL الأساسي. يوقّع مالك
الحساب أو سلطة الإغلاق الخاصة بالحساب على تعليمة الإغلاق.

## كيفية إغلاق حساب رمز

إغلاق حساب رمز يستخدم تعليمة _rs`CloseAccount`_ الخاصة ببرنامج الرموز.

تقوم تعليمة _rs`CloseAccount`_ بنقل lamports حساب المصدر إلى حساب الوجهة، وتمسح
حساب المصدر، وتحذف حساب المصدر. يطبق برنامج Token Extensions فحوصات إغلاق إضافية
لبعض الامتدادات، لكن تدفق الإغلاق الأساسي هو نفسه.

### مرجع المصدر

| العنصر                      | الوصف                                                                               | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------------- | ----------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | تخزن حالة حساب الرمز الرصيد وحالة الأصل وسلطة الإغلاق المستخدمة أثناء إغلاق الحساب. | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | تعليمة تغلق حساب رمز وتنقل lamports حساب الرمز إلى حساب وجهة.                       | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | منطق المعالج المشترك لإغلاق حساب الرمز.                                             | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |

### Typescript

توضح أمثلة `Kit` أدناه النهج الموصى به باستخدام `@solana/kit`. تم تضمين أمثلة
قديمة باستخدام `@solana/web3.js` للمرجعية.
