---
title: إلغاء المفوض
description: إزالة المفوض الحالي لحساب الرمز ومسح الرصيد المفوض المتبقي.
...
## ماذا يفعل إلغاء المفوض؟

إلغاء المفوض يمسح المفوض الحالي لحساب الرمز ويعيد المبلغ المفوض إلى الصفر.

بعد الإلغاء، لم يعد المفوض السابق يملك صلاحية نقل أو حرق الرموز نيابة عن حساب
الرمز.

## كيفية إلغاء المفوض

إلغاء المفوض يستخدم تعليمة _rs`Revoke`_ من Token Program.

تعليمة _rs`Revoke`_ تمسح حقول المفوض في حساب الرمز المصدر. في Token Program،
مالك حساب الرمز يوقع على الإلغاء. في Token Extensions Program، يمكن للمفوض
الحالي أيضاً إلغاء التفويض.

### المرجع المصدري

| العنصر               | الوصف                                       | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| -------------------- | ------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_        | حالة حساب الرمز تخزن المفوض والمبلغ المفوض. | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Revoke`_         | تعليمة تزيل المفوض الحالي من حساب الرمز.    | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L128-L140) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L170-L182) |
| _rs`process_revoke`_ | منطق المعالج المشترك لإلغاء التفويض.        | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L395-L420)     | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L643-L674)     |

### Typescript

الأمثلة `Kit` أدناه توضح النهج الموصى به باستخدام `@solana/kit`. الأمثلة القديمة
باستخدام `@solana/web3.js` مضمنة للمرجعية.
