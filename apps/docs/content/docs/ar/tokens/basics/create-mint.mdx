---
title: إنشاء حساب سك العملات
description:
  إنشاء حساب سك العملات وتكوين الأرقام العشرية للسك، وسلطة السك، وسلطة التجميد.
...
يُعرّف حساب السك ويحدد بشكل فريد عملة رقمية على سولانا، ويخزن الحالة المشتركة
التي تنطبق على جميع حسابات العملات لذلك السك.

يُعرّف Token Program نوع حساب _rs`Mint`_ كالتالي:
...
لكل عملة حساب سك واحد، وعنوان السك هو المعرف الفريد للعملة عبر المحافظ
...
[مستكشف سولانا](https://explorer.solana.com/address/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v).

## كيفية إنشاء حساب السك

يتطلب إنشاء السك تعليمتين:

1. تعليمة _rs`CreateAccount`_ من System Program تُنشئ حساباً جديداً معفى من rent
   وتعيّن Token Program كمالك للبرنامج للحساب الجديد.
2. تعليمة _rs`InitializeMint`_ أو _rs`InitializeMint2`_ من Token Program تُهيئ
   الحساب الجديد كحساب سك.
...
| ------------------------------ | ------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                     | حالة السك الأساسية المخزنة في كل حساب سك.                                      | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`InitializeMint`_           | تعليمة تهيئة السك التي تتوقع حساب rent sysvar في قائمة الحسابات الخاصة بها.    | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L24-L44)   | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L50-L74)   |
| _rs`InitializeMint2`_          | تعليمة تهيئة السك التي لا تتطلب حساب rent sysvar في قائمة الحسابات الخاصة بها. | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L401-L414) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L471-L486) |
| _rs`_process_initialize_mint`_ | منطق المعالج المشترك لتهيئة السك.                                              | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
| _rs`process_initialize_mint`_  | المعالج العام لـ _rs`InitializeMint`_.                                         | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L62-L70)       | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L132-L140)     |
| _rs`process_initialize_mint2`_ | المعالج العام لـ _rs`InitializeMint2`_.                                        | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L72-L81)       | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L142-L151)     |
...
يخزن mint account المعلومات الأساسية للرمز فقط (الإمداد، الكسور العشرية،
الصلاحيات). لإضافة بيانات وصفية مقروءة للإنسان مثل الاسم والرمز والصورة إلى
الرمز الخاص بك، لديك خياران:

<Cards>
  <Card title="بيانات رموز Metaplex الوصفية" href="/docs/tokens/metaplex">
   أضف البيانات الوصفية باستخدام برنامج Metaplex Token Metadata. يعمل مع كل من Token Program الأصلي وToken-2022.
  </Card>

  <Card title="البيانات الوصفية لامتدادات الرموز" href="/docs/tokens/extensions/metadata">
