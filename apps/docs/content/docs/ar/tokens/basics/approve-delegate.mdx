...
description: تفويض عنوان آخر لتحويل أو حرق كمية محدودة من الرموز من حساب رمزي.
...
## ما هو دور الموافقة على المفوض؟

الموافقة على المفوض تسمح لعنوان آخر بتحويل أو حرق كمية محدودة من الرموز من حساب
رمزي نيابة عن مالك الحساب الرمزي.

يخزن الحساب الرمزي مفوضاً واحداً حالياً ومبلغاً مفوضاً واحداً في كل مرة.
الموافقة على مفوض جديد تستبدل المفوض السابق والحد المسموح به على الحساب.

## كيفية الموافقة على مفوض

الموافقة على المفوض تستخدم تعليمة _rs`Approve`_ أو _rs`ApproveChecked`_ من Token
Program.

تستخدم الأمثلة أدناه _rs`ApproveChecked`_، والتي تتطلب من المستدعي توفير العملة
والكسور العشرية حتى يمكن للتعليمة التحقق من العملة المتوقعة ودقة الرمز قبل
معالجة الموافقة.

يوقع مالك الحساب الرمزي على الموافقة.

### مرجع المصدر

| العنصر                | الوصف                                                                                                                                  | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_         | تخزن حالة الحساب الرمزي المالك والمفوض والمبلغ المفوض.                                                                                 | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | تعليمة الموافقة على المفوض التي تسجل المفوض والحد المسموح به لحساب رمزي دون مطالبة المستدعي بتوفير العملة أو الكسور العشرية.           | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | تعليمة الموافقة على المفوض التي تتطلب من المستدعي توفير العملة والكسور العشرية والتحقق من تلك القيم قبل تسجيل المفوض والحد المسموح به. | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
| _rs`process_approve`_ | منطق المعالج المشترك للموافقة على المفوض.                                                                                              | [المصدر](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L344-L392)     | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L582-L640)     |
