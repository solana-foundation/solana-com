---
title: إيقاف السك مؤقتاً
description:
  إنشاء mint من نوع Token-2022 مع صلاحية إيقاف مؤقت، حظر التحويلات أثناء إيقاف
  الـ mint، واستئناف النشاط لاحقاً.
...
## ما هو الـ Mint القابل للإيقاف المؤقت؟

إضافة _rs`PausableConfig`_ في برنامج Token Extensions تمنح صلاحية الإيقاف المؤقت
القدرة على إيقاف واستئناف نشاط الرموز للـ mint بالكامل.

عندما يتم إيقاف الـ mint مؤقتاً، يرفض برنامج Token Extensions:

- التحويلات
- عمليات السك
- عمليات الحرق

عند استئناف الـ mint، تعمل هذه التعليمات بشكل طبيعي مرة أخرى.

## كيفية إنشاء وإيقاف واستئناف Mint

لإنشاء وإيقاف واستئناف mint:

1. احسب حجم حساب الـ mint والإيجار المطلوب للـ mint وإضافة _rs`PausableConfig`_.
2. أنشئ حساب الـ mint باستخدام _rs`CreateAccount`_، وقم بتهيئة
   _rs`PausableConfig`_، وقم بتهيئة الـ mint باستخدام _rs`InitializeMint`_.
3. اسك الرموز وأنشئ حسابات الرموز للـ mint.
4. استخدم _rs`Pause`_ لحظر نشاط الرموز للـ mint.
5. استخدم _rs`Resume`_ للسماح بنشاط الرموز مرة أخرى.

<ScrollyCoding>

## !!steps احسب حجم الحساب

احسب حجم حساب الـ mint للـ mint الأساسي بالإضافة إلى إضافة _rs`PausableConfig`_.
...
احسب الإيجار باستخدام الحجم المطلوب للـ mint بالإضافة إلى إضافة
...
أنشئ حساب mint account بالمساحة المحسوبة وlamport.
...
## !!steps تهيئة PausableConfig

قم بتهيئة امتداد _rs`PausableConfig`_ على mint.
...
قم بتهيئة mint باستخدام _rs`InitializeMint`_ في نفس المعاملة.
...
## !!steps إيقاف mint مؤقتاً

أوقف mint مؤقتاً باستخدام _rs`Pause`_.
...
استأنف mint باستخدام _rs`Resume`_.
...
<Callout type="info" title="ترتيب التعليمات">
  يجب أن يأتي _rs`PausableInstruction::Initialize`_ قبل _rs`InitializeMint`_.
  يجب تضمين _rs`CreateAccount`_ و _rs`PausableInstruction::Initialize`_ و
...
| العنصر                                | الوصف                                                                                      | المصدر                                                                                                                                |
| ------------------------------------- | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PausableConfig`_                  | امتداد Mint الذي يخزن صلاحية الإيقاف المؤقت وما إذا كان mint موقوفاً حالياً.               | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/mod.rs#L17-L22)         |
| _rs`PausableInstruction::Initialize`_ | تعليمة تقوم بتهيئة إعدادات الإيقاف المؤقت قبل _rs`InitializeMint`_.                        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L20-L32) |
| _rs`PausableInstruction::Pause`_      | تعليمة تحدد mint كموقوف مؤقتاً.                                                            | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L33-L44) |
| _rs`PausableInstruction::Resume`_     | تعليمة تحدد mint كغير موقوف.                                                               | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L45-L56) |
| _rs`process_initialize`_              | منطق المعالج الذي يهيئ _rs`PausableConfig`_ على mint غير مهيأ ويخزن صلاحية الإيقاف المؤقت. | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L22-L35)     |
| _rs`process_toggle_pause`_            | منطق المعالج الذي يتحقق من صلاحية الإيقاف المؤقت قبل تبديل علامة الإيقاف المؤقت لـ mint.   | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L39-L64)     |

### Typescript

يستخدم مثال `Kit` أدناه التعليمات المُولدة مباشرة. تم تضمين أمثلة قديمة باستخدام
