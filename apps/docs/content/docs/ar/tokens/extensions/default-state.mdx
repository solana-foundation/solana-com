...
description:
  قم بتكوين mint الخاص ببرنامج Token Extensions بحيث تبدأ حسابات الرموز المُنشأة
  حديثًا في حالة افتراضية محددة، مثل التجميد.
...
## ما هي الحالة الافتراضية للحساب؟

يحدد امتداد mint الخاص بـ _rs`DefaultAccountState`_ في Token Extensions Program
الحالة الأولية _rs`AccountState`_ لكل token account جديد يتم إنشاؤه لذلك الـ
mint.

يُستخدم هذا بشكل شائع لجعل حسابات الرموز الجديدة تبدأ في حالة مُجمدة. لا يمكن
استخدام حسابات الرموز التي تبدأ مُجمدة حتى تقوم سلطة التجميد الخاصة بالـ mint
بإلغاء تجميدها.

## كيفية إنشاء mint مع حالة افتراضية للحساب

لإنشاء mint مع حالة افتراضية للحساب:

1. احسب حجم حساب الـ mint والإيجار (rent) المطلوب للـ mint وامتداد
   _rs`DefaultAccountState`_.
2. أنشئ حساب الـ mint باستخدام _rs`CreateAccount`_، وهيّئ
   _rs`DefaultAccountState`_، ثم هيّئ الـ mint باستخدام _rs`InitializeMint`_.
3. أنشئ حسابات رموز للـ mint ولاحظ أن كل token account جديد يبدأ في الحالة
   الافتراضية الحالية.
4. استخدم _rs`UpdateDefaultAccountState`_ لتغيير الحالة التي يجب أن تبدأ بها
   حسابات الرموز المستقبلية.
...
_rs`DefaultAccountState`_. هذا هو الحجم المُستخدَم في _rs`CreateAccount`_.
...
أنشئ حساب الـ mint بالمساحة والـ lamports المحسوبة.
...
## !!steps تهيئة DefaultAccountState

تهيئة الامتداد _rs`DefaultAccountState`_ على العملة الرمزية.
...
## !!steps تهيئة العملة الرمزية

تهيئة العملة الرمزية باستخدام _rs`InitializeMint`_ في نفس المعاملة.
...
## !!steps إنشاء حساب رمزي

إنشاء حساب رمزي للعملة الرمزية. نظرًا لأن الحالة الافتراضية الحالية هي
_rs`Frozen`_، يبدأ الحساب الرمزي الجديد في حالة مجمدة.
...
## !!steps تحديث حالة الحساب الافتراضية

استخدم _rs`UpdateDefaultAccountState`_ لتغيير الحالة الافتراضية بحيث لا تبدأ
الحسابات الرمزية المستقبلية في حالة مجمدة.
...
<Callout type="info" title="ترتيب التعليمات">
  يجب أن يأتي _rs`DefaultAccountStateInstruction::Initialize`_ قبل
  _rs`InitializeMint`_. يجب تضمين _rs`CreateAccount`_
  و_rs`DefaultAccountStateInstruction::Initialize`_ و_rs`InitializeMint`_ في نفس
  المعاملة.
</Callout>
...
| العنصر                                           | الوصف                                                                                         | المصدر                                                                                                                                             |
| ------------------------------------------------ | --------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`DefaultAccountState`_                        | امتداد العملة الرمزية الذي يخزن الحالة التي يجب أن ترثها الحسابات الرمزية الجديدة.            | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/mod.rs#L16-L21)         |
| _rs`DefaultAccountStateInstruction::Initialize`_ | التعليمة التي تهيئ حالة الحساب الرمزي الافتراضية للعملة الرمزية قبل _rs`InitializeMint`_.     | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L20-L36) |
| _rs`DefaultAccountStateInstruction::Update`_     | التعليمة التي تسمح لصلاحية التجميد بتغيير الحالة الافتراضية للحسابات الرمزية المستقبلية.      | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L37-L53) |
| _rs`process_initialize_default_account_state`_   | منطق المعالج الذي يكتب الحالة الافتراضية الأولية على عملة رمزية غير مهيأة.                    | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L29-L40)     |
| _rs`process_update_default_account_state`_       | منطق المعالج الذي يتحقق من صلاحية التجميد قبل تغيير الحالة الافتراضية المخزنة للعملة الرمزية. | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L43-L74)     |

### TypeScript

يستخدم المثال `Kit` أدناه التعليمات المُنشأة مباشرة. تم تضمين أمثلة قديمة
