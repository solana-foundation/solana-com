...
title: تحويل الرموز
description:
  تعلم كيفية تحويل الرموز بشكل خاص من حساب رموز إلى آخر.
---

## كيفية نقل الرموز بشكل سري من token account إلى آخر

لنقل الرموز المميزة بشكل سري من token account إلى token account آخر، يجب على كلٍّ من
المُرسِل والمُستقبِل أن يمتلكا token accounts مُهيَّأة بحالة
_rs`ConfidentialTransferAccount`_ ومعتمدة لاستخدام الأرصدة
السرية. كما يجب أن يمتلك token account المُرسِل رصيداً سرياً متاحاً للتحويل منه.
...
1. أنشئ
   ...
   **إثبات المساواة
   ...
   يتحقق من أن نص التشفير الخاص بالرصيد المتاح الجديد بعد عملية النقل يتطابق مع
   [التزام Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme) المقابل
   له، مما يضمن أن الرصيد المتاح الجديد لحساب المصدر محسوبٌ بشكل صحيح على النحو
   `new_balance = current_balance - transfer_amount`.

   **إثبات صحة نص التشفير
   ...
   يتحقق من أن نصوص تشفير مبلغ التحويل مُولَّدة بشكل صحيح لجميع الأطراف الثلاثة
   (المصدر والوجهة والمراجع)، مما يضمن تشفير مبلغ التحويل بشكل صحيح باستخدام
   ...
   **إثبات النطاق
   ...
   يتحقق من أن الرصيد المتاح الجديد ومبلغ التحويل (المقسَّم إلى بتات
   منخفضة/عالية) كلها غير سالبة وضمن نطاق محدد.

2. لكل إثبات:
   - استدعِ برنامج إثبات ZK ElGamal للتحقق من بيانات الإثبات.
   - خزِّن البيانات الوصفية الخاصة بالإثبات في حساب "حالة السياق" للاستخدام في
     التعليمات الأخرى.
...
4. أغلق حسابات حالة سياق الإثبات لاسترداد SOL المُستخدَم في إنشائها.

يوضح المخطط التالي الخطوات المتبعة في نقل الرموز من token account الخاص
بالمُرسِل إلى token account الخاص بالمستلم.
...
يُوضِّح المثال التالي كيفية نقل الرموز المميزة بشكل سري من حساب إلى
آخر. يجب أن يكون كلا الحسابين مُهيَّأين مسبقاً للأرصدة السرية، وأن
يمتلك المُرسِل رصيداً سرياً متاحاً.

تعتمد الأرصدة السرية على برنامج ZK ElGamal Proof، المُفعَّل
على الشبكة الرئيسية وشبكة devnet. لا يُفعِّل `solana-test-validator` الافتراضي هذا البرنامج، غير أن validator المحلي الذي يعمل بنسخ من الشبكة الرئيسية مثل [Surfpool](https://surfpool.run) يدعمه.
شغِّل المثال على أحد هذه البيئات (يستخدم الكود شبكة devnet) مع حساب دافع ممول،
واستبدل العناصر النائبة بعنوان mint الخاص بك وحسابَي المُرسِل والمُستقبِل.
