---
title: تحويل الرموز
description: تعلم كيفية تحويل الرموز بشكل خاص من حساب رموز إلى آخر.
---
...
## كيفية تحويل الرموز بشكل سري من حساب رموز إلى آخر

لتحويل الرموز بشكل سري من حساب رموز إلى آخر، يجب أن يكون لدى كل من المرسل
والمستقبل حسابات رموز مهيأة بحالة _rs`ConfidentialTransferAccount`_ ومعتمدة
للتحويلات السرية. يجب أيضاً أن يكون لدى حساب رموز المرسل رصيد سري متاح للتحويل
منه.
...
1. أنشئ
   ...
   **إثبات المساواة
   ...
   يتحقق من أن نص الرصيد المتاح الجديد المشفر بعد التحويل يطابق
   [التزام Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme) المقابل
   له، مما يضمن حساب الرصيد المتاح الجديد لحساب المصدر بشكل صحيح كـ
   ...
   **إثبات صحة النص المشفر
   ...
   يتحقق من أن نصوص مبلغ التحويل المشفرة تم إنشاؤها بشكل صحيح لجميع الأطراف
   الثلاثة (المصدر، الوجهة، والمدقق)، مما يضمن تشفير مبلغ التحويل بشكل صحيح تحت
...
2. لكل إثبات:
   ...
   - خزّن البيانات الوصفية الخاصة بالإثبات في حساب "حالة سياق الإثبات"
...
4. أغلق حسابات حالة سياق الإثبات لاسترداد SOL المستخدم لإنشائها.

يوضح الرسم التخطيطي التالي الخطوات المتضمنة في تحويل الرموز من حساب رمز المرسل
إلى حساب رمز المستقبل.
...
لتحويل الرموز بشكل سري من حساب رمز إلى آخر، يجب عليك:
...
يوفر صندوق `spl_token_client` الطرق التالية:
...
لتشغيل المثال، ابدأ مدقق محلي مع برنامج Token Extension المستنسخ من الشبكة
الرئيسية باستخدام الأمر التالي. يجب أن يكون لديك Solana CLI مثبتاً لبدء المدقق
المحلي.
...
<Callout type="info">
  في وقت كتابة هذا، لم يتم تفعيل التحويلات السرية على المدقق المحلي الافتراضي.
  يجب عليك استنساخ برنامج Token Extension من الشبكة الرئيسية لتشغيل مثال الكود.
</Callout>
