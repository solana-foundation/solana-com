...
1. أنشئ
   ...
   **إثبات المساواة
   ...
   يتحقق من أن النص المشفر للرصيد المتاح المتبقي بعد السحب يطابق
   [التزام Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme) المقابل
   له، مما يضمن حساب الرصيد المتاح الجديد للحساب بشكل صحيح كـ
...
4. أغلق حسابات الإثبات لاسترداد SOL المستخدم لإنشائها.

يوضح الرسم التخطيطي التالي الخطوات المتضمنة في سحب الرموز من الرصيد السري المتاح
إلى الرصيد العام:
...
- توليد إثبات مساواة وإثبات نطاق من جانب العميل
- استدعاء برنامج إثبات Zk ElGamal للتحقق من الإثباتات وتهيئة حسابات "حالة
  السياق"
- استدعاء تعليمة
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  مع توفير حسابي الإثبات.
- إغلاق حسابي الإثبات لاسترداد الإيجار.

توفر حزمة `spl_token_client` الطرق التالية:
...
لتشغيل المثال، ابدأ مدقق محلي مع برنامج Token Extension Program المستنسخ من
الشبكة الرئيسية باستخدام الأمر التالي. يجب أن يكون لديك Solana CLI مثبتًا لبدء
المدقق المحلي.
...
<Callout type="info">
  ...
  يجب عليك استنساخ برنامج Token Extension Program من الشبكة الرئيسية لتشغيل مثال
  الكود.
</Callout>
