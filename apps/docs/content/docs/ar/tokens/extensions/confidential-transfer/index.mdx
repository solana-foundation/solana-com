...
<Callout type="warn">
  برنامج ZK ElGamal معطل مؤقتاً على الشبكة الرئيسية وشبكة التطوير حيث يخضع
...
تتيح لك التحويلات السرية نقل الرموز بين حسابات الرموز دون الكشف عن مبلغ التحويل.
هذا مفيد للمعاملات التي تحافظ على الخصوصية. فقط مبالغ التحويل وأرصدة الرموز
خاصة. تظل عناوين حسابات الرموز عامة.

- [نظرة عامة على البروتوكول](https://www.solana-program.com/docs/confidential-balances/overview) -
  تفاصيل حول البروتوكول التشفيري الأساسي
...
- [كتاب وصفات الأرصدة السرية](https://github.com/solana-developers/Confidential-Balances-Sample) -
  مقتطفات كود حول كيفية استخدام امتداد التحويل السري
...
إلى برنامج Token Extension تتيح لك نقل الرموز بين الحسابات دون الكشف عن مبلغ
التحويل.
...
1. إنشاء حساب إصدار مع امتداد التحويل السري.
2. إنشاء حسابات رموز مع امتداد التحويل السري للمرسل والمستلم.
3. إصدار رموز إلى حساب المرسل.
4. **إيداع** الرصيد العام للمرسل إلى **الرصيد السري المعلق**.
5. **تطبيق** الرصيد المعلق للمرسل إلى **الرصيد السري المتاح**.
6. **نقل** الرموز بشكل سري من حساب رموز المرسل إلى حساب رموز المستلم.
7. **تطبيق** الرصيد المعلق للمستلم إلى **الرصيد السري المتاح**.
8. **سحب** الرصيد السري المتاح للمستلم إلى **الرصيد العام**.

لمزيد من التفاصيل حول الخطوات في تدفق التحويل السري، راجع الصفحات المقابلة:
...
<Cards>
  ...
  >
    كيفية نقل الرموز بشكل سري بين token accounts
  </Card>
  <Card
    title="دليل المُصدِر"
    href="/docs/tokens/extensions/confidential-transfer/issuer-guide"
  >
    كيفية إصدار وتشغيل رمز النقل السري (سياسة الموافقة، المدققون، الرسوم،
    الإصدار والحرق)
  </Card>
</Cards>

يوضح الرسم التخطيطي أدناه تسلسلاً مفصلاً للتدفق الأساسي لتحويلات الرموز السرية:
...
يوضح مثال Rust أدناه خطوات التحويلات السرية للرموز المقابلة للرسم التخطيطي
السابق. تم فصل الخطوات إلى دوال مساعدة للوضوح.

لتشغيل المثال، ابدأ validator محلي مع برنامج Token Extension المستنسخ من mainnet
باستخدام الأمر التالي. يجب أن يكون لديك Solana CLI مثبتاً لاستخدام validator
المحلي.
...
<Callout type="info">
  في وقت كتابة هذا، لم يتم تفعيل التحويلات السرية على validator المحلي
  الافتراضي. يجب عليك استنساخ برنامج Token Extension من mainnet لتشغيل مثال
  الكود.
</Callout>
...
القائمة الكاملة لـ
[تعليمات](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
ملحق التحويل السري هي كما يلي:

| التعليمة                            | الوصف                                                                                                                            |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | يقوم بإعداد حساب mint للتحويلات السرية. يجب تضمين هذه التعليمة في نفس المعاملة مع تعليمة _rs`TokenInstruction::InitializeMint`_. |
| _rs`UpdateMint`_                    | يحدّث إعدادات التحويل السري لحساب mint.                                                                                          |
| _rs`ConfigureAccount`_              | يقوم بإعداد حساب token للتحويلات السرية.                                                                                         |
| _rs`ApproveAccount`_                | يوافق على حساب token للتحويلات السرية إذا كان mint يتطلب الموافقة على حسابات token الجديدة.                                      |
| _rs`EmptyAccount`_                  | يفرغ الأرصدة السرية المعلقة والمتاحة للسماح بإغلاق حساب token.                                                                   |
| _rs`Deposit`_                       | يحول رصيد token العام إلى رصيد سري معلق.                                                                                         |
| _rs`Withdraw`_                      | يحول الرصيد السري المتاح مرة أخرى إلى رصيد عام.                                                                                  |
| _rs`Transfer`_                      | يحول الرموز بين حسابات token بشكل سري.                                                                                           |
| _rs`ApplyPendingBalance`_           | يحول الرصيد المعلق إلى رصيد متاح بعد الإيداعات أو التحويلات.                                                                     |
| _rs`EnableConfidentialCredits`_     | يسمح لحساب token باستقبال تحويلات الرموز السرية.                                                                                 |
| _rs`DisableConfidentialCredits`_    | يحظر التحويلات السرية الواردة مع السماح بالتحويلات العامة.                                                                       |
| _rs`EnableNonConfidentialCredits`_  | يسمح لحساب token باستقبال تحويلات الرموز العامة.                                                                                 |
| _rs`DisableNonConfidentialCredits`_ | يحظر التحويلات العادية لجعل الحساب يستقبل التحويلات السرية فقط.                                                                  |
| _rs`TransferWithFee`_               | يحول الرموز بين حسابات token بشكل سري مع رسوم.                                                                                   |
| _rs`ConfigureAccountWithRegistry`_  | طريقة بديلة لتكوين حسابات token للتحويلات السرية باستخدام حساب _rs`ElGamalRegistry`_ بدلاً من إثبات _rs`VerifyPubkeyValidity`_.  |
