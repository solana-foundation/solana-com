...
description:
  تعرّف على كيفية إنشاء token account مع تفعيل ميزة الأرصدة السرية.
---

## كيفية إنشاء token account مع الأرصدة السرية

تتيح الأرصدة السرية إجراء تحويلات رمزية خاصة عن طريق إضافة حالة إضافية إلى
token account. يشرح هذا القسم كيفية إنشاء token account مع تفعيل الامتداد
المطلوب.

يوضح المخطط التالي الخطوات المتبعة في إنشاء token account مع تفعيل الأرصدة
السرية:
...
### حالة token account للأرصدة السرية
...
يحتوي _rs`ConfidentialTransferAccount`_ على عدة حقول تُدير الأرصدة السرية:

- **approved**: حالة الموافقة على الحساب للأرصدة السرية. إذا كان إعداد
  `auto_approve_new_accounts` في mint account مضبوطاً على `true`، فسيتم
  الموافقة تلقائياً على جميع token accounts للأرصدة السرية.
...
يستخدم إنشاء وضبط token account للأرصدة السرية التعليمات التالية، والتي
تتناسب جميعها مع معاملة واحدة:
...
4. **ضبط الأرصدة السرية**: استدعاء تعليمة
   ...
   الخاصة بـ Token Extension Program، مع الإشارة إلى حساب حالة سياق الإثبات
   عبر _rs`ProofLocation::ContextStateAccount`_، لتهيئة حالة
   _rs`ConfidentialTransferAccount`_.

<Callout type="info">

**يستطيع مالك token account فقط تفعيل الأرصدة السرية على token account**.

تتطلب تعليمة _rs`ConfigureAccount`_ إنشاء مفاتيح تشفير وإثبات من جانب العميل،
ولا يمكن إنشاؤه إلا من قِبل مالك token account.

يتحقق إثبات صحة pubkey من أن مفتاح ElGamal العام للحساب صالح. يُنشأ باستخدام
_rs`build_pubkey_validity_proof_data`_، ويُتحقق منه على السلسلة بواسطة برنامج
ZK ElGamal Proof في حساب حالة سياق، ثم يُشار إليه من _rs`ConfigureAccount`_
عبر _rs`ProofLocation::ContextStateAccount`_، بحيث لا تنتقل أي بيانات إثبات
في تعليمة الرمز نفسها. للاطلاع على تفاصيل التنفيذ، راجع:
...
ينشئ الكود التالي associated token account ويضبطه لاستخدام الأرصدة السرية مع
mint متوافق موجود مسبقاً.

تعتمد الأرصدة السرية على برنامج ZK ElGamal Proof، المُفعَّل على الشبكة
الرئيسية وشبكة devnet. لا يُفعِّل `solana-test-validator` الافتراضي هذا
البرنامج، غير أن مُحقق محلي يعمل بنسخ شبكة mainnet مثل [Surfpool](https://surfpool.run)
يدعمه. شغِّل المثال على أحد هذين الخيارين (يستخدم الكود شبكة devnet) مع
حساب دافع ممول، واستبدل العنصر النائب لـ mint بـ mint تم إنشاؤه وفقاً لـ
