...
- **approved**: حالة الموافقة على التحويلات السرية للحساب. إذا تم تعيين إعداد
  `auto_approve_new_accounts` لحساب mint على `true`، فإن جميع حسابات token تتم
  الموافقة عليها تلقائياً للتحويلات السرية.
...
- **actual_pending_balance_credit_counter**: قيمة
  `pending_balance_credit_counter` على حساب token في وقت معالجة آخر تعليمة
...
تتم إضافة جميع مبالغ الإيداعات والتحويلات في البداية إلى الرصيد المعلق. يجب على
مالكي حسابات token استخدام تعليمة _rs`ApplyPendingBalance`_ لتحويل الرصيد المعلق
إلى الرصيد المتاح. لا تؤثر التحويلات أو الإيداعات الواردة على الرصيد المتاح
لحساب token.
...
لأن فك تشفير ElGamal يتطلب مزيداً من الحساب للأرقام الأكبر. يمكنك العثور على
تطبيق العمليات الحسابية للنص المشفر
...
1. يبحث العميل عن الأرصدة المعلقة والمتاحة الحالية، ويشفر المجموع، ويقدم
   `decryptable_available_balance` مشفراً باستخدام مفتاح AES لمالك حساب token.

2. تتتبع عدادات الأرصدة المعلقة المتوقعة والفعلية التغييرات في قيمة العداد بين
   وقت إنشاء تعليمة _rs`ApplyPendingBalance`_ ومعالجتها:
   - `expected_pending_balance_credit_counter`: قيمة
     `pending_balance_credit_counter` عندما ينشئ العميل تعليمة
     _rs`ApplyPendingBalance`_
   - `actual_pending_balance_credit_counter`: قيمة
     `pending_balance_credit_counter` على حساب token في وقت معالجة تعليمة
     _rs`ApplyPendingBalance`_

تشير العدادات المتطابقة المتوقعة/الفعلية إلى أن `decryptable_available_balance`
يطابق {/_ INLINE_CODE_PLACEHOLDER_11a8b30a620b105f786c13a04043b092 _/.

عند جلب حالة حساب الرمز لقراءة `decryptable_available_balance`، تتطلب قيم
...
1. ابدأ بـ `decryptable_available_balance` من حساب الرمز
...
1. **إنشاء حساب الرمز**: استدعِ تعليمة
   _rs`AssociatedTokenAccountInstruction:Create`_ من Associated Token Program
   لإنشاء حساب الرمز.
