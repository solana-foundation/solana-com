...
<Callout type="warn">
  برنامج ZK ElGamal معطل مؤقتاً على الشبكة الرئيسية وشبكة المطورين حيث يخضع
  لتدقيق أمني. هذا يعني أن امتداد التحويلات السرية غير متاح حالياً. بينما
  المفاهيم لا تزال صالحة، فإن أمثلة الكود لن تعمل.
</Callout>

## كيفية إنشاء حساب رمز مميز مع امتداد التحويل السري

يتيح امتداد التحويل السري تحويلات الرموز المميزة الخاصة من خلال إضافة حالة
إضافية إلى حساب الرمز المميز. يشرح هذا القسم كيفية إنشاء حساب رمز مميز مع تمكين
هذا الامتداد.

يوضح الرسم التخطيطي التالي الخطوات المتضمنة في إنشاء حساب رمز مميز مع امتداد
التحويل السري:
...
### حالة حساب الرمز المميز للتحويل السري
...
يحتوي _rs`ConfidentialTransferAccount`_ على عدة حقول لإدارة التحويلات السرية:

- **approved**: حالة موافقة الحساب على التحويلات السرية. إذا تم تعيين إعداد
  `auto_approve_new_accounts` لحساب mint كـ `true`، فإن جميع حسابات الرموز
  المميزة تتم الموافقة عليها تلقائياً للتحويلات السرية.

- **elgamal_pubkey**: المفتاح العام ElGamal المستخدم لتشفير الأرصدة ومبالغ
  التحويل.

- **pending_balance_lo**: البتات الـ 16 السفلية المشفرة للرصيد المعلق. يتم تقسيم
  الرصيد إلى أجزاء عالية ومنخفضة لفك التشفير بكفاءة.

- **pending_balance_hi**: البتات الـ 48 العليا المشفرة للرصيد المعلق. يتم تقسيم
  الرصيد إلى أجزاء عالية ومنخفضة لفك التشفير بكفاءة.
...
- **allow_confidential_credits**: إذا كانت القيمة true، يسمح بالتحويلات السرية
  الواردة.

- **allow_non_confidential_credits**: إذا كانت القيمة true، يسمح بالتحويلات غير
  السرية الواردة.

- **pending_balance_credit_counter**: يحسب اعتمادات الرصيد المعلق الواردة من
  تعليمات الإيداع والتحويل.

- **maximum_pending_balance_credit_counter**: الحد الأقصى لعدد الاعتمادات
  المعلقة قبل طلب تعليمة _rs`ApplyPendingBalance`_ لتحويل الرصيد المعلق إلى
  الرصيد المتاح.

- **expected_pending_balance_credit_counter**: قيمة
  `pending_balance_credit_counter` المقدمة من العميل من خلال بيانات التعليمة في
  آخر مرة تمت فيها معالجة تعليمة _rs`ApplyPendingBalance`_.

- **actual_pending_balance_credit_counter**: قيمة
  `pending_balance_credit_counter` على حساب الرمز في وقت معالجة آخر تعليمة
  _rs`ApplyPendingBalance`_.

<Callout type="info">
  **الرصيد المعلق مقابل الرصيد المتاح**

يتم فصل الأرصدة السرية إلى أرصدة معلقة ومتاحة لمنع هجمات DoS. بدون هذا الفصل،
يمكن للمهاجم إرسال الرموز بشكل متكرر إلى حساب رمز، مما يمنع مالك حساب الرمز من
القدرة على تحويل الرموز. لن يتمكن مالك حساب الرمز من تحويل الرموز لأن الرصيد
المشفر سيتغير بين وقت إرسال المعاملة ووقت معالجتها، مما يؤدي إلى فشل المعاملة.

تتم إضافة جميع مبالغ الإيداع والتحويل في البداية إلى الرصيد المعلق. يجب على
مالكي حسابات الرموز استخدام تعليمة _rs`ApplyPendingBalance`_ لتحويل الرصيد
...
يتم تقسيم الرصيد المعلق السري إلى `pending_balance_lo` و`pending_balance_hi` لأن
فك تشفير ElGamal يتطلب المزيد من الحسابات للأرقام الأكبر. يمكنك العثور على تطبيق
العمليات الحسابية للنص المشفر
...
1. يبحث العميل عن الأرصدة المعلقة والمتاحة الحالية، ويشفر المجموع، ويوفر
   `decryptable_available_balance` مشفرًا باستخدام مفتاح AES الخاص بمالك حساب
   الرمز المميز.

2. تتتبع عدادات الائتمان المعلقة المتوقعة والفعلية التغييرات في قيمة العداد بين
   وقت إنشاء تعليمة _rs`ApplyPendingBalance`_ ومعالجتها:
   - `expected_pending_balance_credit_counter`: قيمة
     `pending_balance_credit_counter` عندما ينشئ العميل تعليمة
     _rs`ApplyPendingBalance`_
   - `actual_pending_balance_credit_counter`: قيمة
     `pending_balance_credit_counter` على حساب الرمز المميز في وقت معالجة تعليمة
     _rs`ApplyPendingBalance`_

تطابق العدادات المتوقعة/الفعلية يشير إلى أن `decryptable_available_balance`
يطابق `available_balance`.
...
الأخيرة التي تطابق فرق العداد لحساب الرصيد الصحيح.
...
1. **إنشاء حساب الرمز المميز**: استدعِ تعليمة
   _rs`AssociatedTokenAccountInstruction:Create`_ الخاصة بـ Associated Token
   Program لإنشاء حساب الرمز المميز.

2. **إعادة تخصيص مساحة الحساب**: استدعِ تعليمة
   _rs`TokenInstruction::Reallocate`_ من Token Extension Program لإضافة مساحة
   لحالة _rs`ConfidentialTransferAccount`_.

3. **تكوين التحويلات السرية**: استدعِ تعليمة
   ...
   من Token Extension Program لتهيئة حالة _rs`ConfidentialTransferAccount`_.

<Callout type="info">

**يمكن لمالك حساب الرمز المميز فقط تكوين حساب الرمز المميز للتحويلات السرية**.

تتطلب تعليمة _rs`ConfigureAccount`_ إنشاء مفاتيح التشفير وبيانات الإثبات من جانب
العميل والتي يمكن إنشاؤها فقط بواسطة مالك حساب الرمز.

ينشئ _rs`PubkeyValidityProofData`_ إثباتًا يتحقق من صحة مفتاح ElGamal. لتفاصيل
التنفيذ، راجع:
