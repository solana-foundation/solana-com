...
description: تعلم كيفية إنشاء mint للرمز المميز مع ملحق التحويل السري.
---

## كيفية إنشاء mint مع امتداد النقل السري

يُتيح امتداد النقل السري إجراء عمليات نقل رمزية خاصة عن طريق إضافة حالة إضافية
إلى mint account. يشرح هذا القسم كيفية إنشاء mint رمزي مع تفعيل هذا الامتداد.

يوضح الرسم البياني التالي الخطوات المتبعة في إنشاء mint مع امتداد النقل السري:
...
- **authority**: الحساب الذي يملك صلاحية تغيير إعدادات النقل السري للـ mint
  والموافقة على الحسابات السرية الجديدة في حال كانت الموافقة التلقائية معطّلة.

- **auto_approve_new_accounts**: عند التعيين على true، يمكن للمستخدمين إنشاء
  token account مع تفعيل النقل السري بشكل افتراضي. وعند التعيين على false، يجب
  على الجهة المُخوَّلة الموافقة على كل token account جديد قبل استخدامه للنقل
  السري.
...
يتطلب إنشاء mint مع تفعيل النقل السري ثلاث تعليمات في معاملة واحدة:

1. **إنشاء mint account**: استدعاء تعليمة _rs`CreateAccount`_ الخاصة بـ System
   Program لإنشاء mint account.

2. **تهيئة امتداد النقل السري**: استدعاء تعليمة
   ...
   الخاصة بـ Token Extensions Program لتهيئة حالة _rs`ConfidentialTransferMint`_
   للـ mint.
...
يوضح الكود التالي كيفية إنشاء mint مع امتداد النقل السري (Confidential
Transfer).

تعتمد عمليات النقل السري على برنامج ZK ElGamal Proof، الذي يكون مُفعَّلاً على
الشبكة الرئيسية (mainnet) وشبكة المطورين (devnet). إن `solana-test-validator`
الافتراضي لا يُفعِّله، أما validator المحلي الذي يعمل بنسخة مفرَّعة من الشبكة
الرئيسية مثل [Surfpool](https://surfpool.run) فيدعمه. شغِّل المثال على إحداها
(يستخدم الكود شبكة devnet) مع حساب ممول، واستبدل عناوين mint والحسابات الوهمية
بعناوينك الخاصة.
