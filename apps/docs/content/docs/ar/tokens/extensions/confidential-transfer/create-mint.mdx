---
title: إنشاء mint للرمز المميز
description:
  تعرّف على كيفية إنشاء mint للرمز المميز مع تفعيل ميزة الأرصدة السرية.
---

## كيفية إنشاء mint مع الأرصدة السرية

تتيح الأرصدة السرية إجراء تحويلات رمز مميز خاصة عبر إضافة حالة إضافية إلى
مint account. يشرح هذا القسم كيفية إنشاء mint للرمز المميز مع تفعيل
الامتداد المطلوب.

يوضح المخطط التالي الخطوات المتبعة في إنشاء mint مع
امتداد الأرصدة السرية:
...
يحتوي _rs`ConfidentialTransferMint`_ على ثلاثة حقول تهيئة:

- **authority**: الحساب الذي يملك صلاحية تغيير إعدادات الأرصدة السرية
  للـ mint والموافقة على الحسابات السرية الجديدة في حال تعطيل الموافقة التلقائية.

- **auto_approve_new_accounts**: عند الضبط على true، يمكن للمستخدمين إنشاء
  token accounts مع تفعيل الأرصدة السرية بشكل افتراضي. أما عند الضبط على false، فيجب على
  authority الموافقة على كل token account جديد قبل أن يتمكن من استخدامه
  للتحويلات السرية.

- **auditor_elgamal_pubkey**: مدقق اختياري يمكنه فك تشفير مبالغ النقل في
  المعاملات السرية، مما يوفر آلية امتثال مع الحفاظ على الخصوصية من العامة.

### التعليمات المطلوبة

يتطلب إنشاء mint مع تفعيل الأرصدة السرية ثلاث تعليمات
في معاملة واحدة:

1. **إنشاء mint account**: استدعاء تعليمة _rs`CreateAccount`_ الخاصة بـ System
   Program لإنشاء mint account.

2. **تهيئة الأرصدة السرية**: استدعاء تعليمة
   ...
   الخاصة بـ Token Extension Program لضبط حالة _rs`ConfidentialTransferMint`_
   للـ mint.

3. **تهيئة Mint**: استدعاء تعليمة _rs`Instruction::InitializeMint`_ الخاصة بـ
   Token Extensions Program لتهيئة حالة mint القياسية.

بينما يمكنك كتابة هذه التعليمات يدويًا، توفر حزمة `spl_token_client` طريقة
`create_mint` التي تبني معاملة وترسلها بجميع التعليمات الثلاث في استدعاء دالة
واحد، كما هو موضح في المثال أدناه.

## مثال على الكود

يوضح الكود التالي كيفية إنشاء mint مع تفعيل الأرصدة السرية.

تعتمد الأرصدة السرية على برنامج ZK ElGamal Proof، المفعّل على mainnet وdevnet. لا يُفعّل `solana-test-validator` الافتراضي هذا البرنامج، غير أن validator المحلي الذي يعمل بنسخة من mainnet مثل [Surfpool](https://surfpool.run) يدعمه.
شغّل المثال على أحد هذه البيئات (يستخدم الكود devnet) مع حساب دافع ممول،
واستبدل عناوين mint والحسابات الوهمية بعناوينك الخاصة.
