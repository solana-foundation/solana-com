...
title: رسوم التحويل
description:
  أنشئ عملة رقمية مع رسوم التحويل، انقل الرسوم المحتجزة من حسابات الرموز إلى
  العملة، واسحبها إلى حساب رموز مستقبل الرسوم.
...
يطبق امتداد `TransferFeeConfig`\_ في برنامج امتدادات الرموز رسومًا على كل عملية
تحويل لتلك العملة.
...
- يمكن لـ `withdraw_withheld_authority` نقل الرسوم المجمعة إلى حساب رموز مستقبل
  الرسوم

<Callout type="info" title="امتدادات حساب الرموز">
  عندما يتم تهيئة حساب رموز لعملة مع `TransferFeeConfig`_، يتم تهيئة حساب الرموز
  مع `TransferFeeAmount`_. عندما يتم إنشاء حساب الرموز من خلال [برنامج الرموز
  المرتبطة](/docs/tokens/basics/create-token-account#how-to-create-an-associated-token-account)،
...
1. احسب حجم حساب العملة والإيجار المطلوب للعملة وامتداد `TransferFeeConfig`\_.
2. أنشئ حساب العملة باستخدام `CreateAccount`_، هيئ `TransferFeeConfig`_، وهيئ
   العملة باستخدام `InitializeMint`\_.
3. أنشئ حسابات رموز للعملة. يتم تفعيل `TransferFeeAmount`\_ تلقائيًا لحسابات
   الرموز.
4. استخدم `TransferCheckedWithFee`_ لتحويل الرموز والتحقق من الرسوم المتوقعة.
   يعمل `TransferChecked`_ أيضًا ويحتجز تلقائيًا الرسوم المحددة في حساب الرموز
   الوجهة.
5. استخدم `WithdrawWithheldTokensFromAccounts`\_ لنقل الرسوم المحتجزة مباشرة من
   حسابات الرموز إلى حساب رموز مستقبل الرسوم، موقعة من
   `withdraw_withheld_authority` الخاص بالعملة.
6. استخدم `HarvestWithheldTokensToMint`_ بدون صلاحيات لنقل الرسوم المحتجزة من
   حسابات الرموز إلى حساب العملة، ثم استخدم `WithdrawWithheldTokensFromMint`_
   لسحب الرسوم المحتجزة في حساب العملة إلى حساب رموز مستقبل الرسوم، موقعة من
   `withdraw_withheld_authority` الخاص بالعملة.
7. استخدم `SetTransferFee`\_ لتحديث إعدادات رسوم التحويل التالية، والتي تصبح
   سارية المفعول بعد فترتين زمنيتين.

<ScrollyCoding>

## !!steps احسب حجم الحساب

احسب حجم حساب mint للـ base mint بالإضافة إلى امتداد _rs`TransferFeeConfig`_.
هذا هو الحجم المستخدم في _rs`CreateAccount`_.
...
## !!steps هيئ TransferFeeConfig

هيئ امتداد _rs`TransferFeeConfig`_ على الـ mint.
...
هيئ الـ mint باستخدام _rs`InitializeMint`_ في نفس المعاملة.
...
## !!steps أنشئ حسابات الـ token واسك الـ tokens

أنشئ حسابات token للمصدر والمستلمين ومتلقي الرسوم، ثم اسك الـ tokens إلى token
account المصدر.
...
## !!steps التحويل باستخدام TransferCheckedWithFee

قم بتحويل الرموز باستخدام _rs`TransferCheckedWithFee`_ والتحقق من الرسوم
المتوقعة.
...
## !!steps التحويل باستخدام TransferChecked

قم بتحويل الرموز باستخدام _rs`TransferChecked`_. سيتم الاحتفاظ بالرسوم المُعدّة
...
## !!steps سحب الرسوم المحتجزة من حسابات الرموز

استخدم _rs`WithdrawWithheldTokensFromAccounts`_ لنقل الرسوم المحتجزة مباشرةً من
...
## !!steps حصاد الرسوم المحتجزة إلى حساب الإصدار

استخدم _rs`HarvestWithheldTokensToMint`_ لنقل الرسوم المحتجزة من حسابات الرموز
...
## !!steps سحب الرسوم المحتجزة من حساب الإصدار

استخدم _rs`WithdrawWithheldTokensFromMint`_ لنقل الرسوم المحصودة من حساب الإصدار
...
## !!steps تحديث رسوم التحويل التالية

استخدم _rs`SetTransferFee`_ لتحديث إعدادات رسوم التحويل التالية.
...
<Callout type="info" title="ترتيب التعليمات">
  يجب أن يأتي _rs`InitializeTransferFeeConfig`_ قبل _rs`InitializeMint`_. يجب
  تضمين _rs`CreateAccount`_، _rs`InitializeTransferFeeConfig`_، و
...
| العنصر                                                           | الوصف                                                                                                                                                                                         | المصدر                                                                                                                                      |
| ---------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`TransferFeeConfig`_                                          | امتداد سك يخزن سلطات الرسوم والمبلغ المحتجز المتراكم وتكوينات رسوم التحويل الأقدم والأحدث.                                                                                                    | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L128-L145)         |
| _rs`TransferFee`_                                                | بنية تكوين الرسوم المخزنة في _rs`TransferFeeConfig`_ التي تحدد الحقبة التي تدخل فيها الرسوم حيز التنفيذ والحد الأقصى للرسوم والرسوم بالنقاط الأساسية.                                         | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L32-L40)           |
| _rs`TransferFeeAmount`_                                          | امتداد حساب الرمز المميز الذي يخزن المبلغ المحتجز أثناء التحويلات لحساب الرمز المميز.                                                                                                         | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L169-L177)         |
| _rs`TransferFeeInstruction::InitializeTransferFeeConfig`_        | تعليمات تهيئة إعدادات رسوم التحويل قبل _rs`InitializeMint`_.                                                                                                                                  | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L24-L48)   |
| _rs`TransferFeeInstruction::TransferCheckedWithFee`_             | تعليمات التحويل التي تتحقق من أن الرسوم المقدمة من المتصل تطابق تكوين رسوم التحويل الحالي للسك.                                                                                               | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L49-L80)   |
| _rs`TransferFeeInstruction::WithdrawWithheldTokensFromAccounts`_ | تعليمات تنقل الرسوم المحتجزة مباشرة من حسابات الرمز المميز إلى حساب رمز مستقبل الرسوم، موقعة من قبل `withdraw_withheld_authority` الخاص بالسك.                                                | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L99-L122)  |
| _rs`TransferFeeInstruction::HarvestWithheldTokensToMint`_        | تعليمات بدون إذن تنقل الرسوم المحتجزة من حسابات الرمز المميز إلى مجمع الاحتجاز الخاص بحساب السك.                                                                                              | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L123-L134) |
| _rs`TransferFeeInstruction::WithdrawWithheldTokensFromMint`_     | تعليمات تسحب الرسوم المحتجزة لحساب السك إلى حساب رمز مستقبل الرسوم، موقعة من قبل `withdraw_withheld_authority` الخاص بالسك.                                                                   | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L81-L98)   |
| _rs`TransferFeeInstruction::SetTransferFee`_                     | تعليمات تحدث تكوين رسوم التحويل الأحدث، والذي يدخل حيز التنفيذ بعد حقبتين.                                                                                                                    | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L135-L154) |
| _rs`process_initialize_transfer_fee_config`_                     | منطق المعالج الذي يهيئ امتداد _rs`TransferFeeConfig`_ على سك غير مهيأ ويضبط تكوينات رسوم التحويل الأقدم والأحدث على الرسوم الأولية.                                                           | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L26-L58)       |
| _rs`process_withdraw_withheld_tokens_from_accounts`_             | منطق المعالج الذي يتحقق من صحة `withdraw_withheld_authority` وينقل الرسوم المحتجزة من حسابات الرمز المميز إلى حساب رمز مميز الوجهة.                                                           | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L208-L278)     |
| _rs`process_harvest_withheld_tokens_to_mint`_                    | منطق المعالج الذي يحصد الرسوم المحتجزة من حسابات الرمز المميز إلى مجمع الاحتجاز الخاص بحساب السك.                                                                                             | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L181-L206)     |
| _rs`process_withdraw_withheld_tokens_from_mint`_                 | منطق المعالج الذي يتحقق من صحة `withdraw_withheld_authority` وينقل المبلغ المحتجز لحساب السك إلى حساب رمز مميز الوجهة.                                                                        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L114-L158)     |
| _rs`process_set_transfer_fee`_                                   | منطق المعالج الذي يتحقق من صحة سلطة تكوين رسوم التحويل ويحدث تكوين رسوم التحويل الأحدث ليدخل حيز التنفيذ بعد حقبتين.                                                                          | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L61-L112)      |
| _rs`get_required_init_account_extensions`_                       | يُستخدم لإضافة امتدادات حساب الرمز المميز تلقائيًا عند تهيئة حسابات الرمز المميز بناءً على الامتدادات الممكّنة على السك. بالنسبة لسكات _rs`TransferFeeConfig`_، يضيف _rs`TransferFeeAmount`_. | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mod.rs#L1301-L1303)                    |
