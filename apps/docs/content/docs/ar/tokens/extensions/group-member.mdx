...
description:
  أنشئ عمليات سك المجموعات وأعضاء المجموعات التي تخزن بيانات المجموعة مباشرة على
  حساب السك باستخدام ملحقات GroupPointer وGroupMemberPointer وTokenGroup
  وTokenGroupMember.
...
المجموعة هي عملية سك تمثل مجموعة. عضو المجموعة هو عملية سك تنتمي إلى تلك
المجموعة.

استخدم هذه الملحقات عندما يجب أن تمثل إحدى عمليات السك المجموعة وأن تمثل عمليات
السك الأخرى العناصر التي تنتمي إليها.

يمكن لبرنامج Token Extensions تخزين بيانات مجموعة الرموز مباشرة على عملية السك
باستخدام أربعة ملحقات ذات صلة:

- _rs`GroupPointer`_ يوجه عملية السك إلى الحساب الذي يخزن بيانات المجموعة.
- _rs`TokenGroup`_ يخزن بيانات المجموعة نفسها، بما في ذلك سلطة التحديث والحجم
  الحالي والحجم الأقصى.
- _rs`GroupMemberPointer`_ يوجه عملية السك إلى الحساب الذي يخزن بيانات العضو.
- _rs`TokenGroupMember`_ يخزن عنوان مجموعة العضو ورقم العضو.

يمكن لـ _rs`GroupPointer`_ و _rs`GroupMemberPointer`_ الإشارة إلى أي حساب مملوك
لبرنامج يطبق
[واجهة مجموعة الرموز](https://github.com/solana-program/token-group/tree/main/interface).

يطبق برنامج Token Extensions أيضًا تلك الواجهة مباشرة من خلال ملحقات السك
_rs`TokenGroup`_ و _rs`TokenGroupMember`_.

## كيفية إنشاء مجموعات وأعضاء مخزنة على حساب السك

لإنشاء مجموعات وأعضاء مخزنة على حساب السك:

1. أنشئ حساب سك المجموعة وابدأ تشغيل _rs`GroupPointer`_.
2. ابدأ تشغيل سك المجموعة باستخدام _rs`InitializeMint`_.
3. ابدأ تشغيل _rs`TokenGroup`_ على نفس عملية السك.
4. أنشئ حساب سك العضو وابدأ تشغيل _rs`GroupMemberPointer`_.
5. ابدأ تشغيل سك العضو باستخدام _rs`InitializeMint`_.
6. ابدأ تشغيل _rs`TokenGroupMember`_ على نفس عملية السك بحيث يشير إلى سك
   المجموعة.

<ScrollyCoding>

## !!steps احسب حجم الصك الجماعي والإيجار
...
## !!steps إنشاء وتهيئة الصك الجماعي

أنشئ حساب الصك الجماعي، وهيئ _rs`GroupPointer`_، وهيئ الصك، وهيئ
_rs`TokenGroup`_ في معاملة واحدة.
...
## !!steps احسب حجم صك العضو والإيجار
...
## !!steps إنشاء وتهيئة صك العضو

أنشئ حساب صك العضو، وهيئ _rs`GroupMemberPointer`_، وهيئ الصك، وهيئ
_rs`TokenGroupMember`_ في معاملة واحدة.
...
<Callout type="info" title="المؤشرات وترتيب التعليمات">
يخزن _rs`GroupPointer`_ و _rs`GroupMemberPointer`_ عنوان الحساب
حيث تتواجد بيانات المجموعة أو العضو. يخزن _rs`TokenGroup`_ و
_rs`TokenGroupMember`_ بيانات المجموعة أو العضو الفعلية.

يجب أن يأتي _rs`GroupPointerInstruction::Initialize`_ و
_rs`GroupMemberPointerInstruction::Initialize`_ قبل _rs`InitializeMint`_. يجب أن
يأتي _rs`TokenGroupInstruction::InitializeGroup`_ و
_rs`TokenGroupInstruction::InitializeMember`_ بعد _rs`InitializeMint`_. لكل صك،
يجب تضمين _rs`CreateAccount`_ وتعليمة تهيئة المؤشر و _rs`InitializeMint`_ في نفس
المعاملة.
...
_rs`GroupPointer`_ و _rs`GroupMemberPointer`_ هما تعليمات مؤشر في برنامج Token
Extensions. يتبع _rs`TokenGroup`_ و _rs`TokenGroupMember`_ واجهة مجموعة الرموز،
...
| العنصر                                          | الوصف                                                                        | المصدر                                                                                                                                            |
| ----------------------------------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`GroupPointer`_                              | امتداد صك يخزن الصلاحية والعنوان للحساب الذي يخزن بيانات المجموعة.           | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_pointer/mod.rs#L12-L25)                |
| _rs`GroupMemberPointer`_                        | امتداد صك يخزن الصلاحية والعنوان للحساب الذي يخزن بيانات العضو.              | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_member_pointer/mod.rs#L12-L25)         |
| _rs`GroupPointerInstruction::Initialize`_       | يهيئ امتداد مؤشر المجموعة قبل _rs`InitializeMint`_.                          | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_pointer/instruction.rs#L22-L38)        |
| _rs`GroupPointerInstruction::Update`_           | يحدّث عنوان المجموعة المخزن بواسطة امتداد _rs`GroupPointer`_ للصك.           | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_pointer/instruction.rs#L39-L55)        |
| _rs`GroupMemberPointerInstruction::Initialize`_ | يهيئ امتداد مؤشر عضو المجموعة قبل _rs`InitializeMint`_.                      | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_member_pointer/instruction.rs#L22-L38) |
| _rs`GroupMemberPointerInstruction::Update`_     | يحدّث عنوان العضو المخزن بواسطة امتداد _rs`GroupMemberPointer`_ للصك.        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_member_pointer/instruction.rs#L39-L55) |
| _rs`process_initialize`_ (`GroupPointer`)       | يكتب صلاحية _rs`GroupPointer`_ الأولية وعنوان المجموعة على الصك.             | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/group_pointer/processor.rs#L25-L50)            |
| _rs`process_update`_ (`GroupPointer`)           | يتحقق من صلاحية مؤشر المجموعة، ثم يعيد كتابة عنوان المجموعة المخزن في الصك.  | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/group_pointer/processor.rs#L52-L78)            |
| _rs`process_initialize`_ (`GroupMemberPointer`) | يكتب صلاحية _rs`GroupMemberPointer`_ الأولية وعنوان العضو على الصك.          | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/group_member_pointer/processor.rs#L25-L50)     |
| _rs`process_update`_ (`GroupMemberPointer`)     | يتحقق من صلاحية مؤشر عضو المجموعة، ثم يعيد كتابة عنوان العضو المخزن في الصك. | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/group_member_pointer/processor.rs#L52-L78)     |

#### مجموعة الرموز وعضو مجموعة الرموز

| العنصر                                            | الوصف                                                                                                                         | المصدر                                                                                                                                 |
| ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`TokenGroup`_                                  | حالة واجهة مجموعة الرموز المخزنة على عملية السك، بما في ذلك سلطة التحديث والحجم الحالي والحجم الأقصى.                         | [المصدر](https://github.com/solana-program/token-group/blob/main/interface/src/state.rs#L13-L27)                                       |
| _rs`TokenGroupMember`_                            | حالة واجهة عضو مجموعة الرموز المخزنة على عملية السك، بما في ذلك عملية سك العضو وعنوان المجموعة ورقم العضو.                    | [المصدر](https://github.com/solana-program/token-group/blob/main/interface/src/state.rs#L64-L76)                                       |
| _rs`TokenGroupInstruction::InitializeGroup`_      | تعليمات واجهة مجموعة الرموز المدعومة من قبل Token Extensions Program لتهيئة مجموعة جديدة لعملية سك تم تهيئتها بالفعل.         | [المصدر](https://github.com/solana-program/token-group/blob/main/interface/src/instruction.rs#L52-L62)                                 |
| _rs`TokenGroupInstruction::UpdateGroupMaxSize`_   | تعليمات واجهة مجموعة الرموز المدعومة من قبل Token Extensions Program لتحديث الحد الأقصى لعدد الأعضاء المسموح بهم في المجموعة. | [المصدر](https://github.com/solana-program/token-group/blob/main/interface/src/instruction.rs#L64-L70)                                 |
| _rs`TokenGroupInstruction::UpdateGroupAuthority`_ | تعليمات واجهة مجموعة الرموز المدعومة من قبل Token Extensions Program لتدوير أو إلغاء سلطة التحديث الخاصة بالمجموعة.           | [المصدر](https://github.com/solana-program/token-group/blob/main/interface/src/instruction.rs#L72-L78)                                 |
| _rs`TokenGroupInstruction::InitializeMember`_     | تعليمات واجهة مجموعة الرموز المدعومة من قبل Token Extensions Program لتهيئة عضو جديد لمجموعة تم تهيئتها بالفعل.               | [المصدر](https://github.com/solana-program/token-group/blob/main/interface/src/instruction.rs#L80-L92)                                 |
| _rs`process_initialize_group`_                    | يتحقق من صحة عملية السك، ويتأكد من وجود _rs`GroupPointer`_، ويخصص حالة _rs`TokenGroup`_ على عملية سك المجموعة.                | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_group/processor.rs#L43-L90)   |
| _rs`process_update_group_max_size`_               | يتحقق من صحة سلطة التحديث الحالية، ثم يحدث الحجم الأقصى للمجموعة.                                                             | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_group/processor.rs#L95-L117)  |
| _rs`process_update_group_authority`_              | يتحقق من صحة سلطة التحديث الحالية، ثم يدور أو يلغي سلطة التحديث الخاصة بالمجموعة.                                             | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_group/processor.rs#L119-L141) |
| _rs`process_initialize_member`_                   | يتحقق من صحة عملية سك العضو وسلطة المجموعة، ويزيد حجم المجموعة، ويخصص حالة _rs`TokenGroupMember`_ على عملية السك.             | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_group/processor.rs#L143-L202) |
