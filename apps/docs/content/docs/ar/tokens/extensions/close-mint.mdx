...
title: إغلاق العملة
description:
  إنشاء حساب سك في برنامج Token Extensions مع ملحق MintCloseAuthority، ثم حرق
  الكمية المتبقية وإغلاق حساب السك.
...
يتيح ملحق _rs`MintCloseAuthority`_ في Token Extensions Program لصلاحية محددة
إغلاق حساب السك واستعادة الإيجار الخاص به بمجرد وصول إجمالي المعروض من السك إلى
الصفر.

بدون هذا الملحق، لا يمكن إغلاق حساب السك.

- يجب أن يكون إجمالي المعروض صفراً، لذا يجب حرق جميع الرموز في جميع حسابات
  الرموز قبل إغلاق حساب السك
...
1. احسب حجم حساب السك والإيجار المطلوب لحساب السك وملحق
   _rs`MintCloseAuthority`_.
2. أنشئ حساب السك باستخدام _rs`CreateAccount`_، وقم بتهيئة
   _rs`MintCloseAuthority`_، وقم بتهيئة السك باستخدام _rs`InitializeMint`_.
3. إذا كان إجمالي المعروض صفراً، يمكن إغلاق حساب السك باستخدام
   _rs`CloseAccount`_، موقعاً من قبل صلاحية الإغلاق.

<ScrollyCoding>

## !!steps احسب حجم الحساب

احسب حجم حساب السك للسك الأساسي بالإضافة إلى ملحق _rs`MintCloseAuthority`_. هذا
هو الحجم المستخدم في _rs`CreateAccount`_.
...
احسب الإيجار باستخدام الحجم المطلوب للسك بالإضافة إلى ملحق
_rs`MintCloseAuthority`_.
...
## !!steps تهيئة MintCloseAuthority

قم بتهيئة امتداد _rs`MintCloseAuthority`_ على حساب mint.
...
## !!steps تهيئة حساب mint

قم بتهيئة حساب mint باستخدام _rs`InitializeMint`_ في نفس المعاملة.
...
<Callout type="info" title="ترتيب التعليمات">
  يجب أن يأتي _rs`InitializeMintCloseAuthority`_ قبل _rs`InitializeMint`_. يجب
  تضمين _rs`CreateAccount`_ و _rs`InitializeMintCloseAuthority`_ و
...
| العنصر                                        | الوصف                                                                                             | المصدر                                                                                                                                |
| --------------------------------------------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`MintCloseAuthority`_                      | امتداد حساب mint يخزن الصلاحية المسموح لها بإغلاق حساب mint بمجرد وصول الإمداد إلى الصفر.         | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mint_close_authority.rs#L14-L20) |
| _rs`InitializeMintCloseAuthority`_            | تعليمة تقوم بتهيئة صلاحية إغلاق حساب mint قبل _rs`InitializeMint`_.                               | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1830-L1843)                |
| _rs`CloseAccount`_                            | تعليمة الرمز الأساسية التي تغلق حساب mint بعد وصول الإمداد الكلي إلى الصفر.                       | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1582-L1608)                |
| _rs`process_initialize_mint_close_authority`_ | منطق المعالج الذي يكتب امتداد _rs`MintCloseAuthority`_ على حساب mint غير مهيأ.                    | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1376-L1389)                    |
| _rs`process_close_account`_                   | معالج إغلاق الحساب المشترك الذي يفرض قاعدة الإمداد الصفري لحسابات mint التي لديها صلاحية الإغلاق. | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1288)                    |
