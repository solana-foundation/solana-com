...
title: المالك الثابت
description:
  قم بتهيئة حساب رمز Token Extension Program بملكية ثابتة بحيث لا يمكن إعادة
  تعيين مالكه أبدًا.
...
## ما هي الملكية الثابتة؟

امتداد حساب _rs`ImmutableOwner`_ في Token Extension Program يجعل مالك حساب الرمز
دائمًا.

بمجرد تهيئة حساب رمز بـ _rs`ImmutableOwner`_، لا يمكن لتعليمات
_rs`SetAuthority`_ اللاحقة تغيير صلاحية _rs`AccountOwner`_ الخاصة به.

<Callout type="info" title="حسابات الرموز المرتبطة">
  حسابات الرموز المرتبطة التي تم إنشاؤها من خلال Associated Token Program لـ
  Token Extension Program تقوم تلقائيًا بتهيئة _rs`ImmutableOwner`_ على حساب
  الرمز.
</Callout>

## كيفية تهيئة حساب بملكية ثابتة

لتهيئة حساب بملكية ثابتة:

1. أنشئ وهيئ رمزًا مميزًا.
2. أنشئ حساب رمز بمساحة كافية لـ _rs`ImmutableOwner`_.
3. هيئ _rs`ImmutableOwner`_ على حساب الرمز قبل _rs`InitializeAccount`_.
4. محاولة تغيير مالك الحساب لاحقًا بـ _rs`SetAuthority`_ تفشل مع
   _rs`TokenError::ImmutableOwner`_.

<ScrollyCoding>

## !!steps احسب حجم حساب الرمز

احسب حجم حساب الرمز لحساب الرمز الأساسي بالإضافة إلى امتداد
_rs`ImmutableOwner`_. هذا هو الحجم المستخدم في _rs`CreateAccount`_.
...
احسب rent باستخدام حجم حساب الرمز المطلوب لامتداد _rs`ImmutableOwner`_.
...
## !!steps أنشئ حساب الرمز

أنشئ حساب الرمز بالمساحة و lamport المحسوبة.
...
## !!steps تهيئة ImmutableOwner

تهيئة امتداد _rs`ImmutableOwner`_ على حساب الرمز المميز.
...
## !!steps تهيئة حساب الرمز المميز

تهيئة حساب الرمز المميز باستخدام _rs`InitializeAccount`_ في نفس المعاملة.
...
<Callout type="info" title="ترتيب التعليمات">
  يجب أن يأتي _rs`InitializeImmutableOwner`_ قبل _rs`InitializeAccount`_. يجب
  تضمين _rs`CreateAccount`_ و _rs`InitializeImmutableOwner`_ و
  _rs`InitializeAccount`_ في نفس المعاملة.
</Callout>
...
| العنصر                                   | الوصف                                                                                                | المصدر                                                                                                                           |
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| _rs`ImmutableOwner`_                     | امتداد الحساب الذي يحدد مالك حساب الرمز المميز على أنه غير قابل للتغيير.                             | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L13-L16) |
| _rs`InitializeImmutableOwner`_           | التعليمات التي تهيئ امتداد المالك غير القابل للتغيير على حساب رمز مميز غير مهيأ.                     | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1982-L1991)           |
| _rs`process_initialize_immutable_owner`_ | منطق المعالج الذي يخصص ويهيئ _rs`ImmutableOwner`_ على الحساب.                                        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1532-L1543)               |
| _rs`SetAuthority`_                       | تعليمات الرمز المميز الأساسية المستخدمة لتغيير الصلاحيات، بما في ذلك مالك حساب الرمز المميز.         | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1611-L1633)           |
| _rs`process_set_authority`_              | منطق المعالج الذي يرفض تغييرات المالك عندما يكون حساب الرمز المميز لديه _rs`ImmutableOwner`_ مفعلاً. | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L703-L900)                 |
