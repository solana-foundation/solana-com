...
description:
  إنشاء عملة برنامج Token Extension مع معدل فائدة على السلسلة، وتحديث هذا المعدل، وتحويل مبالغ الرموز إلى مبلغ واجهة المستخدم مع الفائدة.
...
يتيح امتداد عملة _rs`InterestBearingConfig`_ في برنامج Token Extension تخزين
معدل فائدة سنوي مباشرة على السلسلة.

الرموز التي تحمل فائدة **لا** تضيف المزيد من الرموز إلى حسابات الرموز بمرور
الوقت. يظل مبلغ الرموز المخزن في كل حساب رموز كما هو حتى تقوم تعليمة برنامج
الرموز بتغييره، مثل السك أو النقل أو الحرق.

مع مرور الوقت، يمكن أن يزداد مبلغ واجهة المستخدم المحسوب مع الفائدة حتى لو ظل
مبلغ الرموز وإجمالي المعروض من الرموز كما هو.
...
- يخزن
  ...
  `initialization_timestamp` و `pre_update_average_rate` و
  `last_update_timestamp` و `current_rate`.
- يحسب
  ...
  مبلغ واجهة المستخدم على خطوتين: يحسب أولاً الفائدة من التهيئة إلى
  `last_update_timestamp` باستخدام `pre_update_average_rate`، ثم يحسب الفائدة من
  `last_update_timestamp` إلى الطابع الزمني الحالي باستخدام `current_rate`.
- عند تشغيل
  ...
  ويحدث `pre_update_average_rate` إلى متوسط تاريخي جديد موزون بالوقت، ثم يسجل
  `last_update_timestamp` و `current_rate` جديدين.
- بعد تغيرات المعدل بمرور الوقت، يتم تمثيل التغييرات الأقدم بواسطة
  `pre_update_average_rate`، بينما يتم حساب الفترة من `last_update_timestamp`
  إلى الوقت الحالي باستخدام `current_rate`.

## كيفية إنشاء واستخدام الرموز التي تحمل فائدة

لإنشاء واستخدام الرموز التي تحمل فائدة:

1. احسب حجم حساب العملة والإيجار المطلوب للعملة وامتداد
   _rs`InterestBearingConfig`_.
2. أنشئ حساب العملة باستخدام _rs`CreateAccount`_، وهيئ
   _rs`InterestBearingConfig`_، وهيئ العملة باستخدام _rs`InitializeMint`_.
3. اسك الرموز كالمعتاد.
4. استخدم _rs`UpdateRate`_ لتغيير المعدل الحالي للعملة بمرور الوقت.
5. حول مبالغ الرموز إلى مبلغ واجهة مستخدم مع الفائدة باستخدام
   _rs`AmountToUiAmount`_ أو الطرق المساعدة التي تجلب حساب العملة ومتغير نظام
   الساعة، ثم احسب مبلغ واجهة المستخدم دون إرسال معاملة.

<Callout type="info" title="تحويل المبلغ بين واجهة المستخدم خارج السلسلة وعلى السلسلة">
  يقوم مسار المساعد خارج السلسلة بجلب حساب السك وsysvar الخاص بالساعة، ثم يحسب
  مبلغ واجهة المستخدم محليًا دون إرسال معاملة. يستخدم المسار على السلسلة
  _rs`AmountToUiAmount`_، والذي يعمل في برنامج الرموز ويُرجع مبلغ واجهة المستخدم
  في بيانات إرجاع المعاملة. يمكن إرسال _rs`AmountToUiAmount`_ في معاملة أو
  محاكاته.
</Callout>
...
احسب حجم حساب السك للسك الأساسي بالإضافة إلى امتداد _rs`InterestBearingConfig`_.
...
احسب الإيجار باستخدام الحجم المطلوب للسك بالإضافة إلى امتداد
_rs`InterestBearingConfig`_.
...
## !!steps تهيئة InterestBearingConfig

هيّئ امتداد _rs`InterestBearingConfig`_ على السك.
...
هيّئ السك باستخدام _rs`InitializeMint`_ في نفس المعاملة.
...
## !!steps إنشاء حساب رمز وسك الرموز
...
## !!steps احسب مبلغ واجهة المستخدم باستخدام المساعد خارج السلسلة

جلب حساب السك ومتغير النظام للساعة، ثم احسب مبلغ واجهة المستخدم دون إرسال
معاملة.
...
<Callout type="info" title="ترتيب التعليمات">
  يجب أن يأتي _rs`InterestBearingMintInstruction::Initialize`_ قبل
  _rs`InitializeMint`_. يجب تضمين _rs`CreateAccount`_،
  _rs`InterestBearingMintInstruction::Initialize`_، و _rs`InitializeMint`_ في
  نفس المعاملة.
</Callout>
...
| العنصر                                             | الوصف                                                                                                                   | المصدر                                                                                                                                              |
| ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InterestBearingConfig`_                      | امتداد السك الذي يخزن سلطة معدل الفائدة والطوابع الزمنية والمعدلات الحالية والتاريخية.                         | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L40-L153)         |
| _rs`InterestBearingMintInstruction::Initialize`_ | تعليمة تهيئ إعدادات السك الحامل للفائدة قبل _rs`InitializeMint`_.                                         | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/instruction.rs#L17-L90)  |
| _rs`InterestBearingMintInstruction::UpdateRate`_ | تعليمة تغير معدل الفائدة الحالي للسك.                                                                    | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/instruction.rs#L35-L113) |
| _rs`AmountToUiAmount`_                           | تعليمة ترجع سلسلة مبلغ واجهة المستخدم الحالي لمبلغ رمز باستخدام إعدادات السك النشطة.                      | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L520-L538)                                |
| _rs`InterestBearingConfig::amount_to_ui_amount`_ | مساعد يحول مبلغ رمز إلى مبلغ واجهة مستخدم مع فائدة لطابع زمني معين.                                           | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L85-L96)          |
| _rs`process_initialize`_                         | منطق المعالج الذي يهيئ _rs`InterestBearingConfig`_ ويسجل الطوابع الزمنية والمعدل الأولي.                     | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/interest_bearing_mint/processor.rs#L21-L43)      |
| _rs`process_update_rate`_                        | منطق المعالج الذي يحدث المعدل الحالي ويعيد حساب متوسط المعدل المرجح بالوقت ويسجل طابع التحديث الزمني. | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/interest_bearing_mint/processor.rs#L46-L77)      |
| _rs`process_amount_to_ui_amount`_                | منطق المعالج الذي يرجع سلسلة مبلغ واجهة المستخدم باستخدام إعدادات السك الحامل للفائدة النشطة.                     | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1548-L1569)                                  |

### تايب سكريبت

يستخدم مثال `Kit` أدناه التعليمات المُنشأة مباشرة. تم تضمين أمثلة قديمة باستخدام
`@solana/web3.js` و `@solana/spl-token` للمرجعية.
