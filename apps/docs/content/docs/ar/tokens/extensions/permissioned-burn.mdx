...
<Callout type="warn" title="التوفر">
  يأتي الحرق المصرَّح به مع Token-2022
  [`program@v11.0.0`](https://github.com/solana-program/token-2022/releases/tag/program%40v11.0.0).
  وهو متاح على devnet اليوم ومن المقرر تفعيله على الشبكة الرئيسية في يونيو 2026.
  يُنشر Token Extension Program بشكل منفصل على كل مجموعة، لذا تأكد من أن النشر
  على المجموعة المستهدفة يتضمن الإصدار v11.0.0 أو أحدث.
</Callout>

يتضمن validator الاختباري المحلي أيضاً إصداراً قديماً من Token-2022، لذا لتشغيل
الأمثلة الواردة في هذه الصفحة، قم بتحميل برنامج v11.0.0 من devnet إلى validator
الاختباري الخاص بك:
...
1. احسب حجم mint account و rent المطلوب لـ mint وامتداد
   _rs`PermissionedBurnConfig`_.
2. أنشئ mint account باستخدام _rs`CreateAccount`_، وابدأ تهيئة
   _rs`PermissionedBurnConfig`_، ثم هيّئ mint باستخدام _rs`InitializeMint`_.
3. أنشئ token account واسكّ الرموز.
4. احرق باستخدام _rs`PermissionedBurnInstruction::BurnChecked`_ موقّعاً من قِبل
   كل من مالك token account وسلطة الحرق.
5. يمكنك اختيارياً تعطيل الحرق المقيّد بالصلاحيات باستخدام _rs`SetAuthority`_
   عبر _rs`AuthorityType::PermissionedBurn`_ وصلاحية `None`، مما يُعيد تفعيل
   تعليمات الحرق القياسية. تُوضّح أمثلة الكود الكاملة أدناه هذه الخطوة.

<ScrollyCoding>

## !!steps حساب حجم الحساب

احسب حجم mint account للـ mint الأساسي مع امتداد _rs`PermissionedBurnConfig`_.
...
احسب rent باستخدام الحجم المطلوب لـ mint مع امتداد _rs`PermissionedBurnConfig`_.
...
## !!steps تهيئة PermissionedBurn

هيّئ امتداد _rs`PermissionedBurnConfig`_ على الـ mint مع سلطة الحرق.
...
## !!steps تهيئة الـ mint

هيّئ الـ mint باستخدام _rs`InitializeMint`_ في نفس المعاملة.
...
قِبل مالك token account وصلاحية الحرق معًا.
...
<Callout type="info" title="Instruction Order">
  يجب أن يأتي _rs`PermissionedBurnInstruction::Initialize`_ قبل
  _rs`InitializeMint`_. ويجب تضمين _rs`CreateAccount`_
...
| العنصر                                         | الوصف                                                                                                       | المصدر                                                                                                                                         |
| ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | امتداد للـ mint يخزّن الصلاحية المطلوبة للتوقيع المشترك على كل عملية حرق للـ mint.                          | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
| _rs`PermissionedBurnInstruction::Initialize`_  | تعليمة تُهيئ إعداد الحرق المقيَّد قبل _rs`InitializeMint`_.                                                 | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L41-L49) |
...
| _rs`PermissionedBurnInstruction::BurnChecked`_ | تعليمة حرق مع فحص للأرقام العشرية تستخدم نفس الحسابات المستخدمة في _rs`PermissionedBurnInstruction::Burn`_. | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | مُميِّز الصلاحية المستخدَم مع _rs`SetAuthority`_ لتدوير صلاحية الحرق أو تعطيلها على الـ mint.               | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | منطق المعالج الذي يُهيئ _rs`PermissionedBurnConfig`_ على mint غير مُهيَّأ ويخزّن صلاحية الحرق.              | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
...
| _rs`process_set_authority`_                    | منطق المعالج الذي يتحقق من صلاحية الحرق ويُدوِّرها عند استخدام _rs`AuthorityType::PermissionedBurn`_.       | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### TypeScript

يستخدم المثال `Kit` أدناه التعليمات المُنشأة مباشرةً. لا تتضمن حزمة
`@solana/spl-token` القديمة المنشورة دعم الحرق المقيّد بعد، لذا لم يُدرج أي مثال
للإصدار القديم.
