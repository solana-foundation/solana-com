...
description:
  أنشئ حساب إصدار يخزن البيانات الوصفية مباشرة على حساب الإصدار باستخدام امتدادات MetadataPointer وTokenMetadata، ثم قم بتحديث هذه البيانات الوصفية وإزالتها وقفلها من خلال واجهة token-metadata.
...
يستخدم مثال `Kit` أدناه التعليمات المُولَّدة مباشرةً. وقد أُدرجت الأمثلة القديمة التي تستخدم `@solana/web3.js` و`@solana/spl-token` و`@solana/spl-token-metadata` كمرجع. تُقيِّد الأمثلة القديمة قيمة `maxSupportedTransactionVersion` عند `0` لأن web3.js v1 لا يستطيع تفسير [معاملات v1](/docs/core/transactions/versioned-transactions#v1-message-format).
