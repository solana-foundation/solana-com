...
title: حماية CPI
description:
  قم بتفعيل امتداد الحساب CpiGuard لحظر تعليمات الرموز المستدعاة من خلال
  الاستدعاءات عبر البرامج.
...
امتداد حساب _rs`CpiGuard`_ في برنامج Token Extensions يمنع البرامج الأخرى على
السلسلة من استخدام حساب الرموز لتعليمات رموز معينة من خلال الاستدعاء عبر البرامج
(Cross Program Invocation).

عند تفعيل CPI guard، يرفض Token Extensions Program هذه التعليمات عندما يستدعيها
برنامج آخر على السلسلة من خلال CPI:

- _rs`Transfer`_، _rs`TransferChecked`_، و _rs`TransferCheckedWithFee`_ عندما
  يتم استخدام مالك حساب الرموز كسلطة للتعليمة
- _rs`Burn`_ و _rs`BurnChecked`_ عندما يتم استخدام مالك حساب الرموز كسلطة
  للتعليمة
- _rs`Approve`_ و _rs`ApproveChecked`_
- _rs`CloseAccount`_ عندما يتم إرسال lamport إلى مكان آخر غير مالك حساب الرموز
- _rs`SetAuthority`_ عند إضافة أو استبدال سلطة الإغلاق

عند تفعيل CPI guard، تظل هذه الحالات مسموحة:

- _rs`Transfer`_، _rs`TransferChecked`_، _rs`TransferCheckedWithFee`_،
  _rs`Burn`_، و _rs`BurnChecked`_ من خلال CPI عندما يوقع مفوض أو مفوض دائم بدلاً
  من مالك حساب الرموز
- _rs`CloseAccount`_ من خلال CPI عندما يتم إرسال lamport إلى مالك حساب الرموز
- _rs`SetAuthority`_ لإزالة سلطة إغلاق موجودة
- _rs`Revoke`_

عند استخدام مالك حساب الرموز كسلطة للتعليمة، لا تزال تعليمات Token Extensions
Program قابلة للمعالجة إذا تمت إضافة التعليمة مباشرة إلى المعاملة بدلاً من
استدعائها من خلال CPI. يظل تغيير مالك حساب الرموز باستخدام _rs`SetAuthority`_
محظوراً أثناء تفعيل CPI guard، حتى خارج CPI. لا يمكن أيضاً استدعاء
_rs`EnableCpiGuard`_ و _rs`DisableCpiGuard`_ من خلال CPI.

## كيفية تفعيل CPI Guard

لتفعيل CPI guard:

1. إنشاء وتهيئة mint.
2. حساب حجم حساب الرمز المميز و rent المطلوب لحساب رمز مميز مع _rs`CpiGuard`_.
3. إنشاء وتهيئة حساب رمز مميز للـ mint.
4. استدعاء _rs`EnableCpiGuard`_ على حساب الرمز المميز.
5. استدعاء _rs`DisableCpiGuard`_ على حساب الرمز المميز للسماح بتلك التعليمات من
   خلال CPI مرة أخرى.

<ScrollyCoding>

## !!steps إنشاء وتهيئة الـ mint

إنشاء وتهيئة الـ mint الذي سيستخدمه حساب الرمز المميز.
...
## !!steps حساب حجم حساب الرمز المميز

حساب حجم حساب الرمز المميز لحساب الرمز المميز الأساسي بالإضافة إلى امتداد
_rs`CpiGuard`_. هذا هو الحجم المستخدم في _rs`CreateAccount`_.
...
حساب rent باستخدام حجم حساب الرمز المميز المطلوب لامتداد _rs`CpiGuard`_.
...
## !!steps إنشاء وتهيئة حساب الرمز المميز

إنشاء حساب الرمز المميز بالمساحة و lamport المحسوبة، ثم تهيئته للـ mint.
...
## !!steps تفعيل حماية CPI

تفعيل _rs`CpiGuard`_ على حساب الرمز المميز.
...
## !!steps تعطيل حماية CPI

تعطيل _rs`CpiGuard`_ على حساب الرمز المميز.
...
| العنصر                             | الوصف                                                                                                                                                                                        | المصدر                                                                                                                                 |
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`CpiGuard`_                     | امتداد الحساب الذي يخزن ما إذا كان حارس CPI مفعلاً حالياً لحساب الرمز المميز.                                                                                                                | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/mod.rs#L20-L25)         |
| _rs`CpiGuardInstruction::Enable`_  | التعليمة التي تفعّل حارس CPI على حساب الرمز المميز.                                                                                                                                          | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/instruction.rs#L20-L38) |
| _rs`CpiGuardInstruction::Disable`_ | التعليمة التي تعطّل حارس CPI على حساب الرمز المميز.                                                                                                                                          | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/instruction.rs#L39-L53) |
| _rs`process_toggle_cpi_guard`_     | منطق المعالج المستخدم بواسطة _rs`Enable`_ و _rs`Disable`_ الذي يتحقق من ملكية الحساب، يرفض تغييرات CPI على الحارس نفسه، ويهيئ _rs`CpiGuard`_ إذا لم يكن موجوداً بالفعل قبل تعيين `lock_cpi`. | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/cpi_guard/processor.rs#L19-L52)     |

### Typescript

يستخدم مثال `Kit` أدناه التعليمات المولدة مباشرةً. تم تضمين الأمثلة القديمة
