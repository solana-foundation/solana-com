...
title: تحويل المذكرة
description:
  اطلب أن تتضمن التحويلات الواردة مذكرة من خلال تمكين امتداد حساب MemoTransfer
  على حساب رمز Token Extensions Program.
...
يتطلب امتداد حساب _rs`MemoTransfer`_ في Token Extensions Program تعليمات مذكرة
مباشرة قبل أي تحويل وارد إلى حساب الرمز هذا. التحويلات الصادرة من حساب الرمز هذا
لا تتطلب مذكرة.

يتم تكوين الامتداد على حساب الرمز الوجهة، وليس على السك. بمجرد التمكين، تفشل
التحويلات بدون مذكرة برسالة _rs`TokenError::NoMemo`_ حتى يقوم مالك حساب الرمز
بتعطيل المتطلب.

## كيفية إنشاء حساب رمز مع تمكين MemoTransfer

لإنشاء حساب رمز مع تمكين _rs`MemoTransfer`_:

1. إنشاء وتهيئة السك.
2. حساب حجم حساب الرمز والإيجار المطلوب لحساب رمز مع _rs`MemoTransfer`_.
3. إنشاء حساب الرمز وتمكين _rs`MemoTransfer`_ على هذا الحساب.
4. يجب أن تتضمن تعليمات _rs`Transfer`_ و _rs`TransferChecked`_ إلى حساب الرمز
   هذا تعليمات مذكرة مباشرة قبل تعليمات التحويل.

<ScrollyCoding>

## !!steps إنشاء وتهيئة السك

إنشاء وتهيئة السك الذي سيستخدمه حساب الرمز.
...
## !!steps حساب حجم حساب الرمز

احسب حجم حساب الرمز لحساب الرمز الأساسي بالإضافة إلى امتداد _rs`MemoTransfer`_.
...
احسب الإيجار باستخدام حجم حساب الرمز المطلوب لامتداد _rs`MemoTransfer`_.
...
أنشئ حساب الرمز المميز بالمساحة المحسوبة واللامبورتات، ثم قم بتهيئته للنعناع.
...
## !!steps تمكين MemoTransfer

قم بتمكين _rs`MemoTransfer`_ على حساب الرمز المميز.
...
| العنصر                                          | الوصف                                                                                 | المصدر                                                                                                                                     |
| ----------------------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`MemoTransfer`_                              | امتداد حساب يخزن ما إذا كانت التحويلات الواردة تتطلب مذكرة.                           | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/mod.rs#L17-L22)         |
| _rs`RequiredMemoTransfersInstruction::Enable`_  | تعليمات تمكّن التحويلات المطلوبة بمذكرة على حساب الرمز المميز.                        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L50-L71) |
| _rs`RequiredMemoTransfersInstruction::Disable`_ | تعليمات تعطّل التحويلات المطلوبة بمذكرة على حساب الرمز المميز.                        | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L74-L95) |
| _rs`process_toggle_required_memo_transfers`_    | منطق المعالج الذي يهيئ أو يعيد كتابة امتداد _rs`MemoTransfer`_ بعد التحقق من الملكية. | [المصدر](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/memo_transfer/processor.rs#L20-L48)     |

### Typescript

يستخدم مثال `Kit` أدناه التعليمات المُنشأة مباشرةً. تم تضمين أمثلة قديمة
باستخدام `@solana/web3.js` و `@solana/spl-token` و `@solana/spl-memo` للرجوع
إليها.
