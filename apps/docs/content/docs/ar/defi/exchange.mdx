...
- `{"maxSupportedTransactionVersion": 0}`
  ...
  يجب إضافة معامل `maxSupportedTransactionVersion` إلى طلبات `getBlock` و`getTransaction` لتجنب اضطراب عملية اكتشاف الإيداعات. أحدث إصدار للمعاملة هو `0` ويجب تحديده كقيمة أقصى لإصدار المعاملة المدعوم.
...
أولاً، احصل على blockhash حديث باستخدام نقطة النهاية
[`getFees`](/docs/rpc/deprecated/getfees) أو أمر CLI:
...
يمكنك التحقق من صلاحية blockhash معين عن طريق إرسال طلب
[`getFeeCalculatorForBlockhash`](/docs/rpc/deprecated/getfeecalculatorforblockhash)
مع الـ blockhash كمعامل. إذا كانت قيمة الاستجابة `null`، فهذا يعني أن الـ blockhash قد انتهت صلاحيته، وأن معاملة السحب التي تستخدمه لن تنجح أبداً.
