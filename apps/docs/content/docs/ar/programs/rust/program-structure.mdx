...
برامج سولانا المكتوبة بلغة Rust لديها متطلبات هيكلية بسيطة، مما يتيح المرونة في
...
للتبسيط، سيتم تنفيذ البرنامج في ملف `lib.rs` واحد، على الرغم من أنك قد ترغب
عملياً في تقسيم البرامج الأكبر إلى ملفات متعددة.
...
<Callout type="warn">
  قم بتحديث حقل `edition` في `Cargo.toml` إلى 2021. وإلا فقد تواجه خطأ عند بناء
  البرنامج.
</Callout>
...
الآن دعنا نضيف التبعيات اللازمة لبناء برنامج سولانا. نحتاج إلى `solana-program`
لـ SDK الأساسي و `borsh` للتسلسل.
...
إلغاء تسلسل بيانات `input` إلى معاملات دالة `process_instruction`.
...
الآن دعنا نحدد هيكل البيانات الذي سيتم تخزينه في حسابات العداد الخاصة بنا. هذه
هي البيانات التي سيتم تخزينها في حقل `data` للحساب.
...
enum `CounterInstruction` الخاصة بنا. تتولى طريقة `try_from_slice` في Borsh هذا
التحويل تلقائياً.
...
الآن دعنا نحدّث دالة `process_instruction` الرئيسية لتوجيه التعليمات إلى دوال
المعالجة المناسبة لها.

نمط التوجيه هذا شائع في برامج سولانا. يتم إلغاء تسلسل `instruction_data` إلى
متغير من enum يمثل التعليمة، ثم يتم استدعاء دالة المعالجة المناسبة. تتضمن كل
دالة معالجة التطبيق الخاص بتلك التعليمة.

أضف الكود التالي إلى `lib.rs` لتحديث دالة `process_instruction` وإضافة معالجات
...
الكتابة إلى الحساب وتهيئة البيانات.
...
- يقرأ حقل `data` للحساب الخاص بـ `counter_account`
- يحوله إلى بنية `CounterAccount`
- يزيد حقل `count` بمقدار 1
- يحول بنية `CounterAccount` مرة أخرى إلى حقل `data` للحساب
...
أولاً، لنضف التبعيات اللازمة للاختبار. سنستخدم `litesvm` للاختبار و
`solana-sdk`.
...
تضمن سمة `#[cfg(test)]` أن هذا الكود يتم تجميعه فقط عند تشغيل الاختبارات.
...
<Callout type="warn">
  ...
  الاختبار بتحميل البرنامج المترجم.
</Callout>
...
شغّل الاختبارات باستخدام الأمر التالي. يطبع العلم `--nocapture` مخرجات الاختبار.
...
المعاملات الناتجة. لاحظ أنه يجب تعيين الكتلة على Solana Explorer إلى "Custom RPC
URL"، والذي يكون افتراضياً `http://localhost:8899` الذي يعمل عليه
`solana-test-validator`.
