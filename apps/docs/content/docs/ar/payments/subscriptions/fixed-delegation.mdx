...
- المبالغ بالوحدات الأساسية. بالنسبة لرمز مميز بـ 6 منازل عشرية، `1_000_000`
  يعني `1` رمز مميز.
- قم بتشغيل `initSubscriptionAuthority` فقط عندما لا يكون حساب صلاحية الاشتراك
  موجوداً بالفعل.
- يوقّع المستخدم على معاملات الإعداد والإلغاء. يوقّع المفوض له على عمليات
  التحويل.
