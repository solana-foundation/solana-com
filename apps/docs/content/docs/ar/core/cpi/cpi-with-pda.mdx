---
title: استدعاءات البرامج المتقاطعة مع موقعي PDA
description:
  كيفية استخدام دالة invoke_signed لاستدعاءات البرامج المتقاطعة على سولانا عندما
  يحتاج البرنامج المستدعي للتوقيع كـ PDA، مع أمثلة Anchor CpiContext و Native
  Rust.
...
<Callout type="info" title="ملخص">
  استخدم _rs`invoke_signed`_ عندما يحتاج البرنامج المستدعي للتوقيع نيابة عن PDA
  يمتلكه. يقوم وقت التشغيل باشتقاق مفاتيح PDA العامة من بذور الموقع المقدمة
  ويضيفها إلى مجموعة الموقعين الصالحين قبل التحقق من الصلاحيات.
</Callout>
...
عندما يتطلب استدعاء برنامج متقاطع موقع PDA، استخدم
[`invoke_signed`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/program/src/program.rs#L51)
مع [بذور الموقع](mention:signer-seeds) المستخدمة لاشتقاق [PDA](/docs/core/pda).
للحصول على تفاصيل حول كيفية تحقق وقت التشغيل من توقيعات PDA، راجع
...
توضح الأمثلة أدناه كيفية إجراء استدعاء برنامج متقاطع مع موقعي PDA باستخدام
Anchor و Native Rust. يتضمن كل مثال تعليمة واحدة لتحويل SOL من PDA إلى حساب
مستلم، باستخدام استدعاء برنامج متقاطع موقع بواسطة PDA.

#### Anchor

توضح الأمثلة التالية طريقتين لتنفيذ استدعاءات البرامج المتقاطعة في برنامج
[Anchor](https://www.anchor-lang.com/docs). الأمثلة متكافئة وظيفياً، لكنها توضح
...
- **المثال 2**: يستخدم دالة _rs`system_instruction::transfer`_ من حزمة
  _rs`solana_program`_. (المثال 1 هو تجريد لهذا التنفيذ.)
- **المثال 3**: يبني تعليمة الاستدعاء المتقاطع يدوياً. هذا النهج مفيد عندما لا
  تتوفر حزمة للمساعدة في بناء التعليمة التي تريد استدعاءها.
...
يقوم المثال أدناه بإجراء CPI مع موقعين PDA من برنامج مكتوب بلغة Rust الأصلية.
يتضمن تعليمة واحدة تنقل SOL من حساب PDA إلى حساب آخر. يتم توقيع CPI بواسطة حساب
PDA. (يستخدم ملف الاختبار [LiteSVM](https://github.com/LiteSVM/litesvm) لاختبار
البرنامج.)
