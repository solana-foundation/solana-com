...
- [`data`](mention:instruction-data): مصفوفة بايتات تحتوي على [بيانات] إضافية
...
معرّف [`program_id`](/docs/references/terminology#program-id) الخاص بالتعليمة هو
...
![معاملة تحتوي على تعليمة واحدة. تحتوي التعليمة على هيكلين من `AccountMeta` في مصفوفة `accounts` الخاصة بها.](/assets/docs/core/transactions/accountmeta.svg)
