...
يوضح المثال أدناه كيفية استخدام Solana SDK لإنشاء keypair. يتضمن `Keypair`:
...
- [`lamports`](mention:lamports): عدد lamports في الحساب
- [`data`](mention:data): بيانات الحساب
- [`owner`](mention:owner): معرف البرنامج الذي يملك الحساب
...
يُشار إلى هذا الحقل عادةً باسم "بيانات الحساب". يُعتبر `data` في هذا الحقل
عشوائياً لأنه يمكن أن يحتوي على أي تسلسل من البايتات. يحدد كل برنامج بنية
...
2. إلغاء تسلسل حقل `data` الخاص بالحساب من البايتات الخام إلى بنية البيانات
   المناسبة، كما هو محدد بواسطة البرنامج الذي يملك الحساب.
...
البرنامج الوحيد الذي يمكنه تغيير `data` الخاص بالحساب أو خصم lamports، كما هو
محدد بواسطة تعليمات البرنامج.
...
- إذا كان `true`: الحساب هو program account
- إذا كان `false`: الحساب هو data account
...
في الرسم التخطيطي أدناه، يمكنك رؤية برنامج محمل يُستخدم لنشر حساب برنامج. يحتوي
حقل `data` الخاص بحساب البرنامج على كود البرنامج القابل للتنفيذ.
...
البرامج المنشورة باستخدام loader-v3 لا تحتوي على كود البرنامج في حقل `data`
الخاص بها. بدلاً من ذلك، يشير حقل `data` الخاص بها إلى **حساب بيانات برنامج**
منفصل، يحتوي على كود البرنامج. (انظر الرسم التخطيطي أدناه.)
...
يقوم المثال أدناه بجلب حساب Token Program. لاحظ أن حقل `executable` مضبوط على
`true`، مما يشير إلى أن الحساب هو برنامج.
...
إنشاء حساب بيانات جديد. غالباً ما تكون عملية إنشاء حساب حالة البرنامج مجردة،
ولكن من المفيد فهم العملية الأساسية.
...
لا يتم تعيين مالك جديد لجميع الحسابات بعد إنشائها بواسطة System Program. تُسمى
الحسابات المملوكة لـ System Program بحسابات النظام. جميع حسابات المحفظة هي
...
رؤية عنوان `owner` الخاص بالحساب وهو `11111111111111111111111111111111` (وهو
