...
- [`declare_id!`](mention:one) -makro määrittää ohjelmasi ketjuosoitteen. Solana
...
Funktion runko [asettaa `new_account` -kohteen `data` -kentän](mention:three)
annettuun `data` -argumenttiin ja tulostaa sitten viestin ohjelmalokeille.
...
3. Tallenna tulosteessa oleva transaktiotunniste (`TxHash`)
