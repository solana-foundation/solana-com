...
Aloita päivittämällä `lib.rs`-tiedosto tuodaksesi näkyviin kohteet
...
Päivitä seuraavaksi _rs`Update`_ -rakenne sisältämään uusi tili nimeltä
`vault_account`. Tämä ohjelman hallitsema tili vastaanottaa SOL:n käyttäjältä,
...
Tämä tili toimii ohjelman hallitsemana "holvitilinä", joka vastaanottaa SOL:n
...
- PDA:na sillä ei ole yksityistä avainta, joten vain ohjelma voi
  "allekirjoittaa" osoitteen puolesta CPI-kutsuissa
...
Lisää seuraavaksi CPI-logiikka `update`-ohjeeseen siirtääksesi 0,001 SOL:n
...
miten CPI suoritetaan ohjelman sisältä, mahdollistaen Solana-ohjelmien
koostettavuuden.
...
- CPI:ssä tarvittavat tilit (määritelty _rs`Transfer`_ -rakenteessa)
...
määritellään ohjelma, tilit ja instruction data tietylle kutsulle. Kun ohjelman
`update` -ohje vastaanottaa kutsun, se kutsuu sisäisesti System Program
-ohjelman siirto-ohjetta.
...
mahdollistaa holvissa olevien SOL:ien siirtämisen takaisin käyttäjälle, kun tämä
sulkee viestinsä tilin.
...
käyttäjän holvitilille poisto-ohjeen aikana, jotta kertynyt SOL voidaan siirtää
...
_rs`signer_seeds`_ määrittelee valinnaiset siemenet ja bump-siemenet, joita
...
_rs`Transfer`_ -rakenne määrittelee vaaditut tilit System Program -ohjelman
...
ohjeisiin. Tämä vaatii holvi-PDA:n johdannaisemisen ja sen sisällyttämisen
