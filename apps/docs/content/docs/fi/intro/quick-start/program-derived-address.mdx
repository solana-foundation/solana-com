...
description:
  Opi rakentamaan CRUD (Create, Read, Update, Delete) -Solana-ohjelma
  käyttämällä Program Derived Address -osoitteita (PDA) ja Anchor-kehystä. Tämä
  vaiheittainen opas näyttää, kuinka luoda, päivittää ja poistaa
  onchain-viestitilejä käyttämällä PDA:ita, toteuttaa tilin validointi ja
  kirjoittaa testit. Täydellinen kehittäjille, jotka haluavat ymmärtää PDA:iden
  käytön Solana-ohjelmissa.
...
["bump" seed](/docs/core/pda#canonical-bump) -arvon, jota käytetään ohjelman
johdannaisen osoitteen (PDA) johtamiseen. Tämän arvon tallentaminen säästää
laskentaa eliminoimalla tarpeen laskea se uudelleen myöhemmissä ohjeissa.
...
- `user` - _rs`Pubkey`_, joka tunnistaa käyttäjän, joka loi viestitilin.
- `message` - _rs`String`_, joka sisältää käyttäjän viestin.
- `bump` - _rs`u8`_, joka tallentaa ["bump" seed](/docs/core/pda#canonical-bump)
  -arvon ohjelman johdannaisen osoitteen (PDA) johtamiseen. Tämän arvon
  tallentaminen säästää laskentaa eliminoimalla tarpeen laskea se uudelleen
  myöhemmissä ohjeissa.

Kun ohjelma luo tilin, se serialisoi _rs`MessageAccount`_ -datan ja tallentaa
sen uuden tilin data-kenttään.

Myöhemmin, kun ohjelmaa luetaan tilistä, ohjelma deserialisoi tämän datan
takaisin _rs`MessageAccount`_ -datatyypiksi. Testausosiossa esitellään tilin
datan luomisen ja lukemisen prosessi.
...
Lisää nyt _rs`create`_ -ohje, joka luo ja alustaa _rs`MessageAccount`_ -tilin.

Aloita määrittämällä ohjeen vaatimat tilit päivittämällä _rs`Create`_ -rakenne
seuraavasti:
...
_rs`payer`_ -rajoite määrittää, mikä tili maksaa uuden tilin luomisesta.

### !space

_rs`space`_ -rajoite määrittää tavujen määrän, joka varataan uuden tilin
data-kentälle.
...
1. Tilityyppi (kuten _rs`Signer<'info>`_ tai _rs`Account<'info, T>`_), joka
   määrittää millaisen tilin ohjelma odottaa
...
Tässä esimerkissä _rs`Create`_ -rakenne määrittelee tilit, joita tarvitaan
_rs`create`_ -ohjeessa.

1. _rs`user: Signer<'info>`_
   - Edustaa käyttäjää, joka luo viestitilin
   - Tarvitsee muutettavan tilan (_rs`#[account(mut)]`_), koska se maksaa
     uudesta tilistä
...
- _rs`b"message"`_ - Kiinteä merkkijono ensimmäisenä siemenenä.
...
- Anchor-tilin erottelutunnus (tunniste): 8 tavua
...
2. Tilin datan alustaminen:
   ...
   - Asettaa `bump` -arvon, jota käytetään PDA:n johtamiseen, haettuna
     `ctx.bumps.message_account` -kentästä.
...
Seuraavaksi lisää `update` -ohje, joka muuttaa `MessageAccount` -tilin uudella
viestillä.

Kuten edellisessä vaiheessa, määrittele ensin tilit, joita `update` -ohje
vaatii.
...
_rs`Update`_ -rakenne määrittelee tilit, joita tarvitaan _rs`update`_ -ohjeessa.

1. _rs`user: Signer<'info>`_
   - Edustaa käyttäjää, joka päivittää viestitilin
   - Merkitty muokattavaksi (_rs`#[account(mut)]`_), koska se saattaa maksaa
     lisätilasta `message_account` -kentälle tarvittaessa
...
Seuraavaksi lisää _rs`delete`_ -ohje sulkemaan _rs`MessageAccount`_.
...
_rs`seeds`_ -rajoite määrittää seed-arvot, joita käytetään PDA:n johtamiseen.
...
1. _rs`user: Signer<'info>`_
   ...
   - Merkitty muokattavaksi (_rs`#[account(mut)]`_) vastaanottamaan lamportit
     suljetulta tililtä
...
Olet nyt saanut valmiiksi perus-CRUD-ohjelman. Julkaise ohjelma suorittamalla
`deploy` Playground-terminaalissa.
...
Lisää alla oleva koodi _ts`describe()`_ -kohdan sisään, mutta ennen _ts`it()`_
-osioita.
...
[`pg.program`](mention:one) mahdollistaa pääsyn metodeihin, joilla voi olla
vuorovaikutuksessa ohjelman kanssa, kun taas [`pg.wallet`](mention:two) antaa
pääsyn playground-lompakkoosi.
...
Ensin testitiedosto lähettää tapahtuman, joka kutsuu `create` -ohjetta välittäen
"Hello, World!" viestinä.
...
Ensin testitiedosto lähettää tapahtuman, joka kutsuu `update` -ohjetta välittäen
"Hello, Solana!" uutena viestinä.
...
_ts`fetchNullable()`_ -metodi hakee verkosta tilin datan, jota ei välttämättä
ole olemassa.
...
Ensin tämä koodi lähettää transaktion, joka kutsuu `delete` -ohjetta sulkeakseen
viestitilin.
...
tilin käyttäen sen osoitetta (`messagePda`) metodilla `fetchNullable`. Tämä
metodi palauttaa null, kun tiliä ei enää ole olemassa sulkemisen jälkeen.
...
käyttöönotettua ohjelmaa vastaan ja kirjaa linkit SolanaFM:ään transaktion
