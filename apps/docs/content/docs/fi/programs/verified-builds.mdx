...
- **Väärä allekirjoittaja**: Varmista, että allekirjoittajasi on ohjelman
  päivitysvaltuutettu suorittamalla `solana program show YourProgramId`
- **Ei on-chain PDA:ta**: Suorita `solana-verify verify-from-repo -um` ja
  **valitse KYLLÄ kehotettaessa**. Ilman PDA:n lataamista API ei voi hakea
  vahvistusmetatietojasi.
- **PDA-datan epäsuhta**: Päivitä PDA:si, jos olet ottanut ohjelmasi uudelleen
  käyttöön. PDA-datasi on vastattava käyttöönotettua ohjelmaasi.
- **Virheellinen commit-hajautus**: Luo PDA:si käyttämällä tarkkaa
  commit-hajautusta, jonka otit käyttöön
...
- **On-Chain-tallennus**: Tallenna vahvistuksen metatiedot (repo-URL,
  ...
  Verify -ohjelma (`verifycLy8mB96wd9wqq3WDXQwM4oU6r42Th37Db9fC`)
...
- **Luotettava ilman luottamusta**: API hylkää mielivaltaiset tiedot - se
  käyttää vain sitä, mitä päivitysoikeutesi on tallentanut lohkoketjuun
- **Yksinkertaisempaa**: Anna vain allekirjoittaja + program_id; API hakee
  kaiken muun PDA:stasi
- **Peukaloinnin kestävä**: PDA:si luo muuttumattoman tietueen, jonka kuka
  tahansa voi vahvistaa itsenäisesti
- **Omistajuuden todistus**: Allekirjoittajasi täytyy olla päivitysoikeus,
  todistaa kryptografisesti, että hallitset ohjelmaa
...
- **Ketjussa oleva PDA**: Vahvistusmetatietosi sijaitsevat ketjussa, eikä mikään
  keskusviranomainen hallitse niitä
...
- **Jatkuva uudelleentodentaminen**: API todentaa automaattisesti kaikki
  ohjelmat uudelleen 24 tunnin välein
...
Todenna mikä tahansa ohjelma itse lukemalla sen ketjussa oleva PDA ja
suorittamalla todentaminen paikallisesti:
