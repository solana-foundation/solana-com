...
Laskentayksikön käytön vertailuarvojen mittaamiseksi lisää `mollusk-svm-bencher`
...
Käyttääksesi Token Program, token2022-ohjelmaa (Token Extensions) ja Associated
Token Program testaukseen Molluskin kanssa, lisää `mollusk-svm-programs-token`
riippuvuudeksi tiedostoon `Cargo.toml`:
...
3. [Käsittele ja validoi](mention:process) - Käsittele käsky Molluskilla ja
   validoi tulos
...
Kaikkia kenttiä voidaan muokata kourallisen apumetodien avulla, mutta käyttäjät
voivat myös suoraan käyttää ja muokata niitä, jos haluavat enemmän hallintaa.
...
| [`process_and_validate_instruction`](#single-instruction-with-checks)          | Käsittele käsky ja suorita sarja tarkistuksia tulokselle, panikoiden jos jokin tarkistus epäonnistuu.                     |
| [`process_instruction_chain`](#multiple-instructions)                          | Käsittele useita käskyjä ja palauta tulos.                                                                                |
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | Käsittele useita käskyjä ja suorita sarja tarkistuksia jokaiselle tulokselle, panikoiden jos jokin tarkistus epäonnistuu. |

[`InstructionResult`](https://docs.rs/mollusk-svm-result/0.5.0/mollusk_svm_result/types/struct.InstructionResult.html)
sisältää käsitellyn käskyn yksityiskohdat.
...
havainnollistamista varten. Käytännössä käytät Molluskia tyypillisesti
...
käskyn validointitarkistuksilla. Tämä metodi panikoituu, jos jokin tarkistus
epäonnistuu.
...
identtisiä `Mollusk` -metodien kanssa.
...
tilejä sisäisesti `account_store` -rakenteen kautta. Tämä poistaa tarpeen
`accounts` -parametrille ohjeiden käsittelyssä.
...
Seuraava esimerkki käsittelee kaksi erillistä SOL-siirto-ohjetta pysyvällä
tilitilalla ohjeiden välillä `account_store` -rakenteen kautta.
...
Käytä `warp_to_slot` -metodia päivittääksesi sysvar clock -arvon simuloidaksesi
...
Seuraava esimerkki näyttää, kuinka Molluskin sysvar-arvoa muokataan suoraan
...
seuraa ohjelman ohjeiden laskentayksikköjen käyttöä. Tulokset kirjoitetaan
markdown-tiedostoon.
...
Benchmark-tulokset kirjoitetaan määritettyyn `out_dir` -hakemistoon
...
-cratea lisätäksesi Token Program -ohjelman, token2022-ohjelman (Token
Extensions) ja Associated Token Program -ohjelman Molluskiin testausta varten.
...
<Callout type="info">
  Alla oleva esimerkki määrittelee testitilit manuaalisesti havainnollistamista
  varten. `mollusk-svm-programs-token` sisältää myös apufunktioita mint- ja
  token-tilien luomiseen.
</Callout>
