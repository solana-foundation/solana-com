...
<Callout type="warn">
  ...
  saatat kohdata virheen ohjelman käännösvaiheessa.
</Callout>
...
joka vaaditaan Solana-ohjelman käännökseen.
...
Käännä seuraavaksi ohjelma käyttämällä `cargo build-sbf` -komentoa.
...
2. Keypair-tiedosto (esim. `hello_world-keypair.json`): Tämän keypairin julkista
   avainta käytetään ohjelman tunnuksena, kun ohjelma otetaan käyttöön.
...
Seuraavaksi testaa ohjelma käyttämällä `litesvm` -cratea. Lisää seuraavat
...
erillisessä terminaalissa julkaistaksesi ohjelman paikalliseen validaattoriin.
...
Rust-asiakasohjelma, joka rahoittaa uuden keypair-avaimen transaktiokuluja
varten ja kutsuu sitten hello world -ohjelmaa.
...
"Hello, Solana!" "Hello, world!" -viestin sijaan.
...
Ota ohjelma uudelleen käyttöön käyttämällä samaa `solana program deploy`
-komentoa.
