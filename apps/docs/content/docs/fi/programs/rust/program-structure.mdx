...
Rustilla kirjoitetut Solana-ohjelmat vaativat vain vähän rakenteellisia
vaatimuksia, mikä mahdollistaa joustavuuden koodin organisoinnissa. Ainoa
...
- `entrypoint.rs`: Määrittää sisääntulokohdän, joka reitittää saapuvat käskyt.
- `state.rs`: Määrittää ohjelman tilan (tilin datan).
...
2. `IncrementCounter`: Kasvattaa olemassa olevalle tilille tallennettua arvoa.
...
<Callout type="warn">
  ...
  kohdata virheen ohjelmaa rakentaessasi.
</Callout>
...
Solana-ohjelman `entrypoint`-funktiolla on seuraava funktiosignatuuri.
Kehittäjät voivat vapaasti luoda oman toteutuksensa `entrypoint`-funktiosta.
...
`CounterInstruction`-enum-variantiksi. Borshin `try_from_slice`-metodi
...
Päivitetään nyt pää-`process_instruction`-funktio ohjaamaan instruktiot niiden
asianmukaisiin käsittelijäfunktioihin.

Tämä reitityskaava on yleinen Solana-ohjelmissa. `instruction_data`
deserialisoidaan instruktion esittävän enumin variantiksi, jonka jälkeen
kutsutaan asianmukaista käsittelijäfunktiota. Jokainen käsittelijäfunktio
sisältää kyseisen instruktion toteutuksen.

Lisää seuraava koodi tiedostoon `lib.rs` päivittäen
`process_instruction`-funktiota ja lisäten käsittelijät `InitializeCounter`- ja
`IncrementCounter`-instruktioille:
...
Ohjelmamme tekee CPI-kutsun System Programin `create_account` ohjeeseen. Uusi
tili luodaan ohjelmamme omistukseen, mikä antaa ohjelmalle mahdollisuuden
...
`cargo build-sbf` komento ohjelman rakentamiseksi. Tämä luo `counter_program.so`
tiedoston `target/deploy` hakemistoon.
...
on oltava asetettu tilaan "Custom RPC URL", joka oletuksena on
`http://localhost:8899`, jolla `solana-test-validator` on käynnissä.
