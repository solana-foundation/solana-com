---
title: TypeScript
description: Asenna ja käytä @solana/keychain -kirjastoa TypeScript-sovelluksissa
---
...
Fordefillä on kaksi allekirjoitustilaa, jotka valitaan sen mukaan, välitätkö `chain`-parametrin. API-pyynnöt todennetaan ECDSA P-256 -pyyntöallekirjoituksella — anna allekirjoitusavain `privateKeyPem`-muodossa tai säilytä se KMS/HSM-järjestelmässä välittämällä `requestSigner`.

**Natiivi Solana-tila** (suositellaan ketjussa tapahtuvaan käyttöön): aseta `chain`, ja Fordefi allekirjoittaa sekä lähettää transaktion itse. Koska Fordefi saattaa korvata lohkohashin tai maksut ennen allekirjoitusta, allekirjoittaja on Kitin `TransactionSendingSigner` — käytä sitä `signAndSendTransactionMessageWithSigners`-funktion kanssa, ei yllä näytetyn osittaisen allekirjoittajan kulun kanssa.
...
**Musta laatikko -tila**: jätä `chain` pois. Fordefi allekirjoittaa raakatavut eikä lähetä transaktiota; kokoa ja lähetä transaktio itse. Käytä tätä Fordefin musta laatikko -holvin kanssa.
