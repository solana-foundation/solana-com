...
| `aws_kms`    | AWS KMS Ed25519:llä            |
| `gcp_kms`    | GCP Cloud KMS Ed25519:llä      |
...
| `all`        | Ota kaikki taustat käyttöön             |
...
Fordefi MPC -säilytyksen allekirjoitusta varten. API-pyynnöt todennetaan ECDSA P-256 -pyyntöallekirjoituksella, joka parsitaan `private_key_pem`-arvosta. Jos haluat säilyttää kyseisen avaimen KMS/HSM-järjestelmässä, toteuta `FordefiRequestSigner` ja käytä `Signer::from_fordefi_with_signer`-funktiota.

Fordefi tukee kahta allekirjoitustilaa. Aseta `chain` **natiivi Solana -tilaa** varten, jossa Fordefi voi korvata lohkohashin tai maksut ja lähettää transaktion itse. Jätä `chain` arvoon `None` **musta laatikko -tilaa** varten, jossa Fordefi allekirjoittaa raakatavut ja sinä lähetät transaktion itse.
