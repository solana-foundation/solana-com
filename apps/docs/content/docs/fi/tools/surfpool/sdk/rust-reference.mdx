---
title: Rust API -viite
description:
  Täydellinen julkinen API surfpool-sdk-crateille — Surfnet, rakentaja,
  huijauskoodit, rakentajat, virheet ja uudelleenviennit.
---

`surfpool-sdk`-crate tarjoaa pienen rajapinnan, jonka keskipisteenä on
`Surfnet`-rakenne. Tällä sivulla luetellaan kaikki julkiset elementit;
käyttöohjeita varten katso oppaat alkaen
[Yleiskatsauksesta](/docs/tools/surfpool/sdk/overview).

## Craten juuri-viennit

| Elementti             | Tyyppi          | Huomiot                                                                         |
| --------------------- | --------------- | ------------------------------------------------------------------------------- |
| `Surfnet`             | rakenne         | Käynnissä oleva Surfnet-instanssi — RPC/WS-päätepisteet, maksaja, huijauskoodit |
| `SurfnetBuilder`      | rakenne         | Sujuva rakentaja, jota käyttää `Surfnet::builder()`                             |
| `Cheatcodes<'_>`      | rakenne         | Lainattu huijauskoodi-apuri, jonka palauttaa `Surfnet::cheatcodes()`            |
| `SurfnetError`        | enum            | Kaikki SDK:n palauttamat virhevariantit                                         |
| `SurfnetResult<T>`    | tyyppialias     | `Result<T, SurfnetError>`                                                       |
| `Keypair`             | uudelleenvienti | `solana_keypair::Keypair`                                                       |
| `Pubkey`              | uudelleenvienti | `solana_pubkey::Pubkey`                                                         |
| `Signer`              | uudelleenvienti | `solana_signer::Signer`                                                         |
| `RpcClient`           | uudelleenvienti | `solana_rpc_client::rpc_client::RpcClient`                                      |
| `BlockProductionMode` | uudelleenvienti | `surfpool_types::BlockProductionMode`                                           |
| `SimnetCommand`       | uudelleenvienti | `surfpool_types::SimnetCommand`                                                 |
| `SimnetEvent`         | uudelleenvienti | `surfpool_types::SimnetEvent`                                                   |
| `SvmFeatureConfig`    | uudelleenvienti | `surfpool_types::SvmFeatureConfig`                                              |

## Surfnet

```rust
pub struct Surfnet { /* private */ }
```

### Konstruktorit

```rust
impl Surfnet {
    pub async fn start() -> SurfnetResult<Self>;
    pub fn builder() -> SurfnetBuilder;
}
```

### Hakijat

```rust
impl Surfnet {
    pub fn rpc_url(&self) -> &str;
    pub fn ws_url(&self) -> &str;
    pub fn payer(&self) -> &Keypair;
    pub fn instance_id(&self) -> &str;
    pub fn rpc_client(&self) -> RpcClient;
    pub fn cheatcodes(&self) -> Cheatcodes<'_>;
    pub fn events(&self) -> &crossbeam_channel::Receiver<SimnetEvent>;
    pub fn send_command(&self, cmd: SimnetCommand) -> SurfnetResult<()>;
}
```

### Elinkaari

```rust
impl Surfnet {
    pub fn stop(&mut self) -> SurfnetResult<()>;
}

impl Drop for Surfnet { /* calls stop() if not already stopped */ }
```

`stop` odottaa enintään viisi sekuntia, kunnes ajoympäristö vapauttaa porttinsa.
Virhe `Drop`:n aikana on hiljainen; virhe eksplisiittisen `stop`:n aikana
palauttaa `SurfnetError::Runtime`:n.

## SurfnetBuilder

```rust
pub struct SurfnetBuilder { /* private */ }
```

Jokainen asettaja palauttaa `Self`:n, joten kutsut voidaan ketjuttaa. Päätekutsu
on `start().await`.

```rust
impl SurfnetBuilder {
    pub fn offline(self, offline: bool) -> Self;
    pub fn remote_rpc_url(self, url: impl Into<String>) -> Self;
    pub fn block_production_mode(self, mode: BlockProductionMode) -> Self;
    pub fn slot_time_ms(self, ms: u64) -> Self;
    pub fn airdrop_addresses(self, addrs: Vec<Pubkey>) -> Self;
    pub fn airdrop_sol(self, lamports: u64) -> Self;
    pub fn skip_blockhash_check(self, skip: bool) -> Self;
    pub fn payer(self, payer: Keypair) -> Self;
    pub fn enable_feature(self, feature_id: Pubkey) -> Self;
    pub fn disable_feature(self, feature_id: Pubkey) -> Self;
    pub fn feature_config(self, config: SvmFeatureConfig) -> Self;
    pub async fn start(self) -> SurfnetResult<Surfnet>;
}
```

## Huijauskoodit

```rust
pub struct Cheatcodes<'a> { /* private */ }
```

### SOL-rahoitus

```rust
impl<'a> Cheatcodes<'a> {
    pub fn fund_sol(&self, address: &Pubkey, lamports: u64) -> SurfnetResult<()>;
    pub fn fund_sol_many(&self, accounts: &[(&Pubkey, u64)]) -> SurfnetResult<()>;
}
```

### Token-rahoitus

```rust
impl<'a> Cheatcodes<'a> {
    pub fn fund_token(
        &self,
        owner: &Pubkey,
        mint: &Pubkey,
        amount: u64,
        token_program: Option<&Pubkey>,
    ) -> SurfnetResult<()>;

    pub fn fund_token_many(
        &self,
        owners: &[&Pubkey],
        mint: &Pubkey,
        amount: u64,
        token_program: Option<&Pubkey>,
    ) -> SurfnetResult<()>;

    pub fn set_token_balance(
        &self,
        owner: &Pubkey,
        mint: &Pubkey,
        amount: u64,
        token_program: Option<&Pubkey>,
    ) -> SurfnetResult<()>;

    pub fn get_ata(
        &self,
        owner: &Pubkey,
        mint: &Pubkey,
        token_program: Option<&Pubkey>,
    ) -> Pubkey;
}
```

### Tilin muokkaus

```rust
impl<'a> Cheatcodes<'a> {
    pub fn set_account(
        &self,
        address: &Pubkey,
        lamports: u64,
        data: &[u8],
        owner: &Pubkey,
    ) -> SurfnetResult<()>;

    pub fn execute<B: CheatcodeBuilder>(&self, builder: B) -> SurfnetResult<()>;
}
```

### Aikamatkustus

```rust
impl<'a> Cheatcodes<'a> {
    pub fn time_travel_to_slot(&self, slot: u64) -> SurfnetResult<EpochInfo>;
    pub fn time_travel_to_epoch(&self, epoch: u64) -> SurfnetResult<EpochInfo>;
    pub fn time_travel_to_timestamp(&self, timestamp_ms: u64) -> SurfnetResult<EpochInfo>;
}
```

### Ohjelman käyttöönotto

```rust
impl<'a> Cheatcodes<'a> {
    pub fn deploy_program(&self, program_name: &str) -> SurfnetResult<Pubkey>;
    pub fn deploy(&self, builder: DeployProgram) -> SurfnetResult<Pubkey>;
}
```

## CheatcodeBuilder-rajapinta

```rust
pub trait CheatcodeBuilder {
    const METHOD: &'static str;
    fn build(self) -> serde_json::Value;
}
```

Toteuta tämä mukautetuille JSON-RPC-huijauskoodeille, joita tyypitetty rakentaja
ei vielä tue.

## Rakentajat

Kaikki rakentajat sijaitsevat `surfpool_sdk::cheatcodes::builders`.

### SetAccount

```rust
pub struct SetAccount { /* private */ }

impl SetAccount {
    pub fn new(address: Pubkey) -> Self;
    pub fn lamports(self, lamports: u64) -> Self;
    pub fn data(self, data: Vec<u8>) -> Self;
    pub fn owner(self, owner: Pubkey) -> Self;
    pub fn rent_epoch(self, epoch: u64) -> Self;
    pub fn executable(self, executable: bool) -> Self;
}
```

### SetTokenAccount

```rust
pub struct SetTokenAccount { /* private */ }

impl SetTokenAccount {
    pub fn new(owner: Pubkey, mint: Pubkey) -> Self;
    pub fn amount(self, amount: u64) -> Self;
    pub fn delegate(self, delegate: Pubkey) -> Self;
    pub fn clear_delegate(self) -> Self;
    pub fn state(self, state: impl Into<String>) -> Self;
    pub fn delegated_amount(self, amount: u64) -> Self;
    pub fn close_authority(self, authority: Pubkey) -> Self;
    pub fn clear_close_authority(self) -> Self;
    pub fn token_program(self, program: Pubkey) -> Self;
}
```

### ResetAccount

```rust
pub struct ResetAccount { /* private */ }

impl ResetAccount {
    pub fn new(address: Pubkey) -> Self;
    pub fn include_owned_accounts(self, include: bool) -> Self;
}
```

### StreamAccount

```rust
pub struct StreamAccount { /* private */ }

impl StreamAccount {
    pub fn new(address: Pubkey) -> Self;
    pub fn include_owned_accounts(self, include: bool) -> Self;
}
```

### DeployProgram

```rust
pub struct DeployProgram { /* private */ }

impl DeployProgram {
    pub fn new(program_id: Pubkey) -> Self;
    pub fn from_keypair_path(path: impl AsRef<Path>) -> SurfnetResult<Self>;
    pub fn so_path(self, path: impl Into<PathBuf>) -> Self;
    pub fn so_bytes(self, bytes: Vec<u8>) -> Self;
    pub fn idl_path(self, path: impl Into<PathBuf>) -> Self;
}
```

## SurfnetError

```rust
#[derive(Debug)]
pub enum SurfnetError {
    PortAllocation(String),
    Startup(String),
    Runtime(String),
    Cheatcode(String),
    Aborted(String),
}
```

| Variantti        | Syy                                                                   |
| ---------------- | --------------------------------------------------------------------- |
| `PortAllocation` | RPC- tai WebSocket-portille ei voitu sitoutua vapaaseen porttiin.     |
| `Startup`        | Suoritusympäristön alustaminen epäonnistui.                           |
| `Runtime`        | Suoritusympäristön säie kaatui tai päättyi odottamatta.               |
| `Cheatcode`      | Huijauskoodin JSON-RPC-kutsu palautti virheen.                        |
| `Aborted`        | Suoritusympäristö sammutettiin tai peruutettiin käynnistyksen aikana. |

Paketti toteuttaa `std::error::Error` ja `Display` kohteelle `SurfnetError`,
joten se toimii `?` ja `anyhow` kanssa suoraan käyttövalmiina.
