...
> _Huom: Kun `fallback_if_unavailable` on true, järjestelmä sallii transaktioiden
> jatkumisen, jos Redis on tilapäisesti poissa käytöstä, estäen palveluhäiriöt.
> Asettamalla `max_transactions` arvoon 0 sallitaan rajaton määrä transaktioita._
...
| `get_payer_signer`          | Palauttaa seuraavan maksajan ja maksun osoitteen        | ✅       | boolean |
...
[Solana-dokumentaatiosta](https://solana.com/developers/cookbook/transactions/durable-nonces).
...
| ---------------------------- | ------------------------------------------------ |
| `confidential_transfer_mint` | Luottamuksellisten saldojen konfiguraatio mintille |
...
| Laajennuksen nimi                  | Kuvaus                                 |
| ------------------------------- | ------------------------------------------- |
| `confidential_transfer_account` | Luottamuksellisten saldojen tila tilille |
