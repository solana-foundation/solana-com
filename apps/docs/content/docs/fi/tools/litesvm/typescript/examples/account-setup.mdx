...
description: Määritä testitilit ja -tila LiteSVM:llä
---

# Esimerkki tilin määrityksestä

Havainnollistaa testitilin tilan määrittämistä suoraan ilman transaktioita.
...
## Program Data Account -määritys

Määritä ohjelman omistama tilitietue:
...
<Callout type="info">
  `setAccount` on nopeampi kuin transaktioiden suorittaminen tilien
  joukkомääritystä varten.
</Callout>

## Tärkeimmät kohdat

1. **Suora tila**: `setAccount` asettaa tilin tilan ilman transaktioita
2. **Vuokravapautus**: Käytä `minimumBalanceForRentExemption` laskemaan
   tarvittavat lamport
3. **Tilirakenne**: Anna kaikki vaaditut kentät: address, data, executable,
   lamports, programAddress, space
4. **Vahvistus**: Käytä `getAccount` tai `client.rpc.getAccountInfo`
   vahvistamiseen
