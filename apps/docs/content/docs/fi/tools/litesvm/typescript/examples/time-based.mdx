...
description: Testaa aikariippuvaista logiikkaa LiteSVM-kellomanipulaatiolla
---

# Aikapohjainen testauesimerkki

Havainnollistaa `warpToSlot` -käyttöä aikariippuvaisen ohjelmalogiiikan
testaamisessa.

## Kellon perusmanipulaatio
...
Voit lukea epoch-aikataulun ymmärtääksesi slot/epoch-suhteen klusterissa:
...
<Callout type="warn">
  ...
  päivitä `epoch`- tai `unixTimestamp`-arvoja. Käytä `setClock` -funktiota
  kellokenttien täydelliseen hallintaan.
</Callout>

## Manuaalinen kellomanipulaatio

Kun ohjelmasi lukee `epoch`- tai `unix_timestamp`-arvoja Clock sysvar
-muuttujasta, käytä `setClock` asettaaksesi nämä kentät suoraan:
...
Tämä vastaa Rustin `set_sysvar(&clock)`-mallia. Käytä `warpToSlot`
yksinkertaiseen slot-etenemiseen ja `setClock` silloin, kun tarvitset epoch- tai
aikaleimahallintaa.

## Käyttötapaukset

Aikamanipulaatio on hyödyllistä seuraavien testaamiseen:

| Skenaario                  | Lähestymistapa                            |
| -------------------------- | ----------------------------------------- |
| Tokenien vesting           | Hyppää vesting-kynnyksen/välietappien ohi |
| Huutokauppojen päättymiset | Hyppää huutokaupan loppu-slotin ohi       |
| Staking-palkkiot           | Käytä `setClock` epochin asettamiseen     |
| Aikalukitut nostot         | Hyppää avaus-slotin ohi                   |
| Nopeusrajoitus             | Hyppää sallittujen välien välillä         |
| Vanhenevat tilaukset       | Hyppää tilauksen vanhenemisajan ohi       |

<Callout type="info">
  Muista, että `warpToSlot` muuttaa vain slotia. Unix-aikaleima ei välttämättä
  päivity suhteessa siihen riippuen SVM-toteutuksesta.
</Callout>

## Keskeiset kohdat

1. **Ota Sysvars käyttöön**: Kutsu `withSysvars()` ennen kellometodien käyttöä
2. **Hyppää eteenpäin**: Käytä `warpToSlot(slot)` slotin edistämiseen
3. **Täysi hallinta**: Käytä `setClock` kun haluat asettaa `epoch`-,
   `unixTimestamp`- tai muita kellokenttiä
4. **Lue kello**: Käytä `getClock()` lukeaksesi nykyisen slotin, epochin ja
   aikaleiman
