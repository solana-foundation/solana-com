...
description: Testaa aikasidonnaista logiikkaa LiteSVM-kellon manipuloinnilla
---

Havainnollistaa `warpToSlot`-funktion käyttöä aikasidonnaisen ohjelmaluogiikan testaamiseen.
...
## Aikalukitun logiikan testaaminen

Esimerkkimalli aikalukitun toiminnallisuuden testaamiseen:
...
Voit lukea epoch-aikataulun ymmärtääksesi slot/epoch-suhteen
klusterissa:
...
<Callout type="warn">
  `warpToSlot` edistää kellon `slot`-kenttää, mutta **ei** automaattisesti
  päivitä `epoch`- tai `unixTimestamp`-kenttiä. Käytä `setClock`-funktiota kellon
  kenttien täydelliseen hallintaan.
</Callout>

## Manuaalinen kellon manipulointi

Kun ohjelmasi lukee `epoch`- tai `unix_timestamp`-arvon Clock-sysvarista, käytä
`setClock`-funktiota asettaaksesi nämä kentät suoraan:
...
Aikamanipulointi on hyödyllistä seuraavien testaamiseen:

| Skenaario                | Lähestymistapa                           |
| ----------------------- | ---------------------------------- |
| Tokenin vesting           | Siirry vesting-rajan/välietappien ohi |
| Huutokaupan päättymiset         | Siirry huutokaupan päättymisslot:n ohi         |
| Staking-palkkiot         | Käytä `setClock`-funktiota epoch:n asettamiseen        |
| Aikalukitut nostot | Siirry avausslot:n ohi              |
| Nopeusrajoitus           | Siirry sallittujen välien välillä     |
| Vanhenevat tilaukset         | Siirry tilauksen vanhentumisen ohi         |

<Callout type="info">
  Muista, että `warpToSlot` muuttaa vain slot:n. Unix-aikaleima ei välttämättä
  päivity suhteessa SVM-toteutuksesta riippuen.
</Callout>

## Keskeiset kohdat

1. **Ota Sysvarit käyttöön**: Kutsu `withSysvars()` ennen kellometodien käyttöä
2. **Siirry eteenpäin**: Käytä `warpToSlot(slot)`-funktiota slot:n edistämiseen
3. **Täysi hallinta**: Käytä `setClock`-funktiota, kun haluat asettaa `epoch`-,
   `unixTimestamp`- tai muita kellokenttiä
4. **Lue kello**: Käytä `getClock()`-funktiota nykyisen slot:n, epoch:n ja aikaleiman lukemiseen
5. **Testausmalli**: Testaa ennen ja jälkeen aikarajojen
