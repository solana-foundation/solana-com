---
title: Aika & Sysvarit
description: Kellon manipulointi ja sysvar-käyttö TypeScriptissä
---

# Aika & Sysvarit

Menetelmät ajan manipulointiin ja järjestelmämuuttujien käyttöön.

<Callout type="warn">
  Ota sysvarit ensin käyttöön `client.svm.withSysvars()` ennen
  sysvar-menetelmien käyttöä.
</Callout>
...
Siirry kellossa tiettyyn slot-arvoon.
...
Korvaa kellon sysvar. Hae kello, muuta sen kenttiä ja kirjoita se sitten
takaisin. Käytä tätä, kun tarvitset tarkkaa hallintaa useiden kellokenttien yli
— esimerkiksi epoch-arvon tai unix-aikaleiman asettamiseen, jota `warpToSlot` ei
päivitä.
...
<Callout type="info">
  `warpToSlot` päivittää vain `slot`. Käytä `setClock`, kun ohjelmasi lukee
  `epoch`- tai `unix_timestamp`-arvoja Clock sysvarista.
</Callout>
...
| --------------------- | -------- | -------------------------- |
| `slot`                | `bigint` | Nykyinen slot-numero       |
| `epoch`               | `bigint` | Nykyinen epoch-numero      |
| `unixTimestamp`       | `bigint` | Unix-aikaleima sekunneissa |
| `epochStartTimestamp` | `bigint` | Nykyisen epoch-jakson alku |
| `leaderScheduleEpoch` | `bigint` | Johtaja-aikataulu epoch    |
...
Laske rent-vapautukseen tarvittava lamport-minimimäärä.
...
Hae viimeisimmät slot-tiivisteet.
...
Hae tai aseta viimeisen uudelleenkäynnistyksen slot.
...
Käytä `warpToSlot` aikariippuvaisen ohjelman toiminnan testaamiseen:
