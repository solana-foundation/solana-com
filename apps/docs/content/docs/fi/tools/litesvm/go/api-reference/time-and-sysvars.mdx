---
title: Aika & Sysvarit
description: Kellon manipulointi ja sysvar-käyttö Go-kielessä
---

Menetelmiä ajan manipulointiin ja järjestelmämuuttujien käyttämiseen. Jokaisella sysvar-getterillä on vastaava setter; rakenteet ovat tavallisia Go-peilejä Solana-lähteestä.
...
Siirtää sisäisen kellon tiettyyn slot-arvoon.
...
Ylikirjoittaa Clock-sysvarin. Lue se, muuta kenttiä ja kirjoita takaisin, kun tarvitset
hallintaa `Epoch`- tai `UnixTimestamp`-arvoihin (joita `WarpToSlot` ei päivitä).
...
<Callout type="info">
  `WarpToSlot` päivittää vain `Slot`-arvon. Käytä `SetClock`-funktiota, kun ohjelmasi lukee
  `Epoch`- tai `UnixTimestamp`-arvoa Clock-sysvarista.
</Callout>
...
`total_points` on `u128`-tyyppinen Solana-puolella; `litesvm-go` paljastaa molemmat puoliskot
explisiittisesti, jotta Go-kutsujat eivät tarvitse u128-kirjastoa. Arvoille, jotka mahtuvat 64
bittiin, `TotalPointsHi` on `0` ja `TotalPointsLo` on arvo.
...
`SlotHistory` on noin 128 kt:n bitvektori; `litesvm-go` tarjoaa sen kahvana eikä
siivuna.
...
| Metodi                                           | Kuvaus                                |
| ------------------------------------------------ | ------------------------------------------ |
| `litesvm.NewSlotHistory() (*SlotHistory, error)` | Tyhjä kahva                               |
| `sh.Add(slot uint64)`                            | Tallenna slot                              |
| `sh.Check(slot uint64) SlotHistoryCheck`         | `Future` / `TooOld` / `Found` / `NotFound` |
| `sh.Oldest() uint64`                             | Vanhin seurattu slot                        |
| `sh.Newest() uint64`                             | Uusin seurattu slot                        |
| `sh.NextSlot() uint64`                           | Seuraava tallennettava slot                        |
| `sh.SetNextSlot(slot uint64) error`              | Ylikirjoita seuraavan slot-merkinnän arvo              |
| `sh.Close()`                                     | Vapauta kahva                         |
| `svm.SlotHistory() (*SlotHistory, error)`        | Lue uuteen kahvaan                   |
| `svm.SetSlotHistory(*SlotHistory) error`         | Asenna SVM:lle                         |
...
## Aikariippuvaisen logiikan testaaminen

Käytä `WarpToSlot`-funktiota tavalliseen slot-pohjaiseen etenemiseen ja `SetClock`-funktiota täyteen hallintaan:
