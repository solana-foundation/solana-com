...
description: Lataa ja testaa mukautettuja Solana-ohjelmia litesvm-go:lla
---

Mukautetun Solana-ohjelman testaaminen Go:sta tarkoittaa sen käännetyn `.so`-tiedoston lataamista
SVM:ään, tarvittavien tilien määrittämistä ja instruktioiden lähettämistä sitä vastaan.
`litesvm-go` hoitaa kaiken tämän prosessin sisällä – validator ei tarvita.
...
## Täydellinen ohjelman testiasetuksen rakenne
...
Testeihin, jotka tarvitsevat vakio-ohjelmia (SPL Token, Memo, ...):
...
1. **Konfiguraatio** – ota käyttöön sysvars, builtins ja precompiles tarpeen mukaan.
2. **Ohjelman lataaminen** – `AddProgramFromFile` (tai `AddProgram` muistissa oleville
   tavuille).
3. **Tilien määrittäminen** – alusta data-tilit etukäteen `SetAccount`-funktiolla.
4. **Lähettäminen** – rakenna instruktio `solana-go`:lla, muodosta rakenne ja lähetä
   `SendLegacyTransaction`-funktiolla.
5. **Varmennus** – lue tilit takaisin `GetAccount`-funktiolla ja tarkista lokit,
   laskentayksiköt ja tilan lopputulos.

Kun tämä runko toimii, laajenna lisäämällä enemmän instruktioita, enemmän
tilejä ja apufunktio toistuvaa koodia varten.
