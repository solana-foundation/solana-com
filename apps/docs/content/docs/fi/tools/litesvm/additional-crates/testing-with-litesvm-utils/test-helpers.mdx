---
title: Testiapuvälineet
description:
  Apuohjelmat tilien luomiseen, token-operaatioihin, PDA-johdannaisuuteen ja
  slot-hallintaan
---

## Rahoitettujen tilien luominen

Yleisin testaustarpeen on tilien luominen SOL:lla. `TestHelpers` -ominaisuus
tekee tästä helppoa.
...
Luo token mint -sopimuksia ja tilejä ilman tavanomaista mallipohjakoodia.
...
<Callout type="info">
  **Käytä ATA:ita useimmissa tapauksissa.** Associated Token Accounts ovat
  deterministisiä ja noudattavat Solana-käytäntöjä. Käytä tavallisia token
  account -tilejä vain, kun tarvitset useita tilejä samalle
  mint/omistaja-parille.
</Callout>
...
Johda Program Derived Addresses helposti testeihisi.

### Perus-PDA-johdannaisuus
...
<Callout type="info">
  Käytä `derive_pda`, kun tarvitset bump seed -arvon (esim. CPI-kutsuihin).
  Käytä `get_pda`, kun tarvitset vain osoitteen.
</Callout>

## Slot-hallinta

Hallitse ajan etenemistä aikariippuvaisen logiikan testaamiseksi.

### Nykyisen slot-arvon hakeminen
...
### slot-arvojen eteenpäin siirtäminen
...
Tässä on kattava esimerkki, jossa käytetään useita testausapuvälineitä yhdessä:
