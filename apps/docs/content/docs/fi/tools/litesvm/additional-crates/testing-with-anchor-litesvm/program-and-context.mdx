---
title: Ohjelma ja konteksti
description:
  Testiympäristöjen määrittäminen ja rakennusohjeiden luominen AnchorContext- ja
  Program-objekteilla
---

## Asiakastyyppien luominen declare_program!-makrolla

Anchor 1.0 esittelee `declare_program!`-makron, joka lukee ohjelmasi IDL:n
käännösaikana ja luo tyyppiturvalliset `client::accounts::*`- ja
`client::args::*`-moduulit. Tämä on suositeltu tapa rakentaa ohjeita testeissä.
...
<Callout type="info">
  Suorita ensin `anchor build`, jotta `target/idl/my_program.json` luodaan.
  `declare_program!` lukee tämän tiedoston käännösaikana. Makron nimen täytyy
  vastata ohjelmasi crate-nimeä (snake_case).
</Callout>

## Testiympäristön määrittäminen

`AnchorLiteSVM`-rakentaja tarjoaa sujuvan API:n testiympäristösi määrittämiseen.
...
### Yksittäisen ohjelman pikamääritys
...
<Callout type="info">
  Jos et määritä maksajaa, `AnchorLiteSVM` luo ja rahoittaa sellaisen
  automaattisesti puolestasi.
</Callout>

## AnchorContext

`AnchorContext` on päärajapinta testiympäristösi kanssa vuorovaikuttamiseen.
...
### Tilin olemassaolon tarkistaminen
...
## Ohjeiden rakentaminen Program-ohjelmalla

`Program`-rakenne tarjoaa sujuvan API:n ohjeiden rakentamiseen, joka mukailee
anchor-clientiä. Käytä `declare_program!`:n luomia tyyppejä tyyppiturvallisesti.

### Perusohjeiden rakentaminen
...
<Callout type="info">
  `.accounts()`-metodi hyväksyy minkä tahansa tyypin, joka toteuttaa
  `ToAccountMetas`-rajapinnan, jonka kaikki Anchor-tilirakenteet toteuttavat
  automaattisesti. Vastaavasti `.args()` hyväksyy minkä tahansa tyypin, joka
  toteuttaa `InstructionData`-rajapinnan.
</Callout>
...
### Useiden ohjeiden suorittaminen
...
### send_and_confirm_transaction-funktion käyttäminen
...
Tässä on kattava esimerkki, joka käyttää escrow-mallia `declare_program!`:n
kanssa:
