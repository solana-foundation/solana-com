---
title: Tapahtumat
description: Menetelmät tapahtumien käsittelyyn
---
...
Suorita tapahtuma ja muuta tilaa.
...
Palauttaa `SimulatedTransactionInfo`, joka sisältää:

- `meta`: Tapahtuman metatiedot (lokit, laskentayksiköt, allekirjoitus jne.)
- `post_accounts`: Tilitilat simuloinnin jälkeen
...
## Milloin käyttää simulointia

Käytä `simulate_transaction()` seuraaviin tarkoituksiin:

- Testaa tapahtuman kelpoisuus muuttamatta tilaa
- Tarkista laskentayksikköjen kulutus
- Varmista tapahtumalokien sisältö ilman sitoutumista
- Testaa virheenkäsittelypolkuja
...
## Tapahtuman metatiedot

`send_transaction()` palauttaa `TransactionMetadata` suoraan, kun taas
`simulate_transaction()` palauttaa `SimulatedTransactionInfo`, joka sisältää
...
Hae transaktio historiasta. Edellyttää, että `with_transaction_history()` on
asetettu.
...
Hae nykyinen lohkohajautusarvo transaktioita varten.
...
Pakota nykyinen lohkohajautusarvo vanhenemaan.
