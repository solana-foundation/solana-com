...
[Solana Pay](https://solanapay.com) -toiminnallisuuden maksukokemusten
rakentamiseen, yhteensopivana
[`@solana/kit`](https://github.com/anza-xyz/kit) -kirjaston kanssa. Se käsittelee URL-
koodauksen/jäsentämisen, QR-koodin luomisen mukautetulla tyylillä sekä
tapahtumien rakentamisen sekä SOL- että SPL-tokenisiirroille.
...
- **`recipient`** (`Address`, pakollinen) - Kauppiaan lompakko-osoite
  (base58-koodattu julkinen avain), johon maksu vastaanotetaan. Voi olla merkkijono tai
  `Address`-tyyppi kohteesta `@solana/kit`.
...
Rakentaa täydellisen Solana-tapahtumasanoman maksunsiirtoa varten. Tämä
funktio tunnistaa automaattisesti, luodaanko SOL- vai SPL-tokenisiirto
`splToken`-parametrin perusteella, ja rakentaa kaikki tarvittavat ohjeet.
Funktio asettaa lohkohashin elinajan tapahtumalle käyttäen viimeisintä
lohkohashia RPC-asiakkaalta ja palauttaa täydellisen, allekirjoittamattoman tapahtuman valmiina
allekirjoitettavaksi ja lähetettäväksi RPC:lle (tyyppi
`TransactionMessageWithBlockhashLifetime`, yhteensopiva Solana Kitin kanssa).

#### Parametrit

- **`rpc`** (`Rpc<SolanaRpcApi>`) - Solana RPC -asiakas kohteesta `@solana/kit`. Luo
  komennolla `createSolanaRpc(rpcUrl)`.
