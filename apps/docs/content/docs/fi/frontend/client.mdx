...
<Callout type="info">
  Käytätkö headless-tilaa — API-reitissä, workerissa tai skriptissä ilman
  selainlompakkoa? Allekirjoita taustajärjestelmän avainhallintopalvelulla
  raakamuotoisen keypair:in sijaan. Katso [Keychain](/docs/tools/keychain) ja
  [Allekirjoittaminen
...
- **Headless-tilakone**: Aja asiakasta API-reiteissä, skripteissä tai
  workereissa hyödyntääksesi samaa lompakko- ja RPC-orkestrointilogiikkaa, joka
  toimii käyttöliittymässäsi.
- **Reaaliaikaiset koontinäytöt**: Yhdistä tarkkailijat (saldot, tilit,
  allekirjoitukset) haluamaasi käyttöliittymäkirjastoon; asiakas hallitsee
  websocket-jakelun ja välimuistin invalidoinnin.
- **Mukautetut storeet**: Injektoi oma Zustand-storea hydratoidaksesi
  IndexedDB:stä/localStoragesta, peilataksesi tilaa palvelinistuntoihin tai
  koordinoidaksesi selainvälilehtien välillä.
- **Silta React-hookeihin**: Välitä konfiguroitu asiakasinstanssi
  `@solana/react-hooks`:lle, kun haluat ergonomiset hookit saman ajonaikaisen
  ympäristön päälle.
- **Testattavuus**: Yksittäinen asiakasrajapinta voidaan mockita
  yksikkötesteissä, mikä helpottaa RPC-vastausten tai lompakkoistuintojen
