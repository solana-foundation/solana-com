...
description:
  Määritä Solana-lompakkointegraatio Next.js:ssä käyttämällä @solana/kit- ja
  @solana/react-paketteja SOL-siirtojen lähettämiseen.
---

Määritä minimaalinen Solana-lompakkointegraatio Next.js:ssä (App Router)
käyttämällä `@solana/kit`, kit-lisäosia ja `@solana/react`. Luot
lompakkoyhdistämisen pudotusvalikon ja SOL-siirtokomponentin.
...
- [Kit-lisäosat](https://github.com/anza-xyz/kit-plugins)
...
Kun sinulta kysytään, hyväksy kaikki oletusasetukset (aloituspaketti sisältää
Tailwindin, jota tämä opas käyttää yksinkertaisiin apuluokkiin).

<Callout type="info" title="Kaksi muutosta scaffold-rakenteeseen">
Oletusarvoinen `create-next-app`-tuloste tarvitsee kaksi pientä muutosta tätä opetusohjelmaa varten:

- Aseta `tsconfig.json`:ssä arvo `"target": "ES2020"` (tai uudempi).
  Siirtomäärät käyttävät `bigint`-literaaleja, kuten `1_000_000_000n`, jotka
  edellyttävät ES2020:tä.
- Poista `app/globals.css`:stä generoitu
  `@media (prefers-color-scheme: dark)`-lohko ja `body`-taustavärisääntö. Tämä
  käyttöliittymä on vain vaalea; tummat oletusarvot muuten ohittavat
  `bg-white`-apuluokan ja tekevät sivusta lukukelvottoman tummassa tilassa.
...
Tämä opetusohjelma on suunnattu Kit 7+:lle, `@solana/kit-plugin-*`-paketeille
versiosta 0.13+ alkaen ja `@solana/react` 7+:lle. Jos paketinhallintasi
ilmoittaa vertaisriippuvuusvaroituksista `@solana-program/*`-paketista, joka
määrittelee vanhemman Kit-vertaisalueen, ohita ne — paketit toimivat Kit 7:n
kanssa.
...
`ClientProvider`:n avulla. `walletSigner`-lisäosa tekee yhdistetystä lompakosta
maksajan ja identiteetin; vie asiakastyyppi, jotta komponentit voivat tyypittää
`useClient`:n.
...
## !!steps 2. Päivitä asettelu

Kääri sovelluksesi `Providers`-komponentilla. Päivitä `app/layout.tsx` tuomaan
ja käyttämään sitä:
...
Tämä käärii koko sovelluksen Solana-kontekstiin, jotta kaikki alikomponentit
voivat käyttää asiakasta ja lompakkokoukkkuja.

## !!steps 3. Wallet Connect Button

Avattava valikko Wallet Standard -lompakoiden yhdistämiseen/katkaisemiseen.
`useWallets` palauttaa selaimessa havaitut lompakot; `useConnect` /
`useDisconnect` palauttavat `ActionResult` — tämä komponentti käyttää sen
kenttiä `dispatch`, `isRunning` ja `error`.
...
![Lompakon pudotusvalikon vaihtoehdot](/assets/docs/frontend/02-wallet-dropdown-options.webp)

![Lompakko yhdistettynä pudotusvalikossa](/assets/docs/frontend/03-wallet-dropdown-connected.webp)

## !!steps 4. SOL Transfer

Luo siirtoinstruktiio `@solana-program/system`:llä ja lähetä se
`client.sendTransaction`:n kautta. Yhdistetty lompakko allekirjoittaa ja maksaa.
...
Lompakot tulevat Wallet Standard -etsinnästä; kun yhteys on muodostettu,
SOL-siirto käyttää yhdistettyä allekirjoittajaa maksun maksajana.

## !!steps 5. Sivu

`app/page.tsx` renderöi lompakonyhdistys- ja SOL-siirtokomponentit:
