---
title: Maksupainike
description: Valmis React-komponentti Solana-maksuille
---

<Callout type="caution">
  Commerce Kit on tällä hetkellä beta-vaiheessa. API:t saattavat muuttua ennen
  vakaata julkaisua. Lisätietoja Commerce Kitistä löydät [Commerce Kit
  -dokumentaatiosta](https://launch.solana.com/docs/commerce-kit).
</Callout>

`PaymentButton`-komponentti
[Commerce Kitistä](https://www.npmjs.com/package/@solana-commerce/kit) tarjoaa
täydellisen maksukäyttöliittymän – käsittelee lompakon yhdistämisen, tokenin
valinnan, tapahtuman käsittelyn ja käyttöliittymän tilan valmiina.

## Asennus

```bash
pnpm add @solana-commerce/kit
```

## Peruskäyttö

```tsx
import { PaymentButton } from "@solana-commerce/kit";

function Checkout() {
  return (
    <PaymentButton
      config={{
        merchant: { name: "My Store", wallet: "your-wallet-address" },
        mode: "tip"
      }}
      onPaymentSuccess={(signature) => {
        console.log("Payment confirmed:", signature);
      }}
    />
  );
}
```

Tutustu
[Commerce Kit Playgroundiin](https://launch.solana.com/products/commercekit/playground)
konfigurointia ja komponentin testaamista varten.

### Tippipainike

![Tippipainike](/assets/docs/payments/tip-button.png)

### Maksuikkuna

![Maksuikkuna](/assets/docs/payments/pay-modal.png)

### Solana Pay QR-koodiikkuna

![Solana Pay QR-koodiikkuna](/assets/docs/payments/scan-modal.png)

## Maksutilat

`mode`-asetus määrittää painikkeen toiminnan:

| Tila  | Kuvaus                                             |
| ----- | -------------------------------------------------- |
| `tip` | Käyttäjä valitsee oman summan (lahjoitukset/tipit) |

## Konfiguraatio

### Vaaditut asetukset

| Ominaisuus        | Tyyppi   | Kuvaus                                            |
| ----------------- | -------- | ------------------------------------------------- |
| `merchant.name`   | `string` | Yrityksen nimi näytetään kassalla                 |
| `merchant.wallet` | `string` | Solana-lompakko-osoite maksujen vastaanottamiseen |
| `mode`            | 'tip'`   | Maksuvirran tyyppi                                |

### Valinnaiset asetukset

| Ominaisuus     | Tyyppi                  | Kuvaus                                                   |
| -------------- | ----------------------- | -------------------------------------------------------- |
| `network`      | `'mainnet' \| 'devnet'` | Solana-verkko (oletus: `'mainnet'`)                      |
| `rpcUrl`       | `string`                | Mukautettu RPC-päätepiste                                |
| `allowedMints` | `string[]`              | Rajoita hyväksytyt tokenit mint-osoitteen mukaan         |
| `showQR`       | `boolean`               | Ota käyttöön QR-koodimaksuvaihtoehto                     |
| `theme`        | `ThemeConfig`           | Visuaalinen mukauttaminen (värit, reunan pyöristys jne.) |

{/\*

### Maksukonfiguraatio

`buyNow`- ja `cart`-tiloissa tuotteet toimitetaan `paymentConfig`-parametrin
kautta:

```tsx
paymentConfig={{
  products: [
    { id: '1', name: 'T-Shirt', price: 25, quantity: 2 },
    { id: '2', name: 'Hoodie', price: 45, quantity: 1 }
  ]
}}
```

\*/}

## Tapahtumakutsut

| Kutsu              | Parametrit            | Kuvaus                                    |
| ------------------ | --------------------- | ----------------------------------------- |
| `onPaymentSuccess` | `(signature: string)` | Transaktio vahvistettu lohkoketjussa      |
| `onPaymentError`   | `(error: Error)`      | Maksu epäonnistui missä tahansa vaiheessa |
| `onPaymentStart`   | `()`                  | Maksuvirta aloitettu                      |
| `onCancel`         | `()`                  | Käyttäjä peruutti maksun                  |

## Mukautettu laukaisin

Korvaa oletuspainike omalla elementilläsi:

```tsx
<PaymentButton
  config={{
    merchant: { name: "Shop", wallet: "address" },
    mode: "tip"
  }}
>
  <button className="my-custom-button">Pay with Solana</button>
</PaymentButton>
```
