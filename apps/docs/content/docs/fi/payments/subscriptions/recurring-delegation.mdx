...
- `amountPerPeriod` on perusyksiköissä. 6 desimaalin tokenille `1_000_000`
  tarkoittaa `1` tokenia.
- Ohjelma hylkää siirrot, jotka ylittävät kuluvan jakson jäljellä olevan
  käyttörajan.
...
- `expiryTs` on ehdoton raja: kun se ylittyy, siirrot epäonnistuvat ja sponsori
  voi periä rent takaisin. Käyttöaikaan ei liity siirtymäaikaa.
- Aseta `startTs` arvoon `0` käynnistääksesi delegoinnin, kun transaktio
  kirjautuu lohkoketjuun kiinteän ajan sijaan. Tämä laajentaa aikaikkunaa
  käyttäjän allekirjoituksen hankkimiseen ja käyttöönoton suorittamiseen. Tässä
  tapauksessa `expiryTs` on oltava nollasta poikkeava — käynnistymiseen
  perustuva delegointi ei voi myöskään olla asetettu vanhenemaan ikinä.
- Käyttäjä allekirjoittaa asetuksen ja peruutuksen transaktiot. Valtuutettu
