---
title: Tilaussuunnitelma
description: Opas tilaussuunnitelmatransaktioiden määrittämiseen.
---

Tilaussuunnitelma mahdollistaa kauppiaan julkaista laskutusehdot, jotka
käyttäjät voivat hyväksyä. Kun käyttäjä tilaa, kauppias tai hyväksytty noutaja
voi periä enintään suunnitelman summan jokaiselta laskutusjaksolta.

Tämä opas esittää koko työnkulun rakennuspalikoina. Kauppias luo suunnitelman,
tilaaja hyväksyy sen, ja kauppias tai noutaja kerää maksut tuloksena syntyneestä
tilaus-PDA:sta.
...
Kauppias omistaa suunnitelman. Suunnitelma-PDA johdetaan kauppiaan osoitteesta
ja `planId`.

Sponsori voi rahoittaa suunnitelman rent-maksun välittämällä valinnaisen `payer`-parametrin, jolloin kauppias säilyy suunnitelman omistajana. Suunnitelman poistaminen palauttaa maksun omistajalle, ei maksajalle, joten sponsorointi tulee rajoittaa ketjun ulkopuolisesti.
...
Kauppias voi päivittää muokattavat suunnitelmakentät luomisen jälkeen. Olemassa
olevat tilaajat pitävät hyväksymänsä ehdot, kun taas uudet tilaajat hyväksyvät
nykyiset suunnitelman ehdot.

Muutama sääntö pätee:

- Rajallista `endTs` voidaan vain lyhentää, ei koskaan pidentää tai tyhjentää
  (`PlanEndTsCannotExtend`).
- `Sunset`-suunnitelmassa voit poistaa vetäjiä (uuden joukon on oltava nykyisen
  osajoukko) vaarantuneen vetäjän oikeuksien peruuttamiseksi; tila, `endTs` ja
  metatiedot pysyvät muuttumattomina.
- Muokkaukset toimivat suunnitelman viimeisen laskutuskauden aikana, kunhan
  `endTs` pysyy muuttumattomana.
- Instruktiossa on allekirjoitushetkellä havaittu suunnitelman tila
  (`expectedCreatedAt`, `expectedEndTs`, `expectedPullers`,
  `expectedMetadataUri`). Jos live-suunnitelma ei enää vastaa tätä tilaa, päivitys
  hylätään (`StalePlanApproval`), joten vanhentunut allekirjoitettu päivitys ei voi
  palauttaa poistettuja pullereita tai kumota myöhempiä muokkauksia. Liitännäisasiakkaan
  `updatePlan` hakee live-tilan puolestasi; kun rakennat manuaalisesti, hae suunnitelma
  ja välitä kentät itse.
...
Tilaaja hyväksyy voimassa olevat suunnitelmaehdot. Tilauksen PDA johdetaan
suunnitelman PDA:sta ja tilaajan osoitteesta.

Yhdistääksesi auktoriteetin alustuksen ja tilauksen yhdeksi transaktioksi, välitä
sentineliarvo `UNKNOWN_INIT_ID` (viety TypeScript SDK:sta)
`expectedSubscriptionAuthorityInitId`-parametrina. Ohjelma hyväksyy auktoriteetin
vain, jos se luotiin nykyisessä slot-yksikössä, joten sentineliarvo toimii uusille
rekisteröitymisille; palaavan käyttäjän, jonka auktoriteetti luotiin aiemmassa
slot-yksikössä, on välitettävä todellinen `initId` (liitännäisasiakas hakee sen
puolestasi) tai kutsu epäonnistuu virheellä `StaleSubscriptionAuthority`.
...
Kauppias tai sallittu vetäjä allekirjoittaa keräyksen. Kun suunnitelma käyttää
kohteen sallittujen listaa, vastaanottajan token account -omistajan on oltava
merkittynä `destinations`:ssä.
...
Peruuttaminen merkitsee tilauksen päättyväksi. Kumoaminen sulkee tilauksen
PDA:n, kun peruutuksen voimassaoloaika on kulunut. Tilaaja allekirjoittaa
molemmat tapahtumat.
...
Kun sekä tilaaja että nykyinen suunnitelman omistaja allekirjoittavat,
`cancelSubscriptionNow` asettaa tilauksen päättymään peruutushetkellä laskutuskauden
loppuun sijasta. Se voi myös lyhentää odottavaa lisäaikakauden peruutusta. Hyväksyntä
on sidottu allekirjoitushetkellä havaittuun kauden alkuun; jos tilaus on muuttunut
sen jälkeen, transaktio hylätään (`StaleSubscriptionApproval`).
...
Peruutetun tilauksen voi reaktivoida ennen sen kumoamista. Kun tilaus on
kumottu, tilaustilin tiedot suljetaan ja tilaajan täytyy
[tilata uudelleen](#subscribe). Jatkaminen edellyttää tilaajan
`SubscriptionAuthority` suunnitelman minttiä varten, jonka ohjelma vahvistaa
(omistaja, mintti ja `init_id`) ja hylkää, jos se on vanhentunut tai uudelleen
alustettu.

Instruktiossa on tilaajan allekirjoitushetkellä havaitsema vanhentumisaika
(`expectedExpiresAtTs`); ristiriita hylätään (`StaleSubscriptionApproval`),
joten vanhentunut allekirjoitettu jatkaminen ei voi poistaa myöhempää peruutusta,
jota tilaaja ei ole hyväksynyt.
...
- `amount` on perusyksiköissä. 6 desimaalin tokenille `5_000_000` tarkoittaa `5`
  tokenia.
- TypeScript SDK hakee reaaliaikaiset suunnitelman ehdot `subscribe` aikana, kun
  jätät ne pois.
- Rust `SubscribeBuilder` tarvitsee odotetut suunnitelman ehdot. Hae ja pura
  suunnitelmatili ensin, sitten välitä nämä kentät `SubscribeData` kautta.
- Vain kauppias tai `pullers`-listalla oleva lompakko voi kerätä maksuja.
- Tilaaja allekirjoittaa asennuksen, peruutuksen ja kumoamisen tapahtumat.
  Kauppias tai hyväksytty kerääjä allekirjoittaa maksutapahtumat.
