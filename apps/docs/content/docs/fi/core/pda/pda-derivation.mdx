...
Bump seed on yksi tavu (u8), joka liitetään valinnaisten seedien perään
johdannaisessa.
[`find_program_address`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L800)
etsii arvosta 255 alaspäin arvoon 1, kutsuen _rs`create_program_address`_
-funktiota jokaisella arvolla, kunnes tulos putoaa Ed25519-käyrän ulkopuolelle.
Ensimmäinen onnistunut arvo on **kanoninen bump**.
...
PDA:n johdannainen laskee vain osoitteen. Se ei luo lohkoketjussa olevaa tiliä
tuohon osoitteeseen. Tili on luotava erikseen erillisellä instruktiolla
(tyypillisesti _rs`create_account`_ CPI:n kautta).
...
### Kaikkien mahdollisten bump-arvojen iterointi
...
<Callout type="warn">
  Tämä eroaa nykyisestä kanonisesta hakukäyttäytymisestä, joka on dokumentoitu
  `sol_try_find_program_address` -toiminnolle ja joka iteroi arvosta 255
  alaspäin arvoon 1.
</Callout>
...
Ensimmäinen kelpaava bump on 254, mikä tekee siitä kanonisen bumpin.
