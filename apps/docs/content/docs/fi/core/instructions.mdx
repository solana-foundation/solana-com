...
Ohjeet ovat perustavanlaatuinen rakennuspalikka Solana-lohkoketjun kanssa
vuorovaikuttamiseen. Ohje on käytännössä julkinen funktio, jota kuka tahansa
Solana-verkon käyttäjä voi kutsua. Jokaista ohjetta käytetään tietyn toiminnon
...
ohjeensa. Solana-verkon kanssa vuorovaikuttamiseksi yksi tai useampi ohje
...
viestin, joka sisältää SOL-siirto-ohjeen.
...
- [`data`](mention:instruction-data): Tavutaulukko, joka sisältää lisä[dataa]
  käskyn käyttöön.
...
Käskyn [`program_id`](/docs/references/terminology#program-id) on sen ohjelman
...
![Transaktio, jossa on yksi käsky. Käsky sisältää kaksi `AccountMeta` -rakennetta `accounts` -taulukossaan.](/assets/docs/core/transactions/accountmeta.svg)

Tilin metatiedot sisältävät seuraavat tiedot:

- [pubkey](mention:pubkey): Tilin julkinen avainosoite
- [is_signer](mention:is-signer): Asetetaan arvoon `true`, jos tilin on
  allekirjoitettava transaktio
- [is_writable](mention:is-writable): Asetetaan arvoon `true`, jos käsky muokkaa
  tilin dataa
