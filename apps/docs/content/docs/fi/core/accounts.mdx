...
- [`lamports`](mention:lamports): Tilin lamport-määrä
- [`data`](mention:data): Tilin data
- [`owner`](mention:owner): Tilin omistavan ohjelman ID
- [`executable`](mention:executable): Ilmaisee, sisältääkö tili suoritettavaa
  binääriä
...
1. Tilin hakeminen sen [osoitteella](#account-address)
2. Tilin `data` -kentän deserialisointi raakatavuista sopivaan datarakenteeseen,
   kuten tilin omistava ohjelma on määritellyt.
...
ensin luotava uusi datatili. Ohjelman tilatiliä luova prosessi on usein
abstraktoitu, mutta taustalla tapahtuvan prosessin ymmärtäminen on hyödyllistä.
...
Kaikille tileille ei määritetä uutta omistajaa sen jälkeen, kun System Program
on luonut ne. System Programin omistamia tilejä kutsutaan järjestelmätileiksi.
Kaikki lompakkotilit ovat järjestelmätilejä, mikä mahdollistaa niiden
[transaktiomaksujen](/docs/core/fees) maksamisen.
