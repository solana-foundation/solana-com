...
title: Oletustila
description:
  Määritä Token Extensions Program -mintti niin, että äskettäin luodut
  token-tilit alkavat valitusta oletustilasta, kuten jäädytettynä.
...
## Mikä on oletustilin tila?

Token Extensions Program -ohjelman _rs`DefaultAccountState`_-mintin laajennus
asettaa alkuperäisen _rs`AccountState`_-tilan jokaiselle uudelle token-tilille,
joka luodaan kyseiselle mintille.

Tätä käytetään yleisimmin siihen, että uudet token-tilit alkavat jäädytettynä.
Jäädytettynä alkavia token-tilejä ei voi käyttää ennen kuin mintin
jäädytysvaltuudet sulattavat ne.

## Kuinka luoda mintti oletustilin tilalla

Luodaksesi mintin oletustilin tilalla:

1. Laske minttitilien koko ja tarvittava rent mintille ja
   _rs`DefaultAccountState`_-laajennukselle.
2. Luo minttitili _rs`CreateAccount`_-komennolla, alusta
   _rs`DefaultAccountState`_ ja alusta mintti _rs`InitializeMint`_-komennolla.
3. Luo token-tilejä mintille ja huomaa, että jokainen uusi token-tili alkaa
   nykyisestä oletustilasta.
4. Käytä _rs`UpdateDefaultAccountState`_-komentoa muuttaaksesi tilan, josta
   tulevien token-tilien tulisi alkaa.

<ScrollyCoding>

## !!steps Laske tilin koko

Laske minttitilien koko perusmintille lisättynä
_rs`DefaultAccountState`_-laajennuksella. Tätä kokoa käytetään
...
Laske rent käyttäen mintille tarvittavaa kokoa lisättynä
_rs`DefaultAccountState`_-laajennuksella.
...
## !!steps Luo minttitili

Luo minttitili lasketulla tilalla ja lamporteilla.
...
## !!steps Alusta DefaultAccountState

Alusta _rs`DefaultAccountState`_-laajennus mintille.
...
Alusta mint _rs`InitializeMint`_-komennon kanssa samassa transaktiossa.
...
## !!steps Luo token-tili

Luo token-tili mintille. Koska nykyinen oletustila on _rs`Frozen`_, uusi
token-tili alkaa jäädytettynä.
...
## !!steps Päivitä oletustilin tila

Käytä _rs`UpdateDefaultAccountState`_-komentoa muuttaaksesi oletustilaa, jotta
tulevat token-tilit eivät enää ala jäädytettyinä.
...
<Callout type="info" title="Käskyjen järjestys">
  _rs`DefaultAccountStateInstruction::Initialize`_ -komennon on tultava ennen
  _rs`InitializeMint`_-komentoa. _rs`CreateAccount`_,
  _rs`DefaultAccountStateInstruction::Initialize`_ ja _rs`InitializeMint`_
  -komennot on sisällytettävä samaan transaktioon.
</Callout>
...
| Kohde                                            | Kuvaus                                                                                                    | Lähde                                                                                                                                             |
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`DefaultAccountState`_                        | Mint-laajennus, joka tallentaa tilan, jonka uusien token-tilien tulisi periä.                             | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/mod.rs#L16-L21)         |
| _rs`DefaultAccountStateInstruction::Initialize`_ | Käsky, joka alustaa mintin oletusarvoisen token-tilin tilan ennen _rs`InitializeMint`_-komentoa.          | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L20-L36) |
| _rs`DefaultAccountStateInstruction::Update`_     | Käsky, jonka avulla jäädytysvaltuutus voi muuttaa oletustilaa tuleville token-tileille.                   | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L37-L53) |
| _rs`process_initialize_default_account_state`_   | Prosessorilogiikka, joka kirjoittaa alkuperäisen oletustilan alustamattomalle mintille.                   | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L29-L40)     |
| _rs`process_update_default_account_state`_       | Prosessorilogiikka, joka vahvistaa jäädytysvaltuutuksen ennen mintin tallennetun oletustilan muuttamista. | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L43-L74)     |

### Typescript

Alla oleva `Kit`-esimerkki käyttää generoituja käskyjä suoraan. Vanhat esimerkit
käyttäen `@solana/web3.js`- ja `@solana/spl-token`-komentoja on sisällytetty
viitteeksi.
