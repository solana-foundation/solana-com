...
description:
  Luo mint, joka tallentaa metadatan suoraan mint-tilille käyttäen MetadataPointer- ja TokenMetadata-laajennuksia, ja päivitä, poista ja lukitse metadata token-metadata-rajapinnan kautta.
...
Alla oleva `Kit`-esimerkki käyttää suoraan generoituja ohjeita. Vanhat esimerkit, jotka käyttävät `@solana/web3.js`-, `@solana/spl-token`- ja `@solana/spl-token-metadata`-kirjastoja, on sisällytetty viitteeksi. Vanhoissa esimerkeissä `maxSupportedTransactionVersion` on rajattu arvoon `0`, koska web3.js v1 ei pysty deserialisoimaan [v1-transaktioita](/docs/core/transactions/versioned-transactions#v1-message-format).
