...
description:
  Luo ryhmä- ja ryhmäjäsenminttejä, jotka tallentavat ryhmätiedot suoraan
  mint-tilille käyttäen GroupPointer-, GroupMemberPointer-, TokenGroup- ja
  TokenGroupMember-laajennuksia.
...
## Mitä ovat ryhmä- ja ryhmäjäsenlaajennukset?

Ryhmä on mintti, joka edustaa kokoelmaa. Ryhmäjäsen on mintti, joka kuuluu
kyseiseen kokoelmaan.

Käytä näitä laajennuksia, kun yhden mintin tulisi edustaa kokoelmaa ja muiden
minttien tulisi edustaa siihen kuuluvia kohteita.

Token Extensions Program voi tallentaa tokenryhmän tiedot suoraan minttiin
neljän toisiinsa liittyvän laajennuksen avulla:

- _rs`GroupPointer`_ osoittaa mintin tiliin, joka tallentaa ryhmän tiedot.
- _rs`TokenGroup`_ tallentaa itse ryhmätiedot, mukaan lukien päivitysvaltuutus,
  nykyinen koko ja enimmäiskoko.
- _rs`GroupMemberPointer`_ osoittaa mintin tiliin, joka tallentaa jäsenen
  tiedot.
- _rs`TokenGroupMember`_ tallentaa jäsenen ryhmäosoitteen ja jäsennumeron.

_rs`GroupPointer`_ ja _rs`GroupMemberPointer`_ voivat viitata mihin tahansa
tiliin, jonka omistaa
[Token group -rajapinnan](https://github.com/solana-program/token-group/tree/main/interface)
toteuttava ohjelma.

Token Extensions Program toteuttaa kyseisen rajapinnan myös suoraan
_rs`TokenGroup`_- ja _rs`TokenGroupMember`_-minttilaajennosten kautta.

## Miten luodaan ryhmiä ja jäseniä, jotka tallennetaan mint-tilille

Mint-tilille tallennettujen ryhmien ja jäsenten luominen:

1. Luo ryhmäminttitili ja alusta _rs`GroupPointer`_.
2. Alusta ryhmämintti _rs`InitializeMint`_-komennolla.
3. Alusta _rs`TokenGroup`_ samalle mintille.
4. Luo jäsenminttitili ja alusta _rs`GroupMemberPointer`_.
5. Alusta jäsenmintti _rs`InitializeMint`_-komennolla.
6. Alusta _rs`TokenGroupMember`_ samalle mintille siten, että se viittaa
   ryhmäminttiin.

<ScrollyCoding>

## !!steps Laske ryhmä-mintin koko ja vuokra

Laske ryhmä-mintille tarvittava koko ja vuokra.
...
## !!steps Luo ja alusta ryhmä-mint

Luo ryhmä-mint-tili, alusta _rs`GroupPointer`_, alusta mint ja alusta
_rs`TokenGroup`_ yhdessä transaktiossa.
...
## !!steps Laske jäsen-mintin koko ja vuokra

Laske jäsen-mintille tarvittava koko ja vuokra.
...
## !!steps Luo ja alusta jäsen-mint

Luo jäsen-mint-tili, alusta _rs`GroupMemberPointer`_, alusta mint ja alusta
_rs`TokenGroupMember`_ yhdessä transaktiossa.
...
<Callout type="info" title="Osoittimet ja käskyjen järjestys">
_rs`GroupPointer`_ ja _rs`GroupMemberPointer`_ tallentavat sen tilin osoitteen, jossa ryhmän tai jäsenen tiedot sijaitsevat. _rs`TokenGroup`_ ja _rs`TokenGroupMember`_ tallentavat varsinaiset ryhmän tai jäsenen tiedot.

_rs`GroupPointerInstruction::Initialize`_ ja
_rs`GroupMemberPointerInstruction::Initialize`_ on suoritettava ennen
_rs`InitializeMint`_-käskyä. _rs`TokenGroupInstruction::InitializeGroup`_ ja
_rs`TokenGroupInstruction::InitializeMember`_ on suoritettava
_rs`InitializeMint`_-käskyn jälkeen. Jokaiselle mintille _rs`CreateAccount`_,
osoittimen alustuskäsky ja _rs`InitializeMint`_ on sisällytettävä samaan
transaktioon.
...
_rs`GroupPointer`_ ja _rs`GroupMemberPointer`_ ovat Token Extensions Program
-ohjelman osoitinkäskyjä. _rs`TokenGroup`_ ja _rs`TokenGroupMember`_ noudattavat
Token group -rajapintaa, jonka Token Extensions Program toteuttaa.

#### Ryhmäosoitin ja ryhmäjäsenosoitin

| Kohde                                           | Kuvaus                                                                                                      | Lähde                                                                                                                                            |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`GroupPointer`_                              | Mintin laajennus, joka tallentaa ryhmän tiedot sisältävän tilin valtuutuksen ja osoitteen.                  | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_pointer/mod.rs#L12-L25)                |
| _rs`GroupMemberPointer`_                        | Mintin laajennus, joka tallentaa jäsenen tiedot sisältävän tilin valtuutuksen ja osoitteen.                 | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_member_pointer/mod.rs#L12-L25)         |
| _rs`GroupPointerInstruction::Initialize`_       | Alustaa ryhmäosoitinlaajennuksen ennen _rs`InitializeMint`_-käskyä.                                         | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_pointer/instruction.rs#L22-L38)        |
| _rs`GroupPointerInstruction::Update`_           | Päivittää mintin _rs`GroupPointer`_-laajennuksen tallentaman ryhmäosoitteen.                                | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_pointer/instruction.rs#L39-L55)        |
| _rs`GroupMemberPointerInstruction::Initialize`_ | Alustaa ryhmäjäsenosoitinlaajennuksen ennen _rs`InitializeMint`_-käskyä.                                    | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_member_pointer/instruction.rs#L22-L38) |
| _rs`GroupMemberPointerInstruction::Update`_     | Päivittää mintin _rs`GroupMemberPointer`_-laajennuksen tallentaman jäsenosoitteen.                          | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_member_pointer/instruction.rs#L39-L55) |
| _rs`process_initialize`_ (`GroupPointer`)       | Kirjoittaa alkuperäisen _rs`GroupPointer`_-valtuutuksen ja ryhmäosoitteen mintille.                         | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/group_pointer/processor.rs#L25-L50)            |
| _rs`process_update`_ (`GroupPointer`)           | Validoi ryhmäosoittimen valtuutuksen ja kirjoittaa sitten mintin tallentaman ryhmäosoitteen uudelleen.      | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/group_pointer/processor.rs#L52-L78)            |
| _rs`process_initialize`_ (`GroupMemberPointer`) | Kirjoittaa alkuperäisen _rs`GroupMemberPointer`_-valtuutuksen ja jäsenosoitteen mintille.                   | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/group_member_pointer/processor.rs#L25-L50)     |
| _rs`process_update`_ (`GroupMemberPointer`)     | Validoi ryhmäjäsenosoittimen valtuutuksen ja kirjoittaa sitten mintin tallentaman jäsenosoitteen uudelleen. | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/group_member_pointer/processor.rs#L52-L78)     |

#### Token Group ja Token Group Member

| Kohde                                             | Kuvaus                                                                                                                        | Lähde                                                                                                                                 |
| ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`TokenGroup`_                                  | Token group -rajapintatila, joka on tallennettu mintiin, mukaan lukien päivitysviranomainen, nykyinen koko ja maksimikoko.    | [Lähde](https://github.com/solana-program/token-group/blob/main/interface/src/state.rs#L13-L27)                                       |
| _rs`TokenGroupMember`_                            | Token group member -rajapintatila, joka on tallennettu mintiin, mukaan lukien jäsen-mint, ryhmän osoite ja jäsennumero.       | [Lähde](https://github.com/solana-program/token-group/blob/main/interface/src/state.rs#L64-L76)                                       |
| _rs`TokenGroupInstruction::InitializeGroup`_      | Token group -rajapintakomento, jota Token Extensions Program tukee uuden ryhmän alustamiseen jo alustetulle mintille.         | [Lähde](https://github.com/solana-program/token-group/blob/main/interface/src/instruction.rs#L52-L62)                                 |
| _rs`TokenGroupInstruction::UpdateGroupMaxSize`_   | Token group -rajapintakomento, jota Token Extensions Program tukee ryhmän jäsenten enimmäismäärän päivittämiseen.             | [Lähde](https://github.com/solana-program/token-group/blob/main/interface/src/instruction.rs#L64-L70)                                 |
| _rs`TokenGroupInstruction::UpdateGroupAuthority`_ | Token group -rajapintakomento, jota Token Extensions Program tukee ryhmän päivitysviranomaisen vaihtamiseen tai poistamiseen. | [Lähde](https://github.com/solana-program/token-group/blob/main/interface/src/instruction.rs#L72-L78)                                 |
| _rs`TokenGroupInstruction::InitializeMember`_     | Token group -rajapintakomento, jota Token Extensions Program tukee uuden jäsenen alustamiseen jo alustetulle ryhmälle.        | [Lähde](https://github.com/solana-program/token-group/blob/main/interface/src/instruction.rs#L80-L92)                                 |
| _rs`process_initialize_group`_                    | Validoi mintin, tarkistaa että _rs`GroupPointer`_ on läsnä, ja varaa _rs`TokenGroup`_-tilan ryhmän mintille.                  | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_group/processor.rs#L43-L90)   |
| _rs`process_update_group_max_size`_               | Validoi nykyisen päivitysviranomaisen, jonka jälkeen päivittää ryhmän maksimikoon.                                            | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_group/processor.rs#L95-L117)  |
| _rs`process_update_group_authority`_              | Validoi nykyisen päivitysviranomaisen, jonka jälkeen vaihtaa tai poistaa ryhmän päivitysviranomaisen.                         | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_group/processor.rs#L119-L141) |
| _rs`process_initialize_member`_                   | Validoi jäsen-mintin ja ryhmän viranomaisen, kasvattaa ryhmän kokoa, ja varaa _rs`TokenGroupMember`_-tilan mintille.          | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_group/processor.rs#L143-L202) |
...
token-ryhmän apufunktioita, on sisällytetty viitteeksi.
