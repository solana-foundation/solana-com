...
description:
  Vaadi saapuvia siirtoja sisällyttämään muistio ottamalla käyttöön
  MemoTransfer-tililaajennuksen Token Extensions Program -token-tilillä.
...
## Mikä on muistion siirto?

Token Extensions Program -ohjelman _rs`MemoTransfer`_-tililaajennuksen avulla
vaaditaan muistio-ohje välittömästi ennen mitä tahansa saapuvaa siirtoa
kyseiselle token-tilille. Lähtevät siirrot kyseiseltä token-tililtä eivät vaadi
muistiota.

Laajennus määritetään kohde-token-tilille, ei token-määrittelylle (mint). Kun
laajennus on otettu käyttöön, siirrot ilman muistiota epäonnistuvat virheellä
_rs`TokenError::NoMemo`_, kunnes token-tilin omistaja poistaa vaatimuksen
käytöstä.

## Kuinka luoda token-tili MemoTransfer-laajennuksella

Luodaksesi token-tilin, jossa _rs`MemoTransfer`_ on käytössä:

1. Luo ja alusta token-määrittely (mint).
2. Laske token-tilin koko ja vuokra, joka tarvitaan token-tilille
   _rs`MemoTransfer`_-laajennuksella.
3. Luo token-tili ja ota _rs`MemoTransfer`_ käyttöön kyseisellä tilillä.
4. _rs`Transfer`_- ja _rs`TransferChecked`_-ohjeet tälle token-tilille on
   sisällytettävä muistio-ohje välittömästi ennen siirto-ohjetta.

<ScrollyCoding>

## !!steps Luo ja alusta token-määrittely

Luo ja alusta token-määrittely, jota token-tili käyttää.
...
## !!steps Laske token-tilin koko

Laske token-tilin koko perus-token-tilille lisättynä
_rs`MemoTransfer`_-laajennuksella. Tätä kokoa käytetään
...
Laske vuokra käyttämällä token-tilin kokoa, joka tarvitaan
_rs`MemoTransfer`_-laajennukselle.
...
## !!steps Luo ja alusta token-tili

Luo token-tili lasketulla tilavarauksella ja lamporteilla, ja alusta se sitten
minttiä varten.
...
## !!steps Ota MemoTransfer käyttöön

Ota _rs`MemoTransfer`_ käyttöön token-tilillä.
...
| Kohde                                           | Kuvaus                                                                                                                        | Lähde                                                                                                                                     |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`MemoTransfer`_                              | Tililaajennos, joka tallentaa tiedon siitä, vaativatko saapuvat siirrot muistiinpanon.                                        | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/mod.rs#L17-L22)         |
| _rs`RequiredMemoTransfersInstruction::Enable`_  | Ohje, joka ottaa käyttöön muistiinpanoa vaativat siirrot token-tilillä.                                                       | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L50-L71) |
| _rs`RequiredMemoTransfersInstruction::Disable`_ | Ohje, joka poistaa käytöstä muistiinpanoa vaativat siirrot token-tilillä.                                                     | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L74-L95) |
| _rs`process_toggle_required_memo_transfers`_    | Prosessorilogiikka, joka alustaa tai kirjoittaa uudelleen _rs`MemoTransfer`_-laajennoksen omistajuuden vahvistamisen jälkeen. | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/memo_transfer/processor.rs#L20-L48)     |

### Typescript

Alla oleva `Kit`-esimerkki käyttää generoituja ohjeita suoraan. Vanhat
esimerkit, joissa käytetään `@solana/web3.js`-, `@solana/spl-token`- ja
`@solana/spl-memo`-ohjelmia, on sisällytetty viitteeksi.
