...
description: Opi ottamaan käyttöön PausableMintExtension.
---

## Kuinka ottaa käyttöön PausableExtension
...
mahdollistaa nimetyn keskeytysvaltuutuksen pysäyttää kaiken token-toiminnan
mintissä. Keskeytysvaltuutus voi keskeyttää tai jatkaa mintin toimintaa milloin
tahansa.

Kun toiminta on keskeytetty, Token-2022-ohjelma hylkää kaikki siirrot, lyönnit
ja polttamiset kyseiselle tokenille. Kun keskeytys poistetaan, normaalit
toiminnot jatkuvat.

Tämä on yleinen malli sekä lohkoketjun token-järjestelmissä että perinteisessä
rahoituksessa, jossa kyky jäädyttää toiminta on tarpeen vaatimustenmukaisuuden,
tapausten käsittelyn tai hallinnollisen valvonnan vuoksi.
