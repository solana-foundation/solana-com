---
title: Keskeytettävä lyönti
description:
  Luo Token-2022-mint paussiauktoriteetilla, estä siirrot mintin ollessa
  pysäytettynä ja jatka toimintaa myöhemmin.
...
Token Extensions Program -ohjelman _rs`PausableConfig`_-mintlaajennuksella
paussiauktoriteetti voi pysäyttää ja jatkaa token-toimintaa koko mintille.
...
1. Laske mint-tilin koko ja vaadittu vuokra mintille ja
   _rs`PausableConfig`_-laajennukselle.
2. Luo mint-tili _rs`CreateAccount`_-komennolla, alusta _rs`PausableConfig`_ ja
   alusta mint _rs`InitializeMint`_-komennolla.
3. Mintaa tokeneita ja luo token-tilejä mintille.
4. Käytä _rs`Pause`_-komentoa estääksesi token-toiminnan mintille.
5. Käytä _rs`Resume`_-komentoa salliaksesi token-toiminnan uudelleen.

<ScrollyCoding>

## !!steps Laske tilin koko

Laske mint-tilin koko perus-mintille lisättynä _rs`PausableConfig`_-
laajennuksella. Tätä kokoa käytetään _rs`CreateAccount`_-komennossa.
...
Laske vuokra käyttämällä mintille tarvittavaa kokoa lisättynä
_rs`PausableConfig`_- laajennuksella.
...
## !!steps Alusta PausableConfig

Alusta _rs`PausableConfig`_ -laajennus mintille.
...
Pysäytä mint käyttäen _rs`Pause`_ -komentoa.
...
## !!steps Jatka mintin toimintaa

Jatka mintin toimintaa käyttäen _rs`Resume`_ -komentoa.
...
<Callout type="info" title="Komentojen järjestys">
  _rs`PausableInstruction::Initialize`_ -komennon on tultava ennen
  _rs`InitializeMint`_ -komentoa. _rs`CreateAccount`_,
  _rs`PausableInstruction::Initialize`_ ja _rs`InitializeMint`_ on
...
| Kohde                                 | Kuvaus                                                                                                                       | Lähde                                                                                                                                |
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`PausableConfig`_                  | Mintin laajennus, joka tallentaa pysäytysvaltuuden ja tiedon siitä, onko mint tällä hetkellä pysäytettynä.                   | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/mod.rs#L17-L22)         |
| _rs`PausableInstruction::Initialize`_ | Komento, joka alustaa pysäytettävän konfiguraation ennen _rs`InitializeMint`_ -komentoa.                                     | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L20-L32) |
| _rs`PausableInstruction::Pause`_      | Komento, joka merkitsee mintin pysäytetyksi.                                                                                 | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L33-L44) |
| _rs`PausableInstruction::Resume`_     | Komento, joka merkitsee mintin toimintaa jatkavaksi.                                                                         | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L45-L56) |
| _rs`process_initialize`_              | Käsittelylogiikka, joka alustaa _rs`PausableConfig`_ -laajennuksen alustamattomalle mintille ja tallentaa pysäytysvaltuuden. | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L22-L35)     |
| _rs`process_toggle_pause`_            | Käsittelylogiikka, joka vahvistaa pysäytysvaltuuden ennen mintin pysäytyslipun vaihtamista.                                  | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L39-L64)     |

### Typescript

Alla oleva `Kit` -esimerkki käyttää generoituja komentoja suoraan. Vanhat
esimerkit, joissa käytetään `@solana/web3.js`- ja `@solana/spl-token`
-komentoja, on sisällytetty viitteeksi.
