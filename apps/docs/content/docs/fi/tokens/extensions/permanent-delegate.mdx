...
description:
  Luo Token Extensions Program -mintti pysyvällä delegaatilla, joka voi siirtää
  tai polttaa tokeneita mistä tahansa kyseisen mintin tilistä.
...
## Mikä on pysyvä delegaatti?

Token Extensions Program -ohjelman _rs`PermanentDelegate`_ -minttilaajennus
määrittää delegaattiauktoriteetin itse mintille.

Pysyvä delegaatti on minttitason auktoriteetti, joka voi valtuuttaa siirtoja ja
polttoja mille tahansa kyseisen mintin token accountille. Se toimii globaalina
delegaattina kaikille mintin token accounteille, eikä token account -omistaja
voi peruuttaa pysyvää delegaattia token accountiltaan.

Mintin auktoriteetti voi myös vaihtaa pysyvän delegaatin myöhemmin käyttämällä
_rs`SetAuthority`_-toimintoa komennolla _rs`AuthorityType::PermanentDelegate`_.

## Pysyvän delegaatin luominen ja käyttäminen

Pysyvän delegaatin luomiseksi ja käyttämiseksi:

1. Laske mintin tilin koko ja tarvittava rent mintille ja
   _rs`PermanentDelegate`_-laajennukselle.
2. Luo mintin tili komennolla _rs`CreateAccount`_, alusta
   _rs`PermanentDelegate`_ ja alusta mintti komennolla _rs`InitializeMint`_.
3. Lähetä _rs`TransferChecked`_ tai _rs`BurnChecked`_, jonka on allekirjoittanut
   pysyvä delegaatti. Token Program käsittelee nämä ohjeet samalla tavalla kuin
   jos token account -omistaja olisi allekirjoittanut ne.

<ScrollyCoding>

## !!steps Laske tilin koko

Laske mintin tilin koko perusmintille sekä
_rs`PermanentDelegate`_-laajennukselle. Tätä kokoa käytetään komennossa
_rs`CreateAccount`_.
...
Laske rent käyttäen mintille tarvittavaa kokoa sekä
_rs`PermanentDelegate`_-laajennusta.
...
## !!steps Alusta PermanentDelegate

Alusta _rs`PermanentDelegate`_-laajennus mintille.
...
## !!steps Luo token accountit ja mintaa tokeneita

Luo token accountit omistajalle ja vastaanottajalle, ja mintaa sitten tokeneita
omistajan token accountille.
...
## !!steps Siirrä pysyvän delegaatin avulla

Siirrä tokeneita _rs`TransferChecked`_-komennolla, jonka pysyvä delegaatti on
allekirjoittanut.
...
<Callout type="info" title="Käskyjen järjestys">
  _rs`InitializePermanentDelegate`_ on suoritettava ennen
  _rs`InitializeMint`_-käskyä. _rs`CreateAccount`_,
  _rs`InitializePermanentDelegate`_ ja _rs`InitializeMint`_ on sisällytettävä
  samaan transaktioon.
</Callout>
...
| Kohde                                       | Kuvaus                                                                                                                       | Lähde                                                                                                                              |
| ------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermanentDelegate`_                     | Mintin laajennus, joka tallentaa valtuutetun delegaatin kaikille mintin token accounteille.                                  | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/permanent_delegate.rs#L15-L21) |
| _rs`InitializePermanentDelegate`_           | Käsky, joka alustaa pysyvän delegaatin ennen _rs`InitializeMint`_-käskyä.                                                    | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1952-L1967)              |
| _rs`SetAuthority`_                          | Perus token-käsky, jota käytetään _rs`AuthorityType::PermanentDelegate`_-käskyn kanssa delegaatin vaihtamiseen.              | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1488-L1520)              |
| _rs`AuthorityType::PermanentDelegate`_      | Valtuutuksen erottelutunniste, jota käytetään _rs`SetAuthority`_-käskyn kanssa mintin pysyvän delegaatin vaihtamiseen.       | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1138-L1139)              |
| _rs`process_initialize_permanent_delegate`_ | Prosessorilogiikka, joka kirjoittaa pysyvän delegaatin laajennuksen minttiin.                                                | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1542-L1557)                  |
| _rs`process_set_authority`_                 | Prosessorilogiikka, joka validoi ja vaihtaa pysyvän delegaatin, kun _rs`AuthorityType::PermanentDelegate`_-käskyä käytetään. | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L849-L860)                    |
