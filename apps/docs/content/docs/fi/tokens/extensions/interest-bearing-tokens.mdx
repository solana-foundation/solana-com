...
description:
  Luo Token Extensions Program -mintti, jossa on ketjussa oleva korko, päivitä kyseinen korko ja muunna token-määrät käyttöliittymän määräksi koron kanssa.
...
## Mikä on korkoa tuottava mintti?

Token Extensions Program -ohjelman _rs`InterestBearingConfig`_ mintin laajennus
mahdollistaa vuotuisen koron tallentamisen suoraan ketjuun.

Korkoa tuottavat tokenit **eivät** lisää tokeneja token-tileille ajan myötä.
Kuhunkin token-tiliin tallennettu token-määrä pysyy samana, kunnes jokin
token-ohjelman käsky muuttaa sitä, kuten mintaus, siirto tai polttaminen.

Ajan kuluessa laskettu käyttöliittymän määrä koron kanssa voi kasvaa, vaikka
token-määrä ja token-tarjonta pysyvätkin samoina.

## Kuinka historialliset koron muutokset lasketaan
...
- [`InterestBearingConfig`](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L40-L50)
  tallentaa `initialization_timestamp`:n, `pre_update_average_rate`:n,
  `last_update_timestamp`:n ja `current_rate`:n.
- [`amount_to_ui_amount`](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L85-L94)
  laskee käyttöliittymän määrän kahdessa vaiheessa: se laskee ensin koron
  alustamisesta `last_update_timestamp`:hen käyttäen
  `pre_update_average_rate`:tä, ja sitten koron `last_update_timestamp`:sta
  nykyiseen aikaleimaan käyttäen `current_rate`:tä.
- Kun
  ...
  kutsuu
  [`time_weighted_average_rate`](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L121-L149):ia
  ja päivittää `pre_update_average_rate`:n uudeksi ajalla painotetuksi
  historialliseksi keskiarvoksi, minkä jälkeen se tallentaa uuden
  `last_update_timestamp`:n ja `current_rate`:n.
- Ajan myötä tapahtuvien koron muutosten jälkeen aikaisemmat muutokset esitetään
  `pre_update_average_rate`:llä, kun taas ajanjakso `last_update_timestamp`:sta
  nykyhetkeen lasketaan `current_rate`:lla.

## Kuinka luodaan ja käytetään korkoa tuottavia tokeneita

Korkoa tuottavien tokeneiden luominen ja käyttö:

1. Laske mintin tilin koko ja tarvittava vuokra mintille ja
   _rs`InterestBearingConfig`_ -laajennukselle.
2. Luo mintin tili _rs`CreateAccount`_:lla, alusta _rs`InterestBearingConfig`_
   ja alusta mintti _rs`InitializeMint`_:lla.
3. Mintaa tokeneita tavalliseen tapaan.
4. Käytä _rs`UpdateRate`_:ä mintin nykyisen koron muuttamiseen ajan myötä.
5. Muunna token-määrät käyttöliittymän määräksi koron kanssa
   _rs`AmountToUiAmount`_:lla tai apumetodeilla, jotka hakevat mintin tilin ja
   kellon sysvar:in ja laskevat sitten käyttöliittymän määrän lähettämättä
   transaktiota.

<Callout type="info" title="Offchain- ja Onchain-käyttöliittymäsumman muunnos">
  Offchain-aputoiminnon polku hakee mint accountin ja kellotaajuuden sysvarin,
  laskee sitten käyttöliittymän summan paikallisesti lähettämättä transaktiota.
  Onchain-polku käyttää _rs`AmountToUiAmount`_-toimintoa, joka suoritetaan Token
  Programissa ja palauttaa käyttöliittymän summan transaktion paluudatassa.
  _rs`AmountToUiAmount`_ voidaan lähettää transaktiossa tai simuloida.
</Callout>
...
Laske mint accountin koko perus-mintille lisättynä
_rs`InterestBearingConfig`_-laajennuksella. Tämä on koko, jota käytetään
_rs`CreateAccount`_-toiminnossa.
...
Laske rent käyttäen kokoa, joka tarvitaan mintille lisättynä
_rs`InterestBearingConfig`_-laajennuksella.
...
## !!steps Alusta InterestBearingConfig

Alusta _rs`InterestBearingConfig`_-laajennus mintille.
...
Alusta mint _rs`InitializeMint`_-toiminnolla samassa transaktiossa.
...
## !!steps Luo token account ja lyö tokeneita

Luo token account mintille ja lyö sitten tokeneita kyseiselle token accountille.
...
## !!steps Laske käyttöliittymän määrä offline-avustajalla

Hae mint account ja kellosysvar, laske sitten käyttöliittymän määrä lähettämättä
transaktiota.
...
<Callout type="info" title="Ohjeiden järjestys">
  _rs`InterestBearingMintInstruction::Initialize`_ täytyy tulla ennen
  _rs`InitializeMint`_-ohjetta. _rs`CreateAccount`_,
  _rs`InterestBearingMintInstruction::Initialize`_ ja _rs`InitializeMint`_ on
...
| Kohde                                             | Kuvaus                                                                                                                   | Lähde                                                                                                                                              |
| ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InterestBearingConfig`_                      | Mint-laajennus, joka tallentaa korkoviranomaisen, aikaleimat sekä nykyiset ja historialliset korot.                         | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L40-L153)         |
| _rs`InterestBearingMintInstruction::Initialize`_ | Ohje, joka alustaa korkoa tuottavan konfiguraation ennen _rs`InitializeMint`_-ohjetta.                                         | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/instruction.rs#L17-L90)  |
| _rs`InterestBearingMintInstruction::UpdateRate`_ | Ohje, joka muuttaa mintin nykyistä korkoa.                                                                    | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/instruction.rs#L35-L113) |
| _rs`AmountToUiAmount`_                           | Ohje, joka palauttaa nykyisen käyttöliittymän määrän merkkijonona token-määrälle käyttäen mintin aktiivista konfiguraatiota.                      | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L520-L538)                                |
| _rs`InterestBearingConfig::amount_to_ui_amount`_ | Apufunktio, joka muuntaa token-määrän käyttöliittymän määräksi korkoineen tietylle aikaleimaukselle.                                           | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L85-L96)          |
| _rs`process_initialize`_                         | Prosessorilogiikka, joka alustaa _rs`InterestBearingConfig`_-laajennuksen ja tallentaa alkuperäiset aikaleimat ja koron.                     | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/interest_bearing_mint/processor.rs#L21-L43)      |
| _rs`process_update_rate`_                        | Prosessorilogiikka, joka päivittää nykyisen koron, laskee uudelleen aikapainotetun keskikoron ja tallentaa päivitysleiman. | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/interest_bearing_mint/processor.rs#L46-L77)      |
| _rs`process_amount_to_ui_amount`_                | Prosessorilogiikka, joka palauttaa käyttöliittymän määrän merkkijonona käyttäen mintin aktiivista korkoa tuottavaa konfiguraatiota.                     | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1548-L1569)                                  |

### Typescript

Alla oleva `Kit`-esimerkki käyttää generoituja ohjeita suoraan. Vanhemmat
esimerkit käyttäen `@solana/web3.js`- ja `@solana/spl-token`-kirjastoja on
sisällytetty viitteeksi.
