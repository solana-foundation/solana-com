...
- Käyttäjän reaaliaikaisen saldon saamiseksi voit hakea päivitetyt tiedot yllä
  olevista summista kutsumalla joko getTokenAccountBalance- tai
  getAccountInfo-funktiota
- Jos sinun täytyy tietää UI Amount mielivaltaiselle summalle, voit saada tämän
  laskennan kutsumalla
  ...
  - Huomio: amountToUiAmount vaatii transaktiosimulaation, mikä tarkoittaa, että
    ...
    ei saisi olla oletustapa saldon hakemiseen.
...
- Käyttäjien tulisi syöttää summat, jotka tulkitaan skaalatuksi
  "UIAmount"-arvoksi. Sovelluksen, joka käsittelee tämän, tulisi muuntaa se
  raakaksi token-summaksi transaktiota varten.
  - Jos pyöristysongelmia ilmenee, pyöristä alaspäin ja jätä mieluummin pieni
    määrä pölyä jäljelle sen sijaan, että riskeeraat transaktion epäonnistumisen
...
- Nykyinen kerroin voidaan lukea token mintistä milloin tahansa kutsumalla
  `getAccountInfo`-funktiota. Lisäksi, jos tuleva kerroin on asetettu, tämä
  tieto on myös saatavilla token mintistä. Suosittelemme, ettei tätä kerrointa
  näytetä, koska se voi hämmentää käyttökokemusta.
