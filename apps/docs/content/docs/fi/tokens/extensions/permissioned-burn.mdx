...
Paikallinen testivalidator sisältää myös vanhemman Token-2022-koontiversio,
joten tällä sivulla olevien esimerkkien suorittamiseksi lataa v11.0.0-ohjelma
devnetistä testivalidatoriisi:
...
## Kuinka luoda mint ja polttaa luvalla

Luodaksesi mintin luvallisella poltolla ja polttaaksesi tokeneita:

1. Laske mint account -koko ja tarvittava rent mintille sekä
   _rs`PermissionedBurnConfig`_-laajennukselle.
2. Luo mint account _rs`CreateAccount`_-komennolla, alusta
   _rs`PermissionedBurnConfig`_ ja alusta mint _rs`InitializeMint`_-komennolla.
3. Luo token account ja mintaa tokeneita.
4. Polta _rs`PermissionedBurnInstruction::BurnChecked`_-komennolla, jonka
   allekirjoittavat sekä token account -omistaja että polttovaltuutettu.
5. Poista luvallinen poltto valinnaisesti käytöstä _rs`SetAuthority`_-komennolla
   käyttäen _rs`AuthorityType::PermissionedBurn`_-komentoa ja
   `None`-valtuutusta, mikä palauttaa käyttöön tavalliset polttokäskyt. Alla
   olevat täydelliset koodiesimerkit näyttävät tämän vaiheen.

<ScrollyCoding>

## !!steps Laske tilin koko

Laske mint account -koko perusmintille sekä
...
Laske rent käyttäen mintille sekä _rs`PermissionedBurnConfig`_-laajennukselle
tarvittavaa kokoa.
...
Alusta _rs`PermissionedBurnConfig`_-laajennus mintissä polttovaltuutetulla.
...
Alusta mint _rs`InitializeMint`_-komennolla samassa transaktiossa.
...
## !!steps Luo token account ja lyö tokeneita liikkeelle

Luo token account maksajalle ja lyö sille tokeneita liikkeelle.
...
Polta tokeneita _rs`PermissionedBurnInstruction::BurnChecked`_-komennolla, jonka
allekirjoittavat sekä associated token account -omistaja että polttovaltuus.
...
<Callout type="info" title="Ohjeiden järjestys">
  _rs`PermissionedBurnInstruction::Initialize`_ täytyy tulla ennen
  ...
  sisällyttää samaan transaktioon.
</Callout>
...
| Kohde                                          | Kuvaus                                                                                                                                   | Lähde                                                                                                                                         |
| ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | Mint-laajennus, joka tallentaa valtuuden, jota tarvitaan jokaisen polton yhteissignointiin mintille.                                     | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
...
| _rs`PermissionedBurnInstruction::Burn`_        | Polttoohje, joka vaatii allekirjoitukset polttovaltuudelta sekä token account -omistajalta tai -valtuutetulta.                           | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | Polttoohje desimaalitarkistuksella, joka käyttää samoja tilejä kuin _rs`PermissionedBurnInstruction::Burn`_.                             | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | Valtuuden erottelija, jota käytetään _rs`SetAuthority`_-komennon kanssa polttovaltuuden kiertämiseen tai poistamiseen käytöstä mintillä. | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | Prosessorlogiikka, joka alustaa _rs`PermissionedBurnConfig`_ alustamattomalle mintille ja tallentaa polttovaltuuden.                     | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
| _rs`process_burn`_                             | Prosessorlogiikka, joka hylkää tavallisen polton, kun polttovaltuus on asetettu, ja tarkistaa valtuuden allekirjoituksen.                | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | Prosessorlogiikka, joka validoi ja kiertää polttovaltuuden, kun _rs`AuthorityType::PermissionedBurn`_ on käytössä.                       | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### TypeScript

Alla oleva `Kit` esimerkki käyttää generoituja ohjeita suoraan. Julkaistu
perinteinen `@solana/spl-token` paketti ei vielä tue käyttöoikeusrajoitettua
polttamista, joten perinteistä esimerkkiä ei ole sisällytetty.
