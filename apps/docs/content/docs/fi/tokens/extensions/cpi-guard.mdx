...
title: CPI Guard
description:
  Ota käyttöön CpiGuard-tilin laajennus estääksesi token-käskyt, jotka kutsutaan
  cross-program invocation -kutsujen kautta.
...
Token Extensions Program -ohjelman _rs`CpiGuard`_-tilin laajennus estää toista
ketjussa olevaa ohjelmaa käyttämästä token-tiliä tiettyihin token-käskyihin
Cross Program Invocation (CPI) -kutsun kautta.

Kun CPI guard on käytössä, Token Extensions Program hylkää nämä käskyt, kun
toinen ketjussa oleva ohjelma kutsuu niitä CPI:n kautta:

- _rs`Transfer`_, _rs`TransferChecked`_ ja _rs`TransferCheckedWithFee`_, kun
  token-tilin omistajaa käytetään käskyn valtuutena
- _rs`Burn`_ ja _rs`BurnChecked`_, kun token-tilin omistajaa käytetään käskyn
  valtuutena
- _rs`Approve`_ ja _rs`ApproveChecked`_
- _rs`CloseAccount`_, kun lamportit lähetettäisiin muualle kuin token-tilin
  omistajalle
- _rs`SetAuthority`_, kun lisätään tai korvataan sulkemisvaltuutus

Kun CPI guard on käytössä, nämä tapaukset ovat edelleen sallittuja:

- _rs`Transfer`_, _rs`TransferChecked`_, _rs`TransferCheckedWithFee`_,
  _rs`Burn`_ ja _rs`BurnChecked`_ CPI:n kautta, kun delegaatti tai pysyvä
  delegaatti allekirjoittaa token-tilin omistajan sijasta
- _rs`CloseAccount`_ CPI:n kautta, kun lamportit lähetetään token-tilin
  omistajalle
- _rs`SetAuthority`_ olemassa olevan sulkemisvaltuutuksen poistamiseksi
- _rs`Revoke`_

Kun token-tilin omistajaa käytetään käskyn valtuutena, Token Extensions Program
-käskyt voidaan edelleen käsitellä, jos käsky lisätään suoraan transaktioon sen
sijaan, että se kutsuttaisiin CPI:n kautta. Token-tilin omistajan vaihtaminen
_rs`SetAuthority`_-käskyllä pysyy estettynä, kun CPI guard on käytössä, myös
CPI:n ulkopuolella. _rs`EnableCpiGuard`_- ja _rs`DisableCpiGuard`_-käskyjä ei
myöskään voi kutsua CPI:n kautta.

## CPI Guardin käyttöönotto

Ota CPI guard käyttöön:

1. Luo ja alusta mint.
2. Laske token account -koon ja rent-määrän tarve token accountille, jossa on
   _rs`CpiGuard`_.
3. Luo ja alusta token account mintille.
4. Kutsu _rs`EnableCpiGuard`_ token accountilla.
5. Kutsu _rs`DisableCpiGuard`_ token accountilla salliaksesi nämä käskyt
   uudelleen CPI:n kautta.

<ScrollyCoding>

## !!steps Luo ja alusta mint

Luo ja alusta mint, jota token account käyttää.
...
_rs`CpiGuard`_-laajennuksella. Tätä kokoa käytetään
_rs`CreateAccount`_-käskyssä.
...
Laske rent käyttäen token accountin kokoa, joka tarvitaan
_rs`CpiGuard`_-laajennukselle.
...
## !!steps Luo ja alusta token account

Luo token account lasketulla tilalla ja lamporteilla, ja alusta se sitten
mintille.
...
## !!steps Ota CPI guard käyttöön

Ota _rs`CpiGuard`_ käyttöön token accountilla.
...
## !!steps Poista CPI guard käytöstä

Poista _rs`CpiGuard`_ käytöstä token accountilla.
...
| Kohde                              | Kuvaus                                                                                                                                                                                                                                  | Lähde                                                                                                                                 |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`CpiGuard`_                     | Tilin laajennus, joka tallentaa tiedon siitä, onko CPI-suojaus tällä hetkellä käytössä token accountissa.                                                                                                                               | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/mod.rs#L20-L25)         |
| _rs`CpiGuardInstruction::Enable`_  | Käsky, joka ottaa CPI-suojauksen käyttöön token accountissa.                                                                                                                                                                            | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/instruction.rs#L20-L38) |
| _rs`CpiGuardInstruction::Disable`_ | Käsky, joka poistaa CPI-suojauksen käytöstä token accountissa.                                                                                                                                                                          | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/instruction.rs#L39-L53) |
| _rs`process_toggle_cpi_guard`_     | Prosessorilogiikka, jota käyttävät _rs`Enable`_ ja _rs`Disable`_ ja joka validoi tilin omistajuuden, hylkää CPI-muutokset itse suojaukseen ja alustaa _rs`CpiGuard`_:n, jos sitä ei ole vielä olemassa, ennen `lock_cpi`:n asettamista. | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/cpi_guard/processor.rs#L19-L52)     |

### Typescript

Alla oleva `Kit`-esimerkki käyttää generoituja käskyjä suoraan. Vanhat esimerkit
käyttäen `@solana/web3.js`- ja `@solana/spl-token`-funktioita on sisällytetty
vertailua varten.
