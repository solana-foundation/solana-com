...
description:
  Opi siirtämään tokeneita yksityisesti yhdeltä token-tililtä toiselle.
---

## Kuinka siirtää tokeneita luottamuksellisesti yhdestä token accountista toiseen

Siirtääksesi tokeneita luottamuksellisesti yhdestä token accountista toiseen, sekä lähettäjällä että vastaanottajalla täytyy olla token accountit, jotka on konfiguroitu _rs`ConfidentialTransferAccount`_-tilaan ja hyväksytty käyttämään Confidential Balancesia. Lähettäjän token accountilla täytyy myös olla saatavilla oleva luottamuksellinen saldo siirtoa varten.

Siirtääksesi tokeneita luottamuksellisesti:

1. Luo
   [kolme todistetta](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)
   asiakaspuolella:
   ...
   **Yhtäläisyystodiste
   ...
   Varmistaa, että siirron jälkeinen uusi käytettävissä oleva saldosalausviesti
   vastaa sitä vastaavaa
   [Pedersen-sitoumusta](https://en.wikipedia.org/wiki/Commitment_scheme),
   varmistaen, että lähdetilin uusi käytettävissä oleva saldo lasketaan oikein
   muodossa `new_balance = current_balance - transfer_amount`.

   **Salaustekstin voimassaolotodiste
   ...
   Varmistaa, että siirtomäärän salaustekstit on asianmukaisesti luotu kaikille
   kolmelle osapuolelle (lähde, kohde ja tarkastaja), varmistaen, että
   siirtomäärä on oikein salattu kunkin osapuolen julkisella avaimella.
   ...
   **Aluetodiste
   ...
   Varmistaa, että uusi käytettävissä oleva saldo ja siirtomäärä (jaettuna
   matala/korkea-bitteihin) ovat kaikki ei-negatiivisia ja määritetyllä
   alueella.

2. Jokaista todistetta varten:
   - Kutsu ZK ElGamal -todistusohjelmaa todistetietojen vahvistamiseksi.
   - Tallenna todistuskohtainen metatietoja todisteen "kontekstitila"-tilille
...
3. Kutsu
   ...
   -ohjetta antaen todistekontekstitilit.

4. Sulje todistekontekstitilit palauttaaksesi niiden luomiseen käytetyn SOL:n.

Seuraava kaavio näyttää tokeneita lähettäjän token accountista vastaanottajan
token accountiin siirtämiseen liittyvät vaiheet.
...
- Luoda yhtäläisyystodiste, salaustekstin voimassaolotodiste ja aluetodiste
  asiakaspuolella
- Kutsua Zk ElGamal -todistusohjelmaa todisteiden vahvistamiseksi ja
  "kontekstitila"-tilien alustamiseksi
- Kutsua
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  -ohjetta antaen kolme todistetiliä.
- Sulkea kolme todistetiliä palauttaaksesi rent.

Alla oleva Rust-esimerkki luo todisteet
`spl-token-confidential-transfer-proof-generation`-craten avulla, vahvistaa
jokaisen kontekstitilatilille ZK ElGamal Proof -ohjelman kautta, viittaa kolmeen
tiliin siirtoinstruktiossa ja sulkee ne sen jälkeen. TypeScript-esimerkki
käyttää `getConfidentialTransferInstructionPlan`-apufunktiota paketista
`@solana-program/token-2022/confidential`, joka kokoaa samat todistettilit,
siirron ja sulkemiset monivaiheiseksi transaktioinstruktioksi.

## Esimerkkikoodi

Seuraava esimerkki siirtää tokeneita luottamuksellisesti yhdeltä tililtä toiselle. Molempien tilien täytyy olla jo konfiguroitu Confidential Balancesia varten, ja lähettäjällä täytyy olla saatavilla oleva luottamuksellinen saldo.

Confidential Balances on riippuvainen ZK ElGamal Proof -ohjelmasta, joka on käytössä mainnetissä ja devnetissä. Tavallinen `solana-test-validator` ei ota sitä käyttöön, mutta mainnet-haarukointia tukeva paikallinen validaattori, kuten [Surfpool](https://surfpool.run), ottaa sen käyttöön.
Aja esimerkki jompaakumpaa vasten (koodi käyttää devnetiä) rahoitetulla maksajalla ja korvaa paikanvaraimet mint-osoitteellasi sekä lähettäjän ja vastaanottajan tileillä.
