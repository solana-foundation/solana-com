...
<Callout type="warn">
  ...
  luottamuksellisten siirtojen laajennus ei ole tällä hetkellä käytettävissä.
  Vaikka konseptit ovat edelleen päteviä, koodiesimerkit eivät toimi.
</Callout>

## Miten siirtää tokeneita luottamuksellisesti yhdeltä token-tililtä toiselle

Siirtääksesi tokeneita luottamuksellisesti yhdeltä token-tililtä toiselle, sekä
lähettäjällä että vastaanottajalla täytyy olla token-tilit, jotka on konfiguoitu
_rs`ConfidentialTransferAccount`_ -tilalla ja hyväksytty luottamuksellisia
siirtoja varten. Lähettäjän token-tilillä täytyy myös olla käytettävissä oleva
luottamuksellinen saldo, josta siirtää.
...
1. Luo
   ...
   **Yhtäläisyystodiste
   ...
   Varmistaa, että uusi käytettävissä olevan saldon salausteksti siirron jälkeen
   ...
   **Salaustekstin pätevyystodiste
   ...
   Varmistaa, että siirtosumman salaustekstit on luotu oikein kaikille kolmelle
   osapuolelle (lähde, kohde ja tarkastaja), varmistaen että siirtosumma on
   salattu oikein kunkin osapuolen julkisella avaimella.
   ...
   **Arvoaluetodiste
   ...
   Varmistaa, että uusi käytettävissä oleva saldo ja siirtosumma (jaettuna
   ala-/yläbitteihin) ovat kaikki ei-negatiivisia ja määritetyllä arvoalueella.

2. Jokaiselle todisteelle:
   - Kutsu ZK ElGamal -todistusohjelmaa todistustietojen vahvistamiseksi.
   - Tallenna todistekohtaiset metatiedot todisteen "kontekstitila"-tilille
...
3. Kutsu
   ...
   -ohjetta, tarjoten todisteiden kontekstitilatilit.

4. Sulje proof context -tilit palauttaaksesi niiden luomiseen käytetyn SOL:n.

Seuraava kaavio näyttää vaiheet, jotka liittyvät tokenien siirtämiseen
lähettäjän token accountilta vastaanottajan token accountille.
...
- Luotava equality proof, ciphertext validity proof ja range proof
  asiakaspuolella
- Kutsuttava Zk ElGamal proof -ohjelmaa todentamaan proofit ja alustamaan
  "context state" -tilit
- Kutsuttava
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  -käskyä tarjoamalla kolme proof-tiliä.
- Suljettava kolme proof-tiliä palauttaaksesi rentin.

`spl_token_client` -crate tarjoaa seuraavat metodit:

- `confidential_transfer_create_context_state_account` -metodi, joka luo
  proof-tilin.
- `confidential_transfer_transfer` -metodi, joka kutsuu `Transfer` -käskyä.
- `confidential_transfer_close_context_state_account` -metodi, joka sulkee
  proof-tilin.

### Esimerkkikoodi

Seuraava esimerkki havainnollistaa, kuinka siirtää tokeneita luottamuksellisesti
token accountien välillä.

Esimerkin suorittamiseksi käynnistä paikallinen validaattori, jossa on Token
Extension Program kloonattu mainnetistä käyttämällä seuraavaa komentoa. Sinulla
on oltava Solana CLI asennettuna paikallisen validaattorin käynnistämiseksi.
...
<Callout type="info">
  Kirjoitushetkellä Confidential Transfers ei ole käytössä oletusarvoisessa
  paikallisessa validaattorissa. Sinun on kloonattava mainnetin Token Extension
  Program suorittaaksesi esimerkkikoodin.
</Callout>
