...
2. **Luottamuksellinen käytettävissä oleva saldo**: Odottava saldo
   "kohdistetaan" sitten käytettävissä olevaan saldoon, jolloin tokenit ovat
   käytettävissä luottamuksellisiin siirtoihin.

Tässä osiossa selitetään ensimmäinen vaihe: julkisen tokenisaldon tallettaminen
luottamukselliseen odottavaan saldoon.

Seuraava kaavio näyttää vaiheet, jotka liittyvät tokeneiden tallettamiseen
...
Seuraava esimerkki havainnollistaa, kuinka julkinen tokenisaldo talletetaan
luottamukselliseen odottavaan saldoon.

Confidential Balances käyttää ZK ElGamal Proof -ohjelmaa, joka on käytössä
mainnetissä ja devnetissä. Tavallinen `solana-test-validator` ei ota sitä käyttöön, mutta
mainnet-forkaava paikallinen validator kuten [Surfpool](https://surfpool.run) ottaa.
Aja esimerkki jotakin näistä vasten (koodi käyttää devnetiä) rahoitetulla maksajalla,
ja korvaa paikkamerkkeinä toimivat mint- ja tiliosoitteet omillasi.
