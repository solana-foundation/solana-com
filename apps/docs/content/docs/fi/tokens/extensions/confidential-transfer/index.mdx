...
<Callout type="warn">
  ...
  ovat edelleen päteviä, koodiesimerkit eivät toimi.
</Callout>

## Mitä ovat Confidential Transfer -siirrot?

Confidential transfer -siirrot mahdollistavat tokenien siirtämisen token
accountien välillä paljastamatta siirtosummaa. Tämä on hyödyllistä yksityisyyttä
...
- [Pikaopas](https://www.solana-program.com/docs/confidential-balances#setup) -
  Asennus ja peruskomentorivitoiminnot
- [Confidential Balances -keittokirja](https://github.com/solana-developers/Confidential-Balances-Sample) -
...
Token Extension -ohjelmaan, jotka mahdollistavat tokenien siirtämisen tilien
välillä paljastamatta siirtosummaa.
...
2. Luo token accountit confidential transfer -laajennuksella lähettäjälle ja
   vastaanottajalle.
...
5. **Käytä** lähettäjän odottava saldo **luottamukselliseen käytettävissä
   olevaan saldoon**.
6. **Siirrä** luottamuksellisesti tokeneita lähettäjän token accountista
   vastaanottajan token accountiin.
...
Lisätietoja luottamuksellisen siirron kulun vaiheista löydät vastaavista
sivuista:
...
<Cards>
  ...
  >
    Miten luodaan mint account luottamuksellisten siirtojen laajennuksella
  </Card>
  <Card
    title="Luo token account"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Miten konfiguroidaan token account luottamuksellisten siirtojen
    laajennuksella
  </Card>
  <Card
  ...
  >
    Miten talletetaan tokeneita luottamukselliseen odottavaan saldoon
  </Card>
  <Card
    title="Käytä odottavaa saldoa"
    href="/docs/tokens/extensions/confidential-transfer/apply-pending-balance"
  >
    Miten käytetään odottavaa saldoa käytettävissä olevaan luottamukselliseen
    saldoon
  </Card>
  <Card
  ...
  >
    Miten nostetaan tokeneita luottamuksellisesta käytettävissä olevasta
    saldosta
  </Card>
  <Card
  ...
  >
    Miten siirretään tokeneita luottamuksellisesti token accountien välillä
  </Card>
</Cards>

Alla oleva kaavio näyttää yksityiskohtaisen sekvenssin luottamuksellisten
...
Alla oleva Rust-esimerkki näyttää luottamuksellisten tokensiirtojen vaiheet,
jotka vastaavat edeltävää kaaviota. Vaiheet on erotettu apufunktioihin selkeyden
vuoksi.

Esimerkin suorittamiseksi käynnistä paikallinen validaattori, jossa Token
Extension Program on kloonattu mainnetistä käyttäen seuraavaa komentoa. Sinulla
on oltava Solana CLI asennettuna paikallisen validaattorin käyttämiseksi.
...
<Callout type="info">
  ...
  oletusarvoisessa paikallisessa validaattorissa. Sinun on kloonattava mainnetin
  Token Extension Program esimerkkikoodin suorittamiseksi.
</Callout>
...
Täydellinen lista luottamuksellisten siirtojen laajennuksen
...
| Ohje                                | Kuvaus                                                                                                                                                                      |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Asettaa mint accountin luottamuksellisia siirtoja varten. Tämä ohje on sisällytettävä samaan transaktioon kuin _rs`TokenInstruction::InitializeMint`_ -ohje.                |
| _rs`UpdateMint`_                    | Päivittää mintin luottamuksellisten siirtojen asetukset.                                                                                                                    |
| _rs`ConfigureAccount`_              | Asettaa token accountin luottamuksellisia siirtoja varten.                                                                                                                  |
...
| _rs`EmptyAccount`_                  | Tyhjentää odottavan ja käytettävissä olevan luottamuksellisen saldon, jotta token accountin sulkeminen on mahdollista.                                                      |
| _rs`Deposit`_                       | Muuntaa julkisen tokensaldon odottavaksi luottamukselliseksi saldoksi.                                                                                                      |
...
| _rs`EnableConfidentialCredits`_     | Sallii token accountin vastaanottaa luottamuksellisia tokensiirtoja.                                                                                                        |
| _rs`DisableConfidentialCredits`_    | Estää saapuvat luottamukselliset siirrot, mutta sallii edelleen julkiset siirrot.                                                                                           |
| _rs`EnableNonConfidentialCredits`_  | Sallii token accountin vastaanottaa julkisia tokensiirtoja.                                                                                                                 |
| _rs`DisableNonConfidentialCredits`_ | Estää tavalliset siirrot, jotta account vastaanottaa vain luottamuksellisia siirtoja.                                                                                       |
| _rs`TransferWithFee`_               | Siirtää tokeneita token accountien välillä luottamuksellisesti maksulla.                                                                                                    |
| _rs`ConfigureAccountWithRegistry`_  | Vaihtoehtoinen tapa konfiguroida token accounteja luottamuksellisia siirtoja varten käyttäen _rs`ElGamalRegistry`_ -accountia _rs`VerifyPubkeyValidity`_ -todisteen sijaan. |
