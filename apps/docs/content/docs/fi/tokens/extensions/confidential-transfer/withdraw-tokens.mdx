---
title: Tokenien nosto
description: Opi nostamaan tokeneita luottamuksellisesta tilasta.
---

## Kuinka nostaa tokeneita luottamuksellisesta käytettävissä olevasta saldosta

Tokeneiden nostaminen luottamuksellisesta käytettävissä olevasta saldosta
julkiseen saldoon:
...
1. Luo
   ...
   **Yhdenvertaisuustodiste
   ...
   Varmistaa, että noston jälkeen jäljellä oleva käytettävissä olevan saldon
   salateksti vastaa sitä vastaavaa
   [Pedersen-sitoumusta](https://en.wikipedia.org/wiki/Commitment_scheme),
   varmistaen, että tilin uusi käytettävissä oleva saldo lasketaan oikein
   muodossa `remaining_balance = current_balance - withdraw_amount`.
   ...
   **Alueentodiste
   ...
   Varmistaa, että noston jälkeen jäljellä oleva käytettävissä oleva saldo on
   ei-negatiivinen ja määritetyn alueen sisällä.

2. Kullekin todisteelle:
   - Kutsu ZK ElGamal -todisteohjelmaa todistetietojen verifioimiseksi.
   - Tallenna todiste-kohtainen metatiedot todisteen "kontekstitila"-tilille
...
3. Kutsu
   ...
   -ohjetta antaen kaksi todistustiliä.

4. Sulje todistustilit ja palauta niiden luomiseen käytetty SOL.

Seuraava kaavio näyttää vaiheet, jotka liittyvät tokeneiden nostamiseen
...
Tokeneiden nostamiseksi luottamuksellisesta käytettävissä olevasta saldosta
julkiseen saldoon sinun tulee:

- Luo yhdenvertaisuustodiste ja alueentodiste asiakaspuolella
- Kutsu Zk ElGamal -todisteohjelmaa todisteiden verifioimiseksi ja
  "kontekstitila"-tilien alustamiseksi
- Kutsu
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  -ohjetta antaen kaksi todistustiliä.
- Sulje kaksi todistustiliä rent-maksujen palauttamiseksi.

Alla oleva Rust-esimerkki luo todisteet
`spl-token-confidential-transfer-proof-generation`-paketilla, verifioi jokaisen
kontekstitila-tilille ZK ElGamal Proof -ohjelman kautta, viittaa molempiin
tileihin nostamisohjeessa ja sulkee ne sen jälkeen. TypeScript-esimerkki käyttää
`getConfidentialWithdrawInstructionPlan`-apufunktiota kohteesta
`@solana-program/token-2022/confidential`, joka kokoaa samat todistustilit,
noston ja sulkemiset monivaiheiseksi transaktio-ohjesuunnitelmaksi.

## Esimerkkikoodi

Seuraava esimerkki siirtää tokenit luottamuksellisesta käytettävissä olevasta saldosta
takaisin julkiseen saldoon. Tilin täytyy olla jo määritetty
luottamuksellisia saldoja varten ja siinä tulee olla käytettävissä oleva luottamuksellinen saldo.

Luottamukselliset saldot ovat riippuvaisia ZK ElGamal Proof -ohjelmasta, joka on käytössä
mainnetissä ja devnetissä. Vakio `solana-test-validator` ei ota sitä käyttöön, mutta
mainnet-haarukoiva paikallinen validator, kuten [Surfpool](https://surfpool.run), ottaa.
Aja esimerkki yhtä näistä vasten (koodi käyttää devnetiä) rahoitetulla maksajalla
ja korvaa paikkamerkit omalla mintillä ja token account.
...
fn main() -> Result<()> {
    ...
    // configured for Confidential Balances with an available confidential
