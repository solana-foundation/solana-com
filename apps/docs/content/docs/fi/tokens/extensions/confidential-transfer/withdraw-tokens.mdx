...
1. Luo
   ...
   **Yhtäläisyystodiste
   ...
   salausteksti vastaa sen vastaavaa
   ...
   **Arvoaluetodiste
   ...
   ei-negatiivinen ja määritetyllä arvoalueella.
...
3. Kutsu
   ...
   -ohjetta tarjoten kaksi todistustiliä.

4. Sulje todistustilit palauttaaksesi niiden luomiseen käytetyn SOL:n.
...
- Luoda yhtäläisyystodiste ja arvoaluetodiste asiakaspuolella
- Kutsua ZK ElGamal -todistusohjelmaa todisteiden vahvistamiseksi ja
  "kontekstitila"-tilien alustamiseksi
- Kutsua
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  -ohjetta tarjoten kaksi todistustiliä.
- Sulkea kaksi todistustiliä vuokran palauttamiseksi.

Crate `spl_token_client` tarjoaa seuraavat metodit:

- Metodi `confidential_transfer_create_context_state_account`, joka luo
  todistustilin.
- Metodi `confidential_transfer_withdraw`, joka kutsuu ohjetta `Withdraw`.
- Metodi `confidential_transfer_close_context_state_account`, joka sulkee
  todistustilin.

### Esimerkkikoodi

Seuraava esimerkki havainnollistaa, kuinka tokeneita nostetaan
luottamuksellisesta käytettävissä olevasta saldosta julkiseen saldoon.

Esimerkin suorittamiseksi käynnistä paikallinen validaattori, jossa on
mainnetistä kloonattu Token Extension Program, käyttämällä seuraavaa komentoa.
Sinulla on oltava Solana CLI asennettuna paikallisen validaattorin
käynnistämiseksi.
