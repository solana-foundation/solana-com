...
## Kuinka luoda mint Confidential Transfer -laajennuksella

Confidential Transfer -laajennus mahdollistaa yksityiset tokenisiirrot
lisäämällä ylimääräistä tilaa mint account -tilille. Tässä osiossa selitetään,
kuinka luoda token mint tällä laajennuksella käytössä.
...
_rs`ConfidentialTransferMint`_ sisältää kolme konfiguraatiokenttää:
...
- **auto_approve_new_accounts**: Kun asetettu arvoon true, käyttäjät voivat
  ...
  käytössä. Kun false, viranomaisen on hyväksyttävä jokainen uusi token account
  ennen kuin sitä voidaan käyttää luottamuksellisiin siirtoihin.

- **auditor_elgamal_pubkey**: Valinnainen tarkastaja, joka voi purkaa
  siirtomäärät luottamuksellisissa transaktioissa tarjoten
  vaatimustenmukaisuusmekanismin samalla säilyttäen yksityisyyden suurelta
  yleisöltä.

### Vaaditut käskyt

Mintin luominen Confidential Transfer -laajennuksella käytössä vaatii kolme
käskyä yhdessä transaktiossa:

1. **Luo Mint Account**: Kutsu System Program'n _rs`CreateAccount`_ -käskyä
   luodaksesi mint account -tilin.

2. **Alusta Confidential Transfer -laajennus**: Kutsu Token Extensions Program'n
   ...
   -käskyä määrittääksesi _rs`ConfidentialTransferMint`_ -tilan mintille.

3. **Alusta Mint**: Kutsu Token Extensions Program'n
   _rs`Instruction::InitializeMint`_ -käskyä alustaaksesi tavanomaisen
   mint-tilan.

Vaikka voisit kirjoittaa nämä käskyt manuaalisesti, `spl_token_client` -paketti
tarjoaa `create_mint` -metodin, joka rakentaa ja lähettää transaktion kaikilla
kolmella käskyllä yhdessä funktiokutsussa, kuten alla olevassa esimerkissä on
osoitettu.

## Esimerkkikoodi

Seuraava koodi havainnollistaa, kuinka luodaan mint Confidential Transfer
-laajennuksella.

Luottamukselliset siirrot ovat riippuvaisia ZK ElGamal Proof -ohjelmasta, joka
on käytössä mainnetissä ja devnetissä. Tavallinen `solana-test-validator` ei ota
sitä käyttöön, mutta mainnettia haaruttava paikallinen validator, kuten
[Surfpool](https://surfpool.run), ottaa. Aja esimerkki jompaankumpaan näistä
vasten (koodi käyttää devnetiä) rahoitetulla maksajalla ja korvaa paikkamerkin
mint- ja tiliosoitteet omillasi.
