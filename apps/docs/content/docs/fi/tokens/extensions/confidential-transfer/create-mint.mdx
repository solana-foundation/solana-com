...
description:
  Opi luomaan token mint, jossa Confidential Balances on käytössä.
---

## Kuinka luoda mint Confidential Balancesin kanssa

Confidential Balances mahdollistaa yksityiset tokensiirrot lisäämällä ylimääräistä tilaa
mint accountiin. Tässä osiossa selitetään, kuinka luodaan token mint tarvittava
laajennus käytössä.

Seuraava kaavio näyttää vaiheet, joita tarvitaan mintin luomiseen
Confidential Balances -laajennuksen kanssa:
...
### Confidential Balances -mintin tila
...
- **authority**: Tili, jolla on lupa muuttaa Confidential Balances
  -asetuksia mintille ja hyväksyä uudet luottamukselliset tilit, jos automaattinen hyväksyntä
  on poistettu käytöstä.

- **auto_approve_new_accounts**: Kun asetettu arvoon true, käyttäjät voivat luoda token
  accounteja, joissa Confidential Balances on oletuksena käytössä. Kun arvo on false, authority
  täytyy hyväksyä jokainen uusi token account ennen kuin sitä voidaan käyttää
...
Mintin luominen Confidential Balances käytössä vaatii kolme ohjetta
yhdessä transaktiossa:

1. **Luo Mint Account**: Kutsu System Program'n _rs`CreateAccount`_ -käskyä
   luodaksesi mint account -tilin.

2. **Alusta Confidential Balances**: Kutsu Token Extension Programin
   ...
   -ohjetta määrittääksesi _rs`ConfidentialTransferMint`_ -tilan
   mintille.
...
Seuraava koodi näyttää, kuinka luodaan mint Confidential Balances
käytössä.
...
mainnet-haarukoiva paikallinen validator, kuten [Surfpool](https://surfpool.run), tekee niin.
Aja esimerkki jotain näistä vasten (koodi käyttää devnetiä) rahoitetulla maksajalla
ja korvaa paikkamerkkinä toimivat mint- ja tiliosoitteet omillasi.
