...
title: Sync Native
description:
  Päivitä käärityn SOL-token accountin määrä SOL:n siirtämisen jälkeen tilille.
...
## Mikä on kääritty SOL?

Kääritty SOL (WSOL) on SOL:ia, jota säilytetään token accountissa Token
Programin natiivimintille. Natiivimint on mint-osoite, jota Token Program
käyttää token accounteille, jotka käärivät SOL:ia. Natiivimint mahdollistaa
SOL:in käytön Token Programin käskyjen kautta.

| Ohjelma                  | Natiivimint-osoite                             |
| ------------------------ | ---------------------------------------------- |
...
Kääritty SOL-token account tallentaa SOL:in tilin `lamports`-kenttään, mutta
seuraa käärityssä SOL-token accountissa olevaa SOL:ia token-saldona tilin
_rs`amount`_-kentän kautta. Jos siirrät SOL:ia käärittyyn SOL-token accountiin,
lamport-saldo kasvaa välittömästi, mutta token-kentän _rs`amount`_ arvo ei
muutu, ennen kuin kutsut Token Programin _rs`SyncNative`_-käskyä.

## Kuinka synkronoida natiivi SOL

Käytä Token Programin _rs`SyncNative`_-käskyä SOL:in siirtämisen jälkeen
käärittyyn SOL-token accountiin. _rs`SyncNative`_-käsky päivittää token
accountin _rs`amount`_-kentän vastaamaan tilillä olevien lamporttien määrää sen
vuokra-vapaan vähimmäismäärän yläpuolella.

### Lähdeviite

| Kohde                     | Kuvaus                                                                                                                                                                                                           | Token Program                                                                                                 | Token Extensions Program                                                                                            |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_             | Token accountin tila käyttää _rs`is_native`_-arvoa merkitsemään tilin WSOL:iksi ja tallentamaan natiivivarannon, vuokra-vapaan vähimmäis lamport-saldon, joka tilin on säilytettävä, sekä seuratun token-määrän. | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`SyncNative`_          | Käsky, joka päivittää käärityn SOL-token accountin _rs`amount`_-kentän vastaamaan tilillä olevien lamporttien määrää sen vuokra-vapaan vähimmäismäärän yläpuolella.                                              | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| _rs`process_sync_native`_ | Jaettu prosessorilogiikka käärittyjen SOL-saldojen synkronointiin.                                                                                                                                               | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |
