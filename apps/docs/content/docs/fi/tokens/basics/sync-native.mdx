...
Kääritty SOL (WSOL) on token account, joka seuraa tilillä olevien lamportsien
määrää token-saldona. WSOL mahdollistaa integraation DeFi-protokollien kanssa
SOL:n siirtämiseksi SPL-tokenina.

[`SyncNative`](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/interface/src/instruction.rs#L387)-käsky
synkronoi wrapped SOL (WSOL) -token accountin saldon sen sisältämän todellisen
SOL:n (lamporttien) kanssa. Kun siirrät natiivi SOL:ia WSOL-token accountille,
token-saldo ei päivity automaattisesti. Sinun on kutsuttava SyncNative-käskyä,
jotta oikea WSOL-saldo näkyy.
