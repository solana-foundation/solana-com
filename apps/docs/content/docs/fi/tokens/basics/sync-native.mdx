---
title: Sync Native
description: Päivitä käärityn SOL-token-tilin määrä siirrettyäsi SOL:ia tilille.
...
Kääritty SOL (WSOL) on SOL:ia, jota säilytetään token-tilillä Token Programin
native-mintille. Native-mint on mint-osoite, jota Token Program käyttää SOL:ia
kääriville token-tileille. Native-mint mahdollistaa SOL:in käytön Token
Programin ohjeiden kautta.

| Ohjelma                  | Native mint -osoite                            |
| ------------------------ | ---------------------------------------------- |
...
Kääritty SOL token-tili tallentaa SOL:in tilin `lamports`-kenttään, mutta seuraa
käärityllä SOL token-tilillä olevaa SOL:ia token-saldona tilin `amount`-kentän
kautta. Jos siirrät SOL:ia käärittyyn SOL token-tiliin, lamport-saldo kasvaa
välittömästi, mutta token `amount`-kenttä ei muutu ennen kuin kutsut Token
Programin `SyncNative`-ohjetta.

## Kuinka synkronoida natiivi SOL

Käytä Token Programin `SyncNative`-ohjetta siirrettyäsi SOL:ia käärittyyn SOL
token-tiliin. `SyncNative`-ohje päivittää token-tilin `amount`-kentän vastaamaan
tilillä olevia lamportteja yli sen vuokrasta vapautetun varauksen.

### Lähdeviittaus

| Kohde                 | Kuvaus                                                                                                                                                                                                        | Token Program                                                                                                 | Token Extensions Program                                                                                            |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `Account`             | Token-tilin tila käyttää `is_native`-arvoa merkitäkseen tilin WSOL:iksi ja tallentaa natiivivarauksen, vuokrasta vapautetun vähimmäislamport-saldon, jonka tilin on säilytettävä, sekä seuratun token-määrän. | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| `SyncNative`          | Ohje, joka päivittää käärityn SOL token-tilin `amount`-kentän vastaamaan tilillä olevia lamportteja yli sen vuokrasta vapautetun varauksen.                                                                   | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| `process_sync_native` | Jaettu prosessorilogiikka käärittyjen SOL-saldojen synkronointiin.                                                                                                                                            | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |
