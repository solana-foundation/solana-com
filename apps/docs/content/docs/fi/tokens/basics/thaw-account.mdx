...
title: Sulauta tili
description:
  Pura jäädytetty token account, jotta siirrot, polttamiset ja
  delegaattimuutokset voivat jatkua.
...
## Mitä token accountin purkaminen tekee?

Purkaminen muuttaa token accountin jäädytetystä tilasta takaisin alustettuun
tilaan.

Onnistuneen purkamisen jälkeen tili voi jälleen siirtää, polttaa ja sulkea
tokeneita. Vain mintin jäädytysvaltuutus voi purkaa token accounteja.

## Kuinka purkaa token account

Token accountin purkaminen käyttää Token Programin _rs`ThawAccount`_ -käskyä.
