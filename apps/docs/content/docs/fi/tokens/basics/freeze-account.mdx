...
description:
  Jäädytä token-tili mintin jäädytysvaltuudella estääksesi siirrot, polttamiset
  ja delegaattimuutokset.
...
Vain mintti, jolla on edelleen jäädytysvaltuus, voi jäädyttää token-tilejä. Jos
mintin jäädytysvaltuus peruutetaan, kyseisen mintin tilejä ei voi enää
jäädyttää.

## Kuinka jäädyttää token-tili

Token-tilin jäädyttäminen käyttää Token Programin _rs`FreezeAccount`_-käskyä.

_rs`FreezeAccount`_-käsky varmistaa, että token-tili kuuluu minttiin ja että
allekirjoittaja on mintin jäädytysvaltuus, minkä jälkeen se muuttaa token-tilin
tilan alustetusta jäädytetyksi.
