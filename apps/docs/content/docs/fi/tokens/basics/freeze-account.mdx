...
[`FreezeAccount`](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L228)-ohje
estää kaikki token-siirrot ja token-polttamiset tietystä token-tilistä.
Jäädytyksen jälkeen tili ei voi lähettää tai vastaanottaa tokeneita, ennen kuin
se on sulatettu. Jäädytetty tili voidaan silti sulkea, jos sen token-saldo on
nolla.

Vain token-mint accountin jäädytysvaltuuttaja voi jäädyttää tilejä. Jos
jäädytysvaltuutus peruutetaan (asetetaan nollaksi) mint accountissa, tokeneita
ei voida koskaan jäädyttää.
