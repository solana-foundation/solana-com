---
title: Tilin jäädyttäminen
description:
  Jäädytä token-tili mintin jäädytysvaltuutuksella estääksesi siirrot,
  polttamisen ja valtuutettujen muutokset.
...
## Mitä token-tilin jäädyttäminen tekee?

Token accountin jäädyttäminen säilyttää saman omistajan, mintin ja saldon, mutta
estää token accountia vastaanottamasta, siirtämästä tai polttamasta tokeneita,
kunnes token account vapautetaan.

Jäädytetty tili voidaan silti sulkea, jos jäädytetyllä tilillä on nolla
token-saldo.

Vain mintti, jolla on vielä jäädytysvaltuutus, voi jäädyttää token-tilejä. Jos
mintin jäädytysvaltuutus peruutetaan, kyseisen mintin tilejä ei voida enää
jäädyttää.

## Miten token-tili jäädytetään

Token-tilin jäädyttäminen käyttää Token Program -ohjelman
_rs`FreezeAccount`_-käskyä.

_rs`FreezeAccount`_-käsky varmistaa, että token-tili kuuluu minttiin ja että
allekirjoittaja on mintin jäädytysvaltuutus, minkä jälkeen token-tilin tila
muutetaan alustetusta jäädytetyksi.

[Natiivit token-tilit](/docs/tokens/basics/sync-native) eivät tue jäädyttämistä.

### Lähdeviitteet

| Kohde                               | Kuvaus                                                                                              | Token Program                                                                                                 | Token Extensions Program                                                                                            |
| ----------------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Mintin tila tallentaa valinnaisen jäädytysvaltuutuksen, jota käytetään token-tilien jäädyttämiseen. | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Token-tilin tilat, mukaan lukien _rs`Initialized`_ ja _rs`Frozen`_.                                 | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | Käsky, joka muuttaa token-tilin jäädytettyyn tilaan.                                                | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
| _rs`process_toggle_freeze_account`_ | Jaettu prosessointilogiikka token-tilien jäädyttämiseen ja sulattamiseen.                           | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |
