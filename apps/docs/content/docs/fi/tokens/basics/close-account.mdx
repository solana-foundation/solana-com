...
## Miten token-tili suljetaan

Token-tilin sulkeminen käyttää Token Program -ohjelman
_rs`CloseAccount`_-käskyä.
...
`@solana/kit`:tä. Vanhat esimerkit käyttäen `@solana/web3.js`:tä on sisällytetty
viitteeksi.
