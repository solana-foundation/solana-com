...
description:
  Sulje token-tili ja palauta tilin lamportit kohdetilille.
...
## Mitä token-tilin sulkeminen tekee?

Token account -tilin sulkeminen poistaa tilin ja siirtää kaikki sen lamportit,
mukaan lukien palautettavan vähimmäissaldovarauksen, kohdетilille.

Token-saldon on oltava nolla ennen kuin tili voidaan sulkea. Jäädytetty tili
voidaan silti sulkea, jos jäädytetyn tilin token-saldo on nolla. Wrapped SOL
token account -tilit ovat poikkeus ja ne voidaan sulkea token-saldon kanssa
taustalla olevan SOL:n takaisin saamiseksi. Tilin omistaja tai tilin sulkemisvaltuutettu
allekirjoittaa sulkemisohjeen ja valitsee kohteen. Tili, joka maksoi
ATA:n luomisesta, ei automaattisesti saa takaisin perittävää saldoa.

## Kuinka sulkea token-tili

Token-tilin sulkeminen käyttää Token Programin _rs`CloseAccount`_-ohjetta.

_rs`CloseAccount`_-ohje siirtää lähdetilin lamportit kohdetilille, tyhjentää
lähdetilin ja poistaa lähdetilin. Token Extensions Program soveltaa
lisäsulkemistarkistuksia tiettyihin laajennuksiin, mutta perussulkemisprosessi
on sama.
...
| --------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | Token-tilin tila tallentaa saldon, natiivin tilan ja sulkemisvaltuutuksen, joita käytetään tilin sulkemisen aikana.     | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | Ohje, joka sulkee token-tilin ja siirtää token-tilin lamportit kohdetilille. | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | Jaettu prosessorilogiikka token-tilin sulkemiselle.                                                               | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |
