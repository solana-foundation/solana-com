...
description:
  Luo token-mintti ja määritä mintin desimaalit, minttausvaltuutus ja
  jäädytysvaltuutus.
...
Mint account määrittelee ja yksilöi ainutlaatuisesti tokenin Solanassa ja
tallentaa jaetun tilan, joka koskee kaikkia kyseisen mintin token accounteja.

Token Program määrittelee _rs`Mint`_ account-tyypin seuraavasti:
...
Jokaisella tokenilla on yksi mint account, ja mintin osoite on tokenin
yksilöllinen tunniste lompakoissa, sovelluksissa ja selaimissa.

Esimerkiksi USD Coinilla (USDC) on minttiosoite
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`. Minttiosoite yksilöi USDC:n
ainutlaatuisesti koko Solana-ekosysteemissä. Voit tarkastella tätä minttiä
...
Mintin luominen vaatii kaksi käskyä:

1. System Program -ohjelman _rs`CreateAccount`_ -käsky luo uuden rent-vapautetun
   tilin ja määrittää Token Program -ohjelman uuden tilin omistajaohjelmaksi.
2. Token Program -ohjelman _rs`InitializeMint`_ tai _rs`InitializeMint2`_ -käsky
   alustaa uuden tilin mintiksi.

Sisällytä _rs`CreateAccount`_ -käsky ja mintin alustuskäsky samaan transaktioon.

Mintin alustuksen aikana Token Program tarkistaa, ettei minttiä ole jo alustettu
ja että se on rent-vapautettu. Token Program kirjoittaa sitten
minttausvaltuutuksen, jäädytysvaltuutuksen, desimaalit ja _rs`is_initialized`_
-lipun mint account -tietoihin.

### Lähdeviitteet

| Kohde                          | Kuvaus                                                                         | Token Program                                                                                                 | Token Extensions Program                                                                                            |
| ------------------------------ | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                     | Perus-minttitila, joka tallennetaan jokaiseen mint accountiin.                 | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`InitializeMint`_           | Mintin alustuskäsky, joka odottaa rent sysvar -tilin olevan tililuettelossaan. | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L24-L44)   | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L50-L74)   |
| _rs`InitializeMint2`_          | Mintin alustuskäsky, joka ei vaadi rent sysvar -tiliä tililuettelossaan.       | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L401-L414) | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L471-L486) |
| _rs`_process_initialize_mint`_ | Jaettu prosessorilogiikka mintin alustukselle.                                 | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
| _rs`process_initialize_mint`_  | Julkinen käsittelijä _rs`InitializeMint`_ -käskylle.                           | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L62-L70)       | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L132-L140)     |
| _rs`process_initialize_mint2`_ | Julkinen käsittelijä _rs`InitializeMint2`_ -käskylle.                          | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L72-L81)       | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L142-L151)     |

### Typescript

Alla olevat `Kit`-esimerkit näyttävät suositellun lähestymistavan käyttäen
`@solana/kit`. Vanhat esimerkit, jotka käyttävät `@solana/web3.js`, on
...
## Kuinka Lisätä Metatietoja

Mint account tallentaa vain perus-token-tietoja (tarjonta, desimaalit,
valtuutukset). Jotta voit lisätä tokenillesi ihmisluettavia metatietoja, kuten
nimen, symbolin ja kuvan, sinulla on kaksi vaihtoehtoa:

<Cards>
  <Card title="Metaplex Token Metadata" href="/docs/tokens/metaplex">
   Lisää metatiedot käyttäen Metaplex Token Metadata Program -ohjelmaa. Toimii sekä alkuperäisen Token Program -ohjelman että Token-2022:n kanssa.
  </Card>

  <Card title="Token Extensions Metadata" href="/docs/tokens/extensions/metadata">
   Käytä sisäänrakennettua metadata-laajennusta Token-2022:n kanssa. 
   Metadata-laajennus on käytettävissä vain Token-2022:lla luoduille mint account -tileille.
   Jotta voit lisätä metatietoja alkuperäisellä Token Program -ohjelmalla luoduille mint account -tileille, käytä Metaplex Token Metadata Program -ohjelmaa.
  </Card>
</Cards>
