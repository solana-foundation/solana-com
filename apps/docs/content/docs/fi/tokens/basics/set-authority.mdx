...
description:
  Muuta tai peruuta valtuutusrooleja mint-tilillä tai token-tilillä, mukaan
...
Valtuutuksen asettaminen muuttaa tai peruuttaa tiettyjä valtuutusrooleja
mint-tilillä tai token-tilillä.

Mint-tilit voivat tallentaa mint-valtuutuksen ja freeze-valtuutuksen.
Token-tilit tallentavat omistajan ja voivat valinnaisesti tallentaa
close-valtuutuksen. Jokainen rooli hallitsee tiettyä käyttöoikeusryhmää, ja
valtuutuksen asettaminen arvoon _rs`None`_ poistaa valitun valtuutusroolin
pysyvästi.
...
Valtuutuksen muuttaminen käyttää Token Program -ohjelman
_rs`SetAuthority`_-käskyä.

Jokainen kutsu päivittää yhden valtuutustyypin yhdellä mint- tai token-tilillä.
Nykyinen valtuutus allekirjoittaa muutoksen, ja uusi valtuutus voi olla toinen
osoite tai _rs`None`_ valitun valtuutuksen pysyvää peruuttamista varten.
...
| _rs`Account`_               | Token-tilin tila tallentaa _rs`owner`_- ja _rs`close_authority`_-arvot.       | [Lähde](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Lähde](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)          |
