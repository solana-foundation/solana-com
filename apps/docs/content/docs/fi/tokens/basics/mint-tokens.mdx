---
title: Tokenien minttaus
description: Opi minttaamaan uusia tokenien yksiköitä.
---

## Tokenien minttaus

Minttaus luo uusia tokenien yksiköitä käyttäen
...
- Vain mint authority voi minttata uusia tokeneita
- Kohde-token accountin on oltava olemassa mintattujen tokenien
  vastaanottamiseksi
...
async fn main() -> Result<()> {
    // Luo yhteys paikalliseen validatoriin
    ...
    // Generoi uusi keypair maksajalle
    let fee_payer = Keypair::new();

    // Airdrop 1 SOL maksajalle
    ...
    // Generoi keypair käytettäväksi mintin osoitteena
    let mint = Keypair::new();

    // Hae oletusarvoinen mint accountin koko (tavuina), ei laajennuksia käytössä
    let mint_space = Mint::LEN;
    let mint_rent = client
        .get_minimum_balance_for_rent_exemption(mint_space)
        .await?;

    // Ohje uuden accountin luomiseksi mintille (token program)
    let create_account_instruction = create_account(
        &fee_payer.pubkey(), // maksaja
        &mint.pubkey(),      // uusi account (mint)
        mint_rent,           // lamportit
        mint_space as u64,   // tila
        &token_program_id(), // ohjelmatunnus
    );

    // Ohje mint accountin datan alustamiseksi
    ...
    // Laske associated token account -osoite fee_payerille
    ...
    // Lähetä ja vahvista transaktio
    client.send_and_confirm_transaction(&transaction).await?;

    // Mintattavien tokenien määrä (100 tokenia 2 desimaalilla)
    let amount = 100;

    // Luo mint_to-ohje tokenien mintaamiseksi associated token accountiin
    let mint_to_instruction = mint_to(
        &token_program_id(),
        &mint.pubkey(),            // mint
        &associated_token_address, // kohde
        &fee_payer.pubkey(),       // valtuutus
        &[&fee_payer.pubkey()],    // allekirjoittaja
        amount,                    // määrä
    )?;

    // Luo transaktio tokenien mintaamiseksi
    let transaction = Transaction::new_signed_with_payer(
        &[mint_to_instruction],
        Some(&fee_payer.pubkey()),
        &[&fee_payer],
    ...
    println!("Mintattu 1.00 tokenia associated token accountiin");
    println!("\nMintin osoite: {}", mint.pubkey());
    println!("{:#?}", mint_data);

    println!(
        "\nAssociated token accountin osoite: {}",
        associated_token_address
    );
    println!("{:#?}", token_data);

    println!("Transaktioallekirjoitus: {}", transaction_signature);
...
async def main():
    ...
    # Esimerkkiavainparit ja -osoitteet
    payer = Keypair()
    ...
    # Lyötävä määrä (pienimmässä yksikössä)
    amount_to_mint = 1000000000  # 1 token 9 desimaalilla
    ...
    async with rpc:
        ...
        print(f"Kohde: {destination_token_account}")
        print(f"Määrä: {amount_to_mint}")
        print(f"Mint-valtuutus: {mint_authority.pubkey()}")
        print(f"Maksaja: {payer.pubkey()}")
        print(f"Mint-transaktio luotu onnistuneesti")
