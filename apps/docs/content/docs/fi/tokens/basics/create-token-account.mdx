...
description:
  Luo token-tilejä ja assosioituja token-tilejä tietylle mintille ja
  omistajalle.
...
Tässä _rs`owner`_ tarkoittaa auktoriteettia, joka voi siirtää, polttaa tai
delegoida tokeneita token-tililtä. Tilin program owner on edelleen Token Program
tai Token Extensions Program.

Jokainen token-tili on sidottu täsmälleen yhteen minttiin, mikä tarkoittaa, että
token-tili voi sisältää vain yhden tokenin yksiköitä – sen tokenin, joka on
tunnistettu token-tilin _rs`mint`_-kentässä.

## Mikä on assosioitu token-tili

Assosioitu token-tili (ATA) on lompakon ja mintin oletusarvoinen token-tili.
...
Vain Associated Token Programin luomia token-tilejä kutsutaan assosioituksiksi
token-tileiksi.

Associated Token Program on tapa luoda token-tili standardoidussa,
deterministisessä osoitteessa. Tuloksena oleva tili on silti token-tili, jonka
omistaa Token Program tai Token Extensions Program, ei Associated Token Program.

Associated Token Program johtaa ATA-osoitteen kuten näytetään tiedostossa
...
Mille tahansa lompakon, token-ohjelman ja mintin yhdistelmälle on olemassa
täsmälleen yksi ATA-osoite. Associated Token Program luo standardin token-tilin
kyseiseen osoitteeseen, ja tuloksena oleva tili käyttää edelleen Token Programin
tai Token Extensions Programin määrittelemää _rs`Account`_-tyyppiä.

## Liitetyn tokentilin luominen

Liitetyn tokentilin luominen käyttää Associated Token Program -ohjelman
_rs`Create`_- tai _rs`CreateIdempotent`_-käskyä. Associated Token Program
johdattaa ATA-osoitteen, luo tilin ATA-osoitteeseen ja alustaa tilin
tokentiliksi, jonka omistaa Token Program tai Token Extensions Program.

<Callout type="info" title="Suositeltu">
  Useimmissa sovelluksissa tokentilit kannattaa luoda Associated Token Program
  -ohjelman kautta sen sijaan, että ne luotaisiin kutsumalla suoraan System
  Program- ja Token Program -käskyjä. Liitetty tokentili käyttää deterministista
  osoitetta, joka johdetaan omistajasta, token-ohjelmasta ja mintistä, mikä
  tekee tietyn mintin oletustokentilin helpommin löydettäväksi lompakoille ja
  sovelluksille.
</Callout>
...
| Kohde                                         | Kuvaus                                                                                                                                                                                                                                                         | Lähde                                                                                                                          |
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| _rs`Create`_                                  | Luo liitetyn tokentilin johdettuun ATA-osoitteeseen.                                                                                                                                                                                                           | [Lähde](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L20-L29) |
| _rs`CreateIdempotent`_                        | Luo liitetyn tokentilin, mutta onnistuu silti, jos kyseinen ATA on jo olemassa samalle omistajalle ja mintille.                                                                                                                                                | [Lähde](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | Johdattaa ATA-osoitteen ja käyttää _rs`create_pda_account`_-funktiota sekä CPI-kutsuja valittuun token-ohjelmaan tokentilin alustamiseksi. Kun valittu ohjelma on Token Extensions Program, ATA-prosessori alustaa myös muuttumattoman omistajan laajennuksen. | [Lähde](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | _rs`process_create_associated_token_account`_-funktion käyttämä apuväline PDA-tilin luomiseen CPI-kutsulla System Program -ohjelmaan.                                                                                                                          | [Lähde](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |

### Typescript

Alla olevat `Kit`-esimerkit osoittavat suositellun lähestymistavan käyttäen
...
Jokaiselle lompakolle ja token-yhdistelmälle on täsmälleen yksi ATA-osoite. Tämä
poistaa tarpeen seurata token account -osoitteita manuaalisesti – voit aina
johtaa oikean osoitteen.

<Callout type="info">
  **Kuinka Associated Token Program toimii**

Associated Token Program on apuohjelma, joka:

- Luo token accounteja deterministisiin osoitteisiin (PDA:t)
- Tekee Cross-Program Invocationeja (CPI:t) Token Programiin
- Ei ylläpidä mitään tilaa itse

Tuloksena olevat token accountit ovat Token Programin omistuksessa ja käyttävät
standardia `Account` -rakennetta.

</Callout>

## Kuinka luoda token account
...
alustamisen Cross Program Invocationien (CPI) kautta.
