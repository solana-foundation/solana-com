...
<Callout type="info">
  Menjalankan tanpa antarmuka — di rute API, worker, atau skrip tanpa dompet
  browser? Tanda tangani dengan layanan manajemen kunci backend alih-alih
  keypair mentah. Lihat [Keychain](/docs/tools/keychain) dan [Penandatanganan di
  Produksi](/docs/core/transactions/signing-in-production).
</Callout>

## Pola umum untuk pengembang Solana

- **Mesin status tanpa antarmuka**: Jalankan klien di dalam rute API, skrip,
  atau worker untuk menggunakan kembali logika orkestrasi dompet + RPC yang sama
  yang menggerakkan UI Anda.
- **Dasbor realtime**: Gabungkan watcher (saldo, akun, tanda tangan) dengan
  library UI pilihan Anda; klien menangani fan-out websocket dan invalidasi
  cache.
- **Store kustom**: Masukkan store Zustand Anda sendiri untuk menghidrasi dari
  IndexedDB/localStorage, mencerminkan status ke sesi server, atau berkoordinasi
  antar tab browser.
- **Jembatan ke React hooks**: Teruskan instance klien yang telah dikonfigurasi
  ke `@solana/react-hooks` saat Anda menginginkan hooks yang ergonomis di atas
  runtime yang sama.
- **Kemampuan pengujian**: Antarmuka klien tunggal dapat di-mock dalam unit
  test, sehingga mudah untuk mensimulasikan respons RPC atau sesi dompet tanpa
  dompet browser.
