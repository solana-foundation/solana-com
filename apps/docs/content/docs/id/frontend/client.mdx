...
<Callout type="info">
  Menjalankan headless — di rute API, worker, atau skrip tanpa wallet browser?
  Tandatangani dengan layanan manajemen kunci backend alih-alih keypair mentah.
  Lihat [Keychain](/docs/tools/keychain) dan [Penandatanganan dalam
  Produksi](/docs/core/transactions/signing-in-production).
</Callout>

## Pola umum untuk pengembang Solana

- **State machine headless**: Jalankan klien di dalam rute API, skrip, atau
...
- **Dasbor realtime**: Gabungkan watcher (saldo, akun, tanda tangan) dengan
  pustaka UI pilihan Anda; klien menangani fan-out websocket dan invalidasi
  cache.
- **Store kustom**: Injeksikan store Zustand Anda sendiri untuk menghidrasi dari
  IndexedDB/localStorage, mencerminkan state ke sesi server, atau berkoordinasi
  antar tab browser.
- **Jembatan ke React hooks**: Teruskan instance klien yang telah dikonfigurasi
  ...
  runtime yang sama.
- **Kemampuan pengujian**: Antarmuka klien tunggal dapat di-mock dalam pengujian
  unit, sehingga mudah untuk mensimulasikan respons RPC atau sesi wallet tanpa
  wallet browser.
