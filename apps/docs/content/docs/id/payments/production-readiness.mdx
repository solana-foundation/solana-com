...
Untuk penandatanganan backend produksi, pertimbangkan menggunakan
...
kunci melalui antarmuka tunggal: Memory, Vault, AWS KMS, Privy, Turnkey,
Fireblocks, CDP, Dfns, Para. Ini memungkinkan Anda mengembangkan secara lokal
dengan kunci in-memory, kemudian beralih ke backend tingkat produksi tanpa
...
Lihat
[panduan Menambahkan Penanda Tangan](https://github.com/solana-foundation/solana-keychain/blob/main/docs/ADDING_SIGNERS.md)
untuk mengintegrasikan layanan manajemen kunci tambahan.

### Keamanan RPC

Perlakukan endpoint RPC seperti API key—jangan tampilkan di kode frontend dimana
mereka dapat diekstrak dan disalahgunakan. Gunakan proxy backend atau variabel
environment yang tidak dibundel ke dalam kode klien.
...
**Konfigurasi berbasis environment:** Devnet dan Mainnet seringkali menggunakan
token mint yang sama sekali berbeda. Siapkan konfigurasi aplikasi Anda untuk
memuat alamat yang tepat per environment—jangan hardcode alamat mainnet dan lupa
menggantinya saat pengujian, atau lebih buruk lagi, mengirim alamat devnet ke
produksi.
...
gagal—atau lebih buruk lagi, mengakibatkan kehilangan dana yang tidak dapat
...
- [ ] SOL Mainnet telah diperoleh untuk biaya dan rent
- [ ] RPC produksi telah dikonfigurasi (bukan endpoint publik)
- [ ] Endpoint RPC cadangan telah dikonfigurasi
- [ ] Priority fees telah diimplementasikan dengan harga dinamis
- [ ] Logika retry menangani kedaluwarsa blockhash
- [ ] Level konfirmasi sesuai untuk kasus penggunaan
- [ ] Semua kesalahan umum ditangani dengan baik
- [ ] Gasless telah dikonfigurasi (jika berlaku)
- [ ] Alamat token Mainnet telah diverifikasi (bukan mint devnet)
- [ ] Semua kunci telah dicadangkan dengan aman
- [ ] Manajemen kunci telah ditinjau (tidak ada kunci di frontend)
- [ ] Pemantauan dan peringatan transaksi aktif
- [ ] Load testing telah dilakukan pada volume yang diharapkan
...
- **Inisialisasi akun:** Jika program Anda memerlukan akun admin, PDA, atau akun
  ...
  Token Account (ATA) yang diperlukan program Anda.

### Proses Deployment

- **Akun buffer:** Program berukuran besar di-deploy melalui akun buffer.
  Perintah `solana program deploy` menangani ini secara otomatis, tetapi pahami
  bahwa deployment tidak bersifat atomic—jika terganggu, Anda mungkin perlu
  memulihkan atau menutup akun buffer. Lihat
  [Deploying Programs](https://solana.com/docs/programs/deploying).
- **Otoritas upgrade:** Tentukan apakah program Anda harus dapat di-upgrade
  setelah peluncuran. Untuk immutabilitas, cabut otoritas upgrade setelah
  deployment. Untuk fleksibilitas, amankan kunci otoritas upgrade dengan tepat.
- **Rent:** Pastikan dompet deployment Anda memiliki cukup SOL untuk menutupi
  minimum rent-exempt untuk semua akun program.
- **Verifikasi:** [Verifikasi](https://solana.com/docs/programs/verified-builds)
  program Anda untuk memastikan bahwa program executable yang Anda deploy ke
  jaringan Solana sesuai dengan kode sumber di repositori Anda
