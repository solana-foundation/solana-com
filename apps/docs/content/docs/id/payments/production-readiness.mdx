...
Untuk penandatanganan backend di produksi, gunakan
[Keychain](/docs/tools/keychain)—sebuah library penandatanganan terpadu yang
...
Openfort, dan Utila. Ini memungkinkan Anda mengembangkan secara lokal dengan
kunci di memori, lalu beralih ke backend kelas produksi tanpa mengubah kode
aplikasi.

Tidak yakin backend mana yang cocok? Lihat
[Memilih backend](/docs/tools/keychain/choosing-a-backend). Keychain tersedia
untuk [Rust](https://crates.io/crates/solana-keychain) maupun
[TypeScript](https://www.npmjs.com/package/@solana/keychain).

### Keamanan RPC

Perlakukan endpoint RPC seperti API key—jangan mengeksposnya di kode frontend
tempat endpoint tersebut dapat diekstrak dan disalahgunakan. Gunakan proxy
backend atau variabel lingkungan yang tidak dibundel ke dalam kode klien.
...
Untuk pemantauan transaksi secara real-time dalam skala besar, lihat
...
nama dan simbol yang sama tetapi mint yang berbeda. Aplikasi Anda sebaiknya
meng-hardcode atau membuat allowlist alamat mint (berdasarkan kebutuhan Anda),
dan tidak pernah menerimanya secara dinamis dari sumber yang tidak tepercaya.

**Konfigurasi berbasis lingkungan:** Devnet dan Mainnet sering menggunakan token
mints yang sepenuhnya berbeda. Atur konfigurasi aplikasi Anda untuk memuat
alamat yang benar per lingkungan—jangan hardcode alamat mainnet dan lupa
menggantinya saat pengujian, atau lebih buruk lagi, mengirimkan alamat devnet ke
produksi.

Beberapa token stablecoin mint yang umum adalah:
...
gagal—atau lebih buruk lagi, mengakibatkan hilangnya dana yang tidak dapat
...
Memasukkan alamat yang benar ke dalam daftar izin melindungi dari token palsu,
tetapi tidak melindungi dari pengiriman ke _jenis_ akun yang salah. Alamat
penerima bisa berupa dompet, token account, mint, atau program, dan
masing-masing memerlukan penanganan yang berbeda. SOL asli yang dikirim ke
selain dompet akan lepas dari kendali pengirim — hilang sepenuhnya untuk mint
atau program, dan hanya dapat dipulihkan oleh pemilik akun untuk token account —
tanpa transaksi gagal yang memperingatkan pengguna.
...
- [ ] Biaya prioritas diimplementasikan dengan penetapan harga dinamis
...
- [ ] Validasi alamat penerima sebelum pengiriman (wallet vs token account vs
      mint vs program)
...
- [ ] Diuji beban pada volume yang diperkirakan

## Mendeploy Program

Jika Anda mendeploy program Solana kustom sebagai bagian dari infrastruktur
pembayaran Anda, terdapat pertimbangan tambahan.
...
- **Program keypair:** Program Anda akan memiliki alamat yang berbeda di mainnet
  ...
  Perbarui semua referensi dalam konfigurasi aplikasi Anda. Simpan program
  keypair Anda di lokasi yang aman (perhatikan bahwa menjalankan `cargo clean`
...
- **Inisialisasi akun:** Jika program Anda memerlukan akun admin, PDA, atau akun
  ...
  berinteraksi dengan aplikasi Anda. Begitu pula untuk Associated Token Accounts
  (ATA) apa pun yang dibutuhkan program Anda.
...
- **Upgrade authority:** Tentukan apakah program Anda harus dapat diupgrade
  setelah diluncurkan. Untuk imutabilitas, cabut upgrade authority setelah
