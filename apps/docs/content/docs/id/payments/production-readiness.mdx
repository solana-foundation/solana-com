...
- [ ] Uji beban pada volume yang diharapkan
...
- **Inisialisasi akun:** Jika program Anda memerlukan akun admin, PDA, atau akun
  state lainnya, pastikan ini dibuat di mainnet sebelum pengguna berinteraksi
  dengan aplikasi Anda. Hal yang sama berlaku untuk Associated Token Account
  (ATA) yang dibutuhkan oleh program Anda.

### Proses Deployment

- **Akun buffer:** Program besar di-deploy melalui akun buffer. Perintah
  `solana program deploy` menangani ini secara otomatis, tetapi pahami bahwa
  deployment tidak bersifat atomik—jika terputus, Anda mungkin perlu memulihkan
  atau menutup akun buffer. Lihat
  [Deploying Programs](https://solana.com/docs/programs/deploying).
- **Upgrade authority:** Tentukan apakah program Anda harus dapat di-upgrade
  setelah peluncuran. Untuk immutability, cabut upgrade authority setelah
