...
description: Pelajari cara melampirkan ID invoice atau catatan ke pembayaran.
---

Program memo Solana memungkinkan Anda melampirkan nomor faktur, ID pesanan, atau
referensi khusus ke pembayaran apa pun. Memo ini dicatat secara permanen di
dalam blockchain dan terlihat di log transaksi, sehingga memudahkan untuk
mencocokkan pembayaran dengan sistem internal Anda.

## Cara kerjanya

Program Memo menulis teks ke log transaksi. Log ini diindeks oleh explorer dan
penyedia RPC, membuat memo dapat dicari untuk rekonsiliasi pembayaran.
...
Menambahkan memo memerlukan pembuatan instruksi secara langsung, yang memberi
Anda kontrol atas apa yang disertakan dalam transaksi.
...
## !!steps Impor program Memo

Impor `getAddMemoInstruction` dari `@solana-program/memo` untuk membuat
instruksi memo.

<CodePlaceholder title="Pembayaran dengan memo" />
...
Buat instruksi transfer token dengan menentukan ATA sumber, ATA tujuan,
authority (penandatangan), dan jumlah dalam unit dasar.
...
## !!steps Buat instruksi memo

Buat instruksi memo dengan pesan. Pesan ini akan terlihat dalam log program
transaksi.
...
## !!steps Kirim transaksi dengan memo
...
## !!steps Lihat memo di log transaksi

Setelah transaksi dikonfirmasi, ambil untuk melihat memo di log. Memo muncul
sebagai pesan log dari program Memo.
