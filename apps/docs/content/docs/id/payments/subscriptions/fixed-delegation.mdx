---
title: Delegasi Tetap
description: Panduan untuk mengatur transaksi delegasi tetap.
---
...
Panduan ini menjaga alur tetap terlihat. Setiap cuplikan menggunakan fungsi SDK
secara langsung sehingga Anda dapat melihat akun mana yang diturunkan, instruksi
mana yang dikirim, dan penandatangan mana yang membayar atau mengotorisasi
setiap langkah.
...
Penerima delegasi menandatangani transfer. SDK memerlukan PDA delegasi yang
sama, token account pengguna, dan token account penerima.
...
Pendelegasi dapat mencabut delegasi kapan saja. Pencabutan akan menutup PDA
delegasi dan mengembalikan rent-nya ke penandatangan.
...
- Jumlah dalam satuan dasar. Untuk token dengan 6 desimal, `1_000_000` berarti
  `1` token.
- Jalankan `initSubscriptionAuthority` hanya saat akun Subscription Authority
  belum ada.
- `expiryTs` adalah batas keras: setelah terlewati, transfer gagal dan sponsor
  dapat memulihkan rent. Tidak ada masa tenggang pengeluaran.
