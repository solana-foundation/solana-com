...
- `amountPerPeriod` dalam satuan dasar. Untuk token 6 desimal, `1_000_000`
  berarti `1` token.
- Program menolak transfer yang melebihi sisa allowance periode saat ini.
- Setelah periode berikutnya dimulai, jumlah yang ditarik akan direset.
- `expiryTs` adalah batas keras: setelah melewatinya, transfer akan gagal dan
  sponsor dapat memulihkan rent. Tidak ada masa tenggang waktu pengeluaran.
- Tetapkan `startTs` ke `0` untuk memulai delegasi saat transaksi mendarat di
  chain, bukan pada waktu yang ditentukan. Ini memperluas jendela waktu untuk
  mendapatkan pengguna agar menandatangani dan bergabung. Saat Anda melakukan
  ini, `expiryTs` harus bernilai non-zero — sebuah delegasi start-on-landing
  tidak dapat juga diatur agar tidak pernah kedaluwarsa.
- Pengguna menandatangani transaksi setup dan revoke. Delegatee menandatangani
  transfer.
