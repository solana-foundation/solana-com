...
Fungsi-fungsi ini berinteraksi dengan Solana untuk memverifikasi transaksi dan menunggu
konfirmasi. Mereka memerlukan klien Solana RPC dari pustaka `@solana/kit`.
...
- **`rpc`** (`Rpc<SolanaRpcApi>`, diperlukan) - Klien RPC dari `@solana/kit`.
  Buat dengan `createSolanaRpc(rpcUrl)`.
