---
title: Referensi API Rust
description:
  API publik lengkap untuk crate surfpool-sdk — Surfnet, builder, cheatcodes,
  builders, errors, dan re-exports.
---

Crate `surfpool-sdk` mengekspos antarmuka kecil yang berpusat pada struct
`Surfnet`. Halaman ini mencantumkan setiap item publik; untuk panduan penggunaan
naratif, lihat panduan yang dimulai dengan
...
| Item                  | Jenis      | Catatan                                                                      |
| --------------------- | ---------- | ---------------------------------------------------------------------------- |
| `Surfnet`             | struct     | Instansi Surfnet yang berjalan — endpoint RPC/WS, payer, cheatcodes          |
| `SurfnetBuilder`      | struct     | Builder fluent yang digunakan oleh `Surfnet::builder()`                      |
| `Cheatcodes<'_>`      | struct     | Helper cheatcode yang dipinjam dan dikembalikan oleh `Surfnet::cheatcodes()` |
| `SurfnetError`        | enum       | Semua varian error yang dikembalikan SDK                                     |
...
`stop` memblokir hingga lima detik sambil menunggu runtime melepaskan port-nya.
Kegagalan selama `Drop` bersifat diam; kegagalan selama `stop` eksplisit
mengembalikan `SurfnetError::Runtime`.
...
Setiap setter mengembalikan `Self`, sehingga panggilan dapat dirantai. Panggilan
terminal adalah `start().await`.
...
Implementasikan ini untuk cheatcode JSON-RPC kustom yang belum dibungkus oleh
builder bertipe.

## Builder

Semua builder berada di `surfpool_sdk::cheatcodes::builders`.
...
| Varian           | Penyebab                                                     |
| ---------------- | ------------------------------------------------------------ |
| `PortAllocation` | Tidak dapat mengikat ke port bebas untuk RPC atau WebSocket. |
| `Startup`        | Runtime gagal diinisialisasi.                                |
| `Runtime`        | Thread runtime panik atau keluar secara tidak terduga.       |
| `Cheatcode`      | Panggilan JSON-RPC cheatcode mengembalikan error.            |
| `Aborted`        | Runtime dimatikan atau dibatalkan saat startup.              |

Crate ini mengimplementasikan `std::error::Error` dan `Display` pada
`SurfnetError`, sehingga bekerja dengan `?` dan `anyhow` secara langsung.
