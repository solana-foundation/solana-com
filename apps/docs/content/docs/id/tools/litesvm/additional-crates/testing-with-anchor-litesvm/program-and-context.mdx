---
title: Program & Konteks
description:
  Menyiapkan lingkungan pengujian dan membangun instruksi dengan AnchorContext
  dan Program
---

## Menghasilkan Tipe Klien dengan declare_program!

Anchor 1.0 memperkenalkan `declare_program!`, yang membaca IDL program Anda pada
waktu kompilasi dan menghasilkan modul `client::accounts::*` dan
`client::args::*` yang aman terhadap tipe. Ini adalah cara yang direkomendasikan
untuk membangun instruksi dalam pengujian.
...
<Callout type="info">
  Jalankan `anchor build` terlebih dahulu untuk menghasilkan
  `target/idl/my_program.json`. `declare_program!` membaca file ini pada waktu
  kompilasi. Nama makro harus sesuai dengan nama crate program Anda
  (snake_case).
</Callout>

## Menyiapkan Lingkungan Pengujian

Builder `AnchorLiteSVM` menyediakan API yang mudah digunakan untuk
...
### Singkatan untuk Program Tunggal
...
<Callout type="info">
  Jika Anda tidak menentukan payer, `AnchorLiteSVM` secara otomatis membuat dan
  mendanai satu untuk Anda.
</Callout>

## AnchorContext

`AnchorContext` adalah antarmuka utama untuk berinteraksi dengan lingkungan
pengujian Anda.
...
## Membangun Instruksi dengan Program

Struct `Program` menyediakan API yang mudah digunakan untuk membangun instruksi
yang mencerminkan anchor-client. Gunakan tipe yang dihasilkan oleh
`declare_program!` untuk keamanan tipe.

### Membangun Instruksi Dasar
...
<Callout type="info">
  Metode `.accounts()` menerima tipe apa pun yang mengimplementasikan
  `ToAccountMetas`, yang secara otomatis dilakukan oleh semua struct akun
  Anchor. Demikian pula, `.args()` menerima tipe apa pun yang
  mengimplementasikan `InstructionData`.
</Callout>
...
### Menggunakan send_and_confirm_transaction
...
Berikut adalah contoh komprehensif menggunakan pola escrow dengan
`declare_program!`:
