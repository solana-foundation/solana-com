---
title: Asersi
description: Verifikasi status akun, saldo, dan kepemilikan dengan helper asersi
---

## Keberadaan Akun

Verifikasi apakah akun ada atau telah ditutup.
...
<Callout type="info">
  `assert_account_closed` lolos ketika sebuah akun tidak ada, memiliki nol
  lamport, atau memiliki data kosong. Gunakan ini untuk memverifikasi
  pembersihan setelah menutup akun.
</Callout>

## Asersi Saldo

Verifikasi saldo SOL dan token.
...
Verifikasi kepemilikan akun dan properti data.
...
<Callout type="info">
  Ukuran akun umum: - **Akun Mint**: 82 byte - **token account**: 165 byte -
  **Akun Sistem**: 0 byte (hanya lamport)
</Callout>

## Contoh Lengkap

Berikut adalah pengujian komprehensif yang mendemonstrasikan berbagai
pernyataan:
...
## Menangani Kegagalan Pernyataan

Ketika sebuah pernyataan gagal, ia akan panik dengan pesan deskriptif:
...
<Callout type="warn">
  Pernyataan dirancang untuk gagal cepat dengan pesan kesalahan yang jelas.
  Gunakan secara bebas di seluruh pengujian Anda untuk mendeteksi masalah lebih
  awal.
</Callout>
