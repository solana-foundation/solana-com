---
title: Transaksi
description: Metode untuk menangani transaksi
---
...
Jalankan transaksi dan ubah state.
...
Mengembalikan `SimulatedTransactionInfo` yang berisi:

- `meta`: Metadata transaksi (log, unit komputasi, tanda tangan, dll.)
- `post_accounts`: Status akun setelah simulasi
...
Gunakan `simulate_transaction()` untuk:

- Menguji validitas transaksi tanpa mengubah state
- Memeriksa konsumsi unit komputasi
- Memverifikasi log transaksi tanpa komitmen
- Menguji jalur penanganan kesalahan
...
`send_transaction()` mengembalikan `TransactionMetadata` secara langsung,
sedangkan `simulate_transaction()` mengembalikan `SimulatedTransactionInfo` yang
berisi `meta: TransactionMetadata`:
...
Kueri transaksi dari riwayat. Membutuhkan `with_transaction_history()` untuk
diatur.
...
Dapatkan blockhash saat ini untuk transaksi.
...
Paksa blockhash saat ini untuk kedaluwarsa.
