---
title: Transfer SOL
description: Bangun dan kirim transaksi transfer SOL dengan litesvm-go
---

Transaksi Solana paling dasar: memindahkan SOL dari satu akun ke akun lainnya. Ini adalah
tes pertama yang baik untuk memverifikasi bahwa pengaturan `litesvm-go` Anda berfungsi sebelum menguji
logika program yang lebih kompleks.
...
<Callout type="info">
  Saldo akhir pengirim sedikit kurang dari 9 SOL karena biaya transaksi
  (5.000 lamport per tanda tangan).
</Callout>

Ini mencakup jenis transaksi paling sederhana. Untuk pengujian dengan program kustom, lihat
[Pengujian Program](/docs/tools/litesvm/go/examples/program-testing). Untuk
logika yang bergantung pada waktu, lihat
[Pengujian Berbasis Waktu](/docs/tools/litesvm/go/examples/time-based).
