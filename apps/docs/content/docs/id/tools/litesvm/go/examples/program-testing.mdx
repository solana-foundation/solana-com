...
description: Muat dan uji program Solana kustom dengan litesvm-go
---

Menguji program Solana kustom dari Go berarti memuat file `.so` yang telah dikompilasi
ke dalam SVM, menyiapkan akun yang dibutuhkan, dan mengirimkan instruksi
kepadanya. `litesvm-go` menangani semua ini dalam proses - tidak perlu validator.
...
Untuk pengujian yang membutuhkan program standar (SPL Token, Memo, ...):
...
1. **Konfigurasi** - aktifkan sysvars, builtins, precompiles sesuai kebutuhan.
2. **Pemuatan program** - `AddProgramFromFile` (atau `AddProgram` untuk byte
   dalam memori).
3. **Pengaturan akun** - isi awal akun data dengan `SetAccount`.
4. **Pengiriman** - bangun instruksi dengan `solana-go`, marshal, kirimkan melalui
   `SendLegacyTransaction`.
5. **Verifikasi** - baca kembali akun dengan `GetAccount` dan periksa log,
   unit komputasi, dan status akhir.

Setelah kerangka ini berjalan, tingkatkan skala dengan menambahkan lebih banyak instruksi, lebih banyak
akun, dan helper untuk kode boilerplate.
