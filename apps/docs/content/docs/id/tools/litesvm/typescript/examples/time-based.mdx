...
description: Uji logika yang bergantung pada waktu dengan manipulasi clock LiteSVM
---

Mendemonstrasikan penggunaan `warpToSlot` untuk menguji logika program yang bergantung pada waktu.

## Manipulasi Clock Dasar
...
Anda dapat membaca jadwal epoch untuk memahami hubungan slot/epoch pada
kluster:
...
<Callout type="warn">
  `warpToSlot` memajukan field `slot` pada clock tetapi **tidak** secara otomatis
  memperbarui `epoch` atau `unixTimestamp`. Gunakan `setClock` untuk kontrol penuh atas
  field clock.
</Callout>

## Manipulasi Clock Manual

Ketika program Anda membaca `epoch` atau `unix_timestamp` dari Clock sysvar, gunakan
`setClock` untuk mengatur field tersebut secara langsung:
...
Ini mencerminkan pola `set_sysvar(&clock)` di Rust. Gunakan `warpToSlot` untuk kemajuan
slot sederhana, gunakan `setClock` saat Anda membutuhkan kontrol epoch atau timestamp.
...
| Skenario                | Pendekatan                           |
| ----------------------- | ---------------------------------- |
| Vesting token           | Warp melewati cliff/milestone vesting |
...
<Callout type="info">
  Ingat bahwa `warpToSlot` hanya mengubah slot. Unix timestamp mungkin tidak
  diperbarui secara proporsional tergantung pada implementasi SVM.
</Callout>

## Poin Utama

1. **Aktifkan Sysvar**: Panggil `withSysvars()` sebelum menggunakan metode clock
...
3. **Kontrol Penuh**: Gunakan `setClock` saat Anda perlu mengatur `epoch`,
   `unixTimestamp`, atau field clock lainnya
4. **Baca Clock**: Gunakan `getClock()` untuk membaca slot, epoch, timestamp saat ini
