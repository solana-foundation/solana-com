...
Mengatur status akun secara langsung tanpa transaksi.
...
Mendapatkan saldo SOL akun. Mengembalikan `null` jika akun tidak ada.
...
Memuat program dari path file `.so`.
...
<Callout type="info">
  Menggunakan `setAccount` lebih cepat daripada `airdrop` untuk pengaturan akun massal karena
  tidak mengeksekusi transaksi.
</Callout>
