...
Menguji program Solana kustom berarti memuat file `.so` yang telah dikompilasi ke dalam
SVM, menyiapkan akun yang diharapkan, dan mengirim instruksi ke dalamnya.
LiteSVM menangani semua ini secara in-process — tidak memerlukan validator. Kami merekomendasikan
untuk menghasilkan klien program dengan
[Codama JS Renderer](https://github.com/codama-idl/renderers-js) sehingga Anda dapat
...
## Pengaturan Uji Program Lengkap
...
<Callout type="info">
  Bangun program Solana Anda dengan `cargo build-sbf` untuk menghasilkan file `.so`.
</Callout>

Polanya selalu sama:

1. **Pemuatan Program**: Gunakan `addProgramFromFile` untuk memuat program yang telah dikompilasi dan
   impor myProgramPlugin() dari klien Anda
2. **Konfigurasi**: Aktifkan sysvar, builtin, dan precompile sesuai kebutuhan
3. **Pengaturan Akun**: Isi awal akun data dengan `setAccount`
4. **Pengiriman**: Gunakan `client.myProgram.testInstruction({..}).sendTransaction()` —
   tanpa pembangunan transaksi manual
5. **Verifikasi**: Periksa status akun setelah eksekusi
   (`client.myProgram.myAccount.fetch`)

Setelah kerangka ini berjalan, Anda dapat membangun rangkaian pengujian lengkap dengan
menambahkan lebih banyak instruksi dan pernyataan.
