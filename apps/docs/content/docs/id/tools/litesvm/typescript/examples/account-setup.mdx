...
description: Siapkan akun pengujian dan status dengan LiteSVM
---

# Contoh Pengaturan Akun

Mendemonstrasikan pengaturan status akun pengujian secara langsung tanpa
transaksi.
...
Siapkan program-owned data account:
...
<Callout type="info">
  Menggunakan `setAccount` lebih cepat daripada menjalankan transaksi untuk
  pengaturan akun secara massal.
</Callout>

## Poin-Poin Utama

1. **Status Langsung**: `setAccount` mengatur status akun tanpa transaksi
2. **Pembebasan Sewa**: Gunakan `minimumBalanceForRentExemption` untuk
   menghitung lamport yang diperlukan
3. **Struktur Akun**: Sediakan semua kolom yang diperlukan: address, data,
   executable, lamports, programAddress, space
4. **Verifikasi**: Gunakan `getAccount` atau `client.rpc.getAccountInfo` untuk
   verifikasi
