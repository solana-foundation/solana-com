---
title: Rust
description: Instal dan gunakan solana-keychain dalam aplikasi Rust
---

## Instalasi

Tambahkan `solana-keychain` ke `Cargo.toml` Anda dengan hanya backend yang Anda
butuhkan:
...
| Fitur      | Deskripsi                     |
| ------------ | ------------------------------- |
| `memory`     | Penandatanganan keypair lokal (default) |
| `vault`      | Mesin Transit HashiCorp Vault  |
| `aws_kms`    | AWS KMS dengan Ed25519            |
| `gcp_kms`    | GCP Cloud KMS dengan Ed25519      |
| `privy`      | Dompet tertanam Privy          |
| `turnkey`    | API Turnkey                     |
...
| `all`        | Aktifkan semua backend             |

## Penggunaan Dasar

Semua penandatangan mengimplementasikan
...
### Penandatangan Memori

Untuk pengembangan dan pengujian dengan keypair lokal:
...
Untuk HSM yang dihosting sendiri dengan mesin Transit:
...
Untuk penandatanganan native-cloud dengan kunci Ed25519 AWS KMS:
...
Untuk penandatanganan dompet tertanam:
...
Untuk penandatanganan non-kustodian:
...
Untuk penandatanganan MPC institusional:
...
Untuk penandatanganan cloud-native dengan kunci Ed25519 GCP Cloud KMS:
...
Untuk penandatanganan dompet terkelola Coinbase:
...
Untuk penandatanganan dompet terkelola Crossmint:
...
Untuk penandatanganan infrastruktur dompet Dfns:
...
Untuk penandatanganan dompet MPC Para:
...
Untuk penandatanganan kustodi MPC Fordefi. Permintaan API diautentikasi dengan tanda tangan permintaan ECDSA
P-256 yang diurai dari `private_key_pem`; untuk menyimpan kunci tersebut di
KMS/HSM, implementasikan `FordefiRequestSigner` dan gunakan
`Signer::from_fordefi_with_signer`.

Fordefi memiliki dua mode penandatanganan. Atur `chain` untuk **mode Solana native**, di mana
Fordefi dapat mengganti blockhash atau biaya dan menyiarkan transaksi itu sendiri.
Biarkan `chain` sebagai `None` untuk **mode black box**, di mana Fordefi menandatangani byte mentah
dan Anda menyiarkan transaksi sendiri.
...
Gunakan enum `Signer` untuk memilih backend saat runtime:
