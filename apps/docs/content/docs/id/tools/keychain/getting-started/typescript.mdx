...
Fordefi memiliki dua mode penandatanganan, dipilih berdasarkan apakah Anda meneruskan `chain`. Permintaan API diautentikasi dengan tanda tangan permintaan ECDSA P-256 — berikan kunci penandatanganan sebagai `privateKeyPem`, atau simpan di KMS/HSM dengan meneruskan `requestSigner`.

**Mode Solana Native** (direkomendasikan untuk penggunaan on-chain): atur `chain`, dan Fordefi menandatangani serta menyiarkan transaksi itu sendiri. Karena Fordefi mungkin mengganti blockhash atau biaya sebelum penandatanganan, penanda tangan adalah Kit `TransactionSendingSigner` — gunakan bersama `signAndSendTransactionMessageWithSigners`, bukan alur partial-signer seperti yang ditunjukkan di atas.
...
**Mode black box**: hilangkan `chain`. Fordefi menandatangani byte mentah dan tidak menyiarkan; Anda merakit dan mengirimkan transaksi sendiri. Gunakan ini dengan vault black box Fordefi.
