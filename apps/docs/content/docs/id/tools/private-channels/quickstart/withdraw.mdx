...
dana. Bukti SMT memastikan setiap nonce penarikan hanya dapat digunakan sekali,
...
1. `indexer-private-channel` melakukan polling channel setiap 1 detik untuk event pembakaran
   dan menulis catatan penarikan yang tertunda saat penarikan Anda terdeteksi
2. `operator-private-channel` melakukan polling database setiap 1 detik untuk catatan
   yang tertunda dan mengirimkan `ReleaseFunds` ke Escrow Program di Solana devnet
   dengan bukti eksklusi SMT yang diperlukan; ia melakukan polling untuk konfirmasi devnet hingga
   5 kali dengan interval 400 md sebelum mencoba ulang
...
Setelah pembakaran sisi channel, operator yang telah diprovisikan harus memanggil `ReleaseFunds` pada
...
- `sibling_proofs`: 512 byte (16 x 32-byte sibling hash untuk bukti pohon)
...
Anda telah menjalankan siklus Private Channels secara penuh di devnet: menyetor token SPL ke
escrow, mengirimkan transfer off-chain melalui gateway, dan menarik dana
...
<Cards>
  ...
  >
    Pahami peserta dan alur lengkap dari deposit hingga penarikan secara mendalam.
  </Card>
  <Card
