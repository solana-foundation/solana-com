...
> _Catatan: Ketika `fallback_if_unavailable` bernilai true, sistem mengizinkan transaksi
> untuk tetap diproses jika Redis tidak tersedia sementara, sehingga mencegah gangguan layanan.
> Menetapkan `max_transactions` ke 0 akan mengizinkan transaksi tanpa batas._
...
| `get_payer_signer`          | Mengembalikan fee payer dan alamat pembayaran berikutnya        | ✅       | boolean |
...
[Dokumentasi Solana](https://solana.com/developers/cookbook/transactions/durable-nonces).
...
| ---------------------------- | ------------------------------------------------ |
| `confidential_transfer_mint` | Konfigurasi Confidential Balances untuk mint |
...
| Nama Ekstensi                  | Deskripsi                                 |
| ------------------------------- | ------------------------------------------- |
| `confidential_transfer_account` | Status Confidential Balances untuk akun |
