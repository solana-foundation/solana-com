...
Tambahkan layanan Anda ke daftar penanda tangan di README utama.

## Daftar Periksa Pengiriman

- [ ] Penanda tangan Anda didukung dalam crate `solana-keychain`
- [ ] Memperbarui dependensi `solana-keychain` di Cargo.toml ke versi terbaru
- [ ] Menambahkan struct konfigurasi untuk penanda tangan Anda
- [ ] Menambahkan varian `SignerTypeConfig`
- [ ] Menambahkan logika build di `build_signer_from_config`
- [ ] Menambahkan logika validasi di `validate_individual_signer_config`
- [ ] Mengekspor struct konfigurasi di `mod.rs`
- [ ] (Opsional) Menambahkan metode builder mock pengujian di `config_mock.rs`
- [ ] Kode dikompilasi tanpa peringatan
- [ ] Semua pengujian lulus (`make test` dan `make test-integration`)
- [ ] Dokumentasi ditambahkan mengikuti
      [`ADDING_SIGNERS.md`](https://github.com/solana-foundation/solana-keychain/blob/main/docs/ADDING_SIGNERS.md)
- [ ] File konfigurasi contoh dibuat (`.toml` dan `.env.example`)
- [ ] Tidak ada nilai hardcoded atau rahasia
- [ ] Pesan kesalahan bersifat informatif
- [ ] Mengikuti konvensi penamaan Rust (snake_case)
- [ ] Linting lulus (`make lint`)
- [ ] Hubungi tim Kora dengan API Keys untuk pengujian integrasi

## Mendapatkan Bantuan

- **Untuk implementasi penanda tangan**: Buka isu di repositori
  [`solana-keychain`](https://github.com/solana-foundation/solana-keychain)
- **Untuk integrasi Kora**: Buka isu di repositori Kora untuk diskusi desain
- Bergabunglah dengan saluran komunitas kami
- Tinjau konfigurasi penanda tangan yang ada di
...
Selamat datang di ekosistem Kora! Kami sangat antusias menyambut solusi
manajemen kunci Anda sebagai bagian dari platform ini.
