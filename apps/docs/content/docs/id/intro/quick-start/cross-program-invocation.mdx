...
mengubah struct _rs`Update`_ dan fungsi `update`.

Mulailah dengan memperbarui file `lib.rs` untuk membawa ke dalam scope item-item
dari modul `system_program`.
...
Selanjutnya, perbarui struct _rs`Update`_ untuk menyertakan akun baru bernama
...
Akun ini berfungsi sebagai "vault" yang dikontrol program yang menerima SOL dari
pengguna ketika mereka memperbarui pesan mereka.
...
- Sebagai PDA, ia tidak memiliki private key, sehingga hanya program yang dapat
  "menandatangani" untuk alamat tersebut saat melakukan CPI
...
(CPI) untuk memanggil instruksi `transfer` System Program. Ini menunjukkan cara
melakukan CPI dari dalam program, memungkinkan komposabilitas program Solana.
...
Pengaturan untuk CPI sesuai dengan cara instruksi sisi klien dibangun, di mana
...
secara internal memanggil instruksi transfer System Program.
...
_rs`Delete`_ dan fungsi `delete`.
...
`vault_account` menggunakan derivasi PDA yang sama seperti pada struct Update.

Menambahkan `vault_account` ke struct Delete memungkinkan program mengakses akun
vault pengguna selama instruksi delete untuk mentransfer SOL yang terakumulasi
kembali ke pengguna.
...
_rs`signer_seeds`_ menentukan seeds opsional dan bump seeds yang digunakan untuk
menurunkan PDA.
...
_rs`ctx: Context<Delete>`_ untuk menggunakan context dalam body fungsi.
