...
untuk ditambahkan pada langkah-langkah berikut.
...
program starter berhasil di-build.
...
Atribut _rs`#[account]`_ dalam program Anchor menganotasi struct yang
merepresentasikan data akun (tipe data untuk disimpan dalam field data Account).

### !user

Field _rs`user`_ berisi _rs`Pubkey`_ yang mengidentifikasi pengguna yang membuat
akun pesan.

### !message

Field _rs`message`_ menyimpan _rs`String`_ yang berisi pesan pengguna.

### !bump

Field _rs`bump`_ menyimpan _rs`u8`_ ["bump" seed](/docs/core/pda#canonical-bump)
...
pengguna yang berisi tiga field:
...
- `bump` - _rs`u8`_ yang menyimpan ["bump" seed](/docs/core/pda#canonical-bump)
  ...
  komputasi dengan menghilangkan kebutuhan untuk menghitung ulang pada instruksi
  selanjutnya.

Saat membuat akun, program melakukan serialisasi data _rs`MessageAccount`_ dan
menyimpannya di field data akun baru.

Kemudian, saat membaca dari akun, program melakukan deserialisasi data ini
kembali ke tipe data _rs`MessageAccount`_. Bagian pengujian mendemonstrasikan
proses pembuatan dan pembacaan data akun.

</Accordion>
</Accordions>

Build program kembali dengan menjalankan _shell`build`_ di terminal.
...
Mulai dengan mendefinisikan akun yang diperlukan untuk instruksi dengan
...
Batasan _rs`mut`_ mendeklarasikan akun sebagai mutable.
...
Batasan _rs`bump`_ mendeklarasikan bump seed untuk PDA.
...
Batasan _rs`space`_ menentukan jumlah byte yang dialokasikan untuk field data
akun baru.
...
Tipe _rs`Account<'info, T>`_ mengharuskan akun cocok dengan tipe yang
ditentukan.

Dalam kasus ini, akun harus cocok dengan tipe _rs`MessageAccount`_ kustom.

### !program

Tipe _rs`Program<'info, T>`_ mengharuskan akun cocok dengan program.

Dalam kasus ini, akun harus cocok dengan tipe _rs`System`_, yang merujuk pada
System Program.
...
1. Tipe akun (seperti _rs`Signer<'info>`_ atau _rs`Account<'info, T>`_) yang
   menentukan jenis akun yang diharapkan program
...
Dalam contoh ini, struct _rs`Create`_ mendefinisikan akun-akun yang diperlukan
untuk instruksi _rs`create`_.

1. _rs`user: Signer<'info>`_
   - Mewakili pengguna yang membuat akun pesan
   - Memerlukan status mutable (_rs`#[account(mut)]`_) karena membayar untuk
     akun baru
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - `payer = user` mengidentifikasi siapa yang membayar untuk pembuatan akun
     baru
   - `space` mengalokasikan byte yang diperlukan untuk kolom data akun
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - Di balik layar, batasan `init` memanggil System Program untuk membuat akun
     baru dengan `space` yang ditentukan dan mengubah pemilik menjadi program
     saat ini.
...
- Diskriminator akun Anchor (identifier): 8 byte
...
Tipe _rs`String`_ memerlukan 4 byte untuk menyimpan panjang string, dan panjang
yang tersisa berisi data aktual.

</Accordion>
</Accordions>

Selanjutnya, tambahkan logika bisnis untuk instruksi _rs`create`_ dengan
memperbarui fungsi `create` dengan kode berikut:
...
pesan baru. Fungsi ini memerlukan dua parameter:
...
2. Menginisialisasi data akun:
   ...
   - Menetapkan bidang `user` ke kunci publik akun `user`.

   ```rs
   account_data.user = ctx.accounts.user.key();
   ```

   - Menetapkan bidang `message` ke `message` dari argumen fungsi.

   ```rs
   account_data.message = message;
   ```

   - Menetapkan nilai `bump` yang digunakan untuk menurunkan PDA, diambil dari
...
Batasan _rs`realloc::payer`_ menentukan akun yang membayar untuk realokasi.
...
1. _rs`user: Signer<'info>`_
   ...
   - Ditandai sebagai mutable (_rs`#[account(mut)]`_) karena mungkin membayar
     untuk ruang lebih banyak untuk `message_account` saat diperlukan
...
3. _rs`system_program: Program<'info, System>`_
   - Diperlukan untuk realokasi potensial ruang akun
...
seed yang disimpan pada _rs`message_account`_, daripada membiarkan Anchor
menghitungnya ulang.
...
sudah ada. Fungsi ini memiliki dua parameter:
...
2. Memperbarui data akun:
   - Mengakses `message_account` dari context.
   - Mengatur kolom `message` ke `message` baru dari argumen fungsi.
...
Constraint _rs`seeds`_ menentukan seed yang digunakan untuk menurunkan PDA.

### !bump

Constraint _rs`bump`_ menentukan bump seed untuk PDA.
...
Constraint _rs`close`_ menutup akun.
...
Struct _rs`Delete`_ mendefinisikan akun yang diperlukan untuk instruksi
_rs`delete`_:
...
Fungsi `delete` menerima satu parameter:

1. _rs`_ctx: Context<Delete>`_ - Menyediakan akses ke akun yang ditentukan dalam
   struktur _rs`Delete`_. Sintaks _rs`_ctx`_ menunjukkan bahwa fungsi tidak
   menggunakan Context dalam bodynya.

Body fungsi hanya mencetak pesan ke log program menggunakan makro _rs`msg!()`_.
Fungsi tidak memerlukan logika tambahan karena batasan _rs`close`_ dalam
struktur _rs`Delete`_ menangani penutupan akun.
...
Kode starter juga menyertakan file tes di `anchor.test.ts`.
...
Tambahkan kode di bawah ini di dalam _ts`describe()`_, tetapi sebelum bagian
_ts`it()`_.
...
untuk memeriksa bahwa file tersebut berjalan sesuai harapan. Langkah selanjutnya
...
Pertama, file pengujian mengirim transaksi yang memanggil instruksi `create`,
meneruskan "Hello, World!" sebagai pesan.
...
Setelah mengirim transaksi dan membuat akun, file pengujian mengambil akun
...
meneruskan "Hello, Solana!" sebagai pesan baru.
...
Setelah mengirimkan transaksi dan memperbarui akun, file pengujian mengambil
akun menggunakan alamatnya (`messagePda`).
...
Pertama, kode ini mengirimkan transaksi yang memanggil instruksi `delete` untuk
menutup akun pesan.
...
Setelah mengirimkan transaksi dan menutup akun, contoh ini mencoba mengambil
...
Setelah menyiapkan pengujian Anda, jalankan file pengujian dengan _shell`test`_
di terminal Playground. Perintah ini menjalankan pengujian terhadap program yang
