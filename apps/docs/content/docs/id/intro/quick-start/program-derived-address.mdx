...
description:
  ...
  menggunakan Program Derived Address (PDA) dan framework Anchor. Panduan
  langkah demi langkah ini mendemonstrasikan cara membuat, memperbarui, dan
  menghapus akun pesan onchain menggunakan PDA, mengimplementasikan validasi
  akun, dan menulis pengujian. Cocok untuk pengembang yang ingin memahami cara
  menggunakan PDA dalam program Solana.
...
Mulai dengan mendefinisikan akun-akun yang diperlukan untuk instruksi dengan
memperbarui struct _rs`Create`_ dengan kode berikut:
...
Constraint _rs`mut`_ mendeklarasikan akun sebagai mutable.

### !init

Constraint _rs`init`_ membuat akun baru.

### !seeds

Constraint _rs`seeds`_ mendefinisikan input opsional yang digunakan untuk
menurunkan PDA.

### !bump

Constraint _rs`bump`_ mendeklarasikan bump seed untuk PDA.

Jika Anda tidak menentukan nilai, Anchor akan menghitungnya secara otomatis.

### !payer

Constraint _rs`payer`_ menentukan akun mana yang membayar untuk pembuatan akun
baru.

### !space

Constraint _rs`space`_ menentukan jumlah byte yang dialokasikan untuk field data
akun baru.
...
Dalam kasus ini, akun harus cocok dengan tipe custom _rs`MessageAccount`_.

### !program

Tipe _rs`Program<'info, T>`_ mengharuskan akun cocok dengan sebuah program.
...
2. Constraint opsional (seperti _rs`#[account(mut)]`_ atau
   _rs`#[account(init)]`_) yang mendefinisikan persyaratan tambahan
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - Constraint `init` membuat akun selama eksekusi instruksi
   - Constraint `seeds` dan `bump` menurunkan alamat akun sebagai Program
     Derived Address (PDA)
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - Di balik layar, constraint `init` memanggil System Program untuk membuat
     akun baru dengan `space` yang ditentukan dan mengubah owner menjadi program
     saat ini.
...
Constraint `seeds` dan `bump` bersama-sama mendefinisikan alamat akun sebagai
...
Constraint `seeds` mendefinisikan input opsional yang digunakan untuk menurunkan
PDA.

- _rs`b"message"`_ - String tetap sebagai seed pertama.
- _rs`user.key().as_ref()`_ - Public key dari akun _rs`user`_ sebagai seed
  kedua.

Constraint `bump` memberi tahu Anchor untuk secara otomatis menemukan dan
...
1. _rs`ctx: Context<Create>`_ - Menyediakan akses ke akun-akun yang ditentukan
   dalam struct _rs`Create`_.
...
2. Menginisialisasi data akun:
   ...
   - Menetapkan kolom `user` ke public key dari akun `user`.

   ```rs
   account_data.user = ctx.accounts.user.key();
   ```

   - Menetapkan kolom `message` ke `message` dari argumen fungsi.
...
yang disimpan pada _rs`message_account`_, daripada membiarkan Anchor
menghitungnya ulang.
...
1. _rs`ctx: Context<Update>`_ - Menyediakan akses ke akun yang ditentukan dalam
   struct _rs`Update`_.
2. _rs`message: String`_ - Pesan baru untuk menggantikan yang sudah ada.
...
2. Memperbarui data akun:
   - Mengakses `message_account` dari context.
   - Mengatur field `message` ke `message` baru dari argumen fungsi.
...
Batasan _rs`seeds`_ menentukan seed yang digunakan untuk menurunkan PDA.
...
Fungsi tidak memerlukan logika tambahan karena constraint _rs`close`_ dalam
...
Kode starter juga menyertakan file test di `anchor.test.ts`.
...
Pertama, file pengujian mengirimkan transaksi yang memanggil instruksi `update`,
...
Setelah mengirim transaksi dan menutup akun, contoh ini mencoba mengambil akun
menggunakan alamatnya (`messagePda`) dengan `fetchNullable`. Metode ini
