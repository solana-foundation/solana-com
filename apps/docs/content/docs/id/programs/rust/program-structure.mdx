...
memungkinkan fleksibilitas dalam cara kode diorganisir. Satu-satunya persyaratan
adalah program harus memiliki `entrypoint`, yang mendefinisikan di mana eksekusi
program dimulai.
...
Sebuah `entrypoint` program Solana memiliki signature fungsi berikut. Developer
bebas membuat implementasi sendiri dari fungsi `entrypoint`.
...
Sekarang mari kita definisikan struktur data yang akan disimpan di akun counter
kita. Ini adalah data yang akan disimpan di field `data` dari akun.
...
Sekarang kita perlu mendeserialisasi `instruction_data` (raw bytes) menjadi
salah satu varian enum `CounterInstruction` kita. Method `try_from_slice` dari
Borsh menangani konversi ini secara otomatis.
...
Sekarang mari kita perbarui fungsi `process_instruction` utama untuk merutekan
instruksi ke fungsi handler yang sesuai.

Pola routing ini umum digunakan dalam program Solana. `instruction_data`
dideserialisasi menjadi varian dari enum yang merepresentasikan instruksi,
kemudian fungsi handler yang sesuai dipanggil. Setiap fungsi handler berisi
...
- Mendeserialkannya menjadi struct `CounterAccount`
- Menambahkan field `count` sebesar 1
- Menserialkan struct `CounterAccount` kembali ke field `data` akun
...
Pertama, mari tambahkan dependensi yang diperlukan untuk testing. Kita akan
menggunakan `litesvm` untuk testing dan `solana-sdk`.
...
Sekarang kita perlu membangun dan memuat program kita ke dalam environment
testing. Jalankan perintah `cargo build-sbf` untuk membangun program. Ini akan
...
<Callout type="warn">
  Anda harus menjalankan `cargo build-sbf` sebelum menjalankan test untuk
  menghasilkan file `.so`. Test memuat program yang telah dikompilasi.
</Callout>
...
Jalankan test dengan perintah berikut. Flag `--nocapture` mencetak output dari
test.
...
output tanda tangan transaksi. Perhatikan bahwa cluster di Solana Explorer harus
diatur ke "Custom RPC URL", yang secara default adalah `http://localhost:8899`
tempat `solana-test-validator` berjalan.
