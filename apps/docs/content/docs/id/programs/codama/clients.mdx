...
| [`@codama/nodes-from-anchor`](https://github.com/codama-idl/codama/tree/main/packages/nodes-from-anchor) | Mengonversi IDL Anchor ke IDL Codama untuk menghasilkan klien                                         |
...
Dalam proyek Anchor, IDL ditemukan di `target/idl/<program_name>.json` setelah
menjalankan `anchor build`.
...
Ini akan memunculkan pertanyaan untuk membuat file `codama.json` di root proyek
...
Anda tetapi tidak menyertakan `package.json` (untuk TypeScript) atau
`Cargo.toml` (untuk Rust).

Anda perlu membuat file `package.json` dan `Cargo.toml` baru atau menambahkan
dependensi yang diperlukan dari klien yang dihasilkan ke file proyek Anda yang
sudah ada.

### Terintegrasi ke dalam aplikasi Anda

Hasilkan klien langsung ke direktori sumber aplikasi Anda dan kemudian tambahkan
dependensi yang diperlukan ke `package.json` atau `Cargo.toml` Anda yang sudah
ada.
...
Buat paket TypeScript atau crate Rust terpisah untuk setiap client. Misalnya,
