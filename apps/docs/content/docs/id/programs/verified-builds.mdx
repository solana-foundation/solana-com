...
Perintah ini memuat gambar yang disimpan di
`ellipsislabs/hello_world_verifiable_build:latest`, dan memverifikasi bahwa hash
jalur eksekusi dalam kontainer sama dengan hash program onchain yang diberikan
ke perintah. Karena build sudah diunggah ke gambar, tidak perlu rebuild penuh
dari eksekusi yang dapat memakan waktu lama.
...
- **Penandatangan salah**: Konfirmasi bahwa penandatangan Anda adalah otoritas
  upgrade program dengan menjalankan `solana program show YourProgramId`
...
- **Ketidaksesuaian data PDA**: Perbarui PDA Anda jika Anda telah mendeploy
  ulang program Anda. Data PDA Anda harus sesuai dengan program yang dideploy.
- **Hash commit salah**: Buat PDA Anda menggunakan hash commit yang persis sama
...
- **Penyimpanan On-Chain**: Simpan metadata verifikasi Anda (URL repo, hash
  commit, parameter build) onchain dalam PDA yang dimiliki oleh program Otter
...
- **Kepercayaan Terdesentralisasi**: Siapa pun dapat membaca PDA Anda dan secara
  independen memverifikasi program Anda

## Mengapa saya harus membuat PDA sebelum menggunakan API?

API hanya berfungsi dengan PDA onchain karena:

- **Tanpa kepercayaan**: API menolak data arbitrer - hanya menggunakan apa yang
  disimpan otoritas upgrade Anda onchain
- **Lebih sederhana**: Cukup berikan penandatangan + program_id; API mendapatkan
  semua yang lain dari PDA Anda
- **Anti-rusak**: PDA Anda membuat catatan yang tidak dapat diubah yang dapat
  diverifikasi secara independen oleh siapa pun
- **Bukti kepemilikan**: Penandatangan Anda harus menjadi otoritas upgrade,
  secara kriptografis membuktikan Anda mengontrol program
...
- **PDA On-Chain**: Metadata verifikasi Anda tersimpan onchain, tidak dikontrol
...
1. Metadata PDA adalah autentik (tersimpan onchain)
