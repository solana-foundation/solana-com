...
- **Program ID**: Alamat onchain dari program Anda. Pengguna berinteraksi dengan
  program Anda dengan merujuk pada Program ID.
- **program account**: Akun onchain yang menyimpan metadata program Anda. Alamat
  program account adalah Program ID. program account juga menyimpan alamat dari
  programdata account.
- **programdata account**: Akun onchain yang menyimpan kode executable program
  Anda yang telah dideploy.
- **Program Authority**: Akun yang memiliki izin untuk memperbarui atau menutup
  program. Secara default, ini adalah wallet CLI Anda.
...
Program ID adalah alamat onchain dari program account Anda. Pengguna dan program
lain memanggil program Anda dengan merujuk ID ini dalam transaksi mereka.
...
- **`--use-rpc`**: Kirim transaksi ke RPC yang dikonfigurasi. Tanda ini memerlukan koneksi RPC [berbobot-stake](/docs/defi/stake-weighted-qos) dari penyedia seperti [Triton](https://triton.one/) atau [Helius](https://www.helius.dev/).
