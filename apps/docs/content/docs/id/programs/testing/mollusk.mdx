---
title: Mollusk
description: Panduan lengkap untuk menguji program Solana menggunakan Mollusk
---

[Mollusk](https://github.com/anza-xyz/mollusk/tree/main) adalah test harness
ringan untuk menguji program Solana. Ini menyediakan antarmuka sederhana untuk
menguji instruksi program Solana dalam lingkungan Solana Virtual Machine (SVM)
yang diminimalkan. Semua akun pengujian harus didefinisikan secara eksplisit,
memastikan pengujian yang deterministik dan dapat diulang.

- [Dokumentasi Mollusk SVM](https://docs.rs/mollusk-svm/latest/mollusk_svm/)
- [Contoh Penggunaan Mollusk Test](https://github.com/anza-xyz/mollusk/tree/0.5.0/harness/tests)
...
Untuk melakukan benchmark penggunaan compute unit, tambahkan
...
Contoh berikut menunjukkan pengaturan minimal untuk menguji program Solana dasar
menggunakan Mollusk.
...
Contoh ini mendemonstrasikan cara menguji program Solana dasar menggunakan
Mollusk. Program ini hanya mencetak "Hello, world!" ke log program saat
dipanggil.

Menjalankan `cargo build-sbf` menghasilkan program yang dikompilasi di
...
1. [Buat instance `Mollusk`](mention:mollusk) - Inisialisasi Mollusk dengan
   program ID dan path ke program yang dikompilasi (file `.so`)
...
Ketika pengujian berhasil dijalankan, Anda akan melihat output yang mirip dengan
berikut:
...
menyediakan antarmuka sederhana untuk menguji program Solana. Semua field dapat
dimanipulasi melalui beberapa metode helper, tetapi pengguna juga dapat langsung
mengakses dan memodifikasinya jika menginginkan kontrol lebih.
...
<Callout type="info">
  Saat memberikan path file, jangan sertakan ekstensi `.so`. Misalnya,
  `"path/to/my_program"` benar, tetapi `"path/to/my_program.so"` tidak.
</Callout>
...
| [`process_and_validate_instruction`](#single-instruction-with-checks)          | Memproses instruksi dan melakukan serangkaian pemeriksaan pada hasilnya, panic jika ada pemeriksaan yang gagal.                 |
| [`process_instruction_chain`](#multiple-instructions)                          | Memproses beberapa instruksi dan mengembalikan hasilnya.                                                                        |
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | Memproses beberapa instruksi dan melakukan serangkaian pemeriksaan pada setiap hasilnya, panic jika ada pemeriksaan yang gagal. |
...
Gunakan metode `process_instruction` untuk memproses satu instruksi tanpa
...
Gunakan metode `process_and_validate_instruction` untuk memproses satu instruksi
dengan pemeriksaan validasi. Metode ini akan panic jika ada pemeriksaan yang
gagal.
...
Contoh berikut memproses rangkaian dua instruksi transfer SOL dengan pemeriksaan
...
Gunakan yang berikut untuk memvalidasi status Account:
...
### Status Account persisten

`MolluskContext` adalah pembungkus di sekitar `Mollusk` yang mempertahankan
status account di beberapa pemanggilan instruksi melalui `account_store`-nya.
Metode untuk memproses instruksi identik dengan `Mollusk`.

Tidak seperti `Mollusk`, yang memerlukan pengiriman `accounts` ke setiap metode
(misalnya `process_instruction`), `MolluskContext` mengelola account secara
internal melalui `account_store`-nya. Ini menghilangkan kebutuhan untuk
parameter `accounts` saat memproses instruksi.
...
Contoh berikut memproses dua instruksi transfer SOL terpisah dengan status
account persisten di antara instruksi melalui `account_store`.
...
Mollusk menyediakan struct
[`Sysvars`](https://docs.rs/mollusk-svm/0.5.0/mollusk_svm/sysvar/struct.Sysvars.html)
kustom untuk memodifikasi nilainya untuk pengujian.

Gunakan metode `warp_to_slot` untuk memperbarui sysvar clock untuk
mensimulasikan bergerak maju atau mundur dalam waktu ke slot tertentu.
...
dengan mengakses field `sysvars` untuk mengubah parameter rent. Anda dapat
...
melacak penggunaan compute unit dari instruksi program. Hasil ditulis ke file
markdown.
...
Contoh berikut melakukan benchmark penggunaan compute unit dari instruksi
transfer SOL.
...
untuk menambahkan Token Program, token2022 program (Token Extensions), dan
...
<Callout type="info">
  Contoh di bawah ini secara manual mendefinisikan akun pengujian untuk tujuan
  demonstrasi. `mollusk-svm-programs-token` juga menyertakan fungsi helper untuk
  membuat akun mint dan token.
</Callout>
