...
ringan untuk menguji program Solana. Mollusk menyediakan antarmuka sederhana
untuk menguji instruksi program Solana dalam lingkungan Solana Virtual Machine
...
menyediakan interface sederhana untuk menguji program Solana. Semua field dapat
...
<Callout type="info">
  Saat menyediakan file path, jangan sertakan ekstensi `.so`. Misalnya,
...
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | Memproses beberapa instruksi dan melakukan serangkaian pemeriksaan pada setiap hasil, panic jika ada pemeriksaan yang gagal. |

[`InstructionResult`](https://docs.rs/mollusk-svm-result/0.5.0/mollusk_svm_result/types/struct.InstructionResult.html)
berisi detail dari instruksi yang telah diproses.
...
Contoh-contoh di bawah ini menjalankan Mollusk dalam fungsi `main` untuk tujuan
demonstrasi. Dalam praktiknya, Anda biasanya akan menggunakan Mollusk dalam
modul test yang dianotasi dengan atribut `#[test]`.
...
adalah wrapper di sekitar `Mollusk` yang mempertahankan state akun di seluruh
beberapa panggilan instruksi melalui `account_store`-nya. Metode untuk memproses
instruksi identik dengan `Mollusk`.
...
internal melalui `account_store`. Ini menghilangkan kebutuhan untuk parameter
...
persisten di antara instruksi melalui `account_store`.
...
Gunakan metode `warp_to_slot` untuk memperbarui sysvar clock guna mensimulasikan
bergerak maju atau mundur dalam waktu ke slot tertentu.
