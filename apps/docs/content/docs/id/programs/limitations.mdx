...
Mengembangkan program di blockchain Solana memiliki beberapa batasan yang
...
Program Rust onchain mendukung sebagian besar libstd, libcore, dan liballoc
...
- Tidak ada dukungan untuk `println!`, `print!`, gunakan makro
  ...
  sebagai gantinya.
- Runtime memberlakukan batasan pada jumlah instruksi yang dapat dieksekusi
  program selama pemrosesan satu instruksi. Lihat
...
Ketika program melebihi batas kedalaman call stack yang diizinkan, program akan
menerima error `CallDepthExceeded`.

## Kedalaman panggilan CPI - error `CallDepth`

Invokasi lintas-program memungkinkan program untuk memanggil program lain secara
langsung, tetapi kedalamannya saat ini dibatasi hingga `4`.

Ketika program melebihi
[kedalaman panggilan invokasi lintas-program](/docs/core/cpi) yang diizinkan,
program akan menerima error `CallDepth`
