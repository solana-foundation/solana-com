...
- **Freeze authority**: Mengontrol kemampuan untuk membekukan dan mencairkan
  token account. Dapat mencegah token account mana pun untuk mentransfer token.
  Sering dicabut untuk menjamin pengguna bahwa token mereka tidak dapat
  dibekukan.

### Token account authority

- **Account owner**: Memiliki kontrol penuh atas token account. Dapat
  mentransfer token, membakar token, menyetujui delegate, dan menutup account
  ketika saldo nol.
- **Close authority**: Dapat menutup token account ketika saldonya nol. Secara
  default, ini adalah account owner tetapi dapat didelegasikan ke account lain.
...
<Callout type="info">
  ...
  memiliki implementasi serupa untuk mencapai fungsi yang sama.
</Callout>
