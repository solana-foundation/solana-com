...
description:
  Buat mint token dan konfigurasi desimal mint, mint authority, dan freeze
  authority.
...
Mint account mendefinisikan dan mengidentifikasi secara unik sebuah token di
Solana, serta menyimpan state bersama yang berlaku untuk semua token account
untuk mint tersebut.

Token Program mendefinisikan tipe akun _rs`Mint`_ sebagai:
...
Setiap token memiliki satu mint account, dan alamat mint adalah identitas unik
token tersebut di seluruh wallet, aplikasi, dan explorer.

Sebagai contoh, USD Coin (USDC) memiliki alamat mint
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`. Alamat mint ini secara unik
...
1. Instruksi _rs`CreateAccount`_ dari System Program membuat akun baru yang
   ...
   baru tersebut.
2. Instruksi _rs`InitializeMint`_ atau _rs`InitializeMint2`_ dari Token Program
   menginisialisasi akun baru tersebut sebagai mint.

Sertakan instruksi _rs`CreateAccount`_ dan instruksi inisialisasi mint dalam
transaksi yang sama.
...
freeze authority, desimal, dan flag _rs`is_initialized`_ ke dalam data mint
account.
...
| ------------------------------ | ---------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                     | State mint dasar yang disimpan di setiap mint account.                                   | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`InitializeMint`_           | Instruksi inisialisasi mint yang memerlukan akun sysvar rent dalam daftar akunnya.       | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L24-L44)   | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L50-L74)   |
| _rs`InitializeMint2`_          | Instruksi inisialisasi mint yang tidak memerlukan akun sysvar rent dalam daftar akunnya. | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L401-L414) | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L471-L486) |
| _rs`_process_initialize_mint`_ | Logika prosesor bersama untuk inisialisasi mint.                                         | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
| _rs`process_initialize_mint`_  | Handler publik untuk _rs`InitializeMint`_.                                               | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L62-L70)       | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L132-L140)     |
| _rs`process_initialize_mint2`_ | Handler publik untuk _rs`InitializeMint2`_.                                              | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L72-L81)       | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L142-L151)     |
...
Mint account hanya menyimpan informasi token dasar (pasokan, desimal, otoritas).
Untuk menambahkan metadata yang mudah dibaca seperti nama, simbol, dan gambar ke
token Anda, Anda memiliki dua opsi:

<Cards>
  <Card title="Metaplex Token Metadata" href="/docs/tokens/metaplex">
   Tambahkan metadata menggunakan Metaplex Token Metadata Program. Berfungsi dengan Token Program asli maupun Token-2022.
  </Card>
