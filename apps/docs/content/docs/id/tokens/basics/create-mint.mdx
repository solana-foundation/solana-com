...
description:
  Membuat token mint dan mengonfigurasi desimal mint, mint authority, dan freeze
  authority.
...
Akun mint mendefinisikan dan mengidentifikasi secara unik sebuah token di
Solana, dan menyimpan state bersama yang berlaku untuk semua token account untuk
mint tersebut.
...
Setiap token memiliki satu mint account, dan alamat mint adalah identifier unik
token tersebut di seluruh dompet, aplikasi, dan explorer.
...
Mint account hanya menyimpan informasi token dasar (suplai, desimal, otoritas).
Untuk menambahkan metadata yang dapat dibaca manusia seperti nama, simbol, dan
gambar ke token Anda, Anda memiliki dua opsi:
