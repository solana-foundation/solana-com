...
Wrapped SOL (WSOL) adalah token account yang melacak jumlah lamports dalam akun
...
mensinkronkan saldo token account wrapped SOL (WSOL) dengan SOL aktual
(lamports) yang tersimpan di dalamnya. Ketika Anda mentransfer SOL native ke
token account WSOL, saldo token tidak otomatis diperbarui. Anda harus memanggil
SyncNative untuk mencerminkan saldo WSOL yang benar.
