...
title: Sync Native
description:
  Perbarui jumlah akun token SOL yang dibungkus setelah mentransfer SOL ke dalam
  akun.
...
melalui instruksi-instruksi Token Program.
...
melalui field `amount` token account. Jika Anda mentransfer SOL ke dalam token
account wrapped SOL, saldo lamport meningkat secara langsung, tetapi field
`amount` tidak berubah hingga Anda memanggil instruksi `SyncNative` Token
Program.

## Cara Menyinkronkan Native SOL

Gunakan instruksi `SyncNative` Token Program setelah mentransfer SOL ke dalam
token account wrapped SOL. Instruksi `SyncNative` memperbarui field `amount`
token account agar sesuai dengan lamport yang disimpan dalam akun di atas
cadangan bebas sewa.
...
| --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `Account`             | State token account menggunakan `is_native` untuk menandai akun sebagai WSOL dan menyimpan cadangan native, saldo lamport minimum bebas sewa yang harus dijaga akun, serta jumlah token yang dilacak. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| `SyncNative`          | Instruksi yang memperbarui field `amount` token account wrapped SOL agar sesuai dengan lamport yang disimpan dalam akun di atas cadangan bebas sewa.                                                  | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| `process_sync_native` | Logika prosesor bersama untuk menyinkronkan saldo wrapped SOL.                                                                                                                                        | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |
