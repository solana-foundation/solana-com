...
title: Sync Native
description:
  Perbarui jumlah token account SOL yang dibungkus setelah mentransfer SOL ke
  dalam akun.
...
Wrapped SOL (WSOL) adalah SOL yang disimpan dalam token account untuk native
mint Token Program. Native mint adalah alamat mint yang digunakan Token Program
untuk token account yang membungkus SOL. Native mint memungkinkan SOL digunakan
melalui instruksi Token Program.

| Program                  | Alamat native mint                             |
| ------------------------ | ---------------------------------------------- |
...
Token account wrapped SOL menyimpan SOL dalam field `lamports` akun, tetapi
melacak SOL yang disimpan dalam token account wrapped SOL sebagai saldo token
melalui field _rs`amount`_ token account. Jika Anda mentransfer SOL ke dalam
token account wrapped SOL, saldo lamport akan meningkat secara langsung, tetapi
field _rs`amount`_ tidak berubah hingga Anda memanggil instruksi
_rs`SyncNative`_ Token Program.

## Cara Menyinkronkan Native SOL

Gunakan instruksi _rs`SyncNative`_ Token Program setelah mentransfer SOL ke
dalam token account wrapped SOL. Instruksi _rs`SyncNative`_ memperbarui field
_rs`amount`_ token account agar sesuai dengan lamport yang disimpan dalam akun
di atas cadangan rent-exempt-nya.

### Referensi Source

| Item                      | Deskripsi                                                                                                                                                                                                | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_             | State token account menggunakan _rs`is_native`_ untuk menandai akun sebagai WSOL dan menyimpan cadangan native, saldo lamport minimum rent-exempt yang harus dijaga akun, dan jumlah token yang dilacak. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`SyncNative`_          | Instruksi yang memperbarui field _rs`amount`_ token account wrapped SOL agar sesuai dengan lamport yang disimpan dalam akun di atas cadangan rent-exempt-nya.                                            | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| _rs`process_sync_native`_ | Logika processor bersama untuk menyinkronkan saldo wrapped SOL.                                                                                                                                          | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |

### Typescript

Contoh-contoh `Kit` di bawah ini menunjukkan pendekatan yang direkomendasikan
