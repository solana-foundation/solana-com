...
description: Menutup akun token dan mengembalikan lamport akun ke akun tujuan.
...
Menutup akun token akan menghapus akun tersebut dan mengembalikan lamport rent
akun token ke akun tujuan.

Saldo token harus nol sebelum akun dapat ditutup. Akun yang dibekukan masih
dapat ditutup jika saldo token akun yang dibekukan adalah nol. Akun token
Wrapped SOL adalah pengecualian dan dapat ditutup dengan saldo token untuk
mengklaim kembali SOL yang mendasarinya. Pemilik akun atau close authority akun
menandatangani instruksi penutupan.
