---
title: Menutup Akun Token
description:
  Menutup akun token dan mengembalikan lamport akun ke akun tujuan.
...
## Apa yang Dilakukan Menutup Akun Token?

Menutup token account akan menghapus akun tersebut dan mentransfer semua lamport-nya,
termasuk saldo minimum penyimpanan yang dapat dikembalikan, ke akun tujuan.

Saldo token harus nol sebelum akun dapat ditutup. Akun yang dibekukan
tetap dapat ditutup jika saldo token akun yang dibekukan tersebut adalah nol. Token account
Wrapped SOL merupakan pengecualian dan dapat ditutup dengan saldo token untuk
mengambil kembali SOL yang mendasarinya. Pemilik akun atau close authority akun
menandatangani instruksi penutupan dan memilih tujuan. Akun yang membayar
untuk membuat associated token account tidak secara otomatis menerima saldo yang dipulihkan.

## Cara Menutup Akun Token

Menutup akun token menggunakan instruksi _rs`CloseAccount`_ dari Token Program.

Instruksi _rs`CloseAccount`_ mentransfer lamport akun sumber ke akun tujuan,
menghapus data akun sumber, dan menghapus akun sumber. Token Extension Program
menerapkan pemeriksaan penutupan tambahan untuk ekstensi tertentu, tetapi alur
penutupan dasar tetap sama.

### Referensi Sumber

| Item                        | Deskripsi                                                                                                     | Token Program                                                                                                  | Token Extension Program                                                                                              |
| --------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | State akun token menyimpan saldo, status native, dan close authority yang digunakan saat penutupan akun.     | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | Instruksi yang menutup akun token dan mentransfer lamport akun token ke akun tujuan. | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | Logika prosesor bersama untuk penutupan akun token.                                                               | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |
