...
description:
  Buat akun token dan associated token account untuk mint dan pemilik tertentu.
...
Token account menyimpan token untuk satu mint dan satu pemilik token account di
Solana.
...
mendelegasikan token dari token account. Pemilik program akun tetap Token
Program atau Token Extensions Program.

Setiap token account terikat pada satu mint saja, yang berarti token account
hanya dapat menyimpan unit dari satu token, yaitu token yang diidentifikasi oleh
field _rs`mint`_ pada token account.

## Apa Itu Associated Token Account

Associated token account (ATA) adalah token account default untuk dompet dan
mint.
...
Associated Token Program menurunkan alamat ATA seperti yang ditunjukkan di
...
Untuk setiap kombinasi dompet, token program, dan mint, ada tepat satu alamat
...
2. **Program owner**: Program yang memiliki account (selalu Token Program atau
   Token Extension Program untuk token account).

Saat bekerja dengan token account, "owner" biasanya merujuk pada otoritas yang
dapat mentransfer token, bukan program yang memiliki account.

</Callout>

## Apa itu associated token account?

Associated token account (ATA) adalah token account default untuk wallet yang
menyimpan token tertentu. ATA menggunakan alamat deterministik (PDA) yang dibuat
oleh
...
ATA menggunakan alamat deterministik, sehingga memudahkan untuk menemukan token
account wallet mana pun untuk mint tertentu. Lihat
[implementasi derivasinya di sini](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/interface/src/address.rs#L56).
