...
Sebuah token account menyimpan token untuk satu mint dan satu pemilik token
account di Solana.
...
mendelegasikan token dari token account. Pemilik program akun tersebut tetap
merupakan Token Program atau Token Extensions Program.

Setiap token account terikat pada tepat satu mint, yang berarti token account
hanya dapat menyimpan unit dari satu token, yaitu token yang diidentifikasi oleh
field _rs`mint`_ dari token account.

## Apa Itu Akun Token Terkait

Sebuah associated token account (ATA) adalah token account default untuk sebuah
dompet dan mint.
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program/src)
menurunkan alamat ATA dari alamat dompet, alamat program token, dan alamat mint.

Hanya akun token yang dibuat oleh Associated Token Program yang disebut sebagai
associated token account.

Associated Token Program adalah cara untuk membuat token account pada alamat
standar yang deterministik. Akun yang dihasilkan tetap merupakan token account
yang dimiliki oleh Token Program atau Token Extensions Program, bukan oleh
...
Untuk kombinasi dompet, program token, dan mint apa pun, hanya ada satu alamat
ATA. Associated Token Program membuat token account standar pada alamat
tersebut, dan akun yang dihasilkan tetap menggunakan tipe _rs`Account`_ yang
didefinisikan oleh Token Program atau Token Extensions Program.
...
menurunkan alamat ATA, membuat akun di alamat ATA, dan menginisialisasi akun
sebagai token account yang dimiliki oleh Token Program atau Token Extension
Program.

<Callout type="info" title="Direkomendasikan">
  Untuk sebagian besar aplikasi, buat token account melalui Associated Token
  Program daripada membuatnya dengan langsung memanggil instruksi System Program
  dan Token Program. Associated token account menggunakan alamat deterministik
  yang diturunkan dari pemilik, token program, dan mint, yang membuat token
  account default untuk mint tertentu lebih mudah ditemukan oleh dompet dan
  aplikasi.
</Callout>
...
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------- |
| _rs`Create`_                                  | Membuat associated token account di alamat ATA yang diturunkan.                                                                                                                                                                                                          | [Sumber](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L20-L29) |
...
| _rs`process_create_associated_token_account`_ | Menurunkan alamat ATA dan menggunakan _rs`create_pda_account`_ ditambah CPI ke token program yang dipilih untuk menginisialisasi token account. Ketika program yang dipilih adalah Token Extension Program, prosesor ATA juga menginisialisasi ekstensi immutable owner. | [Sumber](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
...
menginisialisasinya melalui cross program invocations (CPI).
