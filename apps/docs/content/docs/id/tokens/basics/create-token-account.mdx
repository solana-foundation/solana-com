...
description: Pelajari cara membuat SPL Token Account.
---

## Apa itu token account?

Token account menyimpan saldo Anda untuk token tertentu. Setiap token account
terkait dengan tepat satu mint dan melacak saldo token Anda beserta detail
tambahan.
...
<Callout type="info">
  Perhatikan bahwa dalam kode sumber, developer menyebut token account sebagai
...
1. **Mint**: Jenis token spesifik yang disimpannya
2. **Owner**: Otoritas yang dapat mentransfer token dari account ini
...
- Account ini hanya menyimpan token USDC
- Circle mengontrol token melalui otoritas owner token account:
