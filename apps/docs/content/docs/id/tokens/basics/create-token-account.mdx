---
title: Membuat Akun Token
description:
  Membuat akun token dan akun token terkait untuk mint dan pemilik tertentu.
...
Akun token menyimpan token untuk satu mint dan satu pemilik akun token di
Solana.
...
Di sini, _rs`owner`_ berarti otoritas yang dapat mentransfer, membakar, atau
mendelegasikan token dari akun token. Pemilik program akun tetap Token Program
atau Token Extensions Program.

Setiap akun token terikat pada satu mint saja, yang berarti akun token hanya
dapat menampung unit dari satu token, yaitu token yang diidentifikasi oleh field
_rs`mint`_ pada akun token.

## Apa Itu Akun Token Terkait

Akun token terkait (ATA) adalah akun token default untuk sebuah dompet dan mint.
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program/src)
menurunkan alamat ATA dari alamat dompet, alamat token program, dan alamat mint.

Hanya akun token yang dibuat oleh Associated Token Program yang disebut sebagai
akun token terkait.

Associated Token Program adalah cara untuk membuat akun token pada alamat yang
standar dan deterministik. Akun yang dihasilkan tetap merupakan akun token yang
dimiliki oleh Token Program atau Token Extensions Program, bukan oleh Associated
Token Program.

Associated Token Program menurunkan alamat ATA seperti yang ditunjukkan dalam
...
Untuk setiap kombinasi dompet, token program, dan mint, hanya ada satu alamat
ATA. Associated Token Program membuat akun token standar pada alamat tersebut,
dan akun yang dihasilkan tetap menggunakan tipe _rs`Account`_ yang didefinisikan
oleh Token Program atau Token Extensions Program.

## Biaya dan Pembayar

Membuat ATA memerlukan saldo minimum yang dapat dikembalikan untuk penyimpanan onchain akun tersebut, ditambah biaya transaksi. `payer` yang diteruskan ke instruksi pembuatan mendanai saldo penyimpanan dan harus menandatangani. Pembayar dapat berbeda dari pembayar biaya transaksi maupun dompet yang akan memiliki ATA, sehingga sebuah aplikasi dapat membuat ATA atas nama penerima.

Pada tarif penyimpanan klaster saat ini, biaya dasarnya adalah:

| Token program                             | Ukuran ATA  | Saldo penyimpanan minimum             |
| ----------------------------------------- | --------- | ----------------------------------- |
| Original Token Program                    | 165 byte | 2.039.280 lamport (0,00203928 SOL) |
| Token-2022 dengan immutable owner otomatis | 170 byte | 2.074.080 lamport (0,00207408 SOL) |

Mint Token-2022 dapat memerlukan ekstensi akun selain ekstensi _rs`ImmutableOwner`_ otomatis. ATA tersebut berukuran lebih besar dan memerlukan saldo minimum yang lebih tinggi. Associated Token Program meminta Token Program yang dipilih untuk mengetahui ukuran akun yang diperlukan dan mendanai ATA sesuai kebutuhan.

Jangan meng-hardcode saldo ini dalam logika aplikasi. Lakukan query
[`getMinimumBalanceForRentExemption`](/docs/rpc/http/getminimumbalanceforrentexemption)
untuk ukuran data yang diperlukan, atau biarkan Associated Token Program menghitung dan mendanainya saat pembuatan.

<Callout type="info" title="Saldo penyimpanan dapat dikembalikan">
  Menutup token account yang kosong akan mentransfer seluruh lamport-nya, termasuk
  saldo penyimpanan minimum, ke tujuan yang ditentukan oleh instruksi penutupan.
  Pembayar pembuatan awal tidak secara otomatis menjadi tujuan pengembalian dana. Lihat [Close Token Account](/docs/tokens/basics/close-account).
</Callout>

## Cara Membuat Associated Token Account

Membuat associated token account menggunakan instruksi _rs`Create`_ atau
_rs`CreateIdempotent`_ dari Associated Token Program. Associated Token Program
menurunkan alamat ATA, membuat akun pada alamat ATA tersebut, dan
menginisialisasi akun sebagai token account yang dimiliki oleh Token Program
atau Token Extension Program.

<Callout type="info" title="Direkomendasikan">
  Untuk sebagian besar aplikasi, buat token account melalui Associated Token
  Program alih-alih membuatnya dengan langsung memanggil instruksi System
  Program dan Token Program. Associated token account menggunakan alamat
  deterministik yang diturunkan dari pemilik, token program, dan mint, yang
  membuat token account default untuk mint tertentu lebih mudah ditemukan oleh
  wallet dan aplikasi.
</Callout>
...
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| _rs`Create`_                                  | Membuat associated token account pada alamat ATA yang diturunkan.                                                                                                                                                                                                | [Sumber](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L20-L29) |
| _rs`CreateIdempotent`_                        | Membuat associated token account, tetapi tetap berhasil jika ATA tersebut sudah ada untuk pemilik dan mint yang sama.                                                                                                                                               | [Sumber](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | Menurunkan alamat ATA dan menggunakan _rs`create_pda_account`_ ditambah CPI ke token program yang dipilih untuk menginisialisasi token account. Ketika program yang dipilih adalah Token Extension Program, processor ATA juga menginisialisasi ekstensi immutable owner. | [Sumber](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | Helper yang digunakan oleh _rs`process_create_associated_token_account`_ untuk membuat akun PDA melalui CPI ke System Program.                                                                                                                                         | [Sumber](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |

### Typescript

Contoh `Kit` di bawah ini menunjukkan pendekatan yang direkomendasikan
menggunakan `@solana/kit`. Contoh legacy menggunakan `@solana/web3.js`
...
1. Instruksi _rs`CreateAccount`_ dari System Program membuat akun baru yang
   bebas rent dan menetapkan Token Program sebagai pemilik program dari akun
   baru tersebut.
2. Instruksi _rs`InitializeAccount`_, _rs`InitializeAccount2`_, atau
   _rs`InitializeAccount3`_ dari Token Program menginisialisasi akun baru untuk
   mint dan owner.

Sertakan instruksi _rs`CreateAccount`_ dan instruksi inisialisasi token account
dalam transaksi yang sama.

Selama inisialisasi token account, Token Program memeriksa bahwa akun belum
diinisialisasi dan bebas rent.

<Callout type="info">
  Bagian di bawah ini menunjukkan cara membuat token account dengan memanggil
  langsung instruksi System Program dan Token Program. Untuk sebagian besar
  aplikasi, gunakan Associated Token Program sebagai gantinya. Gunakan panggilan
  langsung System Program dan Token Program ketika Anda memiliki alasan khusus
  untuk tidak menggunakan associated token account atau ketika Anda perlu
  membuat token account PDA khusus dengan melakukan CPI ke instruksi System
  Program dan Token Program dari program Solana Anda sendiri.
</Callout>
...
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_                     | Field dasar token account yang disimpan di setiap token account.                                                                       | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`InitializeAccount`_           | Instruksi inisialisasi token account yang mengharapkan owner dan akun rent sysvar dalam daftar akunnya.                    | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L45-L63)   | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L75-L93)   |
| _rs`InitializeAccount2`_          | Instruksi inisialisasi token account yang meneruskan owner dalam instruction data alih-alih daftar akun.                 | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L352-L365) | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L419-L433) |
| _rs`InitializeAccount3`_          | Instruksi inisialisasi token account yang meneruskan owner dalam instruction data dan tidak memerlukan akun rent sysvar. | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L377-L387) | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L445-L456) |
| _rs`_process_initialize_account`_ | Logika prosesor bersama untuk inisialisasi token account.                                                                           | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L83-L141)      | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L153-L234)     |
| _rs`process_initialize_account`_  | Handler publik untuk _rs`InitializeAccount`_.                                                                                        | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L143-L150)     | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L236-L240)     |
| _rs`process_initialize_account2`_ | Handler publik untuk _rs`InitializeAccount2`_.                                                                                       | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L152-L160)     | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L242-L246)     |
| _rs`process_initialize_account3`_ | Handler publik untuk _rs`InitializeAccount3`_.                                                                                       | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L162-L170)     | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L248-L252)     |
