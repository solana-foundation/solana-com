...
description:
  Bekukan akun token dengan otoritas pembekuan mint untuk memblokir transfer,
  pembakaran, dan perubahan delegasi.
...
## Apa yang Dilakukan oleh Pembekuan Akun Token?

Membekukan token account akan mempertahankan pemilik, mint, dan saldo yang sama,
tetapi mencegah token account dari menerima, mentransfer, atau membakar token
hingga token account dicairkan kembali.

Akun yang dibekukan masih dapat ditutup jika akun yang dibekukan memiliki saldo
token nol.

Hanya mint yang masih memiliki otoritas pembekuan yang dapat membekukan akun
token. Jika otoritas pembekuan mint dicabut, akun untuk mint tersebut tidak
dapat lagi dibekukan.

## Cara Membekukan Akun Token

Membekukan akun token menggunakan instruksi _rs`FreezeAccount`_ dari Token
Program.

Instruksi _rs`FreezeAccount`_ memverifikasi bahwa akun token termasuk dalam mint
dan bahwa penandatangan adalah otoritas pembekuan mint, kemudian mengubah status
akun token dari initialized menjadi frozen.

[Akun token native](/docs/tokens/basics/sync-native) tidak mendukung pembekuan.

### Referensi Sumber

| Item                                | Deskripsi                                                                                     | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ----------------------------------- | --------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Status mint menyimpan otoritas pembekuan opsional yang digunakan untuk membekukan akun token. | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Status akun token, termasuk _rs`Initialized`_ dan _rs`Frozen`_.                               | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | Instruksi yang mengubah akun token menjadi status frozen.                                     | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
| _rs`process_toggle_freeze_account`_ | Logika prosesor bersama untuk membekukan dan mencairkan akun token.                           | [Sumber](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |
