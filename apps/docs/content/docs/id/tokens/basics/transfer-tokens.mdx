...
description:
  Pindahkan token antar token account untuk mint yang sama dengan instruksi
...
## Apa yang Dilakukan oleh Transfer Token?

Transfer token memindahkan token dari satu token account ke token account lain
untuk mint yang sama.

Transfer tidak mengubah pasokan mint. Transfer hanya memperbarui saldo antar
token account.

## Cara Transfer Token

Transfer token menggunakan instruksi _rs`Transfer`_ atau _rs`TransferChecked`_
dari Token Program.

Contoh di bawah ini menggunakan _rs`TransferChecked`_, yang mengharuskan
pemanggil menyediakan mint dan desimal sehingga instruksi dapat memverifikasi
mint yang diharapkan dan presisi token sebelum memindahkan token antar akun.

Di Token Extension Program, _rs`Transfer`_ sudah ditinggalkan dan digantikan
dengan _rs`TransferChecked`_ atau _rs`TransferCheckedWithFee`_.

Pemilik source account atau delegasi yang disetujui menandatangani transfer. Di
Token Extension Program, permanent delegate mint juga dapat mengotorisasi
transfer jika mint memiliki ekstensi permanent delegate yang diaktifkan.

### Referensi Sumber

| Item                   | Deskripsi                                                                                                                                                                                             | Token Program                                                                                                  | Token Extension Program                                                                                              |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_          | State token account menyimpan saldo yang diperbarui oleh transfer.                                                                                                                                    | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | Instruksi transfer yang memindahkan token antar token account tanpa mengharuskan pemanggil menyediakan mint atau desimal. Di Token Extension Program, gunakan _rs`TransferChecked`_ sebagai gantinya. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Instruksi transfer yang mengharuskan pemanggil menyediakan mint dan desimal serta memeriksa nilai tersebut sebelum memindahkan token antar token account dari mint yang sama.                         | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | Logika prosesor bersama untuk transfer token.                                                                                                                                                         | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |
