...
Transfer token memindahkan token dari satu token account ke token account
lainnya untuk mint yang sama.

Transfer tidak mengubah supply mint. Transfer hanya memperbarui saldo antar
token account.
...
pemanggil untuk menyediakan mint dan desimal sehingga instruksi dapat
memverifikasi mint yang diharapkan dan presisi token sebelum memindahkan token
antar akun.

Dalam Token Extensions Program, _rs`Transfer`_ sudah tidak digunakan lagi dan
digantikan oleh _rs`TransferChecked`_ atau _rs`TransferCheckedWithFee`_.

Pemilik akun sumber atau delegasi yang disetujui menandatangani transfer. Dalam
Token Extensions Program, delegasi permanen mint juga dapat mengotorisasi
transfer jika mint memiliki ekstensi delegasi permanen yang diaktifkan.
