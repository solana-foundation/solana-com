---
title: Mint token
description: Pelajari cara mint unit baru dari sebuah token.
---

## Cara mint token

Minting membuat unit baru dari sebuah token menggunakan instruksi
...
- Hanya mint authority yang dapat mint token baru
- Token account tujuan harus sudah ada untuk menerima token yang di-mint
...
<Callout type="info">
  ...
  memiliki implementasi yang serupa untuk mencapai fungsi yang sama.
</Callout>
...
async fn main() -> Result<()> {
    ...
    // Dapatkan ukuran default mint account (dalam bytes), tanpa ekstensi yang diaktifkan
    let mint_space = Mint::LEN;
    let mint_rent = client
        .get_minimum_balance_for_rent_exemption(mint_space)
        .await?;

    // Instruksi untuk membuat account baru untuk mint (token program)
    let create_account_instruction = create_account(
        &fee_payer.pubkey(), // payer
        &mint.pubkey(),      // account baru (mint)
        mint_rent,           // lamports
    ...
    // Jumlah token yang akan di-mint (100 token dengan 2 tempat desimal)
    let amount = 100;

    // Buat instruksi mint_to untuk mint token ke associated token account
    ...
    // Buat transaksi untuk minting token
    let transaction = Transaction::new_signed_with_payer(
        &[mint_to_instruction],
        Some(&fee_payer.pubkey()),
        &[&fee_payer],
    ...
    println!("Minted 1.00 token ke associated token account");
...
async def main():
    ...
    # Jumlah untuk mint (dalam unit terkecil)
    amount_to_mint = 1000000000  # 1 token dengan 9 desimal
    ...
    async with rpc:
        ...
        print(f"Transaksi mint berhasil dibuat")
