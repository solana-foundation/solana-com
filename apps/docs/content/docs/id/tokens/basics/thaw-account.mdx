...
description:
  Mencairkan token account yang dibekukan agar transfer, pembakaran, dan
  perubahan delegasi dapat dilanjutkan kembali.
...
## Apa yang Dilakukan Pencairan Token Account?
...
lagi. Hanya freeze authority dari mint yang dapat mencairkan token account.

## Cara Mencairkan Token Account
...
tersebut dan bahwa penandatangan adalah freeze authority dari mint, kemudian
mengubah status token account dari beku kembali ke terinisialisasi.

[Token account native](/docs/tokens/basics/sync-native) tidak mendukung
pencairan.
