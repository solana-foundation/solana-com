...
title: CPI Guard
description:
  Aktifkan ekstensi akun CpiGuard untuk memblokir instruksi token yang dipanggil
  melalui cross-program invocation.
...
Ekstensi akun _rs`CpiGuard`_ dari Token Extensions Program mencegah program
onchain lain menggunakan token account untuk instruksi token tertentu melalui
cross-program invocation (CPI).

Ketika CPI guard diaktifkan, Token Extensions Program menolak
instruksi-instruksi berikut ketika program onchain lain memanggilnya melalui
CPI:

- _rs`Transfer`_, _rs`TransferChecked`_, dan _rs`TransferCheckedWithFee`_ ketika
  pemilik token account digunakan sebagai otoritas instruksi
- _rs`Burn`_ dan _rs`BurnChecked`_ ketika pemilik token account digunakan
  sebagai otoritas instruksi
- _rs`Approve`_ dan _rs`ApproveChecked`_
- _rs`CloseAccount`_ ketika lamport akan dikirim ke tempat lain selain pemilik
  token account
- _rs`SetAuthority`_ ketika menambahkan atau mengganti otoritas penutupan

Ketika CPI guard diaktifkan, kasus-kasus berikut masih diizinkan:

- _rs`Transfer`_, _rs`TransferChecked`_, _rs`TransferCheckedWithFee`_,
  _rs`Burn`_, dan _rs`BurnChecked`_ melalui CPI ketika delegasi atau delegasi
  permanen menandatangani alih-alih pemilik token account
- _rs`CloseAccount`_ melalui CPI ketika lamport dikirim ke pemilik token account
- _rs`SetAuthority`_ untuk menghapus otoritas penutupan yang sudah ada
- _rs`Revoke`_

Ketika pemilik token account digunakan sebagai otoritas instruksi, instruksi
Token Extensions Program masih dapat diproses jika instruksi ditambahkan
langsung ke transaksi alih-alih dipanggil melalui CPI. Mengubah pemilik token
account dengan _rs`SetAuthority`_ tetap diblokir selama CPI guard diaktifkan,
bahkan di luar CPI. _rs`EnableCpiGuard`_ dan _rs`DisableCpiGuard`_ juga tidak
dapat dipanggil melalui CPI.

## Cara Mengaktifkan CPI Guard

Untuk mengaktifkan CPI guard:

1. Buat dan inisialisasi mint.
2. Hitung ukuran token account dan rent yang diperlukan untuk token account
   dengan _rs`CpiGuard`_.
3. Buat dan inisialisasi token account untuk mint.
4. Panggil _rs`EnableCpiGuard`_ pada token account.
5. Panggil _rs`DisableCpiGuard`_ pada token account untuk mengizinkan instruksi
   tersebut melalui CPI lagi.

<ScrollyCoding>

## !!steps Buat dan inisialisasi mint

Buat dan inisialisasi mint yang akan digunakan oleh token account.
...
Hitung ukuran token account untuk token account dasar ditambah ekstensi
_rs`CpiGuard`_. Ini adalah ukuran yang digunakan dalam _rs`CreateAccount`_.
...
## !!steps Buat dan inisialisasi token account

Buat token account dengan space dan lamport yang telah dihitung, kemudian
inisialisasi untuk mint.
...
## !!steps Aktifkan CPI guard

Aktifkan _rs`CpiGuard`_ pada token account.
...
## !!steps Nonaktifkan CPI guard

Nonaktifkan _rs`CpiGuard`_ pada token account.
...
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`CpiGuard`_                     | Ekstensi akun yang menyimpan informasi apakah CPI guard saat ini diaktifkan untuk token account.                                                                                                                                        | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/mod.rs#L20-L25)         |
| _rs`CpiGuardInstruction::Enable`_  | Instruksi yang mengaktifkan CPI guard pada token account.                                                                                                                                                                               | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/instruction.rs#L20-L38) |
| _rs`CpiGuardInstruction::Disable`_ | Instruksi yang menonaktifkan CPI guard pada token account.                                                                                                                                                                              | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/instruction.rs#L39-L53) |
| _rs`process_toggle_cpi_guard`_     | Logika prosesor yang digunakan oleh _rs`Enable`_ dan _rs`Disable`_ yang memvalidasi kepemilikan akun, menolak perubahan CPI terhadap guard itu sendiri, dan menginisialisasi _rs`CpiGuard`_ jika belum ada sebelum mengatur `lock_cpi`. | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/cpi_guard/processor.rs#L19-L52)     |
