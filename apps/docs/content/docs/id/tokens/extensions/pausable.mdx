...
title: Pausable mint
description:
  Buat mint Token-2022 dengan pause authority, blokir transfer saat mint dijeda,
  dan lanjutkan aktivitas nanti.
...
## Apa Itu Mint yang Dapat Dijeda?

Ekstensi mint _rs`PausableConfig`_ dari Token Extensions Program memberikan
pause authority kemampuan untuk menjeda dan melanjutkan aktivitas token untuk
seluruh mint.

Ketika mint dijeda, Token Extensions Program menolak:

- Transfer
- Mint
- Burn

Ketika mint dilanjutkan, instruksi-instruksi tersebut akan bekerja normal
kembali.

## Cara Membuat, Menjeda, dan Melanjutkan Mint

Untuk membuat, menjeda, dan melanjutkan mint:
...
2. Buat mint account dengan _rs`CreateAccount`_, inisialisasi
   _rs`PausableConfig`_, dan inisialisasi mint dengan _rs`InitializeMint`_.
3. Mint token dan buat token account untuk mint tersebut.
4. Gunakan _rs`Pause`_ untuk memblokir aktivitas token untuk mint.
5. Gunakan _rs`Resume`_ untuk mengizinkan aktivitas token kembali.
...
_rs`PausableConfig`_. Ini adalah ukuran yang digunakan dalam
_rs`CreateAccount`_.
...
## !!steps Inisialisasi PausableConfig

Inisialisasi ekstensi _rs`PausableConfig`_ pada mint.
...
## !!steps Lanjutkan mint

Lanjutkan mint dengan _rs`Resume`_.
...
<Callout type="info" title="Urutan Instruksi">
  _rs`PausableInstruction::Initialize`_ harus dijalankan sebelum
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`PausableInstruction::Initialize`_, dan _rs`InitializeMint`_ harus
...
| ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PausableConfig`_                  | Ekstensi mint yang menyimpan otoritas jeda dan apakah mint saat ini dijeda.                                                 | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/mod.rs#L17-L22)         |
| _rs`PausableInstruction::Initialize`_ | Instruksi yang menginisialisasi konfigurasi pausable sebelum _rs`InitializeMint`_.                                          | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L20-L32) |
| _rs`PausableInstruction::Pause`_      | Instruksi yang menandai mint sebagai dijeda.                                                                                | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L33-L44) |
| _rs`PausableInstruction::Resume`_     | Instruksi yang menandai mint sebagai tidak dijeda.                                                                          | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L45-L56) |
| _rs`process_initialize`_              | Logika pemroses yang menginisialisasi _rs`PausableConfig`_ pada mint yang belum diinisialisasi dan menyimpan otoritas jeda. | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L22-L35)     |
| _rs`process_toggle_pause`_            | Logika pemroses yang memverifikasi otoritas jeda sebelum mengubah status jeda mint.                                         | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L39-L64)     |
