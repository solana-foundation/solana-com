...
description:
  Buat mint dengan biaya transfer, pindahkan biaya yang ditahan dari akun token
  ke mint, dan tarik ke akun token penerima biaya.
...
## Apa Itu Biaya Transfer?

Ekstensi mint _rs`TransferFeeConfig`_ dari Token Extensions Program menerapkan
biaya pada setiap transfer untuk mint tersebut.

Saat transfer dilakukan:

- Jumlah yang ditransfer dikurangi dengan biaya yang dikonfigurasi
- Biaya yang ditahan dilacak pada token account tujuan
- Biaya yang ditahan dapat ditarik langsung dari token account atau dikumpulkan
  ke mint
- `withdraw_withheld_authority` dapat memindahkan biaya yang dikumpulkan ke
  token account penerima biaya

<Callout type="info" title="Ekstensi Token Account">
  Ketika token account diinisialisasi untuk mint dengan _rs`TransferFeeConfig`_,
  token account tersebut diinisialisasi dengan _rs`TransferFeeAmount`_. Ketika
  token account dibuat melalui [Associated Token
  Program](/docs/tokens/basics/create-token-account#how-to-create-an-associated-token-account),
  ukuran akun yang diperlukan dihitung dan token account dibuat dengan ukuran
  yang diperlukan dan lamport bebas rent.
</Callout>

## Cara Membuat Mint dengan Biaya Transfer

Untuk membuat mint dengan biaya transfer:

1. Hitung ukuran mint account dan rent yang diperlukan untuk mint dan ekstensi
   _rs`TransferFeeConfig`_.
2. Buat mint account dengan _rs`CreateAccount`_, inisialisasi
   _rs`TransferFeeConfig`_, dan inisialisasi mint dengan _rs`InitializeMint`_.
3. Buat token account untuk mint. _rs`TransferFeeAmount`_ secara otomatis
   diaktifkan untuk token account.
4. Gunakan _rs`TransferCheckedWithFee`_ untuk mentransfer token dan
   memverifikasi biaya yang diharapkan. _rs`TransferChecked`_ juga berfungsi dan
   secara otomatis menahan biaya yang dikonfigurasi pada token account tujuan.
5. Gunakan _rs`WithdrawWithheldTokensFromAccounts`_ untuk memindahkan biaya yang
   ditahan langsung dari token account ke token account penerima biaya,
   ditandatangani oleh `withdraw_withheld_authority` mint.
6. Gunakan _rs`HarvestWithheldTokensToMint`_ tanpa izin untuk memindahkan biaya
   yang ditahan dari token account ke mint account, lalu gunakan
   _rs`WithdrawWithheldTokensFromMint`_ untuk menarik biaya yang ditahan mint
   account ke token account penerima biaya, ditandatangani oleh
   `withdraw_withheld_authority` mint.
7. Gunakan _rs`SetTransferFee`_ untuk memperbarui konfigurasi biaya transfer
   berikutnya, yang akan berlaku mulai dua epoch kemudian.

<ScrollyCoding>

## !!steps Hitung ukuran akun

Hitung ukuran akun mint untuk mint dasar ditambah ekstensi
_rs`TransferFeeConfig`_. Ini adalah ukuran yang digunakan dalam
_rs`CreateAccount`_.
...
## !!steps Buat akun mint

Buat akun mint dengan ruang dan lamport yang telah dihitung.
...
## !!steps Inisialisasi TransferFeeConfig

Inisialisasi ekstensi _rs`TransferFeeConfig`_ pada mint.
...
## !!steps Buat token account dan mint token

Buat token account untuk sumber, penerima, dan penerima biaya, kemudian mint
token ke token account sumber.
...
## !!steps Transfer dengan TransferCheckedWithFee

Transfer token dengan _rs`TransferCheckedWithFee`_ dan verifikasi biaya yang
diharapkan.
...
## !!steps Transfer dengan TransferChecked

Transfer token dengan _rs`TransferChecked`_. Biaya yang dikonfigurasi tetap
ditahan pada token account tujuan.
...
## !!steps Tarik biaya yang ditahan dari token account

Gunakan _rs`WithdrawWithheldTokensFromAccounts`_ untuk memindahkan biaya yang
ditahan langsung dari token account ke token account penerima biaya.
...
## !!steps Kumpulkan biaya yang ditahan ke mint

Gunakan _rs`HarvestWithheldTokensToMint`_ untuk memindahkan biaya yang ditahan
dari token account ke akumulator yang ditahan di mint account.
...
## !!steps Tarik biaya yang ditahan dari mint

Gunakan _rs`WithdrawWithheldTokensFromMint`_ untuk memindahkan biaya yang
terkumpul dari mint account ke token account penerima biaya.
...
## !!steps Perbarui biaya transfer berikutnya

Gunakan _rs`SetTransferFee`_ untuk memperbarui konfigurasi biaya transfer
berikutnya.
...
<Callout type="info" title="Urutan Instruksi">
  _rs`InitializeTransferFeeConfig`_ harus dijalankan sebelum
  _rs`InitializeMint`_. _rs`CreateAccount`_, _rs`InitializeTransferFeeConfig`_,
  dan _rs`InitializeMint`_ harus disertakan dalam transaksi yang sama.
</Callout>
...
| ---------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`TransferFeeConfig`_                                          | Ekstensi mint yang menyimpan otoritas biaya, jumlah yang ditahan terakumulasi, serta konfigurasi biaya transfer lama dan baru.                                                                                                    | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L128-L145)         |
| _rs`TransferFee`_                                                | Struct konfigurasi biaya yang disimpan dalam _rs`TransferFeeConfig`_ yang menentukan epoch saat biaya berlaku, biaya maksimum, dan biaya dalam basis poin.                                                                        | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L32-L40)           |
| _rs`TransferFeeAmount`_                                          | Ekstensi token account yang menyimpan jumlah yang ditahan selama transfer untuk sebuah token account.                                                                                                                             | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L169-L177)         |
| _rs`TransferFeeInstruction::InitializeTransferFeeConfig`_        | Instruksi yang menginisialisasi pengaturan biaya transfer sebelum _rs`InitializeMint`_.                                                                                                                                           | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L24-L48)   |
| _rs`TransferFeeInstruction::TransferCheckedWithFee`_             | Instruksi transfer yang memverifikasi bahwa biaya yang diberikan pemanggil sesuai dengan konfigurasi biaya transfer mint saat ini.                                                                                                | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L49-L80)   |
| _rs`TransferFeeInstruction::WithdrawWithheldTokensFromAccounts`_ | Instruksi yang memindahkan biaya yang ditahan langsung dari token account ke token account penerima biaya, ditandatangani oleh `withdraw_withheld_authority` mint.                                                                | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L99-L122)  |
| _rs`TransferFeeInstruction::HarvestWithheldTokensToMint`_        | Instruksi tanpa izin yang memindahkan biaya yang ditahan dari token account ke akumulator biaya yang ditahan di mint account.                                                                                                     | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L123-L134) |
| _rs`TransferFeeInstruction::WithdrawWithheldTokensFromMint`_     | Instruksi yang menarik biaya yang ditahan dari mint account ke token account penerima biaya, ditandatangani oleh `withdraw_withheld_authority` mint.                                                                              | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L81-L98)   |
| _rs`TransferFeeInstruction::SetTransferFee`_                     | Instruksi yang memperbarui konfigurasi biaya transfer yang lebih baru, yang berlaku mulai dua epoch kemudian.                                                                                                                     | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L135-L154) |
| _rs`process_initialize_transfer_fee_config`_                     | Logika pemroses yang menginisialisasi ekstensi _rs`TransferFeeConfig`_ pada mint yang belum diinisialisasi dan menetapkan konfigurasi biaya transfer lama dan baru ke biaya awal.                                                 | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L26-L58)       |
| _rs`process_withdraw_withheld_tokens_from_accounts`_             | Logika pemroses yang memvalidasi `withdraw_withheld_authority` dan memindahkan biaya yang ditahan dari token account ke token account tujuan.                                                                                     | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L208-L278)     |
| _rs`process_harvest_withheld_tokens_to_mint`_                    | Logika pemroses yang mengumpulkan biaya yang ditahan dari token account ke akumulator biaya yang ditahan di mint account.                                                                                                         | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L181-L206)     |
| _rs`process_withdraw_withheld_tokens_from_mint`_                 | Logika pemroses yang memvalidasi `withdraw_withheld_authority` dan memindahkan jumlah yang ditahan dari mint account ke token account tujuan.                                                                                     | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L114-L158)     |
| _rs`process_set_transfer_fee`_                                   | Logika pemroses yang memvalidasi otoritas konfigurasi biaya transfer dan memperbarui konfigurasi biaya transfer yang lebih baru untuk berlaku mulai dua epoch kemudian.                                                           | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L61-L112)      |
| _rs`get_required_init_account_extensions`_                       | Digunakan untuk secara otomatis menambahkan ekstensi token account saat token account diinisialisasi berdasarkan ekstensi yang diaktifkan pada mint. Untuk mint _rs`TransferFeeConfig`_, ini menambahkan _rs`TransferFeeAmount`_. | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mod.rs#L1301-L1303)                    |

### Typescript

Contoh `Kit` di bawah ini menggunakan instruksi Token-2022 eksplisit. Contoh
lama menggunakan `@solana/web3.js` dan `@solana/spl-token` disertakan sebagai
referensi.
