...
title: Transfer Memo
description:
  Mengharuskan transfer masuk untuk menyertakan memo dengan mengaktifkan
  ekstensi akun MemoTransfer pada akun token Token Extensions Program.
...
## Apa Itu Transfer Memo?

Ekstensi akun _rs`MemoTransfer`_ pada Token Extensions Program mengharuskan
adanya instruksi memo segera sebelum transfer masuk ke token account tersebut.
Transfer keluar dari token account tersebut tidak memerlukan memo.

Ekstensi ini dikonfigurasi pada token account tujuan, bukan pada mint. Setelah
diaktifkan, transfer tanpa memo akan gagal dengan _rs`TokenError::NoMemo`_
sampai pemilik token account menonaktifkan persyaratan tersebut.

## Cara Membuat Token Account dengan MemoTransfer Diaktifkan

Untuk membuat token account dengan _rs`MemoTransfer`_ diaktifkan:

1. Buat dan inisialisasi mint.
2. Hitung ukuran token account dan rent yang diperlukan untuk token account
   dengan _rs`MemoTransfer`_.
3. Buat token account dan aktifkan _rs`MemoTransfer`_ pada akun tersebut.
4. Instruksi _rs`Transfer`_ dan _rs`TransferChecked`_ ke token account tersebut
   harus menyertakan instruksi memo segera sebelum instruksi transfer.
...
_rs`MemoTransfer`_. Ini adalah ukuran yang digunakan dalam _rs`CreateAccount`_.
...
Buat token account dengan ruang dan lamport yang telah dihitung, lalu
...
## !!steps Aktifkan MemoTransfer

Aktifkan _rs`MemoTransfer`_ pada token account.
...
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`MemoTransfer`_                              | Ekstensi akun yang menyimpan apakah transfer masuk memerlukan memo.                                                     | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/mod.rs#L17-L22)         |
| _rs`RequiredMemoTransfersInstruction::Enable`_  | Instruksi yang mengaktifkan transfer dengan memo wajib pada token account.                                              | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L50-L71) |
| _rs`RequiredMemoTransfersInstruction::Disable`_ | Instruksi yang menonaktifkan transfer dengan memo wajib pada token account.                                             | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L74-L95) |
| _rs`process_toggle_required_memo_transfers`_    | Logika prosesor yang menginisialisasi atau menulis ulang ekstensi _rs`MemoTransfer`_ setelah memverifikasi kepemilikan. | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/memo_transfer/processor.rs#L20-L48)     |

### Typescript

Contoh `Kit` di bawah ini menggunakan instruksi yang dihasilkan secara langsung.
Contoh legacy menggunakan `@solana/web3.js`, `@solana/spl-token`, dan
`@solana/spl-memo` disertakan sebagai referensi.
