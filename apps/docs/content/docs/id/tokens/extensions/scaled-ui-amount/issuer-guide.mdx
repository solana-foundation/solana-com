...
Untuk memperbarui multiplier jumlah UI, Anda perlu menggunakan perintah
`update-ui-amount-multiplier`. Timestamp, dalam detik sejak Unix epoch, bersifat
opsional dan dapat digunakan untuk mengatur waktu mulai kustom untuk multiplier
baru. Jika tidak ada timestamp yang diberikan, timestamp saat ini akan
digunakan.
