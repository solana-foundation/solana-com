...
Ekstensi mint _rs`ScaledUiAmount`_ dari Token Extensions Program memungkinkan
...
_rs`ScaledUiAmount`_ memungkinkan dompet dan aplikasi menampilkan jumlah UI baru
menggunakan pengali dari mint.

Fitur ini juga dapat digunakan untuk dividen atau mendistribusikan hasil.

Tidak ada token baru yang dibuat. Jumlah token yang tersimpan dalam token
account tetap sama. Hanya jumlah UI yang ditampilkan yang berubah.

<Callout type="info" title="Ekstensi Tidak Kompatibel">
  Jangan aktifkan _rs`ScaledUiAmount`_ dan _rs`InterestBearingConfig`_ pada mint
  yang sama. Token Extensions Program menolak kombinasi ekstensi tersebut.
</Callout>

## Cara Kerja Scaled UI Amount

- _rs`ScaledUiAmountConfig`_ menyimpan otoritas pembaruan, `multiplier`,
  `new_multiplier`, dan `new_multiplier_effective_timestamp`.
- Sebelum `new_multiplier_effective_timestamp`, konversi menggunakan
  `multiplier`. Pada atau setelah timestamp tersebut, konversi menggunakan
  `new_multiplier`.
- _rs`UpdateMultiplier`_ dapat menerapkan pengali baru secara langsung atau
  menjadwalkannya untuk timestamp Unix di masa mendatang.
- _rs`AmountToUiAmount`_, _rs`UiAmountToAmount`_, dan helper konversi ekstensi
  menggunakan aritmatika floating-point untuk scaled UI amount mint, sehingga
  konversi tidak dijamin untuk round-trip secara tepat.
...
2. Buat mint account dengan _rs`CreateAccount`_, inisialisasi
   _rs`ScaledUiAmount`_, dan inisialisasi mint dengan _rs`InitializeMint`_.
3. Gunakan _rs`AmountToUiAmount`_, _rs`UiAmountToAmount`_, atau helper klien
   saat menampilkan saldo atau mengonversi jumlah UI.
4. Gunakan _rs`UpdateMultiplier`_ untuk memperbarui pengali yang ditampilkan
   secara langsung atau pada timestamp Unix di masa mendatang.

<Callout type="info" title="Urutan Instruksi">
  _rs`ScaledUiAmountMintInstruction::Initialize`_ harus dilakukan sebelum
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`ScaledUiAmountMintInstruction::Initialize`_, dan _rs`InitializeMint`_
...
<Cards>
  ...
  >
    Konversi jumlah token untuk ditampilkan dan dukung Scaled UI Amount di
    dompet serta aplikasi.
  </Card>
</Cards>
...
| ----------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`ScaledUiAmountConfig`_                            | Status ekstensi mint yang menyimpan otoritas, pengali saat ini, pengali berikutnya, dan timestamp saat mulai berlaku.         | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L41-L55)         |
| _rs`ScaledUiAmountMintInstruction::Initialize`_       | Instruksi yang menginisialisasi _rs`ScaledUiAmount`_ pada mint sebelum _rs`InitializeMint`_.                                  | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/instruction.rs#L24-L42) |
| _rs`ScaledUiAmountMintInstruction::UpdateMultiplier`_ | Instruksi yang menetapkan pengali baru dan timestamp Unix saat mulai berlaku.                                                 | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/instruction.rs#L43-L66) |
| _rs`amount_to_ui_amount`_                             | Pembantu yang mengonversi jumlah token menjadi string jumlah UI menggunakan pengali saat ini, desimal, dan timestamp.         | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L69-L84)         |
| _rs`try_ui_amount_into_amount`_                       | Pembantu yang mengonversi string jumlah UI kembali menjadi jumlah token menggunakan pengali saat ini, desimal, dan timestamp. | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L86-L107)        |
| _rs`process_initialize`_                              | Prosesor yang menginisialisasi _rs`ScaledUiAmountConfig`_ dan menyimpan pengali awal pada mint yang belum diinisialisasi.     | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/scaled_ui_amount/processor.rs#L37-L57)     |
| _rs`process_update_multiplier`_                       | Prosesor yang memvalidasi otoritas, memperbarui pengali terjadwal, dan menerapkannya segera bila sesuai.                      | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/scaled_ui_amount/processor.rs#L59-L108)    |
| _rs`process_amount_to_ui_amount`_                     | Jalur konversi Token Program yang menggunakan _rs`ScaledUiAmountConfig`_ ketika mint memiliki ekstensi yang diaktifkan.       | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1548-L1571)                            |
| _rs`process_ui_amount_to_amount`_                     | Jalur konversi Token Program yang mengonversi jumlah UI kembali menjadi jumlah token untuk mint scaled UI amount.             | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1576-L1595)                            |
