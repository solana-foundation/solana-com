...
description:
  Konfigurasikan mint Token Extensions Program agar akun token yang baru dibuat
  dimulai dalam status default yang dipilih, seperti beku.
...
## Apa Itu Status Akun Default?

Ekstensi mint _rs`DefaultAccountState`_ pada Token Extensions Program menetapkan
_rs`AccountState`_ awal untuk setiap token account baru yang dibuat untuk mint
tersebut.

Ini paling sering digunakan untuk membuat token account baru dimulai dalam
status beku. Token account yang dimulai dalam status beku tidak dapat digunakan
sampai freeze authority dari mint mencairkannya.

## Cara Membuat Mint dengan Status Akun Default

Untuk membuat mint dengan status akun default:
...
2. Buat mint account dengan _rs`CreateAccount`_, inisialisasi
   _rs`DefaultAccountState`_, dan inisialisasi mint dengan _rs`InitializeMint`_.
3. Buat token account untuk mint dan perhatikan bahwa setiap token account baru
   dimulai dalam status default saat ini.
4. Gunakan _rs`UpdateDefaultAccountState`_ untuk mengubah status yang seharusnya
   dimulai oleh token account di masa mendatang.
...
_rs`DefaultAccountState`_. Ini adalah ukuran yang digunakan dalam
_rs`CreateAccount`_.
...
## !!steps Inisialisasi DefaultAccountState

Inisialisasi ekstensi _rs`DefaultAccountState`_ pada mint.
...
Buat token account untuk mint. Karena status default saat ini adalah
_rs`Frozen`_, token account baru dimulai dalam keadaan frozen.
...
## !!steps Perbarui status default account

Gunakan _rs`UpdateDefaultAccountState`_ untuk mengubah status default agar token
account di masa mendatang tidak lagi dimulai dalam keadaan frozen.
...
<Callout type="info" title="Urutan Instruksi">
  _rs`DefaultAccountStateInstruction::Initialize`_ harus dilakukan sebelum
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`DefaultAccountStateInstruction::Initialize`_, dan _rs`InitializeMint`_
...
| ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`DefaultAccountState`_                        | Ekstensi mint yang menyimpan status yang harus diwariskan oleh token account baru.                             | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/mod.rs#L16-L21)         |
| _rs`DefaultAccountStateInstruction::Initialize`_ | Instruksi yang menginisialisasi status default token account pada mint sebelum _rs`InitializeMint`_.           | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L20-L36) |
| _rs`DefaultAccountStateInstruction::Update`_     | Instruksi yang memungkinkan freeze authority mengubah status default untuk token account di masa mendatang.    | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L37-L53) |
| _rs`process_initialize_default_account_state`_   | Logika processor yang menulis status default awal pada mint yang belum diinisialisasi.                         | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L29-L40)     |
| _rs`process_update_default_account_state`_       | Logika processor yang memverifikasi freeze authority sebelum mengubah status default yang tersimpan pada mint. | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L43-L74)     |

### Typescript

Contoh `Kit` di bawah ini menggunakan instruksi yang dihasilkan secara langsung.
Contoh legacy menggunakan `@solana/web3.js` dan `@solana/spl-token` disertakan
sebagai referensi.
