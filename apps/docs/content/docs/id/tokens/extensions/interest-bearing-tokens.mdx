...
description:
  Buat mint Token Extensions Program dengan tingkat bunga onchain, perbarui tingkat tersebut, dan konversi jumlah token menjadi jumlah UI dengan bunga.
...
Ekstensi mint _rs`InterestBearingConfig`_ pada Token Extensions Program
memungkinkan mint menyimpan tingkat bunga tahunan secara langsung di onchain.

Token berbunga **tidak** menambahkan lebih banyak token ke token account dari
waktu ke waktu. Jumlah token yang tersimpan di setiap token account tetap sama
hingga ada instruksi program token yang mengubahnya, seperti minting, transfer,
atau burning.

Seiring berjalannya waktu, jumlah UI yang dihitung dengan bunga dapat meningkat
meskipun jumlah token dan pasokan token tetap sama.
...
- [`InterestBearingConfig`](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L40-L50)
  menyimpan `initialization_timestamp`, `pre_update_average_rate`,
  `last_update_timestamp`, dan `current_rate`.
- [`amount_to_ui_amount`](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L85-L94)
  menghitung jumlah UI dalam dua langkah: pertama menghitung bunga dari
  inisialisasi hingga `last_update_timestamp` menggunakan
  `pre_update_average_rate`, kemudian menghitung bunga dari
  `last_update_timestamp` hingga timestamp saat ini menggunakan `current_rate`.
- Ketika
  ...
  dan memperbarui `pre_update_average_rate` menjadi rata-rata historis
  tertimbang waktu yang baru, kemudian mencatat `last_update_timestamp` dan
  `current_rate` yang baru.
- Setelah perubahan tingkat dari waktu ke waktu, perubahan yang lebih awal
  direpresentasikan oleh `pre_update_average_rate`, sementara periode dari
  `last_update_timestamp` hingga waktu saat ini dihitung dengan `current_rate`.
...
2. Buat mint account dengan _rs`CreateAccount`_, inisialisasi
   _rs`InterestBearingConfig`_, dan inisialisasi mint dengan
   _rs`InitializeMint`_.
3. Mint token seperti biasa.
4. Gunakan _rs`UpdateRate`_ untuk mengubah tingkat saat ini dari mint dari waktu
   ke waktu.
5. Konversi jumlah token menjadi jumlah UI dengan bunga menggunakan
   _rs`AmountToUiAmount`_ atau metode helper yang mengambil mint account dan
   clock sysvar, kemudian menghitung jumlah UI tanpa mengirim transaksi.

<Callout type="info" title="Konversi Jumlah UI Offchain dan Onchain">
  Jalur helper offchain mengambil mint account dan clock sysvar, lalu menghitung
  jumlah UI secara lokal tanpa mengirim transaksi. Jalur onchain menggunakan
  _rs`AmountToUiAmount`_, yang berjalan di Token Program dan mengembalikan
  jumlah UI dalam data return transaksi. _rs`AmountToUiAmount`_ dapat dikirim
  dalam transaksi atau disimulasikan.
</Callout>
...
Hitung ukuran mint account untuk base mint ditambah ekstensi
_rs`InterestBearingConfig`_. Ini adalah ukuran yang digunakan dalam
_rs`CreateAccount`_.
...
## !!steps Inisialisasi InterestBearingConfig

Inisialisasi ekstensi _rs`InterestBearingConfig`_ pada mint.
...
## !!steps Hitung jumlah UI dengan bantuan offchain

Ambil akun mint dan clock sysvar, lalu hitung jumlah UI tanpa mengirim
transaksi.
...
<Callout type="info" title="Urutan Instruksi">
  _rs`InterestBearingMintInstruction::Initialize`_ harus datang sebelum
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`InterestBearingMintInstruction::Initialize`_, dan _rs`InitializeMint`_
...
| ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InterestBearingConfig`_                      | Ekstensi mint yang menyimpan otoritas tingkat bunga, timestamp, serta tingkat saat ini dan historis.                         | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L40-L153)         |
| _rs`InterestBearingMintInstruction::Initialize`_ | Instruksi yang menginisialisasi konfigurasi interest bearing sebelum _rs`InitializeMint`_.                                         | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/instruction.rs#L17-L90)  |
| _rs`InterestBearingMintInstruction::UpdateRate`_ | Instruksi yang mengubah tingkat bunga saat ini dari mint.                                                                    | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/instruction.rs#L35-L113) |
| _rs`AmountToUiAmount`_                           | Instruksi yang mengembalikan string jumlah UI saat ini untuk jumlah token menggunakan konfigurasi aktif mint.                      | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L520-L538)                                |
| _rs`InterestBearingConfig::amount_to_ui_amount`_ | Pembantu yang mengonversi jumlah token menjadi jumlah UI dengan bunga untuk timestamp tertentu.                                           | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L85-L96)          |
| _rs`process_initialize`_                         | Logika prosesor yang menginisialisasi _rs`InterestBearingConfig`_ dan mencatat timestamp serta tingkat awal.                     | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/interest_bearing_mint/processor.rs#L21-L43)      |
| _rs`process_update_rate`_                        | Logika prosesor yang memperbarui tingkat saat ini, menghitung ulang tingkat rata-rata tertimbang waktu, dan mencatat timestamp pembaruan. | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/interest_bearing_mint/processor.rs#L46-L77)      |
| _rs`process_amount_to_ui_amount`_                | Logika prosesor yang mengembalikan string jumlah UI menggunakan konfigurasi interest bearing aktif dari mint.                     | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1548-L1569)                                  |
