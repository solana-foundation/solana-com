...
Test validator lokal juga menyertakan build Token-2022 versi lama, jadi untuk
menjalankan contoh-contoh di halaman ini, muat program v11.0.0 dari devnet ke
dalam test validator Anda:
...
Untuk membuat mint dengan burn berizin dan membakar token:
...
5. Opsional, nonaktifkan pembakaran berizin dengan _rs`SetAuthority`_
   menggunakan _rs`AuthorityType::PermissionedBurn`_ dan authority `None`, yang
   mengaktifkan kembali instruksi burn standar. Contoh kode lengkap di bawah ini
   menunjukkan langkah ini.
...
Buat token account untuk payer dan mint token ke dalamnya.
...
## !!steps Burn dengan burn authority

Burn token dengan _rs`PermissionedBurnInstruction::BurnChecked`_ yang
ditandatangani oleh pemilik token account dan burn authority.
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | Ekstensi mint yang menyimpan otoritas yang diperlukan untuk menandatangani setiap burn bersama pada mint.                            | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
...
| _rs`PermissionedBurnInstruction::Burn`_        | Instruksi burn yang memerlukan tanda tangan dari burn authority dan pemilik atau delegasi token account.                             | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
...
| _rs`AuthorityType::PermissionedBurn`_          | Diskriminator otoritas yang digunakan bersama _rs`SetAuthority`_ untuk merotasi atau menonaktifkan burn authority pada mint.         | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | Logika pemroses yang menginisialisasi _rs`PermissionedBurnConfig`_ pada mint yang belum diinisialisasi dan menyimpan burn authority. | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
| _rs`process_burn`_                             | Logika pemroses yang menolak burn standar ketika burn authority disetel dan memverifikasi tanda tangan otoritas.                     | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | Logika pemroses yang memvalidasi dan merotasi burn authority ketika _rs`AuthorityType::PermissionedBurn`_ digunakan.                 | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### Typescript

Contoh `Kit` di bawah ini menggunakan instruksi yang dihasilkan secara langsung.
Paket `@solana/spl-token` warisan yang diterbitkan belum menyertakan dukungan
burn dengan izin, sehingga tidak ada contoh warisan yang disertakan.
