...
description:
  Buat mint Token-2022 dengan ekstensi permissioned burn sehingga setiap
  pembakaran memerlukan co-tanda tangan dari burn authority yang dikonfigurasi,
  lalu bakar token dengan instruksi permissioned burn.
...
## Apa Itu Permissioned Burn?

Ekstensi mint _rs`PermissionedBurnConfig`_ dari Token Extension Program
mengharuskan burn authority yang dikonfigurasi untuk ikut menandatangani setiap
pembakaran pada mint tersebut.

Selama burn authority ditetapkan:

- Instruksi standar _rs`Burn`_ dan _rs`BurnChecked`_ akan gagal dengan
  _rs`TokenError::InvalidInstruction`_.
- Pembakaran harus menggunakan _rs`PermissionedBurnInstruction::Burn`_ atau
  _rs`PermissionedBurnInstruction::BurnChecked`_, yang ditandatangani oleh burn
  authority dan pemilik atau delegasi token account.

Burn authority adalah co-penanda tangan, bukan pengganti pemilik. Authority saja
tidak dapat membakar token dari token account milik orang lain, dan pemilik
token account tidak dapat membakar tanpa tanda tangan authority. Permanent
delegate pun harus menggunakan instruksi permissioned burn dan tetap memerlukan
co-tanda tangan dari burn authority.

Hal ini memungkinkan penerbit menjaga sinkronisasi suplai token dengan catatan
off-chain, misalnya aset yang ditokenisasi dan harus tetap didukung 1:1, dengan
mencegah pemegang membakar token secara sepihak.

Burn authority dapat dirotasi di kemudian hari dengan _rs`SetAuthority`_
menggunakan _rs`AuthorityType::PermissionedBurn`_. Menetapkan authority ke
`None` akan menonaktifkan permissioned burning dan mengaktifkan kembali
instruksi burn standar. Data ekstensi itu sendiri tetap berada di mint.

<Callout type="warn" title="Ketersediaan">
  Permissioned burn tersedia dalam Token-2022
  [`program@v11.0.0`](https://github.com/solana-program/token-2022/releases/tag/program%40v11.0.0).
  Saat ini tersedia di devnet dan dijadwalkan untuk diaktifkan di mainnet pada
  Juni 2026. Token Extension Program di-deploy secara terpisah ke setiap
  kluster, jadi pastikan deployment di kluster yang Anda targetkan menyertakan
  v11.0.0 atau yang lebih baru.
</Callout>

Test validator lokal juga menyertakan build Token-2022 yang lebih lama, sehingga
untuk menjalankan contoh-contoh di halaman ini, muat program v11.0.0 dari devnet
ke dalam test validator Anda:
...
## Cara Membuat Mint dan Burn Dengan Izin

Untuk membuat mint dengan burn berbasis izin dan membakar token:

1. Hitung ukuran mint account dan rent yang dibutuhkan untuk mint dan ekstensi
   _rs`PermissionedBurnConfig`_.
2. Buat mint account dengan _rs`CreateAccount`_, inisialisasi
   _rs`PermissionedBurnConfig`_, dan inisialisasi mint dengan
   _rs`InitializeMint`_.
3. Buat token account dan mint token.
4. Burn dengan _rs`PermissionedBurnInstruction::BurnChecked`_ yang
   ditandatangani oleh pemilik token account dan burn authority.
5. Secara opsional nonaktifkan pembakaran berbasis izin dengan
   _rs`SetAuthority`_ menggunakan _rs`AuthorityType::PermissionedBurn`_ dan
   otoritas `None`, yang mengaktifkan kembali instruksi burn standar. Contoh
   kode lengkap di bawah menunjukkan langkah ini.
...
_rs`PermissionedBurnConfig`_. Ini adalah ukuran yang digunakan dalam
_rs`CreateAccount`_.
...
Hitung rent menggunakan ukuran yang dibutuhkan untuk mint ditambah ekstensi
_rs`PermissionedBurnConfig`_.
...
## !!steps Inisialisasi PermissionedBurn

Inisialisasi ekstensi _rs`PermissionedBurnConfig`_ pada mint dengan burn
authority.
...
## !!steps Buat token account dan cetak token

Buat token account untuk pembayar dan cetak token ke dalamnya.
...
## !!steps Bakar dengan otoritas burn

Bakar token dengan _rs`PermissionedBurnInstruction::BurnChecked`_ yang
ditandatangani oleh pemilik token account maupun otoritas burn.
...
<Callout type="info" title="Urutan Instruksi">
  _rs`PermissionedBurnInstruction::Initialize`_ harus didahulukan sebelum
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`PermissionedBurnInstruction::Initialize`_, dan _rs`InitializeMint`_ harus
...
| ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | Ekstensi mint yang menyimpan otoritas yang diperlukan untuk ikut menandatangani setiap operasi burn pada mint.                            | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
| _rs`PermissionedBurnInstruction::Initialize`_  | Instruksi yang menginisialisasi konfigurasi permissioned burn sebelum _rs`InitializeMint`_.                                               | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L41-L49) |
| _rs`PermissionedBurnInstruction::Burn`_        | Instruksi burn yang memerlukan tanda tangan dari otoritas burn serta pemilik atau delegasi token account.                                 | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | Instruksi burn dengan pemeriksaan desimal yang menggunakan akun yang sama dengan _rs`PermissionedBurnInstruction::Burn`_.                 | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | Diskriminator otoritas yang digunakan bersama _rs`SetAuthority`_ untuk merotasi atau menonaktifkan otoritas burn pada mint.               | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | Logika prosesor yang menginisialisasi _rs`PermissionedBurnConfig`_ pada mint yang belum diinisialisasi dan menyimpan otoritas burn.       | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
| _rs`process_burn`_                             | Logika prosesor yang menolak operasi burn standar ketika otoritas burn telah ditetapkan dan memverifikasi tanda tangan otoritas tersebut. | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | Logika prosesor yang memvalidasi dan merotasi otoritas burn ketika _rs`AuthorityType::PermissionedBurn`_ digunakan.                       | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### Typescript

Contoh `Kit` di bawah ini menggunakan instruksi yang dihasilkan secara langsung.
Paket `@solana/spl-token` legacy yang telah diterbitkan belum menyertakan
dukungan burn dengan izin, sehingga tidak ada contoh legacy yang disertakan.
