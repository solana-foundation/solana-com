...
description:
  Buat mint Token Extensions Program dengan delegat permanen yang dapat
  mentransfer atau membakar token dari akun mana pun untuk mint tersebut.
...
## Apa Itu Delegat Permanen?

Ekstensi mint _rs`PermanentDelegate`_ dari Token Extensions Program menetapkan
otoritas delegat ke mint itu sendiri.

Delegat permanen adalah otoritas tingkat mint yang dapat mengotorisasi transfer
dan pembakaran untuk token account mana pun untuk mint tersebut. Ini bertindak
sebagai delegat global untuk semua token account untuk mint tersebut, dan
pemilik token account tidak dapat mencabut delegat permanen dari token account
mereka.

Otoritas mint juga dapat merotasi delegat permanen nanti dengan
_rs`SetAuthority`_ menggunakan _rs`AuthorityType::PermanentDelegate`_.

## Cara Membuat dan Menggunakan Delegat Permanen

Untuk membuat dan menggunakan delegat permanen:
...
2. Buat mint account dengan _rs`CreateAccount`_, inisialisasi
   _rs`PermanentDelegate`_, dan inisialisasi mint dengan _rs`InitializeMint`_.
3. Kirim _rs`TransferChecked`_ atau _rs`BurnChecked`_ yang ditandatangani oleh
   delegat permanen. Token program memproses instruksi tersebut dengan cara yang
   sama seolah-olah pemilik token account yang menandatanganinya.
...
_rs`PermanentDelegate`_. Ini adalah ukuran yang digunakan dalam
_rs`CreateAccount`_.
...
## !!steps Inisialisasi PermanentDelegate

Inisialisasi ekstensi _rs`PermanentDelegate`_ pada mint.
...
Buat token account untuk pemilik dan penerima, lalu mint token ke token account
pemilik.
...
## !!steps Transfer dengan permanent delegate

Transfer token dengan _rs`TransferChecked`_ yang ditandatangani oleh permanent
delegate.
...
<Callout type="info" title="Urutan Instruksi">
  _rs`InitializePermanentDelegate`_ harus dijalankan sebelum
  _rs`InitializeMint`_. _rs`CreateAccount`_, _rs`InitializePermanentDelegate`_,
...
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermanentDelegate`_                     | Ekstensi mint yang menyimpan delegasi yang diberi otorisasi di setiap token account untuk mint tersebut.                  | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/permanent_delegate.rs#L15-L21) |
| _rs`InitializePermanentDelegate`_           | Instruksi yang menginisialisasi permanent delegate sebelum _rs`InitializeMint`_.                                          | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1952-L1967)              |
| _rs`SetAuthority`_                          | Instruksi token dasar yang digunakan dengan _rs`AuthorityType::PermanentDelegate`_ untuk merotasi delegasi.               | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1488-L1520)              |
| _rs`AuthorityType::PermanentDelegate`_      | Diskriminator otoritas yang digunakan dengan _rs`SetAuthority`_ untuk merotasi permanent delegate pada mint.              | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1138-L1139)              |
| _rs`process_initialize_permanent_delegate`_ | Logika prosesor yang menulis ekstensi permanent delegate ke mint.                                                         | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1542-L1557)                  |
| _rs`process_set_authority`_                 | Logika prosesor yang memvalidasi dan merotasi permanent delegate ketika _rs`AuthorityType::PermanentDelegate`_ digunakan. | [Sumber](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L849-L860)                    |

### Typescript

Contoh `Kit` di bawah ini menggunakan instruksi yang dihasilkan secara langsung.
Contoh lawas menggunakan `@solana/web3.js` dan `@solana/spl-token` disertakan
sebagai referensi.
