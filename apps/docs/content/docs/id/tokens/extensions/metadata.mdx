...
description:
  Buat mint yang menyimpan metadata langsung pada akun mint menggunakan ekstensi MetadataPointer dan TokenMetadata, kemudian perbarui, hapus, dan kunci metadata tersebut melalui antarmuka token-metadata.
...
Contoh `Kit` di bawah ini menggunakan instruksi yang dihasilkan secara langsung. Contoh lama yang menggunakan `@solana/web3.js`, `@solana/spl-token`, dan
`@solana/spl-token-metadata` disertakan sebagai referensi. Contoh lama membatasi
`maxSupportedTransactionVersion` pada `0` karena web3.js v1 tidak dapat melakukan deserialisasi
