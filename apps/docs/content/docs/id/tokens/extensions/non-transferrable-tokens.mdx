...
description:
  Buat mint yang tokennya dapat di-mint, dibakar, dan ditutup, tetapi tidak
  dapat ditransfer antar token account.
...
## Apa Itu Token Non-Transferable?

Ekstensi mint _rs`NonTransferable`_ dari Token Extensions Program membuat setiap
token account untuk mint tersebut menjadi non-transferable. Setelah token
di-mint, pemegang token tidak dapat memindahkannya ke token account lain dengan
_rs`Transfer`_ atau _rs`TransferChecked`_.

Pola ini berguna untuk aset yang harus tetap terikat pada satu dompet.

Token non-transferable masih dapat:

- Di-mint oleh mint authority
- Dibakar oleh pemilik token account atau delegasi yang diotorisasi
- Token account dapat ditutup setelah saldo mencapai nol

<Callout type="info" title="Ekstensi Token Account">
  Ketika token account diinisialisasi untuk mint dengan _rs`NonTransferable`_,
  token account diinisialisasi dengan _rs`NonTransferableAccount`_ dan
  _rs`ImmutableOwner`_. Ketika token account dibuat melalui [Associated Token
  ...
  yang diperlukan serta lamport bebas rent.
</Callout>

## Cara Membuat Mint Non-Transferable

Untuk membuat mint non-transferable:
...
2. Buat mint account dengan _rs`CreateAccount`_, inisialisasi
   _rs`NonTransferable`_, dan inisialisasi mint dengan _rs`InitializeMint`_.
3. Buat token account untuk mint. _rs`NonTransferableAccount`_ dan
   _rs`ImmutableOwner`_ secara otomatis diaktifkan untuk token account.
4. _rs`Transfer`_ dan _rs`TransferChecked`_ akan gagal dengan
...
_rs`NonTransferable`_. Ini adalah ukuran yang digunakan dalam
_rs`CreateAccount`_.
...
## !!steps Inisialisasi NonTransferable

Inisialisasi ekstensi _rs`NonTransferable`_ pada mint.
...
<Callout type="info" title="Urutan Instruksi">
  _rs`InitializeNonTransferableMint`_ harus dilakukan sebelum
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`InitializeNonTransferableMint`_, dan _rs`InitializeMint`_ harus disertakan
...
| ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| _rs`NonTransferable`_                          | Ekstensi mint yang menandai token dari mint sebagai non-transferable.                                                                                                                                                                                           | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/non_transferable.rs#L8-L13)  |
| _rs`NonTransferableAccount`_                   | Ekstensi token account yang ditambahkan ke token account untuk mint non-transferable.                                                                                                                                                                           | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/non_transferable.rs#L15-L21) |
| _rs`ImmutableOwner`_                           | Ekstensi token account yang mencegah perubahan kepemilikan dan diperlukan untuk token account non-transferable.                                                                                                                                                 | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L8-L16)   |
| _rs`InitializeNonTransferableMint`_            | Instruksi yang menginisialisasi ekstensi non-transferable tingkat mint sebelum _rs`InitializeMint`_.                                                                                                                                                            | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L642-L653)              |
| _rs`process_initialize_non_transferable_mint`_ | Logika prosesor yang menginisialisasi ekstensi mint _rs`NonTransferable`_ pada mint yang belum diinisialisasi.                                                                                                                                                  | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1646-L1655)                |
| _rs`get_required_init_account_extensions`_     | Digunakan untuk secara otomatis menambahkan ekstensi token account ketika token account diinisialisasi berdasarkan ekstensi yang diaktifkan pada mint. Untuk mint _rs`NonTransferable`_, ini menambahkan _rs`NonTransferableAccount`_ dan _rs`ImmutableOwner`_. | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mod.rs#L1304-L1307)          |

### Typescript

Contoh `Kit` di bawah ini menggunakan instruksi yang dihasilkan secara langsung.
Contoh lama menggunakan `@solana/web3.js` disertakan sebagai referensi.
