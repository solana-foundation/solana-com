...
## Cara menyetor token ke saldo tertunda yang bersifat rahasia

Sebelum token dapat ditransfer secara rahasia, saldo token publik harus
dikonversi menjadi saldo rahasia. Konversi ini terjadi dalam dua tahap:

1. **Saldo Tertunda yang Bersifat Rahasia**: Awalnya, token "disetor" dari saldo
   publik ke saldo rahasia yang bersifat "tertunda".
2. **Saldo Tersedia yang Bersifat Rahasia**: Saldo tertunda kemudian
   "diterapkan" ke saldo tersedia, sehingga token siap digunakan untuk transfer
   rahasia.

Bagian ini menjelaskan tahap pertama: menyetor saldo token publik ke saldo
tertunda yang bersifat rahasia.

Diagram berikut menunjukkan langkah-langkah yang terlibat dalam menyetor token
dari saldo publik ke saldo tertunda yang bersifat rahasia:
...
Untuk mengonversi saldo publik menjadi saldo tertunda yang bersifat rahasia,
panggil instruksi
...
[Jumlah maksimum](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/mod.rs#L21)
per instruksi deposit dibatasi hingga 2^48.

Crate `spl_token_client` menyediakan metode `confidential_transfer_deposit` yang
membangun dan mengirimkan transaksi dengan instruksi `Deposit`, sebagaimana
didemonstrasikan dalam contoh di bawah ini.

## Contoh Kode

Contoh berikut menunjukkan cara menyetor saldo token publik ke saldo tertunda
yang bersifat rahasia.

Confidential Balances bergantung pada program ZK ElGamal Proof, yang telah diaktifkan
di mainnet dan devnet. `solana-test-validator` standar tidak mengaktifkannya, tetapi
validator lokal yang melakukan forking mainnet seperti [Surfpool](https://surfpool.run) mengaktifkannya.
Jalankan contoh ini pada salah satu dari keduanya (kode menggunakan devnet) dengan payer yang memiliki saldo,
lalu ganti alamat mint dan akun placeholder dengan milik Anda sendiri.
