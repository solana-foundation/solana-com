...
<Callout type="warn">
  ...
  menjalani audit keamanan. Ini berarti ekstensi confidential transfer saat ini
  tidak tersedia. Meskipun konsepnya masih valid, contoh kode tidak akan
  berjalan.
</Callout>

## Apa itu confidential transfer?

Confidential transfer memungkinkan Anda mentransfer token antar akun token tanpa
mengungkapkan jumlah transfer. Ini berguna untuk transaksi yang menjaga privasi.
Hanya jumlah transfer dan saldo token yang bersifat privat. Alamat akun token
tetap publik.
...
- [Panduan quick start](https://www.solana-program.com/docs/confidential-balances#setup) -
  Setup dan perintah CLI dasar
...
ke program Token Extension yang memungkinkan Anda mentransfer token antar akun
...
Alur dasar confidential token transfer adalah sebagai berikut:
...
5. **Apply** pending balance pengirim ke **confidential available balance**.
6. Secara rahasia **transfer** token dari token account pengirim ke token
   account penerima.
7. **Apply** pending balance penerima ke **confidential available balance**.
8. **Withdraw** confidential available balance penerima ke **saldo publik**.
...
<Cards>
  ...
  >
    Cara mendepositkan token ke saldo tertunda rahasia
  </Card>
  <Card
...
rahasia yang sesuai dengan diagram sebelumnya. Langkah-langkahnya dipisahkan ke
dalam fungsi pembantu untuk kejelasan.

Untuk menjalankan contoh, mulai validator lokal dengan Token Extension Program
yang dikloning dari mainnet menggunakan perintah berikut. Anda harus menginstal
Solana CLI untuk menggunakan validator lokal.
...
<Callout type="info">
  ...
  lokal default. Anda harus mengkloning Token Extension Program mainnet untuk
...
Daftar lengkap
[instruksi](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
ekstensi Confidential Transfer adalah sebagai berikut:
...
| _rs`EmptyAccount`_                  | Mengosongkan saldo rahasia tertunda dan tersedia untuk memungkinkan penutupan token account.                                                                      |
| _rs`Deposit`_                       | Mengonversi saldo token publik menjadi saldo rahasia tertunda.                                                                                                    |
| _rs`Withdraw`_                      | Mengonversi saldo rahasia tersedia kembali ke saldo publik.                                                                                                       |
...
| _rs`DisableConfidentialCredits`_    | Memblokir transfer rahasia masuk sambil tetap mengizinkan transfer publik.                                                                                        |
