...
_rs`ConfidentialTransferMint`_ berisi tiga field konfigurasi:

- **authority**: Akun yang memiliki izin untuk mengubah pengaturan confidential
  transfer untuk mint dan menyetujui confidential account baru jika
  auto-approval dinonaktifkan.
...
1. **Membuat mint account**: Panggil instruksi _rs`CreateAccount`_ dari System
...
2. **Inisialisasi ekstensi Confidential Transfer**: Panggil instruksi
   ...
   dari Token Extension Program untuk mengonfigurasi state
   _rs`ConfidentialTransferMint`_ untuk mint.

3. **Inisialisasi Mint**: Panggil instruksi _rs`Instruction::InitializeMint`_
   dari Token Extension Program untuk menginisialisasi state mint standar.

Meskipun Anda dapat menulis instruksi ini secara manual, crate
`spl_token_client` menyediakan metode `create_mint` yang membangun dan mengirim
transaksi dengan ketiga instruksi dalam satu pemanggilan fungsi, seperti yang
