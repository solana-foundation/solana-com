...
description:
  Pelajari cara membuat token mint dengan Confidential Balances yang diaktifkan.
---

## Cara membuat mint dengan Confidential Balances

Confidential Balances memungkinkan transfer token secara privat dengan menambahkan state tambahan ke
mint account. Bagian ini menjelaskan cara membuat token mint dengan
extension yang diperlukan diaktifkan.

Diagram berikut menunjukkan langkah-langkah yang terlibat dalam membuat mint dengan
extension Confidential Balances:
...
### State Mint Confidential Balances
...
_rs`ConfidentialTransferMint`_ memiliki tiga bidang konfigurasi:

- **authority**: Akun yang memiliki izin untuk mengubah pengaturan Confidential Balances
  pada mint dan menyetujui token account baru jika persetujuan otomatis
  dinonaktifkan.

- **auto_approve_new_accounts**: Ketika diatur ke true, pengguna dapat membuat token
  account dengan Confidential Balances yang diaktifkan secara default. Ketika false, authority
  harus menyetujui setiap token account baru sebelum dapat digunakan untuk
  transfer konfidensial.

- **auditor_elgamal_pubkey**: Auditor opsional yang dapat mendekripsi jumlah
  transfer dalam transaksi rahasia, menyediakan mekanisme kepatuhan sambil tetap
  menjaga privasi dari publik umum.

### Instruksi yang Diperlukan

Membuat mint dengan Confidential Balances yang diaktifkan memerlukan tiga instruksi
dalam satu transaksi:

1. **Buat Mint Account**: Panggil instruksi _rs`CreateAccount`_ dari System
   Program untuk membuat mint account.

2. **Initialize Confidential Balances**: Memanggil instruksi
   [ConfidentialTransferInstruction::InitializeMint](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L48) dari Token Extension Program
   untuk mengonfigurasi state _rs`ConfidentialTransferMint`_ pada
   mint.

3. **Inisialisasi Mint**: Panggil instruksi _rs`Instruction::InitializeMint`_
   dari Token Extensions Program untuk menginisialisasi state mint standar.
...
Kode berikut mendemonstrasikan cara membuat mint dengan Confidential Balances
yang diaktifkan.

Confidential Balances bergantung pada program ZK ElGamal Proof, yang diaktifkan
pada mainnet dan devnet. `solana-test-validator` standar tidak mengaktifkannya, tetapi
validator lokal yang melakukan fork mainnet seperti [Surfpool](https://surfpool.run) mendukungnya.
Jalankan contoh ini pada salah satu dari keduanya (kode menggunakan devnet) dengan payer yang memiliki saldo,
dan ganti alamat mint dan akun placeholder dengan milik Anda sendiri.
