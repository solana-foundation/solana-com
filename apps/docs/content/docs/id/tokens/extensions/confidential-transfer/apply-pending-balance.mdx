...
1. **Saldo tertunda konfidensial**: Awalnya, token "didepositkan" dari saldo
   publik ke saldo konfidensial "tertunda".
...
Ketika token "didepositkan" dari saldo publik atau ketika token ditransfer
secara konfidensial dari satu token account ke token account lain, token awalnya
ditambahkan ke saldo tertunda konfidensial. Sebelum token dapat digunakan untuk
transfer konfidensial, saldo tertunda harus "diterapkan" ke saldo tersedia.
...
Contoh berikut mendemonstrasikan cara menerapkan saldo tertunda konfidensial ke
saldo tersedia konfidensial.
...
Solana CLI untuk memulai validator lokal.
