...
<Callout type="warn">
  ZK ElGamal Program sementara dinonaktifkan di mainnet dan devnet karena sedang
...
### State Confidential Transfer Token Account
...
_rs`ConfidentialTransferAccount`_ berisi beberapa field untuk mengelola
confidential transfers:

- **approved**: Status persetujuan account untuk confidential transfers. Jika
  konfigurasi `auto_approve_new_accounts` pada mint account diatur sebagai
  `true`, semua token account secara otomatis disetujui untuk confidential
  transfers.

- **elgamal_pubkey**: Kunci publik ElGamal yang digunakan untuk mengenkripsi
  saldo dan jumlah transfer.

- **pending_balance_lo**: 16 bit terendah dari pending balance yang dienkripsi.
  Saldo dibagi menjadi bagian tinggi dan rendah untuk dekripsi yang efisien.

- **pending_balance_hi**: 48 bit tertinggi dari pending balance yang dienkripsi.
  Saldo dibagi menjadi bagian tinggi dan rendah untuk dekripsi yang efisien.

- **available_balance**: Saldo terenkripsi yang tersedia untuk transfer.

- **decryptable_available_balance**: Saldo tersedia yang dienkripsi dengan kunci
  Advanced Encryption Standard (AES) untuk dekripsi yang efisien oleh pemilik
  account.

- **allow_confidential_credits**: Jika true, mengizinkan transfer konfidensial
  masuk.

- **allow_non_confidential_credits**: Jika true, mengizinkan transfer
  non-konfidensial masuk.

- **pending_balance_credit_counter**: Menghitung kredit saldo tertunda masuk
  dari instruksi deposit dan transfer.
...
karena saldo terenkripsi akan berubah antara saat transaksi dikirim dan saat
diproses, mengakibatkan transaksi gagal.
...
<Callout type="info">
  **Pemisahan tinggi/rendah saldo tertunda**
...
Saat mengambil state token account untuk membaca
...
2. Ambil transaksi terbaru termasuk instruksi deposit dan transfer hingga
   perbedaan penghitung (aktual - diharapkan):
...
2. **Realokasi ruang account**: Panggil instruksi
   _rs`TokenInstruction::Reallocate`_ dari Token Extension Program untuk
   menambahkan ruang bagi state _rs`ConfidentialTransferAccount`_.

3. **Konfigurasi Confidential Transfers**: Panggil instruksi
   ...
   dari Token Extension Program untuk menginisialisasi state
...
proof di sisi klien yang hanya dapat dibuat oleh pemilik token account.

_rs`PubkeyValidityProofData`_ membuat proof yang memverifikasi bahwa kunci
...
Kode berikut mendemonstrasikan cara membuat Associated Token Account dengan
ekstensi Confidential Transfer.

Untuk menjalankan contoh ini, mulai validator lokal dengan Token Extension
Program yang dikloning dari mainnet menggunakan perintah berikut. Anda harus
menginstal Solana CLI untuk memulai validator lokal.
