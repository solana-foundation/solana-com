...
description:
  Pelajari cara membuat token account dengan Confidential Balances yang diaktifkan.
---

## Cara membuat token account dengan Confidential Balances

Confidential Balances memungkinkan transfer token yang bersifat privat dengan menambahkan state tambahan ke
token account. Bagian ini menjelaskan cara membuat token account dengan
extension yang diperlukan diaktifkan.

Diagram berikut menunjukkan langkah-langkah yang terlibat dalam membuat token account dengan
Confidential Balances yang diaktifkan:
...
### State Token Account Confidential Balances
...
_rs`ConfidentialTransferAccount`_ berisi beberapa field yang mengelola
Confidential Balances:

- **approved**: Status persetujuan akun untuk Confidential Balances. Jika
  konfigurasi `auto_approve_new_accounts` pada mint account diatur sebagai `true`, semua
  token account secara otomatis disetujui untuk Confidential Balances.
...
Membuat dan mengonfigurasi token account untuk Confidential Balances menggunakan
...
4. **Konfigurasi Confidential Balances**: Panggil instruksi
   ...
   dari Token Extension Program,
   dengan mereferensikan akun state konteks bukti melalui
   _rs`ProofLocation::ContextStateAccount`_, untuk menginisialisasi
   state _rs`ConfidentialTransferAccount`_.

<Callout type="info">

**Hanya pemilik token account yang dapat mengaktifkan Confidential Balances pada sebuah token
account**.

Instruksi _rs`ConfigureAccount`_ memerlukan pembuatan kunci enkripsi
dan bukti di sisi klien yang hanya dapat dibuat oleh pemilik token account.

Bukti validitas pubkey memverifikasi bahwa kunci publik ElGamal akun adalah
valid. Bukti ini dihasilkan dengan _rs`build_pubkey_validity_proof_data`_, diverifikasi di
chain oleh program ZK ElGamal Proof ke dalam sebuah akun state konteks, lalu
direferensikan dari _rs`ConfigureAccount`_ melalui
_rs`ProofLocation::ContextStateAccount`_, sehingga tidak ada byte bukti yang dikirim dalam instruksi
token itu sendiri. Untuk detail implementasi, lihat:
...
Kode berikut membuat associated token account dan mengonfigurasinya untuk menggunakan
Confidential Balances dengan mint yang kompatibel yang sudah ada.

Confidential Balances bergantung pada program ZK ElGamal Proof, yang diaktifkan
di mainnet dan devnet. `solana-test-validator` standar tidak mengaktifkannya, tetapi
validator lokal yang melakukan fork mainnet seperti [Surfpool](https://surfpool.run) mengaktifkannya.
Jalankan contoh ini terhadap salah satunya (kode menggunakan devnet) dengan payer yang memiliki saldo,
dan ganti placeholder mint dengan mint yang dibuat sesuai
