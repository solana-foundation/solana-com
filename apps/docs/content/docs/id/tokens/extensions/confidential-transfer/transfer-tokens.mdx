...
description:
  Pelajari cara mentransfer token secara pribadi dari satu akun token ke akun token lainnya.
---

## Cara mentransfer token secara rahasia dari satu token account ke token account lainnya

Untuk mentransfer token secara rahasia dari satu token account ke token account lain, pengirim dan penerima harus memiliki token account yang dikonfigurasi dengan status _rs`ConfidentialTransferAccount`_ dan disetujui untuk menggunakan Confidential
Balances. Token account pengirim juga harus memiliki saldo rahasia yang tersedia untuk ditransfer.
...
1. Buat
   ...
   **Bukti Kesetaraan
   ...
   Memverifikasi bahwa ciphertext saldo tersedia yang baru setelah transfer
   sesuai dengan
   [komitmen Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme) yang
   bersesuaian, memastikan saldo tersedia baru akun sumber dihitung dengan benar
   sebagai `new_balance = current_balance - transfer_amount`.
   ...
   **Bukti Validitas Ciphertext
   ...
   Memverifikasi bahwa ciphertext jumlah transfer dihasilkan dengan benar untuk
   ketiga pihak (sumber, tujuan, dan auditor), memastikan jumlah transfer
   dienkripsi dengan benar menggunakan kunci publik masing-masing pihak.
   ...
   **Bukti Rentang
   ...
   Memverifikasi bahwa saldo tersedia yang baru dan jumlah transfer (dibagi
...
2. Untuk setiap bukti:
   ...
   - Simpan metadata spesifik bukti dalam akun "context state" bukti untuk
     digunakan dalam instruksi lainnya.
...
3. Panggil instruksi
   ...
   dengan menyertakan akun context state bukti.

4. Tutup akun context state bukti untuk memulihkan SOL yang digunakan untuk
   membuatnya.

Diagram berikut menunjukkan langkah-langkah yang terlibat dalam mentransfer
token dari token account pengirim ke token account penerima.
...
Untuk mentransfer token secara rahasia dari satu token account ke token account
lainnya, Anda harus:

- Buat bukti kesetaraan, bukti validitas ciphertext, dan bukti rentang di sisi
  klien
- Panggil program bukti Zk ElGamal untuk memverifikasi bukti dan
  menginisialisasi akun "context state"
- Panggil instruksi
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  dengan menyertakan tiga akun bukti.
- Tutup tiga akun bukti untuk memulihkan rent.

Contoh Rust di bawah ini menghasilkan bukti dengan crate
`spl-token-confidential-transfer-proof-generation`, memverifikasi setiap bukti
ke dalam akun status konteks melalui program ZK ElGamal Proof, mereferensikan
tiga akun dalam instruksi transfer, dan menutupnya setelahnya. Contoh TypeScript
menggunakan helper `getConfidentialTransferInstructionPlan` dari
`@solana-program/token-2022/confidential`, yang menyusun akun bukti, transfer,
dan penutupan yang sama sebagai rencana instruksi multi-transaksi.

## Contoh Kode

Contoh berikut mentransfer token secara rahasia dari satu akun ke akun lain. Kedua akun harus sudah dikonfigurasi untuk Confidential Balances, dan pengirim harus memiliki saldo rahasia yang tersedia.

Confidential Balances bergantung pada program ZK ElGamal Proof, yang diaktifkan di mainnet dan devnet. `solana-test-validator` standar tidak mengaktifkannya, tetapi validator lokal yang melakukan fork mainnet seperti [Surfpool](https://surfpool.run) sudah mengaktifkannya.
Jalankan contoh ini pada salah satu dari keduanya (kode menggunakan devnet) dengan payer yang memiliki dana, dan ganti placeholder dengan mint serta akun pengirim dan penerima Anda.
