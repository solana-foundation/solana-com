...
## Cara mentransfer token secara rahasia dari satu akun token ke akun token lainnya

Untuk mentransfer token secara rahasia dari satu akun token ke akun token
lainnya, baik pengirim maupun penerima harus memiliki akun token yang
dikonfigurasi dengan state _rs`ConfidentialTransferAccount`_ dan disetujui untuk
confidential transfers. Akun token pengirim juga harus memiliki saldo rahasia
...
1. Buat
   ...
   **Equality proof
   ...
   sesuai, memastikan saldo tersedia baru akun sumber dihitung dengan benar
   ...
   **Range proof
   ...
   menjadi bit rendah/tinggi) semuanya non-negatif dan berada dalam rentang yang
   ditentukan.

2. Untuk setiap bukti:
   - Panggil ZK ElGamal proof program untuk memverifikasi data bukti.
...
4. Tutup akun state konteks proof untuk memulihkan SOL yang digunakan untuk
   membuatnya.
...
account lainnya, Anda harus:

- Generate equality proof, ciphertext validity proof, dan range proof di sisi
  klien
- Invoke program Zk ElGamal proof untuk memverifikasi proof dan menginisialisasi
  akun "context state"
- Invoke instruksi
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  dengan menyediakan tiga akun proof.
- Tutup ketiga akun proof untuk memulihkan rent.
...
- Metode `confidential_transfer_create_context_state_account` yang membuat akun
  proof.
- Metode `confidential_transfer_transfer` yang meng-invoke instruksi `Transfer`.
- Metode `confidential_transfer_close_context_state_account` yang menutup akun
  proof.

### Contoh kode

Contoh berikut mendemonstrasikan cara mentransfer token secara konfidensial
antara token account.

Untuk menjalankan contoh ini, mulai validator lokal dengan Token Extension
Program yang di-clone dari mainnet menggunakan perintah berikut. Anda harus
...
<Callout type="info">
  Pada saat penulisan ini, Confidential Transfers belum diaktifkan pada
  validator lokal default. Anda harus meng-clone Token Extension Program dari
  mainnet untuk menjalankan contoh kode.
</Callout>
