...
suatu transaksi. Unit komputasi (CU) adalah satuan perhitungan yang diukur oleh runtime
selama eksekusi transaksi. Setiap operasi (aritmatika, akses memori,
syscall) membutuhkan sejumlah CU yang tetap. Anda dapat menetapkan biaya prioritas dengan
menyertakan instruksi anggaran komputasi dalam transaksi Anda. (Lihat
[panduan Cara Menggunakan Biaya Prioritas](/docs/core/fees) untuk detailnya.)
...
<Callout type="info" title="Transaksi v1 menetapkan biaya secara langsung">
  Formula harga-dikali-batas ini berlaku untuk transaksi legacy dan v0. Dalam
  [format
  v1](/docs/core/transactions/versioned-transactions#v1-message-format) yang akan datang,
  biaya prioritas adalah total absolut dalam lamport yang ditetapkan langsung di konfigurasi
  pesan — tanpa harga per-CU, tanpa perkalian, tanpa pembulatan.
</Callout>
