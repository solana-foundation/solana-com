...
Tanda tangan pertama dalam array adalah milik **pembayar biaya**, akun yang
membayar
[biaya dasar dan biaya prioritas transaksi](/docs/core/fees/fee-structure#base-fee).
Tanda tangan pertama ini juga berfungsi sebagai **ID transaksi**, digunakan
...
- Harus memiliki lamport yang cukup untuk menutupi
...
- [`num_readonly_signed_accounts`](mention:num_readonly_signed_accounts): Jumlah
  akun yang ditandatangani yang bersifat read-only.
- [`num_readonly_unsigned_accounts`](mention:num_readonly_unsigned_accounts):
  Jumlah akun yang tidak ditandatangani yang bersifat read-only.
...
<Callout>
  ...
  hitungan dalam [`header`](#header) pesan untuk menentukan izin untuk setiap
...
transaksi tiba, maka akan ditolak dengan _rs`BlockhashNotFound`_, kecuali jika
merupakan [transaksi nonce tahan lama](/docs/core/transactions/durable-nonces)
yang valid.
...
3. [`data`](mention:instruction-data): Array byte yang berisi diskriminator
   instruksi dan argumen yang diserialisasi.
...
![Array compact dari instruksi](/assets/docs/core/transactions/compact_array_of_ixs.png)

## Format biner transaksi

Transaksi diserialisasi menggunakan skema encoding compact. Semua array dengan
panjang variabel (signature, kunci akun, instruksi) diawali dengan encoding
...
| `instructions`            | variabel                     | Array [instruksi yang dikompilasi](#instructions) |
...
Dengan _rs`PACKET_DATA_SIZE`_ = 1.232 bytes, ruang yang tersedia dapat dihitung:
...
pengirim mengindikasikan bahwa akun tersebut harus menandatangani transaksi. Ini
memungkinkan System Program mengurangi lamport. Baik akun pengirim maupun
penerima harus dapat ditulis (writable), agar saldo lamport mereka dapat
berubah. Untuk mengeksekusi instruksi ini, wallet pengirim mengirimkan transaksi
yang berisi tanda tangannya dan pesan yang berisi instruksi transfer SOL.
...
**SOL** asli tidak memiliki perlindungan semacam itu, sehingga pengirim harus
...
### Verifikasi penerima sebelum mentransfer

Karena transfer SOL berhasil ke akun mana pun, periksa penerima sebelum
menandatangani. Ambil akun tersebut dan hanya kirim ke dompet System Program
(atau alamat
[pada kurva](/docs/payments/send-payments/verify-address#on-curve-addresses)
yang tidak didanai); tolak mint, token account, program, dan PDA yang tidak Anda
kendalikan.
...
<Callout>
  ...
  [Verifikasi Alamat](/docs/payments/send-payments/verify-address).
</Callout>
