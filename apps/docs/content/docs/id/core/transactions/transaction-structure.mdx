...
description:
  Anatomi transaksi Solana termasuk tanda tangan, format pesan, header, alamat akun, blockhash, instruksi terkompilasi, encoding biner, dan anggaran ukuran.
...
<Callout type="info" title="Ringkasan">
  Sebuah transaksi memiliki tanda tangan + sebuah pesan. Pesan tersebut berisi header,
  alamat akun, blockhash terbaru, dan instruksi yang telah dikompilasi. Ukuran
  serial maksimal: 1.232 byte (4.096 byte untuk format v1 yang akan datang).
</Callout>
...
<Callout type="info" title="Transaksi v1 meningkatkan batas menjadi 4.096 byte">
  Halaman ini menjelaskan format transaksi lama (legacy), yang juga diikuti oleh
  format v0 selain awalan versi dan tabel pencarian alamatnya. Format
  [v1](/docs/core/transactions/versioned-transactions#v1-message-format)
  yang akan datang meningkatkan batas ukuran menjadi 4.096 byte dan menggunakan tata letak wire yang berbeda.
</Callout>
...
<Callout type="info" title="Prefiks pesan lama dan berversi">
  Dalam pesan transaksi legacy, byte pertama pesan adalah
  `num_required_signatures`, diikuti oleh dua byte `MessageHeader` lainnya. Dalam
  pesan transaksi berversi, byte pertama adalah awalan versi;
  `MessageHeader` tiga byte dimulai segera setelah awalan tersebut. Lihat
  [transaksi berversi](/docs/core/transactions/versioned-transactions) untuk
  tata letak pesan v0 dan v1 selengkapnya.
</Callout>

![Diagram yang menampilkan tiga bagian dari header pesan](/assets/docs/core/transactions/message_header.png)
...
![Diagram yang menampilkan urutan array alamat akun](/assets/docs/core/transactions/compat_array_of_account_addresses.png)
...
[transaksi nonce tahan lama](/docs/core/transactions/durable-nonces) yang valid.
...
Transaksi diserialisasi menggunakan skema enkoding kompak. Semua array dengan
panjang variabel (tanda tangan, kunci akun, instruksi) diawali dengan enkoding
...
pengirim menunjukkan bahwa akun tersebut harus menandatangani transaksi. Ini
...
native tidak memiliki perlindungan semacam itu, sehingga pengirim harus
...
namun perhatikan bahwa setiap instruksi memuat informasi yang diperlukan yang sama.
...
inner (CPI) sebagai blob yang telah dikodekan. Untuk menguraikannya menjadi
alamat dan menelusuri pohon instruksi secara lengkap, lihat
