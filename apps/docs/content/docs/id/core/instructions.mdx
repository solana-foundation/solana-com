...
Solana. Instruksi pada dasarnya adalah fungsi publik yang dapat dipanggil oleh
siapa saja yang menggunakan jaringan Solana. Setiap instruksi digunakan untuk
...
serangkaian instruksinya sendiri. Untuk berinteraksi dengan jaringan Solana,
...
ini, SOL ditransfer dari satu akun ke akun lainnya.
...
kunci publik dari program yang berisi logika bisnis instruksi.

### Metadata akun

<WithMentions>

Array `accounts` instruksi adalah array dari struct
...
(Ini memungkinkan transaksi untuk mengeksekusi instruksi secara paralel, selama
mereka tidak memodifikasi akun yang sama.)

Diagram di bawah ini menggambarkan transaksi yang berisi satu instruksi. Array
`accounts` instruksi berisi metadata untuk dua akun.

![Transaksi dengan satu instruksi. Instruksi berisi dua struct `AccountMeta` dalam array `accounts`-nya.](/assets/docs/core/transactions/accountmeta.svg)
...
`data` instruksi adalah array byte yang menentukan instruksi mana dari program
yang akan dipanggil. Ini juga mencakup argumen apa pun yang diperlukan oleh
instruksi.
...
informasi wajib yang sama: [`program_id`](mention:program-id),
...
<Callout type="info">
  ...
  dapat membuat instruksi secara manual.
</Callout>
