...
Bump seed adalah satu byte (u8) yang ditambahkan ke seed opsional selama proses
derivasi.
[`find_program_address`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L800)
mencari dari 255 ke bawah hingga 1, memanggil _rs`create_program_address`_
dengan setiap nilai hingga hasilnya keluar dari kurva Ed25519. Nilai pertama
yang berhasil adalah **bump kanonik**.
...
untuk _rs`create_program_address`_ membebankan
...
Menurunkan PDA hanya menghitung sebuah alamat. Ini tidak membuat akun onchain di
...
### Iterasi Semua Bump yang Mungkin
...
<Callout type="warn">
  Ini berbeda dari perilaku pencarian kanonik saat ini yang didokumentasikan
  untuk `sol_try_find_program_address`, yang melakukan iterasi dari 255 ke bawah
  hingga 1.
</Callout>
