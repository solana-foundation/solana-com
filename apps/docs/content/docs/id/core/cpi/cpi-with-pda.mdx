---
title: CPI dengan penanda tangan PDA
description:
  Cara menggunakan fungsi invoke_signed untuk Cross-Program Invocations pada
  Solana ketika program pemanggil perlu menandatangani sebagai PDA, dengan
  contoh Anchor CpiContext dan Native Rust.
...
<Callout type="info" title="Ringkasan">
  Gunakan _rs`invoke_signed`_ ketika program pemanggil perlu menandatangani atas
  nama PDA yang dimilikinya. Runtime menurunkan pubkey PDA dari signer seeds
  yang disediakan dan menambahkannya ke set penanda tangan yang valid sebelum
...
Ketika CPI memerlukan penanda tangan PDA, gunakan
[`invoke_signed`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/program/src/program.rs#L51)
dengan [signer seeds](mention:signer-seeds) yang digunakan untuk menurunkan
[PDA](/docs/core/pda). Untuk detail tentang bagaimana runtime memverifikasi
tanda tangan PDA, lihat
...
Contoh di bawah ini membuat CPI dengan penanda tangan PDA menggunakan Anchor dan
...
Contoh berikut menunjukkan dua pendekatan untuk mengimplementasikan CPI dalam
program [Anchor](https://www.anchor-lang.com/docs). Contoh-contoh ini secara
fungsional setara, tetapi mendemonstrasikan tingkat abstraksi yang berbeda.

- **Contoh 1**: Menggunakan _rs`CpiContext`_ dan fungsi helper Anchor.
- **Contoh 2**: Menggunakan fungsi _rs`system_instruction::transfer`_ dari crate
  _rs`solana_program`_. (Contoh 1 adalah abstraksi dari implementasi ini.)
...
Contoh di bawah ini melakukan CPI dengan penandatangan PDA dari program yang
ditulis dalam Native Rust. Ini mencakup satu instruksi yang mentransfer SOL dari
