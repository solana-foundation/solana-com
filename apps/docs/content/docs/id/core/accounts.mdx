...
Contoh di bawah ini menunjukkan cara menggunakan Solana SDK untuk membuat
...
- [`executable`](mention:executable): Menunjukkan apakah akun berisi biner yang
  dapat dieksekusi
- [`rent_epoch`](mention:rent_epoch): Field rent epoch yang sudah tidak
  digunakan
...
Field ini umumnya disebut sebagai "data akun". `data` dalam field ini dianggap
arbitrer karena dapat berisi urutan byte apa pun. Setiap program menentukan
struktur data yang disimpan dalam field ini.
...
2. Mendeserialisasi field `data` akun dari byte mentah ke struktur data yang
...
- Jika `true`: Akun adalah program account
- Jika `false`: Akun adalah akun data
...
Pada diagram di bawah ini, Anda dapat melihat program loader digunakan untuk
men-deploy akun program. `data` akun program berisi kode program yang dapat
dieksekusi.
...
Program yang di-deploy menggunakan loader-v3 tidak berisi kode program dalam
field `data` mereka. Sebaliknya, `data` mereka menunjuk ke **akun data program**
terpisah, yang berisi kode program. (Lihat diagram di bawah ini.)
...
Contoh di bawah ini mengambil akun Token Program. Perhatikan bahwa field
`executable` diatur ke `true`, yang menunjukkan bahwa akun tersebut adalah
program.
...
Pada contoh di bawah ini, keypair baru dibuat dan didanai dengan SOL. Setelah
menjalankan kode, Anda dapat melihat alamat `owner` akun adalah
`11111111111111111111111111111111` (yaitu
