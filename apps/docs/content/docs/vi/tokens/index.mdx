...
quyền đối với Token Account. Điều này khác với program owner được chỉ định trong
kiểu [Account](/docs/core/accounts#account-type) cơ bản, đó là Token Program cho
tất cả các Token Account.
...
CLI _sh`spl-token`_ giúp bạn thử nghiệm với các SPL token. Các ví dụ dưới đây sử
...
Việc tạo token và account yêu cầu SOL cho tiền đặt cọc rent của account và phí
giao dịch. Đối với người dùng Solana Playground lần đầu, hãy tạo ví Playground
và chạy lệnh _sh`solana airdrop`_ trong terminal Playground. Bạn cũng có thể
nhận devnet SOL từ [web faucet](https://faucet.solana.com/) công khai.
...
Các địa chỉ account hiển thị trong đầu ra terminal của bạn khác với các ví dụ
dưới đây. Sử dụng các địa chỉ từ terminal Playground của bạn khi thực hiện theo.
Ví dụ, lệnh _sh`create-token`_ xuất ra địa chỉ mint account với ví Playground
của bạn làm mint authority.
...
Lệnh _rs`MintTo`_ trên Token Program tạo token mới. Mint authority phải ký giao
dịch. Lệnh này tạo token vào token account và tăng tổng cung trên mint account.
...
Lệnh _rs`Transfer`_ trên Token Program xử lý chuyển token. Chủ sở hữu của token
account người gửi phải ký giao dịch. Lệnh này chuyển token giữa các token
account. Đây là ví dụ Javascript trên
