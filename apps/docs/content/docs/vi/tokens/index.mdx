...
- [Token Programs](#token-programs) chứa toàn bộ logic lệnh để tương tác với
...
[data accounts](/docs/core/accounts/account-types#data-accounts) thuộc sở hữu
của Token Program.
...
chỉ định trong loại
[Account](/docs/core/accounts/account-structure#account-fields) cơ bản, đó là
...
Có hai cách để thêm siêu dữ liệu (tên, ký hiệu, hình ảnh) vào token trên Solana:

1. **[Metaplex Token Metadata](/docs/tokens/metaplex)** - Tạo một tài khoản siêu
   dữ liệu riêng biệt được liên kết với mint của bạn. Hoạt động với Token
   Program gốc và Token-2022 và được hỗ trợ rộng rãi bởi các ví và sàn giao
   dịch.

2. **[Token Extensions Metadata](/docs/tokens/extensions/metadata)** - Lưu trữ
   siêu dữ liệu trực tiếp trên mint account bằng Token-2022. Đây là phương pháp
   mới hơn với hỗ trợ tích hợp sẵn. Tiện ích mở rộng siêu dữ liệu chỉ khả dụng
   cho các mint account được tạo bằng Token-2022. Để thêm siêu dữ liệu vào mint
   account được tạo bằng Token Program gốc, hãy sử dụng Metaplex Token Metadata
   Program.

#### Ví dụ SPL Token CLI

Token Extensions Program cho phép bạn lưu trữ siêu dữ liệu trực tiếp trên Mint
Account. Để tạo một token với tiện ích mở rộng siêu dữ liệu:
...
Để khởi tạo siêu dữ liệu:

```sh title="Terminal"
spl-token initialize-metadata <TOKEN_MINT_ADDRESS> <YOUR_TOKEN_NAME> <YOUR_TOKEN_SYMBOL> <YOUR_TOKEN_URI>
```

URI của token liên kết đến siêu dữ liệu ngoài chuỗi. Xem ví dụ định dạng JSON
...
Xem siêu dữ liệu trên
[Solana Explorer](https://explorer.solana.com/address/BdhzpzhTD1MFqBiwNdrRy4jFo2FHFufw3n9e8sVjJczP?cluster=devnet).

Tìm hiểu thêm trong phần [Token Extensions](/docs/tokens/extensions/metadata).
Để biết chi tiết về Token Extensions, xem phần
[Token Extensions](/docs/tokens/extensions) và
