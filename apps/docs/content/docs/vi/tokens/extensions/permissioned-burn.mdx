...
Một test validator cục bộ cũng đi kèm với bản dựng Token-2022 cũ hơn, vì vậy để
...
1. Tính toán kích thước mint account và rent cần thiết cho mint và phần mở rộng
...
5. Tùy chọn vô hiệu hóa tính năng burn có phân quyền bằng _rs`SetAuthority`_ sử
   dụng _rs`AuthorityType::PermissionedBurn`_ và quyền `None`, điều này kích
   hoạt lại các lệnh burn tiêu chuẩn. Các ví dụ mã đầy đủ bên dưới minh họa bước
   này.
...
Tính toán kích thước mint account cho mint cơ sở cộng với phần mở rộng
...
Tính toán rent bằng kích thước cần thiết cho mint cộng với phần mở rộng
_rs`PermissionedBurnConfig`_.
...
Tạo một token account cho người thanh toán và mint token vào đó.
...
<Callout type="info" title="Instruction Order">
  ...
  được đưa vào cùng một giao dịch.
</Callout>
...
| ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | Phần mở rộng mint lưu trữ authority cần thiết để đồng ký mọi lần đốt token cho mint.                                   | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
| _rs`PermissionedBurnInstruction::Initialize`_  | Lệnh khởi tạo cấu hình permissioned burn trước _rs`InitializeMint`_.                                                   | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L41-L49) |
| _rs`PermissionedBurnInstruction::Burn`_        | Lệnh đốt token yêu cầu chữ ký từ burn authority và chủ sở hữu hoặc người được ủy quyền của token account.              | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | Lệnh đốt token có kiểm tra số thập phân, sử dụng các tài khoản giống như _rs`PermissionedBurnInstruction::Burn`_.      | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | Bộ phân biệt authority được dùng với _rs`SetAuthority`_ để xoay vòng hoặc vô hiệu hóa burn authority trên một mint.    | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | Logic xử lý khởi tạo _rs`PermissionedBurnConfig`_ trên một mint chưa được khởi tạo và lưu trữ burn authority.          | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
| _rs`process_burn`_                             | Logic xử lý từ chối các lần đốt token thông thường khi burn authority được thiết lập và xác minh chữ ký của authority. | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | Logic xử lý xác thực và xoay vòng burn authority khi sử dụng _rs`AuthorityType::PermissionedBurn`_.                    | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### Typescript

Ví dụ `Kit` dưới đây sử dụng trực tiếp các hướng dẫn đã được tạo ra. Gói
`@solana/spl-token` kế thừa đã xuất bản chưa hỗ trợ đốt có quyền hạn, vì vậy
không có ví dụ kế thừa nào được đưa vào.
