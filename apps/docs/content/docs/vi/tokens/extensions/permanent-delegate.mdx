...
description:
  Tạo một mint Token Extension Program với permanent delegate có thể chuyển hoặc
  đốt token từ bất kỳ tài khoản nào cho mint đó.
...
Extension _rs`PermanentDelegate`_ của Token Extension Program gán một quyền ủy
quyền cho chính mint.

Permanent delegate là một quyền hạn cấp mint có thể cho phép chuyển token và đốt
token cho bất kỳ token account nào của mint đó. Nó hoạt động như một delegate
toàn cục cho tất cả token account của mint, và chủ sở hữu token account không
thể thu hồi permanent delegate khỏi token account của họ.

Quyền hạn mint cũng có thể thay đổi permanent delegate sau này bằng
_rs`SetAuthority`_ sử dụng _rs`AuthorityType::PermanentDelegate`_.
...
2. Tạo mint account với _rs`CreateAccount`_, khởi tạo _rs`PermanentDelegate`_,
   và khởi tạo mint với _rs`InitializeMint`_.
3. Gửi _rs`TransferChecked`_ hoặc _rs`BurnChecked`_ được ký bởi permanent
   delegate. Token program xử lý các lệnh đó giống như khi chủ sở hữu token
   account ký chúng.
...
_rs`PermanentDelegate`_. Đây là kích thước được sử dụng trong
_rs`CreateAccount`_.
...
## !!steps Khởi tạo PermanentDelegate

Khởi tạo tiện ích mở rộng _rs`PermanentDelegate`_ trên mint.
...
Tạo token account cho chủ sở hữu và người nhận, sau đó mint token vào token
account của chủ sở hữu.
...
## !!steps Chuyển token với permanent delegate

Chuyển token với _rs`TransferChecked`_ được ký bởi permanent delegate.
...
<Callout type="info" title="Thứ tự lệnh">
  _rs`InitializePermanentDelegate`_ phải được thực hiện trước
  _rs`InitializeMint`_. _rs`CreateAccount`_, _rs`InitializePermanentDelegate`_,
  và _rs`InitializeMint`_ phải được bao gồm trong cùng một giao dịch.
</Callout>
...
| Mục                                         | Mô tả                                                                                               | Nguồn                                                                                                                              |
| ------------------------------------------- | --------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermanentDelegate`_                     | Tiện ích mở rộng mint lưu trữ delegate được ủy quyền trên mọi token account của mint.               | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/permanent_delegate.rs#L15-L21) |
| _rs`InitializePermanentDelegate`_           | Lệnh khởi tạo permanent delegate trước _rs`InitializeMint`_.                                        | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1952-L1967)              |
| _rs`SetAuthority`_                          | Lệnh token cơ bản được sử dụng với _rs`AuthorityType::PermanentDelegate`_ để xoay delegate.         | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1488-L1520)              |
| _rs`AuthorityType::PermanentDelegate`_      | Bộ phân biệt quyền được sử dụng với _rs`SetAuthority`_ để xoay permanent delegate trên mint.        | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1138-L1139)              |
| _rs`process_initialize_permanent_delegate`_ | Logic xử lý ghi tiện ích mở rộng permanent delegate vào mint.                                       | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1542-L1557)                  |
| _rs`process_set_authority`_                 | Logic xử lý xác thực và xoay permanent delegate khi sử dụng _rs`AuthorityType::PermanentDelegate`_. | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L849-L860)                    |
