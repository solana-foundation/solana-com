...
- Để biết số dư trực tiếp của người dùng, bạn có thể nhận thông tin cập nhật về
  các số tiền trên bằng cách gọi getTokenAccountBalance hoặc getAccountInfo
- Nếu bạn cần biết UI Amount cho một số tiền bất kỳ, bạn có thể nhận phép tính
  này bằng cách gọi hàm
  ...
  - Lưu ý: amountToUiAmount yêu cầu mô phỏng giao dịch, điều này có nghĩa là nó
    cũng cần một fee payer hợp lệ có số dư. Do đó, đây không nên là cách mặc
    định để lấy số dư.
...
- Người dùng nên nhập số tiền để được hiểu là “UIAmount” đã được điều chỉnh. Ứng
...
- Hệ số nhân hiện tại có thể được đọc từ token mint bất kỳ lúc nào bằng cách gọi
  `getAccountInfo`. Ngoài ra, nếu hệ số nhân tương lai được đặt, thông tin này
  cũng có sẵn từ token mint. Chúng tôi khuyến nghị không hiển thị hệ số nhân này
  vì nó có thể gây nhầm lẫn cho trải nghiệm người dùng.
