...
description:
  Tìm hiểu cách tạo token mint với tính năng Confidential Balances được bật.
---

## Cách tạo mint với Confidential Balances

Confidential Balances cho phép chuyển token riêng tư bằng cách thêm trạng thái bổ sung vào
mint account. Phần này giải thích cách tạo token mint với
extension cần thiết được bật.

Sơ đồ sau đây minh họa các bước liên quan đến việc tạo mint với
...
### Trạng thái Mint của Confidential Balances
...
- **authority**: Tài khoản có quyền thay đổi cài đặt Confidential Balances
  cho mint và phê duyệt các tài khoản bảo mật mới nếu tính năng tự động phê duyệt
  bị tắt.

- **auto_approve_new_accounts**: Khi được đặt thành true, người dùng có thể tạo token
  account với Confidential Balances được bật theo mặc định. Khi đặt thành false, authority
  phải phê duyệt từng token account mới trước khi có thể sử dụng cho
  các giao dịch bảo mật.
...
Việc tạo mint với Confidential Balances được bật yêu cầu ba lệnh
trong một giao dịch duy nhất:

1. **Tạo Mint Account**: Gọi lệnh _rs`CreateAccount`_ của System Program để tạo
   mint account.

2. **Khởi tạo Confidential Balances**: Gọi lệnh
   ...
   của Token Extension Program để cấu hình trạng thái _rs`ConfidentialTransferMint`_ cho
   mint.
...
Đoạn mã sau đây minh họa cách tạo mint với Confidential Balances
được bật.

Confidential Balances phụ thuộc vào chương trình ZK ElGamal Proof, được bật
trên mainnet và devnet. Một `solana-test-validator` thông thường không bật tính năng này, nhưng một
validator cục bộ fork từ mainnet như [Surfpool](https://surfpool.run) thì có.
Chạy ví dụ trên một trong những môi trường đó (mã sử dụng devnet) với một payer có đủ số dư,
và thay thế địa chỉ mint và tài khoản placeholder bằng địa chỉ của bạn.
