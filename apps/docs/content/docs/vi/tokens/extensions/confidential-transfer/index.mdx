...
<Callout type="warn">
  ...
  devnet do đang trải qua kiểm toán bảo mật. Điều này có nghĩa là tiện ích mở
  rộng chuyển khoản bảo mật hiện không khả dụng. Mặc dù các khái niệm vẫn còn
  giá trị, nhưng các ví dụ mã sẽ không chạy được.
</Callout>
...
riêng tư. Chỉ có số lượng chuyển khoản và số dư token là riêng tư. Địa chỉ token
account vẫn công khai.
...
- [Sổ tay số dư bảo mật](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Các đoạn mã về cách sử dụng tiện ích mở rộng Confidential Transfer

### Nó hoạt động như thế nào?

Tiện ích mở rộng Confidential Transfer bổ sung các
...
vào chương trình Token Extension cho phép bạn chuyển token giữa các tài khoản mà
...
4. **Gửi** số dư công khai của người gửi vào **số dư bảo mật đang chờ xử lý**.
5. **Áp dụng** số dư đang chờ xử lý của người gửi vào **số dư bảo mật khả
   dụng**.
6. **Chuyển** token một cách bảo mật từ token account của người gửi sang token
   account của người nhận.
7. **Áp dụng** số dư đang chờ xử lý của người nhận vào **số dư bảo mật khả
   dụng**.
8. **Rút** số dư bảo mật khả dụng của người nhận về **số dư công khai**.
...
<Cards>
  ...
  >
    Cách phát hành và vận hành token chuyển bảo mật (chính sách phê duyệt, kiểm
    toán viên, phí, mint và burn)
  </Card>
</Cards>

Sơ đồ dưới đây hiển thị trình tự chi tiết của quy trình cơ bản cho chuyển khoản
token bảo mật:
...
Ví dụ Rust dưới đây hiển thị các bước cho chuyển khoản token bảo mật tương ứng
với sơ đồ trước đó. Các bước được tách thành các hàm trợ giúp để dễ hiểu hơn.

Để chạy ví dụ, khởi động validator cục bộ với Token Extension Program được sao
chép từ mainnet bằng lệnh sau. Bạn phải cài đặt Solana CLI để sử dụng validator
cục bộ.
...
## Các lệnh chuyển khoản bảo mật

Danh sách đầy đủ các
[lệnh](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
tiện ích mở rộng chuyển khoản bảo mật như sau:

| Lệnh                                | Mô tả                                                                                                                                                             |
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Thiết lập mint account cho chuyển khoản bảo mật. Lệnh này phải được bao gồm trong cùng giao dịch với lệnh _rs`TokenInstruction::InitializeMint`_.                 |
| _rs`UpdateMint`_                    | Cập nhật cài đặt chuyển khoản bảo mật cho mint.                                                                                                                   |
| _rs`ConfigureAccount`_              | Thiết lập token account cho chuyển khoản bảo mật.                                                                                                                 |
| _rs`ApproveAccount`_                | Phê duyệt token account cho chuyển khoản bảo mật nếu mint yêu cầu phê duyệt cho token account mới.                                                                |
...
| _rs`Withdraw`_                      | Chuyển đổi số dư bảo mật khả dụng trở lại số dư công khai.                                                                                                        |
| _rs`Transfer`_                      | Chuyển token giữa các token account một cách bảo mật.                                                                                                             |
| _rs`ApplyPendingBalance`_           | Chuyển đổi số dư đang chờ xử lý thành số dư khả dụng sau khi gửi hoặc chuyển khoản.                                                                               |
| _rs`EnableConfidentialCredits`_     | Cho phép token account nhận chuyển khoản token bảo mật.                                                                                                           |
| _rs`DisableConfidentialCredits`_    | Chặn chuyển khoản bảo mật đến trong khi vẫn cho phép chuyển khoản công khai.                                                                                      |
| _rs`EnableNonConfidentialCredits`_  | Cho phép token account nhận chuyển khoản token công khai.                                                                                                         |
| _rs`DisableNonConfidentialCredits`_ | Chặn chuyển khoản thông thường để tài khoản chỉ nhận chuyển khoản bảo mật.                                                                                        |
| _rs`TransferWithFee`_               | Chuyển token giữa các token account một cách bảo mật với phí.                                                                                                     |
| _rs`ConfigureAccountWithRegistry`_  | Cách thay thế để cấu hình token account cho chuyển khoản bảo mật bằng cách sử dụng tài khoản _rs`ElGamalRegistry`_ thay vì bằng chứng _rs`VerifyPubkeyValidity`_. |
