...
<Callout type="warn">
  ZK ElGamal Program tạm thời bị vô hiệu hóa trên mainnet và devnet khi đang
  trải qua kiểm toán bảo mật. Điều này có nghĩa là tiện ích mở rộng chuyển khoản
  bảo mật hiện không khả dụng. Mặc dù các khái niệm vẫn hợp lệ, các ví dụ mã sẽ
  không chạy được.
</Callout>
...
2. **Số dư bảo mật khả dụng**: Số dư chờ xử lý sau đó được "áp dụng" vào số dư
   khả dụng, làm cho token có sẵn cho các giao dịch chuyển khoản bảo mật.

Phần này giải thích giai đoạn thứ hai: áp dụng số dư chờ xử lý vào số dư khả
dụng.

Khi token được "gửi" từ số dư công khai hoặc khi token được chuyển bảo mật từ
một token account này sang token account khác, token ban đầu được thêm vào số dư
bảo mật chờ xử lý. Trước khi token có thể được sử dụng cho các giao dịch chuyển
khoản bảo mật, số dư chờ xử lý phải được "áp dụng" vào số dư khả dụng.
...
Để chuyển đổi số dư chờ xử lý thành số dư khả dụng, hãy gọi instruction
...
`confidential_transfer_apply_pending_balance` để xây dựng và gửi một transaction
với instruction `ApplyPendingBalance`, như được minh họa trong ví dụ dưới đây.

### Mã ví dụ

Ví dụ sau đây minh họa cách áp dụng số dư bảo mật đang chờ xử lý vào số dư bảo
mật khả dụng.

Để chạy ví dụ, khởi động một validator cục bộ với Token Extension Program được
