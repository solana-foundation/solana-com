...
được chuyển đổi thành số dư bảo mật. Quá trình chuyển đổi này diễn ra theo hai
giai đoạn:

1. **Số Dư Chờ Xử Lý Bảo Mật**: Ban đầu, các token được "nạp" từ số dư công khai
   vào số dư bảo mật "chờ xử lý".
2. **Số Dư Khả Dụng Bảo Mật**: Số dư chờ xử lý sau đó được "áp dụng" vào số dư
   khả dụng, giúp các token sẵn sàng cho các giao dịch bảo mật.

Phần này giải thích giai đoạn thứ hai: áp dụng số dư chờ xử lý vào số dư khả
dụng.

Khi các token được "nạp" từ số dư công khai hoặc khi các token được chuyển bảo
mật từ token account này sang token account khác, các token sẽ ban đầu được thêm
vào số dư chờ xử lý bảo mật. Trước khi token có thể được sử dụng cho các giao
dịch bảo mật, số dư chờ xử lý phải được "áp dụng" vào số dư khả dụng.
...
Sơ đồ sau đây mô tả các bước liên quan đến việc áp dụng số dư chờ xử lý vào số
dư khả dụng:
...
Để chuyển đổi số dư chờ xử lý thành số dư khả dụng, hãy gọi lệnh
...
Ví dụ sau đây minh họa cách áp dụng số dư chờ xử lý bảo mật vào số dư khả dụng
bảo mật.

Confidential Balances phụ thuộc vào chương trình ZK ElGamal Proof, vốn đã được kích hoạt
trên mainnet và devnet. Một `solana-test-validator` thông thường không kích hoạt nó, nhưng
một validator cục bộ fork từ mainnet như [Surfpool](https://surfpool.run) thì có.
Hãy chạy ví dụ này với một trong các môi trường đó (code sử dụng devnet) cùng với một tài khoản payer có đủ số dư,
và thay thế các địa chỉ mint và tài khoản giữ chỗ bằng địa chỉ của bạn.
