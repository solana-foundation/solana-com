...
description:
  Tìm hiểu cách tạo token account với tính năng Confidential Balances được bật.
---

## Cách tạo token account với Confidential Balances

Confidential Balances cho phép chuyển token riêng tư bằng cách thêm trạng thái bổ sung vào
token account. Phần này giải thích cách tạo token account với
extension cần thiết được bật.

Sơ đồ sau đây mô tả các bước liên quan đến việc tạo token account với
Confidential Balances được bật:
...
### Trạng thái Token Account của Confidential Balances
...
_rs`ConfidentialTransferAccount`_ chứa một số trường để quản lý
Confidential Balances:

- **approved**: Trạng thái phê duyệt của tài khoản đối với Confidential Balances. Nếu
  cấu hình `auto_approve_new_accounts` của mint account được đặt thành `true`, tất cả
  token account sẽ tự động được phê duyệt cho Confidential Balances.
...
Việc tạo và cấu hình token account cho Confidential Balances sử dụng
các lệnh sau, tất cả đều nằm gọn trong một giao dịch duy nhất:
...
4. **Cấu hình Confidential Balances**: Gọi lệnh
   ...
   của Token Extensions Program,
   tham chiếu đến tài khoản trạng thái context proof qua
   _rs`ProofLocation::ContextStateAccount`_, để khởi tạo
   trạng thái _rs`ConfidentialTransferAccount`_.

<Callout type="info">

**Chỉ chủ sở hữu token account mới có thể bật Confidential Balances trên token
account**.

Lệnh _rs`ConfigureAccount`_ yêu cầu tạo khóa mã hóa và bằng chứng ở phía client, và chỉ có chủ sở hữu token account mới có thể tạo được.

Bằng chứng hợp lệ pubkey xác minh rằng khóa công khai ElGamal của tài khoản là
hợp lệ. Nó được tạo bằng _rs`build_pubkey_validity_proof_data`_, được xác minh trên
chuỗi bởi chương trình ZK ElGamal Proof vào một tài khoản trạng thái context, và sau đó
được tham chiếu từ _rs`ConfigureAccount`_ qua
_rs`ProofLocation::ContextStateAccount`_, do đó không có byte bằng chứng nào được truyền trong
bản thân lệnh token. Để biết chi tiết triển khai, xem:
...
Đoạn mã sau tạo một associated token account và cấu hình nó để sử dụng
Confidential Balances với một mint tương thích hiện có.

Confidential Balances phụ thuộc vào chương trình ZK ElGamal Proof, được bật
trên mainnet và devnet. Một `solana-test-validator` thông thường không bật tính năng này, nhưng
một validator local có khả năng fork mainnet như [Surfpool](https://surfpool.run) thì có.
Chạy ví dụ với một trong số đó (mã sử dụng devnet) với một payer đã được nạp tiền,
và thay thế placeholder mint bằng một mint được tạo theo hướng dẫn
