...
- **approved**: Trạng thái phê duyệt của tài khoản cho các giao dịch chuyển
  khoản bảo mật. Nếu cấu hình `auto_approve_new_accounts` của mint account được
  đặt là `true`, tất cả token account sẽ tự động được phê duyệt cho các giao
  dịch chuyển khoản bảo mật.
...
- **maximum_pending_balance_credit_counter**: Giới hạn số lượng các khoản tín
  dụng đang chờ xử lý trước khi yêu cầu lệnh _rs`ApplyPendingBalance`_ để chuyển
  đổi số dư đang chờ xử lý thành số dư khả dụng.
...
Tất cả các khoản tiền gửi và số tiền chuyển khoản ban đầu được thêm vào số dư
đang chờ xử lý. Chủ sở hữu token account phải sử dụng lệnh
_rs`ApplyPendingBalance`_ để chuyển đổi số dư đang chờ xử lý thành số dư khả
dụng. Các giao dịch chuyển khoản hoặc tiền gửi đến không ảnh hưởng đến số dư khả
dụng của token account.
...
`pending_balance_hi` vì việc giải mã ElGamal yêu cầu nhiều tính toán hơn cho các
số lớn hơn. Bạn có thể tìm thấy cách triển khai phép toán ciphertext
...
1. Client tra cứu số dư đang chờ xử lý và số dư khả dụng hiện tại, mã hóa tổng,
   và cung cấp `decryptable_available_balance` được mã hóa bằng khóa AES của chủ
   sở hữu token account.

2. Các bộ đếm tín dụng đang chờ xử lý dự kiến và thực tế theo dõi các thay đổi
   đối với giá trị bộ đếm giữa thời điểm lệnh _rs`ApplyPendingBalance`_ được tạo
   và được xử lý:
   - `expected_pending_balance_credit_counter`: Giá trị
...
yêu cầu client tra cứu các lệnh deposit/transfer gần đây khớp với sự chênh lệch
bộ đếm để tính toán số dư chính xác.

**Quy trình đối chiếu số dư**

Khi các bộ đếm số dư chờ xử lý dự kiến và thực tế khác nhau, hãy làm theo các
bước sau để đối chiếu `decryptable_available_balance`:

1. Bắt đầu với `decryptable_available_balance` từ token account
2. Truy xuất các giao dịch gần đây nhất bao gồm các lệnh deposit và transfer lên
   đến chênh lệch bộ đếm (thực tế - dự kiến):
   - Cộng các số tiền công khai từ các lệnh deposit
   - Giải mã và cộng các số tiền ciphertext đích từ các lệnh transfer
...
3. **Cấu hình chuyển khoản bảo mật**: Gọi lệnh
...
_rs`PubkeyValidityProofData`_ tạo một chứng minh xác thực rằng khóa ElGamal hợp
lệ. Để biết chi tiết triển khai, xem:
