...
1. Tạo
   ...
   **Bằng chứng bình đẳng
   ...
   Xác minh rằng ciphertext số dư khả dụng còn lại sau khi rút khớp với
   ...
   **Bằng chứng phạm vi
   ...
   Xác minh rằng số dư khả dụng còn lại sau khi rút không âm và nằm trong phạm
   vi được chỉ định.

2. Đối với mỗi bằng chứng:
   - Gọi ZK ElGamal proof program để xác minh dữ liệu bằng chứng.
   - Lưu trữ metadata cụ thể của bằng chứng trong tài khoản "context state" để
     sử dụng trong các instruction khác.
...
4. Đóng các tài khoản bằng chứng để thu hồi SOL đã sử dụng để tạo chúng.
...
### Các instruction bắt buộc

Để rút token từ số dư khả dụng bảo mật sang số dư công khai, bạn phải:

- Tạo bằng chứng bình đẳng và bằng chứng phạm vi ở phía client
- Gọi Zk ElGamal proof program để xác minh các bằng chứng và khởi tạo các tài
  khoản "context state"
- Gọi instruction
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  cung cấp hai tài khoản bằng chứng.
