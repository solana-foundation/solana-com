...
1. Tạo
   ...
   **Bằng chứng bằng nhau
   ...
   Xác minh rằng văn bản mã hóa số dư khả dụng còn lại sau khi rút khớp với
   [cam kết Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme) tương
   ứng, đảm bảo số dư khả dụng mới của tài khoản được tính đúng là
   ...
   **Bằng chứng phạm vi
   ...
   Xác minh rằng số dư khả dụng còn lại sau khi rút là không âm và nằm trong một
   phạm vi xác định.
...
4. Đóng các tài khoản bằng chứng để thu hồi SOL đã dùng để tạo chúng.

Sơ đồ sau đây mô tả các bước liên quan đến việc rút token từ số dư khả dụng bảo
mật sang số dư công khai:
...
- Tạo bằng chứng bằng nhau và bằng chứng phạm vi ở phía client
- Gọi chương trình bằng chứng Zk ElGamal để xác minh các bằng chứng và khởi tạo
  các tài khoản "context state"
- Gọi lệnh
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  cung cấp hai tài khoản bằng chứng.
- Đóng hai tài khoản bằng chứng để thu hồi rent.
...
một tài khoản context state thông qua chương trình ZK ElGamal Proof, tham chiếu
cả hai tài khoản trong lệnh rút, và đóng chúng sau đó. Ví dụ TypeScript sử dụng
hàm trợ giúp `getConfidentialWithdrawInstructionPlan` từ
`@solana-program/token-2022/confidential`, giúp tập hợp cùng các tài khoản bằng
chứng, lệnh rút và đóng thành một kế hoạch lệnh đa giao dịch.

## Ví Dụ Mã

Ví dụ sau đây rút token từ số dư khả dụng bảo mật
trở lại số dư công khai. Tài khoản phải được cấu hình sẵn cho
Số Dư Bảo Mật và có số dư bảo mật khả dụng.

Số Dư Bảo Mật phụ thuộc vào chương trình ZK ElGamal Proof, được kích hoạt
trên mainnet và devnet. Một `solana-test-validator` mặc định không kích hoạt nó, nhưng một
validator cục bộ fork từ mainnet như [Surfpool](https://surfpool.run) thì có.
Chạy ví dụ với một trong số đó (mã sử dụng devnet) với một payer được nạp tiền,
và thay thế các placeholder bằng mint và token account của bạn.
