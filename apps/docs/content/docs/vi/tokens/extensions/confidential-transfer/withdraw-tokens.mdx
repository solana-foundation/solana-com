...
Ví dụ sau đây rút token từ số dư khả dụng bảo mật trở lại số dư công khai. Tài
khoản phải được cấu hình sẵn cho chuyển khoản bảo mật và có số dư bảo mật khả
dụng.

Chuyển khoản bảo mật phụ thuộc vào chương trình ZK ElGamal Proof, được kích hoạt
trên mainnet và devnet. Một `solana-test-validator` thông thường không kích hoạt
nó, nhưng một validator cục bộ forking mainnet như
[Surfpool](https://surfpool.run) thì có. Chạy ví dụ với một trong số đó (mã sử
dụng devnet) với một payer được nạp tiền, và thay thế các placeholder bằng mint
và token account của bạn.
