...
<Callout type="warn">
  ...
  đang trải qua kiểm toán bảo mật. Điều này có nghĩa là tiện ích mở rộng chuyển
  khoản bảo mật hiện không khả dụng. Mặc dù các khái niệm vẫn hợp lệ, nhưng các
  ví dụ mã sẽ không chạy được.
</Callout>

## Cách chuyển token một cách bảo mật từ tài khoản token này sang tài khoản khác

Để chuyển token một cách bảo mật từ tài khoản token này sang tài khoản khác, cả
người gửi và người nhận đều phải có tài khoản token được cấu hình với trạng thái
_rs`ConfidentialTransferAccount`_ và được phê duyệt cho chuyển khoản bảo mật.
Tài khoản token của người gửi cũng phải có số dư bảo mật khả dụng để chuyển.
...
1. Tạo
   ...
   **Bằng chứng đẳng thức
   ...
   Xác minh rằng ciphertext số dư khả dụng mới sau khi chuyển khoản khớp với
   ...
   **Bằng chứng tính hợp lệ của ciphertext
   ...
   Xác minh rằng các ciphertext số tiền chuyển được tạo đúng cách cho cả ba bên
   (nguồn, đích và kiểm toán viên), đảm bảo số tiền chuyển được mã hóa chính xác
   dưới khóa công khai của từng bên.
   ...
   **Bằng chứng phạm vi
   ...
   Xác minh rằng số dư khả dụng mới và số tiền chuyển (được chia thành các bit
   thấp/cao) đều không âm và nằm trong phạm vi được chỉ định.

2. Đối với mỗi bằng chứng:
   - Gọi chương trình bằng chứng ZK ElGamal để xác minh dữ liệu bằng chứng.
   - Lưu trữ metadata cụ thể của bằng chứng trong tài khoản "trạng thái ngữ cảnh
     bằng chứng" để sử dụng trong các lệnh khác.
...
4. Đóng các tài khoản trạng thái ngữ cảnh bằng chứng để thu hồi SOL đã sử dụng
   để tạo chúng.

Sơ đồ sau đây cho thấy các bước liên quan đến việc chuyển token từ tài khoản
token của người gửi sang tài khoản token của người nhận.
...
- Tạo bằng chứng đẳng thức, bằng chứng tính hợp lệ của ciphertext và bằng chứng
  phạm vi ở phía client
...
- Phương thức `confidential_transfer_create_context_state_account` tạo một tài
  khoản bằng chứng.
- Phương thức `confidential_transfer_transfer` gọi lệnh `Transfer`.
...
<Callout type="info">
  Tại thời điểm viết bài, tính năng Confidential Transfers chưa được kích hoạt
  trên validator cục bộ mặc định. Bạn phải sao chép Token Extension Program từ
