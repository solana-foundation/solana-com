...
Để chuyển token một cách bí mật từ token account này sang token account khác, cả
người gửi và người nhận đều phải có token account được cấu hình với trạng thái
_rs`ConfidentialTransferAccount`_ và được phê duyệt cho chuyển khoản bí mật.
Token account của người gửi cũng phải có số dư bí mật khả dụng để thực hiện
chuyển khoản.
...
Ví dụ sau đây chuyển token một cách bảo mật từ tài khoản này sang tài khoản
khác. Cả hai tài khoản phải được cấu hình sẵn cho chuyển token bảo mật, và người
gửi phải có số dư bảo mật khả dụng.

Chuyển token bảo mật phụ thuộc vào chương trình ZK ElGamal Proof, vốn được kích
hoạt trên mainnet và devnet. Một `solana-test-validator` thông thường không kích
hoạt chương trình này, nhưng một validator cục bộ fork từ mainnet như
[Surfpool](https://surfpool.run) thì có. Hãy chạy ví dụ trên một trong số đó (mã
sử dụng devnet) với tài khoản payer đã được nạp tiền, và thay thế các
placeholder bằng mint của bạn cùng các tài khoản người gửi và người nhận.
