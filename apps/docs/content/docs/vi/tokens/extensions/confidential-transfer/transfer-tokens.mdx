...
title: Chuyển token
description:
  Tìm hiểu cách chuyển token một cách riêng tư từ tài khoản token này sang tài khoản khác.
---

## Cách chuyển token bí mật từ token account này sang token account khác

Để chuyển token bí mật từ token account này sang token account khác, cả người gửi và người nhận đều phải có token account được cấu hình với trạng thái _rs`ConfidentialTransferAccount`_ và được phê duyệt để sử dụng Confidential Balances. Token account của người gửi cũng phải có số dư bí mật khả dụng để thực hiện chuyển khoản.

Để chuyển token một cách bí mật:
...
1. Tạo
   ...
   **Bằng chứng đẳng thức
   ...
   Xác minh rằng bản mã số dư khả dụng mới sau khi chuyển khoản khớp với
   ...
   **Bằng chứng hợp lệ bản mã
   ...
   Xác minh rằng các bản mã số tiền chuyển khoản được tạo đúng cách cho cả ba
   bên (nguồn, đích và kiểm toán viên), đảm bảo số tiền chuyển khoản được mã hóa
   chính xác bằng khóa công khai của từng bên.
   ...
   **Bằng chứng phạm vi
   ...
   Xác minh rằng số dư khả dụng mới và số tiền chuyển khoản (được chia thành các
   bit thấp/cao) đều không âm và nằm trong một phạm vi xác định.

2. Đối với mỗi bằng chứng:
   - Gọi chương trình bằng chứng ZK ElGamal để xác minh dữ liệu bằng chứng.
   - Lưu trữ siêu dữ liệu đặc thù của bằng chứng trong tài khoản "context state"
     của bằng chứng để sử dụng trong các lệnh khác.
...
3. Gọi lệnh
   ...
   cung cấp các tài khoản context state của bằng chứng.

4. Đóng các tài khoản context state của bằng chứng để thu hồi SOL đã dùng để tạo
   chúng.

Sơ đồ sau đây mô tả các bước liên quan đến việc chuyển token từ token account
của người gửi sang token account của người nhận.
...
Để chuyển token một cách bí mật từ token account này sang token account khác,
bạn phải:

- Tạo bằng chứng đẳng thức, bằng chứng hợp lệ bản mã và bằng chứng phạm vi phía
  client
- Gọi chương trình bằng chứng Zk ElGamal để xác minh các bằng chứng và khởi tạo
  các tài khoản "context state"
- Gọi lệnh
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  cung cấp ba tài khoản bằng chứng.
- Đóng ba tài khoản bằng chứng để thu hồi rent.

Ví dụ Rust bên dưới tạo các bằng chứng bằng crate
`spl-token-confidential-transfer-proof-generation`, xác minh từng bằng chứng vào
tài khoản trạng thái context thông qua chương trình ZK ElGamal Proof, tham chiếu
ba tài khoản trong lệnh chuyển, và đóng chúng sau đó. Ví dụ TypeScript sử dụng
helper `getConfidentialTransferInstructionPlan` từ
`@solana-program/token-2022/confidential`, giúp tập hợp các tài khoản bằng
chứng, lệnh chuyển và đóng tài khoản tương tự thành một kế hoạch lệnh đa giao
dịch.

## Mã Ví Dụ

Ví dụ sau đây thực hiện chuyển token bí mật từ tài khoản này sang tài khoản khác. Cả hai tài khoản phải được cấu hình sẵn cho Confidential Balances, và người gửi phải có số dư bí mật khả dụng.

Confidential Balances phụ thuộc vào chương trình ZK ElGamal Proof, chương trình này đã được bật trên mainnet và devnet. Một `solana-test-validator` thông thường không bật tính năng này, nhưng một validator cục bộ fork từ mainnet như [Surfpool](https://surfpool.run) thì có.
Hãy chạy ví dụ với một trong số đó (mã sử dụng devnet) cùng với một tài khoản payer đã được nạp tiền, và thay thế các placeholder bằng mint của bạn cùng các tài khoản người gửi và người nhận.
