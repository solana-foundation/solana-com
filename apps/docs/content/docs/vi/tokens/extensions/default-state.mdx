...
description:
  Cấu hình mint của Token Extensions Program để các tài khoản token mới được tạo
  bắt đầu ở trạng thái mặc định đã chọn, chẳng hạn như bị đóng băng.
...
Tiện ích mở rộng mint _rs`DefaultAccountState`_ của Token Extensions Program
thiết lập _rs`AccountState`_ ban đầu cho mọi token account mới được tạo cho mint
đó.

Điều này thường được sử dụng để làm cho các token account mới bắt đầu ở trạng
thái đóng băng. Các token account bắt đầu bị đóng băng không thể được sử dụng
cho đến khi quyền hạn đóng băng của mint thực hiện rã đóng băng cho chúng.
...
1. Tính toán kích thước mint account và rent cần thiết cho mint và tiện ích mở
   rộng _rs`DefaultAccountState`_.
2. Tạo mint account với _rs`CreateAccount`_, khởi tạo _rs`DefaultAccountState`_,
   và khởi tạo mint với _rs`InitializeMint`_.
3. Tạo các token account cho mint và quan sát rằng mỗi token account mới bắt đầu
   ở trạng thái mặc định hiện tại.
4. Sử dụng _rs`UpdateDefaultAccountState`_ để thay đổi trạng thái mà các token
   account trong tương lai nên bắt đầu.
...
_rs`DefaultAccountState`_. Đây là kích thước được sử dụng trong
_rs`CreateAccount`_.
...
## !!steps Khởi tạo DefaultAccountState

Khởi tạo extension _rs`DefaultAccountState`_ trên mint.
...
Tạo một token account cho mint. Vì trạng thái mặc định hiện tại là _rs`Frozen`_,
...
## !!steps Cập nhật trạng thái mặc định của account

Sử dụng _rs`UpdateDefaultAccountState`_ để thay đổi trạng thái mặc định sao cho
...
<Callout type="info" title="Thứ tự lệnh">
  _rs`DefaultAccountStateInstruction::Initialize`_ phải được thực hiện trước
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`DefaultAccountStateInstruction::Initialize`_, và _rs`InitializeMint`_ phải
...
| Mục                                              | Mô tả                                                                                               | Nguồn                                                                                                                                             |
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`DefaultAccountState`_                        | Extension của mint lưu trữ trạng thái mà các token account mới sẽ kế thừa.                          | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/mod.rs#L16-L21)         |
| _rs`DefaultAccountStateInstruction::Initialize`_ | Lệnh khởi tạo trạng thái mặc định của token account cho mint trước _rs`InitializeMint`_.            | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L20-L36) |
| _rs`DefaultAccountStateInstruction::Update`_     | Lệnh cho phép freeze authority thay đổi trạng thái mặc định cho các token account trong tương lai.  | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L37-L53) |
| _rs`process_initialize_default_account_state`_   | Logic xử lý ghi trạng thái mặc định ban đầu lên mint chưa được khởi tạo.                            | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L29-L40)     |
| _rs`process_update_default_account_state`_       | Logic xử lý xác minh freeze authority trước khi thay đổi trạng thái mặc định được lưu trữ của mint. | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L43-L74)     |
