...
description:
  Khởi tạo tài khoản token của Token Extensions Program với quyền sở hữu bất
  biến để chủ sở hữu không bao giờ có thể được chỉ định lại.
...
## Immutable Owner Là Gì?

Tiện ích mở rộng tài khoản _rs`ImmutableOwner`_ của Token Extensions Program làm
cho chủ sở hữu của token account trở nên cố định.

Sau khi token account được khởi tạo với _rs`ImmutableOwner`_, các lệnh
_rs`SetAuthority`_ sau đó không thể thay đổi quyền hạn _rs`AccountOwner`_ của
nó.

<Callout type="info" title="Associated Token Accounts">
  Associated token accounts được tạo thông qua Associated Token Program cho
  Token Extensions Program sẽ tự động khởi tạo _rs`ImmutableOwner`_ trên token
  account.
</Callout>

## Cách Khởi Tạo Tài Khoản Immutable Owner

Để khởi tạo tài khoản immutable owner:

1. Tạo và khởi tạo một mint.
2. Tạo token account với đủ không gian cho _rs`ImmutableOwner`_.
3. Khởi tạo _rs`ImmutableOwner`_ trên token account trước khi thực hiện
   _rs`InitializeAccount`_.
4. Việc thử thay đổi chủ sở hữu tài khoản sau đó bằng _rs`SetAuthority`_ sẽ thất
   bại với lỗi _rs`TokenError::ImmutableOwner`_.

<ScrollyCoding>

## !!steps Tính toán kích thước token account

Tính toán kích thước token account cho token account cơ bản cộng với tiện ích mở
rộng _rs`ImmutableOwner`_. Đây là kích thước được sử dụng trong
_rs`CreateAccount`_.
...
Tính toán rent sử dụng kích thước token account cần thiết cho tiện ích mở rộng
...
Tạo token account với không gian và lamports đã được tính toán.
...
## !!steps Khởi tạo ImmutableOwner

Khởi tạo phần mở rộng _rs`ImmutableOwner`_ trên token account.
...
## !!steps Khởi tạo token account

Khởi tạo token account với _rs`InitializeAccount`_ trong cùng một giao dịch.
...
<Callout type="info" title="Thứ tự lệnh">
  _rs`InitializeImmutableOwner`_ phải được thực hiện trước
  _rs`InitializeAccount`_. _rs`CreateAccount`_, _rs`InitializeImmutableOwner`_,
  và _rs`InitializeAccount`_ phải được bao gồm trong cùng một giao dịch.
</Callout>
...
| Mục                                      | Mô tả                                                                                  | Nguồn                                                                                                                           |
| ---------------------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| _rs`ImmutableOwner`_                     | Phần mở rộng tài khoản đánh dấu chủ sở hữu token account là bất biến.                  | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L13-L16) |
| _rs`InitializeImmutableOwner`_           | Lệnh khởi tạo phần mở rộng immutable-owner trên token account chưa được khởi tạo.      | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1982-L1991)           |
| _rs`process_initialize_immutable_owner`_ | Xử lý logic để cấp phát và khởi tạo _rs`ImmutableOwner`_ trên tài khoản.               | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1532-L1543)               |
| _rs`SetAuthority`_                       | Lệnh token cơ bản được sử dụng để thay đổi quyền, bao gồm chủ sở hữu token account.    | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1611-L1633)           |
| _rs`process_set_authority`_              | Xử lý logic từ chối thay đổi chủ sở hữu khi token account đã bật _rs`ImmutableOwner`_. | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L703-L900)                 |

### Typescript

Ví dụ `Kit` dưới đây sử dụng trực tiếp các lệnh đã được tạo. Các ví dụ cũ sử
dụng `@solana/web3.js` và `@solana/spl-token` được đính kèm để tham khảo.
