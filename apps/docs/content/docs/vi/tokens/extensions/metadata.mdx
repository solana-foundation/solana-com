...
description:
  Tạo một mint lưu trữ metadata trực tiếp trên mint account bằng cách sử dụng các extension MetadataPointer và TokenMetadata, sau đó cập nhật, xóa và khóa metadata đó thông qua giao diện token-metadata.
...
Extension mint _rs`MetadataPointer`_ của Token Extensions Program lưu trữ hai
trường trên một mint:

- Một authority có thể cập nhật con trỏ
- Địa chỉ của tài khoản lưu trữ metadata của token

Con trỏ đó có thể tham chiếu đến bất kỳ tài khoản nào được sở hữu bởi một chương
trình triển khai
[giao diện token-metadata](https://github.com/solana-program/token-metadata/tree/main/interface).

Token Extensions Program cũng triển khai giao diện tương tự trực tiếp thông qua
extension mint _rs`TokenMetadata`_. Với _rs`TokenMetadata`_, mint lưu trữ
_rs`name`_, _rs`symbol`_, _rs`uri`_, _rs`update_authority`_, và metadata tùy
chỉnh của token trực tiếp trên mint account.

<Callout type="info" title="Off-chain Metadata URI">
  Trường _rs`uri`_ trỏ đến metadata JSON off-chain. Xem [Định dạng Metadata
  Off-chain](/docs/tokens/metaplex#off-chain-metadata-format).
</Callout>

Hai extension giải quyết các vấn đề khác nhau:

- _rs`MetadataPointer`_ chỉ định tài khoản lưu trữ metadata.
- _rs`TokenMetadata`_ lưu trữ metadata trực tiếp trên mint account.

<Callout type="info" title="Extension Độ Dài Biến Đổi">
  _rs`TokenMetadata`_ là một extension TLV có độ dài biến đổi.
  _rs`InitializeTokenMetadata`_, _rs`UpdateField`_, và _rs`RemoveKey`_ có thể
...
1. Tính toán kích thước mint account và rent cần thiết cho mint, các extension
   và metadata.
2. Tạo mint account với _rs`CreateAccount`_, khởi tạo _rs`MetadataPointer`_, và
   khởi tạo mint với _rs`InitializeMint`_.
3. Khởi tạo _rs`TokenMetadata`_ trên mint account, sau đó sử dụng
   _rs`UpdateField`_ để thêm hoặc cập nhật metadata.
4. Sử dụng _rs`RemoveKey`_, _rs`UpdateAuthority`_, và _rs`Emit`_ để xóa metadata
   tùy chỉnh, thay đổi hoặc xóa quyền cập nhật, hoặc trả về metadata hiện tại
   trong dữ liệu trả về của giao dịch.
...
_rs`MetadataPointer`_. Đây là kích thước được sử dụng trong _rs`CreateAccount`_.
...
Tính toán rent sử dụng kích thước tối đa cần thiết sau khi _rs`TokenMetadata`_
...
## !!steps Khởi tạo MetadataPointer

Khởi tạo _rs`MetadataPointer`_ và đặt địa chỉ metadata của nó thành địa chỉ
mint.
...
## !!steps Khởi tạo và cập nhật metadata

Khởi tạo _rs`TokenMetadata`_ trên mint. Sử dụng _rs`UpdateField`_ để thêm
metadata tùy chỉnh; nếu trường không tồn tại, _rs`UpdateField`_ sẽ thêm nó.
...
## !!steps Cập nhật, xóa hoặc phát ra metadata

Sau khi mint được khởi tạo, sử dụng _rs`UpdateField`_ để cập nhật metadata,
_rs`UpdateMetadataPointer`_ để cập nhật metadata pointer, _rs`RemoveKey`_ để xóa
metadata tùy chỉnh, _rs`UpdateAuthority`_ để xóa quyền cập nhật, và _rs`Emit`_
...
<Callout type="info" title="Thứ tự lệnh">
  _rs`MetadataPointerInstruction::Initialize`_ phải được thực hiện trước
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`MetadataPointerInstruction::Initialize`_, và _rs`InitializeMint`_ phải
...
| Mục                                         | Mô tả                                                                                                       | Nguồn                                                                                                                                        |
| -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`MetadataPointer`_                        | Mint extension lưu trữ quyền metadata pointer và địa chỉ tài khoản metadata.                       | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/metadata_pointer/mod.rs#L12-L22)         |
| _rs`MetadataPointerInstruction::Initialize`_ | Khởi tạo metadata pointer extension trước _rs`InitializeMint`_.                                           | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/metadata_pointer/instruction.rs#L22-L38) |
| _rs`MetadataPointerInstruction::Update`_     | Cập nhật địa chỉ metadata được lưu trữ bởi metadata pointer extension của mint.                                     | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/metadata_pointer/instruction.rs#L39-L55) |
| _rs`process_initialize`_ (`MetadataPointer`) | Logic xử lý metadata pointer yêu cầu ít nhất một quyền hoặc một địa chỉ metadata trong quá trình khởi tạo. | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/metadata_pointer/processor.rs#L25-L48)     |
| _rs`process_update`_ (`MetadataPointer`)     | Xác thực quyền metadata pointer, sau đó ghi đè địa chỉ metadata được lưu trữ của mint.                       | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/metadata_pointer/processor.rs#L51-L77)     |

#### Token Metadata

| Mục                                            | Mô tả                                                                                                                         | Nguồn                                                                                                                                    |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`TokenMetadata`_                             | Trạng thái giao diện token-metadata có độ dài biến được lưu trữ trong mục TLV.                                                               | [Nguồn](https://github.com/solana-program/token-metadata/blob/interface%40v1.0.0/interface/src/state.rs#L18-L38)                         |
| _rs`Field`_                                     | Enum trường được sử dụng bởi _rs`UpdateField`_ để nhắm đến _rs`name`_, _rs`symbol`_, _rs`uri`_, hoặc khóa tùy chỉnh.                                | [Nguồn](https://github.com/solana-program/token-metadata/blob/interface%40v1.0.0/interface/src/state.rs#L109-L122)                       |
| _rs`TokenMetadataInstruction::Initialize`_      | Khởi tạo các trường cơ bản _rs`name`_, _rs`symbol`_, và _rs`uri`_ cho tài khoản token-metadata.                                   | [Nguồn](https://github.com/solana-program/token-metadata/blob/interface%40v1.0.0/interface/src/instruction.rs#L81-L95)                   |
| _rs`TokenMetadataInstruction::UpdateField`_     | Thêm hoặc cập nhật trường cơ bản hoặc trường metadata tùy chỉnh trên token metadata.                                                        | [Nguồn](https://github.com/solana-program/token-metadata/blob/interface%40v1.0.0/interface/src/instruction.rs#L97-L117)                  |
| _rs`TokenMetadataInstruction::RemoveKey`_       | Xóa khóa metadata tùy chỉnh. Các trường cơ bản không thể bị xóa bằng lệnh này.                                                 | [Nguồn](https://github.com/solana-program/token-metadata/blob/interface%40v1.0.0/interface/src/instruction.rs#L119-L134)                 |
| _rs`TokenMetadataInstruction::UpdateAuthority`_ | Xoay quyền cập nhật metadata, hoặc xóa hoàn toàn để làm cho metadata không thể thay đổi.                                            | [Nguồn](https://github.com/solana-program/token-metadata/blob/interface%40v1.0.0/interface/src/instruction.rs#L136-L144)                 |
| _rs`TokenMetadataInstruction::Emit`_            | Trả về metadata được serialize thông qua dữ liệu trả về của giao dịch, tùy chọn cho một phạm vi byte.                                       | [Nguồn](https://github.com/solana-program/token-metadata/blob/interface%40v1.0.0/interface/src/instruction.rs#L146-L159)                 |
| _rs`process_initialize`_ (`TokenMetadata`)      | Logic xử lý token-metadata yêu cầu tài khoản metadata phải là chính mint và mint phải có _rs`MetadataPointer`_. | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_metadata/processor.rs#L43-L101)  |
| _rs`process_update_field`_                      | Phân bổ lại và ghi đè mục TLV _rs`TokenMetadata`_ có độ dài biến sau khi cập nhật trường.                                    | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_metadata/processor.rs#L104-L132) |
| _rs`process_remove_key`_                        | Xác thực quyền cập nhật, xóa khóa metadata tùy chỉnh, và ghi đè mục TLV.                                          | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_metadata/processor.rs#L135-L159) |
| _rs`process_update_authority`_                  | Xác thực quyền cập nhật hiện tại, sau đó xoay hoặc xóa quyền metadata tại chỗ.                                     | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_metadata/processor.rs#L162-L187) |
| _rs`process_emit`_                              | Đọc _rs`TokenMetadata`_ đã serialize từ mint và ghi phần được yêu cầu vào dữ liệu trả về.                               | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_metadata/processor.rs#L190-L203) |

### Typescript

Ví dụ `Kit` bên dưới sử dụng trực tiếp các hướng dẫn đã được tạo. Các ví dụ cũ hơn sử dụng `@solana/web3.js`, `@solana/spl-token` và
`@solana/spl-token-metadata` được đưa vào để tham khảo. Các ví dụ cũ giới hạn
`maxSupportedTransactionVersion` ở mức `0` vì web3.js v1 không thể giải mã hóa
