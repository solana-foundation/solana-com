...
cho phép một mint account chỉ định địa chỉ của tài khoản metadata của nó. Con
trỏ này có thể tham chiếu đến bất kỳ tài khoản nào thuộc sở hữu của một chương
trình triển khai
[Token Metadata Interface](https://github.com/solana-program/token-metadata/tree/main),
mang lại sự linh hoạt trong việc lưu trữ metadata.
