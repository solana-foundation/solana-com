---
title: Chuyển Khoản với Memo
description:
  Yêu cầu các giao dịch chuyển đến phải bao gồm memo bằng cách kích hoạt tiện
  ích mở rộng tài khoản MemoTransfer trên token account của Token Extensions
  Program.
...
## Chuyển Khoản với Memo là gì?

Tiện ích mở rộng tài khoản _rs`MemoTransfer`_ của Token Extensions Program yêu
cầu một lệnh memo ngay trước bất kỳ giao dịch chuyển đến nào tới token account
đó. Các giao dịch chuyển đi từ token account đó không yêu cầu memo.

Tiện ích mở rộng này được cấu hình trên token account đích, không phải trên
mint. Sau khi được kích hoạt, các giao dịch chuyển khoản không có memo sẽ thất
bại với lỗi _rs`TokenError::NoMemo`_ cho đến khi chủ sở hữu token account vô
hiệu hóa yêu cầu này.

## Cách Tạo Token Account với MemoTransfer Được Kích Hoạt

Để tạo một token account với _rs`MemoTransfer`_ được kích hoạt:

1. Tạo và khởi tạo một mint.
2. Tính toán kích thước token account và số tiền rent cần thiết cho một token
   account có _rs`MemoTransfer`_.
3. Tạo token account và kích hoạt _rs`MemoTransfer`_ trên tài khoản đó.
4. Các lệnh _rs`Transfer`_ và _rs`TransferChecked`_ tới token account đó phải
   bao gồm một lệnh memo ngay trước lệnh chuyển khoản.
...
rộng _rs`MemoTransfer`_. Đây là kích thước được sử dụng trong
_rs`CreateAccount`_.
...
Tạo token account với dung lượng và lamport đã tính toán, sau đó khởi tạo nó cho
mint.
...
## !!steps Bật MemoTransfer

Bật _rs`MemoTransfer`_ trên token account.
...
| Mục                                             | Mô tả                                                                                           | Nguồn                                                                                                                                     |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`MemoTransfer`_                              | Phần mở rộng tài khoản lưu trữ liệu các giao dịch chuyển đến có yêu cầu memo hay không.         | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/mod.rs#L17-L22)         |
| _rs`RequiredMemoTransfersInstruction::Enable`_  | Lệnh bật yêu cầu memo cho các giao dịch chuyển trên token account.                              | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L50-L71) |
| _rs`RequiredMemoTransfersInstruction::Disable`_ | Lệnh tắt yêu cầu memo cho các giao dịch chuyển trên token account.                              | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L74-L95) |
| _rs`process_toggle_required_memo_transfers`_    | Logic xử lý khởi tạo hoặc ghi đè phần mở rộng _rs`MemoTransfer`_ sau khi xác minh quyền sở hữu. | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/memo_transfer/processor.rs#L20-L48)     |

### Typescript

Ví dụ `Kit` bên dưới sử dụng trực tiếp các lệnh đã được tạo. Các ví dụ cũ sử
dụng `@solana/web3.js`, `@solana/spl-token`, và `@solana/spl-memo` được bao gồm
để tham khảo.
