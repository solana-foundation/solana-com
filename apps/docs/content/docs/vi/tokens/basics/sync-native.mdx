...
Wrapped SOL (WSOL) là một token account theo dõi số lượng lamports trong tài
...
đồng bộ hóa số dư token account của wrapped SOL (WSOL) với SOL thực tế
(lamports) được lưu trữ trong đó. Khi bạn chuyển SOL gốc vào một token account
WSOL, số dư token không tự động cập nhật. Bạn phải gọi SyncNative để phản ánh số
dư WSOL chính xác.
