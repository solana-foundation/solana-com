...
description:
  Cập nhật số lượng trong token account SOL được bọc sau khi chuyển SOL vào tài
  khoản.
...
Wrapped SOL (WSOL) là SOL được giữ trong một token account cho native mint của
Token Program. Native mint là địa chỉ mint mà Token Program sử dụng cho các
token account bọc SOL. Native mint cho phép SOL được sử dụng thông qua các lệnh
của Token Program.

| Chương trình             | Địa chỉ native mint                            |
| ------------------------ | ---------------------------------------------- |
...
Một wrapped SOL token account lưu trữ SOL trong trường `lamports` của tài khoản,
nhưng theo dõi số SOL được giữ trong wrapped SOL token account dưới dạng số dư
token thông qua trường _rs`amount`_ của token account. Nếu bạn chuyển SOL vào
một wrapped SOL token account, số dư lamport sẽ tăng ngay lập tức, nhưng trường
_rs`amount`_ của token sẽ không thay đổi cho đến khi bạn gọi lệnh
_rs`SyncNative`_ của Token Program.

## Cách đồng bộ hóa Native SOL

Sử dụng lệnh _rs`SyncNative`_ của Token Program sau khi chuyển SOL vào một
wrapped SOL token account. Lệnh _rs`SyncNative`_ cập nhật trường _rs`amount`_
của token account để khớp với số lamport được giữ trong tài khoản trên mức dự
trữ miễn phí tiền thuê của nó.

### Tham chiếu nguồn

| Mục                       | Mô tả                                                                                                                                                                                                       | Token Program                                                                                                 | Token Extensions Program                                                                                            |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_             | Trạng thái token account sử dụng _rs`is_native`_ để đánh dấu tài khoản là WSOL và lưu trữ dự trữ native, số dư lamport tối thiểu miễn phí tiền thuê mà tài khoản phải giữ, và số lượng token được theo dõi. | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`SyncNative`_          | Một lệnh cập nhật trường _rs`amount`_ của wrapped SOL token account để khớp với số lamport được giữ trong tài khoản trên mức dự trữ miễn phí tiền thuê của nó.                                              | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| _rs`process_sync_native`_ | Logic xử lý chung để đồng bộ hóa số dư wrapped SOL.                                                                                                                                                         | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |

### Typescript

Các ví dụ dưới đây cho thấy phương pháp được khuyến nghị sử dụng `Kit`. Các ví
