...
description:
  Cập nhật số lượng trong token account SOL được gói sau khi chuyển SOL vào tài
  khoản.
...
SOL được gói (WSOL) là SOL được giữ trong token account cho native mint của
Token Program. Native mint là địa chỉ mint mà Token Program sử dụng cho các
token account gói SOL. Native mint cho phép SOL được sử dụng thông qua các lệnh
của Token Program.
...
Token account SOL được gói lưu trữ SOL trong trường `lamports` của tài khoản,
nhưng theo dõi lượng SOL được giữ trong token account SOL được gói dưới dạng số
dư token thông qua trường _rs`amount`_ của token account. Nếu bạn chuyển SOL vào
token account SOL được gói, số dư lamport tăng ngay lập tức, nhưng trường
_rs`amount`_ của token không thay đổi cho đến khi bạn gọi lệnh _rs`SyncNative`_
của Token Program.

## Cách Đồng Bộ SOL Native

Sử dụng lệnh _rs`SyncNative`_ của Token Program sau khi chuyển SOL vào token
account SOL được gói. Lệnh _rs`SyncNative`_ cập nhật trường _rs`amount`_ của
token account để khớp với số lamport được giữ trong tài khoản vượt quá dự trữ
miễn thuê của nó.
...
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_             | Trạng thái token account sử dụng _rs`is_native`_ để đánh dấu tài khoản là WSOL và lưu trữ dự trữ native, số dư lamport tối thiểu miễn thuê mà tài khoản phải giữ, và số lượng token được theo dõi. | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`SyncNative`_          | Một lệnh cập nhật trường _rs`amount`_ của token account SOL được gói để khớp với số lamport được giữ trong tài khoản vượt quá dự trữ miễn thuê của nó.                                             | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| _rs`process_sync_native`_ | Logic xử lý chung để đồng bộ hóa số dư SOL được gói.                                                                                                                                               | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |
