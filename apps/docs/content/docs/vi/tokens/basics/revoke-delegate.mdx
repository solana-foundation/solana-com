...
description:
  Xóa delegate hiện tại của token account và xóa hạn mức ủy quyền còn lại.
...
## Thu Hồi Delegate Làm Gì?

Thu hồi delegate sẽ xóa delegate hiện tại của token account và đặt lại số lượng
đã ủy quyền về 0.

Sau khi thu hồi, delegate trước đó sẽ không còn quyền chuyển hoặc đốt token thay
mặt cho token account.

## Cách Thu Hồi Delegate

Thu hồi delegate sử dụng lệnh _rs`Revoke`_ của Token Program.

Lệnh _rs`Revoke`_ xóa các trường delegate trên token account nguồn. Trong Token
Program, chủ sở hữu token account ký xác nhận việc thu hồi. Trong Token
Extensions Program, delegate hiện tại cũng có thể thu hồi ủy quyền.

### Tài Liệu Tham Khảo Mã Nguồn

| Mục                  | Mô tả                                                              | Token Program                                                                                                 | Token Extensions Program                                                                                            |
| -------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_        | Trạng thái token account lưu trữ delegate và số lượng đã ủy quyền. | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Revoke`_         | Lệnh xóa delegate hiện tại khỏi token account.                     | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L128-L140) | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L170-L182) |
| _rs`process_revoke`_ | Logic xử lý chung cho việc thu hồi delegate.                       | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L395-L420)     | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L643-L674)     |
