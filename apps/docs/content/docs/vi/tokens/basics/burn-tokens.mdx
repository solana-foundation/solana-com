---
title: Đốt Token
description: Đốt token từ tài khoản token và giảm tổng nguồn cung của mint.
...
Đốt token giảm vĩnh viễn số dư của tài khoản token và giảm tổng nguồn cung của
mint đi cùng một lượng tương ứng.

## Cách Đốt Token

Đốt token sử dụng lệnh _rs`Burn`_ hoặc _rs`BurnChecked`_ của Token Program.

Các ví dụ dưới đây sử dụng _rs`BurnChecked`_, yêu cầu người gọi cung cấp số thập
phân của mint để lệnh có thể xác minh độ chính xác token mong đợi trước khi đốt.

[Native mint](/docs/tokens/basics/sync-native) không hỗ trợ đốt. Hãy sử dụng
_rs`CloseAccount`_ thay thế.

Chủ sở hữu tài khoản token hoặc người được ủy quyền đã phê duyệt sẽ ký giao
dịch. Trong Token Extension Program, một mint permanent delegate cũng có thể ủy
quyền đốt nếu mint đã bật tiện ích mở rộng permanent delegate.

### Tham Chiếu Nguồn

| Mục                | Mô tả                                                                                                                                     | Token Program                                                                                                  | Token Extension Program                                                                                              |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_         | Trạng thái mint lưu trữ tổng nguồn cung token.                                                                                            | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | Trạng thái tài khoản token lưu trữ số dư token bị giảm do đốt.                                                                            | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Lệnh đốt giảm số dư tài khoản token và nguồn cung mint mà không yêu cầu người gọi cung cấp số thập phân của mint.                         | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Lệnh đốt yêu cầu người gọi cung cấp số thập phân của mint và kiểm tra giá trị đó trước khi giảm số dư tài khoản token và nguồn cung mint. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | Logic xử lý chung cho việc đốt token.                                                                                                     | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |
