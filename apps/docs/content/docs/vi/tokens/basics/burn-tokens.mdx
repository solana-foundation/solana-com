...
tiêu hủy vĩnh viễn các token bằng cách giảm số dư trong tài khoản token. Các
token bị đốt sẽ được loại bỏ khỏi lưu thông và giảm `supply` được theo dõi trên
mint account. Chỉ chủ sở hữu tài khoản token mới có thể đốt token từ tài khoản
của họ.
