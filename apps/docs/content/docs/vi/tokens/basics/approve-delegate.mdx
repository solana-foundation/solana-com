...
description:
  Ủy quyền cho một địa chỉ khác để chuyển hoặc đốt một lượng token giới hạn từ
  một token account.
...
## Việc Phê Duyệt Delegate Có Tác Dụng Gì?

Phê duyệt một delegate cho phép một địa chỉ khác chuyển hoặc đốt một lượng token
giới hạn từ một token account thay mặt cho chủ sở hữu token account.

Một token account lưu trữ một delegate hiện tại và một số lượng được ủy quyền
tại một thời điểm. Phê duyệt một delegate mới sẽ thay thế delegate trước đó và
hạn mức trên tài khoản.

## Cách Phê Duyệt Một Delegate

Phê duyệt một delegate sử dụng lệnh _rs`Approve`_ hoặc _rs`ApproveChecked`_ của
Token Program.

Các ví dụ dưới đây sử dụng _rs`ApproveChecked`_, yêu cầu người gọi cung cấp mint
và decimals để lệnh có thể xác minh mint dự kiến và độ chính xác của token trước
khi xử lý phê duyệt.
...
| --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_         | Trạng thái token account lưu trữ chủ sở hữu, delegate và số lượng được ủy quyền.                                                          | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | Một lệnh phê duyệt delegate ghi lại delegate và hạn mức cho một token account mà không yêu cầu người gọi cung cấp mint hoặc decimals.     | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | Một lệnh phê duyệt delegate yêu cầu người gọi cung cấp mint và decimals và kiểm tra các giá trị đó trước khi ghi lại delegate và hạn mức. | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
| _rs`process_approve`_ | Logic xử lý chung cho phê duyệt delegate.                                                                                                 | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L344-L392)     | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L582-L640)     |

### Typescript

Các ví dụ `Kit` bên dưới cho thấy phương pháp được khuyến nghị sử dụng
