---
title: Phê Duyệt Người Được Ủy Quyền
description:
  Cho phép một địa chỉ khác chuyển hoặc đốt một lượng token giới hạn từ token
  account.
...
## Phê Duyệt Người Được Ủy Quyền Có Tác Dụng Gì?

Phê duyệt người được ủy quyền cho phép một địa chỉ khác chuyển hoặc đốt một
lượng token giới hạn từ token account thay mặt cho chủ sở hữu token account.

Một token account lưu trữ một người được ủy quyền hiện tại và một hạn mức được
ủy quyền tại một thời điểm. Phê duyệt người được ủy quyền mới sẽ thay thế người
được ủy quyền trước đó và hạn mức trên account.

## Cách Phê Duyệt Người Được Ủy Quyền

Phê duyệt người được ủy quyền sử dụng lệnh _rs`Approve`_ hoặc
_rs`ApproveChecked`_ của Token Program.

Các ví dụ dưới đây sử dụng _rs`ApproveChecked`_, yêu cầu người gọi cung cấp mint
và số chữ số thập phân để lệnh có thể xác minh mint dự kiến và độ chính xác của
token trước khi xử lý phê duyệt.

Chủ sở hữu token account ký phê duyệt.

### Tham Chiếu Mã Nguồn

| Mục                   | Mô Tả                                                                                                                                                               | Token Program                                                                                                 | Token Extensions Program                                                                                            |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_         | Trạng thái token account lưu trữ chủ sở hữu, người được ủy quyền và số lượng được ủy quyền.                                                                         | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | Lệnh phê duyệt ủy quyền ghi lại người được ủy quyền và hạn mức cho token account mà không yêu cầu người gọi cung cấp mint hoặc số chữ số thập phân.                 | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | Lệnh phê duyệt ủy quyền yêu cầu người gọi cung cấp mint và số chữ số thập phân, đồng thời kiểm tra các giá trị đó trước khi ghi lại người được ủy quyền và hạn mức. | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
| _rs`process_approve`_ | Logic xử lý chung cho việc phê duyệt ủy quyền.                                                                                                                      | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L344-L392)     | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L582-L640)     |
