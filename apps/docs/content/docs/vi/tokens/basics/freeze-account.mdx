...
description:
  Đóng băng tài khoản token bằng quyền đóng băng của mint để chặn các giao dịch
  chuyển, đốt và thay đổi ủy quyền.
...
Đóng băng tài khoản token giữ nguyên chủ sở hữu, mint và số dư, nhưng ngăn chặn
tài khoản token chuyển hoặc đốt token cho đến khi tài khoản token được mở băng.

Tài khoản đã đóng băng vẫn có thể được đóng nếu tài khoản đóng băng có số dư
token bằng không.

Chỉ mint còn có quyền đóng băng mới có thể đóng băng các tài khoản token. Nếu
...
| _rs`FreezeAccount`_                 | Lệnh thay đổi tài khoản token sang trạng thái đóng băng.                                        | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
| _rs`process_toggle_freeze_account`_ | Logic xử lý chung để đóng băng và mở băng các tài khoản token.                                  | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |
