...
description: Tìm hiểu cách thiết lập quyền hạn cho mint hoặc token account.
---

## Hiểu về quyền hạn token

### Quyền hạn mint account

- **Mint Authority**: Kiểm soát việc tạo token mới. Có thể mint token vào bất kỳ
  token account nào. Thường bị thu hồi sau khi nguồn cung ban đầu được tạo để
  tạo ra token có nguồn cung cố định.
- **Freeze Authority**: Kiểm soát khả năng đóng băng và rã đông token account.
  Có thể ngăn bất kỳ token account nào chuyển token. Thường bị thu hồi để đảm
  bảo người dùng rằng token của họ không thể bị đóng băng.

### Quyền hạn token account

- **Account Owner**: Có toàn quyền kiểm soát token account. Có thể chuyển token,
  đốt token, phê duyệt delegate và đóng account khi số dư bằng không.
- **Close Authority**: Có thể đóng token account khi số dư của nó bằng không.
  Theo mặc định, đây là account owner nhưng có thể được ủy quyền cho account
  khác.
...
thay đổi hoặc thu hồi quyền hạn trên mint và token account. Chỉ quyền hạn hiện
tại mới có thể chuyển quyền của họ sang địa chỉ mới hoặc thu hồi chúng vĩnh viễn
bằng cách đặt quyền hạn thành null. Sau khi bị thu hồi, quyền hạn không thể được
khôi phục.
...
<Callout type="info">
  ...
  chia sẻ các triển khai tương tự để đạt được cùng chức năng.
</Callout>
