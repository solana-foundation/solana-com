---
title: Mint token
description: Tìm hiểu cách mint các đơn vị mới của một token.
---

## Cách mint token

Mint tạo ra các đơn vị mới của một token bằng cách sử dụng lệnh
...
- Chỉ có mint authority mới có thể mint token mới
- Một token account đích phải tồn tại để nhận các token được mint
...
async fn main() -> Result<()> {
    // Tạo kết nối đến local validator
    ...
    // Tạo một keypair mới cho fee payer
    let fee_payer = Keypair::new();

    // Airdrop 1 SOL cho fee payer
    ...
    // Lấy kích thước mặc định của mint account (tính bằng byte), không có extension nào được bật
    ...
    // Lệnh để khởi tạo dữ liệu của mint account
    ...
    // Tạo transaction và thêm các lệnh
    ...
    // Số lượng token để mint (100 token với 2 chữ số thập phân)
    let amount = 100;

    // Tạo lệnh mint_to để mint token vào associated token account
    let mint_to_instruction = mint_to(
        &token_program_id(),
        &mint.pubkey(),            // mint
        &associated_token_address, // đích đến
        &fee_payer.pubkey(),       // quyền hạn
        &[&fee_payer.pubkey()],    // người ký
        amount,                    // số lượng
    )?;

    // Tạo transaction để mint token
    let transaction = Transaction::new_signed_with_payer(
        &[mint_to_instruction],
        Some(&fee_payer.pubkey()),
        &[&fee_payer],
    ...
    println!("Đã mint 1.00 token vào associated token account");
    println!("\nĐịa chỉ mint: {}", mint.pubkey());
    println!("{:#?}", mint_data);

    println!(
        "\nĐịa chỉ associated token account: {}",
        associated_token_address
    );
    println!("{:#?}", token_data);

    println!("Chữ ký transaction: {}", transaction_signature);
...
async def main():
    ...
    # Số lượng cần mint (đơn vị nhỏ nhất)
    amount_to_mint = 1000000000  # 1 token với 9 chữ số thập phân
    ...
    async with rpc:
        ...
        print(f"Đích đến: {destination_token_account}")
        print(f"Số lượng: {amount_to_mint}")
        print(f"Quyền mint: {mint_authority.pubkey()}")
        print(f"Người trả phí: {payer.pubkey()}")
        print(f"Đã tạo transaction mint thành công")
