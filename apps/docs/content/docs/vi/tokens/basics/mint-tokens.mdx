---
title: Đúc token
description: Đúc các đơn vị token mới vào tài khoản token với quyền đúc.
...
Đúc token tạo ra các đơn vị mới của một token hiện có và tăng tổng cung của
mint.

Chỉ có quyền đúc mới có thể đúc token mới. Tài khoản token đích phải đã tồn tại
và phải thuộc về cùng một mint.

## Cách Đúc Token

Đúc token sử dụng lệnh _rs`MintTo`_ hoặc _rs`MintToChecked`_ của Token Program.

Các ví dụ dưới đây sử dụng _rs`MintToChecked`_, yêu cầu người gọi cung cấp số
thập phân của mint để lệnh có thể xác minh độ chính xác token mong đợi trước khi
đúc.

[Native mint](/docs/tokens/basics/sync-native) không hỗ trợ đúc token.

### Tài Liệu Tham Khảo Nguồn

| Mục                   | Mô Tả                                                                                                                             | Token Program                                                                                                 | Token Extensions Program                                                                                            |
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_            | Trạng thái mint lưu trữ quyền đúc và tổng cung token.                                                                             | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_         | Trạng thái tài khoản token lưu trữ số dư đích được cập nhật bởi quá trình đúc.                                                    | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`MintTo`_          | Lệnh đúc token vào tài khoản token hiện có mà không yêu cầu người gọi cung cấp số thập phân của mint.                             | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L159-L177) | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L202-L220) |
| _rs`MintToChecked`_   | Lệnh đúc yêu cầu người gọi cung cấp số thập phân của mint và kiểm tra giá trị đó trước khi đúc token vào tài khoản token hiện có. | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L301-L325) | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L367-L391) |
| _rs`process_mint_to`_ | Logic xử lý dùng chung cho việc đúc token.                                                                                        | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L521-L583)     | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L981-L1067)    |

### Typescript

Các ví dụ `Kit` dưới đây trình bày cách tiếp cận được khuyến nghị sử dụng
