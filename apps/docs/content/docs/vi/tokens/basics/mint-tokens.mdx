---
title: Mint token
description: Tìm hiểu cách mint các đơn vị mới của token.
---

## Cách mint token

Mint tạo ra các đơn vị mới của token bằng cách sử dụng lệnh
...
- Chỉ mint authority mới có thể mint token mới
- Token account đích phải tồn tại để nhận các token được mint
...
<Callout type="info">
  ...
  có cách triển khai tương tự để đạt được cùng chức năng.
</Callout>
...
async fn main() -> Result<()> {
    // Tạo kết nối đến validator cục bộ
    ...
    // Tạo một keypair mới cho người trả phí
    let fee_payer = Keypair::new();

    // Airdrop 1 SOL cho người trả phí
    ...
    // Tạo keypair để sử dụng làm địa chỉ của mint
    let mint = Keypair::new();

    // Lấy kích thước mặc định của mint account (tính bằng byte), không bật extension
    let mint_space = Mint::LEN;
    let mint_rent = client
        .get_minimum_balance_for_rent_exemption(mint_space)
        .await?;

    // Lệnh để tạo account mới cho mint (token program)
    let create_account_instruction = create_account(
        &fee_payer.pubkey(), // người trả tiền
        &mint.pubkey(),      // account mới (mint)
        mint_rent,           // lamports
        mint_space as u64,   // không gian
        &token_program_id(), // program id
    );

    // Lệnh để khởi tạo dữ liệu mint account
    let initialize_mint_instruction = initialize_mint(
        &token_program_id(),
        &mint.pubkey(),            // mint
        &fee_payer.pubkey(),       // quyền mint
        Some(&fee_payer.pubkey()), // quyền đóng băng
        2,                         // số chữ số thập phân
    )?;

    // Tính toán địa chỉ associated token account cho fee_payer
    let associated_token_address = get_associated_token_address_with_program_id(
        &fee_payer.pubkey(), // chủ sở hữu
        &mint.pubkey(),      // mint
        &token_program_id(), // program_id
    );

    // Lệnh để tạo associated token account
    let create_ata_instruction = create_associated_token_account(
        &fee_payer.pubkey(), // địa chỉ tài trợ
        &fee_payer.pubkey(), // địa chỉ ví
        &mint.pubkey(),      // địa chỉ mint
        &token_program_id(), // program id
    );

    // Tạo giao dịch và thêm các lệnh
