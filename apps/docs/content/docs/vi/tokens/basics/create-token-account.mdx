...
description:
  Tạo token account và associated token account cho một mint cụ thể và chủ sở
  hữu.
...
có thể chứa đơn vị của một token duy nhất, token được xác định bởi trường
...
suy ra địa chỉ ATA từ địa chỉ ví, địa chỉ token program và địa chỉ mint.

Chỉ những token account được tạo bởi Associated Token Program mới được gọi là
associated token account.

Associated Token Program là một cách để tạo token account tại một địa chỉ tiêu
chuẩn, xác định được. Tài khoản kết quả vẫn là một token account thuộc sở hữu
của Token Program hoặc Token Extensions Program, chứ không phải của Associated
Token Program.

Associated Token Program suy ra địa chỉ ATA như được trình bày trong
...
Đối với bất kỳ tổ hợp ví, token program và mint nào, chỉ có chính xác một địa
chỉ ATA. Associated Token Program tạo một token account tiêu chuẩn tại địa chỉ
đó, và tài khoản kết quả vẫn sử dụng kiểu _rs`Account`_ được định nghĩa bởi
Token Program hoặc Token Extensions Program.

## Cách Tạo Associated Token Account

Việc tạo associated token account sử dụng lệnh _rs`Create`_ hoặc
_rs`CreateIdempotent`_ của Associated Token Program. Associated Token Program
suy ra địa chỉ ATA, tạo tài khoản tại địa chỉ ATA đó, và khởi tạo tài khoản như
một token account được sở hữu bởi Token Program hoặc Token Extension Program.
...
| _rs`process_create_associated_token_account`_ | Suy ra địa chỉ ATA và sử dụng _rs`create_pda_account`_ cùng với các CPI đến token program đã chọn để khởi tạo token account. Khi chương trình được chọn là Token Extension Program, bộ xử lý ATA cũng khởi tạo tiện ích mở rộng immutable owner. | [Nguồn](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | Hàm trợ giúp được sử dụng bởi _rs`process_create_associated_token_account`_ để tạo tài khoản PDA thông qua CPI vào System Program.                                                                                                               | [Nguồn](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |

### Typescript

Các ví dụ `Kit` bên dưới minh họa cách tiếp cận được khuyến nghị sử dụng
...
Việc tạo token account yêu cầu hai lệnh (instruction):

1. Lệnh _rs`CreateAccount`_ của System Program tạo một tài khoản miễn phí rent
   mới và chỉ định Token Program làm chủ sở hữu chương trình của tài khoản mới.
...
Bao gồm lệnh _rs`CreateAccount`_ và lệnh khởi tạo token account trong cùng một
giao dịch.

Trong quá trình khởi tạo token account, Token Program kiểm tra xem tài khoản
chưa được khởi tạo và đã miễn phí rent.

<Callout type="info">
  Phần dưới đây cho thấy cách tạo token account bằng cách gọi trực tiếp các lệnh
  của System Program và Token Program. Đối với hầu hết ứng dụng, hãy sử dụng
  Associated Token Program thay thế. Chỉ sử dụng các lệnh gọi trực tiếp System
  Program và Token Program khi bạn có lý do cụ thể để không sử dụng associated
  token account hoặc khi bạn cần tạo token account PDA tùy chỉnh bằng cách thực
  hiện CPI đến các lệnh System Program và Token Program từ chương trình Solana
  của riêng bạn.
</Callout>
...
| _rs`process_initialize_account`_  | Hàm xử lý công khai cho _rs`InitializeAccount`_.                                                        | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L143-L150)     | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L236-L240)     |
| _rs`process_initialize_account2`_ | Hàm xử lý công khai cho _rs`InitializeAccount2`_.                                                       | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L152-L160)     | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L242-L246)     |
| _rs`process_initialize_account3`_ | Hàm xử lý công khai cho _rs`InitializeAccount3`_.                                                       | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L162-L170)     | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L248-L252)     |
