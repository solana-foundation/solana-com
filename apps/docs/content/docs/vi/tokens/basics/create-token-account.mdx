...
Token account lưu trữ số dư của một token cụ thể. Mỗi token account được liên
kết với chính xác một mint và theo dõi số dư token cùng các thông tin bổ sung.
...
<Callout type="info">
  ...
  đều chia sẻ cùng một implementation cơ bản cho token account.
</Callout>

Để nắm giữ token, bạn cần một token account cho mint cụ thể đó. Mỗi token
account theo dõi:

1. **Mint**: Loại token cụ thể mà nó nắm giữ
2. **Owner**: Quyền hạn có thể chuyển token từ account này
...
- Account này chỉ nắm giữ USDC token
- Circle kiểm soát token thông qua quyền owner của token account:
...
1. **Token Account Owner**: Quyền hạn có thể chuyển, đốt hoặc ủy quyền token.
   Thông tin này được lưu trữ trong dữ liệu của token account.

2. **Program Owner**: Program sở hữu account (luôn là Token Program hoặc Token
   Extension Program đối với token account).

Khi làm việc với token account, "owner" thường đề cập đến quyền hạn có thể
chuyển token, chứ không phải program sở hữu account.
...
cho một mint nhất định. Xem
[triển khai derivation tại đây](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/interface/src/address.rs#L56).
...
Đối với bất kỳ tổ hợp ví và token nào, chỉ có duy nhất một địa chỉ ATA. Điều này
loại bỏ nhu cầu theo dõi địa chỉ token account thủ công—bạn luôn có thể derive
địa chỉ chính xác.

<Callout type="info">
  **Cách hoạt động của Associated Token Program**

Associated Token Program là một helper program:

- Tạo token account tại các địa chỉ xác định (PDA)
- Thực hiện Cross-Program Invocation (CPI) đến Token Program
- Không duy trì bất kỳ state nào
...
Việc tạo token account yêu cầu instruction
...
Bạn cần hai instruction để tạo token account:

1. System Program: Tạo một account với không gian được phân bổ cho token account
...
Việc tạo associated token account (ATA) yêu cầu instruction
...
Instruction duy nhất này tự động xử lý việc tạo token account và khởi tạo nó
thông qua Cross Program Invocations (CPI).
