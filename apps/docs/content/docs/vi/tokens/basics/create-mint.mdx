...
description:
  Tạo một mint token và cấu hình số chữ số thập phân, quyền mint và quyền đóng
  băng của mint.
...
Một mint account định nghĩa và nhận dạng duy nhất một token trên Solana, đồng
thời lưu trữ trạng thái dùng chung áp dụng cho tất cả các token account của mint
đó.
...
đó trên các ví, ứng dụng và trình khám phá.

Ví dụ, USD Coin (USDC) có địa chỉ mint là
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`. Địa chỉ mint này nhận dạng duy
nhất USDC trong toàn bộ hệ sinh thái Solana. Bạn có thể xem mint này trên
[Solana Explorer](https://explorer.solana.com/address/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v).

## Cách tạo một Mint Account

Việc tạo một mint yêu cầu hai lệnh:

1. Lệnh _rs`CreateAccount`_ của System Program tạo một tài khoản mới được miễn
   phí rent và chỉ định Token Program làm chủ sở hữu chương trình của tài khoản
   mới.
2. Lệnh _rs`InitializeMint`_ hoặc _rs`InitializeMint2`_ của Token Program khởi
   tạo tài khoản mới như một mint.

Bao gồm lệnh _rs`CreateAccount`_ và lệnh khởi tạo mint trong cùng một giao dịch.

Trong quá trình khởi tạo mint, Token Program kiểm tra rằng mint account chưa
được khởi tạo và đã được miễn phí rent. Sau đó, Token Program ghi quyền mint,
quyền đóng băng, số chữ số thập phân và cờ _rs`is_initialized`_ vào dữ liệu của
mint account.
...
| _rs`_process_initialize_mint`_ | Logic xử lý dùng chung cho quá trình khởi tạo mint.                                      | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
| _rs`process_initialize_mint`_  | Trình xử lý công khai cho _rs`InitializeMint`_.                                          | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L62-L70)       | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L132-L140)     |
| _rs`process_initialize_mint2`_ | Trình xử lý công khai cho _rs`InitializeMint2`_.                                         | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L72-L81)       | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L142-L151)     |

### Typescript

Các ví dụ bên dưới cho thấy cách tiếp cận được khuyến nghị sử dụng `Kit`. Các ví
dụ cũ sử dụng `@solana/kit` được đưa vào để tham khảo.
...
Một mint account chỉ lưu trữ thông tin token cơ bản (nguồn cung, số thập phân,
quyền hạn). Để thêm metadata dễ đọc như tên, ký hiệu và hình ảnh cho token của
bạn, bạn có hai tùy chọn:
...
<Cards>
  ...
  <Card title="Token Extensions Metadata" href="/docs/tokens/extensions/metadata">
   ...
   Tiện ích mở rộng metadata chỉ khả dụng cho các mint account được tạo với Token-2022.
   Để thêm metadata vào các mint account được tạo với Token Program gốc, hãy sử dụng Metaplex Token Metadata Program.
  </Card>
</Cards>
