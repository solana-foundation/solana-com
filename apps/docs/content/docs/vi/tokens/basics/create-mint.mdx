...
Token Program định nghĩa kiểu _rs`Mint`_ account như sau:
...
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`. Địa chỉ mint này xác định duy
...
1. Lệnh _rs`CreateAccount`_ của System Program tạo một account mới miễn phí rent
   và gán Token Program làm chủ sở hữu chương trình của account mới.
2. Lệnh _rs`InitializeMint`_ hoặc _rs`InitializeMint2`_ của Token Program khởi
   tạo account mới như một mint.

Bao gồm lệnh _rs`CreateAccount`_ và lệnh khởi tạo mint trong cùng một giao dịch.

Trong quá trình khởi tạo mint, Token Program kiểm tra xem mint account đã được
khởi tạo chưa và có miễn phí rent hay không. Sau đó Token Program ghi quyền
mint, quyền đóng băng, số chữ số thập phân, và cờ _rs`is_initialized`_ vào dữ
liệu của mint account.
...
| _rs`_process_initialize_mint`_ | Logic bộ xử lý chung cho việc khởi tạo mint.                                             | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
| _rs`process_initialize_mint`_  | Hàm xử lý công khai cho _rs`InitializeMint`_.                                            | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L62-L70)       | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L132-L140)     |
| _rs`process_initialize_mint2`_ | Hàm xử lý công khai cho _rs`InitializeMint2`_.                                           | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L72-L81)       | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L142-L151)     |

### Typescript

Các ví dụ dưới đây sử dụng `Kit` cho thấy cách tiếp cận được khuyến nghị sử dụng
`@solana/kit`. Các ví dụ cũ sử dụng `@solana/web3.js` được đính kèm để tham
khảo.
...
<Cards>
  <Card title="Metaplex Token Metadata" href="/docs/tokens/metaplex">
   Thêm metadata bằng Metaplex Token Metadata Program. Hoạt động với cả Token Program gốc và Token-2022.
  </Card>
