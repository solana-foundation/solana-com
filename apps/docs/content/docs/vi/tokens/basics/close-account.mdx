...
description:
  Đóng một tài khoản token và trả lại lamport của tài khoản cho một tài khoản đích.
...
Đóng một token account sẽ xóa tài khoản đó và chuyển toàn bộ lamport của nó,
bao gồm cả số dư lưu trữ tối thiểu có thể hoàn lại, đến một tài khoản đích.

Số dư token phải bằng không trước khi có thể đóng tài khoản. Một tài khoản bị đóng băng
vẫn có thể được đóng nếu số dư token của tài khoản đó bằng không. Các token account của Wrapped SOL
là ngoại lệ và có thể được đóng khi số dư token còn lại để
lấy lại SOL cơ bản. Chủ sở hữu tài khoản hoặc close authority của tài khoản
ký lệnh đóng và chọn đích đến. Tài khoản đã thanh toán
để tạo một associated token account không tự động nhận lại số dư đã thu hồi.

## Cách Đóng Một Tài Khoản Token

Đóng một tài khoản token sử dụng lệnh _rs`CloseAccount`_ của Token Program.

Lệnh _rs`CloseAccount`_ chuyển lamport của tài khoản nguồn sang một tài khoản
