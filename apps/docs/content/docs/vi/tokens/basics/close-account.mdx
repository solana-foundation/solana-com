...
description:
  Đóng tài khoản token và trả lamport của tài khoản về tài khoản đích.
...
Đóng tài khoản token sẽ xóa tài khoản và trả lamport rent của tài khoản token về
tài khoản đích.
...
không. Tài khoản token Wrapped SOL là trường hợp ngoại lệ và có thể được đóng
khi còn số dư token để thu hồi SOL cơ bản. Chủ sở hữu tài khoản hoặc quyền đóng
tài khoản sẽ ký lệnh đóng.

## Cách Đóng Tài Khoản Token

Đóng tài khoản token sử dụng lệnh _rs`CloseAccount`_ của Token Program.

Lệnh _rs`CloseAccount`_ chuyển lamport của tài khoản nguồn sang tài khoản đích,
xóa tài khoản nguồn và xóa tài khoản nguồn. Token Extension Program áp dụng thêm
các kiểm tra đóng cho một số extension nhất định, nhưng quy trình đóng cơ bản
vẫn giống nhau.
...
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | Trạng thái tài khoản token lưu trữ số dư, trạng thái native và quyền đóng được sử dụng trong quá trình đóng tài khoản. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | Lệnh đóng tài khoản token và chuyển lamport của tài khoản token sang tài khoản đích.                                   | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | Logic xử lý chung cho việc đóng tài khoản token.                                                                       | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |
