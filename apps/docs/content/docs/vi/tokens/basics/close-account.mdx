...
description:
  Đóng tài khoản token và hoàn trả lamport của tài khoản về tài khoản đích.
...
## Đóng Tài Khoản Token Làm Gì?

Đóng tài khoản token sẽ xóa tài khoản và hoàn trả các lamport tiền thuê của tài
khoản token về tài khoản đích.

Số dư token phải bằng không trước khi tài khoản có thể được đóng. Tài khoản bị
đóng băng vẫn có thể được đóng nếu số dư token của tài khoản bị đóng băng bằng
không. Các tài khoản token Wrapped SOL là ngoại lệ và có thể được đóng khi vẫn
còn số dư token để thu hồi SOL cơ bản. Chủ sở hữu tài khoản hoặc quyền đóng tài
khoản sẽ ký lệnh đóng.

## Cách Đóng Tài Khoản Token

Đóng tài khoản token sử dụng lệnh _rs`CloseAccount`_ của Token Program.

Lệnh _rs`CloseAccount`_ chuyển các lamport từ tài khoản nguồn đến tài khoản
đích, xóa tài khoản nguồn và xóa bỏ tài khoản nguồn. Token Extension Program áp
dụng các kiểm tra đóng bổ sung cho một số tiện ích mở rộng nhất định, nhưng quy
trình đóng cơ bản vẫn giống nhau.

### Tham Chiếu Nguồn

| Mục                         | Mô Tả                                                                                                                  | Token Program                                                                                                 | Token Extension Program                                                                                             |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | Trạng thái tài khoản token lưu trữ số dư, trạng thái native và quyền đóng được sử dụng trong quá trình đóng tài khoản. | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | Một lệnh đóng tài khoản token và chuyển các lamport của tài khoản token đến tài khoản đích.                            | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | Logic xử lý chung để đóng tài khoản token.                                                                             | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |
