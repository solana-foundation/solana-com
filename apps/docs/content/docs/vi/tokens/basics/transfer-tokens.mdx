...
title: Chuyển Token
description:
  Chuyển token giữa các token account cho cùng một mint bằng các lệnh transfer
  của Token Program.
...
Chuyển token không thay đổi nguồn cung của mint. Chuyển token chỉ cập nhật số dư
giữa các token account.
...
mint và số thập phân để lệnh có thể xác minh mint và độ chính xác token mong đợi
trước khi di chuyển token giữa các tài khoản.

Trong Token Extensions Program, _rs`Transfer`_ đã lỗi thời và được thay thế bởi
_rs`TransferChecked`_ hoặc _rs`TransferCheckedWithFee`_.

Chủ sở hữu tài khoản nguồn hoặc một delegate được phê duyệt sẽ ký giao dịch
chuyển. Trong Token Extensions Program, một mint permanent delegate cũng có thể
ủy quyền cho việc chuyển nếu mint có extension permanent delegate được kích
hoạt.
...
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_          | Trạng thái token account lưu trữ số dư được cập nhật bởi giao dịch chuyển.                                                                                                                     | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | Một lệnh chuyển di chuyển token giữa các token account mà không yêu cầu người gọi cung cấp mint hoặc số thập phân. Trong Token Extensions Program, hãy sử dụng _rs`TransferChecked`_ thay thế. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Một lệnh chuyển yêu cầu người gọi cung cấp mint và số thập phân, và kiểm tra các giá trị đó trước khi di chuyển token giữa các token account của cùng một mint.                                | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | Logic xử lý chung cho việc chuyển token.                                                                                                                                                       | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |
