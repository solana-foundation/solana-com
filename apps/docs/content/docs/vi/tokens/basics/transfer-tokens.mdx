...
title: Chuyển Token
description:
  Chuyển token giữa các token account cho cùng một mint với các lệnh chuyển
  khoản của Token Program.
...
Chuyển token di chuyển token từ token account này sang token account khác cho
cùng một mint.

Việc chuyển không thay đổi lượng cung của mint. Chuyển chỉ cập nhật số dư giữa
các token account.

## Cách Chuyển Token

Chuyển token sử dụng lệnh _rs`Transfer`_ hoặc _rs`TransferChecked`_ của Token
Program.

Các ví dụ dưới đây sử dụng _rs`TransferChecked`_, yêu cầu người gọi cung cấp
mint và số thập phân để lệnh có thể xác minh mint và độ chính xác của token như
mong đợi trước khi di chuyển token giữa các tài khoản.

Trong Token Extensions Program, _rs`Transfer`_ không còn được khuyến nghị sử
dụng, thay vào đó hãy dùng _rs`TransferChecked`_ hoặc
_rs`TransferCheckedWithFee`_.

Chủ sở hữu tài khoản nguồn hoặc một đại biểu được ủy quyền ký xác nhận giao dịch
chuyển. Trong Token Extensions Program, một đại biểu vĩnh viễn của mint cũng có
thể ủy quyền cho việc chuyển nếu mint đã kích hoạt extension đại biểu vĩnh viễn.

### Tài Liệu Tham Khảo Nguồn

| Mục                    | Mô tả                                                                                                                                                                                      | Token Program                                                                                                 | Token Extensions Program                                                                                            |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_          | Trạng thái token account lưu trữ số dư được cập nhật bởi giao dịch chuyển.                                                                                                                 | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | Lệnh chuyển di chuyển token giữa các token account mà không yêu cầu người gọi cung cấp mint hoặc số thập phân. Trong Token Extensions Program, hãy sử dụng _rs`TransferChecked`_ thay thế. | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Lệnh chuyển yêu cầu người gọi cung cấp mint và số thập phân, và kiểm tra các giá trị đó trước khi di chuyển token giữa các token account của cùng một mint.                                | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | Logic xử lý chung cho việc chuyển token.                                                                                                                                                   | [Nguồn](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [Nguồn](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |
