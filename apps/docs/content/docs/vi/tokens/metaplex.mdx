...
Chương trình Token Metadata tạo ra một Program Derived Address (PDA) cho mỗi
token mint. Tài khoản metadata này lưu trữ thông tin onchain như tên và ký hiệu
của token, cùng với một URI trỏ đến metadata JSON off-chain (hình ảnh, mô tả,
v.v.).
...
Tài khoản metadata onchain chứa:
