...
_rs`Update`_. Tài khoản này đóng vai trò là "kho" được kiểm soát bởi chương
trình, nhận SOL từ người dùng khi họ cập nhật tin nhắn của mình.
...
- Địa chỉ của tài khoản được lấy từ một PDA được tạo ra bằng cách sử dụng seeds
...
khoản vault của người dùng trong lệnh xóa để chuyển bất kỳ SOL tích lũy nào trở
lại cho người dùng.
...
- `cpi_context` (chương trình, các tài khoản và PDA signer)
...
Tiếp theo, cập nhật file `anchor.test.ts` để bao gồm tài khoản vault mới trong
các lệnh. Điều này yêu cầu tạo vault PDA và đưa nó vào các lệnh update và
delete.
