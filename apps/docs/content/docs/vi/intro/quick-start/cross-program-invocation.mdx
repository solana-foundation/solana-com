...
Đầu tiên, chương trình cần một cơ chế "trả phí để cập nhật" đơn giản bằng cách
thay đổi cấu trúc _rs`Update`_ và hàm `update`.

Bắt đầu bằng cách cập nhật tệp `lib.rs` để đưa vào phạm vi các mục từ module
`system_program`.
...
Tiếp theo, cập nhật cấu trúc _rs`Update`_ để bao gồm một tài khoản mới có tên
`vault_account`. Tài khoản này, được kiểm soát bởi chương trình, nhận SOL từ
người dùng khi họ cập nhật tài khoản tin nhắn của họ.
...
_rs`Update`_. Tài khoản này đóng vai trò là "kho bạc" được kiểm soát bởi chương
trình, nhận SOL từ người dùng khi họ cập nhật tin nhắn của họ.
...
- Địa chỉ của tài khoản đến từ một PDA được tạo ra bằng cách sử dụng seeds
  _rs`[b"vault", user.key().as_ref()]`_
- Là một PDA, nó không có khóa riêng, vì vậy chỉ chương trình mới có thể "ký"
  cho địa chỉ khi thực hiện CPI
- Là kiểu _rs`SystemAccount`_, System Program sở hữu nó giống như các tài khoản
  ví thông thường
...
của người dùng đến tài khoản vault.
...
Cấu trúc _rs`CpiContext`_ chỉ định chương trình và các tài khoản cho một Cross
...
Trong lệnh `update`, việc triển khai bao gồm một Cross Program Invocation (CPI)
để gọi lệnh `transfer` của System Program. Điều này minh họa cách thực hiện CPI
từ bên trong chương trình, cho phép khả năng kết hợp của các chương trình
Solana.
...
- `to` - Tài khoản kho (đích đến của tiền)
...
- `cpi_context` (chương trình và các tài khoản)
- `amount` cần chuyển (1.000.000 lamports, hoặc 0,001 SOL)
...
Thiết lập cho CPI tương tự như cách các lệnh phía client được xây dựng, trong đó
bạn chỉ định chương trình, tài khoản và instruction data cho một lệnh cụ thể cần
gọi. Khi lệnh `update` của chương trình nhận được một lời gọi, nó sẽ gọi nội bộ
lệnh chuyển khoản của System Program.
...
Bây giờ thêm cơ chế "hoàn tiền khi xóa" bằng cách thay đổi cấu trúc _rs`Delete`_
và hàm `delete`.

Đầu tiên, cập nhật cấu trúc _rs`Delete`_ để bao gồm `vault_account`. Điều này
cho phép chuyển bất kỳ SOL nào trong kho trở lại cho người dùng khi họ đóng tài
khoản tin nhắn của mình.
...
Cũng thêm `system_program` vì CPI cho việc chuyển khoản yêu cầu gọi System
Program.
...
`vault_account` sử dụng cùng cách tạo PDA như trong cấu trúc Update.

Việc thêm `vault_account` vào cấu trúc Delete cho phép chương trình truy cập tài
khoản kho của người dùng trong quá trình thực hiện lệnh xóa để chuyển bất kỳ SOL
tích lũy nào trở lại cho người dùng.
...
Tiếp theo, thêm logic CPI trong lệnh `delete` để chuyển SOL từ tài khoản kho trở
lại tài khoản của người dùng.
...
- `cpi_context` (chương trình, tài khoản và PDA signer)
- Số lượng cần chuyển (toàn bộ số dư của tài khoản kho)
...
Tiếp theo, cập nhật file `anchor.test.ts` để bao gồm tài khoản kho mới trong các
lệnh. Điều này yêu cầu tạo vault PDA và đưa nó vào các lệnh gọi update và
delete.
...
Sau đó, cập nhật hướng dẫn cập nhật để bao gồm [`vaultAccount`](mention:one)
...
Sau đó, cập nhật hướng dẫn xóa để bao gồm [`vaultAccount`](mention:one)
...
Nhấp vào nút `Import` và nhập tên dự án để thêm nó vào danh sách dự án của bạn
trong Solana Playground. Sau khi dự án được nhập, mọi thay đổi sẽ được tự động
lưu và duy trì.
