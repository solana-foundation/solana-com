...
Trong tệp `lib.rs`, bạn sẽ tìm thấy một chương trình với các lệnh
[`create`](mention:one), [`update`](mention:two), và [`delete`](mention:three)
để thêm vào trong các bước sau.
...
Trong `lib.rs`, cập nhật cấu trúc _rs`MessageAccount`_ với nội dung sau:
...
Thuộc tính _rs`#[account]`_ trong chương trình Anchor chú thích các cấu trúc đại
diện cho dữ liệu tài khoản (kiểu dữ liệu để lưu trữ trong trường data của
Account).
...
["bump" seed](/docs/core/pda#canonical-bump) được sử dụng để tạo program derived
address (PDA). Việc lưu trữ giá trị này giúp tiết kiệm tài nguyên tính toán bằng
cách loại bỏ nhu cầu tính toán lại trong các lệnh sau.
...
Trong ví dụ này, cấu trúc _rs`MessageAccount`_ lưu trữ một tin nhắn được tạo bởi
...
- `bump` - _rs`u8`_ lưu trữ ["bump" seed](/docs/core/pda#canonical-bump) để tạo
  program derived address (PDA). Việc lưu trữ giá trị này giúp tiết kiệm tài
...
Thuộc tính _rs`#[derive(Accounts)]`_ trong chương trình Anchor được sử dụng để
chú thích các struct định nghĩa các tài khoản cần thiết cho một lệnh.

Mỗi trường trong cấu trúc đại diện cho một tài khoản được xác thực theo hai
cách:

1. Loại tài khoản (như _rs`Signer<'info>`_ hoặc _rs`Account<'info, T>`_) chỉ
...
1. _rs`user: Signer<'info>`_
   ...
   - Cần trạng thái có thể thay đổi (_rs`#[account(mut)]`_) vì phải trả tiền cho
     tài khoản mới
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - Các ràng buộc `seeds` và `bump` suy ra địa chỉ tài khoản dưới dạng Program
     Derived Address (PDA)
   - `payer = user` xác định ai trả tiền cho việc tạo tài khoản mới
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - Ở hậu trường, ràng buộc `init` gọi System Program để tạo một tài khoản mới
...
Tiếp theo, thêm logic nghiệp vụ cho chỉ thị _rs`create`_ bằng cách cập nhật hàm
...
2. Khởi tạo dữ liệu tài khoản:
   ...
   - Đặt trường `message` thành `message` từ đối số hàm.
...
Tiếp theo, thêm lệnh `update` để thay đổi `MessageAccount` bằng tin nhắn mới.
...
1. _rs`user: Signer<'info>`_
   ...
   - Được đánh dấu là có thể thay đổi (_rs`#[account(mut)]`_) vì có thể phải
     thanh toán cho thêm không gian cho `message_account` khi cần
   - Phải ký giao dịch
...
Code khởi đầu cũng bao gồm một tệp kiểm thử trong `anchor.test.ts`.
...
Thêm đoạn code bên dưới vào bên trong _ts`describe()`_, nhưng trước các phần
_ts`it()`_.
