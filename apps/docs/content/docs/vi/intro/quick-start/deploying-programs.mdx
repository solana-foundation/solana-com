...
Truy cập trường `data` của tài khoản `new_account` và đặt nó bằng giá trị của
`data: u64`
...
Thân hàm [đặt trường `data` của `new_account`](mention:three) bằng đối số `data`
được cung cấp, sau đó in một thông báo vào program logs.
...
1. Mở tệp kiểm tra có tên `tests/anchor.test.ts`. Tệp này minh họa cách gọi
...
3. Lưu trữ mã băm giao dịch (`TxHash`) có trong kết quả đầu ra
...
<Callout type="warn">
  **Solana Explorer:** Đặt cụm mạng thành `devnet`, để khớp với Solana
  Playground.
</Callout>
...
Đóng chương trình để thu hồi SOL được phân bổ cho chương trình on-chain. Để làm
