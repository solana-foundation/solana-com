...
Địa chỉ onchain của chương trình của bạn. Nó sẽ được cập nhật khi bạn build dự
án
...
- Macro [`declare_id!`](mention:one) chỉ định địa chỉ onchain của chương trình
...
<Callout>
  [ID chương trình](/docs/intro/quick-start/build-first-program) của bạn, được
  định nghĩa trong _rs`declare_id!()`_, giờ đây sẽ được cập nhật.
</Callout>

2. Thêm SOL vào ví của bạn _(Tùy chọn)_

Triển khai một chương trình yêu cầu SOL trong program account onchain. Nếu cần
thiết, [thêm SOL vào ví của bạn](/docs/intro/quick-start#add-sol-to-your-wallet)
trước khi tiến hành bước 3.
...
Đóng chương trình để thu hồi SOL đã được phân bổ cho chương trình onchain. Để
thực hiện, chạy `solana program close [ProgramID]`, sử dụng địa chỉ chương trình
được lưu trữ trong macro _rs`declare_id!()`_.
...
khám phá [địa chỉ dẫn xuất từ chương trình](/docs/core/pda) và
[gọi chéo giữa các chương trình](/docs/core/cpi).
