...
Hướng dẫn này bao gồm một bản triển khai hoàn chỉnh có thể chạy cục bộ. [Mã nguồn](https://github.com/Woody4618/access-control-list-acl-demo) cung cấp các bản triển khai tham khảo cho mục đích khám phá và học tập.

Mã của các chương trình ACL có sẵn trong [kho token-acl](https://github.com/solana-foundation/token-acl) và Chương trình ABL Gate có sẵn trong [kho token-acl-gate](https://github.com/solana-foundation/token-acl-gate).

**Quan trọng**: Chương trình ABL (Allow Block List) Gate được sử dụng trong hướng dẫn này là một **bản triển khai tham khảo**. Mặc dù đã được kiểm toán và sẵn sàng cho môi trường sản xuất, các tổ chức phát hành có thể tự do tạo các Chương trình Gate tùy chỉnh phù hợp hơn với nhu cầu tuân thủ cụ thể của họ. Bạn chỉ bị ràng buộc bởi đặc tả Token ACL (sRFC37), không phải thiết kế Chương trình Gate cụ thể này.

**KHÔNG sử dụng mã này trực tiếp trong môi trường sản xuất** nếu chưa có:
