...
Các giao dịch Solana có giới hạn kích thước (~1232 byte). Mỗi lệnh chuyển
thêm khoảng ~200 byte. Đối với các thanh toán đa bên rất lớn, hãy cân nhắc sử dụng
[Address Lookup Tables](/developers/cookbook/transactions/lookup-tables) để
nén các địa chỉ tài khoản và đưa nhiều giao dịch chuyển hơn vào mỗi giao dịch.
