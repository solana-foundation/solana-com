...
Việc rút tiền từ Private Channels là một quy trình hai bước. Đầu tiên, người dùng đốt
...
Sử dụng biến thể `Async`, tự động suy ra `tokenAccount` PDA và là
dạng được khuyến nghị cho môi trường production:
...
xử lý việc thanh toán tự động:

1. `indexer-private-channel` thăm dò kênh mỗi 1 giây để tìm sự kiện đốt token
   và ghi một bản ghi rút tiền đang chờ xử lý khi phát hiện bản ghi của bạn
2. `operator-private-channel` thăm dò cơ sở dữ liệu mỗi 1 giây để tìm các
   bản ghi đang chờ xử lý và gửi `ReleaseFunds` đến Escrow Program trên Solana devnet
   với bằng chứng loại trừ SMT cần thiết; nó thăm dò xác nhận devnet tối đa
   5 lần với khoảng cách 400 ms trước khi thử lại

Tiền thường xuất hiện trong ví devnet của bạn trong vài giây trong điều kiện bình thường.

## Cách Operator Thực Hiện Thanh Toán (Tham khảo)

Sau khi đốt token phía kênh, một operator đã được cấp phép phải gọi `ReleaseFunds` trên
...
được xử lý tự động bởi `operator-private-channel`; bạn không cần gọi
nó theo cách thủ công.

Operator cung cấp:

- `amount`: phải khớp với số lượng đã đốt
- `user`: ví nhận trên devnet
- `new_withdrawal_root`: gốc SMT được cập nhật sau lần rút tiền này
- `transaction_nonce`: nonce duy nhất cho leaf này
- `sibling_proofs`: 512 byte (16 x 32-byte sibling hash cho bằng chứng cây)

## Xác minh

Sau khi lệnh gọi của operator được xác nhận, hãy kiểm tra số dư token devnet của bạn:
...
escrow, gửi một giao dịch off-chain qua gateway, và rút tiền
...
<Cards>
  ...
  >
    Tìm hiểu sâu về các thành phần tham gia và toàn bộ luồng từ nạp tiền đến rút tiền.
  </Card>
  <Card
  ...
  <Card
    title="Tài liệu Tham khảo Release Funds"
  ...
  >
    Tìm hiểu cách bằng chứng rút tiền ngăn chặn double-spend.
  </Card>
</Cards>
