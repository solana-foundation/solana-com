...
> _Lưu ý: Khi `fallback_if_unavailable` là true, hệ thống cho phép các giao dịch
> tiếp tục nếu Redis tạm thời không khả dụng, ngăn chặn sự gián đoạn dịch vụ.
> Đặt `max_transactions` thành 0 sẽ cho phép giao dịch không giới hạn._
...
| `get_payer_signer`          | Trả về người thanh toán phí tiếp theo và địa chỉ thanh toán        | ✅       | boolean |
...
[Tài liệu Solana](https://solana.com/developers/cookbook/transactions/durable-nonces).
...
| ---------------------------- | ------------------------------------------------ |
| `confidential_transfer_mint` | Cấu hình Số dư Bảo mật cho mint |
...
| Tên tiện ích mở rộng                  | Mô tả                                 |
| ------------------------------- | ------------------------------------------- |
| `confidential_transfer_account` | Trạng thái Số dư Bảo mật cho tài khoản |
