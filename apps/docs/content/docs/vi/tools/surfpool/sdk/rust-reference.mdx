---
title: Tài liệu tham khảo API Rust
description:
  API công khai đầy đủ cho crate surfpool-sdk — Surfnet, builder, cheatcodes,
  builders, errors và re-exports.
---

Crate `surfpool-sdk` cung cấp một bề mặt nhỏ tập trung vào struct `Surfnet`.
Trang này liệt kê mọi mục công khai; để xem hướng dẫn sử dụng chi tiết, hãy xem
các hướng dẫn bắt đầu với [Tổng quan](/docs/tools/surfpool/sdk/overview).

## Xuất khẩu từ Crate Root

| Mục                   | Loại       | Ghi chú                                                              |
| --------------------- | ---------- | -------------------------------------------------------------------- |
| `Surfnet`             | struct     | Phiên bản Surfnet đang chạy — các endpoint RPC/WS, payer, cheatcodes |
| `SurfnetBuilder`      | struct     | Builder dạng Fluent được sử dụng bởi `Surfnet::builder()`            |
| `Cheatcodes<'_>`      | struct     | Helper cheatcode được mượn, trả về bởi `Surfnet::cheatcodes()`       |
| `SurfnetError`        | enum       | Tất cả các biến thể lỗi mà SDK trả về                                |
...
`stop` chờ tối đa năm giây để runtime giải phóng các cổng của nó. Lỗi trong quá
trình `Drop` sẽ bị bỏ qua; lỗi trong quá trình gọi `stop` tường minh sẽ trả về
...
Mỗi setter trả về `Self`, vì vậy các lệnh gọi có thể được nối chuỗi. Lệnh gọi
kết thúc là `start().await`.
...
Triển khai trait này cho các cheatcode JSON-RPC tùy chỉnh chưa được bọc bởi một
builder có kiểu dữ liệu.

## Các Builder

Tất cả các builder đều nằm trong `surfpool_sdk::cheatcodes::builders`.
...
| Biến thể         | Nguyên nhân                                                  |
| ---------------- | ------------------------------------------------------------ |
| `PortAllocation` | Không thể liên kết đến một cổng rảnh cho RPC hoặc WebSocket. |
| `Startup`        | Runtime khởi tạo thất bại.                                   |
| `Runtime`        | Luồng runtime bị panic hoặc thoát bất ngờ.                   |
| `Cheatcode`      | Một lệnh gọi JSON-RPC cheatcode trả về lỗi.                  |
| `Aborted`        | Runtime đã bị tắt hoặc hủy trong quá trình khởi động.        |

Crate triển khai `std::error::Error` và `Display` trên `SurfnetError`, vì vậy nó
hoạt động với `?` và `anyhow` ngay lập tức.
