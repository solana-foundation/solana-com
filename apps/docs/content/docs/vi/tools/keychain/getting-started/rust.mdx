...
Dùng để ký với lưu ký MPC Fordefi. Các yêu cầu API được xác thực bằng chữ ký yêu cầu ECDSA P-256 được phân tích cú pháp từ `private_key_pem`; để giữ khóa đó trong KMS/HSM, hãy triển khai `FordefiRequestSigner` và sử dụng `Signer::from_fordefi_with_signer`.

Fordefi có hai chế độ ký. Đặt `chain` cho **chế độ Solana gốc**, trong đó Fordefi có thể thay thế blockhash hoặc phí và tự phát sóng giao dịch. Để `chain` là `None` cho **chế độ hộp đen**, trong đó Fordefi ký các byte thô và bạn tự phát sóng giao dịch.
