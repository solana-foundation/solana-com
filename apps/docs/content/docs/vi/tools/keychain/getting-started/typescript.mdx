...
### Sử dụng với Kit Client (plugin keychainSigner)

`@solana/keychain-kit-plugin` cài đặt một keychain signer trực tiếp trên
[Kit client](https://github.com/anza-xyz/kit), kết nối nó như `payer` và
`identity` để bạn bỏ qua bước thiết lập thủ công `pipe` như trên:
...
Plugin này chấp nhận cùng cấu hình backend-tagged như `createKeychainSigner`, và
chỉ backend mà nó điều phối tới mới được đóng gói (các gói backend được tải qua
`import()` động).

Dùng `keychainPayer` hoặc `keychainIdentity` để chỉ thiết lập một vai trò, và
kết hợp nhiều backend trên một client duy nhất:
...
Đã có sẵn `SolanaSigner`? Mọi keychain signer đều là một Kit `TransactionSigner`
hợp lệ, vì vậy bạn có thể cài đặt một signer hiện có bằng
[`@solana/kit-plugin-signer`](https://github.com/anza-xyz/kit-plugins/tree/main/packages/kit-plugin-signer)
thay thế (`pnpm add @solana/kit-plugin-signer`):
...
Giao diện `SolanaSigner` mở rộng các kiểu `@solana/signers` để đảm bảo khả năng
tương thích đầy đủ:
...
## Xây Dựng Trình Ký Tùy Chỉnh

Triển khai giao diện `SolanaSigner` để tạo các trình ký tùy chỉnh:
...
Xem [Hướng dẫn Thêm Trình Ký](/docs/tools/keychain/adding-signers) để tích hợp
