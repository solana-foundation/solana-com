---
title: Chương trình & Ngữ cảnh
description:
  Thiết lập môi trường kiểm thử và xây dựng hướng dẫn với AnchorContext và
  Program
---

## Tạo Kiểu Client với declare_program!

Anchor 1.0 giới thiệu `declare_program!`, đọc IDL của chương trình tại thời điểm
biên dịch và tạo ra các module `client::accounts::*` và `client::args::*` an
toàn kiểu dữ liệu. Đây là cách được khuyến nghị để xây dựng hướng dẫn trong kiểm
thử.
...
<Callout type="info">
  Chạy `anchor build` trước để tạo `target/idl/my_program.json`.
  `declare_program!` đọc tệp này tại thời điểm biên dịch. Tên macro phải khớp
  với tên crate của chương trình (snake_case).
</Callout>

## Thiết Lập Môi Trường Kiểm Thử

Trình xây dựng `AnchorLiteSVM` cung cấp API liền mạch để cấu hình môi trường
kiểm thử của bạn.
...
### Cách Viết Tắt cho Một Chương Trình
...
<Callout type="info">
  Nếu bạn không chỉ định người thanh toán, `AnchorLiteSVM` sẽ tự động tạo và nạp
  tiền cho một tài khoản.
</Callout>

## AnchorContext

`AnchorContext` là giao diện chính để tương tác với môi trường kiểm thử của bạn.
...
## Xây Dựng Lệnh Với Program

Struct `Program` cung cấp một API linh hoạt để xây dựng các lệnh, phản ánh
anchor-client. Sử dụng các kiểu được tạo bởi `declare_program!` để đảm bảo an
toàn kiểu dữ liệu.
...
<Callout type="info">
  Phương thức `.accounts()` chấp nhận bất kỳ kiểu nào triển khai
  `ToAccountMetas`, điều mà tất cả các struct tài khoản Anchor đều thực hiện tự
  động. Tương tự, `.args()` chấp nhận bất kỳ kiểu nào triển khai
  `InstructionData`.
</Callout>
...
### Sử Dụng send_and_confirm_transaction
...
Đây là một ví dụ toàn diện sử dụng mẫu escrow với `declare_program!`:
