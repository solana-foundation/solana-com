---
title: Các Thao Tác Token
description:
  Làm việc với các thao tác token - đúc, chuyển tiền và quản lý tài khoản
---

## Tạo Token Mint

Một mint là nền tảng của bất kỳ SPL token nào. Nó xác định các thuộc tính của
token và kiểm soát ai có thể đúc token mới.
...
Một token account lưu trữ số dư của một spl-token cụ thể cho một người dùng cụ
thể. Để hiểu sự khác biệt giữa loại này và một ATA, hãy đọc
...
Associated Token Accounts (ATA) là các địa chỉ xác định trước để lưu trữ token
cho một người dùng cụ thể. Để hiểu sự khác biệt giữa loại này và một ATA, hãy
đọc
[phần này](/docs/tools/litesvm/additional-crates/testing-with-spl-tokens#types-of-token-accounts).
...
## Đúc Token vào Token Account

Sau khi bạn có mint và các token account, bạn có thể đúc token vào các tài khoản
đó.
...
Chuyển token giữa các tài khoản bằng cách sử dụng lệnh Transfer.
...
## Làm Việc với Số Thập Phân

Hầu hết các token sử dụng số thập phân để biểu thị các lượng phân số. Đây là
