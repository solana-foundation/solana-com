...
| ---------------- | ------------ | ----------------------------- |
| `address`        | `Address`    | pubkey của tài khoản            |
| `data`           | `Uint8Array` | Dữ liệu byte của tài khoản            |
| `executable`     | `boolean`    | Tài khoản có thể thực thi hay không |
| `lamports`       | `Lamports`   | Số dư SOL                   |
| `programAddress` | `Address`    | Chương trình sở hữu                 |
...
<Callout type="info">
  Sử dụng `setAccount` nhanh hơn `airdrop` khi thiết lập tài khoản hàng loạt vì
  nó không thực thi giao dịch.
</Callout>
