...
Kiểm thử một chương trình Solana tùy chỉnh có nghĩa là tải tệp `.so` đã biên dịch vào
SVM, thiết lập các tài khoản mà nó yêu cầu, và gửi các lệnh đến nó.
LiteSVM xử lý tất cả những điều này trong tiến trình — không cần validator. Chúng tôi khuyến nghị
tạo các client cho chương trình bằng
...
Để kiểm thử yêu cầu các chương trình chuẩn:
...
<Callout type="info">
  Biên dịch chương trình Solana của bạn bằng `cargo build-sbf` để tạo tệp `.so`.
</Callout>

Quy trình luôn giống nhau:

1. **Tải Chương Trình**: Sử dụng `addProgramFromFile` để tải các chương trình đã biên dịch và
   import myProgramPlugin() từ client của bạn
2. **Cấu Hình**: Kích hoạt các sysvar, builtin và precompile khi cần thiết
3. **Thiết Lập Tài Khoản**: Khởi tạo sẵn các tài khoản dữ liệu bằng `setAccount`
4. **Gửi Lệnh**: Sử dụng `client.myProgram.testInstruction({..}).sendTransaction()` —
...
Khi bạn đã có bộ khung này hoạt động, bạn có thể xây dựng một bộ kiểm thử đầy đủ bằng cách
thêm nhiều lệnh và các xác nhận hơn.
