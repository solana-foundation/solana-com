---
title: CPI với PDA Signers
description:
  Cách sử dụng hàm invoke_signed cho Cross-Program Invocations trên Solana khi
  chương trình gọi cần ký dưới danh nghĩa PDA, với các ví dụ về Anchor
  CpiContext và Native Rust.
...
<Callout type="info" title="Tóm tắt">
  Sử dụng _rs`invoke_signed`_ khi chương trình gọi cần ký thay mặt cho một PDA
...
với [signer seeds](mention:signer-seeds) được dùng để suy ra
[PDA](/docs/core/pda). Để biết chi tiết về cách runtime xác minh chữ ký PDA, hãy
xem [PDA Signing](/docs/core/cpi/cpi-cost-model#pda-signing).
...
Các ví dụ dưới đây thực hiện CPI với PDA signers bằng Anchor và Native Rust. Mỗi
ví dụ bao gồm một instruction duy nhất để chuyển SOL từ một PDA đến một tài
khoản người nhận, sử dụng CPI được ký bởi PDA.

#### Anchor

Các ví dụ sau đây trình bày hai cách tiếp cận để triển khai CPI trong một chương
...
- **Ví dụ 2**: Sử dụng hàm _rs`system_instruction::transfer`_ từ crate
  _rs`solana_program`_. (Ví dụ 1 là một trừu tượng hóa của cách triển khai này.)
- **Ví dụ 3**: Xây dựng instruction CPI thủ công. Cách tiếp cận này hữu ích khi
  không có crate nào có sẵn để giúp xây dựng instruction mà bạn muốn gọi.
...
Native Rust. Nó bao gồm một lệnh duy nhất chuyển SOL từ tài khoản PDA sang tài
khoản khác. CPI được ký bởi tài khoản PDA. (Tệp kiểm thử sử dụng
[LiteSVM](https://github.com/LiteSVM/litesvm) để kiểm thử chương trình.)
