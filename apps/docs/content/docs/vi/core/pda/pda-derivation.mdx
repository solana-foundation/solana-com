...
Bump seed là một byte đơn (u8) được nối thêm vào các seed tùy chọn trong quá
trình suy dẫn.
[`find_program_address`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L800)
tìm kiếm từ 255 xuống 1, gọi _rs`create_program_address`_ với từng giá trị cho
đến khi kết quả rơi ra khỏi đường cong Ed25519. Giá trị đầu tiên thành công
chính là **bump chính tắc**.
...
cho _rs`create_program_address`_ tốn
...
Việc tạo PDA chỉ tính toán một địa chỉ. Nó không tạo tài khoản onchain tại địa
chỉ đó. Tài khoản phải được tạo rõ ràng thông qua một lệnh riêng biệt (thường là
_rs`create_account`_ thông qua CPI).
...
### Lặp Qua Tất Cả Các Bump Có Thể
...
<Callout type="warn">
  Điều này khác biệt với hành vi tìm kiếm chính tắc hiện tại được ghi nhận cho
  `sol_try_find_program_address`, vốn lặp từ 255 xuống 1.
</Callout>
...
Trong ví dụ này, bump 255 tạo ra một địa chỉ nằm trên đường cong và thất bại.
Bump hợp lệ đầu tiên là 254, khiến nó trở thành bump chính tắc.
