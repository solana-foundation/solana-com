...
<Callout>
  Chiều cao tối đa của lời gọi chỉ thị chương trình được gọi là
...
được sử dụng. Nó nhận [signer seeds](mention:signer-seeds) được dùng để tạo ra
[PDA](<(/docs/core/pda)>) người ký. Solana runtime gọi nội bộ
...
- **Ví dụ 3**: Xây dựng chỉ thị CPI thủ công. Cách tiếp cận này hữu ích khi
  không có crate nào có sẵn để giúp xây dựng chỉ thị bạn muốn gọi.
...
- **Ví dụ 2**: Sử dụng hàm _rs`system_instruction::transfer`_ từ crate
  `solana_program`.
- **Ví dụ 3**: Xây dựng chỉ thị CPI thủ công. Cách tiếp cận này hữu ích khi
  không có crate nào tồn tại để giúp xây dựng chỉ thị bạn muốn gọi.
