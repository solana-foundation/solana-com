...
description:
  Cấu trúc chi tiết của giao dịch Solana bao gồm chữ ký, định dạng thông điệp, tiêu đề, địa chỉ tài khoản, blockhash, lệnh đã biên dịch, mã hóa nhị phân và giới hạn kích thước.
...
<Callout type="info" title="Tóm tắt">
  Một giao dịch bao gồm chữ ký và một thông điệp. Thông điệp chứa tiêu đề,
  địa chỉ tài khoản, blockhash gần đây và các lệnh đã được biên dịch. Kích thước
  tuần tự hóa tối đa: 1.232 byte (4.096 byte cho định dạng v1 sắp ra mắt).
</Callout>
...
<Callout type="info" title="Giao dịch v1 nâng giới hạn lên 4.096 byte">
  Trang này mô tả định dạng giao dịch kế thừa (legacy), mà định dạng v0 cũng
  tuân theo ngoại trừ tiền tố phiên bản và bảng tra cứu địa chỉ. Định dạng
  [v1](/docs/core/transactions/versioned-transactions#v1-message-format) sắp ra mắt
  sẽ nâng giới hạn kích thước lên 4.096 byte và sử dụng bố cục wire khác.
</Callout>
...
<Callout type="info" title="Tiền tố thông điệp kế thừa và có phiên bản">
  ...
  thông điệp giao dịch có phiên bản, byte đầu tiên là tiền tố phiên bản thay thế;
  `MessageHeader` ba byte bắt đầu ngay sau tiền tố đó. Xem
  [giao dịch có phiên bản](/docs/core/transactions/versioned-transactions) để biết
  bố cục thông điệp đầy đủ của v0 và v1.
</Callout>
