...
Không có kiểm tra ở cấp độ giao thức nào đảm bảo rằng người nhận có thể chuyển
SOL trở lại. lamport chỉ có thể được chuyển ra bởi chương trình sở hữu tài khoản
đó, vì vậy việc gửi SOL đến một [token mint](/docs/tokens/basics/create-mint),
một chương trình, hoặc một [PDA](/docs/core/pda) mà bạn không kiểm soát **có
nguy cơ mất tiền vĩnh viễn** — chỉ có một authority được chỉ định bởi chương
trình sở hữu mới có thể hoàn trả. SOL gửi đến một
...
Việc chuyển **token** SPL có tính tự bảo vệ một phần: Token Program từ chối một
giao dịch chuyển mà các tài khoản không khớp với mint kỳ vọng. Việc chuyển
...
Ví dụ dưới đây hiển thị mã liên quan đến các sơ đồ ở trên. Xem
...
Ví dụ sau đây cho thấy cấu trúc của một giao dịch chứa một lệnh chuyển SOL duy
nhất.
...
<WithMentions>
Đoạn mã dưới đây hiển thị đầu ra từ các đoạn mã trước.
Định dạng khác nhau giữa các SDK,
nhưng hãy chú ý rằng mỗi lệnh đều chứa cùng thông tin bắt buộc.
...
Vì một lệnh chuyển SOL có thể thành công vào bất kỳ tài khoản nào, hãy kiểm tra
người nhận trước khi ký. Tải tài khoản và chỉ gửi đến ví System Program (hoặc
một địa chỉ
[trên đường cong](/docs/payments/send-payments/verify-address#on-curve-addresses)
chưa được nạp tiền); từ chối các mint, token account, chương trình và PDA mà bạn
không kiểm soát.
...
<Callout>
  ...
  các lệnh gửi SPL token (token account, ATA, Token-2022), hãy xem [Xác minh Địa
  chỉ](/docs/payments/send-payments/verify-address).
</Callout>

## Lấy chi tiết giao dịch

Sau khi gửi, hãy truy xuất chi tiết giao dịch bằng chữ ký giao dịch và phương
thức RPC [getTransaction](/docs/rpc/http/gettransaction).
