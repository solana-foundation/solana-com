...
- [`executable`](mention:executable): Cho biết tài khoản có chứa mã nhị phân
  thực thi hay không
- [`rent_epoch`](mention:rent_epoch): Trường rent epoch đã lỗi thời
...
Trường này thường được gọi là "dữ liệu tài khoản". Trường `data` này được coi là
tùy ý vì nó có thể chứa bất kỳ chuỗi byte nào. Mỗi chương trình xác định cấu
trúc của dữ liệu được lưu trữ trong trường này.
...
1. Lấy tài khoản bằng [địa chỉ](#account-address) của nó
2. Giải mã trường `data` của tài khoản từ byte thô thành cấu trúc dữ liệu phù
   hợp, như được định nghĩa bởi chương trình sở hữu tài khoản.
...
làm chủ sở hữu. Chủ sở hữu của tài khoản là chương trình duy nhất có thể thay
đổi `data` của tài khoản hoặc trừ lamport, như được chỉ định bởi các lệnh của
chương trình.
...
Trong sơ đồ bên dưới, bạn có thể thấy một chương trình loader được sử dụng để
triển khai một program account. Trường `data` của program account chứa mã chương
trình thực thi.
...
trường `data` của chúng. Thay vào đó, trường `data` của chúng trỏ đến một
**program data account** riêng biệt, chứa mã chương trình. (Xem sơ đồ bên dưới.)
...
Ví dụ bên dưới lấy thông tin tài khoản Token Program. Lưu ý rằng trường
...
làm được điều này, trước tiên chúng phải tạo một tài khoản dữ liệu mới. Quá
trình tạo tài khoản trạng thái chương trình thường được trừu tượng hóa, nhưng
việc hiểu rõ quy trình cơ bản sẽ rất hữu ích.
...
Không phải tất cả các tài khoản đều được chỉ định chủ sở hữu mới sau khi được
tạo bởi System Program. Các tài khoản thuộc sở hữu của System Program được gọi
là tài khoản hệ thống. Tất cả các tài khoản ví đều là tài khoản hệ thống, cho
phép chúng thanh toán [phí giao dịch](/docs/core/fees).
...
Trong ví dụ bên dưới, một keypair mới được tạo và được nạp SOL. Sau khi chạy mã,
bạn có thể thấy địa chỉ `owner` của tài khoản là
`11111111111111111111111111111111` (là
