...
- [`header`](mention:message-header): [Header](#header) của message
...
- [`instructions`](mention:instructions): Một mảng các [chỉ thị](#instructions)
...
`header` của message là một cấu trúc
...
của thông điệp là một mảng các địa chỉ tài khoản, được gửi theo
[định dạng mảng compact](/docs/references/terminology#compact-array-format).
Tiền tố của mảng cho biết độ dài của nó. Mỗi phần tử trong mảng là một khóa công
khai, trỏ đến một tài khoản được sử dụng bởi các lệnh của nó. Mảng
...
`recent_blockhash` của thông điệp là một giá trị hash hoạt động như dấu thời
gian của giao dịch và ngăn chặn các giao dịch trùng lặp. Một blockhash hết hạn
sau
[150 block](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/clock/src/lib.rs#L134).
(Tương đương với một phút—giả sử mỗi block là 400ms.) Sau khi block hết hạn,
giao dịch sẽ hết hạn và không thể được xử lý.
...
của thông điệp là một mảng tất cả các lệnh cần được xử lý, được gửi theo
[định dạng mảng compact](/docs/references/terminology#compact-array-format).
...
1. [`program_id_index`](mention:program-id-index): Một chỉ số trỏ đến một địa
   ...
   chỉ của chương trình xử lý lệnh.
2. [`accounts`](mention:account-indexes): Một mảng các chỉ số trỏ đến các địa
   ...
   cần thiết cho lệnh này.
3. [`data`](mention:instruction-data): Một mảng byte chỉ định lệnh nào cần gọi
   trên chương trình. Nó cũng bao gồm bất kỳ dữ liệu bổ sung nào được yêu cầu
   bởi lệnh. (Ví dụ: các tham số hàm)
