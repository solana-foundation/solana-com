...
- [`signatures`](mention:signatures): Một mảng các [chữ ký](#signatures)
...
Các giao dịch có giới hạn tổng kích thước là
...
<Callout>
  ...
  Solana sử dụng 1280 byte (MTU tối thiểu yêu cầu cho IPv6) trừ đi 48 byte cho
...
có kích thước 64 byte và được tạo bằng cách ký `Message` của giao dịch với khóa
riêng của tài khoản. Một chữ ký phải được cung cấp cho mỗi
[tài khoản ký](#account-addresses) được bao gồm trong bất kỳ lệnh nào của giao
dịch.
...
ký giao dịch có thể được sử dụng để tra cứu thông tin chi tiết của giao dịch
trên mạng.
...
của message là một mảng các địa chỉ tài khoản, được gửi theo
[định dạng mảng compact](/docs/references/terminology#compact-array-format).
Tiền tố của mảng cho biết độ dài của nó. Mỗi phần tử trong mảng là một public
key, trỏ đến một tài khoản được sử dụng bởi các instruction của nó. Mảng
`accounts_keys` phải đầy đủ và được sắp xếp nghiêm ngặt như sau:
...
<Callout>
  Thứ tự nghiêm ngặt cho phép mảng `account_keys` được kết hợp với thông tin
  trong [`header`](#header) của message để xác định quyền cho từng tài khoản.
</Callout>
...
`recent_blockhash` của message là một giá trị hash hoạt động như dấu thời gian
của transaction và ngăn chặn các transaction trùng lặp. Một blockhash hết hạn
sau
[150 block](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/clock/src/lib.rs#L134).
(Tương đương một phút—giả sử mỗi block là 400ms.) Sau khi block hết hạn,
transaction sẽ hết hạn và không thể được xử lý.

<Callout>
  Phương thức RPC [`getLatestBlockhash`](/docs/rpc/http/getlatestblockhash) cho
  phép bạn lấy blockhash hiện tại và block height cuối cùng mà tại đó blockhash
  sẽ còn hợp lệ.
</Callout>
...
của message là một mảng tất cả các instruction cần được xử lý, được gửi theo
[định dạng mảng compact](/docs/references/terminology#compact-array-format).
Tiền tố của mảng cho biết độ dài của nó. Mỗi phần tử trong mảng là một cấu trúc
[`CompiledInstruction`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/instruction.rs#L22)
và bao gồm các thông tin sau:

<WithMentions>

1. [`program_id_index`](mention:program-id-index): Một chỉ số trỏ đến một địa
   chỉ trong mảng [`account_keys`](#account-addresses). Giá trị này cho biết địa
   chỉ của program xử lý instruction.
2. [`accounts`](mention:account-indexes): Một mảng các chỉ số trỏ đến các địa
   chỉ trong mảng `account_keys`. Mỗi chỉ số trỏ đến địa chỉ của một tài khoản
   cần thiết cho instruction này.
3. [`data`](mention:instruction-data): Một mảng byte chỉ định instruction nào sẽ
   được gọi trên program. Nó cũng bao gồm bất kỳ dữ liệu bổ sung nào cần thiết
   cho instruction. (Ví dụ: các tham số của hàm)
