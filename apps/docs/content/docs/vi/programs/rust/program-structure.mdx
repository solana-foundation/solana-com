...
phép tính linh hoạt trong cách tổ chức code. Yêu cầu duy nhất là chương trình
phải có một `entrypoint`, xác định nơi bắt đầu thực thi của chương trình.
...
- `entrypoint.rs`: Định nghĩa entrypoint để định tuyến các instruction đến.
- `state.rs`: Định nghĩa trạng thái chương trình (dữ liệu account).
- `instructions.rs`: Định nghĩa các instruction mà chương trình có thể thực thi.
- `processor.rs`: Định nghĩa các instruction handler (hàm) triển khai logic
  nghiệp vụ cho từng instruction.
...
1. `InitializeCounter`: Tạo và khởi tạo một account mới với giá trị ban đầu.
2. `IncrementCounter`: Tăng giá trị được lưu trữ trong một account hiện có.

Để đơn giản, chương trình sẽ được triển khai trong một file `lib.rs` duy nhất,
...
Bạn sẽ thấy các file `src/lib.rs` và `Cargo.toml` mặc định.
...
Bây giờ hãy thêm các dependency cần thiết để build chương trình Solana. Chúng ta
...
Các chương trình Solana phải được compile dưới dạng dynamic library. Thêm phần
...
xử lý việc giải mã hóa dữ liệu `input` thành các tham số của hàm
`process_instruction`.

Một `entrypoint` của chương trình Solana có chữ ký hàm như sau. Các nhà phát
triển hoàn toàn tự do tạo triển khai riêng của hàm `entrypoint`.
...
Bây giờ hãy định nghĩa cấu trúc dữ liệu sẽ được lưu trữ trong các tài khoản
counter của chúng ta. Đây là dữ liệu sẽ được lưu trữ trong trường `data` của tài
khoản.
...
Bây giờ hãy cập nhật hàm `process_instruction` chính để định tuyến các
instruction đến các hàm xử lý tương ứng của chúng.

Mô hình định tuyến này phổ biến trong các chương trình Solana.
`instruction_data` được giải mã hóa thành một biến thể của enum đại diện cho
instruction, sau đó hàm xử lý tương ứng được gọi. Mỗi hàm xử lý bao gồm triển
khai cho instruction đó.

Thêm đoạn mã sau vào `lib.rs` cập nhật hàm `process_instruction` và thêm các
handler cho các instruction `InitializeCounter` và `IncrementCounter`:
...
Chương trình của chúng ta thực hiện CPI để gọi lệnh `create_account` của System
Program. Tài khoản mới được tạo với chương trình của chúng ta làm chủ sở hữu,
cho phép chương trình của chúng ta ghi vào tài khoản và khởi tạo dữ liệu.
...
- Giải mã hóa nó thành struct `CounterAccount`
- Tăng trường `count` lên 1
- Mã hóa struct `CounterAccount` trở lại vào trường `data` của tài khoản
...
Đầu tiên, hãy thêm các dependency cần thiết cho việc kiểm thử. Chúng ta sẽ sử
...
Bây giờ chúng ta cần build và tải chương trình của mình vào môi trường kiểm thử.
Chạy lệnh `cargo build-sbf` để build chương trình. Lệnh này sẽ tạo file
...
Cập nhật hàm `test_counter_program` để tải chương trình vào môi trường test.
...
<Callout type="warn">
  ...
  tải chương trình đã được biên dịch.
</Callout>
...
Chạy test bằng lệnh sau. Cờ `--nocapture` sẽ in kết quả đầu ra của test.
...
Khi validator cục bộ đang chạy, bạn có thể xem các giao dịch trên
...
`solana-test-validator` đang chạy trên đó.
