...
chương trình Solana tối giản in ra "Hello, world!" vào program log khi chương
trình được gọi.
...
Tiếp theo, kiểm tra chương trình bằng crate `litesvm`. Thêm các dependency sau
vào `Cargo.toml`.
...
Tiếp theo, deploy chương trình. Khi phát triển locally, chúng ta có thể sử dụng
...
Mở terminal mới và chạy lệnh `solana-test-validators` để khởi động local
validator.
...
terminal riêng để deploy chương trình lên local validator.
