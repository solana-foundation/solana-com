...
- **Program ID**: Địa chỉ onchain của chương trình của bạn. Người dùng tương tác
  với chương trình của bạn bằng cách tham chiếu đến Program ID.
- **Program Account**: Tài khoản onchain lưu trữ metadata của chương trình. Địa
  chỉ của program account chính là Program ID. Program account cũng lưu trữ địa
  chỉ của ProgramData Account.
- **ProgramData Account**: Tài khoản onchain lưu trữ mã thực thi đã triển khai
  của chương trình.
- **Program Authority**: Tài khoản có quyền cập nhật hoặc đóng chương trình. Mặc
  định, đây là ví CLI của bạn.
...
Program ID là địa chỉ onchain của program account của bạn. Người dùng và các
chương trình khác gọi chương trình của bạn bằng cách tham chiếu đến ID này trong
...
- **`--use-rpc`**: Gửi giao dịch đến RPC đã cấu hình. Cờ này yêu cầu kết nối RPC
  [có trọng số stake](/docs/defi/stake-weighted-qos) từ các nhà cung cấp như
  [Triton](https://triton.one/) hoặc [Helius](https://www.helius.dev/).
