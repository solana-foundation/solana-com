...
đường dẫn thực thi trong container giống với hash của program onchain được cung
cấp cho lệnh. Bởi vì bản build đã được tải lên image, không cần phải build lại
toàn bộ file thực thi, quá trình này có thể mất rất nhiều thời gian.
...
- **Signer sai**: Xác nhận signer của bạn là upgrade authority của program bằng
  cách chạy `solana program show YourProgramId`
...
- **Dữ liệu PDA không khớp**: Cập nhật PDA của bạn nếu bạn đã triển khai lại
  program. Dữ liệu PDA của bạn phải khớp với program đã triển khai.
- **Commit hash không chính xác**: Tạo PDA của bạn bằng commit hash chính xác mà
  bạn đã triển khai
- **Sự khác biệt môi trường build**: Sử dụng Docker với solana-verify khi tạo
  PDA của bạn
...
- **Lưu Trữ On-Chain**: Lưu trữ metadata xác minh của bạn (URL repo, commit
  hash, tham số build) onchain trong PDA thuộc sở hữu của program Otter Verify
  (`verifycLy8mB96wd9wqq3WDXQwM4oU6r42Th37Db9fC`)
- **Liên Kết Mật Mã**: PDA của bạn được tạo ra từ địa chỉ program của bạn, tạo
  một liên kết bất biến đến dữ liệu xác minh của bạn
- **Tin Cậy Phi Tập Trung**: Bất kỳ ai cũng có thể đọc PDA của bạn và xác minh
  program của bạn một cách độc lập

## Tại sao tôi phải tạo PDA trước khi sử dụng API?

API chỉ hoạt động với các PDA onchain vì:

- **Không Cần Tin Cậy**: API từ chối dữ liệu tùy ý - nó chỉ sử dụng những gì
  upgrade authority của bạn đã lưu trữ onchain
- **Đơn Giản Hơn**: Chỉ cần cung cấp signer + program_id; API lấy mọi thứ khác
  từ PDA của bạn
- **Chống Giả Mạo**: PDA của bạn tạo một bản ghi bất biến mà bất kỳ ai cũng có
  thể xác minh một cách độc lập
- **Chứng Minh Quyền Sở Hữu**: Signer của bạn phải là upgrade authority, chứng
  minh bằng mật mã rằng bạn kiểm soát program
...
- **PDA Trên Chuỗi**: Siêu dữ liệu xác minh của bạn tồn tại trên chuỗi, không bị
  kiểm soát bởi bất kỳ cơ quan trung tâm nào
- **Chứng Minh Quyền Nâng Cấp**: Chỉ có quyền nâng cấp của chương trình bạn mới
  có thể tạo/cập nhật PDA
- **Xác Minh Độc Lập**: Bất kỳ ai cũng có thể xác minh bằng cách đọc PDA của bạn
  và chạy `solana-verify` cục bộ
- **Tái Xác Minh Liên Tục**: API tự động tái xác minh tất cả các chương trình
  mỗi 24 giờ
