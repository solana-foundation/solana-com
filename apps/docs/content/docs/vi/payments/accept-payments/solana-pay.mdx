...
<Callout type="caution">
  ...
  phát hành phiên bản ổn định. Để biết thêm chi tiết, vui lòng xem [tài liệu
...
| `amount`    | `bigint`    | Số lượng tính bằng lamport hoặc đơn vị nhỏ nhất của token |
| `splToken`  | `Address`   | Địa chỉ mint của SPL token (bỏ qua nếu dùng SOL)          |
| `reference` | `Address[]` | Mã tham chiếu duy nhất để theo dõi giao dịch              |
| `label`     | `string`    | Tên người bán hiển thị trong ví                           |
| `message`   | `string`    | Thông báo hiển thị trước khi thanh toán                   |
| `memo`      | `string`    | Ghi chú onchain đính kèm giao dịch                        |
