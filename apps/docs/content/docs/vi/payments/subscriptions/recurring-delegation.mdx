...
- `amountPerPeriod` tính theo đơn vị cơ bản. Đối với token có 6 chữ số thập
  phân, `1_000_000` có nghĩa là `1` token.
- Chương trình từ chối các giao dịch chuyển vượt quá hạn mức còn lại của kỳ hiện
  tại.
- Khi kỳ tiếp theo bắt đầu, số lượng đã rút sẽ được đặt lại.
