---
title: Thanh toán với memo
description: Tìm hiểu cách đính kèm ID hóa đơn hoặc ghi chú vào một khoản thanh toán.
---

Chương trình memo của Solana cho phép bạn đính kèm số hóa đơn, mã đơn hàng hoặc
các tham chiếu tùy chỉnh vào bất kỳ khoản thanh toán nào. Các memo này được ghi
lại vĩnh viễn trên chuỗi và hiển thị trong nhật ký giao dịch, giúp bạn dễ dàng
đối chiếu các khoản thanh toán với hệ thống nội bộ của mình.

## Cách hoạt động

Chương trình memo ghi văn bản vào nhật ký của giao dịch. Các nhật ký này được
lập chỉ mục bởi các trình khám phá và nhà cung cấp RPC, giúp memo có thể tìm
kiếm được để đối chiếu thanh toán.
...
Thêm memo yêu cầu xây dựng các lệnh trực tiếp, điều này cho bạn quyền kiểm soát
những gì được bao gồm trong giao dịch.
...
## !!steps Tạo lệnh chuyển khoản

Tạo lệnh chuyển token chỉ định ATA nguồn, ATA đích, quyền hạn (người ký), và số
...
Tạo một lệnh memo với một thông điệp. Thông điệp này sẽ hiển thị trong nhật ký
chương trình của giao dịch.
...
## !!steps Xem ghi chú trong nhật ký giao dịch

Sau khi giao dịch được xác nhận, hãy truy xuất nó để xem ghi chú trong nhật ký.
Ghi chú sẽ xuất hiện dưới dạng thông báo nhật ký từ chương trình Memo.

<CodePlaceholder title="Thanh toán có ghi chú" />
