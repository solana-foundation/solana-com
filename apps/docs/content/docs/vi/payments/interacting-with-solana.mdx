...
<Callout type="caution" title="Không sử dụng RPC công khai cho production">
  Các endpoint công khai (`api.mainnet.solana.com`, `api.devnet.solana.com`) bị
  giới hạn tốc độ, không có SLA, và không phù hợp cho các luồng thanh toán trong môi trường production.
  Hãy sử dụng [nhà cung cấp RPC](https://solana.com/rpc) để bảo mật một endpoint RPC riêng tư
  cho các triển khai production.
</Callout>
