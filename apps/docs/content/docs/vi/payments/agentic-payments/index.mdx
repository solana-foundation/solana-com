...
Xác minh cài đặt bằng:

```bash
pay --version
```

Bộ công cụ sẽ tự động thiết lập địa chỉ cho bạn. Chạy lệnh sau để thiết lập và
...
Các thanh toán tự động (agentic payments) cần một cơ chế để client và server có
thể thương lượng điều khoản thanh toán qua hạ tầng web tiêu chuẩn mà không làm
gián đoạn các giao diện web truyền thống. Lĩnh vực thanh toán vi mô vẫn còn non
trẻ và các công cụ, tiêu chuẩn mới nhiều khả năng sẽ xuất hiện khi lĩnh vực này
phát triển. Tuy nhiên, [giao thức x402](https://www.x402.org/) đã nổi lên như
một tiêu chuẩn ban đầu với sự hỗ trợ mạnh mẽ từ hệ sinh thái. x402 sử dụng mã
trạng thái
[HTTP 402 "Payment Required"](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/402)
— một tiêu chuẩn có từ HTTP/1.1 nhưng chỉ trở nên thực tế với việc thanh toán
qua blockchain.

### Cách thức hoạt động

Client gửi một yêu cầu, nhận về phản hồi 402 kèm điều khoản thanh toán, sau đó
gửi lại yêu cầu kèm theo chữ ký thanh toán. Server ủy quyền việc xác minh và
thanh toán cho một facilitator — một bên trung gian tùy chọn xử lý việc gửi giao
dịch lên onchain. Sau khi facilitator xác nhận thanh toán, server trả về nội
dung được yêu cầu. Sự tách biệt này cho phép các nhà cung cấp API chấp nhận
thanh toán mà không cần trực tiếp quản lý bất kỳ hạ tầng onchain nào.

![Sơ đồ luồng x402](/assets/guides/x402/x402-flow-solana.png)

Giao thức này không lưu trạng thái (không cần session hay API key), hoạt động
với hạ tầng HTTP tiêu chuẩn và hỗ trợ bất kỳ SPL token nào mà server chấp nhận.
Dưới đây là đoạn mã minh họa cách thiết lập máy chủ Express sử dụng middleware
x402:
...
<Cards>
  ...
  <Card
    ...
    description="Triển khai thanh toán x402 không cần gas bằng cơ sở hạ tầng ký của Kora."
  />
</Cards>
...
| [MCPay.tech](https://mcpay.tech/)                               | Thanh toán theo yêu cầu cho máy chủ MCP     |
| [PayAI](https://payai.network/)                                 | Bộ hỗ trợ x402 với hỗ trợ Solana            |
| [x402 GitHub](https://github.com/coinbase/x402)                 | Triển khai tham chiếu                       |
| [ACK](https://github.com/agentcommercekit/ack)                  | Bộ công cụ thương mại tác nhân              |
| [A2A x402](https://github.com/google-agentic-commerce/a2a-x402) | Thanh toán tác nhân-đến-tác nhân của Google |
