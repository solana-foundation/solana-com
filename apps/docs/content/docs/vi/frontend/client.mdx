...
<Callout type="info">
  Chạy headless — trong một API route, worker, hoặc script không có ví trình
  duyệt? Hãy ký bằng dịch vụ quản lý khóa backend thay vì dùng keypair thô. Xem
  [Keychain](/docs/tools/keychain) và [Ký trong Môi Trường
  Production](/docs/core/transactions/signing-in-production).
</Callout>

## Các mẫu phổ biến cho các nhà phát triển Solana

- **Máy trạng thái headless**: Chạy client bên trong các API route, script hoặc
  worker để tái sử dụng cùng logic điều phối ví + RPC vốn đã cung cấp sức mạnh
  cho giao diện người dùng của bạn.
- **Bảng điều khiển thời gian thực**: Kết hợp các watcher (số dư, tài khoản, chữ
  ký) với thư viện giao diện người dùng ưa thích của bạn; client xử lý việc phân
  phối websocket và làm mới cache.
- **Store tùy chỉnh**: Inject store Zustand của riêng bạn để hydrate từ
  IndexedDB/localStorage, đồng bộ trạng thái với các phiên máy chủ, hoặc điều
  phối giữa các tab trình duyệt.
- **Cầu nối tới React hooks**: Truyền một client instance đã được cấu hình vào
  `@solana/react-hooks` khi bạn muốn các hook tiện lợi trên cùng một runtime.
- **Khả năng kiểm thử**: Giao diện client đơn nhất có thể được mock trong các
  unit test, giúp dễ dàng mô phỏng các phản hồi RPC hoặc phiên ví mà không cần
  ví trình duyệt.
