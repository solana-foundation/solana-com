...
<Callout type="info">
  ...
  duyệt? Ký bằng dịch vụ quản lý khóa backend thay vì dùng keypair thô. Xem
...
- **Bảng điều khiển thời gian thực**: Kết hợp các watcher (số dư, tài khoản, chữ
  ký) với thư viện giao diện người dùng bạn ưa thích; client xử lý việc phân
  phối websocket và làm mới cache.
- **Store tùy chỉnh**: Inject store Zustand của riêng bạn để nạp dữ liệu từ
  IndexedDB/localStorage, đồng bộ trạng thái với phiên máy chủ, hoặc phối hợp
  giữa các tab trình duyệt.
- **Kết nối với React hooks**: Truyền một client instance đã được cấu hình vào
