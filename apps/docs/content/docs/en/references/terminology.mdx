...
transaction format allows. Only version 0 (`v0`) transactions support address
lookup tables; version 1 (`v1`) removes them in favor of a larger transaction
size that fits the same addresses inline.
...
extensions such as Confidential Balances, custom transfer logic, extended
...
allows. Version 1 (`v1`), active on all clusters, raises the maximum transaction
size to 4,096 bytes, carries resource limits in a config section of the message
instead of ComputeBudget instructions, and drops address lookup table support.
See [versioned transactions](/docs/core/transactions/versioned-transactions).
