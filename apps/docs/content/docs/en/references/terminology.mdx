...
a [program derived address](#program-derived-account-pda).
...
[onchain program](#onchain-program) and one or more seeds created by the program
author. PDA addresses are not public keys, and PDAs do not have do not have
private keys.
