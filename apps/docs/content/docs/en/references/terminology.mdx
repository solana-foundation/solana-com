...
allows. Version 1 (`v1`), not yet active on any cluster, raises the maximum
transaction size to 4,096 bytes, carries resource limits in a config section of
the message instead of ComputeBudget instructions, and drops address lookup
table support. See
