...
title: RPC Endpoints
description:
  Learn about Solana's network clusters (Devnet, Testnet, and Mainnet), their
  public RPC endpoints, rate limits, and use cases. Learn how to connect to
  different Solana networks for development, testing, and production.
...
- Key differences between Devnet and Mainnet:
  ...
  - Devnet typically runs the same software release branch version as Mainnet
    Beta, but may run a newer minor release version than Mainnet.
...
- `https://api.mainnet.solana.com` - Solana Labs hosted API node cluster, backed
...
## Finding a dedicated RPC provider

Because the public endpoints are rate limited and not intended for production,
applications typically run their own node or use a dedicated RPC provider.
Several third parties operate Solana RPC and API nodes, each with its own
pricing and rate limits.
[Chain.Love](https://mainnet.solana.chain.love/toolbox/apis) maintains an open
directory of them, alongside Solana indexers, oracles, explorers and bridges;
its catalogue is published as CSV under CC BY-SA 4.0 and edited through public
pull requests.
