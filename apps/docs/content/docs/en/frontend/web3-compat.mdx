...
<Callout type="warn" title="v1 transactions force this migration">
  When the [v1 transaction
  format](/docs/core/transactions/versioned-transactions#v1-message-format)
  activates, web3.js v1 can no longer read every transaction on the network — it
  cannot deserialize v1, and no backport is planned. Any v1 code that calls
  `getTransaction` or `getBlock` is capped at `maxSupportedTransactionVersion:
  0` and will fail on v1 transactions. Kit 8.x and web3.js v3 both handle the
  format.
</Callout>
