...
<Callout type="warn" title="v1 transactions and web3.js v1">
  ...
  format](/docs/core/transactions/versioned-transactions#v1-message-format) from
  release 1.99.0 onward: `getTransaction`, `getBlock`, and
  `VersionedTransaction.deserialize` accept v1 once you pass
  `maxSupportedTransactionVersion: 1`. Reading is the only supported direction —
  building, signing, and sending v1 throw on v1 of the library. Anything earlier
  than 1.99.0 is capped at `maxSupportedTransactionVersion: 0` and fails on v1
  transactions outright. If you send transactions, Kit 8.x and web3.js v3 are
