...
```terminal
$ npm install @solana/kit @solana/kit-plugin-rpc @solana/kit-plugin-signer @solana/kit-plugin-instruction-plan
```
...
then transaction planning. `solanaDevnetRpc()` adds `client.rpc`, subscriptions,
and the transaction planner/executor; `planAndSendTransactions()` exposes
...
The presets are shortcuts over granular plugins. Reach for them when you want to
swap a piece — for example a custom planner or a different subscription channel:
