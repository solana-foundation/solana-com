...
```terminal
$ npm install @solana/kit @solana/kit-plugin-rpc @solana/kit-plugin-wallet @solana/kit-plugin-instruction-plan @solana/react @solana-program/system
```
...
import { useState } from "react";
import type { SolanaClient } from "../providers";
...
export function SolTransferCard() {
  ...
  async function sendSol() {
    ...
    try {
      ...
      const sig = await client.sendTransaction([transfer]);
