...
| Settlement Time  | 2 days                | One transaction             |
| Transaction Cost | Per-trade fees apply  | &lt;$0.01                   |
| Settlement Risk  | Sequential, both legs | None between the two legs   |
| Confirmation     | End of day            | Sub-second                  |

Solana's atomic transaction bundling removes the need to settle the two legs
sequentially, so neither side is exposed to the other having failed to deliver.
It does not remove counterparty risk generally: the issuer's credit, the cash
instrument's issuer, and any delegated authority in the flow all remain. The
settlement agent pattern below is itself an intermediary.
...
2. **Metadata Extension**: Stores bond details onchain (ISIN, maturity date,
   coupon rate, etc.)
...
[Address Lookup Tables](/developers/cookbook/transactions/lookup-tables) to
compress account addresses and fit more transfers per transaction.

The upcoming
[v1 transaction format](/docs/core/transactions/versioned-transactions#v1-message-format)
raises the limit to 4096 bytes but removes Address Lookup Table support, so v1
settlements inline their addresses instead. Either way the runtime loads at most
...
1. **Explore Token Extensions**: Learn about other
   [Token 2022 extensions](/docs/tokens/extensions/transfer-hook) like Transfer
   Hooks for additional compliance features

2. **Implement Token ACL**: For more sophisticated compliance needs, consider
   using [Token ACL](/docs/tokenization/compliance/token-acl) to implement
   allow/block lists with permissionless thaw capabilities. This allows users to
