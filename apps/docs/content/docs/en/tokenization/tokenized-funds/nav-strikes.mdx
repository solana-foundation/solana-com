...
description:
  ...
  using SPL Token 2022 extensions, enabling automated subscription and
...
1. **Fund Share Token**: SPL Token 2022 with metadata storing NAV onchain
...
4. Whitelist eligible investors
...
3. Order settles in a single transaction
```
...
| Settlement Time   | 1-2 days             | One transaction per order      |
| Transaction Cost  | Per-order fees apply | &lt;$0.01                      |
| Strikes Per Day   | 1                    | Set by the fund's schedule     |
| Stale prices risk | High                 | Lower                          |
| NAV Transparency  | End of day report    | Onchain, as of the last strike |

Solana's atomic transactions let each order settle in a single transaction, so
the cash and share legs cannot come apart. The fund administrator remains in the
flow: it computes the NAV off-chain and writes it onchain at each strike.
...
2. **Metadata Extension**: Stores NAV, strike schedule, and AUM onchain for
   transparency
...
updated onchain at each NAV strike using
`getUpdateTokenMetadataFieldInstruction`. This provides:

- **Real-time NAV visibility** for all participants
- **Audit trail** of all NAV updates onchain
- **Integration** with DeFi protocols that can read onchain metadata
...
async function printBalances(
  ...
  console.log(
    "│  Strikes/Day:              1 (4PM)              4 (this example) │"
  );
  console.log(
    "│  Settlement:              T+1/T+2                   One txn     │"
  );
  console.log(
...
3. **Key Management**: Custody solutions with proper backup and recovery
   procedures
...
1. **Explore Token Extensions**: Learn about other
   [Token 2022 extensions](/docs/tokens/extensions/transfer-hook) like for
   example permanent delegate for additional compliance features

2. **Implement Token ACL**: For enhanced compliance, consider using
   [Token ACL](/docs/tokenization/compliance/token-acl) to manage investor
   whitelisting with permissionless thaw. This enables self-service KYC
   verification where investors can automatically activate their accounts after
   passing compliance checks, reducing operational overhead while maintaining
   regulatory control.

3. **Study DvP Settlement**: See our
   [Delivery vs Payment guide](/docs/tokenization/settlement/dvp) for securities
   settlement patterns
