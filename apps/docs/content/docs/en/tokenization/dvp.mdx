...
| Settlement Time    | 2 days                | Near instant                                                                                |
| Pre-confirmation   | N/A                   | ≈400ms                                                                                      |
| Consensus Finality | End of day            | ≈12.8 seconds today ([≈150ms target with Alpenglow](https://solana.com/upgrades/alpenglow)) |
...
fast, low-cost, and secure settlements.
...
async function runDvPExample() {
  ...
  // 4. Create a devnet settlement mint that stands in for USDC
  // On mainnet, use the official USDC mint instead
  ...
  // 5. Create commercial paper (bond) token with metadata
  ...
  // 6. Whitelist issuer and mint bonds
  ...
  // 7. Whitelist investor (KYC/AML approved)
  await dvp.whitelist(bondMint, investor.publicKey, settlementAgent);

  // 8. Create settlement accounts and fund the investor
  console.log("\n💵 Setting up the devnet settlement token...");
  ...
  // 9. Delegate authority to settlement agent
  ...
  // 10. Execute atomic DvP settlement
  console.log("\n⚡ Executing atomic DvP settlement...");
...
2. **Implement Token ACL**: For more sophisticated compliance needs, consider
   using [Token ACL](/docs/tokenization/token-acl) to implement allow/block
