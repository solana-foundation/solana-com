...
description:
  What to settle before a financial application moves from devnet to mainnet -
  RPC, transaction landing, authority custody, monitoring, and reconciliation.
seoTitle: Taking a Solana Financial Application to Mainnet
url: /docs/tools/production-readiness
type: conceptual
related:
...
This page covers the differences that matter when an application moves from
devnet to mainnet.
...
(`api.mainnet.solana.com`) are rate-limited with no SLA. They're fine for
development but will fail production workloads - like running a financial
application through a shared API with no uptime guarantee.
...
Choosing a provider is its own decision - workload shape, method and history
support, latency, failover, and credential handling. See
[RPC providers](/docs/rpc/providers) for the selection criteria and checklist,
and [RPC Infrastructure Providers](https://solana.com/rpc) for the provider
list. What follows is what to put in place once a provider is chosen.
...
A transaction that does not land has not failed - it has not happened yet, and
the distinction matters to anything keeping books.
...
- Store the `lastValidBlockHeight` returned by `getLatestBlockhash` - this tells
  you when your transaction expires
- Blockhashes expire after ~150 blocks (~60-90 seconds)

**Send options:**

- `maxRetries: 0` - Disable automatic RPC retries. Handle retries yourself so
  you can refresh the blockhash when needed.
- `skipPreflight: true` - Bypass simulation before sending. Use this when you've
...
Transactions can fail for many reasons. Unlike an API that returns success or
failure immediately, a Solana transaction requires confirmation tracking.
...
- **Idempotency**: The same signed transaction always produces the same
  signature - resubmitting is safe
...
- [Guide: Transaction Confirmation & Expiration](https://solana.com/developers/cookbook/transactions/confirmation)
...
| `confirmed` | Supermajority voted       | Cleared funds          | **Most transfers**     |
| `finalized` | Rooted, irreversible      | Settled funds          | High-value, compliance |

**When to use each:**

- **UI updates**: Show `processed` for immediate feedback ("Transaction
  submitted")
...
Which commitment level your records treat as settled is a decision about your
books, not a client configuration detail.
[Settlement and finality](/docs/tokenization/settlement/finality) is the
authoritative page on what each level means, how many slots each takes, why a
blockhash should be fetched at one level and a result judged at another, and why
...
function handleTransactionError(error: unknown): {
  ...
  return { message: "Transaction failed - please retry", retryable: true };
}
```
...
transactions solve this - similar to how Venmo users don't think about ACH fees.
See
...
An issued asset has authorities - mint, freeze, pause, metadata, and any
delegate - and losing one is usually unrecoverable. Before mainnet:

- No authority is a single hot key on a single machine.
- Each authority is held by the smallest party that needs it, rather than one
  key holding everything. See
  [program and authority design](/docs/tokenization/design/program-and-authority-design).
- Someone other than the original engineer can execute a recovery, and that has
  been rehearsed rather than documented.
...
Treat RPC endpoints like API keys - don't expose them in frontend code where
they can be extracted and abused. Use a backend proxy or environment variables
that aren't bundled into client code. See [RPC providers](/docs/rpc/providers)
for credential handling as part of provider selection.
...
Monitoring tells you the system is healthy; reconciliation tells you the books
are right. A financial application has to answer what happened, not only whether
the last call returned successfully.

- Record the transaction signature and the commitment level at which you
  accepted it, for every state change that matters.
- Order your own records by slot, not by wall-clock time.
- Reconcile against the chain rather than against your own optimistic writes,
  and reconcile against a custodian's view where one holds the asset. See
  [custody integration](/docs/tokenization/custody/custody-integration).
- Alert on transactions that were sent and never confirmed, which is the failure
  mode that quietly breaks books.

### Verify Addresses

Every token and program has exactly one correct address on mainnet. Spoofed
tokens mimicking USDC or other stablecoins are common - they'll have the same
name and symbol but a different mint. Your application should hardcode or
allowlist the mint addresses (based on your requirements), never accept them
dynamically from untrusted sources.
...
addresses per environment - don't hardcode mainnet addresses and forget to swap
...
fail - or worse, result in irreversible loss of funds. The Token Program
addresses are:
...
- [ ] The commitment level your books call settled, written down
- [ ] Every authority held in auditable signing infrastructure, with a rehearsed
      recovery path
- [ ] Signatures and commitment levels recorded for every material state change
- [ ] Reconciliation running against the chain and against any custodian

## Deploying Programs

If you're deploying a custom Solana program as part of your infrastructure,
...
- **Buffer accounts:** Large programs deploy via buffer accounts. The
  ...
  that deployment isn't atomic - if interrupted, you may need to recover or
  close buffer accounts. See
...
- **Verification:** [Verify](https://solana.com/docs/programs/verified-builds)
  your program to ensure that the executable program you deploy to Solana’s
...
- [Settlement and finality](/docs/tokenization/settlement/finality) - which
  commitment level to trust
- [Developer tools](/docs/finance/developer-tools) - the tooling referenced here
- [Payments](/docs/payments) - payment-flow specifics such as fee abstraction
