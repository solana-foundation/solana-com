...
into an interactive set of buttons. This pushes the ability to interact onchain
...
- `links.next` - An optional value use to "chain" multiple Actions together in
  series. After the included `transaction` has been confirmed onchain, the
...
Action's transaction is confirmed onchain, the next action can be obtained and
...
confirmed onchain, the blink client should either:
...
onchain activity to a specific Action Provider (i.e. service) in a verifiable
way.
...
3. Verify the specific transaction is the first onchain occurrence of the
   `reference` onchain:
...
transaction is the first onchain occurrence of the `reference`. If it is not,
