...
Each integration phase is its own `[[test]]` target in `tests/Cargo.toml`. Every
target boots an embedded surfnet plus a Kora node through `harness_context`,
seeded from scratch. Kora keeps config in process-global state, so a new signer
needs a new target rather than a new module in an existing one.

#### 1. Add a Signer Fixture

Create a signer configuration in `tests/src/common/fixtures/`:
...
Register the phase in `tests/Cargo.toml`:

```toml
[[test]]
name = "your_service"
path = "your_service/main.rs"
```

Then create `tests/your_service/main.rs`. It points the harness at your fixture
and must contain at least one `#[tokio::test]` that exercises the signer:
...
#[tokio::test]
async fn test_your_service_signs_as_fee_payer() {
    let ctx = ctx().await;

    let response: serde_json::Value = ctx
        .rpc_call("getPayerSigner", rpc_params![])
        .await
        .expect("Failed to get payer signer");

    response.assert_success();
    assert!(response["signer_address"].as_str().is_some());
}
```

Add a `signTransaction` test as well so the signer's signing path runs, not just
key lookup. `tests/multi_signer/` is a complete example to copy from.
...
# Run only your phase
cargo test -p tests --test your_service

# Run every integration phase
