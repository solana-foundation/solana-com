...
- **`link`** (`URL`, required) - The link to the transaction
  ([`Link`](/docs/tools/solana-pay/specification/version1-1#link)). If the URL
  contains query parameters, it must be URL-encoded.
...
- Blockhash lifetime (ages 0 through 150 inclusive, usually about 60 seconds,
  with the exact expiration in `lastValidBlockHeight`)
