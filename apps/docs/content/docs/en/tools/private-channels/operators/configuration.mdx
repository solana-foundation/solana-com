...
> (`127.0.0.1`) only. Gateway, Streamer, and Auth service are bound to all
> network interfaces because they serve external clients directly. RBAC is
> enforced at the gateway layer, not the node level.
