...
>
> The gateway binary's own built-in default for `GATEWAY_PORT` is `8898`. Every
> tracked env file (`.env.example`, `versions.env`, `.env.devnet`) sets it to
> `8899`, which is what the Docker Compose setup actually uses. The `8898`
> default only applies if you run the binary directly with no env file.
