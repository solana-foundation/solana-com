...
1. **Fee payer validation**: The fee payer (first account) must exist, be a
   system account or nonce account, and have enough lamports to cover fees while
   remaining rent-exempt
   ([`validate_fee_payer()`](https://github.com/anza-xyz/agave/blob/v3.1.8/svm/src/account_loader.rs#L369-L417)).
   If the payer is neither a system account nor a nonce account, the transaction
...
3. **Program account validation**: Every program invoked by an instruction must
   ...
   with `TransactionError::ProgramAccountNotFound`. If it exists but has an
   invalid owner, the transaction fails with
   `TransactionError::InvalidProgramForExecution`.

4. **Rent epoch update**: Rent-exempt accounts that do not yet have `rent_epoch`
   set to `u64::MAX` are updated during loading
   ([`update_rent_exempt_status_for_account()`](https://github.com/anza-xyz/agave/blob/v3.1.8/svm/src/account_loader.rs#L351-L362)).

5. **Non-existent accounts**: Accounts that do not exist on-chain are loaded as
