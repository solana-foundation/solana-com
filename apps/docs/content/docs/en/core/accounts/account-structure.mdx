...
on mainnet `lamports_per_byte_year` = 3,480 and `exemption_threshold` = 2.0, so
the effective rate is 3,480 \* 2 = 6,960 lamports/byte;
...
Any program can credit (add) lamports to any **writable** account, but only the
owner can debit them.
...
Previously tracked when an account would next owe rent. Since rent collection is
deprecated, `rent_epoch` is set to `u64::MAX` (18446744073709551615) for all
...
**Example**: A 0-byte data account requires
`(0 + 128) * 3480 * 2 = 890,880 lamports` (~0.00089 SOL).
