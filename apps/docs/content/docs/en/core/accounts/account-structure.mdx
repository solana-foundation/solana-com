...
Every account must maintain a minimum lamport balance to keep its data stored
onchain. This refundable storage balance is proportional to the account's data
size.

<Callout type="info">
  The runtime and RPC method names call this the "rent-exempt minimum," but
  ongoing rent collection is disabled. Treat the balance as a refundable account
  storage deposit: the full balance can be recovered when the account is closed.
</Callout>
...
pay for maintaining its data onchain. Since ongoing rent collection is disabled,
`rent_epoch` is set to `u64::MAX` for all new accounts that meet the minimum
storage balance.
