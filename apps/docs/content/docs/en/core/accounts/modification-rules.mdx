...
fails with `AccountBorrowFailed`. Similarly, if an instruction completes while a
borrow is still outstanding, the runtime returns `AccountBorrowOutstanding`.
These are common errors developers encounter when passing the same account to
multiple arguments of a single instruction.
