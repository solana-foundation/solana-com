...
title: Account Types
description:
  Solana account types — program accounts for executable code, data accounts
  for program state, system accounts owned by the System Program, and sysvars
  for cluster-wide state at predefined addresses.
