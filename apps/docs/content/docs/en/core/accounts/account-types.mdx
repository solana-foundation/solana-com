...
async fn main() -> Result<()> {
    ...
    // Deserialize the account data (the `SysvarSerialize` trait and
    // `bincode`-based helpers were removed in the latest sysvar crate;
    // use `wincode::deserialize` and verify the sysvar account address).
