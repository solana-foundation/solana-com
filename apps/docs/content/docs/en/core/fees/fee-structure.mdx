...
<Callout type="info" title="v1 transactions set the fee directly">
  This price-times-limit formula applies to legacy and v0 transactions. In the
  upcoming [v1
  format](/docs/core/transactions/versioned-transactions#v1-message-format), the
  priority fee is an absolute total in lamports set directly in the message
  config — no per-CU price, no multiplication, no rounding.
</Callout>
