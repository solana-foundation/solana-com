...
description:
  Solana Program Derived Addresses (PDAs) are deterministic, off-curve
  addresses derived from seeds and a program ID. No private key exists — only
  the deriving program can sign via invoke_signed.
url: /docs/core/pda
type: conceptual
prerequisites:
  - /docs/core/accounts
