...
title: PDA Accounts
description:
  How to create on-chain accounts at PDA addresses using invoke_signed and
  Anchor's init constraint. Covers the invoke_signed verification flow and
  Anchor patterns for PDA account creation.
---

{/* TOC: PDA Signing via invoke_signed, Create a PDA Account */}

<Callout type="info" title="Summary">
  Create PDA accounts via `invoke_signed` with the PDA's seeds. Only the
  owning program can sign for a PDA. Anchor's `init` constraint automates PDA
  account creation.
</Callout>
...
`invoke_signed` with the PDA's seeds. The runtime verifies the seeds derive the
expected PDA using the **calling program's ID**, ensuring only the owning program
can sign. For the full verification flow, see
[PDA Signing](/docs/core/cpi/cpi-advanced#pda-signing).
...
[invoke the System Program](/docs/core/programs/builtin-programs#the-system-program) to create a
