...
<Callout type="info" title="Summary">
  ...
  result is off the Ed25519 curve. The canonical bump (highest valid value)
  costs ~1,500 CUs. Max 16 seeds, 32 bytes each.
</Callout>
...
[`create_program_address`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L911-L946)
...
1. Validate that the number of seeds does not exceed `MAX_SEEDS` (16) and no
   ...
   return `PubkeyError::MaxSeedLengthExceeded`.
...
6. Check if the result is a valid point on the Ed25519 curve using
   [`bytes_are_curve_point`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L456-L467),
   which attempts to decompress the bytes as a `CompressedEdwardsY` point. If
   decompression succeeds, the point is on the curve.
7. If the result IS on the curve, return `PubkeyError::InvalidSeeds` (the
...
[`create_with_seed`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L522-L542)
function explicitly rejects owners whose bytes end with `"ProgramDerivedAddress"`.

## Bump Seed Search

[`find_program_address`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L800-L845)
