...
function explicitly rejects owners whose bytes end with
...
For simpler cases, use a subset of these seeds. Click **&#9655; Run** to execute
the code.
...
<Callout type="warn">
  Seeds are hashed sequentially, so `["ab", "cd"]` and `["abcd"]` produce the
  same PDA. Use a fixed-length seed or a separator to avoid collisions. For
  example, using `["ab", "-", "cd"]` is unambiguous.
</Callout>
