...
| `MAX_PROCESSING_AGE`           | 150 slots   | Maximum accepted recent-blockhash age; blockhashes are accepted up to age 150                                                                                                                                                                                                                                  | [`MAX_PROCESSING_AGE`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v3.2.0/clock/src/lib.rs#L98-L101)             |
...
Validators accept a recent blockhash up to age 150, commonly described as 150
slots or about 60 seconds. Because skipped slots and changing slot duration make
time conversions unreliable, applications should use the `lastValidBlockHeight`
returned by [`getLatestBlockhash`](/docs/rpc/http/getlatestblockhash) to
determine when a specific blockhash expires.
