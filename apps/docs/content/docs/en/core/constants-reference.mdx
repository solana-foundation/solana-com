...
`MAX_PROCESSING_AGE` is the protocol limit to use when describing blockhash
validity. Its wall-clock duration varies with slot production and should not be
used as an application deadline. Use the `lastValidBlockHeight` returned by
