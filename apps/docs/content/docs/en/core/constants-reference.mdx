...
| `MAX_PROCESSING_AGE`           | 150 ages    | Maximum accepted recent-blockhash age; ages 0 through 150 are valid                                                                                                                                                                                                                                            | [`MAX_PROCESSING_AGE`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v3.2.0/clock/src/lib.rs#L98-L101)             |
...
`MAX_PROCESSING_AGE` measures blockhash age, not elapsed slots or wall-clock
time. Validators accept ages 0 through 150. Because skipped slots and changing
slot duration make time conversions unreliable, applications should use the
`lastValidBlockHeight` returned by
