...
title: Core Programs
description:
  Solana's core programs including the System Program, Vote, Stake, Compute
  Budget, Address Lookup Table, and loaders.
...
<Callout type="info" title="Summary">
  Core programs provide fundamental network functionality: account creation
  (System Program), program deployment (loaders), and network operations (Vote,
  Stake, Compute Budget, Address Lookup Table). This page lists each program and
  its instructions.
</Callout>
