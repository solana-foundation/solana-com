...
<Callout type="info" title="Summary">
  Programs compile to sBPF via LLVM and execute in a sandboxed VM with a 1.4M CU
  budget per transaction. The runtime provides syscalls for logging, CPI,
...
5. **Executes the BPF program.** For BPF programs, the
   ...
   - Runs the compiled code via JIT on x86_64 or interpretation on other
     architectures
...
The
[`decayed_usage_counter()`](https://github.com/anza-xyz/agave/blob/v3.1.8/program-runtime/src/loaded_programs.rs#L513)
...
happens, all `Loaded` programs are re-verified against the new environment.
Programs that no longer pass verification transition to `FailedVerification`;
programs that previously failed may transition to `Loaded`.
