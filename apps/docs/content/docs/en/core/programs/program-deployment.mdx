...
<Callout type="info" title="Summary">
  Programs can be upgraded if an upgrade authority is set. Revoking the
  authority makes the program immutable. This page covers the upgrade mechanism,
  loader instructions, build verification, and program types.
</Callout>
