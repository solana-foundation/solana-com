...
function is used. The _rs`invoke`_ function calls the _rs`invoke_signed`_ function with
an empty _rs`signers_seeds`_ array. The empty signers array indicates that no PDAs
