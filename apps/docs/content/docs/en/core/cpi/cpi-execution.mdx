...
The SBF VM dispatches to one of two registered syscall handlers depending on the
caller's language:

- **Rust programs**:
  [`sol_invoke_signed_rust`](https://github.com/anza-xyz/agave/blob/v3.1.8/syscalls/src/cpi.rs#L11-L33)
  dispatches to `SyscallInvokeSignedRust`
- **C programs**:
  [`sol_invoke_signed_c`](https://github.com/anza-xyz/agave/blob/v3.1.8/syscalls/src/cpi.rs#L78-L100)
  dispatches to `SyscallInvokeSignedC`

Both handlers call into the same shared entry point:
...
1. [Validates the account info count](https://github.com/anza-xyz/agave/blob/v3.1.8/program-runtime/src/cpi.rs#L164-L190)
   against `MAX_CPI_ACCOUNT_INFOS` (128, or 255 with SIMD-0339).
...
1. Calls
   ...
   to add a new frame to the instruction stack. `push()` enforces the
...
3. Calls
   ...
   to remove the callee's frame from the instruction stack. `pop()` also
   verifies the lamport balance has not changed
...
2. **Signer privilege requires authorization.** An account can be a signer in
   ...
   PDA derived from the calling program's seeds via `invoke_signed`.
