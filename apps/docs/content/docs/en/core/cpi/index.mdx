...
<Cards>
  ...
  >
    12-step CPI execution flow, privilege extension rules, reentrancy, account
    validation.
  </Card>
  <Card
...
- **Two functions**: `invoke` (no PDA signing) and `invoke_signed` (with PDA
  signing).
...
- **Reentrancy**: Direct self-recursion is allowed (A->A->A). Indirect
  reentrancy is not (A->B->A returns `ReentrancyNotAllowed`).
...
\*Legacy value was 64 before `increase_tx_account_lock_limit` feature
activation.

See [Instructions](/docs/core/instructions) for instruction trace length limits.
See [PDA](/docs/core/pda) for seed and derivation constraints.
