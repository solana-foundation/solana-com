---
title: Core Concepts
description:
  Solana's core building blocks -- accounts, programs, instructions, transactions,
  fees, program-derived addresses, and cross-program invocations.
---

Seven concepts form the foundation of Solana development. Start with Accounts and read through Programs, Instructions, Transactions, and Fees in order -- each builds on the previous. PDAs build on Accounts and Programs. CPIs build on Programs, Instructions, and PDAs.

<Cards>
  <Card title="Accounts" href="/docs/core/accounts">
    Solana stores all state in accounts, a key-value store keyed by 32-byte addresses. Each account has five fields: lamports, data, owner, executable, and rent_epoch.
  </Card>
  <Card title="Programs" href="/docs/core/programs">
    Solana's smart contracts, compiled to sBPF. Programs are stateless -- all mutable state lives in separate data accounts passed via instructions.
  </Card>
  <Card title="Instructions" href="/docs/core/instructions">
    A request to execute one program, containing a program ID, accounts list, and opaque data bytes. Multiple instructions compose into a single transaction.
  </Card>
  <Card title="Transactions" href="/docs/core/transactions">
    The atomic unit of execution on Solana. Contains signatures and a message (account addresses, recent blockhash, and compiled instructions). Max 1232 bytes.
  </Card>
  <Card title="Fees" href="/docs/core/fees">
    Base fee of 5,000 lamports per signature, plus an optional priority fee based on compute units requested. Max 1.4M compute units per transaction.
  </Card>
  <Card title="Program Derived Addresses (PDAs)" href="/docs/core/pda">
    Deterministic addresses derived from a program ID and seeds. No private key -- only the owning program can sign on behalf of a PDA via invoke_signed.
  </Card>
  <Card title="Cross-Program Invocations (CPIs)" href="/docs/core/cpi">
    One program calling another via invoke or invoke_signed. Signer and writable privileges extend to the callee. Max instruction stack depth of 5 (top-level + CPIs).
  </Card>
</Cards>

## Related topics

<Cards>
  <Card title="Tokens" href="/docs/core/tokens">
    Fungible and non-fungible digital assets on Solana (SPL Token, Token-2022).
  </Card>
  <Card title="Clusters and Endpoints" href="/docs/references/clusters">
    Mainnet, Devnet, and Testnet -- validator groups serving JSON-RPC requests.
  </Card>
</Cards>
