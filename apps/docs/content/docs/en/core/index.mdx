...
title: Core Concepts
description:
  Solana's core building blocks -- accounts, programs, instructions,
...
and read through Programs, Instructions, Transactions, and Fees in order -- each
builds on the previous. PDAs build on Accounts and Programs. CPIs build on
Programs, Instructions, and PDAs.
...
<Cards>
  ...
  <Card title="Programs" href="/docs/core/programs">
    Solana's smart contracts, compiled to sBPF. Programs are stateless -- all
  ...
  <Card title="Program Derived Addresses (PDAs)" href="/docs/core/pda">
    Deterministic addresses derived from a program ID and seeds. No private key
    -- only the owning program can sign on behalf of a PDA via invoke_signed.
  </Card>
