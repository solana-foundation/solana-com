...
writable accounts can execute concurrently. Providing accurate `AccountMeta` is
what enables this parallelism.
...
- `AccountMeta::new(pubkey, is_signer)` creates a **writable** account reference
  (`is_writable` defaults to `true`).
- `AccountMeta::new_readonly(pubkey, is_signer)` creates a **read-only** account
  reference (`is_writable` is `false`).
...
- **Anchor programs**: Use an 8-byte discriminator (the first 8 bytes of the
  ...
  arguments.
- **Custom programs**: Can use any encoding. Borsh is the most common because it
  has a stable [specification](https://borsh.io/) and implementations in both
  [Rust](https://docs.rs/borsh) and
...
stage of the transaction pipeline. For details on how the runtime processes each
instruction, see the
