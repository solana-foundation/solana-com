...
related:
  - /docs/core/instructions/advanced-instructions
...
<Callout type="info" title="Summary">
  ...
  (opaque byte array). Instructions are compiled into index-based form inside
  transactions.
</Callout>
...
[Execute instructions](/docs/core/transactions/transaction-pipeline#7-execute-instructions)
stage of the transaction pipeline. For details on how the runtime processes
each instruction, see the
[transaction pipeline](/docs/core/transactions/transaction-pipeline#instruction-processing-details).
