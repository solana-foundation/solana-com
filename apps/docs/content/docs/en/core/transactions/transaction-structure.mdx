...
values. Each `Signature` is 64 bytes, produced by Ed25519-signing the serialized
`Message` with a signer account's private key. One signature is required for
every [signer account](#account-addresses) referenced by the transaction's
instructions.

The first signature in the array belongs to the **fee payer** -- the account
that pays the transaction's [base fee](/docs/core/fees/fee-structure#base-fee).
This first signature also serves as the **transaction ID**, used to look up the
...
public key at index `i` in the `account_keys` array, over the serialized message
bytes. The maximum number of signatures per transaction is 12
...
[durable nonce transaction](/docs/core/transactions/durable-nonces).
...
For a single-signer transaction with 3 accounts and 1 instruction with 12 bytes
of data:
