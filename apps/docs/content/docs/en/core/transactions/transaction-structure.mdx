...
<Callout type="info" title="Summary">
  ...
  size: 1,232 bytes (4,096 bytes for the upcoming v1 format).
</Callout>
...
<Callout type="info" title="v1 transactions raise the limit to 4,096 bytes">
  ...
  follows apart from its version prefix and address lookup tables. The upcoming
  [v1 format](/docs/core/transactions/versioned-transactions#v1-message-format)
  raises the size limit to 4,096 bytes and uses a different wire layout.
</Callout>
