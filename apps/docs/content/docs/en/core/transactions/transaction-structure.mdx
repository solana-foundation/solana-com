...
the transaction arrives, it is rejected with _rs`BlockhashNotFound`_, unless it is a
valid [durable nonce transaction](/docs/core/transactions/durable-nonces).
...
Given _rs`PACKET_DATA_SIZE`_ = 1,232 bytes, the available space can be calculated:
