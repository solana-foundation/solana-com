...
-- a HashMap keyed by blockhash, with a monotonically increasing
`last_hash_index` tracking the most recent entry. Each new slot, the bank
registers the slot's blockhash with `register_hash()`. The queue retains up to
