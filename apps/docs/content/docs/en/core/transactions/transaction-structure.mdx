...
<Callout type="info" title="Summary">
  ...
  size: 1,232 bytes (4,096 bytes for the v1 format).
</Callout>
...
<Callout type="info" title="v1 transactions raise the limit to 4,096 bytes">
  This page describes the legacy transaction format, which the v0 format also
  follows apart from its version prefix and address lookup tables. The [v1
  format](/docs/core/transactions/versioned-transactions#v1-format) raises the
  size limit to 4,096 bytes and uses a different wire layout.
</Callout>
...
<Callout type="info" title="Legacy and versioned message prefixes">
  ...
  the full v0 and v1 message layouts.
</Callout>
...
console.log("Sender prebalance:", Number(preBalance1) / LAMPORTS_PER_SOL);
console.log("Recipient prebalance:", Number(preBalance2) / LAMPORTS_PER_SOL);
console.log("Sender postbalance:", Number(postBalance1) / LAMPORTS_PER_SOL);
console.log("Recipient postbalance:", Number(postBalance2) / LAMPORTS_PER_SOL);
