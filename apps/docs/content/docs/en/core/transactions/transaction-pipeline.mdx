...
The [v1 format](/docs/core/transactions/versioned-transactions#v1-format) raises
the limit to 4,096 bytes. QUIC does not cap stream size, so a v1 transaction
that exceeds the MTU is split across multiple QUIC frames on ingestion rather
than being rejected.
...
For [v1 transactions](/docs/core/transactions/versioned-transactions#v1-format)
...
<Callout type="warn" title="v1 transactions default this limit to zero">
  ...
  format](/docs/core/transactions/versioned-transactions#v1-format) the loaded
  accounts data size limit is read from the message config, and a field left
  unset defaults to **0 bytes** rather than to a generous default. A v1
