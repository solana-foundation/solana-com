...
raises the limit to 4,096 bytes. QUIC does not cap stream size, so a v1
transaction that exceeds the MTU is split across multiple QUIC frames on
ingestion rather than being rejected.
...
there are no compute budget instructions to parse: the limits and priority fee
come from the message config, and any ComputeBudget instructions in the message
are ignored for configuration. Config fields left unset default to zero, which
surfaces later at [account loading](#6-load-accounts).
...
<Callout type="warn" title="v1 transactions default this limit to zero">
  The 64 MiB default applies to legacy and v0 transactions. In the [v1
  format](/docs/core/transactions/versioned-transactions#v1-message-format) the
  loaded accounts data size limit is read from the message config, and a field
  left unset defaults to **0 bytes** rather than to a generous default. A v1
  transaction with an empty config therefore fails at this stage with
  _rs`MaxLoadedAccountsDataSizeExceeded`_ — the requested budget is too small
  for the first account loaded, so the failure lands before any instruction
  runs, with an error that mentions neither v1 nor the config. Always set the
  compute unit limit and the loaded accounts data size limit explicitly when
  building v1 transactions.
</Callout>
...
| [`MaxLoadedAccountsDataSizeExceeded`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/transaction-error/src/lib.rs#L122)     | Account loading        | Total loaded data exceeds the transaction's loaded accounts data size limit (64 MiB by default; 0 in v1 unless set in the message config) |
...
| [`BailOut`](https://github.com/anza-xyz/solana-sdk/blob/master/transaction-error/src/lib.rs)                                            | Block production       | Block production bailed out; transaction is discarded to protect the leader and is explicitly excluded from consensus (does not propagate to followers) |
