...
<Callout type="info" title="Summary">
  `@solana/transaction-introspection` turns a confirmed-transaction RPC
  response into a list of instructions with real addresses. It works with
  `getTransaction`, and — since `@solana/kit` v7.1.0 — also with
  `getTransactionsForAddress` results and `getBlock` transactions (with
  `transactionDetails: 'full'`). It resolves account indices (including
  lookup-table addresses), decodes inner CPI instructions, and walks the full
  instruction tree in explorer order.
</Callout>
...
<Callout type="info" title="Preparing for v1 transactions">
  The [v1 format](/docs/core/transactions/versioned-transactions#v1-format) is
  active, so pass `maxSupportedTransactionVersion: 1` or the fetch fails on v1
  transactions. `decodeTransactionFromRpcResponse` handles `legacy`, `0`, and
  `1` alike (from `@solana/kit` 7.0). Note that a v1 message carries its
  resource limits in a `transactionConfig` object rather than in ComputeBudget
  instructions, so code that recovers compute limits or priority fees by
  scanning the decoded instructions finds nothing for v1.
</Callout>
...
<Callout type="info" title="Also works with `getTransactionsForAddress` and `getBlock`">
  Since `@solana/kit` v7.1.0, `decodeTransactionFromRpcResponse` reads only the
  shared `transaction` / `meta` / `version` envelope, so it accepts elements of
  a `getTransactionsForAddress` `data` array and elements of a `getBlock`
  `transactions` array (with `transactionDetails: 'full'`) identically — just
  `map` over them. Note that the `'json'` overload's `transaction` is typed as
  `never`: the JSON path decompiles the message server-side, so there are no
  re-encodable wire bytes to return.
</Callout>
