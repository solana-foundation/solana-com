...
- to have a self-administered gateway to the Solana mainnet cluster to get data
  and submit withdrawal transactions
...
1. [Install the Solana command-line tool suite](/docs/intro/installation)
...
[Current parameters for Mainnet](https://docs.anza.xyz/clusters/available#example-solana-validator-command-line-2)
...
Solana accounts do not require any onchain initialization; once they contain
...
[rent](/docs/core/accounts/account-structure) in SOL. In order to find the
minimum rent-exempt balance for your deposit accounts, query the
...
- `{"maxSupportedTransactionVersion": 1}`
  ...
  transaction version is `1`, introduced by the
  [v1 format](/docs/core/transactions/versioned-transactions#v1-format) — set
  the value to the integer `1`. A single unsupported-version transaction fails
  the **entire** `getBlock` response, and `getSignaturesForAddress` never fails
  on version — so without the opt-in, v1 deposits appear in signature listings
  but cannot be fetched.

  Two more changes matter for deposit pipelines: v1 transactions carry their
  compute and fee limits in a `transactionConfig` message object rather than
  ComputeBudget instructions (any pipeline scanning instructions for fees
  reports zero for v1), and v1 does not use address lookup tables.

It's important to understand that versioned transactions allow users to create
transactions that use another set of account keys loaded from onchain address
lookup tables.
...
The
[Solana System Program SDK](https://www.npmjs.com/package/@solana-program/system)
(`@solana-program/system`) and
[Solana Javascript SDK](https://github.com/solana-foundation/solana-web3.js)
(`@solana/web3.js`) offers a similar approach for the JS ecosystem. Use the
`getTransferSolInstruction` or `SystemProgram` to build a transfer transaction,
and submit it using the `sendAndConfirmTransaction` method.
...
[recent blockhash](/docs/core/transactions/transaction-structure#recent-blockhash)
...
First, get a recent blockhash and its last valid block height using the
[`getLatestBlockhash`](/docs/rpc/http/getlatestblockhash) endpoint:
...
You can check whether a particular blockhash is still valid by sending an
[`isBlockhashValid`](/docs/rpc/http/isblockhashvalid) request with the blockhash
as a parameter. If the response value is `false`, the blockhash is expired, and
...
[base Transaction Fee](/docs/core/fees/fee-structure#base-fee) to ensure
...
> [guide on how to use priority fees](/docs/core/fees) which includes more
> information about implementing priority fees.
...
[using durable nonces and priority fees together](/docs/core/transactions/durable-nonces)
...
This prints the installed version, for example:

```text
spl-token-cli 5.6.1
```

Use version 5.6.1 or newer if you need SPL Token-2022 support.
...
1. SPL Token accounts must remain
   [rent-exempt](/docs/core/accounts/account-structure) for the duration of
...
Confidential Balances, fees on transfer, closing mints, metadata, permanent
...
Mints may be configured with Confidential Balances, so that token amounts are
...
Balances on token accounts, so exchanges can force users to send tokens
non-confidentially.

To enable Confidential Balances, configure the account:
...
[clusters](/docs/references/clusters) before moving to production on mainnet.
