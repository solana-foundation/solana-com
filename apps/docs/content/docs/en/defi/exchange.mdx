...
- `{"maxSupportedTransactionVersion": 1}`
  ...
  transaction version is `0`, with the
  [v1 format](/docs/core/transactions/versioned-transactions#v1-message-format)
  activating with Agave v4.2 — set the value to the integer `1` now to stay
  ahead of activation (requesting a higher version than a node supports is
  safe). A single unsupported-version transaction fails the **entire**
  `getBlock` response, and `getSignaturesForAddress` never fails on version — so
  without the opt-in, v1 deposits appear in signature listings but cannot be
  fetched.

  Once v1 activates, two more changes matter for deposit pipelines: v1
  transactions carry their compute and fee limits in a `transactionConfig`
  message object rather than ComputeBudget instructions (any pipeline scanning
  instructions for fees reports zero for v1), and v1 does not use address lookup
  tables.
