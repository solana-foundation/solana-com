...
<Callout type="caution" title="Do not use public RPC for production">
  The public endpoints (`api.mainnet.solana.com`, `api.devnet.solana.com`) are
  rate-limited, have no SLA, and are unsuitable for production payment flows.
  Use an [RPC provider](https://solana.com/rpc) to secure a private RPC endpoint
...
> A recent blockhash is valid through the `lastValidBlockHeight` returned with
> it. Fetch a fresh one immediately before signing and sending, and use that
> block height instead of a wall-clock timeout to detect expiration.
