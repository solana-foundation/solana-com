...
- Set `startTs` to `0` (Rust: `start_ts = 0`) to start the delegation when the
  transaction lands on chain instead of at a fixed time. This widens the window
  to get the user to sign and onboard. When you do this, `expiryTs` must be
  non-zero — a start-on-landing delegation cannot also be set to never expire.
