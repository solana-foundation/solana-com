...
- `update_plan` takes the `event_authority` and `self_program` accounts so it
  can emit a `PlanUpdatedEvent`. The TypeScript overlay appends them
  automatically; in Rust, pass `event_authority` (the `self_program` account
