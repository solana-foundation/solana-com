...
- `FixedTransferEvent`, `RecurringTransferEvent`, and
  `SubscriptionTransferEvent` — one per delegated transfer.
...
**Breaking (event wire format):** transfer events now include
`receiverTokenAccount`, `SubscriptionCreatedEvent` includes `payer`, and
`SubscriptionTransferEvent` includes `puller`. The fields are appended (existing
offsets unchanged) but the total event size differs — update any custom log
decoders to read the added fields.
