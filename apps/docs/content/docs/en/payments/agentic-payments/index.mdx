...
If you want a local CLI for agentic payment flows, `pay` can wrap tools like
`curl`, `codex`, and `claude` so they can satisfy `402 Payment Required`
challenges with a local wallet approval flow.

<Cards>
  <Card
    title="pay"
    href="/docs/payments/agentic-payments/pay"
    description="Set up the pay CLI and use it with HTTP clients and coding agents."
  />
</Cards>
