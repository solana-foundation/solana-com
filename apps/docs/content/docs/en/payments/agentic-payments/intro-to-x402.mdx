...
app.get("/premium", async (req, res) => {
  ...
  if (xPaymentHeader) {
    try {
      ...
      // @solana/web3.js v1.99.0+ can deserialize v1 transactions, so a
      // facilitator that must settle v1 payments can set
      // maxSupportedTransactionVersion to 1 once the v1 format is active.
