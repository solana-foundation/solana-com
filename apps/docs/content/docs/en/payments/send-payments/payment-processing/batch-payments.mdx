...
raises the size limit to 4096 bytes, which fits roughly three times as many
transfers per transaction. Batching is still required beyond that, since the
runtime loads at most 64 accounts per transaction regardless of format.
