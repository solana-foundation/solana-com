...
Sending a token payment on Solana using `@solana/client` requires two steps:

1. Configure the `splToken()` helper with the mint address
2. Send the payment by calling `sendTransfer()`
