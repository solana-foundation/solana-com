...
flowchart TD
    ...
    D -- "Yes" --> V2["VERIFY: Confirm the recipient intends to use a smart contract wallet"]
    ...
    K -- "No" --> V1["VERIFY: Confirm the recipient intends to use a smart contract wallet"]
```
...
- **Off-curve (PDA)**: Require explicit recipient verification before creating
  ...
  recipient's program can access its ATA. Confirm the recipient's wallet or
  program supports the PDA and that they intend to receive this mint. A small
  token transfer only proves that the ATA was created; it does not prove the
  recipient can withdraw from it.
...
- **Other Program**: Programs like multisig wallets may be acceptable owners of
...
`USER_VERIFICATION` result is not automatically accepted; your application must
complete its verification flow before sending SPL tokens. A token account
returns `IS_TOKEN_ACCOUNT`, so it can receive SPL tokens but **not** native SOL
— the distinction that prevents SOL from being locked.
...
export async function validateAddress(
  ...
  if (!account.exists) {
    ...
    if (isOffCurveAddress(inputAddress)) {
      return {
        ...
        reason: "Unfunded PDA: confirm the recipient controls this address"
      };
    }
  ...
  return {
    type: "USER_VERIFICATION",
    reason: "Program-owned account: confirm the recipient intends to use it"
  };
}
