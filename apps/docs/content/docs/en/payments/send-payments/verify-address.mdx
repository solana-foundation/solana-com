...
verification ensures you and your users only send to addresses that can properly
receive and access the funds. If you operate an exchange, it is recommended to
run these checks and have your user verify the address before sending.
...
Smart contract wallets have off-curve addresses.
...
After deriving the ATA address, check whether the token account exists onchain.
...
System accounts can exist off-curve. If your user wants to send tokens to an
off-curve address, it is recommended to prompt the user to verify they intend to
send funds to a smart contract wallet.
...
Accounts owned by other programs are valid but are governed by the authority
rules set forth by the respective owning program. It is recommended to prompt
the user to verify they intend to send funds to a smart contract wallet.
...
flowchart TD
    ...
    D -- "Yes" --> V1["VERIFY: Confirm the recipient intends to use a smart contract wallet"]
    ...
    H -- "Yes" --> V3["VERIFY: Confirm the recipient intends to use a smart contract wallet"]

    F -- "Other Program" --> K{"Allowed by policy?"}
    K -- "Yes" --> W3["IS_WALLET: Derive ATA and send"]
    K -- "No" --> V2["VERIFY: Confirm the recipient intends to use a smart contract wallet"]
```
...
- **Off-curve (PDA)**: Require explicit recipient verification before creating
  an ATA or sending SPL tokens. Without an existing account, you cannot
  determine from the address alone which program derived this PDA or whether the
  recipient's program can access its ATA.
...
- **Other Program**: Programs like multisig wallets are acceptable owners of
  token accounts. Require the user to confirm they intend to send to a smart
  contract wallet.

  Some common smart contract wallets:
...
| `USER_VERIFICATION` | Address requires recipient confirmation | Require explicit review before sending SPL tokens or funding an ATA |
| `REJECT`            | Invalid address                         | Do not send                                                         |

It then maps each result to per-asset acceptability with `canReceiveNativeSol`
(wallets only) and `canReceiveSplToken` (wallets or token accounts). A
`USER_VERIFICATION` result is not automatically accepted; your application
should have a user confirm their intent before sending SPL tokens. A token
...
export async function validateAddress(
  ...
  if (!account.exists) {
    // An unfunded PDA may belong to a smart contract wallet, but the address
    // alone cannot prove that its program can access a newly-created ATA.
    if (isOffCurveAddress(inputAddress)) {
      return {
        type: "USER_VERIFICATION",
        reason:
          "Unfunded PDA: confirm the recipient is using a smart contract wallet and controls this address"
      };
    }
  ...
  // contract wallet. Require verification before automatically sending to the
  // latter.
  if (owner === SYSTEM_PROGRAM) {
    if (isOffCurveAddress(inputAddress)) {
      return {
        type: "USER_VERIFICATION",
        reason:
          "Off-curve System Program account: confirm recipient is using a smart contract wallet"
      };
    }
    return { type: "IS_WALLET" };
  }
  ...
  // An account owned by another program can be a smart contract wallet.
  return {
    type: "USER_VERIFICATION",
    reason:
      "Program-owned account: confirm the recipient is using a smart contract wallet"
  };
}
