...
After fetching an [account](/docs/core/accounts), check its program owner and
account type to determine how to handle the address.
...
requires paying rent for the new token account (currently ~0.002 SOL). Since the
recipient owns the ATA, this SOL cannot be recovered by the sender.
...
receive is owned by a token program, you should verify is a token account (not a
mint account) and matches your expected token mint before sending.

<Callout type="info">
  The Token Programs automatically validates that both accounts are token
  accounts for the same mint. If validation fails, the transaction is rejected
  and no funds are lost.
</Callout>

### Mint Accounts

[Mint accounts](/docs/tokens/basics/create-mint) define the token itself
(supply, decimals, authorities). These are owned by the Token Program but are
**not** valid recipients for token transfers. Sending to a mint address will
fail or result in lost funds.
...
- **Off-curve (PDA)**: Reject the address. Without an existing account, you
  cannot determine which program derived this PDA. Sending tokens could result
  in funds locked in an inaccessible address. Conservatively reject the address
  to avoid sending to an address that may be inaccessible.
...
- **Token Program / Token-2022**: Verify the account is a token account (not a
  mint account) and that it holds the token you intend to send. If valid, send
  tokens directly to this address. If it's a mint account or wrong token,
  reject.
...
The following code shows an example of the address validation logic. This is
reference code for illustration purposes.

The demo below uses three example outcomes:
