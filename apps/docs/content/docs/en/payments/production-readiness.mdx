...
- Blockhashes remain valid through
  [`MAX_PROCESSING_AGE`](/docs/core/constants-reference#transactions), meaning
  ages 0 through 150 inclusive, and usually for about 60 seconds
...
- **Blockhash expiration**: Transactions are valid through the
  `lastValidBlockHeight` returned by `getLatestBlockhash`
