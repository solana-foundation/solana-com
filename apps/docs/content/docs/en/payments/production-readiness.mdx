...
Building locally and testing on devnet are great ways to get started with Solana payments. However, when you're ready to deploy to Mainnet, you need to be aware of the mainnet's nuances.
Devnet forgives mistakes. Mainnet doesn't. This guide covers the differences that matter to ensure your users have a smooth experience.
...
[Public endpoints](https://solana.com/docs/references/clusters#solana-public-rpc-endpoints) (`api.mainnet-beta.solana.com`) are rate-limited with no SLA. They're fine for development but will fail production payment flows—like trying to run a payment processor through a shared API with no uptime guarantee.

<Callout type="caution" title="Never use public RPC for production">
Use a [private RPC provider](https://solana.com/rpc) for reliable, low-latency access.
</Callout>
...
For a full list of RPC providers, see the [RPC Infrastructure Providers](https://solana.com/rpc) guide.
...
[Solana Kit](https://github.com/anza-xyz/kit/tree/main/packages/rpc-transport-http) provides a library for customizing RPC transports which allow you to build your own redundant RPC client. Here's an example of how you might use it to build a redundant RPC client:
...
If you prefer not to build your own routing tools, you can leverage a third party service like [Iron Forge](https://www.ironforge.sanctum.so/) to handle the routing for you.
...
The [`@solana-program/compute-budget`](https://github.com/solana-program/compute-budget) package provides a helper function to easily update or append the compute unit price (in micro-lamports) instruction to a transaction.
...
Most RPC providers offer priority fee APIs:
...
For the full fee mechanics, see [Transaction Fees](/docs/core/fees) and our guide: [How to Add Priority Fees to a Transaction](https://solana.com/developers/cookbook/transactions/add-priority-fees).
...
The Solana JSON RPC API has a [`simulatetransaction`](/docs/rpc/http/simulatetransaction) method that can be used to estimate the compute units required for a transaction, which includes an estimate of the compute units that will be used. The [`@solana-program/compute-budget`](https://github.com/solana-program/compute-budget) package provides a helper function to easily estimate the compute units required for a transaction (which uses the `simulatetransaction` method under the hood).
...
const txWithComputeUnitLimit = updateOrAppendSetComputeUnitLimitInstruction(computeUnitLimit, tx);
```
...
For more on checking transaction status, see [Interacting with Solana](/docs/payments/interacting-with-solana).
...
function handlePaymentError(error: unknown): { message: string; retryable: boolean } {
  ...
  if (isSolanaError(error, SOLANA_ERROR__TRANSACTION_ERROR__INSUFFICIENT_FUNDS_FOR_FEE)) {
...
Users expect to pay in stablecoins, not acquire SOL for network fees. Gasless transactions solve this—similar to how Venmo users don't think about ACH fees. See [Fee Abstraction](/docs/payments/send-payments/payment-processing/fee-abstraction) for complete implementation.
...
For real-time transaction monitoring at scale, see our [Indexing Guide](/docs/payments/accept-payments/indexing).
...
- **Solana CLI version:** Ensure you are using the latest version of the [Solana CLI](https://solana.com/docs/intro/installation). 
...
- **Buffer accounts:** Large programs deploy via buffer accounts. The `solana program deploy` command handles this automatically, but understand that deployment isn't atomic—if interrupted, you may need to recover or close buffer accounts. See [Deploying Programs](https://solana.com/docs/programs/deploying).
...
- **Verification:** [Verify](https://solana.com/docs/programs/verified-builds) your program to ensure that the executable program you deploy to Solana’s network matches the source code in your repository

For complete program deployment guidance, see [Deploying Programs](https://solana.com/docs/programs/deploying).
