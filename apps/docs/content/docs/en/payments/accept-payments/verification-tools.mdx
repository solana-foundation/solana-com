...
description: Verify incoming stablecoin payments and reconcile them with your orders.
---
...
Check your stablecoin balance using the [`getTokenAccountBalance`](/docs/rpc/http/gettokenaccountbalance) RPC method:
...
async function watchPayments(walletAddress: Address, onPayment: (amount: bigint) => void) {
...
Each notification contains a base64 encoded string of the transaction data. Because we know the account we are looking at is a token account, we can decode the data using the `getTokenCodec` method from the `@solana-program/token` package.
...
Solana has RPC methods that allow to to fetch an accounts transaction history ([`getSignaturesForAddress`](/docs/rpc/http/getsignaturesforaddress)) and get the details of a transaction ([`getTransaction`](/docs/rpc/http/gettransaction)). To parse transaction history, we fetch recent signatures for our token account, then retrieve each transaction's pre/post token balances. By comparing our ATA's balance before and after each transaction, we can determine the payment amount and direction (incoming vs outgoing). 

```typescript
async function getRecentPayments(walletAddress: Address, limit = 100): Promise<Payment[]> {
  const signatures = await rpc.getSignaturesForAddress(ata, { limit }).send();
...
For production systems handling high volumes, consider dedicated [indexing solutions](/docs/payments/accept-payments/indexing) that parse individual transfer instructions and provide transaction-level detail.
...
async function getTransactionMemo(signature: Signature): Promise<string | null> {
  const tx = await rpc
