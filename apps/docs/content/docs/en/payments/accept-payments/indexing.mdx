---
title: Indexing
description: Production-grade transaction monitoring for high-volume payment systems
---
...
[Geyser](https://docs.solanalabs.com/validator/geyser) is Solana's plugin interface for streaming real-time account and transaction data directly from validators. Instead of polling RPC, you subscribe to a stream that pushes updates as they're processed—providing **sub-100ms latency** compared to ~200-400ms for WebSocket subscriptions. [Yellowstone gRPC](https://github.com/rpcpool/yellowstone-grpc) is one of the most widely used implementations of the Geyser plugin interface. Yellowstone is a real-time streaming solution for:
...
To use Yellowstone, you will need a gRPC endpoing from an RPC service provider. Some options include:
...
[Carbon](https://github.com/sevenlabs-hq/carbon) is a Rust framework for building production indexers built on top of Yellowstone gRPC. Its pipeline architecture connects data sources to decoders to custom processors:
...
[Yellowstone Vixen](https://github.com/rpcpool/yellowstone-vixen) is an open-source Rust framework for transforming raw Yellowstone events into structured, typed data. It uses a **Parser + Handler** architecture:
...
- [Vixen Streams](https://blog.triton.one/introducing-vixen-streams-real-time-parsed-solana-program-streaming/) - Triton's hosted service built on Vixen

## Getting Started

Several RPC providers offer hosted indexing infrastructure. Check [solana.com/rpc](https://solana.com/rpc) for a current list of providers offering webhooks, enhanced APIs, and gRPC streaming.
