...
Geyser is Solana's plugin interface for streaming real-time account and transaction data directly from validators. Instead of polling RPC, you subscribe to a stream that pushes updates as they're processed—providing **sub-100ms latency** compared to ~200-400ms for WebSocket subscriptions. [Yellowstone gRPC](https://github.com/rpcpool/yellowstone-grpc) is one of the most widely used implementations of the Geyser plugin interface. Yellowstone is a real-time streaming solution for:
