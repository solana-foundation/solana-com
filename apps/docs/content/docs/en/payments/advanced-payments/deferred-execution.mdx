...
network state that proves the transaction was created "now." A recent blockhash
is accepted through
[`MAX_PROCESSING_AGE`](/docs/core/constants-reference#transactions), meaning
ages 0 through 150 inclusive, and usually for about 60 seconds. Use the
`lastValidBlockHeight` returned with the blockhash to determine its exact
expiration. This works perfectly for real-time payments. But it breaks workflows
that need a gap between signing and submission, such as:

| Scenario                   | Why Standard Transactions Fail                                                   |
| -------------------------- | -------------------------------------------------------------------------------- |
| **Treasury operations**    | CFO in Tokyo signs, Controller in NYC approves—the blockhash expires too quickly |
...
In traditional finance, a signed check doesn't expire in about a minute. Certain
