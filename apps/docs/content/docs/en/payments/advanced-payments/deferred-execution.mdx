...
is accepted for up to
[`MAX_PROCESSING_AGE`](/docs/core/constants-reference#transactions), or 150
slots and usually about 60 seconds. Use the `lastValidBlockHeight` returned with
the blockhash to determine its exact expiration. This works perfectly for
