...
network state that proves the transaction was created "now." The network's
[`MAX_PROCESSING_AGE`](/docs/core/constants-reference#transactions) boundary is
inclusive: blockhash ages 0 through 150 are accepted. Do not calculate expiry
from that age, though. The wall-clock duration varies, so use the
`lastValidBlockHeight` returned with the blockhash. This works perfectly for
...
Instead of a short-lived recent blockhash, you use a **nonce account**, a
special account that stores a _unique_ value which can be used in place of a
...
flowchart LR
    S1[Sign] --> S2[Submit]
    S2 -. "Last valid block height reached" .-> S3[Expires]
```
