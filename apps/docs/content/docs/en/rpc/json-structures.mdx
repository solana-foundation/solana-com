...
0-based position of the transaction within its block, matching its index in the
`transactions` array of [Confirmed Blocks](#confirmed-blocks). Omitted when the
RPC node does not report it (Agave added this field in v4.0).
...
Program log output captured during simulation. This field is `null` if the
simulation failed before execution began; otherwise, it is an array that may be
empty.
...
v1 messages additionally carry a `transactionConfig` object holding the
transaction's resource limits (requests must set
`maxSupportedTransactionVersion: 1` to receive v1 transactions):
...
- `transactionConfig` is present only for v1 messages — it is absent entirely
  (never `null`) for legacy and v0.
- Unset inner fields are explicit `null`, not omitted.
- `priorityFee` is a total in lamports, not micro-lamports per compute unit.
- `addressTableLookups` is omitted for v1 messages, which do not support address
  lookup tables.
...
is currently emitted for v0 messages even when the array is empty, and is
omitted for v1 messages, which do not support address lookup tables. Uses the
...
The compute budget a v1 message declares inline, in place of the ComputeBudget
program instructions used by legacy and v0 messages. Present only for v1
messages, and absent entirely — never `null` — for legacy and v0. Each inner
field is an explicit `null` when the transaction did not request that limit.

##### !! computeUnitLimit

!type u32 | null

The maximum number of compute units the transaction may consume. A v1
transaction that requests no limit is budgeted zero compute units and fails at
execution.
...
The heap frame size in bytes requested for execution. When `null`, the runtime
allocated the default 32,768 bytes.

##### !! loadedAccountsDataSizeLimit

!type u32 | null

The maximum size in bytes of account data the transaction may load. When `null`,
the transaction was budgeted zero bytes.

##### !! priorityFee

!type u64 | null

The total priority fee in lamports paid for prioritization, for the whole
transaction rather than a price per compute unit.
