...
is active, v1 messages additionally carry a `transactionConfig` object holding
the transaction's resource limits (requests must set
...
Reward category. When present, the value is `Fee`, `Rent`, `Voting`,
`Staking`, or `VAT`.
