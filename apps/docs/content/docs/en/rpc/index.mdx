...
<Callout type="info">
  ...
  production applications. Follow the [production readiness
  guide](/docs/payments/production-readiness#rpc-infrastructure) when choosing
  dedicated or private RPC infrastructure. Shared public endpoints may return
  `429` when you exceed rate limits and `403` when traffic is blocked.
</Callout>
