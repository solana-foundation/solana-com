...
Commitment level to use for preflight when `skipPreflight` is `false`. See
[Configuring State Commitment](/docs/rpc#configuring-state-commitment).
Default `finalized`. When `skipPreflight` is `true`, this field is ignored and
Agave uses `processed` internally.
