...
Returns the fee, in lamports, that the cluster would charge for a particular
serialized message.
...
Fee corresponding to the message at the specified blockhash, or `null` if the
referenced blockhash is no longer valid
