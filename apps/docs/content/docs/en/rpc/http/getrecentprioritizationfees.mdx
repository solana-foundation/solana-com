...
Optional array of account addresses (up to a maximum of 128 addresses), as
base-58 encoded strings

<Callout type="info">
  If this parameter is provided, the response reflects the fee required to land
  a transaction that locks all of the provided accounts as writable.
</Callout>

If you omit `params` entirely, the RPC returns the recent prioritization-fee
samples without filtering by writable account locks.
