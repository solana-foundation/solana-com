...
- `addresses` (array) - accounts to return, as base-58 encoded strings. The
  list length must not exceed the number of account keys in the transaction.
- `encoding` (string) - encoding for returned Account data (default: `base64`).
  Values: `base64`, `base64+zstd`, `jsonParsed`. Note: `base58` is **not
  supported** for this field. `jsonParsed` encoding attempts to use
  program-specific state parsers to return more human-readable and explicit
  account state data. If `jsonParsed` is requested but a parser cannot be
  found, the field falls back to `[data, "base64"]`.
...
- An object containing:
  ...
  - `data: <[string, encoding]|object>` - data associated with the account,
    either as encoded binary data or a parsed object with `program`, `parsed`,
    and `space`
  ...
  - `space: <u64|null>` - account data size in bytes. This may be null if the
    RPC response omits the size for the selected encoding
...
Transaction addresses loaded from address lookup tables, containing:

- `writable: <[string]>` - Ordered list of base-58 encoded addresses for
  writable loaded accounts
- `readonly: <[string]>` - Ordered list of base-58 encoded addresses for
