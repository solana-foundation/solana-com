...
- `encoding`: Encoding for returned Account data (default: `base64`).
  ...
  found, the field falls back to `base64` encoding, detectable when the
  returned `accounts.data` field is type `string`.
...
transaction was processed. Omitted if token balance recording is not yet
enabled.
