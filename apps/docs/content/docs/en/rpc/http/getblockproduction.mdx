...
Returns recent block-production counts for validator identities in the current
or previous epoch.
...
| `lastSlot`  | `u64` | Last slot for which to return block-production information, inclusive. If omitted, Agave uses the highest available slot in the selected range. |
...
Each tuple value is ordered as:
