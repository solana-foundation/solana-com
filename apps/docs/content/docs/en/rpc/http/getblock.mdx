...
`{program, programId, parsed, stackHeight}` when the RPC node can parse the
...
The highest transaction version to return. Set it to `0` to fetch legacy and v0
transactions. Set it to `1` to also fetch
[v1 transactions](/docs/core/transactions/versioned-transactions#v1-message-format)
once the v1 format is active — the value must be the JSON integer `1`, not the
string `"1"`. Requesting a version higher than a node supports is safe; the
parameter is a ceiling, not a declaration.

A single transaction above the requested version fails the **entire** call with
error `-32015` — there is no partial result. Requests with `transactionDetails`
of `signatures` or `none` never fail on version; `accounts` and `full` can.

If you omit this parameter, only legacy transactions are allowed — any block
containing a versioned transaction results in an error.
...
[Confirmed Blocks](/docs/rpc/json-structures#confirmed-blocks) structure.
...
| `preTokenBalances`  | `array \| null`            | Token balances before execution. Each element uses [Token Balances](/docs/rpc/json-structures#token-balances).    |
| `postTokenBalances` | `array \| null`            | Token balances after execution. Each element uses [Token Balances](/docs/rpc/json-structures#token-balances).     |
| `rewards`           | `array \| null`            | Rewards applied while processing the transaction. Each element uses [Rewards](/docs/rpc/json-structures#rewards). |
...
This field uses the shared [Rewards](/docs/rpc/json-structures#rewards)
structure.
...
| `rewardType`  | `string \| null` | Reward category. When present, the value is `fee`, `rent`, `voting`, `staking`, or `VAT`.       |
