...
Returns block metadata and transaction data for a confirmed block in the
ledger.
...
- `version: <"legacy"|number>` - Transaction version. This field is omitted if
...
- `postBalance: <u64>` - Account balance in lamports after the reward was
  applied
- `rewardType: <string|null>` - Reward category. When present, the value is
  `"fee"`, `"rent"`, `"voting"`, or `"staking"`.
