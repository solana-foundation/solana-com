...
instruction, or `{programId, accounts, data, stackHeight}` when it falls back
to a partially decoded form. The nested `parsed` payload remains
program-specific.
...
| `meta` | `object \| null` | Transaction status metadata, or `null` if metadata is unavailable. For `transactionDetails: "accounts"`, Agave returns the simplified metadata object documented below. |
