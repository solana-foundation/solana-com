...
| `meta`        | `object \| null`                         | Transaction status metadata, or `null` if metadata is unavailable. For `transactionDetails: "full"`, this uses [Transaction Status Metadata](/docs/rpc/json-structures#transaction-status-metadata). For `transactionDetails: "accounts"`, Agave returns a simplified subset of that structure.                                               |
