...
Encoding for the returned Transaction. See
...
| `preTokenBalances` | `array \| null` | Token balances before execution. Each element is a [token balance](/docs/rpc/json-structures#token-balances) object. |
| `postTokenBalances` | `array \| null` | Token balances after execution. Each element is a [token balance](/docs/rpc/json-structures#token-balances) object. |
| `rewards` | `array \| null` | Rewards applied while processing the transaction. Each element is a [reward](/docs/rpc/json-structures#rewards) object. |
...
[Transaction](/docs/rpc/json-structures#transactions) payload. The shape
...
`accountKeys` is an array of account descriptor objects and `instructions` is
an array of parsed or partially decoded instruction objects. The exact parsed
instruction schema depends on the program and is documented in
[Parsed Responses](/docs/rpc#parsed-responses) and
[JSON Structures](/docs/rpc/json-structures).
