...
| `transactionConfig`   | `object`        | Inline compute budget declared by a v1 message. Present only for v1 transactions.        |

**`message.transactionConfig` fields**

Present only when the transaction is v1, and absent entirely — never `null` —
for legacy and v0. Each field is an explicit `null` when the transaction did not
request that limit.

| Field                         | Type          | Description                                                                                                           |
| ----------------------------- | ------------- | --------------------------------------------------------------------------------------------------------------------- |
| `computeUnitLimit`            | `u32 \| null` | Maximum compute units the transaction may consume. A v1 transaction that requests no limit is budgeted zero.          |
| `heapSize`                    | `u32 \| null` | Heap frame size in bytes requested for execution. When `null`, the runtime allocated the default 32,768 bytes.        |
| `loadedAccountsDataSizeLimit` | `u32 \| null` | Maximum size in bytes of account data the transaction may load. When `null`, the transaction was budgeted zero bytes. |
| `priorityFee`                 | `u64 \| null` | Total priority fee in lamports for the whole transaction, not a price per compute unit.                               |
...
Position of the transaction within its block: a 0-based index into the ordered
list of all transactions in the slot, including vote transactions. This matches
the transaction's position in the `transactions` array returned by
[`getBlock`](/docs/rpc/http/getblock). Omitted when the RPC node does not report
it (Agave added this field in v4.0).
