...
Returns the node's software version and optional FeatureSet identifier.
...
First 4 bytes of the current software's FeatureSet identifier. This may be
`null` if the node does not report a FeatureSet.
