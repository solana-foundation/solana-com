...
each containing an array of vote account objects.

##### !! current

!type array

Vote accounts that are considered current for the evaluated bank.

##### !! delinquent

!type array

Vote accounts that are considered delinquent for the evaluated bank.

Each element of `current` and `delinquent` is a JSON object with the following
fields:
...
Vote commission expressed as an 8-bit fraction used for rewards payout
