...
| Field | Type | Description |
| --- | --- | --- |
| `data` | `[string, encoding] \| object` | Account data. Binary forms depend on the `encoding` parameter. When `encoding` is `jsonParsed`, this field is `{program, parsed, space}`; if no parser is available, Agave falls back to `[data, "base64"]`. |
