...
!type RpcResponse&lt;RpcBlockUpdate&gt;

Standard PubSub `RpcResponse` wrapper. For notifications, `context` includes
`slot` and omits `apiVersion`.
...
externally tagged enum such as `{ "UnsupportedTransactionVersion": 0 }` when the
block contains a transaction version above `maxSupportedTransactionVersion`.
