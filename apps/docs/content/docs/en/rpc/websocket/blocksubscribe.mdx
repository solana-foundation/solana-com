...
This field accepts any `u8`. Versioned transactions exposed by RPC use version
`0`, and version `1` once the
[v1 format](/docs/core/transactions/versioned-transactions#v1-message-format) is
active — set this to `1` to keep the subscription working across v1 activation.

On a version error the notification arrives success-shaped with `block: null`
and a populated `err`, and the subscription does **not** advance past that slot
— it retries it indefinitely. Treat `block: null` with an error as a failure,
never as an empty block.
