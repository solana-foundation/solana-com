...
The example below shows a `frozen` notification captured from Agave. Other
variants use the field matrix above.
...
Notification wrapper with the serialized `SlotUpdate` payload and the
subscription id.
...
Tagged enum payload with a `type` discriminator. The fields present depend on
the variant listed in the table above.
