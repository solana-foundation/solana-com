...
Only the mint authority can mint new tokens. The destination token account
must already exist and must belong to the same mint.
...
The examples below use _rs`MintToChecked`_, which requires the caller to provide the
