...
Thawing a token account uses the Token Program's _rs`ThawAccount`_
instruction.

The _rs`ThawAccount`_ instruction verifies that the token account belongs to
the mint and that the signer is the mint's freeze authority, then changes the
token account state from frozen back to initialized.
