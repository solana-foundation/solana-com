...
Approving a delegate allows another address to transfer or burn a limited
amount of tokens from a token account on behalf of the token account owner.
...
The examples below use _rs`ApproveChecked`_, which requires the caller to provide the
mint and decimals so the instruction can verify the expected mint and token
precision before processing the approval.
