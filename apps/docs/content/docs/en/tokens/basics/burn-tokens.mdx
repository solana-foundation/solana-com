...
The examples below use _rs`BurnChecked`_, which requires the caller to provide the
mint's decimals so the instruction can verify the expected token precision
before burning.

The native mint does not support burning. Use _rs`CloseAccount`_ instead.

The token account owner or an approved delegate signs the transaction. In the Token
Extension Program, a mint permanent delegate can also authorize the burn if the
mint has the permanent delegate extension enabled.
