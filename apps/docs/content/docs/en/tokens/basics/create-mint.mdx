...
A mint account uniquely represents a token on Solana and stores its global
metadata.
...
Every token on Solana has a mint account, and the mint's address serves as the
...
For new code, use `@solana/kit`. The `Kit Plugin` example below shows the recommended approach. `Legacy` examples using `@solana/web3.js` are included for reference.
...
// Connect to a local validator with a generated, funded payer
...
const transactionResult = await client.sendTransaction([
