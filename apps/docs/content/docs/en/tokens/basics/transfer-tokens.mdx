...
Transferring tokens moves tokens from one token account to another token
account for the same mint.

Transfers do not change the mint supply. Transfers only update balances
...
The examples below use _rs`TransferChecked`_, which requires the caller to provide the
mint and decimals so the instruction can verify the expected mint and token
precision before moving tokens between accounts.
