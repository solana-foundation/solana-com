...
async def main():
    ...
    async with rpc:
        ...
        source_token_account = await token.create_associated_token_account(fee_payer.pubkey())
        destination_token_account = await token.create_associated_token_account(receiver.pubkey())
        await token.mint_to_checked(source_token_account, fee_payer, AMOUNT_TO_MINT, DECIMALS)
            ...
            instructions=[
                ...
                transfer_checked(
                    TransferCheckedParams(
                        ...
                        mint=mint_address,  # Mint for the token being transferred.
