...
account. Once frozen, the account cannot send tokens or receive tokens until it
is thawed. A frozen account can still be closed if it has a zero token balance.
Only the freeze authority of the token's mint can freeze accounts. If the freeze
authority is revoked (set to null) on the mint account, tokens can never be
frozen.
