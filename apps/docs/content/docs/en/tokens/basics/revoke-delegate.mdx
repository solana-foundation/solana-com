...
The _rs`Revoke`_ instruction clears the delegate fields on the source token account. In the
Token Program, the token account owner signs the revocation. In the Token
Extension Program, the current delegate can also revoke the delegation.
