...
Creating an ATA requires a refundable minimum balance for the account's onchain
storage, plus the transaction fee. The `payer` passed to the create instruction
funds the storage balance and must sign. The payer can be different from both
the transaction fee payer and the wallet that will own the ATA, so an
application can create an ATA on behalf of a recipient.

At the current cluster storage rate, the base costs are:

| Token program                             | ATA size  | Minimum storage balance             |
| ----------------------------------------- | --------- | ----------------------------------- |
| Original Token Program                    | 165 bytes | 2,039,280 lamports (0.00203928 SOL) |
| Token-2022 with automatic immutable owner | 170 bytes | 2,074,080 lamports (0.00207408 SOL) |

Token-2022 mints can require account extensions in addition to the automatic
_rs`ImmutableOwner`_ extension. Those ATAs are larger and require a higher
minimum balance. The Associated Token Program asks the selected Token Program
for the required account size and funds the ATA accordingly.

Do not hard-code these balances in application logic. Query
[`getMinimumBalanceForRentExemption`](/docs/rpc/http/getminimumbalanceforrentexemption)
for the required data size, or let the Associated Token Program calculate and
fund it during creation.

<Callout type="info" title="The storage balance is refundable">
  Closing an empty token account transfers all of its lamports, including the
  minimum storage balance, to the destination specified by the close
  instruction. The original creation payer is not automatically the refund
  destination. See [Close Token Account](/docs/tokens/basics/close-account).
</Callout>
