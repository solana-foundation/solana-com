        ...
        message = MessageV0.try_compile(
            ...
            instructions=[
                ...
                create_associated_token_account(
                    ...
                    mint=mint_address,  # Mint for the token this account holds.
...
async def main():
    ...
    async with rpc:
        ...
        airdrop_signature = (await rpc.request_airdrop(fee_payer.pubkey(), 1_000_000_000)).value
        await rpc.confirm_transaction(airdrop_signature)
        token = await AsyncToken.create_mint(rpc, fee_payer, fee_payer.pubkey(), 2, TOKEN_PROGRAM_ID, fee_payer.pubkey())
        mint_address = token.pubkey
        ...
        transaction = VersionedTransaction(message, [fee_payer, token_account])
        result = await rpc.send_transaction(transaction)
        await rpc.confirm_transaction(
            result.value,
            last_valid_block_height=latest_blockhash.value.last_valid_block_height,
