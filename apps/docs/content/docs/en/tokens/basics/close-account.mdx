...
Closing a token account deletes the account and returns the token account's
rent lamports to a destination account.
...
Closing a token account uses the Token Program's _rs`CloseAccount`_
instruction.
