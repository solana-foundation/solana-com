...
Closing a token account deletes the account and transfers all of its lamports,
including its refundable minimum storage balance, to a destination account.
...
signs the close instruction and selects the destination. The account that paid
to create an ATA does not automatically receive the recovered balance.
