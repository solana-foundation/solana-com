...
Wrapped SOL (WSOL) is SOL held in a token account for the Token Program's
native mint. The native mint is the mint address the Token Program uses for
token accounts that wrap SOL. The native mint enables SOL to be used through the Token
...
tracks the SOL held in the wrapped SOL token account as a token balance
through the token account _rs`amount`_ field. If you transfer SOL into a
wrapped SOL token account, the lamport balance increases immediately, but the token _rs`amount`_ field does not change until
you call the Token Program's _rs`SyncNative`_ instruction.

## How to Sync Native SOL

Use the Token Program's _rs`SyncNative`_ instruction after transferring SOL
into a wrapped SOL token account. The _rs`SyncNative`_ instruction updates the token account
_rs`amount`_ field to match the lamports held in the account above its
rent-exempt reserve.
