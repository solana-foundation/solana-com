...
description:
  Learn how to create a token account with Confidential Balances enabled.
---

## How to create a token account with Confidential Balances

Confidential Balances enables private token transfers by adding extra state to
the token account. This section explains how to create a token account with the
required extension enabled.

The following diagram shows the steps involved in creating a token account with
Confidential Balances enabled:
...
### Confidential Balances Token Account State
...
The _rs`ConfidentialTransferAccount`_ contains several fields that manage
Confidential Balances:

- **approved**: The account's approval status for Confidential Balances. If the
  mint account's `auto_approve_new_accounts` configuration is set as `true`, all
  token accounts are automatically approved for Confidential Balances.
...
Creating and configuring a token account for Confidential Balances uses the
...
4. **Configure Confidential Balances**: Invoke the Token Extension Program's
...
**Only the token account owner can enable Confidential Balances on a token
account**.
...
The following code creates an associated token account and configures it to use
Confidential Balances with an existing compatible mint.
