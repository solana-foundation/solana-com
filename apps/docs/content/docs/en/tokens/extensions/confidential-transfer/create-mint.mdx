...
description:
  Learn how to create a token mint with Confidential Balances enabled.
---

## How to create a mint with Confidential Balances

Confidential Balances enables private token transfers by adding extra state to
the mint account. This section explains how to create a token mint with the
...
### Confidential Balances Mint State
...
- **authority**: The account that has permission to change Confidential Balances
...
- **auto_approve_new_accounts**: When set to true, users can create token
  accounts with Confidential Balances enabled by default. When false, the
...
Creating a mint with Confidential Balances enabled requires three instructions
in a single transaction:

1. **Create the Mint Account**: Invoke the System Program's _rs`CreateAccount`_
   instruction to create the mint account.

2. **Initialize Confidential Balances**: Invoke the Token Extension Program's
...
The following code demonstrates how to create a mint with Confidential Balances
enabled.
