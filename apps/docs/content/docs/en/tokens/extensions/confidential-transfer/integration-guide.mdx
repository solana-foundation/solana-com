...
let elgamal_keypair =
    ElGamalKeypair::new_from_signer_legacy(&signer, &token_account_pubkey.to_bytes())
        .expect("Failed to derive ElGamal keypair");
let aes_key = AeKey::new_from_signer_legacy(&signer, &token_account_pubkey.to_bytes())
