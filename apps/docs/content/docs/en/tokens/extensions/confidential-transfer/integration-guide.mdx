...
and account balances encrypted onchain. Supporting it takes more than reading
one extra field. Balances are encrypted, so displaying them requires the owner's
decryption keys, and sending requires generating zero-knowledge proofs on the
client.
...
Identifying the instruction is the easy part. The gaps to plan for on the
indexing side:
