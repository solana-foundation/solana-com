...
These proofs are larger than the legacy and v0 transaction size limit allows
inline, so the usual pattern is to create **proof context state accounts**,
verify each proof into them, reference them from the transfer instruction, then
close them to reclaim rent. That spans a few dependent transactions. The
transaction v1 format raises that limit, which lets a confidential transfer run
in a single onchain transaction and simplifies this flow.
