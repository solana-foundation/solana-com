...
_rs`ConfidentialTransferAccount`_ state and approved to use Confidential
Balances. The sender's token account must also have an available confidential
...
another. Both accounts must already be configured for Confidential Balances, and
the sender must have an available confidential balance.
