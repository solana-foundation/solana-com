...
`@solana/spl-token-metadata` are included for reference. The legacy examples cap
`maxSupportedTransactionVersion` at `0` because web3.js v1 cannot deserialize
...
const { returnData } = (updateMetadataTransaction?.meta ?? {}) as {
  returnData?: { data: [string, string] };
};
const emittedDataBase64 = returnData?.data?.[0];
if (!emittedDataBase64) {
  throw new Error("Expected token metadata return data");
}
const emittedTokenMetadata = unpackTokenMetadata(
