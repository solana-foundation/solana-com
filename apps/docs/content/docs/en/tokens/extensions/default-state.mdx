...
title: Default State
description:
  Configure a Token Extension Program mint so newly created token accounts start
  in a chosen default state, such as frozen.
...
## How to Create a Mint with a Default Account State

To create a mint with a default account state:
...
## !!steps Create a token account

Create a token account for the mint. Because the current default state is
_rs`Frozen`_, the new token account starts frozen.
...
## !!steps Update the default account state

Use _rs`UpdateDefaultAccountState`_ to change the default state so future token
accounts no longer start frozen.
...
| _rs`DefaultAccountStateInstruction::Initialize`_ | Instruction that initializes the mint's default token account state before _rs`InitializeMint`_.    | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L20-L36) |
| _rs`DefaultAccountStateInstruction::Update`_     | Instruction that lets the freeze authority change the default state for future token accounts.      | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L37-L53) |
| _rs`process_initialize_default_account_state`_   | Processor logic that writes the initial default state onto an uninitialized mint.                   | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L29-L40)     |
| _rs`process_update_default_account_state`_       | Processor logic that verifies the freeze authority before changing the mint's stored default state. | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L43-L74)     |
