...
web3.js v1. The v3 helper takes a Kit RPC, so build one for the same endpoint as
the `Connection` with `createSolanaRpc` from `@solana/kit`.
...
The addresses for `mint`, `wsol_mint`, and `associated_token_program` will be
used to derive the addresses for the wSOL Associated Token Accounts. These
