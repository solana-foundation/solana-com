...
assigns a delegate with authority over all token accounts for a mint. Unlike
regular delegates that token account owners can revoke, a permanent delegate
cannot be changed or removed by token account owners. The permanent delegate can
transfer or burn tokens in any token account of that mint. The permanent
delegate role can be changed to a new account by the current permanent delegate.
