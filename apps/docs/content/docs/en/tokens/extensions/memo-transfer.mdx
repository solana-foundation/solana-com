...
title: Memo Transfer
description:
  Require incoming transfers to include a memo by enabling the MemoTransfer
  account extension on a Token Extension Program token account.
...
Outgoing transfers from that token account do not require a memo.
...
the token account owner disables the requirement.
...
_rs`MemoTransfer`_ extension. This is the size used in _rs`CreateAccount`_.
...
Calculate rent using the token account size needed for the _rs`MemoTransfer`_
extension.
...
## !!steps Enable MemoTransfer

Enable _rs`MemoTransfer`_ on the token account.
...
| _rs`RequiredMemoTransfersInstruction::Enable`_  | Instruction that enables memo-required transfers on a token account.                                     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L50-L71) |
| _rs`RequiredMemoTransfersInstruction::Disable`_ | Instruction that disables memo-required transfers on a token account.                                    | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L74-L95) |
| _rs`process_toggle_required_memo_transfers`_    | Processor logic that initializes or rewrites the _rs`MemoTransfer`_ extension after verifying ownership. | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/memo_transfer/processor.rs#L20-L48)     |
