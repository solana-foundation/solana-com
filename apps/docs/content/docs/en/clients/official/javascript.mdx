...
`Keypair`, `Transaction`, `PublicKey` — on top of `@solana/kit` internals. Most
methods that were synchronous in the legacy SDK, such as `Keypair.generate()`
and `transaction.sign()`, are now async.
...
`@solana/web3.js` (legacy, `1.x`) is the original TypeScript SDK for Solana and
is no longer supported. It is in maintenance mode. New apps should use
`@solana/kit` or web3.js v3. Existing codebases should plan a move to
[upgrade to web3.js v3](https://github.com/solana-foundation/solana-web3.js/blob/v3.x/docs/web3js-v1-to-v3-migration.md)
or migrate to Kit; see [Migrating to Kit](/docs/frontend/web3-compat).
