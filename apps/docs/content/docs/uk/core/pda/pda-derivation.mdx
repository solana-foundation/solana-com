...
Bump seed — це один байт (u8), який додається до необов'язкових seeds під час
деривації.
[`find_program_address`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L800)
здійснює пошук від 255 до 1, викликаючи _rs`create_program_address`_ з кожним
значенням, поки результат не опиниться поза кривою Ed25519. Перше значення, яке
спрацює, є **канонічним bump**.
...
[Онлайн-системний виклик](https://github.com/anza-xyz/agave/blob/v3.1.8/syscalls/src/lib.rs#L798-L834)
для _rs`create_program_address`_ стягує
[1 500 обчислювальних одиниць](https://github.com/anza-xyz/agave/blob/v3.1.8/program-runtime/src/execution_budget.rs#L200)
за кожен виклик.
...
Виведення PDA обчислює лише адресу. Воно не створює онлайн-акаунт за цією
адресою. Акаунт має бути явно створений через окрему інструкцію (зазвичай
...
<Callout type="warn">
  Це відрізняється від поточної поведінки канонічного пошуку, задокументованої
  для `sol_try_find_program_address`, яка виконує ітерацію від 255 до 1.
</Callout>
...
У цьому прикладі bump 255 створює адресу на кривій і не проходить. Перший
дійсний bump — це 254, що робить його канонічним bump.
