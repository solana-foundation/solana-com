...
Транзакції мають загальне обмеження розміру в
...
<Callout>
  ...
  без фрагментації, Solana використовує 1280 байтів (мінімальний MTU, необхідний
...
Масив `signatures` транзакції містить структури `Signature`. Кожна
[`Signature`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/signature/src/lib.rs#L30)
має розмір 64 байти і створюється шляхом підписання `Message` транзакції
приватним ключем акаунта. Підпис має бути наданий для кожного
[акаунта підписанта](#account-addresses), включеного в будь-яку з інструкцій
транзакції.
...
- [`recent_blockhash`](mention:recent-blockhash):
  [хеш блоку](#recent-blockhash), який виступає міткою часу для транзакції
- [`instructions`](mention:instructions): масив [інструкцій](#instructions)

<Callout>
  Щоб заощадити місце, транзакція не зберігає дозволи для кожного акаунта
  окремо. Натомість дозволи акаунтів визначаються за допомогою `header` та
  `account_keys`.
</Callout>
...
- [`num_readonly_signed_accounts`](mention:num_readonly_signed_accounts):
  загальна кількість акаунтів лише для читання, які потребують підписів
- [`num_readonly_unsigned_accounts`](mention:num_readonly_unsigned_accounts):
  загальна кількість акаунтів лише для читання, які не потребують підписів
...
[форматі компактного масиву](/docs/references/terminology#compact-array-format).
Префікс масиву вказує його довжину. Кожен елемент масиву — це публічний ключ, що
вказує на обліковий запис, який використовується його інструкціями. Масив
`accounts_keys` має бути повним і суворо впорядкованим наступним чином:
...
<Callout>
  Сувора впорядкованість дозволяє поєднувати масив `account_keys` з інформацією
  в [`header`](#header) повідомлення для визначення дозволів для кожного
  облікового запису.
</Callout>
...
`recent_blockhash` повідомлення — це хеш-значення, яке діє як мітка часу
транзакції і запобігає дублюванню транзакцій. Blockhash втрачає чинність через
[150 блоків](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/clock/src/lib.rs#L134).
(Еквівалентно одній хвилині — припускаючи, що кожен блок триває 400 мс.) Після
закінчення терміну дії блоку транзакція втрачає чинність і не може бути
оброблена.

<Callout>
  RPC-метод [`getLatestBlockhash`](/docs/rpc/http/getlatestblockhash) дозволяє
  отримати поточний blockhash і останню висоту блоку, на якій blockhash буде
  дійсним.
</Callout>
...
повідомлення — це масив усіх інструкцій для обробки, надісланий у
[форматі компактного масиву](/docs/references/terminology#compact-array-format).
Префікс масиву вказує його довжину. Кожен елемент масиву — це структура
[`CompiledInstruction`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/instruction.rs#L22)
і включає наступну інформацію:

<WithMentions>

1. [`program_id_index`](mention:program-id-index): індекс, що вказує на адресу в
   масиві [`account_keys`](#account-addresses). Це значення вказує адресу
