...
складається з наступної інформації:
...
Перший підпис належить обліковому запису, який оплатить
...
- [`recent_blockhash`](mention:recent-blockhash):
  [хеш блоку](#recent-blockhash), що діє як мітка часу для транзакції
...
`accounts_keys` має бути повним і суворо впорядкованим таким чином:
...
`recent_blockhash` повідомлення — це хеш-значення, що діє як мітка часу
транзакції і запобігає дублюванню транзакцій. Блокхеш втрачає чинність через
...
<Callout>
  ...
  отримати поточний блокхеш і останню висоту блоку, на якій блокхеш буде чинним.
</Callout>
...
3. [`data`](mention:instruction-data): байтовий масив, що визначає, яку
   інструкцію викликати в програмі. Він також містить будь-які додаткові дані,
