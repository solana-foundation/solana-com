...
- [`lamports`](mention:lamports): кількість lamports в акаунті
- [`data`](mention:data): дані акаунта
- [`owner`](mention:owner): ID програми, яка володіє акаунтом
- [`executable`](mention:executable): вказує, чи містить акаунт виконуваний
  бінарний код
- [`rent_epoch`](mention:rent_epoch): застаріле поле rent epoch
...
Це поле зазвичай називають "дані акаунта". `data` у цьому полі вважається
довільним, оскільки може містити будь-яку послідовність байтів. Кожна програма
визначає структуру даних, що зберігаються в цьому полі.
...
2. Десеріалізація поля `data` акаунта з необроблених байтів у відповідну
   структуру даних, як визначено програмою, яка володіє акаунтом.
...
або зменшувати кількість lamports, як зазначено в інструкціях програми.
...
На діаграмі нижче ви можете побачити, що програма-завантажувач використовується
...
Програми, розгорнуті за допомогою loader-v3, не містять програмний код у своєму
полі `data`. Натомість їхнє поле `data` вказує на окремий **program data
account**, який містить програмний код. (Див. діаграму нижче.)
...
Приклад нижче отримує обліковий запис Token Program. Зверніть увагу, що поле
`executable` встановлено на `true`, що вказує на те, що обліковий запис є
програмою.
...
Програми використовують акаунти даних для збереження свого стану. Для цього вони
спочатку повинні створити новий акаунт даних. Процес створення акаунта стану
програми часто абстрагований, але корисно розуміти базовий процес.
...
Не всі облікові записи отримують нового власника після створення System Program.
Облікові записи, які належать System Program, називаються системними обліковими
записами. Усі облікові записи гаманців є системними обліковими записами, що
дозволяє їм сплачувати [комісії за транзакції](/docs/core/fees).
...
У прикладі нижче генерується новий keypair і поповнюється SOL. Після виконання
