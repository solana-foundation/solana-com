...
Кожен підпис створюється приватним ключем. Де зберігається цей ключ — у
локальному keypair, хмарному HSM або KMS, чи у сервісі керування гаманцями — є
рішенням на рівні виробничої архітектури. Дивіться
...
сплачує
[базову комісію та комісію за пріоритет](/docs/core/fees/fee-structure#base-fee)
...
- Має бути першим обліковим записом у повідомленні (індекс 0) і підписантом.
- Має бути обліковим записом, що належить System Program, або nonce-обліковим
  записом (перевіряється
  [`validate_fee_payer`](https://github.com/anza-xyz/agave/blob/v3.1.8/svm/src/account_loader.rs#L369-L417)).
- Має містити достатньо lamport для покриття `rent_exempt_minimum + total_fee`;
  інакше транзакція завершиться помилкою _rs`InsufficientFundsForFee`_.

## Повідомлення

Поле `message` є
[`Message`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/message/src/legacy.rs#L161)-структурою,
що містить корисне навантаження транзакції:
...
- [`recent_blockhash`](mention:recent-blockhash):
  [Blockhash](#recent-blockhash), що виконує роль мітки часу для транзакції
...
Поле `header` є
[`MessageHeader`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/message/src/lib.rs#L109)-структурою
з трьома полями `u8`, які розподіляють масив `account_keys` на групи за рівнями
доступу:
...
- [`num_readonly_signed_accounts`](mention:num_readonly_signed_accounts):
  Кількість підписаних облікових записів лише для читання.
- [`num_readonly_unsigned_accounts`](mention:num_readonly_unsigned_accounts):
  Кількість непідписаних облікових записів лише для читання.
...
є компактно-закодованим масивом відкритих ключів. Кожен елемент ідентифікує
обліковий запис, що використовується принаймні однією з інструкцій транзакції.
Масив має містити всі облікові записи та дотримуватися такого суворого порядку:
...
<Callout>
  ...
  лічильниками в [`header`](#header) повідомлення визначати права доступу для
  кожного акаунту без зберігання окремих прапорців метаданих. Лічильники
  заголовка розбивають масив на чотири групи дозволів, перелічені вище.
</Callout>
...
2. **Дедублікація**: запобігає повторній обробці однієї й тієї самої транзакції.
...
_rs`BlockhashNotFound`_, якщо тільки це не є дійсною
[транзакцією з durable nonce](/docs/core/transactions/durable-nonces).

<Callout>
  Метод RPC [`getLatestBlockhash`](/docs/rpc/http/getlatestblockhash) дозволяє
  отримати поточний blockhash та висоту останнього блоку, до якої blockhash буде
  дійсним.
</Callout>
...
— це компактно закодований масив структур
...
1. [`program_id_index`](mention:program-id-index): Індекс у масиві
   `account_keys`, що ідентифікує програму для виклику.
2. [`accounts`](mention:account-indexes): Масив індексів у `account_keys`, що
   вказує акаунти для передачі в програму.
...
вигляді компактного кодування довжини compact-u16. Цей формат використовує 1
байт для значень 0–127 та 2–3 байти для більших значень.
...
Кожна скомпільована інструкція серіалізується так:
...
Діаграма нижче показує, як транзакції та інструкції взаємодіють між собою,
дозволяючи користувачам взаємодіяти з мережею. У цьому прикладі SOL
переказується з одного облікового запису на інший.
...
<Callout type="warn" title="Перевірте отримувача перед відправленням SOL">

Переказ через System Program додає lamport на **будь-який** рахунок. На рівні
протоколу не існує перевірки того, чи зможе отримувач повернути SOL назад.
Lamport можуть бути виведені лише програмою-власником рахунку, тому відправлення
SOL на [token mint](/docs/tokens/basics/create-mint), програму або
[PDA](/docs/core/pda), яким ви не керуєте, **загрожує безповоротною втратою
коштів** — повернути їх може лише повноважна особа, визначена
програмою-власником. SOL, надісланий на
[token account](/docs/tokens/basics/create-token-account), може бути отриманий
назад лише власником цього рахунку, але ніяк не відправником.

Переказ SPL **токенів** має часткову вбудовану захист: Token Program відхиляє
переказ, якщо рахунки не відповідають очікуваному mint. Переказ нативного
**SOL** такого захисту не має, тому відправник повинен перевірити отримувача
...
Наведений нижче приклад демонструє код, що стосується діаграм вище. Дивіться
...
Наступний приклад демонструє структуру транзакції, що містить єдину інструкцію
переказу SOL.
...
<WithMentions>
Наведений нижче код показує результат виконання попередніх фрагментів коду.
...
отримувача перед підписанням. Отримайте інформацію про акаунт і надсилайте кошти
лише на гаманець System Program (або на незаповнену адресу
...
<Callout>
  ...
  дивіться [Verify Address](/docs/payments/send-payments/verify-address).
</Callout>

## Отримання деталей транзакції

Після відправлення отримайте деталі транзакції за допомогою підпису транзакції
та методу RPC [getTransaction](/docs/rpc/http/gettransaction).
