...
Переказ через System Program додає lamport до **будь-якого** акаунту. На рівні
протоколу немає перевірки того, чи зможе отримувач вивести SOL назад. Lamport
можуть бути переміщені лише програмою-власником акаунту, тому надсилання SOL на
[mint токена](/docs/tokens/basics/create-mint), програму або
[PDA](/docs/core/pda), який ви не контролюєте, **загрожує безповоротною втратою
коштів** — повернути їх може лише авторитет, визначений програмою-власником.
SOL, надісланий на [token account](/docs/tokens/basics/create-token-account),
може бути повернутий лише власником цього акаунту, але не відправником.

Перекази SPL **токенів** частково захищені самі по собі: Token Program відхиляє
переказ, якщо акаунти не відповідають очікуваному mint. Перекази нативного
**SOL** не мають такого захисту, тому відправник повинен перевірити отримувача
перед підписанням. Дивіться
[Перевірка адреси](/docs/payments/send-payments/verify-address) для ознайомлення
з повною логікою класифікації.

</Callout>

Приклад нижче демонструє код, що відповідає наведеним вище діаграмам. Дивіться
...
<WithMentions>
Код нижче відображає вивід попередніх фрагментів коду.
Формат відрізняється залежно від SDK,
проте зверніть увагу, що кожна інструкція містить однакову необхідну інформацію.
...
лише на гаманець System Program (або на незафондовану
[адресу на кривій](/docs/payments/send-payments/verify-address#on-curve-addresses));
відхиляйте mint'и, token account'и, програми та PDA, які ви не контролюєте.
...
<Callout>
  ...
  також обробляє відправлення токенів SPL (token account, ATA, Token-2022),
