---
title: CPI з підписантами PDA
description:
  Як використовувати функцію invoke_signed для міжпрограмних викликів на Solana,
  коли програма-викликач повинна підписувати як PDA, з прикладами Anchor
  CpiContext та Native Rust.
...
<Callout type="info" title="Підсумок">
  Використовуйте _rs`invoke_signed`_, коли програма-викликач повинна підписувати
  від імені PDA, яким вона володіє. Середовище виконання виводить публічні ключі
  PDA з наданих сідів підписанта та додає їх до набору дійсних підписантів перед
  перевіркою привілеїв.
</Callout>

<WithMentions>

## CPI з підписантами PDA

Коли CPI вимагає підписанта PDA, використовуйте
[`invoke_signed`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/program/src/program.rs#L51)
з [сідами підписанта](mention:signer-seeds), які використовуються для виведення
[PDA](/docs/core/pda). Для деталей про те, як середовище виконання перевіряє
підписи PDA, дивіться
...
Приклади нижче виконують CPI з підписантами PDA, використовуючи Anchor та Native
Rust. Кожен приклад включає одну інструкцію для переказу SOL з PDA на акаунт
одержувача, використовуючи CPI, підписаний PDA.

#### Anchor

Наступні приклади показують два підходи до реалізації CPI в програмі
[Anchor](https://www.anchor-lang.com/docs). Приклади є функціонально
еквівалентними, але демонструють різні рівні абстракції.

- **Приклад 1**: використовує _rs`CpiContext`_ та допоміжну функцію Anchor.
- **Приклад 2**: використовує функцію _rs`system_instruction::transfer`_ з
  крейту _rs`solana_program`_. (Приклад 1 є абстракцією цієї реалізації.)
- **Приклад 3**: конструює інструкцію CPI вручну. Цей підхід є корисним, коли
  немає доступного крейту для побудови інструкції, яку ви хочете викликати.
...
Наведений нижче приклад виконує CPI з підписантами PDA з програми, написаної на
нативному Rust. Він включає одну інструкцію, яка переказує SOL з облікового
запису PDA на інший. CPI підписується обліковим записом PDA. (Тестовий файл
