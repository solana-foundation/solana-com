---
title: CPI без підписувачів PDA
description:
  Як використовувати функцію invoke для міжпрограмних викликів на Solana, коли
  не потрібен підпис PDA, з прикладами Anchor CpiContext та Native Rust.
...
<Callout type="info" title="Підсумок">
  Використовуйте _rs`invoke`_ для CPI, де всі необхідні підписувачі вже
...
Функція _rs`invoke`_ викликає функцію _rs`invoke_signed`_ з порожнім масивом
_rs`signers_seeds`_. Порожній масив підписувачів вказує на те, що PDA для
підпису не потрібні.
...
Наведені нижче приклади виконують CPI за допомогою Anchor та Native Rust. Вони
включають одну інструкцію, яка переказує SOL з одного акаунта на інший.

#### Anchor

Наступні приклади демонструють два підходи до реалізації CPI у програмі
[Anchor](https://www.anchor-lang.com/docs). Приклади функціонально еквівалентні,
але демонструють різні рівні абстракції.
...
- **Приклад 2**: використовує функцію _rs`system_instruction::transfer`_ з
  крейта _rs`solana_program`_.
- **Приклад 3**: конструює інструкцію CPI вручну. Цей підхід корисний, коли не
  існує крейта для побудови інструкції, яку ви хочете викликати.
...
Наступний приклад показує, як виконати CPI з програми, написаної на Native Rust.
Він включає одну інструкцію, яка переказує SOL з одного акаунта на інший.
(Тестовий файл використовує [LiteSVM](https://github.com/LiteSVM/litesvm) для
тестування програми.)
