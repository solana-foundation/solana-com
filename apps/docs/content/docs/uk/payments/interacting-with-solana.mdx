...
<Callout type="caution" title="Не використовуйте публічний RPC для продакшену">
  Публічні точки доступу (`api.mainnet.solana.com`, `api.devnet.solana.com`) мають
  обмеження частоти запитів, не мають SLA і не підходять для виробничих платіжних потоків.
  Використовуйте [провайдера RPC](https://solana.com/rpc), щоб отримати приватну точку доступу RPC
  для виробничих розгортань.
</Callout>
