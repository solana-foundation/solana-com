---
title: Фіксоване делегування
description: Посібник з налаштування транзакцій фіксованого делегування.
---

Фіксоване делегування дозволяє користувачеві схвалити інший гаманець або сервіс
для виведення до визначеної фіксованої суми токенів. Кожна успішна передача
зменшує залишковий ліміт. Використовуйте `expiryTs = 0` для відсутності терміну
дії.

Цей посібник зберігає прозорість процесу. Кожен фрагмент використовує функції
SDK безпосередньо, тож ви можете бачити, який обліковий запис виводиться, яка
інструкція надсилається і який підписувач оплачує або авторизує кожен крок.
...
Делегат підписує передачу. SDK потребує той самий PDA делегування, token account
користувача та token account отримувача.
...
Делегатор може відкликати делегування в будь-який момент. Відкликання закриває
PDA делегування та повертає його rent підписанту.
...
- token account користувача повинен існувати до ініціалізації.
- Суми вказуються в базових одиницях. Для токена з 6 десятковими знаками
  `1_000_000` означає `1` токен.
- Запускайте `initSubscriptionAuthority` лише тоді, коли обліковий запис
  Subscription Authority ще не існує.
- `expiryTs` є жорстким обмеженням: після його настання перекази не виконуються,
  а спонсор може повернути rent. Пільгового періоду витрат немає.
- Користувач підписує транзакції налаштування та відкликання. Делегат підписує
  перекази.
