---
title: Платіж з memo
description: Дізнайтеся, як додавати ідентифікатори рахунків або примітки до платежу.
---

Програма memo Solana дозволяє прикріплювати номери рахунків, ідентифікатори
замовлень або довільні посилання до будь-якого платежу. Ці memo назавжди
записуються в блокчейні та відображаються в журналах транзакцій, що полегшує
зіставлення платежів з вашими внутрішніми системами.

## Як це працює

Програма Memo записує текст у логи транзакції. Ці логи індексуються експлорерами
та RPC-провайдерами, що робить memo доступними для пошуку під час звірки
платежів.
...
Додавання memo вимагає безпосереднього створення інструкцій, що дає вам контроль
над тим, що включається в транзакцію.

Кроки нижче показують основний процес. Перегляньте [демо](#demo) для повного
робочого коду.

<ScrollyCoding>

## !!steps Імпорт програми Memo

Імпортуйте `getAddMemoInstruction` з `@solana-program/memo` для створення
інструкцій memo.

<CodePlaceholder title="Платіж з memo" />
...
Створіть інструкцію переказу токенів, вказавши вихідний ATA, цільовий ATA,
власника (підписувача) та суму в базових одиницях.
...
## !!steps Створення інструкції memo

Створіть інструкцію memo з повідомленням. Це повідомлення буде видимим у логах
програми транзакції.
...
## !!steps Відправлення транзакції з memo

Об'єднайте інструкції переказу та memo в одній транзакції.
...
## !!steps Перегляд мемо в журналах транзакцій

Після підтвердження транзакції отримайте її, щоб переглянути мемо в журналах.
Мемо з'являється як повідомлення журналу від програми Memo.

<CodePlaceholder title="Платіж з мемо" />
