...
description:
  Переміщення токенів між token accounts для одного й того ж mint за допомогою
  інструкцій transfer Token Program.
...
## Що робить переказ токенів?

Переказ токенів переміщує токени з одного token account в інший token account
для одного й того ж mint.

Перекази не змінюють загальну пропозицію mint. Перекази лише оновлюють залишки
між token accounts.

## Як переказувати токени

Для переказу токенів використовується інструкція _rs`Transfer`_ або
_rs`TransferChecked`_ Token Program.

Приклади нижче використовують _rs`TransferChecked`_, яка вимагає від викликача
надати mint і decimals, щоб інструкція могла перевірити очікуваний mint і
точність токенів перед переміщенням токенів між акаунтами.

У Token Extensions Program інструкція _rs`Transfer`_ вважається застарілою на
користь _rs`TransferChecked`_ або _rs`TransferCheckedWithFee`_.

Власник вихідного акаунта або затверджений делегат підписує переказ. У Token
Extensions Program постійний делегат mint також може авторизувати переказ, якщо
для mint увімкнено розширення постійного делегата.

### Посилання на вихідний код

| Елемент                | Опис                                                                                                                                                                                           | Token Program                                                                                                   | Token Extensions Program                                                                                              |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_          | Стан token account зберігає залишки, які оновлюються під час переказу.                                                                                                                         | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | Інструкція переказу, яка переміщує токени між token accounts без вимоги від викликача надавати mint або decimals. У Token Extensions Program замість неї використовуйте _rs`TransferChecked`_. | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Інструкція переказу, яка вимагає від викликача надати mint і decimals та перевіряє ці значення перед переміщенням токенів між token accounts одного й того ж mint.                             | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | Спільна логіка процесора для переказів токенів.                                                                                                                                                | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |
