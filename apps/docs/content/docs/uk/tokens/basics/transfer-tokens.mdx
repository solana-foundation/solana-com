...
description:
  Переміщуйте токени між token account для одного й того ж mint за допомогою
...
Перекази не змінюють загальну кількість mint. Перекази лише оновлюють баланси
між token account.

## Як переказувати токени

Переказ токенів використовує інструкцію _rs`Transfer`_ або _rs`TransferChecked`_
Token Program.

Приклади нижче використовують _rs`TransferChecked`_, що вимагає від викликача
надати mint та decimals, щоб інструкція могла перевірити очікуваний mint та
точність токена перед переміщенням токенів між обліковими записами.

У Token Extensions Program _rs`Transfer`_ застаріла на користь
_rs`TransferChecked`_ або _rs`TransferCheckedWithFee`_.

Власник source account або затверджений делегат підписує переказ. У Token
Extensions Program постійний делегат mint також може авторизувати переказ, якщо
mint має увімкнене розширення постійного делегата.

### Посилання на вихідний код

| Елемент                | Опис                                                                                                                                                                                            | Token Program                                                                                                   | Token Extensions Program                                                                                              |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_          | Стан token account зберігає баланси, оновлені переказом.                                                                                                                                        | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | Інструкція переказу, яка переміщує токени між token account без вимоги від викликача надавати mint або decimals. У Token Extensions Program замість цього використовуйте _rs`TransferChecked`_. | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Інструкція переказу, яка вимагає від викликача надати mint та decimals і перевіряє ці значення перед переміщенням токенів між token account одного mint.                                        | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
