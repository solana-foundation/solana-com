...
description:
  Змінюйте або відкликайте ролі повноважень на обліковому записі mint або token
  account, включаючи повноваження mint, freeze, owner та close.
...
## Що робить встановлення повноважень?

Встановлення повноважень змінює або відкликає конкретні ролі повноважень на
обліковому записі mint або token account.

Облікові записи mint можуть зберігати повноваження mint і повноваження freeze.
Облікові записи token account зберігають власника і можуть опціонально зберігати
повноваження close. Кожна роль контролює конкретний набір дозволів, і
встановлення повноважень на _rs`None`_ назавжди видаляє обрану роль повноважень.

Token Extensions Program також визначає додаткові типи повноважень для
конкретних розширень.

## Як встановити повноваження

Зміна повноважень використовує інструкцію _rs`SetAuthority`_ Token Program.

Кожен виклик оновлює один тип повноважень на одному обліковому записі mint або
token account. Поточні повноваження підписують зміну, а нові повноваження можуть
бути іншою адресою або _rs`None`_ для постійного відкликання обраних
повноважень.
...
| --------------------------- | --------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                  | Стан mint зберігає _rs`mint_authority`_ і _rs`freeze_authority`_.                                   | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)           |
| _rs`Account`_               | Стан token account зберігає _rs`owner`_ і _rs`close_authority`_.                                    | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)          |
| _rs`AuthorityType`_         | Ролі повноважень, які може оновлювати _rs`SetAuthority`_.                                           | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L739-L751) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1116-L1157) |
| _rs`SetAuthority`_          | Інструкція, яка змінює або відкликає одні повноваження на обліковому записі mint або token account. | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L141-L158) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L183-L200)   |
| _rs`process_set_authority`_ | Спільна логіка процесора для оновлення повноважень.                                                 | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L423-L518)     | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L677-L978)       |
