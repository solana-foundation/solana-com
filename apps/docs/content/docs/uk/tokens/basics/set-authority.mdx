...
- **Mint Authority**: Контролює створення нових токенів. Може карбувати токени
  на будь-який token account. Часто відкликається після створення початкового
  запасу для створення токена з фіксованим запасом.
- **Freeze Authority**: Контролює можливість заморожувати та розморожувати token
  account. Може запобігти будь-якому token account переказувати токени. Часто
  відкликається, щоб гарантувати користувачам, що їхні токени не можуть бути
  заморожені.

### Повноваження token account

- **Account Owner**: Має повний контроль над token account. Може переказувати
  токени, спалювати токени, затверджувати делегатів та закривати обліковий
  запис, коли баланс дорівнює нулю.
...
поточний власник повноважень може передати свої дозволи на нову адресу або
