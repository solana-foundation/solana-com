...
description:
  Зміна або відкликання ролей авторитету на mint або token account, включаючи
  mint, freeze, owner та close authority.
...
## Що робить встановлення авторитету?

Встановлення авторитету змінює або відкликає конкретні ролі авторитету на mint
або token account.

Mint account може зберігати mint authority та freeze authority. Token account
зберігають owner і можуть опціонально зберігати close authority. Кожна роль
контролює конкретний набір дозволів, і встановлення авторитету на _rs`None`_
назавжди видаляє обрану роль авторитету.

Token Extensions Program також визначає додаткові типи авторитету для конкретних
розширень.

## Як встановити авторитет

Зміна авторитету використовує інструкцію _rs`SetAuthority`_ Token Program.

Кожен виклик оновлює один тип авторитету на одному mint або token account.
Поточний авторитет підписує зміну, а новим авторитетом може бути інша адреса або
_rs`None`_ для назавжди відкликання обраного авторитету.

### Посилання на вихідний код

| Елемент                     | Опис                                                                           | Token Program                                                                                                   | Token Extensions Program                                                                                                |
| --------------------------- | ------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                  | Стан mint зберігає _rs`mint_authority`_ та _rs`freeze_authority`_.             | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)           |
| _rs`Account`_               | Стан token account зберігає _rs`owner`_ та _rs`close_authority`_.              | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)          |
| _rs`AuthorityType`_         | Ролі авторитету, які може оновлювати _rs`SetAuthority`_.                       | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L739-L751) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1116-L1157) |
| _rs`SetAuthority`_          | Інструкція, яка змінює або відкликає один авторитет на mint або token account. | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L141-L158) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L183-L200)   |
| _rs`process_set_authority`_ | Спільна логіка процесора для оновлення авторитетів.                            | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L423-L518)     | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L677-L978)       |
...
`@solana/web3.js` включені для довідки.
