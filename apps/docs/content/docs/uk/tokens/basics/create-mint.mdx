...
description:
  Створіть mint токена та налаштуйте кількість десяткових знаків, повноваження
  на карбування та повноваження на заморожування.
...
Mint account визначає та унікально ідентифікує токен у Solana, а також зберігає
спільний стан, що застосовується до всіх token account для цього mint.

Token Program визначає тип облікового запису _rs`Mint`_ як:
...
Кожен токен має один mint account, і адреса mint є унікальним ідентифікатором
токена у гаманцях, додатках та провідниках.

Наприклад, USD Coin (USDC) має адресу mint
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`. Адреса mint унікально
ідентифікує USDC в усій екосистемі Solana. Ви можете переглянути цей mint у
[Solana Explorer](https://explorer.solana.com/address/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v).

## Як створити Mint Account

Створення mint потребує двох інструкцій:

1. Інструкція _rs`CreateAccount`_ System Program створює новий rent-exempt
   обліковий запис і призначає Token Program власником програми нового
   облікового запису.
2. Інструкція _rs`InitializeMint`_ або _rs`InitializeMint2`_ Token Program
   ініціалізує новий обліковий запис як mint.

Включіть інструкцію _rs`CreateAccount`_ та інструкцію ініціалізації mint в одну
транзакцію.

Під час ініціалізації mint Token Program перевіряє, що mint account ще не
ініціалізований і є rent-exempt. Потім Token Program записує повноваження на
карбування, повноваження на заморожування, десяткові знаки та прапорець
_rs`is_initialized`_ у дані mint account.

### Посилання на вихідний код

| Елемент                        | Опис                                                                                                     | Token Program                                                                                                   | Token Extensions Program                                                                                              |
| ------------------------------ | -------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                     | Базовий стан mint, що зберігається в кожному mint account.                                               | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`InitializeMint`_           | Інструкція ініціалізації mint, яка очікує обліковий запис rent sysvar у списку облікових записів.        | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L24-L44)   | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L50-L74)   |
| _rs`InitializeMint2`_          | Інструкція ініціалізації mint, яка не потребує облікового запису rent sysvar у списку облікових записів. | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L401-L414) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L471-L486) |
| _rs`_process_initialize_mint`_ | Спільна логіка обробника для ініціалізації mint.                                                         | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
| _rs`process_initialize_mint`_  | Публічний обробник для _rs`InitializeMint`_.                                                             | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L62-L70)       | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L132-L140)     |
| _rs`process_initialize_mint2`_ | Публічний обробник для _rs`InitializeMint2`_.                                                            | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L72-L81)       | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L142-L151)     |

### Typescript

Наведені нижче приклади `Kit` демонструють рекомендований підхід із
використанням `@solana/kit`. Застарілі приклади з використанням
`@solana/web3.js` наведено для довідки.
...
mint account зберігає лише базову інформацію про токен (пропозицію, десяткові
знаки, повноваження). Щоб додати до свого токена метадані, зрозумілі людині,
такі як назва, символ та зображення, у вас є два варіанти:

<Cards>
  <Card title="Metaplex Token Metadata" href="/docs/tokens/metaplex">
   Додайте метадані за допомогою Metaplex Token Metadata Program. Працює як з оригінальним Token Program, так і з Token-2022.
  </Card>

  <Card title="Token Extensions Metadata" href="/docs/tokens/extensions/metadata">
   Використовуйте вбудоване розширення метаданих у Token-2022. 
   Розширення метаданих доступне лише для mint account, створених за допомогою Token-2022.
   Щоб додати метадані до mint account, створених за допомогою оригінального Token Program, використовуйте Metaplex Token Metadata Program.
  </Card>
</Cards>
