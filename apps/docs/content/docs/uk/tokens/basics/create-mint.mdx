---
title: Створення монетного двору токена
description:
  Створіть монетний двір токена та налаштуйте десяткові знаки, повноваження
  карбування та повноваження заморожування.
...
спільний стан, який застосовується до всіх token account для цього монетного
двору.
...
Кожен токен має один mint account, і адреса монетного двору є унікальним
ідентифікатором токена в гаманцях, додатках та оглядачах.
...
1. Інструкція _rs`CreateAccount`_ System Program створює новий обліковий запис,
   звільнений від rent, і призначає Token Program як власника програми нового
   облікового запису.
2. Інструкція _rs`InitializeMint`_ або _rs`InitializeMint2`_ Token Program
   ініціалізує новий обліковий запис як монетний двір.

Включіть інструкцію _rs`CreateAccount`_ та інструкцію ініціалізації монетного
...
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                     | Базовий стан монетного двору, що зберігається у кожному mint account.                                                     | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`InitializeMint`_           | Інструкція ініціалізації монетного двору, яка очікує обліковий запис sysvar rent у своєму списку облікових записів.       | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L24-L44)   | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L50-L74)   |
| _rs`InitializeMint2`_          | Інструкція ініціалізації монетного двору, яка не вимагає облікового запису sysvar rent у своєму списку облікових записів. | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L401-L414) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L471-L486) |
| _rs`_process_initialize_mint`_ | Спільна логіка процесора для ініціалізації монетного двору.                                                               | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
...
Обліковий запис mint зберігає лише базову інформацію про токен (пропозицію,
десяткові знаки, повноваження). Щоб додати до вашого токена зрозумілі для людини
метадані, такі як назва, символ і зображення, у вас є два варіанти:
...
<Cards>
  ...
  <Card title="Token Extensions Metadata" href="/docs/tokens/extensions/metadata">
   Використовуйте вбудоване розширення метаданих з Token-2022. 
   Розширення метаданих доступне лише для облікових записів mint, створених за допомогою Token-2022.
   Щоб додати метадані до облікових записів mint, створених за допомогою оригінальної Token Program, використовуйте Metaplex Token Metadata Program.
  </Card>
</Cards>
