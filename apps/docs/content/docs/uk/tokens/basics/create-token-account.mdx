...
Token account зберігає ваш баланс конкретного токена. Кожен token account
пов'язаний лише з одним mint і відстежує ваш баланс токенів та додаткові деталі.
...
<Callout type="info">
  Зверніть увагу, що у вихідному коді розробники називають Token account типом
  `Account`. Як [Token
...
1. **Mint**: конкретний тип токена, який він зберігає
2. **Owner**: повноваження, яке може переказувати токени з цього акаунта
...
- Цей акаунт зберігає лише токени USDC
- Circle контролює токени через повноваження власника token account:
...
<Callout type="info">
  **Розуміння термінології "Owner"**

Термін "owner" може використовуватися у двох різних контекстах:

1. **Token Account Owner**: повноваження, яке може переказувати, спалювати або
