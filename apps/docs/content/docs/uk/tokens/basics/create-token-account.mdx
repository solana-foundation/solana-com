...
description:
  Створення облікових записів токенів і associated token accounts для
  конкретного mint і власника.
...
Обліковий запис токена (token account) зберігає токени для одного mint і одного
власника облікового запису токена в Solana.
...
Тут _rs`owner`_ означає повноваження, які можуть переміщувати, спалювати або
делегувати токени з облікового запису токена. Власником програми облікового
запису як і раніше залишається Token Program або Token Extensions Program.

Кожен обліковий запис токена прив'язаний рівно до одного mint, що означає, що
...
Associated token account (ATA) — це обліковий запис токена за замовчуванням для
гаманця та mint.
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program/src)
виводить адресу ATA з адреси гаманця, адреси програми токена та адреси mint.

Тільки облікові записи токенів, створені Associated Token Program, називаються
associated token accounts.

Associated Token Program — це спосіб створення облікового запису токена за
стандартною, детермінованою адресою. Отриманий обліковий запис як і раніше
залишається обліковим записом токена, що належить Token Program або Token
Extensions Program, а не Associated Token Program.
...
Для будь-якої комбінації гаманця, програми токена та mint існує рівно одна
адреса ATA. Associated Token Program створює стандартний обліковий запис токена
за цією адресою, і отриманий обліковий запис як і раніше використовує тип
_rs`Account`_, визначений Token Program або Token Extensions Program.

2. **Program Owner**: програма, якій належить акаунт (завжди Token Program або
   Token Extension Program для token accounts).

Працюючи з token accounts, "owner" зазвичай означає повноваження, яке може
переказувати токени, а не програму, якій належить акаунт.

</Callout>

## Що таке Associated Token Account?

Associated token account (ATA) — це стандартний токен-акаунт для гаманця, який
зберігає конкретний токен. Він використовує детерміновану адресу (PDA), створену
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program).

<Callout type="info">
  Лише токен-акаунти, створені Associated Token Program, називаються "associated
  token accounts".
</Callout>

ATA використовують детерміновані адреси, що спрощує пошук токен-акаунта
будь-якого гаманця для заданого mint. Див.
...
усуває потребу вручну відстежувати адреси токен-акаунтів — ви завжди можете
...
- Створює токен-акаунти за детермінованими адресами (PDA)
- Виконує Cross-Program Invocations (CPI) до Token Program
- Не підтримує власного стану

Отримані токен-акаунти належать Token Program і використовують стандартну
структуру `Account`.
...
Створення токен-акаунта вимагає інструкції
...
Для створення токен-акаунта потрібні дві інструкції:

1. System Program: створити акаунт із виділеним простором для токен-акаунта та
   передати право власності Token Program
2. Token Program: ініціалізувати дані токен-акаунта
...
ініціалізацію через міжпрограмні виклики (CPI).
