...
description:
  Створіть token accounts та associated token accounts для певного mint і
  власника.
...
Token account зберігає токени для одного mint і одного власника token account на
Solana.
...
Тут _rs`owner`_ означає повноваження, які можуть передавати, спалювати або
делегувати токени з token account. Власником програми облікового запису все ще
залишається Token Program або Token Extensions Program.

Кожен token account прив'язаний точно до одного mint, що означає, що token
account може зберігати одиниці лише одного токена, токена, визначеного полем
_rs`mint`_ token account.

## Що таке Associated Token Account

Associated token account (ATA) — це типовий token account для гаманця та mint.
...
Лише token accounts, створені Associated Token Program, називаються associated
token accounts.

Associated Token Program — це спосіб створити token account за стандартною,
детермінованою адресою. Отриманий обліковий запис все ще є token account, яким
володіє Token Program або Token Extensions Program, а не Associated Token
Program.
...
Для будь-якої комбінації гаманця, програми токенів та mint існує точно одна
адреса ATA. Associated Token Program створює стандартний token account за цією
адресою, і отриманий обліковий запис все ще використовує тип _rs`Account`_,
визначений Token Program або Token Extensions Program.

## Як створити асоційований токен-акаунт

Створення асоційованого токен-акаунта використовує інструкцію _rs`Create`_ або
_rs`CreateIdempotent`_ програми Associated Token Program. Associated Token
Program виводить адресу ATA, створює акаунт за адресою ATA та ініціалізує акаунт
як токен-акаунт, що належить Token Program або Token Extension Program.

<Callout type="info" title="Рекомендовано">
  Для більшості застосунків створюйте токен-акаунти через Associated Token
  Program замість того, щоб створювати їх безпосередньо викликом інструкцій
  System Program та Token Program. Асоційований токен-акаунт використовує
  детерміновану адресу, виведену з власника, токен-програми та мінта, що
  полегшує гаманцям та застосункам пошук стандартного токен-акаунта для
  конкретного мінта.
</Callout>
...
| _rs`CreateIdempotent`_                        | Створює асоційований токен-акаунт, але все одно завершується успішно, якщо цей ATA вже існує для того самого власника та мінта.                                                                                                                       | [Джерело](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | Виводить адресу ATA та використовує _rs`create_pda_account`_ разом із CPI до обраної токен-програми для ініціалізації токен-акаунта. Коли обраною програмою є Token Extension Program, процесор ATA також ініціалізує розширення незмінного власника. | [Джерело](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | Допоміжна функція, що використовується _rs`process_create_associated_token_account`_ для створення PDA-акаунта через CPI до System Program.                                                                                                           | [Джерело](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |

### Typescript

Приклади `Kit` нижче демонструють рекомендований підхід з використанням
`@solana/kit`. Устарілі приклади з використанням `@solana/web3.js` включені для
довідки.
...
1. Інструкція _rs`CreateAccount`_ System Program створює новий акаунт,
   звільнений від rent, і призначає Token Program власником нового акаунта.
2. Інструкція _rs`InitializeAccount`_, _rs`InitializeAccount2`_ або
   _rs`InitializeAccount3`_ Token Program ініціалізує новий акаунт для mint та
   власника.
...
ініціалізований і звільнений від rent.

<Callout type="info">
  Розділ нижче показує, як створити токен-акаунт шляхом прямого виклику
  інструкцій System Program та Token Program. Для більшості застосунків
  використовуйте Associated Token Program натомість. Використовуйте прямі
  виклики System Program та Token Program, коли у вас є конкретна причина не
  використовувати associated token account або коли вам потрібно створити
  кастомні PDA токен-акаунти, здійснюючи CPI до інструкцій System Program та
  Token Program з вашої власної програми Solana.
</Callout>
...
| _rs`InitializeAccount3`_          | Інструкція ініціалізації токен-акаунта, яка передає власника в instruction data і не вимагає акаунта rent sysvar. | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L377-L387) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L445-L456) |
...
приклади з використанням `@solana/kit` включені для довідки.
