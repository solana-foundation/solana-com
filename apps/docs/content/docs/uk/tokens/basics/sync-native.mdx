...
Wrapped SOL (WSOL) — це token account, який відстежує кількість лампортів в
обліковому записі як баланс токена. WSOL забезпечує інтеграцію з протоколами
DeFi для передачі SOL як токена SPL.
...
синхронізує баланс токен-акаунта wrapped SOL (WSOL) з фактичною кількістю SOL
(lamports), що зберігається в ньому. Коли ви переказуєте нативний SOL на
токен-акаунт WSOL, баланс токенів не оновлюється автоматично. Ви повинні
викликати SyncNative, щоб відобразити коректний баланс WSOL.
