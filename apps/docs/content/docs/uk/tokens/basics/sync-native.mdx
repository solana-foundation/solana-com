---
title: Синхронізація нативного SOL
description:
  Оновлення суми обгорнутого SOL в токен-акаунті після переказу SOL на акаунт.
...
Обгорнутий SOL (WSOL) — це SOL, що зберігається в токен-акаунті для нативного
mint Token Program. Нативний mint — це адреса mint, яку Token Program
використовує для токен-акаунтів, що обгортають SOL. Нативний mint дозволяє
використовувати SOL через інструкції Token Program.

| Програма                 | Адреса нативного mint                          |
| ------------------------ | ---------------------------------------------- |
...
Токен-акаунт обгорнутого SOL зберігає SOL у полі `lamports` акаунта, але
відстежує SOL, утримуваний у токен-акаунті обгорнутого SOL, як баланс токенів
через поле `amount` токен-акаунта. Якщо ви переказуєте SOL на токен-акаунт
обгорнутого SOL, баланс у lamport збільшується негайно, але поле `amount` не
змінюється, доки ви не викличете інструкцію `SyncNative` Token Program.

## Як синхронізувати нативний SOL

Використовуйте інструкцію `SyncNative` Token Program після переказу SOL на
токен-акаунт обгорнутого SOL. Інструкція `SyncNative` оновлює поле `amount`
токен-акаунта відповідно до lamport, утримуваних на акаунті понад його резерв
для звільнення від оплати оренди.

### Посилання на вихідний код

| Елемент               | Опис                                                                                                                                                                                                                                  | Token Program                                                                                                   | Token Extensions Program                                                                                              |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `Account`             | Стан токен-акаунта використовує `is_native` для позначення акаунта як WSOL і зберігає нативний резерв, мінімальний баланс у lamport для звільнення від оплати оренди, який акаунт повинен підтримувати, та відстежувану суму токенів. | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| `SyncNative`          | Інструкція, яка оновлює поле `amount` токен-акаунта обгорнутого SOL відповідно до lamport, утримуваних на акаунті понад його резерв для звільнення від оплати оренди.                                                                 | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| `process_sync_native` | Спільна логіка процесора для синхронізації балансів обгорнутого SOL.                                                                                                                                                                  | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |
