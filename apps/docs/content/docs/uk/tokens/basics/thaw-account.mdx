...
description:
  Розморозьте заморожений token account, щоб відновити можливість переказів,
  спалювання та зміни делегатів.
...
## Що робить розморожування token account?

Розморожування змінює стан token account із замороженого назад до
ініціалізованого стану.

Після успішного розморожування обліковий запис знову може отримувати, передавати
та спалювати токени. Тільки уповноважена особа заморожування mint може
розморожувати token account.

## Як розморозити token account

Розморожування token account використовує інструкцію _rs`ThawAccount`_ Token
Program.

Інструкція _rs`ThawAccount`_ перевіряє, що token account належить монетному
двору і що підписувач є freeze authority монетного двору, а потім змінює стан
token account із замороженого назад на ініціалізований.

[Нативні token accounts](/docs/tokens/basics/sync-native) не підтримують
розморожування.
...
| ----------------------------------- | ---------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Стан монетного двору зберігає опціональний freeze authority для розморожування token accounts. | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Стани token account, включаючи _rs`Frozen`_ та _rs`Initialized`_.                              | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`ThawAccount`_                   | Інструкція, яка змінює заморожений token account назад до ініціалізованого стану.              | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L229-L243) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L292-L306) |
| _rs`process_toggle_freeze_account`_ | Спільна логіка процесора для заморожування та розморожування token accounts.                   | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |

### Typescript

Приклади `Kit` нижче показують рекомендований підхід з використанням
`@solana/kit`. Легасі-приклади з використанням `@solana/web3.js` включено для
довідки.
