...
description:
  Спалюйте токени з токен-акаунта та зменшуйте загальну пропозицію монети.
...
## Що робить спалювання?

Спалювання токенів назавжди зменшує баланс токен-акаунта та зменшує загальну
пропозицію монети на ту саму кількість.

## Як спалювати токени

Спалювання токенів використовує інструкцію _rs`Burn`_ або _rs`BurnChecked`_
Token Program.

Приклади нижче використовують _rs`BurnChecked`_, яка вимагає від викликача
надати кількість десяткових знаків монети, щоб інструкція могла перевірити
очікувану точність токена перед спалюванням.

[Нативна монета](/docs/tokens/basics/sync-native) не підтримує спалювання.
Використовуйте натомість _rs`CloseAccount`_.

Власник токен-акаунта або затверджений делегат підписує транзакцію. У Token
Extension Program постійний делегат монети також може авторизувати спалювання,
якщо монета має увімкнене розширення постійного делегата.

### Посилання на джерела

| Елемент            | Опис                                                                                                                                                                             | Token Program                                                                                                   | Token Extension Program                                                                                               |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_         | Стан монети зберігає загальну пропозицію токенів.                                                                                                                                | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | Стан токен-акаунта зберігає баланс токенів, зменшений внаслідок спалювання.                                                                                                      | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Інструкція спалювання, яка зменшує баланс токен-акаунта та пропозицію монети без вимоги від викликача надавати кількість десяткових знаків монети.                               | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Інструкція спалювання, яка вимагає від викликача надати кількість десяткових знаків монети та перевіряє це значення перед зменшенням балансу токен-акаунта та пропозиції монети. | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | Спільна логіка процесора для спалювання токенів.                                                                                                                                 | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |

### Typescript

Наведені нижче приклади `Kit` демонструють рекомендований підхід з використанням
`@solana/kit`. Устарілі приклади з `@solana/web3.js` наведено для довідки.
