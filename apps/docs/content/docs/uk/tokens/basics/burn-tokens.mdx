...
description:
  Спалюйте токени з облікового запису токенів та зменшуйте загальну пропозицію
  карбування.
...
Спалювання токенів безповоротно зменшує баланс облікового запису токенів і
знижує загальну пропозицію карбування на ту саму величину.
...
Наведені нижче приклади використовують _rs`BurnChecked`_, що вимагає від
викликача надати кількість десяткових знаків карбування, щоб інструкція могла
перевірити очікувану точність токена перед спалюванням.

[Нативне карбування](/docs/tokens/basics/sync-native) не підтримує спалювання.
Натомість використовуйте _rs`CloseAccount`_.

Власник облікового запису токенів або затверджений делегат підписує транзакцію.
У Token Extensions Program постійний делегат карбування також може авторизувати
спалювання, якщо для карбування увімкнено розширення постійного делегата.

### Посилання на вихідний код

| Елемент            | Опис                                                                                                                                                                                                | Token Program                                                                                                   | Token Extensions Program                                                                                              |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_         | Стан карбування зберігає загальну пропозицію токенів.                                                                                                                                               | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | Стан облікового запису токенів зберігає баланс токенів, зменшений внаслідок спалювання.                                                                                                             | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Інструкція спалювання, яка зменшує баланс облікового запису токенів і пропозицію карбування без вимоги до викликача надавати кількість десяткових знаків карбування.                                | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Інструкція спалювання, яка вимагає від викликача надати кількість десяткових знаків карбування та перевіряє це значення перед зменшенням балансу облікового запису токенів і пропозиції карбування. | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
...
`@solana/kit`. Приклади застарілого коду з використанням `@solana/web3.js`
включені для довідки.
