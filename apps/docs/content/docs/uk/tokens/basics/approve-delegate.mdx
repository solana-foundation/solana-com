...
description:
  Авторизувати іншу адресу для переказу або спалювання обмеженої кількості
  токенів з облікового запису токенів.
...
Схвалення делегата дозволяє іншій адресі переказувати або спалювати обмежену
кількість токенів з облікового запису токенів від імені власника облікового
запису токенів.

Обліковий запис токенів зберігає одного поточного делегата та одну делеговану
суму одночасно. Схвалення нового делегата замінює попереднього делегата та
дозволену суму в обліковому записі.

## Як схвалити делегата

Схвалення делегата використовує інструкцію _rs`Approve`_ або
_rs`ApproveChecked`_ Token Program.

Приклади нижче використовують _rs`ApproveChecked`_, яка вимагає від викликача
надати mint та decimals, щоб інструкція могла перевірити очікуваний mint та
точність токена перед обробкою схвалення.
...
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_         | Стан облікового запису токенів зберігає власника, делегата та делеговану суму.                                                                           | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | Інструкція схвалення делегата, яка записує делегата та дозволену суму для облікового запису токенів без вимоги від викликача надавати mint або decimals. | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | Інструкція схвалення делегата, яка вимагає від викликача надати mint та decimals і перевіряє ці значення перед записом делегата та дозволеної суми.      | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
| _rs`process_approve`_ | Спільна логіка процесора для схвалення делегата.                                                                                                         | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L344-L392)     | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L582-L640)     |

### Typescript

Приклади нижче з `Kit` демонструють рекомендований підхід з використанням
`@solana/kit`. Застарілі приклади з використанням `@solana/web3.js` наведено для
довідки.
