...
description:
  Авторизуйте іншу адресу для передачі або спалювання обмеженої кількості
  токенів з токен-акаунта.
...
Схвалення делегата дозволяє іншій адресі передавати або спалювати обмежену
кількість токенів з токен-акаунта від імені власника токен-акаунта.

Токен-акаунт зберігає одного поточного делегата та одну делеговану суму
одночасно. Схвалення нового делегата замінює попереднього делегата та ліміт на
акаунті.
...
Приклади нижче використовують _rs`ApproveChecked`_, що вимагає від виклику
надання mint та decimals, щоб інструкція могла перевірити очікуваний mint та
точність токена перед обробкою схвалення.

Власник токен-акаунта підписує схвалення.

### Посилання на Вихідний Код

| Елемент               | Опис                                                                                                                                     | Token Program                                                                                                   | Token Extensions Program                                                                                              |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_         | Стан токен-акаунта зберігає власника, делегата та делеговану суму.                                                                       | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | Інструкція схвалення делегата, що записує делегата та ліміт для токен-акаунта без необхідності надання mint або decimals викликом.       | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | Інструкція схвалення делегата, що вимагає від виклику надання mint та decimals і перевіряє ці значення перед записом делегата та ліміту. | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
