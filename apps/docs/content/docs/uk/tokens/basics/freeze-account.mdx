...
description:
  Заморозьте токен-акаунт за допомогою повноважень на заморожування mint, щоб
  заблокувати перекази, спалювання та зміни делегатів.
...
Заморожування token account зберігає того самого власника, mint і баланс, але
запобігає отриманню, переказу або спаленню токенів з token account до тих пір,
поки token account не буде розморожено.

Заморожений акаунт все ще може бути закритий, якщо заморожений акаунт має
нульовий баланс токенів.

Тільки mint, який все ще має повноваження на заморожування, може заморожувати
токен-акаунти. Якщо повноваження на заморожування mint відкликано, акаунти для
цього mint більше не можуть бути заморожені.

## Як заморозити токен-акаунт

Заморожування токен-акаунта використовує інструкцію _rs`FreezeAccount`_ Token
Program.

Інструкція _rs`FreezeAccount`_ перевіряє, що токен-акаунт належить до mint і що
підписувач є повноваженням на заморожування mint, а потім змінює стан
токен-акаунта з ініціалізованого на заморожений.
...
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Стан mint зберігає необов'язкові повноваження на заморожування, які використовуються для заморожування токен-акаунтів. | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
