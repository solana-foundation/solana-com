...
description:
  Заморозьте токен-акаунт за допомогою повноважень заморозки монетного двору,
  щоб заблокувати переказування, спалювання та зміни делегата.
...
## Що робить заморозка токен-акаунта?

Заморозка токен-акаунта зберігає того ж власника, монетний двір та баланс, але
запобігає переказуванню або спалюванню токенів з токен-акаунта до тих пір, поки
токен-акаунт не буде розморожено.

Заморожений акаунт все ще може бути закритий, якщо заморожений акаунт має
нульовий баланс токенів.

Лише монетний двір, який все ще має повноваження заморозки, може заморожувати
токен-акаунти. Якщо повноваження заморозки монетного двору відкликано, акаунти
для цього монетного двору більше не можуть бути заморожені.

## Як заморозити токен-акаунт

Заморозка токен-акаунта використовує інструкцію _rs`FreezeAccount`_ Token
Program.

Інструкція _rs`FreezeAccount`_ перевіряє, що токен-акаунт належить монетному
двору і що підписувач є повноваженням заморозки монетного двору, після чого
змінює стан токен-акаунта з ініціалізованого на заморожений.

[Нативні токен-акаунти](/docs/tokens/basics/sync-native) не підтримують
заморозку.

### Довідка до вихідного коду

| Елемент                             | Опис                                                                                                                       | Token Program                                                                                                   | Token Extension Program                                                                                               |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Стан монетного двору зберігає необов'язкові повноваження заморозки, які використовуються для заморожування токен-акаунтів. | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Стани токен-акаунта, включаючи _rs`Initialized`_ та _rs`Frozen`_.                                                          | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | Інструкція, яка переводить токен-акаунт у заморожений стан.                                                                | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
| _rs`process_toggle_freeze_account`_ | Спільна логіка процесора для заморозки та розморожування токен-акаунтів.                                                   | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |

### Typescript

Наведені нижче приклади `Kit` демонструють рекомендований підхід із
використанням `@solana/kit`. Приклади застарілого підходу з використанням
