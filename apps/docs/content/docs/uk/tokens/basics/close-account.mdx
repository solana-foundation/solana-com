...
description:
  Закрийте токен-акаунт і поверніть лампорти акаунта на цільовий акаунт.
...
Закриття токен-акаунта видаляє акаунт і повертає лампорти rent токен-акаунта на
цільовий акаунт.

Баланс токенів має бути нульовим перед закриттям акаунта. Заморожений акаунт все
одно може бути закритий, якщо баланс токенів замороженого акаунта дорівнює нулю.
Токен-акаунти Wrapped SOL є винятком і можуть бути закриті з балансом токенів
для повернення базового SOL. Власник акаунта або повноважена особа на закриття
акаунта підписує інструкцію закриття.

## Як закрити токен-акаунт

Закриття токен-акаунта використовує інструкцію _rs`CloseAccount`_ Token Program.

Інструкція _rs`CloseAccount`_ передає лампорти вихідного акаунта на цільовий
акаунт, очищує вихідний акаунт і видаляє вихідний акаунт. Token Extension
...
`@solana/kit`. Для довідки наведено застарілі приклади з використанням
`@solana/web3.js`.
