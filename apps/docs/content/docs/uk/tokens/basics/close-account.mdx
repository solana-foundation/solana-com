---
title: Закриття токен-акаунта
description:
  Закрийте токен-акаунт і поверніть lamport акаунта на акаунт призначення.
...
Закриття token account видаляє обліковий запис і переказує всі його lamport,
включно з відшкодовуваним мінімальним балансом для зберігання, на цільовий обліковий запис.

Баланс токенів має бути нульовим, перш ніж можна буде закрити обліковий запис. Заморожений обліковий запис
все одно можна закрити, якщо баланс токенів на ньому дорівнює нулю. Виняток становлять token account для Wrapped SOL
— їх можна закрити з ненульовим балансом токенів, щоб повернути базовий SOL. Власник облікового запису або уповноважений на закриття
підписує інструкцію закриття й вибирає призначення. Обліковий запис, який сплатив
за створення associated token account, не отримує відновлений баланс автоматично.

## Як закрити токен-акаунт

Закриття токен-акаунта використовує інструкцію _rs`CloseAccount`_ Token Program.

Інструкція _rs`CloseAccount`_ переносить lamport вихідного акаунта на акаунт
