---
title: Mint токенів
description: Дізнайтеся, як створювати нові одиниці токена.
---

## Як створювати токени

Mint створює нові одиниці токена за допомогою інструкції
...
- Тільки mint authority може створювати нові токени
- Цільовий token account повинен існувати для отримання створених токенів
...
async fn main() -> Result<()> {
    // Створення з'єднання з локальним validator
    ...
    // Генерація нового keypair для платника комісій
    let fee_payer = Keypair::new();

    // Airdrop 1 SOL платнику комісій
    ...
    // Генерація keypair для використання як адреси mint
    let mint = Keypair::new();

    // Отримання стандартного розміру mint account (у байтах), без увімкнених розширень
    let mint_space = Mint::LEN;
    let mint_rent = client
        .get_minimum_balance_for_rent_exemption(mint_space)
        .await?;

    // Інструкція для створення нового акаунта для mint (token program)
    let create_account_instruction = create_account(
        &fee_payer.pubkey(), // платник
        &mint.pubkey(),      // новий акаунт (mint)
        mint_rent,           // lamports
        mint_space as u64,   // простір
        &token_program_id(), // program id
    );

    // Інструкція для ініціалізації даних mint account
    ...
    // Обчислюємо адресу associated token account для fee_payer
    let associated_token_address = get_associated_token_address_with_program_id(
        &fee_payer.pubkey(), // власник
        &mint.pubkey(),      // mint
        &token_program_id(), // program_id
    );

    // Інструкція для створення associated token account
    let create_ata_instruction = create_associated_token_account(
        &fee_payer.pubkey(), // адреса фінансування
        &fee_payer.pubkey(), // адреса гаманця
        &mint.pubkey(),      // адреса mint
        &token_program_id(), // program id
    );

    // Створюємо транзакцію та додаємо інструкції
    ...
    // Надсилаємо та підтверджуємо транзакцію
    client.send_and_confirm_transaction(&transaction).await?;

    // Кількість токенів для mint (100 токенів з 2 десятковими знаками)
    let amount = 100;

    // Створюємо інструкцію mint_to для mint токенів до associated token account
    let mint_to_instruction = mint_to(
        &token_program_id(),
        &mint.pubkey(),            // mint
        &associated_token_address, // призначення
        &fee_payer.pubkey(),       // authority
        &[&fee_payer.pubkey()],    // signer
        amount,                    // кількість
    )?;

    // Створюємо транзакцію для mint токенів
    let transaction = Transaction::new_signed_with_payer(
        &[mint_to_instruction],
        Some(&fee_payer.pubkey()),
        &[&fee_payer],
    ...
    println!("Mint 1.00 токенів до associated token account");
    println!("\nАдреса Mint: {}", mint.pubkey());
    println!("{:#?}", mint_data);

    println!(
        "\nАдреса Associated Token Account: {}",
        associated_token_address
    );
    println!("{:#?}", token_data);

    println!("Підпис транзакції: {}", transaction_signature);
...
async def main():
    ...
    # Сума для карбування (у найменших одиницях)
    amount_to_mint = 1000000000  # 1 токен з 9 десятковими знаками

    async with rpc:
        # Створити інструкцію карбування
        ...
        print(f"Призначення: {destination_token_account}")
        print(f"Сума: {amount_to_mint}")
        print(f"Повноваження карбування: {mint_authority.pubkey()}")
        print(f"Платник: {payer.pubkey()}")
        print(f"Транзакцію карбування успішно створено")
