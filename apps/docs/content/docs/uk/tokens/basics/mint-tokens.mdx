...
description:
  Карбування нових одиниць токенів на токен-акаунт за допомогою повноважень
  карбування.
...
Карбування створює нові одиниці існуючого токена і збільшує загальну пропозицію
карбувальника.

Лише повноваження карбування можуть карбувати нові токени. Цільовий токен-акаунт
повинен вже існувати і належати тому самому карбувальнику.
...
Наведені нижче приклади використовують _rs`MintToChecked`_, що вимагає від
викликача надати десяткові знаки карбувальника, щоб інструкція могла перевірити
очікувану точність токена перед карбуванням.

[Нативне карбування](/docs/tokens/basics/sync-native) не підтримує карбування.

### Довідка джерел

| Елемент               | Опис                                                                                                                                                              | Token Program                                                                                                   | Token Extensions Program                                                                                              |
| --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_            | Стан карбувальника зберігає повноваження карбування та загальну пропозицію токенів.                                                                               | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_         | Стан токен-акаунта зберігає цільовий баланс, оновлений в результаті карбування.                                                                                   | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`MintTo`_          | Інструкція карбування, яка карбує токени на існуючий токен-акаунт без вимоги до викликача надавати десяткові знаки карбувальника.                                 | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L159-L177) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L202-L220) |
| _rs`MintToChecked`_   | Інструкція карбування, яка вимагає від викликача надати десяткові знаки карбувальника і перевіряє це значення перед карбуванням токенів на існуючий токен-акаунт. | [Джерело](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L301-L325) | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L367-L391) |
