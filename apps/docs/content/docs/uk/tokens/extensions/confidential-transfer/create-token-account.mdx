...
description:
  Дізнайтеся, як створити token account з увімкненими Confidential Balances.
---

## Як створити token account з Confidential Balances

Confidential Balances забезпечує приватні перекази токенів, додаючи додатковий стан до
token account. У цьому розділі пояснюється, як створити token account із
увімкненим необхідним розширенням.

Наступна діаграма показує кроки, необхідні для створення token account із
увімкненими Confidential Balances:
...
### Стан token account для Confidential Balances
...
_rs`ConfidentialTransferAccount`_ містить кілька полів, що керують
Confidential Balances:

- **approved**: Статус підтвердження облікового запису для Confidential Balances. Якщо
  конфігурацію `auto_approve_new_accounts` у mint account встановлено як `true`, усі
  token accounts автоматично отримують підтвердження для Confidential Balances.
...
Для створення та налаштування token account для Confidential Balances використовуються
наступні інструкції, які вміщуються в одній транзакції:
...
4. **Налаштування Confidential Balances**: Викличте інструкцію
   ...
   Token Extension Program, посилаючись на обліковий запис стану контексту доведення через
   _rs`ProofLocation::ContextStateAccount`_, щоб ініціалізувати
   стан _rs`ConfidentialTransferAccount`_.

<Callout type="info">

**Лише власник token account може увімкнути Confidential Balances на token
account**.

Інструкція _rs`ConfigureAccount`_ вимагає генерації на стороні клієнта
ключів шифрування та доведення, яке може бути згенероване лише власником token account.

Доведення дійсності pubkey перевіряє, що публічний ключ ElGamal облікового запису є
дійсним. Воно генерується за допомогою _rs`build_pubkey_validity_proof_data`_, перевіряється
в мережі програмою ZK ElGamal Proof у обліковий запис стану контексту, а потім
на нього посилаються з _rs`ConfigureAccount`_ через
_rs`ProofLocation::ContextStateAccount`_, тому жодні байти доведення не передаються в самій
інструкції токена. Для отримання деталей реалізації дивіться:
...
Наступний код створює associated token account і налаштовує його для використання
Confidential Balances із наявним сумісним mint.

Confidential Balances залежить від програми ZK ElGamal Proof, яка увімкнена
в mainnet і devnet. Стандартний `solana-test-validator` не вмикає її, але
локальний validator із форком mainnet, наприклад [Surfpool](https://surfpool.run), вмикає.
Запустіть приклад проти одного з них (у коді використовується devnet) із фінансованим платником
та замініть заповнювач mint на mint, створений відповідно до
