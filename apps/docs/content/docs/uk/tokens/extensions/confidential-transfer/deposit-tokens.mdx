...
Конфіденційні перекази залежать від програми ZK ElGamal Proof, яка увімкнена у
mainnet та devnet. Стандартний `solana-test-validator` не вмикає її, але
локальний validator з форком mainnet, наприклад
[Surfpool](https://surfpool.run), підтримує це. Запустіть приклад на одній з цих
мереж (у коді використовується devnet) з поповненим платником, та замініть
заповнювачі адрес mint і акаунтів своїми власними.
