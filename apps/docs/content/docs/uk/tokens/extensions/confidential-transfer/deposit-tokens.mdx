...
необхідно конвертувати у конфіденційний баланс. Ця конвертація відбувається у
два етапи:

1. **Конфіденційний очікуваний баланс**: Спочатку токени «вносяться» з
...
Цей розділ пояснює перший етап: внесення публічного балансу токенів на
конфіденційний очікуваний баланс.

Наступна діаграма показує кроки, задіяні у процесі внесення токенів з публічного
...
на одну інструкцію внесення обмежена значенням 2^48.

Крейт `spl_token_client` надає метод `confidential_transfer_deposit`, який будує
та надсилає транзакцію з інструкцією `Deposit`, як показано у прикладі нижче.

## Приклад коду

Наступний приклад демонструє, як внести публічний баланс токенів на
конфіденційний очікуваний баланс.
...
локальний validator з форком mainnet, наприклад [Surfpool](https://surfpool.run), підтримує.
