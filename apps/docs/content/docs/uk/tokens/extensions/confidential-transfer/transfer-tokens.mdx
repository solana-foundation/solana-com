...
description:
  Дізнайтеся, як приватно переказувати токени з одного токен-акаунта на інший.
---

## Як конфіденційно перенести токени з одного token account на інший

Щоб конфіденційно переказати токени з одного token account на інший, і відправник, і отримувач повинні мати token account, налаштовані зі станом _rs`ConfidentialTransferAccount`_ та схвалені для використання конфіденційних балансів. Token account відправника також повинен мати доступний конфіденційний баланс для переказу.
...
1. Створіть
   ...
   **Доказ рівності
   ...
   гарантуючи, що новий доступний баланс вихідного рахунку правильно обчислено
   ...
   **Доказ дійсності шифротексту
   ...
   Перевіряє, що шифротексти суми переказу правильно сформовані для всіх трьох
   сторін (відправника, одержувача та аудитора), гарантуючи, що сума переказу
   правильно зашифрована під публічним ключем кожної сторони.
   ...
   **Доказ діапазону
   ...
   Перевіряє, що новий доступний баланс і сума переказу (розбита на
   молодші/старші біти) є невід'ємними та знаходяться в заданому діапазоні.

2. Для кожного доказу:
   - Викличте програму ZK ElGamal proof для перевірки даних доказу.
   - Збережіть специфічні для доказу метадані в обліковому записі "context
     state" для використання в інших інструкціях.
...
3. Викличте інструкцію
   ...
   надавши облікові записи контексту стану доказів.

4. Закрийте облікові записи контексту стану доказів, щоб повернути SOL,
   витрачений на їх створення.

Наступна діаграма показує кроки, пов'язані з переказом токенів з token account
відправника на token account одержувача.
...
- Згенерувати доказ рівності, доказ дійсності шифротексту та доказ діапазону на
  стороні клієнта
- Викликати програму Zk ElGamal proof для перевірки доказів та ініціалізації
  облікових записів "context state"
- Викликати інструкцію
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604),
  надавши три облікові записи доказів.
...
Наступний приклад конфіденційно переказує токени з одного облікового запису на інший. Обидва облікові записи вже повинні бути налаштовані для конфіденційних балансів, а відправник повинен мати доступний конфіденційний баланс.

Конфіденційні баланси залежать від програми ZK ElGamal Proof, яка увімкнена в mainnet і devnet. Стандартний `solana-test-validator` не вмикає її, але локальний validator з форком mainnet, наприклад [Surfpool](https://surfpool.run), підтримує це.
Запустіть приклад на одному з них (у коді використовується devnet) із фінансованим платником та замініть заповнювачі своїм mint і обліковими записами відправника та отримувача.
