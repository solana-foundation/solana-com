...
<Callout type="warn">
  ZK ElGamal Program тимчасово вимкнено в мейннеті та девнеті, оскільки
  проходить аудит безпеки. Це означає, що розширення конфіденційних переказів
  наразі недоступне. Хоча концепції залишаються актуальними, приклади коду не
  працюватимуть.
</Callout>

## Як конфіденційно переказати токени з одного токен-акаунта на інший

Щоб конфіденційно переказати токени з одного токен-акаунта на інший, і
відправник, і одержувач повинні мати токен-акаунти, налаштовані зі станом
_rs`ConfidentialTransferAccount`_ та схвалені для конфіденційних переказів.
Токен-акаунт відправника також повинен мати доступний конфіденційний баланс для
переказу.
...
1. Створіть
   ...
   **Доказ рівності
   ...
   гарантуючи, що новий доступний баланс вихідного акаунта правильно обчислено
   як `new_balance = current_balance - transfer_amount`.

   **Доказ валідності шифротексту
   ...
   сторін (джерело, призначення та аудитор), гарантуючи, що сума переказу
   ...
   **Доказ діапазону
   ...
   молодші/старші біти) є невід'ємними та знаходяться в межах вказаного
   діапазону.

2. Для кожного доказу:
   - Викличте програму ZK ElGamal proof для перевірки даних доказу.
   - Збережіть метадані, специфічні для доказу, в акаунті «контекстного стану
...
3. Викличте інструкцію
   ...
   надавши акаунти контекстного стану доказу.

4. Закрийте акаунти стану контексту доказу, щоб повернути SOL, використані для
   їх створення.

Наступна діаграма показує кроки, задіяні в переказі токенів з token account
...
Щоб конфіденційно переказати токени з одного token account на інший, ви повинні:

- Згенерувати доказ рівності, доказ валідності шифротексту та доказ діапазону на
  стороні клієнта
- Викликати програму Zk ElGamal proof для перевірки доказів та ініціалізації
  акаунтів "стану контексту"
- Викликати інструкцію
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604),
  надавши три акаунти доказів.
- Закрити три акаунти доказів, щоб повернути rent.

Крейт `spl_token_client` надає наступні методи:
...
Наступний приклад демонструє, як конфіденційно переказувати токени між token
accounts.

Щоб запустити приклад, запустіть локальний validator з Token Extension Program,
клонованою з mainnet, використовуючи наступну команду. Ви повинні мати
