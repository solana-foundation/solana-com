...
mainnet та devnet. Стандартний `solana-test-validator` не вмикає її, але
локальний validator з форком mainnet, наприклад
[Surfpool](https://surfpool.run), це робить. Запустіть приклад на одній з цих
мереж (у коді використовується devnet) з поповненим рахунком платника та
замініть заповнювачі адрес mint і акаунтів на власні.
