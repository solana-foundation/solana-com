...
додатковий стан до облікового запису мінту. У цьому розділі пояснюється, як
створити токен мінт з увімкненим цим розширенням.

Наступна діаграма показує кроки, необхідні для створення мінту з розширенням
...
### Стан Confidential Transfer мінту
...
_rs`ConfidentialTransferMint`_ містить три конфігураційні поля:
...
- **auto_approve_new_accounts**: коли встановлено значення true, користувачі
  можуть створювати токен акаунти з увімкненими конфіденційними переказами за
  замовчуванням. Коли false, authority повинен схвалити кожен новий токен акаунт
  перед тим, як його можна буде використовувати для конфіденційних переказів.

- **auditor_elgamal_pubkey**: опціональний аудитор, який може розшифровувати
  суми переказів у конфіденційних транзакціях, забезпечуючи механізм
  відповідності вимогам, зберігаючи при цьому конфіденційність від широкої
  публіки.
...
1. **Створення облікового запису мінту**: викликати інструкцію
   _rs`CreateAccount`_ System Program для створення облікового запису мінту.
...
2. **Ініціалізація розширення Confidential Transfer**: Викликати інструкцію
   ...
   програми Token Extension для налаштування стану
   _rs`ConfidentialTransferMint`_ для мінту.

3. **Ініціалізація мінту**: Викликати інструкцію
   _rs`Instruction::InitializeMint`_ програми Token Extension для ініціалізації
...
Наступний код демонструє, як створити мінт з розширенням Confidential Transfer.

Щоб запустити приклад, запустіть локальний validator з програмою Token
Extension, клонованою з mainnet, використовуючи наступну команду. Для запуску
...
<Callout type="info">
  ...
  локальному validator. Ви повинні клонувати програму Token Extension з mainnet,
