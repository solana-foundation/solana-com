...
description:
  Дізнайтеся, як створити карбувальник токенів із увімкненими Confidential Balances.
---

## Як створити карбувальник із Confidential Balances

Confidential Balances забезпечує приватні перекази токенів, додаючи додатковий стан до
mint account. У цьому розділі пояснюється, як створити карбувальник токенів із
увімкненим необхідним розширенням.

Наступна діаграма показує кроки, пов'язані зі створенням карбувальника з
розширенням Confidential Balances:
...
### Стан карбувальника Confidential Balances
...
- **authority**: Обліковий запис, який має дозвіл на зміну налаштувань Confidential Balances
  для карбувальника та затвердження нових конфіденційних облікових записів, якщо автоматичне
  затвердження вимкнено.

- **auto_approve_new_accounts**: Якщо встановлено значення true, користувачі можуть створювати token
  accounts із увімкненими Confidential Balances за замовчуванням. Якщо false, authority
  повинен затвердити кожен новий token account, перш ніж його можна буде використовувати для
...
Для створення карбувальника з увімкненими Confidential Balances потрібні три інструкції
в одній транзакції:

1. **Створення Mint Account**: Викличте інструкцію _rs`CreateAccount`_ System
   Program для створення mint account.

2. **Ініціалізація Confidential Balances**: Викличте інструкцію Token Extensions Program
   ...
   для налаштування стану _rs`ConfidentialTransferMint`_ для
   карбувальника.
...
Наступний код демонструє, як створити карбувальник із увімкненими Confidential Balances.

Confidential Balances залежить від програми ZK ElGamal Proof, яка увімкнена
на mainnet і devnet. Стандартний `solana-test-validator` не вмикає її, але
локальний validator із форком mainnet, наприклад [Surfpool](https://surfpool.run), підтримує це.
Запустіть приклад проти одного з них (у коді використовується devnet) із фінансованим платником
та замініть адреси-заповнювачі карбувальника й облікового запису на власні.
