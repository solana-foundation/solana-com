...
<Callout type="warn">
  ZK ElGamal Program тимчасово вимкнено на mainnet та devnet, оскільки проходить
  аудит безпеки. Це означає, що розширення конфіденційних переказів наразі
  недоступне. Хоча концепції залишаються актуальними, приклади коду не
  працюватимуть.
</Callout>

## Що таке конфіденційні перекази?

Конфіденційні перекази дозволяють переказувати токени між token account без
розкриття суми переказу. Це корисно для транзакцій, що зберігають
конфіденційність. Лише суми переказів та баланси токенів є приватними. Адреси
...
до програми Token Extension, які дозволяють переказувати токени між account без
...
Базовий процес конфіденційних переказів токенів виглядає наступним чином:

1. Створіть mint account з розширенням конфіденційних переказів.
2. Створіть token account з розширенням конфіденційних переказів для відправника
   та одержувача.
3. Створіть токени на рахунок відправника.
4. **Депонуйте** публічний баланс відправника до **конфіденційного очікуваного
   балансу**.
5. **Застосуйте** очікуваний баланс відправника до **конфіденційного доступного
   балансу**.
6. Конфіденційно **переказуйте** токени з token account відправника на token
   account одержувача.
7. **Застосуйте** очікуваний баланс одержувача до **конфіденційного доступного
   балансу**.
8. **Виведіть** конфіденційний доступний баланс одержувача до **публічного
   балансу**.

Для отримання детальної інформації про кроки в процесі конфіденційного переказу
дивіться відповідні сторінки:
...
<Cards>
  ...
  >
    Як створити mint account з розширенням конфіденційного переказу
  </Card>
  <Card
    title="Створення token account"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Як налаштувати token account з розширенням конфіденційного переказу
  </Card>
  <Card
...
Діаграма нижче показує детальну послідовність базового процесу конфіденційних
переказів токенів:
...
клонованою з mainnet, використовуючи наступну команду. У вас має бути
встановлений Solana CLI для використання локального validator.
...
<Callout type="info">
  На момент написання конфіденційні перекази не ввімкнені на стандартному
  локальному validator. Ви повинні клонувати Token Extension Program з mainnet,
...
розширення конфіденційного переказу наступний:

| Інструкція                          | Опис                                                                                                                                                                   |
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Налаштовує mint account для конфіденційних переказів. Ця інструкція має бути включена в ту саму транзакцію, що й інструкція _rs`TokenInstruction::InitializeMint`_.    |
...
| _rs`ApproveAccount`_                | Схвалює token account для конфіденційних переказів, якщо mint вимагає схвалення для нових token accounts.                                                              |
| _rs`EmptyAccount`_                  | Очищає очікуваний та доступний конфіденційні баланси, щоб дозволити закриття token account.                                                                            |
...
| _rs`ApplyPendingBalance`_           | Конвертує очікуваний баланс у доступний баланс після внесень або переказів.                                                                                            |
| _rs`EnableConfidentialCredits`_     | Дозволяє token account отримувати конфіденційні перекази токенів.                                                                                                      |
| _rs`DisableConfidentialCredits`_    | Блокує вхідні конфіденційні перекази, але все ще дозволяє публічні перекази.                                                                                           |
...
| _rs`ConfigureAccountWithRegistry`_  | Альтернативний спосіб налаштування token accounts для конфіденційних переказів з використанням _rs`ElGamalRegistry`_ account замість _rs`VerifyPubkeyValidity`_ proof. |
