...
1. Створіть
   ...
   **Доказ рівності
   ...
   Перевіряє, що шифротекст залишку доступного балансу після виведення
   відповідає відповідному
   [зобов'язанню Педерсена](https://en.wikipedia.org/wiki/Commitment_scheme),
   гарантуючи, що новий доступний баланс рахунку правильно обчислюється як
   ...
   **Доказ діапазону
   ...
   Перевіряє, що залишок доступного балансу після виведення є невід'ємним і
   знаходиться у вказаному діапазоні.

2. Для кожного доказу:
   - Викличте програму доказів ZK ElGamal для перевірки даних доказу.
   - Збережіть метадані, специфічні для доказу, в обліковому записі
     "контекстного стану" доказу для використання в інших інструкціях.
...
3. Викличте інструкцію
   ...
   надавши два облікові записи доказів.

4. Закрийте облікові записи доказів, щоб повернути SOL, використаний для їх
   створення.

Наступна діаграма показує кроки, задіяні у виведенні токенів з конфіденційного
доступного балансу на публічний баланс:
...
- Викликати програму доказів Zk ElGamal для перевірки доказів та ініціалізації
  облікових записів "контекстного стану"
- Викликати інструкцію
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493),
  надавши два облікові записи доказів.
- Закрити два облікові записи доказів для повернення rent.

Наведений нижче приклад на Rust генерує докази за допомогою крейта
`spl-token-confidential-transfer-proof-generation`, перевіряє кожен у обліковому
записі контекстного стану через програму доказів ZK ElGamal, посилається на
обидва облікові записи в інструкції виведення та закриває їх після цього.
Приклад на TypeScript використовує допоміжну функцію
`getConfidentialWithdrawInstructionPlan` з
`@solana-program/token-2022/confidential`, яка збирає ті самі облікові записи
доказів, виведення та закриття як план інструкцій із декількох транзакцій.

## Приклад коду

Наступний приклад виводить токени з конфіденційного доступного балансу
назад до публічного балансу. Обліковий запис вже має бути налаштований для
Конфіденційних балансів і мати доступний конфіденційний баланс.

Конфіденційні баланси залежать від програми ZK ElGamal Proof, яка увімкнена
в mainnet та devnet. Стандартний `solana-test-validator` не вмикає її, але
локальний validator з форком mainnet, наприклад [Surfpool](https://surfpool.run), підтримує це.
Запустіть приклад на одному з них (у коді використовується devnet) із фінансованим платником
та замініть заповнювачі своїм mint і token account.
