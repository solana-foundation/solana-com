...
description:
  Налаштуйте mint Token Extension Program так, щоб новостворені токен-акаунти
  починали роботу в обраному стандартному стані, наприклад, замороженими.
...
Розширення mint _rs`DefaultAccountState`_ Token Extension Program встановлює
початковий _rs`AccountState`_ для кожного нового токен-акаунта, створеного для
цього mint.

Найчастіше це використовується для того, щоб нові токен-акаунти починали роботу
замороженими. Токен-акаунти, які починають роботу замороженими, не можуть
використовуватися, доки їх не розморозить freeze authority mint.
...
2. Створіть mint account за допомогою _rs`CreateAccount`_, ініціалізуйте
   _rs`DefaultAccountState`_ та ініціалізуйте mint за допомогою
   _rs`InitializeMint`_.
3. Створіть токен-акаунти для mint та переконайтеся, що кожен новий токен-акаунт
   починає роботу в поточному стандартному стані.
4. Використовуйте _rs`UpdateDefaultAccountState`_, щоб змінити стан, у якому
   повинні починати роботу майбутні токен-акаунти.
...
_rs`DefaultAccountState`_. Це розмір, який використовується в
_rs`CreateAccount`_.
...
## !!steps Ініціалізація DefaultAccountState

Ініціалізуйте розширення _rs`DefaultAccountState`_ для mint.
...
## !!steps Створення токен-акаунта

Створіть токен-акаунт для mint. Оскільки поточний стандартний стан —
_rs`Frozen`_, новий токен-акаунт починається заморожений.
...
## !!steps Оновлення стандартного стану акаунта

Використовуйте _rs`UpdateDefaultAccountState`_, щоб змінити стандартний стан, і
майбутні токен-акаунти більше не почнуться заморожені.
...
<Callout type="info" title="Порядок інструкцій">
  _rs`DefaultAccountStateInstruction::Initialize`_ повинна йти перед
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`DefaultAccountStateInstruction::Initialize`_ та _rs`InitializeMint`_ мають
...
| ------------------------------------------------ | -------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`DefaultAccountState`_                        | Розширення mint, яке зберігає стан, який мають успадковувати нові токен-акаунти.                   | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/mod.rs#L16-L21)         |
| _rs`DefaultAccountStateInstruction::Initialize`_ | Інструкція, яка ініціалізує стандартний стан токен-акаунта mint перед _rs`InitializeMint`_.        | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L20-L36) |
| _rs`DefaultAccountStateInstruction::Update`_     | Інструкція, яка дозволяє freeze authority змінювати стандартний стан для майбутніх токен-акаунтів. | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L37-L53) |
| _rs`process_initialize_default_account_state`_   | Логіка процесора, яка записує початковий стандартний стан на неініціалізований mint.               | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L29-L40)     |
| _rs`process_update_default_account_state`_       | Логіка процесора, яка перевіряє freeze authority перед зміною збереженого стандартного стану mint. | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L43-L74)     |
