...
description:
  Створіть мінт Token Extensions Program з постійним делегатом, який може
  переводити або спалювати токени з будь-якого облікового запису для цього
  мінту.
...
## Що таке постійний делегат?

Розширення мінту _rs`PermanentDelegate`_ Token Extensions Program призначає
повноваження делегата самому мінту.

Постійний делегат — це повноваження на рівні мінту, яке може авторизувати
перекази та спалювання для будь-якого token account цього мінту. Він діє як
глобальний делегат для всіх token account мінту, і власники token account не
можуть відкликати постійного делегата зі своїх token account.

Повноваження мінту також можуть згодом змінити постійного делегата за допомогою
_rs`SetAuthority`_, використовуючи _rs`AuthorityType::PermanentDelegate`_.

## Як створити та використовувати постійного делегата

Щоб створити та використовувати постійного делегата:

1. Обчисліть розмір облікового запису мінту та rent, необхідні для мінту та
   розширення _rs`PermanentDelegate`_.
2. Створіть обліковий запис мінту за допомогою _rs`CreateAccount`_,
   ініціалізуйте _rs`PermanentDelegate`_ та ініціалізуйте мінт за допомогою
   _rs`InitializeMint`_.
3. Надішліть _rs`TransferChecked`_ або _rs`BurnChecked`_, підписані постійним
   делегатом. Token Program обробляє ці інструкції так само, як якби їх підписав
   власник token account.

<ScrollyCoding>

## !!steps Обчислити розмір облікового запису

Обчисліть розмір облікового запису мінту для базового мінту плюс розширення
_rs`PermanentDelegate`_. Це розмір, який використовується в _rs`CreateAccount`_.
...
Обчисліть rent, використовуючи розмір, необхідний для мінту плюс розширення
...
## !!steps Створити обліковий запис мінту
...
## !!steps Ініціалізація PermanentDelegate

Ініціалізуйте розширення _rs`PermanentDelegate`_ на mint.
...
Створіть token account для власника та одержувача, потім викарбуйте токени на
token account власника.
...
## !!steps Передача з постійним делегатом

Передайте токени за допомогою _rs`TransferChecked`_, підписаного постійним
делегатом.
...
<Callout type="info" title="Порядок інструкцій">
  _rs`InitializePermanentDelegate`_ має йти перед _rs`InitializeMint`_.
  _rs`CreateAccount`_, _rs`InitializePermanentDelegate`_ та _rs`InitializeMint`_
...
| ------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`PermanentDelegate`_                     | Розширення mint, яке зберігає делегата, уповноваженого для кожного token account для mint.                             | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/permanent_delegate.rs#L15-L21) |
| _rs`InitializePermanentDelegate`_           | Інструкція, яка ініціалізує постійного делегата перед _rs`InitializeMint`_.                                            | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1952-L1967)              |
| _rs`SetAuthority`_                          | Базова інструкція токена, що використовується з _rs`AuthorityType::PermanentDelegate`_ для зміни делегата.             | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1488-L1520)              |
| _rs`AuthorityType::PermanentDelegate`_      | Дискримінатор повноважень, що використовується з _rs`SetAuthority`_ для зміни постійного делегата на mint.             | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1138-L1139)              |
| _rs`process_initialize_permanent_delegate`_ | Логіка процесора, яка записує розширення постійного делегата на mint.                                                  | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1542-L1557)                  |
| _rs`process_set_authority`_                 | Логіка процесора, яка перевіряє та змінює постійного делегата при використанні _rs`AuthorityType::PermanentDelegate`_. | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L849-L860)                    |

### Typescript

Приклад `Kit` нижче використовує згенеровані інструкції безпосередньо. Застарілі
приклади з використанням `@solana/web3.js` та `@solana/spl-token` наведені для
довідки.
