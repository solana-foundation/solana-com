---
title: Призупинення карбування
description:
  Створіть мінт Token-2022 з повноваженнями паузи, блокуйте перекази під час
  паузи мінта та відновлюйте активність пізніше.
...
Розширення мінта _rs`PausableConfig`_ програми Token Extension надає
...
1. Розрахуйте розмір облікового запису мінта та необхідну ренту для мінта та
   розширення _rs`PausableConfig`_.
2. Створіть обліковий запис мінта за допомогою _rs`CreateAccount`_,
   ініціалізуйте _rs`PausableConfig`_ та ініціалізуйте мінт за допомогою
   _rs`InitializeMint`_.
3. Змінтіть токени та створіть токен-акаунти для мінта.
4. Використовуйте _rs`Pause`_, щоб заблокувати активність токенів для мінта.
5. Використовуйте _rs`Resume`_, щоб знову дозволити активність токенів.

<ScrollyCoding>

## !!steps Розрахуйте розмір облікового запису

Розрахуйте розмір облікового запису мінта для базового мінта плюс розширення
_rs`PausableConfig`_. Це розмір, який використовується в _rs`CreateAccount`_.
...
## !!steps Створіть обліковий запис мінта
...
## !!steps Ініціалізація PausableConfig

Ініціалізуйте розширення _rs`PausableConfig`_ на mint.
...
## !!steps Призупинення mint

Призупиніть mint за допомогою _rs`Pause`_.
...
## !!steps Відновлення mint

Відновіть mint за допомогою _rs`Resume`_.
...
<Callout type="info" title="Порядок інструкцій">
  _rs`PausableInstruction::Initialize`_ повинен виконуватись перед
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`PausableInstruction::Initialize`_ та _rs`InitializeMint`_ повинні бути
...
| ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PausableConfig`_                  | Розширення mint, яке зберігає повноваження на призупинення та поточний стан призупинення mint.                             | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/mod.rs#L17-L22)         |
| _rs`PausableInstruction::Initialize`_ | Інструкція, яка ініціалізує pausable config перед _rs`InitializeMint`_.                                                    | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L20-L32) |
| _rs`PausableInstruction::Pause`_      | Інструкція, яка позначає mint як призупинений.                                                                             | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L33-L44) |
| _rs`PausableInstruction::Resume`_     | Інструкція, яка позначає mint як активний.                                                                                 | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L45-L56) |
| _rs`process_initialize`_              | Логіка процесора, яка ініціалізує _rs`PausableConfig`_ на неініціалізованому mint і зберігає повноваження на призупинення. | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L22-L35)     |
| _rs`process_toggle_pause`_            | Логіка процесора, яка перевіряє повноваження на призупинення перед перемиканням прапорця призупинення mint.                | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L39-L64)     |
