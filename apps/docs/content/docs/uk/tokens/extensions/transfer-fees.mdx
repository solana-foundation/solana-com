...
description:
  Створіть mint з комісіями за переказ, перемістіть утримані комісії з token
  accounts до mint та виведіть їх на token account отримувача комісій.
...
Розширення mint _rs`TransferFeeConfig`_ Token Extensions Program застосовує
комісію до кожного переказу для цього mint.

Під час переказу:

- Сума переказу зменшується на налаштовану комісію
- Утримана комісія відстежується на token account призначення
- Утримані комісії можна вивести безпосередньо з token accounts або зібрати до
  mint
- `withdraw_withheld_authority` може перемістити зібрані комісії на token
  account отримувача комісій

<Callout type="info" title="Розширення Token Account">
  Коли token account ініціалізується для mint з _rs`TransferFeeConfig`_, token
  account ініціалізується з _rs`TransferFeeAmount`_. Коли token account
  створюється через [Associated Token
  Program](/docs/tokens/basics/create-token-account#how-to-create-an-associated-token-account),
  необхідний розмір облікового запису обчислюється, і token account створюється
  з необхідним розміром та lamports для звільнення від rent.
</Callout>
...
1. Обчисліть розмір облікового запису mint та необхідний rent для mint та
   розширення _rs`TransferFeeConfig`_.
2. Створіть обліковий запис mint за допомогою _rs`CreateAccount`_, ініціалізуйте
   _rs`TransferFeeConfig`_ та ініціалізуйте mint за допомогою
   _rs`InitializeMint`_.
3. Створіть token accounts для mint. _rs`TransferFeeAmount`_ автоматично
   активується для token accounts.
4. Використовуйте _rs`TransferCheckedWithFee`_ для переказу токенів та перевірки
   очікуваної комісії. _rs`TransferChecked`_ також працює та автоматично утримує
   налаштовану комісію на token account призначення.
5. Використовуйте _rs`WithdrawWithheldTokensFromAccounts`_ для переміщення
   утриманих комісій безпосередньо з token accounts на token account отримувача
   комісій, підписаний `withdraw_withheld_authority` mint.
6. Використовуйте _rs`HarvestWithheldTokensToMint`_ без дозволів для переміщення
   утриманих комісій з token accounts на обліковий запис mint, потім
   використовуйте _rs`WithdrawWithheldTokensFromMint`_ для виведення утриманих
   комісій облікового запису mint на token account отримувача комісій,
   підписаний `withdraw_withheld_authority` mint.
7. Використовуйте _rs`SetTransferFee`_ для оновлення наступної конфігурації
   комісій за переказ, яка набуває чинності через дві epochs.

<ScrollyCoding>

## !!steps Обчислення розміру облікового запису

Обчисліть розмір mint account для базового mint плюс розширення
_rs`TransferFeeConfig`_. Це розмір, який використовується в _rs`CreateAccount`_.
...
Обчисліть rent, використовуючи розмір, необхідний для mint плюс розширення
...
## !!steps Ініціалізація TransferFeeConfig

Ініціалізуйте розширення _rs`TransferFeeConfig`_ на mint.
...
Ініціалізуйте mint з _rs`InitializeMint`_ в тій самій транзакції.
...
## !!steps Створення token accounts і карбування токенів

Створіть token accounts для джерела, одержувачів і отримувача комісії, потім
викарбуйте токени на token account джерела.
...
## !!steps Переказ з TransferCheckedWithFee

Переказуйте токени за допомогою _rs`TransferCheckedWithFee`_ та перевірте
очікувану комісію.
...
## !!steps Переказ з TransferChecked

Переказуйте токени за допомогою _rs`TransferChecked`_. Налаштована комісія все
одно утримується на цільовому обліковому записі токенів.
...
## !!steps Вилучення утриманих комісій з облікових записів токенів

Використовуйте _rs`WithdrawWithheldTokensFromAccounts`_, щоб перемістити
утримані комісії безпосередньо з облікових записів токенів на обліковий запис
токенів одержувача комісій.
...
## !!steps Збір утриманих комісій на mint

Використовуйте _rs`HarvestWithheldTokensToMint`_, щоб перемістити утримані
комісії з облікових записів токенів до акумулятора утриманих комісій облікового
запису mint.
...
## !!steps Вилучення утриманих комісій з mint

Використовуйте _rs`WithdrawWithheldTokensFromMint`_, щоб перемістити зібрані
комісії з облікового запису mint на обліковий запис токенів одержувача комісій.
...
## !!steps Оновлення наступної комісії за переказ

Використовуйте _rs`SetTransferFee`_, щоб оновити конфігурацію наступної комісії
за переказ.
...
<Callout type="info" title="Порядок інструкцій">
  _rs`InitializeTransferFeeConfig`_ має йти перед _rs`InitializeMint`_.
  _rs`CreateAccount`_, _rs`InitializeTransferFeeConfig`_ та _rs`InitializeMint`_
  мають бути включені в одну транзакцію.
</Callout>
...
| ---------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`TransferFeeConfig`_                                          | Розширення mint, яке зберігає авторитети комісій, накопичену утриману суму, а також старішу та новішу конфігурації комісій за переказ.                                                                                                  | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L128-L145)         |
| _rs`TransferFee`_                                                | Структура конфігурації комісії, що зберігається в _rs`TransferFeeConfig`_ і визначає епоху, коли комісія набуває чинності, максимальну комісію та комісію в базисних пунктах.                                                           | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L32-L40)           |
| _rs`TransferFeeAmount`_                                          | Розширення облікового запису токенів, яке зберігає суму, утриману під час переказів для облікового запису токенів.                                                                                                                      | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L169-L177)         |
| _rs`TransferFeeInstruction::InitializeTransferFeeConfig`_        | Інструкція, яка ініціалізує налаштування комісії за переказ перед _rs`InitializeMint`_.                                                                                                                                                 | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L24-L48)   |
| _rs`TransferFeeInstruction::TransferCheckedWithFee`_             | Інструкція переказу, яка перевіряє, що надана викликачем комісія відповідає поточній конфігурації комісії за переказ mint.                                                                                                              | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L49-L80)   |
| _rs`TransferFeeInstruction::WithdrawWithheldTokensFromAccounts`_ | Інструкція, яка переміщує утримані комісії безпосередньо з облікових записів токенів до облікового запису токенів отримувача комісій, підписана `withdraw_withheld_authority` mint.                                                     | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L99-L122)  |
| _rs`TransferFeeInstruction::HarvestWithheldTokensToMint`_        | Інструкція без дозволу, яка переміщує утримані комісії з облікових записів токенів до акумулятора утриманих коштів облікового запису mint.                                                                                              | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L123-L134) |
| _rs`TransferFeeInstruction::WithdrawWithheldTokensFromMint`_     | Інструкція, яка виводить утримані комісії облікового запису mint до облікового запису токенів отримувача комісій, підписана `withdraw_withheld_authority` mint.                                                                         | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L81-L98)   |
| _rs`TransferFeeInstruction::SetTransferFee`_                     | Інструкція, яка оновлює новішу конфігурацію комісії за переказ, що набуває чинності через дві епохи.                                                                                                                                    | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L135-L154) |
| _rs`process_initialize_transfer_fee_config`_                     | Логіка процесора, яка ініціалізує розширення _rs`TransferFeeConfig`_ на неініціалізованому mint і встановлює як старішу, так і новішу конфігурації комісій за переказ на початкову комісію.                                             | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L26-L58)       |
| _rs`process_withdraw_withheld_tokens_from_accounts`_             | Логіка процесора, яка перевіряє `withdraw_withheld_authority` і переміщує утримані комісії з облікових записів токенів до цільового облікового запису токенів.                                                                          | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L208-L278)     |
| _rs`process_harvest_withheld_tokens_to_mint`_                    | Логіка процесора, яка збирає утримані комісії з облікових записів токенів до акумулятора утриманих коштів облікового запису mint.                                                                                                       | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L181-L206)     |
| _rs`process_withdraw_withheld_tokens_from_mint`_                 | Логіка процесора, яка перевіряє `withdraw_withheld_authority` і переміщує утриману суму облікового запису mint до цільового облікового запису токенів.                                                                                  | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L114-L158)     |
| _rs`process_set_transfer_fee`_                                   | Логіка процесора, яка перевіряє авторитет конфігурації комісії за переказ і оновлює новішу конфігурацію комісії за переказ, щоб вона набула чинності через дві епохи.                                                                   | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L61-L112)      |
| _rs`get_required_init_account_extensions`_                       | Використовується для автоматичного додавання розширень облікового запису токенів при ініціалізації облікових записів токенів на основі розширень, увімкнених на mint. Для mint з _rs`TransferFeeConfig`_ додає _rs`TransferFeeAmount`_. | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mod.rs#L1301-L1303)                    |

### Typescript

Наведений нижче приклад `Kit` використовує явні інструкції Token-2022. Застарілі
