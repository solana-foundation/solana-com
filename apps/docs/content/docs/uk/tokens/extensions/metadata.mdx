...
description:
  Створіть mint, що зберігає метадані безпосередньо на mint account, використовуючи розширення MetadataPointer і TokenMetadata, а потім оновлюйте, видаляйте та блокуйте ці метадані через інтерфейс token-metadata.
...
приклади з використанням `@solana/web3.js`, `@solana/spl-token` та
`@solana/spl-token-metadata` включено для довідки. У застарілих прикладах
`maxSupportedTransactionVersion` обмежено значенням `0`, оскільки web3.js v1 не може десеріалізувати
[транзакції v1](/docs/core/transactions/versioned-transactions#v1-message-format).
