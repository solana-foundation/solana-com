...
description:
  Створіть mint з процентною ставкою в Token Extensions Program, оновіть цю ставку та конвертуйте суми токенів у суму для інтерфейсу з урахуванням процентів.
...
## Що таке процентний mint?

Розширення mint _rs`InterestBearingConfig`_ у Token Extensions Program дозволяє
mint зберігати річну процентну ставку безпосередньо в блокчейні.

Процентні токени **не** додають більше токенів до token account з часом. Сума
токенів, що зберігається в кожному token account, залишається незмінною до тих
пір, поки інструкція токен-програми не змінить її, наприклад, при карбуванні,
переказі або спалюванні.

З плином часу розрахована сума для інтерфейсу з урахуванням процентів може
зростати, навіть якщо сума токенів і загальна пропозиція залишаються незмінними.

## Як розраховуються історичні зміни ставки
...
- [`InterestBearingConfig`](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L40-L50)
  зберігає `initialization_timestamp`, `pre_update_average_rate`,
  `last_update_timestamp` та `current_rate`.
- [`amount_to_ui_amount`](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L85-L94)
  обчислює суму для інтерфейсу у два етапи: спочатку обчислює проценти від
  ініціалізації до `last_update_timestamp`, використовуючи
  `pre_update_average_rate`, потім обчислює проценти від `last_update_timestamp`
  до поточної мітки часу, використовуючи `current_rate`.
- Коли виконується
  ...
  і оновлює `pre_update_average_rate` до нового середньозваженого за часом
  історичного значення, потім записує нові `last_update_timestamp` та
  `current_rate`.
- Після змін ставки з часом, попередні зміни представлені
  `pre_update_average_rate`, тоді як період від `last_update_timestamp` до
  поточного часу обчислюється з `current_rate`.

## Як створити та використовувати процентні токени

Щоб створити та використовувати процентні токени:

1. Обчисліть розмір mint account та необхідну суму rent для mint і розширення
   _rs`InterestBearingConfig`_.
2. Створіть mint account за допомогою _rs`CreateAccount`_, ініціалізуйте
   _rs`InterestBearingConfig`_ та ініціалізуйте mint за допомогою
   _rs`InitializeMint`_.
3. Карбуйте токени як зазвичай.
4. Використовуйте _rs`UpdateRate`_, щоб змінювати поточну ставку mint з часом.
5. Конвертуйте суми токенів у суму для інтерфейсу з урахуванням процентів за
   допомогою _rs`AmountToUiAmount`_ або допоміжних методів, які завантажують
   mint account та системну змінну clock, а потім обчислюють суму для інтерфейсу
   без відправки транзакції.

<Callout type="info" title="Конвертація суми UI офчейн та ончейн">
  Офчейн допоміжний шлях отримує mint account та clock sysvar, потім обчислює
  суму UI локально без відправки транзакції. Ончейн шлях використовує
  _rs`AmountToUiAmount`_, який виконується в Token Program та повертає суму UI у
  даних повернення транзакції. _rs`AmountToUiAmount`_ може бути відправлений у
  транзакції або симульований.
</Callout>

<ScrollyCoding>

## !!steps Обчислення розміру акаунта

Обчисліть розмір mint account для базового mint плюс _rs`InterestBearingConfig`_
розширення. Це розмір, який використовується в _rs`CreateAccount`_.
...
Обчисліть rent, використовуючи розмір, необхідний для mint плюс
_rs`InterestBearingConfig`_ розширення.
...
## !!steps Ініціалізація InterestBearingConfig

Ініціалізуйте _rs`InterestBearingConfig`_ розширення на mint.
...
## !!steps Створення token account та карбування токенів

Створіть token account для mint, потім карбуйте токени на цей token account.
...
## !!steps Обчислення суми UI за допомогою офлайн-помічника

Отримайте mint account та clock sysvar, а потім обчисліть суму UI без
...
<Callout type="info" title="Порядок інструкцій">
  _rs`InterestBearingMintInstruction::Initialize`_ має передувати
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`InterestBearingMintInstruction::Initialize`_ та _rs`InitializeMint`_ мають
...
| ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InterestBearingConfig`_                      | Розширення mint, яке зберігає орган процентної ставки, часові мітки та поточні й історичні ставки.                         | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L40-L153)         |
| _rs`InterestBearingMintInstruction::Initialize`_ | Інструкція, яка ініціалізує конфігурацію процентної ставки перед _rs`InitializeMint`_.                                         | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/instruction.rs#L17-L90)  |
| _rs`InterestBearingMintInstruction::UpdateRate`_ | Інструкція, яка змінює поточну процентну ставку mint.                                                                    | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/instruction.rs#L35-L113) |
| _rs`AmountToUiAmount`_                           | Інструкція, яка повертає поточний рядок суми UI для суми токенів, використовуючи активну конфігурацію mint.                      | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L520-L538)                                |
| _rs`InterestBearingConfig::amount_to_ui_amount`_ | Помічник, який перетворює суму токенів у суму UI з врахуванням процентів для часової мітки.                                           | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L85-L96)          |
| _rs`process_initialize`_                         | Логіка процесора, яка ініціалізує _rs`InterestBearingConfig`_ та записує початкові часові мітки і ставку.                     | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/interest_bearing_mint/processor.rs#L21-L43)      |
| _rs`process_update_rate`_                        | Логіка процесора, яка оновлює поточну ставку, перераховує середньозважену ставку та записує часову мітку оновлення. | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/interest_bearing_mint/processor.rs#L46-L77)      |
| _rs`process_amount_to_ui_amount`_                | Логіка процесора, яка повертає рядок суми UI, використовуючи активну конфігурацію процентної ставки mint.                     | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1548-L1569)                                  |

### Typescript

Приклад `Kit` нижче використовує згенеровані інструкції безпосередньо. Застарілі
приклади з використанням `@solana/web3.js` та `@solana/spl-token` наведено для
довідки.
