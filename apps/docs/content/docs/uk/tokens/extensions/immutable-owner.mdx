...
description:
  Ініціалізуйте обліковий запис токена Token Extensions Program з незмінним
  власником, щоб його власника ніколи не можна було переназначити.
...
## Що таке незмінний власник?

Розширення облікового запису токена _rs`ImmutableOwner`_ у Token Extensions
Program робить власника облікового запису токена постійним.

Після того, як обліковий запис токена ініціалізовано з _rs`ImmutableOwner`_,
подальші інструкції _rs`SetAuthority`_ не можуть змінити його повноваження
_rs`AccountOwner`_.

<Callout type="info" title="Асоційовані облікові записи токенів">
  Асоційовані облікові записи токенів, створені через Associated Token Program
  для Token Extensions Program, автоматично ініціалізують _rs`ImmutableOwner`_
  на обліковому записі токена.
</Callout>

## Як ініціалізувати обліковий запис із незмінним власником

Для ініціалізації облікового запису з незмінним власником:

1. Створіть і ініціалізуйте mint.
2. Створіть обліковий запис токена з достатнім простором для
   _rs`ImmutableOwner`_.
3. Ініціалізуйте _rs`ImmutableOwner`_ на обліковому записі токена перед
   _rs`InitializeAccount`_.
4. Спроба змінити власника облікового запису пізніше за допомогою
   _rs`SetAuthority`_ завершиться помилкою _rs`TokenError::ImmutableOwner`_.

<ScrollyCoding>

## !!steps Обчислити розмір облікового запису токена

Обчисліть розмір облікового запису токена для базового облікового запису токена
плюс розширення _rs`ImmutableOwner`_. Це розмір, який використовується в
_rs`CreateAccount`_.
...
## !!steps Обчислити орендну плату

Обчисліть орендну плату, використовуючи розмір облікового запису токена,
необхідний для розширення _rs`ImmutableOwner`_.
...
## !!steps Створити обліковий запис токена

Створіть обліковий запис токена з обчисленим простором і lamport.
...
## !!steps Ініціалізація ImmutableOwner

Ініціалізуйте розширення _rs`ImmutableOwner`_ на токен-акаунті.
...
## !!steps Ініціалізація токен-акаунта

Ініціалізуйте токен-акаунт за допомогою _rs`InitializeAccount`_ у тій самій
транзакції.
...
<Callout type="info" title="Порядок інструкцій">
  _rs`InitializeImmutableOwner`_ повинна виконуватися перед
  _rs`InitializeAccount`_. _rs`CreateAccount`_, _rs`InitializeImmutableOwner`_
  та _rs`InitializeAccount`_ повинні бути включені в одну транзакцію.
</Callout>
...
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- |
| _rs`ImmutableOwner`_                     | Розширення акаунта, яке позначає власника токен-акаунта як незмінного.                                 | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L13-L16) |
| _rs`InitializeImmutableOwner`_           | Інструкція, яка ініціалізує розширення незмінного власника на неініціалізованому токен-акаунті.        | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1982-L1991)           |
| _rs`process_initialize_immutable_owner`_ | Логіка процесора, яка виділяє та ініціалізує _rs`ImmutableOwner`_ на акаунті.                          | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1532-L1543)               |
| _rs`SetAuthority`_                       | Базова інструкція токена, що використовується для зміни повноважень, включаючи власника токен-акаунта. | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1611-L1633)           |
| _rs`process_set_authority`_              | Логіка процесора, яка відхиляє зміни власника, коли на токен-акаунті увімкнено _rs`ImmutableOwner`_.   | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L703-L900)                 |
