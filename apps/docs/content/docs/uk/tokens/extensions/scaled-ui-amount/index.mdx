...
## Що таке розширення масштабованої суми UI?

Розширення mint _rs`ScaledUiAmount`_ у Token Extensions Program дозволяє mint
застосовувати оновлюваний множник до суми UI, що відображається для балансів
токенів.

Це корисно, коли токен представляє реальний актив або інструмент, наприклад,
дроблення акцій або дивіденди. Наприклад, дроблення акцій подвоює кількість
акцій для всіх власників. Без зміни балансу кожного token account,
_rs`ScaledUiAmount`_ дозволяє гаманцям та додаткам відображати нову суму UI,
використовуючи множник mint.

Цю функцію також можна використовувати для дивідендів або розподілу дохідності.

Нові токени не створюються. Кількість токенів, збережена в token account,
залишається незмінною. Змінюється лише відображувана сума UI.

<Callout type="info" title="Несумісне розширення">
  Не вмикайте _rs`ScaledUiAmount`_ та _rs`InterestBearingConfig`_ на одному
  mint. Token Extensions Program відхиляє таку комбінацію розширень.
</Callout>

## Як працює масштабована сума UI

- _rs`ScaledUiAmountConfig`_ зберігає повноваження оновлення, `multiplier`,
  `new_multiplier` та `new_multiplier_effective_timestamp`.
- До `new_multiplier_effective_timestamp` конвертації використовують
  `multiplier`. На цю мітку часу або після неї конвертації використовують
  `new_multiplier`.
- _rs`UpdateMultiplier`_ може застосувати новий множник негайно або запланувати
  його на майбутню мітку часу Unix.
- _rs`AmountToUiAmount`_, _rs`UiAmountToAmount`_ та допоміжні функції
  конвертації розширення використовують арифметику з плаваючою комою для mint з
  масштабованою сумою UI, тому конвертації не гарантовано виконують точне
  зворотне перетворення.

## Як використовувати масштабовану суму UI

Щоб використовувати масштабовану суму UI:

1. Обчисліть розмір mint account та необхідну rent для mint та розширення
   _rs`ScaledUiAmount`_.
2. Створіть mint account за допомогою _rs`CreateAccount`_, ініціалізуйте
   _rs`ScaledUiAmount`_ та ініціалізуйте mint за допомогою _rs`InitializeMint`_.
3. Використовуйте _rs`AmountToUiAmount`_, _rs`UiAmountToAmount`_ або клієнтські
   допоміжні функції при відображенні балансів або конвертації сум UI.
4. Використовуйте _rs`UpdateMultiplier`_ для негайного оновлення відображуваного
   множника або на майбутню мітку часу Unix.

<Callout type="info" title="Порядок інструкцій">
  _rs`ScaledUiAmountMintInstruction::Initialize`_ має йти перед
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`ScaledUiAmountMintInstruction::Initialize`_ та _rs`InitializeMint`_ мають
...
<Cards>
  ...
  >
    Створіть токен із масштабованою суму інтерфейсу та оновлюйте множник з
    часом.
  </Card>
  <Card
  ...
  >
    Конвертуйте суми токенів для відображення та підтримуйте масштабовану суму
    інтерфейсу в гаманцях та додатках.
  </Card>
</Cards>
...
| ----------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`ScaledUiAmountConfig`_                            | Стан розширення токена, що зберігає повноваження, поточний множник, наступний множник та часову мітку, коли він набуває чинності.              | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L41-L55)         |
| _rs`ScaledUiAmountMintInstruction::Initialize`_       | Інструкція, що ініціалізує _rs`ScaledUiAmount`_ на токені перед _rs`InitializeMint`_.                                                          | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/instruction.rs#L24-L42) |
| _rs`ScaledUiAmountMintInstruction::UpdateMultiplier`_ | Інструкція, що встановлює новий множник та часову мітку Unix, коли він набуває чинності.                                                       | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/instruction.rs#L43-L66) |
| _rs`amount_to_ui_amount`_                             | Допоміжна функція, що конвертує суму токена в рядок суми інтерфейсу, використовуючи поточний множник, десяткові розряди та часову мітку.       | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L69-L84)         |
| _rs`try_ui_amount_into_amount`_                       | Допоміжна функція, що конвертує рядок суми інтерфейсу назад у суму токена, використовуючи поточний множник, десяткові розряди та часову мітку. | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L86-L107)        |
| _rs`process_initialize`_                              | Процесор, що ініціалізує _rs`ScaledUiAmountConfig`_ та зберігає початковий множник на неініціалізованому токені.                               | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/scaled_ui_amount/processor.rs#L37-L57)     |
| _rs`process_update_multiplier`_                       | Процесор, що перевіряє повноваження, оновлює запланований множник та застосовує його негайно, коли це доречно.                                 | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/scaled_ui_amount/processor.rs#L59-L108)    |
| _rs`process_amount_to_ui_amount`_                     | Шлях конвертації Token Program, що використовує _rs`ScaledUiAmountConfig`_, коли токен має увімкнене розширення.                               | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1548-L1571)                            |
| _rs`process_ui_amount_to_amount`_                     | Шлях конвертації Token Program, що конвертує суму інтерфейсу назад у суму токена для токенів із масштабованою сумою інтерфейсу.                | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1576-L1595)                            |

### Typescript

Наведений нижче приклад `Kit` використовує згенеровані інструкції безпосередньо.
Застарілі приклади з використанням `@solana/web3.js` та `@solana/spl-token`
включено для довідки.
