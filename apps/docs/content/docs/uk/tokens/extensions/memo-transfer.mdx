---
title: Переказ з примітками
description:
  Вимагайте, щоб вхідні перекази включали примітку, увімкнувши розширення
  облікового запису MemoTransfer на обліковому записі токенів Token Extensions
  Program.
...
## Що таке Переказ з примітками?

Розширення облікового запису _rs`MemoTransfer`_ Token Extensions Program вимагає
інструкції примітки безпосередньо перед будь-яким вхідним переказом на цей
обліковий запис токенів. Вихідні перекази з цього облікового запису токенів не
вимагають примітки.

Розширення налаштовується на обліковому записі токенів призначення, а не на
mint. Після увімкнення перекази без примітки завершуються помилкою
_rs`TokenError::NoMemo`_, доки власник облікового запису токенів не вимкне цю
вимогу.

## Як створити обліковий запис токенів з увімкненим MemoTransfer

Щоб створити обліковий запис токенів з увімкненим _rs`MemoTransfer`_:

1. Створіть та ініціалізуйте mint.
2. Обчисліть розмір облікового запису токенів та необхідну орендну плату для
   облікового запису токенів з _rs`MemoTransfer`_.
3. Створіть обліковий запис токенів та увімкніть _rs`MemoTransfer`_ на цьому
   обліковому записі.
4. Інструкції _rs`Transfer`_ та _rs`TransferChecked`_ на цей обліковий запис
   токенів повинні включати інструкцію примітки безпосередньо перед інструкцією
   переказу.

<ScrollyCoding>

## !!steps Створіть та ініціалізуйте mint

Створіть та ініціалізуйте mint, який використовуватиме обліковий запис токенів.
...
## !!steps Обчисліть розмір облікового запису токенів

Обчисліть розмір облікового запису токенів для базового облікового запису
токенів плюс розширення _rs`MemoTransfer`_. Це розмір, який використовується в
_rs`CreateAccount`_.
...
## !!steps Обчисліть орендну плату

Обчисліть орендну плату, використовуючи розмір облікового запису токенів,
необхідний для розширення _rs`MemoTransfer`_.
...
## !!steps Створення та ініціалізація токен-акаунта

Створіть токен-акаунт з розрахованим простором і lamport, потім ініціалізуйте
його для mint.
...
## !!steps Увімкнення MemoTransfer

Увімкніть _rs`MemoTransfer`_ на токен-акаунті.
...
| ----------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`MemoTransfer`_                              | Розширення акаунта, що зберігає інформацію про те, чи потрібне memo для вхідних переказів.                | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/mod.rs#L17-L22)         |
| _rs`RequiredMemoTransfersInstruction::Enable`_  | Інструкція, що вмикає обов'язкові memo для переказів на токен-акаунті.                                    | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L50-L71) |
| _rs`RequiredMemoTransfersInstruction::Disable`_ | Інструкція, що вимикає обов'язкові memo для переказів на токен-акаунті.                                   | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L74-L95) |
| _rs`process_toggle_required_memo_transfers`_    | Логіка процесора, що ініціалізує або перезаписує розширення _rs`MemoTransfer`_ після перевірки власності. | [Джерело](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/memo_transfer/processor.rs#L20-L48)     |
...
`@solana/spl-memo` наведені для довідки.
