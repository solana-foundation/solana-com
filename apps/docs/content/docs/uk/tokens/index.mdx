...
Token Extensions Program дозволяє зберігати метадані (назва, символ, посилання
на зображення) безпосередньо в Mint Account.
...
URI токена посилається на метадані поза мережею. Дивіться приклад формату JSON
...
Дізнайтеся більше в
[посібнику з розширення Metadata](/developers/guides/token-extensions/metadata-pointer).
Для деталей про Token Extensions дивіться
[посібник для початківців](/developers/guides/token-extensions/getting-started)
та
