...
яким є Token Program для всіх token accounts.
...
використовують термінал [Solana Playground](https://beta.solpg.io/) для
виконання CLI команд безпосередньо в браузері без локального встановлення.

Створення токенів та акаунтів вимагає SOL для депозитів rent акаунтів та комісій
за транзакції. Для користувачів Solana Playground, які використовують його
вперше, створіть Playground гаманець та виконайте команду _sh`solana airdrop`_ у
терміналі Playground. Ви також можете отримати devnet SOL з публічного
...
Знайдіть більше прикладів у
[документації](https://www.solana-program.com/docs/token) _sh`spl-token`_.

<Callout type="info">

Адреси акаунтів, показані у вашому терміналі, відрізняються від прикладів нижче.
Використовуйте адреси зі свого терміналу Playground, коли слідуєте інструкціям.
Наприклад, команда _sh`create-token`_ виводить адресу mint account з вашим
гаманцем Playground як mint authority.
