...
description:
  Дізнайтеся, як створити програму Solana з функціоналом CRUD (створення,
  читання, оновлення, видалення) за допомогою Program Derived Address (PDA) та
  фреймворку Anchor. Цей покроковий посібник демонструє, як створювати,
  оновлювати та видаляти облікові записи повідомлень в блокчейні за допомогою
  PDA, впроваджувати перевірку облікових записів та писати тести. Ідеально
  підходить для розробників, які прагнуть зрозуміти, як використовувати PDA у
  програмах Solana.
...
дані облікового запису (тип даних для зберігання в полі data облікового запису).
...
Пізніше, під час зчитування з облікового запису, програма десеріалізує ці дані
назад у тип даних _rs`MessageAccount`_. Розділ тестування демонструє процес
створення та зчитування даних облікового запису.
...
Обмеження _rs`space`_ визначає кількість байтів для виділення в полі даних
...
У цьому випадку обліковий запис має відповідати користувацькому типу
_rs`MessageAccount`_.
...
У цьому випадку обліковий запис має відповідати типу _rs`System`_, який
...
1. Тип облікового запису (наприклад, _rs`Signer<'info>`_ або
   _rs`Account<'info, T>`_), який визначає, який тип облікового запису очікує
   програма
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - `space` виділяє необхідну кількість байтів для поля data облікового запису
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   - Існуючий обліковий запис, який зберігає повідомлення користувача для
     оновлення
...
Обмеження _rs`seeds`_ визначає seeds, які використовуються для виведення PDA.
...
У цьому випадку обліковий запис _rs`user`_ отримує лампорти із закритого
_rs`message_account`_.
...
1. _rs`user: Signer<'info>`_
   ...
   - Позначений як змінюваний (_rs`#[account(mut)]`_) для отримання lamports із
...
Після підготовки тестів запустіть тестовий файл командою _shell`test`_ у
терміналі Playground. Ця команда виконує тести для програми, розгорнутої в
