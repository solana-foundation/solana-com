...
Далі оновіть структуру _rs`Update`_, щоб включити новий акаунт під назвою
...
_rs`Update`_. Цей акаунт служить контрольованим програмою "сховищем", яке
...
- Адреса акаунта походить від PDA, виведеного з використанням seeds
  _rs`[b"vault", user.key().as_ref()]`_
- Як PDA, він не має приватного ключа, тому лише програма може "підписати"
  адресу при виконанні CPI
- Як тип _rs`SystemAccount`_, він належить System Program, як звичайні акаунти
  гаманців
...
В інструкції `delete` ви побачите, як програма може "підписати" цей PDA в CPI.
...
Структура _rs`CpiContext`_ визначає програму та акаунти для міжпрограмного
виклику (CPI).
...
В інструкції `update` реалізація включає міжпрограмний виклик (CPI) для виклику
інструкції `transfer` System Program. Це демонструє, як виконувати CPI зсередини
...
- `amount` для переказу (1 000 000 лампортів, або 0,001 SOL)
...
Налаштування для CPI відповідає тому, як будуються інструкції на стороні
...
дозволить перевести будь-які SOL у сховищі назад користувачеві, коли вони
...
до акаунта сховища користувача під час виконання інструкції видалення для
переказу будь-яких накопичених SOL назад користувачу.
...
_rs`signer_seeds`_ визначають опціональні seeds та bump seeds, які
використовуються для виведення PDA.

### !with_signer

Метод _rs`with_signer()`_ передає signer seeds разом з CPI.
...
_rs`ctx: Context<Delete>`_, щоб використовувати контекст у тілі функції.
...
- `cpi_context` (програму, акаунти та PDA signer)
