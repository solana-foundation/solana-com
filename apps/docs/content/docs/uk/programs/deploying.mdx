...
| Передати повноваження програми       | `solana program set-upgrade-authority <program-id> --new-upgrade-authority <path_to_keypair>` |
...
Зберіть свою програму за допомогою команди `cargo build-sbf`:
...
Ви можете згенерувати нові keypair за допомогою команди `solana-keygen`:
...
Оновіть свою програму, використовуючи ту саму команду `solana program deploy`:
...
Щоб передати повноваження програми іншому обліковому запису, використовуйте
команду `solana program set-upgrade-authority`:
...
Щоб закрити вашу програму та повернути SOL, виділений для облікового запису
програми, використовуйте команду `solana program close`:
...
- **`--with-compute-unit-price`**: встановлює пріоритетну комісію в
  мікро-lamports (0,000001 SOL) за обчислювальну одиницю. Перевірте
  [Helius Priority Fee API](https://docs.helius.dev/guides/priority-fee-api) для
  поточних ставок. Пріоритетна комісія — це додаткова комісія, яка сплачується
  поточному лідеру для пріоритизації вашої транзакції.
- **`--max-sign-attempts`**: кількість спроб повторити з новим blockhash, якщо
  транзакції втрачають чинність. (Типово: 5)
- **`--use-rpc`**: надсилає транзакції до налаштованого RPC. Цей прапорець
  вимагає [зваженого за ставкою](/developers/guides/advanced/stake-weighted-qos)
