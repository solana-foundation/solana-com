...
Для бенчмаркінгу використання compute unit додайте `mollusk-svm-bencher` як
...
1. [Створіть екземпляр `Mollusk`](mention:mollusk) — ініціалізуйте Mollusk з
   program ID та шляхом до скомпільованої програми (файл `.so`)
...
надає простий інтерфейс для тестування програм Solana. Усі поля можна
маніпулювати через кілька допоміжних методів, але користувачі також можуть
безпосередньо отримувати доступ та змінювати їх, якщо потрібен більший контроль.
...
Приклади нижче запускають Mollusk у функції `main` для демонстрації. На практиці
ви зазвичай використовуватимете Mollusk у тестовому модулі, анотованому
атрибутом `#[test]`.
...
є обгорткою навколо `Mollusk`, яка підтримує стан акаунтів між кількома
викликами інструкцій через свій `account_store`. Методи для обробки інструкцій
ідентичні до `Mollusk`.
...
Наступний приклад обробляє дві окремі інструкції переказу SOL зі збереженням
стану акаунтів між інструкціями через `account_store`.
...
для зміни її значень під час тестування.

Використовуйте метод `warp_to_slot` для оновлення sysvar clock, щоб симулювати
переміщення вперед або назад у часі до конкретного слоту.
...
Наступний приклад показує, як змінити Mollusk sysvar безпосередньо через доступ
до поля `sysvars` для зміни параметрів rent. Ви можете змінювати інші значення
sysvar таким же чином.
...
Результати бенчмарку записуються до вказаного `out_dir` як файл markdown з
