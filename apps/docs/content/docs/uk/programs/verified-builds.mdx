...
`ellipsislabs/hello_world_verifiable_build:latest`, і перевіряє, що хеш шляху
виконуваного файлу в контейнері збігається з хешем ончейн-програми, наданої
команді. Оскільки збірка вже була завантажена в образ, немає потреби в повній
пересборці виконуваного файлу, що може зайняти багато часу.
...
- **Неправильний підписант**: Підтвердьте, що ваш підписант є повноважним
  органом оновлення програми, виконавши `solana program show YourProgramId`
- **Відсутній ончейн PDA**: Виконайте `solana-verify verify-from-repo -um` і
  **виберіть ТАК на запит**. Без завантаження PDA API не зможе отримати ваші
  метадані верифікації.
- **Невідповідність даних PDA**: Оновіть свій PDA, якщо ви передеплоїли свою
  програму. Дані вашого PDA повинні відповідати вашій розгорнутій програмі.
- **Невірний хеш коміту**: Створіть свій PDA, використовуючи точний хеш коміту,
  який ви розгорнули
- **Відмінності в середовищі збірки**: Використовуйте Docker з solana-verify при
  створенні вашого PDA

## Чому хеш моєї локальної збірки не збігається з ончейн хешем?
...
- **Ончейн зберігання**: Зберігайте метадані верифікації (URL репозиторію, хеш
  коміту, параметри збірки) ончейн у PDA, що належить програмі Otter Verify
  (`verifycLy8mB96wd9wqq3WDXQwM4oU6r42Th37Db9fC`)
- **Криптографічне з'єднання**: Ваш PDA виводиться з адреси вашої програми,
  створюючи незмінне з'єднання з вашими даними верифікації
- **Децентралізована довіра**: Будь-хто може прочитати ваш PDA і незалежно
  верифікувати вашу програму

## Чому я повинен створити PDA перед використанням API?

API працює лише з ончейн PDA, оскільки:

- **Без довіри**: API відхиляє довільні дані - він використовує лише те, що ваш
  повноважний орган оновлення зберіг ончейн
- **Простіше**: Просто надайте підписанта + program_id; API отримує все інше з
  вашого PDA
- **Захист від підробки**: Ваш PDA створює незмінний запис, який будь-хто може
  верифікувати незалежно
- **Підтвердження власності**: Ваш підписант повинен бути повноважним органом
  оновлення, криптографічно доводячи, що ви контролюєте програму
...
- **On-Chain PDA**: Ваші метадані верифікації зберігаються ончейн, не
  контролюються жодним центральним органом
- **Підтвердження повноважень оновлення**: Лише орган оновлення вашої програми
  може створювати/оновлювати PDA
- **Незалежна верифікація**: Будь-хто може перевірити, прочитавши ваш PDA та
  запустивши `solana-verify` локально
- **Безперервна повторна верифікація**: API автоматично повторно верифікує всі
  програми кожні 24 години
...
Перевірте будь-яку програму самостійно, прочитавши її ончейн PDA та запустивши
...
1. Метадані PDA є автентичними (зберігаються ончейн)
2. Вихідний код у репозиторії PDA відповідає розгорнутій програмі
3. Вам не потрібно довіряти API - перевіряйте все самостійно ончейн
