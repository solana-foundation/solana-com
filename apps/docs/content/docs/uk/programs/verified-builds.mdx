...
команді. Оскільки збірка вже завантажена в образ, немає потреби в повній
...
За замовчуванням команда `verify-from-repo` використовує останній коміт з гілки
...
Якщо ви хочете мати повністю безтрастове налаштування, ви можете запустити
[API верифікації](https://github.com/otter-sec/solana-verified-programs-api)
самостійно або виконати верифікацію програми локально, використовуючи команду
`verify-from-repo` з даними верифікації в мережі, які збережені в
[PDA](https://explorer.solana.com/address/63XDCHrwZu3mXsw2RUFb4tbNpChuUHx4eA5aJMnHkpQQ/anchor-account),
...
своєї програми. У майбутньому, після реалізації, `security.txt` міститиме
...
верифікатора. За замовчуванням це той самий публічний ключ, який зібрав і
розгорнув програму. Але це також може бути інший публічний ключ, який можна
вказати в `security.txt`.

Функція `security.txt` дозволяє розробникам вбудовувати контактну та інформацію
про безпеку безпосередньо в їхні смарт-контракти Solana. Натхненний
[securitytxt.org](https://securitytxt.org), цей підхід забезпечує
стандартизований спосіб для дослідників безпеки зв'язатися з супроводжувачами
...
лише за адресою контракту може бути складною та трудомісткою. Вбудовування файлу
`security.txt` у програму гарантує, що дослідники безпеки зможуть легко
зв'язатися з потрібними людьми, потенційно запобігаючи експлойтам та
забезпечуючи своєчасні звіти про помилки.
...
інформації про безпеку. Ви можете включити контактні дані, URL проєкту та навіть
політику безпеки. Ось приклад:
...
можна легко запитати через інструменти, такі як Solana Explorer, гарантуючи, що
ваші контактні дані та інформація про безпеку доступні будь-кому, хто хоче
...
- Верифікація: перед розгортанням перевірте формат та вміст за допомогою
  інструменту `query-security-txt`, який може валідувати як ончейн-програми, так
  і локальні бінарні файли:
