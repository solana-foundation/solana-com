...
difficulty: advanced
title: "Страйки NAV для фондів грошового ринку на Solana"
description:
...
Страйки NAV (чистої вартості активів) — це фіксовані моменти протягом торгового дня, коли визначається ціна акцій фонду та виконуються ордери. Традиційні фонди зазвичай мають один денний страйк на момент закриття ринку, але швидкість і низькі витрати Solana дозволяють проводити кілька внутрішньоденних страйків, надаючи інвесторам більше гнучкості.

## Що таке NAV?

Фонд — це пул активів, яким спільно володіють багато інвесторів. **NAV (чиста вартість активів)** — це просто ціна однієї акції у фонді. Уявіть це так:

```
NAV = (Total Value of Everything in the Fund - liabilities) ÷ Number of Shares
```

Наприклад, якщо фонд утримує активи на $10 мільйонів і має 10 мільйонів акцій в обігу, кожна акція коштує $1,00.

**Чому NAV важливий?**

- **Купівля акцій (підписка)**: Ви сплачуєте поточний NAV. Якщо NAV становить $1,02 і ви інвестуєте $102, ви отримаєте 100 акцій.
- **Продаж акцій (погашення)**: Ви отримуєте поточний NAV. Якщо ви погашаєте 100 акцій за NAV $1,02, ви отримаєте $102.

Фонди грошового ринку зазвичай підтримують стабільний NAV близько $1,00, з незначними коливаннями залежно від нарахованих відсотків.

## Проблема

Коли ви інвестуєте у фонд грошового ринку, традиційно дві речі відбуваються в різний час:

1. Ви подаєте ордер → Очікуєте закриття ринку
2. NAV розраховується → Акції випускаються/погашаються на T+1 або T+2

Це породжує кілька проблем:

- **Затримка виконання**: Ордери, подані вранці, очікують до 16:00
- **Затримка розрахунків**: Ваші кошти перебувають у підвішеному стані 1–2 дні
- **Обмежена гнучкість**: Лише одна можливість для транзакції на день
- **Застарілі ціни**: NAV може бути застарілим між подачею ордера та його виконанням

## Рішення

**Страйки NAV на Solana** забезпечують кілька денних вікон розрахунків з атомарним виконанням. Ордери накопичуються в черзі та миттєво виконуються на кожному страйку за поточним NAV.
...
Цей посібник демонструє, як реалізувати страйки NAV на Solana за допомогою розширень SPL Token 2022 для відповідного випуску акцій фонду та стандартного USDC для розрахунків — без написання власних програм на Rust.

<Callout type="info" title="Навчальна референсна реалізація">

Ви можете скористатися
[вихідним кодом](https://github.com/Woody4618/nav-strikes-reference) цієї
реалізації, щоб спробувати страйки NAV локально.

Цей посібник надає референсну реалізацію виключно для ознайомлення та навчальних цілей. **НЕ використовуйте цей код безпосередньо у виробничому середовищі** без:
...
- Ретельного тестування та доопрацювань

</Callout>

## Огляд архітектури

Система страйків NAV складається з таких основних компонентів:

1. **Токен акцій фонду**: SPL Token 2022 з метаданими, що зберігають NAV у блокчейні
2. **Розрахункова валюта**: Стандартний USDC (наявний SPL-токен)
3. **Адміністратор фонду**: Оркеструє страйки через делеговані повноваження
4. **Система білого списку**: Контролює, які адреси можуть утримувати акції фонду

### Ключові принципи проєктування

- **Без власних програм**: Використовуються лише Token Extensions та стандартний USDC
- **Атомарні розрахунки**: Одна транзакція гарантує, що підписка/погашення або завершується, або скасовується повністю
- **Стан заморожування за замовчуванням**: Акції потребують явного включення до білого списку для відповідності нормативним вимогам
- **Делеговані повноваження**: Адміністратор фонду діє без прийняття коштів під зберігання
- **NAV у блокчейні**: Поточна ціна зберігається в метаданих токена, відкрита для перевірки

### Етап 1: Налаштування фонду

1. Створення токена акцій фонду з розширеннями метаданих
2. Ініціалізація NAV на рівні $1,00
3. Встановлення розкладу денних страйків
4. Додавання інституційних інвесторів до білого списку

### Етап 2: Передстрайковий період

1. Інвестори подають заявки на підписку/погашення
2. Ордери стають у чергу до наступного часу страйку
3. Фонд розраховує попередній NAV
4. Виконуються перевірки ризиків

### Етап 3: Виконання страйку NAV
...
### Етап 4: Після страйку

1. Генерація торгових підтверджень
2. Оновлення складу фонду
3. Підготовка до наступного страйку
4. Звірка з кастодіаном

### Патерн делегованих повноважень

Інвестори заздалегідь авторизують адміністратора фонду на переміщення їхніх токенів, після чого адміністратор виконує транзакції під час страйку без необхідності отримувати підписи інвесторів:
...
### Потік підписки (USDC → акції фонду)
...
### Потік погашення (акції фонду → USDC)
...
## Чому Solana для страйків NAV?

Архітектура Solana забезпечує суттєві переваги порівняно з традиційними фондовими розрахунками:

| Аспект            | Традиційний (T+1) | Страйки NAV на Solana          |
| ----------------- | ----------------- | --------------------------- |
| Логіка розрахунків  | Трансфер-агенти   | Атомарне об'єднання транзакцій |
| Час розрахунків   | 1–2 дні          | &lt;1 секунди                |
| Вартість транзакції  | $50–200           | &lt;$0,01                   |
| Страйків на день   | 1                 | 4+ (налаштовується)           |
| Ризик застарілих цін | Високий              | Нижчий                       |
| Прозорість NAV  | Звіт наприкінці дня | У блокчейні, в реальному часі          |

Атомарні транзакції Solana усувають посередників, забезпечуючи миттєві, дешеві та безпечні розрахунки з прозорістю NAV у блокчейні.

## Токен фонду з Token 2022 Extensions

SPL Token 2022 надає потужні розширення, що забезпечують відповідний випуск акцій фонду без власних програм:

### Необхідні розширення для акцій фонду

1. **Розширення стану облікового запису за замовчуванням**: Встановлює всі нові token account у заморожений стан за замовчуванням, вимагаючи явного включення до білого списку (відповідність KYC/AML)
2. **Розширення метаданих**: Зберігає NAV, розклад страйків і AUM у блокчейні для забезпечення прозорості

### Конфігурація повноважень

- **Повноваження на карбування**: Адміністратор фонду (контролює випуск акцій)
- **Повноваження на заморожування**: Адміністратор фонду (керує білим списком)
- **Повноваження на оновлення**: Адміністратор фонду (оновлює метадані NAV)

<Callout type="info">

Стан заморожування за замовчуванням є критично важливим для відповідності нормативним вимогам. Він гарантує, що лише явно включені до білого списку (верифіковані за KYC) адреси можуть отримувати та утримувати акції фонду.

</Callout>

### Поля метаданих у блокчейні
...
## Повна реалізація страйків NAV

### Налаштування рушія страйків NAV

Зверніть увагу, що в цьому посібнику ми використовуємо бібліотеку @solana/kit для створення рушія страйків NAV. Також можна знайти реалізацію на `web3.js` у
[вихідному коді](https://github.com/Woody4618/nav-strikes-reference). Спочатку створіть основний клас рушія, який обробляє всі операції фонду:
...
<Callout type="tip" title="Динамічне оновлення метаданих">

Для акцій фонду поля `currentNAV`, `lastStrikeTime` і `totalAUM` оновлюються у блокчейні на кожному страйку NAV за допомогою
`getUpdateTokenMetadataFieldInstruction`. Це забезпечує:

- **Відображення NAV у реальному часі** для всіх учасників
- **Журнал аудиту** всіх оновлень NAV у блокчейні
- **Інтеграцію** з DeFi-протоколами, що можуть зчитувати метадані з блокчейну
...
Ось повний приклад, що демонструє повний день операцій зі страйками NAV:
...
## Міркування для виробничого середовища

Перед розгортанням у виробничому середовищі переконайтеся, що ви врахували:

1. **Безпека**: Професійне управління ключами, контроль на основі мультипідпису та комплексний аудит безпеки кодової бази
2. **Відповідність нормативним вимогам**: Реєстрація цінних паперів, системи KYC/AML, обмеження на передачу та обов'язкова звітність
3. **Управління ключами**: Інституційні рішення для зберігання з належними процедурами резервного копіювання та відновлення
4. **Обробка транзакцій**: Пріоритетні комісії, логіка повторних спроб, обробка підтверджень та резервування RPC
5. **Операції фонду**: Розрахунок NAV з авторитетних джерел, інтеграція з кастодіаном та звірка
6. **Моніторинг**: Відстеження в реальному часі, сповіщення та автоматизована регуляторна звітність

## Наступні кроки

Після ознайомлення зі страйками NAV на Solana:

1. **Вивчіть Token Extensions**: Дізнайтеся про інші
   [Token 2022 extensions](/docs/tokens/extensions/transfer-hook), наприклад постійний делегат для додаткових функцій відповідності

2. **Реалізуйте Token ACL**: Для розширеної відповідності розгляньте використання
   [Token ACL](/docs/tokenization/token-acl) для управління білим списком інвесторів
   із безавторизованим розморожуванням. Це дозволяє інвесторам самостійно проходити KYC-верифікацію та автоматично активувати свої облікові записи після проходження перевірок відповідності, знижуючи операційне навантаження при збереженні регуляторного контролю.

3. **Вивчіть розрахунки DvP**: Перегляньте наш
   [посібник із розрахунків «постачання проти платежу»](/docs/tokenization/dvp) для патернів розрахунків із цінними паперами

4. **Виробнича архітектура**: Розробіть надійне планування, моніторинг та системи аварійного відновлення

## Висновок

Модель атомарних транзакцій Solana та Token 2022 Extensions забезпечують потужну основу для реалізації кількох денних страйків NAV для фондів грошового ринку. Поєднання:

- Нативного атомарного виконання (без ризику смарт-контрактів)
- Завершеності транзакцій за менш ніж секунду
- Майже нульових витрат на транзакції
- Прозорості NAV у блокчейні
- Вбудованих функцій відповідності (заморожування за замовчуванням, метадані)

Робить Solana ідеальною платформою для модернізації інфраструктури фондових розрахунків і надання інвесторам більш частих можливостей для торгівлі.

Однак перехід від цієї навчальної референсної реалізації до виробничого середовища потребує значної додаткової роботи у сферах безпеки, відповідності, зберігання та операцій. Завжди залучайте кваліфікованих юридичних, регуляторних і технічних експертів при роботі з токенізацією фондів.
