...
> _Примітка: Якщо `fallback_if_unavailable` має значення true, система дозволяє транзакціям
> продовжуватися, коли Redis тимчасово недоступний, запобігаючи перебоям у роботі сервісу.
> Встановлення `max_transactions` у значення 0 дозволить необмежену кількість транзакцій._
...
| `get_payer_signer`          | Повернути наступного платника комісії та адресу платежу        | ✅       | логічний |
...
[документації Solana](https://solana.com/developers/cookbook/transactions/durable-nonces).
...
| ---------------------------- | ------------------------------------------------ |
| `confidential_transfer_mint` | Конфігурація конфіденційних балансів для монетного двору |
...
| Назва розширення                  | Опис                                 |
| ------------------------------- | ------------------------------------------- |
| `confidential_transfer_account` | Стан конфіденційних балансів для облікового запису |
