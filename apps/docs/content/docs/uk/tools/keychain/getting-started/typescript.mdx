...
Fordefi має два режими підписання, які вибираються залежно від того, чи передаєте ви `chain`. API-запити автентифікуються за допомогою підпису запиту ECDSA P-256 — надайте ключ підписання як `privateKeyPem`, або зберігайте його в KMS/HSM, передавши `requestSigner`.

**Нативний режим Solana** (рекомендується для використання в мережі): встановіть `chain`, і Fordefi самостійно підписує та транслює транзакцію. Оскільки Fordefi може замінити blockhash або комісії перед підписанням, підписувач є Kit `TransactionSendingSigner` — використовуйте його з `signAndSendTransactionMessageWithSigners`, а не з потоком часткового підписувача, показаним вище.
...
**Режим чорної скриньки**: пропустіть `chain`. Fordefi підписує необроблені байти та не транслює; ви самостійно збираєте та надсилаєте транзакцію. Використовуйте це з vault-сховищем типу "чорна скринька" Fordefi.
