---
title: Довідник API Rust
description:
  Повний публічний API для крейту surfpool-sdk — Surfnet, білдер, читкоди,
  білдери, помилки та реекспорти.
---

Крейт `surfpool-sdk` надає невеликий інтерфейс, зосереджений навколо структури
`Surfnet`. На цій сторінці перелічено кожен публічний елемент; для ознайомлення
з принципами використання зверніться до посібників, починаючи з
[Огляду](/docs/tools/surfpool/sdk/overview).

## Експорти кореня крейту

| Елемент               | Тип        | Примітки                                                               |
| --------------------- | ---------- | ---------------------------------------------------------------------- |
| `Surfnet`             | struct     | Запущений екземпляр Surfnet — RPC/WS ендпоінти, платник, читкоди       |
| `SurfnetBuilder`      | struct     | Флюентний білдер, що використовується `Surfnet::builder()`             |
| `Cheatcodes<'_>`      | struct     | Запозичений помічник читкодів, що повертається `Surfnet::cheatcodes()` |
| `SurfnetError`        | enum       | Усі варіанти помилок, що повертає SDK                                  |
...
`stop` очікує до п'яти секунд, поки середовище виконання звільнить свої порти.
Збій під час `Drop` ігнорується; збій під час явного виклику `stop` повертає
...
Кожен сетер повертає `Self`, тому виклики можна ланцюгувати. Термінальний виклик
— це `start().await`.
...
Реалізуйте це для власних JSON-RPC чіткодів, які ще не обгорнуті типізованим
білдером.

## Білдери

Усі білдери знаходяться в `surfpool_sdk::cheatcodes::builders`.
...
| Варіант          | Причина                                                                |
| ---------------- | ---------------------------------------------------------------------- |
| `PortAllocation` | Не вдалося прив'язатися до вільного порту для RPC або WebSocket.       |
| `Startup`        | Не вдалося ініціалізувати середовище виконання.                        |
| `Runtime`        | Потік середовища виконання аварійно завершився або вийшов несподівано. |
| `Cheatcode`      | Виклик JSON-RPC чіткоду повернув помилку.                              |
| `Aborted`        | Середовище виконання було зупинено або скасовано під час запуску.      |

Крейт реалізує `std::error::Error` та `Display` для `SurfnetError`, тому він
працює з `?` та `anyhow` з коробки.
