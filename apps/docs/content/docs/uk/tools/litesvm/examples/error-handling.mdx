...
description: Як тестувати умови виникнення помилок та граничні випадки
---
...
1. **Перевірка помилок**: Використовуйте `assert!(result.is_err())` для
   підтвердження виникнення помилок
2. **Відповідність помилок**: Зіставляйте з конкретними типами помилок для
   забезпечення правильної обробки помилок
3. **Перевірка стану**: Перевіряйте, що стан не змінився при виникненні помилок
4. **Власні помилки**: Тестуйте власні коди помилок вашої програми
5. **Граничні випадки**: Тестуйте граничні умови та недійсні вхідні дані
