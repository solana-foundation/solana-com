---
title: Допоміжні інструменти для тестування
description:
  Утиліти для створення акаунтів, операцій з токенами, виведення PDA та
  керування slot
---

## Створення акаунтів із балансом

Найпоширеніша потреба при тестуванні — створення акаунтів із SOL. Трейт
`TestHelpers` робить це тривіальним завданням.
...
Створюйте токен-мінти та акаунти без зайвого шаблонного коду.
...
<Callout type="info">
  **У більшості випадків використовуйте ATA.** Associated Token Accounts є
  детермінованими та відповідають конвенціям Solana. Використовуйте звичайні
  token account лише тоді, коли вам потрібно кілька акаунтів для однієї пари
  mint/owner.
</Callout>
...
Легко виводьте Program Derived Addresses для ваших тестів.

### Базове виведення PDA
...
<Callout type="info">
  Використовуйте `derive_pda`, коли вам потрібен bump seed (наприклад, для
  CPI-викликів). Використовуйте `get_pda`, коли вам потрібна лише адреса.
</Callout>

## Керування slot

Керуйте прогресією часу для тестування логіки, залежної від часу.

### Отримати поточний slot
...
Ось вичерпний приклад використання кількох тестових помічників разом:
