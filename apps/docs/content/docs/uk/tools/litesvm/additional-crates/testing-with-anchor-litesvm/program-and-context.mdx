---
title: Програма та контекст
description:
  Налаштування тестових середовищ та побудова інструкцій за допомогою
  AnchorContext і Program
---

## Генерація типів клієнта за допомогою declare_program!

Anchor 1.0 представляє `declare_program!`, який зчитує IDL вашої програми під
час компіляції та генерує типобезпечні модулі `client::accounts::*` і
`client::args::*`. Це рекомендований спосіб побудови інструкцій у тестах.
...
<Callout type="info">
  Спочатку запустіть `anchor build`, щоб згенерувати
  `target/idl/my_program.json`. `declare_program!` зчитує цей файл під час
  компіляції. Назва макросу повинна збігатися з назвою крейту вашої програми
  (snake_case).
</Callout>

## Налаштування тестового середовища

Будівник `AnchorLiteSVM` надає зручний API для налаштування вашого тестового
середовища.
...
### Скорочення для однієї програми
...
<Callout type="info">
  Якщо ви не вкажете платника, `AnchorLiteSVM` автоматично створить і поповнить
  його для вас.
</Callout>

## AnchorContext

`AnchorContext` є основним інтерфейсом для взаємодії з вашим тестовим
середовищем.
...
### Створення фінансованих акаунтів
...
## Побудова інструкцій за допомогою Program

Структура `Program` надає зручний API для побудови інструкцій, що відображає
anchor-client. Використовуйте типи, згенеровані `declare_program!`, для безпеки
типів.

### Базова побудова інструкцій
...
<Callout type="info">
  Метод `.accounts()` приймає будь-який тип, що реалізує `ToAccountMetas`, що
  всі структури облікових записів Anchor роблять автоматично. Аналогічно,
  `.args()` приймає будь-який тип, що реалізує `InstructionData`.
</Callout>
...
### Використання send_and_confirm_transaction
...
Ось комплексний приклад використання патерну ескроу з `declare_program!`:
