---
title: Допоміжні функції транзакцій
description:
  Виконуйте транзакції та аналізуйте результати з розширеною обробкою помилок
---

## Надсилання транзакцій

Трейт `TransactionHelpers` надає зручні методи для надсилання інструкцій та
аналізу результатів.

### Надіслати одну інструкцію
...
Методи `send_instruction` та `send_instructions` повертають `TransactionResult`,
який обгортає метадані транзакції з корисними утилітами для тестування.

### Перевірити успішне виконання
...
<Callout type="info">
  `assert_success()` завершиться з помилкою та виведе повні логи транзакції,
  якщо транзакція не вдалася, що значно спрощує налагодження.
</Callout>
...
### Перевірити код помилки

Для власних помилок програми можна перевірити код помилки:
...
### Перевірити помилку Anchor

Для програм Anchor перевіряйте за назвами помилок:
...
### Виведення логів для налагодження
...
### Перевірка обчислювальних одиниць
...
<Callout type="info">
  Моніторинг обчислювальних одиниць корисний для оптимізації. Solana обмежує
  транзакції до 1,4 млн обчислювальних одиниць, і вищий рівень використання
  коштує більше.
</Callout>

## Повний приклад

Ось комплексний приклад, що демонструє допоміжні функції для транзакцій:
...
Перелік `TransactionError` надає структуровану інформацію про помилки:

| Варіант                   | Опис                                                      |
| ------------------------- | --------------------------------------------------------- |
| `ExecutionFailed(String)` | Виконання транзакції завершилося помилкою з повідомленням |
| `BuildError(String)`      | Не вдалося побудувати транзакцію                          |
| `AssertionFailed(String)` | Перевірка результату не пройшла                           |
