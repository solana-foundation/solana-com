---
title: Транзакції
description: Методи для обробки транзакцій
---
...
Виконати транзакцію та змінити стан.
...
Повертає `SimulatedTransactionInfo`, який містить:

- `meta`: Метадані транзакції (логи, обчислювальні одиниці, підпис тощо)
- `post_accounts`: Стани акаунтів після симуляції
...
Використовуйте `simulate_transaction()`, щоб:

- Перевірити дійсність транзакції без зміни стану
- Перевірити споживання обчислювальних одиниць
- Переглянути логи транзакції без підтвердження
- Протестувати шляхи обробки помилок
...
## Тестування помилкових випадків
...
`send_transaction()` повертає `TransactionMetadata` безпосередньо, тоді як
`simulate_transaction()` повертає `SimulatedTransactionInfo`, який містить
...
Де `TransactionResult = Result<TransactionMetadata, FailedTransactionMetadata>`.

Запит транзакції з історії. Потребує встановлення `with_transaction_history()`.
...
Отримати поточний blockhash для транзакцій.
...
Примусово завершити термін дії поточного blockhash.
...
### Відстеження одиниць обчислення
