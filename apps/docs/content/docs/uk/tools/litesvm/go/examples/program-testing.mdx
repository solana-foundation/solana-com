...
description: Завантаження та тестування власних програм Solana за допомогою litesvm-go
---

Тестування власної програми Solana з Go означає завантаження скомпільованого файлу `.so`
у SVM, налаштування очікуваних акаунтів і надсилання до неї інструкцій.
`litesvm-go` виконує все це в рамках одного процесу — жодного validator не потрібно.
...
Для тестів, що потребують стандартних програм (SPL Token, Memo, ...):
...
1. **Конфігурація** — увімкніть sysvars, builtins, precompiles за потреби.
2. **Завантаження програми** — `AddProgramFromFile` (або `AddProgram` для байтів
   у пам'яті).
3. **Налаштування акаунтів** — попередньо заповніть акаунти даних за допомогою `SetAccount`.
4. **Надсилання** — сформуйте інструкцію за допомогою `solana-go`, серіалізуйте та відправте через
   `SendLegacyTransaction`.
5. **Перевірка** — зчитайте акаунти за допомогою `GetAccount` та перевірте логи,
   обчислювальні одиниці та стан після виконання.

Після того як цей шаблон запрацює, розширюйте тести, додаючи більше інструкцій, більше
акаунтів та допоміжний код для типових операцій.
