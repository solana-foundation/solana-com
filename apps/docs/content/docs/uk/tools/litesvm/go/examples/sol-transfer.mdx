---
title: Переказ SOL
description: Побудуйте та надішліть транзакцію переказу SOL за допомогою litesvm-go
---

Найпростіша транзакція Solana: переміщення SOL з одного акаунта на інший. Це
гарний перший тест для перевірки правильності налаштування `litesvm-go` перед тестуванням
більш складної логіки програм.
...
<Callout type="info">
  Кінцевий баланс відправника трохи менший за 9 SOL через комісію за транзакцію
  (5 000 lamport за підпис).
</Callout>

Це охоплює найпростіший тип транзакцій. Для тестування з користувацькими програмами дивіться
[Тестування програм](/docs/tools/litesvm/go/examples/program-testing). Для
логіки, що залежить від часу, дивіться
[Тестування на основі часу](/docs/tools/litesvm/go/examples/time-based).
