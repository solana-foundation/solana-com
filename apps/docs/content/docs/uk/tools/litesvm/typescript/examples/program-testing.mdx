...
description: Завантажуйте та тестуйте власні програми Solana за допомогою LiteSVM
---

Тестування власної програми Solana означає завантаження скомпільованого файлу `.so` у
SVM, налаштування очікуваних акаунтів і надсилання інструкцій до неї.
LiteSVM виконує все це у процесі — validator не потрібен. Ми рекомендуємо
генерувати клієнти програм за допомогою
...
Для тестування, яке вимагає стандартних програм:
...
<Callout type="info">
  Зберіть вашу програму Solana за допомогою `cargo build-sbf`, щоб згенерувати файл `.so`.
</Callout>

Шаблон завжди однаковий:

1. **Завантаження програми**: Використовуйте `addProgramFromFile` для завантаження скомпільованих програм і
   імпортуйте myProgramPlugin() з вашого клієнта
2. **Конфігурація**: Увімкніть sysvars, builtins і precompiles за потреби
3. **Налаштування акаунтів**: Попередньо заповніть акаунти даних за допомогою `setAccount`
4. **Надсилання**: Використовуйте `client.myProgram.testInstruction({..}).sendTransaction()` —
   без ручного формування транзакцій
5. **Перевірка**: Перевірте стан акаунта після виконання
   (`client.myProgram.myAccount.fetch`)

Після того як цей каркас запрацює, ви зможете побудувати повний набір тестів,
додаючи більше інструкцій та перевірок.
