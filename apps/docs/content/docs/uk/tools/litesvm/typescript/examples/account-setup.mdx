...
description: Налаштування тестових акаунтів та стану за допомогою LiteSVM
---

# Приклад налаштування акаунта

Демонструє налаштування стану тестового акаунта безпосередньо без транзакцій.

## Базове налаштування акаунта
...
Налаштування акаунта даних, що належить програмі:
...
<Callout type="info">
  Використання `setAccount` є швидшим, ніж виконання транзакцій для масового
  налаштування акаунтів.
</Callout>

## Ключові моменти

1. **Прямий стан**: `setAccount` встановлює стан акаунта без транзакцій
2. **Звільнення від ренти**: Використовуйте `minimumBalanceForRentExemption` для
   розрахунку необхідних lamports
3. **Структура акаунта**: Вкажіть усі обов'язкові поля: address, data,
   executable, lamports, programAddress, space
4. **Перевірка**: Використовуйте `getAccount` або `client.rpc.getAccountInfo`
   для перевірки
