...
description: Тестуйте часозалежну логіку за допомогою маніпуляцій із годинником LiteSVM
---

Демонструє використання `warpToSlot` для тестування часозалежної логіки програми.
...
Ви можете зчитати розклад epoch, щоб зрозуміти зв'язок між slot та epoch у
кластері:
...
<Callout type="warn">
  ...
  `epoch` або `unixTimestamp`. Використовуйте `setClock` для повного контролю над
  полями годинника.
</Callout>

## Ручні маніпуляції з годинником

Коли ваша програма зчитує `epoch` або `unix_timestamp` із системної змінної Clock,
використовуйте `setClock` для безпосереднього встановлення цих полів:
...
Це відповідає шаблону Rust `set_sysvar(&clock)`. Використовуйте `warpToSlot` для простого
просування slot, а `setClock` — коли потрібен контроль над epoch або часовою міткою.
...
| Сценарій                | Підхід                           |
| ----------------------- | ---------------------------------- |
| Вестинг токенів           | Перемотати повз межу/віхи вестингу |
| Завершення аукціонів         | Перемотати повз кінцевий slot аукціону         |
| Нагороди за стейкінг         | Використовуйте `setClock` для встановлення epoch        |
| Виведення коштів із часовим блокуванням | Перемотати повз slot розблокування              |
| Обмеження швидкості           | Перемотати між дозволеними інтервалами     |
| Ордери з терміном дії         | Перемотати повз закінчення терміну ордера         |

<Callout type="info">
  Майте на увазі, що `warpToSlot` змінює лише slot. Unix-мітка часу може не
  оновлюватися пропорційно залежно від реалізації SVM.
</Callout>

## Ключові моменти

1. **Увімкніть Sysvars**: Викличте `withSysvars()` перед використанням методів годинника
2. **Просування вперед**: Використовуйте `warpToSlot(slot)` для просування slot
3. **Повний контроль**: Використовуйте `setClock`, коли потрібно встановити `epoch`,
   `unixTimestamp` або інші поля годинника
4. **Читання годинника**: Використовуйте `getClock()` для зчитування поточного slot, epoch, часової мітки
