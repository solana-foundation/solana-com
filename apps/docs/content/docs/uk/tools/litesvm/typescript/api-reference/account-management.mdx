...
| ---------------- | ------------ | ----------------------------- |
| `address`        | `Address`    | Публічний ключ акаунту (pubkey)            |
...
Розрахувати мінімальну кількість lamport-ів, необхідну для звільнення від rent.
...
Завантажити програму з файлового шляху `.so`.
...
Для тестування можна ефективно налаштувати кілька акаунтів одночасно:
...
<Callout type="info">
  Використання `setAccount` є швидшим за `airdrop` для масового налаштування акаунтів, оскільки
  не виконує транзакції.
</Callout>
