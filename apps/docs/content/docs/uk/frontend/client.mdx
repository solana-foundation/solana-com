...
<Callout type="info">
  ...
  гаманця? Замість сирого keypair підписуйте за допомогою серверного сервісу
  управління ключами. Дивіться [Keychain](/docs/tools/keychain) та [Підписання в
  продакшні](/docs/core/transactions/signing-in-production).
</Callout>

## Поширені патерни для Solana-розробників

- **Фонові скінченні автомати**: Запускайте клієнт всередині API-маршрутів,
  скриптів або воркерів, щоб повторно використовувати ту саму логіку
  оркестрування гаманця + RPC, що живить ваш UI.
...
- **Власні сховища**: Підключіть власне Zustand-сховище для гідратації з
  IndexedDB/localStorage, дзеркального відображення стану в серверні сесії або
  координації між вкладками браузера.
- **Інтеграція з React-хуками**: Передайте налаштований екземпляр клієнта до
  `@solana/react-hooks`, якщо ви хочете зручні хуки поверх того самого рантайму.
- **Тестованість**: Єдиний інтерфейс клієнта можна підробити в юніт-тестах, що
  полегшує симуляцію RPC-відповідей або сесій гаманця без браузерного гаманця.
