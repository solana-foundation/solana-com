...
- Das [`declare_id!`](mention:one) Makro gibt die On-Chain-Adresse Ihres
...
Schließen Sie das Programm, um die dem On-Chain-Programm zugewiesenen SOL
zurückzugewinnen. Führen Sie dazu `solana program close [ProgramID]` aus, unter
Verwendung der Programmadresse, die im _rs`declare_id!()`_ Makro gespeichert
ist.
