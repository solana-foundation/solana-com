...
Die Onchain-Adresse Ihres Programms. Sie wird aktualisiert, wenn Sie das Projekt
erstellen
...
Greift auf das `data` Feld des `new_account` Kontos zu und setzt es auf den Wert
von `data: u64`

### !msg

Das _rs`msg!`_ Makro protokolliert eine Nachricht in den Programm-Logs der
Transaktion
...
- Das [`declare_id!`](mention:one)-Makro gibt die Onchain-Adresse Ihres
  Programms an. Solana Playground aktualisiert diese Adresse automatisch, wenn
  Sie das Programm erstellen.
...
- Das [`#[program]`](mention:one) Attribut annotiert ein Modul, das Funktionen
  enthält, die die Anweisungen des Programms darstellen.
...
- `data: u64`: Ein benutzerdefinierter Parameter, den Sie beim Aufruf der
  Anweisung angeben müssen.

Der Funktionskörper [setzt das `data` Feld von `new_account`](mention:three) auf
das bereitgestellte `data` Argument und gibt dann eine Nachricht in den
Programm-Logs aus.
...
- Das [`#[derive(Accounts)]`](mention:one) Attribut definiert eine Struktur, die
  ...
  Anchor führt automatisch gängige Sicherheitsprüfungen im Zusammenhang mit der
  Konto-Validierung durch Feldtypen (z. B. `Signer<'info>`) und Einschränkungen
  (z. B. `#[account(mut)]`) durch.
...
<Callout>
  Ihre [Programm-ID](/docs/intro/quick-start/build-first-program), definiert in
  _rs`declare_id!()`_, wird nun aktualisiert.
</Callout>

2. SOL zu deinem Wallet hinzufügen _(Optional)_

Die Bereitstellung eines Programms erfordert SOL im Onchain-Konten des
Programms. Falls erforderlich,
[fügen Sie SOL zu Ihrem Wallet hinzu](/docs/intro/quick-start#add-sol-to-your-wallet),
...
1. Öffnen Sie die Testdatei namens `tests/anchor.test.ts`. Diese Datei zeigt,
   wie die [`initialize`](mention:one) Struktur des Programms vom Client
   aufgerufen wird.
...
**Hinweis:** Falls bevorzugt, können Sie die `Test` Schaltfläche im linken
...
<Callout type="warn">
  **Solana Explorer:** Setzen Sie den Netzwerk-Cluster auf `devnet`, um mit dem
  von Solana Playground übereinzustimmen.
</Callout>
...
Schließen Sie das Programm, um die dem Onchain-Programm zugewiesenen SOL
wiederzuerlangen. Führen Sie dazu `solana program close [ProgramID]` aus und
verwenden Sie die Programm-Adresse, die im _rs`declare_id!()`_-Makro gespeichert
ist.
...
[programmabgeleitete Adressen](/docs/core/pda) und
[programmübergreifende Aufrufe](/docs/core/cpi) zu erkunden.
