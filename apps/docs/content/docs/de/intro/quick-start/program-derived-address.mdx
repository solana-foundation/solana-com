...
description:
  Erfahren Sie, wie Sie ein CRUD-Programm (Erstellen, Lesen, Aktualisieren,
  Löschen) auf Solana mit Program Derived Addresses (PDAs) und dem
  Anchor-Framework entwickeln. Diese Schritt-für-Schritt-Anleitung zeigt, wie
  Sie Onchain-Nachrichtenkonten mithilfe von PDAs erstellen, aktualisieren und
  löschen, die Kontenvalidierung implementieren und Tests schreiben. Ideal für
  Entwickler, die verstehen möchten, wie PDAs in Solana-Programmen eingesetzt
  werden.
...
In der Datei `lib.rs` finden Sie ein Programm mit den Anweisungen
[`create`](mention:one), [`update`](mention:two) und [`delete`](mention:three),
die in den folgenden Schritten hinzugefügt werden.
...
überprüfen, ob das Starterprogramm erfolgreich erstellt wird.
...
Aktualisieren Sie in `lib.rs` die Struktur _rs`MessageAccount`_ mit Folgendem:
...
Das Attribut _rs`#[account]`_ in einem Anchor-Programm annotiert Strukturen, die
Kontodaten darstellen (Datentyp, der im Datenfeld des Kontos gespeichert werden
soll).

### !user

Das Feld _rs`user`_ enthält einen _rs`Pubkey`_, der den Benutzer identifiziert,
der das Nachrichtenkonto erstellt hat.

### !message

Das Feld _rs`message`_ enthält einen _rs`String`_ mit der Nachricht des
Benutzers.

### !bump

Das Feld _rs`bump`_ speichert einen _rs`u8`_
...
Rechenleistung, indem die Notwendigkeit entfällt, ihn in späteren Anweisungen
neu zu berechnen.
...
In diesem Beispiel speichert die Struktur _rs`MessageAccount`_ eine von
Benutzern erstellte Nachricht, die drei Felder enthält:

- `user` - _rs`Pubkey`_, der den Benutzer identifiziert, der das
  Nachrichtenkonto erstellt hat.
- `message` - _rs`String`_, der die Nachricht des Benutzers enthält.
- `bump` - _rs`u8`_, der den ["Bump"-Seed](/docs/core/pda#canonical-bump) zur
  Ableitung der Program Derived Address (PDA) speichert. Das Speichern dieses
  Werts spart Rechenleistung, indem die Notwendigkeit entfällt, ihn in späteren
  Anweisungen neu zu berechnen.

Beim Erstellen eines Kontos serialisiert das Programm die _rs`MessageAccount`_
Daten und speichert sie im Datenfeld des neuen Kontos.
...
des Erstellens und Lesens von Kontodaten.

</Accordion>
</Accordions>

Erstelle das Programm erneut, indem du _shell`build`_ im Terminal ausführst.
...
Die _rs`space`_ Einschränkung gibt die Anzahl der Bytes an, die für das
Datenfeld des neuen Kontos zugewiesen werden sollen.
...
Der _rs`Account<'info, T>`_ Typ erfordert, dass das Konto mit dem angegebenen
Typ übereinstimmt.

In diesem Fall muss das Konto mit dem benutzerdefinierten _rs`MessageAccount`_
Typ übereinstimmen.

### !program

Der _rs`Program<'info, T>`_ Typ erfordert, dass das Konto mit einem Programm
übereinstimmt.

In diesem Fall muss das Konto mit dem _rs`System`_ Typ übereinstimmen, der sich
auf das System Program bezieht.
...
In diesem Beispiel definiert die _rs`Create`_ Struktur die Konten, die für die
_rs`create`_ Anweisung erforderlich sind.
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - Im Hintergrund ruft die `init` Einschränkung das System Program auf, um ein
     ...
     Eigentümer auf das aktuelle Programm.
...
- _rs`b"message"`_ - Eine feste Zeichenkette als erster Seed.
- _rs`user.key().as_ref()`_ - Der öffentliche Schlüssel des _rs`user`_ Kontos
  als zweiter Seed.

Die `bump` Einschränkung weist Anchor an, automatisch den korrekten Bump-Seed zu
finden und zu verwenden. Anchor verwendet die `seeds` und `bump`, um die PDA
abzuleiten.
...
- Anchor-Konto-Diskriminator (Identifikator): 8 Bytes
...
speichern, und die verbleibende Länge enthält die tatsächlichen Daten.

</Accordion>
</Accordions>

Fügen Sie als Nächstes die Geschäftslogik für die _rs`create`_ Anweisung hinzu,
...
2. Initialisierung der Kontodaten:
   ...
   - Setzt den `bump` Wert, der zur Ableitung der PDA verwendet wird, abgerufen
     aus `ctx.bumps.message_account`.
...
Füge als Nächstes die `update` Anweisung hinzu, um das `MessageAccount` mit
einer neuen Nachricht zu ändern.

Wie im vorherigen Schritt, gib zuerst die Konten an, die für die `update`
...
1. _rs`user: Signer<'info>`_
   ...
   - Als veränderbar markiert (_rs`#[account(mut)]`_), da es möglicherweise für
     mehr Speicherplatz für das `message_account` bezahlt, wenn erforderlich
   - Muss die Transaktion signieren

2. _rs`message_account: Account<'info, MessageAccount>`_
   - Das vorhandene Konto, das die Nachricht des Benutzers zur Aktualisierung
     speichert
...
3. _rs`system_program: Program<'info, System>`_
   - Erforderlich für die potenzielle Neuzuweisung von Kontospeicherplatz
...
verwendet, der auf dem _rs`message_account`_ gespeichert ist, anstatt ihn von
Anchor neu berechnen zu lassen.
...
Die Funktion `update` implementiert die Logik zur Änderung eines bestehenden
...
2. _rs`message: String`_ - Die neue Nachricht, die die bestehende ersetzt.
...
2. Aktualisiert Kontodaten:
   - Greift auf das `message_account` aus dem Kontext zu.
   - Setzt das `message` Feld auf die neue `message` aus dem Funktionsargument.
...
Die _rs`seeds`_ Einschränkung spezifiziert die seeds, die zur Ableitung der PDA
verwendet werden.

### !bump

Die _rs`bump`_ Einschränkung spezifiziert den bump seed für die PDA.
...
In diesem Fall erhält das _rs`user`_ Konto die Lamports aus dem geschlossenen
...
Die _rs`Delete`_ Struktur definiert die für die _rs`delete`_ Anweisung
...
1. _rs`user: Signer<'info>`_
   ...
   - Muss signieren, um sicherzustellen, dass nur der korrekte Benutzer sein
     Nachrichtenkonto schließen kann

2. _rs`message_account: Account<'info, MessageAccount>`_
   - Das zu schließende Konto
   - `mut` Einschränkung zeigt Datenänderung an
   - `seeds` und `bump` Einschränkungen verifizieren das Konto als korrektes PDA
   - `close = user` Einschränkung markiert dieses Konto zum Schließen und
     überträgt seine Lamports an das `user` Konto
...
Die `delete` Funktion nimmt einen Parameter entgegen:
...
1. _rs`_ctx: Context<Delete>`_ - Bietet Zugriff auf die Konten, die in der
   ...
   Funktion den Context in ihrem Body nicht verwendet.

Der Funktionskörper gibt lediglich eine Nachricht in die Programmlogs aus, indem
er das _rs`msg!()`_ Makro verwendet. Die Funktion benötigt keine zusätzliche
Logik, da die _rs`close`_ Einschränkung in der _rs`Delete`_ Struktur das
Schließen des Kontos übernimmt.
...
Sie haben nun das grundlegende CRUD-Programm fertiggestellt. Deployen Sie das
...
Fügen Sie den folgenden Code innerhalb von _ts`describe()`_, aber vor den
_ts`it()`_ Abschnitten hinzu.
...
Programm ermöglicht, während [`pg.wallet`](mention:two) Zugriff auf Ihr
Playground-Wallet bietet.
...
Führen Sie die Testdatei aus, indem Sie _shell`test`_ im Playground-Terminal
ausführen, um zu überprüfen, ob sie wie erwartet läuft. Die nächsten Schritte
...
aufruft und „Hello, World!" als Nachricht übergibt.
...
aufruft und „Hello, Solana!" als neue Nachricht übergibt.
...
Nach dem Senden der Transaktion und dem Aktualisieren des Kontos ruft die
...
aufruft, um das Nachrichten-Konto zu schließen.
...
<Callout type="info">
  Beachten Sie, dass in diesem Beispiel beim erneuten Ausführen des Tests die
  `create`-Anweisung fehlschlägt, da `messageAccount` bereits als Konto
  existiert. Für eine gegebene PDA kann nur ein Konto existieren.
</Callout>
