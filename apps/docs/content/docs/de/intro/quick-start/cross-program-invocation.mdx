...
SOL-Überweisungen zwischen Konten durch Aufruf des System Programs zu handhaben.
...
namens `vault_account` einzubeziehen. Dieses vom Programm kontrollierte Konto
empfängt SOL von einem Benutzer, wenn dieser sein Nachrichten-Konto
aktualisiert.
...
Struktur hinzu. Dieses Konto dient als programmgesteuerter "Vault", der SOL von
Benutzern empfängt, wenn sie ihre Nachrichten aktualisieren.
...
- Als PDA hat es keinen privaten Schlüssel, sodass nur das Programm für die
  Adresse "signieren" kann, wenn CPIs durchgeführt werden
- Als _rs`SystemAccount`_ Typ gehört es dem System Program wie normale
  Wallet-Konten
...
In der `delete` Anweisung sehen Sie, wie das Programm für diese PDA in einem CPI
"signieren" kann.

</Accordion>
</Accordions>

Als Nächstes fügen Sie die CPI-Logik in der `update` Anweisung hinzu, um 0,001
SOL vom Konto des Benutzers auf das Vault-Konto zu überweisen.
...
Die _rs`CpiContext`_ Struktur spezifiziert das Programm und die Konten für einen
...
In der `update` Anweisung enthält die Implementierung einen Cross Program
Invocation (CPI), um die `transfer` Anweisung des System Programs aufzurufen.
Dies zeigt, wie ein CPI innerhalb des Programms durchgeführt wird und ermöglicht
...
- `to` - Das Tresor-Konto (Ziel der Mittel)
...
- Die im CPI erforderlichen Konten (definiert in der _rs`Transfer`_ Struktur)
...
- Den `cpi_context` (Programm und Konten)
- Den zu übertragenden `amount` (1.000.000 Lamports oder 0,001 SOL)
...
Die Einrichtung für einen CPI entspricht der Art und Weise, wie clientseitige
Anweisungen erstellt werden, bei der Sie das Programm, die Konten und die
instruction data für eine bestimmte aufzurufende Anweisung angeben. Wenn die
`update` Anweisung des Programms einen Aufruf erhält, ruft sie intern die
Transfer-Anweisung des System Programs auf.
...
Fügen Sie nun einen "Rückerstattung bei Löschung"-Mechanismus hinzu, indem Sie
die _rs`Delete`_ Struktur und die `delete` Funktion ändern.

Aktualisieren Sie zunächst die _rs`Delete`_ Struktur, um den `vault_account`
einzuschließen. Dies ermöglicht die Übertragung von SOL im Tresor zurück an den
Benutzer, wenn dieser sein Nachrichtenkonto schließt.
...
Füge außerdem das `system_program` hinzu, da der CPI für die Überweisung das
Aufrufen des System Programs erfordert.
...
Das `vault_account` verwendet dieselbe PDA-Ableitung wie in der Update-Struktur.

Das Hinzufügen des `vault_account` zur Delete-Struktur ermöglicht es dem
Programm, während der Delete-Anweisung auf das Tresor-Konto des Benutzers
zuzugreifen, um angesammeltes SOL an den Benutzer zurückzuüberweisen.
...
Als Nächstes füge die CPI-Logik in der `delete` Anweisung hinzu, um SOL vom
Tresor-Konto zurück auf das Konto des Benutzers zu überweisen.
...
Die _rs`signer_seeds`_ spezifizieren die optionalen Seeds und Bump-Seeds, die
zur Ableitung der PDA verwendet werden.
...
Beachte, dass _rs`_ctx: Context<Delete>`_ sich zu _rs`ctx: Context<Delete>`_
ändert, um den Kontext im Funktionskörper zu verwenden.
...
Die _rs`transfer()`_ Funktion ruft dann die Überweisungsanweisung im System
Program auf und übergibt:

- Den `cpi_context` (Programm, Konten und PDA-Signer)
- Den zu überweisenden Betrag (das gesamte Guthaben des Tresor-Kontos)
...
Als Nächstes aktualisiere die `anchor.test.ts` Datei, um das neue Tresor-Konto
in die Anweisungen einzubeziehen. Dies erfordert die Ableitung der Tresor-PDA
und deren Einbeziehung in die Update- und Delete-Anweisungsaufrufe.
...
Aktualisieren Sie dann die Update-Anweisung, um die
[`vaultAccount`](mention:one) einzuschließen
...
Aktualisieren Sie dann die Delete-Anweisung, um die
[`vaultAccount`](mention:one) einzuschließen
...
Klicken Sie auf die Schaltfläche `Import` und geben Sie einen Projektnamen ein,
um es zu Ihrer Liste der Projekte in Solana Playground hinzuzufügen. Sobald ein
Projekt importiert wurde, werden alle Änderungen automatisch gespeichert und
persistent gehalten.
