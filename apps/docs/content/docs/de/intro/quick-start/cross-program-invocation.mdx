...
namens `vault_account` einzuschließen. Dieses vom Programm kontrollierte Konto
empfängt SOL von einem Benutzer, wenn dieser sein Nachrichtenkonto aktualisiert.
...
Benutzern empfängt, wenn diese ihre Nachrichten aktualisieren.
...
SOL vom Konto des Benutzers auf das Tresor-Konto zu überweisen.
...
Dies zeigt, wie man einen CPI innerhalb des Programms durchführt und ermöglicht
...
Die `transfer` Funktion ruft dann die Überweisungsanweisung im System Program
auf und übergibt:
...
Anweisungsdaten für eine bestimmte aufzurufende Anweisung angeben. Wenn die
`update` Anweisung des Programms einen Aufruf erhält, ruft sie intern die
Überweisungsanweisung des System Programs auf.
...
Aktualisieren Sie zunächst die _rs`Delete`_ Struktur, um das `vault_account`
einzubeziehen. Dies ermöglicht die Übertragung von SOL im Vault zurück an den
Benutzer, wenn dieser sein Nachrichten-Konto schließt.
...
Fügen Sie außerdem das `system_program` hinzu, da der CPI für die Überweisung
das Aufrufen des System Programs erfordert.
...
Programm, während der Delete-Anweisung auf das Vault-Konto des Benutzers
...
Beachten Sie, dass _rs`_ctx: Context<Delete>`_ sich zu
...
Als Nächstes aktualisieren Sie die `anchor.test.ts` Datei, um das neue
Vault-Konto in die Anweisungen einzubeziehen. Dies erfordert die Ableitung der
Vault-PDA und deren Einbeziehung in die Update- und Delete-Anweisungsaufrufe.
