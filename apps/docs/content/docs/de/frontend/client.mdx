...
<Callout type="info">
  Headless-Betrieb — in einer API-Route, einem Worker oder einem Skript ohne
...
- **Echtzeit-Dashboards**: Kombinieren Sie Beobachter (Guthaben, Konten,
  Signaturen) mit Ihrer bevorzugten UI-Bibliothek; der Client übernimmt
  WebSocket-Fan-out und Cache- Invalidierung.
- **Benutzerdefinierte Stores**: Binden Sie Ihren eigenen Zustand-Store ein, um
  Daten aus IndexedDB/localStorage zu laden, den Zustand mit Server-Sitzungen zu
  synchronisieren oder zwischen Browser-Tabs zu koordinieren.
