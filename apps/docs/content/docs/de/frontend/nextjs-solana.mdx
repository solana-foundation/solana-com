...
description:
  Solana-Wallet-Integration in Next.js mit @solana/kit und @solana/react
  einrichten, um SOL-Transfers zu senden.
---
...
Dropdown zur Wallet-Verbindung und eine SOL-Transfer-Komponente.
...
Wenn Sie dazu aufgefordert werden, akzeptieren Sie alle Standardeinstellungen
(der Starter enthält Tailwind, das in diesem Tutorial für einfache
Utility-Styles verwendet wird).

<Callout type="info" title="Zwei Scaffold-Anpassungen">
Die standardmäßige `create-next-app`-Ausgabe benötigt zwei kleine Änderungen für dieses Tutorial:

- In `tsconfig.json` setze `"target": "ES2020"` (oder höher). Die
  Transferbeträge verwenden `bigint`-Literale wie `1_000_000_000n`, die ES2020
  erfordern.
- In `app/globals.css` entferne den generierten
  `@media (prefers-color-scheme: dark)`-Block und die `body`-Hintergrundregeln.
  Diese Oberfläche ist nur für den hellen Modus; die dunklen Standardwerte
  überschreiben sonst das `bg-white`-Utility und machen die Seite im Dunkelmodus
  unleserlich.
...
Dieses Tutorial richtet sich an Kit 7+, die `@solana/kit-plugin-*`-Pakete ab
0.13+ und `@solana/react` 7+. Falls dein Paketmanager Peer-Warnungen von einem
`@solana-program/*`-Paket anzeigt, das einen älteren Kit-Peer-Bereich
deklariert, überschreibe ihn – die Pakete funktionieren mit Kit 7.

<ScrollyCoding>

## !!steps 1. Solana-Provider erstellen

`app/providers.tsx` erstellt einen Client im Modulbereich und stellt ihn über
`ClientProvider` bereit. Das `walletSigner`-Plugin macht die verbundene Wallet
zum Fee- Payer und zur Identität; exportiere den Client-Typ, damit Komponenten
`useClient` typisieren können.
...
Umhülle deine Anwendung mit der `Providers`-Komponente. Aktualisiere
...
Dies umhüllt die gesamte App mit dem Solana-Kontext, sodass alle untergeordneten
Komponenten auf den Client und die Wallet-Hooks zugreifen können.

## !!steps 3. Wallet Connect Button

Dropdown zum Verbinden/Trennen von Wallet Standard Wallets. `useWallets` gibt
die im Browser erkannten Wallets zurück; `useConnect` / `useDisconnect` geben
ein `ActionResult` zurück — diese Komponente verwendet dessen `dispatch`-,
`isRunning`- und `error`-Felder.
...
Erstelle eine Transfer-Anweisung mit `@solana-program/system` und sende sie über
`client.sendTransaction`. Das verbundene Wallet signiert und bezahlt.
...
Wallets werden über die Wallet Standard-Erkennung bereitgestellt; nach der
Verbindung verwendet der SOL-Transfer den verbundenen Signer als Fee-Zahler.
...
Öffne http://localhost:3000, verbinde ein Devnet-Wallet und sende einen
SOL-Transfer.
