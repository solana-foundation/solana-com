...
description:
  Ändern oder widerrufen Sie Berechtigungsrollen auf einem Mint oder
  Token-Konto, einschließlich Mint-, Freeze-, Owner- und Close-Berechtigung.
...
## Was bewirkt das Festlegen von Berechtigungen?

Das Festlegen von Berechtigungen ändert oder widerruft bestimmte
Berechtigungsrollen auf einem Mint- oder Token-Konto.
...
Das Ändern einer Berechtigung verwendet die Anweisungen _rs`SetAuthority`_ des
Token Program.

Jeder Aufruf aktualisiert einen Berechtigungstyp auf einem Mint- oder
Token-Konto. Die aktuelle Berechtigung signiert die Änderung, und die neue
Berechtigung kann eine andere Adresse oder _rs`None`_ sein, um die ausgewählte
Berechtigung dauerhaft zu widerrufen.
