...
description:
  Erfahren Sie, wie Sie Berechtigungen für Mints oder Token-Konten festlegen.
---

## Token-Berechtigungen verstehen

### Mint-Konten-Berechtigungen

- **Mint-Berechtigung**: Kontrolliert die Erstellung neuer Token. Kann Token an
  jedes Token-Konto minten. Wird oft nach der Erstellung des anfänglichen
  Angebots widerrufen, um ein Token mit festem Angebot zu erstellen.
- **Freeze-Berechtigung**: Kontrolliert die Fähigkeit, Token-Konten einzufrieren
  und aufzutauen. Kann verhindern, dass Token-Konten Token übertragen. Wird oft
  widerrufen, um Benutzern zu garantieren, dass ihre Token nicht eingefroren
  werden können.

### Token-Konten-Berechtigungen

- **Konten-Eigentümer**: Hat die volle Kontrolle über das Token-Konto. Kann
  Token übertragen, Token verbrennen, Delegierte genehmigen und das Konto
  schließen, wenn der Saldo null ist.
- **Close-Berechtigung**: Kann das Token-Konto schließen, wenn sein Saldo null
  ist. Standardmäßig ist dies der Konten-Eigentümer, kann aber an ein anderes
  Konto delegiert werden.

## Berechtigung festlegen

Die
[`SetAuthority`](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L153)-Anweisung
ändert oder widerruft Berechtigungen für Mints und Token-Konten. Nur die
aktuelle Berechtigung kann ihre Berechtigungen an eine neue Adresse übertragen
oder sie dauerhaft widerrufen, indem die Berechtigung auf null gesetzt wird.
Nach dem Widerruf kann die Berechtigung nicht wiederhergestellt werden.
