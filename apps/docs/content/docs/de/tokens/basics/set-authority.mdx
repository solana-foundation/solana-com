---
title: Berechtigung festlegen
description:
  Ändern oder widerrufen Sie Berechtigungsrollen für ein Mint- oder Token-Konto,
  einschließlich Mint-, Freeze-, Owner- und Close-Berechtigung.
...
## Was bewirkt das Setzen einer Berechtigung?

Das Setzen einer Berechtigung ändert oder widerruft bestimmte
Berechtigungsrollen für ein Mint- oder Token-Konto.

Mint-Konten können eine Mint-Berechtigung und eine Freeze-Berechtigung
speichern. Token-Konten speichern einen Owner und können optional eine
Close-Berechtigung speichern. Jede Rolle steuert einen bestimmten
Berechtigungssatz, und das Setzen einer Berechtigung auf _rs`None`_ entfernt die
ausgewählte Berechtigungsrolle dauerhaft.

Das Token Extensions Program definiert außerdem zusätzliche Berechtigungstypen
für spezifische Erweiterungen.

## So setzen Sie eine Berechtigung

Das Ändern einer Berechtigung verwendet die _rs`SetAuthority`_-Anweisung des
Token Program.

Jeder Aufruf aktualisiert einen Berechtigungstyp für ein Mint- oder Token-Konto.
Die aktuelle Berechtigung signiert die Änderung, und die neue Berechtigung kann
eine andere Adresse oder _rs`None`_ sein, um die ausgewählte Berechtigung
dauerhaft zu widerrufen.

### Quellenreferenz

| Element                     | Beschreibung                                                                                | Token Program                                                                                                  | Token Extensions Program                                                                                               |
| --------------------------- | ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                  | Der Mint-Status speichert _rs`mint_authority`_ und _rs`freeze_authority`_.                  | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)           |
| _rs`Account`_               | Der Token-Kontostatus speichert _rs`owner`_ und _rs`close_authority`_.                      | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)          |
| _rs`AuthorityType`_         | Die Berechtigungsrollen, die _rs`SetAuthority`_ aktualisieren kann.                         | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L739-L751) | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1116-L1157) |
| _rs`SetAuthority`_          | Eine Anweisung, die eine Berechtigung für ein Mint- oder Token-Konto ändert oder widerruft. | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L141-L158) | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L183-L200)   |
| _rs`process_set_authority`_ | Gemeinsame Prozessorlogik für Berechtigungsaktualisierungen.                                | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L423-L518)     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L677-L978)       |

### Typescript

Die nachfolgenden `Kit`-Beispiele zeigen den empfohlenen Ansatz mit
