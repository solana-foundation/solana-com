---
title: Delegierten genehmigen
description:
  Autorisieren Sie eine andere Adresse, eine begrenzte Anzahl von Token von
  einem Token-Konto zu übertragen oder zu brennen.
...
## Was bewirkt die Genehmigung eines Delegierten?

Die Genehmigung eines Delegierten erlaubt es einer anderen Adresse, eine
begrenzte Anzahl von Token von einem Token-Konto im Namen des
Token-Konto-Eigentümers zu übertragen oder zu brennen.

Ein Token-Konto speichert gleichzeitig einen aktuellen Delegierten und einen
delegierten Betrag. Die Genehmigung eines neuen Delegierten ersetzt den
vorherigen Delegierten und die Zulage auf dem Konto.

## Wie man einen Delegierten genehmigt

Die Genehmigung eines Delegierten verwendet die _rs`Approve`_- oder
_rs`ApproveChecked`_- Anweisungen des Token Program.

Die folgenden Beispiele verwenden _rs`ApproveChecked`_, was erfordert, dass der
Aufrufer die Mint- und Dezimalstellen angibt, damit die Anweisung die erwartete
Mint und Token-Präzision überprüfen kann, bevor die Genehmigung verarbeitet
wird.

Der Token-Konto-Eigentümer signiert die Genehmigung.

### Quellenreferenz

| Element               | Beschreibung                                                                                                                                                                                         | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_         | Der Token-Konto-Status speichert den Eigentümer, den Delegierten und den delegierten Betrag.                                                                                                         | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | Eine Delegierten-Genehmigungs- Anweisungen, die den Delegierten und die Zulage für ein Token-Konto aufzeichnet, ohne dass der Aufrufer die Mint oder Dezimalstellen angeben muss.                    | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | Eine Delegierten-Genehmigungs- Anweisungen, die erfordert, dass der Aufrufer die Mint und Dezimalstellen angibt und diese Werte überprüft, bevor der Delegierte und die Zulage aufgezeichnet werden. | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
| _rs`process_approve`_ | Gemeinsame Prozessor-Logik für die Delegierten-Genehmigung.                                                                                                                                          | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L344-L392)     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L582-L640)     |
