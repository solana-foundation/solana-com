...
description:
  Autorisieren Sie eine andere Adresse, um eine begrenzte Anzahl von Token von
  einem Token-Konto zu übertragen oder zu verbrennen.
...
Die Genehmigung eines Delegierten erlaubt es einer anderen Adresse, im Namen des
Token-Konto-Inhabers eine begrenzte Anzahl von Token von einem Token-Konto zu
übertragen oder zu verbrennen.

Ein Token-Konto speichert jeweils einen aktuellen Delegierten und einen
delegierten Betrag. Die Genehmigung eines neuen Delegierten ersetzt den
vorherigen Delegierten und die Zuteilung auf dem Konto.

## So genehmigen Sie einen Delegierten

Die Genehmigung eines Delegierten verwendet die Anweisung _rs`Approve`_ oder
_rs`ApproveChecked`_ des Token Program.

Die folgenden Beispiele verwenden _rs`ApproveChecked`_, das vom Aufrufer
verlangt, dass Mint und Dezimalstellen angegeben werden, damit die Anweisung das
erwartete Mint und die Token-Präzision überprüfen kann, bevor die Genehmigung
verarbeitet wird.

Der Inhaber des Token-Kontos signiert die Genehmigung.

### Quellreferenz

| Element               | Beschreibung                                                                                                                                                                                   | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_         | Der Token-Konto-Status speichert den Inhaber, den Delegierten und den delegierten Betrag.                                                                                                      | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | Eine Delegierten-Genehmigungs-Anweisung, die den Delegierten und die Zuteilung für ein Token-Konto aufzeichnet, ohne dass der Aufrufer Mint oder Dezimalstellen angeben muss.                  | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | Eine Delegierten-Genehmigungs-Anweisung, die vom Aufrufer verlangt, Mint und Dezimalstellen anzugeben, und diese Werte überprüft, bevor der Delegierte und die Zuteilung aufgezeichnet werden. | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
| _rs`process_approve`_ | Gemeinsame Prozessorlogik für die Delegierten-Genehmigung.                                                                                                                                     | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L344-L392)     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L582-L640)     |
