...
Account ist genau einem Mint zugeordnet und verfolgt Ihr Token-Guthaben sowie
zusätzliche Details.
...
<Callout type="info">
  Beachten Sie, dass Entwickler im Quellcode einen Token Account als
  ...
  teilen dieselbe Basisimplementierung für den Token Account.
</Callout>

Um Tokens zu halten, benötigen Sie einen Token Account für diesen spezifischen
Mint. Jeder Token Account verfolgt:

1. **Mint**: Den spezifischen Token-Typ, den er hält
2. **Owner**: Die Autorität, die Tokens von diesem Konten übertragen kann

### Beispiel: USD Coin (USDC)

- Mint-Adresse: `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Token Account von Circle: `3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
- Dieser Konten hält nur USDC-Tokens
- Circle kontrolliert Token über die Owner-Autorität des Token Accounts:
...
<Callout type="info">
  **Die Terminologie „Owner" verstehen**
...
1. **Token Account Owner**: Die Autorität, die Tokens übertragen, verbrennen
   oder delegieren kann. Diese wird in den Daten des Token Accounts gespeichert.

2. **Program Owner**: Das Programm, das den Konten besitzt (immer das Token
   Program oder Token Extension Program für Token Accounts).

Bei der Arbeit mit Token Accounts bezieht sich „Owner" typischerweise auf die
Autorität, die Tokens übertragen kann, nicht auf das Programm, das den Konten
besitzt.
...
ATAs verwenden deterministische Adressen, was es einfach macht, das Token-Konto
einer beliebigen Wallet für einen bestimmten Mint zu finden. Siehe die
...
Für jede Kombination aus Wallet und Token gibt es genau eine ATA-Adresse. Dies
macht es überflüssig, Token-Konto-Adressen manuell zu verfolgen – Sie können die
richtige Adresse immer ableiten.
...
- Cross-Program Invocations (CPIs) an das Token Program durchführt
...
Das Erstellen eines Token-Kontos erfordert die
...
Sie benötigen zwei Anweisungen, um ein Token-Konto zu erstellen:

1. System Program: Erstellen Sie ein Konto mit zugewiesenem Speicherplatz für
   ein Token-Konto und übertragen Sie das Eigentum an das Token Program
2. Token Program: Initialisieren Sie die Token-Konto-Daten
...
## Wie man einen Associated Token Account erstellt
...
Diese einzelne Anweisung übernimmt automatisch das Erstellen des Token-Kontens
und dessen Initialisierung durch Cross Program Invocations (CPI).
