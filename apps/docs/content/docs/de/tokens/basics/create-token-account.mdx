...
Ein token account speichert Token für eine Mint und einen
Token-Konten-Eigentümer auf Solana.
...
übertragen, verbrennen oder delegieren kann. Der Programmeigentümer des Kontos
ist weiterhin das Token Program oder Token Extensions Program.

Jedes token account ist an genau eine Mint gebunden, was bedeutet, dass das
token account nur Einheiten eines einzigen Tokens halten kann – des Tokens, der
durch das _rs`mint`_-Feld des token account identifiziert wird.
...
Das Associated Token Program ist eine Möglichkeit, ein token account an einer
standardisierten, deterministischen Adresse zu erstellen. Das resultierende
Konto ist weiterhin ein token account, das dem Token Program oder Token
Extensions Program gehört, nicht dem Associated Token Program.
...
dieser Adresse, und das resultierende Konto verwendet weiterhin den
_rs`Account`_-Typ, der vom Token Program oder Token Extensions Program definiert
wird.

## So erstellen Sie ein Associated Token Account

Das Erstellen eines Associated Token Accounts verwendet die _rs`Create`_- oder
_rs`CreateIdempotent`_-Anweisung des Associated Token Programs. Das Associated
Token Program leitet die ATA-Adresse ab, erstellt das Konto an der ATA-Adresse
und initialisiert das Konto als Token-Konto, das dem Token Program oder Token
Extension Program gehört.

<Callout type="info" title="Empfohlen">
  Für die meisten Anwendungen sollten Token-Konten über das Associated Token
  Program erstellt werden, anstatt sie direkt über die Anweisungen des System
  Programs und Token Programs zu erstellen. Ein Associated Token Account
  verwendet eine deterministische Adresse, die vom Inhaber, Token Program und
  Mint abgeleitet wird, wodurch das Standard-Token-Konto für einen bestimmten
  Mint für Wallets und Anwendungen leichter auffindbar ist.
</Callout>
...
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| _rs`Create`_                                  | Erstellt ein Associated Token Account an der abgeleiteten ATA-Adresse.                                                                                                                                                                                                                 | [Quelle](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L20-L29) |
| _rs`CreateIdempotent`_                        | Erstellt das Associated Token Account, ist aber trotzdem erfolgreich, wenn dieses ATA bereits für denselben Inhaber und Mint existiert.                                                                                                                                                | [Quelle](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | Leitet die ATA-Adresse ab und verwendet _rs`create_pda_account`_ sowie CPIs zum ausgewählten Token Program, um das Token-Konto zu initialisieren. Wenn das ausgewählte Programm das Token Extension Program ist, initialisiert der ATA-Prozessor auch die Immutable-Owner-Erweiterung. | [Quelle](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
...
1. Die _rs`CreateAccount`_- Anweisungen des System Program erstellt ein neues
   mietbefreites Konto und weist das Token Program als Programmeigentümer des
   neuen Kontos zu.
2. Die _rs`InitializeAccount`_-, _rs`InitializeAccount2`_- oder
   _rs`InitializeAccount3`_- Anweisungen des Token Program initialisiert das
   neue Konto für eine Mint und einen Eigentümer.

Fügen Sie die _rs`CreateAccount`_- Anweisungen und die
Token-Konto-Initialisierungs Anweisungen in derselben Transaktion ein.

Während der Token-Konto-Initialisierung prüft das Token Program, ob das Konto
noch nicht initialisiert ist und mietbefreit ist.

<Callout type="info">
  Der folgende Abschnitt zeigt, wie Sie ein Token-Konto erstellen, indem Sie
  ...
  Verwenden Sie direkte Aufrufe des System Program und Token Program, wenn Sie
...
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_                     | Die grundlegenden Token-Kontofelder, die in jedem Token-Konto gespeichert sind.                                                              | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`InitializeAccount`_           | Eine Token-Konto-Initialisierungs Anweisungen, die den Eigentümer und das Mietsysvar-Konto in ihrer Kontenliste erwartet.                    | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L45-L63)   | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L75-L93)   |
| _rs`InitializeAccount2`_          | Eine Token-Konto-Initialisierungs Anweisungen, die den Eigentümer in den instruction data anstatt in der Kontenliste übergibt.               | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L352-L365) | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L419-L433) |
| _rs`InitializeAccount3`_          | Eine Token-Konto-Initialisierungs Anweisungen, die den Eigentümer in den instruction data übergibt und das Mietsysvar-Konto nicht erfordert. | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L377-L387) | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L445-L456) |
| _rs`_process_initialize_account`_ | Gemeinsame Prozessorlogik für die Token-Konto-Initialisierung.                                                                               | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L83-L141)      | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L153-L234)     |
...
Die nachfolgenden `Kit`-Beispiele zeigen den empfohlenen Ansatz unter Verwendung
von `@solana/kit`. Legacy-Beispiele mit `@solana/web3.js` sind als Referenz
enthalten.
