...
description:
  Erstellen Sie Token-Konten und zugeordnete Token-Konten für eine bestimmte
  Mint und einen bestimmten Eigentümer.
...
Hierbei bedeutet _rs`owner`_ die Berechtigung, Token vom token account zu
übertragen, zu verbrennen oder zu delegieren. Der Programm-Eigentümer des Kontos
...
Extensions Program, nicht im Besitz des Associated Token Program.

Das Associated Token Program leitet die ATA-Adresse ab, wie in
...
oder Token Extensions Program gehört.

<Callout type="info" title="Empfohlen">
  Für die meisten Anwendungen sollten Token-Konten über das Associated Token
  Program erstellt werden, anstatt sie durch direkten Aufruf der System Program-
  ...
  bestimmten Mint für Wallets und Anwendungen einfacher zu finden ist.
</Callout>
...
| _rs`CreateIdempotent`_                        | Erstellt das Associated Token Account, ist aber auch dann erfolgreich, wenn dieses ATA bereits für denselben Eigentümer und Mint existiert.                                                                                                                                               | [Quelle](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | Leitet die ATA-Adresse ab und verwendet _rs`create_pda_account`_ plus CPIs an das ausgewählte Token-Programm, um das Token-Konto zu initialisieren. Wenn das ausgewählte Programm das Token Extensions Program ist, initialisiert der ATA-Prozessor auch die Immutable-Owner-Erweiterung. | [Quelle](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
...
<Callout type="info">
  Der folgende Abschnitt zeigt, wie Sie ein Token-Konto erstellen, indem Sie die
  Anweisungen des System Program und Token Program direkt aufrufen. Verwenden
  Sie für die meisten Anwendungen stattdessen das Associated Token Program.
...
Die untenstehenden `Kit`-Beispiele zeigen den empfohlenen Ansatz mit
