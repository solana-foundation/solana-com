...
description:
  Erstellen Sie Token-Konten und zugehörige Token-Konten für eine bestimmte Mint und einen bestimmten Eigentümer.
...
Das Erstellen eines ATA erfordert ein rückerstattungsfähiges Mindestguthaben für den Onchain-Speicher des Konten sowie die Transaktionsgebühr. Der `payer`, der an die Erstellungs-Anweisungen übergeben wird, finanziert das Speicherguthaben und muss unterschreiben. Der Zahler kann sich sowohl vom Transaktionsgebührenzahler als auch vom Wallet, das den ATA besitzen wird, unterscheiden – so kann eine Anwendung einen ATA im Namen eines Empfängers erstellen.

Zu den aktuellen Cluster-Speicherraten betragen die Grundkosten:

| Token-Programm                             | ATA-Größe  | Mindestspeicherguthaben             |
| ----------------------------------------- | --------- | ----------------------------------- |
| Original Token Program                    | 165 Bytes | 2.039.280 lamports (0,00203928 SOL) |
| Token-2022 mit automatisch unveränderlichem Eigentümer | 170 Bytes | 2.074.080 lamports (0,00207408 SOL) |

Token-2022-Mints können zusätzlich zur automatischen _rs`ImmutableOwner`_-Erweiterung weitere Konten-Erweiterungen erfordern. Diese ATAs sind größer und benötigen ein höheres Mindestguthaben. Das Associated Token Program fragt das ausgewählte Token Program nach der erforderlichen Kontengröße und finanziert den ATA entsprechend.

Diese Guthaben sollten nicht fest in der Anwendungslogik kodiert werden. Verwenden Sie
[`getMinimumBalanceForRentExemption`](/docs/rpc/http/getminimumbalanceforrentexemption)
für die erforderliche Datengröße, oder lassen Sie das Associated Token Program dies während der Erstellung berechnen und finanzieren.

<Callout type="info" title="Das Speicherguthaben ist rückerstattungsfähig">
  Das Schließen eines leeren token account überträgt alle seine lamports, einschließlich des
  Mindestspeicherguthabens, an das in der Schließungs-Anweisung angegebene Ziel. Der ursprüngliche Ersteller ist nicht automatisch der Rückerstattungsempfänger. Siehe [Close Token Account](/docs/tokens/basics/close-account).
</Callout>
