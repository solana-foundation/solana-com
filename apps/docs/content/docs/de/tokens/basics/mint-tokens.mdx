...
Beim Prägen werden neue Einheiten eines bestehenden Tokens erstellt und der
Gesamtvorrat des Mints erhöht.

Nur die Mint-Berechtigung kann neue Token prägen. Das Ziel-Token-Konto muss
bereits existieren und demselben Mint angehören.

## Wie man Token prägt

Das Prägen von Token verwendet die _rs`MintTo`_- oder _rs`MintToChecked`_-
Anweisungen des Token Program.

Die folgenden Beispiele verwenden _rs`MintToChecked`_, das vom Aufrufer
verlangt, die Dezimalstellen des Mints anzugeben, damit die Anweisung die
