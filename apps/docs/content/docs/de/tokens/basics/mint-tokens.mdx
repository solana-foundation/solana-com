...
- Nur die Mint-Autorität kann neue Tokens prägen
- Ein Ziel-Token-Konto muss existieren, um die geprägten Tokens zu erhalten
...
async fn main() -> Result<()> {
    // Verbindung zum lokalen Validator erstellen
    ...
    // Standard-Mint-Kontengröße (in Bytes) abrufen, keine Extensions aktiviert
    let mint_space = Mint::LEN;
    let mint_rent = client
        .get_minimum_balance_for_rent_exemption(mint_space)
        .await?;

    // Anweisung zum Erstellen eines neuen Kontos für Mint (Token Program)
    ...
    // Anweisung zum Initialisieren der Mint-Kontodaten
    ...
    // Berechne die associated token account Adresse für fee_payer
    ...
    // Erstelle Transaktion und füge Anweisungen hinzu
    ...
    // Anzahl der zu mintenden Token (100 Token mit 2 Dezimalstellen)
    let amount = 100;

    // Erstelle mint_to Anweisung, um Token zum associated token account zu minten
    ...
    // Erstelle Transaktion zum Minten von Token
    let transaction = Transaction::new_signed_with_payer(
        &[mint_to_instruction],
        Some(&fee_payer.pubkey()),
        &[&fee_payer],
    ...
    println!("1,00 Token zum associated token account gemintet");
    println!("\nMint-Adresse: {}", mint.pubkey());
    println!("{:#?}", mint_data);

    println!(
        "\nAssociated Token Account Adresse: {}",
        associated_token_address
    );
    println!("{:#?}", token_data);

    println!("Transaktionssignatur: {}", transaction_signature);
...
async def main():
    ...
    # Beispiel-Keypairs und Adressen
    payer = Keypair()
    ...
    # Zu prägende Menge (in kleinster Einheit)
    amount_to_mint = 1000000000  # 1 Token mit 9 Dezimalstellen

    async with rpc:
        # Mint-Anweisung erstellen
        ...
        print(f"Ziel: {destination_token_account}")
        print(f"Menge: {amount_to_mint}")
        print(f"Mint-Autorität: {mint_authority.pubkey()}")
        print(f"Zahler: {payer.pubkey()}")
        print(f"Mint-Transaktion erfolgreich erstellt")
