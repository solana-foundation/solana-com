...
description:
  Einfrieren eines Token-Kontos mit der Freeze-Autorität der Mint, um
  Überweisungen, Verbrennungen und Delegierungsänderungen zu blockieren.
...
Das Einfrieren eines Token-Kontos behält denselben Eigentümer, dieselbe Mint und
denselben Saldo bei, verhindert jedoch, dass das Token-Konto Tokens überweist
oder verbrennt, bis das Token-Konto wieder aufgetaut wird.

Ein eingefrorenes Konto kann trotzdem geschlossen werden, wenn das eingefrorene
Konto einen Token-Saldo von null aufweist.

Nur eine Mint, die noch über eine Freeze-Autorität verfügt, kann Token-Konten
einfrieren. Wenn die Freeze-Autorität einer Mint widerrufen wird, können Konten
für diese Mint nicht mehr eingefroren werden.

## So frieren Sie ein Token-Konto ein

Das Einfrieren eines Token-Kontos verwendet die _rs`FreezeAccount`_- Anweisungen
des Token Program.

Die _rs`FreezeAccount`_-Anweisung verifiziert, dass das Token-Konto zur Mint
gehört und dass der Signer die Freeze-Autorität der Mint ist, und ändert dann
den Token-Konto-Status von initialisiert auf eingefroren.
...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Der Mint-Status speichert die optionale Freeze-Autorität, die zum Einfrieren von Token-Konten verwendet wird. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Die Token-Konto-Zustände, einschließlich _rs`Initialized`_ und _rs`Frozen`_.                                  | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | Eine Anweisung, die ein Token-Konto in den eingefrorenen Zustand versetzt.                                    | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
| _rs`process_toggle_freeze_account`_ | Gemeinsame Prozessorlogik zum Einfrieren und Auftauen von Token-Konten.                                       | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |
