...
Anweisung verhindert alle Token-Überweisungen oder Token-Verbrennungen von einem
bestimmten Token-Konto. Sobald das Konto eingefroren ist, kann es weder Token
senden noch empfangen, bis es wieder freigegeben wird. Ein eingefrorenes Konto
kann weiterhin geschlossen werden, wenn es einen Token-Saldo von null aufweist.
Nur die Freeze-Berechtigung der Token-Mint kann Konten einfrieren. Wenn die
Freeze-Berechtigung auf dem Mint-Konto widerrufen wird (auf null gesetzt),
können Token niemals eingefroren werden.
