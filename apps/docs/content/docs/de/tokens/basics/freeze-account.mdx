...
description:
  Frieren Sie ein Token-Konto mit der Freeze-Berechtigung des Mints ein, um
  Übertragungen, Burns und Änderungen von Delegierten zu blockieren.
...
## Was bewirkt das Einfrieren eines Token-Kontos?

Das Einfrieren eines token account behält denselben Eigentümer, Mint und Saldo
bei, verhindert jedoch, dass das token account Token empfängt, überträgt oder
verbrennt, bis das token account wieder aufgetaut wird.

Ein eingefrorenes Konto kann weiterhin geschlossen werden, wenn das eingefrorene
Konto einen Token-Saldo von null aufweist.

Nur ein Mint, der noch über eine Freeze-Berechtigung verfügt, kann Token-Konten
einfrieren. Wenn die Freeze-Berechtigung eines Mints widerrufen wird, können
Konten für diesen Mint nicht mehr eingefroren werden.

## So frieren Sie ein Token-Konto ein

Das Einfrieren eines Token-Kontos verwendet die _rs`FreezeAccount`_-Anweisung
des Token-Programms.

Die _rs`FreezeAccount`_-Anweisung überprüft, ob das Token-Konto zum Mint gehört
und ob der Signer die Freeze-Berechtigung des Mints ist, und ändert dann den
Zustand des Token-Kontos von initialisiert zu eingefroren.
...
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Der Mint-Zustand speichert die optionale Freeze-Berechtigung, die zum Einfrieren von Token-Konten verwendet wird. | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Die Token-Kontenzustände, einschließlich _rs`Initialized`_ und _rs`Frozen`_.                                      | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | Eine Anweisung, die ein Token-Konto in den eingefrorenen Zustand versetzt.                                        | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
| _rs`process_toggle_freeze_account`_ | Gemeinsame Prozessorlogik für das Einfrieren und Auftauen von Token-Konten.                                       | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |
