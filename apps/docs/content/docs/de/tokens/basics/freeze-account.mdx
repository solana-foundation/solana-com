...
bestimmten Token-Konto. Sobald ein Konto eingefroren ist, kann es keine Token
senden oder empfangen, bis es wieder aufgetaut wird. Ein eingefrorenes Konto
kann dennoch geschlossen werden, wenn es einen Token-Saldo von null aufweist.
Nur die Freeze-Autorität der Token-Mint kann Konten einfrieren. Wenn die
Freeze-Autorität auf dem Mint-Konto widerrufen wird (auf null gesetzt), können
