...
description:
  Schließen Sie ein Token-Konto und geben Sie die Lamports des Kontos an ein Zielkonto zurück.
...
Das Schließen eines token account löscht das Konto und überträgt alle seine lamports,
einschließlich des rückerstattungsfähigen Mindestspeicherguthabens, auf ein Zielkonto.

Das Token-Guthaben muss null sein, bevor das Konto geschlossen werden kann. Ein gesperrtes Konto
kann dennoch geschlossen werden, wenn das Token-Guthaben des gesperrten Kontos null ist. Wrapped SOL
token accounts sind die Ausnahme und können mit einem Token-Guthaben geschlossen werden, um
den zugrunde liegenden SOL zurückzufordern. Der Kontoinhaber oder die Close-Autorität des Kontos
unterzeichnet die Abschluss- Anweisungen und wählt das Ziel aus. Das Konto, das für die
Erstellung eines associated token account bezahlt hat, erhält das zurückgewonnene Guthaben nicht automatisch.
