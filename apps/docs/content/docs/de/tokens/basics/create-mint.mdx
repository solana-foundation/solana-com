...
description:
  Erstellen Sie einen Token-Mint und konfigurieren Sie die Dezimalstellen, die
  Mint-Authority und die Freeze-Authority des Mints.
...
Ein Mint-Konto definiert und identifiziert einen Token auf Solana eindeutig und
speichert den gemeinsamen Zustand, der für alle Token-Konten dieses Mints gilt.
...
Jeder Token hat ein Mint-Konto, und die Mint-Adresse ist die eindeutige Kennung
des Tokens über Wallets, Anwendungen und Explorer hinweg.
...
USDC eindeutig im gesamten Solana-Ökosystem. Sie können diesen Mint im
...
1. Die _rs`CreateAccount`_- Anweisungen des System Program erstellt ein neues
   mietbefreites Konto und weist das Token Program als Programmbesitzer des
   neuen Kontos zu.
2. Die _rs`InitializeMint`_- oder _rs`InitializeMint2`_- Anweisungen des Token
   Program initialisiert das neue Konto als Mint.

Fügen Sie die _rs`CreateAccount`_- Anweisungen und die Mint-Initialisierungs-
Anweisungen in dieselbe Transaktion ein.

Während der Mint-Initialisierung prüft das Token Program, ob das Mint-Konto noch
nicht initialisiert ist und mietbefreit ist. Das Token Program schreibt dann die
Mint-Authority, Freeze-Authority, Dezimalstellen und das
_rs`is_initialized`_-Flag in die Mint-Kontodaten.

### Quellreferenz

| Element                        | Beschreibung                                                                                            | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ------------------------------ | ------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                     | Der Basis-Mint-Zustand, der in jedem Mint-Konto gespeichert ist.                                        | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`InitializeMint`_           | Eine Mint-Initialisierungs- Anweisungen, die das Rent-Sysvar-Konto in ihrer Kontoliste erwartet.        | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L24-L44)   | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L50-L74)   |
| _rs`InitializeMint2`_          | Eine Mint-Initialisierungs- Anweisungen, die das Rent-Sysvar-Konto nicht in ihrer Kontoliste erfordert. | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L401-L414) | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L471-L486) |
...
Die `Kit`-Beispiele unten zeigen den empfohlenen Ansatz unter Verwendung von
...
Ein Mint-Account speichert nur grundlegende Token-Informationen (Umlauf,
Dezimalstellen, Autoritäten). Um menschenlesbare Metadaten wie Name, Symbol und
Bild zu Ihrem Token hinzuzufügen, haben Sie zwei Optionen:

<Cards>
  <Card title="Metaplex Token Metadata" href="/docs/tokens/metaplex">
   Fügen Sie Metadaten mithilfe des Metaplex Token Metadata Programs hinzu. Funktioniert sowohl mit dem ursprünglichen Token Program als auch mit Token-2022.
  </Card>
  ...
  <Card title="Token Extensions Metadata" href="/docs/tokens/extensions/metadata">
   ...
   Die Metadaten-Erweiterung ist nur für Mint-Accounts verfügbar, die mit Token-2022 erstellt wurden.
   Um Metadaten zu Mint-Accounts hinzuzufügen, die mit dem ursprünglichen Token Program erstellt wurden, verwenden Sie das Metaplex Token Metadata Program.
  </Card>
</Cards>
