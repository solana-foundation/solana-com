...
title: Sync Native
description:
  Aktualisieren Sie den Betrag eines Wrapped-SOL-Token-Kontos nach der
  Übertragung von SOL auf das Konto.
...
Wrapped SOL (WSOL) ist SOL, das in einem Token-Konto für den nativen Mint des
Token Program gehalten wird. Der native Mint ist die Mint-Adresse, die das Token
Program für Token-Konten verwendet, die SOL wrappen. Der native Mint ermöglicht
es, SOL über die Anweisungen des Token Program zu verwenden.

| Programm                 | Native Mint-Adresse                            |
| ------------------------ | ---------------------------------------------- |
...
Ein Wrapped-SOL-Token-Konto speichert SOL im `lamports`-Feld des Kontos,
verfolgt jedoch das im Wrapped-SOL-Token-Konto gehaltene SOL als Token-Saldo
über das _rs`amount`_-Feld des Token-Kontos. Wenn Sie SOL auf ein
Wrapped-SOL-Token-Konto übertragen, erhöht sich der Lamport-Saldo sofort, aber
das Token-_rs`amount`_-Feld ändert sich nicht, bis Sie die _rs`SyncNative`_-
Anweisung des Token Program aufrufen.

## So synchronisieren Sie natives SOL

Verwenden Sie die _rs`SyncNative`_- Anweisung des Token Program, nachdem Sie SOL
auf ein Wrapped-SOL-Token-Konto übertragen haben. Die _rs`SyncNative`_-
Anweisung aktualisiert das Token-Konto-_rs`amount`_-Feld, um den im Konto
gehaltenen Lamports oberhalb seiner mietbefreiten Reserve zu entsprechen.

### Quellreferenz

| Element                   | Beschreibung                                                                                                                                                                                                                    | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_             | Der Token-Konto-Status verwendet _rs`is_native`_, um das Konto als WSOL zu markieren und speichert die native Reserve, den mietbefreiten Mindest-Lamport-Saldo, den das Konto behalten muss, sowie den verfolgten Token-Betrag. | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`SyncNative`_          | Eine Anweisung, die das _rs`amount`_-Feld eines Wrapped-SOL-Token-Kontos aktualisiert, um den im Konto gehaltenen Lamports oberhalb seiner mietbefreiten Reserve zu entsprechen.                                                | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| _rs`process_sync_native`_ | Gemeinsame Prozessorlogik zur Synchronisierung von Wrapped-SOL-Salden.                                                                                                                                                          | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |
