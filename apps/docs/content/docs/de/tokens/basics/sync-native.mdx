...
title: Sync Native
description:
  Aktualisieren Sie den Betrag eines Wrapped-SOL-Token-Kontos, nachdem SOL auf
  das Konto übertragen wurde.
...
Wrapped SOL (WSOL) ist SOL, das in einem Token-Konto für den Native Mint des
...
das Token-_rs`amount`_-Feld ändert sich erst, wenn Sie die _rs`SyncNative`_-
Anweisungen des Token Program aufrufen.

## So synchronisieren Sie natives SOL

Verwenden Sie die _rs`SyncNative`_- Anweisungen des Token Program, nachdem Sie
SOL auf ein Wrapped-SOL-Token-Konto übertragen haben. Die _rs`SyncNative`_-
Anweisungen aktualisieren das _rs`amount`_-Feld des Token-Kontos, damit es mit
den im Konto gehaltenen Lamports übereinstimmt, die über dessen
Mietfreiheitsreserve hinausgehen.

### Quellenreferenz

| Element                   | Beschreibung                                                                                                                                                                                                                  | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_             | Der Token-Konto-Status verwendet _rs`is_native`_, um das Konto als WSOL zu markieren, und speichert die Native Reserve, den mietfreien Mindest-Lamport-Saldo, den das Konto behalten muss, sowie den verfolgten Token-Betrag. | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`SyncNative`_          | Eine Anweisung, die das _rs`amount`_-Feld eines Wrapped-SOL-Token-Kontos aktualisiert, damit es mit den im Konto gehaltenen Lamports übereinstimmt, die über dessen Mietfreiheitsreserve hinausgehen.                         | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
