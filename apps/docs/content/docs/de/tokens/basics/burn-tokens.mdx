...
description:
  Verbrennen Sie Token von einem Token-Konto und reduzieren Sie das
...
## Was bewirkt das Verbrennen?

Das Verbrennen von Token verringert dauerhaft den Kontostand eines Token-Kontos
und reduziert das Gesamtangebot des Mints um denselben Betrag.

## So verbrennen Sie Token

Das Verbrennen von Token verwendet die _rs`Burn`_- oder _rs`BurnChecked`_-
Anweisungen des Token Program.

Die folgenden Beispiele verwenden _rs`BurnChecked`_, wodurch der Aufrufer die
Dezimalstellen des Mints angeben muss, damit die Anweisung die erwartete
Token-Präzision vor dem Verbrennen überprüfen kann.

Der [native Mint](/docs/tokens/basics/sync-native) unterstützt kein Verbrennen.
Verwenden Sie stattdessen _rs`CloseAccount`_.

Der Besitzer des Token-Kontos oder ein genehmigter Delegat signiert die
Transaktion. Im Token Extensions Program kann ein permanenter Delegat des Mints
das Verbrennen ebenfalls autorisieren, wenn für den Mint die Erweiterung für
permanente Delegaten aktiviert ist.
...
| _rs`Account`_      | Der Token-Konto-Status speichert den Token-Saldo, der durch ein Verbrennen reduziert wird.                                                                                                              | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Eine Verbrennungs-Anweisung, die den Saldo eines Token-Kontos und das Mint-Angebot verringert, ohne dass der Aufrufer die Dezimalstellen des Mints angeben muss.                                        | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Eine Verbrennungs-Anweisung, die erfordert, dass der Aufrufer die Dezimalstellen des Mints angibt und diesen Wert überprüft, bevor der Saldo eines Token-Kontos und das Mint-Angebot verringert werden. | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | Gemeinsame Prozessorlogik für Token-Verbrennungen.                                                                                                                                                      | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |
