...
description:
  Verschieben Sie Token zwischen Token-Konten für dieselbe Mint mit Transfer-
...
## Was bewirkt das Übertragen von Token?

Das Übertragen von Token verschiebt Token von einem token account zu einem
anderen token account für dieselbe Mint.

Übertragungen ändern nicht die Mint-Menge. Übertragungen aktualisieren nur die
Guthaben zwischen Token-Konten.

## Wie man Token überträgt

Das Übertragen von Token verwendet die _rs`Transfer`_- oder
_rs`TransferChecked`_-Anweisung des Token Program.

Die nachstehenden Beispiele verwenden _rs`TransferChecked`_, was erfordert, dass
der Aufrufer die Mint und Dezimalstellen bereitstellt, damit die Anweisung die
erwartete Mint und Token-Präzision verifizieren kann, bevor Token zwischen
Konten verschoben werden.

Im Token Extensions Program ist _rs`Transfer`_ zugunsten von
_rs`TransferChecked`_ oder _rs`TransferCheckedWithFee`_ veraltet.

Der Eigentümer des Quell-Kontos oder ein genehmigter Delegat signiert die
Übertragung. Im Token Extensions Program kann ein permanenter Delegat der Mint
die Übertragung ebenfalls autorisieren, wenn die Mint die permanente
Delegat-Erweiterung aktiviert hat.

### Quellreferenz

| Element                | Beschreibung                                                                                                                                                                                                                 | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_          | Der Token-Kontostatus speichert die durch eine Übertragung aktualisierten Guthaben.                                                                                                                                          | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | Eine Übertragungsanweisung, die Token zwischen Token-Konten verschiebt, ohne dass der Aufrufer die Mint oder Dezimalstellen bereitstellen muss. Im Token Extensions Program verwenden Sie stattdessen _rs`TransferChecked`_. | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Eine Übertragungsanweisung, die erfordert, dass der Aufrufer die Mint und Dezimalstellen bereitstellt und diese Werte prüft, bevor Token zwischen Token-Konten derselben Mint verschoben werden.                             | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | Gemeinsame Prozessorlogik für Token-Übertragungen.                                                                                                                                                                           | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |
