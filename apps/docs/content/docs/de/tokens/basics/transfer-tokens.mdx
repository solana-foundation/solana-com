...
description:
  Verschieben Sie Token zwischen Token-Konten für dieselbe Mint mit Token
  Program Transfer- Anweisungen.
...
Das Übertragen von Token verschiebt Token von einem Token-Konto zu einem anderen
Token-Konto für dieselbe Mint.

Überweisungen ändern nicht den Mint-Supply. Überweisungen aktualisieren nur
Salden zwischen Token-Konten.

## So übertragen Sie Token

Das Übertragen von Token verwendet die _rs`Transfer`_- oder
_rs`TransferChecked`_-Anweisung des Token Programs.

Die folgenden Beispiele verwenden _rs`TransferChecked`_, was erfordert, dass der
Aufrufer die Mint und Dezimalstellen bereitstellt, damit die Anweisung die
erwartete Mint und Token-Präzision überprüfen kann, bevor Token zwischen Konten
verschoben werden.
