...
description:
  Entfernen Sie den aktuellen Delegierten eines Token-Kontos und setzen Sie
  dessen verbleibende delegierte Menge zurück.
...
Token-Kontos und setzt die delegierte Menge auf null zurück.

Nach dem Widerruf hat der vorherige Delegierte keine Berechtigung mehr, Token im
Namen des Token-Kontos zu übertragen oder zu verbrennen.

## So widerrufen Sie einen Delegierten

Das Widerrufen eines Delegierten verwendet die `Revoke`-Anweisung des
Token-Programms.

Die `Revoke`-Anweisung löscht die Delegiertenfelder im Quell-Token-Konto. Im
Token-Programm signiert der Inhaber des Token-Kontos den Widerruf. Im Token
Extensions Program kann auch der aktuelle Delegierte die Delegierung widerrufen.

### Quellenreferenz

| Element          | Beschreibung                                                                  | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ---------------- | ----------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `Account`        | Der Token-Konto-Status speichert den Delegierten und die delegierte Menge.    | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| `Revoke`         | Eine Anweisung, die den aktuellen Delegierten von einem Token-Konto entfernt. | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L128-L140) | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L170-L182) |
| `process_revoke` | Gemeinsame Prozessorlogik für den Widerruf von Delegierten.                   | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L395-L420)     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L643-L674)     |
