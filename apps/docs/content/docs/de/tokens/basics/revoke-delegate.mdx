...
description:
  Entfernen Sie den aktuellen Delegierten eines Token-Kontos und löschen Sie die
  verbleibende delegierte Zuteilung.
...
## Was bewirkt das Widerrufen eines Delegierten?

Das Widerrufen eines Delegierten löscht den aktuellen Delegierten des
Token-Kontos und setzt den delegierten Betrag auf null zurück.

Nach dem Widerruf hat der bisherige Delegierte keine Befugnis mehr, Token im
Namen des Token-Kontos zu übertragen oder zu verbrennen.

## So widerrufen Sie einen Delegierten

Das Widerrufen eines Delegierten verwendet die Anweisung _rs`Revoke`_ des Token
Program.

Die Anweisung _rs`Revoke`_ löscht die Delegiertenfelder im Quell-Token-Konto. Im
Token Program signiert der Kontoinhaber den Widerruf. Im Token Extensions
Program kann auch der aktuelle Delegierte die Delegierung widerrufen.

### Quellreferenz

| Element              | Beschreibung                                                                  | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| -------------------- | ----------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_        | Der Token-Kontostatus speichert den Delegierten und den delegierten Betrag.   | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Revoke`_         | Eine Anweisung, die den aktuellen Delegierten von einem Token-Konto entfernt. | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L128-L140) | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L170-L182) |
| _rs`process_revoke`_ | Gemeinsame Prozessorlogik für den Widerruf von Delegierten.                   | [Quelle](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L395-L420)     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L643-L674)     |
