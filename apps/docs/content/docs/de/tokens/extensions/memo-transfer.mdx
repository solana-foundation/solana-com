...
title: Memo-Transfer
description:
  Fordern Sie an, dass eingehende Transfers ein Memo enthalten, indem Sie die
  MemoTransfer-Kontoerweiterung auf einem Token Extensions Program-Token-Konto
  aktivieren.
...
Die _rs`MemoTransfer`_-Kontoerweiterung des Token Extensions Program erfordert
...
Sobald aktiviert, schlagen Transfers ohne Memo mit _rs`TokenError::NoMemo`_
fehl, bis der Token-Kontoinhaber die Anforderung deaktiviert.

## So erstellen Sie ein Token-Konto mit aktiviertem MemoTransfer

So erstellen Sie ein Token-Konto mit aktiviertem _rs`MemoTransfer`_:

1. Erstellen und initialisieren Sie einen Mint.
2. Berechnen Sie die Token-Kontogröße und die benötigte Miete für ein
   Token-Konto mit _rs`MemoTransfer`_.
3. Erstellen Sie das Token-Konto und aktivieren Sie _rs`MemoTransfer`_ auf
   diesem Konto.
4. _rs`Transfer`_- und _rs`TransferChecked`_- Anweisungen auf dieses Token-Konto
   müssen eine Memo-Anweisung unmittelbar vor der Transfer-Anweisung enthalten.
...
## !!steps Token-Kontogröße berechnen

Berechnen Sie die Token-Kontogröße für das Basis-Token-Konto zuzüglich der
_rs`MemoTransfer`_-Erweiterung. Dies ist die Größe, die in _rs`CreateAccount`_
verwendet wird.
...
_rs`MemoTransfer`_-Erweiterung benötigt wird.
...
## !!steps Erstellen und Initialisieren des Token-Kontos
...
## !!steps MemoTransfer aktivieren

Aktivieren Sie _rs`MemoTransfer`_ auf dem Token-Konto.
...
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`MemoTransfer`_                              | Kontenerweiterung, die speichert, ob eingehende Überweisungen ein Memo erfordern.                                                     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/mod.rs#L17-L22)         |
| _rs`RequiredMemoTransfersInstruction::Enable`_  | Anweisungen, die Überweisungen mit Memo-Pflicht auf einem Token-Konto aktivieren.                                                     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L50-L71) |
| _rs`RequiredMemoTransfersInstruction::Disable`_ | Anweisungen, die Überweisungen mit Memo-Pflicht auf einem Token-Konto deaktivieren.                                                   | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L74-L95) |
| _rs`process_toggle_required_memo_transfers`_    | Prozessorlogik, die die _rs`MemoTransfer`_-Erweiterung initialisiert oder neu schreibt, nachdem die Eigentümerschaft überprüft wurde. | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/memo_transfer/processor.rs#L20-L48)     |
