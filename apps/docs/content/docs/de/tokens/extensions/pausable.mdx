---
title: Pausierbare Prägung
description:
  Erstellen Sie eine Token-2022-Mint mit einer Pausenautorität, blockieren Sie
  Übertragungen, während die Mint pausiert ist, und setzen Sie die Aktivität
  später fort.
...
Die _rs`PausableConfig`_ Mint-Erweiterung des Token Extensions Program gibt
einer Pausenautorität die Möglichkeit, die Token-Aktivität für die gesamte Mint
...
1. Berechnen Sie die Größe des Mint-Kontos und die benötigte Miete für die Mint
   und die _rs`PausableConfig`_ Erweiterung.
2. Erstellen Sie das Mint-Konto mit _rs`CreateAccount`_, initialisieren Sie
   _rs`PausableConfig`_ und initialisieren Sie die Mint mit
   _rs`InitializeMint`_.
3. Minten Sie Token und erstellen Sie Token-Konten für die Mint.
4. Verwenden Sie _rs`Pause`_, um Token-Aktivität für die Mint zu blockieren.
5. Verwenden Sie _rs`Resume`_, um Token-Aktivität wieder zu erlauben.

<ScrollyCoding>

## !!steps Kontogröße berechnen

Berechnen Sie die Größe des Mint-Kontos für die Basis-Mint plus die
_rs`PausableConfig`_ Erweiterung. Dies ist die Größe, die in _rs`CreateAccount`_
verwendet wird.
...
Berechnen Sie die Miete anhand der benötigten Größe für die Mint plus die
_rs`PausableConfig`_ Erweiterung.
...
## !!steps PausableConfig initialisieren

Initialisieren Sie die _rs`PausableConfig`_-Erweiterung für das Mint.
...
## !!steps Mint pausieren

Pausieren Sie das Mint mit _rs`Pause`_.
...
## !!steps Mint fortsetzen

Setzen Sie das Mint mit _rs`Resume`_ fort.
...
<Callout type="info" title=" Anweisungenreihenfolge">
  _rs`PausableInstruction::Initialize`_ muss vor _rs`InitializeMint`_ kommen.
  _rs`CreateAccount`_, _rs`PausableInstruction::Initialize`_ und
...
| ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PausableConfig`_                  | Mint-Erweiterung, die die Pausierungsberechtigung speichert und angibt, ob das Mint derzeit pausiert ist.                              | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/mod.rs#L17-L22)         |
| _rs`PausableInstruction::Initialize`_ | Anweisungen, die die pausierbare Konfiguration vor _rs`InitializeMint`_ initialisiert.                                                 | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L20-L32) |
| _rs`PausableInstruction::Pause`_      | Anweisungen, die das Mint als pausiert kennzeichnet.                                                                                   | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L33-L44) |
| _rs`PausableInstruction::Resume`_     | Anweisungen, die das Mint als nicht pausiert kennzeichnet.                                                                             | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L45-L56) |
| _rs`process_initialize`_              | Prozessorlogik, die _rs`PausableConfig`_ auf einem nicht initialisierten Mint initialisiert und die Pausierungsberechtigung speichert. | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L22-L35)     |
| _rs`process_toggle_pause`_            | Prozessorlogik, die die Pausierungsberechtigung überprüft, bevor das Pausierungs-Flag des Mints umgeschaltet wird.                     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L39-L64)     |
