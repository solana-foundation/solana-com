---
title: Metadaten-Zeiger & Token-Metadaten
description:
  Erstellen Sie einen Mint, der Metadaten direkt im Mint-Konto mit den MetadataPointer- und TokenMetadata-Erweiterungen speichert, und aktualisieren, entfernen und sperren Sie diese Metadaten über die Token-Metadaten-Schnittstelle.
...
## Was sind Metadaten-Zeiger- und Token-Metadaten-Erweiterungen?

Die _rs`MetadataPointer`_-Mint-Erweiterung des Token Extensions Program
speichert zwei Felder auf einem Mint:

- Eine Autorisierung, die den Zeiger aktualisieren kann
- Die Adresse des Kontos, das die Metadaten des Tokens speichert

Dieser Zeiger kann auf jedes Konto verweisen, das einem Programm gehört, welches
die
[Token-Metadaten-Schnittstelle](https://github.com/solana-program/token-metadata/tree/main/interface)
implementiert.

Das Token Extensions Program implementiert dieselbe Schnittstelle auch direkt
über die _rs`TokenMetadata`_-Mint-Erweiterung. Mit _rs`TokenMetadata`_ speichert
der Mint den _rs`name`_, das _rs`symbol`_, den _rs`uri`_, das
_rs`update_authority`_ und benutzerdefinierte Metadaten des Tokens direkt im
Mint-Konto.

<Callout type="info" title="Off-Chain-Metadaten-URI">
  Das Feld _rs`uri`_ verweist auf Off-Chain-JSON-Metadaten. Siehe
  [Off-Chain-Metadaten-Format](/docs/tokens/metaplex#off-chain-metadata-format).
</Callout>

Die beiden Erweiterungen lösen unterschiedliche Probleme:

- _rs`MetadataPointer`_ gibt das Konto an, das Metadaten speichert.
- _rs`TokenMetadata`_ speichert die Metadaten direkt im Mint-Konto.

<Callout type="info" title="Erweiterung mit variabler Länge">
  _rs`TokenMetadata`_ ist eine TLV-Erweiterung mit variabler Länge.
  _rs`InitializeTokenMetadata`_, _rs`UpdateField`_ und _rs`RemoveKey`_ können
  die Größe der Mint-Kontodaten ändern, übertragen jedoch keine zusätzlichen
  Lamports. Das Mint-Konto benötigt genügend Lamports, um für die gespeicherten
  Metadaten mietbefreit zu bleiben. Wenn die Metadaten später wachsen, müssen
  zusätzliche Lamports auf das Mint-Konto übertragen werden, bevor die
  Anweisungen ausgeführt werden, die dessen Größe ändern.
</Callout>

## So speichern Sie Token-Metadaten im Mint-Konto

Um Metadaten im Mint-Konto zu speichern:

1. Berechnen Sie die Größe des Mint-Kontos und die benötigte Miete für das Mint,
   die Erweiterungen und die Metadaten.
2. Erstellen Sie das Mint-Konto mit _rs`CreateAccount`_, initialisieren Sie
   _rs`MetadataPointer`_ und initialisieren Sie das Mint mit
   _rs`InitializeMint`_.
3. Initialisieren Sie _rs`TokenMetadata`_ auf dem Mint-Konto und verwenden Sie
   dann _rs`UpdateField`_, um Metadaten hinzuzufügen oder zu aktualisieren.
4. Verwenden Sie _rs`RemoveKey`_, _rs`UpdateAuthority`_ und _rs`Emit`_, um
   benutzerdefinierte Metadaten zu entfernen, die Upgrade-Authority zu ändern
   oder zu löschen oder die aktuellen Metadaten in den Transaktionsrückgabedaten
   zurückzugeben.
...
Berechnen Sie die Größe des Mint-Kontos für das Basis-Mint plus die
_rs`MetadataPointer`_- Erweiterung. Dies ist die Größe, die in
...
Berechnen Sie die Miete anhand der maximalen Größe, die nach der Speicherung von
_rs`TokenMetadata`_ auf dem Mint benötigt wird.
...
## !!steps MetadataPointer initialisieren

Initialisieren Sie _rs`MetadataPointer`_ und setzen Sie dessen Metadatenadresse
auf die Mint- Adresse.
...
## !!steps Metadaten initialisieren und aktualisieren

Initialisieren Sie _rs`TokenMetadata`_ auf dem Mint. Verwenden Sie
_rs`UpdateField`_, um benutzerdefinierte Metadaten hinzuzufügen; falls das Feld
nicht existiert, fügt _rs`UpdateField`_ es hinzu.
...
## !!steps Metadaten aktualisieren, entfernen oder ausgeben

Nachdem die Mint initialisiert wurde, verwenden Sie _rs`UpdateField`_, um
Metadaten zu aktualisieren, _rs`UpdateMetadataPointer`_, um den Metadaten-Zeiger
zu aktualisieren, _rs`RemoveKey`_, um benutzerdefinierte Metadaten zu entfernen,
_rs`UpdateAuthority`_, um die Update-Authority zu löschen, und _rs`Emit`_, um
die aktuellen Metadaten zurückzugeben.
...
<Callout type="info" title="Anweisungsreihenfolge">
  _rs`MetadataPointerInstruction::Initialize`_ muss vor _rs`InitializeMint`_
  kommen. _rs`CreateAccount`_, _rs`MetadataPointerInstruction::Initialize`_ und
...
| -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`MetadataPointer`_                        | Mint-Erweiterung, die die Metadaten-Zeiger-Authority und die Metadaten-Kontoadresse speichert.                       | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/metadata_pointer/mod.rs#L12-L22)         |
| _rs`MetadataPointerInstruction::Initialize`_ | Initialisiert die Metadaten-Zeiger-Erweiterung vor _rs`InitializeMint`_.                                           | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/metadata_pointer/instruction.rs#L22-L38) |
| _rs`MetadataPointerInstruction::Update`_     | Aktualisiert die Metadatenadresse, die von der Metadaten-Zeiger-Erweiterung der Mint gespeichert wird.                                     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/metadata_pointer/instruction.rs#L39-L55) |
| _rs`process_initialize`_ (`MetadataPointer`) | Metadaten-Zeiger-Prozessorlogik, die während der Initialisierung mindestens eine Authority oder eine Metadatenadresse erfordert. | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/metadata_pointer/processor.rs#L25-L48)     |
| _rs`process_update`_ (`MetadataPointer`)     | Validiert die Metadaten-Zeiger-Authority und schreibt dann die gespeicherte Metadatenadresse der Mint neu.                       | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/metadata_pointer/processor.rs#L51-L77)     |

#### Token-Metadaten

| Element                                            | Beschreibung                                                                                                                         | Quelle                                                                                                                                    |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`TokenMetadata`_                             | Schnittstellenstatus für Token-Metadaten variabler Länge, der in einem TLV-Eintrag gespeichert ist.                                                               | [Quelle](https://github.com/solana-program/token-metadata/blob/interface%40v1.0.0/interface/src/state.rs#L18-L38)                         |
| _rs`Field`_                                     | Feld-Enum, das von _rs`UpdateField`_ verwendet wird, um _rs`name`_, _rs`symbol`_, _rs`uri`_ oder einen benutzerdefinierten Schlüssel anzusprechen.                                | [Quelle](https://github.com/solana-program/token-metadata/blob/interface%40v1.0.0/interface/src/state.rs#L109-L122)                       |
| _rs`TokenMetadataInstruction::Initialize`_      | Initialisiert die Basis-Felder _rs`name`_, _rs`symbol`_ und _rs`uri`_ für ein Token-Metadaten-Konto.                                   | [Quelle](https://github.com/solana-program/token-metadata/blob/interface%40v1.0.0/interface/src/instruction.rs#L81-L95)                   |
| _rs`TokenMetadataInstruction::UpdateField`_     | Fügt ein Basis-Feld oder ein benutzerdefiniertes Metadatenfeld zu den Token-Metadaten hinzu oder aktualisiert es.                                                        | [Quelle](https://github.com/solana-program/token-metadata/blob/interface%40v1.0.0/interface/src/instruction.rs#L97-L117)                  |
| _rs`TokenMetadataInstruction::RemoveKey`_       | Entfernt einen benutzerdefinierten Metadatenschlüssel. Basis-Felder können mit dieser Anweisung nicht entfernt werden.                                                 | [Quelle](https://github.com/solana-program/token-metadata/blob/interface%40v1.0.0/interface/src/instruction.rs#L119-L134)                 |
| _rs`TokenMetadataInstruction::UpdateAuthority`_ | Rotiert die Metadaten-Update-Authority oder löscht sie vollständig, um Metadaten unveränderlich zu machen.                                            | [Quelle](https://github.com/solana-program/token-metadata/blob/interface%40v1.0.0/interface/src/instruction.rs#L136-L144)                 |
| _rs`TokenMetadataInstruction::Emit`_            | Gibt die serialisierten Metadaten über Transaktionsrückgabedaten zurück, optional für einen Byte-Bereich.                                       | [Quelle](https://github.com/solana-program/token-metadata/blob/interface%40v1.0.0/interface/src/instruction.rs#L146-L159)                 |
| _rs`process_initialize`_ (`TokenMetadata`)      | Token-Metadaten-Prozessorlogik, die erfordert, dass das Metadaten-Konto die Mint selbst ist und die Mint _rs`MetadataPointer`_ hat. | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_metadata/processor.rs#L43-L101)  |
| _rs`process_update_field`_                      | Weist den TLV-Eintrag mit variabler Länge _rs`TokenMetadata`_ nach einer Feldaktualisierung neu zu und schreibt ihn neu.                                    | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_metadata/processor.rs#L104-L132) |
| _rs`process_remove_key`_                        | Validiert die Update-Authority, entfernt einen benutzerdefinierten Metadatenschlüssel und schreibt den TLV-Eintrag neu.                                          | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_metadata/processor.rs#L135-L159) |
| _rs`process_update_authority`_                  | Validiert die aktuelle Update-Authority und rotiert oder löscht dann die Metadaten-Authority vor Ort.                                     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_metadata/processor.rs#L162-L187) |
| _rs`process_emit`_                              | Liest die serialisierten _rs`TokenMetadata`_ aus der Mint und schreibt den angeforderten Ausschnitt in die Rückgabedaten.                               | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_metadata/processor.rs#L190-L203) |

### Typescript

Das `Kit`-Beispiel unten verwendet die generierten Anweisungen direkt. Legacy-Beispiele mit `@solana/web3.js`, `@solana/spl-token` und
`@solana/spl-token-metadata` sind als Referenz enthalten. Die Legacy-Beispiele begrenzen
`maxSupportedTransactionVersion` auf `0`, da web3.js v1 keine
[v1-Transaktionen](/docs/core/transactions/versioned-transactions#v1-message-format) deserialisieren kann.
