...
description:
  Erstellen Sie einen Mint, der Metadaten direkt im Mint-Konto mit den
  MetadataPointer- und TokenMetadata-Erweiterungen speichert, und aktualisieren,
  entfernen und sperren Sie diese Metadaten über die
  Token-Metadaten-Schnittstelle.
...
Das nachfolgende `Kit`-Beispiel verwendet die generierten Anweisungen direkt.
Legacy-Beispiele mit `@solana/web3.js`, `@solana/spl-token` und
`@solana/spl-token-metadata` sind als Referenz enthalten.
