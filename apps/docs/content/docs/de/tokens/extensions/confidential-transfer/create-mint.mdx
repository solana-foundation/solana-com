...
description:
  Erfahren Sie, wie Sie einen Token-Mint mit aktivierten Confidential Balances erstellen.
---

## So erstellen Sie einen Mint mit Confidential Balances

Confidential Balances ermöglicht private Token-Transfers, indem dem mint account zusätzlicher Zustand hinzugefügt wird. Dieser Abschnitt erklärt, wie ein Token-Mint mit der erforderlichen aktivierten Erweiterung erstellt wird.

Das folgende Diagramm zeigt die Schritte zur Erstellung eines Mints mit der Confidential Balances-Erweiterung:

```mermaid title="Create Mint with Confidential Balances"
sequenceDiagram
    participant Payer as Wallet
    participant Token22 as Token Extensions Program
    participant Mint as Mint Account

    Payer->>Token22: create_mint <br>(with Confidential Balances)
    activate Token22
    Token22-->>Mint: Initialize mint with extension data
    deactivate Token22
```

### Confidential Balances Mint-Zustand
...
- **authority**: Das Konten, das die Berechtigung hat, Confidential Balances-Einstellungen für den Mint zu ändern und neue vertrauliche Konten zu genehmigen, wenn die automatische Genehmigung deaktiviert ist.

- **auto_approve_new_accounts**: Wenn auf „true“ gesetzt, können Benutzer token accounts mit standardmäßig aktivierten Confidential Balances erstellen. Wenn „false“, muss die Autorität jedes neue token account genehmigen, bevor es für vertrauliche Transfers verwendet werden kann.
...
Das Erstellen eines Mints mit aktivierten Confidential Balances erfordert drei Anweisungen in einer einzigen Transaktion:
...
2. **Confidential Balances initialisieren**: Rufen Sie die Anweisungen [ConfidentialTransferInstruction::InitializeMint](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L48) des Token Extension Program auf, um den _rs`ConfidentialTransferMint`_-Zustand für den Mint zu konfigurieren.
...
Der folgende Code zeigt, wie ein Mint mit aktivierten Confidential Balances erstellt wird.

Confidential Balances ist abhängig vom ZK ElGamal Proof-Programm, das auf Mainnet und Devnet aktiviert ist. Ein standardmäßiger `solana-test-validator` aktiviert es nicht, aber ein Mainnet-forking lokaler Validatoren wie [Surfpool](https://surfpool.run) schon.
Führen Sie das Beispiel gegen eines davon aus (der Code verwendet Devnet) mit einem finanzierten Zahler und ersetzen Sie die Platzhalter-Mint- und Kontenadressen durch Ihre eigenen.
