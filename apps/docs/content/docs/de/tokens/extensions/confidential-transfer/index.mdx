...
## Was sind Confidential Transfers?

Confidential Transfers ermöglichen es Ihnen, Token zwischen Token-Konten zu
übertragen, ohne den Übertragungsbetrag offenzulegen. Dies ist nützlich für
datenschutzwahrende Transaktionen. Nur die Übertragungsbeträge und
Token-Guthaben sind privat. Die Token-Kontenadressen bleiben öffentlich.
...
zum Token Extension-Programm hinzu, die es Ihnen ermöglichen, Token zwischen
Konten zu übertragen, ohne den Übertragungsbetrag offenzulegen.
...
Der grundlegende Ablauf vertraulicher Token-Übertragungen ist wie folgt:

1. Erstellen Sie ein Mint-Konto mit der Confidential Transfer-Erweiterung.
2. Erstellen Sie Token-Konten mit Confidential Transfer-Erweiterung für Sender
   und Empfänger.
3. Prägen Sie Token zum Sender-Konto.
4. **Einzahlen** des öffentlichen Guthabens des Senders in das **vertrauliche
   ausstehende Guthaben**.
5. **Anwenden** des ausstehenden Guthabens des Senders auf das **vertrauliche
   verfügbare Guthaben**.
6. Vertrauliche **Übertragung** von Token vom Sender-Token-Konto zum Empfänger-
   Token-Konto.
7. **Anwenden** des ausstehenden Guthabens des Empfängers auf das **vertrauliche
   verfügbare Guthaben**.
8. **Abheben** des vertraulichen verfügbaren Guthabens des Empfängers in das
...
<Cards>
  <Card
    title="Mint-Konto erstellen"
    href="/docs/tokens/extensions/confidential-transfer/create-mint"
  >
    So erstellen Sie ein Mint-Konto mit der Confidential Transfer-Erweiterung
  </Card>
  <Card
    title="Token-Konto erstellen"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    So konfigurieren Sie ein Token-Konto mit der Confidential
  ...
  >
    So übertragen Sie Token vertraulich zwischen Token-Konten
  </Card>
</Cards>

Das folgende Diagramm zeigt eine detaillierte Abfolge des grundlegenden Ablaufs
für vertrauliche Token-Überweisungen:
...
Token-Überweisungen entsprechend dem vorherigen Diagramm. Die Schritte sind zur
besseren Übersicht in Hilfsfunktionen unterteilt.

Um das Beispiel auszuführen, starten Sie einen lokalen Validator mit dem Token
Extensions Program, das vom Mainnet geklont wurde, mit folgendem Befehl. Sie
müssen die Solana CLI installiert haben, um den lokalen Validator zu verwenden.
...
<Callout type="info">
  Zum Zeitpunkt der Erstellung ist Confidential Transfers auf dem
  standardmäßigen lokalen Validator nicht aktiviert. Sie müssen das Mainnet
  Token Extensions Program klonen, um den Beispielcode auszuführen.
</Callout>
...
Die vollständige Liste der Confidential Transfer-Erweiterungs-
[Anweisungen](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
lautet wie folgt:

| Anweisung                           | Beschreibung                                                                                                                                                                                    |
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Richtet ein Mint-Konto für vertrauliche Überweisungen ein. Diese Anweisung muss in derselben Transaktion wie die _rs`TokenInstruction::InitializeMint`_ Anweisung enthalten sein.               |
| _rs`UpdateMint`_                    | Aktualisiert die Einstellungen für vertrauliche Überweisungen für ein Mint.                                                                                                                     |
| _rs`ConfigureAccount`_              | Richtet ein Token-Konto für vertrauliche Überweisungen ein.                                                                                                                                     |
| _rs`ApproveAccount`_                | Genehmigt ein Token-Konto für vertrauliche Überweisungen, wenn das Mint eine Genehmigung für neue Token-Konten erfordert.                                                                       |
| _rs`EmptyAccount`_                  | Leert die ausstehenden und verfügbaren vertraulichen Guthaben, um das Schließen eines Token-Kontos zu ermöglichen.                                                                              |
| _rs`Deposit`_                       | Wandelt öffentliches Token-Guthaben in ausstehendes vertrauliches Guthaben um.                                                                                                                  |
| _rs`Withdraw`_                      | Wandelt verfügbares vertrauliches Guthaben zurück in öffentliches Guthaben um.                                                                                                                  |
| _rs`Transfer`_                      | Überträgt Token vertraulich zwischen Token-Konten.                                                                                                                                              |
| _rs`ApplyPendingBalance`_           | Wandelt ausstehendes Guthaben nach Einzahlungen oder Überweisungen in verfügbares Guthaben um.                                                                                                  |
| _rs`EnableConfidentialCredits`_     | Ermöglicht einem Token-Konto den Empfang vertraulicher Token-Überweisungen.                                                                                                                     |
| _rs`DisableConfidentialCredits`_    | Blockiert eingehende vertrauliche Überweisungen, während öffentliche Überweisungen weiterhin möglich sind.                                                                                      |
| _rs`EnableNonConfidentialCredits`_  | Ermöglicht einem Token-Konto den Empfang öffentlicher Token-Überweisungen.                                                                                                                      |
| _rs`DisableNonConfidentialCredits`_ | Blockiert reguläre Überweisungen, damit das Konto nur vertrauliche Überweisungen empfangen kann.                                                                                                |
| _rs`TransferWithFee`_               | Überträgt Token vertraulich zwischen Token-Konten mit einer Gebühr.                                                                                                                             |
| _rs`ConfigureAccountWithRegistry`_  | Alternative Methode zur Konfiguration von Token-Konten für vertrauliche Überweisungen unter Verwendung eines _rs`ElGamalRegistry`_ Kontos anstelle eines _rs`VerifyPubkeyValidity`_ Nachweises. |
