...
Dieser Abschnitt erläutert die erste Stufe: das Einzahlen des öffentlichen
Token-Guthabens auf das vertrauliche ausstehende Guthaben.

Das folgende Diagramm zeigt die Schritte zum Einzahlen von Token vom
öffentlichen Guthaben auf das vertrauliche ausstehende Guthaben:
...
Um ein öffentliches Guthaben in ein vertrauliches ausstehendes Guthaben
...
Das `spl_token_client`-Crate stellt eine `confidential_transfer_deposit`-Methode
bereit, die eine Transaktion mit der `Deposit`-Anweisung erstellt und sendet,
wie im folgenden Beispiel gezeigt.

## Beispielcode

Das folgende Beispiel zeigt, wie das öffentliche Token-Guthaben auf das
vertrauliche ausstehende Guthaben eingezahlt wird.

Confidential Balances hängt vom ZK ElGamal Proof-Programm ab, das auf Mainnet und Devnet aktiviert ist. Ein Standard-`solana-test-validator` aktiviert es nicht, aber ein Mainnet-forking lokaler Validator wie [Surfpool](https://surfpool.run) schon.
