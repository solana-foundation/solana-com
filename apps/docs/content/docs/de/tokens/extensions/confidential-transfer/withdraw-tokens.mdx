...
<Callout type="warn">
  ...
  es einem Sicherheitsaudit unterzogen wird. Dies bedeutet, dass die Erweiterung
  für vertrauliche Überweisungen derzeit nicht verfügbar ist. Obwohl die
  Konzepte weiterhin gültig sind, werden die Code-Beispiele nicht ausgeführt.
</Callout>

## Wie man Token aus dem vertraulichen verfügbaren Guthaben abhebt

Um Token aus dem vertraulichen verfügbaren Guthaben auf das öffentliche Guthaben
abzuheben:
...
1. Erstellen Sie
   ...
   **Gleichheitsnachweis
   ...
   Überprüft, dass der verbleibende verfügbare Guthaben-Ciphertext nach der
   Abhebung mit seinem entsprechenden
   [Pedersen Commitment](https://en.wikipedia.org/wiki/Commitment_scheme)
   übereinstimmt, um sicherzustellen, dass das neue verfügbare Guthaben des
   Kontos korrekt als `remaining_balance = current_balance - withdraw_amount`
   berechnet wird.
   ...
   **Bereichsnachweis
   ...
   Überprüft, dass das verbleibende verfügbare Guthaben nach der Abhebung nicht
   negativ ist und sich innerhalb eines bestimmten Bereichs befindet.

2. Für jeden Nachweis:
   - Rufen Sie das ZK ElGamal Proof Program auf, um die Nachweisdaten zu
     verifizieren.
   - Speichern Sie die nachweisspezifischen Metadaten in einem "Context State"
     Konto, um sie in anderen Anweisungen zu verwenden.
...
3. Rufen Sie die
   ...
   Anweisung auf und übergeben Sie die beiden Nachweis-Konten.

4. Schließen Sie die Nachweis-Konten, um die SOL zurückzuerhalten, die für deren
   Erstellung verwendet wurden.

Das folgende Diagramm zeigt die Schritte, die beim Abheben von Token aus dem
vertraulichen verfügbaren Guthaben auf das öffentliche Guthaben erforderlich
sind:
...
- Das Zk ElGamal Proof Program aufrufen, um die Nachweise zu verifizieren und
  die "Context State" Konten zu initialisieren
- Die
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  Anweisung aufrufen und die beiden Nachweis-Konten übergeben.
- Die beiden Nachweis-Konten schließen, um die Miete zurückzuerhalten.
...
<Callout type="info">
  ...
  Standard-lokalen Validator nicht aktiviert. Sie müssen das Mainnet Token
