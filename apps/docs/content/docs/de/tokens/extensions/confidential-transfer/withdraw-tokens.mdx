---
title: Token abheben
description: Erfahren Sie, wie Sie Token aus dem vertraulichen Zustand abheben.
---

## So werden Token aus dem vertraulichen verfügbaren Guthaben abgehoben

So werden Token vom vertraulichen verfügbaren Guthaben auf das öffentliche
Guthaben übertragen:

1. Erstellen Sie
   [zwei Beweise](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/confidential-transfer/proof-generation/src/withdraw.rs#L22)
   ...
   **Gleichheitsnachweis
   ...
   Verifiziert, dass der verbleibende verfügbare Guthaben-Chiffretext nach der
   Auszahlung mit dem entsprechenden
   [Pedersen-Commitment](https://en.wikipedia.org/wiki/Commitment_scheme)
   übereinstimmt und sicherstellt, dass das neue verfügbare Guthaben der Konten
   korrekt als `remaining_balance = current_balance - withdraw_amount` berechnet
   wird.
   ...
   **Bereichsnachweis
   ...
   Verifiziert, dass das verbleibende verfügbare Guthaben nach der Auszahlung
   nicht negativ und innerhalb eines festgelegten Bereichs liegt.

2. Für jeden Beweis:
   - Rufen Sie das ZK-ElGamal-Beweisprogramm auf, um die Beweisdaten zu
     verifizieren.
   - Speichern Sie die beweisspezifischen Metadaten in einem
     "Kontext-Zustand"-Konten, um sie in anderen Anweisungen zu verwenden.
...
3. Rufen Sie die
   ...
   Anweisung auf und geben Sie die beiden Beweis-Konten an.

4. Schließen Sie die Beweis-Konten, um die zur Erstellung verwendeten SOL
   zurückzuerhalten.

Das folgende Diagramm zeigt die Schritte zum Abheben von Token aus dem
vertraulichen verfügbaren Guthaben auf das öffentliche Guthaben:
...
Um Token vom vertraulichen verfügbaren Guthaben auf das öffentliche Guthaben zu
übertragen, müssen Sie:
...
- Das ZK-ElGamal-Beweisprogramm aufrufen, um die Beweise zu verifizieren und die
  "Kontext-Zustand"-Konten zu initialisieren
- Die
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  Anweisung aufrufen und die beiden Beweis-Konten angeben.
...
Das folgende Beispiel hebt Token vom vertraulichen verfügbaren Guthaben
zurück auf das öffentliche Guthaben ab. Die Konten müssen bereits für
Vertrauliche Guthaben konfiguriert sein und ein verfügbares vertrauliches Guthaben aufweisen.

Vertrauliche Guthaben sind vom ZK ElGamal Proof-Programm abhängig, das auf
Mainnet und Devnet aktiviert ist. Ein Standard-`solana-test-validator` aktiviert es nicht, aber ein
Mainnet-forkender lokaler validator wie [Surfpool](https://surfpool.run) schon.
Führen Sie das Beispiel gegen eines davon aus (der Code verwendet Devnet) mit einem finanzierten Zahler,
und ersetzen Sie die Platzhalter durch Ihre Mint und token account.
