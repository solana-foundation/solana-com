...
description:
  Erfahren Sie, wie Sie ein token account mit aktivierten Confidential Balances erstellen.
---

## So erstellen Sie ein token account mit Confidential Balances

Confidential Balances ermöglicht private Token-Transfers durch das Hinzufügen von zusätzlichem Zustand zum
token account. In diesem Abschnitt wird erläutert, wie ein token account mit der
erforderlichen aktivierten Erweiterung erstellt wird.

Das folgende Diagramm zeigt die Schritte zur Erstellung eines token account mit
aktivierten Confidential Balances:
...
### Confidential Balances Token Account-Zustand
...
Das _rs`ConfidentialTransferAccount`_ enthält mehrere Felder zur Verwaltung
von Confidential Balances:

- **approved**: Der Genehmigungsstatus des Konten für Confidential Balances. Wenn die
  Konfiguration `auto_approve_new_accounts` des mint account auf `true` gesetzt ist, werden alle
  token accounts automatisch für Confidential Balances genehmigt.
...
Das Erstellen und Konfigurieren eines token account für Confidential Balances verwendet die
folgenden Anweisungen, die alle in eine einzige Transaktion passen:
...
4. **Confidential Balances konfigurieren**: Rufen Sie die
   [ConfidentialTransferInstruction::ConfigureAccount](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L201)-Anweisung des Token Extensions Program auf,
   referenzieren Sie das Proof-Kontext-Zustands-Konto über
   _rs`ProofLocation::ContextStateAccount`_, um den
   _rs`ConfidentialTransferAccount`_-Zustand zu initialisieren.

<Callout type="info">

**Nur der Eigentümer des token account kann Confidential Balances für ein token
account aktivieren**.

Die _rs`ConfigureAccount`_-Anweisung erfordert die clientseitige Generierung von
Verschlüsselungsschlüsseln und einen Beweis, der nur vom Eigentümer des token account
generiert werden kann.

Der pubkey-Gültigkeitsbeweis verifiziert, dass der ElGamal-öffentliche Schlüssel des Konten gültig ist. Er wird mit _rs`build_pubkey_validity_proof_data`_ generiert, on-chain vom ZK ElGamal Proof-Programm in ein Kontext-Zustands-Konto verifiziert und anschließend von _rs`ConfigureAccount`_ über
_rs`ProofLocation::ContextStateAccount`_ referenziert, sodass keine Proof-Bytes in der Token-Anweisung selbst übertragen werden. Implementierungsdetails finden Sie unter:
...
Der folgende Code erstellt ein associated token account und konfiguriert es für die Verwendung
von Confidential Balances mit einem vorhandenen kompatiblen Mint.

Confidential Balances ist vom ZK ElGamal Proof-Programm abhängig, das auf Mainnet und Devnet aktiviert ist. Ein Standard-`solana-test-validator` aktiviert es nicht, aber ein
mainnet-forkendes lokales Validatoren wie [Surfpool](https://surfpool.run) schon.
Führen Sie das Beispiel gegen eines davon aus (der Code verwendet Devnet) mit einem finanzierten Zahler
und ersetzen Sie den Mint-Platzhalter durch einen Mint, der gemäß
[Create a Mint](/docs/tokens/extensions/confidential-transfer/create-mint) erstellt wurde.
