...
description:
  Erfahren Sie, wie Sie Tokens privat von einem Token-Konto auf ein anderes
  übertragen.
---
...
<Callout type="warn">
  ...
  Erweiterung für vertrauliche Überweisungen derzeit nicht verfügbar ist. Die
  Konzepte sind zwar weiterhin gültig, die Code-Beispiele werden jedoch nicht
  funktionieren.
</Callout>

## Wie man Tokens vertraulich von einem Token-Konto auf ein anderes überträgt

Um Tokens vertraulich von einem Token-Konto auf ein anderes zu übertragen,
müssen sowohl der Absender als auch der Empfänger über Token-Konten verfügen,
die mit dem _rs`ConfidentialTransferAccount`_-Status konfiguriert und für
vertrauliche Überweisungen genehmigt sind. Das Token-Konto des Absenders muss
außerdem über ein verfügbares vertrauliches Guthaben verfügen, von dem
übertragen werden kann.

Um Tokens vertraulich zu übertragen:
...
1. Erstellen Sie
   ...
   **Gleichheitsbeweis
   ...
   Überprüft, dass der neue Chiffretext des verfügbaren Guthabens nach der
   Überweisung mit seinem entsprechenden
   ...
   **Chiffretext-Gültigkeitsbeweis
   ...
   Überprüft, dass die Chiffretexte des Überweisungsbetrags für alle drei
   Parteien (Quelle, Ziel und Prüfer) ordnungsgemäß generiert wurden und stellt
   sicher, dass der Überweisungsbetrag korrekt unter dem öffentlichen Schlüssel
   jeder Partei verschlüsselt ist.
...
2. Für jeden Beweis:
   ...
   - Speichern Sie die beweisspezifischen Metadaten in einem
     "Kontext-Status"-Konto des Beweises zur Verwendung in anderen Anweisungen.

3. Rufen Sie die
   [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)-Anweisung
   auf und geben Sie die Kontext-Status-Konten des Beweises an.

4. Schließen Sie die Proof-Context-State-Konten, um die SOL zurückzuerhalten,
   die für deren Erstellung verwendet wurden.

Das folgende Diagramm zeigt die Schritte, die beim Übertragen von Token vom
Token-Konto eines Absenders zum Token-Konto eines Empfängers erforderlich sind.
...
Um vertraulich Token von einem Token-Konto auf ein anderes zu übertragen, müssen
Sie:

- Einen Equality-Proof, Ciphertext-Validity-Proof und Range-Proof clientseitig
  generieren
- Das Zk-ElGamal-Proof-Programm aufrufen, um die Proofs zu verifizieren und die
  "Context-State"-Konten zu initialisieren
- Die
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)-Anweisung
  aufrufen und dabei die drei Proof-Konten angeben.
- Die drei Proof-Konten schließen, um die Miete zurückzuerhalten.

Die `spl_token_client`-Crate bietet die folgenden Methoden:

- `confidential_transfer_create_context_state_account`-Methode, die ein
  Proof-Konto erstellt.
...
- `confidential_transfer_close_context_state_account`-Methode, die ein
  Proof-Konto schließt.

### Beispielcode

Das folgende Beispiel zeigt, wie man vertraulich Token zwischen Token-Konten
überträgt.

Um das Beispiel auszuführen, starten Sie einen lokalen Validator mit dem von
Mainnet geklonten Token Extension Program mit folgendem Befehl. Sie müssen die
Solana CLI installiert haben, um den lokalen Validator zu starten.
...
<Callout type="info">
  Zum Zeitpunkt der Erstellung dieses Dokuments sind vertrauliche Überweisungen
  auf dem Standard-Local-Validator nicht aktiviert. Sie müssen das Mainnet Token
  Extension Program klonen, um den Beispielcode auszuführen.
</Callout>
