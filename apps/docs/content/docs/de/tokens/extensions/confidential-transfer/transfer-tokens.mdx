...
description:
  Erfahren Sie, wie Sie Tokens privat von einem Token-Konto auf ein anderes übertragen.
---

## So übertragen Sie Token vertraulich von einem token account auf einen anderen

Um Token vertraulich von einem token account auf einen anderen zu übertragen, müssen sowohl der Sender als auch der Empfänger über token accounts verfügen, die mit dem Status _rs`ConfidentialTransferAccount`_ konfiguriert und für die Verwendung von Confidential Balances genehmigt sind. Das token account des Senders muss außerdem ein verfügbares vertrauliches Guthaben zum Übertragen aufweisen.

So übertragen Sie Token vertraulich:

1. Erstellen Sie
   [drei Nachweise](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)
   clientseitig:
   ...
   **Gleichheitsnachweis
   ...
   Überprüft, ob der neue verfügbare Kontostand-Chiffretext nach der Übertragung
   mit der entsprechenden
   [Pedersen-Verpflichtung](https://en.wikipedia.org/wiki/Commitment_scheme)
   übereinstimmt und stellt sicher, dass der neue verfügbare Kontostand des
   Quellkontos korrekt berechnet wird als
   `new_balance = current_balance - transfer_amount`.

   **Chiffretext-Gültigkeitsnachweis
   ...
   Überprüft, ob die Übertragungs-Chiffretexte für alle drei Parteien (Quelle,
   Ziel und Prüfer) ordnungsgemäß erstellt wurden, und stellt sicher, dass der
   Übertragungsbetrag korrekt unter dem öffentlichen Schlüssel jeder Partei
   verschlüsselt ist.
   ...
   **Bereichsnachweis
   ...
   Überprüft, ob der neue verfügbare Kontostand und der Übertragungsbetrag
   (aufgeteilt in niedrige/hohe Bits) alle nicht-negativ und innerhalb eines
   festgelegten Bereichs sind.

2. Für jeden Nachweis:
   - Rufen Sie das ZK-ElGamal-Nachweisprogramm auf, um die Nachweisdaten zu
     verifizieren.
   - Speichern Sie die nachweisbezogenen Metadaten in einem
     Nachweis-„Kontext-Status“-Konten zur Verwendung in anderen Anweisungen.
...
3. Rufen Sie die
   ...
   Anweisungen auf und geben Sie dabei die Nachweis-Kontextstatus-Konten an.

4. Schließen Sie die Nachweis-Kontextstatus-Konten, um das zum Erstellen
   verwendete SOL zurückzugewinnen.

Das folgende Diagramm zeigt die Schritte, die beim Übertragen von Token vom
token account eines Senders auf den token account eines Empfängers erforderlich
sind.
...
- Clientseitig einen Gleichheitsnachweis, einen Chiffretext-Gültigkeitsnachweis
  und einen Bereichsnachweis generieren
- Das ZK-ElGamal-Nachweisprogramm aufrufen, um die Nachweise zu verifizieren und
  die „Kontext-Status“-Konten zu initialisieren
- Die
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  Anweisungen aufrufen und dabei die drei Nachweiskonten angeben.
...
Das folgende Beispiel überträgt Token vertraulich von einem Konto auf ein anderes. Beide Konten müssen bereits für Confidential Balances konfiguriert sein, und der Sender muss über ein verfügbares vertrauliches Guthaben verfügen.

Confidential Balances ist auf das ZK ElGamal Proof-Programm angewiesen, das auf Mainnet und Devnet aktiviert ist. Ein standardmäßiger `solana-test-validator` aktiviert es nicht, aber ein Mainnet-forkender lokaler Validator wie [Surfpool](https://surfpool.run) schon.
Führen Sie das Beispiel mit einem finanzierten Zahler gegen eines dieser Netzwerke aus (der Code verwendet Devnet), und ersetzen Sie die Platzhalter durch Ihre Mint sowie die Sender- und Empfänger-Konten.
