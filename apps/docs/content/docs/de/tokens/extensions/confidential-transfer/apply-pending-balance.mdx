...
## Wie man den ausstehenden Saldo auf den verfügbaren Saldo anwendet

Bevor Token vertraulich übertragen werden können, muss der öffentliche
Token-Saldo in einen vertraulichen Saldo umgewandelt werden. Diese Umwandlung
erfolgt in zwei Phasen:

1. **Vertraulicher ausstehender Saldo**: Zunächst werden Token vom öffentlichen
   Saldo in einen "ausstehenden" vertraulichen Saldo "eingezahlt".
...
Dieser Abschnitt erklärt die zweite Phase: das Anwenden des ausstehenden Saldos
auf den verfügbaren Saldo.

Wenn Token vom öffentlichen Saldo "eingezahlt" werden oder wenn Token
vertraulich von einem Token-Konten auf ein anderes übertragen werden, werden die
...
Saldos auf den verfügbaren Saldo erforderlich sind:
...
Die `spl_token_client` Crate stellt eine
`confidential_transfer_apply_pending_balance` Methode bereit, die eine
Transaktion mit der `ApplyPendingBalance` Anweisung erstellt und sendet, wie im
folgenden Beispiel demonstriert wird.

### Beispielcode

Das folgende Beispiel zeigt, wie der vertrauliche ausstehende Saldo auf den
vertraulichen verfügbaren Saldo angewendet wird.

Um das Beispiel auszuführen, starten Sie einen lokalen Validator mit dem Token
Extension Program, das von Mainnet geklont wurde, mit folgendem Befehl. Sie
...
<Callout type="info">
  ...
  Token Extension Program klonen, um den Beispielcode auszuführen.
</Callout>
