...
description:
  Erstellen Sie ein Token Extension Program-Mint mit einem permanenten
  Delegierten, der Token von jedem Konto für dieses Mint übertragen oder
  verbrennen kann.
...
## Was ist ein permanenter Delegierter?

Die _rs`PermanentDelegate`_-Mint-Erweiterung des Token Extension Programs weist
dem Mint selbst eine Delegierungsberechtigung zu.

Ein permanenter Delegierter ist eine Berechtigung auf Mint-Ebene, die
Überweisungen und Verbrennungen für jedes Token-Konto dieses Mints autorisieren
kann. Er fungiert als globaler Delegierter für alle Token-Konten des Mints, und
Token-Kontoinhaber können den permanenten Delegierten nicht von ihren
Token-Konten widerrufen.

Die Mint-Berechtigung kann den permanenten Delegierten später auch mit
_rs`SetAuthority`_ unter Verwendung von _rs`AuthorityType::PermanentDelegate`_
rotieren.

## So erstellen und verwenden Sie einen permanenten Delegierten

Um einen permanenten Delegierten zu erstellen und zu verwenden:

1. Berechnen Sie die Größe des Mint-Kontos und die benötigte Miete für das Mint
   und die _rs`PermanentDelegate`_-Erweiterung.
2. Erstellen Sie das Mint-Konto mit _rs`CreateAccount`_, initialisieren Sie
   _rs`PermanentDelegate`_ und initialisieren Sie das Mint mit
   _rs`InitializeMint`_.
3. Senden Sie _rs`TransferChecked`_ oder _rs`BurnChecked`_, die vom permanenten
   Delegierten signiert sind. Das Token-Programm verarbeitet diese Anweisungen
   auf die gleiche Weise, als ob der Token-Kontoinhaber sie signiert hätte.
...
_rs`PermanentDelegate`_-Erweiterung. Dies ist die Größe, die in
...
_rs`PermanentDelegate`_-Erweiterung benötigten Größe.
...
## !!steps PermanentDelegate initialisieren

Initialisieren Sie die Erweiterung _rs`PermanentDelegate`_ auf dem Mint.
...
## !!steps Token-Konten erstellen und Tokens prägen

Erstellen Sie Token-Konten für den Eigentümer und Empfänger und prägen Sie dann
Tokens auf das Token-Konto des Eigentümers.
...
## !!steps Übertragung mit dem permanenten Delegierten

Übertragen Sie Tokens mit _rs`TransferChecked`_, signiert vom permanenten
Delegierten.
...
<Callout type="info" title="Reihenfolge der  Anweisungen">
  _rs`InitializePermanentDelegate`_ muss vor _rs`InitializeMint`_ kommen.
  _rs`CreateAccount`_, _rs`InitializePermanentDelegate`_ und
...
| ------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermanentDelegate`_                     | Mint-Erweiterung, die den autorisierten Delegierten für jedes Token-Konto des Mints speichert.                                     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/permanent_delegate.rs#L15-L21) |
| _rs`InitializePermanentDelegate`_           | Anweisung, die den permanenten Delegierten vor _rs`InitializeMint`_ initialisiert.                                                 | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1952-L1967)              |
| _rs`SetAuthority`_                          | Basis-Token-Anweisung, die mit _rs`AuthorityType::PermanentDelegate`_ verwendet wird, um den Delegierten zu rotieren.              | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1488-L1520)              |
| _rs`AuthorityType::PermanentDelegate`_      | Autoritätsdiskriminator, der mit _rs`SetAuthority`_ verwendet wird, um den permanenten Delegierten auf einem Mint zu rotieren.     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1138-L1139)              |
| _rs`process_initialize_permanent_delegate`_ | Prozessorlogik, die die Erweiterung des permanenten Delegierten auf das Mint schreibt.                                             | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1542-L1557)                  |
| _rs`process_set_authority`_                 | Prozessorlogik, die den permanenten Delegierten validiert und rotiert, wenn _rs`AuthorityType::PermanentDelegate`_ verwendet wird. | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L849-L860)                    |
