...
description:
  Konfigurieren Sie ein Token Extensions Program Mint, sodass neu erstellte
  Token-Konten in einem ausgewählten Standardzustand beginnen, wie z. B.
  eingefroren.
url: /docs/tokens/extensions/default-state
type: tutorial
prerequisites:
  - /docs/prerequisites/create-mint
  - /docs/prerequisites/create-token-account
related:
  - /docs/related/freeze-account
  - /docs/related/thaw-account
  - /docs/related/extensions
---

## Was ist der Standard-Kontenstatus?

Die _rs`DefaultAccountState`_ Mint-Erweiterung des Token Extensions Program legt
den initialen _rs`AccountState`_ für jedes neue Token-Konto fest, das für dieses
Mint erstellt wird.

Dies wird am häufigsten verwendet, um neue Token-Konten im eingefrorenen Zustand
zu starten. Token-Konten, die eingefroren starten, können erst verwendet werden,
nachdem die Freeze-Authority des Mint sie aufgetaut hat.

## So erstellen Sie ein Mint mit einem Standard-Kontenstatus

Um ein Mint mit einem Standard-Kontenstatus zu erstellen:

1. Berechnen Sie die Mint-Kontengröße und die benötigte Miete für das Mint und
   die _rs`DefaultAccountState`_ Erweiterung.
2. Erstellen Sie das Mint-Konto mit _rs`CreateAccount`_, initialisieren Sie
   _rs`DefaultAccountState`_ und initialisieren Sie das Mint mit
   _rs`InitializeMint`_.
3. Erstellen Sie Token-Konten für das Mint und beobachten Sie, dass jedes neue
   Token-Konto im aktuellen Standardzustand startet.
4. Verwenden Sie _rs`UpdateDefaultAccountState`_, um den Zustand zu ändern, in
   dem zukünftige Token-Konten starten sollen.
...
_rs`DefaultAccountState`_ Erweiterung. Dies ist die Größe, die in
...
_rs`DefaultAccountState`_ Erweiterung benötigt wird.
...
## !!steps Das Mint-Konto erstellen
...
## !!steps DefaultAccountState initialisieren

Initialisieren Sie die _rs`DefaultAccountState`_-Erweiterung auf dem Mint.
...
## !!steps Ein token account erstellen

Erstellen Sie ein token account für den Mint. Da der aktuelle Standardzustand
_rs`Frozen`_ ist, beginnt das neue token account im eingefrorenen Zustand.
...
## !!steps Standardkontenstatus aktualisieren

Verwenden Sie _rs`UpdateDefaultAccountState`_, um den Standardzustand zu ändern,
...
<Callout type="info" title="Reihenfolge der  Anweisungen">
  _rs`DefaultAccountStateInstruction::Initialize`_ muss vor _rs`InitializeMint`_
  kommen. _rs`CreateAccount`_, _rs`DefaultAccountStateInstruction::Initialize`_
...
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`DefaultAccountState`_                        | Mint-Erweiterung, die den Zustand speichert, den neue token accounts erben sollen.                                  | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/mod.rs#L16-L21)         |
| _rs`DefaultAccountStateInstruction::Initialize`_ | Anweisung, die den Standard-Token-Kontenstatus des Mints vor _rs`InitializeMint`_ initialisiert.                    | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L20-L36) |
| _rs`DefaultAccountStateInstruction::Update`_     | Anweisung, die der Freeze-Autorität ermöglicht, den Standardzustand für zukünftige token accounts zu ändern.        | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L37-L53) |
| _rs`process_initialize_default_account_state`_   | Prozessorlogik, die den anfänglichen Standardzustand auf einen nicht initialisierten Mint schreibt.                 | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L29-L40)     |
| _rs`process_update_default_account_state`_       | Prozessorlogik, die die Freeze-Autorität überprüft, bevor der gespeicherte Standardzustand des Mints geändert wird. | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L43-L74)     |
