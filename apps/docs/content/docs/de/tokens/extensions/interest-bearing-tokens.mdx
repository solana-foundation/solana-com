...
ermöglicht es Ihnen, einen Zinssatz direkt auf einem Mint-Konten zu setzen. Die
Zinsen werden kontinuierlich auf Basis des Netzwerk-Zeitstempels aufgezinst,
aber der tatsächliche Token- Saldo, der in jedem Token-Konten gespeichert ist,
bleibt unverändert. Der Gesamtwert (Token- Saldo plus aufgelaufene Zinsen) ist
nur eine Berechnung. Mit dieser Erweiterung werden keine neuen Token geminted.
