...
description:
  Erstellen Sie eine Token Extensions Program-Mint mit einem Onchain-Zinssatz, aktualisieren Sie diesen Zinssatz und konvertieren Sie Token-Beträge in einen UI-Betrag mit Zinsen.
...
Die _rs`InterestBearingConfig`_-Mint-Erweiterung des Token Extensions Program
ermöglicht es einer Mint, einen annualisierten Zinssatz direkt onchain zu
speichern.

Zinstragende Token fügen **nicht** im Laufe der Zeit mehr Token zu token account
hinzu. Der in jedem token account gespeicherte Token-Betrag bleibt gleich, bis
eine Anweisungen des Token-Programms ihn ändert, wie z. B. beim Prägen,
Übertragen oder Verbrennen.

Mit der Zeit kann der berechnete UI-Betrag mit Zinsen steigen, auch wenn der
Token-Betrag und die Token-Versorgung gleich bleiben.
...
- [`InterestBearingConfig`](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L40-L50)
  speichert `initialization_timestamp`, `pre_update_average_rate`,
  `last_update_timestamp` und `current_rate`.
- [`amount_to_ui_amount`](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L85-L94)
  berechnet den UI-Betrag in zwei Schritten: Zunächst werden Zinsen von der
  Initialisierung bis `last_update_timestamp` unter Verwendung von
  `pre_update_average_rate` berechnet, dann werden Zinsen von
  `last_update_timestamp` bis zum aktuellen Zeitstempel unter Verwendung von
  `current_rate` berechnet.
- Wenn
  ...
  auf und aktualisiert `pre_update_average_rate` auf einen neuen zeitgewichteten
  historischen Durchschnitt, dann wird ein neuer `last_update_timestamp` und
  `current_rate` gespeichert.
- Nach Zinsänderungen im Laufe der Zeit werden die früheren Änderungen durch
  `pre_update_average_rate` dargestellt, während der Zeitraum von
  `last_update_timestamp` bis zur aktuellen Zeit mit `current_rate` berechnet
  wird.
...
1. Berechnen Sie die Größe des mint account und die benötigte rent für die Mint
   und die _rs`InterestBearingConfig`_-Erweiterung.
2. Erstellen Sie das mint account mit _rs`CreateAccount`_, initialisieren Sie
   _rs`InterestBearingConfig`_ und initialisieren Sie die Mint mit
   _rs`InitializeMint`_.
3. Prägen Sie Token wie üblich.
4. Verwenden Sie _rs`UpdateRate`_, um den aktuellen Zinssatz der Mint im Laufe
   der Zeit zu ändern.
5. Konvertieren Sie Token-Beträge in einen UI-Betrag mit Zinsen mit
   _rs`AmountToUiAmount`_ oder Hilfsmethoden, die das mint account und den
   Clock-Sysvar abrufen und dann den UI-Betrag berechnen, ohne eine Transaktion
   zu senden.

<Callout type="info" title="Offchain- und Onchain-Konvertierung von UI-Beträgen">
  Der Offchain-Hilfspfad ruft das Mint-Konten und die Clock-Sysvar ab und
  berechnet dann den UI-Betrag lokal, ohne eine Transaktion zu senden. Der
  Onchain- Pfad verwendet _rs`AmountToUiAmount`_, das im Token Program
  ausgeführt wird und den UI-Betrag in den Transaktionsrückgabedaten zurückgibt.
  _rs`AmountToUiAmount`_ kann in einer Transaktion gesendet oder simuliert
  werden.
</Callout>
...
_rs`InterestBearingConfig`_-Erweiterung. Dies ist die Größe, die in
...
Berechnen Sie die Miete unter Verwendung der für das Mint plus die
_rs`InterestBearingConfig`_-Erweiterung benötigten Größe.
...
## !!steps Das Mint-Konten erstellen

Erstellen Sie das Mint-Konten mit dem berechneten Speicherplatz und den
Lamports.
...
## !!steps InterestBearingConfig initialisieren

Initialisieren Sie die _rs`InterestBearingConfig`_-Erweiterung auf dem Mint.
...
## !!steps Ein Token-Konten erstellen und Tokens prägen

Erstellen Sie ein Token-Konten für das Mint und prägen Sie dann Tokens auf
dieses Token-Konten.
...
## !!steps Berechnen Sie den UI-Betrag mit dem Offchain-Helfer

Rufen Sie das Mint-Konto und den Clock-Sysvar ab und berechnen Sie dann den
UI-Betrag, ohne eine Transaktion zu senden.
...
<Callout type="info" title="Anweisungsreihenfolge">
  _rs`InterestBearingMintInstruction::Initialize`_ muss vor _rs`InitializeMint`_
  erfolgen. _rs`CreateAccount`_,
  _rs`InterestBearingMintInstruction::Initialize`_ und _rs`InitializeMint`_
  müssen in derselben Transaktion enthalten sein.
</Callout>
...
| ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InterestBearingConfig`_                      | Mint-Erweiterung, die die Zinssatzbehörde, Zeitstempel sowie aktuelle und historische Zinssätze speichert.                         | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L40-L153)         |
| _rs`InterestBearingMintInstruction::Initialize`_ | Anweisung, die die zinstragende Konfiguration vor _rs`InitializeMint`_ initialisiert.                                         | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/instruction.rs#L17-L90)  |
| _rs`InterestBearingMintInstruction::UpdateRate`_ | Anweisung, die den aktuellen Zinssatz des Mints ändert.                                                                    | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/instruction.rs#L35-L113) |
| _rs`AmountToUiAmount`_                           | Anweisung, die den aktuellen UI-Betrag als Zeichenkette für einen Token-Betrag unter Verwendung der aktiven Konfiguration des Mints zurückgibt.                      | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L520-L538)                                |
| _rs`InterestBearingConfig::amount_to_ui_amount`_ | Helfer, der einen Token-Betrag in einen UI-Betrag mit Zinsen für einen Zeitstempel umwandelt.                                           | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L85-L96)          |
| _rs`process_initialize`_                         | Prozessorlogik, die _rs`InterestBearingConfig`_ initialisiert und die anfänglichen Zeitstempel und Zinssätze aufzeichnet.                     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/interest_bearing_mint/processor.rs#L21-L43)      |
| _rs`process_update_rate`_                        | Prozessorlogik, die den aktuellen Zinssatz aktualisiert, den zeitgewichteten Durchschnittszinssatz neu berechnet und den Aktualisierungszeitstempel aufzeichnet. | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/interest_bearing_mint/processor.rs#L46-L77)      |
| _rs`process_amount_to_ui_amount`_                | Prozessorlogik, die die UI-Betragszeichenkette unter Verwendung der aktiven zinstragenden Konfiguration des Mints zurückgibt.                     | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1548-L1569)                                  |

### Typescript

Das nachstehende Beispiel `Kit` verwendet die generierten Anweisungen direkt.
