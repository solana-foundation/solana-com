...
description:
  Erstellen Sie ein Mint, dessen Token geprägt, verbrannt und geschlossen werden
  können, aber nicht zwischen Token-Konten übertragen werden können.
...
Die _rs`NonTransferable`_-Mint-Erweiterung des Token Extensions Program macht
jedes Token-Konto für dieses Mint nicht-übertragbar. Nachdem Token geprägt
wurden, können Token-Inhaber diese nicht mit _rs`Transfer`_ oder
_rs`TransferChecked`_ auf ein anderes Token-Konto verschieben.

Dieses Muster ist nützlich für Assets, die an ein Wallet gebunden bleiben
sollen.
...
<Callout type="info" title="Token-Konto-Erweiterungen">
  Wenn ein Token-Konto für ein Mint mit _rs`NonTransferable`_ initialisiert
  wird, wird das Token-Konto mit _rs`NonTransferableAccount`_ und
  _rs`ImmutableOwner`_ initialisiert. Wenn das Token-Konto über das [Associated
  Token
...
1. Berechnen Sie die Mint-Kontogröße und die benötigte Miete für das Mint und
   die _rs`NonTransferable`_-Erweiterung.
2. Erstellen Sie das Mint-Konto mit _rs`CreateAccount`_, initialisieren Sie
   _rs`NonTransferable`_ und initialisieren Sie das Mint mit
   _rs`InitializeMint`_.
3. Erstellen Sie Token-Konten für das Mint. _rs`NonTransferableAccount`_ und
   _rs`ImmutableOwner`_ werden automatisch für Token-Konten aktiviert.
4. _rs`Transfer`_ und _rs`TransferChecked`_ schlagen mit
   _rs`TokenError::NonTransferable`_ fehl.

<ScrollyCoding>

## !!steps Kontogröße berechnen

Berechnen Sie die Mint-Kontogröße für das Basis-Mint plus die
_rs`NonTransferable`_-Erweiterung. Dies ist die Größe, die in
...
Berechnen Sie die Miete anhand der für die Prägung benötigten Größe plus der
_rs`NonTransferable`_-Erweiterung.
...
## !!steps Prägungskonto erstellen

Erstellen Sie das Prägungskonto mit dem berechneten Speicherplatz und den
Lamports.
...
## !!steps NonTransferable initialisieren

Initialisieren Sie die _rs`NonTransferable`_-Erweiterung auf der Prägung.
...
## !!steps Prägung initialisieren

Initialisieren Sie die Prägung mit _rs`InitializeMint`_ in derselben
Transaktion.
...
<Callout type="info" title="Reihenfolge der  Anweisungen">
  _rs`InitializeNonTransferableMint`_ muss vor _rs`InitializeMint`_ erfolgen.
  _rs`CreateAccount`_, _rs`InitializeNonTransferableMint`_ und
...
| ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| _rs`NonTransferable`_                          | Prägungserweiterung, die Token aus der Prägung als nicht übertragbar markiert.                                                                                                                                                                                                            | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/non_transferable.rs#L8-L13)  |
| _rs`NonTransferableAccount`_                   | Token-Konten-Erweiterung, die zu Token-Konten für nicht übertragbare Prägungen hinzugefügt wird.                                                                                                                                                                                          | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/non_transferable.rs#L15-L21) |
| _rs`ImmutableOwner`_                           | Token-Konten-Erweiterung, die Eigentümerwechsel verhindert und für nicht übertragbare Token-Konten erforderlich ist.                                                                                                                                                                      | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L8-L16)   |
| _rs`InitializeNonTransferableMint`_            | Anweisung, die die Nicht-Übertragbar-Erweiterung auf Prägungsebene vor _rs`InitializeMint`_ initialisiert.                                                                                                                                                                                | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L642-L653)              |
| _rs`process_initialize_non_transferable_mint`_ | Prozessorlogik, die die _rs`NonTransferable`_-Prägungserweiterung auf einer nicht initialisierten Prägung initialisiert.                                                                                                                                                                  | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1646-L1655)                |
| _rs`get_required_init_account_extensions`_     | Wird verwendet, um automatisch Token-Konten-Erweiterungen hinzuzufügen, wenn Token-Konten basierend auf den bei der Prägung aktivierten Erweiterungen initialisiert werden. Für _rs`NonTransferable`_-Prägungen werden _rs`NonTransferableAccount`_ und _rs`ImmutableOwner`_ hinzugefügt. | [Quelle](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mod.rs#L1304-L1307)          |
