...
Eine System Program-Übertragung fügt lamports zu **jedem** Konten hinzu. Es gibt
keine protokollseitige Prüfung, ob der Empfänger die SOL wieder
herausverschieben kann. Lamports können nur vom besitzenden Programm des Konten
bewegt werden. Daher birgt das Senden von SOL an eine
...
dauerhaften Verlusts von Guthaben** – nur eine vom besitzenden Programm
festgelegte Autorität kann diese zurückgeben. SOL, das an ein
[token account](/docs/tokens/basics/create-token-account) gesendet wird, ist nur
vom Eigentümer dieses Konten wiederherstellbar, niemals vom Absender.

SPL-**Token**-Übertragungen sind teilweise selbstschützend: Das Token Program
lehnt eine Übertragung ab, deren Konten nicht mit der erwarteten Mint
übereinstimmen. Native **SOL**-Übertragungen haben keinen solchen Schutz, daher
muss der Absender den Empfänger vor dem Signieren überprüfen. Siehe
...
SOL-Übertragungs- Anweisungen enthält.
...
<WithMentions>
Der folgende Code zeigt die Ausgabe der vorherigen Code-Ausschnitte.
Das Format unterscheidet sich je nach SDK,
aber beachten Sie, dass jede Anweisung dieselben erforderlichen Informationen enthält.
...
Da eine SOL-Übertragung in jedes Konten erfolgreich ist, überprüfen Sie den
Empfänger vor dem Signieren. Rufen Sie das Konten ab und senden Sie SOL nur an
...
lehnen Sie Mints, token accounts, Programme und PDAs ab, die Sie nicht
kontrollieren.
...
<Callout>
  Dieser Codeausschnitt prüft native SOL-Empfänger. Für die vollständige
  Klassifizierung, die auch SPL-Token-Überweisungen (token accounts, ATAs,
...
Nach der Übermittlung können Sie Transaktionsdetails mithilfe der
