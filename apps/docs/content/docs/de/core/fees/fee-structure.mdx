...
Die Priorisierungsgebühr ist eine optionale Gebühr, die die Planungspriorität
einer Transaktion erhöht. Eine Compute Unit (CU) ist eine Recheneinheit, die die Laufzeitumgebung
bei der Transaktionsausführung misst. Jede Operation (Arithmetik, Speicherzugriff,
Syscall) kostet eine feste Anzahl von CUs. Sie können die Priorisierungsgebühr festlegen, indem Sie
Compute-Budget- Anweisungen in Ihre Transaktion aufnehmen. (Weitere Informationen finden Sie im
[Leitfaden zur Verwendung von priority fee](/docs/core/fees).)
...
<Callout type="info" title="v1-Transaktionen legen die Fee direkt fest">
  Diese Preis-mal-Limit-Formel gilt für Legacy- und v0-Transaktionen. Im
  kommenden [v1-Format](/docs/core/transactions/versioned-transactions#v1-message-format) ist die
  priority fee ein absoluter Gesamtbetrag in lamports, der direkt in der
  Nachrichtenkonfiguration festgelegt wird – kein CU-Preis, keine Multiplikation, keine Rundung.
</Callout>
