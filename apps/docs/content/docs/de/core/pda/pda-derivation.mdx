...
Der bump seed ist ein einzelnes Byte (u8), das während der Ableitung an die
...
sucht von 255 abwärts bis 1 und ruft _rs`create_program_address`_ mit jedem Wert
...
Die Ableitung einer PDA berechnet lediglich eine Adresse. Sie erstellt kein
Onchain-Konto unter dieser Adresse. Das Konto muss explizit durch eine separate
Anweisung erstellt werden (typischerweise _rs`create_account`_ über CPI).
...
### Iteration über alle möglichen Bumps
...
<Callout type="warn">
  Dies unterscheidet sich vom aktuellen kanonischen Suchverhalten, das für
  `sol_try_find_program_address` dokumentiert ist und von 255 abwärts bis 1
  iteriert.
</Callout>

<Callout type="info">
  Das Kit-Beispiel ist nicht enthalten, da die
...
erste gültige Bump ist 254, wodurch er zum kanonischen Bump wird.
