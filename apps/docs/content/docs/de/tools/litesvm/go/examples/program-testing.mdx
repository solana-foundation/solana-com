---
title: Programmtests
description: Benutzerdefinierte Solana-Programme mit litesvm-go laden und testen
---

Ein benutzerdefiniertes Solana-Programm aus Go heraus zu testen bedeutet, die kompilierte `.so`-Datei in die SVM zu laden, die erwarteten Konten einzurichten und  Anweisungen dagegen zu senden. `litesvm-go` übernimmt all das prozessintern – kein validator erforderlich.
...
Für Tests, die Standardprogramme benötigen (SPL Token, Memo, ...):
...
1. **Konfiguration** – Sysvars, Builtins und Precompiles nach Bedarf aktivieren.
2. **Programm laden** – `AddProgramFromFile` (oder `AddProgram` für In-Memory-Bytes).
3. **Konten einrichten** – Datenkonten mit `SetAccount` vorbelegen.
4. **Senden** –  Anweisungen mit `solana-go` erstellen, marshallen und über `SendLegacyTransaction` übermitteln.
5. **Verifizierung** – Konten mit `GetAccount` auslesen und Logs, Compute Units sowie den Endzustand prüfen.

Sobald dieses Grundgerüst funktioniert, kann es durch weitere  Anweisungen, mehr Konten und einen Helfer für den Boilerplate-Code erweitert werden.
