...
description: Testkonten und Zustand mit LiteSVM einrichten
---

# Beispiel zur Konten-Einrichtung

Demonstriert die direkte Einrichtung von Test-Kontenzuständen ohne
Transaktionen.
...
## Einrichtung eines program data account

Einen programmeigenen Daten-Konten einrichten:
...
<Callout type="info">
  Die Verwendung von `setAccount` ist für die Massen-Konteneinrichtung schneller
  als die Ausführung von Transaktionen.
</Callout>

## Wichtige Punkte

1. **Direkter Zustand**: `setAccount` setzt den Kontenzustand ohne Transaktionen
2. **Mietbefreiung**: Verwende `minimumBalanceForRentExemption` zur Berechnung
   der erforderlichen lamports
3. **Kontenstruktur**: Alle erforderlichen Felder angeben: Adresse, Daten,
   ausführbar, lamports, programAddress, Speicherplatz
4. **Verifizierung**: Verwende `getAccount` oder `client.rpc.getAccountInfo` zur
   Verifizierung
