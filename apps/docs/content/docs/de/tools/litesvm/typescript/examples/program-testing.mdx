...
description: Benutzerdefinierte Solana-Programme mit LiteSVM laden und testen
---

Das Testen eines benutzerdefinierten Solana-Programms bedeutet, seine kompilierte `.so`-Datei in die
SVM zu laden, die erwarteten Konten einzurichten und Anweisungen dagegen zu senden.
LiteSVM erledigt all dies prozessintern – kein validator erforderlich. Wir empfehlen,
Programm-Clients mit dem
[Codama JS Renderer](https://github.com/codama-idl/renderers-js) zu generieren, damit Sie
Zugang zu einem kompatiblen Kit-Plugin haben.
...
## Standard-Programme verwenden

Für Tests, die Standard-Programme erfordern:
...
<Callout type="info">
  Erstellen Sie Ihr Solana-Programm mit `cargo build-sbf`, um die `.so`-Datei zu generieren.
</Callout>

Das Muster ist immer dasselbe:

1. **Programm laden**: Verwenden Sie `addProgramFromFile`, um kompilierte Programme zu laden, und
   importieren Sie myProgramPlugin() aus Ihrem Client
2. **Konfiguration**: Aktivieren Sie Sysvars, Builtins und Precompiles nach Bedarf
3. **Konten einrichten**: Füllen Sie Daten-Konten vorab mit `setAccount`
4. **Senden**: Verwenden Sie `client.myProgram.testInstruction({..}).sendTransaction()` –
   kein manuelles Erstellen von Transaktionen
5. **Überprüfung**: Prüfen Sie den Konten-Zustand nach der Ausführung
   (`client.myProgram.myAccount.fetch`)

Sobald dieses Grundgerüst funktioniert, können Sie eine vollständige Testsuite aufbauen, indem Sie
weitere Anweisungen und Assertions hinzufügen.
