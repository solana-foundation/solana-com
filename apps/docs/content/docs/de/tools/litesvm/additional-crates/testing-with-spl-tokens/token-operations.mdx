...
description:
  Arbeiten mit Token-Operationen – Minting, Transfers und Kontenverwaltung
---

## Einen Token Mint erstellen

Ein Mint ist die Grundlage jedes SPL-Tokens. Er definiert die Eigenschaften des
Tokens und steuert, wer neue Tokens prägen kann.
...
## Einen token account erstellen

Ein token account speichert das Guthaben eines bestimmten spl-tokens für einen
bestimmten Benutzer. Um den Unterschied zwischen diesem und einem ATA zu
verstehen, lesen Sie
...
## Einen associated token account erstellen

Associated Token Accounts (ATAs) sind deterministische Adressen zum Speichern
von Tokens für einen bestimmten Benutzer. Um den Unterschied zwischen diesem und
einem ATA zu verstehen, lesen Sie
[diesen Abschnitt](/docs/tools/litesvm/additional-crates/testing-with-spl-tokens#types-of-token-accounts).
...
## Tokens in einen token account prägen

Sobald Sie einen Mint und token accounts haben, können Sie Tokens auf diese
Konten prägen.
...
## Einen Token-Transfer durchführen

Übertragen Sie Tokens zwischen Konten mithilfe der Transfer- Anweisungen.
...
Die meisten Tokens verwenden Dezimalstellen, um Bruchbeträge darzustellen. So
gehen Sie korrekt damit um:
