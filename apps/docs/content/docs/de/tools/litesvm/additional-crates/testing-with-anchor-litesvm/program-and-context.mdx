...
description:
  Einrichten von Testumgebungen und Erstellen von Anweisungen mit AnchorContext
  und Program
---

## Client-Typen mit declare_program! generieren

Anchor 1.0 führt `declare_program!` ein, das die IDL Ihres Programms zur
Kompilierzeit liest und typsichere `client::accounts::*`- und
`client::args::*`-Module generiert. Dies ist der empfohlene Weg, um Anweisungen
in Tests zu erstellen.
...
<Callout type="info">
  Führen Sie zuerst `anchor build` aus, um `target/idl/my_program.json` zu
  generieren. `declare_program!` liest diese Datei zur Kompilierzeit. Der
  Makroname muss mit dem Crate-Namen Ihres Programms übereinstimmen
  (snake_case).
</Callout>

## Einrichten der Testumgebung

Der `AnchorLiteSVM`-Builder bietet eine fließende API zur Konfiguration Ihrer
Testumgebung.
...
### Kurzschreibweise für ein einzelnes Programm
...
<Callout type="info">
  Wenn Sie keinen Zahler angeben, erstellt und finanziert `AnchorLiteSVM`
  automatisch einen für Sie.
</Callout>

## Der AnchorContext

`AnchorContext` ist die Hauptschnittstelle für die Interaktion mit Ihrer
Testumgebung.
...
## Anweisungen mit Program erstellen

Das `Program`-Struct bietet eine flüssige API zum Erstellen von Anweisungen, die
den Anchor-Client widerspiegelt. Verwende die von `declare_program!` generierten
Typen für Typsicherheit.

### Grundlegendes Erstellen von Anweisungen
...
<Callout type="info">
  Die Methode `.accounts()` akzeptiert jeden Typ, der `ToAccountMetas`
  implementiert, was alle Anchor-Konten-Structs automatisch tun. Ebenso
  akzeptiert `.args()` jeden Typ, der `InstructionData` implementiert.
</Callout>
...
### Verwendung von send_and_confirm_transaction
...
Hier ist ein umfassendes Beispiel, das das Treuhand-Muster mit
`declare_program!` verwendet:
