---
title: Transaktionen
description: Methoden zur Verarbeitung von Transaktionen
---
...
Transaktion ausführen und Zustand ändern.
...
Transaktion simulieren, ohne den Zustand zu ändern.

Gibt `SimulatedTransactionInfo` zurück, das Folgendes enthält:

- `meta`: Transaktionsmetadaten (Logs, Compute-Einheiten, Signatur usw.)
- `post_accounts`: Konten-Zustände nach der Simulation
...
## Wann Simulation verwendet werden sollte

Verwende `simulate_transaction()`, um:

- Transaktionsgültigkeit zu testen, ohne den Zustand zu ändern
- Verbrauch von Compute-Einheiten zu prüfen
- Transaktionslogs ohne Commitment zu überprüfen
- Fehlerbehandlungspfade zu testen
...
`send_transaction()` gibt `TransactionMetadata` direkt zurück, während
`simulate_transaction()` `SimulatedTransactionInfo` zurückgibt, das
`meta: TransactionMetadata` enthält:
...
### Auf Transaktionsdaten zugreifen
...
Transaktion aus dem Verlauf abfragen. Erfordert, dass
`with_transaction_history()` gesetzt ist.
...
Aktuellen Blockhash für Transaktionen abrufen.
...
Aktuellen Blockhash ablaufen lassen.
