...
| `fordefi`    | Fordefi MPC-Verwahrung             |
...
Für die Unterzeichnung mit Fordefi MPC-Verwahrung. API-Anfragen werden mit einer ECDSA
P-256-Anfrage-Signatur authentifiziert, die aus `private_key_pem` geparst wird; um diesen Schlüssel in einem
KMS/HSM zu verwahren, implementieren Sie `FordefiRequestSigner` und verwenden Sie
`Signer::from_fordefi_with_signer`.

Fordefi hat zwei Signiermodi. Setzen Sie `chain` für den **nativen Solana-Modus**, bei dem
Fordefi den Blockhash oder die Fee ersetzen und die Transaktion selbst übertragen kann.
Lassen Sie `chain` als `None` für den **Black-Box-Modus**, bei dem Fordefi rohe Bytes
signiert und Sie die Transaktion selbst übertragen.
