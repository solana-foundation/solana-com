---
title: TypeScript
description: Installieren und verwenden Sie @solana/keychain in TypeScript-Anwendungen
---
...
Fordefi hat zwei Signierungsmodi, die dadurch ausgewählt werden, ob Sie `chain` übergeben. API-Anfragen werden mit einer ECDSA P-256-Anforderungssignatur authentifiziert – geben Sie den Signierungsschlüssel als `privateKeyPem` an oder bewahren Sie ihn in einem KMS/HSM auf, indem Sie einen `requestSigner` übergeben.

**Nativer Solana-Modus** (empfohlen für die On-Chain-Nutzung): Setzen Sie `chain`, und Fordefi signiert und sendet die Transaktion selbst. Da Fordefi vor dem Signieren möglicherweise den Blockhash oder die Fee ersetzt, ist der Signer ein Kit `TransactionSendingSigner` – verwenden Sie ihn mit `signAndSendTransactionMessageWithSigners` und nicht den oben gezeigten Partial-Signer-Flow.
...
**Black-Box-Modus**: Lassen Sie `chain` weg. Fordefi signiert rohe Bytes und sendet diese nicht; Sie stellen die Transaktion selbst zusammen und übermitteln sie. Verwenden Sie dies mit einem Fordefi-Black-Box-Vault.
