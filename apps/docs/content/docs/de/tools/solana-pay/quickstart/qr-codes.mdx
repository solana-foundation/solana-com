...
description: Generieren und personalisieren Sie QR-Codes für mobile Wallet-Zahlungen
---

QR-Codes sind die gängigste Methode, um die Lücke zwischen Webanwendungen und
mobilen Wallets zu überbrücken. Solana Pay enthält eine integrierte
`createQR`-Funktion, die von `@solana/qr-code-styling` unterstützt wird —
zusätzliche QR-Code-Pakete werden nicht benötigt.

## Grundlegende QR-Code-Generierung

### Einen QR-Code generieren und anzeigen
...
Die `createQR`-Funktion akzeptiert optional Parameter für Größe,
Hintergrundfarbe und Vordergrundfarbe:
...
### QR-Optionen für erweiterte Verwendung abrufen

Verwenden Sie `createQROptions`, um das Roh-Konfigurationsobjekt für mehr
...
## Verwendung des Merchant-Clients

Der Merchant-Client stellt ebenfalls QR-Code-Methoden bereit:
...
### Zahlungs-QR mit Statusaktualisierungen
...
Auf mobilen Geräten sollten Sie zusätzlich zum QR-Code einen direkten Link
bereitstellen:
