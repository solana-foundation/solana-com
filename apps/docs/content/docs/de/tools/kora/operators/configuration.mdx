...
> _Hinweis: Wenn `fallback_if_unavailable` auf true gesetzt ist, erlaubt das System Transaktionen
> fortzuführen, falls Redis vorübergehend nicht verfügbar ist, um Dienstunterbrechungen zu verhindern.
> Wenn `max_transactions` auf 0 gesetzt wird, sind unbegrenzte Transaktionen möglich._
...
| `get_payer_signer`          | Den nächsten Gebührenzahler und die Zahlungsadresse zurückgeben        | ✅       | boolean |
...
| ---------------------------- | ------------------------------------------------ |
| `confidential_transfer_mint` | Konfiguration der vertraulichen Salden für den Mint |
...
| Erweiterungsname                  | Beschreibung                                 |
| ------------------------------- | ------------------------------------------- |
| `confidential_transfer_account` | Status der vertraulichen Salden für die Konten |
