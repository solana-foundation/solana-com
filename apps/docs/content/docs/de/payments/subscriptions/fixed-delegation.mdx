...
- Das token account des Nutzers muss vor der Initialisierung vorhanden sein.
- Beträge sind in Basiseinheiten. Bei einem Token mit 6 Dezimalstellen bedeutet
  `1_000_000` `1` Token.
- Führen Sie `initSubscriptionAuthority` nur aus, wenn das Konten der
  Subscription Authority noch nicht existiert.
- `expiryTs` ist ein harter Grenzwert: Sobald er überschritten wird, schlagen
  Transfers fehl und der Sponsor kann rent zurückfordern. Es gibt keine
  Nachfrist für Ausgaben.
- Der Nutzer unterzeichnet Setup- und Widerrufstransaktionen. Der
  Bevollmächtigte unterzeichnet Transfers.
