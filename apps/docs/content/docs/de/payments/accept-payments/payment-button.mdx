...
| ----------------- | -------- | ----------------------------------------- |
| `merchant.name`   | `string` | Firmenname beim Checkout angezeigt        |
| `merchant.wallet` | `string` | Solana-Wallet-Adresse für Zahlungsempfang |
...
Für `buyNow`- und `cart`-Modi Produkte über `paymentConfig` bereitstellen:
...
| `onCancel`         | `()`                  | Benutzer hat Zahlung abgebrochen           |

## Benutzerdefinierter Trigger

Standard-Button durch eigenes Element ersetzen:
