...
Nach der Ableitung der ATA-Adresse prüfen Sie, ob das Token-Konto onchain
existiert. Wenn die ATA nicht existiert, können Sie eine Anweisung zum Erstellen
des Token-Kontos des Empfängers in dieselbe Transaktion wie die Übertragung
aufnehmen. Dies erfordert jedoch die Zahlung der Miete für das neue Token-Konto.
Da der Empfänger die ATA besitzt, können die für die Miete gezahlten SOL nicht
vom Absender zurückgefordert werden.
