...
- **Token Program / Token-2022**: Überprüfen Sie, ob das Konten ein token
  account (kein mint account) ist und dass es den Token (Mint) enthält, den Sie
  senden möchten. Wenn gültig, senden Sie Token direkt an diese Adresse. Wenn es
  sich um ein mint account oder ein token account für einen anderen Mint
  handelt, lehnen Sie die Adresse ab.
