...
Betracht ziehen – eine einheitliche Signatur-Bibliothek, die mehrere
...
mit In-Memory-Keys entwickeln und dann zu produktionsreifen Backends wechseln,
...
[Anleitung zum Hinzufügen von Signern](https://github.com/solana-foundation/solana-keychain/blob/main/docs/ADDING_SIGNERS.md)
an, um zusätzliche Key-Management-Dienste zu integrieren.

### RPC-Sicherheit

Behandeln Sie RPC-Endpunkte wie API-Schlüssel – stellen Sie sie nicht in
Frontend-Code bereit, wo sie extrahiert und missbraucht werden können. Verwenden
Sie einen Backend-Proxy oder Umgebungsvariablen, die nicht in Client-Code
gebündelt werden.

- [QuickNode: Best Practices für Endpunkt-Sicherheit](https://www.quicknode.com/guides/quicknode-products/endpoint-security/endpoint-security-best-practices)
...
| RPC-Fehlerrate          | Provider-Zustand             |
...
Für die Transaktionsüberwachung in Echtzeit im großen Maßstab siehe unsere
[Indexierungs-Anleitung](/docs/payments/accept-payments/indexing).

### Adressen verifizieren

Jeder Token und jedes Programm hat genau eine korrekte Adresse im Mainnet.
Gefälschte Tokens, die USDC oder andere Stablecoins nachahmen, sind weit
...
**Umgebungsbasierte Konfiguration:** Devnet und Mainnet verwenden oft völlig
unterschiedliche Token-Mints. Richten Sie die Konfiguration Ihrer Anwendung so
ein, dass die korrekten Adressen für jede Umgebung geladen werden – hardcodieren
Sie nicht Mainnet-Adressen und vergessen dann, diese beim Testen auszutauschen,
...
Auch Programmadressen sind wichtig. Das Senden von Anweisungen an das falsche
Programm schlägt fehl – oder führt schlimmstenfalls zu einem unwiderruflichen
Verlust von Geldern. Die Adressen des Token Program sind:
...
- [ ] Alle gängigen Fehler werden sauber behandelt
...
- [ ] Transaktionsüberwachung und Benachrichtigungen aktiv
...
- **Programm-Keypair:** Ihr Programm wird eine andere Adresse im Mainnet haben
  als im Devnet (es sei denn, Sie verwenden dasselbe Keypair erneut).
...
- **Konten initialisieren:** Wenn Ihr Programm Admin-Konten, PDAs oder andere
  Statuskonten erfordert, stellen Sie sicher, dass diese im Mainnet erstellt
...
- **Miete:** Stellen Sie sicher, dass Ihr Deployment-Wallet genügend SOL hat, um
  die mietbefreiten Mindestbeträge für alle program accounts abzudecken.
- **Verifizierung:**
