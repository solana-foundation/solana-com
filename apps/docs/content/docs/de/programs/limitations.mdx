...
Onchain-Rust-Programme unterstützen den Großteil von Rusts libstd, libcore und
liballoc sowie viele Drittanbieter-Crates.
...
- Eingeschränkter Zugriff auf:
  - `std::hash`
  - `std::os`
- Bincode ist sowohl hinsichtlich der Zyklen als auch der Aufruftiefe extrem
...
- Keine Unterstützung für `println!`, `print!`, verwenden Sie stattdessen das
  [`msg!`](https://github.com/solana-labs/solana/blob/d9b0fc0e3eec67dfe4a97d9298b15969b2804fab/sdk/program/src/log.rs#L33)-Makro.
- Die Runtime erzwingt ein Limit für die Anzahl der Anweisungen, die ein
  Programm während der Verarbeitung einer Anweisung ausführen kann. Weitere
...
## Aufrufstapeltiefe - `CallDepthExceeded`-Fehler
...
Wenn ein Programm das zulässige Limit für die Aufrufstapeltiefe überschreitet,
erhält es den `CallDepthExceeded`-Fehler.

## CPI-Aufruftiefe - `CallDepth`-Fehler

Cross-Program-Invocations ermöglichen es Programmen, andere Programme direkt
aufzurufen, aber die Tiefe ist derzeit auf `4` beschränkt.

Wenn ein Programm die zulässige
[Cross-Program-Invocation-Aufruftiefe](/docs/core/cpi) überschreitet, erhält es
einen `CallDepth`-Fehler
