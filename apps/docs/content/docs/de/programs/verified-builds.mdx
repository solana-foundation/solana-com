...
Root-Verzeichnis Ihres Repositories haben. Wenn Sie nur ein Programm in Ihrem
Repository haben und eine `cargo.lock`-Datei in Ihrem Root, können Sie direkt
zum nächsten Schritt übergehen und Ihr Programm bauen.
...
Root-Verzeichnis Ihres Repositories erstellen.
...
Stellen Sie sicher, dass Ihr Programm im `workspace/members`-Array enthalten ist
und dass die `Cargo.toml` Ihres Programms den korrekten `lib`-Namen konfiguriert
hat.

> Wichtig ist der `lib name`, nicht der Package-Name!
...
ausführen, um eine Lock-Datei zu erstellen und mit dem Bauen Ihres Programms
fortzufahren.
...
Um Ihr Solana-Programm verifizierbar zu bauen, navigieren Sie zum Verzeichnis,
das die `Cargo.toml`-Datei Ihres Workspaces enthält, und führen Sie aus:
...
> ihn nicht versehentlich mit
> {/* INLINE_CODE_PLACEHOLDER_1c58028c75dfa3b13753d584583ffa6e */} oder
> `cargo build-sbf` überschreiben, da diese höchstwahrscheinlich nicht zum
> gleichen Hash führen und Ihre Verifizierung daher fehlschlagen wird.
...
> die den Bibliotheksnamen deines Programms enthält.
...
Für den Rest dieses Multisig-Leitfadens verwenden wir eine Beispiel-Programm-ID
...
Optional: Prüfe, ob du zuerst lokal verifizieren kannst (dieser Befehl verwendet
die Beispiel-Programm-ID `6XBGfP17P3KQAKoJb2s5M5fR4aFTXzPeuC1af2GYkvhD`):
...
Build-Daten onchain hochzuladen, wenn du den Befehl
`solana-verify verify-from-repo` verwendest.
...
Transaktion zur Verwendung in einem Transaction Inspector benötigst, kannst du
...
erstellt, findest du im Ellipsis-Labs-Repository unter
...
> mehrere Programme enthält. Wir verwenden das `--bpf`-Flag, weil `squads_mpl`
...
betreiben oder die Programmverifizierung lokal selbst durchführen, indem du den
`verify-from-repo`-Befehl verwendest. Dabei werden die
Onchain-Verifizierungsdaten genutzt, die in einem
[PDA](https://explorer.solana.com/address/63XDCHrwZu3mXsw2RUFb4tbNpChuUHx4eA5aJMnHkpQQ/anchor-account)
...
einfachen Zugriff auf die im Verifizierungs-PDA gespeicherten
Verifizierungsdaten zu ermöglichen. Der PDA, der alle Informationen enthält, die
zum Erstellen und Verifizieren eines Programms benötigt werden, wird von der
Programm-Adresse und dem Verifizierer-Pubkey abgeleitet. Standardmäßig ist dies
derselbe Pubkey, der das Programm erstellt und bereitgestellt hat. Es kann aber
auch ein anderer Pubkey sein, der in der `security.txt` angegeben werden kann.
...
zeitaufwändig sein, die Entwickler allein anhand der Contract-Adresse zu
identifizieren. Die Einbettung einer `security.txt`-Datei innerhalb des
Programms stellt sicher, dass Sicherheitsforscher die richtigen Personen leicht
kontaktieren können, wodurch potenzielle Exploits verhindert und zeitnahe
