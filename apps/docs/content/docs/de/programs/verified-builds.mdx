...
- **Kein On-Chain-PDA**: Führen Sie `solana-verify verify-from-repo -um` aus und
  **wählen Sie JA, wenn Sie dazu aufgefordert werden**. Ohne das Hochladen des
  PDA kann die API Ihre Verifizierungsmetadaten nicht abrufen.
...
- **Unterschiede in der Build-Umgebung**: Verwenden Sie Docker mit
  solana-verify, wenn Sie Ihren PDA erstellen
...
- **On-Chain-Speicherung**: Speichern Sie Ihre Verifizierungsmetadaten
  ...
  Verify-Programm gehört (`verifycLy8mB96wd9wqq3WDXQwM4oU6r42Th37Db9fC`)
- **Kryptografische Verknüpfung**: Ihr PDA wird von Ihrer Programmadresse
  abgeleitet, wodurch eine unveränderliche Verknüpfung zu Ihren
...
- **Vertrauenslos**: Die API lehnt beliebige Daten ab – sie verwendet nur das,
  was Ihre Upgrade-Authority on-chain gespeichert hat
...
- **On-Chain-PDA**: Ihre Verifizierungsmetadaten befinden sich in der Blockchain
  und werden von keiner zentralen Instanz kontrolliert
- **Nachweis der Upgrade-Authority**: Nur die Upgrade-Authority Ihres Programms
  kann das PDA erstellen/aktualisieren
- **Unabhängige Verifizierung**: Jeder kann durch Auslesen Ihres PDA und lokales
  Ausführen von `solana-verify` verifizieren
- **Kontinuierliche Re-Verifizierung**: Die API verifiziert automatisch alle
...
Verifizieren Sie jedes Programm selbst, indem Sie dessen On-Chain-PDA auslesen
...
1. Die PDA-Metadaten sind authentisch (in der Blockchain gespeichert)
2. Der Quellcode im Repository des PDA stimmt mit dem deployteten Programm
   überein
3. Sie müssen der API nicht vertrauen – verifizieren Sie alles selbst in der
   Blockchain
