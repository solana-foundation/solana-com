...
leichtgewichtiges Test-Framework zum Testen von Solana-Programmen. Es bietet
eine einfache Schnittstelle zum Testen von Solana-Programm-Anweisungen in einer
minimierten Solana Virtual Machine (SVM)-Umgebung. Alle Test-Konten müssen
...
Um die Compute-Unit-Nutzung zu benchmarken, füge `mollusk-svm-bencher` als
...
Program für Tests mit Mollusk zu verwenden, füge `mollusk-svm-programs-token`
als Abhängigkeit in `Cargo.toml` hinzu:
...
Dieses Beispiel zeigt, wie man ein einfaches Solana-Programm mit Mollusk testet.
...
1. [Erstelle eine `Mollusk`-Instanz](mention:mollusk) - Initialisiere Mollusk
   mit einer Programm-ID und dem Pfad zum kompilierten Programm (`.so`-Datei)
2. [Erstelle eine Instruktion](mention:instruction) - Erstelle eine Instruktion
   zum Aufrufen des Programms
3. [Verarbeite und validiere](mention:process) - Verarbeite die Instruktion mit
   Mollusk und validiere das Ergebnis
...
Struct bietet eine einfache Schnittstelle zum Testen von Solana-Programmen. Alle
Felder können durch eine Handvoll Hilfsmethoden manipuliert werden, aber
Benutzer können auch direkt darauf zugreifen und sie ändern, wenn sie mehr
Kontrolle wünschen.

Um Mollusk mit einer Standardinstanz zu initialisieren, verwende die
...
Um Mollusk mit einem bestimmten Programm zu initialisieren, verwende die
`Mollusk::new`-Methode.
...
Um ein Programm zu Mollusk hinzuzufügen, verwende die
`Mollusk::add_program`-Methode.
...
<Callout type="info">
  Wenn du den Dateipfad angibst, füge nicht die `.so`-Erweiterung hinzu. Zum
...
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | Verarbeitet mehrere Anweisungen und führt eine Reihe von Prüfungen für jedes Ergebnis durch, wobei bei fehlgeschlagenen Prüfungen ein Panic ausgelöst wird. |
...
Das folgende Beispiel verarbeitet eine SOL-Überweisungsanweisung ohne
Validierungsprüfungen.

<Callout type="info">

Die folgenden Beispiele führen Mollusk in der `main`-Funktion zu
Demonstrationszwecken aus. In der Praxis verwenden Sie Mollusk typischerweise in
...
Anweisung mit Validierungsprüfungen zu verarbeiten. Diese Methode löst einen
Panic aus, wenn eine Prüfung fehlschlägt.
...
ist ein Wrapper um `Mollusk`, der den Kontenstatus über mehrere
...
intern über seinen `account_store`. Dadurch entfällt die Notwendigkeit des
`accounts`-Parameters bei der Verarbeitung von Anweisungen.
...
beständigem Konto-Status zwischen den Anweisungen über den `account_store`.
...
Das folgende Beispiel zeigt, wie Sie die Mollusk-Sysvar direkt ändern können,
indem Sie auf das `sysvars`-Feld zugreifen, um die Rent-Parameter zu ändern. Sie
können andere Sysvar-Werte auf die gleiche Weise ändern.
...
um das Token Program, Token2022 Program (Token Extensions) und Associated Token
Program zu Mollusk für Tests hinzuzufügen.
...
<Callout type="info">
  Das folgende Beispiel definiert die Test-Konten manuell zu
  Demonstrationszwecken. Das `mollusk-svm-programs-token` enthält auch
  Hilfsfunktionen zum Erstellen der Mint- und Token-Konten.
</Callout>
