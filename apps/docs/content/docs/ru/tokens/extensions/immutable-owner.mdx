---
title: Постоянный владелец
description:
  Инициализируйте аккаунт токена Token Extensions Program с неизменяемым
  владельцем, чтобы его владелец никогда не мог быть переназначен.
...
## Что такое неизменяемый владелец?

Расширение аккаунта _rs`ImmutableOwner`_ Token Extensions Program делает
владельца token account постоянным.

После того как token account инициализирован с _rs`ImmutableOwner`_, последующие
инструкции _rs`SetAuthority`_ не могут изменить его полномочие
_rs`AccountOwner`_.

<Callout type="info" title="Associated Token Accounts">
  Associated token accounts, создаваемые через Associated Token Program для
  Token Extensions Program, автоматически инициализируют _rs`ImmutableOwner`_ на
  token account.
</Callout>

## Как инициализировать аккаунт с неизменяемым владельцем

Чтобы инициализировать аккаунт с неизменяемым владельцем:

1. Создайте и инициализируйте mint.
2. Создайте token account с достаточным пространством для _rs`ImmutableOwner`_.
3. Инициализируйте _rs`ImmutableOwner`_ на token account перед
   _rs`InitializeAccount`_.
4. Попытка изменить владельца аккаунта позже с помощью _rs`SetAuthority`_
   завершится ошибкой _rs`TokenError::ImmutableOwner`_.

<ScrollyCoding>

## !!steps Рассчитайте размер token account

Рассчитайте размер token account для базового token account плюс расширение
_rs`ImmutableOwner`_. Это размер, используемый в _rs`CreateAccount`_.
...
Рассчитайте rent, используя размер token account, необходимый для расширения
...
## !!steps Создайте token account

Создайте token account с рассчитанным пространством и lamports.
...
## !!steps Инициализация ImmutableOwner

Инициализируйте расширение _rs`ImmutableOwner`_ на token account.
...
## !!steps Инициализация token account

Инициализируйте token account с помощью _rs`InitializeAccount`_ в той же
транзакции.
...
<Callout type="info" title="Порядок инструкций">
  _rs`InitializeImmutableOwner`_ должна идти перед _rs`InitializeAccount`_.
  _rs`CreateAccount`_, _rs`InitializeImmutableOwner`_ и _rs`InitializeAccount`_
...
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| _rs`ImmutableOwner`_                     | Расширение аккаунта, которое помечает владельца token account как неизменяемого.                                                | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L13-L16) |
| _rs`InitializeImmutableOwner`_           | Инструкция, которая инициализирует расширение неизменяемого владельца на неинициализированном token account.                    | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1982-L1991)           |
| _rs`process_initialize_immutable_owner`_ | Логика процессора, которая выделяет и инициализирует _rs`ImmutableOwner`_ на аккаунте.                                          | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1532-L1543)               |
| _rs`SetAuthority`_                       | Базовая токен-инструкция, используемая для изменения полномочий, включая владельца token account.                               | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1611-L1633)           |
| _rs`process_set_authority`_              | Логика процессора, которая отклоняет изменения владельца, когда token account имеет включенное расширение _rs`ImmutableOwner`_. | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L703-L900)                 |
