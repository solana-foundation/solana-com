---
title: Приостанавливаемый выпуск
description:
  Создайте минт Token-2022 с полномочием паузы, блокируйте переводы во время
  паузы минта и возобновляйте активность позже.
...
## Что такое минт с возможностью паузы?

Расширение минта _rs`PausableConfig`_ программы Token Extensions Program дает
полномочию паузы возможность приостанавливать и возобновлять активность токенов
для всего минта.

Когда минт приостановлен, программа Token Extensions Program отклоняет:

- Переводы
- Эмиссию
- Сжигание

Когда минт возобновлен, эти инструкции снова работают нормально.

## Как создать, приостановить и возобновить минт

Чтобы создать, приостановить и возобновить минт:

1. Рассчитайте размер аккаунта минта и необходимую rent для минта и расширения
   _rs`PausableConfig`_.
2. Создайте аккаунт минта с помощью _rs`CreateAccount`_, инициализируйте
   _rs`PausableConfig`_ и инициализируйте минт с помощью _rs`InitializeMint`_.
3. Выпустите токены и создайте token accounts для минта.
4. Используйте _rs`Pause`_ для блокировки активности токенов для минта.
5. Используйте _rs`Resume`_ для повторного разрешения активности токенов.
...
_rs`PausableConfig`_. Это размер, используемый в _rs`CreateAccount`_.
...
## !!steps Инициализация PausableConfig

Инициализируйте расширение _rs`PausableConfig`_ на минте.
...
## !!steps Приостановка минта

Приостановите минт с помощью _rs`Pause`_.
...
## !!steps Возобновление минта

Возобновите минт с помощью _rs`Resume`_.
...
<Callout type="info" title="Порядок инструкций">
  _rs`PausableInstruction::Initialize`_ должна идти перед _rs`InitializeMint`_.
  _rs`CreateAccount`_, _rs`PausableInstruction::Initialize`_ и
...
| ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PausableConfig`_                  | Расширение минта, которое хранит полномочия приостановки и информацию о том, приостановлен ли минт в данный момент.               | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/mod.rs#L17-L22)         |
| _rs`PausableInstruction::Initialize`_ | Инструкция, которая инициализирует настройку возможности приостановки перед _rs`InitializeMint`_.                                 | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L20-L32) |
| _rs`PausableInstruction::Pause`_      | Инструкция, которая помечает минт как приостановленный.                                                                           | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L33-L44) |
| _rs`PausableInstruction::Resume`_     | Инструкция, которая снимает отметку приостановки с минта.                                                                         | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L45-L56) |
| _rs`process_initialize`_              | Логика процессора, которая инициализирует _rs`PausableConfig`_ на неинициализированном минте и сохраняет полномочия приостановки. | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L22-L35)     |
| _rs`process_toggle_pause`_            | Логика процессора, которая проверяет полномочия приостановки перед переключением флага приостановки минта.                        | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L39-L64)     |

### Typescript

Пример ниже `Kit` использует сгенерированные инструкции напрямую. Устаревшие
