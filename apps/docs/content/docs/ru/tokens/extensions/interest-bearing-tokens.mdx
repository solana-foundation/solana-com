...
позволяет напрямую установить процентную ставку для mint account. Проценты
начисляются непрерывно на основе сетевого таймстемпа, но фактический баланс
токенов, хранящийся в каждом token account, остаётся неизменным. Общая стоимость
(баланс токенов плюс начисленные проценты) является только расчётной величиной.
Новые токены с помощью этого расширения не выпускаются.
