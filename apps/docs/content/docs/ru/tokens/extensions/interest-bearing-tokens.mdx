...
постоянно начисляются на основе сетевого таймстемпа, но фактический баланс
токенов, хранящийся в каждом token account, остаётся неизменным. Общая стоимость
(баланс токенов плюс начисленные проценты) — это только вычисление. Новые токены
с этим расширением не выпускаются.
