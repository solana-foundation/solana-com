...
description:
  Создайте минт Token Extensions Program с процентной ставкой на блокчейне, обновите эту ставку и конвертируйте количество токенов в UI-сумму с учетом процентов.
...
## Что такое минт с начислением процентов?

Расширение минта _rs`InterestBearingConfig`_ в Token Extensions Program
позволяет минту хранить годовую процентную ставку непосредственно на блокчейне.

Токены с начислением процентов **не** добавляют больше токенов в токен-аккаунты
с течением времени. Количество токенов, хранящееся в каждом токен-аккаунте,
остается неизменным до тех пор, пока инструкция токен-программы не изменит его,
например, при минтинге, переводе или сжигании.

С течением времени рассчитанная UI-сумма с процентами может увеличиваться, даже
если количество токенов и их общее предложение остаются прежними.

## Как рассчитываются исторические изменения ставки
...
- [`InterestBearingConfig`](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L40-L50)
  хранит `initialization_timestamp`, `pre_update_average_rate`,
  `last_update_timestamp` и `current_rate`.
- [`amount_to_ui_amount`](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L85-L94)
  вычисляет UI-сумму в два этапа: сначала рассчитывает проценты от инициализации
  до `last_update_timestamp`, используя `pre_update_average_rate`, затем
  рассчитывает проценты от `last_update_timestamp` до текущей временной метки,
  используя `current_rate`.
- Когда выполняется
  ...
  и обновляет `pre_update_average_rate` до нового взвешенного по времени
  исторического среднего, затем записывает новые значения
  `last_update_timestamp` и `current_rate`.
- После изменения ставки с течением времени более ранние изменения представлены
  в `pre_update_average_rate`, в то время как период от `last_update_timestamp`
  до текущего времени рассчитывается с использованием `current_rate`.

## Как создавать и использовать токены с начислением процентов

Чтобы создавать и использовать токены с начислением процентов:

1. Рассчитайте размер минт-аккаунта и необходимую ренту для минта и расширения
   _rs`InterestBearingConfig`_.
2. Создайте минт-аккаунт с помощью _rs`CreateAccount`_, инициализируйте
   _rs`InterestBearingConfig`_ и инициализируйте минт с помощью
   _rs`InitializeMint`_.
3. Минтите токены как обычно.
4. Используйте _rs`UpdateRate`_ для изменения текущей ставки минта со временем.
5. Конвертируйте количество токенов в UI-сумму с процентами с помощью
   _rs`AmountToUiAmount`_ или вспомогательных методов, которые получают
   минт-аккаунт и системную переменную clock, а затем вычисляют UI-сумму без
   отправки транзакции.

<Callout type="info" title="Преобразование UI Amount Offchain и Onchain">
  Путь offchain-хелпера извлекает mint account и системную переменную clock,
  затем вычисляет UI amount локально без отправки транзакции. Путь onchain
  использует _rs`AmountToUiAmount`_, который выполняется в Token Program и
  возвращает UI amount в данных возврата транзакции. _rs`AmountToUiAmount`_
  может быть отправлен в транзакции или симулирован.
</Callout>

<ScrollyCoding>

## !!steps Вычислить размер аккаунта

Вычислите размер mint account для базового минта плюс расширение
_rs`InterestBearingConfig`_. Это размер, используемый в _rs`CreateAccount`_.
...
## !!steps Вычислить rent

Вычислите rent, используя размер, необходимый для минта плюс расширение
_rs`InterestBearingConfig`_.
...
Создайте mint account с вычисленным пространством и lamport.
...
## !!steps Инициализировать InterestBearingConfig

Инициализируйте расширение _rs`InterestBearingConfig`_ на минте.
...
Инициализируйте минт с _rs`InitializeMint`_ в той же транзакции.
...
Создайте token account для минта, затем выпустите токены на этот token account.
...
## !!steps Рассчитайте UI-сумму с помощью offchain-помощника

Получите mint account и clock sysvar, затем рассчитайте UI-сумму без отправки
транзакции.
...
<Callout type="info" title="Порядок инструкций">
  _rs`InterestBearingMintInstruction::Initialize`_ должна предшествовать
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`InterestBearingMintInstruction::Initialize`_ и _rs`InitializeMint`_ должны
...
| ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InterestBearingConfig`_                      | Расширение mint, которое хранит полномочия процентной ставки, временные метки и текущие и исторические ставки.                         | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L40-L153)         |
| _rs`InterestBearingMintInstruction::Initialize`_ | Инструкция, которая инициализирует конфигурацию с процентной ставкой перед _rs`InitializeMint`_.                                         | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/instruction.rs#L17-L90)  |
| _rs`InterestBearingMintInstruction::UpdateRate`_ | Инструкция, которая изменяет текущую процентную ставку mint.                                                                    | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/instruction.rs#L35-L113) |
| _rs`AmountToUiAmount`_                           | Инструкция, которая возвращает текущую строку UI-суммы для количества токенов, используя активную конфигурацию mint.                      | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L520-L538)                                |
| _rs`InterestBearingConfig::amount_to_ui_amount`_ | Помощник, который преобразует количество токенов в UI-сумму с учётом процентов для временной метки.                                           | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/interest_bearing_mint/mod.rs#L85-L96)          |
| _rs`process_initialize`_                         | Логика процессора, которая инициализирует _rs`InterestBearingConfig`_ и записывает начальные временные метки и ставку.                     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/interest_bearing_mint/processor.rs#L21-L43)      |
| _rs`process_update_rate`_                        | Логика процессора, которая обновляет текущую ставку, пересчитывает средневзвешенную по времени ставку и записывает временную метку обновления. | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/interest_bearing_mint/processor.rs#L46-L77)      |
| _rs`process_amount_to_ui_amount`_                | Логика процессора, которая возвращает строку UI-суммы, используя активную конфигурацию mint с процентной ставкой.                     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1548-L1569)                                  |
