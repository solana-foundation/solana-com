...
пример создания токена с включённым расширением Scaled UI Amount с помощью CLI
`spl-token`:
...
Чтобы обновить множитель UI-значения, используйте команду
`update-ui-amount-multiplier`. Метка времени в секундах с начала эпохи Unix
является необязательной и может быть использована для задания пользовательского
времени старта для нового множителя. Если метка времени не указана, будет
