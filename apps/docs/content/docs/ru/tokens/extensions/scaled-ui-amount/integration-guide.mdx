...
- Чтобы получить актуальную информацию о балансе пользователя, вы можете вызвать
  getTokenAccountBalance или getAccountInfo для обновления вышеуказанных сумм.
- Если вам нужно узнать UI Amount для произвольной суммы, вы можете выполнить
  ...
  - Примечание: amountToUiAmount требует симуляции транзакции, а значит,
    необходим действующий fee payer с балансом. Поэтому этот способ не должен
    использоваться по умолчанию для получения баланса.
...
- Пользователи должны вводить суммы, которые будут интерпретироваться как
  ...
  конвертировать их в исходное количество токенов для транзакции.
  ...
  - Для этой конвертации вы можете использовать функцию
...
- Текущий множитель можно узнать из token mint в любой момент, вызвав
  `getAccountInfo`. Если также установлен будущий множитель, эта информация
  также доступна из token mint. Мы рекомендуем не показывать этот множитель, так
  как это может запутать UX.
