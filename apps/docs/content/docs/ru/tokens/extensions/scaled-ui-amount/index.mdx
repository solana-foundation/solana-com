...
Расширение минта _rs`ScaledUiAmount`_ Token Extensions Program позволяет минту
применять обновляемый множитель к отображаемой сумме UI для балансов токенов.

Это полезно, когда токен представляет собой реальный актив или инструмент,
например, дробление акций или дивиденды. Например, дробление акций удваивает
количество акций для всех держателей. Без изменения баланса каждого token
account, _rs`ScaledUiAmount`_ позволяет кошелькам и приложениям отображать новую
сумму UI, используя множитель минта.

Эта функция также может использоваться для дивидендов или распределения
доходности.

Новые токены не создаются. Количество токенов, хранящееся в token account,
остается прежним. Изменяется только отображаемая сумма UI.

<Callout type="info" title="Несовместимое расширение">
  Не включайте _rs`ScaledUiAmount`_ и _rs`InterestBearingConfig`_ на одном и том
  же минте. Token Extensions Program отклоняет такую комбинацию расширений.
</Callout>

## Как работает Scaled UI Amount

- _rs`ScaledUiAmountConfig`_ хранит полномочия на обновление, `multiplier`,
  `new_multiplier` и `new_multiplier_effective_timestamp`.
- До `new_multiplier_effective_timestamp` преобразования используют
  `multiplier`. В этот момент времени или после него преобразования используют
  `new_multiplier`.
- _rs`UpdateMultiplier`_ может применить новый множитель немедленно или
  запланировать его на будущую временную метку Unix.
- _rs`AmountToUiAmount`_, _rs`UiAmountToAmount`_ и вспомогательные функции
  преобразования расширения используют арифметику с плавающей точкой для минтов
  со scaled UI amount, поэтому не гарантируется, что преобразования будут точно
  обратимыми.

## Как использовать Scaled UI Amount

Чтобы использовать scaled UI amount:

1. Вычислите размер mint account и необходимый rent для минта и расширения
   _rs`ScaledUiAmount`_.
2. Создайте mint account с помощью _rs`CreateAccount`_, инициализируйте
   _rs`ScaledUiAmount`_ и инициализируйте минт с помощью _rs`InitializeMint`_.
3. Используйте _rs`AmountToUiAmount`_, _rs`UiAmountToAmount`_ или
   вспомогательные клиентские функции при отображении балансов или
   преобразовании сумм UI.
4. Используйте _rs`UpdateMultiplier`_ для обновления отображаемого множителя
   немедленно или в будущую временную метку Unix.

<Callout type="info" title="Порядок инструкций">
  _rs`ScaledUiAmountMintInstruction::Initialize`_ должна идти перед
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`ScaledUiAmountMintInstruction::Initialize`_ и _rs`InitializeMint`_ должны
...
<Cards>
  ...
  >
    Создайте минт с масштабируемой суммой UI и обновляйте множитель с течением
    времени.
  </Card>
  <Card
  ...
  >
    Преобразуйте суммы токенов для отображения и поддержите масштабируемую сумму
    UI в кошельках и приложениях.
  </Card>
</Cards>
...
| ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`ScaledUiAmountConfig`_                            | Состояние расширения минта, которое хранит полномочия, текущий множитель, следующий множитель и временную метку вступления в силу.      | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L41-L55)         |
| _rs`ScaledUiAmountMintInstruction::Initialize`_       | Инструкция, которая инициализирует _rs`ScaledUiAmount`_ для минта перед _rs`InitializeMint`_.                                           | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/instruction.rs#L24-L42) |
| _rs`ScaledUiAmountMintInstruction::UpdateMultiplier`_ | Инструкция, которая устанавливает новый множитель и временную метку Unix, когда он вступает в силу.                                     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/instruction.rs#L43-L66) |
| _rs`amount_to_ui_amount`_                             | Помощник, который преобразует сумму токенов в строку суммы UI, используя текущий множитель, десятичные знаки и временную метку.         | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L69-L84)         |
| _rs`try_ui_amount_into_amount`_                       | Помощник, который преобразует строку суммы UI обратно в сумму токенов, используя текущий множитель, десятичные знаки и временную метку. | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L86-L107)        |
| _rs`process_initialize`_                              | Процессор, который инициализирует _rs`ScaledUiAmountConfig`_ и сохраняет начальный множитель для неинициализированного минта.           | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/scaled_ui_amount/processor.rs#L37-L57)     |
| _rs`process_update_multiplier`_                       | Процессор, который проверяет полномочия, обновляет запланированный множитель и применяет его немедленно при необходимости.              | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/scaled_ui_amount/processor.rs#L59-L108)    |
| _rs`process_amount_to_ui_amount`_                     | Путь преобразования Token Program, который использует _rs`ScaledUiAmountConfig`_, когда у минта включено расширение.                    | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1548-L1571)                            |
| _rs`process_ui_amount_to_amount`_                     | Путь преобразования Token Program, который преобразует сумму UI обратно в сумму токенов для минтов с масштабируемой суммой UI.          | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1576-L1595)                            |
