...
description:
  Создание минта, токены которого могут быть выпущены, сожжены и закрыты, но не
  могут быть переведены между токен-аккаунтами.
...
Расширение минта _rs`NonTransferable`_ Token Extensions Program делает каждый
токен-аккаунт для этого минта непередаваемым. После выпуска токенов их владельцы
не могут переместить их на другой токен-аккаунт с помощью _rs`Transfer`_ или
_rs`TransferChecked`_.

Этот паттерн полезен для активов, которые должны оставаться привязанными к
одному кошельку.
...
<Callout type="info" title="Расширения токен-аккаунта">
  Когда токен-аккаунт инициализируется для минта с _rs`NonTransferable`_,
  токен-аккаунт инициализируется с _rs`NonTransferableAccount`_ и
  _rs`ImmutableOwner`_. Когда токен-аккаунт создается через [Associated Token
  Program](/docs/tokens/basics/create-token-account#how-to-create-an-associated-token-account),
  вычисляется требуемый размер аккаунта, и токен-аккаунт создается с необходимым
  размером и lamport, освобождающими от rent.
</Callout>
...
1. Рассчитайте размер mint account и rent, необходимый для минта и расширения
   _rs`NonTransferable`_.
2. Создайте mint account с помощью _rs`CreateAccount`_, инициализируйте
   _rs`NonTransferable`_ и инициализируйте минт с помощью _rs`InitializeMint`_.
3. Создайте токен-аккаунты для минта. _rs`NonTransferableAccount`_ и
   _rs`ImmutableOwner`_ автоматически включаются для токен-аккаунтов.
4. _rs`Transfer`_ и _rs`TransferChecked`_ завершаются с ошибкой
...
_rs`NonTransferable`_. Это размер, используемый в _rs`CreateAccount`_.
...
## !!steps Инициализировать NonTransferable

Инициализируйте расширение _rs`NonTransferable`_ на mint.
...
<Callout type="info" title="Порядок инструкций">
  _rs`InitializeNonTransferableMint`_ должна выполняться перед
  _rs`InitializeMint`_. _rs`CreateAccount`_, _rs`InitializeNonTransferableMint`_
  и _rs`InitializeMint`_ должны быть включены в одну и ту же транзакцию.
</Callout>
...
| ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| _rs`NonTransferable`_                          | Расширение mint, которое помечает токены от mint как непередаваемые.                                                                                                                                                                          | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/non_transferable.rs#L8-L13)  |
| _rs`NonTransferableAccount`_                   | Расширение token account, добавляемое к token accounts для непередаваемых mint.                                                                                                                                                               | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/non_transferable.rs#L15-L21) |
| _rs`ImmutableOwner`_                           | Расширение token account, которое предотвращает изменение владельца и требуется для непередаваемых token accounts.                                                                                                                            | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L8-L16)   |
| _rs`InitializeNonTransferableMint`_            | Инструкция, которая инициализирует расширение непередаваемости на уровне mint перед _rs`InitializeMint`_.                                                                                                                                     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L642-L653)              |
| _rs`process_initialize_non_transferable_mint`_ | Логика процессора, которая инициализирует расширение mint _rs`NonTransferable`_ на неинициализированном mint.                                                                                                                                 | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1646-L1655)                |
| _rs`get_required_init_account_extensions`_     | Используется для автоматического добавления расширений token account при инициализации token accounts на основе расширений, включённых на mint. Для mint _rs`NonTransferable`_ добавляет _rs`NonTransferableAccount`_ и _rs`ImmutableOwner`_. | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mod.rs#L1304-L1307)          |

### Typescript

Приведённый ниже пример `Kit` использует сгенерированные инструкции напрямую.
Устаревшие примеры с использованием `@solana/web3.js` включены для справки.
