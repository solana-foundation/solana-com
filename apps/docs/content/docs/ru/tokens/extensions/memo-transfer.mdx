---
title: Передача с памяткой
description:
  Требуйте, чтобы входящие переводы включали памятку, активировав расширение
  аккаунта MemoTransfer на аккаунте токена Token Extensions Program.
...
## Что такое передача с памяткой?

Расширение аккаунта _rs`MemoTransfer`_ Token Extensions Program требует наличия
инструкции памятки непосредственно перед любым входящим переводом на этот
аккаунт токена. Исходящие переводы с этого аккаунта токена не требуют памятки.

Расширение настраивается на целевом аккаунте токена, а не на минте. После
активации переводы без памятки завершаются ошибкой _rs`TokenError::NoMemo`_,
пока владелец аккаунта токена не отключит это требование.

## Как создать аккаунт токена с активированным MemoTransfer

Чтобы создать аккаунт токена с активированным _rs`MemoTransfer`_:

1. Создайте и инициализируйте минт.
2. Вычислите размер аккаунта токена и необходимую сумму rent для аккаунта токена
   с расширением _rs`MemoTransfer`_.
3. Создайте аккаунт токена и активируйте _rs`MemoTransfer`_ на этом аккаунте.
4. Инструкции _rs`Transfer`_ и _rs`TransferChecked`_ к этому аккаунту токена
   должны включать инструкцию памятки непосредственно перед инструкцией
   перевода.

<ScrollyCoding>

## !!steps Создайте и инициализируйте минт

Создайте и инициализируйте минт, который будет использовать аккаунт токена.
...
## !!steps Вычислите размер аккаунта токена

Вычислите размер аккаунта токена для базового аккаунта токена плюс расширение
_rs`MemoTransfer`_. Это размер, используемый в _rs`CreateAccount`_.
...
## !!steps Вычислите rent

Вычислите rent, используя размер аккаунта токена, необходимый для расширения
_rs`MemoTransfer`_.
...
Создайте токен-аккаунт с рассчитанным пространством и lamport, затем
инициализируйте его для минта.
...
## !!steps Включение MemoTransfer

Включите _rs`MemoTransfer`_ для токен-аккаунта.
...
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`MemoTransfer`_                              | Расширение аккаунта, которое хранит информацию о том, требуют ли входящие переводы memo.                            | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/mod.rs#L17-L22)         |
| _rs`RequiredMemoTransfersInstruction::Enable`_  | Инструкция, которая включает обязательное memo для переводов на токен-аккаунт.                                      | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L50-L71) |
| _rs`RequiredMemoTransfersInstruction::Disable`_ | Инструкция, которая отключает обязательное memo для переводов на токен-аккаунт.                                     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L74-L95) |
| _rs`process_toggle_required_memo_transfers`_    | Логика процессора, которая инициализирует или перезаписывает расширение _rs`MemoTransfer`_ после проверки владения. | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/memo_transfer/processor.rs#L20-L48)     |
...
`@solana/spl-memo` включены для справки.
