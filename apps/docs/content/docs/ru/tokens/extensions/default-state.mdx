...
description:
  Настройте минт Token Extensions Program таким образом, чтобы вновь создаваемые
  токен-аккаунты начинали работу в выбранном состоянии по умолчанию, например, в
  замороженном.
...
Расширение минта _rs`DefaultAccountState`_ Token Extensions Program
устанавливает начальное _rs`AccountState`_ для каждого нового токен-аккаунта,
создаваемого для этого минта.

Чаще всего это используется для того, чтобы новые токен-аккаунты начинали работу
в замороженном состоянии. Замороженные токен-аккаунты не могут быть использованы
до тех пор, пока полномочие заморозки минта не разморозит их.

## Как создать минт с состоянием аккаунта по умолчанию

Чтобы создать минт с состоянием аккаунта по умолчанию:

1. Рассчитайте размер mint account и rent, необходимые для минта и расширения
   _rs`DefaultAccountState`_.
2. Создайте mint account с помощью _rs`CreateAccount`_, инициализируйте
   _rs`DefaultAccountState`_ и инициализируйте минт с помощью
   _rs`InitializeMint`_.
3. Создайте токен-аккаунты для минта и убедитесь, что каждый новый токен-аккаунт
   начинает работу в текущем состоянии по умолчанию.
4. Используйте _rs`UpdateDefaultAccountState`_, чтобы изменить состояние, в
   котором будущие токен-аккаунты должны начинать работу.

<ScrollyCoding>

## !!steps Расчёт размера аккаунта

Рассчитайте размер mint account для базового минта плюс расширение
_rs`DefaultAccountState`_. Это размер, используемый в _rs`CreateAccount`_.
...
## !!steps Инициализация DefaultAccountState

Инициализируйте расширение _rs`DefaultAccountState`_ на минте.
...
## !!steps Создание token account

Создайте token account для минта. Поскольку текущее состояние по умолчанию —
_rs`Frozen`_, новый token account начинает работу в замороженном состоянии.
...
## !!steps Обновление состояния аккаунта по умолчанию

Используйте _rs`UpdateDefaultAccountState`_, чтобы изменить состояние по
...
<Callout type="info" title="Порядок инструкций">
  _rs`DefaultAccountStateInstruction::Initialize`_ должна идти перед
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`DefaultAccountStateInstruction::Initialize`_ и _rs`InitializeMint`_ должны
...
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`DefaultAccountState`_                        | Расширение минта, которое хранит состояние, наследуемое новыми token accounts.                                           | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/mod.rs#L16-L21)         |
| _rs`DefaultAccountStateInstruction::Initialize`_ | Инструкция, которая инициализирует состояние token account по умолчанию для минта перед _rs`InitializeMint`_.            | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L20-L36) |
| _rs`DefaultAccountStateInstruction::Update`_     | Инструкция, которая позволяет полномочиям на заморозку изменить состояние по умолчанию для будущих token accounts.       | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L37-L53) |
| _rs`process_initialize_default_account_state`_   | Логика процессора, которая записывает начальное состояние по умолчанию на неинициализированный минт.                     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L29-L40)     |
| _rs`process_update_default_account_state`_       | Логика процессора, которая проверяет полномочия на заморозку перед изменением сохранённого состояния минта по умолчанию. | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L43-L74)     |
