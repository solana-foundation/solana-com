...
устанавливает состояние по умолчанию для всех новых token account, создаваемых
для конкретного mint. Если эта расширенная функция включена, она автоматически
инициализирует каждый новый token account с заданным состоянием, например,
замороженным. Это позволяет создателям токенов задавать определённое поведение,
например, требовать, чтобы все новые аккаунты по умолчанию были заморожены до их
явной разморозки.
