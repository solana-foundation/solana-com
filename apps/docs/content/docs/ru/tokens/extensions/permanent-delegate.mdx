...
description:
  Создайте mint Token Extensions Program с постоянным делегатом, который может
  переводить или сжигать токены с любого аккаунта для этого mint.
...
## Что такое постоянный делегат?

Расширение mint _rs`PermanentDelegate`_ Token Extensions Program назначает
делегированные полномочия самому mint.

Постоянный делегат — это полномочие на уровне mint, которое может авторизовать
переводы и сжигание для любого token account этого mint. Он действует как
глобальный делегат для всех token account данного mint, и владельцы token
account не могут отозвать постоянного делегата от своих token account.

Полномочие mint также может позже заменить постоянного делегата с помощью
_rs`SetAuthority`_, используя _rs`AuthorityType::PermanentDelegate`_.

## Как создать и использовать постоянного делегата

Чтобы создать и использовать постоянного делегата:

1. Рассчитайте размер mint account и rent, необходимые для mint и расширения
   _rs`PermanentDelegate`_.
2. Создайте mint account с помощью _rs`CreateAccount`_, инициализируйте
   _rs`PermanentDelegate`_ и инициализируйте mint с помощью
   _rs`InitializeMint`_.
3. Отправьте _rs`TransferChecked`_ или _rs`BurnChecked`_, подписанные постоянным
   делегатом. Token Program обрабатывает эти инструкции так же, как если бы их
   подписал владелец token account.

<ScrollyCoding>

## !!steps Рассчитать размер аккаунта

Рассчитайте размер mint account для базового mint плюс расширение
_rs`PermanentDelegate`_. Это размер, используемый в _rs`CreateAccount`_.
...
Рассчитайте rent, используя размер, необходимый для mint плюс расширение
_rs`PermanentDelegate`_.
...
## !!steps Инициализация PermanentDelegate

Инициализируйте расширение _rs`PermanentDelegate`_ для минта.
...
## !!steps Создание token accounts и минт токенов

Создайте token accounts для владельца и получателя, затем заминтьте токены на
token account владельца.
...
## !!steps Перевод с постоянным делегатом

Переведите токены с помощью _rs`TransferChecked`_, подписанного постоянным
делегатом.
...
<Callout type="info" title="Порядок инструкций">
  _rs`InitializePermanentDelegate`_ должна быть выполнена перед
  _rs`InitializeMint`_. _rs`CreateAccount`_, _rs`InitializePermanentDelegate`_ и
  _rs`InitializeMint`_ должны быть включены в одну транзакцию.
</Callout>
...
| ------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermanentDelegate`_                     | Расширение минта, которое хранит делегата, авторизованного для каждого token account этого минта.                            | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/permanent_delegate.rs#L15-L21) |
| _rs`InitializePermanentDelegate`_           | Инструкция, которая инициализирует постоянного делегата перед _rs`InitializeMint`_.                                          | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1952-L1967)              |
| _rs`SetAuthority`_                          | Базовая инструкция токена, используемая с _rs`AuthorityType::PermanentDelegate`_ для смены делегата.                         | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1488-L1520)              |
| _rs`AuthorityType::PermanentDelegate`_      | Идентификатор полномочий, используемый с _rs`SetAuthority`_ для смены постоянного делегата минта.                            | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1138-L1139)              |
| _rs`process_initialize_permanent_delegate`_ | Логика процессора, которая записывает расширение постоянного делегата в минт.                                                | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1542-L1557)                  |
| _rs`process_set_authority`_                 | Логика процессора, которая проверяет и меняет постоянного делегата при использовании _rs`AuthorityType::PermanentDelegate`_. | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L849-L860)                    |

### Typescript

Пример `Kit` ниже использует сгенерированные инструкции напрямую. Устаревшие
примеры с использованием `@solana/web3.js` и `@solana/spl-token` приведены для
справки.
