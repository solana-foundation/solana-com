...
назначает делегата с полномочиями над всеми token account для данного mint. В
отличие от обычных делегатов, которых владелец token account может отозвать,
постоянного делегата нельзя изменить или удалить владельцем token account.
Постоянный делегат может переводить или сжигать токены в любом token account
этого mint. Роль постоянного делегата может быть передана новому аккаунту
текущим постоянным делегатом.
