...
description:
  Создайте минт с комиссиями за перевод, переместите удержанные комиссии из
  аккаунтов токенов в минт и выведите их в аккаунт токена получателя комиссий.
...
## Что такое комиссии за перевод?

Расширение минта _rs`TransferFeeConfig`_ Token Extensions Program применяет
комиссию к каждому переводу для этого минта.

Во время перевода:

- Переведённая сумма уменьшается на настроенную комиссию
- Удержанная комиссия отслеживается в целевом аккаунте токена
- Удержанные комиссии могут быть выведены напрямую из аккаунтов токенов или
  собраны в минт
- `withdraw_withheld_authority` может переместить собранные комиссии в аккаунт
  токена получателя комиссий

<Callout type="info" title="Расширения аккаунта токена">
  Когда аккаунт токена инициализируется для минта с _rs`TransferFeeConfig`_,
  аккаунт токена инициализируется с _rs`TransferFeeAmount`_. Когда аккаунт
  токена создаётся через [Associated Token
  Program](/docs/tokens/basics/create-token-account#how-to-create-an-associated-token-account),
  вычисляется необходимый размер аккаунта, и аккаунт токена создаётся с
  требуемым размером и необходимыми для освобождения от rent ламортами.
</Callout>

## Как создать минт с комиссией за перевод

Чтобы создать минт с комиссией за перевод:

1. Вычислите размер аккаунта минта и необходимый rent для минта и расширения
   _rs`TransferFeeConfig`_.
2. Создайте аккаунт минта с помощью _rs`CreateAccount`_, инициализируйте
   _rs`TransferFeeConfig`_ и инициализируйте минт с помощью
   _rs`InitializeMint`_.
3. Создайте аккаунты токенов для минта. _rs`TransferFeeAmount`_ автоматически
   включается для аккаунтов токенов.
4. Используйте _rs`TransferCheckedWithFee`_ для перевода токенов и проверки
   ожидаемой комиссии. _rs`TransferChecked`_ также работает и автоматически
   удерживает настроенную комиссию на целевом аккаунте токена.
5. Используйте _rs`WithdrawWithheldTokensFromAccounts`_ для перемещения
   удержанных комиссий напрямую из аккаунтов токенов в аккаунт токена получателя
   комиссий, подписанный `withdraw_withheld_authority` минта.
6. Используйте не требующую разрешений _rs`HarvestWithheldTokensToMint`_ для
   перемещения удержанных комиссий из аккаунтов токенов в аккаунт минта, затем
   используйте _rs`WithdrawWithheldTokensFromMint`_ для вывода удержанных
   комиссий аккаунта минта в аккаунт токена получателя комиссий, подписанный
   `withdraw_withheld_authority` минта.
7. Используйте _rs`SetTransferFee`_ для обновления конфигурации следующей
   комиссии за перевод, которая вступит в силу через две эпохи.

<ScrollyCoding>

## !!steps Рассчитать размер аккаунта

Рассчитайте размер mint account для базового минта плюс расширение
_rs`TransferFeeConfig`_. Это размер, используемый в _rs`CreateAccount`_.
...
## !!steps Инициализировать TransferFeeConfig

Инициализируйте расширение _rs`TransferFeeConfig`_ на минте.
...
## !!steps Создать token account и выпустить токены

Создайте token account для источника, получателей и получателя комиссии, затем
выпустите токены на token account источника.
...
## !!steps Перевод с TransferCheckedWithFee

Переведите токены с помощью _rs`TransferCheckedWithFee`_ и проверьте ожидаемую
комиссию.
...
## !!steps Перевод с TransferChecked

Переведите токены с помощью _rs`TransferChecked`_. Настроенная комиссия
по-прежнему удерживается на целевом token account.
...
## !!steps Вывод удержанных комиссий из token accounts

Используйте _rs`WithdrawWithheldTokensFromAccounts`_ для перемещения удержанных
комиссий непосредственно из token accounts на token account получателя комиссии.
...
## !!steps Сбор удержанных комиссий в mint

Используйте _rs`HarvestWithheldTokensToMint`_ для перемещения удержанных
комиссий из token accounts в аккумулятор удержанных средств mint account.
...
## !!steps Вывод удержанных комиссий из mint

Используйте _rs`WithdrawWithheldTokensFromMint`_ для перемещения собранных
комиссий из mint account на token account получателя комиссии.
...
## !!steps Обновление следующей комиссии за перевод

Используйте _rs`SetTransferFee`_ для обновления конфигурации следующей комиссии
за перевод.
...
<Callout type="info" title="Порядок инструкций">
  _rs`InitializeTransferFeeConfig`_ должна идти перед _rs`InitializeMint`_.
  _rs`CreateAccount`_, _rs`InitializeTransferFeeConfig`_ и _rs`InitializeMint`_
  должны быть включены в одну транзакцию.
</Callout>
...
| ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`TransferFeeConfig`_                                          | Расширение mint, которое хранит полномочия на сборы, накопленную удержанную сумму и старую и новую конфигурации комиссий за перевод.                                                                                     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L128-L145)         |
| _rs`TransferFee`_                                                | Структура конфигурации сборов, хранящаяся в _rs`TransferFeeConfig`_, которая определяет epoch вступления в силу, максимальный сбор и сбор в базисных пунктах.                                                            | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L32-L40)           |
| _rs`TransferFeeAmount`_                                          | Расширение token account, которое хранит сумму, удержанную во время переводов для аккаунта токенов.                                                                                                                      | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/mod.rs#L169-L177)         |
| _rs`TransferFeeInstruction::InitializeTransferFeeConfig`_        | Инструкция, которая инициализирует настройки комиссии за перевод перед _rs`InitializeMint`_.                                                                                                                             | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L24-L48)   |
| _rs`TransferFeeInstruction::TransferCheckedWithFee`_             | Инструкция перевода, которая проверяет соответствие предоставленной вызывающей стороной комиссии текущей конфигурации комиссии за перевод mint.                                                                          | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L49-L80)   |
| _rs`TransferFeeInstruction::WithdrawWithheldTokensFromAccounts`_ | Инструкция, которая перемещает удержанные сборы напрямую из аккаунтов токенов в аккаунт токенов получателя сборов, подписанная `withdraw_withheld_authority` mint.                                                       | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L99-L122)  |
| _rs`TransferFeeInstruction::HarvestWithheldTokensToMint`_        | Инструкция без требования разрешений, которая перемещает удержанные сборы из аккаунтов токенов в накопитель удержанных сборов mint account.                                                                              | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L123-L134) |
| _rs`TransferFeeInstruction::WithdrawWithheldTokensFromMint`_     | Инструкция, которая выводит удержанные сборы mint account в аккаунт токенов получателя сборов, подписанная `withdraw_withheld_authority` mint.                                                                           | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L81-L98)   |
| _rs`TransferFeeInstruction::SetTransferFee`_                     | Инструкция, которая обновляет новую конфигурацию комиссии за перевод, вступающую в силу через два epoch.                                                                                                                 | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/transfer_fee/instruction.rs#L135-L154) |
| _rs`process_initialize_transfer_fee_config`_                     | Логика процессора, которая инициализирует расширение _rs`TransferFeeConfig`_ на неинициализированном mint и устанавливает старую и новую конфигурации комиссии за перевод в начальную комиссию.                          | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L26-L58)       |
| _rs`process_withdraw_withheld_tokens_from_accounts`_             | Логика процессора, которая проверяет `withdraw_withheld_authority` и перемещает удержанные сборы из аккаунтов токенов в целевой аккаунт токенов.                                                                         | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L208-L278)     |
| _rs`process_harvest_withheld_tokens_to_mint`_                    | Логика процессора, которая собирает удержанные сборы из аккаунтов токенов в накопитель удержанных сборов mint account.                                                                                                   | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L181-L206)     |
| _rs`process_withdraw_withheld_tokens_from_mint`_                 | Логика процессора, которая проверяет `withdraw_withheld_authority` и перемещает удержанную сумму mint account в целевой аккаунт токенов.                                                                                 | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L114-L158)     |
| _rs`process_set_transfer_fee`_                                   | Логика процессора, которая проверяет полномочия конфигурации комиссии за перевод и обновляет новую конфигурацию комиссии за перевод для вступления в силу через два epoch.                                               | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/transfer_fee/processor.rs#L61-L112)      |
| _rs`get_required_init_account_extensions`_                       | Используется для автоматического добавления расширений token account при инициализации аккаунтов токенов на основе расширений, включенных на mint. Для mint с _rs`TransferFeeConfig`_ добавляет _rs`TransferFeeAmount`_. | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mod.rs#L1301-L1303)                    |

### Typescript

В приведенном ниже примере `Kit` используются явные инструкции Token-2022.
