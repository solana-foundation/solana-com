...
description:
  Создайте минт, который хранит метаданные непосредственно в аккаунте минта с использованием расширений MetadataPointer и TokenMetadata, а затем обновляйте, удаляйте и блокируйте эти метаданные через интерфейс token-metadata.
...
примеры с использованием `@solana/web3.js`, `@solana/spl-token` и
`@solana/spl-token-metadata` приведены для справки. В устаревших примерах
`maxSupportedTransactionVersion` ограничен значением `0`, поскольку web3.js v1 не может десериализовать
[транзакции v1](/docs/core/transactions/versioned-transactions#v1-message-format).
