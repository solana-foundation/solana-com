...
description:
  Узнайте, как создать mint token с включёнными Confidential Balances.
---

## Как создать mint с Confidential Balances

Confidential Balances обеспечивает конфиденциальность переводов токенов, добавляя дополнительное состояние в mint account. В этом разделе объясняется, как создать mint токена с включённым необходимым расширением.

Следующая диаграмма показывает шаги, необходимые для создания mint с расширением Confidential Balances:
...
### Состояние Mint с Confidential Balances
...
- **authority**: Аккаунт, имеющий право изменять настройки Confidential Balances для mint и одобрять новые конфиденциальные аккаунты, если автоматическое одобрение отключено.

- **auto_approve_new_accounts**: Если установлено значение true, пользователи могут создавать token accounts с включёнными Confidential Balances по умолчанию. Если false, authority должен одобрить каждый новый token account перед тем, как он сможет использоваться для конфиденциальных переводов.
...
Для создания mint с включёнными Confidential Balances требуется три инструкции в одной транзакции:

1. **Создание mint account**: Вызовите инструкцию _rs`CreateAccount`_ System
   Program для создания mint account.

2. **Инициализация Confidential Balances**: Вызовите инструкцию [ConfidentialTransferInstruction::InitializeMint](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L48) Token Extensions Program для настройки состояния _rs`ConfidentialTransferMint`_ для mint.
...
В следующем примере кода показано, как создать mint с включёнными Confidential Balances.

Confidential Balances зависит от программы ZK ElGamal Proof, которая включена в mainnet и devnet. Стандартный `solana-test-validator` не поддерживает её, однако локальный validator с форком mainnet, например [Surfpool](https://surfpool.run), поддерживает.
Запустите пример на одной из этих сетей (в коде используется devnet) с пополненным аккаунтом плательщика и замените placeholder-адреса mint и аккаунтов на свои.
