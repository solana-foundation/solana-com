...
description:
  Узнайте, как создать token account с включёнными Конфиденциальными Балансами.
---

## Как создать token account с Конфиденциальными Балансами

Конфиденциальные Балансы обеспечивают приватные переводы токенов, добавляя дополнительное состояние в
token account. В этом разделе объясняется, как создать token account с
включённым необходимым расширением.

На следующей диаграмме показаны шаги по созданию token account с
включёнными Конфиденциальными Балансами:
...
### Состояние Token Account для Конфиденциальных Балансов
...
_rs`ConfidentialTransferAccount`_ содержит несколько полей, управляющих
Конфиденциальными Балансами:

- **approved**: Статус одобрения аккаунта для Конфиденциальных Балансов. Если в
  mint account параметр `auto_approve_new_accounts` установлен в значение `true`, все
  token accounts автоматически одобряются для Конфиденциальных Балансов.
...
Создание и настройка token account для Конфиденциальных Балансов выполняется с помощью
следующих инструкций, которые умещаются в одну транзакцию:
...
4. **Настройка Конфиденциальных Балансов**: Вызовите инструкцию
   ...
   _rs`ProofLocation::ContextStateAccount`_, для инициализации
   состояния _rs`ConfidentialTransferAccount`_.

<Callout type="info">

**Только владелец token account может включить Конфиденциальные Балансы на token
account**.

Инструкция _rs`ConfigureAccount`_ требует генерации на стороне клиента
ключей шифрования и доказательства, которое может быть создано только владельцем token account.

Доказательство валидности pubkey подтверждает, что публичный ключ ElGamal аккаунта
является действительным. Оно генерируется с помощью _rs`build_pubkey_validity_proof_data`_, верифицируется
в сети программой ZK ElGamal Proof в аккаунт состояния контекста, а затем
ссылается из _rs`ConfigureAccount`_ через
_rs`ProofLocation::ContextStateAccount`_, так что никакие байты доказательства не передаются в самой
инструкции токена. Подробности реализации см. в:
...
Следующий код создаёт associated token account и настраивает его для использования
Конфиденциальных Балансов с существующим совместимым минтом.

Конфиденциальные Балансы зависят от программы ZK ElGamal Proof, которая включена
в mainnet и devnet. Стандартный `solana-test-validator` не включает её, однако
локальный валидатор с форком mainnet, например [Surfpool](https://surfpool.run), — включает.
Запустите пример на одном из них (в коде используется devnet) с пополненным аккаунтом плательщика
и замените заполнитель минта на минт, созданный согласно
