...
Структура _rs`ConfidentialTransferAccount`_ содержит несколько полей для
управления конфиденциальными переводами:
...
- **approved**: Статус одобрения аккаунта для конфиденциальных переводов. Если
  ...
  все token accounts автоматически одобряются для конфиденциальных переводов.
...
- **maximum_pending_balance_credit_counter**: Лимит количества ожидающих
  зачислений до необходимости выполнения инструкции _rs`ApplyPendingBalance`_
  для перевода ожидаемого баланса в доступный.
...
- **actual_pending_balance_credit_counter**: Значение
  `pending_balance_credit_counter` на token account в момент последней обработки
...
Все депозиты и суммы переводов изначально добавляются к ожидаемому балансу.
Владельцы token account должны использовать инструкцию _rs`ApplyPendingBalance`_
для перевода ожидаемого баланса в доступный. Входящие переводы или депозиты не
...
Конфиденциальный ожидаемый баланс разделён на `pending_balance_lo` и
`pending_balance_hi`, потому что для больших чисел расшифровка ElGamal требует
больше вычислений. Реализацию арифметики с шифротекстом можно найти
...
1. Клиент получает текущие значения ожидаемого и доступного балансов, шифрует их
   сумму и предоставляет `decryptable_available_balance`, зашифрованный с
   помощью AES-ключа владельца token account.

2. Ожидаемые и фактические счетчики ожидающих зачислений отслеживают изменения
   значения счётчика между созданием и обработкой инструкции
   _rs`ApplyPendingBalance`_:
   - `expected_pending_balance_credit_counter`: Значение
     `pending_balance_credit_counter` при создании клиентом инструкции
     _rs`ApplyPendingBalance`_
   - `actual_pending_balance_credit_counter`: Значение
     `pending_balance_credit_counter` на token account в момент обработки
     инструкции _rs`ApplyPendingBalance`_
...
При получении состояния token account для чтения `decryptable_available_balance`
разные значения ожидаемых и фактических счетчиков требуют от клиента поиска
последних инструкций по депозиту/переводу, соответствующих разнице счетчиков,
чтобы рассчитать правильный баланс.

**Процесс сверки баланса**

Если ожидаемый и фактический счетчики ожидаемого баланса различаются, выполните
следующие шаги, чтобы сверить `decryptable_available_balance`:

1. Начните с `decryptable_available_balance` из token account
2. Получите самые последние транзакции, включая инструкции по депозиту и
   переводу до разницы счетчиков (фактический - ожидаемый):
   - Добавьте публичные суммы из инструкций по депозиту
   - Расшифруйте и добавьте суммы назначения из инструкций по переводу
...
2. **Реализуйте перераспределение пространства аккаунта**: Вызовите инструкцию
   Token Extension Program _rs`TokenInstruction::Reallocate`_ для добавления
   пространства для состояния _rs`ConfidentialTransferAccount`_.

3. **Настройте конфиденциальные переводы**: Вызовите инструкцию Token Extension
   Program
...
доказательства на стороне клиента, которые может сгенерировать только владелец
token account.

_rs`PubkeyValidityProofData`_ создает доказательство, подтверждающее, что ключ
ElGamal является действительным. Подробности реализации см. здесь:
