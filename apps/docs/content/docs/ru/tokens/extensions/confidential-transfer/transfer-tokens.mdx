...
<Callout type="warn">
  ...
  аудита безопасности. Это означает, что расширение для конфиденциальных
  переводов сейчас недоступно. Хотя концепции остаются актуальными, примеры кода
  не будут работать.
</Callout>

## Как конфиденциально перевести токены с одного токенного аккаунта на другой

Чтобы конфиденциально перевести токены с одного токенного аккаунта на другой, и
у отправителя, и у получателя должны быть токенные аккаунты, настроенные с
состоянием _rs`ConfidentialTransferAccount`_ и одобренные для конфиденциальных
переводов. У токенного аккаунта отправителя также должен быть доступный
конфиденциальный баланс для перевода.

Чтобы перевести токены конфиденциально:
...
1. Создайте
   ...
   **Доказательство равенства
   ...
   Проверяет, что новое зашифрованное значение доступного баланса после перевода
   соответствует своему
   [коммитменту по Педерсену](https://en.wikipedia.org/wiki/Commitment_scheme),
   гарантируя, что новый доступный баланс исходного аккаунта вычислен корректно
   ...
   **Доказательство корректности шифротекста
   ...
   Проверяет, что шифротексты суммы перевода корректно сгенерированы для всех
   трёх сторон (отправитель, получатель и аудитор), гарантируя, что сумма
   перевода правильно зашифрована под публичным ключом каждой стороны.
   ...
   **Доказательство диапазона
   ...
   Проверяет, что новый доступный баланс и сумма перевода (разделённая на
   младшие/старшие биты) неотрицательны и находятся в заданном диапазоне.

2. Для каждого доказательства:
   - Вызовите программу доказательств ZK ElGamal для проверки данных
     доказательства.
   - Сохраните метаданные, относящиеся к доказательству, в аккаунте "context
     state" для использования в других инструкциях.
...
3. Вызовите инструкцию
   ...
   передав аккаунты состояния контекста доказательств.

4. Закройте аккаунты состояния proof context, чтобы вернуть SOL, использованный
   для их создания.

На следующей схеме показаны шаги перевода токенов с токен-аккаунта отправителя
на токен-аккаунт получателя.
...
Чтобы конфиденциально перевести токены с одного токен-аккаунта на другой,
необходимо:

- Сгенерировать на стороне клиента доказательство равенства, доказательство
  корректности шифротекста и доказательство диапазона
- Вызвать программу доказательств Zk ElGamal для проверки доказательств и
  инициализации аккаунтов "context state"
- Вызвать инструкцию
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604),
  предоставив три proof-аккаунта.
- Закрыть три proof-аккаунта, чтобы вернуть rent.
...
Следующий пример демонстрирует, как конфиденциально перевести токены между
токен-аккаунтами.

Чтобы запустить пример, запустите локальный валидатор с программой Token
Extension, клонированной с mainnet, используя следующую команду. Для запуска
локального валидатора у вас должен быть установлен Solana CLI.
