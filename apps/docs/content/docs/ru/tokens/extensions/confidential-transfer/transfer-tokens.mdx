...
description:
  Узнайте, как приватно переводить токены с одного токенного аккаунта на другой.
---

## Как конфиденциально переводить токены с одного token account на другой

Для конфиденциального перевода токенов из одного token account в другой и отправитель, и получатель должны иметь token account, настроенные с состоянием _rs`ConfidentialTransferAccount`_, и получить разрешение на использование конфиденциальных балансов. На token account отправителя также должен быть доступный конфиденциальный баланс для перевода.
...
1. Создайте
   ...
   **Доказательство равенства
   ...
   Проверяет, что новый шифртекст доступного баланса после перевода
   соответствует соответствующему
   [обязательству Педерсена](https://en.wikipedia.org/wiki/Commitment_scheme),
   гарантируя, что новый доступный баланс аккаунта источника корректно вычислен
   как `new_balance = current_balance - transfer_amount`.

   **Доказательство корректности шифртекста
   ...
   Проверяет, что шифртексты суммы перевода корректно сформированы для всех трёх
   сторон (источника, получателя и аудитора), гарантируя, что сумма перевода
   правильно зашифрована под открытым ключом каждой стороны.
   ...
   **Доказательство диапазона
   ...
   Проверяет, что новый доступный баланс и сумма перевода (разбитая на
   младшие/старшие биты) неотрицательны и находятся в заданном диапазоне.

2. Для каждого доказательства:
   - Вызовите программу доказательств ZK ElGamal для верификации данных
     доказательства.
   - Сохраните специфические метаданные доказательства в аккаунте "контекстного
     состояния" доказательства для использования в других инструкциях.
...
3. Вызовите инструкцию
   ...
   предоставив аккаунты контекстного состояния доказательств.

4. Закройте аккаунты контекстного состояния доказательств, чтобы вернуть SOL,
   использованный для их создания.

Следующая диаграмма показывает шаги, задействованные при переводе токенов с
token account отправителя на token account получателя.
...
Чтобы конфиденциально переводить токены с одного token account на другой,
необходимо:

- Сформировать на стороне клиента доказательство равенства, доказательство
  корректности шифртекста и доказательство диапазона
- Вызвать программу доказательств Zk ElGamal для верификации доказательств и
  инициализации аккаунтов "контекстного состояния"
- Вызвать инструкцию
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604),
  предоставив три аккаунта доказательств.
...
В следующем примере токены конфиденциально переводятся из одного аккаунта в другой. Оба аккаунта должны быть заранее настроены для работы с конфиденциальными балансами, а у отправителя должен быть доступный конфиденциальный баланс.

Конфиденциальные балансы зависят от программы ZK ElGamal Proof, которая включена в mainnet и devnet. Стандартный `solana-test-validator` не поддерживает её, однако локальный validator с форком mainnet, например [Surfpool](https://surfpool.run), поддерживает.
Запустите пример на одной из таких сред (в коде используется devnet) с пополненным аккаунтом плательщика и замените заполнители своим mint, а также аккаунтами отправителя и получателя.
