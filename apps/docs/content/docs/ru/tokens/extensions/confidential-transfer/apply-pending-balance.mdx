...
однако локальный validator с форком mainnet, например
[Surfpool](https://surfpool.run), поддерживает её. Запустите пример с одним из
них (в коде используется devnet), используя аккаунт с достаточным балансом, и
замените placeholder-адреса минта и аккаунта на собственные.
