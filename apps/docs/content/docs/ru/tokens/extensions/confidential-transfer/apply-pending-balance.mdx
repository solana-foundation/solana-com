...
1. **Конфиденциальный ожидаемый баланс**: Сначала токены "депонируются" с
   публичного баланса на "ожидаемый" конфиденциальный баланс.
...
В этом разделе рассматривается второй этап: применение ожидаемого баланса к
доступному балансу.

Когда токены "депонируются" с публичного баланса или когда токены
конфиденциально переводятся с одного token account на другой, токены изначально
добавляются к конфиденциальному ожидаемому балансу. Прежде чем токены можно
будет использовать для конфиденциальных переводов, ожидаемый баланс должен быть
"применён" к доступному балансу.
...
На следующей схеме показаны шаги применения ожидаемого баланса к доступному
балансу:
...
Чтобы преобразовать ожидаемый баланс в доступный, вызовите инструкцию
...
<Callout type="info">
  На момент написания конфиденциальные переводы не включены по умолчанию в
  локальном validator. Для запуска примера необходимо клонировать программу
