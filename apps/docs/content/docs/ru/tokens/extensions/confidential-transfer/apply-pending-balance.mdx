...
description: Узнайте, как применить ожидаемый баланс, чтобы сделать средства доступными.
---

## Как применить отложенный баланс к доступному балансу

Прежде чем токены можно будет передавать конфиденциально, публичный баланс
токенов необходимо преобразовать в конфиденциальный баланс. Это преобразование
происходит в два этапа:

1. **Конфиденциальный отложенный баланс**: Изначально токены «вносятся» из
   публичного баланса в «отложенный» конфиденциальный баланс.
2. **Конфиденциальный доступный баланс**: Затем отложенный баланс «применяется»
   к доступному балансу, делая токены доступными для конфиденциальных переводов.

В этом разделе объясняется второй этап: применение отложенного баланса к
доступному балансу.

Когда токены «вносятся» из публичного баланса или конфиденциально переводятся с
одного token account на другой, токены изначально добавляются в конфиденциальный
отложенный баланс. Прежде чем токены можно будет использовать для
конфиденциальных переводов, отложенный баланс должен быть «применён» к
доступному балансу.
...
На следующей диаграмме показаны шаги, выполняемые при применении отложенного
баланса к доступному балансу:
...
Чтобы преобразовать отложенный баланс в доступный баланс, используйте инструкцию
...
Крейт `spl_token_client` предоставляет метод
`confidential_transfer_apply_pending_balance`, который формирует и отправляет
транзакцию с инструкцией `ApplyPendingBalance`, как показано в примере ниже.

## Пример кода

Следующий пример демонстрирует, как применить конфиденциальный отложенный баланс
к конфиденциальному доступному балансу.

Confidential Balances зависит от программы ZK ElGamal Proof, которая включена
в mainnet и devnet. Стандартный `solana-test-validator` не поддерживает её, однако
локальный validator с форком mainnet, например [Surfpool](https://surfpool.run), поддерживает.
Запустите пример на одной из этих сетей (в коде используется devnet) с пополненным аккаунтом плательщика
и замените заполнители адресов mint и аккаунта на свои собственные.
