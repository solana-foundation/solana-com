...
<Callout type="warn">
  Программа ZK ElGamal временно отключена в mainnet и devnet на время проведения
  аудита безопасности. Это означает, что расширение конфиденциальных переводов
  сейчас недоступно. Хотя концепции остаются актуальными, примеры кода не будут
  работать.
</Callout>
...
раскрывая сумму перевода. Это полезно для транзакций с сохранением приватности.
Только суммы переводов и балансы токенов остаются приватными. Адреса
токен-аккаунтов остаются публичными.
...
- [Краткое руководство](https://www.solana-program.com/docs/confidential-balances#setup)
  — настройка и базовые команды CLI
- [Cookbook по конфиденциальным балансам](https://github.com/solana-developers/Confidential-Balances-Sample)
  — примеры кода по использованию расширения Confidential Transfer
...
в программу расширения токенов, что позволяет переводить токены между аккаунтами
...
Базовый процесс конфиденциальных переводов токенов выглядит так:

1. Создайте mint account с расширением конфиденциальных переводов.
2. Создайте токен-аккаунты с этим расширением для отправителя и получателя.
...
Подробнее о шагах процесса конфиденциального перевода токенов смотрите на
соответствующих страницах:

<Cards>
  <Card
    title="Создать аккаунт mint"
    href="/docs/tokens/extensions/confidential-transfer/create-mint"
  >
    Как создать аккаунт mint с расширением конфиденциального перевода
  </Card>
  <Card
    title="Создать токен-аккаунт"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Как настроить токен-аккаунт с расширением конфиденциального перевода
  </Card>
  <Card
  ...
  <Card
    title="Применить ожидающий баланс"
  ...
  <Card
    title="Вывести токены"
    href="/docs/tokens/extensions/confidential-transfer/withdraw-tokens"
  >
    Как вывести токены с конфиденциального доступного баланса
  </Card>
  <Card
...
конфиденциального перевода токенов:
...
Пример на Rust ниже демонстрирует шаги конфиденциального перевода токенов,
соответствующие приведённой выше диаграмме. Для удобства шаги разделены на
вспомогательные функции.

Чтобы запустить пример, запустите локальный валидатор с программой Token
Extension, клонированной с mainnet, используя следующую команду. Для
использования локального валидатора у вас должен быть установлен Solana CLI.
...
<Callout type="info">
  ...
  локальном валидаторе. Для запуска примера необходимо клонировать программу
  Token Extension с mainnet.
</Callout>
...
## Инструкции по конфиденциальному переводу

Полный список инструкций расширения конфиденциального перевода
[instructions](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
...
| _rs`UpdateMint`_                    | Обновляет параметры конфиденциального перевода для mint.                                                                                                                         |
| _rs`ConfigureAccount`_              | Настраивает токен-аккаунт для конфиденциальных переводов.                                                                                                                        |
| _rs`ApproveAccount`_                | Одобряет токен-аккаунт для конфиденциальных переводов, если mint требует одобрения для новых токен-аккаунтов.                                                                    |
| _rs`EmptyAccount`_                  | Очищает ожидающий и доступный конфиденциальные балансы для возможности закрытия токен-аккаунта.                                                                                  |
...
| _rs`Transfer`_                      | Конфиденциально переводит токены между токен-аккаунтами.                                                                                                                         |
| _rs`ApplyPendingBalance`_           | Конвертирует ожидающий баланс в доступный после депозита или перевода.                                                                                                           |
| _rs`EnableConfidentialCredits`_     | Позволяет токен-аккаунту принимать конфиденциальные переводы токенов.                                                                                                            |
| _rs`DisableConfidentialCredits`_    | Блокирует входящие конфиденциальные переводы, но разрешает публичные переводы.                                                                                                   |
...
| _rs`TransferWithFee`_               | Конфиденциально переводит токены между токен-аккаунтами с комиссией.                                                                                                             |
