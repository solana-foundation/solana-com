...
Чтобы вывести токены из конфиденциального доступного баланса на публичный
баланс:
...
1. Создайте
   ...
   **Доказательство равенства
   ...
   Проверяет, что зашифрованный текст оставшегося доступного баланса после
   вывода соответствует соответствующему
   [обязательству Педерсена](https://en.wikipedia.org/wiki/Commitment_scheme),
   гарантируя, что новый доступный баланс аккаунта корректно вычислен как
   ...
   **Доказательство диапазона
   ...
   Проверяет, что оставшийся доступный баланс после вывода является
   неотрицательным и находится в заданном диапазоне.

2. Для каждого доказательства:
   - Вызовите программу ZK ElGamal для проверки данных доказательства.
   - Сохраните специфичные метаданные доказательства в аккаунте «контекстного
     состояния» для использования в других инструкциях.
...
3. Вызовите инструкцию
   ...
   передав два аккаунта доказательств.

4. Закройте аккаунты доказательств, чтобы вернуть SOL, использованный для их
   создания.

Следующая диаграмма показывает шаги, связанные с выводом токенов из
конфиденциального доступного баланса на публичный баланс:
...
- Сгенерировать доказательство равенства и доказательство диапазона на стороне
  клиента
- Вызвать программу ZK ElGamal для проверки доказательств и инициализации
  аккаунтов «контекстного состояния»
- Вызвать инструкцию
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493),
  передав два аккаунта доказательств.
...
В следующем примере токены выводятся из конфиденциального доступного баланса
обратно в публичный баланс. Аккаунт должен быть уже настроен для
Конфиденциальных балансов и иметь доступный конфиденциальный баланс.

Конфиденциальные балансы зависят от программы ZK ElGamal Proof, которая включена
в mainnet и devnet. Стандартный `solana-test-validator` не включает её, но
локальный validator с форком mainnet, например [Surfpool](https://surfpool.run), поддерживает её.
Запустите пример на одной из этих сетей (в коде используется devnet) с финансируемым плательщиком
и замените заполнители на ваш mint и token account.
