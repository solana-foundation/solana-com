...
Чтобы вывести токены из доступного конфиденциального баланса на публичный
баланс:
...
1. Создайте
   ...
   **Доказательство равенства
   ...
   Проверяет, что шифротекст оставшегося доступного баланса после вывода
   ...
   **Доказательство диапазона
   ...
   Проверяет, что оставшийся доступный баланс после вывода неотрицателен и
...
3. Вызовите инструкцию
   ...
   предоставив два аккаунта с доказательствами.

4. Закройте аккаунты с доказательствами, чтобы вернуть SOL, использованный для
   их создания.

На следующей схеме показаны шаги по выводу токенов из доступного
конфиденциального баланса на публичный баланс:
...
- Сгенерировать на стороне клиента доказательство равенства и доказательство
  диапазона
...
- Вызвать инструкцию
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493),
  предоставив два аккаунта с доказательствами.
- Закрыть два аккаунта с доказательствами, чтобы вернуть rent.

Пакет `spl_token_client` предоставляет следующие методы:
...
локального validator у вас должен быть установлен Solana CLI.
...
<Callout type="info">
  ...
  validator. Для запуска примера необходимо клонировать программу Token
  Extension с mainnet.
</Callout>
