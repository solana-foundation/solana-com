---
title: Разрешённое сжигание
description:
  Создайте минт Token-2022 с расширением разрешённого сжигания, чтобы каждое
  сжигание требовало совместной подписи от настроенного органа сжигания, затем
  сжигайте токены с помощью инструкций разрешённого сжигания.
...
## Что такое разрешённое сжигание?

Расширение минта _rs`PermissionedBurnConfig`_ Token Extensions Program требует,
чтобы настроенный орган сжигания совместно подписывал каждое сжигание для
данного минта.

Пока установлен орган сжигания:

- Стандартные инструкции _rs`Burn`_ и _rs`BurnChecked`_ завершаются с ошибкой
  _rs`TokenError::InvalidInstruction`_.
- Для сжигания необходимо использовать _rs`PermissionedBurnInstruction::Burn`_
  или _rs`PermissionedBurnInstruction::BurnChecked`_, подписанные как органом
  сжигания, так и владельцем или делегатом token account.

Орган сжигания является совместным подписантом, а не заменой владельца. Орган
сжигания в одиночку не может сжигать токены из чужого token account, а владелец
token account не может сжигать токены без подписи органа сжигания. Постоянный
делегат также обязан использовать инструкции разрешённого сжигания и по-прежнему
нуждается в совместной подписи органа сжигания.

Это позволяет эмитенту поддерживать синхронизацию эмиссии токенов с внебиржевыми
записями — например, для токенизированного актива, который должен оставаться
обеспеченным в соотношении 1:1, — предотвращая односторонне сжигание токенов
держателями.

Орган сжигания может быть позднее заменён с помощью _rs`SetAuthority`_,
используя _rs`AuthorityType::PermissionedBurn`_. Установка органа в `None`
отключает разрешённое сжигание и повторно включает стандартные инструкции
сжигания. Данные самого расширения остаются на минте.

<Callout type="warn" title="Доступность">
  Разрешённое сжигание включено в Token-2022
  [`program@v11.0.0`](https://github.com/solana-program/token-2022/releases/tag/program%40v11.0.0).
  Сегодня оно доступно в devnet и запланировано к включению в mainnet в июне
  2026 года. Token Extension Program развёртывается отдельно для каждого
  кластера, поэтому убедитесь, что развёртывание на целевом кластере включает
  версию v11.0.0 или новее.
</Callout>

Локальный тестовый validator также включает более старую сборку Token-2022,
поэтому для запуска примеров на этой странице загрузите программу v11.0.0 из
devnet в ваш тестовый validator:
...
## Как создать Mint с разрешённым сжиганием

Чтобы создать mint с разрешённым сжиганием и сжечь токены:

1. Вычислите размер mint account и необходимый rent для mint и расширения
   _rs`PermissionedBurnConfig`_.
2. Создайте mint account с помощью _rs`CreateAccount`_, инициализируйте
   _rs`PermissionedBurnConfig`_ и инициализируйте mint с помощью
   _rs`InitializeMint`_.
3. Создайте token account и выпустите токены.
4. Выполните сжигание с помощью _rs`PermissionedBurnInstruction::BurnChecked`_,
   подписанного как владельцем token account, так и burn authority.
5. При необходимости отключите разрешённое сжигание с помощью
   _rs`SetAuthority`_, используя _rs`AuthorityType::PermissionedBurn`_ и
   authority `None`, что повторно включает стандартные инструкции сжигания.
   Полные примеры кода ниже демонстрируют этот шаг.

<ScrollyCoding>

## !!steps Вычисление размера аккаунта

Вычислите размер mint account для базового mint плюс расширение
_rs`PermissionedBurnConfig`_. Этот размер используется в _rs`CreateAccount`_.
...
## !!steps Вычисление rent

Вычислите rent, используя размер, необходимый для mint плюс расширение
_rs`PermissionedBurnConfig`_.
...
Создайте mint account с вычисленными пространством и lamports.
...
## !!steps Инициализация PermissionedBurn

Инициализируйте расширение _rs`PermissionedBurnConfig`_ на mint с burn
authority.
...
## !!steps Инициализация mint

Инициализируйте mint с помощью _rs`InitializeMint`_ в той же транзакции.
...
Создайте token account для плательщика и выпустите на него токены.
...
## !!steps Сжечь с использованием полномочий сжигания

Сожгите токены с помощью _rs`PermissionedBurnInstruction::BurnChecked`_,
подписанного как владельцем token account, так и органом полномочий сжигания.
...
<Callout type="info" title="Порядок инструкций">
  _rs`PermissionedBurnInstruction::Initialize`_ должна предшествовать
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`PermissionedBurnInstruction::Initialize`_ и _rs`InitializeMint`_ должны
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`PermissionedBurnConfig`_                   | Расширение минта, хранящее полномочия, необходимые для совместной подписи каждого сжигания для данного минта.                         | [Источник](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
| _rs`PermissionedBurnInstruction::Initialize`_  | Инструкция, инициализирующая конфигурацию разрешённого сжигания перед _rs`InitializeMint`_.                                           | [Источник](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L41-L49) |
| _rs`PermissionedBurnInstruction::Burn`_        | Инструкция сжигания, требующая подписей от органа полномочий сжигания и владельца token account или его делегата.                     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | Инструкция сжигания с проверкой десятичных знаков, использующая те же аккаунты, что и _rs`PermissionedBurnInstruction::Burn`_.        | [Источник](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | Дискриминатор полномочий, используемый совместно с _rs`SetAuthority`_ для смены или отключения полномочий сжигания на минте.          | [Источник](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | Логика процессора, инициализирующая _rs`PermissionedBurnConfig`_ на неинициализированном минте и сохраняющая полномочия сжигания.     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
| _rs`process_burn`_                             | Логика процессора, отклоняющая стандартные операции сжигания при установленных полномочиях сжигания и проверяющая подпись полномочий. | [Источник](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | Логика процессора, проверяющая и изменяющая полномочия сжигания при использовании _rs`AuthorityType::PermissionedBurn`_.              | [Источник](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### TypeScript

Приведённый ниже пример с `Kit` использует сгенерированные инструкции напрямую.
Опубликованный устаревший пакет `@solana/spl-token` пока не поддерживает
сжигание с разрешением, поэтому пример для устаревшего варианта не включён.
