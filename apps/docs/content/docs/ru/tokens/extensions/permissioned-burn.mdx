...
Локальный тестовый validator также включает устаревшую сборку Token-2022,
...
## Как создать Mint и сжигание с разрешением

Чтобы создать mint с разрешённым сжиганием и сжечь токены:

1. Вычислите размер mint account и rent, необходимые для mint и расширения
...
4. Сожгите токены с помощью _rs`PermissionedBurnInstruction::BurnChecked`_,
   подписанного как владельцем token account, так и уполномоченным по сжиганию.
5. При необходимости отключите разрешённое сжигание с помощью
   _rs`SetAuthority`_, используя _rs`AuthorityType::PermissionedBurn`_ и права
   `None`, что повторно активирует стандартные инструкции сжигания. Полные
   примеры кода ниже демонстрируют этот шаг.
...
Инициализируйте расширение _rs`PermissionedBurnConfig`_ на mint с уполномоченным
по сжиганию.
...
## !!steps Создание token account и минтинг токенов

Создайте token account для плательщика и заминтите на него токены.
...
## !!steps Сжигание с помощью burn authority

Сжигайте токены с _rs`PermissionedBurnInstruction::BurnChecked`_, подписанным
как владельцем token account, так и burn authority.
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`PermissionedBurnConfig`_                   | Расширение минта, хранящее authority, необходимую для совместной подписи каждого сжигания для данного минта.                   | [Источник](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
| _rs`PermissionedBurnInstruction::Initialize`_  | Инструкция, инициализирующая конфигурацию permissioned burn до _rs`InitializeMint`_.                                           | [Источник](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L41-L49) |
| _rs`PermissionedBurnInstruction::Burn`_        | Инструкция сжигания, требующая подписей от burn authority и владельца token account или делегата.                              | [Источник](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
...
| _rs`AuthorityType::PermissionedBurn`_          | Дискриминатор authority, используемый вместе с _rs`SetAuthority`_ для ротации или отключения burn authority на минте.          | [Источник](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | Логика процессора, инициализирующая _rs`PermissionedBurnConfig`_ на неинициализированном минте и сохраняющая burn authority.   | [Источник](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
| _rs`process_burn`_                             | Логика процессора, отклоняющая стандартные операции сжигания при установленной burn authority и проверяющая подпись authority. | [Источник](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | Логика процессора, валидирующая и выполняющая ротацию burn authority при использовании _rs`AuthorityType::PermissionedBurn`_.  | [Источник](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### Typescript

Пример с `Kit` ниже использует сгенерированные инструкции напрямую.
Опубликованный устаревший пакет `@solana/spl-token` пока не включает поддержку
разрешённого сжигания, поэтому пример для устаревшего варианта не приводится.
