...
- [Token Programs](#token-programs) содержат всю логику инструкций для
...
— это по сути
[аккаунты данных](/docs/core/accounts/account-types#data-accounts),
принадлежащие Token Program.
...
[аккаунт](/docs/core/accounts/account-structure#account-fields), которым для
всех token accounts является Token Program.
...
Существует два способа добавления метаданных (название, символ, изображение) к
токенам на Solana:

1. **[Metaplex Token Metadata](/docs/tokens/metaplex)** - Создаёт отдельный
   аккаунт метаданных, связанный с вашим mint. Работает с оригинальной Token
   Program и Token-2022 и широко поддерживается кошельками и маркетплейсами.

2. **[Token Extensions Metadata](/docs/tokens/extensions/metadata)** - Хранит
   метаданные непосредственно в mint account с использованием Token-2022. Новый
   подход со встроенной поддержкой. Расширение метаданных доступно только для
   mint accounts, созданных с Token-2022. Чтобы добавить метаданные к mint
   accounts, созданным с помощью оригинальной Token Program, используйте
   Metaplex Token Metadata Program.

#### Пример SPL Token CLI

Token Extensions Program позволяет хранить метаданные непосредственно в Mint
Account. Чтобы создать токен с расширением метаданных:
...
URI токена ссылается на внецепочечные метаданные. Посмотрите пример формата JSON
...
Узнайте больше в разделе [Token Extensions](/docs/tokens/extensions/metadata).
Подробности о Token Extensions смотрите в разделе
[Token Extensions](/docs/tokens/extensions) и в
[документации SPL](https://www.solana-program.com/docs/token-2022/extensions).
