...
[аккаунт](/docs/core/accounts#account-type), которым для всех Token Account
...
CLI _sh`spl-token`_ помогает экспериментировать с SPL токенами. В приведённых
ниже примерах используется терминал [Solana Playground](https://beta.solpg.io/)
для запуска CLI-команд прямо в браузере без локальной установки.
...
[веб-кран](https://faucet.solana.com/).
...
Больше примеров вы найдёте в
[документации](https://www.solana-program.com/docs/token) по CLI
_sh`spl-token`_.
...
Команда _sh`create-account`_ создаёт associated token account с вашим адресом
...
Инструкция _rs`MintTo`_ в Token Program создаёт новые токены. Mint authority
должен подписать транзакцию. Эта инструкция выпускает токены на token account и
увеличивает общий объём в mint account. Вот пример на Javascript в
...
отправляющего token account должен подписать транзакцию. Эта инструкция
