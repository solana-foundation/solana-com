...
description:
  Заморозьте аккаунт токена с помощью полномочия заморозки минта, чтобы
  заблокировать переводы, сжигание и изменения делегатов.
...
## Что делает заморозка аккаунта токена?

Заморозка аккаунта токена сохраняет того же владельца, минт и баланс, но
предотвращает передачу или сжигание токенов с аккаунта токена до тех пор, пока
аккаунт токена не будет разморожен.

Замороженный аккаунт все еще может быть закрыт, если на замороженном аккаунте
нулевой баланс токенов.

Только минт, у которого все еще есть полномочие заморозки, может замораживать
аккаунты токенов. Если полномочие заморозки минта отозвано, аккаунты для этого
минта больше не могут быть заморожены.

## Как заморозить аккаунт токена

Заморозка аккаунта токена использует инструкцию _rs`FreezeAccount`_ Token
Program.

Инструкция _rs`FreezeAccount`_ проверяет, что аккаунт токена принадлежит минту и
что подписант является полномочием заморозки минта, затем изменяет состояние
аккаунта токена с инициализированного на замороженное.
...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Состояние минта хранит опциональное полномочие заморозки, используемое для заморозки аккаунтов токенов. | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Состояния аккаунта токена, включая _rs`Initialized`_ и _rs`Frozen`_.                                    | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | Инструкция, которая переводит аккаунт токена в замороженное состояние.                                  | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
