...
предотвращает все переводы токенов или сжигание токенов с определённого
токен-аккаунта. После заморозки аккаунт не может отправлять или получать токены
до его разморозки. Замороженный аккаунт всё ещё может быть закрыт, если у него
нулевой баланс токенов. Только орган заморозки (freeze authority) минта токена
может замораживать аккаунты. Если орган заморозки отозван (установлен в null)
для минт-аккаунта, токены никогда не смогут быть заморожены.
