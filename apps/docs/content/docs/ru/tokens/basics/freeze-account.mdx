...
description:
  Заморозьте токен-аккаунт с помощью полномочий заморозки минта, чтобы
  заблокировать переводы, сжигание и изменения делегата.
...
## Что даёт заморозка токен-аккаунта?

Заморозка token account сохраняет того же владельца, минт и баланс, но
предотвращает получение, передачу или сжигание токенов на этом аккаунте до тех
пор, пока token account не будет разморожен.

Замороженный аккаунт всё ещё может быть закрыт, если у замороженного аккаунта
нулевой баланс токенов.

Только минт, у которого всё ещё есть полномочия заморозки, может замораживать
токен-аккаунты. Если полномочия заморозки минта отозваны, аккаунты для этого
минта больше не могут быть заморожены.

## Как заморозить токен-аккаунт

Заморозка токен-аккаунта использует инструкцию _rs`FreezeAccount`_ Token
Program.

Инструкция _rs`FreezeAccount`_ проверяет, что токен-аккаунт принадлежит минту и
что подписант является полномочием заморозки минта, затем изменяет состояние
токен-аккаунта с инициализированного на замороженное.
...
| ----------------------------------- | ----------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Состояние минта хранит опциональные полномочия заморозки, используемые для заморозки токен-аккаунтов. | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Состояния токен-аккаунта, включая _rs`Initialized`_ и _rs`Frozen`_.                                   | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | Инструкция, которая изменяет токен-аккаунт в замороженное состояние.                                  | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
| _rs`process_toggle_freeze_account`_ | Общая логика обработчика для заморозки и разморозки токен-аккаунтов.                                  | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |

### Typescript

В примерах ниже показан рекомендуемый подход с использованием `Kit`. Устаревшие
