...
предотвращает все переводы токенов или их сжигание с определённого аккаунта
токенов. После заморозки аккаунт не может отправлять или получать токены до его
разморозки. Замороженный аккаунт всё ещё может быть закрыт, если на нём нулевой
баланс токенов. Только уполномоченный орган по заморозке mint account токена
может замораживать аккаунты. Если полномочия на заморозку отозваны (установлены
в null) на mint account, токены никогда не смогут быть заморожены.
