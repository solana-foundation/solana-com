...
запрещает любые переводы токенов или сжигание токенов с определённого
токен-аккаунта. После заморозки аккаунт не может отправлять токены, получать
токены или быть закрыт до тех пор, пока не будет разморожен. Только freeze
authority (владелец права заморозки) mint'а токена может замораживать аккаунты.
Если freeze authority был отозван (установлен в null) на аккаунте mint, токены
больше не могут быть заморожены.
