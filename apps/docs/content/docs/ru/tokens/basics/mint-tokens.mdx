...
description:
  Создание новых единиц токенов на токен-аккаунте с использованием права на
  создание.
...
## Что делает создание токенов?

Создание токенов генерирует новые единицы существующего токена и увеличивает
общее предложение токена.

Только владелец права на создание токенов может создавать новые токены. Целевой
токен-аккаунт уже должен существовать и принадлежать тому же токену.

## Как создавать токены

Создание токенов использует инструкцию _rs`MintTo`_ или _rs`MintToChecked`_
Token Program.

Примеры ниже используют _rs`MintToChecked`_, которая требует, чтобы вызывающая
сторона предоставила количество десятичных знаков токена, чтобы инструкция могла
проверить ожидаемую точность токена перед созданием.

[Нативный токен](/docs/tokens/basics/sync-native) не поддерживает создание.

### Справочная информация об исходном коде

| Элемент               | Описание                                                                                                                                                                                     | Token Program                                                                                                        | Token Extensions Program                                                                                                   |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_            | Состояние токена хранит право на создание и общее предложение токенов.                                                                                                                       | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_         | Состояние токен-аккаунта хранит баланс получателя, обновляемый при создании токенов.                                                                                                         | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`MintTo`_          | Инструкция создания, которая создает токены на существующем токен-аккаунте без требования от вызывающей стороны предоставлять количество десятичных знаков токена.                           | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L159-L177) | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L202-L220) |
| _rs`MintToChecked`_   | Инструкция создания, которая требует от вызывающей стороны предоставить количество десятичных знаков токена и проверяет это значение перед созданием токенов на существующем токен-аккаунте. | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L301-L325) | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L367-L391) |
| _rs`process_mint_to`_ | Общая логика процессора для создания токенов.                                                                                                                                                | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L521-L583)     | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L981-L1067)    |

### Typescript

Примеры `Kit` ниже демонстрируют рекомендуемый подход с использованием
`@solana/kit`. Примеры устаревшего подхода с использованием `@solana/web3.js`
