---
title: Минтинг токенов
description: Узнайте, как создавать новые единицы токена.
---

## Как минтить токены

Минтинг создает новые единицы токена с использованием инструкции
...
- Только mint authority может минтить новые токены
- Целевой token account должен существовать для получения заминченных токенов
...
<Callout type="info">
  ...
  используют схожие реализации для достижения одинаковой функциональности.
</Callout>
...
async fn main() -> Result<()> {
    // Создаём подключение к локальному валидатору
    ...
    // Генерируем новую keypair для fee payer
    let fee_payer = Keypair::new();

    // Делаем airdrop 1 SOL на fee payer
    ...
    // Генерируем keypair для адреса mint
    let mint = Keypair::new();

    // Получаем стандартный размер mint аккаунта (в байтах), без расширений
    let mint_space = Mint::LEN;
    let mint_rent = client
        .get_minimum_balance_for_rent_exemption(mint_space)
        .await?;

    // Инструкция для создания нового аккаунта под mint (token program)
    let create_account_instruction = create_account(
        &fee_payer.pubkey(), // payer
        &mint.pubkey(),      // новый аккаунт (mint)
        mint_rent,           // lamports
        mint_space as u64,   // space
        &token_program_id(), // program id
    );

    // Инструкция для инициализации данных mint аккаунта
    ...
    // Вычисляем адрес associated token account для fee_payer
    let associated_token_address = get_associated_token_address_with_program_id(
        &fee_payer.pubkey(), // владелец
        &mint.pubkey(),      // mint
        &token_program_id(), // program_id
    );

    // Инструкция для создания associated token account
    let create_ata_instruction = create_associated_token_account(
        &fee_payer.pubkey(), // адрес для финансирования
        &fee_payer.pubkey(), // адрес кошелька
        &mint.pubkey(),      // адрес mint
        &token_program_id(), // program id
    );

    // Создаём транзакцию и добавляем инструкции
    ...
    // Отправить и подтвердить транзакцию
    client.send_and_confirm_transaction(&transaction).await?;

    // Количество токенов для выпуска (100 токенов с двумя знаками после запятой)
    let amount = 100;

    // Создать инструкцию mint_to для выпуска токенов на associated token account
    let mint_to_instruction = mint_to(
        &token_program_id(),
        &mint.pubkey(),            // mint
        &associated_token_address, // получатель
        &fee_payer.pubkey(),       // authority
        &[&fee_payer.pubkey()],    // подписант
        amount,                    // количество
    )?;

    // Создать транзакцию для выпуска токенов
    let transaction = Transaction::new_signed_with_payer(
        &[mint_to_instruction],
        Some(&fee_payer.pubkey()),
        &[&fee_payer],
    ...
    println!("Выпущено 1.00 токенов на associated token account");
    println!("\nАдрес mint: {}", mint.pubkey());
    println!("{:#?}", mint_data);
    ...
    println!("Подпись транзакции: {}", transaction_signature);
...
async def main():
    ...
    # Количество для выпуска (в наименьших единицах)
    amount_to_mint = 1000000000  # 1 токен с 9 знаками после запятой

    async with rpc:
        # Создать инструкцию mint
        ...
        print(f"Mint transaction создана успешно")
