...
description:
  Выпустите новые единицы токенов на аккаунт токенов с правами эмиссии.
...
## Что делает выпуск токенов?

Выпуск создаёт новые единицы существующего токена и увеличивает общее
предложение эмиссии.

Только орган эмиссии может выпускать новые токены. Целевой аккаунт токенов
должен уже существовать и должен принадлежать той же эмиссии.

## Как выпускать токены

Выпуск токенов использует инструкцию _rs`MintTo`_ или _rs`MintToChecked`_ Token
Program.

Примеры ниже используют _rs`MintToChecked`_, которая требует от вызывающей
стороны предоставить количество десятичных знаков эмиссии, чтобы инструкция
могла проверить ожидаемую точность токена перед выпуском.

[Нативная эмиссия](/docs/tokens/basics/sync-native) не поддерживает выпуск
токенов.
...
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_            | Состояние эмиссии хранит орган эмиссии и общее предложение токенов.                                                                                                                          | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_         | Состояние аккаунта токенов хранит целевой баланс, обновляемый при выпуске.                                                                                                                   | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`MintTo`_          | Инструкция выпуска, которая выпускает токены на существующий аккаунт токенов без требования от вызывающей стороны предоставлять количество десятичных знаков эмиссии.                        | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L159-L177) | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L202-L220) |
| _rs`MintToChecked`_   | Инструкция выпуска, которая требует от вызывающей стороны предоставить количество десятичных знаков эмиссии и проверяет это значение перед выпуском токенов на существующий аккаунт токенов. | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L301-L325) | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L367-L391) |
| _rs`process_mint_to`_ | Общая логика процессора для выпуска токенов.                                                                                                                                                 | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L521-L583)     | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L981-L1067)    |
