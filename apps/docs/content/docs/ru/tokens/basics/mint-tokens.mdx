...
Выпуск создает новые единицы токена с помощью инструкции
...
- Только mint authority может выпускать новые токены
- Для получения выпущенных токенов должен существовать целевой token account
...
<Callout type="info">
  ...
  реализованы схожим образом для достижения одинаковой функциональности.
</Callout>
...
async fn main() -> Result<()> {
    // Создать подключение к локальному validator
    ...
    // Сгенерировать новую keypair для fee payer
    let fee_payer = Keypair::new();

    // Airdrop 1 SOL для fee payer
    ...
    // Сгенерировать keypair для адреса mint
    let mint = Keypair::new();

    // Получить стандартный размер mint account (в байтах), без расширений
    let mint_space = Mint::LEN;
    let mint_rent = client
        .get_minimum_balance_for_rent_exemption(mint_space)
        .await?;

    // Инструкция для создания нового аккаунта для mint (token program)
    ...
    // Инструкция для инициализации данных mint account
