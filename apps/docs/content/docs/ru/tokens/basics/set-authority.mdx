...
description:
  Узнайте, как установить полномочия для mint account или token account.
---

## Понимание полномочий токенов

### Полномочия mint account

- **Mint Authority**: Управляет созданием новых токенов. Может выпускать токены
  на любой token account. Часто отзывается после создания первоначального
  предложения, чтобы сделать токен с фиксированным предложением.
- **Freeze Authority**: Управляет возможностью замораживать и размораживать
  token account. Может запретить любому token account переводить токены. Часто
  отзывается, чтобы гарантировать пользователям, что их токены не могут быть
  заморожены.

### Полномочия token account

- **Владелец аккаунта**: Имеет полный контроль над token account. Может
  переводить токены, сжигать токены, назначать делегатов и закрывать аккаунт при
  нулевом балансе.
- **Close Authority**: Может закрыть token account при нулевом балансе. По
  умолчанию это владелец аккаунта, но полномочие может быть делегировано другому
  аккаунту.
...
изменяет или отзывает полномочия для mint account и token account. Только
текущий держатель полномочий может передать свои права новому адресу или
отозвать их навсегда, установив полномочие в null. После отзыва полномочие не
