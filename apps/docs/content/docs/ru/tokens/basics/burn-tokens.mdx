...
description:
  Сжигайте токены из token account и уменьшайте общий объем эмиссии минта.
...
Сжигание токенов навсегда уменьшает баланс token account и снижает общий объем
эмиссии минта на ту же сумму.
...
указать количество десятичных знаков минта, чтобы инструкция могла проверить
ожидаемую точность токена перед сжиганием.

[Нативный минт](/docs/tokens/basics/sync-native) не поддерживает сжигание.
Вместо этого используйте _rs`CloseAccount`_.

Владелец token account или утвержденный делегат подписывает транзакцию. В Token
Extension Program постоянный делегат минта также может авторизовать сжигание,
если в минте включено расширение постоянного делегата.

### Справочные материалы из исходного кода

| Элемент            | Описание                                                                                                                                                                                       | Token Program                                                                                                    | Token Extension Program                                                                                                |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_         | Состояние минта хранит общий объем токенов в обращении.                                                                                                                                        | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | Состояние token account хранит баланс токенов, уменьшенный в результате сжигания.                                                                                                              | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Инструкция сжигания, которая уменьшает баланс token account и объем эмиссии минта без требования от вызывающей стороны указывать количество десятичных знаков минта.                           | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Инструкция сжигания, которая требует от вызывающей стороны указать количество десятичных знаков минта и проверяет это значение перед уменьшением баланса token account и объема эмиссии минта. | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | Общая логика обработки для сжигания токенов.                                                                                                                                                   | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |

### Typescript

Примеры ниже показывают рекомендуемый подход с использованием `Kit`. Устаревшие
примеры с использованием `@solana/kit` приведены для справки.
