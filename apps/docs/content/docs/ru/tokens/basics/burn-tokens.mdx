...
title: Сжечь токены
description:
  Сожгите токены из аккаунта токенов и уменьшите общее предложение минта.
...
Сжигание токенов необратимо уменьшает баланс аккаунта токенов и снижает общее
предложение минта на ту же величину.
...
предоставить decimals минта, чтобы инструкция могла проверить ожидаемую точность
токена перед сжиганием.

[Native mint](/docs/tokens/basics/sync-native) не поддерживает сжигание.
Используйте вместо этого _rs`CloseAccount`_.

Владелец аккаунта токенов или утверждённый делегат подписывает транзакцию. В
Token Extensions Program постоянный делегат минта также может авторизовать
...
Устаревшие примеры с использованием `@solana/web3.js` приведены для справки.
