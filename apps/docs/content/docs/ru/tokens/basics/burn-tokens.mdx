...
title: Сжечь токены
description:
  Сжигайте токены с аккаунта токенов и уменьшайте общее предложение минта.
...
Сжигание токенов навсегда уменьшает баланс аккаунта токенов и сокращает общее
предложение минта на ту же сумму.

## Как Сжигать Токены

Сжигание токенов использует инструкцию _rs`Burn`_ или _rs`BurnChecked`_ Token
Program.

Примеры ниже используют _rs`BurnChecked`_, которая требует от вызывающей стороны
предоставить количество десятичных знаков минта, чтобы инструкция могла
проверить ожидаемую точность токена перед сжиганием.

[Нативный минт](/docs/tokens/basics/sync-native) не поддерживает сжигание.
Вместо этого используйте _rs`CloseAccount`_.

Владелец аккаунта токенов или утверждённый делегат подписывает транзакцию. В
Token Extension Program постоянный делегат минта также может авторизовать
сжигание, если у минта включено расширение постоянного делегата.

### Справочные Материалы

| Элемент            | Описание                                                                                                                                                                                            | Token Program                                                                                                    | Token Extension Program                                                                                                |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_         | Состояние минта хранит общее предложение токенов.                                                                                                                                                   | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | Состояние аккаунта токенов хранит баланс токенов, уменьшенный в результате сжигания.                                                                                                                | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Инструкция сжигания, которая уменьшает баланс аккаунта токенов и предложение минта без требования от вызывающей стороны предоставлять количество десятичных знаков минта.                           | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Инструкция сжигания, которая требует от вызывающей стороны предоставить количество десятичных знаков минта и проверяет это значение перед уменьшением баланса аккаунта токенов и предложения минта. | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | Общая логика процессора для сжигания токенов.                                                                                                                                                       | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |

### Typescript

Приведенные ниже примеры с `Kit` демонстрируют рекомендуемый подход с
использованием `@solana/kit`. Устаревшие примеры с `@solana/web3.js` включены
для справки.
