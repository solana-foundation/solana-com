...
передачи SOL в виде SPL-токена.
...
синхронизирует баланс токен-аккаунта обёрнутого SOL (WSOL) с фактическим
количеством SOL (лампортов), хранящихся в нём. Когда вы переводите нативный SOL
на WSOL токен-аккаунт, баланс токенов не обновляется автоматически. Для
корректного отображения баланса WSOL необходимо вызвать SyncNative.
