...
description:
  Обновление суммы обернутого SOL в token account после перевода SOL на аккаунт.
...
## Что такое обернутый SOL?

Обернутый SOL (WSOL) — это SOL, хранящийся в token account для нативного минта
Token Program. Нативный минт — это адрес минта, который Token Program использует
для token account, оборачивающих SOL. Нативный минт позволяет использовать SOL
через инструкции Token Program.

| Программа                | Адрес нативного минта                          |
| ------------------------ | ---------------------------------------------- |
...
Обернутый SOL token account хранит SOL в поле `lamports` аккаунта, но учитывает
SOL, находящийся в обернутом SOL token account, как баланс токенов через поле
`amount` token account. Если вы переводите SOL в обернутый SOL token account,
баланс в lamport увеличивается немедленно, но поле `amount` не изменяется до тех
пор, пока вы не вызовете инструкцию `SyncNative` Token Program.

## Как синхронизировать нативный SOL

Используйте инструкцию `SyncNative` Token Program после перевода SOL в обернутый
SOL token account. Инструкция `SyncNative` обновляет поле `amount` token account
в соответствии с количеством lamport, хранящихся в аккаунте сверх резерва,
необходимого для освобождения от арендной платы.

### Ссылка на исходный код

| Элемент               | Описание                                                                                                                                                                                                                                     | Token Program                                                                                                    | Token Extensions Program                                                                                               |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `Account`             | Состояние token account использует `is_native` для пометки аккаунта как WSOL и хранит нативный резерв — минимальный баланс в lamport, освобождающий от арендной платы, который аккаунт должен сохранять, и отслеживаемое количество токенов. | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| `SyncNative`          | Инструкция, которая обновляет поле `amount` обернутого SOL token account в соответствии с количеством lamport, хранящихся в аккаунте сверх резерва, освобождающего от арендной платы.                                                        | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| `process_sync_native` | Общая логика процессора для синхронизации балансов обернутого SOL.                                                                                                                                                                           | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |
