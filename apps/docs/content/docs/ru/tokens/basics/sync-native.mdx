...
description:
  Обновите сумму в аккаунте обернутых SOL токенов после перевода SOL на аккаунт.
...
## Что такое обернутые SOL?

Обернутые SOL (WSOL) — это SOL, хранящиеся в token account для нативного минта
...
Token account обернутых SOL хранит SOL в поле `lamports` аккаунта, но
отслеживает SOL, хранящиеся в token account обернутых SOL, как баланс токенов
через поле `amount` token account. Если вы переведете SOL на token account
обернутых SOL, баланс в lamport увеличится немедленно, но поле `amount` не
изменится до тех пор, пока вы не вызовете инструкцию `SyncNative` Token Program.

## Как синхронизировать нативные SOL

Используйте инструкцию `SyncNative` Token Program после перевода SOL на token
account обернутых SOL. Инструкция `SyncNative` обновляет поле `amount` token
account, чтобы оно соответствовало lamport, хранящимся на аккаунте сверх
резерва, освобожденного от аренды.

### Исходный код

| Элемент               | Описание                                                                                                                                                                                                                           | Token Program                                                                                                        | Token Extensions Program                                                                                                   |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `Account`             | Состояние token account использует `is_native` для обозначения аккаунта как WSOL и хранит нативный резерв, минимальный баланс в lamport, освобожденный от аренды, который аккаунт должен сохранять, и отслеживаемую сумму токенов. | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| `SyncNative`          | Инструкция, которая обновляет поле `amount` token account обернутых SOL, чтобы оно соответствовало lamport, хранящимся на аккаунте сверх резерва, освобожденного от аренды.                                                        | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| `process_sync_native` | Общая логика процессора для синхронизации балансов обернутых SOL.                                                                                                                                                                  | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |

### Typescript

Примеры ниже демонстрируют рекомендуемый подход с использованием `Kit`.
Устаревшие примеры с использованием `@solana/kit` и `@solana/web3.js` приведены
для справки.
