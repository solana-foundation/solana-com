...
description:
  Разморозьте замороженный аккаунт токенов, чтобы возобновить переводы, сжигание
  и изменение делегирования.
...
## Что делает разморозка аккаунта токенов?

Разморозка переводит аккаунт токенов из замороженного состояния обратно в
инициализированное состояние.

После успешной разморозки аккаунт снова может переводить, сжигать и закрывать
токены. Только freeze authority минта может размораживать аккаунты токенов.

## Как разморозить аккаунт токенов

Разморозка аккаунта токенов использует инструкцию _rs`ThawAccount`_ Token
Program.

Инструкция _rs`ThawAccount`_ проверяет, что аккаунт токенов принадлежит минту и
что подписант является freeze authority минта, затем изменяет состояние аккаунта
токенов с замороженного обратно на инициализированное.

[Нативные аккаунты токенов](/docs/tokens/basics/sync-native) не поддерживают
разморозку.
...
| ----------------------------------- | ---------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Состояние минта хранит опциональный freeze authority, используемый для разморозки аккаунтов токенов. | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Состояния аккаунта токенов, включая _rs`Frozen`_ и _rs`Initialized`_.                                | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`ThawAccount`_                   | Инструкция, которая переводит замороженный аккаунт токенов обратно в инициализированное состояние.   | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L229-L243) | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L292-L306) |
| _rs`process_toggle_freeze_account`_ | Общая логика процессора для заморозки и разморозки аккаунтов токенов.                                | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |

### Typescript

Примеры `Kit` ниже показывают рекомендуемый подход с использованием
