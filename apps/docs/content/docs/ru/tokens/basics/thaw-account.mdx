...
description:
  Разморозьте замороженный токен-аккаунт, чтобы возобновить переводы, сжигание и
...
## Что делает разморозка токен-аккаунта?

Разморозка изменяет состояние токен-аккаунта из замороженного обратно в
инициализированное.

После успешной разморозки аккаунт снова может переводить, сжигать и закрывать
токены. Только уполномоченный по заморозке минта может размораживать
токен-аккаунты.

## Как разморозить токен-аккаунт

Разморозка токен-аккаунта использует инструкцию _rs`ThawAccount`_ Token Program.

Инструкция _rs`ThawAccount`_ проверяет, что токен-аккаунт принадлежит минту и
что подписант является уполномоченным по заморозке минта, затем изменяет
состояние токен-аккаунта из замороженного обратно в инициализированное.

[Нативные токен-аккаунты](/docs/tokens/basics/sync-native) не поддерживают
разморозку.
...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Состояние минта хранит опциональный уполномоченный по заморозке, используемый для разморозки токен-аккаунтов. | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Состояния токен-аккаунта, включая _rs`Frozen`_ и _rs`Initialized`_.                                           | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`ThawAccount`_                   | Инструкция, которая изменяет замороженный токен-аккаунт обратно в инициализированное состояние.               | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L229-L243) | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L292-L306) |
| _rs`process_toggle_freeze_account`_ | Общая логика процессора для заморозки и разморозки токен-аккаунтов.                                           | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |

### Typescript

Примеры `Kit` ниже демонстрируют рекомендуемый подход с использованием
`@solana/kit`. Устаревшие примеры с `@solana/web3.js` приведены для справки.
