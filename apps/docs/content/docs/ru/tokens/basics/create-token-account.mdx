...
description: Узнайте, как создать SPL токен-аккаунты.
---

## Что такое токен-аккаунт?

Токен-аккаунт хранит ваш баланс определённого токена. Каждый токен-аккаунт
связан ровно с одним mint и отслеживает ваш баланс токенов и дополнительные
детали.
...
<Callout type="info">
  ...
  используют одну и ту же базовую реализацию токен-аккаунта.
</Callout>

Чтобы хранить токены, вам нужен токен-аккаунт для конкретного mint. Каждый
токен-аккаунт отслеживает:

1. **Mint**: Какой тип токена он содержит
2. **Владелец**: Авторитет, который может переводить токены с этого аккаунта

### Пример: USD Coin (USDC)

- Адрес mint: `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Токен-аккаунт Circle: `3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
- Этот аккаунт хранит только токены USDC
- Circle управляет токеном через полномочия владельца токен-аккаунта:
  `7VHUFJHWu2CuExkJcJrzhQPJ2oygupTWkL2A2For4BmE`

Вы можете посмотреть детали этого токен-аккаунта в
