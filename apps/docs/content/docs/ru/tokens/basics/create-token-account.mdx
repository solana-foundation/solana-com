...
description:
  Создавайте токен-аккаунты и associated token account для конкретного минта и
  владельца.
...
Здесь _rs`owner`_ означает полномочия, которые могут переводить, сжигать или
...
Associated Token Program — это способ создать токен-аккаунт по стандартному,
детерминированному адресу. Полученный аккаунт по-прежнему является
...
2. **Владелец программы**: Программа, которой принадлежит аккаунт (для
   токен-аккаунтов это всегда Token Program или Token Extension Program).

При работе с токен-аккаунтами под «владельцем» обычно подразумевается авторитет,
который может переводить токены, а не программа, которой принадлежит аккаунт.

</Callout>

## Что такое ассоциированный токен-аккаунт?

Ассоциированный токен-аккаунт (ATA) — это основной токен-аккаунт для кошелька,
предназначенный для хранения определённого токена. Он использует
детерминированный адрес (PDA), созданный с помощью
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program).

<Callout type="info">
  Только токен-аккаунты, созданные через Associated Token Program, называются
  «ассоциированными токен-аккаунтами».
</Callout>

ATA используют детерминированные адреса, что позволяет легко находить
токен-аккаунт любого кошелька для заданного mint. См.
[реализацию деривации здесь](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/interface/src/address.rs#L56).
...
Для любой пары кошелёк–токен существует ровно один адрес ATA. Это избавляет от
необходимости вручную отслеживать адреса токен-аккаунтов — вы всегда можете
получить правильный адрес программно.
...
- Создаёт токен-аккаунты по детерминированным адресам (PDA)
- Выполняет Cross-Program Invocations (CPI) в Token Program
- Не хранит собственное состояние

В результате токен-аккаунты принадлежат Token Program и используют стандартную
структуру `Account`.
...
Для создания токен-аккаунта нужны две инструкции:

1. System Program: создать аккаунт с выделенным пространством для токен-аккаунта
   и передать его в собственность Token Program
...
Создание associated token account (ATA) требует инструкции
...
Эта единственная инструкция автоматически создаёт token account и инициализирует
его через Cross Program Invocations (CPI).
