...
предоставляет другому аккаунту (делегату) разрешение переводить определённое
количество токенов с вашего token account. Делегат может переводить токены в
пределах утверждённой суммы, но не больше. Только владелец token account может
одобрить делегата, и у каждого token account может быть только один делегат
одновременно. Любое новое одобрение заменяет предыдущее.
