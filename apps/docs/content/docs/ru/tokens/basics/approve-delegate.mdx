---
title: Одобрение делегата
description:
  Авторизуйте другой адрес для перевода или сжигания ограниченного количества
  токенов с аккаунта токенов.
...
## Что делает одобрение делегата?

Одобрение делегата позволяет другому адресу переводить или сжигать ограниченное
количество токенов с аккаунта токенов от имени владельца аккаунта токенов.

Аккаунт токенов хранит одного текущего делегата и одну делегированную сумму
одновременно. Одобрение нового делегата заменяет предыдущего делегата и лимит на
аккаунте.

## Как одобрить делегата

Одобрение делегата использует инструкцию _rs`Approve`_ или _rs`ApproveChecked`_
Token Program.

Примеры ниже используют _rs`ApproveChecked`_, которая требует от вызывающей
стороны предоставить mint и decimals, чтобы инструкция могла проверить ожидаемый
mint и точность токенов перед обработкой одобрения.

Владелец аккаунта токенов подписывает одобрение.

### Ссылка на исходный код

| Элемент               | Описание                                                                                                                                                     | Token Program                                                                                                        | Token Extension Program                                                                                                    |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_         | Состояние аккаунта токенов хранит владельца, делегата и делегированную сумму.                                                                                | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | Инструкция одобрения делегата, которая записывает делегата и лимит для аккаунта токенов без требования от вызывающей стороны предоставить mint или decimals. | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | Инструкция одобрения делегата, которая требует от вызывающей стороны предоставить mint и decimals и проверяет эти значения перед записью делегата и лимита.  | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
| _rs`process_approve`_ | Общая логика процессора для одобрения делегата.                                                                                                              | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L344-L392)     | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L582-L640)     |
