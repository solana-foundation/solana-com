...
description:
  Перемещайте токены между токен-аккаунтами одного минта с помощью инструкций
  transfer из Token Program.
...
Перевод токенов перемещает токены из одного токен-аккаунта в другой
токен-аккаунт того же минта.

Переводы не изменяют общее предложение минта. Переводы только обновляют балансы
между токен-аккаунтами.

## Как переводить токены

Для перевода токенов используется инструкция _rs`Transfer`_ или
_rs`TransferChecked`_ из Token Program.

В примерах ниже используется _rs`TransferChecked`_, которая требует от
...
В Token Extensions Program инструкция _rs`Transfer`_ устарела в пользу
_rs`TransferChecked`_ или _rs`TransferCheckedWithFee`_.

Владелец исходного аккаунта или утвержденный делегат подписывает перевод. В
Token Extensions Program постоянный делегат минта также может авторизовать
перевод, если у минта включено расширение постоянного делегата.

### Справочная информация по исходному коду

| Элемент                | Описание                                                                                                                                                                                                                                   | Token Program                                                                                                        | Token Extensions Program                                                                                                   |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_          | Состояние токен-аккаунта хранит балансы, обновляемые при переводе.                                                                                                                                                                         | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | Инструкция перевода, которая перемещает токены между токен-аккаунтами без требования от вызывающей стороны предоставлять минт или количество десятичных знаков. В Token Extensions Program используйте вместо этого _rs`TransferChecked`_. | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Инструкция перевода, которая требует от вызывающей стороны предоставить минт и количество десятичных знаков и проверяет эти значения перед перемещением токенов между токен-аккаунтами одного минта.                                       | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | Общая логика процессора для переводов токенов.                                                                                                                                                                                             | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |
