...
description:
  Удаление текущего делегата токен-аккаунта и сброс оставшегося делегированного
  лимита.
...
Отзыв делегата очищает текущего делегата токен-аккаунта и сбрасывает
делегированную сумму до нуля.

После отзыва предыдущий делегат больше не имеет полномочий переводить или
сжигать токены от имени токен-аккаунта.

## Как отозвать делегата

Отзыв делегата использует инструкцию _rs`Revoke`_ Token Program.

Инструкция _rs`Revoke`_ очищает поля делегата в исходном токен-аккаунте. В Token
Program владелец токен-аккаунта подписывает отзыв. В Token Extensions Program
текущий делегат также может отозвать делегирование.

### Справочные материалы

| Элемент              | Описание                                                         | Token Program                                                                                                        | Token Extensions Program                                                                                                   |
| -------------------- | ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_        | Состояние токен-аккаунта хранит делегата и делегированную сумму. | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Revoke`_         | Инструкция, которая удаляет текущего делегата из токен-аккаунта. | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L128-L140) | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L170-L182) |
| _rs`process_revoke`_ | Общая логика обработчика для отзыва делегата.                    | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L395-L420)     | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L643-L674)     |
