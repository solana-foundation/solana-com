...
description:
  Удаление текущего делегата аккаунта токена и сброс оставшегося делегированного
  лимита.
...
## Что делает отзыв делегата?

Отзыв делегата удаляет текущего делегата аккаунта токена и сбрасывает
делегированную сумму до нуля.

После отзыва предыдущий делегат больше не имеет полномочий переводить или
сжигать токены от имени аккаунта токена.

## Как отозвать делегата

Для отзыва делегата используется инструкция _rs`Revoke`_ Token Program.

Инструкция _rs`Revoke`_ очищает поля делегата в исходном аккаунте токена. В
Token Program владелец аккаунта токена подписывает отзыв. В Token Extensions
Program текущий делегат также может отозвать делегирование.

### Справочная информация

| Элемент              | Описание                                                          | Token Program                                                                                                    | Token Extensions Program                                                                                               |
| -------------------- | ----------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_        | Состояние аккаунта токена хранит делегата и делегированную сумму. | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Revoke`_         | Инструкция, которая удаляет текущего делегата из аккаунта токена. | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L128-L140) | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L170-L182) |
| _rs`process_revoke`_ | Общая логика процессора для отзыва делегата.                      | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L395-L420)     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L643-L674)     |

### Typescript

Приведенные ниже примеры `Kit` демонстрируют рекомендуемый подход с
использованием `@solana/kit`. Примеры с использованием устаревшего
`@solana/web3.js` включены для справки.
