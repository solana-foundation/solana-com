...
безвозвратно закрывает token account и переводит все оставшиеся SOL (rent) на
указанный аккаунт назначения. Баланс token account должен быть нулевым перед
закрытием. Выполнить эту инструкцию может только владелец token account или
назначенный close authority.
