---
title: Закрытие токен-аккаунта
description:
  Закройте токен-аккаунт и верните lamports аккаунта на целевой аккаунт.
...
## Что происходит при закрытии токен-аккаунта?

Закрытие token account удаляет аккаунт и переводит все его lamports,
включая возвращаемый минимальный баланс для хранения данных, на аккаунт назначения.

Баланс токенов должен быть равен нулю, прежде чем аккаунт можно будет закрыть. Замороженный аккаунт
всё равно можно закрыть, если баланс токенов замороженного аккаунта равен нулю. Исключением являются token accounts для Wrapped SOL — их можно закрыть с ненулевым балансом токенов, чтобы вернуть лежащие в основе SOL. Владелец аккаунта или уполномоченный по закрытию аккаунта подписывает инструкцию закрытия и выбирает получателя. Аккаунт, который оплатил создание ATA, не получает возвращённый баланс автоматически.

## Как закрыть токен-аккаунт

Закрытие токен-аккаунта использует инструкцию _rs`CloseAccount`_ Token Program.

Инструкция _rs`CloseAccount`_ переносит lamports исходного аккаунта на целевой
аккаунт, очищает исходный аккаунт и удаляет исходный аккаунт. Token Extension
Program применяет дополнительные проверки при закрытии для определённых
расширений, но базовый процесс закрытия остаётся тем же.

### Справочник по исходному коду

| Элемент                        | Описание                                                                                                     | Token Program                                                                                                  | Token Extension Program                                                                                              |
| --------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | Состояние токен-аккаунта хранит баланс, статус native и уполномоченного на закрытие, используемых при закрытии аккаунта.     | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | Инструкция, которая закрывает токен-аккаунт и переносит lamports токен-аккаунта на целевой аккаунт. | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | Общая логика процессора для закрытия токен-аккаунта.                                                               | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |
