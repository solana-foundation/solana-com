...
description:
  Закрыть токен-аккаунт и вернуть lamports аккаунта на аккаунт назначения.
...
## Что делает закрытие токен-аккаунта?

Закрытие токен-аккаунта удаляет аккаунт и возвращает lamports rent
токен-аккаунта на аккаунт назначения.

Баланс токенов должен быть равен нулю, прежде чем аккаунт можно будет закрыть.
Замороженный аккаунт всё ещё можно закрыть, если баланс токенов замороженного
аккаунта равен нулю. Токен-аккаунты Wrapped SOL являются исключением и могут
быть закрыты с балансом токенов для возврата базового SOL. Владелец аккаунта или
close authority аккаунта подписывает инструкцию закрытия.

## Как закрыть токен-аккаунт

Закрытие токен-аккаунта использует инструкцию _rs`CloseAccount`_ Token Program.

Инструкция _rs`CloseAccount`_ переносит lamports исходного аккаунта на аккаунт
назначения, очищает исходный аккаунт и удаляет его. Token Extension Program
применяет дополнительные проверки при закрытии для определённых расширений, но
базовый процесс закрытия остаётся тем же.

### Справочник по исходному коду

| Элемент                     | Описание                                                                                                     | Token Program                                                                                                    | Token Extension Program                                                                                                |
| --------------------------- | ------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | Состояние токен-аккаунта хранит баланс, статус native и close authority, используемые при закрытии аккаунта. | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | Инструкция, которая закрывает токен-аккаунт и переносит lamports токен-аккаунта на аккаунт назначения.       | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | Общая логика обработчика для закрытия токен-аккаунта.                                                        | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |
