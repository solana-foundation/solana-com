...
description:
  Создайте минт токена и настройте количество десятичных знаков минта,
  полномочия минтинга и полномочия заморозки.
...
Аккаунт минта определяет и уникально идентифицирует токен в Solana, а также
хранит общее состояние, применимое ко всем токен-аккаунтам для этого минта.
...
У каждого токена есть один аккаунт минта, и адрес минта является уникальным
...
_rs`is_initialized`_ в данные аккаунта минта.

### Справочные источники

| Элемент                        | Описание                                                                                                 | Token Program                                                                                                    | Token Extensions Program                                                                                               |
| ------------------------------ | -------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                     | Базовое состояние минта, хранящееся в каждом аккаунте минта.                                             | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`InitializeMint`_           | Инструкция инициализации минта, которая ожидает аккаунт системной переменной rent в списке аккаунтов.    | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L24-L44)   | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L50-L74)   |
| _rs`InitializeMint2`_          | Инструкция инициализации минта, которая не требует аккаунт системной переменной rent в списке аккаунтов. | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L401-L414) | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L471-L486) |
| _rs`_process_initialize_mint`_ | Общая логика обработчика для инициализации минта.                                                        | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
| _rs`process_initialize_mint`_  | Публичный обработчик для _rs`InitializeMint`_.                                                           | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L62-L70)       | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L132-L140)     |
| _rs`process_initialize_mint2`_ | Публичный обработчик для _rs`InitializeMint2`_.                                                          | [Источник](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L72-L81)       | [Источник](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L142-L151)     |
...
знаки, полномочия). Чтобы добавить к вашему токену человекочитаемые метаданные,
такие как название, символ и изображение, у вас есть два варианта:
...
<Cards>
  ...
  <Card title="Расширение метаданных Token Extensions" href="/docs/tokens/extensions/metadata">
   Используйте встроенное расширение метаданных с Token-2022. 
   Расширение метаданных доступно только для аккаунтов mint, созданных с помощью Token-2022.
   Чтобы добавить метаданные к аккаунтам mint, созданным с помощью оригинального Token Program, используйте Metaplex Token Metadata Program.
  </Card>
</Cards>
