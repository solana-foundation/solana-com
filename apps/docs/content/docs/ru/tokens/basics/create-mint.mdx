...
description:
  Создайте mint токена и настройте количество десятичных знаков, полномочия на
  эмиссию и полномочия на заморозку.
...
## Что такое mint account

Mint account определяет и уникально идентифицирует токен на Solana, а также
хранит общее состояние, которое применяется ко всем token account для данного
mint.

Token Program определяет тип аккаунта _rs`Mint`_ следующим образом:
...
У каждого токена есть один mint account, и адрес mint является уникальным
идентификатором токена в кошельках, приложениях и обозревателях.

Например, USD Coin (USDC) имеет адрес mint
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`. Адрес mint уникально
идентифицирует USDC во всей экосистеме Solana. Вы можете просмотреть этот mint
на
[Solana Explorer](https://explorer.solana.com/address/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v).

## Как создать аккаунт минта

Создание mint требует двух инструкций:

1. Инструкция _rs`CreateAccount`_ System Program создает новый аккаунт,
   освобожденный от rent, и назначает Token Program в качестве владельца
   программы нового аккаунта.
2. Инструкция _rs`InitializeMint`_ или _rs`InitializeMint2`_ Token Program
   инициализирует новый аккаунт как mint.

Включите инструкцию _rs`CreateAccount`_ и инструкцию инициализации mint в одну
транзакцию.

Во время инициализации mint Token Program проверяет, что mint account еще не
инициализирован и освобожден от rent. Затем Token Program записывает полномочия
на эмиссию, полномочия на заморозку, количество десятичных знаков и флаг
_rs`is_initialized`_ в данные mint account.

### Ссылки на исходный код

| Элемент                        | Описание                                                                                        | Token Program                                                                                                        | Token Extensions Program                                                                                                   |
| ------------------------------ | ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                     | Базовое состояние mint, хранящееся в каждом mint account.                                       | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`InitializeMint`_           | Инструкция инициализации mint, которая ожидает аккаунт rent sysvar в своем списке аккаунтов.    | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L24-L44)   | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L50-L74)   |
| _rs`InitializeMint2`_          | Инструкция инициализации mint, которая не требует аккаунт rent sysvar в своем списке аккаунтов. | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L401-L414) | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L471-L486) |
| _rs`_process_initialize_mint`_ | Общая логика процессора для инициализации mint.                                                 | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
| _rs`process_initialize_mint`_  | Публичный обработчик для _rs`InitializeMint`_.                                                  | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L62-L70)       | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L132-L140)     |
| _rs`process_initialize_mint2`_ | Публичный обработчик для _rs`InitializeMint2`_.                                                 | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L72-L81)       | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L142-L151)     |

### Typescript

Примеры ниже показывают рекомендуемый подход с использованием `Kit`. Устаревшие
примеры с использованием `@solana/kit` включены для справки.
...
Mint account хранит только базовую информацию о токене (общий объем эмиссии,
количество десятичных знаков, полномочия). Чтобы добавить к вашему токену
метаданные в читаемом формате, такие как название, символ и изображение, у вас
есть два варианта:

<Cards>
  <Card title="Metaplex Token Metadata" href="/docs/tokens/metaplex">
   Добавьте метаданные с помощью Metaplex Token Metadata Program. Работает как с оригинальной Token Program, так и с Token-2022.
  </Card>

  <Card title="Token Extensions Metadata" href="/docs/tokens/extensions/metadata">
   Используйте встроенное расширение метаданных в Token-2022. 
   Расширение метаданных доступно только для mint account, созданных с помощью Token-2022.
   Чтобы добавить метаданные к mint account, созданным с помощью оригинальной Token Program, используйте Metaplex Token Metadata Program.
  </Card>
</Cards>
