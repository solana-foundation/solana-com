...
description:
  Создайте минт токена и настройте количество десятичных знаков, полномочие на
  выпуск и полномочие на заморозку.
...
## Что такое минт-аккаунт

Минт-аккаунт определяет и уникально идентифицирует токен в Solana, а также
хранит общее состояние, применимое ко всем token account этого минта.
...
У каждого токена есть один минт-аккаунт, и адрес минта является уникальным
...
идентифицирует USDC во всей экосистеме Solana. Вы можете просмотреть этот минт в
...
2. Инструкция _rs`InitializeMint`_ или _rs`InitializeMint2`_ Token Program
   инициализирует новый аккаунт как минт.

Включите инструкцию _rs`CreateAccount`_ и инструкцию инициализации минта в одну
транзакцию.

Во время инициализации минта Token Program проверяет, что минт-аккаунт ещё не
инициализирован и освобождён от rent. Затем Token Program записывает полномочие
на выпуск, полномочие на заморозку, количество десятичных знаков и флаг
_rs`is_initialized`_ в данные минт-аккаунта.

### Ссылки на исходный код

| Элемент                        | Описание                                                                                   | Token Program                                                                                                        | Token Extensions Program                                                                                                   |
| ------------------------------ | ------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                     | Базовое состояние минта, хранящееся в каждом минт-аккаунте.                                | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`InitializeMint`_           | Инструкция инициализации минта, которая ожидает аккаунт sysvar rent в списке аккаунтов.    | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L24-L44)   | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L50-L74)   |
| _rs`InitializeMint2`_          | Инструкция инициализации минта, которая не требует аккаунт sysvar rent в списке аккаунтов. | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L401-L414) | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L471-L486) |
| _rs`_process_initialize_mint`_ | Общая логика обработки инициализации минта.                                                | [Исходный код](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [Исходный код](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
...
примеры с `@solana/kit` включены для справки.
...
Mint account хранит только базовую информацию о токене (эмиссия, десятичные
знаки, полномочия). Чтобы добавить человекочитаемые метаданные, такие как
название, символ и изображение к вашему токену, у вас есть два варианта:
...
<Cards>
  ...
  <Card title="Token Extensions Metadata" href="/docs/tokens/extensions/metadata">
   Используйте встроенное расширение метаданных с Token-2022. 
   Расширение метаданных доступно только для аккаунтов mint, созданных с Token-2022.
   Чтобы добавить метаданные к аккаунтам mint, созданным с оригинальным Token Program, используйте Metaplex Token Metadata Program.
  </Card>
</Cards>
