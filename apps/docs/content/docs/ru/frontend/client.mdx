...
<Callout type="info">
  Работаете в безголовом режиме — в API-маршруте, воркере или скрипте без
  браузерного кошелька? Используйте сервис управления ключами на бэкенде вместо
  использования raw keypair. Смотрите [Keychain](/docs/tools/keychain) и
  [Подписание в продакшене](/docs/core/transactions/signing-in-production).
</Callout>

## Распространённые паттерны для Solana-разработчиков

- **Безголовые конечные автоматы**: Запускайте клиент внутри API-маршрутов,
  скриптов или воркеров, чтобы переиспользовать одну и ту же логику оркестрации
  кошелька и RPC, которая лежит в основе вашего UI.
- **Дашборды реального времени**: Комбинируйте наблюдателей (балансы, аккаунты,
  подписи) с предпочитаемой UI-библиотекой; клиент берёт на себя
  мультиплексирование WebSocket и инвалидацию кэша.
- **Кастомные хранилища**: Подключите собственное Zustand-хранилище для
  гидратации из IndexedDB/localStorage, зеркалирования состояния в серверные
  сессии или координации между вкладками браузера.
- **Интеграция с React-хуками**: Передайте сконфигурированный экземпляр клиента
  в `@solana/react-hooks`, если хотите удобные хуки поверх того же рантайма.
- **Тестируемость**: Единый интерфейс клиента можно замокировать в юнит-тестах,
  что упрощает симуляцию RPC-ответов или сессий кошелька без наличия браузерного
  кошелька.
