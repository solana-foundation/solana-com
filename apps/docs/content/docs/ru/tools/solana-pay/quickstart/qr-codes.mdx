---
title: Интеграция QR-кодов
description: Генерация и настройка QR-кодов для платежей через мобильные кошельки
---

QR-коды — это наиболее распространенный способ связать веб-приложения с
мобильными кошельками. Solana Pay включает встроенную функцию `createQR` на базе
`@solana/qr-code-styling` — дополнительные пакеты для QR-кодов не требуются.

## Базовая генерация QR-кодов
...
Функция `createQR` принимает необязательные параметры для размера, цвета фона и
...
### Получение параметров QR для расширенного использования

Используйте `createQROptions`, чтобы получить объект конфигурации для более
точного управления:
...
## Использование клиента мерчанта

Клиент мерчанта также предоставляет методы для работы с QR-кодами:
...
### QR-код платежа с обновлением статуса
...
## Интеграция с системами точек продаж

### Отображение QR на POS-терминале
...
### Оптимизированное отображение для мобильных устройств

На мобильных устройствах рекомендуется предоставлять прямую ссылку наряду с
QR-кодом:
