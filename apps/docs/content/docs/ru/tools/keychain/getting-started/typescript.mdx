...
Fordefi поддерживает два режима подписания, выбор которых определяется наличием параметра `chain`. Запросы к API аутентифицируются с помощью подписи ECDSA P-256 — укажите ключ подписания в параметре `privateKeyPem` либо храните его в KMS/HSM, передав `requestSigner`.

**Нативный режим Solana** (рекомендуется для использования в сети): задайте `chain`, и Fordefi самостоятельно подпишет и отправит транзакцию. Поскольку перед подписанием Fordefi может заменить blockhash или комиссии, подписант является Kit `TransactionSendingSigner` — используйте его с `signAndSendTransactionMessageWithSigners`, а не через поток с частичным подписантом, описанный выше.
...
**Режим чёрного ящика**: не указывайте `chain`. Fordefi подписывает сырые байты и не выполняет отправку транзакции — сборку и отправку вы осуществляете самостоятельно. Используйте этот режим с хранилищем Fordefi типа «чёрный ящик».
