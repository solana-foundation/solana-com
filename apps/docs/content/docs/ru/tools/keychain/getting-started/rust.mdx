...
| `fordefi`    | Хранение с MPC-кастодией Fordefi             |
...
Для подписания с использованием MPC-кастодии Fordefi. API-запросы аутентифицируются с помощью подписи запроса ECDSA P-256, извлечённой из `private_key_pem`; чтобы хранить этот ключ в KMS/HSM, реализуйте `FordefiRequestSigner` и используйте `Signer::from_fordefi_with_signer`.

Fordefi поддерживает два режима подписания. Укажите `chain` для **нативного режима Solana**, при котором Fordefi может заменить blockhash или комиссии и самостоятельно транслирует транзакцию. Оставьте `chain` равным `None` для **режима чёрного ящика**, при котором Fordefi подписывает необработанные байты, а трансляцию транзакции выполняете вы самостоятельно.
