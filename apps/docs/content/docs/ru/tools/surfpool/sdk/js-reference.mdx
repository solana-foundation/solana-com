...
description:
  Полный публичный API пакета @solana/surfpool — класс Surfnet, типы конфигурации, параметры развёртывания и структура событий.
---
...
<Callout type="info" title="Точка входа Kit">
  Символы ниже являются корневыми экспортами `@solana/surfpool`. Подпуть `@solana/surfpool/kit` добавляет `surfpool()`, `surfnetCheatcodes()`, `createSurfnetCheatcodesRpc()`, `DEFAULT_SURFNET_ENDPOINT`, типы конфигурации плагина и сгенерированные wire-типы чит-кодов — всё это задокументировано в разделе [Kit Plugin](/docs/tools/surfpool/sdk/kit-plugin), а не в данной таблице.
</Callout>
