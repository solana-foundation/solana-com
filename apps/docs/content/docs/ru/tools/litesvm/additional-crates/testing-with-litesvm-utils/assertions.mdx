...
title: Утверждения
description:
  Проверка состояния аккаунтов, балансов и владения с помощью вспомогательных
  функций утверждений
---

## Существование аккаунта

Проверьте, существуют ли аккаунты или были ли они закрыты.

### Утверждение о существовании аккаунта
...
### Утверждение о закрытии аккаунта
...
<Callout type="info">
  `assert_account_closed` срабатывает, когда аккаунт либо не существует, имеет
  нулевой баланс в lamport, либо содержит пустые данные. Используйте это для
  проверки очистки после закрытия аккаунтов.
</Callout>

## Утверждения о балансе

Проверка балансов SOL и token account.

### Утверждение о балансе SOL
...
### Утверждение о балансе token account
...
### Утверждение о предложении Mint
...
Проверка владения аккаунтом и свойств данных.

### Утверждение о владельце аккаунта
...
### Утверждение о длине данных аккаунта
...
<Callout type="info">
  Стандартные размеры аккаунтов: - **Mint Account**: 82 байта - **Token
  Account**: 165 байт - **System Account**: 0 байт (только lamport)
</Callout>

## Полный пример

Вот исчерпывающий тест, демонстрирующий множество проверок:
...
Когда проверка завершается неудачно, она вызывает панику с подробным сообщением:
...
<Callout type="warn">
  Проверки предназначены для быстрого завершения с понятными сообщениями об
  ошибках. Используйте их повсеместно в тестах, чтобы выявлять проблемы на
  ранних этапах.
</Callout>
