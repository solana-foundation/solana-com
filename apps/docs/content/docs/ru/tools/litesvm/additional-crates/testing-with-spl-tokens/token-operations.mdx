---
title: Операции с токенами
description:
  Работа с операциями над токенами — минтинг, переводы и управление аккаунтами
---

## Создание Token Mint

Минт — это основа любого SPL-токена. Он определяет свойства токена и управляет
тем, кто может выпускать новые токены.
...
Token account хранит баланс определённого spl-token для конкретного
пользователя. Чтобы понять разницу между ним и ATA, прочитайте
...
Associated Token Accounts (ATAs) — это детерминированные адреса для хранения
токенов конкретного пользователя. Чтобы понять разницу между ним и ATA,
прочитайте
[этот раздел](/docs/tools/litesvm/additional-crates/testing-with-spl-tokens#types-of-token-accounts).
...
## Минтинг токенов в Token Account

Когда у вас есть минт и token accounts, вы можете выпускать токены на эти
аккаунты.
...
Переводите токены между аккаунтами с помощью инструкции Transfer.
...
Большинство токенов используют десятичные числа для представления дробных сумм.
Вот как правильно с ними работать:
