...
description:
  Получение и десериализация аккаунтов Anchor с автоматической обработкой
  дискриминаторов
---

## Понимание дискриминаторов аккаунтов Anchor

Программы Anchor добавляют 8-байтовый дискриминатор к данным всех аккаунтов.
Этот дискриминатор представляет собой хэш имени типа аккаунта и используется для
проверки того, что данные аккаунта соответствуют ожидаемому типу.
...
Утилиты аккаунта `anchor-litesvm` обрабатывают этот дискриминатор автоматически.

## Получение аккаунтов Anchor

### Базовое получение аккаунтов
...
<Callout type="info">
  `get_account<T>()` автоматически проверяет 8-байтовый дискриминатор, чтобы убедиться, что данные аккаунта соответствуют ожидаемому типу.
</Callout>

### Десериализация без проверки

Для PDA или пользовательских структур аккаунтов, где проверка дискриминатора
может завершиться неудачей, используйте `get_account_unchecked`:
...
<Callout type="warn">
  Используйте `get_account_unchecked` осторожно. Без проверки дискриминатора вы
  можете десериализовать недопустимые данные. Используйте это только тогда,
  когда вы уверены в типе аккаунта.
</Callout>

## Использование модуля аккаунта напрямую

Вы также можете использовать функции десериализации аккаунтов напрямую:
...
Перечисление `AccountError` предоставляет подробную информацию об ошибках:
...
| Ошибка                         | Описание                                                |
| ------------------------------ | ------------------------------------------------------- |
| `AccountNotFound(Pubkey)`      | По указанному адресу аккаунт не найден                  |
| `DiscriminatorMismatch`        | Дискриминатор аккаунта не соответствует ожидаемому типу |
| `DeserializationError(String)` | Не удалось десериализовать данные аккаунта              |

## Работа с несколькими типами аккаунтов

### Получение аккаунтов различных типов
...
### Проверка существования аккаунта перед получением
...
Вот подробный пример, демонстрирующий управление аккаунтом:
