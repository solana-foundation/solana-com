---
title: Программа и контекст
description:
  Настройка тестовых сред и создание инструкций с использованием AnchorContext и
  Program
---

## Генерация клиентских типов с помощью declare_program!

Anchor 1.0 представляет `declare_program!`, который считывает IDL вашей
программы во время компиляции и генерирует типобезопасные модули
`client::accounts::*` и `client::args::*`. Это рекомендуемый способ построения
инструкций в тестах.
...
<Callout type="info">
  Сначала запустите `anchor build`, чтобы сгенерировать
  `target/idl/my_program.json`. `declare_program!` считывает этот файл во время
  компиляции. Имя макроса должно совпадать с именем крейта вашей программы
  (snake_case).
</Callout>

## Настройка тестовой среды

Конструктор `AnchorLiteSVM` предоставляет гибкий API для настройки вашей
тестовой среды.
...
### Сокращённая запись для одной программы
...
<Callout type="info">
  Если вы не указываете плательщика, `AnchorLiteSVM` автоматически создаёт и
  пополняет его для вас.
</Callout>

## AnchorContext

`AnchorContext` — основной интерфейс для взаимодействия с вашей тестовой средой.
...
## Создание инструкций с помощью Program

Структура `Program` предоставляет гибкий API для построения инструкций, который
повторяет anchor-client. Используйте типы, сгенерированные `declare_program!`,
для обеспечения безопасности типов.

### Базовое построение инструкций
...
<Callout type="info">
  Метод `.accounts()` принимает любой тип, реализующий `ToAccountMetas`, что все
  структуры аккаунтов Anchor делают автоматически. Аналогично, `.args()`
  принимает любой тип, реализующий `InstructionData`.
</Callout>
...
### Использование send_and_confirm_transaction
...
Вот исчерпывающий пример использования паттерна escrow с `declare_program!`:
