...
description: Настройка тестовых аккаунтов и состояния с помощью litesvm-go
---

Демонстрирует непосредственное заполнение состояния аккаунта с помощью `SetAccount` без выполнения транзакций. Это значительно быстрее, чем аирдроп и пополнение через среду выполнения, когда необходимо подготовить множество аккаунтов для теста.
...
## Настройка programdata account

Настройте принадлежащий программе аккаунт данных, сериализовав структуру в поле `data` аккаунта. В этом фрагменте дополнительно импортируется `encoding/binary`:
...
<Callout type="info">
  `SetAccount` работает быстрее, чем выполнение транзакций при массовой настройке аккаунтов, и позволяет сосредоточить код теста на проверяемом поведении, а не на подготовке состояния.
</Callout>

## Ключевые моменты

1. **Прямое состояние** — `SetAccount` записывает состояние аккаунта, минуя среду выполнения.
2. **Освобождение от ренты** — используйте `MinimumBalanceForRentExemption` для вычисления lamport перед созданием аккаунта.
3. **Владение** — `NewAccount` возвращает дескриптор, которым вы продолжаете владеть. SVM хранит собственную копию после вызова `SetAccount`; освободите свою, когда она больше не нужна.
4. **Верификация** — используйте `GetAccount` для считывания данных в дескриптор или `Balance` для проверки только lamport.
