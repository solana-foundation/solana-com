...
description: Загрузка и тестирование пользовательских программ Solana с помощью litesvm-go
---

Тестирование пользовательской программы Solana из Go подразумевает загрузку скомпилированного файла `.so`
в SVM, настройку ожидаемых аккаунтов и отправку инструкций
против неё. `litesvm-go` обрабатывает всё это внутри процесса — validator не требуется.
...
Для тестов, требующих стандартные программы (SPL Token, Memo, ...):
...
1. **Конфигурация** — включите sysvars, builtins, precompiles по мере необходимости.
2. **Загрузка программы** — `AddProgramFromFile` (или `AddProgram` для байтов в памяти).
3. **Настройка аккаунтов** — предварительно заполните аккаунты данных с помощью `SetAccount`.
4. **Отправка** — сформируйте инструкцию с помощью `solana-go`, сериализуйте и отправьте через
   `SendLegacyTransaction`.
5. **Верификация** — считайте аккаунты обратно с помощью `GetAccount` и проверьте логи,
   вычислительные единицы и состояние после выполнения.

Как только этот скелет заработает, масштабируйте его, добавляя больше инструкций, больше
аккаунтов и вспомогательную функцию для шаблонного кода.
