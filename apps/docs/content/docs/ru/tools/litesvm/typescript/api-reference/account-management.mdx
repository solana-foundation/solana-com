...
description: Методы управления аккаунтами, балансами и rent в TypeScript
---
...
| ---------------- | ------------ | ----------------------------- |
| `address`        | `Address`    | Публичный ключ аккаунта            |
...
Вычислить минимальное количество lamport, необходимое для освобождения от rent.
...
Загрузить программу из пути к файлу `.so`.
...
Для тестирования можно эффективно настроить несколько аккаунтов одновременно:
...
<Callout type="info">
  Использование `setAccount` быстрее, чем `airdrop`, при массовой настройке аккаунтов, поскольку не выполняет транзакции.
</Callout>
