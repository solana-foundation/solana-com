...
description: Настройка тестовых аккаунтов и состояния с помощью LiteSVM
---

# Пример настройки аккаунта

Демонстрирует настройку состояния тестового аккаунта напрямую, без транзакций.
...
## Настройка program data account

Настройка аккаунта с данными, принадлежащего программе:
...
<Callout type="info">
  Использование `setAccount` быстрее, чем выполнение транзакций для массовой
  настройки аккаунтов.
</Callout>

## Ключевые моменты

1. **Прямое состояние**: `setAccount` устанавливает состояние аккаунта без
   транзакций
2. **Освобождение от ренты**: Используйте `minimumBalanceForRentExemption` для
   расчёта необходимых lamport
3. **Структура аккаунта**: Укажите все обязательные поля: address, data,
   executable, lamports, programAddress, space
4. **Проверка**: Используйте `getAccount` или `client.rpc.getAccountInfo` для
   проверки
