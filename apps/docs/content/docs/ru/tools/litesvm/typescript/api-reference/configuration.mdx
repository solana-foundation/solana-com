---
title: Конфигурация
description: Методы строителя для настройки LiteSVM на TypeScript
---

LiteSVM использует паттерн строителя для конфигурации. Все методы возвращают `LiteSVM`
для цепочки вызовов.

## Типичная настройка тестов
...
<Callout type="info">
  Отключение верификации подписи ускоряет тесты, но снижает их реалистичность.
</Callout>
...
Включить или отключить проверку истечения срока действия блокхэша.
...
Установить значение lamport по умолчанию для аккаунтов.
...
Установить ограничение на объём вывода логов в байтах.
