...
description: Загрузка и тестирование пользовательских программ Solana с помощью LiteSVM
---

Тестирование пользовательской программы Solana означает загрузку скомпилированного файла `.so` в
SVM, настройку ожидаемых ею аккаунтов и отправку инструкций. LiteSVM обрабатывает всё это внутри процесса — validator не нужен. Мы рекомендуем
генерировать клиенты программ с помощью
...
## Полная настройка тестирования программы
...
<Callout type="info">
  Соберите вашу программу Solana с помощью `cargo build-sbf`, чтобы сгенерировать файл `.so`.
</Callout>

Схема всегда одинакова:

1. **Загрузка программы**: Используйте `addProgramFromFile` для загрузки скомпилированных программ и
   импортируйте myProgramPlugin() из вашего клиента
2. **Конфигурация**: Включите sysvars, builtins и precompiles по мере необходимости
3. **Настройка аккаунтов**: Предварительно заполните аккаунты данных с помощью `setAccount`
4. **Отправка**: Используйте `client.myProgram.testInstruction({..}).sendTransaction()` —
   без ручного формирования транзакций
5. **Проверка**: Проверьте состояние аккаунта после выполнения
   (`client.myProgram.myAccount.fetch`)

Как только этот каркас заработает, вы сможете создать полный набор тестов, добавляя новые инструкции и проверки.
