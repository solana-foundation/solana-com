---
title: Перевод SOL
description: Создание и отправка транзакции перевода SOL с помощью LiteSVM
---

Самая базовая транзакция в Solana: перемещение SOL с одного аккаунта на другой. Это хороший первый тест для проверки работоспособности вашей настройки LiteSVM перед тестированием более сложной логики программ.
...
<Callout type="info">
  Итоговый баланс отправителя немного меньше 9 SOL из-за комиссий за транзакцию (5 000 lamport за подпись).
</Callout>

Здесь рассмотрен простейший тип транзакции. Для тестирования пользовательских программ см. [Тестирование программ](/docs/tools/litesvm/typescript/examples/program-testing). Для переводов токенов (минты, ATA) см. [Тестирование токенов](/docs/tools/litesvm/typescript/tokens).
