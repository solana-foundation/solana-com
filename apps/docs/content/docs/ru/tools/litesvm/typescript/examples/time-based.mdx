...
description: Тестирование логики, зависящей от времени, с управлением часами LiteSVM
---

Демонстрирует использование `warpToSlot` для тестирования логики программы, зависящей от времени.

## Базовое управление часами
...
Вы можете прочитать расписание epoch, чтобы понять соотношение slot/epoch в
кластере:
...
<Callout type="warn">
  ...
  `epoch` или `unixTimestamp`. Используйте `setClock` для полного контроля над полями
  часов.
</Callout>

## Ручное управление часами

Когда ваша программа читает `epoch` или `unix_timestamp` из системной переменной Clock, используйте
`setClock` для прямой установки этих полей:
...
Это аналог паттерна Rust `set_sysvar(&clock)`. Используйте `warpToSlot` для простого
продвижения slot, и `setClock` когда необходим контроль над epoch или временной меткой.

## Варианты использования

Управление временем полезно для тестирования:

| Сценарий                | Подход                           |
| ----------------------- | ---------------------------------- |
| Вестинг токенов           | Перемотка за точку/вехи вестинга |
| Завершение аукционов         | Перемотка за конечный slot аукциона         |
| Стейкинг-вознаграждения         | Используйте `setClock` для установки epoch        |
| Снятие средств с временной блокировкой | Перемотка за slot разблокировки              |
| Ограничение частоты запросов           | Перемотка между разрешёнными интервалами     |
| Истекающие ордера         | Перемотка за время истечения ордера         |

<Callout type="info">
  Помните, что `warpToSlot` изменяет только slot. Временная метка Unix может не
  обновляться пропорционально в зависимости от реализации SVM.
</Callout>

## Ключевые моменты

1. **Включение системных переменных**: Вызовите `withSysvars()` перед использованием методов часов
...
4. **Чтение часов**: Используйте `getClock()` для чтения текущего slot, epoch, временной метки
5. **Паттерн тестирования**: Тестируйте до и после временных границ
