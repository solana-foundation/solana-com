...
> _Примечание: Если `fallback_if_unavailable` имеет значение true, система разрешает транзакциям
> продолжаться при временной недоступности Redis, предотвращая перебои в работе сервиса.
> Установка `max_transactions` в значение 0 разрешает неограниченное количество транзакций._
...
| `get_payer_signer`          | Вернуть следующего плательщика комиссии и адрес платежа        | ✅       | boolean |
...
[документации Solana](https://solana.com/developers/cookbook/transactions/durable-nonces).
...
| ---------------------------- | ------------------------------------------------ |
| `confidential_transfer_mint` | Конфигурация конфиденциальных балансов для минта |
...
| Название расширения                  | Описание                                 |
| ------------------------------- | ------------------------------------------- |
| `confidential_transfer_account` | Состояние конфиденциальных балансов для аккаунта |
