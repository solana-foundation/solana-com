...
difficulty: advanced
title: "Фиксация NAV для фондов денежного рынка на Solana"
description:
...
Фиксация NAV (чистой стоимости активов) — это заранее установленные моменты торгового дня, в которые определяется цена паёв фонда и исполняются заявки. Традиционные фонды, как правило, имеют одну дневную точку фиксации в момент закрытия рынка, однако скорость и низкие издержки Solana позволяют проводить несколько внутридневных фиксаций, предоставляя инвесторам большую гибкость.

## Что такое NAV?

Фонд — это пул активов, которым совместно владеют множество инвесторов. **NAV (чистая стоимость активов)** — это просто цена одного пая фонда. Представьте себе следующее:

```
NAV = (Total Value of Everything in the Fund - liabilities) ÷ Number of Shares
```

Например, если фонд владеет активами на $10 млн и в обращении находится 10 млн паёв, каждый пай стоит $1,00.

**Почему NAV важен?**

- **Покупка паёв (подписка)**: вы платите текущий NAV. Если NAV равен $1,02 и вы инвестируете $102, вы получаете 100 паёв.
- **Продажа паёв (погашение)**: вы получаете текущий NAV. Если вы погашаете 100 паёв при NAV $1,02, вы получаете $102.

Фонды денежного рынка, как правило, поддерживают стабильный NAV на уровне $1,00 с небольшими колебаниями в зависимости от начисленных процентов.

## Проблема

При инвестировании в фонд денежного рынка традиционно два события происходят в разное время:

1. Вы подаёте заявку → Ждёте закрытия рынка
2. Рассчитывается NAV → Паи выпускаются/погашаются на T+1 или T+2

Это порождает ряд проблем:

- **Задержка исполнения**: заявки, поданные утром, ожидают до 16:00
- **Задержка расчётов**: ваши средства находятся в подвешенном состоянии 1–2 дня
- **Ограниченная гибкость**: только одна возможность совершить транзакцию в день
- **Устаревшие цены**: NAV может устареть в промежутке между подачей заявки и её исполнением

## Решение

**Фиксация NAV на Solana** обеспечивает несколько ежедневных расчётных окон с атомарным исполнением. Заявки накапливаются в очереди и мгновенно исполняются в каждый момент фиксации по текущему NAV.
...
В данном руководстве демонстрируется реализация фиксации NAV на Solana с использованием расширений SPL Token 2022 для соответствующего выпуска паёв фонда и стандартного USDC для расчётов — без написания собственных программ на Rust.

<Callout type="info" title="Образовательная эталонная реализация">

Вы можете воспользоваться
[исходным кодом](https://github.com/Woody4618/nav-strikes-reference) данной
реализации, чтобы протестировать фиксацию NAV локально.

Данное руководство представляет собой эталонную реализацию исключительно в ознакомительных и образовательных целях. **НЕ используйте этот код напрямую в продакшене** без:
...
Система фиксации NAV состоит из следующих ключевых компонентов:

1. **Токен паёв фонда**: SPL Token 2022 с метаданными, хранящими NAV в блокчейне
2. **Расчётная валюта**: стандартный USDC (существующий SPL-токен)
3. **Администратор фонда**: оркестрирует фиксации через делегированные полномочия
4. **Система белого списка**: контролирует, какие адреса могут держать паи фонда

### Ключевые принципы проектирования

- **Без собственных программ**: используются только Token Extensions Program и стандартный USDC
- **Атомарные расчёты**: одна транзакция гарантирует, что подписка/погашение либо завершается полностью, либо отменяется целиком
- **Состояние заморозки по умолчанию**: паи требуют явного включения в белый список для соответствия нормативным требованиям
- **Делегированные полномочия**: администратор фонда действует без принятия активов под стражу
- **NAV в блокчейне**: текущая цена хранится в метаданных токена и публично верифицируема

### Этап 1: Настройка фонда

1. Создание токена паёв фонда с расширениями метаданных
2. Инициализация NAV на уровне $1,00
3. Установка ежедневного расписания фиксаций
4. Включение институциональных инвесторов в белый список

### Этап 2: Период перед фиксацией

1. Инвесторы подают заявки на подписку/погашение
2. Заявки ставятся в очередь до следующего времени фиксации
3. Фонд рассчитывает предварительный NAV
4. Выполняются проверки рисков

### Этап 3: Исполнение фиксации NAV
...
### Этап 4: После фиксации

1. Формирование торговых подтверждений
2. Обновление состава фонда
3. Подготовка к следующей фиксации
4. Сверка с кастодианом

### Паттерн делегированных полномочий

Инвесторы заранее авторизуют администратора фонда на перемещение их токенов, после чего администратор исполняет операции в момент фиксации без необходимости получения подписей от инвесторов:
...
### Поток погашения (паи фонда → USDC)
...
## Почему Solana подходит для фиксации NAV?

Архитектура Solana предоставляет значительные преимущества по сравнению с традиционными расчётами по фондам:

| Параметр            | Традиционный (T+1) | Фиксация NAV на Solana          |
| ----------------- | ----------------- | --------------------------- |
| Логика расчётов  | Трансфер-агенты   | Атомарная группировка транзакций |
| Время расчётов   | 1–2 дня          | &lt;1 секунды                |
| Стоимость транзакции  | $50–200           | &lt;$0,01                   |
| Фиксаций в день   | 1                 | 4+ (настраивается)           |
| Риск устаревших цен | Высокий              | Ниже                       |
| Прозрачность NAV  | Отчёт по итогам дня | В блокчейне, в реальном времени          |

Атомарные транзакции Solana устраняют посредников, обеспечивая мгновенные, дешёвые и безопасные расчёты с прозрачностью NAV в блокчейне.

## Токен фонда с Token Extensions Program

SPL Token 2022 предоставляет мощные расширения, позволяющие осуществлять соответствующий нормативным требованиям выпуск паёв фонда без написания собственных программ:

### Необходимые расширения для паёв фонда

1. **Расширение состояния аккаунта по умолчанию**: переводит все новые token account в состояние заморозки по умолчанию, требуя явного включения в белый список (соответствие требованиям KYC/AML)
2. **Расширение метаданных**: хранит NAV, расписание фиксаций и AUM в блокчейне для обеспечения прозрачности

### Конфигурация прав доступа

- **Право на минтинг**: администратор фонда (контролирует выпуск паёв)
- **Право на заморозку**: администратор фонда (управляет белым списком)
- **Право на обновление**: администратор фонда (обновляет метаданные NAV)

<Callout type="info">

Состояние заморозки по умолчанию критически важно для соответствия нормативным требованиям. Оно гарантирует, что только явно включённые в белый список (прошедшие проверку KYC) адреса могут получать и держать паи фонда.

</Callout>

### Поля метаданных в блокчейне
...
## Полная реализация фиксации NAV

### Настройка движка фиксации NAV

Обратите внимание, что в данном руководстве для создания движка фиксации NAV используется библиотека @solana/kit. Реализацию на `web3.js` также можно найти в
[исходном коде](https://github.com/Woody4618/nav-strikes-reference). Для начала создайте основной класс движка, который обрабатывает все операции фонда:
...
<Callout type="tip" title="Динамическое обновление метаданных">

Для паёв фонда поля `currentNAV`, `lastStrikeTime` и `totalAUM` обновляются в блокчейне при каждой фиксации NAV с помощью
`getUpdateTokenMetadataFieldInstruction`. Это обеспечивает:

- **Видимость NAV в реальном времени** для всех участников
- **Журнал аудита** всех обновлений NAV в блокчейне
- **Интеграцию** с DeFi-протоколами, способными считывать метаданные из блокчейна
...
Ниже приведён полный пример, демонстрирующий целый день операций по фиксации NAV:
...
Перед развёртыванием в продакшене убедитесь, что вы решили следующие вопросы:

1. **Безопасность**: профессиональное управление ключами, контроль на основе мультиподписи и комплексный аудит безопасности кодовой базы
2. **Соответствие нормативным требованиям**: регистрация ценных бумаг, системы KYC/AML, ограничения на передачу и обязательная отчётность
3. **Управление ключами**: институциональные решения для хранения с надлежащими процедурами резервного копирования и восстановления
4. **Обработка транзакций**: приоритетные комиссии, логика повторных попыток, обработка подтверждений и резервирование RPC
5. **Операции фонда**: расчёт NAV из авторитетных источников, интеграция с кастодианом и сверка
6. **Мониторинг**: отслеживание в реальном времени, оповещения и автоматическая регуляторная отчётность

## Следующие шаги

После изучения принципов фиксации NAV на Solana:

1. **Изучите Token Extensions**: узнайте о других
   [расширениях Token 2022](/docs/tokens/extensions/transfer-hook), например о постоянном делегате, для реализации дополнительных функций соответствия требованиям

2. **Реализуйте Token ACL**: для расширенного соответствия требованиям рассмотрите использование
   [Token ACL](/docs/tokenization/token-acl) для управления белым списком инвесторов
   с разморозкой без разрешений. Это позволяет реализовать самостоятельную верификацию KYC, при которой инвесторы могут автоматически активировать свои аккаунты после прохождения проверки соответствия требованиям, снижая операционную нагрузку при сохранении регуляторного контроля.

3. **Изучите расчёты DvP**: см. наше
   [руководство по расчётам «поставка против платежа»](/docs/tokenization/dvp) для паттернов расчётов с ценными бумагами

4. **Производственная архитектура**: проектирование надёжных систем планирования, мониторинга и аварийного восстановления

## Заключение

Модель атомарных транзакций Solana и Token Extensions Program образуют мощную основу для реализации нескольких ежедневных фиксаций NAV для фондов денежного рынка. Совокупность:

- Нативного атомарного исполнения (без рисков смарт-контрактов)
- Финализации за доли секунды
- Практически нулевых транзакционных издержек
- Прозрачности NAV в блокчейне
- Встроенных функций соответствия требованиям (заморозка по умолчанию, метаданные)

Делает Solana идеальной платформой для модернизации инфраструктуры расчётов по фондам и предоставления инвесторам более частых возможностей для торговли.

Однако переход от данной образовательной эталонной реализации к продакшену требует значительной дополнительной работы в области безопасности, соответствия нормативным требованиям, хранения активов и операционной деятельности. При токенизации фондов всегда привлекайте квалифицированных юридических, регуляторных и технических специалистов.
