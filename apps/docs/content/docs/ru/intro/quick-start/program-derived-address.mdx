...
description:
  Узнайте, как создать CRUD-программу (создание, чтение, обновление, удаление)
  на Solana с использованием Program Derived Address (PDA) и фреймворка Anchor.
  Это пошаговое руководство демонстрирует, как создавать, обновлять и удалять
  ончейн-аккаунты сообщений с помощью PDA, реализовывать валидацию аккаунтов и
  писать тесты. Идеально подходит для разработчиков, желающих понять, как
  использовать PDA в программах Solana.
...
В файле `lib.rs` вы найдёте программу с инструкциями [`create`](mention:one),
...
что стартовая программа успешно собирается.
...
представляют данные аккаунта (тип данных для хранения в поле data аккаунта).
...
Поле _rs`message`_ хранит _rs`String`_, содержащий сообщение пользователя.

### !bump

Поле _rs`bump`_ сохраняет _rs`u8`_ ["bump" seed](/docs/core/pda#canonical-bump),
используемый для получения program derived address (PDA). Хранение этого
значения экономит вычисления, так как нет необходимости пересчитывать его в
...
- `bump` — _rs`u8`_, который хранит ["bump" seed](/docs/core/pda#canonical-bump)
  ...
  вычисления, так как нет необходимости пересчитывать его в следующих
  инструкциях.
...
данных _rs`MessageAccount`_. В разделе тестирования показан процесс создания и
чтения данных аккаунта.
...
Ограничение _rs`mut`_ объявляет аккаунт изменяемым.
...
Ограничение _rs`space`_ указывает количество байт, выделяемых для поля данных
нового аккаунта.
...
2. Необязательные ограничения (например, _rs`#[account(mut)]`_ или
   _rs`#[account(init)]`_), которые задают дополнительные требования
...
1. _rs`user: Signer<'info>`_
   ...
   - Требует статуса изменяемого (_rs`#[account(mut)]`_), так как он оплачивает
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   - Новый аккаунт, в котором хранится сообщение пользователя
   - Ограничение `init` создаёт аккаунт во время выполнения инструкции
   - Ограничения `seeds` и `bump` формируют адрес аккаунта как Program Derived
     Address (PDA)
   - `payer = user` определяет, кто оплачивает создание нового аккаунта
   - `space` выделяет необходимое количество байт для поля data аккаунта

3. _rs`system_program: Program<'info, System>`_
   - Необходим для создания аккаунта
   - Внутри, ограничение `init` вызывает System Program для создания нового
     аккаунта с указанным `space` и меняет владельца на текущую программу.

---

Аннотация _rs`#[instruction(message: String)]`_ позволяет структуре _rs`Create`_
получать доступ к параметру _rs`message`_ из инструкции `create`.
...
Вычисление `space` _rs`(8 + 32 + 4 + message.len() + 1)`_ выделяет место для
типа данных `MessageAccount`:

- Дискриминатор (идентификатор) Anchor Account: 8 байт
...
1. Выведите сообщение в логи программы с помощью макроса _rs`msg!()`_.
...
2. Инициализация данных аккаунта:
   ...
   - Устанавливает поле `message` значением `message` из аргумента функции.

   ```rs
   account_data.message = message;
   ```

   - Устанавливает значение `bump`, используемое для получения PDA, взятое из
...
Далее добавьте инструкцию `update` для изменения `MessageAccount` на новое
сообщение.
...
Ограничение _rs`realloc::payer`_ указывает аккаунт, который оплачивает
перераспределение.
...
1. _rs`user: Signer<'info>`_
   ...
   - Отмечен как изменяемый (_rs`#[account(mut)]`_), так как может оплачивать
     дополнительное место для `message_account` при необходимости
   - Должен подписать транзакцию

2. _rs`message_account: Account<'info, MessageAccount>`_
   - Существующий аккаунт, в котором хранится сообщение пользователя для
     обновления
   - Ограничение `mut` указывает на модификацию данных этого аккаунта
...
bump seed, сохранённый в _rs`message_account`_, вместо того чтобы Anchor
пересчитывал его заново.

Атрибут _rs`#[instruction(message: String)]`_ позволяет структуре _rs`Update`_
получать доступ к параметру _rs`message`_ из инструкции _rs`update`_.
...
1. Вывести сообщение в логи программы с помощью макроса _rs`msg!()`_.

2. Обновить данные аккаунта:
   - Получить доступ к `message_account` из контекста.
   - Установить поле `message` в новое значение `message` из аргумента функции.
...
Ограничение _rs`seeds`_ указывает seed'ы, используемые для получения PDA.

### !bump

Ограничение _rs`bump`_ указывает bump seed для PDA.

В этом случае программа использует уже сохранённый bump seed из
...
В этом случае аккаунт _rs`user`_ получает лампорты с закрытого
_rs`message_account`_.
...
1. _rs`user: Signer<'info>`_
   ...
   - Отмечен как изменяемый (_rs`#[account(mut)]`_), чтобы получить лампорты с
     закрытого аккаунта
   - Должен подписывать, чтобы только правильный пользователь мог закрыть свой
     аккаунт сообщения
...
1. _rs`_ctx: Context<Delete>`_ — обеспечивает доступ к аккаунтам, указанным в
   ...
   использует Context в своём теле.

Тело функции просто выводит сообщение в логи программы с помощью макроса
_rs`msg!()`_ . Дополнительная логика не требуется, так как ограничение
...
Теперь вы завершили базовую CRUD-программу. Задеплойте программу, выполнив
...
В стартовом коде также есть тестовый файл в `anchor.test.ts`.
...
Добавьте код ниже внутрь _ts`describe()`_, но перед секциями _ts`it()`_.
...
Solana Playground убирает часть шаблонного кода: [`pg.program`](mention:one)
предоставляет доступ к методам для взаимодействия с программой, а
[`pg.wallet`](mention:two) даёт доступ к вашему кошельку Playground.
...
проверить, что он работает как ожидается. Далее будут добавлены сами тесты.
...
передавая новое сообщение "Hello, Solana!".
...
Метод _ts`fetchNullable()`_ получает данные аккаунта из сети, которые могут
отсутствовать.
...
аккаунта по его адресу (`messagePda`) с помощью `fetchNullable`. Этот метод
...
devnet, и выводит ссылки на SolanaFM для просмотра деталей транзакции.
...
<Callout type="info">
  ...
  существует как аккаунт. Для одного PDA может существовать только один аккаунт.
</Callout>
