...
Инструкции `update` и `delete` требуют изменений для обработки переводов SOL
между аккаунтами с помощью вызова System Program.
...
Сначала программе нужен простой механизм "pay-to-update" — для этого измените
структуру _rs`Update`_ и функцию `update`.

Начните с обновления файла `lib.rs`, чтобы подключить элементы из модуля
`system_program`.
...
Далее обновите структуру _rs`Update`_, чтобы добавить новый аккаунт с именем
`vault_account`. Этот аккаунт, управляемый программой, получает SOL от
пользователя при обновлении его аккаунта сообщения.
...
В этом разделе добавляется новый аккаунт с именем `vault_account` в структуру
_rs`Update`_ . Этот аккаунт служит программным "хранилищем", которое получает
SOL от пользователей при обновлении их сообщений.
...
Ключевые особенности `vault_account`:

- Адрес аккаунта формируется из PDA, полученного с помощью seeds
...
- Как тип _rs`SystemAccount`_, System Program владеет им так же, как обычными
  аккаунтами-кошельками
...
В инструкции `delete` вы увидите, как программа может "подписывать" этот PDA при
CPI.
...
Структура _rs`CpiContext`_ указывает программу и аккаунты для межпрограммного
вызова (CPI).
...
В инструкции `update` реализация включает межпрограммный вызов (CPI) для вызова
инструкции `transfer` в System Program. Это демонстрирует, как выполнять CPI
внутри программы, что обеспечивает компонуемость программ Solana.

Структура _rs`Transfer`_ указывает необходимые аккаунты для инструкции перевода
в System Program:

- `from` — аккаунт пользователя (источник средств)
- `to` — аккаунт-хранилище (получатель средств)
...
Затем функция `transfer` вызывает инструкцию перевода в System Program,
передавая:

- `cpi_context` (программа и аккаунты)
- `amount` для перевода (1 000 000 лампортов или 0,001 SOL)
...
Настройка для CPI аналогична построению клиентских инструкций, где вы указываете
программу, аккаунты и instruction data для конкретной инструкции. Когда
инструкция `update` программы получает вызов, она внутренне вызывает инструкцию
перевода в System Program.
...
Сначала обновите структуру _rs`Delete`_ и добавьте `vault_account`. Это позволит
переводить все SOL из хранилища обратно пользователю при закрытии его аккаунта
сообщений.
...
Также добавьте `system_program`, так как для CPI-перевода требуется вызов System
Program.
...
`vault_account` использует ту же схему получения PDA, что и в структуре Update.
...
перевести все накопленные SOL обратно пользователю.
...
_rs`signer_seeds`_ указывают опциональные seeds и bump seeds, используемые для
получения PDA.

### !with_signer

Метод _rs`with_signer()`_ передаёт seeds подписи вместе с CPI.
...
- `cpi_context` (программа, аккаунты и PDA-подписант)
...
Далее обновите файл `anchor.test.ts`, чтобы добавить новый аккаунт хранилища в
инструкции. Для этого нужно получить vault PDA и включить его в вызовы
инструкций update и delete.
...
Затем обновите инструкцию update, чтобы включить [`vaultAccount`](mention:one)
...
Затем обновите инструкцию delete, чтобы включить [`vaultAccount`](mention:one)
...
список проектов в Solana Playground. После импорта проекта все изменения будут
сохраняться и сохраняться автоматически.
