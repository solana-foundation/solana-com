...
пользователя, когда тот обновляет свой аккаунт сообщений.
...
- Как тип _rs`SystemAccount`_, он принадлежит System Program, как и обычные
  кошельки
...
внутри программы, что позволяет создавать компонуемые программы Solana.
...
Настройка CPI аналогична построению клиентских инструкций, где вы указываете
...
позволит переводить все SOL из хранилища обратно пользователю при закрытии его
аккаунта сообщения.
...
Метод _rs`with_signer()`_ передаёт seeds подписанта вместе с CPI.
...
инструкции. Для этого потребуется получить PDA хранилища и включить его в вызовы
