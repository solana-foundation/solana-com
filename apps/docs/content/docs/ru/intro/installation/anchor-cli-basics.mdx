...
Например, следующая команда создаст проект с именем `my-project`. Каталог
...
Скомпилированную программу можно найти в каталоге `/target/deploy`.

<Callout type="warn">

При запуске `anchor build` если вы столкнётесь со следующими ошибками:
...
<Callout>
  ...
  validator, развёртывает программу, выполняет тесты, затем останавливает
  validator.
</Callout>
