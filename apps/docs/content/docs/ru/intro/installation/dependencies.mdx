...
Далее закройте Windows Terminal и снова откройте терминал Linux, найдя Ubuntu.
Теперь терминал должен выглядеть как на изображении ниже, и `ctrl + v`
(сочетание клавиш для вставки) будет работать.
...
<Callout>
Если при установке `protobuf-compiler` возникает следующая ошибка, сначала выполните _shell`sudo apt-get update`
...
<Callout>
  ...
  обеспечения, которую вы хотите установить (например, `v2.0.3`), или
  использовать одно из трёх символических названий каналов: `stable`, `beta` или
  `edge`.
</Callout>

2. Добавьте переменную окружения PATH

При первой установке Solana CLI может появиться запрос на добавление переменной
среды `PATH`. Для этого закройте и снова откройте терминал или выполните в shell
следующую команду:
...
- Если в выводе содержится `/bash`, используйте `.bashrc`.
- Если в выводе содержится `/zsh`, используйте `.zshrc`.
...
полученной при установке, не сохраняется после закрытия терминала. Вместо этого
добавьте PATH в конфигурационный файл shell, выполнив следующую команду:
...
<Callout>
  Приведённые выше шаги не применяются к пользователям `bash`, так как изменения
...
Файл теста Anchor по умолчанию (TypeScript), созданный с помощью команды
_shell`anchor init`_, требует Node.js и Yarn. (Шаблон теста на Rust доступен
через _shell`anchor init --test-template rust`_.)
...
3. Используйте `nvm`, чтобы установить Node:
