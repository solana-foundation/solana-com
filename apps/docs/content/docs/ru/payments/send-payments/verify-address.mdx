...
После получения адреса ATA проверьте, существует ли token account в блокчейне.
Если ATA не существует, вы можете включить инструкцию для создания token account
получателя в той же транзакции, что и перевод. Однако для этого потребуется
оплатить rent для нового token account. Поскольку получатель владеет ATA, SOL,
