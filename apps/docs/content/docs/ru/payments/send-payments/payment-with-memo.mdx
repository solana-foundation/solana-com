---
title: Платёж с мемо
description: Узнайте, как прикреплять идентификаторы счетов или заметки к платежу.
---

Программа memo в Solana позволяет прикреплять номера счетов, идентификаторы
заказов или пользовательские ссылки к любому платежу. Эти memo записи навсегда
сохраняются в блокчейне и отображаются в логах транзакций, что позволяет легко
сопоставлять платежи с вашими внутренними системами.

## Как это работает

Мемо-программа записывает текст в логи транзакции. Эти логи индексируются
эксплорерами и RPC-провайдерами, что делает мемо доступными для поиска и сверки
платежей.
...
Чтобы добавить мемо, необходимо формировать инструкции напрямую, что даёт вам
полный контроль над содержимым транзакции.

Ниже приведены основные шаги. Полный рабочий пример смотрите в разделе
[Демо](#demo).

<ScrollyCoding>

## !!steps Импорт Memo Program

Импортируйте `getAddMemoInstruction` из `@solana-program/memo` для создания
инструкций мемо.

<CodePlaceholder title="Платёж с мемо" />
...
## !!steps Создание инструкции перевода

Создайте инструкцию перевода токенов, указав исходный ATA, целевой ATA,
авторитет (подписант) и сумму в базовых единицах.
...
## !!steps Создание инструкции мемо

Создайте инструкцию мемо с сообщением. Это сообщение будет видно в логах
программы транзакции.
...
## !!steps Отправка транзакции с мемо

Объедините инструкции перевода и мемо в одной транзакции.
...
## !!steps Просмотр Memo в журналах транзакций

После подтверждения транзакции получите её, чтобы просмотреть memo в логах. Memo
появится как сообщение журнала от программы Memo.

<CodePlaceholder title="Платёж с Memo" />
