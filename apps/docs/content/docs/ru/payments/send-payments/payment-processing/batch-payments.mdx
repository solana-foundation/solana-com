...
description: Узнайте, как отправлять платежи нескольким получателям в одной транзакции.
---
...
увеличивает ограничение размера до 4096 байт, что позволяет уместить примерно в три раза больше
переводов в одной транзакции. Пакетная обработка по-прежнему необходима за пределами этого лимита, поскольку
среда выполнения загружает не более 64 аккаунтов на транзакцию независимо от формата.
