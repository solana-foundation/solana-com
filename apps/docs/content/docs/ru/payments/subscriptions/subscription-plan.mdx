...
Подписчик принимает текущие условия плана. PDA подписки выводится из PDA плана и
адреса подписчика.
...
Продавец или внесённый в белый список получатель подписывает операцию сбора
средств. Когда план использует список разрешённых получателей, владелец
токен-аккаунта получателя должен быть указан в `destinations`.
...
Отмена помечает подписку как завершающуюся. Аннулирование закрывает PDA подписки
после истечения срока отмены. Обе транзакции подписываются подписчиком.
...
- TypeScript SDK получает актуальные условия плана во время `subscribe`, когда
  вы их не указываете.
...
- Только продавец или кошелёк, указанный в `pullers`, может получать платежи.
- Подписчик подписывает транзакции настройки, отмены и отзыва. Продавец или
  утверждённый инициатор списания подписывает транзакции сбора средств.
