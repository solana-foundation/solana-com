---
title: Периодическое делегирование
description: Руководство по настройке периодических транзакций делегирования.
---

Периодическое делегирование позволяет пользователю одобрить другому кошельку или
сервису снятие средств до определённого лимита, который сбрасывается каждый
период.

Это руководство демонстрирует все составляющие процесса. Сначала вы создаёте
аккаунты, затем инициализируете Subscription Authority при необходимости,
создаёте периодическое делегирование, после чего используете этот PDA
делегирования для переводов.
...
2. Получение token account пользователя, PDA Subscription Authority и PDA
   периодического делегирования.
3. Инициализация Subscription Authority, если она ещё не существует.
4. Создание периодического делегирования с указанием суммы периода, длительности
   периода, времени начала и срока действия.
...
Делегат подписывает каждый перевод. Программа проверяет текущий период и
отклоняет переводы, которые превысят оставшийся лимит периода.
...
Делегирующий может отозвать повторяющееся делегирование в любое время. Отзыв
закрывает PDA делегирования и возвращает его rent подписанту.
...
- `amountPerPeriod` указывается в базовых единицах. Для токена с 6 десятичными
  знаками `1_000_000` означает `1` токен.
- Программа отклоняет переводы, превышающие оставшийся лимит текущего периода.
- С началом следующего периода использованная сумма сбрасывается.
- `expiryTs` — жёсткое ограничение: после его наступления переводы завершаются
  ошибкой, а спонсор может вернуть rent. Льготного периода для расходования
  средств не предусмотрено.
- Установите `startTs` в `0`, чтобы делегирование начиналось в момент попадания
  транзакции в блокчейн, а не в фиксированное время. Это расширяет окно для
  получения подписи пользователя и его подключения. В таком случае `expiryTs`
  должен быть ненулевым — делегирование с началом при приземлении транзакции не
  может быть одновременно бессрочным.
