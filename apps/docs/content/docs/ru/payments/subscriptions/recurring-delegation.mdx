...
- Программа отклоняет переводы, которые превышают оставшийся лимит текущего
  периода.
- После начала следующего периода снятая сумма сбрасывается.
- Пользователь подписывает транзакции настройки и отзыва. Уполномоченный
  подписывает переводы.
