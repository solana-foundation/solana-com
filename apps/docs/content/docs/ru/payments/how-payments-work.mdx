...
В Solana программы представляют собой исполняемую логику, которая управляет
состоянием аккаунтов. Токеновые аккаунты управляются **Token Program** — это
ончейн-код, который проверяет переводы и атомарно обновляет балансы.
...
Token-2022 (также известный как «Token Extensions») добавляет такие функции, как хуки переводов,
комиссии за переводы и конфиденциальные балансы. Оба программы работают аналогично для базовых
переводов, но при получении ATA необходимо использовать правильную программу.
...
<Callout type="info">
  Solana использует локальные рынки комиссий — транзакции конкурируют с другими транзакциями,
  которые записывают в то же состояние. Это означает, что комиссии за платежи остаются низкими и предсказуемыми
  даже в периоды высокой сетевой активности в других местах.
</Callout>
...
<Cards>
  ...
  <Card title="Отправьте свой первый платёж" href="/docs/payments/send-payments">
