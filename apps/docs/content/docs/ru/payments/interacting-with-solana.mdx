...
<Callout type="caution" title="Не используйте публичные RPC в продакшене">
  Публичные конечные точки (`api.mainnet.solana.com`, `api.devnet.solana.com`) имеют
  ограничения по частоте запросов, не имеют соглашения об уровне обслуживания и не подходят для производственных платёжных потоков.
  Используйте [провайдера RPC](https://solana.com/rpc), чтобы получить приватную конечную точку RPC
  для производственного развёртывания.
</Callout>
