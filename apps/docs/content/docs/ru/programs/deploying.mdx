...
- **Program ID**: Адрес вашей программы в блокчейне. Пользователи
  взаимодействуют с вашей программой, ссылаясь на Program ID.
- **Program Account**: Аккаунт в блокчейне, который хранит метаданные вашей
  программы. Адрес program account является Program ID. Program account также
  хранит адрес ProgramData Account.
- **ProgramData Account**: Аккаунт в блокчейне, который хранит развернутый
  исполняемый код вашей программы.
- **Program Authority**: Аккаунт, имеющий разрешение на обновление или закрытие
  программы. По умолчанию это ваш CLI-кошелек.
...
Program ID — это адрес вашего program account в блокчейне. Пользователи и другие
программы вызывают вашу программу, ссылаясь на этот ID в своих транзакциях.
...
- **`--use-rpc`**: Отправлять транзакции в настроенный RPC. Этот флаг требует
  [stake-weighted](/docs/defi/stake-weighted-qos) RPC соединения от таких
  провайдеров, как [Triton](https://triton.one/) или
