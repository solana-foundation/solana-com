...
<Callout type="warn">
  Обновите поле `edition` в файле `Cargo.toml` на `2021`. В противном случае при
...
Программы Solana можно обновлять, повторно развёртывая их на тот же program ID.
...
Задеплойте программу повторно, используя ту же команду `solana program deploy`.
