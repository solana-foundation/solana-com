...
Bump seed — это один байт (u8), добавляемый к опциональным сидам во время
деривации.
[`find_program_address`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L800)
производит поиск от 255 вниз до 1, вызывая _rs`create_program_address`_ с каждым
значением до тех пор, пока результат не окажется вне кривой Ed25519. Первое
значение, которое срабатывает, является **каноническим bump**.
...
Выведение PDA вычисляет только адрес. Оно не создаёт онchain-аккаунт по этому
адресу. Аккаунт должен быть явно создан отдельной инструкцией (обычно
...
<Callout type="warn">
  Это отличается от текущего поведения канонического поиска,
  задокументированного для `sol_try_find_program_address`, который выполняет
  итерацию от 255 вниз до 1.
</Callout>

<Callout type="info">
  Пример Kit не включён, поскольку функция
...
В этом примере bump 255 генерирует адрес на кривой и терпит неудачу. Первый
допустимый bump — 254, что делает его каноническим bump.
