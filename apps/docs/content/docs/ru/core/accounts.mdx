...
[10 МБ](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/system_instruction.rs#L85)
и содержит следующую информацию:

<WithMentions>

- [`lamports`](mention:lamports): Количество лампортов в аккаунте
- [`data`](mention:data): Данные аккаунта
- [`owner`](mention:owner): ID программы, которой принадлежит аккаунт
- [`executable`](mention:executable): Показывает, содержит ли аккаунт
...
Это поле часто называют «данные аккаунта». `data` в этом поле считаются
произвольными, так как могут содержать любую последовательность байтов. Каждая
программа определяет структуру данных, хранящихся в этом поле.
...
1. Получение аккаунта по его [адресу](#account-address)
2. Десериализация поля данных аккаунта `data` из сырых байтов в соответствующую
   структуру данных, определённую программой-владельцем аккаунта.
...
У каждого аккаунта Solana есть [программа](/docs/core/programs), назначенная его
владельцем. Только программа-владелец аккаунта может изменять его `data` или
...
- Если `true`: Аккаунт является program account
- Если `false`: Аккаунт является data account
...
На диаграмме ниже показано, что для деплоя аккаунта программы используется
загрузчик (loader program). Поле `data` аккаунта программы содержит исполняемый
код программы.
...
Программы, развернутые с помощью loader-v3, не содержат код программы в своем
поле `data`. Вместо этого их поле `data` указывает на отдельный **program data
account**, который содержит код программы. (См. диаграмму ниже.)
...
В примере ниже происходит получение Token Program аккаунта. Обратите внимание,
что поле `executable` установлено в значение `true`, что указывает на то, что
аккаунт является программой.
...
Программы используют аккаунты данных для хранения своего состояния. Для этого им
сначала нужно создать новый аккаунт данных. Процесс создания аккаунта состояния
программы часто абстрагируется, но полезно понимать, как он устроен на самом
деле.
...
Не всем аккаунтам назначается новый владелец после их создания с помощью System
Program. Аккаунты, принадлежащие System Program, называются системными
аккаунтами. Все кошельки являются системными аккаунтами, что позволяет им
оплачивать [транзакционные комиссии](/docs/core/fees).
...
В примере ниже создается новая keypair и пополняется SOL. После выполнения кода
вы увидите адрес поля `owner` аккаунта — это `11111111111111111111111111111111`
