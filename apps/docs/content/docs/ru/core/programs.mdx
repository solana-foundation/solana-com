...
[развернул программу](/docs/programs/deploying).) Если права управляющего
...
| v1        | `BPFLoader1111111111111111111111111111111111` | Инструкции по управлению отключены, но программы продолжают выполняться                                                  | —                                                                                                                                             |
| v2        | `BPFLoader2111111111111111111111111111111111` | Инструкции по управлению отключены, но программы продолжают выполняться                                                  | [Инструкции](https://docs.rs/solana-loader-v2-interface/latest/solana_loader_v2_interface/enum.LoaderInstruction.html)                        |
| v3        | `BPFLoaderUpgradeab1e11111111111111111111111` | Программы можно обновлять после развертывания. Исполняемый файл программы хранится в отдельном аккаунте данных программы | [Инструкции](https://docs.rs/solana-loader-v3-interface/latest/solana_loader_v3_interface/instruction/enum.UpgradeableLoaderInstruction.html) |
...
Помимо программ загрузчика, Solana предоставляет следующие предварительно
...
| ----------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| Программа Ed25519 | `Ed25519SigVerify111111111111111111111111111` | Проверяет подписи ed25519. Если хотя бы одна подпись не проходит проверку, возвращается ошибка. | [Инструкции](https://docs.rs/solana-ed25519-program/latest/solana_ed25519_program/index.html) |
...
для выравнивания. Далее для каждой подписи сериализуется следующая структура.
...
| Программа           | ID программы                                  | Описание                                                                                                                                          | Инструкции                                                                                      |
| ------------------- | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| Программа Secp256r1 | `Secp256r1SigVerify1111111111111111111111111` | Проверяет до 8 подписей secp256r1. Принимает подпись, открытый ключ и сообщение. Возвращает ошибку, если какая-либо подпись не проходит проверку. | [Инструкции](https://docs.rs/solana-secp256r1-program/latest/solana_secp256r1_program/all.html) |
...
затем сериализуется и добавляется в instruction data.
...
| **Config**               | `Config1111111111111111111111111111111111111` | Добавление конфигурационных данных в цепочку, а также списка публичных ключей, которым разрешено их изменять. В отличие от других программ, программа Config не определяет отдельных инструкций. У неё есть только одна неявная инструкция: "store". Её instruction data — это набор ключей, ограничивающих доступ к аккаунту и данным внутри него | [ConfigInstruction](https://docs.rs/solana-config-program/latest/solana_config_program/config_instruction/index.html)                                 |
| **Compute Budget**       | `ComputeBudget111111111111111111111111111111` | Установка лимитов и цен на вычислительные единицы для транзакций, позволяя пользователям управлять вычислительными ресурсами и приоритетными комиссиями                                                                                                                                                                                            | [ComputeBudgetInstruction](https://docs.rs/solana-compute-budget-interface/latest/solana_compute_budget_interface/enum.ComputeBudgetInstruction.html) |
