...
Транзакции имеют общий лимит размера —
...
Поле `message` транзакции — это структура
...
- [`header`](mention:message-header): Заголовок [message header](#header)
...
Поле `header` сообщения — это структура
...
- [`num_readonly_signed_accounts`](mention:num_readonly_signed_accounts): Общее
  количество только для чтения аккаунтов, для которых требуются подписи
- [`num_readonly_unsigned_accounts`](mention:num_readonly_unsigned_accounts):
  Общее количество только для чтения аккаунтов, для которых подписи не требуются
...
в сообщении содержит адреса аккаунтов и передаётся в
...
<Callout>
  Строгий порядок позволяет объединять массив `account_keys` с информацией из
  [`header`](#header) сообщения для определения прав доступа для каждого
  аккаунта.
</Callout>
...
Значение `recent_blockhash` в сообщении — это хеш, который служит временной
меткой транзакции и предотвращает дублирование транзакций. Blockhash истекает
через
[150 блоков](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/clock/src/lib.rs#L134).
(Это примерно одна минута, если каждый блок создаётся за 400 мс.) После
истечения срока действия блока транзакция становится недействительной и не может
быть обработана.

<Callout>
  Метод RPC [`getLatestBlockhash`](/docs/rpc/http/getlatestblockhash) позволяет
  получить текущий blockhash и последний номер блока, на котором он будет
  действителен.
</Callout>
...
в сообщении содержит все инструкции для обработки и передаётся в
[компактном формате массива](/docs/references/terminology#compact-array-format).
Префикс массива указывает его длину. Каждый элемент массива — это структура
[`CompiledInstruction`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/instruction.rs#L22),
включающая следующую информацию:
