---
title: CPI с PDA-подписантами
description:
  Как использовать функцию invoke_signed для межпрограммных вызовов (CPI) в
  Solana, когда вызывающая программа должна подписывать как PDA, с примерами на
  Anchor CpiContext и нативном Rust.
...
<Callout type="info" title="Резюме">
  Используйте _rs`invoke_signed`_, когда вызывающая программа должна подписывать
...
с [сид-сигнатурами](mention:signer-seeds), которые использовались для получения
[PDA](/docs/core/pda). Подробнее о том, как runtime проверяет подписи PDA,
смотрите в разделе [Подпись PDA](/docs/core/cpi/cpi-cost-model#pda-signing).
...
В примерах ниже показан CPI с PDA-подписантами с использованием Anchor и
нативного Rust. В каждом примере выполняется одна инструкция по переводу SOL с
PDA на аккаунт получателя через CPI, подписанный PDA.
...
но демонстрируют разный уровень абстракции.

- **Пример 1**: Использует Anchor _rs`CpiContext`_ и вспомогательную функцию.
- **Пример 2**: Использует функцию _rs`system_instruction::transfer`_ из крейта
  _rs`solana_program`_. (Пример 1 — это абстракция над этой реализацией.)
- **Пример 3**: CPI-инструкция собирается вручную. Такой подход полезен, если
  нет готового крейта для сборки нужной инструкции.
...
В приведённом ниже примере показано выполнение CPI с использованием
PDA-подписантов из программы, написанной на нативном Rust. В примере реализована
одна инструкция, которая переводит SOL с аккаунта PDA на другой аккаунт. CPI
подписывается PDA-аккаунтом. (В тестовом файле используется
