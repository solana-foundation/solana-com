...
Приоритетная комиссия — это необязательная комиссия, которая повышает приоритет планирования транзакции. Вычислительная единица (CU) — это единица измерения вычислений, которую среда выполнения отслеживает в процессе выполнения транзакции. Каждая операция (арифметическая, обращение к памяти, системный вызов) стоит фиксированное количество CU. Вы можете установить приоритетную комиссию, включив в транзакцию инструкции бюджета вычислений. (Подробнее см. в руководстве [Как использовать приоритетные комиссии](/docs/core/fees).)
...
<Callout type="info" title="В транзакциях v1 комиссия задаётся напрямую">
  Эта формула «цена × лимит» применяется к транзакциям legacy и v0. В предстоящем [формате
  v1](/docs/core/transactions/versioned-transactions#v1-message-format) приоритетная комиссия представляет собой абсолютную сумму в lamport, задаваемую напрямую в конфигурации сообщения — без цены за CU, без умножения, без округления.
</Callout>
