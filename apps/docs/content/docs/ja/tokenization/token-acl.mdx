...
このガイドには、ローカルで実行できる完全な動作実装が含まれています。
[ソースコード](https://github.com/Woody4618/access-control-list-acl-demo)
は、調査および教育目的のリファレンス実装を提供します。

ACLプログラムのコードは
[token-aclリポジトリ](https://github.com/solana-foundation/token-acl)で公開されており、
ABL Gateプログラムは
[token-acl-gateリポジトリ](https://github.com/solana-foundation/token-acl-gate)で公開されています。

**重要**: このガイドで使用されているABL（許可ブロックリスト）Gateプログラムは、
**リファレンス実装**です。監査済みで本番環境に対応していますが、発行者は
特定のコンプライアンス要件に合わせたカスタムGateプログラムを自由に作成できます。Token ACL仕様（sRFC37）に
従う必要はありますが、この特定のGateプログラムの設計には縛られません。

**このコードを本番環境で直接使用しないでください**。以下を行わずに使用することは禁止されています：
...
4. **仕様を読む**：完全な技術仕様については[sRFC37](https://github.com/solana-foundation/token-acl/blob/master/srfc37.md)を参照し、[sRFC37のディスカッション](https://github.com/solana-foundation/SRFCs/discussions/2)にご参加ください。
