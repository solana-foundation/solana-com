...
Solanaのトランザクションにはサイズ制限（〜1232バイト）があります。各転送instructionsは約200バイトを追加します。非常に大規模なマルチパーティ決済の場合は、[アドレスルックアップテーブル](/developers/cookbook/transactions/lookup-tables)を使用してアカウントアドレスを圧縮し、1トランザクションあたりのより多くの転送を収めることを検討してください。

近日公開予定の[v1トランザクション形式](/docs/core/transactions/versioned-transactions#v1-message-format)では制限が4096バイトに引き上げられますが、アドレスルックアップテーブルのサポートが削除されるため、v1の決済ではアドレスをインラインで記述します。いずれの場合も、ランタイムはトランザクションごとに最大64アカウントをロードします。
