...
クライアントはリクエストを送信し、支払い条件を含む402レスポンスを受け取った後、署名付きの支払いとともに再試行します。サーバーは検証と決済をファシリテーター（オンチェーントランザクションの送信を処理するオプションの仲介者）に委任します。ファシリテーターが決済を確認すると、サーバーはリクエストされたコンテンツを返します。この分離により、APIプロバイダーはオンチェーンインフラを直接管理することなく支払いを受け付けることができます。
