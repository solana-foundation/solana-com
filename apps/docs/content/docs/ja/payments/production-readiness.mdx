...
本番環境のバックエンド署名には、[solana-keychain](https://github.com/solana-foundation/solana-keychain)の使用を検討してください。これは、単一のインターフェースを通じて複数の鍵管理バックエンド(Memory、Vault、AWS
KMS、Privy、Turnkey、Fireblocks、CDP、Dfns、Para)を抽象化する統合署名ライブラリです。これにより、ローカル環境ではメモリ内鍵で開発し、アプリケーションコードを変更することなく本番グレードのバックエンドに切り替えることができます。

[Rust](https://crates.io/crates/solana-keychain)と[TypeScript](https://www.npmjs.com/package/@solana/keychain)の両方で利用可能です。

追加の鍵管理サービスを統合するには、[署名者追加ガイド](https://github.com/solana-foundation/solana-keychain/blob/main/docs/ADDING_SIGNERS.md)をご確認ください。

### RPCセキュリティ

RPCエンドポイントはAPIキーと同様に扱ってください。抽出・悪用される可能性のあるフロントエンドコードに公開しないでください。クライアントコードにバンドルされない、バックエンドプロキシまたは環境変数を使用してください。
...
すべてのトークンとプログラムには、メインネット上に正確に1つの正しいアドレスが存在します。USDCや他のステーブルコインを模倣した偽装トークンは一般的です。これらは同じ名前とシンボルを持ちますが、異なるミントアドレスを持ちます。アプリケーションは(要件に基づいて)ミントアドレスをハードコーディングまたは許可リスト化し、信頼できないソースから動的に受け入れないようにしてください。

**環境ベースの設定:**
DevnetとMainnetでは、まったく異なるトークンミントを使用することがよくあります。環境ごとに正しいアドレスを読み込むようにアプリケーション設定を構成してください。テスト中にMainnetアドレスをハードコーディングして切り替えを忘れたり、さらに悪いことに、Devnetアドレスを本番環境にデプロイしたりしないようにしましょう。

一般的なステーブルコインミントには以下のものがあります：
...
プログラムアドレスも重要です。間違ったプログラムにinstructionsを送信すると失敗するか、最悪の場合、資金の取り返しのつかない損失につながります。Token
...
- [ ] 手数料とrentのためのMainnet SOLを取得済み
- [ ] 本番RPCを設定済み（パブリックエンドポイントではない）
- [ ] フォールバックRPCエンドポイントを設定済み
- [ ] 動的価格設定による優先手数料を実装済み
- [ ] ブロックハッシュの有効期限切れを処理する再試行ロジックを実装済み
- [ ] ユースケースに適した確認レベルを設定済み
- [ ] すべての一般的なエラーを適切に処理済み
- [ ] Gasless設定を完了済み（該当する場合）
- [ ] Mainnetトークンアドレスを検証済み（Devnetミントではない）
- [ ] すべての鍵を安全にバックアップ済み
- [ ] 鍵管理を確認済み（フロントエンドに鍵を含めない）
- [ ] トランザクション監視とアラート機能を有効化済み
- [ ] 想定されるボリュームでの負荷テストを実施済み

## プログラムのデプロイ

決済インフラの一部としてカスタムSolanaプログラムをデプロイする場合、追加の考慮事項があります。
...
- **プログラムのkeypair:**
  プログラムは、devnetとmainnetで異なるアドレスを持つことになります(同じkeypairを再利用している場合を除く)。アプリケーション設定内のすべての参照を更新してください。プログラムのkeypairは安全な場所に保管してください(`cargo clean`を実行すると、プログラムのkeypairが削除される可能性があることに注意してください)。
- **アカウントの初期化:**
  プログラムが管理者アカウント、PDA、またはその他の状態アカウントを必要とする場合、ユーザーがアプリケーションと対話する前に、これらがmainnet上で作成されていることを確認してください。プログラムに必要なAssociated
  Token Account(ATA)についても同様です。

### デプロイプロセス

- **バッファアカウント:**
  大規模なプログラムは、バッファアカウントを介してデプロイされます。`solana program deploy`コマンドはこれを自動的に処理しますが、デプロイはアトミックではないことを理解してください。中断された場合、バッファアカウントを復旧または閉鎖する必要がある場合があります。[プログラムのデプロイ](https://solana.com/docs/programs/deploying)を参照してください。
- **アップグレード権限:**
  プログラムが起動後にアップグレード可能であるべきかどうかを決定してください。不変性を確保する場合は、デプロイ後にアップグレード権限を取り消してください。柔軟性を持たせる場合は、アップグレード権限キーを適切に保護してください。
- **レント:** すべてのprogram
  accountのレント免除最小額をカバーするのに十分なSOLがデプロイウォレットにあることを確認してください。
- **検証:**
  プログラムを[検証](https://solana.com/docs/programs/verified-builds)して、Solanaネットワークにデプロイする実行可能プログラムがリポジトリ内のソースコードと一致することを確認してください。

プログラムデプロイの完全なガイダンスについては、[プログラムのデプロイ](https://solana.com/docs/programs/deploying)を参照してください。
