...
本番環境のバックエンド署名には、[solana-keychain](https://github.com/solana-foundation/solana-keychain)の使用をご検討ください。これは、Memory、Vault、Privy、Turnkey、AWS
KMS、Fireblocks、GCP
KMS、CDP、Para、Dfns、Crossmintなど、複数の鍵管理バックエンドを単一のインターフェースで抽象化した統合署名ライブラリです。これにより、ローカル環境ではメモリ内の鍵を使用して開発し、アプリケーションコードを変更することなく本番環境レベルのバックエンドに切り替えることができます。

[Rust](https://crates.io/crates/solana-keychain)と[TypeScript](https://www.npmjs.com/package/@solana/keychain)の両方で利用可能です。

追加の鍵管理サービスを統合するには、[Adding Signersガイド](https://github.com/solana-foundation/solana-keychain/blob/main/docs/ADDING_SIGNERS.md)をご確認ください。
