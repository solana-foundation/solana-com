...
本番環境のバックエンド署名には、[Keychain](/docs/tools/keychain)をご利用ください。これは、Memory、Vault、Privy、Turnkey、AWS
KMS、Fireblocks、GCP
KMS、CDP、Para、Dfns、Crossmint、Openfort、Utilaなど、複数のキー管理バックエンドを単一のインターフェースで抽象化した統合署名ライブラリです。ローカル開発時はメモリ内キーを使用し、アプリケーションコードを変更することなく本番グレードのバックエンドへ切り替えることができます。

どのバックエンドが適しているか迷っている場合は、[バックエンドの選び方](/docs/tools/keychain/choosing-a-backend)をご覧ください。Keychainは[Rust](https://crates.io/crates/solana-keychain)と[TypeScript](https://www.npmjs.com/package/@solana/keychain)の両方で利用可能です。

### RPCセキュリティ

RPCエンドポイントはAPIキーと同様に扱ってください。フロントエンドのコードに公開すると、抽出・悪用される可能性があります。バックエンドプロキシを使用するか、クライアントコードにバンドルされない環境変数を利用してください。
...
本番環境では以下のメトリクスを追跡してください：

| メトリクス             | 目的                     |
| ---------------------- | ------------------------ |
| トランザクション成功率 | 問題の早期検出           |
| 確認レイテンシ         | ネットワーク健全性の監視 |
| 優先手数料の支出       | コスト管理               |
| RPCエラー率            | プロバイダーの健全性     |

以下の項目についてアラートを設定してください：
...
大規模なリアルタイムトランザクション監視については、[インデックスガイド](/docs/payments/accept-payments/indexing)をご覧ください。

### アドレスの検証

すべてのトークンとプログラムには、メインネット上に正確に1つの正しいアドレスが存在します。USDCや他のステーブルコインを模倣した偽のトークンが一般的に出回っており、同じ名前とシンボルを持ちながら異なるミントアドレスを使用しています。アプリケーションは（要件に基づいて）ミントアドレスをハードコードするかallowlistに登録し、信頼できないソースから動的に受け入れることは絶対に避けてください。

**環境ベースの設定:**
DevnetとMainnetでは、使用するトークンのミントアドレスが全く異なる場合があります。環境ごとに正しいアドレスを読み込むようにアプリケーションの設定を行いましょう。メインネットのアドレスをハードコードしたままテスト時に切り替えを忘れたり、さらに悪いことにDevnetのアドレスを本番環境にリリースしてしまうといったミスは避けなければなりません。

代表的なステーブルコインのミントアドレスは以下の通りです：
...
プログラムアドレスも重要です。誤ったプログラムにinstructionsを送信すると、処理が失敗するか、最悪の場合は資金が取り戻せない形で失われます。Token
...
正しいアドレスをallowlistに登録することで偽装トークンへの対策にはなりますが、誤った*種類*のアカウントへの送金を防ぐことはできません。送金先アドレスは、ウォレット、token
account、ミント、またはプログラムのいずれかである可能性があり、それぞれ異なる扱いが必要です。ウォレット以外へのネイティブSOLの送金は送信者の管理下を離れてしまいます。ミントやプログラムへの送金は完全に失われ、token
accountへの送金はそのアカウントの所有者のみが回収できますが、いずれの場合もユーザーへの警告となる失敗トランザクションは発生しません。

<Callout type="warn" title="送金前に受取人アドレスを検証する">
...
- [ ] メインネットのfeeとrentのためのSOLを取得済み
- [ ] 本番用RPCを設定済み（パブリックエンドポイントは使用しない）
- [ ] フォールバックRPCエンドポイントを設定済み
- [ ] 動的価格設定によるプライオリティフィーを実装済み
- [ ] リトライロジックでブロックハッシュの有効期限切れに対応済み
- [ ] ユースケースに適した承認レベルを設定済み
- [ ] よくあるエラーをすべて適切に処理済み
- [ ] Gaslessを設定済み（該当する場合）
- [ ] メインネットのトークンアドレスを確認済み（devnetのミントではないこと）
- [ ] 送信前に受信者アドレスを検証済み（ウォレット、token
      account、ミント、program accountの区別）
- [ ] すべての鍵を安全にバックアップ済み
- [ ] 鍵管理を確認済み（フロントエンドに鍵を置かないこと）
- [ ] トランザクションの監視とアラートが有効
- [ ] 想定されるトラフィック量での負荷テスト済み

## プログラムのデプロイ

決済インフラの一部としてカスタムSolanaプログラムをデプロイする場合は、追加の考慮事項があります。

### デプロイ前の準備

- **Solana CLIのバージョン:**
  [Solana CLI](https://solana.com/docs/intro/installation)の最新バージョンを使用していることを確認してください。
- **Program keypair:**
  プログラムはメインネットとdevnetで異なるアドレスを持ちます（同じkeypairを再利用しない限り）。アプリケーション設定内のすべての参照を更新してください。program
  keypairは安全な場所に保管してください（`cargo clean`を実行するとprogram
  keypairが削除される可能性があることに注意してください）。
- **アカウントの初期化:**
  プログラムが管理者アカウント、PDA、またはその他のステートアカウントを必要とする場合、ユーザーがアプリケーションを利用する前にメインネット上でこれらを作成しておいてください。プログラムが必要とするassociated
  token accountについても同様です。

### デプロイメントプロセス

- **Bufferアカウント:** 大規模なプログラムはbuffer
  accountsを経由してデプロイされます。`solana program deploy`コマンドがこれを自動的に処理しますが、デプロイはアトミックではないことを理解しておいてください。中断された場合、buffer
  accountsの回復またはクローズが必要になることがあります。詳細は[プログラムのデプロイ](https://solana.com/docs/programs/deploying)を参照してください。
- **アップグレード権限:**
  プログラムをリリース後にアップグレード可能にするかどうかを決定してください。イミュータビリティを確保するには、デプロイ後にアップグレード権限を破棄してください。柔軟性を持たせる場合は、アップグレード権限キーを適切に保護してください。
- **Rent:** デプロイ用ウォレットに、すべてのprogram
  accountのrent免除最低額をカバーするのに十分なSOLがあることを確認してください。
- **検証:**
  プログラムを[検証](https://solana.com/docs/programs/verified-builds)して、Solanaネットワークにデプロイされた実行可能プログラムがリポジトリのソースコードと一致することを確認してください。

完全なプログラムデプロイメントのガイダンスについては、
