...
本番環境でのバックエンド署名には、[solana-keychain](https://github.com/solana-foundation/solana-keychain)の使用をご検討ください。これは、Memory、Vault、Privy、Turnkey、AWS
KMS、Fireblocks、GCP
KMS、CDP、Para、Dfnsといった複数の鍵管理バックエンドを単一のインターフェースで抽象化する統合署名ライブラリです。これにより、ローカル環境ではメモリ内鍵を使用して開発を行い、アプリケーションコードを変更することなく本番環境レベルのバックエンドに切り替えることができます。
