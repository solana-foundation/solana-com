...
(`api.mainnet.solana.com`) はレート制限があり、SLAもありません。開発用途には問題ありませんが、本番環境の決済フローには対応できません。稼働保証のない共有APIで決済処理システムを運用しようとするようなものです。
...
- [ガイド: トランザクションの確認と有効期限](https://solana.com/developers/cookbook/transactions/confirmation)
