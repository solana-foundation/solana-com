...
本番環境のバックエンド署名には、[solana-keychain](https://github.com/solana-foundation/solana-keychain)の使用を検討してください。これは、Memory、Vault、Privy、Turnkey、AWS
KMS、Fireblocks、GCP
KMS、CDP、Para、Dfnsといった複数の鍵管理バックエンドを単一のインターフェースで抽象化する統合署名ライブラリです。これにより、ローカル環境ではメモリ内鍵で開発し、アプリケーションコードを変更することなく本番環境向けのバックエンドに切り替えることができます。
