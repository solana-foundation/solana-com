...
最近のブロックハッシュの代わりに、ノンス値をトランザクションのライフタイムとして使用します。これを行うには、まずノンスアカウントからノンス値を取得し、それを使用してトランザクションのライフタイムを設定する必要があります。

### ノンス値の取得

まず、ノンスアカウントからノンス値を取得します:
...
1. ノンス値をトランザクションの「ブロックハッシュ」として設定します
2. `advanceNonceAccount`命令を自動的に先頭に追加します(すべての永続的ノンストランザクションで最初の命令として必要)
...
シリアライズされた文字列をデータベースに保存します。ノンスが進められるまで有効です。

## マルチパーティ承認ワークフロー

トランザクションをデシリアライズして追加の署名を追加し、保存または送信のために再度シリアライズします:
...
トランザクションはシリアライズ、保存され、承認者間で受け渡すことができます。必要なすべての署名が収集されたら、ネットワークに送信します。
...
<Callout type="caution">
  各ノンスは一度しか使用できません。トランザクションが失敗した場合、または送信しないことを決定した場合は、同じノンスアカウントで別のトランザクションを準備する前にノンスを進める必要があります。
</Callout>

## 使用済みまたは放棄されたノンスの進行

保留中のトランザクションを無効化するか、ノンスを再利用のために準備するには、手動で進めます:
...
これにより新しいノンス値が生成され、古い値で署名されたトランザクションは永久に無効になります。
