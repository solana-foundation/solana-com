...
[Commerce Kit](https://www.npmjs.com/package/@solana-commerce/kit)の`PaymentButton`コンポーネントは、ウォレット接続、トークン選択、トランザクション処理、UI状態を標準で処理する完全な決済インターフェースを提供します。
...
`buyNow`および`cart`モードの場合、`paymentConfig`経由で商品を提供します:
...
| コールバック       | パラメータ            | 説明                                       |
| ------------------ | --------------------- | ------------------------------------------ |
| `onPaymentSuccess` | `(signature: string)` | トランザクションがオンチェーンで確認された |
| `onPaymentError`   | `(error: Error)`      | 支払いがいずれかの段階で失敗した           |
| `onPaymentStart`   | `()`                  | 支払いフローが開始された                   |
| `onCancel`         | `()`                  | ユーザーが支払いをキャンセルした           |
