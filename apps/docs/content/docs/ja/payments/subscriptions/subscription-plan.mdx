---
title: サブスクリプションプラン
description: サブスクリプションプラントランザクションの設定ガイド。
---
...
スポンサーはオプションの `payer` を渡すことでプランのrentを負担できますが、
マーチャントはプランのオーナーであり続けます。プランを削除した場合、返金はオーナーに対して行われ、payerには行われません。そのため、スポンサーシップの管理はオフチェーンで行ってください。
...
- 有限の `endTs`
  は短縮のみ可能で、延長またはクリアはできません (`PlanEndTsCannotExtend`)。
- `Sunset`
  プランでは、プラーを削除することができます（新しいセットは現在のセットのサブセットである必要があります）。これにより、侵害されたプラーへのアクセスを取り消すことができます。ただし、ステータス、`endTs`、およびメタデータは変更されません。
- 編集は、`endTs` が変更されていない限り、プランの最終請求期間中に有効です。
- このinstructionsは、署名時に観測されたプランの状態
  （`expectedCreatedAt`、`expectedEndTs`、`expectedPullers`、
  `expectedMetadataUri`）を保持します。ライブのプランが一致しない場合、更新は
  拒否されます（`StalePlanApproval`）。そのため、古い署名済み更新によって削除されたpullerを復元したり、後の編集を元に戻したりすることはできません。プラグインクライアントの `updatePlan` は自動的にライブ状態を取得しますが、手動で構築する場合はプランを取得してフィールドを自分で渡してください。
...
サブスクライバーは現在のプランの条件に同意します。サブスクリプション PDA は、プラン PDA とサブスクライバーアドレスから導出されます。

認証局の初期化とサブスクライブを単一のトランザクションにまとめるには、`expectedSubscriptionAuthorityInitId` としてセンチネル値 `UNKNOWN_INIT_ID`（TypeScript SDKからエクスポートされています）を渡します。プログラムは、認証局が現在のslotで作成された場合にのみ受け付けるため、このセンチネル値は新規サインアップに有効です。それ以前のslotで認証局が作成された既存ユーザーは、実際の `initId`（プラグインクライアントが自動取得します）を渡す必要があり、渡さない場合は `StaleSubscriptionAuthority` エラーで失敗します。
...
マーチャントまたはホワイトリストに登録されたプラーが回収に署名します。プランが宛先許可リストを使用している場合、受信者の token
account のオーナーが `destinations` にリストされている必要があります。
...
キャンセルはサブスクリプションを終了予定としてマークします。失効は、キャンセルの有効期限が経過した後にサブスクリプションPDAを閉じます。サブスクライバーは両方のトランザクションに署名します。
...
サブスクライバーと現在のプランオーナーの両方が署名した場合、`cancelSubscriptionNow` は請求期間の終了時ではなく、キャンセル時点でサブスクリプションを失効させます。また、保留中のグレースピリオドキャンセルを短縮することもできます。この承認は署名時に観測された期間開始時刻に紐付けられており、それ以降にサブスクリプションが変更されていた場合、トランザクションは拒否されます（`StaleSubscriptionApproval`）。
...
キャンセルされたサブスクリプションは、失効する前に再アクティブ化できます。一度失効すると、サブスクリプションアカウントは閉鎖され、サブスクライバーは再度[サブスクライブ](#subscribe)する必要があります。再開には、プログラムが検証するプランのミントに対するサブスクライバーの`SubscriptionAuthority`が必要です（オーナー、ミント、および`init_id`）。古くなっているか再初期化されている場合は拒否されます。

このinstructionsは、サブスクライバーが署名時に観測した有効期限（`expectedExpiresAtTs`）を保持します。不一致の場合は拒否されます（`StaleSubscriptionApproval`）。そのため、古い署名済みの再開によって、サブスクライバーが承認していない後のキャンセルをクリアすることはできません。
...
- `amount`は基本単位で表されます。小数点以下6桁のトークンの場合、`5_000_000`は`5`トークンを意味します。
- TypeScript
  SDKは、省略した場合に`subscribe`中にライブのプラン条件を取得します。
- Rustの`SubscribeBuilder`には期待されるプラン条件が必要です。まずプランアカウントを取得してデコードし、`SubscribeData`を通じてそれらのフィールドを渡してください。
- マーチャントまたは`pullers`に登録されているウォレットのみが支払いを収集できます。
- サブスクライバーはセットアップ、キャンセル、および失効トランザクションに署名します。マーチャントまたは承認済みプラーが収集トランザクションに署名します。
