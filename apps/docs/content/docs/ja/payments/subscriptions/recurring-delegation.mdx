...
- `amountPerPeriod` は基本単位です。小数点以下6桁のトークンの場合、`1_000_000`
  は `1` トークンを意味します。
- プログラムは、現在の期間の残余許容量を超える送金を拒否します。
- 次の期間が始まると、引き出し済み金額がリセットされます。
- `expiryTs`
  はハードストップです。これを過ぎると送金が失敗し、スポンサーは rent を回収できます。支出時のグレース期間はありません。
- `startTs` を `0`
  に設定すると、固定時刻ではなく、トランザクションがチェーンに記録された時点でデリゲーションが開始されます。これにより、ユーザーに署名してもらいオンボードするまでの猶予時間が広がります。この設定を行う場合、`expiryTs`
  はゼロ以外の値でなければなりません — 開始時刻をチェーン記録時点とするデリゲーションは、有効期限なしに設定することはできません。
- ユーザーはセットアップおよび失効トランザクションに署名します。デリゲイトは送金トランザクションに署名します。
