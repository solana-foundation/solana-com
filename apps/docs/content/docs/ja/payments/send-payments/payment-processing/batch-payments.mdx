...
次期
[v1トランザクション形式](/docs/core/transactions/versioned-transactions#v1-message-format)
では、サイズ制限が4096バイトに引き上げられ、1トランザクションあたり約3倍の転送が可能になります。それを超える場合は引き続きバッチ処理が必要です。ランタイムは形式に関わらず、1トランザクションにつき最大64アカウントまでしか読み込まないためです。
