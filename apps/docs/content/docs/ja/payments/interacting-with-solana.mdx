...
<Callout type="caution" title="本番環境では公開RPCを使用しないでください">
  パブリックエンドポイント（`api.mainnet.solana.com`、`api.devnet.solana.com`）はレート制限があり、SLAもなく、本番環境の決済フローには適していません。本番環境へのデプロイには、[RPCプロバイダー](https://solana.com/rpc)を利用してプライベートRPCエンドポイントを確保してください。
</Callout>
