...
accountに対してToken
Programである基本[Account](/docs/core/accounts#account-type)タイプで指定されたプログラム所有者とは異なります。
...
_sh`spl-token`_ CLIは、SPL
tokenの実験に役立ちます。以下の例では、[Solana Playground](https://beta.solpg.io/)のターミナルを使用して、ローカルインストールなしでブラウザ内で直接CLIコマンドを実行します。

tokenとアカウントの作成には、アカウントのrentデポジットとトランザクション手数料のためのSOLが必要です。Solana
...
ターミナル出力に表示されるアカウントアドレスは、以下の例とは異なります。操作を進める際は、Playgroundターミナルのアドレスを使用してください。例えば、*sh`create-token`*コマンドは、Playgroundウォレットをmint
authorityとするmint accountアドレスを出力します。
...
新しいtokenに対する*sh`supply`*コマンドは`0`を返します。
...
注記: `<TOKEN_ADDRESS>`はtoken accountのmint accountを指定します。
...
accountの総供給量を増やします。[Solana Playground](https://beta.solpg.io/660cea45cffcf4b13384d012)のJavaScriptの例を参照してください。
...
Token Programの*rs`Transfer`*命令はトークンの転送を処理します。送信者のtoken
accountの所有者がトランザクションに署名する必要があります。この命令はtoken
account間でトークンを移動します。[Solana Playground](https://beta.solpg.io/660ced84cffcf4b13384d013)のJavaScriptの例を参照してください。
