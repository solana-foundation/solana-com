---
title: 委任の取り消し
description: トークンアカウントの現在のデリゲートを削除し、残りの委任許可量をクリアします。
...
デリゲートを取り消すと、トークンアカウントの現在のデリゲートがクリアされ、委任量がゼロにリセットされます。

取り消し後、以前のデリゲートはトークンアカウントに代わってトークンを転送またはバーンする権限を持たなくなります。

## デリゲートを取り消す方法

デリゲートの取り消しには、Token
Programの`Revoke`インストラクションを使用します。

`Revoke`インストラクションは、送信元トークンアカウントのデリゲートフィールドをクリアします。Token
Programでは、トークンアカウントの所有者が取り消しに署名します。Token Extensions
Programでは、現在のデリゲートも委任を取り消すことができます。

### ソースリファレンス

| 項目             | 説明                                                                 | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ---------------- | -------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `Account`        | トークンアカウントの状態には、デリゲートと委任量が保存されます。     | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| `Revoke`         | トークンアカウントから現在のデリゲートを削除するインストラクション。 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L128-L140) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L170-L182) |
| `process_revoke` | デリゲート取り消しの共有プロセッサロジック。                         | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L395-L420)     | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L643-L674)     |
