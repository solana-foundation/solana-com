...
命令は、現在承認されているデリゲートからすべての転送権限を削除します。取り消し後、デリゲートはあなたのアカウントからトークンを転送できなくなります。Token
Programでは、token
accountの所有者のみがデリゲートを取り消すことができます。Token Extension
Programでは、token
accountの所有者と現在のデリゲートが委任を取り消すことができます。
