...
凍結解除が成功すると、アカウントは再びトークンの受信、転送、バーンが可能になります。トークンアカウントの凍結解除は、ミントの凍結権限を持つ者のみが実行できます。

## token accountを解凍する方法

token accountの解凍には、Token Programの _rs`ThawAccount`_ 命令を使用します。

_rs`ThawAccount`_ 命令は、token
...
| _rs`AccountState`_                  | _rs`Frozen`_ と _rs`Initialized`_ を含むtoken accountの状態。                             | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
