...
description:
  凍結されたtoken
  accountを解凍し、転送、バーン、および委任変更を再開できるようにします。
...
解凍により、token accountが凍結状態から初期化状態に戻ります。

解凍が成功すると、アカウントは再びトークンの転送、バーン、およびクローズが可能になります。Mintの凍結権限者のみがtoken
accountを解凍できます。

## Token Accountを解凍する方法

Token accountの解凍には、Token Programの*rs`ThawAccount`*
instructionを使用します。

_rs`ThawAccount`_ instructionは、token
accountがmintに属していること、および署名者がmintの凍結権限者であることを検証した後、token
...
| ----------------------------------- | --------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Mint状態には、token accountを解凍するために使用されるオプションの凍結権限が格納されています。 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | *rs`Frozen`*および*rs`Initialized`*を含むtoken accountの状態。                                | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`ThawAccount`_                   | 凍結されたtoken accountを初期化状態に戻すinstruction。                                        | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L229-L243) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L292-L306) |
| _rs`process_toggle_freeze_account`_ | Token accountの凍結と解凍のための共有プロセッサロジック。                                     | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |
