---
title: トークンアカウントの作成
description: SPLトークンアカウントの作成方法を学びます。
---

## トークンアカウントとは？

トークンアカウントは、特定のトークンの残高を保存します。各トークンアカウントは、正確に1つのミントに関連付けられており、トークン残高と追加の詳細を追跡します。
...
<Callout type="info">
  ソースコードでは、開発者はトークンアカウントを`Account`型と呼んでいることに注意してください。[Token
  ...
  Extension
  Program](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/state.rs#L102-L123)の両方が、トークンアカウントの同じ基本実装を共有しています。
</Callout>

トークンを保有するには、その特定のミントのトークンアカウントが必要です。各トークンアカウントは以下を追跡します：

1. **ミント**：保有する特定のトークンタイプ
2. **オーナー**：このアカウントからトークンを転送できる権限

### 例：USD Coin (USDC)

- ミントアドレス：`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Circleのトークンアカウント：`3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
- このアカウントはUSDCトークンのみを保有します
- Circleはトークンアカウントのオーナー権限を通じてトークンを管理します：`7VHUFJHWu2CuExkJcJrzhQPJ2oygupTWkL2A2For4BmE`
...
「オーナー」という用語は、2つの異なる文脈で使用されることがあります：

1. **トークンアカウントオーナー**：トークンを転送、バーン、または委任できる権限。これはトークンアカウントのデータに保存されます。

2. **プログラムオーナー**：アカウントを所有するプログラム（トークンアカウントの場合は常にToken
   ProgramまたはToken Extension Program）。

トークンアカウントを扱う際、「オーナー」は通常、アカウントを所有するプログラムではなく、トークンを転送できる権限を指します。
...
associated token
account(ATA)は、ウォレットが特定のトークンを保持するためのデフォルトのtoken
accountです。[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program)によって作成される決定論的アドレス(PDA)を使用します。

<Callout type="info">
  Associated Token Programによって作成されたtoken accountのみが「associated
  token account」と呼ばれます。
</Callout>

ATAは決定論的アドレスを使用するため、任意のウォレットの特定のミントに対するtoken
accountを簡単に見つけることができます。[導出の実装はこちら](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/interface/src/address.rs#L56)を参照してください。
