...
title: トークンアカウントの作成
description:
  特定のミントとオーナーのためのtoken accountとassociated token
  accountを作成します。
...
token accountは、Solana上で1つのミントと1つのtoken
accountオーナーのためのトークンを保管します。
...
ownerは引き続きToken ProgramまたはToken Extensions Programです。

各token accountは正確に1つのミントに紐づいており、つまりtoken accountはtoken
accountの*rs`mint`*フィールドで識別される1つのトークンの単位のみを保持できます。

## Associated Token Accountとは

associated token account(ATA)は、ウォレットとミントのデフォルトtoken
accountです。
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program/src)は、ウォレットアドレス、トークンプログラムアドレス、ミントアドレスからATAアドレスを導出します。

Associated Token Programによって作成されたtoken accountのみがassociated token
accountと呼ばれます。

Associated Token Programは、標準的で決定論的なアドレスでtoken
accountを作成する方法です。作成されたアカウントは、Associated Token
Programではなく、Token ProgramまたはToken Extensions Programが所有するtoken
accountです。
...
任意のウォレット、トークンプログラム、ミントの組み合わせに対して、正確に1つのATAアドレスが存在します。Associated
Token Programはそのアドレスに標準token
accountを作成し、作成されたアカウントはToken ProgramまたはToken Extensions
Programによって定義された*rs`Account`*タイプを引き続き使用します。

## Associated Token Accountの作成方法

Associated Token Accountの作成には、Associated Token
Programの*rs`Create`*または\_rs`CreateIdempotent`\_instructionを使用します。Associated
Token ProgramはATAアドレスを導出し、ATAアドレスにアカウントを作成し、Token
ProgramまたはToken Extension Programが所有するtoken
accountとしてアカウントを初期化します。

<Callout type="info" title="推奨">
  ほとんどのアプリケーションでは、System ProgramとToken Program
  instructionsを直接呼び出してtoken accountを作成するのではなく、Associated
  Token Programを通じてtoken accountを作成してください。associated token
  accountは、所有者、token
  program、mintから導出される決定論的なアドレスを使用するため、ウォレットやアプリケーションが特定のmintのデフォルトtoken
  accountを見つけやすくなります。
</Callout>
...
| 項目                                          | 説明                                                                                                                                                                                                                              | ソース                                                                                                                          |
| --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| _rs`Create`_                                  | 導出されたATAアドレスにassociated token accountを作成します。                                                                                                                                                                     | [Source](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L20-L29) |
| _rs`CreateIdempotent`_                        | associated token accountを作成しますが、同じ所有者とmintに対してそのATAが既に存在する場合でも成功します。                                                                                                                         | [Source](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | ATAアドレスを導出し、*rs`create_pda_account`*と選択されたtoken programへのCPIを使用してtoken accountを初期化します。選択されたプログラムがToken Extension Programの場合、ATAプロセッサはimmutable owner extensionも初期化します。 | [Source](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | *rs`process_create_associated_token_account`*が使用するヘルパーで、System ProgramへのCPIによってPDAアカウントを作成します。                                                                                                       | [Source](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |

### Typescript

以下の`Kit`例は、`@solana/kit`を使用した推奨アプローチを示しています。
`@solana/web3.js`を使用したレガシー例も参考用に含まれています。
...
任意のウォレットとトークンの組み合わせに対して、ATAアドレスは正確に1つ存在します。これにより、token
accountアドレスを手動で追跡する必要がなくなり、常に正しいアドレスを導出できます。
...
- 決定論的アドレス(PDA)でtoken accountを作成
- Token Programへのクロスプログラム呼び出し(CPI)を実行
- 自身では状態を保持しない

作成されたtoken accountはToken
Programによって所有され、標準的な`Account`構造を使用します。
...
token
accountの作成には、[`InitializeAccount`](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L63)instructionが必要です。[実装はこちら](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/processor.rs#L83)を参照してください。

token accountを作成するには、2つのinstructionsが必要です:

1. System Program: token
   account用のスペースを割り当てたアカウントを作成し、所有権をToken
   Programに移譲
2. Token Program: token accountデータを初期化
...
associated token
account（ATA）の作成には、[`Create`](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/program/src/instruction.rs#L18)インストラクションが必要です。[実装はこちら](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/program/src/processor.rs#L66)を参照してください。

この単一のインストラクションは、クロスプログラム呼び出し（CPI）を通じて、token
