---
title: トークンアカウントの作成
description: 特定のミントとオーナーに対応するトークンアカウントと関連トークンアカウントを作成します。
...
トークンアカウントは、Solana上で1つのミントと1つのトークンアカウントオーナーに対応するトークンを保存します。
...
ここで、_rs`owner`_
とは、トークンアカウントからトークンを転送、バーン、または委任できる権限を意味します。アカウントのプログラムオーナーは、引き続きToken
ProgramまたはToken Extensions Programです。

各トークンアカウントは、厳密に1つのミントに紐付けられています。つまり、トークンアカウントは1種類のトークンのみを保持でき、そのトークンはトークンアカウントの*rs`mint`*
フィールドで識別されます。

## 関連トークンアカウントとは

associated token
account(ATA)は、ウォレットとミントに対応するデフォルトのトークンアカウントです。
...
Associated Token
Programは、標準的で決定論的なアドレスにトークンアカウントを作成する方法です。作成されたアカウントは、Associated
Token Programではなく、Token ProgramまたはToken Extensions
Programが所有するトークンアカウントです。
...
任意のウォレット、トークンプログラム、ミントの組み合わせに対して、ATAアドレスは厳密に1つ存在します。Associated
Token
Programはそのアドレスに標準的なトークンアカウントを作成し、作成されたアカウントは引き続きToken
ProgramまたはToken Extensions Programで定義された*rs`Account`* 型を使用します。

## 関連トークンアカウントの作成方法

関連トークンアカウントの作成には、Associated Token Programの
*rs`Create`*または*rs`CreateIdempotent`*命令を使用します。Associated Token
ProgramはATAアドレスを導出し、ATAアドレスにアカウントを作成し、Token
ProgramまたはToken Extension Programが所有するtoken
accountとしてアカウントを初期化します。

<Callout type="info" title="推奨">
  ほとんどのアプリケーションでは、System ProgramとToken
  Programの命令を直接呼び出してtoken accountを作成するのではなく、Associated
  Token Programを通じてtoken accountを作成してください。associated token
  accountは、所有者、token
  program、mintから導出される決定的なアドレスを使用するため、ウォレットやアプリケーションが特定のmintのデフォルトtoken
  accountを見つけやすくなります。
</Callout>
...
| _rs`CreateIdempotent`_                        | associated token accountを作成しますが、同じ所有者とmintのATAがすでに存在する場合でも成功します。                                                                                                                          | [ソース](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | ATAアドレスを導出し、*rs`create_pda_account`*と選択されたtoken programへのCPIを使用してtoken accountを初期化します。選択されたプログラムがToken Extension Programの場合、ATAプロセッサは不変所有者拡張機能も初期化します。 | [ソース](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | *rs`process_create_associated_token_account`*が使用するヘルパーで、System ProgramへのCPIによってPDAアカウントを作成します。                                                                                                | [ソース](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |

### Typescript

以下の`Kit`例は、`@solana/kit`を使用した推奨アプローチを示しています。
`@solana/web3.js`を使用したレガシー例は参考として含まれています。
...
トークンアカウントの作成には2つのinstructionsが必要です:

1. System Programの*rs`CreateAccount`*
   instructionは、新しいrentエクゼンプトアカウントを作成し、新しいアカウントのプログラム所有者としてToken
   Programを割り当てます。
2. Token
   Programの*rs`InitializeAccount`*、_rs`InitializeAccount2`_、または*rs`InitializeAccount3`*
   instructionは、ミントと所有者のために新しいアカウントを初期化します。

_rs`CreateAccount`_ instructionとtoken
account初期化instructionを同じトランザクションに含めてください。

token account初期化中、Token
Programはアカウントがまだ初期化されておらず、rentエクゼンプトであることを確認します。
...
<Callout type="info">
  ...
  accountを使用しない特定の理由がある場合や、独自のSolanaプログラムからSystem
  ProgramとToken Program instructionsへのCPIを行ってカスタムPDA token
  accountを作成する必要がある場合に、直接System ProgramとToken
  Programの呼び出しを使用してください。
</Callout>
...
| --------------------------------- | --------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_                     | すべてのtoken accountに格納される基本的なtoken accountフィールド。                                  | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
...
| _rs`InitializeAccount2`_          | アカウントリストではなくinstruction dataで所有者を渡すtoken account初期化instruction。              | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L352-L365) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L419-L433) |
...
| _rs`process_initialize_account`_  | *rs`InitializeAccount`*のパブリックハンドラー。                                                     | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L143-L150)     | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L236-L240)     |
| _rs`process_initialize_account2`_ | *rs`InitializeAccount2`*のパブリックハンドラー。                                                    | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L152-L160)     | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L242-L246)     |
| _rs`process_initialize_account3`_ | *rs`InitializeAccount3`*のパブリックハンドラー。                                                    | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L162-L170)     | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L248-L252)     |
