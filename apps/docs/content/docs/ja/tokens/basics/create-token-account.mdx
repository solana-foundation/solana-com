...
<Callout type="info">
  ソースコードでは、開発者がToken
  accountを`Account`型として呼んでいることに注意してください。[Token
  ...
  Extension
  Program](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/state.rs#L102-L123)の両方が、Token
  accountの同じ基本実装を共有しています。
</Callout>
...
- Mintアドレス: `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Circleのtoken account: `3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
- このアカウントはUSDCトークンのみを保持します
- Circleはtoken account owner authorityを通じてトークンを管理します:
...
作成されたtoken accountsはToken
Programによって所有され、標準の`Account`構造を使用します。
...
token
accountの作成には、[`InitializeAccount`](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L63)インストラクションが必要です。[実装はこちら](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/processor.rs#L83)を参照してください。
