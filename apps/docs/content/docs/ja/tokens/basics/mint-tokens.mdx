---
title: トークンのミント
description: ミント権限を使用して、トークンアカウントに新しいトークンユニットをミントします。
...
ミントは既存のトークンの新しいユニットを作成し、ミントの総供給量を増加させます。

ミント権限のみが新しいトークンをミントできます。送信先のtoken
accountは既に存在している必要があり、同じミントに属している必要があります。

## トークンをミントする方法

トークンのミントには、Token Programの*rs`MintTo`*または*rs`MintToChecked`*
instructionを使用します。

以下の例では*rs`MintToChecked`*を使用しており、これは呼び出し側がミントの小数点以下桁数を提供することを要求し、instructionがミント前に期待されるトークンの精度を検証できるようにします。

[ネイティブミント](/docs/tokens/basics/sync-native)はミントをサポートしていません。

### ソースリファレンス

| 項目                  | 説明                                                                                                                                           | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_            | ミント状態はミント権限と総トークン供給量を保存します。                                                                                         | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_         | token account状態は、ミントによって更新される送信先の残高を保存します。                                                                        | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`MintTo`_          | 呼び出し側がミントの小数点以下桁数を提供することなく、既存のtoken accountにトークンをミントするミントinstruction。                             | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L159-L177) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L202-L220) |
| _rs`MintToChecked`_   | 呼び出し側がミントの小数点以下桁数を提供することを要求し、既存のtoken accountにトークンをミントする前にその値をチェックするミントinstruction。 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L301-L325) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L367-L391) |
| _rs`process_mint_to`_ | トークンをミントするための共有プロセッサロジック。                                                                                             | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L521-L583)     | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L981-L1067)    |

### Typescript

以下の`Kit`の例は、`@solana/kit`を使用した推奨アプローチを示しています。
`@solana/web3.js`を使用したレガシー例は参考用に含まれています。
