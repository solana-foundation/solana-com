---
title: トークンのミント
description: ミント権限を使用してトークンアカウントに新しいトークン単位をミントします。
...
ミントは既存のトークンの新しい単位を作成し、ミントの総供給量を増やします。

新しいトークンをミントできるのはミント権限のみです。宛先のtoken
accountは既に存在し、同じミントに属している必要があります。

## トークンのミント方法

トークンのミントには、Token Programの _rs`MintTo`_ または _rs`MintToChecked`_
instructionsを使用します。

以下の例では _rs`MintToChecked`_
を使用しています。これは呼び出し側がミントの小数点以下桁数を提供する必要があり、instructionがミント前に期待されるトークンの精度を検証できるようにします。
...
| _rs`Account`_         | token account状態は、ミントによって更新される宛先残高を保存します。                                                          | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`MintTo`_          | 呼び出し側がミントの小数点以下桁数を提供する必要なく、既存のtoken accountにトークンをミントするミントinstruction。           | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L159-L177) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L202-L220) |
| _rs`MintToChecked`_   | 呼び出し側がミントの小数点以下桁数を提供し、既存のtoken accountにトークンをミントする前にその値を確認するミントinstruction。 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L301-L325) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L367-L391) |
| _rs`process_mint_to`_ | トークンのミントのための共有プロセッサーロジック。                                                                           | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L521-L583)     | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L981-L1067)    |
