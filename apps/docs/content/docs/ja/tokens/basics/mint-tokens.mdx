...
ミントは、[`MintTo`](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L174)instructionを使用してトークンの新しいユニットを作成します。

- mint authorityのみが新しいトークンをミントできます
- ミントされたトークンを受け取るためのtoken accountが存在する必要があります
...
async fn main() -> Result<()> {
    ...
    // fee payerの新しいkeypairを生成
    let fee_payer = Keypair::new();

    // fee payerに1 SOLをエアドロップ
    ...
    // mint用の新しいaccountを作成するinstruction(token program)
    ...
    // ミントするトークンの量(小数点以下2桁で100トークン)
    let amount = 100;

    // associated token accountにトークンをミントするためのmint_to instructionを作成
    ...
    println!("associated token accountに1.00トークンをミントしました");
...
async def main():
    ...
    # ミントする量(最小単位)
    amount_to_mint = 1000000000  # 小数点以下9桁で1トークン
    ...
    async with rpc:
        ...
        print(f"ミント: {mint_address}")
        print(f"送信先: {destination_token_account}")
        print(f"量: {amount_to_mint}")
        print(f"ミント権限: {mint_authority.pubkey()}")
        print(f"支払者: {payer.pubkey()}")
        print(f"ミントトランザクションが正常に作成されました")
