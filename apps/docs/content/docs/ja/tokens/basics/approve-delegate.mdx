...
命令は、別のアカウント(委任先)に対して、あなたのトークンアカウントから特定の量のトークンを転送する権限を付与します。委任先は承認された量までトークンを転送できますが、それを超えることはできません。トークンアカウントの所有者のみが委任先を承認でき、各トークンアカウントは一度に1つの委任先のみを持つことができます。新しい承認は以前の承認を上書きします。
