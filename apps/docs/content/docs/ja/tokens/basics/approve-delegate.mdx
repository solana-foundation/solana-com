...
description:
  token
  accountから限られた量のトークンを転送または焼却する権限を別のアドレスに付与します。
...
デリゲートを承認すると、token accountの所有者に代わって、別のアドレスがtoken
accountから限られた量のトークンを転送または焼却できるようになります。

token
accountは、一度に1つの現在のデリゲートと1つの委任量を保存します。新しいデリゲートを承認すると、アカウント上の以前のデリゲートと許可量が置き換えられます。

## デリゲートを承認する方法

デリゲートの承認には、Token Programの _rs`Approve`_ または _rs`ApproveChecked`_
instructionを使用します。

以下の例では _rs`ApproveChecked`_
を使用しています。これは呼び出し元がミントと小数点以下桁数を提供する必要があり、instructionが承認を処理する前に期待されるミントとトークンの精度を検証できるようにします。
...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_         | token accountの状態は、所有者、デリゲート、委任量を保存します。                                                                             | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | token accountのデリゲートと許可量を記録するデリゲート承認instructionで、呼び出し元がミントや小数点以下桁数を提供する必要はありません。      | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | 呼び出し元がミントと小数点以下桁数を提供する必要があり、デリゲートと許可量を記録する前にこれらの値をチェックするデリゲート承認instruction。 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
| _rs`process_approve`_ | デリゲート承認の共有プロセッサロジック。                                                                                                    | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L344-L392)     | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L582-L640)     |
