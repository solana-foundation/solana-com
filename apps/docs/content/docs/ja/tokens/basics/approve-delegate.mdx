---
title: 委任の承認
description: トークンアカウントから制限された量のトークンを転送または焼却する権限を別のアドレスに委任します。
...
委任の承認により、別のアドレスがtoken accountの所有者に代わって、token
accountから制限された量のトークンを転送または焼却できるようになります。

token
accountは、一度に1つの現在の委任先と1つの委任量を保存します。新しい委任先を承認すると、アカウント上の以前の委任先と許可量が置き換えられます。

## 委任先を承認する方法

委任先の承認には、Token Programの*rs`Approve`*または*rs`ApproveChecked`*
instructionを使用します。

以下の例では*rs`ApproveChecked`*を使用しています。これは、呼び出し元がミントと小数点以下桁数を提供する必要があり、承認を処理する前にinstructionが期待されるミントとトークンの精度を検証できるようにします。
...
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_         | token accountの状態は、所有者、委任先、および委任量を保存します。                                                                 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | 呼び出し元がミントや小数点以下桁数を提供する必要なく、token accountの委任先と許可量を記録する委任承認instruction。                | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | 呼び出し元がミントと小数点以下桁数を提供する必要があり、委任先と許可量を記録する前にそれらの値をチェックする委任承認instruction。 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
| _rs`process_approve`_ | 委任承認の共有プロセッサロジック。                                                                                                | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L344-L392)     | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L582-L640)     |
