...
デリゲートの承認には、Token Programの*rs`Approve`*または*rs`ApproveChecked`*
instructionを使用します。

以下の例では*rs`ApproveChecked`*を使用しています。これは呼び出し元がミントと小数点以下桁数を提供する必要があり、instructionが承認を処理する前に期待されるミントとトークンの精度を検証できるようにします。
