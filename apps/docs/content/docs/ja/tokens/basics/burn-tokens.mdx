---
title: トークンを焼却する
description: token accountからトークンをバーンし、mintの総供給量を減らします。
...
トークンのバーンは、token
accountの残高を永続的に減少させ、同じ量だけmintの総供給量を削減します。

## トークンをバーンする方法

トークンのバーンには、Token
Programの*rs`Burn`*または*rs`BurnChecked`*のinstructionsを使用します。

以下の例では*rs`BurnChecked`*を使用しており、これは呼び出し側がmintのdecimalsを提供する必要があるため、バーン前に期待されるトークンの精度をinstructionが検証できます。

[ネイティブmint](/docs/tokens/basics/sync-native)はバーンをサポートしていません。代わりに*rs`CloseAccount`*を使用してください。

token
accountの所有者または承認されたデリゲートがトランザクションに署名します。Token
Extensions
Programでは、mintにパーマネントデリゲート拡張が有効になっている場合、mintのパーマネントデリゲートもバーンを承認できます。

### ソース参照

| 項目               | 説明                                                                                                                                   | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_         | mintの状態には、総トークン供給量が格納されます。                                                                                       | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | token accountの状態には、バーンによって減少するトークン残高が格納されます。                                                            | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | token accountの残高とmint供給量を減少させるバーンinstructionで、呼び出し側がmintのdecimalsを提供する必要がありません。                 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | 呼び出し側がmintのdecimalsを提供する必要があり、token accountの残高とmint供給量を減少させる前にその値をチェックするバーンinstruction。 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
