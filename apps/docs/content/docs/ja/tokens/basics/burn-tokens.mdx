---
title: トークンを焼却する
description: token accountからトークンをバーンし、ミントの総供給量を減らします。
...
トークンのバーンは、token
accountの残高を永続的に減少させ、ミントの総供給量も同じ量だけ減少させます。

## トークンのバーン方法

トークンのバーンには、Token
Programの*rs`Burn`*または\_rs`BurnChecked`\_instructionsを使用します。

以下の例では*rs`BurnChecked`*を使用しています。これは、バーンを実行する前にinstructionが想定されるトークンの精度を検証できるよう、呼び出し側がミントのデシマルを提供する必要があります。

[ネイティブミント](/docs/tokens/basics/sync-native)はバーンをサポートしていません。代わりに*rs`CloseAccount`*を使用してください。

token accountの所有者または承認された委任者がトランザクションに署名します。Token
Extensions
Programでは、ミントにパーマネント委任者拡張が有効になっている場合、ミントのパーマネント委任者もバーンを承認できます。

### ソースリファレンス

| 項目               | 説明                                                                                                                             | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_         | ミントステートはトークンの総供給量を保存します。                                                                                 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | token accountステートは、バーンによって減少するトークン残高を保存します。                                                        | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | 呼び出し側がミントのデシマルを提供することなく、token accountの残高とミントの供給量を減少させるバーンinstruction。               | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | 呼び出し側がミントのデシマルを提供し、token accountの残高とミントの供給量を減少させる前にその値をチェックするバーンinstruction。 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | トークンバーンの共有プロセッサロジック。                                                                                         | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |

### Typescript

以下の`Kit`の例は、`@solana/kit`を使用した推奨アプローチを示しています。
`@solana/web3.js`を使用した従来の例も参考として含まれています。
