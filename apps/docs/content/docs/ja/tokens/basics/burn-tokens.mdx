---
title: トークンのバーン
description: トークンアカウントからトークンをバーンし、ミントの総供給量を減らします。
...
トークンをバーンすると、トークンアカウントの残高が永続的に減少し、ミントの総供給量も同じ量だけ減少します。

## トークンをバーンする方法

トークンのバーンには、Token Programの _rs`Burn`_ または _rs`BurnChecked`_
instructionsを使用します。

以下の例では _rs`BurnChecked`_
を使用しています。これは、バーンする前にトークンの精度を検証するために、呼び出し側がミントの小数点以下桁数を提供する必要があります。

[ネイティブミント](/docs/tokens/basics/sync-native)はバーンをサポートしていません。代わりに
_rs`CloseAccount`_ を使用してください。

トークンアカウントの所有者または承認された委任者がトランザクションに署名します。Token
Extensions
Programでは、ミントに永続的委任者拡張が有効になっている場合、ミントの永続的委任者もバーンを承認できます。

### ソースリファレンス

| 項目               | 説明                                                                                                                                 | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_         | ミント状態は総トークン供給量を保存します。                                                                                           | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | トークンアカウント状態は、バーンによって減少したトークン残高を保存します。                                                           | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | 呼び出し側がミントの小数点以下桁数を提供する必要なく、トークンアカウント残高とミント供給量を減少させるバーンinstructions。           | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | 呼び出し側がミントの小数点以下桁数を提供し、トークンアカウント残高とミント供給量を減少させる前にその値を検証するバーンinstructions。 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
