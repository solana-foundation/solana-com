---
title: トークンアカウントのクローズ
description:
  トークンアカウントをクローズし、アカウントのlamportを宛先アカウントに返却します。
...
## トークンアカウントのクローズとは何をするのか?

token accountを閉じると、そのアカウントが削除され、返金可能な最小ストレージ残高を含むすべてのlamportが送信先アカウントに転送されます。

アカウントを閉じる前に、トークン残高がゼロである必要があります。凍結されたアカウントのトークン残高がゼロであれば、凍結されたアカウントも閉じることができます。Wrapped SOL token accountは例外であり、トークン残高がある状態でも閉じることができ、基となるSOLを回収できます。アカウントオーナーまたはアカウントのclose authorityがclose instructionsに署名し、送信先を選択します。ATAの作成費用を支払ったアカウントが、自動的に回収された残高を受け取るわけではありません。

## トークンアカウントをクローズする方法

トークンアカウントのクローズには、Token Programの _rs`CloseAccount`_
instructionを使用します。

_rs`CloseAccount`_
instructionは、送信元アカウントのlamportを宛先アカウントに転送し、送信元アカウントをクリアして削除します。Token
Extensions
Programは特定の拡張機能に対して追加のクローズチェックを適用しますが、基本的なクローズフローは同じです。

### ソースリファレンス

| 項目                        | 説明                                                                                                     | Token Program                                                                                                  | Token Extensions Program                                                                                              |
| --------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | トークンアカウントの状態は、アカウントクローズ時に使用される残高、ネイティブステータス、クローズ権限を保存します。     | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | トークンアカウントをクローズし、トークンアカウントのlamportを宛先アカウントに転送するinstruction。 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | トークンアカウントのクローズのための共有プロセッサロジック。                                                               | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |

### Typescript

以下の例では、`Kit`を使用した推奨アプローチを示しています。
