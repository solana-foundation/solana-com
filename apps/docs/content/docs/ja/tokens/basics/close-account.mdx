---
title: トークンアカウントを閉じる
description: トークンアカウントを閉じ、アカウントのlamportを送信先アカウントに返却します。
...
## トークンアカウントを閉じると何が起こるか？

トークンアカウントを閉じると、アカウントが削除され、トークンアカウントのrentとしてのlamportが送信先アカウントに返却されます。

アカウントを閉じる前に、トークン残高はゼロでなければなりません。凍結されたアカウントでも、トークン残高がゼロであれば閉じることができます。ラップされたSOLトークンアカウントは例外で、基礎となるSOLを回収するためにトークン残高があっても閉じることができます。アカウント所有者またはアカウントのクローズ権限が閉じるinstructionに署名します。

## トークンアカウントを閉じる方法

トークンアカウントを閉じるには、Token Programの*rs`CloseAccount`*
instructionを使用します。

_rs`CloseAccount`_
instructionは、ソースアカウントのlamportを送信先アカウントに転送し、ソースアカウントをクリアして削除します。Token
Extensions
...
| --------------------------- | --------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | token accountの状態は、アカウント閉鎖時に使用される残高、ネイティブステータス、クローズ権限を保存します。 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | token accountを閉じ、token accountのlamportを送信先アカウントに転送するinstruction。                      | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | token accountクローズのための共有プロセッサロジック。                                                     | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |
