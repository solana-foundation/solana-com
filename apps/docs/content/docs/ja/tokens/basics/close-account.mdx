---
title: トークンアカウントを閉じる
description: トークンアカウントを閉鎖し、アカウントのlamportを宛先アカウントに返却します。
...
トークンアカウントを閉鎖すると、アカウントが削除され、トークンアカウントのrentのlamportが宛先アカウントに返却されます。

アカウントを閉鎖する前に、トークン残高はゼロでなければなりません。凍結されたアカウントでも、トークン残高がゼロであれば閉鎖することができます。ラップされたSOLトークンアカウントは例外であり、基礎となるSOLを回収するためにトークン残高がある状態でも閉鎖できます。アカウント所有者またはアカウントの閉鎖権限者が閉鎖instructionsに署名します。

## トークンアカウントを閉鎖する方法

トークンアカウントの閉鎖には、Token Programの _rs`CloseAccount`_
instructionを使用します。

_rs`CloseAccount`_
instructionは、送信元アカウントのlamportを宛先アカウントに転送し、送信元アカウントをクリアして削除します。Token
Extensions
Programは特定の拡張機能に対して追加の閉鎖チェックを適用しますが、基本的な閉鎖フローは同じです。

### ソースリファレンス

| 項目                        | 説明                                                                                                         | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------------- | ------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | トークンアカウント状態には、アカウント閉鎖時に使用される残高、ネイティブステータス、閉鎖権限が格納されます。 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | トークンアカウントを閉鎖し、トークンアカウントのlamportを宛先アカウントに転送するinstruction。               | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | トークンアカウント閉鎖のための共有プロセッサーロジック。                                                     | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |

### Typescript

以下の`Kit`の例では、`@solana/kit`を使用した推奨アプローチを示しています。参考のため、`@solana/web3.js`を使用したレガシー例も含まれています。
