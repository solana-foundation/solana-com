...
命令は、ラップされたSOL（WSOL）token
accountの残高を、その中に保存されている実際のSOL（lamports）と同期します。ネイティブSOLをWSOL
token
accountに転送しても、トークン残高は自動的に更新されません。正しいWSOL残高を反映させるには、SyncNativeを呼び出す必要があります。
