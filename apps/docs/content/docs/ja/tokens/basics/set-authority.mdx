...
accountは所有者を保存し、オプションでクローズ権限を保存できます。各ロールは特定の権限セットを制御し、権限を*rs`None`*に設定すると、選択した権限ロールが永久に削除されます。
...
accountの1つの権限タイプを更新します。現在の権限が変更に署名し、新しい権限は別のアドレスまたは*rs`None`*にすることができ、*rs`None`*を指定すると選択した権限が永久に取り消されます。

### ソースリファレンス

| 項目                        | 説明                                                                        | Token Program                                                                                                  | Token Extensions Program                                                                                               |
| --------------------------- | --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                  | mintの状態は*rs`mint_authority`*と*rs`freeze_authority`*を保存します。      | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)           |
