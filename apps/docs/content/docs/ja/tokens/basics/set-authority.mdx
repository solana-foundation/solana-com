---
title: 権限の設定
description: mintまたはtoken accountの権限を設定する方法を学びます。
---
...
- **mint authority**: 新しいトークンの作成を制御します。任意のtoken
  accountにトークンをmintできます。固定供給量のトークンを作成するため、初期供給後に取り消されることがよくあります。
- **freeze authority**: token
  accountをfreezeおよびthawする機能を制御します。任意のtoken
  accountがトークンを転送することを防ぐことができます。ユーザーのトークンがfreezeされないことを保証するため、取り消されることがよくあります。

### token accountの権限

- **account owner**: token
  accountを完全に制御します。トークンの転送、トークンのburn、delegateの承認、残高がゼロの場合のアカウントのcloseが可能です。
- **close authority**: 残高がゼロの場合にtoken
  accountをcloseできます。デフォルトではaccount
  ownerですが、別のアカウントに委任できます。

## 権限の設定方法

`SetAuthority`命令は、mintおよびtoken
accountの権限を変更または取り消します。現在の権限者のみが、権限を新しいアドレスに転送するか、nullに設定して永久に取り消すことができます。一度取り消されると、権限を復元することはできません。

<Callout type="info">
  [Token
  Program](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L153)と[Token
  Extension
  Program](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/instruction.rs#L195)は、同じ機能を実現するために類似した実装を共有しています。
</Callout>
