...
title: 権限の設定
description:
  ミントまたはtoken accountのmint authority、freeze authority、owner、close
  authorityなどの権限ロールを変更または取り消します。
...
権限の設定は、mint accountまたはtoken
accountの特定の権限ロールを変更または取り消します。

mint accountはmint authorityとfreeze authorityを保存できます。token
accountはownerを保存し、オプションでclose
authorityを保存できます。各ロールは特定の権限セットを制御し、権限を*rs`None`*に設定すると、選択された権限ロールが永久に削除されます。

Token Extensions Programは、特定の拡張機能用に追加の権限タイプも定義しています。

## 権限を設定する方法

権限の変更には、Token Programの*rs`SetAuthority`* instructionを使用します。

各呼び出しは、1つのmint accountまたはtoken
accountの1つの権限タイプを更新します。現在の権限が変更に署名し、新しい権限は別のアドレスまたは*rs`None`*のいずれかになり、*rs`None`*を指定すると選択された権限が永久に取り消されます。

### ソースリファレンス

| 項目                        | 説明                                                                           | Token Program                                                                                                  | Token Extensions Program                                                                                               |
| --------------------------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                  | mint accountの状態は*rs`mint_authority`*と*rs`freeze_authority`*を保存します。 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)           |
| _rs`Account`_               | token accountの状態は*rs`owner`*と*rs`close_authority`*を保存します。          | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)          |
| _rs`AuthorityType`_         | *rs`SetAuthority`*が更新できる権限ロール。                                     | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L739-L751) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1116-L1157) |
