...
命令は、特定のトークンアカウントからのすべてのトークン転送またはトークンバーンを防止します。凍結されると、そのアカウントは解凍されるまでトークンを送信したり受信したりすることができません。凍結されたアカウントは、トークン残高がゼロの場合にのみクローズできます。トークンのミントの凍結権限者のみがアカウントを凍結できます。ミントアカウントで凍結権限が取り消された（nullに設定された）場合、トークンは二度と凍結できなくなります。
