...
トークンアカウントのフリーズには、Token Programの _rs`FreezeAccount`_
instructionを使用します。

_rs`FreezeAccount`_
instructionは、トークンアカウントがミントに属していること、および署名者がミントのフリーズ権限であることを検証し、トークンアカウントの状態を初期化済みからフリーズ済みに変更します。

[ネイティブトークンアカウント](/docs/tokens/basics/sync-native)はフリーズをサポートしていません。

### ソースリファレンス

| 項目                                | 説明                                                                                                   | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | ミントの状態は、トークンアカウントをフリーズするために使用されるオプションのフリーズ権限を保存します。 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | _rs`Initialized`_ と _rs`Frozen`_ を含むトークンアカウントの状態。                                     | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
