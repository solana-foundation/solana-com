---
title: アカウントのフリーズ
description: ミントのフリーズ権限を使用してトークンアカウントをフリーズし、転送、バーン、委任の変更をブロックします。
...
トークンアカウントをフリーズすると、所有者、ミント、残高は同じままですが、トークンアカウントが解凍されるまで、トークンの転送やバーンができなくなります。

フリーズされたアカウントは、トークン残高がゼロの場合、クローズすることができます。

フリーズ権限を持つミントのみが、トークンアカウントをフリーズできます。ミントのフリーズ権限が取り消された場合、そのミントのアカウントはフリーズできなくなります。

## トークンアカウントのフリーズ方法

トークンアカウントのフリーズには、Token
Programの\_rs`FreezeAccount`\_instructionを使用します。

\_rs`FreezeAccount`\_instructionは、トークンアカウントがミントに属していること、署名者がミントのフリーズ権限であることを検証し、トークンアカウントの状態を初期化済みからフリーズ済みに変更します。

[ネイティブトークンアカウント](/docs/tokens/basics/sync-native)は、フリーズをサポートしていません。

### ソースリファレンス

| 項目                                | 説明                                                                                                         | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | ミント状態には、トークンアカウントをフリーズするために使用されるオプションのフリーズ権限が格納されています。 | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | *rs`Initialized`*と*rs`Frozen`*を含むトークンアカウントの状態。                                              | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | トークンアカウントをフリーズ状態に変更するinstruction。                                                      | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
| _rs`process_toggle_freeze_account`_ | トークンアカウントのフリーズと解凍のための共有プロセッサロジック。                                           | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |
