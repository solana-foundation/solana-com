...
[`FreezeAccount`](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L228)命令は、特定のトークンアカウントからのすべてのトークン転送またはトークンバーンを防止します。凍結されると、アカウントは解凍されるまで、トークンの送信、トークンの受信、またはクローズができなくなります。トークンのミントの凍結権限のみがアカウントを凍結できます。ミントアカウントで凍結権限が取り消された場合(nullに設定)、トークンは永久に凍結できなくなります。
