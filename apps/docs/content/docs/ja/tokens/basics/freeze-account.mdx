...
token accountを凍結すると、同じ所有者、ミント、残高が維持されますが、token
accountの凍結が解除されるまで、トークンの受け取り、転送、またはバーンができなくなります。
...
_rs`FreezeAccount`_
命令は、トークンアカウントがミントに属していること、署名者がミントのフリーズ権限であることを検証し、トークンアカウントの状態を初期化済みからフリーズ済みに変更します。
