...
Token Programは、\_rs`Mint`アカウントタイプを次のように定義しています：
...
1. System
   Programの*rs`CreateAccount`*命令は、rentを免除された新しいアカウントを作成し、Token
   Programを新しいアカウントのプログラムオーナーとして割り当てます。
2. Token
   Programの*rs`InitializeMint`*または*rs`InitializeMint2`*命令は、新しいアカウントをミントとして初期化します。

*rs`CreateAccount`*命令とミント初期化命令を同じトランザクションに含めてください。

ミント初期化時に、Token Programはmint
accountがまだ初期化されておらず、rentが免除されていることを確認します。その後、Token
Programはミント権限、凍結権限、小数点桁数、\_rs`is_initialized`フラグをmint
accountデータに書き込みます。
...
| _rs`process_initialize_mint`_  | \_rs`InitializeMint`のパブリックハンドラ。                              | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L62-L70)       | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L132-L140)     |
| _rs`process_initialize_mint2`_ | \_rs`InitializeMint2`のパブリックハンドラ。                             | [ソース](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L72-L81)       | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L142-L151)     |
