...
です。このアドレスは、Solana エコシステム全体で USDC を一意に識別します。このミントは
...
命令が必要です。
[実装の詳細はこちら](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/processor.rs#L27)を参照してください。
...
mint
accountは基本的なトークン情報（供給量、小数点以下桁数、権限）のみを保存します。トークンに名前、シンボル、画像などの人間が読めるメタデータを追加するには、2つの選択肢があります：
...
mint
accountは基本的なトークン情報(供給量、小数点以下桁数、権限)のみを保存します。名前、シンボル、画像などの人間が読めるメタデータをトークンに追加するには、2つのオプションがあります:

<Cards>
  <Card title="Metaplex Token Metadata" href="/docs/tokens/metaplex">
   Metaplex Token Metadataプログラムを使用してメタデータを追加します。オリジナルのToken ProgramとToken-2022の両方で動作します。
  </Card>

  <Card title="Token Extensions Metadata" href="/docs/tokens/extensions/metadata">
   Token-2022の組み込みメタデータ拡張機能を使用します。
   メタデータ拡張機能は、Token-2022で作成されたmint accountでのみ利用可能です。
   オリジナルのToken Programで作成されたmint accountにメタデータを追加する場合は、Metaplex Token Metadataプログラムを使用してください。
  </Card>
</Cards>
