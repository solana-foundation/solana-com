...
description:
  Token Extensions
  Programのトークンアカウントを不変の所有権で初期化し、所有者を再割り当てできないようにします。
...
Token Extensions
Programの*rs`ImmutableOwner`*アカウント拡張機能により、トークンアカウントの所有者が永続的になります。

トークンアカウントが*rs`ImmutableOwner`*で初期化されると、後続の*rs`SetAuthority`*
instructionsでその*rs`AccountOwner`*権限を変更できなくなります。

<Callout type="info" title="関連付けられたトークンアカウント">
  Token Extensions Program用のAssociated Token
  Programを通じて作成されたassociated token
  accountは、トークンアカウント上で_rs`ImmutableOwner`_を自動的に初期化します。
</Callout>
...
1. ミントを作成して初期化します。
2. *rs`ImmutableOwner`*に十分なスペースを持つトークンアカウントを作成します。
3. *rs`InitializeAccount`*の前にトークンアカウント上で*rs`ImmutableOwner`*を初期化します。
4. 後で*rs`SetAuthority`*でアカウント所有者を変更しようとすると、*rs`TokenError::ImmutableOwner`*で失敗します。

<ScrollyCoding>

## !!steps トークンアカウントサイズの計算

基本トークンアカウントと*rs`ImmutableOwner`*拡張機能のトークンアカウントサイズを計算します。これは*rs`CreateAccount`*で使用されるサイズです。
...
*rs`ImmutableOwner`*拡張機能に必要なトークンアカウントサイズを使用してrentを計算します。
...
## !!steps ImmutableOwnerの初期化

token accountに*rs`ImmutableOwner`*拡張を初期化します。
...
## !!steps token accountの初期化

同じトランザクション内で*rs`InitializeAccount`*を使用してtoken
accountを初期化します。
...
<Callout type="info" title="instructionsの順序">
  _rs`InitializeImmutableOwner`_は_rs`InitializeAccount`_より前に実行する必要があります。
  _rs`CreateAccount`_、_rs`InitializeImmutableOwner`_、および_rs`InitializeAccount`_は同じトランザクションに含める必要があります。
</Callout>
...
| 項目                                     | 説明                                                                                        | ソース                                                                                                                           |
| ---------------------------------------- | ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| _rs`ImmutableOwner`_                     | token account所有者を不変としてマークするアカウント拡張。                                   | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L13-L16) |
| _rs`InitializeImmutableOwner`_           | 未初期化のtoken accountにimmutable-owner拡張を初期化するinstruction。                       | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1982-L1991)           |
| _rs`process_initialize_immutable_owner`_ | アカウントに*rs`ImmutableOwner`*を割り当てて初期化するプロセッサロジック。                  | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1532-L1543)               |
| _rs`SetAuthority`_                       | token account所有者を含む権限を変更するために使用される基本トークンinstruction。            | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1611-L1633)           |
| _rs`process_set_authority`_              | token accountに*rs`ImmutableOwner`*が有効な場合、所有者の変更を拒否するプロセッサロジック。 | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L703-L900)                 |

### Typescript

以下の`Kit`の例では、生成されたinstructionsを直接使用しています。`@solana/web3.js`および`@solana/spl-token`を使用したレガシー例も参考のために含まれています。
