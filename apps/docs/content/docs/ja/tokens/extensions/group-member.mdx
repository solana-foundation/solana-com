...
title: トークングループとメンバー
description:
  GroupPointer、GroupMemberPointer、TokenGroup、TokenGroupMember拡張機能を使用して、グループデータをmint
  accountに直接保存するグループおよびグループメンバーミントを作成します。
...
## グループおよびグループメンバー拡張機能とは?

グループとは、コレクションを表すミントです。グループメンバーとは、そのコレクションに属するミントです。

1つのミントがコレクションを表し、他のミントがそれに属するアイテムを表す場合に、これらの拡張機能を使用します。

Token Extensions
Programは、4つの関連する拡張機能を使用して、トークングループデータをミントに直接保存できます:

- _rs`GroupPointer`_
  は、グループのデータを保存するアカウントにミントを指定します。
- _rs`TokenGroup`_
  は、更新権限、現在のサイズ、最大サイズを含むグループデータ自体を保存します。
- _rs`GroupMemberPointer`_
  は、メンバーのデータを保存するアカウントにミントを指定します。
- _rs`TokenGroupMember`_
  は、メンバーのグループアドレスとメンバー番号を保存します。

_rs`GroupPointer`_ および _rs`GroupMemberPointer`_
は、[トークングループインターフェース](https://github.com/solana-program/token-group/tree/main/interface)を実装するプログラムが所有する任意のアカウントを参照できます。

Token Extensions Programは、_rs`TokenGroup`_ および _rs`TokenGroupMember`_
ミント拡張機能を通じて、そのインターフェースを直接実装しています。

## mint accountに保存されるグループとメンバーの作成方法

mint accountに保存されるグループとメンバーを作成するには:

1. グループmint accountを作成し、_rs`GroupPointer`_ を初期化します。
2. _rs`InitializeMint`_ でグループミントを初期化します。
3. 同じミントに _rs`TokenGroup`_ を初期化します。
4. メンバーmint accountを作成し、_rs`GroupMemberPointer`_ を初期化します。
5. _rs`InitializeMint`_ でメンバーミントを初期化します。
6. グループミントを参照するように、同じミントに _rs`TokenGroupMember`_
   を初期化します。

<ScrollyCoding>

## !!steps グループミントのサイズとレントを計算する

グループミントに必要なサイズとレントを計算します。
...
## !!steps グループミントの作成と初期化

グループミントアカウントを作成し、*rs`GroupPointer`*を初期化し、ミントを初期化し、*rs`TokenGroup`*を1つのトランザクションで初期化します。
...
## !!steps メンバーミントのサイズとレントを計算する

メンバーミントに必要なサイズとレントを計算します。
...
## !!steps メンバーミントの作成と初期化

メンバーミントアカウントを作成し、*rs`GroupMemberPointer`*を初期化し、ミントを初期化し、*rs`TokenGroupMember`*を1つのトランザクションで初期化します。
...
<Callout type="info" title="ポインタとインストラクションの順序">
_rs`GroupPointer`_と_rs`GroupMemberPointer`_は、グループまたはメンバーデータが保存されているアカウントアドレスを格納します。_rs`TokenGroup`_と_rs`TokenGroupMember`_は、実際のグループまたはメンバーデータを格納します。

*rs`GroupPointerInstruction::Initialize`*と*rs`GroupMemberPointerInstruction::Initialize`*は、*rs`InitializeMint`*の前に実行する必要があります。*rs`TokenGroupInstruction::InitializeGroup`*と*rs`TokenGroupInstruction::InitializeMember`*は、*rs`InitializeMint`*の後に実行する必要があります。各ミントについて、_rs`CreateAccount`_、ポインタ初期化インストラクション、および*rs`InitializeMint`*は、同じトランザクションに含める必要があります。

</Callout>

### ソースリファレンス

*rs`GroupPointer`*と*rs`GroupMemberPointer`*は、Token Extensions
Programのポインタinstructionsです。*rs`TokenGroup`*と*rs`TokenGroupMember`*は、Token
Extensions Programが実装するトークングループインターフェースに従います。

#### グループポインタとグループメンバーポインタ

| 項目                                            | 説明                                                                                             | ソース                                                                                                                                            |
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`GroupPointer`_                              | グループのデータを格納するアカウントの権限とアドレスを保存するミント拡張機能。                   | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_pointer/mod.rs#L12-L25)                |
| _rs`GroupMemberPointer`_                        | メンバーのデータを格納するアカウントの権限とアドレスを保存するミント拡張機能。                   | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_member_pointer/mod.rs#L12-L25)         |
| _rs`GroupPointerInstruction::Initialize`_       | *rs`InitializeMint`*の前にグループポインタ拡張機能を初期化します。                               | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_pointer/instruction.rs#L22-L38)        |
| _rs`GroupPointerInstruction::Update`_           | ミントの*rs`GroupPointer`*拡張機能に格納されているグループアドレスを更新します。                 | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_pointer/instruction.rs#L39-L55)        |
| _rs`GroupMemberPointerInstruction::Initialize`_ | *rs`InitializeMint`*の前にグループメンバーポインタ拡張機能を初期化します。                       | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_member_pointer/instruction.rs#L22-L38) |
| _rs`GroupMemberPointerInstruction::Update`_     | ミントの*rs`GroupMemberPointer`*拡張機能に格納されているメンバーアドレスを更新します。           | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/group_member_pointer/instruction.rs#L39-L55) |
| _rs`process_initialize`_ (`GroupPointer`)       | 初期の*rs`GroupPointer`*権限とグループアドレスをミントに書き込みます。                           | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/group_pointer/processor.rs#L25-L50)            |
| _rs`process_update`_ (`GroupPointer`)           | グループポインタ権限を検証し、ミントに格納されているグループアドレスを再書き込みします。         | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/group_pointer/processor.rs#L52-L78)            |
| _rs`process_initialize`_ (`GroupMemberPointer`) | 初期の*rs`GroupMemberPointer`*権限とメンバーアドレスをミントに書き込みます。                     | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/group_member_pointer/processor.rs#L25-L50)     |
| _rs`process_update`_ (`GroupMemberPointer`)     | グループメンバーポインタ権限を検証し、ミントに格納されているメンバーアドレスを再書き込みします。 | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/group_member_pointer/processor.rs#L52-L78)     |

#### トークングループとトークングループメンバー

| 項目                                              | 説明                                                                                                                                                             | ソース                                                                                                                                 |
| ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`TokenGroup`_                                  | ミント上に保存されるトークングループインターフェースの状態。更新権限、現在のサイズ、最大サイズを含みます。                                                       | [ソース](https://github.com/solana-program/token-group/blob/main/interface/src/state.rs#L13-L27)                                       |
| _rs`TokenGroupMember`_                            | ミント上に保存されるトークングループメンバーインターフェースの状態。メンバーミント、グループアドレス、メンバー番号を含みます。                                   | [ソース](https://github.com/solana-program/token-group/blob/main/interface/src/state.rs#L64-L76)                                       |
| _rs`TokenGroupInstruction::InitializeGroup`_      | すでに初期化されたミントに対して新しいグループを初期化するために、Token Extensions Programでサポートされているトークングループインターフェースのinstructions。   | [ソース](https://github.com/solana-program/token-group/blob/main/interface/src/instruction.rs#L52-L62)                                 |
| _rs`TokenGroupInstruction::UpdateGroupMaxSize`_   | グループで許可されるメンバーの最大数を更新するために、Token Extensions Programでサポートされているトークングループインターフェースのinstructions。               | [ソース](https://github.com/solana-program/token-group/blob/main/interface/src/instruction.rs#L64-L70)                                 |
| _rs`TokenGroupInstruction::UpdateGroupAuthority`_ | グループの更新権限をローテーションまたはクリアするために、Token Extensions Programでサポートされているトークングループインターフェースのinstructions。           | [ソース](https://github.com/solana-program/token-group/blob/main/interface/src/instruction.rs#L72-L78)                                 |
| _rs`TokenGroupInstruction::InitializeMember`_     | すでに初期化されたグループに対して新しいメンバーを初期化するために、Token Extensions Programでサポートされているトークングループインターフェースのinstructions。 | [ソース](https://github.com/solana-program/token-group/blob/main/interface/src/instruction.rs#L80-L92)                                 |
| _rs`process_initialize_group`_                    | ミントを検証し、*rs`GroupPointer`*が存在することを確認し、グループミント上の*rs`TokenGroup`*状態を割り当てます。                                                 | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_group/processor.rs#L43-L90)   |
| _rs`process_update_group_max_size`_               | 現在の更新権限を検証し、グループの最大サイズを更新します。                                                                                                       | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_group/processor.rs#L95-L117)  |
| _rs`process_update_group_authority`_              | 現在の更新権限を検証し、グループの更新権限をローテーションまたはクリアします。                                                                                   | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_group/processor.rs#L119-L141) |
| _rs`process_initialize_member`_                   | メンバーミントとグループ権限を検証し、グループサイズをインクリメントし、ミント上の*rs`TokenGroupMember`*状態を割り当てます。                                     | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/token_group/processor.rs#L143-L202) |

### TypeScript

以下の`Kit`
の例では、生成されたインストラクションを直接使用しています。`@solana/web3.js`、`@solana/spl-token`、およびトークングループヘルパーを使用したレガシーの例は、参考として含まれています。
