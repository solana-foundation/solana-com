...
<Callout type="warn" title="利用可能状況">
  許可制バーンはToken-2022
  [`program@v11.0.0`](https://github.com/solana-program/token-2022/releases/tag/program%40v11.0.0)
  に同梱されています。
  現在devnetで利用可能であり、2026年6月にメインネットで有効化される予定です。Token
  Extension
  Programは各クラスターに個別にデプロイされるため、対象クラスターのデプロイメントがv11.0.0以降を含むことを確認してください。
</Callout>

ローカルテストvalidatorには古いバージョンのToken-2022ビルドが同梱されているため、このページのサンプルを実行するには、devnetからv11.0.0プログラムをテストvalidatorに読み込んでください：
...
権限付きBurnのMintを作成し、トークンをBurnするには：

1. mint
   accountのサイズと、MintおよびRs`PermissionedBurnConfig`\_エクステンションに必要なrentを計算します。
2. _rs`CreateAccount`_ でmint accountを作成し、_rs`PermissionedBurnConfig`_
   を初期化して、_rs`InitializeMint`_ でMintを初期化します。
3. token accountを作成し、トークンをMintします。
4. token accountオーナーとBurn権限者の両方が署名した
   _rs`PermissionedBurnInstruction::BurnChecked`_ でBurnします。
5. 任意で、_rs`AuthorityType::PermissionedBurn`_ と `None` 権限者を使用して
   _rs`SetAuthority`_ で権限付きBurnを無効化すると、標準のBurn
   instructionsが再度有効になります。以下の完全なコードサンプルにこの手順が示されています。

<ScrollyCoding>

## !!steps アカウントサイズの計算

ベースMintに*rs`PermissionedBurnConfig`*エクステンションを加えたmint
accountのサイズを計算します。これは*rs`CreateAccount`*で使用するサイズです。
...
MintにRs`PermissionedBurnConfig`\_エクステンションを加えたサイズに必要なrentを計算します。
...
計算したスペースとlamportでmint accountを作成します。
...
Burn権限者を指定して、Mint上の*rs`PermissionedBurnConfig`*エクステンションを初期化します。
...
同じトランザクション内で*rs`InitializeMint`*を使用してMintを初期化します。
...
token accountの所有者とバーン権限の両方が署名した
...
<Callout type="info" title="Instructionの順序">
  ...
  より前に実行する必要があります。_rs`CreateAccount`_、
...
| 項目                                           | 説明                                                                                                         | ソース                                                                                                                                         |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | ミントのすべてのバーンに共同署名するために必要な権限を保存するミント拡張。                                   | [ソース](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
| _rs`PermissionedBurnInstruction::Initialize`_  | _rs`InitializeMint`_ の前にパーミッションドバーン設定を初期化するinstructions。                              | [ソース](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L41-L49) |
| _rs`PermissionedBurnInstruction::Burn`_        | バーン権限およびtoken accountの所有者またはデリゲートの署名を必要とするバーンinstructions。                  | [ソース](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | _rs`PermissionedBurnInstruction::Burn`_ と同じアカウントを使用する、小数点チェック付きのバーンinstructions。 | [ソース](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | ミントのバーン権限のローテーションまたは無効化のために _rs`SetAuthority`_ と併用する権限識別子。             | [ソース](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
