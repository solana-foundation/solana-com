---
title: 一時停止可能なミント
description: 一時停止権限を持つToken-2022ミントを作成し、ミントが一時停止されている間は転送をブロックし、後でアクティビティを再開します。
...
Token Extensions
Programの*rs`PausableConfig`*ミント拡張機能により、一時停止権限者がミント全体のトークンアクティビティを一時停止および再開できます。

ミントが一時停止されると、Token Extensions Programは以下を拒否します：

- 転送
- ミント
- バーン

ミントが再開されると、これらのinstructionsは再び正常に機能します。

## ミントの作成、一時停止、再開の方法

ミントを作成、一時停止、再開するには：

1. ミントアカウントのサイズと、ミントおよび*rs`PausableConfig`*拡張機能に必要なrentを計算します。
2. *rs`CreateAccount`*でミントアカウントを作成し、*rs`PausableConfig`*を初期化し、*rs`InitializeMint`*でミントを初期化します。
3. トークンをミントし、ミントのためのトークンアカウントを作成します。
4. *rs`Pause`*を使用してミントのトークンアクティビティをブロックします。
5. *rs`Resume`*を使用してトークンアクティビティを再び許可します。

<ScrollyCoding>

## !!steps アカウントサイズの計算

基本ミントに*rs`PausableConfig`*拡張機能を加えたミントアカウントのサイズを計算します。これは*rs`CreateAccount`*で使用されるサイズです。
...
ミントに*rs`PausableConfig`*拡張機能を加えたサイズを使用してrentを計算します。
...
## !!steps PausableConfigの初期化

ミント上で*rs`PausableConfig`*拡張機能を初期化します。
...
<Callout type="info" title="命令の順序">
  _rs`PausableInstruction::Initialize`_は_rs`InitializeMint`_の前に実行する必要があります。
  _rs`CreateAccount`_、_rs`PausableInstruction::Initialize`_、および_rs`InitializeMint`_は同じトランザクションに含める必要があります。
</Callout>
...
| 項目                                  | 説明                                                                                           | ソース                                                                                                                                |
| ------------------------------------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PausableConfig`_                  | 一時停止権限とミントが現在一時停止されているかどうかを保存するミント拡張機能。                 | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/mod.rs#L17-L22)         |
| _rs`PausableInstruction::Initialize`_ | *rs`InitializeMint`*の前に一時停止可能な設定を初期化するinstructions。                         | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L20-L32) |
| _rs`PausableInstruction::Pause`_      | ミントを一時停止としてマークするinstructions。                                                 | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L33-L44) |
| _rs`PausableInstruction::Resume`_     | ミントの一時停止を解除するinstructions。                                                       | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L45-L56) |
| _rs`process_initialize`_              | 未初期化のミント上で*rs`PausableConfig`*を初期化し、一時停止権限を保存するプロセッサロジック。 | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L22-L35)     |
| _rs`process_toggle_pause`_            | ミントの一時停止フラグを切り替える前に一時停止権限を検証するプロセッサロジック。               | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L39-L64)     |

### Typescript

以下の`Kit`の例では、生成されたinstructionsを直接使用しています。`@solana/web3.js`および`@solana/spl-token`を使用した従来の例は参考のために含まれています。
