...
title: トークンミントの作成
description:
  Confidential Balancesを有効にしたトークンミントの作成方法を学びます。
---

## Confidential Balancesを使用したミントの作成方法

Confidential Balancesは、mint accountに追加の状態を付与することで、プライベートなトークン転送を可能にします。このセクションでは、必要なエクステンションを有効にしたトークンミントの作成方法について説明します。

次の図は、Confidential Balancesエクステンションを使用したミントの作成に関わるステップを示しています：
...
- **authority**: ミントのConfidential Balances設定を変更し、自動承認が無効な場合に新しい秘密アカウントを承認する権限を持つアカウントです。

- **auto_approve_new_accounts**: trueに設定すると、ユーザーはデフォルトでConfidential Balancesが有効なtoken accountを作成できます。falseの場合、authorityは各新規token accountが秘密転送に使用される前に承認する必要があります。
...
Confidential Balancesを有効にしたミントの作成には、1つのトランザクション内で3つのinstructionsが必要です：

1. **Mint Accountの作成**：System Programの _rs`CreateAccount`_
   instructionを呼び出して、mint accountを作成します。

2. **Confidential Balancesの初期化**: Token Extensions ProgramのToken Extension Programの
   ...
   instructionを呼び出して、ミントの_rs`ConfidentialTransferMint`_状態を設定します。
...
以下のコードは、Confidential Balancesを有効にしたミントの作成方法を示しています。

Confidential BalancesはZK ElGamal Proofプログラムに依存しており、mainnetおよびdevnetで有効になっています。標準の`solana-test-validator`ではこれが有効になっていませんが、[Surfpool](https://surfpool.run)のようなmainnetフォーキングのローカルvalidatorでは有効になっています。
資金が入ったpayerを使用して、これらのいずれか（コードはdevnetを使用）に対してサンプルを実行し、プレースホルダーのミントおよびアカウントアドレスを自分のものに置き換えてください。
