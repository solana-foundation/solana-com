...
1. **機密保留残高**：最初に、トークンは公開残高から「保留中」の機密残高へ「預け入れ」られます。
2. **機密利用可能残高**：次に、保留残高が利用可能残高へ「適用」され、トークンが機密転送に使用できる状態になります。
...
instructionを呼び出します。預け入れinstructionあたりの
[最大金額](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/mod.rs#L21)
は2^48に制限されています。
...
Confidential Balancesは、メインネットおよびdevnetで有効になっているZK ElGamal ProofプログラムによってサポートされていますConfidential Balancesは、メインネットおよびdevnetで有効になっているZK ElGamal Proofプログラムに依存しています。標準の`solana-test-validator`はこれを有効にしていませんが、[Surfpool](https://surfpool.run)のようなメインネットフォーキングのローカルvalidatorは対応しています。
資金が入金済みのペイヤーを使用して、これらのいずれか（コードはdevnetを使用）に対してサンプルを実行し、プレースホルダーのミントおよびアカウントアドレスをご自身のものに置き換えてください。
