...
<Callout type="warn">
  ZK
  ElGamalプログラムは、セキュリティ監査を受けているため、メインネットとdevnetで一時的に無効化されています。これは、機密転送拡張機能が現在利用できないことを意味します。概念は依然として有効ですが、コード例は実行されません。
</Callout>

## 保留残高を利用可能残高に適用する方法

トークンを機密的に転送する前に、公開トークン残高を機密残高に変換する必要があります。この変換は2つの段階で行われます。

1. **機密保留残高**: 最初に、トークンは公開残高から「保留」機密残高に「入金」されます。
2. **機密利用可能残高**: 次に、保留残高が利用可能残高に「適用」され、トークンが機密転送に利用可能になります。

このセクションでは、2番目の段階である保留残高を利用可能残高に適用することについて説明します。

トークンが公開残高から「入金」されるとき、またはトークンがあるtoken
accountから別のtoken
accountに機密的に転送されるとき、トークンは最初に機密保留残高に追加されます。トークンを機密転送に使用する前に、保留残高を利用可能残高に「適用」する必要があります。
...
次の図は、保留残高を利用可能残高に適用する際の手順を示しています。
...
保留残高を利用可能残高に変換するには、[ConfidentialTransferInstruction::ApplyPendingBalance](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L1152)命令を呼び出します。

`spl_token_client`クレートは、以下の例で示されているように、`ApplyPendingBalance`命令を含むトランザクションを構築して送信する`confidential_transfer_apply_pending_balance`メソッドを提供します。

### サンプルコード

以下の例は、機密保留残高を機密利用可能残高に適用する方法を示しています。

この例を実行するには、次のコマンドを使用して、mainnetからクローンしたToken
Extension
...
<Callout type="info">
  執筆時点では、機密転送はデフォルトのローカルバリデータでは有効になっていません。サンプルコードを実行するには、mainnetのToken
