...
トークンが公開残高から「デポジット」される場合、またはトークンがある token
account から別の token
account へ機密転送される場合、トークンは最初に機密保留残高に追加されます。機密転送にトークンを使用するには、保留中の残高を利用可能な残高に「適用」する必要があります。
...
instruction を呼び出してください。

`spl_token_client` クレートは、以下の例に示すように、`ApplyPendingBalance`
instruction を含むトランザクションを構築して送信する
...
Confidential Balancesは、メインネットおよびdevnetで有効になっているZK ElGamal Proofプログラムに依存しています。標準の`solana-test-validator`ではこれが有効になっていませんが、[Surfpool](https://surfpool.run)のようなメインネットフォーキングのローカルvalidatorでは有効になっています。
資金が入ったpayerを使用してdevnet（コードはdevnetを使用）でサンプルを実行し、プレースホルダーのmintアドレスとアカウントアドレスを自分のものに置き換えてください。
