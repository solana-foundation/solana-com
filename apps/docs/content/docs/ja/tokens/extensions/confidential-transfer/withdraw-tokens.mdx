...
1. クライアント側で
   ...
   **等価性証明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**: 引き出し後の残りの利用可能残高の暗号文が、対応する
   ...
   **範囲証明 ([BatchedRangeProofU64Data](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_range_proof/batched_range_proof_u64.rs#L37))**: 引き出し後の残りの利用可能残高が非負であり、指定された範囲内であることを検証します。
...
- ZK
  ElGamal 証明プログラムを呼び出して証明を検証し、「コンテキスト状態」アカウントを初期化する
- 2つの証明アカウントを提供して
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  instructionを呼び出す
...
以下の例では、機密利用可能残高からトークンを引き出し、公開残高に戻します。アカウントはあらかじめConfidential Balances用に設定されており、利用可能な機密残高を保有している必要があります。

Confidential BalancesはZK ElGamal Proofプログラムに依存しており、このプログラムはメインネットおよびデブネットで有効化されています。標準の`solana-test-validator`ではこれが有効化されていませんが、[Surfpool](https://surfpool.run)のようなメインネットフォーキング対応のローカルバリデーターでは有効化されています。
資金が入ったペイヤーを使用して、これらのいずれか（コードはデブネットを使用）に対してサンプルを実行し、プレースホルダーをご自身のmintおよびtoken accountに置き換えてください。
