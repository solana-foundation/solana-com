---
title: トークンの転送
description: あるトークンアカウントから別のトークンアカウントへプライベートにトークンを転送する方法を学びます。
---

<Callout type="warn">
  ZK
  ElGamalプログラムは、セキュリティ監査中のため、メインネットとdevnetで一時的に無効化されています。これは、機密転送拡張機能が現在利用できないことを意味します。概念は依然として有効ですが、コード例は実行されません。
</Callout>

## あるトークンアカウントから別のトークンアカウントへ機密転送する方法

あるトークンアカウントから別のトークンアカウントへ機密転送するには、送信者と受信者の両方が*rs`ConfidentialTransferAccount`*状態で構成され、機密転送が承認されたトークンアカウントを持っている必要があります。送信者のトークンアカウントには、転送可能な機密残高も必要です。
...
1. クライアント側で[3つの証明](https://github.com/solana-program/token-2022/blob/d073cb89dbcd430387b5c6fb4b7157911351e4a3/confidential-transfer/proof-generation/src/transfer.rs#L35)を作成します：
   ...
   **等価性証明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**：転送後の新しい利用可能残高の暗号文が対応する[Pedersenコミットメント](https://en.wikipedia.org/wiki/Commitment_scheme)と一致することを検証し、送信元アカウントの新しい利用可能残高が`new_balance = current_balance - transfer_amount`として正しく計算されていることを保証します。

   **暗号文有効性証明 ([BatchedGroupedCiphertext3HandlesValidityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_grouped_ciphertext_validity/handles_3.rs#L63))**：転送金額の暗号文が3者（送信元、送信先、監査人）すべてに対して適切に生成されていることを検証し、転送金額が各当事者の公開鍵で正しく暗号化されていることを保証します。

   **範囲証明 ([BatchedRangeProofU128Data](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_range_proof/batched_range_proof_u128.rs#L37))**：新しい利用可能残高と転送金額（下位/上位ビットに分割）がすべて非負であり、指定された範囲内にあることを検証します。

2. 各証明について：
   - ZK ElGamal証明プログラムを呼び出して証明データを検証します。
...
4. プルーフコンテキスト状態アカウントを閉じて、作成に使用したSOLを回収します。
...
あるトークンアカウントから別のトークンアカウントへ機密転送するには、次の操作が必要です：

- クライアント側で等価性プルーフ、暗号文有効性プルーフ、範囲プルーフを生成する
- Zk
  ElGamalプルーフプログラムを呼び出してプルーフを検証し、「コンテキスト状態」アカウントを初期化する
- 3つのプルーフアカウントを提供して[ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  instructionを呼び出す
- 3つのプルーフアカウントを閉じてrentを回収する

`spl_token_client`クレートは次のメソッドを提供します：

- プルーフアカウントを作成する`confidential_transfer_create_context_state_account`メソッド
- `Transfer` instructionを呼び出す`confidential_transfer_transfer`メソッド
- プルーフアカウントを閉じる`confidential_transfer_close_context_state_account`メソッド

### サンプルコード

次の例は、トークンアカウント間でトークンを機密転送する方法を示しています。

この例を実行するには、次のコマンドを使用して、メインネットからクローンしたToken
Extension
Programを使用してローカルvalidatorを起動します。ローカルvalidatorを起動するには、Solana
...
<Callout type="info">
  執筆時点では、機密転送はデフォルトのローカルvalidatorでは有効になっていません。サンプルコードを実行するには、メインネットのToken
