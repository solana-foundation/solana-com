...
title: トークンアカウントの作成
description:
  Confidential Balancesを有効にしたtoken accountの作成方法を学びます。
---

## Confidential Balancesを使用したtoken accountの作成方法

Confidential Balancesは、token accountに追加のステートを付与することで、プライベートなトークン転送を可能にします。このセクションでは、必要な拡張機能を有効にしたtoken accountの作成方法について説明します。

以下の図は、Confidential Balancesを有効にしたtoken accountを作成する際の手順を示しています：
...
### Confidential Balances Token Accountのステート
...
_rs`ConfidentialTransferAccount`_には、Confidential Balancesを管理するいくつかのフィールドが含まれています：

- **approved**：Confidential Balancesに対するアカウントの承認ステータス。mint accountの`auto_approve_new_accounts`設定が`true`に設定されている場合、すべてのtoken accountはConfidential Balancesに対して自動的に承認されます。
...
Confidential Balances向けのtoken accountの作成と設定には、以下のinstructionsを使用します。これらはすべて1つのトランザクションに収まります：
...
4. **Confidential Balancesの設定**：Token Extensions Programの
   ...
   instructionを呼び出し、_rs`ProofLocation::ContextStateAccount`_を通じてプルーフコンテキストステートアカウントを参照して、
   _rs`ConfidentialTransferAccount`_ステートを初期化します。

<Callout type="info">

**token accountのオーナーのみが、token accountのConfidential Balancesを有効にできます**。

_rs`ConfigureAccount`_ instructionは、token accountのオーナーのみが生成できる暗号化キーとプルーフをクライアント側で生成する必要があります。

pubkey有効性プルーフは、アカウントのElGamal公開鍵が有効であることを検証します。これは_rs`build_pubkey_validity_proof_data`_で生成され、ZK ElGamal Proofプログラムによってオンチェーンでコンテキストステートアカウントに検証され、_rs`ProofLocation::ContextStateAccount`_を通じて_rs`ConfigureAccount`_から参照されるため、プルーフのバイト列はtoken instructionそのものの中では転送されません。実装の詳細については、以下を参照してください：
...
以下のコードは、associated token accountを作成し、既存の互換性のあるmintを使用してConfidential Balancesを使用するように設定します。

Confidential Balancesは、mainnetおよびdevnetで有効になっているZK ElGamal Proofプログラムに依存しています。標準の`solana-test-validator`はこれを有効にしませんが、[Surfpool](https://surfpool.run)のようなmainnetフォーキングのローカルバリデーターは有効にします。
資金が入ったpayerを使用して、いずれかの環境（コードはdevnetを使用）に対してサンプルを実行し、mintのプレースホルダーを[Mintの作成](/docs/tokens/extensions/confidential-transfer/create-mint)に従って作成されたmintに置き換えてください。
