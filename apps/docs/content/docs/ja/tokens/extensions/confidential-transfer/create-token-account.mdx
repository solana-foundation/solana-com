...
_rs`ConfidentialTransferAccount`_ には、confidential
...
- **expected_pending_balance_credit_counter**: 最後に _rs`ApplyPendingBalance`_
...
機密保留中の残高は`pending_balance_lo`と`pending_balance_hi`に分割されています。これは、ElGamal復号化が大きな数値に対してより多くの計算を必要とするためです。暗号文演算の実装は[こちら](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/confidential-transfer/ciphertext-arithmetic/src/lib.rs#L54)で確認でき、
...
1. **トークンアカウントの作成**：Associated Token Programの
   ...
   命令を呼び出してトークンアカウントを作成します。

2. **アカウントスペースの再割り当て**: Token Extension Programの
   _rs`TokenInstruction::Reallocate`_ instructionを呼び出して、
   _rs`ConfidentialTransferAccount`_ 状態のスペースを追加します。
