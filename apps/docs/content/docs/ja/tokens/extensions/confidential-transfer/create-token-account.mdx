...
- **approved**: 機密転送に対するアカウントの承認ステータス。mint
  accountの`auto_approve_new_accounts`設定が`true`に設定されている場合、すべてのtoken
  accountは自動的に機密転送が承認されます。
...
- **maximum_pending_balance_credit_counter**: 保留中の残高を利用可能な残高に変換するために*rs`ApplyPendingBalance`*命令が必要になる前の、保留中のクレジットのカウント制限。

- **expected_pending_balance_credit_counter**: 最後に*rs`ApplyPendingBalance`*命令が処理された際に、instruction
  dataを通じてクライアントが提供した`pending_balance_credit_counter`の値。

- **actual_pending_balance_credit_counter**: 最後に*rs`ApplyPendingBalance`*命令が処理された時点でのtoken
  account上の`pending_balance_credit_counter`の値。
...
すべての入金と転送金額は、最初に保留中の残高に追加されます。token
accountの所有者は、*rs`ApplyPendingBalance`*命令を使用して、保留中の残高を利用可能な残高に変換する必要があります。受信した転送や入金は、token
accountの利用可能な残高には影響しません。
...
機密保留中の残高は`pending_balance_lo`と`pending_balance_hi`に分割されています。これは、ElGamal復号化が大きな数値に対してより多くの計算を必要とするためです。暗号文演算の実装は[こちら](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/confidential-transfer/ciphertext-arithmetic/src/lib.rs#L54)で確認でき、*rs`ApplyPendingBalance`*命令で[こちら](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L1180-L1184)のように使用されています。
...
保留中の残高を利用可能な残高に変換するために*rs`ApplyPendingBalance`*命令を呼び出す際:

1. クライアントは現在の保留中の残高と利用可能な残高を検索し、その合計を暗号化し、token
   accountの所有者のAES鍵を使用して暗号化された`decryptable_available_balance`を提供します。

2. 期待される保留中のクレジットカウンターと実際の保留中のクレジットカウンターは、*rs`ApplyPendingBalance`*命令が作成されてから処理されるまでの間のカウンター値の変更を追跡します。
   - `expected_pending_balance_credit_counter`: クライアントが*rs`ApplyPendingBalance`*命令を作成した時点での`pending_balance_credit_counter`の値
   - `actual_pending_balance_credit_counter`:
     *rs`ApplyPendingBalance`*命令が処理された時点でのtoken
     account上の`pending_balance_credit_counter`の値

期待値/実際のカウンターが一致している場合、`decryptable_available_balance`が`available_balance`と一致していることを示します。

トークンアカウントの状態を取得して`decryptable_available_balance`を読み取る際、期待値/実際のカウンター値が異なる場合、クライアントはカウンターの差分に一致する最近の入金/転送instructionsを検索して、正しい残高を計算する必要があります。

**残高照合プロセス**

期待値と実際の保留中残高カウンターが異なる場合、`decryptable_available_balance`を調整するために次の手順に従ってください:

1. トークンアカウントの`decryptable_available_balance`から開始します
2. カウンターの差分(実際 - 期待値)までの入金および転送instructionsを含む最新のトランザクションを取得します:
   - 入金instructionsから公開金額を追加します
   - 転送instructionsから宛先の暗号文金額を復号化して追加します
...
1. **トークンアカウントの作成**: Associated Token
   ...
   instructionを呼び出して、トークンアカウントを作成します。
