...
account間でトークンを転送できます。これはプライバシーを保護するトランザクションに役立ちます。非公開となるのは転送金額とトークン残高のみです。token
accountのアドレスは公開されたままです。

- [プロトコルの概要](https://www.solana-program.com/docs/confidential-balances/overview) - 基盤となる暗号化プロトコルの詳細
- [クイックスタートガイド](https://www.solana-program.com/docs/confidential-balances#setup) - セットアップと基本的なCLIコマンド
- [コンフィデンシャルバランスクックブック](https://github.com/solana-developers/Confidential-Balances-Sample) - コンフィデンシャル転送拡張機能の使用方法に関するコードスニペット

### どのように機能しますか？

コンフィデンシャル転送拡張機能は、Token Extensions
...
コンフィデンシャルトークン転送の基本的な流れは以下のとおりです：

1. コンフィデンシャル転送拡張機能付きのmint accountを作成する。
2. 送信者と受信者用にコンフィデンシャル転送拡張機能付きのtoken
   accountを作成する。
...
<Cards>
  ...
  >
    コンフィデンシャル転送トークンの発行と運用方法（承認ポリシー、監査者、手数料、ミントおよびバーン）
  </Card>
</Cards>

以下の図は、コンフィデンシャルトークン転送の基本フローの詳細なシーケンスを示しています：
...
## コンフィデンシャル転送のinstructions

コンフィデンシャル転送拡張機能の全instructions一覧は
...
| Instruction                         | 説明                                                                                                                                                                       |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | コンフィデンシャル転送のためのmint accountを設定します。このinstructionは _rs`TokenInstruction::InitializeMint`_ instructionと同じトランザクションに含める必要があります。 |
| _rs`UpdateMint`_                    | mintのコンフィデンシャル転送設定を更新します。                                                                                                                             |
| _rs`ConfigureAccount`_              | コンフィデンシャル転送のためのtoken accountを設定します。                                                                                                                  |
| _rs`ApproveAccount`_                | mintが新しいtoken accountの承認を要求する場合に、コンフィデンシャル転送用のtoken accountを承認します。                                                                     |
| _rs`EmptyAccount`_                  | token accountをクローズできるよう、保留中および利用可能なコンフィデンシャル残高を空にします。                                                                              |
| _rs`Deposit`_                       | パブリックトークン残高を保留中のコンフィデンシャル残高に変換します。                                                                                                       |
| _rs`Withdraw`_                      | 利用可能なコンフィデンシャル残高をパブリック残高に戻します。                                                                                                               |
| _rs`Transfer`_                      | token account間でトークンをコンフィデンシャルに転送します。                                                                                                                |
| _rs`ApplyPendingBalance`_           | 入金または転送後に保留中の残高を利用可能な残高に変換します。                                                                                                               |
| _rs`EnableConfidentialCredits`_     | token accountがコンフィデンシャルトークン転送を受け取れるようにします。                                                                                                    |
| _rs`DisableConfidentialCredits`_    | パブリック転送は引き続き許可しながら、受信コンフィデンシャル転送をブロックします。                                                                                         |
| _rs`EnableNonConfidentialCredits`_  | token accountがパブリックトークン転送を受け取れるようにします。                                                                                                            |
| _rs`DisableNonConfidentialCredits`_ | 通常の転送をブロックし、アカウントがコンフィデンシャル転送のみを受け取れるようにします。                                                                                   |
| _rs`TransferWithFee`_               | token account間でトークンを手数料付きでコンフィデンシャルに転送します。                                                                                                    |
| _rs`ConfigureAccountWithRegistry`_  | _rs`VerifyPubkeyValidity`_ proofの代わりに _rs`ElGamalRegistry`_ アカウントを使用して、コンフィデンシャル転送用のtoken accountを設定する代替方法です。                     |
