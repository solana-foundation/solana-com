...
<Callout type="warn">
  ZK
  ElGamalプログラムは、セキュリティ監査を受けているため、メインネットとdevnetで一時的に無効化されています。これは、confidential
  transfers拡張機能が現在利用できないことを意味します。概念は依然として有効ですが、コード例は実行されません。
</Callout>

## Confidential Transfersとは？

Confidential
transfersを使用すると、転送金額を明らかにすることなく、トークンアカウント間でトークンを転送できます。これはプライバシーを保護するトランザクションに役立ちます。転送金額とトークン残高のみがプライベートです。トークンアカウントのアドレスは公開されたままです。
...
Extensionプログラムに[instructions](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)を追加し、転送金額を明らかにすることなくアカウント間でトークンを転送できるようにします。
...
機密トークン転送の基本的なフローは以下の通りです：

1. confidential transfer拡張機能を持つmintアカウントを作成します。
2. 送信者と受信者のためにconfidential
   transfer拡張機能を持つトークンアカウントを作成します。
3. 送信者アカウントにトークンをmintします。
4. 送信者の公開残高を**機密保留残高**に**入金**します。
5. 送信者の保留残高を**機密利用可能残高**に**適用**します。
6. 送信者トークンアカウントから受信者トークンアカウントへ機密的にトークンを**転送**します。
7. 受信者の保留残高を**機密利用可能残高**に**適用**します。
8. 受信者の機密利用可能残高を**公開残高**に**出金**します。

機密転送フローの手順の詳細については、対応するページを参照してください：
...
以下のRustサンプルは、前述の図に対応する機密トークン転送の手順を示しています。手順は明確にするためにヘルパー関数に分けられています。

サンプルを実行するには、次のコマンドを使用して、mainnetからクローンしたToken
Extension
Programを使用してローカルvalidatorを起動します。ローカルvalidatorを使用するには、Solana
...
<Callout type="info">
  執筆時点では、機密転送はデフォルトのローカルvalidatorでは有効になっていません。サンプルコードを実行するには、mainnetのToken
...
機密転送拡張機能の[instructions](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)の完全なリストは次のとおりです：

| Instruction                         | 説明                                                                                                                                                     |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | mint accountを機密転送用に設定します。このinstructionは _rs`TokenInstruction::InitializeMint`_ instructionと同じトランザクションに含める必要があります。 |
| _rs`UpdateMint`_                    | mintの機密転送設定を更新します。                                                                                                                         |
| _rs`ConfigureAccount`_              | token accountを機密転送用に設定します。                                                                                                                  |
| _rs`ApproveAccount`_                | mintが新しいtoken accountの承認を必要とする場合、token accountを機密転送用に承認します。                                                                 |
...
| _rs`Transfer`_                      | token account間でトークンを機密転送します。                                                                                                              |
| _rs`ApplyPendingBalance`_           | 入金または転送後に保留残高を利用可能残高に変換します。                                                                                                   |
| _rs`EnableConfidentialCredits`_     | token accountが機密トークン転送を受信できるようにします。                                                                                                |
| _rs`DisableConfidentialCredits`_    | 公開転送を許可したまま、受信機密転送をブロックします。                                                                                                   |
| _rs`EnableNonConfidentialCredits`_  | token accountが公開トークン転送を受信できるようにします。                                                                                                |
| _rs`DisableNonConfidentialCredits`_ | accountが機密転送のみを受信するように、通常の転送をブロックします。                                                                                      |
| _rs`TransferWithFee`_               | token account間で手数料付きでトークンを機密転送します。                                                                                                  |
| _rs`ConfigureAccountWithRegistry`_  | _rs`VerifyPubkeyValidity`_ proofの代わりに _rs`ElGamalRegistry`_ accountを使用してtoken accountを機密転送用に設定する代替方法。                          |
