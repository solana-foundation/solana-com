---
title: 譲渡不可トークン
description: トークンの発行、焼却、クローズは可能だが、トークンアカウント間での転送ができないmintを作成します。
...
Token Extensions
ProgramのCODE-PLACEHOLDERミント拡張機能により、そのmintのすべてのトークンアカウントが譲渡不可になります。トークンが発行された後、トークン所有者はCODE-PLACEHOLDERやCODE-PLACEHOLDERを使用して他のトークンアカウントに移動することはできません。

このパターンは、特定のウォレットに紐付けておくべき資産に有用です。

譲渡不可トークンでも以下は可能です:

- mint権限による発行
- トークンアカウント所有者または承認された委任者による焼却
- 残高がゼロになった後のトークンアカウントのクローズ

<Callout type="info" title="トークンアカウント拡張機能">
  CODE-PLACEHOLDERを持つmint用にトークンアカウントが初期化されると、トークンアカウントはCODE-PLACEHOLDERとCODE-PLACEHOLDERで初期化されます。トークンアカウントが[Associated
  Token
  Program](/docs/tokens/basics/create-token-account#how-to-create-an-associated-token-account)を通じて作成される場合、必要なアカウントサイズが計算され、トークンアカウントは必要なサイズとrent免除lamportで作成されます。
</Callout>
...
1. mintとCODE-PLACEHOLDER拡張機能に必要なmintアカウントサイズとrentを計算します。
2. CODE-PLACEHOLDERでmintアカウントを作成し、CODE-PLACEHOLDERを初期化し、CODE-PLACEHOLDERでmintを初期化します。
3. mint用のトークンアカウントを作成します。CODE-PLACEHOLDERとCODE-PLACEHOLDERはトークンアカウントで自動的に有効になります。
4. CODE-PLACEHOLDERとCODE-PLACEHOLDERはCODE-PLACEHOLDERで失敗します。

<ScrollyCoding>

## !!steps アカウントサイズの計算

ベースmintにCODE-PLACEHOLDER拡張機能を加えたmintアカウントサイズを計算します。これがCODE-PLACEHOLDERで使用されるサイズです。
...
ミントに必要なサイズに _rs`NonTransferable`_
エクステンションを加えてrentを計算します。
...
## !!steps NonTransferableを初期化する

ミント上で _rs`NonTransferable`_ エクステンションを初期化します。
...
<Callout type="info" title="Instructionの順序">
  _rs`InitializeNonTransferableMint`_ は _rs`InitializeMint`_
  より前に実行する必要があります。
  _rs`CreateAccount`_、_rs`InitializeNonTransferableMint`_、および
...
| 項目                                           | 説明                                                                                                                                                                                                                                    | ソース                                                                                                                            |
| ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| _rs`NonTransferable`_                          | ミントからのトークンを譲渡不可としてマークするミントエクステンション。                                                                                                                                                                  | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/non_transferable.rs#L8-L13)  |
| _rs`NonTransferableAccount`_                   | 譲渡不可ミント用のtoken accountに追加されるtoken accountエクステンション。                                                                                                                                                              | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/non_transferable.rs#L15-L21) |
| _rs`ImmutableOwner`_                           | 所有権の変更を防ぐtoken accountエクステンションで、譲渡不可token accountに必須。                                                                                                                                                        | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L8-L16)   |
| _rs`InitializeNonTransferableMint`_            | _rs`InitializeMint`_ の前にミントレベルの譲渡不可エクステンションを初期化するinstruction。                                                                                                                                              | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L642-L653)              |
| _rs`process_initialize_non_transferable_mint`_ | 未初期化のミント上で _rs`NonTransferable`_ ミントエクステンションを初期化するプロセッサーロジック。                                                                                                                                     | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1646-L1655)                |
| _rs`get_required_init_account_extensions`_     | ミントで有効化されているエクステンションに基づいてtoken accountの初期化時にtoken accountエクステンションを自動的に追加します。_rs`NonTransferable`_ ミントの場合、_rs`NonTransferableAccount`_ と _rs`ImmutableOwner`_ が追加されます。 | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mod.rs#L1304-L1307)          |

### Typescript

以下の `Kit`
の例では、生成されたinstructionsを直接使用しています。参考のため、`@solana/web3.js`
を使用したレガシー例も含まれています。
