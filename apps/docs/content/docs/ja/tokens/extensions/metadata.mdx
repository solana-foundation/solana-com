...
description:
  MetadataPointerとTokenMetadata拡張機能を使用してmintアカウントに直接メタデータを保存するmintを作成し、token-metadataインターフェースを通じてそのメタデータを更新、削除、ロックします。
...
以下の`Kit`の例では、生成されたinstructionsを直接使用しています。`@solana/web3.js`、`@solana/spl-token`、および`@solana/spl-token-metadata`を使用したレガシーの例は参照用として含まれています。web3.js v1はv1トランザクションをデシリアライズできないため、レガシーの例では`maxSupportedTransactionVersion`を`0`に制限しています。[v1 transactions](/docs/core/transactions/versioned-transactions#v1-message-format)の部分はレガシーの例では対応していません。
