...
title: CPI ガード
description:
  CpiGuardアカウント拡張機能を有効にして、Cross Program
  Invocationを通じて呼び出されるトークンinstructionsをブロックします。
...
Token Extensions
Programの*rs`CpiGuard`*アカウント拡張機能は、別のオンチェーンプログラムがCross
Program Invocation(CPI)を通じて特定のトークンinstructionsにtoken
accountを使用することを防ぎます。

CPI
Guardが有効な場合、別のオンチェーンプログラムがCPIを通じてこれらのinstructionsを呼び出すと、Token
Extensions Programはそれらを拒否します:

- token
  accountの所有者がinstructionの権限として使用される場合の*rs`Transfer`*、_rs`TransferChecked`_、および*rs`TransferCheckedWithFee`*
- token
  accountの所有者がinstructionの権限として使用される場合の*rs`Burn`*および*rs`BurnChecked`*
- *rs`Approve`*および*rs`ApproveChecked`*
- lamportがtoken accountの所有者以外の場所に送信される場合の*rs`CloseAccount`*
- クローズ権限を追加または置き換える場合の*rs`SetAuthority`*

CPI Guardが有効な場合でも、以下のケースは許可されます:

- token
  accountの所有者ではなくデリゲートまたはパーマネントデリゲートが署名する場合の、CPIを通じた*rs`Transfer`*、_rs`TransferChecked`_、_rs`TransferCheckedWithFee`_、_rs`Burn`_、および*rs`BurnChecked`*
- lamportがtoken
  accountの所有者に送信される場合の、CPIを通じた*rs`CloseAccount`*
- 既存のクローズ権限を削除するための*rs`SetAuthority`*
- _rs`Revoke`_

token
accountの所有者がinstructionの権限として使用される場合でも、instructionがCPIを通じて呼び出されるのではなく、トランザクションに直接追加される場合、Token
Extensions ProgramのinstructionsはOK処理されます。*rs`SetAuthority`*によるtoken
accountの所有者の変更は、CPI
Guard有効時にはCPI外でもブロックされたままです。*rs`EnableCpiGuard`*および*rs`DisableCpiGuard`*もCPIを通じて呼び出すことはできません。
...
1. ミントを作成して初期化します。
2. _rs`CpiGuard`_
   を持つトークンアカウントに必要なトークンアカウントのサイズと rent を計算します。
3. ミント用のトークンアカウントを作成して初期化します。
4. トークンアカウントに対して _rs`EnableCpiGuard`_ を呼び出します。
5. トークンアカウントに対して _rs`DisableCpiGuard`_
   を呼び出して、CPI経由で再度それらの instructions を許可します。
...
## !!steps トークンアカウントのサイズを計算

基本トークンアカウントと _rs`CpiGuard`_
拡張機能を合わせたトークンアカウントのサイズを計算します。このサイズは
_rs`CreateAccount`_ で使用されます。
...
_rs`CpiGuard`_
拡張機能に必要なトークンアカウントのサイズを使用して rent を計算します。
...
トークンアカウントで _rs`CpiGuard`_ を有効にします。
...
トークンアカウントで _rs`CpiGuard`_ を無効にします。
...
| 項目                               | 説明                                                                                                                                                                                             | ソース                                                                                                                                 |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`CpiGuard`_                     | トークンアカウントでCPIガードが現在有効になっているかどうかを保存するアカウント拡張機能。                                                                                                        | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/mod.rs#L20-L25)         |
| _rs`CpiGuardInstruction::Enable`_  | トークンアカウントでCPIガードを有効にするinstruction。                                                                                                                                           | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/instruction.rs#L20-L38) |
| _rs`CpiGuardInstruction::Disable`_ | トークンアカウントでCPIガードを無効にするinstruction。                                                                                                                                           | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/instruction.rs#L39-L53) |
| _rs`process_toggle_cpi_guard`_     | *rs`Enable`*と*rs`Disable`*で使用されるプロセッサーロジック。アカウント所有権を検証し、ガード自体へのCPI変更を拒否し、`lock_cpi`を設定する前に*rs`CpiGuard`*がまだ存在しない場合は初期化します。 | [ソース](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/cpi_guard/processor.rs#L19-L52)     |

### Typescript

以下の`Kit`の例では、生成されたinstructionsを直接使用しています。`@solana/web3.js`と`@solana/spl-token`を使用した従来の例も参考として含まれています。
