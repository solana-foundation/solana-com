...
は、ミントに対して作成されるすべての新しいトークンアカウントのデフォルト状態を設定します。この拡張機能を有効にすると、すべての新しいトークンアカウントが凍結などの指定された状態で自動的に初期化されます。これにより、トークン作成者は、明示的に凍結解除されるまですべての新しいアカウントをデフォルトで凍結するなど、特定の動作を強制できます。
