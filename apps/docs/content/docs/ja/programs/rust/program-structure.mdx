...
Rustで記述されたSolanaプログラムは最小限の構造要件しかなく、コードの整理方法に柔軟性があります。唯一の要件は、プログラムが`entrypoint`を持つ必要があることで、これがプログラムの実行開始位置を定義します。
...
簡潔にするため、プログラムは単一の`lib.rs`ファイルに実装されますが、実際には大規模なプログラムを複数のファイルに分割することをお勧めします。
...
次に、Solanaプログラムをビルドするために必要な依存関係を追加しましょう。コアSDKには`solana-program`が、シリアライゼーションには`borsh`が必要です。
...
次に、カウンターアカウントに保存されるデータ構造を定義しましょう。これはアカウントの`data`フィールドに保存されるデータです。
...
次に、`instruction_data`(生のバイト列)を`CounterInstruction`
enumのバリアントの1つにデシリアライズする必要があります。Borshの`try_from_slice`メソッドがこの変換を自動的に処理します。

Borshデシリアライゼーションを使用するように`process_instruction`関数を更新してください：
...
次に、instructionsを適切なハンドラー関数にルーティングするようにメインの`process_instruction`関数を更新しましょう。

このルーティングパターンはSolanaプログラムで一般的です。`instruction_data`はinstructionを表すenumのバリアントにデシリアライズされ、その後適切なハンドラー関数が呼び出されます。各ハンドラー関数には、そのinstructionの実装が含まれています。

以下のコードを`lib.rs`に追加して、`process_instruction`関数を更新し、`InitializeCounter`と`IncrementCounter`
...
このプログラムはCPIを使用してSystem Programの`create_account`
instructionを呼び出します。新しいアカウントは、このプログラムをownerとして作成されるため、プログラムはアカウントへの書き込みとデータの初期化が可能になります。
...
- `counter_account`のアカウント`data`フィールドを読み取ります
- それを`CounterAccount`構造体にデシリアライズします
- `count`フィールドを1増やします
- `CounterAccount`構造体をアカウントの`data`フィールドにシリアライズして戻します
...
次に、プログラムをビルドしてテスト環境にロードする必要があります。`cargo build-sbf`コマンドを実行してプログラムをビルドします。これにより、`target/deploy`ディレクトリに`counter_program.so`ファイルが生成されます。
...
テスト環境にプログラムをロードするために、`test_counter_program`関数を更新してください。
...
<Callout type="warn">
  テストを実行する前に`cargo
  build-sbf`を実行して、`.so`ファイルを生成する必要があります。テストはコンパイル済みのプログラムをロードします。
</Callout>
...
次のコマンドでテストを実行してください。`--nocapture`フラグはテストの出力を表示します。
...
`examples/client.rs`にクライアントコードを追加し、`program_id`を前のコマンドの出力に置き換えます。
...
ローカルvalidatorが実行されている状態で、出力されたトランザクション署名を使用して[Solana Explorer](https://explorer.solana.com/?cluster=custom)でトランザクションを表示できます。Solana
