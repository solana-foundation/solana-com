...
まず、`--lib` フラグを使用して、標準的な `cargo new`
コマンドで新しいRustプロジェクトを作成します。

```terminal
$ cargo new hello_world --lib
```

プロジェクトディレクトリに移動します。デフォルトの `src/lib.rs` ファイルと
`Cargo.toml` ファイルが表示されます。
...
<Callout type="warn">
  ...
  に更新してください。そうしないと、プログラムのビルド時にエラーが発生する可能性があります。
</Callout>
...
次に、`solana-program`
依存関係を追加します。これはSolanaプログラムをビルドするために必要な最小限の依存関係です。
...
<Callout type="warn">
  この設定を含めない場合、プログラムをビルドしても `target/deploy`
  ディレクトリが生成されません。
</Callout>
...
次に、`src/lib.rs`
の内容を以下のコードに置き換えます。これは、プログラムが呼び出されたときにプログラムログに「Hello,
world!」を出力する最小限のSolanaプログラムです。

`msg!`
マクロは、Solanaプログラムでプログラムログにメッセージを出力するために使用されます。
...
このコマンドは、2つの重要なファイルを含む `target/deploy`
ディレクトリを生成します。
...
次に、`litesvm`クレートを使用してプログラムをテストします。以下の依存関係を`Cargo.toml`に追加します。
...
以下のテストを`src/lib.rs`のプログラムコードの下に追加します。これはhello
...
<Callout type="info">
  Solana
  Explorerのクラスターもlocalhostである必要があることに注意してください。Solana
  Explorerの「Custom RPC
  URL」オプションはデフォルトで`http://localhost:8899`に設定されています。
</Callout>
