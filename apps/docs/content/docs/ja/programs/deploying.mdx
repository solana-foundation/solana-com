...
| 新しいプログラムのデプロイ                  | `solana program deploy <path_to_program.so>`                                                  |
| 既存のプログラムの更新                      | `solana program deploy <path_to_program.so>` (デプロイと同じ)                                 |
| プログラム情報の表示                        | `solana program show <program-id>`                                                            |
| プログラム権限の移転                        | `solana program set-upgrade-authority <program-id> --new-upgrade-authority <path_to_keypair>` |
...
- **プログラムID**: プログラムのオンチェーンアドレスです。ユーザーはプログラムIDを参照してプログラムと対話します。
- **program
  account**: プログラムのメタデータを保存するオンチェーンアカウントです。program
  accountのアドレスがプログラムIDとなります。program accountには、programdata
  accountのアドレスも保存されます。
- **programdata
  account**: プログラムのデプロイされた実行可能コードを保存するオンチェーンアカウントです。
- **プログラム権限**: プログラムを更新または閉鎖する権限を持つアカウントです。デフォルトでは、これはCLIウォレットとなります。
...
これにより、デフォルトで `~/.config/solana/id.json` にkeypairが作成されます。
...
プログラムのメタデータを確認するには、`solana program show`
コマンドを使用します:
...
プログラムIDは、program
accountのオンチェーンアドレスです。ユーザーや他のプログラムは、トランザクション内でこのIDを参照してプログラムを呼び出します。
...
プログラムの権限を別のアカウントに移転するには、
`solana program set-upgrade-authority` コマンドを使用します:
...
プログラムを不変にするには、`solana program set-upgrade-authority` コマンドに
`--final` フラグを付けて、プログラムの権限を削除します:
...
プログラムを閉じて、program accountに割り当てられたSOLを回収するには、
`solana program close` コマンドを使用します:
...
- **`--with-compute-unit-price`**: コンピュートユニットあたりのマイクロランポート(0.000001
  SOL)単位で優先手数料を設定します。現在のレートについては
  [Helius Priority Fee API](https://docs.helius.dev/guides/priority-fee-api)を確認してください。優先手数料は、トランザクションを優先的に処理してもらうために現在のリーダーに支払う追加手数料です。
- **`--max-sign-attempts`**: トランザクションが期限切れになった場合に、新しいブロックハッシュで再試行する回数。(デフォルト: 5)
- **`--use-rpc`**: 設定されたRPCにトランザクションを送信します。このフラグには、[Triton](https://triton.one/)や
  [Helius](https://www.helius.dev/)などのプロバイダーからの
  [ステーク加重](/developers/guides/advanced/stake-weighted-qos)RPC接続が必要です。
