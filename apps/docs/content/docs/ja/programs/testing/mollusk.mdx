...
`mollusk-svm`を`Cargo.toml`の依存関係として追加します:
...
コンピュートユニットの使用量をベンチマークするには、`mollusk-svm-bencher`を`Cargo.toml`の依存関係として追加します:
...
Token Program、Token2022プログラム（Token Extensions）、およびAssociated Token
ProgramをMolluskでテストするために使用するには、`mollusk-svm-programs-token`を`Cargo.toml`の依存関係として追加します:
...
`cargo build-sbf`を実行すると、コンパイルされたプログラムが`/target/deploy/<program_name>.so`に生成されます。
...
1. [`Mollusk`インスタンスを作成](mention:mollusk) - プログラムIDとコンパイルされたプログラムへのパス（`.so`ファイル）でMolluskを初期化します
2. [インストラクションを構築](mention:instruction) - プログラムを呼び出すためのインストラクションを作成します
3. [処理と検証](mention:process) -
   Molluskを使用してインストラクションを処理し、結果を検証します
...
以下の例では、デモンストレーションのために`main`関数内でMolluskを実行しています。実際には、通常`#[test]`属性で注釈されたテストモジュール内でMolluskを使用します。
...
`process_and_validate_instruction`メソッドを使用して、検証チェック付きで単一のinstructionを処理します。このメソッドは、チェックが失敗した場合にパニックします。
...
`process_and_validate_instruction_chain`メソッドを使用して、各instruction後に検証チェック付きで複数のinstructionsを処理します。各instructionには、合格する必要がある独自のチェックセットがあります。
...
`Mollusk`とは異なり、各メソッド（例：`process_instruction`）に`accounts`を渡す必要がある一方で、`MolluskContext`は`account_store`を通じてアカウントを内部的に管理します。これにより、instructionsを処理する際に`accounts`パラメータが不要になります。
...
次の例では、`account_store`を通じてinstructions間でアカウント状態を永続化しながら、2つの個別のSOL転送instructionsを処理しています。
...
`warp_to_slot`メソッドを使用してsysvarクロックを更新し、特定のslotへの時間の前進または後退をシミュレートします。
...
次の例は、`sysvars`フィールドにアクセスしてrentパラメータを変更することで、Mollusk
sysvarを直接変更する方法を示しています。他のsysvar値も同じ方法で変更できます。
...
依存関係として[`mollusk-svm-bencher`](https://docs.rs/mollusk-svm-bencher/0.5.0/mollusk_svm_bencher/index.html)が必要です。
...
[`mollusk-svm-programs-token`](https://docs.rs/mollusk-svm-programs-token/0.5.0/mollusk_svm_programs_token/index.html)クレートを使用して、テスト用にToken
Program、Token2022プログラム（Token Extensions）、Associated Token
ProgramをMolluskに追加します。
