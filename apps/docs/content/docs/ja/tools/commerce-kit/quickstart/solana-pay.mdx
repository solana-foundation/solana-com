...
`@solana-commerce/solana-pay` パッケージは、[Solana Pay](https://solanapay.com) の完全な機能を提供し、[`@solana/kit`](https://github.com/anza-xyz/kit) ライブラリと互換性のある決済体験の構築に対応しています。URL のエンコード／パース、カスタムスタイルによる QR コード生成、SOL および SPL トークン転送の両方に対応したトランザクション構築を処理します。
...
- **`recipient`** (`Address`、必須) - 支払いを受け取るマーチャントのウォレットアドレス（base58 エンコードされた公開鍵）。`@solana/kit` の文字列または `Address` 型を使用できます。
...
支払い転送のための完全な Solana トランザクションメッセージを構築します。この関数は `splToken` パラメータに基づいて SOL または SPL トークン転送のいずれを作成するかを自動的に検出し、必要なすべての instructions を構築します。RPC クライアントから取得した最新のブロックハッシュを使用してトランザクションのブロックハッシュライフタイムを設定し、署名および RPC への送信に対応した完全な未署名トランザクション（型: `TransactionMessageWithBlockhashLifetime`、Solana Kit 互換）を返します。

#### パラメータ

- **`rpc`** (`Rpc<SolanaRpcApi>`) - `@solana/kit` の Solana RPC クライアント。`createSolanaRpc(rpcUrl)` で作成します。
