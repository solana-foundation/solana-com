...
これらの関数はSolanaと連携してトランザクションを検証し、
承認を待機します。`@solana/kit`ライブラリのSolana RPCクライアントが必要です。
...
- **`rpc`** (`Rpc<SolanaRpcApi>`、必須) - `@solana/kit`のRPCクライアント。
  `createSolanaRpc(rpcUrl)`で作成します。
