...
| `aws_kms`    | AWS KMS（Ed25519使用）            |
| `gcp_kms`    | GCP Cloud KMS（Ed25519使用）      |
| `privy`      | Privyの組み込みウォレット          |
| `turnkey`    | Turnkey API                     |
| `fireblocks` | Fireblocks MPC                  |
| `cdp`        | Coinbase Developer Platform     |
| `crossmint`  | Crossmintの管理ウォレット       |
| `dfns`       | Dfnsウォレットインフラストラクチャ      |
| `openfort`   | Openfortの組み込みウォレット       |
| `para`       | Para MPCウォレット                |
...
Fordefi MPCカストディの署名に使用します。APIリクエストは、`private_key_pem`から解析されたECDSA P-256リクエスト署名で認証されます。そのキーをKMS/HSMで管理する場合は、`FordefiRequestSigner`を実装し、`Signer::from_fordefi_with_signer`を使用してください。

Fordefiには2つの署名モードがあります。**ネイティブSolanaモード**では`chain`を設定します。このモードでは、Fordefiがブロックハッシュや手数料を置き換え、トランザクションを自らブロードキャストする場合があります。**ブラックボックスモード**では`chain`を`None`のままにします。このモードでは、FordefiがRAWバイトに署名し、トランザクションのブロードキャストはご自身で行います。
