...
Fordefiには2つの署名モードがあり、`chain`を渡すかどうかによって選択されます。APIリクエストはECDSA P-256リクエスト署名で認証されます。署名キーは`privateKeyPem`として指定するか、`requestSigner`を渡すことでKMS/HSMに保持することができます。

**ネイティブSolanaモード**（オンチェーン利用に推奨）：`chain`を設定すると、Fordefiがトランザクションの署名とブロードキャストを行います。Fordefiは署名前にブロックハッシュや手数料を置き換える場合があるため、このサイナーはKitの`TransactionSendingSigner`です。上記の部分署名フローではなく、`signAndSendTransactionMessageWithSigners`と組み合わせて使用してください。
...
**ブラックボックスモード**：`chain`を省略します。Fordefiは生のバイト列に署名しますが、ブロードキャストは行いません。トランザクションのアセンブルと送信はご自身で行ってください。このモードはFordefiのブラックボックスボールトと組み合わせて使用します。
