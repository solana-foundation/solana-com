...
## SolanaSignerインターフェース

`SolanaSigner`インターフェースは、完全な互換性のために`@solana/signers`型を拡張します：
...
`SolanaSigner`インターフェースを実装して、カスタムサイナーを作成します：
...
追加の鍵管理サービスを統合するには、[サイナーの追加ガイド](/docs/tools/keychain/adding-signers)を参照してください。
