---
title: トークン操作
description: トークン操作（ミント、転送、アカウント管理）の使い方
---

## トークンミントの作成

ミントはすべてのSPLトークンの基盤です。トークンのプロパティを定義し、新しいトークンをミントできるユーザーを制御します。
...
token
accountは、特定のユーザーの特定のspl-tokenの残高を保持します。これとATAの違いを理解するには、
[このセクション](/docs/tools/litesvm/additional-crates/testing-with-spl-tokens#types-of-token-accounts)をご覧ください。
...
Associated Token
Accounts（ATAs）は、特定のユーザーのトークンを保持するための決定論的なアドレスです。これとATAの違いを理解するには、
[このセクション](/docs/tools/litesvm/additional-crates/testing-with-spl-tokens#types-of-token-accounts)をご覧ください。
...
## token accountへのトークンのミント

ミントとtoken
accountが用意できたら、それらのアカウントにトークンをミントできます。
...
Transferのinstructionsを使用して、アカウント間でトークンを転送します。
...
ほとんどのトークンは小数点を使用して端数の量を表します。正しく処理する方法は次のとおりです：
