---
title: プログラムとコンテキスト
description: AnchorContextとProgramを使用したテスト環境のセットアップとinstructionsの構築
---

## declare_program!によるクライアント型の生成

Anchor 1.0では、プログラムのIDLをコンパイル時に読み込み、型安全な
`client::accounts::*` および `client::args::*` モジュールを生成する
`declare_program!`
が導入されました。これはテストでinstructionsを構築する際に推奨される方法です。
...
<Callout type="info">
  まず `anchor build` を実行して `target/idl/my_program.json`
  を生成してください。 `declare_program!`
  はコンパイル時にこのファイルを読み込みます。マクロ名はプログラムのクレート名（snake_case）と一致する必要があります。
</Callout>

## テスト環境のセットアップ

`AnchorLiteSVM` ビルダーは、テスト環境を設定するための流暢なAPIを提供します。
...
<Callout type="info">
  ペイヤーを指定しない場合、`AnchorLiteSVM`
  は自動的にペイヤーを作成して資金を供給します。
</Callout>

## AnchorContext

`AnchorContext` は、テスト環境と対話するためのメインインターフェースです。
...
## Programによるinstructionsの構築

`Program`
構造体は、anchor-clientを模倣したinstructionsを構築するための流暢なAPIを提供します。型の安全性のために、`declare_program!`
によって生成された型を使用してください。
...
<Callout type="info">
  `.accounts()` メソッドは、`ToAccountMetas`
  を実装する任意の型を受け入れます。これはすべてのAnchorアカウント構造体が自動的に実装するものです。同様に、`.args()`
  は`InstructionData` を実装する任意の型を受け入れます。
</Callout>
...
### send_and_confirm_transactionの使用
...
以下は、`declare_program!` を使用したエスクローパターンの包括的な例です：
