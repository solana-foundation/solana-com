---
title: アカウントのセットアップ
description: LiteSVMを使用してテストアカウントと状態をセットアップする
---

# アカウントセットアップの例

トランザクションを使わずに、テストアカウントの状態を直接セットアップする方法を示します。
...
program-owned data accountをセットアップします：
...
<Callout type="info">
  `setAccount`を使用すると、大量のアカウントのセットアップにおいてトランザクションを実行するよりも高速です。
</Callout>

## 重要なポイント

1. **直接的な状態設定**:
   `setAccount`はトランザクションなしでアカウントの状態を設定します
2. **レント免除**:
   `minimumBalanceForRentExemption`を使用して必要なlamportsを計算します
3. **アカウント構造**: 必要なフィールドをすべて指定してください：address、data、executable、lamports、programAddress、space
4. **検証**: `getAccount`または`client.rpc.getAccountInfo`を使用して確認します
