...
LiteSVMは設定にビルダーパターンを使用します。すべてのメソッドはチェーン可能なように`LiteSVM`を返します。
...
<Callout type="info">
  署名検証を無効にするとテストは高速になりますが、現実に即さなくなります。
</Callout>
...
デフォルトプログラム（System Program、BPFローダーなど）を追加します。
...
特定のSolanaバージョンのテスト用にフィーチャーセットを設定します。
