...
アカウント、残高、およびアカウント状態を設定するためのメソッド。
...
| ---------------- | ------------ | ---------------------------- |
| `address`        | `Address`    | アカウントの公開鍵（pubkey） |
...
<Callout type="info">
  `setAccount`
  は、トランザクションを実行しないため、バルクアカウントのセットアップにおいて
  `airdrop` よりも高速です。
</Callout>
