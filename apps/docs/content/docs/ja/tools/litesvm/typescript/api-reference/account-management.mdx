---
title: アカウント管理
description: TypeScriptでアカウント、残高、rentを管理するためのメソッド
---

アカウントの管理、残高の確認、およびアカウント状態の設定を行うメソッド。
...
### EncodedAccountのプロパティ

| プロパティ         | 型         | 説明                   |
| ---------------- | ------------ | ----------------------------- |
| `address`        | `Address`    | アカウントのpubkey            |
| `data`           | `Uint8Array` | アカウントデータのバイト列            |
| `executable`     | `boolean`    | アカウントが実行可能かどうか |
| `lamports`       | `Lamports`   | SOL残高                   |
| `programAddress` | `Address`    | オーナープログラム                 |
| `space`          | `bigint`     | バイト単位のデータサイズ            |
...
アカウントのSOL残高を取得します。アカウントが存在しない場合は`null`を返します。
...
<Callout type="info">
  バルクアカウントセットアップでは、`setAccount`はトランザクションを実行しないため、`airdrop`より高速です。
</Callout>
