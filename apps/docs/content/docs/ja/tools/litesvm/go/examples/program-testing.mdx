---
title: プログラムのテスト
description: litesvm-goを使用したカスタムSolanaプログラムの読み込みとテスト
---

GoからカスタムSolanaプログラムをテストするには、コンパイル済みの`.so`ファイルをSVMに読み込み、必要なアカウントをセットアップして、instructions を送信します。`litesvm-go`はこれらすべてをインプロセスで処理するため、validatorは不要です。
...
標準プログラム（SPL Token、Memoなど）が必要なテストの場合：
...
1. **設定** - 必要に応じてsysvar、ビルトイン、プリコンパイルを有効にする。
2. **プログラムの読み込み** - `AddProgramFromFile`（またはインメモリバイトの場合は`AddProgram`）。
3. **アカウントのセットアップ** - `SetAccount`を使用してデータアカウントを事前に設定する。
4. **送信** - `solana-go`でinstructionsを構築し、マーシャリングして`SendLegacyTransaction`で送信する。
5. **検証** - `GetAccount`でアカウントを読み返し、ログ、コンピュートユニット、および実行後の状態をアサートする。

この基本構造が動作するようになったら、instructionsやアカウントを追加し、ボイラープレート用のヘルパーを作成することでスケールアップできます。
