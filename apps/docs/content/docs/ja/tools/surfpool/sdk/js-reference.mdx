...
title: JS APIリファレンス
description:
  "@solana/surfpoolパッケージの完全なパブリックAPI — Surfnetクラス、設定タイプ、デプロイオプション、およびイベントの形式。"
---
...
<Callout type="info" title="キットのエントリーポイント">
  以下のシンボルは `@solana/surfpool` のルートエクスポートです。`@solana/surfpool/kit` サブパスには `surfpool()`、`surfnetCheatcodes()`、`createSurfnetCheatcodesRpc()`、`DEFAULT_SURFNET_ENDPOINT`、プラグイン設定タイプ、および生成されたチートコードワイヤータイプが追加されており、これらはすべてこの表ではなく [キットプラグイン](/docs/tools/surfpool/sdk/kit-plugin) に記載されています。
</Callout>
