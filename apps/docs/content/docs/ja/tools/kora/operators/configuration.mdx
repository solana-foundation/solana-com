...
> _注意: `fallback_if_unavailable` が true の場合、Redisが一時的に利用できない状態でもトランザクションの処理を継続し、サービスの中断を防ぎます。
> `max_transactions` を 0 に設定すると、トランザクション数が無制限になります。_
...
| `get_payer_signer`          | 次の手数料支払者と支払いアドレスを返す        | ✅       | boolean |
...
| 拡張機能名               | 説明                                      |
| ---------------------------- | ------------------------------------------------ |
| `confidential_transfer_mint` | ミントのConfidential Balances設定 |
...
| 拡張機能名                  | 説明                                 |
| ------------------------------- | ------------------------------------------- |
| `confidential_transfer_account` | アカウントのConfidential Balancesの状態 |
