...
メインREADMEのsignerリストにあなたのサービスを追加してください。

## 提出チェックリスト

- [ ] あなたのsignerが`solana-keychain`クレートでサポートされている
- [ ] Cargo.tomlの`solana-keychain`依存関係を最新バージョンに更新した
- [ ] signerの設定構造体を追加した
- [ ] `SignerTypeConfig`バリアントを追加した
- [ ] `build_signer_from_config`にビルドロジックを追加した
- [ ] `validate_individual_signer_config`にvalidationロジックを追加した
- [ ] `mod.rs`に設定構造体をエクスポートした
- [ ] （任意）`config_mock.rs`にテストモックビルダーメソッドを追加した
- [ ] コードが警告なしにコンパイルされる
- [ ] すべてのテストが通過する（`make test`および`make test-integration`）
- [ ] ドキュメントが以下のガイドラインに従って追加されている
      [`ADDING_SIGNERS.md`](https://github.com/solana-foundation/solana-keychain/blob/main/docs/ADDING_SIGNERS.md)
- [ ] サンプル設定ファイルが作成されている（`.toml`および`.env.example`）
- [ ] ハードコードされた値やシークレットがない
- [ ] エラーメッセージがわかりやすい
- [ ] Rustの命名規則（snake_case）に従っている
- [ ] リンティングが通過する（`make lint`）
- [ ] 統合テスト用のAPIキーをKoraチームに連絡する

## ヘルプの取得

- **signerの実装について**:
  [`solana-keychain`](https://github.com/solana-foundation/solana-keychain)リポジトリにissueを作成してください
- **Kora統合について**: 設計の議論のためKoraリポジトリにissueを作成してください
- コミュニティチャンネルに参加してください
- 既存のsigner設定を[`crates/lib/src/signer/config.rs`](https://github.com/solana-foundation/kora/blob/v2.0.5/crates/lib/src/signer/config.rs)で確認してください
...
Koraエコシステムへようこそ！あなたの鍵管理ソリューションがプラットフォームの一部となることを楽しみにしています。
