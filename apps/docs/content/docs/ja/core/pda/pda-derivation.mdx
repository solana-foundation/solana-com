...
の[オンチェーンsyscall](https://github.com/anza-xyz/agave/blob/v3.1.8/syscalls/src/lib.rs#L798-L834)は、呼び出しごとに[1,500 CU](https://github.com/anza-xyz/agave/blob/v3.1.8/program-runtime/src/execution_budget.rs#L200)を消費します。
...
PDAの導出はアドレスのみを計算します。そのアドレスにオンチェーンアカウントを作成するわけではありません。アカウントは別の命令(通常はCPI経由の
_rs`create_account`_)を通じて明示的に作成する必要があります。
