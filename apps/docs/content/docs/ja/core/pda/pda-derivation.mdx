...
bump seedは、派生時にオプションのシードに追加される1バイト（u8）です。
[`find_program_address`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L800)
は255から1まで降順で検索し、各値で`create_program_address`を呼び出して、結果がEd25519曲線から外れるまで処理します。最初に成功した値が**正規のbump**となります。
...
1. シードの数が _rs`MAX_SEEDS`_ (16)を超えず、個々のシードが _rs`MAX_SEED_LEN`_
   (32バイト)を超えないことを検証します。いずれかのチェックが失敗した場合、
   _rs`PubkeyError::MaxSeedLengthExceeded`_ を返します。
...
*rs`create_program_address`*の[オンチェーンシステムコール](https://github.com/anza-xyz/agave/blob/v3.1.8/syscalls/src/lib.rs#L798-L834)は、呼び出しごとに[1,500 CU](https://github.com/anza-xyz/agave/blob/v3.1.8/program-runtime/src/execution_budget.rs#L200)を消費します。
...
PDAの導出は、アドレスの計算のみを行います。そのアドレスにオンチェーンアカウントを作成するわけではありません。アカウントは、別のinstructionを通じて明示的に作成する必要があります（通常はCPI経由で*rs`create_account`*を使用）。
...
<Callout type="warn">
  これは、`sol_try_find_program_address`のために文書化されている現在の正規検索動作とは異なり、255から1まで降順で反復処理します。
</Callout>
...
この例では、bump
255は曲線上のアドレスを生成して失敗します。最初の有効なbumpは254であり、これが正規のbumpとなります。
