...
description:
  "@solana/transaction-introspectionを使用して、確認済みのSolanaトランザクションをデコードおよび検査します。アカウントインデックスとルックアップテーブルアドレスを解決し、外部および内部（CPI）instructionsをたどり、プログラムクライアントで解析します。"
...
<Callout type="info" title="v1トランザクションへの準備">
  [v1
  フォーマット](/docs/core/transactions/versioned-transactions#v1-message-format)
が有効になったら、`maxSupportedTransactionVersion: 1` を渡してください。指定しない場合、v1トランザクションのフェッチが失敗します。`decodeTransactionFromRpcResponse` は `legacy`、`0`、`1` のいずれも処理できます（`@solana/kit` 7.0以降）。v1メッセージはリソース制限をComputeBudgetのinstructionsではなく `transactionConfig` オブジェクトに格納するため、デコードされたinstructionsをスキャンしてコンピュート制限や優先手数料を取得するコードは、v1では何も見つけられないことに注意してください。
</Callout>
