...
- `rent_exempt_minimum + total_fee`をカバーするのに十分なlamportsを保持している必要があります。そうでない場合、トランザクションは
...
ブロックハッシュは150スロット後に期限切れになります。トランザクションが到着したときにブロックハッシュが有効でない場合、[durable nonceトランザクション](/docs/core/transactions/durable-nonces)でない限り、
_rs`BlockhashNotFound`_ で拒否されます。
...
[`instructions`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/message/src/legacy.rs#L176)フィールドは、[`CompiledInstruction`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/message/src/compiled_instruction.rs#L21)構造体のコンパクトエンコードされた配列です。各
_rs`CompiledInstruction`_
は、完全な公開鍵ではなく、`account_keys`配列へのインデックスによってアカウントを参照します。これには以下が含まれます。
...
System
Program の転送は、**あらゆる**アカウントにlamportを追加します。受取人がSOLを移動できるかどうかのプロトコルレベルのチェックは存在しません。lamportはアカウントの所有プログラムによってのみ移動できるため、[token mint](/docs/tokens/basics/create-mint)、プログラム、または管理していない[PDA](/docs/core/pda)にSOLを送信すると、**資金が永久に失われるリスク**があります。所有プログラムが指定した権限者のみが資金を返還できます。[token account](/docs/tokens/basics/create-token-account)に送信されたSOLは、そのアカウントの所有者のみが回収できます。送信者が回収することはできません。

SPL **token**の転送には自己保護機能が一部備わっています。Token
Programは、アカウントが期待されるmintと一致しない転送を拒否します。ネイティブの**SOL**転送にはそのような保護機能がないため、送信者は署名する前に受取人を確認する必要があります。完全な分類ロジックについては、[アドレスの確認](/docs/payments/send-payments/verify-address)を参照してください。
...
各instructionsには同じ必須情報が含まれていることに注意してください。
...
SOLの転送はあらゆるアカウントに対して成功するため、署名する前に受取人を確認してください。アカウントを取得し、System
Program ウォレット（または未資金の[オンカーブ](/docs/payments/send-payments/verify-address#on-curve-addresses)アドレス）にのみ送信し、管理していないmint、token
account、プログラム、およびPDAへの送信は拒否してください。
...
<Callout>
  このスニペットはネイティブSOLの受取人を確認します。SPLトークンの送信（token
  account、ATA、Token-2022）にも対応した完全な分類については、[アドレスの確認](/docs/payments/send-payments/verify-address)をご参照ください。
</Callout>
