...
title: トランザクション構造
description:
  Solanaトランザクションの構造：署名、メッセージ形式、ヘッダー、アカウントアドレス、ブロックハッシュ、コンパイル済みインストラクション、バイナリエンコーディング、サイズ制限。
...
<Callout type="info" title="概要">
  トランザクションには署名とメッセージが含まれます。メッセージにはヘッダー、アカウントアドレス、最近のブロックハッシュ、およびコンパイル済みのinstructionsが含まれます。シリアル化後の最大サイズ：1,232バイト（今後のv1フォーマットでは4,096バイト）。
</Callout>
...
<Callout type="info" title="v1トランザクションでは上限が4,096バイトに引き上げられます">
  このページではレガシートランザクションフォーマットについて説明します。v0フォーマットも、バージョンプレフィックスとアドレスルックアップテーブルを除いて同じ仕様に従っています。今後リリース予定の
  [v1フォーマット](/docs/core/transactions/versioned-transactions#v1-message-format)
  ではサイズ制限が4,096バイトに引き上げられ、異なるワイヤーレイアウトが使用されます。
</Callout>
...
<Callout type="info" title="レガシーおよびバージョン付きメッセージプレフィックス">
  レガシートランザクションメッセージでは、メッセージの最初のバイトは`num_required_signatures`であり、その後に残り2つの`MessageHeader`バイトが続きます。バージョン付きトランザクションメッセージでは、最初のバイトの代わりにバージョンプレフィックスが使用されます。3バイトの`MessageHeader`はそのプレフィックスの直後から始まります。v0およびv1メッセージの完全なレイアウトについては、[バージョン付きトランザクション](/docs/core/transactions/versioned-transactions)を参照してください。
</Callout>
...
ブロックハッシュは150
slot後に期限切れになります。トランザクションが到着した時点でブロックハッシュが有効でなくなっている場合、[耐久性ノンストランザクション](/docs/core/transactions/durable-nonces)でない限り、_rs`BlockhashNotFound`_
で拒否されます。
...
| `instructions`            | 可変長                      | [コンパイル済みinstructionsの配列](#instructions) |
...
Program が lamport を差し引くことができます。lamport 残高を変更するために、送信者と受信者の両方のアカウントが書き込み可能である必要があります。この instruction を実行するために、送信者のウォレットは署名と SOL 送金 instruction を含むメッセージを含んだトランザクションを送信します。

![SOL送金ダイアグラム](/assets/docs/core/transactions/sol-transfer.svg)

トランザクションが送信された後、System
Program は送金 instruction を処理し、両方のアカウントの lamport 残高を更新します。

![SOL送金プロセスダイアグラム](/assets/docs/core/transactions/sol-transfer-process.svg)

<Callout type="warn" title="SOLを送信する前に受取人を確認してください">

System Program の送金は **あらゆる**
アカウントに lamport を追加します。受取人が SOL を送り返せるかどうかのプロトコルレベルのチェックはありません。lamport はアカウントの所有プログラムによってのみ移動できるため、管理していない
[token mint](/docs/tokens/basics/create-mint)、プログラム、または
[PDA](/docs/core/pda) に SOL を送金すると **資金の永久損失のリスク**
があります — 所有プログラムが指定した権限者のみが返還できます。[token account](/docs/tokens/basics/create-token-account)
に送金された SOL は、そのアカウントの所有者のみが回収できます。送信者には回収できません。

SPL **トークン** 送金には部分的な自己保護機能があります：Token
Program は、アカウントが期待される mint と一致しない送金を拒否します。ネイティブ
**SOL**
送金にはそのようなガードがないため、署名前に送信者が受取人を確認する必要があります。分類ロジックの詳細については、[アドレスの確認](/docs/payments/send-payments/verify-address)
を参照してください。

</Callout>

以下の例は、上記の図に関連するコードを示しています。System Programの
[`transfer`関数](https://github.com/anza-xyz/agave/blob/v3.1.8/programs/system/src/system_processor.rs#L210)をご参照ください。
...
SDKによってフォーマットは異なりますが、各instructionsには同じ必要な情報が含まれていることに注目してください。
...
SOLの転送はどのアカウントにも成功するため、署名前に受取人を確認してください。アカウントを取得し、System
Programウォレット（または未資金の[オンカーブ](/docs/payments/send-payments/verify-address#on-curve-addresses)アドレス）にのみ送信してください。自分が管理していないミント、token
account、プログラム、およびPDAは拒否してください。
...
<Callout>
  ...
  account、ATA、Token-2022）も処理する完全な分類については、[アドレスの確認](/docs/payments/send-payments/verify-address)をご参照ください。
</Callout>
...
生のレスポンスでは、アカウントはインデックスで識別され、内部（CPI）instructions はエンコードされたブロブとして保存されます。これらをアドレスに解決し、完全なinstructionsツリーをたどるには、[Transaction Introspection](/docs/core/transactions/transaction-introspection)
を参照してください。
