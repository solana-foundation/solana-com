...
<Callout type="info" title="概要">
  呼び出し元プログラムが所有するPDAの代わりに署名する必要がある場合は、
  _rs`invoke_signed`_
  を使用します。ランタイムは提供された署名者シードからPDA公開鍵を導出し、権限チェックの前に有効な署名者のセットに追加します。
</Callout>
...
- **例2**: _rs`solana_program`_ クレートの _rs`system_instruction::transfer`_
