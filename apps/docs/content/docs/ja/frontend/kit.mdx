...
seoTitle: Next.jsとSolana Kitを使用したSolanaウォレット連携
description: Next.jsでSolana Kitを使用してSolanaウォレット連携を設定します。
---

Next.jsでSolana Kitを使用して最小限のSolanaウォレット連携を設定します。

このガイドでは、Next.jsアプリケーションで`@solana/kit`を使用してSolanaウォレット機能を実装する最小限の例を提供します。ウォレット接続ボタンとトランザクション送信用のコンポーネントを作成します。

![Nextjs Kitアプリケーション](/assets/docs/frontend/nextjs-kit.gif)

Reactアプリケーションで`@solana/kit`を使用するより包括的な例については、Solana
Kitリポジトリの[Reactアプリの例](https://github.com/anza-xyz/kit/tree/main/examples/react-app)を参照してください。

## リソース

- [Solana Kit ドキュメント](https://www.solanakit.com/docs)
- [Solana Kit GitHubリポジトリ](https://github.com/anza-xyz/kit)
...
UIコンポーネント用の[shadcn](https://ui.shadcn.com/)を使用して新しいNext.jsプロジェクトを作成し、必要なSolana依存関係をインストールします。
...
次に、Next.jsアプリケーション全体をSolanaプロバイダーでラップします。
...
- メモプログラムを呼び出すinstructionsを含むSolanaトランザクションを作成します
- 接続されたウォレットにトランザクションの署名と送信を要求します
- Solana Explorerでトランザクションを表示するためのリンクを表示します
...
<Callout type="info">
  ...
  SOLで資金提供](https://faucet.solana.com/)されている必要があることに注意してください。
</Callout>
