...
<Callout type="info">
  ヘッドレスで実行中 —
  ブラウザウォレットのないAPIルート、ワーカー、またはスクリプトで使用していますか？
  生のkeypairの代わりに、バックエンドの鍵管理サービスで署名してください。
  [Keychain](/docs/tools/keychain) および
  [本番環境での署名](/docs/core/transactions/signing-in-production)
  を参照してください。
</Callout>
...
- **ヘッドレスステートマシン**:
  APIルート、スクリプト、またはワーカー内でクライアントを実行し、UIを動かすのと同じウォレット +
  RPCオーケストレーションロジックを再利用できます。
- **リアルタイムダッシュボード**: ウォッチャー（残高、アカウント、署名）をお好みのUIライブラリと組み合わせることで、クライアントがWebSocketのファンアウトとキャッシュの無効化を処理します。
- **カスタムストア**: 独自のZustandストアを注入して、IndexedDB/localStorageからのハイドレーション、サーバーセッションへの状態のミラーリング、またはブラウザタブ間の調整を行えます。
- **Reactフックへのブリッジ**: 同じランタイム上でエルゴノミックなフックを使用したい場合は、設定済みのクライアントインスタンスを`@solana/react-hooks`に渡してください。
- **テスト容易性**: 単一のクライアントインターフェースはユニットテストでモック化でき、ブラウザウォレットなしでRPCレスポンスやウォレットセッションのシミュレーションが容易になります。
