...
まず、プログラムに簡単な「更新時支払い」メカニズムを実装するため、*rs`Update`*構造体と`update`関数を変更します。
...
次に、*rs`Update`*構造体を更新して、`vault_account`という新しいアカウントを追加します。このアカウントはプログラムによって制御され、ユーザーがメッセージアカウントを更新する際にSOLを受け取ります。
...
- アカウントのアドレスは、*rs`[b"vault", user.key().as_ref()]`*をシードとして使用して導出されたPDAから取得されます
...
*rs`transfer()`*関数は、System Programの転送instructionを呼び出します。
...
まず、*rs`Delete`*構造体を更新して`vault_account`を含めます。これにより、ユーザーがメッセージアカウントを閉じるときに、ボールト内のSOLをユーザーに返金できるようになります。
