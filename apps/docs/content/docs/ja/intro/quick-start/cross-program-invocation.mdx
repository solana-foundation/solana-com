...
まず、プログラムに簡単な「更新時支払い」メカニズムを追加するために、*rs`Update`*構造体と`update`関数を変更します。
...
このセクションでは、*rs`Update`*構造体に`vault_account`という新しいアカウントを追加します。このアカウントは、ユーザーがメッセージを更新するときにユーザーからSOLを受け取るプログラム制御の「ボールト」として機能します。
...
- アカウントのアドレスは、*rs`[b"vault", user.key().as_ref()]`*というシードを使用して導出されたPDAから取得されます
- PDAであるため、秘密鍵を持たないため、CPIを実行する際にプログラムのみがアドレスに対して「署名」できます
...
`delete`命令では、プログラムがCPIでこのPDAに対して「署名」する方法を確認できます。
...
*rs`transfer()`*関数は、System Programの転送instructionsを呼び出します。
...
`update` instructionsの実装には、System Programの`transfer`
instructionsを呼び出すためのクロスプログラム呼び出し(CPI)が含まれています。これは、プログラム内からCPIを実行する方法を示しており、Solanaプログラムの組み合わせ可能性を実現します。
...
`transfer`関数は、System Programの転送instructionsを呼び出し、以下を渡します:
...
CPIのセットアップは、クライアント側のinstructionsの構築方法と一致しており、呼び出す特定のinstructionsのプログラム、アカウント、instruction
dataを指定します。プログラムの`update`
instructionsが呼び出しを受け取ると、内部的にSystem
Programの転送instructionsを呼び出します。
...
まず、*rs`Delete`*構造体を更新して`vault_account`を含めます。これにより、ユーザーがメッセージアカウントを閉じる際に、ボールト内のSOLをユーザーに返金できるようになります。
...
また、`system_program`を追加します。転送のCPIにはSystem
Programの呼び出しが必要なためです。
...
*rs`signer_seeds`*は、PDAの導出に使用されるオプションのシードとバンプシードを指定します。

### !with_signer

*rs`with_signer()`*メソッドは、CPIと共に署名者シードを渡します。
...
Programの転送instructionを呼び出し、以下を渡します：
...
次に、`anchor.test.ts`ファイルを更新して、instructionsに新しいボールトアカウントを含めます。これには、ボールトPDAを導出し、更新および削除instruction呼び出しに含める必要があります。
