...
_rs`payer`_ 制約は、新しいアカウントの作成費用を支払うアカウントを指定します。
...
1. プログラムが期待するアカウントの種類を指定するアカウント型（_rs`Signer<'info>`_
...
1. _rs`user: Signer<'info>`_
   ...
   - このアカウントからのlamportの控除を承認するため、トランザクションに署名する必要があります
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - `payer = user` は新しいアカウントの作成費用を支払う者を識別します
...
`create`
関数は、新しいメッセージアカウントのデータを初期化するためのロジックを実装します。2つのパラメータを取ります：
...
次に、`MessageAccount` を新しいメッセージで変更するための `update`
instructionを追加します。
...
1. _rs`user: Signer<'info>`_
   ...
   - 必要に応じて `message_account`
     のためにより多くのスペースの費用を支払う可能性があるため、可変（_rs`#[account(mut)]`_）としてマークされています
...
制約は、Anchorに再計算させるのではなく、*rs`message_account`*に保存されているbump
seedを使用することに注意してください。

_rs`#[instruction(message: String)]`_ 属性により、*rs`Update`*構造体が
...
`update`
関数は、既存のメッセージアカウントを変更するためのロジックを実装します。この関数は2つのパラメータを取ります：
...
次に、_rs`delete`_ instructionを追加して、*rs`MessageAccount`*を閉じます。
...
_rs`seeds`_ 制約は、PDAを導出するために使用されるseedを指定します。

### !bump

_rs`bump`_ 制約は、PDAのbump seedを指定します。

この場合、プログラムは *rs`message_account`*に保存されている既存のbump
seedを使用します。
...
この場合、_rs`user`_ アカウントは、閉じられた
*rs`message_account`*からlamportを受け取ります。
...
1. _rs`user: Signer<'info>`_
   ...
   - 閉じられたアカウントからlamportsを受け取るため、mutable
     (_rs`#[account(mut)]`_) としてマークされています
...
_ts`create()`_ メソッドは `create` 命令を呼び出します。

### !accounts

_ts`accounts()`_ メソッドは _ts`create()`_ 命令に必要なアカウントを指定します。
...
まず、テストファイルは `create`
命令を呼び出すトランザクションを送信し、メッセージとして「Hello,
World!」を渡します。
...
_ts`update()`_ メソッドは `update` 命令を呼び出します。

### !accounts

_ts`accounts()`_ メソッドは _ts`update()`_ 命令に必要なアカウントを指定します。
...
_ts`delete()`_ メソッドは `delete` instructionを呼び出します。

### !accounts

_ts`accounts()`_ メソッドは _ts`delete()`_
...
まず、このコードは `delete`
instructionを呼び出すトランザクションを送信して、メッセージアカウントを閉じます。
...
トランザクションを送信してアカウントを閉じた後、この例では `fetchNullable`
を使用してそのアドレス（`messagePda`）でアカウントを取得しようとします。このメソッドは、閉じた後にアカウントが存在しなくなった場合にnullを返します。
...
<Callout type="info">
  この例では、テストを再度実行すると、`messageAccount`
  がすでにアカウントとして存在するため、`create`
  instructionが失敗することに注意してください。特定のPDAに対して存在できるアカウントは1つだけです。
</Callout>
