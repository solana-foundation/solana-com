...
LinuxまたはWSLにAnchor CLIをインストールする際、次のエラーが発生することがあります：

```
error: could not exec the linker cc = note: Permission denied (os error 13)
```

このエラーメッセージが表示された場合は、以下の手順に従ってください：

1. このページの上部にある
   [Linux](/docs/intro/installation/dependencies) セクションに記載されている依存関係をインストールします。
