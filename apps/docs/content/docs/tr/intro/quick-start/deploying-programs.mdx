...
Programınızın onchain adresi. Projeyi derlediğinizde güncellenecektir
...
- [`declare_id!`](mention:one) makrosu, programınızın onchain adresini belirtir.
  Solana Playground, programı derlediğinizde bu adresi otomatik olarak
  güncelleyecektir.
...
<Callout>
  ...
  kimliğiniz](/docs/intro/quick-start/build-first-program) artık
  güncellenecektir.
</Callout>

2. Cüzdanınıza SOL ekleyin _(İsteğe bağlı)_

Bir programı dağıtmak, program onchain hesabında SOL gerektirir. Gerekirse, 3.
adıma geçmeden önce
...
Onchain programa tahsis edilen SOL'u geri almak için programı kapatın. Bunu
yapmak için, _rs`declare_id!()`_ makrosunda saklanan program adresini kullanarak
`solana program close [ProgramID]` komutunu çalıştırın.
...
[program-türetilmiş adresler](/docs/core/pda) ve
[programlar arası çağrı](/docs/core/cpi) konularını keşfetmeye hazırsınız.
