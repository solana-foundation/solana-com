...
description:
  ...
  (Oluşturma, Okuma, Güncelleme, Silme) Solana programı oluşturmayı öğrenin. Bu
  adım adım kılavuz, PDA'lar kullanarak zincir üstü mesaj hesaplarının nasıl
  oluşturulacağını, güncelleneceğini ve silineceğini, hesap doğrulamasının nasıl
  uygulanacağını ve testlerin nasıl yazılacağını göstermektedir. Solana
  programlarında PDA'ların nasıl kullanılacağını anlamak isteyen geliştiriciler
  için idealdir.
