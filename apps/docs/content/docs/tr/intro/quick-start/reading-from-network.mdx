...
Farklı hesapları getirerek Solana ağından veri okuyun. Bu kılavuz,
Solana [hesaplarının](/docs/core/accounts) yapısını açıklar. Her Solana
hesabının, hesabın zincir üzerindeki verilerini bulmak için kullanılan
benzersiz bir [adresi](/docs/core/accounts/account-structure#account-address) vardır. Solana hesapları,
[durum verisi veya çalıştırılabilir bir program](/docs/core/accounts/account-types) içerir.
...
[Mint](mention:one)
veri türüne seri dışı bırakmak için [_ts`fetchMint()`_](https://github.com/solana-program/token/blob/program%40v8.0.0/program/src/state.rs#L16-L30)
fonksiyonunu içerir.
