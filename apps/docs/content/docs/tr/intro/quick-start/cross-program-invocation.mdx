...
kontrollü bir "kasa" görevi görür.
...
- PDA olduğu için özel anahtarı yoktur, bu nedenle CPI'lar gerçekleştirilirken
  yalnızca program adres için "imza atabilir"
- _rs`SystemAccount`_ türü olarak, System Program tarafından normal cüzdan
  hesapları gibi sahiplenilir
...
`delete` talimatında, programın bir CPI'da bu PDA için nasıl "imza
atabileceğini" göreceksiniz.

</Accordion>
</Accordions>

Ardından, kullanıcının hesabından kasa hesabına 0.001 SOL aktarmak için `update`
...
için bir Cross Program Invocation (CPI) içerir. Bu, program içinden nasıl CPI
...
eşleşir; burada çağrılacak belirli bir talimat için programı, hesapları ve
instruction data'yı belirtirsiniz. Programın `update` talimatı bir çağrı
aldığında, dahili olarak System Program'ın transfer talimatını çağırır.
...
İlk olarak, _rs`Delete`_ yapısını `vault_account` içerecek şekilde güncelleyin.
Bu, kullanıcılar mesaj hesaplarını kapattıklarında kasadaki herhangi bir SOL'un
kullanıcıya geri aktarılmasını sağlar.
...
kullanıcıya geri transfer edebilir.
...
Ardından, kasa hesabından kullanıcının hesabına SOL transfer etmek için `delete`
talimatına CPI mantığını ekleyin.
...
_rs`_ctx: Context<Delete>`_ öğesinin, fonksiyonun gövdesinde bağlamı kullanmak
...
Ardından, talimatlara yeni kasa hesabını dahil etmek için `anchor.test.ts`
dosyasını güncelleyin. Bu, kasa PDA'sını türetmeyi ve onu güncelleme ve silme
