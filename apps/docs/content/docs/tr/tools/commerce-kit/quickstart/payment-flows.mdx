...
Bu fonksiyonlar, işlemleri doğrulamak ve onayları beklemek için Solana ile etkileşime girer.
`@solana/kit` kütüphanesinden bir Solana RPC istemcisi gerektirirler.
...
- **`rpc`** (`Rpc<SolanaRpcApi>`, zorunlu) - `@solana/kit` kütüphanesinden RPC istemcisi.
