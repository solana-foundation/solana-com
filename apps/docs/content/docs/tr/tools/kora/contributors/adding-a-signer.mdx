...
Servisinizi ana README'nin imzalayan listesine ekleyin.

## Gönderim Kontrol Listesi

- [ ] İmzalayanınız `solana-keychain` crate'inde destekleniyor
- [ ] Cargo.toml'daki `solana-keychain` bağımlılığı en son sürüme güncellendi
- [ ] İmzalayanınız için yapılandırma yapısı eklendi
- [ ] `SignerTypeConfig` varyantı eklendi
- [ ] `build_signer_from_config` içinde derleme mantığı eklendi
- [ ] `validate_individual_signer_config` içinde doğrulama mantığı eklendi
- [ ] Yapılandırma yapısı `mod.rs` içinde dışa aktarıldı
- [ ] (İsteğe bağlı) `config_mock.rs` içinde test mock builder metodu eklendi
- [ ] Kod uyarısız derleniyor
- [ ] Tüm testler geçiyor (`make test` ve `make test-integration`)
- [ ] Belgeler şu kılavuza uygun olarak eklendi:
      [`ADDING_SIGNERS.md`](https://github.com/solana-foundation/solana-keychain/blob/main/docs/ADDING_SIGNERS.md)
- [ ] Örnek yapılandırma dosyaları oluşturuldu (`.toml` ve `.env.example`)
- [ ] Sabit kodlanmış değer veya gizli bilgi yok
- [ ] Hata mesajları açıklayıcı
- [ ] Rust adlandırma kurallarına uyuyor (snake_case)
- [ ] Linting geçiyor (`make lint`)
- [ ] Entegrasyon testi için API Anahtarlarıyla Kora ekibiyle iletişime geçin

## Yardım Alma

- **İmzalayan uygulaması için**:
  [`solana-keychain`](https://github.com/solana-foundation/solana-keychain)
  deposunda bir konu açın
- **Kora entegrasyonu için**: Tasarım tartışmaları için Kora deposunda bir konu
  açın
...
- [`crates/lib/src/signer/config.rs`](https://github.com/solana-foundation/kora/blob/v2.0.5/crates/lib/src/signer/config.rs)
  içindeki mevcut imzalayan yapılandırmalarını inceleyin
...
Kora ekosistemine hoş geldiniz! Anahtar yönetim çözümünüzü platformun bir
parçası olarak aramızda görmekten heyecan duyuyoruz.
