...
> _Not: `fallback_if_unavailable` true olduğunda, sistem Redis geçici olarak kullanılamaz durumdaysa
> işlemlerin devam etmesine izin vererek hizmet kesintisini önler.
> `max_transactions` değerini 0 olarak ayarlamak sınırsız işleme izin verir._
...
| `get_payer_signer`          | Sonraki ücret ödeyicisini ve ödeme adresini döndür        | ✅       | boolean |
...
[Solana Dokümanları](https://solana.com/developers/cookbook/transactions/durable-nonces)'nda
öğrenin.
...
| Uzantı Adı               | Açıklama                                      |
| ---------------------------- | ------------------------------------------------ |
| `confidential_transfer_mint` | Mint için Gizli Bakiyeler yapılandırması |
...
| Uzantı Adı                  | Açıklama                                 |
| ------------------------------- | ------------------------------------------- |
| `confidential_transfer_account` | Hesap için Gizli Bakiyeler durumu |
