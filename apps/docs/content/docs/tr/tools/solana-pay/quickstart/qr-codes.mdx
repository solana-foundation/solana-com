...
description: Mobil cüzdan ödemeleri için QR kodları oluşturun ve özelleştirin
---

QR kodlar, web uygulamaları ile mobil cüzdanlar arasındaki boşluğu kapatmanın en
yaygın yoludur. Solana Pay, `@solana/qr-code-styling` tarafından desteklenen
yerleşik bir `createQR` fonksiyonu içerir — ek QR kod paketlerine gerek yoktur.
...
### QR Kodlarını Özelleştirme

`createQR` fonksiyonu, boyut, arka plan rengi ve ön plan rengi için isteğe bağlı
parametreler kabul eder:
...
### Gelişmiş Kullanım için QR Seçeneklerini Alma

Daha fazla kontrol için ham yapılandırma nesnesini almak üzere `createQROptions`
kullanın:
...
## Merchant Client Kullanımı

Merchant client ayrıca QR kod yöntemlerini de sunar:
...
### Durum Güncellemeleriyle Ödeme QR Kodu
...
## Satış Noktası Entegrasyonu

### POS Terminal QR Görüntüleme
...
### Mobil Cihazlar İçin Optimize Edilmiş Görüntüleme

Mobil cihazlarda, QR kodunun yanında doğrudan bir bağlantı sağlamayı düşünün:
