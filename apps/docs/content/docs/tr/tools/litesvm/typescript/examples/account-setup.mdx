...
description: LiteSVM ile test hesaplarını ve durumu ayarlayın
---

# Hesap Kurulumu Örneği

Test hesap durumunu doğrudan işlem olmadan ayarlamayı gösterir.
...
Program'a ait bir program data account ayarlayın:
...
<Callout type="info">
  `setAccount` kullanmak, toplu hesap kurulumu için işlem yürütmekten daha
  hızlıdır.
</Callout>

## Temel Noktalar

1. **Doğrudan Durum**: `setAccount`, hesap durumunu işlemler olmadan ayarlar
2. **Kira Muafiyeti**: Gerekli lamport miktarını hesaplamak için
   `minimumBalanceForRentExemption` kullanın
3. **Hesap Yapısı**: Tüm gerekli alanları sağlayın: address, data, executable,
   lamports, programAddress, space
4. **Doğrulama**: Doğrulamak için `getAccount` veya `client.rpc.getAccountInfo`
   kullanın
