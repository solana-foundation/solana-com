---
title: Yapılandırma
description: TypeScript'te LiteSVM yapılandırması için builder metodları
---

LiteSVM, yapılandırma için builder desenini kullanır. Tüm metodlar zincirleme için `LiteSVM` döndürür.
...
<Callout type="info">
  İmza doğrulamanın devre dışı bırakılması testleri daha hızlı yapar ancak daha az gerçekçi hale getirir.
</Callout>
...
Önceden derlenmiş programları etkinleştirin.
...
Varsayılan programları ekleyin (System Program, BPF Loader, vb.).
...
Hesaplar için varsayılan lamport değerini ayarlayın.
...
Log çıktısı bayt sınırını ayarlayın.
...
İşlem geçmişi depolamayı etkinleştirin.
...
Belirli Solana sürümlerini test etmek için özellik kümesini yapılandırın.
