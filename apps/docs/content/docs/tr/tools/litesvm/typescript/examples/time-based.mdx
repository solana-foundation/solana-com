...
`warpToSlot` kullanarak zamana bağımlı program mantığını test etmeyi gösterir.
...
Zaman kilitli işlevselliği test etmek için örnek kalıp:
...
## Epoch Programını Okuma

Kümedeki slot/epoch ilişkisini anlamak için epoch programını okuyabilirsiniz:
...
Programınız Clock sysvar'ından `epoch` veya `unix_timestamp` okurken, bu alanları doğrudan ayarlamak için `setClock` kullanın:
...
| Senaryo                | Yaklaşım                           |
| ----------------------- | ---------------------------------- |
| Token vesting           | Vesting eşiğini/kilometre taşlarını geçmek için ileri sarın |
| Açık artırma bitişleri         | Açık artırma bitiş slot'unu geçmek için ileri sarın         |
| Staking ödülleri         | epoch ayarlamak için `setClock` kullanın        |
| Zaman kilitli çekimler | Kilit açma slot'unu geçmek için ileri sarın              |
| Hız sınırlama           | İzin verilen aralıklar arasında ileri sarın     |
| Süresi dolan emirler         | Emir son kullanma tarihini geçmek için ileri sarın         |

<Callout type="info">
  `warpToSlot` yalnızca slot'u değiştirir. Unix zaman damgası, SVM uygulamasına bağlı olarak orantılı biçimde güncellenmeyebilir.
</Callout>

## Önemli Noktalar

1. **Sysvar'ları Etkinleştirin**: Saat metodlarını kullanmadan önce `withSysvars()` çağırın
2. **İleri Sarın**: Slot'u ilerletmek için `warpToSlot(slot)` kullanın
3. **Tam Kontrol**: `epoch`, `unixTimestamp` veya diğer saat alanlarını ayarlamanız gerektiğinde `setClock` kullanın
4. **Saati Okuyun**: Mevcut slot, epoch ve zaman damgasını okumak için `getClock()` kullanın
5. **Test Kalıbı**: Zaman sınırlarından önce ve sonra test edin
