...
[Codama JS Renderer](https://github.com/codama-idl/renderers-js) ile oluşturmanızı öneririz.
...
1. **Program Yükleme**: Derlenmiş programları yüklemek için `addProgramFromFile` kullanın ve
   istemcinizden myProgramPlugin() içe aktarın
...
4. **Gönderme**: `client.myProgram.testInstruction({..}).sendTransaction()` kullanın —
   manuel işlem oluşturma gerekmez
5. **Doğrulama**: Yürütme sonrasında hesap durumunu kontrol edin
