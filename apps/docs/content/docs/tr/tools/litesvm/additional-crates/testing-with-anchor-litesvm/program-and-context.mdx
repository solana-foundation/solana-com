---
title: Program ve Bağlam
description:
  AnchorContext ve Program ile test ortamları kurma ve yapı talimatları
  oluşturma
---

## declare_program! ile İstemci Türleri Oluşturma

Anchor 1.0, derleme zamanında programınızın IDL'sini okuyarak tür güvenli
`client::accounts::*` ve `client::args::*` modülleri oluşturan
`declare_program!` makrosunu tanıtır. Bu, testlerde talimat oluşturmanın
önerilen yoludur.
...
<Callout type="info">
  `target/idl/my_program.json` dosyasını oluşturmak için önce `anchor build`
  komutunu çalıştırın. `declare_program!` bu dosyayı derleme zamanında okur.
  Makro adı, programınızın crate adıyla (snake_case) eşleşmelidir.
</Callout>

## Test Ortamının Kurulması

`AnchorLiteSVM` oluşturucu, test ortamınızı yapılandırmak için akıcı bir API
sunar.
...
<Callout type="info">
  Bir ödeyici belirtmezseniz, `AnchorLiteSVM` sizin için otomatik olarak bir
  tane oluşturur ve fonlar.
</Callout>

## AnchorContext

`AnchorContext`, test ortamınızla etkileşim için ana arayüzdür.
...
## Program ile Talimat Oluşturma

`Program` yapısı, Anchor-client'ı yansıtan talimatlar oluşturmak için akıcı bir
API sunar. Tür güvenliği için `declare_program!` tarafından oluşturulan türleri
kullanın.

### Temel Talimat Oluşturma
...
<Callout type="info">
  `.accounts()` metodu, `ToAccountMetas` uygulayan herhangi bir türü kabul eder;
  tüm Anchor hesap yapıları bunu otomatik olarak yapar. Benzer şekilde,
  `.args()`, `InstructionData` uygulayan herhangi bir türü kabul eder.
</Callout>
...
### Birden Fazla Talimatın Yürütülmesi
...
### send_and_confirm_transaction Kullanımı
...
`declare_program!` ile emanet desenini kullanan kapsamlı bir örnek:
