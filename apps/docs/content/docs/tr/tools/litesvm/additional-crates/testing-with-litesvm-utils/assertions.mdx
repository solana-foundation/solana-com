...
title: Assertions
description:
  Assertion yardımcıları ile hesap durumlarını, bakiyeleri ve sahipliği
  doğrulayın
---

## Hesap Varlığı

Hesapların var olup olmadığını veya kapatılıp kapatılmadığını doğrulayın.
...
<Callout type="info">
  `assert_account_closed` bir hesap ya mevcut değilse, sıfır lamports içeriyorsa
  ya da boş veriye sahipse başarılı olur. Hesapları kapattıktan sonra temizliği
  doğrulamak için kullanın.
</Callout>

## Bakiye Assertions

SOL ve token bakiyelerini doğrulayın.

### SOL Bakiyesini Doğrula
...
Hesap sahipliğini ve veri özelliklerini doğrulayın.

### Hesap Sahibini Doğrula
...
<Callout type="info">
  Yaygın hesap boyutları: - **Mint Account**: 82 bayt - **Token Account**: 165
  bayt - **System Account**: 0 bayt (yalnızca lamports)
</Callout>

## Tam Örnek

Birden fazla doğrulamayı gösteren kapsamlı bir test:
...
Bir doğrulama başarısız olduğunda, açıklayıcı bir mesajla hata verir:
...
<Callout type="warn">
  Doğrulamalar, açık hata mesajlarıyla hızlı başarısız olacak şekilde
  tasarlanmıştır. Sorunları erken yakalamak için testleriniz boyunca bunları bol
  miktarda kullanın.
</Callout>
