...
Fordefi, `chain` parametresini iletip iletmediğinize göre seçilen iki imzalama moduna sahiptir. API istekleri, ECDSA P-256 istek imzasıyla doğrulanır — imzalama anahtarını `privateKeyPem` olarak sağlayın ya da bir `requestSigner` ileterek KMS/HSM üzerinde saklayın.

**Yerel Solana modu** (zincir üstü kullanım için önerilir): `chain` parametresini ayarlayın; Fordefi işlemi kendisi imzalar ve yayınlar. Fordefi imzalamadan önce blockhash veya ücretleri değiştirebileceğinden, imzalayıcı bir Kit `TransactionSendingSigner`'dır — bunu yukarıda gösterilen kısmi imzalayıcı akışıyla değil, `signAndSendTransactionMessageWithSigners` ile kullanın.
...
**Kara kutu modu**: `chain` parametresini atlayın. Fordefi ham baytları imzalar ve yayınlamaz; işlemi kendiniz oluşturup gönderirsiniz. Bunu bir Fordefi kara kutu kasasıyla kullanın.
