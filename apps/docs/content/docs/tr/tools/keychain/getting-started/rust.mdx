...
| `all`        | Tüm arka uçları etkinleştir             |
...
Fordefi MPC saklama imzalama için. API istekleri, `private_key_pem` kaynağından ayrıştırılan bir ECDSA
P-256 istek imzasıyla doğrulanır; bu anahtarı bir
KMS/HSM'de tutmak için `FordefiRequestSigner` uygulayın ve
`Signer::from_fordefi_with_signer` kullanın.

Fordefi'nin iki imzalama modu vardır. **Yerel Solana modu** için `chain` ayarlayın; bu modda
Fordefi blockhash veya ücretleri değiştirebilir ve işlemi kendisi yayınlar.
**Kara kutu modu** için `chain`'i `None` olarak bırakın; bu modda Fordefi ham baytları
imzalar ve işlemi kendiniz yayınlarsınız.
