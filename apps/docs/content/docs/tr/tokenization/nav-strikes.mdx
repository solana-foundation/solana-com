...
difficulty: advanced
title: "Solana'da Para Piyasası Fonları için NAV Fiyatlaması"
description:
...
NAV (Net Varlık Değeri) fiyatlaması, işlem günü boyunca fon paylarının fiyatlandırıldığı ve emirlerin gerçekleştirildiği belirli zaman dilimleridir. Geleneksel fonlar genellikle piyasa kapanışında günlük tek bir fiyatlama yaparken, Solana'nın hızı ve düşük maliyetleri gün içinde birden fazla fiyatlama imkânı sunarak yatırımcılara daha fazla esneklik sağlar.

## NAV Nedir?

Fon, birçok yatırımcının birlikte sahip olduğu bir varlık havuzudur. **NAV (Net Varlık Değeri)**, bir fondaki tek bir payın fiyatıdır. Şöyle düşünebilirsiniz:

```
NAV = (Total Value of Everything in the Fund - liabilities) ÷ Number of Shares
```

Örneğin, bir fon 10 milyon dolar varlık tutuyorsa ve 10 milyon pay ihraç edilmişse, her pay 1,00 dolar değerindedir.

**NAV neden önemlidir?**

- **Pay satın alma (abonelik)**: Geçerli NAV'ı ödersiniz. NAV 1,02 dolar ise ve 102 dolar yatırım yaparsanız, 100 pay alırsınız.
- **Pay satma (itfa)**: Geçerli NAV'ı alırsınız. 100 payı 1,02 dolarlık NAV üzerinden itfa ederseniz, 102 dolar alırsınız.

Para piyasası fonları genellikle kazanılan faize bağlı küçük dalgalanmalarla birlikte 1,00 dolar civarında istikrarlı bir NAV korur.

## Sorun

Bir para piyasası fonuna yatırım yaptığınızda, geleneksel olarak iki şey farklı zamanlarda gerçekleşir:

1. Emir gönderirsiniz → Piyasa kapanışını beklersiniz
2. NAV hesaplanır → Paylar T+1 veya T+2'de ihraç edilir/itfa edilir

Bu durum çeşitli sorunlara yol açar:

- **Gecikmeli gerçekleşme**: Sabah gönderilen emirler saat 16:00'a kadar bekler
- **Takas gecikmesi**: Paranız 1-2 gün askıda kalır
- **Sınırlı esneklik**: Günde yalnızca bir işlem fırsatı
- **Eski fiyatlandırma**: Emir ile gerçekleşme arasında NAV güncelliğini yitirebilir

## Çözüm

**Solana'da NAV Fiyatlaması**, atomik gerçekleşme ile birden fazla günlük takas penceresi sağlar. Emirler sıraya girer ve her fiyatlama zamanında geçerli NAV ile anında gerçekleştirilir.
...
Bu kılavuz, uyumlu fon payı ihracı için SPL Token 2022 uzantılarını ve takas için standart USDC'yi kullanarak, özel Rust programı yazmadan Solana'da NAV fiyatlamasının nasıl uygulanacağını gösterir.

<Callout type="info" title="Eğitim Amaçlı Referans Uygulama">

Bu uygulamanın
[kaynak kodunu](https://github.com/Woody4618/nav-strikes-reference) NAV fiyatlamasını yerel ortamda denemek için kullanabilirsiniz.

Bu kılavuz yalnızca keşif ve eğitim amaçlı bir referans uygulama sunmaktadır. **Bu kodu doğrudan üretim ortamında KULLANMAYIN**; önce şunları tamamlayın:
...
NAV Fiyatlama sistemi şu temel bileşenlerden oluşur:

1. **Fon Payı Token'ı**: Zincir üzerinde NAV depolayan meta veri içeren SPL Token 2022
2. **Takas Para Birimi**: Standart USDC (mevcut SPL token)
3. **Fon Yöneticisi**: Yetki devri aracılığıyla fiyatlamaları yönetir
4. **Beyaz Liste Sistemi**: Fon paylarını tutabilecek adresleri denetler

### Temel Tasarım İlkeleri

- **Özel Program Yok**: Yalnızca Token Extensions Program ve standart USDC kullanır
- **Atomik Takas**: Tek işlem, abonelik/itfanın ya tamamen tamamlanmasını ya da tamamen başarısız olmasını sağlar
- **Varsayılan Dondurulmuş Durum**: Mevzuat uyumluluğu için paylar açık beyaz listeye alınmayı gerektirir
- **Yetki Devri**: Fon yöneticisi, velayet üstlenmeksizin işlem yapar
- **Zincir Üzerinde NAV**: Geçerli fiyat token meta verilerinde saklanır, herkesçe doğrulanabilir

### Aşama 1: Fon Kurulumu

1. Meta veri uzantılarıyla Fon Payı token'ı oluşturun
2. NAV'ı 1,00 dolar olarak başlatın
3. Günlük fiyatlama programını ayarlayın
4. Kurumsal yatırımcıları beyaz listeye ekleyin

### Aşama 2: Fiyatlama Öncesi Dönem

1. Yatırımcılar abonelik/itfa talepleri gönderir
2. Emirler bir sonraki fiyatlama zamanı için sıraya girer
3. Fon ön NAV hesaplaması yapar
4. Risk kontrolleri gerçekleştirilir

### Aşama 3: NAV Fiyatlama Gerçekleştirmesi
...
### Aşama 4: Fiyatlama Sonrası

1. İşlem teyitleri oluşturun
2. Fon kompozisyonunu güncelleyin
3. Bir sonraki fiyatlama için hazırlanın
4. Saklayıcı ile mutabakat sağlayın

### Yetki Devri Modeli

Yatırımcılar, token'larını taşıması için fon yöneticisine önceden yetki verir; ardından yönetici, yatırımcı imzasına gerek kalmadan fiyatlama zamanında işlemi gerçekleştirir:
...
### Abonelik Akışı (USDC → Fon Payları)
...
## NAV Fiyatlaması için Neden Solana?

Solana'nın mimarisi, geleneksel fon takasına kıyasla önemli avantajlar sunar:

| Konu            | Geleneksel (T+1) | Solana NAV Fiyatlaması          |
| ----------------- | ----------------- | --------------------------- |
| Takas Mantığı  | Transfer acenteleri   | Atomik işlem paketleme |
| Takas Süresi   | 1-2 gün          | &lt;1 saniye                |
| İşlem Maliyeti  | 50-200 dolar           | &lt;0,01 dolar                   |
| Günlük Fiyatlama Sayısı   | 1                 | 4+ (yapılandırılabilir)           |
| Eski fiyat riski | Yüksek              | Düşük                       |
| NAV Şeffaflığı  | Gün sonu raporu | Zincir üzerinde, gerçek zamanlı          |

Solana'nın atomik işlemleri, aracıları ortadan kaldırırken zincir üzerinde NAV şeffaflığıyla anında, ucuz ve güvenli takas sağlar.

## Token Extensions Program ile Fon Token'ı

SPL Token 2022, özel program gerektirmeden uyumlu fon payı ihracını mümkün kılan güçlü Token Extensions Program sağlar:

### Fon Payları için Temel Uzantılar

1. **Varsayılan Hesap Durumu Uzantısı**: Tüm yeni token account'ları varsayılan olarak dondurulmuş şekilde ayarlar; açık beyaz listeye alma (KYC/AML uyumluluğu) gerektirir
2. **Meta Veri Uzantısı**: Şeffaflık için NAV, fiyatlama programı ve AUM'u zincir üzerinde depolar

### Yetki Yapılandırması

- **Mint Yetkisi**: Fon Yöneticisi (pay ihracını denetler)
- **Dondurma Yetkisi**: Fon Yöneticisi (beyaz listeyi yönetir)
- **Güncelleme Yetkisi**: Fon Yöneticisi (NAV meta verilerini günceller)

<Callout type="info">

Varsayılan dondurulmuş durum, mevzuat uyumluluğu açısından kritik öneme sahiptir. Yalnızca açıkça beyaz listeye alınmış (KYC doğrulanmış) adreslerin fon paylarını alıp tutabilmesini güvence altına alır.

</Callout>

### Zincir Üzerinde Meta Veri Alanları
...
## Eksiksiz NAV Fiyatlama Uygulaması

### NAV Fiyatlama Motorunun Kurulumu

Bu kılavuzda NAV Fiyatlama Motorunu oluşturmak için @solana/kit kütüphanesini kullandığımızı belirtmek gerekir. [Kaynak kodunda](https://github.com/Woody4618/nav-strikes-reference) `web3.js` ile yapılmış uygulamaları da bulabilirsiniz. İlk olarak, tüm fon işlemlerini yöneten çekirdek motor sınıfını oluşturun:
...
<Callout type="tip" title="Dinamik Meta Veri Güncellemeleri">

Fon payları için `currentNAV`, `lastStrikeTime` ve `totalAUM` alanları,
`getUpdateTokenMetadataFieldInstruction` kullanılarak her NAV fiyatlamasında zincir üzerinde güncellenir. Bu şunları sağlar:

- Tüm katılımcılar için **gerçek zamanlı NAV görünürlüğü**
- Tüm NAV güncellemelerinin zincir üzerinde **denetim izi**
- Zincir üzerindeki meta verileri okuyabilen DeFi protokolleriyle **entegrasyon**
...
İşte tam bir NAV fiyatlama günü işlemlerini gösteren eksiksiz bir örnek:
...
## Üretim Ortamına Geçiş Değerlendirmeleri

Üretim ortamına geçmeden önce şu konuları ele aldığınızdan emin olun:

1. **Güvenlik**: Profesyonel anahtar yönetimi, çoklu imza denetimleri ve kod tabanının kapsamlı güvenlik denetimleri
2. **Mevzuat Uyumluluğu**: Menkul kıymet tescili, KYC/AML sistemleri, transfer kısıtlamaları ve zorunlu raporlama
3. **Anahtar Yönetimi**: Uygun yedekleme ve kurtarma prosedürleriyle kurumsal saklama çözümleri
4. **İşlem İşleme**: Öncelik ücretleri, yeniden deneme mantığı, teyit yönetimi ve RPC yedekliliği
5. **Fon İşlemleri**: Yetkili kaynaklardan NAV hesaplaması, saklayıcı entegrasyonu ve mutabakat
6. **İzleme**: Gerçek zamanlı takip, uyarı ve otomatik mevzuat raporlaması

## Sonraki Adımlar

Solana'da NAV fiyatlamasını anladıktan sonra:

1. **Token Extensions'ı Keşfedin**: Ek uyumluluk özellikleri için kalıcı delege gibi diğer
   [Token 2022 uzantıları](/docs/tokens/extensions/transfer-hook) hakkında bilgi edinin

2. **Token ACL Uygulayın**: Gelişmiş uyumluluk için, izinsiz çözme ile yatırımcı beyaz liste yönetimini sağlamak üzere
   [Token ACL](/docs/tokenization/token-acl) kullanımını değerlendirin. Bu, yatırımcıların uyumluluk kontrollerini geçtikten sonra hesaplarını otomatik olarak etkinleştirebildiği, operasyonel yükü azaltırken mevzuat denetimini koruyan self-servis KYC doğrulamasını mümkün kılar.

3. **DvP Takasını İnceleyin**: Menkul kıymet takas modelleri için
   [Teslimat ve Ödeme kılavuzumuza](/docs/tokenization/dvp) bakın

4. **Üretim Mimarisi**: Güçlü zamanlama, izleme ve felaket kurtarma sistemleri tasarlayın

## Sonuç

Solana'nın atomik işlem modeli ve Token Extensions Program, para piyasası fonları için birden fazla günlük NAV fiyatlaması uygulamak adına güçlü bir temel sağlar. Şunların bir araya gelmesi:

- Yerel atomik gerçekleşme (akıllı sözleşme riski yok)
- Saniyenin altında kesinlik
- Sıfıra yakın işlem maliyetleri
- Zincir üzerinde NAV şeffaflığı
- Yerleşik uyumluluk özellikleri (dondurulmuş varsayılanlar, meta veri)

Solana'yı fon takas altyapısını modernize etmek ve yatırımcılara daha sık işlem fırsatları sunmak için ideal bir platform hâline getirir.

Ancak bu eğitim amaçlı referans uygulamadan üretim ortamına geçiş, güvenlik, uyumluluk, saklama ve operasyonlar konularında önemli ek çalışma gerektirir. Fon tokenizasyonuyla ilgilenirken her zaman nitelikli hukuki, düzenleyici ve teknik uzmanlarla iş birliği yapın.
