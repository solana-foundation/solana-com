...
<Callout type="info">
  Tarayıcı cüzdanı olmayan bir API rotasında, worker'da veya komut dosyasında
  başsız mı çalışıyorsunuz? Ham bir keypair yerine bir arka uç anahtar yönetim
  hizmeti ile imzalayın. Bkz. [Keychain](/docs/tools/keychain) ve [Üretimde
  İmzalama](/docs/core/transactions/signing-in-production).
</Callout>

## Solana geliştiricileri için yaygın kalıplar

- **Başsız durum makineleri**: İstemciyi API rotalarında, komut dosyalarında
  veya worker'larda çalıştırarak kullanıcı arayüzünüzü destekleyen aynı cüzdan +
  RPC orkestrasyon mantığını yeniden kullanın.
- **Gerçek zamanlı gösterge panelleri**: İzleyicileri (bakiyeler, hesaplar,
  imzalar) tercih ettiğiniz UI kütüphanesiyle birleştirin; istemci websocket
  dağıtımını ve önbellek geçersiz kılmayı yönetir.
- **Özel depolar**: IndexedDB/localStorage'dan beslemek, durumu sunucu
