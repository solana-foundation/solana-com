...
<Callout type="info">
  Başsız modda çalışıyor musunuz — tarayıcı cüzdanı olmayan bir API rotası,
  worker veya script içinde mi? Ham bir keypair yerine backend anahtar yönetim
  servisiyle imzalayın. Bkz. [Keychain](/docs/tools/keychain) ve [Üretimde
  İmzalama](/docs/core/transactions/signing-in-production).
</Callout>

## Solana geliştiricileri için yaygın kullanım kalıpları

- **Başsız durum makineleri**: İstemciyi API rotaları, scriptler veya worker'lar
  içinde çalıştırarak kullanıcı arayüzünüzü destekleyen aynı cüzdan + RPC
  orkestrasyon mantığını yeniden kullanın.
- **Gerçek zamanlı panolar**: İzleyicileri (bakiyeler, hesaplar, imzalar) tercih
  ettiğiniz UI kütüphanesiyle birleştirin; istemci websocket dağıtımını ve
  önbellek geçersizleştirmeyi yönetir.
- **Özel store'lar**: IndexedDB/localStorage'dan hidrasyon sağlamak, durumu
  sunucu oturumlarına yansıtmak veya tarayıcı sekmeleri arasında koordinasyon
  için kendi Zustand store'unuzu enjekte edin.
- **React hook'larına köprü**: Aynı çalışma zamanının üzerinde ergonomik
  ...
  `@solana/react-hooks`'e geçirin.
- **Test edilebilirlik**: Tek istemci arayüzü birim testlerinde taklit
  edilebilir; bu sayede tarayıcı cüzdanı olmadan RPC yanıtlarını veya cüzdan
  oturumlarını simüle etmek kolaylaşır.
