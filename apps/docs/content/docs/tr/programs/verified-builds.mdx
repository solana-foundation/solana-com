...
kendiniz çalıştırabilir veya programın dağıtım yetkisinden ve
...
kaydedilen zincir üzerindeki doğrulama verilerini kullanarak `verify-from-repo`
komutuyla program doğrulamasını yerel olarak kendiniz çalıştırabilirsiniz.
...
saklanan doğrulama verilerine kolay erişim için doğrulayıcı public key'i
tutacaktır. Bir programı derlemek ve doğrulamak için gereken tüm bilgileri
içeren PDA, programın adresinden ve doğrulayıcı pubkey'inden türetilir.
Varsayılan olarak bu, programı derleyen ve dağıtan pubkey ile aynıdır. Ancak
`security.txt`'de belirtilebilecek başka bir pubkey de olabilir.
...
araştırmacılarının yalnızca sözleşmenin adresini bilseler bile proje sahiplerine
ulaşmaları için standartlaştırılmış bir yol sağlar.

## Neden security.txt kullanmalı?

Birçok proje için, özellikle daha küçük veya özel projeler için, geliştiricileri
sadece kontrat adresinden belirlemek zor ve zaman alıcı olabilir. Program içine
...
zamanında hata raporlarını garanti eder.
...
- Doğrulama: Dağıtmadan önce, hem onchain programları hem de yerel ikili
  dosyaları doğrulayabilen `query-security-txt` aracını kullanarak formatı ve
  içeriği doğrulayın:
