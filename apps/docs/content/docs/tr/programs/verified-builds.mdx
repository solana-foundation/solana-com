...
imajı yükler ve konteyner içindeki çalıştırılabilir dosya yolunun hash
değerinin, komuta sağlanan zincir üstündeki programın hash değeriyle aynı
olduğunu doğrular. Yapı zaten bir imaja yüklendiği için, uzun sürebilecek tam
bir yeniden yapılandırmaya gerek yoktur.
...
- **Zincir üzerinde PDA yok**: `solana-verify verify-from-repo -um` komutunu
  çalıştırın ve **sorulduğunda EVET'i seçin**. PDA'yı yüklemeden API doğrulama
  meta verilerinizi alamaz.
- **PDA veri uyuşmazlığı**: Programınızı yeniden dağıttıysanız PDA'nızı
  güncelleyin. PDA verileriniz dağıtılmış programınızla eşleşmelidir.
- **Yanlış commit hash'i**: PDA'nızı dağıttığınız tam commit hash'ini kullanarak
  oluşturun
- **Yapı ortamı farklılıkları**: PDA'nızı oluştururken solana-verify ile Docker
  kullanın

## Yerel yapı hash'im zincir üzerindeki hash ile eşleşmiyor. Neden?
...
- **Zincir Üstü Depolama**: Doğrulama meta verilerinizi (repo URL'si, commit
  hash'i, derleme parametreleri) Otter Verify programının sahip olduğu bir PDA
  içinde zincir üstünde saklayın (`verifycLy8mB96wd9wqq3WDXQwM4oU6r42Th37Db9fC`)
...
API yalnızca zincir üstü PDA'larla çalışır çünkü:

- **Güvensiz**: API rastgele verileri reddeder - yalnızca yükseltme yetkilinizin
  zincir üstünde sakladığı verileri kullanır
- **Daha Basit**: Sadece imzalayan + program_id sağlayın; API diğer her şeyi
  PDA'nızdan alır
- **Kurcalamaya Karşı Dayanıklı**: PDA'nız herkesin bağımsız olarak
  doğrulayabileceği değiştirilemez bir kayıt oluşturur
- **Sahiplik Kanıtı**: İmzalayanınız yükseltme yetkilisi olmalıdır ve bu,
  programı kontrol ettiğinizi kriptografik olarak kanıtlar
...
- **Zincir Üstü PDA**: Doğrulama meta verileriniz merkezi bir otorite tarafından
  kontrol edilmeden zincir üzerinde bulunur
- **Yükseltme Yetkisi Kanıtı**: Yalnızca programınızın yükseltme yetkisi PDA
...
1. PDA meta verileri otantiktir (zincir üzerinde saklanır)
2. PDA'nın deposundaki kaynak kod, dağıtılan programla eşleşir
3. API'ye güvenmenize gerek yok - her şeyi zincir üzerinde kendiniz doğrulayın
