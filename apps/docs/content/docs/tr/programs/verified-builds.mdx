...
> Ücretsiz RPC'lerin hız sınırlarına takılmamak için ücretli bir RPC URL'si
...
Bu komut, `Cargo.lock` dosyasında belirtilen solana sürümünü kullanan bir docker
container ile doğrulanabilir bir yapı oluşturacaktır.
...
Bu multisig kılavuzunun geri kalanında, örnek program ID'si olarak
...
İsteğe bağlı: Önce yerel olarak doğrulayıp doğrulayamayacağınızı kontrol edin
(bu komut örnek program ID'si `6XBGfP17P3KQAKoJb2s5M5fR4aFTXzPeuC1af2GYkvhD`
kullanır):
...
Program yetkiniz yerel olarak mevcut olduğunda, `solana-verify verify-from-repo`
komutunu kullanırken yapı verilerini zincir üzerine yüklemeniz için bir istem
alırsınız.
...
Bu size base58 formatında bir işlem döndürecektir. Bir işlem denetleyicisinde
kullanmak üzere base64 kodlu bir işlem istiyorsanız, `--encoding base64`
kullanabilirsiniz.
...
Bu komut, `ellipsislabs/hello_world_verifiable_build:latest` konumunda saklanan
imajı yükler ve konteyner içindeki yürütülebilir dosya yolunun hash değerinin,
komuta sağlanan onchain programın hash değeriyle aynı olduğunu doğrular. Derleme
zaten bir imaja yüklendiği için, uzun zaman alabilen yürütülebilir dosyanın tam
bir yeniden derlemesine gerek yoktur.
...
İşte bu [depodan](https://github.com/solana-developers/verified-program) kaynak
kodu kullanarak `FWEYpBAf9WsemQiNbAewhyESfR38GBBHLrCaU3MpEKWv` kimliğine sahip
örnek bir programı doğrulama örneği:

```bash
solana-verify verify-from-repo https://github.com/solana-developers/verified-program --url YOUR-RPC-URL --program-id FWEYpBAf9WsemQiNbAewhyESfR38GBBHLrCaU3MpEKWv --mount-path waffle --library-name waffle --commit-hash 5b82b86f02afbde330dff3e1847bed2d42069f4e
```

Varsayılan olarak `verify-from-repo` komutu main dalındaki son commit'i alır.
Depo üzerinde çalışmaya devam etmek istemeniz durumunda `commit-hash`
parametresini kullanarak belirli bir commit'i de tanımlayabilirsiniz:
...
> `library-name` parametresini belirtmemiz gerektiğine dikkat edin çünkü Squads
> deposu birden fazla program içerir. `--bpf` bayrağını kullanıyoruz çünkü
> `squads_mpl` daha önce Anchor ile doğrulanmıştı.
...
- **Yanlış imzalayan**: `solana program show YourProgramId` komutunu
  çalıştırarak imzalayanınızın programın yükseltme yetkisi olduğunu doğrulayın
- **Onchain PDA yok**: `solana-verify verify-from-repo -um` komutunu çalıştırın
  ve **sorulduğunda EVET'i seçin**. PDA yüklenmeden API, doğrulama meta
  verilerinizi alamaz.
...
- **Yanlış commit hash değeri**: PDA'nızı, dağıttığınız tam commit hash değerini
  kullanarak oluşturun
- **Derleme ortamı farklılıkları**: PDA'nızı oluştururken solana-verify ile
  Docker kullanın

## Yerel derleme hash değerim neden onchain hash değeriyle eşleşmiyor?

Bu genellikle şu anlama gelir:

- Farklı Rust/Solana araç zinciri sürümleri kullanıyorsunuz
- Bağımlılıklarınız yapılar arasında güncellendi
- Docker konteynerinde yapı oluşturmadınız
- Yanlış commit'i kontrol ettiniz

Bunu, dağıttığınız tam commit'i kullanarak Docker'da `solana-verify build` ile
yapı oluşturarak düzeltin.

## Doğrulamam ne kadar sürecek?
...
Programınızın yükseltme yetkisi yoksa veya bir PDA oluşturmadan önce
değiştirilemez hale getirildiyse, bu durum için beyaz listeye alınmış bir
adresimiz var. contact@osec.io adresinden bizimle iletişime geçin, programınızı
doğrulamanıza yardımcı olalım.

## PDA nedir ve neden önemlidir?

PDA'nız (Program Türevli Hesap) güvensiz doğrulamayı mümkün kılar:

- **Onchain Depolama**: Doğrulama meta verilerinizi (repo URL'si, commit hash
  değeri, derleme parametreleri) Otter Verify programına
  (`verifycLy8mB96wd9wqq3WDXQwM4oU6r42Th37Db9fC`) ait bir PDA'da onchain olarak
  depolayın
- **Kriptografik Bağlantı**: PDA'nız program adresinizden türetilir ve doğrulama
  verilerinize değiştirilemez bir bağlantı oluşturur
- **Merkeziyetsiz Güven**: Herkes PDA'nızı okuyabilir ve programınızı bağımsız
  olarak doğrulayabilir

## Neden API'yi kullanmadan önce PDA'yı oluşturmalıyım?

API yalnızca onchain PDA'larla çalışır çünkü:

- **Güvensiz**: API rastgele verileri reddeder - yalnızca yükseltme yetkinizin
  onchain olarak depoladığı verileri kullanır
- **Daha basit**: Sadece imzalayan + program_id sağlayın; API diğer her şeyi
  PDA'nızdan alır
- **Kurcalamaya dayanıklı**: PDA'nız, herkesin bağımsız olarak doğrulayabileceği
  değiştirilemez bir kayıt oluşturur
- **Sahiplik kanıtı**: İmzalayanınız yükseltme yetkisi olmalıdır ve bu, programı
  kontrol ettiğinizi kriptografik olarak kanıtlar
...
- Bunun dışında yeniden doğrulama konusunda endişelenmeyin - API tüm programları
  her 24 saatte bir otomatik olarak yeniden doğrular

## Programımı yükselttiğimde ne olur?

Yükseltme sonrasında şu adımları izleyin:

1. API yükseltmenizi algılar ve programınızın doğrulamasını kaldırır.
...
4. API yeni sürümünüzü güncellenmiş PDA'ya karşı doğrulayacaktır

**Önemli**: PDA'nızı her zaman yükseltme yetkinizi kullanarak, yükseltilmiş
program için yeni commit hash ile güncelleyin.

## Doğrulama sonuçlarına güvenebilir miyim?

Evet - sistem güven gerektirmeyecek şekilde tasarlanmıştır ve bağımsız olarak
doğrulanabilir:

**Güvenilir Kılan Unsurlar:**

- **Zincir Üstü PDA**: Doğrulama meta verileriniz merkezî bir otorite tarafından
  kontrol edilmeden zincir üstünde yaşar
- **Yükseltme Yetkisi Kanıtı**: Yalnızca programınızın yükseltme yetkisi PDA'yı
  oluşturabilir/güncelleyebilir
- **Bağımsız Doğrulama**: Herkes PDA'nızı okuyarak ve `solana-verify` komutunu
  yerel olarak çalıştırarak doğrulayabilir
- **Sürekli Yeniden Doğrulama**: API, tüm programları her 24 saatte bir otomatik
  olarak yeniden doğrular

**Bu Sınırlamaları Anlayın:**

- Doğrulama, kaynağın dağıtımla eşleştiğini onaylar - kodunuzun güvenli olduğunu
  **DEĞİL**
- Programlarla etkileşime geçmeden önce her zaman kodu inceleyin
- Doğrulanmış ≠ denetlenmiş veya güvenli
- Güvenilir bir kaynaktan geldiğini doğrulamak için PDA'daki depo ve commit'i
  kontrol edin

## Bir programı bağımsız olarak nasıl doğrulayabilirim?

Zincir üstü PDA'sını okuyarak ve doğrulamayı yerel olarak çalıştırarak herhangi
bir programı kendiniz doğrulayın:

**Adım 1: Zincir Üstü PDA'yı Okuyun**
...
**Adım 2: Yerel Olarak Doğrulayın**
...
1. PDA meta verileri otantiktir (zincir üstünde saklanır)
2. PDA'nın deposundaki kaynak kodu, dağıtılmış programla eşleşir
3. API'ye güvenmenize gerek yoktur - her şeyi zincir üstünde kendiniz doğrulayın

## Başka biri izinsiz olarak programımı doğrulayabilir mi?

Evet, bu nedenle imzalayanın yükseltme yetkisi olmasını zorunlu kılıyoruz.
Doğrulamayı yalnızca imzalayan yükseltme yetkisi ise geçerli sayarız.

## Doğrulanabilir derlemeler oluşturmak için neye ihtiyacım var?
...
Hayır - doğrulama, genel kaynak kodunu gerektirir:

- PDA'nız, herkesin erişebileceği genel bir depo URL'si saklar
- Güvensiz doğrulama, genel kod erişimine bağlıdır
- Kullanıcıların programınızın ne yaptığını anlamak için kaynak kodunuzu okuması
  gerekir
- Tüm amaç, kullanıcıların kaynağın dağıtımla eşleştiğini bağımsız olarak
  doğrulamasına izin vermektir
...
Multisig kontrolündeki programlar için şu adımları izleyin:
...
**Kritik**: PDA işlemini dışa aktarmadan önce yapı hash'inin eşleştiğini
doğrulamak için her zaman yerel olarak doğrulayın (adım 2).

# Sonuç

[Solana'da doğrulanmış yapılar](/developers/guides/advanced/verified-builds)
kullanmak, programlarınızın ağ üzerindeki bütünlüğünü ve güvenilirliğini sağlar
ve geliştiricilerin SDK'larınızı doğrudan Solana Explorer'dan bulmalarına olanak
tanır. Solana Verify CLI ve Docker gibi araçlardan yararlanarak, kaynak
kodunuzla uyumlu, doğrulanabilir ve güvenli yapılar oluşturabilirsiniz. Her
zaman tutarlı ortamlar kullanmak için gerekli önlemleri alın ve güvenli
yükseltmeler ve dağıtımlar için yönetişim çözümlerini değerlendirin.
...
Solana Foundation tarafından oluşturulur ve barındırılır.

Projenizi indirilen bir docker imajında oluşturduğunuzu ve potansiyel olarak
hassas bilgiler de dahil olmak üzere tüm kurulumunuzun yapı için bu docker
...
imajlarının kurcalanmadığından emin olabilirsiniz. Kendi docker imajlarınızı
oluşturmak için gerekli betikleri
[Doğrulanmış yapılar deposunda](https://github.com/Ellipsis-Labs/solana-verifiable-build)
bulabilir ve bunu forklayıp github eylemlerini kendiniz çalıştırabilir veya
doğru olduklarını doğrulayabilirsiniz.
...
platformuna belirli ölçüde güvenmektesiniz.

API veya Solana Explorer, tehlikeye girdiğinde potansiyel olarak yanlış bilgi
görüntüleyebilir.
...
kullanarak programın dağıtım yetkisinden ve
[verify programından](https://explorer.solana.com/address/verifycLy8mB96wd9wqq3WDXQwM4oU6r42Th37Db9fC)
türetilen bir
[PDA](https://explorer.solana.com/address/63XDCHrwZu3mXsw2RUFb4tbNpChuUHx4eA5aJMnHkpQQ/anchor-account)
içinde kaydedilen zincir üstü doğrulama verilerini kullanarak kendiniz yerel
olarak çalıştırabilirsiniz.

Doğrulama programı [OtterSec ekibi](https://osec.io/) tarafından dağıtılmıştır
ve henüz dondurulmamıştır, bu nedenle istediğiniz zaman yükseltilebilir.

Solana Foundation, OtterSec ve Ellipsis Labs ekibi, doğrulanmış yapılar hattının
...
saklanan doğrulama verilerine kolay erişim için doğrulayıcı genel anahtarını
tutacaktır. Bir programı oluşturmak ve doğrulamak için gereken tüm bilgileri
içeren PDA, programın adresinden ve doğrulayıcı pubkey'den türetilir. Varsayılan
...
doğrudan Solana akıllı sözleşmelerine gömmelerini sağlar.
[securitytxt.org](https://securitytxt.org)'dan ilham alan bu yaklaşım, güvenlik
araştırmacılarının yalnızca sözleşmenin adresini bilseler bile proje
yürütücülerine ulaşmaları için standardize bir yol sağlar.

## Neden security.txt kullanmalı?

Birçok proje için, özellikle daha küçük veya özel olanlar için, sadece kontrat
adresinden geliştiricileri belirlemek zor ve zaman alıcı olabilir. Program içine
bir `security.txt` dosyası gömmek, güvenlik araştırmacılarının doğru kişilerle
kolayca iletişim kurabilmesini sağlar, potansiyel olarak istismarları önler ve
zamanında hata raporları alınmasını garanti eder.
...
Güvenlik bilgilerinizi tanımlamak için kontratınızda `security_txt!` makrosunu
kullanın. İletişim bilgilerini, proje URL'lerini ve hatta bir güvenlik
...
`security.txt` bilgisi programınıza gömüldükten sonra, Solana Explorer gibi
araçlar aracılığıyla kolayca sorgulanabilir, böylece iletişim ve güvenlik
bilgileriniz potansiyel sorunları bildirmek isteyen herkes tarafından
erişilebilir olur.

## En iyi uygulamalar

- Bağlantı Kullanın: Değişme olasılığı yüksek bilgiler için (örn. iletişim
  bilgileri), bunları kontrata sabit kodlamak yerine bir web sayfasına bağlantı
  vermeniz önerilir. Bu, sık program güncellemelerine olan ihtiyacı ortadan
  kaldırır.

- Doğrulama: Dağıtmadan önce, `query-security-txt` aracını kullanarak formatı ve
  içeriği doğrulayın; bu araç hem zincir üstü programları hem de yerel ikili
...
Güvenlik iletişim bilgilerini doğrudan kontratınıza gömerek, araştırmacıların
