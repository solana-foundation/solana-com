...
Solana blok zincirinde program geliştirmenin bazı doğal sınırlamaları vardır.
...
Onchain Rust programları Rust'ın libstd, libcore ve liballoc kütüphanelerinin
çoğunu ve ayrıca birçok üçüncü taraf crate'ini destekler.
...
- Sınırlı erişim:
  - `std::hash`
  - `std::os`
- Bincode hem döngü hem de çağrı derinliği açısından son derece hesaplama
  maliyetlidir ve kullanılmamalıdır
- String biçimlendirme de hesaplama açısından maliyetli olduğundan
  kaçınılmalıdır.
- `println!`, `print!` desteği yok, bunun yerine
  ...
  makrosunu kullanın.
- Runtime, bir programın tek bir instruction işleme sırasında yürütebileceği
  instruction sayısına bir sınır koyar. Daha fazla bilgi için
...
Bir program izin verilen çağrı yığını derinliği sınırını aştığında,
`CallDepthExceeded` hatası alır.

## CPI çağrı derinliği - `CallDepth` hatası

Cross-program invocation'lar programların diğer programları doğrudan çağırmasına
izin verir, ancak derinlik şu anda `4` ile sınırlıdır.

Bir program izin verilen
[cross-program invocation çağrı derinliğini](/docs/core/cpi) aştığında,
`CallDepth` hatası alır
