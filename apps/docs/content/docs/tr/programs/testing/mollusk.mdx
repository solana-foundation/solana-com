...
Hesaplama birimi kullanımını kıyaslamak için `mollusk-svm-bencher` bağımlılığını
...
Associated Token Program kullanmak için `mollusk-svm-programs-token`
...
1. [Bir `Mollusk` örneği oluşturun](mention:mollusk) - Mollusk'u bir program
   ID'si ve derlenmiş programa giden yol (`.so` dosyası) ile başlatın
...
Testi çalıştırmak için `cargo test` komutunu kullanın.
...
Mollusk'u belirli bir program ile başlatmak için `Mollusk::new` metodunu
kullanın.
...
<Callout type="info">
  Dosya yolunu belirtirken `.so` uzantısını eklemeyin. Örneğin,
...
| Yöntem                                                                         | Açıklama                                                                                                                         |
| ------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| [`process_instruction`](#single-instruction)                                   | Bir talimatı işle ve sonucu döndür.                                                                                              |
| [`process_and_validate_instruction`](#single-instruction-with-checks)          | Bir talimatı işle ve sonuç üzerinde bir dizi kontrol gerçekleştir, herhangi bir kontrol başarısız olursa panic yap.              |
| [`process_instruction_chain`](#multiple-instructions)                          | Birden fazla talimatı işle ve sonucu döndür.                                                                                     |
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | Birden fazla talimatı işle ve her sonuç üzerinde bir dizi kontrol gerçekleştir, herhangi bir kontrol başarısız olursa panic yap. |

[`InstructionResult`](https://docs.rs/mollusk-svm-result/0.5.0/mollusk_svm_result/types/struct.InstructionResult.html),
işlenmiş bir talimatın detaylarını içerir.
...
Pratikte, genellikle Mollusk'u `#[test]` niteliği ile işaretlenmiş bir test
...
Doğrulama kontrolleri ile tek bir talimatı işlemek için
`process_and_validate_instruction` yöntemini kullanın. Bu yöntem, herhangi bir
kontrol başarısız olursa panic yapacaktır.
...
Her talimatın ardından doğrulama kontrolleri ile birden fazla talimatı işlemek
...
`Mollusk` etrafında, `account_store` aracılığıyla birden fazla talimat çağrısı
boyunca hesap durumunu koruyan bir sarmalayıcıdır. Talimatları işleme yöntemleri
`Mollusk` ile aynıdır.

`Mollusk`'dan farklı olarak, her metoda `accounts` parametresi geçirmeyi
gerektiren (örneğin `process_instruction`), `MolluskContext` hesapları dahili
olarak `account_store` üzerinden yönetir. Bu, talimatları işlerken `accounts`
parametresine ihtiyaç duyulmasını ortadan kaldırır.
...
Aşağıdaki örnek, `account_store` aracılığıyla talimatlar arasında kalıcı hesap
...
Belirli bir slot'a ileri veya geri giderek zamanı simüle etmek için sysvar
clock'u güncellemek üzere `warp_to_slot` metodunu kullanın.
...
Benchmark sonuçları, belirtilen `out_dir` konumuna `compute_units.md` adlı bir
...
<Callout type="info">
  ...
  `mollusk-svm-programs-token`, mint ve token hesaplarını oluşturmak için
