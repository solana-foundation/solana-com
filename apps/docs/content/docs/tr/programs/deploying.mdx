...
- **Program ID**: Programınızın zincir üzerindeki adresi. Kullanıcılar, Program
  ID'yi referans alarak programınızla etkileşime girer.
- **program account**: Programınızın meta verilerini depolayan zincir üzerindeki
  hesap. program account adresi, Program ID'dir. program account ayrıca
  programdata account adresini de saklar.
- **programdata account**: Programınızın dağıtılmış çalıştırılabilir kodunu
  depolayan zincir üzerindeki hesap.
- **Program Authority**: Programı güncelleme veya kapatma iznine sahip olan
  hesap. Varsayılan olarak bu, CLI cüzdanınızdır.
...
Program ID, program account hesabınızın zincir üzerindeki adresidir.
Kullanıcılar ve diğer programlar, işlemlerinde bu ID'yi referans alarak
programınızı çağırır.
...
- **`--use-rpc`**: İşlemleri yapılandırılmış RPC'ye gönderin. Bu bayrak,
  ...
  sağlayıcılardan [stake-weighted](/docs/defi/stake-weighted-qos) RPC bağlantısı
  gerektirir.
