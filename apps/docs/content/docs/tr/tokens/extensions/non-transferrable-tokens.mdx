---
title: Transfer Edilemeyen Token'lar
description:
  Token'ları basılabilen, yakılabilen ve kapatılabilen ancak token hesapları
  arasında transfer edilemeyen bir mint oluşturun.
...
Token Extensions Program'ın _rs`NonTransferable`_ mint uzantısı, o mint için her
token account'u transfer edilemez hale getirir. Token'lar basıldıktan sonra,
token sahipleri bunları _rs`Transfer`_ veya _rs`TransferChecked`_ ile başka bir
...
<Callout type="info" title="Token Account Uzantıları">
  _rs`NonTransferable`_ içeren bir mint için bir token account başlatıldığında,
  token account _rs`NonTransferableAccount`_ ve _rs`ImmutableOwner`_ ile
...
1. Mint hesap boyutunu ve mint için gerekli rent ile _rs`NonTransferable`_
   uzantısını hesaplayın.
2. Mint hesabını _rs`CreateAccount`_ ile oluşturun, _rs`NonTransferable`_'ı
   başlatın ve mint'i _rs`InitializeMint`_ ile başlatın.
3. Mint için token hesapları oluşturun. _rs`NonTransferableAccount`_ ve
   _rs`ImmutableOwner`_, token hesapları için otomatik olarak etkinleştirilir.
4. _rs`Transfer`_ ve _rs`TransferChecked`_, _rs`TokenError::NonTransferable`_
   ile başarısız olur.
...
Temel mint için mint hesap boyutunu artı _rs`NonTransferable`_ uzantısını
...
## !!steps Rent hesaplama

Mint için gerekli boyut ve _rs`NonTransferable`_ uzantısını kullanarak rent
hesaplayın.
...
## !!steps NonTransferable başlatma

Mint üzerinde _rs`NonTransferable`_ uzantısını başlatın.
...
<Callout type="info" title="Talimat Sırası">
  _rs`InitializeNonTransferableMint`_, _rs`InitializeMint`_ öncesinde
  gelmelidir. _rs`CreateAccount`_, _rs`InitializeNonTransferableMint`_ ve
...
| Öğe                                            | Açıklama                                                                                                                                                                                                                                        | Kaynak                                                                                                                            |
| ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| _rs`NonTransferable`_                          | Mint'ten gelen token'ları transfer edilemez olarak işaretleyen mint uzantısı.                                                                                                                                                                   | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/non_transferable.rs#L8-L13)  |
| _rs`NonTransferableAccount`_                   | Transfer edilemez mint'ler için token hesaplarına eklenen token hesabı uzantısı.                                                                                                                                                                | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/non_transferable.rs#L15-L21) |
| _rs`ImmutableOwner`_                           | Sahiplik değişikliklerini engelleyen ve transfer edilemez token hesapları için gerekli olan token hesabı uzantısı.                                                                                                                              | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L8-L16)   |
| _rs`InitializeNonTransferableMint`_            | _rs`InitializeMint`_ öncesinde mint seviyesinde transfer edilemez uzantısını başlatan talimat.                                                                                                                                                  | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L642-L653)              |
| _rs`process_initialize_non_transferable_mint`_ | Başlatılmamış bir mint üzerinde _rs`NonTransferable`_ mint uzantısını başlatan işlemci mantığı.                                                                                                                                                 | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1646-L1655)                |
| _rs`get_required_init_account_extensions`_     | Mint üzerinde etkinleştirilen uzantılara göre token hesapları başlatıldığında otomatik olarak token hesabı uzantıları eklemek için kullanılır. _rs`NonTransferable`_ mint'leri için _rs`NonTransferableAccount`_ ve _rs`ImmutableOwner`_ ekler. | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mod.rs#L1304-L1307)          |
