...
description:
  Herhangi bir hesaptan token'ları transfer edebilen veya yakabilen kalıcı bir
  delege ile bir Token Extension Program mint'i oluşturun.
...
Token Extensions Program'ın _rs`PermanentDelegate`_ mint uzantısı, mint'in
kendisine bir delege yetkisi atar.

Kalıcı delege, o mint için herhangi bir token hesabı için transfer ve yakma
işlemlerini yetkilendirebilen bir mint düzeyinde yetkidir. Mint için tüm token
hesapları için küresel bir delege görevi görür ve token hesap sahipleri, kalıcı
delegeyi kendi token hesaplarından geri alamazlar.

Mint yetkisi ayrıca _rs`AuthorityType::PermanentDelegate`_ kullanarak
_rs`SetAuthority`_ ile kalıcı delegeyi daha sonra değiştirebilir.

## Kalıcı Delege Nasıl Oluşturulur ve Kullanılır

Kalıcı delege oluşturmak ve kullanmak için:

1. Mint hesabı boyutunu ve mint ile _rs`PermanentDelegate`_ uzantısı için
   gereken rent'i hesaplayın.
2. _rs`CreateAccount`_ ile mint hesabını oluşturun, _rs`PermanentDelegate`_'i
   başlatın ve _rs`InitializeMint`_ ile mint'i başlatın.
3. Kalıcı delege tarafından imzalanmış _rs`TransferChecked`_ veya
   _rs`BurnChecked`_ gönderiniz. Token program bu talimatları, token hesabı
   sahibi tarafından imzalanmış gibi aynı şekilde işler.

<ScrollyCoding>

## !!steps Hesap boyutunu hesaplayın

Temel mint artı _rs`PermanentDelegate`_ uzantısı için mint hesabı boyutunu
hesaplayın. Bu, _rs`CreateAccount`_'de kullanılan boyuttur.
...
Mint artı _rs`PermanentDelegate`_ uzantısı için gereken boyutu kullanarak rent'i
hesaplayın.
...
## !!steps PermanentDelegate'i Başlatma

Mint üzerinde _rs`PermanentDelegate`_ uzantısını başlatın.
...
## !!steps Token account oluşturma ve token basma

Sahip ve alıcı için token account oluşturun, ardından sahip token account'una
token basın.
...
## !!steps Kalıcı delege ile transfer

Kalıcı delege tarafından imzalanan _rs`TransferChecked`_ ile token transfer
edin.
...
<Callout type="info" title="Komut Sırası">
  _rs`InitializePermanentDelegate`_, _rs`InitializeMint`_'den önce gelmelidir.
  _rs`CreateAccount`_, _rs`InitializePermanentDelegate`_ ve _rs`InitializeMint`_
...
| Öğe                                         | Açıklama                                                                                                       | Kaynak                                                                                                                              |
| ------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermanentDelegate`_                     | Mint için her token account genelinde yetkilendirilmiş delegeyi saklayan mint uzantısı.                        | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/permanent_delegate.rs#L15-L21) |
| _rs`InitializePermanentDelegate`_           | _rs`InitializeMint`_'den önce kalıcı delegeyi başlatan komut.                                                  | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1952-L1967)              |
| _rs`SetAuthority`_                          | Delegeyi döndürmek için _rs`AuthorityType::PermanentDelegate`_ ile kullanılan temel token komutu.              | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1488-L1520)              |
| _rs`AuthorityType::PermanentDelegate`_      | Bir mint üzerindeki kalıcı delegeyi döndürmek için _rs`SetAuthority`_ ile kullanılan yetki ayırıcısı.          | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1138-L1139)              |
| _rs`process_initialize_permanent_delegate`_ | Kalıcı delege uzantısını mint üzerine yazan işlemci mantığı.                                                   | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1542-L1557)                  |
| _rs`process_set_authority`_                 | _rs`AuthorityType::PermanentDelegate`_ kullanıldığında kalıcı delegeyi doğrulayan ve döndüren işlemci mantığı. | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L849-L860)                    |
