...
<Callout type="warn" title="Kullanılabilirlik">
  İzinli yakma özelliği Token-2022
  [`program@v11.0.0`](https://github.com/solana-program/token-2022/releases/tag/program%40v11.0.0)
  sürümüyle birlikte gelmiştir. Bugün devnet üzerinde kullanılabilir olup
  mainnet'te Haziran 2026'da etkinleştirilmesi planlanmaktadır. Token Extension
  Program her kümeye ayrı olarak dağıtıldığından, hedeflediğiniz kümedeki
  dağıtımın v11.0.0 veya daha yeni bir sürümü içerdiğini doğrulayın.
</Callout>

Yerel bir test validator'ı aynı zamanda daha eski bir Token-2022 derlemesi
içerir; bu nedenle bu sayfadaki örnekleri çalıştırmak için devnet'teki v11.0.0
...
2. _rs`CreateAccount`_ ile mint account'ı oluşturun,
   _rs`PermissionedBurnConfig`_ uzantısını başlatın ve _rs`InitializeMint`_ ile
   mint'i başlatın.
3. Bir token account oluşturun ve token'ları mint edin.
4. Hem token account sahibi hem de yakma yetkisi tarafından imzalanan
   _rs`PermissionedBurnInstruction::BurnChecked`_ ile yakın.
5. İsteğe bağlı olarak, _rs`AuthorityType::PermissionedBurn`_ ve bir `None`
   yetkisi kullanarak _rs`SetAuthority`_ ile izinli yakmayı devre dışı
   bırakabilirsiniz; bu işlem standart yakma talimatlarını yeniden
   etkinleştirir. Aşağıdaki tam kod örnekleri bu adımı göstermektedir.
...
Mint üzerinde _rs`PermissionedBurnConfig`_ uzantısını yakma yetkisiyle başlatın.
...
## !!steps Bir token account oluşturun ve token basın
...
## !!steps Burn authority ile yakın

_rs`PermissionedBurnInstruction::BurnChecked`_ ile token'ları, hem token account
sahibi hem de burn authority tarafından imzalanmış şekilde yakın.
...
<Callout type="info" title="Talimat Sırası">
  ...
  ve _rs`InitializeMint`_ aynı işlem içinde yer almalıdır.
</Callout>
...
| Öğe                                            | Açıklama                                                                                                                      | Kaynak                                                                                                                                         |
| ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | Mint için her yakma işlemini birlikte imzalamak üzere gereken yetkiyi saklayan mint uzantısı.                                 | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
...
| _rs`PermissionedBurnInstruction::Burn`_        | Burn authority ile token account sahibi veya delegesinden imza gerektiren yakma talimatı.                                     | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | _rs`PermissionedBurnInstruction::Burn`_ ile aynı hesapları kullanan, ondalık kontrolü olan yakma talimatı.                    | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | Bir mint üzerindeki burn authority'yi döndürmek veya devre dışı bırakmak için _rs`SetAuthority`_ ile kullanılan yetki ayrımı. | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | Başlatılmamış bir mint üzerinde _rs`PermissionedBurnConfig`_'i başlatan ve burn authority'yi saklayan işlemci mantığı.        | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
| _rs`process_burn`_                             | Burn authority ayarlandığında standart yakmaları reddeden ve yetkinin imzasını doğrulayan işlemci mantığı.                    | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | _rs`AuthorityType::PermissionedBurn`_ kullanıldığında burn authority'yi doğrulayan ve döndüren işlemci mantığı.               | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### Typescript

Aşağıdaki `Kit` örneği, oluşturulan talimatları doğrudan kullanır. Yayımlanan
eski `@solana/spl-token` paketi henüz izinli yakma desteği içermediğinden, eski
bir örnek dahil edilmemiştir.
