...
description:
  Token Extension Program token hesabını değiştirilemez sahiplik ile başlatarak
  sahibinin asla yeniden atanamayacağından emin olun.
...
## Değiştirilemez Sahip Nedir?

Token Extension Program'ın _rs`ImmutableOwner`_ hesap uzantısı, bir token
hesabının sahibini kalıcı hale getirir.

Bir token hesabı _rs`ImmutableOwner`_ ile başlatıldığında, sonraki
_rs`SetAuthority`_ talimatları onun _rs`AccountOwner`_ yetkisini değiştiremez.

<Callout type="info" title="İlişkili Token Hesapları">
  Token Extension Program için Associated Token Program aracılığıyla oluşturulan
  associated token account'lar, token hesabında _rs`ImmutableOwner`_ özelliğini
  otomatik olarak başlatır.
</Callout>

## Değiştirilemez Sahip Hesabı Nasıl Başlatılır

Değiştirilemez sahip hesabını başlatmak için:

1. Bir mint oluşturun ve başlatın.
2. _rs`ImmutableOwner`_ için yeterli alana sahip bir token hesabı oluşturun.
3. _rs`InitializeAccount`_ öncesinde token hesabında _rs`ImmutableOwner`_
   özelliğini başlatın.
4. Daha sonra _rs`SetAuthority`_ ile hesap sahibini değiştirme girişimi
   _rs`TokenError::ImmutableOwner`_ hatasıyla başarısız olur.

<ScrollyCoding>

## !!steps Token hesap boyutunu hesaplayın

Temel token hesabı artı _rs`ImmutableOwner`_ uzantısı için token hesap boyutunu
...
_rs`ImmutableOwner`_ uzantısı için gereken token hesap boyutunu kullanarak
rent'i hesaplayın.
...
## !!steps Token hesabını oluşturun

Hesaplanan alan ve lamport ile token hesabını oluşturun.
...
## !!steps ImmutableOwner'ı Başlat

Token hesabında _rs`ImmutableOwner`_ uzantısını başlatın.
...
## !!steps Token hesabını başlat

Token hesabını aynı işlemde _rs`InitializeAccount`_ ile başlatın.
...
<Callout type="info" title="Talimat Sırası">
  _rs`InitializeImmutableOwner`_, _rs`InitializeAccount`_ öncesinde gelmelidir.
  _rs`CreateAccount`_, _rs`InitializeImmutableOwner`_ ve _rs`InitializeAccount`_
...
| Öğe                                      | Açıklama                                                                                                   | Kaynak                                                                                                                           |
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| _rs`ImmutableOwner`_                     | Token hesabı sahibini değiştirilemez olarak işaretleyen hesap uzantısı.                                    | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L13-L16) |
| _rs`InitializeImmutableOwner`_           | Başlatılmamış bir token hesabında değiştirilemez-sahip uzantısını başlatan talimat.                        | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1982-L1991)           |
| _rs`process_initialize_immutable_owner`_ | Hesapta _rs`ImmutableOwner`_ alanını tahsis eden ve başlatan işlemci mantığı.                              | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1532-L1543)               |
| _rs`SetAuthority`_                       | Token hesabı sahibi de dahil olmak üzere yetkileri değiştirmek için kullanılan temel token talimatı.       | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1611-L1633)           |
| _rs`process_set_authority`_              | Token hesabında _rs`ImmutableOwner`_ etkinleştirildiğinde sahip değişikliklerini reddeden işlemci mantığı. | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L703-L900)                 |
