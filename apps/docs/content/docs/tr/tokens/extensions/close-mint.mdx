...
title: Mint'i Kapat
description:
  MintCloseAuthority uzantısıyla bir Token Extensions Program mint'i oluşturun,
  ardından kalan arzı yakın ve mint'i kapatın.
...
Token Extensions Program'ın _rs`MintCloseAuthority`_ mint uzantısı, belirlenen
...
- Toplam arz sıfır olmalıdır, bu nedenle bir mint kapatılmadan önce tüm token
  account'lardaki tüm token'lar yakılmalıdır
...
1. Mint hesabı boyutunu ve mint ile _rs`MintCloseAuthority`_ uzantısı için
   gerekli rent'i hesaplayın.
2. _rs`CreateAccount`_ ile mint hesabını oluşturun, _rs`MintCloseAuthority`_'i
   başlatın ve mint'i _rs`InitializeMint`_ ile başlatın.
3. Toplam arz sıfır ise, mint account kapatma yetkisi tarafından imzalanarak
   _rs`CloseAccount`_ ile kapatılabilir.

<ScrollyCoding>

## !!steps Hesap boyutunu hesaplayın

Temel mint artı _rs`MintCloseAuthority`_ uzantısı için mint account boyutunu
hesaplayın. Bu, _rs`CreateAccount`_'da kullanılan boyuttur.
...
Mint artı _rs`MintCloseAuthority`_ uzantısı için gereken boyutu kullanarak
rent'i hesaplayın.
...
## !!steps MintCloseAuthority'yi Başlatma

Mint üzerinde _rs`MintCloseAuthority`_ uzantısını başlatın.
...
<Callout type="info" title="Talimat Sırası">
  _rs`InitializeMintCloseAuthority`_, _rs`InitializeMint`_'den önce gelmelidir.
  _rs`CreateAccount`_, _rs`InitializeMintCloseAuthority`_ ve
...
| Öğe                                           | Açıklama                                                                                                | Kaynak                                                                                                                                |
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`MintCloseAuthority`_                      | Arzı sıfıra ulaştığında bir mint'i kapatmasına izin verilen yetkiyi saklayan mint uzantısı.             | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mint_close_authority.rs#L14-L20) |
| _rs`InitializeMintCloseAuthority`_            | _rs`InitializeMint`_'den önce mint kapatma yetkisini başlatan talimat.                                  | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1830-L1843)                |
| _rs`CloseAccount`_                            | Toplam arz sıfıra ulaştıktan sonra mint hesabını kapatan temel token talimatı.                          | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1582-L1608)                |
| _rs`process_initialize_mint_close_authority`_ | _rs`MintCloseAuthority`_ uzantısını başlatılmamış bir mint üzerine yazan işlemci mantığı.               | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1376-L1389)                    |
| _rs`process_close_account`_                   | Kapatma yetkisine sahip mint'ler için sıfır arz kuralını uygulayan paylaşılan hesap kapatma işleyicisi. | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1288)                    |
