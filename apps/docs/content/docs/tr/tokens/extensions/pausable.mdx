---
title: Duraklatılabilir basım
description:
  Duraklatma yetkisine sahip bir Token-2022 mint oluşturun, mint
  duraklatıldığında transferleri engelleyin ve etkinliği daha sonra yeniden
  başlatın.
...
## Duraklatılabilir Mint Nedir?

Token Extensions Program'ın _rs`PausableConfig`_ mint uzantısı, duraklatma
yetkisine tüm mint için token etkinliğini duraklatma ve devam ettirme yeteneği
verir.

Bir mint duraklatıldığında, Token Extensions Program şunları reddeder:

- Transferler
- Mint işlemleri
- Yakma işlemleri

Mint yeniden başlatıldığında, bu talimatlar tekrar normal şekilde çalışır.

## Bir Mint Nasıl Oluşturulur, Duraklatılır ve Devam Ettirilir

Bir mint oluşturmak, duraklatmak ve devam ettirmek için:

1. Mint hesabının boyutunu ve mint ile _rs`PausableConfig`_ uzantısı için
   gereken rent'i hesaplayın.
2. _rs`CreateAccount`_ ile mint hesabını oluşturun, _rs`PausableConfig`_'i
   başlatın ve _rs`InitializeMint`_ ile mint'i başlatın.
3. Token'ları mint edin ve mint için token hesapları oluşturun.
4. Mint için token etkinliğini engellemek üzere _rs`Pause`_ kullanın.
5. Token etkinliğini tekrar izin vermek için _rs`Resume`_ kullanın.

<ScrollyCoding>

## !!steps Hesap boyutunu hesaplayın

Temel mint artı _rs`PausableConfig`_ uzantısı için mint hesap boyutunu
...
Mint artı _rs`PausableConfig`_ uzantısı için gereken boyutu kullanarak rent'i
hesaplayın.
...
## !!steps PausableConfig'i Başlatın

Mint üzerinde _rs`PausableConfig`_ uzantısını başlatın.
...
## !!steps Mint'i duraklatın

Mint'i _rs`Pause`_ ile duraklatın.
...
## !!steps Mint'i devam ettirin

Mint'i _rs`Resume`_ ile devam ettirin.
...
<Callout type="info" title="Talimat Sırası">
  _rs`PausableInstruction::Initialize`_, _rs`InitializeMint`_ öncesinde
  gelmelidir. _rs`CreateAccount`_, _rs`PausableInstruction::Initialize`_ ve
...
| Öğe                                   | Açıklama                                                                                                      | Kaynak                                                                                                                                |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PausableConfig`_                  | Duraklat yetkisini ve mint'in şu anda duraklatılıp duraklatılmadığını saklayan mint uzantısı.                 | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/mod.rs#L17-L22)         |
| _rs`PausableInstruction::Initialize`_ | _rs`InitializeMint`_ öncesinde duraklatılabilir yapılandırmayı başlatan talimat.                              | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L20-L32) |
| _rs`PausableInstruction::Pause`_      | Mint'i duraklatılmış olarak işaretleyen talimat.                                                              | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L33-L44) |
| _rs`PausableInstruction::Resume`_     | Mint'i duraklatılmamış olarak işaretleyen talimat.                                                            | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L45-L56) |
| _rs`process_initialize`_              | Başlatılmamış bir mint üzerinde _rs`PausableConfig`_ başlatan ve duraklat yetkisini saklayan işlemci mantığı. | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L22-L35)     |
| _rs`process_toggle_pause`_            | Mint'in duraklatma bayrağını değiştirmeden önce duraklat yetkisini doğrulayan işlemci mantığı.                | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L39-L64)     |
