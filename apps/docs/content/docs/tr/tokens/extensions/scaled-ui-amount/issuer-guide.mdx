...
Token mint'te Ölçeklendirilmiş UI Miktar uzantısını etkinleştirmek için, `Mint`
hesabındaki `scaled_ui_amount_extension` alanını `true` olarak ayarlamanız
...
kullanmanız gerekir. Unix epoch'undan bu yana geçen saniye cinsinden bir zaman
damgası isteğe bağlıdır ve yeni çarpan için özel bir başlangıç zamanı ayarlamak
üzere kullanılabilir. Zaman damgası sağlanmazsa, mevcut zaman damgası
kullanılacaktır.
...
Bakiyeyi almak için `balance` komutunu kullanmanız gerekir.
