...
## Ölçekli UI Miktarı Uzantısı Nedir?

Token Extensions Program'ın _rs`ScaledUiAmount`_ mint uzantısı, bir mint'in
token bakiyeleri için gösterilen UI miktarına güncellenebilir bir çarpan
uygulamasına olanak tanır.

Bu, bir token'ın hisse bölünmesi veya temettü gibi gerçek dünya varlıklarını
veya araçlarını temsil ettiğinde kullanışlıdır. Örneğin, bir hisse bölünmesi tüm
sahipler için hisse sayısını iki katına çıkarır. Her token hesabı bakiyesini
değiştirmeden, _rs`ScaledUiAmount`_ cüzdanların ve uygulamaların mint'in
çarpanını kullanarak yeni bir UI miktarı görüntülemesine olanak tanır.

Bu özellik ayrıca temettüler veya getiri dağıtımı için de kullanılabilir.

Yeni token oluşturulmaz. Token hesaplarında saklanan token miktarı aynı kalır.
Yalnızca görüntülenen UI miktarı değişir.

<Callout type="info" title="Uyumsuz Uzantı">
  Aynı mint üzerinde _rs`ScaledUiAmount`_ ve _rs`InterestBearingConfig`_
  uzantılarını etkinleştirmeyin. Token Extensions Program bu uzantı
  kombinasyonunu reddeder.
</Callout>

## Ölçekli UI Miktarı Nasıl Çalışır

- _rs`ScaledUiAmountConfig`_ güncelleme yetkisini, `multiplier`,
  `new_multiplier` ve `new_multiplier_effective_timestamp` değerlerini saklar.
- `new_multiplier_effective_timestamp` öncesinde, dönüşümler `multiplier`
  kullanır. Bu zaman damgasında veya sonrasında, dönüşümler `new_multiplier`
  kullanır.
- _rs`UpdateMultiplier`_ yeni bir çarpanı anında uygulayabilir veya gelecekteki
  bir Unix zaman damgası için planlayabilir.
- _rs`AmountToUiAmount`_, _rs`UiAmountToAmount`_ ve uzantı dönüşüm yardımcıları,
  ölçekli UI miktarı mint'leri için kayan noktalı aritmetik kullanır, bu nedenle
  dönüşümlerin tam olarak geri dönüşüm yapması garanti edilmez.

## Ölçekli UI Miktarı Nasıl Kullanılır

Ölçekli UI miktarını kullanmak için:

1. Mint hesabı boyutunu ve mint ile _rs`ScaledUiAmount`_ uzantısı için gereken
   rent'i hesaplayın.
2. _rs`CreateAccount`_ ile mint hesabını oluşturun, _rs`ScaledUiAmount`_
   uzantısını başlatın ve mint'i _rs`InitializeMint`_ ile başlatın.
3. Bakiyeleri görüntülerken veya UI miktarlarını dönüştürürken
   _rs`AmountToUiAmount`_, _rs`UiAmountToAmount`_ veya istemci yardımcılarını
   kullanın.
4. Görüntülenen çarpanı anında veya gelecekteki bir Unix zaman damgasında
   güncellemek için _rs`UpdateMultiplier`_ kullanın.

<Callout type="info" title="Talimat Sırası">
  _rs`ScaledUiAmountMintInstruction::Initialize`_, _rs`InitializeMint`_
  öncesinde gelmelidir. _rs`CreateAccount`_,
  _rs`ScaledUiAmountMintInstruction::Initialize`_ ve _rs`InitializeMint`_ aynı
  işlemde yer almalıdır.
</Callout>
...
<Cards>
  <Card
    title="Yayımcı Kılavuzu"
    href="/docs/tokens/extensions/scaled-ui-amount/issuer-guide"
  >
    Ölçekli UI Miktarı ile bir mint oluşturun ve çarpanı zaman içinde
    güncelleyin.
  </Card>
  <Card
  ...
  >
    Token miktarlarını görüntüleme için dönüştürün ve cüzdanlarda ve
    uygulamalarda Ölçekli UI Miktarını destekleyin.
  </Card>
</Cards>
...
| Öğe                                                   | Açıklama                                                                                                                              | Kaynak                                                                                                                                        |
| ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`ScaledUiAmountConfig`_                            | Yetkiyi, mevcut çarpanı, sonraki çarpanı ve geçerli olacağı zaman damgasını saklayan mint uzantısı durumu.                            | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L41-L55)         |
| _rs`ScaledUiAmountMintInstruction::Initialize`_       | _rs`InitializeMint`_ öncesinde bir mint üzerinde _rs`ScaledUiAmount`_ başlatan talimat.                                               | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/instruction.rs#L24-L42) |
| _rs`ScaledUiAmountMintInstruction::UpdateMultiplier`_ | Yeni bir çarpan ve bunun geçerli olacağı Unix zaman damgasını ayarlayan talimat.                                                      | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/instruction.rs#L43-L66) |
| _rs`amount_to_ui_amount`_                             | Mevcut çarpanı, ondalık basamakları ve zaman damgasını kullanarak bir token miktarını UI miktarı dizesine dönüştüren yardımcı.        | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L69-L84)         |
| _rs`try_ui_amount_into_amount`_                       | Mevcut çarpanı, ondalık basamakları ve zaman damgasını kullanarak bir UI miktarı dizesini tekrar token miktarına dönüştüren yardımcı. | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L86-L107)        |
| _rs`process_initialize`_                              | _rs`ScaledUiAmountConfig`_ başlatan ve başlangıç çarpanını başlatılmamış bir mint üzerinde saklayan işlemci.                          | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/scaled_ui_amount/processor.rs#L37-L57)     |
| _rs`process_update_multiplier`_                       | Yetkiyi doğrulayan, zamanlanmış çarpanı güncelleyen ve uygun olduğunda hemen uygulayan işlemci.                                       | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/scaled_ui_amount/processor.rs#L59-L108)    |
| _rs`process_amount_to_ui_amount`_                     | Bir mint uzantıyı etkinleştirdiğinde _rs`ScaledUiAmountConfig`_ kullanan Token Program dönüşüm yolu.                                  | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1548-L1571)                            |
| _rs`process_ui_amount_to_amount`_                     | Ölçekli UI miktarı mintleri için bir UI miktarını tekrar token miktarına dönüştüren Token Program dönüşüm yolu.                       | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1576-L1595)                            |
