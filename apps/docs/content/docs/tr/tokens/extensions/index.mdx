...
Her uzantı, genellikle mint veya token account oluşturma sırasında başlatılan belirli bir durum ekler. Her iki hesabı başlatırken, farklı işlevler için belirli uzantıları aynı anda etkinleştirebilirsiniz. Çoğu uzantı, bir hesap başlatıldıktan sonra eklenemez. Bu, tokenınızı tasarlarken önemli bir husus olup tokenınızın desteklemesini istediğiniz özellikler için önceden plan yapmanız gerekecektir. Aşağıdaki bölümler her uzantıyı ve nasıl entegre edileceğini açıklamaktadır.

<Callout type="info">
  Bazı uzantılar birbiriyle uyumsuzdur ve bunları aynı token mint'inde veya
  token hesabında aynı anda etkinleştiremezsiniz. Örneğin, _rs`NonTransferable`_
  uzantısını _rs`TransferFeeConfig`_ uzantısıyla birleştiremezsiniz, çünkü
  çelişen davranışlara sahiptirler.
</Callout>
...
enum tanımlar. Her varyant, benzersiz işlevselliğe sahip farklı bir uzantıyı
temsil eder.

_rs`ExtensionType`_ enum şu şekilde tanımlanır:
...
Her uzantı, bir mint veya token hesabına ekstra durum ekleyerek özelleştirilmiş
işlevsellik sağlar. Tüm uzantıya özgü durum, temel hesap veri tipini takip eden
...
alanında saklanır. `tlv_data`'i (uzantı durumunu içeren), o hesap için
etkinleştirilen belirli uzantı tiplerine göre daha fazla deserialize
etmelisiniz.
