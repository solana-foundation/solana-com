...
token hesaplarını Çapraz Program Çağrıları (CPI'ler) aracılığıyla beklenmeyen
...
aktarılabilmesini sağlar ve diğer tüm programların CPI aracılığıyla token
transfer etmesini engeller.
