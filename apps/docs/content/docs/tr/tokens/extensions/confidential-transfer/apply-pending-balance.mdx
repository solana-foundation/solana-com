---
title: Bekleyen bakiyeyi uygulama
description:
  Fonları kullanılabilir hale getirmek için bekleyen bakiyenin nasıl
  uygulanacağını öğrenin.
---
...
Token'lar gizli olarak transfer edilmeden önce, genel token bakiyesinin gizli
bakiyeye dönüştürülmesi gerekir. Bu dönüşüm iki aşamada gerçekleşir:

1. **Gizli bekleyen bakiye**: Başlangıçta, token'lar genel bakiyeden "bekleyen"
   gizli bakiyeye "yatırılır".
2. **Gizli kullanılabilir bakiye**: Bekleyen bakiye daha sonra kullanılabilir
   bakiyeye "uygulanır" ve token'ları gizli transferler için kullanılabilir hale
   getirir.

Bu bölüm ikinci aşamayı açıklar: bekleyen bakiyeyi kullanılabilir bakiyeye
uygulama.

Token'lar genel bakiyeden "yatırıldığında" veya token'lar bir token hesabından
diğerine gizli olarak transfer edildiğinde, token'lar başlangıçta gizli bekleyen
bakiyeye eklenir. Token'lar gizli transferler için kullanılmadan önce, bekleyen
bakiyenin kullanılabilir bakiyeye "uygulanması" gerekir.
...
Aşağıdaki diyagram, bekleyen bakiyeyi kullanılabilir bakiyeye uygulama
adımlarını gösterir:
...
`ApplyPendingBalance` talimatıyla bir işlem oluşturan ve gönderen bir
`confidential_transfer_apply_pending_balance` yöntemi sağlar.

### Örnek kod

Aşağıdaki örnek, gizli bekleyen bakiyenin gizli kullanılabilir bakiyeye nasıl
