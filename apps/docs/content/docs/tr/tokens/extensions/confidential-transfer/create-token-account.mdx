...
description:
  Gizli Bakiyeler etkinleştirilmiş bir token account'un nasıl oluşturulacağını öğrenin.
---

## Gizli Bakiyeler ile token account nasıl oluşturulur

Gizli Bakiyeler, token account'a ekstra durum bilgisi ekleyerek özel token transferlerini mümkün kılar. Bu bölümde, gerekli uzantı etkinleştirilmiş bir token account'un nasıl oluşturulacağı açıklanmaktadır.

Aşağıdaki diyagram, Gizli Bakiyeler etkinleştirilmiş bir token account oluşturmada yer alan adımları göstermektedir:
...
### Gizli Bakiyeler Token Account Durumu
...
_rs`ConfidentialTransferAccount`_, Gizli Bakiyeleri yöneten çeşitli alanlar içerir:

- **approved**: Gizli Bakiyeler için hesabın onay durumu. Eğer mint account'un `auto_approve_new_accounts` yapılandırması `true` olarak ayarlanmışsa, tüm token account'lar Gizli Bakiyeler için otomatik olarak onaylanır.
...
Gizli Bakiyeler için bir token account oluşturmak ve yapılandırmak, tek bir işleme sığan aşağıdaki talimatları kullanır:
...
4. **Gizli Bakiyeleri Yapılandır**: Token Extensions Program'ın
   ...
   talimatını çağırın; _rs`ConfidentialTransferAccount`_ durumunu başlatmak için
   _rs`ProofLocation::ContextStateAccount`_ aracılığıyla kanıt bağlam durumu hesabına başvurun.

<Callout type="info">

**Yalnızca token account sahibi, bir token account üzerinde Gizli Bakiyeleri etkinleştirebilir**.

_rs`ConfigureAccount`_ talimatı, yalnızca token account sahibi tarafından oluşturulabilecek şifreleme anahtarlarının ve bir kanıtın istemci tarafında üretilmesini gerektirir.

Pubkey geçerlilik kanıtı, hesabın ElGamal pubkey'inin geçerli olduğunu doğrular. _rs`build_pubkey_validity_proof_data`_ ile oluşturulur, ZK ElGamal Proof programı tarafından zincir üzerinde bir bağlam durumu hesabına doğrulanır ve ardından _rs`ConfigureAccount`_ tarafından _rs`ProofLocation::ContextStateAccount`_ aracılığıyla referans alınır; böylece token talimatının kendisinde hiçbir kanıt baytı iletilmez. Uygulama ayrıntıları için bkz.:
...
Aşağıdaki kod, mevcut uyumlu bir mint ile Gizli Bakiyeleri kullanmak üzere yapılandırılmış bir associated token account oluşturur.

Gizli Bakiyeler, mainnet ve devnet üzerinde etkinleştirilen ZK ElGamal Proof programına bağlıdır. Standart bir `solana-test-validator` bunu etkinleştirmez; ancak [Surfpool](https://surfpool.run) gibi mainnet'i çatallayan yerel bir validator etkinleştirir.
Örneği, fonlanmış bir ödeyici ile bu ortamlardan birine karşı çalıştırın (kod devnet kullanır) ve mint yer tutucusunu [Mint Oluştur](/docs/tokens/extensions/confidential-transfer/create-mint) bölümüne göre oluşturulmuş bir mint ile değiştirin.
