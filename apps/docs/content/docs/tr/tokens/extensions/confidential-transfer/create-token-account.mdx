...
- **maximum_pending_balance_credit_counter**: Bekleyen bakiyeyi kullanılabilir
  ...
  önce bekleyen kredilerin sayı sınırı.

- **expected_pending_balance_credit_counter**: _rs`ApplyPendingBalance`_
  talimatı en son işlendiğinde, istemci tarafından talimat verisi aracılığıyla
...
hesaplama gerektirir. Ciphertext aritmetik implementasyonunu
...
Bir token hesabının durumunu `decryptable_available_balance` değerini okumak
için getirirken, farklı beklenen/gerçek sayaç değerleri, istemcinin doğru
...
1. Token hesabındaki `decryptable_available_balance` değeriyle başlayın
2. Sayaç farkına kadar (gerçek - beklenen) yatırma ve transfer talimatlarını
   ...
   - Transfer talimatlarından hedef şifreli metin miktarlarını şifresini çözün
     ve ekleyin
...
3. **Gizli transferleri yapılandırın**: _rs`ConfidentialTransferAccount`_
...
_rs`ConfigureAccount`_ talimatı, istemci tarafında şifreleme anahtarlarının ve
yalnızca token hesabı sahibi tarafından oluşturulabilen kanıt verilerinin
