...
<Callout type="warn">
  ZK ElGamal Programı, güvenlik denetiminden geçerken mainnet ve devnet'te
  geçici olarak devre dışı bırakılmıştır. Bu, gizli transfer uzantısının şu anda
  kullanılamadığı anlamına gelir. Kavramlar hala geçerli olsa da, kod örnekleri
  çalışmayacaktır.
</Callout>

## Gizli Transfer uzantısı ile token hesabı nasıl oluşturulur

Gizli Transfer uzantısı, token hesabına ekstra durum ekleyerek özel token
transferlerini etkinleştirir. Bu bölüm, bu uzantı etkinleştirilmiş bir token
hesabının nasıl oluşturulacağını açıklar.
...
### Gizli Transfer token hesabı durumu

Uzantı, token hesabına
...
_rs`ConfidentialTransferAccount`_, gizli transferleri yönetmek için birkaç alan
içerir:

- **approved**: Hesabın gizli transferler için onay durumu. Mint hesabının
  `auto_approve_new_accounts` yapılandırması `true` olarak ayarlanmışsa, tüm
  token hesapları otomatik olarak gizli transferler için onaylanır.

- **elgamal_pubkey**: Bakiyeleri ve transfer tutarlarını şifrelemek için
  kullanılan ElGamal genel anahtarı.
...
- **decryptable_available_balance**: Hesap sahibi tarafından verimli şifre çözme
  için Advanced Encryption Standard (AES) anahtarı ile şifrelenmiş
...
- **maximum_pending_balance_credit_counter**: Bekleyen bakiyeyi kullanılabilir
  bakiyeye dönüştürmek için _rs`ApplyPendingBalance`_ talimatının gerekli olduğu
  bekleyen kredilerin sayı sınırı.

- **expected_pending_balance_credit_counter**: _rs`ApplyPendingBalance`_
  talimatı son işlendiğinde istemci tarafından talimata ait veri aracılığıyla
  sağlanan `pending_balance_credit_counter` değeri.
...
bakiyeler olarak ayrılır. Bu ayrım olmadan, bir saldırgan tekrar tekrar bir
token hesabına token gönderebilir ve token hesabı sahibinin token transfer etme
yeteneğini engelleyebilir. Token hesabı sahibi token transfer edemez çünkü
şifrelenmiş bakiye, işlem gönderildiği zaman ile işlendiği zaman arasında
değişir ve bu da başarısız bir işlemle sonuçlanır.

Tüm yatırma ve transfer tutarları başlangıçta bekleyen bakiyeye eklenir. Token
hesabı sahipleri, bekleyen bakiyeyi kullanılabilir bakiyeye dönüştürmek için
_rs`ApplyPendingBalance`_ talimatını kullanmalıdır. Gelen transferler veya
yatırmalar bir token hesabının kullanılabilir bakiyesini etkilemez.

</Callout>

<Callout type="info">
  **Bekleyen bakiye yüksek/düşük ayrımı**

Gizli bekleyen bakiye `pending_balance_lo` ve `pending_balance_hi` olarak
ayrılır çünkü ElGamal şifre çözme işlemi daha büyük sayılar için daha fazla
hesaplama gerektirir. Ciphertext aritmetik uygulamasını
[buradan](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/confidential-transfer/ciphertext-arithmetic/src/lib.rs#L54)
...
_rs`ApplyPendingBalance`_ talimatı çağrılırken:
...
2. Beklenen ve gerçek bekleyen kredi sayaçları, _rs`ApplyPendingBalance`_
   talimatının oluşturulması ve işlenmesi arasında sayaç değerindeki
   değişiklikleri takip eder:
   - `expected_pending_balance_credit_counter`: İstemci
     _rs`ApplyPendingBalance`_ talimatını oluştururken
     `pending_balance_credit_counter` değeri
   - `actual_pending_balance_credit_counter`: _rs`ApplyPendingBalance`_ talimatı
     işlendiği anda token hesabındaki `pending_balance_credit_counter` değeri

Eşleşen beklenen/gerçek sayaçlar, `decryptable_available_balance` değerinin
`available_balance` ile eşleştiğini gösterir.

`decryptable_available_balance` değerini okumak için bir token hesabının durumu
getirilirken, farklı beklenen/gerçek sayaç değerleri, istemcinin doğru bakiyeyi
hesaplamak için sayaç farkıyla eşleşen son yatırma/transfer talimatlarını
aramasını gerektirir.
...
2. Sayaç farkına kadar (gerçek - beklenen) yatırma ve transfer talimatlarını
   içeren en son işlemleri getirin:
   - Yatırma talimatlarından genel tutarları ekleyin
   - Transfer talimatlarından hedef şifreli metin tutarlarını şifresini çözün ve
     ekleyin
...
Gizli Transfer uzantısına sahip bir token hesabı oluşturmak üç talimata ihtiyaç
duyar:

1. **Token hesabını oluşturun**: Token hesabını oluşturmak için Associated Token
   Program'ın _rs`AssociatedTokenAccountInstruction:Create`_ talimatını çağırın.

2. **Hesap alanını yeniden tahsis et**: _rs`TokenInstruction::Reallocate`_
   durumu için alan eklemek üzere Token Extension Program'ın
   _rs`TokenInstruction::Reallocate`_ talimatını çağırın.

3. **Gizli transferleri yapılandır**: _rs`ConfidentialTransferAccount`_ durumunu
   başlatmak için Token Extension Program'ın
...
doğrulayan bir kanıt oluşturur. Uygulama detayları için bkz:
...
Aşağıdaki kod, Gizli Transfer uzantısı ile bir Associated Token Account'un nasıl
oluşturulacağını göstermektedir,

Örneği çalıştırmak için, aşağıdaki komutu kullanarak mainnet'ten klonlanan Token
Extension Program ile yerel bir validator başlatın. Yerel validator'ı başlatmak
için Solana CLI'nin yüklü olması gerekir.
