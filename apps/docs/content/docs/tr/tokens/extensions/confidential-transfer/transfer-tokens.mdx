...
description:
  Bir token hesabından diğerine gizli olarak token transferi yapmayı öğrenin.
---

## Bir token account'tan diğerine token'ları gizli olarak nasıl transfer edersiniz

Bir token account'tan diğerine gizli olarak token aktarmak için hem
gönderenin hem de alıcının _rs`ConfidentialTransferAccount`_ durumuyla
yapılandırılmış ve Gizli Bakiyeleri kullanmak üzere onaylanmış token account'lara
sahip olması gerekir. Gönderenin token account'ında ayrıca aktarım için
kullanılabilir gizli bir bakiye bulunmalıdır.
...
1. İstemci tarafında
   ...
   **Eşitlik Kanıtı
   ...
   Transfer sonrasında yeni mevcut bakiye şifreli metninin karşılık gelen
   [Pedersen taahhüdüyle](https://en.wikipedia.org/wiki/Commitment_scheme)
   eşleştiğini doğrular; kaynak hesabın yeni mevcut bakiyesinin
   `new_balance = current_balance - transfer_amount` olarak doğru hesaplandığını
   garanti eder.

   **Şifreli Metin Geçerlilik Kanıtı
   ...
   Transfer miktarı şifreli metinlerinin üç taraf için de (kaynak, hedef ve
   denetçi) uygun şekilde oluşturulduğunu doğrular; transfer miktarının her
   tarafın genel anahtarı altında doğru şekilde şifrelendiğini garanti eder.
   ...
   **Aralık Kanıtı
   ...
   Yeni mevcut bakiye ve transfer miktarının (düşük/yüksek bitlere bölünmüş)
   tamamının negatif olmadığını ve belirtilen bir aralık içinde olduğunu
   doğrular.

2. Her kanıt için:
   - Kanıt verilerini doğrulamak üzere ZK ElGamal kanıt programını çağırın.
   - Diğer talimatlarda kullanmak üzere kanıta özgü meta verileri bir kanıt
     "bağlam durumu" hesabında saklayın.

3. Kanıt bağlam durumu hesaplarını sağlayarak
...
4. Oluşturmak için kullanılan SOL'u geri almak amacıyla kanıt bağlam durumu
   hesaplarını kapatın.

Aşağıdaki diyagram, token'ları göndericinin token account'ından alıcının token
account'ına transfer etme adımlarını göstermektedir.
...
Token'ları bir token account'tan diğerine gizli olarak transfer etmek için
şunları yapmanız gerekir:

- İstemci tarafında bir eşitlik kanıtı, şifreli metin geçerlilik kanıtı ve
  aralık kanıtı oluşturun
- Kanıtları doğrulamak ve "bağlam durumu" hesaplarını başlatmak için Zk ElGamal
  kanıt programını çağırın
- Üç kanıt hesabını sağlayarak
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  talimatını çağırın.
- rent'i geri almak için üç kanıt hesabını kapatın.

Aşağıdaki Rust örneği, `spl-token-confidential-transfer-proof-generation`
crate'i ile kanıtları oluşturur, her birini ZK ElGamal Proof programı
aracılığıyla bir bağlam durumu hesabına doğrular, transfer talimatındaki üç
hesaba başvurur ve ardından bunları kapatır. TypeScript örneği,
`@solana-program/token-2022/confidential`'den
`getConfidentialTransferInstructionPlan` yardımcısını kullanır; bu yardımcı,
aynı kanıt hesaplarını, transferi ve kapatma işlemini çok işlemli bir talimat
planı olarak bir araya getirir.

## Örnek Kod

Aşağıdaki örnek, token'ları bir hesaptan diğerine gizli olarak aktarır.
Her iki hesap da Gizli Bakiyeler için önceden yapılandırılmış olmalı ve
gönderenin kullanılabilir bir gizli bakiyesi bulunmalıdır.

Gizli Bakiyeler, mainnet ve devnet üzerinde etkinleştirilen ZK ElGamal Proof programına bağlıdır. Standart bir `solana-test-validator` bunu etkinleştirmez; ancak [Surfpool](https://surfpool.run) gibi mainnet'i çatallayan yerel bir validator bunu destekler.
Örneği, bu seçeneklerden biri üzerinde (kod devnet kullanır) fonlanmış bir ödeyici ile çalıştırın ve yer tutucuları mint'inizle, gönderen ve alıcı hesaplarıyla değiştirin.
