...
## Bir token hesabından diğerine gizli olarak token transferi nasıl yapılır

Bir token hesabından diğerine gizli olarak token transfer etmek için hem
gönderenin hem de alıcının token hesaplarının _rs`ConfidentialTransferAccount`_
durumu ile yapılandırılmış ve gizli transferler için onaylanmış olması gerekir.
Gönderenin token hesabında ayrıca transfer edilebilecek kullanılabilir bir gizli
bakiye bulunmalıdır.

Tokenları gizli olarak transfer etmek için:
...
1. İstemci tarafında
   ...
   **Eşitlik kanıtı
   ...
   Transferden sonraki yeni kullanılabilir bakiye şifreli metninin karşılık
   gelen [Pedersen taahhüdü](https://en.wikipedia.org/wiki/Commitment_scheme)
   ile eşleştiğini doğrular ve kaynak hesabın yeni kullanılabilir bakiyesinin
   `new_balance = current_balance - transfer_amount` olarak doğru şekilde
   ...
   **Şifreli metin geçerlilik kanıtı
   ...
   Transfer tutarı şifreli metinlerinin üç taraf (kaynak, hedef ve denetçi) için
   düzgün şekilde oluşturulduğunu doğrular ve transfer tutarının her bir tarafın
   genel anahtarı altında doğru şekilde şifrelendiğinden emin olur.
   ...
   **Aralık kanıtı
   ...
   bölünmüş) tümünün negatif olmadığını ve belirtilen aralıkta olduğunu
   doğrular.
...
4. Oluşturmak için kullanılan SOL'u geri almak için kanıt bağlam durumu
   hesaplarını kapatın.

Aşağıdaki diyagram, token'ları bir gönderenin token hesabından bir alıcının
token hesabına aktarma adımlarını göstermektedir.
...
`spl_token_client` crate'i aşağıdaki yöntemleri sağlar:

- Bir kanıt hesabı oluşturan
  `confidential_transfer_create_context_state_account` yöntemi.
- `Transfer` talimatını çağıran `confidential_transfer_transfer` yöntemi.
- Bir kanıt hesabını kapatan `confidential_transfer_close_context_state_account`
  yöntemi.

### Örnek kod

Aşağıdaki örnek, token hesapları arasında token'ların gizli bir şekilde nasıl
...
<Callout type="info">
  Bu yazının yazıldığı sırada, Confidential Transfers varsayılan yerel
  validator'da etkin değildir. Örnek kodu çalıştırmak için mainnet Token
  Extension Program'ı klonlamanız gerekir.
</Callout>
