...
Token'ları bir token account'tan diğerine gizli olarak transfer etmek için hem
gönderici hem de alıcının _rs`ConfidentialTransferAccount`_ durumuyla
yapılandırılmış ve gizli transferler için onaylanmış token account'larına sahip
olması gerekir. Göndericinin token account'ının ayrıca transfer edilebilecek
mevcut bir gizli bakiyesi olması gerekir.
...
Aşağıdaki örnek, token'ları bir hesaptan diğerine gizli olarak aktarır. Her iki
hesabın da gizli transferler için önceden yapılandırılmış olması ve göndericinin
kullanılabilir bir gizli bakiyeye sahip olması gerekir.

Gizli transferler, mainnet ve devnet'te etkinleştirilen ZK ElGamal Proof
...
validator etkinleştirir. Örneği, fonlanmış bir ödeyici ile bu ortamlardan birine
karşı çalıştırın (kod devnet kullanır) ve yer tutucuları kendi mint'iniz ile
gönderici ve alıcı hesaplarıyla değiştirin.
