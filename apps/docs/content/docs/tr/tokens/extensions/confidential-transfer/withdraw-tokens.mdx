...
1. İstemci tarafında
   ...
   **Eşitlik Kanıtı
   ...
   Çekimin ardından kalan kullanılabilir bakiye şifreli metninin, karşılık gelen
   [Pedersen taahhüdüyle](https://en.wikipedia.org/wiki/Commitment_scheme)
   eşleştiğini doğrular; böylece hesabın yeni kullanılabilir bakiyesinin
   `remaining_balance = current_balance - withdraw_amount` olarak doğru biçimde
   hesaplandığını güvence altına alır.
   ...
   **Aralık Kanıtı
   ...
   Çekimin ardından kalan kullanılabilir bakiyenin negatif olmadığını ve belirli
   bir aralık içinde olduğunu doğrular.

2. Her kanıt için:
   - Kanıt verilerini doğrulamak üzere ZK ElGamal kanıt programını çağırın.
   - Diğer talimatlarla kullanmak üzere kanıta özgü meta verileri bir kanıt
...
4. Kanıt hesaplarını kapatarak bunları oluşturmak için kullanılan SOL'u geri
   kazanın.

Aşağıdaki şema, gizli kullanılabilir bakiyeden genel bakiyeye token çekerken
gerçekleştirilen adımları göstermektedir:
...
Aşağıdaki örnek, gizli kullanılabilir bakiyeden token'ları
geri genel bakiyeye çeker. Hesabın Gizli Bakiyeler için
zaten yapılandırılmış olması ve kullanılabilir bir gizli bakiye içermesi gerekir.

Gizli Bakiyeler, mainnet ve devnet üzerinde etkinleştirilmiş olan ZK ElGamal Proof programına bağlıdır. Standart bir `solana-test-validator` bunu etkinleştirmez; ancak [Surfpool](https://surfpool.run) gibi mainnet'i çatallayan yerel bir validator etkinleştirir.
Örneği, fonlanmış bir ödeyici ile bu seçeneklerden birine karşı çalıştırın (kod devnet kullanır) ve yer tutucuları kendi mint'inizle ve token account'unuzla değiştirin.
