...
1. İstemci tarafında
   ...
   **Eşitlik kanıtı
   ...
   Çekme işleminden sonra kalan kullanılabilir bakiye şifreli metninin, karşılık
   gelen [Pedersen taahhüdü](https://en.wikipedia.org/wiki/Commitment_scheme)
   ile eşleştiğini doğrular, hesabın yeni kullanılabilir bakiyesinin
   `remaining_balance = current_balance - withdraw_amount` olarak doğru şekilde
   ...
   **Aralık kanıtı
   ...
   Çekme işleminden sonra kalan kullanılabilir bakiyenin negatif olmadığını ve
   belirtilen aralıkta olduğunu doğrular.
...
3. İki kanıt hesabını sağlayarak
...
4. Oluşturmak için kullanılan SOL'u geri almak için kanıt hesaplarını kapatın.

Aşağıdaki diyagram, gizli kullanılabilir bakiyeden genel bakiyeye token çekme
işleminde yer alan adımları gösterir:
...
Gizli kullanılabilir bakiyeden genel bakiyeye token çekmek için şunları
yapmalısınız:
...
- İki kanıt hesabını sağlayarak
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  talimatını çağırın.
- Kirayı geri almak için iki kanıt hesabını kapatın.

`spl_token_client` crate'i aşağıdaki metodları sağlar:

- Bir kanıt hesabı oluşturan
  `confidential_transfer_create_context_state_account` metodu.
- `Withdraw` talimatını çağıran `confidential_transfer_withdraw` metodu.
