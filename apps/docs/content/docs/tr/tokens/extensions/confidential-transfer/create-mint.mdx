...
description:
  Confidential Balances etkin bir token mint'inin nasıl oluşturulacağını öğrenin.
---

## Confidential Balances ile mint nasıl oluşturulur

Confidential Balances, mint account'a ekstra durum ekleyerek özel token transferlerini etkinleştirir.
Bu bölüm, gerekli uzantı etkinleştirilmiş bir token mint'inin nasıl oluşturulacağını açıklar.

Aşağıdaki diyagram, Confidential Balances uzantısıyla bir mint oluşturma adımlarını göstermektedir:
...
### Confidential Balances Mint Durumu
...
- **authority**: Mint için Confidential Balances ayarlarını değiştirme ve otomatik onay devre dışı bırakıldığında yeni gizli hesapları onaylama iznine sahip hesap.

- **auto_approve_new_accounts**: True olarak ayarlandığında, kullanıcılar varsayılan olarak Confidential Balances etkin token account'lar oluşturabilir. False olarak ayarlandığında, authority'nin her yeni token account'ı gizli transferler için kullanılabilir hale gelmeden önce onaylaması gerekir.
...
Confidential Balances etkin bir mint oluşturmak, tek bir işlemde üç talimat gerektirir:

1. **Mint Account Oluşturma**: mint account'ı oluşturmak için System Program'ın
   _rs`CreateAccount`_ talimatını çağırın.

2. **Confidential Balances'ı Başlat**: Mint için _rs`ConfidentialTransferMint`_ durumunu yapılandırmak üzere Token Extension Program'ın
...
Aşağıdaki kod, Confidential Balances etkin bir mint'in nasıl oluşturulacağını göstermektedir.

Confidential Balances, mainnet ve devnet'te etkinleştirilen ZK ElGamal Proof programına bağlıdır. Standart bir `solana-test-validator` bunu etkinleştirmez, ancak [Surfpool](https://surfpool.run) gibi mainnet'i forklayan yerel bir validator etkinleştirir.
Örneği bu ortamlardan birinde (kod devnet kullanır) fonlanmış bir ödeyici ile çalıştırın ve yer tutucu mint ile hesap adreslerini kendinizinkilerle değiştirin.
