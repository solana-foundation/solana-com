...
## Gizli bekleyen bakiyeye token yatırma

Tokenlar gizli olarak transfer edilmeden önce, genel token bakiyesinin gizli bir
bakiyeye dönüştürülmesi gerekir. Bu dönüşüm iki aşamada gerçekleşir:

1. **Gizli Bekleyen Bakiye**: Başlangıçta tokenlar, genel bakiyeden "bekleyen"
   gizli bakiyeye "yatırılır".
2. **Gizli Kullanılabilir Bakiye**: Bekleyen bakiye daha sonra kullanılabilir
   bakiyeye "uygulanarak" tokenlar gizli transferler için kullanılabilir hale
   getirilir.

Bu bölüm, birinci aşamayı açıklar: genel token bakiyesinin gizli bekleyen
bakiyeye yatırılması.

Aşağıdaki diyagram, genel bakiyeden gizli bekleyen bakiyeye token yatırma
işlemindeki adımları göstermektedir:
...
Genel bakiyeyi gizli bekleyen bakiyeye dönüştürmek için
...
talimatını çağırın. Yatırma talimatı başına
...
`spl_token_client` crate'i, aşağıdaki örnekte gösterildiği gibi `Deposit`
talimatını içeren bir işlem oluşturup gönderen bir
`confidential_transfer_deposit` yöntemi sağlar.

## Örnek Kod

Aşağıdaki örnek, genel token bakiyesinin gizli bekleyen bakiyeye nasıl
yatırılacağını göstermektedir.

Gizli Bakiyeler, mainnet ve devnet üzerinde etkin olan ZK ElGamal Proof programına bağlıdır. Standart bir `solana-test-validator` bunu etkinleştirmez; ancak [Surfpool](https://surfpool.run) gibi mainnet'i çatallayan yerel bir validator bunu destekler.
Örneği, fonlanmış bir ödeyici ile bu ortamlardan birinde çalıştırın (kod devnet kullanır) ve yer tutucu mint ile hesap adreslerini kendi adreslerinizle değiştirin.
