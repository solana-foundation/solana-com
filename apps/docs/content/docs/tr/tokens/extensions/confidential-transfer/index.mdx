...
Gizli transferler, transfer miktarını açığa çıkarmadan token hesapları arasında
token transferi yapmanızı sağlar. Bu, gizliliği koruyan işlemler için
kullanışlıdır. Yalnızca transfer miktarları ve token bakiyeleri özeldir. Token
hesap adresleri herkese açık kalır.

- [Protokol genel bakış](https://www.solana-program.com/docs/confidential-balances/overview) -
  Temel kriptografik protokol hakkında ayrıntılar
...
Gizli transfer uzantısı, Token Extension programına hesaplar arasında transfer
miktarını açığa çıkarmadan token transferi yapmanızı sağlayan
...
1. Gizli transfer uzantısına sahip bir mint hesabı oluşturun.
2. Gönderen ve alıcı için gizli transfer uzantısına sahip token hesapları
   oluşturun.
3. Gönderen hesabına token basın.
...
5. Göndericinin bekleyen bakiyesini **gizli kullanılabilir bakiyeye**
   **uygulayın**.
6. Gönderen token hesabından alıcı token hesabına gizli olarak token
   **transferi** yapın.
...
Gizli transfer akışındaki adımlarla ilgili daha fazla ayrıntı için ilgili
sayfalara bakın:
...
<Cards>
  ...
  >
    Confidential Transfer uzantısıyla mint account nasıl oluşturulur
  </Card>
  <Card
    title="Token Account Oluştur"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Confidential Transfer uzantısıyla token account nasıl yapılandırılır
  </Card>
  <Card
    title="Token Yatır"
    href="/docs/tokens/extensions/confidential-transfer/deposit-tokens"
  >
    Tokenlar gizli bekleyen bakiyeye nasıl yatırılır
  </Card>
  <Card
    title="Bekleyen Bakiyeyi Uygula"
    href="/docs/tokens/extensions/confidential-transfer/apply-pending-balance"
  >
    Bekleyen bakiye mevcut gizli bakiyeye nasıl uygulanır
  </Card>
  <Card
    title="Token Çek"
    href="/docs/tokens/extensions/confidential-transfer/withdraw-tokens"
  >
    Tokenlar gizli mevcut bakiyeden nasıl çekilir
  </Card>
  <Card
    title="Token Transfer Et"
    href="/docs/tokens/extensions/confidential-transfer/transfer-tokens"
  >
    Tokenlar token account'lar arasında gizli olarak nasıl transfer edilir
  </Card>
  <Card
    title="İhraçcı Rehberi"
    href="/docs/tokens/extensions/confidential-transfer/issuer-guide"
  >
    Gizli transfer tokeni nasıl çıkarılır ve işletilir (onay politikası,
    denetçiler, ücretler, mint ve burn)
  </Card>
</Cards>

Aşağıdaki diyagram, gizli token transferleri için temel akışın ayrıntılı bir
...
için adımları göstermektedir. Adımlar netlik için yardımcı fonksiyonlara
ayrılmıştır.

Örneği çalıştırmak için, aşağıdaki komutu kullanarak mainnet'ten klonlanan Token
Extension Program ile yerel bir validator başlatın. Yerel validator'ı kullanmak
...
Gizli Transfer uzantısı
[talimatlarının](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
tam listesi aşağıdaki gibidir:

| Talimat                             | Açıklama                                                                                                                                                       |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Mint hesabını gizli transferler için ayarlar. Bu talimat _rs`TokenInstruction::InitializeMint`_ talimatıyla aynı işleme dahil edilmelidir.                     |
...
| _rs`EmptyAccount`_                  | Bir token hesabının kapatılmasına izin vermek için bekleyen ve kullanılabilir gizli bakiyeleri boşaltır.                                                       |
| _rs`Deposit`_                       | Açık token bakiyesini bekleyen gizli bakiyeye dönüştürür.                                                                                                      |
| _rs`Withdraw`_                      | Kullanılabilir gizli bakiyeyi tekrar açık bakiyeye dönüştürür.                                                                                                 |
| _rs`Transfer`_                      | Token hesapları arasında gizli olarak token transfer eder.                                                                                                     |
| _rs`ApplyPendingBalance`_           | Yatırma veya transferlerden sonra bekleyen bakiyeyi kullanılabilir bakiyeye dönüştürür.                                                                        |
| _rs`EnableConfidentialCredits`_     | Bir token hesabının gizli token transferleri almasına izin verir.                                                                                              |
| _rs`DisableConfidentialCredits`_    | Gelen gizli transferleri engeller ancak yine de açık transferlere izin verir.                                                                                  |
...
| _rs`TransferWithFee`_               | Token hesapları arasında ücretli olarak gizli şekilde token transfer eder.                                                                                     |
| _rs`ConfigureAccountWithRegistry`_  | _rs`VerifyPubkeyValidity`_ kanıtı yerine bir _rs`ElGamalRegistry`_ hesabı kullanarak token hesaplarını gizli transferler için yapılandırmanın alternatif yolu. |
