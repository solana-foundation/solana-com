---
title: Gizli Bakiyeler
description:
  Gizli Bakiyeler uzantısı ve token mint'lerine ile hesaplarına isteğe bağlı özellikler eklemek için nasıl kullanılacağı hakkında bilgi edinin.
---

## Gizli Bakiyeler Nedir?

<Embed url="https://youtu.be/Bqs95tFcRIU" />

Gizli Bakiyeler, token account'lar arasında transfer miktarını açıklamadan token transferi yapmanızı sağlayan bir Token-2022 uzantısıdır. Bu, gizliliği koruyan işlemler için oldukça kullanışlıdır. Yalnızca transfer miktarları ve token bakiyeleri gizlidir. Token account adresleri herkese açık kalmaya devam eder.
...
- [Gizli Bakiyeler Cookbook](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Gizli Bakiyelerin nasıl kullanılacağını gösteren kod parçacıkları

### Nasıl çalışır?

Gizli Bakiyeler uzantısı, Token Extension program'ına hesaplar arasında transfer miktarını açıklamadan token transferi yapmanıza olanak tanıyan
...
1. Gizli Bakiyeler etkin bir mint account oluşturun.
2. Gönderici ve alıcı için Gizli Bakiyeler etkin token account'lar oluşturun.
...
5. Göndericinin bekleyen bakiyesini **gizli kullanılabilir bakiyeye**
   **uygulayın**.
6. Gönderici token account'tan alıcı token account'a gizli olarak token
   **transfer edin**.
...
Gizli Bakiyeler mint'i, isteğe bağlı bir global denetçi ElGamal açık anahtarı saklayabilir. Bir denetçi yapılandırıldığında, her gizli transfer; gönderici, alıcı ve denetçi için şifrelenmiş miktarı içerir. Transfer kanıtı, üç şifreli metnin de aynı miktarı içerdiğini doğrular. Halk hâlâ miktarı okuyamaz, ancak denetçi gizli anahtarının sahibi, söz konusu mint için transfer miktarlarının şifresini çözebilir.

Gizli Bakiyeler yetkisi, uzantıyı başlatırken denetçi anahtarını ayarlar ya da daha sonra _rs`UpdateMint`_ ile değiştirir. Yetki, anahtarı döndürebilir veya temizleyebilir. Döndürme işlemi yalnızca gelecekteki transferlere uygulanır; bu nedenle geçmiş denetim erişimi gerektiğinde önceki gizli anahtarlar saklanmalıdır. Denetçi anahtarları okuma erişimi sağlar; token transferlerine izin vermez ve bir hesabın tam gizli bakiyesini açığa çıkarmaz.

Kurulum ve döndürme örnekleri için bkz.
[Denetçi yapılandırma](/docs/tokens/extensions/confidential-transfer/issuer-guide#configure-an-auditor).
Cüzdanlar, gezginler, borsalar ve uyumluluk sağlayıcıları ayrıca
[denetçi entegrasyon kılavuzunu](/docs/tokens/extensions/confidential-transfer/integration-guide#auditors-and-compliance) incelemelidir.

Gizli transfer akışındaki adımlara ilişkin daha fazla ayrıntı için ilgili
sayfalara bakın:
...
<Cards>
  ...
  >
    Gizli Bakiyeler etkin bir mint account nasıl oluşturulur
  </Card>
  <Card
    title="Token Account Oluştur"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Gizli Bakiyeler etkin bir token account nasıl yapılandırılır
  </Card>
  <Card
    title="Token Yatır"
  ...
  >
    Cüzdanlar, gezginler ve borsaların Gizli Bakiyeler token'larını nasıl destekleyebileceği
  </Card>
  <Card
    title="İhraçcı Kılavuzu"
    href="/docs/tokens/extensions/confidential-transfer/issuer-guide"
  >
    Gizli Bakiyeler token'ı nasıl çıkarılır ve yönetilir (onay politikası, denetçiler, ücretler, mint ve yakma)
  </Card>
</Cards>

Aşağıdaki diyagram, gizli token transferlerinin temel akışına ait ayrıntılı bir
sıralamayı göstermektedir:
...
## Gizli Bakiyeler Talimatları

Gizli Bakiyeler uzantısının tam
[talimat](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
listesi aşağıdaki gibidir:

| Talimat                         | Açıklama                                                                                                                                                       |
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Bir mint account'ta Gizli Bakiyeleri etkinleştirir. Bu talimat, _rs`TokenInstruction::InitializeMint`_ talimatıyla aynı işleme dahil edilmelidir. |
| _rs`UpdateMint`_                    | Mint'in hesap onay politikasını ve isteğe bağlı global denetçi ElGamal açık anahtarını günceller.                                                                        |
| _rs`ConfigureAccount`_              | Bir token account'ta Gizli Bakiyeleri etkinleştirir.                                                                                                                 |
| _rs`ApproveAccount`_                | Mint, yeni token account'lar için onay gerektiriyorsa bir token account'ı Gizli Bakiyeler için onaylar.                                                          |
| _rs`EmptyAccount`_                  | Bir token account'ı kapatmaya izin vermek için bekleyen ve kullanılabilir gizli bakiyeleri boşaltır.                                                                         |
| _rs`Deposit`_                       | Genel token bakiyesini bekleyen gizli bakiyeye dönüştürür.                                                                                                  |
| _rs`Withdraw`_                      | Kullanılabilir gizli bakiyeyi tekrar genel bakiyeye dönüştürür.                                                                                                   |
| _rs`Transfer`_                      | Token'ları token account'lar arasında gizli olarak aktarır.                                                                                                           |
| _rs`ApplyPendingBalance`_           | Yatırma işlemleri veya transferlerin ardından bekleyen bakiyeyi kullanılabilir bakiyeye dönüştürür.                                                                                      |
...
| _rs`TransferWithFee`_               | Token'ları token account'lar arasında ücretli olarak gizli biçimde aktarır.                                                                                                |
| _rs`ConfigureAccountWithRegistry`_  | _rs`VerifyPubkeyValidity`_ kanıtı yerine _rs`ElGamalRegistry`_ hesabı kullanarak token account'ları Gizli Bakiyeler için yapılandırmanın alternatif yolu.         |
