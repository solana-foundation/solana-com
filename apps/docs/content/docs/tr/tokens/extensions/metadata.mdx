...
description:
  MetadataPointer ve TokenMetadata uzantılarını kullanarak metadata'yı doğrudan mint hesabında saklayan bir mint oluşturun, ardından token-metadata arayüzü üzerinden bu metadata'yı güncelleyin, kaldırın ve kilitleyin.
...
Aşağıdaki `Kit` örneği, oluşturulan talimatları doğrudan kullanır. `@solana/web3.js`, `@solana/spl-token` ve `@solana/spl-token-metadata` kullanan eski örnekler referans olarak eklenmiştir. Eski örnekler, web3.js v1 [v1 işlemlerini](/docs/core/transactions/versioned-transactions#v1-message-format) seri dışı bırakamadığından `maxSupportedTransactionVersion` değerini `0` ile sınırlandırır.
