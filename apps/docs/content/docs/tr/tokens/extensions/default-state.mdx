...
description:
  Token Extensions Program mint'ini, yeni oluşturulan token hesaplarının
  dondurulmuş gibi seçilmiş bir varsayılan durumda başlaması için yapılandırın.
...
## Varsayılan Hesap Durumu Nedir?

Token Extensions Program'ın _rs`DefaultAccountState`_ mint uzantısı, o mint için
oluşturulan her yeni token account için başlangıç _rs`AccountState`_ değerini
ayarlar.

Bu en yaygın olarak yeni token hesaplarının dondurulmuş durumda başlaması için
kullanılır. Dondurulmuş durumda başlayan token hesapları, mint'in freeze yetkisi
bunları çözene kadar kullanılamaz.

## Varsayılan Hesap Durumuna Sahip Mint Nasıl Oluşturulur

Varsayılan hesap durumuna sahip bir mint oluşturmak için:

1. Mint için gereken mint hesap boyutunu ve rent'i ve _rs`DefaultAccountState`_
   uzantısını hesaplayın.
2. _rs`CreateAccount`_ ile mint hesabını oluşturun, _rs`DefaultAccountState`_
   uzantısını başlatın ve mint'i _rs`InitializeMint`_ ile başlatın.
3. Mint için token hesapları oluşturun ve her yeni token hesabının mevcut
   varsayılan durumda başladığını gözlemleyin.
4. Gelecekteki token hesaplarının başlaması gereken durumu değiştirmek için
   _rs`UpdateDefaultAccountState`_ kullanın.

<ScrollyCoding>

## !!steps Hesap boyutunu hesaplayın

Temel mint artı _rs`DefaultAccountState`_ uzantısı için mint hesap boyutunu
...
Mint artı _rs`DefaultAccountState`_ uzantısı için gereken boyutu kullanarak
rent'i hesaplayın.
...
Hesaplanan alan ve lamport'larla mint hesabını oluşturun.
...
## !!steps DefaultAccountState'i Başlat

Mint üzerinde _rs`DefaultAccountState`_ uzantısını başlatın.
...
## !!steps Token hesabı oluştur

Mint için bir token hesabı oluşturun. Mevcut varsayılan durum _rs`Frozen`_
olduğundan, yeni token hesabı dondurulmuş olarak başlar.
...
## !!steps Varsayılan hesap durumunu güncelle

Gelecekteki token hesaplarının artık dondurulmuş olarak başlamaması için
varsayılan durumu değiştirmek üzere _rs`UpdateDefaultAccountState`_ kullanın.
...
<Callout type="info" title="Talimat Sırası">
  _rs`DefaultAccountStateInstruction::Initialize`_, _rs`InitializeMint`_'den
  önce gelmelidir. _rs`CreateAccount`_,
  _rs`DefaultAccountStateInstruction::Initialize`_ ve _rs`InitializeMint`_ aynı
...
| Öğe                                              | Açıklama                                                                                                  | Kaynak                                                                                                                                             |
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`DefaultAccountState`_                        | Yeni token hesaplarının devralması gereken durumu saklayan mint uzantısı.                                 | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/mod.rs#L16-L21)         |
| _rs`DefaultAccountStateInstruction::Initialize`_ | _rs`InitializeMint`_'den önce mint'in varsayılan token hesap durumunu başlatan talimat.                   | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L20-L36) |
| _rs`DefaultAccountStateInstruction::Update`_     | Dondurma yetkisinin gelecekteki token hesapları için varsayılan durumu değiştirmesine izin veren talimat. | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L37-L53) |
| _rs`process_initialize_default_account_state`_   | Başlatılmamış bir mint üzerine başlangıç varsayılan durumunu yazan işlemci mantığı.                       | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L29-L40)     |
| _rs`process_update_default_account_state`_       | Mint'in saklanan varsayılan durumunu değiştirmeden önce dondurma yetkisini doğrulayan işlemci mantığı.    | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L43-L74)     |

### Typescript

Aşağıdaki `Kit` örneği, üretilen talimatları doğrudan kullanır.
