---
title: Token Yakma
description: Token hesabından token'ları yakın ve mint'in toplam arzını azaltın.
...
Token yakmak, bir token hesabı bakiyesini kalıcı olarak azaltır ve mint'in
toplam arzını aynı miktarda düşürür.

## Token'lar Nasıl Yakılır

Token yakmak, Token Program'ın _rs`Burn`_ veya _rs`BurnChecked`_ talimatını
kullanır.

Aşağıdaki örnekler, çağıranın mint'in ondalık sayısını sağlamasını gerektiren
_rs`BurnChecked`_ kullanır; böylece talimat yakmadan önce beklenen token
hassasiyetini doğrulayabilir.

[Native mint](/docs/tokens/basics/sync-native) yakmayı desteklemez. Bunun yerine
_rs`CloseAccount`_ kullanın.

Token hesabı sahibi veya onaylı bir delege işlemi imzalar. Token Extensions
Program'da, mint kalıcı delege uzantısı etkinleştirilmişse, bir mint kalıcı
delegesi de yakmaya yetki verebilir.

### Kaynak Referansı

| Öğe                | Açıklama                                                                                                                                                      | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_         | Mint durumu, toplam token arzını depolar.                                                                                                                     | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | Token hesap durumu, yakma ile azaltılan token bakiyesini depolar.                                                                                             | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Çağıranın mint'in ondalık sayısını sağlamasını gerektirmeden bir token hesap bakiyesini ve mint arzını azaltan yakma talimatı.                                | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Çağıranın mint'in ondalık sayısını sağlamasını gerektiren ve bir token hesap bakiyesini ve mint arzını azaltmadan önce bu değeri kontrol eden yakma talimatı. | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | Token yakma için paylaşılan işlemci mantığı.                                                                                                                  | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |
