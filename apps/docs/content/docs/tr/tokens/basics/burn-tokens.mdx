---
title: Token Yakma
description: Bir token hesabından token yakın ve mint'in toplam arzını azaltın.
...
Token yakmak, bir token hesap bakiyesini kalıcı olarak azaltır ve mint'in toplam
arzını aynı miktarda düşürür.

## Token Nasıl Yakılır

Token yakmak için Token Program'ın _rs`Burn`_ veya _rs`BurnChecked`_ talimatı
kullanılır.

Aşağıdaki örnekler _rs`BurnChecked`_ kullanır; bu talimat, yakmadan önce
beklenen token hassasiyetini doğrulayabilmek için çağıranın mint'in ondalık
basamak sayısını sağlamasını gerektirir.

[Native mint](/docs/tokens/basics/sync-native) yakmayı desteklemez. Bunun yerine
_rs`CloseAccount`_ kullanın.

Token hesap sahibi veya onaylanmış bir delege işlemi imzalar. Token Extensions
Program'da, mint'te kalıcı delege uzantısı etkinleştirilmişse, bir mint kalıcı
delegesi de yakmayı yetkilendirebilir.

### Kaynak Referansı

| Öğe                | Açıklama                                                                                                                                                               | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_         | Mint durumu, toplam token arzını saklar.                                                                                                                               | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | Token hesap durumu, bir yakma işlemiyle azaltılan token bakiyesini saklar.                                                                                             | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Çağıranın mint'in ondalık basamak sayısını sağlamasını gerektirmeden bir token hesap bakiyesini ve mint arzını azaltan yakma talimatı.                                 | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Çağıranın mint'in ondalık basamak sayısını sağlamasını gerektiren ve bir token hesap bakiyesini ile mint arzını azaltmadan önce bu değeri kontrol eden yakma talimatı. | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | Token yakma işlemleri için paylaşılan işlemci mantığı.                                                                                                                 | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |
