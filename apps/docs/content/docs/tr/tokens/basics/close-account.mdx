...
description:
  Bir token hesabını kapatın ve hesabın lamport'larını bir hedef hesaba iade
  edin.
...
Bir token hesabını kapatmak, hesabı siler ve token hesabının rent lamport'larını
bir hedef hesaba iade eder.

Hesap kapatılmadan önce token bakiyesi sıfır olmalıdır. Dondurulmuş bir hesap,
token bakiyesi sıfır ise yine de kapatılabilir. Sarılmış SOL token hesapları
istisnadır ve alttaki SOL'u geri almak için token bakiyesi ile kapatılabilir.
Hesap sahibi veya hesabın kapatma yetkisi, kapatma talimatını imzalar.
