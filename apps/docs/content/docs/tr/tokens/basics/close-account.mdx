---
title: Token Hesabını Kapatma
description:
  Bir token hesabını kapatın ve hesabın lamport'larını bir hedef hesaba iade edin.
...
## Token Hesabını Kapatmak Ne İşe Yarar?

Bir token account kapatıldığında hesap silinir ve geri alınabilir minimum depolama bakiyesi dahil tüm lamport'lar bir hedef hesaba aktarılır.

Hesap kapatılabilmesi için token bakiyesinin sıfır olması gerekir. Dondurulmuş bir hesabın token bakiyesi sıfırsa, o hesap yine de kapatılabilir. Wrapped SOL token account'ları istisnadır ve temel alınan SOL'u geri almak amacıyla token bakiyesiyle birlikte kapatılabilir. Hesap sahibi veya hesabın kapatma yetkisi, kapatma talimatını imzalar ve hedefi belirler. Bir ATA oluşturmak için ödeme yapan hesap, kurtarılan bakiyeyi otomatik olarak almaz.

## Token Hesabı Nasıl Kapatılır

Bir token hesabını kapatmak, Token Program'ın _rs`CloseAccount`_ talimatını
kullanır.

_rs`CloseAccount`_ talimatı, kaynak hesabın lamport'larını bir hedef hesaba
transfer eder, kaynak hesabı temizler ve kaynak hesabı siler. Token Extension
Program, belirli uzantılar için ek kapatma kontrolleri uygular, ancak temel
kapatma akışı aynıdır.

### Kaynak Referansı

| Öğe                        | Açıklama                                                                                                     | Token Program                                                                                                  | Token Extension Program                                                                                              |
| --------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | Token hesap durumu, hesap kapatma sırasında kullanılan bakiye, yerel durum ve kapatma yetkisini saklar.     | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | Bir token hesabını kapatan ve token hesabının lamport'larını bir hedef hesaba transfer eden talimat. | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | Token hesabı kapatma için paylaşılan işlemci mantığı.                                                               | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |
