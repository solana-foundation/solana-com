...
description:
  Belirli bir basım ve sahip için token hesapları ve ilişkili token hesapları oluşturun.
...
Token hesabı, Solana üzerinde bir basım ve bir token hesabı sahibi için
tokenları saklar.
...
Burada _rs`owner`_, token hesabından token transfer etme, yakma veya devretme
...
tokenın birimlerini tutabilir; bu token, token hesabının _rs`mint`_ alanı
tarafından tanımlanan tokendır.

## Associated Token Account Nedir

Associated token account (ATA), bir cüzdan ve basım için varsayılan token
hesabıdır.
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program/src),
ATA adresini cüzdan adresi, token program adresi ve basım adresinden türetir.

Yalnızca Associated Token Program tarafından oluşturulan token hesaplarına
associated token account denir.

Associated Token Program, standart ve deterministik bir adreste token hesabı
oluşturmanın bir yoludur. Ortaya çıkan hesap, Associated Token Program'a değil,
Token Program veya Token Extensions Program'a ait bir token account olmaya devam
eder.

Associated Token Program, ATA adresini
...
dosyasında gösterildiği şekilde türetir:
...
Program tarafından tanımlanan _rs`Account`_ türünü kullanır.

## Maliyet ve Ödeyici

Bir ATA oluşturmak, hesabın zincir üstü depolaması için iade edilebilir minimum bir bakiye ve işlem ücreti gerektirir. Oluşturma talimatına iletilen `payer`, depolama bakiyesini karşılar ve imzalaması gerekir. Ödeyici; işlem ücreti ödeyicisinden ve ATA'ya sahip olacak cüzdandan farklı olabilir; bu sayede bir uygulama, alıcı adına ATA oluşturabilir.

Mevcut küme depolama oranında temel maliyetler şunlardır:

| Token programı                             | ATA boyutu  | Minimum depolama bakiyesi             |
| ----------------------------------------- | --------- | ----------------------------------- |
| Original Token Program                    | 165 bayt | 2.039.280 lamport (0,00203928 SOL) |
| Token-2022 otomatik değişmez sahip ile | 170 bayt | 2.074.080 lamport (0,00207408 SOL) |

Token-2022 mint'leri, otomatik _rs`ImmutableOwner`_ uzantısına ek olarak hesap uzantıları gerektirebilir. Bu tür ATA'lar daha büyüktür ve daha yüksek bir minimum bakiye gerektirir. Associated Token Program, gerekli hesap boyutunu seçilen Token Program'dan sorgular ve ATA'yı buna göre finanse eder.

Bu bakiyeleri uygulama mantığına sabit kodlamayın. Gerekli veri boyutu için
[`getMinimumBalanceForRentExemption`](/docs/rpc/http/getminimumbalanceforrentexemption)
sorgulayın veya oluşturma sırasında Associated Token Program'ın hesaplamasına ve finanse etmesine izin verin.

<Callout type="info" title="Depolama bakiyesi iade edilebilir">
  Boş bir token account'ı kapatmak, minimum depolama bakiyesi dahil tüm lamport'larını, kapatma talimatında belirtilen hedefe aktarır. Orijinal oluşturma ödeyicisi otomatik olarak iade hedefi değildir. Bkz. [Close Token Account](/docs/tokens/basics/close-account).
</Callout>

## İlişkili Token Hesabı Nasıl Oluşturulur

İlişkili bir token hesabı oluşturmak, Associated Token Program'ın _rs`Create`_
veya _rs`CreateIdempotent`_ talimatını kullanır. Associated Token Program, ATA
...
<Callout type="info" title="Önerilen">
  Çoğu uygulama için, token hesaplarını doğrudan System Program ve Token Program
  talimatlarını çağırarak oluşturmak yerine Associated Token Program
  aracılığıyla oluşturun. İlişkili bir token hesabı, sahip, token programı ve
  mint'ten türetilen deterministik bir adres kullanır; bu da belirli bir mint
  için varsayılan token hesabının cüzdanlar ve uygulamalar tarafından
...
| Öğe                                          | Açıklama                                                                                                                                                                                                                                                    | Kaynak                                                                                                                          |
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| _rs`Create`_                                  | Türetilen ATA adresinde ilişkili bir token hesabı oluşturur.                                                                                                                                                                                                | [Kaynak](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L20-L29) |
| _rs`CreateIdempotent`_                        | İlişkili token hesabını oluşturur, ancak aynı sahip ve mint için söz konusu ATA zaten mevcutsa yine de başarılı olur.                                                                                                                                               | [Kaynak](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | ATA adresini türetir ve token hesabını başlatmak için seçilen token programına _rs`create_pda_account`_ artı CPI'ları kullanır. Seçilen program Token Extensions Program olduğunda, ATA işlemcisi ayrıca değiştirilemez sahip uzantısını da başlatır. | [Kaynak](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | _rs`process_create_associated_token_account`_ tarafından PDA hesabını System Program'a CPI ile oluşturmak için kullanılan yardımcı.                                                                                                                                         | [Kaynak](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |
...
1. System Program'ın _rs`CreateAccount`_ instruction'ı, yeni bir rent-muaf hesap
   oluşturur ve Token Program'ı yeni hesabın program sahibi olarak atar.
2. Token Program'ın _rs`InitializeAccount`_, _rs`InitializeAccount2`_ veya
   _rs`InitializeAccount3`_ instruction'ı, yeni hesabı bir mint ve sahip için
   başlatır.

_rs`CreateAccount`_ instruction'ını ve token hesabı başlatma instruction'ını
aynı transaction içinde ekleyin.

Token hesabı başlatma sırasında, Token Program hesabın henüz başlatılmamış
olduğunu ve rent-muaf olduğunu kontrol eder.

<Callout type="info">
  Aşağıdaki bölüm, System Program ve Token Program instruction'larını doğrudan
  çağırarak bir token hesabının nasıl oluşturulacağını gösterir. Çoğu uygulama
  için bunun yerine Associated Token Program'ı kullanın. System Program ve Token
  Program'a doğrudan çağrıları yalnızca associated token account kullanmamak
  için belirli bir nedeniniz olduğunda veya kendi Solana programınızdan System
  Program ve Token Program instruction'larına CPI yaparak özel PDA token
  hesapları oluşturmanız gerektiğinde kullanın.
</Callout>
...
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_                     | Her token hesabında saklanan temel token hesabı alanları.                                                                       | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`InitializeAccount`_           | Hesaplar listesinde sahip ve rent sysvar hesabı bekleyen bir token hesabı başlatma instruction'ı.                    | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L45-L63)   | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L75-L93)   |
| _rs`InitializeAccount2`_          | Sahibi hesaplar listesi yerine instruction data içinde ileten bir token hesabı başlatma instruction'ı.                 | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L352-L365) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L419-L433) |
| _rs`InitializeAccount3`_          | Sahibi instruction data içinde ileten ve rent sysvar hesabı gerektirmeyen bir token hesabı başlatma instruction'ı. | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L377-L387) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L445-L456) |
| _rs`_process_initialize_account`_ | Token hesabı başlatma için paylaşılan işlemci mantığı.                                                                           | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L83-L141)      | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L153-L234)     |
| _rs`process_initialize_account`_  | _rs`InitializeAccount`_ için genel işleyici.                                                                                        | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L143-L150)     | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L236-L240)     |
| _rs`process_initialize_account2`_ | _rs`InitializeAccount2`_ için genel işleyici.                                                                                       | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L152-L160)     | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L242-L246)     |
| _rs`process_initialize_account3`_ | _rs`InitializeAccount3`_ için genel işleyici.                                                                                       | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L162-L170)     | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L248-L252)     |
