...
description:
  Belirli bir mint ve sahip için token hesapları ve ilişkili token hesapları
  oluşturun.
...
Token hesabı, Solana üzerinde bir mint ve bir token hesabı sahibi için
token'ları saklar.
...
Burada _rs`owner`_, token hesabından token'ları transfer edebilen, yakabilen
veya devredebilen yetkiyi ifade eder. Hesabın program sahibi hala Token Program
veya Token Extensions Program'dır.

Her token hesabı tam olarak bir mint'e bağlıdır, yani token hesabı yalnızca bir
token'ın birimlerini tutabilir - token hesabının _rs`mint`_ alanı tarafından
tanımlanan token.

## İlişkili Token Hesabı Nedir

İlişkili token hesabı (ATA), bir cüzdan ve mint için varsayılan token hesabıdır.
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program/src),
ATA adresini cüzdan adresi, token program adresi ve mint adresinden türetir.

Yalnızca Associated Token Program tarafından oluşturulan token hesapları
ilişkili token hesapları olarak adlandırılır.
...
Token Program veya Token Extensions Program'a ait olan bir token hesabıdır.
...
dosyasında gösterildiği gibi türetir:
...
Herhangi bir cüzdan, token program ve mint kombinasyonu için tam olarak bir ATA
adresi vardır. Associated Token Program bu adreste standart bir token hesabı
oluşturur ve ortaya çıkan hesap hala Token Program veya Token Extensions Program
tarafından tanımlanan _rs`Account`_ türünü kullanır.

## İlişkili Token Hesabı Nasıl Oluşturulur

İlişkili token hesabı oluşturmak, Associated Token Program'ın _rs`Create`_ veya
_rs`CreateIdempotent`_ talimatını kullanır. Associated Token Program, ATA
adresini türetir, ATA adresinde hesap oluşturur ve hesabı Token Program veya
Token Extension Program'a ait bir token hesabı olarak başlatır.
...
<Callout type="info" title="Önerilen">
  ...
  aracılığıyla oluşturun. İlişkili token hesabı, sahibinden, token programından
  ve mint'ten türetilen deterministik bir adres kullanır, bu da belirli bir mint
  için varsayılan token hesabını cüzdanların ve uygulamaların bulmasını
  kolaylaştırır.
</Callout>
...
| Öğe                                           | Açıklama                                                                                                                                                                                                                                     | Kaynak                                                                                                                          |
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| _rs`Create`_                                  | Türetilen ATA adresinde ilişkili token hesabı oluşturur.                                                                                                                                                                                     | [Kaynak](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L20-L29) |
| _rs`CreateIdempotent`_                        | İlişkili token hesabını oluşturur, ancak aynı sahip ve mint için ATA zaten mevcutsa yine de başarılı olur.                                                                                                                                   | [Kaynak](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | ATA adresini türetir ve token hesabını başlatmak için seçilen token programa _rs`create_pda_account`_ artı CPI'ları kullanır. Seçilen program Token Extension Program olduğunda, ATA işlemcisi ayrıca değişmez sahip uzantısını da başlatır. | [Kaynak](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | System Program'a CPI yaparak PDA hesabını oluşturmak için _rs`process_create_associated_token_account`_ tarafından kullanılan yardımcı.                                                                                                      | [Kaynak](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |

### Typescript

Aşağıdaki `Kit` örnekleri, `@solana/kit` kullanılarak önerilen yaklaşımı
göstermektedir. `@solana/web3.js` kullanan eski örnekler referans için dahil
edilmiştir.
...
Bir token hesabı oluşturmak için iki talimat gerekir:

1. System Program'ın _rs`CreateAccount`_ talimatı, yeni bir rent muafiyetli
   hesap oluşturur ve Token Program'ı yeni hesabın program sahibi olarak atar.
2. Token Program'ın _rs`InitializeAccount`_, _rs`InitializeAccount2`_ veya
   _rs`InitializeAccount3`_ talimatı, yeni hesabı bir mint ve sahip için
   başlatır.

_rs`CreateAccount`_ talimatını ve token hesabı başlatma talimatını aynı işleme
dahil edin.

Token hesabı başlatma sırasında Token Program, hesabın zaten başlatılmadığını ve
rent muafiyetli olduğunu kontrol eder.

<Callout type="info">
  Aşağıdaki bölüm, System Program ve Token Program talimatlarını doğrudan
  çağırarak bir token hesabının nasıl oluşturulacağını gösterir. Çoğu uygulama
  için bunun yerine Associated Token Program'ı kullanın. Bir ilişkili token
  hesabı kullanmamak için özel bir nedeniniz olduğunda veya kendi Solana
  programınızdan System Program ve Token Program talimatlarına CPI'lar yaparak
  özel PDA token hesapları oluşturmanız gerektiğinde doğrudan System Program ve
  Token Program çağrılarını kullanın.
</Callout>
...
| --------------------------------- | ------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_                     | Her token hesabında depolanan temel token hesabı alanları.                                                    | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`InitializeAccount`_           | Hesap listesinde sahip ve rent sysvar hesabını bekleyen bir token hesabı başlatma talimatı.                   | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L45-L63)   | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L75-L93)   |
| _rs`InitializeAccount2`_          | Sahibi hesap listesi yerine instruction data içinde ileten bir token hesabı başlatma talimatı.                | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L352-L365) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L419-L433) |
...
Aşağıdaki `Kit` örnekleri, `@solana/kit` kullanılarak önerilen yaklaşımı
göstermektedir. Referans olması için `@solana/web3.js` kullanan eski örnekler de
dahil edilmiştir.
