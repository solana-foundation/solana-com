---
title: Token basma
description: Mint yetkisi ile bir token hesabına yeni token birimleri basın.
...
Basım, mevcut bir token'ın yeni birimlerini oluşturur ve mint'in toplam arzını
artırır.

Yalnızca mint yetkisi yeni token'lar basabilir. Hedef token account zaten mevcut
olmalı ve aynı mint'e ait olmalıdır.

## Token'lar Nasıl Basılır

Token'lar basmak için Token Program'ın _rs`MintTo`_ veya _rs`MintToChecked`_
talimatı kullanılır.

Aşağıdaki örnekler _rs`MintToChecked`_ kullanır; bu talimat, çağrıcının mint'in
ondalık basamaklarını sağlamasını gerektirir, böylece talimat basmadan önce
beklenen token hassasiyetini doğrulayabilir.

[Native mint](/docs/tokens/basics/sync-native) basımı desteklemez.

### Kaynak Referans

| Öğe                   | Açıklama                                                                                                                                                     | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_            | Mint durumu, mint yetkisini ve toplam token arzını saklar.                                                                                                   | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_         | Token account durumu, basım tarafından güncellenen hedef bakiyeyi saklar.                                                                                    | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`MintTo`_          | Mevcut bir token account'a çağrıcının mint'in ondalık basamaklarını sağlamasını gerektirmeden token basan bir basım talimatı.                                | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L159-L177) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L202-L220) |
| _rs`MintToChecked`_   | Çağrıcının mint'in ondalık basamaklarını sağlamasını gerektiren ve mevcut bir token account'a token basmadan önce bu değeri kontrol eden bir basım talimatı. | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L301-L325) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L367-L391) |
| _rs`process_mint_to`_ | Token basımı için paylaşılan işlemci mantığı.                                                                                                                | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L521-L583)     | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L981-L1067)    |

### Typescript

Aşağıdaki `Kit` örnekleri, `@solana/kit` kullanılarak önerilen yaklaşımı
gösterir. `@solana/web3.js` kullanan eski örnekler referans için dahil
edilmiştir.
