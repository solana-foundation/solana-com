...
- Yalnızca mint yetkisi yeni token basabilir
...
async fn main() -> Result<()> {
    // Yerel validator'a bağlantı oluştur
    ...
    // Fee payer için yeni bir keypair oluştur
    let fee_payer = Keypair::new();

    // Fee payer'a 1 SOL airdrop yap
    ...
    // Mint adresi olarak kullanmak için keypair oluştur
    let mint = Keypair::new();

    // Varsayılan mint account boyutunu al (byte cinsinden), hiçbir uzantı etkin değil
    let mint_space = Mint::LEN;
    let mint_rent = client
        .get_minimum_balance_for_rent_exemption(mint_space)
        .await?;

    // Mint için yeni account oluşturma talimatı (token program)
    let create_account_instruction = create_account(
        &fee_payer.pubkey(), // payer
        &mint.pubkey(),      // yeni account (mint)
        mint_rent,           // lamports
        mint_space as u64,   // space
        &token_program_id(), // program id
    );

    // Mint account verisini başlatma talimatı
    ...
    // Transaction oluştur ve talimatları ekle
    ...
    // Transaction'ı gönder ve onayla
    client.send_and_confirm_transaction(&transaction).await?;

    // Mint edilecek token miktarı (2 ondalık basamaklı 100 token)
    let amount = 100;

    // Associated token account'a token mint etmek için mint_to talimatı oluştur
    let mint_to_instruction = mint_to(
        &token_program_id(),
        &mint.pubkey(),            // mint
        &associated_token_address, // hedef
        &fee_payer.pubkey(),       // yetki
        &[&fee_payer.pubkey()],    // imzalayan
        amount,                    // miktar
    )?;

    // Token mint etmek için transaction oluştur
    let transaction = Transaction::new_signed_with_payer(
        &[mint_to_instruction],
        Some(&fee_payer.pubkey()),
        &[&fee_payer],
    ...
    println!("Associated token account'a 1.00 token mint edildi");
    println!("\nMint adresi: {}", mint.pubkey());
    println!("{:#?}", mint_data);

    println!(
        "\nAssociated token account adresi: {}",
        associated_token_address
    );
    println!("{:#?}", token_data);

    println!("Transaction imzası: {}", transaction_signature);
...
async def main():
    ...
    # Basılacak miktar (en küçük birimde)
    amount_to_mint = 1000000000  # 9 ondalık basamaklı 1 token

    async with rpc:
        # Basım talimatı oluştur
        ...
        print(f"Hedef: {destination_token_account}")
        print(f"Miktar: {amount_to_mint}")
        print(f"Mint yetkisi: {mint_authority.pubkey()}")
        print(f"Ödeyici: {payer.pubkey()}")
        print(f"Basım işlemi başarıyla oluşturuldu")
