...
Basım, mevcut bir tokenın yeni birimlerini oluşturur ve mint'in toplam arzını
artırır.

Yalnızca mint yetkisi yeni token basabilir. Hedef token account zaten mevcut
olmalı ve aynı mint'e ait olmalıdır.

## Token Nasıl Basılır

Token basımı, Token Program'ın _rs`MintTo`_ veya _rs`MintToChecked`_ komutunu
kullanır.

Aşağıdaki örnekler _rs`MintToChecked`_ kullanır; bu komut, çağrıcının mint'in
ondalık sayısını sağlamasını gerektirir, böylece komut basmadan önce beklenen
token hassasiyetini doğrulayabilir.
...
| _rs`Account`_         | Token account durumu, basım ile güncellenen hedef bakiyeyi saklar.                                                                                    | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`MintTo`_          | Çağrıcının mint'in ondalık sayısını sağlamasını gerektirmeden mevcut bir token account'a token basan bir basım komutu.                                | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L159-L177) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L202-L220) |
| _rs`MintToChecked`_   | Çağrıcının mint'in ondalık sayısını sağlamasını gerektiren ve mevcut bir token account'a token basmadan önce bu değeri kontrol eden bir basım komutu. | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L301-L325) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L367-L391) |
