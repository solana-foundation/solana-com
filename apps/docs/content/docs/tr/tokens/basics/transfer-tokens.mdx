...
description:
  Aynı mint için token hesapları arasında Token Program transfer talimatlarıyla
  token'ları taşıyın.
...
## Token Transfer Etmek Ne İşe Yarar?

Token transfer etmek, aynı mint için bir token account'tan başka bir token
account'a token'ları taşır.

Transferler mint arzını değiştirmez. Transferler yalnızca token account'ları
arasındaki bakiyeleri günceller.

## Token'lar Nasıl Transfer Edilir

Token transferi, Token Program'ın _rs`Transfer`_ veya _rs`TransferChecked`_
talimatını kullanır.

Aşağıdaki örnekler, çağıranın mint ve ondalık basamakları sağlamasını gerektiren
_rs`TransferChecked`_ kullanır, böylece talimat hesaplar arasında token'ları
...
Extensions Program'da, mint kalıcı delege uzantısı etkinleştirilmişse, bir mint
kalıcı delegesi de transferi yetkilendirebilir.
...
| _rs`Transfer`_         | Çağıranın mint veya ondalık basamakları sağlamasını gerektirmeden token account'lar arasında token'ları taşıyan bir transfer talimatı. Token Extensions Program'da bunun yerine _rs`TransferChecked`_ kullanın. | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Çağıranın mint ve ondalık basamakları sağlamasını gerektiren ve aynı mint'e ait token account'lar arasında token'ları taşımadan önce bu değerleri kontrol eden bir transfer talimatı.                           | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
