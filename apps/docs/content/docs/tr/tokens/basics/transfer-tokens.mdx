...
description:
  Aynı mint için Token Program transfer talimatlarıyla token hesapları arasında
  token taşıyın.
...
## Token Transferi Ne İşe Yarar?

Token transferi, aynı mint için bir token account'tan başka bir token account'a
token taşır.

Transferler mint arzını değiştirmez. Transferler yalnızca token account'lar
arasındaki bakiyeleri günceller.

## Token Transferi Nasıl Yapılır

Token transferi, Token Program'ın _rs`Transfer`_ veya _rs`TransferChecked`_
talimatını kullanır.

Aşağıdaki örnekler _rs`TransferChecked`_ kullanır; bu talimat, çağıranın mint ve
ondalık sayıları sağlamasını gerektirir, böylece talimat hesaplar arasında token
taşımadan önce beklenen mint'i ve token hassasiyetini doğrulayabilir.

Token Extensions Program'da _rs`Transfer`_, _rs`TransferChecked`_ veya
_rs`TransferCheckedWithFee`_ lehine kullanımdan kaldırılmıştır.

Kaynak hesap sahibi veya onaylanmış bir delege transferi imzalar. Token
Extensions Program'da, mint'te kalıcı delege uzantısı etkinleştirilmişse, bir
mint kalıcı delegesi de transferi yetkilendirebilir.

### Kaynak Referansı

| Öğe                    | Açıklama                                                                                                                                                                                                | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_          | token account durumu, bir transfer tarafından güncellenen bakiyeleri saklar.                                                                                                                            | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | Çağıranın mint veya ondalık sayıları sağlamasını gerektirmeden token account'lar arasında token taşıyan bir transfer talimatı. Token Extensions Program'da bunun yerine _rs`TransferChecked`_ kullanın. | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Çağıranın mint ve ondalık sayıları sağlamasını gerektiren ve aynı mint'e ait token account'lar arasında token taşımadan önce bu değerleri kontrol eden bir transfer talimatı.                           | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | Token transferleri için paylaşılan işlemci mantığı.                                                                                                                                                     | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |

### Typescript

Aşağıdaki `Kit` örnekleri, `@solana/kit` kullanılarak önerilen yaklaşımı
göstermektedir. Referans olması açısından `@solana/web3.js` kullanan eski
