...
işleminden sonra, delege artık hesabınızdan herhangi bir token transfer edemez.
Token Program için, yalnızca token hesabı sahibi bir delegeyi iptal edebilir.
Token Extension Program için ise token hesabı sahibi ve mevcut delege,
delegasyonu iptal edebilir.
