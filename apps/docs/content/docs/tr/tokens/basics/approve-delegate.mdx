...
description:
  Bir token hesabından sınırlı miktarda token transferi veya yakılması için
  başka bir adresi yetkilendirin.
...
## Delegeyi Onaylamak Ne İşe Yarar?

Bir delegeyi onaylamak, başka bir adresin token hesabı sahibi adına token
hesabından sınırlı miktarda token transfer etmesine veya yakmasına izin verir.

Bir token hesabı aynı anda bir mevcut delege ve bir devredilebilir miktar
saklar. Yeni bir delegeyi onaylamak, hesaptaki önceki delegeyi ve izni
değiştirir.
...
değerlerini sağlamasını gerektirir, böylece komut onayı işlemeden önce beklenen
mint ve token hassasiyetini doğrulayabilir.
...
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_         | token hesabı durumu, sahibi, delegeyi ve devredilebilir miktarı saklar.                                                                              | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | Çağrıcının mint veya ondalık değerlerini sağlamasını gerektirmeden bir token hesabı için delegeyi ve izni kaydeden bir delege onay komutu.           | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | Çağrıcının mint ve ondalık değerlerini sağlamasını gerektiren ve delegeyi ve izni kaydetmeden önce bu değerleri kontrol eden bir delege onay komutu. | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
