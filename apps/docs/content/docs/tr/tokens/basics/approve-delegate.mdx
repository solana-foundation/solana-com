---
title: Delege Onaylama
description:
  Başka bir adresin token hesabından sınırlı miktarda token transfer etmesine
  veya yakmasına yetki verin.
...
## Delege Onaylamak Ne İşe Yarar?

Delege onaylamak, başka bir adresin token hesabı sahibi adına token hesabından
sınırlı miktarda token transfer etmesine veya yakmasına izin verir.

Bir token account aynı anda bir mevcut delege ve bir delege edilmiş miktar
saklar. Yeni bir delege onaylamak, hesaptaki önceki delegeyi ve izin miktarını
değiştirir.

## Delege Nasıl Onaylanır

Delege onaylamak, Token Program'ın _rs`Approve`_ veya _rs`ApproveChecked`_
talimatını kullanır.

Aşağıdaki örnekler _rs`ApproveChecked`_ kullanır; bu talimat, çağrıcının mint ve
ondalık basamakları sağlamasını gerektirir, böylece talimat onayı işlemeden önce
beklenen mint ve token hassasiyetini doğrulayabilir.

Token hesabı sahibi onayı imzalar.

### Kaynak Referansı

| Öğe                   | Açıklama                                                                                                                                                         | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_         | Token account durumu, sahibi, delegeyi ve delege edilmiş miktarı saklar.                                                                                         | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | Çağrıcının mint veya ondalık basamakları sağlamasını gerektirmeden bir token account için delegeyi ve izin miktarını kaydeden delege onaylama talimatı.          | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | Çağrıcının mint ve ondalık basamakları sağlamasını gerektiren ve delegeyi ve izin miktarını kaydetmeden önce bu değerleri kontrol eden delege onaylama talimatı. | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
| _rs`process_approve`_ | Delege onaylama için paylaşılan işlemci mantığı.                                                                                                                 | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L344-L392)     | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L582-L640)     |
