...
title: Hesabı Çöz
description:
  Transferlerin, yakmaların ve delege değişikliklerinin devam edebilmesi için
  dondurulmuş bir token hesabını çözün.
...
## Token Hesabını Çözmek Ne İşe Yarar?

Çözme, bir token hesabını dondurulmuş durumdan geri başlatılmış duruma getirir.

Çözme başarılı olduktan sonra, hesap yeniden token transferi yapabilir,
yakabilir ve kapatabilir. Yalnızca mint'in dondurma yetkisi token hesaplarını
çözebilir.

## Token Hesabı Nasıl Çözülür

Bir token hesabını çözmek, Token Program'ın _rs`ThawAccount`_ talimatını
kullanır.

_rs`ThawAccount`_ talimatı, token hesabının mint'e ait olduğunu ve imzalayanın
mint'in dondurma yetkisi olduğunu doğrular, ardından token hesabı durumunu
dondurulmuştan geri başlatılmışa değiştirir.

[Yerel token hesapları](/docs/tokens/basics/sync-native) çözmeyi desteklemez.

### Kaynak Referansı

| Öğe                                 | Açıklama                                                                                      | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ----------------------------------- | --------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Mint durumu, token hesaplarını çözmek için kullanılan isteğe bağlı dondurma yetkisini saklar. | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | _rs`Frozen`_ ve _rs`Initialized`_ dahil olmak üzere token hesap durumları.                    | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`ThawAccount`_                   | Dondurulmuş bir token hesabını geri başlatılmış duruma değiştiren bir talimat.                | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L229-L243) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L292-L306) |
| _rs`process_toggle_freeze_account`_ | Token hesaplarını dondurma ve çözme için paylaşılan işlemci mantığı.                          | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |
