...
Çözme işlemi, bir token hesabını donmuş durumdan başlatılmış duruma geri
döndürür.

Dondurma işlemi başarıyla geri alındıktan sonra, hesap yeniden token alabilir,
transfer edebilir ve yakabilir. Yalnızca mint'in dondurma yetkisi, token
hesaplarını geri çözebilir.

## Token Hesabı Nasıl Çözülür

Bir token hesabını çözmek için Token Program'ın _rs`ThawAccount`_ talimatı
kullanılır.

_rs`ThawAccount`_ talimatı, token hesabının mint'e ait olduğunu ve imzalayanın
mint'in dondurma yetkisi olduğunu doğrular, ardından token hesabının durumunu
donmuş durumdan başlatılmış duruma geri döndürür.

[Yerel token hesapları](/docs/tokens/basics/sync-native) çözme işlemini
desteklemez.
