...
title: Hesabı Dondur
description:
  Token hesabını dondurarak transferleri, yakma işlemlerini ve yetki
  değişikliklerini engellemek için mint'in dondurma yetkisini kullanın.
...
## Token Hesabını Dondurmak Ne İşe Yarar?

Bir token hesabını dondurmak, aynı sahibi, mint'i ve bakiyeyi korur, ancak token
hesabı çözülene kadar token'ların transferini veya yakılmasını engeller.

Dondurulmuş bir hesap, sıfır token bakiyesine sahipse yine de kapatılabilir.

Yalnızca hala dondurma yetkisine sahip bir mint, token hesaplarını dondurabilir.
Bir mint'in dondurma yetkisi iptal edilirse, o mint için hesaplar artık
dondurulamaz.

## Token Hesabı Nasıl Dondurulur

Bir token hesabını dondurmak, Token Program'ın _rs`FreezeAccount`_ talimatını
kullanır.

_rs`FreezeAccount`_ talimatı, token hesabının mint'e ait olduğunu ve imzalayanın
mint'in dondurma yetkisi olduğunu doğrular, ardından token hesabı durumunu
başlatılmış durumundan dondurulmuş durumuna değiştirir.

[Yerel token hesapları](/docs/tokens/basics/sync-native) dondurma işlemini
desteklemez.
...
| ----------------------------------- | ------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Mint durumu, token hesaplarını dondurmak için kullanılan isteğe bağlı dondurma yetkisini saklar. | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | _rs`Initialized`_ ve _rs`Frozen`_ dahil olmak üzere token hesabı durumları.                      | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | Bir token hesabını dondurulmuş duruma değiştiren bir talimat.                                    | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
...
göstermektedir. `@solana/web3.js` kullanan eski örnekler referans olarak dahil
edilmiştir.
