...
title: Sync Native
description:
  Hesaba SOL aktardıktan sonra sarılmış SOL token hesabı miktarını güncelleyin.
...
Sarılmış SOL (WSOL), Token Program'ın native mint'i için bir token hesabında
tutulan SOL'dur. Native mint, Token Program'ın SOL'u sarmalayan token hesapları
için kullandığı mint adresidir. Native mint, SOL'un Token Program'ın talimatları
aracılığıyla kullanılmasını sağlar.

| Program                  | Native mint adresi                             |
| ------------------------ | ---------------------------------------------- |
...
aracılığıyla bir token bakiyesi olarak takip eder. Sarılmış bir SOL token
hesabına SOL transfer ederseniz, lamport bakiyesi hemen artar, ancak token
`amount` alanı, Token Program'ın `SyncNative` talimatını çağırana kadar
değişmez.

## Native SOL Nasıl Senkronize Edilir

Sarılmış bir SOL token hesabına SOL aktardıktan sonra Token Program'ın
`SyncNative` talimatını kullanın. `SyncNative` talimatı, token hesabının
`amount` alanını, hesapta kira-muaf rezervinin üzerinde tutulan lamport'larla
eşleşecek şekilde günceller.

### Kaynak Referansı

| Öğe                   | Açıklama                                                                                                                                                                                         | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `Account`             | Token hesabı durumu, hesabı WSOL olarak işaretlemek için `is_native` kullanır ve native rezervi, hesabın tutması gereken minimum kira-muaf lamport bakiyesini ve izlenen token miktarını saklar. | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| `SyncNative`          | Sarılmış bir SOL token hesabının `amount` alanını, hesapta kira-muaf rezervinin üzerinde tutulan lamport'larla eşleşecek şekilde güncelleyen bir talimattır.                                     | [Kaynak](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Kaynak](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
