...
description: Mint'ler veya token hesapları için yetki belirlemeyi öğrenin.
---

## Token yetkilerini anlama

### Mint hesabı yetkileri

- **Mint yetkisi**: Yeni token'ların oluşturulmasını kontrol eder. Herhangi bir
  token hesabına token basabilir. Sabit arzlı bir token oluşturmak için
  genellikle ilk arz oluşturulduktan sonra iptal edilir.
- **Dondurma yetkisi**: Token hesaplarını dondurma ve çözme yeteneğini kontrol
  eder. Herhangi bir token hesabının token transfer etmesini engelleyebilir.
  Kullanıcılara token'larının dondurulamayacağını garanti etmek için genellikle
  iptal edilir.

### Token hesabı yetkileri

- **Hesap sahibi**: Token hesabı üzerinde tam kontrole sahiptir. Token transfer
  edebilir, token yakabilir, delegeleri onaylayabilir ve bakiye sıfır olduğunda
  hesabı kapatabilir.
...
talimatı, mint'ler ve token hesapları üzerindeki yetkileri değiştirir veya iptal
eder. Yalnızca mevcut yetki, izinlerini yeni bir adrese aktarabilir veya yetkiyi
null olarak ayarlayarak kalıcı olarak iptal edebilir. İptal edildikten sonra
yetki geri yüklenemez.
