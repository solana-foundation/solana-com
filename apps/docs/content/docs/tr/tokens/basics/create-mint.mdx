...
tanımlayarak, o mint için tüm token account'lara uygulanan paylaşılan durumu
saklar.
...
gezginler arasında token'ın benzersiz tanımlayıcısıdır.
...
1. System Program'ın _rs`CreateAccount`_ talimatı, yeni bir rent muafiyetli
   hesap oluşturur ve yeni hesabın program sahibi olarak Token Program'ı atar.
...
rent muafiyetli olduğunu kontrol eder. Token Program daha sonra mint yetkisini,
dondurma yetkisini, ondalık basamakları ve _rs`is_initialized`_ bayrağını mint
account verilerine yazar.
...
## Meta Veri Nasıl Eklenir

Bir mint account yalnızca temel token bilgilerini (arz, ondalık basamaklar,
yetkiler) depolar. Tokenınıza ad, sembol ve görsel gibi okunabilir meta veriler
eklemek için iki seçeneğiniz vardır:

<Cards>
  <Card title="Metaplex Token Metadata" href="/docs/tokens/metaplex">
   Metaplex Token Metadata Program'ı kullanarak meta veri ekleyin. Hem orijinal Token Program hem de Token-2022 ile çalışır.
  </Card>

  <Card title="Token Extensions Metadata" href="/docs/tokens/extensions/metadata">
   Token-2022 ile yerleşik meta veri uzantısını kullanın. 
   Meta veri uzantısı yalnızca Token-2022 ile oluşturulan mint account'lar için kullanılabilir.
   Orijinal Token Program ile oluşturulan mint account'lara meta veri eklemek için Metaplex Token Metadata Program'ı kullanın.
  </Card>
</Cards>
