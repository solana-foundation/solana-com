...
Token Metadata Programı, her token mint için bir Program Derived Address (PDA)
oluşturur. Bu metadata hesabı, token'ın adı ve sembolü gibi zincir üstü
bilgileri ve zincir dışı JSON metadata'ya (resimler, açıklamalar vb.) işaret
eden bir URI'yi saklar.
...
Zincir üstü metadata hesabı şunları içerir:
