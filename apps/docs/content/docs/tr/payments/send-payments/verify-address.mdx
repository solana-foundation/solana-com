...
ATA adresini türettikten sonra, token hesabının zincir üzerinde mevcut olup
olmadığını kontrol edin. Eğer ATA mevcut değilse, alıcının token hesabını
oluşturmak için bir talimatı transfer ile aynı işleme dahil edebilirsiniz. Ancak
bu, yeni token hesabı için rent ödemeyi gerektirir. Alıcı ATA'ya sahip
olduğundan, rent için ödenen SOL gönderen tarafından geri alınamaz.
