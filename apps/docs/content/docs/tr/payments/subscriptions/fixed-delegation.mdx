---
title: Sabit Yetkilendirme
description: Sabit yetkilendirme işlemlerini kurma rehberi.
---
...
Bu rehber, akışı görünür tutar. Her kod parçası SDK fonksiyonlarını doğrudan
kullanır, böylece hangi hesabın türetildiğini, hangi talimatın gönderildiğini ve
hangi imzalayanın her adımı ödediğini veya yetkilendirdiğini görebilirsiniz.
...
Yetkilendirilen taraf transferi imzalar. SDK'ya aynı yetkilendirme PDA'sı,
kullanıcının token hesabı ve alıcının token hesabı gereklidir.
...
Delege eden kişi, delegasyonu istediği zaman iptal edebilir. İptal işlemi,
...
- Başlatma öncesinde kullanıcının token account hesabının mevcut olması gerekir.
- Miktarlar temel birimler cinsindendir. 6 ondalık basamaklı bir token için
  `1_000_000`, `1` token anlamına gelir.
- `initSubscriptionAuthority`'i yalnızca Subscription Authority hesabı henüz
  mevcut olmadığında çalıştırın.
- `expiryTs` kesin bir sınırdır: bu süre geçtikten sonra transferler başarısız
  olur ve sponsor rent bedelini geri alabilir. Harcama süresi için ek bir
  tolerans süresi yoktur.
- Kullanıcı, kurulum ve iptal işlemlerini imzalar. Yetkili kişi ise transferleri
  imzalar.
