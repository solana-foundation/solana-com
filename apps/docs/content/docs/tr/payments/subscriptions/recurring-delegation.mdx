...
- `amountPerPeriod` temel birimlerle ifade edilir. 6 ondalıklı bir token için,
  `1_000_000` değeri `1` token anlamına gelir.
- Program, mevcut dönemin kalan limitini aşan transferleri reddeder.
- Bir sonraki dönem başladığında, çekilen miktar sıfırlanır.
- Kullanıcı kurulum ve iptal işlemlerini imzalar. Delegatee transfer işlemlerini
  imzalar.
