...
description: Abonelik planı işlemlerini kurma kılavuzu.
---

Abonelik planı, bir satıcının kullanıcıların kabul edebileceği faturalandırma
koşullarını yayınlamasına olanak tanır. Bir kullanıcı abone olduktan sonra,
satıcı veya onaylanmış bir tahsildar her faturalandırma döneminde plan tutarına
kadar tahsilat yapabilir.

Bu kılavuz, tam akışı yapı taşları olarak göstermektedir. Satıcı bir plan
oluşturur, abone bunu kabul eder ve satıcı veya tahsildar ortaya çıkan abonelik
PDA'sından ödemeleri tahsil eder.
...
Satıcı, planın sahibidir. Plan PDA'sı, satıcı adresinden ve `planId`'den
türetilir.

Bir sponsor, isteğe bağlı `payer` parametresini geçirerek planın rent bedelini karşılayabilir; bu durumda satıcı, planın sahibi olmaya devam eder. Planın silinmesi durumunda geri ödeme ödeyene değil, sahibine yapılır; bu nedenle sponsorluk işlemlerini zincir dışında kontrol altında tutun.
...
Satıcı, oluşturulduktan sonra değiştirilebilir plan alanlarını güncelleyebilir.
Mevcut aboneler kabul ettikleri koşulları korurken, yeni aboneler mevcut plan
koşullarını kabul eder.

Birkaç kural geçerlidir:

- Sonlu bir `endTs` yalnızca kısaltılabilir, hiçbir zaman uzatılamaz veya
  temizlenemez (`PlanEndTsCannotExtend`).
- `Sunset` planında, ele geçirilmiş bir çekiciyi iptal etmek için çekicileri
  kaldırabilirsiniz (yeni küme, mevcut kümenin bir alt kümesi olmalıdır); durum,
  `endTs` ve meta veriler dondurulmuş kalır.
- Düzenlemeler, `endTs` değişmediği sürece bir planın son faturalandırma
  döneminde de geçerlidir.
- Talimat, imzalama anında gözlemlenen plan durumunu taşır
  (`expectedCreatedAt`, `expectedEndTs`, `expectedPullers`,
  `expectedMetadataUri`). Canlı plan artık eşleşmiyorsa güncelleme reddedilir (`StalePlanApproval`); dolayısıyla bayat bir imzalı güncelleme, kaldırılmış çekicileri geri getiremez veya sonraki düzenlemeleri geri alamaz. Eklenti istemcisinin `updatePlan` işlevi canlı durumu sizin için getirir; manuel olarak oluştururken planı kendiniz çekip alanları kendiniz iletmeniz gerekir.
...
Abone, mevcut plan koşullarını kabul eder. Abonelik PDA'sı, plan PDA'sından ve
abone adresinden türetilir.

Yetki başlatma ve aboneliği tek bir işlemde birleştirmek için `expectedSubscriptionAuthorityInitId` olarak TypeScript SDK tarafından dışa aktarılan `UNKNOWN_INIT_ID` sentinel değerini geçirin. Program yetkiyi yalnızca mevcut slot içinde oluşturulmuşsa kabul eder; bu nedenle sentinel değeri yeni kayıtlar için geçerlidir. Yetkisi daha önceki bir slot'ta oluşturulmuş olan geri dönen bir kullanıcının gerçek `initId` değerini geçirmesi gerekir (eklenti istemcisi bunu sizin için getirir); aksi takdirde çağrı `StaleSubscriptionAuthority` hatasıyla başarısız olur.
...
Ödemeyi merchant veya beyaz listedeki bir çekici imzalar. Plan bir hedef izin
listesi kullandığında, alıcı token account sahibi `destinations` listesinde yer
almalıdır.
...
## İptal Et ve İptal Yetki Kaldır

İptal etmek, aboneliği sona erecek şekilde işaretler. Yetki kaldırma ise iptal
süresinin dolmasının ardından abonelik PDA'sını kapatır. Abone her iki işlemi de
imzalar.
...
Hem abonenin hem de mevcut plan sahibinin imzalaması durumunda `cancelSubscriptionNow`, aboneliği fatura döneminin sonunda değil, iptal anında sona erdirir. Aynı zamanda bekleyen bir ek süre iptalini de kısaltabilir. Onay, imzalama anında gözlemlenen dönem başlangıcına bağlıdır; abonelik o tarihten bu yana değişmişse işlem reddedilir (`StaleSubscriptionApproval`).
...
İptal edilmiş bir abonelik, yetkisi kaldırılmadan önce yeniden
etkinleştirilebilir. Yetki kaldırıldıktan sonra abonelik hesabı kapatılır ve
abonenin yeniden [abone olması](#subscribe) gerekir. Sürdürme işlemi, aboneden
planın mint'i için `SubscriptionAuthority` gerektirir; program bu bilgiyi
(sahip, mint ve `init_id`) doğrular ve bilgi güncel değilse ya da yeniden
başlatılmışsa reddeder.

Talimat, abonenin imzalama anında gözlemlediği son kullanma tarihini taşır (`expectedExpiresAtTs`); bir uyuşmazlık reddedilir (`StaleSubscriptionApproval`). Bu sayede bayat bir imzalı yeniden başlatma, abonenin hiç onaylamadığı sonraki bir iptali geçersiz kılamaz.
...
- `amount` temel birimler cinsindendir. 6 ondalık basamaklı bir token için
  `5_000_000`, `5` token anlamına gelir.
- TypeScript SDK, `subscribe` sırasında plan koşullarını atladığınızda bunları
  canlı olarak çeker.
- Rust `SubscribeBuilder`, beklenen plan koşullarına ihtiyaç duyar. Önce plan
  hesabını çekip çözümleyin, ardından bu alanları `SubscribeData` aracılığıyla
  iletin.
- Yalnızca merchant veya `pullers` listesinde yer alan bir cüzdan ödeme
  toplayabilir.
- Abone, kurulum, iptal ve yetki kaldırma işlemlerini imzalar. Merchant veya
  onaylı toplayıcı ise ödeme toplama işlemlerini imzalar.
