...
hesapları, transferleri doğrulayan ve bakiyeleri atomik olarak güncelleyen
zincir üstü kod olan **Token Programı** tarafından yönetilir.
...
Token-2022 ("Token Extensions" olarak da bilinir), transfer kancaları,
aktarım ücretleri ve Gizli Bakiyeler gibi özellikler ekler. Her iki program da temel
aktarımlar için benzer şekilde çalışır, ancak ATA türetirken doğru olanı kullanmanız gerekir.
...
<Callout type="info">
  Solana yerel ücret piyasaları kullanır; işlemler, aynı duruma yazan diğer işlemlerle
  rekabet eder. Bu, başka yerlerde yüksek ağ aktivitesi dönemlerinde bile ödeme ücretlerinin
  düşük ve öngörülebilir kalması anlamına gelir.
</Callout>
