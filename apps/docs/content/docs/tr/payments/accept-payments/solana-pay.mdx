...
| `amount`    | `bigint`    | lamport veya token küçük birimlerinde tutar        |
| `splToken`  | `Address`   | SPL token mint adresi (SOL için dahil edilmez)     |
| `reference` | `Address[]` | Ödemeleri takip etmek için benzersiz referans(lar) |
| `label`     | `string`    | Cüzdanda gösterilen satıcı adı                     |
| `message`   | `string`    | Ödeme öncesi gösterilen mesaj                      |
