...
<Callout type="caution" title="Üretim için genel RPC kullanmayın">
  Genel kullanıma açık uç noktalar (`api.mainnet.solana.com`, `api.devnet.solana.com`)
  hız sınırlamasına tabidir, SLA sunmaz ve üretim ortamındaki ödeme akışları için uygun değildir.
  Üretim dağıtımlarında özel bir RPC uç noktasını güvence altına almak için bir [RPC sağlayıcısı](https://solana.com/rpc) kullanın.
</Callout>
