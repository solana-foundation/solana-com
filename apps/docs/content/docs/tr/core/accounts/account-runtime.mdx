...
4. **Var olmayan hesaplar**: Zincir üzerinde mevcut olmayan hesaplar,
   `rent_epoch` değeri `u64::MAX` olarak ayarlanmış şekilde varsayılan hesaplar
   (0 lamport, boş veri, System Program'a ait) olarak yüklenir.
