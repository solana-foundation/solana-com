...
Önceliklendirme ücreti, bir işlemin zamanlama önceliğini artıran isteğe bağlı bir ücrettir. Hesaplama birimi (CU), çalışma zamanının işlem yürütme sırasında ölçtüğü bir hesaplama birimidir. Her işlem (aritmetik, bellek erişimi, sistem çağrısı) sabit sayıda CU harcar. Önceliklendirme ücretini, işleminize hesaplama bütçesi talimatları ekleyerek ayarlayabilirsiniz. (Ayrıntılar için [Öncelik Ücretleri Nasıl Kullanılır kılavuzuna](/docs/core/fees) bakın.)
...
<Callout type="info" title="v1 işlemleri ücreti doğrudan belirler">
  Bu fiyat-çarpı-limit formülü, eski ve v0 işlemleri için geçerlidir. Yakında gelecek olan [v1
  formatında](/docs/core/transactions/versioned-transactions#v1-message-format), öncelik ücreti, mesaj yapılandırmasında doğrudan ayarlanan mutlak bir toplam lamport değeridir — CU başına fiyat yok, çarpma yok, yuvarlama yok.
</Callout>
...
- **reward**: doğrulayıcının ücret geliri = önceliklendirme ücreti + temel
  ...
  tarafından hesaplanır.
- **cost**: zamanlayıcının
  [tahmini CU maliyeti](https://github.com/anza-xyz/agave/blob/v3.1.8/cost-model/src/cost_model.rs#L37-L57)
