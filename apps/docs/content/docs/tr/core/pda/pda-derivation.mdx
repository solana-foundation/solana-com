...
Bump seed, türev işlemi sırasında opsiyonel seed'lere eklenen tek bir bayttır
(u8).
[`find_program_address`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L800)
255'ten 1'e kadar arama yapar ve her değerle _rs`create_program_address`_
fonksiyonunu çağırarak sonucun Ed25519 eğrisinin dışına çıkmasını sağlar.
Başarılı olan ilk değer **canonical bump**'tır.
...
_rs`create_program_address`_ için her çağrıda
...
Bir PDA türetmek yalnızca bir adres hesaplar. Bu adres üzerinde onchain bir
hesap oluşturmaz. Hesap, ayrı bir talimat aracılığıyla açıkça oluşturulmalıdır
(genellikle CPI yoluyla _rs`create_account`_).
...
<Callout type="warn">
  Bu, `sol_try_find_program_address` için belgelenen ve 255'ten 1'e doğru
  yineleme yapan mevcut canonical arama davranışından farklıdır.
</Callout>
...
<Callout type="info">
  ...
  fonksiyonu export edilmemiştir.
</Callout>
...
Bu örnekte, 255 bump değeri eğri üzerinde bir adres üretir ve başarısız olur.
Geçerli olan ilk bump 254'tür ve bu nedenle canonical bump'tır.
