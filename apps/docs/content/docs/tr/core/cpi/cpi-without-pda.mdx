---
title: PDA imzalayıcıları olmayan CPI'lar
description:
  Solana üzerinde PDA imzalama gerektirmeyen programlar arası çağrılar için
  invoke fonksiyonunun nasıl kullanılacağı, Anchor CpiContext ve Native Rust
  örnekleri ile.
...
<Callout type="info" title="Özet">
  ...
  _rs`invoke`_ kullanın. PDA imzalama gerekmez. İmzalayıcı ve yazılabilir
...
fonksiyonu kullanılır. _rs`invoke`_ fonksiyonu, boş bir _rs`signers_seeds`_
dizisi ile _rs`invoke_signed`_ fonksiyonunu çağırır. Boş imzalayıcılar dizisi,
...
Aşağıdaki örnekler Anchor ve Native Rust kullanarak bir CPI yapar. Bir hesaptan
diğerine SOL transfer eden tek bir talimat içerir.

#### Anchor

Aşağıdaki örnekler, bir [Anchor](https://www.anchor-lang.com/docs) programında
CPI'ları uygulamanın iki yaklaşımını gösterir. Örnekler işlevsel olarak
...
- **Örnek 2**: _rs`solana_program`_ crate'inden
  _rs`system_instruction::transfer`_ fonksiyonunu kullanır.
- **Örnek 3**: CPI talimatını manuel olarak oluşturur. Bu yaklaşım, çağırmak
  istediğiniz talimatı oluşturmaya yardımcı olacak bir crate olmadığında
  kullanışlıdır.
...
gösterir. Bir hesaptan diğerine SOL transfer eden tek bir talimat içerir. (Test
