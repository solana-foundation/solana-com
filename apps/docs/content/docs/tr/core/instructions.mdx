...
![Tek talimatlı bir transaction. Talimat, `accounts` dizisinde iki `AccountMeta` struct'ı içerir.](/assets/docs/core/transactions/accountmeta.svg)

Hesap meta verileri aşağıdaki bilgileri içerir:

- [pubkey](mention:pubkey): Hesabın public key adresi
- [is_signer](mention:is-signer): Hesap transaction'ı imzalamalıysa `true`
  olarak ayarlanır
...
bilgiyi içerdiğine dikkat edin: [`program_id`](mention:program-id),
