...
<Callout type="info">
  ...
  sans portefeuille de navigateur ? Signez avec un service de gestion de clés
...
- **Tableaux de bord en temps réel** : Combinez des observateurs (soldes,
  comptes, signatures) avec votre bibliothèque UI préférée ; le client gère la
  distribution websocket et l'invalidation du cache.
- **Stores personnalisés** : Injectez votre propre store Zustand pour hydrater
  depuis IndexedDB/localStorage, synchroniser l'état avec les sessions serveur
  ou coordonner entre les onglets du navigateur.
- **Pont vers les hooks React** : Passez une instance client configurée à
  `@solana/react-hooks` lorsque vous souhaitez des hooks ergonomiques au-dessus
  du même runtime.
- **Testabilité** : L'interface client unique peut être simulée dans les tests
  ...
  portefeuille sans portefeuille de navigateur.
