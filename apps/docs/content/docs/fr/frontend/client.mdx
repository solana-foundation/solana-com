...
<Callout type="info">
  Fonctionnement sans interface — dans une route API, un worker ou un script
  sans portefeuille navigateur ? Signez avec un service de gestion de clés
  backend plutôt qu'avec un keypair brut. Consultez
  [Keychain](/docs/tools/keychain) et [Signature en
  production](/docs/core/transactions/signing-in-production).
</Callout>

## Patterns courants pour les développeurs Solana

- **Machines à états headless** : Exécutez le client dans des routes API, des
  scripts ou des workers pour réutiliser la même logique d'orchestration
  portefeuille + RPC qui alimente votre interface.
- **Tableaux de bord en temps réel** : Combinez des observateurs (soldes,
  comptes, signatures) avec votre bibliothèque d'interface préférée ; le client
  gère le fan-out websocket et l'invalidation du cache.
- **Stores personnalisés** : Injectez votre propre store Zustand pour hydrater
  depuis IndexedDB/localStorage, synchroniser l'état avec des sessions serveur,
  ou coordonner entre onglets du navigateur.
- **Passerelle vers les hooks React** : Transmettez une instance client
  configurée à `@solana/react-hooks` lorsque vous souhaitez des hooks
  ergonomiques par-dessus le même runtime.
- **Testabilité** : L'interface client unique peut être simulée dans les tests
  unitaires, facilitant la simulation de réponses RPC ou de sessions de
  portefeuille sans portefeuille navigateur.
