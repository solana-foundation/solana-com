...
- Créez un pool de comptes nonce pour la préparation de transactions en
  parallèle
- Suivez quels nonces sont "en cours d'utilisation" (ont des transactions
  signées en attente)
- Implémentez le recyclage des nonces après que les transactions sont soumises
  ou abandonnées
...
- _N'importe qui_ avec les octets de transaction sérialisés peut la soumettre au
  réseau
