...
<Callout type="caution" title="N'utilisez pas le RPC public en production">
  Les points de terminaison publics (`api.mainnet.solana.com`, `api.devnet.solana.com`) sont
  soumis à des limitations de débit, ne disposent d'aucun SLA et ne sont pas adaptés aux flux de paiement en production.
  Utilisez un [fournisseur RPC](https://solana.com/rpc) pour sécuriser un point de terminaison RPC privé
