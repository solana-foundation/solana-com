...
- `amountPerPeriod` est en unités de base. Pour un token à 6 décimales,
  `1_000_000` signifie `1` token.
- Le programme rejette les transferts qui dépassent l'allocation restante de la
  période en cours.
- Au début de la période suivante, le montant prélevé est réinitialisé.
- `expiryTs` est une limite stricte : une fois dépassée, les transferts échouent
  et le sponsor peut récupérer le rent. Il n'existe pas de délai de grâce basé
  sur le temps de dépense.
- Définissez `startTs` sur `0` pour démarrer la délégation au moment où la
  transaction est enregistrée sur la chaîne plutôt qu'à une heure fixe. Cela
  élargit la fenêtre pour amener l'utilisateur à signer et à s'inscrire. Dans ce
  cas, `expiryTs` doit être non nul — une délégation à démarrage à
  l'atterrissage ne peut pas également être configurée pour ne jamais expirer.
- L'utilisateur signe les transactions de configuration et de révocation. Le
  délégué signe les transferts.
