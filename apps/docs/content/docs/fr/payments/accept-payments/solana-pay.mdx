...
<Callout type="caution">
  ...
  avant la sortie stable. Pour plus de détails, consultez la [documentation du
...
| ----------- | ----------- | --------------------------------------------------- |
| `recipient` | `Address`   | Adresse du portefeuille du commerçant (obligatoire) |
| `amount`    | `bigint`    | Montant en lamports ou unités mineures de jeton     |
| `splToken`  | `Address`   | Adresse de création du jeton SPL (omettre pour SOL) |
| `reference` | `Address[]` | Référence(s) unique(s) pour le suivi des paiements  |
| `label`     | `string`    | Nom du commerçant affiché dans le portefeuille      |
| `message`   | `string`    | Message affiché avant le paiement                   |
| `memo`      | `string`    | Mémo onchain attaché à la transaction               |
