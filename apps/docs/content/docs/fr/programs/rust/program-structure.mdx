...
Ajoutons maintenant les dépendances nécessaires pour compiler un programme
...
Un `entrypoint` de programme Solana possède la signature de fonction suivante.
Les développeurs sont libres de créer leur propre implémentation de la fonction
`entrypoint`.
...
l'une des variantes de notre enum `CounterInstruction`. La méthode
`try_from_slice` de Borsh gère automatiquement cette conversion.
...
acheminer les instructions vers leurs fonctions de traitement appropriées.

Ce modèle de routage est courant dans les programmes Solana. Les
`instruction_data` sont désérialisées en une variante d'une enum représentant
l'instruction, puis la fonction de traitement appropriée est appelée. Chaque
fonction de traitement inclut l'implémentation de cette instruction.
...
propriétaire, ce qui donne à notre programme la possibilité d'écrire dans le
compte et d'initialiser les données.
...
en utilisant les signatures de transaction en sortie. Notez que le cluster sur
Solana Explorer doit être défini sur « Custom RPC URL », qui par défaut est
