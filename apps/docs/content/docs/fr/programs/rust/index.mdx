...
Tout d'abord, créez un nouveau projet Rust en utilisant la commande standard
...
Accédez au répertoire du projet. Vous devriez voir les fichiers par défaut
...
<Callout type="warn">
  Mettez à jour le champ `edition` dans `Cargo.toml` en `2021`. Sinon, vous
...
Ensuite, ajoutez la dépendance `solana-program`. Il s'agit de la dépendance
minimale requise pour compiler un programme Solana.
...
<Callout type="warn">
  ...
  sera pas généré lors de la compilation du programme.
</Callout>

<CodePlaceholder title="Cargo.toml">
  Votre fichier `Cargo.toml` devrait ressembler à ceci :
</CodePlaceholder>
...
1. Un fichier `.so` (par exemple, `hello_world.so`) : il s'agit du programme
   Solana compilé qui sera déployé sur le réseau en tant que « smart contract ».
2. Un fichier keypair (par exemple, `hello_world-keypair.json`) : la clé
   publique de ce keypair est utilisée comme identifiant du programme lors du
...
Ensuite, testez le programme en utilisant la crate `litesvm`. Ajoutez les
...
Pendant que le validateur de test est en cours d'exécution, exécutez la commande
...
<Callout type="info">
  ...
  L'option « Custom RPC URL » sur Solana Explorer est définie par défaut sur
  `http://localhost:8899`.
</Callout>
...
Les programmes Solana peuvent être mis à jour en les redéployant avec le même ID
de programme. Modifiez le programme dans `src/lib.rs` pour afficher « Hello,
...
<Callout type="info">
  Vous pouvez également supprimer le fichier keypair existant (par exemple
