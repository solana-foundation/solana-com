...
Les programmes Rust onchain prennent en charge la plupart des bibliothèques
libstd, libcore et liballoc de Rust, ainsi que de nombreuses crates tierces.
...
- Accès limité à :
  - `std::hash`
  - `std::os`
- Bincode est extrêmement coûteux en termes de calcul, tant en cycles qu'en
  profondeur d'appel, et devrait être évité
- Le formatage de chaînes devrait être évité car il est également coûteux en
  termes de calcul.
- Pas de prise en charge de `println!`, `print!`, utilisez plutôt la macro
...
## Profondeur de la pile d'appels - erreur `CallDepthExceeded`
...
Les invocations inter-programmes permettent aux programmes d'invoquer d'autres
programmes directement, mais la profondeur est actuellement limitée à `4`.

Lorsqu'un programme dépasse la profondeur d'appel
[d'invocation inter-programmes](/docs/core/cpi) autorisée, il recevra une erreur
`CallDepth`
