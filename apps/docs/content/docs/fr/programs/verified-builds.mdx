...
Si vous souhaitez avoir une configuration totalement autonome, vous pouvez
exécuter
l'[API Verify](https://github.com/otter-sec/solana-verified-programs-api)
vous-même ou exécuter la vérification du programme localement vous-même en
utilisant la commande `verify-from-repo` en utilisant les données de
vérification sur la chaîne qui sont enregistrées dans un
[PDA](https://explorer.solana.com/address/63XDCHrwZu3mXsw2RUFb4tbNpChuUHx4eA5aJMnHkpQQ/anchor-account)
qui est dérivé de l'autorité de déploiement du programme et du
...
informations de contact et de sécurité directement dans leurs smart contracts
Solana. Inspirée de [securitytxt.org](https://securitytxt.org), cette approche
fournit un moyen standardisé pour les chercheurs en sécurité de contacter les
mainteneurs de projet, même s'ils ne connaissent que l'adresse du contrat.
...
peuvent être facilement interrogées via des outils comme Solana Explorer,
garantissant que vos coordonnées et détails de sécurité sont disponibles pour
