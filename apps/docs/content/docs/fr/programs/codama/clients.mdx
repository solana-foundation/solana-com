...
Pour suivre, créez un nouveau répertoire appelé `example` et ajoutez le fichier
`idl.json` et le fichier `package.json` à partir des extraits de code.
...
Dans un projet Anchor, l'IDL se trouve dans `target/idl/<program_name>.json`
après avoir exécuté `anchor build`.
...
Cela vous invitera avec des questions pour créer un fichier `codama.json` à la
racine de votre projet et installer les dépendances Codama nécessaires.
...
mais n'incluent pas `package.json` (pour TypeScript) ou `Cargo.toml` (pour
Rust).

Vous devrez soit créer un nouveau fichier `package.json` et `Cargo.toml`, soit
...
puis ajoutez les dépendances requises à votre `package.json` ou `Cargo.toml`
existant.
