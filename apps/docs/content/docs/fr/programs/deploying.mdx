...
- **ID de programme** : L'adresse on-chain de votre programme. Les utilisateurs
  interagissent avec votre programme en référençant l'ID de programme.
- **Compte de programme** : Le compte on-chain qui stocke les métadonnées de
  votre programme. L'adresse du compte de programme est l'ID de programme. Le
  compte de programme stocke également l'adresse du compte ProgramData.
- **Compte ProgramData** : Le compte on-chain qui stocke le code exécutable
  déployé de votre programme.
- **Autorité de programme** : Le compte qui a la permission de mettre à jour ou
...
L'ID de programme est l'adresse on-chain de votre compte de programme. Les
utilisateurs et les autres programmes invoquent votre programme en référençant
cet ID dans leurs transactions.
...
- **`--use-rpc`**: Envoie les transactions au RPC configuré. Ce flag nécessite une connexion RPC [pondérée par le stake](/docs/defi/stake-weighted-qos) auprès de fournisseurs tels que [Triton](https://triton.one/) ou [Helius](https://www.helius.dev/).
