...
L'adresse onchain de votre programme. Elle sera mise à jour lorsque vous
compilerez le projet
...
- La macro [`declare_id!`](mention:one) spécifie l'adresse onchain de votre
...
<Callout>
  Votre [ID de programme](/docs/intro/quick-start/build-first-program), défini
  dans _rs`declare_id!()`_, sera maintenant mis à jour.
</Callout>

2. Ajouter du SOL à votre portefeuille _(Facultatif)_

Le déploiement d'un programme nécessite du SOL dans le program account onchain.
Si nécessaire,
...
Fermez le programme pour récupérer le SOL alloué au programme onchain. Pour ce
...
explorer les [adresses dérivées de programme](/docs/core/pda) et
[l'invocation inter-programmes](/docs/core/cpi).
