...
Lisez les données du réseau Solana en récupérant différents comptes. Cette
section vous aidera à comprendre la structure des [comptes](/docs/core/accounts)
Solana. Chaque compte Solana possède une
[adresse](/docs/core/accounts#account-address) unique qui permet de localiser
ses données onchain correspondantes. Les comptes contiennent soit
...
Le champ `lamports` contient le solde SOL du compte, en
...
Le champ `rentEpoch` est un champ obsolète provenant d'un mécanisme de loyer
déprécié. (Ce champ est inclus pour la rétrocompatibilité.)
...
n'est pas un champ du [type Account](/docs/core/accounts#account-structure)
lui-même, mais il est inclus dans la réponse.

Dans cet exemple, le champ `space` est 0 car le champ `data` contient 0 octet de
données.
...
Chaque compte de programme appartient à son
[programme de chargement](/docs/core/programs#loader-programs). Dans cet
exemple, le `owner` est le programme BPFLoader2.
...
Le champ `data` contient l'état sérialisé du compte `Mint`, tel que l'autorité
de mint, l'offre totale, le nombre de décimales.

Pour lire à partir d'un mint account, vous devez désérialiser le champ `data`
dans le type de données `Mint`, ce qui est illustré dans
...
des programmes Solana fournissent des bibliothèques clientes avec des fonctions
d'aide qui abstraient le processus de désérialisation. Ces fonctions
convertissent les octets bruts du compte en types de données structurés, ce qui
facilite le travail avec les données du compte.
...
Le champ `mintAuthority` indique le seul compte pouvant créer de nouvelles
unités du jeton.
...
Le champ `supply` indique le nombre total de jetons qui ont été créés. Cette
valeur est mesurée dans la plus petite unité du jeton. Pour obtenir l'offre
...
Le champ `decimals` indique le nombre de décimales pour le jeton.
...
accounts. Un token account gelé ne peut pas transférer ni brûler le jeton qu'il
contient.
