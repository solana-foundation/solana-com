...
description:
  Apprenez à créer un programme Solana CRUD (Créer, Lire, Mettre à jour,
  Supprimer) en utilisant les Program Derived Address (PDA) et le framework
  Anchor. Ce guide étape par étape montre comment créer, mettre à jour et
  supprimer des comptes de messages onchain à l'aide des PDA, implémenter la
  validation des comptes et écrire des tests. Parfait pour les développeurs
  souhaitant comprendre comment utiliser les PDA dans les programmes Solana.
...
1. _rs`user: Signer<'info>`_
   ...
   - Nécessite le statut mutable (_rs`#[account(mut)]`_) car il paie pour le
     nouveau compte
   - Doit signer la transaction pour approuver la déduction de lamports depuis
     ce compte
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - `space` alloue les octets requis pour le champ data du compte
...
- _rs`b"message"`_ - Une chaîne fixe comme première seed.
...
la bump seed correcte. Anchor utilise `seeds` et `bump` pour dériver le PDA.
...
_rs`Update`_ d'accéder au paramètre _rs`message`_ depuis l'instruction
_rs`update`_.
...
2. Met à jour les données du compte :
   - Accède au `message_account` depuis le contexte.
   - Définit le champ `message` avec le nouveau `message` depuis l'argument de
     la fonction.
