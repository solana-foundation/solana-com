...
vérifier que le programme de démarrage se compile correctement.
...
représentent les données de compte (type de données à stocker dans le champ data
du compte).

### !user

Le champ _rs`user`_ contient une _rs`Pubkey`_ qui identifie l'utilisateur ayant
...
dérivée de programme (PDA). Le stockage de cette valeur économise du calcul en
éliminant le besoin de la recalculer dans les instructions ultérieures.
...
- `bump` - _rs`u8`_ qui stocke le ["bump" seed](/docs/core/pda#canonical-bump)
  pour dériver l'adresse dérivée de programme (PDA). Le stockage de cette valeur
  économise du calcul en éliminant le besoin de la recalculer dans les
...
1. Le type de compte (comme _rs`Signer<'info>`_ ou _rs`Account<'info, T>`_) qui
   spécifie le type de compte attendu par le programme
2. Des contraintes optionnelles (comme _rs`#[account(mut)]`_ ou
...
1. _rs`user: Signer<'info>`_
   ...
   - Nécessite le statut mutable (_rs`#[account(mut)]`_) car il paie pour le
     nouveau compte
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - La contrainte `init` crée le compte lors de l'exécution de l'instruction
   ...
   - `space` alloue les octets requis pour le champ de données du compte
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - En coulisses, la contrainte `init` appelle le System Program pour créer un
     nouveau compte avec l'espace `space` spécifié et change le propriétaire
...
Les contraintes `seeds` et `bump` définissent ensemble l'adresse d'un compte en
tant que Program Derived Address (PDA).
...
- _rs`user.key().as_ref()`_ - La clé publique du compte _rs`user`_ comme second
  seed.
...
La fonction `create` implémente la logique pour initialiser les données d'un
...
2. Initialisation des données du compte :
   ...
   - Définit le champ `user` sur la clé publique du compte `user`.

   ```rs
   account_data.user = ctx.accounts.user.key();
   ```

   - Définit le champ `message` sur le `message` de l'argument de la fonction.
...
stocké sur le _rs`message_account`_, plutôt que de laisser Anchor le recalculer.
...
La fonction `update` implémente la logique de modification d'un compte de
message existant. Elle prend deux paramètres :

1. _rs`ctx: Context<Update>`_ - Fournit l'accès aux comptes spécifiés dans la
   structure _rs`Update`_.
2. _rs`message: String`_ - Le nouveau message pour remplacer l'existant.
...
2. Met à jour les données du compte :
   - Accède au `message_account` depuis le contexte.
   - Définit le champ `message` avec le nouveau `message` de l'argument de la
     fonction.
...
La contrainte _rs`seeds`_ spécifie les seeds utilisées pour dériver le PDA.
...
Le corps de la fonction affiche simplement un message dans les logs du programme
en utilisant la macro _rs`msg!()`_. La fonction ne nécessite aucune logique
supplémentaire car la contrainte _rs`close`_ dans la structure _rs`Delete`_ gère
...
exécutant `deploy` dans le terminal Playground.
...
Solana Playground supprime une partie de la configuration standard où
[`pg.program`](mention:one) permet d'accéder aux méthodes pour interagir avec le
...
pour vérifier qu'il s'exécute comme prévu. Les étapes suivantes ajoutent les
tests réels.
...
La méthode _ts`fetch()`_ récupère les données de compte du réseau.
...
`fetchNullable`. Cette méthode retourne null lorsque le compte n'existe plus
après sa fermeture.
...
déployé sur le devnet et affiche des liens vers SolanaFM pour consulter les
détails de la transaction.
