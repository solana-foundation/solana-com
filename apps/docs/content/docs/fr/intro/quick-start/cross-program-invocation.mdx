...
Tout d'abord, le programme a besoin d'un mécanisme simple de « paiement pour
mise à jour » en modifiant la structure _rs`Update`_ et la fonction `update`.
...
- En tant que type _rs`SystemAccount`_, le System Program en est propriétaire
  comme pour les comptes de portefeuille classiques
...
Ajoutez maintenant un mécanisme de « remboursement à la suppression » en
...
transférer tout SOL accumulé vers l'utilisateur.
...
Les _rs`signer_seeds`_ spécifient les seeds optionnelles et bump seeds utilisées
pour dériver le PDA.
...
- Le montant à transférer (la totalité du solde du compte coffre-fort)
...
compte coffre-fort dans les instructions. Cela nécessite de dériver le PDA du
