...
les transferts de SOL entre comptes en invoquant le System Program.
...
D'abord, le programme a besoin d'un mécanisme simple de « paiement pour mise à
jour » en modifiant la structure _rs`Update`_ et la fonction `update`.
...
appelé `vault_account`. Ce compte, contrôlé par le programme, reçoit du SOL d'un
utilisateur lorsqu'il met à jour son compte de message.
...
reçoit du SOL des utilisateurs lorsqu'ils mettent à jour leurs messages.
...
- En tant que PDA, il n'a pas de clé privée, donc seul le programme peut «
  signer » pour l'adresse lors de l'exécution de CPI
- En tant que type _rs`SystemAccount`_, le System Program en est propriétaire
  comme les comptes de portefeuille classiques
...
Dans l'instruction `delete`, vous verrez comment le programme peut « signer »
pour ce PDA dans un CPI.
...
invocation de programme croisée (CPI).
...
Dans l'instruction `update`, l'implémentation inclut une invocation de programme
croisée (CPI) pour invoquer l'instruction `transfer` du System Program. Cela
démontre comment effectuer une CPI depuis le programme, permettant ainsi la
composabilité des programmes Solana.

La structure _rs`Transfer`_ spécifie les comptes requis pour l'instruction de
transfert du System Program :

- `from` - Le compte de l'utilisateur (source des fonds)
- `to` - Le compte coffre-fort (destination des fonds)
...
instruction data pour une instruction particulière à invoquer. Lorsque
...
Ajoutez maintenant un mécanisme de "remboursement à la suppression" en modifiant
la structure _rs`Delete`_ et la fonction `delete`.

Tout d'abord, mettez à jour la structure _rs`Delete`_ pour inclure le
`vault_account`. Cela permet de transférer tout SOL présent dans le coffre-fort
à l'utilisateur lorsqu'il ferme son compte de message.
...
Le `vault_account` utilise la même dérivation de PDA que dans la structure
Update.

L'ajout du `vault_account` à la structure Delete permet au programme d'accéder
au compte du coffre-fort de l'utilisateur pendant l'instruction de suppression
pour transférer tout SOL accumulé vers l'utilisateur.
...
SOL du compte du coffre-fort vers le compte de l'utilisateur.
...
Les _rs`signer_seeds`_ spécifient les seeds optionnelles et les bump seeds
utilisées pour dériver le PDA.
...
- Le montant à transférer (la totalité du solde du compte du coffre-fort)
...
compte du coffre-fort dans les instructions. Cela nécessite de dériver le PDA du
coffre-fort et de l'inclure dans les appels d'instructions update et delete.
...
Cliquez sur le bouton `Import` et saisissez un nom de projet pour l'ajouter à
votre liste de projets dans Solana Playground. Une fois qu'un projet est
importé, toutes les modifications sont automatiquement enregistrées et
conservées.
