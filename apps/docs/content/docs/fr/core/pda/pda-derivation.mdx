...
Le bump seed est un octet unique (u8) ajouté aux seeds optionnelles lors de la
dérivation.
[`find_program_address`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L800)
effectue une recherche de 255 à 1, en appelant _rs`create_program_address`_ avec
chaque valeur jusqu'à ce que le résultat sorte de la courbe Ed25519. La première
valeur qui réussit est le **bump canonique**.
...
L'[appel système onchain](https://github.com/anza-xyz/agave/blob/v3.1.8/syscalls/src/lib.rs#L798-L834)
...
La dérivation d'un PDA calcule uniquement une adresse. Elle ne crée pas de
compte onchain à cette adresse. Le compte doit être explicitement créé via une
...
### Itération de tous les bumps possibles
...
<Callout type="warn">
  Ceci est distinct du comportement de recherche canonique actuel documenté pour
  `sol_try_find_program_address`, qui itère de 255 à 1.
</Callout>
