...
Solana. Une instruction est essentiellement une fonction publique que tout
utilisateur du réseau Solana peut appeler. Chaque instruction est utilisée pour
...
son propre ensemble d'instructions. Pour interagir avec le réseau Solana, une ou
...
et le message contenant l'instruction de transfert de SOL.
...
- [`data`](mention:instruction-data) : un tableau d'octets contenant des
  [données] supplémentaires à utiliser par l'instruction.
...
Le `data` de l'instruction est un tableau d'octets qui spécifie quelle
instruction du programme invoquer. Il inclut également tous les arguments requis
par l'instruction.
...
diffère entre les SDK, mais notez que chaque instruction contient les trois
mêmes informations requises : [`program_id`](mention:program-id),
