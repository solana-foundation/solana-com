...
Les frais de priorité sont des frais facultatifs qui augmentent la priorité de planification d'une transaction. Une unité de calcul (CU) est une unité de computation que le runtime mesure lors de l'exécution d'une transaction. Chaque opération (arithmétique, accès mémoire, appel système) coûte un nombre fixe de CUs. Vous pouvez définir les frais de priorité en incluant des instructions de budget de calcul dans votre transaction. (Consultez le guide [Comment utiliser les frais de priorité](/docs/core/fees) pour plus de détails.)
...
<Callout type="info" title="Les transactions v1 définissent les frais directement">
  Cette formule prix × limite s'applique aux transactions legacy et v0. Dans le
  prochain [format
  v1](/docs/core/transactions/versioned-transactions#v1-message-format), les
  frais de priorité sont un total absolu en lamports défini directement dans la
  configuration du message — pas de prix par CU, pas de multiplication, pas d'arrondi.
</Callout>
