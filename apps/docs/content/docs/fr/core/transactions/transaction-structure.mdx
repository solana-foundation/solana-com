...
Un transfert par System Program ajoute des lamports à **n'importe quel** compte.
Il n'existe aucune vérification au niveau du protocole garantissant que le
destinataire peut renvoyer les SOL. Les lamports ne peuvent être retirés que par
le programme propriétaire du compte. Ainsi, envoyer des SOL à un
[token mint](/docs/tokens/basics/create-mint), à un programme ou à une
[PDA](/docs/core/pda) que vous ne contrôlez pas **risque une perte définitive de
...
Les transferts de **tokens** SPL sont partiellement auto-protégés : le Token
Program rejette tout transfert dont les comptes ne correspondent pas au mint
attendu. Les transferts natifs de **SOL** ne bénéficient d'aucune telle
protection, c'est pourquoi l'expéditeur doit vérifier le destinataire avant de
signer. Consultez
...
<WithMentions>
Le code ci-dessous affiche la sortie des extraits de code précédents.
...
Étant donné qu'un transfert de SOL aboutit vers n'importe quel compte, vérifiez
le destinataire avant de signer. Récupérez le compte et n'envoyez des SOL qu'à
un portefeuille System Program (ou à une adresse
[sur la courbe](/docs/payments/send-payments/verify-address#on-curve-addresses)
non financée) ; rejetez les mints, les token accounts, les programmes et les PDA
...
<Callout>
  ...
  complète qui gère également les envois de token SPL (token accounts, ATA,
  Token-2022), consultez [Vérifier une
  adresse](/docs/payments/send-payments/verify-address).
</Callout>
