...
description:
  Décodez et inspectez une transaction Solana confirmée avec @solana/transaction-introspection - résolvez les indices de comptes et les adresses de tables de correspondance, parcourez les instructions externes et internes (CPI), et analysez-les avec des clients de programme.
...
<Callout type="info" title="Préparation aux transactions v1">
  Une fois le [format
  v1](/docs/core/transactions/versioned-transactions#v1-message-format)
  activé, passez `maxSupportedTransactionVersion: 1` sinon la récupération échoue sur les
  transactions v1. `decodeTransactionFromRpcResponse` gère `legacy`, `0` et
  `1` de la même manière (depuis `@solana/kit` 7.0). Notez qu'un message v1 contient ses
  limites de ressources dans un objet `transactionConfig` plutôt que dans des instructions ComputeBudget,
  ainsi le code qui récupère les limites de calcul ou les frais de priorité en
  analysant les instructions décodées ne trouve rien pour v1.
</Callout>
