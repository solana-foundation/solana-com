...
title: Sync Native
description:
  Mettre à jour le montant d'un compte de jetons SOL encapsulés après avoir
  transféré des SOL dans le compte.
...
Le SOL encapsulé (WSOL) est du SOL détenu dans un token account pour le mint
natif du Token Program. Le mint natif est l'adresse de mint que le Token Program
utilise pour les token accounts qui encapsulent le SOL. Le mint natif permet au
SOL d'être utilisé via les instructions du Token Program.
...
de solde de jetons via le champ _rs`amount`_ du token account. Si vous
...
des SOL dans un token account SOL encapsulé. L'instruction _rs`SyncNative`_ met
à jour le champ _rs`amount`_ du token account pour correspondre aux lamports
...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_             | L'état du token account utilise _rs`is_native`_ pour marquer le compte comme WSOL et stocke la réserve native, le solde minimum en lamports exempt de loyer que le compte doit conserver, ainsi que le montant de jetons suivi. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`SyncNative`_          | Une instruction qui met à jour le champ _rs`amount`_ d'un token account SOL encapsulé pour correspondre aux lamports détenus dans le compte au-dessus de sa réserve exemptée de loyer.                                          | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
