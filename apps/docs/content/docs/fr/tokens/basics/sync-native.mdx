...
title: Sync Native
description:
  Mettre à jour le montant d'un compte de jetons SOL encapsulés après le
  transfert de SOL dans le compte.
...
Le SOL encapsulé (WSOL) est du SOL détenu dans un compte de jetons pour le mint
natif du Token Program. Le mint natif est l'adresse de mint que le Token Program
utilise pour les comptes de jetons qui encapsulent le SOL. Le mint natif permet
d'utiliser le SOL via les instructions du Token Program.
...
Un compte de jetons SOL encapsulés stocke le SOL dans le champ `lamports` du
compte, mais suit le SOL détenu dans le compte de jetons SOL encapsulés en tant
que solde de jetons via le champ _rs`amount`_ du compte de jetons. Si vous
transférez du SOL dans un compte de jetons SOL encapsulés, le solde en lamports
augmente immédiatement, mais le champ _rs`amount`_ ne change pas tant que vous
n'appelez pas l'instruction _rs`SyncNative`_ du Token Program.

## Comment synchroniser le SOL natif

Utilisez l'instruction _rs`SyncNative`_ du Token Program après avoir transféré
du SOL dans un compte de jetons SOL encapsulés. L'instruction _rs`SyncNative`_
met à jour le champ _rs`amount`_ du compte de jetons pour qu'il corresponde aux
lamports détenus dans le compte au-delà de sa réserve d'exonération de loyer.

### Référence source

| Élément                   | Description                                                                                                                                                                                                                               | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_             | L'état du compte de jetons utilise _rs`is_native`_ pour marquer le compte comme WSOL et stocke la réserve native, le solde minimum en lamports d'exonération de loyer que le compte doit conserver, ainsi que le montant de jetons suivi. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`SyncNative`_          | Une instruction qui met à jour le champ _rs`amount`_ d'un compte de jetons SOL encapsulés pour qu'il corresponde aux lamports détenus dans le compte au-delà de sa réserve d'exonération de loyer.                                        | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| _rs`process_sync_native`_ | Logique de processeur partagée pour la synchronisation des soldes de SOL encapsulés.                                                                                                                                                      | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |
