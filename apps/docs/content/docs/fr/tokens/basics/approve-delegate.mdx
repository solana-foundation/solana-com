...
description:
  Autoriser une autre adresse à transférer ou brûler un montant limité de jetons
  depuis un compte de jetons.
...
## Que fait l'approbation d'un délégué ?

Approuver un délégué permet à une autre adresse de transférer ou de brûler un
montant limité de jetons depuis un token account au nom du propriétaire du token
account.

Un token account stocke un délégué actuel et un montant délégué à la fois.
L'approbation d'un nouveau délégué remplace le délégué précédent et
l'autorisation sur le compte.

## Comment approuver un délégué

L'approbation d'un délégué utilise l'instruction _rs`Approve`_ ou
_rs`ApproveChecked`_ du Token Program.

Les exemples ci-dessous utilisent _rs`ApproveChecked`_, qui nécessite que
l'appelant fournisse le mint et les décimales afin que l'instruction puisse
vérifier le mint attendu et la précision des jetons avant de traiter
l'approbation.

Le propriétaire du token account signe l'approbation.

### Référence source

| Élément               | Description                                                                                                                                                                       | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_         | L'état du token account stocke le propriétaire, le délégué et le montant délégué.                                                                                                 | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | Une instruction d'approbation de délégué qui enregistre le délégué et l'autorisation pour un token account sans exiger que l'appelant fournisse le mint ou les décimales.         | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | Une instruction d'approbation de délégué qui nécessite que l'appelant fournisse le mint et les décimales et vérifie ces valeurs avant d'enregistrer le délégué et l'autorisation. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
| _rs`process_approve`_ | Logique partagée du processeur pour l'approbation de délégué.                                                                                                                     | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L344-L392)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L582-L640)     |

### Typescript

Les exemples `Kit` ci-dessous montrent l'approche recommandée utilisant
`@solana/kit`. Les exemples legacy utilisant `@solana/web3.js` sont inclus à
titre de référence.
