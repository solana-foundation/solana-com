...
tokens jusqu'au montant approuvé, mais ne peut pas le dépasser. Seul le
propriétaire du token account peut approuver un délégué, et chaque token account
ne peut avoir qu'un seul délégué à la fois. Toute nouvelle approbation remplace
la précédente.
