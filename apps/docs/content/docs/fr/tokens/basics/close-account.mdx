---
title: Fermer un compte de jeton
description:
  Fermer un compte de jeton et retourner les lamports du compte vers un compte de destination.
...
## Que fait la fermeture d'un compte de jeton ?

La fermeture d'un token account supprime le compte et transfère tous ses lamports,
y compris son solde minimum de stockage remboursable, vers un compte de destination.

Le solde en tokens doit être nul avant de pouvoir fermer le compte. Un compte gelé
peut toujours être fermé si le solde en tokens du compte gelé est nul. Les token accounts
de SOL encapsulé font exception et peuvent être fermés avec un solde en tokens pour
récupérer le SOL sous-jacent. Le propriétaire du compte ou l'autorité de fermeture du compte
signe l'instruction de fermeture et sélectionne la destination. Le compte qui a payé
pour créer un associated token account ne reçoit pas automatiquement le solde récupéré.

## Comment fermer un compte de jeton

La fermeture d'un compte de jeton utilise l'instruction _rs`CloseAccount`_ du
Token Program.

L'instruction _rs`CloseAccount`_ transfère les lamports du compte source vers un
compte de destination, efface le compte source et supprime le compte source. Le
Token Extensions Program applique des vérifications de fermeture supplémentaires
pour certaines extensions, mais le flux de fermeture de base reste le même.

### Référence source

| Élément                        | Description                                                                                                     | Token Program                                                                                                  | Token Extension Program                                                                                              |
| --------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | L'état du token account stocke le solde, le statut natif et l'autorité de fermeture utilisés lors de la fermeture du compte.     | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | Une instruction qui ferme un token account et transfère les lamports du token account vers un compte de destination. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | Logique de processeur partagée pour la fermeture de token account.                                                               | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |
