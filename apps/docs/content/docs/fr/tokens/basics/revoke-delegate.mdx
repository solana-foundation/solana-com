...
description:
  Supprimez le délégué actuel d'un compte de jetons et effacez son allocation
  déléguée restante.
...
## Que fait la révocation d'un délégué ?

La révocation d'un délégué efface le délégué actuel du compte de jetons et
réinitialise le montant délégué à zéro.

Après la révocation, le délégué précédent n'a plus l'autorité de transférer ou
de brûler des jetons au nom du compte de jetons.

## Comment révoquer un délégué

La révocation d'un délégué utilise l'instruction _rs`Revoke`_ du Token Program.

L'instruction _rs`Revoke`_ efface les champs de délégué du compte de jetons
source. Dans le Token Program, le propriétaire du compte de jetons signe la
révocation. Dans le Token Extensions Program, le délégué actuel peut également
...
| -------------------- | --------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_        | L'état du compte de jetons stocke le délégué et le montant délégué.   | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Revoke`_         | Une instruction qui supprime le délégué actuel d'un compte de jetons. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L128-L140) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L170-L182) |
| _rs`process_revoke`_ | Logique de processeur partagée pour la révocation de délégation.      | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L395-L420)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L643-L674)     |

### Typescript

Les exemples `Kit` ci-dessous présentent l'approche recommandée utilisant
`@solana/kit`. Des exemples historiques utilisant `@solana/web3.js` sont inclus
