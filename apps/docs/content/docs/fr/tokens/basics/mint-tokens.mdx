...
description:
  Frappez de nouvelles unités de jetons sur un compte de jetons avec l'autorité
  de frappe.
...
La frappe crée de nouvelles unités d'un jeton existant et augmente
l'approvisionnement total de la frappe.

Seule l'autorité de frappe peut frapper de nouveaux jetons. Le compte de jetons
de destination doit déjà exister et doit appartenir à la même frappe.

## Comment frapper des jetons

La frappe de jetons utilise l'instruction _rs`MintTo`_ ou _rs`MintToChecked`_ du
Token Program.

Les exemples ci-dessous utilisent _rs`MintToChecked`_, qui exige que l'appelant
fournisse les décimales de la frappe afin que l'instruction puisse vérifier la
précision attendue du jeton avant la frappe.

La [frappe native](/docs/tokens/basics/sync-native) ne prend pas en charge la
frappe.
...
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_            | L'état de la frappe stocke l'autorité de frappe et l'approvisionnement total en jetons.                                                                                    | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_         | L'état du token account stocke le solde de destination mis à jour par la frappe.                                                                                           | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`MintTo`_          | Une instruction de frappe qui frappe des jetons sur un token account existant sans exiger que l'appelant fournisse les décimales de la frappe.                             | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L159-L177) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L202-L220) |
| _rs`MintToChecked`_   | Une instruction de frappe qui exige que l'appelant fournisse les décimales de la frappe et vérifie cette valeur avant de frapper des jetons sur un token account existant. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L301-L325) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L367-L391) |
| _rs`process_mint_to`_ | Logique de traitement partagée pour la frappe de jetons.                                                                                                                   | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L521-L583)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L981-L1067)    |

### Typescript

Les exemples `Kit` ci-dessous montrent l'approche recommandée utilisant
`@solana/kit`. Des exemples legacy utilisant `@solana/web3.js` sont inclus à
titre de référence.
