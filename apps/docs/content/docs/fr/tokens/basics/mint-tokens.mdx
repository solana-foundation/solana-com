...
async fn main() -> Result<()> {
    ...
    // Générer un nouveau keypair pour le payeur de frais
    ...
    // Générer un keypair à utiliser comme adresse du mint
    let mint = Keypair::new();

    // Obtenir la taille par défaut du mint account (en octets), aucune extension activée
    ...
    // Instruction pour initialiser les données du mint account
    let initialize_mint_instruction = initialize_mint(
        &token_program_id(),
        &mint.pubkey(),            // mint
        &fee_payer.pubkey(),       // autorité de création
    ...
    // Instruction pour créer le associated token account
    ...
    // Créer la transaction et ajouter les instructions
    ...
    // Quantité de tokens à créer (100 tokens avec 2 décimales)
    let amount = 100;

    // Créer l'instruction mint_to pour créer des tokens vers le associated token account
    let mint_to_instruction = mint_to(
        &token_program_id(),
        &mint.pubkey(),            // mint
        &associated_token_address, // destination
        &fee_payer.pubkey(),       // autorité
        &[&fee_payer.pubkey()],    // signataire
        amount,                    // quantité
    )?;

    // Créer la transaction pour la création de tokens
    let transaction = Transaction::new_signed_with_payer(
        &[mint_to_instruction],
        Some(&fee_payer.pubkey()),
        &[&fee_payer],
    ...
    println!("1,00 tokens créés vers le associated token account");
    println!("\nAdresse du mint : {}", mint.pubkey());
    println!("{:#?}", mint_data);

    println!(
        "\nAdresse du associated token account : {}",
        associated_token_address
    );
    println!("{:#?}", token_data);

    println!("Signature de la transaction : {}", transaction_signature);
...
async def main():
    ...
    # Exemples de keypairs et d'adresses
    payer = Keypair()
    ...
    # Montant à créer (en plus petite unité)
    amount_to_mint = 1000000000  # 1 jeton avec 9 décimales

    async with rpc:
        # Créer l'instruction de création
        ...
        print(f"Montant : {amount_to_mint}")
        print(f"Autorité de création : {mint_authority.pubkey()}")
        print(f"Payeur : {payer.pubkey()}")
        print(f"Transaction de création créée avec succès")
