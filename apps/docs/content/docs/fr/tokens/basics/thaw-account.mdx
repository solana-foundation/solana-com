...
description:
  Dégeler un compte de jetons gelé pour permettre à nouveau les transferts, les
  destructions et les modifications de délégation.
...
## Que fait le dégel d'un compte de jetons ?

Le dégel fait passer un compte de jetons de l'état gelé à l'état initialisé.

Une fois le dégel réussi, le compte peut à nouveau transférer, détruire et
fermer des jetons. Seule l'autorité de gel du mint peut dégeler les comptes de
jetons.

## Comment dégeler un compte de jetons

Le dégel d'un compte de jetons utilise l'instruction _rs`ThawAccount`_ du Token
Program.

L'instruction _rs`ThawAccount`_ vérifie que le compte de jetons appartient au
mint et que le signataire est l'autorité de gel du mint, puis fait passer l'état
du compte de jetons de gelé à initialisé.

[Les comptes de jetons natifs](/docs/tokens/basics/sync-native) ne prennent pas
en charge le dégel.
...
| ----------------------------------- | ------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | L'état du mint stocke l'autorité de gel optionnelle utilisée pour dégeler les comptes de jetons. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Les états du compte de jetons, y compris _rs`Frozen`_ et _rs`Initialized`_.                      | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`ThawAccount`_                   | Une instruction qui fait repasser un compte de jetons gelé à l'état initialisé.                  | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L229-L243) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L292-L306) |
| _rs`process_toggle_freeze_account`_ | Logique du processeur partagée pour geler et dégeler les comptes de jetons.                      | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |
