...
description:
  Transférez des jetons entre des token accounts pour le même mint avec les
...
uniquement à jour les soldes entre les token accounts.
...
vérifier le mint attendu et la précision des jetons avant de déplacer les jetons
entre les comptes.

Dans le Token Extensions Program, _rs`Transfer`_ est déprécié en faveur de
