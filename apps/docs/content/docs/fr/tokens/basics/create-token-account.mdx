...
description:
  Créer des comptes de jetons et des comptes de jetons associés pour un mint spécifique et un propriétaire.
...
Un token account stocke des jetons pour un seul mint et un seul propriétaire de
token account sur Solana.
...
champ _rs`mint`_ du token account.

## Qu'est-ce qu'un compte de jetons associé

Un associated token account (ATA) est le token account par défaut pour un
portefeuille et un mint. Le
[Programme de jetons associés](https://github.com/solana-program/associated-token-account/tree/main/program/src)
dérive l'adresse ATA à partir de l'adresse du portefeuille, de l'adresse du
programme de jetons et de l'adresse du mint.

Seuls les comptes de jetons créés par l'Associated Token Program sont appelés
associated token accounts.

L'Associated Token Program est un moyen de créer un token account à une adresse
standard et déterministe. Le compte résultant reste un token account détenu par
le Token Program ou le Token Extensions Program, et non par l'Associated Token
Program.

L'Associated Token Program dérive l'adresse ATA comme indiqué dans
...
_rs`Account`_ défini par le Token Program ou le Token Extensions Program.

## Coût et Payeur

La création d'un ATA nécessite un solde minimum remboursable pour le stockage onchain du compte, ainsi que les frais de transaction. Le `payer` transmis à l'instruction de création finance le solde de stockage et doit signer. Le payeur peut être différent du payeur des frais de transaction et du portefeuille qui sera propriétaire de l'ATA, ce qui permet à une application de créer un ATA au nom d'un destinataire.

Au taux de stockage actuel du cluster, les coûts de base sont :

| Token program                             | Taille de l'ATA  | Solde de stockage minimum             |
| ----------------------------------------- | --------- | ----------------------------------- |
| Original Token Program                    | 165 octets | 2 039 280 lamports (0,00203928 SOL) |
| Token-2022 avec propriétaire immuable automatique | 170 octets | 2 074 080 lamports (0,00207408 SOL) |

Les mints Token-2022 peuvent nécessiter des extensions de compte en plus de l'extension automatique _rs`ImmutableOwner`_. Ces ATAs sont plus volumineux et requièrent un solde minimum plus élevé. L'Associated Token Program demande au Token Program sélectionné la taille de compte requise et finance l'ATA en conséquence.

Ne codez pas ces soldes en dur dans la logique applicative. Interrogez
[`getMinimumBalanceForRentExemption`](/docs/rpc/http/getminimumbalanceforrentexemption)
pour la taille de données requise, ou laissez l'Associated Token Program calculer et financer ce montant lors de la création.

<Callout type="info" title="Le solde de stockage est remboursable">
  La fermeture d'un token account vide transfère l'intégralité de ses lamports, y compris le solde de stockage minimum, vers la destination spécifiée par l'instruction de fermeture. Le payeur de la création initiale n'est pas automatiquement la destination du remboursement. Voir [Close Token Account](/docs/tokens/basics/close-account).
</Callout>
...
<Callout type="info" title="Recommandé">
  ...
  instructions du programme système et du programme de jetons. Un compte de
  jetons associé utilise une adresse déterministe dérivée du propriétaire, du
  programme de jetons et du mint, ce qui facilite la recherche du compte de
  jetons par défaut pour un mint donné par les portefeuilles et les
  applications.
</Callout>
...
| _rs`CreateIdempotent`_                        | Crée le compte de jetons associé, mais réussit quand même si cet ATA existe déjà pour le même propriétaire et mint.                                                                                                                                               | [Source](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | Dérive l'adresse ATA et utilise _rs`create_pda_account`_ ainsi que des CPI vers le programme de jetons sélectionné pour initialiser le compte de jetons. Lorsque le programme sélectionné est le programme d'extensions de jetons, le processeur ATA initialise également l'extension de propriétaire immuable. | [Source](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | Utilitaire utilisé par _rs`process_create_associated_token_account`_ pour créer le compte PDA par CPI vers le programme système.                                                                                                                                         | [Source](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |
...
1. L'instruction _rs`CreateAccount`_ du System Program crée un nouveau compte
   exempté de rent et attribue le Token Program comme propriétaire du programme
   du nouveau compte.
...
<Callout type="info">
  ...
  plupart des applications, utilisez plutôt l'Associated Token Program. Utilisez
  les appels directs au System Program et au Token Program lorsque vous avez une
  raison spécifique de ne pas utiliser un associated token account ou lorsque
  vous devez créer des token accounts PDA personnalisés en effectuant des CPI
  vers les instructions du System Program et du Token Program depuis votre
  propre programme Solana.
</Callout>
