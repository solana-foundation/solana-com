...
Un compte de jetons stocke des jetons pour une seule émission et un seul
propriétaire de compte de jetons sur Solana.
...
jetons depuis le compte de jetons. Le propriétaire du programme du compte reste
le Token Program ou le Token Extensions Program.

Chaque compte de jetons est lié à une seule émission, ce qui signifie que le
compte de jetons ne peut contenir que des unités d'un seul jeton, celui
identifié par le champ _rs`mint`_ du compte de jetons.

## Qu'est-ce qu'un compte de jetons associé

Un associated token account (ATA) est le compte de jetons par défaut pour un
portefeuille et une émission. Le
...
programme de jetons et de l'adresse de l'émission.

Seuls les comptes de jetons créés par l'Associated Token Program sont appelés
associated token accounts.

L'Associated Token Program est une méthode pour créer un compte de jetons à une
adresse standard et déterministe. Le compte résultant reste un token account
détenu par le Token Program ou le Token Extensions Program, et non par
...
existe exactement une seule adresse d'ATA. L'Associated Token Program crée un
...
compte en tant que compte de jetons détenu par le Programme de jetons ou le
...
| _rs`CreateIdempotent`_                        | Crée le compte de jetons associé, mais réussit même si cet ATA existe déjà pour le même propriétaire et le même mint.                                                                                                                                                                                           | [Source](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
...
| _rs`create_pda_account`_                      | Assistant utilisé par _rs`process_create_associated_token_account`_ pour créer le compte PDA par CPI vers le Programme système.                                                                                                                                                                                 | [Source](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |

### Typescript

Les exemples `Kit` ci-dessous montrent l'approche recommandée utilisant
`@solana/kit`. Des exemples anciens utilisant `@solana/web3.js` sont inclus à
titre de référence.
...
compte n'est pas déjà initialisé et est exempté de rent.
...
Les exemples historiques utilisant `@solana/web3.js` sont inclus à titre de
référence.
