...
<Callout type="info">
  Notez que dans le code source, les développeurs appellent un token account un
  type `Account`. Le [Token
...
1. **Mint** : le type de jeton spécifique qu'il détient
...
- Circle contrôle les jetons via l'autorité owner du token account :
...
1. **Token account owner** : l'autorité qui peut transférer, brûler ou déléguer
   des jetons. Ceci est stocké dans les données du token account.

2. **Program owner** : le programme qui possède le compte (toujours le Token
   Program ou le Token Extension Program pour les token accounts).

Lorsque vous travaillez avec des token accounts, « owner » fait généralement
...
Un associated token account (ATA) est le compte de jetons par défaut d'un
portefeuille pour détenir un jeton spécifique. Il utilise une adresse
...
<Callout type="info">
  Seuls les comptes de jetons créés par le Associated Token Program sont appelés
  "associated token accounts".
</Callout>

Les ATA utilisent des adresses déterministes, ce qui facilite la recherche du
compte de jetons d'un portefeuille pour un mint donné. Voir
...
adresse ATA. Cela élimine le besoin de suivre manuellement les adresses des
comptes de jetons—vous pouvez toujours dériver l'adresse correcte.

<Callout type="info">
  **Comment fonctionne le Associated Token Program**

Le Associated Token Program est un assistant qui :

- Crée des comptes de jetons à des adresses déterministes (PDA)
- Effectue des invocations inter-programmes (CPI) vers le Token Program
- Ne maintient aucun état lui-même

Les comptes de jetons résultants appartiennent au Token Program et utilisent la
structure `Account` standard.
...
son initialisation via des invocations de programmes croisés (CPI).
