...
détruit définitivement des tokens en réduisant le solde dans un token account.
Les tokens brûlés sont retirés de la circulation et diminuent le `supply` suivi
sur le mint account. Seul le propriétaire du token account peut brûler des
tokens de son compte.
