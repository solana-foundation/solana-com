...
description:
  Brûler des jetons d'un compte de jetons et réduire l'offre totale du mint.
...
## Que fait la combustion ?

La combustion de jetons diminue de manière permanente le solde d'un compte de
jetons et réduit l'offre totale du mint du même montant.

## Comment brûler des jetons

La combustion de jetons utilise l'instruction _rs`Burn`_ ou _rs`BurnChecked`_ du
Token Program.

Les exemples ci-dessous utilisent _rs`BurnChecked`_, qui exige que l'appelant
fournisse les décimales du mint afin que l'instruction puisse vérifier la
précision attendue du jeton avant la combustion.

Le [native mint](/docs/tokens/basics/sync-native) ne prend pas en charge la
combustion. Utilisez _rs`CloseAccount`_ à la place.

Le propriétaire du compte de jetons ou un délégué approuvé signe la transaction.
Dans le Token Extensions Program, un délégué permanent du mint peut également
autoriser la combustion si le mint a l'extension de délégué permanent activée.

### Référence source

| Élément            | Description                                                                                                                                                                         | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_         | L'état du mint stocke l'offre totale de jetons.                                                                                                                                     | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | L'état du compte de jetons stocke le solde de jetons réduit par une combustion.                                                                                                     | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Une instruction de combustion qui diminue le solde d'un compte de jetons et l'offre du mint sans exiger que l'appelant fournisse les décimales du mint.                             | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Une instruction de combustion qui exige que l'appelant fournisse les décimales du mint et vérifie cette valeur avant de diminuer le solde d'un compte de jetons et l'offre du mint. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | Logique de processeur partagée pour les combustions de jetons.                                                                                                                      | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |
