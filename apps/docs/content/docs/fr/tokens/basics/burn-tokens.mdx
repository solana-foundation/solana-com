...
Brûler des jetons diminue de façon permanente le solde d'un token account et
réduit l'offre totale du mint du même montant.
...
précision attendue du jeton avant de procéder à la combustion.

Le [native mint](/docs/tokens/basics/sync-native) ne prend pas en charge la
combustion. Utilisez _rs`CloseAccount`_ à la place.

Le propriétaire du token account ou un délégué approuvé signe la transaction.
...
| _rs`Account`_      | L'état du token account stocke le solde de jetons réduit par une combustion.                                                                                                     | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Une instruction de combustion qui diminue le solde d'un token account et l'offre du mint sans exiger que l'appelant fournisse les décimales du mint.                             | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Une instruction de combustion qui exige que l'appelant fournisse les décimales du mint et vérifie cette valeur avant de diminuer le solde d'un token account et l'offre du mint. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | Logique partagée du processeur pour les combustions de jetons.                                                                                                                   | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |
