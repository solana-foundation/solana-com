...
description:
  Modifier ou révoquer les rôles d'autorité sur un compte de mint ou de jeton, y
  compris l'autorité de mint, de gel, de propriétaire et de fermeture.
...
## Que fait la définition d'autorité ?

La définition d'autorité modifie ou révoque des rôles d'autorité spécifiques sur
un mint account ou un token account.

Les mint accounts peuvent stocker une autorité de mint et une autorité de gel.
Les token accounts stockent un propriétaire et peuvent éventuellement stocker
une autorité de fermeture. Chaque rôle contrôle un ensemble de permissions
spécifiques, et définir une autorité sur _rs`None`_ supprime définitivement le
rôle d'autorité sélectionné.

Le Token Extensions Program définit également des types d'autorité
supplémentaires pour des extensions spécifiques.

## Comment définir une autorité

La modification d'une autorité utilise l'instruction _rs`SetAuthority`_ du Token
Program.

Chaque appel met à jour un type d'autorité sur un mint account ou un token
account. L'autorité actuelle signe la modification, et la nouvelle autorité peut
être une autre adresse ou _rs`None`_ pour révoquer définitivement l'autorité
sélectionnée.
...
| --------------------------- | -------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                  | L'état du mint stocke _rs`mint_authority`_ et _rs`freeze_authority`_.                        | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)           |
| _rs`Account`_               | L'état du token account stocke _rs`owner`_ et _rs`close_authority`_.                         | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)          |
| _rs`AuthorityType`_         | Les rôles d'autorité que _rs`SetAuthority`_ peut mettre à jour.                              | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L739-L751) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1116-L1157) |
| _rs`SetAuthority`_          | Une instruction qui modifie ou révoque une autorité sur un mint account ou un token account. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L141-L158) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L183-L200)   |
| _rs`process_set_authority`_ | Logique de processeur partagée pour les mises à jour d'autorité.                             | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L423-L518)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L677-L978)       |
