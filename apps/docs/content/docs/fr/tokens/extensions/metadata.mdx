...
description:
  Créez un mint qui stocke les métadonnées directement sur le compte mint en utilisant les extensions MetadataPointer et TokenMetadata, puis mettez à jour, supprimez et verrouillez ces métadonnées via l'interface token-metadata.
...
L'exemple `Kit` ci-dessous utilise directement les instructions générées. Les exemples legacy utilisant `@solana/web3.js`, `@solana/spl-token` et
`@solana/spl-token-metadata` sont inclus à titre de référence. Les exemples legacy limitent
`maxSupportedTransactionVersion` à `0` car web3.js v1 ne peut pas désérialiser
[les transactions v1](/docs/core/transactions/versioned-transactions#v1-message-format).
