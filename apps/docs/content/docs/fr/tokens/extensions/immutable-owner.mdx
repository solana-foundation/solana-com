...
description:
  Initialiser un compte de jeton Token Extensions Program avec propriété
  immuable afin que son propriétaire ne puisse jamais être réassigné.
...
## Qu'est-ce que le propriétaire immuable ?

L'extension de compte _rs`ImmutableOwner`_ du Token Extensions Program rend le
propriétaire d'un token account permanent.

Une fois qu'un token account est initialisé avec _rs`ImmutableOwner`_, les
instructions _rs`SetAuthority`_ ultérieures ne peuvent pas modifier son autorité
_rs`AccountOwner`_.

<Callout type="info" title="Comptes de jetons associés">
  Les associated token accounts créés via l'Associated Token Program pour le
  Token Extensions Program initialisent automatiquement _rs`ImmutableOwner`_ sur
  le token account.
</Callout>

## Comment initialiser un compte avec propriétaire immuable

Pour initialiser un compte avec propriétaire immuable :

1. Créer et initialiser un mint.
2. Créer un token account avec suffisamment d'espace pour _rs`ImmutableOwner`_.
3. Initialiser _rs`ImmutableOwner`_ sur le token account avant
   _rs`InitializeAccount`_.
4. Toute tentative de modification ultérieure du propriétaire du compte avec
   _rs`SetAuthority`_ échoue avec _rs`TokenError::ImmutableOwner`_.

<ScrollyCoding>

## !!steps Calculer la taille du token account

Calculer la taille du token account pour le compte de jeton de base plus
l'extension _rs`ImmutableOwner`_. C'est la taille utilisée dans
_rs`CreateAccount`_.
...
Calculer le rent en utilisant la taille du token account nécessaire pour
l'extension _rs`ImmutableOwner`_.
...
## !!steps Créer le token account

Créer le token account avec l'espace et les lamports calculés.
...
## !!steps Initialiser ImmutableOwner

Initialiser l'extension _rs`ImmutableOwner`_ sur le compte de jetons.
...
## !!steps Initialiser le compte de jetons

Initialiser le compte de jetons avec _rs`InitializeAccount`_ dans la même
transaction.
...
<Callout type="info" title="Ordre des instructions">
  _rs`InitializeImmutableOwner`_ doit précéder _rs`InitializeAccount`_.
  _rs`CreateAccount`_, _rs`InitializeImmutableOwner`_ et _rs`InitializeAccount`_
...
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| _rs`ImmutableOwner`_                     | Extension de compte qui marque un propriétaire de compte de jetons comme immuable.                                           | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L13-L16) |
| _rs`InitializeImmutableOwner`_           | Instruction qui initialise l'extension de propriétaire immuable sur un compte de jetons non initialisé.                      | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1982-L1991)           |
| _rs`process_initialize_immutable_owner`_ | Logique de traitement qui alloue et initialise _rs`ImmutableOwner`_ sur le compte.                                           | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1532-L1543)               |
| _rs`SetAuthority`_                       | Instruction de jeton de base utilisée pour modifier les autorités, y compris un propriétaire de compte de jetons.            | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1611-L1633)           |
| _rs`process_set_authority`_              | Logique de traitement qui rejette les changements de propriétaire lorsque le compte de jetons a _rs`ImmutableOwner`_ activé. | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L703-L900)                 |
