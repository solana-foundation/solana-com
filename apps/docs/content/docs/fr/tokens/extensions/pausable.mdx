...
title: Mint pausable
description:
  Créez un mint Token-2022 avec une autorité de pause, bloquez les transferts
  pendant que le mint est en pause et reprenez l'activité ultérieurement.
...
## Qu'est-ce qu'un mint pausable ?

L'extension de mint _rs`PausableConfig`_ du Token Extensions Program donne à une
autorité de pause la capacité de mettre en pause et de reprendre l'activité de
jetons pour l'ensemble du mint.

Lorsqu'un mint est en pause, le Token Extensions Program rejette :

- Les transferts
- Les émissions
- Les destructions

Lorsque le mint est repris, ces instructions fonctionnent à nouveau normalement.

## Comment créer, mettre en pause et reprendre un mint

Pour créer, mettre en pause et reprendre un mint :

1. Calculez la taille du mint account et le rent nécessaire pour le mint et
   l'extension _rs`PausableConfig`_.
2. Créez le mint account avec _rs`CreateAccount`_, initialisez
   _rs`PausableConfig`_, et initialisez le mint avec _rs`InitializeMint`_.
3. Émettez des jetons et créez des token accounts pour le mint.
4. Utilisez _rs`Pause`_ pour bloquer l'activité des jetons pour le mint.
5. Utilisez _rs`Resume`_ pour autoriser à nouveau l'activité des jetons.
...
_rs`PausableConfig`_. C'est la taille utilisée dans _rs`CreateAccount`_.
...
## !!steps Initialiser PausableConfig

Initialisez l'extension _rs`PausableConfig`_ sur le mint.
...
## !!steps Mettre le mint en pause

Mettez le mint en pause avec _rs`Pause`_.
...
## !!steps Reprendre le mint

Reprenez le mint avec _rs`Resume`_.
...
<Callout type="info" title="Ordre des instructions">
  _rs`PausableInstruction::Initialize`_ doit venir avant _rs`InitializeMint`_.
  _rs`CreateAccount`_, _rs`PausableInstruction::Initialize`_ et
...
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PausableConfig`_                  | Extension de mint qui stocke l'autorité de pause et indique si le mint est actuellement en pause.                   | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/mod.rs#L17-L22)         |
| _rs`PausableInstruction::Initialize`_ | Instruction qui initialise la configuration pausable avant _rs`InitializeMint`_.                                    | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L20-L32) |
| _rs`PausableInstruction::Pause`_      | Instruction qui marque le mint comme étant en pause.                                                                | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L33-L44) |
| _rs`PausableInstruction::Resume`_     | Instruction qui marque le mint comme n'étant plus en pause.                                                         | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L45-L56) |
| _rs`process_initialize`_              | Logique du processeur qui initialise _rs`PausableConfig`_ sur un mint non initialisé et stocke l'autorité de pause. | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L22-L35)     |
| _rs`process_toggle_pause`_            | Logique du processeur qui vérifie l'autorité de pause avant de basculer l'indicateur de pause du mint.              | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L39-L64)     |

### Typescript

L'exemple `Kit` ci-dessous utilise directement les instructions générées. Des
exemples legacy utilisant `@solana/web3.js` et `@solana/spl-token` sont inclus à
titre de référence.
