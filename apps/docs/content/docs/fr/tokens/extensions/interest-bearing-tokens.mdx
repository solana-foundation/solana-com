...
vous permet de définir un taux d'intérêt directement sur un mint account. Les
intérêts sont composés en continu en fonction de l'horodatage du réseau, mais le
solde de tokens réel stocké dans chaque token account reste inchangé. La valeur
totale (solde de tokens plus intérêts accumulés) n'est qu'un calcul. Aucun
nouveau token n'est créé avec cette extension.
