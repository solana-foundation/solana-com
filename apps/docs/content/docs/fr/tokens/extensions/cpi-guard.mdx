...
title: CPI Guard
description:
  Activez l'extension de compte CpiGuard pour bloquer les instructions de jetons
  invoquées via Cross Program Invocation.
...
## Qu'est-ce que CPI Guard ?

L'extension de compte _rs`CpiGuard`_ du Token Extensions Program empêche un
autre programme onchain d'utiliser un token account pour certaines instructions
de jetons via Cross Program Invocation (CPI).

Lorsque CPI guard est activé, le Token Extensions Program rejette ces
instructions quand un autre programme onchain les invoque via CPI :

- _rs`Transfer`_, _rs`TransferChecked`_ et _rs`TransferCheckedWithFee`_ lorsque
  le propriétaire du token account est utilisé comme autorité de l'instruction
- _rs`Burn`_ et _rs`BurnChecked`_ lorsque le propriétaire du token account est
  utilisé comme autorité de l'instruction
- _rs`Approve`_ et _rs`ApproveChecked`_
- _rs`CloseAccount`_ lorsque des lamport seraient envoyés ailleurs que vers le
  propriétaire du token account
- _rs`SetAuthority`_ lors de l'ajout ou du remplacement de l'autorité de
  fermeture

Lorsque CPI guard est activé, ces cas restent autorisés :

- _rs`Transfer`_, _rs`TransferChecked`_, _rs`TransferCheckedWithFee`_,
  _rs`Burn`_ et _rs`BurnChecked`_ via CPI lorsqu'un délégué ou un délégué
  permanent signe au lieu du propriétaire du token account
- _rs`CloseAccount`_ via CPI lorsque des lamport sont envoyés au propriétaire du
  token account
- _rs`SetAuthority`_ pour supprimer une autorité de fermeture existante
- _rs`Revoke`_

Lorsque le propriétaire du token account est utilisé comme autorité de
l'instruction, les instructions du Token Extensions Program peuvent toujours
être traitées si l'instruction est ajoutée directement à la transaction au lieu
d'être invoquée via CPI. La modification du propriétaire du token account avec
_rs`SetAuthority`_ reste bloquée tant que CPI guard est activé, même en dehors
de CPI. _rs`EnableCpiGuard`_ et _rs`DisableCpiGuard`_ ne peuvent pas non plus
être invoqués via CPI.

## Comment activer CPI Guard

Pour activer CPI guard :

1. Créer et initialiser un mint.
2. Calculer la taille du compte de jetons et le rent nécessaire pour un compte
   de jetons avec _rs`CpiGuard`_.
3. Créer et initialiser un compte de jetons pour le mint.
4. Invoquer _rs`EnableCpiGuard`_ sur le compte de jetons.
5. Invoquer _rs`DisableCpiGuard`_ sur le compte de jetons pour autoriser à
   nouveau ces instructions via CPI.

<ScrollyCoding>

## !!steps Créer et initialiser le mint

Créer et initialiser le mint que le compte de jetons utilisera.
...
## !!steps Calculer la taille du compte de jetons

Calculer la taille du compte de jetons pour le compte de jetons de base plus
l'extension _rs`CpiGuard`_. Il s'agit de la taille utilisée dans
_rs`CreateAccount`_.
...
Calculer le rent en utilisant la taille du compte de jetons nécessaire pour
l'extension _rs`CpiGuard`_.
...
## !!steps Créer et initialiser le compte de jetons

Créer le compte de jetons avec l'espace et les lamports calculés, puis
l'initialiser pour le mint.
...
## !!steps Activer la protection CPI

Activer _rs`CpiGuard`_ sur le compte de jetons.
...
## !!steps Désactiver la protection CPI

Désactiver _rs`CpiGuard`_ sur le compte de jetons.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`CpiGuard`_                     | Extension de compte qui stocke si la protection CPI est actuellement activée pour le token account.                                                                                                                                                     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/mod.rs#L20-L25)         |
| _rs`CpiGuardInstruction::Enable`_  | Instruction qui active la protection CPI sur un token account.                                                                                                                                                                                          | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/instruction.rs#L20-L38) |
| _rs`CpiGuardInstruction::Disable`_ | Instruction qui désactive la protection CPI sur un token account.                                                                                                                                                                                       | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/instruction.rs#L39-L53) |
| _rs`process_toggle_cpi_guard`_     | Logique de traitement utilisée par _rs`Enable`_ et _rs`Disable`_ qui valide la propriété du compte, rejette les modifications CPI de la protection elle-même, et initialise _rs`CpiGuard`_ si elle n'est pas déjà présente avant de définir `lock_cpi`. | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/cpi_guard/processor.rs#L19-L52)     |
