...
description:
  Exiger que les transferts entrants incluent un mémo en activant l'extension de
  compte MemoTransfer sur un compte de jetons du Token Extensions Program.
...
## Qu'est-ce que le transfert avec mémo ?

L'extension de compte _rs`MemoTransfer`_ du Token Extensions Program exige
qu'une instruction de mémo précède immédiatement tout transfert entrant vers ce
compte de jetons. Les transferts sortants de ce compte de jetons ne nécessitent
pas de mémo.

L'extension est configurée sur le compte de jetons de destination, et non sur le
mint. Une fois activée, les transferts sans mémo échouent avec l'erreur
_rs`TokenError::NoMemo`_ jusqu'à ce que le propriétaire du compte de jetons
désactive cette exigence.

## Comment créer un compte de jetons avec MemoTransfer activé

Pour créer un compte de jetons avec _rs`MemoTransfer`_ activé :

1. Créez et initialisez un mint.
2. Calculez la taille du compte de jetons et le loyer nécessaire pour un compte
   de jetons avec _rs`MemoTransfer`_.
3. Créez le compte de jetons et activez _rs`MemoTransfer`_ sur ce compte.
4. Les instructions _rs`Transfer`_ et _rs`TransferChecked`_ vers ce compte de
   jetons doivent inclure une instruction de mémo immédiatement avant
...
Créez et initialisez le mint que le compte de jetons utilisera.
...
Calculez la taille du compte de jetons pour le compte de jetons de base plus
l'extension _rs`MemoTransfer`_. C'est la taille utilisée dans
_rs`CreateAccount`_.
...
Calculez le loyer en utilisant la taille du compte de jetons nécessaire pour
l'extension _rs`MemoTransfer`_.
...
Créez le compte de jetons avec l'espace et les lamports calculés, puis
initialisez-le pour le mint.
...
## !!steps Activer MemoTransfer

Activez _rs`MemoTransfer`_ sur le compte de jetons.
...
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`MemoTransfer`_                              | Extension de compte qui stocke si les transferts entrants nécessitent un mémo.                                     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/mod.rs#L17-L22)         |
| _rs`RequiredMemoTransfersInstruction::Enable`_  | Instruction qui active les transferts avec mémo obligatoire sur un compte de jetons.                               | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L50-L71) |
| _rs`RequiredMemoTransfersInstruction::Disable`_ | Instruction qui désactive les transferts avec mémo obligatoire sur un compte de jetons.                            | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L74-L95) |
| _rs`process_toggle_required_memo_transfers`_    | Logique de traitement qui initialise ou réécrit l'extension _rs`MemoTransfer`_ après vérification de la propriété. | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/memo_transfer/processor.rs#L20-L48)     |

### Typescript

L'exemple `Kit` ci-dessous utilise les instructions générées directement. Les
exemples hérités utilisant `@solana/web3.js`, `@solana/spl-token` et
`@solana/spl-memo` sont inclus à titre de référence.
