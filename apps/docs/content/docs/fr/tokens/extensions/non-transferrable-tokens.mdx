...
description:
  Créez un mint dont les jetons peuvent être émis, brûlés et clôturés, mais ne
  peuvent pas être transférés entre comptes de jetons.
...
## Qu'est-ce que les jetons non transférables ?

L'extension _rs`NonTransferable`_ du mint du Token Extensions Program rend
chaque token account de ce mint non transférable. Une fois les jetons émis, les
détenteurs ne peuvent pas les déplacer vers un autre token account avec
_rs`Transfer`_ ou _rs`TransferChecked`_.

Ce modèle est utile pour les actifs qui doivent rester attachés à un seul
portefeuille.

Les jetons non transférables peuvent toujours être :

- Émis par l'autorité de mint
- Brûlés par le propriétaire du token account ou un délégué autorisé
- Les token accounts peuvent être clôturés une fois le solde réduit à zéro

<Callout type="info" title="Extensions de compte de jetons">
  Lorsqu'un token account est initialisé pour un mint avec
  _rs`NonTransferable`_, le token account est initialisé avec
  _rs`NonTransferableAccount`_ et _rs`ImmutableOwner`_. Lorsque le token account
  est créé via l'[Associated Token
  Program](/docs/tokens/basics/create-token-account#how-to-create-an-associated-token-account),
  la taille de compte requise est calculée et le token account est créé avec la
  taille requise et les lamports exemptés de rent.
</Callout>

## Comment créer un mint non transférable

Pour créer un mint non transférable :

1. Calculez la taille du mint account et le rent nécessaire pour le mint et
   l'extension _rs`NonTransferable`_.
2. Créez le mint account avec _rs`CreateAccount`_, initialisez
   _rs`NonTransferable`_, et initialisez le mint avec _rs`InitializeMint`_.
3. Créez des token accounts pour le mint. _rs`NonTransferableAccount`_ et
   _rs`ImmutableOwner`_ sont automatiquement activées pour les token accounts.
4. _rs`Transfer`_ et _rs`TransferChecked`_ échouent avec
...
_rs`NonTransferable`_. C'est la taille utilisée dans _rs`CreateAccount`_.
...
## !!steps Initialiser NonTransferable

Initialisez l'extension _rs`NonTransferable`_ sur le mint.
...
<Callout type="info" title="Ordre des instructions">
  _rs`InitializeNonTransferableMint`_ doit venir avant _rs`InitializeMint`_.
  _rs`CreateAccount`_, _rs`InitializeNonTransferableMint`_ et
  _rs`InitializeMint`_ doivent être inclus dans la même transaction.
</Callout>
...
| ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| _rs`NonTransferable`_                          | Extension de mint qui marque les jetons du mint comme non transférables.                                                                                                                                                                                                      | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/non_transferable.rs#L8-L13)  |
| _rs`NonTransferableAccount`_                   | Extension de compte de jetons ajoutée aux comptes de jetons pour les mints non transférables.                                                                                                                                                                                 | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/non_transferable.rs#L15-L21) |
| _rs`ImmutableOwner`_                           | Extension de compte de jetons qui empêche les changements de propriété et est requise pour les comptes de jetons non transférables.                                                                                                                                           | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L8-L16)   |
| _rs`InitializeNonTransferableMint`_            | Instruction qui initialise l'extension non transférable au niveau du mint avant _rs`InitializeMint`_.                                                                                                                                                                         | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L642-L653)              |
| _rs`process_initialize_non_transferable_mint`_ | Logique du processeur qui initialise l'extension de mint _rs`NonTransferable`_ sur un mint non initialisé.                                                                                                                                                                    | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1646-L1655)                |
| _rs`get_required_init_account_extensions`_     | Utilisé pour ajouter automatiquement des extensions de compte de jetons lors de l'initialisation des comptes de jetons en fonction des extensions activées sur le mint. Pour les mints _rs`NonTransferable`_, il ajoute _rs`NonTransferableAccount`_ et _rs`ImmutableOwner`_. | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mod.rs#L1304-L1307)          |

### Typescript

L'exemple `Kit` ci-dessous utilise directement les instructions générées. Des
exemples hérités utilisant `@solana/web3.js` sont inclus à titre de référence.
