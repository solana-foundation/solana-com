...
description:
  Créez un mint du Token Extensions Program avec un délégué permanent qui peut
  transférer ou brûler des jetons depuis n'importe quel compte pour ce mint.
...
## Qu'est-ce qu'un délégué permanent ?

L'extension de mint _rs`PermanentDelegate`_ du Token Extensions Program attribue
une autorité de délégation au mint lui-même.

Un délégué permanent est une autorité au niveau du mint qui peut autoriser les
transferts et les destructions pour n'importe quel token account de ce mint. Il
agit comme un délégué global pour tous les token accounts du mint, et les
propriétaires de token accounts ne peuvent pas révoquer le délégué permanent de
leurs token accounts.

L'autorité du mint peut également modifier le délégué permanent ultérieurement
avec _rs`SetAuthority`_ en utilisant _rs`AuthorityType::PermanentDelegate`_.

## Comment créer et utiliser un délégué permanent

Pour créer et utiliser un délégué permanent :

1. Calculez la taille du mint account et le rent nécessaire pour le mint et
   l'extension _rs`PermanentDelegate`_.
2. Créez le mint account avec _rs`CreateAccount`_, initialisez
   _rs`PermanentDelegate`_, et initialisez le mint avec _rs`InitializeMint`_.
3. Soumettez _rs`TransferChecked`_ ou _rs`BurnChecked`_ signé par le délégué
   permanent. Le token program traite ces instructions de la même manière que si
   le propriétaire du token account les avait signées.
...
_rs`PermanentDelegate`_. C'est la taille utilisée dans _rs`CreateAccount`_.
...
## !!steps Initialiser PermanentDelegate

Initialisez l'extension _rs`PermanentDelegate`_ sur le mint.
...
## !!steps Créer des token accounts et mint des tokens

Créez des token accounts pour le propriétaire et le destinataire, puis mintez
des tokens vers le token account du propriétaire.
...
## !!steps Transférer avec le délégué permanent

Transférez des tokens avec _rs`TransferChecked`_ signé par le délégué permanent.
...
<Callout type="info" title="Ordre des instructions">
  _rs`InitializePermanentDelegate`_ doit venir avant _rs`InitializeMint`_.
  _rs`CreateAccount`_, _rs`InitializePermanentDelegate`_ et _rs`InitializeMint`_
...
| ------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermanentDelegate`_                     | Extension de mint qui stocke le délégué autorisé pour chaque token account du mint.                                               | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/permanent_delegate.rs#L15-L21) |
| _rs`InitializePermanentDelegate`_           | Instruction qui initialise le délégué permanent avant _rs`InitializeMint`_.                                                       | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1952-L1967)              |
| _rs`SetAuthority`_                          | Instruction de token de base utilisée avec _rs`AuthorityType::PermanentDelegate`_ pour faire pivoter le délégué.                  | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1488-L1520)              |
| _rs`AuthorityType::PermanentDelegate`_      | Discriminateur d'autorité utilisé avec _rs`SetAuthority`_ pour faire pivoter le délégué permanent sur un mint.                    | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1138-L1139)              |
| _rs`process_initialize_permanent_delegate`_ | Logique du processeur qui écrit l'extension de délégué permanent sur le mint.                                                     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1542-L1557)                  |
| _rs`process_set_authority`_                 | Logique du processeur qui valide et fait pivoter le délégué permanent lorsque _rs`AuthorityType::PermanentDelegate`_ est utilisé. | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L849-L860)                    |
