...
description:
  Créez un mint du Token Extensions Program avec l'extension MintCloseAuthority,
  puis brûlez l'offre restante et fermez le mint.
...
L'extension de mint _rs`MintCloseAuthority`_ du Token Extensions Program permet
à une autorité désignée de fermer un mint account et de récupérer son rent une
fois que l'offre totale du mint atteint zéro.

Sans cette extension, un mint account ne peut pas être fermé.

- L'offre totale doit être de zéro, donc tous les tokens de tous les token
  accounts doivent être brûlés avant qu'un mint puisse être fermé
...
1. Calculez la taille du mint account et le rent nécessaire pour le mint et
   l'extension _rs`MintCloseAuthority`_.
2. Créez le mint account avec _rs`CreateAccount`_, initialisez
   _rs`MintCloseAuthority`_, et initialisez le mint avec _rs`InitializeMint`_.
3. Si l'offre totale est zéro, le mint account peut être fermé avec
   _rs`CloseAccount`_, signé par l'autorité de fermeture.
...
_rs`MintCloseAuthority`_. C'est la taille utilisée dans _rs`CreateAccount`_.
...
## !!steps Initialiser MintCloseAuthority

Initialisez l'extension _rs`MintCloseAuthority`_ sur le mint.
...
<Callout type="info" title="Ordre des instructions">
  _rs`InitializeMintCloseAuthority`_ doit précéder _rs`InitializeMint`_.
  _rs`CreateAccount`_, _rs`InitializeMintCloseAuthority`_ et
...
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`MintCloseAuthority`_                      | Extension de mint qui stocke l'autorité autorisée à fermer un mint une fois que son offre atteint zéro.                   | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mint_close_authority.rs#L14-L20) |
| _rs`InitializeMintCloseAuthority`_            | Instruction qui initialise l'autorité de fermeture du mint avant _rs`InitializeMint`_.                                    | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1830-L1843)                |
| _rs`CloseAccount`_                            | Instruction de token de base qui ferme le compte mint après que l'offre totale atteint zéro.                              | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1582-L1608)                |
| _rs`process_initialize_mint_close_authority`_ | Logique de processeur qui écrit l'extension _rs`MintCloseAuthority`_ sur un mint non initialisé.                          | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1376-L1389)                    |
| _rs`process_close_account`_                   | Gestionnaire de fermeture de compte partagé qui applique la règle d'offre zéro pour les mints avec autorité de fermeture. | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1288)                    |
