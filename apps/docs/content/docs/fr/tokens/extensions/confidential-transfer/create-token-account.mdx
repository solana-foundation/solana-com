...
<Callout type="warn">
  Le ZK ElGamal Program est temporairement désactivé sur le mainnet et le devnet
  pendant qu'il fait l'objet d'un audit de sécurité. Cela signifie que
  l'extension confidential transfers est actuellement indisponible. Bien que les
  concepts restent valides, les exemples de code ne fonctionneront pas.
</Callout>
...
pour prévenir les attaques DoS. Sans cette séparation, un attaquant pourrait
envoyer des tokens de manière répétée vers un token account, bloquant la
...
disponible. Les transferts ou dépôts entrants n'affectent pas le solde
...
les nombres plus grands. Vous pouvez trouver l'implémentation de l'arithmétique
des textes chiffrés
...
1. Le client consulte les soldes en attente et disponibles actuels, chiffre la
...
`decryptable_available_balance`, des valeurs de compteurs attendus/réels
différentes requièrent que le client consulte les instructions récentes de
...
2. Récupérez les transactions les plus récentes incluant les instructions de
   ...
   - Déchiffrez et ajoutez les montants de texte chiffré de destination des
...
1. **Créer le token account** : Invoquez l'instruction
   _rs`AssociatedTokenAccountInstruction:Create`_ de l'Associated Token Program
   pour créer le token account.

2. **Réallouer l'espace du compte** : invoquez l'instruction
   _rs`TokenInstruction::Reallocate`_ du Token Extension Program pour ajouter de
   l'espace pour l'état _rs`ConfidentialTransferAccount`_.

3. **Configurer les transferts confidentiels** : invoquez l'instruction
   ...
   du Token Extension Program pour initialiser l'état
