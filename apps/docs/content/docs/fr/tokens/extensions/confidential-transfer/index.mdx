...
<Callout type="warn">
  ...
  que les concepts restent valides, les exemples de code ne fonctionneront pas.
</Callout>

## Qu'est-ce que les transferts confidentiels ?

Les transferts confidentiels vous permettent de transférer des tokens entre des
comptes de tokens sans révéler le montant du transfert. Cela est utile pour les
transactions préservant la confidentialité. Seuls les montants de transfert et
les soldes de tokens sont privés. Les adresses des comptes de tokens restent
publiques.
...
au programme Token Extension qui vous permettent de transférer des tokens entre
des comptes sans révéler le montant du transfert.
...
1. Créer un compte de mint avec l'extension de transfert confidentiel.
2. Créer des comptes de tokens avec l'extension de transfert confidentiel pour
   l'expéditeur et le destinataire.
3. Minter des tokens vers le compte de l'expéditeur.
...
5. **Appliquer** le solde en attente de l'expéditeur au **solde confidentiel
   disponible**.
6. **Transférer** confidentiellement des tokens du compte de tokens de
   l'expéditeur vers le compte de tokens du destinataire.
...
<Cards>
  ...
  >
    Comment créer un mint account avec l'extension de transfert confidentiel
  </Card>
  <Card
    title="Créer un token account"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Comment configurer un token account avec l'extension de transfert
    confidentiel
  </Card>
  <Card
  ...
  >
    Comment déposer des tokens vers le solde confidentiel en attente
  </Card>
  <Card
  ...
  >
    Comment transférer des tokens de manière confidentielle entre token accounts
  </Card>
</Cards>
...
Pour exécuter l'exemple, démarrez un validateur local avec le Token Extension
Program cloné depuis le mainnet en utilisant la commande suivante. Vous devez
avoir la CLI Solana installée pour utiliser le validateur local.
...
<Callout type="info">
  ...
  sur le validateur local par défaut. Vous devez cloner le Token Extension
  Program du mainnet pour exécuter l'exemple de code.
</Callout>
...
| _rs`ApproveAccount`_                | Approuve un token account pour les transferts confidentiels si le mint nécessite une approbation pour les nouveaux token accounts.                                                     |
...
| _rs`Transfer`_                      | Transfère des tokens entre token accounts de manière confidentielle.                                                                                                                   |
...
| _rs`TransferWithFee`_               | Transfère des tokens entre token accounts de manière confidentielle avec des frais.                                                                                                    |
| _rs`ConfigureAccountWithRegistry`_  | Méthode alternative pour configurer les token accounts pour les transferts confidentiels en utilisant un compte _rs`ElGamalRegistry`_ au lieu d'une preuve _rs`VerifyPubkeyValidity`_. |
