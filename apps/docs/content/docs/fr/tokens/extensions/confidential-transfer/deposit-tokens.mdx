...
solde public de tokens doit être converti en solde confidentiel. Cette
conversion s'effectue en deux étapes :

1. **Solde en attente confidentiel** : Dans un premier temps, les tokens sont «
   déposés » depuis le solde public vers un solde confidentiel « en attente ».
2. **Solde disponible confidentiel** : Le solde en attente est ensuite «
   appliqué » au solde disponible, rendant les tokens disponibles pour les
   transferts confidentiels.

Cette section explique la première étape : le dépôt du solde public de tokens
vers le solde en attente confidentiel.

Le diagramme suivant illustre les étapes du dépôt de tokens depuis le solde
public vers le solde en attente confidentiel :
...
Pour convertir un solde public en solde en attente confidentiel, invoquez
l'instruction
...
La crate `spl_token_client` fournit une méthode `confidential_transfer_deposit`
qui construit et envoie une transaction avec l'instruction `Deposit`, comme
illustré dans l'exemple ci-dessous.

## Exemple de code

L'exemple suivant montre comment déposer un solde public de tokens vers un solde
en attente confidentiel.

Les Confidential Balances dépendent du programme ZK ElGamal Proof, qui est activé
sur mainnet et devnet. Un `solana-test-validator` standard ne l'active pas, mais un
validator local avec fork du mainnet tel que [Surfpool](https://surfpool.run) le fait.
Exécutez l'exemple sur l'un d'eux (le code utilise devnet) avec un payeur approvisionné,
