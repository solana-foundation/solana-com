...
## Comment transférer confidentiellement des jetons d'un compte de jetons à un autre

Pour transférer confidentiellement des jetons d'un compte de jetons à un autre,
l'expéditeur et le destinaire doivent tous deux avoir des comptes de jetons
configurés avec l'état _rs`ConfidentialTransferAccount`_ et approuvés pour les
transferts confidentiels. Le compte de jetons de l'expéditeur doit également
disposer d'un solde confidentiel disponible à transférer.

Pour transférer des jetons de manière confidentielle :
...
1. Créez
   ...
   **Preuve d'intervalle
   ...
   : Vérifie que le nouveau solde disponible et le montant du transfert (divisé
   en bits bas/haut) sont tous non négatifs et dans un intervalle spécifié.
...
4. Fermez les comptes d'état de contexte de preuve pour récupérer les SOL
   utilisés pour les créer.

Le diagramme suivant montre les étapes impliquées dans le transfert de jetons du
token account d'un expéditeur vers le token account d'un destinataire.
...
Pour transférer confidentiellement des jetons d'un token account à un autre,
vous devez :

- Générer une preuve d'égalité, une preuve de validité de texte chiffré et une
...
- La méthode `confidential_transfer_create_context_state_account` qui crée un
  compte de preuve.
- La méthode `confidential_transfer_transfer` qui invoque l'instruction
  `Transfer`.
