...
l'expéditeur et le destinataire doivent tous deux avoir des token accounts
configurés avec l'état _rs`ConfidentialTransferAccount`_ et approuvés pour les
transferts confidentiels. Le token account de l'expéditeur doit également
...
1. Créer
   ...
   **Preuve d'égalité
   ...
   Vérifie que le nouveau chiffrement du solde disponible après le transfert
   correspond à son
...
4. Fermer les comptes d'état de contexte de preuve pour récupérer le SOL utilisé
   pour les créer.
