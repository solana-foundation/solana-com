...
description:
  Apprenez comment appliquer le solde en attente pour rendre les fonds
  disponibles.
---
...
solde de tokens public doit être converti en solde confidentiel. Cette
conversion se déroule en deux étapes :

1. **Solde confidentiel en attente** : initialement, les tokens sont « déposés »
   du solde public vers un solde confidentiel « en attente ».
...
Cette section explique la deuxième étape : appliquer le solde en attente au
solde disponible.

Lorsque les tokens sont « déposés » depuis le solde public ou lorsque les tokens
sont transférés de manière confidentielle d'un token account à un autre, les
tokens sont initialement ajoutés au solde confidentiel en attente. Avant que les
tokens puissent être utilisés pour des transferts confidentiels, le solde en
attente doit être « appliqué » au solde disponible.
...
L'exemple suivant montre comment appliquer le solde confidentiel en attente au
...
suivante. Vous devez avoir la CLI Solana installée pour démarrer le validateur
local.
...
<Callout type="info">
  ...
  de jetons du réseau principal pour exécuter l'exemple de code.
</Callout>
