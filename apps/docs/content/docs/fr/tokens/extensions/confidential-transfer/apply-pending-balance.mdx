...
description: Apprenez comment appliquer le solde en attente pour rendre les fonds disponibles.
---
...
1. **Solde confidentiel en attente** : Dans un premier temps, les tokens sont «
...
Lorsque des tokens sont « déposés » depuis le solde public ou lorsqu'ils sont
transférés de manière confidentielle d'un token account à un autre, les tokens
sont initalement ajoutés au solde confidentiel en attente. Avant que les tokens
...
Le diagramme suivant illustre les étapes impliquées dans l'application du solde
en attente au solde disponible :
...
Confidential Balances dépend du programme ZK ElGamal Proof, qui est activé
...
Exécutez l'exemple sur l'un d'eux (le code utilise le devnet) avec un payeur financé,
