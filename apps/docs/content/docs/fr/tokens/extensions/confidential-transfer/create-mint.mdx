...
description:
  Apprenez à créer un mint de token avec les Soldes Confidentiels activés.
---

## Comment créer un mint avec les Soldes Confidentiels

Les Soldes Confidentiels permettent des transferts de tokens privés en ajoutant un état supplémentaire au
mint account. Cette section explique comment créer un mint de token avec
l'extension requise activée.

Le diagramme suivant illustre les étapes de création d'un mint avec
l'extension Soldes Confidentiels :
...
### État du Mint avec Soldes Confidentiels
...
- **authority** : Le compte qui dispose de l'autorisation de modifier les paramètres des Soldes Confidentiels
  pour le mint et d'approuver les nouveaux comptes confidentiels si l'approbation automatique
  est désactivée.

- **auto_approve_new_accounts** : Lorsque défini sur true, les utilisateurs peuvent créer des token
  accounts avec les Soldes Confidentiels activés par défaut. Lorsque défini sur false, l'authority doit approuver chaque nouveau token account avant qu'il puisse être utilisé pour
...
La création d'un mint avec les Soldes Confidentiels activés nécessite trois instructions
dans une seule transaction :

1. **Créer le Mint Account** : Invoquer l'instruction _rs`CreateAccount`_ du
   System Program pour créer le mint account.

2. **Initialiser les Soldes Confidentiels** : Invoquez l'instruction
   ...
   du Token Extension Program pour configurer l'état _rs`ConfidentialTransferMint`_ du
   mint.
...
Le code suivant montre comment créer un mint avec les Soldes Confidentiels
activés.
...
Exécutez l'exemple sur l'un d'eux (le code utilise le devnet) avec un payer approvisionné,
et remplacez les adresses de mint et de compte fictives par les vôtres.
