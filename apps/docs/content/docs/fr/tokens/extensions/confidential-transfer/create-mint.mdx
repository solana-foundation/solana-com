...
ajoutant un état supplémentaire au compte mint. Cette section explique comment
...
### État Confidential Transfer du mint
...
- **auto_approve_new_accounts** : lorsqu'il est défini sur true, les
  utilisateurs peuvent créer des token accounts avec les transferts
  confidentiels activés par défaut. Lorsqu'il est false, l'authority doit
  approuver chaque nouveau token account avant qu'il puisse être utilisé pour
  des transferts confidentiels.

- **auditor_elgamal_pubkey** : un auditeur optionnel qui peut déchiffrer les
  montants de transfert dans les transactions confidentielles, fournissant un
...
1. **Créer le compte mint** : invoquez l'instruction _rs`CreateAccount`_ du
   System Program pour créer le compte mint.

2. **Initialiser l'extension Confidential Transfer** : invoquez l'instruction
   ...
   du Token Extension Program pour configurer l'état
   _rs`ConfidentialTransferMint`_ pour le mint.

3. **Initialiser le mint** : invoquez l'instruction
   _rs`Instruction::InitializeMint`_ du Token Extension Program pour initialiser
   l'état standard du mint.

Bien que vous puissiez écrire ces instructions manuellement, la crate
...
<Callout type="info">
  ...
  le validateur local par défaut. Vous devez cloner le Token Extension Program
  du mainnet pour exécuter l'exemple de code.
</Callout>
