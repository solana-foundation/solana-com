...
description: Apprenez à retirer des tokens de l'état confidentiel.
---
...
Pour retirer des tokens du solde confidentiel disponible vers le solde public :
...
2. Pour chaque preuve :
   ...
   - Stockez les métadonnées spécifiques à la preuve dans un compte d'état de
     contexte de preuve pour les utiliser dans d'autres instructions.
...
4. Fermez les comptes de preuve pour récupérer les SOL utilisés pour les créer.

Le diagramme suivant montre les étapes impliquées dans le retrait de tokens du
...
- Invoquer l'instruction
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  en fournissant les deux comptes de preuve.
- Fermer les deux comptes de preuve pour récupérer le loyer.

La crate `spl_token_client` fournit les méthodes suivantes :

- La méthode `confidential_transfer_create_context_state_account` qui crée un
  compte de preuve.
- La méthode `confidential_transfer_withdraw` qui invoque l'instruction
  `Withdraw`.
- La méthode `confidential_transfer_close_context_state_account` qui ferme un
  compte de preuve.

### Exemple de code

L'exemple suivant montre comment retirer des jetons du solde confidentiel
...
avoir la CLI Solana installée pour démarrer le validateur local.
