...
1. Créez
   ...
   **Preuve d'égalité
   ...
   associé, garantissant que le nouveau solde disponible du compte est
...
2. Pour chaque preuve :
   - Invoquez le programme de preuve ZK ElGamal pour vérifier les données de
     preuve.
   - Stockez les métadonnées spécifiques à la preuve dans un compte « état de
...
Le diagramme suivant illustre les étapes du retrait de tokens depuis le solde
disponible confidentiel vers le solde public :
...
- Invoquer le programme de preuve Zk ElGamal pour vérifier les preuves et
  initialiser les comptes « état de contexte »
...
compte d'état de contexte via le programme ZK ElGamal Proof, référence les deux
comptes dans l'instruction de retrait et les ferme ensuite. L'exemple TypeScript
utilise l'assistant `getConfidentialWithdrawInstructionPlan` de
`@solana-program/token-2022/confidential`, qui assemble les mêmes comptes de
preuve, le retrait et les fermetures sous forme de plan d'instructions
multi-transactions.

## Exemple de code

L'exemple suivant retire des tokens du solde disponible confidentiel
pour les reverser dans le solde public. Le compte doit déjà être configuré pour
les soldes confidentiels et disposer d'un solde confidentiel disponible.

Les soldes confidentiels dépendent du programme ZK ElGamal Proof, qui est activé
sur le mainnet et le devnet. Un `solana-test-validator` standard ne l'active pas, mais un
validator local avec fork du mainnet tel que [Surfpool](https://surfpool.run) le fait.
Exécutez l'exemple sur l'un d'eux (le code utilise le devnet) avec un payeur approvisionné,
et remplacez les espaces réservés par votre mint et votre token account.
