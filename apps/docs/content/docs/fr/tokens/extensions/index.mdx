...
Chaque extension ajoute un état spécifique qui est généralement initialisé lors de la création d'un mint ou d'un token account. Lors de l'initialisation de l'un ou l'autre compte, vous pouvez activer des extensions spécifiques simultanément pour différentes fonctionnalités. La plupart des extensions ne peuvent pas être ajoutées après l'initialisation d'un compte. C'est un point important à prendre en compte lors de la conception de votre token, car vous devrez planifier à l'avance les fonctionnalités que vous souhaitez que votre token prenne en charge. Les sections ci-dessous décrivent chaque extension et comment l'intégrer.
...
qui répertorie toutes les extensions disponibles que vous pouvez ajouter à un
mint account ou token account. Chaque variante représente une extension
différente avec une fonctionnalité unique.
...
supplémentaire à un mint account ou token account. Tout l'état spécifique aux
extensions est stocké dans le champ
...
qui suit le type de données de compte de base. Vous devez ensuite désérialiser
le `tlv_data` (contenant l'état de l'extension) en fonction des types
d'extensions spécifiques activés pour ce compte.
