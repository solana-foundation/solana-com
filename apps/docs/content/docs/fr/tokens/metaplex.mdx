...
Le Programme de Métadonnées de Jeton crée une adresse dérivée du programme (PDA)
pour chaque création de jeton. Ce compte de métadonnées stocke des informations
en chaîne comme le nom et le symbole du jeton, ainsi qu'un URI pointant vers des
métadonnées JSON hors chaîne (images, descriptions, etc.).
