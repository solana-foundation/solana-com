...
du programme spécifié dans le type de base
...
La création de tokens et de comptes nécessite des SOL pour les dépôts de rent
des comptes et les frais de transaction. Pour les nouveaux utilisateurs de
Solana Playground, créez un portefeuille Playground et exécutez la commande
_sh`solana airdrop`_ dans le terminal Playground. Vous pouvez également obtenir
des SOL devnet depuis le [faucet web](https://faucet.solana.com/) public.
...
Les adresses de compte affichées dans votre sortie de terminal diffèrent des
exemples ci-dessous. Utilisez les adresses de votre terminal Playground lorsque
vous suivez. Par exemple, la commande _sh`create-token`_ génère une adresse de
mint account avec votre portefeuille Playground comme autorité de mint.
...
La commande _sh`supply`_ pour un nouveau token renvoie `0` :
...
de mint doit signer la transaction. L'instruction mint des tokens vers un token
account et augmente l'offre totale sur le mint account. Voici un exemple
Javascript sur
...
L'instruction _rs`Transfer`_ du Token Program gère les transferts de tokens. Le
propriétaire du token account de l'expéditeur doit signer la transaction.
L'instruction déplace des tokens entre les token accounts. Voici un exemple
Javascript sur
