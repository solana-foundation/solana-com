...
- Les [Token Programs](#token-programs) contiennent toute la logique
...
[comptes de données](/docs/core/accounts/account-types#data-accounts)
appartenant à un Token Program.
...
[Account](/docs/core/accounts/account-structure#account-fields), qui est le
Token Program pour tous les token accounts.
...
Il existe deux façons d'ajouter des métadonnées (nom, symbole, image) aux tokens
sur Solana :

1. **[Metaplex Token Metadata](/docs/tokens/metaplex)** - Crée un compte de
   métadonnées séparé lié à votre mint. Fonctionne avec le Token Program
   d'origine et Token-2022 et est largement pris en charge par les portefeuilles
   et les places de marché.

2. **[Token Extensions Metadata](/docs/tokens/extensions/metadata)** - Stocke
   les métadonnées directement sur le mint account en utilisant Token-2022. Une
   approche plus récente avec prise en charge intégrée. L'extension de
   métadonnées n'est disponible que pour les mint accounts créés avec
   Token-2022. Pour ajouter des métadonnées aux mint accounts créés avec le
   Token Program d'origine, utilisez le Metaplex Token Metadata Program.

#### Exemple avec SPL Token CLI

Le Token Extensions Program vous permet de stocker les métadonnées directement
sur le Mint Account. Pour créer un token avec l'extension de métadonnées :
...
L'URI du token pointe vers les métadonnées hors chaîne. Consultez un exemple de
format JSON
...
[section Token Extensions](/docs/tokens/extensions/metadata). Pour plus de
détails sur Token Extensions, voir la
[section Token Extensions](/docs/tokens/extensions) et la
