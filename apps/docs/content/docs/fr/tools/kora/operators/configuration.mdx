...
> _Remarque : Lorsque `fallback_if_unavailable` est défini sur true, le système autorise les transactions
> à se poursuivre si Redis est temporairement indisponible, évitant ainsi toute interruption de service.
> Définir `max_transactions` sur 0 permettra un nombre illimité de transactions._
...
| `get_payer_signer`          | Retourner le prochain payeur de frais et l'adresse de paiement        | ✅       | booléen |
...
| ---------------------------- | ------------------------------------------------ |
| `confidential_transfer_mint` | Configuration des soldes confidentiels pour le mint |
...
| Nom de l'extension                  | Description                                 |
| ------------------------------- | ------------------------------------------- |
| `confidential_transfer_account` | État des soldes confidentiels pour le compte |
