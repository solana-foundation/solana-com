---
title: Intégration de code QR
description: Générez et personnalisez des codes QR pour les paiements par portefeuille mobile
---

Les codes QR sont le moyen le plus courant de faire le lien entre les
applications web et les portefeuilles mobiles. Solana Pay inclut une fonction
`createQR` intégrée alimentée par `@solana/qr-code-styling` — aucun package de
code QR supplémentaire n'est nécessaire.
...
La fonction `createQR` accepte des paramètres facultatifs pour la taille, la
couleur de fond et la couleur de premier plan :
...
Utilisez `createQROptions` pour obtenir l'objet de configuration brut et avoir
plus de contrôle :
...
Sur les appareils mobiles, envisagez de fournir un lien direct en complément du
code QR :
