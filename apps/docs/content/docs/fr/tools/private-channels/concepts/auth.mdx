...
Private Channels inclut un service d'authentification optionnel qui restreint l'accès à la passerelle via l'authentification JWT et le contrôle d'accès basé sur les rôles (RBAC). Lorsque l'authentification est désactivée, la passerelle accepte toutes les connexions. Lorsqu'elle est activée, les clients doivent présenter un JWT valide dans chaque requête.
...
> démarré par défaut**. Pour l'inclure, passez `--profile auth` à votre
> commande `docker compose`, en ajoutant `--env-file .env` afin que les secrets tels que
> `JWT_SECRET` et `POSTGRES_PASSWORD` soient bien résolus (Compose désactive le
...
- Nom d'utilisateur : 5 à 32 caractères, alphanumériques avec `_` et `-`
- Mot de passe : 6 à 128 caractères
- Retourne l'utilisateur créé ; le mot de passe n'est jamais retourné

### `POST /auth/login`

S'authentifier et recevoir un JWT signé valable 24 heures.

```json
{ "username": "alice", "password": "hunter2" }
```

Retourne `{ "token": "<jwt>" }`. Un nom d'utilisateur ou un mot de passe incorrect retourne tous les deux
`401` afin d'éviter l'énumération des noms d'utilisateur.

### `POST /auth/challenge-wallet`

Demander un défi de signature pour prouver la propriété d'un portefeuille Solana. Nécessite un
JWT valide.

Retourne un message, un nonce et une date d'expiration. Le défi expire dans 10 minutes.
...
Le service reconstruit le message du défi, vérifie la signature Ed25519
et enregistre le portefeuille. Chaque nonce ne peut être consommé qu'une seule fois ; les rejeux sont
rejetés.
...
> `iss` et `aud` sont présents dans le payload JWT mais sont validés par la
...
Passer le token dans l'en-tête `Authorization` :
...
- L'accès est restreint aux portefeuilles vérifiés appartenant à l'utilisateur
...
Rôle élevé. Doit être attribué directement ; il n'existe aucun chemin en libre-service pour
passer du rôle `user` au rôle `operator`.
...
<Callout type="warning">
  Il s'agit d'une opération de base de données privilégiée. Restreignez l'accès à la base de données du service d'authentification en conséquence et auditez toute modification de rôle.
</Callout>
...
Cette commande insère directement un portefeuille vérifié dans la table `verified_wallets`,
en contournant le flux challenge/verify. Elle impose une contrainte d'unicité sur
`(user_id, pubkey)`. Elle n'accorde **pas** le rôle `operator` en elle-même ; utilisez
`set-role` ou la mise à jour SQL ci-dessus pour cela. Cette commande sert à rattacher un
portefeuille à un compte (par exemple, un compte de service) sans nécessiter le
flux interactif challenge/verify.

**Capacités :**

- Contourne tous les contrôles de propriété de portefeuille
...
| `getAccountInfo`              | Nœud de lecture  | 401    | restreint par propriété¹ | ✓          |
| `getTokenAccountBalance`      | Nœud de lecture  | 401    | restreint par propriété¹ | ✓          |
| `getSignaturesForAddress`     | Nœud de lecture  | 401    | restreint par propriété¹ | ✓          |
| `getBlock`                    | Nœud de lecture  | 401    | 403              | ✓          |
| `getTransaction`              | Nœud de lecture  | 401    | 403              | ✓          |
| `simulateTransaction`         | Nœud de lecture  | 401    | 403              | ✓          |

¹ **Restreint par propriété** : pour un token account SPL (dont le champ owner est `TokenkegQ...`
ou `TokenzQ...`, avec au moins 165 octets de données), la passerelle vérifie que le champ
`owner` ou `delegate` correspond à l'un des portefeuilles vérifiés de l'utilisateur authentifié. Pour tout autre type de compte (un portefeuille System Program, ou un PDA inconnu), elle vérifie plutôt si le pubkey interrogé est lui-même l'un des portefeuilles vérifiés de l'utilisateur, ces comptes ne disposant pas de champ owner/delegate à inspecter.
