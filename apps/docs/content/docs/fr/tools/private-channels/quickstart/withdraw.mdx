...
description:
  Retirez des tokens d'une instance de Private Channels vers votre portefeuille Solana
---
...
Aucune action supplémentaire n'est requise après l'appel à `WithdrawFunds`. Les services
opérateurs gèrent le règlement automatiquement :

1. `indexer-private-channel` interroge le canal toutes les secondes à la recherche d'événements de brûlage
   et enregistre une entrée de retrait en attente lorsque le vôtre est détecté
2. `operator-private-channel` interroge la base de données toutes les secondes pour les
   entrées en attente et soumet `ReleaseFunds` à l'Escrow Program sur Solana devnet
...
## Comment l'opérateur règle les fonds (Référence)
...
- `sibling_proofs` : 512 octets (16 hachages de nœuds frères de 32 octets pour la preuve d'arbre)

## Vérifier

Une fois l'appel de l'opérateur confirmé, vérifiez votre solde de tokens devnet :
...
<Cards>
  ...
  >
    Comprenez en détail les participants et le flux complet du dépôt au retrait.
  </Card>
  <Card
