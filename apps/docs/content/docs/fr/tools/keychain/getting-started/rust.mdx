...
| ------------ | ------------------------------- |
| `memory`     | Signature keypair locale (par défaut) |
...
| `utila`      | Conservation MPC Utila               |
| `fordefi`    | Garde MPC Fordefi             |
...
Pour la signature avec la garde MPC Fordefi. Les requêtes API sont authentifiées avec une signature de requête ECDSA P-256 analysée depuis `private_key_pem` ; pour conserver cette clé dans un KMS/HSM, implémentez `FordefiRequestSigner` et utilisez `Signer::from_fordefi_with_signer`.

Fordefi dispose de deux modes de signature. Définissez `chain` pour le **mode Solana natif**, où Fordefi peut remplacer le blockhash ou les frais et diffuse lui-même la transaction. Laissez `chain` à `None` pour le **mode boîte noire**, où Fordefi signe des octets bruts et vous diffusez la transaction vous-même.
