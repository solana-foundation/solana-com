---
title: TypeScript
description: Installer et utiliser @solana/keychain dans des applications TypeScript
---
...
`@solana/keychain-kit-plugin` installe un signataire de trousseau directement
sur un [client Kit](https://github.com/anza-xyz/kit), en le configurant comme
`payer` et `identity`, ce qui vous évite la configuration manuelle de `pipe`
décrite ci-dessus :
...
Le plugin accepte la même configuration avec balise backend que
`createKeychainSigner`, et seul le backend vers lequel il distribue est inclus
dans le bundle (les packages backend se chargent via `import()` dynamique).

Utilisez `keychainPayer` ou `keychainIdentity` pour définir un seul rôle, et
combinez des backends sur un même client :
...
Vous avez déjà un `SolanaSigner` ? Chaque signataire de trousseau est un Kit
`TransactionSigner` valide, vous pouvez donc en installer un existant avec
[`@solana/kit-plugin-signer`](https://github.com/anza-xyz/kit-plugins/tree/main/packages/kit-plugin-signer)
à la place (`pnpm add @solana/kit-plugin-signer`) :
...
Fordefi propose deux modes de signature, selon que vous passez `chain` ou non. Les requêtes API sont authentifiées avec une signature de requête ECDSA P-256 — fournissez la clé de signature via `privateKeyPem`, ou conservez-la dans un KMS/HSM en passant un `requestSigner`.

**Mode Solana natif** (recommandé pour une utilisation on-chain) : définissez `chain`, et Fordefi signe et diffuse la transaction lui-même. Étant donné que Fordefi peut remplacer le blockhash ou les frais avant la signature, le signataire est un `TransactionSendingSigner` Kit — utilisez-le avec `signAndSendTransactionMessageWithSigners`, et non avec le flux de signataire partiel présenté ci-dessus.
...
**Mode boîte noire** : omettez `chain`. Fordefi signe les octets bruts et ne diffuse pas la transaction ; vous assemblez et soumettez la transaction vous-même. Utilisez ce mode avec un coffre Fordefi en mode boîte noire.
