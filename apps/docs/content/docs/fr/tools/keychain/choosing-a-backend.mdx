...
description:
  Sélectionnez un backend de signature Keychain en fonction de vos besoins — garde des clés, infrastructure existante, contrôles d'approbation, et selon que vous signez pour votre propre application ou que vous provisionnez des portefeuilles pour vos utilisateurs finaux.
related:
...
Si les signatures doivent passer par une politique d'approbation, une limite de dépense ou un flux de conformité avant d'être produites — ou si vous avez besoin d'un dépositaire réglementé détenant les clés — utilisez un backend **MPC / garde institutionnelle** : Fireblocks, Dfns, Para, Utila ou Fordefi. Ces solutions divisent ou conservent la clé et co-signent selon votre politique.

Si vous avez seulement besoin d'une clé qui signe à la demande, continuez ci-dessous.
...
- **Cloud KMS / HSM** — un fournisseur cloud stocke la clé dans une
  ...
  KMS**.
- **MPC & garde institutionnelle** — la clé est divisée ou confiée à un
  prestataire, qui co-signe selon votre politique (approbations, limites).
...
| Fordefi         | Garde MPC / institutionnelle  | Garde institutionnelle avec contrôles de politique | Le mode natif diffuse automatiquement ; en mode boîte noire, c'est vous qui envoyez |
