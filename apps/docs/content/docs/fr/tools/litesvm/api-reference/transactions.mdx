---
title: Transactions
description: Méthodes de gestion des transactions
---
...
Exécuter une transaction et modifier l'état.
...
Retourne `SimulatedTransactionInfo` qui contient :

- `meta` : Métadonnées de transaction (journaux, unités de calcul, signature,
  etc.)
- `post_accounts` : États des comptes après simulation
...
Utiliser `simulate_transaction()` pour :

- Tester la validité d'une transaction sans modifier l'état
- Vérifier la consommation d'unités de calcul
- Consulter les journaux de transaction sans engagement
...
`send_transaction()` retourne `TransactionMetadata` directement, tandis que
`simulate_transaction()` retourne `SimulatedTransactionInfo` qui contient
...
Où `TransactionResult = Result<TransactionMetadata, FailedTransactionMetadata>`.

Interroger une transaction depuis l'historique. Nécessite que
`with_transaction_history()` soit défini.
...
Obtenir le blockhash actuel pour les transactions.
...
Forcer l'expiration du blockhash actuel.
...
### Vérifier les journaux de transaction
