---
title: Opérations sur les tokens
description:
  Travailler avec les opérations sur les tokens - émission, transferts et
  gestion des comptes
---

## Créer un Token Mint

Un mint est la base de tout token SPL. Il définit les propriétés du token et
contrôle qui peut émettre de nouveaux tokens.
...
Un token account contient le solde d'un spl-token spécifique pour un utilisateur
donné. Pour comprendre la différence entre celui-ci et un ATA, lisez
...
Les Associated Token Accounts (ATAs) sont des adresses déterministes permettant
de détenir des tokens pour un utilisateur spécifique. Pour comprendre la
différence entre celui-ci et un ATA, lisez
[cette section](/docs/tools/litesvm/additional-crates/testing-with-spl-tokens#types-of-token-accounts).
...
## Émettre des tokens dans un Token Account

Une fois que vous disposez d'un mint et de token accounts, vous pouvez émettre
des tokens vers ces comptes.
...
## Effectuer un transfert de tokens

Transférez des tokens entre des comptes à l'aide de l'instruction Transfer.
...
fractionnaires. Voici comment les gérer correctement :
