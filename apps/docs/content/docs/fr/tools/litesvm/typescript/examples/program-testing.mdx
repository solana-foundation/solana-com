---
title: Tests de programme
description: Charger et tester des programmes Solana personnalisés avec LiteSVM
---

Tester un programme Solana personnalisé consiste à charger son fichier `.so` compilé dans la
SVM, à configurer les comptes qu'il attend et à lui envoyer des instructions.
LiteSVM gère tout cela en interne — aucun validator requis. Nous recommandons
de générer les clients de programme avec le
[Codama JS Renderer](https://github.com/codama-idl/renderers-js) afin d'avoir
accès à un Kit Plugin compatible.
...
1. **Chargement du programme** : Utilisez `addProgramFromFile` pour charger les programmes compilés et
   importez myProgramPlugin() depuis votre client
2. **Configuration** : Activez les sysvars, les builtins et les précompilations selon les besoins
3. **Configuration des comptes** : Pré-remplissez les comptes de données avec `setAccount`
4. **Envoi** : Utilisez `client.myProgram.testInstruction({..}).sendTransaction()` —
   aucune construction manuelle de transaction
5. **Vérification** : Contrôlez l'état du compte après exécution
   (`client.myProgram.myAccount.fetch`)

Une fois ce squelette fonctionnel, vous pouvez construire une suite de tests complète en
ajoutant davantage d'instructions et d'assertions.
