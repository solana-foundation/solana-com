...
description: Configurer des comptes de test et l'état avec LiteSVM
---

# Exemple de configuration de compte

Illustre la configuration directe de l'état des comptes de test sans
transactions.
...
Configurer un account de données appartenant à un program account :
...
<Callout type="info">
  L'utilisation de `setAccount` est plus rapide que l'exécution de transactions
  pour la configuration en masse de comptes.
</Callout>

## Points clés

1. **État direct** : `setAccount` définit l'état du compte sans transactions
2. **Exemption de loyer** : Utilisez `minimumBalanceForRentExemption` pour
   calculer les lamport requis
3. **Structure du compte** : Fournissez tous les champs requis : address, data,
   executable, lamports, programAddress, space
4. **Vérification** : Utilisez `getAccount` ou `client.rpc.getAccountInfo` pour
   vérifier
