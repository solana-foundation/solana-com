...
description: Testez la logique dépendante du temps avec la manipulation d'horloge LiteSVM
---
...
Vous pouvez lire le calendrier d'epoch pour comprendre la relation slot/epoch sur le
cluster :
...
1. **Activer les Sysvars** : Appelez `withSysvars()` avant d'utiliser les méthodes d'horloge
2. **Avance Temporelle** : Utilisez `warpToSlot(slot)` pour faire progresser le slot
3. **Contrôle Total** : Utilisez `setClock` lorsque vous devez définir `epoch`,
   `unixTimestamp`, ou d'autres champs de l'horloge
4. **Lire l'Horloge** : Utilisez `getClock()` pour lire le slot, l'epoch et l'horodatage actuels
5. **Modèle de Test** : Testez avant et après les limites temporelles
