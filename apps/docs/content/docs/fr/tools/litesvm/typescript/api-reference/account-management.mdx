...
description:
  Méthodes pour gérer les comptes, les soldes et le rent en TypeScript
---
...
Calculer le nombre minimum de lamport requis pour l'exemption de rent.
...
Pour les tests, vous pouvez configurer plusieurs comptes efficacement :
...
<Callout type="info">
  Utiliser `setAccount` est plus rapide que `airdrop` pour la configuration de
  comptes en masse, car cela n'exécute pas de transactions.
</Callout>
