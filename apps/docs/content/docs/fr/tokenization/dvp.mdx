...
Les transactions Solana ont une limite de taille (~1 232 octets). Chaque instruction de transfert
ajoute ~200 octets. Pour les règlements multi-parties de grande envergure, envisagez d'utiliser
des [Address Lookup Tables](/developers/cookbook/transactions/lookup-tables) pour
compresser les adresses de comptes et intégrer davantage de transferts par transaction.
