...
Το bump seed είναι ένα μονό byte (u8) που προσαρτάται στα προαιρετικά seeds κατά
την παραγωγή. Η
[`find_program_address`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L800)
αναζητά από το 255 προς τα κάτω μέχρι το 1, καλώντας την
_rs`create_program_address`_ με κάθε τιμή μέχρι το αποτέλεσμα να βγει εκτός της
καμπύλης Ed25519. Η πρώτη τιμή που πετυχαίνει είναι το **κανονικό bump**.
...
Η παραγωγή ενός PDA υπολογίζει μόνο μια διεύθυνση. Δεν δημιουργεί έναν onchain
λογαριασμό στη συγκεκριμένη διεύθυνση. Ο λογαριασμός πρέπει να δημιουργηθεί ρητά
μέσω μιας ξεχωριστής εντολής (συνήθως _rs`create_account`_ μέσω CPI).
...
### Επανάληψη Όλων των Πιθανών Bumps
...
<Callout type="warn">
  Αυτό διαφέρει από την τρέχουσα κανονική συμπεριφορά αναζήτησης που
  τεκμηριώνεται για το `sol_try_find_program_address`, το οποίο επαναλαμβάνει
  από το 255 προς τα κάτω μέχρι το 1.
</Callout>
...
αποτυγχάνει. Το πρώτο έγκυρο bump είναι το 254, καθιστώντας το το κανονικό bump.
