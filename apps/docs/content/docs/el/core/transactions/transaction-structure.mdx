...
Μια μεταφορά System Program προσθέτει lamports σε **οποιονδήποτε** λογαριασμό.
Δεν υπάρχει έλεγχος σε επίπεδο πρωτοκόλλου που να διασφαλίζει ότι ο παραλήπτης
μπορεί να μετακινήσει το SOL πίσω. Τα lamports μπορούν να μετακινηθούν μόνο από
το πρόγραμμα που κατέχει τον λογαριασμό, επομένως η αποστολή SOL σε ένα
[token mint](/docs/tokens/basics/create-mint), ένα πρόγραμμα, ή ένα
[PDA](/docs/core/pda) που δεν ελέγχετε **εγκυμονεί κίνδυνο μόνιμης απώλειας
...
μόνο από τον κάτοχο αυτού του λογαριασμού, ποτέ από τον αποστολέα.

Οι μεταφορές SPL **token** είναι μερικώς αυτοπροστατευόμενες: το Token Program
απορρίπτει μια μεταφορά της οποίας οι λογαριασμοί δεν αντιστοιχούν στο
αναμενόμενο mint. Οι μεταφορές εγγενούς **SOL** δεν διαθέτουν τέτοια προστασία,
επομένως ο αποστολέας πρέπει να επαληθεύσει τον παραλήπτη πριν υπογράψει.
Ανατρέξτε στο
...
Η μορφή διαφέρει ανάλογα με το SDK,
...
Επειδή μια μεταφορά SOL επιτυγχάνει σε οποιονδήποτε λογαριασμό, ελέγξτε τον
παραλήπτη πριν υπογράψετε. Ανακτήστε τον λογαριασμό και στείλτε μόνο σε ένα
πορτοφόλι System Program (ή σε μια αναξιοποίητη διεύθυνση
[on-curve](/docs/payments/send-payments/verify-address#on-curve-addresses))·
απορρίψτε mints, token accounts, προγράμματα και PDAs που δεν ελέγχετε.
...
<Callout>
  ...
  ταξινόμηση που διαχειρίζεται επίσης αποστολές SPL token (token accounts, ATAs,
  Token-2022), δείτε [Επαλήθευση
  Διεύθυνσης](/docs/payments/send-payments/verify-address).
</Callout>
