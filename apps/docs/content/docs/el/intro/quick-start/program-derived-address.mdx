...
Το χαρακτηριστικό _rs`#[account]`_ σε ένα πρόγραμμα Anchor σχολιάζει δομές που
...
παραγωγή μιας διεύθυνσης που προέρχεται από το πρόγραμμα (PDA). Η αποθήκευση
αυτής της τιμής εξοικονομεί υπολογιστικούς πόρους εξαλείφοντας την ανάγκη
...
- `bump` - _rs`u8`_ που αποθηκεύει το
  ...
  που προέρχεται από το πρόγραμμα (PDA). Η αποθήκευση αυτής της τιμής
  εξοικονομεί υπολογιστικούς πόρους εξαλείφοντας την ανάγκη επανυπολογισμού της
  σε μεταγενέστερες εντολές.
...
Το χαρακτηριστικό _rs`#[derive(Accounts)]`_ σε ένα πρόγραμμα Anchor επισημαίνει
...
2. Προαιρετικοί περιορισμοί (όπως _rs`#[account(mut)]`_ ή
   _rs`#[account(init)]`_) που ορίζουν επιπλέον απαιτήσεις
...
νέου λογαριασμού μηνύματος. Δέχεται δύο παραμέτρους:
...
Ο περιορισμός _rs`realloc`_ επαναδιανέμει τα δεδομένα του λογαριασμού.
...
3. _rs`system_program: Program<'info, System>`_
   - Απαιτείται για πιθανή επαναδιανομή του χώρου του λογαριασμού
...
λογαριασμού μηνυμάτων. Δέχεται δύο παραμέτρους:
...
Πρώτα, το αρχείο δοκιμής στέλνει μια συναλλαγή που επικαλείται την εντολή
`update`, περνώντας το "Hello, Solana!" ως νέο μήνυμα.
...
(`messagePda`) με το `fetchNullable`. Αυτή η μέθοδος επιστρέφει null όταν ο
