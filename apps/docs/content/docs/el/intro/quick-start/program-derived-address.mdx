...
description:
  Μάθετε πώς να δημιουργήσετε ένα πρόγραμμα CRUD (Create, Read, Update, Delete)
  στο Solana χρησιμοποιώντας Program Derived Addresses (PDAs) και το πλαίσιο
  Anchor. Αυτός ο οδηγός βήμα προς βήμα δείχνει πώς να δημιουργείτε, να
  ενημερώνετε και να διαγράφετε λογαριασμούς μηνυμάτων onchain χρησιμοποιώντας
  PDAs, να υλοποιείτε επικύρωση λογαριασμών και να γράφετε δοκιμές. Ιδανικός για
  προγραμματιστές που θέλουν να κατανοήσουν πώς να χρησιμοποιούν τα PDAs σε
  προγράμματα Solana.
...
που θα προσθέσετε στα ακόλουθα βήματα.
...
ελέγξετε ότι το αρχικό πρόγραμμα κάνει build επιτυχώς.
...
Στο `lib.rs`, ενημερώστε τη δομή _rs`MessageAccount`_ με τα ακόλουθα:
...
data του λογαριασμού).

### !user

Το πεδίο _rs`user`_ περιέχει ένα _rs`Pubkey`_ που αναγνωρίζει τον χρήστη που
...
παραγωγή μιας διεύθυνσης που προέρχεται από πρόγραμμα (PDA). Η αποθήκευση αυτής
της τιμής εξοικονομεί υπολογιστικούς πόρους αποφεύγοντας την ανάγκη
επανυπολογισμού της σε μεταγενέστερες εντολές.
...
- `user` - _rs`Pubkey`_ που αναγνωρίζει τον χρήστη που δημιούργησε τον
...
- `bump` - _rs`u8`_ που αποθηκεύει το
  ...
  που προέρχεται από πρόγραμμα (PDA). Η αποθήκευση αυτής της τιμής εξοικονομεί
  υπολογιστικούς πόρους αποφεύγοντας την ανάγκη επανυπολογισμού της σε
...
δείχνει τη διαδικασία δημιουργίας και ανάγνωσης δεδομένων λογαριασμού.

</Accordion>
</Accordions>

Κατασκευάστε ξανά το πρόγραμμα εκτελώντας _shell`build`_ στο τερματικό.
...
Τώρα, προσθέστε την εντολή _rs`create`_ που δημιουργεί και αρχικοποιεί το
_rs`MessageAccount`_.

Ξεκινήστε ορίζοντας τους λογαριασμούς που απαιτούνται για την εντολή
...
Ο περιορισμός _rs`space`_ καθορίζει τον αριθμό των bytes που θα εκχωρηθούν για
το πεδίο δεδομένων του νέου λογαριασμού.
...
δομές που ορίζουν τους λογαριασμούς που απαιτούνται από μια εντολή.
...
2. Προαιρετικοί περιορισμοί (όπως _rs`#[account(mut)]`_ ή
   _rs`#[account(init)]`_) που ορίζουν πρόσθετες απαιτήσεις
...
Σε αυτό το παράδειγμα, η δομή _rs`Create`_ ορίζει τους λογαριασμούς που
...
1. _rs`user: Signer<'info>`_
   ...
   - Χρειάζεται κατάσταση μεταβλητού (_rs`#[account(mut)]`_) καθώς πληρώνει για
     τον νέο λογαριασμό
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - Το `payer = user` προσδιορίζει ποιος πληρώνει για τη δημιουργία του νέου
     λογαριασμού
   - Το `space` διαθέτει τα απαιτούμενα bytes για το πεδίο δεδομένων του
     λογαριασμού
...
Οι περιορισμοί `seeds` και `bump` μαζί ορίζουν τη διεύθυνση ενός λογαριασμού ως
Program Derived Address (PDA).
...
- _rs`b"message"`_ - Ένα σταθερό string ως το πρώτο seed.
...
Ο υπολογισμός `space` _rs`(8 + 32 + 4 + message.len() + 1)`_ δεσμεύει χώρο για
τον τύπο δεδομένων `MessageAccount`:
...
Ένας τύπος _rs`String`_ χρειάζεται 4 bytes για να αποθηκεύσει το μήκος του
string, και το υπόλοιπο μήκος περιέχει τα πραγματικά δεδομένα.

</Accordion>
</Accordions>

Στη συνέχεια, προσθέστε τη λογική επιχειρηματικών κανόνων για την εντολή
_rs`create`_ ενημερώνοντας τη συνάρτηση `create` με τα ακόλουθα:
...
2. Αρχικοποίηση δεδομένων λογαριασμού:
   - Έχει πρόσβαση στο `message_account` από το context.
...
Όπως στο προηγούμενο βήμα, πρώτα καθορίστε τους λογαριασμούς που απαιτούνται από
την εντολή `update`.
...
Ο περιορισμός _rs`realloc`_ επαναδιαθέτει τα δεδομένα του λογαριασμού.
...
Η δομή _rs`Update`_ ορίζει τους λογαριασμούς που απαιτούνται για την εντολή
_rs`update`_.

1. _rs`user: Signer<'info>`_
   - Αντιπροσωπεύει τον χρήστη που ενημερώνει τον λογαριασμό μηνύματος
   - Επισημαίνεται ως μεταβλητός (_rs`#[account(mut)]`_) καθώς ενδέχεται να
     πληρώσει για περισσότερο χώρο για το `message_account` όταν χρειάζεται
   - Πρέπει να υπογράψει τη συναλλαγή

2. _rs`message_account: Account<'info, MessageAccount>`_
   - Ο υπάρχων λογαριασμός που αποθηκεύει το μήνυμα του χρήστη προς ενημέρωση
...
3. _rs`system_program: Program<'info, System>`_
   - Απαιτείται για πιθανή επαναδιάθεση του χώρου του λογαριασμού
...
bump seed που είναι αποθηκευμένο στο _rs`message_account`_, αντί να το
επανυπολογίζει το Anchor.
...
Σε αυτήν την περίπτωση, το πρόγραμμα χρησιμοποιεί το υπάρχον bump seed που είναι
...
Σε αυτήν την περίπτωση, ο λογαριασμός _rs`user`_ λαμβάνει τα lamports από τον
...
1. _rs`_ctx: Context<Delete>`_ - Παρέχει πρόσβαση στους λογαριασμούς που
   ...
   συνάρτηση δεν χρησιμοποιεί το Context στο σώμά της.
...
_rs`Delete`_ χειρίζεται το κλείσιμο του λογαριασμού.
...
Ολοκληρώσατε τώρα το βασικό πρόγραμμα CRUD. Αναπτύξτε το πρόγραμμα εκτελώντας
`deploy` στο τερματικό του Playground.
...
Το Solana Playground αφαιρεί κάποιο boilerplate setup όπου το
...
Εκτελέστε το αρχείο δοκιμής τρέχοντας _shell`test`_ στο τερματικό του Playground
...
Η μέθοδος _ts`create()`_ επικαλείται την εντολή `create`.
...
`create`, περνώντας "Hello, World!" ως μήνυμα.
...
Πρώτα, το αρχείο δοκιμής στέλνει μια συναλλαγή που επικαλείται την εντολή
`update`, περνώντας "Hello, Solana!" ως το νέο μήνυμα.
...
(`messagePda`) με `fetchNullable`. Αυτή η μέθοδος επιστρέφει null όταν ο
λογαριασμός δεν υπάρχει πλέον μετά το κλείσιμο.
...
<Callout type="info">
  ...
  `create` αποτυγχάνει επειδή το `messageAccount` υπάρχει ήδη ως λογαριασμός.
  Μόνο ένας λογαριασμός μπορεί να υπάρχει για ένα δεδομένο PDA.
</Callout>
