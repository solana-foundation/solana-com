...
description:
  Μεταφορά tokens μεταξύ token accounts για το ίδιο mint με οδηγίες μεταφοράς
  του Token Program.
...
## Τι Κάνει η Μεταφορά Tokens;

Η μεταφορά tokens μετακινεί tokens από έναν token account σε άλλον token account
για το ίδιο mint.

Οι μεταφορές δεν αλλάζουν την προσφορά του mint. Οι μεταφορές ενημερώνουν μόνο
τα υπόλοιπα μεταξύ token accounts.

## Πώς να Μεταφέρετε Tokens

Η μεταφορά tokens χρησιμοποιεί την οδηγία _rs`Transfer`_ ή _rs`TransferChecked`_
του Token Program.

Τα παρακάτω παραδείγματα χρησιμοποιούν την _rs`TransferChecked`_, η οποία
απαιτεί από τον καλούντα να παρέχει το mint και τα δεκαδικά ώστε η οδηγία να
επαληθεύει το αναμενόμενο mint και την ακρίβεια του token πριν μετακινήσει
tokens μεταξύ των λογαριασμών.

Στο Token Extensions Program, η _rs`Transfer`_ είναι καταργημένη υπέρ της
_rs`TransferChecked`_ ή της _rs`TransferCheckedWithFee`_.

Ο ιδιοκτήτης του source account ή ένας εγκεκριμένος αντιπρόσωπος υπογράφει τη
μεταφορά. Στο Token Extensions Program, ένας μόνιμος αντιπρόσωπος του mint
μπορεί επίσης να εξουσιοδοτήσει τη μεταφορά εάν το mint έχει ενεργοποιημένη την
επέκταση μόνιμου αντιπροσώπου.

### Αναφορά Πηγής

| Στοιχείο               | Περιγραφή                                                                                                                                                                                                              | Token Program                                                                                                | Token Extensions Program                                                                                           |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Account`_          | Η κατάσταση του token account αποθηκεύει τα υπόλοιπα που ενημερώνονται από μια μεταφορά.                                                                                                                               | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | Μια οδηγία μεταφοράς που μετακινεί tokens μεταξύ token accounts χωρίς να απαιτεί από τον καλούντα να παρέχει το mint ή τα δεκαδικά. Στο Token Extensions Program, χρησιμοποιήστε την _rs`TransferChecked`_ αντί αυτής. | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Μια οδηγία μεταφοράς που απαιτεί από τον καλούντα να παρέχει το mint και τα δεκαδικά και ελέγχει αυτές τις τιμές πριν μετακινήσει tokens μεταξύ token accounts του ίδιου mint.                                         | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | Κοινή λογική επεξεργασίας για μεταφορές tokens.                                                                                                                                                                        | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |
