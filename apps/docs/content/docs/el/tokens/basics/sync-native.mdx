...
description:
  Ενημερώστε το ποσό ενός λογαριασμού wrapped SOL token μετά τη μεταφορά SOL
  στον λογαριασμό.
...
## Τι Είναι το Wrapped SOL;

Το Wrapped SOL (WSOL) είναι SOL που κατέχεται σε έναν token account για το
native mint του Token Program. Το native mint είναι η διεύθυνση mint που
χρησιμοποιεί το Token Program για token accounts που περιτυλίγουν SOL. Το native
mint επιτρέπει τη χρήση του SOL μέσω των εντολών του Token Program.

| Πρόγραμμα                | Διεύθυνση native mint                          |
| ------------------------ | ---------------------------------------------- |
...
Ένας wrapped SOL token account αποθηκεύει SOL στο πεδίο `lamports` του
λογαριασμού, αλλά παρακολουθεί το SOL που κατέχεται στον wrapped SOL token
account ως υπόλοιπο token μέσω του πεδίου `amount` του token account. Εάν
μεταφέρετε SOL σε έναν wrapped SOL token account, το υπόλοιπο lamport αυξάνεται
αμέσως, αλλά το πεδίο `amount` δεν αλλάζει μέχρι να καλέσετε την εντολή
`SyncNative` του Token Program.

## Πώς να Συγχρονίσετε το Native SOL

Χρησιμοποιήστε την εντολή `SyncNative` του Token Program μετά τη μεταφορά SOL σε
έναν wrapped SOL token account. Η εντολή `SyncNative` ενημερώνει το πεδίο
`amount` του token account ώστε να ταιριάζει με τα lamports που κατέχονται στον
λογαριασμό πάνω από το αποθεματικό απαλλαγής ενοικίου του.

### Αναφορά Πηγής

| Στοιχείο              | Περιγραφή                                                                                                                                                                                                                                                      | Token Program                                                                                                | Token Extensions Program                                                                                           |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| `Account`             | Η κατάσταση του token account χρησιμοποιεί το `is_native` για να επισημάνει τον λογαριασμό ως WSOL και αποθηκεύει το native reserve, το ελάχιστο υπόλοιπο lamport απαλλαγής ενοικίου που πρέπει να διατηρεί ο λογαριασμός, και το παρακολουθούμενο ποσό token. | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| `SyncNative`          | Μια εντολή που ενημερώνει το πεδίο `amount` ενός wrapped SOL token account ώστε να ταιριάζει με τα lamports που κατέχονται στον λογαριασμό πάνω από το αποθεματικό απαλλαγής ενοικίου του.                                                                     | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| `process_sync_native` | Κοινή λογική επεξεργαστή για τον συγχρονισμό των υπολοίπων wrapped SOL.                                                                                                                                                                                        | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |
