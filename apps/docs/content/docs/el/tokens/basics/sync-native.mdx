...
description:
  Ενημερώστε το ποσό ενός λογαριασμού token περιτυλιγμένου SOL μετά τη μεταφορά
  SOL στον λογαριασμό.
...
## Τι Είναι το Περιτυλιγμένο SOL;

Το Περιτυλιγμένο SOL (WSOL) είναι SOL που κατέχεται σε έναν token account για το
εγγενές mint του Token Program. Το εγγενές mint είναι η διεύθυνση mint που
χρησιμοποιεί το Token Program για token accounts που περιτυλίγουν SOL. Το
εγγενές mint επιτρέπει στο SOL να χρησιμοποιείται μέσω των εντολών του Token
Program.

| Πρόγραμμα                | Διεύθυνση εγγενούς mint                        |
| ------------------------ | ---------------------------------------------- |
...
Ένας περιτυλιγμένος SOL token account αποθηκεύει SOL στο πεδίο `lamports` του
λογαριασμού, αλλά παρακολουθεί το SOL που κατέχεται στον περιτυλιγμένο SOL token
account ως υπόλοιπο token μέσω του πεδίου _rs`amount`_ του token account. Εάν
μεταφέρετε SOL σε έναν περιτυλιγμένο SOL token account, το υπόλοιπο lamport
αυξάνεται άμεσα, αλλά το πεδίο _rs`amount`_ του token δεν αλλάζει μέχρι να
καλέσετε την εντολή _rs`SyncNative`_ του Token Program.

## Πώς να Συγχρονίσετε το Εγγενές SOL

Χρησιμοποιήστε την εντολή _rs`SyncNative`_ του Token Program μετά τη μεταφορά
SOL σε έναν περιτυλιγμένο SOL token account. Η εντολή _rs`SyncNative`_
ενημερώνει το πεδίο _rs`amount`_ του token account ώστε να ταιριάζει με τα
lamports που κατέχονται στον λογαριασμό πάνω από το αποθεματικό απαλλαγής
ενοικίου.
...
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_             | Η κατάσταση του token account χρησιμοποιεί το _rs`is_native`_ για να επισημάνει τον λογαριασμό ως WSOL και αποθηκεύει το εγγενές αποθεματικό, το ελάχιστο υπόλοιπο lamport απαλλαγής ενοικίου που πρέπει να διατηρεί ο λογαριασμός, και το παρακολουθούμενο ποσό token. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`SyncNative`_          | Μια εντολή που ενημερώνει το πεδίο _rs`amount`_ ενός περιτυλιγμένου SOL token account ώστε να ταιριάζει με τα lamports που κατέχονται στον λογαριασμό πάνω από το αποθεματικό απαλλαγής ενοικίου.                                                                       | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| _rs`process_sync_native`_ | Κοινή λογική επεξεργαστή για τον συγχρονισμό των υπολοίπων περιτυλιγμένου SOL.                                                                                                                                                                                          | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |
