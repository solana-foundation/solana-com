...
description: Μάθετε πώς να δημιουργείτε νέες μονάδες ενός token.
---

## Πώς να δημιουργήσετε tokens

Η δημιουργία (minting) δημιουργεί νέες μονάδες ενός token χρησιμοποιώντας την
εντολή
...
- Μόνο η αρχή δημιουργίας (mint authority) μπορεί να δημιουργήσει νέα tokens
- Πρέπει να υπάρχει ένας token account προορισμού για να λάβει τα δημιουργημένα
  tokens
...
<Callout type="info">
  ...
  μοιράζονται παρόμοιες υλοποιήσεις για να επιτύχουν την ίδια λειτουργικότητα.
</Callout>
...
async fn main() -> Result<()> {
    // Δημιουργία σύνδεσης με τον τοπικό validator
    ...
    // Δημιουργία νέου keypair για τον πληρωτή των τελών
    let fee_payer = Keypair::new();

    // Airdrop 1 SOL στον πληρωτή των τελών
    ...
    // Δημιουργία keypair για χρήση ως διεύθυνση του mint
    let mint = Keypair::new();

    // Λήψη προεπιλεγμένου μεγέθους mint account (σε bytes), χωρίς ενεργοποιημένες επεκτάσεις
    let mint_space = Mint::LEN;
    let mint_rent = client
        .get_minimum_balance_for_rent_exemption(mint_space)
        .await?;

    // Εντολή για δημιουργία νέου λογαριασμού για το mint (token program)
    let create_account_instruction = create_account(
        &fee_payer.pubkey(), // πληρωτής
        &mint.pubkey(),      // νέος λογαριασμός (mint)
        mint_rent,           // lamports
        mint_space as u64,   // χώρος
        &token_program_id(), // program id
    );

    // Εντολή για αρχικοποίηση δεδομένων mint account
    let initialize_mint_instruction = initialize_mint(
        &token_program_id(),
        &mint.pubkey(),            // mint
        &fee_payer.pubkey(),       // αρχή δημιουργίας (mint authority)
        Some(&fee_payer.pubkey()), // αρχή πάγωσης (freeze authority)
        2,                         // δεκαδικά ψηφία
    )?

    // Υπολογισμός της διεύθυνσης του associated token account για τον fee_payer
    ...
    // Εντολή για δημιουργία associated token account
    ...
    // Δημιουργία συναλλαγής και προσθήκη εντολών
    ...
    // Αποστολή και επιβεβαίωση συναλλαγής
    client.send_and_confirm_transaction(&transaction).await?;

    // Ποσότητα tokens προς mint (100 tokens με 2 δεκαδικά ψηφία)
    let amount = 100;

    // Δημιουργία εντολής mint_to για mint tokens στο associated token account
    ...
    // Δημιουργία συναλλαγής για mint tokens
    let transaction = Transaction::new_signed_with_payer(
        &[mint_to_instruction],
        Some(&fee_payer.pubkey()),
        &[&fee_payer],
    ...
    println!("Έγινε mint 1.00 tokens στο associated token account");
    println!("\nΔιεύθυνση Mint: {}", mint.pubkey());
    println!("{:#?}", mint_data);

    println!(
        "\nΔιεύθυνση Associated Token Account: {}",
        associated_token_address
    );
    println!("{:#?}", token_data);

    println!("Υπογραφή συναλλαγής: {}", transaction_signature);
...
async def main():
    ...
    # Παραδείγματα keypairs και διευθύνσεων
    payer = Keypair()
    ...
    # Ποσό προς κοπή (στη μικρότερη μονάδα)
    amount_to_mint = 1000000000  # 1 token με 9 δεκαδικά

    async with rpc:
        # Δημιουργία εντολής κοπής
        ...
        print(f"Προορισμός: {destination_token_account}")
        print(f"Ποσό: {amount_to_mint}")
        print(f"Αρχή κοπής: {mint_authority.pubkey()}")
        print(f"Πληρωτής: {payer.pubkey()}")
        print(f"Η συναλλαγή κοπής δημιουργήθηκε επιτυχώς")
