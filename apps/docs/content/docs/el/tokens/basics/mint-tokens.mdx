---
title: Δημιουργία tokens
description:
  Δημιουργήστε νέες μονάδες token σε ένα token account με την εξουσία
  δημιουργίας.
...
## Τι Κάνει η Δημιουργία (Minting);

Η δημιουργία (minting) δημιουργεί νέες μονάδες ενός υπάρχοντος token και αυξάνει
τη συνολική προσφορά του mint.

Μόνο η εξουσία δημιουργίας (mint authority) μπορεί να δημιουργήσει νέα tokens.
Το token account προορισμού πρέπει να υπάρχει ήδη και πρέπει να ανήκει στο ίδιο
mint.

## Πώς να Δημιουργήσετε Tokens

Η δημιουργία tokens χρησιμοποιεί την εντολή _rs`MintTo`_ ή _rs`MintToChecked`_
του Token Program.

Τα παρακάτω παραδείγματα χρησιμοποιούν την _rs`MintToChecked`_, η οποία απαιτεί
από τον καλούντα να παρέχει τα δεκαδικά του mint ώστε η εντολή να επαληθεύσει
την αναμενόμενη ακρίβεια του token πριν από τη δημιουργία.

Το [native mint](/docs/tokens/basics/sync-native) δεν υποστηρίζει δημιουργία.

### Αναφορά Πηγής

| Στοιχείο              | Περιγραφή                                                                                                                                                            | Token Program                                                                                                | Token Extensions Program                                                                                           |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Mint`_            | Η κατάσταση mint αποθηκεύει την εξουσία δημιουργίας και τη συνολική προσφορά token.                                                                                  | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_         | Η κατάσταση token account αποθηκεύει το υπόλοιπο προορισμού που ενημερώνεται με τη δημιουργία.                                                                       | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`MintTo`_          | Μια εντολή δημιουργίας που δημιουργεί tokens σε ένα υπάρχον token account χωρίς να απαιτεί από τον καλούντα να παρέχει τα δεκαδικά του mint.                         | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L159-L177) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L202-L220) |
| _rs`MintToChecked`_   | Μια εντολή δημιουργίας που απαιτεί από τον καλούντα να παρέχει τα δεκαδικά του mint και ελέγχει αυτή την τιμή πριν δημιουργήσει tokens σε ένα υπάρχον token account. | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L301-L325) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L367-L391) |
| _rs`process_mint_to`_ | Κοινή λογική επεξεργαστή για τη δημιουργία tokens.                                                                                                                   | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L521-L583)     | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L981-L1067)    |
