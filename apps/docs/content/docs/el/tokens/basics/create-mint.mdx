...
description:
  Δημιουργήστε ένα mint token και διαμορφώστε τα δεκαδικά ψηφία, την αρχή
  έκδοσης και την αρχή δέσμευσης του mint.
...
Ένας mint account ορίζει και αναγνωρίζει μοναδικά ένα token στο Solana, και
αποθηκεύει την κοινόχρηστη κατάσταση που ισχύει για όλους τους λογαριασμούς
token για αυτό το mint.

Το Token Program ορίζει τον τύπο λογαριασμού _rs`Mint`_ ως:
...
Κάθε token έχει έναν mint account, και η διεύθυνση του mint είναι το μοναδικό
αναγνωριστικό του token σε πορτοφόλια, εφαρμογές και explorers.

Για παράδειγμα, το USD Coin (USDC) έχει τη διεύθυνση mint
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`. Η διεύθυνση mint αναγνωρίζει
μοναδικά το USDC σε όλο το οικοσύστημα του Solana. Μπορείτε να δείτε αυτό το
mint στον
...
1. Η οδηγία _rs`CreateAccount`_ του System Program δημιουργεί έναν νέο
   λογαριασμό απαλλαγμένο από rent και εκχωρεί το Token Program ως τον κάτοχο
   προγράμματος του νέου λογαριασμού.
2. Η οδηγία _rs`InitializeMint`_ ή _rs`InitializeMint2`_ του Token Program
   αρχικοποιεί τον νέο λογαριασμό ως mint.

Συμπεριλάβετε την οδηγία _rs`CreateAccount`_ και την οδηγία αρχικοποίησης mint
στην ίδια συναλλαγή.

Κατά την αρχικοποίηση του mint, το Token Program ελέγχει ότι ο mint account δεν
έχει ήδη αρχικοποιηθεί και είναι απαλλαγμένος από rent. Το Token Program στη
συνέχεια γράφει την αρχή έκδοσης (mint authority), την αρχή δέσμευσης (freeze
authority), τα δεκαδικά ψηφία και τη σημαία _rs`is_initialized`_ στα δεδομένα
του mint account.
...
| ------------------------------ | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Mint`_                     | Η βασική κατάσταση mint που αποθηκεύεται σε κάθε mint account.                                      | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`InitializeMint`_           | Μια οδηγία αρχικοποίησης mint που αναμένει τον λογαριασμό rent sysvar στη λίστα λογαριασμών της.    | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L24-L44)   | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L50-L74)   |
| _rs`InitializeMint2`_          | Μια οδηγία αρχικοποίησης mint που δεν απαιτεί τον λογαριασμό rent sysvar στη λίστα λογαριασμών της. | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L401-L414) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L471-L486) |
| _rs`_process_initialize_mint`_ | Κοινόχρηστη λογική επεξεργασίας για την αρχικοποίηση mint.                                          | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
| _rs`process_initialize_mint`_  | Δημόσιος χειριστής για _rs`InitializeMint`_.                                                        | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L62-L70)       | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L132-L140)     |
| _rs`process_initialize_mint2`_ | Δημόσιος χειριστής για _rs`InitializeMint2`_.                                                       | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L72-L81)       | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L142-L151)     |
...
Ένας λογαριασμός mint αποθηκεύει μόνο βασικές πληροφορίες του token (προσφορά,
δεκαδικά ψηφία, αρχές). Για να προσθέσετε αναγνώσιμα από τον άνθρωπο
μεταδεδομένα όπως όνομα, σύμβολο και εικόνα στο token σας, έχετε δύο επιλογές:

<Cards>
  <Card title="Metaplex Token Metadata" href="/docs/tokens/metaplex">
   Προσθέστε μεταδεδομένα χρησιμοποιώντας το Metaplex Token Metadata Program. Λειτουργεί τόσο με το αρχικό Token Program όσο και με το Token-2022.
  </Card>

  <Card title="Token Extensions Metadata" href="/docs/tokens/extensions/metadata">
   Χρησιμοποιήστε την ενσωματωμένη επέκταση μεταδεδομένων με το Token-2022. 
   Η επέκταση μεταδεδομένων είναι διαθέσιμη μόνο για λογαριασμούς mint που δημιουργήθηκαν με το Token-2022.
   Για να προσθέσετε μεταδεδομένα σε λογαριασμούς mint που δημιουργήθηκαν με το αρχικό Token Program, χρησιμοποιήστε το Metaplex Token Metadata Program.
  </Card>
</Cards>
