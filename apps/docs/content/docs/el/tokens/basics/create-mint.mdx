...
description:
  Δημιουργήστε ένα token mint και διαμορφώστε τα δεκαδικά του mint, την εξουσία
  έκδοσης και την εξουσία δέσμευσης.
...
Ένας λογαριασμός mint ορίζει και αναγνωρίζει μοναδικά ένα token στο Solana, και
...
Κάθε token έχει έναν λογαριασμό mint, και η διεύθυνση mint είναι το μοναδικό
...
Κατά την αρχικοποίηση του mint, το Token Program ελέγχει ότι ο λογαριασμός mint
δεν έχει ήδη αρχικοποιηθεί και είναι απαλλαγμένος από rent. Στη συνέχεια, το
Token Program καταγράφει την εξουσία έκδοσης, την εξουσία δέσμευσης, τα δεκαδικά
και τη σημαία _rs`is_initialized`_ στα δεδομένα του λογαριασμού mint.

### Αναφορά Πηγής

| Στοιχείο                       | Περιγραφή                                                                                           | Token Program                                                                                                | Token Extensions Program                                                                                           |
| ------------------------------ | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Mint`_                     | Η βασική κατάσταση mint που αποθηκεύεται σε κάθε λογαριασμό mint.                                   | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
...
| _rs`_process_initialize_mint`_ | Κοινή λογική επεξεργαστή για την αρχικοποίηση mint.                                                 | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
| _rs`process_initialize_mint`_  | Δημόσιος χειριστής για την _rs`InitializeMint`_.                                                    | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L62-L70)       | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L132-L140)     |
| _rs`process_initialize_mint2`_ | Δημόσιος χειριστής για την _rs`InitializeMint2`_.                                                   | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L72-L81)       | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L142-L151)     |

### Typescript

Τα παρακάτω παραδείγματα `Kit` δείχνουν την προτεινόμενη προσέγγιση
χρησιμοποιώντας το `@solana/kit`. Παραδείγματα παλαιού τύπου που χρησιμοποιούν
το `@solana/web3.js` περιλαμβάνονται για αναφορά.
...
Ένας λογαριασμός mint αποθηκεύει μόνο βασικές πληροφορίες token (προσφορά,
δεκαδικά ψηφία, αρχές). Για να προσθέσετε ευανάγνωστα μεταδεδομένα όπως όνομα,
σύμβολο και εικόνα στο token σας, έχετε δύο επιλογές:
