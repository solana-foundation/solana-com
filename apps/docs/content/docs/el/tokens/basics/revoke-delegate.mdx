...
description:
  Αφαιρέστε τον τρέχοντα εκπρόσωπο ενός λογαριασμού token και μηδενίστε το
  υπόλοιπο εξουσιοδοτημένο ποσό.
...
## Τι Κάνει η Ανάκληση Εκπροσώπου;

Η ανάκληση εκπροσώπου καθαρίζει τον τρέχοντα εκπρόσωπο του λογαριασμού token και
επαναφέρει το εξουσιοδοτημένο ποσό σε μηδέν.

Μετά την ανάκληση, ο προηγούμενος εκπρόσωπος δεν έχει πλέον εξουσιοδότηση να
μεταφέρει ή να καταστρέψει tokens εκ μέρους του λογαριασμού token.

## Πώς να Ανακαλέσετε έναν Εκπρόσωπο

Η ανάκληση εκπροσώπου χρησιμοποιεί την εντολή _rs`Revoke`_ του Token Program.

Η εντολή _rs`Revoke`_ καθαρίζει τα πεδία εκπροσώπου στον πηγαίο λογαριασμό
token. Στο Token Program, ο κάτοχος του λογαριασμού token υπογράφει την
ανάκληση. Στο Token Extensions Program, ο τρέχων εκπρόσωπος μπορεί επίσης να
ανακαλέσει την εξουσιοδότηση.

### Αναφορά Πηγαίου Κώδικα

| Στοιχείο             | Περιγραφή                                                                               | Token Program                                                                                                | Token Extensions Program                                                                                           |
| -------------------- | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Account`_        | Η κατάσταση του λογαριασμού token αποθηκεύει τον εκπρόσωπο και το εξουσιοδοτημένο ποσό. | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Revoke`_         | Εντολή που αφαιρεί τον τρέχοντα εκπρόσωπο από έναν λογαριασμό token.                    | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L128-L140) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L170-L182) |
| _rs`process_revoke`_ | Κοινή λογική επεξεργαστή για ανάκληση εκπροσώπου.                                       | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L395-L420)     | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L643-L674)     |

### Typescript

Τα παραδείγματα `Kit` παρακάτω δείχνουν την προτεινόμενη προσέγγιση
χρησιμοποιώντας `@solana/kit`. Παραδείγματα παλαιού τύπου (legacy)
χρησιμοποιώντας `@solana/web3.js` περιλαμβάνονται για αναφορά.
