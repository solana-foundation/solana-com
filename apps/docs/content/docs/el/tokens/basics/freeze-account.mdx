...
description:
  Πάγωμα ενός λογαριασμού token με την εξουσία πάγωσης του mint για να
  αποκλειστούν μεταφορές, καταστροφές και αλλαγές εξουσιοδότησης.
...
## Τι Κάνει το Πάγωμα Ενός Λογαριασμού Token;

Το πάγωμα ενός λογαριασμού token διατηρεί τον ίδιο κάτοχο, mint και υπόλοιπο,
αλλά αποτρέπει τον λογαριασμό token από το να μεταφέρει ή να καταστρέψει tokens
μέχρι να ξεπαγώσει ο λογαριασμός token.

Ένας παγωμένος λογαριασμός μπορεί ακόμα να κλείσει εάν ο παγωμένος λογαριασμός
έχει μηδενικό υπόλοιπο token.

Μόνο ένα mint που εξακολουθεί να έχει εξουσία πάγωσης μπορεί να παγώσει
λογαριασμούς token. Εάν η εξουσία πάγωσης ενός mint ανακληθεί, οι λογαριασμοί
για αυτό το mint δεν μπορούν πλέον να παγώσουν.

## Πώς να Παγώσετε Έναν Λογαριασμό Token

Το πάγωμα ενός λογαριασμού token χρησιμοποιεί την εντολή _rs`FreezeAccount`_ του
Token Program.

Η εντολή _rs`FreezeAccount`_ επαληθεύει ότι ο λογαριασμός token ανήκει στο mint
και ότι ο υπογράφων είναι η εξουσία πάγωσης του mint, στη συνέχεια αλλάζει την
κατάσταση του λογαριασμού token από αρχικοποιημένο σε παγωμένο.
...
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Mint`_                          | Η κατάσταση mint αποθηκεύει την προαιρετική εξουσία πάγωσης που χρησιμοποιείται για το πάγωμα λογαριασμών token. | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Οι καταστάσεις λογαριασμού token, συμπεριλαμβανομένων των _rs`Initialized`_ και _rs`Frozen`_.                    | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | Μια εντολή που αλλάζει έναν λογαριασμό token στην κατάσταση παγώματος.                                           | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
