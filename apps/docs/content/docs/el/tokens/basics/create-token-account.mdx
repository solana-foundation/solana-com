...
description:
  Δημιουργήστε λογαριασμούς token και associated token accounts για ένα
...
Εδώ, _rs`owner`_ σημαίνει την αρχή που μπορεί να μεταφέρει, να καταστρέψει ή να
αναθέσει tokens από τον token account. Ο κάτοχος του προγράμματος του
λογαριασμού εξακολουθεί να είναι το Token Program ή το Token Extensions Program.

Κάθε token account συνδέεται ακριβώς με ένα mint, που σημαίνει ότι ο token
account μπορεί να κατέχει μονάδες μόνο ενός token, του token που προσδιορίζεται
...
token program και τη διεύθυνση mint.
...
μια τυπική, ντετερμινιστική διεύθυνση. Ο προκύπτων λογαριασμός παραμένει ένας
token account που ανήκει στο Token Program ή Token Extensions Program, όχι στο
...
account σε αυτήν τη διεύθυνση, και ο προκύπτων λογαριασμός εξακολουθεί να
χρησιμοποιεί τον τύπο _rs`Account`_ που ορίζεται από το Token Program ή Token
Extensions Program.

## Πώς να Δημιουργήσετε έναν Συσχετισμένο Λογαριασμό Token

Η δημιουργία ενός συσχετισμένου λογαριασμού token χρησιμοποιεί την εντολή
_rs`Create`_ ή _rs`CreateIdempotent`_ του Associated Token Program. Το
Associated Token Program παράγει τη διεύθυνση ATA, δημιουργεί τον λογαριασμό στη
διεύθυνση ATA και αρχικοποιεί τον λογαριασμό ως λογαριασμό token που ανήκει στο
Token Program ή το Token Extension Program.

<Callout type="info" title="Συνιστάται">
  Για τις περισσότερες εφαρμογές, δημιουργήστε λογαριασμούς token μέσω του
  Associated Token Program αντί να τους δημιουργείτε καλώντας απευθείας τις
  εντολές του System Program και του Token Program. Ένας συσχετισμένος
  λογαριασμός token χρησιμοποιεί μια ντετερμινιστική διεύθυνση που προκύπτει από
  τον κάτοχο, το πρόγραμμα token και το mint, κάτι που καθιστά τον προεπιλεγμένο
  λογαριασμό token για ένα συγκεκριμένο mint ευκολότερο στην εύρεση από
...
| --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| _rs`Create`_                                  | Δημιουργεί έναν συσχετισμένο λογαριασμό token στην παραγόμενη διεύθυνση ATA.                                                                                                                                                                                                                    | [Πηγή](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L20-L29) |
| _rs`CreateIdempotent`_                        | Δημιουργεί τον συσχετισμένο λογαριασμό token, αλλά εξακολουθεί να επιτυγχάνει ακόμα και αν αυτό το ATA υπάρχει ήδη για τον ίδιο κάτοχο και mint.                                                                                                                                                | [Πηγή](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | Παράγει τη διεύθυνση ATA και χρησιμοποιεί το _rs`create_pda_account`_ συν CPIs στο επιλεγμένο πρόγραμμα token για να αρχικοποιήσει τον λογαριασμό token. Όταν το επιλεγμένο πρόγραμμα είναι το Token Extension Program, ο επεξεργαστής ATA αρχικοποιεί επίσης την επέκταση αμετάβλητου κατόχου. | [Πηγή](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | Βοηθητική λειτουργία που χρησιμοποιείται από το _rs`process_create_associated_token_account`_ για τη δημιουργία του λογαριασμού PDA μέσω CPI στο System Program.                                                                                                                                | [Πηγή](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |

### Typescript

Τα παραδείγματα `Kit` παρακάτω δείχνουν την προτεινόμενη προσέγγιση
χρησιμοποιώντας το `@solana/kit`. Παλαιότερα παραδείγματα που χρησιμοποιούν το
...
1. Η εντολή _rs`CreateAccount`_ του System Program δημιουργεί έναν νέο
   λογαριασμό απαλλαγμένο από rent και αναθέτει το Token Program ως κάτοχο
   προγράμματος του νέου λογαριασμού.
2. Η εντολή _rs`InitializeAccount`_, _rs`InitializeAccount2`_, ή
   _rs`InitializeAccount3`_ του Token Program αρχικοποιεί τον νέο λογαριασμό για
   ένα mint και έναν κάτοχο.

Συμπεριλάβετε την εντολή _rs`CreateAccount`_ και την εντολή αρχικοποίησης token
account στην ίδια συναλλαγή.
...
<Callout type="info">
  ...
  απευθείας τις εντολές του System Program και του Token Program. Για τις
  περισσότερες εφαρμογές, χρησιμοποιήστε το Associated Token Program αντί αυτού.
  Χρησιμοποιήστε απευθείας κλήσεις στο System Program και Token Program όταν
  έχετε έναν συγκεκριμένο λόγο να μη χρησιμοποιήσετε ένα associated token
  account ή όταν χρειάζεται να δημιουργήσετε προσαρμοσμένους token accounts PDA
  κάνοντας CPIs στο System Program και τις εντολές του Token Program από το δικό
  σας πρόγραμμα Solana.
</Callout>
...
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Account`_                     | Τα βασικά πεδία token account που αποθηκεύονται σε κάθε token account.                                                         | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`InitializeAccount`_           | Μια εντολή αρχικοποίησης token account που αναμένει τον κάτοχο και τον λογαριασμό rent sysvar στη λίστα λογαριασμών της.       | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L45-L63)   | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L75-L93)   |
| _rs`InitializeAccount2`_          | Μια εντολή αρχικοποίησης token account που περνάει τον κάτοχο στα instruction data αντί για τη λίστα λογαριασμών.              | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L352-L365) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L419-L433) |
| _rs`InitializeAccount3`_          | Μια εντολή αρχικοποίησης token account που περνάει τον κάτοχο στα instruction data και δεν απαιτεί τον λογαριασμό rent sysvar. | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L377-L387) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L445-L456) |
