...
Ένα token account αποθηκεύει το υπόλοιπό σας για ένα συγκεκριμένο token. Κάθε
token account συνδέεται ακριβώς με ένα mint και παρακολουθεί το υπόλοιπο των
tokens σας καθώς και πρόσθετες λεπτομέρειες.
...
Για να κατέχετε tokens, χρειάζεστε ένα token account για αυτό το συγκεκριμένο
mint. Κάθε token account παρακολουθεί:

1. **Mint**: Τον συγκεκριμένο τύπο token που κατέχει
2. **Owner**: Την εξουσία που μπορεί να μεταφέρει tokens από αυτό το account
...
- Αυτό το account κατέχει μόνο USDC tokens
- Η Circle ελέγχει το token μέσω της εξουσίας owner του token account:
  `7VHUFJHWu2CuExkJcJrzhQPJ2oygupTWkL2A2For4BmE`

Μπορείτε να δείτε τις λεπτομέρειες αυτού του token account στο
...
1. **Token Account Owner**: Η εξουσία που μπορεί να μεταφέρει, να καταστρέψει ή
   να αναθέσει tokens. Αυτό αποθηκεύεται στα δεδομένα του token account.

2. **Program Owner**: Το πρόγραμμα που κατέχει το account (πάντα το Token
   Program ή το Token Extension Program για token accounts).

Όταν εργάζεστε με token accounts, ο όρος "owner" συνήθως αναφέρεται στην εξουσία
που μπορεί να μεταφέρει tokens, όχι στο πρόγραμμα που κατέχει το account.

</Callout>

## Τι είναι ένα Associated Token Account;

Ένα associated token account (ATA) είναι ο προεπιλεγμένος λογαριασμός token για
ένα πορτοφόλι ώστε να κρατά ένα συγκεκριμένο token. Χρησιομοποιεί μια
...
<Callout type="info">
  Μόνο οι λογαριασμοί token που δημιουργούνται από το Associated Token Program
  ονομάζονται "associated token accounts".
</Callout>

Τα ATA χρησιμοποιούν ντετερμινιστικές διευθύνσεις, διευκολύνοντας την εύρεση του
λογαριασμού token οποιουδήποτε πορτοφολιού για ένα δεδομένο mint. Δείτε την
[υλοποίηση της παραγωγής εδώ](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/interface/src/address.rs#L56).
...
ATA. Αυτό εξαλείφει την ανάγκη χειροκίνητης παρακολούθησης των διευθύνσεων
λογαριασμών token—μπορείτε πάντα να παράγετε τη σωστή διεύθυνση.
...
- Δημιουργεί λογαριασμούς token σε ντετερμινιστικές διευθύνσεις (PDA)
- Πραγματοποιεί Cross-Program Invocations (CPI) στο Token Program
- Δεν διατηρεί καμία κατάσταση από μόνο του

Οι λογαριασμοί token που προκύπτουν ανήκουν στο Token Program και χρησιμοποιούν
...
Η δημιουργία ενός λογαριασμού token απαιτεί την εντολή
...
Χρειάζεστε δύο εντολές για να δημιουργήσετε έναν λογαριασμό token:

1. System Program: Δημιουργία ενός λογαριασμού με διατεθειμένο χώρο για έναν
   λογαριασμό token και μεταφορά κυριότητας στο Token Program
2. Token Program: Αρχικοποίηση των δεδομένων του λογαριασμού token
...
Αυτή η μοναδική εντολή χειρίζεται αυτόματα τη δημιουργία του λογαριασμού token
