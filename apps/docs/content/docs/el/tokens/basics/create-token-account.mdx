...
description:
  Δημιουργήστε λογαριασμούς token και συσχετισμένους λογαριασμούς token για ένα
  συγκεκριμένο mint και κάτοχο.
...
Ένας token account αποθηκεύει tokens για ένα mint και έναν κάτοχο λογαριασμού
token στο Solana.
...
Εδώ, το _rs`owner`_ σημαίνει την εξουσιοδότηση που μπορεί να μεταφέρει, να
καταστρέψει ή να αναθέσει tokens από τον token account. Ο κάτοχος προγράμματος
του λογαριασμού εξακολουθεί να είναι το Token Program ή το Token Extensions
Program.

Κάθε token account συνδέεται ακριβώς με ένα mint, πράγμα που σημαίνει ότι ο
token account μπορεί να κατέχει μονάδες μόνο ενός token, του token που
προσδιορίζεται από το πεδίο _rs`mint`_ του token account.

## Τι Είναι ένας Συσχετισμένος Λογαριασμός Token

Ένας associated token account (ATA) είναι ο προεπιλεγμένος token account για ένα
πορτοφόλι και ένα mint. Το
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program/src)
παράγει τη διεύθυνση ATA από τη διεύθυνση του πορτοφολιού, τη διεύθυνση του
προγράμματος token και τη διεύθυνση του mint.
...
μια τυποποιημένη, ντετερμινιστική διεύθυνση. Ο προκύπτων λογαριασμός εξακολουθεί
να είναι ένας token account που ανήκει στο Token Program ή στο Token Extensions
...
Για οποιονδήποτε συνδυασμό πορτοφολιού, προγράμματος token και mint, υπάρχει
ακριβώς μία διεύθυνση ATA. Το Associated Token Program δημιουργεί έναν
τυποποιημένο token account σε αυτή τη διεύθυνση, και ο προκύπτων λογαριασμός
εξακολουθεί να χρησιμοποιεί τον τύπο _rs`Account`_ που ορίζεται από το Token
Program ή το Token Extensions Program.
...
<Callout type="info" title="Συνιστάται">
  ...
  account χρησιμοποιεί μια ντετερμινιστική διεύθυνση που προκύπτει από τον
  κάτοχο, το token program και το mint, γεγονός που διευκολύνει τα πορτοφόλια
  και τις εφαρμογές να βρίσκουν τον προεπιλεγμένο token account για ένα
  συγκεκριμένο mint.
</Callout>
...
| _rs`CreateIdempotent`_                        | Δημιουργεί τον associated token account, αλλά εξακολουθεί να επιτυγχάνει ακόμη και αν το ATA υπάρχει ήδη για τον ίδιο κάτοχο και mint.                                                                                                                                                 | [Πηγή](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | Παράγει τη διεύθυνση ATA και χρησιμοποιεί το _rs`create_pda_account`_ συν CPIs στο επιλεγμένο token program για να αρχικοποιήσει τον token account. Όταν το επιλεγμένο πρόγραμμα είναι το Token Extension Program, ο επεξεργαστής ATA αρχικοποιεί επίσης την επέκταση immutable owner. | [Πηγή](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | Βοηθητική συνάρτηση που χρησιμοποιείται από το _rs`process_create_associated_token_account`_ για να δημιουργήσει τον λογαριασμό PDA μέσω CPI στο System Program.                                                                                                                       | [Πηγή](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |

### Typescript

Τα παραδείγματα `Kit` που ακολουθούν δείχνουν την προτεινόμενη προσέγγιση
χρησιμοποιώντας το `@solana/kit`. Παραδείγματα παλαιότερης έκδοσης που
χρησιμοποιούν το `@solana/web3.js` περιλαμβάνονται για αναφορά.
...
## Πώς να Δημιουργήσετε Λογαριασμό Token

Η δημιουργία λογαριασμού token απαιτεί δύο οδηγίες:
...
Συμπεριλάβετε την οδηγία _rs`CreateAccount`_ και την οδηγία αρχικοποίησης
λογαριασμού token στην ίδια συναλλαγή.

Κατά την αρχικοποίηση του λογαριασμού token, το Token Program ελέγχει ότι ο
λογαριασμός δεν έχει ήδη αρχικοποιηθεί και είναι απαλλαγμένος από rent.

<Callout type="info">
  Η παρακάτω ενότητα δείχνει πώς να δημιουργήσετε έναν λογαριασμό token καλώντας
  ...
  Χρησιμοποιήστε απευθείας κλήσεις στο System Program και Token Program όταν
  έχετε συγκεκριμένο λόγο να μην χρησιμοποιήσετε associated token account ή όταν
  χρειάζεται να δημιουργήσετε προσαρμοσμένους λογαριασμούς token PDA
  πραγματοποιώντας CPIs στις οδηγίες του System Program και Token Program από το
  δικό σας πρόγραμμα Solana.
</Callout>
...
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Account`_                     | Τα βασικά πεδία λογαριασμού token που αποθηκεύονται σε κάθε λογαριασμό token.                                                      | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`InitializeAccount`_           | Μια οδηγία αρχικοποίησης λογαριασμού token που αναμένει τον κάτοχο και τον λογαριασμό rent sysvar στη λίστα λογαριασμών της.       | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L45-L63)   | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L75-L93)   |
| _rs`InitializeAccount2`_          | Μια οδηγία αρχικοποίησης λογαριασμού token που περνάει τον κάτοχο στα δεδομένα οδηγιών αντί για τη λίστα λογαριασμών.              | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L352-L365) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L419-L433) |
| _rs`InitializeAccount3`_          | Μια οδηγία αρχικοποίησης λογαριασμού token που περνάει τον κάτοχο στα δεδομένα οδηγιών και δεν απαιτεί τον λογαριασμό rent sysvar. | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L377-L387) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L445-L456) |
| _rs`_process_initialize_account`_ | Κοινή λογική επεξεργαστή για αρχικοποίηση λογαριασμού token.                                                                       | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L83-L141)      | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L153-L234)     |
| _rs`process_initialize_account`_  | Δημόσιος χειριστής για _rs`InitializeAccount`_.                                                                                    | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L143-L150)     | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L236-L240)     |
| _rs`process_initialize_account2`_ | Δημόσιος χειριστής για _rs`InitializeAccount2`_.                                                                                   | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L152-L160)     | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L242-L246)     |
| _rs`process_initialize_account3`_ | Δημόσιος χειριστής για _rs`InitializeAccount3`_.                                                                                   | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L162-L170)     | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L248-L252)     |
