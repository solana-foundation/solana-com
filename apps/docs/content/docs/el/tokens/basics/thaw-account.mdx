...
description:
  ...
  μεταφορές, η καταστροφή και οι αλλαγές εξουσιοδότησης.
...
Το ξεπάγωμα αλλάζει έναν λογαριασμό token από την κατάσταση παγωμένου πίσω στην
κατάσταση αρχικοποιημένου.

Μετά από ένα επιτυχημένο ξεπάγωμα, ο λογαριασμός μπορεί και πάλι να μεταφέρει,
να καταστρέψει και να κλείσει tokens. Μόνο η εξουσιοδότηση πάγωμα του mint
μπορεί να ξεπαγώσει λογαριασμούς token.

## Πώς να Ξεπαγώσετε έναν Λογαριασμό Token

Το ξεπάγωμα ενός λογαριασμού token χρησιμοποιεί την εντολή _rs`ThawAccount`_ του
Token Program.

Η εντολή _rs`ThawAccount`_ επαληθεύει ότι ο λογαριασμός token ανήκει στο mint
και ότι ο υπογράφων είναι η εξουσιοδότηση πάγωματος του mint, στη συνέχεια
αλλάζει την κατάσταση του λογαριασμού token από παγωμένη πίσω σε αρχικοποιημένη.

Οι [εγγενείς λογαριασμοί token](/docs/tokens/basics/sync-native) δεν
υποστηρίζουν ξεπάγωμα.

### Αναφορά Πηγής

| Στοιχείο                            | Περιγραφή                                                                                                                      | Token Program                                                                                                | Token Extension Program                                                                                            |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Mint`_                          | Η κατάσταση του mint αποθηκεύει την προαιρετική εξουσιοδότηση πάγωματος που χρησιμοποιείται για το ξεπάγωμα λογαριασμών token. | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Οι καταστάσεις του λογαριασμού token, συμπεριλαμβανομένων των _rs`Frozen`_ και _rs`Initialized`_.                              | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`ThawAccount`_                   | Μια εντολή που αλλάζει έναν παγωμένο λογαριασμό token πίσω στην αρχικοποιημένη κατάσταση.                                      | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L229-L243) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L292-L306) |
| _rs`process_toggle_freeze_account`_ | Κοινή λογική επεξεργαστή για το πάγωμα και το ξεπάγωμα λογαριασμών token.                                                      | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |

### Typescript

Τα παρακάτω παραδείγματα `Kit` δείχνουν την προτεινόμενη προσέγγιση
χρησιμοποιώντας το `@solana/kit`. Παλαιότερα παραδείγματα με χρήση του
