...
description:
  Ξεπαγώστε έναν παγωμένο λογαριασμό token ώστε να μπορούν να συνεχιστούν οι
  μεταφορές, οι καταστροφές και οι αλλαγές εξουσιοδότησης.
...
## Τι Κάνει το Ξεπάγωμα ενός Λογαριασμού Token;

Το ξεπάγωμα αλλάζει έναν token account από την κατάσταση παγωμένου πίσω στην
κατάσταση αρχικοποιημένου.

Μετά την επιτυχή ολοκλήρωση του ξεπαγώματος, ο λογαριασμός μπορεί να μεταφέρει,
να καταστρέφει και να κλείνει tokens και πάλι. Μόνο η εξουσία πάγωσης του mint
μπορεί να ξεπαγώσει token accounts.

## Πώς να Ξεπαγώσετε έναν Λογαριασμό Token

Το ξεπάγωμα ενός token account χρησιμοποιεί την εντολή _rs`ThawAccount`_ του
Token Program.

Η εντολή _rs`ThawAccount`_ επαληθεύει ότι ο token account ανήκει στο mint και
ότι ο υπογράφων είναι η εξουσία πάγωσης του mint, στη συνέχεια αλλάζει την
κατάσταση του token account από παγωμένη πίσω σε αρχικοποιημένη.

[Οι εγγενείς token accounts](/docs/tokens/basics/sync-native) δεν υποστηρίζουν
ξεπάγωμα.
...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Mint`_                          | Η κατάσταση του mint αποθηκεύει την προαιρετική εξουσία πάγωσης που χρησιμοποιείται για το ξεπάγωμα token accounts. | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Οι καταστάσεις του token account, συμπεριλαμβανομένων των _rs`Frozen`_ και _rs`Initialized`_.                       | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`ThawAccount`_                   | Μια εντολή που αλλάζει έναν παγωμένο token account πίσω στην κατάσταση αρχικοποιημένου.                             | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L229-L243) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L292-L306) |
| _rs`process_toggle_freeze_account`_ | Κοινή λογική επεξεργασίας για το πάγωμα και το ξεπάγωμα token accounts.                                             | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |
