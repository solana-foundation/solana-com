...
από τον καλούντα να παρέχει τα δεκαδικά ψηφία του mint ώστε η εντολή να
επαληθεύσει την αναμενόμενη ακρίβεια token πριν από την καταστροφή.

Το [native mint](/docs/tokens/basics/sync-native) δεν υποστηρίζει καταστροφή.
Χρησιμοποιήστε την _rs`CloseAccount`_ αντί αυτού.

Ο κάτοχος του λογαριασμού token ή ένας εγκεκριμένος εκπρόσωπος υπογράφει τη
συναλλαγή. Στο Token Extension Program, ένας μόνιμος εκπρόσωπος mint μπορεί
επίσης να εξουσιοδοτήσει την καταστροφή εάν το mint έχει ενεργοποιημένη την
επέκταση μόνιμου εκπροσώπου.

### Αναφορά Πηγής

| Στοιχείο           | Περιγραφή                                                                                                                                                                             | Token Program                                                                                                | Token Extension Program                                                                                            |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Mint`_         | Η κατάσταση mint αποθηκεύει τη συνολική προσφορά token.                                                                                                                               | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | Η κατάσταση λογαριασμού token αποθηκεύει το υπόλοιπο token που μειώνεται από μια καταστροφή.                                                                                          | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Μια εντολή καταστροφής που μειώνει το υπόλοιπο λογαριασμού token και την προσφορά mint χωρίς να απαιτεί από τον καλούντα να παρέχει τα δεκαδικά ψηφία του mint.                       | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Μια εντολή καταστροφής που απαιτεί από τον καλούντα να παρέχει τα δεκαδικά ψηφία του mint και ελέγχει αυτή την τιμή πριν μειώσει το υπόλοιπο λογαριασμού token και την προσφορά mint. | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | Κοινή λογική επεξεργαστή για καταστροφές token.                                                                                                                                       | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |
