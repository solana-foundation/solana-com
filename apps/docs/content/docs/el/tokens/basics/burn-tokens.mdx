...
title: Καύση Tokens
description:
  Καύση tokens από ένα token account και μείωση της συνολικής προσφοράς του
  mint.
...
Η καύση tokens μειώνει μόνιμα το υπόλοιπο ενός token account και μειώνει τη
συνολική προσφορά του mint κατά το ίδιο ποσό.

## Πώς να Κάψετε Tokens

Η καύση tokens χρησιμοποιεί την εντολή _rs`Burn`_ ή _rs`BurnChecked`_ του Token
Program.

Τα παρακάτω παραδείγματα χρησιμοποιούν την _rs`BurnChecked`_, η οποία απαιτεί
από τον καλούντα να παρέχει τα δεκαδικά του mint ώστε η εντολή να μπορεί να
επαληθεύσει την αναμενόμενη ακρίβεια του token πριν από την καύση.

Το [native mint](/docs/tokens/basics/sync-native) δεν υποστηρίζει καύση.
Χρησιμοποιήστε την _rs`CloseAccount`_ αντί αυτού.

Ο κάτοχος του token account ή ένας εγκεκριμένος delegate υπογράφει τη συναλλαγή.
Στο Token Extension Program, ένας mint permanent delegate μπορεί επίσης να
εξουσιοδοτήσει την καύση εάν το mint έχει ενεργοποιημένη την επέκταση permanent
delegate.
...
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Mint`_         | Η κατάσταση του mint αποθηκεύει τη συνολική προσφορά token.                                                                                                                    | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | Η κατάσταση του token account αποθηκεύει το υπόλοιπο token που μειώνεται από μια καύση.                                                                                        | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Μια εντολή καύσης που μειώνει το υπόλοιπο ενός token account και την προσφορά του mint χωρίς να απαιτεί από τον καλούντα να παρέχει τα δεκαδικά του mint.                      | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Μια εντολή καύσης που απαιτεί από τον καλούντα να παρέχει τα δεκαδικά του mint και ελέγχει την τιμή αυτή πριν μειώσει το υπόλοιπο του token account και την προσφορά του mint. | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | Κοινή λογική επεξεργαστή για καύσεις token.                                                                                                                                    | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |

### Typescript

Τα παραδείγματα `Kit` παρακάτω δείχνουν την προτεινόμενη προσέγγιση
χρησιμοποιώντας `@solana/kit`. Παλαιότερα παραδείγματα χρησιμοποιώντας
