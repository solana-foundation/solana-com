...
description:
  Καταστρέψτε tokens από έναν λογαριασμό token και μειώστε τη συνολική προσφορά
  του mint.
...
## Τι Κάνει η Καταστροφή;

Η καταστροφή tokens μειώνει μόνιμα το υπόλοιπο ενός λογαριασμού token και
μειώνει τη συνολική προσφορά του mint κατά το ίδιο ποσό.

## Πώς να Καταστρέψετε Tokens

Η καταστροφή tokens χρησιμοποιεί την εντολή _rs`Burn`_ ή _rs`BurnChecked`_ του
Token Program.
...
επαληθεύσει την αναμενόμενη ακρίβεια του token πριν από την καταστροφή.

Το [native mint](/docs/tokens/basics/sync-native) δεν υποστηρίζει καταστροφή.
Χρησιμοποιήστε το _rs`CloseAccount`_ αντί αυτού.

Ο κάτοχος του λογαριασμού token ή ένας εγκεκριμένος αντιπρόσωπος υπογράφει τη
συναλλαγή. Στο Token Extension Program, ένας μόνιμος αντιπρόσωπος του mint
μπορεί επίσης να εξουσιοδοτήσει την καταστροφή εάν το mint έχει ενεργοποιημένη
την επέκταση μόνιμου αντιπροσώπου.
...
| _rs`Account`_      | Η κατάσταση του λογαριασμού token αποθηκεύει το υπόλοιπο token που μειώνεται από μια καταστροφή.                                                                                    | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Μια εντολή καταστροφής που μειώνει το υπόλοιπο λογαριασμού token και την προσφορά του mint χωρίς να απαιτεί από τον καλούντα να παρέχει τα δεκαδικά του mint.                       | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Μια εντολή καταστροφής που απαιτεί από τον καλούντα να παρέχει τα δεκαδικά του mint και ελέγχει αυτή την τιμή πριν μειώσει το υπόλοιπο λογαριασμού token και την προσφορά του mint. | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | Κοινή λογική επεξεργασίας για καταστροφές token.                                                                                                                                    | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |
