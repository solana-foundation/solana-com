...
description:
  Εξουσιοδοτήστε μια άλλη διεύθυνση να μεταφέρει ή να καταστρέψει περιορισμένη
  ποσότητα tokens από έναν λογαριασμό token.
...
## Τι Κάνει η Έγκριση Αντιπροσώπου;

Η έγκριση ενός αντιπροσώπου επιτρέπει σε μια άλλη διεύθυνση να μεταφέρει ή να
καταστρέψει περιορισμένη ποσότητα tokens από έναν λογαριασμό token εκ μέρους του
κατόχου του λογαριασμού token.

Ένας λογαριασμός token αποθηκεύει έναν τρέχοντα αντιπρόσωπο και ένα
εξουσιοδοτημένο ποσό κάθε φορά. Η έγκριση νέου αντιπροσώπου αντικαθιστά τον
προηγούμενο αντιπρόσωπο και το όριο στον λογαριασμό.

## Πώς να Εγκρίνετε έναν Αντιπρόσωπο

Η έγκριση αντιπροσώπου χρησιμοποιεί την εντολή _rs`Approve`_ ή
_rs`ApproveChecked`_ του Token Program.

Τα παρακάτω παραδείγματα χρησιμοποιούν την _rs`ApproveChecked`_, η οποία απαιτεί
από τον καλούντα να παρέχει το mint και τα δεκαδικά ψηφία ώστε η εντολή να
επαληθεύσει το αναμενόμενο mint και την ακρίβεια των tokens πριν από την
επεξεργασία της έγκρισης.

Ο κάτοχος του λογαριασμού token υπογράφει την έγκριση.

### Αναφορά Πηγής

| Στοιχείο              | Περιγραφή                                                                                                                                                                       | Token Program                                                                                                | Token Extension Program                                                                                            |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Account`_         | Η κατάσταση του λογαριασμού token αποθηκεύει τον κάτοχο, τον αντιπρόσωπο και το εξουσιοδοτημένο ποσό.                                                                           | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | Μια εντολή έγκρισης αντιπροσώπου που καταγράφει τον αντιπρόσωπο και το όριο για έναν λογαριασμό token χωρίς να απαιτεί από τον καλούντα να παρέχει το mint ή τα δεκαδικά ψηφία. | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | Μια εντολή έγκρισης αντιπροσώπου που απαιτεί από τον καλούντα να παρέχει το mint και τα δεκαδικά ψηφία και ελέγχει αυτές τις τιμές πριν καταγράψει τον αντιπρόσωπο και το όριο. | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
| _rs`process_approve`_ | Κοινή λογική επεξεργαστή για την έγκριση αντιπροσώπου.                                                                                                                          | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L344-L392)     | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L582-L640)     |
