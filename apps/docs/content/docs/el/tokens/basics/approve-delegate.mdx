...
description:
  ...
  ποσότητα tokens από έναν token account.
...
καταστρέψει περιορισμένη ποσότητα tokens από έναν token account εκ μέρους του
κατόχου του token account.

Ένας token account αποθηκεύει έναν τρέχοντα αντιπρόσωπο και ένα εξουσιοδοτημένο
ποσό κάθε φορά. Η έγκριση νέου αντιπροσώπου αντικαθιστά τον προηγούμενο
αντιπρόσωπο και το όριο στον λογαριασμό.
...
από τον καλούντα να παρέχει το mint και τα decimals ώστε η εντολή να επαληθεύσει
το αναμενόμενο mint και την ακρίβεια των tokens πριν επεξεργαστεί την έγκριση.

Ο κάτοχος του token account υπογράφει την έγκριση.

### Αναφορά Πηγής

| Στοιχείο              | Περιγραφή                                                                                                                                                                 | Token Program                                                                                                | Token Extensions Program                                                                                           |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| _rs`Account`_         | Η κατάσταση του token account αποθηκεύει τον κάτοχο, τον αντιπρόσωπο και το εξουσιοδοτημένο ποσό.                                                                         | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | Μια εντολή έγκρισης αντιπροσώπου που καταγράφει τον αντιπρόσωπο και το όριο για έναν token account χωρίς να απαιτεί από τον καλούντα να παρέχει το mint ή τα decimals.    | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | Μια εντολή έγκρισης αντιπροσώπου που απαιτεί από τον καλούντα να παρέχει το mint και τα decimals και ελέγχει αυτές τις τιμές πριν καταγράψει τον αντιπρόσωπο και το όριο. | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
| _rs`process_approve`_ | Κοινή λογική επεξεργασίας για έγκριση αντιπροσώπου.                                                                                                                       | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L344-L392)     | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L582-L640)     |
