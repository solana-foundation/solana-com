...
description:
  Κλείστε έναν λογαριασμό token και επιστρέψτε τα lamports του λογαριασμού σε έναν λογαριασμό προορισμού.
...
## Τι Κάνει το Κλείσιμο ενός Λογαριασμού Token;

Το κλείσιμο ενός token account διαγράφει τον λογαριασμό και μεταφέρει όλα τα lamports του,
συμπεριλαμβανομένου του επιστρεφόμενου ελάχιστου υπολοίπου αποθήκευσης, σε έναν λογαριασμό προορισμού.

Το υπόλοιπο token πρέπει να είναι μηδέν πριν κλείσει ο λογαριασμός. Ένας παγωμένος λογαριασμός μπορεί να κλείσει εφόσον το υπόλοιπο token του παγωμένου λογαριασμού είναι μηδέν. Τα token accounts Wrapped SOL αποτελούν εξαίρεση και μπορούν να κλείσουν με υπόλοιπο token για την ανάκτηση του υποκείμενου SOL. Ο κάτοχος του λογαριασμού ή η αρχή κλεισίματος του λογαριασμού υπογράφει την εντολή κλεισίματος και επιλέγει τον προορισμό. Ο λογαριασμός που πλήρωσε για τη δημιουργία ενός associated token account δεν λαμβάνει αυτόματα το ανακτηθέν υπόλοιπο.

## Πώς να Κλείσετε έναν Λογαριασμό Token

Το κλείσιμο ενός λογαριασμού token χρησιμοποιεί την εντολή _rs`CloseAccount`_
του Token Program.

Η εντολή _rs`CloseAccount`_ μεταφέρει τα lamports του λογαριασμού πηγής σε έναν
λογαριασμό προορισμού, διαγράφει τα δεδομένα του λογαριασμού πηγής και διαγράφει
τον λογαριασμό πηγής. Το Token Extension Program εφαρμόζει επιπλέον ελέγχους
κλεισίματος για ορισμένες επεκτάσεις, αλλά η βασική ροή κλεισίματος είναι η
ίδια.
...
| --------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | Η κατάσταση του λογαριασμού token αποθηκεύει το υπόλοιπο, την κατάσταση native και την εξουσία κλεισίματος που χρησιμοποιούνται κατά το κλείσιμο του λογαριασμού.     | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | Μια εντολή που κλείνει έναν λογαριασμό token και μεταφέρει τα lamports του λογαριασμού token σε έναν λογαριασμό προορισμού. | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | Κοινή λογική επεξεργαστή για το κλείσιμο λογαριασμού token.                                                               | [Πηγή](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |
