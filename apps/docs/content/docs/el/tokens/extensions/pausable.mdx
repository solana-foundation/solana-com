...
title: Παυσίμη κοπής
description:
  Δημιουργήστε ένα Token-2022 mint με εξουσιοδότηση παύσης, μπλοκάρετε μεταφορές
  όταν το mint είναι σε παύση και συνεχίστε τη δραστηριότητα αργότερα.
...
## Τι Είναι ένα Mint με Δυνατότητα Παύσης;

Η επέκταση mint _rs`PausableConfig`_ του Token Extensions Program παρέχει στην
εξουσιοδότηση παύσης τη δυνατότητα να διακόπτει και να συνεχίζει τη
δραστηριότητα token για ολόκληρο το mint.

Όταν ένα mint βρίσκεται σε παύση, το Token Extensions Program απορρίπτει:

- Μεταφορές
- Εκδόσεις (Mints)
- Καταστροφές (Burns)

Όταν το mint συνεχιστεί, αυτές οι εντολές λειτουργούν κανονικά ξανά.

## Πώς να Δημιουργήσετε, Παύσετε και Συνεχίσετε ένα Mint

Για να δημιουργήσετε, παύσετε και συνεχίσετε ένα mint:

1. Υπολογίστε το μέγεθος του λογαριασμού mint και το rent που απαιτείται για το
   mint και την επέκταση _rs`PausableConfig`_.
2. Δημιουργήστε τον λογαριασμό mint με _rs`CreateAccount`_, αρχικοποιήστε το
   _rs`PausableConfig`_ και αρχικοποιήστε το mint με _rs`InitializeMint`_.
3. Εκδώστε tokens και δημιουργήστε token accounts για το mint.
4. Χρησιμοποιήστε το _rs`Pause`_ για να μπλοκάρετε τη δραστηριότητα token για το
   mint.
5. Χρησιμοποιήστε το _rs`Resume`_ για να επιτρέψετε ξανά τη δραστηριότητα token.
...
_rs`PausableConfig`_. Αυτό είναι το μέγεθος που χρησιμοποιείται στο
_rs`CreateAccount`_.
...
## !!steps Αρχικοποίηση PausableConfig

Αρχικοποιήστε την επέκταση _rs`PausableConfig`_ στο mint.
...
## !!steps Παύση του mint

Θέστε το mint σε παύση με _rs`Pause`_.
...
## !!steps Συνέχιση του mint

Συνεχίστε τη λειτουργία του mint με _rs`Resume`_.
...
<Callout type="info" title="Σειρά Εντολών">
  Το _rs`PausableInstruction::Initialize`_ πρέπει να προηγείται του
  _rs`InitializeMint`_. Τα _rs`CreateAccount`_,
  _rs`PausableInstruction::Initialize`_ και _rs`InitializeMint`_ πρέπει να
...
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PausableConfig`_                  | Επέκταση mint που αποθηκεύει την αρχή παύσης και αν το mint είναι επί του παρόντος σε παύση.                              | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/mod.rs#L17-L22)         |
| _rs`PausableInstruction::Initialize`_ | Εντολή που αρχικοποιεί τη ρύθμιση παύσης πριν από το _rs`InitializeMint`_.                                                | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L20-L32) |
| _rs`PausableInstruction::Pause`_      | Εντολή που σημειώνει το mint ως παυμένο.                                                                                  | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L33-L44) |
| _rs`PausableInstruction::Resume`_     | Εντολή που σημειώνει το mint ως μη παυμένο.                                                                               | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L45-L56) |
| _rs`process_initialize`_              | Λογική επεξεργασίας που αρχικοποιεί το _rs`PausableConfig`_ σε ένα μη αρχικοποιημένο mint και αποθηκεύει την αρχή παύσης. | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L22-L35)     |
| _rs`process_toggle_pause`_            | Λογική επεξεργασίας που επαληθεύει την αρχή παύσης πριν εναλλάξει τη σημαία παύσης του mint.                              | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L39-L64)     |
