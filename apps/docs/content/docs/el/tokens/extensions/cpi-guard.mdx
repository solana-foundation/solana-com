...
title: CPI Guard
description:
  Ενεργοποιήστε την επέκταση λογαριασμού CpiGuard για να αποκλείσετε εντολές
  token που καλούνται μέσω cross-program invocations.
...
Η επέκταση λογαριασμού _rs`CpiGuard`_ του Token Extensions Program εμποδίζει ένα
άλλο onchain πρόγραμμα να χρησιμοποιήσει έναν token account για συγκεκριμένες
εντολές token μέσω Cross Program Invocation (CPI).

Όταν το CPI guard είναι ενεργοποιημένο, το Token Extensions Program απορρίπτει
αυτές τις εντολές όταν ένα άλλο onchain πρόγραμμα τις καλεί μέσω CPI:

- _rs`Transfer`_, _rs`TransferChecked`_ και _rs`TransferCheckedWithFee`_ όταν ο
  κάτοχος του token account χρησιμοποιείται ως εξουσιοδότηση της εντολής
- _rs`Burn`_ και _rs`BurnChecked`_ όταν ο κάτοχος του token account
  χρησιμοποιείται ως εξουσιοδότηση της εντολής
- _rs`Approve`_ και _rs`ApproveChecked`_
- _rs`CloseAccount`_ όταν θα αποστέλλονταν lamport κάπου αλλού εκτός από τον
  κάτοχο του token account
- _rs`SetAuthority`_ κατά την προσθήκη ή αντικατάσταση της εξουσιοδότησης
  κλεισίματος

Όταν το CPI guard είναι ενεργοποιημένο, αυτές οι περιπτώσεις εξακολουθούν να
επιτρέπονται:

- _rs`Transfer`_, _rs`TransferChecked`_, _rs`TransferCheckedWithFee`_,
  _rs`Burn`_ και _rs`BurnChecked`_ μέσω CPI όταν υπογράφει ένας delegate ή
  permanent delegate αντί του κατόχου του token account
- _rs`CloseAccount`_ μέσω CPI όταν τα lamport αποστέλλονται στον κάτοχο του
  token account
- _rs`SetAuthority`_ για την αφαίρεση μιας υπάρχουσας εξουσιοδότησης κλεισίματος
- _rs`Revoke`_

Όταν ο κάτοχος του token account χρησιμοποιείται ως εξουσιοδότηση της εντολής,
οι εντολές του Token Extensions Program μπορούν ακόμα να επεξεργαστούν εάν η
εντολή προστεθεί άμεσα στη συναλλαγή αντί να κληθεί μέσω CPI. Η αλλαγή του
κατόχου του token account με _rs`SetAuthority`_ παραμένει αποκλεισμένη όσο το
CPI guard είναι ενεργοποιημένο, ακόμα και εκτός CPI. Οι _rs`EnableCpiGuard`_ και
_rs`DisableCpiGuard`_ επίσης δεν μπορούν να κληθούν μέσω CPI.
...
2. Υπολογίστε το μέγεθος του token account και το rent που απαιτείται για ένα
   token account με _rs`CpiGuard`_.
3. Δημιουργήστε και αρχικοποιήστε ένα token account για το mint.
4. Καλέστε την _rs`EnableCpiGuard`_ στο token account.
5. Καλέστε την _rs`DisableCpiGuard`_ στο token account για να επιτρέψετε αυτές
   τις οδηγίες μέσω CPI ξανά.
...
## !!steps Υπολογίστε το μέγεθος του token account

Υπολογίστε το μέγεθος του token account για το βασικό token account συν την
επέκταση _rs`CpiGuard`_. Αυτό είναι το μέγεθος που χρησιμοποιείται στην
_rs`CreateAccount`_.
...
## !!steps Υπολογίστε το rent

Υπολογίστε το rent χρησιμοποιώντας το μέγεθος του token account που απαιτείται
για την επέκταση _rs`CpiGuard`_.
...
## !!steps Δημιουργήστε και αρχικοποιήστε το token account

Δημιουργήστε το token account με τον υπολογισμένο χώρο και τα lamports, και στη
συνέχεια αρχικοποιήστε το για το mint.
...
## !!steps Ενεργοποιήστε το CPI guard

Ενεργοποιήστε το _rs`CpiGuard`_ στο token account.
...
## !!steps Απενεργοποιήστε το CPI guard

Απενεργοποιήστε το _rs`CpiGuard`_ στο token account.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`CpiGuard`_                     | Επέκταση λογαριασμού που αποθηκεύει εάν το CPI guard είναι ενεργοποιημένο για τον token account.                                                                                                                                                  | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/mod.rs#L20-L25)         |
| _rs`CpiGuardInstruction::Enable`_  | Εντολή που ενεργοποιεί το CPI guard σε έναν token account.                                                                                                                                                                                        | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/instruction.rs#L20-L38) |
| _rs`CpiGuardInstruction::Disable`_ | Εντολή που απενεργοποιεί το CPI guard σε έναν token account.                                                                                                                                                                                      | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/instruction.rs#L39-L53) |
| _rs`process_toggle_cpi_guard`_     | Λογική επεξεργαστή που χρησιμοποιείται από τις _rs`Enable`_ και _rs`Disable`_ που επικυρώνει την κυριότητα λογαριασμού, απορρίπτει αλλαγές CPI στο ίδιο το guard και αρχικοποιεί το _rs`CpiGuard`_ εάν δεν υπάρχει ήδη πριν ορίσει το `lock_cpi`. | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/cpi_guard/processor.rs#L19-L52)     |
