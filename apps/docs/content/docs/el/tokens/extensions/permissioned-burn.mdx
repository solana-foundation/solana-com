...
Ένας τοπικός test validator περιλαμβάνει επίσης μια παλαιότερη έκδοση του
Token-2022, οπότε για να εκτελέσετε τα παραδείγματα αυτής της σελίδας, φορτώστε
το πρόγραμμα v11.0.0 από το devnet στον test validator σας:
...
1. Υπολογίστε το μέγεθος του mint account και το rent που χρειάζεται για το mint
...
3. Δημιουργήστε ένα token account και κόψτε tokens.
4. Εκτελέστε burn με _rs`PermissionedBurnInstruction::BurnChecked`_
   υπογεγραμμένο τόσο από τον κάτοχο του token account όσο και από την αρχή
   burn.
5. Προαιρετικά απενεργοποιήστε το εξουσιοδοτημένο burning με _rs`SetAuthority`_
   χρησιμοποιώντας _rs`AuthorityType::PermissionedBurn`_ και μια αρχή `None`, η
...
Υπολογίστε το rent χρησιμοποιώντας το μέγεθος που χρειάζεται για το mint συν την
...
## !!steps Κάψιμο με την αρχή εξουσιοδότησης καψίματος

Κάψτε token με _rs`PermissionedBurnInstruction::BurnChecked`_ υπογεγραμμένο και
από τον ιδιοκτήτη του token account και την αρχή εξουσιοδότησης καψίματος.
...
| ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | Επέκταση mint που αποθηκεύει την αρχή εξουσιοδότησης που απαιτείται για τη συν-υπογραφή κάθε καψίματος για το mint.                            | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
| _rs`PermissionedBurnInstruction::Initialize`_  | Εντολή που αρχικοποιεί τη ρύθμιση παραμέτρων εξουσιοδοτημένου καψίματος πριν από _rs`InitializeMint`_.                                         | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L41-L49) |
| _rs`PermissionedBurnInstruction::Burn`_        | Εντολή καψίματος που απαιτεί υπογραφές από την αρχή εξουσιοδότησης καψίματος και τον ιδιοκτήτη ή εντολοδόχο του token account.                 | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | Εντολή καψίματος με έλεγχο δεκαδικών που χρησιμοποιεί τους ίδιους λογαριασμούς με _rs`PermissionedBurnInstruction::Burn`_.                     | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | Διακριτής αρχής που χρησιμοποιείται με _rs`SetAuthority`_ για εναλλαγή ή απενεργοποίηση της αρχής εξουσιοδότησης καψίματος σε ένα mint.        | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
| _rs`process_initialize`_                       | Λογική επεξεργαστή που αρχικοποιεί _rs`PermissionedBurnConfig`_ σε μη αρχικοποιημένο mint και αποθηκεύει την αρχή εξουσιοδότησης καψίματος.    | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/extension/permissioned_burn/processor.rs#L29-L47)     |
| _rs`process_burn`_                             | Λογική επεξεργαστή που απορρίπτει τυπικά καψίματα όταν η αρχή εξουσιοδότησης καψίματος είναι ενεργή και επαληθεύει την υπογραφή της αρχής.     | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | Λογική επεξεργαστή που επικυρώνει και εναλλάσσει την αρχή εξουσιοδότησης καψίματος όταν χρησιμοποιείται _rs`AuthorityType::PermissionedBurn`_. | [Source](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### Typescript

Το παρακάτω παράδειγμα με `Kit` χρησιμοποιεί απευθείας τις παραγόμενες οδηγίες.
Το δημοσιευμένο παλαιό πακέτο `@solana/spl-token` δεν περιλαμβάνει ακόμη
υποστήριξη για εξουσιοδοτημένη καύση (permissioned burn), οπότε δεν
συμπεριλαμβάνεται παλαιό παράδειγμα.
