...
description:
  Δημιουργήστε ένα mint του Token Extension Program με μόνιμο εκπρόσωπο που
  μπορεί να μεταφέρει ή να καταστρέφει tokens από οποιονδήποτε λογαριασμό για
  αυτό το mint.
...
Η επέκταση mint _rs`PermanentDelegate`_ του Token Extension Program εκχωρεί μια
εξουσία εκπροσώπου στο ίδιο το mint.

Ένας μόνιμος εκπρόσωπος είναι μια εξουσία σε επίπεδο mint που μπορεί να
εξουσιοδοτεί μεταφορές και καταστροφές για οποιονδήποτε token account για αυτό
το mint. Λειτουργεί ως καθολικός εκπρόσωπος για όλους τους token accounts για το
mint, και οι κάτοχοι token account δεν μπορούν να ανακαλέσουν τον μόνιμο
εκπρόσωπο από τους token accounts τους.

Η εξουσία του mint μπορεί επίσης να αλλάξει τον μόνιμο εκπρόσωπο αργότερα με την
_rs`SetAuthority`_ χρησιμοποιώντας την _rs`AuthorityType::PermanentDelegate`_.
...
1. Υπολογίστε το μέγεθος του λογαριασμού mint και το rent που απαιτείται για το
   mint και την επέκταση _rs`PermanentDelegate`_.
2. Δημιουργήστε τον λογαριασμό mint με την _rs`CreateAccount`_, αρχικοποιήστε
   την _rs`PermanentDelegate`_, και αρχικοποιήστε το mint με την
   _rs`InitializeMint`_.
3. Υποβάλετε την _rs`TransferChecked`_ ή την _rs`BurnChecked`_ υπογεγραμμένη από
   τον μόνιμο εκπρόσωπο. Το Token Program επεξεργάζεται αυτές τις οδηγίες με τον
   ίδιο τρόπο όπως εάν ο κάτοχος του token account τις είχε υπογράψει.
...
_rs`PermanentDelegate`_. Αυτό είναι το μέγεθος που χρησιμοποιείται στην
_rs`CreateAccount`_.
...
επέκταση _rs`PermanentDelegate`_.
...
## !!steps Αρχικοποίηση PermanentDelegate

Αρχικοποιήστε την επέκταση _rs`PermanentDelegate`_ στο mint.
...
## !!steps Δημιουργία token accounts και έκδοση tokens

Δημιουργήστε token accounts για τον κάτοχο και τον παραλήπτη, στη συνέχεια
εκδώστε tokens στο token account του κατόχου.
...
## !!steps Μεταφορά με τον μόνιμο αντιπρόσωπο

Μεταφέρετε tokens με _rs`TransferChecked`_ υπογεγραμμένο από τον μόνιμο
αντιπρόσωπο.
...
<Callout type="info" title="Σειρά Εντολών">
  Η _rs`InitializePermanentDelegate`_ πρέπει να προηγείται της
  _rs`InitializeMint`_. Οι _rs`CreateAccount`_,
  _rs`InitializePermanentDelegate`_ και _rs`InitializeMint`_ πρέπει να
...
| ------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermanentDelegate`_                     | Επέκταση mint που αποθηκεύει τον εξουσιοδοτημένο αντιπρόσωπο σε κάθε token account για το mint.                                        | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/permanent_delegate.rs#L15-L21) |
| _rs`InitializePermanentDelegate`_           | Εντολή που αρχικοποιεί τον μόνιμο αντιπρόσωπο πριν από την _rs`InitializeMint`_.                                                       | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1952-L1967)              |
| _rs`SetAuthority`_                          | Βασική εντολή token που χρησιμοποιείται με την _rs`AuthorityType::PermanentDelegate`_ για την εναλλαγή του αντιπροσώπου.               | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1488-L1520)              |
| _rs`AuthorityType::PermanentDelegate`_      | Διακριτικό εξουσιοδότησης που χρησιμοποιείται με την _rs`SetAuthority`_ για την εναλλαγή του μόνιμου αντιπροσώπου σε ένα mint.         | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1138-L1139)              |
| _rs`process_initialize_permanent_delegate`_ | Λογική επεξεργαστή που γράφει την επέκταση μόνιμου αντιπροσώπου στο mint.                                                              | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1542-L1557)                  |
| _rs`process_set_authority`_                 | Λογική επεξεργαστή που επικυρώνει και εναλλάσσει τον μόνιμο αντιπρόσωπο όταν χρησιμοποιείται η _rs`AuthorityType::PermanentDelegate`_. | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L849-L860)                    |

### Typescript

Το παράδειγμα `Kit` παρακάτω χρησιμοποιεί απευθείας τις δημιουργημένες εντολές.
Παλαιότερα παραδείγματα που χρησιμοποιούν τις `@solana/web3.js` και
