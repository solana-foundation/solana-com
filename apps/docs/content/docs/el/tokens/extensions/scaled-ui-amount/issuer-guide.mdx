...
ορίσετε το πεδίο `scaled_ui_amount_extension` σε `true` στο account `Mint`.
Ακολουθεί ένα παράδειγμα για το πώς να δημιουργήσετε ένα token με ενεργοποιημένη
την επέκταση Scaled UI Amount χρησιμοποιώντας το `spl-token` CLI:
...
Για να ενημερώσετε τον πολλαπλασιαστή ποσού UI, πρέπει να χρησιμοποιήσετε την
εντολή `update-ui-amount-multiplier`. Μια χρονική σήμανση, σε δευτερόλεπτα από
το Unix epoch, είναι προαιρετική και μπορεί να χρησιμοποιηθεί για να ορίσετε μια
προσαρμοσμένη ώρα έναρξης για τον νέο πολλαπλασιαστή. Εάν δεν παρέχεται χρονική
σήμανση, θα χρησιμοποιηθεί η τρέχουσα χρονική σήμανση.
