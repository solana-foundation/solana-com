...
title: Κλείσιμο Mint
description:
  Δημιουργήστε ένα mint του Token Extensions Program με την επέκταση
  MintCloseAuthority, στη συνέχεια καταστρέψτε την εναπομένουσα προσφορά και
  κλείστε το mint.
...
Η επέκταση mint _rs`MintCloseAuthority`_ του Token Extensions Program επιτρέπει
...
- Η συνολική προσφορά πρέπει να είναι μηδέν, επομένως όλα τα tokens σε όλους
  τους λογαριασμούς token πρέπει να καταστραφούν προτού κλείσει ένα mint
...
1. Υπολογίστε το μέγεθος του λογαριασμού mint και το rent που απαιτείται για το
   mint και την επέκταση _rs`MintCloseAuthority`_.
2. Δημιουργήστε τον λογαριασμό mint με _rs`CreateAccount`_, αρχικοποιήστε το
   _rs`MintCloseAuthority`_, και αρχικοποιήστε το mint με _rs`InitializeMint`_.
3. Εάν η συνολική προσφορά είναι μηδέν, ο λογαριασμός mint μπορεί να κλείσει με
   _rs`CloseAccount`_, υπογεγραμμένο από την αρχή κλεισίματος.
...
_rs`MintCloseAuthority`_. Αυτό είναι το μέγεθος που χρησιμοποιείται στο
_rs`CreateAccount`_.
...
επέκταση _rs`MintCloseAuthority`_.
...
## !!steps Αρχικοποίηση MintCloseAuthority

Αρχικοποιήστε την επέκταση _rs`MintCloseAuthority`_ στο mint.
...
<Callout type="info" title="Σειρά Εντολών">
  Το _rs`InitializeMintCloseAuthority`_ πρέπει να προηγείται του
  _rs`InitializeMint`_. Τα _rs`CreateAccount`_,
  _rs`InitializeMintCloseAuthority`_ και _rs`InitializeMint`_ πρέπει να
...
| --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| _rs`MintCloseAuthority`_                      | Επέκταση mint που αποθηκεύει την εξουσιοδότηση που επιτρέπεται να κλείσει ένα mint μόλις η προσφορά του φτάσει το μηδέν.      | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mint_close_authority.rs#L14-L20) |
| _rs`InitializeMintCloseAuthority`_            | Εντολή που αρχικοποιεί την εξουσιοδότηση κλεισίματος mint πριν από το _rs`InitializeMint`_.                                   | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1830-L1843)                |
| _rs`CloseAccount`_                            | Βασική εντολή token που κλείνει τον λογαριασμό mint αφού η συνολική προσφορά φτάσει το μηδέν.                                 | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1582-L1608)                |
| _rs`process_initialize_mint_close_authority`_ | Λογική επεξεργασίας που γράφει την επέκταση _rs`MintCloseAuthority`_ σε ένα μη αρχικοποιημένο mint.                           | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1376-L1389)                    |
| _rs`process_close_account`_                   | Κοινός χειριστής κλεισίματος λογαριασμού που επιβάλλει τον κανόνα μηδενικής προσφοράς για mints με εξουσιοδότηση κλεισίματος. | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1288)                    |
