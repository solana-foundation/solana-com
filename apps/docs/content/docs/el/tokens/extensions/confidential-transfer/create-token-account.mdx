...
προσθέτοντας πρόσθετη κατάσταση στον token account. Αυτή η ενότητα εξηγεί πώς να
δημιουργήσετε έναν token account με αυτήν την επέκταση ενεργοποιημένη.

Το παρακάτω διάγραμμα δείχνει τα βήματα που εμπλέκονται στη δημιουργία ενός
...
### Κατάσταση Confidential Transfer token account
...
- **approved**: Η κατάσταση έγκρισης του λογαριασμού για confidential transfers.
  Εάν η διαμόρφωση `auto_approve_new_accounts` του mint account έχει οριστεί ως
  `true`, όλοι οι token accounts εγκρίνονται αυτόματα για confidential
  transfers.
...
- **pending_balance_lo**: Τα κρυπτογραφημένα χαμηλότερα 16 bits του εκκρεμούς
  υπολοίπου. Το υπόλοιπο χωρίζεται σε υψηλό και χαμηλό τμήμα για αποτελεσματική
  αποκρυπτογράφηση.
...
- **decryptable_available_balance**: Το διαθέσιμο υπόλοιπο κρυπτογραφημένο με
  ...
  αποκρυπτογράφηση από τον κάτοχο του λογαριασμού.
...
- **pending_balance_credit_counter**: Μετρά τις εισερχόμενες εκκρεμείς πιστώσεις
  υπολοίπου από οδηγίες κατάθεσης και μεταφοράς.

- **maximum_pending_balance_credit_counter**: Το όριο μέτρησης εκκρεμών
...
- **expected_pending_balance_credit_counter**: Η τιμή
  ...
  δεδομένων οδηγίας την τελευταία φορά που επεξεργάστηκε η οδηγία
...
αποτραπούν επιθέσεις DoS. Χωρίς αυτόν τον διαχωρισμό, ένας επιτιθέμενος θα
μπορούσε επανειλημμένα να στέλνει tokens σε ένα token account, μπλοκάροντας την
ικανότητα του κατόχου του token account να μεταφέρει tokens. Ο κάτοχος του token
account δεν θα μπορούσε να μεταφέρει tokens επειδή το κρυπτογραφημένο υπόλοιπο
θα άλλαζε μεταξύ της υποβολής της συναλλαγής και της επεξεργασίας της, με
αποτέλεσμα μια αποτυχημένη συναλλαγή.

Όλα τα ποσά καταθέσεων και μεταφορών προστίθενται αρχικά στο εκκρεμές υπόλοιπο.
Οι κάτοχοι token account πρέπει να χρησιμοποιήσουν την οδηγία
...
`pending_balance_hi` επειδή η αποκρυπτογράφηση ElGamal απαιτεί περισσότερο
υπολογισμό για μεγαλύτερους αριθμούς. Μπορείτε να βρείτε την υλοποίηση
...
Όταν καλείτε την εντολή _rs`ApplyPendingBalance`_ για να μετατρέψετε το εκκρεμές
υπόλοιπο σε διαθέσιμο υπόλοιπο:

1. Ο πελάτης αναζητά τα τρέχοντα εκκρεμή και διαθέσιμα υπόλοιπα, κρυπτογραφεί το
   άθροισμα, και παρέχει ένα `decryptable_available_balance` κρυπτογραφημένο
   χρησιμοποιώντας το κλειδί AES του κατόχου του token account.

2. Οι αναμενόμενοι και πραγματικοί μετρητές πίστωσης εκκρεμότητας παρακολουθούν
   τις αλλαγές στην τιμή του μετρητή μεταξύ της δημιουργίας και της επεξεργασίας
   της εντολής _rs`ApplyPendingBalance`_:
   - `expected_pending_balance_credit_counter`: Η τιμή
     `pending_balance_credit_counter` όταν ο πελάτης δημιουργεί την εντολή
     _rs`ApplyPendingBalance`_
   - `actual_pending_balance_credit_counter`: Η τιμή
     `pending_balance_credit_counter` στον token account τη στιγμή που
     επεξεργάζεται η εντολή _rs`ApplyPendingBalance`_

Οι ταιριαστοί αναμενόμενοι/πραγματικοί μετρητές υποδεικνύουν ότι το
`decryptable_available_balance` ταιριάζει με το `available_balance`.

Όταν ανακτάτε την κατάσταση ενός token account για να διαβάσετε το
`decryptable_available_balance`, διαφορετικές τιμές αναμενόμενων/πραγματικών
μετρητών απαιτούν από τον πελάτη να αναζητήσει πρόσφατες εντολές
κατάθεσης/μεταφοράς που ταιριάζουν με τη διαφορά του μετρητή για να υπολογίσει
το σωστό υπόλοιπο.

**Διαδικασία συμφωνίας υπολοίπου**

Όταν οι αναμενόμενοι και πραγματικοί μετρητές εκκρεμούς υπολοίπου διαφέρουν,
ακολουθήστε αυτά τα βήματα για να συμφωνήσετε το
`decryptable_available_balance`:

1. Ξεκινήστε με το `decryptable_available_balance` από τον token account
2. Ανακτήστε τις πιο πρόσφατες συναλλαγές που περιλαμβάνουν εντολές κατάθεσης
   και μεταφοράς μέχρι τη διαφορά του μετρητή (πραγματικός - αναμενόμενος):
   - Προσθέστε δημόσια ποσά από εντολές κατάθεσης
   - Αποκρυπτογραφήστε και προσθέστε κρυπτογραφημένα ποσά προορισμού από εντολές
     μεταφοράς
...
1. **Δημιουργία του token account**: Καλέστε την εντολή
   ...
   για να δημιουργήσετε τον token account.

2. **Επαναδιανομή χώρου λογαριασμού**: Καλέστε την οδηγία
   _rs`TokenInstruction::Reallocate`_ του Token Extension Program για να
   προσθέσετε χώρο για την κατάσταση _rs`ConfidentialTransferAccount`_.

3. **Διαμόρφωση εμπιστευτικών μεταφορών**: Καλέστε την οδηγία
   ...
   του Token Extension Program για να αρχικοποιήσετε την κατάσταση
...
Η οδηγία _rs`ConfigureAccount`_ απαιτεί δημιουργία κλειδιών κρυπτογράφησης και
δεδομένων απόδειξης από την πλευρά του πελάτη που μπορούν να δημιουργηθούν μόνο
από τον κάτοχο του token account.

Η _rs`PubkeyValidityProofData`_ δημιουργεί μια απόδειξη που επαληθεύει ότι ένα
