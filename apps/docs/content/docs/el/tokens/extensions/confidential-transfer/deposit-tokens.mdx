...
Πριν μπορέσουν να μεταφερθούν τα tokens εμπιστευτικά, το δημόσιο υπόλοιπο tokens
πρέπει να μετατραπεί σε εμπιστευτικό υπόλοιπο. Αυτή η μετατροπή γίνεται σε δύο
στάδια:
...
Αυτή η ενότητα εξηγεί το πρώτο στάδιο: κατάθεση δημόσιου υπολοίπου tokens στο
εμπιστευτικό εκκρεμές υπόλοιπο.

Το παρακάτω διάγραμμα δείχνει τα βήματα που απαιτούνται για την κατάθεση tokens
από το δημόσιο υπόλοιπο στο εμπιστευτικό εκκρεμές υπόλοιπο:
...
Το crate `spl_token_client` παρέχει μια μέθοδο `confidential_transfer_deposit`
που δημιουργεί και αποστέλλει μια συναλλαγή με την εντολή `Deposit`, όπως
φαίνεται στο παρακάτω παράδειγμα.

## Παράδειγμα Κώδικα

Το παρακάτω παράδειγμα δείχνει πώς να καταθέσετε δημόσιο υπόλοιπο tokens στο
εμπιστευτικό εκκρεμές υπόλοιπο.
...
τοπικός validator με fork του mainnet, όπως το [Surfpool](https://surfpool.run), το ενεργοποιεί.
Εκτελέστε το παράδειγμα σε ένα από αυτά (ο κώδικας χρησιμοποιεί το devnet) με έναν χρηματοδοτημένο payer,
και αντικαταστήστε τις placeholders διευθύνσεις mint και account με τις δικές σας.
