...
description: Μάθετε πώς να εφαρμόσετε το εκκρεμές υπόλοιπο για να καταστήσετε τα κεφάλαια διαθέσιμα.
---

## Πώς να εφαρμόσετε το εκκρεμές υπόλοιπο στο διαθέσιμο υπόλοιπο

Πριν μεταφερθούν τα token εμπιστευτικά, το δημόσιο υπόλοιπο token πρέπει να
μετατραπεί σε εμπιστευτικό υπόλοιπο. Αυτή η μετατροπή γίνεται σε δύο στάδια:

1. **Εμπιστευτικό Εκκρεμές Υπόλοιπο**: Αρχικά, τα token «κατατίθενται» από το
...
2. **Εμπιστευτικό Διαθέσιμο Υπόλοιπο**: Το εκκρεμές υπόλοιπο στη συνέχεια
   «εφαρμόζεται» στο διαθέσιμο υπόλοιπο, καθιστώντας τα token διαθέσιμα για
...
Όταν τα token «κατατίθενται» από το δημόσιο υπόλοιπο ή όταν μεταφέρονται
εμπιστευτικά από ένα token account σε άλλο, τα token προστίθενται αρχικά στο
εμπιστευτικό εκκρεμές υπόλοιπο. Πριν χρησιμοποιηθούν τα token για εμπιστευτικές
μεταφορές, το εκκρεμές υπόλοιπο πρέπει να «εφαρμοστεί» στο διαθέσιμο υπόλοιπο.
...
Το παρακάτω διάγραμμα δείχνει τα βήματα που απαιτούνται για την εφαρμογή του
...
Για να μετατρέψετε ένα εκκρεμές υπόλοιπο σε διαθέσιμο υπόλοιπο, χρησιμοποιήστε
την εντολή
...
`confidential_transfer_apply_pending_balance` που δημιουργεί και αποστέλλει μια
συναλλαγή με την εντολή `ApplyPendingBalance`, όπως φαίνεται στο παρακάτω
παράδειγμα.
...
στο mainnet και στο devnet. Ένας τυπικός `solana-test-validator` δεν το ενεργοποιεί, αλλά ένας
τοπικός validator με fork του mainnet, όπως το [Surfpool](https://surfpool.run), το κάνει.
Εκτελέστε το παράδειγμα σε ένα από αυτά (ο κώδικας χρησιμοποιεί το devnet) με έναν χρηματοδοτημένο payer,
και αντικαταστήστε τις placeholder διευθύνσεις mint και λογαριασμού με τις δικές σας.
