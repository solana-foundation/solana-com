...
<Callout type="warn">
  ...
  confidential transfers δεν είναι προς το παρόν διαθέσιμη. Ενώ οι έννοιες
  παραμένουν έγκυρες, τα παραδείγματα κώδικα δεν θα εκτελεστούν.
</Callout>

## Τι είναι τα Confidential Transfers;

Τα confidential transfers σας επιτρέπουν να μεταφέρετε tokens μεταξύ token
accounts χωρίς να αποκαλύπτετε το ποσό της μεταφοράς. Αυτό είναι χρήσιμο για
συναλλαγές που διατηρούν το απόρρητο. Μόνο τα ποσά μεταφοράς και τα υπόλοιπα των
tokens είναι ιδιωτικά. Οι διευθύνσεις των token accounts παραμένουν δημόσιες.
...
- [Confidential Balances Cookbook](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Αποσπάσματα κώδικα για το πώς να χρησιμοποιήσετε την επέκταση Confidential
  Transfer
...
στο πρόγραμμα Token Extension που σας επιτρέπει να μεταφέρετε tokens μεταξύ
...
Η βασική ροή των confidential token transfers είναι η εξής:

1. Δημιουργήστε ένα mint account με την επέκταση confidential transfer.
2. Δημιουργήστε token accounts με επέκταση confidential transfer για τον
   αποστολέα και τον παραλήπτη.
3. Κάντε mint tokens στον λογαριασμό του αποστολέα.
4. **Καταθέστε** το δημόσιο υπόλοιπο του αποστολέα στο **confidential pending
   balance**.
5. **Εφαρμόστε** το pending balance του αποστολέα στο **confidential available
   balance**.
6. Μεταφέρετε εμπιστευτικά tokens από τον token account του αποστολέα στον token
   account του παραλήπτη.
7. **Εφαρμόστε** το pending balance του παραλήπτη στο **confidential available
   balance**.
8. **Αποσύρετε** το confidential available balance του παραλήπτη στο **δημόσιο
   υπόλοιπο**.
...
<Cards>
  <Card
    title="Δημιουργία λογαριασμού mint"
    href="/docs/tokens/extensions/confidential-transfer/create-mint"
  >
    Πώς να δημιουργήσετε έναν λογαριασμό mint με την επέκταση εμπιστευτικής
    μεταφοράς
  </Card>
  <Card
  ...
  >
    Πώς να διαμορφώσετε έναν λογαριασμό token με την επέκταση εμπιστευτικής
    μεταφοράς
  </Card>
  <Card
  ...
  >
    Πώς να καταθέσετε tokens στο εμπιστευτικό υπόλοιπο σε εκκρεμότητα
  </Card>
  <Card
    title="Εφαρμογή υπολοίπου σε εκκρεμότητα"
    href="/docs/tokens/extensions/confidential-transfer/apply-pending-balance"
  >
    Πώς να εφαρμόσετε το υπόλοιπο σε εκκρεμότητα στο διαθέσιμο εμπιστευτικό
    υπόλοιπο
  </Card>
  <Card
  ...
  >
    Πώς να μεταφέρετε εμπιστευτικά tokens μεταξύ λογαριασμών token
  </Card>
</Cards>
...
Το παράδειγμα Rust παρακάτω δείχνει τα βήματα για εμπιστευτικές μεταφορές token
που αντιστοιχούν στο προηγούμενο διάγραμμα. Τα βήματα χωρίζονται σε βοηθητικές
functions για σαφήνεια.
...
εντολή. Πρέπει να έχετε εγκατεστημένο το Solana CLI για να χρησιμοποιήσετε τον
τοπικό validator.
...
<Callout type="info">
  Τη στιγμή που γράφτηκε αυτό, οι εμπιστευτικές μεταφορές δεν είναι
...
Η πλήρης λίστα των
[οδηγιών](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
της επέκτασης εμπιστευτικής μεταφοράς είναι οι εξής:

| Οδηγία                              | Περιγραφή                                                                                                                                                                         |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Ρυθμίζει τον λογαριασμό mint για εμπιστευτικές μεταφορές. Αυτή η οδηγία πρέπει να συμπεριληφθεί στην ίδια συναλλαγή με την οδηγία _rs`TokenInstruction::InitializeMint`_.         |
| _rs`UpdateMint`_                    | Ενημερώνει τις ρυθμίσεις εμπιστευτικής μεταφοράς για ένα mint.                                                                                                                    |
| _rs`ConfigureAccount`_              | Ρυθμίζει έναν λογαριασμό token για εμπιστευτικές μεταφορές.                                                                                                                       |
| _rs`ApproveAccount`_                | Εγκρίνει έναν λογαριασμό token για εμπιστευτικές μεταφορές εάν το mint απαιτεί έγκριση για νέους λογαριασμούς token.                                                              |
| _rs`EmptyAccount`_                  | Αδειάζει τα υπόλοιπα σε εκκρεμότητα και διαθέσιμα εμπιστευτικά υπόλοιπα για να επιτρέψει το κλείσιμο ενός λογαριασμού token.                                                      |
| _rs`Deposit`_                       | Μετατρέπει το δημόσιο υπόλοιπο token σε εμπιστευτικό υπόλοιπο σε εκκρεμότητα.                                                                                                     |
| _rs`Withdraw`_                      | Μετατρέπει το διαθέσιμο εμπιστευτικό υπόλοιπο πίσω σε δημόσιο υπόλοιπο.                                                                                                           |
| _rs`Transfer`_                      | Μεταφέρει tokens μεταξύ λογαριασμών token εμπιστευτικά.                                                                                                                           |
| _rs`ApplyPendingBalance`_           | Μετατρέπει το υπόλοιπο σε εκκρεμότητα σε διαθέσιμο υπόλοιπο μετά από καταθέσεις ή μεταφορές.                                                                                      |
| _rs`EnableConfidentialCredits`_     | Επιτρέπει σε έναν λογαριασμό token να λαμβάνει εμπιστευτικές μεταφορές token.                                                                                                     |
...
| _rs`EnableNonConfidentialCredits`_  | Επιτρέπει σε έναν λογαριασμό token να λαμβάνει δημόσιες μεταφορές token.                                                                                                          |
| _rs`DisableNonConfidentialCredits`_ | Αποκλείει κανονικές μεταφορές για να κάνει τον λογαριασμό να λαμβάνει μόνο εμπιστευτικές μεταφορές.                                                                               |
| _rs`TransferWithFee`_               | Μεταφέρει tokens μεταξύ λογαριασμών token εμπιστευτικά με χρέωση.                                                                                                                 |
| _rs`ConfigureAccountWithRegistry`_  | Εναλλακτικός τρόπος διαμόρφωσης λογαριασμών token για εμπιστευτικές μεταφορές χρησιμοποιώντας έναν λογαριασμό _rs`ElGamalRegistry`_ αντί για απόδειξη _rs`VerifyPubkeyValidity`_. |
