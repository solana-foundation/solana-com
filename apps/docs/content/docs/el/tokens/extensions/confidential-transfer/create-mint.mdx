...
<Callout type="warn">
  Το ZK ElGamal Program είναι προσωρινά απενεργοποιημένο στο mainnet και το
...
προσθέτοντας πρόσθετη κατάσταση στο mint account. Αυτή η ενότητα εξηγεί πώς να
...
- **authority**: Ο λογαριασμός που έχει άδεια να αλλάξει τις ρυθμίσεις
  confidential transfer για το mint και να εγκρίνει νέους confidential
...
- **auto_approve_new_accounts**: Όταν οριστεί σε true, οι χρήστες μπορούν να
  δημιουργήσουν token accounts με confidential transfers ενεργοποιημένα από
...
- **auditor_elgamal_pubkey**: Ένας προαιρετικός auditor που μπορεί να
  αποκρυπτογραφήσει τα ποσά μεταφοράς σε confidential συναλλαγές, παρέχοντας
  έναν μηχανισμό συμμόρφωσης διατηρώντας παράλληλα το απόρρητο από το ευρύ
  κοινό.
...
2. **Αρχικοποίηση επέκτασης Confidential Transfer**: Καλέστε την εντολή
   ...
   του Token Extension Program για να διαμορφώσετε την κατάσταση
   _rs`ConfidentialTransferMint`_ για το mint.

3. **Αρχικοποίηση mint**: Καλέστε την εντολή _rs`Instruction::InitializeMint`_
   του Token Extension Program για να αρχικοποιήσετε την τυπική κατάσταση mint.

Ενώ θα μπορούσατε να γράψετε αυτές τις εντολές χειροκίνητα, το crate
`spl_token_client` παρέχει μια μέθοδο `create_mint` που δημιουργεί και στέλνει
μια συναλλαγή με και τις τρεις εντολές σε μία μόνο κλήση συνάρτησης, όπως
επιδεικνύεται στο παρακάτω παράδειγμα.

### Παράδειγμα κώδικα

Ο ακόλουθος κώδικας επιδεικνύει πώς να δημιουργήσετε ένα mint με την επέκταση
...
<Callout type="info">
  Τη στιγμή της συγγραφής, το Confidential Transfers δεν είναι ενεργοποιημένο
  στον προεπιλεγμένο τοπικό validator. Πρέπει να κλωνοποιήσετε το Token
  Extension Program του mainnet για να εκτελέσετε τον κώδικα του παραδείγματος.
</Callout>
