...
description:
  Ρυθμίστε ένα mint του Token Extension Program ώστε οι νέοι λογαριασμοί token
  να ξεκινούν σε μια επιλεγμένη προεπιλεγμένη κατάσταση, όπως παγωμένοι.
...
## Τι Είναι η Προεπιλεγμένη Κατάσταση Λογαριασμού;

Η επέκταση mint _rs`DefaultAccountState`_ του Token Extension Program ορίζει την
αρχική _rs`AccountState`_ για κάθε νέο λογαριασμό token που δημιουργείται για
αυτό το mint.

Αυτό χρησιμοποιείται συνήθως για να κάνει τους νέους λογαριασμούς token να
ξεκινούν παγωμένοι. Οι λογαριασμοί token που ξεκινούν παγωμένοι δεν μπορούν να
χρησιμοποιηθούν μέχρι η αρχή παγώματος του mint να τους ξεπαγώσει.

## Πώς να Δημιουργήσετε ένα Mint με Προεπιλεγμένη Κατάσταση Λογαριασμού

Για να δημιουργήσετε ένα mint με προεπιλεγμένη κατάσταση λογαριασμού:

1. Υπολογίστε το μέγεθος του λογαριασμού mint και το rent που χρειάζεται για το
   mint και την επέκταση _rs`DefaultAccountState`_.
2. Δημιουργήστε τον λογαριασμό mint με _rs`CreateAccount`_, αρχικοποιήστε το
   _rs`DefaultAccountState`_ και αρχικοποιήστε το mint με _rs`InitializeMint`_.
3. Δημιουργήστε λογαριασμούς token για το mint και παρατηρήστε ότι κάθε νέος
   λογαριασμός token ξεκινά στην τρέχουσα προεπιλεγμένη κατάσταση.
4. Χρησιμοποιήστε το _rs`UpdateDefaultAccountState`_ για να αλλάξετε την
   κατάσταση στην οποία θα πρέπει να ξεκινούν οι μελλοντικοί λογαριασμοί token.

<ScrollyCoding>

## !!steps Υπολογισμός μεγέθους λογαριασμού

Υπολογίστε το μέγεθος λογαριασμού mint για το βασικό mint συν την επέκταση
_rs`DefaultAccountState`_. Αυτό είναι το μέγεθος που χρησιμοποιείται στο
_rs`CreateAccount`_.
...
επέκταση _rs`DefaultAccountState`_.
...
## !!steps Αρχικοποίηση DefaultAccountState

Αρχικοποίηση της επέκτασης _rs`DefaultAccountState`_ στο mint.
...
## !!steps Δημιουργία λογαριασμού token

Δημιουργήστε έναν token account για το mint. Επειδή η τρέχουσα προεπιλεγμένη
κατάσταση είναι _rs`Frozen`_, ο νέος token account ξεκινά παγωμένος.
...
## !!steps Ενημέρωση της προεπιλεγμένης κατάστασης λογαριασμού

Χρησιμοποιήστε _rs`UpdateDefaultAccountState`_ για να αλλάξετε την προεπιλεγμένη
κατάσταση ώστε οι μελλοντικοί token accounts να μην ξεκινούν πλέον παγωμένοι.
...
<Callout type="info" title="Σειρά Εντολών">
  Το _rs`DefaultAccountStateInstruction::Initialize`_ πρέπει να προηγείται του
  _rs`InitializeMint`_. Τα _rs`CreateAccount`_,
  _rs`DefaultAccountStateInstruction::Initialize`_ και _rs`InitializeMint`_
  πρέπει να συμπεριληφθούν στην ίδια συναλλαγή.
</Callout>
...
| ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`DefaultAccountState`_                        | Επέκταση mint που αποθηκεύει την κατάσταση που θα κληρονομούν οι νέοι token accounts.                                   | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/mod.rs#L16-L21)         |
| _rs`DefaultAccountStateInstruction::Initialize`_ | Εντολή που αρχικοποιεί την προεπιλεγμένη κατάσταση token account του mint πριν από το _rs`InitializeMint`_.             | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L20-L36) |
| _rs`DefaultAccountStateInstruction::Update`_     | Εντολή που επιτρέπει στην εξουσία πάγωματος να αλλάξει την προεπιλεγμένη κατάσταση για μελλοντικούς token accounts.     | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L37-L53) |
| _rs`process_initialize_default_account_state`_   | Λογική επεξεργαστή που γράφει την αρχική προεπιλεγμένη κατάσταση σε ένα μη αρχικοποιημένο mint.                         | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L29-L40)     |
| _rs`process_update_default_account_state`_       | Λογική επεξεργαστή που επαληθεύει την εξουσία πάγωματος πριν αλλάξει την αποθηκευμένη προεπιλεγμένη κατάσταση του mint. | [Πηγή](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L43-L74)     |

### Typescript

Το παράδειγμα `Kit` παρακάτω χρησιμοποιεί απευθείας τις δημιουργημένες εντολές.
