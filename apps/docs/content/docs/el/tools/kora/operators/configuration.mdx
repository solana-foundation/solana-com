...
> _Σημείωση: Όταν το `fallback_if_unavailable` είναι true, το σύστημα επιτρέπει τις συναλλαγές
> να προχωρούν εάν το Redis είναι προσωρινά μη διαθέσιμο, αποτρέποντας τη διακοπή της υπηρεσίας.
> Ορίζοντας το `max_transactions` σε 0 θα επιτρέψετε απεριόριστες συναλλαγές._
...
| `get_payer_signer`          | Επιστροφή του επόμενου πληρωτή τέλους και της διεύθυνσης πληρωμής        | ✅       | boolean |
...
| ---------------------------- | ------------------------------------------------ |
| `confidential_transfer_mint` | Διαμόρφωση Εμπιστευτικών Υπολοίπων για το mint |
...
| Όνομα Επέκτασης                  | Περιγραφή                                 |
| ------------------------------- | ------------------------------------------- |
| `confidential_transfer_account` | Κατάσταση Εμπιστευτικών Υπολοίπων για τον λογαριασμό |
