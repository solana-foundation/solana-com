---
title: TypeScript
description: Εγκαταστήστε και χρησιμοποιήστε το @solana/keychain σε εφαρμογές TypeScript
---
...
Το Fordefi διαθέτει δύο τρόπους υπογραφής, που επιλέγονται ανάλογα με το αν περνάτε το `chain`. Τα αιτήματα API πιστοποιούνται με υπογραφή αιτήματος ECDSA P-256 — παρέχετε το κλειδί υπογραφής ως `privateKeyPem`, ή διατηρήστε το σε KMS/HSM περνώντας ένα `requestSigner`.

**Εγγενής λειτουργία Solana** (συνιστάται για χρήση on-chain): ορίστε το `chain`, και το Fordefi υπογράφει και μεταδίδει την ίδια τη συναλλαγή. Επειδή το Fordefi ενδέχεται να αντικαταστήσει το blockhash ή τα τέλη πριν από την υπογραφή, ο υπογράφων είναι ένα Kit `TransactionSendingSigner` — χρησιμοποιήστε το με `signAndSendTransactionMessageWithSigners`, όχι με τη ροή partial-signer που φαίνεται παραπάνω.
...
**Λειτουργία Black Box**: παραλείψτε το `chain`. Το Fordefi υπογράφει ανεπεξέργαστα bytes και δεν μεταδίδει· εσείς συναρμολογείτε και υποβάλλετε τη συναλλαγή μόνοι σας. Χρησιμοποιήστε το με ένα θησαυροφυλάκιο black box του Fordefi.
