...
| `utila`      | Φύλαξη Utila MPC               |
| `fordefi`    | Θεματοφυλακή Fordefi MPC             |
...
Για υπογραφή με θεματοφυλακή Fordefi MPC. Τα αιτήματα API πιστοποιούνται με υπογραφή αιτήματος ECDSA
P-256 που αναλύεται από το `private_key_pem`· για να διατηρήσετε αυτό το κλειδί σε
KMS/HSM, υλοποιήστε το `FordefiRequestSigner` και χρησιμοποιήστε
το `Signer::from_fordefi_with_signer`.

Το Fordefi διαθέτει δύο τρόπους υπογραφής. Ορίστε το `chain` για **native λειτουργία Solana**, όπου
το Fordefi μπορεί να αντικαταστήσει το blockhash ή τα τέλη και μεταδίδει το ίδιο την συναλλαγή.
Αφήστε το `chain` ως `None` για **λειτουργία black box**, όπου το Fordefi υπογράφει ακατέργαστα bytes
και εσείς μεταδίδετε την συναλλαγή μόνοι σας.
