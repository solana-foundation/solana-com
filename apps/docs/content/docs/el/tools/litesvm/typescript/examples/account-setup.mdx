...
description: Ρύθμιση δοκιμαστικών λογαριασμών και κατάστασης με το LiteSVM
---

# Παράδειγμα Ρύθμισης Λογαριασμού

Παρουσιάζει τη ρύθμιση κατάστασης δοκιμαστικού λογαριασμού απευθείας, χωρίς
συναλλαγές.
...
Ρύθμιση ενός program data account που ανήκει σε πρόγραμμα:
...
<Callout type="info">
  Η χρήση του `setAccount` είναι ταχύτερη από την εκτέλεση συναλλαγών για τη
  μαζική ρύθμιση λογαριασμών.
</Callout>

## Βασικά Σημεία

1. **Άμεση Κατάσταση**: Το `setAccount` ορίζει την κατάσταση λογαριασμού χωρίς
   συναλλαγές
2. **Εξαίρεση Ενοικίου**: Χρησιμοποιήστε το `minimumBalanceForRentExemption` για
   τον υπολογισμό των απαιτούμενων lamports
3. **Δομή Λογαριασμού**: Δώστε όλα τα απαιτούμενα πεδία: address, data,
   executable, lamports, programAddress, space
4. **Επαλήθευση**: Χρησιμοποιήστε το `getAccount` ή το
   `client.rpc.getAccountInfo` για επαλήθευση
