---
title: Λειτουργίες Token
description:
  Εργασία με λειτουργίες token - κοπή, μεταφορές και διαχείριση λογαριασμών
---

## Δημιουργία Token Mint

Ένα mint είναι το θεμέλιο κάθε SPL token. Ορίζει τις ιδιότητες του token και
ελέγχει ποιος μπορεί να κόψει νέα token.
...
Ένα token account διατηρεί το υπόλοιπο ενός συγκεκριμένου spl-token για έναν
συγκεκριμένο χρήστη. Για να κατανοήσετε τη διαφορά μεταξύ αυτού και ενός ATA,
διαβάστε
...
## Δημιουργία Associated Token Account

Τα Associated Token Accounts (ATAs) είναι ντετερμινιστικές διευθύνσεις για τη
διατήρηση token για έναν συγκεκριμένο χρήστη. Για να κατανοήσετε τη διαφορά
μεταξύ αυτού και ενός ATA, διαβάστε
[αυτή την ενότητα](/docs/tools/litesvm/additional-crates/testing-with-spl-tokens#types-of-token-accounts).
...
## Κοπή Token σε Token Account

Αφού έχετε ένα mint και token accounts, μπορείτε να κόψετε token σε αυτά τα
accounts.
...
## Πραγματοποίηση Μεταφοράς Token

Μεταφέρετε token μεταξύ accounts χρησιμοποιώντας την εντολή Transfer.
...
Τα περισσότερα token χρησιμοποιούν δεκαδικά για την αναπαράσταση κλασματικών
ποσών. Δείτε πώς να τα χειριστείτε σωστά:
