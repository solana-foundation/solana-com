---
title: Βοηθητικά Εργαλεία Δοκιμών
description:
  Βοηθητικά εργαλεία για δημιουργία λογαριασμών, λειτουργίες token, παραγωγή PDA
  και διαχείριση slot
---

## Δημιουργία Χρηματοδοτημένων Λογαριασμών

Η πιο συνηθισμένη ανάγκη κατά τη δοκιμή είναι η δημιουργία λογαριασμών με SOL.
Το trait `TestHelpers` το κάνει αυτό απλό.

### Μεμονωμένος Λογαριασμός
...
Δημιουργήστε token mints και λογαριασμούς χωρίς τον συνηθισμένο boilerplate
κώδικα.
...
<Callout type="info">
  **Χρησιμοποιήστε ATA για τις περισσότερες περιπτώσεις.** Τα Associated Token
  Accounts είναι ντετερμινιστικά και ακολουθούν τις συμβάσεις του Solana.
  Χρησιμοποιήστε regular token accounts μόνο όταν χρειάζεστε πολλαπλούς
  λογαριασμούς για το ίδιο ζεύγος mint/owner.
</Callout>
...
Παράγετε εύκολα Program Derived Addresses για τις δοκιμές σας.
...
<Callout type="info">
  Χρησιμοποιήστε το `derive_pda` όταν χρειάζεστε το bump seed (π.χ., για κλήσεις
  CPI). Χρησιμοποιήστε το `get_pda` όταν χρειάζεστε μόνο τη διεύθυνση.
</Callout>

## Διαχείριση Slot

Ελέγξτε την εξέλιξη του χρόνου για δοκιμή λογικής που εξαρτάται από τον χρόνο.
...
Ακολουθεί ένα ολοκληρωμένο παράδειγμα που χρησιμοποιεί πολλαπλά βοηθητικά
εργαλεία δοκιμών μαζί:
