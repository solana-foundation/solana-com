...
description:
  Μάθετε πώς να επισυνάπτετε αναγνωριστικά τιμολογίων ή σημειώσεις σε μια
  πληρωμή.
---

Το πρόγραμμα memo του Solana σας επιτρέπει να επισυνάπτετε αριθμούς τιμολογίων,
αναγνωριστικά παραγγελιών ή προσαρμοσμένες αναφορές σε οποιαδήποτε πληρωμή.
Αυτές οι σημειώσεις καταγράφονται μόνιμα on-chain και είναι ορατές στα αρχεία
καταγραφής συναλλαγών, διευκολύνοντας την αντιστοίχιση πληρωμών με τα εσωτερικά
σας συστήματα.

## Πώς λειτουργεί

Το πρόγραμμα Memo γράφει κείμενο στα αρχεία καταγραφής της συναλλαγής. Αυτά τα
αρχεία καταγραφής ευρετηριάζονται από explorers και παρόχους RPC, καθιστώντας
τις σημειώσεις αναζητήσιμες για συμφωνία πληρωμών.
...
Η προσθήκη σημείωσης απαιτεί την άμεση κατασκευή οδηγιών, γεγονός που σας δίνει
έλεγχο σχετικά με το τι περιλαμβάνεται στη συναλλαγή.
...
## !!steps Εισαγωγή προγράμματος Memo

Εισαγάγετε `getAddMemoInstruction` από `@solana-program/memo` για να
δημιουργήσετε οδηγίες memo.

<CodePlaceholder title="Πληρωμή με σημείωση" />
...
## !!steps Δημιουργία οδηγίας μεταφοράς

Δημιουργήστε την οδηγία μεταφοράς token καθορίζοντας το ATA προέλευσης, το ATA
προορισμού, την εξουσιοδότηση (υπογράφοντα) και το ποσό σε βασικές μονάδες.
...
## !!steps Δημιουργία οδηγίας Memo

Δημιουργήστε μια οδηγία memo με ένα μήνυμα. Αυτό το μήνυμα θα είναι ορατό στα
αρχεία καταγραφής προγράμματος της συναλλαγής.
...
## !!steps Αποστολή συναλλαγής με Memo

Συνδυάστε τις οδηγίες μεταφοράς και memo σε μία συναλλαγή.
...
## !!steps Προβολή σημειώματος στα αρχεία καταγραφής συναλλαγών

Αφού επιβεβαιωθεί η συναλλαγή, ανακτήστε την για να δείτε το σημείωμα στα αρχεία
καταγραφής. Το σημείωμα εμφανίζεται ως μήνυμα καταγραφής από το πρόγραμμα Memo.

<CodePlaceholder title="Πληρωμή με σημείωμα" />
