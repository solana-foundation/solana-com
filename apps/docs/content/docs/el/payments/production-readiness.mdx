...
Για υπογραφή backend σε περιβάλλον παραγωγής, χρησιμοποιήστε το
[Keychain](/docs/tools/keychain)—μια ενοποιημένη βιβλιοθήκη υπογραφής που
αφαιρεί την πολυπλοκότητα πολλαπλών backends διαχείρισης κλειδιών μέσω ενός
ενιαίου interface: Memory, Vault, Privy, Turnkey, AWS KMS, Fireblocks, GCP KMS,
CDP, Para, Dfns, Crossmint, Openfort και Utila. Αυτό σας επιτρέπει να
αναπτύσσετε τοπικά με κλειδιά στη μνήμη και στη συνέχεια να μεταβαίνετε σε
backends επιπέδου παραγωγής χωρίς να αλλάζετε τον κώδικα της εφαρμογής.
...
frontend όπου μπορούν να εξαχθούν και να γίνουν κατάχρηση. Χρησιμοποιήστε ένα
backend proxy ή μεταβλητές περιβάλλοντος που δεν είναι ενσωματωμένες στον κώδικα
του client.
...
Παρακολουθήστε αυτές τις μετρικές σε περιβάλλον παραγωγής:

| Μετρική                      | Γιατί                           |
| ---------------------------- | ------------------------------- |
| Ποσοστό επιτυχίας συναλλαγών | Έγκαιρος εντοπισμός προβλημάτων |
| Καθυστέρηση επιβεβαίωσης     | Παρακολούθηση υγείας δικτύου    |
| Δαπάνη τέλους προτεραιότητας | Διαχείριση κόστους              |
| Ποσοστό σφαλμάτων RPC        | Υγεία παρόχου                   |

Ρυθμίστε ειδοποιήσεις για:

- Μεταφορές άνω του ορίου από το ταμείο
- Αιχμές στο ποσοστό αποτυχημένων συναλλαγών
- Ασυνήθιστα μοτίβα παραληπτών
- Αυξήσεις στο ποσοστό σφαλμάτων RPC
...
Κάθε token και πρόγραμμα έχει ακριβώς μία σωστή διεύθυνση στο mainnet.
Παραποιημένα tokens που μιμούνται το USDC ή άλλα stablecoins είναι
συνηθισμένα—θα έχουν το ίδιο όνομα και σύμβολο αλλά διαφορετικό mint. Η εφαρμογή
σας θα πρέπει να κωδικοποιεί σταθερά ή να επιτρέπει τις διευθύνσεις mint (βάσει
των απαιτήσεών σας), χωρίς ποτέ να τις δέχεται δυναμικά από μη αξιόπιστες πηγές.

**Διαμόρφωση βάσει περιβάλλοντος:** Το Devnet και το Mainnet χρησιμοποιούν συχνά
εντελώς διαφορετικά token mints. Ρυθμίστε τη διαμόρφωση της εφαρμογής σας ώστε
να φορτώνει τις σωστές διευθύνσεις ανά περιβάλλον — μην κωδικοποιείτε σκληρά
διευθύνσεις mainnet και ξεχνάτε να τις αλλάζετε κατά τη δοκιμή, ή ακόμα
χειρότερα, να αποστέλλετε διευθύνσεις devnet στην παραγωγή.
...
Οι διευθύνσεις προγραμμάτων έχουν επίσης σημασία. Η αποστολή οδηγιών στο λάθος
πρόγραμμα θα αποτύχει — ή ακόμα χειρότερα, θα οδηγήσει σε μη αναστρέψιμη απώλεια
...
Η καταχώριση των σωστών διευθύνσεων σε λίστα επιτρεπόμενων προστατεύει από
πλαστά token, αλλά δεν προστατεύει από την αποστολή στο λάθος _είδος_
λογαριασμού. Μια διεύθυνση παραλήπτη μπορεί να είναι ένα πορτοφόλι, ένα token
account, ένα mint ή ένα πρόγραμμα, και το καθένα απαιτεί διαφορετικό χειρισμό.
Το εγγενές SOL που αποστέλλεται σε οτιδήποτε άλλο εκτός από ένα πορτοφόλι φεύγει
από τον έλεγχο του αποστολέα — χάνεται οριστικά για ένα mint ή πρόγραμμα,
ανακτάται μόνο από τον κάτοχο του λογαριασμού για ένα token account — χωρίς
αποτυχημένη συναλλαγή που να προειδοποιεί τον χρήστη.

<Callout type="warn" title="Επαλήθευση παραληπτών πριν από την αποστολή">
  Ταξινομήστε κάθε διεύθυνση παραλήπτη πριν υπογράψετε μια μεταφορά. Δείτε
  [Επαλήθευση Διεύθυνσης](/docs/payments/send-payments/verify-address) για το
  πλήρες δέντρο αποφάσεων και τον κώδικα αναφοράς που διακρίνει πορτοφόλια,
  token accounts, mints και προγράμματα.
</Callout>

## Λίστα Ελέγχου Πριν την Κυκλοφορία

- [ ] Αποκτήθηκε SOL mainnet για χρεώσεις και rent
- [ ] Διαμορφώθηκε RPC παραγωγής (όχι δημόσιο endpoint)
- [ ] Διαμορφώθηκε εφεδρικό RPC endpoint
- [ ] Υλοποιήθηκαν προτεραιότητες χρεώσεων με δυναμική τιμολόγηση
- [ ] Η λογική επανάληψης διαχειρίζεται τη λήξη blockhash
- [ ] Το επίπεδο επιβεβαίωσης είναι κατάλληλο για την περίπτωση χρήσης
- [ ] Όλα τα κοινά σφάλματα αντιμετωπίζονται ομαλά
- [ ] Διαμορφώθηκε το Gasless (εάν εφαρμόζεται)
- [ ] Επαληθεύτηκαν οι διευθύνσεις token mainnet (όχι devnet mints)
- [ ] Επικύρωση διεύθυνσης παραλήπτη πριν από την αποστολή (wallet έναντι token
      account έναντι mint έναντι program)
- [ ] Όλα τα κλειδιά έχουν δημιουργηθεί αντίγραφα ασφαλείας με ασφάλεια
- [ ] Η διαχείριση κλειδιών έχει ελεγχθεί (κανένα κλειδί στο frontend)
- [ ] Παρακολούθηση συναλλαγών και ειδοποιήσεις ενεργές
- [ ] Δοκιμές φόρτου στον αναμενόμενο όγκο
...
- **Program keypair:** Το πρόγραμμά σας θα έχει διαφορετική διεύθυνση στο
  ...
  program keypair σας σε ασφαλές σημείο (σημειώστε ότι η εκτέλεση `cargo clean`
  θα διαγράψει πιθανώς το program keypair σας).
- **Αρχικοποίηση λογαριασμών:** Εάν το πρόγραμμά σας απαιτεί λογαριασμούς
  ...
  έχουν δημιουργηθεί στο mainnet πριν οι χρήστες αλληλεπιδράσουν με την εφαρμογή
...
- **Buffer accounts:** Τα μεγάλα προγράμματα αναπτύσσονται μέσω buffer accounts.
  Η εντολή `solana program deploy` το χειρίζεται αυτόματα, αλλά να κατανοείτε
  ότι η ανάπτυξη δεν είναι ατομική — εάν διακοπεί, ίσως χρειαστεί να ανακτήσετε
  ή να κλείσετε τα buffer accounts. Δείτε
  [Deploying Programs](https://solana.com/docs/programs/deploying).
- **Upgrade authority:** Αποφασίστε αν το πρόγραμμά σας θα πρέπει να είναι
  αναβαθμίσιμο μετά την κυκλοφορία. Για αμεταβλητότητα, ανακαλέστε την upgrade
  authority μετά την ανάπτυξη. Για ευελιξία, ασφαλίστε το κλειδί upgrade
  authority κατάλληλα.
- **Rent:** Βεβαιωθείτε ότι το πορτοφόλι ανάπτυξής σας έχει αρκετό SOL για να
  καλύψει τα ελάχιστα rent-exempt για όλα τα program accounts.
- **Επαλήθευση:**
  ...
  της Solana αντιστοιχεί στον πηγαίο κώδικα στο αποθετήριό σας

Για πλήρη καθοδήγηση ανάπτυξης προγράμματος, δείτε
