...
| Recurso      | Descrição                     |
| ------------ | ------------------------------- |
| `memory`     | Assinatura por keypair local (padrão) |
| `vault`      | Motor de Trânsito HashiCorp Vault  |
...
| `fordefi`    | Custódia MPC da Fordefi             |
...
Para assinatura com custódia MPC da Fordefi. As requisições de API são autenticadas com uma assinatura de requisição ECDSA
P-256 extraída de `private_key_pem`; para manter essa chave em um
KMS/HSM, implemente `FordefiRequestSigner` e utilize
`Signer::from_fordefi_with_signer`.

A Fordefi possui dois modos de assinatura. Defina `chain` para o **modo Solana nativo**, onde
a Fordefi pode substituir o blockhash ou as taxas e transmite a transação ela mesma.
Deixe `chain` como `None` para o **modo caixa-preta**, onde a Fordefi assina bytes brutos
e você mesmo transmite a transação.
