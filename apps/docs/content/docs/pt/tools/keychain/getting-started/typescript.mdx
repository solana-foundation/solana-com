...
### Usando com um Kit Client (plugin keychainSigner)

`@solana/keychain-kit-plugin` instala um keychain signer diretamente em um
[Kit client](https://github.com/anza-xyz/kit), configurando-o como `payer` e
`identity` para que você pule a configuração manual de `pipe` mostrada acima:
...
O plugin aceita a mesma configuração marcada por backend que
`createKeychainSigner`, e apenas o backend para o qual ele despacha é empacotado
(os pacotes de backend são carregados via `import()` dinâmico).

Use `keychainPayer` ou `keychainIdentity` para definir apenas um papel e
misturar backends em um único client:
...
Já tem um `SolanaSigner`? Todo keychain signer é um Kit `TransactionSigner`
válido, então você pode instalar um existente com
[`@solana/kit-plugin-signer`](https://github.com/anza-xyz/kit-plugins/tree/main/packages/kit-plugin-signer)
em vez disso (`pnpm add @solana/kit-plugin-signer`):
...
O Fordefi tem dois modos de assinatura, selecionados conforme você passa ou não o `chain`. As requisições à API são autenticadas com uma assinatura de requisição ECDSA P-256 — forneça a chave de assinatura como `privateKeyPem`, ou mantenha-a em um KMS/HSM passando um `requestSigner`.

**Modo Solana nativo** (recomendado para uso on-chain): defina `chain`, e o Fordefi assina e transmite a transação por conta própria. Como o Fordefi pode substituir o blockhash ou as taxas antes de assinar, o assinante é um `TransactionSendingSigner` do Kit — use-o com `signAndSendTransactionMessageWithSigners`, e não com o fluxo de assinante parcial mostrado acima.
...
**Modo caixa-preta**: omita `chain`. O Fordefi assina bytes brutos e não transmite a transação; você mesmo monta e envia a transação. Use este modo com um cofre caixa-preta do Fordefi.
...
## Criando Signers Personalizados
...
integrar serviços adicionais de gerenciamento de chaves.
