...
> _Nota: Quando `fallback_if_unavailable` é verdadeiro, o sistema permite que as transações
> prossigam se o Redis estiver temporariamente indisponível, evitando interrupções no serviço.
> Definir `max_transactions` como 0 permitirá transações ilimitadas._
...
| `get_payer_signer`          | Retornar o próximo pagador de taxas e endereço de pagamento        | ✅       | boolean |
...
| ---------------------------- | ------------------------------------------------ |
| `confidential_transfer_mint` | Configuração de Saldos Confidenciais para o mint |
...
| Nome da Extensão                  | Descrição                                 |
| ------------------------------- | ------------------------------------------- |
| `confidential_transfer_account` | Estado de Saldos Confidenciais para a conta |
