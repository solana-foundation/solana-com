---
title: Utilitários de Transação
description:
  Execute transações e analise resultados com tratamento de erros detalhado
---

## Enviando Transações

O trait `TransactionHelpers` fornece métodos convenientes para enviar instruções
e analisar resultados.

### Enviar uma Única Instrução
...
Os métodos `send_instruction` e `send_instructions` retornam um
`TransactionResult` que encapsula os metadados da transação com utilitários
úteis para testes.
...
<Callout type="info">
  `assert_success()` entrará em pânico com os logs completos da transação se a
  transação falhar, facilitando muito a depuração.
</Callout>
...
Para erros de programa personalizados, você pode verificar o código de erro:
...
### Verificar Erro do Anchor

Para programas Anchor, verifique os nomes dos erros:
...
### Verificar Unidades de Computação
...
<Callout type="info">
  Monitorar unidades de computação é útil para otimização. A Solana limita as
  transações a 1,4 milhão de unidades de computação, e um uso mais elevado tem
  custo maior.
</Callout>

## Exemplo Completo

Aqui está um exemplo abrangente demonstrando os auxiliares de transação:
...
O enum `TransactionError` fornece informações de erro estruturadas:

| Variante                  | Descrição                                 |
| ------------------------- | ----------------------------------------- |
| `ExecutionFailed(String)` | Execução da transação falhou com mensagem |
| `BuildError(String)`      | Falha ao construir a transação            |
| `AssertionFailed(String)` | Uma asserção sobre o resultado falhou     |
