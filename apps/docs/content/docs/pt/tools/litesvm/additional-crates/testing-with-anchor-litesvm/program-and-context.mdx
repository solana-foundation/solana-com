---
title: Programa e Contexto
description:
  Configuração de ambientes de teste e construção de instruções com
  AnchorContext e Program
---

## Gerando Tipos de Cliente com declare_program!

Anchor 1.0 introduz `declare_program!`, que lê o IDL do seu programa em tempo de
compilação e gera módulos `client::accounts::*` e `client::args::*` com
segurança de tipos. Esta é a forma recomendada de construir instruções em
testes.
...
<Callout type="info">
  Execute `anchor build` primeiro para gerar `target/idl/my_program.json`.
  `declare_program!` lê este arquivo em tempo de compilação. O nome da macro
  deve corresponder ao nome do crate do seu programa (snake_case).
</Callout>

## Configurando o Ambiente de Teste

O builder `AnchorLiteSVM` fornece uma API fluente para configurar seu ambiente
de teste.
...
### Forma Abreviada para Programa Único
...
<Callout type="info">
  Se você não especificar um pagador, `AnchorLiteSVM` cria e financia um
  automaticamente para você.
</Callout>

## O AnchorContext

`AnchorContext` é a interface principal para interagir com seu ambiente de
teste.
...
### Verificando a Existência de Contas
...
## Construindo Instruções com Program

A struct `Program` fornece uma API fluente para construção de instruções que
espelha o anchor-client. Use os tipos gerados por `declare_program!` para
segurança de tipos.

### Construção Básica de Instruções
...
<Callout type="info">
  O método `.accounts()` aceita qualquer tipo que implemente `ToAccountMetas`, o
  que todas as structs de conta Anchor fazem automaticamente. Da mesma forma,
  `.args()` aceita qualquer tipo que implemente `InstructionData`.
</Callout>
...
### Usando send_and_confirm_transaction
...
Aqui está um exemplo abrangente usando o padrão de escrow com
`declare_program!`:
