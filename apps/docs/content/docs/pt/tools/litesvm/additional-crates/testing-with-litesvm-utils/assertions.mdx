...
title: Asserções
description:
  Verifique estados de contas, saldos e propriedade com auxiliares de asserção
---

## Existência de Conta

Verifique se as contas existem ou foram encerradas.
...
### Verificar se a Conta Foi Encerrada
...
<Callout type="info">
  `assert_account_closed` é aprovado quando uma conta não existe, tem zero
  lamports ou tem dados vazios. Use isso para verificar a limpeza após o
  encerramento de contas.
</Callout>

## Asserções de Saldo

Verifique saldos de SOL e tokens.
...
### Verificar Fornecimento do Mint
...
Verifique a propriedade e as propriedades dos dados da conta.

### Verificar Proprietário da Conta
...
### Verificar Comprimento dos Dados da Conta
...
<Callout type="info">
  Tamanhos comuns de conta: - **Mint Account**: 82 bytes - **token account**:
  165 bytes - **System Account**: 0 bytes (apenas lamports)
</Callout>

## Exemplo Completo

Aqui está um teste abrangente demonstrando múltiplas asserções:
...
## Tratamento de Falhas de Asserção

Quando uma asserção falha, ela entrará em pânico com uma mensagem descritiva:
...
<Callout type="warn">
  As asserções são projetadas para falhar rapidamente com mensagens de erro
  claras. Use-as amplamente em seus testes para detectar problemas cedo.
</Callout>
