...
description: Como escrever testes com tokens SPL usando litesvm-token
---

<Callout>Requer `cargo add --dev litesvm-token spl-token`</Callout>
...
1. **litesvm-token**: Crate auxiliar que simplifica os testes com tokens SPL
2. **Criação de Mint**: Crie um mint com decimais e autoridades especificados
3. **Token Accounts**: Crie token accounts para armazenar tokens dos usuários
   - Para entender a diferença entre este e um ATA, leia
     [esta seção](/docs/tools/litesvm/additional-crates/testing-with-spl-tokens#types-of-token-accounts).
4. **Minting**: Faça mint de tokens para token accounts usando a autoridade de
   mint
5. **Decimais**: Lembre-se de considerar os decimais ao especificar os valores

## Veja Também

- [Testes com Tokens SPL](/docs/tools/litesvm/additional-crates/testing-with-spl-tokens) -
  Guia abrangente para testes com tokens SPL
