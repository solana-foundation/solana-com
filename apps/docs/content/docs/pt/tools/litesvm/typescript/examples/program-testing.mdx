...
Testar um programa Solana personalizado significa carregar seu arquivo `.so` compilado na
SVM, configurar as contas que ele espera e enviar instruções para ele.
O LiteSVM lida com tudo isso em processo — sem necessidade de validator. Recomendamos
gerar clientes de programa com o
[Codama JS Renderer](https://github.com/codama-idl/renderers-js) para que você tenha
acesso a um Kit Plugin compatível.
...
Para testes que exigem programas padrão:
...
1. **Carregamento do Programa**: Use `addProgramFromFile` para carregar programas compilados e
   importe myProgramPlugin() do seu cliente
2. **Configuração**: Habilite sysvars, builtins e precompiles conforme necessário
...
Assim que você tiver esse esqueleto funcionando, poderá construir um conjunto completo de testes
adicionando mais instruções e asserções.
