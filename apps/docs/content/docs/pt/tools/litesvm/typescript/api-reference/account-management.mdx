...
Métodos para gerenciar contas, saldos e definir o estado das contas.
...
Financia uma conta com lamport.
...
Define diretamente o estado de uma conta sem uma transação.
...
| ---------------- | ------------ | ----------------------------- |
| `address`        | `Address`    | Chave pública da conta (pubkey)            |
...
Obtém o saldo SOL de uma conta. Retorna `null` se a conta não existir.
...
Calcula o mínimo de lamport necessários para isenção de rent.
...
Carrega um programa a partir de bytes.
...
Carrega um programa a partir de um caminho de arquivo `.so`.
...
<Callout type="info">
  Usar `setAccount` é mais rápido do que `airdrop` para configuração em massa de contas, pois não executa transações.
</Callout>
