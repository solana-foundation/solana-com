...
description: Teste lógica dependente de tempo com manipulação de relógio do LiteSVM
---

Demonstra o uso de `warpToSlot` para testar lógica de programa dependente de tempo.
...
Exemplo de padrão para testar funcionalidade bloqueada por tempo:
...
Você pode ler o agendamento de epoch para entender a relação slot/epoch no
cluster:
...
<Callout type="warn">
  `warpToSlot` avança o campo `slot` do relógio, mas **não** atualiza automaticamente
  o `epoch` ou o `unixTimestamp`. Use `setClock` para controle total dos campos
  do relógio.
</Callout>
...
1. **Habilitar Sysvars**: Chame `withSysvars()` antes de usar os métodos de relógio
...
4. **Ler Relógio**: Use `getClock()` para ler o slot, epoch e timestamp atuais
