...
description: Configure contas de teste e estado com LiteSVM
---

# Exemplo de Configuração de Conta

Demonstra a configuração do estado de contas de teste diretamente, sem
transações.

## Configuração Básica de Conta
...
Configure um program data account de propriedade do programa:
...
<Callout type="info">
  Usar `setAccount` é mais rápido do que executar transações para a configuração
  de contas em massa.
</Callout>

## Pontos Principais

1. **Estado Direto**: `setAccount` define o estado da conta sem transações
2. **Isenção de Aluguel**: Use `minimumBalanceForRentExemption` para calcular os
   lamports necessários
3. **Estrutura da Conta**: Forneça todos os campos obrigatórios: address, data,
   executable, lamports, programAddress, space
4. **Verificação**: Use `getAccount` ou `client.rpc.getAccountInfo` para
   verificar
