...
Executa uma transação. Retorna metadados em caso de sucesso ou detalhes do erro em caso de falha.
...
<Callout type="warn">
  As propriedades do resultado da transação, como `err()`, `computeUnitsConsumed()`,
  `logs()` e `signature()`, são **funções getter**, não propriedades. Sempre
  as chame com parênteses.
</Callout>
...
<Callout type="info">
  A simulação não altera o estado. Use-a para verificar as transações antes de enviá-las.
</Callout>
...
Obtém o blockhash com informações de validade para construir transações manualmente.
Use o helper RPC, pois ele retorna tanto `blockhash` quanto `lastValidBlockHeight`.
...
<Callout type="info">
  ...
  nunca precisa buscar o blockhash manualmente — `client.sendTransaction()` (e
  o helper `.sendTransaction()` em cadeias de instruções) cuida disso para você.
</Callout>
...
Avança para um novo blockhash. Útil para testar cenários de expiração de blockhash.
