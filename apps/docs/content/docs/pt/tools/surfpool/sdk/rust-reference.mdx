...
description:
  API pública completa para o crate surfpool-sdk — Surfnet, builder, cheatcodes,
  builders, erros e re-exports.
---

O crate `surfpool-sdk` expõe uma interface enxuta centrada na struct `Surfnet`.
Esta página lista todos os itens públicos; para uso narrativo, consulte os guias
a partir de [Visão Geral](/docs/tools/surfpool/sdk/overview).

## Exportações da Raiz do Crate

| Item                  | Tipo       | Notas                                                                  |
| --------------------- | ---------- | ---------------------------------------------------------------------- |
| `Surfnet`             | struct     | Instância do Surfnet em execução — endpoints RPC/WS, payer, cheatcodes |
| `SurfnetBuilder`      | struct     | Builder fluente utilizado por `Surfnet::builder()`                     |
| `Cheatcodes<'_>`      | struct     | Helper de cheatcode emprestado retornado por `Surfnet::cheatcodes()`   |
| `SurfnetError`        | enum       | Todas as variantes de erro retornadas pelo SDK                         |
...
`stop` aguarda até cinco segundos para que o runtime libere suas portas. Falhas
durante `Drop` são silenciosas; falhas durante um `stop` explícito retornam um
...
Cada setter retorna `Self`, permitindo o encadeamento de chamadas. A chamada
terminal é `start().await`.
...
Implemente isto para cheatcodes JSON-RPC personalizados que ainda não foram
encapsulados por um builder tipado.

## Builders

Todos os builders residem em `surfpool_sdk::cheatcodes::builders`.
...
| Variante         | Causa                                                              |
| ---------------- | ------------------------------------------------------------------ |
| `PortAllocation` | Não foi possível vincular a uma porta livre para RPC ou WebSocket. |
| `Startup`        | O runtime falhou ao inicializar.                                   |
| `Runtime`        | A thread do runtime entrou em pânico ou encerrou inesperadamente.  |
| `Cheatcode`      | Uma chamada JSON-RPC de cheatcode retornou um erro.                |
| `Aborted`        | O runtime foi encerrado ou cancelado durante a inicialização.      |

O crate implementa `std::error::Error` e `Display` em `SurfnetError`, portanto
funciona com `?` e `anyhow` nativamente.
