---
title: Integração de Código QR
description: Gere e personalize códigos QR para pagamentos em carteiras móveis
---

Códigos QR são a forma mais comum de conectar aplicações web e carteiras móveis.
O Solana Pay inclui uma função `createQR` integrada alimentada por
`@solana/qr-code-styling` — não são necessários pacotes adicionais de código QR.

## Geração Básica de Código QR

### Gerar e Exibir um Código QR
...
A função `createQR` aceita parâmetros opcionais para tamanho, cor de fundo e cor
de primeiro plano:
...
### Obtenção de Opções de QR para Uso Avançado

Use `createQROptions` para obter o objeto de configuração bruto para mais
controle:
...
## Usando o Cliente Comerciante

O cliente comerciante também expõe métodos de código QR:
...
### QR de Pagamento com Atualizações de Status
...
### Exibição de QR em Terminal POS
...
Em dispositivos móveis, considere fornecer um link direto além do código QR:
