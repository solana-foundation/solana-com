...
fornece implementações de referência para fins de exploração e aprendizado.

O código dos programas ACL está disponível no
[repositório token-acl](https://github.com/solana-foundation/token-acl) e o
Programa ABL Gate está disponível no
[repositório token-acl-gate](https://github.com/solana-foundation/token-acl-gate).

**Importante**: O Programa ABL (Allow Block List) Gate utilizado neste guia é uma
**implementação de referência**. Embora seja auditado e esteja pronto para produção, os emissores
são livres para criar Programas Gate personalizados que se adaptem melhor às suas necessidades
específicas de conformidade. Você está vinculado apenas à especificação Token ACL (sRFC37),
não a este design específico do Programa Gate.
