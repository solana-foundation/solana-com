...
As transações da Solana têm um limite de tamanho (~1232 bytes). Cada instrução de transferência
adiciona ~200 bytes. Para liquidações multipartes muito grandes, considere usar
