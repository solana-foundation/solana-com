...
description:
  Aprenda a construir um programa Solana CRUD (Criar, Ler, Atualizar, Excluir)
  usando Program Derived Addresses (PDAs) e o framework Anchor. Este guia passo
  a passo demonstra como criar, atualizar e excluir contas de mensagens onchain
  usando PDAs, implementar validação de contas e escrever testes. Perfeito para
  desenvolvedores que desejam entender como usar PDAs em programas Solana.
...
O atributo _rs`#[derive(Accounts)]`_ num programa Anchor anota estruturas que
...
1. _rs`user: Signer<'info>`_
   ...
   - Precisa de estado mutável (_rs`#[account(mut)]`_) pois paga pela nova conta
...
A restrição `bump` indica ao Anchor para encontrar e usar automaticamente o bump
...
- Endereço do utilizador (_rs`Pubkey`_): 32 bytes
- Mensagem do utilizador (_rs`String`_): 4 bytes para o comprimento +
  comprimento variável da mensagem
...
2. _rs`message: String`_ - A mensagem do utilizador para armazenamento.
...
2. Inicialização dos dados da conta:
   - Acede ao `message_account` a partir do contexto.
   ...
   - Define o valor `bump` usado para derivar o PDA, obtido de
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - Restrição `realloc` permite redimensionar os dados da conta
...
Você concluiu agora o programa CRUD básico. Implemente o programa executando
...
O código inicial também inclui um ficheiro de teste em `anchor.test.ts`.
...
Adicione o código abaixo dentro de _ts`describe()`_, mas antes das secções
_ts`it()`_.
...
Após preparar os seus testes, execute o arquivo de teste com _shell`test`_ no
terminal do Playground. Este comando executa os testes no programa implementado
na devnet e regista links para o SolanaFM para visualizar os detalhes da
transação.
