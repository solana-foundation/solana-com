...
No ficheiro `lib.rs`, encontrará um programa com as instruções
[`create`](mention:one), [`update`](mention:two) e [`delete`](mention:three)
para adicionar nos próximos passos.
...
O atributo _rs`#[account]`_ num programa Anchor anota estruturas que representam
dados de conta (tipo de dados a armazenar no campo de dados da conta).

### !user

O campo _rs`user`_ contém um _rs`Pubkey`_ que identifica o utilizador que criou
a conta de mensagem.

### !message

O campo _rs`message`_ contém um _rs`String`_ com a mensagem do utilizador.

### !bump

O campo _rs`bump`_ armazena um _rs`u8`_
["bump" seed](/docs/core/pda#canonical-bump) usado para derivar um endereço
derivado de programa (PDA). Armazenar este valor poupa computação ao eliminar a
necessidade de recalculá-lo em instruções posteriores.
...
Neste exemplo, a estrutura _rs`MessageAccount`_ armazena uma mensagem criada por
utilizadores que contém três campos:

- `user` - _rs`Pubkey`_ que identifica o utilizador que criou a conta de
  mensagem.
- `message` - _rs`String`_ que contém a mensagem do utilizador.
- `bump` - _rs`u8`_ que armazena o ["bump" seed](/docs/core/pda#canonical-bump)
  para derivar o endereço derivado de programa (PDA). Armazenar este valor poupa
  computação ao eliminar a necessidade de recalculá-lo em instruções
  posteriores.

Ao criar uma conta, o programa serializa os dados _rs`MessageAccount`_ e
armazena-os no campo de dados da nova conta.

Posteriormente, ao ler da conta, o programa desserializa esses dados de volta
para o tipo de dados _rs`MessageAccount`_. A secção de testes demonstra o
...
Comece por definir as contas necessárias para a instrução atualizando a
estrutura _rs`Create`_ com o seguinte:
...
A restrição _rs`space`_ especifica o número de bytes a atribuir para o campo de
dados da nova conta.
...
2. Restrições opcionais (como _rs`#[account(mut)]`_ ou _rs`#[account(init)]`_)
   que definem requisitos adicionais
...
1. _rs`user: Signer<'info>`_
   - Representa o usuário criando a conta de mensagem
   - Precisa de status mutável (_rs`#[account(mut)]`_) pois paga pela nova conta
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - Restrição `init` cria a conta durante a execução da instrução
   - Restrições `seeds` e `bump` derivam o endereço da conta como um Program
     Derived Address (PDA)
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - Nos bastidores, a restrição `init` chama o System Program para criar uma
     nova conta com o `space` especificado e altera o proprietário para o
     programa atual.
...
- _rs`b"message"`_ - Uma string fixa como o primeiro seed.
- _rs`user.key().as_ref()`_ - A chave pública da conta _rs`user`_ como o segundo
  seed.

A restrição `bump` instrui o Anchor a encontrar e usar automaticamente o bump
seed correto. O Anchor usa `seeds` e `bump` para derivar o PDA.
...
- Discriminador de conta Anchor (identificador): 8 bytes
- Endereço do usuário (_rs`Pubkey`_): 32 bytes
- Mensagem do usuário (_rs`String`_): 4 bytes para o comprimento + comprimento
...
Em seguida, adicione a lógica de negócio para a instrução _rs`create`_
...
2. Inicialização dos dados da conta:
...
Como no passo anterior, primeiro especifique as contas necessárias para a
instrução `update`.
...
1. _rs`user: Signer<'info>`_
   - Representa o usuário que está atualizando a conta de mensagem
...
Note que a restrição _rs`bump = message_account.bump`_ usa o bump seed
...
2. Atualiza os dados da conta:
   - Acessa o `message_account` do contexto.
   - Define o campo `message` para a nova `message` do argumento da função.
...
1. _rs`user: Signer<'info>`_
   - Representa o usuário que fecha a conta de mensagem
...
_rs`close`_ na estrutura _rs`Delete`_ trata do fechamento da conta.
...
Você concluiu o programa CRUD básico. Implemente o programa executando `deploy`
...
O Solana Playground remove alguma configuração padrão onde
[`pg.program`](mention:one) permite acesso a métodos para interagir com o
programa, enquanto [`pg.wallet`](mention:two) dá acesso à sua carteira do
playground.
...
Execute o arquivo de teste executando _shell`test`_ no terminal do Playground
para verificar se ele é executado conforme esperado. Os próximos passos
adicionam os testes reais.
...
Após enviar a transação e criar a conta, o arquivo de teste busca a conta usando
seu endereço (`messagePda`).
...
Depois de enviar a transação e atualizar a conta, o arquivo de teste busca a
conta usando seu endereço (`messagePda`).
...
O método _ts`fetchNullable()`_ recupera dados de conta da rede que podem não
existir.
...
Depois de enviar a transação e fechar a conta, o exemplo tenta buscar a conta
usando seu endereço (`messagePda`) com `fetchNullable`. Este método retorna null
quando a conta não existe mais após o fechamento.
...
Depois de preparar os seus testes, execute o arquivo de teste com _shell`test`_
no terminal do Playground. Este comando executa os testes no programa
implementado na devnet e regista links para o SolanaFM para visualizar os
detalhes da transação.
...
<Callout type="info">
  Note que neste exemplo, se executar o teste novamente, a instrução `create`
  falha porque `messageAccount` já existe como uma conta. Apenas uma conta pode
  existir para um determinado PDA.
</Callout>
