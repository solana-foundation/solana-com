...
- O endereço da conta provém de um PDA derivado usando as sementes
...
Na instrução `delete`, verá como o programa pode "assinar" por este PDA num CPI.
...
(CPI) para invocar a instrução `transfer` do System Program. Isto demonstra como
realizar uma CPI a partir do programa, permitindo a componibilidade dos
programas Solana.

A estrutura _rs`Transfer`_ especifica as contas necessárias para a instrução de
transferência do System Program:

- `from` - A conta do utilizador (origem dos fundos)
...
cliente são construídas, onde especifica o programa, as contas e os dados de
instrução para uma instrução específica a invocar. Quando a instrução `update`
do programa recebe uma invocação, invoca internamente a instrução de
transferência do System Program.
...
Primeiro, atualize a estrutura _rs`Delete`_ para incluir o `vault_account`. Isto
...
conta vault do utilizador durante a instrução de eliminação para transferir
qualquer SOL acumulado de volta para o utilizador.
...
- O montante a transferir (o saldo total da conta vault)
