...
Token Programs contêm toda a lógica de instrução para interagir com tokens na
rede (tanto fungíveis quanto não fungíveis). Todos os tokens na Solana são
efetivamente [contas de dados](/docs/core/accounts/account-types#data-accounts)
pertencentes a um Token Program.
...
identificando quem tem autoridade sobre a token account. Isto difere do
proprietário do programa especificado no tipo base
[Account](/docs/core/accounts/account-structure#account-fields), que é o Token
Program para todas as token accounts.
...
A CLI _sh`spl-token`_ ajuda a experimentar com tokens SPL. Os exemplos abaixo
usam o terminal do [Solana Playground](https://beta.solpg.io/) para executar
comandos CLI diretamente no navegador sem instalação local.

Criar tokens e contas requer SOL para depósitos de rent de conta e taxas de
transação. Para utilizadores do Solana Playground pela primeira vez, crie uma
carteira Playground e execute o comando _sh`solana airdrop`_ no terminal do
Playground. Também pode obter SOL de devnet na
[faucet web](https://faucet.solana.com/) pública.
...
[documentação](https://www.solana-program.com/docs/token) da _sh`spl-token`_.

<Callout type="info">

Os endereços de conta mostrados na saída do seu terminal diferem dos exemplos
abaixo. Use os endereços do seu terminal Playground ao acompanhar. Por exemplo,
o comando _sh`create-token`_ produz um endereço de mint account com a sua
carteira Playground como a autoridade do mint.
...
A instrução _rs`Transfer`_ no Token Program processa transferências de tokens. O
...
Existem duas maneiras de adicionar metadados (nome, símbolo, imagem) aos tokens
na Solana:

1. **[Metaplex Token Metadata](/docs/tokens/metaplex)** - Cria uma conta de
   metadados separada vinculada ao seu mint. Funciona com o Token Program
   original e o Token-2022 e é amplamente suportado por carteiras e mercados.

2. **[Token Extensions Metadata](/docs/tokens/extensions/metadata)** - Armazena
   metadados diretamente na mint account usando o Token-2022. Uma abordagem mais
   recente com suporte integrado. A extensão de metadados está disponível apenas
   para mint accounts criadas com o Token-2022. Para adicionar metadados a mint
   accounts criadas com o Token Program original, use o Metaplex Token Metadata
   Program.

#### Exemplo de SPL Token CLI

O Token Extensions Program permite armazenar metadados diretamente na Mint
Account. Para criar um token com a extensão de metadados:
...
O URI do token vincula-se aos metadados off-chain. Veja um exemplo de formato
JSON
...
Saiba mais na [seção Token Extensions](/docs/tokens/extensions/metadata). Para
detalhes sobre Token Extensions, consulte a
[seção Token Extensions](/docs/tokens/extensions) e a
