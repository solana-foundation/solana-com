...
Note que os dados de cada Token Account incluem um campo `owner` identificando
quem tem autoridade sobre o Token Account. Isso difere do proprietário do
programa especificado no tipo base [Account](/docs/core/accounts#account-type),
que é o Token Program para todos os Token Accounts.
...
A CLI _sh`spl-token`_ ajuda você a experimentar com tokens SPL. Os exemplos
abaixo usam o terminal do [Solana Playground](https://beta.solpg.io/) para
executar comandos CLI diretamente no navegador sem instalação local.

Criar tokens e contas requer SOL para depósitos de rent de conta e taxas de
transação. Para usuários iniciantes do Solana Playground, crie uma carteira
Playground e execute o comando _sh`solana airdrop`_ no terminal do Playground.
Você também pode obter SOL de devnet na [faucet web](https://faucet.solana.com/)
pública.
...
abaixo. Use os endereços do seu terminal do Playground ao acompanhar. Por
exemplo, o comando _sh`create-token`_ gera um endereço de mint account com sua
carteira Playground como a autoridade de mint.
...
O comando _sh`create-account`_ cria um associated token account com o endereço
da sua carteira como o proprietário.
...
mint deve assinar a transação. A instrução cria tokens para uma token account e
aumenta o fornecimento total na mint account. Aqui está um exemplo em Javascript
no [Solana Playground](https://beta.solpg.io/660cea45cffcf4b13384d012).
...
A instrução _rs`Transfer`_ no Token Program gere transferências de tokens. O
