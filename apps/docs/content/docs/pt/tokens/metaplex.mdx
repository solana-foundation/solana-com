...
O Programa de Metadados de Token cria um Endereço Derivado de Programa (PDA)
para cada cunhagem de token. Esta conta de metadados armazena informações
onchain como o nome do token e símbolo, além de um URI apontando para metadados
JSON off-chain (imagens, descrições, etc.).
...
A conta de metadados onchain contém:
