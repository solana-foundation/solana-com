...
impede que os tokens sejam transferidos entre contas de token após a cunhagem.
Uma vez cunhados, os tokens permanecem permanentemente na conta de token. No
entanto, o proprietário da conta de token ainda pode queimar tokens e fechar a
conta de token quando o saldo chegar a zero.
