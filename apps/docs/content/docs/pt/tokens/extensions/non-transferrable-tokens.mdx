...
description:
  Crie um mint cujos tokens podem ser mintados, queimados e encerrados, mas não
  podem ser transferidos entre contas de token.
...
## O Que São Tokens Não Transferíveis?

A extensão de mint _rs`NonTransferable`_ do Token Extensions Program torna cada
token account desse mint não transferível. Após os tokens serem mintados, os
detentores de tokens não podem movê-los para outra token account com
_rs`Transfer`_ ou _rs`TransferChecked`_.

Este padrão é útil para ativos que devem permanecer vinculados a uma carteira.

Tokens não transferíveis ainda podem ser:

- Mintados pela autoridade do mint
- Queimados pelo proprietário da token account ou por um delegado autorizado
- Token accounts podem ser encerradas após o saldo atingir zero

<Callout type="info" title="Extensões de Token Account">
  Quando uma token account é inicializada para um mint com
  _rs`NonTransferable`_, a token account é inicializada com
  _rs`NonTransferableAccount`_ e _rs`ImmutableOwner`_. Quando a token account é
  criada através do [Associated Token
...
## Como Criar um Mint Não Transferível

Para criar um mint não transferível:

1. Calcule o tamanho da mint account e o rent necessário para o mint e a
   extensão _rs`NonTransferable`_.
2. Crie a mint account com _rs`CreateAccount`_, inicialize
   _rs`NonTransferable`_, e inicialize o mint com _rs`InitializeMint`_.
3. Crie token accounts para o mint. _rs`NonTransferableAccount`_ e
   _rs`ImmutableOwner`_ são automaticamente habilitadas para token accounts.
4. _rs`Transfer`_ e _rs`TransferChecked`_ falham com
...
_rs`NonTransferable`_. Este é o tamanho usado em _rs`CreateAccount`_.
...
Inicialize a extensão _rs`NonTransferable`_ no mint.
...
<Callout type="info" title="Ordem das Instruções">
  _rs`InitializeNonTransferableMint`_ deve vir antes de _rs`InitializeMint`_.
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| _rs`NonTransferable`_                          | Extensão de mint que marca tokens do mint como não transferíveis.                                                                                                                                                                                      | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/non_transferable.rs#L8-L13)  |
| _rs`NonTransferableAccount`_                   | Extensão de token account adicionada às token accounts para mints não transferíveis.                                                                                                                                                                   | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/non_transferable.rs#L15-L21) |
| _rs`ImmutableOwner`_                           | Extensão de token account que impede mudanças de propriedade e é necessária para token accounts não transferíveis.                                                                                                                                     | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L8-L16)   |
| _rs`InitializeNonTransferableMint`_            | Instrução que inicializa a extensão não transferível no nível do mint antes de _rs`InitializeMint`_.                                                                                                                                                   | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L642-L653)              |
| _rs`process_initialize_non_transferable_mint`_ | Lógica do processador que inicializa a extensão de mint _rs`NonTransferable`_ em um mint não inicializado.                                                                                                                                             | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1646-L1655)                |
| _rs`get_required_init_account_extensions`_     | Usado para adicionar automaticamente extensões de token account quando as token accounts são inicializadas com base nas extensões habilitadas no mint. Para mints _rs`NonTransferable`_, adiciona _rs`NonTransferableAccount`_ e _rs`ImmutableOwner`_. | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mod.rs#L1304-L1307)          |

### Typescript

O exemplo `Kit` abaixo usa as instruções geradas diretamente. Exemplos legados
usando `@solana/web3.js` estão incluídos para referência.
