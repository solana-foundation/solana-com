...
impede que os tokens sejam transferidos entre token accounts após a cunhagem.
Uma vez cunhados, os tokens permanecem permanentemente na token account. No
entanto, o proprietário da token account ainda pode queimar tokens e fechar a
token account quando o saldo chegar a zero.
