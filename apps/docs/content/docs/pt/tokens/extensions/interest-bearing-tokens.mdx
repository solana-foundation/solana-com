...
permite definir uma taxa de juros diretamente numa mint account. Os juros
compõem-se continuamente com base no timestamp da rede, mas o saldo de tokens
real armazenado em cada token account permanece inalterado. O valor total (saldo
de tokens mais juros acumulados) é apenas um cálculo. Nenhum token novo é
cunhado com esta extensão.
