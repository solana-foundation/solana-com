...
description:
  Exija que transferências recebidas incluam um memo ativando a extensão de
  conta MemoTransfer em uma conta de token do Token Extensions Program.
...
## O Que É Transferência com Memo?

A extensão de conta _rs`MemoTransfer`_ do Token Extensions Program exige uma
instrução de memo imediatamente antes de qualquer transferência recebida para
essa token account. Transferências enviadas dessa token account não exigem um
memo.

A extensão é configurada na token account de destino, não no mint. Uma vez
ativada, transferências sem um memo falham com _rs`TokenError::NoMemo`_ até que
o proprietário da token account desative o requisito.

## Como Criar uma Token Account com MemoTransfer Ativado

Para criar uma token account com _rs`MemoTransfer`_ ativado:

1. Crie e inicialize um mint.
2. Calcule o tamanho da token account e o rent necessário para uma token account
   com _rs`MemoTransfer`_.
3. Crie a token account e ative _rs`MemoTransfer`_ nessa conta.
4. Instruções _rs`Transfer`_ e _rs`TransferChecked`_ para essa token account
   devem incluir uma instrução de memo imediatamente antes da instrução de
   transferência.
...
_rs`MemoTransfer`_. Este é o tamanho usado em _rs`CreateAccount`_.
...
## !!steps Criar e inicializar a conta de token

Crie a conta de token com o espaço e lamports calculados, e então inicialize-a
para o mint.
...
## !!steps Ativar MemoTransfer

Ative _rs`MemoTransfer`_ na conta de token.
...
| ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`MemoTransfer`_                              | Extensão de conta que armazena se as transferências recebidas exigem um memo.                                   | [Origem](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/mod.rs#L17-L22)         |
| _rs`RequiredMemoTransfersInstruction::Enable`_  | Instrução que ativa transferências obrigatórias com memo em uma conta de token.                                 | [Origem](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L50-L71) |
| _rs`RequiredMemoTransfersInstruction::Disable`_ | Instrução que desativa transferências obrigatórias com memo em uma conta de token.                              | [Origem](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L74-L95) |
| _rs`process_toggle_required_memo_transfers`_    | Lógica de processamento que inicializa ou reescreve a extensão _rs`MemoTransfer`_ após verificar a propriedade. | [Origem](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/memo_transfer/processor.rs#L20-L48)     |

### Typescript

O exemplo `Kit` abaixo usa as instruções geradas diretamente. Exemplos legados
usando `@solana/web3.js`, `@solana/spl-token` e `@solana/spl-memo` estão
