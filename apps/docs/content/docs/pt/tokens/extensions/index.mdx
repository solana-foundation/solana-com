...
Cada extensão adiciona um estado específico que geralmente é inicializado durante a criação da cunhagem ou do token account. Ao inicializar qualquer uma das contas, você pode habilitar extensões específicas simultaneamente para diferentes funcionalidades. A maioria das extensões não pode ser adicionada após a inicialização de uma conta. Esta é uma consideração importante ao projetar seu token, pois você precisará planejar com antecedência quais recursos deseja que seu token suporte. As seções abaixo descrevem cada extensão e como integrá-la.
...
Cada extensão adiciona funcionalidade especializada ao incluir estado extra a um
mint ou token account. Todo o estado específico da extensão é armazenado no
campo
...
`tlv_data` (contendo o estado da extensão) de acordo com os tipos de extensão
específicos habilitados para essa conta.
