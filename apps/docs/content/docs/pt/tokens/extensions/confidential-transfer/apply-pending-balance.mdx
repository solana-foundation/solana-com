...
público de tokens deve ser convertido em saldo confidencial. Essa conversão
...
Quando os tokens são "depositados" do saldo público ou quando são transferidos
de forma confidencial de um token account para outro, os tokens são inicialmente
adicionados ao saldo confidencial pendente. Antes que os tokens possam ser
usados para transferências confidenciais, o saldo pendente deve ser "aplicado"
ao saldo disponível.
...
Para converter um saldo pendente em saldo disponível, invoque a instrução
...
com a instrução `ApplyPendingBalance`, conforme demonstrado no exemplo abaixo.

## Código de Exemplo

O exemplo a seguir demonstra como aplicar o saldo confidencial pendente ao saldo
