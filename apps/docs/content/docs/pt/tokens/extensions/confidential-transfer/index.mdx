...
As transferências confidenciais permitem que você transfira tokens entre token
accounts sem revelar o valor da transferência. Isso é útil para transações que
preservam a privacidade. Apenas os valores das transferências e os saldos de
tokens são privados. Os endereços das token accounts permanecem públicos.
...
- [Guia de início rápido](https://www.solana-program.com/docs/confidential-balances#setup) -
  Configuração e comandos básicos da CLI
...
ao programa Token Extension que permite transferir tokens entre contas sem
...
Para mais detalhes sobre as etapas no fluxo de transferência confidencial,
...
<Cards>
  ...
  >
    Como criar uma mint account com a extensão Confidential Transfer
  </Card>
  <Card
    title="Criar token account"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Como configurar uma token account com a extensão Confidential Transfer
  </Card>
  <Card
  ...
  >
    ...
    aprovação, auditores, taxas, mint e burn)
  </Card>
</Cards>
...
O exemplo em Rust abaixo mostra as etapas para transferências confidenciais de
tokens correspondentes ao diagrama anterior. As etapas são separadas em funções
auxiliares para maior clareza.

Para executar o exemplo, inicie um validator local com o Token Extension Program
clonado da mainnet usando o seguinte comando. Você deve ter a Solana CLI
instalada para usar o validator local.
...
<Callout type="info">
  No momento da redação, as transferências confidenciais não estão ativadas no
  validator local padrão. Você deve clonar o Token Extension Program da mainnet
...
| _rs`EmptyAccount`_                  | Esvazia os saldos confidenciais pendentes e disponíveis para permitir o encerramento de uma token account.                                                              |
| _rs`Deposit`_                       | Converte o saldo público de tokens em saldo pendente confidencial.                                                                                                      |
| _rs`Withdraw`_                      | Converte o saldo disponível confidencial de volta para saldo público.                                                                                                   |
| _rs`Transfer`_                      | Transfere tokens entre token accounts confidencialmente.                                                                                                                |
...
| _rs`DisableConfidentialCredits`_    | Bloqueia transferências confidenciais recebidas, mas ainda permite transferências públicas.                                                                             |
| _rs`EnableNonConfidentialCredits`_  | Permite que uma token account receba transferências públicas de tokens.                                                                                                 |
| _rs`DisableNonConfidentialCredits`_ | Bloqueia transferências regulares para fazer a conta receber apenas transferências confidenciais.                                                                       |
| _rs`TransferWithFee`_               | Transfere tokens entre token accounts confidencialmente com uma taxa.                                                                                                   |
