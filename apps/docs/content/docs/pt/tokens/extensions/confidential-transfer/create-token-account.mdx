...
description:
  Aprenda a criar uma token account com Saldos Confidenciais ativados.
---

## Como criar uma token account com Saldos Confidenciais

Os Saldos Confidenciais permitem transferências privadas de tokens adicionando estado extra à
token account. Esta seção explica como criar uma token account com a
extensão necessária ativada.

O diagrama a seguir mostra as etapas envolvidas na criação de uma token account com
os Saldos Confidenciais ativados:
...
### Estado da Token Account com Saldos Confidenciais
...
O _rs`ConfidentialTransferAccount`_ contém vários campos que gerenciam
os Saldos Confidenciais:

- **aprovado**: O status de aprovação da conta para Saldos Confidenciais. Se a
  configuração `auto_approve_new_accounts` do mint account estiver definida como `true`, todas as
  token accounts são automaticamente aprovadas para Saldos Confidenciais.
...
A criação e configuração de uma token account para Saldos Confidenciais utiliza as
seguintes instruções, que cabem todas em uma única transação:
...
4. **Configurar Saldos Confidenciais**: Invoque a instrução
   ...
   do Token Extension Program,
   referenciando a conta de estado de contexto de prova via
   _rs`ProofLocation::ContextStateAccount`_, para inicializar o
   estado _rs`ConfidentialTransferAccount`_.

<Callout type="info">

**Apenas o proprietário da token account pode ativar os Saldos Confidenciais em uma
token account**.

A instrução _rs`ConfigureAccount`_ requer a geração no lado do cliente de
chaves de criptografia e uma prova que só pode ser gerada pelo proprietário da token account.

A prova de validade do pubkey verifica que a chave pública ElGamal da conta é
válida. Ela é gerada com _rs`build_pubkey_validity_proof_data`_, verificada on-chain
pelo programa ZK ElGamal Proof em uma conta de estado de contexto, e então
referenciada a partir de _rs`ConfigureAccount`_ via
_rs`ProofLocation::ContextStateAccount`_, de modo que nenhum byte de prova trafega na
própria instrução do token. Para detalhes de implementação, consulte:
...
O código a seguir cria uma associated token account e a configura para usar
Saldos Confidenciais com um mint compatível existente.

Os Saldos Confidenciais dependem do programa ZK ElGamal Proof, que está ativado
na mainnet e na devnet. Um `solana-test-validator` padrão não o ativa, mas um
validator local com fork da mainnet, como o [Surfpool](https://surfpool.run), sim.
Execute o exemplo em um desses ambientes (o código usa a devnet) com um pagador com saldo,
e substitua o placeholder do mint por um mint criado conforme
