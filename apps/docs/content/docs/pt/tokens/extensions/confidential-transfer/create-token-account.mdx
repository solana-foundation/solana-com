...
O _rs`ConfidentialTransferAccount`_ contém vários campos para gerir
transferências confidenciais:

- **approved**: O estado de aprovação da conta para transferências
  confidenciais. Se a configuração `auto_approve_new_accounts` da conta mint
  estiver definida como `true`, todas as contas de token são automaticamente
...
- **expected_pending_balance_credit_counter**: O valor
  `pending_balance_credit_counter` fornecido pelo cliente através dos dados da
...
As transferências ou depósitos recebidos não afetam o saldo disponível de uma
conta de token.
...
`pending_balance_hi` porque a desencriptação ElGamal requer mais computação para
números maiores. Pode encontrar a implementação da aritmética de texto cifrado
...
1. O cliente consulta os saldos pendentes e disponíveis atuais, encripta a soma,
   e fornece um `decryptable_available_balance` encriptado usando a chave AES do
   proprietário da conta de token.

2. Os contadores de crédito pendente esperado e real acompanham as alterações ao
   ...
   - `actual_pending_balance_credit_counter`: O valor
     `pending_balance_credit_counter` na conta de token no momento em que a
...
`decryptable_available_balance`, valores diferentes dos contadores
esperados/reais exigem que o cliente procure instruções recentes de
depósito/transferência correspondentes à diferença do contador para calcular o
saldo correto.

**Processo de reconciliação de saldo**

Quando os contadores de saldo pendente esperado e real diferem, siga estes
passos para reconciliar o `decryptable_available_balance`:
...
2. Busque as transações mais recentes incluindo instruções de depósito e
   ...
   - Desencripte e adicione valores de texto cifrado de destino das instruções
     de transferência
...
1. **Criar a token account**: Invoque a instrução
...
2. **Realocar espaço da conta**: Invoque a instrução
   _rs`TokenInstruction::Reallocate`_ do Token Extension Program para adicionar
...
3. **Configurar transferências confidenciais**: Invoque a instrução
   ...
   do Token Extension Program para inicializar o estado
...
A instrução _rs`ConfigureAccount`_ requer geração do lado do cliente de chaves
de encriptação e dados de prova que só podem ser gerados pelo proprietário da
token account.
