...
description:
  Aprenda como criar um token mint com Saldos Confidenciais habilitado.
---

## Como criar um mint com Saldos Confidenciais

Os Saldos Confidenciais permitem transferências privadas de tokens adicionando estado extra ao
mint account. Esta seção explica como criar um token mint com a
extensão necessária habilitada.

O diagrama a seguir mostra as etapas envolvidas na criação de um mint com a
extensão de Saldos Confidenciais:
...
### Estado do Mint de Saldos Confidenciais
...
- **authority**: A conta que tem permissão para alterar as configurações de Saldos Confidenciais
  do mint e aprovar novas contas confidenciais caso a aprovação automática
  esteja desabilitada.

- **auto_approve_new_accounts**: Quando definido como true, os usuários podem criar token
  accounts com Saldos Confidenciais habilitado por padrão. Quando false, a
  authority deve aprovar cada novo token account antes que ele possa ser usado para
...
Criar um mint com Saldos Confidenciais habilitado requer três instruções
em uma única transação:

1. **Criar o Mint Account**: Invocar a instrução _rs`CreateAccount`_ do System
   Program para criar o mint account.

2. **Inicializar Saldos Confidenciais**: Invoque a instrução
   ...
   do Token Extension Program para configurar o estado _rs`ConfidentialTransferMint`_ do
   mint.
...
O código a seguir demonstra como criar um mint com Saldos Confidenciais
habilitado.

Os Saldos Confidenciais dependem do programa ZK ElGamal Proof, que está habilitado
na mainnet e na devnet. Um `solana-test-validator` padrão não o habilita, mas um
validator local com fork da mainnet, como o [Surfpool](https://surfpool.run), o faz.
Execute o exemplo em um desses ambientes (o código usa a devnet) com um pagador com saldo suficiente,
e substitua os endereços de mint e conta de exemplo pelos seus próprios.
