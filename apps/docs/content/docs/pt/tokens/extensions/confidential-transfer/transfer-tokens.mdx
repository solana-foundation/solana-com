...
Para transferir tokens confidencialmente de um token account para outro, tanto o
remetente quanto o destinatário devem ter token accounts configurados com o
estado _rs`ConfidentialTransferAccount`_ e aprovados para transferências
confidenciais. O token account do remetente também deve ter um saldo
...
O exemplo a seguir transfere tokens de forma confidencial de uma conta para
outra. Ambas as contas já devem estar configuradas para transferências
confidenciais, e o remetente deve ter um saldo confidencial disponível.
...
(o código usa a devnet) com um pagador com saldo, e substitua os marcadores pelo
seu mint e pelas contas do remetente e destinatário.
