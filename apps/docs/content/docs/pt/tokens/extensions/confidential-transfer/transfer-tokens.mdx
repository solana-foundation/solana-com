...
1. Crie
   ...
   **Prova de igualdade
   ...
   garantindo que o novo saldo disponível da conta de origem seja calculado
   corretamente como `new_balance = current_balance - transfer_amount`.
   ...
   **Prova de validade do texto cifrado
   ...
   adequadamente para todas as três partes (origem, destino e auditor),
   garantindo que o valor da transferência seja criptografado corretamente sob a
...
2. Para cada prova:
   ...
   - Armazene os metadados específicos da prova numa conta de "estado de
     contexto" da prova para usar noutras instruções.
...
4. Feche as contas de estado de contexto de prova para recuperar o SOL usado
   para criá-las.

O diagrama a seguir mostra as etapas envolvidas na transferência de tokens da
conta de token de um remetente para a conta de token de um destinatário.
...
- Fechar as três contas de prova para recuperar o aluguel.
...
<Callout type="info">
  No momento da redação, as transferências confidenciais não estão habilitadas
  no validador local padrão. Você deve clonar o Token Extension Program da
