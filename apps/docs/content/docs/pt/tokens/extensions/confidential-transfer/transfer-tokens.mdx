...
description:
  Aprenda como transferir tokens de forma privada de uma conta de token para outra.
---

## Como transferir tokens confidencialmente de um token account para outro

Para transferir tokens de forma confidencial de um token account para outro, tanto o remetente quanto o destinatário devem ter token accounts configurados com o estado _rs`ConfidentialTransferAccount`_ e aprovados para usar Saldos Confidenciais. O token account do remetente também deve ter um saldo confidencial disponível para transferência.
...
1. Crie
   ...
   **Prova de Igualdade
   ...
   Verifica que o novo texto cifrado do saldo disponível após a transferência
   corresponde ao seu
   ...
   **Prova de Validade do Texto Cifrado
   ...
   Verifica que os textos cifrados do valor de transferência são gerados
   corretamente para todas as três partes (origem, destino e auditor),
   garantindo que o valor da transferência seja corretamente criptografado sob a
   chave pública de cada parte.
   ...
   **Prova de Intervalo
   ...
   Verifica que o novo saldo disponível e o valor de transferência (dividido em
   bits baixos/altos) são todos não negativos e estão dentro de um intervalo
   especificado.
...
2. Para cada prova:
   ...
   - Armazene os metadados específicos da prova em uma conta de "estado de
     contexto" da prova para uso em outras instruções.
...
3. Invoque a instrução
   ...
   fornecendo as contas de estado de contexto das provas.

4. Feche as contas de estado de contexto das provas para recuperar o SOL usado
   para criá-las.

O diagrama a seguir mostra as etapas envolvidas na transferência de tokens do
token account do remetente para o token account do destinatário.
...
Para transferir tokens confidencialmente de um token account para outro, você
deve:

- Gerar uma prova de igualdade, uma prova de validade do texto cifrado e uma
  prova de intervalo no lado do cliente
...
O exemplo em Rust abaixo gera as provas com o crate
`spl-token-confidential-transfer-proof-generation`, verifica cada uma em uma
conta de estado de contexto por meio do programa ZK ElGamal Proof, referencia as
três contas na instrução de transferência e as fecha em seguida. O exemplo em
TypeScript usa o helper `getConfidentialTransferInstructionPlan` do
`@solana-program/token-2022/confidential`, que monta as mesmas contas de prova,
transferência e fechamento como um plano de instrução de múltiplas transações.

## Código de Exemplo

O exemplo a seguir transfere tokens de forma confidencial de uma conta para outra. Ambas as contas já devem estar configuradas para Saldos Confidenciais, e o remetente deve ter um saldo confidencial disponível.

Os Saldos Confidenciais dependem do programa ZK ElGamal Proof, que está habilitado na mainnet e na devnet. Um `solana-test-validator` padrão não o habilita, mas um validator local com fork da mainnet, como o [Surfpool](https://surfpool.run), sim.
Execute o exemplo em um desses ambientes (o código usa a devnet) com um pagador com saldo suficiente, e substitua os marcadores pelo seu mint e pelas contas do remetente e do destinatário.
