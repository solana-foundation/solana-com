...
protege contas de tokens de transferências inesperadas através de invocações de
programas cruzados (CPIs). Quando ativada, esta extensão garante que os tokens
só podem ser transferidos através de chamadas diretas à instrução de
transferência do Token Extensions Program, bloqueando todos os outros programas
de transferir tokens através de CPI.
