...
description:
  Crie um mint do Token Extensions Program com um delegado permanente que pode
  transferir ou queimar tokens de qualquer conta para esse mint.
...
## O Que É um Delegado Permanente?

A extensão de mint _rs`PermanentDelegate`_ do Token Extensions Program atribui
uma autoridade de delegado ao próprio mint.

Um delegado permanente é uma autoridade no nível do mint que pode autorizar
transferências e queimas para qualquer token account daquele mint. Ele age como
um delegado global para todas as token accounts do mint, e os proprietários das
token accounts não podem revogar o delegado permanente das suas contas.

A autoridade do mint também pode rotacionar o delegado permanente posteriormente
com _rs`SetAuthority`_ usando _rs`AuthorityType::PermanentDelegate`_.

## Como Criar e Usar um Delegado Permanente

Para criar e usar um delegado permanente:

1. Calcule o tamanho da mint account e o rent necessário para o mint e a
   extensão _rs`PermanentDelegate`_.
2. Crie a mint account com _rs`CreateAccount`_, inicialize
   _rs`PermanentDelegate`_ e inicialize o mint com _rs`InitializeMint`_.
3. Envie _rs`TransferChecked`_ ou _rs`BurnChecked`_ assinado pelo delegado
   permanente. O Token Program processa essas instruções da mesma forma como se
   o proprietário da token account as tivesse assinado.
...
_rs`PermanentDelegate`_. Este é o tamanho usado em _rs`CreateAccount`_.
...
## !!steps Inicializar PermanentDelegate

Inicialize a extensão _rs`PermanentDelegate`_ no mint.
...
Crie token accounts para o proprietário e destinatário, em seguida cunhe tokens
para a token account do proprietário.
...
## !!steps Transferir com o delegado permanente

Transfira tokens com _rs`TransferChecked`_ assinado pelo delegado permanente.
...
<Callout type="info" title="Ordem das Instruções">
  _rs`InitializePermanentDelegate`_ deve vir antes de _rs`InitializeMint`_.
...
| ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermanentDelegate`_                     | Extensão do mint que armazena o delegado autorizado em todas as token accounts do mint.                                 | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/permanent_delegate.rs#L15-L21) |
| _rs`InitializePermanentDelegate`_           | Instrução que inicializa o delegado permanente antes de _rs`InitializeMint`_.                                           | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1952-L1967)              |
| _rs`SetAuthority`_                          | Instrução base de token usada com _rs`AuthorityType::PermanentDelegate`_ para alternar o delegado.                      | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1488-L1520)              |
| _rs`AuthorityType::PermanentDelegate`_      | Discriminador de autoridade usado com _rs`SetAuthority`_ para alternar o delegado permanente em um mint.                | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1138-L1139)              |
| _rs`process_initialize_permanent_delegate`_ | Lógica do processador que escreve a extensão de delegado permanente no mint.                                            | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1542-L1557)                  |
| _rs`process_set_authority`_                 | Lógica do processador que valida e alterna o delegado permanente quando _rs`AuthorityType::PermanentDelegate`_ é usado. | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L849-L860)                    |
