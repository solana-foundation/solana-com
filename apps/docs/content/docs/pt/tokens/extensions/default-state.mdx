...
title: Estado padrão
description:
  Configure um mint do Token Extensions Program para que as contas de token
  recém-criadas comecem em um estado padrão escolhido, como congelado.
...
## O Que É o Estado Padrão de Conta?

A extensão de mint _rs`DefaultAccountState`_ do Token Extensions Program define
o _rs`AccountState`_ inicial para cada nova token account criada para esse mint.

Isso é usado mais comumente para fazer com que novas contas de token iniciem
congeladas. Contas de token que iniciam congeladas não podem ser usadas até que
a autoridade de congelamento do mint as descongele.

## Como Criar um Mint com um Estado Padrão de Conta

Para criar um mint com um estado padrão de conta:

1. Calcule o tamanho da mint account e o rent necessário para o mint e a
   extensão _rs`DefaultAccountState`_.
2. Crie a mint account com _rs`CreateAccount`_, inicialize
   _rs`DefaultAccountState`_, e inicialize o mint com _rs`InitializeMint`_.
3. Crie token accounts para o mint e observe que cada nova token account inicia
   no estado padrão atual.
4. Use _rs`UpdateDefaultAccountState`_ para alterar o estado em que as futuras
   token accounts devem iniciar.
...
_rs`DefaultAccountState`_. Este é o tamanho usado em _rs`CreateAccount`_.
...
Crie a mint account com o espaço e os lamports calculados.
...
Inicialize a extensão _rs`DefaultAccountState`_ no mint.
...
## !!steps Criar uma token account

Crie uma token account para o mint. Como o estado padrão atual é _rs`Frozen`_, a
nova token account inicia congelada.
...
## !!steps Atualizar o estado padrão da conta

Use _rs`UpdateDefaultAccountState`_ para alterar o estado padrão de modo que as
futuras token accounts não iniciem mais congeladas.
...
<Callout type="info" title="Ordem das Instruções">
  _rs`DefaultAccountStateInstruction::Initialize`_ deve vir antes de
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`DefaultAccountStateInstruction::Initialize`_ e _rs`InitializeMint`_ devem
...
| ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`DefaultAccountState`_                        | Extensão de mint que armazena o estado que as novas token accounts devem herdar.                                     | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/mod.rs#L16-L21)         |
| _rs`DefaultAccountStateInstruction::Initialize`_ | Instrução que inicializa o estado padrão da token account do mint antes de _rs`InitializeMint`_.                     | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L20-L36) |
| _rs`DefaultAccountStateInstruction::Update`_     | Instrução que permite à autoridade de congelamento alterar o estado padrão para futuras token accounts.              | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L37-L53) |
| _rs`process_initialize_default_account_state`_   | Lógica do processador que grava o estado padrão inicial em um mint não inicializado.                                 | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L29-L40)     |
| _rs`process_update_default_account_state`_       | Lógica do processador que verifica a autoridade de congelamento antes de alterar o estado padrão armazenado do mint. | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L43-L74)     |
