...
description:
  Crie uma cunhagem que armazene metadados diretamente na conta de cunhagem usando as extensões MetadataPointer e TokenMetadata, e depois atualize, remova e bloqueie esses metadados através da interface token-metadata.
...
O exemplo `Kit` abaixo usa as instruções geradas diretamente. Exemplos legados usando `@solana/web3.js`, `@solana/spl-token` e
`@solana/spl-token-metadata` são incluídos para referência. Os exemplos legados limitam
`maxSupportedTransactionVersion` a `0` porque o web3.js v1 não consegue desserializar
