...
fecha permanentemente uma token account e transfere todo o SOL restante (rent)
para uma conta de destino especificada. O saldo da token account deve ser zero
antes de fechar. Apenas o proprietário da token account ou a autoridade de
encerramento designada podem executar esta instrução.
