...
description:
  Fechar uma conta de token e retornar os lamports da conta para uma conta de destino.
...
## O Que Significa Fechar uma Conta de Token?

Fechar um token account exclui a conta e transfere todos os seus lamports,
incluindo o saldo mínimo de armazenamento reembolsável, para uma conta de destino.

O saldo de tokens deve ser zero antes de fechar a conta. Um conta congelada
ainda pode ser fechada se o saldo de tokens do associated token account congelado for zero. Os
token accounts de SOL encapsulado são a exceção e podem ser fechados com saldo de tokens para
recuperar o SOL subjacente. O proprietário da conta ou a autoridade de fechamento da conta
assina a instrução de fechamento e seleciona o destino. A conta que pagou
para criar um ATA não recebe automaticamente o saldo recuperado.

## Como Fechar uma Conta de Token

Fechar uma conta de token usa a instrução _rs`CloseAccount`_ do Token Program.

A instrução _rs`CloseAccount`_ transfere os lamports da conta de origem para uma
conta de destino, limpa a conta de origem e exclui a conta de origem. O Token
Extension Program aplica verificações adicionais de fechamento para certas
extensões, mas o fluxo básico de fechamento é o mesmo.

### Referência da Fonte

| Item                        | Descrição                                                                                                     | Token Program                                                                                                  | Token Extension Program                                                                                              |
| --------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | O estado da conta de token armazena o saldo, status nativo e autoridade de fechamento usados durante o fechamento da conta.     | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | Uma instrução que fecha uma conta de token e transfere os lamports da conta de token para uma conta de destino. | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | Lógica de processador compartilhada para fechamento de conta de token.                                                               | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |
