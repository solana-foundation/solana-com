...
description:
  Fechar uma conta de token e retornar os lamports da conta para uma conta de
  destino.
...
Fechar uma conta de token exclui a conta e retorna os lamports de rent da conta
de token para uma conta de destino.

O saldo de tokens deve ser zero antes que a conta possa ser fechada. Uma conta
congelada ainda pode ser fechada se o saldo de tokens da conta congelada for
zero. As contas de token de SOL encapsulado são a exceção e podem ser fechadas
com saldo de tokens para recuperar o SOL subjacente. O proprietário da conta ou
a autoridade de fechamento da conta assina a instrução de fechamento.
