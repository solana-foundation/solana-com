...
description: Aprenda como criar token accounts SPL.
---

## O que é uma token account?

Uma token account armazena o seu saldo de um token específico. Cada token
account está associada a exatamente um mint e rastreia o seu saldo de tokens e
detalhes adicionais.
...
<Callout type="info">
  Note que no código-fonte, os programadores chamam uma token account de tipo
  `Account`. Tanto o [Token
  ...
  partilham a mesma implementação base para a token account.
</Callout>

Para deter tokens, precisa de uma token account para esse mint específico. Cada
token account rastreia:

1. **Mint**: O tipo específico de token que detém
2. **Owner**: A autoridade que pode transferir tokens desta conta

### Exemplo: USD Coin (USDC)

- Endereço do mint: `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Token account da Circle: `3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
- Esta conta detém apenas tokens USDC
- A Circle controla o token através da autoridade owner da token account:
  `7VHUFJHWu2CuExkJcJrzhQPJ2oygupTWkL2A2For4BmE`

Pode visualizar os detalhes desta token account no
[Solana Explorer](https://explorer.solana.com/address/3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa).

<Callout type="info">
  **Compreender a terminologia "Owner"**

O termo "owner" pode ser usado em dois contextos diferentes:

1. **Token Account Owner**: A autoridade que pode transferir, queimar ou delegar
   tokens. Isto é armazenado nos dados da token account.

2. **Program Owner**: O programa que detém a conta (sempre o Token Program ou
   Token Extension Program para token accounts).

Ao trabalhar com token accounts, "owner" normalmente refere-se à autoridade que
pode transferir tokens, não ao programa que detém a conta.
...
armazenar um token específico. Ela usa um endereço determinístico (PDA) criado
pelo
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program).

<Callout type="info">
  Apenas token accounts criadas pelo Associated Token Program são chamadas de
  "associated token accounts".
</Callout>

ATAs usam endereços determinísticos, facilitando encontrar a token account de
qualquer carteira para um determinado mint. Veja a
...
Isso elimina a necessidade de rastrear endereços de token accounts
manualmente—você sempre pode derivar o endereço correto.
