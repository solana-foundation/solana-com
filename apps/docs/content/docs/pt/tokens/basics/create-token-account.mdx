...
description:
  Criar contas de token e contas de token associadas para um mint e proprietário
  específicos.
...
Uma conta de token armazena tokens para um mint e um proprietário de conta de
token na Solana.
...
tokens da conta de token. O proprietário do programa da conta continua sendo o
Token Program ou Token Extensions Program.

Cada conta de token está vinculada a exatamente um mint, o que significa que a
conta de token pode conter unidades de apenas um token, o token identificado
pelo campo _rs`mint`_ da conta de token.

## O Que É uma Conta de Token Associada

Uma associated token account (ATA) é a conta de token padrão para uma carteira e
mint. O
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program/src)
deriva o endereço da ATA a partir do endereço da carteira, endereço do programa
de token e endereço do mint.
...
de propriedade do Token Program ou Token Extensions Program, não do Associated
Token Program.

O Associated Token Program deriva o endereço da ATA conforme mostrado em
...
Para qualquer combinação de carteira, programa de token e mint, existe
exatamente um endereço de ATA. O Associated Token Program cria uma conta de
token padrão nesse endereço, e a conta resultante ainda utiliza o tipo
_rs`Account`_ definido pelo Token Program ou Token Extensions Program.
...
como uma conta de token de propriedade do Token Program ou Token Extensions
Program.
...
<Callout type="info" title="Recomendado">
  ...
  Program e Token Program. Uma associated token account utiliza um endereço
  determinístico derivado do proprietário, programa de token e mint, o que
  facilita às carteiras e aplicações encontrarem a conta de token padrão para um
  determinado mint.
</Callout>
...
| --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| _rs`Create`_                                  | Cria uma associated token account no endereço ATA derivado.                                                                                                                                                                                                                       | [Fonte](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L20-L29) |
| _rs`CreateIdempotent`_                        | Cria a associated token account, mas ainda assim tem sucesso se essa ATA já existir para o mesmo proprietário e mint.                                                                                                                                                             | [Fonte](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | Deriva o endereço da ATA e utiliza _rs`create_pda_account`_ mais CPIs para o programa de token selecionado para inicializar a token account. Quando o programa selecionado é o Token Extensions Program, o processador ATA também inicializa a extensão de proprietário imutável. | [Fonte](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
...
Criar uma token account requer duas instruções:

1. A instrução _rs`CreateAccount`_ do System Program cria uma nova conta isenta
   de rent e atribui o Token Program como proprietário do programa da nova
   conta.
...
não está já inicializada e se está isenta de rent.
...
<Callout type="info">
  ...
  use o Associated Token Program em vez disso. Use chamadas diretas ao System
  Program e Token Program quando tiver um motivo específico para não usar uma
  associated token account ou quando precisar criar contas de token PDA
  personalizadas fazendo CPIs para as instruções do System Program e Token
  Program a partir do seu próprio programa Solana.
</Callout>
