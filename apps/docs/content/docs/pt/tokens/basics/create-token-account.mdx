...
description:
  Crie contas de token e contas de token associadas para um mint e proprietário
  específicos.
...
Uma token account armazena tokens para um mint e um proprietário de token
account na Solana.
...
tokens da token account. O proprietário do programa da conta ainda é o Token
Program ou Token Extensions Program.

Cada token account está vinculada a exatamente um mint, o que significa que a
token account pode conter unidades de apenas um token, o token identificado pelo
campo _rs`mint`_ da token account.

## O Que É uma Conta de Token Associada

Uma associated token account (ATA) é a token account padrão para uma carteira e
mint. O
...
O Associated Token Program é uma forma de criar uma token account em um endereço
padrão e determinístico. A conta resultante ainda é uma token account
...
exatamente um endereço de ATA. O Associated Token Program cria uma token account
...
A criação de uma conta de token associada usa a instrução _rs`Create`_ ou
...
como uma token account pertencente ao Token Program ou Token Extension Program.
...
<Callout type="info" title="Recomendado">
  ...
  Program e Token Program. Uma associated token account usa um endereço
  determinístico derivado do proprietário, programa de token e mint, o que torna
  a conta de token padrão para um determinado mint mais fácil de encontrar para
  carteiras e aplicações.
</Callout>
...
| _rs`process_create_associated_token_account`_ | Deriva o endereço da ATA e usa _rs`create_pda_account`_ mais CPIs para o programa de token selecionado para inicializar a token account. Quando o programa selecionado é o Token Extension Program, o processador da ATA também inicializa a extensão de proprietário imutável. | [Fonte](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | Auxiliar usado por _rs`process_create_associated_token_account`_ para criar a conta PDA por CPI no System Program.                                                                                                                                                              | [Fonte](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |
...
Criar uma conta de token requer duas instruções:
...
Inclua a instrução _rs`CreateAccount`_ e a instrução de inicialização da conta
de token na mesma transação.

Durante a inicialização da conta de token, o Token Program verifica se a conta
não está já inicializada e está isenta de rent.

<Callout type="info">
  A seção abaixo mostra como criar uma conta de token chamando diretamente as
  ...
  Program quando tiver uma razão específica para não usar uma associated token
...
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_                     | Os campos base da conta de token armazenados em cada conta de token.                                                                  | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`InitializeAccount`_           | Uma instrução de inicialização de conta de token que espera o proprietário e a conta sysvar de rent na sua lista de contas.           | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L45-L63)   | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L75-L93)   |
| _rs`InitializeAccount2`_          | Uma instrução de inicialização de conta de token que passa o proprietário nos dados da instrução em vez da lista de contas.           | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L352-L365) | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L419-L433) |
| _rs`InitializeAccount3`_          | Uma instrução de inicialização de conta de token que passa o proprietário nos dados da instrução e não requer a conta sysvar de rent. | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L377-L387) | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L445-L456) |
| _rs`_process_initialize_account`_ | Lógica compartilhada do processador para inicialização de conta de token.                                                             | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L83-L141)      | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L153-L234)     |
