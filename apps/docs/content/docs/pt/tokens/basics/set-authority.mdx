...
description:
  Altere ou revogue funções de autoridade em uma conta de mint ou token account,
  incluindo autoridade de mint, congelamento, proprietário e fechamento.
...
## O Que Faz a Configuração de Autoridade?

Configurar autoridade altera ou revoga funções específicas de autoridade em uma
conta de mint ou token account.

Contas de mint podem armazenar uma autoridade de mint e uma autoridade de
congelamento. Token accounts armazenam um proprietário e podem opcionalmente
armazenar uma autoridade de fechamento. Cada função controla um conjunto
específico de permissões, e configurar uma autoridade como _rs`None`_ remove
permanentemente a função de autoridade selecionada.

O Token Extensions Program também define tipos adicionais de autoridade para
extensões específicas.

## Como Configurar Autoridade

Alterar uma autoridade usa a instrução _rs`SetAuthority`_ do Token Program.

Cada chamada atualiza um tipo de autoridade em uma conta de mint ou token
account. A autoridade atual assina a mudança, e a nova autoridade pode ser outro
endereço ou _rs`None`_ para revogar permanentemente a autoridade selecionada.

### Referência de Código

| Item                        | Descrição                                                                                | Token Program                                                                                                  | Token Extensions Program                                                                                               |
| --------------------------- | ---------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                  | O estado do mint armazena _rs`mint_authority`_ e _rs`freeze_authority`_.                 | [Código](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Código](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)           |
| _rs`Account`_               | O estado da token account armazena _rs`owner`_ e _rs`close_authority`_.                  | [Código](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Código](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)          |
| _rs`AuthorityType`_         | As funções de autoridade que _rs`SetAuthority`_ pode atualizar.                          | [Código](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L739-L751) | [Código](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1116-L1157) |
| _rs`SetAuthority`_          | Uma instrução que altera ou revoga uma autoridade em uma conta de mint ou token account. | [Código](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L141-L158) | [Código](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L183-L200)   |
| _rs`process_set_authority`_ | Lógica compartilhada do processador para atualizações de autoridade.                     | [Código](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L423-L518)     | [Código](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L677-L978)       |

### Typescript

Os exemplos `Kit` abaixo mostram a abordagem recomendada usando `@solana/kit`.
Exemplos legados usando `@solana/web3.js` estão incluídos como referência.
