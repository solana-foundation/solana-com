...
description:
  Queime tokens de uma conta de token e reduza a oferta total da mint.
...
oferta total da mint na mesma quantidade.

## Como Queimar Tokens

Queimar tokens utiliza a instrução _rs`Burn`_ ou _rs`BurnChecked`_ do Token
Program.

Os exemplos abaixo usam _rs`BurnChecked`_, que requer que o chamador forneça os
decimais da mint para que a instrução possa verificar a precisão esperada do
token antes de queimar.

A [native mint](/docs/tokens/basics/sync-native) não suporta queima. Use
_rs`CloseAccount`_ em vez disso.

O proprietário da token account ou um delegado aprovado assina a transação. No
Token Extensions Program, um delegado permanente da mint também pode autorizar a
queima se a mint tiver a extensão de delegado permanente habilitada.

### Referência de Código

| Item               | Descrição                                                                                                                                                              | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_         | O estado da mint armazena a oferta total de tokens.                                                                                                                    | [Código](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Código](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | O estado da token account armazena o saldo de tokens reduzido por uma queima.                                                                                          | [Código](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Código](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Uma instrução de queima que diminui o saldo de uma token account e a oferta da mint sem exigir que o chamador forneça os decimais da mint.                             | [Código](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Código](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Uma instrução de queima que requer que o chamador forneça os decimais da mint e verifica esse valor antes de diminuir o saldo de uma token account e a oferta da mint. | [Código](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Código](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | Lógica compartilhada do processador para queimas de tokens.                                                                                                            | [Código](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [Código](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |
