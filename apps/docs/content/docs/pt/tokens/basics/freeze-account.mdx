...
description:
  Congele uma token account com a autoridade de congelamento do mint para
  bloquear transferências, queimas e alterações de delegação.
...
## O Que Faz o Congelamento de uma Token Account?

Congelar uma token account mantém o mesmo proprietário, mint e saldo, mas impede
a token account de receber, transferir ou queimar tokens até que a token account
seja descongelada.

Uma conta congelada ainda pode ser fechada se a conta congelada tiver saldo zero
de tokens.

Somente um mint que ainda possui uma autoridade de congelamento pode congelar
token accounts. Se a autoridade de congelamento de um mint for revogada, as
contas desse mint não poderão mais ser congeladas.

## Como Congelar uma Token Account

Congelar uma token account usa a instrução _rs`FreezeAccount`_ do Token Program.

A instrução _rs`FreezeAccount`_ verifica se a token account pertence ao mint e
se o signatário é a autoridade de congelamento do mint, e então altera o estado
da token account de inicializada para congelada.

[Token accounts nativas](/docs/tokens/basics/sync-native) não suportam
congelamento.
...
| ----------------------------------- | --------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | O estado do mint armazena a autoridade de congelamento opcional usada para congelar token accounts. | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Os estados da token account, incluindo _rs`Initialized`_ e _rs`Frozen`_.                            | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | Uma instrução que altera uma token account para o estado congelado.                                 | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
| _rs`process_toggle_freeze_account`_ | Lógica de processamento compartilhada para congelar e descongelar token accounts.                   | [Source](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Source](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |
