...
impede todas as transferências de tokens ou queimas de tokens de uma conta de
token específica. Uma vez congelada, a conta não pode enviar tokens nem receber
tokens até ser descongelada. Uma conta congelada ainda pode ser fechada se tiver
um saldo de tokens zero. Apenas a autoridade de congelamento da mint account
pode congelar contas. Se a autoridade de congelamento for revogada (definida
como nula) na mint account, os tokens nunca poderão ser congelados.
