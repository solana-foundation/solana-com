...
Cunhar cria novas unidades de um token usando a instrução
...
- Apenas a autoridade do mint pode cunhar novos tokens
...
async fn main() -> Result<()> {
    // Criar conexão com o validator local
    ...
    // Obter tamanho padrão da mint account (em bytes), sem extensões ativadas
    ...
    // Instrução para inicializar dados da mint account
    ...
    // Quantidade de tokens a cunhar (100 tokens com 2 casas decimais)
    let amount = 100;

    // Criar instrução mint_to para cunhar tokens na associated token account
    let mint_to_instruction = mint_to(
        &token_program_id(),
        &mint.pubkey(),            // mint
        &associated_token_address, // destino
        &fee_payer.pubkey(),       // autoridade
        &[&fee_payer.pubkey()],    // signatário
        amount,                    // quantidade
    )?;

    // Criar transação para cunhar tokens
    let transaction = Transaction::new_signed_with_payer(
        &[mint_to_instruction],
        Some(&fee_payer.pubkey()),
        &[&fee_payer],
    ...
    println!("Cunhados 1.00 tokens na associated token account");
    println!("\nEndereço do Mint: {}", mint.pubkey());
    println!("{:#?}", mint_data);

    println!(
        "\nEndereço da Associated Token Account: {}",
        associated_token_address
    );
    println!("{:#?}", token_data);

    println!("Assinatura da Transação: {}", transaction_signature);
...
async def main():
    ...
    # Exemplos de keypairs e endereços
    payer = Keypair()
    ...
    # Quantidade a cunhar (na menor unidade)
    amount_to_mint = 1000000000  # 1 token com 9 casas decimais

    async with rpc:
        # Criar instrução de cunhagem
        ...
        print(f"Quantidade: {amount_to_mint}")
        print(f"Autoridade de cunhagem: {mint_authority.pubkey()}")
        print(f"Pagador: {payer.pubkey()}")
        print(f"Transação de cunhagem criada com sucesso")
