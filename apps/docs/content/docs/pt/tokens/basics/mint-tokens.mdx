...
title: Cunhar tokens
description:
  Cunhar novas unidades de token para uma token account com a autoridade de
  cunhagem.
...
## O Que a Cunhagem Faz?

Cunhar cria novas unidades de um token existente e aumenta o fornecimento total
da cunhagem.

Apenas a autoridade de cunhagem pode cunhar novos tokens. A token account de
destino já deve existir e deve pertencer à mesma cunhagem.

## Como Cunhar Tokens

A cunhagem de tokens utiliza a instrução _rs`MintTo`_ ou _rs`MintToChecked`_ do
Token Program.

Os exemplos abaixo usam _rs`MintToChecked`_, que requer que o chamador forneça
os decimais da cunhagem para que a instrução possa verificar a precisão esperada
do token antes de cunhar.

A [cunhagem nativa](/docs/tokens/basics/sync-native) não suporta cunhagem.

### Referência do Código-Fonte

| Item                  | Descrição                                                                                                                                                          | Token Program                                                                                                 | Token Extensions Program                                                                                            |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_            | O estado da cunhagem armazena a autoridade de cunhagem e o fornecimento total de tokens.                                                                           | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_         | O estado da token account armazena o saldo de destino atualizado pela cunhagem.                                                                                    | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`MintTo`_          | Uma instrução de cunhagem que cunha tokens para uma token account existente sem exigir que o chamador forneça os decimais da cunhagem.                             | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L159-L177) | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L202-L220) |
| _rs`MintToChecked`_   | Uma instrução de cunhagem que requer que o chamador forneça os decimais da cunhagem e verifica esse valor antes de cunhar tokens para uma token account existente. | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L301-L325) | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L367-L391) |
| _rs`process_mint_to`_ | Lógica compartilhada do processador para cunhar tokens.                                                                                                            | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L521-L583)     | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L981-L1067)    |
