...
description:
  Crie uma mint de token e configure os decimais, a autoridade de cunhagem e a
  autoridade de congelamento da mint.
...
armazena o estado compartilhado que se aplica a todas as token accounts dessa
mint.
...
Cada token possui uma mint account, e o endereço da mint é o identificador
exclusivo do token em carteiras, aplicativos e exploradores.

Por exemplo, o USD Coin (USDC) tem o endereço de mint
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`. O endereço da mint identifica
exclusivamente o USDC em todo o ecossistema Solana. Você pode visualizar essa
mint no
...
Criar uma mint requer duas instruções:
...
2. A instrução _rs`InitializeMint`_ ou _rs`InitializeMint2`_ do Token Program
   inicializa a nova conta como uma mint.

Inclua a instrução _rs`CreateAccount`_ e a instrução de inicialização da mint na
mesma transação.

Durante a inicialização da mint, o Token Program verifica se a mint account
ainda não está inicializada e se está isenta de rent. O Token Program então
registra a autoridade de cunhagem, a autoridade de congelamento, os decimais e o
sinalizador _rs`is_initialized`_ nos dados da mint account.

### Referência de Código-Fonte

| Item                           | Descrição                                                                                            | Token Program                                                                                                 | Token Extensions Program                                                                                            |
| ------------------------------ | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                     | O estado base da mint armazenado em cada mint account.                                               | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
...
| _rs`_process_initialize_mint`_ | Lógica compartilhada do processador para inicialização de mint.                                      | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
...
casas decimais, autoridades). Para adicionar metadados legíveis como nome,
símbolo e imagem ao seu token, você tem duas opções:
...
<Cards>
  ...
  <Card title="Token Extensions Metadata" href="/docs/tokens/extensions/metadata">
   ...
   Para adicionar metadados a mint accounts criadas com o Token Program original, use o Metaplex Token Metadata Program.
  </Card>
</Cards>
