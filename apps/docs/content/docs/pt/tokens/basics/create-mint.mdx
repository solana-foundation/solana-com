...
description:
  Crie um mint de token e configure os decimais, a autoridade de mint e a
  autoridade de congelamento do mint.
...
Uma mint account define e identifica exclusivamente um token na Solana, e
armazena o estado compartilhado que se aplica a todas as token accounts desse
mint.

O Token Program define o tipo de conta _rs`Mint`_ como:
...
Cada token possui uma mint account, e o endereço do mint é o identificador único
do token em carteiras, aplicativos e exploradores.

Por exemplo, o USD Coin (USDC) possui o endereço de mint
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`. O endereço do mint identifica
...
Criar um mint requer duas instruções:
...
1. A instrução _rs`CreateAccount`_ do System Program cria uma nova conta isenta
   ...
   conta.
2. A instrução _rs`InitializeMint`_ ou _rs`InitializeMint2`_ do Token Program
   inicializa a nova conta como um mint.

Inclua a instrução _rs`CreateAccount`_ e a instrução de inicialização do mint na
mesma transação.

Durante a inicialização do mint, o Token Program verifica se a mint account
ainda não foi inicializada e está isenta de rent. O Token Program então grava a
autoridade de mint, a autoridade de congelamento, os decimais e a flag
_rs`is_initialized`_ nos dados da mint account.

### Referência de Código-Fonte

| Item                           | Descrição                                                                                            | Token Program                                                                                                 | Token Extensions Program                                                                                            |
| ------------------------------ | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                     | O estado base do mint armazenado em cada mint account.                                               | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`InitializeMint`_           | Uma instrução de inicialização de mint que espera a conta sysvar de rent em sua lista de contas.     | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L24-L44)   | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L50-L74)   |
| _rs`InitializeMint2`_          | Uma instrução de inicialização de mint que não requer a conta sysvar de rent em sua lista de contas. | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L401-L414) | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L471-L486) |
| _rs`_process_initialize_mint`_ | Lógica de processamento compartilhada para inicialização de mint.                                    | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L27-L60)       | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L87-L130)      |
| _rs`process_initialize_mint`_  | Manipulador público para _rs`InitializeMint`_.                                                       | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L62-L70)       | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L132-L140)     |
| _rs`process_initialize_mint2`_ | Manipulador público para _rs`InitializeMint2`_.                                                      | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L72-L81)       | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L142-L151)     |

### Typescript

Os exemplos abaixo mostram a abordagem recomendada usando `@solana/kit`.
Exemplos legados usando `@solana/web3.js` são incluídos para referência.
...
Uma mint account armazena apenas informações básicas do token (fornecimento,
decimais, autoridades). Para adicionar metadados legíveis por humanos, como
nome, símbolo e imagem ao seu token, você tem duas opções:

<Cards>
  <Card title="Metadados de Token Metaplex" href="/docs/tokens/metaplex">
   Adicione metadados usando o Programa de Metadados de Token Metaplex. Funciona tanto com o Token Program original quanto com o Token-2022.
  </Card>

  <Card title="Metadados de Extensões de Token" href="/docs/tokens/extensions/metadata">
   Use a extensão de metadados integrada com o Token-2022. 
   A extensão de metadados está disponível apenas para mint accounts criadas com o Token-2022.
   Para adicionar metadados a mint accounts criadas com o Token Program original, use o Programa de Metadados de Token Metaplex.
  </Card>
</Cards>
