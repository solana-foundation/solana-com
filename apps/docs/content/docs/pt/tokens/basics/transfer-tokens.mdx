...
description:
  Mova tokens entre token accounts da mesma mint com instruções de transferência
  do Token Program.
...
account da mesma mint.

As transferências não alteram o fornecimento da mint. As transferências apenas
atualizam os saldos entre token accounts.

## Como Transferir Tokens

A transferência de tokens usa a instrução _rs`Transfer`_ ou
_rs`TransferChecked`_ do Token Program.

Os exemplos abaixo usam _rs`TransferChecked`_, que requer que o chamador forneça
...
No Token Extensions Program, _rs`Transfer`_ está descontinuado em favor de
_rs`TransferChecked`_ ou _rs`TransferCheckedWithFee`_.

O proprietário do token account de origem ou um delegado aprovado assina a
transferência. No Token Extensions Program, um delegado permanente da mint
também pode autorizar a transferência se a mint tiver a extensão de delegado
...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_          | O estado do token account armazena os saldos atualizados por uma transferência.                                                                                                                   | [Código-Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Código-Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | Uma instrução de transferência que move tokens entre token accounts sem exigir que o chamador forneça a mint ou os decimais. No Token Extensions Program, use _rs`TransferChecked`_ em vez disso. | [Código-Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Código-Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Uma instrução de transferência que requer que o chamador forneça a mint e os decimais e verifica esses valores antes de mover tokens entre token accounts da mesma mint.                          | [Código-Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Código-Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | Lógica compartilhada do processador para transferências de tokens.                                                                                                                                | [Código-Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [Código-Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |
