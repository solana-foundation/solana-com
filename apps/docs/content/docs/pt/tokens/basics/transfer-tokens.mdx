...
description:
  Mova tokens entre token accounts do mesmo mint com instruções de transferência
  do Token Program.
...
A transferência de tokens move tokens de um token account para outro token
account do mesmo mint.

As transferências não alteram o fornecimento do mint. As transferências apenas
atualizam os saldos entre token accounts.

## Como Transferir Tokens

A transferência de tokens utiliza a instrução _rs`Transfer`_ ou
_rs`TransferChecked`_ do Token Program.

Os exemplos abaixo utilizam _rs`TransferChecked`_, que requer que o chamador
forneça o mint e os decimais para que a instrução possa verificar o mint
esperado e a precisão do token antes de mover tokens entre contas.

No Token Extensions Program, _rs`Transfer`_ está obsoleto em favor de
_rs`TransferChecked`_ ou _rs`TransferCheckedWithFee`_.

O proprietário do source account ou um delegado aprovado assina a transferência.
No Token Extensions Program, um delegado permanente do mint também pode
autorizar a transferência se o mint tiver a extensão de delegado permanente
ativada.
...
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_          | O estado do token account armazena os saldos atualizados por uma transferência.                                                                                                                | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | Uma instrução de transferência que move tokens entre token accounts sem exigir que o chamador forneça o mint ou decimais. No Token Extensions Program, use _rs`TransferChecked`_ em vez disso. | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Uma instrução de transferência que requer que o chamador forneça o mint e os decimais e verifica esses valores antes de mover tokens entre token accounts do mesmo mint.                       | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | Lógica do processador compartilhada para transferências de tokens.                                                                                                                             | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |
