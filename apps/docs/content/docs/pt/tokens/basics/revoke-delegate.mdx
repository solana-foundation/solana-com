...
description:
  Remover o delegado atual de uma conta de token e limpar sua permissão delegada
  restante.
...
## O Que Faz a Revogação de um Delegado?

Revogar um delegado limpa o delegado atual da conta de token e redefine a
quantidade delegada para zero.

Após a revogação, o delegado anterior não tem mais autoridade para transferir ou
queimar tokens em nome da conta de token.

## Como Revogar um Delegado

A revogação de um delegado usa a instrução _rs`Revoke`_ do Token Program.

A instrução _rs`Revoke`_ limpa os campos de delegado na conta de token de
origem. No Token Program, o proprietário da conta de token assina a revogação.
No Token Extensions Program, o delegado atual também pode revogar a delegação.

### Referência de Origem

| Item                 | Descrição                                                               | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| -------------------- | ----------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_        | O estado da conta de token armazena o delegado e a quantidade delegada. | [Origem](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Origem](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Revoke`_         | Uma instrução que remove o delegado atual de uma conta de token.        | [Origem](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L128-L140) | [Origem](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L170-L182) |
| _rs`process_revoke`_ | Lógica compartilhada do processador para revogação de delegado.         | [Origem](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L395-L420)     | [Origem](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L643-L674)     |

### Typescript

Os exemplos de `Kit` abaixo mostram a abordagem recomendada usando
`@solana/kit`. Exemplos legados usando `@solana/web3.js` estão incluídos como
referência.
