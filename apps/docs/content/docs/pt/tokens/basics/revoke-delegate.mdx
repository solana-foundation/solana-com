...
description:
  Remover o delegado atual de uma conta de tokens e limpar a permissão delegada
  restante.
...
Revogar um delegado limpa o delegado atual da token account e redefine a
quantidade delegada para zero.

Após a revogação, o delegado anterior não possui mais autoridade para transferir
ou queimar tokens em nome da token account.

## Como Revogar um Delegado

Revogar um delegado utiliza a instrução _rs`Revoke`_ do Token Program.

A instrução _rs`Revoke`_ limpa os campos de delegado na token account de origem.
No Token Program, o proprietário da token account assina a revogação. No Token
Extensions Program, o delegado atual também pode revogar a delegação.

### Referência de Origem

| Item                 | Descrição                                                              | Token Program                                                                                                 | Token Extensions Program                                                                                            |
| -------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_        | O estado da token account armazena o delegado e a quantidade delegada. | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Revoke`_         | Uma instrução que remove o delegado atual de uma token account.        | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L128-L140) | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L170-L182) |
| _rs`process_revoke`_ | Lógica compartilhada do processador para revogação de delegado.        | [Fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L395-L420)     | [Fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L643-L674)     |
