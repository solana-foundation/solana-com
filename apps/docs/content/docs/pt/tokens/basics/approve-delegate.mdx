...
específica de tokens da sua conta de tokens. O delegado pode transferir tokens
até o montante aprovado, mas não pode excedê-lo. Apenas o proprietário da conta
de tokens pode aprovar um delegado, e cada conta de tokens pode ter apenas um
delegado de cada vez. Qualquer nova aprovação substitui a anterior.
