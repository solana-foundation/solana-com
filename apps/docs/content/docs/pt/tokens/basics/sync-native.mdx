...
sincroniza o saldo de uma conta de token SOL encapsulado (WSOL) com o SOL real
(lamports) armazenado nela. Quando transfere SOL nativo para uma conta de token
WSOL, o saldo do token não é atualizado automaticamente. Deve chamar SyncNative
para refletir o saldo WSOL correto.
