...
title: Sync Native
description:
  Atualizar o saldo de uma conta de token SOL encapsulado após transferir SOL
  para a conta.
...
## O Que É SOL Encapsulado?

SOL encapsulado (WSOL) é SOL mantido em uma token account para o mint nativo do
Token Program. O mint nativo é o endereço de mint que o Token Program usa para
token accounts que encapsulam SOL. O mint nativo permite que SOL seja usado
através das instruções do Token Program.

| Programa                 | Endereço do mint nativo                        |
| ------------------------ | ---------------------------------------------- |
...
Uma token account de SOL encapsulado armazena SOL no campo `lamports` da conta,
mas rastreia o SOL mantido na token account de SOL encapsulado como um saldo de
token através do campo _rs`amount`_ da token account. Se você transferir SOL
para uma token account de SOL encapsulado, o saldo em lamports aumenta
imediatamente, mas o campo _rs`amount`_ de token não muda até que você chame a
instrução _rs`SyncNative`_ do Token Program.

## Como Sincronizar SOL Nativo

Use a instrução _rs`SyncNative`_ do Token Program após transferir SOL para uma
token account de SOL encapsulado. A instrução _rs`SyncNative`_ atualiza o campo
_rs`amount`_ da token account para corresponder aos lamports mantidos na conta
acima de sua reserva isenta de aluguel.

### Referência de Código-Fonte

| Item                      | Descrição                                                                                                                                                                                                         | Token Program                                                                                                        | Token Extensions Program                                                                                                   |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_             | O estado da token account usa _rs`is_native`_ para marcar a conta como WSOL e armazena a reserva nativa, o saldo mínimo de lamports isento de aluguel que a conta deve manter, e a quantidade de token rastreada. | [Código-fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Código-fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`SyncNative`_          | Uma instrução que atualiza o campo _rs`amount`_ de uma token account de SOL encapsulado para corresponder aos lamports mantidos na conta acima de sua reserva isenta de aluguel.                                  | [Código-fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Código-fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| _rs`process_sync_native`_ | Lógica compartilhada do processador para sincronizar saldos de SOL encapsulado.                                                                                                                                   | [Código-fonte](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Código-fonte](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |

### Typescript

Os exemplos abaixo mostram a abordagem recomendada usando `Kit`. Exemplos
legados usando `@solana/kit` estão incluídos para referência.
