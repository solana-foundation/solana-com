...
<Callout type="info">
  Executando em modo headless — em uma rota de API, worker ou script sem
  carteira de navegador? Assine com um serviço de gerenciamento de chaves de
  backend em vez de um keypair bruto. Veja [Keychain](/docs/tools/keychain) e
...
- **Máquinas de estado headless**: Execute o cliente dentro de rotas de API,
  ...
  carteira + RPC que alimenta sua UI.
- **Dashboards em tempo real**: Combine watchers (saldos, contas, assinaturas)
  com sua biblioteca de UI preferida; o cliente gerencia o fan-out de websocket
  e a invalidação de cache.
- **Stores personalizadas**: Injete sua própria store Zustand para hidratar a
  partir de IndexedDB/localStorage, espelhar o estado em sessões do servidor ou
  coordenar entre abas do navegador.
- **Bridge para hooks React**: Passe uma instância de cliente configurada para
  `@solana/react-hooks` quando quiser hooks ergonômicos sobre o mesmo runtime.
- **Testabilidade**: A interface de cliente único pode ser mockada em testes
