...
<Callout type="info">
  Executando sem interface — em uma rota de API, worker ou script sem carteira
  de navegador? Assine com um serviço de gerenciamento de chaves no backend em
  vez de um keypair bruto. Veja [Keychain](/docs/tools/keychain) e [Assinatura
  em Produção](/docs/core/transactions/signing-in-production).
</Callout>
...
- **Bridge para hooks React**: Passe uma instância do cliente configurada para
