...
Configure uma integração mínima de carteira Solana no Next.js com o Solana Kit.

Este guia fornece um exemplo mínimo de implementação da funcionalidade de
carteira Solana em uma aplicação Next.js usando `@solana/kit`. Você criará um
...
no repositório do Solana Kit.

## Recursos

- [Documentação do Solana Kit](https://www.solanakit.com/docs)
- [Repositório GitHub do Solana Kit](https://github.com/anza-xyz/kit)
...
componentes de interface e instale as dependências Solana necessárias.
...
Instale as seguintes dependências Solana:
...
- Conectar aos endpoints RPC da devnet da Solana
- Filtrar as carteiras Solana disponíveis instaladas no navegador do usuário
...
Em seguida, envolva toda a aplicação Next.js com o provedor Solana.
...
- Cria uma transação Solana com uma instrução invocando o programa memo
- Solicita que a carteira conectada assine e envie a transação
- Exibe um link para visualizar a transação no Solana Explorer
...
<Callout type="info">
  ...
  cluster devnet e [financiada com SOL da devnet](https://faucet.solana.com/)
