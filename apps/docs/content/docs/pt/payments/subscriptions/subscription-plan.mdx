...
description: Guia para configurar transações de plano de assinatura.
---

Um plano de assinatura permite que um comerciante publique termos de cobrança
que os usuários podem aceitar. Após um usuário assinar, o comerciante ou um
cobrador aprovado pode coletar até o valor do plano a cada período de cobrança.

Este guia mostra o fluxo completo como blocos de construção. O comerciante cria
um plano, o assinante o aceita, e o comerciante ou cobrador coleta pagamentos do
PDA de assinatura resultante.
...
O comerciante é o proprietário do plano. O PDA do plano é derivado do endereço
do comerciante e `planId`.

Um patrocinador pode financiar o rent do plano passando o `payer` opcional, enquanto o comerciante permanece como proprietário do plano. A exclusão do plano reembolsa o proprietário, não o pagador, portanto, controle o patrocínio fora da cadeia.
...
O comerciante pode atualizar campos mutáveis do plano após a criação. Assinantes
existentes mantêm os termos que aceitaram, enquanto novos assinantes aceitam os
termos atuais do plano.

Algumas regras se aplicam:

- Um `endTs` finito só pode ser encurtado, nunca estendido ou removido
  (`PlanEndTsCannotExtend`).
- Em um plano `Sunset` você pode remover pullers (o novo conjunto deve ser um
  subconjunto do atual) para revogar um puller comprometido; o status, `endTs` e
  os metadados permanecem congelados.
- As edições funcionam durante o período de cobrança final de um plano, desde
  que o `endTs` não seja alterado.
- A instrução carrega o estado do plano observado no momento da assinatura
  (`expectedCreatedAt`, `expectedEndTs`, `expectedPullers`,
  `expectedMetadataUri`). Se o plano ativo não corresponder mais, a atualização é
  rejeitada (`StalePlanApproval`); portanto, uma atualização assinada desatualizada não pode restaurar
  pullers removidos nem reverter edições posteriores. O método `updatePlan` do cliente do plugin
  busca o estado atual para você; ao construir manualmente, busque o plano e
  passe os campos você mesmo.
...
O assinante aceita os termos do plano atual. O PDA de assinatura é derivado do
PDA do plano e do endereço do assinante.

Para agrupar a inicialização de autoridade e a assinatura em uma única transação, passe
o sentinel `UNKNOWN_INIT_ID` (exportado pelo SDK TypeScript) como
`expectedSubscriptionAuthorityInitId`. O programa aceita a autoridade somente se
ela foi criada no slot atual, portanto o sentinel funciona para novos cadastros; um
usuário recorrente cuja autoridade foi criada em um slot anterior deve passar o
`initId` real (o cliente do plugin o busca para você) ou a chamada falha com
...
O comerciante ou um puller autorizado assina a cobrança. Quando o plano utiliza
uma lista de destinos permitidos, o proprietário do token account receptor deve
estar listado em `destinations`.
...
Cancelar marca a subscrição como encerrando. Revogar fecha a PDA da subscrição
após o prazo de expiração do cancelamento ter decorrido. O subscritor assina
ambas as transações.
...
Quando tanto o assinante quanto o proprietário atual do plano assinam,
`cancelSubscriptionNow` expira a assinatura no momento do cancelamento, em vez de
no final do período de cobrança. Também pode encurtar um cancelamento com período de carência pendente. A aprovação está vinculada ao início do período observado no momento da assinatura; se
a assinatura tiver sido alterada desde então, a transação é rejeitada
...
Uma subscrição cancelada pode ser reativada antes de ser revogada. Uma vez
revogada, a conta de subscrição é encerrada e o subscritor deve
[subscrever](#subscribe) novamente. Retomar requer o `SubscriptionAuthority` do
subscritor para o mint do plano, que o programa valida (proprietário, mint e
`init_id`) e rejeita se estiver desatualizado ou reinicializado.

A instrução carrega a expiração observada pelo assinante no momento da assinatura
(`expectedExpiresAtTs`); uma incompatibilidade é rejeitada (`StaleSubscriptionApproval`),
portanto, um resumo assinado desatualizado não pode limpar um cancelamento posterior que o assinante nunca
aproveu.
...
- `amount` está em unidades base. Para um token de 6 decimais, `5_000_000`
  significa `5` tokens.
- O SDK TypeScript obtém os termos do plano em tempo real durante `subscribe`
  quando os omite.
- O `SubscribeBuilder` em Rust necessita dos termos esperados do plano. Obtenha
  e descodifique a conta do plano primeiro, em seguida passe esses campos
  através de `SubscribeData`.
- Apenas o comerciante ou uma carteira listada em `pullers` pode cobrar
  pagamentos.
- O subscritor assina as transações de configuração, cancelamento e revogação. O
  comerciante ou o cobrador aprovado assina as transações de cobrança.
