...
O assinante aceita os termos atuais do plano. O PDA da assinatura é derivado do
...
uma lista de permissões de destino, o proprietário da token account receptora
deve estar listado em `destinations`.
...
Cancelar marca a assinatura como encerrando. Revogar fecha o PDA da assinatura
após o período de expiração do cancelamento ter decorrido. O assinante assina
ambas as transações.
...
- `amount` está em unidades base. Para um token de 6 decimais, `5_000_000`
  significa `5` tokens.
- O SDK TypeScript busca os termos do plano em tempo real durante `subscribe`
  quando você os omite.
- O `SubscribeBuilder` em Rust precisa dos termos do plano esperados. Busque e
  decodifique a conta do plano primeiro, depois passe esses campos através de
  `SubscribeData`.
- Apenas o comerciante ou uma carteira listada em `pullers` pode coletar
  pagamentos.
- O assinante assina as transações de configuração, cancelamento e revogação. O
  comerciante ou extrator aprovado assina as transações de cobrança.
