...
- A token account do utilizador deve existir antes da inicialização.
- Os valores estão em unidades base. Para um token de 6 casas decimais,
  `1_000_000` significa `1` token.
- Execute `initSubscriptionAuthority` apenas quando a conta Subscription
  Authority ainda não existir.
- O utilizador assina as transações de configuração e revogação. O delegado
