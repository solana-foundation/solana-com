...
<Callout type="caution">
  ...
  lançamento estável. Para mais detalhes, consulte a [documentação do Commerce
...
| `amount`    | `bigint`    | Quantia em lamports ou unidades menores de token       |
| `splToken`  | `Address`   | Endereço mint do token SPL (omitir para SOL)           |
| `reference` | `Address[]` | Referência(s) única(s) para rastreamento de pagamentos |
| `label`     | `string`    | Nome do comerciante mostrado na carteira               |
| `message`   | `string`    | Mensagem mostrada antes do pagamento                   |
| `memo`      | `string`    | Memo onchain anexado à transação                       |
