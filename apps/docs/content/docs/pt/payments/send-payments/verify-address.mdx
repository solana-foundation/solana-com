...
Após derivar o endereço da ATA, verifique se a token account existe onchain. Se
a ATA não existir, você pode incluir uma instrução para criar a token account do
destinatário na mesma transação da transferência. No entanto, isso requer o
pagamento de rent pela nova token account. Como o destinatário possui a ATA, o
SOL pago pelo rent não pode ser recuperado pelo remetente.
