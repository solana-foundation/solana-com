...
<Callout type="caution" title="Não use RPC público para produção">
  Os endpoints públicos (`api.mainnet.solana.com`, `api.devnet.solana.com`) têm
  limite de taxa, não possuem SLA e são inadequados para fluxos de pagamento em produção.
  Use um [provedor de RPC](https://solana.com/rpc) para garantir um endpoint RPC privado
