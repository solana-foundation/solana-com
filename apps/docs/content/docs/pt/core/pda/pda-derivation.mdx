...
O bump seed é um único byte (u8) anexado às seeds opcionais durante a derivação.
[`find_program_address`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L800)
pesquisa de 255 até 1, chamando _rs`create_program_address`_ com cada valor até
que o resultado saia da curva Ed25519. O primeiro valor que tiver sucesso é o
**bump canônico**.
...
Derivar um PDA apenas calcula um endereço. Isso não cria uma conta onchain nesse
endereço. A conta deve ser criada explicitamente através de uma instrução
...
### Iterando Todos os Bumps Possíveis
...
<Callout type="warn">
  Isto é distinto do comportamento de pesquisa canônica atual documentado para
  `sol_try_find_program_address`, que itera de 255 até 1.
</Callout>
...
Neste exemplo, o bump 255 produz um endereço na curva e falha. O primeiro bump
válido é 254, tornando-o o bump canônico.
