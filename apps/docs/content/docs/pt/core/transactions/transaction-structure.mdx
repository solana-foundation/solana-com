...
há nenhuma verificação a nível de protocolo que garanta que o destinatário pode
mover o SOL de volta. Os lamports só podem ser movidos pela programa
proprietária da conta, portanto enviar SOL para um
...
[token account](/docs/tokens/basics/create-token-account) só é recuperável pelo
proprietário dessa conta, nunca pelo remetente.

As transferências de **token** SPL têm proteção parcial: o Token Program rejeita
uma transferência cujas contas não correspondam ao mint esperado. Transferências
de **SOL** nativo não possuem essa proteção, portanto o remetente deve verificar
o destinatário antes de assinar. Consulte
...
<Callout>
  ...
  completa que também lida com envios de token SPL (token accounts, ATAs,
