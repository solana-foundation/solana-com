...
A taxa de priorização é uma taxa opcional que aumenta a prioridade de agendamento
de uma transação. Uma unidade de computação (CU) é uma unidade de processamento que o runtime
contabiliza durante a execução da transação. Cada operação (aritmética, acesso à memória,
syscall) tem um custo fixo em CUs. Você pode definir a taxa de priorização incluindo
instruções de orçamento de computação na sua transação. (Consulte o
[guia Como Usar Taxas de Prioridade](/docs/core/fees) para mais detalhes.)
...
<Callout type="info" title="Transações v1 definem a taxa diretamente">
  Esta fórmula de preço multiplicado pelo limite aplica-se a transações legadas e v0. No próximo [formato
  v1](/docs/core/transactions/versioned-transactions#v1-message-format), a taxa de prioridade é um total absoluto em lamports definido diretamente na configuração da mensagem
  — sem preço por CU, sem multiplicação, sem arredondamento.
</Callout>
