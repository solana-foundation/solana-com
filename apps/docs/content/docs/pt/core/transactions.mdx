...
- [`recent_blockhash`](mention:recent-blockhash): um
  [blockhash](#recent-blockhash) que atua como um carimbo de data/hora para a
  transação
...
<Callout>
  ...
  individualmente. Em vez disso, as permissões de conta são determinadas usando
  o `header` e `account_keys`.
</Callout>
