---
title: CPIs sem signatários PDA
description:
  Como usar a função invoke para invocações entre programas no Solana quando não
  é necessária assinatura PDA, com exemplos de Anchor CpiContext e Rust nativo.
...
<Callout type="info" title="Resumo">
  Use _rs`invoke`_ para CPIs onde todos os signatários necessários já assinaram
...
é usada. A função _rs`invoke`_ chama a função _rs`invoke_signed`_ com um array
_rs`signers_seeds`_ vazio. O array de signatários vazio indica que não são
...
Os exemplos abaixo fazem uma CPI usando Anchor e Rust nativo. Inclui uma única
instrução que transfere SOL de uma conta para outra.

#### Anchor

Os exemplos seguintes mostram duas abordagens para implementar CPIs num programa
...
- **Exemplo 1**: Usa _rs`CpiContext`_ e função auxiliar do Anchor.
...
- **Exemplo 3**: Constrói a instrução CPI manualmente. Esta abordagem é útil
  quando não existe uma crate para ajudar a construir a instrução que deseja
  invocar.
...
O exemplo seguinte mostra como fazer uma CPI a partir de um programa escrito em
Rust nativo. Inclui uma única instrução que transfere SOL de uma conta para
outra. (O ficheiro de teste usa [LiteSVM](https://github.com/LiteSVM/litesvm)
