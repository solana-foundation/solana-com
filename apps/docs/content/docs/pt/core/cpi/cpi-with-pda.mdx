---
title: CPIs com signatários PDA
description:
  Como usar a função invoke_signed para invocações entre programas na Solana
  quando o programa chamador precisa assinar como um PDA, com exemplos de Anchor
...
<Callout type="info" title="Resumo">
  Use _rs`invoke_signed`_ quando o programa chamador precisar assinar em nome de
  um PDA que ele possui. O runtime deriva as chaves públicas do PDA a partir das
  sementes de signatário fornecidas e adiciona-as ao conjunto de signatários
  válidos antes da verificação de privilégios.
</Callout>
...
Quando um CPI requer um signatário PDA, use
[`invoke_signed`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/program/src/program.rs#L51)
com as [sementes de signatário](mention:signer-seeds) usadas para derivar o
[PDA](/docs/core/pda). Para detalhes sobre como o runtime verifica assinaturas
PDA, consulte [assinatura PDA](/docs/core/cpi/cpi-cost-model#pda-signing).
...
Os exemplos abaixo fazem um CPI com signatários PDA usando Anchor e Rust nativo.
Cada exemplo inclui uma única instrução para transferir SOL de um PDA para uma
conta destinatária, usando um CPI assinado pelo PDA.

#### Anchor

Os exemplos a seguir mostram duas abordagens para implementar CPIs num programa
[Anchor](https://www.anchor-lang.com/docs). Os exemplos são funcionalmente
equivalentes, mas demonstram diferentes níveis de abstração.

- **Exemplo 1**: usa _rs`CpiContext`_ do Anchor e função auxiliar.
- **Exemplo 2**: usa a função _rs`system_instruction::transfer`_ da crate
  _rs`solana_program`_. (O exemplo 1 é uma abstração desta implementação.)
- **Exemplo 3**: constrói a instrução CPI manualmente. Esta abordagem é útil
  quando não há uma crate disponível para ajudar a construir a instrução que
  deseja invocar.
...
O exemplo abaixo faz uma CPI com signatários PDA a partir de um programa escrito
em Rust nativo. Ele inclui uma única instrução que transfere SOL de uma conta
PDA para outra. A CPI é assinada pela conta PDA. (O arquivo de teste usa
