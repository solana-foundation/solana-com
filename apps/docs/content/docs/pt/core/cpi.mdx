...
Uma invocação entre programas (CPI) ocorre quando um programa Solana invoca
diretamente as instruções de outro programa. Isso permite a composição de
programas. Se você pensar em uma [instrução](/docs/core/instructions) da Solana
...
<Callout>
  A altura máxima da invocação de instruções do programa é chamada de
  [`max_instruction_stack_depth`](https://github.com/anza-xyz/agave/blob/v2.1.13/compute-budget/src/compute_budget.rs#L38)
  e é definida pela constante [MAX_INSTRUCTION_STACK_DEPTH](https://github.com/anza-xyz/agave/blob/v2.1.13/compute-budget/src/compute_budget.rs#L13)
  de 5.
...
é usada. Ela recebe as [sementes de assinante](mention:signer-seeds) usadas para
derivar os [PDAs](/docs/core/pda) assinantes. O runtime da Solana internamente
chama
...
- **Exemplo 3**: Constrói a instrução CPI manualmente. Esta abordagem é útil
  quando não há um crate disponível para ajudar a construir a instrução que você
  deseja invocar.
