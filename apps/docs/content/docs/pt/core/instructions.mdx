...
O array `accounts` da instrução é um array de estruturas
...
interage. (Isso permite que a transação execute instruções em paralelo, desde
que não modifiquem a mesma conta.)
...
programa invocar. Também inclui quaisquer argumentos exigidos pela instrução.
...
diferente entre SDKs, mas observe que cada instrução contém as mesmas três
...
<Callout type="info">
  ...
  disponível, pode construir manualmente a instrução.
</Callout>
