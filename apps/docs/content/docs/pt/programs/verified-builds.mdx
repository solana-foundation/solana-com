...
- **Sem PDA on-chain**: Execute `solana-verify verify-from-repo -um` e
  **selecione SIM quando solicitado**. Sem fazer upload do PDA, a API não
  consegue recuperar seus metadados de verificação.
- **Incompatibilidade de dados do PDA**: Atualize seu PDA se você reimplantou
  seu programa. Os dados do seu PDA devem corresponder ao programa implantado.
- **Hash de commit incorreto**: Crie seu PDA usando o hash de commit exato que
  você implantou
- **Diferenças no ambiente de build**: Use Docker com solana-verify ao criar seu
  PDA
...
- **Vínculo Criptográfico**: Seu PDA é derivado do endereço do seu programa,
  criando um vínculo imutável aos seus dados de verificação
...
- **Sem necessidade de confiança**: A API rejeita dados arbitrários - ela usa
  apenas o que sua autoridade de atualização armazenou on-chain
...
- **Prova de Propriedade**: Seu signatário deve ser a autoridade de atualização,
  provando criptograficamente que você controla o programa
...
- **PDA On-Chain**: Seus metadados de verificação ficam on-chain, não
...
Verifique qualquer programa você mesmo lendo seu PDA on-chain e executando a
