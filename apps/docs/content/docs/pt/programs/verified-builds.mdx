...
> Recomenda-se usar um URL de RPC pago, caso contrário poderá encontrar limites
> de taxa dos RPCs gratuitos. Portanto, em vez de `-um` deve usar
...
caminho executável no contêiner é o mesmo que o hash do program onchain
fornecido ao comando. Como o build já foi carregado para uma imagem, não há
necessidade de uma reconstrução completa do executável, que pode levar muito
tempo.
...
> Note que precisámos especificar o `library-name` porque o repositório Squads
> inclui múltiplos programas. Usamos a flag `--bpf` porque `squads_mpl` foi
> previamente verificado com Anchor.
...
- **Signatário incorreto**: Confirme se o seu signatário é a autoridade de
  atualização do program executando `solana program show YourProgramId`
- **Sem PDA onchain**: Execute `solana-verify verify-from-repo -um` e
  **selecione SIM quando solicitado**. Sem carregar o PDA, a API não consegue
  recuperar os seus metadados de verificação.
- **Dados do PDA incompatíveis**: Atualize o seu PDA se tiver reimplementado o
  seu program. Os dados do seu PDA devem corresponder ao seu program
  implementado.
- **Hash de commit incorreto**: Crie o seu PDA usando exatamente o hash de
  commit que você implementou
- **Diferenças no ambiente de build**: Use Docker com solana-verify ao criar o
  seu PDA

## O hash do meu build local não corresponde ao hash onchain. Porquê?
...
Se seu programa não tem autoridade de atualização ou foi tornado imutável antes
de você poder criar um PDA, temos um endereço autorizado para esta situação.
Entre em contato conosco em contact@osec.io, e nós o ajudaremos a verificar seu
programa.

## O que é o PDA e por que é importante?

Seu PDA (Program Derived Account) possibilita a verificação sem necessidade de
confiança:

- **Armazenamento Onchain**: Armazene os seus metadados de verificação (URL do
  repo, hash de commit, parâmetros de build) onchain num PDA pertencente ao
  program Otter Verify (`verifycLy8mB96wd9wqq3WDXQwM4oU6r42Th37Db9fC`)
- **Ligação Criptográfica**: O seu PDA é derivado do endereço do seu program,
  criando uma ligação imutável aos seus dados de verificação
- **Confiança Descentralizada**: Qualquer pessoa pode ler o seu PDA e verificar
  independentemente o seu program

## Por que devo criar o PDA antes de usar a API?

A API só funciona com PDAs onchain porque:

- **Sem Confiança**: A API rejeita dados arbitrários - só utiliza o que a sua
  autoridade de atualização armazenou onchain
- **Mais Simples**: Apenas forneça o signatário + program_id; a API obtém todo o
  resto do seu PDA
- **À Prova de Adulteração**: O seu PDA cria um registo imutável que qualquer
  pessoa pode verificar independentemente
- **Prova de Propriedade**: O seu signatário deve ser a autoridade de
  atualização, provando criptograficamente que você controla o program
...
- Não se preocupe em reverificar caso contrário - a API reverifica
...
- **PDA On-Chain**: Seus metadados de verificação residem onchain, não
  controlados por nenhuma autoridade central
- **Prova de Autoridade de Upgrade**: Apenas a autoridade de upgrade do seu
  programa pode criar/atualizar o PDA
- **Verificação Independente**: Qualquer pessoa pode verificar lendo seu PDA e
  executando `solana-verify` localmente
- **Re-verificação Contínua**: A API re-verifica automaticamente todos os
  programas a cada 24 horas

**Entenda Estas Limitações:**

- A verificação confirma que a fonte corresponde ao deployment - **NÃO** que seu
  código é seguro
...
Verifique qualquer programa você mesmo lendo seu PDA onchain e executando a
verificação localmente:
...
3. Você não precisa confiar na API - verifique tudo você mesmo onchain
...
Nós apenas consideramos a verificação válida se o signatário for a autoridade de
atualização.
...
- Seu PDA armazena uma URL de repositório público que qualquer pessoa pode
  acessar
...
- O objetivo principal é permitir que os usuários verifiquem independentemente
  se o código-fonte corresponde à implantação

Repositórios privados quebram o modelo de confiança central do sistema de
verificação.
...
**Crítico**: Sempre verifique localmente (passo 2) para confirmar que o hash da
build corresponde antes de exportar a transação do PDA.
...
desenvolvedores encontrem seus SDKs diretamente a partir de um Solana Explorer.
Ao aproveitar ferramentas como a Solana Verify CLI e o Docker, você pode manter
builds verificáveis e seguras que se alinham com seu código-fonte. Sempre tome
as precauções necessárias para usar ambientes consistentes e considere soluções
de governança para upgrades e implantações seguras.

## Segurança + Aviso

Embora as builds verificadas sejam uma ferramenta poderosa para garantir a
integridade dos seus programas Solana, ela não é completamente sem confiança na
configuração padrão. As imagens docker são construídas e hospedadas pela Solana
Foundation.

Esteja ciente de que você está construindo seu projeto em uma imagem docker
baixada e que toda a sua configuração é copiada para essa imagem docker para a
construção, incluindo informações potencialmente sensíveis.

Se você quiser ter uma configuração completamente sem confiança, pode construir
as imagens docker você mesmo e hospedá-las em sua própria infraestrutura. Dessa
forma, você pode ter certeza de que as imagens docker não foram adulteradas.
Você pode encontrar os scripts para criar suas próprias imagens docker no
[repositório de Verified builds](https://github.com/Ellipsis-Labs/solana-verifiable-build)
e pode bifurcá-lo e executar as github actions você mesmo ou validar que elas
estão corretas.
...
por quaisquer perdas ou danos que possam ocorrer ao usar o pipeline de builds
verificados.
...
pelo [securitytxt.org](https://securitytxt.org), essa abordagem fornece uma
...
- Use Links: Para informações que provavelmente mudarão (por exemplo, detalhes
  de contato), é recomendado vincular a uma página web em vez de codificá-las
...
você facilita que os pesquisadores entrem em contato com você, promovendo melhor
segurança e comunicação dentro do ecossistema Solana.
...
Você pode verificar o status de verificação e navegar pelos programas
verificados em [verify.osec.io](https://verify.osec.io).
