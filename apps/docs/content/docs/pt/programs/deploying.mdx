...
- **ID do Programa**: O endereço onchain do seu programa. Os usuários interagem
  com seu programa referenciando o ID do Programa.
- **Conta do Programa**: A conta onchain que armazena os metadados do seu
  programa. O endereço da conta do programa é o ID do Programa. A conta do
  programa também armazena o endereço da Conta ProgramData.
- **Conta ProgramData**: A conta onchain que armazena o código executável
  implantado do seu programa.
- **Autoridade do Programa**: A conta que tem permissão para atualizar ou fechar
  o programa. Por padrão, esta é sua carteira CLI.
...
O ID do Programa é o endereço onchain da sua conta do programa. Usuários e
outros programas invocam seu programa referenciando este ID em suas transações.
...
- **`--use-rpc`**: Envia transações para o RPC configurado. Esta flag requer uma
  conexão RPC [ponderada por stake](/docs/defi/stake-weighted-qos) de
