...
| Implantar novo programa              | `solana program deploy <path_to_program.so>`                                                  |
| Atualizar programa existente         | `solana program deploy <path_to_program.so>` (igual a implantar)                              |
...
Escolha em qual cluster Solana implantar. Use o comando `solana config get` para
...
Use o comando `solana program deploy` para implantar o seu programa:
...
Atualize seu programa usando o mesmo comando `solana program deploy`:

1. Faça alterações no código do seu programa
2. Recompile seu programa: `cargo build-sbf`
...
Para tornar seu programa imutável, use o comando
...
Para fechar seu programa e recuperar o SOL alocado para a conta do programa, use
o comando `solana program close`:
...
Para baixar um programa implantado, use o comando `solana program dump`:
...
- **`--with-compute-unit-price`**: Define a taxa de prioridade em micro-lamports
  ...
  atual para priorizar sua transação.
...
- **`--use-rpc`**: Envia transações para o RPC configurado. Esta flag requer uma
  ...
  provedores como [Triton](https://triton.one/) ou
