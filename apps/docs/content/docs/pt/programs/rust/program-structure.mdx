...
Programas Solana escritos em Rust têm requisitos estruturais mínimos, permitindo
flexibilidade na forma como o código é organizado. O único requisito é que um
programa deve ter um `entrypoint`, que define onde a execução de um programa
começa.
...
Deverá ver os ficheiros predefinidos `src/lib.rs` e `Cargo.toml`.
...
lida com a desserialização dos dados de `input` nos parâmetros da função
`process_instruction`.

Um `entrypoint` de programa Solana tem a seguinte assinatura de função. Os
desenvolvedores são livres para criar sua própria implementação da função
`entrypoint`.
...
as instruções para as suas funções de tratamento apropriadas.
...
a função de tratamento apropriada é chamada. Cada função de tratamento inclui a
implementação para essa instrução.

Adicione o seguinte código ao `lib.rs` atualizando a função
`process_instruction` e adicionando os tratadores para as instruções
...
O nosso programa faz um CPI para chamar a instrução `create_account` do System
...
- Desserializa-o numa estrutura `CounterAccount`
...
O atributo `#[cfg(test)]` garante que este código só é compilado ao executar
testes.
...
Adicione o código do cliente a `examples/client.rs` e substitua o `program_id`
...
Este comando compila o seu programa e gera dois ficheiros importantes em
`target/deploy/`:
...
Pode verificar a implementação usando o comando `solana program show` com o ID
do seu programa:
