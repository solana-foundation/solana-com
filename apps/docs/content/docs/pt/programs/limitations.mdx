...
Programas Rust onchain suportam a maior parte das bibliotecas libstd, libcore e
liballoc do Rust, bem como muitas crates de terceiros.
...
- Acesso limitado a:
  - `std::hash`
  - `std::os`
- Bincode é extremamente dispendioso computacionalmente tanto em ciclos quanto
  em profundidade de chamada e deve ser evitado
- A formatação de strings deve ser evitada, pois também é computacionalmente
  dispendiosa.
...
- O runtime impõe um limite no número de instruções que um programa pode
  executar durante o processamento de uma instrução. Consulte
...
Quando um programa excede o limite de profundidade da pilha de chamadas
permitido, ele receberá o erro `CallDepthExceeded`.

## Profundidade de chamada CPI - erro `CallDepth`

As invocações entre programas permitem que os programas invoquem outros
programas diretamente, mas a profundidade está atualmente limitada a `4`.

Quando um programa excede a
[profundidade de chamada de invocação entre programas](/docs/core/cpi)
permitida, ele receberá um erro `CallDepth`
