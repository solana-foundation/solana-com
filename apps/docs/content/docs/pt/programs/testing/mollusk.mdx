...
instruções de programas Solana em um ambiente minimizado da Máquina Virtual
Solana (SVM). Todas as contas de teste devem ser explicitamente definidas,
...
Para usar o Token Program, o programa token2022 (Token Extensions) e o
Associated Token Program para testes com Mollusk, adicione
`mollusk-svm-programs-token` como dependência em `Cargo.toml`:
...
O exemplo a seguir mostra uma configuração mínima para testar um programa Solana
...
Para inicializar Mollusk com uma instância padrão, use o método
`Mollusk::default`.
...
Para inicializar Mollusk com um programa específico, use o método
`Mollusk::new`.
...
o seu próprio conjunto de verificações que devem passar.
...
exemplo, `process_instruction`), `MolluskContext` gere contas internamente
...
Mollusk fornece uma estrutura
[`Sysvars`](https://docs.rs/mollusk-svm/0.5.0/mollusk_svm/sysvar/struct.Sysvars.html)
personalizada para modificar seus valores para testes.

Use o método `warp_to_slot` para atualizar o relógio sysvar e simular o avanço
ou retrocesso no tempo para um slot específico.
...
O exemplo a seguir mostra como modificar o sysvar Mollusk diretamente acessando
o campo `sysvars` para alterar os parâmetros de rent. Você pode modificar outros
valores de sysvar da mesma forma.
...
para adicionar o Token Program, Token2022 Program (Token Extensions) e
...
<Callout type="info">
  ...
  para criar as contas mint e token.
</Callout>
