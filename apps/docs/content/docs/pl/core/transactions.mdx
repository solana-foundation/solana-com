...
Struktura `message` transakcji to
...
w wiadomości to tablica adresów kont, przesyłana w
[skomprymowanym formacie tablicy](/docs/references/terminology#compact-array-format).
Prefiks tablicy wskazuje jej długość. Każdy element tablicy to klucz publiczny,
który wskazuje na konto używane przez instrukcje. Tablica `accounts_keys` musi
być kompletna i ściśle uporządkowana w następujący sposób:
...
`recent_blockhash` wiadomości to wartość skrótu, która pełni rolę znacznika
czasu transakcji i zapobiega duplikacji transakcji. Blockhash wygasa po
...
w wiadomości to tablica wszystkich instrukcji do przetworzenia, przesyłana w
[skomprymowanym formacie tablicy](/docs/references/terminology#compact-array-format).
