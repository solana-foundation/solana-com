...
4. **Nieistniejące konta**: Konta, które nie istnieją w sieci blockchain, są
   ładowane jako konta domyślne (0 lamport, puste dane, należące do System
   Program) z `rent_epoch` ustawionym na `u64::MAX`.
