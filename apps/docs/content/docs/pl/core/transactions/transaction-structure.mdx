...
Każdy podpis jest generowany przez klucz prywatny. To, gdzie ten klucz się
znajduje — lokalny keypair, chmurowy HSM lub KMS, czy zarządzana usługa portfela
— jest decyzją projektową na poziomie produkcyjnym. Zobacz
[Podpisywanie w środowisku produkcyjnym](/docs/core/transactions/signing-in-production).

Pierwszy podpis w tablicy należy do **płatnika opłat**, czyli konta, które
pokrywa
[opłatę podstawową i opłatę priorytyzacyjną](/docs/core/fees/fee-structure#base-fee)
transakcji. Ten pierwszy podpis pełni również funkcję **identyfikatora
transakcji**, służącego do wyszukiwania transakcji w sieci. Identyfikator
...
- Musi posiadać wystarczającą liczbę lamport, aby pokryć
  `rent_exempt_minimum + total_fee`; w przeciwnym razie transakcja kończy się
...
- [`header`](mention:message-header): [Nagłówek](#header) wiadomości
...
- [`num_readonly_signed_accounts`](mention:num_readonly_signed_accounts): Liczba
  podpisanych kont tylko do odczytu.
...
Tablica musi zawierać każde konto i musi przestrzegać następującej ścisłej
kolejności:
...
<Callout>
  To ścisłe porządkowanie pozwala na połączenie tablicy `account_keys` z trzema
  licznikami w [`header`](#header) nagłówka wiadomości w celu określenia
  uprawnień dla każdego konta bez przechowywania flag metadanych per konto.
...
Pole `recent_blockhash` to 32-bajtowy skrót służący dwóm celom:
...
<Callout>
  Metoda RPC [`getLatestBlockhash`](/docs/rpc/http/getlatestblockhash) umożliwia
  pobranie aktualnego blockhasha oraz ostatniej wysokości bloku, przy której
  blockhash będzie ważny.
</Callout>
...
to kodowana kompaktowo tablica struktur
...
tablicy `account_keys` zamiast pełnego klucza publicznego. Zawiera:
...
poprzedzone kodowaniem długości compact-u16. Format ten używa 1 bajtu dla
wartości 0–127 oraz 2–3 bajtów dla większych wartości.

**Struktura starszego formatu transakcji** (w sieci):
...
Transfer System Program dodaje lamport do **dowolnego** konta. Na poziomie
protokołu nie ma żadnego sprawdzenia, czy odbiorca może z powrotem wyprowadzić
SOL. Lamport mogą być wyprowadzone tylko przez program będący właścicielem
konta, dlatego wysłanie SOL do [mint tokena](/docs/tokens/basics/create-mint),
programu lub [PDA](/docs/core/pda), nad którym nie masz kontroli, **grozi trwałą
utratą środków** — tylko uprawnienie określone przez program właściciela może je
...
Poniższy przykład pokazuje kod odpowiadający powyższym diagramom. Zobacz funkcję
...
Poniższy przykład pokazuje strukturę transakcji zawierającej jedną instrukcję
transferu SOL.
...
### Zweryfikuj odbiorcę przed wykonaniem transferu

Ponieważ transfer SOL powiedzie się na dowolne konto, sprawdź odbiorcę przed
podpisaniem. Pobierz konto i wysyłaj środki wyłącznie do portfela System Program
(lub niefinansowanego adresu
[on-curve](/docs/payments/send-payments/verify-address#on-curve-addresses));
odrzucaj minty, token account, programy i PDA, których nie kontrolujesz.
...
<Callout>
  Ten fragment kodu sprawdza odbiorców natywnego SOL. Aby zapoznać się z pełną
  klasyfikacją obejmującą również wysyłanie tokenów SPL (token account, ATA,
  Token-2022), zobacz [Verify
  Address](/docs/payments/send-payments/verify-address).
</Callout>

## Pobieranie szczegółów transakcji

Po przesłaniu transakcji pobierz jej szczegóły, korzystając z sygnatury
