...
description:
  Anatomia transakcji Solana, obejmująca podpisy, format wiadomości, nagłówek, adresy kont, blockhash, skompilowane instrukcje, kodowanie binarne oraz limit rozmiaru.
...
<Callout type="info" title="Podsumowanie">
  Transakcja zawiera podpisy oraz wiadomość. Wiadomość składa się z nagłówka,
  adresów kont, ostatniego blockhash oraz skompilowanych instrukcji. Maksymalny
  rozmiar po serializacji: 1 232 bajty (4 096 bajtów dla nadchodzącego formatu v1).
</Callout>
...
<Callout type="info" title="Transakcje v1 zwiększają limit do 4 096 bajtów">
  Ta strona opisuje starszy format transakcji, który jest również stosowany przez
  format v0, z wyjątkiem prefiksu wersji i tablic wyszukiwania adresów. Nadchodzący
  [format v1](/docs/core/transactions/versioned-transactions#v1-message-format)
  zwiększa limit rozmiaru do 4 096 bajtów i używa innego układu przesyłania danych.
</Callout>
...
<Callout type="info" title="Prefiksy starszych i wersjonowanych wiadomości">
  W wiadomości transakcji w formacie legacy, pierwszym bajtem wiadomości jest
  `num_required_signatures`, po którym następują pozostałe dwa bajty `MessageHeader`. W
  wiadomości transakcji w wersjonowanej, pierwszym bajtem jest zamiast tego prefiks wersji;
  trzycyfrowy `MessageHeader` zaczyna się bezpośrednio po tym prefiksie. Zobacz
  [transakcje wersjonowane](/docs/core/transactions/versioned-transactions), aby zapoznać się
  z pełnym układem wiadomości v0 i v1.
</Callout>
...
<Callout>
  ...
  licznikami w [`header`](#header) nagłówka wiadomości w celu określenia
...
[transakcja z trwałym nonce](/docs/core/transactions/durable-nonces).
...
Każdy _rs`CompiledInstruction`_ odwołuje się do kont za pomocą indeksu w tablicy
...
2. [`accounts`](mention:account-indexes): Tablica indeksów w tablicy
   `account_keys` określająca konta przekazywane do programu.
3. [`data`](mention:instruction-data): Tablica bajtów zawierająca dyskryminator
   instrukcji i zserializowane argumenty.
...
poprzedzone kodowaniem długości w formacie compact-u16. Format ten używa 1 bajtu
dla wartości 0–127 oraz 2–3 bajtów dla większych wartości.
...
Przy założeniu _rs`PACKET_DATA_SIZE`_ = 1 232 bajtów, dostępna przestrzeń może
być obliczona:
...
Poniższy diagram przedstawia, jak transakcje i instrukcje współpracują ze sobą,
umożliwiając użytkownikom interakcję z siecią. W tym przykładzie SOL jest
transferowane z jednego konta na drugie.
...
aby ichs saldo w lamport mogło ulec zmianie. Aby wykonać tę instrukcję, portfel
...
aktualizuje saldo w lamport obu kont.
...
Transfer za pomocą System Program dodaje lamport do **dowolnego** konta. Nie
istnieje żadna weryfikacja na poziomie protokołu, że odbiorca może wyprowadzić
SOL z powrotem. Lamport mogą być wyprowadzone tylko przez program będący
właścicielem konta, dlatego wysłanie SOL na
[mint tokena](/docs/tokens/basics/create-mint), program lub
[PDA](/docs/core/pda), nad którym nie masz kontroli, **grozi trwałą utratą
środków** — tylko upoważnienie określone przez program właściciela może je
zwrócić. SOL wysłane na
[token account](/docs/tokens/basics/create-token-account) może odzyskać jedynie
właściciel tego konta, nigdy nadawca.

Transfery **tokenów** SPL są częściowo samochronne: Token Program odrzuca
transfer, którego konta nie odpowiadają oczekiwanemu mintowi. Natywne transfery
**SOL** nie posiadają takiego zabezpieczenia, dlatego nadawca musi zweryfikować
...
<WithMentions>
Poniższy kod pokazuje dane wyjściowe z poprzednich fragmentów kodu.
...
podpisaniem. Pobierz konto i wysyłaj wyłącznie do portfela System Program (lub
niefinansowanego adresu
[on-curve](/docs/payments/send-payments/verify-address#on-curve-addresses));
odrzucaj minty, token accounts, programy i PDA, których nie kontrolujesz.
...
<Callout>
  Ten fragment kodu sprawdza natywnych odbiorców SOL. Aby zapoznać się z pełną
  klasyfikacją uwzględniającą również wysyłanie tokenów SPL (token accounts,
  ATA, Token-2022), zobacz [Verify
  Address](/docs/payments/send-payments/verify-address).
</Callout>

## Pobieranie szczegółów transakcji

Po wysłaniu pobierz szczegóły transakcji, używając sygnatury transakcji i metody
...
instrukcje (CPI) jako zakodowane dane binarne. Aby rozwiązać je na adresy i
prześledzić pełne drzewo instrukcji, zobacz
