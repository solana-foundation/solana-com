...
Opłata za priorytetyzację to opcjonalna opłata, która zwiększa priorytet planowania
transakcji. Jednostka obliczeniowa (CU) to jednostka miary obliczeń, którą środowisko uruchomieniowe
mierzy podczas wykonywania transakcji. Każda operacja (arytmetyczna, dostęp do pamięci,
syscall) kosztuje określoną liczbę jednostek CU. Opłatę za priorytetyzację można ustawić, dołączając
instrukcje budżetu obliczeniowego do transakcji. (Szczegółowe informacje można znaleźć w
przewodniku [Jak korzystać z opłat priorytetowych](/docs/core/fees).)
...
<Callout type="info" title="Transakcje v1 ustawiają opłatę bezpośrednio">
  Ta formuła cena razy limit dotyczy transakcji legacy i v0. W nadchodzącym [formacie
  v1](/docs/core/transactions/versioned-transactions#v1-message-format) opłata priorytetowa jest bezwzględną sumą w lamports ustawianą bezpośrednio w konfiguracji wiadomości — bez ceny za CU, bez mnożenia, bez zaokrąglania.
</Callout>
