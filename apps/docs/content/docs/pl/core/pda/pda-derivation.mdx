...
Bump seed to pojedynczy bajt (u8) dołączany do opcjonalnych wartości seed
podczas derywacji.
[`find_program_address`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L800)
przeszukuje wartości od 255 w dół do 1, wywołując _rs`create_program_address`_ z
każdą wartością, aż wynik znajdzie się poza krzywą Ed25519. Pierwsza wartość,
która zakończy się sukcesem, to **kanoniczny bump**.
...
Wyprowadzenie PDA oblicza jedynie adres. Nie tworzy to konta onchain pod tym
adresem. Konto musi zostać jawnie utworzone przez osobną instrukcję (zazwyczaj
_rs`create_account`_ poprzez CPI).
...
### Iterowanie wszystkich możliwych wartości Bump
...
<Callout type="warn">
  Różni się to od obecnego kanonicznego zachowania przeszukiwania
  udokumentowanego dla `sol_try_find_program_address`, które iteruje od 255 w
  dół do 1.
</Callout>

<Callout type="info">
  Przykład Kit nie jest dołączony, ponieważ funkcja
...
W tym przykładzie bump 255 generuje adres znajdujący się na krzywej i kończy się
niepowodzeniem. Pierwszym ważnym bumpem jest 254, co czyni go kanonicznym
bumpem.
