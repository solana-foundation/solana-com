...
Transakcje Solana mają limit rozmiaru (~1232 bajty). Każda instrukcja transferu
dodaje ~200 bajtów. W przypadku bardzo dużych rozliczeń wielostronnych rozważ użycie
[Tabel Wyszukiwania Adresów](/developers/cookbook/transactions/lookup-tables), aby
skompresować adresy kont i zmieścić więcej transferów w jednej transakcji.

Nadchodzący
[format transakcji v1](/docs/core/transactions/versioned-transactions#v1-message-format)
podnosi limit do 4096 bajtów, ale usuwa obsługę Tabel Wyszukiwania Adresów, więc rozliczenia
w formacie v1 zawierają adresy bezpośrednio. W obu przypadkach środowisko uruchomieniowe
ładuje maksymalnie 64 konta na transakcję.
