...
Programy Rust działające onchain obsługują większość bibliotek libstd, libcore i
liballoc, a także wiele pakietów zewnętrznych.
...
- Ograniczony dostęp do:
  - `std::hash`
  - `std::os`
- Bincode jest wyjątkowo kosztowny obliczeniowo zarówno pod względem cykli, jak
  i głębokości wywołań, dlatego należy go unikać
- Formatowanie stringów również powinno być unikane, ponieważ jest kosztowne
  obliczeniowo.
- Brak wsparcia dla `println!`, `print!`; zamiast tego użyj makra
  [`msg!`](https://github.com/solana-labs/solana/blob/d9b0fc0e3eec67dfe4a97d9298b15969b2804fab/sdk/program/src/log.rs#L33).
- Środowisko uruchomieniowe narzuca limit liczby instrukcji, które program może
  wykonać podczas przetwarzania jednej instrukcji. Więcej informacji znajdziesz
  w sekcji [budżet obliczeniowy](/docs/core/fees#compute-budget).
...
wywoływać inne programy bezpośrednio, ale głębokość jest obecnie ograniczona do
`4`.
