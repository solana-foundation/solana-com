...
katalogu głównym repozytorium. Jeśli w repozytorium masz tylko jeden program i
...
możesz zobaczyć przykład workspace z programem w podfolderze. Zwróć też uwagę,
...
Gdy plik `Cargo.toml` jest już na miejscu, możesz uruchomić
`cargo generate-lockfile`, aby utworzyć plik lock i kontynuować budowanie
programu.
...
> użyciu polecenia `verify-from-repo` musisz dodać flagę `--mount-path`. Będzie
> to ścieżka do folderu zawierającego `Cargo.toml`, który zawiera nazwę
> biblioteki twojego programu.
...
> Zaleca się użycie płatnego adresu URL RPC, ponieważ w przeciwnym razie możesz
> napotkać limity zapytań darmowych RPC. Zamiast `-um` powinieneś użyć
...
hash programu w łańcuchu bloków odpowiada hashowi wygenerowanego artefaktu
...
Gdy masz lokalnie autoryzację programu, zostaniesz poproszony o przesłanie
...
czy hash programu w łańcuchu bloków pasuje do hasha wygenerowanego artefaktu
kompilacji z twojego repozytorium.
...
Jeśli chcesz mieć całkowicie bezzaufaną konfigurację, możesz samodzielnie
uruchomić
[Verify API](https://github.com/otter-sec/solana-verified-programs-api) lub
przeprowadzić weryfikację programu lokalnie, używając polecenia
`verify-from-repo` i danych weryfikacyjnych zapisanych w
[PDA](https://explorer.solana.com/address/63XDCHrwZu3mXsw2RUFb4tbNpChuUHx4eA5aJMnHkpQQ/anchor-account),
...
Oprócz zweryfikowanych buildów możesz także dodać plik `security.txt` do swojego
programu. W przyszłości, po wdrożeniu, `security.txt` będzie przechowywać
publiczny klucz weryfikatora, aby ułatwić dostęp do danych weryfikacyjnych
zapisanych w PDA. PDA zawierający wszystkie informacje potrzebne do zbudowania i
weryfikacji programu jest wyprowadzany z adresu programu oraz pubkey
weryfikatora. Domyślnie jest to ten sam pubkey, który zbudował i wdrożył
program, ale może to być także inny pubkey określony w `security.txt`.

Funkcja `security.txt` pozwala deweloperom na umieszczenie informacji
kontaktowych i bezpieczeństwa bezpośrednio w smart kontraktach Solana.
Inspirowana [securitytxt.org](https://securitytxt.org), ta metoda zapewnia
ustandaryzowany sposób kontaktu badaczy bezpieczeństwa z opiekunami projektu,
nawet jeśli znany jest tylko adres kontraktu.

## Dlaczego warto używać security.txt?

Dla wielu projektów, zwłaszcza mniejszych lub prywatnych, zidentyfikowanie
deweloperów wyłącznie na podstawie adresu kontraktu może być trudne i
czasochłonne. Osadzenie pliku `security.txt` w programie zapewnia, że badacze
bezpieczeństwa mogą łatwo skontaktować się z odpowiednimi osobami, co może
zapobiec exploitom i umożliwić szybkie zgłaszanie błędów.

## Jak wdrożyć security.txt

Aby dodać plik `security.txt` do swojego programu Solana, wykonaj następujące
kroki:

Dodaj zależność `solana-security-txt` do pliku `Cargo.toml`:
...
bezpieczeństwie. Możesz dodać dane kontaktowe, adresy URL projektu, a nawet
...
danych kontaktowych i informacji o bezpieczeństwie dla każdego, kto chce zgłosić
potencjalny problem.
...
- Weryfikacja: Przed wdrożeniem zweryfikuj format i zawartość za pomocą
  narzędzia `query-security-txt`, które umożliwia walidację zarówno programów
  on-chain, jak i lokalnych binariów:
