...
`verify-from-repo` i danych weryfikacyjnych zapisanych w łańcuchu w
[PDA](https://explorer.solana.com/address/63XDCHrwZu3mXsw2RUFb4tbNpChuUHx4eA5aJMnHkpQQ/anchor-account),
który jest wyprowadzany z uprawnień wdrożeniowych programu oraz
...
program, ale może to być także inny pubkey określony w pliku `security.txt`.

Funkcja `security.txt` pozwala deweloperom na osadzenie informacji kontaktowych
i dotyczących bezpieczeństwa bezpośrednio w smart kontraktach Solana.
Inspirowana [securitytxt.org](https://securitytxt.org), ta metoda zapewnia
ustandaryzowany sposób kontaktowania się badaczy bezpieczeństwa z opiekunami
projektów, nawet jeśli znany jest tylko adres kontraktu.
...
dotyczące bezpieczeństwa. Możesz dodać dane kontaktowe, adresy URL projektu, a
nawet politykę bezpieczeństwa. Oto przykład:
...
- Weryfikacja: Przed wdrożeniem zweryfikuj format i zawartość za pomocą
  ...
  onchain, jak i lokalnych binariów:
