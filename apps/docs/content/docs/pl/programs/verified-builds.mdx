...
Korzystanie z [zweryfikowanych kompilacji na Solana](/docs/programs/verified-builds) zapewnia integralność i wiarygodność programów w sieci oraz umożliwia deweloperom znajdowanie Twoich SDK bezpośrednio w Solana Explorer. Wykorzystując narzędzia takie jak Solana Verify CLI i Docker, możesz utrzymywać weryfikowalne i bezpieczne kompilacje zgodne z kodem źródłowym. Zawsze stosuj niezbędne środki ostrożności, aby korzystać ze spójnych środowisk, i rozważ rozwiązania z zakresu zarządzania na potrzeby bezpiecznych aktualizacji i wdrożeń.
