...
programu token do testowania z Mollusk, dodaj `mollusk-svm-programs-token` jako
zależność w `Cargo.toml`:
...
struktura zapewnia prosty interfejs do testowania programów Solana. Wszystkie
pola można obsługiwać za pomocą kilku metod pomocniczych, ale użytkownicy mogą
też uzyskać do nich bezpośredni dostęp i modyfikować je, jeśli potrzebują
większej kontroli.
...
| [`process_and_validate_instruction`](#single-instruction-with-checks)          | Przetwarza instrukcję i wykonuje serię kontroli na wyniku, przerywając działanie, jeśli któraś zawiedzie.              |
| [`process_instruction_chain`](#multiple-instructions)                          | Przetwarza wiele instrukcji i zwraca wynik.                                                                            |
| [`process_and_validate_instruction_chain`](#multiple-instructions-with-checks) | Przetwarza wiele instrukcji i wykonuje serię kontroli na każdym wyniku, przerywając działanie, jeśli któraś zawiedzie. |
...
sprawdzania wyniku. Możesz samodzielnie zweryfikować wyniki po przetworzeniu.
...
demonstracyjnych. W praktyce Mollusk jest zwykle używany w module testowym
...
instrukcję z kontrolą poprawności. Metoda ta przerwie działanie, jeśli któraś
kontrola zawiedzie.
...
kolejno, bez kontroli poprawności.
...
zestaw kontroli, które muszą zostać spełnione.
...
jest wrapperem wokół `Mollusk`, który utrzymuje stan konta pomiędzy wywołaniami
...
wewnętrznie przez swój `account_store`. Dzięki temu nie trzeba przekazywać
parametru `accounts` podczas przetwarzania instrukcji.
...
Poniższy przykład przetwarza dwie oddzielne instrukcje transferu SOL z
utrzymaniem stanu konta pomiędzy instrukcjami przez `account_store`.
...
Użyj metody `warp_to_slot`, aby zaktualizować sysvar clock i zasymulować
...
Poniższy przykład pokazuje, jak bezpośrednio zmodyfikować sysvar Mollusk,
odwołując się do pola `sysvars`, aby zmienić parametry rent. W ten sam sposób
...
program tokenów do Mollusk na potrzeby testów.
...
<Callout type=\"info\">
  ...
  `mollusk-svm-programs-token` zawiera także funkcje pomocnicze do
tworzenia mintów i kont tokenów.
</Callout>
