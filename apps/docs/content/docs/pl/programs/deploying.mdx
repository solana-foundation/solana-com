...
- **`--with-compute-unit-price`**: Ustaw opłatę priorytetową w mikro-lamportach
  (0,000001 SOL) za jednostkę obliczeniową. Sprawdź
  [Helius Priority Fee API](https://docs.helius.dev/guides/priority-fee-api),
  aby poznać aktualne stawki. Opłata priorytetowa to dodatkowa opłata płacona
  obecnemu liderowi za priorytetowe potraktowanie twojej transakcji.
...
- **`--use-rpc`**: Wysyłaj transakcje do skonfigurowanego RPC. Ta flaga wymaga
  połączenia RPC
  [ważonego stakami](/developers/guides/advanced/stake-weighted-qos) od
  dostawców takich jak [Triton](https://triton.one/) lub
