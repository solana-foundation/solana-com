...
Następnie zamień zawartość pliku `src/lib.rs` na poniższy kod. Jest to minimalny
...
2. Plik keypair (np. `hello_world-keypair.json`): Klucz publiczny tego keypair
   jest używany jako ID programu podczas wdrażania programu.
...
Następnie przetestuj program, używając paczki `litesvm`. Dodaj poniższe
zależności do `Cargo.toml`.
...
Dodaj poniższy test do `src/lib.rs`, poniżej kodu programu. To jest moduł
...
Uruchom test za pomocą polecenia `cargo test`. Log programu wyświetli "Hello,
world!".
...
rozpocząć lokalnego validatora.

```terminal
$ solana-test-validator
```

Gdy testowy validator jest uruchomiony, uruchom polecenie
...
Programy Solana można aktualizować, wdrażając je ponownie pod tym samym ID
programu. Zaktualizuj program w pliku `src/lib.rs`, aby wyświetlał "Hello,
Solana!" zamiast "Hello, world!".
