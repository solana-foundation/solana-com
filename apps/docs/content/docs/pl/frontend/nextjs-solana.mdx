...
description:
  Skonfiguruj integrację portfela Solana w Next.js z @solana/kit i
  @solana/react, aby wysyłać transfery SOL.
---

Skonfiguruj minimalną integrację portfela Solana w Next.js (App Router) z
`@solana/kit`, wtyczkami kit oraz `@solana/react`. Stworzysz rozwijane menu do
łączenia portfela oraz komponent do transferu SOL.
...
Gdy pojawi się pytanie, zaakceptuj wszystkie domyślne opcje (starter zawiera
Tailwind, którego ten tutorial używa do prostych styli narzędziowych).

<Callout type="info" title="Dwie poprawki w szablonie">
Domyślny wynik `create-next-app` wymaga dwóch drobnych zmian na potrzeby tego samouczka:

- W `tsconfig.json` ustaw `"target": "ES2020"` (lub nowszy). Kwoty transferów
  używają literałów `bigint`, takich jak `1_000_000_000n`, które wymagają
  ES2020.
- W `app/globals.css` usuń wygenerowany blok
  `@media (prefers-color-scheme: dark)` oraz regułę tła `body`. Ten interfejs
  działa tylko w trybie jasnym; domyślne ustawienia ciemnego motywu w przeciwnym
  razie nadpisują klasę narzędziową `bg-white` i sprawiają, że strona staje się
  nieczytelna w trybie ciemnym.
...
Ten samouczek jest skierowany do Kit 7+, pakietów `@solana/kit-plugin-*` w
wersji 0.13+ oraz `@solana/react` 7+. Jeśli menedżer pakietów wyświetla
ostrzeżenia o zależnościach peer z pakietu `@solana-program/*` deklarującego
starszy zakres peer Kit, nadpisz go — te pakiety działają z Kit 7.

<ScrollyCoding>

## !!steps 1. Create Solana Provider

`app/providers.tsx` tworzy jednego klienta w zakresie modułu i udostępnia go za
pomocą `ClientProvider`. Wtyczka `walletSigner` ustawia podłączony portfel jako
płatnika opłat i tożsamość; wyeksportuj typ klienta, aby komponenty mogły
typować `useClient`.
...
Owiń swoją aplikację komponentem `Providers`. Zaktualizuj `app/layout.tsx`, aby
go zaimportować i użyć:
...
To owija całą aplikację kontekstem Solana, dzięki czemu wszystkie komponenty
potomne mogą uzyskać dostęp do klienta i hooków portfela.

## !!steps 3. Przycisk połączenia portfela

Menu rozwijane do łączenia/rozłączania portfeli zgodnych z Wallet Standard.
`useWallets` zwraca portfele wykryte w przeglądarce; `useConnect` /
`useDisconnect` zwracają `ActionResult` — ten komponent używa jego pól
`dispatch`, `isRunning` oraz `error`.
...
![Opcje menu portfela](/assets/docs/frontend/02-wallet-dropdown-options.webp)

![Połączony portfel w menu](/assets/docs/frontend/03-wallet-dropdown-connected.webp)

## !!steps 4. Transfer SOL

Zbuduj instrukcję transferu za pomocą `@solana-program/system` i wyślij ją przez
`client.sendTransaction`. Podłączony portfel podpisuje transakcję i pokrywa
opłaty.
...
Portfele pochodzą z mechanizmu wykrywania Wallet Standard; po połączeniu
transfer SOL wykorzystuje podłączony portfel jako płatnika opłat.
...
Otwórz http://localhost:3000, połącz portfel Devnet i wykonaj transfer SOL.
