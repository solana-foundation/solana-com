...
<Callout type="info">
  Działasz w trybie headless — w trasie API, workerze lub skrypcie bez portfela
  przeglądarkowego? Zamiast surowego keypair użyj backendowego serwisu
  zarządzania kluczami. Zobacz [Keychain](/docs/tools/keychain) oraz
  [Podpisywanie w Produkcji](/docs/core/transactions/signing-in-production).
</Callout>
...
- **Dashboardy czasu rzeczywistego**: Łącz obserwatory (salda, konta, sygnatury)
  z preferowaną biblioteką UI; klient obsługuje rozgałęzianie websocketów i
...
- **Niestandardowe store'y**: Wstrzyknij własny store Zustand, aby hydratować
  ...
  koordynować pracę między kartami przeglądarki.
- **Połączenie z hookami React**: Przekaż skonfigurowaną instancję klienta do
  `@solana/react-hooks`, gdy chcesz ergonomicznych hooków na bazie tego samego
  środowiska uruchomieniowego.
- **Testowalność**: Pojedynczy interfejs klienta można mockować w testach
  jednostkowych, co ułatwia symulowanie odpowiedzi RPC lub sesji portfela bez
  obecności portfela przeglądarkowego.
