...
title: Stały delegat
description:
  Utwórz mint Token Extensions Program z trwałym delegatem, który może przesyłać
  lub spalać tokeny z dowolnego konta dla tego minta.
...
## Czym jest trwały delegat?

Rozszerzenie minta _rs`PermanentDelegate`_ w Token Extensions Program przypisuje
uprawnienia delegata do samego minta.

Trwały delegat to uprawnienie na poziomie minta, które może autoryzować
transfery i spalanie dla dowolnego konta tokenów tego minta. Działa jako
globalny delegat dla wszystkich kont tokenów danego minta, a właściciele kont
tokenów nie mogą odwołać trwałego delegata ze swoich kont tokenów.

Uprawnienie minta może również później zmienić trwałego delegata za pomocą
_rs`SetAuthority`_ używając _rs`AuthorityType::PermanentDelegate`_.

## Jak utworzyć i używać trwałego delegata

Aby utworzyć i używać trwałego delegata:

1. Oblicz rozmiar konta minta i czynsz potrzebny dla minta oraz rozszerzenia
   _rs`PermanentDelegate`_.
2. Utwórz konto minta za pomocą _rs`CreateAccount`_, zainicjalizuj
   _rs`PermanentDelegate`_ i zainicjalizuj mint za pomocą _rs`InitializeMint`_.
3. Wyślij _rs`TransferChecked`_ lub _rs`BurnChecked`_ podpisane przez trwałego
   delegata. Token Program przetwarza te instrukcje w taki sam sposób, jakby
   właściciel konta tokenów je podpisał.

<ScrollyCoding>

## !!steps Oblicz rozmiar konta

Oblicz rozmiar konta minta dla podstawowego minta plus rozszerzenie
_rs`PermanentDelegate`_. Jest to rozmiar używany w _rs`CreateAccount`_.
...
Oblicz czynsz używając rozmiaru potrzebnego dla minta plus rozszerzenie
_rs`PermanentDelegate`_.
...
## !!steps Inicjalizacja PermanentDelegate

Zainicjalizuj rozszerzenie _rs`PermanentDelegate`_ na mint.
...
## !!steps Tworzenie kont tokenów i mintowanie tokenów

Utwórz token account dla właściciela i odbiorcy, następnie mintuj tokeny na
token account właściciela.
...
## !!steps Transfer za pomocą permanent delegate

Przenieś tokeny za pomocą _rs`TransferChecked`_ podpisanego przez permanent
delegate.
...
<Callout type="info" title="Kolejność instrukcji">
  _rs`InitializePermanentDelegate`_ musi być wykonana przed
  _rs`InitializeMint`_. _rs`CreateAccount`_, _rs`InitializePermanentDelegate`_ i
...
| Element                                     | Opis                                                                                                                | Źródło                                                                                                                              |
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermanentDelegate`_                     | Rozszerzenie mint, które przechowuje delegata autoryzowanego dla każdego token account dla danego mint.             | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/permanent_delegate.rs#L15-L21) |
| _rs`InitializePermanentDelegate`_           | Instrukcja inicjalizująca permanent delegate przed _rs`InitializeMint`_.                                            | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1952-L1967)              |
| _rs`SetAuthority`_                          | Podstawowa instrukcja tokena używana z _rs`AuthorityType::PermanentDelegate`_ do rotacji delegata.                  | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1488-L1520)              |
| _rs`AuthorityType::PermanentDelegate`_      | Dyskryminator uprawnień używany z _rs`SetAuthority`_ do rotacji permanent delegate na mint.                         | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1138-L1139)              |
| _rs`process_initialize_permanent_delegate`_ | Logika procesora zapisująca rozszerzenie permanent delegate na mint.                                                | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1542-L1557)                  |
| _rs`process_set_authority`_                 | Logika procesora walidująca i rotująca permanent delegate, gdy używany jest _rs`AuthorityType::PermanentDelegate`_. | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L849-L860)                    |
