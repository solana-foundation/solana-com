...
## Czym jest rozszerzenie Scaled UI Amount?

Rozszerzenie mint _rs`ScaledUiAmount`_ w Token Extensions Program pozwala mint
zastosować aktualizowalny mnożnik do kwoty UI wyświetlanej dla sald tokenów.

Jest to przydatne, gdy token reprezentuje rzeczywisty zasób lub instrument, taki
jak podział akcji lub dywidenda. Na przykład, podział akcji podwaja liczbę akcji
dla wszystkich posiadaczy. Bez zmiany salda każdego konta tokenowego,
_rs`ScaledUiAmount`_ umożliwia portfelom i aplikacjom wyświetlanie nowej kwoty
UI przy użyciu mnożnika mint.

Ta funkcja może być również wykorzystywana do dywidend lub dystrybucji zysków.

Żadne nowe tokeny nie są tworzone. Kwota tokenów przechowywana w kontach
tokenowych pozostaje taka sama. Zmienia się tylko wyświetlana kwota UI.

<Callout type="info" title="Niekompatybilne rozszerzenie">
  Nie włączaj _rs`ScaledUiAmount`_ i _rs`InterestBearingConfig`_ na tym samym
  mint. Token Extensions Program odrzuca taką kombinację rozszerzeń.
</Callout>

## Jak działa Scaled UI Amount

- _rs`ScaledUiAmountConfig`_ przechowuje uprawnienia do aktualizacji,
  `multiplier`, `new_multiplier` oraz `new_multiplier_effective_timestamp`.
- Przed `new_multiplier_effective_timestamp` konwersje używają `multiplier`. W
  momencie tego znacznika czasu lub po nim konwersje używają `new_multiplier`.
- _rs`UpdateMultiplier`_ może zastosować nowy mnożnik natychmiast lub zaplanować
  go na przyszły znacznik czasu Unix.
- _rs`AmountToUiAmount`_, _rs`UiAmountToAmount`_ oraz pomocnicze funkcje
  konwersji rozszerzenia wykorzystują arytmetykę zmiennoprzecinkową dla mint ze
  skalowaną kwotą UI, więc konwersje nie gwarantują dokładnego zaokrąglenia w
  obie strony.

## Jak używać Scaled UI Amount

Aby użyć scaled UI amount:

1. Oblicz rozmiar mint account oraz rent potrzebne dla mint i rozszerzenia
   _rs`ScaledUiAmount`_.
2. Utwórz mint account za pomocą _rs`CreateAccount`_, zainicjalizuj
   _rs`ScaledUiAmount`_ i zainicjalizuj mint za pomocą _rs`InitializeMint`_.
3. Używaj _rs`AmountToUiAmount`_, _rs`UiAmountToAmount`_ lub pomocniczych
   funkcji klienta podczas wyświetlania sald lub konwertowania kwot UI.
4. Użyj _rs`UpdateMultiplier`_, aby zaktualizować wyświetlany mnożnik
   natychmiast lub w przyszłym znaczniku czasu Unix.

<Callout type="info" title="Kolejność instrukcji">
  _rs`ScaledUiAmountMintInstruction::Initialize`_ musi wystąpić przed
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`ScaledUiAmountMintInstruction::Initialize`_ oraz _rs`InitializeMint`_
...
<Cards>
  <Card
    title="Przewodnik dla emitenta"
    href="/docs/tokens/extensions/scaled-ui-amount/issuer-guide"
  >
    Utwórz mint ze skalowaną kwotą interfejsu użytkownika i aktualizuj mnożnik w
    czasie.
  </Card>
  <Card
  ...
  >
    Konwertuj kwoty tokenów do wyświetlania i obsługuj skalowaną kwotę
    interfejsu użytkownika w portfelach i aplikacjach.
  </Card>
</Cards>
...
| Element                                               | Opis                                                                                                                                                                        | Źródło                                                                                                                                        |
| ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`ScaledUiAmountConfig`_                            | Stan rozszerzenia mint przechowujący uprawnienia, aktualny mnożnik, następny mnożnik oraz znacznik czasu jego aktywacji.                                                    | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L41-L55)         |
| _rs`ScaledUiAmountMintInstruction::Initialize`_       | Instrukcja inicjalizująca _rs`ScaledUiAmount`_ na mint przed _rs`InitializeMint`_.                                                                                          | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/instruction.rs#L24-L42) |
| _rs`ScaledUiAmountMintInstruction::UpdateMultiplier`_ | Instrukcja ustawiająca nowy mnożnik oraz znacznik czasu Unix, kiedy ma on wejść w życie.                                                                                    | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/instruction.rs#L43-L66) |
| _rs`amount_to_ui_amount`_                             | Funkcja pomocnicza konwertująca kwotę tokenu na ciąg znaków kwoty interfejsu użytkownika przy użyciu aktualnego mnożnika, miejsc dziesiętnych i znacznika czasu.            | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L69-L84)         |
| _rs`try_ui_amount_into_amount`_                       | Funkcja pomocnicza konwertująca ciąg znaków kwoty interfejsu użytkownika z powrotem na kwotę tokenu przy użyciu aktualnego mnożnika, miejsc dziesiętnych i znacznika czasu. | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/scaled_ui_amount/mod.rs#L86-L107)        |
| _rs`process_initialize`_                              | Procesor inicjalizujący _rs`ScaledUiAmountConfig`_ i zapisujący początkowy mnożnik na niezainicjalizowanym mint.                                                            | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/scaled_ui_amount/processor.rs#L37-L57)     |
| _rs`process_update_multiplier`_                       | Procesor weryfikujący uprawnienia, aktualizujący zaplanowany mnożnik i stosujący go natychmiast, gdy jest to odpowiednie.                                                   | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/scaled_ui_amount/processor.rs#L59-L108)    |
| _rs`process_amount_to_ui_amount`_                     | Ścieżka konwersji Token Program wykorzystująca _rs`ScaledUiAmountConfig`_, gdy mint ma włączone rozszerzenie.                                                               | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1548-L1571)                            |
| _rs`process_ui_amount_to_amount`_                     | Ścieżka konwersji Token Program konwertująca kwotę interfejsu użytkownika z powrotem na kwotę tokenu dla mint ze skalowaną kwotą interfejsu użytkownika.                    | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1576-L1595)                            |
