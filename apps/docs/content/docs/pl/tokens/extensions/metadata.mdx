...
description:
  Utwórz mint, który przechowuje metadane bezpośrednio na koncie mint przy użyciu rozszerzeń MetadataPointer i TokenMetadata, a następnie aktualizuj, usuwaj i blokuj te metadane poprzez interfejs token-metadata.
...
przykłady wykorzystujące `@solana/web3.js`, `@solana/spl-token` oraz
`@solana/spl-token-metadata` zostały dołączone w celach informacyjnych. W starszych przykładach
`maxSupportedTransactionVersion` jest ograniczone do `0`, ponieważ web3.js v1 nie potrafi deserializować
