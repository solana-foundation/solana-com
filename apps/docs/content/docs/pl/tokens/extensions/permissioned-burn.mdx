...
Lokalny test validator zawiera również starszą wersję Token-2022, dlatego aby
...
## Jak utworzyć mint i spalanie z uprawnieniami
...
3. Utwórz token account i mintuj tokeny.
4. Spal za pomocą _rs`PermissionedBurnInstruction::BurnChecked`_ podpisanego
   przez właściciela token account oraz przez organ spalania.
5. Opcjonalnie wyłącz uprawnione spalanie za pomocą _rs`SetAuthority`_ używając
   _rs`AuthorityType::PermissionedBurn`_ i uprawnienia `None`, co ponownie
   włącza standardowe instrukcje spalania. Poniższe pełne przykłady kodu
   pokazują ten krok.
...
Oblicz rozmiar mint account dla podstawowego mint oraz rozszerzenia
...
Oblicz rent używając rozmiaru potrzebnego dla mint oraz rozszerzenia
_rs`PermissionedBurnConfig`_.
...
Spalaj tokeny za pomocą _rs`PermissionedBurnInstruction::BurnChecked`_
...
<Callout type="info" title="Kolejność instrukcji">
  _rs`PermissionedBurnInstruction::Initialize`_ musi wystąpić przed
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`PermissionedBurnInstruction::Initialize`_ i _rs`InitializeMint`_ muszą być
  zawarte w tej samej transakcji.
</Callout>
...
| Element                                        | Opis                                                                                                                                 | Źródło                                                                                                                                         |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermissionedBurnConfig`_                   | Rozszerzenie minta przechowujące uprawnienie wymagane do współpodpisywania każdego spalenia dla minta.                               | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/mod.rs#L16-L25)         |
...
| _rs`AuthorityType::PermissionedBurn`_          | Dyskryminator uprawnień używany z _rs`SetAuthority`_ do rotacji lub wyłączania burn authority na mincie.                             | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
...
| _rs`process_burn`_                             | Logika procesora odrzucająca standardowe spalenia, gdy burn authority jest ustawione, i weryfikująca podpis uprawnienia.             | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
| _rs`process_set_authority`_                    | Logika procesora walidująca i rotująca burn authority, gdy używany jest _rs`AuthorityType::PermissionedBurn`_.                       | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L996-L1008)                              |

### Typescript

Poniższy przykład `Kit` używa wygenerowanych instrukcji bezpośrednio.
Opublikowany starszy pakiet `@solana/spl-token` nie obsługuje jeszcze
uprzywilejowanego spalania, dlatego nie dołączono przykładu dla starszej wersji.
