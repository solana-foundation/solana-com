...
ustawia domyślny stan dla wszystkich nowych token account tworzonych dla danego
mintu. Po włączeniu ta rozszerzenie automatycznie inicjalizuje każdy nowy token
account z określonym stanem, na przykład zamrożonym. Pozwala to twórcom tokenów
