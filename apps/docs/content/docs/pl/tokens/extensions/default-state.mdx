...
title: Domyślny stan
description:
  Skonfiguruj mint Token Extensions Program tak, aby nowo tworzone konta tokenów
  rozpoczynały w wybranym stanie domyślnym, takim jak zamrożony.
...
## Czym jest domyślny stan konta?

Rozszerzenie mint _rs`DefaultAccountState`_ w Token Extensions Program ustawia
początkowy _rs`AccountState`_ dla każdego nowego konta tokenu tworzonego dla
tego mint.

Najczęściej jest to wykorzystywane do uruchamiania nowych kont tokenów w stanie
zamrożonym. Konta tokenów, które rozpoczynają w stanie zamrożonym, nie mogą być
używane, dopóki freeze authority mint ich nie odblokuje.

## Jak utworzyć mint z domyślnym stanem konta

Aby utworzyć mint z domyślnym stanem konta:

1. Oblicz rozmiar konta mint oraz rent potrzebny dla mint i rozszerzenia
   _rs`DefaultAccountState`_.
2. Utwórz konto mint za pomocą _rs`CreateAccount`_, zainicjalizuj
   _rs`DefaultAccountState`_ i zainicjalizuj mint za pomocą
   _rs`InitializeMint`_.
3. Utwórz konta tokenów dla mint i zaobserwuj, że każde nowe konto tokenu
   rozpoczyna w aktualnym stanie domyślnym.
4. Użyj _rs`UpdateDefaultAccountState`_, aby zmienić stan, w jakim przyszłe
   konta tokenów powinny się rozpoczynać.

<ScrollyCoding>

## !!steps Oblicz rozmiar konta

Oblicz rozmiar konta mint dla podstawowego mint plus rozszerzenie
_rs`DefaultAccountState`_. Jest to rozmiar używany w _rs`CreateAccount`_.
...
## !!steps Inicjalizacja DefaultAccountState

Zainicjuj rozszerzenie _rs`DefaultAccountState`_ na mincie.
...
## !!steps Inicjalizacja minta

Zainicjuj mint z _rs`InitializeMint`_ w tej samej transakcji.
...
## !!steps Utworzenie konta tokenów

Utwórz konto tokenów dla minta. Ponieważ aktualny stan domyślny to _rs`Frozen`_,
nowe konto tokenów rozpoczyna w stanie zamrożonym.
...
## !!steps Aktualizacja domyślnego stanu konta

Użyj _rs`UpdateDefaultAccountState`_, aby zmienić stan domyślny, dzięki czemu
przyszłe konta tokenów nie będą już rozpoczynać w stanie zamrożonym.
...
<Callout type="info" title="Kolejność instrukcji">
  _rs`DefaultAccountStateInstruction::Initialize`_ musi wystąpić przed
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`DefaultAccountStateInstruction::Initialize`_ i _rs`InitializeMint`_ muszą
...
| Element                                          | Opis                                                                                                              | Źródło                                                                                                                                             |
| ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`DefaultAccountState`_                        | Rozszerzenie minta, które przechowuje stan, jaki powinny dziedziczyć nowe konta tokenów.                          | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/mod.rs#L16-L21)         |
| _rs`DefaultAccountStateInstruction::Initialize`_ | Instrukcja, która inicjalizuje domyślny stan konta tokenów minta przed _rs`InitializeMint`_.                      | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L20-L36) |
| _rs`DefaultAccountStateInstruction::Update`_     | Instrukcja, która pozwala uprawnieniu do zamrażania zmienić stan domyślny dla przyszłych kont tokenów.            | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L37-L53) |
| _rs`process_initialize_default_account_state`_   | Logika procesora, która zapisuje początkowy stan domyślny na niezainicjalizowanym mincie.                         | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L29-L40)     |
| _rs`process_update_default_account_state`_       | Logika procesora, która weryfikuje uprawnienie do zamrażania przed zmianą przechowywanego stanu domyślnego minta. | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L43-L74)     |

### Typescript

Poniższy przykład `Kit` używa wygenerowanych instrukcji bezpośrednio. Starsze
przykłady używające `@solana/web3.js` i `@solana/spl-token` są załączone dla
odniesienia.
