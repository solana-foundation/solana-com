...
description:
  Zainicjuj konto tokena Token Extensions Program z niezmiennym właścicielem,
  aby jego właściciel nigdy nie mógł zostać ponownie przypisany.
...
## Czym jest Immutable Owner?

Rozszerzenie konta _rs`ImmutableOwner`_ w Token Extensions Program sprawia, że
właściciel konta tokena staje się trwały.

Po zainicjowaniu konta tokena z _rs`ImmutableOwner`_, późniejsze instrukcje
_rs`SetAuthority`_ nie mogą zmienić jego uprawnień _rs`AccountOwner`_.

<Callout type="info" title="Associated Token Accounts">
  Konta associated token account utworzone za pomocą Associated Token Program
  dla Token Extensions Program automatycznie inicjują _rs`ImmutableOwner`_ na
  koncie tokena.
</Callout>

## Jak zainicjować konto Immutable Owner

Aby zainicjować konto immutable owner:

1. Utwórz i zainicjuj mint.
2. Utwórz konto tokena z wystarczającą przestrzenią dla _rs`ImmutableOwner`_.
3. Zainicjuj _rs`ImmutableOwner`_ na koncie tokena przed
   _rs`InitializeAccount`_.
4. Próba późniejszej zmiany właściciela konta za pomocą _rs`SetAuthority`_
   kończy się niepowodzeniem z błędem _rs`TokenError::ImmutableOwner`_.

<ScrollyCoding>

## !!steps Oblicz rozmiar konta tokena

Oblicz rozmiar konta tokena dla podstawowego konta tokena wraz z rozszerzeniem
_rs`ImmutableOwner`_. To jest rozmiar używany w _rs`CreateAccount`_.
...
Oblicz rent używając rozmiaru konta tokena potrzebnego dla rozszerzenia
...
## !!steps Utwórz konto tokena

Utwórz konto tokena z obliczoną przestrzenią i lamportami.
...
## !!steps Inicjalizacja ImmutableOwner

Zainicjuj rozszerzenie _rs`ImmutableOwner`_ na koncie tokenów.
...
## !!steps Inicjalizacja konta tokenów

Zainicjuj konto tokenów za pomocą _rs`InitializeAccount`_ w tej samej
transakcji.
...
<Callout type="info" title="Kolejność instrukcji">
  _rs`InitializeImmutableOwner`_ musi być wykonane przed
  _rs`InitializeAccount`_. _rs`CreateAccount`_, _rs`InitializeImmutableOwner`_ i
  _rs`InitializeAccount`_ muszą być zawarte w tej samej transakcji.
</Callout>
...
| Element                                  | Opis                                                                                                    | Źródło                                                                                                                           |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| _rs`ImmutableOwner`_                     | Rozszerzenie konta, które oznacza właściciela konta tokenów jako niezmiennego.                          | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L13-L16) |
| _rs`InitializeImmutableOwner`_           | Instrukcja inicjalizująca rozszerzenie niezmiennego właściciela na niezainicjalizowanym koncie tokenów. | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1982-L1991)           |
| _rs`process_initialize_immutable_owner`_ | Logika procesora alokująca i inicjalizująca _rs`ImmutableOwner`_ na koncie.                             | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1532-L1543)               |
| _rs`SetAuthority`_                       | Podstawowa instrukcja tokenów służąca do zmiany uprawnień, w tym właściciela konta tokenów.             | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1611-L1633)           |
| _rs`process_set_authority`_              | Logika procesora odrzucająca zmiany właściciela, gdy konto tokenów ma włączone _rs`ImmutableOwner`_.    | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L703-L900)                 |
