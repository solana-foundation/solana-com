...
description:
  Wymagaj, aby przychodzące transfery zawierały notatkę, włączając rozszerzenie
  konta MemoTransfer na koncie tokenu Token Extensions Program.
...
## Czym jest Transfer z notatką?

Rozszerzenie konta _rs`MemoTransfer`_ programu Token Extensions Program wymaga
instrukcji notatki bezpośrednio przed każdym przychodzącym transferem na to
konto tokenu. Transfery wychodzące z tego konta tokenu nie wymagają notatki.

Rozszerzenie jest konfigurowane na docelowym koncie tokenu, a nie na mincie. Po
włączeniu, transfery bez notatki kończą się błędem _rs`TokenError::NoMemo`_,
dopóki właściciel konta tokenu nie wyłączy tego wymagania.

## Jak utworzyć konto tokenu z włączonym MemoTransfer

Aby utworzyć konto tokenu z włączonym _rs`MemoTransfer`_:

1. Utwórz i zainicjalizuj mint.
2. Oblicz rozmiar konta tokenu oraz rent potrzebny dla konta tokenu z
   rozszerzeniem _rs`MemoTransfer`_.
3. Utwórz konto tokenu i włącz _rs`MemoTransfer`_ na tym koncie.
4. Instrukcje _rs`Transfer`_ i _rs`TransferChecked`_ na to konto tokenu muszą
   zawierać instrukcję notatki bezpośrednio przed instrukcją transferu.

<ScrollyCoding>

## !!steps Utwórz i zainicjalizuj mint

Utwórz i zainicjalizuj mint, którego będzie używać konto tokenu.
...
## !!steps Oblicz rozmiar konta tokenu

Oblicz rozmiar konta tokenu dla podstawowego konta tokenu wraz z rozszerzeniem
_rs`MemoTransfer`_. Jest to rozmiar używany w _rs`CreateAccount`_.
...
Oblicz rent, używając rozmiaru konta tokenu potrzebnego dla rozszerzenia
_rs`MemoTransfer`_.
...
## !!steps Utwórz i zainicjuj konto tokena

Utwórz konto tokena z obliczoną przestrzenią i lamportami, a następnie zainicjuj
je dla minta.
...
## !!steps Włącz MemoTransfer

Włącz _rs`MemoTransfer`_ na koncie tokena.
...
| Element                                         | Opis                                                                                                      | Źródło                                                                                                                                     |
| ----------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`MemoTransfer`_                              | Rozszerzenie konta przechowujące informację, czy przychodzące przelewy wymagają notatki.                  | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/mod.rs#L17-L22)         |
| _rs`RequiredMemoTransfersInstruction::Enable`_  | Instrukcja włączająca wymóg notatki dla przelewów na koncie tokena.                                       | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L50-L71) |
| _rs`RequiredMemoTransfersInstruction::Disable`_ | Instrukcja wyłączająca wymóg notatki dla przelewów na koncie tokena.                                      | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L74-L95) |
| _rs`process_toggle_required_memo_transfers`_    | Logika procesora inicjalizująca lub nadpisująca rozszerzenie _rs`MemoTransfer`_ po weryfikacji własności. | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/memo_transfer/processor.rs#L20-L48)     |

### Typescript

Poniższy przykład `Kit` używa wygenerowanych instrukcji bezpośrednio. Starsze
przykłady wykorzystujące `@solana/web3.js`, `@solana/spl-token` i
`@solana/spl-memo` zostały dołączone jako odniesienie.
