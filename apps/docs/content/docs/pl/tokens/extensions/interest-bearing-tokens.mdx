...
pozwala bezpośrednio ustawić stopę procentową na mint account. Odsetki są
naliczane w sposób ciągły na podstawie znacznika czasu sieci, ale faktyczny stan
tokenów zapisany w każdym token account pozostaje niezmieniony. Całkowita
wartość (saldo tokenów plus narosłe odsetki) to jedynie wyliczenie. Wraz z tą
rozszerzeniem nie są mintowane żadne nowe tokeny.
