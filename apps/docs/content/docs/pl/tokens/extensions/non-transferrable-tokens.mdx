...
zapobiega transferowi tokenów między token account po ich wybiciu. Po wybiciu
tokeny pozostają na stałe na token account. Jednak właściciel token account
nadal może spalić tokeny i zamknąć token account, gdy saldo osiągnie zero.
