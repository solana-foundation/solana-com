...
title: Pausable Mint
description:
  Utwórz mint Token-2022 z uprawnieniem do wstrzymania, blokuj transfery w
  czasie wstrzymania mintu i wznów aktywność później.
...
## Czym jest wstrzymywalny mint?

Rozszerzenie mintu _rs`PausableConfig`_ w programie Token Extensions daje
uprawnieniu do wstrzymania możliwość wstrzymywania i wznawiania aktywności
tokenów dla całego mintu.

Kiedy mint jest wstrzymany, program Token Extensions odrzuca:

- Transfery
- Mintowanie
- Spalanie

Kiedy mint zostanie wznowiony, te instrukcje działają znowu normalnie.

## Jak utworzyć, wstrzymać i wznowić mint

Aby utworzyć, wstrzymać i wznowić mint:

1. Oblicz rozmiar konta mintu i wymagany depozyt dla mintu oraz rozszerzenia
   _rs`PausableConfig`_.
2. Utwórz konto mintu za pomocą _rs`CreateAccount`_, zainicjalizuj
   _rs`PausableConfig`_ i zainicjalizuj mint za pomocą _rs`InitializeMint`_.
3. Mintuj tokeny i utwórz konta tokenów dla tego mintu.
4. Użyj _rs`Pause`_, aby zablokować aktywność tokenów dla mintu.
5. Użyj _rs`Resume`_, aby ponownie zezwolić na aktywność tokenów.

<ScrollyCoding>

## !!steps Oblicz rozmiar konta

Oblicz rozmiar konta mintu dla podstawowego mintu oraz rozszerzenia
_rs`PausableConfig`_. Jest to rozmiar używany w _rs`CreateAccount`_.
...
## !!steps Oblicz depozyt

Oblicz depozyt używając rozmiaru potrzebnego dla mintu oraz rozszerzenia
_rs`PausableConfig`_.
...
## !!steps Inicjalizacja PausableConfig

Zainicjalizuj rozszerzenie _rs`PausableConfig`_ dla minta.
...
## !!steps Wstrzymanie minta

Wstrzymaj mint za pomocą _rs`Pause`_.
...
## !!steps Wznowienie minta

Wznów mint za pomocą _rs`Resume`_.
...
<Callout type="info" title="Kolejność instrukcji">
  _rs`PausableInstruction::Initialize`_ musi występować przed
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`PausableInstruction::Initialize`_ oraz _rs`InitializeMint`_ muszą być
...
| Element                               | Opis                                                                                                                            | Źródło                                                                                                                                |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PausableConfig`_                  | Rozszerzenie minta przechowujące uprawnienia do wstrzymania oraz informację, czy mint jest obecnie wstrzymany.                  | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/mod.rs#L17-L22)         |
| _rs`PausableInstruction::Initialize`_ | Instrukcja inicjalizująca konfigurację pausable przed _rs`InitializeMint`_.                                                     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L20-L32) |
| _rs`PausableInstruction::Pause`_      | Instrukcja oznaczająca mint jako wstrzymany.                                                                                    | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L33-L44) |
| _rs`PausableInstruction::Resume`_     | Instrukcja oznaczająca mint jako niewstrzymany.                                                                                 | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L45-L56) |
| _rs`process_initialize`_              | Logika procesora inicjalizująca _rs`PausableConfig`_ na niezainicjalizowanym mincie i przechowująca uprawnienia do wstrzymania. | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L22-L35)     |
| _rs`process_toggle_pause`_            | Logika procesora weryfikująca uprawnienia do wstrzymania przed przełączeniem flagi wstrzymania minta.                           | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L39-L64)     |

### Typescript

Poniższy przykład `Kit` wykorzystuje bezpośrednio wygenerowane instrukcje.
Starsze przykłady używające `@solana/web3.js` oraz `@solana/spl-token` zostały
