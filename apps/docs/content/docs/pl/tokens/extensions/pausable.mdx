---
title: Pausable Mint
description: Dowiedz się, jak włączyć rozszerzenie PausableMintExtension.
---

## Jak włączyć PausableExtension
...
pozwala wyznaczonemu autorytetowi pauzy wstrzymać wszystkie operacje na tokenie
dla danej emisji (mint). Autorytet pauzy może w dowolnym momencie wstrzymać lub
wznowić emisję.

Gdy emisja jest wstrzymana, program Token-2022 odrzuca wszystkie transfery,
emisje (mint) i spalania (burn) tego tokena. Po wznowieniu, normalne operacje są
przywracane.

To powszechny wzorzec zarówno w systemach tokenów blockchain, jak i w
tradycyjnych finansach, gdzie możliwość zamrożenia aktywności jest potrzebna ze
względów zgodności, reagowania na incydenty lub kontroli administracyjnej.
