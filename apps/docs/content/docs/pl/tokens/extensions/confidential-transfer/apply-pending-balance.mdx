...
<Callout type="warn">
  ...
  trwającego audytu bezpieczeństwa. Oznacza to, że rozszerzenie poufnych
...
Zanim tokeny będą mogły być przesyłane w sposób poufny, publiczne saldo tokenów
musi zostać przekształcone w saldo poufne. Ta konwersja odbywa się w dwóch
etapach:

1. **Poufne oczekujące saldo**: Na początku tokeny są „deponowane” z publicznego
   salda do „oczekującego” poufnego salda.
2. **Poufne dostępne saldo**: Następnie oczekujące saldo jest „zastosowane” do
   dostępnego salda, dzięki czemu tokeny są dostępne do poufnych transferów.

Ta sekcja wyjaśnia drugi etap: zastosowanie oczekującego salda do salda
dostępnego.

Gdy tokeny są „deponowane” z publicznego salda lub gdy tokeny są przesyłane
poufnie z jednego token account do innego, tokeny są początkowo dodawane do
poufnego oczekującego salda. Zanim tokeny będą mogły być użyte do poufnych
...
Aby przekształcić oczekujące saldo w saldo dostępne, wywołaj instrukcję
...
Pakiet `spl_token_client` odostępnia metodę
`confidential_transfer_apply_pending_balance`, która buduje i wysyła transakcję
z instrukcją `ApplyPendingBalance`, jak pokazano w przykładzie poniżej.
...
Aby uruchomić ten przykład, uruchom lokalny validator z Token Extension Program
sklonowanym z mainnetu, używając poniższego polecenia. Do uruchomienia lokalnego
