...
- **authority**: Konto posiadające uprawnienia do zmiany ustawień poufnych
  przelewów dla mintu oraz zatwierdzania nowych kont poufnych, jeśli
  automatyczne zatwierdzanie jest wyłączone.

- **auto_approve_new_accounts**: Gdy ustawione na true, użytkownicy mogą tworzyć
  token accounts z domyślnie włączonymi przelewami poufnymi. Gdy ustawione na
  false, authority musi zatwierdzić każdy nowy token account przed jego użyciem
  do przelewów poufnych.
...
Utworzenie mintu z włączonym Confidential Transfer wymaga trzech instrukcji w
jednej transakcji:
...
2. **Zainicjalizuj Rozszerzenie Confidential Transfer**: Wywołaj instrukcję
   ...
   Token Extensions Program, aby skonfigurować stan
   _rs`ConfidentialTransferMint`_ dla mintu.
...
validator z forkiem mainnet, takie jak [Surfpool](https://surfpool.run), tak.
