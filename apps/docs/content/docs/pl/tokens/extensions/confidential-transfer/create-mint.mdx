...
title: Utwórz mint
description:
  Dowiedz się, jak utworzyć mint tokena z włączoną funkcją Confidential Balances.
---

## Jak utworzyć mint z Confidential Balances

Confidential Balances umożliwia prywatne transfery tokenów poprzez dodanie dodatkowego stanu do
mint account. Ta sekcja wyjaśnia, jak utworzyć mint tokena z
wymaganym rozszerzeniem włączonym.

Poniższy diagram przedstawia kroki związane z tworzeniem mintu z rozszerzeniem
...
### Stan Mintu Confidential Balances
...
- **authority**: Konto posiadające uprawnienia do zmiany ustawień Confidential Balances
  dla mintu oraz zatwierdzania nowych poufnych kont, jeśli auto-zatwierdzanie
  jest wyłączone.

- **auto_approve_new_accounts**: Gdy ustawiono na true, użytkownicy mogą tworzyć token
  accounts z domyślnie włączoną funkcją Confidential Balances. Gdy ustawiono na false, authority
  musi zatwierdzić każdy nowy token account przed jego użyciem do
  poufnych transferów.

- **auditor_elgamal_pubkey**: Opcjonalny audytor, który może odszyfrowywać kwoty
  przelewów w poufnych transakcjach, zapewniając mechanizm zgodności przy
  jednoczesnym zachowaniu prywatności wobec ogółu użytkowników.

### Wymagane Instrukcje

Utworzenie mintu z włączoną funkcją Confidential Balances wymaga trzech instrukcji
w ramach jednej transakcji:

1. **Utwórz Mint Account**: Wywołaj instrukcję _rs`CreateAccount`_ System
   Program, aby utworzyć mint account.

2. **Inicjalizacja Confidential Balances**: Wywołaj instrukcję
   ...
   Token Extension Program, aby skonfigurować stan _rs`ConfidentialTransferMint`_ dla
   mintu.

3. **Zainicjalizuj Mint**: Wywołaj instrukcję _rs`Instruction::InitializeMint`_
   Token Extensions Program, aby zainicjalizować standardowy stan mintu.
...
Poniższy kod demonstruje, jak utworzyć mint z włączoną funkcją Confidential Balances.

Confidential Balances zależy od programu ZK ElGamal Proof, który jest włączony
na mainnet i devnet. Standardowy `solana-test-validator` nie włącza go, ale
lokalny validator z forkiem mainnetu, taki jak [Surfpool](https://surfpool.run), tak.
Uruchom przykład na jednym z nich (kod używa devnet) z zasilonym kontem płatnika
i zastąp przykładowe adresy mintu oraz konta własnymi.
