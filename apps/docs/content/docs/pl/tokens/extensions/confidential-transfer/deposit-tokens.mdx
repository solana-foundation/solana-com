...
zostać przekonwertowane na saldo poufne. Konwersja przebiega w dwóch etapach:

1. **Poufne saldo oczekujące**: Początkowo tokeny są „wpłacane“ z salda
   publicznego na „oczekujące“ saldo poufne.
2. **Poufne saldo dostępne**: Saldo oczekujące jest następnie „stosowane“ do
   salda dostępnego, udostępniając tokeny do poufnych transferów.

Ta sekcja opisuje pierwszy etap: wpłacanie publicznego salda tokenów na poufne
saldo oczekujące.

Poniższy diagram przedstawia kroki związane z wpłacaniem tokenów z salda
publicznego na poufne saldo oczekujące:
...
Aby przekonwertować saldo publiczne na poufne saldo oczekujące, wywołaj
instrukcję
...
na instrukcję wpłaty jest ograniczona do 2^48.

Skrzynka `spl_token_client` udostępnia metodę `confidential_transfer_deposit`,
która buduje i wysyła transakcję z instrukcją `Deposit`, co zostało
...
Confidential Balances opiera się na programie ZK ElGamal Proof, który jest włączony
w sieci mainnet i devnet. Standardowy `solana-test-validator` nie obsługuje go, ale
lokalny validator z forkiem mainnet, taki jak [Surfpool](https://surfpool.run), już tak.
Uruchom przykład na jednej z tych sieci (kod używa devnet) z zasilonym kontem płatnika
