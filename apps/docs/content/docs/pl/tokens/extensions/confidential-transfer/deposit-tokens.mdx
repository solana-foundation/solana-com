...
Poufne transfery opierają się na programie ZK ElGamal Proof, który jest włączony
w sieci mainnet i devnet. Standardowy `solana-test-validator` nie włącza go, ale
lokalny validator z forkiem mainnet, taki jak [Surfpool](https://surfpool.run),
tak. Uruchom przykład na jednym z nich (kod używa devnet) z zasilonym kontem
