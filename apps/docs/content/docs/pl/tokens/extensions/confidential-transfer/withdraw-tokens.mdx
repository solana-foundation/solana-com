...
1. Utwórz
   ...
   **Dowód równości
   ...
   zapewniając poprawne obliczenie nowego dostępnego salda konta jako
...
2. Dla każdego dowodu:
   ...
   - Zapisz metadane specyficzne dla dowodu w koncie "stanu kontekstu" dowodu,
     aby użyć ich w innych instrukcjach.
...
- Wywołać program dowodów Zk ElGamal w celu weryfikacji dowodów i zainicjowania
  kont "stanu kontekstu"
- Wywołać instrukcję
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493)
  podając dwa konta dowodowe.
...
Poniższy przykład wypłaca tokeny z poufnego dostępnego salda
z powrotem do salda publicznego. Konto musi być już skonfigurowane dla
Poufnych Sald i posiadać dostępne poufne saldo.

Poufne Salda zależą od programu ZK ElGamal Proof, który jest włączony
w mainnet i devnet. Standardowy `solana-test-validator` go nie włącza, ale
lokalne środowisko walidacyjne z forkiem mainnet, takie jak [Surfpool](https://surfpool.run), już tak.
Uruchom przykład na jednym z nich (kod używa devnet) z zasilonym płatnikiem
i zastąp elementy zastępcze swoim mint i token account.
