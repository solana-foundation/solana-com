...
<Callout type="warn">
  Program ZK ElGamal jest tymczasowo wyłączony na mainnecie i devnecie ze
  względu na trwający audyt bezpieczeństwa. Oznacza to, że rozszerzenie poufnych
...
1. Wygeneruj
   ...
   **Dowód równości
   ...
   Weryfikuje, że szyfrogram dostępnego salda po wypłacie odpowiada
   odpowiedniemu
   [zaangażowaniu Pedersena](https://pl.wikipedia.org/wiki/Schemat_zaanga%C5%BCowania),
   zapewniając, że nowe dostępne saldo na koncie zostało poprawnie obliczone
   jako `remaining_balance = current_balance - withdraw_amount`.
...
2. Dla każdego dowodu:
   ...
   - Zapisz metadane specyficzne dla dowodu w koncie "context state", aby użyć
     ich w innych instrukcjach.

3. Wywołaj
   [instrukcję ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493),
   przekazując dwa konta dowodów.

4. Zamknij konta dowodów, aby odzyskać SOL użyte do ich utworzenia.

Poniższy diagram przedstawia kroki związane z wypłatą tokenów z poufnego
dostępnego salda na saldo publiczne:
...
Aby wypłacić tokeny z poufnego dostępnego salda na saldo publiczne, należy:

- Wygenerować po stronie klienta dowód równości i dowód zakresu
- Wywołać program dowodów ZK ElGamal, aby zweryfikować dowody i zainicjować
  konta "context state"
- Wywołać
  [instrukcję ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493),
  przekazując dwa konta dowodów.
- Zamknąć oba konta dowodów, aby odzyskać rent.

Pakiet `spl_token_client` udostępnia następujące metody:
...
Poniższy przykład pokazuje, jak wypłacić tokeny z poufnego dostępnego salda na
saldo publiczne.

Aby uruchomić przykład, uruchom lokalnego validatora z Token Extension Program
sklonowanym z mainnetu, używając poniższego polecenia. Musisz mieć zainstalowane
Solana CLI, aby uruchomić lokalnego validatora.
...
<Callout type="info">
  W momencie pisania tego tekstu Confidential Transfers nie jest włączone w
  domyślnym lokalnym validatorze. Aby uruchomić przykładowy kod, musisz
  sklonować Token Extension Program z mainnetu.
</Callout>
