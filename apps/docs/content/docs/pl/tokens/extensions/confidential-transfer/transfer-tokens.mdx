...
Aby poufnie przenosić tokeny z jednego token account na inny, zarówno nadawca,
jak i odbiorca muszą posiadać token accounts skonfigurowane ze stanem
_rs`ConfidentialTransferAccount`_ i zatwierdzone do poufnych transferów. Token
account nadawcy musi również posiadać dostępne poufne saldo do przesłania.
...
Poniższy przykład poufnie przesyła tokeny z jednego konta na drugie. Oba konta
muszą być już skonfigurowane do poufnych transferów, a nadawca musi posiadać
dostępne poufne saldo.

Poufne transfery są uzależnione od programu ZK ElGamal Proof, który jest
włączony w sieci mainnet i devnet. Standardowy `solana-test-validator` go nie
włącza, jednak lokalny validator z forkiem mainnet, taki jak
[Surfpool](https://surfpool.run), już tak. Uruchom przykład w jednej z tych
sieci (kod używa devnet) z zasilonym płatnikiem i zastąp symbole zastępcze swoim
mintem oraz kontami nadawcy i odbiorcy.
