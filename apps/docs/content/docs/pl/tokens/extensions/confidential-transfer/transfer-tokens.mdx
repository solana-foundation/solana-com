...
<Callout type="warn">
  ...
  trwającego audytu bezpieczeństwa. Oznacza to, że rozszerzenie do poufnych
  transferów jest obecnie niedostępne. Chociaż koncepcje pozostają aktualne,
  przykłady kodu nie będą działać.
</Callout>

## Jak poufnie przesyłać tokeny z jednego konta tokenów na drugie

Aby poufnie przesłać tokeny z jednego konta tokenów na drugie, zarówno nadawca,
...
tokenów nadawcy musi również posiadać dostępne poufne saldo, z którego można
przesłać środki.
...
1. Utwórz
   ...
   **Dowód równości
   ...
   Weryfikuje, że nowy zaszyfrowany dostępny stan salda po transferze odpowiada
   odpowiedniemu
   [zaangażowaniu Pedersena](https://pl.wikipedia.org/wiki/Schemat_zaanga%C5%BCowania),
   zapewniając, że nowe dostępne saldo na koncie źródłowym zostało poprawnie
   obliczone jako `new_balance = current_balance - transfer_amount`.
   ...
   **Dowód poprawności szyfrogramu
   ...
   wszystkich trzech stron (źródło, cel i audytor), zapewniając, że kwota
   transferu została poprawnie zaszyfrowana pod kluczem publicznym każdej ze
   stron.
   ...
   **Dowód zakresu
   ...
   Weryfikuje, że nowe dostępne saldo oraz kwota transferu (podzielona na
   niższe/wyższe bity) są nieujemne i mieszczą się w określonym zakresie.
...
3. Wywołaj
   [instrukcję ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604),
   przekazując konta context state dowodów.

4. Zamknij konta stanu kontekstu dowodu, aby odzyskać SOL użyte do ich
   utworzenia.

Poniższy diagram przedstawia kroki związane z przesyłaniem tokenów z konta token
account nadawcy na konto token account odbiorcy.
...
Aby poufnie przesłać tokeny z jednego token account na inny, należy:

- Wygenerować po stronie klienta dowód równości, dowód poprawności szyfrogramu
  oraz dowód zakresu
...
- Wywołać instrukcję
  ...
  przekazując trzy konta dowodów.
- Zamknąć trzy konta dowodów, aby odzyskać rent.

Crate `spl_token_client` udostępnia następujące metody:
...
Poniższy przykład pokazuje, jak poufnie przesyłać tokeny między kontami token
account.

Aby uruchomić przykład, uruchom lokalny validator z Token Extension Program
sklonowanym z mainnet, używając poniższego polecenia. Do uruchomienia lokalnego
validatora musisz mieć zainstalowane Solana CLI.
