---
title: Przesyłanie tokenów
description:
  Dowiedz się, jak prywatnie przesyłać tokeny z jednego konta tokenów na drugie.
---

## Jak poufnie przenosić tokeny z jednego token account na inny

Aby poufnie przenieść tokeny z jednego token account na inny, zarówno
nadawca, jak i odbiorca muszą mieć token accounts skonfigurowane ze stanem
_rs`ConfidentialTransferAccount`_ i zatwierdzone do korzystania z Poufnych
Sald. Token account nadawcy musi również posiadać dostępne poufne
saldo do przelania.

Aby poufnie przenieść tokeny:
...
1. Utwórz
   ...
   **Dowód równości
   ...
   Weryfikuje, że nowy szyfrogram dostępnego salda po transferze odpowiada
   odpowiadającemu mu
   ...
   **Dowód poprawności szyfrogramu
   ...
   Weryfikuje, że szyfrogramy kwoty transferu są poprawnie wygenerowane dla
   wszystkich trzech stron (źródła, odbiorcy i audytora), zapewniając, że kwota
   transferu jest prawidłowo zaszyfrowana przy użyciu klucza publicznego każdej
   ze stron.
   ...
   **Dowód zakresu
   ...
   Weryfikuje, że nowe dostępne saldo i kwota transferu (podzielona na bity
   niskie/wysokie) są nieujemne i mieszczą się w określonym zakresie.

2. Dla każdego dowodu:
   - Wywołaj program dowodów ZK ElGamal, aby zweryfikować dane dowodu.
   - Zapisz metadane specyficzne dla dowodu w koncie "context state" dowodu, aby
     użyć ich w innych instrukcjach.
...
3. Wywołaj instrukcję
   ...
   podając konta stanu kontekstu dowodów.

4. Zamknij konta stanu kontekstu dowodów, aby odzyskać SOL użyte do ich
   utworzenia.

Poniższy diagram przedstawia kroki związane z przesyłaniem tokenów z token
account nadawcy do token account odbiorcy.
...
Aby poufnie przenosić tokeny z jednego token account na inny, należy:

- Wygenerować dowód równości, dowód poprawności szyfrogramu i dowód zakresu po
  stronie klienta
- Wywołać program dowodów Zk ElGamal, aby zweryfikować dowody i zainicjować
  konta "context state"
- Wywołać instrukcję
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  podając trzy konta dowodów.
- Zamknąć trzy konta dowodów, aby odzyskać rent.

Poniższy przykład w języku Rust generuje dowody za pomocą skrzynki
`spl-token-confidential-transfer-proof-generation`, weryfikuje każdy z nich w
koncie stanu kontekstu za pośrednictwem programu ZK ElGamal Proof, odwołuje się
do trzech kont w instrukcji transferu i zamyka je po zakończeniu. Przykład w
TypeScript używa pomocnika `getConfidentialTransferInstructionPlan` z
`@solana-program/token-2022/confidential`, który składa te same konta dowodów,
transfer i zamknięcie jako plan instrukcji wielotransakcyjnej.

## Przykładowy kod

Poniższy przykład poufnie przenosi tokeny z jednego konta na
drugie. Oba konta muszą być już skonfigurowane dla Poufnych Sald, a
nadawca musi posiadać dostępne poufne saldo.

Poufne Salda są zależne od programu ZK ElGamal Proof, który jest włączony
w sieci mainnet i devnet. Standardowy `solana-test-validator` nie włącza go, ale
lokalny validator z forkiem sieci mainnet, taki jak [Surfpool](https://surfpool.run), już tak.
Uruchom przykład względem jednej z tych sieci (kod używa devnet) z zasilonym płatnikiem
i zastąp symbole zastępcze swoim mintem oraz kontami nadawcy i odbiorcy.
