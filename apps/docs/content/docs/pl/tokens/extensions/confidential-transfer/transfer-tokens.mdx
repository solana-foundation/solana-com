...
Aby poufnie przenieść tokeny z jednego token account na inny, zarówno nadawca,
jak i odbiorca muszą mieć konta token account skonfigurowane ze stanem
_rs`ConfidentialTransferAccount`_ i zatwierdzone do poufnych transferów. Konto
token account nadawcy musi również posiadać dostępne poufne saldo, z którego
...
- Metoda `confidential_transfer_close_context_state_account`, która zamyka proof
  account.
