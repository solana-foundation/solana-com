...
<Callout type="warn">
  ...
  transferów jest obecnie niedostępne. Chociaż koncepcje pozostają aktualne,
...
ujawniania kwoty transferu. Jest to przydatne w transakcjach chroniących
prywatność. Tylko kwoty transferów i salda tokenów pozostają prywatne. Adresy
kont tokenów pozostają publiczne.

- [Przegląd protokołu](https://www.solana-program.com/docs/confidential-balances/overview)
  – Szczegóły dotyczące zastosowanego protokołu kryptograficznego
- [Szybki start](https://www.solana-program.com/docs/confidential-balances#setup)
  – Konfiguracja i podstawowe polecenia CLI
- [Cookbook poufnych sald](https://github.com/solana-developers/Confidential-Balances-Sample)
  – Przykłady kodu pokazujące, jak korzystać z rozszerzenia poufnych transferów

### Jak to działa?

Rozszerzenie poufnych transferów dodaje
...
do programu Token Extension, które pozwalają przesyłać tokeny między kontami bez
ujawniania kwoty transferu.
...
Podstawowy przebieg poufnych transferów tokenów wygląda następująco:

1. Utwórz mint account z rozszerzeniem poufnych transferów.
2. Utwórz konta tokenów z rozszerzeniem poufnych transferów dla nadawcy i
   odbiorcy.
3. Wymintuj tokeny na konto nadawcy.
4. **Zdeponuj** publiczne saldo nadawcy do **poufnego salda oczekującego**.
5. **Zastosuj** oczekujące saldo nadawcy do **poufnego salda dostępnego**.
6. Poufnie **prześlij** tokeny z konta tokenów nadawcy na konto tokenów
   odbiorcy.
7. **Zastosuj** oczekujące saldo odbiorcy do **poufnego salda dostępnego**.
8. **Wypłać** poufne dostępne saldo odbiorcy do **salda publicznego**.

Aby uzyskać więcej informacji o poszczególnych krokach w procesie poufnego
transferu, zobacz odpowiednie strony:
...
<Cards>
  ...
  >
    Jak zastosować saldo oczekujące do dostępnego salda poufnego
  </Card>
  <Card
    title="Wypłać tokeny"
    href="/docs/tokens/extensions/confidential-transfer/withdraw-tokens"
  >
    Jak wypłacić tokeny z dostępnego salda poufnego
  </Card>
  <Card
  ...
  >
    Jak emitować poufny token transferu i zarządzać nim (polityka zatwierdzania,
    audytorzy, opłaty, emisja i spalanie)
  </Card>
</Cards>

Poniższy diagram przedstawia szczegółową sekwencję podstawowego przebiegu
...
Poniższy przykład w języku Rust pokazuje kroki poufnych transferów tokenów
odpowiadające powyższemu diagramowi. Kroki zostały rozdzielone na funkcje
pomocnicze dla przejrzystości.

Aby uruchomić przykład, uruchom lokalny validator z Token Extension Program
sklonowanym z mainnetu za pomocą poniższego polecenia. Musisz mieć zainstalowane
Solana CLI, aby korzystać z lokalnego validatora.
...
<Callout type="info">
  W momencie pisania tego tekstu poufne transfery nie są domyślnie włączone w
  lokalnym validatorze. Musisz sklonować Token Extension Program z mainnetu, aby
...
## Instrukcje poufnego transferu

Pełna lista instrukcji rozszerzenia poufnego transferu znajduje się
[tutaj](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29):

| Instrukcja                          | Opis                                                                                                                                                       |
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Konfiguruje konto mint do poufnych transferów. Ta instrukcja musi być zawarta w tej samej transakcji co instrukcja _rs`TokenInstruction::InitializeMint`_. |
| _rs`UpdateMint`_                    | Aktualizuje ustawienia poufnych transferów dla mint.                                                                                                       |
| _rs`ConfigureAccount`_              | Konfiguruje konto tokenów do poufnych transferów.                                                                                                          |
| _rs`ApproveAccount`_                | Zatwierdza konto tokenów do poufnych transferów, jeśli mint wymaga zatwierdzenia nowych kont tokenów.                                                      |
| _rs`EmptyAccount`_                  | Opróżnia oczekujące i dostępne poufne salda, aby umożliwić zamknięcie konta tokenów.                                                                       |
...
| _rs`Transfer`_                      | Poufnie przenosi tokeny między kontami tokenów.                                                                                                            |
| _rs`ApplyPendingBalance`_           | Konwertuje saldo oczekujące na dostępne saldo po wpłatach lub transferach.                                                                                 |
| _rs`EnableConfidentialCredits`_     | Pozwala kontu tokenów na odbieranie poufnych transferów tokenów.                                                                                           |
| _rs`DisableConfidentialCredits`_    | Blokuje przychodzące poufne transfery, ale nadal pozwala na publiczne transfery.                                                                           |
| _rs`EnableNonConfidentialCredits`_  | Pozwala kontu tokenów na odbieranie publicznych transferów tokenów.                                                                                        |
| _rs`DisableNonConfidentialCredits`_ | Blokuje zwykłe transfery, aby konto mogło otrzymywać tylko poufne transfery.                                                                               |
| _rs`TransferWithFee`_               | Poufnie przenosi tokeny między kontami tokenów z opłatą.                                                                                                   |
| _rs`ConfigureAccountWithRegistry`_  | Alternatywny sposób konfigurowania kont tokenów do poufnych transferów przy użyciu konta _rs`ElGamalRegistry`_ zamiast dowodu _rs`VerifyPubkeyValidity`_.  |
