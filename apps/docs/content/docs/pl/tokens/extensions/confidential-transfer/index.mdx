...
title: Poufne Salda
description:
  Dowiedz się o rozszerzeniu Poufnych Sald i jak używać go do dodawania opcjonalnych funkcji do mintów tokenów i kont.
---
...
Uprawnienia Poufnych Sald ustawiają klucz audytora podczas inicjalizacji rozszerzenia lub zmieniają go później za pomocą _rs`UpdateMint`_. Uprawnienia mogą rotować lub usuwać klucz. Rotacja dotyczy tylko przyszłych transferów, dlatego należy zachować poprzednie tajne klucze, gdy wymagany jest historyczny dostęp do audytu. Klucze audytora zapewniają dostęp do odczytu; nie autoryzują transferów tokenów ani nie ujawniają pełnego poufnego salda konta.

Przykłady konfiguracji i rotacji można znaleźć w sekcji
[Konfiguracja audytora](/docs/tokens/extensions/confidential-transfer/issuer-guide#configure-an-auditor).
Portfele, eksploratory, giełdy i dostawcy usług compliance powinni również zapoznać się z
[wytycznymi dotyczącymi integracji audytora](/docs/tokens/extensions/confidential-transfer/integration-guide#auditors-and-compliance).
...
| Instrukcja                         | Opis                                                                                                                                                       |
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Włącza Poufne Salda na mint account. Ta instrukcja musi być zawarta w tej samej transakcji co instrukcja _rs`TokenInstruction::InitializeMint`_. |
| _rs`UpdateMint`_                    | Aktualizuje politykę zatwierdzania kont minta oraz opcjonalny globalny klucz publiczny ElGamal audytora.                                                                        |
| _rs`ConfigureAccount`_              | Włącza Poufne Salda na token account.                                                                                                                 |
| _rs`ApproveAccount`_                | Zatwierdza token account dla Poufnych Sald, jeśli mint wymaga zatwierdzenia dla nowych token accounts.                                                          |
...
| _rs`ConfigureAccountWithRegistry`_  | Alternatywny sposób konfigurowania token accounts dla Poufnych Sald przy użyciu konta _rs`ElGamalRegistry`_ zamiast dowodu _rs`VerifyPubkeyValidity`_.         |
