...
- **approved**: Status zatwierdzenia konta dla poufnych transferów. Jeśli
  ...
  `true`, wszystkie token accounts są automatycznie zatwierdzane do poufnych
  transferów.
...
- **maximum_pending_balance_credit_counter**: Limit liczby oczekujących kredytów
  ...
  przekształcenia salda oczekującego na dostępne.
...
- **actual_pending_balance_credit_counter**: Wartość
  `pending_balance_credit_counter` na token account w momencie ostatniego
...
oczekującego. Właściciele token accounts muszą użyć instrukcji
_rs`ApplyPendingBalance`_, aby przekształcić saldo oczekujące na dostępne.
Przychodzące transfery lub depozyty nie wpływają na dostępne saldo token
account.
...
Poufne saldo oczekujące jest podzielone na `pending_balance_lo` oraz
`pending_balance_hi`, ponieważ deszyfrowanie ElGamal wymaga większej mocy
obliczeniowej dla większych liczb. Implementację arytmetyki szyfrogramów
znajdziesz
...
salda oczekującego na dostępne:

1. Klient sprawdza aktualne salda oczekujące i dostępne, szyfruje sumę i
   przekazuje `decryptable_available_balance` zaszyfrowaną kluczem AES
   właściciela token account.

2. Liczniki oczekiwanych i rzeczywistych kredytów śledzą zmiany wartości
   licznika między utworzeniem a przetworzeniem instrukcji
...
Zgodność liczników oczekiwanych/rzeczywistych oznacza, że
`decryptable_available_balance` odpowiada `available_balance`.

Podczas pobierania stanu token account, aby odczytać
`decryptable_available_balance`, różne wartości liczników
oczekiwanych/rzeczywistych wymagają, aby klient wyszukał ostatnie instrukcje
wpłat/przelewów odpowiadające różnicy liczników, aby obliczyć prawidłowe saldo.

**Proces uzgadniania salda**

Gdy liczniki oczekiwane i rzeczywiste dla oczekującego salda się różnią, wykonaj
następujące kroki, aby uzgodnić `decryptable_available_balance`:

1. Zacznij od `decryptable_available_balance` z token account
2. Pobierz najnowsze transakcje zawierające instrukcje wpłat i przelewów do
   różnicy liczników (rzeczywisty - oczekiwany):
...
1. **Utwórz token account**: Wywołaj instrukcję Associated Token Program
   _rs`AssociatedTokenAccountInstruction:Create`_, aby utworzyć token account.

2. **Zwiększ przestrzeń konta**: Wywołaj instrukcję Token Extension Program
   _rs`TokenInstruction::Reallocate`_, aby dodać miejsce na stan
   _rs`ConfidentialTransferAccount`_.

3. **Skonfiguruj poufne transfery**: Wywołaj instrukcję Token Extension Program
   ...
   aby zainicjować stan _rs`ConfidentialTransferAccount`_.
...
szyfrujących i danych dowodowych, które mogą być wygenerowane tylko przez
właściciela token account.

_rs`PubkeyValidityProofData`_ tworzy dowód potwierdzający, że klucz ElGamal jest
prawidłowy. Szczegóły implementacji znajdziesz tutaj:
