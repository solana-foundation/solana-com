...
description:
  Dowiedz się, jak utworzyć token account z włączonymi Poufnymi Saldami.
---

## Jak utworzyć token account z Poufnymi Saldami

Poufne Salda umożliwiają prywatne transfery tokenów poprzez dodanie dodatkowego stanu do
token account. Ta sekcja wyjaśnia, jak utworzyć token account z
wymaganym rozszerzeniem włączonym.

Poniższy diagram przedstawia kroki związane z tworzeniem token account z
włączonymi Poufnymi Saldami:
...
### Stan Token Account dla Poufnych Sald
...
_rs`ConfidentialTransferAccount`_ zawiera kilka pól zarządzających
Poufnymi Saldami:

- **approved**: Status zatwierdzenia konta dla Poufnych Sald. Jeśli
  konfiguracja `auto_approve_new_accounts` mint account jest ustawiona na `true`, wszystkie
  token accounts są automatycznie zatwierdzane dla Poufnych Sald.

- **elgamal_pubkey**: Klucz publiczny ElGamal używany do szyfrowania sald i kwot
  przelewów.

- **pending_balance_lo**: Zaszyfrowane młodsze 16 bitów oczekującego salda.
  Saldo jest podzielone na część wysoką i niską dla wydajnego odszyfrowywania.

- **pending_balance_hi**: Zaszyfrowane starsze 48 bitów oczekującego salda.
  Saldo jest podzielone na część wysoką i niską dla wydajnego odszyfrowywania.

- **available_balance**: Zaszyfrowane saldo dostępne do przelewów.

- **decryptable_available_balance**: Dostępne saldo zaszyfrowane kluczem
  Advanced Encryption Standard (AES) umożliwiające wydajne odszyfrowanie przez
  właściciela konta.

- **allow_confidential_credits**: Jeśli true, zezwala na przychodzące poufne
  przelewy.

- **allow_non_confidential_credits**: Jeśli true, zezwala na przychodzące
  niepoufne przelewy.

- **pending_balance_credit_counter**: Zlicza przychodzące uznania oczekującego
  salda z instrukcji depozytów i przelewów.

- **maximum_pending_balance_credit_counter**: Limit liczby oczekujących uznań
  przed wymaganiem instrukcji _rs`ApplyPendingBalance`_ do przekształcenia
  oczekującego salda w dostępne saldo.

- **expected_pending_balance_credit_counter**: Wartość
  `pending_balance_credit_counter` dostarczona przez klienta poprzez instruction
  data podczas ostatniego przetwarzania instrukcji _rs`ApplyPendingBalance`_.

- **actual_pending_balance_credit_counter**: Wartość
  `pending_balance_credit_counter` na token account w momencie przetworzenia
  ostatniej instrukcji _rs`ApplyPendingBalance`_.

<Callout type="info">
  **Saldo oczekujące a saldo dostępne**

Poufne salda są podzielone na salda oczekujące i dostępne, aby zapobiec atakom
DoS. Bez tego podziału atakujący mógłby wielokrotnie wysyłać tokeny na token
account, blokując właścicielowi token account możliwość transferu tokenów.
Właściciel token account nie byłby w stanie transferować tokenów, ponieważ
zaszyfrowane saldo zmieniałoby się między momentem przesłania transakcji a jej
przetworzeniem, co skutkowałoby niepowodzeniem transakcji.
...
_rs`ApplyPendingBalance`_, aby przekonwertować saldo oczekujące na saldo
...
Podczas wywoływania instrukcji _rs`ApplyPendingBalance`_ w celu
przekonwertowania salda oczekującego na saldo dostępne:

1. Klient pobiera aktualne salda oczekujące i dostępne, szyfruje ich sumę i
   dostarcza `decryptable_available_balance` zaszyfrowane kluczem AES
   właściciela token account.

2. Oczekiwane i rzeczywiste liczniki kredytów oczekujących śledzą zmiany
   wartości licznika między momentem utworzenia a przetworzeniem instrukcji
...
2. Pobierz najnowsze transakcje zawierające instrukcje depozytów i transferów do
...
Tworzenie i konfigurowanie token account dla Poufnych Sald wykorzystuje
następujące instrukcje, które mieszczą się w jednej transakcji:

1. **Utwórz token account**: Wywołaj instrukcję
   _rs`AssociatedTokenAccountInstruction::Create`_ Associated Token Program, aby
   utworzyć token account pod jego deterministycznym adresem.

2. **Przydziel przestrzeń dla konta**: Wywołaj instrukcję
   _rs`TokenInstruction::Reallocate`_ Token Extensions Program, aby dodać
   przestrzeń dla stanu _rs`ConfidentialTransferAccount`_.

3. **Zweryfikuj dowód poprawności pubkey**: Utwórz konto należące do programu ZK
   ElGamal Proof, a następnie wywołaj jego instrukcję _rs`VerifyPubkeyValidity`_
   w celu weryfikacji dowodu i zapisania zweryfikowanego wyniku na tym koncie
   stanu kontekstu.

4. **Konfigurowanie Poufnych Sald**: Wywołaj instrukcję
   ...
   Token Extensions Program, odwołując się do konta stanu kontekstu dowodu poprzez
   _rs`ProofLocation::ContextStateAccount`_, aby zainicjować
   stan _rs`ConfidentialTransferAccount`_.

<Callout type="info">

**Tylko właściciel token account może włączyć Poufne Salda na token
account**.

Instrukcja _rs`ConfigureAccount`_ wymaga generowania po stronie klienta
kluczy szyfrowania oraz dowodu, który może być wygenerowany wyłącznie przez właściciela token account.

Dowód ważności pubkey weryfikuje, że klucz publiczny ElGamal konta jest
prawidłowy. Jest generowany za pomocą _rs`build_pubkey_validity_proof_data`_, weryfikowany on-chain
przez program ZK ElGamal Proof do konta stanu kontekstu, a następnie
odwoływany z _rs`ConfigureAccount`_ poprzez
_rs`ProofLocation::ContextStateAccount`_, dzięki czemu żadne bajty dowodu nie są przesyłane w samej instrukcji tokena. Szczegóły implementacji można znaleźć tutaj:
...
Poniższy kod tworzy associated token account i konfiguruje go do używania
Poufnych Sald z istniejącym kompatybilnym mintem.

Poufne Salda są zależne od programu ZK ElGamal Proof, który jest włączony
w sieci mainnet i devnet. Standardowy `solana-test-validator` go nie włącza, ale
lokalny walidator z forkiem mainnet, taki jak [Surfpool](https://surfpool.run), już tak.
Uruchom przykład na jednej z tych sieci (kod używa devnet) z zasilonym płatnikiem
i zastąp symbol zastępczy minta mintem utworzonym zgodnie z
