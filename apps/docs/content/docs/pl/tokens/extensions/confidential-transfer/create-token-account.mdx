...
description:
  Dowiedz się, jak utworzyć token account z rozszerzeniem Confidential Transfer.
---

## Jak utworzyć token account z rozszerzeniem Confidential Transfer

Rozszerzenie Confidential Transfer umożliwia prywatne przelewy tokenów poprzez
dodanie dodatkowego stanu do token account. W tej sekcji wyjaśniono, jak
utworzyć token account z włączonym tym rozszerzeniem.
...
_rs`ConfidentialTransferAccount`_ zawiera kilka pól do zarządzania poufnymi
przelewami:

- **approved**: Status zatwierdzenia konta dla poufnych przelewów. Jeśli
  konfiguracja `auto_approve_new_accounts` mint account jest ustawiona jako
  `true`, wszystkie token accounts są automatycznie zatwierdzone do poufnych
  przelewów.
...
Tworzenie i konfigurowanie token account do poufnych transferów wykorzystuje
następujące instrukcje, które wszystkie mieszczą się w jednej transakcji:
...
4. **Skonfiguruj poufne transfery**: Wywołaj instrukcję
   ...
   Token Extensions Program, odwołując się do konta stanu kontekstu dowodu
   poprzez _rs`ProofLocation::ContextStateAccount`_, aby zainicjować stan
   _rs`ConfidentialTransferAccount`_.

<Callout type="info">

**Tylko właściciel token account może skonfigurować token account do
confidential transfers**.

Instrukcja _rs`ConfigureAccount`_ wymaga po stronie klienta wygenerowania kluczy
...
poprawny. Generowany jest przy użyciu _rs`build_pubkey_validity_proof_data`_,
weryfikowany onchain przez program ZK ElGamal Proof do konta stanu kontekstu, a
następnie przywoływany z _rs`ConfigureAccount`_ poprzez
_rs`ProofLocation::ContextStateAccount`_, dzięki czemu żadne bajty dowodu nie są
...
Poniższy kod tworzy associated token account i konfiguruje go do poufnych
transferów w oparciu o istniejący poufny mint.
...
[Surfpool](https://surfpool.run), już tak. Uruchom przykład na jednej z tych
sieci (kod używa devnet) z zasilonym kontem płatnika i zastąp symbol zastępczy
mintu mintem utworzonym zgodnie z
[Utwórz Mint](/docs/tokens/extensions/confidential-transfer/create-mint).
