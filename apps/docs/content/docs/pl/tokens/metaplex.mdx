...
Program Token Metadata tworzy Program Derived Address (PDA) dla każdego minta
tokenów. To konto metadanych przechowuje informacje onchain, takie jak nazwa i
symbol tokenu, a także URI wskazujący na metadane JSON off-chain (obrazy, opisy
itp.).
...
Konto metadanych onchain zawiera:
