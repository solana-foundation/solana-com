---
title: Tworzenie konta tokenów
description:
  Twórz konta tokenów i powiązane konta tokenów dla konkretnego mint i właściciela.
...
Konto tokenów przechowuje tokeny dla jednego mint i jednego właściciela konta
tokenów w sieci Solana.
...
Tutaj _rs`owner`_ oznacza uprawnioną stronę, która może przenosić, niszczyć lub
...
identyfikowanego przez pole _rs`mint`_ konta tokenów.

## Czym jest powiązane konto tokenów

Powiązane konto tokenów (ATA) to domyślne token account dla portfela i mint.
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program/src)
wyprowadza adres ATA z adresu portfela, adresu programu tokenów oraz adresu
mint.

Tylko konta tokenów utworzone przez Associated Token Program są nazywane
associated token account.

Associated Token Program to sposób na utworzenie konta tokenów pod standardowym,
deterministycznym adresem. Powstałe konto nadal jest kontem tokenów posiadanym
przez Token Program lub Token Extensions Program, a nie przez Associated Token
Program.

Associated Token Program wyprowadza adres ATA w sposób pokazany w
...
adresem, a powstałe konto nadal wykorzystuje typ _rs`Account`_ zdefiniowany
przez Token Program lub Token Extensions Program.

## Koszt i płatnik

Utworzenie ATA wymaga zwrotnego minimalnego salda na potrzeby przechowywania danych konta w sieci, a także opłaty transakcyjnej. Parametr `payer` przekazany do instrukcji tworzenia finansuje saldo przechowywania i musi złożyć podpis. Płatnik może być inną osobą niż płatnik opłaty transakcyjnej i właściciel portfela, do którego należeć będzie ATA – dzięki temu aplikacja może tworzyć ATA w imieniu odbiorcy.

Przy obecnej stawce przechowywania danych w klastrze koszty podstawowe wynoszą:

| Token program                             | Rozmiar ATA  | Minimalne saldo przechowywania             |
| ----------------------------------------- | --------- | ----------------------------------- |
| Original Token Program                    | 165 bajtów | 2 039 280 lamports (0,00203928 SOL) |
| Token-2022 z automatycznym niezmiennym właścicielem | 170 bajtów | 2 074 080 lamports (0,00207408 SOL) |

Minty Token-2022 mogą wymagać rozszerzeń konta oprócz automatycznego rozszerzenia _rs`ImmutableOwner`_. Takie ATA są większe i wymagają wyższego minimalnego salda. Associated Token Program pyta wybrany Token Program o wymagany rozmiar konta i odpowiednio finansuje ATA.

Nie koduj tych sald na stałe w logice aplikacji. Użyj zapytania
[`getMinimumBalanceForRentExemption`](/docs/rpc/http/getminimumbalanceforrentexemption)
aby uzyskać wymagany rozmiar danych lub pozwól, aby Associated Token Program obliczył i sfinansował je podczas tworzenia.

<Callout type="info" title="Saldo przechowywania jest zwrotne">
  Zamknięcie pustego token account powoduje przelanie wszystkich jego lamports, w tym minimalnego salda przechowywania, na adres docelowy określony w instrukcji zamknięcia. Pierwotny płatnik tworzenia nie jest automatycznie odbiorcą zwrotu. Zobacz [Close Token Account](/docs/tokens/basics/close-account).
</Callout>

## Jak utworzyć powiązane konto tokenów

Tworzenie powiązanego konta tokenów wykorzystuje instrukcję _rs`Create`_ lub
_rs`CreateIdempotent`_ programu Associated Token Program. Program Associated
...
<Callout type="info" title="Zalecane">
  W większości aplikacji zaleca się tworzenie kont tokenów za pomocą programu
  Associated Token Program zamiast tworzenia ich przez bezpośrednie wywoływanie
  instrukcji System Program i Token Program. Powiązane konto tokenów używa
  deterministycznego adresu wyprowadzonego z właściciela, programu tokenów i
  jednostki emisyjnej, co ułatwia portfelom i aplikacjom znalezienie domyślnego
  konta tokenów dla danej jednostki emisyjnej.
</Callout>
...
| Element                                          | Opis                                                                                                                                                                                                                                                    | Źródło                                                                                                                          |
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| _rs`Create`_                                  | Tworzy powiązane konto tokenów pod wyprowadzonym adresem ATA.                                                                                                                                                                                                | [Źródło](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L20-L29) |
| _rs`CreateIdempotent`_                        | Tworzy powiązane konto tokenów, ale nadal kończy się powodzeniem, jeśli to ATA już istnieje dla tego samego właściciela i jednostki emisyjnej.                                                                                                                                               | [Źródło](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | Wyprowadza adres ATA i używa _rs`create_pda_account`_ oraz CPI do wybranego programu tokenów w celu zainicjalizowania konta tokenów. Gdy wybranym programem jest Token Extension Program, procesor ATA inicjalizuje również rozszerzenie niezmiennego właściciela. | [Źródło](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | Funkcja pomocnicza używana przez _rs`process_create_associated_token_account`_ do utworzenia konta PDA poprzez CPI do System Program.                                                                                                                                         | [Źródło](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |
...
2. Instrukcja _rs`InitializeAccount`_, _rs`InitializeAccount2`_ lub
   _rs`InitializeAccount3`_ Token Program inicjalizuje nowe konto dla tokenu i
   właściciela.

Umieść instrukcję _rs`CreateAccount`_ oraz instrukcję inicjalizacji konta
tokenowego w tej samej transakcji.

Podczas inicjalizacji konta tokenowego Token Program sprawdza, czy konto nie
jest już zainicjalizowane oraz czy jest zwolnione z rent.

<Callout type="info">
  Poniższa sekcja pokazuje, jak utworzyć konto tokenowe poprzez bezpośrednie
  wywołanie instrukcji System Program i Token Program. W większości aplikacji
  użyj Associated Token Program. Bezpośrednich wywołań System Program i Token
  Program używaj wtedy, gdy masz konkretny powód, aby nie używać associated
  token account lub gdy musisz utworzyć niestandardowe konta tokenowe PDA,
  wykonując CPI do instrukcji System Program i Token Program z własnego programu
  Solana.
</Callout>
...
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_                     | Podstawowe pola konta tokenowego przechowywane w każdym koncie tokenowym.                                                                       | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`InitializeAccount`_           | Instrukcja inicjalizacji konta tokenowego, która oczekuje właściciela i konta rent sysvar na liście kont.                    | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L45-L63)   | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L75-L93)   |
| _rs`InitializeAccount2`_          | Instrukcja inicjalizacji konta tokenowego, która przekazuje właściciela w danych instrukcji zamiast na liście kont.                 | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L352-L365) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L419-L433) |
| _rs`InitializeAccount3`_          | Instrukcja inicjalizacji konta tokenowego, która przekazuje właściciela w danych instrukcji i nie wymaga konta rent sysvar. | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L377-L387) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L445-L456) |
| _rs`_process_initialize_account`_ | Współdzielona logika procesora dla inicjalizacji konta tokenowego.                                                                           | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L83-L141)      | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L153-L234)     |
| _rs`process_initialize_account`_  | Publiczny handler dla _rs`InitializeAccount`_.                                                                                        | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L143-L150)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L236-L240)     |
| _rs`process_initialize_account2`_ | Publiczny handler dla _rs`InitializeAccount2`_.                                                                                       | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L152-L160)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L242-L246)     |
| _rs`process_initialize_account3`_ | Publiczny handler dla _rs`InitializeAccount3`_.                                                                                       | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L162-L170)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L248-L252)     |
