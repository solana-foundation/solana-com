...
description: Dowiedz się, jak tworzyć konta SPL Token.
---

## Czym jest konto tokena?

Konto tokena przechowuje Twój stan posiadania konkretnego tokena. Każde konto
tokena jest powiązane dokładnie z jednym mintem i śledzi Twój bilans tokenów
oraz dodatkowe szczegóły.
...
<Callout type="info">
  ...
  korzystają z tej samej bazowej implementacji konta tokena.
</Callout>

Aby przechowywać tokeny, potrzebujesz konta tokena dla danego minta. Każde konto
tokena śledzi:

1. **Mint**: Typ tokena, który przechowuje
2. **Właściciel**: Podmiot uprawniony do transferu tokenów z tego konta

### Przykład: USD Coin (USDC)

- Adres mint: `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Konto tokena Circle: `3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
- To konto przechowuje wyłącznie tokeny USDC
- Circle zarządza tokenem poprzez uprawnienia właściciela konta tokena:
  `7VHUFJHWu2CuExkJcJrzhQPJ2oygupTWkL2A2For4BmE`

Możesz zobaczyć szczegóły tego konta tokena w
[Solana Explorer](https://explorer.solana.com/address/3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa).

<Callout type="info">
  **Wyjaśnienie pojęcia "właściciel"**

Termin "właściciel" może być używany w dwóch różnych kontekstach:

1. **Właściciel konta tokena**: Podmiot uprawniony do transferu, spalania lub
   delegowania tokenów. Ta informacja jest zapisana w danych konta tokena.

2. **Właściciel programu**: Program, do którego należy konto (dla kont tokena
   jest to zawsze Token Program lub Token Extension Program).

Pracując z kontami tokena, "właściciel" zazwyczaj oznacza podmiot uprawniony do
transferu tokenów, a nie program będący właścicielem konta.
...
przechowywania konkretnego tokena. Wykorzystuje deterministyczny adres (PDA)
utworzony przez
...
<Callout type="info">
  ...
  "powiązanymi kontami tokena".
</Callout>

ATA korzystają z deterministycznych adresów, co ułatwia znalezienie konta tokena
dowolnego portfela dla danego mintu. Zobacz
[implementację wyznaczania adresu tutaj](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/interface/src/address.rs#L56).
...
Dla każdej kombinacji portfela i tokena istnieje dokładnie jeden adres ATA.
Eliminuje to konieczność ręcznego śledzenia adresów kont tokenów — zawsze możesz
wyznaczyć poprawny adres.

<Callout type="info">
  **Jak działa Associated Token Program**

Associated Token Program to narzędzie, które:

- Tworzy konta tokenów na deterministycznych adresach (PDA)
- Wykonuje Cross-Program Invocations (CPI) do Token Program
- Sam nie przechowuje żadnego stanu

Utworzone w ten sposób konta tokenów są własnością Token Program i korzystają ze
standardowej struktury `Account`.
...
Aby utworzyć konto tokena, potrzebujesz dwóch instrukcji:

1. System Program: Utwórz konto z zarezerwowaną przestrzenią na konto tokena i
   przenieś własność na Token Program
2. Token Program: Zainicjuj dane konta tokena
...
Utworzenie Associated Token Account (ATA) wymaga instrukcji
...
Ta pojedyncza instrukcja automatycznie obsługuje utworzenie token account oraz
jego inicjalizację przez Cross Program Invocations (CPI).
