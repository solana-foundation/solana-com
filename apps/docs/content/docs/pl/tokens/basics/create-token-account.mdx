...
description:
  Twórz konta tokenów i powiązane konta tokenów dla konkretnego minta i
  właściciela.
...
Konto tokena przechowuje tokeny dla jednego minta i jednego właściciela konta
tokena w sieci Solana.
...
Tutaj _rs`owner`_ oznacza uprawnienie do transferu, spalania lub delegowania
tokenów z konta tokena. Właścicielem programu tego konta nadal pozostaje Token
Program lub Token Extensions Program.

Każde konto tokena jest powiązane z dokładnie jednym mintem, co oznacza, że
konto tokena może przechowywać jednostki tylko jednego tokena — tokena
identyfikowanego przez pole _rs`mint`_ konta tokena.

## Czym jest powiązane konto tokena

Powiązane konto tokena (associated token account, ATA) to domyślne konto tokena
dla portfela i minta.
...
Associated Token Program to sposób na utworzenie konta tokena pod standardowym,
deterministycznym adresem. Powstałe konto nadal jest kontem tokena należącym do
Token Program lub Token Extensions Program, a nie do Associated Token Program.

Associated Token Program wyprowadza adres ATA w sposób pokazany w pliku
...
Dla każdej kombinacji portfela, programu tokenów i minta istnieje dokładnie
jeden adres ATA. Associated Token Program tworzy standardowe konto tokena pod
tym adresem, a powstałe konto nadal wykorzystuje typ _rs`Account`_ zdefiniowany
...
<Callout type="info" title="Zalecane">
  W większości aplikacji należy tworzyć konta tokenów za pośrednictwem
  Associated Token Program zamiast tworzyć je bezpośrednio wywołując instrukcje
  System Program i Token Program. Powiązane konto tokenów wykorzystuje
  deterministyczny adres wyprowadzony z właściciela, programu tokenów i mint, co
  ułatwia portfelom i aplikacjom znalezienie domyślnego konta tokenów dla danego
  mint.
</Callout>
...
| _rs`CreateIdempotent`_                        | Tworzy powiązane konto tokenów, ale nadal kończy się sukcesem, jeśli dane ATA już istnieje dla tego samego właściciela i mint.                                                                                                                                       | [Źródło](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | Wyprowadza adres ATA i używa _rs`create_pda_account`_ wraz z CPI do wybranego programu tokenów w celu zainicjalizowania konta tokenów. Gdy wybranym programem jest Token Extension Program, procesor ATA inicjalizuje również rozszerzenie niezmiennego właściciela. | [Źródło](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | Pomocnik używany przez _rs`process_create_associated_token_account`_ do utworzenia konta PDA za pomocą CPI do System Program.                                                                                                                                        | [Źródło](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |
...
## Jak utworzyć konto tokenów

Utworzenie konta tokenów wymaga dwóch instrukcji:
...
2. Instrukcja _rs`InitializeAccount`_, _rs`InitializeAccount2`_ lub
   _rs`InitializeAccount3`_ Token Program inicjalizuje nowe konto dla monety i
   właściciela.

Umieść instrukcję _rs`CreateAccount`_ oraz instrukcję inicjalizacji token
account w tej samej transakcji.

Podczas inicjalizacji token account, Token Program sprawdza, czy konto nie
zostało jeszcze zainicjalizowane i jest zwolnione z rent.

<Callout type="info">
  Poniższa sekcja pokazuje, jak utworzyć token account bezpośrednio wywołując
  instrukcje System Program i Token Program. W przypadku większości aplikacji
  należy używać Associated Token Program. Bezpośrednich wywołań System Program i
  Token Program używaj, gdy masz konkretny powód, aby nie korzystać z associated
  token account lub gdy musisz tworzyć niestandardowe konta tokenów PDA,
  wykonując CPI do instrukcji System Program i Token Program z własnego programu
  Solana.
</Callout>

### Odniesienie do kodu źródłowego

| Element                           | Opis                                                                                                                    | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_                     | Podstawowe pola token account przechowywane w każdym koncie tokenów.                                                    | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`InitializeAccount`_           | Instrukcja inicjalizacji token account, która oczekuje właściciela i konta rent sysvar na swojej liście kont.           | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L45-L63)   | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L75-L93)   |
| _rs`InitializeAccount2`_          | Instrukcja inicjalizacji token account, która przekazuje właściciela w instruction data zamiast na liście kont.         | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L352-L365) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L419-L433) |
| _rs`InitializeAccount3`_          | Instrukcja inicjalizacji token account, która przekazuje właściciela w instruction data i nie wymaga konta rent sysvar. | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L377-L387) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L445-L456) |
| _rs`_process_initialize_account`_ | Współdzielona logika procesora dla inicjalizacji token account.                                                         | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L83-L141)      | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L153-L234)     |
...
Poniższe przykłady `Kit` przedstawiają zalecane podejście z wykorzystaniem
