...
description:
  Twórz konta tokenów i powiązane konta tokenów dla określonego mintu i
  właściciela.
...
## Czym jest konto tokenu

Konto tokenu przechowuje tokeny dla jednego mintu i jednego właściciela token
account w sieci Solana.
...
Tutaj _rs`owner`_ oznacza uprawnienie, które może przenosić, wypalać lub
delegować tokeny z token account. Właścicielem programu konta jest nadal Token
Program lub Token Extensions Program.

Każde konto tokenu jest powiązane z dokładnie jednym mintem, co oznacza, że
token account może przechowywać jednostki tylko jednego tokenu — tokenu
identyfikowanego przez pole _rs`mint`_ token account.

## Czym jest powiązane konto tokenu

Powiązane konto tokenu (ATA — associated token account) jest domyślnym kontem
tokenu dla portfela i mintu. Program
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program/src)
wyprowadza adres ATA na podstawie adresu portfela, adresu programu tokenów oraz
adresu mintu.

Tylko konta tokenów utworzone przez Associated Token Program nazywane są
powiązanymi kontami tokenów.

Associated Token Program to sposób tworzenia token account pod standardowym,
deterministycznym adresem. Wynikowe konto pozostaje kontem tokenu należącym do
Token Program lub Token Extensions Program, a nie do Associated Token Program.

Associated Token Program wyprowadza adres ATA zgodnie z
...
Dla każdej kombinacji portfela, programu tokenów i mintu istnieje dokładnie
jeden adres ATA. Associated Token Program tworzy standardowe konto tokenu pod
tym adresem, a wynikowe konto nadal wykorzystuje typ _rs`Account`_ zdefiniowany
...
_rs`CreateIdempotent`_ z Associated Token Program. Associated Token Program
wyprowadza adres ATA, tworzy konto pod adresem ATA i inicjalizuje konto jako
konto tokenów należące do Token Program lub Token Extension Program.

<Callout type="info" title="Zalecane">
  W przypadku większości aplikacji należy tworzyć konta tokenów za pośrednictwem
  Associated Token Program zamiast tworzyć je bezpośrednio, wywołując instrukcje
  System Program i Token Program. Powiązane konto tokenów używa
  deterministycznego adresu wyprowadzonego z właściciela, programu tokenów i
  mintu, co ułatwia portfelom i aplikacjom odnalezienie domyślnego konta tokenów
  dla danego mintu.
</Callout>
...
| _rs`CreateIdempotent`_                        | Tworzy powiązane konto tokenów, ale nadal kończy się powodzeniem, jeśli ATA już istnieje dla tego samego właściciela i mintu.                                                                                                                                         | [Źródło](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | Wyprowadza adres ATA i wykorzystuje _rs`create_pda_account`_ oraz CPI do wybranego programu tokenów w celu inicjalizacji konta tokenów. Gdy wybranym programem jest Token Extension Program, procesor ATA inicjalizuje również rozszerzenie niezmiennego właściciela. | [Źródło](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
...
`@solana/kit`. Przykłady legacy wykorzystujące `@solana/web3.js` są dołączone w
...
2. Instrukcja _rs`InitializeAccount`_, _rs`InitializeAccount2`_ lub
   _rs`InitializeAccount3`_ programu Token Program inicjalizuje nowe konto dla
   monety i właściciela.

Dołącz instrukcję _rs`CreateAccount`_ oraz instrukcję inicjalizacji konta
tokenowego w tej samej transakcji.

Podczas inicjalizacji konta tokenowego Token Program sprawdza, czy konto nie
jest już zainicjalizowane i jest zwolnione z rent.

<Callout type="info">
  Poniższa sekcja pokazuje, jak utworzyć konto tokenowe poprzez bezpośrednie
  wywoływanie instrukcji System Program i Token Program. W większości aplikacji
  należy używać Associated Token Program. Bezpośrednich wywołań System Program i
  Token Program należy używać, gdy istnieje konkretny powód, aby nie korzystać z
  associated token account lub gdy trzeba utworzyć niestandardowe konta tokenowe
  PDA poprzez wykonywanie CPI do instrukcji System Program i Token Program z
  własnego programu Solana.
</Callout>
...
Poniższe przykłady w `Kit` pokazują zalecane podejście przy użyciu
`@solana/kit`. Przykłady starszej wersji używające `@solana/web3.js` są
