---
title: Ustawianie uprawnień
description:
  Zmień lub odwołaj uprawnienia na koncie mint lub token account, w tym
  uprawnienia mint, freeze, owner i close authority.
...
## Co robi ustawienie uprawnień?

Ustawienie uprawnień zmienia lub odwołuje określone role uprawnień na koncie
mint lub token account.

Konta mint mogą przechowywać uprawnienia mint oraz freeze. Konta token
przechowują właściciela i opcjonalnie mogą przechowywać uprawnienie close. Każda
rola kontroluje określony zestaw uprawnień, a ustawienie uprawnień na _rs`None`_
trwale usuwa wybraną rolę uprawnień.

Token Extensions Program definiuje również dodatkowe typy uprawnień dla
określonych rozszerzeń.

## Jak ustawić uprawnienia

Zmiana uprawnień wykorzystuje instrukcję _rs`SetAuthority`_ Token Program.

Każde wywołanie aktualizuje jeden typ uprawnień na jednym koncie mint lub token
account. Obecne uprawnienie podpisuje zmianę, a nowe uprawnienie może być innym
adresem lub _rs`None`_, aby trwale odwołać wybrane uprawnienie.

### Źródło odniesienia

| Element                     | Opis                                                                                       | Token Program                                                                                                  | Token Extensions Program                                                                                               |
| --------------------------- | ------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                  | Stan mint przechowuje _rs`mint_authority`_ oraz _rs`freeze_authority`_.                    | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)           |
| _rs`Account`_               | Stan token account przechowuje _rs`owner`_ oraz _rs`close_authority`_.                     | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)          |
| _rs`AuthorityType`_         | Role uprawnień, które _rs`SetAuthority`_ może aktualizować.                                | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L739-L751) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1116-L1157) |
| _rs`SetAuthority`_          | Instrukcja, która zmienia lub odwołuje jedno uprawnienie na koncie mint lub token account. | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L141-L158) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L183-L200)   |
| _rs`process_set_authority`_ | Wspólna logika procesora dla aktualizacji uprawnień.                                       | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L423-L518)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L677-L978)       |

### Typescript

Poniższe przykłady `Kit` pokazują zalecane podejście z użyciem `@solana/kit`.
Przykłady starszej wersji z użyciem `@solana/web3.js` są dołączone w celach
referencyjnych.
