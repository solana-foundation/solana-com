...
description:
  Zmień lub cofnij role uprawnień na koncie mint lub token account, w tym
...
## Co robi ustawianie uprawnień?

Ustawianie uprawnień zmienia lub cofa określone role uprawnień na koncie mint
lub token account.

Konta mint mogą przechowywać uprawnienie mint authority i freeze authority.
Konta token przechowują właściciela (owner) i mogą opcjonalnie przechowywać
close authority. Każda rola kontroluje określony zestaw uprawnień, a ustawienie
uprawnienia na _rs`None`_ trwale usuwa wybraną rolę uprawnień.
...
Zmiana uprawnień wykorzystuje instrukcję _rs`SetAuthority`_ programu Token
Program.
...
adresem lub _rs`None`_, aby trwale cofnąć wybrane uprawnienie.

### Odniesienie do źródła

| Element                     | Opis                                                                                   | Token Program                                                                                                  | Token Extensions Program                                                                                               |
| --------------------------- | -------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                  | Stan mint przechowuje _rs`mint_authority`_ i _rs`freeze_authority`_.                   | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)           |
| _rs`Account`_               | Stan token account przechowuje _rs`owner`_ i _rs`close_authority`_.                    | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)          |
...
Starsze przykłady używające `@solana/web3.js` zostały dołączone jako
odniesienie.
