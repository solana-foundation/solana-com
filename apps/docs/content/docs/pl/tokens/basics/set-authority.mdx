...
description:
  Dowiedz się, jak ustawić uprawnienia dla mintów lub token accountów.
---

## Zrozumienie uprawnień tokenów

### Uprawnienia mint accountu

- **Mint Authority**: Kontroluje tworzenie nowych tokenów. Może mintować tokeny
  na dowolny token account. Często jest odbierane po utworzeniu początkowej
  podaży, aby zapewnić stałą liczbę tokenów.
- **Freeze Authority**: Kontroluje możliwość zamrażania i odmrażania token
  accountów. Może uniemożliwić dowolnemu token accountowi transfer tokenów.
  Często odbierane, aby zagwarantować użytkownikom, że ich tokeny nie mogą
  zostać zamrożone.

### Uprawnienia token accountu

- **Account Owner**: Ma pełną kontrolę nad token accountem. Może transferować
  tokeny, spalać tokeny, zatwierdzać delegatów oraz zamknąć konto, gdy saldo
  wynosi zero.
- **Close Authority**: Może zamknąć token account, gdy jego saldo wynosi zero.
  Domyślnie jest to właściciel konta, ale uprawnienie to może zostać przekazane
  innemu kontu.
...
zmienia lub odbiera uprawnienia na mintach i token accountach. Tylko obecny
właściciel uprawnienia może przekazać swoje uprawnienia na nowy adres lub trwale
je odebrać, ustawiając uprawnienie na null. Po odebraniu uprawnienia nie można
go przywrócić.
