...
description:
  Zaktualizuj kwotę w koncie tokena opakowanego SOL po przeniesieniu SOL na
  konto.
...
Opakowany SOL (WSOL) to SOL przechowywany na koncie tokenowym dla natywnego
minta Token Program. Natywny mint to adres minta, którego Token Program używa
dla kont tokenowych opakowujących SOL. Natywny mint umożliwia korzystanie z SOL
poprzez instrukcje Token Program.
...
SOL przechowywany na koncie tokena opakowanego SOL jako saldo tokena poprzez
pole _rs`amount`_ konta tokenowego. Jeśli przeniesiesz SOL na konto tokena
opakowanego SOL, saldo w lamportach wzrasta natychmiast, ale pole _rs`amount`_
nie zmienia się, dopóki nie wywołasz instrukcji _rs`SyncNative`_ Token Program.

## Jak zsynchronizować natywny SOL

Użyj instrukcji _rs`SyncNative`_ Token Program po przeniesieniu SOL na konto
tokena opakowanego SOL. Instrukcja _rs`SyncNative`_ aktualizuje pole
_rs`amount`_ konta tokenowego, aby odpowiadało lamportom przechowywanym na
koncie powyżej jego rezerwy zwolnionej z czynszu.

### Odniesienie do źródła

| Element                   | Opis                                                                                                                                                                                                           | Token Program                                                                                                  | Token Extension Program                                                                                              |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_             | Stan konta tokenowego używa _rs`is_native`_, aby oznaczyć konto jako WSOL i przechowuje natywną rezerwę, minimalną kwotę lamportów zwolnioną z czynszu, którą konto musi zachować, oraz śledzoną kwotę tokena. | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`SyncNative`_          | Instrukcja, która aktualizuje pole _rs`amount`_ konta tokena opakowanego SOL, aby odpowiadało lamportom przechowywanym na koncie powyżej jego rezerwy zwolnionej z czynszu.                                    | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| _rs`process_sync_native`_ | Współdzielona logika procesora do synchronizacji sald opakowanego SOL.                                                                                                                                         | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |

### Typescript

Poniższe przykłady `Kit` pokazują zalecane podejście z wykorzystaniem
`@solana/kit`. Przykłady w starszej wersji wykorzystujące `@solana/web3.js` są
