...
description:
  Zamroź konto tokenów przy użyciu uprawnień do zamrażania minta, aby zablokować
  transfery, spalanie i zmiany delegatów.
...
Zamrożenie token account zachowuje tego samego właściciela, mint i saldo, ale
uniemożliwia token account otrzymywanie, przesyłanie lub spalanie tokenów do
czasu odblokowania token account.

Zamrożone konto może nadal zostać zamknięte, jeśli zamrożone konto ma zerowe
saldo tokenów.

Tylko mint, który nadal posiada uprawnienie do zamrażania, może zamrażać konta
tokenów. Jeśli uprawnienie do zamrażania minta zostanie odwołane, konta dla tego
minta nie mogą być już zamrażane.

## Jak zamrozić konto tokenów

Zamrażanie konta tokenów wykorzystuje instrukcję _rs`FreezeAccount`_ Token
Program.

Instrukcja _rs`FreezeAccount`_ weryfikuje, czy konto tokenów należy do minta
oraz czy sygnatariusz jest uprawniony do zamrażania minta, a następnie zmienia
stan konta tokenów z zainicjalizowanego na zamrożony.
...
| ----------------------------------- | ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Stan minta przechowuje opcjonalne uprawnienie do zamrażania używane do zamrażania kont tokenów. | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Stany konta tokenów, w tym _rs`Initialized`_ i _rs`Frozen`_.                                    | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | Instrukcja zmieniająca konto tokenów w stan zamrożony.                                          | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
| _rs`process_toggle_freeze_account`_ | Współdzielona logika procesora do zamrażania i odmrażania kont tokenów.                         | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |

### Typescript

Poniższe przykłady `Kit` pokazują zalecane podejście z wykorzystaniem
