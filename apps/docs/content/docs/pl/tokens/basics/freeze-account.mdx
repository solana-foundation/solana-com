...
zapobiega wszystkim transferom tokenów lub ich spalaniu z określonego konta
tokenów. Po zamrożeniu konto nie może wysyłać ani otrzymywać tokenów do czasu
jego rozmrożenia. Zamrożone konto nadal może zostać zamknięte, jeśli ma zerowe
saldo tokenów. Tylko uprawniony do zamrażania (freeze authority) mint account
może zamrażać konta. Jeśli uprawnienie do zamrażania zostanie odwołane
(ustawione na null) w mint account, tokeny nigdy nie będą mogły zostać
zamrożone.
