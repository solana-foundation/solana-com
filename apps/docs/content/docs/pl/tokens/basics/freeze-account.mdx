...
uniemożliwia wszelkie transfery tokenów lub spalanie tokenów z określonego konta
tokenowego. Po zamrożeniu konto nie może wysyłać ani otrzymywać tokenów, ani
zostać zamknięte, dopóki nie zostanie odmrożone. Tylko autorytet zamrażania
(freeze authority) danej emisji tokena (mint) może zamrażać konta. Jeśli
autorytet zamrażania zostanie odebrany (ustawiony na null) na koncie emisji,
tokeny nie mogą być już nigdy zamrożone.
