...
description:
  Zamroź konto tokenowe za pomocą uprawnień freeze authority minta, aby
  zablokować transfery, spalanie oraz zmiany delegata.
...
## Co oznacza zamrożenie konta tokenowego?

Zamrożenie konta tokenowego zachowuje tego samego właściciela, mint oraz saldo,
ale uniemożliwia transferowanie lub spalanie tokenów z tego konta do czasu jego
odmrożenia.

Zamrożone konto można nadal zamknąć, jeśli zamrożone konto ma zerowe saldo
tokenów.

Tylko mint, który nadal posiada uprawnienia freeze authority, może zamrażać
konta tokenowe. Jeśli uprawnienia freeze authority minta zostaną odwołane, kont
dla tego minta nie można już zamrażać.

## Jak zamrozić konto tokenowe

Zamrażanie konta tokenowego wykorzystuje instrukcję _rs`FreezeAccount`_ Token
Program.

Instrukcja _rs`FreezeAccount`_ weryfikuje, że konto tokenowe należy do minta
oraz że sygnatariusz jest freeze authority minta, a następnie zmienia stan konta
tokenowego z zainicjalizowanego na zamrożony.

[Natywne konta tokenowe](/docs/tokens/basics/sync-native) nie obsługują
zamrażania.
...
| ----------------------------------- | ----------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Stan minta przechowuje opcjonalne uprawnienia freeze authority używane do zamrażania kont tokenowych. | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Stany konta tokenowego, w tym _rs`Initialized`_ oraz _rs`Frozen`_.                                    | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | Instrukcja, która zmienia konto tokenowe w stan zamrożony.                                            | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
| _rs`process_toggle_freeze_account`_ | Współdzielona logika procesora do zamrażania i odmrażania kont tokenowych.                            | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |
