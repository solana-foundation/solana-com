...
description: Wypal tokeny z konta tokenów i zmniejsz całkowitą podaż minta.
...
Wypalanie tokenów trwale zmniejsza saldo konta tokenów i redukuje całkowitą
podaż minta o tę samą wartość.

## Jak wypalać tokeny

Wypalanie tokenów wykorzystuje instrukcję _rs`Burn`_ lub _rs`BurnChecked`_ Token
Program.

Poniższe przykłady wykorzystują _rs`BurnChecked`_, która wymaga od wywołującego
podania miejsc dziesiętnych minta, aby instrukcja mogła zweryfikować oczekiwaną
precyzję tokena przed wypaleniem.

[Natywny mint](/docs/tokens/basics/sync-native) nie obsługuje wypalania. Zamiast
tego użyj _rs`CloseAccount`_.

Właściciel konta tokenów lub zatwierdzony delegat podpisuje transakcję. W Token
Extensions Program, stały delegat minta może również autoryzować wypalanie,
jeśli mint ma włączone rozszerzenie stałego delegata.

### Odniesienie do źródła

| Element            | Opis                                                                                                                                                               | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_         | Stan minta przechowuje całkowitą podaż tokenów.                                                                                                                    | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_      | Stan konta tokenów przechowuje saldo tokenów zmniejszone przez wypalanie.                                                                                          | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Burn`_         | Instrukcja wypalania, która zmniejsza saldo konta tokenów i podaż minta bez wymagania od wywołującego podania miejsc dziesiętnych minta.                           | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L178-L196) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L221-L239) |
| _rs`BurnChecked`_  | Instrukcja wypalania, która wymaga od wywołującego podania miejsc dziesiętnych minta i sprawdza tę wartość przed zmniejszeniem salda konta tokenów i podaży minta. | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L326-L350) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L393-L417) |
| _rs`process_burn`_ | Współdzielona logika procesora dla wypalania tokenów.                                                                                                              | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L586-L668)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1070-L1194)   |
...
zostały dołączone w celach informacyjnych.
