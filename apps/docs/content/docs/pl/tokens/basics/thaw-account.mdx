...
description:
  Odblokuj zamrożone konto tokenów, aby wznowić transfery, spalanie i zmiany
  delegowania.
...
## Co robi odblokowanie konta tokenów?

Odblokowanie przywraca konto tokenów ze stanu zamrożonego z powrotem do stanu
zainicjalizowanego.

Po pomyślnym odblokowaniu konto może ponownie transferować, spalać i zamykać
tokeny. Tylko uprawnienie do zamrażania danego mint może odblokować konta
tokenów.

## Jak odblokować konto tokenów

Odblokowanie konta tokenów wykorzystuje instrukcję _rs`ThawAccount`_ Token
Program.

Instrukcja _rs`ThawAccount`_ weryfikuje, że token account należy do mint oraz że
podpisujący jest uprawniony do zamrażania mint, a następnie zmienia stan token
account z zamrożonego z powrotem na zainicjalizowany.

[Natywne konta tokenów](/docs/tokens/basics/sync-native) nie obsługują
odblokowania.
...
| ----------------------------------- | -------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | Stan mint przechowuje opcjonalne uprawnienie do zamrażania używane do odblokowywania kont tokenów. | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Stany token account, w tym _rs`Frozen`_ i _rs`Initialized`_.                                       | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`ThawAccount`_                   | Instrukcja, która zmienia zamrożone konto tokenów z powrotem do stanu zainicjalizowanego.          | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L229-L243) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L292-L306) |
| _rs`process_toggle_freeze_account`_ | Wspólna logika procesora do zamrażania i odblokowywania kont tokenów.                              | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L713-L754)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1293-L1341)   |

### Typescript

Poniższe przykłady `Kit` pokazują zalecane podejście z użyciem `@solana/kit`.
Przykłady starszych rozwiązań wykorzystujących `@solana/web3.js` są dołączone w
