...
Transfer tokenów przenosi tokeny z jednego token account do innego token account
dla tego samego mint.

Transfery nie zmieniają podaży mint. Transfery aktualizują jedynie salda między
kontami tokenów.

## Jak transferować tokeny
...
zweryfikować oczekiwany mint i precyzję tokenów przed przeniesieniem tokenów
między kontami.

W Token Extensions Program, _rs`Transfer`_ jest przestarzała na rzecz
...
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_          | Stan token account przechowuje salda aktualizowane przez transfer.                                                                                                                                          | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | Instrukcja transferu, która przenosi tokeny między kontami tokenów bez wymagania od wywołującego podania mint lub miejsc dziesiętnych. W Token Extensions Program, zamiast tego użyj _rs`TransferChecked`_. | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Instrukcja transferu, która wymaga od wywołującego podania mint i miejsc dziesiętnych oraz sprawdza te wartości przed przeniesieniem tokenów między kontami tokenów tego samego mint.                       | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | Wspólna logika procesora dla transferów tokenów.                                                                                                                                                            | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |

### Typescript

Poniższe przykłady `Kit` przedstawiają zalecane podejście z wykorzystaniem
`@solana/kit`. Przykłady starszego kodu używające `@solana/web3.js` są dołączone
