---
title: Zamknij konto tokenów
description:
  Zamknij konto tokenów i zwróć lamporty konta na konto docelowe.
...
## Co robi zamknięcie konta tokenów?

Zamknięcie token account usuwa konto i przenosi wszystkie jego lamports,
włącznie z podlegającym zwrotowi minimalnym saldem magazynowania, na konto docelowe.

Saldo tokenów musi wynosić zero, zanim konto będzie mogło zostać zamknięte. Zamrożone konto
nadal może zostać zamknięte, jeśli saldo tokenów zamrożonego konta wynosi zero. Token account SOL
zawinięty w token jest wyjątkiem i może zostać zamknięte z niezerowym saldem tokenów, aby odzyskać
leżące u podstaw SOL. Właściciel konta lub uprawnienie do zamknięcia konta
podpisuje instrukcję zamknięcia i wybiera cel. Konto, które zapłaciło
za utworzenie associated token account, nie otrzymuje automatycznie odzyskanego salda.

## Jak zamknąć konto tokenów

Zamykanie konta tokenów wykorzystuje instrukcję _rs`CloseAccount`_ Token
Program.

Instrukcja _rs`CloseAccount`_ przenosi lamporty konta źródłowego na konto
docelowe, czyści konto źródłowe i usuwa konto źródłowe. Token Extensions Program
stosuje dodatkowe kontrole zamknięcia dla niektórych rozszerzeń, ale podstawowy
przepływ zamknięcia jest taki sam.

### Odniesienie do źródła

| Element                        | Opis                                                                                                     | Token Program                                                                                                  | Token Extensions Program                                                                                              |
| --------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | Stan konta tokenów przechowuje saldo, status natywny i upoważnienie do zamknięcia używane podczas zamykania konta.     | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | Instrukcja, która zamyka konto tokenów i przenosi lamporty konta tokenów na konto docelowe. | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | Wspólna logika procesora dla zamknięcia konta tokenów.                                                               | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |

### Typescript

Poniższe przykłady pokazują zalecane podejście z użyciem `Kit`. Przykłady legacy
używające `@solana/web3.js` są dołączone w celach informacyjnych.
