...
Zamknięcie konta tokenów usuwa konto i zwraca lamporty rent tego konta na konto
docelowe.

Saldo tokenów musi wynosić zero przed zamknięciem konta. Zamrożone konto nadal
może zostać zamknięte, jeśli saldo tokenów zamrożonego konta wynosi zero. Konta
tokenów Wrapped SOL są wyjątkiem i mogą zostać zamknięte z saldem tokenów, aby
odzyskać podstawowy SOL. Właściciel konta lub uprawnienie do zamykania konta
podpisuje instrukcję zamknięcia.

## Jak zamknąć konto tokenów

Zamknięcie konta tokenów wykorzystuje instrukcję _rs`CloseAccount`_ Token
Program.

Instrukcja _rs`CloseAccount`_ przenosi lamporty konta źródłowego na konto
docelowe, czyści konto źródłowe i usuwa konto źródłowe. Token Extension Program
stosuje dodatkowe kontrole zamknięcia dla niektórych rozszerzeń, ale podstawowy
przepływ zamykania jest taki sam.
...
| _rs`CloseAccount`_          | Instrukcja zamykająca konto tokenów i przenosząca lamporty konta tokenów na konto docelowe.                       | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | Wspólna logika procesora dla zamykania konta tokenów.                                                             | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |

### Typescript

Poniższe przykłady `Kit` pokazują zalecane podejście z użyciem `@solana/kit`.
Przykłady starszej wersji z użyciem `@solana/web3.js` są dołączone jako
odniesienie.
