...
description: Zamknij konto tokenów i zwróć lamporty konta na konto docelowe.
...
## Co oznacza zamknięcie konta tokenów?

Zamknięcie konta tokenów usuwa konto i zwraca lamporty rent konta tokenów na
konto docelowe.

Saldo tokenów musi wynosić zero, zanim konto będzie mogło zostać zamknięte.
Zamrożone konto nadal może zostać zamknięte, jeśli saldo tokenów zamrożonego
konta wynosi zero. Konta tokenów Wrapped SOL stanowią wyjątek i mogą zostać
zamknięte z saldem tokenów w celu odzyskania bazowego SOL. Właściciel konta lub
uprawnienie do zamknięcia konta podpisuje instrukcję zamknięcia.
...
| _rs`process_close_account`_ | Współdzielona logika procesora do zamykania konta tokenów.                                                        | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |

### Typescript

Poniższe przykłady `Kit` pokazują zalecane podejście z użyciem `@solana/kit`.
Przykłady legacy używające `@solana/web3.js` zostały dołączone jako odniesienie.
