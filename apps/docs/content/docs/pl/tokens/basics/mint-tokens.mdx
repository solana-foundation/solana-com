---
title: Tworzenie tokenów (mintowanie)
description:
  Wybij nowe jednostki tokenów na konto tokenowe przy użyciu uprawnień do
  wybijania.
...
Wybijanie tworzy nowe jednostki istniejącego tokena i zwiększa całkowitą podaż
tokena.

Tylko uprawnienie do wybijania może wybijać nowe tokeny. Docelowe konto tokenowe
musi już istnieć i musi należeć do tego samego tokena.

## Jak wybijać tokeny

Wybijanie tokenów wykorzystuje instrukcję _rs`MintTo`_ lub _rs`MintToChecked`_
Token Program.

Poniższe przykłady używają _rs`MintToChecked`_, która wymaga od wywołującego
podania liczby miejsc dziesiętnych tokena, aby instrukcja mogła zweryfikować
oczekiwaną precyzję tokena przed wybiciem.

[Natywny token](/docs/tokens/basics/sync-native) nie obsługuje wybijania.

### Odniesienie do źródła

| Element               | Opis                                                                                                                                                                    | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_            | Stan tokena przechowuje uprawnienia do wybijania i całkowitą podaż tokena.                                                                                              | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_         | Stan konta tokenowego przechowuje docelowe saldo aktualizowane przez wybijanie.                                                                                         | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`MintTo`_          | Instrukcja wybijania, która wybija tokeny na istniejące konto tokenowe bez wymagania od wywołującego podania liczby miejsc dziesiętnych tokena.                         | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L159-L177) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L202-L220) |
| _rs`MintToChecked`_   | Instrukcja wybijania, która wymaga od wywołującego podania liczby miejsc dziesiętnych tokena i sprawdza tę wartość przed wybiciem tokenów na istniejące konto tokenowe. | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L301-L325) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L367-L391) |
| _rs`process_mint_to`_ | Współdzielona logika procesora do wybijania tokenów.                                                                                                                    | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L521-L583)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L981-L1067)    |

### Typescript

Poniższe przykłady `Kit` pokazują zalecane podejście z użyciem `@solana/kit`.
Przykłady starszej wersji wykorzystujące `@solana/web3.js` są dołączone jako
odniesienie.
