---
title: Mintowanie tokenów
description: Dowiedz się, jak mintować nowe jednostki tokena.
---

## Jak mintować tokeny

Mintowanie tworzy nowe jednostki tokena za pomocą instrukcji
...
- Tylko mint authority może mintować nowe tokeny
- Musi istnieć docelowy token account, aby otrzymać wybite tokeny
...
<Callout type="info">
  ...
  mają podobne implementacje, aby zapewnić tę samą funkcjonalność.
</Callout>
...
async fn main() -> Result<()> {
    // Utwórz połączenie z lokalnym validatorem
    ...
    // Wygeneruj nowy keypair dla fee payera
    let fee_payer = Keypair::new();

    // Airdrop 1 SOL dla fee payera
    ...
    // Wygeneruj keypair do użycia jako adres mint
    let mint = Keypair::new();

    // Pobierz domyślny rozmiar mint account (w bajtach), bez rozszerzeń
    let mint_space = Mint::LEN;
    let mint_rent = client
        .get_minimum_balance_for_rent_exemption(mint_space)
        .await?;

    // Instrukcja utworzenia nowego konta dla mint (token program)
    let create_account_instruction = create_account(
        &fee_payer.pubkey(), // płatnik
        &mint.pubkey(),      // nowe konto (mint)
        mint_rent,           // lamporty
        mint_space as u64,   // rozmiar
        &token_program_id(), // program id
    );

    // Instrukcja inicjalizacji danych mint account
    ...
    // Oblicz adres associated token account dla fee_payer
    let associated_token_address = get_associated_token_address_with_program_id(
        &fee_payer.pubkey(), // właściciel
        &mint.pubkey(),      // mint
        &token_program_id(), // program_id
    );

    // Instrukcja utworzenia associated token account
    let create_ata_instruction = create_associated_token_account(
        &fee_payer.pubkey(), // adres finansujący
        &fee_payer.pubkey(), // adres portfela
        &mint.pubkey(),      // adres mint
        &token_program_id(), // program id
    );

    // Utwórz transakcję i dodaj instrukcje
    ...
    // Wyślij i potwierdź transakcję
    client.send_and_confirm_transaction(&transaction).await?;

    // Ilość tokenów do wybicia (100 tokenów z 2 miejscami po przecinku)
    let amount = 100;

    // Utwórz instrukcję mint_to, aby wybić tokeny na associated token account
    let mint_to_instruction = mint_to(
        &token_program_id(),
        &mint.pubkey(),            // mint
        &associated_token_address, // miejsce docelowe
        &fee_payer.pubkey(),       // autorytet
        &[&fee_payer.pubkey()],    // podpisujący
        amount,                    // ilość
    )?;

    // Utwórz transakcję do mintowania tokenów
    let transaction = Transaction::new_signed_with_payer(
        &[mint_to_instruction],
        Some(&fee_payer.pubkey()),
        &[&fee_payer],
    ...
    println!("Wybito 1,00 tokenów na associated token account");
    println!("\nAdres mint: {}", mint.pubkey());
    println!("{:#?}", mint_data);

    println!(
        "\nAdres associated token account: {}",
        associated_token_address
    );
    println!("{:#?}", token_data);

    println!("Podpis transakcji: {}", transaction_signature);
...
async def main():
    ...
    # Ilość do wybicia (w najmniejszej jednostce)
    amount_to_mint = 1000000000  # 1 token z 9 miejscami po przecinku

    async with rpc:
        # Utwórz instrukcję mintowania
        ...
        print(f"Docelowe konto: {destination_token_account}")
        print(f"Ilość: {amount_to_mint}")
        print(f"Autorytet mintowania: {mint_authority.pubkey()}")
        print(f"Płatnik: {payer.pubkey()}")
        print(f"Transakcja mintowania utworzona pomyślnie")
