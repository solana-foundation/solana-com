...
description: Dowiedz się, jak wybijać nowe jednostki tokena.
---

## Jak wybijać tokeny

Proces mintowania tworzy nowe jednostki tokena za pomocą instrukcji
...
- Tylko autorytet mintowania może wybijać nowe tokeny
...
async fn main() -> Result<()> {
    ...
    // Wykonaj airdrop 1 SOL dla fee payera
    ...
    // Oblicz adres associated token account dla fee payera
    ...
    // Liczba tokenów do wybicia (100 tokenów z 2 miejscami po przecinku)
    let amount = 100;
    ...
    // Utwórz transakcję do wybicia tokenów
    let transaction = Transaction::new_signed_with_payer(
        &[mint_to_instruction],
        Some(&fee_payer.pubkey()),
        &[&fee_payer],
...
async def main():
    ...
    async with rpc:
        ...
        print(f"Transakcja mint utworzona pomyślnie")
