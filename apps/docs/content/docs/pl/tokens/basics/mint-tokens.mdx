...
description:
  Wybij nowe jednostki tokenów na konto tokenów przy użyciu uprawnień do bicia.
...
## Co robi bicie tokenów?

Bicie tokenów tworzy nowe jednostki istniejącego tokenu i zwiększa całkowitą
podaż minta.

Tylko uprawnienia do bicia mogą tworzyć nowe tokeny. Docelowe konto tokenów musi
już istnieć i musi należeć do tego samego minta.

## Jak bić tokeny

Bicie tokenów wykorzystuje instrukcję _rs`MintTo`_ lub _rs`MintToChecked`_ Token
Program.
...
precyzję tokenu przed biciem.

[Natywny mint](/docs/tokens/basics/sync-native) nie obsługuje bicia.

### Odniesienie do źródła

| Element               | Opis                                                                                                                                                     | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_            | Stan minta przechowuje uprawnienia do bicia i całkowitą podaż tokenów.                                                                                   | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`Account`_         | Stan konta tokenów przechowuje saldo docelowe aktualizowane przez bicie.                                                                                 | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`MintTo`_          | Instrukcja bicia, która bije tokeny na istniejące konto tokenów bez wymagania od wywołującego podania miejsc dziesiętnych minta.                         | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L159-L177) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L202-L220) |
| _rs`MintToChecked`_   | Instrukcja bicia, która wymaga od wywołującego podania miejsc dziesiętnych minta i sprawdza tę wartość przed biciem tokenów na istniejące konto tokenów. | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L301-L325) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L367-L391) |
| _rs`process_mint_to`_ | Współdzielona logika procesora do bicia tokenów.                                                                                                         | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L521-L583)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L981-L1067)    |

### Typescript

Poniższe przykłady `Kit` pokazują zalecane podejście z użyciem `@solana/kit`.
Przykłady starszej wersji używające `@solana/web3.js` są dołączone w celach
referencyjnych.
