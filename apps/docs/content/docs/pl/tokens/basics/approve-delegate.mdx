...
description:
  Autoryzuj inny adres do transferu lub spalenia ograniczonej liczby tokenów z
  konta tokenowego.
...
## Co robi zatwierdzenie delegata?

Zatwierdzenie delegata umożliwia innemu adresowi transfer lub spalenie
ograniczonej liczby tokenów z token account w imieniu właściciela konta
tokenowego.

Token account przechowuje jeden aktualny delegat i jedną delegowaną kwotę na
raz. Zatwierdzenie nowego delegata zastępuje poprzedniego delegata i limit na
koncie.

## Jak zatwierdzić delegata

Zatwierdzenie delegata wykorzystuje instrukcję _rs`Approve`_ lub
_rs`ApproveChecked`_ Token Program.

Poniższe przykłady używają _rs`ApproveChecked`_, która wymaga od wywołującego
podania minta i miejsc dziesiętnych, aby instrukcja mogła zweryfikować
oczekiwany mint i precyzję tokena przed przetworzeniem zatwierdzenia.

Właściciel token account podpisuje zatwierdzenie.

### Referencje źródłowe

| Element               | Opis                                                                                                                                                           | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_         | Stan token account przechowuje właściciela, delegata i delegowaną kwotę.                                                                                       | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | Instrukcja zatwierdzania delegata, która zapisuje delegata i limit dla token account bez wymagania od wywołującego podania minta lub miejsc dziesiętnych.      | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | Instrukcja zatwierdzania delegata, która wymaga od wywołującego podania minta i miejsc dziesiętnych i sprawdza te wartości przed zapisaniem delegata i limitu. | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
| _rs`process_approve`_ | Wspólna logika procesora dla zatwierdzania delegata.                                                                                                           | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L344-L392)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L582-L640)     |

### Typescript

Poniższe przykłady `Kit` pokazują zalecane podejście z użyciem `@solana/kit`.
Przykłady starszego typu używające `@solana/web3.js` zostały dołączone jako
