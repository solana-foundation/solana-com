---
title: Zatwierdzanie Delegata
description:
  Autoryzuj inny adres do przesyłania lub spalania ograniczonej liczby tokenów z
  konta tokenów.
...
## Co oznacza zatwierdzenie delegata?

Zatwierdzenie delegata pozwala innemu adresowi na przesyłanie lub spalanie
ograniczonej liczby tokenów z konta tokenów w imieniu właściciela konta tokenów.

Konto tokenów przechowuje jednego aktualnego delegata i jedną zatwierdzoną kwotę
na raz. Zatwierdzenie nowego delegata zastępuje poprzedniego delegata i
przyznany limit na koncie.

## Jak zatwierdzić delegata

Zatwierdzanie delegata wykorzystuje instrukcję _rs`Approve`_ lub
_rs`ApproveChecked`_ Token Program.

Poniższe przykłady wykorzystują _rs`ApproveChecked`_, która wymaga od
wywołującego podania mint i miejsc dziesiętnych, aby instrukcja mogła
zweryfikować oczekiwany mint i precyzję tokena przed przetworzeniem
zatwierdzenia.

Właściciel konta tokenów podpisuje zatwierdzenie.

### Odniesienie do źródła

| Element               | Opis                                                                                                                                                                   | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_         | Stan konta tokenów przechowuje właściciela, delegata i zatwierdzoną kwotę.                                                                                             | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Approve`_         | Instrukcja zatwierdzania delegata, która rejestruje delegata i limit dla konta tokenów bez wymagania od wywołującego podania mint lub miejsc dziesiętnych.             | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L109-L127) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L150-L168) |
| _rs`ApproveChecked`_  | Instrukcja zatwierdzania delegata, która wymaga od wywołującego podania mint i miejsc dziesiętnych oraz sprawdza te wartości przed zarejestrowaniem delegata i limitu. | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L274-L300) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L340-L366) |
| _rs`process_approve`_ | Współdzielona logika procesora dla zatwierdzania delegata.                                                                                                             | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L344-L392)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L582-L640)     |

### Typescript

Poniższe przykłady `Kit` pokazują zalecane podejście z użyciem `@solana/kit`.
Przykłady starszej wersji wykorzystujące `@solana/web3.js` zostały załączone w
