...
description:
  Usuń obecnego delegata konta tokenów i wyczyść jego pozostały delegowany
  limit.
...
## Co robi cofnięcie delegata?

Cofnięcie delegata usuwa obecnego delegata konta tokenów i resetuje delegowaną
kwotę do zera.

Po cofnięciu poprzedni delegat nie ma już uprawnień do transferowania ani
niszczenia tokenów w imieniu konta tokenów.

## Jak cofnąć delegata

Cofnięcie delegata wykorzystuje instrukcję _rs`Revoke`_ Token Program.

Instrukcja _rs`Revoke`_ czyści pola delegata na źródłowym koncie tokenów. W
Token Program właściciel konta tokenów podpisuje cofnięcie. W Token Extensions
Program obecny delegat może również cofnąć delegację.

### Źródło odniesienia

| Pozycja              | Opis                                                        | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| -------------------- | ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_        | Stan konta tokenów przechowuje delegata i delegowaną kwotę. | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Revoke`_         | Instrukcja, która usuwa obecnego delegata z konta tokenów.  | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L128-L140) | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L170-L182) |
| _rs`process_revoke`_ | Współdzielona logika procesora dla cofnięcia delegata.      | [Źródło](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L395-L420)     | [Źródło](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L643-L674)     |

### Typescript

Poniższe przykłady `Kit` pokazują zalecane podejście z użyciem `@solana/kit`.
Przykłady legacy z użyciem `@solana/web3.js` są dołączone jako odniesienie.
