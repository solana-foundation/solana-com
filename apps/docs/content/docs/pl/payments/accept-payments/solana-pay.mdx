...
<Callout type="caution">
  Commerce Kit jest obecnie w wersji beta. Interfejsy API mogą ulec zmianie
  przed wydaniem stabilnym. Aby uzyskać dodatkowe informacje, zobacz
  [dokumentację Commerce Kit](/docs/tools/commerce-kit).
</Callout>
...
| Parametr    | Typ         | Opis                                                  |
| ----------- | ----------- | ----------------------------------------------------- |
| `recipient` | `Address`   | Adres portfela sprzedawcy (wymagane)                  |
| `amount`    | `bigint`    | Kwota w lamportach lub minimalnych jednostkach tokena |
| `splToken`  | `Address`   | Adres mint tokena SPL (pomiń dla SOL)                 |
| `reference` | `Address[]` | Unikalne odniesienie(a) do śledzenia płatności        |
| `label`     | `string`    | Nazwa sprzedawcy wyświetlana w portfelu               |
| `message`   | `string`    | Wiadomość wyświetlana przed płatnością                |
| `memo`      | `string`    | Notatka onchain dołączona do transakcji               |
