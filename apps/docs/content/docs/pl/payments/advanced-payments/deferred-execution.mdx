...
transakcji. Aby to zrobić, najpierw musisz pobrać wartość nonce z konta nonce, a
następnie użyć jej do ustawienia okresu ważności transakcji.
...
nonce nie zostanie zwiększony.

## Przepływ zatwierdzania wielostronnego
...
Transakcję można serializować, przechowywać i przekazywać pomiędzy
zatwierdzającymi. Gdy wszystkie wymagane podpisy zostaną zebrane, wyślij ją do
sieci.

## Wykonaj, gdy gotowe

Gdy wszystkie zatwierdzenia są gotowe, wyślij zserializowaną transakcję do
sieci:
...
<Callout type="caution">
  Każdego nonce można użyć tylko raz. Jeśli transakcja się nie powiedzie lub
  zdecydujesz się jej nie wysyłać, musisz zwiększyć nonce przed przygotowaniem
  kolejnej transakcji z tym samym kontem nonce.
</Callout>

## Zwiększanie użytego lub porzuconego nonce
...
- _Każdy_, kto posiada zserializowane bajty transakcji, może ją przesłać do
  sieci
