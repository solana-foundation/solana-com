...
Subskrybent akceptuje aktualne warunki planu. PDA subskrypcji jest wyprowadzane
...
Sprzedawca lub osoba z białej listy podpisuje pobranie. Gdy plan używa listy
dozwolonych miejsc docelowych, właściciel konta tokenów odbiorcy musi znajdować
się na liście w `destinations`.
...
Anulowanie oznacza subskrypcję jako kończącą się. Odwołanie zamyka PDA
subskrypcji po upływie terminu anulowania. Subskrybent podpisuje obie
transakcje.
...
- TypeScript SDK pobiera aktualne warunki planu podczas `subscribe`, gdy je
  pomijasz.
...
- Tylko sprzedawca lub portfel wymieniony w `pullers` może pobierać płatności.
- Subskrybent podpisuje transakcje konfiguracji, anulowania i odwołania.
  Sprzedawca lub zatwierdzony pobierający podpisuje transakcje pobierania
  płatności.
