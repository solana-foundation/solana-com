---
title: Cykliczne delegowanie
description: Przewodnik konfiguracji transakcji cyklicznego delegowania.
---

Cykliczne delegowanie pozwala użytkownikowi zatwierdzić inny portfel lub usługę
do pobierania środków do limitu, który resetuje się w każdym okresie.

Ten przewodnik zachowuje widoczność wszystkich elementów. Najpierw wyprowadzasz
konta, inicjalizujesz Subscription Authority w razie potrzeby, tworzysz
cykliczne delegowanie, a następnie używasz tego PDA delegowania do transferów.
...
2. Wyprowadź token account użytkownika, PDA Subscription Authority oraz PDA
   cyklicznego delegowania.
3. Zainicjalizuj Subscription Authority, jeśli jeszcze nie istnieje.
4. Utwórz cykliczne delegowanie z kwotą okresu, długością okresu, czasem
   rozpoczęcia i wygaśnięciem.
...
Delegat podpisuje każdy transfer. Program sprawdza bieżący okres i odrzuca
transfery, które przekroczyłyby pozostały limit okresu.
...
Delegujący może w każdej chwili cofnąć delegację cykliczną. Cofnięcie zamyka PDA
delegacji i zwraca jej rent do podpisującego.
...
- `amountPerPeriod` podawany jest w jednostkach bazowych. Dla tokena z 6
  miejscami dziesiętnymi `1_000_000` oznacza `1` tokena.
- Program odrzuca przelewy przekraczające pozostały limit bieżącego okresu.
- Po rozpoczęciu nowego okresu pobrana kwota jest resetowana.
- `expiryTs` to twarde ograniczenie: po jego upływie przelewy są odrzucane, a
  sponsor może odzyskać rent. Nie ma okresu prolongaty.
- Ustaw `startTs` na `0`, aby uruchomić delegację w momencie, gdy transakcja
  zostanie zapisana w łańcuchu, zamiast o stałej godzinie. Dzięki temu wydłuża
  się okno czasowe na uzyskanie podpisu użytkownika i jego wdrożenie. W takim
  przypadku `expiryTs` musi być różne od zera — delegacja uruchamiana przy
  lądowaniu nie może być jednocześnie ustawiona jako nigdy niewygasająca.
