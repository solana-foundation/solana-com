...
<Callout type="caution" title="Nie używaj publicznego RPC w produkcji">
  Publiczne punkty końcowe (`api.mainnet.solana.com`, `api.devnet.solana.com`) są
  objęte limitami żądań, nie mają umowy SLA i nie nadają się do produkcyjnych przepływów płatności.
  Użyj [dostawcy RPC](https://solana.com/rpc), aby uzyskać prywatny punkt końcowy RPC
