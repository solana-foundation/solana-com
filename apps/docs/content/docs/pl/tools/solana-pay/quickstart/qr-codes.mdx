---
title: Integracja kodów QR
description: Generuj i dostosowuj kody QR do płatności w portfelach mobilnych
---

Kody QR to najpopularniejszy sposób łączenia aplikacji internetowych z
portfelami mobilnymi. Solana Pay zawiera wbudowaną funkcję `createQR` opartą na
`@solana/qr-code-styling` — nie są potrzebne dodatkowe pakiety do kodów QR.

## Podstawowe generowanie kodów QR
...
Funkcja `createQR` przyjmuje opcjonalne parametry rozmiaru, koloru tła i koloru
pierwszego planu:
...
### Pobieranie opcji QR do zaawansowanego użytku

Użyj `createQROptions`, aby uzyskać surowy obiekt konfiguracji zapewniający
większą kontrolę:
...
## Korzystanie z klienta sprzedawcy

Klient sprzedawcy udostępnia również metody kodów QR:
...
### QR płatności z aktualizacjami statusu
...
## Integracja z terminalem sprzedaży

### Wyświetlanie QR w terminalu POS
...
### Wyświetlanie zoptymalizowane pod kątem urządzeń mobilnych

Na urządzeniach mobilnych rozważ udostępnienie bezpośredniego linku obok kodu
QR:
