---
title: Asercje
description: Weryfikuj stany kont, salda i własność za pomocą helperów asercji
---

## Istnienie konta

Sprawdź, czy konta istnieją lub zostały zamknięte.

### Asercja istnienia konta
...
<Callout type="info">
  `assert_account_closed` przechodzi, gdy konto nie istnieje, ma zero lamports
  lub ma puste dane. Użyj tego, aby zweryfikować czyszczenie po zamknięciu kont.
</Callout>

## Asercje salda

Weryfikuj salda SOL i token account.
...
Weryfikuj własność konta i właściwości danych.

### Asercja właściciela konta
...
<Callout type="info">
  Typowe rozmiary kont: - **Mint Account**: 82 bajty - **Token Account**: 165
  bajtów - **System Account**: 0 bajtów (tylko lamports)
</Callout>

## Kompletny przykład

Oto kompleksowy test demonstrując wiele asercji:
...
Gdy asercja nie powiedzie się, wywoła panikę z opisowym komunikatem:
...
<Callout type="warn">
  Asercje są zaprojektowane tak, aby szybko zawodzić z czytelnymi komunikatami o
  błędach. Używaj ich często w swoich testach, aby wcześnie wykrywać problemy.
</Callout>
