---
title: Pomocnicze funkcje transakcji
description: Wykonuj transakcje i analizuj wyniki z rozbudowaną obsługą błędów
---

## Wysyłanie transakcji

Trait `TransactionHelpers` udostępnia wygodne metody wysyłania instrukcji i
analizowania wyników.

### Wyślij pojedynczą instrukcję
...
Metody `send_instruction` i `send_instructions` zwracają `TransactionResult`,
który opakowuje metadane transakcji przydatnymi narzędziami testowymi.
...
<Callout type="info">
  `assert_success()` wywoła panikę z pełnymi logami transakcji, jeśli transakcja
  się nie powiodła, co znacznie ułatwia debugowanie.
</Callout>

### Potwierdź niepowodzenie
...
W przypadku niestandardowych błędów programu możesz sprawdzać kod błędu:
...
### Potwierdź błąd Anchor

Dla programów Anchor sprawdzaj nazwy błędów:
...
### Drukowanie logów do debugowania
...
### Sprawdzanie jednostek obliczeniowych
...
<Callout type="info">
  Monitorowanie jednostek obliczeniowych jest przydatne do optymalizacji. Solana
  ogranicza transakcje do 1,4 mln jednostek obliczeniowych, a wyższe zużycie
  kosztuje więcej.
</Callout>

## Kompletny przykład

Oto kompleksowy przykład demonstrujący helpery transakcji:
...
Enum `TransactionError` dostarcza ustrukturyzowanych informacji o błędach:

| Wariant                   | Opis                                                |
| ------------------------- | --------------------------------------------------- |
| `ExecutionFailed(String)` | Wykonanie transakcji nie powiodło się z komunikatem |
| `BuildError(String)`      | Nie udało się zbudować transakcji                   |
| `AssertionFailed(String)` | Asercja wyniku nie powiodła się                     |
