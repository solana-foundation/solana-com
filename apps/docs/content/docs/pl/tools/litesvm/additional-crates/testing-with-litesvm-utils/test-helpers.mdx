---
title: Pomocnicze narzędzia testowe
description:
  Narzędzia do tworzenia kont, operacji na tokenach, wyprowadzania PDA oraz
  zarządzania slot
---

## Tworzenie zasilonych kont

Najpowszechniejszą potrzebą podczas testowania jest tworzenie kont z SOL. Cecha
`TestHelpers` sprawia, że jest to proste.
...
Twórz minty tokenów i konta bez zbędnego powtarzalnego kodu.

### Tworzenie mintu tokenu
...
<Callout type="info">
  **W większości przypadków używaj ATA.** Associated Token Accounts są
  deterministyczne i zgodne z konwencjami Solany. Używaj zwykłych token account
  tylko wtedy, gdy potrzebujesz wielu kont dla tej samej pary mint/właściciel.
</Callout>
...
Łatwo wyprowadzaj Program Derived Addresses na potrzeby swoich testów.

### Podstawowe wyprowadzanie PDA
...
<Callout type="info">
  Użyj `derive_pda` gdy potrzebujesz bump seed (np. do wywołań CPI). Użyj
  `get_pda` gdy potrzebujesz tylko adresu.
</Callout>

## Zarządzanie slot

Kontroluj postęp czasu na potrzeby testowania logiki zależnej od czasu.

### Pobierz bieżący slot
...
Oto kompleksowy przykład użycia wielu pomocników testowych jednocześnie:
