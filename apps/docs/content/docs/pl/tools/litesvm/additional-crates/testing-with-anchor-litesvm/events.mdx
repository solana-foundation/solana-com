---
title: Zdarzenia
description: Parsowanie i weryfikacja zdarzeń Anchor z logów transakcji
---

## Zrozumienie zdarzeń Anchor

Programy Anchor mogą emitować zdarzenia, które są rejestrowane podczas
wykonywania transakcji. Zdarzenia te są zakodowane w Base64 w logach transakcji
z prefiksem `Program data:`. Crate `anchor-litesvm` udostępnia narzędzia do
parsowania i weryfikacji tych zdarzeń.
...
Najpierw zdefiniuj zdarzenia w swoim programie Anchor:
...
### Parsowanie wszystkich zdarzeń danego typu
...
### Parsowanie pojedynczego zdarzenia
...
### Sprawdzenie, czy zdarzenie zostało wyemitowane
...
<Callout type="info">
  `assert_event_emitted` spowoduje panikę, jeśli w logach transakcji nie zostaną
  znalezione żadne zdarzenia określonego typu.
</Callout>

### Sprawdzenie liczby zdarzeń
...
## Ręczne parsowanie zdarzeń

W przypadku zaawansowanych zastosowań możesz parsować dane zdarzeń bezpośrednio:
...
Enum `EventError` dostarcza szczegółowych informacji o błędach:
...
| Błąd                  | Opis                                        |
| --------------------- | ------------------------------------------- |
| `EventNotFound`       | Brak zdarzenia określonego typu w logach    |
| `ParseError(String)`  | Nie udało się przetworzyć danych zdarzenia  |
| `Base64Error`         | Nie udało się zdekodować Base64             |
| `InvalidFormat`       | Dane zdarzenia mają nieprawidłową strukturę |
| `AnchorError(String)` | Deserializacja Anchor nie powiodła się      |

## Kompletny przykład

Oto kompleksowy przykład demonstrujący obsługę zdarzeń:
...
<Callout type="info">
  Zdarzenia to doskonały sposób na weryfikację poprawności wykonania programu
  bez konieczności pobierania i deserializacji kont. Są szczególnie przydatne do
  śledzenia zmian stanu w czasie.
</Callout>
