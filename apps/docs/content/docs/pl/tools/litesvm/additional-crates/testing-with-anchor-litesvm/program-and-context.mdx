...
description:
  Konfigurowanie środowisk testowych i budowanie instrukcji za pomocą
  AnchorContext i Program
---

## Generowanie typów klienta za pomocą declare_program!

Anchor 1.0 wprowadza `declare_program!`, który odczytuje IDL programu w trakcie
kompilacji i generuje bezpieczne typowo moduły `client::accounts::*` i
`client::args::*`. Jest to zalecany sposób budowania instrukcji w testach.
...
<Callout type="info">
  Najpierw uruchom `anchor build`, aby wygenerować `target/idl/my_program.json`.
  `declare_program!` odczytuje ten plik w trakcie kompilacji. Nazwa makra musi
  odpowiadać nazwie skrzynki programu (snake_case).
</Callout>

## Konfigurowanie środowiska testowego

Konstruktor `AnchorLiteSVM` udostępnia płynne API do konfigurowania środowiska
testowego.
...
### Skrócona składnia dla pojedynczego programu
...
<Callout type="info">
  Jeśli nie określisz płatnika, `AnchorLiteSVM` automatycznie utworzy i zasili
  go środkami.
</Callout>

## AnchorContext

`AnchorContext` to główny interfejs do interakcji ze środowiskiem testowym.
...
## Budowanie instrukcji za pomocą programu

Struktura `Program` zapewnia płynne API do budowania instrukcji, które
odzwierciedla anchor-client. Używaj typów generowanych przez `declare_program!`
dla bezpieczeństwa typów.

### Podstawowe budowanie instrukcji
...
<Callout type="info">
  Metoda `.accounts()` przyjmuje dowolny typ implementujący `ToAccountMetas`, co
  wszystkie struktury kont Anchor robią automatycznie. Podobnie, `.args()`
  przyjmuje dowolny typ implementujący `InstructionData`.
</Callout>
...
### Używanie send_and_confirm_transaction
...
Oto kompleksowy przykład wykorzystujący wzorzec escrow z `declare_program!`:
