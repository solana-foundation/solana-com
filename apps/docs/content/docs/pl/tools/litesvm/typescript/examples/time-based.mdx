...
description: Testuj logikę zależną od czasu za pomocą manipulacji zegarem LiteSVM
---
...
## Testowanie logiki zablokowanej czasowo

Przykładowy wzorzec testowania funkcjonalności zablokowanej czasowo:
...
Możesz odczytać harmonogram epoch, aby zrozumieć relację slot/epoch w
klastrze:
...
<Callout type="warn">
  ...
  pola `epoch` ani `unixTimestamp`. Użyj `setClock`, aby mieć pełną kontrolę nad
  polami zegara.
</Callout>

## Ręczna manipulacja zegarem

Gdy Twój program odczytuje `epoch` lub `unix_timestamp` z sysvaru Clock, użyj
...
To odzwierciedla wzorzec Rust `set_sysvar(&clock)`. Używaj `warpToSlot` do prostego
...
| ----------------------- | ---------------------------------- |
| Nabywanie tokenów (vesting)           | Przesuń poza próg/kamienie milowe vestingu |
| Zakończenia aukcji         | Przesuń poza slot zakończenia aukcji         |
| Nagrody za stakowanie         | Użyj `setClock`, aby ustawić epoch        |
| Wypłaty zablokowane czasowo | Przesuń poza slot odblokowania              |
...
2. **Przesuwanie do przodu**: Użyj `warpToSlot(slot)`, aby przesunąć slot
3. **Pełna kontrola**: Użyj `setClock`, gdy chcesz ustawić `epoch`,
   `unixTimestamp` lub inne pola zegara
4. **Odczyt zegara**: Użyj `getClock()`, aby odczytać bieżący slot, epoch i znacznik czasu
