...
description: Ładowanie i testowanie niestandardowych programów Solana za pomocą litesvm-go
---

Testowanie niestandardowego programu Solana z poziomu Go oznacza załadowanie skompilowanego pliku `.so`
do SVM, skonfigurowanie oczekiwanych kont i wysyłanie instrukcji
do programu. `litesvm-go` obsługuje to wszystko w procesie — nie jest potrzebny żaden validator.
...
Do testów wymagających standardowych programów (SPL Token, Memo, ...):
...
<Callout type="info">
  Skompiluj swój program Solana za pomocą `cargo build-sbf`, aby wygenerować plik `.so`.
</Callout>

Schemat jest zawsze taki sam:

1. **Konfiguracja** — włącz sysvary, wbudowane funkcje i prekompilacje według potrzeb.
2. **Ładowanie programu** — `AddProgramFromFile` (lub `AddProgram` dla bajtów
   w pamięci).
3. **Konfiguracja kont** — wstępnie wypełnij konta danych za pomocą `SetAccount`.
4. **Wysyłanie** — zbuduj instrukcję za pomocą `solana-go`, serializuj i prześlij przez
   `SendLegacyTransaction`.
5. **Weryfikacja** — odczytaj konta za pomocą `GetAccount` i sprawdź logi,
   jednostki obliczeniowe oraz stan końcowy.

Gdy ten szkielet będzie już działać, rozbuduj go, dodając więcej instrukcji, więcej
kont i pomocnika do obsługi powtarzalnego kodu.
