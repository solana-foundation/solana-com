...
- [ ] Twój signer jest obsługiwany w paczce `solana-keychain`
...
- [ ] (Opcjonalnie) Dodano metodę konstruktora testowego mocka w
      `config_mock.rs`
- [ ] Kod kompiluje się bez ostrzeżeń
- [ ] Wszystkie testy przechodzą (`make test` i `make test-integration`)
- [ ] Dokumentacja dodana zgodnie z
      [`ADDING_SIGNERS.md`](https://github.com/solana-foundation/solana-keychain/blob/main/docs/ADDING_SIGNERS.md)
- [ ] Utworzono przykładowe pliki konfiguracyjne (`.toml` i `.env.example`)
- [ ] Brak zakodowanych na stałe wartości lub sekretów
- [ ] Komunikaty błędów są pomocne
- [ ] Zgodność z konwencjami nazewnictwa Rust (snake_case)
...
- **W sprawie integracji z Kora**: Otwórz zgłoszenie w repozytorium Kora w celu
  omówienia projektu
- Dołącz do naszych kanałów społecznościowych
- Przejrzyj istniejące konfiguracje signerów w
...
Witamy w ekosystemie Kora! Cieszymy się, że Twoje rozwiązanie do zarządzania
