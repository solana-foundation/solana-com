...
> _Uwaga: Gdy `fallback_if_unavailable` ma wartość true, system zezwala na kontynuowanie transakcji
> w przypadku tymczasowej niedostępności Redis, zapobiegając przerwaniu usługi.
> Ustawienie `max_transactions` na 0 zezwoli na nieograniczoną liczbę transakcji._
...
| `get_payer_signer`          | Zwraca następnego płatnika opłaty i adres płatności        | ✅       | boolean |
...
| ---------------------------- | ------------------------------------------------ |
| `confidential_transfer_mint` | Konfiguracja Poufnych Sald dla mint |
...
| Nazwa rozszerzenia                  | Opis                                 |
| ------------------------------- | ------------------------------------------- |
| `confidential_transfer_account` | Stan Poufnych Sald dla konta |
