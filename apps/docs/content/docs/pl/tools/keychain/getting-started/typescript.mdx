...
Fordefi obsługuje dwa tryby podpisywania, zależne od tego, czy przekażesz `chain`. Żądania API są uwierzytelniane za pomocą sygnatury żądania ECDSA P-256 — dostarcz klucz podpisujący jako `privateKeyPem` lub przechowuj go w KMS/HSM, przekazując `requestSigner`.

**Natywny tryb Solana** (zalecany do użytku on-chain): ustaw `chain`, a Fordefi sam podpisze i rozgłosi transakcję. Ponieważ Fordefi może zastąpić blockhash lub opłaty przed podpisaniem, sygnatariusz jest Kit `TransactionSendingSigner` — używaj go z `signAndSendTransactionMessageWithSigners`, a nie z przepływem częściowego sygnatariusza pokazanym powyżej.
...
**Tryb czarnej skrzynki**: pomiń `chain`. Fordefi podpisuje surowe bajty i nie rozgłasza transakcji; samodzielnie składasz i przesyłasz transakcję. Używaj tego trybu z sejfem czarnej skrzynki Fordefi.
