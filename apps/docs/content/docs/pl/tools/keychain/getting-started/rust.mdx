...
| `utila`      | Usługa powiernicza Utila MPC               |
| `fordefi`    | Custody MPC Fordefi             |
...
Do podpisywania w ramach custody MPC Fordefi. Żądania API są uwierzytelniane za pomocą podpisu żądania ECDSA P-256 odczytanego z `private_key_pem`; aby przechowywać ten klucz w KMS/HSM, zaimplementuj `FordefiRequestSigner` i użyj `Signer::from_fordefi_with_signer`.

Fordefi oferuje dwa tryby podpisywania. Ustaw `chain` dla **natywnego trybu Solana**, w którym Fordefi może zastąpić blockhash lub opłaty i samodzielnie rozgłasza transakcję. Pozostaw `chain` jako `None` dla **trybu czarnej skrzynki**, w którym Fordefi podpisuje surowe bajty, a transakcję rozgłaszasz samodzielnie.
