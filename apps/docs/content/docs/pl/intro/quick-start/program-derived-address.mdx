...
czy program startowy buduje się poprawnie.
...
W pliku `lib.rs` zaktualizuj strukturę _rs`MessageAccount`_ o następujące
elementy:
...
typu danych _rs`MessageAccount`_. Sekcja testowa pokazuje proces tworzenia i
odczytywania danych konta.
...
Rozpocznij od zdefiniowania kont wymaganych przez instrukcję, aktualizując
strukturę _rs`Create`_ w następujący sposób:
...
1. _rs`user: Signer<'info>`_
   ...
   - Wymaga statusu zmiennego (_rs`#[account(mut)]`_), ponieważ płaci za nowe
     konto
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - `space` przydziela wymaganą liczbę bajtów na pole danych konta
...
Obliczenie `space` _rs`(8 + 32 + 4 + message.len() + 1)`_ przydziela miejsce na
typ danych `MessageAccount`:
...
Typ _rs`String`_ potrzebuje 4 bajtów na przechowywanie długości ciągu, a
...
Zaktualizuj strukturę `Update` o następujące elementy:
...
1. _rs`user: Signer<'info>`_
   ...
   - Oznaczone jako mutowalne (_rs`#[account(mut)]`_), ponieważ może płacić za
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - Ograniczenie `realloc` pozwala na zmianę rozmiaru danych konta
...
przechowywanego w _rs`message_account`_, zamiast ponownie wyliczać go przez
Anchor.

Atrybut _rs`#[instruction(message: String)]`_ umożliwia strukturze _rs`Update`_
odwołanie się do parametru _rs`message`_ z instrukcji _rs`update`_.
...
Zaktualizuj strukturę _rs`Delete`_ w następujący sposób:
...
W tym przypadku program używa istniejącego bump seed przechowywanego w
...
1. _rs`user: Signer<'info>`_
   ...
   - Oznaczone jako mutowalne (_rs`#[account(mut)]`_), aby otrzymać lamporty z
     zamykanego konta
...
Ukończyłeś właśnie podstawowy program CRUD. Wdróż program, uruchamiając `deploy`
...
[`pg.wallet`](mention:two) daje dostęp do twojego portfela w playgroundzie.
...
konto nie istnieje już po zamknięciu.
...
Playground. To polecenie uruchamia testy względem programu wdrożonego na
devnecie i loguje linki do SolanaFM, aby zobaczyć szczegóły transakcji.
