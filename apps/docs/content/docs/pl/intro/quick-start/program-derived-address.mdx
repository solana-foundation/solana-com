...
description:
  Dowiedz się, jak zbudować program CRUD (tworzenie, odczyt, aktualizacja,
  usuwanie) na Solana przy użyciu Program Derived Address (PDA) i frameworka
  Anchor. Ten przewodnik krok po kroku pokazuje, jak tworzyć, aktualizować i
  usuwać konta wiadomości onchain przy użyciu PDA, implementować walidację kont
  i pisać testy. Idealny dla deweloperów, którzy chcą zrozumieć, jak
  wykorzystywać PDA w programach Solana.
...
[`update`](mention:two) oraz [`delete`](mention:three), które należy dodać w
kolejnych krokach.
...
Zanim zaczniesz, uruchom _shell`build`_ w terminalu Playground, aby sprawdzić,
czy program startowy kompiluje się poprawnie.
...
W pliku `lib.rs` zaktualizuj strukturę _rs`MessageAccount`_ w następujący
sposób:
...
reprezentują dane konta (typ danych przechowywany w polu data konta).
...
Pole _rs`message`_ przechowuje _rs`String`_, który zawiera wiadomość
użytkownika.

### !bump

Pole _rs`bump`_ przechowuje _rs`u8`_
[„bump” seed](/docs/core/pda#canonical-bump) używany do wyprowadzenia program
derived address (PDA). Przechowywanie tej wartości oszczędza zasoby
obliczeniowe, eliminując konieczność ponownego wyliczania jej w kolejnych
instrukcjach.
...
- `bump` – _rs`u8`_, który przechowuje
  [„bump” seed](/docs/core/pda#canonical-bump) do wyprowadzenia program derived
  address (PDA). Przechowywanie tej wartości oszczędza zasoby obliczeniowe,
  eliminując konieczność ponownego wyliczania jej w kolejnych instrukcjach.

Podczas tworzenia konta program serializuje dane _rs`MessageAccount`_ i zapisuje
je w polu danych nowego konta.

Później, podczas odczytu z konta, program deserializuje te dane z powrotem do
typu danych _rs`MessageAccount`_. W sekcji testowania pokazano proces tworzenia
i odczytywania danych konta.

</Accordion>
</Accordions>

Zbuduj program ponownie, uruchamiając w terminalu _shell`build`_.
...
strukturę _rs`Create`_ o następujące elementy:
...
Ograniczenie _rs`space`_ określa liczbę bajtów przeznaczonych na pole danych
nowego konta.
...
Typ _rs`Account<'info, T>`_ wymaga, aby konto odpowiadało określonemu typowi.
...
Atrybut _rs`#[derive(Accounts)]`_ w programie Anchor adnotuje struktury, które
definiują konta wymagane przez instrukcję.
...
1. Typ konta (np. _rs`Signer<'info>`_ lub _rs`Account<'info, T>`_), który
   określa, jakiego rodzaju konta oczekuje program
...
1. _rs`user: Signer<'info>`_
   ...
   - Wymaga statusu mutowalnego (_rs`#[account(mut)]`_), ponieważ płaci za nowe
     konto
   - Musi podpisać transakcję, aby zatwierdzić pobranie lamportów z tego konta

2. _rs`message_account: Account<'info, MessageAccount>`_
   - Nowe konto przechowujące wiadomość użytkownika
   - Ograniczenie `init` tworzy konto podczas wykonywania instrukcji
   - Ograniczenia `seeds` i `bump` wyznaczają adres konta jako Program Derived
     Address (PDA)
   - `payer = user` określa, kto płaci za utworzenie nowego konta
   - `space` przydziela wymaganą liczbę bajtów na pole data konta

3. _rs`system_program: Program<'info, System>`_
   - Niezbędne do utworzenia konta
   - W tle ograniczenie `init` wywołuje System Program, aby utworzyć nowe konto
     z określonym `space` i zmienia właściciela na bieżący program.
...
- _rs`b"message"`_ – Stały string jako pierwszy seed.
...
poprawnego bump seed. Anchor używa `seeds` oraz `bump` do wyprowadzenia PDA.

---

Obliczenie `space` _rs`(8 + 32 + 4 + message.len() + 1)`_ alokuje miejsce na typ
danych `MessageAccount`:

- Anchor Account discriminator (identyfikator): 8 bajtów
...
Typ _rs`String`_ potrzebuje 4 bajtów na przechowanie długości stringa, a
pozostała długość zawiera właściwe dane.

</Accordion>
</Accordions>

Następnie dodaj logikę biznesową dla instrukcji _rs`create`_ poprzez
zaktualizowanie funkcji `create` o następujące elementy:
...
1. _rs`ctx: Context<Create>`_ – Zapewnia dostęp do kont określonych w strukturze
   _rs`Create`_.
2. _rs`message: String`_ – Wiadomość użytkownika do zapisania.

Ciało funkcji wykonuje następującą logikę:

1. Wyświetl wiadomość w logach programu za pomocą makra _rs`msg!()`_.
...
2. Inicjalizacja danych konta:
   ...
   - Ustawia pole `message` na wartość `message` przekazaną jako argument
     funkcji.
   ...
   - Ustawia wartość `bump` używaną do wyznaczenia PDA, pobraną z
...
1. _rs`user: Signer<'info>`_
   ...
   - Oznaczone jako zmienne (_rs`#[account(mut)]`_), ponieważ może płacić za
     dodatkowe miejsce dla `message_account` w razie potrzeby
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - Ograniczenie `realloc` umożliwia zmianę rozmiaru danych konta
   - Ograniczenia `seeds` i `bump` weryfikują konto jako poprawne PDA

3. _rs`system_program: Program<'info, System>`_
   - Wymagane do ewentualnej realokacji miejsca na koncie
   - Ograniczenie `realloc` wywołuje System Program w celu dostosowania rozmiaru
     danych konta

Zwróć uwagę, że ograniczenie _rs`bump = message_account.bump`_ używa bump seed
przechowywanego na _rs`message_account`_, zamiast ponownie wyliczać go przez
Anchor.

Atrybut _rs`#[instruction(message: String)]`_ umożliwia strukturze _rs`Update`_
dostęp do parametru _rs`message`_ z instrukcji _rs`update`_.
...
1. _rs`ctx: Context<Update>`_ – Zapewnia dostęp do kont określonych w strukturze
   _rs`Update`_ .
2. _rs`message: String`_ – Nowa wiadomość, która zastąpi obecną.

Ciało funkcji następnie:

1. Wyświetl komunikat w logach programu za pomocą makra _rs`msg!()`_.

2. Aktualizacja danych konta:
   - Uzyskuje dostęp do `message_account` z kontekstu.
   - Ustawia pole `message` na nową wartość `message` przekazaną jako argument
     funkcji.
...
Ograniczenie _rs`seeds`_ określa seed-y używane do wyprowadzenia PDA.
...
1. _rs`user: Signer<'info>`_
   - Reprezentuje użytkownika zamykającego konto z wiadomością
   - Oznaczony jako mutowalny (_rs`#[account(mut)]`_), aby otrzymać lamporty z
     zamykanego konta
   - Musi podpisać, aby mieć pewność, że tylko właściwy użytkownik może zamknąć
     swoje konto z wiadomością

2. _rs`message_account: Account<'info, MessageAccount>`_
   - Konto do zamknięcia
   - Ograniczenie `mut` wskazuje na modyfikację danych
   - Ograniczenia `seeds` i `bump` weryfikują konto jako poprawne PDA
   - Ograniczenie `close = user` oznacza to konto do zamknięcia i przekazuje
     jego lamporty na konto `user`
...
1. _rs`_ctx: Context<Delete>`_ – Zapewnia dostęp do kont określonych w
   strukturze _rs`Delete`_. Składnia _rs`_ctx`_ pokazuje, że funkcja nie
   korzysta z Context w swoim ciele.

Ciało funkcji po prostu wypisuje wiadomość do logów programu za pomocą makra
...
Ukończyłeś już podstawowy program CRUD. Wdróż program, uruchamiając `deploy` w
terminalu Playground.
...
Kod startowy zawiera także plik testowy w `anchor.test.ts`.
...
Solana Playground usuwa część powtarzalnej konfiguracji, gdzie
[`pg.program`](mention:one) umożliwia dostęp do metod interakcji z programem,
podczas gdy [`pg.wallet`](mention:two) daje dostęp do twojego portfela w
playgroundzie.
...
Uruchom plik testowy, wpisując _shell`test`_ w terminalu Playground, aby
sprawdzić, czy działa zgodnie z oczekiwaniami. Kolejne kroki dodadzą właściwe
testy.
...
przekazując jako wiadomość "Hello, World!".
...
Najpierw plik testowy wysyła transakcję wywołującą instrukcję `update`,
przekazując jako nową wiadomość "Hello, Solana!".
...
Po wysłaniu transakcji i zamknięciu konta przykład próbuje pobrać konto,
używając jego adresu (`messagePda`) za pomocą `fetchNullable`. Ta metoda zwraca
null, gdy konto nie istnieje już po zamknięciu.
...
<Callout type="info">
  ...
  jako konto. Dla danego PDA może istnieć tylko jedno konto.
</Callout>
