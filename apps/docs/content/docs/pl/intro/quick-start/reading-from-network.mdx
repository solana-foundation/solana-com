...
Odczytuj dane z sieci Solana, pobierając różne konta. Ta sekcja pomože ci
zrozumieć strukturę [kont](/docs/core/accounts) Solany. Każde konto Solany ma
unikalny [adres](/docs/core/accounts#account-address), który służy do
lokalizacji jego odpowiadających mu danych onchain. Konta zawierają albo
[dane stanu, albo program wykonywalny](/docs/core/accounts#types-of-accounts).
...
Pole `data` zawiera dane konta zapisane jako bajty. Dla kont portfeli to pole
jest puste (0 bajtów).
...
programu. Dla kont portfeli to pole ma wartość `false`.
...
Pole `owner` wskazuje program, który jest właścicielem konta. W przypadku
portfeli właścicielem jest zawsze System Program, o adresie
...
Pole `rentEpoch` to pole historyczne pochodzące ze zdeprecjonowanego mechanizmu
rent. (To pole jest uwzględnione dla zachowania kompatybilności wstecznej.)
...
[typie Account](/docs/core/accounts#account-structure) jako takim, ale jest
uwzględnione w odpowiedzi.

W tym przykładzie pole `space` ma wartość 0, ponieważ pole `data` zawiera 0
bajtów danych.
...
Pole `executable` jest ustawione na `true`, co oznacza, że pole `data` konta
...
[loader programu](/docs/core/programs#loader-programs). W tym przykładzie
...
Pole `data` mint accountu przechowuje stan, a nie kod wykonywalny, więc pole
`executable` jest `false`.
...
Pole `data` zawiera zserializowany stan konta `Mint`, taki jak authority mintu,
całkowita podaż, liczba miejsc po przecinku.

Aby odczytać dane z mint accountu, należy zdeserializować pole `data` do typu
danych `Mint`, co pokazano w [następnym przykładzie](#deserialize-mint-account).
...
pole `data` może być modyfikowane tylko przez instrukcje Token Programu.
...
Zanim surowe bajty w polu `data` konta będą mogły być interpretowane w sensowny
sposób, muszą zostać zdeserializowane. Odpowiedni typ danych jest definiowany
przez program będący właścicielem konta. Większość programów Solana udostępnia
biblioteki klienckie z funkcjami pomocniczymi, które upraszczają proces
deserializacji. Funkcje te konwertują surowe bajty konta na ustrukturyzowane
typy danych, co ułatwia pracę z danymi konta.

<WithMentions>

Na przykład biblioteka _shell`@solana/spl-token`_ zawiera funkcję
[_ts`getMint()`_](mention:one), która pomaga zdeserializować pole `data` mint
accountu do typu danych
...
Funkcja _ts`getMint()`_ deserializuje pole `data` mint accountu do typu konta
Mint.
...
wartość jest podana w najmniejszej jednostce tokena. Aby uzyskać całkowitą podaż
w standardowych jednostkach, przelicz wartość pola `supply` przez `decimals`.
...
Pole `isInitialized` wskazuje, czy mint account został zainicjalizowany. To pole
jest wykorzystywane jako kontrola bezpieczeństwa w Token Program.
...
Pole `freezeAuthority` pokazuje konto z uprawnieniami do zamrażania token
account. Zamrożony token account nie może transferować ani spalać
przechowywanego tokena.
