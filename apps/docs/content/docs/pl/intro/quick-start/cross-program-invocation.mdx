...
Funkcja _rs`transfer()`_ wywołuje instrukcję transferu Programu Systemowego.
...
aby wywołać instrukcję `transfer` Programu Systemowego. Pokazuje to, jak wykonać
CPI wewnątrz programu, umożliwiając komponowalność programów Solana.
...
Konfiguracja CPI odpowiada temu, jak budowane są instrukcje po stronie klienta,
...
wywołuje instrukcję transferu Programu Systemowego.
...
Najpierw zaktualizuj strukturę _rs`Delete`_, aby dodać `vault_account`. Pozwala
...
Struktura _rs`signer_seeds`_ określa opcjonalne seedy i bump seedy używane do
wyznaczania PDA.
