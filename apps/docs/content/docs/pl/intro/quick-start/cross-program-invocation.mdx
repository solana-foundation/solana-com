...
struktury _rs`Update`_ oraz funkcji `update`.
...
Następnie zaktualizuj strukturę _rs`Update`_, aby dodać nowe konto o nazwie
...
To konto pełni rolę kontrolowanego przez program "skarbcem", który otrzymuje SOL
od użytkowników podczas aktualizacji ich wiadomości.
...
- Jako PDA nie posiada klucza prywatnego, więc tylko program może "podpisywać"
  ten adres podczas wykonywania CPI
- Jako typ _rs`SystemAccount`_, System Program jest jego właścicielem jak
  zwykłych kont portfela
...
W instrukcji `delete` zobaczysz, jak program może "podpisać" tę PDA w CPI.
...
Struktura _rs`CpiContext`_ określa program i konta dla wywołania
międzyprogramowego (CPI).
...
W instrukcji `update` implementacja zawiera wywołanie międzyprogramowe (CPI),
aby wywołać instrukcję `transfer` System Programu. Pokazuje to, jak wykonać CPI
z poziomu programu, umożliwiając kompozycyjność programów Solana.

Struktura _rs`Transfer`_ określa wymagane konta dla instrukcji transferu System
Programu:

- `from` – konto użytkownika (źródło środków)
- `to` – konto skarbca (cel transferu)
...
- `amount` do przelania (1 000 000 lamportów, czyli 0,001 SOL)
...
Konfiguracja CPI odpowiada sposobowi budowania instrukcji po stronie klienta,
gdzie określasz program, konta i instruction data dla danej instrukcji do
wywołania. Gdy instrukcja `update` programu otrzyma wywołanie, wewnętrznie
wywołuje instrukcję transferu System Programu.
...
_rs`Delete`_ oraz funkcji `delete`.

Najpierw zaktualizuj strukturę _rs`Delete`_, aby uwzględnić `vault_account`.
Pozwala to na zwrot wszystkich SOL ze skarbca do użytkownika, gdy zamyka on
...
`vault_account` używa tej samej metody wyznaczania PDA, co w strukturze Update.

Dodanie `vault_account` do struktury Delete umożliwia programowi dostęp do konta
skarbca użytkownika podczas instrukcji usuwania, aby przelać zgromadzone SOL z
powrotem do użytkownika.
...
_rs`signer_seeds`_ określają opcjonalne seedy i bump seedy używane do
wyznaczania PDA.

### !with_signer

Metoda _rs`with_signer()`_ przekazuje seedy podpisującego wraz z CPI.
...
Zwróć uwagę, że _rs`_ctx: Context<Delete>`_ zmienia się na
...
Programie, przekazując:

- `cpi_context` (program, konta i podpisujący PDA)
- Kwotę do przelania (całe saldo konta skarbca)
