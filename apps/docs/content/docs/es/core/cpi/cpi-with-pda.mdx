---
title: CPIs con firmantes PDA
description:
  Cómo usar la función invoke_signed para invocaciones entre programas en Solana
  cuando el programa que llama necesita firmar como un PDA, con ejemplos de
  Anchor CpiContext y Rust nativo.
...
<Callout type="info" title="Resumen">
  Usa _rs`invoke_signed`_ cuando el programa que llama necesita firmar en nombre
...
con las [semillas de firmante](mention:signer-seeds) usadas para derivar el
[PDA](/docs/core/pda). Para más detalles sobre cómo el runtime verifica las
firmas PDA, consulta [firma PDA](/docs/core/cpi/cpi-cost-model#pda-signing).
...
Los ejemplos a continuación realizan un CPI con firmantes PDA usando Anchor y
Rust nativo. Cada ejemplo incluye una única instrucción para transferir SOL
desde un PDA a una cuenta destinataria, usando un CPI firmado por el PDA.

#### Anchor

Los siguientes ejemplos muestran dos enfoques para implementar CPIs en un
programa [Anchor](https://www.anchor-lang.com/docs). Los ejemplos son
funcionalmente equivalentes, pero demuestran diferentes niveles de abstracción.

- **Ejemplo 1**: Usa _rs`CpiContext`_ de Anchor y función auxiliar.
- **Ejemplo 2**: Usa la función _rs`system_instruction::transfer`_ del crate
  _rs`solana_program`_. (El ejemplo 1 es una abstracción de esta
  implementación.)
...
El siguiente ejemplo realiza un CPI con firmantes PDA desde un programa escrito
en Rust nativo. Incluye una única instrucción que transfiere SOL desde una
cuenta PDA a otra. El CPI está firmado por la cuenta PDA. (El archivo de prueba
utiliza [LiteSVM](https://github.com/LiteSVM/litesvm) para probar el programa.)
