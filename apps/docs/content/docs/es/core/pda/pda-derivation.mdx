...
El bump seed es un solo byte (u8) que se añade a las semillas opcionales durante
la derivación.
[`find_program_address`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/pubkey/src/lib.rs#L800)
busca desde 255 hacia abajo hasta 1, llamando a _rs`create_program_address`_ con
cada valor hasta que el resultado queda fuera de la curva Ed25519. El primer
...
La
[syscall onchain](https://github.com/anza-xyz/agave/blob/v3.1.8/syscalls/src/lib.rs#L798-L834)
...
Derivar un PDA solo calcula una dirección. No crea una cuenta onchain en esa
dirección. La cuenta debe crearse explícitamente a través de una instrucción
separada (típicamente _rs`create_account`_ mediante CPI).
...
### Iterando Todos los Bumps Posibles
...
<Callout type="warn">
  Esto es distinto del comportamiento de búsqueda canónico actual documentado
  para `sol_try_find_program_address`, que itera desde 255 hacia abajo hasta 1.
</Callout>
...
En este ejemplo, el bump 255 produce una dirección en curva y falla. El primer
bump válido es 254, lo que lo convierte en el bump canónico.
