...
Una transferencia de System Program añade lamports a **cualquier** cuenta. No
existe ninguna verificación a nivel de protocolo que garantice que el
destinatario pueda mover el SOL de vuelta. Los lamports solo pueden ser movidos
por el programa propietario de la cuenta, por lo que enviar SOL a un
[token mint](/docs/tokens/basics/create-mint), un programa o una
[PDA](/docs/core/pda) que no controlas **conlleva el riesgo de pérdida
permanente de fondos** — solo una autoridad especificada por el programa
propietario puede devolverlos. El SOL enviado a un
...
Las transferencias de **tokens** SPL tienen cierta protección incorporada: el
Token Program rechaza una transferencia cuyas cuentas no coincidan con el mint
esperado. Las transferencias nativas de **SOL** no cuentan con dicha protección,
...
El formato varía entre los distintos SDKs,
...
verifica el destinatario antes de firmar. Consulta la cuenta y envía SOL
únicamente a una billetera de System Program (o a una dirección
...
<Callout>
  Este fragmento verifica los destinatarios nativos de SOL. Para la
  clasificación completa que también gestiona envíos de tokens SPL (token
  accounts, ATAs, Token-2022), consulta [Verificar
  dirección](/docs/payments/send-payments/verify-address).
</Callout>

## Obtención de los detalles de la transacción

Tras el envío, recupera los detalles de la transacción utilizando la firma de la
transacción y el método RPC [getTransaction](/docs/rpc/http/gettransaction).

<Callout>
  También puedes usar el [Explorador de Solana](https://explorer.solana.com).
</Callout>
