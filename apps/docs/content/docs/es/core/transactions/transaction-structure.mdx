...
description:
  Anatomía de una transacción de Solana incluyendo firmas, formato de mensaje, encabezado, direcciones de cuenta, blockhash, instrucciones compiladas, codificación binaria y límite de tamaño.
...
<Callout type="info" title="Resumen">
  ...
  direcciones de cuenta, un blockhash reciente e instrucciones compiladas. Tamaño
  máximo serializado: 1,232 bytes (4,096 bytes para el próximo formato v1).
</Callout>
...
<Callout type="info" title="Las transacciones v1 elevan el límite a 4,096 bytes">
  Esta página describe el formato de transacción heredado, que el formato v0 también
  sigue, salvo por su prefijo de versión y las tablas de búsqueda de direcciones. El próximo
  [formato v1](/docs/core/transactions/versioned-transactions#v1-message-format)
  eleva el límite de tamaño a 4,096 bytes y utiliza un diseño de cable diferente.
</Callout>
...
<Callout type="info" title="Prefijos de mensajes heredados y versionados">
  En un mensaje de transacción heredado, el primer byte del mensaje es
  `num_required_signatures`, seguido de los otros dos bytes de `MessageHeader`. En
  un mensaje de transacción versionado, el primer byte es un prefijo de versión;
  el `MessageHeader` de tres bytes comienza inmediatamente después de ese prefijo. Consulte
  [transacciones versionadas](/docs/core/transactions/versioned-transactions) para
  ver los diseños completos de los mensajes v0 y v1.
</Callout>
...
<Callout>
  ...
  conteos del [`header`](#header) del mensaje para determinar los permisos de
...
![Diagrama que muestra el orden del arreglo de direcciones de cuentas](/assets/docs/core/transactions/compat_array_of_account_addresses.png)
...
Cada _rs`CompiledInstruction`_ referencia cuentas por índice en el array
...
Las transacciones se serializan utilizando un esquema de codificación compacta.
...
utiliza 1 byte para valores del 0 al 127 y de 2 a 3 bytes para valores mayores.
...
Para ejecutar esta instrucción, la billetera del remitente envía la transacción
con su firma y el mensaje que contiene la instrucción de transferencia de SOL.
...
Una transferencia del System Program añade lamports a **cualquier** cuenta. No
existe una verificación a nivel de protocolo de que el destinatario pueda
retirar el SOL. Los lamports solo pueden ser retirados por el programa
propietario de la cuenta, por lo que enviar SOL a un
[token mint](/docs/tokens/basics/create-mint), a un programa o a una
[PDA](/docs/core/pda) que no controlas **supone un riesgo de pérdida permanente
...
recuperado por el propietario de esa cuenta, nunca por el remitente.

Las transferencias de **tokens** SPL tienen cierta protección propia: el Token
Program rechaza una transferencia cuyas cuentas no coincidan con el mint
esperado. Las transferencias nativas de **SOL** no cuentan con esta protección,
por lo que el remitente debe verificar el destinatario antes de firmar. Consulta
[Verificar dirección](/docs/payments/send-payments/verify-address) para conocer
la lógica completa de clasificación.
...
El formato varía según el SDK,
pero ten en cuenta que cada instrucción contiene la misma información requerida.
...
Dado que una transferencia de SOL se puede realizar a cualquier cuenta, verifica
el destinatario antes de firmar. Obtén la cuenta y envía solo a una billetera de
System Program (o a una dirección
...
<Callout>
  ...
  Token-2022), consulta [Verificar
  Dirección](/docs/payments/send-payments/verify-address).
</Callout>

## Obtención de los detalles de la transacción

Tras el envío, recupera los detalles de la transacción usando la firma de la
...
instrucciones internas (CPI) como blobs codificados. Para resolverlos en
