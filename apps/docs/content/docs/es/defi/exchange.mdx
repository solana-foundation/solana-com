...
- `{"maxSupportedTransactionVersion": 0}`
  ...
  `getTransaction` para evitar interrupciones en la detección de depósitos. La versión de transacción más reciente es `0` y debe especificarse como el valor máximo de versión de transacción admitida.
...
Primero, obtén un blockhash reciente usando el endpoint [`getFees`](/docs/rpc/deprecated/getfees) o el comando de la CLI:
...
Puedes verificar si un blockhash en particular sigue siendo válido enviando una solicitud [`getFeeCalculatorForBlockhash`](/docs/rpc/deprecated/getfeecalculatorforblockhash) con el blockhash como parámetro. Si el valor de la respuesta es `null`, el blockhash ha expirado y la transacción de retiro que utiliza ese blockhash nunca debería completarse.
...
También conocido como "Token Extensions", el estándar incluye muchas funciones nuevas que
los creadores de tokens y los titulares de cuentas pueden habilitar opcionalmente. Entre estas funciones se incluyen
transferencias confidenciales, comisiones en transferencias, cierre de mints, metadatos, delegados permanentes,
propiedad inmutable y mucho más. Consulte la
[guía de extensiones](https://spl.solana.com/token-2022/extensions) para obtener más
información.
...
Los mints pueden configurarse para transferencias confidenciales, de modo que los montos de los tokens estén
cifrados, pero los propietarios de las cuentas sigan siendo públicos.

Los exchanges pueden configurar token accounts para enviar y recibir transferencias confidenciales,
a fin de ocultar los montos de los usuarios. No es obligatorio habilitar las transferencias
confidenciales en los token accounts, por lo que los exchanges pueden exigir a los usuarios que envíen tokens
de forma no confidencial.
