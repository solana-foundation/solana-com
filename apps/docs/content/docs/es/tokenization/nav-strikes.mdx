...
difficulty: advanced
title: "Strikes de NAV para Fondos del Mercado Monetario en Solana"
description:
...
Los strikes de NAV (Valor Liquidativo) son momentos fijos durante la jornada de negociación en los que se valoran las participaciones del fondo y se ejecutan las órdenes. Los fondos tradicionales suelen tener un único strike diario al cierre del mercado, pero la velocidad y los bajos costes de Solana permiten múltiples strikes intradía, ofreciendo a los inversores mayor flexibilidad.

## ¿Qué es el NAV?

Un fondo es un conjunto de activos que varios inversores poseen en común. El **NAV (Valor Liquidativo)** es simplemente el precio de una participación en el fondo. Piénsalo así:

```
NAV = (Total Value of Everything in the Fund - liabilities) ÷ Number of Shares
```

Por ejemplo, si un fondo tiene 10 millones de dólares en activos y 10 millones de participaciones en circulación, cada participación vale $1,00.

**¿Por qué importa el NAV?**

- **Compra de participaciones (suscripción)**: Pagas el NAV vigente. Si el NAV es $1,02 y
  inviertes $102, obtienes 100 participaciones.
- **Venta de participaciones (reembolso)**: Recibes el NAV vigente. Si reembolsas
  100 participaciones al NAV de $1,02, obtienes $102.

Los fondos del mercado monetario suelen mantener un NAV estable en torno a $1,00, con pequeñas fluctuaciones según los intereses generados.

## El Problema

Cuando inviertes en un fondo del mercado monetario, tradicionalmente ocurren dos cosas en momentos distintos:

1. Envías una orden → Esperas al cierre del mercado
2. Se calcula el NAV → Las participaciones se emiten/reembolsan en T+1 o T+2

Esto genera varios problemas:

- **Ejecución retrasada**: Las órdenes enviadas por la mañana esperan hasta las 4 PM
- **Retraso en la liquidación**: Tu dinero queda en suspenso durante 1 o 2 días
- **Flexibilidad limitada**: Solo una oportunidad al día para operar
- **Precios desactualizados**: El NAV puede quedar obsoleto entre la orden y la ejecución

## La Solución

Los **Strikes de NAV en Solana** permiten múltiples ventanas de liquidación diaria con ejecución atómica. Las órdenes se encolan y se ejecutan instantáneamente en cada momento de strike con el NAV vigente.
...
Esta guía muestra cómo implementar strikes de NAV en Solana utilizando las extensiones SPL Token 2022 para la emisión de participaciones de fondos conforme a la normativa y USDC estándar para la liquidación, todo ello sin escribir programas Rust personalizados.

<Callout type="info" title="Implementación de Referencia Educativa">

Puedes usar el
[código fuente](https://github.com/Woody4618/nav-strikes-reference) de esta
implementación para probar los strikes de NAV de forma local.

Esta guía proporciona una implementación de referencia únicamente para fines de exploración y educación. **NO uses este código directamente en producción** sin:

- Auditorías de seguridad exhaustivas
- Sistemas adecuados de gestión de claves
- Revisión del cumplimiento normativo
- Consulta legal
- Pruebas exhaustivas y modificaciones

</Callout>

## Descripción General de la Arquitectura

El sistema de Strikes de NAV se compone de los siguientes componentes principales:

1. **Token de Participación del Fondo**: SPL Token 2022 con metadatos que almacenan el NAV en cadena
2. **Moneda de Liquidación**: USDC estándar (token SPL existente)
3. **Administrador del Fondo**: Orquesta los strikes mediante autoridad delegada
4. **Sistema de Lista Blanca**: Controla qué direcciones pueden poseer participaciones del fondo

### Principios Clave de Diseño

- **Sin Programas Personalizados**: Utiliza únicamente Token Extensions Program y USDC estándar
- **Liquidación Atómica**: Una única transacción garantiza que la suscripción/reembolso se complete o falle de forma conjunta
- **Estado Congelado por Defecto**: Las participaciones requieren inclusión explícita en la lista blanca para el cumplimiento normativo
- **Autoridad Delegada**: El administrador del fondo opera sin tomar custodia
- **NAV en Cadena**: El precio actual se almacena en los metadatos del token y es verificable públicamente

### Etapa 1: Configuración del Fondo

1. Crear el token de participación del fondo con extensiones de metadatos
2. Inicializar el NAV en $1,00
3. Establecer el calendario diario de strikes
4. Incluir a los inversores institucionales en la lista blanca

### Etapa 2: Período Pre-Strike

1. Los inversores envían solicitudes de suscripción/reembolso
2. Las órdenes se encolan para el siguiente momento de strike
3. El fondo calcula el NAV preliminar
4. Se realizan verificaciones de riesgo

### Etapa 3: Ejecución del Strike de NAV
...
1. Generar confirmaciones de operaciones
2. Actualizar la composición del fondo
3. Prepararse para el siguiente strike
4. Conciliar con el custodio

### Patrón de Autoridad Delegada

Los inversores preautorizan al administrador del fondo para mover sus tokens; el administrador ejecuta en el momento del strike sin necesidad de las firmas de los inversores:
...
### Flujo de Suscripción (USDC → Participaciones del Fondo)
...
### Flujo de Reembolso (Participaciones del Fondo → USDC)
...
## ¿Por qué Solana para los Strikes de NAV?

La arquitectura de Solana ofrece ventajas significativas frente a la liquidación tradicional de fondos:

| Aspecto            | Tradicional (T+1) | Strikes de NAV en Solana          |
| ----------------- | ----------------- | --------------------------- |
| Lógica de Liquidación  | Agentes de transferencia   | Agrupación atómica de transacciones |
| Tiempo de Liquidación   | 1-2 días          | &lt;1 segundo                |
| Coste de Transacción  | $50-200           | &lt;$0,01                   |
| Strikes por Día   | 1                 | 4+ (configurable)           |
| Riesgo de precios desactualizados | Alto              | Menor                       |
| Transparencia del NAV  | Informe al cierre del día | En cadena, en tiempo real          |

Las transacciones atómicas de Solana eliminan intermediarios y ofrecen liquidaciones instantáneas, económicas y seguras con transparencia del NAV en cadena.

## Token del Fondo con Token Extensions

SPL Token 2022 ofrece potentes extensiones que permiten la emisión de participaciones de fondos conforme a la normativa sin necesidad de programas personalizados:

### Extensiones Esenciales para las Participaciones del Fondo

1. **Extensión de Estado de Cuenta por Defecto**: Establece que todos los nuevos token account queden congelados por defecto, requiriendo inclusión explícita en la lista blanca (cumplimiento KYC/AML)
2. **Extensión de Metadatos**: Almacena en cadena el NAV, el calendario de strikes y el AUM para mayor transparencia

### Configuración de Autoridades

- **Autoridad de Emisión**: Administrador del Fondo (controla la emisión de participaciones)
- **Autoridad de Congelación**: Administrador del Fondo (gestiona la lista blanca)
- **Autoridad de Actualización**: Administrador del Fondo (actualiza los metadatos del NAV)

<Callout type="info">

El estado congelado por defecto es fundamental para el cumplimiento normativo. Garantiza que solo las direcciones explícitamente incluidas en la lista blanca (verificadas mediante KYC) puedan recibir y poseer participaciones del fondo.

</Callout>

### Campos de Metadatos en Cadena
...
## Implementación Completa de Strikes de NAV

### Configuración del Motor de Strikes de NAV

Ten en cuenta que en esta guía usamos la librería @solana/kit para crear el Motor de Strikes de NAV. También puedes encontrar una implementación con `web3.js` en el
[código fuente](https://github.com/Woody4618/nav-strikes-reference). Primero, crea
la clase del motor principal que gestiona todas las operaciones del fondo:
...
<Callout type="tip" title="Actualizaciones Dinámicas de Metadatos">

Para las participaciones del fondo, los campos `currentNAV`, `lastStrikeTime` y `totalAUM` se actualizan en cadena en cada strike de NAV mediante
`getUpdateTokenMetadataFieldInstruction`. Esto proporciona:

- **Visibilidad del NAV en tiempo real** para todos los participantes
- **Registro de auditoría** de todas las actualizaciones del NAV en cadena
- **Integración** con protocolos DeFi que pueden leer los metadatos en cadena
...
A continuación se muestra un ejemplo completo que ilustra una jornada completa de operaciones de strike de NAV:
...
Antes de desplegar en producción, asegúrate de abordar lo siguiente:

1. **Seguridad**: Gestión profesional de claves, controles multi-firma y auditorías de seguridad exhaustivas del código
2. **Cumplimiento Normativo**: Registro de valores, sistemas KYC/AML, restricciones de transferencia e informes requeridos
3. **Gestión de Claves**: Soluciones de custodia institucional con procedimientos adecuados de respaldo y recuperación
4. **Procesamiento de Transacciones**: Tarifas de prioridad, lógica de reintentos, gestión de confirmaciones y redundancia de RPC
5. **Operaciones del Fondo**: Cálculo del NAV a partir de fuentes autorizadas, integración con el custodio y conciliación
6. **Monitorización**: Seguimiento en tiempo real, alertas e informes regulatorios automatizados

## Próximos Pasos

Tras comprender los strikes de NAV en Solana:

1. **Explorar Token Extensions**: Aprende sobre otras
   [extensiones de Token 2022](/docs/tokens/extensions/transfer-hook) como, por ejemplo,
   el delegado permanente para funciones adicionales de cumplimiento

2. **Implementar Token ACL**: Para un cumplimiento mejorado, considera utilizar
   [Token ACL](/docs/tokenization/token-acl) para gestionar la lista blanca de inversores
   con descongelación sin permisos. Esto permite la verificación KYC de autoservicio, donde
   los inversores pueden activar automáticamente sus cuentas tras superar las
   verificaciones de cumplimiento, reduciendo la carga operativa y manteniendo el control regulatorio.

3. **Estudiar la Liquidación DvP**: Consulta nuestra
   [guía de Entrega contra Pago](/docs/tokenization/dvp) para patrones de liquidación de valores

4. **Arquitectura de Producción**: Diseñar sistemas robustos de programación, monitorización y recuperación ante desastres

## Conclusión

El modelo de transacciones atómicas de Solana y las Token Extensions proporcionan una base sólida para implementar múltiples strikes de NAV diarios en fondos del mercado monetario.
La combinación de:

- Ejecución atómica nativa (sin riesgo de contrato inteligente)
- Finalidad en menos de un segundo
- Costes de transacción casi nulos
- Transparencia del NAV en cadena
- Funciones de cumplimiento integradas (valores predeterminados congelados, metadatos)

Hacen de Solana una plataforma ideal para modernizar la infraestructura de liquidación de fondos y ofrecer a los inversores oportunidades de negociación más frecuentes.

No obstante, pasar de esta referencia educativa a un entorno de producción requiere trabajo adicional considerable en materia de seguridad, cumplimiento normativo, custodia y operaciones. Trabaja siempre con expertos legales, regulatorios y técnicos cualificados cuando te adentrés en la tokenización de fondos.
