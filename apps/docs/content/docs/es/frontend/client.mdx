...
<Callout type="info">
  ...
  billetera de navegador? Firma con un servicio de gestión de claves de backend
  en lugar de un keypair sin procesar. Consulta [Keychain](/docs/tools/keychain)
  y [Firma en Producción](/docs/core/transactions/signing-in-production).
</Callout>
...
- **Dashboards en tiempo real**: Combina watchers (saldos, cuentas, firmas) con
  tu biblioteca de UI preferida; el cliente gestiona el fan-out de websockets y
  la invalidación de caché.
- **Stores personalizados**: Inyecta tu propio store de Zustand para hidratar
  desde IndexedDB/localStorage, replicar el estado en sesiones del servidor o
...
- **Testeabilidad**: La interfaz de cliente unificada puede ser simulada en
  pruebas unitarias, lo que facilita simular respuestas RPC o sesiones de
  billetera sin necesidad de una billetera de navegador.
