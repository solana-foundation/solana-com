...
El Programa de Metadatos de Tokens crea una Program Derived Address (PDA) para
cada creación de token. Esta cuenta de metadatos almacena información en cadena
como el nombre y símbolo del token, además de un URI que apunta a metadatos JSON
fuera de la cadena (imágenes, descripciones, etc.).
