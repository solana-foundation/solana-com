...
description:
  Crea un mint cuyos tokens pueden ser acuñados, quemados y cerrados, pero no
  pueden ser transferidos entre cuentas de tokens.
...
## ¿Qué Son los Tokens No Transferibles?

La extensión de mint _rs`NonTransferable`_ del Token Extensions Program hace que
cada token account para ese mint sea no transferible. Después de que los tokens
son acuñados, los titulares de tokens no pueden moverlos a otra token account
con _rs`Transfer`_ o _rs`TransferChecked`_.

Este patrón es útil para activos que deben permanecer asociados a una sola
billetera.

Los tokens no transferibles aún pueden ser:

- Acuñados por la autoridad del mint
- Quemados por el propietario de la token account o un delegado autorizado
- Las token accounts pueden cerrarse después de que el saldo llegue a cero

<Callout type="info" title="Extensiones de Token Account">
  Cuando se inicializa una token account para un mint con _rs`NonTransferable`_,
  la token account se inicializa con _rs`NonTransferableAccount`_ y
  _rs`ImmutableOwner`_. Cuando la token account se crea a través del [Associated
  Token
  Program](/docs/tokens/basics/create-token-account#how-to-create-an-associated-token-account),
  se calcula el tamaño de cuenta requerido y la token account se crea con el
  tamaño necesario y los lamports exentos de rent.
</Callout>

## Cómo Crear un Mint No Transferible

Para crear un mint no transferible:

1. Calcula el tamaño de la mint account y el rent necesario para el mint y la
   extensión _rs`NonTransferable`_.
2. Crea la mint account con _rs`CreateAccount`_, inicializa
   _rs`NonTransferable`_ e inicializa el mint con _rs`InitializeMint`_.
3. Crea token accounts para el mint. _rs`NonTransferableAccount`_ y
   _rs`ImmutableOwner`_ se habilitan automáticamente para las token accounts.
4. _rs`Transfer`_ y _rs`TransferChecked`_ fallan con
...
_rs`NonTransferable`_. Este es el tamaño utilizado en _rs`CreateAccount`_.
...
## !!steps Inicializar NonTransferable

Inicializa la extensión _rs`NonTransferable`_ en el mint.
...
<Callout type="info" title="Orden de Instrucciones">
  _rs`InitializeNonTransferableMint`_ debe ejecutarse antes de
  _rs`InitializeMint`_. _rs`CreateAccount`_, _rs`InitializeNonTransferableMint`_
  y _rs`InitializeMint`_ deben incluirse en la misma transacción.
</Callout>
...
| ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| _rs`NonTransferable`_                          | Extensión de mint que marca los tokens del mint como no transferibles.                                                                                                                                                                                           | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/non_transferable.rs#L8-L13)  |
| _rs`NonTransferableAccount`_                   | Extensión de token account agregada a las token accounts para mints no transferibles.                                                                                                                                                                            | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/non_transferable.rs#L15-L21) |
| _rs`ImmutableOwner`_                           | Extensión de token account que previene cambios de propiedad y es requerida para token accounts no transferibles.                                                                                                                                                | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/immutable_owner.rs#L8-L16)   |
| _rs`InitializeNonTransferableMint`_            | Instrucción que inicializa la extensión no transferible a nivel de mint antes de _rs`InitializeMint`_.                                                                                                                                                           | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L642-L653)              |
| _rs`process_initialize_non_transferable_mint`_ | Lógica del procesador que inicializa la extensión de mint _rs`NonTransferable`_ en un mint no inicializado.                                                                                                                                                      | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1646-L1655)                |
| _rs`get_required_init_account_extensions`_     | Se utiliza para agregar automáticamente extensiones de token account cuando se inicializan las token accounts basándose en las extensiones habilitadas en el mint. Para mints _rs`NonTransferable`_, agrega _rs`NonTransferableAccount`_ e _rs`ImmutableOwner`_. | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/mod.rs#L1304-L1307)          |

### Typescript

El siguiente ejemplo `Kit` utiliza las instrucciones generadas directamente. Se
incluyen ejemplos heredados que usan `@solana/web3.js` como referencia.
