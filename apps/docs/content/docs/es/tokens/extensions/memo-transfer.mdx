...
description:
  Requiere que las transferencias entrantes incluyan un memo habilitando la
  extensión de cuenta MemoTransfer en una cuenta de token del Token Extensions
  Program.
...
## ¿Qué es la Transferencia con Memo?

La extensión de cuenta _rs`MemoTransfer`_ del Token Extensions Program requiere
una instrucción de memo inmediatamente antes de cualquier transferencia entrante
a esa token account. Las transferencias salientes de esa token account no
requieren un memo.

La extensión se configura en la token account de destino, no en el mint. Una vez
habilitada, las transferencias sin memo fallarán con _rs`TokenError::NoMemo`_
hasta que el propietario de la token account deshabilite el requisito.

## Cómo Crear una Token Account con MemoTransfer Habilitado

Para crear una token account con _rs`MemoTransfer`_ habilitado:

1. Crear e inicializar un mint.
2. Calcular el tamaño de la token account y el rent necesario para una token
   account con _rs`MemoTransfer`_.
3. Crear la token account y habilitar _rs`MemoTransfer`_ en esa cuenta.
4. Las instrucciones _rs`Transfer`_ y _rs`TransferChecked`_ hacia esa token
   account deben incluir una instrucción de memo inmediatamente antes de la
...
Crear e inicializar el mint que la token account utilizará.
...
Calcular el tamaño de la token account para la token account base más la
extensión _rs`MemoTransfer`_. Este es el tamaño utilizado en
_rs`CreateAccount`_.
...
Calcular el rent utilizando el tamaño de la token account necesario para la
extensión _rs`MemoTransfer`_.
...
## !!steps Crear e inicializar la cuenta de token

Crea la cuenta de token con el espacio y lamports calculados, luego iníciala
para el mint.
...
## !!steps Habilitar MemoTransfer

Habilita _rs`MemoTransfer`_ en la cuenta de token.
...
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| _rs`MemoTransfer`_                              | Extensión de cuenta que almacena si las transferencias entrantes requieren un memo.                                 | [Origen](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/mod.rs#L17-L22)         |
| _rs`RequiredMemoTransfersInstruction::Enable`_  | Instrucción que habilita las transferencias con memo obligatorio en una cuenta de token.                            | [Origen](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L50-L71) |
| _rs`RequiredMemoTransfersInstruction::Disable`_ | Instrucción que deshabilita las transferencias con memo obligatorio en una cuenta de token.                         | [Origen](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/memo_transfer/instruction.rs#L74-L95) |
| _rs`process_toggle_required_memo_transfers`_    | Lógica del procesador que inicializa o reescribe la extensión _rs`MemoTransfer`_ después de verificar la propiedad. | [Origen](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/memo_transfer/processor.rs#L20-L48)     |

### Typescript

El ejemplo `Kit` a continuación utiliza las instrucciones generadas
directamente. Se incluyen ejemplos heredados usando `@solana/web3.js`,
`@solana/spl-token` e `@solana/spl-memo` como referencia.
