...
description:
  Crea un mint que almacena metadatos directamente en la cuenta del mint utilizando las extensiones MetadataPointer y TokenMetadata, luego actualiza, elimina y bloquea esos metadatos a través de la interfaz token-metadata.
...
El ejemplo de `Kit` que aparece a continuación utiliza las instrucciones generadas directamente. Se incluyen ejemplos heredados que usan `@solana/web3.js`, `@solana/spl-token` y `@solana/spl-token-metadata` como referencia. Los ejemplos heredados limitan `maxSupportedTransactionVersion` a `0` porque web3.js v1 no puede deserializar [transacciones v1](/docs/core/transactions/versioned-transactions#v1-message-format).
