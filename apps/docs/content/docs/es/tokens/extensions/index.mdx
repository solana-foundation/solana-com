...
Cada extensión agrega un estado específico que generalmente se inicializa durante la creación del mint o del token account. Al inicializar cualquiera de las dos cuentas, puedes habilitar extensiones específicas de forma simultánea para diferentes funcionalidades. La mayoría de las extensiones no se pueden agregar una vez que una cuenta ha sido inicializada. Este es un aspecto importante a tener en cuenta al diseñar tu token, ya que deberás planificar con anticipación qué características deseas que soporte. Las secciones a continuación describen cada extensión y cómo integrarla.
...
que sigue al tipo de datos de cuenta base. Debes deserializar adicionalmente el
`tlv_data` (que contiene el estado de la extensión) según los tipos de extensión
específicos habilitados para esa cuenta.
