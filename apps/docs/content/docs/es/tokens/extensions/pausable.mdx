---
title: Acuñación pausable
description:
  Crea un mint de Token-2022 con una autoridad de pausa, bloquea transferencias
  mientras el mint está pausado y reanuda la actividad más tarde.
...
## ¿Qué es un mint pausable?

La extensión de mint _rs`PausableConfig`_ del Token Extensions Program otorga a
una autoridad de pausa la capacidad de pausar y reanudar la actividad de tokens
para todo el mint.

Cuando un mint está pausado, el Token Extensions Program rechaza:

- Transferencias
- Acuñaciones
- Quemas

Cuando el mint se reanuda, esas instrucciones vuelven a funcionar con
normalidad.

## Cómo crear, pausar y reanudar un mint

Para crear, pausar y reanudar un mint:

1. Calcula el tamaño de la cuenta de mint y el rent necesario para el mint y la
   extensión _rs`PausableConfig`_.
2. Crea la cuenta de mint con _rs`CreateAccount`_, inicializa
   _rs`PausableConfig`_ e inicializa el mint con _rs`InitializeMint`_.
3. Acuña tokens y crea cuentas de tokens para el mint.
4. Utiliza _rs`Pause`_ para bloquear la actividad de tokens del mint.
5. Utiliza _rs`Resume`_ para permitir la actividad de tokens nuevamente.

<ScrollyCoding>

## !!steps Calcular el tamaño de la cuenta

Calcula el tamaño de la cuenta de mint para el mint base más la extensión
_rs`PausableConfig`_. Este es el tamaño utilizado en _rs`CreateAccount`_.
...
Inicializa la extensión _rs`PausableConfig`_ en la mint.
...
## !!steps Pausar la mint

Pausa la mint con _rs`Pause`_.
...
## !!steps Reanudar la mint

Reanuda la mint con _rs`Resume`_.
...
<Callout type="info" title="Orden de Instrucciones">
  _rs`PausableInstruction::Initialize`_ debe ir antes de _rs`InitializeMint`_.
  _rs`CreateAccount`_, _rs`PausableInstruction::Initialize`_, y
  _rs`InitializeMint`_ deben incluirse en la misma transacción.
</Callout>
...
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PausableConfig`_                  | Extensión de mint que almacena la autoridad de pausa y si la mint está actualmente pausada.                             | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/mod.rs#L17-L22)         |
| _rs`PausableInstruction::Initialize`_ | Instrucción que inicializa la configuración pausable antes de _rs`InitializeMint`_.                                     | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L20-L32) |
| _rs`PausableInstruction::Pause`_      | Instrucción que marca la mint como pausada.                                                                             | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L33-L44) |
| _rs`PausableInstruction::Resume`_     | Instrucción que marca la mint como no pausada.                                                                          | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/pausable/instruction.rs#L45-L56) |
| _rs`process_initialize`_              | Lógica del procesador que inicializa _rs`PausableConfig`_ en una mint no inicializada y almacena la autoridad de pausa. | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L22-L35)     |
| _rs`process_toggle_pause`_            | Lógica del procesador que verifica la autoridad de pausa antes de alternar el indicador de pausa de la mint.            | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/pausable/processor.rs#L39-L64)     |
