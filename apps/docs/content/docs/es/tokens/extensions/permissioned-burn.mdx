...
<Callout type="warn" title="Disponibilidad">
  La quema con permisos se incluye en Token-2022
  [`program@v11.0.0`](https://github.com/solana-program/token-2022/releases/tag/program%40v11.0.0).
  Está disponible en devnet hoy y está programada para habilitarse en mainnet en
  junio de 2026. El Token Extensions Program se despliega por separado en cada
  clúster, así que confirma que el despliegue en el clúster de destino incluye
  v11.0.0 o posterior.
</Callout>
...
Para crear un mint account con quema con permisos y quemar tokens:
...
5. Opcionalmente, desactiva la quema con permisos con _rs`SetAuthority`_ usando
   _rs`AuthorityType::PermissionedBurn`_ y una autoridad `None`, lo que reactiva
   las instrucciones de quema estándar. Los ejemplos de código completos que
   aparecen a continuación muestran este paso.
...
Crea el mint account con el espacio y los lamport calculados.
...
| _rs`PermissionedBurnInstruction::Burn`_        | Instrucción de quema que requiere firmas de la autoridad de quema y el propietario o delegado del token account.                     | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L50-L71) |
| _rs`PermissionedBurnInstruction::BurnChecked`_ | Instrucción de quema con verificación de decimales que utiliza las mismas cuentas que _rs`PermissionedBurnInstruction::Burn`_.       | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/extension/permissioned_burn/instruction.rs#L72-L79) |
| _rs`AuthorityType::PermissionedBurn`_          | Discriminador de autoridad usado con _rs`SetAuthority`_ para rotar o deshabilitar la autoridad de quema en un mint.                  | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/interface/src/instruction.rs#L1276-L1277)                         |
...
| _rs`process_burn`_                             | Lógica del procesador que rechaza quemas estándar cuando la autoridad de quema está establecida y verifica la firma de la autoridad. | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v11.0.0/program/src/processor.rs#L1114-L1179)                             |
...
directamente. El paquete heredado `@solana/spl-token` publicado aún no incluye
