...
description:
  Configura un mint del Token Extensions Program para que las cuentas de tokens
  recién creadas comiencen en un estado predeterminado elegido, como congelado.
...
La extensión de mint _rs`DefaultAccountState`_ del Token Extensions Program
establece el _rs`AccountState`_ inicial para cada nueva token account creada
para ese mint.

Esto se utiliza más comúnmente para hacer que las nuevas cuentas de tokens
comiencen congeladas. Las cuentas de tokens que comienzan congeladas no se
pueden usar hasta que la autoridad de congelación del mint las descongele.

## Cómo Crear un Mint con un Estado de Cuenta Predeterminado

Para crear un mint con un estado de cuenta predeterminado:

1. Calcula el tamaño de la cuenta del mint y el rent necesario para el mint y la
   extensión _rs`DefaultAccountState`_.
2. Crea la cuenta del mint con _rs`CreateAccount`_, inicializa
   _rs`DefaultAccountState`_ e inicializa el mint con _rs`InitializeMint`_.
3. Crea cuentas de tokens para el mint y observa que cada nueva token account
   comienza en el estado predeterminado actual.
4. Usa _rs`UpdateDefaultAccountState`_ para cambiar el estado en el que deberían
   comenzar las futuras cuentas de tokens.

<ScrollyCoding>

## !!steps Calcular el tamaño de la cuenta

Calcula el tamaño de la cuenta del mint para el mint base más la extensión
_rs`DefaultAccountState`_. Este es el tamaño utilizado en _rs`CreateAccount`_.
...
## !!steps Crear la cuenta del mint
...
## !!steps Inicializar DefaultAccountState

Inicializa la extensión _rs`DefaultAccountState`_ en el mint.
...
## !!steps Crear una token account

Crea una token account para el mint. Dado que el estado predeterminado actual es
_rs`Frozen`_, la nueva token account comienza congelada.
...
## !!steps Actualizar el estado predeterminado de cuenta

Usa _rs`UpdateDefaultAccountState`_ para cambiar el estado predeterminado y que
las futuras token accounts ya no comiencen congeladas.
...
<Callout type="info" title="Orden de Instrucciones">
  _rs`DefaultAccountStateInstruction::Initialize`_ debe ir antes de
  _rs`InitializeMint`_. _rs`CreateAccount`_,
  _rs`DefaultAccountStateInstruction::Initialize`_ y _rs`InitializeMint`_ deben
...
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`DefaultAccountState`_                        | Extensión del mint que almacena el estado que las nuevas token accounts deben heredar.                                          | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/mod.rs#L16-L21)         |
| _rs`DefaultAccountStateInstruction::Initialize`_ | Instrucción que inicializa el estado predeterminado de token account del mint antes de _rs`InitializeMint`_.                    | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L20-L36) |
| _rs`DefaultAccountStateInstruction::Update`_     | Instrucción que permite a la autoridad de congelación cambiar el estado predeterminado para futuras token accounts.             | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/default_account_state/instruction.rs#L37-L53) |
| _rs`process_initialize_default_account_state`_   | Lógica del procesador que escribe el estado predeterminado inicial en un mint no inicializado.                                  | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L29-L40)     |
| _rs`process_update_default_account_state`_       | Lógica del procesador que verifica la autoridad de congelación antes de cambiar el estado predeterminado almacenado en el mint. | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/default_account_state/processor.rs#L43-L74)     |

### Typescript

El ejemplo `Kit` a continuación usa las instrucciones generadas directamente. Se
incluyen ejemplos heredados usando `@solana/web3.js` e `@solana/spl-token` como
referencia.
