...
protege las cuentas de tokens de transferencias inesperadas a través de
invocaciones de programas cruzados (CPI). Cuando está habilitada, esta extensión
garantiza que los tokens solo puedan transferirse mediante llamadas directas a
la instrucción de transferencia del Token Extensions Program, bloqueando todos
los demás programas para que no transfieran tokens a través de CPI.
