...
title: CPI Guard
description:
  Habilita la extensión de cuenta CpiGuard para bloquear instrucciones de token
  invocadas a través de Cross Program Invocation.
...
La extensión de cuenta _rs`CpiGuard`_ del Token Extensions Program impide que
otro programa en cadena utilice un token account para ciertas instrucciones de
token a través de Cross Program Invocation (CPI).

Cuando CPI guard está habilitado, el Token Extensions Program rechaza estas
instrucciones cuando otro programa en cadena las invoca a través de CPI:

- _rs`Transfer`_, _rs`TransferChecked`_ y _rs`TransferCheckedWithFee`_ cuando el
  propietario del token account se utiliza como autoridad de la instrucción
- _rs`Burn`_ y _rs`BurnChecked`_ cuando el propietario del token account se
  utiliza como autoridad de la instrucción
- _rs`Approve`_ y _rs`ApproveChecked`_
- _rs`CloseAccount`_ cuando los lamport se enviarían a un destino distinto del
  propietario del token account
- _rs`SetAuthority`_ al agregar o reemplazar la autoridad de cierre
...
- _rs`Transfer`_, _rs`TransferChecked`_, _rs`TransferCheckedWithFee`_,
  _rs`Burn`_ y _rs`BurnChecked`_ a través de CPI cuando un delegado o delegado
  permanente firma en lugar del propietario del token account
- _rs`CloseAccount`_ a través de CPI cuando los lamport se envían al propietario
  del token account
- _rs`SetAuthority`_ para eliminar una autoridad de cierre existente
- _rs`Revoke`_

Cuando el propietario del token account se utiliza como autoridad de la
instrucción, las instrucciones del Token Extensions Program aún pueden
procesarse si la instrucción se agrega directamente a la transacción en lugar de
invocarse a través de CPI. Cambiar el propietario del token account con
_rs`SetAuthority`_ permanece bloqueado mientras CPI guard está habilitado,
incluso fuera de CPI. _rs`EnableCpiGuard`_ y _rs`DisableCpiGuard`_ tampoco
pueden invocarse a través de CPI.
...
2. Calcular el tamaño de la cuenta de tokens y el rent necesario para una cuenta
   de tokens con _rs`CpiGuard`_.
3. Crear e inicializar una cuenta de tokens para el mint.
4. Invocar _rs`EnableCpiGuard`_ en la cuenta de tokens.
5. Invocar _rs`DisableCpiGuard`_ en la cuenta de tokens para permitir esas
   instrucciones a través de CPI nuevamente.

<ScrollyCoding>

## !!steps Crear e inicializar el mint
...
## !!steps Calcular el tamaño de la cuenta de tokens

Calcula el tamaño de la cuenta de tokens para la cuenta de tokens base más la
extensión _rs`CpiGuard`_. Este es el tamaño utilizado en _rs`CreateAccount`_.
...
Calcula el rent utilizando el tamaño de cuenta de tokens necesario para la
extensión _rs`CpiGuard`_.
...
## !!steps Crear e inicializar la cuenta de tokens

Crea la cuenta de tokens con el espacio y lamports calculados, luego
inicialízala para el mint.
...
## !!steps Habilitar la protección CPI

Habilita _rs`CpiGuard`_ en la cuenta de tokens.
...
## !!steps Deshabilitar la protección CPI

Deshabilita _rs`CpiGuard`_ en la cuenta de tokens.
...
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`CpiGuard`_                     | Extensión de cuenta que almacena si la protección CPI está actualmente habilitada para la token account.                                                                                                                                     | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/mod.rs#L20-L25)         |
| _rs`CpiGuardInstruction::Enable`_  | Instrucción que habilita la protección CPI en una token account.                                                                                                                                                                             | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/instruction.rs#L20-L38) |
| _rs`CpiGuardInstruction::Disable`_ | Instrucción que deshabilita la protección CPI en una token account.                                                                                                                                                                          | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/cpi_guard/instruction.rs#L39-L53) |
| _rs`process_toggle_cpi_guard`_     | Lógica de procesamiento utilizada por _rs`Enable`_ y _rs`Disable`_ que valida la propiedad de la cuenta, rechaza cambios CPI a la protección en sí misma e inicializa _rs`CpiGuard`_ si aún no está presente antes de establecer `lock_cpi`. | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/extension/cpi_guard/processor.rs#L19-L52)     |

### Typescript

El ejemplo `Kit` a continuación utiliza las instrucciones generadas
directamente. Se incluyen ejemplos heredados usando `@solana/web3.js` y
