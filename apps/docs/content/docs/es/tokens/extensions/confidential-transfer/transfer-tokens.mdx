...
el emisor como el destinatario deben tener token accounts configurados con el
estado _rs`ConfidentialTransferAccount`_ y aprobados para usar Balances
Confidenciales. El token account del emisor también debe tener un balance
...
1. Crea
   ...
   **Prueba de igualdad
   ...
   Verifica que el nuevo texto cifrado del saldo disponible tras la
   transferencia coincida con su correspondiente
   [compromiso de Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme),
   garantizando que el nuevo saldo disponible de la cuenta de origen se calcule
   correctamente como `new_balance = current_balance - transfer_amount`.

   **Prueba de validez del texto cifrado
   ...
   correctamente para las tres partes (origen, destino y auditor), garantizando
   que el monto de transferencia esté correctamente cifrado bajo la clave
   ...
   **Prueba de rango
   ...
   en bits bajos/altos) sean no negativos y estén dentro de un rango
   especificado.
...
2. Para cada prueba:
   ...
   - Almacena los metadatos específicos de la prueba en una cuenta de "estado de
     contexto" de prueba para usarla en otras instrucciones.
...
4. Cierra las cuentas de estado de contexto de prueba para recuperar el SOL
   utilizado para crearlas.

El siguiente diagrama muestra los pasos involucrados en la transferencia de
tokens desde el token account del remitente al token account del destinatario.
...
- Generar una prueba de igualdad, una prueba de validez del texto cifrado y una
  prueba de rango en el lado del cliente
...
El siguiente ejemplo transfiere tokens de forma confidencial de una cuenta a
otra. Ambas cuentas ya deben estar configuradas para Balances Confidenciales, y
el emisor debe tener un balance confidencial disponible.

Balances Confidenciales depende del programa ZK ElGamal Proof, que está habilitado
...
Ejecuta el ejemplo en uno de ellos (el código usa devnet) con un pagador con
fondos, y reemplaza los marcadores de posición con tu mint y las cuentas del
