...
<Callout type="warn">
  El Programa ZK ElGamal está temporalmente deshabilitado en mainnet y devnet
...
tokens son privados. Las direcciones de las cuentas de tokens permanecen
públicas.
...
al programa Token Extension que te permite transferir tokens entre cuentas sin
...
1. Crear una cuenta de mint con la extensión de transferencia confidencial.
2. Crear cuentas de tokens con extensión de transferencia confidencial para el
   remitente y el destinatario.
3. Acuñar tokens a la cuenta del remitente.
...
5. **Aplicar** el saldo pendiente del remitente al **saldo disponible
   confidencial**.
6. **Transferir** confidencialmente tokens desde la cuenta de tokens del
   remitente a la cuenta de tokens del destinatario.
...
<Cards>
  <Card
    title="Crear cuenta de mint"
    href="/docs/tokens/extensions/confidential-transfer/create-mint"
  >
    Cómo crear una cuenta de mint con la extensión de Transferencia Confidencial
  </Card>
  <Card
    title="Crear cuenta de token"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Cómo configurar una cuenta de token con la extensión de Transferencia
    Confidencial
  </Card>
  <Card
  ...
  >
    Cómo depositar tokens al saldo pendiente confidencial
  </Card>
  <Card
  ...
  >
    Cómo transferir tokens confidencialmente entre cuentas de token
  </Card>
</Cards>
...
El ejemplo en Rust a continuación muestra los pasos para transferencias
...
el CLI de Solana para usar el validador local.
...
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Configura la cuenta de mint para transferencias confidenciales. Esta instrucción debe incluirse en la misma transacción que la instrucción _rs`TokenInstruction::InitializeMint`_. |
| _rs`UpdateMint`_                    | Actualiza la configuración de transferencia confidencial para un mint.                                                                                                             |
| _rs`ConfigureAccount`_              | Configura una cuenta de token para transferencias confidenciales.                                                                                                                  |
| _rs`ApproveAccount`_                | Aprueba una cuenta de token para transferencias confidenciales si el mint requiere aprobación para nuevas cuentas de token.                                                        |
| _rs`EmptyAccount`_                  | Vacía los saldos confidenciales pendientes y disponibles para permitir cerrar una cuenta de token.                                                                                 |
...
| _rs`Transfer`_                      | Transfiere tokens entre cuentas de token confidencialmente.                                                                                                                        |
| _rs`ApplyPendingBalance`_           | Convierte el saldo pendiente en saldo disponible después de depósitos o transferencias.                                                                                            |
| _rs`EnableConfidentialCredits`_     | Permite que una cuenta de token reciba transferencias confidenciales de tokens.                                                                                                    |
...
| _rs`EnableNonConfidentialCredits`_  | Permite que una cuenta de token reciba transferencias públicas de tokens.                                                                                                          |
...
| _rs`TransferWithFee`_               | Transfiere tokens entre cuentas de token confidencialmente con una comisión.                                                                                                       |
| _rs`ConfigureAccountWithRegistry`_  | Forma alternativa de configurar cuentas de token para transferencias confidenciales usando una cuenta _rs`ElGamalRegistry`_ en lugar de una prueba _rs`VerifyPubkeyValidity`_.     |
