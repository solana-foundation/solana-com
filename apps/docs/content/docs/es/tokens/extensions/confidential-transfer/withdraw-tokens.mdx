...
1. Crea
   ...
   **Prueba de igualdad
   ...
   Verifica que el texto cifrado del saldo disponible restante después del
   retiro coincida con su
   [compromiso de Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme)
   correspondiente, asegurando que el nuevo saldo disponible de la cuenta se
   ...
   **Prueba de rango
   ...
   Verifica que el saldo disponible restante después del retiro sea no negativo
   y esté dentro de un rango especificado.

2. Para cada prueba:
   - Invoca el ZK ElGamal proof program para verificar los datos de la prueba.
   - Almacena los metadatos específicos de la prueba en una cuenta de "estado de
     contexto" de la prueba para usar en otras instrucciones.
...
- Invocar el Zk ElGamal proof program para verificar las pruebas e inicializar
...
Para ejecutar el ejemplo, inicia un validador local con el programa de extensión
de tokens clonado desde mainnet usando el siguiente comando. Debes tener
...
<Callout type="info">
  ...
  extensión de tokens de mainnet para ejecutar el código de ejemplo.
</Callout>
