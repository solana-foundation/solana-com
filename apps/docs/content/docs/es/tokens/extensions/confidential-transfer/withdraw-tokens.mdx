...
1. Crea
   ...
   **Prueba de igualdad
   ...
   Verifica que el texto cifrado del saldo disponible restante tras el retiro
   coincida con su
   [compromiso de Pedersen](https://en.wikipedia.org/wiki/Commitment_scheme)
   correspondiente, garantizando que el nuevo saldo disponible de la cuenta se
   calcule correctamente como
   ...
   **Prueba de rango
   ...
   Verifica que el saldo disponible restante tras el retiro sea no negativo y
   esté dentro de un rango especificado.

2. Para cada prueba:
   - Invoca el programa de pruebas ZK ElGamal para verificar los datos de la
     prueba.
   - Almacena los metadatos específicos de la prueba en una cuenta de "estado de
     contexto" de la prueba para usarla en otras instrucciones.
...
4. Cierra las cuentas de prueba para recuperar el SOL utilizado para crearlas.

El siguiente diagrama muestra los pasos involucrados en el retiro de tokens del
saldo disponible confidencial al saldo público:
...
- Generar una prueba de igualdad y una prueba de rango en el lado del cliente
- Invocar el programa de pruebas ZK ElGamal para verificar las pruebas e
...
El ejemplo en Rust que aparece a continuación genera las pruebas con el crate
`spl-token-confidential-transfer-proof-generation`, verifica cada una en una
cuenta de estado de contexto a través del programa ZK ElGamal Proof, hace
referencia a ambas cuentas en la instrucción de retiro y las cierra
posteriormente. El ejemplo en TypeScript utiliza el helper
`getConfidentialWithdrawInstructionPlan` de
`@solana-program/token-2022/confidential`, que ensambla las mismas cuentas de
prueba, el retiro y el cierre como un plan de instrucciones de múltiples
transacciones.

## Código de ejemplo

El siguiente ejemplo retira tokens del saldo confidencial disponible
de vuelta al saldo público. La cuenta ya debe estar configurada para
Saldos Confidenciales y contener un saldo confidencial disponible.

Los Saldos Confidenciales dependen del programa ZK ElGamal Proof, que está habilitado
...
y reemplaza los marcadores de posición con tu mint y token account.
