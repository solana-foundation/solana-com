...
description:
  Aprende a crear un token mint con Confidential Balances habilitado.
---

## Cómo crear un mint con Confidential Balances

Confidential Balances permite transferencias privadas de tokens añadiendo estado adicional al
mint account. Esta sección explica cómo crear un token mint con la
extensión requerida habilitada.

El siguiente diagrama muestra los pasos necesarios para crear un mint con la
extensión Confidential Balances:
...
### Estado del Mint de Confidential Balances
...
- **authority**: La cuenta que tiene permiso para cambiar la configuración de Confidential Balances
  para el mint y aprobar nuevas cuentas confidenciales si la aprobación automática
  está deshabilitada.

- **auto_approve_new_accounts**: Cuando se establece en true, los usuarios pueden crear token
  accounts con Confidential Balances habilitado por defecto. Cuando es false, la
  authority debe aprobar cada nuevo token account antes de que pueda usarse para
...
- **auditor_elgamal_pubkey**: Un auditor opcional que puede descifrar los montos
  ...
  de cumplimiento normativo mientras se mantiene la privacidad frente al público
  en general.

### Instrucciones Requeridas

Crear un mint con Confidential Balances habilitado requiere tres instrucciones
en una sola transacción:

1. **Crear el Mint Account**: Invocar la instrucción _rs`CreateAccount`_ del
   System Program para crear el mint account.

2. **Inicializar Confidential Balances**: Invoca la instrucción
   ...
   del Token Extension Program para configurar el estado _rs`ConfidentialTransferMint`_ para el
   mint.
...
El siguiente código demuestra cómo crear un mint con Confidential Balances
habilitado.

Confidential Balances depende del programa ZK ElGamal Proof, que está habilitado
en mainnet y devnet. Un `solana-test-validator` estándar no lo habilita, pero un
validator local con bifurcación de mainnet como [Surfpool](https://surfpool.run) sí lo hace.
Ejecuta el ejemplo contra uno de ellos (el código usa devnet) con un pagador con fondos,
y reemplaza las direcciones de mint y cuenta de marcador de posición con las tuyas propias.
