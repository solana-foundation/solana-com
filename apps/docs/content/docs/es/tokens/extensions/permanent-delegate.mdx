...
description:
  Crea un mint del Token Extensions Program con un delegado permanente que puede
  transferir o quemar tokens de cualquier cuenta para ese mint.
...
La extensión de mint _rs`PermanentDelegate`_ del Token Extensions Program asigna
una autoridad de delegado al propio mint.

Un delegado permanente es una autoridad a nivel de mint que puede autorizar
transferencias y quemas para cualquier token account de ese mint. Actúa como un
delegado global para todas las cuentas de tokens del mint, y los propietarios de
las cuentas de tokens no pueden revocar el delegado permanente de sus cuentas de
tokens.

La autoridad del mint también puede rotar el delegado permanente más adelante
con _rs`SetAuthority`_ usando _rs`AuthorityType::PermanentDelegate`_.
...
1. Calcula el tamaño de la mint account y el rent necesario para el mint y la
   extensión _rs`PermanentDelegate`_.
2. Crea la mint account con _rs`CreateAccount`_, inicializa
   _rs`PermanentDelegate`_ e inicializa el mint con _rs`InitializeMint`_.
3. Envía _rs`TransferChecked`_ o _rs`BurnChecked`_ firmado por el delegado
   permanente. El Token Program procesa esas instrucciones de la misma manera
   que si el propietario de la token account las hubiera firmado.
...
_rs`PermanentDelegate`_. Este es el tamaño utilizado en _rs`CreateAccount`_.
...
Inicializa la extensión _rs`PermanentDelegate`_ en el mint.
...
## !!steps Crear token accounts y acuñar tokens

Crea token accounts para el propietario y el destinatario, luego acuña tokens en
la token account del propietario.
...
## !!steps Transferir con el delegado permanente

Transfiere tokens con _rs`TransferChecked`_ firmado por el delegado permanente.
...
<Callout type="info" title="Orden de Instrucciones">
  _rs`InitializePermanentDelegate`_ debe ir antes de _rs`InitializeMint`_.
  _rs`CreateAccount`_, _rs`InitializePermanentDelegate`_ y _rs`InitializeMint`_
...
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- |
| _rs`PermanentDelegate`_                     | Extensión de mint que almacena el delegado autorizado en cada token account del mint.                                    | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/extension/permanent_delegate.rs#L15-L21) |
| _rs`InitializePermanentDelegate`_           | Instrucción que inicializa el delegado permanente antes de _rs`InitializeMint`_.                                         | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1952-L1967)              |
| _rs`SetAuthority`_                          | Instrucción base de token utilizada con _rs`AuthorityType::PermanentDelegate`_ para rotar el delegado.                   | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1488-L1520)              |
| _rs`AuthorityType::PermanentDelegate`_      | Discriminador de autoridad utilizado con _rs`SetAuthority`_ para rotar el delegado permanente en un mint.                | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L1138-L1139)              |
| _rs`process_initialize_permanent_delegate`_ | Lógica del procesador que escribe la extensión de delegado permanente en el mint.                                        | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1542-L1557)                  |
| _rs`process_set_authority`_                 | Lógica del procesador que valida y rota el delegado permanente cuando se utiliza _rs`AuthorityType::PermanentDelegate`_. | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L849-L860)                    |
