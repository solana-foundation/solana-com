...
description:
  Cerrar una cuenta de token y devolver los lamports de la cuenta a una cuenta de destino.
...
## ¿Qué hace el cierre de una cuenta de token?

Cerrar un token account elimina la cuenta y transfiere todos sus lamports,
incluyendo su saldo mínimo de almacenamiento reembolsable, a una cuenta de destino.

El saldo de tokens debe ser cero antes de que la cuenta pueda cerrarse. Un token account congelado
aún puede cerrarse si su saldo de tokens es cero. Los token accounts de SOL envuelto
son la excepción y pueden cerrarse con un saldo de tokens para
recuperar el SOL subyacente. El propietario de la cuenta o la autoridad de cierre de la cuenta
firma la instrucción de cierre y selecciona el destino. La cuenta que pagó
para crear un associated token account no recibe automáticamente el saldo recuperado.

## Cómo cerrar una cuenta de token

Cerrar una cuenta de token utiliza la instrucción _rs`CloseAccount`_ del Token
Program.

La instrucción _rs`CloseAccount`_ transfiere los lamports de la cuenta de origen
a una cuenta de destino, limpia la cuenta de origen y elimina la cuenta de
origen. El Token Extensions Program aplica verificaciones de cierre adicionales
para ciertas extensiones, pero el flujo de cierre base es el mismo.

### Referencia de código fuente

| Elemento                        | Descripción                                                                                                     | Token Program                                                                                                  | Token Extensions Program                                                                                              |
| --------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_               | El estado de la cuenta de token almacena el saldo, el estado nativo y la autoridad de cierre utilizada durante el cierre de la cuenta.     | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`CloseAccount`_          | Una instrucción que cierra una cuenta de token y transfiere los lamports de la cuenta de token a una cuenta de destino. | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L197-L212) | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L240-L274) |
| _rs`process_close_account`_ | Lógica de procesador compartida para el cierre de cuentas de token.                                                               | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L671-L709)     | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1197-L1289)   |
