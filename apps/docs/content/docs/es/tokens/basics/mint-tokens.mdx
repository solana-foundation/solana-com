...
La acuñación crea nuevas unidades de un token usando la instrucción
...
async fn main() -> Result<()> {
    ...
    // Obtener el tamaño predeterminado de la mint account (en bytes), sin extensiones habilitadas
    ...
    // Instrucción para inicializar los datos de la mint account
    let initialize_mint_instruction = initialize_mint(
        &token_program_id(),
        &mint.pubkey(),            // mint
        &fee_payer.pubkey(),       // autoridad de acuñación
    ...
    let create_ata_instruction = create_associated_token_account(
        &fee_payer.pubkey(), // dirección de financiamiento
    ...
    // Cantidad de tokens a acuñar (100 tokens con 2 decimales)
    let amount = 100;

    // Crear instrucción mint_to para acuñar tokens a la associated token account
    let mint_to_instruction = mint_to(
        &token_program_id(),
        &mint.pubkey(),            // mint
        &associated_token_address, // destino
        &fee_payer.pubkey(),       // autoridad
        &[&fee_payer.pubkey()],    // firmante
        amount,                    // cantidad
    )?;

    // Crear transacción para acuñar tokens
    let transaction = Transaction::new_signed_with_payer(
        &[mint_to_instruction],
        Some(&fee_payer.pubkey()),
        &[&fee_payer],
    ...
    println!("Se acuñaron 1.00 tokens a la associated token account");
    println!("\nDirección del mint: {}", mint.pubkey());
    println!("{:#?}", mint_data);

    println!(
        "\nDirección de la associated token account: {}",
        associated_token_address
    );
    println!("{:#?}", token_data);

    println!("Firma de la transacción: {}", transaction_signature);
...
async def main():
    ...
    # Ejemplos de keypairs y direcciones
    payer = Keypair()
    ...
    # Cantidad a acuñar (en la unidad más pequeña)
    amount_to_mint = 1000000000  # 1 token con 9 decimales

    async with rpc:
        # Crear instrucción de acuñación
        ...
        # Obtener el blockhash más reciente
        ...
        print(f"Destino: {destination_token_account}")
        print(f"Cantidad: {amount_to_mint}")
        print(f"Autoridad de acuñación: {mint_authority.pubkey()}")
        print(f"Pagador: {payer.pubkey()}")
        print(f"Transacción de acuñación creada exitosamente")
