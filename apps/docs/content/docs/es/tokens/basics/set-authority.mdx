...
## Comprender las autoridades de tokens

### Autoridades de mint account

- **Autoridad de mint**: Controla la creación de nuevos tokens. Puede acuñar
  tokens a cualquier token account. A menudo se revoca después de crear el
  suministro inicial para crear un token de suministro fijo.
- **Autoridad de congelación**: Controla la capacidad de congelar y descongelar
  token accounts. Puede evitar que cualquier token account transfiera tokens. A
  menudo se revoca para garantizar a los usuarios que sus tokens no pueden ser
  congelados.

### Autoridades de token account

- **Propietario de cuenta**: Tiene control total sobre el token account. Puede
  transferir tokens, quemar tokens, aprobar delegados y cerrar la cuenta cuando
  el saldo es cero.
