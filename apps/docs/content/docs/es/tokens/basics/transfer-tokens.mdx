...
description:
  Mueva tokens entre cuentas de tokens para el mismo mint con instrucciones de
...
Transferir tokens mueve tokens de una token account a otra token account para el
mismo mint.

Las transferencias no cambian el suministro del mint. Las transferencias solo
actualizan los saldos entre token accounts.
...
invocador proporcione el mint y los decimales para que la instrucción pueda
verificar el mint esperado y la precisión del token antes de mover tokens entre
cuentas.

En el Token Extensions Program, _rs`Transfer`_ está en desuso a favor de
...
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_          | El estado de la token account almacena los saldos actualizados por una transferencia.                                                                                                                            | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | Una instrucción de transferencia que mueve tokens entre token accounts sin requerir que el invocador proporcione el mint o los decimales. En el Token Extensions Program, use _rs`TransferChecked`_ en su lugar. | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Una instrucción de transferencia que requiere que el invocador proporcione el mint y los decimales y verifica esos valores antes de mover tokens entre token accounts del mismo mint.                            | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | Lógica compartida del procesador para transferencias de tokens.                                                                                                                                                  | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |

### Typescript

Los ejemplos a continuación muestran el enfoque recomendado usando `Kit`. Los
ejemplos heredados que usan `@solana/kit` se incluyen como referencia.
