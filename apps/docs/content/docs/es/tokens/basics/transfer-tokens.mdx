...
description:
  Transfiere tokens entre cuentas de tokens del mismo mint con instrucciones de
  transferencia del Token Program.
...
## ¿Qué Hace la Transferencia de Tokens?

La transferencia de tokens mueve tokens de una token account a otra token
account del mismo mint.

Las transferencias no cambian el suministro del mint. Las transferencias solo
actualizan los saldos entre cuentas de tokens.

## Cómo Transferir Tokens

La transferencia de tokens utiliza la instrucción _rs`Transfer`_ o
_rs`TransferChecked`_ del Token Program.

Los ejemplos a continuación utilizan _rs`TransferChecked`_, que requiere que el
llamador proporcione el mint y los decimales para que la instrucción pueda
verificar el mint esperado y la precisión del token antes de mover tokens entre
cuentas.

En el Token Extensions Program, _rs`Transfer`_ está obsoleto en favor de
_rs`TransferChecked`_ o _rs`TransferCheckedWithFee`_.

El propietario de la cuenta de origen o un delegado aprobado firma la
transferencia. En el Token Extensions Program, un delegado permanente del mint
también puede autorizar la transferencia si el mint tiene habilitada la
extensión de delegado permanente.

### Referencia de Código Fuente

| Elemento               | Descripción                                                                                                                                                                                                            | Token Program                                                                                                         | Token Extensions Program                                                                                                    |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_          | El estado de la token account almacena los saldos actualizados por una transferencia.                                                                                                                                  | [Código fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Código fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Transfer`_         | Una instrucción de transferencia que mueve tokens entre cuentas de tokens sin requerir que el llamador proporcione el mint o los decimales. En el Token Extensions Program, utilice _rs`TransferChecked`_ en su lugar. | [Código fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L89-L108)  | [Código fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L121-L149) |
| _rs`TransferChecked`_  | Una instrucción de transferencia que requiere que el llamador proporcione el mint y los decimales, y verifica esos valores antes de mover tokens entre cuentas de tokens del mismo mint.                               | [Código fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L245-L273) | [Código fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L308-L339) |
| _rs`process_transfer`_ | Lógica de procesador compartida para transferencias de tokens.                                                                                                                                                         | [Código fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L227-L341)     | [Código fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L308-L579)     |
