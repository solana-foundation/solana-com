...
description:
  Congela una cuenta de tokens con la autoridad de congelación del mint para
  bloquear transferencias, quemas y cambios de delegado.
...
## ¿Qué Hace Congelar una Cuenta de Tokens?

Congelar una cuenta de tokens mantiene el mismo propietario, mint y saldo, pero
impide que la cuenta de tokens transfiera o queme tokens hasta que la cuenta de
tokens sea descongelada.

Una cuenta congelada aún puede cerrarse si la cuenta congelada tiene un saldo de
tokens de cero.

Solo un mint que todavía tiene una autoridad de congelación puede congelar
cuentas de tokens. Si se revoca la autoridad de congelación de un mint, las
cuentas para ese mint ya no pueden ser congeladas.

## Cómo Congelar una Cuenta de Tokens

Congelar una cuenta de tokens utiliza la instrucción _rs`FreezeAccount`_ del
Token Program.

La instrucción _rs`FreezeAccount`_ verifica que la cuenta de tokens pertenezca
al mint y que el firmante sea la autoridad de congelación del mint, luego cambia
el estado de la cuenta de tokens de inicializado a congelado.
...
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Mint`_                          | El estado del mint almacena la autoridad de congelación opcional utilizada para congelar cuentas de tokens. | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L13-L30)         | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L25-L42)         |
| _rs`AccountState`_                  | Los estados de la cuenta de tokens, incluidos _rs`Initialized`_ y _rs`Frozen`_.                             | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L185-L195)       | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L203-L213)       |
| _rs`FreezeAccount`_                 | Una instrucción que cambia una cuenta de tokens al estado congelado.                                        | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L213-L228) | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L276-L291) |
