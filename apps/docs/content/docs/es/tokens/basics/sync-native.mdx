...
El SOL envuelto (WSOL) es una cuenta de token que rastrea la cantidad de
lamports en la cuenta como el saldo del token. WSOL permite la integración con
protocolos DeFi para transferir SOL como un token SPL.
...
sincroniza el saldo de una cuenta de token SOL envuelto (WSOL) con el SOL real
(lamports) almacenado en ella. Cuando transfieres SOL nativo a una cuenta de
token WSOL, el saldo del token no se actualiza automáticamente. Debes llamar a
SyncNative para reflejar el saldo correcto de WSOL.
