...
title: Sync Native
description:
  Actualizar el saldo de una cuenta de token SOL envuelto después de transferir
  SOL a la cuenta.
...
SOL envuelto (WSOL) es SOL mantenido en una token account para el mint nativo
del Token Program. El mint nativo es la dirección de mint que el Token Program
utiliza para las cuentas de token que envuelven SOL. El mint nativo permite que
SOL se utilice a través de las instrucciones del Token Program.

| Programa                 | Dirección del mint nativo                      |
| ------------------------ | ---------------------------------------------- |
...
Una token account de SOL envuelto almacena SOL en el campo `lamports` de la
cuenta, pero rastrea el SOL mantenido en la token account de SOL envuelto como
un saldo de token a través del campo _rs`amount`_ de la cuenta de token. Si
transfieres SOL a una token account de SOL envuelto, el saldo de lamports
aumenta inmediatamente, pero el campo _rs`amount`_ de token no cambia hasta que
llames a la instrucción _rs`SyncNative`_ del Token Program.

## Cómo sincronizar SOL nativo

Utiliza la instrucción _rs`SyncNative`_ del Token Program después de transferir
SOL a una token account de SOL envuelto. La instrucción _rs`SyncNative`_
actualiza el campo _rs`amount`_ de la cuenta de token para que coincida con los
lamports mantenidos en la cuenta por encima de su reserva exenta de renta.

### Referencia de código fuente

| Elemento                  | Descripción                                                                                                                                                                                                                       | Token Program                                                                                                  | Token Extensions Program                                                                                             |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_             | El estado de la cuenta de token utiliza _rs`is_native`_ para marcar la cuenta como WSOL y almacena la reserva nativa, el saldo mínimo de lamports exento de renta que la cuenta debe mantener, y la cantidad de tokens rastreada. | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`SyncNative`_          | Una instrucción que actualiza el campo _rs`amount`_ de una token account de SOL envuelto para que coincida con los lamports mantenidos en la cuenta por encima de su reserva exenta de renta.                                     | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L366-L376) | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L434-L444) |
| _rs`process_sync_native`_ | Lógica de procesamiento compartida para sincronizar saldos de SOL envuelto.                                                                                                                                                       | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L757-L779)     | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L1344-L1371)   |

### Typescript

Los ejemplos a continuación muestran el enfoque recomendado utilizando `Kit`.
Los ejemplos heredados que utilizan `@solana/kit` se incluyen como referencia.
