...
description:
  Eliminar el delegado actual de una cuenta de tokens y restablecer su cantidad
  delegada restante.
...
## ¿Qué Hace la Revocación de un Delegado?

Revocar un delegado elimina el delegado actual de la cuenta de tokens y
restablece la cantidad delegada a cero.

Después de la revocación, el delegado anterior ya no tiene autoridad para
transferir o quemar tokens en nombre de la cuenta de tokens.

## Cómo Revocar un Delegado

Revocar un delegado utiliza la instrucción _rs`Revoke`_ del Token Program.

La instrucción _rs`Revoke`_ borra los campos del delegado en la cuenta de tokens
de origen. En el Token Program, el propietario de la cuenta de tokens firma la
revocación. En el Token Extensions Program, el delegado actual también puede
...
| -------------------- | ----------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_        | El estado de la cuenta de tokens almacena el delegado y la cantidad delegada. | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`Revoke`_         | Una instrucción que elimina el delegado actual de una cuenta de tokens.       | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L128-L140) | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L170-L182) |
| _rs`process_revoke`_ | Lógica compartida del procesador para la revocación de delegados.             | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L395-L420)     | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L643-L674)     |

### Typescript

Los ejemplos de `Kit` a continuación muestran el enfoque recomendado usando
`@solana/kit`. Los ejemplos antiguos que usan `@solana/web3.js` se incluyen como
referencia.
