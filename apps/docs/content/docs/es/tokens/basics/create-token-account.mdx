...
está asociada exactamente con un mint y rastrea tu saldo de tokens y detalles
adicionales.
...
token account rastrea:

1. **Mint**: El tipo de token específico que contiene
...
- Circle controla el token a través de la autoridad owner de la token account:
...
1. **Token Account Owner**: La autoridad que puede transferir, quemar o delegar
   tokens. Esto se almacena en los datos de la token account.

2. **Program Owner**: El programa que posee la cuenta (siempre el Token Program
   o Token Extension Program para token accounts).

Al trabajar con token accounts, "owner" típicamente se refiere a la autoridad
que puede transferir tokens, no al programa que posee la cuenta.
...
una billetera contenga un token específico. Utiliza una dirección determinista
(PDA) creada por el
...
Las ATA utilizan direcciones deterministas, lo que facilita encontrar la token
account de cualquier billetera para un mint determinado. Consulta la
[implementación de la derivación aquí](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/interface/src/address.rs#L56).
...
Para cualquier combinación de billetera y token, existe exactamente una
dirección ATA. Esto elimina la necesidad de rastrear manualmente las direcciones
de token accounts: siempre puedes derivar la dirección correcta.

<Callout type="info">
  **Cómo funciona el Associated Token Program**

El Associated Token Program es un programa auxiliar que:

- Crea token accounts en direcciones deterministas (PDA)
- Realiza invocaciones entre programas (CPI) al Token Program
...
Crear una token account requiere la instrucción
...
Crear una associated token account (ATA) requiere la instrucción
...
su inicialización mediante invocaciones entre programas (CPI).
