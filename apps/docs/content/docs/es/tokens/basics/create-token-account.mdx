---
title: Crear una Cuenta de Tokens
description:
  Cree cuentas de tokens y cuentas de tokens asociadas para un mint específico y propietario.
...
Una token account almacena tokens para un mint y un propietario de cuenta de
tokens en Solana.
...
Aquí, _rs`owner`_ significa la autoridad que puede transferir, quemar o delegar
...
identificado por el campo _rs`mint`_ de la token account.

## Qué es una Cuenta de Tokens Asociada

Una associated token account (ATA) es la token account predeterminada para una
billetera y un mint. El
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program/src)
deriva la dirección de ATA a partir de la dirección de la billetera, la
dirección del programa de tokens y la dirección del mint.

Solo las cuentas de tokens creadas por el Associated Token Program se denominan
associated token accounts.

El Associated Token Program es una forma de crear una token account en una
dirección estándar y determinista. La cuenta resultante sigue siendo una token
account propiedad del Token Program o Token Extensions Program, no del
Associated Token Program.

El Associated Token Program deriva la dirección de ATA como se muestra en
...
_rs`Account`_ definido por el Token Program o Token Extensions Program.

## Costo y Pagador

Crear una ATA requiere un saldo mínimo reembolsable para el almacenamiento en cadena de la cuenta, más la comisión de transacción. El `payer` pasado a la instrucción de creación financia el saldo de almacenamiento y debe firmar. El pagador puede ser diferente tanto del pagador de la comisión de transacción como de la billetera que será propietaria de la ATA, por lo que una aplicación puede crear una ATA en nombre de un destinatario.

A la tasa actual de almacenamiento del clúster, los costos base son:

| Token program                             | Tamaño de la ATA  | Saldo mínimo de almacenamiento             |
| ----------------------------------------- | --------- | ----------------------------------- |
| Original Token Program                    | 165 bytes | 2,039,280 lamports (0.00203928 SOL) |
| Token-2022 con propietario inmutable automático | 170 bytes | 2,074,080 lamports (0.00207408 SOL) |

Los mints de Token-2022 pueden requerir extensiones de cuenta además de la extensión automática _rs`ImmutableOwner`_. Esas ATAs son más grandes y requieren un saldo mínimo mayor. El Associated Token Program le solicita al Token Program seleccionado el tamaño de cuenta requerido y financia la ATA en consecuencia.

No codifiques estos saldos de forma fija en la lógica de la aplicación. Consulta
[`getMinimumBalanceForRentExemption`](/docs/rpc/http/getminimumbalanceforrentexemption)
para el tamaño de datos requerido, o deja que el Associated Token Program lo calcule y financie durante la creación.

<Callout type="info" title="El saldo de almacenamiento es reembolsable">
  Cerrar un token account vacío transfiere todos sus lamports, incluido el
  saldo mínimo de almacenamiento, al destino especificado por la instrucción de cierre. El pagador original de la creación no es automáticamente el destino del reembolso. Consulta [Close Token Account](/docs/tokens/basics/close-account).
</Callout>

## Cómo Crear una Cuenta de Token Asociada

La creación de una cuenta de token asociada utiliza la instrucción _rs`Create`_
o _rs`CreateIdempotent`_ del Programa de Token Asociado. El Programa de Token
...
<Callout type="info" title="Recomendado">
  Para la mayoría de las aplicaciones, cree cuentas de token a través del
  Programa de Token Asociado en lugar de crearlas llamando directamente a las
  instrucciones del Programa del Sistema y del Programa de Token. Una cuenta de
  token asociada utiliza una dirección determinista derivada del propietario, el
  programa de token y el mint, lo que facilita que las billeteras y aplicaciones
  encuentren la cuenta de token predeterminada para un mint determinado.
</Callout>
...
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| _rs`Create`_                                  | Crea una cuenta de token asociada en la dirección ATA derivada.                                                                                                                                                                                                | [Fuente](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L20-L29) |
| _rs`CreateIdempotent`_                        | Crea la cuenta de token asociada, pero aún así tiene éxito si esa ATA ya existe para el mismo propietario y mint.                                                                                                                                               | [Fuente](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | Deriva la dirección ATA y utiliza _rs`create_pda_account`_ más CPIs al programa de token seleccionado para inicializar la cuenta de token. Cuando el programa seleccionado es el Token Extension Program, el procesador ATA también inicializa la extensión de propietario inmutable. | [Fuente](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | Función auxiliar utilizada por _rs`process_create_associated_token_account`_ para crear la cuenta PDA mediante CPI al Programa del Sistema.                                                                                                                                         | [Fuente](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |
...
1. La instrucción _rs`CreateAccount`_ del System Program crea una nueva cuenta
   exenta de rent y asigna el Token Program como propietario del programa de la
   nueva cuenta.
2. La instrucción _rs`InitializeAccount`_, _rs`InitializeAccount2`_ o
   _rs`InitializeAccount3`_ del Token Program inicializa la nueva cuenta para
   una casa de acuñación y propietario.

Incluye la instrucción _rs`CreateAccount`_ y la instrucción de inicialización de
la cuenta de token en la misma transacción.

Durante la inicialización de la cuenta de token, el Token Program verifica que
la cuenta no esté ya inicializada y esté exenta de rent.

<Callout type="info">
  La sección siguiente muestra cómo crear una cuenta de token invocando
  directamente las instrucciones del System Program y del Token Program. Para la
  mayoría de las aplicaciones, utiliza el Associated Token Program en su lugar.
  Utiliza las llamadas directas al System Program y Token Program cuando tengas
  una razón específica para no usar una associated token account o cuando
  necesites crear cuentas de token PDA personalizadas realizando CPIs a las
  instrucciones del System Program y Token Program desde tu propio programa
  Solana.
</Callout>
...
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`Account`_                     | Los campos base de la cuenta de token almacenados en cada cuenta de token.                                                                       | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/state.rs#L84-L108)        | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/state.rs#L99-L123)        |
| _rs`InitializeAccount`_           | Una instrucción de inicialización de cuenta de token que espera el propietario y la cuenta sysvar de rent en su lista de cuentas.                    | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L45-L63)   | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L75-L93)   |
| _rs`InitializeAccount2`_          | Una instrucción de inicialización de cuenta de token que pasa el propietario en instruction data en lugar de en la lista de cuentas.                 | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L352-L365) | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L419-L433) |
| _rs`InitializeAccount3`_          | Una instrucción de inicialización de cuenta de token que pasa el propietario en instruction data y no requiere la cuenta sysvar de rent. | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/interface/src/instruction.rs#L377-L387) | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/interface/src/instruction.rs#L445-L456) |
| _rs`_process_initialize_account`_ | Lógica del procesador compartida para la inicialización de cuentas de token.                                                                           | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L83-L141)      | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L153-L234)     |
| _rs`process_initialize_account`_  | Manejador público para _rs`InitializeAccount`_.                                                                                        | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L143-L150)     | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L236-L240)     |
| _rs`process_initialize_account2`_ | Manejador público para _rs`InitializeAccount2`_.                                                                                       | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L152-L160)     | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L242-L246)     |
| _rs`process_initialize_account3`_ | Manejador público para _rs`InitializeAccount3`_.                                                                                       | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L162-L170)     | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L248-L252)     |
