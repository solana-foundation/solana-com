...
Una token account almacena tokens para un único mint y un único propietario de
token account en Solana.
...
Aquí, _rs`owner`_ se refiere a la autoridad que puede transferir, quemar o
delegar tokens desde la token account. El propietario del programa de la cuenta
sigue siendo el Token Program o el Token Extensions Program.
...
account propiedad del Token Program o del Token Extensions Program, no del
...
account estándar en esa dirección, y la cuenta resultante aún utiliza el tipo
_rs`Account`_ definido por el Token Program o el Token Extensions Program.
...
la cuenta como una cuenta de token propiedad del Token Program o Token Extension
Program.
...
<Callout type="info" title="Recomendado">
  ...
  instrucciones del System Program y Token Program. Una cuenta de token asociada
  utiliza una dirección determinista derivada del propietario, el programa de
  token y el mint, lo que hace que la cuenta de token predeterminada para un
  mint determinado sea más fácil de encontrar para billeteras y aplicaciones.
</Callout>
...
| _rs`CreateIdempotent`_                        | Crea la cuenta de token asociada, pero aún tiene éxito si esa ATA ya existe para el mismo propietario y mint.                                                                                                                                                                               | [Fuente](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | Deriva la dirección ATA y utiliza _rs`create_pda_account`_ además de CPIs al programa de token seleccionado para inicializar la cuenta de token. Cuando el programa seleccionado es el Token Extension Program, el procesador ATA también inicializa la extensión de propietario inmutable. | [Fuente](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | Auxiliar utilizado por _rs`process_create_associated_token_account`_ para crear la cuenta PDA mediante CPI en el System Program.                                                                                                                                                            | [Fuente](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |

### Typescript

Los ejemplos `Kit` a continuación muestran el enfoque recomendado utilizando
`@solana/kit`. Los ejemplos heredados que utilizan `@solana/web3.js` se incluyen
como referencia.
...
<Callout type="info">
  ...
  directamente a las instrucciones del System Program y del Token Program. Para
  la mayoría de las aplicaciones, utiliza el Associated Token Program en su
  lugar. Usa las llamadas directas al System Program y Token Program cuando
  tengas una razón específica para no usar una associated token account o cuando
  necesites crear cuentas de token PDA personalizadas haciendo CPIs a las
  instrucciones del System Program y Token Program desde tu propio programa de
  Solana.
</Callout>
...
| _rs`_process_initialize_account`_ | Lógica de procesador compartida para la inicialización de cuentas de token.                                                              | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L83-L141)      | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L153-L234)     |
| _rs`process_initialize_account`_  | Controlador público para _rs`InitializeAccount`_.                                                                                        | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L143-L150)     | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L236-L240)     |
| _rs`process_initialize_account2`_ | Controlador público para _rs`InitializeAccount2`_.                                                                                       | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L152-L160)     | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L242-L246)     |
| _rs`process_initialize_account3`_ | Controlador público para _rs`InitializeAccount3`_.                                                                                       | [Fuente](https://github.com/solana-program/token/blob/program%40v9.0.0/program/src/processor.rs#L162-L170)     | [Fuente](https://github.com/solana-program/token-2022/blob/program%40v10.0.0/program/src/processor.rs#L248-L252)     |

### Typescript

Los ejemplos a continuación muestran el enfoque recomendado usando
