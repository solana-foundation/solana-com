...
description:
  Crea cuentas de tokens y cuentas de tokens asociadas para un mint y
...
Una cuenta de tokens almacena tokens para un mint y un propietario de cuenta de
tokens en Solana.
...
tokens desde la cuenta de tokens. El propietario del programa de la cuenta sigue
siendo el Token Program o Token Extensions Program.

Cada cuenta de tokens está vinculada a exactamente un mint, lo que significa que
la cuenta de tokens puede contener unidades de un solo token, el token
identificado por el campo _rs`mint`_ de la cuenta de tokens.

## Qué es una Cuenta de Tokens Asociada

Una cuenta de tokens asociada (ATA) es la cuenta de tokens predeterminada para
una billetera y un mint. El
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program/src)
deriva la dirección de la ATA a partir de la dirección de la billetera, la
...
El Associated Token Program es una forma de crear una cuenta de tokens en una
dirección estándar y determinista. La cuenta resultante sigue siendo una cuenta
de tokens propiedad del Token Program o Token Extensions Program, no del
Associated Token Program.

El Associated Token Program deriva la dirección de la ATA como se muestra en
...
exactamente una dirección de ATA. El Associated Token Program crea una cuenta de
tokens estándar en esa dirección, y la cuenta resultante sigue utilizando el
tipo _rs`Account`_ definido por el Token Program o Token Extensions Program.

## Cómo crear una cuenta de tokens asociada

La creación de una cuenta de tokens asociada utiliza la instrucción _rs`Create`_
o _rs`CreateIdempotent`_ del Associated Token Program. El Associated Token
Program deriva la dirección ATA, crea la cuenta en la dirección ATA e inicializa
la cuenta como una cuenta de tokens propiedad del Token Program o Token
Extension Program.

<Callout type="info" title="Recomendado">
  Para la mayoría de las aplicaciones, cree cuentas de tokens a través del
  Associated Token Program en lugar de crearlas llamando directamente a las
  instrucciones del System Program y Token Program. Una associated token account
  utiliza una dirección determinística derivada del propietario, el programa de
  tokens y el mint, lo que facilita a las billeteras y aplicaciones encontrar la
  cuenta de tokens predeterminada para un mint determinado.
</Callout>
...
| --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| _rs`Create`_                                  | Crea una associated token account en la dirección ATA derivada.                                                                                                                                                                                                                               | [Fuente](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L20-L29) |
| _rs`CreateIdempotent`_                        | Crea la associated token account, pero aún tiene éxito si esa ATA ya existe para el mismo propietario y mint.                                                                                                                                                                                 | [Fuente](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/interface/src/instruction.rs#L30-L40) |
| _rs`process_create_associated_token_account`_ | Deriva la dirección ATA y utiliza _rs`create_pda_account`_ además de CPIs al programa de tokens seleccionado para inicializar la cuenta de tokens. Cuando el programa seleccionado es el Token Extension Program, el procesador ATA también inicializa la extensión de propietario inmutable. | [Fuente](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/processor.rs#L64-L161)    |
| _rs`create_pda_account`_                      | Función auxiliar utilizada por _rs`process_create_associated_token_account`_ para crear la cuenta PDA mediante CPI al System Program.                                                                                                                                                         | [Fuente](https://github.com/solana-program/associated-token-account/blob/program%40v8.0.0/program/src/tools/account.rs#L14-L70) |

### Typescript

Los ejemplos de `Kit` a continuación muestran el enfoque recomendado usando
`@solana/kit`. Se incluyen ejemplos heredados usando `@solana/web3.js` como
referencia.
...
2. La instrucción _rs`InitializeAccount`_, _rs`InitializeAccount2`_, o
   _rs`InitializeAccount3`_ del Token Program inicializa la nueva cuenta para un
   mint y un propietario.
...
la cuenta no esté ya inicializada y sea exenta de rent.

<Callout type="info">
  La sección a continuación muestra cómo crear una cuenta de token llamando
  directamente a las instrucciones del System Program y Token Program. Para la
  mayoría de las aplicaciones, utiliza el Associated Token Program en su lugar.
  Usa las llamadas directas al System Program y Token Program cuando tengas una
  razón específica para no usar una associated token account o cuando necesites
  crear cuentas de token PDA personalizadas realizando CPIs a las instrucciones
  del System Program y Token Program desde tu propio programa de Solana.
</Callout>
...
Los ejemplos a continuación muestran el enfoque recomendado usando `Kit`. Los
ejemplos heredados usando `@solana/kit` se incluyen como referencia.
