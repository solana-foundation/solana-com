...
identifica quién tiene autoridad sobre el token account. Esto difiere del
propietario del programa especificado en el tipo base
[Account](/docs/core/accounts#account-type), que es el Token Program para todos
los token accounts.
...
Crear tokens y cuentas requiere SOL para depósitos de rent de cuenta y tarifas
...
El comando _sh`create-account`_ crea un associated token account con la
...
Nota: `<TOKEN_ADDRESS>` especifica el mint account para el token account.
...
de acuñación debe firmar la transacción. La instrucción acuña tokens a un token
account y aumenta el suministro total en el mint account. Aquí hay un ejemplo en
Javascript en
...
tokens. El propietario del token account del remitente debe firmar la
