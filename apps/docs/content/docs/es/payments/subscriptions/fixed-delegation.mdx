...
- Los montos están en unidades base. Para un token de 6 decimales, `1_000_000`
  equivale a `1` token.
- Ejecuta `initSubscriptionAuthority` solo cuando la cuenta de Subscription
  Authority no exista todavía.
- `expiryTs` es un límite absoluto: una vez que pasa, las transferencias fallan
  y el patrocinador puede recuperar el rent. No hay período de gracia para el
  tiempo de gasto.
