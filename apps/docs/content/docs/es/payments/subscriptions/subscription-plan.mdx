...
Un patrocinador puede financiar el rent del plan pasando el parámetro opcional `payer` mientras el comerciante sigue siendo el propietario del plan. Eliminar el plan reembolsa al propietario, no al pagador, por lo que se debe gestionar el patrocinio fuera de la cadena.
...
- La instrucción contiene el estado del plan observado en el momento de la firma
  (`expectedCreatedAt`, `expectedEndTs`, `expectedPullers`,
  `expectedMetadataUri`). Si el plan activo ya no coincide, la actualización es
  rechazada (`StalePlanApproval`), por lo que una actualización firmada obsoleta no puede restaurar
  pullers eliminados ni revertir ediciones posteriores. El método `updatePlan`
  del cliente del plugin obtiene el estado activo por ti; si construyes manualmente, obtén el plan y
  pasa los campos tú mismo.
...
Para agrupar la inicialización de la autoridad y la suscripción en una sola transacción, pasa
el centinela `UNKNOWN_INIT_ID` (exportado por el SDK de TypeScript) como
`expectedSubscriptionAuthorityInitId`. El programa acepta la autoridad únicamente si
fue creada en el slot actual, por lo que el centinela funciona para nuevos registros; un
usuario que regresa cuya autoridad fue creada en un slot anterior debe pasar el
`initId` real (el cliente del plugin lo obtiene por ti) o la llamada fallará con
...
Cuando tanto el suscriptor como el propietario actual del plan firman,
`cancelSubscriptionNow` vence la suscripción en el momento de la cancelación en lugar de
al final del período de facturación. También puede acortar una cancelación pendiente en período de gracia. La aprobación está vinculada al inicio del período observado en el momento de la firma; si
la suscripción ha cambiado desde entonces, la transacción es rechazada
...
La instrucción contiene la fecha de vencimiento observada por el suscriptor en el momento de la firma
(`expectedExpiresAtTs`); una discrepancia es rechazada (`StaleSubscriptionApproval`),
por lo que una reanudación firmada obsoleta no puede eliminar una cancelación posterior que el suscriptor nunca
aprobó.
