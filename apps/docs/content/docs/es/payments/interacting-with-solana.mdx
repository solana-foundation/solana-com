...
<Callout type="caution" title="No uses RPC público para producción">
  Los endpoints públicos (`api.mainnet.solana.com`, `api.devnet.solana.com`) tienen
  límite de velocidad, no cuentan con SLA y no son adecuados para flujos de pago en producción.
  Utiliza un [proveedor de RPC](https://solana.com/rpc) para asegurar un endpoint RPC privado
