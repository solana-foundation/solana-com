...
<Callout type="caution">
  Commerce Kit se encuentra actualmente en beta. Las API pueden cambiar antes
  del lanzamiento estable. Para más detalles sobre Commerce Kit, consulta la
...
| Modo  | Descripción                                        |
| ----- | -------------------------------------------------- |
| `tip` | El usuario elige su propio monto (donaciones/tips) |
...
| `merchant.wallet` | `string` | Dirección de wallet Solana para recibir pagos |
...
| ------------------ | --------------------- | ----------------------------------- |
| `onPaymentSuccess` | `(signature: string)` | Transacción confirmada en la cadena |
| `onPaymentError`   | `(error: Error)`      | Pago fallido en cualquier etapa     |
| `onPaymentStart`   | `()`                  | Flujo de pago iniciado              |
| `onCancel`         | `()`                  | El usuario canceló el pago          |
