...
Ejecuta la prueba usando el comando `cargo test`. El log del programa mostrará
"Hello, world!".
...
Añade las dependencias `solana-client` y {/_
INLINE_CODE_PLACEHOLDER_deabe82dda182005cf088776022a3aa4 _/.
...
Los programas de Solana pueden actualizarse redistribuyéndolos al mismo ID de
...
Redistribuye el programa usando el mismo comando `solana program deploy`.
...
<Callout type="info">
  Alternativamente, puedes eliminar el archivo de keypair existente (por
  ejemplo, `./target/deploy/hello_world-keypair.json`) y ejecutar `cargo
  build-sbf` nuevamente, lo cual generará un nuevo archivo de keypair.
</Callout>
