...
Ahora agreguemos las dependencias necesarias para compilar un programa de
...
Los programas de Solana deben compilarse como bibliotecas dinámicas. Agrega la
...
maneja la deserialización de los datos de `input` en los parámetros de la
...
instrucciones a sus funciones de manejo apropiadas.
...
instrucción, luego se llama a la función de manejo apropiada. Cada función de
manejo incluye la implementación para esa instrucción.
...
- Lee el campo `data` de la cuenta para `counter_account`
...
Ahora necesitamos compilar y cargar nuestro programa en el entorno de pruebas.
...
{/_ CODE_PLACEHOLDER_46363dbbfb42c409476506fa30c14d0b _/
...
Agrega el código del cliente a `examples/client.rs` y reemplaza el `program_id`
...
`http://localhost:8899` donde se está ejecutando `solana-test-validator`.
