...
- `entrypoint.rs`: Define el punto de entrada que enruta las instrucciones
  entrantes.
- `state.rs`: Define el estado del programa (datos de cuenta).
...
Comencemos construyendo el programa de contador. Crearemos un programa que puede
...
macro maneja la deserialización de los datos de `input` en los parámetros de la
...
## !!steps Definir el enum de instrucciones
...
## !!steps Implementar la deserialización de instrucciones
...
## !!steps Enrutar instrucciones a los manejadores
...
## !!steps Implementar el manejador de inicialización
...
Nuestro programa realiza un CPI para llamar a la instrucción `create_account`
...
## !!steps Implementar manejador de incremento
...
Primero, añadamos las dependencias necesarias para las pruebas. Usaremos
...
Ahora añadamos un módulo de prueba a nuestro programa. Comenzaremos con la
...
El atributo `#[cfg(test)]` asegura que este código solo se compile al ejecutar
pruebas.

## !!steps Inicializar entorno de prueba

Configuremos el entorno de prueba con LiteSVM y financiemos una cuenta pagadora.
...
## Parte 4: Desplegando el programa
