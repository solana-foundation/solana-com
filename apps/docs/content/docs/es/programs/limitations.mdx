...
Desarrollar programas en la blockchain de Solana tiene algunas limitaciones
...
Los programas Rust onchain admiten la mayoría de libstd, libcore y liballoc de
Rust, así como muchas bibliotecas de terceros.
...
- Bincode es extremadamente costoso computacionalmente tanto en ciclos como en
  profundidad de llamadas y debe evitarse
- Debe evitarse el formateo de cadenas ya que también es computacionalmente
  costoso.
...
Cuando un programa excede el límite de profundidad de pila de llamadas
permitido, recibirá el error `CallDepthExceeded`.

## Profundidad de llamadas CPI - error `CallDepth`

Las invocaciones entre programas permiten que los programas invoquen otros
programas directamente, pero la profundidad está actualmente limitada a `4`.

Cuando un programa excede la
[profundidad de llamadas de invocación entre programas](/docs/core/cpi)
permitida, recibirá un error `CallDepth`
