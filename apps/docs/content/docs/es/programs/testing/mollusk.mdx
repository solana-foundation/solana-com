...
pruebas para probar programas de Solana. Proporciona una interfaz simple para
probar instrucciones de programas de Solana en un entorno minimizado de la
Máquina Virtual de Solana (SVM). Todas las cuentas de prueba deben definirse
...
Para medir el uso de unidades de cómputo, añade `mollusk-svm-bencher` como
...
Para utilizar el Token Program, el programa token2022 (Token Extensions) y el
Associated Token Program para realizar pruebas con Mollusk, añade
...
1. [Crear una instancia de `Mollusk`](mention:mollusk) - Inicializa Mollusk con
   un ID de programa y la ruta al programa compilado (archivo `.so`)
2. [Construir una instrucción](mention:instruction) - Crea una instrucción para
   invocar el programa
3. [Procesar y validar](mention:process) - Procesa la instrucción usando Mollusk
...
comprobaciones sobre el resultado. Puedes validar manualmente los resultados
...
SOL con verificaciones de validación después de cada instrucción.
...
Mollusk proporciona una estructura
[`Sysvars`](https://docs.rs/mollusk-svm/0.5.0/mollusk_svm/sysvar/struct.Sysvars.html)
personalizada para modificar sus valores para pruebas.
...
Associated Token Program a Mollusk para pruebas.
