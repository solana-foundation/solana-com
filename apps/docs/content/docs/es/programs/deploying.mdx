...
- **Program ID**: La dirección onchain de tu programa. Los usuarios interactúan
  con tu programa haciendo referencia al Program ID.
- **program account**: La cuenta onchain que almacena los metadatos de tu
  programa. La dirección de la program account es el Program ID. La program
  account también almacena la dirección de la programdata account.
- **programdata account**: La cuenta onchain que almacena el código ejecutable
  desplegado de tu programa.
...
El Program ID es la dirección onchain de tu program account. Los usuarios y
otros programas invocan tu programa haciendo referencia a este ID en sus
transacciones.
...
- **`--use-rpc`**: Envía transacciones al RPC configurado. Esta opción requiere una conexión RPC [ponderada por stake](/docs/defi/stake-weighted-qos) de proveedores como [Triton](https://triton.one/) o [Helius](https://www.helius.dev/).
