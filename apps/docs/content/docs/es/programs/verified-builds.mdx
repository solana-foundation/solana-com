...
Si deseas tener una configuración completamente confiable, puedes ejecutar la
[API de verificación](https://github.com/otter-sec/solana-verified-programs-api)
tú mismo o ejecutar la verificación del programa localmente usando el comando
`verify-from-repo` utilizando los datos de verificación en la cadena que se
guardan en una
[PDA](https://explorer.solana.com/address/63XDCHrwZu3mXsw2RUFb4tbNpChuUHx4eA5aJMnHkpQQ/anchor-account)
que se deriva de la autoridad de despliegue del programa y el
...
Además de las compilaciones verificadas, también puedes agregar un archivo
...
esta es la misma pubkey que compiló y desplegó el programa. Pero también puede
...
comuniquen con los mantenedores del proyecto, incluso si solo conocen la
dirección del contrato.

## ¿Por qué usar security.txt?

Para muchos proyectos, especialmente los más pequeños o privados, identificar a
los desarrolladores solo a partir de la dirección del contrato puede ser difícil
y llevar mucho tiempo. Incorporar un archivo `security.txt` dentro del programa
garantiza que los investigadores de seguridad puedan contactar fácilmente a las
personas correctas, previniendo potencialmente exploits y asegurando informes de
errores oportunos.

## Cómo implementar security.txt

Para agregar un `security.txt` a tu programa de Solana, incluye los siguientes
pasos:
...
puede consultarse fácilmente a través de herramientas como Solana Explorer,
asegurando que tus detalles de contacto y seguridad estén disponibles para
cualquiera que busque reportar posibles problemas.
...
- Verificación: antes de desplegar, verifica el formato y el contenido usando la
