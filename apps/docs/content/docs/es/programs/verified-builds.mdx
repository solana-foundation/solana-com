...
Después de subir tu PDA de verificación onchain con `verify-from-repo`, envía un
trabajo de verificación remota a la
...
El `--uploader` es la dirección que subió la PDA de verificación, generalmente
la autoridad de actualización de tu programa. Si tu programa está controlado por
un multisig, continúa en la sección de
[verificación con multisig](#how-to-verify-your-program-when-its-controlled-by-a-multisig-like-squads)
de esta guía a continuación.

> El indicador heredado `--remote` en `verify-from-repo` ha sido descontinuado.
> Sube tu PDA primero, luego ejecuta `remote submit-job`.

Esto envía un trabajo a la API de OtterSec. Puedes verificar el estado del
trabajo con:
...
Una vez que la verificación se haya completado con éxito, lo cual puede tardar
un tiempo, podrás ver tu programa como verificado en la
...
contribuyendo a un ecosistema de Solana más saludable.

</Step>
</Steps>

## Cómo verificar tu programa cuando está controlado por un Multisig como Squads

Para que la verificación remota funcione, debes escribir los datos de
verificación en una PDA firmada por la autoridad del programa. Si tu programa
está controlado por un multisig, puedes exportar esta transacción de escritura
de PDA y enviarla a través de [Squads Protocol](https://squads.so/protocol) u
otra solución multisig de tu elección.

<Steps>
<Step>

### 1. Compilar el programa verificable
...
Esto generará una compilación verificable usando un contenedor Docker con la
...
Para el resto de esta guía de multisig, usaremos un ID de programa de ejemplo de
...
Una vez hecho esto, confirmamos el proyecto en GitHub. Aquí tienes un ejemplo:
https://github.com/solana-developers/verify-squads

Opcional: Comprueba si puedes verificarlo localmente primero (este comando usa
el ID de programa de ejemplo `6XBGfP17P3KQAKoJb2s5M5fR4aFTXzPeuC1af2GYkvhD`):

```bash
solana-verify verify-from-repo https://github.com/solana-developers/verify-squads --program-id 6XBGfP17P3KQAKoJb2s5M5fR4aFTXzPeuC1af2GYkvhD
```

Solo para asegurarte de que los parámetros son correctos.

</Step>
<Step>

### 4. Transferir la autoridad del programa al multisig

Si aún no has transferido la autoridad de tus programas al multisig, copia la
autoridad del multisig. La necesitarás en el siguiente paso.
...
Cuando tienes la autoridad de tu programa localmente, se te solicita subir los
datos de compilación en la cadena al usar el comando
`solana-verify verify-from-repo`.

Dado que no puedes hacerlo cuando usas un multisig, debes exportar la
transacción PDA manualmente y luego activar la transacción a través de Squads.
...
### 6. Enviar la transacción a través de Squads
...
llamada al programa de verificación de osec y al programa de presupuesto de
cómputo, y nada más!
...
el explorador de Solana y en otros lugares.

</Step>
<Step>

### 8. Actualizar el programa (Opcional)

Cuando actualices tu programa, deberás exportar una nueva transacción PDA y
enviarla nuevamente a través de Squads.

Para realizar una actualización del programa:
...
Luego transfiere la autoridad del buffer al multisig o crea directamente el
...
### 9. Exportar y enviar una nueva transacción PDA

No olvides confirmar tus cambios en GitHub. Exporta nuevamente la transacción de
actualización PDA:
...
Envía la transacción nuevamente a través de Squads.
...
¡Felicitaciones, has verificado tu programa tras una actualización multisig!

</Step>
</Steps>

## Verificar desde una imagen de Docker

También puedes verificar tu programa contra una imagen de Docker ejecutando el
siguiente comando:
...
ejecutable en el contenedor sea el mismo que el hash del programa onchain
proporcionado al comando. Dado que la compilación ya fue cargada en una imagen,
no es necesario recompilar el ejecutable por completo, lo cual puede llevar
mucho tiempo.
...
Aquí tienes un ejemplo de verificación de un programa de ejemplo con el ID
...
De forma predeterminada, el comando `verify-from-repo` toma el último commit en
la rama principal. También puedes definir un commit específico en caso de que
quieras continuar trabajando en el repositorio usando el parámetro
`commit-hash`: `--commit-hash 5b82b86f02afbde330dff3e1847bed2d42069f4e`

Cuando se te solicite, responde sí para subir tu PDA de verificación en la
cadena. Luego, envía un trabajo de verificación remota a través de la API de
OtterSec:
...
> Ten en cuenta que fue necesario especificar `library-name` porque el
> repositorio de Squads incluye múltiples programas. Usamos el indicador `--bpf`
> porque `squads_mpl` fue verificado previamente con Anchor.
...
Comprueba estos problemas comunes:

- **Firmante incorrecto**: Confirma que tu firmante es la autoridad de
  actualización del programa ejecutando `solana program show YourProgramId`
- **Sin PDA en la cadena**: Ejecuta `solana-verify verify-from-repo -um` y
  **selecciona SÍ cuando se te indique**. Sin subir el PDA, la API no puede
  recuperar tus metadatos de verificación.
- **Discrepancia en los datos del PDA**: Actualiza tu PDA si has vuelto a
  desplegar tu programa. Los datos de tu PDA deben coincidir con el programa
  desplegado.
- **Hash de confirmación incorrecto**: Crea tu PDA usando el hash de
  confirmación exacto que desplegaste.
- **Diferencias en el entorno de compilación**: Usa Docker con solana-verify al
  crear tu PDA

## El hash de mi compilación local no coincide con el hash en la cadena. ¿Por qué?

Esto normalmente significa:

- Estás usando versiones diferentes de la cadena de herramientas de Rust/Solana
- Tus dependencias se actualizaron entre compilaciones
- No compilaste en un contenedor Docker
- Hiciste checkout del commit incorrecto

Soluciona esto compilando con `solana-verify build` en Docker usando el commit
exacto que desplegaste.

## ¿Cuánto tiempo tardará mi verificación?
...
Sigue tu progreso usando el endpoint de estado del trabajo.
...
de que pudieras crear un PDA, disponemos de una dirección en lista blanca para
esta situación. Contáctanos en contact@osec.io y te ayudaremos a verificar tu
programa.

## ¿Qué es el PDA y por qué es importante?

Tu PDA (Cuenta Derivada de Programa) permite la verificación sin necesidad de
confianza:

- **Almacenamiento en Cadena**: Guarda tus metadatos de verificación (URL del
  repositorio, hash del commit, parámetros de compilación) en cadena en una PDA
  propiedad del program account de Otter Verify
...
## ¿Por qué debo crear la PDA antes de usar la API?

La API solo funciona con PDAs en cadena porque:

- **Sin Confianza**: La API rechaza datos arbitrarios - solo utiliza lo que tu
  autoridad de actualización almacenó en cadena
- **Más Simple**: Solo proporciona el firmante + program_id; la API obtiene todo
  lo demás desde tu PDA
- **A Prueba de Manipulaciones**: Tu PDA crea un registro inmutable que
  cualquiera puede verificar de forma independiente
...
## ¿Con qué frecuencia debo verificar mi program account?

Verifica tu program account:

- Después de cada despliegue o actualización
- Cuando actualices tu repositorio de código fuente
- No te preocupes por volver a verificar en otros casos - la API vuelve a
  verificar automáticamente todos los programas cada 24 horas

## ¿Qué ocurre cuando actualizo mi program account?

Sigue estos pasos después de actualizar:

1. La API detecta tu actualización y desverifica tu program account.
...
3. Envía una nueva solicitud de verificación usando tu autoridad de
   actualización
4. La API verificará tu nueva versión con la PDA actualizada

**Importante**: Siempre actualiza tu PDA usando tu autoridad de actualización
con el nuevo hash del commit para el program account actualizado.

## ¿Puedo confiar en los resultados de verificación?

Sí: el sistema está diseñado para ser trustless e independientemente
verificable:

**¿Qué lo hace confiable?**

- **PDA en cadena**: Los metadatos de tu verificación residen en la cadena, sin
  estar controlados por ninguna autoridad central
- **Prueba de autoridad de actualización**: Solo la autoridad de actualización
  de tu programa puede crear/actualizar el PDA
- **Verificación independiente**: Cualquiera puede verificarlo leyendo tu PDA y
...
- Revisa siempre el código antes de interactuar con los programas
- Verificado ≠ auditado ni seguro
- Comprueba el repositorio y el commit en el PDA para confirmar que proviene de
  una fuente de confianza

## ¿Cómo puedo verificar un programa de forma independiente?

Verifica cualquier programa por ti mismo leyendo su PDA en cadena y ejecutando
la verificación localmente:

**Paso 1: Leer el PDA en cadena**
...
1. Los metadatos del PDA son auténticos (almacenados en cadena)
2. El código fuente en el repositorio del PDA coincide con el programa
   desplegado
3. No necesitas confiar en la API: verifica todo tú mismo en cadena

## ¿Puede otra persona verificar mi programa sin permiso?

Sí, esa es la razón por la que requerimos que el firmante sea la autoridad de
actualización. Solo consideramos válida la verificación si el firmante es la
...
- Docker (para builds deterministas)
...
- Tu PDA almacena una URL de repositorio público a la que cualquiera puede
  acceder
- La verificación sin confianza depende del acceso al código público
...
Los repositorios privados rompen el modelo de confianza central del sistema de
verificación.
...
**Importante**: Verifica siempre de forma local (paso 2) para confirmar que el
hash de compilación coincide antes de exportar la transacción PDA.

# Conclusión

Usar [compilaciones verificadas en Solana](/docs/programs/verified-builds)
garantiza la integridad y fiabilidad de tus programas en la red, y permite a los
desarrolladores encontrar tus SDKs directamente desde Solana Explorer.
Aprovechando herramientas como Solana Verify CLI y Docker, puedes mantener
compilaciones verificables y seguras que se corresponden con tu código fuente.
Toma siempre las precauciones necesarias para usar entornos consistentes y
considera soluciones de gobernanza para actualizaciones y despliegues seguros.

## Seguridad y aviso legal

Aunque las compilaciones verificadas son una herramienta poderosa para
garantizar la integridad de tus programas de Solana, la configuración
predeterminada no es completamente sin confianza. Las imágenes de Docker son
compiladas y alojadas por la Solana Foundation.

Ten en cuenta que estás compilando tu proyecto en una imagen de Docker
descargada y que toda tu configuración, incluyendo información potencialmente
sensible, se copia en dicha imagen durante la compilación.

Si deseas una configuración completamente sin confianza, puedes compilar las
imágenes de Docker tú mismo y alojarlas en tu propia infraestructura. De esta
forma, puedes asegurarte de que las imágenes de Docker no han sido manipuladas.
Puedes encontrar los scripts para crear tus propias imágenes de Docker en el
[repositorio de compilaciones verificadas](https://github.com/Ellipsis-Labs/solana-verifiable-build),
donde puedes hacer un fork y ejecutar las acciones de GitHub por tu cuenta, o
validar que son correctas.

Además, para la verificación remota, estás confiando en cierta medida en la API
de OtterSec y en el
[Solana Explorer](https://explorer.solana.com/address/PhoeNiXZ8ByJGLkxNfZRnkUfjvmuYqLR89jjFHGqdXY).

La API o Solana Explorer podrían mostrar información incorrecta si se vieran
comprometidos.

Si deseas una configuración completamente sin necesidad de confianza, puedes
ejecutar la
[API de verificación](https://github.com/otter-sec/solana-verified-programs-api)
por tu cuenta, o bien ejecutar la verificación del programa de forma local
utilizando el comando `verify-from-repo` con los datos de verificación on-chain
guardados en una
[PDA](https://explorer.solana.com/address/63XDCHrwZu3mXsw2RUFb4tbNpChuUHx4eA5aJMnHkpQQ/anchor-account)
...
El programa de verificación es desplegado por el
[equipo de OtterSec](https://osec.io/) y aún no está congelado, por lo que puede
actualizarse en cualquier momento.

La Solana Foundation, OtterSec y el equipo de Ellipsis Labs no son responsables
de ninguna pérdida o daño que pueda derivarse del uso del pipeline de builds
verificados.

# Security.txt para programas de Solana

Además de las builds verificadas, también puedes añadir un archivo
...
acceso a los datos de verificación almacenados en la PDA de verificación. La PDA
que contiene toda la información necesaria para compilar y verificar un programa
se deriva de la dirección del programa y del pubkey del verificador. Por
defecto, este es el mismo pubkey que compiló y desplegó el programa, aunque
también puede ser otro pubkey que se especifique en el `security.txt`.

La función `security.txt` permite a los desarrolladores incorporar información
de contacto y seguridad directamente en sus contratos inteligentes de Solana.
Inspirada en [securitytxt.org](https://securitytxt.org), este enfoque
proporciona una forma estandarizada para que los investigadores de seguridad
contacten con los responsables del proyecto, incluso si solo conocen la
dirección del contrato.

## ¿Por qué usar security.txt?

Para muchos proyectos, especialmente los más pequeños o privados, identificar a
los desarrolladores a partir de la dirección del contrato puede ser difícil y
llevar mucho tiempo. Incorporar un archivo `security.txt` en el programa
garantiza que los investigadores de seguridad puedan contactar fácilmente con
las personas correctas, lo que podría prevenir exploits y garantizar informes de
errores oportunos.
...
Usa el macro `security_txt!` en tu contrato para definir la información de
...
Una vez que la información de `security.txt` esté integrada en tu programa, se
puede consultar fácilmente a través de herramientas como Solana Explorer, lo que
garantiza que tus datos de contacto y seguridad estén disponibles para
cualquiera que desee reportar posibles problemas.

## Buenas prácticas

- Usa enlaces: Para información que pueda cambiar con frecuencia (p. ej., datos
  de contacto), se recomienda enlazar a una página web en lugar de codificarlos
  directamente en el contrato. Esto evita la necesidad de actualizaciones
  frecuentes del programa.

- Verificación: Antes de desplegar, verifica el formato y el contenido
  utilizando la herramienta `query-security-txt`, que puede validar tanto
  programas onchain como binarios locales:
...
facilitas que los investigadores puedan comunicarse contigo, fomentando una
...
El proyecto `security.txt` está mantenido por
[Neodyme Labs](https://github.com/neodyme-labs)

Puedes consultar el estado de verificación y explorar los programas verificados
en [verify.osec.io](https://verify.osec.io).
