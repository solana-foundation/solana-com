...
Comienza actualizando el archivo `lib.rs` para traer al alcance elementos del
módulo `system_program`.
...
SOL de un usuario cuando actualiza su cuenta de mensaje.
...
_rs`Update`_. Esta cuenta sirve como un "depósito" controlado por el programa
que recibe SOL de los usuarios cuando actualizan sus mensajes.
...
Aspectos clave del `vault_account`:

- La dirección de la cuenta proviene de un PDA derivado usando las semillas
...
0.001 SOL desde la cuenta del usuario a la cuenta vault.
...
En la instrucción `update`, la implementación incluye una invocación entre
programas (CPI) para invocar la instrucción `transfer` del System Program. Esto
demuestra cómo realizar una CPI desde dentro del programa, permitiendo la
componibilidad de los programas de Solana.

La estructura _rs`Transfer`_ especifica las cuentas requeridas para la
instrucción de transferencia del System Program:

- `from` - La cuenta del usuario (origen de los fondos)
- `to` - La cuenta de bóveda (destino de los fondos)
...
- La `amount` a transferir (1,000,000 lamports, o 0.001 SOL)
...
La configuración para una CPI coincide con cómo se construyen las instrucciones
del lado del cliente, donde especificas el programa, las cuentas y los datos de
instrucción para una instrucción particular a invocar. Cuando la instrucción
`update` del programa recibe una invocación, internamente invoca la instrucción
...
Los _rs`signer_seeds`_ especifican las semillas opcionales y las semillas bump
...
_rs`ctx: Context<Delete>`_ para utilizar el contexto en el cuerpo de la función.
...
- La cantidad a transferir (el saldo completo de la cuenta de bóveda)
...
cuenta de bóveda en las instrucciones. Esto requiere derivar el PDA de la bóveda
e incluirlo en las llamadas de instrucción de actualización y eliminación.
...
Haz clic en el botón `Import` e ingresa un nombre de proyecto para agregarlo a
tu lista de proyectos en Solana Playground. Una vez que se importa un proyecto,
todos los cambios se guardan automáticamente y de forma persistente.
