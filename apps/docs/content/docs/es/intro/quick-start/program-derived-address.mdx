...
para agregar en los siguientes pasos.
...
Antes de comenzar, ejecuta _shell`build`_ en la terminal del Playground para
verificar que el programa inicial se compile correctamente.
...
representan datos de cuenta (tipo de dato para almacenar en el campo de datos de
la cuenta).
...
El campo _rs`bump`_ almacena un _rs`u8`_
["bump" seed](/docs/core/pda#canonical-bump) utilizado para derivar una Program
Derived Address (PDA). Almacenar este valor ahorra cómputo al eliminar la
necesidad de recalcularlo en instrucciones posteriores.
...
- `bump` - _rs`u8`_ que almacena el ["bump" seed](/docs/core/pda#canonical-bump)
  para derivar la Program Derived Address (PDA). Almacenar este valor ahorra
  cómputo al eliminar la necesidad de recalcularlo en instrucciones posteriores.

Al crear una cuenta, el programa serializa los datos de _rs`MessageAccount`_ y
los almacena en el campo de datos de la nueva cuenta.

Posteriormente, al leer desde la cuenta, el programa deserializa estos datos de
...
Ahora, añade la instrucción _rs`create`_ que crea e inicializa
...
La restricción _rs`bump`_ declara el bump seed para la PDA.
...
La restricción `seeds` define las entradas opcionales utilizadas para derivar la
PDA.
...
La restricción `bump` indica a Anchor que encuentre y utilice automáticamente el
bump seed correcto. Anchor utiliza `seeds` y `bump` para derivar la PDA.
...
- Discriminador de cuenta Anchor (identificador): 8 bytes
...
- Bump seed de la PDA (_rs`u8`_): 1 byte
...
2. _rs`message: String`_ - El mensaje del usuario para almacenar.
...
2. Inicializando datos de la cuenta:
   ...
   - Establece el valor `bump` utilizado para derivar el PDA, obtenido de
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - La restricción `realloc` invoca el System Program para ajustar el tamaño de
     los datos de la cuenta

Ten en cuenta que la restricción _rs`bump = message_account.bump`_ utiliza el
bump seed almacenado en el _rs`message_account`_, en lugar de hacer que Anchor
lo recalcule.
...
2. Actualiza los datos de la cuenta:
   - Accede al `message_account` desde el contexto.
   - Establece el campo `message` con el nuevo `message` del argumento de la
     función.
...
A continuación, añade la instrucción _rs`delete`_ para cerrar el
_rs`MessageAccount`_.
...
La restricción _rs`seeds`_ especifica las seeds utilizadas para derivar el PDA.

### !bump

La restricción _rs`bump`_ especifica el bump seed para el PDA.

En este caso, el programa utiliza el bump seed existente almacenado en el
...
En este caso, la cuenta _rs`user`_ recibe los lamports de la cuenta
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - Las restricciones `seeds` y `bump` verifican la cuenta como el PDA correcto
   - La restricción `close = user` marca esta cuenta para cerrar y transfiere
...
1. _rs`_ctx: Context<Delete>`_ - Proporciona acceso a las cuentas especificadas
   ...
   no usa el Context en su cuerpo.

El cuerpo de la función solo imprime un mensaje en los logs del programa usando
la macro _rs`msg!()`_. La función no necesita lógica adicional porque la
...
Ahora has completado el programa CRUD básico. Despliega el programa ejecutando
...
Añade el código a continuación dentro de _ts`describe()`_, pero antes de las
secciones _ts`it()`_.
...
Solana Playground elimina parte de la configuración repetitiva donde
[`pg.program`](mention:one) permite acceder a métodos para interactuar con el
programa, mientras que [`pg.wallet`](mention:two) da acceso a tu billetera de
playground.
...
Ejecuta el archivo de prueba ejecutando _shell`test`_ en la terminal de
Playground para verificar que se ejecute como se espera. Los siguientes pasos
agregan las pruebas reales.
...
El método _ts`fetchNullable()`_ recupera datos de cuenta desde la red que
podrían no existir.
...
devuelve null cuando la cuenta ya no existe después de cerrarla.
...
en la terminal del Playground. Este comando ejecuta las pruebas contra el
...
<Callout type="info">
  ...
  instrucción `create` falla porque `messageAccount` ya existe como una cuenta.
  Solo una cuenta puede existir para una PDA dada.
</Callout>
