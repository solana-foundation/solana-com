...
description:
  Aprende a construir un programa CRUD (Crear, Leer, Actualizar, Eliminar) en
  Solana utilizando Program Derived Addresses (PDAs) y el framework Anchor. Esta
  guía paso a paso demuestra cómo crear, actualizar y eliminar cuentas de
  mensajes onchain usando PDAs, implementar validación de cuentas y escribir
  pruebas. Perfecto para desarrolladores que buscan entender cómo usar PDAs en
  programas de Solana.
...
representan datos de cuenta (tipo de dato a almacenar en el campo de datos de la
cuenta).
...
["bump" seed](/docs/core/pda#canonical-bump) utilizado para derivar una
dirección derivada de programa (PDA). Almacenar este valor ahorra cómputo al
eliminar la necesidad de recalcularlo en instrucciones posteriores.
...
- `bump` - _rs`u8`_ que almacena el ["bump" seed](/docs/core/pda#canonical-bump)
  para derivar la dirección derivada de programa (PDA). Almacenar este valor
  ahorra cómputo al eliminar la necesidad de recalcularlo en instrucciones
  posteriores.
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - Internamente, la restricción `init` llama al System Program para crear una
     nueva cuenta con el `space` especificado y cambia el propietario al
     programa actual.
...
- _rs`b"message"`_ - Una cadena fija como el primer seed.
- _rs`user.key().as_ref()`_ - La clave pública de la cuenta _rs`user`_ como el
  segundo seed.
...
2. Inicializando datos de cuenta:
   ...
   - Establece el valor `bump` usado para derivar el PDA, obtenido de
...
bump seed almacenado en _rs`message_account`_, en lugar de hacer que Anchor lo
recalcule.
