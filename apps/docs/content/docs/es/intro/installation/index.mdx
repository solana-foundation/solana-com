---
title: Instalación local
seoTitle: Instala la CLI de Solana y Anchor con un solo comando
description:
  A quick installation guide that allows you to set up your local Solana
  development environment with a single command. Install Rust, the Solana CLI,
  and Anchor Framework on Windows (WSL), Linux, and Mac.
---

Configura tu entorno de desarrollo local de Solana con un solo comando. Esto instala
Rust, la CLI de Solana, Anchor Framework, Surfpool, Node.js y Yarn en Windows
(WSL), Linux y Mac.

## Requisitos previos

<Accordions>
<Accordion title="Windows">

Para desarrollar programas de Solana en Windows **debes** usar
[Windows Subsystem for Linux](https://learn.microsoft.com/en-us/windows/wsl/install)
(WSL). Después de instalar WSL, todas las demás dependencias se instalarán a
través de la terminal de Linux.

Para instalar WSL, ejecuta el siguiente comando en Windows PowerShell:

```terminal
$ wsl --install
```

El proceso de instalación te pedirá que crees una cuenta de usuario
predeterminada.

![Instalación de WSL](/assets/docs/intro/installation/wsl-install.png)

Por defecto, WSL instala Ubuntu. Puedes abrir una terminal de Linux buscando
"Ubuntu" en la barra de búsqueda.

![WSL Ubuntu](/assets/docs/intro/installation/wsl-ubuntu-search.png)

Si tu terminal de Ubuntu se ve como la imagen a continuación, es posible que
encuentres un problema donde `ctrl + v` (atajo de teclado para pegar) no
funciona en la terminal.

![Terminal de Ubuntu](/assets/docs/intro/installation/wsl-ubuntu-terminal-1.png)

Si encuentras este problema, abre Windows Terminal buscando "Terminal" en la
barra de búsqueda.

![Windows Terminal](/assets/docs/intro/installation/wsl-windows-terminal.png)

A continuación, cierra Windows Terminal y vuelve a abrir una terminal de Linux
buscando Ubuntu nuevamente. La terminal ahora debería verse como la imagen a
continuación, donde `ctrl + v` (atajo de teclado para pegar) funciona.

![Terminal de Ubuntu](/assets/docs/intro/installation/wsl-ubuntu-terminal-2.png)

Si usas VS Code, la
[extensión WSL](https://code.visualstudio.com/docs/remote/wsl-tutorial) te
permite usar WSL y VS Code juntos.

![Configuración de WSL en VS Code](/assets/docs/intro/installation/wsl-vscode.png)

Verás lo siguiente en la barra de estado de VS Code:

![WSL: Ubuntu](/assets/docs/intro/installation/wsl-vscode-ubuntu.png)

Una vez que tengas WSL configurado, estás listo para:

- Completar la [instalación rápida](#installation) usando la terminal de Ubuntu
  (Linux)
- Instalar [Rust](/docs/intro/installation/dependencies#install-rust), la
  [CLI de Solana](/docs/intro/installation/dependencies#install-solana-cli) y la
  [CLI de Anchor](/docs/intro/installation/dependencies#install-anchor-cli)
  individualmente, como se describe a continuación

</Accordion>
<Accordion title="Linux">

<Tabs items={[".deb", ".rpm"]}>
<Tab value=".deb" >
Si estás en Debian o alternativas derivadas de Debian como Ubuntu o Mint, sigue los pasos a continuación:

1. Actualiza las listas de paquetes para asegurar que todos los paquetes estén
   actualizados:

```terminal
$ sudo apt-get update
```

2. Instala las siguientes dependencias:

```terminal
$ sudo apt-get install -y \
    build-essential \
    pkg-config \
    libudev-dev llvm libclang-dev \
    protobuf-compiler libssl-dev
```

<Callout>
Si encuentras el siguiente error al instalar `protobuf-compiler`, asegúrate
de ejecutar primero _shell`sudo apt-get update`

```
Package protobuf-compiler is not available, but is referred to by another package.
This may mean that the package is missing, has been obsoleted, or
is only available from another source
```

</Callout>

  </Tab>

    <Tab value=".rpm" >
    Para derivados de RHEL como Fedora o Red Hat Linux, sigue los siguientes pasos.

Actualiza las listas de paquetes para obtener las versiones más recientes de los
paquetes ejecutando el comando.

```terminal
$ sudo dnf upgrade --refresh
```

A continuación, instala las siguientes dependencias:

```terminal
$ sudo dnf group install c-development development-tools \
&& sudo dnf install pkg-config openssl-devel
```

  </Tab>
</Tabs>

</Accordion>
</Accordions>

## Instalación

1. Ingresa el siguiente comando en tu terminal para instalar todas las
   dependencias necesarias:

```terminal
$ curl --proto '=https' --tlsv1.2 -sSfL https://solana-install.solana.workers.dev | bash
```

Una instalación exitosa devolverá una salida como la siguiente:

```txt title="Example output"
Installed Versions:
Rust: rustc 1.91.1 (ed61e7d7e 2025-11-07)
Solana CLI: solana-cli 3.0.10 (src:96c3a851; feat:3604001754, client:Agave)
Anchor CLI: anchor-cli 0.32.1
Surfpool CLI: surfpool 0.12.0
Node.js: v24.10.0
Yarn: 1.22.1
```

2. Verifica una instalación exitosa comprobando la versión de cada dependencia
   instalada.

```terminal
$ rustc --version && solana --version && anchor --version && surfpool --version && node --version && yarn --version
```

<Callout>
  Si el comando de instalación falla, [instala cada dependencia
  individualmente](/docs/intro/installation/dependencies).
</Callout>

## Recursos para el desarrollo local

<Cards>
  <Card
    title="Instalar dependencias"
    href="/docs/intro/installation/dependencies"
  >
    Instala cada herramienta individualmente o soluciona problemas del instalador de un solo comando.
  </Card>
  <Card
    title="Conceptos básicos de la CLI de Solana"
    href="/docs/intro/installation/solana-cli-basics"
  >
    Configura clústeres, gestiona keypairs y usa los comandos más comunes de la CLI de Solana.
  </Card>
  <Card
    title="Conceptos básicos de la CLI de Anchor"
    href="/docs/intro/installation/anchor-cli-basics"
  >
    Crea, compila, prueba e implementa proyectos de Anchor.
  </Card>
  <Card
    title="Conceptos básicos de la CLI de Surfpool"
    href="/docs/intro/installation/surfpool-cli-basics"
  >
    Ejecuta un validator local e inspecciónalo con Surfpool Studio.
  </Card>
</Cards>
