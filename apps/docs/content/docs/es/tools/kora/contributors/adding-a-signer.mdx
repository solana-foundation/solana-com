...
- [ ] Dependencia `solana-keychain` actualizada en Cargo.toml a la última
      versión
- [ ] Añadida la estructura de configuración para tu firmante
- [ ] Añadida la variante `SignerTypeConfig`
- [ ] Añadida la lógica de compilación en `build_signer_from_config`
- [ ] Añadida la lógica de validación en `validate_individual_signer_config`
- [ ] Estructura de configuración exportada en `mod.rs`
- [ ] (Opcional) Añadido el método constructor de mock de prueba en
      `config_mock.rs`
- [ ] El código compila sin advertencias
- [ ] Todas las pruebas pasan (`make test` e `make test-integration`)
- [ ] Documentación añadida siguiendo
      [`ADDING_SIGNERS.md`](https://github.com/solana-foundation/solana-keychain/blob/main/docs/ADDING_SIGNERS.md)
- [ ] Archivos de configuración de ejemplo creados (`.toml` e `.env.example`)
- [ ] Sin valores codificados de forma fija ni secretos
- [ ] Los mensajes de error son útiles
- [ ] Sigue las convenciones de nomenclatura de Rust (snake_case)
- [ ] El linting pasa (`make lint`)
- [ ] Contacta al equipo de Kora con las claves de API para las pruebas de
      integración

## Obtener ayuda

- **Para la implementación del firmante**: Abre un issue en el repositorio
  [`solana-keychain`](https://github.com/solana-foundation/solana-keychain)
- **Para la integración con Kora**: Abre un issue en el repositorio de Kora para
  discusiones de diseño
- Únete a nuestros canales de la comunidad
...
¡Bienvenido al ecosistema de Kora! Nos entusiasma contar con tu solución de
gestión de claves como parte de la plataforma.
