...
> _Nota: Cuando `fallback_if_unavailable` es verdadero, el sistema permite que las transacciones
> continúen si Redis no está disponible temporalmente, evitando interrupciones del servicio.
> Establecer `max_transactions` en 0 permitirá transacciones ilimitadas._
...
| `get_payer_signer`          | Devuelve el siguiente pagador de tarifas y la dirección de pago        | ✅       | booleano |
...
[Documentos de Solana](https://solana.com/developers/cookbook/transactions/durable-nonces).
...
| ---------------------------- | ------------------------------------------------ |
| `confidential_transfer_mint` | Configuración de Saldos Confidenciales para el mint |
...
| Nombre de la Extensión                  | Descripción                                 |
| ------------------------------- | ------------------------------------------- |
| `confidential_transfer_account` | Estado de Saldos Confidenciales para la cuenta |
