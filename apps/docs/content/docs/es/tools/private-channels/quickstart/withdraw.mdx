...
su saldo de tokens del lado del canal llamando a `WithdrawFunds` en el Programa de
Retiro. Segundo, un operador llama a `ReleaseFunds` en el Programa de Custodia (devnet,
en este recorrido) con una prueba de exclusión de Sparse Merkle Tree válida para liberar
...
Llama a `WithdrawFunds` en el Programa de Retiro para quemar tu saldo del lado del canal.
Usa la variante `Async`, que deriva automáticamente el PDA de `tokenAccount` y es la
...
- ID del Programa de Retiro: `J231K9UEpS4y4KAPwGc4gsMNCjKFRMYcQBcjVW7vBhVi`
- Envía esta transacción al gateway (`http://localhost:8899`), no a un
  RPC público de Solana
- Si se proporciona `destination`, los fondos liberados se envían a esa billetera en lugar de la
  del firmante
...
1. `indexer-private-channel` consulta el canal cada 1 segundo en busca de eventos de quema
   y registra un retiro pendiente cuando se detecta el tuyo
2. `operator-private-channel` consulta la base de datos cada 1 segundo en busca de
   registros pendientes y envía `ReleaseFunds` al Programa de Custodia en Solana devnet
...
Tras la quema del lado del canal, un operador aprovisionado debe llamar a `ReleaseFunds` en
el Programa de Custodia con una prueba de exclusión SMT válida; el operador no puede liberar
fondos sin demostrar que el nonce de retiro no ha sido utilizado anteriormente. Este paso
...
- `user`: billetera receptora en devnet
- `new_withdrawal_root`: raíz SMT actualizada tras este retiro
- `transaction_nonce`: nonce único para esta hoja
- `sibling_proofs`: 512 bytes (16 hashes de 32 bytes de nodos hermanos para la prueba del árbol)
...
el contrato de custodia, enviaste una transferencia fuera de cadena a través del gateway y retiraste fondos
...
<Cards>
  ...
  >
    Comprende en profundidad a los participantes y el flujo completo desde el depósito hasta el retiro.
  </Card>
  <Card
  ...
  <Card
    title="Referencia de Liberación de Fondos"
    href="/docs/tools/private-channels/instructions/release-funds"
  >
    Referencia completa de la instrucción ReleaseFunds para las herramientas del operador.
  </Card>
  <Card
