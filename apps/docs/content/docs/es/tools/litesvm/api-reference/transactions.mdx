---
title: Transacciones
description: Métodos para gestionar transacciones
---
...
Ejecutar una transacción y modificar el estado.
...
Simular una transacción sin modificar el estado.

Devuelve `SimulatedTransactionInfo` que contiene:

- `meta`: Metadatos de la transacción (registros, unidades de cómputo, firma,
  etc.)
- `post_accounts`: Estados de las cuentas tras la simulación
...
Usa `simulate_transaction()` para:

- Probar la validez de una transacción sin modificar el estado
- Comprobar el consumo de unidades de cómputo
- Verificar los registros de la transacción sin compromiso
- Probar las rutas de manejo de errores
...
`send_transaction()` devuelve `TransactionMetadata` directamente, mientras que
`simulate_transaction()` devuelve `SimulatedTransactionInfo` que contiene
...
### Acceso a los datos de la transacción
...
Consultar transacción del historial. Requiere que `with_transaction_history()`
esté configurado.
...
Obtener el blockhash actual para las transacciones.
...
Forzar la expiración del blockhash actual.
