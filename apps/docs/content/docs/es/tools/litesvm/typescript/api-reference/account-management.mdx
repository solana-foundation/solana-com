...
Financiar una cuenta con lamports.
...
Establecer directamente el estado de una cuenta sin una transacción.
...
Obtener el saldo SOL de una cuenta. Devuelve `null` si la cuenta no existe.
...
Calcular el mínimo de lamports necesarios para la exención de rent.
...
Para pruebas, puedes configurar múltiples cuentas de forma eficiente:
...
<Callout type="info">
  Usar `setAccount` es más rápido que `airdrop` para la configuración masiva de cuentas, ya que
