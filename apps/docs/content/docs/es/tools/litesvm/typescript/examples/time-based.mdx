...
description: Prueba lógica dependiente del tiempo con la manipulación del reloj de LiteSVM
---

Demuestra el uso de `warpToSlot` para probar lógica de programas dependiente del tiempo.
...
<Callout type="warn">
  `warpToSlot` avanza el campo `slot` del reloj, pero **no** actualiza automáticamente
  `epoch` ni `unixTimestamp`. Usa `setClock` para tener control total sobre los
  campos del reloj.
</Callout>

## Manipulación Manual del Reloj

Cuando tu programa lee `epoch` o `unix_timestamp` del sysvar Clock, usa
...
avance simple de slot, y `setClock` cuando necesites controlar el epoch o la marca de tiempo.
...
| Retiros bloqueados por tiempo | Avanzar más allá del slot de desbloqueo              |
| Limitación de velocidad           | Avanzar entre intervalos permitidos     |
| Órdenes con vencimiento         | Avanzar más allá de la expiración de la orden         |

<Callout type="info">
  Ten en cuenta que `warpToSlot` solo cambia el slot. La marca de tiempo unix puede no
  actualizarse proporcionalmente según la implementación del SVM.
</Callout>

## Puntos Clave

1. **Habilitar Sysvars**: Llama a `withSysvars()` antes de usar los métodos del reloj
2. **Avanzar**: Usa `warpToSlot(slot)` para adelantar el slot
3. **Control Total**: Usa `setClock` cuando necesites establecer `epoch`,
   `unixTimestamp` u otros campos del reloj
4. **Leer el Reloj**: Usa `getClock()` para leer el slot, epoch y marca de tiempo actuales
