...
Probar un programa personalizado de Solana implica cargar su archivo `.so` compilado en la
SVM, configurar las cuentas que espera y enviar instrucciones contra él.
LiteSVM gestiona todo esto en proceso — no se necesita ningún validator. Recomendamos
generar clientes de programa con el
...
1. **Carga del Programa**: Usa `addProgramFromFile` para cargar programas compilados e
   importa myProgramPlugin() desde tu cliente
2. **Configuración**: Habilita sysvars, builtins y precompiles según sea necesario
3. **Configuración de Cuentas**: Pre-rellena las cuentas de datos con `setAccount`
...
Una vez que tengas este esqueleto funcionando, puedes construir un conjunto completo de pruebas
añadiendo más instrucciones y aserciones.
