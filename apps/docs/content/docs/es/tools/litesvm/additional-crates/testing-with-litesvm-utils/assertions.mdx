...
title: Aserciones
description:
  Verifica estados de cuentas, saldos y propiedad con helpers de aserción
---

## Existencia de Cuentas

Verifica si las cuentas existen o han sido cerradas.

### Verificar que una Cuenta Existe
...
### Verificar que una Cuenta está Cerrada
...
<Callout type="info">
  `assert_account_closed` se cumple cuando una cuenta no existe, tiene cero
  lamports o tiene datos vacíos. Úsalo para verificar la limpieza después de
  cerrar cuentas.
</Callout>

## Aserciones de Saldo

Verifica los saldos de SOL y token account.
...
## Propiedades de Cuentas

Verifica la propiedad y las propiedades de datos de las cuentas.

### Verificar Propietario de la Cuenta
...
### Verificar la Longitud de los Datos de la Cuenta
...
<Callout type="info">
  Tamaños comunes de cuentas: - **Mint Account**: 82 bytes - **token account**:
  165 bytes - **System Account**: 0 bytes (solo lamports)
</Callout>

## Ejemplo completo

Aquí hay una prueba exhaustiva que demuestra múltiples aserciones:
...
## Manejo de fallos en las aserciones

Cuando una aserción falla, generará un pánico con un mensaje descriptivo:
...
<Callout type="warn">
  Las aserciones están diseñadas para fallar rápidamente con mensajes de error
  claros. Úsalas con frecuencia en tus pruebas para detectar problemas a tiempo.
</Callout>
