...
| `cdp`        | Plataforma para Desarrolladores de Coinbase     |
| `crossmint`  | Carteras gestionadas de Crossmint       |
| `dfns`       | Infraestructura de carteras de Dfns      |
...
Para la firma con custodia MPC de Fordefi. Las solicitudes a la API se autentican con una firma de solicitud ECDSA P-256 obtenida de `private_key_pem`; para mantener esa clave en un KMS/HSM, implemente `FordefiRequestSigner` y utilice `Signer::from_fordefi_with_signer`.

Fordefi tiene dos modos de firma. Configure `chain` para el **modo nativo de Solana**, donde Fordefi puede reemplazar el blockhash o las comisiones y transmite la transacción por sí mismo. Deje `chain` como `None` para el **modo de caja negra**, donde Fordefi firma bytes sin procesar y usted mismo transmite la transacción.
