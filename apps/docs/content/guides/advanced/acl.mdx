...
│                                                     │
│   ❌ Terrible UX - users wait hours/days            │
│                                                     │
└─────────────────────────────────────────────────────┘
```
...
2. **Gate Programs**: External programs that implement the allow/block logic.
   The ABL (Allow Block List) Gate Program is the reference implementation.
...
Do make it easy the program are already deployed on devnet, you can use the
...
1. **Solana CLI**
   ...
   (For running it locally use 2.xNOT 3.x - there's a known issue with
...
2. **Explore Custom Gate Programs**: Build your own Gate Program for custom
   allow/block logic
