...
import { getMetadataFromSlug, DefiDocsPage } from "../defi";
...
export default async function Page(props: Props) {
  ...
  return <DefiDocsPage slug={["defi", ...slug]} locale={locale} />;
}

export async function generateStaticParams() {
  return toStaticParams(docsSource)
    .filter((param) => param.slug[0] === "defi")
...
export async function generateMetadata(props: Props) {
  const { slug, locale } = await props.params;
  return getMetadataFromSlug(["defi", ...slug], locale);
}
