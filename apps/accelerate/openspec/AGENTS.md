...
- Validate: `openspec validate [change-id] --strict --no-interactive` and fix
  issues
...
3. Draft spec deltas using `## ADDED|MODIFIED|REMOVED Requirements` with at
   least one `#### Scenario:` per requirement.
4. Run `openspec validate <id> --strict --no-interactive` and resolve any issues
...
- Run `openspec validate --strict --no-interactive` to confirm the archived
  change passes checks
