...
{/* Illustration recommendation: A visual showing seed phrase security best practices - physical storage methods (safe, bank vault) with red X marks over digital storage (screenshots, cloud, email) */}
...
{/* Illustration recommendation: Examples of common scams - fake websites with subtle URL differences, social media impersonators, too-good-to-be-true offers, with warning indicators */}
...
{/* Illustration recommendation: A transaction approval screen with annotations pointing out what to check - recipient address, amounts, permissions being granted, with green checkmarks and red warnings */}
...
{/* Illustration recommendation: A comparison of security tools - hardware wallet (most secure), multi-sig setup (shared control), hot wallet (convenient), showing appropriate use cases for each */}
