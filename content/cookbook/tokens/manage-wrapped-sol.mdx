...
Wrapped SOL just is like any other token mint. The difference is using `syncNative`
...
```typescript title="add-balance-by-sol.ts"
import {
  clusterApiUrl,
  Connection,
  Keypair,
...
```typescript title="add-balance-by-token.ts"
import {
  clusterApiUrl,
  Connection,
  Keypair,
