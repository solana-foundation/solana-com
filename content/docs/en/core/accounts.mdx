...
Except for loader-v3 all loaders store the executable code of the programs they
manage in a so called program account:
...
Loader-v3 works differently from all other loaders as it has one indirection for
each program. The program account only contains the address of the programdata
account which then in turn holds the actual executable code:
...
On Solana the executable code of a program is stored in a different account than
the state of the program is. This is comparable to how operating systems
