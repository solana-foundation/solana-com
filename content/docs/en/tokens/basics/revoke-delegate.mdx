...
Revoking delegates involves removing the an assigned delegate from spending the authority's pre-approved token balance. This can be done by calling the [Revoke](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/interface/src/instruction.rs#L133C5-L133C11) instruction on the token program/s. Only the authority of the source token account can revoke a previously approved delegate when calling this instruction.
