...
description: Learn how to freeze tokens.
---

## How To Freeze Tokens

Freezing tokens on Solana refers to restricting tokens from being transfered, rendering them unusable withhin the Solana network. Only the address that is set in the freeze authority field for a token mint account has the power to do this and majority of the time it is usually revoked/set to null. To freeze a token account, call the [FreezeAccount](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L228) instruction on the token program/s with the account you'd like to freeze tokens for if you are the freeze authority.
...
console.log("\nFreezing token account: ", payerAssociatedTokenAcc);

const txSignature = await pipe(
	await createDefaultTransaction(client, payer),
	(tx) => appendTransactionMessageInstruction(freezeIx, tx),
...
use spl_token::{
    instruction::{ freeze_account, initialize_mint2, mint_to_checked },
    state::Mint,
...
async fn main() -> anyhow::Result<()> {
    ...
    match client.send_and_confirm_transaction(&transaction).await {
        ...
        Err(err) => eprintln!("Error freezing tokens: {}", err),
    }
...
async fn setup(
    ...
    let mut transaction = Transaction::new_with_payer(
        &[create_mint_account_ix, initialize_mint_ix, create_ata_ix, mint_to_ix],
        Some(&authority_keypair.pubkey())
    );
