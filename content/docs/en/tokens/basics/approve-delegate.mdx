...
Approving delegates involves giving another account the ability to spend a specified amount of tokens in your token account. This happens when you call the [`ApproveChecked`](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L295) instruction on the token program. Only the authority of the source token account can call this instruction to approve a delegate. A token account can only have one approved delegate at a time.
