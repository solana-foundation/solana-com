...
## How To Thaw a Frozen Token Account

Thawing token accounts is the process of unfrezing/thawing a previously frozen token account. Only the freeze authority address is able to do this by calling the [ThawAccount](https://github.com/solana-program/token/blob/a7c488ca39ed4cd71a87950ed854929816e9099f/program/src/instruction.rs#L243) instruction on the token program/s.
...
// Generate keypairs for mint address and fee payer
...
console.log("\nThawing frozen token account: ", payerAssociatedTokenAcc);

const txSignature = await pipe(
	await createDefaultTransaction(client, payer),
	(tx) => appendTransactionMessageInstruction(thawIx, tx),
 ...
 * for the payer and mints tokens to the token account
 *
 */
async function setup() {
	...
	const initializeMintInstruction = getInitializeMintInstruction({
		...
		freezeAuthority: payer.address, // if left out, freeze auth is set to null and token accounts can't be frozen
	});
...
console.log("Thawing frozen token account...");
const thawIx = await thawAccount(
	connection,
	feePayer,
	tokenAccount.address,
	mint,
...
console.log("Thaw transaction:", thawIx);
```
...
use spl_token::{
    instruction::{ freeze_account, initialize_mint2, mint_to_checked, thaw_account },
    state::Mint,
...
async fn main() -> anyhow::Result<()> {
    ...
    match client.send_and_confirm_transaction(&transaction).await {
        ...
        Err(err) => eprintln!("Error thawing tokens: {}", err),
    }

    Ok(())
}

// Helper function to airdrop, create the mint account and ATA, mint tokens to the ATA and freeze ATA
async fn setup(
    client: &RpcClient,
    ...
    let mut transaction = Transaction::new_with_payer(
        &[create_mint_account_ix, initialize_mint_ix, create_ata_ix, mint_to_ix, freeze_ix],
        Some(&authority_keypair.pubkey())
    );
...
async fn main() -> Result<()> {
    ...
    // Generate keypairs to use for authority and mint
    let authority_keypair = Keypair::new(); // will also act as tx fee payer
    ...
    // Freeze tokens minted to the mint authority token account
    let transaction_signature = token.freeze(
    ...
    println!("Successfully froze mint authority tokens");
    println!("Transaction Signature: {}", transaction_signature);

    // Thaw the frozen token account
    let transaction_signature = token.thaw(
        &mint_auth_associated_token_acc, // source token account
        &authority_keypair.pubkey(), // authority pubkey
        &[&authority_keypair] // signer keypairs
    ).await?;

    println!("Successfully thawed frozen tokens");
    println!("Transaction Signature: {}", transaction_signature);
