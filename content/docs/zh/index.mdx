---
title: Solana 文档
seoTitle: 了解 Solana 区块链的工作原理
description:
  "Solana 是为大规模采用而设计的高性能区块链。了解 为什么 Solana
  是寻求构建可扩展区块链应用程序的开发者的首选。"
---

## 入门指南

<Cards>
  <Card title="快速入门" href="/docs/intro/quick-start" icon={<Rocket />}>
    直接在浏览器中构建您的第一个 Solana 程序
  </Card>

  <Card title="设置本地环境" href="/docs/intro/installation" icon={<Download />}>
    安装 Solana 开发所需的依赖项
  </Card>
</Cards>

### 部署您的第一个 Solana 应用程序

<Cards>
  <Card title="在 Vercel 上部署" href="https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2Fsolana-developers%2Fanchor-web3js-nextjs&root-directory=frontend&demo-title=Anchor%20Web3.js%20Next.js%20Demo&demo-description=An%20example%20deployment%20of%20the%20Anchor%20Web3.js%20Next.js%20project.&demo-url=https%3A%2F%2Fanchor-web3js-nextjs.vercel.app%2F&project-name=anchor-web3js-nextjs&repository-name=anchor-web3js-nextjs">
    在 Vercel 上部署您的第一个 Solana 应用程序
  </Card>

  <Card title="Vercel 预览" href="https://anchor-web3js-nextjs-jade.vercel.app/">
    尝试已部署的应用程序
  </Card>
</Cards>

部署您的第一个 Solana 应用程序 - 一个使用 Anchor 框架构建的简单计数器程序。这个示例涵盖了开始在 Solana 上构建所需的所有核心概念。查看源代码[这里](https://github.com/solana-developers/anchor-web3js-nextjs)。

<Accordions>
  <Accordion title="应用程序演示">
    <video
      width="100%"
      autoPlay
      loop
      muted
      playsInline
      style={{
borderRadius: "10px",
}}
    >
      <source src="/assets/docs/counter.mp4" type="video/mp4" />

      您的浏览器不支持视频标签。
    </video>
  </Accordion>
</Accordions>

## 尝试 Solana：玩 2048

在 Solana 上玩 2048，每一步都会发送一个交易。点击"开始游戏"即可获得一个已充值的开发网钱包并使用方向键进行游戏。如果在移动设备上，可以通过滑动来控制方块。

<Accordions>
  <Accordion title="玩 Solana 2048">
    <iframe
      src="https://solplay.de/solana-2048/"
      width="100%"
      height="500"
      style={{
      borderRadius: "12px",
      border: "2px solid rgba(0, 0, 0, 0.2)",
      maxWidth: "100%",
      minHeight: "300px",
      aspectRatio: "1"
    }}
      scrolling="no"
    />
  </Accordion>
</Accordions>

由 [Jonas](https://x.com/SolPlay_jonas)
开发，他来自 Solana 基金会的开发者关系团队。

## 开始学习

了解 Solana 开发的关键概念。

<Cards>
  <Card title="账户" href="/docs/core/accounts">
    Solana 如何存储数据
  </Card>

  <Card title="Solana 上的费用" href="/docs/core/fees">
    在 Solana 上发送交易的成本
  </Card>

  <Card title="交易" href="/docs/core/transactions">
    如何与 Solana 网络交互
  </Card>

  <Card title="程序" href="/docs/core/programs">
    Solana上的智能合约
  </Card>

  <Card title="程序派生地址" href="/docs/core/pda">
    如何生成确定性地址
  </Card>

  <Card title="跨程序调用" href="/docs/core/cpi">
    如何从一个程序调用另一个程序
  </Card>
</Cards>

### 客户端开发

如果您在客户端进行开发，以下社区贡献的SDK可以帮助您使用流行语言与网络交互：

| 语言         | SDK                                                                                                                                     |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| Rust       | [solana\_sdk](https://docs.rs/solana-sdk/latest/solana_sdk/)                                                                            |
| Typescript | [@solana/web3.js](https://github.com/anza-xyz/solana-web3.js)                                                                           |
| Python     | [solders](https://github.com/kevinheavey/solders)                                                                                       |
| Java       | [sava](https://sava.software) 或 [solanaj](https://github.com/skynetcap/solanaj) 或 [solana4j](https://github.com/LMAX-Exchange/solana4j) |
| C++        | [solcpp](https://github.com/mschneider/solcpp) 或 [solana-c](https://github.com/VAR-META-Tech/solana-c-sdk)                              |
| Go         | [solana-go](https://github.com/gagliardetto/solana-go)                                                                                  |
| Kotlin     | [sol4k](https://github.com/sol4k/sol4k) 或 [solanaKT](https://github.com/metaplex-foundation/SolanaKT)                                   |
| Dart       | [solana](https://github.com/espresso-cash/espresso-cash-public/tree/master/packages/solana)                                             |
| C#         | [solnet](https://github.com/bmresearch/Solnet)                                                                                          |
| GdScript   | [godot](https://github.com/Virus-Axel/godot-solana-sdk/)                                                                                |

## 运行验证节点

了解运行Solana验证节点并帮助保障网络安全所需的条件。

<Cards>
  <Card title="验证节点" href="https://docs.anza.xyz/validator/anatomy">
    保障Solana网络安全的独立节点
  </Card>

  <Card title="系统要求" href="https://docs.anza.xyz/operations/requirements">
    运行验证节点的推荐硬件要求和所需SOL
  </Card>

  <Card title="验证节点设置" href="https://docs.anza.xyz/operations/setup-a-validator">
    设置验证节点并首次连接到集群
  </Card>
</Cards>

## 获取支持

从Solana社区获取帮助 [Solana StackExchange](https://solana.stackexchange.com)。
