...
请注意，bump seed
253-251 都可以生成具有不同地址的有效 PDA。这意味着，给定相同的可选种子和
`programId`，具有不同值的 bump seed 仍然可以生成有效的 PDA。
...
该程序包含一个 `initialize`
指令，用于创建一个新账户，并使用 PDA 作为该账户的地址。新账户存储 `user`
的地址以及用于生成 PDA 的 `bump` 种子。
...
在此示例中，用于 PDA 生成的种子包括固定字符串 `data` 和指令中提供的 `user`
...
`init` 约束指示 Anchor 调用系统程序（System
Program）以使用 PDA 作为地址创建一个新账户。Anchor 通过 [CPI](/docs/core/cpi)
实现这一操作。
...
指令，以使用 PDA 作为地址创建一个新的链上账户。在此示例中，Anchor 可以在指令账户中推断出 PDA 地址，因此无需显式提供。
...
请注意，在此示例中，如果使用相同的 `user` 地址作为种子多次调用 `initialize`
指令，则交易会失败。这是因为在生成的地址处已经存在一个账户。
