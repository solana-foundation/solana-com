...
为了简化理解，可以将程序账户视为程序本身。当调用程序的指令时，需要指定程序账户的地址（通常称为“Program
ID”）。

以下示例获取了 Token Program 账户，以展示程序账户具有相同的基础 `Account`
类型，除了 `executable` 字段被设置为
`true`。由于程序账户在其数据字段中包含可执行代码，因此我们不会对数据进行反序列化。
...
当部署一个 Solana 程序时，它会存储在一个程序账户中。程序账户由
[Loader Program](/docs/core/programs#loader-programs)
拥有。加载器有多个版本，但除了 loader-v3 之外，所有版本都将可执行代码直接存储在程序账户中。loader-v3 将可执行代码存储在一个单独的“程序数据账户”中，而程序账户仅指向它。当部署新程序时，Solana
CLI 默认使用最新的加载器版本。
...
Loader-v3 在部署或升级程序时，有一种用于临时存储程序上传的特殊账户类型。在 loader-v4 中，仍然存在缓冲区，但它们只是普通的程序账户。

### 程序数据账户

Loader-v3 的工作方式与其他所有 BPF
Loader 程序不同。程序账户仅包含程序数据账户的地址，而程序数据账户存储实际的可执行代码：
