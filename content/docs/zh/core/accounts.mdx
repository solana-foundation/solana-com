...
账户的余额以 lamports 表示，lamports 是 SOL 的最小单位（1 SOL =
10 亿 lamports）。账户的 SOL 余额是 `lamports` 字段中的金额转换为 SOL 的值。

Solana 账户必须保持一个与账户中存储的数据量（以字节为单位）成比例的最低 lamports 余额。这个最低余额被称为“租金”。

当账户关闭时，存储在账户中的 lamports 余额可以完全回收。
...
一个字节数组，用于存储账户的任意数据。数据字段通常被称为“账户数据”。
...
1. 使用账户的地址（公钥）获取账户
2. 将账户的数据字段从原始字节反序列化为适当的数据结构，该数据结构由拥有该账户的程序定义
...
- 如果 `false`，该账户是一个存储状态数据的账户。

对于可执行账户，`owner`
字段包含加载程序的程序 ID。加载程序是内置程序，负责加载和管理可执行程序账户。
...
为了在链上存储数据，账户还必须保持与账户中存储的数据量（以字节为单位）成比例的 lamport（SOL）余额。这个余额被称为“租金”，但它更像是一种押金，因为当您关闭账户时可以取回全额金额。您可以在[这里](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/rent/src/lib.rs#L93-L97)使用这些[常量](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/rent/src/lib.rs#L47-L70)找到计算方法。

“租金”一词来源于一种已弃用的机制，该机制会定期从低于租金阈值的账户中扣除 lamports。这种机制现已不再活跃。
...
请注意，Solana 上的所有“钱包”账户都是由 System
Program 所有的“系统账户”。这些账户中的 lamport 余额显示了钱包拥有的 SOL 数量。只有系统账户可以支付交易费用。

![系统账户](/assets/docs/core/accounts/system-account.svg)

当 SOL 第一次发送到一个新地址时，会在该地址自动创建一个由 System
Program 所有的账户。

在下面的示例中，生成了一个新的 keypair 并用 SOL 进行了资助。运行代码查看输出。请注意，账户的
...
Sysvar 账户是位于预定义地址的特殊账户，用于提供访问集群状态数据的功能。这些账户会动态更新网络集群的相关数据。您可以在[此处](https://docs.anza.xyz/runtime/sysvars)找到 Sysvar 账户的完整列表。
...
account 存储程序的可执行代码。program
account 由[Loader Program](/docs/core/programs#loader-programs)拥有。
...
account 的数据字段中包含可执行代码，我们不会对数据进行反序列化。
...
account 由[Loader Program](/docs/core/programs#loader-programs)拥有。Loader 有多个版本，但除了 loader-v3 以外，所有版本都将可执行代码直接存储在 program
account 中。loader-v3 则将可执行代码存储在一个单独的 "program data
account" 中，而 program account 仅指向它。当您部署一个新程序时，Solana
...
Loader-v3 为在部署或升级期间临时上传程序提供了一种特殊的账户类型。在 Loader-v4 中，仍然存在缓冲区，但它们只是普通的 program
account。

### 程序数据账户

Loader-v3 的工作方式与所有其他 BPF Loader 程序不同。program
account 仅包含一个程序数据账户的地址，该账户存储实际的可执行代码：
...
在 Solana 上，程序的可执行代码存储在与程序状态不同的账户中。这类似于操作系统通常将程序文件和数据文件分开存储的方式。

为了维护状态，程序定义了指令来创建它们拥有的独立账户。每个账户都有自己唯一的地址，并可以存储程序定义的任意数据。

![数据账户](/assets/docs/core/accounts/data-account.svg)

请注意，只有 [System Program](/docs/core/accounts#system-program)
可以创建新账户。一旦 System
Program 创建了一个账户，它可以将新账户的所有权分配给另一个程序。
...
以下示例展示了如何创建和获取由 Token 2022 程序拥有的 Token Mint account。
