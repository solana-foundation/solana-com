...
- `data`：存储账户任意数据的字节数组。对于非可执行账户，这通常存储旨在只读的状态。对于程序账户（智能合约），这包含可执行的程序代码。数据字段通常被称为"账户数据"。
- `executable`：这个布尔标志用于指示账户是否为程序。
- `lamports`：账户余额，以lamport为单位，lamport是SOL的最小单位（1 SOL =
  10亿lamport）。
...
要在链上存储数据，账户必须维持一个与账户存储数据量（以字节为单位）成比例的最低lamport（SOL）余额。这个最低余额被称为"rent"，尽管它的功能更像是一种押金，因为当账户关闭时可以完全收回这笔金额。你可以在这里找到计算方法
...
"rent"这个术语源于一个已废弃的机制，该机制会定期从低于rent阈值的账户中扣除lamports。这个机制现在已不再活跃。
...
Sysvar 账户是位于预定义地址的特殊账户，提供对集群状态数据的访问。这些账户会随着网络集群的数据动态更新。您可以在[这里](https://docs.anza.xyz/runtime/sysvars)找到 Sysvar 账户的完整列表。
...
Loader-v3 有一种特殊的账户类型，用于在部署或重新部署/升级期间临时存放程序上传。在 loader-v4 中仍然有缓冲区，但它们只是普通的程序账户。
...
在 Solana 上，程序的可执行代码存储在与程序状态不同的账户中。这类似于操作系统通常为程序本身及其数据使用单独文件的方式。

为了维护状态，程序定义了创建由程序拥有的单独账户的指令。每个这样的账户都有自己唯一的地址，并且可以存储程序定义的任何任意数据。
...
2. 调用现在拥有该账户的自定义程序，然后初始化账户数据，按照程序指令中的定义
