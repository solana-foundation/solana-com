...
函数处理不需要 PDA 签名者的 CPI。该函数使用空的 `signers_seeds` 数组调用
`invoke_signed` 函数，表示不需要 PDA 签名。
...
- 示例 2：使用 `solana_program` crate 中的 _rs`system_instruction::transfer`_
  函数构建 CPI 指令。示例 1 抽象了此实现。
- 示例 3：手动构建 CPI 指令。这种方法在没有可用 crate 帮助构建所需指令时非常有用。
...
[`create_program_address`](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/bpf_loader/src/syscalls/cpi.rs#L552)，使用
`signers_seeds` 和调用程序的
`program_id`。当验证了有效的 PDA 后，该地址会被[添加为有效签名者](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/bpf_loader/src/syscalls/cpi.rs#L554)。
