...
函数处理不需要 PDA 签名者的 CPI。该函数调用 `invoke_signed` 函数，并传递一个空的
`signers_seeds` 数组，表示不需要 PDA 进行签名。
...
- 示例 1：使用 Anchor 的 _rs`CpiContext`_ 和辅助函数来构建 CPI 指令。
...
`signers_seeds` 和 `program_id`。当验证了有效的 PDA 后，该地址会被
