...
description:
  ...
  其他程序的指令，处理PDA签名者，并在Solana网络上组合功能。
h1: 跨程序调用（CPI）
---

跨程序调用（CPI）是指一个程序调用另一个程序的指令。这允许Solana程序的组合性。
...
- **签名者权限**从调用程序扩展到被调用程序。
- 在进行跨程序调用时，**程序可以代表其自身程序ID派生的PDA进行签名**。
- 被调用程序可以进一步对其他程序进行CPI调用，最多深度为4。

## 什么是CPI？

跨程序调用（CPI）是指一个程序调用另一个程序的指令。

使用CPI编写程序指令遵循与构建一个[instruction](/docs/core/transactions#instruction)以添加到交易相同的模式。
在内部，每个CPI指令必须指定：
...
- 初始交易的签名者权限扩展到被调用程序（例如A->B）
- 被调用程序可以进一步对其他程序进行CPI调用，最多深度为4（例如B->C，C->D）
- 程序可以代表其程序ID派生的[PDA](/docs/core/pda)进行“签名”
...
<Callout>
  ...
  为5。这表示程序指令调用堆栈的最大高度。堆栈高度从初始交易的1开始，每次程序调用另一个指令时增加1。此设置将CPI的调用深度限制为4。
</Callout>

当交易被处理时，账户权限从一个程序扩展到另一个程序。这意味着什么呢？
...
- 程序B可以使用签名账户进行签名
- 程序B可以写入可写账户
- 如果程序B进行自己的CPI调用，它甚至可以将这些相同的权限向前传递

## 跨程序调用

[`invoke`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/program.rs#L26-L28)
函数处理不需要PDA签名者的CPI。该函数调用`invoke_signed`函数，使用空的`signers_seeds`数组，表示不需要PDA进行签名。
...
以下示例展示了如何使用[Anchor框架](https://www.anchor-lang.com/docs)和原生Rust进行CPI调用。示例程序包括一个单一指令，通过CPI将SOL从一个账户转移到另一个账户。

### Anchor框架

以下示例展示了在Anchor程序中创建跨程序调用（CPI）的三种方法，每种方法具有不同的抽象级别。所有示例的工作方式相同。主要目的是展示CPI的实现细节。

- 示例1：使用Anchor的_rs`CpiContext`_和辅助函数构造CPI指令。
- 示例2：使用来自`solana_program` crate的_rs`system_instruction::transfer`_函数构造CPI指令。示例1对该实现进行了抽象。
- 示例3：手动构造CPI指令。当没有crate可用于帮助构建所需调用的指令时，此方法非常有用。
...
以下示例展示了如何从用原生Rust编写的程序中进行CPI调用。该程序包括一个单一指令，通过CPI将SOL从一个账户转移到另一个账户。测试文件使用[LiteSVM](https://github.com/LiteSVM/litesvm)来测试程序。
...
test("sol转账cpi", () => {
  ...
  const instructionIndex = 0; // 指令索引0对应SolTransfer枚举

  const data = Buffer.alloc(9); // 1字节用于指令枚举 + 8字节用于u64
  data.writeUInt8(instructionIndex, 0); // 第一个字节标识指令
  data.writeBigUInt64LE(transferAmount, 1); // 剩余字节为指令参数
...
你可以参考[程序派生地址](/docs/core/pda)页面，了解如何派生PDA。
...
在处理包含CPI的指令时，Solana运行时内部调用
[`create_program_address`](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/bpf_loader/src/syscalls/cpi.rs#L552)
使用`signers_seeds`和调用程序的`program_id`。当一个有效的PDA被验证时，该地址将
[添加为有效签名者](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/bpf_loader/src/syscalls/cpi.rs#L554)。

以下示例展示了如何使用带PDA签名者的CPI，使用[Anchor框架](https://www.anchor-lang.com/docs)和原生Rust。示例程序包括一个单一指令，通过CPI将SOL从PDA转移到接收账户，使用PDA进行签名。

### Anchor框架

以下示例包括在Anchor程序中实现跨程序调用（CPI）的三种方法，每种方法具有不同的抽象级别。所有示例在功能上是等效的。主要目的是说明CPI的实现细节。

- 示例1：使用Anchor的_rs`CpiContext`_和辅助函数构造CPI指令。
- 示例2：使用来自`solana_program` crate的_rs`system_instruction::transfer`_函数构造CPI指令。示例1是该实现的抽象。
- 示例3：手动构造CPI指令。当没有crate可用于帮助构建所需调用的指令时，此方法非常有用。
...
以下示例展示了如何从用原生Rust编写的程序中进行带PDA签名者的CPI调用。该程序包括一个单一指令，通过CPI将SOL从PDA转移到接收账户，使用PDA进行签名。测试文件使用[LiteSVM](https://github.com/LiteSVM/litesvm)来测试程序。
