...
发送方账户必须签署 (`is_signer`) 交易，以便系统程序扣除其 lamport 余额。由于 lamport 余额会发生变化，发送方和接收方账户必须是可写的 (`is_writable`)。
...
RPC 方法获取当前区块哈希和区块哈希有效的最后区块高度。以下是在
...
- `header`：指定 `accountKeys` 数组中地址的读/写和签名权限
...
- `signatures`：包含所有指令中要求作为签名者的账户签名的数组。签名是通过使用账户对应的私钥签署交易消息生成的。
