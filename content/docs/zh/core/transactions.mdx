...
交易本质上是一个处理一个或多个指令的请求。你可以将交易想象成一个装有表单的信封。每个表单都是一条指令，告诉网络需要执行的操作。发送交易就像邮寄信封以处理表单。
...
- 交易是**原子性**的——如果任何指令失败，整个交易将失败且不会发生任何更改。
- 交易中的指令按顺序执行。
- 交易的大小限制为
...
下图表示一个包含单条指令的交易，用于将 SOL 从发送方转移到接收方。

在 Solana 上，“钱包”是由 [System Program](/docs/core/accounts#system-program)
拥有的账户。只有程序所有者可以更改账户的数据，因此转移 SOL 需要发送交易以调用 System
Program。

![SOL 转账](/assets/docs/core/transactions/sol-transfer.svg)

发送方账户必须签署 (`is_signer`) 交易，以允许 System
Program 扣除其 lamport 余额。由于 lamport 余额会发生变化，发送方和接收方账户必须是可写的 (`is_writable`)。

发送交易后，System Program 会处理转账指令。然后，System
Program 更新发送方和接收方账户的 lamport 余额。

![SOL 转账流程](/assets/docs/core/transactions/sol-transfer-process.svg)

以下示例展示了如何发送一笔将 SOL 从一个账户转移到另一个账户的交易。查看 System
Program 的转账指令源代码
[这里](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L183)。
...
以下示例展示了如何手动构建转账指令。 `Expanded Instruction` 选项卡在功能上等同于
`Instruction` 选项卡。
...
在以下部分中，我们将逐步讲解交易和指令的细节。

## 指令

在 Solana
[程序](/docs/core/accounts#program-account)中，[指令](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/instruction/src/lib.rs#L94)可以被视为一个公共函数，任何使用 Solana 网络的人都可以调用。

您可以将 Solana 程序想象成托管在 Solana 网络上的一个 Web 服务器，其中每个指令就像一个公共 API 端点，用户可以调用它来执行特定操作。调用指令类似于向 API 端点发送
`POST` 请求，允许用户执行程序的业务逻辑。

<WithMentions>

要在 Solana 上调用程序的指令，您需要构建一个
`Instruction`，其中包含以下三部分信息：

- [程序 ID](mention:program-id)：包含被调用指令业务逻辑的程序地址。
- [账户](mention:accounts)：指令读取或写入的所有账户的列表。
- [指令数据](mention:instruction-data)：一个字节数组，指定要在程序上调用的指令以及指令所需的任何参数。
...
在创建 `Instruction` 时，您必须为每个所需账户提供一个
[`AccountMeta`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/instruction/src/account_meta.rs#L25)。
`AccountMeta` 指定以下内容：

- [公钥](mention:pubkey)：账户的地址
- [是否签名者](mention:is-signer)：账户是否必须签署交易
- [是否可写](mention:is-writable)：指令是否修改账户的数据
...
要了解指令需要哪些账户，包括哪些必须是可写的、只读的或签署交易的，您必须参考程序定义的指令实现。
...
以下示例展示了前面代码片段的输出。具体格式因 SDK 而异，但每个 Solana 指令都需要以下信息：

<WithMentions>

- [程序 ID](mention:program-id)：将执行指令的程序地址。
- [账户](mention:accounts)：指令所需的账户列表。对于每个账户，指令必须指定其地址、是否需要签署交易，以及是否会被写入。
- [数据](mention:data)：一个字节缓冲区，用于告诉程序要执行的指令以及指令所需的任何参数。
...
1. [签名](mention:signatures)：一个包含所有指令所需签名账户的
   [签名](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/signature/src/lib.rs#L30)
   数组。签名是通过使用账户的私钥对交易 `Message` 进行签署生成的。
2. [消息](mention:message)：交易的
   [消息](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/message/legacy.rs#L131)
   包括要原子处理的指令列表。
...
- [账户地址](mention:account-addresses)：指令所需的账户地址数组。
...
指定了交易中账户的权限。它与严格排序的
[账户地址](/docs/core/transactions#array-of-account-addresses)
结合使用，以确定哪些账户是签名者，哪些账户是可写的。
...
此格式用于编码交易消息中
[账户地址](/docs/core/transactions#array-of-account-addresses)
和[指令](/docs/core/transactions#array-of-instructions)数组的长度。

### 账户地址数组

交易消息包含一个单一的列表，列出其指令所需的所有[账户地址](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/message/legacy.rs#L138)。该数组以一个[compact-u16](/docs/core/transactions#compact-array-format)数字开头，指示其包含的地址数量。
...
RPC 方法获取当前区块哈希以及区块哈希有效的最后区块高度。

### 指令数组

交易消息包含一个[指令](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/message/legacy.rs#L146)数组，类型为
[CompiledInstruction](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/instruction.rs#L22)。指令在添加到交易时会被转换为此类型。

与消息中的账户地址数组类似，它以一个[compact-u16](/docs/core/transactions#compact-array-format)长度开头，后跟指令数据。每个指令包含：

<WithMentions>

1. [程序 ID 索引](mention:program-id-index)：一个索引，指向账户地址数组中程序的地址。这指定了处理指令的程序。
2. [账户索引](mention:account-indexes)：一个索引数组，指向此指令所需的账户地址。
3. [指令数据](mention:instruction-data)：一个字节数组，指定要在程序上调用的指令以及指令所需的任何附加数据（例如函数参数）。
