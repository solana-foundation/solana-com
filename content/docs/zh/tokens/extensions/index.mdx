...
<Callout type="info">
  某些扩展彼此不兼容，无法在同一个 token 铸造或 token account
  上同时启用。例如，_rs`NonTransferable`_ 扩展无法与 _rs`TransferFeeConfig`_
  扩展结合使用，因为它们的行为存在冲突。
</Callout>
...
每个扩展通过向铸造或 token
account 添加额外状态来提供专门的功能。所有扩展特定的状态都存储在
...
字段中，该字段遵循基础账户数据类型。您必须根据为该账户启用的特定扩展类型进一步反序列化
`tlv_data`（包含扩展状态）。
