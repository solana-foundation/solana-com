...
- 用户应输入被解释为缩放后的“UIAmount”的金额。需要处理此金额的应用程序应将其转换为交易所需的原始 token 金额。
  ...
  - 要进行此转换，您可以使用
    ...
    （web3.js v1）函数或通过模拟交易使用 amountToUiAmount。
...
- 可以随时通过调用 `getAccountInfo`
  从代币铸造中读取当前乘数。此外，如果设置了未来的乘数，这些信息也可以从代币铸造中获取。我们建议不要显示此乘数，因为这可能会让用户体验感到困惑。
...
- 提供历史数据的服务应存储并**展示扩展的缩放 UI 金额的缩放价格和非缩放价格**。
...
| 要求                    | 描述                                                      | 优先级 |
| :---------------------- | :-------------------------------------------------------- | :----- |
| 支持缩放数据的 API 更新 | 扩展 API 功能以包括随时间变化的乘数更新以及缩放价格数据。 | **P0** |
...
| 价格数据适配             | 在任何使用价格数据展示当前价格的地方，向终端用户提供缩放价格。           | **P1** |
| 显示价格历史图表         | 在价格图表中反映缩放价格。                                               | **P1** |
