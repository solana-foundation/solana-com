...
| 指令                                | 描述                                                                                                                 |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | 设置用于保密转账的 mint account 。此指令必须与 _rs`TokenInstruction::InitializeMint`_ 指令包含在同一笔交易中。       |
...
| _rs`ConfigureAccountWithRegistry`_  | 使用 _rs`ElGamalRegistry`_ 账户而不是 _rs`VerifyPubkeyValidity`_ 证明，配置 token account 以进行保密转账的替代方法。 |
