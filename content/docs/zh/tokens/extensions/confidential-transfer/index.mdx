---
title: 保密转账
description: 了解保密转账扩展及其如何为代币 mint 和账户添加可选功能。
---
...
保密转账扩展为 Token Extension 程序添加了
[指令](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)，允许您在账户之间转移代币而不透露转账金额。
...
<Callout type="info">
  在撰写本文时，默认的本地 validator 尚未启用保密转账功能。您必须克隆主网的
...
| 指令                                                               | 描述                                                                                                                                                                                                    |
| ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_ | 设置用于保密转账的 mint account。此指令必须与 _rs`TokenInstruction::InitializeMint`_ 指令在同一事务中包含。                                                                 |
| _rs`UpdateMint`_ | 更新 mint 的保密转账设置。                                                                                                                                                                              |
| _rs`ConfigureAccount`_ | 设置用于保密转账的 token account。                                                                                                                                                                      |
| _rs`ApproveAccount`_ | 如果 mint 要求新 token account 获得批准，则批准该 token account 进行保密转账。                                                                                                                          |
| _rs`EmptyAccount`_ | 清空待处理和可用的保密余额以关闭 token account。                                                                                                                                                        |
| _rs`Deposit`_ | 将公共代币余额转换为待处理的保密余额。                                                                                                                                                                  |
| _rs`Withdraw`_ | 将可用的保密余额转换回公共余额。                                                                                                                                                                        |
| _rs`Transfer`_ | 在 token account 之间进行保密代币转账。                                                                                                                                                                 |
...
| _rs`TransferWithFee`_ | 在 token account 之间进行保密代币转账并收取费用。                                                                                                                                                       |
| _rs`ConfigureAccountWithRegistry`_ | 使用 _rs`ElGamalRegistry`_ 账户而不是 _rs`VerifyPubkeyValidity`_ 证明来配置 token account 进行保密转账的替代方法。 |
