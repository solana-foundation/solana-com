...
1. 客户端生成
   ...
   **等式证明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**：验证提取后剩余的可用余额密文是否与其对应的
   [Pedersen 承诺](https://en.wikipedia.org/wiki/Commitment_scheme)匹配，确保账户的新可用余额正确计算为 `remaining_balance = current_balance - withdraw_amount`。
...
Program 的本地 validator。在启动本地 validator 之前，您必须安装 Solana CLI。
...
<Callout type="info">
  ...
  上启用。您必须克隆主网的 Token Extension Program 才能运行示例代码。
</Callout>
