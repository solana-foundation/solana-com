...
1. **保密待处理余额**：首先，代币从公共余额“存入”到“待处理”的保密余额中。
2. **保密可用余额**：然后将待处理余额“应用”到可用余额中，使代币可用于保密转账。

本节将解释第二阶段：将待处理余额应用到可用余额。

当代币从公共余额“存入”或从一个代币账户保密转账到另一个代币账户时，这些代币最初会被添加到保密待处理余额中。在代币可以用于保密转账之前，必须将待处理余额“应用”到可用余额。
...
`confidential_transfer_apply_pending_balance` 方法，该方法构建并发送包含
...
---CODE-PLACEHOLDER-2e2b8c3b5b7c4b6e8f9d3a1c2d4e5f6a---
