---
title: 转移代币
description: 了解如何将代币从一个代币账户私密地转移到另一个代币账户。
---

## 如何将代币从一个代币账户私密地转移到另一个代币账户

要将代币私密地从一个代币账户转移到另一个代币账户，发送方和接收方都必须配置了
_rs`ConfidentialTransferAccount`_
状态并已批准进行私密转移的代币账户。发送方的代币账户还必须有可用的私密余额以进行转移。
...
1. 在客户端创建
   ...
   **等式证明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**：验证转移后新的可用余额密文是否与其对应的
   [Pedersen 承诺](https://en.wikipedia.org/wiki/Commitment_scheme)匹配，确保源账户的新可用余额正确计算为
   `new_balance = current_balance - transfer_amount`。

   **密文有效性证明 ([BatchedGroupedCiphertext3HandlesValidityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_grouped_ciphertext_validity/handles_3.rs#L63))**：验证转移金额密文是否为所有三方（源账户、目标账户和审计方）正确生成，确保转移金额在每方的公钥下正确加密。

   **范围证明 ([BatchedRangeProofU128Data](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_range_proof/batched_range_proof_u128.rs#L37))**：验证新的可用余额和转移金额（分为低位/高位）均为非负数且在指定范围内。
...
下图显示了从发送方的代币账户向接收方的代币账户转移代币的步骤。
...
- 调用 Zk ElGamal 证明程序以验证这些证明并初始化 "上下文状态"账户
- 调用
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  指令，提供三个证明账户。
- 关闭这三个证明账户以回收租金。
...
要运行此示例，请使用以下命令启动一个从主网克隆的包含 Token Extension
...
---CODE-PLACEHOLDER-1c2b8f7e3e5a4b6a9f8d7c9e0a7f3e2b---
