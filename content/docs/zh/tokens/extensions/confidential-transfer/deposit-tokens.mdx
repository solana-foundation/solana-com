...
`spl_token_client`
crate 提供了一个 `confidential_transfer_deposit` 方法，该方法构建并发送包含 `Deposit` 指令的交易，如下例所示。
...
<Callout type="info">
  在撰写本文时，保密转账功能尚未在默认的本地验证器上启用。 您必须克隆主网的
