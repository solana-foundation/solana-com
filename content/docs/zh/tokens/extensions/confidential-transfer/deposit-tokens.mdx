---
title: 存入代币
description: 了解如何将代币存入机密状态。
---

## 如何将代币存入机密待处理余额

在代币可以进行机密转账之前，公开代币余额必须转换为机密余额。这种转换分为两个阶段：

1. **机密待处理余额**：最初，代币从公开余额"存入"到机密"待处理"余额。
2. **机密可用余额**：然后将待处理余额"应用"到可用余额，使代币可用于机密转账。

本节解释第一阶段：将公开代币余额存入机密待处理余额。

下图显示了将代币从公开余额存入机密待处理余额所涉及的步骤：
...
sequenceDiagram
    ...
    Note over TokenAccount: 具有公开余额的代币账户

    Owner->>Token22: confidential_transfer_deposit()
    activate Token22
    Token22-->>TokenAccount: 将公开余额转换为 <br> 机密待处理余额
    deactivate Token22

    Note over TokenAccount: 待处理余额必须应用到 <br> 可用余额才能使用
```

### 所需指令

要将公开余额转换为机密待处理余额，请调用
...
指令。每个存款指令的
[最大金额](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/mod.rs#L21)
限制为2^48。

`spl_token_client` crate提供了 `confidential_transfer_deposit`
方法，用于构建和发送带有 `Deposit` 指令的交易，如下例所示。

### 示例代码

以下示例演示了如何将公开代币余额存入机密待处理余额。

要运行示例，请使用以下命令从主网克隆Token Extension
Program启动本地验证器。您必须安装Solana CLI才能启动本地验证器。
...
<Callout type="info">
  在撰写本文时，默认本地验证器上尚未启用机密传输功能。您必须克隆主网上的Token
...
<CodeTabs flags="r">
  ...
  #[tokio::main] async fn main() -> Result<()> { // Create connection to local
  test validator let rpc_client = Arc::new(RpcClient::new_with_commitment(
  ...
  (~/.config/solana/id.json) // This enables using the same wallet as the Solana
  CLI tools fn load_keypair() -> Result<Keypair> { // Get the default keypair path
  let keypair_path = dirs::home_dir() .context("Could not find home directory")?
