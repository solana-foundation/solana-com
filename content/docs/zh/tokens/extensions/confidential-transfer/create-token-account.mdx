...
The *rs`ConfidentialTransferAccount`* contains several fields to manage
confidential transfers:

* **approved**: The account's approval status for confidential transfers. If the
  mint account's
  `auto_approve_new_accounts`配置设置为`true`，所有代币账户都会自动获得机密转账的批准。

* **elgamal\_pubkey**: The ElGamal public key used to encrypt balances and
  transfer amounts.

* **pending\_balance\_lo**: The encrypted lower 16 bits of the pending balance.
  The balance is split into high and low parts for efficient decryption.

* **pending\_balance\_hi**：加密的待处理余额的高48位。余额被分为高位和低位部分以提高解密效率。

* **available\_balance**：可用于转账的加密余额。

* **decryptable\_available\_balance**：使用高级加密标准（AES）密钥加密的可用余额，便于账户所有者高效解密。

* **allow\_confidential\_credits**：如果为true，允许接收机密转账。

* **allow\_non\_confidential\_credits**：如果为true，允许接收非机密转账。

* **pending\_balance\_credit\_counter**: 计算来自存款和转账指令的待处理余额入账数量。

* **maximum\_pending\_balance\_credit\_counter**: 在需要执行*rs`ApplyPendingBalance`*&#x6307;令将待处理余额转换为可用余额之前，待处理入账的数量限制。

* **expected\_pending\_balance\_credit\_counter**: 上次处理`pending_balance_credit_counter`值，该值由客户端通过指令数据在上次处理*rs`ApplyPendingBalance`*&#x6307;令时提供。

* **actual\_pending\_balance\_credit\_counter**: 在上次处理`pending_balance_credit_counter`值，该值是在处理最后一个*rs`ApplyPendingBalance`*&#x6307;令时代币账户上的值。
...
Token account owners must use the *rs`ApplyPendingBalance`* instruction to
...
which is used in the *rs`ApplyPendingBalance`* instruction
...
When calling the *rs`ApplyPendingBalance`* instruction to convert the pending
...
1. **Create the Token Account**: Invoke the Associated Token Program's
   *rs`AssociatedTokenAccountInstruction:Create`* instruction to create the
   token account.

2. **Reallocate Account Space**: Invoke the Token Extension Program's
   *rs`TokenInstruction::Reallocate`* instruction to add space for the
   *rs`ConfidentialTransferAccount`* state.
...
3. **Configure Confidential Transfers**: Invoke the Token Extension Program's
   ...
   instruction to initialize the *rs`ConfidentialTransferAccount`* state.
...
The *rs`ConfigureAccount`* instruction requires client-side generation of
encryption keys and proof data that can only be generated by the token account
owner.

The *rs`PubkeyValidityProofData`* creates a proof that verifies an ElGamal key
