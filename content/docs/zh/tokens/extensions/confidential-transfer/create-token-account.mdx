...
description: 了解如何使用机密转账扩展创建一个 token account。
---

## 如何使用机密转账扩展创建一个 token account

机密转账扩展通过向 token
account 添加额外状态，实现了私密的代币转账。本节将解释如何启用此扩展来创建一个 token
account。

下图展示了使用机密转账扩展创建 token account 的步骤：
...
- **pending_balance_credit_counter**：记录存款和转账指令中待处理余额的计数。

- **maximum_pending_balance_credit_counter**：待处理余额计数的上限，在需要
  _rs`ApplyPendingBalance`_ 指令将待处理余额转换为可用余额之前的计数限制。

- **expected_pending_balance_credit_counter**：客户端通过指令数据提供的
  `pending_balance_credit_counter` 值，上次处理 _rs`ApplyPendingBalance`_
  指令时的值。
...
机密余额分为待处理余额和可用余额，以防止 DoS 攻击。如果没有这种区分，攻击者可以反复向代币账户发送代币，阻止代币账户所有者转移代币的能力。由于在交易提交和处理之间加密余额会发生变化，导致交易失败，代币账户所有者将无法转移代币。

所有存款和转账金额最初都会添加到待处理余额中。代币账户所有者必须使用
_rs`ApplyPendingBalance`_
指令将待处理余额转换为可用余额。传入的转账或存款不会影响代币账户的可用余额。
...
机密待处理余额被分为 `pending_balance_lo` 和
...
_rs`ApplyPendingBalance`_
指令中[使用](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L1180-L1184)。
...
当调用 _rs`ApplyPendingBalance`_ 指令将待处理余额转换为可用余额时：

1. 客户端查找当前的待处理余额和可用余额，加密总和，并提供一个使用代币账户所有者的 AES 密钥加密的
   `decryptable_available_balance`。

2. 预期和实际的待处理信用计数器跟踪在创建和处理 _rs`ApplyPendingBalance`_
   指令之间计数器值的变化：
   - `expected_pending_balance_credit_counter`：客户端创建
     _rs`ApplyPendingBalance`_ 指令时的 `pending_balance_credit_counter` 值
   - `actual_pending_balance_credit_counter`：在处理 _rs`ApplyPendingBalance`_
     指令时，代币账户上的 `pending_balance_credit_counter` 值
...
1. 从 token account 的 `decryptable_available_balance` 开始
...
使用机密转账扩展创建一个 token account 需要三个指令：

1. **创建 Token Account**：调用 Associated Token Program 的
   _rs`AssociatedTokenAccountInstruction:Create`_ 指令以创建token account。
...
要运行示例，请使用以下命令启动一个从主网克隆的 Token Extension
...
<Callout type="info">
  在撰写本文时，Confidential Transfers
  尚未在默认的本地验证器上启用。您必须克隆主网 Token Extensions Program
  才能运行示例代码。
</Callout>
...
---CODE-PLACEHOLDER-2e2b8b5b7c3f4a1e9f3c8d9a1e5d7c6a---
