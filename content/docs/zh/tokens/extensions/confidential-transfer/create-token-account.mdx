...
- **approved**：账户对机密转账的批准状态。如果 mint account 的
  `auto_approve_new_accounts` 配置设置为 `true`，则所有 token
  account 会自动获得机密转账的批准。
...
- **maximum_pending_balance_credit_counter**：在需要 _rs`ApplyPendingBalance`_
  指令将待处理余额转换为可用余额之前，待处理信用的计数限制。
...
所有存款和转账金额最初都会添加到待处理余额中。token account 的所有者必须使用
...
`pending_balance_hi`，因为 ElGamal 解密对较大数字需要更多计算。您可以在[此处](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/confidential-transfer/ciphertext-arithmetic/src/lib.rs#L54)找到密文算术实现，它在[此处](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L1180-L1184)用于
_rs`ApplyPendingBalance`_ 指令。
...
`decryptable_available_balance` 进行对账：
...
1. **创建 Token Account**：调用 Associated Token Program 的
   _rs`AssociatedTokenAccountInstruction:Create`_ 指令来创建 token account。

2. **重新分配账户空间**：调用 Token Extension Program 的
   _rs`TokenInstruction::Reallocate`_ 指令为 _rs`ConfidentialTransferAccount`_
   状态添加空间。
...
3. **配置机密转账**：调用 Token Extension Program 的
   ...
   指令来初始化 _rs`ConfidentialTransferAccount`_ 状态。
...
创建一个证明，用于验证 ElGamal 密钥的有效性。有关实现细节，请参见：
