...
_rs`ConfidentialTransferMint`_
包含三个配置字段：
...
1. **创建 Mint Account**：调用 System Program 的
   _rs`CreateAccount`_
   指令来创建 mint account。

2. **初始化 Confidential Transfer 扩展**：调用 Token Extension Program 的
   ...
   指令来配置 mint 的
   _rs`ConfidentialTransferMint`_ 状态。
...
crate 提供了一个 `create_mint` 方法，可以在单个函数调用中构建并发送包含所有三个指令的交易，如以下示例所示。
