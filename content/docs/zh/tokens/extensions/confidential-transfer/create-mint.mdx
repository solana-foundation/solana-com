...
description: 了解如何使用 Confidential Transfer 扩展创建一个 token mint。
---

## 如何使用 Confidential Transfer 扩展创建一个 mint

Confidential Transfer 扩展通过向 mint
account 添加额外的状态，支持私密的 token 转账。本节将解释如何启用此扩展来创建一个 token
mint。
...
- **authority**：有权限更改 mint 的 Confidential
  Transfer 设置并在禁用自动审批时批准新的 confidential accounts 的账户。

- **auto_approve_new_accounts**：当设置为 true 时，用户可以默认启用 Confidential
  Transfer 创建 token
  accounts。当设置为 false 时，authority 必须批准每个新的 token
  account 后才能用于 Confidential Transfer。

- **auditor_elgamal_pubkey**：一个可选的审计员，可以解密 Confidential
  Transfer 交易中的转账金额，提供合规机制，同时保持对公众的隐私。

### 必需的指令

启用 Confidential Transfer 的 mint 创建需要在单个交易中包含三个指令：

1. **创建 Mint Account**：调用系统程序的 _rs`CreateAccount`_ 指令来创建 mint
   account。

2. **初始化机密传输扩展**：调用 Token Extensions Program 的
...
3. **初始化 Mint**：调用 Token Extensions Program 的
   _rs`Instruction::InitializeMint`_ 指令来初始化标准 mint 状态。

虽然您可以手动编写这些指令，但 `spl_token_client` crate 提供了一个 `create_mint`
方法，该方法通过单个函数调用构建并发送包含所有三个指令的交易，如下面的示例所示。
