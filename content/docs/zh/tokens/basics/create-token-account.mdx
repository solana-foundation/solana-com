...
<Callout type="info">
  请注意，在源代码中，开发者将 Token account 称为 `Account` 类型。 [Token
...
- USD Coin 的铸造地址：`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Circle（USD Coin 的发行方）在 `3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
  拥有一个 token account
- 此 token account 仅持有 USD Coin token（铸造）的单位
...
<Callout type="info">
  Associated Token Program 是一个辅助程序，用于创建具有确定性地址（PDA）的 token
  account。在创建 associated token account 时，Associated Token Program 会对
  Token Program 或 Token Extension Program 进行 CPI（Cross-Program
  Invocation）。创建的账户由 token program 拥有，并具有与 token program 中定义的
  `Account` 类型结构相同的结构。Associated Token Program 不维护任何状态 -
  它仅提供了一种标准化的方法，用于在确定性地址（PDA）创建 token account。
</Callout>
