...
2. 一个 keypair 文件（例如，`hello_world-keypair.json`）：在部署程序时，此 keypair 的公钥用作程序 ID。
...
在 `src/lib.rs` 中添加以下测试代码，放在程序代码的下方。这是一个调用 Hello
World 程序的测试模块。
...
打开一个新的终端并运行 `solana-test-validators` 命令以启动本地 validator。

```shell title="Terminal"
solana-test-validator
```

当测试 validator 正在运行时，在另一个终端中运行 `solana program deploy`
