...
中。如果您不包含此配置，在构建程序时将不会生成 `target/deploy` 目录。
...
在 Solana 程序中，`msg!` 宏用于将消息打印到程序日志中。
...
在程序代码下方，将以下测试添加到 `src/lib.rs` 中。这是一个调用 Hello
World 程序的测试模块。
...
将以下代码添加到 `examples/client.rs`
中。这是一个 Rust 客户端脚本，用于为新的 keypair 提供资金以支付交易费用，然后调用 Hello
World 程序。
...
您可以在
[Solana Explorer](https://explorer.solana.com/?cluster=custom&customUrl=http%3A%2F%2Flocalhost%3A8899)（本地集群）上检查交易签名，以在程序日志中看到 "Hello,
world!"。
