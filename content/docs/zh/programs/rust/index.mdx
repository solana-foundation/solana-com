...
导航到项目目录。你应该会看到默认的 `src/lib.rs` 和 `Cargo.toml` 文件。
...
然后，将以下代码片段添加到 `Cargo.toml`
文件中。如果不包含此配置，在构建程序时将不会生成 `target/deploy` 目录。
...
将以下测试添加到 `src/lib.rs` 文件中，位于程序代码的下方。这是一个调用 Hello
World 程序的测试模块。
...
打开一个新终端，运行 `solana-test-validators` 命令以启动本地验证器。

```shell title="Terminal"
solana-test-validator
```

当测试验证器正在运行时，在另一个终端中运行 `solana program deploy`
命令，将程序部署到本地验证器。
...
Explorer 上的集群也必须是 localhost。Solana Explorer 的 "Custom RPC
...
中更新程序，使其打印 "Hello, Solana!" 而不是 "Hello, world!"。
