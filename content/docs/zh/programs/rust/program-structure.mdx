...
为了简化操作，请使用来自 `solana_program` crate 的
...
要从我们程序的指令中调用系统程序，我们通过 `invoke`
函数进行跨程序调用（CPI）。CPI允许一个程序调用其他程序的指令——在本例中，是系统程序的
`create_account` 指令。
...
来执行测试。可选地，使用 `--nocapture` 标志以在输出中查看打印语句。
...
接下来，使用 `ProgramTest`
设置测试。然后创建一个新的 keypair 作为我们将初始化的计数器账户的地址，并定义一个初始值来设置计数器。
