...
用 Rust 编写的 Solana 程序对结构的要求极少，这使得代码组织方式具有很大的灵活性。唯一的要求是程序必须包含一个
...
为了简化，程序将实现为单个 `lib.rs`
文件，尽管在实际中，您可能希望将较大的程序拆分为多个文件。
...
文件中。如果您不包含此配置，在构建程序时将不会生成 `target/deploy` 目录。
...
宏定义程序中的入口点，该宏来自 `solana_program` crate。
...
- `accounts`：调用指令所需账户的 `AccountInfo`
...
将以下代码添加到 `lib.rs` 中。此代码使用上一步中定义的 `CounterInstruction`
...
为了定义指令所需的账户，我们创建一个迭代器来遍历 `accounts` 切片，并使用
`next_account_info` 函数获取每个账户。您定义的账户数量即为指令所需的账户。
...
一旦定义了空间并计算了 rent，就通过调用系统程序的 `create_account`
指令来创建账户。
...
要从我们程序的指令中调用系统程序，我们通过 `invoke` 函数进行 Cross Program
Invocation (CPI)。CPI 允许一个程序调用其他程序的指令——在本例中是系统程序的
`create_account` 指令。
...
1. 使用提供给指令的 `initial_value` 创建一个新的 `CounterAccount` 结构体。
2. 获取对新账户数据字段的可变引用。
3. 将 `CounterAccount` 结构体序列化到账户的数据字段中，从而将 `initial_value`
   存储在账户中。
...
函数类似，我们从创建账户的迭代器开始。在这种情况下，我们只期望一个账户，即需要更新的账户。
...
来执行测试。可选地，使用 `--nocapture` 标志查看输出中的打印语句。
...
请注意，增量指令的指令数据是
`[1]`，对应于变体 1（`IncrementCounter`）。由于增量指令没有其他参数，数据仅仅是指令变体。
