...
用 Rust 编写的 Solana 程序对结构的要求极少，允许代码组织方式具有灵活性。唯一的要求是程序必须有一个
`entrypoint`，它定义了程序开始执行的位置。
...
- `processor.rs`：定义实现每个指令业务逻辑的指令处理器（函数）。
...
为了简化起见，该程序将实现于一个单一的 `lib.rs`
文件中，尽管在实际中，您可能希望将较大的程序拆分为多个文件。
...
宏，该宏来自 `solana_program` crate，用于在程序中定义入口点。
...
将 `lib.rs` 中的默认代码替换为以下代码。此代码片段：

1. 从 `solana_program` 导入所需的依赖项
2. 使用 `entrypoint!` 宏定义程序入口点
3. 实现 `process_instruction` 函数，该函数将指令路由到相应的处理函数
...
当 Solana 程序被调用时，入口点会
[反序列化](https://github.com/solana-labs/solana/blob/v2.0/sdk/program/src/entrypoint.rs#L277)
[输入数据](https://github.com/solana-labs/solana/blob/v2.0/sdk/program/src/entrypoint.rs#L129-L131)（以字节形式提供）为三个值，并将它们传递给
...
在构建 Solana 程序时，通常会从定义程序的状态开始——即存储在由程序创建和拥有的账户中的数据。

程序状态是使用 Rust 结构体定义的，这些结构体表示程序账户的数据布局。您可以定义多个结构体来表示程序的不同账户类型。

在处理账户时，您需要一种方法将程序的数据类型与存储在账户数据字段中的原始字节之间进行转换：
...
1. 将 `borsh` crate 添加为 `Cargo.toml` 的依赖项：
...
将 `CounterAccount` 结构体添加到 `lib.rs`
...
指令是指 Solana 程序可以执行的不同操作。可以将其视为程序的公共 API——它们定义了用户在与程序交互时可以采取的操作。
...
为了将指令数据（字节）转换为枚举的变体，通常会实现一个辅助方法。此方法：
...
将以下代码添加到 `lib.rs` 中。此代码使用前一步中定义的 `CounterInstruction`
枚举和 `unpack` 方法，将传入的指令路由到相应的处理器函数：
...
1. 将被创建和初始化的计数器账户
2. 将为新账户创建提供资金的付款账户
3. 我们调用以创建新账户的 System Program

为了定义指令所需的账户，我们创建一个迭代器，用于遍历 `accounts` 切片，并使用
`next_account_info` 函数获取每个账户。您定义的账户数量即为指令所需的账户数量。
...
1. 指定分配给账户数据字段的空间（以字节为单位）。由于我们存储的是一个 u64 值（`count`），因此需要 8 个字节。

2. 计算所需的最低“租金”余额。在 Solana 上，账户必须根据存储的数据量维持一定数量的 lamports（租金）最低余额。
...
一旦定义了空间并计算了租金，通过调用 System Program 的 `create_account`
指令来创建账户。

在 Solana 上，新账户只能由 System
Program 创建。在创建账户时，我们需要指定分配的字节数以及新账户的程序所有者。System
Program：
...
要从我们程序的指令中调用 System Program，我们通过 `invoke` 函数进行 Cross
Program Invocation
(CPI)。CPI 允许一个程序调用其他程序的指令——在本例中，是调用 System Program 的
`create_account` 指令。
...
1. 使用传递给指令的 `initial_value` 创建一个新的 `CounterAccount` 结构体。
2. 获取新账户数据字段的可变引用。
3. 将 `CounterAccount` 结构体序列化到账户的数据字段中，从而有效地将
   `initial_value` 存储到账户中。
...
与 `process_initialize_counter`
函数类似，我们首先创建一个账户的迭代器。在本例中，我们只期望一个账户，即需要更新的账户。

请注意，在实际操作中，开发者必须实施各种安全检查以验证传递给程序的账户。由于所有账户都是由指令的调用者提供的，因此无法保证提供的账户是程序所期望的账户。缺少账户验证检查是程序漏洞的常见来源。

以下示例包括一个检查，以确保我们所指的 `counter_account`
账户是由执行程序拥有的。
...
然后将以下测试模块添加到 `lib.rs` 并运行 `cargo test-sbf`
以执行测试。可选地，使用 `--nocapture` 标志查看输出中的打印语句。
...
接下来，使用 `ProgramTest`
设置测试。然后创建一个新的 keypair，用作我们将初始化的计数器账户的地址，并定义一个初始值来设置计数器。
...
- 使用变体 0 (`InitializeCounter`) 和初始值创建指令数据
