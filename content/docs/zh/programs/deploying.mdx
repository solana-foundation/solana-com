...
* `Owner`：部署此程序的加载器。
* `ProgramData Address`是与程序账户关联的programdata账户，它保存程序的可执行文件（仅限loader-v3）。
* `Status`：`retracted`，`deployed`或`finalized`（仅限loader-v4）。
* `Authority`是程序的升级权限。
* `Last Deployed In Slot`是程序最后部署的槽位。
* `Data Length`是为部署预留的空间大小。当前部署的程序实际使用的空间可能会更少。
...
3. （在线）创建缓冲区并上传可执行文件
4. （可选）验证缓冲区的链上内容
5. （离线）签署使用缓冲区重新部署程序的交易 `--blockhash <VALUE> --sign-only`
6. （在线）使用此签名广播重新部署交易
