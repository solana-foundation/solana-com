...
对于新的部署，请使用 `solana program-v4 deploy`，而不是
...
该参数名为 `program-id`，尽管它需要提供一个 keypair 的文件路径：
...
请注意，初始部署使用了 `program-keypair`，而重新部署则改为使用 `program-id`：
...
- `--with-compute-unit-price`：为交易设置计算单元价格，单位为每计算单元 0.000001
  lamports（微 lamports）。使用
  [Helius 的优先费用 API](https://docs.helius.dev/guides/priority-fee-api)
  获取优先费用的估算值。
- `--use-rpc`：将写入交易发送到配置的 RPC，而不是 validator TPU。此标志需要一个
  [基于质押权重的](/developers/guides/advanced/stake-weighted-qos) RPC连接，例如
  [Helius](https://www.helius.dev/) 或
  [Triton](https://triton.one/)。此标志也可以通过以下方式配置为默认值：`solana config set --url <RPC_URL>`。
- `--max-sign-attempts`：在 blockhash 过期后签署或重新签署交易的最大尝试次数。如果在程序部署期间发送的任何交易在最初选择的最近 blockhash 过期后仍未确认，这些交易将使用新的最近 blockhash 重新签署并重新发送。使用此设置调整交易签署迭代的最大次数。每个 blockhash 的有效期约为 60 秒，这意味着使用默认值 5 将导致发送交易至少 5 分钟，或直到所有交易被确认，以先到者为准。
...
- `Program Id` 是可以在调用程序时引用于指令的 `program_id` 字段的地址。
- `Owner`：部署此程序时使用的加载器。
- `ProgramData Address`
  是与程序账户关联的 programdata 账户，存储程序的可执行文件（仅限 loader-v3）。
...
4. （可选）验证缓冲区的链上内容
5. （离线）签署一笔交易以使用缓冲区重新部署程序
   `--blockhash <VALUE> --sign-only`
6. （在线）使用此签名广播重新部署交易
   `--blockhash <VALUE> --signer <OFFLINE_SIGNER_PUBKEY>:<OFFLINE_SIGNER_SIGNATURE>`

查找一个最近的 `blockhash`，并将其粘贴进去以生成离线交易签名。`blockhash`
大约在 60 秒后过期。如果您没能及时完成，只需获取另一个新的哈希并重复操作，直到成功，或者考虑使用持久交易随机数。
