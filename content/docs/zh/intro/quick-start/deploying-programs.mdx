...
_rs`declare_id!()`_ 宏指定了程序的链上地址。Solana
Playground 在您构建项目时会更新此地址。

### !program

_rs`#[program]`_ 属性注解了一个模块，该模块包含表示程序指令的函数。

### !accounts

_rs`#[derive(Accounts)]`_ 属性注解了一个结构体，该结构体包含程序的账户。
...
_rs`ctx.accounts.new_account.data`_ 正在访问 `data` 字段，并将其设置为
`data: u64` 参数的值。

### !msg

_rs`msg!`_ 宏将一条消息记录到交易的程序日志中。

### !init

`init`
约束指定程序必须创建账户。在底层，此约束调用系统程序以创建一个新账户，并将此程序设置为所有者。作为程序所有者，该程序随后可以写入账户数据。

### !payer

`payer` 约束指定为新账户支付费用的账户。
...
- [`#[program]`](mention:one) 属性注解了一个模块，该模块包含表示程序指令的函数。
...
在此示例中，[`initialize`](mention:two) 指令需要两个参数：
...
- [`#[derive(Accounts)]`](mention:one)
  属性定义了一个结构体，该结构体指定了特定指令所需的账户，其中每个字段代表一个独立的账户。

  Anchor 会通过字段类型（例如 `Signer<'info>`）和约束（例如
...
命令会编译程序。程序的字节码在部署时会存储在一个可执行的程序账户中。
...
在部署程序后运行测试文件，可以在终端中运行 _shell`test`_。
...
您可以通过运行以下命令并指定 _rs`declare_id!()`_ 中找到的程序地址来关闭程序：
