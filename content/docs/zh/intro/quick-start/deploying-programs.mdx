...
_rs`#[program]`_ 属性标注了一个模块，其中包含表示程序指令的函数。

### !accounts

_rs`#[derive(Accounts)]`_ 属性标注了一个结构体，其中包含程序的账户。
...
_rs`ctx.accounts.new_account.data`_ 正在访问 `data` 字段，该字段属于
...
`init`
约束指定程序必须创建账户。在底层，此约束调用系统程序以创建一个新账户，并将此程序设置为所有者。作为程序所有者，此程序随后可以写入账户数据。
...
- [`#[program]`](mention:one) 属性用于标注包含程序指令的模块。
...
要构建程序，只需在终端中运行
`build`。该命令会编译程序。程序的字节码在部署时会存储在一个可执行的程序账户中。
...
入门代码包含一个测试文件，位于
`tests/anchor.test.ts`。此文件演示了如何从客户端调用程序中的
[`initialize`](mention:one) 指令。
