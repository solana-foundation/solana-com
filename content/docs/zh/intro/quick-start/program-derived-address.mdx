...
_rs`message`_ 字段包含一个 _rs`String`_，存储用户的消息。
...
- `message` - 一个 _rs`String`_，存储用户的消息。
- `bump` - 一个 _rs`u8`_，存储用于派生程序派生地址（PDA）的
...
现在，添加 _rs`create`_ 指令，用于创建和初始化 _rs`MessageAccount`_。
...
_rs`payer`_ 约束指定了哪个账户支付新账户的创建费用。
...
在这种情况下，该账户必须匹配 _rs`System`_ 类型，该类型指的是系统程序。
...
结构体中的每个字段代表一个账户，并通过以下两种方式进行验证：

1. 账户类型（如 _rs`Signer<'info>`_ 或
   _rs`Account<'info, T>`_），指定程序期望的账户类型
2. 可选约束条件（如 _rs`#[account(mut)]`_ 或
   _rs`#[account(init)]`_），定义额外的要求

这些共同使 Anchor 能够自动验证传递给指令的账户并确保程序的安全性。
...
1. _rs`user: Signer<'info>`_
   ...
   - 表示创建消息账户的用户
   - 需要可变状态（_rs`#[account(mut)]`_），因为它支付新账户的费用
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - `payer = user` 标识谁支付新账户的创建费用
   - `space` 为账户的数据字段分配所需的字节数

3. _rs`system_program: Program<'info, System>`_

   - 必需用于创建账户
   - 在后台，`init` 约束调用系统程序（System Program）来创建一个具有指定 `space`
     的新账户，并将所有者更改为当前程序。
...
`seeds` 和 `bump` 约束共同定义了一个账户地址为程序派生地址（PDA）。
...
通过 Anchor 程序创建的所有账户需要 8 字节用于账户标识符（discriminator），它作为账户类型的标识符，Anchor 在创建账户时会自动生成。
...
1. _rs`user: Signer<'info>`_
   ...
   - 表示更新消息账户的用户
   - 标记为可变 (_rs`#[account(mut)]`_)，因为可能需要支付更多空间费用以便在需要时为
...
在此情况下，_rs`user`_ 账户接收从关闭的 _rs`message_account`_
中转移的 lamports。
...
1. _rs`user: Signer<'info>`_
   ...
   - 表示用户关闭消息账户
   - 标记为可变的 (_rs`#[account(mut)]`_)，以接收来自关闭账户的 lamports
...
起始代码还包括一个位于`anchor.test.ts`中的测试文件。
