...
- `user` - 一个 _rs`Pubkey`_，用于标识创建消息账户的用户。
- `message` - 一个 _rs`String`_，其中包含用户的消息。
- `bump` - 一个 _rs`u8`_，用于存储
  ["bump" 种子](/docs/core/pda#canonical-bump)，以派生程序派生地址 (PDA)。存储此值可以节省计算资源，无需在后续指令中重新计算。
...
通过在终端中运行 _shell`build`_ 再次构建程序。
...
_rs`Signer<'info>`_ 类型要求账户签署交易。

### !account

_rs`Account<'info, T>`_ 类型要求账户与指定类型匹配。

在此情况下，账户必须与自定义类型 _rs`MessageAccount`_ 匹配。

### !program

_rs`Program<'info, T>`_ 类型要求账户与一个程序匹配。

在此情况下，账户必须与 _rs`System`_ 类型匹配，该类型指的是系统程序。
...
1. _rs`user: Signer<'info>`_
   ...
   - 表示创建消息账户的用户
   - 需要可变状态（_rs`#[account(mut)]`_），因为它为新账户支付
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - 账户创建所需
   - 在后台，`init` 约束调用系统程序 (System Program) 创建一个具有指定 `space`
     的新账户，并将所有者更改为当前程序。
...
- 用户消息 (_rs`String`_)：4 字节用于存储长度 + 可变消息长度
...
接下来，通过更新 `create` 函数，添加 _rs`create`_ 指令的业务逻辑：
...
_rs`Update`_ 结构定义了 _rs`update`_ 指令所需的账户。

1. _rs`user: Signer<'info>`_

   - 表示更新消息账户的用户
   - 标记为可变 (_rs`#[account(mut)]`_)，因为可能需要支付更多空间的费用以便在需要时存储
     `message_account`
   - 必须签署交易
...
_rs`#[instruction(message: String)]`_ 属性使 _rs`Update`_ 结构体能够从
_rs`update`_ 指令中访问 _rs`message`_ 参数。
...
_rs`Delete`_ 结构定义了 _rs`delete`_ 指令所需的账户：
...
1. _rs`_ctx: Context<Delete>`_ - 提供对 _rs`Delete`_
   结构中指定账户的访问权限。_rs`_ctx`_ 语法表明函数在其主体中不使用 Context。

函数主体仅使用 _rs`msg!()`_ 宏将消息打印到程序日志中。函数不需要额外的逻辑，因为
_rs`close`_ 约束在 _rs`Delete`_ 结构中处理了账户关闭。
...
起始代码还包括一个位于 `anchor.test.ts` 的测试文件。
...
运行测试文件，以检查其是否按预期运行。接下来的步骤将添加实际测试。
...
首先，测试文件发送一个交易，调用 `create` 指令，并传递 "Hello,
World!" 作为消息。
...
首先，测试文件发送一笔交易，调用 `update` 指令，并传递 "Hello,
Solana!" 作为新消息。
...
在发送交易并关闭账户后，该示例尝试使用其地址 (`messagePda`) 和 `fetchNullable`
获取账户。当账户在关闭后不再存在时，此方法返回 null。
...
运行测试文件。此命令会针对部署在 devnet 上的程序运行测试，并记录 SolanaFM 的链接以查看交易详情。
...
<Callout type="info">
  请注意，在此示例中，如果再次运行测试，`create` 指令 会失败，因为
