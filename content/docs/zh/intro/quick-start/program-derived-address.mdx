...
`seeds` 和 `bump` 约束共同定义了一个账户地址为 Program Derived Address (PDA)。
...
接下来，通过更新 `create` 函数为 _rs`create`_ 指令添加业务逻辑：
...
1. _rs`user: Signer<'info>`_
   ...
   - 表示更新消息账户的用户
   - 标记为可变 (_rs`#[account(mut)]`_)，因为可能需要支付更多空间的费用以便在需要时为
     `message_account` 提供支持
   - 必须签署交易
...
在此情况下，程序使用存储在 _rs`message_account`_ 中的现有 bump seed。
...
在准备好测试后，在 Playground 终端中使用 _shell`test`_
