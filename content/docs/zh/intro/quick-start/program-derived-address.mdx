...
在 `lib.rs` 中，更新 _rs`MessageAccount`_ 结构体，内容如下：
...
现在，添加 _rs`create`_ 指令，该指令创建并初始化 _rs`MessageAccount`_。

首先，通过更新 _rs`Create`_ 结构体定义指令所需的账户，如下所示：
...
_rs`payer`_ 约束指定哪个账户支付新账户的创建费用。

### !space

_rs`space`_ 约束指定为新账户的数据字段分配的字节数。
...
在这种情况下，该账户必须匹配自定义的 _rs`MessageAccount`_ 类型。

### !program

_rs`Program<'info, T>`_ 类型要求账户匹配一个程序。

在这种情况下，该账户必须匹配 _rs`System`_ 类型，该类型指的是系统程序（System
Program）。
...
1. 账户类型（例如 _rs`Signer<'info>`_ 或
   _rs`Account<'info, T>`_），指定程序期望的账户类型
2. 可选约束条件（例如 _rs`#[account(mut)]`_ 或
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - 必需用于创建账户
   - 在后台，`init` 约束调用系统程序，创建一个具有指定 `space`
     的新账户，并将所有者更改为当前程序。

---

_rs`#[instruction(message: String)]`_ 注解允许 _rs`Create`_ 结构体从 `create`
指令中访问 _rs`message`_ 参数。
...
- _rs`user.key().as_ref()`_ - 公钥 _rs`user`_ 账户作为第二个 seed。

`bump` 约束指示 Anchor 自动查找并使用正确的 bump seed。Anchor 使用 `seeds` 和
`bump` 来派生 PDA。
...
_rs`String`_ 类型需要 4 字节来存储字符串的长度，其余长度包含实际数据。

</Accordion>
</Accordions>

接下来，通过更新 `create` 函数，添加业务逻辑到 _rs`create`_ 指令中：
...
_rs`realloc`_ 约束重新分配了账户的数据。

### !realloc::payer

_rs`realloc::payer`_ 约束指定了为重新分配支付费用的账户。
...
1. _rs`user: Signer<'info>`_
   ...
   - 表示更新消息账户的用户
   - 标记为可变 (_rs`#[account(mut)]`_)，因为可能需要为 `message_account`
     提供更多空间
   - 必须签署交易
...
在此情况下，程序使用存储在 _rs`message_account`_ 上的现有 bump seed。

### !close

_rs`close`_ 约束关闭账户。

在此情况下，_rs`user`_ 账户接收来自关闭的 _rs`message_account`_ 的 lamports。
...
1. _rs`_ctx: Context<Delete>`_ - 提供对 _rs`Delete`_ 结构中指定账户的访问权限。
   _rs`_ctx`_ 语法表明函数在其主体中不使用 Context。

函数主体仅使用 _rs`msg!()`_ 宏将消息打印到程序日志中。函数不需要额外的逻辑，因为
_rs`close`_ 约束在 _rs`Delete`_ 结构中处理账户关闭。
...
将以下代码添加到 _ts`describe()`_ 中，但要放在 _ts`it()`_ 部分之前。
...
<Callout type="info">
  请注意，在此示例中，如果再次运行测试， `create` 指令将失败，因为
