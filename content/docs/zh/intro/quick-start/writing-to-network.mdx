...
account 数据分配所需的 SOL（以 lamports 为单位）。
...
第一个指令调用了 System Program 的 `createAccount` 指令以：

1. [分配所需字节](mention:space) 用于存储 mint 数据
2. [从钱包转移 lamports](mention:lamports) 为新账户提供资金
3. [将账户的所有权](mention:programId) 分配给 Token
   Extensions 程序 (`TOKEN_2022_PROGRAM_ID`)
...
第二个指令调用了 [Token Extensions Program](mention:programId) 的
...
通过将两个指令合并到一个交易中，可以确保账户创建和初始化是原子性操作。要么两个指令都成功，要么都不成功。
...
- [Wallet](mention:wallet) 作为交易费用和账户创建的付款人进行签名
