...
- _钱包地址_：一个由 Ed25519 密钥对生成的 32 字节公钥，通常显示为 base-58 编码的字符串（例如，`7MNj7pL1y7XpPnN7ZeuaE4ctwg3WeufbX5o85sA91J1`）。对应的私钥用于对该地址的交易进行签名。在 Solana 上，地址用作用户钱包、程序（智能合约）或网络上任何其他账户的唯一标识符。
- _连接的集群_：您当前交互的 Solana 网络。常见的集群包括：
  - `devnet`：一个供开发者实验的开发网络
  - `testnet`：一个为验证者测试保留的网络（应用开发者请勿使用）
  - `mainnet-beta`：用于实时交易的主 Solana 网络
