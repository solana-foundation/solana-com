...
Token
Program 是 Solana 上的一个可执行程序账户。与钱包账户类似，程序账户也使用相同的
[账户](https://github.com/anza-xyz/agave/blob/v2.1.11/sdk/account/src/lib.rs#L48-L60)
数据结构，但其字段存在关键差异。
...
当您部署一个 Solana 程序时，程序的可执行代码会存储在账户的 `data` 字段中。
...
所有程序账户都由一个 Loader 程序拥有，Loader 程序是一类内置程序，专门拥有 Solana 上的可执行程序账户。
...
Token Program 定义了 `Mint` 数据类型，该数据类型存储在 mint 账户的 `data`
字段中。
...
`data` 字段包含序列化的 `Mint` 账户状态，例如 mint
authority、总供应量、小数位数。
...
Solana 账户的 `data`
字段包含原始字节数据。要有意义地解释这些数据，必须将其反序列化为拥有该账户的程序定义的适当数据类型。

大多数 Solana 程序提供了带有辅助函数的客户端库，这些函数简化了反序列化过程。这些函数将原始账户字节转换为结构化数据类型，从而更容易处理账户数据。

<WithMentions>

例如，_shell`@solana/spl-token`_ 包含 [_ts`getMint()`_](mention:one)
函数，用于帮助反序列化 Mint 账户的 `data` 字段为
[Mint](https://github.com/solana-program/token/blob/program%40v8.0.0/program/src/state.rs#L16-L30)
数据类型。
...
`mintAuthority` 字段显示了被授权铸造新代币的权限。
...
该值以代币的最小单位表示。要以标准单位获取总供应量，请根据 `supply` 字段的值调整
`decimals`。

<CodePlaceholder title="反序列化的铸币数据" />
...
`isInitialized` 字段指示铸币账户是否已初始化。此字段是 Token
Program 中用于安全检查的。
...
`freezeAuthority` 字段显示被授权冻结代币账户的权限。
