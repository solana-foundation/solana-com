...
*rs`Update`* struct and `update` function.
...
Next, update the *rs`Update`* struct to include a new account called
...
This section adds a new account called `vault_account` to the *rs`Update`*
...
恭喜您完成Solana快速入门指南。您已经获得了关键Solana概念的实践经验，包括：`vault_account`:

* 从账户获取和读取数据*rs`[b"vault", user.key().as_ref()]`*
* 构建和发送交易
* 部署和更新Solana程序*rs`SystemAccount`* type, the System Program owns it like
  regular wallet accounts
...
如果您喜欢通过示例学习，请查看`delete`以获取各种示例程序。

</Accordion>
</Accordions>

点击`update`按钮并输入项目名称，将其添加到您的Solana
...
The *rs`Transfer`* struct specifies the required accounts for the System
Program's transfer instruction.

### !cpi\_context

The *rs`CpiContext`* struct specifies the program and accounts for a Cross
Program Invocation (CPI).

### !transfer

The *rs`transfer()`* function invokes the System Program's transfer instruction.
...
The *rs`CpiContext`* specifies:

* The program to invoke (System Program)
* The accounts required in the CPI (defined in the *rs`Transfer`* struct)
...
Now add a "refund on delete" mechanism by changing the *rs`Delete`* struct and
`delete` function.

First, update the *rs`Delete`* struct to include the `vault_account`. This
...
The *rs`signer_seeds`* specify the optional seeds and bump seeds used to derive
the PDA.

### !with\_signer

The *rs`with_signer()`* method passes the signer seeds with the CPI.
...
Note that *rs`_ctx: Context<Delete>`* changes to *rs`ctx: Context<Delete>`* to
...
代币程序上的*rs`Transfer`* struct specifies the required accounts for the System
Program's transfer instruction:
...
The *rs`transfer()`* function then invokes the transfer instruction on the
...
[Github repo](https://github.com/solana-developers/program-examples/tree/main/basics/hello-solana/anchor)之间发送的最小单位。
