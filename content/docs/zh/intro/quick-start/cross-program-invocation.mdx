...
`update` 和 `delete` 指令需要修改，以通过调用系统程序来处理账户之间的 SOL 转账。
...
首先，程序需要一个简单的“支付更新”机制，通过修改 _rs`Update`_ 结构体和 `update`
函数来实现。
...
本节添加了一个名为 `vault_account` 的新账户到 _rs`Update`_
结构体中。该账户作为一个由程序控制的“金库”，在用户更新消息时接收 SOL。
...
- 该账户的地址来源于使用种子 _rs`[b"vault", user.key().as_ref()]`_ 派生的 PDA
- 作为 PDA，它没有私钥，因此只有程序可以在执行 CPI 时为该地址“签名”
- 作为 _rs`SystemAccount`_ 类型，系统程序像常规钱包账户一样拥有它
...
在 `delete` 指令中，你将看到程序如何在 CPI 中为该 PDA “签名”。
...
_rs`CpiContext`_ 结构体指定了跨程序调用 (CPI) 的程序和账户。
...
在 `update` 指令中，实现在 CPI 中调用系统程序的 `transfer`
指令。这展示了如何在程序中执行 CPI，从而实现 Solana 程序的可组合性。
...
`transfer` 函数随后调用系统程序上的转账指令，传入：

- `cpi_context`（程序和账户）
- 要转账的 `amount`（1,000,000 lamports 或 0.001 SOL）
...
CPI 的设置与客户端指令的构建方式相匹配，您需要为特定指令指定程序、账户和指令数据以进行调用。当程序的
...
`vault_account`。这允许在用户关闭其消息账户时，将金库中的任何 SOL 转回用户账户。
...
还需添加 `system_program`，因为转账所需的 CPI 需要调用系统程序。
...
将 `vault_account`
添加到 Delete 结构体中，使程序能够在删除指令期间访问用户的金库账户，将任何累积的 SOL 转回用户账户。
...
请注意，_rs`_ctx: Context<Delete>`_ 更改为 _rs`ctx: Context<Delete>`_
以在函数体中使用上下文。
...
然后，_rs`transfer()`_ 函数调用了系统程序的转账指令，传递：
...
然后，更新更新指令以包含 [`vaultAccount`](mention:one)
...
点击 `Import` 按钮并输入项目名称，将其添加到 Solana
Playground 的项目列表中。一旦项目被导入，所有更改都会自动保存并持久化。
