...
本节在 _rs`Update`_ 结构体中新增了一个名为 `vault_account`
的账户。此账户作为一个程序控制的“金库”，当用户更新消息时接收他们的 SOL。
...
- 该账户的地址来源于使用种子派生的 PDA _rs`[b"vault", user.key().as_ref()]`_
- 作为 PDA，它没有私钥，因此只有程序可以在执行 CPI 时为该地址“签名”
- 作为 _rs`SystemAccount`_ 类型，系统程序像管理普通钱包账户一样管理它
...
在 `update` 指令中，实现包括一个跨程序调用 (CPI)，用于调用系统程序的 `transfer`
...
- 要调用的程序（系统程序）
- CPI 所需的账户（定义在 _rs`Transfer`_ 结构体中）
...
CPI 的设置方式与客户端指令的构建方式相匹配，您需要为要调用的特定指令指定程序、账户和指令数据。当程序的
...
将 `vault_account`
添加到 Delete 结构中，使程序能够在删除指令期间访问用户的金库账户，将任何累积的 SOL 转回给用户。
...
_rs`transfer()`_ 函数随后调用系统程序上的转账指令，传递：
