...
如果您的 Ubuntu 终端看起来与下图相似，您可能会遇到一个问题，即
`ctrl + v`（粘贴快捷键）在终端中无法使用。
...
接下来，关闭 Windows
Terminal，然后再次搜索 Ubuntu 以重新打开 Linux 终端。此时，终端应看起来与下图相似，`ctrl + v`（粘贴快捷键）可以正常使用。
...
如果您在安装 `protobuf-compiler` 时遇到以下错误，请确保首先运行
...
您可以将 `stable` 替换为与您所需版本匹配的软件版本标签（例如
`v2.0.3`），或者使用以下三个符号通道名称之一：`stable`、`beta` 或 `edge`。

首次安装 Solana CLI 时，您可能会看到以下提示信息，要求您添加一个 `PATH`
环境变量：
...
当使用 `localnet` 作为 `Anchor.toml` 中的 `cluster`
