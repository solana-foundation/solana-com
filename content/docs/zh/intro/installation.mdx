...
如果您的 Ubuntu 终端看起来像下图所示，您可能会遇到一个问题，即
...
如果您使用带有 `zsh` 的 Mac，运行安装过程中记录的默认 `export PATH`
命令，在关闭终端后不会保留。
...
别忘了运行 _shell`avm use`_ 命令来声明您系统上要运行的 Anchor CLI 版本。
...
_shell`anchor init --test-template rust`_ 获取 Rust 测试模板）
...
| 工具     | 描述                                                            | 链接                        |
| -------- | --------------------------------------------------------------- | --------------------------- |
| MCP      | 您可以通过光标连接的 MCP 服务器，以改进 Solana 的 AI 辅助开发。 | https://mcp.solana.com/     |
| LLMs.txt | 针对 LLM 优化的文档，您可以使用它来训练 Solana 文档上的 LLM。   | https://solana.com/llms.txt |
...
RPC URL 和 Websocket URL 指定了 Solana CLI 发出请求的 Solana 集群。
...
要使用 Solana CLI 发送交易，您需要一个存有 SOL 的 Solana 钱包。
...
当在 `Anchor.toml` 中将 `localnet` 用作 `cluster`
时，Anchor 会自动启动本地 validator，部署您的程序，运行测试，然后停止 validator。
