...
CLI 二进制文件添加到您的 `PATH` 中，以便在终端中可以使用该命令。
...
Agave 是 [Anza](https://www.anza.xyz/) 的 validator 客户端，前身是 Solana
Labs 的 validator 客户端。
...
本节将介绍一些常见的 Solana CLI 命令，帮助您快速入门。
...
请求将 SOL 空投到您的钱包中，以支付交易费用和程序部署费用。
...
当使用 `localnet` 作为 `cluster` 在 `Anchor.toml`
中时，Anchor 会自动启动本地 validator，部署您的程序，运行测试，然后停止 validator。
