...
`0 <= S < L`。此要求确保签名不可篡改。每笔交易必须至少有一个用于
...
[代币扩展程序](https://spl.solana.com/token-2022)的程序 ID 为
`TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb`，它包含与[代币程序](#token-program)相同的所有功能，但还附加了扩展功能，例如保密转账、自定义转账逻辑、扩展元数据等。
...
[代币程序](https://spl.solana.com/token)的程序 ID 为
