...
一个 64 字节的 ed25519 签名，由 R（32 字节）和 S（32 字节）组成。要求 R 是一个非小阶的打包 Edwards 点，S 是一个范围在
`0 <= S < L` 内的标量。此要求确保签名不可篡改。每笔交易必须至少有一个
...
一个过去的 [slot](#slot)，因为领导者离线或 [分叉](#fork)
被集群共识放弃以选择更好的替代方案，而未生成
[区块](#block)。跳过的 slot 不会作为后续 slot 的区块祖先出现，也不会增加
...
[Token Extensions Program](https://spl.solana.com/token-2022) 的程序 ID 是
...
[Token Program](https://spl.solana.com/token) 的程序 ID 是
...
在 [权益](#stake) 被委托后的若干 [epoch](#epoch)
期间，它会逐步生效。在此期间，权益被视为“激活中”。更多信息请参阅：[预热和冷却](https://docs.anza.xyz/consensus/stake-delegation-and-rewards#stake-warmup-cooldown-withdrawal)。
