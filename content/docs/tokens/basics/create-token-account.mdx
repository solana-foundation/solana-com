...
An associated token account is a token account with an address that is a Program
...
fn main() -> Result<()> {
    ...
    // Send and confirm transaction
    let transaction_signature2 = client.send_and_confirm_transaction(&transaction2)?;

    println!("Token Account Address: {}", token_account.pubkey());
    println!("Transaction Signature: {}", transaction_signature2);
...
async fn main() -> Result<()> {
    ...
    // Send and confirm transaction
    let transaction_signature2 = client.send_and_confirm_transaction(&transaction2).await?;
...
async fn main() -> Result<()> {
    ...
    // Get the associated token account address for the fee payer and mint
    ...
    // Create instruction for creating an associated token account
    let create_ata_instruction = create_associated_token_account(
        &fee_payer.pubkey(),      // funding address
