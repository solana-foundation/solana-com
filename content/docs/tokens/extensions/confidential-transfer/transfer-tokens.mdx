...
_rs`ConfidentialTransferAccount`_ extension state and approved for confidential
...
1. Create the necessary proofs on the client side:
   ...
   - **Ciphertext Validity Proof**: Verifies that the ciphertext representing
     the transfer amount are properly generated according to the encryption
     scheme.
   - **Range Proof**: Verifies that the transfer amount falls within a valid
     range and is a non-negative value.
