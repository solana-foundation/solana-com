...
1. Create the necessary proofs on the client side:
   ...
   - **Equality Proof**: Verifies that the encrypted transfer amounts under the
     sender's and receiver's encryption keys encrypt the same value.
   - **Ciphertext Validity Proof**: Verifies that the ciphertexts of the
     transfer amount are properly generated according to the encryption scheme.
   - **Range Proof**: Verifies that the encrypted transfer amount falls within a
     valid range and is a non-negative value.

2. For each proof:

   - Invoke the Zero-Knowledge ElGamal proof program to verify the proof data.
