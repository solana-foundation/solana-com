...
- **authority**: The account that has permission to modify confidential transfer
...
- **auto_approve_new_accounts**: When set to true, users can create token
  ...
  authority must manually approve each new token account before it can be used
...
1. **Create the Mint Account**: Invoke the System Program's `CreateAccount`
...
3. **Initialize Base Mint**: Invoke the Token Extension Program's
   `Instruction::InitializeMint` instruction to initialize the standard mint
   state.

While you could implement these instructions manually, the `spl_token_client`
crate provides a `create_mint` method that builds and sends a transaction with
all three instructions in a single function call, as demonstrated in the example
below.
