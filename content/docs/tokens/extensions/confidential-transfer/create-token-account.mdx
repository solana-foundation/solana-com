...
- **approved**: The account's approval status for confidential transfers. If the
  mint account's `auto_approve_new_accounts` is `true`, all token accounts are
  automatically approved for confidential transfers.
...
- **maximum_pending_balance_credit_counter**: The count limit of pending credits
  before requiring an `ApplyPendingBalance` instruction to convert the pending
...
- **expected_pending_balance_credit_counter**: The
  ...
  instruction data the last time the `ApplyPendingBalance` instruction was
  processed.

- **actual_pending_balance_credit_counter**: The
  `pending_balance_credit_counter` value on the token account at the time the
  last `ApplyPendingBalance` instruction was processed.

<Callout type="info">
  **Pending vs Available Balance**

Confidential balances are separated into pending and available balances
toprevent DoS attacks. Without this separation, an attacker could repeatedly
send tokens to a token account, blocking the token account owner's ability to
...
Token account owners must use the `ApplyPendingBalance` instruction to move
funds from pending to available balance. Incoming transfers or deposits don't
affect a token account's available balance.
...
2. Two pending credit counters track changes between instruction creation and
   processing:
   - `expected_pending_balance_credit_counter`: The
   ...
   - `actual_pending_balance_credit_counter`: The
     `pending_balance_credit_counter` value on the token account at the time the
     `ApplyPendingBalance` instruction is processed

Matching counters indicate the `decryptable_available_balance` matches the
`available_balance`. Different values require the client to look up recent
deposit/transfer instructions matching the counter difference to calculate the
correct balance.
...
2. Fetch the most recent transactions including deposit and transfer
   ...
   - Add public amounts from deposit transactions
   - Decrypt and add destination ciphertext amounts from transfer transactions
