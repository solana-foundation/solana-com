...
sequenceDiagram
    ...
    activate ATA
    ATA-->>TokenAccount: Create token account at deterministic address
    deactivate ATA
...
The _rs`ConfidentialTransferAccount`_ contains multiple fields to manage
confidential transfers:

- **approved**: Indicates if the account has been approved for confidential
  transfers. This is set to true automatically if the mint has auto-approval
  enabled.
...
- **available_balance**: The encrypted balance that is available for transfers.

- **decryptable_available_balance**: The available balance encrypted with a
  symmetric key for efficient decryption by the account owner.
...
- **maximum_pending_balance_credit_counter**: The maximum number of pending
  credits before requiring balance application.

- **expected_pending_balance_credit_counter**: The counter value that was
  included in the last `ApplyPendingBalance` instruction, used to prevent replay
  attacks.

- **actual_pending_balance_credit_counter**: The actual counter value when the
  last `ApplyPendingBalance` instruction was executed, used to track the state
  of applied pending balances.

### Required Transaction Instructions
...
2. **Reallocate Account Space**: Invoke the Token Extension Program's
   ...
   _rs`ConfidentialTransferAccount`_ extension.
...
3. **Configure Confidential Transfers**: Invoke the Token Extension Program's
   ...
   instruction to initialize the confidential transfer state and provide the
   ElGamal public key and zero-knowledge proof of its validity.

The reallocation and configuration instruction can be submitted in a separate
transaction from the token account creation.

The configure account instruction requires client-side generation of
cryptographic keys and proof data to ensure secure confidential transfers.
