...
Token account owners must use the `ApplyPendingBalance` instruction to convert
the pending balance to the available balance. Incoming transfers or deposits
don't affect a token account's available balance.
...
The _rs`PubkeyValidityProofData`_ creates a proof that verifies the ElGamal key
