...
- **expected_pending_balance_credit_counter**: The pending counter value that
  was included in the instruction data of the last `ApplyPendingBalance`
  instruction.

- **actual_pending_balance_credit_counter**: The actual pending counter value
  when the last `ApplyPendingBalance` instruction was executed.
...
**Only the token account owner can configure the account for confidential
transfers**.
