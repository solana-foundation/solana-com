...
- [Protocol Overview](https://spl.solana.com/confidential-token/deep-dive/overview) -
...
Confidential transfers are currently only supported on devnet. You can run the
[example](#example) using either a devnet RPC URL or the local validator. If
using the local validator, you must clone the Token Extension Program from the
devnet cluster.
...
[instructions](#confidential-transfer-instructions) to the Token Extension
program that allows you to transfer tokens between accounts without revealing
the transfer amount.

```mermaid title="Confidential Transfer Overview"
    sequenceDiagram
...
```mermaid title="Confidential Transfer Detailed Sequence"
sequenceDiagram
    ...
    activate Token22
    Token22-->>SenderAccount: Increase token account balance
    deactivate Token22
    ...
    Sender->>ElGamal: Create equality proof
    activate ElGamal
    Sender->>ElGamal: Create ciphertext validity proof
    Sender->>ElGamal: Create range proof

    Sender->>Token22: transfer_tokens
    activate Token22
    Token22-->>Token22: Verify proofs
    Token22-->>SenderAccount: Decrease encrypted amount
    Token22-->>RecipientAccount: Increase encrypted amount
    deactivate Token22

    Sender->>ElGamal: Close proof accounts
    deactivate ElGamal
    deactivate Sender
    end

    rect rgba(180, 150, 235, 0.3)
    Note over Recipient,RecipientAccount: 6. Apply & Withdraw (Optional)
    ...
    Recipient->>ElGamal: Create equality proof account
    activate ElGamal
    Recipient->>ElGamal: Create range proof account

    Recipient->>Token22: withdraw_tokens
    activate Token22
    Token22-->>Token22: Verify proofs
    Token22-->>RecipientAccount: Convert from available confidential balance to public balance
    deactivate Token22

    Recipient->>ElGamal: Close proof accounts
    deactivate ElGamal
    deactivate Recipient
    end
```
...
The following Rust example shows the steps for confidential token transfers
corresponding to the diagram above.

<Callout type="info">

Confidential transfers are currently only supported on devnet. You can test the
example using either a devnet RPC URL or a local validator. If using the local
validator, you must clone the Token Extension Program from the devnet cluster.

</Callout>

Use the following command to start a local validator with the Token Extension
Program cloned from devnet.
...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Sets up mint account for confidential transfers. Must be included in same transaction as _rs`TokenInstruction::InitializeMint`_ instruction.            |
...
| _rs`EmptyAccount`_                  | Empties the pending and available confidential balances to allow closing a confidential token account.                                                  |
| _rs`Deposit`_                       | Converts regular tokens into pending confidential balance.                                                                                              |
...
| _rs`EnableConfidentialCredits`_     | Allows an account to receive confidential token transfers.                                                                                              |
| _rs`DisableConfidentialCredits`_    | Blocks incoming confidential transfers while still allowing public transfers.                                                                           |
| _rs`EnableNonConfidentialCredits`_  | Allows an account to receive public token transfers.                                                                                                    |
...
| _rs`ConfigureAccountWithRegistry`_  | Alternative way to set up token accounts for confidential transfers using an _rs`ElGamalRegistry`_ account instead of _rs`VerifyPubkeyValidity`_ proof. |
