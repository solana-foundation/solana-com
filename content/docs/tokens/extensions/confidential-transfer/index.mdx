...
without revealing the token amount. This is useful for privacy-preserving
transactions.

## How does it work?

The Confidential Transfer extension adds a new instructions to the token program
...
The following example demonstrates the complete lifecycle of confidential token
transfers:

1. Initialize a mint with the confidential transfer extension.
2. Create a token account with confidential transfer extension for the sender.
3. Create a token account with confidential transfer extension for the
   recipient.
4. Mint tokens to the sender account.
5. Deposit sender's public balance to a confidential pending balance.
6. Apply sender's pending balance to confidential available balance.
7. Confidently transfer the tokens from sender token account to recipient token
   account.
8. Apply recipient's pending balance to confidential available balance.
9. Withdraw recipient's confidential available balance to public balance.

```mermaid title="Confidential Transfer Example"
sequenceDiagram
