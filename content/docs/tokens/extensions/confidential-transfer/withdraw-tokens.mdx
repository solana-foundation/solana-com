...
1. Create two proofs client side:
   ...
   - Equality Proof: Verifies that the remaining balance (current balance minus
     withdrawal amount) is consistently represented in both the ElGamal
     ciphertext and its corresponding Pedersen commitment
   - Range Proof: Verifies that the remaining balance falls within a valid range
