...
1. Create two proofs client side:

   - **Equality Proof**: Verifies that the remaining balance (current balance
     minus withdrawal amount) is consistently represented in both the ElGamal
     ciphertext and its corresponding Pedersen commitment
   - **Range Proof**: Verifies that the remaining balance falls within a valid
     range and is a non-negative value and is a non-negative value

2. For each proof:

   - Invoke the ZK ElGamal proof program to verify the proof data
   - The ZK ElGamal proof program then stores metadata specific to each type of
     proof in a proof "context state" account to use in subsequent instructions
