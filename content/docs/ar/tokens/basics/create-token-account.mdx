...
token account هو نوع حساب في برامج Token الخاصة بسولانا يخزن معلومات حول ملكية
الفرد لرمز معين (mint). يرتبط كل token account بـ mint واحد ويتتبع تفاصيل مثل
رصيد الرمز والمالك.
...
للاحتفاظ بالرموز لـ mint محدد، يجب على المستخدمين أولاً إنشاء token account.
يتتبع كل token account:

1. mint محدد (نوع الرمز الذي يحتفظ به token account)
2. مالك (السلطة التي يمكنها تحويل الرموز من الحساب)

إليك مثال باستخدام USD Coin (USDC) على سولانا:

- عنوان mint الخاص بـ USD Coin: `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Circle (مصدر USD Coin) لديه token account في
  `3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
- يحتفظ token account هذا فقط بوحدات من رمز USD Coin (mint)
- Circle لديه سلطة المالك في `7VHUFJHWu2CuExkJcJrzhQPJ2oygupTWkL2A2For4BmE`
  ويمكنه تحويل هذه الرموز
...
1. "مالك" token account - هذا هو العنوان المخزن في بيانات token account كحقل
   "owner" من النوع
   ...
   المحدد بواسطة Token Program. يمكن للمالك تحويل أو حرق أو تفويض الرموز من
   الحساب. يشار إلى هذا العنوان أحيانًا باسم "السلطة" الخاصة بـ token account
   لتمييزه عن مالك البرنامج.

2. "مالك" البرنامج - يشير هذا إلى البرنامج الذي يمتلك بيانات الحساب على سولانا.
   بالنسبة لـ token account، يكون هذا دائمًا إما Token Program أو Token
   Extension Program، كما هو محدد في حقل "owner" من نوع سولانا الأساسي
   [Account](https://github.com/anza-xyz/solana-sdk/blob/4e30766b8d327f0191df6490e48d9ef521956495/account/src/lib.rs#L51).

عند العمل مع token accounts، يشير مصطلح "مالك" عادةً إلى السلطة التي يمكنها
إنفاق الرموز، وليس البرنامج الذي يمتلك الحساب.

</Callout>

## ما هو Associated Token Account؟

Associated token account هو token account بعنوان يكون Program Derived Address
(PDA) تم إنشاؤه بواسطة
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program).
يمكنك اعتبار associated token account بمثابة token account الافتراضي للمستخدم
لحفظ وحدات من رمز معين (mint).

<Callout type="info">
  مصطلح "associated token accounts" ينطبق فقط على token accounts التي ينشئها
  Associated Token Program.
</Callout>

توفر associated token accounts طريقة حتمية للعثور على token account الخاص
بالمستخدم لأي mint معين. يمكنك فحص تنفيذ الاشتقاق
[هنا](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/interface/src/address.rs#L56).
...
يضمن هذا الاشتقاق الحتمي أنه لأي مجموعة من عنوان المحفظة و token mint، يوجد
بالضبط عنوان associated token account واحد. يجعل هذا النهج من السهل العثور على
token account الخاص بالمستخدم لأي token mint معين، مما يلغي الحاجة إلى تتبع
عناوين token account بشكل منفصل.

<Callout type="info">
  يعمل Associated Token Program كبرنامج مساعد ينشئ token accounts بعناوين حتمية
  (PDAs). عند إنشاء associated token account، يقوم Associated Token Program
  بإجراء CPI (Cross-Program Invocation) إلى Token Program أو Token Extension
  Program. يمتلك token program الحساب المنشأ الذي له نفس بنية نوع `Account` كما
  هو محدد في token program. لا يحتفظ Associated Token Program بأي حالة - فهو
  يقدم ببساطة طريقة موحدة لإنشاء token accounts في عنوان حتمي يكون PDA.
</Callout>

## كيفية إنشاء حساب توكن

لإنشاء حساب token account، قم باستدعاء التعليمة
...
يمكنك العثور على تطبيقات هذه التعليمة
[هنا](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/processor.rs#L145).

تحتاج المعاملة لإنشاء حساب توكن إلى تعليمتين:

1. استدعاء System Program لإنشاء وتخصيص مساحة لحساب توكن ونقل الملكية إلى Token
   Program.
2. استدعاء Token Program لتهيئة بيانات حساب التوكن.
...
لإنشاء associated token account، قم باستدعاء التعليمة
...
يمكنك العثور على تطبيقات هذه التعليمة
[هنا](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/program/src/processor.rs#L66).

تقوم التعليمة لإنشاء associated token account تلقائيًا باستدعاء System Program
لإنشاء حساب التوكن واستدعاء Token Program لتهيئة بيانات حساب التوكن. يحدث هذا من
خلال Cross Program Invocations (CPI).
