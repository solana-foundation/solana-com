...
title: التحويل السري
description:
  تعرف على ملحق التحويل السري وكيفية استخدامه لإضافة ميزات اختيارية إلى عمليات
  سك الرموز والحسابات.
---
...
التي تكون خاصة. أما عناوين حسابات الرموز فتبقى عامة.
...
- [كتاب الطبخ للأرصدة السرية](https://github.com/solana-developers/Confidential-Balances-Sample) -
  مقتطفات من التعليمات البرمجية حول كيفية استخدام ملحق التحويل السري
...
التدفق الأساسي للتحويلات السرية للرموز هو كما يلي:

1. إنشاء حساب mint account مع ملحق التحويل السري.
2. إنشاء حسابات token account مع ملحق التحويل السري للمرسل والمستلم.
...
6. تحويل الرموز بشكل سري من حساب رموز المرسل إلى حساب رموز المستلم.
...
<Cards>
  ...
  >
    كيفية تحويل الرموز بشكل سري بين حسابات الرموز
  </Card>
</Cards>
...
المستنسخ من الشبكة الرئيسية باستخدام الأمر التالي. يجب أن يكون لديك Solana CLI
مثبتًا لاستخدام الـvalidator المحلي.
...
<Callout type="info">
  في وقت كتابة هذا المقال، لم يتم تمكين التحويلات السرية على الـvalidator المحلي
  الافتراضي. يجب عليك استنساخ برنامج Token Extension Program من الشبكة الرئيسية
  لتشغيل كود المثال.
</Callout>
...
| التعليمة                            | الوصف                                                                                                                           |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | إعداد حساب mint للتحويلات السرية. يجب تضمين هذه التعليمة في نفس المعاملة مع تعليمة _rs`TokenInstruction::InitializeMint`_.      |
| _rs`UpdateMint`_                    | تحديث إعدادات التحويل السري لـmint.                                                                                             |
| _rs`ConfigureAccount`_              | إعداد حساب token للتحويلات السرية.                                                                                              |
| _rs`ApproveAccount`_                | الموافقة على حساب token للتحويلات السرية إذا كان الـmint يتطلب موافقة لحسابات token الجديدة.                                    |
| _rs`EmptyAccount`_                  | تفريغ الأرصدة السرية المعلقة والمتاحة للسماح بإغلاق حساب token.                                                                 |
| _rs`Deposit`_                       | تحويل رصيد الـtoken العام إلى رصيد سري معلق.                                                                                    |
| _rs`Withdraw`_                      | تحويل الرصيد السري المتاح مرة أخرى إلى رصيد عام.                                                                                |
| _rs`Transfer`_                      | تحويل الرموز بين حسابات token بشكل سري.                                                                                         |
| _rs`ApplyPendingBalance`_           | تحويل الرصيد المعلق إلى رصيد متاح بعد الإيداعات أو التحويلات.                                                                   |
| _rs`EnableConfidentialCredits`_     | السماح لحساب token باستلام تحويلات token سرية.                                                                                  |
| _rs`DisableConfidentialCredits`_    | حظر التحويلات السرية الواردة مع السماح بالتحويلات العامة.                                                                       |
| _rs`EnableNonConfidentialCredits`_  | السماح لحساب token باستلام تحويلات token عامة.                                                                                  |
| _rs`DisableNonConfidentialCredits`_ | حظر التحويلات العادية لجعل الحساب يستقبل فقط التحويلات السرية.                                                                  |
| _rs`TransferWithFee`_               | تحويل الرموز بين حسابات token بشكل سري مع رسوم.                                                                                 |
