...
يتيح ملحق النقل السري عمليات نقل رمزية خاصة من خلال إضافة حالة إضافية إلى حساب
العملة الرمزية. يشرح هذا القسم كيفية إنشاء عملة رمزية مع تمكين هذا الملحق.

يوضح الرسم التخطيطي التالي الخطوات المتضمنة في إنشاء عملة رمزية مع ملحق النقل
السري:
...
- **auto_approve_new_accounts**: عند تعيينها على true، يمكن للمستخدمين إنشاء
  حسابات رمزية مع تمكين عمليات النقل السرية افتراضيًا. عند تعيينها على false،
  يجب على السلطة الموافقة على كل حساب رمزي جديد قبل استخدامه في عمليات النقل
  السرية.
...
1. **إنشاء حساب العملة الرمزية**: استدعاء تعليمة _rs`CreateAccount`_ من System
   Program لإنشاء حساب العملة الرمزية.
...
3. **تهيئة العملة الرمزية**: استدعاء تعليمة _rs`Instruction::InitializeMint`_ من
   Token Extensions Program لتهيئة حالة العملة الرمزية القياسية.
...
<Callout type="info">
  في وقت كتابة هذا المحتوى، لم يتم تفعيل التحويلات السرية على الـ validator
