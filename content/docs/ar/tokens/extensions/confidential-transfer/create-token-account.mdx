...
description: تعلم كيفية إنشاء حساب توكن مع امتداد التحويل السري.
---

## كيفية إنشاء حساب توكن مع امتداد التحويل السري

يتيح امتداد التحويل السري إجراء تحويلات توكن خاصة من خلال إضافة حالة إضافية إلى
حساب التوكن. يشرح هذا القسم كيفية إنشاء حساب توكن مع تمكين هذا الامتداد.

يوضح الرسم التخطيطي التالي الخطوات المتضمنة في إنشاء حساب توكن مع امتداد التحويل
السري:
...
- **approved**: حالة موافقة الحساب للتحويلات السرية. إذا كان تكوين
  `auto_approve_new_accounts` في حساب mint مضبوطًا على `true`، فستتم الموافقة
  تلقائيًا على جميع حسابات التوكن للتحويلات السرية.
...
- **pending_balance_credit_counter**: يحسب الاعتمادات المعلقة الواردة للرصيد من
...
- **expected_pending_balance_credit_counter**: القيمة
  `pending_balance_credit_counter` المقدمة من العميل من خلال بيانات التعليمات في
  آخر مرة تمت فيها معالجة تعليمات _rs`ApplyPendingBalance`_.

- **actual_pending_balance_credit_counter**: القيمة
  `pending_balance_credit_counter` الموجودة في حساب الرمز المميز في وقت معالجة
  آخر تعليمات _rs`ApplyPendingBalance`_.

<Callout type="info">
  **الرصيد المعلق مقابل الرصيد المتاح**

يتم فصل الأرصدة السرية إلى أرصدة معلقة وأرصدة متاحة لمنع هجمات حجب الخدمة (DoS).
بدون هذا الفصل، يمكن للمهاجم إرسال الرموز المميزة بشكل متكرر إلى حساب الرمز
المميز، مما يمنع مالك حساب الرمز المميز من القدرة على تحويل الرموز المميزة. لن
يتمكن مالك حساب الرمز المميز من تحويل الرموز المميزة لأن الرصيد المشفر سيتغير
بين وقت إرسال المعاملة ووقت معالجتها، مما يؤدي إلى فشل المعاملة.

تتم إضافة جميع الإيداعات ومبالغ التحويل في البداية إلى الرصيد المعلق. يجب على
مالكي حساب الرمز المميز استخدام تعليمات _rs`ApplyPendingBalance`_ لتحويل الرصيد
المعلق إلى الرصيد المتاح. لا تؤثر التحويلات الواردة أو الإيداعات على الرصيد
المتاح لحساب الرمز المميز.
...
والذي يستخدم في تعليمات _rs`ApplyPendingBalance`_
...
عند استدعاء تعليمات _rs`ApplyPendingBalance`_ لتحويل الرصيد المعلق إلى الرصيد
المتاح:

1. يقوم العميل بالبحث عن الأرصدة المعلقة والمتاحة الحالية، ويقوم بتشفير المجموع،
   ويقدم `decryptable_available_balance` مشفرة باستخدام مفتاح AES الخاص بمالك
   حساب الرمز المميز.

2. تتتبع عدادات الائتمان المعلقة المتوقعة والفعلية التغييرات في قيمة العداد بين
   وقت إنشاء تعليمات _rs`ApplyPendingBalance`_ ومعالجتها:
   - `expected_pending_balance_credit_counter`: قيمة
     `pending_balance_credit_counter` عندما ينشئ العميل تعليمات
     _rs`ApplyPendingBalance`_
   - `actual_pending_balance_credit_counter`: قيمة
     `pending_balance_credit_counter` الموجودة في حساب الرمز المميز في وقت
     معالجة تعليمات _rs`ApplyPendingBalance`_

تشير عدادات المتوقع/الفعلي المتطابقة إلى أن `decryptable_available_balance`
يتطابق مع `available_balance`.

عند جلب حالة حساب الرمز المميز لقراءة `decryptable_available_balance`، تتطلب
القيم المختلفة لعدادات المتوقع/الفعلي من العميل البحث عن تعليمات الإيداع/التحويل
...
2. **إعادة تخصيص مساحة الحساب**: استدعاء تعليمة
   _rs`TokenInstruction::Reallocate`_ الخاصة ببرنامج امتداد الرمز المميز لإضافة
   مساحة لحالة _rs`ConfidentialTransferAccount`_.

3. **تكوين النقل السري**: استدعاء تعليمة
   ...
   الخاصة ببرنامج امتداد الرمز المميز لتهيئة حالة
...
تنشئ _rs`PubkeyValidityProofData`_ إثباتًا يتحقق من صحة مفتاح ElGamal. للاطلاع
على تفاصيل التنفيذ، راجع:
...
لتشغيل المثال، ابدأ مصادقًا محليًا مع برنامج امتداد الرمز المميز المستنسخ من
الشبكة الرئيسية باستخدام الأمر التالي. يجب أن يكون لديك واجهة سطر أوامر Solana
