...
يتيح ملحق التحويل السري إجراء تحويلات توكن خاصة من خلال إضافة حالة إضافية إلى
...
- **pending_balance_credit_counter**: يحسب عدد إضافات الرصيد المعلق الواردة من
  تعليمات الإيداع والتحويل.

- **maximum_pending_balance_credit_counter**: الحد الأقصى لعدد الإضافات المعلقة
  قبل طلب تعليمة _rs`ApplyPendingBalance`_ لتحويل الرصيد المعلق إلى الرصيد
  المتاح.

- **expected_pending_balance_credit_counter**: قيمة
  `pending_balance_credit_counter` المقدمة من العميل من خلال instruction data في
  آخر مرة تمت فيها معالجة تعليمة _rs`ApplyPendingBalance`_.

- **actual_pending_balance_credit_counter**: قيمة
  `pending_balance_credit_counter` الموجودة في token account وقت معالجة آخر
  تعليمة _rs`ApplyPendingBalance`_.

<Callout type="info">
  **الرصيد المعلق مقابل الرصيد المتاح**

يتم فصل الأرصدة السرية إلى أرصدة معلقة وأرصدة متاحة لمنع هجمات رفض الخدمة (DoS).
...
1. يقوم العميل بالبحث عن الأرصدة المعلقة والمتاحة الحالية، ويقوم بتشفير المجموع،
   ويوفر `decryptable_available_balance` مشفرة باستخدام مفتاح AES الخاص بمالك
   token account.
...
2. تتتبع عدادات الائتمان المعلقة المتوقعة والفعلية التغييرات في قيمة العداد بين
   ...
   - `actual_pending_balance_credit_counter`: قيمة
     `pending_balance_credit_counter` الموجودة في token account وقت معالجة
     تعليمة _rs`ApplyPendingBalance`_

تشير عدادات متطابقة متوقعة/فعلية إلى أن `decryptable_available_balance` يتطابق
مع `available_balance`.

عند جلب حالة حساب الرمز المميز لقراءة `decryptable_available_balance`، تتطلب
القيم المختلفة للعدادات المتوقعة/الفعلية من العميل البحث عن تعليمات
الإيداع/التحويل الأخيرة التي تطابق فرق العداد لحساب الرصيد الصحيح.
...
يتطلب إنشاء حساب رمز مميز مع امتداد النقل السري ثلاث تعليمات:
...
3. **تكوين التحويلات السرية**: استدعاء تعليمة
   ...
   لتهيئة حالة _rs`ConfidentialTransferAccount`_.

<Callout type="info">

**فقط مالك حساب الرمز المميز يمكنه تكوين حساب رمز مميز للتحويلات السرية**.
...
لتشغيل المثال، ابدأ مصادقًا محليًا مع برنامج امتدادات الرمز المميز المستنسخ من
...
<Callout type="info">
  ...
  الافتراضي. يجب عليك استنساخ برنامج Token Extension من الشبكة الرئيسية لتشغيل
  رمز المثال.
</Callout>
