...
## كيفية تحويل الرموز بشكل سري من حساب token account إلى آخر

لتحويل الرموز بشكل سري من حساب token account إلى آخر، يجب أن يكون لدى كل من
المرسل والمستلم حسابات token account مهيأة بحالة
_rs`ConfidentialTransferAccount`_ ومعتمدة للتحويلات السرية. يجب أيضًا أن يكون
لدى حساب token account الخاص بالمرسل رصيد سري متاح للتحويل منه.
...
1. قم بإنشاء
   ...
   **إثبات المساواة
   ...
   يتحقق من أن النص المشفر للرصيد الجديد المتاح بعد التحويل يطابق
   [التزام بيدرسن](https://en.wikipedia.org/wiki/Commitment_scheme) المقابل له،
   مما يضمن أن الرصيد الجديد المتاح لحساب المصدر يتم حسابه بشكل صحيح كـ
   ...
   **إثبات النطاق
   ...
   يتحقق من أن الرصيد الجديد المتاح ومبلغ التحويل (مقسم إلى بتات منخفضة/عالية)
...
4. إغلاق حسابات حالة سياق الإثبات لاسترداد الـ SOL المستخدم لإنشائها.

يوضح الرسم البياني التالي الخطوات المتضمنة في تحويل الرموز من حساب token account
الخاص بالمرسل إلى حساب token account الخاص بالمستلم.
...
لتحويل الرموز بشكل سري من حساب token account إلى آخر، يجب عليك:

- إنشاء دليل المساواة، ودليل صحة النص المشفر، ودليل النطاق من جانب العميل
- استدعاء برنامج إثبات Zk ElGamal للتحقق من الأدلة وتهيئة حسابات "حالة السياق"
- استدعاء تعليمات
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  مع توفير حسابات الإثبات الثلاثة.
- إغلاق حسابات الإثبات الثلاثة لاسترداد الـ rent.

توفر مكتبة `spl_token_client` الطرق التالية:

- طريقة `confidential_transfer_create_context_state_account` التي تنشئ حساب
  إثبات.
- طريقة `confidential_transfer_transfer` التي تستدعي تعليمات `Transfer`.
- طريقة `confidential_transfer_close_context_state_account` التي تغلق حساب
  الإثبات.

### مثال على الكود

يوضح المثال التالي كيفية نقل الرموز بشكل سري بين حسابات الرموز.

لتشغيل المثال، ابدأ validator محلي مع برنامج Token Extensions Program مستنسخ من
الشبكة الرئيسية باستخدام الأمر التالي. يجب أن يكون لديك واجهة سطر أوامر سولانا
مثبتة لبدء الـ validator المحلي.
...
---CODE-PLACEHOLDER-a9e1c2d9c95e1edb0eed3915f4ea1c9c---
