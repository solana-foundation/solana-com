...
للتبسيط، يمكنك التعامل مع حساب البرنامج كما لو كان البرنامج نفسه. عندما تستدعي
تعليمات البرنامج، فإنك تحدد عنوان حساب البرنامج (المعروف عادةً باسم "معرّف
البرنامج").

يوضح المثال التالي كيفية جلب حساب Token Program لإظهار أن حسابات البرنامج لها
نفس النوع الأساسي `Account`، باستثناء أن الحقل `executable` يتم تعيينه إلى
`true`. نظرًا لأن حسابات البرنامج تحتوي على كود قابل للتنفيذ في حقل البيانات
الخاص بها، فإننا لا نقوم بفك تشفير البيانات.
...
عند نشر برنامج سولانا، يتم تخزينه في حساب برنامج. تكون حسابات البرنامج مملوكة من
قبل [برنامج التحميل](/docs/core/programs#loader-programs). هناك عدة إصدارات من
برنامج التحميل، ولكن جميعها باستثناء loader-v3 تخزن الكود القابل للتنفيذ مباشرة
في حساب البرنامج. يقوم loader-v3 بتخزين الكود القابل للتنفيذ في "حساب بيانات
برنامج" منفصل ويشير حساب البرنامج فقط إليه. عند نشر برنامج جديد، تستخدم واجهة
سطر أوامر سولانا أحدث إصدار من برنامج التحميل افتراضيًا.
...
يحتوي loader-v3 على نوع حساب خاص للتخزين المؤقت لتحميل البرنامج أثناء النشر أو
الترقيات. في loader-v4، لا تزال هناك مخازن مؤقتة، ولكنها مجرد حسابات برنامج
عادية.
...
لا تخلط بين حسابات بيانات البرنامج هذه وحسابات بيانات البرامج (انظر أدناه).
...
حسابات جديدة. بمجرد أن ينشئ System Program حسابًا، يمكنه بعد ذلك تعيين ملكية
الحساب الجديد لبرنامج آخر.

بمعنى آخر، إنشاء حساب بيانات لبرنامج مخصص يتطلب خطوتين:
...
يوضح المثال التالي كيفية إنشاء واسترجاع حساب Token Mint مملوك لبرنامج
Token 2022.
