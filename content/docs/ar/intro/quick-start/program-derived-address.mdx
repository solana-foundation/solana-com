...
Compiling solana-frozen-abi-macro v1.16.17
Compiling solana-program v1.16.17
Compiling solana-frozen-abi v1.16.17
...
السمة _rs`#[account]`_ في برنامج Anchor تُشير إلى البنيات التي تمثل بيانات
...
- `bump` - _rs`u8`_ يخزن ["bump" seed](/docs/core/pda#canonical-bump) لاشتقاق
  عنوان مشتق من البرنامج (Program Derived Address). تخزين هذه القيمة يوفر في
  الحوسبة من خلال القضاء على الحاجة إلى إعادة حسابها في التعليمات اللاحقة.
...
نوع بيانات _rs`MessageAccount`_. يوضح قسم الاختبار عملية إنشاء وقراءة بيانات
الحساب.
...
2. قيود اختيارية (مثل _rs`#[account(mut)]`_ أو _rs`#[account(init)]`_) التي تحدد
   متطلبات إضافية
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - يحدد `payer = user` من يدفع مقابل إنشاء الحساب الجديد
...
يقوم قيدا `seeds` و `bump` معًا بتحديد عنوان الحساب كعنوان برنامج مشتق (Program
Derived Address - PDA).
...
يخبر قيد `bump` إطار Anchor بالعثور تلقائيًا على bump seed الصحيح واستخدامه.
يستخدم Anchor `seeds` و `bump` لاشتقاق عنوان البرنامج المشتق.
...
- بذرة الإزاحة لعنوان البرنامج المشتق (_rs`u8`_): 1 بايت
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - يتحقق القيدان `seeds` و `bump` من أن الحساب هو العنوان المشتق من البرنامج
     (PDA) الصحيح

3. _rs`system_program: Program<'info, System>`_
   - مطلوب لإعادة تخصيص مساحة الحساب المحتملة
   - يستدعي القيد `realloc` برنامج النظام لتعديل حجم بيانات الحساب

لاحظ أن القيد _rs`bump = message_account.bump`_ يستخدم bump seed المخزن على
_rs`message_account`_، بدلاً من جعل Anchor يعيد حسابه.

تتيح السمة _rs`#[instruction(message: String)]`_ للبنية _rs`Update`_ الوصول إلى
...
تنفذ الدالة `update` المنطق الخاص بتعديل حساب رسالة موجود. وهي تأخذ معلمتين:
...
يحدد قيد _rs`seeds`_ البذور المستخدمة لاشتقاق عنوان PDA.

### !bump

يحدد قيد _rs`bump`_ بذرة الزيادة لعنوان PDA.
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - قيود `seeds` و `bump` تتحقق من أن الحساب هو PDA الصحيح
