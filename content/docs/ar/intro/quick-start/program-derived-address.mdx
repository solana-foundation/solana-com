...
description:
  ...
  صحة الحساب، وكتابة الاختبارات. مثالي للمطورين الذين يتطلعون إلى فهم كيفية
  استخدام PDAs في برامج سولانا.
...
في ملف `lib.rs`، ستجد برنامجًا مع تعليمات [`create_message`](mention:one)،
[`update_message`](mention:two)، و [`delete_message`](mention:three) لإضافتها في
الخطوات التالية.
...
يحتوي الحقل _rs`user`_ على _rs`Pubkey`_ الذي يحدد هوية المستخدم الذي أنشأ حساب
الرسالة.
...
- `user` - _rs`Pubkey`_ الذي يحدد هوية المستخدم الذي أنشأ حساب الرسالة.
...
- `bump` - _rs`u8`_ الذي يخزن ["bump" seed](/docs/core/pda#canonical-bump)
  ...
  يوفر في الحوسبة من خلال القضاء على الحاجة إلى إعادة حسابها في التعليمات
  اللاحقة.
...
إذا لم تحدد قيمة، فإن Anchor يقوم تلقائيًا بحسابها.
...
بك، لكنها لا تؤثر على التحقق. يجب عليك استخدام أسماء وصفية للوضوح.
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - قيود `seeds` و `bump` تشتق عنوان الحساب كعنوان مشتق من البرنامج (PDA)
...
تنفذ الدالة `create` المنطق الخاص بتهيئة بيانات حساب رسالة جديد. وهي تأخذ
معلمتين:
...
يحدد قيد _rs`realloc::payer`_ الحساب الذي يدفع مقابل إعادة التخصيص.
...
قيد _rs`seeds`_ يحدد الـ seeds المستخدمة لاشتقاق عنوان PDA.

### !bump

قيد _rs`bump`_ يحدد الـ bump seed لعنوان PDA.
...
1. _rs`user: Signer<'info>`_
   ...
   - مُعلّم كقابل للتعديل (_rs`#[account(mut)]`_) لاستلام وحدات lamport من
     الحساب المغلق
   - يجب التوقيع للتأكد من أن المستخدم الصحيح فقط هو من يمكنه إغلاق حساب رسالته
...
1. _rs`_ctx: Context<Delete>`_ - توفر الوصول إلى الحسابات المحددة في بنية
   _rs`Delete`_. تُظهر صيغة _rs`_ctx`_ أن الدالة لا تستخدم السياق في جسمها.

يقوم جسم الدالة فقط بطباعة رسالة إلى سجلات البرنامج باستخدام وحدة الماكرو
_rs`msg!()`_. لا تحتاج الدالة إلى منطق إضافي لأن قيد _rs`close`_ في بنية
...
أولاً، يرسل ملف الاختبار معاملة تستدعي التعليمة `update`، وتمرير "Hello,
Solana!" كرسالة جديدة.
...
أخيراً، يقوم ملف الاختبار بتسجيل بيانات الحساب ورابط لتفاصيل المعاملة.
...
بعد إرسال المعاملة وإغلاق الحساب، تحاول العينة جلب الحساب باستخدام عنوانه
...
بيانات الحساب كقيمة null لأن الحساب لم يعد موجودًا.
