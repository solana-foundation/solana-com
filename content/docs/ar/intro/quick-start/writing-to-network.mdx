...
في هذا المثال، نحن نتصل بمصادق الاختبار المحلي الذي يعمل على `localhost:8899`.
...
قبل أن نتمكن من تحويل SOL، يحتاج حساب المرسل إلى امتلاك بعض رصيد SOL.
...
تقوم طريقة `SystemProgram.transfer()` بإنشاء تعليمة تنقل SOL من حساب
...
إنشاء معاملة وإضافة التعليمات البرمجية إلى المعاملة.

في هذا المثال، نقوم بإنشاء معاملة بتعليمات برمجية واحدة. ومع ذلك، يمكنك إضافة
تعليمات برمجية متعددة إلى معاملة واحدة.
...
توقيع وإرسال [المعاملة](mention:transaction) إلى الشبكة.

مطلوب keypair [المرسل](mention:sender) في مصفوفة الموقعين لتفويض تحويل SOL من
حسابهم.
...
Program. يتطلب ذلك تعليمتين برمجيتين:

1. استدعاء System Program لإنشاء حساب جديد.
2. استدعاء Token Extensions Program لتهيئة هذا الحساب كعملية سك.
...
إنشاء keypair جديد لاستخدامه كعنوان لحساب mint المراد إنشاؤه.

### !rentLamports

حساب الـ lamport المطلوبة لحساب Mint.

### !rentExemptionLamports

حساب الـ lamport المطلوبة للإعفاء من الـ rent
...
- [طلب إسقاط جوي](mention:airdrop) لتمويل الـ keypair
...
قم بإنشاء keypair لحساب mint.

سيتم استخدام المفتاح العام لهذا الـ keypair كعنوان لحساب mint.
...
حساب الحد الأدنى من lamports المطلوبة لحساب mint.

تقوم وظيفة `getMinimumBalanceForRentExemptMint` بحساب المقدار الدقيق من SOL
(بوحدة lamports) الذي يجب تخصيصه للبيانات في حساب mint.
...
1. [تخصيص البايتات المطلوبة](mention:space) لتخزين بيانات mint
2. [تحويل lamports](mention:lamports) من المحفظة لتمويل الحساب الجديد
3. [تعيين ملكية](mention:programId) الحساب إلى Token Extensions Program
   (`TOKEN_2022_PROGRAM_ID`)
...
[Token Extensions Program](mention:programId) لتهيئة حساب mint بالبيانات
التالية:
...
إضافة كلتا التعليمتين إلى معاملة واحدة.

من خلال دمج كلتا التعليمتين في معاملة واحدة، فإنك تضمن أن إنشاء الحساب وتهيئته
يحدثان بشكل ذري. إما أن تنجح كلتا التعليمتين، أو لا تنجح أي منهما.

هذا النهج شائع عند بناء معاملات سولانا الأكثر تعقيدًا، حيث يضمن تنفيذ جميع
التعليمات معًا.
