...
В этом разделе добавляется новый аккаунт с названием `vault_account` в структуру
_rs`Update`_. Этот аккаунт служит в качестве контролируемого программой
...
вызова инструкции System Program `transfer`. Это демонстрирует, как выполнять
...
`update` получает вызов, она внутренне вызывает инструкцию перевода System
Program.
...
`vault_account` использует такой же метод получения PDA, как и в структуре
Update.
...
перевода накопленных SOL обратно пользователю.
...
Далее, добавьте логику CPI в инструкцию `delete` для перевода SOL с аккаунта
...
_rs`signer_seeds`_ указывают опциональные seed и bump seed, используемые для
получения PDA.

### !with_signer

Метод _rs`with_signer()`_ передает seed подписывающего вместе с CPI.
...
- `cpi_context` (программу, аккаунты и подписывающий PDA)
