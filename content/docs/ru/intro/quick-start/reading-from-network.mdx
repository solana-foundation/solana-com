...
description: >-
  ...
  получение кошельков, программных аккаунтов и token mint аккаунтов с
...
1. Сгенерировать новый keypair (пару публичного/приватного ключей).
...
аккаунт, принадлежащий System Program. Все аккаунты "кошельков" — это просто
...
основном используются для хранения SOL (отслеживается в поле
`lamports`) и для подписи
транзакций.
...
| Поле                                                           | Описание                                                                                                                                                                                                 |
| -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `data` | Буфер данных аккаунта. Для аккаунтов кошельков он пустой (0 байт). Другие аккаунты используют это поле для хранения данных.                                                                              |
...
| `rentEpoch` | Устаревшее поле, связанное с неиспользуемым механизмом rent в Solana (в настоящее время не используется).                                                                                                |
...
аккаунтами кошельков и программ.
...
Программный аккаунт хранит скомпилированный байт-код для
[исходного кода](https://github.com/solana-program/token/tree/main/program)
Token Program. Вы можете просмотреть этот программный аккаунт в
...
account — это аккаунт, принадлежащий Token Program. Он хранит глобальные
...
Поле `data` аккаунта
Solana содержит необработанные байты. Чтобы осмысленно интерпретировать эти
данные, вы должны десериализовать их в соответствующий тип данных, определенный
программой, которая владеет аккаунтом.
...
Например, _shell`@solana/spl-token`_
включает функцию
[_ts`getMint()`_](mention:one),
которая помогает десериализовать поле
`data` mint account в
тип данных
[Mint](https://github.com/solana-program/token/blob/program%40v8.0.0/program/src/state.rs#L16-L30).
...
| `isInitialized` | Указывает, был ли инициализирован mint account                                     |
| `freezeAuthority` | Полномочие, которому разрешено замораживать token accounts                         |
