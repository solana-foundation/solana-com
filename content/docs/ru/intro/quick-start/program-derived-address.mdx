...
3. _rs`system_program: Program<'info, System>`_
   ...
   - За кулисами, `init` ограничение вызывает системную программу для создания
     нового аккаунта с указанным `space` и меняет владельца на текущую
     программу.
...
Метод _ts`accounts()`_ указывает счета, необходимые для инструкции
_ts`create()`_.
...
Метод _ts`fetch()`_ получает данные счета из сети.
