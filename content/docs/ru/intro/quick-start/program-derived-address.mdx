...
используемый для получения Program Derived Address (PDA). Хранение этого
...
- `bump` - _rs`u8`_, который хранит ["bump" seed](/docs/core/pda#canonical-bump)
  для получения Program Derived Address (PDA). Хранение этого значения экономит
  вычислительные ресурсы, устраняя необходимость пересчитывать его в последующих
  инструкциях.
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - Ограничения `seeds` и `bump` выводят адрес аккаунта как Program Derived
     Address (PDA)
...
Типу _rs`String`_ требуется 4 байта для хранения длины строки, а оставшаяся
длина содержит сами данные.
...
Далее добавьте инструкцию `update` для изменения `MessageAccount` новым
сообщением.
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - Ограничение `realloc` вызывает системную программу для изменения размера
     данных аккаунта
...
2. Обновляет данные аккаунта:
   - Получает доступ к `message_account` из контекста.
   - Устанавливает поле `message` в новое значение `message` из аргумента
     функции.
...
_rs`msg!()`_. Функции не требуется дополнительная логика, потому что ограничение
...
Добавьте код ниже внутрь _ts`describe()`_, но перед разделами _ts`it()`_.
...
программой, в то время как [`pg.wallet`](mention:two) дает доступ к вашему
кошельку в playground.
...
Метод _ts`fetchNullable()`_ получает данные аккаунта из сети, которые могут не
существовать.
...
<Callout type="info">
  ...
  инструкция `create` завершится с ошибкой, потому что `messageAccount` уже
