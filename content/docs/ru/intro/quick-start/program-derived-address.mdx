...
[`update`](mention:two) и [`delete`](mention:three), которые нужно добавить в
следующих шагах.
...
Перед началом выполните _shell`build`_ в терминале Playground, чтобы проверить,
что стартовая программа успешно компилируется.
...
Поле _rs`message`_ содержит _rs`String`_ с сообщением пользователя.

### !bump

Поле _rs`bump`_ хранит _rs`u8`_ ["bump" seed](/docs/core/pda#canonical-bump),
используемый для получения Program Derived Address (PDA). Сохранение этого
значения экономит вычислительные ресурсы, устраняя необходимость пересчитывать
его в последующих инструкциях.
...
- `bump` - _rs`u8`_, который хранит ["bump" seed](/docs/core/pda#canonical-bump)
  для получения Program Derived Address (PDA). Сохранение этого значения
  экономит вычислительные ресурсы, устраняя необходимость пересчитывать его в
  последующих инструкциях.
...
Соберите программу снова, выполнив _shell`build`_ в терминале.
...
Теперь добавьте инструкцию _rs`create`_, которая создает и инициализирует
...
Ограничение _rs`init`_ создает новый аккаунт.
...
В данном случае аккаунт должен соответствовать пользовательскому типу
_rs`MessageAccount`_.
...
В данном случае аккаунт должен соответствовать типу _rs`System`_, который
относится к системной программе.
...
2. Опциональные ограничения (например, _rs`#[account(mut)]`_ или
...
1. _rs`user: Signer<'info>`_
   ...
   - Требуется статус изменяемости (_rs`#[account(mut)]`_), так как он
     оплачивает создание нового аккаунта
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - Ограничение `init` создает аккаунт во время выполнения инструкции
   - Ограничения `seeds` и `bump` получают адрес аккаунта как Program Derived
     Address (PDA)
   - `payer = user` определяет, кто оплачивает создание нового аккаунта
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - За кулисами ограничение `init` вызывает System Program для создания нового
     аккаунта с указанным `space` и меняет владельца на текущую программу.
...
- _rs`b"message"`_ - Фиксированная строка как первый seed.
- _rs`user.key().as_ref()`_ - Публичный ключ аккаунта _rs`user`_ как второй
  seed.

Ограничение `bump` указывает Anchor автоматически найти и использовать
правильный bump seed. Anchor использует `seeds` и `bump` для получения PDA.

---

Расчет `space` _rs`(8 + 32 + 4 + message.len() + 1)`_ выделяет пространство для
типа данных `MessageAccount`:
...
Функция `create` реализует логику для инициализации данных нового аккаунта
...
- Устанавливает значение `bump`, используемое для получения PDA, извлеченное из
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - Ограничения `seeds` и `bump` проверяют, что аккаунт является правильным PDA

3. _rs`system_program: Program<'info, System>`_
   - Требуется для возможного перераспределения пространства аккаунта
   - Ограничение `realloc` вызывает System Program для корректировки размера
     данных аккаунта

Обратите внимание, что ограничение _rs`bump = message_account.bump`_ использует
bump seed, сохраненный в _rs`message_account`_, вместо того, чтобы Anchor
пересчитывал его.
...
Функция `update` реализует логику для изменения существующего аккаунта
сообщения. Она принимает два параметра:

1. _rs`ctx: Context<Update>`_ - Предоставляет доступ к аккаунтам, указанным в
...
В данном случае аккаунт _rs`user`_ получает lamport из закрытого
_rs`message_account`_.
...
1. _rs`user: Signer<'info>`_
   ...
   - Отмечен как изменяемый (_rs`#[account(mut)]`_) для получения lamport из
     закрываемого аккаунта
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - Ограничение `mut` указывает на изменение данных
   - Ограничения `seeds` и `bump` проверяют, что аккаунт является правильным PDA
   - Ограничение `close = user` отмечает этот аккаунт для закрытия и переводит
     его lamport на аккаунт `user`
...
1. _rs`_ctx: Context<Delete>`_ - Предоставляет доступ к аккаунтам, указанным в
   структуре _rs`Delete`_. Синтаксис _rs`_ctx`_ показывает, что функция не
   использует Context в своем теле.

Тело функции просто выводит сообщение в программные логи, используя макрос
_rs`msg!()`_. Функции не требуется дополнительная логика, поскольку ограничение
_rs`close`_ в структуре _rs`Delete`_ обрабатывает закрытие аккаунта.
...
Стартовый код также включает тестовый файл в `anchor.test.ts`.
...
Добавьте код ниже внутри _ts`describe()`_, но перед разделами _ts`it()`_.
...
программой, а [`pg.wallet`](mention:two) дает доступ к вашему кошельку в
playground.
...
Сначала тестовый файл отправляет транзакцию, вызывающую инструкцию `update`,
передавая "Hello, Solana!" как новое сообщение.
...
Метод _ts`fetchNullable()`_ получает данные аккаунта из сети, который может не
существовать.
...
Сначала этот код отправляет транзакцию, вызывающую инструкцию `delete` для
закрытия аккаунта сообщения.
...
метод возвращает null, когда аккаунт больше не существует после закрытия.
...
После подготовки тестов, запустите тестовый файл с помощью _shell`test`_ в
терминале Playground. Эта команда запускает тесты для программы, развернутой в
devnet, и выводит ссылки на SolanaFM для просмотра деталей транзакций.
...
<Callout type="info">
  ...
  инструкция `create` завершится с ошибкой, потому что аккаунт `messageAccount`
  уже существует. Только один аккаунт может существовать для данного PDA.
</Callout>
