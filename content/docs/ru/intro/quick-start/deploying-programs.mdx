...
Вы увидите новый проект, созданный с кодом программы в файле `src/lib.rs`.
...
Атрибут _rs`#[program]`_ аннотирует модуль, содержащий функции, которые
представляют инструкции программы.

### !accounts

Атрибут _rs`#[derive(Accounts)]`_ аннотирует структуру, содержащую аккаунты
программы.

### !account

Атрибут _rs`#[account]`_ определяет структуру, представляющую тип данных для
аккаунтов, которые эта программа может создавать и которыми владеет.
...
`new_account` и устанавливает его значение равным значению параметра
`data: u64`.
...
Ограничение `payer` указывает аккаунт, который оплачивает новый аккаунт.

### !space

Ограничение `space` указывает количество байтов для выделения для аккаунта.
...
1. `ctx: Context<Initialize>` - Передает в функцию аккаунты, необходимые для
   этой инструкции, как указано в структуре `Initialize`.
2. `data: u64` - Пользовательский параметр, который вы должны предоставить при
   вызове инструкции.

Тело функции [устанавливает поле `data` в `new_account`](mention:three) равным
предоставленному аргументу `data` и затем выводит сообщение в логи программы.
...
`build` компилирует программу. Байт-код программы затем сохраняется в
...
собой адрес вашей программы в блокчейне (идентификатор программы).

После сборки программы выполните _shell`deploy`_ в терминале для развертывания
программы в сети (по умолчанию devnet). Развертывание программы требует
