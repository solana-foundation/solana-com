...
Вы увидите созданный новый проект с кодом программы в файле
`src/lib.rs`.

Эта базовая программа Solana создает новый аккаунт и сохраняет в нем число.
Программа содержит одну инструкцию
(`initialize`), которая:

- Требует параметр
  _rs`data: u64`_ в качестве
  входных данных
- Создает новый аккаунт
- Сохраняет значение параметра
  _rs`data: u64`_ в данных
  аккаунта
...
Макрос _rs`declare_id!()`_
указывает адрес вашей программы в сети. Solana Playground обновляет этот адрес
при сборке проекта.

### !program

Атрибут _rs`#[program]`_
помечает модуль, содержащий функции, которые представляют инструкции программы.

### !accounts

Атрибут _rs`#[derive(Accounts)]`_
помечает структуру, содержащую аккаунты программы.

### !account

Атрибут _rs`#[account]`_
определяет структуру, представляющую тип данных для аккаунтов, которые эта
программа может создавать и которыми может владеть.

### !constraints

Атрибуты _rs`#[account(...)]`_
указывают ограничения для аккаунта.
...
_rs`ctx.accounts.new_account.data`_ обращается к
полю `data` аккаунта
`new_account` и устанавливает
его значение равным параметру
`data: u64`.
...
Ограничение `init`
указывает, что программа должна создать аккаунт. Под капотом это ограничение
вызывает системную программу для создания нового аккаунта и делает эту программу
владельцем. Как владелец программы, эта программа может записывать данные в
аккаунт.

### !payer

Ограничение `payer`
указывает аккаунт, который оплачивает создание нового аккаунта.

### !space

Ограничение `space`
указывает количество байтов, которые нужно выделить для аккаунта.
...
На данный момент это покрывает обзор кода программы на высоком уровне:
...
- Атрибут
  [`#[program]`](mention:one)
  помечает модуль, содержащий функции, которые представляют инструкции
  программы.
...
[`initialize`](mention:two)
принимает два параметра:

1. `ctx: Context<Initialize>` - Передает
   функции аккаунты, необходимые для этой инструкции, как указано в структуре
   `Initialize`.
2. `data: u64` -
   Пользовательский параметр, который вы должны предоставить при вызове
   инструкции.

Тело функции
[устанавливает поле `data` аккаунта `new_account`](mention:three)
в значение предоставленного аргумента
`data` и затем выводит
сообщение в логи программы.
...
- Атрибут
  ...
  определяет структуру, которая указывает аккаунты, необходимые для конкретной
  инструкции, где каждое поле представляет отдельный аккаунт.

  Anchor автоматически обрабатывает общие проверки безопасности, связанные с
  валидацией аккаунтов, через типы полей (например,
  `Signer<'info>`) и ограничения
  (например, `#[account(mut)]`).
...
- Атрибут
  [`#[account]`](mention:one)
  определяет структуру, которая представляет тип данных для аккаунтов, которые
  эта программа создает и которыми владеет.
...
Для сборки программы просто выполните
`build` в терминале.
Команда `build`
...
Solana Playground обновляет адрес в
_rs`declare_id!()`_. Этот адрес
представляет адрес вашей программы в сети (идентификатор программы).

После сборки программы выполните
_shell`deploy`_ в
терминале для развертывания программы в сети (devnet по умолчанию).
Развертывание программы требует выделения SOL для аккаунта в сети, который
хранит программу.

Перед развертыванием убедитесь, что у вас достаточно SOL. Вы можете получить SOL
в devnet, либо выполнив
_shell`solana airdrop 5`_ в
терминале Playground, либо используя [Веб-кран](https://faucet.solana.com/).
...
Стартовый код включает тестовый файл, расположенный в
`tests/anchor.test.ts`. Этот файл
демонстрирует, как вызвать инструкцию
...
неизменяемой без какой-либо возможности закрытия или обновления.
