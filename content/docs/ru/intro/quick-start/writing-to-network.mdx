...
рассмотрим, как записывать данные в неё. Запись в сеть Solana включает отправку
...
и атомарно (это означает, что либо все инструкции выполняются успешно, либо вся
...
для всех аккаунтов "кошельков". Чтобы перевести SOL, вы должны вызвать
инструкцию
...
Сгенерируйте новую пару ключей для использования в качестве
`sender`.
...
Запросить аирдроп SOL для пополнения
`sender`.

### !instruction

Создать инструкцию для вызова функции перевода из System Program.
...
Создайте новый keypair для использования в качестве кошелька.

### !mint

Создайте новый keypair для использования в качестве адреса mint account, который
нужно создать.

### !rentLamports

Рассчитайте количество lamports, необходимых для mint account.

### !rentExemptionLamports

Рассчитайте количество lamports, необходимых для освобождения от rent

### !createAccountInstruction

Создайте инструкцию для создания нового аккаунта с пространством для типа mint
account и принадлежащего Token Extensions Program.

### !initializeMintInstruction

Создайте инструкцию для инициализации данных нового аккаунта как типа mint
account.
...
Порядок инструкций здесь имеет значение. Инструкция
`createAccountInstruction` должна идти перед
`initializeMintInstruction`.
...
Создайте подключение и пополните кошелек
...
Сгенерировать keypair для Mint account
...
Рассчитать минимальное количество lamports для освобождения от rent
...
Создать инструкцию для создания нового аккаунта

<WithMentions>

1. [Выделить необходимое пространство](mention:one) для хранения данных mint
2. [Перевести lamports](mention:two) с кошелька для финансирования нового
   аккаунта
3. [Назначить владельца](mention:three) аккаунта программе Token Extensions
   (`TOKEN_2022_PROGRAM_ID`)
...
Создать инструкцию для инициализации Mint account
...
Добавить обе инструкции в транзакцию
...
Отправить и подтвердить транзакцию с обоими необходимыми подписантами
...
Вывести Mint account и подпись транзакции
