...
Затем закройте Windows Terminal и снова откройте терминал Linux, выполнив поиск
Ubuntu. Теперь терминал должен выглядеть как на изображении ниже, где `ctrl + v`
...
Если при установке `protobuf-compiler` вы столкнулись со следующей ошибкой,
...
обеспечения нужного вам выпуска (например, `v2.0.3`), или использовать одно из
трех символических имен каналов: `stable`, `beta` или `edge`.

При первой установке интерфейса командной строки Solana вы можете увидеть
следующее сообщение, предлагающее добавить переменную окружения `PATH`:
...
интерфейса командной строки Solana в ваш `PATH`, чтобы команда была доступна в
вашем терминале.
...
Если вы используете Mac с `zsh`, выполнение команды `export PATH`, записанной
при установке, не сохраняется после закрытия терминала.
...
После применения вышеуказанного решения, попробуйте запустить
_shell`anchor build`_ снова.
...
запускает тесты, а затем останавливает validator.
