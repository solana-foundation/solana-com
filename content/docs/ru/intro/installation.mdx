...
с проблемой, когда
`ctrl + v` (сочетание клавиш
для вставки) не работает в терминале.
...
Теперь терминал должен выглядеть как на изображении ниже, где
`ctrl + v` (сочетание клавиш
для вставки) работает.
...
Если вы столкнетесь со следующей ошибкой при установке
`protobuf-compiler`, убедитесь, что
сначала выполнили
...
Вы можете заменить
`stable` на тег релиза,
соответствующий версии программного обеспечения желаемого выпуска (например,
`v2.0.3`), или
использовать одно из трех символических имен каналов:
`stable`,
`beta` или
`edge`.

При первой установке Solana CLI вы можете увидеть следующее сообщение,
предлагающее добавить переменную окружения
`PATH`:
...
Если вы используете Mac с
`zsh`, запуск
стандартной команды
`export PATH`, полученной при
установке, не сохраняется после закрытия терминала.
...
_shell`avm use`_, чтобы
указать версию Anchor CLI для запуска в вашей системе.
...
команды _shell`anchor init`_,
требует Node.js и Yarn. (Шаблон теста на Rust доступен с использованием
...
При запуске `anchor build`, если
вы столкнетесь со следующими ошибками:
...
Смотрите [эту проблему](https://github.com/coral-xyz/anchor/issues/3392) для
...
При запуске
_shell`anchor test`_ после
создания нового проекта Anchor на Linux или WSL, вы можете столкнуться со
следующими ошибками, если у вас не установлены Node.js или Yarn:
...
URL-адрес RPC и URL-адрес Websocket указывают кластер Solana, к которому CLI
отправляет запросы.
...
эта команда не перезапишет его, если вы явно не принудите перезапись с помощью
флага `--force`.
...
Например, чтобы создать проект с названием
`my-project`, выполните:
...
Для сборки вашего проекта выполните следующую команду:

```terminal
$ anchor build
```

Скомпилированную программу можно найти в директории
`/target/deploy`.
...
Для развертывания вашего проекта выполните следующую команду:

```terminal
$ anchor deploy
```

Эта команда развертывает вашу программу в
`cluster`, указанном в
файле
...
Для тестирования вашего проекта выполните следующую команду:
