...
Solana. Примеры кода можно найти в разделе
[Основы SPL токенов](/docs/tokens/basics).
...
сети (как взаимозаменяемыми, так и невзаимозаменяемыми). Все токены в Solana
...
- **Полномочия на выпуск**: Аккаунт, уполномоченный создавать новые единицы
  токена, увеличивая предложение
- **Полномочия на заморозку**: Аккаунт, уполномоченный замораживать токены в
  token account, предотвращая их передачу или сжигание

![Mint Account](/assets/docs/core/tokens/mint-account.svg)

Полные детали, хранящиеся в каждом mint account, включают следующее:
...
Полные детали, хранящиеся в каждом token account, включают следующее:
...
Обратите внимание, что данные каждого token account включают поле
`owner`, определяющее,
кто имеет полномочия над token account. Это отличается от владельца программы,
указанного в базовом типе [Account](/docs/core/accounts#account-type), которым
является Token Program для всех token accounts.
...
детерминированно, используя предопределенные входные данные, что упрощает поиск
адреса аккаунта.
...
CLI _sh`spl-token`_ помогает
экспериментировать с SPL токенами. Примеры ниже используют терминал
[Solana Playground](https://beta.solpg.io/) для запуска команд CLI прямо в
браузере без локальной установки.

Создание токенов и аккаунтов требует SOL для депозита rent и комиссий за
транзакции. Для новых пользователей Solana Playground создайте кошелек
Playground и выполните команду
_sh`solana airdrop`_ в терминале
...
Выполните _sh`spl-token --help`_,
...
[документации](https://www.solana-program.com/docs/token)
_sh`spl-token`_.

<Callout type="info">

Адреса аккаунтов, отображаемые в выводе вашего терминала, отличаются от примеров
ниже. Используйте адреса из вашего терминала Playground при выполнении
инструкций. Например, команда
_sh`create-token`_ выводит адрес
mint account с вашим кошельком Playground в качестве mint authority.
...
Новый mint account начинается с нулевого запаса. Проверьте текущий запас с
помощью:
...
Команда _sh`supply`_ для
нового токена возвращает
`0`:
...
Команда _sh`create-account`_
создает associated token account с адресом вашего кошелька в качестве владельца.
...
Инструкция _rs`MintTo`_ в
Token Program создает новые токены. Орган, выпускающий токены, должен подписать
транзакцию. Инструкция выпускает токены на Token Account и увеличивает общее
предложение на Mint Account. Вот пример на Javascript на
...
Инструкция _rs`Transfer`_ в
Token Program обрабатывает переводы токенов. Владелец Token Account отправителя
должен подписать транзакцию. Инструкция перемещает токены между Token Accounts.
