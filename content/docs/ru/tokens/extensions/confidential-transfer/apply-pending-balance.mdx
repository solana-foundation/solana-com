---
title: Применение отложенного баланса
description:
  "Узнайте, как применить отложенный баланс, чтобы сделать средства доступными."
---

## Как применить отложенный баланс к доступному балансу

Прежде чем токены можно будет конфиденциально переводить, публичный баланс
токенов должен быть конвертирован в конфиденциальный баланс. Эта конвертация
происходит в два этапа:

1. **Конфиденциальный отложенный баланс**: Изначально токены "депонируются" из
...
Этот раздел объясняет второй этап: применение отложенного баланса к доступному
балансу.

Когда токены "депонируются" из публичного баланса или когда токены
конфиденциально переводятся с одного token account на другой, токены изначально
добавляются к конфиденциальному отложенному балансу. Прежде чем токены можно
будет использовать для конфиденциальных переводов, отложенный баланс должен быть
...
Следующая диаграмма показывает шаги, необходимые для применения отложенного
...
Чтобы конвертировать отложенный баланс в доступный баланс, вызовите инструкцию
...
предоставляет метод
`confidential_transfer_apply_pending_balance`, который создает
и отправляет транзакцию с инструкцией
`ApplyPendingBalance`, как показано в
примере ниже.
...
Program, клонированной с основной сети, используя следующую команду. У вас
...
<Callout type="info">
  ...
  локальном validator. Вы должны клонировать программу Token Extensions Program
  из основной сети, чтобы запустить пример кода.
</Callout>
