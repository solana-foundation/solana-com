...
Конфиденциальные переводы позволяют переводить токены между токен-аккаунтами без
раскрытия суммы перевода. Это полезно для транзакций, сохраняющих
конфиденциальность. Только суммы переводов и балансы токенов являются
...
- [Обзор протокола](https://www.solana-program.com/docs/confidential-balances/overview) -
  Подробности о базовом криптографическом протоколе
...
- [Сборник рецептов по конфиденциальным балансам](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Фрагменты кода о том, как использовать расширение Confidential Transfer
...
в программу Token Extensions, которая позволяет переводить токены между
аккаунтами без раскрытия суммы перевода.
...
2. Создайте token account с расширением конфиденциального перевода для
   отправителя и получателя.
3. Выпустите токены на аккаунт отправителя.
4. **Внесите** публичный баланс отправителя на **конфиденциальный ожидающий
   баланс**.
...
7. **Примените** ожидающий баланс получателя к **конфиденциальному доступному
   балансу**.
8. **Выведите** конфиденциальный доступный баланс получателя на **публичный
   баланс**.

Для получения более подробной информации о шагах в процессе конфиденциального
...
<Cards>
  ...
  >
    Как конфиденциально переводить токены между токен-аккаунтами
  </Card>
</Cards>

Диаграмма ниже показывает подробную последовательность базового процесса
...
Чтобы запустить пример, запустите локальный validator с программой Token
Extension Program, клонированной с основной сети, используя следующую команду. У
вас должен быть установлен Solana CLI для использования локального validator.
...
<Callout type="info">
  На момент написания, Confidential Transfers не включены в стандартном
  локальном validator. Вы должны клонировать Token Extension Program с основной
  сети для запуска примера кода.
</Callout>
...
## Инструкции Confidential Transfer
...
расширения Confidential Transfer:

| Инструкция                          | Описание                                                                                                                                                                         |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Настраивает mint аккаунт для конфиденциальных переводов. Эта инструкция должна быть включена в ту же транзакцию, что и инструкция _rs`TokenInstruction::InitializeMint`_.        |
| _rs`UpdateMint`_                    | Обновляет настройки конфиденциальных переводов для mint.                                                                                                                         |
| _rs`ConfigureAccount`_              | Настраивает token аккаунт для конфиденциальных переводов.                                                                                                                        |
| _rs`ApproveAccount`_                | Одобряет token аккаунт для конфиденциальных переводов, если mint требует одобрения для новых token аккаунтов.                                                                    |
| _rs`EmptyAccount`_                  | Опустошает ожидающие и доступные конфиденциальные балансы, чтобы позволить закрыть token аккаунт.                                                                                |
...
| _rs`Transfer`_                      | Конфиденциально переводит токены между token аккаунтами.                                                                                                                         |
| _rs`ApplyPendingBalance`_           | Конвертирует ожидающий баланс в доступный баланс после депозитов или переводов.                                                                                                  |
| _rs`EnableConfidentialCredits`_     | Позволяет token аккаунту получать конфиденциальные переводы токенов.                                                                                                             |
| _rs`DisableConfidentialCredits`_    | Блокирует входящие конфиденциальные переводы, при этом разрешая публичные переводы.                                                                                              |
| _rs`EnableNonConfidentialCredits`_  | Позволяет token аккаунту получать публичные переводы токенов.                                                                                                                    |
| _rs`DisableNonConfidentialCredits`_ | Блокирует обычные переводы, чтобы аккаунт получал только конфиденциальные переводы.                                                                                              |
| _rs`TransferWithFee`_               | Конфиденциально переводит токены между token аккаунтами с комиссией.                                                                                                             |
| _rs`ConfigureAccountWithRegistry`_  | Альтернативный способ настройки token аккаунтов для конфиденциальных переводов с использованием аккаунта _rs`ElGamalRegistry`_ вместо доказательства _rs`VerifyPubkeyValidity`_. |
