...
description: >-
  ...
  добавления дополнительных функций к токенам и счетам.
---

## Что такое конфиденциальные переводы?

Конфиденциальные переводы позволяют переводить токены между счетами без
раскрытия суммы перевода. Это полезно для транзакций, требующих
конфиденциальности. Приватными остаются только суммы переводов и балансы
токенов. Адреса токен-счетов остаются публичными.
...
- [Сборник примеров по конфиденциальным балансам](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Фрагменты кода по использованию расширения Confidential Transfer
...
в программу Token Extensions, которая позволяет переводить токены между счетами
...
3. Выпустите токены на счет отправителя.
...
5. **Примените** ожидающий баланс отправителя к **конфиденциальному доступному
   балансу**.
6. Конфиденциально **переведите** токены со счета отправителя на счет
   получателя.
...
<Cards>
  ...
  >
    Как конфиденциально переводить токены между token account
  </Card>
</Cards>
...
расширения Confidential Transfer приведен ниже:
...
| _rs`ApproveAccount`_ | Одобряет token account для конфиденциальных переводов, если mint требует одобрения для новых token account.                                                                                                                                                         |
| _rs`EmptyAccount`_ | Опустошает ожидающие и доступные конфиденциальные балансы, чтобы позволить закрыть token account.                                                                                                                                                                   |
