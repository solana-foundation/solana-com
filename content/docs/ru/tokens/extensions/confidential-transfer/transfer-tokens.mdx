...
description:
  "Узнайте, как приватно переводить токены с одного токен-аккаунта на другой."
---

## Как конфиденциально переводить токены с одного токен-аккаунта на другой

Для конфиденциального перевода токенов с одного токен-аккаунта на другой, и
отправитель, и получатель должны иметь токен-аккаунты, настроенные с состоянием
_rs`ConfidentialTransferAccount`_ и одобренные
для конфиденциальных переводов. Токен-аккаунт отправителя также должен иметь
доступный конфиденциальный баланс для перевода.

Для конфиденциального перевода токенов:
...
1. Создайте
   ...
   **Доказательство равенства
   ...
   проверяет, что новый доступный баланс в зашифрованном виде после перевода
   соответствует его
   ...
   **Доказательство действительности шифротекста
   ...
   корректно зашифрована под открытым ключом каждой стороны.
   ...
   **Доказательство диапазона
   ...
   проверяет, что новый доступный баланс и сумма перевода (разделенная на
   младшие/старшие биты) являются неотрицательными и находятся в указанном
   диапазоне.
...
2. Для каждого доказательства:
   ...
   - Вызовите программу доказательства ZK ElGamal для проверки данных
     доказательства.
   - Сохраните специфичные для доказательства метаданные в аккаунте
     "контекстного состояния" доказательства для использования в других
     инструкциях.
...
3. Вызовите инструкцию
   ...
   предоставив аккаунты контекстного состояния доказательства.

4. Закройте аккаунты контекстного состояния доказательства, чтобы вернуть SOL,
   использованный для их создания.

Следующая диаграмма показывает шаги, необходимые для перевода токенов с
токен-аккаунта отправителя на токен-аккаунт получателя.
...
Для конфиденциального перевода токенов с одного токен-аккаунта на другой, вы
должны:

- Генерация доказательства равенства, доказательства действительности
  шифротекста и доказательства диапазона на стороне клиента
- Вызов программы Zk ElGamal proof для проверки доказательств и инициализации
  аккаунтов "context state"
- Вызов инструкции
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  с предоставлением трех аккаунтов доказательств
- Закрытие трех аккаунтов доказательств для возврата rent

Крейт `spl_token_client`
предоставляет следующие методы:

- Метод `confidential_transfer_create_context_state_account`, который
  создает аккаунт доказательства
- Метод `confidential_transfer_transfer`, который
  вызывает инструкцию
  `Transfer`
- Метод `confidential_transfer_close_context_state_account`, который
  закрывает аккаунт доказательства
...
Extensions Program, клонированной с основной сети, используя следующую команду.
У вас должен быть установлен Solana CLI для запуска локального validator.
...
<Callout type="info">
  ...
  локальном validator. Вы должны клонировать Token Extensions Program с основной
