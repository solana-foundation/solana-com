...
description: >-
  Узнайте, как конфиденциально переводить токены с одного токен-аккаунта на
  другой.
---
...
- Генерировать доказательство равенства, доказательство действительности
  шифротекста и доказательство диапазона на стороне клиента
- Вызвать программу Zk ElGamal proof для проверки доказательств и инициализации
  счетов "context state"
- Вызвать инструкцию
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604),
  предоставив три счета доказательств
- Закрыть три счета доказательств для возврата rent

Крейт `spl_token_client`
предоставляет следующие методы:

- Метод `confidential_transfer_create_context_state_account`, который
  создает счет доказательства
- Метод `confidential_transfer_transfer`, который
  вызывает инструкцию
  `Transfer`
- Метод `confidential_transfer_close_context_state_account`, который
  закрывает счет доказательства
...
Чтобы запустить пример, запустите локальный validator с Token Extensions
Program, клонированной из основной сети, используя следующую команду. У вас
должен быть установлен Solana CLI для запуска локального validator.
...
<Callout type="info">
  ...
  локальном validator. Вы должны клонировать Token Extensions Program из
  основной сети, чтобы запустить пример кода.
</Callout>
