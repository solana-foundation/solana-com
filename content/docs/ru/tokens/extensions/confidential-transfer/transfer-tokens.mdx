...
1. Создайте
   ...
   **Доказательство равенства
   ...
   Подтверждает, что новый зашифрованный доступный баланс после перевода
   ...
   **Доказательство корректности шифротекста
   ...
   Подтверждает, что шифротексты суммы перевода корректно сгенерированы для всех
   ...
   **Доказательство диапазона
   ...
   Подтверждает, что новый доступный баланс и сумма перевода (разделенные на
   младшие/старшие биты) являются ненулевыми и находятся в заданном диапазоне.
...
Следующая диаграмма показывает шаги, связанные с переводом токенов с
...
Чтобы конфиденциально перевести токены с одного токен-аккаунта на другой, вам
необходимо:

- Сгенерируйте доказательство равенства, доказательство корректности шифротекста
  и доказательство диапазона на стороне клиента.
- Вызовите программу доказательства Zk ElGamal для проверки доказательств и
  инициализации аккаунтов "состояния контекста".
- Вызовите инструкцию
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604),
  предоставив три аккаунта доказательств.
- Закройте три аккаунта доказательств, чтобы вернуть rent.
...
локального validator необходимо установить Solana CLI.
...
<Callout type="info">
  ...
  локальный validator. Для запуска примера кода необходимо клонировать программу
  Token Extension из mainnet.
</Callout>
...
---CODE-PLACEHOLDER-1e1b0f4b2c3a4c5d6e7f8g9h0i1j2k3l---
