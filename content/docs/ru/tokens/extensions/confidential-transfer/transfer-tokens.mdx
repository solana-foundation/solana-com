...
_rs`ConfidentialTransferAccount`_ и одобренные для конфиденциальных переводов.
Токен-аккаунт отправителя также должен иметь доступный конфиденциальный баланс
для перевода.
...
Крейт `spl_token_client` предоставляет следующие методы:

- Метод `confidential_transfer_create_context_state_account`, который создает
  аккаунт доказательства
- Метод `confidential_transfer_transfer`, который вызывает инструкцию `Transfer`
