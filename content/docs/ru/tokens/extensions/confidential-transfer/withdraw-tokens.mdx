---
title: Вывод токенов
description: "Узнайте, как выводить токены из конфиденциального состояния."
---

## Как выводить токены из конфиденциального доступного баланса
...
1. Создайте
   ...
   **Доказательство равенства
   ...
   Проверяет, что оставшийся шифротекст доступного баланса после вывода
   соответствует его
   [обязательству Педерсена](https://en.wikipedia.org/wiki/Commitment_scheme),
   гарантируя, что новый доступный баланс аккаунта правильно вычислен как
...
3. Вызовите инструкцию
   ...
   предоставив два аккаунта доказательства.

4. Закройте аккаунты доказательства, чтобы вернуть SOL, использованный для их
   создания.

Следующая диаграмма показывает шаги, необходимые для вывода токенов из
...
- Вызвать инструкцию
  [ConfidentialTransferInstruction::Withdraw](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L493),
  предоставив два аккаунта доказательства.
- Закрыть два аккаунта доказательства, чтобы вернуть rent.
...
- Метод `confidential_transfer_withdraw`, который
  вызывает инструкцию
  `Withdraw`.
...
<Callout type="info">
  ...
  сети, чтобы запустить пример кода.
</Callout>
