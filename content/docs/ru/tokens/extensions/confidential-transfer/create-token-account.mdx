...
На следующей схеме показаны шаги, необходимые для создания аккаунта токена с
...
- **approved**: Статус одобрения аккаунта для конфиденциальных переводов. Если
  конфигурация аккаунта токена `auto_approve_new_accounts` установлена как
  `true`, все аккаунты токенов автоматически одобряются для конфиденциальных
  переводов.
...
- **pending_balance_credit_counter**: Счетчик входящих ожидаемых кредитов
  баланса из инструкций депозита и перевода.

- **maximum_pending_balance_credit_counter**: Лимит счетчика ожидаемых кредитов
  перед необходимостью выполнения инструкции _rs`ApplyPendingBalance`_ для
  преобразования ожидаемого баланса в доступный баланс.
...
- **actual_pending_balance_credit_counter**: Значение
  `pending_balance_credit_counter` в token account на момент, когда инструкция
  _rs`ApplyPendingBalance`_ была обработана.

<Callout type="info">
  **Ожидаемый баланс против доступного баланса**

Конфиденциальные балансы разделены на ожидаемые и доступные, чтобы предотвратить
DoS-атаки. Без этого разделения злоумышленник мог бы неоднократно отправлять
токены на token account, блокируя возможность владельца token account переводить
токены. Владелец token account не смог бы перевести токены, так как
зашифрованный баланс изменился бы между моментом отправки транзакции и её
обработкой, что привело бы к неудачной транзакции.
...
_rs`ApplyPendingBalance`_, чтобы конвертировать ожидаемый баланс в доступный.
...
При вызове инструкции _rs`ApplyPendingBalance`_ для конвертации ожидаемого
баланса в доступный:

1. Клиент проверяет текущие ожидаемый и доступный балансы, шифрует сумму и
   предоставляет `decryptable_available_balance`, зашифрованное с использованием
   AES-ключа владельца token account.

2. Ожидаемые и фактические счётчики кредитов отслеживают изменения значения
   счётчика между моментом создания и обработки инструкции
...
`decryptable_available_balance`, различие значений ожидаемых/фактических
...
Создание token account с расширением Confidential Transfer требует выполнения
трех инструкций:

1. **Создайте token account**: Вызовите инструкцию
   _rs`AssociatedTokenAccountInstruction:Create`_ из Associated Token Program
   для создания token account.

2. **Перераспределите пространство аккаунта**: Вызовите инструкцию
   _rs`TokenInstruction::Reallocate`_ из Token Extension Program, чтобы добавить
   пространство для состояния _rs`ConfidentialTransferAccount`_.

3. **Настройте Confidential Transfers**: Вызовите инструкцию
   ...
   из Token Extension Program для инициализации состояния
   _rs`ConfidentialTransferAccount`_.

<Callout type="info">

**Только владелец token account может настроить аккаунт для конфиденциальных
переводов**.
...
клонированной из основной сети, используя следующую команду. Для запуска
...
---CODE-PLACEHOLDER-2e1b8f3c9d3a4b6e8f7c9a1d5e6c7a8d---
