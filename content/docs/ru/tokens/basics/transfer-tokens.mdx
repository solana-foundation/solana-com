...
Перевод токенов подразумевает перемещение токенов с одного token account на
другой token account, которые имеют один и тот же mint. Перевод токенов
...
в токен-программе. Только адрес, указанный как владелец (authority) исходного
token account, может переводить токены с этого аккаунта.
