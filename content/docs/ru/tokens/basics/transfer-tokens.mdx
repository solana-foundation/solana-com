...
Перевод токенов подразумевает перемещение токенов с одного токен аккаунта на
другой токен аккаунт, которые связаны с одним и тем же mint. Перевод токенов
происходит при вызове инструкции
...
в token program. Только адрес, указанный как владелец (authority) исходного
токен аккаунта, может переводить токены с этого аккаунта.
