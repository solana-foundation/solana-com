...
description: Узнайте, как переводить токены между аккаунтами токенов.
---

## Как перевести токены

Перевод токенов включает перемещение токенов из одного аккаунта токенов в другой
аккаунт токенов, которые имеют один и тот же mint. Перевод токенов
осуществляется при вызове инструкции
...
в токен-программе. Только адрес, указанный как владелец (авторитет) исходного
аккаунта токенов, может переводить токены из аккаунта.

<Callout type="info">
  [Токен-программа](https://github.com/solana-program/token/blob/3daf44899f0bd71c879d28dffdfb788dd944f3c5/program/src/processor.rs#L227)
  и [Программа расширений
  токенов](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/processor.rs#L300)
