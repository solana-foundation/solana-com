...
Выпуск токенов (минтинг) — это процесс создания новых единиц токена путем вызова
инструкции
...
в Token Program. Только адрес, указанный как mint authority в mint account,
может выпускать новые токены. Инструкция также требует наличия token account в
качестве получателя выпущенных токенов.
