...
Mint account — это тип аккаунта в Token Programs Solana, который уникально
...
<Callout type="info">
  ...
  используют одну и ту же базовую реализацию для аккаунта Mint.
</Callout>

Каждый токен в Solana существует как mint account, где адрес mint account
является его уникальным идентификатором в сети.

Например, USD Coin (USDC) цифровой доллар в Solana имеет mint адрес
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`. Этот mint адрес
служит уникальным идентификатором USD Coin во всей экосистеме Solana. Вы можете
просмотреть детали этого mint аккаунта на
[Solana Explorer](https://explorer.solana.com/address/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v).

## Как создать Mint Account

Чтобы создать Mint Account, вызовите инструкцию
...
Вы можете найти реализации этой инструкции
...
Транзакция для создания mint аккаунта требует двух инструкций:

1. Вызвать System Program для создания и выделения пространства для mint
   аккаунта и передачи права собственности Token Program.
2. Вызвать Token Program для инициализации данных mint аккаунта.
