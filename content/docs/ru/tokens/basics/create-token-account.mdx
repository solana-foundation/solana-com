...
<Callout type="info">
  ...
  структуру типа `Account`, как определено в token program. Associated Token
  Program не сохраняет состояние - она просто предлагает стандартизированный
  способ создания token account по детерминированному адресу, который является
  PDA.
</Callout>
