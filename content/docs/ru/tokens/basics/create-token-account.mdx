...
<Callout type="info">
  Обратите внимание, что в исходном коде разработчики называют аккаунт токена
  ...
  используют одну и ту же базовую реализацию для аккаунта токена.
</Callout>
...
- Адрес минта USD Coin: `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Circle (эмитент USD Coin) имеет token account по адресу
  `3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
- Этот token account содержит только единицы токена USD Coin (минта)
- Circle имеет права владельца по адресу
...
2. "Владелец" программы - это относится к программе, которая владеет данными
   ...
   Token Extensions Program, как указано в поле "owner" базового типа Solana
   [Account](https://github.com/anza-xyz/solana-sdk/blob/4e30766b8d327f0191df6490e48d9ef521956495/account/src/lib.rs#L51).

При работе с token account, термин "владелец" обычно относится к полномочию,
...
Associated token account предоставляют детерминированный способ нахождения token
...
кошелька и токенового mint существует ровно один адрес associated token account.
Такой подход упрощает поиск token account пользователя для любого заданного
токенового mint, устраняя необходимость отдельно отслеживать адреса token
account.
...
<Callout type="info">
  ...
  Program. Программа токенов владеет созданным аккаунтом, который имеет такую же
  структуру типа `Account`, как определено в программе токенов. Associated Token
  Program не хранит состояние - она просто предлагает стандартизированный способ
  создания token account по детерминированному адресу, который является PDA.
</Callout>
...
1. Вызвать System Program для создания и выделения пространства для
   токен-аккаунта и передачи права собственности Token Program.
...
данных токен-аккаунта. Это происходит через Cross Program Invocation (CPI).
