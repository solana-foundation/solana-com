...
- Каждая инструкция требует трех элементов информации:
  ...
  2. Счета, из которых инструкция читает или в которые записывает
...
Если библиотека недоступна, вы можете вручную создать инструкцию. Это требует
знания деталей реализации инструкции.
...
В разделах ниже мы подробно рассмотрим транзакции и инструкции.

## Инструкции

[Инструкцию](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/instruction/src/lib.rs#L94)
для [программы](/docs/core/accounts#program-account) Solana можно рассматривать
...
Каждый аккаунт, требуемый инструкцией, должен быть предоставлен в виде
...
зависит от SDK, но каждая инструкция Solana требует следующей информации:
...
необходимых для выполнения инструкций. Массив начинается с числа
[compact-u16](/docs/core/transactions#compact-array-format), указывающего
количество содержащихся в нем адресов. Затем адреса упорядочиваются по их
привилегиям, как определено в заголовке сообщения.
...
для получения текущего блокхеша и высоты последнего блока, при которой блокхеш
...
1. Индекс Program ID: Индекс u8, который указывает на адрес программы в массиве
...
Когда вы получаете транзакцию, используя её подпись после отправки в сеть, вы
