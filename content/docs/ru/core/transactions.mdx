...
Счет отправителя должен подписать (`is_signer`) транзакцию, чтобы позволить
Системной программе уменьшить его баланс в лампортах. Счета отправителя и
получателя должны быть доступны для записи (`is_writable`), поскольку их балансы
в лампортах изменяются.
...
блокхеша и высоты последнего блока, при которой блокхеш будет действителен. Вот
пример на [Solana Playground](https://beta.solpg.io/661a06e1cffcf4b13384d046).
...
- `recentBlockhash`: Блокчейн-хеш, используемый для временной метки транзакции
...
- `signatures`: Массив, включающий подписи для всех аккаунтов, требуемых в
  качестве подписантов инструкциями в транзакции. Подпись создается путем
