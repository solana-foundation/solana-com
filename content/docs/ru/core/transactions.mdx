...
Системной Программе вычесть баланс lamport. Аккаунты отправителя и получателя
должны быть доступны для записи (`is_writable`), так как их балансы lamport
изменяются.
...
[`getLatestBlockhash`](/docs/rpc/http/getlatestblockhash) для получения текущего
хеша блока и высоты последнего блока, на которой хеш блока будет действителен.
Вот пример на
...
- `header`: Определяет привилегии чтения/записи и подписи для адресов в массиве
  `accountKeys`
...
- `instructions`: Массив инструкций для выполнения. Каждый `account` и
  `programIdIndex` в инструкции ссылается на массив `accountKeys` по индексу.

- `signatures`: Массив, включающий подписи для всех аккаунтов, требуемых как
  подписанты инструкциями в транзакции. Подпись создается путем подписания
  сообщения транзакции с использованием соответствующего приватного ключа для
  аккаунта.
