...
Отправка транзакции подобна отправке конверта для обработки форм.
...
- Транзакции являются **атомарными** — если какая-либо инструкция не
  выполняется, вся транзакция не выполняется, и изменения не происходят.
...
- Каждая инструкция требует три элемента информации:
  ...
  2. Аккаунты, из которых инструкция читает данные или в которые записывает
     данные
...
получателя должны быть доступными для записи (`is_writable`), так как их балансы
в лампортах изменяются.

После отправки транзакции Системная программа обрабатывает инструкцию перевода.
Затем Системная программа обновляет балансы в лампортах как отправителя, так и
получателя.
...
Клиентские библиотеки часто абстрагируют детали создания инструкций для
программ. Если библиотека недоступна, вы можете вручную создать инструкцию. Для
этого необходимо знать детали реализации инструкции.

Примеры ниже показывают, как вручную создать инструкцию передачи. Вкладка
...
отправке `POST` запроса к конечной точке API, что позволяет пользователям
...
`AccountMeta` определяет следующее:

- [pubkey](mention:pubkey): адрес аккаунта
- [is_signer](mention:is-signer): должен ли аккаунт подписывать транзакцию
...
Указывая заранее, из каких аккаунтов инструкция читает или в какие записывает,
транзакции, которые не изменяют одни и те же аккаунты, могут выполняться
параллельно.

<Callout type="info">

Чтобы узнать, какие аккаунты требуются для инструкции, включая те, которые
должны быть доступными для записи, только для чтения или подписывать транзакцию,
необходимо обратиться к реализации инструкции, определённой программой.
...
- [Accounts](mention:accounts): Список аккаунтов, необходимых для выполнения
  ...
  необходимость подписи транзакции и будет ли он изменен.
...
После того как вы создали инструкции, которые хотите выполнить, следующим шагом
будет создание `Transaction` и добавление инструкций в транзакцию. Транзакция
Solana
...
- [Недавний блокхеш](mention:recent-blockhash): Выполняет роль временной метки
  для транзакции.
...
(MTU) в 1280 байт, за вычетом 48 байт для сетевых заголовков (40 байт IPv6 + 8
байт заголовка).
...
определяет права доступа к аккаунтам в транзакции. Он работает в сочетании со
...
2. [Количество подписанных аккаунтов, доступных только для чтения.](mention:num_readonly_signed_accounts)
3. [Количество неподписанных аккаунтов, доступных только для чтения.](mention:num_readonly_unsigned_accounts)
...
требуемых её инструкциями. Массив начинается с числа в формате
[compact-u16](/docs/core/transactions#compact-array-format), указывающего
количество адресов в массиве.

Чтобы сэкономить место, транзакция не хранит права доступа для каждого аккаунта
по отдельности. Вместо этого она использует комбинацию `MessageHeader` и строгий
порядок адресов аккаунтов для определения прав доступа.
...
аккаунтов в каждой группе прав доступа.
...
1. Выступает в качестве временной метки для создания транзакции
...
считается истёкшей и не может быть обработана.
...
3. [Instruction Data](mention:instruction-data): Массив байтов, который
   определяет, какую инструкцию вызвать в программе, а также любые
   дополнительные данные, необходимые для выполнения инструкции (например,
   аргументы функции).
