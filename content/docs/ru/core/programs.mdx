...
- Пользователи могут проверить, соответствуют ли данные аккаунта программы в
  сети её публичному исходному коду через проверяемые сборки.
...
Чтобы узнать больше о развертывании и обновлении программ, см. страницу
...
отозвано и установлено в
`None`, программа
становится неизменяемой и больше не может быть обновлена.
...
- **Поиск проверенных программ**: Чтобы быстро проверить верифицированные
  программы, пользователи могут искать адрес программы в
...
развертывания. Разрешение на это регулируется полномочиями программы, поскольку
владение аккаунтом каждой программы принадлежит загрузчику.
...
Программа ed25519 обрабатывает инструкцию. Первый
`u8` - это количество
...
| Программа           | ID программы                                                   | Описание                                                                                                         | Инструкции                                                                                        |
| ------------------- | -------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| Программа Secp256r1 | `Secp256r1SigVerify1111111111111111111111111` | Проверяет до 8 подписей secp256r1. Принимает подпись, публичный ключ и сообщение. Возвращает ошибку при неудаче. | [Инструкции](https://docs.rs/solana-secp256k1-recover/latest/solana_secp256k1_recover/index.html) |

Программа secp256r1 обрабатывает инструкцию. Первый
`u8` - это количество
...
случайной податливости подписи.
...
назывались "нативными" программами и распространялись вместе с кодом validator.
...
| **Программа конфигурации**           | `Config1111111111111111111111111111111111111` | Добавление данных конфигурации в цепочку, за которыми следует список публичных ключей, которым разрешено их изменять. В отличие от других программ, программа Config не определяет отдельных инструкций. У неё есть только одна неявная инструкция: "store". Её данные инструкции - это набор ключей, которые контролируют доступ к аккаунту, и данные для хранения в нём. | [ConfigInstruction](https://docs.rs/solana-config-program/latest/solana_config_program/config_instruction/index.html)                                 |
...
| **Программа таблицы поиска адресов** | `AddressLookupTab1e1111111111111111111111111` | Управление таблицами поиска адресов, которые позволяют транзакциям ссылаться на большее количество аккаунтов, чем могло бы поместиться в список аккаунтов транзакции.                                                                                                                                                                                                      | [ProgramInstruction](https://docs.rs/solana-sdk/latest/solana_sdk/address_lookup_table/instruction/enum.ProgramInstruction.html)                      |
| **Программа ZK ElGamal Proof**       | `ZkE1Gama1Proof11111111111111111111111111111` | Обеспечивает верификацию доказательств с нулевым разглашением для данных, зашифрованных по схеме ElGamal.                                                                                                                                                                                                                                                                  | —                                                                                                                                                     |
