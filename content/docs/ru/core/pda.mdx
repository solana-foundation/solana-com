...
Наличие адреса не означает, что там что-то построено.

<Callout type="info">
  Этот раздел охватывает детали получения PDA. Раздел о [Cross Program
...
- PDA - это адреса, которые не лежат на кривой Ed25519 и **не имеют
...
Вывод PDA требует трех входных данных:
...
- Предопределенные опциональные seeds, преобразованные в байты
...
#### Получение PDA с несколькими опциональными seeds
...
опциональным сидам. Функция получения перебирает значения бампа, начиная с 255 и
уменьшая на 1, пока не будет получен действительный адрес вне кривой. Первое
значение бампа, которое дает действительный адрес вне кривой, является
"каноническим бампом".

Следующие примеры показывают получение PDA с использованием всех возможных
значений bump seed (от 255 до 0):
...
Значение bump seed 255 вызывает ошибку, и первым значением bump seed, которое
дает действительный PDA, является 254.

Обратите внимание, что значения bump seed 253-251 также дают действительные PDA
с разными адресами. Это означает, что при одинаковых опциональных сидах и
`programId`, bump seed с
другим значением все равно может дать действительный PDA.

<Callout type="warn">
  При создании программ Solana всегда включайте проверки безопасности, чтобы
  убедиться, что PDA, переданный программе, получен с использованием
  канонического бампа. Отсутствие таких проверок может создать уязвимости,
  позволяющие использовать неожиданные аккаунты в инструкциях программы. Лучшей
  практикой является использование только канонического бампа при получении PDA.
</Callout>
...
Программа включает одну инструкцию
`initialize` для создания
нового аккаунта, используя PDA в качестве адреса аккаунта. Новый аккаунт хранит
адрес `user` и
`bump` сид,
использованный для получения PDA.
...
В этом примере seed для получения PDA включают фиксированную строку
`data` и адрес аккаунта
`user`, предоставленный
в инструкции. Фреймворк Anchor автоматически находит канонический seed
`bump`.
...
Ограничение `init`
указывает Anchor вызвать системную программу для создания нового аккаунта,
используя PDA в качестве адреса. Anchor делает это через [CPI](/docs/core/cpi).
...
Транзакция в тестовом файле вызывает инструкцию
`initialize` для создания
нового аккаунта в блокчейне, используя PDA в качестве адреса. В этом примере
Anchor может определить адрес PDA в аккаунтах инструкции, поэтому его не нужно
указывать явно.
...
Обратите внимание, что в этом примере, если вы вызовете инструкцию
`initialize` более одного
раза, используя тот же адрес
`user` в качестве seed,
транзакция завершится с ошибкой. Это происходит потому, что аккаунт по
полученному адресу уже существует.
