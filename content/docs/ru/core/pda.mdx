...
адресами. Это означает, что при одинаковых опциональных seeds и
...
Программа включает одну
`initialize` инструкцию для
создания нового аккаунта, используя PDA в качестве адреса аккаунта. Новый
аккаунт хранит адрес
`user` и
`bump` seed,
используемый для получения PDA.
...
нового аккаунта в сети, используя PDA в качестве адреса. В этом примере Anchor
