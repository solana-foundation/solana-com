...
description: >-
  Узнайте о межпрограммном вызове (CPI) в Solana - как программы могут вызывать
  инструкции других программ, обрабатывать подписи PDA и объединять
  функциональность по всей сети Solana.
h1: Межпрограммный вызов (CPI)
---

Межпрограммный вызов (Cross Program Invocation, CPI) означает ситуацию, когда
одна программа вызывает инструкции другой программы. Это обеспечивает
композиционность программ Solana.

Инструкции можно представить как конечные точки API, которые программа
предоставляет сети, а CPI - как один API, внутренне вызывающий другой API.

![Межпрограммный вызов](/assets/docs/core/cpi/cpi.svg)

## Ключевые моменты

- **Межпрограммные вызовы** позволяют инструкциям программы Solana напрямую
  вызывать инструкции другой программы.
- **Привилегии подписи** от вызывающей программы распространяются на вызываемую
  программу.
- При выполнении межпрограммного вызова **программы могут подписываться от имени
  PDA**, полученных из их собственного идентификатора программы.
- Вызываемая программа может делать дальнейшие CPI к другим программам, до
  глубины 4.

## Что такое CPI?

Межпрограммный вызов (CPI) - это когда одна программа вызывает инструкции другой
программы.
...
Внутри каждая инструкция CPI должна указывать:
...
- **Данные инструкции**: Указывает, какую инструкцию вызвать в программе, плюс
  любые данные, которые нужны инструкции (аргументы функции)

Когда программа делает межпрограммный вызов (CPI) к другой программе:
...
При обработке транзакции привилегии аккаунта распространяются от одной программы
к другой. Вот что это означает:
...
`invoke_signed` с пустым массивом
`signers_seeds`, что указывает на
отсутствие необходимости в PDA для подписи.
...
Следующие примеры представляют три способа создания межпрограммных вызовов
...
- Пример 1: Использует
  _rs`CpiContext`_ от Anchor и
  вспомогательную функцию для создания инструкции CPI.
- Пример 2: Использует функцию
  _rs`system_instruction::transfer`_ из крейта
  `solana_program` для создания
  инструкции CPI. Пример 1 абстрагирует эту реализацию.
...
## Межпрограммные вызовы с подписантами PDA

Функция
[`invoke_signed`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/program.rs#L51-L73)
обрабатывает CPI, требующие подписантов PDA. Функция принимает seed для вывода
подписывающих PDA как
`signer_seeds`.

Вы можете обратиться к странице [Program Derived Address](/docs/core/pda) для
получения подробной информации о том, как выводить PDA.
...
используя `signers_seeds` и
`program_id` вызывающей
программы. Когда действительный PDA проверен, адрес
...
Следующие примеры демонстрируют, как выполнить CPI с подписантами PDA, используя
...
- Пример 1: Использует
  _rs`CpiContext`_ Anchor и
  вспомогательную функцию для создания инструкции CPI.
- Пример 2: Использует функцию
  _rs`system_instruction::transfer`_ из крейта
  `solana_program` для создания
  инструкции CPI. Пример 1 является абстракцией этой реализации.
...
переводит SOL с PDA на счет получателя, используя CPI, подписанный PDA. Тестовый
файл использует [LiteSVM](https://github.com/LiteSVM/litesvm) для тестирования
программы.
