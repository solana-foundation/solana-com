...
В следующем примере извлекается аккаунт Token Program, чтобы показать, что
аккаунты программ имеют одинаковую базовую `Account` структуру, за исключением
поля `executable`, которое установлено в значение `true`. Поскольку аккаунты
программ содержат исполняемый код в своем поле данных, мы не десериализуем эти
данные.
...
Когда вы разворачиваете программу Solana, она сохраняется в аккаунте программы.
Аккаунты программ принадлежат
...
непосредственно в аккаунте программы. Loader-v3 хранит исполняемый код в
отдельном "аккаунте данных программы", а аккаунт программы просто указывает на
...
Loader-v3 имеет специальный тип аккаунта для временного размещения программы во
время развертывания или обновлений. В loader-v4 буферы все еще существуют, но
они являются обычными аккаунтами программ.

### Аккаунт данных программы

Loader-v3 работает иначе, чем все остальные программы BPF Loader. Аккаунт
программы содержит только адрес аккаунта данных программы, который хранит
фактический исполняемый код:

![Аккаунт данных программы](/assets/docs/core/accounts/program-account-expanded.svg)

Не путайте эти аккаунты данных программ с аккаунтами данных программ (см. ниже).
...
Обратите внимание, что только
[System Program](/docs/core/accounts#system-program) может создавать новые
аккаунты. После того как System Program создаёт аккаунт, она может передать
