...
отображается в виде строки в кодировке base58 (например,
...
- `data`: Байтовый массив, который хранит произвольные данные для аккаунта. Для
  ...
  программный код. Поле данных обычно называется "данными аккаунта".
...
- `rent_epoch`: Устаревшее поле со времен, когда в Solana существовал механизм,
  который периодически вычитал lamport из аккаунтов. Хотя это поле все еще
  существует в типе Account, оно больше не используется с тех пор, как сбор rent
  был отменен.
...
- Изменить поле `data` аккаунта
- Вычесть lamport из баланса аккаунта
