...
комиссию за приоритет, чтобы увеличить вероятность того, что текущий лидер
(validator) обработает вашу транзакцию.

## Ключевые моменты

- **Базовая комиссия** за транзакцию составляет 5000 lamport за каждую подпись в
  транзакции.
- **Комиссия за приоритет** (необязательная) — это дополнительная плата, которую
  вы платите валидатору для увеличения вероятности того, что текущий лидер
...
Комиссия за приоритет — это необязательная плата, которая вносится для
увеличения вероятности того, что текущий лидер обработает вашу транзакцию.
...
- Пользовательский лимит: Вы можете запросить определенный лимит вычислительных
  единиц, включив инструкцию `SetComputeUnitLimit` в вашу транзакцию.
...
- [Priority Fee API](https://docs.helius.dev/solana-apis/priority-fee-api) от
  Helius
