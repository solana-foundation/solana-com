...
Для новых развертываний, пожалуйста, используйте `solana program-v4 deploy`
вместо `solana program deploy`.
...
- `--with-compute-unit-price`: установка цены вычислительной единицы для
  транзакции, с шагом 0.000001 lamport (микро-lamport) за вычислительную
  единицу. Используйте
...
- `--use-rpc`: отправка транзакций записи на настроенный RPC вместо TPU
