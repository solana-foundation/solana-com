...
- `--with-compute-unit-price`: Устанавливает цену вычислительной единицы для
  транзакции в приращениях 0.000000001 lamport (микро-lamport) за вычислительную
  единицу. Используйте
  [Priority Fee API от Helius](https://docs.helius.dev/guides/priority-fee-api)
  для получения оценки приоритетной комиссии, которую следует установить.
- `--use-rpc`: Отправляет транзакции записи на настроенный RPC вместо validator
  TPUs. Этот флаг требует
  [stake-weighted](/developers/guides/advanced/stake-weighted-qos) RPC
  соединения, такого как [Helius](https://www.helius.dev/) или
  [Triton](https://triton.one/). Этот флаг также может быть настроен как
  стандартный с помощью: `solana config set --url <RPC_URL>`.
- `--max-sign-attempts`: Максимальное количество попыток подписать или
  ...
  не подтверждены после истечения срока действия изначально выбранного
  последнего blockhash, эти транзакции будут переподписаны с новым последним
  blockhash и отправлены повторно. Используйте этот параметр для настройки
  максимального количества итераций подписания транзакций. Каждый blockhash
  действителен примерно 60 секунд, что означает, что использование значения по
  умолчанию 5 приведет к отправке транзакций в течение как минимум 5 минут или
  до тех пор, пока все транзакции не будут подтверждены, в зависимости от того,
  что наступит раньше.
...
Подкоманда `show` выводит метаданные программы.
...
- `Program Id` - это адрес, на который можно ссылаться в поле `program_id`
  инструкции при вызове программы.
...
- `ProgramData Address` - это аккаунт programdata, связанный с аккаунтом
  программы, который содержит исполняемый файл программы (только для loader-v3).
- `Status`: `retracted`, `deployed` или `finalized` (только для loader-v4).
- `Authority` - это полномочие на обновление программы.
- `Last Deployed In Slot` - это слот, в котором программа была последний раз
  развернута.
...
Если вы не успели вовремя - просто получите новый хеш и повторяйте попытки, пока
не добьетесь успеха, или рассмотрите возможность использования долговечных
nonce-значений для транзакций.
