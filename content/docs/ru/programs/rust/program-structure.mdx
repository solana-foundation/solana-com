...
программа должна иметь `entrypoint`, который определяет, где начинается
...
- `entrypoint.rs`: Определяет точку входа, которая направляет входящие
  инструкции.
- `state.rs`: Определяет состояние, специфичное для программы (данные аккаунта).
...
Сначала создайте новый проект Rust, используя стандартную команду `cargo init` с
флагом `--lib`.
...
из крейта `solana_program` для определения точки входа в вашей программе.
...
3. Реализует функцию `process_instruction`, которая будет направлять инструкции
   в соответствующие функции-обработчики
...
Макрос `entrypoint!` требует функцию со следующей
...
- `program_id`: Публичный ключ вызываемой программы (текущая программа)
- `accounts`: `AccountInfo` для аккаунтов, необходимых для выполнения вызываемой
  инструкции
- `instruction_data`: Дополнительные данные, передаваемые программе, которые
  определяют инструкцию для выполнения и её необходимые аргументы
...
Добавьте следующий код в `lib.rs` для определения инструкций для программы
счетчика.
...
Обработчики инструкций относятся к функциям, которые содержат бизнес-логику для
каждой инструкции. Обычно функции обработчика называют
`process_<instruction_name>`, но вы можете выбрать любое соглашение об
именовании.

Добавьте следующий код в `lib.rs`. Этот код использует перечисление
`CounterInstruction` и метод `unpack`, определенные на предыдущем шаге, для
маршрутизации входящих инструкций к соответствующим функциям обработчика:
...
1. Создает и выделяет пространство для нового аккаунта для хранения данных
   счетчика
2. Инициализирует данные аккаунта с помощью `initial_value`, переданного в
   инструкцию
...
аккаунта. Количество определяемых вами аккаунтов - это аккаунты, необходимые для
инструкции.
...
1. Укажите пространство (в байтах) для выделения полю данных аккаунта. Поскольку
   мы храним значение u64 (`count`), нам нужно 8 байт.
...
инструкцию `create_account` программы System.
...
Чтобы вызвать программу System из инструкции нашей программы, мы делаем Cross
...
`create_account` программы System.
...
1. Создание новой структуры `CounterAccount` с `initial_value`, предоставленным
   инструкции.
2. Получение изменяемой ссылки на поле данных нового аккаунта.
3. Сериализация структуры `CounterAccount` в поле данных аккаунта, эффективно
...
Как и в функции `process_initialize_counter`, мы начинаем с создания итератора
по аккаунтам. В данном случае мы ожидаем только один аккаунт, который нужно
обновить.
...
ссылаемся как на `counter_account`, принадлежит выполняющейся программе.
...
- Изменяемое заимствование поля данных существующего аккаунта
- Десериализация сырых байтов в нашу структуру `CounterAccount`
- Обновление значения `count`
- Сериализация измененной структуры обратно в поле данных аккаунта
...
Затем добавьте следующий тестовый модуль в `lib.rs` и запустите `cargo test-sbf`
для выполнения тестов. При желании используйте флаг `--nocapture` для просмотра
выводимых сообщений в выходных данных.
...
Далее, настройте тест, используя `ProgramTest`. Затем создайте новую keypair,
которая будет использоваться как адрес для аккаунта счетчика, который мы
инициализируем, и определите начальное значение для установки счетчика.
...
- Создайте instruction data с вариантом 0 (`InitializeCounter`) и начальным
  значением
...
Обратите внимание, что instruction data для инструкции увеличения - это `[1]`,
что соответствует варианту 1 (`IncrementCounter`). Поскольку нет дополнительных
параметров для инструкции увеличения, данные представляют собой просто вариант
инструкции.
