...
`CounterInstruction` и метод `unpack`, определенные в предыдущем шаге, для
...
1. Указать пространство (в байтах) для выделения в поле данных аккаунта.
   Поскольку мы храним значение u64 (`count`), нам нужно 8 байт.
...
инструкцию `create_account` Системной Программы.
...
1. Создания новой структуры `CounterAccount` с `initial_value`, предоставленным
   инструкции.
2. Получения изменяемой ссылки на поле данных нового аккаунта.
3. Сериализации структуры `CounterAccount` в поле данных аккаунта, эффективно
...
Обратите внимание, что данные инструкции для инструкции увеличения — `[1]`, что
соответствует варианту 1 (`IncrementCounter`). Поскольку у инструкции увеличения
