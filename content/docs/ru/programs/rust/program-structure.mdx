...
программа должна иметь `entrypoint`, которая определяет, где начинается
...
Для простоты программа будет реализована в одном `lib.rs` файле, хотя на
...
Затем добавьте следующий фрагмент в файл `Cargo.toml`. Если вы не включите эту
...
Добавьте следующий код в `lib.rs` для определения инструкций программы счетчика.
...
`accounts` и используем функцию `next_account_info` для получения каждого счета.
Количество определяемых вами счетов - это счета, необходимые для выполнения
инструкции.
...
`create_account` Системной Программы.
...
1. Создаем новую структуру `CounterAccount` с `initial_value`, переданным в
   инструкцию.
...
ссылаемся как на `counter_account`, принадлежит выполняемой программе.
...
- Десериализуем сырые байты в нашу структуру `CounterAccount`
...
Затем добавьте следующий тестовый модуль в `lib.rs` и выполните `cargo test-sbf`
для запуска тестов. При желании используйте флаг `--nocapture` для отображения
...
Далее, настройте тест с помощью `ProgramTest`. Затем создайте новый keypair для
использования в качестве адреса счетчика, который мы инициализируем, и
определите начальное значение для счетчика.
...
- `is_signer`: Должен ли аккаунт подписать транзакцию
...
Обратите внимание, что данные инструкции для инструкции инкремента — `[1]`, что
соответствует варианту 1 (`IncrementCounter`). Поскольку у инструкции инкремента
нет дополнительных параметров, данные представляют собой просто вариант
инструкции.
