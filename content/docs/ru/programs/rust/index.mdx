...
1. Файл `.so` (например, `hello_world.so`): это скомпилированная программа
   Solana, которая будет развёрнута в сети как "смарт- контракт".
2. Файл keypair (например, `hello_world-keypair.json`): публичный ключ этой пары
   ключей используется как идентификатор программы при её развёртывании.
...
Добавьте следующий тест в `src/lib.rs`, под кодом программы. Это тестовый
...
идентификатором программы. Обновите программу в `src/lib.rs`, чтобы выводить
...
что сгенерирует новый файл keypair.
