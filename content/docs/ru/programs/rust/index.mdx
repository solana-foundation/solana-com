...
2. Файл keypair (например, `hello_world-keypair.json`): Публичный ключ этого
   keypair используется как идентификатор программы при её развёртывании.
...
(например, `./target/deploy/hello_world-keypair.json`) и снова запустить
`cargo build-sbf`, что сгенерирует новый файл пары ключей.
