...
программа Solana, которая выводит "Hello, world!" в журнал программы при вызове
программы.
...
2. Файл keypair (например, `hello_world-keypair.json`): Публичный ключ этого
   keypair используется как ID программы при её развертывании.

Чтобы просмотреть ID программы, выполните следующую команду в терминале. Эта
команда выводит открытый ключ keypair, расположенного по указанному пути к
файлу:
...
Далее протестируйте программу, используя crate `solana-program-test`. Добавьте
...
Добавьте следующий тест в файл `src/lib.rs`, ниже кода программы. Это модуль
...
Далее мы покажем, как вызвать программу с использованием клиента на Rust.
...
## !!steps Замена ID программы

Перед запуском скрипта замените ID программы в приведенном выше коде на ID вашей
программы.
...
`cargo build-sbf`, что приведёт к созданию нового файла keypair.
