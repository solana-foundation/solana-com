---
title: Menarik Token
description: Pelajari cara menarik token dari state konfidensial.
---

## Cara menarik token dari saldo konfidensial yang tersedia

Untuk menarik token dari saldo konfidensial yang tersedia ke saldo publik:
...
1. Buat
   ...
   **Bukti Kesetaraan
   ...
   Memverifikasi bahwa ciphertext saldo yang tersedia setelah penarikan cocok
   dengan [Pedersen commitment](https://en.wikipedia.org/wiki/Commitment_scheme)
   yang sesuai, memastikan saldo baru yang tersedia pada akun dihitung dengan
   benar sebagai `remaining_balance = current_balance - withdraw_amount`.
   ...
   **Bukti Rentang
   ...
   Memverifikasi bahwa saldo yang tersisa setelah penarikan tidak negatif dan
   berada dalam rentang yang ditentukan.

2. Untuk setiap bukti:

   - Panggil program bukti ZK ElGamal untuk memverifikasi data bukti.
   - Simpan metadata khusus bukti dalam akun "context state" untuk digunakan
     dalam instruksi lain.
...
4. Tutup akun bukti untuk mendapatkan kembali SOL yang digunakan untuk
   membuatnya.

Diagram berikut menunjukkan langkah-langkah yang terlibat dalam penarikan token
dari saldo konfidensial yang tersedia ke saldo publik:
...
Untuk menarik token dari saldo konfidensial yang tersedia ke saldo publik, Anda
harus:
...
Crate `spl_token_client` menyediakan metode berikut:
...
Contoh berikut menunjukkan cara menarik token dari saldo konfidensial yang
