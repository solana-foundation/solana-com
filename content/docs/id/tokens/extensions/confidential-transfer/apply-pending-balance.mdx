...
1. **Saldo Tertunda Rahasia**: Awalnya, token "disetor" dari saldo publik ke
   saldo rahasia yang "tertunda".
2. **Saldo Tersedia Rahasia**: Saldo tertunda kemudian "diterapkan" ke saldo
   yang tersedia, membuat token tersedia untuk transfer rahasia.
...
rahasia dari satu token account ke token account lain, token tersebut pada
awalnya ditambahkan ke saldo tertunda rahasia. Sebelum token dapat digunakan
untuk transfer rahasia, saldo tertunda harus "diterapkan" ke saldo yang
tersedia.
...
<Callout type="info">
  ...
  lokal default. Anda harus mengkloning Program Token Extension dari mainnet
