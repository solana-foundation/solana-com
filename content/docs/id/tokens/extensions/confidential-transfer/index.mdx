...
Confidential transfers memungkinkan Anda mentransfer token antar akun token
tanpa mengungkapkan jumlah transfer. Ini berguna untuk transaksi yang menjaga
privasi. Hanya jumlah transfer dan saldo token yang bersifat privat. Alamat akun
token tetap bersifat publik.
...
- [Confidential Balances Cookbook](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Cuplikan kode tentang cara menggunakan ekstensi Confidential Transfer

### Bagaimana cara kerjanya?

Ekstensi Confidential Transfer menambahkan
...
Alur dasar transfer token confidential adalah sebagai berikut:

1. Buat mint account dengan ekstensi confidential transfer.
2. Buat token account dengan ekstensi confidential transfer untuk pengirim dan
   penerima.
3. Mint token ke akun pengirim.
4. **Deposit** saldo publik pengirim ke **confidential pending balance**.
5. **Apply** saldo pending pengirim ke **confidential available balance**.
6. Secara confidential **transfer** token dari token account pengirim ke token
   account penerima.
7. **Apply** saldo pending penerima ke **confidential available balance**.
8. **Withdraw** confidential available balance penerima ke **public balance**.

Untuk detail lebih lanjut tentang langkah-langkah dalam alur confidential
transfer, lihat halaman terkait:
...
<Cards>
  ...
  >
    Cara deposit token ke confidential pending balance
  </Card>
  <Card
    title="Apply Pending Balance"
    href="/docs/tokens/extensions/confidential-transfer/apply-pending-balance"
  >
    Cara apply pending balance ke available confidential balance
  </Card>
  <Card
    title="Withdraw Token"
    href="/docs/tokens/extensions/confidential-transfer/withdraw-tokens"
  >
    Cara withdraw token dari confidential available balance
  </Card>
  <Card
    title="Transfer Token"
    href="/docs/tokens/extensions/confidential-transfer/transfer-tokens"
  >
    Cara confidentially transfer token antar token account
  </Card>
</Cards>

Diagram di bawah ini menunjukkan urutan detail dari alur dasar untuk transfer
token rahasia:
...
Solana CLI yang terinstal untuk menggunakan validator lokal.
...
| Instruksi                           | Deskripsi                                                                                                                                                      |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Menyiapkan akun mint untuk transfer rahasia. Instruksi ini harus disertakan dalam transaksi yang sama dengan instruksi _rs`TokenInstruction::InitializeMint`_. |
| _rs`UpdateMint`_                    | Memperbarui pengaturan transfer rahasia untuk mint.                                                                                                            |
| _rs`ConfigureAccount`_              | Menyiapkan akun token untuk transfer rahasia.                                                                                                                  |
| _rs`ApproveAccount`_                | Menyetujui akun token untuk transfer rahasia jika mint memerlukan persetujuan untuk akun token baru.                                                           |
| _rs`EmptyAccount`_                  | Mengosongkan saldo rahasia yang tertunda dan tersedia untuk memungkinkan penutupan akun token.                                                                 |
| _rs`Deposit`_                       | Mengonversi saldo token publik menjadi saldo rahasia tertunda.                                                                                                 |
| _rs`Withdraw`_                      | Mengonversi saldo rahasia yang tersedia kembali menjadi saldo publik.                                                                                          |
| _rs`Transfer`_                      | Mentransfer token antar akun token secara rahasia.                                                                                                             |
| _rs`ApplyPendingBalance`_           | Mengonversi saldo tertunda menjadi saldo tersedia setelah deposit atau transfer.                                                                               |
| _rs`EnableConfidentialCredits`_     | Memungkinkan akun token untuk menerima transfer token rahasia.                                                                                                 |
| _rs`DisableConfidentialCredits`_    | Memblokir transfer rahasia masuk sambil tetap memungkinkan transfer publik.                                                                                    |
| _rs`EnableNonConfidentialCredits`_  | Memungkinkan akun token untuk menerima transfer token publik.                                                                                                  |
| _rs`DisableNonConfidentialCredits`_ | Memblokir transfer reguler untuk membuat akun hanya menerima transfer rahasia.                                                                                 |
| _rs`TransferWithFee`_               | Mentransfer token antar akun token secara rahasia dengan biaya.                                                                                                |
| _rs`ConfigureAccountWithRegistry`_  | Cara alternatif untuk mengonfigurasi akun token untuk transfer rahasia menggunakan akun _rs`ElGamalRegistry`_ alih-alih bukti _rs`VerifyPubkeyValidity`_.      |
