...
description: Pelajari cara mentransfer token antar token account.
---

## Cara Mentransfer Token

Mentransfer token melibatkan pemindahan token dari satu token account ke token
account lain yang memiliki mint yang sama. Transfer token terjadi ketika Anda
memanggil instruksi
...
pada token program. Hanya alamat yang ditentukan sebagai pemilik (authority)
dari token account sumber yang dapat mentransfer token keluar dari akun
tersebut.
