...
description: Pelajari tentang Program Token Solana.
---
...
- [Token Program](#token-program) berisi semua logika instruksi untuk
  berinteraksi dengan token di jaringan (baik token fungible maupun
  non-fungible).
...
- [Token Account](#token-account) melacak kepemilikan individual token untuk
  mint account tertentu untuk pemilik tertentu.
...
jaringan (baik token fungible maupun non-fungible). Semua token di Solana pada
...
Associated Token Account menyederhanakan proses menemukan alamat token account
...
Perhatikan bahwa setiap dompet membutuhkan token account-nya sendiri untuk
menyimpan token dari mint yang sama.

![Hubungan Akun yang Diperluas](/assets/docs/core/tokens/token-account-relationship-ata.svg)

## Contoh Token CLI

CLI _sh`spl-token`_ membantu Anda bereksperimen dengan token SPL. Contoh di
bawah menggunakan terminal [Solana Playground](https://beta.solpg.io/) untuk
...
Temukan lebih banyak contoh di _sh`spl-token`_
[dokumentasi](https://www.solana-program.com/docs/token).
...
contoh, perintah _sh`create-token`_ menghasilkan alamat mint account dengan
