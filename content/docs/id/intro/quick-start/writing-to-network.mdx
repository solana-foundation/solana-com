...
setiap instruksi. Ketika Anda mengirimkan transaksi, runtime Solana mengeksekusi
setiap instruksi secara berurutan dan atomik (artinya semua instruksi berhasil
atau seluruh transaksi gagal).
...
Public key dari keypair ini akan digunakan sebagai alamat untuk mint account.
...
instruksi `createInitializeMint2Instruction` untuk menginisialisasi mint account
dengan data berikut:
...
Dengan menggabungkan kedua instruksi dalam satu transaksi, Anda memastikan bahwa
pembuatan akun dan inisialisasi terjadi secara atomik. Kedua instruksi berhasil,
atau tidak sama sekali.

Pendekatan ini umum digunakan saat membangun transaksi Solana yang lebih
kompleks, karena menjamin semua instruksi dieksekusi bersama.
