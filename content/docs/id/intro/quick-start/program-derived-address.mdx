...
description:
  ...
  menulis pengujian. Sempurna untuk pengembang yang ingin memahami cara
  menggunakan PDAs dalam program Solana.
...
[`create_message`](mention:one), [`update_message`](mention:two), dan
[`delete_message`](mention:three) untuk ditambahkan dalam langkah-langkah
berikut.
...
Sebelum memulai, jalankan `anchor build` di terminal Playground untuk memeriksa
...
data akun (tipe data untuk disimpan di kolom data Account).
...
Batasan _rs`mut`_ menyatakan akun sebagai dapat diubah (mutable).
...
Dalam kasus ini, akun harus sesuai dengan tipe _rs`MessageAccount`_ kustom.
...
tetapi tidak memengaruhi validasi. Anda sebaiknya menggunakan nama deskriptif
untuk kejelasan.
...
secara otomatis dihasilkan oleh Anchor saat membuat akun.
...
1. _rs`user: Signer<'info>`_
   ...
   - Ditandai sebagai mutable (_rs`#[account(mut)]`_) karena mungkin membayar
     untuk ruang lebih untuk `message_account` saat diperlukan
...
untuk mengakses parameter _rs`message`_ dari instruksi _rs`update`_.
...
Tubuh fungsi hanya mencetak pesan ke log program menggunakan makro _rs`msg!()`_.
...
untuk memeriksa bahwa file berjalan sesuai harapan. Langkah selanjutnya
...
File pengujian menggunakan ini karena akun ditutup dan datanya menjadi tidak
tersedia.
