...
| `owner`      | Program yang memiliki akun. Untuk dompet, ini selalu System Program, dengan alamat `11111111111111111111111111111111`.         |
| `lamports`   | Saldo SOL akun, dalam satuan lamport (1 SOL = 1.000.000.000 lamport).                                                          |
| `rentEpoch`  | Bidang lama yang terkait dengan mekanisme rent Solana yang sudah tidak digunakan lagi (saat ini tidak dipakai).                |
...
| `data`       | Field ini berisi state akun yang diserialisasi, seperti otoritas mint, total pasokan, jumlah desimal |
...
Field `data` dari akun Solana berisi byte mentah. Untuk menginterpretasikan data
...
yang mengabstraksi proses deserialisasi. Fungsi-fungsi ini mengonversi byte akun
mentah menjadi tipe data terstruktur, sehingga memudahkan untuk bekerja dengan
data akun.

<WithMentions>

Sebagai contoh, _shell`@solana/spl-token`_ mencakup fungsi
