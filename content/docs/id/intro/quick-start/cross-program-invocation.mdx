...
description:
  Pelajari cara mengimplementasikan Cross Program Invocations (CPIs) dalam
  ...
  menangani Program Derived Addresses (PDAs) dalam CPIs. Sempurna untuk
  pengembang yang ingin membangun program Solana yang dapat dikomposisi.
h1: Cross Program Invocation
---

Dalam bagian ini, program CRUD dari bagian PDA sebelumnya diperbarui dengan
menambahkan Cross Program Invocations (CPIs), fitur yang memungkinkan program
Solana untuk memanggil satu sama lain.

Tutorial ini juga menunjukkan bagaimana program dapat "menandatangani" untuk
Program Derived Addresses (PDAs) ketika membuat Cross Program Invocations.

Instruksi `update` dan `delete` perlu dimodifikasi untuk menangani transfer SOL
antara akun dengan memanggil System Program.

Tujuan dari bagian ini termasuk memandu proses implementasi CPIs dalam program
Solana menggunakan framework Anchor, melanjutkan konsep PDA yang telah dibahas
di bagian sebelumnya. Untuk detail lebih lanjut, lihat
...
Bagian ini menambahkan akun baru yang disebut `vault_account` ke struktur
_rs`Update`_. Akun ini berfungsi sebagai "vault" yang dikendalikan program yang
menerima SOL dari pengguna ketika mereka memperbarui pesan mereka.
...
Aspek kunci dari `vault_account`:
...
- Sebagai PDA, akun ini tidak memiliki kunci privat, sehingga hanya program yang
  dapat "menandatangani" untuk alamat tersebut saat melakukan CPI
...
"menandatangani" untuk PDA ini dalam CPI.
...
Struktur _rs`Transfer`_ menentukan akun yang diperlukan untuk instruksi transfer
System Program.

### !cpi_context

Struktur _rs`CpiContext`_ menentukan program dan akun untuk Cross Program
Invocation (CPI).

### !transfer

Fungsi _rs`transfer()`_ memanggil instruksi transfer System Program.
...
cara melakukan CPI dari dalam program, yang memungkinkan komposabilitas program
Solana.
...
- Akun-akun yang diperlukan dalam CPI (didefinisikan dalam struct
  _rs`Transfer`_)
...
Anda menentukan program, akun, dan instruction data untuk instruksi tertentu
yang akan dipanggil. Ketika instruksi `update` program menerima pemanggilan, ia
...
disediakan benar-benar menderivasi ke alamat PDA. Jika terverifikasi, runtime
...
mengimplementasikan instruksi transfer dari System Program. CPI ini menunjukkan
cara melakukan transfer yang memerlukan penandatangan Program Derived Address
(PDA).
...
- `cpi_context` (program, akun, dan PDA signer)
...
secara internal memanggil instruksi transfer dari System Program, menandatangani
untuk PDA agar memungkinkan transfer semua dana dari vault kembali ke pengguna.
...
Hanya upgrade authority dari program yang dapat memperbaruinya. Developer
menetapkan upgrade authority selama deployment program, dan itu tetap menjadi
satu-satunya akun dengan izin untuk mengubah atau menutup program. Jika
seseorang mencabut upgrade authority, maka program menjadi tidak dapat diubah.

Saat men-deploy program di Solana Playground, dompet Playground Anda bertindak
sebagai upgrade authority untuk semua program Anda.
...
Setelah membuat perubahan ini, jalankan test untuk memastikan semuanya berfungsi
...
[Konsep Inti](/docs/core/accounts) yang memberikan penjelasan detail tentang
...
[tautan Solana Playground ini](https://beta.solpg.io/https://github.com/solana-developers/program-examples/tree/main/basics/hello-solana/anchor)
untuk melihat proyek Anchor dari
[repo Github ini](https://github.com/solana-developers/program-examples/tree/main/basics/hello-solana/anchor).
