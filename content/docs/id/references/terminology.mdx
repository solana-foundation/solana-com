...
Seperti rekening di bank tradisional, account Solana dapat menyimpan dana yang
disebut [lamport](#lamport). Seperti file di Linux, account dapat diakses dengan
kunci, yang sering disebut sebagai [public key](#public-key-pubkey) atau pubkey.

Kunci tersebut dapat berupa salah satu dari:

- kunci publik ed25519
- alamat account yang diturunkan dari program (nilai 32byte yang dipaksa keluar
  dari kurva ed25519)
- hash dari kunci publik ed25519 dengan string 32 karakter

## account owner

Alamat program yang memiliki account. Hanya program pemilik yang mampu
memodifikasi account tersebut.
...
Alamat pengguna yang memiliki izin tertentu atas sebuah account.

Contohnya:

- Kemampuan untuk mencetak token baru diberikan kepada account yang merupakan
  'mint authority' untuk token mint.
- Kemampuan untuk meningkatkan program diberikan kepada account yang merupakan
...
[account](#account) yang menyimpan [native token](#native-token) tidak nol.
...
[Native token](#native-token) fraksional dengan nilai 0,000000001 [sol](#sol).
...
untuk membaca dan memodifikasi akun yang berada di bawah kendalinya. Program ini
sering disebut sebagai "[_smart contract_](/docs/core/programs)" di blockchain
lain.
...
[validator](#validator), jumlah point yang diberikan ke [stake](#stake) saat
...
dikontrol oleh private key seperti akun lainnya.
...
Public key dari [keypair](#keypair).
...
dan S adalah skalar dalam rentang [1, l-1]. Persyaratan ini memastikan tidak ada
...
setelah batas epoch ketika ukuran sampel kecil, dan juga untuk validator dengan
...
Sebuah [slot](#slot) yang telah berlalu yang tidak menghasilkan [block](#block),
karena pemimpin sedang offline atau [fork](#fork) yang berisi slot tersebut
ditinggalkan untuk alternatif yang lebih baik berdasarkan konsensus cluster.
Slot yang dilewati tidak akan muncul sebagai leluhur untuk block pada slot
berikutnya, tidak menambah [tinggi block](#block-height), dan tidak mengakhiri
masa berlaku `recent_blockhash` tertua.

Apakah sebuah slot telah dilewati hanya dapat ditentukan ketika slot tersebut
menjadi lebih tua dari [rooted](#root) terbaru (yang berarti tidak dilewati).
...
cluster seperti tinggi tick saat ini, nilai [poin](#point) rewards, dll. Program
dapat mengakses Sysvars melalui akun Sysvar (pubkey) atau dengan melakukan query
melalui syscall.
...
dibedakan oleh alamat token mint unik mereka.
...
[client](#client) menggunakan satu atau lebih [keypair](#keypair) dan dieksekusi
secara atomik dengan hanya dua kemungkinan hasil: sukses atau gagal.

## transaction id

[Signature](#signature) pertama dalam [transaksi](#transaction), yang dapat
...
[buku besar](#ledger). Transaksi dianggap final ketika bloknya menjadi
[root](#root).
...
Peserta penuh dalam [cluster](#cluster) jaringan Solana yang menghasilkan
...
Penghitungan reward untuk [validator](#validator). Kredit vote diberikan kepada
