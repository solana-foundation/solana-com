...
[infrastruktur kompiler LLVM](https://llvm.org/) menjadi
...
sebagai instruksi native yang dikompilasi just-in-time secara efisien.
...
subset dari peta memori. Program akan mengalami panic jika mencoba membaca atau
menulis ke alamat virtual yang tidak diberikan aksesnya, dan `AccessViolation`
error akan dikembalikan yang berisi alamat dan ukuran pelanggaran yang dicoba.

## InvalidAccountData

Error program ini dapat terjadi karena banyak alasan. Biasanya, ini disebabkan
oleh pemberian akun ke program yang tidak diharapkan oleh program tersebut, baik
dalam posisi yang salah dalam instruksi atau akun yang tidak kompatibel dengan
instruksi yang sedang dijalankan.

Implementasi program juga dapat menyebabkan error ini ketika melakukan instruksi
lintas program dan lupa menyediakan akun untuk program yang Anda panggil.
...
menentukan encoding yang tepat yang diharapkan oleh program.
...
ditandatangani, tetapi signer seeds yang diteruskan ke `invoke_signed` tidak
cocok dengan signer seeds yang digunakan untuk membuat alamat program
[`create_program_address`](/docs/core/pda#createprogramaddress).

## Stack

SBF menggunakan stack frame alih-alih pointer stack variabel. Setiap stack frame
berukuran 4KB.

Jika program melanggar ukuran stack frame tersebut, compiler akan melaporkan
...
Pesan tersebut mengidentifikasi simbol mana yang melebihi stack frame-nya,
tetapi namanya mungkin ter-mangled.

> Untuk demangle simbol Rust gunakan
> [rustfilt](https://github.com/luser/rustfilt).

Peringatan di atas berasal dari program Rust, jadi nama simbol yang di-demangle
adalah:
...
dependensi mungkin menyertakan fungsionalitas yang melanggar batasan stack frame
...
Stack frame SBF menempati rentang alamat virtual yang dimulai dari
`0x200000000`.
...
Secara internal, program memiliki akses ke region memori 32KB yang dimulai pada
...
Program di-deploy dan dieksekusi oleh runtime loader, saat ini terdapat dua
loader yang didukung
...
berbeda, hasilnya biasanya kesalahan `AccessViolation` karena ketidakcocokan
...
- Untuk setiap akun
  ...
  - Jika bukan duplikat:
    ...
    - 8 byte angka unsigned jumlah lamport yang dimiliki oleh akun
