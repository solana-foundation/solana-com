...
Untuk deployment baru, silakan gunakan `solana program-v4` alih-alih
`solana program`.

Untuk memigrasikan program yang sudah ada (yang pada dasarnya men-deploy ulang):

```bash
solana program dump <PROGRAM_ID> program.so
solana program-v4 deploy program.so
```

## Persiapan

Pertama, program perlu dibangun (dikompilasi, di-link, di-strip).

```bash
cargo build-sbf
```

Langkah ini harus dilakukan sebelum setiap deployment atau re-deployment.
...
deployment ulang program yang sama di kemudian hari.
...
Parameternya disebut `--program-id` meskipun mengharapkan file-path dari
keypair:

```bash
solana program deploy --program-id program-keypair.json target/deploy/program.so
```
...
Mengunggah executable yang berbeda ke akun program yang sama lagi akan
...
penandatangan adalah kepair otoritas peningkatan.
...
memperbesar akun programdata terlebih dahulu:
...
penerapan ulang menggunakan `program-id` sebagai gantinya:
...
- `--with-compute-unit-price`: Tetapkan harga unit komputasi untuk transaksi,
  dalam kenaikan 0,000001 lamport (mikro-lamport) per unit komputasi. Gunakan
...
- `--max-sign-attempts`: Jumlah maksimum percobaan untuk menandatangani atau
  ...
  nilai default 5 akan menyebabkan pengiriman transaksi selama setidaknya 5
  menit atau sampai semua transaksi dikonfirmasi, mana yang lebih dulu terjadi.

## Melanjutkan Unggahan

Ada kemungkinan unggahan macet atau terhenti.

### Loader-v3

Jika penerapan program gagal, akan ada akun buffer perantara yang menggantung
yang berisi saldo tidak nol. Untuk mendapatkan kembali saldo tersebut, Anda
dapat melanjutkan penerapan yang gagal dengan menyediakan buffer perantara yang
sama ke panggilan baru untuk `deploy`.

Kegagalan penerapan akan mencetak pesan kesalahan yang menentukan frasa seed
...
Saat diminta, masukkan frasa seed 12 kata.

Kemudian keluarkan perintah `deploy` baru dan tentukan buffernya:
...
Dimungkinkan untuk melanjutkan unggahan pada offset byte tertentu:
...
Program dapat dibuat tidak dapat diubah dengan menghapus otoritas
peningkatannya.
...
versi program mana yang ingin mereka gunakan dengan membangun daftar tertaut
...
Untuk program yang diterapkan di bawah loader-v3, hanya akun programdata, akun
...
Program yang di-deploy menggunakan loader-v4 dapat ditutup beserta akun
programnya, program ID dan dana yang terkunci semuanya akan tersedia kembali
...
setelah menutupnya, Anda harus membuat file keypair program baru.
...
Untuk menutup semua akun buffer yang terkait dengan otoritas saat ini:
...
- `Program Id` adalah alamat yang dapat direferensikan di field `program_id`
  instruksi saat memanggil program.
...
- `Authority` adalah otoritas upgrade program.
- `Last Deployed In Slot` adalah slot di mana program terakhir di-deploy.
...
Untuk melihat daftar program yang di-deploy dengan otoritas default:

```shell
solana program show --programs
```

Untuk menampilkan semua akun buffer terlepas dari otoritasnya:

```shell
solana program show --buffers --all
```

Untuk menentukan otoritas yang berbeda:
...
dapat ditransfer ke keypair lain tanpa mengubah keypair program, sehingga
program ID tetap sama. Selain itu, satu authority dapat mengontrol beberapa akun
program.

Jika tidak ditentukan secara eksplisit selama deployment awal, maka keypair
default digunakan sebagai authority. Inilah mengapa men-deploy ulang program
dalam langkah-langkah di atas tidak memerlukan authority yang ditentukan secara
eksplisit.
...
Alih-alih mengunggah langsung ke akun program, executable dapat diunggah
terlebih dahulu ke akun buffer pementasan dan kemudian ditransfer ke akun
program pada langkah kedua (re-/deployment yang sebenarnya). Ini berguna untuk
...
terikat pada akun program tertentu.
...
- penandatangan offline (authority akun program)

Proses umum adalah redeployment dua langkah dengan beberapa tambahan:

1. (online) buat program baru
2. (online) transfer otoritas ke penanda tangan offline
3. (online) buat buffer dan unggah executable ke dalamnya
4. (opsional) verifikasi konten on-chain buffer
5. (offline) tanda tangani transaksi untuk men-deploy ulang program menggunakan
   buffer `--blockhash <VALUE> --sign-only`
6. (online) gunakan tanda tangan ini untuk menyiarkan transaksi redeployment
...
untuk menggunakan durable transaction nonces.
