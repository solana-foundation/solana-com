...
このセクションの目的は、Solana
Playgroundに慣れることに焦点を当てています。このガイドでは、Program Derived
Address (PDA)とCross-Program Invocation
(CPI)のセクションでより詳細な例を説明しています。詳細については、[Solana上のプログラム](/docs/core/programs)ページを参照してください。
...
*rs`declare_id!()`*マクロは、プログラムのオンチェーンアドレスを指定します。Solana
Playgroundはプロジェクトをビルドするときにこのアドレスを更新します。

### !program

_rs`#[program]`_
属性は、プログラムの命令を表す関数を含むモジュールに注釈を付けます。
...
`init`
制約は、プログラムがアカウントを作成する必要があることを指定します。内部的には、この制約はシステムプログラムを呼び出して新しいアカウントを作成し、このプログラムを所有者にします。プログラム所有者として、このプログラムはアカウントデータに書き込むことができます。
...
この例では、[`initialize`](mention:two) 命令は2つのパラメータを取ります：

1. `ctx: Context<Initialize>` - この命令に必要なアカウントを関数に渡します。これは
   `Initialize` 構造体で指定されています。
2. `data: u64` - 命令を呼び出す際に提供する必要があるカスタムパラメータです。

関数本体は[`new_account` の `data` フィールドを提供された `data` 引数に設定し](mention:three)、プログラムログにメッセージを出力します。
...
- [`#[derive(Accounts)]`](mention:one)
  属性は、特定の命令に必要なアカウントを指定する構造体を定義します。各フィールドは個別のアカウントを表します。
...
[`initialize`](mention:one) 命令を呼び出す方法を示しています。
...
トランザクション署名（ハッシュ）を検索することで、[SolanaFM](https://solana.fm/)や[Solana Explorer](https://explorer.solana.com/?cluster=devnet)でトランザクションの詳細を確認することもできます。

<Callout>
  使用するExplorerのクラスター（ネットワーク）接続をSolana
  Playgroundに合わせて更新することを忘れないでください。Solana
