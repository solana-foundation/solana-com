...
description:
  ...
  (PDAs)とAnchorフレームワークを使用して、CRUD（作成、読み取り、更新、削除）Solanaプログラムを構築する方法を学びましょう。このステップバイステップガイドでは、PDAsを使用してオンチェーンメッセージアカウントを作成、更新、削除する方法、アカウント検証を実装する方法、およびテストを作成する方法を説明します。SolanaプログラムでPDAsを使用する方法を理解したい開発者に最適です。
...
このガイドでは、ユーザーがメッセージを作成、更新、削除できるシンプルなプログラムを紹介します。各メッセージは、プログラム自体から派生した決定論的アドレス（Program
...
[`update`](mention:two)、および[`delete`](mention:three)命令を持つプログラムがあります。
...
`lib.rs`では、*rs`MessageAccount`*構造体を以下のように更新してください：
...
Anchorプログラムの*rs`#[account]`*属性は、アカウントデータ（アカウントのデータフィールドに保存するデータ型）を表す構造体に注釈を付けます。
...
*rs`bump`*フィールドには、プログラム派生アドレス（PDA）を導出するために使用される*rs`u8`*[「バンプ」シード](/docs/core/pda#canonical-bump)が保存されます。この値を保存することで、後続の命令で再計算する必要がなくなり、計算リソースを節約できます。
...
- `bump` - Program Derived
  Address（PDA）を導出するための[「バンプ」シード](/docs/core/pda#canonical-bump)を保存する*rs`u8`*。この値を保存することで、後続の命令で再計算する必要がなくなり、計算リソースを節約できます。

アカウントを作成する際、プログラムは*rs`MessageAccount`*データをシリアライズし、新しいアカウントのデータフィールドに保存します。

その後、アカウントからデータを読み取る際、プログラムはこのデータを*rs`MessageAccount`*データ型に逆シリアライズします。テストセクションでは、アカウントデータの作成と読み取りのプロセスを示しています。
...
次に、*rs`create`*命令を追加して *rs`MessageAccount`*を作成し初期化します。

まず、*rs`Create`*構造体を以下のように更新して、命令に必要なアカウントを定義します：
...
*rs`Signer<'info>`*タイプは、アカウントがトランザクションに署名することを要求します。

### !account

*rs`Account<'info, T>`*タイプは、アカウントが指定されたタイプと一致することを要求します。

この場合、アカウントはカスタム _rs`MessageAccount`_ 型と一致する必要があります。

### !program

_rs`Program<'info, T>`_ 型はアカウントがプログラムと一致することを要求します。

この場合、アカウントはSystem Programを参照する _rs`System`_
型と一致する必要があります。

### !instruction

_rs`#[instruction(message: String)]`_ アノテーションにより、_rs`Create`_
構造体は `create` 命令から _rs`message`_ パラメータにアクセスできます。
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - アカウント作成に必要
   - 裏側では、`init` 制約がSystem
     Programを呼び出して指定された`space`で新しいアカウントを作成し、所有者を現在のプログラムに変更します。

---

_rs`#[instruction(message: String)]`_ アノテーションにより、_rs`Create`_
構造体が`create`命令から _rs`message`_
パラメータにアクセスできるようになります。

---

`seeds`と`bump`制約を組み合わせることで、アカウントのアドレスをProgram Derived
Address (PDA)として定義します。
...
- _rs`user.key().as_ref()`_ - 2番目のseedとしての*rs`user`*アカウントの公開鍵。

`bump`制約は、Anchorに正しいbump
seedを自動的に見つけて使用するよう指示します。Anchorは`seeds`と`bump`を使用してPDAを導出します。

---

`space`計算*rs`(8 + 32 + 4 + message.len() + 1)`*は`MessageAccount`データ型のためのスペースを割り当てます：

- Anchorアカウント識別子（ディスクリミネーター）：8バイト
...
Anchorプログラムを通じて作成されるすべてのアカウントには、アカウントタイプの識別子として機能する8バイトのアカウントディスクリミネーターが必要です。これはAnchorがアカウント作成時に自動的に生成します。

*rs`String`*型は文字列の長さを格納するために4バイトが必要で、残りの長さには実際のデータが含まれます。

</Accordion>
</Accordions>

次に、以下のように`create`関数を更新して、*rs`create`*命令のビジネスロジックを追加します：
...
次に、`update` 命令を追加して、新しいメッセージで `MessageAccount`
を変更します。
...
_rs`Update`_ 構造体は _rs`update`_ 命令に必要なアカウントを定義します。
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - `realloc` 制約はアカウントのデータのサイズ変更を許可します
...
に保存されているバンプシードを使用していることに注意してください。

_rs`#[instruction(message: String)]`_ 属性により、_rs`Update`_ 構造体は
_rs`update`_ 命令から _rs`message`_ パラメータにアクセスできるようになります。
...
次に、_rs`MessageAccount`_ を閉じるための _rs`delete`_ 命令を追加します。
...
_rs`Delete`_ 構造体は _rs`delete`_ 命令に必要なアカウントを定義します：
...
以下のコードを*ts`describe()`*の中に、*ts`it()`*セクションの前に追加してください。
...
このセクションでは、このコードは単にテストファイルのセットアップを行います。

<WithMentions>

Solana Playgroundでは、いくつかの定型的なセットアップが省略されています。
[`pg.program`](mention:one)はプログラムとやり取りするためのメソッドへのアクセスを可能にし、
[`pg.wallet`](mention:two)はあなたのプレイグラウンドウォレットへのアクセスを提供します。
...
セットアップの一部として、テストファイルはメッセージアカウントPDAを導出します。これは、プログラムで指定されたのと同じシードを使用して、JavaScriptでPDAを導出する方法を示しています。
...
*ts`create()`*メソッドは`create`命令を呼び出します。
...
まず、テストファイルは `create` 命令を呼び出すトランザクションを送信し、「Hello,
World!」をメッセージとして渡します。
...
テストファイルでこれを使用するのは、アカウントが閉鎖されてデータが利用できなくなるためです。
...
まず、このコードは `delete`
命令を呼び出すトランザクションを送信して、メッセージアカウントを閉鎖します。
...
トランザクションを送信してアカウントを閉鎖した後、このサンプルはアドレス（`messagePda`）を使用して
`fetchNullable`
でアカウントを取得しようとします。このメソッドは、アカウントが閉鎖後に存在しなくなった場合にnullを返します。
