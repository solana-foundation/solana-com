...
CLIがトランザクション手数料の支払いやプログラムのデプロイに使用するデフォルトのSolanaウォレット（keypair）を指しています。デフォルトでは、このファイルは
`~/.config/solana/id.json` に保存されています。
...
デフォルトの場所にファイルシステムウォレットがすでに保存されている場合、
`--force`
...
SOLに制限されています。レート制限に達したりエラーが発生した場合は、代わりに[Webフォーセット](https://faucet.solana.com)の使用を検討してください。
...
[テストバリデーター](https://docs.anza.xyz/cli/examples/test-validator)が含まれています。

別のターミナルで、次のコマンドを実行してローカルバリデーターを起動します：
...
このセクションでは、Anchor
CLIの一般的なコマンドについて説明し、使い始めるのに役立ちます。Anchor
CLIの詳細については、
...
このコマンドはプロジェクト名のディレクトリを新規作成し、基本的なRustプログラムとTypeScriptテストテンプレートを含む新しいAnchorプロジェクトを初期化します。
...
`Anchor.toml`で`cluster`として`localnet`を使用すると、Anchorは自動的にローカルvalidatorを起動し、プログラムをデプロイし、テストを実行した後、validatorを停止します。
