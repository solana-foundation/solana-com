---
title: プログラム構造
description: Rustで書かれたSolanaプログラムの構造化方法を学びましょう。エントリーポイント、状態管理、命令処理、テストなどを含みます。
h1: Rustプログラム構造
---
...
- `entrypoint.rs`：入ってくる命令をルーティングするエントリーポイントを定義します。
- `state.rs`：プログラム固有の状態（アカウントデータ）を定義します。
- `instructions.rs`：プログラムが実行できる命令を定義します。
- `processor.rs`：各命令のビジネスロジックを実装する命令ハンドラ（関数）を定義します。
- `error.rs`：プログラムが返すことができるカスタムエラーを定義します。

[Solana Program Library](https://github.com/solana-program/token/tree/main/program/src)で例を見ることができます。

## サンプルプログラム

複数の命令を持つネイティブRustプログラムの構築方法を示すために、2つの命令を実装する簡単なカウンタープログラムを見ていきましょう：
...
まず、標準的な`cargo init`コマンドを`--lib`フラグと共に使用して、新しいRustプロジェクトを作成します。
...
Solanaプログラムのエントリーポイントは、プログラムが呼び出されたときに実行される関数です。エントリーポイントには以下の基本的な定義があり、開発者は自由にエントリーポイント関数の独自の実装を作成できます。
...
3. 命令を適切なハンドラー関数にルーティングする `process_instruction`
   関数を実装します
...
- `accounts`: 呼び出される命令に必要なアカウントの `AccountInfo`
- `instruction_data`: 実行する命令とその必要な引数を指定するプログラムに渡される追加データ

これら3つのパラメータは、クライアントがプログラムを呼び出すための命令を構築する際に提供しなければならないデータに直接対応しています。
...
Solanaプログラム開発ではどのシリアライゼーション形式も使用できますが、[Borsh](https://borsh.io/)が一般的に使用されています。SolanaプログラムでBorshを使用するには：
...
2. Borshトレイトをインポートし、derive マクロを使用して構造体にトレイトを実装します：
...
`lib.rs` に `Counter`
構造体を追加してプログラムの状態を定義します。この構造体は初期化命令とインクリメント命令の両方で使用されます。
...
命令とは、Solanaプログラムが実行できる異なる操作を指します。これらをプログラムのパブリックAPIと考えてください - ユーザーがプログラムと対話する際に実行できるアクションを定義します。

命令は通常、Rustの列挙型（enum）を使用して定義されます：

- 各列挙型のバリアントは異なる命令を表します
- バリアントのペイロードは命令のパラメータを表します

Rustの列挙型のバリアントは0から始まる番号が暗黙的に付けられることに注意してください。

以下は2つの命令を定義する列挙型の例です：
...
クライアントがプログラムを呼び出す際、命令データ（バイトのバッファ）を提供する必要があります：

- 最初のバイトは実行する命令バリアントを識別します（0、1など）
- 残りのバイトには、シリアル化された命令パラメータが含まれます（必要な場合）

命令データ（バイト）を列挙型のバリアントに変換するために、ヘルパーメソッドを実装するのが一般的です。このメソッドは：

1. 最初のバイトを分離して命令バリアントを取得します
2. バリアントに対してマッチングを行い、残りのバイトから追加パラメータを解析します
3. 対応する列挙型バリアントを返します
...
カウンタープログラムの命令を定義するために、以下のコードを`lib.rs`に追加してください。
...
命令ハンドラーとは、各命令のビジネスロジックを含む関数のことです。ハンドラー関数の名前を`process_<instruction_name>`とするのが一般的ですが、任意の命名規則を選ぶことができます。

以下のコードを`lib.rs`に追加してください。このコードは前のステップで定義した`CounterInstruction`列挙型と`unpack`メソッドを使用して、入力される命令を適切なハンドラー関数にルーティングします：
...
次に、`process_initialize_counter`関数の実装を追加します。この命令ハンドラーは：

1. カウンターデータを保存するための新しいアカウントを作成し、スペースを割り当てます
2. 命令に渡された`initial_value`でアカウントデータを初期化します
...
1. 作成され初期化されるカウンターアカウント
2. 新しいアカウント作成に資金を提供する支払者アカウント
3. 新しいアカウントを作成するために呼び出すシステムプログラム

命令に必要なアカウントを定義するために、`accounts`
スライスに対するイテレータを作成し、`next_account_info`
関数を使用して各アカウントを取得します。定義するアカウントの数が、命令に必要なアカウントとなります。
...
2. 必要な最小「rent」残高を計算します。Solanaでは、アカウントに格納されているデータ量に基づいて、最小限のlamport（rent）残高を維持する必要があります。
...
Programの`create_account`命令を呼び出してアカウントを作成します。
...
1. 新しいアカウントを作成します
2. アカウントのデータフィールド用に指定されたスペースを割り当てます
3. 指定されたプログラムに所有権を移転します

この所有権の移転は重要です。なぜなら、アカウントのデータを変更できるのはアカウントのプログラム所有者だけだからです。この場合、私たちのプログラムを所有者として設定することで、カウンター値を格納するためにアカウントのデータを変更できるようになります。
...
次に、`process_increment_counter`関数の実装を追加します。この命令は、既存のカウンターアカウントの値をインクリメントします。
...
プログラムのインストラクションをテストするには、以下の依存関係を
...
次に、以下のテストモジュールを`lib.rs`に追加し、`cargo test-sbf`を実行してテストを実行します。オプションとして、`--nocapture`フラグを使用すると出力内のprint文を確認できます。
...
次に、`ProgramTest`を使用してテストをセットアップします。そして、初期化するカウンターアカウントのアドレスとして使用する新しいkeypairを作成し、カウンターに設定する初期値を定義します。
...
インストラクションを構築する際、各アカウントは
[`AccountMeta`](https://github.com/solana-labs/solana/blob/v2.0/sdk/program/src/instruction.rs#L539-L545)として提供する必要があります。これは以下を指定します：
...
初期化インストラクションをテストするには：

- バリアント0（`InitializeCounter`）と初期値を含むinstruction dataを作成
- プログラムID、instruction data、必要なアカウントでインストラクションを構築
- 初期化インストラクションを含むトランザクションを送信
...
インクリメントインストラクションをテストするには：

- プログラムID、instruction data、必要なアカウントでインストラクションを構築
- インクリメントインストラクションを含むトランザクションを送信
- アカウントが正しい値にインクリメントされたことを確認

インクリメントインストラクションのinstruction
dataは`[1]`であることに注意してください。これはバリアント1（`IncrementCounter`）に対応します。インクリメントインストラクションには追加のパラメータがないため、データは単にインストラクションバリアントのみです。
