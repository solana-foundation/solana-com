---
title: トークンアカウントを作成する
description: SPLトークンアカウントの作成方法を学びましょう。
---

## トークンアカウントとは？

トークンアカウントは、Solanaのtoken
accountと呼ばれるアカウントタイプで、特定のトークン（ミント）に対する個人の所有権情報を保存します。各token
accountは単一のミントに関連付けられ、トークン残高や所有者などの詳細を追跡します。
...
<Callout type="info">
  ...
  Extensions
  Program](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/state.rs#L102-L123)は、トークンアカウントの基本実装を共有しています。
</Callout>

特定のミントのトークンを保有するには、ユーザーはまずトークンアカウントを作成する必要があります。各トークンアカウントは以下を追跡します：

1. 特定のミント（トークンアカウントが保有するトークンの種類）
2. 所有者（アカウントからトークンを転送できる権限を持つ者）

Solana上のUSDコイン（USDC）を使用した例を示します：

- USDコインのミントアドレス：`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Circle（USDコインの発行者）は `3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
  にトークンアカウントを持っています
- このトークンアカウントはUSDコイントークン（ミント）の単位のみを保有します
- Circleは `7VHUFJHWu2CuExkJcJrzhQPJ2oygupTWkL2A2For4BmE`
  に所有者権限を持ち、これらのトークンを転送できます

[Solana Explorer](https://explorer.solana.com/address/3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa)でこのトークンアカウントの詳細を確認できます。

<Callout type="info">
  「所有者」という用語は2つの異なる文脈で使用されています：

1. トークンアカウントの「所有者」 - これはトークンアカウントのデータに保存されているアドレスで、Token
   Programで定義されている[Account](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/state.rs#L106)タイプの「owner」フィールドです。所有者はアカウントからトークンの転送、バーン、または委任を行うことができます。このアドレスは、プログラム所有者と区別するために、トークンアカウントの「権限」と呼ばれることもあります。

2. プログラム「owner」 - これはSolana上でアカウントデータを所有するプログラムを指します。トークンアカウントの場合、これは常にToken
   ProgramまたはToken Extension
   Programのいずれかであり、基本的なSolanaの[Account](https://github.com/anza-xyz/solana-sdk/blob/4e30766b8d327f0191df6490e48d9ef521956495/account/src/lib.rs#L51)
   タイプの「owner」フィールドで指定されています。

トークンアカウントを扱う場合、「owner」は通常、アカウントを所有するプログラムではなく、トークンを使用する権限を持つ権限者を指します。

</Callout>

## Associated Token Accountとは何か？

Associated token
accountは、[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program)によって作成されたProgram
Derived Address (PDA)のアドレスを持つトークンアカウントです。Associated token
accountは、ユーザーが特定のトークン（ミント）の単位を保持するためのデフォルトのトークンアカウントと考えることができます。

<Callout type="info">
  「associated token accounts」という用語は、Associated Token
  Programが作成するトークンアカウントにのみ適用されます。
</Callout>

Associated token
accountsは、任意のミントに対するユーザーのトークンアカウントを決定論的に見つける方法を提供します。導出の実装は[こちら](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/interface/src/address.rs#L56)で確認できます。
...
この決定論的な導出により、ウォレットアドレスとトークンミントの任意の組み合わせに対して、正確に1つのassociated
token
accountアドレスが存在することが保証されます。このアプローチにより、任意のトークンミントに対するユーザーのトークンアカウントを簡単に見つけることができ、トークンアカウントアドレスを個別に追跡する必要がなくなります。

<Callout type="info">
  Associated Token
  Programは、決定論的なアドレス（PDA）を持つトークンアカウントを作成するヘルパープログラムとして機能します。Associated
  token accountを作成する際、Associated Token ProgramはToken ProgramまたはToken
  Extension Programに対してCPI（Cross-Program
  Invocation）を行います。トークンプログラムは作成されたアカウントを所有し、そのアカウントはトークンプログラムで定義されているのと同じ`Account`型構造を持ちます。Associated
  Token Programは状態を維持しません -
  単に決定論的なアドレス（PDA）でトークンアカウントを作成する標準化された方法を提供するだけです。
</Callout>
...
命令を呼び出します。この命令の実装は
[こちら](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/processor.rs#L145)で確認できます。

token accountを作成するトランザクションには2つの命令が必要です：

1. System Programを呼び出して、token
   accountを作成し、スペースを割り当て、所有権をToken Programに移転します。
2. Token Programを呼び出して、token accountデータを初期化します。
...
associated token accountを作成するには、
...
命令を呼び出します。この命令の実装は
[こちら](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/program/src/processor.rs#L66)で確認できます。

associated token accountを作成する命令は、自動的にSystem
Programを呼び出してtoken accountを作成し、Token Programを呼び出してtoken
accountデータを初期化します。これはCross Program
Invocation（CPI）を通じて行われます。
