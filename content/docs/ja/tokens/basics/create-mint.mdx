...
Solana上のすべてのトークンはミントアカウントとして存在し、ミントアカウントのアドレスがネットワーク上での一意の識別子となります。

例えば、Solana上のUSDコイン（USDC）デジタルドルのミントアドレスは
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`です。このミントアドレスはSolanaエコシステム全体でUSDコインの一意の識別子として機能します。このミントアカウントの詳細は
[Solana Explorer](https://explorer.solana.com/address/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v)で確認できます。
...
[こちら](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/processor.rs#L79)で確認できます。

ミントアカウントを作成するトランザクションには2つのinstructionsが必要です：
