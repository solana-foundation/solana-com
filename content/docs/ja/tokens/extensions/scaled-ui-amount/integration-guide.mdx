...
| リベース済みトークン残高の表示         | 取引や流動性提供のためにUI上でスケーリングされた残高を表示する。（バックエンドでは引き続き生の金額を使用可能）                         | **P0** |
...
| 内部的に生の残高を追跡                 | スケーリングされた残高ではなく、オンチェーントランザクションのために生の残高を追跡する。これにより長期的により正確で管理しやすくなる。 | **P1** |
| 価格フィードの適応                     | 現在の価格を表示するために価格フィードが使用される場所では、エンドユーザーにスケーリングされた価格を提供する。                         | **P1** |
