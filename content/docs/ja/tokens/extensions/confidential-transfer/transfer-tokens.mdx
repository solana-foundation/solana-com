...
状態で構成され、機密転送が承認されたトークンアカウントを持っている必要があります。送信者のトークンアカウントには、転送元となる利用可能な機密残高がなければなりません。
...
1. クライアント側で
   ...
   **暗号文有効性証明 ([BatchedGroupedCiphertext3HandlesValidityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_grouped_ciphertext_validity/handles_3.rs#L63))**：3つの関係者（送信元、送信先、監査人）すべてに対して転送額の暗号文が適切に生成されていることを検証し、転送額が各関係者の公開鍵で正しく暗号化されていることを確認します。
...
- クライアントサイドで等価性証明、暗号文有効性証明、範囲証明を生成する
- Zk
...
- `confidential_transfer_close_context_state_account`
  メソッドは証明アカウントをクローズします。

### サンプルコード

以下の例では、token
accountの間で機密性を保ったトークン転送を行う方法を示しています。

この例を実行するには、以下のコマンドを使用してメインネットからクローンしたToken
Extensions
Programで、ローカルvalidatorを起動します。ローカルvalidatorを起動するにはSolana
...
<Callout type="info">
  執筆時点では、機密転送はデフォルトのローカルvalidatorでは有効になっていません。サンプルコードを実行するには、メインネットからToken
...
---CODE-PLACEHOLDER-a9e1c2d9c95e1edb0eda1e463c6cb8d0---
