...
*confidential-transfer*状態に設定され、機密転送が承認されたトークンアカウントを持っている必要があります。送信者のトークンアカウントには、転送可能な機密残高が利用可能である必要もあります。
...
1. クライアント側で
   ...
   **等価性証明 ([CiphertextCommitmentEqualityProofData](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/ciphertext_commitment_equality.rs#L56))**: 転送後の新しい利用可能残高の暗号文が対応する
   [Pedersenコミットメント](https://en.wikipedia.org/wiki/Commitment_scheme)と一致することを検証し、送信元アカウントの新しい利用可能残高が正しく
   *old_balance - transfer_amount*として計算されていることを確認します。
   ...
   **範囲証明 ([BatchedRangeProofU128Data](https://github.com/anza-xyz/agave/blob/8b33d6d311c95780362a7d235919e7b8d2345939/zk-token-sdk/src/instruction/batched_range_proof/batched_range_proof_u128.rs#L37))**: 新しい利用可能残高と転送額（上位/下位ビットに分割）がすべて非負であり、指定された範囲内にあることを検証します。

2. 各証明について：

   - ZK ElGamal証明プログラムを呼び出して、証明データを検証します。
   - 証明固有のメタデータを証明「コンテキスト状態」アカウントに保存し、他の命令で使用します。
...
3. [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
   命令を呼び出し、証明コンテキスト状態アカウントを提供します。

4. 証明コンテキスト状態アカウントを閉じて、それらの作成に使用されたSOLを回収します。

以下の図は、送信者のトークンアカウントから受信者のトークンアカウントへトークンを転送する際の手順を示しています。
...
あるトークンアカウントから別のトークンアカウントへ機密転送するには、以下を実行する必要があります：

- クライアントサイドで等価性証明、暗号文有効性証明、範囲証明を生成
- Zk
  ElGamal証明プログラムを呼び出して証明を検証し、「コンテキスト状態」アカウントを初期化
- 3つの証明アカウントを提供する[ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)命令を呼び出す
- 3つの証明アカウントをクローズしてrentを回収

`spl_token_client` クレートは以下のメソッドを提供します：

- 証明アカウントを作成する `confidential_transfer_create_context_state_account`
  メソッド
- `Transfer` 命令を呼び出す `confidential_transfer_transfer` メソッド
- 証明アカウントをクローズする
  `confidential_transfer_close_context_state_account` メソッド

### サンプルコード

以下の例では、トークンアカウント間で機密性の高いトークン転送を行う方法を示しています。

この例を実行するには、以下のコマンドを使用してメインネットからクローンしたToken
Extensions
Programで、ローカルvalidatorを起動します。ローカルvalidatorを起動するには、Solana
...
<Callout type="info">
  執筆時点では、機密転送はデフォルトのローカルvalidatorで有効になっていません。サンプルコードを実行するには、メインネットからToken
