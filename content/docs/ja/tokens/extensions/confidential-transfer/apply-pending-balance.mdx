...
トークンを機密的に送金する前に、公開トークン残高を機密残高に変換する必要があります。この変換は2段階で行われます：

1. **機密保留残高**：最初に、トークンは公開残高から「機密保留」残高に「預け入れ」されます。
2. **機密利用可能残高**：その後、保留残高が利用可能残高に「適用」され、トークンが機密送金に利用できるようになります。

このセクションでは、2番目の段階について説明します：保留残高を利用可能残高に適用すること。
...
<Callout type="info">
  執筆時点では、Confidential
  Transfersはデフォルトのローカルバリデータで有効になっていません。サンプルコードを実行するには、メインネットのToken
