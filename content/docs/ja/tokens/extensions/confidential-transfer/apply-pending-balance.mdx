---
title: 保留残高を適用する
description: 資金を利用可能にするために保留残高を適用する方法を学びましょう。
---

## 保留残高を利用可能残高に適用する方法

トークンを機密性を持って送金する前に、公開トークン残高を機密残高に変換する必要があります。この変換は2段階で行われます：

1. **機密保留残高**：最初に、トークンは公開残高から「機密保留」残高に「預け入れ」されます。
2. **機密利用可能残高**：その後、保留残高が利用可能残高に「適用」され、トークンが機密送金に利用できるようになります。

このセクションでは、2番目の段階について説明します：保留残高を利用可能残高に適用する方法です。

トークンが公開残高から「預け入れ」される場合や、あるtoken accountから別のtoken
accountに機密送金される場合、トークンは最初に機密保留残高に追加されます。機密送金にトークンを使用する前に、保留残高を利用可能残高に「適用」する必要があります。
...
以下の図は、保留残高を利用可能残高に適用するステップを示しています：
...
Extensions
Programでローカルバリデータを起動します。ローカルバリデータを起動するには、Solana
