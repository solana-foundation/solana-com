---
title: トークンの預け入れ
description: 機密状態へトークンを預け入れる方法を学びます。
---

## 機密保留残高へトークンを預け入れる方法

トークンを機密に転送する前に、公開トークン残高を機密残高に変換する必要があります。この変換は2段階で行われます：

1. **機密保留残高**：最初に、トークンは公開残高から「保留中」の機密残高に「預け入れ」されます。
2. **機密利用可能残高**：保留残高はその後、利用可能残高に「適用」され、トークンが機密転送に利用できるようになります。

このセクションでは最初の段階について説明します：公開トークン残高から機密保留残高への預け入れです。
...
instructionを呼び出します。預け入れinstructionごとの[最大金額](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/mod.rs#L21)は2^48に制限されています。
...
以下の例は、公開トークン残高を機密保留残高に預け入れる方法を示しています。

例を実行するには、以下のコマンドを使用して、メインネットからクローンしたToken
Extensions
Programでローカルvalidatorを起動します。ローカルvalidatorを起動するには、Solana
...
---CODE-PLACEHOLDER-a9e1c2d9c95e1edb0eda1e463c6cb3d7---
