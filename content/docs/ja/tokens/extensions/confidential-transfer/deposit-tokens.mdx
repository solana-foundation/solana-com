---
title: トークンの預け入れ
description: トークンを秘密状態に預け入れる方法を学びましょう。
---

## トークンを秘密保留残高に預け入れる方法

トークンを秘密裏に送金する前に、公開トークン残高を秘密残高に変換する必要があります。この変換は2段階で行われます：

1. **秘密保留残高**：最初に、トークンは公開残高から「保留中」の秘密残高に「預け入れ」されます。
2. **秘密利用可能残高**：保留残高はその後、利用可能残高に「適用」され、トークンが秘密送金に利用できるようになります。

このセクションでは最初の段階について説明します：公開トークン残高を秘密保留残高に預け入れる方法です。

以下の図は、公開残高から秘密保留残高にトークンを預け入れる際の手順を示しています：
...
公開残高を秘密保留残高に変換するには、[ConfidentialTransferInstruction::Deposit](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L386)命令を呼び出します。預け入れ命令ごとの[最大金額](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/mod.rs#L21)は2^48に制限されています。

`spl_token_client` クレートは `confidential_transfer_deposit`
メソッドを提供しており、これは以下の例で示すように `Deposit`
命令を含むトランザクションを構築して送信します。

### サンプルコード

以下の例は、公開トークン残高を秘密保留残高に預け入れる方法を示しています。
...
<Callout type="info">
  執筆時点では、秘密送金機能はデフォルトのローカルvalidatorでは有効になっていません。サンプルコードを実行するには、メインネットのToken
