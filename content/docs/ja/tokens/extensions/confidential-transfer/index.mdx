...
## Confidential Transferとは何ですか？

Confidential
transferを使用すると、転送金額を明かさずにトークンアカウント間でトークンを転送できます。これはプライバシーを保護するトランザクションに役立ちます。転送金額とトークン残高のみが非公開となり、トークンアカウントのアドレスは公開されたままです。
...
- [Confidential Balancesクックブック](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Confidential Transfer拡張機能の使用方法に関するコードスニペット

### どのように機能しますか？

Confidential
Transfer拡張機能は、転送金額を明かさずにアカウント間でトークンを転送できるようにする[命令](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)をToken
Extensions Programに追加します。
...
1. Confidential transfer拡張機能を持つmint accountを作成します。
2. 送信者と受信者のためにConfidential transfer拡張機能を持つtoken
   accountを作成します。
3. 送信者アカウントにトークンをミントします。
4. 送信者の**公開残高**を**機密保留残高**に**デポジット**します。
5. 送信者の保留残高を**機密利用可能残高**に**適用**します。
6. 送信者のトークンアカウントから受信者のトークンアカウントにトークンを機密に**転送**します。
7. 受信者の保留残高を**機密利用可能残高**に**適用**します。
8. 受信者の機密利用可能残高を**公開残高**に**引き出し**ます。

機密転送フローのステップの詳細については、以下の対応するページを参照してください：
...
<Cards>
  ...
  >
    Confidential Transfer拡張機能を持つミントアカウントの作成方法
  </Card>
  <Card
    title="トークンアカウントの作成"
...
例を実行するには、以下のコマンドを使用して、メインネットからクローンしたToken
Extension
Programでローカルvalidatorを起動します。ローカルvalidatorを使用するには、Solana
...
機密転送拡張の完全な[instructions](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)リストは以下の通りです：

| instruction                         | 説明                                                                                                                                                      |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | 機密転送用のmint accountを設定します。このinstructionは、_rs`TokenInstruction::InitializeMint`_ instructionと同じトランザクションに含める必要があります。 |
| _rs`UpdateMint`_                    | mintの機密転送設定を更新します。                                                                                                                          |
| _rs`ConfigureAccount`_              | 機密転送用のtoken accountを設定します。                                                                                                                   |
...
| _rs`ApplyPendingBalance`_           | 入金または転送後、保留中の残高を利用可能な残高に変換します。                                                                                              |
...
| _rs`ConfigureAccountWithRegistry`_  | _rs`VerifyPubkeyValidity`_ 証明の代わりに _rs`ElGamalRegistry`_ アカウントを使用して、機密転送用のtoken accountを構成する代替方法です。                   |
