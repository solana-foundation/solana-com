...
- **auditor_elgamal_pubkey**: 機密取引の転送額を復号化できるオプションの監査者で、一般公開からプライバシーを維持しながらコンプライアンスメカニズムを提供します。

### 必要なinstructions

Confidential
Transferを有効にしたミントの作成には、単一のトランザクションで3つのinstructionsが必要です：
...
3. **ミントの初期化**: Token Extensions
   Programの\_rs`Instruction::InitializeMint`\_instructionを呼び出して、標準のミント状態を初期化します。

これらのinstructionsを手動で記述することもできますが、`spl_token_client`クレートは`create_mint`メソッドを提供しており、以下の例に示すように、3つのinstructionsをすべて含むトランザクションを1回の関数呼び出しで構築して送信します。
...
例を実行するには、次のコマンドを使用してメインネットからクローンしたToken
Extension
