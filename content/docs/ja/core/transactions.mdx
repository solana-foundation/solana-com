...
title: トランザクションと命令
description:
  Solanaトランザクションと命令について学びましょう -
  Solanaブロックチェーンと対話するための基本的な構成要素です。実践的な例を通じてトランザクション構造と命令の構成を理解しましょう。
---

Solanaでは、ユーザーはネットワークと対話するために[トランザクション](/docs/core/transactions#transaction)を送信します。トランザクションには、処理する操作を指定する1つ以上の[命令](/docs/core/transactions#instruction)が含まれています。命令の実行ロジックはSolanaネットワークにデプロイされた[プログラム](/docs/core/programs)に格納されており、各プログラムは独自の命令セットを定義しています。
...
送信者アカウントはトランザクションに署名（`is_signer`）して、System
Programがlamport残高を減少させることを許可する必要があります。送信者と受信者のアカウントはlamport残高が変更されるため、書き込み可能（`is_writable`）である必要があります。

トランザクションを送信した後、System
Programは転送命令を処理します。その後、System
...
以下の例は、転送命令を手動で構築する方法を示しています。`Expanded Instruction`タブは`Instruction`タブと機能的に同等です。
...
Solana [プログラム](/docs/core/accounts#program-account)
上の[インストラクション](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/instruction/src/lib.rs#L94)は、Solanaネットワークを使用して誰でも呼び出せるパブリック関数と考えることができます。
...
- instruction
  data：プログラム上で呼び出すインストラクションとそのインストラクションが必要とする引数を指定するバイト配列
...
![トランザクションインストラクション](/assets/docs/core/transactions/instruction.svg)

### AccountMeta

インストラクションが必要とする各アカウントは、以下を含む[AccountMeta](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/instruction/src/account_meta.rs#L25)として提供する必要があります：

- `pubkey`：アカウントのアドレス
- `is_signer`：アカウントがトランザクションに署名する必要があるかどうか
- `is_writable`：インストラクションがアカウントのデータを変更するかどうか
...
2. [メッセージ](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/message/legacy.rs#L131)：アトミックに処理される命令のリスト。
...
- [アカウントアドレス](/docs/core/transactions#array-of-account-addresses)：トランザクション上の命令に必要なアカウントアドレスの配列。
- [最近のブロックハッシュ](/docs/core/transactions#recent-blockhash)：トランザクションのタイムスタンプとして機能します。
- [命令](/docs/core/transactions#array-of-instructions)：実行される命令の配列。
...
このフォーマットは、トランザクションメッセージ内の[アカウントアドレス](/docs/core/transactions#array-of-account-addresses)と[instruction data](/docs/core/transactions#array-of-instructions)配列の長さをエンコードするために使用されます。

### アカウントアドレスの配列

トランザクションメッセージには、そのinstructionが必要とする[アカウントアドレス](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/message/legacy.rs#L138)の配列が含まれています。配列は、含まれるアドレスの数を示す[compact-u16](/docs/core/transactions#compact-array-format)から始まります。アドレスはメッセージヘッダーによって決定される権限によって順序付けられています。
...
すべてのトランザクションには、次の2つの目的を果たす[最近のブロックハッシュ](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/message/legacy.rs#L141)が必要です：
...
トランザクションメッセージには、[CompiledInstruction](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/instruction.rs#L22)型の[instruction](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/message/legacy.rs#L146)配列が含まれています。instructionはトランザクションに追加されるときにこの型に変換されます。
...
1. プログラムIDインデックス：アカウントアドレス配列内のプログラムのアドレスを指すu8インデックスです。これは命令を処理するプログラムを指定します。
2. アカウントインデックス：この命令に必要なアカウントアドレスを指すu8インデックスの配列です。
3. instruction
   data：プログラム上で呼び出す命令と、命令に必要な追加データ（例：関数の引数）を指定するバイト配列です。
...
![命令のコンパクト配列](/assets/docs/core/transactions/compact_array_of_ixs.png)
...
- `accountKeys`：トランザクションの命令で使用されるすべてのアカウントアドレスの配列

- `recentBlockhash`：トランザクションのタイムスタンプに使用されるブロックハッシュ

- `instructions`：実行する命令の配列。各命令の `account` と `programIdIndex`
  は、インデックスによって `accountKeys` 配列を参照します。

- `signatures`：トランザクションの命令によって署名者として必要とされるすべてのアカウントの署名を含む配列。署名は、アカウントの対応する秘密鍵を使用してトランザクションメッセージに署名することで作成されます。
