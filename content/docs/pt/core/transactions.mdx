...
- As transações são **atômicas** - se qualquer instrução falhar, toda a
  transação falha e nenhuma alteração ocorre.
...
- Cada instrução requer três informações:
  ...
  2. As contas das quais a instrução lê ou nas quais escreve
...
proprietário pode alterar os dados de uma conta, então transferir SOL requer o
envio de uma transação para invocar o System Program.
...
System Program deduza seu saldo de lamports. As contas do remetente e do
destinatário devem ser graváveis (`is_writable`) já que seus saldos de lamports
mudam.

Após o envio da transação, o System Program processa a instrução de
transferência. O System Program então atualiza os saldos de lamports de ambas as
...
Nas seções abaixo, vamos percorrer os detalhes de transações e instruções.

## Instruções

Uma
[instrução](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/instruction/src/lib.rs#L94)
no [programa](/docs/core/accounts#program-account) Solana pode ser entendida
...
- [ID do Programa](mention:program-id): O endereço do programa com a lógica de
  negócios para a instrução sendo invocada.
- [Contas](mention:accounts): A lista de todas as contas que a instrução lê ou
  escreve.
...
1. [Assinaturas](mention:signatures): Uma matriz de
   [assinaturas](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/signature/src/lib.rs#L30)
   de todas as contas exigidas como signatárias para as instruções na transação.
   Uma assinatura é criada assinando o `Message` da transação com a chave
   privada da conta.
...
- [Endereços de Contas](mention:account-addresses): Um array de endereços de
  contas exigidos pelas instruções na transação.
...
[Endereços de Contas](/docs/core/transactions#array-of-account-addresses) e
...
da instrução. Cada instrução contém:
...
3. [instruction data](mention:instruction-data): Um array de bytes que
   ...
   exigidos pela instrução (ex: argumentos de função).
