...
Cada conta na Solana possui os seguintes campos.
...
lamports). O saldo de SOL de uma conta é a quantidade no campo `lamports`
convertida para SOL.

As contas da Solana devem ter um saldo mínimo de lamports que é proporcional à
...
## !!steps Campo Proprietário

O ID do programa (chave pública) do programa que possui esta conta.

Cada conta da Solana tem um programa designado como seu proprietário. Apenas o
...
lamports) para manter seus dados na rede. No entanto, este mecanismo de cobrança
...
Para armazenar dados na blockchain, as contas também devem manter um saldo de
...
O termo "rent" vem de um mecanismo descontinuado que regularmente deduzia
lamports de contas que ficavam abaixo do limite de rent. Este mecanismo não está
mais ativo.
...
Observe que todas as contas de "carteira" na Solana são "Contas do Sistema"
pertencentes ao System Program. O saldo de lamport nessas contas mostra a
quantidade de SOL pertencente à carteira. Apenas Contas do Sistema podem pagar
taxas de transação.
...
A implantação de um programa Solana cria um program account executável. O
program account armazena o código executável do programa. Program accounts são
de propriedade de um [Programa Carregador](/docs/core/programs#loader-programs).
...
Program accounts são de propriedade de um
...
executável diretamente no program account. O loader-v3 armazena o código
executável em uma "conta de dados do programa" separada, e o program account
...
O Loader-v3 tem um tipo especial de conta para temporariamente preparar o upload
de um programa durante a implantação ou atualizações. No loader-v4, ainda
existem buffers, mas são apenas contas de programa normais.

### Conta de Dados do Programa

O Loader-v3 funciona de forma diferente de todos os outros programas BPF Loader.
A conta do programa contém apenas o endereço de uma conta de dados do programa,
...
sistemas operacionais tipicamente têm arquivos separados para programas e seus
dados.
...
1. Invocar o System Program para criar uma conta, depois transferir a
...
O exemplo a seguir mostra como criar e buscar uma conta Token Mint controlada
pelo programa Token 2022.
