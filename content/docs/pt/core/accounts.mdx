...
Para simplificar, você pode tratar a program account como o próprio programa.
Quando você invoca as instruções de um programa, você especifica o endereço da
program account (comumente chamado de "Program ID").

O exemplo a seguir busca a Token Program account para mostrar que as program
accounts têm o mesmo tipo base `Account`, exceto que o campo `executable` é
definido como `true`. Como as program accounts contêm código executável em seu
...
Quando você implanta um programa Solana, ele é armazenado em uma program
account. As program accounts são de propriedade de um
[Loader Program](/docs/core/programs#loader-programs). Existem várias versões do
loader, mas todas, exceto o loader-v3, armazenam o código executável diretamente
na program account. O loader-v3 armazena o código executável em uma "program
data account" separada, e a program account apenas aponta para ela. Quando você
implanta um novo programa, a CLI da Solana usa a versão mais recente do loader
por padrão.
...
O loader-v3 tem um tipo especial de conta para preparar temporariamente o upload
...
O loader-v3 funciona de maneira diferente de todos os outros programas BPF
Loader. A program account contém apenas o endereço de uma program data account,
...
Na Solana, o código executável de um programa é armazenado em uma conta
diferente do estado do programa. Isso é semelhante a como os sistemas
operacionais geralmente têm arquivos separados para programas e seus dados.

Para manter o estado, os programas definem instruções para criar contas
separadas que eles possuem. Cada uma dessas contas tem seu próprio endereço
único e pode armazenar quaisquer dados arbitrários definidos pelo programa.
...
O exemplo a seguir mostra como criar e buscar uma conta Token Mint possuída pelo
programa Token 2022.
