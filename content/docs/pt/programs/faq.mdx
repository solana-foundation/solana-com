...
Os programas onchain da Solana são compilados através da
[infraestrutura do compilador LLVM](https://llvm.org/) para um
[Formato Executável e Linkável (ELF)](https://en.wikipedia.org/wiki/Executable_and_Linkable_Format)
...
tentar ler ou escrever em um endereço virtual ao qual não foi concedido acesso,
e um erro de "access violation" será retornado contendo o endereço e o tamanho
da violação tentada.
...
`Pack` então tente comprimir e descomprimir o tipo de instrução `T` para
...
O SBF usa frames de pilha em vez de um ponteiro de pilha variável. Cada frame de
pilha tem 4KB de tamanho.

Se um programa violar esse tamanho de frame de pilha, o compilador reportará o
...
A mensagem identifica qual símbolo está excedendo seu frame de pilha, mas o nome
...
A razão pela qual um aviso é reportado em vez de um erro é porque algumas crates
dependentes podem incluir funcionalidades que violam as restrições de frame de
pilha mesmo se o programa não usar essa funcionalidade. Se o programa violar o
tamanho da pilha em tempo de execução, um erro `AccessViolation` será reportado.

Os frames de pilha SBF ocupam um intervalo de endereço virtual começando em
`0x200000000`.

## Tamanho do heap

Os programas têm acesso a um heap de tempo de execução através das APIs `alloc`
do Rust. Para facilitar alocações rápidas, é utilizado um simples heap de 32KB.
...
Para todos os fins práticos, os programas devem sempre ser escritos para o BPF
loader mais recente, e o loader mais recente é o padrão para a interface de
linha de comando e as APIs javascript.

- [Pontos de entrada de programas Rust](/docs/programs/lang-rust#program-entrypoint)

### Implantação

A implantação de programas SBF é o processo de upload de um objeto compartilhado
BPF nos dados de uma conta de programa e marcação da conta como executável. Um
...
deslocamentos alinhados dentro da matriz de bytes alinhada. Isso permite que
