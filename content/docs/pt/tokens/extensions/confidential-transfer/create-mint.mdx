...
description:
  Aprenda como criar um token mint com a extensão Confidential Transfer.
---
...
adicionando estado extra à conta mint. Esta seção explica como criar um token
mint com esta extensão ativada.
...
Criar um mint com Confidential Transfer ativado requer três instruções em uma
única transação:

1. **Criar a Mint Account**: Invocar a instrução _rs`CreateAccount`_ do System
   Program para criar a mint account.
