...
Para usar a extensão Scaled UI Amount, você precisa habilitá-la em um mint de
token ou conta de token. Observe que uma vez que um token é criado, você não
...
### Atualizar o multiplicador de quantidade da UI
