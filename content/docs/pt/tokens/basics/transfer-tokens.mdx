...
description: Aprenda como transferir tokens entre contas de token.
---

## Como transferir tokens

Transferir tokens envolve mover tokens de uma conta de token para outra conta de
token que compartilham o mesmo mint. A transferência de tokens ocorre quando
você invoca a instrução
...
em um token program. Apenas o endereço especificado como proprietário
(autoridade) da conta de token de origem pode transferir tokens para fora da
conta.
