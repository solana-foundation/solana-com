...
description: Aprenda como transferir tokens entre token accounts.
---

## Como transferir tokens

Transferir tokens envolve mover tokens de um token account para outro token
account que compartilham o mesmo mint. A transferência de tokens ocorre quando
...
(authority) do token account de origem pode transferir tokens para fora da
conta.
