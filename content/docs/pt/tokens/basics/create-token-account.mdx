...
saldo de tokens e o proprietário.
...
2. O "owner" do programa - Isso se refere ao programa que possui os dados da
   conta na Solana. Para contas de token, isso é sempre o Token Program ou Token
   Extensions Program, conforme especificado no campo "owner" do tipo base
   Solana
...
de carteira e mint de token, existe exatamente um endereço de associated token
account. Esta abordagem torna simples encontrar a token account de um usuário
para qualquer mint de token, eliminando a necessidade de rastrear endereços de
