...
Esta seção apresenta alguns comandos comuns da CLI Solana para ajudá-lo a
começar.
...
[validador de teste](https://docs.anza.xyz/cli/examples/test-validator)
integrado para desenvolvimento local.

Em um terminal separado, execute o seguinte comando para iniciar um validador
local:
...
Quando usar `localnet` como o `cluster` no `Anchor.toml`, o Anchor
