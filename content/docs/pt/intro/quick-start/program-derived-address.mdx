...
Em `lib.rs`, atualize a struct _rs`MessageAccount`_ com o seguinte:
...
O atributo _rs`#[account]`_ em um programa Anchor anota structs que representam
dados de conta (tipo de dados para armazenar no campo de dados da Conta).
...
O campo _rs`message`_ contém um _rs`String`_ com a mensagem do usuário.
...
Neste exemplo, a struct _rs`MessageAccount`_ armazena uma mensagem criada pelos
usuários que contém três campos:
...
Juntos, esses elementos permitem que o Anchor verifique automaticamente as
contas passadas para a instrução e proteja o programa.
...
Como parte da configuração, o arquivo de teste deriva a PDA da conta de
mensagem. Isso mostra como derivar a PDA em Javascript usando as mesmas seeds
