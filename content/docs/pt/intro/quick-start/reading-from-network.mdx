...
description:
  ...
  contas de carteira, contas de programa e contas de mint de tokens usando
...
Na Solana, todos os dados existem em "accounts". Você pode pensar nos dados na
Solana como um banco de dados público com uma única tabela "Accounts", onde cada
entrada é uma conta com o mesmo tipo base de
...
| `executable` | Indica se a conta contém código de programa executável. Para carteiras e contas de estado, este valor é `false`.                          |
...
autorizadas a criar ou congelar tokens. O endereço da conta Mint identifica
...
Estes são os pontos-chave a observar sobre os dados da conta Mint:

| Campo        | Descrição                                                                                                          |
| ------------ | ------------------------------------------------------------------------------------------------------------------ |
| `executable` | Este campo é `false` porque a conta mint armazena estado, não código executável.                                   |
| `data`       | Este campo contém o estado serializado da conta, como a autoridade de mint, fornecimento total, número de decimais |
| `owner`      | O Token Program (`TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA`) é proprietário da conta mint.                      |
...
O campo `data` de uma conta Solana contém bytes brutos. Para interpretar estes
...
A maioria dos programas Solana fornece bibliotecas cliente com funções
