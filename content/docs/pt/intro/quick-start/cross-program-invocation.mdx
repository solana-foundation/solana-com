...
- O endereço da conta vem de um PDA derivado usando as seeds
  _rs`[b"vault", user.key().as_ref()]`_
- Como um PDA, ele não possui chave privada, então apenas o programa pode
  "assinar" pelo endereço ao realizar CPIs
...
- `to` - A conta vault (destino dos fundos)
...
cliente são construídas, onde você especifica o programa, as contas e os
instruction data para uma instrução específica a ser invocada. Quando a
instrução `update` do programa recebe uma invocação, ela internamente invoca a
instrução de transferência do System Program.
...
Isso permite que um programa "assine" por um PDA que foi derivado do seu program
ID.

Durante o processamento da instrução, o runtime verifica se as signer seeds
fornecidas derivam corretamente para o endereço do PDA. Se verificado, o runtime
trata essa conta PDA como um signatário durante a duração do CPI.
...
Na instrução de exclusão, outra Cross Program Invocation (CPI) implementa a
instrução de transferência do System Program. Este CPI demonstra como fazer uma
transferência que requer um signatário Program Derived Address (PDA).

Primeiro, defina as signer seeds para o PDA do vault:
...
- As sementes de assinatura para o PDA
...
- O valor a ser transferido (o saldo total da conta vault)
...
o programa modificado fique disponível para testes. Na Solana, atualizar um
...
define a autoridade de atualização durante a implantação do programa, e esta
