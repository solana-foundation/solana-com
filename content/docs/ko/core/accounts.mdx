...
간단히 말해, program account를 프로그램 자체로 취급할 수 있습니다. 프로그램의
명령을 호출할 때, program account의 주소(일반적으로 "Program ID"라고 함)를
지정합니다.
...
솔라나에서는 프로그램의 실행 코드가 프로그램의 상태와는 다른 계정에 저장됩니다.
이는 운영 체제가 일반적으로 프로그램과 그 데이터를 위한 별도의 파일을 가지는
방식과 유사합니다.
...
수 있다는 점에 유의하세요. System Program이 계정을 생성하면, 그 후에 새 계정의
소유권을 다른 프로그램에 할당할 수 있습니다.
...
다음 예제는 Token 2022 프로그램이 소유한 토큰 Mint 계정을 생성하고 가져오는
방법을 보여줍니다.
