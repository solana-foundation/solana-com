...
title: 교차 프로그램 호출
description:
  Solana에서 교차 프로그램 호출(CPI)에 대해 배우기 - 프로그램이 다른 프로그램의
  명령을 호출하고, PDA 서명자를 처리하며, Solana 네트워크 전반에 걸쳐 기능을
  구성하는 방법에 대해 설명합니다.
h1: 교차 프로그램 호출 (CPI)
---

교차 프로그램 호출(CPI)은 한 프로그램이 다른 프로그램의 명령을 호출하는 것을
의미합니다. 이는 Solana 프로그램의 조합 가능성을 허용합니다.

명령을 프로그램이 네트워크에 노출하는 API 엔드포인트로 생각할 수 있으며,
CPI는 내부적으로 하나의 API가 다른 API를 호출하는 것으로 볼 수 있습니다.

![교차 프로그램 호출](/assets/docs/core/cpi/cpi.svg)

## 주요 사항

- **교차 프로그램 호출**은 Solana 프로그램 명령이 다른 프로그램의 명령을
  직접 호출할 수 있게 합니다.
- 호출 프로그램의 **서명자 권한**은 피호출 프로그램으로 확장됩니다.
- 교차 프로그램 호출을 할 때, **프로그램은 자신의 프로그램 ID에서 파생된
  PDA를 대신하여 서명할 수 있습니다.**
- 피호출 프로그램은 다른 프로그램에 대해 추가적인 CPI를 수행할 수 있으며,
  최대 깊이는 4입니다.

## CPI란 무엇인가요?

교차 프로그램 호출(CPI)은 한 프로그램이 다른 프로그램의 명령을 호출하는
경우를 말합니다.

CPI로 프로그램 명령을 작성하는 것은 트랜잭션에 추가할 [명령](/docs/core/transactions#instruction)을
구성하는 것과 동일한 패턴을 따릅니다.
내부적으로 각 CPI 명령은 다음을 지정해야 합니다:

- **프로그램 주소**: 호출할 프로그램을 지정합니다.
- **계정**: 명령이 읽거나 쓰는 모든 계정을 나열하며,
  다른 프로그램도 포함됩니다.
- **명령 데이터**: 프로그램에서 호출할 명령과 명령이 필요로 하는 모든 데이터
  (함수 인수)를 지정합니다.

프로그램이 다른 프로그램에 대해 교차 프로그램 호출(CPI)을 수행할 때:

- 초기 트랜잭션의 서명자 권한은 피호출 프로그램으로 확장됩니다 (예: A->B)
- 피호출 프로그램은 다른 프로그램에 대해 최대 깊이 4까지 추가 CPI를 수행할 수 있습니다
  (예: B->C, C->D)
...
<Callout>
  ...
  값은 5입니다. 이는 프로그램 명령 호출 스택의 최대 높이를 나타냅니다.
  스택 높이는 초기 트랜잭션에 대해 1에서 시작하며, 프로그램이 다른 명령을 호출할 때마다
  1씩 증가합니다. 이 설정은 CPI의 호출 깊이를 4로 제한합니다.
</Callout>

트랜잭션이 처리될 때, 계정 권한은 한 프로그램에서 다른 프로그램으로 확장됩니다.
이는 다음과 같은 의미입니다:

프로그램 A가 다음과 같은 명령을 받았다고 가정해 보겠습니다:

- 트랜잭션에 서명한 계정
- 쓸 수 있는 계정 (변경 가능)

프로그램 A가 프로그램 B에 대해 CPI를 수행할 때:

- 프로그램 B는 이러한 동일한 계정을 원래 권한으로 사용할 수 있습니다.
- 프로그램 B는 서명 계정으로 서명할 수 있습니다.
- 프로그램 B는 쓸 수 있는 계정에 쓸 수 있습니다.
- 프로그램 B는 자신의 CPI를 수행할 경우 이러한 동일한 권한을 전달할 수 있습니다.
...
배열과 함께 `invoke_signed` 함수를 호출하여 서명이 필요하지 않음을 나타냅니다.
...
다음 예제는 [Anchor Framework](https://www.anchor-lang.com/docs)와 Native Rust를 사용하여
CPI를 만드는 방법을 보여줍니다. 예제 프로그램은 CPI를 사용하여 한 계정에서 다른 계정으로
SOL을 전송하는 단일 명령을 포함합니다.

### Anchor Framework

다음 예제는 Anchor 프로그램에서 교차 프로그램 호출(CPI)을 생성하는 세 가지 방법을
제시하며, 각 방법은 서로 다른 추상화 수준을 가지고 있습니다. 모든 예제는 동일한 방식으로
작동합니다. 주요 목적은 CPI의 구현 세부 사항을 보여주는 것입니다.

- 예제 1: Anchor의 _rs`CpiContext`_ 및 도우미 함수를 사용하여 CPI 명령을 구성합니다.
- 예제 2: `solana_program` 크레이트의 _rs`system_instruction::transfer`_ 함수를 사용하여
  CPI 명령을 구성합니다. 예제 1은 이 구현을 추상화합니다.
- 예제 3: CPI 명령을 수동으로 구성합니다. 이 접근 방식은 명령을 호출하는 데 도움이 되는
  크레이트가 없을 때 유용합니다.
...
다음 예제는 Native Rust로 작성된 프로그램에서 CPI를 만드는 방법을 보여줍니다.
이 프로그램은 CPI를 사용하여 한 계정에서 다른 계정으로 SOL을 전송하는 단일 명령을 포함합니다.
테스트 파일은 프로그램을 테스트하기 위해 [LiteSVM](https://github.com/LiteSVM/litesvm)을 사용합니다.
...
## PDA 서명자가 있는 교차 프로그램 호출

[`invoke_signed`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/program.rs#L51-L73)
함수는 PDA 서명자가 필요한 CPI를 처리합니다. 이 함수는 서명자 PDA를 파생하기 위한
시드를 `signer_seeds`로 받습니다.

PDA를 파생하는 방법에 대한 자세한 내용은 [프로그램 파생 주소](/docs/core/pda) 페이지를 참조하세요.
...
CPI를 포함하는 명령을 처리할 때, Solana 런타임은 내부적으로
[`create_program_address`](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/bpf_loader/src/syscalls/cpi.rs#L552)
를 호출하여 `signers_seeds`와 호출 프로그램의 `program_id`를 사용합니다. 유효한 PDA가
검증되면, 주소는 [유효한 서명자](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/bpf_loader/src/syscalls/cpi.rs#L554)로 추가됩니다.

다음 예제는 [Anchor Framework](https://www.anchor-lang.com/docs)와 Native Rust를 사용하여
PDA 서명자가 있는 CPI를 만드는 방법을 보여줍니다. 예제 프로그램은 PDA에서 수신 계정으로
SOL을 전송하는 단일 명령을 포함하며, 이 명령은 PDA에 의해 서명됩니다.

### Anchor Framework

다음 예제는 Anchor 프로그램에서 교차 프로그램 호출(CPI)을 구현하는 세 가지 접근 방식을
포함하며, 각 접근 방식은 서로 다른 추상화 수준을 가지고 있습니다. 모든 예제는 기능적으로 동등합니다.
주요 목적은 CPI의 구현 세부 사항을 설명하는 것입니다.

- 예제 1: Anchor의 _rs`CpiContext`_ 및 도우미 함수를 사용하여 CPI 명령을 구성합니다.
- 예제 2: `solana_program` 크레이트의 _rs`system_instruction::transfer`_ 함수를 사용하여
  CPI 명령을 구성합니다. 예제 1은 이 구현을 추상화합니다.
- 예제 3: CPI 명령을 수동으로 구성합니다. 이 접근 방식은 호출하려는 명령을 구성하는 데 도움이 되는
  크레이트가 없을 때 유용합니다.
...
다음 예제는 Native Rust로 작성된 프로그램에서 PDA 서명자가 있는 CPI를 만드는 방법을
보여줍니다. 이 프로그램은 PDA에서 수신 계정으로 SOL을 전송하는 단일 명령을 포함하며,
이 명령은 PDA에 의해 서명됩니다. 테스트 파일은 프로그램을 테스트하기 위해
