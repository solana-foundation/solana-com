...
description:
  ...
  걸쳐 기능을 구성하는 방법을 배워보세요.
h1: Cross Program Invocation (CPI)
---

Cross Program Invocation(CPI)는 하나의 프로그램이 다른 프로그램의 명령어를
...
- **Cross Program Invocations**는 Solana 프로그램 명령어가 다른 프로그램의
...
CPI가 포함된 프로그램 명령어를 작성하는 것은
[instruction](/docs/core/transactions#instruction)을 트랜잭션에 추가하기 위해
구성하는 것과 동일한 패턴을 따릅니다. 내부적으로 각 CPI 명령어는 다음을 지정해야
합니다:
...
<Callout>
  ...
  를 5로 설정합니다. 이는 프로그램 명령 호출 스택의 최대 높이를 나타냅니다. 스택
  높이는 초기 트랜잭션에서 1로 시작하고 프로그램이 다른 명령을 호출할 때마다 1씩
  증가합니다. 이 설정은 CPI의 호출 깊이를 4로 제한합니다.
</Callout>
...
다음 예제는 Anchor 프로그램에서 Cross Program Invocations(CPI)를 생성하는 세
...
- 예제 2: `solana_program` 크레이트의 _rs`system_instruction::transfer`_ 함수를
  사용하여 CPI 명령어를 구성합니다. 예제 1은 이 구현을 추상화합니다.
- 예제 3: CPI 명령어를 수동으로 구성합니다. 이 접근 방식은 명령어 구성을 돕는
  크레이트가 존재하지 않을 때 유용합니다.
...
함수는 PDA 서명자가 필요한 CPI를 처리합니다. 이 함수는 서명자 PDA를 파생하기
위한 seed를 `signer_seeds`로 받습니다.

PDA 파생 방법에 대한 자세한 내용은 [Program Derived Address](/docs/core/pda)
페이지를 참조하세요.
...
CPI를 포함하는 명령어를 처리할 때, Solana 런타임은 내부적으로 호출 프로그램의
...
Rust를 사용하여 PDA 서명자로 CPI를 수행하는 방법을 보여줍니다. 예제 프로그램은
PDA가 서명한 CPI를 사용하여 PDA에서 수신자 계정으로 SOL을 전송하는 단일 명령어를
포함합니다.

### Anchor 프레임워크

다음 예제들은 Anchor 프로그램에서 Cross Program Invocations(CPIs)를 구현하는 세
...
- 예제 1: Anchor의 _rs`CpiContext`_ 및 헬퍼 함수를 사용하여 CPI 명령을
  구성합니다.
- 예제 2: `solana_program` 크레이트에서 _rs`system_instruction::transfer`_
  함수를 사용하여 CPI 명령을 구성합니다. 예제 1은 이 구현의 추상화입니다.
- 예제 3: CPI 명령을 수동으로 구성합니다. 이 접근 방식은 호출하려는 명령을
  구축하는 데 도움이 되는 크레이트가 없을 때 유용합니다.
...
수신자 계정으로 SOL을 전송하는 단일 명령을 포함합니다. 테스트 파일은 프로그램을
테스트하기 위해 [LiteSVM](https://github.com/LiteSVM/litesvm)을 사용합니다.
