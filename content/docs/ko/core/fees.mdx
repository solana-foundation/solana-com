...
모든 Solana 트랜잭션은 검증자가 트랜잭션을 처리하는 데 대한 보상으로 기본
수수료(SOL)가 필요합니다. 또한 현재 리더(검증자)가 귀하의 트랜잭션을 처리할
가능성을 높이기 위해 선택적으로 우선순위 수수료를 지불할 수 있습니다.

## 주요 포인트

- 트랜잭션의 **기본 수수료**는 트랜잭션 서명당 5000 lamport입니다.
- **우선순위 수수료**(선택 사항)는 현재 리더가 귀하의 트랜잭션을 처리할 가능성을
  높이기 위해 검증자에게 지불하는 추가 수수료입니다.
- 우선순위 수수료는 **(컴퓨팅 유닛 한도 × 컴퓨팅 유닛 가격)**과 같습니다.
- **컴퓨팅 유닛 한도**는 트랜잭션이 사용할 수 있는 최대 컴퓨팅 유닛입니다.
- **컴퓨팅 유닛 가격**은 마이크로-lamport 단위로 표시된 컴퓨팅 유닛당
  가격입니다.
...
컴퓨트 유닛 가격은 마이크로 lamport 단위로 지정되는 선택적 금액으로, 요청한 각
컴퓨트 유닛에 대해 지불합니다. 이 가격은 트랜잭션의 우선순위 수수료를 계산하는
데 사용됩니다.
...
필요한 컴퓨트 유닛을 추정하는 것입니다. 그런 다음 이 추정치에 10%의 안전 마진을
추가하고 결과 값을 `Compute Unit Limit`로 사용합니다.
...
다른 트랜잭션과 비교하여 어떻게 우선순위가 매겨지는지 결정하며, 다음 공식을
사용하여 계산됩니다:

**우선순위 = ((컴퓨팅 유닛 한도 × 컴퓨팅 유닛 가격) + 기본 수수료) / (1 + 컴퓨팅
유닛 한도 + 서명 CU + 쓰기 잠금 CU)**

트랜잭션에서 컴퓨트 유닛 한도와 가격을 설정하려면 다음 지침을 사용하세요:
...
이러한 지침을 제공하지 않으면 트랜잭션은 컴퓨트 유닛 가격이 0(우선순위 수수료
없음)인 기본 컴퓨트 유닛 한도를 사용합니다.

우선순위 수수료는 실제 사용된 컴퓨트 유닛이 아니라 트랜잭션에 요청한 컴퓨트 유닛
한도에 따라 달라집니다. 너무 높은 컴퓨트 유닛 한도를 설정하거나 기본 양을
