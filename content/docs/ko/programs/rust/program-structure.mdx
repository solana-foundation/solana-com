...
간단하게 하기 위해 프로그램은 단일 `lib.rs` 파일에 구현되지만, 실제로는 더 큰
...
- `instruction_data`: 실행할 명령어와 필요한 인자를 지정하는 프로그램에 전달되는
  추가 데이터

이 세 가지 매개변수는 클라이언트가 프로그램을 호출하는 명령어를 만들 때 제공해야
하는 데이터와 직접적으로 대응됩니다.
...
- 직렬화(Serialization): 데이터 타입을 바이트로 변환하여 계정의 데이터 필드에
  저장
...
"Counter" 구조체를 "lib.rs"에 추가하여 프로그램 상태를 정의하세요. 이 구조체는
초기화 및 증가 명령 모두에서 사용됩니다.
...
1. 생성되고 초기화될 카운터 계정
2. 새 계정 생성 비용을 지불할 페이어 계정
3. 새 계정을 생성하기 위해 호출하는 시스템 프로그램
...
계정을 생성하기 전에 다음을 수행해야 합니다:

1. 계정의 데이터 필드에 할당할 공간(바이트 단위)을 지정합니다. u64 값(`count`)을
   저장하므로 8바이트가 필요합니다.

2. 필요한 최소 "rent" 잔액을 계산합니다. Solana에서 계정은 계정에 저장된 데이터
   양에 따라 최소한의 lamport(rent) 잔액을 유지해야 합니다.
...
Solana에서 새 계정은 System Program에 의해서만 생성될 수 있습니다. 계정을 생성할
때, 할당할 바이트 수와 새 계정의 프로그램 소유자를 지정합니다. System Program은:

1. 새 계정을 생성합니다
2. 계정의 데이터 필드에 지정된 공간을 할당합니다
3. 지정된 프로그램에 소유권을 이전합니다

이 소유권 이전은 중요합니다. 계정의 프로그램 소유자만이 계정의 데이터를 수정할
수 있기 때문입니다. 이 경우, 우리 프로그램을 소유자로 설정하여 카운터 값을
저장하기 위해 계정의 데이터를 수정할 수 있게 합니다.

우리 프로그램의 명령에서 시스템 프로그램을 호출하기 위해, `invoke` 함수를 통해
Cross Program Invocation(CPI)을 수행합니다. CPI는 한 프로그램이 다른 프로그램의
명령을 호출할 수 있게 해줍니다 - 이 경우에는 시스템 프로그램의 `create_account`
명령입니다.
...
3. `CounterAccount` 구조체를 계정의 데이터 필드에 직렬화하여 계정에
