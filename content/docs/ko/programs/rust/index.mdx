...
<Callout type="warn">
  `Cargo.toml`에서 `edition` 필드를 `2021`로 업데이트하세요. 그렇지 않으면
...
`cargo test` 명령어를 사용하여 테스트를 실행합니다. 프로그램 로그에 "Hello,
world!"가 표시됩니다.
...
<Callout type="info">
  Solana Explorer의 클러스터도 localhost로 설정되어 있어야 합니다. Solana
  Explorer의 "Custom RPC URL" 옵션은 기본적으로 `http://localhost:8899`로
  설정됩니다.
</Callout>
...
다음 코드를 `examples/client.rs`에 추가합니다. 이 Rust 클라이언트 스크립트는
새로운 keypair에 트랜잭션 수수료를 지불할 자금을 제공한 다음 hello world
프로그램을 호출합니다.
...
(로컬 클러스터)에서 트랜잭션 서명을 확인하여 프로그램 로그에서 "Hello, world!"를
볼 수 있습니다.
...
프로그램 로그에서 "Hello, Solana!"를 볼 수 있을 것입니다.
...
클라이언트 코드를 다시 실행하고 Solana Explorer에서 트랜잭션 서명을 확인하여
프로그램 로그에서 "Hello, Solana!"를 볼 수 있습니다.
