...
description:
  ...
  블록체인에서 스마트 컨트랙트를 생성, 빌드, 테스트 및 배포하는 단계별 지침이
  포함되어 있습니다.
h1: Rust로 프로그램 개발하기
---
...
프로젝트 디렉토리로 이동합니다. 기본 `src/lib.rs`와 `Cargo.toml` 파일이 보일
것입니다.
...
Solana 프로그램을 닫아 계정에 할당된 SOL을 회수할 수 있습니다. 프로그램 닫기는
되돌릴 수 없으므로 주의해서 진행해야 합니다.

프로그램을 닫으려면 `solana program close <PROGRAM_ID>` 명령어를 사용하세요.
예시:
...
프로그램이 닫히면 해당 프로그램 ID는 재사용할 수 없습니다. 이전에 닫힌 프로그램
ID로 프로그램을 배포하려고 하면 오류가 발생합니다.
...
프로그램을 닫은 후 동일한 소스 코드로 프로그램을 다시 배포해야 하는 경우, 새로운
