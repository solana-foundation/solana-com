...
다음으로, `src/lib.rs` 파일의 내용을 아래 코드로 교체하세요. 이것은 프로그램이
호출될 때 "Hello, world!"를 프로그램 로그에 출력하는 최소한의 Solana
프로그램입니다.
...
프로그램 ID를 확인하려면 터미널에서 다음 명령어를 실행하세요. 이 명령어는 지정된
...
프로그램 코드 아래에 `src/lib.rs`에 다음 테스트를 추가하세요. 이것은 hello world
...
`cargo test-sbf` 명령어를 사용하여 테스트를 실행하세요. 프로그램 로그에 "Hello,
world!"가 표시됩니다.
...
명령을 실행하여 로컬 validator에 프로그램을 배포하세요.
...
먼저 `examples` 디렉토리와 `client.rs` 파일을 생성하세요.
...
다음 명령을 실행하여 프로그램 ID를 확인할 수 있습니다.
...
(로컬 클러스터)에서 트랜잭션 서명을 검사하여 프로그램 로그에 "Hello, world!"가
표시되는 것을 확인할 수 있습니다.

## !!steps 프로그램 업데이트하기

Solana 프로그램은 동일한 프로그램 ID로 재배포하여 업데이트할 수 있습니다.
`src/lib.rs` 파일의 프로그램을 "Hello, world!" 대신 "Hello, Solana!"를
출력하도록 업데이트하세요.
...
프로그램 로그에 "Hello, Solana!"가 표시되어야 합니다.
...
프로그램 로그에 "Hello, Solana!"가 표시되는 것을 확인하세요.
...
프로그램 ID를 생성해야 합니다. 프로그램을 위한 새로운 keypair를 생성하려면 다음
명령을 실행하세요:
...
삭제하고 `cargo build-sbf` 명령을 다시 실행하면 새로운 keypair 파일이
생성됩니다.
