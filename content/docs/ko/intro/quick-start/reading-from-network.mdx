...
Solana의 계정은 "상태" 또는 "실행 가능한" 프로그램을 저장할 수 있습니다. 각
계정은 온체인 데이터를 찾는 데 사용되는 고유 ID 역할을 하는 "주소"(공개 키)를
가지고 있습니다.

Solana 계정에는 다음이 포함됩니다:

- **상태**: 읽고 유지하기 위한 데이터. 예를 들어, 토큰 정보, 사용자 데이터 또는
  프로그램 내에 정의된 기타 데이터가 있습니다.
...
지갑 계정을 가져오면 응답에는 예제 출력에 표시된 필드가 포함됩니다.
...
`data` 필드에는 바이트로 저장된 계정의 데이터가 포함됩니다.

지갑 계정의 경우 이 필드는 비어 있습니다(0바이트). 다른 계정은 이 필드를
사용하여 프로그램 상태 또는 실행 가능한 프로그램 코드를 저장합니다.
...
`lamports` 필드에는 lamport 단위로 표시된 계정의 SOL 잔액이 포함됩니다.

Lamport는 SOL의 가장 작은 단위입니다. 1 SOL = 1,000,000,000 lamport입니다.
...
이 예제는 지갑 계정과 program account의 차이점을 보여주기 위해 Token Program을
가져옵니다.

program account는 Token Program의
...
Token Program은 솔라나에서 실행 가능한 프로그램 계정입니다. 지갑 계정과
마찬가지로, 프로그램도 동일한 기본
...
`executable` 필드가 `true`로 설정되어 있어, 이 계정의 `data` 필드에 실행 가능한
프로그램 코드가 포함되어 있음을 나타냅니다.
...
프로그램 계정의 경우, `data` 필드에 프로그램의 실행 코드가 저장됩니다. 반면에
지갑 계정은 빈 데이터 필드를 가지고 있습니다.

솔라나 프로그램을 배포할 때, 프로그램의 실행 코드는 계정의 `data` 필드에
저장됩니다.
...
실행 가능한 프로그램 계정에는 또한 계정의 `owner`로 지정된 프로그램이 있습니다.

모든 프로그램 계정은 Loader 프로그램이 소유하고 있으며, 이는 솔라나에서 실행
가능한 프로그램 계정을 소유하는 내장 프로그램의 카테고리입니다.
...
이 예제는 USD 코인(USDC) mint account를 가져와 솔라나의 프로그램이 별도의 계정에
상태를 저장하는 방법을 보여줍니다.
...
저장합니다. Mint 계정의 주소는 솔라나 네트워크에서 토큰을 고유하게 식별합니다.
...
이 예제에서 주목해야 할 핵심 포인트는 mint account가 실행 가능한 코드가 아닌
상태를 저장한다는 것입니다.

Mint account는 Token Program이 소유하며, Token Program은 mint account를 생성하고
...
`data` 필드는 mint 권한, 총 공급량, 소수점 자릿수와 같은 직렬화된 `Mint` 계정
상태를 포함합니다.
...
예를 들어, *shell`@solana/spl-token`*에는 mint account의
[_ts`getMint()`_](mention:one) 함수가 포함되어 있어 `data` 필드를
[Mint](https://github.com/solana-program/token/blob/program%40v8.0.0/program/src/state.rs#L16-L30)
데이터 타입으로 역직렬화하는 데 도움을 줍니다.
...
`mintAuthority` 필드는 새 토큰을 발행할 수 있는 권한이 있는 계정을 보여줍니다.

이는 토큰의 새 단위를 생성할 수 있는 유일한 계정입니다.
...
이 값은 토큰의 최소 단위로 표시됩니다. 표준 단위로 총 공급량을 얻으려면,
`supply` 필드 값을 `decimals`에 맞게 조정하세요.
...
필드는 Token Program에서 사용되는 보안 확인 장치입니다.
...
동결된 token account는 해당 계정의 토큰을 전송하거나 소각할 수 없습니다.
