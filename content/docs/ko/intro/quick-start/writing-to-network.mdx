...
데이터를 네트워크에 쓰는 방법을 배우게 됩니다. Solana 네트워크에 쓰기 위해서는
하나 이상의 명령어를 포함하는 트랜잭션을 보내야 합니다.

프로그램(스마트 컨트랙트)은 각 명령어가 수행하는 비즈니스 로직을 정의합니다.
트랜잭션을 제출하면 Solana 런타임은 각 명령어를 순차적으로 그리고 원자적으로
실행합니다(모든 명령어가 성공하거나 전체 트랜잭션이 실패함을 의미).

이 섹션에서는 다음 예제들을 다룹니다:
...
[`toPubkey`](mention:to) 계정으로 지정된 [`lamports`](mention:lamports) 양만큼
...
트랜잭션을 생성하고 트랜잭션에 명령어를 추가합니다.

이 예제에서는 단일 명령어가 있는 트랜잭션을 생성하고 있습니다. 그러나 트랜잭션에
여러 명령어를 추가할 수 있습니다.
...
방법을 배우게 됩니다. 이를 위해서는 두 가지 명령어가 필요합니다:
...
Mint account 유형을 위한 공간을 가지고 Token Extensions Program이 소유한 새
계정을 생성하는 명령을 구성합니다.
...
- [keypair에 자금을 제공하기 위한 에어드롭 요청하기](mention:airdrop)
...
할당해야 하는 정확한 SOL(lamport 단위) 양을 계산합니다.
...
2. 새 계정에 자금을 제공하기 위해 지갑에서 [lamport 전송](mention:lamports)
...
- mint 권한과 동결 권한 모두를 가진 [지갑](mention:authority)
...
두 명령어를 하나의 트랜잭션에 추가합니다.

두 명령어를 하나의 트랜잭션으로 결합함으로써 계정 생성과 초기화가 원자적으로
발생하도록 보장합니다. 두 명령어 모두 성공하거나 둘 다 실패합니다.
...
- [Mint](mention:mint)는 새 계정에 대한 주소 사용을 승인하기 위해 서명합니다
...
반환된 트랜잭션 서명은 Solana Explorer에서 트랜잭션을 검사하는 데 사용할 수
있습니다.
