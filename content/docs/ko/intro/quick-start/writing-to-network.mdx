...
`sender`에 자금을 지원하기 위해 SOL 에어드롭을 요청하세요.

### !instruction

System Program의 전송 명령을 호출하기 위한 명령을 구축하세요.

### !transaction

새 트랜잭션을 생성하고 전송 명령을 추가하세요.
...
이것은 Solana의 모든 프로그램과 상호작용하기 위한 트랜잭션을 구축하는
단계입니다.
...
방법을 배우게 됩니다. 이를 위해서는 두 가지 명령이 필요합니다:
...
새 계정의 데이터를 Mint account 유형으로 초기화하는 명령어를 구성합니다.

### !transaction

새 트랜잭션을 생성하고 두 명령어를 추가합니다.

여기서 명령어의 순서가 중요합니다. `createAccountInstruction`가
...
3. 계정의 [소유권 할당](mention:three)을 Token Extensions
...
이루어지도록 보장합니다. 두 명령어가 모두 성공하거나 둘 다 실패합니다. 이 접근
방식은 더 복잡한 Solana 트랜잭션을 구축할 때 일반적으로 사용되며, 모든 명령어가
함께 실행되도록 보장합니다.
