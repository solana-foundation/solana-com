...
1. _rs`user: Signer<'info>`_
   ...
   - 필요할 때 `message_account`의 추가 공간 비용을 지불할 수 있으므로 변경
...
_rs`#[instruction(message: String)]`_ 속성을 통해 _rs`Update`_ 구조체가
_rs`update`_ 명령어의 _rs`message`_ 매개변수에 접근할 수 있습니다.
...
다음으로, *rs`MessageAccount`*를 닫기 위한 _rs`delete`_ 명령어를 추가하세요.
...
<Callout type="info">
이 예제에서는 개발 테스트용 Solana 클러스터인 devnet에 프로그램을 배포합니다.
...
먼저, 테스트 파일은
---INLINE-CODE-PLACEHOLDER-16a13b017dc6bcf5d50e667e5fa84c2b--- 명령을 호출하는
...
마지막으로, 테스트 파일은 계정 데이터와 트랜잭션 세부 정보 링크를 로그에
기록합니다. 계정이 더 이상 존재하지 않기 때문에 계정 데이터는 null로 표시됩니다.
...
트랜잭션 세부 정보를 볼 수 있는 SolanaFM 링크를 로그에 기록합니다.
