...
Anchor 프로그램에서 _rs`#[account]`_ 속성은 계정 데이터(Account의 데이터 필드에
...
_rs`message`_ 필드는 사용자의 메시지를 포함하는 *rs`String`*를 보유합니다.

### !bump

_rs`bump`_ 필드는 프로그램 파생 주소(Program Derived Address)를 도출하는 데
사용되는 _rs`u8`_ ["범프" 시드](/docs/core/pda#canonical-bump)를 저장합니다. 이
...
이 예시에서 _rs`MessageAccount`_ 구조체는 사용자가 생성한 메시지를 저장하며 세
개의 필드를 포함합니다:

- `user` - 메시지 계정을 생성한 사용자를 식별하는 *rs`Pubkey`*입니다.
- `message` - 사용자의 메시지를 포함하는 *rs`String`*입니다.
...
Anchor 프로그램을 통해 생성된 모든 계정은 계정 식별자(discriminator)를 위한
...
2. 계정 데이터 초기화:
   ...
   - PDA를 도출하는 데 사용되는 `bump` 값을 `ctx.bumps.message_account`에서
     가져와 설정합니다.
...
_rs`Update`_ 구조체는 _rs`update`_ 명령에 필요한 계정을 정의합니다.
...
명령에서 _rs`message`_ 매개변수에 접근할 수 있게 합니다.

</Accordion>
</Accordions>

다음으로, `update` 명령에 대한 로직을 추가합니다.
...
`update` 함수는 기존 메시지 계정을 수정하는 로직을 구현합니다. 두 가지
매개변수를 받습니다:
...
2. 계정 데이터 업데이트:
   - 컨텍스트에서 `message_account`에 접근합니다.
   - `message` 필드를 함수 인자의 새로운 `message`로 설정합니다.
...
다음과 같이 _rs`Delete`_ 구조체를 업데이트합니다:
...
_rs`seeds`_ 제약 조건은 PDA를 파생시키는 데 사용되는 seed를 지정합니다.
...
이 경우, _rs`user`_ 계정이 닫힌 *rs`message_account`*의 lamport를 받습니다.
...
1. _rs`user: Signer<'info>`_
   ...
   - 닫힌 계정에서 lamport를 받기 위해 가변(mutable)(_rs`#[account(mut)]`_)로
     표시됩니다
...
스타터 코드에는 `anchor.test.ts` 내의 테스트 파일도 포함되어 있습니다.
...
이 섹션에서는 이 코드가 단순히 테스트 파일을 설정합니다.
...
지정된 것과 동일한 시드를 사용하여 Javascript에서 PDA를 도출하는 방법을
보여줍니다.
...
먼저, 테스트 파일은 `create` 명령어를 호출하는 트랜잭션을 전송하며, "Hello,
World!"를 메시지로 전달합니다.
...
_ts`update()`_ 메서드는 `update` 명령어를 호출합니다.
...
먼저, 이 코드는 `delete` 명령을 호출하는 트랜잭션을 보내 메시지 계정을 닫습니다.
