...
컨트랙트)을 빌드, 배포 및 테스트합니다. 마지막에는 Solana 블록체인에 첫 번째
프로그램을 배포하게 될 것입니다.

이 섹션의 목적은 Solana Playground에 익숙해지는 데 중점을 둡니다. 이 가이드는
Program Derived Address (PDA) 및 Cross-Program Invocation (CPI) 섹션에서 더
자세한 예제를 다룹니다. 자세한 내용은 [Solana의 프로그램](/docs/core/programs)
페이지를 참조하세요.
...
프로그램에는 하나의 명령어(`initialize`)가 포함되어 있으며:
...
_rs`#[derive(Accounts)]`_ 속성은 프로그램의 계정이 포함된 구조체에 주석을
답니다.
...
이러한 속성은 프로그램 보안을 강화하기 위해 Anchor가 제공하는 일반적인 검사 또는
기능을 추가합니다.
...
_rs`ctx.accounts.new_account.data`_ 는 `new_account` 계정의 `data` 필드에
...
제약 조건은 시스템 프로그램을 호출하여 새 계정을 생성하고 이 프로그램을 소유자로
...
2. `data: u64` - 명령어를 호출할 때 제공해야 하는 사용자 지정 매개변수입니다.
...
다음 명령을 실행하고 *rs`declare_id!()`*에서 찾은 프로그램 주소를 지정하여
프로그램을 종료할 수 있습니다:
...
프로그램의 업그레이드 권한을 가진 사람만이 프로그램을 종료할 수 있습니다. 배포
과정에서 프로그램을 배포할 때 업그레이드 권한이 설정됩니다. 이 계정은 프로그램을
업데이트하거나 종료할 수 있는 독점적인 권한을 가집니다. 누군가가 업그레이드
권한을 취소하면, 프로그램은 종료나 업데이트가 불가능한 불변 상태가 됩니다.
