...
토큰 전송은 동일한 민트를 공유하는 하나의 token account에서 다른 token account로
토큰을 이동하는 것을 의미합니다. 토큰 전송은 토큰 프로그램에서
...
명령을 호출할 때 발생합니다. 소스 token account의 소유자(권한)로 지정된 주소만
해당 계정에서 토큰을 전송할 수 있습니다.

<Callout type="info">
  [Token
  Program](https://github.com/solana-program/token/blob/3daf44899f0bd71c879d28dffdfb788dd944f3c5/program/src/processor.rs#L227)과
  [Token Extension
  Program](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/processor.rs#L300)은
