...
token account는 솔라나의 Token Programs에서 특정 토큰(민트)에 대한 개인의 소유권
정보를 저장하는 계정 유형입니다. 각 token account는 하나의 민트와 연결되어 토큰
잔액 및 소유자와 같은 세부 정보를 추적합니다.
...
<Callout type="info">
  소스 코드에서 개발자들은 Token account를 `Account` 유형이라고 부릅니다. [Token
...
특정 민트의 토큰을 보유하려면 사용자는 먼저 token account를 생성해야 합니다. 각
token account는 다음 정보를 추적합니다:

1. 특정 민트(token account가 보유하는 토큰 유형)
2. 소유자(계정에서 토큰을 전송할 수 있는 권한을 가진 주체)

솔라나에서 USD 코인(USDC)을 사용한 예시입니다:

- USD 코인 민트 주소: `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Circle(USD 코인 발행자)은 `3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
  주소에 token account를 가지고 있습니다
- 이 token account는 USD 코인 토큰(민트) 단위만 보유합니다
...
이 token account의 세부 정보를 볼 수 있습니다.

<Callout type="info">
  "소유자"라는 용어는 두 가지 다른 맥락에서 사용됩니다:

1. token account "소유자" - 이는 Token Program에서 정의한
   ...
   유형의 "소유자" 필드로 token account의 데이터에 저장된 주소입니다. 소유자는
   계정에서 토큰을 전송, 소각 또는 위임할 수 있습니다. 이 주소는 프로그램
   소유자와 구별하기 위해 때때로 token account의 "권한자"라고도 불립니다.
...
associated token account는 사용자가 특정 토큰(mint)의 단위를 보유하기 위한 기본
토큰 계정이라고 생각할 수 있습니다.
...
associated token account는 주어진 mint에 대한 사용자의 토큰 계정을 결정적으로
...
이러한 결정적 파생은 지갑 주소와 토큰 mint의 모든 조합에 대해 정확히 하나의
associated token account 주소가 존재함을 보장합니다. 이 접근 방식은 주어진 토큰
mint에 대한 사용자의 토큰 계정을 쉽게 찾을 수 있게 해주어, 토큰 계정 주소를
별도로 추적할 필요가 없습니다.
...
1. 시스템 프로그램을 호출하여 토큰 계정을 생성하고 공간을 할당한 다음 소유권을
...
associated token account를 생성하는 명령어는 자동으로 시스템 프로그램을 호출하여
