...
스케일드 UI 금액 확장 기능은 토큰 발행자가 사용자의 토큰 잔액의 UI 금액을 계산할
때 사용할 승수를 지정할 수 있게 해줍니다. 이를 통해 발행자는 리베이싱 토큰을
...
- 최종 사용자는 가능한 한 토큰 가격, 토큰 잔액 및 토큰 금액에 대해 UI 금액(원시
  금액 × 승수)과 상호 작용해야 합니다
- dApp 및 서비스 제공업체는 모든 계산에 원시 금액과 비스케일링된 가격을 사용하고
  최종 단계에서 사용자를 위해 변환해야 합니다
...
- 승수: UI 금액 계산에 사용되는 정적으로 업데이트 가능한 승수
- UI 금액: 승수 × 원시 금액(일명: 스케일링된 금액)
...
- 스케일드 UI 금액 확장 기능을 사용하는 토큰의 금액을 최종 사용자에게 표시할
  ...
  - 토큰이 가진 소수점 자릿수에 따라 이 값을 잘라내는 것을 권장합니다.
    - 예: yUSD가 6자리 소수점을 가지고 있고 사용자의 UI 금액이 1.123456789인
...
- 임의 금액에 대한 UI 금액을 알아야 하는 경우,
  ...
  시뮬레이션하여 이 계산을 얻을 수 있습니다.
  - 참고: amountToUiAmount는 트랜잭션 시뮬레이션이 필요하며, 이는 잔액이 있는
    유효한 수수료 지불자도 필요함을 의미합니다. 이러한 이유로, 이것은 잔액을
...
발행자는 언제든지 승수를 업데이트할 수 있기 때문에 계정 잔액을 최신 상태로
유지하기 위해 주기적으로 폴링하는 것을 고려할 수 있습니다. 발행자가 이 승수를
하루에 한 번 이상 업데이트할 가능성은 낮습니다. 승수가 미래 날짜로 설정된 경우,
이 업데이트 시간에 자동으로 폴링하도록 설정할 수 있습니다

## 트랜잭션에서의 토큰 금액 (전송 / 스왑 등)

- 사용자는 스케일링된 "UIAmount"로 해석될 금액을 입력해야 합니다. 이를 처리하는
  앱은 트랜잭션을 위해 원시 토큰 금액으로 변환해야 합니다.
  - 반올림 문제가 있는 경우, 내림하여 트랜잭션 실패 위험보다는 약간의 먼지(dust)
    금액이 남는 것을 선호합니다
  - 이 변환을 위해
...
- 사용자가 "최대" 또는 "전체" 잔액으로 작업을 요청할 때 앱은 총 원시 금액을
  사용해야 합니다. 이렇게 하면 먼지(dust)가 남지 않습니다.
  - 선택 사항: "최대"가 사용될 때 계정을 자동으로 닫아 사용자의 스토리지
...
- 현재 승수는 언제든지 토큰 민트에서 `getAccountInfo`를 호출하여 읽을 수
  있습니다. 또한, 미래 승수가 설정된 경우 이 정보도 토큰 민트에서 확인할 수
  있습니다. UX 혼란을 방지하기 위해 이 승수를 표시하지 않는 것을 권장합니다.
...
- 과거 데이터를 제공하는 서비스는 **스케일된 UI 금액 확장에 대해 스케일된 가격과
  스케일되지 않은 가격을 모두 저장하고 표시**해야 합니다.
- 스케일된 금액이 가장 자주 사용될 것으로 예상됩니다. 이는 전통적인 금융
...
- 기본적으로 스케일된 UI 금액 확장을 이해하지 못하는 지갑과 애플리케이션은
  스케일되지 않은 가격 \* 원시 금액을 곱하여 활동의 총 가격을 올바르게
  표시합니다.
- 그러나 스케일되지 않은 가격을 표시하여 사용자에게 혼란을 줄 수 있습니다.
...
| 요구사항                           | 설명                                                                                                                                                                    | 우선순위 |
| :--------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------- |
| UiAmount를 사용한 사용자 작업 지원 | UiAmount가 활성화된 앱에서는 모든 사용자 작업이 UiAmount로 입력되어야 합니다. 앱에서 UiAmount가 표시되지 않는 경우, 앱이 업데이트될 때까지 원시 금액을 사용해야 합니다. | **P0**   |
...
| 요구사항                       | 설명                                                                                                | 우선순위 |
| :----------------------------- | :-------------------------------------------------------------------------------------------------- | :------- |
| 스케일링된 잔액 표시           | 스케일링된 금액(uiAmount)을 주요 잔액으로 표시합니다.                                               | **P0**   |
| 토큰 전송 지원                 | 최종 사용자는 스케일링된 잔액(원시 금액 \* 잔액)으로 전송 금액을 입력해야 합니다.                   | **P0**   |
| 현재 시세 표시                 | 사용자에게 스케일링된 현재 시세를 표시합니다                                                        | **P0**   |
...
| 잔액에 대한 스케일링된 잔액 표시            | 현재 잔액에 대해 스케일링된 잔액(UiAmount)을 표시합니다.             | **P0**   |
| 거래에 대한 스케일링된 잔액 표시            | 과거 거래의 전송 금액에 대해 스케일링된 잔액(UiAmount)을 표시합니다. | **P0**   |
| 거래에 대한 스케일링된 가격 표시            | 이전 거래에 대한 스케일링된 가격을 표시합니다                        | **P1**   |
| 승수 업데이트 거래를 올바르게 파싱하고 표시 | 승수 업데이트에 대한 세부 정보를 올바르게 표시합니다                 | **P2**   |

---

### **시장 데이터 애그리게이터 (예: CoinGecko)**

| 요구사항                              | 설명                                                                          | 우선순위 |
| :------------------------------------ | :---------------------------------------------------------------------------- | :------- |
| 스케일링된 데이터를 위한 API 업데이트 | 시간 경과에 따른 승수 변화와 스케일링된 가격 피드를 포함하도록 API 기능 확장. | **P0**   |
| 스케일링 조정이 포함된 총 공급량      | 총 공급량과 총 시가총액을 표시할 때 스케일링된 잔액을 고려                    | **P0**   |
| 역사적 가격 추적                      | 시간 경과에 따른 스케일링된 가격을 사용하여 역사적 가격 차트 제공.            | **P1**   |
| 역사적 승수 추적                      | 이자 발생 토큰에 대한 승수 업데이트의 역사적 마커 제공.                       | **P2**   |
...
| 역사적 승수 데이터               | 역사적 승수 변화가 포함된 API 제공.                                      | **P0**   |
| 역사적 가격 데이터               | 스케일링 및 비스케일링 금액 모두에 기반한 역사적 가격이 포함된 API 제공. | **P0**   |
...
| 요구사항                  | 설명                                                                                             | 우선순위 |
| :------------------------ | :----------------------------------------------------------------------------------------------- | :------- |
| 리베이스된 토큰 잔액 표시 | UI에서 거래 또는 유동성 제공을 위한 스케일링된 잔액 표시. (백엔드는 여전히 원시 금액 사용 가능)  | **P0**   |
| 토큰 액션 지원            | 최종 사용자는 UiAmount 잔액(승수 \* 원시 금액)으로 액션 금액을 입력해야 함.                      | **P0**   |
| 가격 피드 적응            | 현재 가격을 표시하기 위해 가격 피드가 사용되는 모든 곳에서 최종 사용자에게 스케일링된 가격 제공. | **P1**   |
| 가격 이력 그래프 표시     | 가격 그래프에 스케일링된 가격 반영                                                               | **P1**   |
