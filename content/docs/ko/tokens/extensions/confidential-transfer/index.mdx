...
title: 기밀 전송
description:
  기밀 전송 확장 기능에 대해 알아보고 토큰 민트 및 계정에 선택적 기능을 추가하는
  방법을 배워보세요.
---

## 기밀 전송이란 무엇인가요?

기밀 전송을 통해 전송 금액을 공개하지 않고도 토큰 계정 간에 토큰을 전송할 수
있습니다. 이는 프라이버시를 보호하는 거래에 유용합니다. 전송 금액과 토큰 잔액만
비공개로 유지되며, 토큰 계정 주소는 공개 상태로 유지됩니다.
...
추가하여 전송 금액을 공개하지 않고도 계정 간에 토큰을 전송할 수 있게 합니다.
...
1. 기밀 전송 확장 기능이 있는 mint account를 생성합니다.
2. 발신자와 수신자를 위한 기밀 전송 확장 기능이 있는 token account를 생성합니다.
3. 발신자 계정에 토큰을 발행합니다.
4. 발신자의 공개 잔액을 **기밀 대기 잔액**으로 **입금**합니다.
5. 발신자의 대기 잔액을 **기밀 가용 잔액**에 **적용**합니다.
6. 발신자 토큰 계정에서 수신자 토큰 계정으로 토큰을 기밀하게 **전송**합니다.
7. 수신자의 대기 잔액을 **기밀 가용 잔액**에 **적용**합니다.
8. 수신자의 기밀 가용 잔액을 **공개 잔액**으로 **출금**합니다.

기밀 전송 흐름의 단계에 대한 자세한 내용은 다음 페이지를 참조하세요:
...
<Cards>
  ...
  >
    기밀 전송 확장 기능이 있는 mint account를 생성하는 방법
  </Card>
  <Card
  ...
  >
    토큰 계정 간에 기밀하게 토큰을 전송하는 방법
  </Card>
</Cards>

아래 다이어그램은 기밀 토큰 전송의 기본 흐름에 대한 상세 시퀀스를 보여줍니다:
...
<Callout type="info">
  이 글을 작성하는 시점에서는 기본 로컬 validator에서 Confidential Transfers가
...
| 명령어                              | 설명                                                                                                                                      |
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | 기밀 전송을 위한 mint 계정을 설정합니다. 이 명령어는 _rs`TokenInstruction::InitializeMint`_ 명령어와 동일한 트랜잭션에 포함되어야 합니다. |
...
| _rs`Transfer`_                      | token account 간에 토큰을 기밀하게 전송합니다.                                                                                            |
| _rs`ApplyPendingBalance`_           | 입금이나 전송 후 대기 중인 잔액을 사용 가능한 잔액으로 변환합니다.                                                                        |
...
| _rs`TransferWithFee`_               | 수수료를 포함하여 token account 간에 토큰을 기밀하게 전송합니다.                                                                          |
