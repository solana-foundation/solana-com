...
1. 기밀 전송 확장이 포함된 mint account를 생성합니다.
2. 발신자와 수신자를 위한 기밀 전송 확장이 포함된 token account를 생성합니다.
3. 발신자 계정에 토큰을 발행합니다.
...
<Cards>
  ...
  >
    token account 간에 기밀하게 토큰을 전송하는 방법
  </Card>
</Cards>
...
<Callout type="info">
  ...
  예제 코드를 실행하려면 메인넷 Token Extension Program을 복제해야 합니다.
</Callout>
...
| _rs`EmptyAccount`_                  | token account를 닫을 수 있도록 대기 중인 잔액과 사용 가능한 기밀 잔액을 비웁니다.                                                         |
| _rs`Deposit`_                       | 공개 토큰 잔액을 대기 중인 기밀 잔액으로 변환합니다.                                                                                      |
| _rs`Withdraw`_                      | 사용 가능한 기밀 잔액을 다시 공개 잔액으로 변환합니다.                                                                                    |
| _rs`Transfer`_                      | token account 간에 기밀하게 토큰을 전송합니다.                                                                                            |
...
| _rs`TransferWithFee`_               | 수수료를 포함하여 token account 간에 기밀하게 토큰을 전송합니다.                                                                          |
