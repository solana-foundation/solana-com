...
- **approved**: 기밀 전송에 대한 계정의 승인 상태입니다. 만약 mint account의
  `auto_approve_new_accounts` 구성이 `true`로 설정되어 있다면, 모든 토큰 계정은
...
- **maximum_pending_balance_credit_counter**: 대기 중인 잔액을 사용 가능한
  잔액으로 변환하기 위한 _rs`ApplyPendingBalance`_ 명령이 필요하기 전에 대기
  중인 크레딧의 수 제한입니다.
...
- **actual_pending_balance_credit_counter**: 마지막으로
  _rs`ApplyPendingBalance`_ 명령어가 처리될 때 token account에 있는
  `pending_balance_credit_counter` 값입니다.
...
트랜잭션이 제출될 때와 처리될 때 사이에 암호화된 잔액이 변경되어 트랜잭션이
실패하게 되므로, token account 소유자는 토큰을 전송할 수 없게 됩니다.
...
ElGamal 복호화가 더 큰 숫자에 대해 더 많은 계산을 필요로 하기 때문입니다. 암호문
산술 구현은
...
2. 예상 및 실제 대기 중인 크레딧 카운터는 _rs`ApplyPendingBalance`_ 명령어가
   ...
   - `actual_pending_balance_credit_counter`: _rs`ApplyPendingBalance`_ 명령어가
     처리될 때 token account에 있는 `pending_balance_credit_counter` 값
...
카운터 값이 다른 경우 클라이언트는 카운터 차이와 일치하는 최근 입금/전송
명령어를 조회하여 정확한 잔액을 계산해야 합니다.
...
2. 카운터 차이(실제 - 예상)까지 입금 및 전송 명령어를 포함한 가장 최근
   ...
   - 전송 명령어에서 대상 암호화 금액을 복호화하고 추가합니다
...
*rs`PubkeyValidityProofData`*는 ElGamal 키가 유효함을 검증하는 증명을
...
<Callout type="info">
  작성 시점에서는 기밀 전송 기능이 기본 로컬 검증자에서 활성화되어 있지
