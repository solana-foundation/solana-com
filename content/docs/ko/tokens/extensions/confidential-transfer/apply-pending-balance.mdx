...
1. **기밀 대기 잔액**: 처음에는 토큰이 공개 잔액에서 "대기 중인" 기밀 잔액으로
   "입금"됩니다.
2. **기밀 사용 가능 잔액**: 그런 다음 대기 잔액이 사용 가능한 잔액으로
   "적용"되어 토큰을 기밀 전송에 사용할 수 있게 됩니다.

이 섹션에서는 두 번째 단계인 대기 잔액을 사용 가능한 잔액으로 적용하는 방법을
설명합니다.

토큰이 공개 잔액에서 "입금"되거나 토큰이 한 token account에서 다른 token
account로 기밀적으로 전송될 때, 토큰은 처음에 기밀 대기 잔액에 추가됩니다.
토큰을 기밀 전송에 사용하기 전에 대기 잔액을 사용 가능한 잔액으로 "적용"해야
합니다.
...
`ApplyPendingBalance` 명령어가 포함된 트랜잭션을 구축하고 전송하는
