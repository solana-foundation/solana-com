...
- [Tạo keypair](mention:wallet) để thanh toán cho giao dịch
...
bằng lamport) phải được phân bổ cho dữ liệu trên một mint account.
...
1. [Phân bổ số byte cần thiết](mention:space) để lưu trữ dữ liệu mint
...
Thêm cả hai chỉ thị vào một giao dịch duy nhất.

Bằng cách kết hợp cả hai chỉ thị vào một giao dịch duy nhất, bạn đảm bảo rằng
việc tạo tài khoản và khởi tạo xảy ra một cách nguyên tử. Hoặc là cả hai chỉ thị
thành công, hoặc không có chỉ thị nào thành công.

Cách tiếp cận này phổ biến khi xây dựng các giao dịch Solana phức tạp hơn, vì nó
đảm bảo rằng tất cả các chỉ thị được thực thi cùng nhau.
...
Ký và gửi giao dịch. Yêu cầu hai chữ ký:
