...
Token Program là một tài khoản chương trình thực thi trên Solana. Giống như các
tài khoản ví, các chương trình có cùng cấu trúc dữ liệu
[Account](https://github.com/anza-xyz/agave/blob/v2.1.11/sdk/account/src/lib.rs#L48-L60)
...
Các tài khoản chương trình thực thi cũng có một chương trình được chỉ định là
`owner` của tài khoản.

Tất cả các tài khoản chương trình đều thuộc sở hữu của một chương trình Loader,
đây là một loại chương trình tích hợp sẵn sở hữu các tài khoản chương trình thực
thi trên Solana.
...
Ví dụ này lấy mint account của USD Coin (USDC) để hiển thị cách các chương trình
trên Solana lưu trữ trạng thái trong các tài khoản riêng biệt.
...
Các tài khoản mint thuộc sở hữu của Token Program, bao gồm các chỉ thị xác định
cách tạo và cập nhật tài khoản mint.

<CodePlaceholder title="Tài khoản Mint" />
...
Trường `executable` là `false` vì trường `data` của tài khoản mint lưu trữ trạng
thái, không phải mã thực thi.
...
Trường `data` chứa trạng thái tài khoản `Mint` được tuần tự hóa, như quyền mint,
tổng cung, số thập phân.

Để đọc từ tài khoản Mint, bạn phải giải tuần tự hóa trường `data` thành kiểu dữ
...
Token Program (`TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA`) sở hữu tài khoản
mint.

Điều này có nghĩa là trường `data` của tài khoản mint chỉ có thể được sửa đổi
bởi các chỉ thị được định nghĩa trong Token Program.
...
## Giải tuần tự hóa tài khoản Mint
...
Hầu hết các chương trình Solana đều cung cấp các thư viện client với các hàm trợ
giúp nhằm trừu tượng hóa quá trình giải tuần tự. Các hàm này chuyển đổi các byte
tài khoản thô thành các kiểu dữ liệu có cấu trúc, giúp việc làm việc với dữ liệu
tài khoản dễ dàng hơn.

<WithMentions>

Ví dụ, _shell`@solana/spl-token`_ bao gồm hàm [_ts`getMint()`_](mention:one) để
giúp giải tuần tự trường `data` của tài khoản mint account thành kiểu dữ liệu
[Mint](https://github.com/solana-program/token/blob/program%40v8.0.0/program/src/state.rs#L16-L30).
...
Bạn có thể xem dữ liệu đã được giải tuần tự đầy đủ của
[Mint Account](https://explorer.solana.com/address/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v?cluster=mainnet-beta)
...
Trường `mintAuthority` hiển thị quyền hạn được phép tạo ra token mới.
...
Trường `supply` hiển thị tổng số token đã được phát hành.

Giá trị này được tính bằng đơn vị nhỏ nhất của token. Để có được tổng cung ở đơn
vị tiêu chuẩn, điều chỉnh giá trị của trường `supply` theo `decimals`.
...
Trường `freezeAuthority` hiển thị quyền được phép đóng băng các tài khoản token.

Một tài khoản token bị đóng băng không thể chuyển hoặc đốt token trong tài
khoản.
...
Trường `tlvData` chứa dữ liệu bổ sung cho Token Extensions (cần giải mã thêm).
