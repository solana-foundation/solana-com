---
title: Kết hợp nhiều Program
description:
  Tìm hiểu cách triển khai Cross Program Invocations (CPIs) trong các program
  Solana bằng framework Anchor. Hướng dẫn này minh họa cách chuyển SOL giữa các
  tài khoản, tương tác với System Program và xử lý Program Derived Addresses
  (PDAs) trong CPIs. Hoàn hảo cho các nhà phát triển muốn xây dựng các program
  Solana có khả năng kết hợp.
h1: Cross Program Invocation
---

Trong phần này, chương trình CRUD từ phần PDA trước đó được cập nhật bằng cách
thêm Cross Program Invocations (CPIs), một tính năng cho phép các program Solana
có thể gọi lẫn nhau.

Hướng dẫn này cũng chỉ ra cách các program có thể "ký" cho Program Derived
...
program Solana sử dụng framework Anchor, xây dựng dựa trên các khái niệm PDA đã
khám phá trong phần trước. Để biết thêm chi tiết, hãy tham khảo trang
...
`vault_account`. Tài khoản này, được kiểm soát bởi program, nhận SOL từ người
dùng khi họ cập nhật tài khoản tin nhắn của mình.
...
Hàm _rs`transfer()`_ gọi lệnh chuyển khoản của System Program.
...
Trong lệnh `update`, việc triển khai bao gồm Cross Program Invocation (CPI) để
...
- `to` - Tài khoản vault (đích đến của tiền)
...
Hàm `transfer` sau đó gọi lệnh chuyển tiền trên System Program, truyền vào:
...
Cách thiết lập cho CPI tương tự như cách xây dựng lệnh phía client, nơi bạn chỉ
định chương trình, tài khoản và instruction data cho một lệnh cụ thể để gọi. Khi
lệnh `update` của chương trình nhận được lời gọi, nó gọi nội bộ đến lệnh chuyển
tiền của System Program.
...
cho phép chuyển bất kỳ SOL nào trong vault trở lại cho người dùng khi họ đóng
...
Đồng thời thêm `system_program` vì CPI cho việc chuyển tiền yêu cầu gọi System
Program.
...
`vault_account` sử dụng cùng một cách tạo PDA như trong struct Update.

Việc thêm `vault_account` vào struct Delete cho phép chương trình truy cập vào
tài khoản vault của người dùng trong quá trình thực hiện lệnh xóa để chuyển bất
kỳ SOL tích lũy nào trở lại cho người dùng.
...
_rs`signer_seeds`_ chỉ định các seed và bump seed tùy chọn được sử dụng để tạo
PDA.
...
tạo ra chính xác địa chỉ PDA. Nếu được xác minh, runtime sẽ coi tài khoản PDA đó
là người ký trong suốt thời gian thực hiện CPI.

</WithNotes>

Lưu ý rằng _rs`_ctx: Context<Delete>`_ thay đổi thành _rs`ctx: Context<Delete>`_
để sử dụng context trong phần thân của hàm.
...
của System Program. CPI này minh họa cách thực hiện chuyển tiền yêu cầu chữ ký
của Program Derived Address (PDA).

Đầu tiên, định nghĩa các signer seed cho vault PDA:
...
Struct _rs`Transfer`_ chỉ định các tài khoản cần thiết cho lệnh chuyển của
System Program:
...
- Chương trình để gọi (System Program)
- Các tài khoản liên quan đến việc chuyển tiền (được định nghĩa trong struct
  Transfer)
...
Hàm _rs`transfer()`_ sau đó gọi lệnh chuyển tiền trên System Program, truyền
vào:

- `cpi_context` (chương trình, tài khoản và người ký PDA)
- Số tiền cần chuyển (toàn bộ số dư của tài khoản vault)

  ```rs title="lib.rs"
  transfer(cpi_context, ctx.accounts.vault_account.lamports())?;
  ```

Triển khai CPI này cho thấy cách các chương trình có thể sử dụng PDA để quản lý
tiền. Khi lệnh xóa của chương trình được gọi, nó sẽ gọi nội bộ đến lệnh chuyển
tiền của System Program, ký cho PDA để cho phép chuyển tất cả tiền từ vault trở
lại cho người dùng.
...
các lệnh. Điều này yêu cầu phải tạo vault PDA và bao gồm nó trong các lệnh gọi
cập nhật và xóa.
...
Sau khi thực hiện những thay đổi này, chạy các bài test để đảm bảo mọi thứ hoạt
động như mong đợi:
...
để tìm hiểu nhiều chương trình mẫu khác nhau.
...
trong Solana Playground. Khi một dự án được nhập, tất cả các thay đổi sẽ được tự
động lưu và duy trì.
