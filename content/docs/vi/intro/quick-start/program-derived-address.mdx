...
description:
  ...
  này chỉ rõ cách tạo, cập nhật và xóa tài khoản tin nhắn trên chuỗi bằng PDAs,
  triển khai xác thực tài khoản và viết bài kiểm tra. Hoàn hảo cho các nhà phát
  triển muốn hiểu cách sử dụng PDAs trong các chương trình Solana.
...
[đường dẫn Solana Playground](https://beta.solpg.io/66734b7bcffcf4b13384d1ad)
với mã khởi đầu. Sau đó nhấp vào nút "Import" để thêm chương trình vào các dự án
...
Ràng buộc _rs`seeds`_ xác định các đầu vào tùy chọn được sử dụng để tạo ra PDA.
...
Ràng buộc _rs`space`_ chỉ định số byte cần cấp phát cho trường dữ liệu của tài
khoản mới.
...
2. Khởi tạo dữ liệu tài khoản:
   ...
   - Thiết lập trường `user` thành khóa công khai của tài khoản `user`.

   ```rs
   account_data.user = ctx.accounts.user.key();
   ```

   - Thiết lập trường `message` thành `message` từ đối số của hàm.

   ```rs
   account_data.message = message;
   ```

   - Thiết lập giá trị `bump` được sử dụng để tạo PDA, lấy từ
...
Ràng buộc _rs`realloc::zero`_ đặt lại dữ liệu của tài khoản về không.
...
Ràng buộc _rs`seeds`_ chỉ định các seeds được sử dụng để tạo PDA.
...
Solana Playground loại bỏ một số thiết lập mẫu cơ bản trong đó
