...
description: >-
  Tìm hiểu cách xây dựng chương trình CRUD (Tạo, Đọc, Cập nhật, Xóa) trên Solana
  bằng cách sử dụng Program Derived Addresses (PDAs) và framework Anchor. Hướng
  dẫn từng bước này chỉ cho bạn cách tạo, cập nhật và xóa các tài khoản tin nhắn
  trên chuỗi bằng PDAs, triển khai xác thực tài khoản và viết các bài kiểm tra.
  Hoàn hảo cho các nhà phát triển muốn hiểu cách sử dụng PDAs trong các chương
  trình Solana.
...
biết thêm chi tiết, hãy tham khảo trang
...
[`create_message`](mention:one), [`update_message`](mention:two), và
[`delete_message`](mention:three) để thêm vào các bước tiếp theo.
...
Trước khi bắt đầu, chạy `anchor build` trong terminal của Playground để kiểm tra
chương trình khởi đầu có được biên dịch thành công không.
...
Compiling anchor-attribute-interface v0.29.0
Compiling anchor-derive-accounts v0.29.0
Compiling anchor-attribute-program v0.29.0
Compiling anchor-attribute-state v0.29.0
Compiling anchor-derive-space v0.29.0
Compiling anchor-lang v0.29.0
Compiling message_program v0.1.0 (/home/user/message_program/programs/message_program)
    Finished release [optimized] target(s) in 14.09s
```

</Step>
<Step>

### Định nghĩa loại tài khoản tin nhắn

Đầu tiên, định nghĩa cấu trúc cho tài khoản tin nhắn mà chương trình tạo ra. Cấu
trúc này xác định dữ liệu để lưu trữ trong tài khoản được tạo bởi chương trình.
...
Thuộc tính _rs`#[account]`_ trong chương trình Anchor dùng để chú thích các
struct đại diện cho dữ liệu tài khoản (kiểu dữ liệu để lưu trữ trong trường dữ
liệu của Account).
...
["bump" seed](/docs/core/pda#canonical-bump) được sử dụng để tạo ra Program
Derived Address (PDA). Việc lưu trữ giá trị này giúp tiết kiệm tài nguyên tính
toán bằng cách loại bỏ nhu cầu tính toán lại nó trong các chỉ thị sau.
...
Trong ví dụ này, struct _rs`MessageAccount`_ lưu trữ tin nhắn được tạo bởi người
dùng và chứa ba trường:
...
- `bump` - _rs`u8`_ lưu trữ ["bump" seed](/docs/core/pda#canonical-bump) để tạo
  ...
  nguyên tính toán bằng cách loại bỏ nhu cầu tính toán lại nó trong các chỉ thị
  sau.
...
Kiểu _rs`Account<'info, T>`_ yêu cầu tài khoản phải khớp với kiểu được chỉ định.

Trong trường hợp này, tài khoản phải khớp với kiểu _rs`MessageAccount`_ tùy
chỉnh.

### !program

Kiểu _rs`Program<'info, T>`_ yêu cầu tài khoản phải khớp với một chương trình.

Trong trường hợp này, tài khoản phải khớp với kiểu _rs`System`_, đề cập đến
System Program.
...
Chú thích _rs`#[instruction(message: String)]`_ cho phép struct _rs`Create`_
truy cập tham số _rs`message`_ từ lệnh `create`.
...
Tính toán `space` _rs`(8 + 32 + 4 + message.len() + 1)`_ phân bổ không gian cho
...
Tiếp theo, thêm lệnh `update` để thay đổi `MessageAccount` bằng một tin nhắn
mới.
...
Thêm đoạn mã dưới đây vào bên trong _ts`describe()`_, nhưng trước các phần
_ts`it()`_.
...
Trong phần này, đoạn mã này chỉ đơn giản thiết lập tệp kiểm thử.
...
Như một phần của quá trình thiết lập, tệp kiểm thử tạo ra tài khoản message PDA.
Điều này minh họa cách tạo PDA trong Javascript sử dụng cùng các seed được chỉ
...
terminal Playground. Lệnh này chạy các bài kiểm thử đối với chương trình đã
