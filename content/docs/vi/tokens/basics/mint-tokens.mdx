...
Đúc token là quá trình tạo ra các đơn vị mới của một token bằng cách gọi lệnh
...
trên một token program. Chỉ có địa chỉ được chỉ định là mint authority trên mint
account mới có thể đúc token mới. Lệnh này cũng yêu cầu sự tồn tại của một token
account làm nơi đích cho các token được đúc.
