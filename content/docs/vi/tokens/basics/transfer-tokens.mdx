---
title: Chuyển Token
description: Tìm hiểu cách chuyển token giữa các token account.
---

## Cách chuyển Token

Chuyển token là việc di chuyển token từ một token account sang một token account
khác có cùng mint. Việc chuyển token diễn ra khi bạn gọi lệnh
...
trên một token program. Chỉ có địa chỉ được chỉ định là chủ sở hữu (authority)
của token account nguồn mới có thể chuyển token ra khỏi tài khoản đó.
