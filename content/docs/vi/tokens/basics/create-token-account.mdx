...
<Callout type="info">
  Thuật ngữ "owner" được sử dụng trong hai ngữ cảnh khác nhau:
...
Bạn có thể xem associated token account như tài khoản token mặc định cho người
dùng để lưu giữ các đơn vị của một token cụ thể (mint).

<Callout type="info">
  Thuật ngữ "associated token accounts" chỉ áp dụng cho các token account mà
...
Giao dịch để tạo token account cần hai lệnh:
...
Để tạo Associated Token Account, gọi lệnh
...
Lệnh để tạo associated token account tự động gọi System Program để tạo token
