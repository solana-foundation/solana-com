...
Vì Nhà phát hành có thể cập nhật hệ số nhân bất kỳ lúc nào, bạn nên cân nhắc
việc thăm dò định kỳ để cập nhật số dư tài khoản. Nhà phát hành không có khả
năng cập nhật hệ số nhân này nhiều hơn một lần mỗi ngày. Nếu hệ số nhân được đặt
cho một ngày trong tương lai, bạn có thể tự động thăm dò vào thời điểm cập nhật
này

## Số lượng token trong giao dịch (chuyển khoản / hoán đổi v.v.)

- Người dùng nên nhập số lượng được hiểu là "UIAmount" đã được điều chỉnh tỷ lệ.
  Ứng dụng xử lý điều này cần chuyển đổi thành số lượng token thô cho giao dịch.
...
- Các ứng dụng nên sử dụng tổng số lượng thô khi người dùng yêu cầu thực hiện
  ...
  còn dust nào còn lại.
  - Tùy chọn: Bạn có thể cân nhắc tự động đóng tài khoản khi sử dụng "tối đa" để
    hoàn trả cho người dùng khoản tiền gửi lưu trữ của họ

## Giá token

- Giá token luôn nên được hiển thị dưới dạng giá đã điều chỉnh tỷ lệ ở bất cứ
  nơi nào có thể.
- Nếu bạn là nhà cung cấp dịch vụ cung cấp giá, chẳng hạn như oracle, bạn nên
  ...
  - Bất cứ khi nào có thể, hãy cung cấp SDK/API trừu tượng hóa các phức tạp của
    tiện ích mở rộng UIAmount đã điều chỉnh tỷ lệ.
...
- Các dịch vụ cung cấp dữ liệu lịch sử nên lưu trữ và **hiển thị cả giá đã được
  điều chỉnh tỷ lệ và giá chưa điều chỉnh tỷ lệ cho phần mở rộng số lượng UI đã
  điều chỉnh tỷ lệ**.
- Chúng tôi kỳ vọng số lượng đã điều chỉnh tỷ lệ sẽ được sử dụng phổ biến nhất
  vì điều này phù hợp với cách thế giới tài chính truyền thống xử lý các biểu đồ
  liên quan đến token với việc chia tách cổ phiếu.

### Dữ liệu lịch sử cho số lượng

- Nếu bạn muốn hiển thị số dư đã chuyển trong quá khứ, bạn cần truy cập vào hệ
  số nhân tại slot đó. Bạn cũng có thể lưu UiAmount cho các giao dịch chuyển
  khoản khi xử lý giao dịch để tránh phải thực hiện tính toán này trong tương
  lai.
...
- Chúng tôi hy vọng điều này khuyến khích các nhóm cập nhật dapp của họ để tương
  thích với các token sử dụng phần mở rộng số lượng UI đã điều chỉnh tỷ lệ và
  chúng tôi sẵn sàng cung cấp hỗ trợ trong quá trình này.
...
| Hiển thị cập nhật hệ số nhân trong lịch sử giao dịch | Khi hệ số nhân được cập nhật, hiển thị như một sự kiện trong lịch sử giao dịch của người dùng bao gồm số lượng nhận được | **P2**  |
| Hiển thị biểu đồ lịch sử giá                         | Phản ánh giá đã điều chỉnh tỷ lệ trong biểu đồ giá                                                                       | **P1**  |
| Hướng dẫn/Chú giải                                   | Cung cấp chú giải hoặc hướng dẫn để giáo dục người dùng về các token sử dụng tiện ích mở rộng số lượng điều chỉnh tỷ lệ  | **P2**  |
...
| Tổng cung với điều chỉnh theo tỷ lệ    | Khi hiển thị tổng cung và tổng vốn hóa thị trường, cần tính đến số dư đã điều chỉnh                       | **P0**  |
...
| Dữ liệu hệ số nhân lịch sử                 | Cung cấp API với các thay đổi hệ số nhân theo lịch sử.                              | **P0**  |
...
| Yêu cầu                            | Mô tả                                                                                                                   | Ưu tiên |
| :--------------------------------- | :---------------------------------------------------------------------------------------------------------------------- | :------ |
| Hiển thị số dư token đã điều chỉnh | Hiển thị số dư đã điều chỉnh cho giao dịch hoặc cung cấp thanh khoản trên UI. (backend vẫn có thể sử dụng số lượng thô) | **P0**  |
