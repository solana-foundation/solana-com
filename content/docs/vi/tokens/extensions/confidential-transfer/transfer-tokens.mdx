...
## Cách chuyển token một cách bảo mật từ tài khoản token này sang tài khoản token khác
...
trạng thái _rs`ConfidentialTransferAccount`_ và được phê duyệt cho các giao dịch
chuyển bảo mật. Tài khoản token của người gửi cũng phải có số dư bảo mật khả
dụng để chuyển đi.
...
1. Tạo
   ...
   **Bằng chứng về tính hợp lệ của mã hóa
   ...
   Xác minh rằng các mã hóa số tiền chuyển được tạo đúng cách cho cả ba bên
   (nguồn, đích và kiểm toán viên), đảm bảo số tiền chuyển được mã hóa chính xác
   dưới khóa công khai của mỗi bên.
   ...
   **Bằng chứng về phạm vi
   ...
   Xác minh rằng số dư khả dụng mới và số tiền chuyển (được chia thành bit
...
4. Đóng các tài khoản trạng thái ngữ cảnh bằng chứng để lấy lại SOL đã sử dụng
   để tạo chúng.
...
Ví dụ sau đây minh họa cách chuyển token một cách bảo mật giữa các token
account.
