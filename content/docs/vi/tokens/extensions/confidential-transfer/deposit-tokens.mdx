...
2. **Số dư bảo mật khả dụng**: Số dư đang chờ xử lý sau đó được "áp dụng" vào số
   dư khả dụng, giúp token có thể sử dụng cho các giao dịch chuyển khoản bảo
   mật.
...
Để chuyển đổi số dư công khai thành số dư bảo mật đang chờ xử lý, hãy gọi lệnh
...
cho mỗi lệnh gửi được giới hạn ở mức 2^48.

Crate `spl_token_client` cung cấp phương thức `confidential_transfer_deposit` để
xây dựng và gửi giao dịch với lệnh `Deposit`, như được minh họa trong ví dụ dưới
đây.
...
Để chạy ví dụ này, hãy khởi động validator cục bộ với Token Extensions Program
