...
description:
  ...
  các tính năng tùy chọn vào mint và tài khoản token.
---

## Confidential Transfers là gì?

Confidential transfers cho phép bạn chuyển token giữa các token account mà không
tiết lộ số lượng chuyển. Điều này hữu ích cho các giao dịch bảo mật quyền riêng
tư. Chỉ có số lượng chuyển và số dư token là riêng tư. Các địa chỉ token account
vẫn công khai.
...
Tiện ích mở rộng Confidential Transfer thêm
[các chỉ thị](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
vào Token Extensions Program cho phép bạn chuyển token giữa các tài khoản mà
không tiết lộ số lượng chuyển.
...
Quy trình cơ bản của chuyển token bảo mật như sau:

1. Tạo một mint account với tiện ích mở rộng confidential transfer.
2. Tạo token account với tiện ích mở rộng confidential transfer cho người gửi và
   người nhận.
...
5. **Áp dụng** số dư đang chờ xử lý của người gửi vào **số dư bảo mật khả
   dụng**.
6. **Chuyển** bảo mật token từ token account của người gửi đến token account của
   người nhận.
7. **Áp dụng** số dư đang chờ xử lý của người nhận vào **số dư bảo mật khả
   dụng**.
8. **Rút** số dư bảo mật khả dụng của người nhận vào **số dư công khai**.

Để biết thêm chi tiết về các bước trong quy trình chuyển bảo mật, xem các trang
tương ứng:
...
<Cards>
  ...
  >
    Cách tạo mint account với tiện ích mở rộng Confidential Transfer
  </Card>
  <Card
    title="Tạo Token Account"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Cách cấu hình token account với tiện ích mở rộng Confidential Transfer
  </Card>
  <Card
    title="Gửi Token"
  ...
  >
    Cách áp dụng số dư đang chờ xử lý vào số dư bảo mật khả dụng
  </Card>
  <Card
    title="Rút Token"
...
sao chép từ mainnet bằng lệnh sau. Bạn phải cài đặt Solana CLI để sử dụng
validator cục bộ.
...
| Lệnh                                | Mô tả                                                                                                                                                                     |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Thiết lập tài khoản mint cho các giao dịch chuyển bảo mật. Lệnh này phải được đưa vào cùng giao dịch với lệnh _rs`TokenInstruction::InitializeMint`_.                     |
...
| _rs`DisableConfidentialCredits`_    | Chặn các giao dịch chuyển bảo mật đến trong khi vẫn cho phép các giao dịch chuyển công khai.                                                                              |
