...
bổ sung trạng thái bổ sung vào token account. Phần này giải thích cách tạo một
token account với tiện ích mở rộng này được kích hoạt.
...
Tiện ích mở rộng này bổ sung trạng thái
...
_rs`ConfidentialTransferAccount`_ chứa một số trường để quản lý các giao dịch
chuyển token bảo mật:

- **approved**: Trạng thái phê duyệt của tài khoản cho các giao dịch chuyển bảo
  mật. Nếu cấu hình `auto_approve_new_accounts` của mint account được đặt là
  `true`, tất cả token account sẽ tự động được phê duyệt cho các giao dịch
  chuyển bảo mật.

- **elgamal_pubkey**: Khóa công khai ElGamal được sử dụng để mã hóa số dư và số
  lượng chuyển.

- **pending_balance_lo**: 16 bit thấp đã được mã hóa của số dư đang chờ xử lý.
  Số dư được chia thành phần cao và thấp để giải mã hiệu quả.

- **pending_balance_hi**: 48 bit cao đã được mã hóa của số dư đang chờ xử lý. Số
  dư được chia thành phần cao và thấp để giải mã hiệu quả.
...
- **decryptable_available_balance**: Số dư khả dụng được mã hóa bằng khóa
  Advanced Encryption Standard (AES) để chủ tài khoản có thể giải mã hiệu quả.

- **allow_confidential_credits**: Nếu là true, cho phép các giao dịch chuyển bảo
  mật đến.

- **allow_non_confidential_credits**: Nếu là true, cho phép các giao dịch chuyển
  không bảo mật đến.

- **pending_balance_credit_counter**: Đếm số lần ghi có vào số dư đang chờ xử lý
  từ các lệnh gửi tiền và chuyển tiền.

- **maximum_pending_balance_credit_counter**: Giới hạn số lượng ghi có đang chờ
  xử lý trước khi yêu cầu lệnh _rs`ApplyPendingBalance`_ để chuyển đổi số dư
...
Số dư bảo mật được phân tách thành số dư đang chờ xử lý và số dư khả dụng để
ngăn chặn các cuộc tấn công DoS. Nếu không có sự phân tách này, kẻ tấn công có
thể liên tục gửi token đến một token account, chặn khả năng chuyển token của chủ
...
Số dư đang chờ xử lý bảo mật được chia thành `pending_balance_lo` và
`pending_balance_hi` vì giải mã ElGamal đòi hỏi nhiều tính toán hơn cho các số
lớn hơn. Bạn có thể tìm thấy cách triển khai phép tính mã hóa
...
1. Client tra cứu số dư đang chờ xử lý và số dư khả dụng hiện tại, mã hóa tổng,
   và cung cấp một `decryptable_available_balance` được mã hóa bằng khóa AES của
   chủ token account.
...
Các bộ đếm expected/actual khớp nhau cho biết `decryptable_available_balance`
khớp với `available_balance`.

Khi truy xuất trạng thái của token account để đọc
`decryptable_available_balance`, các giá trị bộ đếm expected/actual khác nhau
yêu cầu client phải tra cứu các lệnh gửi/chuyển gần đây khớp với sự chênh lệch
bộ đếm để tính toán số dư chính xác.

**Quy trình đối chiếu số dư**

Khi bộ đếm số dư đang chờ xử lý expected và actual khác nhau, hãy làm theo các
bước sau để đối chiếu `decryptable_available_balance`:

1. Bắt đầu với `decryptable_available_balance` từ token account
2. Lấy các giao dịch gần đây nhất bao gồm các lệnh gửi và chuyển tối đa bằng sự
...
Việc tạo token account với tiện ích mở rộng Confidential Transfer yêu cầu ba
lệnh:
...
2. **Phân bổ lại không gian tài khoản**: Gọi lệnh
   _rs`TokenInstruction::Reallocate`_ của Token Extension Program để thêm không
...
3. **Cấu hình Confidential Transfers**: Gọi lệnh
   ...
   của Token Extension Program để khởi tạo trạng thái
   _rs`ConfidentialTransferAccount`_.

<Callout type="info">

**Chỉ chủ sở hữu token account mới có thể cấu hình token account cho
confidential transfers**.

Lệnh _rs`ConfigureAccount`_ yêu cầu tạo khóa mã hóa và dữ liệu chứng minh ở phía
client mà chỉ có thể được tạo bởi chủ sở hữu token account.

Lệnh _rs`PubkeyValidityProofData`_ tạo một bằng chứng xác minh rằng khóa ElGamal
hợp lệ. Để biết chi tiết triển khai, xem:
...
Đoạn mã sau minh họa cách tạo Associated Token Account với tiện ích mở rộng
Confidential Transfer,

Để chạy ví dụ, hãy khởi động validator cục bộ với Token Extension Program được
...
<Callout type="info">
  ...
  Extension từ mạng chính để chạy mã ví dụ.
</Callout>
...
---CODE-PLACEHOLDER-a9c9c8e5c3c9c8e5c3c9c8e5c3c9c8e5---
