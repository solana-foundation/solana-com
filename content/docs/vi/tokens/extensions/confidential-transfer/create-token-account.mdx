...
- **expected_pending_balance_credit_counter**: Giá trị
  ...
  instruction data lần cuối cùng khi lệnh _rs`ApplyPendingBalance`_ được xử lý.
...
Các bộ đếm dự kiến/thực tế khớp nhau cho biết `decryptable_available_balance`
khớp với `available_balance`.

Khi lấy trạng thái của token account để đọc `decryptable_available_balance`, các
giá trị bộ đếm dự kiến/thực tế khác nhau yêu cầu client phải tra cứu các lệnh
gửi/chuyển gần đây khớp với sự chênh lệch bộ đếm để tính toán số dư chính xác.
...
2. Lấy các giao dịch gần đây nhất bao gồm các lệnh gửi và chuyển tối đa bằng sự
   chênh lệch bộ đếm (thực tế - dự kiến):
...
Đoạn mã sau đây minh họa cách tạo Associated Token Account với phần mở rộng
