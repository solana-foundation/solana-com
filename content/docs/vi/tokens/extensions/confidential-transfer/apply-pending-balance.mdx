...
description: Tìm hiểu cách áp dụng số dư chờ xử lý để sử dụng tiền.
---
...
2. **Số dư bảo mật khả dụng**: Sau đó, số dư chờ xử lý được "áp dụng" vào số dư
   khả dụng, giúp token có thể sử dụng cho các giao dịch chuyển khoản bảo mật.
...
Để chạy ví dụ này, hãy khởi động một validator cục bộ với Token Extensions
Program được sao chép từ mainnet bằng lệnh sau. Bạn phải cài đặt Solana CLI để
