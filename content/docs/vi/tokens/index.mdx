...
Tham khảo phần [SPL Token Basics](/docs/tokens/basics) để xem các ví dụ mã.
...
- [Associated Token Account](#associated-token-account) là một Token Account
  được tạo với địa chỉ được lấy từ địa chỉ chủ sở hữu và mint account.
...
là [tài khoản dữ liệu](/docs/core/accounts#data-account) thuộc sở hữu của Token
Program.
...
- **Mint authority**: Tài khoản được ủy quyền để tạo ra các đơn vị mới của
  token, làm tăng tổng cung
- **Freeze authority**: Tài khoản được ủy quyền để đóng băng token trong một
  Token Account, ngăn chặn việc chuyển hoặc đốt token
...
Associated Token Accounts đơn giản hóa quá trình tìm địa chỉ token account cho
...
Tài khoản Token Liên kết được tạo với địa chỉ được lấy từ địa chỉ của chủ sở hữu
và địa chỉ của tài khoản mint. Điều quan trọng cần hiểu là một Associated Token
...
[Program Derived Address (PDA)](/docs/core/pda). Một PDA tạo ra một địa chỉ một
cách xác định bằng cách sử dụng các đầu vào được xác định trước, giúp dễ dàng
tìm địa chỉ của một tài khoản.
...
có thể nhận SOL devnet từ [web faucet](https://faucet.solana.com/) công khai.
...
dụ, lệnh _sh`create-token`_ xuất ra địa chỉ tài khoản mint với ví Playground của
...
Token Account, hãy gửi một giao dịch với hai chỉ thị. Đây là một ví dụ bằng
Javascript trên
...
Chỉ thị _rs`MintTo`_ trên Token Program tạo ra các token mới. Mint authority
...
Token URI liên kết đến metadata ngoài chuỗi. Xem ví dụ về định dạng JSON
...
Để biết chi tiết về Token Extensions, xem Token Extensions
[Hướng dẫn bắt đầu](/developers/guides/token-extensions/getting-started) và
