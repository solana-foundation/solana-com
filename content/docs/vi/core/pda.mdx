...
- **Địa chỉ tài khoản xác định**: PDAs cung cấp cơ chế để tạo địa chỉ một cách
  xác định bằng cách sử dụng kết hợp các "seeds" tùy chọn (các đầu vào được xác
  định trước) và một ID chương trình cụ thể.
...
trình được sử dụng để tạo địa chỉ. Bạn có thể xem việc tạo PDA như tìm một địa
chỉ trên bản đồ. Chỉ có địa chỉ không có nghĩa là có thứ gì đó được xây dựng tại
vị trí đó.
...
trình "ký" cho PDA mà không cần khóa riêng tư.

Để hiểu rõ hơn, Solana
[Keypairs](https://github.com/anza-xyz/solana-sdk/blob/sdk%40v2.2.2/keypair/src/lib.rs#L26)
là các điểm trên đường cong Ed25519 (mật mã đường cong elliptic) với khóa công
khai và khóa riêng tư tương ứng. Khóa công khai được sử dụng làm địa chỉ (định
danh duy nhất) cho các tài khoản trên chuỗi.
...
1 đơn vị, cho đến khi một giá trị tạo ra một địa chỉ off-curve hợp lệ. Giá trị
bump đầu tiên tạo ra địa chỉ off-curve hợp lệ được gọi là "canonical bump."

Các ví dụ sau đây cho thấy quá trình tạo PDA sử dụng tất cả các bump seeds có
thể có (từ 255 đến 0):

<Callout type="info">
  Ví dụ Kit không được đưa vào vì hàm
...
<Callout type="warn">
  ...
  nhất là chỉ sử dụng canonical bump khi tạo PDA.
</Callout>
...
Giao dịch trong file kiểm thử gọi chỉ thị `initialize` để tạo một tài khoản
...
File kiểm thử cũng cho thấy cách lấy tài khoản on-chain được tạo tại địa chỉ đó
