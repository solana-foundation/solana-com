...
description:
  ...
  chức năng trên toàn mạng Solana.
...
mạng và CPI như một API nội bộ gọi một API khác.
...
- Chương trình được gọi có thể thực hiện thêm các CPI đến các chương trình khác,
...
[lệnh](/docs/core/transactions#instruction) để thêm vào một giao dịch. Bên dưới,
mỗi lệnh CPI phải chỉ định:
...
- Các đặc quyền của người ký từ giao dịch ban đầu được mở rộng đến chương trình
  được gọi (ví dụ: A->B)
- Chương trình được gọi có thể thực hiện thêm các Cross Program Invocation đến
  các chương trình khác, với độ sâu tối đa là 4 (ví dụ: B->C, C->D)
...
trình này sang chương trình khác. Dưới đây là ý nghĩa của điều này:
...
- Chương trình B thậm chí có thể chuyển tiếp các quyền này nếu nó thực hiện các
  CPIs của riêng mình
...
Các ví dụ sau đây cho thấy cách thực hiện một CPI sử dụng
...
- Ví dụ 2: Sử dụng hàm _rs`system_instruction::transfer`_ từ crate
  `solana_program` để xây dựng lệnh CPI. Ví dụ 1 trừu tượng hóa cách thực hiện
  này.
- Ví dụ 3: Xây dựng lệnh CPI một cách thủ công. Cách tiếp cận này hữu ích khi
  không có crate nào tồn tại để giúp xây dựng lệnh.
...
sang tài khoản khác sử dụng CPI. File kiểm thử sử dụng
...
- Ví dụ 3: Xây dựng lệnh CPI thủ công. Cách tiếp cận này hữu ích khi không có
  crate nào có sẵn để giúp xây dựng lệnh bạn muốn gọi.
...
một PDA đến tài khoản người nhận bằng cách sử dụng CPI được ký bởi PDA. File
test sử dụng [LiteSVM](https://github.com/LiteSVM/litesvm) để kiểm tra chương
trình.
