...
description:
  Tìm hiểu về các chương trình Solana (smart contract) và cách phát triển chúng
  bằng Rust hoặc framework Anchor. Hiểu về triển khai, nâng cấp và xác minh
  chương trình trên mạng Solana.
h1: Các chương trình
---

Trên Solana, "smart contract" được gọi là chương trình (programs).
[Chương trình](/docs/core/accounts#program-account) được triển khai trên chuỗi
vào các tài khoản chứa tệp nhị phân thực thi đã được biên dịch của chương trình.
Người dùng tương tác với chương trình bằng cách gửi các giao dịch chứa
[chỉ thị](/docs/core/transactions#instruction) để cho chương trình biết phải làm
gì.

## Điểm chính

- Chương trình là các tài khoản chứa **mã thực thi**, được tổ chức thành các hàm
  gọi là **chỉ thị** (instructions).
- Mặc dù chương trình **không lưu trữ trạng thái**, chúng có thể bao gồm các chỉ
  thị để tạo và cập nhật các tài khoản khác để lưu trữ dữ liệu.
- Một **quyền nâng cấp** có thể cập nhật chương trình. Khi quyền này bị xóa bỏ,
  chương trình trở nên bất biến.
...
- [Rust thuần](/docs/programs/rust): Cách tiếp cận này liên quan đến việc viết
  chương trình Solana bằng Rust mà không sử dụng bất kỳ framework nào. Nó cung
  cấp nhiều linh hoạt hơn nhưng đi kèm với độ phức tạp cao hơn.
...
bởi một tài khoản được chỉ định là "quyền nâng cấp", thường là tài khoản đã
triển khai chương trình ban đầu. Nếu
...
chương trình có khớp với mã nguồn công khai của nó hay không, giúp phát hiện sự
khác biệt giữa phiên bản nguồn và phiên bản đã triển khai.
...
- **Tìm kiếm chương trình đã xác minh**: Để nhanh chóng kiểm tra các chương
  ...
  [Solana Explorer](https://explorer.solana.com/). Xem ví dụ về chương trình đã
  xác minh
...
đầu. Quyền thực hiện điều này được quản lý bởi thẩm quyền của chương trình vì
quyền sở hữu tài khoản của mỗi chương trình thuộc về bộ tải.
...
Chương trình ed25519 xử lý một lệnh. Byte đầu tiên `u8` là số lượng chữ ký cần
...
Chương trình secp256k1 xử lý một lệnh mà byte đầu tiên là số lượng của cấu trúc
sau được tuần tự hóa trong dữ liệu lệnh:
...
biệt, người dùng cũng có thể nhận dữ liệu từ chính giao dịch đó.

Chi phí của giao dịch sẽ tính dựa trên số lượng chữ ký cần xác minh nhân với hệ
...
| Chương trình           | ID Chương trình                               | Mô tả                                                                                                                     | Hướng dẫn                                                                                        |
| ---------------------- | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| Chương trình Secp256r1 | `Secp256r1SigVerify1111111111111111111111111` | Xác minh tối đa 8 chữ ký secp256r1. Nhận vào chữ ký, khóa công khai và thông điệp. Trả về lỗi nếu có bất kỳ thất bại nào. | [Hướng dẫn](https://docs.rs/solana-secp256k1-recover/latest/solana_secp256k1_recover/index.html) |

Chương trình secp256r1 xử lý một instruction. Byte đầu tiên `u8` là số lượng chữ
