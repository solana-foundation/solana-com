...
Trong khi validator thử nghiệm đang chạy, hãy chạy lệnh `solana program deploy`
trong một cửa sổ terminal riêng để triển khai chương trình lên validator cục bộ.
...
Chạy tập lệnh client với lệnh sau.
...
Chạy mã client một lần nữa và kiểm tra chữ ký giao dịch trên Solana Explorer để
...
`./target/deploy/hello_world-keypair.json`) và chạy lại `cargo build-sbf`, cách
này sẽ tạo một tệp keypair mới.
