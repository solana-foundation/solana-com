...
- `processor.rs`: Định nghĩa các trình xử lý lệnh (hàm) thực hiện logic nghiệp
  vụ cho mỗi lệnh.
...
Để minh họa cách xây dựng một chương trình Rust gốc với nhiều lệnh, chúng ta sẽ
xem xét một chương trình đếm đơn giản thực hiện hai lệnh:
...
- `accounts`: `AccountInfo` cho các tài khoản cần thiết bởi chỉ thị đang được
  gọi
- `instruction_data`: Dữ liệu bổ sung được truyền vào chương trình để chỉ định
...
Khi xây dựng một chương trình Solana, bạn thường bắt đầu bằng việc định nghĩa
trạng thái của chương trình - dữ liệu sẽ được lưu trữ trong các tài khoản được
tạo và sở hữu bởi chương trình của bạn.

Trạng thái chương trình được định nghĩa bằng cách sử dụng các struct Rust đại
diện cho cấu trúc dữ liệu của các tài khoản trong chương trình của bạn. Bạn có
thể định nghĩa nhiều struct để đại diện cho các loại tài khoản khác nhau cho
...
Thêm struct `CounterAccount` vào `lib.rs` để định nghĩa trạng thái chương trình.
...
Instruction đề cập đến các hoạt động khác nhau mà chương trình Solana của bạn có
thể thực hiện. Hãy coi chúng như các API công khai cho chương trình của bạn -
chúng định nghĩa những hành động mà người dùng có thể thực hiện khi tương tác
...
thường người ta triển khai một phương thức hỗ trợ. Phương thức này:

1. Tách byte đầu tiên để lấy biến thể instruction
2. Khớp với biến thể và phân tích cú pháp các tham số bổ sung từ các byte còn
   lại
...
Thêm đoạn mã sau vào `lib.rs` để định nghĩa các instruction cho chương trình
counter.
...
Thêm đoạn mã sau vào `lib.rs`. Đoạn mã này sử dụng enum `CounterInstruction` và
phương thức `unpack` đã định nghĩa ở bước trước để định tuyến các instruction
...
1. Tài khoản counter sẽ được tạo và khởi tạo
2. Tài khoản người trả phí sẽ cấp vốn cho việc tạo tài khoản mới
3. System Program mà chúng ta gọi để tạo tài khoản mới

Để xác định các tài khoản cần thiết cho lệnh, chúng ta tạo một iterator qua
slice `accounts` và sử dụng hàm `next_account_info` để lấy từng tài khoản. Số
lượng tài khoản bạn xác định chính là số tài khoản cần thiết cho lệnh.

Thứ tự các tài khoản rất quan trọng - khi xây dựng lệnh ở phía client, các tài
khoản phải được cung cấp theo cùng thứ tự như đã được định nghĩa trong chương
trình để lệnh có thể thực thi thành công.
...
gọi lệnh `create_account` của System Program.
...
Để gọi System Program từ instruction của chương trình của chúng ta, chúng ta
thực hiện Cross Program Invocation (CPI) thông qua hàm `invoke`. CPI cho phép
một chương trình gọi các instruction trên các chương trình khác - trong trường
hợp này, instruction `create_account` của System Program.
...
3. Tuần tự hóa struct `CounterAccount` vào trường dữ liệu của tài khoản, từ đó
   lưu trữ `initial_value` trên tài khoản.
...
Tiếp theo, thêm phần triển khai của hàm `process_increment_counter`. Instruction
...
iterator qua các tài khoản. Trong trường hợp này, chúng ta chỉ cần một tài
khoản, đó là tài khoản sẽ được cập nhật.
...
tài khoản đều được cung cấp bởi người gọi instruction, không có gì đảm bảo rằng
các tài khoản được cung cấp là những tài khoản mà chương trình mong đợi. Thiếu
các kiểm tra xác thực tài khoản là một nguồn phổ biến gây ra lỗ hổng chương
trình.

Ví dụ dưới đây bao gồm một kiểm tra để đảm bảo tài khoản mà chúng ta đang gọi là
`counter_account` thuộc sở hữu của chương trình đang thực thi.
...
- Mượn trường dữ liệu của tài khoản hiện có với quyền thay đổi
...
Sau đó thêm module kiểm thử sau vào `lib.rs` và chạy `cargo test-sbf` để thực
thi các bài kiểm thử. Tùy chọn, sử dụng cờ `--nocapture` để xem các câu lệnh
print trong đầu ra.
...
Đầu tiên, thiết lập module kiểm thử và import các dependency cần thiết:
...
Tiếp theo, thiết lập bài kiểm thử sử dụng `ProgramTest`. Sau đó tạo một keypair
mới để sử dụng làm địa chỉ cho tài khoản counter mà chúng ta sẽ khởi tạo và định
nghĩa một giá trị ban đầu để thiết lập cho counter.
...
- Public key của tài khoản (`Pubkey`)
...
Để kiểm thử instruction khởi tạo:

- Tạo instruction data với biến thể 0 (`InitializeCounter`) và giá trị ban đầu
- Xây dựng instruction với program ID, instruction data, và các tài khoản cần
  thiết
...
Để kiểm thử instruction tăng giá trị:

- Xây dựng instruction với program ID, instruction data, và các tài khoản cần
  thiết
- Gửi một giao dịch với instruction tăng giá trị
- Kiểm tra xem tài khoản đã được tăng giá trị chính xác chưa

Lưu ý rằng instruction data cho instruction tăng giá trị là `[1]`, tương ứng với
biến thể 1 (`IncrementCounter`). Vì không có tham số bổ sung nào cho instruction
