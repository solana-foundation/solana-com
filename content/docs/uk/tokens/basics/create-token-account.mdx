...
Token account — це тип акаунта в Token Programs Solana, який зберігає інформацію
про володіння певним токеном (mint) окремою особою. Кожен token account
пов'язаний з одним mint і відстежує такі деталі, як баланс токенів та власник.
...
2. Власника (авторитет, який може переказувати токени з акаунта)

Ось приклад з використанням USD Coin (USDC) на Solana:

- Адреса mint для USD Coin: `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
...
- Circle має право власника за адресою
...
1. "Власник" token account — це адреса, збережена в даних token account як поле
   ...
   делегувати токени з акаунта. Цю адресу іноді називають "авторитетом" token
   account, щоб відрізнити її від власника програми.
...
account користувача для будь-якого mint. Ви можете переглянути реалізацію
деривації
...
спрощує пошук token account користувача для будь-якого mint токена, усуваючи
необхідність окремо відстежувати адреси token account.
...
<Callout type="info">
  ...
  обліковим записом, який має таку ж структуру типу `Account`, як визначено в
  token program. Associated Token Program не зберігає стан - вона просто
...
Щоб створити Token Account, викличте інструкцію
...
Транзакція для створення token account потребує двох інструкцій:

1. Викликати System Program для створення та виділення місця для token account і
   передачі права власності до Token Program.
2. Викликати Token Program для ініціалізації даних token account.
...
Program для створення token account і викликає Token Program для ініціалізації
даних token account. Це відбувається через Cross Program Invocations (CPI).
