...
Розширення Confidential Transfer забезпечує приватні перекази токенів шляхом
додавання додаткового стану до mint account. Цей розділ пояснює, як створити
токен mint з увімкненим цим розширенням.
...
Стан _rs`ConfidentialTransferMint`_ містить три поля конфігурації:
...
1. **Створення Mint Account**: Викликати інструкцію System Program
   _rs`CreateAccount`_ для створення mint account.
...
2. **Ініціалізація розширення Confidential Transfer**: Викликати інструкцію
   ...
   для налаштування стану _rs`ConfidentialTransferMint`_ для мінту.

3. **Ініціалізація мінту**: Викликати інструкцію Token Extensions Program
   _rs`Instruction::InitializeMint`_ для ініціалізації стандартного стану мінту.
...
бути встановлений Solana CLI для запуску локального validator.
