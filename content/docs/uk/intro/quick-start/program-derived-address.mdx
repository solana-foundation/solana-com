...
description:
  ...
  розробників, які хочуть зрозуміти, як використовувати PDA в програмах Solana.
...
2. Ініціалізація даних облікового запису:
   ...
   - Встановлює поле `user` як відкритий ключ облікового запису `user`.

   ```rs
   account_data.user = ctx.accounts.user.key();
   ```

   - Встановлює поле `message` значенням `message` з аргументу функції.
...
Тіло функції просто виводить повідомлення в журнал програми за допомогою макросу
...
Ви завершили базову програму CRUD. Розгорніть програму, виконавши `deploy` у
