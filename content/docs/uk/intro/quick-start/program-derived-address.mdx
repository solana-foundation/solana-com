...
description:
  Дізнайтеся, як створити CRUD (Create, Read, Update, Delete) програму Solana за
  допомогою Program Derived Addresses (PDAs) та фреймворку Anchor. Цей
  покроковий посібник демонструє, як створювати, оновлювати та видаляти облікові
  записи повідомлень у блокчейні за допомогою PDA, реалізовувати валідацію
  облікових записів та писати тести. Ідеально підходить для розробників, які
  хочуть зрозуміти, як використовувати PDA в програмах Solana.
...
записі з детермінованою адресою, отриманою з самої програми (Program Derived
Address або PDA).
...
У файлі `lib.rs` ви знайдете програму з інструкціями
[create_message](mention:one), [update_message](mention:two) та
[delete_message](mention:three), які потрібно додати в наступних кроках.
...
Перед початком запустіть `anchor build` у терміналі Playground, щоб перевірити,
чи успішно компілюється початкова програма.
...
дані облікового запису (тип даних для зберігання в полі даних Account).
...
- `user` - _rs`Pubkey`_, який ідентифікує користувача, що створив обліковий
  запис повідомлення.
- `message` - _rs`String`_, який містить повідомлення користувача.
...
інструкцію, та забезпечувати безпеку програми.

Назви полів у структурі забезпечують доступ до акаунтів у коді вашої програми,
але не впливають на перевірку. Варто використовувати описові назви для ясності.
...
1. _rs`user: Signer<'info>`_
   ...
   - Потребує статусу змінюваності (_rs`#[account(mut)]`_), оскільки оплачує
     новий акаунт
   - Повинен підписати транзакцію для схвалення зняття lamport з цього акаунту
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - `space` виділяє необхідні байти для поля даних акаунту
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - За лаштунками обмеження `init` викликає System Program для створення нового
     облікового запису із вказаним `space` і змінює власника на поточну
     програму.
...
Зауважте, що обмеження _rs`bump = message_account.bump`_ використовує bump seed,
збережений у _rs`message_account`_, замість того, щоб Anchor перераховував його.
...
Обмеження _rs`seeds`_ визначає seed-значення, які використовуються для отримання
PDA.
...
У цьому випадку обліковий запис _rs`user`_ отримує lamport із закритого
_rs`message_account`_.
...
1. _rs`user: Signer<'info>`_
   ...
   - Повинен підписати, щоб гарантувати, що тільки правильний користувач може
...
_rs`msg!()`_. Функція не потребує додаткової логіки, оскільки обмеження
...
Запустіть тестовий файл, виконавши _shell`test`_ в терміналі Playground, щоб
...
терміналі Playground. Ця команда запускає тести проти програми, розгорнутої в
...
<Callout type="info">
  ...
  запис. Для даного PDA може існувати лише один обліковий запис.
</Callout>
