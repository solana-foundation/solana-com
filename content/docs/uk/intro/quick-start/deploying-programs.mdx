...
(смарт-контракт) за допомогою фреймворку Anchor. Наприкінці ви розгорнете свою
першу програму в блокчейні Solana.

Мета цього розділу — ознайомити вас із Solana Playground. У посібнику
розглядається більш детальний приклад у розділах Program Derived Address (PDA)
та Cross-Program Invocation (CPI). Для отримання додаткової інформації
зверніться до сторінки [Програми на Solana](/docs/core/programs).
...
Ви побачите новий створений проєкт з кодом програми у файлі `lib.rs`.
...
- Вимагає параметр _`data`_ як вхідні дані
- Створює новий обліковий запис
- Зберігає значення параметра _`data`_ у даних облікового запису
...
Макрос _`declare_id!`_ вказує адресу вашої програми в блокчейні. Solana
...
Атрибут _rs`#[derive(Accounts)]`_ позначає структуру, що містить рахунки
програми.

### !account

Атрибут _rs`#[account]`_ визначає структуру, яка представляє тип даних для
рахунків, які ця програма може створювати та якими може володіти.

### !constraints

Атрибути _rs`#[account(...)]`_ визначають обмеження для рахунку.

Вони додають поширені перевірки або функції, які Anchor надає для підвищення
безпеки програми.
...
_rs`ctx.accounts.new_account.data`_ отримує доступ до поля `data` рахунку
`new_account` і встановлює його значення рівним параметру `data: u64`.

### !msg

Макрос _rs`msg!`_ записує повідомлення в журнал програми транзакції.

### !init

Обмеження `init` вказує, що програма повинна створити рахунок. Під капотом це
обмеження викликає системну програму для створення нового рахунку і робить цю
програму власником. Як власник програми, ця програма може записувати дані в
рахунок.

### !payer

Обмеження `payer` вказує рахунок, який оплачує новий рахунок.

### !space

Обмеження `space` вказує кількість байтів, які потрібно виділити для рахунку.
...
1. `ctx: Context<Initialize>` - Передає функції облікові записи, необхідні для
   цієї інструкції, як зазначено в структурі `Initialize`.
2. `data: u64` - Користувацький параметр, який ви повинні надати при виклику
   інструкції.

Тіло функції [встановлює поле `data` об'єкта `new_account`](mention:three) на
наданий аргумент `data`, а потім виводить повідомлення в журнал програми.
...
- Атрибут [`#[derive(Accounts)]`](mention:one) визначає структуру, яка вказує
  облікові записи, необхідні для певної інструкції, де кожне поле представляє
...
### Побудова та розгортання програми

Щоб побудувати програму, просто запустіть `build` в терміналі. Команда `build`
компілює програму. Байт-код програми потім зберігається у виконуваному
обліковому записі програми при розгортанні.
...
Після збірки програми, виконайте _shell`deploy`_ у терміналі, щоб розгорнути
...
для devnet, виконавши _shell`solana airdrop 5`_ у терміналі Playground або
...
Ви також можете використовувати кнопки `Build` та `Deploy` на лівій панелі.
...
Ви також можете використовувати кнопку `Test` на лівій панелі.

![Запуск тесту](/assets/docs/intro/quickstart/pg-test.png)

Потім ви можете переглянути журнали транзакцій, виконавши команду
`solana confirm -v` і вказавши хеш транзакції (підпис) з виводу тесту:
...
<Callout>
  Не забудьте оновити підключення до кластера (мережі) в Explorer, щоб воно
  відповідало Solana Playground. Solana Playground за замовчуванням використовує
  кластер devnet.
</Callout>
...
Тільки власник права на оновлення програми може її закрити. Процес розгортання
встановлює власника права на оновлення, коли ви розгортаєте програму. Цей
обліковий запис має виключний дозвіл на оновлення або закриття програми. Якщо
хтось відкликає право на оновлення, програма стає незмінною без будь-якої
можливості закриття або оновлення.

При розгортанні програм на Solana Playground, ваш гаманець Playground
автоматично стає власником права на оновлення для всіх ваших програм.

</Accordion>
</Accordions>

Вітаємо. Ви щойно створили та розгорнули свою першу програму Solana,
використовуючи фреймворк Anchor.
