...
description:
  ...
  отримання гаманців, програмних рахунків та рахунків токенів за допомогою
  JavaScript/TypeScript, з практичними прикладами використання бібліотеки Solana
  web3.js.
---
...
У Solana всі дані існують у "accounts". Можна уявити дані в Solana як публічну
...
1. Згенерувати нову keypair (пару публічного/приватного ключів).
...
[тип облікового запису](https://github.com/anza-xyz/agave/blob/v2.1.11/sdk/account/src/lib.rs#L48-L60).
...
облікові записи гаманців, програми мають ту саму базову структуру даних
[Account](https://github.com/anza-xyz/agave/blob/v2.1.11/sdk/account/src/lib.rs#L48-L60),
але з ключовими відмінностями в полях:

| Поле         | Опис                                                                                                                                                                                               |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `executable` | Встановлено як `true`, що вказує на те, що цей обліковий запис містить виконуваний програмний код.                                                                                                 |
| `data`       | Для програмних облікових записів це поле зберігає виконуваний код програми. На відміну від цього, облікові записи гаманців мають порожнє поле даних.                                               |
| `owner`      | Обліковий запис належить програмі Loader, яка є категорією вбудованих програм, що володіють виконуваними програмними обліковими записами в Solana. (`BPFLoader2111111111111111111111111111111111`) |

Рахунок програми зберігає скомпільований байткод для
[вихідного коду](https://github.com/solana-program/token/tree/main/program)
Token Program. Ви можете переглянути цей рахунок програми на
...
## Отримання рахунку Mint

Цей приклад отримує рахунок Mint для USD Coin (USDC), щоб показати, як програми
на Solana зберігають стан в окремих рахунках.

Рахунок
[Mint](https://github.com/solana-program/token/blob/program%40v8.0.0/program/src/state.rs#L16-L30)
є рахунком, яким володіє Token Program. Він зберігає глобальні метадані для
конкретного токена, включаючи загальну пропозицію, кількість десяткових знаків
та рахунки, уповноважені випускати або заморожувати токени. Адреса рахунку Mint
унікально ідентифікує токен у мережі Solana.
...
Ось ключові моменти щодо даних рахунку Mint:

| Поле         | Опис                                                                                                                  |
| ------------ | --------------------------------------------------------------------------------------------------------------------- |
| `executable` | Це поле має значення `false`, оскільки рахунок mint зберігає стан, а не виконуваний код.                              |
| `data`       | Це поле містить серіалізований стан рахунку, такий як орган випуску, загальна пропозиція, кількість десяткових знаків |
| `owner`      | Token Program (`TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA`) володіє рахунком mint.                                  |
...
## Десеріалізація рахунку Mint

Поле `data` рахунку Solana містить необроблені байти. Щоб змістовно
інтерпретувати ці дані, вам потрібно десеріалізувати їх у відповідний тип даних,
визначений програмою, яка володіє рахунком.
...
Функція _ts`getMint()`_ десеріалізує поле `data` рахунку в тип даних
[Mint](https://github.com/solana-program/token/blob/program%40v8.0.0/program/src/state.rs#L16-L30),
визначений Token Program.

| Поле              | Опис                                                                      |
| ----------------- | ------------------------------------------------------------------------- |
| `address`         | Адреса рахунку Mint                                                       |
...
| `isInitialized`   | Вказує, чи був ініціалізований рахунок Mint                               |
| `freezeAuthority` | Авторитет, якому дозволено заморожувати token account                     |
