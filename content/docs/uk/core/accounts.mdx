...
Для простоти, ви можете розглядати program account як саму програму. Коли ви
викликаєте інструкції програми, ви вказуєте адресу program account (зазвичай
називається "Program ID").

Наступний приклад отримує Token Program account, щоб показати, що program
account мають той самий базовий `Account` тип, за винятком того, що поле
`executable` встановлено на `true`. Оскільки program account містять виконуваний
код у своєму полі даних, ми не десеріалізуємо дані.
...
Program account належать [Loader Program](/docs/core/programs#loader-programs).
Існує кілька версій завантажувача, але всі, крім loader-v3, зберігають
виконуваний код безпосередньо в program account. Loader-v3 зберігає виконуваний
код в окремому "program data account", а program account просто вказує на нього.
Коли ви розгортаєте нову програму, Solana CLI за замовчуванням використовує
...
Loader-v3 має спеціальний тип облікового запису для тимчасового розміщення
завантаження програми під час розгортання або оновлення. У loader-v4 все ще є
буфери, але вони є просто звичайними program account.
...
Не плутайте ці program data account з data account програм (див. нижче).

## Data Account

У Solana виконуваний код програми зберігається в іншому обліковому записі, ніж
стан програми. Це схоже на те, як операційні системи зазвичай мають окремі файли
...
Зауважте, що лише [System Program](/docs/core/accounts#system-program) може
створювати нові рахунки. Після того, як System Program створює рахунок, він може
передати право власності на новий рахунок іншій програмі.
...
1. Викликати System Program для створення рахунку, а потім передати право
   власності користувацькій програмі
2. Викликати користувацьку програму, яка тепер володіє рахунком, щоб
   ініціалізувати дані рахунку, як визначено в інструкції програми

Цей процес створення рахунку часто абстрагується як один крок, але корисно
розуміти основний процес.

Наступний приклад показує, як створити та отримати рахунок Token Mint, яким
