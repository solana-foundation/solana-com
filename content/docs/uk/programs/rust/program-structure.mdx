...
Точка входу програми Solana — це функція, яка викликається при запуску програми.
...
(надані у вигляді байтів) у три значення та передає їх функції
...
Під час створення програми Solana ви зазвичай починаєте з визначення стану вашої
...
даних вашої програми в байти і навпаки для зберігання в полі даних облікового
запису:

- Серіалізація: Перетворення ваших типів даних у байти для зберігання в полі
  даних облікового запису
- Десеріалізація: Перетворення байтів, збережених в обліковому записі, назад у
  ваші типи даних
...
Коли клієнт викликає вашу програму, він повинен надати instruction data (у
вигляді буфера байтів), де:
...
2. Обліковий запис платника, який фінансуватиме створення нового облікового
   запису
3. System Program, який ми викликаємо для створення нового облікового запису
...
інструкцію `create_account` System Program.

У Solana нові рахунки можуть бути створені лише System Program. При створенні
рахунку ми вказуємо кількість байтів для виділення та власника програми для
нового рахунку. System Program:
...
Щоб оновити дані облікового запису, ми:
...
для використання як адреса облікового запису лічильника, який ми ініціалізуємо,
та визначте початкове значення для лічильника.
...
- Створіть дані інструкції з варіантом 0 (`InitializeCounter`) та початковим
  значенням
- Побудуйте інструкцію з ідентифікатором програми, даними інструкції та
  необхідними обліковими записами
...
Зверніть увагу, що дані instruction data для інструкції інкременту — це `[1]`,
що відповідає варіанту 1 (`IncrementCounter`). Оскільки для інструкції
інкременту немає додаткових параметрів, дані — це просто варіант інструкції.
