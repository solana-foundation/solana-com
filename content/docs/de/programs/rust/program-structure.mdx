...
description:
  ...
  Einstiegspunkten, Zustandsverwaltung, Befehlsverarbeitung und Tests.
...
- `entrypoint.rs`: Definiert den Einstiegspunkt, der eingehende Anweisungen
  weiterleitet.
- `state.rs`: Definieren programmspezifischen Zustand (Account-Daten).
...
Füge als Nächstes den folgenden Codeausschnitt zu `Cargo.toml` hinzu. Wenn du
diese Konfiguration nicht einschließt, wird das Verzeichnis `target/deploy` beim
...
Der Einfachheit halber verwende das Makro
[`entrypoint!`](https://github.com/solana-labs/solana/blob/v2.0/sdk/program/src/entrypoint.rs#L124-L140)
aus dem Paket `solana_program`, um den Einstiegspunkt in deinem Programm zu
definieren.
...
2. Definiert den Programm-Einstiegspunkt mit dem Makro `entrypoint!`
3. Implementiert die Funktion `process_instruction`, die Anweisungen an die
...
Das Makro `entrypoint!` erfordert eine Funktion mit der folgenden
...
(die als Bytes bereitgestellt werden) in drei Werte und übergibt sie an die
Funktion
...
- `program_id`: Der öffentliche Schlüssel des aufgerufenen Programms (aktuelles
  Programm)
- `accounts`: Das `AccountInfo` für Konten, die von der aufgerufenen Anweisung
  benötigt werden
...
Der Programm-Status wird mit Rust-Structs definiert, die das Datenlayout der
Konten deines Programms repräsentieren. Du kannst mehrere Structs definieren, um
verschiedene Arten von Konten für dein Programm darzustellen.
...
Obwohl du für die Solana-Programmentwicklung jedes Serialisierungsformat
verwenden kannst, wird häufig [Borsh](https://borsh.io/) verwendet. Um Borsh in
deinem Solana-Programm zu nutzen:

1. Füge die Crate `borsh` als Abhängigkeit zu deiner `Cargo.toml` hinzu:
...
2. Importiere die Borsh-Traits und verwende das Derive-Makro, um die Traits für
   deine Structs zu implementieren:
...
Füge das Struct `CounterAccount` zu `lib.rs` hinzu, um den Programm-Status zu
definieren. Dieses Struct wird sowohl in der Initialisierungs- als auch in der
Inkrement-Anweisung verwendet.
...
Hier ist ein Beispiel eines Enums, das zwei Anweisungen definiert:
...
1. Erstellt und reserviert Speicherplatz für ein neues Konto zur Speicherung der
   Counter-Daten
...
einen Iterator über den `accounts` Slice und verwenden die `next_account_info`
...
sie im Programm für die Anweisung definiert sind, damit diese erfolgreich
ausgeführt werden kann.

Obwohl die Variablennamen für die Konten keinen Einfluss auf die Funktionalität
...
2. Das erforderliche Mindestguthaben für "rent" berechnen. Auf Solana müssen
...
eine Cross Program Invocation (CPI) über die Funktion `invoke` durch. Eine CPI
...
1. Erstellen einer neuen `CounterAccount`Struktur mit dem `initial_value`, der
...
Als Nächstes fügen wir die Implementierung der Funktion
`process_increment_counter` hinzu. Diese Anweisung erhöht den Wert eines
bestehenden Counter-Kontos.

<Accordions>
<Accordion title="Erklärung">

Genau wie bei der Funktion `process_initialize_counter` beginnen wir mit der
...
Beachte, dass ein Entwickler in der Praxis verschiedene Sicherheitsprüfungen
implementieren muss, um die an das Programm übergebenen Konten zu validieren. Da
alle Konten vom Aufrufer der Anweisung bereitgestellt werden, gibt es keine
Garantie, dass die bereitgestellten Konten diejenigen sind, die das Programm
erwartet. Fehlende Kontovalidierungsprüfungen sind eine häufige Quelle für
Programmvulnerabilitäten.
...
Um die Programmanweisungen zu testen, fügen Sie die folgenden Abhängigkeiten zu
`Cargo.toml` hinzu.
...
Fügen Sie dann das folgende Testmodul zu `lib.rs` hinzu und führen Sie
...
Zuerst richten wir das Testmodul ein und importieren die erforderlichen
Abhängigkeiten:
...
initialisieren werden, und definieren einen Anfangswert für den Counter.
...
angegeben werden, das Folgendes spezifiziert:
...
Um die Initialize-Anweisung zu testen:
...
- Senden Sie eine Transaktion mit der Initialize-Anweisung
...
Um die Increment-Anweisung zu testen:

- Bauen Sie die Anweisung mit der Programm-ID, den instruction data und den
  erforderlichen Konten
- Senden Sie eine Transaktion mit der Increment-Anweisung
- Überprüfen Sie, ob das Konto auf den richtigen Wert erhöht wurde

Beachten Sie, dass die instruction data für die Increment-Anweisung `[1]` ist,
was der Variante 1 (`IncrementCounter`) entspricht. Da es keine zusätzlichen
Parameter für die Increment-Anweisung gibt, sind die Daten einfach die
Anweisungsvariante.
