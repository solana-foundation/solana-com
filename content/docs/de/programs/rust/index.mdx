...
Erstelle zunächst ein neues Rust-Projekt mit dem Standard `cargo new` Befehl und
dem `--lib` Flag.
...
<Callout type="warn">
  Aktualisiere das Feld `edition` in `Cargo.toml` auf `2021`. Andernfalls
  könntest du beim Erstellen des Programms auf einen Fehler stoßen.
</Callout>
...
<Callout type="warn">
  Wenn du diese Konfiguration nicht einbeziehst, wird das Verzeichnis
...
Teste als Nächstes das Programm mit dem `litesvm` Crate. Füge die folgenden
...
Führe den Test mit dem Befehl `cargo test` aus. Das Programmprotokoll wird
...
## !!steps Aktualisiere das Programm
...
## !!steps Stelle das Programm erneut bereit

Stelle das Programm mit demselben Befehl `solana program deploy` erneut bereit.
...
## !!steps Schließe das Programm

Du kannst dein Solana-Programm schließen, um das SOL zurückzugewinnen, das dem
...
Wenn Sie ein Programm mit demselben Quellcode nach dem Schließen erneut
bereitstellen müssen, müssen Sie eine neue Programm-ID generieren. Um ein neues
Keypair für das Programm zu generieren, führen Sie den folgenden Befehl aus:
