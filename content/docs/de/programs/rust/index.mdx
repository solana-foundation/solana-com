...
description:
  ...
  Schritt-für-Schritt-Anleitungen zum Erstellen, Bauen, Testen und Bereitstellen
...
Solana-Programmen in Rust ohne Verwendung des Anchor Frameworks, ein Ansatz, der
oft als Schreiben von "nativem Rust"-Programmen bezeichnet wird.
...
diese Konfiguration nicht einfügst, wird das Verzeichnis `target/deploy` beim
...
Teste als Nächstes das Programm mit der Crate `solana-program-test`. Füge die
...
Füge den folgenden Test zu `src/lib.rs` unterhalb des Programmcodes hinzu. Dies
...
Führe den Test mit dem Befehl `cargo test-sbf` aus. Das Programmprotokoll wird
...
Sie können Ihr Solana-Programm schließen, um das dem Konto zugewiesene SOL
