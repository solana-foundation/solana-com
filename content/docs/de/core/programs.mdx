...
bereitgestellt, die die kompilierte ausführbare Binärdatei des Programms
...
Um mehr über das Bereitstellen und Aktualisieren von Programmen zu erfahren,
...
werden durch ein Konto, das als "Upgrade-Autorität" bezeichnet wird, was
...
widerrufen und auf "null" gesetzt wird, wird das Programm unveränderlich und
...
Die Solana-Entwicklergemeinschaft hat Tools für verifizierbare Builds
eingeführt, die es sowohl Entwicklern als auch Nutzern ermöglichen zu
...
Programm-Binary und wird on-chain in einem ausführbaren Konto gespeichert, wenn
das Programm bereitgestellt wird.
...
| Loader | Program ID                                    | Hinweise                                                                       | Instructions Link                                                                                                                               |
| ------ | --------------------------------------------- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| native | `NativeLoader1111111111111111111111111111111` | Besitzt die anderen vier Loader                                                | —                                                                                                                                               |
| v1     | `BPFLoader1111111111111111111111111111111111` | Management-Anweisungen sind deaktiviert, aber Programme werden noch ausgeführt | —                                                                                                                                               |
| v2     | `BPFLoader2111111111111111111111111111111111` | Management-Anweisungen sind deaktiviert, aber Programme werden noch ausgeführt | [Instructions](https://docs.rs/solana-loader-v2-interface/latest/solana_loader_v2_interface/enum.LoaderInstruction.html)                        |
| v3     | `BPFLoaderUpgradeab1e11111111111111111111111` | Wird schrittweise eingestellt                                                  | [Instructions](https://docs.rs/solana-loader-v3-interface/latest/solana_loader_v3_interface/instruction/enum.UpgradeableLoaderInstruction.html) |
| v4     | `LoaderV411111111111111111111111111111111111` | v4 wird voraussichtlich der Standard-Loader werden                             | [Instructions](https://docs.rs/solana-loader-v4-interface/latest/solana_loader_v4_interface/instruction/enum.LoaderV4Instruction.html)          |

Diese Loader sind notwendig, um benutzerdefinierte Programme zu erstellen und zu
verwalten:

- Ein neues Programm oder einen Buffer bereitstellen
- Ein Programm oder einen Buffer schließen
- Ein bestehendes Programm neu bereitstellen / aktualisieren
- Die Autorität über ein Programm übertragen
- Ein Programm finalisieren
...
Anzahl der folgenden in den instruction data serialisierten Struktur enthält:
...
unbeabsichtigte Signatur- Manipulierbarkeit zu vermeiden.
...
| Programm                         | Programm-ID                                   | Beschreibung                                                                                                                                                                                                                                                                                                                                                                                                      | Anweisungen                                                                                                                                           |
| -------------------------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| **System Program**               | `11111111111111111111111111111111`            | Erstellt neue Konten, weist Kontodaten zu, ordnet Konten besitzenden Programmen zu, überträgt lamports von System-Program-eigenen Konten und bezahlt Transaktionsgebühren.                                                                                                                                                                                                                                        | [SystemInstruction](https://docs.rs/solana-program/latest/solana_program/system_instruction/enum.SystemInstruction.html)                              |
| **Vote Program**                 | `Vote111111111111111111111111111111111111111` | Erstellt und verwaltet Konten, die den Abstimmungsstatus und die Belohnungen des validators verfolgen.                                                                                                                                                                                                                                                                                                            | [VoteInstruction](https://docs.rs/solana-vote-program/latest/solana_vote_program/vote_instruction/enum.VoteInstruction.html)                          |
| **Stake Program**                | `Stake11111111111111111111111111111111111111` | Erstellt und verwaltet Konten, die Einsätze und Belohnungen für Delegationen an validators darstellen.                                                                                                                                                                                                                                                                                                            | [StakeInstruction](https://docs.rs/solana-sdk/latest/solana_sdk/stake/instruction/enum.StakeInstruction.html)                                         |
| **Config Program**               | `Config1111111111111111111111111111111111111` | Fügt Konfigurationsdaten zur Chain hinzu, gefolgt von der Liste der öffentlichen Schlüssel, die zu deren Änderung berechtigt sind. Im Gegensatz zu anderen Programmen definiert das Config-Programm keine einzelnen Anweisungen. Es hat nur eine implizite Anweisung: "store". Seine Anweisungsdaten sind eine Reihe von Schlüsseln, die den Zugriff auf das Konto regeln, sowie die darin zu speichernden Daten. | [ConfigInstruction](https://docs.rs/solana-config-program/latest/solana_config_program/config_instruction/index.html)                                 |
