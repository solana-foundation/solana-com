...
Uploads eines Programms während der Bereitstellung oder
Neubereitstellung/Upgrades. In loader-v4 gibt es noch Puffer, allerdings sind
sie nur normale Programmkonten.
...
Diese Programdata-Konten sind nicht mit den Datenkonten von Programmen zu
verwechseln (siehe unten).
...
kann es die Eigentümerschaft des neuen Kontos an ein anderes Programm
übertragen/zuweisen.

Mit anderen Worten, die Erstellung eines Datenkontos für ein benutzerdefiniertes
Programm erfordert zwei Schritte:

1. Das System-Programm aufrufen, um ein Konto zu erstellen, das dann die
   Eigentümerschaft an das benutzerdefinierte Programm überträgt
2. Das benutzerdefinierte Programm aufrufen, das jetzt Eigentümer des Kontos
   ist, um die Kontodaten gemäß der vom Programm definierten Anweisung zu
   initialisieren

Dieser Prozess der Kontoerstellung wird oft als ein einzelner Schritt
abstrahiert, aber es ist hilfreich, den zugrundeliegenden Prozess zu verstehen.
