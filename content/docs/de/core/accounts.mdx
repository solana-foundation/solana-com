...
- `owner`: Die Programm-ID (öffentlicher Schlüssel) des Programms, das dieses
  Konto besitzt. Nur das Besitzerprogramm kann die Kontodaten ändern oder das
...
- `rent_epoch`: Ein Legacy-Feld aus der Zeit, als Solana einen Mechanismus
  ...
  Account-Typ existiert, wird es nicht mehr verwendet, da die rent-Erhebung
  eingestellt wurde.
...
- [Übertragung / Zuweisung von Programm-Eigentümerschaft](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L113):
  ...
  Programm-Eigentümer einem anderen program account zuweisen. So übernehmen
...
Wallet gehört. Nur Konten, die dem System-Programm gehören, können Fee bezahlen.
...
Adresse des Programm-Kontos an (üblicherweise als "Program ID" bezeichnet).
...
separaten "Programm-Datenkonto" und das Programm-Konto verweist nur darauf. Wenn
...
Programm- Konto enthält nur die Adresse eines Programm-Datenkontos, das den
tatsächlichen ausführbaren Code speichert:

![Programmdatenkonto](/assets/docs/core/accounts/program-account-expanded.svg)

Verwechseln Sie diese Programmdatenkonten nicht mit den Datenkonten von
Programmen (siehe unten).

## Datenkonto

Auf Solana wird der ausführbare Code eines Programms in einem anderen Konto
gespeichert als der Zustand des Programms. Dies ist vergleichbar mit der Art und
Weise, wie Betriebssysteme typischerweise separate Dateien für Programme und
ihre Daten haben.
