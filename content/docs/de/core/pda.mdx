...
description:
  ...
  deterministische Kontoadressen, die eine sichere Programmunterzeichnung
...
- **Deterministische Kontoadressen**: PDAs bieten einen Mechanismus zur
  deterministischen Erstellung einer Adresse unter Verwendung einer Kombination
  aus optionalen "seeds" (vordefinierten Eingaben) und einer spezifischen
  Programm-ID.
- **Ermöglichen der Programmunterzeichnung**: Die Solana-Laufzeitumgebung
  ermöglicht es Programmen, für PDAs zu "unterschreiben", die von der Adresse
  des Programms abgeleitet sind.
...
<Callout type="info">
  ...
  PDAs zur Unterzeichnung verwenden.
</Callout>
...
- Solana-Programme können **im Namen von PDAs unterschreiben, die von ihrer
...
aussehen, aber keine privaten Schlüssel haben. Das bedeutet, dass es nicht
möglich ist, eine gültige Signatur für die Adresse zu generieren. Die
...
der Ed25519-Kurve liegt, indem ein vordefinierter Satz von Eingaben verwendet
wird. Ein Punkt, der nicht auf der Ed25519-Kurve liegt, hat keinen gültigen
...
- **Optionale seeds**: Vordefinierte Eingaben (z.B. Zeichenketten, Zahlen,
  andere Kontoadressen) für die PDA-Ableitung.
...
Off-Curve-Adresse erzeugt, ist der "canonical bump."
...
<Callout type="warn">
  ...
  Programmanweisungen zu verwenden. Es ist bewährte Praxis, nur den kanonischen
...
als Adresse des neuen Kontos verwendet. Das Beispielprogramm verwendet das
...
Adresse des `user` und den `bump` seed, der zur Ableitung der PDA verwendet
wurde.
...
`data` und die Adresse des `user`Kontos, das in der Anweisung bereitgestellt
...
abruft, nachdem die Transaktion gesendet wurde.
