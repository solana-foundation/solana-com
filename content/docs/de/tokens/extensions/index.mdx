...
zusätzliche Anweisungen, die als Extensions bezeichnet werden. Extensions sind
optionale Funktionen, die Sie einem Token-Mint oder Token-Konto hinzufügen
können. Die Implementierung dieser Extension-Anweisungen finden Sie im Token
Extensions Program
[Quellcode](https://github.com/solana-program/token-2022/tree/main/program/src/extension).

Jede Extension fügt einen spezifischen Status hinzu, der in der Regel während
der Mint- oder Token-Kontoerstellung initialisiert wird. Bei der Initialisierung
eines der beiden Konten können Sie bestimmte Extensions gleichzeitig für
verschiedene Funktionalitäten aktivieren. Die meisten Extensions können nach der
Initialisierung eines Kontos nicht mehr hinzugefügt werden. Dies ist eine
...
für jede Extension sind in der
[Token Extensions Entwicklerdokumentation](/developers/guides/token-extensions/getting-started)
verfügbar.

<Callout type="info">
  Einige Extensions sind miteinander inkompatibel und können nicht gleichzeitig
  auf demselben Token-Mint oder Token-Konto aktiviert werden. Zum Beispiel
  können Sie die _rs`NonTransferable`_ Extension nicht mit der
  _rs`TransferFeeConfig`_ Extension kombinieren, da sie widersprüchliche
...
Extension mit einzigartiger Funktionalität.
...
Jede Extension fügt spezialisierte Funktionalität hinzu, indem sie zusätzlichen
Status zu einem Mint oder Token-Konto hinzufügt. Alle extensionsspezifischen
Statusdaten werden im
...
`tlv_data` (das den Extensions-Status enthält) entsprechend den spezifischen
Extensions- Typen, die für dieses Konto aktiviert sind, weiter deserialisieren.
