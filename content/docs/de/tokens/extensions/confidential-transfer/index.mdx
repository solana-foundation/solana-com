...
datenschutzfreundliche Transaktionen. Nur die Überweisungsbeträge und
...
Der grundlegende Ablauf von vertraulichen Token-Übertragungen ist wie folgt:

1. Erstellen Sie ein mint account mit der Confidential Transfer-Erweiterung.
2. Erstellen Sie token accounts mit Confidential Transfer-Erweiterung für Sender
   und Empfänger.
3. Prägen Sie Token auf das Senderkonto.
...
5. **Anwenden** des ausstehenden Guthabens des Senders auf das **vertrauliche
   verfügbare Guthaben**.
6. Vertrauliche **Übertragung** von Token vom Token-Konto des Senders zum
...
Weitere Details zu den Schritten im vertraulichen Übertragungsablauf finden Sie
...
Übersichtlichkeit in Hilfsfunktionen aufgeteilt.
...
<Callout type="info">
  ...
  Standard-Lokalvalidator nicht aktiviert. Sie müssen das Token Extension
...
Die vollständige Liste der Confidential Transfer Extension
[Anweisungen](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
sind wie folgt:

| Anweisung                           | Beschreibung                                                                                                                                                                                      |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Richtet mint account für vertrauliche Transfers ein. Diese Anweisung muss in derselben Transaktion wie die _rs`TokenInstruction::InitializeMint`_ Anweisung enthalten sein.                       |
| _rs`UpdateMint`_                    | Aktualisiert die Einstellungen für vertrauliche Transfers für einen mint.                                                                                                                         |
...
| _rs`ConfigureAccountWithRegistry`_  | Alternative Möglichkeit, token accounts für vertrauliche Transfers zu konfigurieren, indem ein _rs`ElGamalRegistry`_ account anstelle eines _rs`VerifyPubkeyValidity`_ Nachweises verwendet wird. |
