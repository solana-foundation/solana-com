...
Vertrauliche Transfers ermöglichen es Ihnen, Token zwischen Token-Konten zu
übertragen, ohne den Übertragungsbetrag preiszugeben. Dies ist nützlich für
datenschutzfördernde Transaktionen. Nur die Überweisungsbeträge und
Token-Guthaben sind privat. Die Token-Kontoadressen bleiben öffentlich.
...
- [Confidential Balances Cookbook](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Code-Snippets zur Verwendung der Vertraulicher-Transfer-Erweiterung

### Wie funktioniert es?

Die Vertraulicher-Transfer-Erweiterung fügt
...
Konten zu übertragen, ohne den Übertragungsbetrag preiszugeben.
...
1. Erstellen Sie ein mint account mit der Vertraulicher-Transfer-Erweiterung.
2. Erstellen Sie token account mit Vertraulicher-Transfer-Erweiterung für Sender
   und Empfänger.
3. Prägen Sie Token auf das Sender-Konto.
...
5. **Anwenden** des ausstehenden Guthabens des Senders auf das **vertrauliche
   verfügbare Guthaben**.
6. Vertraulich **übertragen** von Token vom Sender-Token-Konto zum
   Empfänger-Token-Konto.
...
Weitere Details zu den Schritten im vertraulichen Transferablauf finden Sie auf
...
<Cards>
  ...
  >
    Wie man ein mint account mit der Vertraulicher-Transfer-Erweiterung erstellt
  </Card>
  <Card
    title="Token-Konto erstellen"
    href="/docs/tokens/extensions/confidential-transfer/create-token-account"
  >
    Wie man ein token account mit der Vertraulicher-Transfer-Erweiterung
    konfiguriert
  </Card>
  <Card
...
Das folgende Rust-Beispiel zeigt die Schritte für vertrauliche Token-Transfers
entsprechend dem vorhergehenden Diagramm. Die Schritte sind zur besseren
Übersichtlichkeit in Hilfsfunktionen unterteilt.
...
<Callout type="info">
  Zum Zeitpunkt der Erstellung sind die vertraulichen Transfers im
...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Richtet ein Mint-Konto für vertrauliche Transfers ein. Diese Anweisung muss in derselben Transaktion wie die _rs`TokenInstruction::InitializeMint`_ Anweisung enthalten sein.               |
| _rs`UpdateMint`_                    | Aktualisiert die Einstellungen für vertrauliche Transfers für eine Mint.                                                                                                                    |
| _rs`ConfigureAccount`_              | Richtet ein Token-Konto für vertrauliche Transfers ein.                                                                                                                                     |
| _rs`ApproveAccount`_                | Genehmigt ein Token-Konto für vertrauliche Transfers, wenn die Mint eine Genehmigung für neue Token-Konten erfordert.                                                                       |
...
| _rs`Deposit`_                       | Konvertiert öffentliches Token-Guthaben in ausstehende vertrauliche Guthaben.                                                                                                               |
...
| _rs`ApplyPendingBalance`_           | Konvertiert ausstehende Guthaben in verfügbare Guthaben nach Einzahlungen oder Überweisungen.                                                                                               |
| _rs`EnableConfidentialCredits`_     | Ermöglicht einem Token-Konto, vertrauliche Token-Transfers zu empfangen.                                                                                                                    |
| _rs`DisableConfidentialCredits`_    | Blockiert eingehende vertrauliche Transfers, während öffentliche Transfers weiterhin möglich sind.                                                                                          |
| _rs`EnableNonConfidentialCredits`_  | Ermöglicht einem Token-Konto, öffentliche Token-Transfers zu empfangen.                                                                                                                     |
| _rs`DisableNonConfidentialCredits`_ | Blockiert reguläre Transfers, damit das Konto nur vertrauliche Transfers empfangen kann.                                                                                                    |
| _rs`TransferWithFee`_               | Überträgt Token vertraulich zwischen Token-Konten mit einer Fee.                                                                                                                            |
| _rs`ConfigureAccountWithRegistry`_  | Alternative Möglichkeit, Token-Konten für vertrauliche Transfers zu konfigurieren, indem ein _rs`ElGamalRegistry`_ Konto anstelle eines _rs`VerifyPubkeyValidity`_ Beweises verwendet wird. |
