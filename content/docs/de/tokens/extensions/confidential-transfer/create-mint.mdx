...
indem sie zusätzliche Daten zum mint account hinzufügt. Dieser Abschnitt
...
- **authority**: Das Konto, das die Berechtigung hat, Einstellungen für
  vertrauliche Übertragungen für den Mint zu ändern und neue vertrauliche Konten
...
- **auto_approve_new_accounts**: Wenn auf true gesetzt, können Benutzer
  standardmäßig token accounts mit aktivierten vertraulichen Übertragungen
  erstellen. Bei false muss die Authority jedes neue token account genehmigen,
  bevor es für vertrauliche Übertragungen verwendet werden kann.
...
- **auditor_elgamal_pubkey**: Ein optionaler Prüfer, der Überweisungsbeträge in
  ...
  Öffentlichkeit gewahrt bleibt.

### Erforderliche Anweisungen

Die Erstellung eines Mints mit aktivierter Confidential Transfer erfordert drei
Anweisungen in einer einzigen Transaktion:

1. **Erstellen des Mint-Kontos**: Rufen Sie die _rs`CreateAccount`_ Anweisung
   des System-Programms auf, um das mint account zu erstellen.
...
2. **Initialisieren der Confidential Transfer-Erweiterung**: Rufen Sie die
   ...
   _rs`ConfidentialTransferMint`_ Status für den Mint zu konfigurieren.

3. **Initialisieren des Mints**: Rufen Sie die _rs`Instruction::InitializeMint`_
   Anweisung des Token Extensions Program auf, um den Standard-Mint- Status zu
   initialisieren.
...
<Callout type="info">
  Zum Zeitpunkt der Erstellung sind Confidential Transfers im Standard-Validator
  nicht aktiviert. Sie müssen das Token Extensions Program von Mainnet klonen,
  um den Beispielcode auszuführen.
</Callout>
