...
2. Für jeden Nachweis:
   ...
   - Speichern Sie die nachweisspezifischen Metadaten in einem
     Nachweis-"Kontext-Status"-Konto, um sie in anderen Anweisungen zu
     verwenden.
...
3. Rufen Sie die
   ...
   Anweisung auf und geben Sie die Nachweis-Kontext-Status-Konten an.

4. Schließen Sie die Nachweis-Kontext-Status-Konten, um das SOL
   zurückzugewinnen, das für ihre Erstellung verwendet wurde.

Das folgende Diagramm zeigt die Schritte, die bei der Übertragung von Token von
einem Token-Konto des Senders auf ein Token-Konto des Empfängers beteiligt sind.
...
Das folgende Beispiel zeigt, wie man vertraulich Token zwischen token accounts
überträgt.
...
<Callout type="info">
  Zum Zeitpunkt des Schreibens sind die Confidential Transfers auf dem
  Standard-Lokalvalidator nicht aktiviert. Du musst das Token Extensions Program
  von Mainnet klonen, um den Beispielcode auszuführen.
</Callout>
