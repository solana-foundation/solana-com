...
description:
  Erfahren Sie, wie Sie Token privat von einem Token-Konto auf ein anderes
  übertragen können.
---
...
sowohl der Sender als auch der Empfänger Token-Konten haben, die mit dem
_rs`ConfidentialTransferAccount`_ Status konfiguriert und für vertrauliche
Überweisungen freigegeben sind. Das Token-Konto des Senders muss außerdem über
ein verfügbares vertrauliches Guthaben verfügen, das übertragen werden kann.
...
1. Erstellen Sie
   ...
   **Gleichheitsnachweis
   ...
   Überprüft, ob der neue verfügbare Guthaben-Chiffretext nach der Übertragung
   ...
   **Chiffretext-Gültigkeitsnachweis
   ...
   Überprüft, ob die Überweisungsbetrags-Chiffretexte für alle drei Parteien
   (Quelle, Ziel und Prüfer) ordnungsgemäß generiert wurden, und stellt sicher,
   ...
   **Bereichsnachweis
   ...
   Überprüft, ob das neue verfügbare Guthaben und der Überweisungsbetrag
...
2. Für jeden Nachweis:
   ...
   - Speichern Sie die nachweisspezifischen Metadaten in einem
     Nachweis-"Kontextstatus"-Konto zur Verwendung in anderen Anweisungen.
...
3. Rufen Sie die
   ...
   Anweisung auf und stellen Sie die Nachweis-Kontextstatus-Konten bereit.

4. Schließen Sie die Nachweis-Kontextstatus-Konten, um das SOL zurückzugewinnen,
   das für ihre Erstellung verwendet wurde.

Das folgende Diagramm zeigt die Schritte, die bei der Übertragung von Token von
einem Token-Konto eines Senders auf ein Token-Konto eines Empfängers beteiligt
sind.
...
- Generiere einen Gleichheitsbeweis, Chiffretext-Gültigkeitsbeweis und
  Bereichsbeweis clientseitig
- Rufe das Zk ElGamal-Beweisprogramm auf, um die Beweise zu verifizieren und die
  "Kontext-Status"-Konten zu initialisieren
- Rufe die
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  Anweisung auf und stelle die drei Beweis-Konten bereit.
- Schließe die drei Beweis-Konten, um die Rent zurückzugewinnen.
...
- `confidential_transfer_transfer` Methode, die die `Transfer` Anweisung
  aufruft.
- `confidential_transfer_close_context_state_account` Methode, die ein Beweis-
  Konto schließt.
...
Um das Beispiel auszuführen, starte einen lokalen Validator mit dem Token
Extensions Program, das von Mainnet geklont wurde, mit dem folgenden Befehl. Du
musst die Solana CLI installiert haben, um den lokalen Validator zu starten.
...
<Callout type="info">
  Zum Zeitpunkt des Schreibens sind die vertraulichen Übertragungen auf dem
  Standard-lokalen Validator nicht aktiviert. Du musst das Token Extensions
