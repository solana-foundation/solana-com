...
description:
  Erfahren Sie, wie Sie ausstehende Guthaben anwenden können, um Gelder
  verfügbar zu machen.
---

## Wie man ausstehenden Saldo auf verfügbaren Saldo anwendet

Bevor Tokens vertraulich übertragen werden können, muss das öffentliche
Token-Guthaben in ein vertrauliches Guthaben umgewandelt werden. Diese
Umwandlung erfolgt in zwei Stufen:

1. **Vertraulicher ausstehender Saldo**: Zunächst werden Tokens vom öffentlichen
...
2. **Vertraulicher verfügbarer Saldo**: Der ausstehende Saldo wird dann auf den
   verfügbaren Saldo "angewendet", wodurch die Tokens für vertrauliche
...
Wenn Tokens vom öffentlichen Guthaben "eingezahlt" werden oder wenn Tokens
vertraulich von einem token account auf einen anderen übertragen werden, werden
die Tokens zunächst dem vertraulichen ausstehenden Saldo hinzugefügt. Bevor
Tokens für vertrauliche Überweisungen verwendet werden können, muss der
ausstehende Saldo auf den verfügbaren Saldo "angewendet" werden.
...
der `ApplyPendingBalance` Anweisung erstellt und sendet, wie im folgenden
Beispiel gezeigt.
