...
Das Übertragen von Token beinhaltet das Verschieben von Token von einem
Token-Konto zu einem anderen Token-Konto, die denselben Mint teilen. Die
Übertragung von Token erfolgt, wenn Sie die
...
Anweisung in einem Token-Programm aufrufen. Nur die als Eigentümer (Authority)
des Quell-Token-Kontos angegebene Adresse kann Token aus dem Konto übertragen.
