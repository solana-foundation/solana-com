...
1. Der "Besitzer" des token accounts - Dies ist eine Adresse, die in den Daten
   des token accounts als "owner"-Feld des
   ...
   manchmal als "Autorität" des token accounts bezeichnet, um sie vom
   Programm-Besitzer zu unterscheiden.

2. Der Programm "owner" - Dies bezieht sich auf das Programm, das die
   Kontendaten auf Solana besitzt. Für token accounts ist dies immer entweder
   das Token Program oder Token Extensions Program, wie im "owner"-Feld des
   Basis-Solana
...
Program Derived Address (PDA) ist, erstellt vom
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program).
Man kann sich einen associated token account als das Standard-Token-Konto für
...
jeden gegebenen Token Mint zu finden und eliminiert die Notwendigkeit,
...
<Callout type="info">
  ...
  Program durch. Das Token- Programm besitzt das erstellte Konto, das die
  gleiche `Account` Typstruktur hat, wie im Token-Programm definiert. Das
  Associated Token Program behält keinen Zustand bei - es bietet lediglich eine
  standardisierte Methode zur Erstellung von token accounts an einer
  deterministischen Adresse, die eine PDA ist.
</Callout>

## Wie man ein Token Konto erstellt

Um ein Token Konto zu erstellen, rufen Sie die
...
Die Transaktion zum Erstellen eines Token Kontos benötigt zwei Anweisungen:

1. Rufen Sie das System Program auf, um Speicherplatz für ein Token Konto zu
   erstellen und zuzuweisen und die Eigentümerschaft an das Token Program zu
   übertragen.
2. Rufen Sie das Token Program auf, um die Token Konto-Daten zu initialisieren.
...
System Program auf, um das Token Konto zu erstellen, und ruft das Token Program
auf, um die Token Konto-Daten zu initialisieren. Dies geschieht durch Cross
