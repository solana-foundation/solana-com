...
liest. Jetzt erkunden Sie, wie man Daten in das Netzwerk schreibt. Das Schreiben
in das Solana-Netzwerk beinhaltet das Senden von Transaktionen, die eine oder
...
einreichen, führt die Solana-Laufzeitumgebung jede Anweisung der Reihe nach und
atomar aus (das bedeutet, entweder alle Anweisungen sind erfolgreich oder die
gesamte Transaktion schlägt fehl).
...
[Gebühren auf Solana](/docs/core/fees).
...
Fordere ein Airdrop von SOL an, um die `sender` zu finanzieren.

### !instruction

Erstelle eine Anweisung, um die Transfer-Anweisung des System-Programms
aufzurufen.
...
Dies sind die Schritte zum Erstellen von Transaktionen, um mit jedem Programm
auf Solana zu interagieren.
...
Berechne die für einen Mint account erforderlichen lamports.
...
Erstelle eine Anweisung zum Erstellen eines neuen Kontos mit Platz für den Mint
account-Typ und im Besitz des Token Extensions Program.

### !initializeMintInstruction

Erstelle eine Anweisung zur Initialisierung der Daten des neuen Kontos als Mint
account-Typ.
...
Generiere ein Keypair für das Mint-Konto
...
2. [Übertrage Lamports](mention:two) vom Wallet, um das neue Konto zu
   finanzieren
3. [Weise den Besitz](mention:three) des Kontos dem Token Extensions Program
   (`TOKEN_2022_PROGRAM_ID`) zu
...
Erstelle eine Anweisung zur Initialisierung des Mint-Kontos
...
Sende und bestätige die Transaktion mit beiden erforderlichen Unterzeichnern
...
Gib das Mint-Konto und die Transaktionssignatur aus
