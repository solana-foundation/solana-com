...
[finalen Code](https://beta.solpg.io/668304cfcffcf4b13384d20a) nach Abschluss
beider Abschnitte, PDA und CPI.
...
- Als _rs`SystemAccount`_ Typ wird es wie reguläre Wallet- Konten vom System
  Program verwaltet
...
Der Aufbau einer CPI entspricht der Art und Weise, wie clientseitige Anweisungen
erstellt werden, wobei das Programm, die Konten und die instruction data für
eine bestimmte auszuführende Anweisung angegeben werden. Wenn die `update`
Anweisung des Programms aufgerufen wird, ruft sie intern die Transfer-Anweisung
des System Programs auf.
...
Benutzer, wenn sie ihr Nachrichtenkonto schließen.
...
Das Hinzufügen der `vault_account` zur Delete-Struktur ermöglicht es dem
Programm, während der Löschanweisung auf das Vault-Konto des Benutzers
zuzugreifen, um angesammelte SOL an den Benutzer zurückzuüberweisen.
...
die Überweisungsanweisung des System-Programms. Dieser CPI zeigt, wie eine
Überweisung durchgeführt wird, die einen Program Derived Address (PDA)
Unterzeichner erfordert.
...
Die _rs`Transfer`_ Struktur gibt die erforderlichen Konten für die
Überweisungsanweisung des System-Programms an:
...
### Programm neu bereitstellen

Nach diesen Änderungen stelle das aktualisierte Programm neu bereit. Dies stellt
sicher, dass das modifizierte Programm für Tests verfügbar wird. Bei Solana
erfordert die Aktualisierung eines Programms einfach die Bereitstellung des
Programms unter derselben Programm-ID.

<Callout>

Stelle sicher, dass dein Playground-Wallet über Devnet-SOL verfügt. Hole dir
...
Nur die Upgrade-Autorität des Programms kann es aktualisieren. Der Entwickler
legt die Upgrade-Autorität während der Programmbereitstellung fest, und sie
bleibt das einzige Konto mit der Berechtigung, das Programm zu ändern oder zu
schließen. Wenn jemand die Upgrade- Autorität widerruft, wird das Programm
unveränderlich.

Bei der Bereitstellung von Programmen auf Solana Playground fungiert dein
Playground-Wallet als Upgrade-Autorität für all deine Programme.
...
ihre Einbeziehung in die Update- und Delete-Anweisungsaufrufe.

#### PDA des Tresors ableiten

Fügen Sie zunächst die Ableitung des Tresor-PDA hinzu:
...
Aktualisieren Sie dann die Update-Anweisung, um den
...
Aktualisieren Sie dann die Delete-Anweisung, um den
[`vaultAccount`](mention:one) einzubeziehen
...
Führen Sie nach diesen Änderungen die Tests aus, um sicherzustellen, dass alles
...
Sie können dann die SolanaFM-Links untersuchen, um die Transaktionsdetails
anzuzeigen, wo Sie die CPIs für die Transfer-Anweisungen innerhalb der Update-
und Delete-Anweisungen finden werden.

![Update CPI](/assets/docs/intro/quickstart/cpi-update.png)

![Delete CPI](/assets/docs/intro/quickstart/cpi-delete.png)

Falls Sie auf Fehler stoßen, können Sie den
[finalen Code](https://beta.solpg.io/668304cfcffcf4b13384d20a) als Referenz
nutzen.
...
Herzlichen Glückwunsch zum Abschluss des Solana Quickstart-Guides. Sie haben
praktische Erfahrungen mit wichtigen Solana-Konzepten gesammelt, darunter:

- Abrufen und Lesen von Daten aus Accounts
...
Um Ihr Verständnis dieser Konzepte zu vertiefen, schauen Sie sich die
[Core Concepts](/docs/core/accounts)-Dokumentation an, die detaillierte
Erklärungen zu den in diesem Guide behandelten Themen bietet.

### Weitere Beispiele erkunden

Wenn Sie lieber anhand von Beispielen lernen, sehen Sie sich das
...
Solana Playground bietet eine praktische Funktion, mit der Sie Projekte über
ihre GitHub-Links importieren oder anzeigen können. Öffnen Sie zum Beispiel
diesen
