...
Dieser Code definiert, welche Daten im Nachrichtenkonto gespeichert werden
...
Zusammen ermöglichen diese Anchor, automatisch die an die Anweisung übergebenen
Konten zu überprüfen und das Programm zu sichern.
...
Die _rs`realloc`_ Einschränkung weist den Daten des Kontos neuen Speicherplatz
zu.
...
Der Funktionskörper gibt lediglich eine Nachricht in den Programmprotokollen
aus, indem er das _rs`msg!()`_ Makro verwendet. Die Funktion benötigt keine
...
Deployment des Programms zu bezahlen. Hol dir Devnet SOL vom
...
Die _ts`fetchNullable()`_ Methode ruft Kontendaten vom Netzwerk ab, die
