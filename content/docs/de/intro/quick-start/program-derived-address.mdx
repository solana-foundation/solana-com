...
description:
  Lernen Sie, wie man ein CRUD-Programm (Create, Read, Update, Delete) für
  Solana mit Program Derived Addresses (PDAs) und dem Anchor-Framework erstellt.
  Diese Schritt-für-Schritt-Anleitung zeigt, wie man On-Chain-Nachrichtenkonten
  mit PDAs erstellt, aktualisiert und löscht, Kontovalidierung implementiert und
  Tests schreibt. Ideal für Entwickler, die verstehen möchten, wie PDAs in
  Solana-Programmen verwendet werden.
...
[fertigen Code](https://beta.solpg.io/668304cfcffcf4b13384d20a) nach Abschluss
beider Abschnitte zu PDA und Cross-Program Invocation (CPI) einsehen.
...
die Sie in den folgenden Schritten hinzufügen werden.
...
Derived Address (PDA) verwendet wird. Das Speichern dieses Wertes spart
...
- `bump` - _rs`u8`_, der den ["Bump"-Seed](/docs/core/pda#canonical-bump) für
  ...
  dieses Wertes spart Rechenleistung, indem die Notwendigkeit einer
  Neuberechnung in späteren Anweisungen entfällt.
...
- _rs`b"message"`_ - Ein fester String als erstes seed.
- _rs`user.key().as_ref()`_ - Der öffentliche Schlüssel des _rs`user`_ Kontos
  als zweites seed.

Die `bump` Einschränkung weist Anchor an, automatisch den richtigen bump seed zu
...
Die _rs`realloc`_ Einschränkung weist den Datenspeicher des Kontos neu zu.
...
1. _rs`user: Signer<'info>`_
   ...
   - Als veränderbar markiert (_rs`#[account(mut)]`_), da es möglicherweise für
     mehr Speicherplatz für das `message_account` bei Bedarf bezahlt
...
In diesem Fall verwendet das Programm den vorhandenen bump seed, der im
...
In diesem Fall erhält das Konto _rs`user`_ die lamports vom geschlossenen
...
Die Struktur _rs`Delete`_ definiert die Konten, die für die Anweisung
_rs`delete`_ erforderlich sind:
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - `mut` Einschränkung zeigt Datenmodifikation an
   - `seeds` und `bump` Einschränkungen verifizieren das Konto als korrektes PDA
...
Du hast jetzt das grundlegende CRUD-Programm fertiggestellt. Stelle das Programm
bereit, indem du `deploy` im Playground-Terminal ausführst.

<Callout type="info">
In diesem Beispiel wirst du das Programm im Devnet bereitstellen, einem Solana-Cluster
für Entwicklungstests.

Das Playground-Wallet verbindet sich standardmäßig mit dem Devnet. Stelle
sicher, dass dein Playground- Wallet über Devnet SOL verfügt, um für die
Programmbereitstellung zu bezahlen. Hol dir Devnet SOL vom
...
Füge den folgenden Code innerhalb von _ts`describe()`_ ein, aber vor den
_ts`it()`_ Abschnitten.
