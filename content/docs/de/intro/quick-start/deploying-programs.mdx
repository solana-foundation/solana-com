...
description:
  Erfahre, wie du dein erstes Solana-Programm mit dem Anchor-Framework und
  Solana Playground erstellst, bereitstellst und testest. Diese
...
h1: Dein erstes Solana-Programm bereitstellen
---

In diesem Abschnitt wirst du ein einfaches Solana-Programm (Smart Contract) mit
dem Anchor-Framework erstellen, bereitstellen und testen. Am Ende hast du dein
erstes Programm auf der Solana-Blockchain bereitgestellt.

Der Zweck dieses Abschnitts liegt darin, dich mit dem Solana Playground vertraut
zu machen. Die Anleitung führt durch ein detaillierteres Beispiel in den
Abschnitten Program Derived Address (PDA) und Cross Program Invocation (CPI).
Weitere Details findest du auf der Seite
...
Öffne zunächst https://beta.solpg.io in einem neuen Browser-Tab.

- Klicke auf die Schaltfläche "Create a new project" im linken Seitenbereich.

- Gib einen Projektnamen ein, wähle Anchor als Framework und klicke dann auf die
  Schaltfläche "Create".

![Neues Projekt](/assets/docs/intro/quickstart/pg-new-project.gif)

Du siehst ein neues Projekt, das mit dem Programmcode in der Datei `src/lib.rs`
erstellt wurde.
...
Das _rs`declare_id!()`_-Makro gibt die On-Chain-Adresse deines Programms an.
Solana Playground aktualisiert diese Adresse, wenn du das Projekt baust.
...
Hintergrund ruft diese Einschränkung das Systemprogramm auf, um ein neues Konto
...
- Das declare_id! Makro spezifiziert die On-Chain-Adresse deines Programms.
  Solana Playground aktualisiert diese Adresse automatisch, wenn du das Programm
  im nächsten Schritt baust.
...
- Das [`#[derive(Accounts)]`](mention:one) Attribut definiert eine Struktur, die
  ...
  der Kontovalidierung durch Feldtypen (z.B. `Signer<'info>`) und
...
Programm im Netzwerk zu deployen (standardmäßig auf devnet). Die
Programm-Bereitstellung erfordert die Zuweisung von SOL zum On-Chain-Konto, das
...
Nach dem Deployen des Programms kannst du dessen Anweisungen aufrufen.
...
Um die Testdatei nach dem Deployen des Programms auszuführen, führe
...
Achte auf die Ausgabe, die bestätigt, dass der Test erfolgreich bestanden wurde.
...
Nur die Upgrade-Autorität des Programms kann es schließen. Der
Bereitstellungsprozess legt die Upgrade-Autorität fest, wenn Sie das Programm
bereitstellen. Dieses Konto hat die exklusive Berechtigung, das Programm zu
aktualisieren oder zu schließen. Wenn jemand die Upgrade-Autorität widerruft,
wird das Programm unveränderlich, ohne jegliche Möglichkeit zum Schließen oder
Aktualisieren.

Wenn Sie Programme auf Solana Playground bereitstellen, wird Ihre
Playground-Wallet automatisch zur Upgrade-Autorität für alle Ihre Programme.

</Accordion>
</Accordions>

Herzlichen Glückwunsch. Sie haben gerade Ihr erstes Solana-Programm mit dem
Anchor-Framework erstellt und bereitgestellt.
