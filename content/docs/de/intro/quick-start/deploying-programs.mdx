...
- Einen Parameter _rs`data: u64`_ als Eingabe erfordert
- Ein neues Konto erstellt
- Den Wert des Parameters _rs`data: u64`_ in den Daten des Kontos speichert
...
Das _rs`#[program]`_ Attribut kennzeichnet ein Modul, das Funktionen enthält,
die die Anweisungen des Programms darstellen.

### !accounts

Das _rs`#[derive(Accounts)]`_ Attribut kennzeichnet eine Struktur, die die
Konten des Programms enthält.

### !account

Das _rs`#[account]`_ Attribut definiert eine Struktur, die den Datentyp für
Konten darstellt, die dieses Programm erstellen und besitzen kann.

### !constraints

Die _rs`#[account(...)]`_ Attribute spezifizieren die Einschränkungen für das
Konto.
...
_rs`ctx.accounts.new_account.data`_ greift auf das Feld `data` des `new_account`
Kontos zu und setzt es auf den Wert des `data: u64` Parameters.

### !msg

Das _rs`msg!`_ Makro protokolliert eine Nachricht in den Programmprotokollen der
Transaktion.

### !init

Die `init` Einschränkung gibt an, dass das Programm das Konto erstellen muss.
Unter der Haube ruft diese Einschränkung das Systemprogramm auf, um ein neues
Konto zu erstellen und macht dieses Programm zum Eigentümer. Als
Programmeigentümer kann dieses Programm dann in die Kontodaten schreiben.

### !payer

Die `payer` Einschränkung gibt das Konto an, das für das neue Konto bezahlt.

### !space

Die `space` Einschränkung gibt die Anzahl der Bytes an, die für das Konto
...
1. `ctx: Context<Initialize>` - Übergibt der Funktion die für diese Anweisung
   erforderlichen Konten, wie in der Struktur `Initialize` angegeben.
...
das bereitgestellte Argument `data` und gibt dann eine Nachricht in die
...
- Das Attribut [`#[derive(Accounts)]`](mention:one) definiert eine Struktur, die
...
Um das Programm zu bauen, führe einfach `build` im Terminal aus. Der Befehl
`build` kompiliert das Programm. Der Programm-Bytecode wird dann bei der
