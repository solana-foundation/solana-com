...
description:
  Przesyłanie własnych programów na blockchain Solana za pomocą Solana CLI.
---

Ten przewodnik zakłada znajomość następujących tematów:

- [Model kont Solana](/core/accounts)
- [Programy Solana w ogólności](/core/programs)
- [Tworzenie własnych programów Solana](/programs/rust/)

## Loader-v3 i Loader-v4

Obecnie trwa przejście z loader-v3 (podkomenda program) na loader-v4 (podkomenda
program **-v4**), ponieważ loader-v3 jest wycofywany.

Do nowych wdrożeń należy używać `solana program-v4 deploy` zamiast
...
Dodatkowo każdy program posiada konto programu oraz ID programu, które jest
adresem tego konta programu. Poniższe polecenie generuje keypair dla konta
programu:
...
To należy wykonać tylko raz dla każdego programu i będzie ponownie używane przy
kolejnych wdrożeniach tego samego programu.
...
Parametr nazywa się `program-id`, mimo że oczekuje ścieżki do pliku keypair:
...
Przesłanie innego pliku wykonywalnego do tego samego konta programu ponownie
nadpisze/zastąpi istniejący plik. Jednak w przypadku ponownych wdrożeń potrzebny
jest tylko identyfikator programu (pubkey pary kluczy programu), a nie cała para
kluczy, ponieważ podpisującym jest para kluczy uprawniona do aktualizacji.
...
zwiększenie rozmiaru konta programdata:
...
- `--with-compute-unit-price`: Ustaw cenę jednostki obliczeniowej dla transakcji
  w przyrostach 0,000001 lamportów (mikrolamportów) na jednostkę obliczeniową.
  Użyj
  [Priority Fee API by Helius](https://docs.helius.dev/guides/priority-fee-api),
  aby uzyskać szacunkową opłatę priorytetową.
- `--use-rpc`: Wysyłaj transakcje zapisu do skonfigurowanego RPC zamiast do TPU
  walidatora. Ta flaga wymaga połączenia RPC z
  [ważeniem staków](/developers/guides/advanced/stake-weighted-qos), takiego jak
  [Helius](https://www.helius.dev/) lub [Triton](https://triton.one/). Tę flagę
  można również skonfigurować jako domyślną za pomocą:
...
- `--max-sign-attempts`: Maksymalna liczba prób podpisania lub ponownego
  ...
  wysłane podczas wdrażania programu pozostaną niepotwierdzone po wygaśnięciu
  pierwotnie wybranego blockhash, te transakcje zostaną ponownie podpisane nowym
  blockhash i wysłane ponownie. Użyj tego ustawienia, aby dostosować maksymalną
  liczbę iteracji podpisywania transakcji. Każdy blockhash jest ważny przez
  około 60 sekund, co oznacza, że użycie domyślnej wartości 5 spowoduje
...
pośrednie konto bufora, które zawiera niezerowe saldo. Aby odzyskać to saldo,
możesz wznowić nieudane wdrożenie, podając ten sam pośredni bufor w nowym
wywołaniu `deploy`.

W przypadku niepowodzenia wdrożenia zostanie wyświetlony komunikat o błędzie
określający frazę seed potrzebną do odzyskania wygenerowanego klucza keypair
pośredniego bufora:
...
Dla programów wdrożonych za pomocą loader-v3 można odzyskać jedynie ich konto
programdata, konta buforów oraz środki zablokowane na tych kontach. Konto
programu wraz z identyfikatorem programu i środkami zablokowanymi na koncie
programu pozostają zablokowane.

Programy wdrożone za pomocą loader-v4 mogą zostać zamknięte, a ich konto
programu, identyfikator programu i zablokowane środki ponownie stają się
dostępne do innych zastosowań.

### Loader-v3

Jest to **nieodwracalna** operacja dla programów wdrożonych za pomocą loader-v3.

Pamiętaj, że po zamknięciu programu jego identyfikator programu nie może być
ponownie użyty. Próba wdrożenia programu z wcześniej zamkniętym identyfikatorem
programu zakończy się błędem. Jeśli musisz ponownie wdrożyć program po jego
zamknięciu, musisz wygenerować nowy plik keypair programu.
...
Polecenie `show` wyświetla metadane programu.
...
zawiera znany plik wykonywalny. Pobierany plik można skrócić, zahashować i
...
programu pozostaje takie samo. Ponadto jeden autorytet może kontrolować wiele
kont programów.

Jeśli nie zostanie to wyraźnie określone podczas początkowego wdrożenia,
...
Lub po wdrożeniu i określeniu obecnego autorytetu:
...
## Zaawansowane: Dwuetapowe ponowne wdrożenie z użyciem bufora

Zamiast przesyłać bezpośrednio do konta programu, plik wykonywalny można
najpierw przesłać do tymczasowego konta bufora, a następnie przenieść do konta
programu w drugim kroku (faktyczne ponowne wdrożenie). Jest to przydatne w
przypadku podpisywania offline i programów zarządzanych przez wiele podmiotów,
takich jak DAO głosujące nad wyborem lub odrzuceniem przesłanego pliku
wykonywalnego przed faktycznym ponownym wdrożeniem.
...
Najpierw należy utworzyć keypair dla konta bufora:

```shell
solana-keygen new -o ~/.config/solana/buffer-keypair.json
```

Konto bufora może być ponownie używane do różnych przesyłek i nie jest powiązane
z żadnym konkretnym kontem programu.
...
transmisji transakcji, tak aby klucze podpisujące mogły być całkowicie odłączone
od jakiejkolwiek sieci, co jest znane jako podpisywanie offline.

Należy zauważyć, że tylko ponowne wdrożenia mogą być wykonywane w trybie
...
- podpisujący online (płatnik opłat i autorytet konta bufora)
...
po prostu zdobądź nowy hash i powtarzaj, aż się uda, lub rozważ użycie trwałych
liczników transakcji.
