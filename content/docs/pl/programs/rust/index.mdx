...
- Chcą poznać podstawowe koncepcje przed przejściem do bardziej zaawansowanych
  frameworków
...
Najpierw utwórz nowy projekt w Rust za pomocą standardowego polecenia
...
Następnie zamień zawartość `src/lib.rs` na poniższy kod. Jest to minimalny
...
poniższe zależności do `Cargo.toml`.
...
Następnie wdrożymy program. Podczas pracy lokalnej możemy użyć
...
w osobnym terminalu, aby wdrożyć program do lokalnego validatora.
...
Możesz sprawdzić ID programu i podpis transakcji na
[Solana Explorer](https://explorer.solana.com/?cluster=custom&customUrl=http%3A%2F%2Flocalhost%3A8899).
Pamiętaj, że klaster na Solana Explorer musi być również localhost. Opcja
"Custom RPC URL" na Solana Explorer domyślnie wskazuje `http://localhost:8899`.
...
[Solana Explorer](https://explorer.solana.com/?cluster=custom&customUrl=http%3A%2F%2Flocalhost%3A8899)
(klaster lokalny), aby zobaczyć "Hello, world!" w logu programu.
...
Ponownie uruchom kod klienta i sprawdź podpis transakcji w Solana Explorer, aby
...
`cargo build-sbf`, co spowoduje wygenerowanie nowego pliku keypair.
...
Pamiętaj, że po zamknięciu programu jego identyfikator programu (program ID) nie
może być ponownie użyty. Próba wdrożenia programu z wcześniej zamkniętym
...
Jeśli musisz ponownie wdrożyć program z tym samym kodem źródłowym po jego
zamknięciu, musisz wygenerować nowy identyfikator programu. Aby wygenerować nowy
keypair dla programu, uruchom następujące polecenie:
