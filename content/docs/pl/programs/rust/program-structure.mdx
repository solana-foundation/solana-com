...
zazwyczaj przestrzegają wspólnego wzorca:

- `entrypoint.rs`: Definiuje punkt wejścia, który kieruje przychodzące
  instrukcje.
...
Aby zademonstrować, jak zbudować natywny program w Rust z wieloma instrukcjami,
przeprowadzimy Cię przez prosty program licznikowy, który implementuje dwie
instrukcje:
...
zostaje uruchomiony. Punkt wejścia ma następującą surową definicję, a
deweloperzy mogą stworzyć własną implementację funkcji punktu wejścia.
...
[dane wejściowe](https://github.com/solana-labs/solana/blob/v2.0/sdk/program/src/entrypoint.rs#L129-L131)
...
podczas tworzenia instrukcji do wywołania programu.
...
swojego programu – danych, które będą przechowywane na kontach utworzonych i
zarządzanych przez Twój program.

Stan programu definiuje się za pomocą struktur Rust, które reprezentują układ
danych na kontach Twojego programu. Możesz zdefiniować wiele struktur, aby
reprezentowały różne typy kont w Twoim programie.
...
Chociaż do tworzenia programów Solana można używać dowolnego formatu
serializacji, często stosowany jest [Borsh](https://borsh.io/). Aby użyć Borsh w
swoim programie Solana:
...
Kiedy klient wywołuje Twój program, musi dostarczyć dane instrukcji (jako bufor
bajtów), gdzie:
...
Aby przekonwertować dane instrukcji (bajty) na wariant enum, często implementuje
się metodę pomocniczą. Ta metoda:
...
Następnie dodaj implementację funkcji `process_initialize_counter`. Ten
...
3. Programu Systemowego, który wywołujemy, aby utworzyć nowe konto

Aby zdefiniować konta wymagane przez instrukcję, tworzymy iterator na fragmencie
`accounts` i używamy funkcji `next_account_info`, aby uzyskać każde konto.
Liczba zdefiniowanych kont to konta wymagane przez instrukcję.

Kolejność kont ma znaczenie – podczas tworzenia instrukcji po stronie klienta
...
1. Określić przestrzeń (w bajtach), którą należy przydzielić dla pola danych
   konta. Ponieważ przechowujemy wartość u64 (`count`), potrzebujemy 8 bajtów.

2. Obliczyć minimalne saldo "czynszu" wymagane. Na Solanie konta muszą
   utrzymywać minimalne saldo w lamportach (czynsz) w zależności od ilości
   danych przechowywanych na koncie.
...
Gdy przestrzeń jest zdefiniowana, a czynsz obliczony, utwórz konto, wywołując
...
3. Serializację struktury `CounterAccount` do pola danych konta, efektywnie
   zapisując `initial_value` na koncie.
...
Należy zauważyć, że w praktyce programista musi zaimplementować różne kontrole
bezpieczeństwa w celu weryfikacji kont przekazanych do programu. Ponieważ
...
Następnie dodaj poniższy moduł testowy do `lib.rs` i uruchom `cargo test-sbf`,
aby wykonać testy. Opcjonalnie użyj flagi `--nocapture`, aby zobaczyć komunikaty
...
Następnie skonfiguruj test za pomocą `ProgramTest`. Utwórz nową parę kluczy
(keypair), która będzie używana jako adres konta licznika, które
zainicjalizujemy, i zdefiniuj początkową wartość, którą ustawimy dla licznika.
...
- Utwórz dane instrukcji z wariantem 0 (`InitializeCounter`) i początkową
  wartością
- Zbuduj instrukcję z identyfikatorem programu, danymi instrukcji i wymaganymi
  kontami
- Wyślij transakcję z instrukcją inicjalizacji
- Sprawdź, czy konto zostało utworzone z poprawną początkową wartością
...
Zwróć uwagę, że dane instrukcji dla instrukcji inkrementacji to `[1]`, które
odpowiadają wariantowi 1 (`IncrementCounter`). Ponieważ nie ma dodatkowych
parametrów dla instrukcji inkrementacji, dane to po prostu wariant instrukcji.
