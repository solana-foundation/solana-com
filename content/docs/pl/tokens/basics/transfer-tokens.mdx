...
Przelewanie tokenów polega na przenoszeniu tokenów z jednego konta tokenów na
inne konto tokenów, które korzysta z tego samego mintu. Przelew tokenów
następuje, gdy wywołasz instrukcję
