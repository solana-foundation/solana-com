...
Przenoszenie tokenów polega na przesyłaniu tokenów z jednego konta tokenów na
inne konto tokenów, które korzysta z tego samego mintu. Przenoszenie tokenów
odbywa się poprzez wywołanie instrukcji
...
w programie tokenów. Tylko adres określony jako właściciel (autorytet)
źródłowego konta tokenów może przenosić tokeny z tego konta.
