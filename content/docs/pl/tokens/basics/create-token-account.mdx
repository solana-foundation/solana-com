...
Konto tokena to typ konta w programach Token Program Solany, który przechowuje
...
2. Program "owner" - Odnosi się to do programu, który jest właścicielem danych
   konta na Solanie. W przypadku kont tokenów jest to zawsze albo Token Program,
   albo Token Extensions Program, jak określono w polu "owner" bazowego typu
   Solana
...
Associated token account to konto tokenów z adresem, który jest Program Derived
...
Można myśleć o associated token account jako o domyślnym koncie tokenów dla
użytkownika do przechowywania jednostek określonego tokena (mint).

<Callout type="info">
  Termin "associated token accounts" odnosi się wyłącznie do kont tokenów
  tworzonych przez Associated Token Program.
</Callout>

Associated token accounts zapewniają deterministyczny sposób na znalezienie
konta tokenów użytkownika dla dowolnego tokena mint. Możesz sprawdzić
implementację tego procesu
...
To deterministyczne pochodzenie zapewnia, że dla dowolnej kombinacji adresu
portfela i tokena mint istnieje dokładnie jeden adres associated token account.
To podejście upraszcza znalezienie konta tokenów użytkownika dla dowolnego
tokena mint, eliminując potrzebę osobnego śledzenia adresów kont tokenów.
...
<Callout type="info">
  ...
  deterministycznym adresem, który jest PDA.
</Callout>
