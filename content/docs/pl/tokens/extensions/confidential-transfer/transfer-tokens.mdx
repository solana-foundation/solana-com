...
jak i odbiorca muszą mieć skonfigurowane konta tokenów z
_rs`ConfidentialTransferAccount`_ stanem i zatwierdzone do poufnych transferów.
Konto tokenów nadawcy musi również posiadać dostępne poufne saldo, z którego
można dokonać transferu.
...
1. Utwórz
   ...
   **Dowód równości
   ...
   Weryfikuje, że nowe zaszyfrowane saldo dostępne po transferze odpowiada jego
   odpowiedniemu
   ...
   **Dowód zakresu
   ...
   Weryfikuje, że nowe dostępne saldo i kwota transferu (podzielone na
   niskie/wysokie bity) są wszystkie nieujemne i mieszczą się w określonym
   zakresie.
...
Poniższy diagram pokazuje kroki związane z przesyłaniem tokenów z konta tokenów
...
Poniższy przykład demonstruje, jak poufnie przesyłać tokeny między kontami
tokenów.
...
zainstalowane Solana CLI, aby uruchomić lokalny validator.
...
---CODE-PLACEHOLDER-8c2b8b7b3e3f4c5a9d6e7f0a1b2c3d4e---
