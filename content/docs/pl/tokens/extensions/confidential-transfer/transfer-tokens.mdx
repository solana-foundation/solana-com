...
Aby poufnie przesyłać tokeny z jednego konta tokenów na drugie, zarówno nadawca,
jak i odbiorca muszą mieć skonfigurowane konta tokenów w stanie
...
1. Utwórz
   ...
   **Dowód równości
   ...
   Weryfikuje, czy nowy szyfrogram dostępnego salda po transferze odpowiada jego
   [zobowiązaniu Pedersena](https://en.wikipedia.org/wiki/Commitment_scheme),
   zapewniając, że nowe dostępne saldo konta źródłowego zostało poprawnie
   ...
   **Dowód ważności szyfrogramu
   ...
   Weryfikuje, czy szyfrogramy kwoty transferu zostały poprawnie wygenerowane
   dla wszystkich trzech stron (źródła, celu i audytora), zapewniając, że kwota
   transferu została poprawnie zaszyfrowana pod każdym kluczem publicznym.
   ...
   **Dowód zakresu
   ...
   Weryfikuje, czy nowe dostępne saldo i kwota transferu (podzielone na bity
...
3. Wywołaj instrukcję
   ...
   dostarczając konta stanu kontekstu dowodu.

4. Zamknij konta stanu kontekstu dowodu, aby odzyskać SOL użyte do ich
   utworzenia.

Poniższy diagram przedstawia kroki związane z przesyłaniem tokenów z konta
tokenów nadawcy na konto tokenów odbiorcy.
...
Aby poufnie przesyłać tokeny z jednego konta tokenów na drugie, musisz:

- Wygeneruj dowód równości, dowód poprawności szyfrogramu oraz dowód zakresu po
  stronie klienta
- Wywołaj program dowodowy Zk ElGamal, aby zweryfikować dowody i zainicjować
  konta "context state"
- Wywołaj instrukcję
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  podając trzy konta dowodowe.
- Zamknij trzy konta dowodowe, aby odzyskać rent.

Biblioteka `spl_token_client` udostępnia następujące metody:
...
sklonowanym z mainnetu za pomocą następującego polecenia. Musisz mieć
...
<Callout type="info">
  ...
  lokalnym walidatorze. Musisz sklonować Token Extension Program z mainnetu, aby
