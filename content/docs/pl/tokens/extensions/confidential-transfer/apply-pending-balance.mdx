...
Zanim tokeny mogą być przesyłane w sposób poufny, publiczne saldo tokenów musi
...
Kiedy tokeny są "deponowane" z publicznego salda lub kiedy tokeny są przesyłane
poufnie z jednego konta tokenów na inne, tokeny są początkowo dodawane do
poufnego oczekującego salda. Zanim tokeny mogą być użyte do poufnych transferów,
oczekujące saldo musi zostać "zastosowane" do dostępnego salda.
...
Poniższy przykład demonstruje, jak zastosować poufne oczekujące saldo do
poufnego dostępnego salda.
...
<Callout type="info">
  W momencie pisania tego tekstu, funkcja Confidential Transfers nie jest
  włączona na domyślnym lokalnym validatorze. Musisz sklonować główną wersję
  programu Token Extensions Program, aby uruchomić przykładowy kod.
</Callout>
