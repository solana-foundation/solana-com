---
title: Wpłata tokenów
description: Dowiedz się, jak wpłacać tokeny do stanu poufnego.
---

## Jak wpłacać tokeny do poufnego salda oczekującego

Zanim tokeny będą mogły być przesyłane w sposób poufny, publiczne saldo tokenów
musi zostać przekształcone w saldo poufne. Proces ten odbywa się w dwóch
etapach:
...
2. **Poufne saldo dostępne**: Następnie saldo oczekujące jest "zastosowane" do
   salda dostępnego, co umożliwia poufne przesyłanie tokenów.
...
Aby przekształcić publiczne saldo w poufne saldo oczekujące, wywołaj instrukcję
...
Poniższy przykład pokazuje, jak wpłacać publiczne saldo tokenów na poufne saldo
oczekujące.

Aby uruchomić przykład, uruchom lokalny walidator z Token Extension Program
sklonowanym z mainnetu za pomocą poniższego polecenia. Aby uruchomić lokalny
walidator, musisz mieć zainstalowane Solana CLI.
...
<Callout type="info">
  ...
  lokalnym walidatorze. Aby uruchomić przykładowy kod, musisz sklonować Token
  Extension Program z mainnetu.
</Callout>
