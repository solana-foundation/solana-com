...
_rs`ConfidentialTransferAccount`_ zawiera kilka pól do zarządzania poufnością
transferów:
...
- **pending_balance_lo**: Zaszyfrowane dolne 16 bitów oczekującego salda. Saldo
  jest podzielone na części wysoką i niską dla efektywnego odszyfrowania.

- **pending_balance_hi**: Zaszyfrowane górne 48 bitów oczekującego salda. Saldo
  jest podzielone na części wysoką i niską dla efektywnego odszyfrowania.
...
- **expected_pending_balance_credit_counter**: Wartość
  `pending_balance_credit_counter` dostarczona przez klienta za pomocą danych
...
DoS. Bez tego podziału atakujący mógłby wielokrotnie przesyłać tokeny na konto
tokena, blokując możliwość ich transferu przez właściciela konta. Właściciel
...
Podczas wywoływania instrukcji _rs`ApplyPendingBalance`_ w celu konwersji salda
...
`decryptable_available_balance`, różne wartości liczników
...
Poniższy kod demonstruje, jak utworzyć powiązane konto tokena z rozszerzeniem
...
<Callout type="info">
  ...
  lokalnym validatorze. Musisz sklonować główną wersję Token Extensions Program,
