...
| Wyświetlanie zrebalansowanych sald tokenów      | Pokazywanie zbilansowanych sald dla handlu lub dostarczania płynności w interfejsie użytkownika. (backend nadal może używać surowych wartości)            | **P0**    |
...
| Historyczne akcje nie powinny być przeskalowane | Historyczne akcje, takie jak transakcje, powinny być wyświetlane z dokładną zbilansowaną kwotą i ceną w momencie akcji.                                   | **P1**    |
| Wewnętrzne śledzenie surowych sald              | Śledzenie surowych sald dla transakcji on-chain zamiast zbilansowanych sald. Będzie to dokładniejsze i łatwiejsze do zarządzania w dłuższej perspektywie. | **P1**    |
| Adaptacja źródła cen                            | W każdym miejscu, gdzie źródło cen jest używane do wyświetlania bieżącej ceny, należy dostarczyć użytkownikom końcowym zbilansowaną cenę.                 | **P1**    |
| Wyświetlanie wykresu historii cen               | Odzwierciedlenie zbilansowanych cen na wykresie cen. Obejmuje to przeskalowanie historycznych cen do bieżącego mnożnika.                                  | **P1**    |
