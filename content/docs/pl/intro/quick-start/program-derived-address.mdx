...
description:
  ...
  Solanie, korzystając z Program Derived Addresses (PDAs) i frameworka Anchor.
  Ten przewodnik krok po kroku pokazuje, jak tworzyć, aktualizować i usuwać
  konta wiadomości w łańcuchu za pomocą PDA, wdrażać walidację kont i pisać
  testy. Idealny dla programistów, którzy chcą zrozumieć, jak używać PDA w
  programach Solana.
...
przy użyciu frameworka Anchor, jednocześnie demonstrując użycie Program Derived
Addresses (PDA). Więcej szczegółów znajdziesz na stronie
...
### Dodaj instrukcję Create

Teraz dodaj instrukcję _rs`create`_, która tworzy i inicjalizuje
_rs`MessageAccount`_.

Zacznij od zdefiniowania kont wymaganych dla instrukcji, aktualizując
_rs`Create`_ struct w następujący sposób:
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - Ograniczenia `seeds` i `bump` wyprowadzają adres konta jako Program Derived
     Address (PDA)
...
dyskryminator konta, który służy jako identyfikator typu konta, który Anchor
automatycznie generuje podczas tworzenia konta.

Typ _rs`String`_ potrzebuje 4 bajtów do przechowywania długości ciągu znaków, a
...
aktualizację funkcji `create` następująco:
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - Ograniczenie `realloc` wywołuje Program Systemowy w celu dostosowania
     rozmiaru danych konta

Należy zauważyć, że ograniczenie _rs`bump = message_account.bump`_ używa bump
seed przechowywanego na _rs`message_account`_, zamiast ponownego obliczania go
przez Anchor.
...
przekazując "Hello, World!" jako wiadomość.
...
<WithNotes>
Zaktualizuj drugi test za pomocą następującego kodu:
...
Najpierw ten kod wysyła transakcję, która wywołuje instrukcję `delete` w celu
...
pomocą jego adresu (`messagePda`) z `fetchNullable`. Ta metoda zwraca wartość
null, gdy konto przestaje istnieć po zamknięciu.
