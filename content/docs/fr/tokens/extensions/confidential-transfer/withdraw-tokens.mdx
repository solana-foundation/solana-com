...
L'exemple suivant démontre comment retirer des jetons du solde confidentiel
disponible vers le solde public.

Pour exécuter l'exemple, démarrez un validateur local avec le Programme
d'Extension de Token cloné depuis le réseau principal en utilisant la commande
suivante. Vous devez avoir installé la CLI Solana pour démarrer le validateur
local.
