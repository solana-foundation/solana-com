...
description:
  ...
  ajouter des fonctionnalités optionnelles aux émissions de jetons et aux
  comptes.
---
...
comptes de jetons sans révéler le montant du transfert. Ceci est utile pour les
transactions préservant la confidentialité. Seuls les montants des transferts et
les soldes des jetons sont privés. Les adresses des comptes de jetons restent
publiques.
...
- [Cookbook des soldes confidentiels](https://github.com/solana-developers/Confidential-Balances-Sample) -
...
au programme d'extension de jetons qui vous permet de transférer des jetons
entre comptes sans révéler le montant du transfert.
...
Le flux de base des transferts confidentiels de jetons est le suivant :

1. Créer un compte d'émission avec l'extension de transfert confidentiel.
2. Créer des comptes de jetons avec l'extension de transfert confidentiel pour
   l'expéditeur et le destinataire.
3. Émettre des jetons sur le compte de l'expéditeur.
...
5. **Appliquer** le solde en attente de l'expéditeur au **solde confidentiel
   disponible**.
6. **Transférer** confidentiellement des jetons du compte de l'expéditeur au
...
<Cards>
  <Card
    title="Créer un compte d'émission"
    href="/docs/tokens/extensions/confidential-transfer/create-mint"
  >
    Comment créer un compte d'émission avec l'extension de transfert
    confidentiel
  </Card>
  <Card
  ...
  >
    Comment configurer un compte de jetons avec l'extension de transfert
    confidentiel
  </Card>
  <Card
  ...
  >
    Comment déposer des jetons sur le solde confidentiel en attente
  </Card>
  <Card
  ...
  >
    Comment transférer confidentiellement des jetons entre comptes de jetons
  </Card>
</Cards>
...
L'exemple en Rust ci-dessous montre les étapes pour les transferts de jetons
confidentiels correspondant au diagramme précédent. Les étapes sont séparées en
fonctions auxiliaires pour plus de clarté.

Pour exécuter l'exemple, démarrez un validateur local avec le Programme
d'Extension de Jetons cloné depuis le réseau principal en utilisant la commande
suivante. Vous devez avoir installé la CLI Solana pour utiliser le validateur
local.
...
<Callout type="info">
  ...
  de Jetons du réseau principal pour exécuter le code d'exemple.
</Callout>
...
[instructions](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/instruction.rs#L29)
d'extension de Transfert Confidentiel est la suivante :

| Instruction                         | Description                                                                                                                                                                               |
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Configure le compte de mint pour les transferts confidentiels. Cette instruction doit être incluse dans la même transaction que l'instruction _rs`TokenInstruction::InitializeMint`_.     |
| _rs`UpdateMint`_                    | Met à jour les paramètres de transfert confidentiel pour un mint.                                                                                                                         |
| _rs`ConfigureAccount`_              | Configure un compte de jetons pour les transferts confidentiels.                                                                                                                          |
| _rs`ApproveAccount`_                | Approuve un compte de jetons pour les transferts confidentiels si le mint exige une approbation pour les nouveaux comptes de jetons.                                                      |
| _rs`EmptyAccount`_                  | Vide les soldes confidentiels en attente et disponibles pour permettre la fermeture d'un compte de jetons.                                                                                |
...
| _rs`Transfer`_                      | Transfère des jetons entre comptes de manière confidentielle.                                                                                                                             |
| _rs`ApplyPendingBalance`_           | Convertit le solde en attente en solde disponible après des dépôts ou transferts.                                                                                                         |
| _rs`EnableConfidentialCredits`_     | Permet à un compte de jetons de recevoir des transferts de jetons confidentiels.                                                                                                          |
| _rs`DisableConfidentialCredits`_    | Bloque les transferts confidentiels entrants tout en permettant les transferts publics.                                                                                                   |
| _rs`EnableNonConfidentialCredits`_  | Permet à un compte de jetons de recevoir des transferts de jetons publics.                                                                                                                |
...
| _rs`TransferWithFee`_               | Transfère des jetons entre comptes de manière confidentielle avec des frais.                                                                                                              |
| _rs`ConfigureAccountWithRegistry`_  | Méthode alternative pour configurer des comptes de jetons pour les transferts confidentiels en utilisant un compte _rs`ElGamalRegistry`_ au lieu d'une preuve _rs`VerifyPubkeyValidity`_. |
