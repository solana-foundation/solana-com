...
description: Apprenez comment créer un mint SPL Token.
---

## Qu'est-ce qu'un compte Mint ?

Un compte mint est un type de compte dans les programmes Token de Solana qui
...
<Callout type="info">
  ...
  partagent la même implémentation de base pour le compte Mint.
</Callout>

Chaque token sur Solana existe sous forme d'un compte mint dont l'adresse sert
d'identifiant unique sur le réseau.

Par exemple, l'USD Coin (USDC), le dollar numérique sur Solana, possède
l'adresse mint `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`. Cette adresse
mint sert d'identifiant unique pour l'USD Coin dans tout l'écosystème Solana.
Vous pouvez consulter les détails de ce compte mint sur
[Solana Explorer](https://explorer.solana.com/address/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v).

## Comment créer un compte Mint

Pour créer un compte Mint, invoquez l'instruction
...
La transaction pour créer un compte mint nécessite deux instructions :

1. Invoquer le System Program pour créer et allouer de l'espace pour un compte
   mint et transférer la propriété au Token Program.
2. Invoquer le Token Program pour initialiser les données du compte mint.
