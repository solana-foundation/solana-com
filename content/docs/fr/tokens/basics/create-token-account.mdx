...
Un token account est un type de compte dans les Token Programs de Solana qui
stocke des informations sur la propriété d'un jeton spécifique (mint) par un
individu. Chaque token account est associé à un seul mint et suit des détails
comme le solde de jetons et le propriétaire.
...
créer un token account. Chaque token account garde une trace de :

1. Un mint spécifique (le type de jeton que le token account détient)
...
- Circle (l'émetteur d'USD Coin) possède un token account à
  `3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
- Ce token account ne détient que des unités du jeton USD Coin (mint)
...
1. Le « propriétaire » du token account - Il s'agit d'une adresse stockée dans
   les données du token account comme champ « owner » du type
   ...
   défini par le Token Program. Le propriétaire peut transférer, brûler ou
   déléguer des jetons depuis le compte. Cette adresse est parfois appelée «
   autorité » du token account pour la distinguer du propriétaire du programme.

2. Le "owner" (propriétaire) du programme - Ceci fait référence au programme qui
   possède les données du compte sur Solana. Pour les token account, il s'agit
   toujours soit du Token Program ou du Token Extensions Program, comme spécifié
   dans le champ "owner" du type de base Solana
   [Account](https://github.com/anza-xyz/solana-sdk/blob/4e30766b8d327f0191df6490e48d9ef521956495/account/src/lib.rs#L51).

Lorsqu'on travaille avec des token account, "owner" fait généralement référence
à l'autorité qui peut dépenser les tokens, et non au programme qui possède le
compte.
...
Un associated token account est un token account dont l'adresse est une Program
Derived Address (PDA) créée par l'
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program).
Vous pouvez considérer un associated token account comme le token account par
défaut pour un utilisateur détenant des unités d'un token spécifique (mint).

<Callout type="info">
  Le terme "associated token accounts" s'applique uniquement aux token account
  que l' Associated Token Program crée.
</Callout>

Les associated token account fournissent une méthode déterministe pour trouver
le token account d'un utilisateur pour n'importe quel mint donné. Vous pouvez
...
portefeuille et de token mint, il existe exactement une adresse d'associated
token account. Cette approche simplifie la recherche du token account d'un
utilisateur pour n'importe quel token mint, éliminant ainsi le besoin de suivre
séparément les adresses des token account.

<Callout type="info">
  L'Associated Token Program fonctionne comme un programme auxiliaire qui crée
  des token account avec des adresses déterministes (PDAs). Lors de la création
  d'un associated token account, l'Associated Token Program effectue un CPI
  (Cross-Program Invocation) vers soit le Token Program soit le Token Extensions
  Program. Le token program possède le compte créé qui a la même structure de
  type `Account` que celle définie dans le token program. L'Associated Token
  Program ne maintient aucun état - il offre simplement une façon standardisée
  de créer des token account à une adresse déterministe qui est une PDA.
</Callout>
...
1. Invoquer le System Program pour créer et allouer de l'espace pour un compte
   de jetons et transférer la propriété au Token Program.
2. Invoquer le Token Program pour initialiser les données du compte de jetons.
...
Pour créer un associated token account, invoquez l'instruction
...
L'instruction pour créer un associated token account invoque automatiquement le
System Program pour créer le compte de jetons et invoque le Token Program pour
initialiser les données du compte de jetons. Cela se produit via des Cross
