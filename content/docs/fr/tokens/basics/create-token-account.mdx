...
Un compte de jetons est un type de compte dans les programmes de jetons de
Solana qui stocke des informations sur la propriété d'un jeton spécifique (mint)
par un individu. Chaque compte de jetons est associé à un seul mint et suit des
détails comme le solde de jetons et le propriétaire.
...
<Callout type="info">
  ...
  un type `Account`. Le [Programme de
  Jetons](https://github.com/solana-program/token/blob/6d18ff73b1dd30703a30b1ca941cb0f1d18c2b2a/program/src/state.rs#L87-L108)
  et le [Programme d'Extension de
  Jetons](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/state.rs#L102-L123)
  partagent la même implémentation de base pour le compte de jetons.
</Callout>

Pour détenir des jetons d'un mint spécifique, les utilisateurs doivent d'abord
créer un compte de jetons. Chaque compte de jetons garde une trace de :

1. Un mint spécifique (le type de jeton que le compte de jetons détient)
2. Un propriétaire (l'autorité qui peut transférer des jetons depuis le compte)

Voici un exemple utilisant USD Coin (USDC) sur Solana :

- L'adresse du mint USD Coin : `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Circle (l'émetteur de USD Coin) possède un compte de jetons à
  `3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
- Ce compte de jetons ne détient que des unités du jeton USD Coin (mint)
- Circle a l'autorité de propriétaire à
  `7VHUFJHWu2CuExkJcJrzhQPJ2oygupTWkL2A2For4BmE` et peut transférer ces jetons
...
<Callout type="info">
  Le terme "propriétaire" est utilisé dans deux contextes différents :

1. Le "propriétaire" du compte de jetons - Il s'agit d'une adresse stockée dans
   les données du compte de jetons comme champ "owner" du type
   ...
   défini par le Programme de Jetons. Le propriétaire peut transférer, brûler ou
   déléguer des jetons depuis le compte. Cette adresse est parfois appelée
   "autorité" du compte de jetons pour la distinguer du propriétaire du
   programme.

2. Le "propriétaire" du programme - Ceci fait référence au programme qui possède
   les données du compte sur Solana. Pour les comptes de jetons, il s'agit
   toujours soit du Programme de Jetons (Token Program) soit du Programme
   d'Extension de Jetons (Token Extension Program), comme spécifié dans le champ
   "owner" du type
...
référence à l'autorité qui peut dépenser les jetons, et non au programme qui
possède le compte.
...
Un compte de jetons associé est un compte de jetons dont l'adresse est une
Adresse Dérivée de Programme (PDA) créée par le
[Programme de Jetons Associés](https://github.com/solana-program/associated-token-account/tree/main/program).
Vous pouvez considérer un compte de jetons associé comme le compte de jetons par
défaut pour un utilisateur détenant des unités d'un jeton spécifique (mint).

<Callout type="info">
  Le terme "comptes de jetons associés" s'applique uniquement aux comptes de
  jetons que le Programme de Jetons Associés crée.
</Callout>

Les comptes de jetons associés fournissent une méthode déterministe pour trouver
le compte de jetons d'un utilisateur pour n'importe quel mint donné. Vous pouvez
examiner l'implémentation de la dérivation
[ici](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/interface/src/address.rs#L56).
...
Cette dérivation déterministe garantit que pour toute combinaison d'adresse de
portefeuille et de mint de jeton, il existe exactement une adresse de compte de
jetons associé. Cette approche simplifie la recherche du compte de jetons d'un
utilisateur pour n'importe quel mint de jeton, éliminant ainsi la nécessité de
suivre séparément les adresses de comptes de jetons.

<Callout type="info">
  Le Programme de Jetons Associés fonctionne comme un programme auxiliaire qui
  crée des comptes de jetons avec des adresses déterministes (PDAs). Lors de la
  création d'un compte de jetons associé, le Programme de Jetons Associés
  effectue un CPI (Invocation Inter-Programme) vers soit le Programme de Jetons
  soit le Programme d'Extension de Jetons. Le programme de jetons possède le
  compte créé qui a la même structure de type `Account` que celle définie dans
  le programme de jetons. Le Programme de Jetons Associés ne maintient aucun
  état - il offre simplement une façon standardisée de créer des comptes de
  jetons à une adresse déterministe qui est une PDA.
</Callout>

## Comment créer un compte de jetons

Pour créer un compte de jetons, invoquez l'instruction
...
[ici](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/processor.rs#L145).

La transaction pour créer un compte de jetons nécessite deux instructions :

1. Invoquer le programme système pour créer et allouer de l'espace pour un
   compte de jetons et transférer la propriété au programme de jetons.
2. Invoquer le programme de jetons pour initialiser les données du compte de
   jetons.
...
Pour créer un compte de jetons associé, invoquez l'instruction
...
Vous pouvez trouver des implémentations de cette instruction
[ici](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/program/src/processor.rs#L66).

L'instruction pour créer un compte de jetons associé invoque automatiquement le
programme système pour créer le compte de jetons et invoque le programme de
jetons pour initialiser les données du compte de jetons. Cela se produit via des
