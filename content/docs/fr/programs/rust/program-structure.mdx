...
[Bibliothèque de programmes Solana](https://github.com/solana-program/token/tree/main/program/src).
...
du crate `solana_program` pour définir le point d'entrée dans votre programme.
...
Solana peut effectuer. Considérez-les comme des API publiques pour votre
programme - elles définissent quelles actions les utilisateurs peuvent
entreprendre lors de l'interaction avec votre programme.

Les instructions sont généralement définies à l'aide d'une énumération Rust où :

- Chaque variante d'énumération représente une instruction différente
...
Lorsqu'un client invoque votre programme, il doit fournir des données
d'instruction (sous forme de tampon d'octets) où :
...
Pour convertir les données d'instruction (octets) en une variante de
l'énumération, il est courant d'implémenter une méthode auxiliaire. Cette
méthode :
...
1. Le compte compteur qui sera créé et initialisé
2. Le compte payeur qui financera la création du nouveau compte
3. Le Programme Système que nous invoquons pour créer le nouveau compte

Pour définir les comptes requis par l'instruction, nous créons un itérateur sur
le slice `accounts` et utilisons la fonction `next_account_info` pour obtenir
...
fonctionnalité du programme, l'utilisation de noms descriptifs est recommandée.
...
2. Calculer le solde minimum de "loyer" requis. Sur Solana, les comptes doivent
   maintenir un solde minimum de lamports (loyer) basé sur la quantité de
   données stockées sur le compte.
...
Une fois l'espace défini et le loyer calculé, créez le compte en invoquant
l'instruction `create_account` du Programme Système.

Sur Solana, les nouveaux comptes ne peuvent être créés que par le Programme
Système. Lors de la création d'un compte, nous spécifions le nombre d'octets à
allouer et le programme propriétaire du nouveau compte. Le Programme Système :
...
Pour invoquer le Programme Système depuis l'instruction de notre programme, nous
effectuons une Invocation Inter-Programme (CPI) via la fonction `invoke`. Une
CPI permet à un programme d'appeler des instructions sur d'autres programmes -
dans ce cas, l'instruction `create_account` du Programme Système.
...
1. Créant une nouvelle structure `CounterAccount` avec le `initial_value` fourni
   à l'instruction.
2. Obtenant une référence mutable vers le champ de données du nouveau compte.
...
comptes fournis soient ceux auxquels le programme s'attend. L'absence de
vérifications de validation des comptes constitue une source courante de
vulnérabilités des programmes.

L'exemple ci-dessous inclut une vérification pour s'assurer que le compte auquel
nous faisons référence en tant que `counter_account` appartient bien au
...
`cargo test-sbf` pour exécuter les tests. Facultativement, utilisez le flag
...
Ensuite, configurez le test en utilisant `ProgramTest`. Puis créez une nouvelle
keypair à utiliser comme adresse pour le compte compteur que nous initialiserons
et définissez une valeur initiale pour le compteur.
...
- Créez des données d'instruction avec la variante 0 (`InitializeCounter`) et la
  valeur initiale
- Construisez l'instruction avec l'ID du programme, les données d'instruction et
  les comptes requis
...
Notez que les données d'instruction pour l'instruction d'incrémentation sont
`[1]`, ce qui correspond à la variante 1 (`IncrementCounter`). Comme il n'y a
pas de paramètres supplémentaires pour l'instruction d'incrémentation, les
données sont simplement la variante d'instruction.
