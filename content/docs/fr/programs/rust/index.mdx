...
pas généré lorsque vous construirez le programme.
...
Ensuite, testez le programme en utilisant la bibliothèque `solana-program-test`.
...
"URL RPC personnalisée" sur Solana Explorer est par défaut
`http://localhost:8899`.
...
être réutilisé. Toute tentative de déploiement d'un programme avec un ID de
programme précédemment fermé entraînera une erreur.
