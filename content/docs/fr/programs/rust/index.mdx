...
Pour afficher l'identifiant du programme, exécutez la commande suivante dans
votre terminal. Cette commande affiche la clé publique du keypair situé au
chemin de fichier spécifié :
...
Rust qui finance une nouvelle keypair pour payer les frais de transaction, puis
...
de programme. Mettez à jour le programme dans `src/lib.rs` pour afficher "Hello,
...
Notez qu'une fois un programme fermé, son ID de programme ne peut plus être
réutilisé. Toute tentative de déploiement d'un programme avec un ID de programme
...
nouvelle keypair pour le programme, exécutez la commande suivante :

```shell title="Terminal"
solana-keygen new -o ./target/deploy/hello_world-keypair.json --force
```

Alternativement, vous pouvez supprimer le fichier keypair existant (par exemple
