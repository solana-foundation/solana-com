...
Si la commande d'installation rapide ne fonctionne pas, veuillez vous référer à
la section [Installer les dépendances](#install-dependencies) ci-dessous pour
les instructions d'installation individuelle de chaque dépendance.
...
Linux ci-dessous avant de procéder à l'installation de Rust, de la CLI Solana et
de la CLI Anchor.
...
étapes individuelles pour l'installation de Rust, Solana CLI et Anchor CLI.
...
Les développeurs construisent des programmes Solana en utilisant le
...
à votre `PATH` afin que la commande soit disponible dans votre terminal.
...
Agave est le client validateur d'[Anza](https://www.anza.xyz/), anciennement
connu sous le nom de client validateur Solana Labs.
...
Installez nvm à l'aide de la commande suivante :
...
Si vous avez déjà un portefeuille de système de fichiers enregistré à
l'emplacement par défaut, cette commande ne l'écrase pas, sauf si vous forcez
...
Les airdrops du devnet limitent les demandes à 5 SOL par requête. Si vous
...
## Notions de base de l'interface en ligne de commande Anchor

Cette section présente quelques commandes courantes de l'interface en ligne de
commande Anchor pour vous aider à démarrer. Pour plus d'informations sur
l'interface en ligne de commande Anchor, consultez la
