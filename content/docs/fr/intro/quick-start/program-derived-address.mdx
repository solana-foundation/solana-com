...
description:
  ...
  Supprimer) en utilisant les adresses dérivées de programme (PDAs) et le
  framework Anchor. Ce guide étape par étape démontre comment créer, mettre à
  jour et supprimer des comptes de messages on-chain en utilisant des PDAs,
  implémenter la validation de compte et écrire des tests. Parfait pour les
  développeurs souhaitant comprendre comment utiliser les PDAs dans les
  programmes Solana.
h1: Adresse dérivée de programme
---

Dans cette section, vous apprendrez à construire un programme CRUD (Create,
Read, Update, Delete) de base.

Ce guide démontre un programme simple où les utilisateurs peuvent créer, mettre
à jour et supprimer un message. Chaque message existe dans un compte avec une
adresse déterministe dérivée du programme lui-même (Adresse dérivée de programme
ou PDA).

Ce guide vous accompagne dans la construction et le test d'un programme Solana
en utilisant le framework Anchor tout en démontrant les adresses dérivées de
programme (PDAs). Pour plus de détails, consultez la page
[Adresses dérivées de programme](/docs/core/pda).

Pour référence, vous pouvez consulter le
[code final](https://beta.solpg.io/668304cfcffcf4b13384d20a) après avoir
complété les sections PDA et invocation inter-programmes (CPI).
...
code de départ. Ensuite, cliquez sur le bouton "Import" pour ajouter le
programme à vos projets Solana Playground.
...
Le champ _rs`bump`_ stocke un _rs`u8`_
["bump" seed](/docs/core/pda#canonical-bump) utilisé pour dériver une adresse
dérivée du programme (PDA). Stocker cette valeur économise du calcul en
éliminant la nécessité de la recalculer dans les instructions ultérieures.
...
- `user` - _rs`Pubkey`_ qui identifie l'utilisateur ayant créé le compte de
  message.
- `message` - _rs`String`_ qui contient le message de l'utilisateur.
- `bump` - _rs`u8`_ qui stocke le ["bump" seed](/docs/core/pda#canonical-bump)
  pour dériver l'adresse dérivée du programme (PDA). Stocker cette valeur
  économise du calcul en éliminant la nécessité de la recalculer dans les
...
La contrainte _rs`mut`_ déclare le compte comme mutable.
...
La contrainte _rs`space`_ spécifie le nombre d'octets à allouer pour le champ de
...
référence au Programme Système.
...
1. _rs`user: Signer<'info>`_
   ...
   - Représente l'utilisateur créant le compte de message
   - Nécessite un statut mutable (_rs`#[account(mut)]`_) puisqu'il paie pour le
     nouveau compte
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - Les contraintes `seeds` et `bump` dérivent l'adresse du compte en tant
     qu'Adresse Dérivée du Programme (PDA)
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - En coulisses, la contrainte `init` appelle le Programme Système pour créer
     un nouveau compte avec le `space` spécifié et change le propriétaire pour
     le programme actuel.
...
comme une Adresse Dérivée du Programme (PDA).
...
La contrainte `seeds` définit les entrées optionnelles utilisées pour dériver la
PDA.

- _rs`b"message"`_ - Une chaîne fixe comme première graine.
...
la bonne graine de bump. Anchor utilise `seeds` et `bump` pour dériver la PDA.

---

Le calcul de `space` _rs`(8 + 32 + 4 + message.len() + 1)`_ alloue de l'espace
pour le type de données `MessageAccount` :
...
- Message utilisateur (_rs`String`_) : 4 octets pour la longueur + longueur
  variable du message
- Graine de bump PDA (_rs`u8`_) : 1 octet
...
1. _rs`user: Signer<'info>`_
   ...
   - Marqué comme mutable (_rs`#[account(mut)]`_) pour recevoir les lamports du
     compte fermé
   - Doit signer pour garantir que seul l'utilisateur approprié peut fermer son
     compte de message
...
Le corps de la fonction affiche simplement un message dans les journaux du
programme en utilisant la macro _rs`msg!()`_. La fonction ne nécessite pas de
...
dédié aux tests de développement.
...
Le code de démarrage inclut également un fichier de test dans `anchor.test.ts`.
...
La méthode _ts`fetchNullable()`_ récupère les données du compte depuis le réseau
...
Examinez les liens SolanaFM pour visualiser les détails de la transaction.
