...
le bump seed correct. Anchor utilise `seeds` et `bump` pour dériver la PDA.
...
- Message utilisateur (_rs`String`_) : 4 octets pour la longueur + longueur
  variable du message
- Bump seed de la PDA (_rs`u8`_) : 1 octet
...
<Callout type="info">
Dans cet exemple, vous allez déployer le programme sur le devnet, un cluster Solana
