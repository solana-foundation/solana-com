...
Les comptes sur Solana peuvent stocker des programmes "d'état" ou "exécutables".
Chaque compte possède une "adresse" (clé publique) qui sert d'identifiant unique
utilisé pour localiser ses données correspondantes sur la chaîne.
...
- **Programmes exécutables** : Comptes contenant le code réel des programmes
  Solana. Ces comptes stockent les instructions que les utilisateurs peuvent
  invoquer.
...
utilisent ce champ pour stocker soit l'état du programme, soit le code du
...
Le champ `lamports` contient le solde SOL du compte, exprimé en lamports.
...
Ce champ n'est actuellement pas utilisé, mais il est inclus pour la
rétrocompatibilité.
...
comptes de portefeuille et les program accounts.
...
Le champ `executable` est `false` car le champ `data` du compte mint stocke
...
de données `Mint`. Ceci est abordé à l'étape suivante.
...
Le Token Program (`TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA`) possède le
compte mint.

Cela signifie que le champ `data` du compte mint ne peut être modifié que par
...
compte mint en type de données
[Mint](https://github.com/solana-program/token/blob/program%40v8.0.0/program/src/state.rs#L16-L30).
...
La fonction _ts`getMint()`_ désérialise le champ `data` d'un mint account en
type de données
[Mint](https://github.com/solana-program/token/blob/program%40v8.0.0/program/src/state.rs#L16-L30)
...
L'adresse du mint account est utilisée pour identifier le token sur le réseau
Solana.
...
C'est le seul compte qui peut créer de nouvelles unités du token.
...
Un token account qui est gelé ne peut pas transférer ou détruire le jeton dans
le compte.
...
Ce champ n'est pertinent que pour les comptes créés par le Token Extensions
Program (Token2022).
