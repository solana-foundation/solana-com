...
description:
  ...
  utilisant le framework Anchor et Solana Playground. Ce guide adapté aux
  débutants vous accompagne dans la création d'un programme simple, son
  déploiement sur devnet, l'exécution de tests et la fermeture du programme.
...
guide présente un exemple plus détaillé dans les sections Programme Derived
...
Ceux-ci ajoutent des vérifications communes ou des fonctionnalités qu'Anchor
fournit pour améliorer la sécurité du programme.

### !parameter

C'est la valeur que vous transmettez à l'instruction lorsque vous l'appelez.
...
capot, cette contrainte invoque le Programme Système pour créer un nouveau
compte et fait de ce programme le propriétaire. En tant que propriétaire du
programme, ce programme peut alors écrire dans les données du compte.
...
Pour l'instant, cela couvre l'aperçu de haut niveau du code du programme :

<WithMentions>

- La macro declare_id! spécifie l'adresse on-chain de votre programme. Solana
  Playground met automatiquement à jour cette adresse lorsque vous compilez le
  programme à l'étape suivante.
...
1. `ctx: Context<Initialize>` - Transmet à la fonction les comptes requis pour
   cette instruction, tels que spécifiés dans la structure `Initialize`.
...
fourni, puis imprime un message dans les logs du programme.
...
obtenir du SOL sur devnet soit en exécutant _shell`solana airdrop 5`_ dans le
...
`solana confirm -v` et en spécifiant le hash de transaction (signature) à partir
...
<Callout>
  ...
  Playground utilise le cluster devnet par défaut.
</Callout>
...
Enfin, la fermeture du programme permet la récupération complète du SOL alloué
au programme on-chain.
...
Félicitations. Vous venez de construire et de déployer votre premier programme
Solana en utilisant le framework Anchor.
