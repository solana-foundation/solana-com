---
title: Composition de programmes multiples
description:
  Apprenez à implémenter des invocations inter-programmes (CPI) dans les
  ...
  gérer les adresses dérivées de programme (PDA) dans les CPI. Parfait pour les
  développeurs souhaitant créer des programmes Solana composables.
h1: Invocation inter-programmes
---

Dans cette section, le programme CRUD de la section PDA précédente est mis à
jour en ajoutant des invocations inter-programmes (CPI), une fonctionnalité qui
...
adresses dérivées de programme (PDA) lors de l'exécution d'invocations
inter-programmes.
...
des CPI dans un programme Solana en utilisant le framework Anchor, en s'appuyant
sur les concepts de PDA explorés dans la section précédente. Pour plus de
détails, consultez la page [Invocation inter-programmes](/docs/core/cpi).
...
Commencez par mettre à jour le fichier `lib.rs` pour importer les éléments du
...
reçoit des SOL des utilisateurs lorsqu'ils mettent à jour leurs messages.
...
- En tant que PDA, il n'a pas de clé privée, donc seul le programme peut
  "signer" pour l'adresse lors de l'exécution des CPI
...
0,001 SOL du compte de l'utilisateur vers le compte coffre-fort.
...
- `to` - Le compte vault (destination des fonds)
...
- Le `amount` à transférer (1 000 000 lamports, soit 0,001 SOL)
...
Tout d'abord, mettez à jour la structure _rs`Delete`_ pour inclure le
`vault_account`. Cela permet le transfert de tout SOL dans le vault vers
...
Ajoutez également le `system_program` car le CPI pour le transfert nécessite
...
La méthode _rs`with_signer()`_ transmet les seeds de signataire avec le CPI.

Cela permet à un programme de "signer" pour un PDA dérivé de son ID de
programme.

Pendant le traitement de l'instruction, le runtime vérifie que les seeds de
signataire fournis dérivent correctement vers l'adresse du PDA. Si vérifié, le
runtime traite ce compte PDA comme un signataire pour la durée du CPI.
...
Dans l'instruction de suppression, une autre Invocation de Programme Croisé
(CPI) implémente l'instruction de transfert du Programme Système. Ce CPI
démontre comment effectuer un transfert qui nécessite un signataire d'Adresse
Dérivée de Programme (PDA).

D'abord, définissez les seeds de signataire pour le PDA du coffre-fort :
...
La structure _rs`Transfer`_ spécifie les comptes requis pour l'instruction de
transfert du Programme Système :
...
- Le programme à invoquer (Programme Système)
...
Programme Système, en transmettant :
...
une invocation, elle appelle en interne l'instruction de transfert du Programme
Système, en signant pour le PDA afin de permettre le transfert de tous les fonds
...
Obtenez du SOL devnet depuis le [Faucet Solana](https://faucet.solana.com/).
...
Seule l'autorité de mise à jour du programme peut le modifier. Le développeur
définit l'autorité de mise à jour lors du déploiement du programme, et elle
reste le seul compte autorisé à modifier ou à fermer le programme. Si quelqu'un
révoque l'autorité de mise à jour, le programme devient alors immuable.
...
Vous pouvez ensuite examiner les liens SolanaFM pour visualiser les détails de
la transaction, où vous trouverez les CPI pour les instructions de transfert au
sein des instructions de mise à jour et de suppression.
...
acquis une expérience pratique avec les concepts clés de Solana, notamment :
