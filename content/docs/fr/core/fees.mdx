...
L'approche recommandée pour définir les frais prioritaires est de d'abord
...
**Priorité = ((Limite d'unité de calcul \* Prix de l'unité de calcul) + Frais de
base) / (1 + Limite d'unité de calcul + Unités de calcul de signature + Unités
de calcul de verrouillage d'écriture)**

Utilisez ces instructions pour définir la limite et le prix des unités de calcul
sur une transaction :

- [`SetComputeUnitLimit`](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/src/compute_budget.rs#L42-L44)
  pour définir une limite d'unité spécifique.
...
d'unité de calcul par défaut avec un prix d'unité de calcul de 0 (pas de frais
de priorisation).

Les frais prioritaires dépendent de la limite d'unité de calcul que vous
demandez pour la transaction, et non des unités de calcul réellement utilisées.
Si vous définissez une limite d'unité de calcul trop élevée ou utilisez le
montant par défaut, vous pourriez payer pour des unités de calcul non utilisées.
