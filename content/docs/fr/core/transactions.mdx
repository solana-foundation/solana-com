...
- Si une transaction inclut plusieurs instructions, celles-ci s'exécutent dans
  l'ordre dans lequel elles ont été ajoutées à la transaction.
...
- Les transactions Solana incluent des instructions qui invoquent des programmes
  sur le réseau.
- Les transactions sont **atomiques** - si une instruction échoue, la
  transaction entière échoue et aucun changement n'est effectué.
...
[Programme Système](/docs/core/accounts#system-program). Seul le propriétaire du
programme peut modifier les données d'un compte, donc transférer du SOL
nécessite d'envoyer une transaction pour invoquer le Programme Système.

![Transfert SOL](/assets/docs/core/transactions/sol-transfer.svg)
...
![Processus de transfert SOL](/assets/docs/core/transactions/sol-transfer-process.svg)

Les exemples ci-dessous montrent comment envoyer une transaction qui transfère
du SOL d'un compte à un autre.
...
considérée comme une fonction publique qui peut être appelée par n'importe quel
utilisateur du réseau Solana.
...
- Données d'instruction : Tableau d'octets spécifiant l'instruction à invoquer
  sur le programme et tous les arguments requis par l'instruction
...
Chaque compte requis par une instruction doit être fourni sous forme
d'[AccountMeta](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/instruction/src/account_meta.rs#L25)
qui contient :
...
- [En-tête de message](/docs/core/transactions#message-header) : Spécifie le
  nombre de signataires et de comptes en lecture seule.
- [Adresses de comptes](/docs/core/transactions#array-of-account-addresses) : Un
  tableau d'adresses de comptes requis par les instructions de la transaction.
...
L'[en-tête de message](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/message/mod.rs#L97)
...
![En-tête de message](/assets/docs/core/transactions/message_header.png)
...
1. Index d'ID de programme : Un index u8 qui pointe vers l'adresse du programme
   dans le tableau d'adresses de compte. Ceci spécifie le programme qui traitera
   l'instruction.
2. Index de comptes : Un tableau d'index u8 qui pointent vers les adresses de
   compte requises pour cette instruction.
3. Données d'instruction : Un tableau d'octets spécifiant quelle instruction
   invoquer sur le programme et toutes données supplémentaires requises par
   l'instruction (ex. arguments de fonction).
...
- `accountKeys` : Tableau de toutes les adresses de compte utilisées dans les
...
- `instructions` : Tableau d'instructions à exécuter. Chaque `account` et
  `programIdIndex` dans une instruction référence le tableau `accountKeys` par
  index.
