...
description:
  Impara a costruire un programma CRUD (Create, Read, Update, Delete) su Solana
  ...
  e scrivere test. Perfetta per gli sviluppatori che vogliono capire come
...
completato sia la sezione sui PDA che quella sulle Cross-Program Invocation
(CPI).
...
programma, ma non influiscono sulla validazione. È consigliabile utilizzare nomi
descrittivi per maggiore chiarezza.
...
Il calcolo di `space` _rs`(8 + 32 + 4 + message.len() + 1)`_ alloca spazio per
il tipo di dati `MessageAccount`:
...
un discriminatore di account, che funge da identificatore per il tipo di account
che Anchor genera automaticamente durante la creazione dell'account.
...
account di messaggi. Accetta due parametri:
...
La struttura _rs`Update`_ definisce gli account richiesti per l'istruzione
_rs`update`_.
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - I vincoli `seeds` e `bump` verificano l'account come il PDA corretto
...
memorizzato sul _rs`message_account`_, invece di far ricalcolare ad Anchor.

L'attributo _rs`#[instruction(message: String)]`_ consente alla struttura
...
Ora, aggiungi la logica per l'istruzione `update`.
...
In questo caso, il programma utilizza il bump seed esistente memorizzato nel
...
1. _rs`user: Signer<'info>`_
   ...
   - Contrassegnato come modificabile (_rs`#[account(mut)]`_) per ricevere i
     lamport dall' account chiuso
   - Deve firmare per garantire che solo l'utente corretto possa chiudere il
     proprio account di messaggio
...
Aggiungi il codice qui sotto all'interno di _ts`describe()`_, ma prima delle
sezioni _ts`it()`_.
...
In questa sezione, questo codice semplicemente configura il file di test.
...
Prima, il file di test invia una transazione che richiama l'istruzione `update`,
...
Ispeziona i link di SolanaFM per visualizzare i dettagli della transazione.
