...
Ora esploriamo come scrivere dati su di essa. Scrivere sulla rete Solana
comporta l'invio di transazioni che contengono una o più istruzioni.
...
programma proprietario può detrarre il saldo SOL (lamport) di un account.
...
L'ordine delle istruzioni è importante qui. L'istruzione
`createAccountInstruction` deve venire prima dell'istruzione
`initializeMintInstruction`.
