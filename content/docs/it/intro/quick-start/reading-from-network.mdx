...
- **Programmi eseguibili**: Account contenenti il codice effettivo dei programmi
  Solana. Questi account memorizzano le istruzioni che gli utenti possono
  invocare.
...
questo campo per memorizzare lo stato del programma o il codice del programma
eseguibile.
...
Per gli account wallet e gli account che memorizzano lo stato del programma,
...
account veniva addebitato un "rent" (in lamport) per mantenere i propri dati
sulla rete.

Questo campo è attualmente inutilizzato, ma è incluso per retrocompatibilità.
...
Il Token Program è un account di programma eseguibile su Solana. Come gli
account dei wallet, i programmi hanno la stessa struttura dati sottostante
[Account](https://github.com/anza-xyz/agave/blob/v2.1.11/sdk/account/src/lib.rs#L48-L60),
...
Per il Token Program, l'`owner` è il programma BPFLoader2.
...
è un account di proprietà del Token Program. Memorizza i metadati globali per un
token specifico, inclusi la fornitura totale, il numero di decimali e gli
...
stato, non il codice eseguibile.
...
memorizza lo stato, non il codice eseguibile.
...
l'autorità di mint, la fornitura totale, il numero di decimali.

Per leggere da un mint account, devi deserializzare il campo `data` nel tipo di
dati `Mint`. Questo è trattato nel passaggio successivo.
...
Il campo `mintAuthority` mostra l'autorità che può coniare nuovi token.
...
Il campo `tlvData` contiene dati extra per Token Extensions (richiede ulteriore
deserializzazione).

Questo campo è rilevante solo per gli account creati dal Token Extension Program
(Token2022).
