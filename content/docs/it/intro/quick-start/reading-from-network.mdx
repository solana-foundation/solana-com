...
description:
  Impara come leggere dati dalla blockchain di Solana. Questa guida copre il
...
Questa sezione esplora come leggere dati dalla rete Solana recuperando diversi
...
per localizzare i dati corrispondenti on-chain.
...
account wallet e account di programma.
...
account wallet, i programmi hanno la stessa struttura di dati sottostante
[Account](https://github.com/anza-xyz/agave/blob/v2.1.11/sdk/account/src/lib.rs#L48-L60),
...
| `data`       | Per gli account di programma, questo campo memorizza il codice eseguibile del programma. Al contrario, gli account wallet hanno un campo dati vuoto.                                               |
| `owner`      | L'account è posseduto da un programma Loader, che è una categoria di programmi integrati che possiedono account di programma eseguibili su Solana. (`BPFLoader2111111111111111111111111111111111`) |

L'account del programma memorizza il bytecode compilato per il
...
Questo esempio recupera l'account Mint di USD Coin (USDC) per mostrare come i
...
account autorizzati a coniare o congelare token. L'indirizzo dell'account Mint
...
Questi sono i punti chiave da notare sui dati dell'account Mint:

| Campo        | Descrizione                                                                                                                    |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------ |
| `executable` | Questo campo è `false` perché l'account mint memorizza lo stato, non codice eseguibile.                                        |
...
| `owner`      | Il Token Program (`TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA`) possiede l'account mint.                                      |
