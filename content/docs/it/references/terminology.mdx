...
title: Terminologia
description:
  Impara la terminologia essenziale utilizzata nella blockchain Solana e nei
  modelli di sviluppo.
keywords:
  - termini
  - dizionario
...
chiave, spesso riferita come [chiave pubblica](#public-key-pubkey) o pubkey.
...
è capace di modificare l'account.
...
Il risultato dell'interpretazione di tutti i programmi nel registro a una data
[altezza di tick](#tick-height). Include almeno l'insieme di tutti gli
[account](#account) che contengono token nativi diversi da zero.

## blocco

Un insieme contiguo di [voci](#entry) sul registro coperte da un
...
Vedi [vote credit](#vote-credit).

## cross-program invocation (CPI)

Una chiamata da un [programma onchain](#onchain-program) a un altro. Per
maggiori informazioni, consulta [chiamate tra programmi](/docs/core/cpi).
...
Quando i nodi che rappresentano 2/3 dello [stake](#stake) hanno un [root](#root)
comune.
...
[validator](#validator) mappate ai [slot](#slot). Il cluster utilizza la
...
[blocco genesi](#genesis-block), ma il ledger di un effettivo
[validator](#validator) potrebbe contenere solo blocchi più recenti per ridurre
lo spazio di archiviazione, poiché quelli più vecchi non sono necessari per la
validazione dei blocchi futuri per design.
...
oltre a una promessa di non votare per un [blocco](#block) in conflitto \(cioè
...
Il codice eseguibile sulla blockchain di Solana che interpreta le
...
necessarie per l'esecuzione al fine di minimizzare le commissioni.
...
memorizzati permanentemente sulla chain fino alla chiusura dell'account. Non è
...
scalare nell'intervallo di [0, l-1]. Questo requisito garantisce che non ci sia
malleabilità della firma. Ogni transazione deve avere almeno una firma per
...
sequenzialmente e non sovrapposti, comprendendo un tempo reale
