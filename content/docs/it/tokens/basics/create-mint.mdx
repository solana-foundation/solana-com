...
Un mint account è un tipo di account nei Token Program di Solana che rappresenta
in modo univoco un token sulla rete e memorizza i metadati globali relativi al
token.
...
Ogni token su Solana esiste come mint account dove l'indirizzo del mint account
è il suo identificatore univoco sulla rete.

Per esempio, l'USD Coin (USDC) digital dollar su Solana ha l'indirizzo mint
`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`. Questo indirizzo mint serve come
identificatore univoco di USD Coin in tutto l'ecosistema Solana. Puoi
visualizzare i dettagli di questo mint account su
...
Per creare un Mint Account, invoca l'istruzione
...
Puoi trovare implementazioni di questa istruzione
[qui](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/processor.rs#L79).

La transazione per creare un mint account richiede due istruzioni:

1. Invocare il System Program per creare e allocare spazio per un mint account e
   trasferire la proprietà al Token Program.
2. Invocare il Token Program per inizializzare i dati del mint account.
