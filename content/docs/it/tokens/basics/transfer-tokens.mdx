...
Trasferire token implica spostare token da un token account a un altro token
account che condividono lo stesso mint. Il trasferimento di token avviene quando
si invoca l'istruzione
...
su un token program. Solo l'indirizzo specificato come proprietario (authority)
del token account di origine può trasferire token fuori dall'account.
