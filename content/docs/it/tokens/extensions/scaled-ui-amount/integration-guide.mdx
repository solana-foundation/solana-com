...
| Visualizzazione saldi token ribasati           | Mostrare i saldi scalati per il trading o la fornitura di liquidità sull'interfaccia utente. (il backend può comunque usare importi grezzi)     | **P0**   |
...
| Tracciamento interno dei saldi grezzi          | Tracciare i saldi grezzi per le transazioni onchain invece dei saldi scalati. Questo sarà più accurato e più facile da gestire a lungo termine. | **P1**   |
| Adattamento del feed dei prezzi                | Ovunque venga utilizzato un feed di prezzi per visualizzare il prezzo corrente, fornire il prezzo scalato agli utenti finali.                   | **P1**   |
