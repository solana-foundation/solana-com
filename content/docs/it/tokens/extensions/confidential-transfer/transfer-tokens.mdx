...
description:
  Scopri come trasferire token in modo privato da un token account a un altro.
---
...
confidenziali. Il token account del mittente deve inoltre avere un saldo
...
1. Crea
   ...
   **Prova di Uguaglianza
   ...
   trasferimento corrisponda al suo corrispondente
   [Pedersen commitment](https://en.wikipedia.org/wiki/Commitment_scheme),
   assicurando che il nuovo saldo disponibile dell'account di origine sia
   ...
   **Prova di Validità del Testo Cifrato
   ...
   assicurando che l'importo del trasferimento sia correttamente criptato sotto
   la chiave pubblica di ciascuna parte.
   ...
   **Prova di Intervallo
   ...
   (suddiviso in bit bassi/alti) siano tutti non negativi e all'interno di un
...
Il seguente diagramma mostra i passaggi coinvolti nel trasferimento di token dal
token account di un mittente al token account di un destinatario.
...
Per trasferire token in modo confidenziale da un token account a un altro, devi:

- Genera una prova di uguaglianza, una prova di validità del testo cifrato e una
  prova di intervallo lato client
- Invoca il programma di prova Zk ElGamal per verificare le prove e
  inizializzare gli account "context state"
- Invoca l'istruzione
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  fornendo i tre account di prova.
- Chiudi i tre account di prova per recuperare il rent.
...
Program clonato dalla mainnet usando il seguente comando. Devi avere la Solana
CLI installata per avviare il validator locale.
