...
I Confidential Transfer ti permettono di trasferire token tra account token
...
sono privati. Gli indirizzi degli account token rimangono pubblici.
...
al Token Extensions Program che ti permettono di trasferire token tra account
...
2. Crea account token con l'estensione confidential transfer per il mittente e
   il destinatario.
3. Conia token nell'account del mittente.
...
5. **Applica** il saldo in attesa del mittente al **saldo confidenziale
   disponibile**.
6. **Trasferisci** confidenzialmente i token dall'account token del mittente
   all'account token del destinatario.
...
<Cards>
  ...
  >
    Come trasferire confidenzialmente token tra account token
  </Card>
</Cards>

Il diagramma seguente mostra una sequenza dettagliata del flusso base per i
trasferimenti di token confidenziali:
...
L'esempio in Rust qui sotto mostra i passaggi per i trasferimenti di token
confidenziali corrispondenti al diagramma precedente. I passaggi sono separati
in funzioni di supporto per maggiore chiarezza.
...
<Callout type="info">
  ...
  validatore locale predefinito. Devi clonare il Token Extension Program dalla
...
## Istruzioni per i Confidential Transfer
...
| _rs`Deposit`_                       | Converte il saldo pubblico del token in saldo confidenziale in sospeso.                                                                                                             |
...
| _rs`EnableConfidentialCredits`_     | Consente a un token account di ricevere trasferimenti di token confidenziali.                                                                                                       |
