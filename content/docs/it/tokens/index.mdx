...
proprietà. I token su Solana sono chiamati SPL
...
Solana. Consulta la sezione [Nozioni base sui token SPL](/docs/tokens/basics)
per esempi di codice.

## Punti chiave

- I [Token Program](#token-program) contengono tutta la logica di istruzione per
  interagire con i token sulla rete (sia fungibili che non fungibili).

- Un [mint account](#mint-account) rappresenta un token specifico e memorizza i
  metadati globali sul token come la fornitura totale e l'autorità di conio
...
I Token Program contengono tutta la logica di istruzione per interagire con i
...
Playground, crea un wallet Playground e esegui il comando _sh`solana airdrop`_
nel terminale Playground. Puoi anche ottenere SOL devnet dal
[web faucet](https://faucet.solana.com/) pubblico.
...
Un nuovo mint account inizia con una fornitura pari a zero. Controlla la
fornitura attuale con:
...
Il comando _sh`create-account`_ crea un associated token account con il tuo
...
La creazione di un associated token account richiede un'istruzione che invoca l'
...
[Getting Started](/developers/guides/token-extensions/getting-started) delle
