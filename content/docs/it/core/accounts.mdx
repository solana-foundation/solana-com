...
Il seguente esempio recupera il Token Program account per mostrare che i program
account hanno lo stesso tipo base `Account`, eccetto che il campo `executable` è
impostato su `true`. Poiché i program account contengono codice eseguibile nel
loro campo dati, non deserializiamo i dati.
...
Quando distribuisci un programma Solana, viene memorizzato in un program
account. I program account sono di proprietà di un
[Loader Program](/docs/core/programs#loader-programs). Esistono diverse versioni
del loader, ma tutte tranne loader-v3 memorizzano il codice eseguibile
direttamente nel program account. Loader-v3 memorizza il codice eseguibile in un
"program data account" separato e il program account si limita a puntare ad
esso. Quando distribuisci un nuovo programma, la CLI di Solana utilizza per
impostazione predefinita la versione più recente del loader.
...
Su Solana, il codice eseguibile di un programma è memorizzato in un account
diverso rispetto allo stato del programma. Questo è simile a come i sistemi
operativi in genere hanno file separati per i programmi e i loro dati.

Per mantenere lo stato, i programmi definiscono istruzioni per creare account
separati di loro proprietà. Ciascuno di questi account ha un proprio indirizzo
univoco e può memorizzare qualsiasi dato arbitrario definito dal programma.
...
L'esempio seguente mostra come creare e recuperare un account mint token di
proprietà del programma Token 2022.
