...
[program](/docs/core/programs) distribuiti sulla rete Solana, dove ogni program
definisce il proprio set di istruzioni.
...
- Ogni istruzione richiede tre informazioni:
  1. L'indirizzo del program da invocare
  2. Gli account da cui l'istruzione legge o su cui scrive
  3. Eventuali dati aggiuntivi richiesti dall'istruzione (ad es., argomenti
     della funzione)
...
Un'[istruzione](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/instruction/src/lib.rs#L94)
...
- **Accounts**: Un elenco di account richiesti dall'istruzione. Per ciascun
  account, l'istruzione deve specificare il suo indirizzo, se deve firmare la
  transazione e se verrà modificato.
- **Data**: Un buffer di byte che indica al programma quale istruzione eseguire
...
![Formato della Transaction](/assets/docs/core/transactions/tx_format.png)

La struttura di un messaggio di transazione consiste in:

- [Message Header](/docs/core/transactions#message-header): Specifica il numero
  di account firmatari e di sola lettura.
- [Account Addresses](/docs/core/transactions#array-of-account-addresses): Un
  array di indirizzi di account richiesti dalle istruzioni nella transazione.
- [Recent Blockhash](/docs/core/transactions#recent-blockhash): Funge da
  timestamp per la transazione.
- [Instructions](/docs/core/transactions#array-of-instructions): Un array di
...
![Messaggio della Transaction](/assets/docs/core/transactions/legacy_message.png)

### Dimensione della Transaction

Le transazioni di Solana hanno un limite di dimensione di
[1232](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/packet/src/lib.rs#L29)
byte. Questo limite deriva dalla dimensione dell'Unità Massima di Trasmissione
(MTU) IPv6 di 1280 byte, meno 48 byte per gli header di rete (40 byte IPv6 + 8
byte header di frammento).
...
- Message: Header (3 byte), chiavi degli account (32 byte ciascuna), recent
  blockhash (32 byte) e istruzioni

![Formato della Transaction](/assets/docs/core/transactions/issues_with_legacy_txs.png)

### Message Header

L'
[header del messaggio](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/message/mod.rs#L97)
...
[Istruzioni](/docs/core/transactions#array-of-instructions) nei messaggi delle
transazioni.
...
indirizzi contiene. Gli indirizzi sono quindi ordinati in base ai loro
privilegi, come determinato dall'intestazione del messaggio.
...
Come l'array degli indirizzi degli account nel messaggio, inizia con una
lunghezza [compact-u16](/docs/core/transactions#compact-array-format) seguita
dai dati dell'instruction data. Ogni istruzione contiene:
...
- `recentBlockhash`: blockhash utilizzato per marcare temporalmente la
  transazione
