...
Prima, crea un nuovo progetto Rust usando il comando standard `cargo new` con il
flag `--lib`.
...
Successivamente, testa il programma usando il crate `litesvm`. Aggiungi le
...
<Callout type="info">
  Nota che il cluster su Solana Explorer deve essere impostato su localhost.
...
<Callout type="info">
  ...
  `./target/deploy/hello_world-keypair.json`) ed eseguire nuovamente `cargo
  build-sbf`, che genererà un nuovo file keypair.
</Callout>
