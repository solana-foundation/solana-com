...
Yksinkertaisuuden vuoksi voit käsitellä program accountia itse ohjelmana. Kun
kutsut ohjelman käskyjä, määrität program accountin osoitteen (jota yleisesti
kutsutaan "Program ID":ksi).

Seuraava esimerkki hakee Token Program -tilin osoittaakseen, että program
accounteilla on sama `Account` -perustyyppi, paitsi että `executable` -kenttä on
asetettu arvoon `true`. Koska program accountit sisältävät suoritettavaa koodia
datakentässään, emme pura datan rakennetta.
...
Kun otat Solana-ohjelman käyttöön, se tallennetaan program accountiin. Program
accountien omistaja on [Loader Program](/docs/core/programs#loader-programs).
Lataajasta on useita versioita, mutta kaikki paitsi loader-v3 tallentavat
suoritettavan koodin suoraan program accountiin. Loader-v3 tallentaa
suoritettavan koodin erilliseen "program data accountiin", ja program account
vain osoittaa siihen. Kun otat käyttöön uuden ohjelman, Solana CLI käyttää
oletusarvoisesti uusinta lataajan versiota.
...
ne ovat vain tavallisia program accounteja.
...
Älä sekoita näitä program data accounteja ohjelmien data accounteihin (katso
alla).
...
Toisin sanoen, datatilin luominen mukautetulle ohjelmalle vaatii kaksi vaihetta:
...
Seuraava esimerkki näyttää, miten luodaan ja haetaan Token Mint -tili, jonka
omistaa Token 2022 -ohjelma.
