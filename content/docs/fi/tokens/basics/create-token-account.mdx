...
Token account on tilityypi Solanan Token Programs -ohjelmissa, joka tallentaa
tietoja yksilön omistuksesta tiettyyn tokeniin (mint). Jokainen token account on
liitetty yhteen mintiin ja seuraa tietoja kuten token-saldoa ja omistajaa.
...
Pitääkseen hallussaan tietyn mintin tokeneita, käyttäjien täytyy ensin luoda
token account. Jokainen token account pitää kirjaa:

1. Tietystä mintistä (token-tyyppi, jonka yksiköitä token account sisältää)
2. Omistajasta (taho, jolla on oikeus siirtää tokeneita tililtä)

Tässä esimerkki USD Coin (USDC) -tokenista Solanassa:

- USD Coin mint -osoite: `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- Circlellä (USD Coinin liikkeellelaskija) on token account osoitteessa
  `3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
- Tämä token account sisältää vain USD Coin -tokenin (mint) yksiköitä
- Circlellä on omistajan valtuudet osoitteessa
  `7VHUFJHWu2CuExkJcJrzhQPJ2oygupTWkL2A2For4BmE` ja se voi siirtää näitä
  tokeneita
...
1. Token accountin "omistaja" - Tämä on osoite, joka on tallennettu token
   accountin tietoihin "owner"-kenttänä
   ...
   -tyypissä, jonka Token Program määrittelee. Omistaja voi siirtää, polttaa tai
   delegoida tokeneita tililtä. Tähän osoitteeseen viitataan joskus token
   accountin "valtuutena" (authority), jotta se erottuisi ohjelman omistajasta.

2. Ohjelman "omistaja" - Tämä viittaa ohjelmaan, joka omistaa tiliin liittyvät
   tiedot Solanassa. Token-tilien kohdalla tämä on aina joko Token Program tai
   Token Extension Program, kuten on määritelty Solanan perus
   [Account](https://github.com/anza-xyz/solana-sdk/blob/4e30766b8d327f0191df6490e48d9ef521956495/account/src/lib.rs#L51)
   -tyypin "owner"-kentässä.

Kun työskennellään token-tilien kanssa, "owner" viittaa tyypillisesti tahoon,
jolla on valtuudet käyttää tokeneita, ei ohjelmaan, joka omistaa tilin.

</Callout>

## Mikä on Associated Token Account?

Associated token account on token-tili, jonka osoite on Program Derived Address
(PDA), jonka on luonut
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program).
Voit ajatella associated token accountia käyttäjän oletusarvoisena token-tilinä
tietyn tokenin (mint) yksiköiden säilyttämiseen.

<Callout type="info">
  Termi "associated token accounts" koskee vain token-tilejä, jotka Associated
  Token Program luo.
</Callout>

Associated token accountit tarjoavat deterministisen tavan löytää käyttäjän
token- tili mille tahansa mintille. Voit tarkastella johtamisen toteutusta
[täältä](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/interface/src/address.rs#L56).
...
Tämä deterministinen johtaminen varmistaa, että jokaiselle lompakko-osoitteen ja
token-mintin yhdistelmälle on olemassa täsmälleen yksi associated token account
-osoite. Tämä lähestymistapa tekee käyttäjän token-tilin löytämisestä
yksinkertaista mille tahansa token-mintille, poistaen tarpeen seurata
token-tilien osoitteita erikseen.

<Callout type="info">
  Associated Token Program toimii apuohjelmana, joka luo token- tilejä
  deterministisillä osoitteilla (PDA). Kun luodaan associated token account,
  Associated Token Program tekee CPI:n (Cross-Program Invocation) joko Token
  Programiin tai Token Extension Programiin. Token program omistaa luodun tilin,
  jolla on sama `Account` -tyyppinen rakenne kuin token programissa on
  määritelty. Associated Token Program ei ylläpidä tilaa - se tarjoaa
  yksinkertaisesti standardoidun tavan luoda token-tilejä deterministiseen
  osoitteeseen, joka on PDA.
</Callout>

## Kuinka luoda token account

Luodaksesi Token Accountin, kutsu
...
-ohjetta. Tämän ohjeen toteutukset löydät
[täältä](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/processor.rs#L145).

Token accountin luomiseen tarvitaan kaksi ohjetta:

1. Kutsu System Programia luomaan ja allokoimaan tilaa token accountille sekä
   siirtämään omistajuus Token Programille.
2. Kutsu Token Programia alustamaan token accountin data.
...
Luodaksesi Associated Token Accountin, kutsu
...
-ohjetta. Tämän ohjeen toteutukset löydät
[täältä](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/program/src/processor.rs#L66).

Associated token accountin luomisohje kutsuu automaattisesti System Programia
luomaan token accountin ja kutsuu Token Programia alustamaan token accountin
datan. Tämä tapahtuu Cross Program Invocation (CPI) -toiminnon kautta.
