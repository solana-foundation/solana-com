...
- [Mint Account](#mint-account) edustaa tiettyä tokenia ja tallentaa globaalia
  metatietoa tokenista, kuten kokonaistarjontaa ja mint-oikeuksia (osoite, jolla
  on valtuudet luoda uusia tokenin yksiköitä).

- [Token Account](#token-account) seuraa yksittäistä tokenien omistusta tietylle
  mint account -tilille tietyn omistajan osalta.

- [Associated Token Account](#associated-token-account) on Token Account, joka
  luodaan osoitteella, joka johdetaan omistajan ja mint account -tilin
  osoitteista.
...
-tilin osoitteen perusteella. Tämä tili toimii globaalina laskurina tietylle
tokenille ja tallentaa tietoja kuten:

- **Tarjonta**: Tokenin kokonaistarjonta
- **Desimaalit**: Tokenin desimaalitarkkuus
- **Mint-valtuutus**: Tili, jolla on valtuudet luoda uusia tokenin yksiköitä,
  kasvattaen tarjontaa
- **Jäädytysvaltuutus**: Tili, jolla on valtuudet jäädyttää tokeneita token
  accountissa, estäen niiden siirtämisen tai polttamisen

![Mint Account](/assets/docs/core/tokens/mint-account.svg)

Mint accountiin tallennetut täydelliset tiedot sisältävät seuraavat:
...
seuratakseen yksittäisten tokenien omistusta. Token Account tallentaa tietoja
kuten:

- **Mint**: Token, jonka yksiköitä Token Account sisältää
- **Omistaja**: Tili, jolla on valtuudet siirtää tokeneita Token Accountista
- **Määrä**: Token Accountin tällä hetkellä sisältämien tokenien määrä
...
![Tilien suhde](/assets/docs/core/tokens/token-account-relationship.svg)

<Callout type="info">

Huomaa, että jokaisen Token Accountin tiedot sisältävät
---INLINE-CODE-PLACEHOLDER-0e68dde11825ce0d5ee0312ced864c3--- -kentän, joka
...
Associated Token Account luodaan osoitteella, joka on johdettu omistajan
...
transaktiomaksuihin. Ensimmäistä kertaa Solana Playgroundia käyttäville, luo
...
osoitteita Playground-päätteestäsi, kun seuraat mukana. Esimerkiksi
...
Uuden Mint Accountin luominen vaatii tapahtuman, jossa on kaksi ohjetta. Tässä
...
Komento _sh`create-account`_ luo associated token accountin, jossa lompakkosi
osoite on omistaja.
...
Uuden Token Accountin luomiseksi uudella keypairilla Associated Token Account
-osoitteen sijaan, lähetä transaktio kahdella ohjeella. Tässä Javascript-
esimerkki
...
_rs`MintTo`_ -ohje Token Programissa luo uusia tokeneita. Mint authorityn täytyy
allekirjoittaa transaktio. Ohje mintaa tokeneita Token Accountiin ja kasvattaa
kokonaistarjontaa Mint Accountissa. Tässä Javascript- esimerkki
...
_rs`Transfer`_ -ohje Token Programissa käsittelee tokenien siirtoja. Lähettäjän
Token Accountin omistajan täytyy allekirjoittaa transaktio. Ohje siirtää
tokeneita Token Accountien välillä. Tässä Javascript-esimerkki
...
Lisätietoja Token Extensions -ominaisuuksista löydät Token Extensions
