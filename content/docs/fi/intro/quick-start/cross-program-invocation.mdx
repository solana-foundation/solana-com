...
Ohjeita `update` ja `delete` täytyy muokata käsittelemään SOL-siirtoja tilien
välillä kutsumalla System Program -ohjelmaa.

Tämän osion tarkoituksena on käydä läpi CPI-kutsujen toteuttamisprosessi
Solana-ohjelmassa käyttäen Anchor-kehystä, rakentaen edellisessä osiossa
...
Aloita päivittämällä `lib.rs`-tiedosto tuodaksesi näkyvyysalueeseen kohteita
...
käyttäjältä, kun he päivittävät viestitiliään.
...
Tämä tili toimii ohjelman hallinnoimana "holvitilinä", joka vastaanottaa
SOL-tokeneita käyttäjiltä, kun he päivittävät viestejään.

Käyttämällä PDA:ta holvitiliä varten ohjelma luo ohjelman hallinnoiman tilin,
joka on ainutlaatuinen jokaiselle käyttäjälle, mahdollistaen varojen hallinnan
ohjelman logiikan sisällä.

---

Keskeiset näkökohdat `vault_account`:
...
- _rs`SystemAccount`_ -tyyppisenä System Program omistaa sen kuten tavalliset
  lompakkotilit

Tämä järjestely mahdollistaa ohjelman:

- Luoda ainutlaatuisia, deterministisiä osoitteita jokaiselle käyttäjän
  "holvitilille"
- Hallita varoja ilman tarvetta yksityiselle avaimelle transaktioiden
  allekirjoittamiseen.
...
_rs`transfer()`_ -funktio kutsuu System Programin siirto-ohjetta.
...
CPI suoritetaan ohjelman sisältä, mahdollistaen Solana-ohjelmien
yhteensopivuuden.
...
- CPI:ssä vaadittavat tilit (määritelty _rs`Transfer`_ -rakenteessa)
...
`transfer` -funktio kutsuu sitten siirto-ohjetta System Program -ohjelmassa,
välittäen:

- `cpi_context` (ohjelma ja tilit)
- Siirrettävän `amount` (1 000 000 lamport, eli 0,001 SOL)
...
määrittelet ohjelman, tilit ja instruction data tietylle ohjeelle jota
kutsutaan. Kun ohjelman `update` -ohje vastaanottaa kutsun, se kutsuu sisäisesti
System Program -ohjelman siirto-ohjetta.
...
Lisää myös `system_program`, koska siirron CPI vaatii System Program -ohjelman
kutsumista.
...
Seuraavaksi lisää CPI-logiikka `delete`ohjeeseen siirtääksesi SOL:ia
holvitilistä takaisin käyttäjän tilille.
...
_rs`signer_seeds`_ määrittää valinnaiset siemenet ja bump-siemenet, joita
käytetään PDA:n johtamiseen.

### !with_signer

_rs`with_signer()`_-metodi välittää allekirjoittajan siemenet CPI:n mukana.

Tämä mahdollistaa ohjelman "allekirjoittamisen" PDA:lle, joka on johdettu sen
ohjelmatunnuksesta.

Ohjeen käsittelyn aikana ajonaikainen ympäristö varmistaa, että annetut
allekirjoittajan siemenet johtavat oikein PDA:n osoitteeseen. Jos tämä
vahvistetaan, ajonaikainen ympäristö kohtelee kyseistä PDA-tiliä
allekirjoittajana CPI:n keston ajan.

</WithNotes>

Huomaa, että _rs`_ctx: Context<Delete>`_ muuttuu muotoon
_rs`ctx: Context<Delete>`_ käyttääkseen kontekstia funktion rungossa.
...
Poisto-ohjeessa toinen Cross Program Invocation (CPI) toteuttaa System Program
-ohjelman siirto-ohjeen. Tämä CPI osoittaa, miten tehdään siirto, joka vaatii
Program Derived Address (PDA) -allekirjoittajan.

Ensin määritellään allekirjoittajan siemenet holvi-PDA:lle:
...
_rs`Transfer`_-rakenne määrittää vaaditut tilit System Program -ohjelman
siirto-ohjeelle:
...
_rs`CpiContext`_ määrittää:

- Kutsuttavan ohjelman (System Program)
- Siirtoon liittyvät tilit (määritelty Transfer-rakenteessa)
- PDA:n allekirjoittajan siemenet
...
_rs`transfer()`_ -funktio kutsuu sitten siirto-ohjetta System Program
-ohjelmassa, välittäen:
...
System Program -ohjelman siirto-ohjetta, allekirjoittaen PDA:n puolesta
salliakseen kaikkien varojen siirron vault-tililtä takaisin käyttäjälle.
...
päivittäminen vaatii vain ohjelman jakamisen samalla ohjelmatunnuksella.

<Callout>

Varmista, että Playground-lompakossasi on devnet SOL -tokeneita. Hanki devnet
SOL -tokeneita [Solana Faucet](https://faucet.solana.com/) -palvelusta.
...
Ensin, lisää holvin PDA-johtaminen:
...
Päivitä sitten päivitysohje sisältämään [`vaultAccount`](mention:one)
...
Päivitä sitten poisto-ohje sisältämään [`vaultAccount`](mention:one)
...
löydät siirtokomentojen CPIt update- ja delete-komentojen sisällä.
...
[Ydinkäsitteet](/docs/core/accounts) -dokumentaatioon, joka tarjoaa
...
-sivustoon, josta löydät erilaisia esimerkkiohjelmia.
...
[Github-reposta](https://github.com/solana-developers/program-examples/tree/main/basics/hello-solana/anchor).

Napsauta `Import` -painiketta ja anna projektin nimi lisätäksesi sen Solana
Playground -projektiluetteloosi. Kun projekti on tuotu, kaikki muutokset
