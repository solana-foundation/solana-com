...
description:
  ...
  vaiheittainen opas näyttää, miten luodaan, päivitetään ja poistetaan
  lohkoketjussa olevia viestejä PDA-osoitteiden avulla, toteutetaan tilien
  validointi ja kirjoitetaan testejä. Täydellinen kehittäjille, jotka haluavat
  ymmärtää, miten PDA-osoitteita käytetään Solana-ohjelmissa.
...
_rs`user`_ -kenttä sisältää _rs`Pubkey`_ -tyypin, joka tunnistaa käyttäjän, joka
loi viestitilin.
...
_rs`bump`_ -kenttä tallentaa _rs`u8`_ -tyyppisen
["bump"-siemenen](/docs/core/pda#canonical-bump), jota käytetään ohjelman
johdannaisen osoitteen (Program Derived Address) luomiseen. Tämän arvon
...
- `user` - _rs`Pubkey`_ -tyyppi, joka tunnistaa käyttäjän, joka loi viestitilin.
- `message` - _rs`String`_ -tyyppi, joka sisältää käyttäjän viestin.
- `bump` - _rs`u8`_ -tyyppi, joka tallentaa
  ["bump"-siemenen](/docs/core/pda#canonical-bump) ohjelman johdannaisen
  osoitteen (PDA) luomista varten. Tämän arvon tallentaminen säästää
...
Tämä koodi määrittelee, mitä tietoja viestiin liittyvään tiliin tallennetaan.
Seuraavaksi lisäät ohjelman ohjeet.
...
_rs`Create`_ -rakenne seuraavasti:
...
_rs`mut`_ -rajoite määrittelee tilin muokattavaksi.
...
_rs`bump`_ -rajoite määrittelee bump seed -arvon PDA:lle.

Jos et määrittele arvoa, Anchor laskee sen automaattisesti.

### !payer

_rs`payer`_ -rajoite määrittelee, mikä tili maksaa uuden tilin luomisen.

### !space

_rs`space`_ -rajoite määrittelee, kuinka monta tavua varataan uuden tilin
tietokentälle.
...
_rs`Account<'info, T>`_ -tyyppi edellyttää, että tili vastaa määriteltyä
tyyppiä.
...
_rs`#[instruction(message: String)]`_ merkintä antaa _rs`Create`_ struktuurin
...
_rs`#[derive(Accounts)]`_ attribuutti Anchor-ohjelmassa merkitsee struktuureja,
jotka määrittelevät ohjeen vaatimat tilit.

Jokainen kenttä struktuurissa edustaa tiliä, joka validoidaan kahdella tavalla:

1. Tilin tyyppi (kuten _rs`Signer<'info>`_ tai _rs`Account<'info, T>`_), joka
   määrittää, millaista tiliä ohjelma odottaa
2. Valinnaiset rajoitukset (kuten _rs`#[account(mut)]`_ tai
...
Struktuurin kenttien nimet tarjoavat pääsyn tileihin ohjelmakoodissasi, mutta
eivät vaikuta validointiin. Sinun tulisi käyttää kuvaavia nimiä selkeyden
vuoksi.

Tässä esimerkissä _rs`Create`_ struktuuri määrittelee _rs`create`_ ohjeen
vaatimat tilit.
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - `init` rajoitus luo tilin ohjeen suorituksen aikana
   - `seeds` ja `bump` rajoitukset johtavat tilin osoitteen Program Derived
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - Taustalla `init` -rajoite kutsuu System Program -ohjelmaa luodakseen uuden
     tilin määritetyllä `space` -arvolla ja muuttaa omistajan nykyiseksi
     ohjelmaksi.
...
`seeds` ja `bump` -rajoitteet yhdessä määrittävät tilin osoitteen Program
...
käyttää oikeaa bump seed -arvoa. Anchor käyttää `seeds` ja `bump` -määrityksiä
PDA:n johtamiseen.
...
- Käyttäjän viesti (_rs`String`_): 4 tavua pituudelle + vaihtelevan pituinen
  viesti
...
erottelijalle, joka toimii tilin tyypin tunnisteena ja jonka Anchor luo
automaattisesti tiliä luotaessa.

_rs`String`_ -tyyppi tarvitsee 4 tavua merkkijonon pituuden tallentamiseen, ja
loput tavut sisältävät varsinaisen datan.
...
Funktio `create` toteuttaa logiikan uuden viestititilin datan alustamiseen. Se
...
2. Tilin tietojen alustaminen:
   ...
   - Asettaa PDA:n johtamiseen käytetyn `bump` -arvon, joka haetaan
     `ctx.bumps.message_account` -kohteesta.
...
1. _rs`user: Signer<'info>`_
   ...
   - Merkitty muokattavaksi (_rs`#[account(mut)]`_), koska se saattaa maksaa
     lisätilasta `message_account`:lle tarvittaessa
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - `realloc` -rajoite kutsuu System Program -ohjelmaa säätämään tilin
     datakokoa

Huomaa, että _rs`bump = message_account.bump`_ -rajoite käyttää bump seed
-arvoa, joka on tallennettu _rs`message_account`_:lle, sen sijaan että Anchor
laskisi sen uudelleen.
...
2. Päivittää tilin tiedot:
   - Käyttää `message_account` kontekstista.
...
tallennettu _rs`message_account`_ tiliin.
...
1. _rs`user: Signer<'info>`_
   ...
   - Edustaa käyttäjää, joka sulkee viestititiä
...
1. _rs`_ctx: Context<Delete>`_ - Tarjoaa pääsyn _rs`Delete`_ struktuurissa
...
_rs`Delete`_ struktuurissa hoitaa tilin sulkemisen.
...
Playground-lompakko yhdistyy oletuksena devnetiin. Varmista, että
...
miten PDA johdetaan Javascriptissä käyttäen samoja siemeniä, jotka on määritelty
ohjelmassa.
...
Ensin testitiedosto lähettää transaktion, joka kutsuu `create` -ohjetta ja
välittää viestin "Hello, World!".
...
Ensin testitiedosto lähettää tapahtuman, joka kutsuu `update` -ohjetta,
...
_ts`fetchNullable()`_ -metodi hakee verkkotiedot tililtä, jota ei välttämättä
ole olemassa.

Testitiedosto käyttää tätä, koska tili sulkeutuu ja tiedot eivät ole enää
saatavilla.
