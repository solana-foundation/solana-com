...
Jos nopea asennuskomento ei toimi, katso alla olevasta
[Asenna riippuvuudet](#install-dependencies) -osiosta ohjeet jokaisen
riippuvuuden asentamiseen erikseen.

Jos nopea asennuskomento suoritetaan onnistuneesti, siirry suoraan
...
[WSL](https://learn.microsoft.com/en-us/windows/wsl/install)** -järjestelmää
(Windows subsystem for Linux). Asenna kaikki muut riippuvuudet Linux-terminaalin
kautta.
...
Kun WSL on asennettu, asenna kaikki muut riippuvuudet Ubuntu (Linux)
-terminaalin kautta.
...
Agave on validator-asiakasohjelma [Anzalta](https://www.anza.xyz/), aiemmin
tunnettu nimellä Solana Labs validator client.
...
Katso lisätietoja
[tästä ongelmasta](https://github.com/coral-xyz/anchor/issues/3392).
...
Kun suoritat _shell`anchor test`_ uuden Anchor-projektin luomisen jälkeen
...
| -------- | --------------------------------------------------------------------------------------------------- | --------------------------- |
| MCP      | MCP-palvelin, johon voit yhdistää kursorilla parantaaksesi Solana-kehitystä tekoälyn avulla.        | https://mcp.solana.com/     |
...
Jos sinulla on jo tiedostojärjestelmän lompakko tallennettuna oletussijaintiin,
...
[testivalidaattorin](https://docs.anza.xyz/cli/examples/test-validator)
...
Kun käytät `localnet` parametria `cluster` komennossa `Anchor.toml`, Anchor
käynnistää automaattisesti paikallisen validatorin, ottaa ohjelmasi käyttöön,
suorittaa testit ja sitten pysäyttää validatorin.
