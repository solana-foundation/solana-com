...
ohjelmalla on oltava `entrypoint`, joka määrittää mistä ohjelman suoritus alkaa.
...
Seuraavaksi lisää seuraava katkelma tiedostoon `Cargo.toml`. Jos et sisällytä
...
voivat vapaasti luoda oman toteutuksensa sisäänkäyntipistefunktiosta.
...
Ohjelman tila määritellään Rust-struktuurien avulla, jotka kuvaavat ohjelmasi
tilien tietorakennetta. Voit määritellä useita struktuureja edustamaan erilaisia
tilityyppejä ohjelmaasi varten.

Kun työskentelet tilien kanssa, tarvitset tavan muuntaa ohjelmasi tietotyyppejä
tavuiksi ja takaisin tilin data-kentässä:

- Serialisointi: Tietotyyppien muuntaminen tavuiksi tallennettavaksi tilin
  data-kenttään
...
2. Tuo Borsh-traitit ja käytä derive-makroa toteuttaaksesi traitit
   struktuureillesi:
...
Lisää `CounterAccount` struktuuri `lib.rs` määritelläksesi ohjelman tilan. Tätä
...
Ajattele niitä ohjelmasi julkisina API:eina - ne määrittelevät, mitä toimintoja
käyttäjät voivat tehdä ollessaan vuorovaikutuksessa ohjelmasi kanssa.
...
Kun asiakas kutsuu ohjelmaasi, heidän täytyy tarjota instruction data (tavujen
puskurina), jossa:
...
Instruction datan (tavujen) muuntamiseksi enum-variantiksi on yleistä toteuttaa
apumetodi. Tämä metodi:

1. Erottaa ensimmäisen tavun saadakseen ohjevariantin
2. Tekee täsmäytyksen variantin perusteella ja jäsentää mahdolliset
   lisäparametrit jäljellä olevista tavuista
...
`process_<instruction_name>`, mutta voit vapaasti valita minkä tahansa
nimeämiskäytännön.

Lisää seuraava koodi tiedostoon `lib.rs`. Tämä koodi käyttää
`CounterInstruction` -enumia ja `unpack` -metodia, jotka määriteltiin
...
3. System Program, jota kutsutaan uuden tilin luomiseksi

Määrittääksemme ohjeen vaatimat tilit luomme iteraattorin `accounts` -siivulle
...
Tilien järjestys on tärkeä - kun rakennat ohjetta asiakaspuolella, tilit on
...
Kun tila on määritelty ja rent laskettu, luo tili kutsumalla System Program
-ohjelman `create_account` -ohjetta.

Solanassa uusia tilejä voi luoda vain System Program. Tiliä luotaessa määritämme
varattavien tavujen määrän ja uuden tilin ohjelmaomistajan. System Program:

1. Luo uuden tilin
2. Varaa määritetyn tilan tilin data-kentälle
3. Siirtää omistajuuden määritetylle ohjelmalle

Tämä omistajuuden siirto on tärkeä, koska vain tilin ohjelmaomistaja voi muokata
...
Kutsuaksemme System Program -ohjelmaa ohjelmamme ohjeesta, teemme Cross Program
Invocation (CPI) -kutsun käyttäen `invoke` -funktiota. CPI mahdollistaa yhden
ohjelman kutsua ohjeita muissa ohjelmissa - tässä tapauksessa System Program
-ohjelman `create_account` -ohjetta.
...
1. Luomalla uuden `CounterAccount` -rakenteen ohjeeseen annetulla
   `initial_value` -arvolla.
2. Hakemalla muutettavan viittauksen uuden tilin data-kenttään.
3. Sarjallistamalla `CounterAccount` -rakenteen tilin data-kenttään, tallentaen
   tehokkaasti `initial_value` -arvon tilille.
...
viittaamme `counter_account` -nimisenä, on suoritettavan ohjelman omistama.
...
- Puramme raakatavut `CounterAccount` -rakenteeksi
...
`--nocapture` nähdäksesi tulosteet.
...
Ensin määritellään testimoduuli ja tuodaan tarvittavat riippuvuudet:
...
Seuraavaksi määritetään testi käyttäen `ProgramTest`. Sitten luodaan uusi
keypair, jota käytetään alustettavan laskuritilin osoitteena, ja määritellään
laskurin alkuarvo.
...
Kun rakennetaan ohjetta, jokainen tili on annettava
...
- Luodaan instruction data variantilla 0 (`InitializeCounter`) ja alkuarvolla
- Rakennetaan ohje ohjelmatunnuksella, instruction datalla ja vaadituilla
  tileillä
- Lähetetään transaktio alustamisohjeella
- Tarkistetaan, että tili luotiin oikealla alkuarvolla
...
- Rakennetaan ohje ohjelmatunnuksella, instruction datalla ja vaadituilla
  tileillä
- Lähetetään transaktio kasvattamisohjeella
- Tarkistetaan, että tilin arvoa kasvatettiin oikeaan arvoon
...
lisäparametreja, data on yksinkertaisesti ohjeen variantti.
