...
base58 gecodeerde string (bijv.
---INLINE-CODE-PLACEHOLDER-305fdbe60787406539d252451aefb0d7---).

De relatie tussen het account en het adres werkt als een sleutel-waarde paar,
...
Hoewel publieke sleutels vaak worden gebruikt als accountadressen, ondersteunt
...
een program ID en optionele inputs (seeds).
...
- `data`: Een byte-array die willekeurige gegevens voor een account opslaat.
  Voor niet-uitvoerbare accounts bevat dit vaak status die bedoeld is om gelezen
  te worden. Voor program accounts (smart contracts) bevat dit de uitvoerbare
  programmacode. Het dataveld wordt vaak "accountgegevens" genoemd.
...
- `owner`: De program ID (publieke sleutel) van het programma dat eigenaar is
  van dit account. Alleen het eigenaarsprogramma kan de accountgegevens wijzigen
  of het lamports-saldo verminderen.
...
- [Overdracht / Toewijzing van programma-eigendom](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L113):
  Zodra het System Program een account heeft aangemaakt, kan het de aangewezen
  programma-eigenaar opnieuw toewijzen aan een ander program account. Zo nemen
...
van het program account (vaak de "Program ID" genoemd).
...
er nog steeds buffers, maar dit zijn gewoon normale program accounts.

### Program data account

Loader-v3 werkt anders dan alle andere BPF Loader-programma's. Het program
account bevat alleen het adres van een program data account, dat de eigenlijke
uitvoerbare code opslaat:

![Programma-data-account](/assets/docs/core/accounts/program-account-expanded.svg)

Verwar deze programma-data-accounts niet met de data-accounts van programma's
(zie hieronder).
...
Voor de eenvoud kun je het program account beschouwen als het programma zelf.
Wanneer je de instructies van een programma aanroept, specificeer je het adres
van het program account (vaak de "Program ID" genoemd).

Het volgende voorbeeld haalt het Token Program account op om te laten zien dat
program accounts hetzelfde basis `Account` type hebben, behalve dat het
`executable` veld is ingesteld op `true`. Omdat program accounts uitvoerbare
code in hun dataveld bevatten, deserialiseren we de data niet.
...
Wanneer je een Solana-programma implementeert, wordt het opgeslagen in een
program account. Program accounts zijn eigendom van een
[Loader Program](/docs/core/programs#loader-programs). Er zijn verschillende
versies van de loader, maar alle behalve loader-v3 slaan de uitvoerbare code
direct op in het program account. Loader-v3 slaat de uitvoerbare code op in een
apart "program data account" en het program account verwijst er alleen naar.
Wanneer je een nieuw programma implementeert, gebruikt de Solana CLI standaard
...
Om de staat te behouden, definiëren programma's instructies om afzonderlijke
accounts te maken die ze bezitten. Elk van deze accounts heeft zijn eigen unieke
adres en kan alle willekeurige gegevens opslaan die door het programma zijn
gedefinieerd.
...
Het volgende voorbeeld laat zien hoe je een Token Mint-account aanmaakt en
ophaalt dat eigendom is van het Token 2022-programma.
