...
Een mint account is een account type in Solana's Token Programs dat uniek een
token op het netwerk vertegenwoordigt en globale metadata over de token opslaat.
...
1. Roep het System Program aan om ruimte te creëren en toe te wijzen voor een
   mint account en draag het eigendom over aan het Token Program.
