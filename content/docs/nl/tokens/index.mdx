...
- [Token Programs](#token-program) bevatten alle instructielogica voor het
  interacteren met tokens op het netwerk (zowel fungibele als niet-fungibele).
...
- Een [Associated Token Account](#associated-token-account) is een Token Account
  gemaakt met een adres afgeleid van de eigenaar en mint account adressen.
...
Token Programs bevat alle instructielogica voor het interacteren met tokens op
het netwerk (zowel fungibele als niet-fungibele). Alle tokens op Solana zijn in
...
in eigendom van het Token Program. Dit account fungeert als een globale teller
voor een specifieke token en slaat gegevens op zoals:
...
bezitten, waarbij het wallet-adres is ingesteld als de eigenaar van het token
...
gemakkelijk is om het adres van een account te vinden.
...
Playground-wallet aan en voer de opdracht _sh`solana airdrop`_ uit in de
...
Voer _sh`spl-token --help`_ uit om alle beschikbare opdrachten te zien.
...
Meer voorbeelden vind je in de _sh`spl-token`_
...
je de stappen volgt. De opdracht _sh`create-token`_ geeft bijvoorbeeld een mint
...
Een nieuwe mint account begint met een voorraad van nul. Controleer de huidige
voorraad met:
...
Het aanmaken van een nieuwe mint account vereist een transactie met twee
...
Opmerking: `<TOKEN_ADDRESS>` specificeert de mint account voor het token
account.
...
- [Het Token Program aan te roepen](https://github.com/solana-program/associated-token-account/blob/9d94201e8158f06015ff80ad47fefac62a2ec450/program/src/processor.rs#L138-L161)
  om de token account data te initialiseren

Om een nieuw Token Account met een nieuwe keypair te maken in plaats van een
...
Om nieuwe eenheden van een token te maken, mint je tokens naar een Token
Account:
...
De _rs`Transfer`_ instructie op het Token Program verwerkt token overdrachten.
...
Met het Token Extensions Program kun je metadata (naam, symbool,
afbeeldingslink) direct op de Mint Account opslaan.
