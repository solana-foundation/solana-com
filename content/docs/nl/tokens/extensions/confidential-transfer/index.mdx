...
## Wat zijn Confidential Transfers?

Confidential transfers stellen je in staat om tokens tussen token accounts over
te maken zonder het overboekingsbedrag bekend te maken. Dit is nuttig voor
transacties waarbij privacy belangrijk is. Alleen de overboekingsbedragen en
...
- [Confidential Balances Cookbook](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Codevoorbeelden over het gebruik van de Confidential Transfer extensie
...
overmaken zonder het overboekingsbedrag bekend te maken.
...
De basisstroom van confidential token transfers is als volgt:
...
4. **Stort** het openbare saldo van de verzender naar het **confidential pending
   balance**.
5. **Pas** het pending balance van de verzender toe op het **confidential
   available balance**.
6. Maak vertrouwelijk **tokens over** van het token account van de verzender
   naar het token account van de ontvanger.
7. **Pas** het pending balance van de ontvanger toe op het **confidential
   available balance**.
8. **Neem** het confidential available balance van de ontvanger **op** naar het
   **public balance**.

Voor meer details over de stappen in de confidential transfer flow, zie de
...
tokenoverdrachten die overeenkomen met het voorgaande diagram. De stappen zijn
...
Extensions Program dat van het mainnet is gekloond met behulp van het volgende
commando. Je moet de Solana CLI geïnstalleerd hebben om de lokale validator te
gebruiken.
...
| _rs`EmptyAccount`_                  | Maakt de in behandeling zijnde en beschikbare vertrouwelijke saldi leeg om het sluiten van een token account mogelijk te maken.                                                          |
| _rs`Deposit`_                       | Zet openbaar tokensaldo om in een in behandeling zijnd vertrouwelijk saldo.                                                                                                              |
...
| _rs`ApplyPendingBalance`_           | Zet in behandeling zijnd saldo om in beschikbaar saldo na stortingen of overdrachten.                                                                                                    |
| _rs`EnableConfidentialCredits`_     | Staat een token account toe om vertrouwelijke tokenoverdrachten te ontvangen.                                                                                                            |
...
| _rs`EnableNonConfidentialCredits`_  | Staat een token account toe om openbare tokenoverdrachten te ontvangen.                                                                                                                  |
