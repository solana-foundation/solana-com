...
## Hoe je vertrouwelijk tokens overmaakt van de ene token account naar de andere

Om vertrouwelijk tokens over te maken van de ene token account naar de andere,
moeten zowel de verzender als de ontvanger token accounts hebben die
geconfigureerd zijn met de
_rs---INLINE-CODE-PLACEHOLDER-2915702bfb467dfedf86c5959c2822b4---_ status en
goedgekeurd zijn voor vertrouwelijke overboekingen. De token account van de
verzender moet ook een beschikbaar vertrouwelijk saldo hebben om over te maken.
...
1. Creëer
   ...
   **Gelijkheidsbewijs
   ...
   Verifieert dat de nieuwe beschikbare saldo-ciphertekst na de overboeking
   overeenkomt met de bijbehorende
   [Pedersen commitment](https://en.wikipedia.org/wiki/Commitment_scheme), wat
   verzekert dat het nieuwe beschikbare saldo van de bronaccount correct wordt
   berekend als `new_balance = current_balance - transfer_amount`.
   ...
   **Ciphertekst Validiteitsbewijs
   ...
   Verifieert dat de cipherteksten van het overboekingsbedrag correct zijn
   gegenereerd voor alle drie partijen (bron, bestemming en auditor), wat
   verzekert dat het overboekingsbedrag correct is versleuteld onder de publieke
   sleutel van elke partij.
   ...
   **Bereikbewijs
   ...
   Verifieert dat het nieuwe beschikbare saldo en overboekingsbedrag (gesplitst
...
4. Sluit de bewijs context state accounts om de SOL terug te krijgen die
   gebruikt is om ze aan te maken.

Het volgende diagram toont de stappen die betrokken zijn bij het overmaken van
tokens van een token account van een verzender naar een token account van een
ontvanger.
...
- Genereer een gelijkheidsbewijs, ciphertext-geldigheidsbewijs en bereikbewijs
  aan de clientzijde
- Roep het Zk ElGamal proof programma aan om de bewijzen te verifiëren en de
  "context state" accounts te initialiseren
- Roep de
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  instructie aan en voorzie de drie bewijs-accounts.
- Sluit de drie bewijs-accounts om rent terug te krijgen.

De `spl_token_client` crate biedt de volgende methoden:

- `confidential_transfer_create_context_state_account` methode die een
  bewijs-account aanmaakt.
- `confidential_transfer_transfer` methode die de `Transfer` instructie
  aanroept.
- `confidential_transfer_close_context_state_account` methode die een bewijs-
  account sluit.

### Voorbeeldcode

Het volgende voorbeeld laat zien hoe je vertrouwelijk tokens kunt overdragen
tussen token accounts.

Om het voorbeeld uit te voeren, start je een lokale validator met het Token
Extensions Program gekloond van mainnet met het volgende commando. Je moet de
...
<Callout type="info">
  ...
  de standaard lokale validator. Je moet het Token Extensions Program van
...
---CODE-PLACEHOLDER-c9c01a4c9b274a468c0c7cb1df8815ae---
