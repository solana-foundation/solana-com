...
extra gegevens toe te voegen aan het token account. Dit gedeelte legt uit hoe je
...
token account met de Confidential Transfer extensie:
...
- **pending_balance_lo**: De versleutelde lagere 16 bits van het in behandeling
  zijnde saldo. Het saldo wordt gesplitst in hoge en lage delen voor efficiënte
  ontsleuteling.

- **pending_balance_hi**: De versleutelde hogere 48 bits van het in behandeling
  zijnde saldo. Het saldo wordt gesplitst in hoge en lage delen voor efficiënte
  ontsleuteling.
...
- **pending_balance_credit_counter**: Telt inkomende in behandeling zijnde
  saldo-bijschrijvingen van stortings- en overdrachtsinstructies.

- **maximum_pending_balance_credit_counter**: De limiet voor het aantal in
  behandeling zijnde bijschrijvingen voordat een _rs`ApplyPendingBalance`_
  instructie vereist is om het in behandeling zijnde saldo om te zetten naar het
  beschikbare saldo.
...
<Callout type="info">
  **Pending vs beschikbaar saldo**

Vertrouwelijke saldi worden gescheiden in pending (in behandeling zijnde) en
beschikbare saldi om DoS-aanvallen te voorkomen. Zonder deze scheiding zou een
aanvaller herhaaldelijk tokens naar een token account kunnen sturen, waardoor de
eigenaar van het token account geen tokens meer kan overmaken. De eigenaar van
het token account zou geen tokens kunnen overmaken omdat het versleutelde saldo
zou veranderen tussen het moment waarop de transactie wordt ingediend en wanneer
deze wordt verwerkt, wat zou resulteren in een mislukte transactie.

Alle stortingen en overboekingsbedragen worden in eerste instantie toegevoegd
aan het pending saldo. Eigenaren van token accounts moeten de
_rs`ApplyPendingBalance`_ instructie gebruiken om het pending saldo om te zetten
naar het beschikbare saldo. Inkomende overboekingen of stortingen hebben geen
invloed op het beschikbare saldo van een token account.

</Callout>

<Callout type="info">
  **Pending saldo hoog/laag splitsing**

Het vertrouwelijke pending saldo is opgesplitst in `pending_balance_lo` en
...
<Callout type="info">
  **Pending saldo credit counters**

Bij het aanroepen van de _rs`ApplyPendingBalance`_ instructie om het pending
saldo om te zetten naar het beschikbare saldo:

1. De client zoekt de huidige pending en beschikbare saldi op, versleutelt de
   som, en levert een `decryptable_available_balance` die is versleuteld met de
   AES-sleutel van de eigenaar van het token account.

2. De verwachte en werkelijke pending credit counters houden wijzigingen bij in
   de counter waarde tussen het moment waarop de _rs`ApplyPendingBalance`_
...
verwachte/werkelijke tellerwaardes dat de client recente
stortings-/overdrachtsinstructies opzoekt die overeenkomen met het
tellerverschil om het correcte saldo te berekenen.

**Proces voor saldoafstemming**

Wanneer de verwachte en werkelijke saldotellers verschillen, volg je deze
stappen om de `decryptable_available_balance` af te stemmen:

1. Begin met de `decryptable_available_balance` uit het token account
2. Haal de meest recente transacties op, inclusief stortings- en overdrachts-
   instructies tot aan het tellerverschil (werkelijk - verwacht):
   - Voeg openbare bedragen van stortingsinstructies toe
   - Decodeer en voeg bestemmings-ciphertext bedragen van overdrachtsinstructies
     toe
...
2. **Hertoewijzen van accountruimte**: Roep de
   _rs`TokenInstruction::Reallocate`_ instructie van het Token Extensions
   Program aan om ruimte toe te voegen voor de _rs`ConfidentialTransferAccount`_
   status.

3. **Configureer vertrouwelijke overdrachten**: Roep de Token Extensions
   Program's
