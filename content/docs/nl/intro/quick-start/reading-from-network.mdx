...
description:
  ...
  behandelt het ophalen van wallet-accounts, programma-accounts en token mint
...
- **Executable programma's**: Accounts die de daadwerkelijke code van Solana-
...
programma-accounts te demonstreren.
...
| `data`       | Voor programma-accounts slaat dit veld de uitvoerbare code van het programma op. Wallet-accounts hebben daarentegen een leeg dataveld.                                                            |
| `owner`      | Het account is eigendom van een Loader-programma, een categorie van ingebouwde programma's die uitvoerbare programma-accounts op Solana bezitten. (`BPFLoader2111111111111111111111111111111111`) |

De programma-account slaat de gecompileerde bytecode op voor de broncode van het
Token Program
[broncode](https://github.com/solana-program/token/tree/main/program). Je kunt
deze programma-account bekijken op de
...
Dit voorbeeld haalt de USD Coin (USDC) Mint-account op om te laten zien hoe
...
account is een account die eigendom is van het Token Program. Het slaat globale
metadata op voor een specifieke token, inclusief de totale voorraad, aantal
decimalen en de accounts die geautoriseerd zijn om tokens te minten of te
bevriezen. Het adres van de mint account identificeert op unieke wijze een token
op het Solana-netwerk.
...
Dit zijn de belangrijkste punten over de gegevens van de mint account:

| Veld         | Beschrijving                                                                                                |
| ------------ | ----------------------------------------------------------------------------------------------------------- |
| `executable` | Dit veld is `false` omdat de mint account status opslaat, geen uitvoerbare code.                            |
| `data`       | Dit veld bevat de geserialiseerde accountstatus, zoals de mint authority, totale voorraad, aantal decimalen |
| `owner`      | Het Token Program (`TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA`) is eigenaar van de mint account.          |
...
| `decimals`        | Het aantal decimalen voor de token                                    |
