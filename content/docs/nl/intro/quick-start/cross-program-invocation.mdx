...
description:
  ...
  Solana-programma's met behulp van het Anchor-framework. Deze tutorial laat
  zien hoe je SOL kunt overdragen tussen accounts, kunt communiceren met het
  System Program en Program Derived Addresses (PDAs) kunt verwerken in CPIs.
  Perfect voor ontwikkelaars die composable Solana-programma's willen bouwen.
...
door de _rs`Update`_ struct en `update` functie aan te passen.
...
`vault_account` toe te voegen. Dit account, beheerd door het programma, ontvangt
...
overdrachts-instructie van het System Program.
...
De _rs`transfer()`_ functie roept de overdrachts-instructie van het System
Program aan.
...
toont aan hoe je een CPI uitvoert vanuit het programma, waardoor de
samenstellingsmogelijkheden van Solana-programma's mogelijk worden.

De _rs`Transfer`_ struct specificeert de vereiste accounts voor de
transfer-instructie van het System Program:

- `from` - De account van de gebruiker (bron van fondsen)
- `to` - De vault-account (bestemming van fondsen)
...
Program, waarbij het volgende wordt doorgegeven:

- De `cpi_context` (programma en accounts)
- De `amount` om over te maken (1.000.000 lamports, of 0,001 SOL)
...
De setup voor een CPI komt overeen met hoe client-side instructies worden
opgebouwd, waarbij je het programma, de accounts en de instruction data
...
transfer-instructie van het System Program aan.
...
Voeg nu een "restitutie bij verwijdering"-mechanisme toe door de _rs`Delete`_
struct en `delete` functie te wijzigen.

Update eerst de _rs`Delete`_ struct om de `vault_account` op te nemen. Dit maakt
het mogelijk om SOL in de vault terug te storten naar de gebruiker wanneer ze
hun berichtaccount sluiten.
...
verwijderinstructie om opgebouwde SOL terug te sturen naar de gebruiker.
...
Tijdens de verwerking van instructies verifieert de runtime dat de opgegeven
...
In de verwijderinstructie implementeert een andere Cross Program Invocation
(CPI) de overdrachtsinstructie van het System Program. Deze CPI demonstreert hoe
...
De _rs`transfer()`_ functie roept vervolgens de transfer instructie aan op het
System Program, waarbij het doorgeeft:
...
fondsen te beheren. Wanneer de delete instructie van het programma wordt
aangeroepen, roept het intern de transfer instructie van het System Program aan,
waarbij het tekent voor de PDA om de overdracht van alle fondsen van de vault
terug naar de gebruiker mogelijk te maken.
...
Alleen de upgrade authority van het programma kan het bijwerken. De ontwikkelaar
stelt de upgrade authority in tijdens de implementatie van het programma, en het
blijft het enige account met toestemming om het programma te wijzigen of te
sluiten. Als iemand de upgrade authority intrekt, dan wordt het programma
onveranderlijk.

Bij het implementeren van programma's op Solana Playground fungeert je
Playground wallet als de upgrade authority voor al je programma's.
