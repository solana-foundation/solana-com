...
Dit gedeelte behandelt de stappen om je lokale omgeving op te zetten voor
Solana- ontwikkeling.
...
Na het installeren van WSL, installeer je de afhankelijkheden die in de
...
Als je dit probleem tegenkomt, open Windows Terminal door te zoeken naar
"Terminal" in de zoekbalk.

![Windows Terminal](/assets/docs/intro/installation/wsl-windows-terminal.png)

Sluit vervolgens Windows Terminal en open opnieuw een Linux-terminal door te
zoeken naar Ubuntu. De terminal zou er nu uit moeten zien zoals de afbeelding
hieronder, waar `ctrl + v` (sneltoets voor plakken) werkt.
...
afhankelijkheden die zijn gespecificeerd in het Linux-gedeelte hieronder voordat
je verdergaat met de individuele stappen voor het installeren van Rust, Solana
CLI en Anchor CLI.
...
zodat Cargo's bin-directory wordt opgenomen:
...
`export PATH` commando dat bij de installatie wordt gelogd niet behouden zodra
...
Om te verifiëren dat de installatie is geslaagd, controleer de Solana
CLI-versie:
...
Om de Solana CLI later bij te werken naar de nieuwste versie, kun je het
...
1. Installeer de afhankelijkheden die staan vermeld in het
...
is beschikbaar via _shell`anchor init --test-template rust`_)
...
Als je bij het uitvoeren van `anchor build` de volgende fouten tegenkomt:
...
De RPC URL en Websocket URL specificeren de Solana cluster waarmee de CLI
verbinding maakt.

Je kunt de Solana CLI cluster updaten met de volgende commando's:
...
Bijvoorbeeld, om een project te maken met de naam `my-project`, voer uit:
...
nieuw Anchor project met een basis Rust programma en TypeScript test template.
...
Dit commando deployt je programma naar de `cluster` die is gespecificeerd in het
...
Dit commando bouwt, deployt en voert de tests uit voor je project.

Wanneer je `localnet` gebruikt als `cluster` in `Anchor.toml`, start Anchor
automatisch een lokale validator, deployt je programma, voert tests uit en stopt
vervolgens de validator.
