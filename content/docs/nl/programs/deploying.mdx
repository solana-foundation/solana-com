...
- online ondertekenaar (betaler van kosten en autoriteit van het buffer account)
- offline ondertekenaar (autoriteit van het program account)
...
3. (online) maak een buffer aan en upload een uitvoerbaar bestand
4. (optioneel) verifieer de on-chain inhoud van de buffer
...
Zoek een recente `blockhash` op en plak deze in om de offline transactie-
handtekening te genereren. De `blockhash` verloopt na ongeveer 60 seconden. Als
je het niet op tijd hebt gedaan - haal gewoon een nieuwe hash op en herhaal tot
je slaagt, of overweeg om duurzame transactie-nonces te gebruiken.
