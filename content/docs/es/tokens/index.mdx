...
Ten en cuenta que los datos de cada Token Account incluyen un campo
`owner` que identifica
quién tiene autoridad sobre la Token Account. Esto difiere del propietario del
programa especificado en el tipo base
...
La CLI _sh`spl-token`_ te
ayuda a experimentar con tokens SPL. Los ejemplos a continuación utilizan el
terminal de [Solana Playground](https://beta.solpg.io/) para ejecutar comandos
CLI directamente en el navegador sin instalación local.
...
wallet de Playground y ejecuta el comando
_sh`solana airdrop`_ en el
terminal de Playground. También puedes obtener SOL de devnet desde el
...
Ejecuta _sh`spl-token --help`_ para
...
[documentación](https://www.solana-program.com/docs/token) de
_sh`spl-token`_.
...
los pasos. Por ejemplo, el comando
_sh`create-token`_ muestra una
dirección de cuenta mint con tu wallet de Playground como autoridad del mint.
...
El comando _sh`supply`_
para un nuevo token devuelve
`0`:
...
El comando _sh`create-account`_
crea una associated token account con la dirección de tu wallet como
propietario.
...
Nota: `<TOKEN_ADDRESS>` especifica
la mint account para la token account.
...
La instrucción
_rs`MintTo`_ en el Token
Program crea nuevos tokens. La autoridad de acuñación debe firmar la
transacción. La instrucción acuña tokens a una Token Account y aumenta el
suministro total en la Mint Account. Aquí hay un ejemplo en Javascript en
...
La instrucción
_rs`Transfer`_ en el Token
Program maneja las transferencias de tokens. El propietario de la Token Account
del remitente debe firmar la transacción. La instrucción mueve tokens entre
Token Accounts. Aquí hay un ejemplo en Javascript en
