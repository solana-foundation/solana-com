...
una fecha futura, puedes programar automáticamente un sondeo en este momento de
actualización
...
- Los usuarios deben ingresar cantidades que se interpretarán como "UIAmount"
  ...
  - Si hay problemas de redondeo, redondea hacia abajo y es preferible dejar una
    pequeña cantidad de dust antes que arriesgarse a que la transacción falle
...
- Las aplicaciones deben usar la cantidad bruta total cuando un usuario solicita
  realizar una acción con el "máximo" o "todo" su saldo. Esto asegura que no
  quede dust.
...
| :------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------- |
| Soportar acciones de usuario usando UiAmount | Todas las acciones del usuario deben ingresarse en UiAmount cuando UiAmount está habilitado en toda la aplicación. Si UiAmount no es visible en la aplicación, deben usar cantidades brutas hasta que la aplicación se actualice. | **P0**    |
...
| Requisito                               | Descripción                                                                                                                                   | Prioridad |
| :-------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------- | :-------- |
| Mostrar saldos de tokens rebasados      | Mostrar saldos escalados para operaciones o provisión de liquidez en la UI. (el backend puede seguir usando cantidades brutas)                | **P0**    |
