...
<Callout type="info">
  ...
  puedes combinar la extensión
  _rs`NonTransferable`_ con la
  extensión _rs`TransferFeeConfig`_,
...
El enum _rs`ExtensionType`_ se
...
el `tlv_data` (que contiene
el estado de la extensión) de acuerdo con los tipos de extensión específicos
habilitados para esa cuenta.
