...
<Callout type="info">
  ...
  simultáneamente en la misma acuñación de token o token account. Por ejemplo,
  no puedes combinar la extensión _rs`NonTransferable`_ con la extensión
...
una acuñación o token account. Todo el estado específico de la extensión se
...
el `tlv_data` (que contiene el estado de la extensión) según los tipos de
extensión específicos habilitados para esa cuenta.
