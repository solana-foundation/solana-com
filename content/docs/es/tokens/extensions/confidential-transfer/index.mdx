...
description:
  ...
  añadir características opcionales a los mints y cuentas de tokens.
---

## ¿Qué son las Transferencias Confidenciales?

Las transferencias confidenciales te permiten transferir tokens entre cuentas
sin revelar el monto de la transferencia. Esto es útil para transacciones que
preservan la privacidad. Solo los montos de transferencia y los saldos de tokens
son privados. Las direcciones de las cuentas de tokens siguen siendo públicas.
...
- [Recetario de Saldos Confidenciales](https://github.com/solana-developers/Confidential-Balances-Sample) -
  Fragmentos de código sobre cómo usar la extensión de Transferencia
  Confidencial

### ¿Cómo funciona?

La extensión de Transferencia Confidencial añade
...
al programa Token Extensions que te permite transferir tokens entre cuentas sin
...
Para ejecutar el ejemplo, inicie un validador local con el Token Extensions
Program clonado desde la red principal utilizando el siguiente comando. Debe
tener instalado Solana CLI para usar el validador local.
...
<Callout type="info">
  ...
  habilitadas en el validador local predeterminado. Debe clonar el Token
...
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _rs`InitializeMint`_                | Configura la cuenta mint para transferencias confidenciales. Esta instrucción debe incluirse en la misma transacción que la instrucción _rs`TokenInstruction::InitializeMint`_.  |
...
| _rs`ApproveAccount`_                | Aprueba una token account para transferencias confidenciales si el mint requiere aprobación para nuevas token accounts.                                                          |
