...
- **maximum_pending_balance_credit_counter**: El límite de recuentos de créditos
...
pendiente. Los propietarios de token accounts deben usar la instrucción
...
2. Los contadores de crédito pendiente esperado y real rastrean los cambios en
   ...
   - `expected_pending_balance_credit_counter`: El valor
     ...
     _rs`ApplyPendingBalance`_
   - `actual_pending_balance_credit_counter`: El valor
     `pending_balance_credit_counter` en la token account en el momento en que
...
Cuando se obtiene el estado de una token account para leer el
...
1. **Crear la Token Account**: Invoca la instrucción
...
por el propietario de la token account.
