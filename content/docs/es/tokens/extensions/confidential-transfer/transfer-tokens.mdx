...
estado _rs`ConfidentialTransferAccount`_ y
aprobadas para transferencias confidenciales. La cuenta de token del remitente
también debe tener un saldo confidencial disponible para transferir.
...
1. Crea
   ...
   **Prueba de rango
   ...
   en bits bajos/altos) sean todos no negativos y dentro de un rango específico.
...
tokens desde una cuenta de token del remitente a una cuenta de token del
destinatario.
...
El crate `spl_token_client`
proporciona los siguientes métodos:

- Método `confidential_transfer_create_context_state_account` que crea
  una cuenta de prueba.
- Método `confidential_transfer_transfer` que
  invoca la instrucción
  `Transfer`.
- Método `confidential_transfer_close_context_state_account` que
  cierra una cuenta de prueba.

### Código de ejemplo

El siguiente ejemplo demuestra cómo transferir tokens de manera confidencial
entre cuentas de token.
