...
1. El "propietario" de la token account - Esta es una dirección almacenada en
   ...
   "autoridad" de la token account para distinguirla del propietario del
   programa.

2. El "owner" (propietario) del programa - Esto se refiere al programa que posee
   los datos de la cuenta en Solana. Para token accounts, este siempre es el
...
<Callout type="info">
  ...
  al Token Program o Token Extension Program. El token program posee la cuenta
  creada que tiene la misma estructura de tipo `Account` según se define en el
