...
Transferir tokens implica mover tokens de una token account a otra token account
que comparten el mismo mint. La transferencia de tokens ocurre cuando invocas la
instrucción
...
en un Token Program. Solo la dirección especificada como propietario (authority)
de la token account de origen puede transferir tokens desde la cuenta.
