...
description: Aprende cómo transferir tokens entre cuentas de token.
---

## Cómo transferir tokens

Transferir tokens implica mover tokens de una cuenta de token a otra cuenta de
token que comparten el mismo mint. La transferencia de tokens ocurre cuando
invocas la instrucción
...
en un programa de token. Solo la dirección especificada como propietario
(autoridad) de la cuenta de token de origen puede transferir tokens desde esa
cuenta.
