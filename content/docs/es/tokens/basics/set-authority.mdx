...
description: Aprende cómo establecer autoridad para mints o token accounts.
---
...
- **Autoridad de mint**: Controla la creación de nuevos tokens. Puede acuñar
  tokens en cualquier token account. A menudo se revoca después de crear el
  suministro inicial para establecer un token con suministro fijo.
- **Autoridad de congelación**: Controla la capacidad de congelar y descongelar
  token accounts. Puede impedir que cualquier token account transfiera tokens. A
  menudo se revoca para garantizar a los usuarios que sus tokens no pueden ser
  congelados.

### Autoridades de token account

- **Propietario de la cuenta**: Tiene control total sobre el token account.
  Puede transferir tokens, quemar tokens, aprobar delegados y cerrar la cuenta
  cuando el saldo es cero.
- **Autoridad de cierre**: Puede cerrar el token account cuando su saldo es
  cero. Por defecto, este es el propietario de la cuenta, pero puede delegarse a
  otra cuenta.
...
cambia o revoca autoridades en mints y token accounts. Solo la autoridad actual
puede transferir sus permisos a una nueva dirección o revocarlos permanentemente
estableciendo la autoridad como nula. Una vez revocada, la autoridad no puede
ser restaurada.
