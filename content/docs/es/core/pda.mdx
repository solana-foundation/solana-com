...
Ten en cuenta que los bump seeds 253-251 derivan PDAs válidas con diferentes
direcciones. Esto significa que dados los mismos seeds opcionales y `programId`,
un bump seed con un valor diferente todavía puede derivar una PDA válida.
...
cuenta usando una PDA como dirección de la cuenta. La nueva cuenta almacena la
dirección del `user` y el seed `bump` utilizado para derivar la PDA.
...
En este ejemplo, los seeds para la derivación de PDA incluyen la cadena fija
`data` y la dirección de la cuenta `user` proporcionada en la instrucción. El
framework Anchor encuentra automáticamente el seed `bump` canónico.
