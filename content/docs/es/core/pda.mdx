...
El programa de ejemplo a continuación muestra cómo crear una cuenta usando un
PDA como la dirección de la nueva cuenta. El programa de ejemplo utiliza el
[framework Anchor](https://www.anchor-lang.com/docs).

El programa incluye una única instrucción `initialize` para crear una nueva
cuenta usando un PDA como la dirección de la cuenta. La nueva cuenta almacena la
dirección del `user` y el seed `bump` utilizado para derivar el PDA.
...
framework Anchor encuentra automáticamente el seed canónico `bump`.
...
una nueva cuenta utilizando el PDA como dirección. Anchor hace esto a través de
un [CPI](/docs/core/cpi).
...
crear una nueva cuenta en la cadena utilizando el PDA como dirección. En este
