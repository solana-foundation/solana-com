...
datos comúnmente se denomina "datos de cuenta".
...
son propiedad de un [Loader Program](/docs/core/programs#loader-programs).
...
El siguiente ejemplo obtiene el Token Program account para mostrar que los
program accounts tienen el mismo tipo base `Account`, excepto que el campo
`executable` está configurado como `true`. Como los program accounts contienen
código ejecutable en su campo de datos, no deserializamos los datos.
...
[Loader Program](/docs/core/programs#loader-programs). Hay varias versiones del
loader, pero todas excepto loader-v3 almacenan el código ejecutable directamente
en el program account. Loader-v3 almacena el código ejecutable en una "cuenta de
datos del programa" separada y el program account simplemente apunta a esta.
Cuando implementas un nuevo programa, la CLI de Solana utiliza la última versión
del loader por defecto.
