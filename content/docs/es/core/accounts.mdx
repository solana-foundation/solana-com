...
- `data`: Un array de
  bytes que almacena datos arbitrarios para una cuenta. Para cuentas no
  ejecutables, esto a menudo almacena el estado que se pretende leer. Para
  cuentas de programa (contratos inteligentes), contiene el código de programa
  ejecutable. El campo de datos es comúnmente llamado "datos de cuenta".
- `executable`: Esta bandera
  muestra si una cuenta es un programa.
- `lamports`: El saldo de la
  cuenta en lamports, la unidad más pequeña de SOL (1 SOL = 1 billón de
  lamports).
- `owner`: El ID del
  programa (clave pública) del programa que posee esta cuenta. Solo el programa
  propietario puede cambiar los datos de la cuenta o deducir su saldo de
  lamports.
- `rent_epoch`: Un campo
  heredado de cuando Solana tenía un mecanismo que periódicamente deducía
  lamports de las cuentas. Aunque este campo todavía existe en el tipo Account,
  ya no se utiliza desde que la recolección de rent fue descontinuada.
...
No confundas estas cuentas de datos de programa con las cuentas de datos de los
...
cuentas. Una vez que el Programa del Sistema crea una cuenta, puede luego
transferir o asignar la propiedad de la nueva cuenta a otro programa.
