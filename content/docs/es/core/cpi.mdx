...
- Las **Cross Program Invocations** permiten que las instrucciones de programas
  de Solana invoquen directamente instrucciones en otro programa.
- Los **privilegios de firmante** de un programa que llama se extienden al
  programa llamado.
- Al realizar una Cross Program Invocation, **los programas pueden firmar en
...
Una Cross Program Invocation (CPI) es cuando un programa invoca las
instrucciones de otro programa.
...
- **Datos de instrucción**: Especifica qué instrucción invocar en el programa,
...
- El Programa B incluso puede pasar estos mismos permisos si realiza sus propios
  CPIs
...
`invoke_signed` con un array
`signers_seeds` vacío, indicando
que no se requieren PDAs para firmar.
...
Los siguientes ejemplos muestran cómo hacer un CPI usando el
...
- Ejemplo 1: Utiliza
  _rs`CpiContext`_ de Anchor y
  función auxiliar para construir la instrucción CPI.
- Ejemplo 2: Utiliza la función
  _rs`system_instruction::transfer`_ del crate
  `solana_program` para construir
  la instrucción CPI. El ejemplo 1 abstrae esta implementación.
- Ejemplo 3: Construye la instrucción CPI manualmente. Este enfoque es útil
  cuando no existe un crate que ayude a construir la instrucción.
...
El siguiente ejemplo muestra cómo hacer un CPI desde un programa escrito en Rust
nativo. El programa incluye una única instrucción que transfiere SOL de una
...
derivación de firmantes PDA como
`signer_seeds`.
...
utilizando el `signers_seeds` y
el `program_id` del programa
que realiza la llamada. Cuando se verifica un PDA válido, la dirección es
[añadida como firmante válido](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/bpf_loader/src/syscalls/cpi.rs#L554).

Los siguientes ejemplos demuestran cómo hacer un CPI con firmantes PDA
...
- Ejemplo 1: Utiliza
  _rs`CpiContext`_ de Anchor y
  una función auxiliar para construir la instrucción CPI.
- Ejemplo 2: Utiliza la función
  _rs`system_instruction::transfer`_ del crate
  `solana_program` para construir
  la instrucción CPI. El Ejemplo 1 es una abstracción de esta implementación.
- Ejemplo 3: Construye la instrucción CPI manualmente. Este enfoque es útil
