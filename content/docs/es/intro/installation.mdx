...
Si tu terminal de Ubuntu se ve como la imagen de abajo, es posible que
...
Puedes reemplazar `stable` con la etiqueta de lanzamiento que coincida con la
...
siguiente mensaje pidiéndote que agregues una variable de entorno `PATH`:
...
Cuando usas `localnet` como `cluster` en `Anchor.toml`, Anchor automáticamente
