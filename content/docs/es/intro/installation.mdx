...
- Los usuarios de Windows primero deben instalar WSL (Subsistema de Windows para
  ...
  a continuación.
- Los usuarios de Linux deben comenzar con las dependencias especificadas en la
  sección de Linux a continuación.
...
encuentres un problema donde
`ctrl + v` (atajo de teclado
para pegar) no funcione en la terminal.
...
donde `ctrl + v` (atajo de
teclado para pegar) funciona.
...
pasos individuales para instalar Rust, Solana CLI y Anchor CLI.
...
Si encuentras el siguiente error al instalar
`protobuf-compiler`, asegúrate de
ejecutar primero
...
Instala el conjunto de herramientas de la CLI de Solana utilizando el comando de
instalación oficial:
...
con la etiqueta de versión que coincida con la versión de software de tu
lanzamiento deseado (por ejemplo,
`v2.0.3`), o usar uno de
los tres nombres de canal simbólicos:
`stable`,
`beta` o
`edge`.

Para una primera instalación de la CLI de Solana, es posible que veas el
siguiente mensaje pidiéndote que añadas una variable de entorno
`PATH`:
...
Solana a tu `PATH` para
que el comando esté disponible en tu terminal.
...
`export PATH` registrado
durante la instalación no persiste una vez que cierras tu terminal.
...
Para actualizar posteriormente la Solana CLI a la última versión, puedes usar el
siguiente comando:
...
_shell`avm use`_ para
declarar la versión de Anchor CLI que se ejecutará en tu sistema.
...
el comando _shell`anchor init`_
requiere Node.js y Yarn. (La plantilla de prueba en Rust está disponible usando
...
Al ejecutar `anchor build`, si
encuentras los siguientes errores:
...
Después de aplicar la solución anterior, intenta ejecutar
...
Al ejecutar
_shell`anchor test`_ después de
crear un nuevo proyecto de Anchor en Linux o WSL, puedes encontrar los
siguientes errores si no tienes Node.js o Yarn instalados:
...
explícitamente la sobrescritura usando la bandera
`--force`.
...
Por ejemplo, para crear un proyecto llamado
`my-project`, ejecuta:
...
Puedes encontrar el programa compilado en el directorio
`/target/deploy`.
...
Este comando despliega tu programa en el
`cluster` especificado en
el archivo
...
Cuando usas `localnet` como
`cluster` en
`Anchor.toml`, Anchor
automáticamente inicia un validador local, despliega tu programa, ejecuta las
pruebas y luego detiene el validador.
