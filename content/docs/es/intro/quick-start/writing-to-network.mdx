...
description: >-
  ...
  nuevos tokens utilizando el System Program y el Token Extensions Program.
---
...
Los programas (contratos inteligentes) procesan estas instrucciones según su
lógica de negocio para cada instrucción respectiva. Cuando envías una
transacción, el runtime de Solana ejecuta cada instrucción en secuencia y de
forma atómica (lo que significa que o todas las instrucciones tienen éxito o
toda la transacción falla).
...
programas de Solana. Para más detalles, consulta las páginas de
...
En Solana, cada cuenta tiene un programa específico como propietario. Solo el
...
Genera un nuevo keypair para usar como el `sender`.

### !receiver

Genera un nuevo keypair para usar como el `receiver`.
...
Construye una instrucción para invocar la instrucción de transferencia del
...
En este ejemplo, aprenderás cómo crear un nuevo token en Solana usando el Token
...
Calcula los lamports necesarios para un mint account.

### !rentExemptionLamports

Calcula los lamports necesarios para la exención de rent
...
El orden de las instrucciones es importante aquí. El `createAccountInstruction`
debe ir antes que el `initializeMintInstruction`.
...
1. [Asignar el espacio requerido](mention:one) para almacenar datos de mint
2. [Transferir lamports](mention:two) desde la wallet para financiar la nueva
   cuenta
