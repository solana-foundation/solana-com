...
Genera un nuevo par de claves para usar como el
`sender`.
...
Solicita un airdrop de SOL para financiar la
`sender`.
...
El orden de las instrucciones es importante aquí. La
`createAccountInstruction` debe ir antes que
la `initializeMintInstruction`.
