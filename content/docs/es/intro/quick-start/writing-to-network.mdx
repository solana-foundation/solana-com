...
hace cada instrucción. Cuando envías una transacción, el entorno de ejecución de
...
Crea un `Connection` para gestionar el envío de transacciones y la obtención de
datos de cuentas.
...
[`toPubkey`](mention:to) por la cantidad de [`lamports`](mention:lamports)
especificada.
...
Genera un nuevo keypair para usar como monedero.
...
mint account y que sea propiedad del Token Extensions Program.
...
Genera un keypair para el mint account.

La clave pública de este keypair se utilizará como dirección para el mint
account.
...
Calcula los lamports mínimos requeridos para un mint account.

La función `getMinimumBalanceForRentExemptMint` calcula exactamente cuánto SOL
(en lamports) debe asignarse para los datos en un mint account.
...
1. [Asignar los bytes necesarios](mention:space) para almacenar datos del mint
2. [Transferir lamports](mention:lamports) desde la billetera para financiar la
   nueva cuenta
3. [Asignar la propiedad](mention:programId) de la cuenta al Token Extensions
...
del [Token Extensions Program](mention:programId) para inicializar el mint
account con los siguientes datos:

- [2 decimales](mention:decimals)
- [La billetera](mention:authority) como autoridad de emisión y autoridad de
  congelación
...
Añade ambas instrucciones a una única transacción.

Al combinar ambas instrucciones en una sola transacción, te aseguras de que la
creación de la cuenta y la inicialización ocurran de forma atómica. O ambas
