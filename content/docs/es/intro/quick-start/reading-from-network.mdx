...
utilizan principalmente para almacenar SOL (registrado en el campo
`lamports`) y para firmar
transacciones.
...
El campo `data` de una
cuenta de Solana contiene bytes en bruto. Para interpretar estos datos de manera
significativa, debes deserializarlos al tipo de datos apropiado definido por el
programa propietario de la cuenta.
...
Por ejemplo,
_shell`@solana/spl-token`_ incluye la
función
[_ts`getMint()`_](mention:one)
para ayudar a deserializar el campo
`data` de una cuenta
mint en el tipo de datos
[Mint](https://github.com/solana-program/token/blob/program%40v8.0.0/program/src/state.rs#L16-L30).
...
La función _ts`getMint()`_
deserializa el campo
`data` de la cuenta en
el tipo de datos
[Mint](https://github.com/solana-program/token/blob/program%40v8.0.0/program/src/state.rs#L16-L30)
