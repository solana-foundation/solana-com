...
cuenta tiene una "dirección" (clave pública) que sirve como su identificador
único utilizado para localizar sus datos correspondientes en la cadena.
...
Cuando consultas una cuenta de billetera, la respuesta incluye los campos
...
Para las cuentas de billetera, este campo está vacío (0 bytes). Otras cuentas
...
Para billeteras y cuentas que almacenan el estado del programa, este campo es
`false`.
...
Para carteras, el propietario siempre es el System Program, con la dirección
...
las cuentas de billetera, los programas tienen la misma estructura de datos
subyacente
...
<CodePlaceholder title="Cuenta Mint" />
...
autoridad de mint, el suministro total y el número de decimales.

Para leer desde una cuenta mint, debes deserializar el campo `data` al tipo de
...
El campo `data` de una cuenta Solana contiene bytes en bruto. Para interpretar
estos datos de manera significativa, debes deserializarlos al tipo de datos
apropiado definido por el programa que es propietario de la cuenta.
...
convierten los bytes en bruto de la cuenta en tipos de datos estructurados,
...
El campo `freezeAuthority` muestra la autoridad permitida para congelar las
cuentas de token.
