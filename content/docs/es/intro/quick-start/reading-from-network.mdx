...
| `executable` | Indica si la cuenta contiene código de programa ejecutable. Para cuentas de wallet y de estado, este valor es `false`.                |
...
| `data`       | Este campo contiene el estado serializado de la cuenta, como la autoridad de mint, suministro total, número de decimales |
| `owner`      | El Token Program (`TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA`) posee la mint account.                                  |
...
El campo `data` de una cuenta de Solana contiene bytes sin procesar. Para
