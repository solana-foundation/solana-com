...
La guía explica un ejemplo más detallado en las secciones de Program Derived
...
ella. El programa contiene una instrucción
(`initialize`) que:
...
El macro _rs`declare_id!()`_
especifica la dirección en cadena de tu programa. Solana Playground actualiza
esta dirección cuando construyes el proyecto.

### !program

El atributo _rs`#[program]`_
anota un módulo que contiene funciones que representan las instrucciones del
programa.
...
define una estructura que representa el tipo de datos para las cuentas que este
programa puede crear y poseer.

### !constraints

Los atributos _rs`#[account(...)]`_
especifican las restricciones para la cuenta.

Estos añaden verificaciones comunes o características que Anchor proporciona
...
_rs`ctx.accounts.new_account.data`_ está
accediendo al campo
`data` de la cuenta
`new_account` y estableciendo
su valor al del parámetro
`data: u64`.
...
La restricción `init`
especifica que el programa debe crear la cuenta. Internamente, esta restricción
invoca al Programa del Sistema para crear una nueva cuenta y hace que este
programa sea el propietario. Como propietario del programa, este programa puede
entonces escribir en los datos de la cuenta.

### !payer

La restricción `payer`
especifica la cuenta que paga por la nueva cuenta.

### !space

La restricción `space`
especifica el número de bytes a asignar para la cuenta.
...
1. `ctx: Context<Initialize>` - Pasa a la
   función las cuentas requeridas para esta instrucción, según se especifica en
   la estructura `Initialize`.
2. `data: u64` - Un parámetro
   personalizado que debes proporcionar cuando llamas a la instrucción.

El cuerpo de la función
[establece el campo `data` de `new_account`](mention:three)
al argumento `data`
...
- El atributo
  ...
  define una estructura que especifica las cuentas requeridas para una
  instrucción particular, donde cada campo representa una cuenta separada.
...
- El atributo
  [`#[account]`](mention:one)
  define una estructura que representa el tipo de datos para las cuentas que
  este programa crea y posee.
...
Para construir el programa, simplemente ejecuta
`build` en la terminal.
El comando `build`
compila el programa. El bytecode del programa se almacena en una cuenta de
programa ejecutable cuando se despliega.
...
Solana Playground actualiza la dirección en
_rs`declare_id!()`_. Esta
dirección representa la dirección en cadena de tu programa (ID del programa).

Después de compilar el programa, ejecuta
_shell`deploy`_ en la
terminal para desplegar el programa en la red (devnet por defecto). El
despliegue del programa requiere asignar SOL a la cuenta on-chain que almacena
el programa.

Antes del despliegue, asegúrate de tener suficiente SOL. Puedes obtener SOL de
devnet ejecutando
_shell`solana airdrop 5`_ en la
terminal de Playground o usando el [Web Faucet](https://faucet.solana.com/).
...
`tests/anchor.test.ts`. Este archivo
demuestra cómo invocar la instrucción
...
`solana confirm -v` y especificando
el hash de la transacción (firma) de la salida de la prueba:
...
dirección del programa que se encuentra en
_rs`declare_id!()`_:
...
implementación establece la autoridad de actualización cuando despliegas el
