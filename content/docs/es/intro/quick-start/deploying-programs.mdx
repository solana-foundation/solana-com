...
`new_account` y estableciendo su valor al del parámetro `data: u64`.
...
La restricción `init` especifica que el programa debe crear la cuenta.
Internamente, esta restricción invoca al Programa del Sistema para crear una
nueva cuenta y hace que este programa sea el propietario. Como propietario del
...
comando `build` compila el programa. El bytecode del programa se almacena en una
...
devnet ejecutando _shell`solana airdrop 5`_ en la terminal de Playground o
usando el [Web Faucet](https://faucet.solana.com/).
...
[`initialize`](mention:one) del programa desde el cliente.
