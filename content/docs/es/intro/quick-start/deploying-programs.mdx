...
Solana Playground actualiza esta dirección cuando compilas el proyecto.
...
capó, esta restricción invoca al Programa del Sistema para crear una nueva
cuenta y hace que este programa sea el propietario. Como propietario del
programa, este programa puede entonces escribir en los datos de la cuenta.
...
Para compilar el programa, simplemente ejecuta `build` en la terminal. El
comando `build` compila el programa. El bytecode del programa se almacena luego
en una cuenta de programa ejecutable cuando se despliega.
...
representa la dirección de tu programa en la cadena (ID del programa).
...
requiere asignar SOL a la cuenta en la cadena que almacena el programa.
...
Puedes ver los registros de la transacción ejecutando el comando
