...
bump seed almacenado en el _rs`message_account`_, en lugar de que Anchor lo
recalcule.
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - Las restricciones `seeds` y `bump` verifican la cuenta como la PDA correcta
...
programa, mientras que [`pg.wallet`](mention:two) da acceso a tu wallet de
playground.
...
<Callout type="info">
  ...
  cuenta. Solo puede existir una cuenta para un PDA determinado.
</Callout>
