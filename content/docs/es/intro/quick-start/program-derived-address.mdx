...
El campo _rs`bump`_ almacena una semilla ["bump"](/docs/core/pda#canonical-bump)
_rs`u8`_ utilizada para derivar una Program Derived Address (PDA). Almacenar
este valor ahorra cómputo al eliminar la necesidad de recalcularlo en
...
- `bump` - _rs`u8`_ que almacena la semilla
  ["bump"](/docs/core/pda#canonical-bump) para derivar la Program Derived
  Address (PDA). Almacenar este valor ahorra cómputo al eliminar la necesidad de
  recalcularlo en instrucciones posteriores.
...
el código de tu programa, pero no afectan la validación. Debes usar nombres
...
A continuación, agrega la lógica de negocio para la instrucción _rs`create`_
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - La restricción `realloc` invoca al Programa del Sistema para ajustar el
     tamaño de datos de la cuenta
