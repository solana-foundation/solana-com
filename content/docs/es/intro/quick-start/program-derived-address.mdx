...
El campo _rs`message`_ contiene un _rs`String`_ que almacena el mensaje del
usuario.

### !bump

El campo _rs`bump`_ almacena una _rs`u8`_
["bump" seed](/docs/core/pda#canonical-bump) utilizada para derivar una Program
...
- `bump` - _rs`u8`_ que almacena la ["bump" seed](/docs/core/pda#canonical-bump)
...
2. Inicialización de datos de la cuenta:
   ...
   - Accede a la `message_account` desde el contexto.
...
1. _rs`user: Signer<'info>`_
   ...
   - Marcada como mutable (_rs`#[account(mut)]`_) ya que podría pagar por más
     espacio para la `message_account` cuando sea necesario
...
programa usando el macro _rs`msg!()`_. La función no necesita lógica adicional
...
Ejecuta el archivo de prueba con _shell`test`_ en la terminal de Playground para
comprobar que funciona como se espera. Los siguientes pasos añaden las pruebas
reales.
