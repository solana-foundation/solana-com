...
Primero, el programa necesita un mecanismo simple de "pagar para actualizar"
...
- La dirección de la cuenta proviene de un PDA derivado usando seeds
...
En la instrucción `delete`, verás cómo el programa puede "firmar" por este PDA
en un CPI.
...
- `to` - La cuenta de la bóveda (destino de los fondos)
...
La función `transfer` luego invoca la instrucción de transferencia en el
...
Esto permite la transferencia de cualquier SOL en la bóveda de vuelta al usuario
cuando cierran su cuenta de mensaje.
...
invocar el Programa del Sistema.
...
El `vault_account` usa la misma derivación PDA que en la estructura Update.

Añadir el `vault_account` a la estructura Delete permite al programa acceder a
la cuenta de bóveda del usuario durante la instrucción de eliminación para
...
SOL desde la cuenta de bóveda de vuelta a la cuenta del usuario.
...
Los _rs`signer_seeds`_ especifican las semillas opcionales y bump seeds
utilizadas para derivar el PDA.

### !with_signer

El método _rs`with_signer()`_ pasa las semillas del firmante con el CPI.
...
cuenta vault en las instrucciones. Esto requiere derivar el PDA del vault e
incluirlo en las llamadas de instrucción de actualización y eliminación.
