...
description: >-
  Carga de programas personalizados en la blockchain de Solana utilizando la CLI
  de Solana.
---
...
loader-v4 (subcomando program **-v4**) ya que loader-v3 está siendo obsoleto.
...
Este paso debe realizarse antes de cada re/implementación.

Verifique que haya fondos suficientes disponibles en la cuenta pagadora
...
la dirección de esa cuenta de programa. Lo siguiente genera un par de claves
para la cuenta del programa:
...
El parámetro se llama `program-id` aunque espera la ruta de archivo de un par de
claves:
...
Subir un ejecutable diferente a la misma cuenta de programa nuevamente
sobrescribirá / reemplazará el anterior. Sin embargo, para redespliegues, solo
se necesita el ID del programa (clave pública del par de claves del programa),
no el par de claves completo, porque el firmante es el par de claves de la
...
Observe que el despliegue inicial utilizó `program-keypair`, mientras que el
...
Durante períodos de congestión, hay algunas banderas adicionales que puede
utilizar para ayudar con el despliegue del programa:
...
- `--max-sign-attempts`: Número máximo de intentos para firmar o volver a firmar
  ...
  volverán a firmar con un nuevo blockhash reciente y se reenviarán. Utilice
  esta configuración para ajustar el número máximo de iteraciones de firma de
...
Si el despliegue del programa falla, habrá una cuenta intermedia de buffer
pendiente que contiene un saldo distinto de cero. Para recuperar ese saldo,
puedes reanudar un despliegue fallido proporcionando el mismo buffer intermedio
a una nueva llamada a `deploy`.

Los fallos de despliegue mostrarán un mensaje de error especificando la frase
semilla necesaria para recuperar el par de claves del buffer intermedio
generado:
...
Para recuperar el par de claves:
...
Luego emite un nuevo comando `deploy` y especifica el buffer:
...
En lugar de sobrescribir programas, también es posible proporcionar a los
usuarios la opción de qué versión de un programa quieren usar mediante la
...
Para programas desplegados bajo loader-v3, solo se pueden reclamar su cuenta
programdata, las cuentas de buffer y los fondos bloqueados en ellas. La cuenta
del programa junto con el ID del programa y los fondos bloqueados
específicamente en la cuenta del programa quedan atrapados.

Los programas desplegados bajo loader-v4 pueden cerrarse con su cuenta de
...
cerrarlo, debes generar un nuevo archivo de par de claves de programa.
...
ejectable conocido. El archivo descargado puede ser truncado, hasheado y
...
El derecho a modificar un programa determinado reside en su autoridad. Esta
autoridad puede ser transferida a otro par de claves sin cambiar el par de
claves del programa, de modo que el ID del programa permanezca igual. Además,
una sola autoridad puede controlar múltiples cuentas de programa.

Si no se especifica explícitamente durante el despliegue inicial, entonces el
par de claves predeterminado se utiliza como autoridad. Por eso al redesplegar
un programa en los pasos anteriores no se requirió especificar explícitamente
una autoridad.

Una autoridad explícita es útil para la firma offline y programas gobernados por
múltiples entidades.

Primero, se debe generar un par de claves para la autoridad:
...
O después del despliegue y utilizando el par de claves predeterminado como la
autoridad actual:
...
## Avanzado: Redespliegue en dos pasos utilizando un búfer

En lugar de cargar directamente a la cuenta del programa, el ejecutable puede
ser cargado primero a una cuenta búfer de preparación y luego ser transferido a
la cuenta del programa en un segundo paso (el redespliegue o despliegue real).
Esto es útil para la firma offline y programas gobernados por múltiples
entidades, como una votación DAO para elegir o rechazar un ejecutable cargado
antes del redespliegue real.

Tenga en cuenta que el uso de cuentas búfer aproximadamente duplica los fondos
requeridos durante el proceso de carga, ya que dos cuentas mantienen un
ejecutable cada una, simultáneamente.

Primero, debe crearse un par de claves para la cuenta búfer:
...
Tenga en cuenta que solo los redespliegues pueden realizarse en modo offline. El
...
3. (en línea) crear buffer y cargar un ejecutable en él
