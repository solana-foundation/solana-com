...
Para nuevos despliegues, utiliza `solana program-v4 deploy` en lugar de
...
El parámetro se llama `program-id` aunque espera la ruta del archivo de un
keypair:
...
- `--use-rpc`: Envía transacciones de escritura al RPC configurado en lugar de a
  los TPUs del validador. Esta opción requiere una conexión RPC
  [ponderada por stake](/developers/guides/advanced/stake-weighted-qos) como
  [Helius](https://www.helius.dev/) o [Triton](https://triton.one/). Esta opción
  también puede configurarse como predeterminada usando:
...
- `--max-sign-attempts`: Número máximo de intentos para firmar o volver a firmar
  ...
  que expire el blockhash reciente elegido inicialmente, esas transacciones se
...
- `Program Id` es la dirección que puede ser referenciada en el campo
  `program_id` de una instrucción al invocar un programa.
- `Owner`: El cargador con el que se desplegó este programa.
- `ProgramData Address` es la cuenta de datos del programa asociada con la
  cuenta del programa que contiene el ejecutable del programa (solo loader-v3).
- `Status`: `retracted`, `deployed` o `finalized` (solo loader-v4).
- `Authority` es la autoridad de actualización del programa.
- `Last Deployed In Slot` es el slot en el que se desplegó el programa por
  última vez.
...
Busca un `blockhash` reciente y pégalo para generar la firma de transacción sin
conexión. El `blockhash` expira después de aproximadamente 60 segundos. Si no lo
lograste a tiempo - simplemente obtén otro hash nuevo y repite hasta que tengas
éxito, o considera usar nonces de transacción duraderos.
