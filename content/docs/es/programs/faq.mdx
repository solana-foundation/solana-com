...
Este error de programa puede ocurrir al intentar deserializar la instrucción,
verifica que la estructura pasada coincida exactamente con la instrucción. Puede
haber algo de relleno entre campos. Si el programa implementa el trait `Pack` de
Rust, intenta empaquetar y desempaquetar el tipo de instrucción `T` para
determinar la codificación exacta que el programa espera.
...
algunas dependencias pueden incluir funcionalidades que violan las restricciones
del marco de pila incluso si el programa no usa esa funcionalidad. Si el
programa viola el tamaño de la pila en tiempo de ejecución, se informará un
...
Los programas tienen acceso a un montón de tiempo de ejecución a través de las
APIs `alloc` de Rust. Para facilitar asignaciones rápidas, se utiliza un simple
montón de incremento de 32KB. El montón no admite `free` ni `realloc`.
...
lo que los desarrolladores deben escribir sus programas para e implementarlos en
el mismo cargador. Si un programa escrito para un cargador se implementa en uno
diferente, el resultado suele ser un error `AccessViolation` debido a la
deserialización no coincidente de los parámetros de entrada del programa.
...
- Para cada cuenta
  - 1 byte indicando si esta es una cuenta duplicada, si no es duplicada
    entonces el valor es 0xff, de lo contrario el valor es el índice de la
    cuenta de la que es duplicada.
  - Si es duplicada: 7 bytes de relleno
  - Si no es duplicada:
    ...
    - 8 bytes número sin signo de lamports que posee la cuenta
