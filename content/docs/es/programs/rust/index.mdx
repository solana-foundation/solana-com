...
A continuación, reemplaza el contenido de
`src/lib.rs` con el siguiente
código. Este es un programa mínimo de Solana que imprime "Hello, world!" en el
registro del programa cuando se invoca.

La macro `msg!` se
utiliza en los programas de Solana para imprimir un mensaje en el registro del
programa.
...
A continuación, compila el programa usando el comando
`cargo build-sbf`.
...
Este comando genera un directorio
`target/deploy` que contiene dos
archivos importantes:

1. Un archivo `.so` (p.
   ej., `hello_world.so`): Este es
   el programa de Solana compilado que se desplegará en la red como un "contrato
   inteligente".
2. Un archivo keypair (p. ej.,
   `hello_world-keypair.json`): La clave
   pública de este keypair se utiliza como ID del programa al desplegarlo.
...
A continuación, prueba el programa usando el crate
`solana-program-test`. Añade las
siguientes dependencias a
`Cargo.toml`.
...
Añade la siguiente prueba a
`src/lib.rs`, debajo del
código del programa. Este es un módulo de prueba que invoca el programa hello
world.
...
`cargo test-sbf`. El registro del
programa mostrará "Hello, world!".
...
`solana-test-validators` para iniciar el
validator local.
...
`solana program deploy` en una terminal
separada para desplegar el programa en el validator local.
...
`examples/client.rs`. Este es un
script cliente en Rust que financia un nuevo keypair para pagar las tarifas de
transacción y luego invoca el programa hello world.
...
programa. Actualiza el programa en
`src/lib.rs` para imprimir
...
Prueba el programa actualizado ejecutando el comando
`cargo test-sbf`.
...
Ejecuta el comando
`cargo build-sbf` para generar un
archivo `.so`
actualizado.
...
Vuelve a desplegar el programa usando el comando
...
Para cerrar un programa, utiliza el comando
`solana program close <PROGRAM_ID>`. Por ejemplo:
...
`./target/deploy/hello_world-keypair.json`) y ejecutar
`cargo build-sbf` nuevamente, lo
que generará un nuevo archivo de keypair.
