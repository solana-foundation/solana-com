...
2. Un archivo keypair (p. ej., `hello_world-keypair.json`): La clave pública de
   este keypair se utiliza como ID del programa al desplegar el programa.
...
es un módulo de prueba que invoca el programa hola mundo.
...
Mientras el validador de prueba está ejecutándose, ejecuta el comando
...
Puedes obtener tu ID de programa ejecutando el siguiente comando.
...
Puedes inspeccionar la firma de la transacción en el
[Explorador de Solana](https://explorer.solana.com/?cluster=custom&customUrl=http%3A%2F%2Flocalhost%3A8899)
...
Redesplega el programa usando el comando `solana program deploy`.
...
transacción en el Explorador de Solana para ver "Hello, Solana!" en el registro
del programa.
...
ser reutilizado. Intentar desplegar un programa con un ID de programa
previamente cerrado resultará en un error.
...
nuevo, lo que generará un nuevo archivo de keypair.
