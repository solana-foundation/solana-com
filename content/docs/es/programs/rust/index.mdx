...
Ahora, reemplaza el contenido de `src/lib.rs` con el siguiente código. Este es
un programa mínimo de Solana que imprime "Hello, world!" en el registro del
programa cuando el programa es invocado.
...
1. Un archivo `.so` (por ejemplo, `hello_world.so`): Este es el programa
   compilado de Solana que será desplegado en la red como un "contrato
   inteligente".
...
Luego, prueba el programa usando el crate `solana-program-test`. Añade las
...
programa. Actualiza el programa en `src/lib.rs` para que imprima "Hello,
