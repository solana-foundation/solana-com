---
title: टोकन अकाउंट बनाएं
description: SPL टोकन अकाउंट्स कैसे बनाएं, यह जानें।
---

## टोकन अकाउंट क्या है?

token account सोलाना के टोकन प्रोग्राम्स में एक अकाउंट प्रकार है जो किसी विशिष्ट
टोकन (मिंट) के व्यक्तिगत स्वामित्व के बारे में जानकारी संग्रहित करता है।
प्रत्येक token account एक अकेले मिंट से जुड़ा होता है और टोकन बैलेंस और मालिक
जैसे विवरणों को ट्रैक करता है।
...
<Callout type="info">
  ध्यान दें कि सोर्स कोड में, डेवलपर्स Token account को `Account` प्रकार कहते
  हैं। [Token
  ...
  दोनों Token account के लिए समान बेस इम्प्लीमेंटेशन शेयर करते हैं।
</Callout>

किसी विशिष्ट मिंट के लिए टोकन रखने के लिए, उपयोगकर्ताओं को पहले एक token account
बनाना होगा। प्रत्येक token account निम्न चीजों का ट्रैक रखता है:

1. एक विशिष्ट मिंट (टोकन प्रकार जिसके यूनिट्स token account रखता है)
2. एक मालिक (वह अथॉरिटी जो अकाउंट से टोकन ट्रांसफर कर सकता है)

यहां सोलाना पर USD कॉइन (USDC) का उपयोग करते हुए एक उदाहरण दिया गया है:

- USD कॉइन मिंट एड्रेस: `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
- सर्कल (USD कॉइन जारीकर्ता) का token account इस पते पर है
  `3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa`
- यह token account केवल USD कॉइन टोकन (मिंट) के यूनिट्स रखता है
- सर्कल के पास `7VHUFJHWu2CuExkJcJrzhQPJ2oygupTWkL2A2For4BmE` पर मालिक अथॉरिटी
  है और इन टोकन्स को ट्रांसफर कर सकता है

आप इस token account के विवरण
[सोलाना एक्सप्लोरर](https://explorer.solana.com/address/3emsAVdmGKERbHjmGfQ6oZ1e35dkf5iYcS6U4CPKFVaa)
पर देख सकते हैं।

<Callout type="info">
  "मालिक" शब्द का उपयोग दो अलग-अलग संदर्भों में किया जाता है:

1. token account "मालिक" - यह एक एड्रेस है जो token account के डेटा में Token
   Program द्वारा परिभाषित
   ...
   प्रकार के "मालिक" फील्ड के रूप में संग्रहित है। मालिक अकाउंट से टोकन
   ट्रांसफर, बर्न या डेलिगेट कर सकता है। इस एड्रेस को कभी-कभी प्रोग्राम मालिक से
   अलग करने के लिए token account की "अथॉरिटी" के रूप में संदर्भित किया जाता है।

2. प्रोग्राम "owner" - यह उस प्रोग्राम को संदर्भित करता है जो Solana पर खाता
   डेटा का मालिक है। Token account के लिए, यह हमेशा या तो Token Program या Token
   Extension Program होता है, जैसा कि बेस Solana
   [Account](https://github.com/anza-xyz/solana-sdk/blob/4e30766b8d327f0191df6490e48d9ef521956495/account/src/lib.rs#L51)
   प्रकार के "owner" फ़ील्ड में निर्दिष्ट किया गया है।

Token account के साथ काम करते समय, "owner" आमतौर पर उस अथॉरिटी को संदर्भित करता
है जो टोकन खर्च कर सकती है, न कि उस प्रोग्राम को जो खाते का मालिक है।

</Callout>

## Associated Token Account क्या है?

Associated token account एक ऐसा token account है जिसका पता एक Program Derived
Address (PDA) है जो
[Associated Token Program](https://github.com/solana-program/associated-token-account/tree/main/program)
द्वारा बनाया गया है। आप associated token account को एक उपयोगकर्ता के लिए किसी
विशिष्ट टोकन (mint) की इकाइयों को रखने के लिए डिफ़ॉल्ट token account के रूप में
सोच सकते हैं।

<Callout type="info">
  "Associated token accounts" शब्द केवल उन token account पर लागू होता है जिन्हें
  Associated Token Program बनाता है।
</Callout>

Associated token account किसी भी दिए गए mint के लिए उपयोगकर्ता के token account
को खोजने का एक निर्धारित तरीका प्रदान करते हैं। आप डेरिवेशन के कार्यान्वयन का
निरीक्षण
[यहां](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/interface/src/address.rs#L56)
कर सकते हैं।
...
यह निर्धारित डेरिवेशन सुनिश्चित करता है कि वॉलेट पते और टोकन mint के किसी भी
संयोजन के लिए, ठीक एक associated token account पता मौजूद है। यह दृष्टिकोण किसी
भी दिए गए टोकन mint के लिए उपयोगकर्ता के token account को खोजना आसान बनाता है,
जिससे token account पतों को अलग से ट्रैक करने की आवश्यकता समाप्त हो जाती है।

<Callout type="info">
  Associated Token Program एक सहायक प्रोग्राम के रूप में कार्य करता है जो
  निर्धारित पतों (PDA) के साथ token account बनाता है। Associated token account
  बनाते समय, Associated Token Program या तो Token Program या Token Extension
  Program को CPI (Cross-Program Invocation) करता है। Token program बनाए गए खाते
  का मालिक होता है जिसमें वही `Account` प्रकार संरचना होती है जैसा कि token
  program में परिभाषित है। Associated Token Program कोई स्थिति नहीं रखता - यह बस
  एक निर्धारित पते पर token account बनाने का एक मानकीकृत तरीका प्रदान करता है जो
  एक PDA है।
</Callout>

## टोकन अकाउंट कैसे बनाएं

टोकन अकाउंट बनाने के लिए,
...
[यहां](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/processor.rs#L145)
पा सकते हैं।

token account बनाने के लिए लेनदेन में दो निर्देशों की आवश्यकता होती है:

1. सिस्टम प्रोग्राम को आमंत्रित करें ताकि token account के लिए स्थान बनाया और
   आवंटित किया जा सके और स्वामित्व Token Program को हस्तांतरित किया जा सके।
2. token account डेटा को आरंभ करने के लिए Token Program को आमंत्रित करें।
...
## associated token account कैसे बनाएं

associated token account बनाने के लिए,
...
निर्देश का उपयोग करें। आप इस निर्देश के कार्यान्वयन
[यहां](https://github.com/solana-program/associated-token-account/blob/c78722ee39ebf6741cdfa81dffd52135370a5015/program/src/processor.rs#L66)
पा सकते हैं।

associated token account बनाने का निर्देश स्वचालित रूप से token account बनाने के
लिए सिस्टम प्रोग्राम को आमंत्रित करता है और token account डेटा को आरंभ करने के
लिए Token Program को आमंत्रित करता है। यह Cross Program Invocations (CPI) के
माध्यम से होता है।
