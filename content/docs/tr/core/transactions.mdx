...
Bir işlem, esasen bir veya daha fazla talimatı işleme koyma isteğidir. Bir
işlemi, içinde formlar bulunan bir zarf olarak düşünebilirsiniz. Her form, ağa
ne yapması gerektiğini söyleyen bir talimattır. İşlemi göndermek, formların
...
Aşağıdaki diyagram, bir göndericiden bir alıcıya SOL transfer etmek için tek bir
...
Gönderen hesap, System Program'ın lamport bakiyesini düşürmesine izin vermek
için işlemi imzalamalıdır (`is_signer`). Gönderen ve alıcı hesaplar yazılabilir
olmalıdır (`is_writable`) çünkü lamport bakiyeleri değişir.

İşlemi gönderdikten sonra, System Program transfer talimatını işler. System
Program daha sonra hem gönderen hem de alıcı hesapların lamport bakiyelerini
günceller.
...
İstemci kütüphaneleri genellikle program talimatlarını oluşturma detaylarını
soyutlar. Eğer bir kütüphane mevcut değilse, talimatı manuel olarak
...
Aşağıdaki bölümlerde, işlemler ve talimatların detaylarını inceleyeceğiz.

## Talimatlar

Solana [programında](/docs/core/accounts#program-account) bir
...
Solana'da bir programın talimatını çağırmak için, üç parça bilgi içeren bir
...
- [is_writable](mention:is-writable): Talimatın hesap verilerini değiştirip
  değiştirmediği
...
tanımlanan talimatın uygulamasına başvurmanız gerekir.
...
Aşağıdaki örnekler, önceki kod parçacıklarından elde edilen çıktıyı
göstermektedir. Tam format SDK'ya bağlı olarak değişir, ancak her Solana
talimatı aşağıdaki bilgileri gerektirir:
...
- [Hesaplar](mention:accounts): Talimat tarafından gereken hesapların listesi.
  ...
  üzerine yazılıp yazılmayacağını belirtmelidir.
...
1. [İmzalar](mention:signatures): İşlemdeki talimatlar için imzalayıcı olarak
   gereken tüm hesaplardan gelen
   [imzaların](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/signature/src/lib.rs#L30)
   bir dizisi. İmza, işlem `Message` hesabın özel anahtarıyla imzalanarak
   oluşturulur.
...
- [Mesaj Başlığı](mention:message-header): İmzalayan ve salt okunur hesapların
  sayısını belirtir.
...
işlemdeki hesap için izinleri belirtir. Hangi hesapların imzalayan ve
...
[hesap adreslerini](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/message/legacy.rs#L138)
içeren tek bir liste içerir. Dizi, kaç adres içerdiğini belirten bir
...
katı bir sıralamasının kombinasyonuna dayanır.
...
tipinde bir dizi
[talimat](https://github.com/anza-xyz/agave/blob/v2.1.13/sdk/program/src/message/legacy.rs#L146)
içerir. Talimatlar, bir işleme eklendiğinde bu tipe dönüştürülür.

Mesajdaki hesap adresleri dizisi gibi, bu da bir
...
1. [Program ID İndeksi](mention:program-id-index): Hesap adresleri dizisinde
   programın adresini işaret eden bir indeks. Bu, talimatı işleyecek programı
   belirtir.
2. [Hesap İndeksleri](mention:account-indexes): Bu talimat için gereken hesap
   adreslerini işaret eden indekslerin bir dizisi.
...
göstermektedir. Tam format SDK'ya bağlı olarak değişir, ancak aynı bilgileri
içerir.
...
Bir "işlem imzası" Solana'da bir işlemi benzersiz şekilde tanımlar. Bu imzayı,
