...
- **Deterministik Hesap Adresleri**: PDA'lar, isteğe bağlı "seed'ler" (önceden
  ...
  deterministik olarak bir adres oluşturmak için bir mekanizma sağlar.
- **Program İmzalamayı Etkinleştirme**: Solana çalışma zamanı, programların
  kendi adresinden türetilen PDA'lar için "imzalamasına" olanak tanır.
...
Bir Program Derived Address (PDA) türetmenin, otomatik olarak o adresteki zincir
...
<Callout type="info">
  Bu bölüm, PDA'ların türetilmesi hakkındaki ayrıntıları kapsar. [Cross Program
  Invocation (CPI)](/docs/core/cpi) bölümü, programların imzalama için PDA'ları
...
PDA'lar, deterministik olarak türetilen ve genel anahtarlara benzeyen, ancak
özel anahtarları olmayan adreslerdir. Bu, adres için geçerli bir imza
oluşturmanın mümkün olmadığı anlamına gelir. Bununla birlikte, Solana çalışma
zamanı, programların özel bir anahtar gerektirmeden PDA'lar için "imza" atmasını
sağlar.

Bağlam olarak, Solana
[Keypair'leri](https://github.com/anza-xyz/solana-sdk/blob/sdk%40v2.2.2/keypair/src/lib.rs#L26),
bir genel anahtar ve karşılık gelen özel anahtara sahip Ed25519 eğrisindeki
(eliptik eğri kriptografisi) noktalardır. Genel anahtarlar, zincir üstü hesaplar
...
PDA, zincir üstü bir hesap için adres (benzersiz tanımlayıcı) görevi görebilir
ve program durumunu kolayca depolamak, eşlemek ve getirmek için bir yöntem
sağlar.
...
- **İsteğe bağlı seed'ler**: PDA türetimi için önceden tanımlanmış girdiler
  (örneğin dizeler, sayılar, diğer hesap adresleri).
- **Bump seed**: Geçerli bir PDA'nın (eğri dışı) oluşturulmasını sağlamak için
...
eğri dışı adres üreten bir değer bulana kadar bump değerlerini yineler. Geçerli
bir eğri dışı adres üreten ilk bump değeri "kanonik bump" olarak adlandırılır.
...
Bump seed 255 bir hata fırlatır ve geçerli bir PDA türeten ilk bump seed
254'tür.
...
<Callout type="warn">
  ...
  hesapların kullanılmasına izin veren güvenlik açıklarına neden olabilir.
...
Bu örnekte, PDA türetimi için kullanılan seed'ler sabit bir dizi olan `data` ve
...
Test dosyası ayrıca işlem gönderildikten sonra o adresteki zincir üstü hesabı
nasıl alacağınızı da gösterir.
