...
olarak minimum bir lamport bakiyesine sahip olması gerekir. Bu minimum bakiye
...
- Çalıştırılamayan hesaplar için bu alan genellikle okunması amaçlanan durumu
  depolar.
...
## !!steps Yürütülebilir Alan

Bu alan, bir hesabın yürütülebilir bir program olup olmadığını gösterir.

- Eğer `true` ise, hesap yürütülebilir bir Solana programıdır.
- Eğer `false` ise, hesap durum bilgisini depolayan bir veri hesabıdır.

Yürütülebilir hesaplar için, `owner` alanı bir yükleyici programın program
kimliğini içerir. Yükleyici programlar, yürütülebilir program hesaplarını
...
Başlangıçta, bu alan bir hesabın ağda verilerini korumak için ne zaman rent
(lamport cinsinden) ödemesi gerektiğini takip ediyordu. Ancak, bu rent toplama
mekanizması artık kullanımdan kaldırılmıştır.
...
Bu bakiyeye "rent" denir, ancak bir hesabı kapattığınızda tam tutarı geri
alabileceğiniz için daha çok bir depozito gibi çalışır. Hesaplamayı
...
- Hesabın `data` alanını değiştir
- Hesabın bakiyesinden lamport düş

Her program, bir hesabın `data` alanında saklanan verinin yapısını tanımlar.
...
tarafından sahiplenilir. System Program aşağıdaki temel işlevleri
gerçekleştirir:
...
| [Alan Tahsisi](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L71)             | Her hesabın veri alanı için byte kapasitesini ayarlar.                                                                                                                                                                                        |
...
"System Hesapları" olduğunu unutmayın. Bu hesaplardaki lamport bakiyesi, cüzdan
tarafından sahip olunan SOL miktarını gösterir. Yalnızca System Hesapları işlem
ücretlerini ödeyebilir.

![System Hesabı](/assets/docs/core/accounts/system-account.svg)
...
`11111111111111111111111111111111` adresine sahip System Program olduğunu
unutmayın.
...
`true` olarak ayarlanmıştır. Program account'ları veri alanlarında
...
Bir Solana programı dağıttığınızda, program bir program account içinde saklanır.
Program account'lar bir [Loader Program](/docs/core/programs#loader-programs)
tarafından sahiplenilir. Yükleyicinin birkaç versiyonu vardır, ancak loader-v3
dışındaki tüm versiyonlar çalıştırılabilir kodu doğrudan program account içinde
saklar. Loader-v3, çalıştırılabilir kodu ayrı bir "program data account" içinde
saklar ve program account sadece buna işaret eder. Yeni bir program
...
olarak hazırlamak için özel bir hesap türüne sahiptir. Loader-v4'te hala
tamponlar vardır, ancak bunlar sadece normal program hesaplarıdır.

### Program Veri Hesabı

Loader-v3, diğer tüm BPF Loader programlarından farklı çalışır. Program hesabı
sadece gerçek yürütülebilir kodu depolayan bir program veri hesabının adresini
içerir:
...
hesabın sahipliğini başka bir programa atayabilir.
...
1. Bir hesap oluşturmak için System Program'ı çağırın, ardından sahipliği özel
   programa aktarın
2. Artık hesaba sahip olan özel programı çağırın ve programın talimatında
...
hesabının nasıl oluşturulacağını ve alınacağını göstermektedir.
