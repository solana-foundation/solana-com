...
Bir programın talimatlarını çağırdığınızda, program account'ının adresini
(yaygın olarak "Program ID" olarak adlandırılır) belirtirsiniz.

Aşağıdaki örnek, program account'larının aynı temel `Account` tipine sahip
olduğunu göstermek için Token Program account'ını getirir, ancak `executable`
alanı `true` olarak ayarlanmıştır. Program account'ları veri alanlarında
yürütülebilir kod içerdiğinden, verileri deserialize etmiyoruz.
...
Bir Solana programını dağıttığınızda, program bir program account'ında saklanır.
Program account'ları bir [Loader Program](/docs/core/programs#loader-programs)
tarafından sahiplenilir. Yükleyicinin birkaç versiyonu vardır, ancak loader-v3
hariç hepsi yürütülebilir kodu doğrudan program account'ında saklar. Loader-v3,
yürütülebilir kodu ayrı bir "program data account"ta saklar ve program account'ı
sadece buna işaret eder. Yeni bir program dağıttığınızda, Solana CLI varsayılan
olarak en son yükleyici sürümünü kullanır.
...
Loader-v3, diğer tüm BPF Loader programlarından farklı çalışır. Program
account'ı sadece gerçek yürütülebilir kodu saklayan bir program data
account'ının adresini içerir:

![Program Data Account](/assets/docs/core/accounts/program-account-expanded.svg)

Bu program data account'larını programların veri hesaplarıyla (aşağıya bakın)
karıştırmayın.
...
Aşağıdaki örnek, Token 2022 programının sahip olduğu bir Token Mint hesabının
