...
Öncelik ücretlerini ayarlamanın önerilen yaklaşımı, öncelikle işlemi
[simüle ederek](/developers/guides/advanced/how-to-request-optimal-compute)
gerekli hesaplama birimlerini tahmin etmektir. Ardından, bu tahmine %10'luk bir
güvenlik marjı ekleyin ve elde edilen değeri `Compute Unit Limit` olarak
kullanın.

Diğer işlemlere göre nasıl önceliklendirildiğini belirleyen
[işlem önceliği](https://github.com/anza-xyz/agave/blob/v2.2.14/core/src/banking_stage/transaction_scheduler/receive_and_buffer.rs#L646),
aşağıdaki formül kullanılarak hesaplanır:

**Öncelik = ((Hesaplama Birimi Limiti \* Hesaplama Birimi Fiyatı) + Temel Ücret)
/ (1 + Hesaplama Birimi Limiti + İmza CU'ları + Yazma Kilidi CU'ları)**

Bir işlemde hesaplama birimi limitini ve fiyatını ayarlamak için bu talimatları
kullanın:
...
Bu talimatları sağlamazsanız, işlem varsayılan hesaplama birimi limitini 0
hesaplama birimi fiyatıyla (önceliklendirme ücreti olmadan) kullanır.

Öncelik ücreti, gerçekte kullanılan hesaplama birimlerine değil, işlem için
talep ettiğiniz hesaplama birimi limitine bağlıdır. Çok yüksek bir hesaplama
birimi limiti ayarlarsanız veya varsayılan miktarı kullanırsanız, kullanılmayan
