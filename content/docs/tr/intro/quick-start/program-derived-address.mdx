...
description:
  ...
  Bu adım adım rehber, PDA'ları kullanarak zincir üzerindeki mesaj hesaplarını
...
etme sürecinde size Program Derived Address'leri (PDA'lar) göstermektedir. Daha
fazla ayrıntı için [Program Derived Addresses](/docs/core/pda) sayfasına bakın.
...
Bir Anchor programında _rs`#[account]`_ özniteliği, hesap verisini (Hesabın veri
alanında saklanacak veri türü) temsil eden yapıları işaretler.
...
üç alan içeren bir mesaj saklar:
...
- `bump` - Program türetilmiş adres (Program Derived Address) oluşturmak için
...
_rs`seeds`_ kısıtlaması, PDA'yı türetmek için kullanılan isteğe bağlı girdileri
tanımlar.
...
Bir Anchor programındaki _rs`#[derive(Accounts)]`_ özniteliği, bir talimat
tarafından gerektirilen hesapları tanımlayan yapıları işaretler.
...
2. _rs`message_account: Account<'info, MessageAccount>`_
   ...
   - `init` kısıtlaması, talimat yürütülürken hesabı oluşturur
...
Şimdi, `create` fonksiyonunu aşağıdakilerle güncelleyerek _rs`create`_ talimatı
...
3. _rs`system_program: Program<'info, System>`_
   ...
   - `realloc` kısıtlaması, hesabın veri boyutunu ayarlamak için Sistem
     Programını çağırır
