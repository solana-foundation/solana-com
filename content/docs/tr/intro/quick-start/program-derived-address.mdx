...
türetilen deterministik bir adrese sahip bir hesapta bulunur (Program Derived
Address veya PDA).

Bu rehber, Program Derived Address (PDA) kavramını gösterirken Anchor framework
kullanarak bir Solana programı oluşturma ve test etme sürecinde size yol
gösterir. Daha fazla ayrıntı için [Program Derived Addresses](/docs/core/pda)
sayfasına bakın.
...
`lib.rs` dosyasında, [`create`](mention:one), [`update`](mention:two) ve
[`delete`](mention:three) talimatlarını içeren bir program bulacaksınız sonraki
...
Bir Anchor programındaki _rs`#[account]`_ özniteliği, hesap verilerini
(Account'un veri alanında saklanacak veri türü) temsil eden yapıları işaretler.
...
_rs`bump`_ alanı, program türetilmiş adres (Program Derived Address) elde etmek
...
- `bump` - Program türetilmiş adres (Program Derived Address) elde etmek için
  kullanılan ["bump" tohumunu](/docs/core/pda#canonical-bump) saklayan bir
  _rs`u8`_. Bu değeri saklamak, daha sonraki talimatlarda yeniden hesaplama
  ihtiyacını ortadan kaldırarak işlem gücünden tasarruf sağlar.
...
Programı tekrar derlemek için terminalde _shell`build`_ komutunu çalıştırın.
...
### Oluşturma Talimatı Ekleme

Şimdi, _rs`create`_ talimatını ekleyin. Bu talimat _rs`MessageAccount`_
oluşturur ve başlatır.

_rs`Create`_ yapısını aşağıdaki gibi güncelleyerek, talimat için gereken
...
_rs`payer`_ kısıtlaması, yeni hesap oluşturma maliyetini hangi hesabın
...
Bu durumda, hesap özel _rs`MessageAccount`_ tipine uymalıdır.

### !program

_rs`Program<'info, T>`_ tipi, hesabın bir programa uymasını gerektirir.

Bu durumda, hesap _rs`System`_ tipine uymalıdır, bu da System Program'a atıfta
bulunur.
...
gereken hesapları tanımlayan yapıları açıklar.
...
2. Ek gereksinimleri tanımlayan isteğe bağlı kısıtlamalar (_rs`#[account(mut)]`_
   veya _rs`#[account(init)]`_ gibi)
...
gören bir hesap ayırıcısı için 8 bayta ihtiyaç duyar.
...
`create` fonksiyonu, yeni bir mesaj hesabının verilerini başlatma mantığını
...
1. _rs`ctx: Context<Update>`_ - _rs`Update`_ yapısında belirtilen hesaplara
   erişim sağlar.
2. _rs`message: String`_ - Mevcut olanın yerine geçecek yeni mesaj.
...
2. Hesap Verilerini Günceller:
   - Bağlamdan `message_account`'e erişir.
...
Şimdi, _rs`MessageAccount`_'yi kapatmak için _rs`delete`_ talimatını ekleyin.
...
_rs`seeds`_ kısıtlaması, PDA'yı türetmek için kullanılan seed'leri belirtir.
...
Bu durumda, _rs`user`_ hesabı, kapatılan _rs`message_account`_'den lamport'ları
alır.
...
İlk testi aşağıdaki gibi güncelleyin:
...
İlk olarak, test dosyası `update` talimatını çağıran bir işlem göndererek
"Hello, Solana!" mesajını yeni mesaj olarak iletir.
