...
Program kodunun `src/lib.rs` dosyasında olduğu yeni bir proje oluşturulduğunu
göreceksiniz.
...
_rs`#[program]`_ özniteliği, programın talimatlarını temsil eden işlevleri
...
planda, bu kısıtlama yeni bir hesap oluşturmak için Sistem Programını çağırır ve
...
Şimdilik, bu program koduna genel bir bakış sunar:
...
Programı yalnızca programın yükseltme yetkilisi kapatabilir. Dağıtım süreci,
programı dağıttığınızda yükseltme yetkilisini belirler. Bu hesap, programı
güncelleme veya kapatma konusunda özel izne sahiptir. Eğer birisi yükseltme
yetkisini iptal ederse, program kapanma veya güncelleme imkanı olmaksızın
değiştirilemez hale gelir.

Solana Playground'da program dağıtırken, Playground cüzdanınız otomatik olarak
tüm programlarınızın yükseltme yetkilisi olur.
