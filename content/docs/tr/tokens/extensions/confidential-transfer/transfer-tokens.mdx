...
hem de alıcının token hesapları _confidential-transfer_ durumu ile
yapılandırılmış ve gizli transferler için onaylanmış olmalıdır. Gönderenin token
hesabında ayrıca transfer edilecek kullanılabilir bir gizli bakiye bulunmalıdır.
...
1. İstemci tarafında
   ...
   **Eşitlik Kanıtı
   ...
   eşleştiğini doğrular, kaynak hesabın yeni kullanılabilir bakiyesinin doğru
   şekilde old_available_balance - transfer_amount olarak hesaplandığını garanti
   eder.
   ...
   **Şifre Metni Geçerlilik Kanıtı
   ...
   düzgün şekilde oluşturulduğunu doğrular, transfer tutarının her tarafın açık
   anahtarı altında doğru şekilde şifrelendiğini sağlar.
   ...
   **Aralık Kanıtı
   ...
   Yeni kullanılabilir bakiyenin ve transfer tutarının (düşük/yüksek bitlere
...
4. Oluşturmak için kullanılan SOL'u geri kazanmak için kanıt bağlam durumu
   hesaplarını kapatın.

Aşağıdaki diyagram, bir gönderenin token hesabından bir alıcının token hesabına
token transferinde yer alan adımları göstermektedir.

```mermaid
sequenceDiagram
    participant Sender
    participant ZkProofProgram
    participant Token2022Program
    participant Recipient

    Sender->>Sender: Generate proofs client-side
    Sender->>ZkProofProgram: Verify equality proof
    ZkProofProgram-->>Sender: Proof context state
    Sender->>ZkProofProgram: Verify ciphertext validity proof
    ZkProofProgram-->>Sender: Proof context state
    Sender->>ZkProofProgram: Verify range proof
    ZkProofProgram-->>Sender: Proof context state
    Sender->>Token2022Program: ConfidentialTransfer::Transfer
    Token2022Program-->>Recipient: Update recipient's confidential balance
    Token2022Program-->>Sender: Update sender's confidential balance
    Sender->>Sender: Close proof context state accounts
```

### Gerekli Talimatlar

Bir token hesabından diğerine gizli olarak token transfer etmek için şunları
yapmalısınız:
...
- Üç kanıt hesabını sağlayan
  [ConfidentialTransferInstruction::Transfer](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/extension/confidential_transfer/processor.rs#L604)
  talimatını çağırın
- Kira ücretini geri almak için üç kanıt hesabını kapatın.

`spl_token_client` paketi aşağıdaki metotları sağlar:

- Bir kanıt hesabı oluşturan
  `confidential_transfer_create_context_state_account` metodu.
- `Transfer` talimatını çağıran `confidential_transfer_transfer` metodu.
- Bir kanıt hesabını kapatan `confidential_transfer_close_context_state_account`
  metodu.

### Örnek Kod

Aşağıdaki örnek, token hesapları arasında gizli token transferi yapmanın nasıl
...
<Callout type="info">
  ...
  etkinleştirilmemiştir. Örnek kodu çalıştırmak için ana ağdan Token Extensions
