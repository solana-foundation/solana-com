...
description:
  Gizli Transfer uzantısı ile nasıl token hesabı oluşturulacağını öğrenin.
---
...
- **pending_balance_lo**: Bekleyen bakiyenin şifrelenmiş alt 16 biti. Bakiye,
  verimli şifre çözme için yüksek ve düşük parçalara bölünmüştür.
...
- **pending_balance_credit_counter**: Depozito ve transfer talimatlarından gelen
...
bakiyeler olarak ayrılmıştır. Bu ayrım olmadan, bir saldırgan bir token hesabına
sürekli token göndererek, token hesabı sahibinin token transfer etme yeteneğini
engelleyebilir. Token hesabı sahibi, işlem gönderildiği ve işlendiği zaman
arasında şifrelenmiş bakiye değişeceği için token transfer edemez, bu da
başarısız bir işlemle sonuçlanır.

Tüm para yatırma ve transfer tutarları başlangıçta bekleyen bakiyeye eklenir.
Token hesabı sahipleri, bekleyen bakiyeyi kullanılabilir bakiyeye dönüştürmek
için _rs`ApplyPendingBalance`_ talimatını kullanmalıdır. Gelen transferler veya
para yatırma işlemleri, bir token hesabının kullanılabilir bakiyesini etkilemez.
...
bölünmüştür çünkü ElGamal şifre çözme, daha büyük sayılar için daha fazla
...
Eşleşen beklenen/gerçek sayaçlar, `decryptable_available_balance`'nin
`available_balance` ile eşleştiğini gösterir.

Bir token hesabının durumunu `decryptable_available_balance`'yi okumak için
alırken, farklı beklenen/gerçek sayaç değerleri, istemcinin doğru bakiyeyi
...
`decryptable_available_balance`'yi uzlaştırmak için şu adımları izleyin:
...
2. Sayaç farkına kadar (gerçek - beklenen) yatırma ve transfer talimatlarını
   ...
   - Transfer talimatlarından hedef şifreli metin miktarlarının şifresini çözün
     ve ekleyin
...
1. **Token Hesabı Oluşturma**: Token hesabını oluşturmak için Associated Token
...
3. **Gizli Transferleri Yapılandırma**: _rs`ConfidentialTransferAccount`_
   durumunu başlatmak için Token Extension Program'ın
...
**Gizli transferler için bir token hesabını yalnızca token hesabı sahibi
yapılandırabilir**.
...
Aşağıdaki kod, Gizli Transfer uzantısı ile bir Associated Token Account
oluşturmayı göstermektedir.

Örneği çalıştırmak için, aşağıdaki komutu kullanarak ana ağdan klonlanmış Token
Extension Program ile yerel bir doğrulayıcı başlatın. Yerel doğrulayıcıyı
