...
token transferlerini mümkün kılar. Bu bölüm, bu uzantı etkinleştirilmiş bir
token hesabının nasıl oluşturulacağını açıklar.

Aşağıdaki diyagram, Gizli Transfer uzantısı ile token hesabı oluşturma
...
- **maximum_pending_balance_credit_counter**: Bekleyen bakiyeyi kullanılabilir
  ...
  önceki bekleyen kredilerin sayı limiti.

- **expected_pending_balance_credit_counter**: İstemci tarafından son kez
  _rs`ApplyPendingBalance`_ talimatı işlendiğinde instruction data aracılığıyla
  sağlanan `pending_balance_credit_counter` değeri.

- **actual_pending_balance_credit_counter**: Son _rs`ApplyPendingBalance`_
  talimatı işlendiği sırada token account üzerindeki
...
bakiyeler olarak ayrılmıştır. Bu ayrım olmadan, bir saldırgan token account'a
sürekli token göndererek token account sahibinin token transfer etme yeteneğini
engelleyebilir. Token account sahibi, işlem gönderildiği ve işlendiği zaman
arasında şifrelenmiş bakiye değişeceği için token transfer edemez ve bu da
başarısız bir işlemle sonuçlanır.

Tüm para yatırma ve transfer tutarları başlangıçta bekleyen bakiyeye eklenir.
Token account sahipleri, bekleyen bakiyeyi kullanılabilir bakiyeye dönüştürmek
için _rs`ApplyPendingBalance`_ talimatını kullanmalıdır. Gelen transferler veya
para yatırma işlemleri, bir token account'ın kullanılabilir bakiyesini
etkilemez.
...
bölünmüştür çünkü ElGamal şifre çözme işlemi büyük sayılar için daha fazla
hesaplama gerektirir. Şifreli metin aritmetik uygulamasını
...
_rs`ApplyPendingBalance`_ talimatını çağırırken:

1. İstemci mevcut bekleyen ve kullanılabilir bakiyeleri arar, toplamı şifreler
   ve token account sahibinin AES anahtarı kullanılarak şifrelenmiş bir
...
2. Beklenen ve gerçek bekleyen kredi sayaçları, _rs`ApplyPendingBalance`_
   ...
   - `actual_pending_balance_credit_counter`: _rs`ApplyPendingBalance`_ talimatı
     işlendiği sırada token account üzerindeki `pending_balance_credit_counter`
     değeri

Eşleşen beklenen/gerçek sayaçlar, `decryptable_available_balance` değerinin
`available_balance` ile eşleştiğini gösterir.

Bir token account'ın durumunu `decryptable_available_balance` değerini okumak
için çekerken, farklı beklenen/gerçek sayaç değerleri, doğru bakiyeyi hesaplamak
için istemcinin sayaç farkıyla eşleşen son yatırma/transfer talimatlarını
...
1. Token account'tan `decryptable_available_balance` değeri ile başlayın
2. Sayaç farkına kadar (gerçek - beklenen) yatırma ve transfer talimatlarını
   ...
   - Transfer talimatlarından hedef şifreli metin miktarlarını deşifre edin ve
     ekleyin
...
Confidential Transfer uzantısına sahip bir token account oluşturmak üç talimat
gerektirir:

1. **Token Account Oluşturma**: Token account oluşturmak için Associated Token
...
**Gizli transferler için bir token account'ı yalnızca token account sahibi
yapılandırabilir**.
...
Aşağıdaki kod, Confidential Transfer uzantısına sahip bir Associated Token
Account oluşturmayı göstermektedir.
...
---CODE-PLACEHOLDER-c9c3c9e5c9c3c9e5c9c3c9e5c9c3c9e5---
