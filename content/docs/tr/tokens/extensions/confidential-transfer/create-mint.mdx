...
description: Confidential Transfer uzantısı ile token mint oluşturmayı öğrenin.
---

## Confidential Transfer uzantısı ile mint nasıl oluşturulur

Confidential Transfer uzantısı, mint hesabına ekstra durum ekleyerek özel token
transferlerini mümkün kılar. Bu bölüm, bu uzantının etkinleştirildiği bir token
mint oluşturmayı açıklamaktadır.

Aşağıdaki diyagram, Confidential Transfer uzantısı ile mint oluşturma adımlarını
göstermektedir:
...
### Confidential Transfer Mint Durumu
...
- **auto_approve_new_accounts**: True olarak ayarlandığında, kullanıcılar
  varsayılan olarak gizli transferleri etkinleştirilmiş token hesapları
  oluşturabilirler. False olduğunda, her yeni token hesabının gizli transferler
  için kullanılabilmesi için yetkilinin onaylaması gerekir.
...
Confidential Transfer etkin bir mint oluşturmak, tek bir işlemde üç talimat
gerektirir:

1. **Mint Hesabı Oluşturma**: Mint hesabını oluşturmak için System Program'ın
   _rs`CreateAccount`_ talimatını çağırın.

2. **Confidential Transfer Uzantısını Başlatma**: Mint için
   _rs`ConfidentialTransferMint`_ durumunu yapılandırmak için Token Extensions
   Program'ın
...
aşağıdaki örnekte gösterildiği gibi, tek bir fonksiyon çağrısında üç talimatın
tümünü içeren bir işlem oluşturup gönderen bir `create_mint` metodu sağlar.

### Örnek Kod

Aşağıdaki kod, Confidential Transfer uzantısına sahip bir mint oluşturmayı
göstermektedir.
...
<Callout type="info">
  Bu yazının yazıldığı tarihte, Confidential Transfers varsayılan yerel
  validator'da etkinleştirilmemiştir. Örnek kodu çalıştırmak için mainnet Token
  Extensions Program'ı klonlamanız gerekir.
</Callout>
