...
description: Gizli Transfer uzantısı ile token mint oluşturmayı öğrenin.
---

## Gizli Transfer uzantısı ile mint nasıl oluşturulur

Gizli Transfer uzantısı, mint account'a ekstra durum ekleyerek özel token
transferlerini mümkün kılar. Bu bölüm, bu uzantının etkinleştirildiği bir token
mint oluşturmayı açıklamaktadır.

Aşağıdaki diyagram, Gizli Transfer uzantısı ile mint oluşturma adımlarını
göstermektedir:
...
- **auto_approve_new_accounts**: True olarak ayarlandığında, kullanıcılar
  varsayılan olarak gizli transferleri etkinleştirilmiş token account'lar
  oluşturabilirler. False olduğunda, her yeni token account gizli transferler
  için kullanılmadan önce authority tarafından onaylanmalıdır.
...
1. **Mint Account Oluşturma**: Mint account oluşturmak için System Program'ın
   _rs`CreateAccount`_ talimatını çağırın.

2. **Gizli Transfer Uzantısını Başlatma**: Mint için
   _rs`ConfidentialTransferMint`_ durumunu yapılandırmak üzere Token Extensions
   Program'ın
...
<Callout type="info">
  ...
  etkinleştirilmemiştir. Örnek kodu çalıştırmak için mainnet Token Extensions
...
---CODE-PLACEHOLDER-c9c0c1c9c0c1c9c0c1c9c0c1c9c0c1c9---
