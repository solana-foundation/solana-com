...
1. İstemci tarafında
   ...
   **Eşitlik Kanıtı
   ...
   Çekim sonrası kalan kullanılabilir bakiye şifreli metninin, ilgili
   [Pedersen taahhüdüne](https://en.wikipedia.org/wiki/Commitment_scheme)
   uyduğunu doğrular, böylece hesabın yeni kullanılabilir bakiyesinin doğru
   şekilde `remaining_balance = current_balance - withdraw_amount` olarak
   hesaplandığını garanti eder.
   ...
   **Aralık Kanıtı
   ...
   Çekim sonrası kalan kullanılabilir bakiyenin negatif olmadığını ve belirli
...
`spl_token_client` crate'i aşağıdaki metotları sağlar:
...
Aşağıdaki örnek, gizli kullanılabilir bakiyeden açık bakiyeye token çekmeyi
göstermektedir.
