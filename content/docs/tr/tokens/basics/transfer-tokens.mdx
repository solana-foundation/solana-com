...
Token transferi, aynı mint'i paylaşan bir token hesabından diğer bir token
hesabına token taşımayı içerir. Token transferi, bir token programında
...
talimatını çağırdığınızda gerçekleşir. Kaynak token hesabının sahibi (yetkili)
olarak belirtilen adres, hesaptan token transfer edebilir.
