...
Token account, Solana'nın Token Programlarında belirli bir token'ın (mint) bir
...
1. Belirli bir mint (token hesabının tuttuğu token türü)
...
- Circle, `7VHUFJHWu2CuExkJcJrzhQPJ2oygupTWkL2A2For4BmE` adresinde sahip
  yetkisine sahiptir ve bu tokenleri transfer edebilir
...
1. Token account "sahibi" - Bu, token account'ın verilerinde Token Programı
...
2. Program "owner" - Bu, Solana'daki hesap verilerinin sahibi olan programı
   ifade eder. Token hesapları için bu, her zaman ya Token Program ya da Token
   Extension Program'dır ve temel Solana
...
Token hesaplarıyla çalışırken, "owner" genellikle hesabın sahibi olan programı
değil, tokenleri harcama yetkisine sahip olan otoriteyi ifade eder.
...
hesabıdır. Associated token account'u, bir kullanıcının belirli bir token'ın
(mint) birimlerini tutmak için kullandığı varsayılan token hesabı olarak
düşünebilirsiniz.

<Callout type="info">
  "Associated token accounts" terimi yalnızca Associated Token Program'ın
  oluşturduğu token hesapları için geçerlidir.
</Callout>

Associated token account'lar, herhangi bir mint için bir kullanıcının token
hesabını belirleyici bir şekilde bulmanın yolunu sağlar. Türetmenin uygulamasını
...
yaklaşım, herhangi bir token mint için bir kullanıcının token hesabını bulma
işlemini basitleştirir ve token hesap adreslerini ayrıca takip etme ihtiyacını
ortadan kaldırır.

<Callout type="info">
  Associated Token Program, belirleyici adreslerle (PDA'lar) token hesapları
  ...
  belirleyici bir adreste (PDA olan) token hesapları oluşturmak için
  standartlaştırılmış bir yol sunar.
</Callout>

## Token Account Nasıl Oluşturulur

Token Account oluşturmak için
...
Token account oluşturmak için işlem iki talimat gerektirir:

1. Token account için alan oluşturmak ve ayırmak için System Program'ı çağırın
   ve sahipliği Token Program'a aktarın.
2. Token account verilerini başlatmak için Token Program'ı çağırın.
...
Associated token account oluşturmak için talimat, token account oluşturmak için
System Program'ı ve token account verilerini başlatmak için Token Program'ı
