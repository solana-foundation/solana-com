...
description:
  Solana CLI kullanarak özel programları Solana blockchain'ine yükleme.
---
...
yeterli bakiye olduğunu kontrol edin:
...
Ayrıca, her programın bir program hesabı ve program kimliği (ID) vardır, bu da
program hesabının adresidir. Aşağıdaki komut, program hesabı için bir anahtar
çifti (keypair) oluşturur:
...
Parametre `program-id` olarak adlandırılır, ancak bir anahtar çiftinin dosya
yolunu bekler:
...
Aynı program hesabına farklı bir yürütülebilir dosya yüklemek, mevcut dosyanın
üzerine yazacak/değiştirecektir. Ancak, yeniden dağıtımlar için, yükseltme
yetkisi keypair'i imzalayan olduğundan, tüm keypair değil, yalnızca program
ID'si (program keypair'inin pubkey'i) gereklidir.
...
`deploy` için yeni bir çağrıya sağlayarak başarısız bir dağıtıma devam
edebilirsiniz.
...
Programların üzerine yazmak yerine, sonlandırılmış programların bağlantılı bir
listesini oluşturarak kullanıcılara hangi program sürümünü kullanmak istedikleri
konusunda seçenek sunmak da mümkündür:
...
hesapları ve bu hesaplarda kilitli fonlar geri alınabilir. Program hesabı,
program kimliği ve özellikle program hesabında kilitli fonlarla birlikte sıkışıp
kalır.
...
çalışmak hata ile sonuçlanacaktır. Bir programı kapattıktan sonra yeniden
dağıtmanız gerekiyorsa, yeni bir program keypair dosyası oluşturmanız gerekir.
...
program ID'si aynı kalır. Ayrıca, tek bir yetki birden fazla program hesabını
kontrol edebilir.
...
Açık bir yetki, çevrimdışı imzalama ve çoklu varlık tarafından yönetilen
...
Çalıştırılabilir dosyayı doğrudan program hesabına yüklemek yerine, önce bir
hazırlık buffer hesabına yüklenebilir ve ardından ikinci bir adımda (asıl
...
yeniden dağıtımdan önce yüklenen çalıştırılabilir dosyayı kabul etme veya
reddetme seçeneği sunulur.

Buffer hesaplarının kullanılmasının, yükleme işlemi sırasında gereken fonları
kabaca iki katına çıkardığını unutmayın, çünkü iki hesap aynı anda birer
...
Buffer hesabı farklı yüklemeler için yeniden kullanılabilir ve herhangi bir
...
yapıştır. `blockhash` yaklaşık 60 saniye sonra sona erer. Eğer zamanında
