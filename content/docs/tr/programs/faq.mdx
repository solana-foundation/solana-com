...
bayt kodunun bir varyasyonunu içerecek şekilde derlenir.
...
Bu program hatası, talimatı deserialize ederken oluşabilir, iletilen yapının
talimata tam olarak uyduğunu kontrol edin. Alanlar arasında bazı dolgu olabilir.
...
beklendiği halde imzalanmadığında bu hata döndürülür.

Bir programın uygulaması, imzalı bir program adresi gerektiren bir
[çapraz program çağrısı](/docs/core/cpi) gerçekleştirirken de bu hataya neden
olabilir, ancak `invoke_signed` için iletilen imzalayıcı tohumları, program
adresini oluşturmak için kullanılan imzalayıcı tohumlarıyla eşleşmez
...
eden işlevselliği içerebilmesidir. Program çalışma zamanında yığın boyutunu
ihlal ederse, bir `AccessViolation` hatası bildirilecektir.
...
yığını kullanılır. Bu yığın `free` veya `realloc` desteklemez.
...
genellikle programın giriş parametrelerinin yanlış seri hale getirilmesinden
kaynaklanan bir `AccessViolation` hatasıdır.
...
talimatlarının talimat verisi olarak gönderir; yükleyici de bu verileri
programın hesap verilerine yazar. Tüm parçalar alındıktan sonra, istemci
yükleyiciye bir
[`Finalize`](https://github.com/solana-labs/solana/blob/bc7133d7526a041d1aaee807b80922baa89b6f90/sdk/program/src/loader_instruction.rs#L30)
talimatı gönderir, yükleyici daha sonra SBF verilerinin geçerli olduğunu
doğrular ve program hesabını _executable_ olarak işaretler. Program hesabı
...
üzerinde bunu seri durumdan çıkarmaktan sorumludur. Kullanımdan kaldırılan
yükleyici ile mevcut yükleyici arasındaki değişikliklerden biri, giriş
parametrelerinin, çeşitli parametrelerin hizalanmış bayt dizisi içinde
hizalanmış ofsetlere düşmesini sağlayacak şekilde serileştirilmesidir. Bu, seri
durumdan çıkarma uygulamalarının bayt dizisine doğrudan başvurmasına ve programa
hizalanmış işaretçiler sağlamasına olanak tanır.

- [Rust program parametresi seri durumdan çıkarma](/docs/programs/lang-rust#parameter-deserialization)

En son yükleyici, program giriş parametrelerini aşağıdaki gibi serileştirir (tüm
kodlama küçük endian'dır):

- 8 bayt işaretsiz hesap sayısı
- Her hesap için
  - Bunun yinelenen bir hesap olup olmadığını belirten 1 bayt, yinelenen değilse
