...
odaklanır, bu yaklaşım genellikle "native Rust" programları yazmak olarak
adlandırılır.
...
Sonra, aşağıdaki kod parçasını `Cargo.toml` dosyasına ekleyin. Bu yapılandırmayı
...
1. Bir `.so` dosyası (örn., `hello_world.so`): Bu, "smart contract" olarak ağa
...
Şimdi, programı `solana-program-test` crate'ini kullanarak test edin. Aşağıdaki
...
Aşağıdaki testi, program kodunun altında `src/lib.rs` dosyasına ekleyin. Bu,
...
Test validator'ı çalışırken, programı yerel validator'a dağıtmak için ayrı bir
...
kimliğini kendi programınızınkiyle değiştirin.
...
Programı `solana program deploy` komutunu kullanarak yeniden dağıtın.
...
Program kapatıldıktan sonra, program kimliğinin tekrar kullanılamayacağını
