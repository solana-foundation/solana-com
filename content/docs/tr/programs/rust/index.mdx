...
## !!steps solana-program bağımlılığını ekle
...
Şimdi, aşağıdaki kod parçasını `Cargo.toml` dosyasına ekleyin. Bu yapılandırmayı
eklemezseniz, programı derlediğinizde `target/deploy` dizini
oluşturulmayacaktır.
...
Program kodunun altında, `src/lib.rs` dosyasına aşağıdaki testi ekleyin. Bu,
merhaba dünya programını çağıran bir test modülüdür.
...
Testi `cargo test-sbf` komutunu kullanarak çalıştırın. Program günlüğü "Hello,
world!" mesajını gösterecektir.
...
İlk olarak, Solana CLI'yi yerel Solana kümesini kullanacak şekilde yapılandırın.
...
Test validator çalışırken, programı yerel validator'a dağıtmak için ayrı bir
...
Komut dosyasını çalıştırmadan önce, yukarıdaki kod parçasındaki program ID'sini
kendi programınızın ID'si ile değiştirin.
...
Program günlüğünde "Hello, world!" ifadesini görmek için işlem imzasını
[Solana Explorer](https://explorer.solana.com/?cluster=custom&customUrl=http%3A%2F%2Flocalhost%3A8899)
(yerel küme) üzerinde inceleyebilirsiniz.

## !!steps Programı güncelleyin
...
## !!steps Programı yeniden dağıtın
