...
- Κάθε λογαριασμός έχει έναν
  ...
  τα δεδομένα του ή να αφαιρέσει το υπόλοιπο lamport. Αλλά οποιοσδήποτε μπορεί
  να αυξήσει το υπόλοιπο.
...
λειτουργεί ως "μοναδικό αναγνωριστικό" για κάθε εγγραφή στον πίνακα "Accounts".
...
[Program Derived Addresses](/docs/core/pda) (PDAs). Τα PDAs είναι ειδικές
διευθύνσεις που μπορείτε να παράγετε με προκαθορισμένο τρόπο από ένα
...
- `data`: Ένας πίνακας byte που αποθηκεύει αυθαίρετα δεδομένα για έναν
  ...
  συμβόλαια), αυτό περιέχει τον εκτελέσιμο κώδικα του προγράμματος. Το πεδίο
  δεδομένων συνήθως ονομάζεται "δεδομένα λογαριασμού".
- `executable`: Αυτή η σημαία δείχνει αν ένας λογαριασμός είναι πρόγραμμα.
- `lamports`: Το υπόλοιπο του λογαριασμού σε lamport, τη μικρότερη μονάδα του
  SOL (1 SOL = 1 δισεκατομμύριο lamport).
- `owner`: Το αναγνωριστικό προγράμματος (δημόσιο κλειδί) του προγράμματος που
  ...
  τα δεδομένα του λογαριασμού ή να αφαιρέσει το υπόλοιπο των lamport.
- `rent_epoch`: Ένα παλαιό πεδίο από την εποχή που το Solana είχε έναν μηχανισμό
  που αφαιρούσε περιοδικά lamport από τους λογαριασμούς. Ενώ αυτό το πεδίο
  εξακολουθεί να υπάρχει στον τύπο Account, δεν χρησιμοποιείται πλέον αφού η
  συλλογή rent καταργήθηκε.
...
## Πρόγραμμα συστήματος

Από προεπιλογή, όλοι οι νέοι λογαριασμοί ανήκουν στο
[Πρόγραμμα Συστήματος](https://github.com/anza-xyz/agave/tree/v2.1.13/programs/system/src).
Το Πρόγραμμα Συστήματος κάνει μερικά βασικά πράγματα:

- [Δημιουργία νέου λογαριασμού](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L146):
  Μόνο το Πρόγραμμα Συστήματος μπορεί να δημιουργήσει νέους λογαριασμούς.
...
- [Μεταφορά / Ανάθεση ιδιοκτησίας προγράμματος](https://github.com/anza-xyz/agave/blob/v2.1.13/programs/system/src/system_processor.rs#L113):
  Μόλις το Πρόγραμμα Συστήματος δημιουργήσει έναν λογαριασμό, μπορεί να
  επανεκχωρήσει τον καθορισμένο ιδιοκτήτη προγράμματος σε διαφορετικό λογαριασμό
  προγράμματος. Έτσι τα προσαρμοσμένα προγράμματα αποκτούν την ιδιοκτησία νέων
  λογαριασμών που δημιουργούνται από το Πρόγραμμα Συστήματος.

Όλοι οι λογαριασμοί "πορτοφολιού" στο Solana είναι απλώς λογαριασμοί που ανήκουν
στο Πρόγραμμα Συστήματος. Το υπόλοιπο lamport σε αυτούς τους λογαριασμούς
δείχνει την ποσότητα SOL που κατέχει το πορτοφόλι. Μόνο οι λογαριασμοί που
ανήκουν στο Πρόγραμμα Συστήματος μπορούν να πληρώσουν τέλη συναλλαγών.
...
Οι λογαριασμοί προγραμμάτων ανήκουν σε ένα
...
προγράμματος. Οι λογαριασμοί προγραμμάτων ανήκουν σε ένα
...
Ο loader-v3 έχει έναν ειδικό τύπο λογαριασμού για την προσωρινή σταδιακή
μεταφόρτωση ενός προγράμματος κατά την ανάπτυξη ή επανανάπτυξη/αναβαθμίσεις.
...
έναν λογαριασμό, μπορεί στη συνέχεια να μεταφέρει ή να αναθέσει την ιδιοκτησία
του νέου λογαριασμού σε άλλο πρόγραμμα.
