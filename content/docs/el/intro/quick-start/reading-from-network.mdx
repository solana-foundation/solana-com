...
Το πεδίο `data` περιέχει τα δεδομένα του λογαριασμού που αποθηκεύονται ως bytes.
...
Για πορτοφόλια και λογαριασμούς που αποθηκεύουν την κατάσταση προγράμματος, αυτό
το πεδίο είναι `false`.
...
Τα lamports είναι η μικρότερη μονάδα του SOL. 1 SOL = 1.000.000.000 lamports.
...
οι λογαριασμοί χρεώνονταν "rent" (σε lamports) για να διατηρούν τα δεδομένα τους
στο δίκτυο.
...
πεδίο στον ίδιο τον τύπο `Account`, αλλά συμπεριλαμβάνεται στην απάντηση.
...
κώδικα του προγράμματος. Αντίθετα, οι λογαριασμοί πορτοφολιού έχουν κενό πεδίο
δεδομένων.
...
Για το Token Program, το `owner` είναι το πρόγραμμα BPFLoader2.
...
εξουσιοδοτημένοι να δημιουργούν ή να παγώνουν tokens. Η διεύθυνση του mint
account προσδιορίζει μοναδικά ένα token στο δίκτυο Solana.
...
`data` ενός λογαριασμού mint σε τύπο δεδομένων
[Mint](https://github.com/solana-program/token/blob/program%40v8.0.0/program/src/state.rs#L16-L30).
...
Αυτός είναι ο μόνος λογαριασμός που μπορεί να δημιουργήσει νέες μονάδες του
token.
