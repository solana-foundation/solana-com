...
Ένας mint account είναι ένας τύπος λογαριασμού στα Token Programs της Solana που
αντιπροσωπεύει μοναδικά ένα token στο δίκτυο και αποθηκεύει καθολικά
μεταδεδομένα σχετικά με το token.
...
Κάθε token στη Solana υπάρχει ως mint account όπου η διεύθυνση του mint account
είναι το μοναδικό αναγνωριστικό του στο δίκτυο.

Για παράδειγμα, το USD Coin (USDC) ψηφιακό δολάριο στη Solana έχει τη διεύθυνση
mint `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`. Αυτή η διεύθυνση mint
λειτουργεί ως μοναδικό αναγνωριστικό του USD Coin σε ολόκληρο το οικοσύστημα της
Solana. Μπορείτε να δείτε λεπτομέρειες σχετικά με αυτόν τον mint account στον
...
Για να δημιουργήσετε έναν Mint Account, καλέστε την εντολή
...
[εδώ](https://github.com/solana-program/token-2022/blob/efd0c957fefbd79882d77df5fb2dac88c001249c/program/src/processor.rs#L79).

Η συναλλαγή για τη δημιουργία ενός mint account χρειάζεται δύο εντολές:

1. Καλέστε το System Program για να δημιουργήσετε και να διαθέσετε χώρο για έναν
   mint account και να μεταφέρετε την ιδιοκτησία στο Token Program.
2. Καλέστε το Token Program για να αρχικοποιήσετε τα δεδομένα του mint account.
