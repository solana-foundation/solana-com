...
- **pending_balance_lo**: Τα κρυπτογραφημένα κατώτερα 16 bits του εκκρεμούς
  υπολοίπου. Το υπόλοιπο χωρίζεται σε υψηλό και χαμηλό μέρος για αποτελεσματική
  αποκρυπτογράφηση.

- **pending_balance_hi**: Τα κρυπτογραφημένα ανώτερα 48 bits του εκκρεμούς
  υπολοίπου. Το υπόλοιπο χωρίζεται σε υψηλό και χαμηλό μέρος για αποτελεσματική
  αποκρυπτογράφηση.

- **available_balance**: Το κρυπτογραφημένο διαθέσιμο υπόλοιπο για μεταφορές.
...
- **expected_pending_balance_credit_counter**: Η τιμή
  ...
  instruction data την τελευταία φορά που εκτελέστηκε η εντολή
  _rs`ApplyPendingBalance`_.

- **actual_pending_balance_credit_counter**: Η τιμή
  `pending_balance_credit_counter` στο token account τη στιγμή που εκτελέστηκε η
  τελευταία εντολή _rs`ApplyPendingBalance`_.
...
μπορούσε να στέλνει επανειλημμένα τόκενς σε ένα token account, εμποδίζοντας τη
δυνατότητα του κατόχου του token account να μεταφέρει τόκενς. Ο κάτοχος του
token account δεν θα μπορούσε να μεταφέρει τόκενς επειδή το κρυπτογραφημένο
υπόλοιπο θα άλλαζε μεταξύ της υποβολής της συναλλαγής και της επεξεργασίας της,
με αποτέλεσμα την αποτυχία της συναλλαγής.

Όλες οι καταθέσεις και τα ποσά μεταφοράς προστίθενται αρχικά στο εκκρεμές
υπόλοιπο. Οι κάτοχοι token account πρέπει να χρησιμοποιούν την εντολή
...
1. Ο πελάτης αναζητά τα τρέχοντα εκκρεμή και διαθέσιμα υπόλοιπα, κρυπτογραφεί το
   ...
   το κλειδί AES του κατόχου του token account.

2. Οι αναμενόμενοι και πραγματικοί μετρητές πιστώσεων εκκρεμούς υπολοίπου
...
αναμενόμενων/πραγματικών απαιτούν από τον πελάτη να αναζητήσει πρόσφατες οδηγίες
κατάθεσης/μεταφοράς που αντιστοιχούν στη διαφορά των μετρητών για να υπολογίσει
το σωστό υπόλοιπο.

**Διαδικασία συμφωνίας υπολοίπου**

Όταν οι αναμενόμενοι και οι πραγματικοί μετρητές εκκρεμούς υπολοίπου διαφέρουν,
...
2. Ανακτήστε τις πιο πρόσφατες συναλλαγές συμπεριλαμβανομένων των οδηγιών
   κατάθεσης και μεταφοράς μέχρι τη διαφορά του μετρητή (πραγματικός -
   αναμενόμενος):
   - Προσθέστε τα δημόσια ποσά από τις οδηγίες κατάθεσης
...
Η οδηγία _rs`ConfigureAccount`_ απαιτεί τη δημιουργία κλειδιών κρυπτογράφησης
...
<Callout type="info">
  ...
  ενεργοποιημένες στον προεπιλεγμένο τοπικό επικυρωτή. Πρέπει να κλωνοποιήσετε
