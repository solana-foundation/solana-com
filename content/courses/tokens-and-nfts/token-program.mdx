...
- Creating Token Mints and Token Accounts requires allocating **rent** in SOL.
  ...
  [Token Extensions Program](/developers/courses/token-extensions/close-mint)
...
![USDC Token Mint](/assets/courses/unboxed/token-program-usdc-mint.png)
...
![ATAs are PDAs](/assets/courses/unboxed/atas-are-pdas.svg)
...
```typescript title="create-token-mint.ts"
...
```typescript title="create-token-metadata.ts"
...
```typescript title="create-token-account.ts"
...
```typescript title="transfer-tokens.ts"
import "dotenv/config";
import {
  getExplorerLink,
  getKeypairFromEnvironment,
...
![Token Program Challenge Frontend](/assets/courses/unboxed/token-program-frontend.png)
