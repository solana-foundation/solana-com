...
![Solana stores programs and data in separate accounts](/assets/courses/unboxed/pdas-global-state.svg)

### Basic scaffolding

We'll start by using the same packages and `.env` file we made earlier in
[Intro to Writing Data](/developers/courses/intro-to-solana/intro-to-writing-data).
...
![Solana Explorer with logs from calling the Ping program](/assets/courses/unboxed/solana-explorer-ping-result.png)
