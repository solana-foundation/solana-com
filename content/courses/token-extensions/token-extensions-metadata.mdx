...
The `metadata` extension must be used in conjunction with
...
The `metadata` extension works directly with the
...
[metadata in a different format while maintaining compatibility with the Interface standards](/developers/guides/token-extensions/metadata-pointer#metadata-interface-instructions).
