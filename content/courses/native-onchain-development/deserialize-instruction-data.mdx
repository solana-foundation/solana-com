...
The
[Hello World lesson](/developers/courses/native-onchain-development/hello-world-program)
...
is submitted. Build and deploy your program from Solana Playground just like in
the last lesson. If you haven't changed the program ID since going through the
last lesson, it will automatically deploy to the same ID. If you'd like it to
have a seperate address, you can generate a new program ID from the playground
before deploying.
...
built in the
[Serialize Custom Instruction Data lesson](/developers/courses/native-onchain-development/serialize-instruction-data-frontend).
In both cases, ensure you copy and paste the program ID for your program into
the appropriate area of the source code to confirm you're testing the right
program.
