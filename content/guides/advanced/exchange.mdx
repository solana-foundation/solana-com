...
The `preTokenBalance` and `postTokenBalance` fields from the transaction's
...
tokens for the first time), it won't appear in the `preTokenBalance` array since
it didn't exist before the transaction. In this scenario, you should treat the
initial balance as zero when calculating deposit amounts. The newly created
account will only appear in the `postTokenBalance` array with its final balance
...
Note that the `preTokenBalance` array does not include the recipient token
account since it didn't exist before the transaction. The recipient token
account only appears in the `postTokenBalance` array with its final balance
after the transaction completes.
...
To accurately track token deposits, you must compare the `preTokenBalance` and
...
- If the `owner` field of the `preTokenBalance` and `postTokenBalance` fields
...
- If the token account ownership changes (different `owner` field between
  `preTokenBalance` and `postTokenBalance`), and the new owner in
  `postTokenBalance` matches your exchange's expected `owner` address, then
  consider the entire balance shown in the `amount` field of `postTokenBalance`
...
`owner` field in the `postTokenBalance` matches a wallet address that your
...
Check whether the `owner` field matches between the `preTokenBalance` and
`postTokenBalance` to determine if the token account ownership has changed
during the transaction.

### !amount

Calculate token balance changes by using the `amount` field in `preTokenBalance`
and `postTokenBalance` for each token account.
