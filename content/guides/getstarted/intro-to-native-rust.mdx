...
In this example, we use a [Cross Program Invocation](/docs/core/cpi) to
invoke the System Program, creating a new account with the executing program as
the `owner`. As part of the
[Solana Account Model](/docs/core/accounts#accountinfo), only the program
...
> [Rust](/docs/clients/rust) and
> [Javascript/Typescript](/docs/clients/rust) under the Solana Clients of the
> documentation.

Below, we'll walk through an example demonstrating how to invoke the
`initialize` instruction from the program above.

```ts title="native.test.ts"
describe("Test", () => {
  it("Initialize", async () => {
